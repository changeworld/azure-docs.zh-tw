...
description: 描述 Azure 事件方格中的自訂主題。
ms.topic: conceptual
ms.date: 07/07/2020
...
**自訂主題** 是應用程式和協力廠商主題。 當您建立或獲指派可存取自訂主題時，您會在訂用帳戶中看見該自訂主題。 
...
如需事件架構的詳細總覽，請參閱 [Azure 事件方格事件架構](event-schema.md)。 若為自訂主題，事件發行者會決定 **資料** 物件。 最高層級的資料應包含與標準資源定義事件相同的欄位。
