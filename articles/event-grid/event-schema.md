...
| 屬性 | 類型 | 必要 | 描述 |
| -------- | ---- | -------- | ----------- |
| 主題 | 字串 | 否，但如果包含的話，必須完全符合事件方格主題 Azure Resource Manager 識別碼。 如果未包含，事件方格將會戳記至事件。 | 事件來源的完整資源路徑。 此欄位不可寫入。 Event Grid 提供此值。 |
...
| dataVersion | 字串 | 否，但會加上空白值的戳記。 | 資料物件的結構描述版本。 發行者會定義結構描述版本。 |
| metadataVersion | 字串 | 非必要，但如果包含的話，必須完全符合事件方格架構 `metadataVersion` (目前只有 `1`) 。 如果未包含，事件方格將會戳記至事件。 | 事件中繼資料的結構描述版本。 「事件方格」會定義最上層屬性的結構描述。 「事件方格」提供此值。 |
...
* [ (管理作業的資源群組) ](event-schema-resource-groups.md)
