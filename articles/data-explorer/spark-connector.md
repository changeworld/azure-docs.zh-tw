...
> [!NOTE]
> 此步驟是選用的。 如果您使用的是預先建立的程式庫，請移至[Spark 叢集設定](#spark-cluster-setup)。

### <a name="build-prerequisites"></a>組建必要條件

1. 安裝 [相依性[]](https://github.com/Azure/azure-kusto-spark#dependencies)中列出的程式庫，包括下列[Kusto JAVA SDK](/azure/kusto/api/java/kusto-java-client-library)程式庫：
...
* [裝置驗證](https://github.com/Azure/azure-kusto-spark/blob/dev/docs/Authentication.md#device-authentication)（適用于非生產案例）
* 存取 Key Vault 資源的[Azure Key Vault](https://github.com/Azure/azure-kusto-spark/blob/dev/docs/Authentication.md#key-vault) ，安裝 keyvault 套件並提供應用程式認證。
...
1. 讀取[少量資料](/azure/kusto/concepts/querylimits)時，請定義資料查詢：
