---
title: 在 Azure 資料資源管理器中管理資料庫許可權
...
1. 使用 `.add` 命令將主體指派給角色：`.add database databasename rolename ('aaduser | aadgroup=user@domain.com')`。 若要將使用者加入到資料庫使用者角色，請執行下列命令，取代您的資料庫名稱和使用者。
    ...
    有關 Azure 活動目錄和 Kusto 授權模型的示例，請參閱[原則和標識提供程式](https://docs.microsoft.com/azure/kusto/management/access-control/principals-and-identity-providers)
