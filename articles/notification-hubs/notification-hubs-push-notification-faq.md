...
2. 使用下列其中一個選項，讓次要通知中樞與主要通知中樞保持同步：

   * 使用應用程式後端，同時在這兩個通知中樞內同時建立及更新安裝。 安裝可讓您指定自己的唯一裝置識別碼，使其更適合用於複寫案例。 如需詳細資訊，請參閱此 [範例程式碼](https://github.com/Azure/azure-notificationhubs-dotnet/tree/main/Samples/RedundantHubSample)。
   * 使用會從主要通知中樞定期取得註冊傾印作為備份的應用程式後端。 然後就可在次要通知中樞執行大量插入作業。

次要通知中樞最終可能會有過期的安裝/註冊。 當推送至過期的控制碼時，通知中樞會根據從 PNS 伺服器收到的回應，自動清除相關聯的安裝/註冊記錄。 若要從次要通知中樞清除過期的記錄，請新增自訂邏輯，以處理每個傳送的意見反應。 然後，在次要通知中樞內到期安裝/註冊。
