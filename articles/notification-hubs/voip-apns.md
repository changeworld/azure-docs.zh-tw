...
- 傳送 VOIP 通知需要將 `apns-topic` 標頭設定為應用程式套件組合識別碼 + `.voip` 尾碼。 例如，針對具有配套識別碼的範例應用程式， `com.microsoft.nhubsample` `apns-topic` 標頭應該設定為`com.microsoft.nhubsample.voip.`

   這個方法不適用於 Azure 通知中樞，因為應用程式的套件組合識別碼必須設定為中樞的 APNS 認證的一部分，而且無法變更此值。 此外，通知中樞不允許 `apns-topic` 在執行時間覆寫標頭的值。
...
- 傳送 VOIP 通知需要將 `apns-push-type` 標頭設定為值 `voip` 。
   ...
   若要傳送 VOIP 通知，您必須為標頭指定明確的值 `apns-push-type` 。
