---
title: 使用 Azure 通知中樞和 iOS SDK 3.0.0 preview 1 版將推播通知傳送至 iOS
description: 在本教學課程中，您將了解如何使用 Azure 通知中樞和 Apple Push Notification Service 將推播通知傳送至 iOS 裝置 (3.0.0-preview1 版)。
author: sethmanheim
ms.author: sethm
ms.date: 06/19/2020
ms.topic: tutorial
...
ms.locfileid: "96001332"
---
# <a name="tutorial-send-push-notifications-to-ios-apps-using-azure-notification-hubs-version-300-preview1"></a>教學課程：使用 Azure 通知中樞 (3.0.0-preview1 版) 將推播通知傳送至 iOS 應用程式
...
5. 新增 Azure 通知中樞 SDK 模組。
   ...
   - 透過將二進位檔複製到您的專案來進行整合：
      ...
      您可以藉由將二進位檔複製到專案中來進行整合，如下所示：

        - 下載以 zip 檔案形式提供的 [Azure 通知中樞 SDK](https://github.com/Azure/azure-notificationhubs-iOS/releases/) 架構，並將其解壓縮。
...
10. 在專案的 **AppDelegate.h** 檔案中，新增下列 `import` 陳述式：

    ```objc
    #import "Constants.h"
    ```

11. 在相同的 **AppDelegate.m** 檔案中，使用下列程式碼取代 `didFinishLaunchingWithOptions` 之後的所有程式碼：
...
2. 此外，在 **ViewController.h** 中的 `@interface` 宣告後面，新增下列屬性宣告：
