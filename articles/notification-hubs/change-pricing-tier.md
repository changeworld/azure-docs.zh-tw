...
在 Azure 通知中樞中， *通知中樞* 是最小的資源/實體。 它通常會對應至一個應用程式，並且可以針對每個平臺通知系統 (PNS) 我們為應用程式支援的每個憑證保存一個憑證。 應用程式可以是混合式或原生和跨平臺應用程式。

*命名空間*是通知中樞的集合。 每個命名空間通常由相關和特定用途的中樞所組成。 例如，您可以將三個不同的命名空間分別用於開發、測試和生產用途。

您可以將命名空間與「 **免費**」、「 **基本**」或「 **標準** 」定價層建立關聯。 針對任何命名空間，您可以使用符合您需求的層級。 以下幾節將說明如何變更通知中樞命名空間的定價層。

## <a name="use-azure-portal"></a>使用 Azure 入口網站

使用 Azure 入口網站時，您可以在 [命名空間] 頁面或在 [中樞] 頁面上，變更命名空間的定價層。 在中樞頁面上變更定價層時，您實際上會在命名空間層級進行變更。 這會變更命名空間及其下所有中樞的定價層。

### <a name="change-tier-on-the-namespace-page"></a>在命名空間頁面上變更層級

下列程式說明如何在命名空間頁面上變更命名空間的定價層。 當您變更命名空間的層級時，它會套用至該命名空間中的所有中樞。

1. 登入 [Azure 入口網站](https://portal.azure.com)。
2. 選取左側功能表上的 [ **所有服務** ]。
3. 選取 [物聯網]**** 區段中的 [通知中樞命名空間]****。 如果您選取文字旁的星號 (`*`) ，它就會加入 [我的最 **愛]** 的左側導覽列中。 這可協助您下一次更快速地存取命名空間頁面。 將它新增至我的最愛之後，請選取 [**通知中樞命名空間** **]**。
...
7. 變更定價層，然後按一下 [ **選取** ] 按鈕。
8. 您可以在 **警示**中看到 [階層變更] 動作的狀態。
9. 切換至 [概觀]**** 頁面。 確認 [基本資訊]**** 區段中的 [定價層]**** 欄位顯示新的層級。
10. 此為選用步驟。 選取命名空間中的任何中樞。 確認您看到與 [基本資訊]**** 區段中相同的定價層。 您應該會看到命名空間中所有中樞的相同定價層。

### <a name="change-tier-on-the-hub-page"></a>在中樞頁面上變更層級

遵循下列步驟來變更中樞頁面上命名空間的定價層。 雖然您從 [中樞] 頁面開始執行這些步驟，但實際上會變更命名空間和命名空間中所有中樞的定價層：
