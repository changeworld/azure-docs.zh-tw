...
---
# <a name="send-secure-push-notifications-with-azure-notification-hubs"></a>使用 Azure 通知中樞傳送安全的推播通知
...
請務必注意在上述流程 (與本教學課程) 中，假設使用者登入後，裝置會將驗證權杖儲存在本機儲存體中。 這種方法可保證順暢的體驗，因為裝置可以使用此權杖來取得通知的安全承載。 如果您的應用程式未將驗證權杖儲存在裝置上，或如果這些權杖可以過期，裝置應用程式會在收到推播通知時顯示一般通知，以提示使用者啟動應用程式。 應用程式會接著驗證使用者，並顯示通知裝載。
...
既然您已將應用程式後端修改為只傳送推播通知的識別碼，您必須變更 Android 應用程式來處理該通知，並回呼後端以取得要顯示的安全訊息。
