...
在壽命延長60天時，一個版本被"*禁用*"。 「停用」表示會從入口網站移除該版本。 您也無法再從 CSCFG 組態檔設定該版本。 現有部署仍可繼續執行。 但新部署和現有部署的程式碼與設定更新將遭到禁止。

在"禁用"後，客體作業系統版本"過期"，並且仍在運行的任何過期版本的安裝都面臨安全和漏洞問題。 通常，過期是分批完成的，因此從禁用到過期的時間段可能有所不同。

將服務配置為手動更新客體作業系統的客戶應確保其服務在受支援的客體作業系統上運行。 如果服務配置為自動更新客體作業系統，則基礎平臺將確保合規性，並將升級到最新的客體作業系統。
