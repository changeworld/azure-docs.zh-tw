...
1. Microsoft 將會支援**至少兩個最新的客體 OS 系列**。 當某個系列遭到淘汰時，客戶有 12 個月的時間 (從官方淘汰日期開始計算) 更新為較新的受支援客體作業系統系列。
...
在60天的存留期內，版本為「*已停用*」。 「停用」表示會從入口網站移除該版本。 您也無法再從 CSCFG 組態檔設定該版本。 現有部署仍可繼續執行。 但新部署和現有部署的程式碼與設定更新將遭到禁止。

在變成「停用」之後，客體作業系統版本「過期」，而任何仍在執行該過期版本的安裝都會暴露于安全性和弱點問題。 一般而言，到期會以批次方式完成，因此從停用到到期的期間可能會有所不同。

設定其服務以手動更新來賓 OS 的客戶，應該確定其服務是在支援的客體作業系統上執行。 如果服務設定為自動更新虛擬作業系統，基礎平臺將確保相容性，並將升級至最新的來賓 OS。
