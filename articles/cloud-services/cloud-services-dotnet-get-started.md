---
title: Azure 雲端服務 (傳統) 和 ASP.NET 入門 |Microsoft Docs
...
ms.locfileid: "98742075"
---
# <a name="get-started-with-azure-cloud-services-classic-and-aspnet"></a>開始使用 Azure 雲端服務 (傳統) 和 ASP.NET
...
![影像顯示 Ad 清單](./media/cloud-services-dotnet-get-started/list.png)
...
本教學課程示範如何在 Azure 雲端服務中執行前端與後端。 替代方式是在 [Azure App Service](../app-service/index.yml) 中執行前端，並且將 [WebJob](https://go.microsoft.com/fwlink/?LinkId=390226) 功能用於後端。 如需使用 WebJob 的教學課程，請參閱 [開始使用 Azure WebJob SDK](https://github.com/Azure/azure-webjobs-sdk/wiki)。 如需如何選擇最符合您的案例之服務的詳細資訊，請參閱 [Azure App Service、雲端服務和虛擬機器比較](/azure/architecture/guide/technology-choices/compute-decision-tree)。
...
![這是 Ad 資料表的影像](./media/cloud-services-dotnet-get-started/adtable.png)
...
![顯示應用程式元件如何互動的圖表。](./media/cloud-services-dotnet-get-started/apparchitecture.png)
...
7. 按 CTRL+F5 執行應用程式。
    ...
    ![Contoso Ads 架構1](./media/cloud-services-dotnet-get-started/home.png)
8. 按一下 [建立廣告]。
9. 輸入部分測試資料，選取要上傳的 *.jpg* 影像，然後按一下 [ **建立**]。

    ![影像顯示建立頁面](./media/cloud-services-dotnet-get-started/create.png)
...
11. 按一下廣告的 [詳細資料]  以查看完整大小的影像。
     ...
     ![詳細資料頁面](./media/cloud-services-dotnet-get-started/details.png)
...
5. 按一下頁面底部的 [新增]  。
    ...
    ![影像顯示新的雲端服務](./media/cloud-services-dotnet-get-started/newcs.png)
...
4. 將 [複寫] 下拉式清單設定為 [本機備援儲存體]。
    ...
    對儲存體帳戶啟用異地複寫時，儲存的內容會複寫至次要資料中心，以便能在主要位置發生重大災害時容錯移轉。 地理區域複寫會引發額外成本。 對於測試和開發帳戶，您通常不會想要付費使用地理區域複寫功能。 如需詳細資訊，請參閱 [建立、管理或刪除儲存體帳戶](../storage/common/storage-account-create.md)。
...
### <a name="configure-the-solution-to-use-your-database-in-azure-sql-database-when-it-runs-in-azure"></a>將解決方案設定為在 Azure 中執行時，于 Azure SQL Database 中使用您的資料庫

Web 專案和背景工作角色專案都有自己的資料庫連接字串，而當應用程式在 Azure 中執行時，每個專案都必須指向 Azure SQL Database 中的資料庫。
...
7. 在 [方案總管] 中，於雲端服務中的 [角色] 下，以滑鼠右鍵按一下 **ContosoAdsWorker**，然後按一下 [屬性]。
    ...
    ![醒目顯示 [屬性] 功能表選項的螢幕擷取畫面。](./media/cloud-services-dotnet-get-started/rolepropertiesworker.png)
8. 按一下 [設定]  索引標籤。
9. 將 [服務組態] 變更為 [雲端]。
...
1. 在 [方案總管] 中，於 **ContosoAdsCloudService** 專案的 [角色] 下的 **ContosoAdsWeb** 上按一下滑鼠右鍵，然後按一下 [屬性]。
    ...
    ![影像顯示角色屬性](./media/cloud-services-dotnet-get-started/roleproperties.png)
2. 按一下 [ **設定** ] 索引標籤。在 [ **服務** 設定] 下拉式清單方塊中，選擇 [ **雲端**]。
...
3. 在精靈的 [設定] 步驟中，按 [下一步]。
    ...
    [ **進階** ] 索引標籤中的預設設定適用於本教學課程。 如需進階索引標籤的詳細資訊，請參閱 [發行 Azure 應用程式精靈](/visualstudio/azure/vs-azure-tools-publish-azure-application-wizard)。
4. 在 [摘要] 步驟中，按一下 [發佈]。
...
3. 在 ContosoAdsCloudService 專案中，於 [角色] 下的 ContosoAdsWeb 上按一下滑鼠右鍵，然後按一下 [屬性]。
    ...
    ![角色屬性影像](./media/cloud-services-dotnet-get-started/roleproperties.png)
...
### <a name="add-code-files"></a>新增程式碼檔案
在本節中，您會將檔案從所下載的方案複製到新方案。 下一節將示範和說明此程式碼的重要部分。

若要將檔案加入至專案或資料夾，請以滑鼠右鍵按一下專案或資料夾，然後按一下 [**加入**  -  **現有專案**]。 選取您需要的檔案，然後按一下 [加入] 。 如果詢問您是否要取代現有的檔案，請按一下 [是] 。
...
然後，它會取得 *映射* blob 容器的參考、建立容器（如果尚未存在），並設定新容器的存取權限。 依預設，新的容器只能允許具有儲存體帳戶認證的用戶端存取 Blob。 網站需要 Blob 處於公用狀態，網站才能使用指向影像 Blob 的 URL 來顯示影像。
...
然後，程式碼會取得 *映射* blob 容器的參考，如稍早在 *Global.asax.cs* 中所見。 在執行該動作時，它會設定適用 Web 應用程式的預設 [重試原則](https://www.asp.net/aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/transient-fault-handling) 。 預設的指數輪詢重試原則可能會導致 web 應用程式在發生暫時性錯誤的重複重試時，停止回應超過一分鐘。 此處指定的重試原則會在每次嘗試後等候 3 秒，最多嘗試 3 次。
...
在每次迴圈的反覆運算之後，如果沒有找到佇列訊息，程式會休息一秒。 如此可避免背景工作角色產生過度的 CPU 時間和儲存體交易成本。 Microsoft 客戶諮詢小組曾經遇過一個案例是，開發人員忘記納入這一點便部署到生產環境，之後便離開去度假。 當他們回來時，他們的監督成本會比休假更高。
...
`RoleEnvironment`當您在 azure 中執行應用程式時，或當您使用 Azure 計算模擬器在本機執行應用程式時，Azure 會提供物件。  如果您在本機執行時遇到此錯誤，請確定您已將 ContosoAdsCloudService 專案設為啟動專案。 這會設定要使用 Azure 計算模擬器執行的專案。
...
根據預設，新的雲端服務專案會使用 Azure 計算模擬器 express 來模擬 Azure 環境。 這是完整計算模擬器的輕量版本，在某些情況下，完整版模擬器能夠正常運作而精簡版則無法。  
