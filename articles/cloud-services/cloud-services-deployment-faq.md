...
有關緩解此類分配失敗的説明，請參閱[雲服務分配失敗：解決方案](cloud-services-allocation-failures.md#solutions)。
...
資料中心中的群集可能具有不同的機器類型配置（例如，A 系列、Av2 系列、D 系列、Dv2 系列、G 系列、H 系列等）。 但並非所有的叢集都一定會有所有種類的 VM。 例如，如果您嘗試新增到雲端服務的 D 系列 VM 已部署在僅限 A 系列的叢集中，就會發生配置失敗。 如果您嘗試變更 VM SKU 大小 (例如，從 A 系列切換至 D 系列)，也會發生這個問題。
...
由於雲服務是一個與 Azure 資源管理器模型不直接相容的經典資源，因此無法將其與 Azure 資源管理器存儲帳戶相關聯。 以下提供一些選項： 
...
- 透過 [Azure 入口網站](https://portal.azure.com)部署。
    ...
    這將在 Azure[門戶](https://portal.azure.com)中工作，因為調用通過允許 Azure 資源管理器和經典資源之間通信的代理/shim。 
