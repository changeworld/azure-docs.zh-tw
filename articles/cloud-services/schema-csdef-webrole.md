---
title: Azure 雲服務定義 WebRole 架構 |微軟文檔
description: Azure Web 角色是為支援ASP.NET、PHP、WCF 和 FastCGI 的 Web 應用程式程式設計定制的。 瞭解 Web 角色的服務定義元素。
ms.custom: ''
ms.date: 04/14/2015
...
|ignoreRoleInstanceStatus|boolean|選擇性。 當這個屬性的值設定為 `true` 時，就會忽略服務的狀態，且負載平衡器不會移除端點。 將此值設定為 `true` 適用於服務的偵錯忙碌執行個體。 預設值是 `false`。 **注：** 即使角色未處於"就緒"狀態，終結點仍可以接收流量。|  
...
| 屬性 | 類型 | 描述 |  
| --------- | ---- | ----------- |  
|assemblyName|字串|必要。 包含進入點之組件的路徑和檔案名稱。 路徑相對於資料夾**\\%ROLEROOT %_Approot（** 不在 中`commandLine`指定**\\%ROLEROOT %_Approot，** 假定它）。 **%ROLEROOT%** 是由 Azure 維護的環境變數，而它代表的是您角色的根資料夾位置。 %ROLEROOT%\Approot 資料夾表示角色的應用程式資料夾。 ** \\ **<br /><br /> 對於 HWC 角色，路徑始終相對於**\\%ROLEROOT%\Approot_bin**資料夾。<br /><br /> 對於完整的 IIS 和 IIS Express Web 角色，如果找不到相對於**\\%ROLEROOT%_Approot**資料夾的程式集，則搜索**\\%ROLEROOT%\Approot_bin。**<br /><br /> 完整 IIS 的這種回復行為並非建議的最佳作法，未來的版本可能會將其移除。|  
...
##  <a name="virtualdirectory"></a><a name="VirtualDirectory"></a>虛擬目錄  
