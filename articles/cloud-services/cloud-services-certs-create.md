...
Azure 所使用的憑證可以包含公開金鑰。 憑證具有指紋，可以明確的方式提供識別它們的方法。 此指紋會用在 Azure [設定檔](cloud-services-configure-ssl-certificate-portal.md) 中，以識別雲端服務應該使用的憑證。 
...
>[!Note]
>[雲端服務常見問題-設定和管理](cloud-services-configuration-and-management-faq.md)文章有一些實用的憑證相關資訊。
...
* 主體名稱必須符合用來存取雲端服務的網域。
    ...
    > 您無法取得 cloudapp.net (或任何 Azure 相關) 網域的 TLS/SSL 憑證;憑證的主體名稱必須符合用來存取應用程式的自訂功能變數名稱。 例如，**contoso.net**，而非 **contoso.cloudapp.net**。
