...
description: 瞭解如何建立和部署雲端服務的憑證，以及如何使用 Azure 中的管理 API 進行驗證。
...
Azure 所使用的憑證可以包含公開金鑰。 憑證具有指紋，可以明確的方式提供識別它們的方法。 此指紋會用於 Azure[設定檔](cloud-services-configure-ssl-certificate-portal.md)中，以識別雲端服務應該使用的憑證。 
...
* 主體名稱必須符合用來存取雲端服務的網域。
    ...
    > 您無法取得 cloudapp.net (或任何與 Azure 相關) 網域的 TLS/SSL 憑證;憑證的主體名稱必須符合用來存取應用程式的自訂功能變數名稱。 例如，**contoso.net**，而非 **contoso.cloudapp.net**。
