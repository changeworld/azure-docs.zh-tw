...
- [為什麼我的雲服務 SSL 憑證的憑證連結不完整？](#why-is-the-certificate-chain-of-my-cloud-service-ssl-certificate-incomplete)
...
- [如何將靜態 IP 位址與雲服務關聯？](#how-do-i-associate-a-static-ip-address-to-my-cloud-service)
...
- [無法使用 RDP 檔將桌面遠端到雲服務 VM。我得到以下錯誤：發生了身份驗證錯誤（代碼：0x80004005）](#i-cannot-remote-desktop-to-cloud-service-vm--by-using-the-rdp-file-i-get-following-error-an-authentication-error-has-occurred-code-0x80004005)
...
- [為什麼我的雲服務 VM 上的磁碟機顯示的可用磁碟空間很少？](#why-does-the-drive-on-my-cloud-service-vm-show-very-little-free-disk-space)
...
- [我想關閉雲服務幾個月。如何在不丟失 IP 位址的情況下降低雲服務的計費成本？](#i-want-to-shut-down-the-cloud-service-for-several-months-how-to-reduce-the-billing-cost-of-cloud-service-without-losing-the-ip-address)
...
此憑證用來加密 Azure Web 角色上的電腦金鑰。 要瞭解更多資訊，請查看[此諮詢](https://docs.microsoft.com/security-updates/securityadvisories/2018/4092731)。

如需詳細資訊，請參閱下列文章：
- [如何配置和運行雲服務的啟動任務](https://docs.microsoft.com/azure/cloud-services/cloud-services-startup-tasks)
...
如需詳細資訊，請參閱下列文件：
* [在 Azure 存儲中存儲和查看診斷資料](/azure/storage/common/storage-introduction)
...
2. [通過 .NET 代碼啟用](https://docs.microsoft.com/azure/cloud-services/cloud-services-dotnet-diagnostics)
...
* [將保留的 IP 關聯到新的雲服務](/previous-versions/azure/virtual-network/virtual-networks-reserved-public-ip#associate-a-reserved-ip-to-a-new-cloud-service)
...
* [使用服務設定檔將保留的 IP 與雲服務關聯](/previous-versions/azure/virtual-network/virtual-networks-reserved-public-ip#associate-a-reserved-ip-to-a-cloud-service-by-using-a-service-configuration-file)
...
您可透過啟動工作將這些步驟自動化，每當建立新的 PaaS 執行個體時，它可以在系統登錄中執行上述變更。 有關詳細資訊，請參閱[如何配置和運行雲服務的啟動任務](cloud-services-startup-tasks.md)。
...
%approot% 磁碟機大小計算為\<.cspkg = 最大日誌大小 = 可用空間>的邊距，或 1.5 GB（以較大者為准）。 您 VM 的大小對這個計算方式並無任何影響。 (VM 大小只會影響暫存 C: 磁碟機的大小。) 
...
SNI 綁定可以使用 PowerShell Cmdlet**新 Web 綁定**在雲服務角色實例的啟動任務中進行配置，如下所示：
