...
- **使用者加上應用程式訪問**：應用程式代表登錄使用者訪問金鑰保存庫。 舉例來說，這類存取包括 Azure PowerShell 和 Azure 入口網站。 使用者存取的授與則是透過兩種方式來完成。 使用者可以從任何應用程式存取 Key Vault，或者必須使用特定應用程式來進行 (稱為「複合身分識別」__)。
- **僅限應用程式訪問**：應用程式作為守護進程服務或後臺作業運行。 系統會將金鑰保存庫的存取權授與給該應用程式識別碼。
...
在此示例中，我們正在開發一個應用程式，該應用程式使用用於 TLS/SSL 的證書、用於存儲資料的 Azure 存儲證書以及用於符號操作的 RSA 2，048 位金鑰。 我們的應用程式會在 Azure 虛擬機器 (VM) (或虛擬機器擴展集) 中執行。 我們可以使用金鑰保存庫來儲存應用程式祕密。 我們可以儲存供應用程式用來向 Azure AD 進行驗證的啟動程序憑證。

我們需要存取下列預存金鑰和祕密：
- **TLS/SSL 憑證**：用於 TLS/SSL。
- **存儲金鑰**：用於訪問存儲帳戶。
- **RSA 2，048 位金鑰**：用於符號操作。
- **引導證書**：用於使用 Azure AD 進行身份驗證。 獲得存取權後，我們就可以擷取儲存體金鑰，以及使用 RSA 金鑰進行簽署。

我們必須定義下列角色，以指定誰可以管理、部署及稽核我們的應用程式：
- **安全團隊**：來自 CSO 辦公室（首席安全官）或類似貢獻者的 IT 人員。 安全性小組負責妥善保管祕密。 機密可以包括 TLS/SSL 憑證、用於簽名的 RSA 金鑰、連接字串和存儲帳戶金鑰。
- **開發人員和操作員**：開發應用程式並在 Azure 中部署應用程式的員工。 此小組的成員不屬於安全性工作人員。 他們不應有權訪問敏感性資料，如 TLS/SSL 憑證和 RSA 金鑰。 其所部署的應用程式才應具備敏感性資料的存取權。
- **審核員**：此角色適用于不是開發成員或一般 IT 人員的貢獻者。 其會檢閱憑證、金鑰和祕密的使用與維護情形，以確保各項作業符合安全性標準。
...
**開發人員和運營商**
- 從安全團隊獲取引導和 TLS/SSL 憑證（指紋）、存儲金鑰（機密 URI）和 RSA 金鑰（金鑰 URI）的引用，以便進行簽名。
...
| 開發人員和&nbsp;操作員 | Key Vault 部署權限<br><br> **注意**：此許可權允許已部署的 VM 從金鑰保存庫獲取機密。 | None |
| 稽核員 | None | 金鑰︰列出<br>密碼︰列出<br><br> **注意**：此許可權使審核員能夠檢查屬性（標記、啟動日期、到期日期）中未在日誌中發出的金鑰和機密。 |
...
- 下載[Azure 金鑰保存庫用戶端示例](https://www.microsoft.com/download/details.aspx?id=45343)。 此內容會示範如何使用啟動程序憑證，向 Azure AD 驗證以存取金鑰保存庫。

您可以使用 Azure 入口網站授與大部分的存取權限。 若要授與細微權限，您可以使用 Azure PowerShell 或 Azure CLI。

本節中的 PowerShell 程式碼片段是使用下列假設來建置的︰
- Azure AD 管理員創建了安全性群組來表示三個角色：Contoso 安全團隊、Contoso 應用 DevOps 和 Contoso 應用審核員。 系統管理員已將使用者新增到其各自的群組。
...
有關使用 Azure 金鑰保存庫的金鑰和機密的詳細資訊，請參閱[關於金鑰和機密](https://msdn.microsoft.com/library/azure/dn903623.aspx)。
