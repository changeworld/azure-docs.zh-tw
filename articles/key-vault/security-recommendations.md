...
| 建議 | 註解 | 資訊安全中心 |
|-|----|--|
|開啟軟刪除 | [軟刪除](key-vault-ovw-soft-delete.md)允許您回復已刪除的保管庫和保管庫物件 |  - |
...
| 建議 | 註解 | 資訊安全中心 |
|-|----|--|
| 限制具有參與者存取權限的使用者數 | 如果使用者對密鑰保管庫管理層具有參與者許可權,則使用者可以通過設置密鑰保管庫訪問策略來授予自己對數據平面的訪問許可權。 您應該嚴格控制誰有權訪問您的密鑰保管庫。 確保只有需要訪問許可權的人員才能訪問和管理您的保管庫。 您可以讀取[金鑰保存的管理](key-vault-secure-your-key-vault.md) | - |
...
| 限制可以存取 Azure 金鑰保管庫紀錄的人員 | [金鑰保管庫日誌](key-vault-logging.md)保存有關在保管庫中執行的活動的資訊,例如建立或刪除保管庫、密鑰、機密,並可能在調查期間使用 |  - |

## <a name="networking"></a>網路功能

| 建議 | 註解 | 資訊安全中心 |
|-|----|--|
|限制網路曝光 | 網路訪問應限於需要保管庫訪問的解決方案使用的虛擬網路。 檢視[Azure 金鑰保存式資訊](key-vault-overview-vnet-service-endpoints.md) | - |
