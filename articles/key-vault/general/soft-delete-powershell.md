...
description: 了解如何使用 Azure PowerShell 來使用 Azure Key Vault 的虛刪除功能，以允許復原金鑰保存庫和金鑰保存庫物件。
services: key-vault
...
- Azure PowerShell 1.0.0 或更新版本 - 如果您尚未安裝，請安裝 Azure PowerShell，並將它與 Azure 訂用帳戶建立關聯，請參閱[如何安裝和設定 Azure PowerShell](https://docs.microsoft.com/powershell/azure/) \(英文\)。 
...
預設會針對所有新的金鑰保存庫自動開啟虛刪除。 2020 年 12 月 31 日之後，就無法再於任何金鑰保存庫上停用虛刪除。 
