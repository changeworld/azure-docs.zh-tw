...
軟刪除功能最初可透過[REST、CLI、PowerShell](soft-delete-cli.md)和[PowerShell](soft-delete-powershell.md) [.NET/C++ 介面](/dotnet/api/microsoft.azure.keyvault?view=azure-dotnet)提供。 [REST](/rest/api/keyvault/)
...
默認保留期為 90 天,但可以通過 Azure 門戶將保留策略間隔設置為 7 到 90 天的值。 設置並保存保留策略間隔后,無法更改該保管庫。 
...
刪除金鑰保管庫物件(如密鑰)後,服務將物件置於已刪除狀態,從而使任何檢索操作都無法訪問該物件。 此狀態下的 Key Vault 物件只能列出、復原或強制/永久刪除。 
...
- [如何透過 PowerShell 使用金鑰保存庫虛刪除](soft-delete-powershell.md) 
