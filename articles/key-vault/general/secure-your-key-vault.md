...
- **使用者加上應用程式訪問**:應用程式代表登錄使用者存取金鑰保管庫。 舉例來說，這類存取包括 Azure PowerShell 和 Azure 入口網站。 使用者存取的授與則是透過兩種方式來完成。 使用者可以從任何應用程式存取 Key Vault，或者必須使用特定應用程式來進行 (稱為「複合身分識別」__)。
- **僅限應用程式訪問**:應用程式作為守護程序服務或後台作業運行。 系統會將金鑰保存庫的存取權授與給該應用程式識別碼。
...
| --- | --- | --- | --- |
| 管理平面 | **全球:**<br> management.azure.com:443<br><br> **Azure 中國 21Vianet:**<br> management.chinacloudapi.cn:443<br><br> **Azure US Gov︰**<br> management.usgovcloudapi.net:443<br><br> **Azure 德國:**<br> management.microsoftazure.de:443 | 建立、讀取、更新及刪除金鑰保存庫<br><br>設定 Key Vault 存取原則<br><br>設定 Key Vault 標籤 | Azure Resource Manager RBAC |
| 資料平面 | **全球:**<br> &lt;vault-name&gt;.vault.azure.net:443<br><br> **Azure 中國 21Vianet:**<br> &lt;vault-name&gt;.vault.azure.cn:443<br><br> **Azure US Gov︰**<br> &lt;vault-name&gt;.vault.usgovcloudapi.net:443<br><br> **Azure 德國:**<br> &lt;vault-name&gt;.vault.microsoftazure.de:443 | 金鑰︰解密、加密、<br> 解除包裝、包裝、驗證、登入、<br> 取得、列出、更新、建立、<br> 匯入、刪除、備份、還原<br><br> 祕密︰取得、列出、設定、刪除 | Key Vault 存取原則 |

## <a name="management-plane-and-rbac"></a>管理平面和 RBAC

在管理平面中,您可以使用 RBAC(基於角色的存取控制)來授權呼叫方可以執行的操作。 在 RBAC 模型中，每個 Azure 訂用帳戶都會有一個 Azure AD 執行個體。 您可以對來自該目錄的使用者、群組和應用程式授與存取權。 授與存取權即可在 Azure 訂用帳戶中管理使用 Azure Resource Manager 部署模型的資源。 若要授與存取權，請使用 [Azure 入口網站](https://portal.azure.com/)、[Azure CLI](/cli/azure/install-azure-cli?view=azure-cli-latest)、[Azure PowerShell](/powershell/azureps-cmdlets-docs) 或 [Azure Resource Manager REST API](https://msdn.microsoft.com/library/azure/dn906885.aspx)。
...
- **特定資源**:為特定資源分配的 RBAC 角色應用於該資源。 在此情況下，資源會是特定的金鑰保存庫。

有數個預先定義的角色。 如果預先定義的角色不符合您的需求，您可以定義您自己的角色。 有關詳細資訊,請參閱[RBAC:內建角色](../../role-based-access-control/built-in-roles.md)。
...
可以使用[Azure 金鑰保管庫的虛擬網路服務終結點](overview-vnet-service-endpoints.md)來限制數據平面訪問。 您可以設定[防火牆和虛擬網路規則](network-security.md)來額外加上一層安全性。

## <a name="example"></a>範例

在此範例中,我們正在開發一個應用程式,該應用程式使用用於 TLS/SSL 的證書、用於儲存資料的 Azure 儲存證書以及用於符號操作的 RSA 2,048 位元金鑰。 我們的應用程式會在 Azure 虛擬機器 (VM) (或虛擬機器擴展集) 中執行。 我們可以使用金鑰保存庫來儲存應用程式祕密。 我們可以儲存供應用程式用來向 Azure AD 進行驗證的啟動程序憑證。

我們需要存取下列預存金鑰和祕密：
- **TLS/SSL 憑證**:用於 TLS/SSL。
- **儲存金鑰**:用於存取儲存帳戶。
- **RSA 2,048 位元金鑰**:用於符號操作。
- **引導證書**:用於使用 Azure AD 進行身份驗證。 獲得存取權後，我們就可以擷取儲存體金鑰，以及使用 RSA 金鑰進行簽署。

我們必須定義下列角色，以指定誰可以管理、部署及稽核我們的應用程式：
- **安全團隊**:來自 CSO 辦公室(首席安全官)或類似貢獻者的 IT 人員。 安全性小組負責妥善保管祕密。 機密可以包括 TLS/SSL 憑證、用於簽名的 RSA 密鑰、連接字串和儲存帳戶金鑰。
- **開發人員和操作員**:開發應用程式並在 Azure 中部署應用程式的員工。 此小組的成員不屬於安全性工作人員。 他們不應有權存取敏感資料,如 TLS/SSL 證書和 RSA 密鑰。 其所部署的應用程式才應具備敏感性資料的存取權。
- **審核員**:此角色適用於不是開發成員或一般 IT 人員的貢獻者。 其會檢閱憑證、金鑰和祕密的使用與維護情形，以確保各項作業符合安全性標準。
...
**開發人員和營運商**
- 從安全團隊獲取引導和 TLS/SSL 憑證(指紋)、儲存金鑰(機密 URI)和 RSA 密鑰(密鑰 URI)的引用,以便進行簽名。
...
| 開發人員和&nbsp;操作員 | Key Vault 部署權限<br><br> **注意**:此許可權允許已部署的 VM 從金鑰保管庫獲取機密。 | None |
| 稽核員 | None | 金鑰︰列出<br>密碼︰列出<br><br> **注意**:此許可權使審核員能夠檢查屬性(標記、啟動日期、到期日期)中未在日誌中發出的密鑰和機密。 |
...
- 下載[Azure 金鑰保存庫客戶端範例](https://www.microsoft.com/download/details.aspx?id=45343)。 此內容會示範如何使用啟動程序憑證，向 Azure AD 驗證以存取金鑰保存庫。

您可以使用 Azure 入口網站授與大部分的存取權限。 若要授與細微權限，您可以使用 Azure PowerShell 或 Azure CLI。

本節中的 PowerShell 程式碼片段是使用下列假設來建置的︰
- Azure AD 管理員建立了安全組來表示三個角色:Contoso 安全團隊、Contoso 應用 DevOps 和 Contoso 應用審核員。 系統管理員已將使用者新增到其各自的群組。
...
針對我們的 DevOps 人員，金鑰保存庫 `deploy/action` 權限的自訂角色指派僅限於該資源群組範圍內。 只有**ContosoAppRG**資源組中創建的 VM 才允許訪問機密(TLS/SSL 和引導證書)。 由 DevOps 成員在其他資源群組中建立的 VM 即使有祕密的 URI，也無法存取這些祕密。
...
* [金鑰保存管理 REST API](https://msdn.microsoft.com/library/azure/mt620024.aspx)
...
有關管理員入門教程,請參閱[什麼是 Azure 密鑰保管庫?](overview.md)

有關金鑰保存庫的使用紀錄記錄的詳細資訊,請參閱[Azure 金鑰保存庫紀錄紀錄](logging.md)。

有關使用 Azure 金鑰保管庫的金鑰和機密的詳細資訊,請參閱[關於金鑰和機密](https://msdn.microsoft.com/library/azure/dn903623.aspx)。
