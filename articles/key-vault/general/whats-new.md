---
title: Azure Key Vault 的新功能 | Microsoft Docs
...
ms.locfileid: "90069351"
---
# <a name="whats-new-for-azure-key-vault"></a>Azure Key Vault 的新功能

> [!WARNING]
> **2020 年 7 月**：金鑰保存庫有兩項更新可能會影響服務的實作：[預設金鑰保存庫虛刪除](#soft-delete-on-by-default)和 [Azure TLS 憑證變更](#azure-tls-certificate-changes)。 如需詳細資料，請參閱下文。
...
- Azure Active Directory (Azure AD) 服務會在 2020 年 7 月 7 日開始轉換。
...
### <a name="will-this-affect-me"></a>這對我有何影響？
...
- 與 Azure 服務通訊的其他作業系統和語言執行階段可能需要額外的步驟，才能使用這些新的根正確建立憑證鏈結： 
    - **Linux**：許多散發套件都需要您將上列的 CA 新增至 /etc/ssl/certs。 如需特定指示，請參閱散發套件文件。
    - **Java**︰請確定 Java 金鑰存放區包含上列的 CA。
    - **在中斷連線的環境中執行 Windows**：在中斷連線的環境中執行的系統，必須將上述的根新增至 [信任的根憑證授權單位] 存放區，並將中繼新增至 [中繼憑證授權單位] 存放區。
