...
使用[az keyvault key create](/cli/azure/keyvault/key?view=azure-cli-latest#az-keyvault-key-create)命令來建立將金鑰作業設定為`import`的 KEK。 記錄從下列命令傳回`kid`的金鑰識別碼（）。 （您將使用`kid` [步驟 3](#step-3-generate-and-prepare-your-key-for-transfer)中的值）。
...
請參閱 HSM 廠商的檔，以下載並安裝 BYOK 工具。 遵循 HSM 廠商提供的指示來產生目標金鑰，然後建立金鑰傳輸套件（BYOK 檔案）。 BYOK 工具會`kid`使用步驟[1](#step-1-generate-a-kek)中所下載的 KEKforBYOK. publickey. PEM 檔案，在 BYOK 檔案[Step 2](#step-2-download-the-kek-public-key)中產生加密的目標金鑰。
