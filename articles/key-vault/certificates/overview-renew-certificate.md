...
使用 Azure Key Vault，您可以輕鬆地佈建、管理及部署網路的數位憑證，以及啟用應用程式的安全通訊。 如需更多有關憑證的一般資訊，請參閱[有關 Azure Key Vault 憑證](https://docs.microsoft.com/azure/key-vault/certificates/about-certificates)。
...
Azure Key Vault 會處理受信任的 Microsoft 憑證授權單位 DigiCert 和 GlobalSign 所發行的憑證端對端維護。 了解如何[使用 Key Vault 整合受信任的 CA ](https://docs.microsoft.com/azure/key-vault/certificates/how-to-integrate-certificate-authority)。
...
Azure Key Vault 也會處理自我簽署憑證的自動更新。 若要深入了解如何變更發佈原則和更新憑證的生命週期屬性，請參閱[在 Key Vault 中設定憑證自動輪替](https://docs.microsoft.com/azure/key-vault/certificates/tutorial-rotate-certificates#update-lifecycle-attributes-of-a-stored-certificate)。
