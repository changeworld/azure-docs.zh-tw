...
從主控台視窗安裝適用於 Python 的 Azure Key Vault 憑證程式庫。
...
若要驗證雲端式 .NET 應用程式，最簡單的方法是使用受控識別；如需詳細資訊，請參閱[使用 App Service 受控識別存取 Azure Key Vault](../general/managed-identity.md)。 不過，為了簡單起見，本快速入門會建立 .NET 主控台應用程式。 使用 Azure 來驗證桌面應用程式時，必須使用服務主體和存取控制原則。
...
建立金鑰保存庫的存取原則，其藉由將 clientId 傳遞至 [az keyvault set-policy](/cli/azure/keyvault?view=azure-cli-latest#az-keyvault-set-policy) 命令，將權限授予您的服務主體。 提供憑證的取得、列出和建立權限給服務主體。
...
適用於 Python 的 Azure Key Vault 用戶端程式庫可讓您管理金鑰和相關資產，例如憑證和祕密。 下列程式碼範例會說明如何建立用戶端、建立憑證、擷取憑證和刪除憑證。
...
現在，您的應用程式已通過驗證，接下來您可以將自我簽署憑證放入金鑰保存庫中。 
...
您可以使用 [az keyvault certificate show](/cli/azure/keyvault/certificate?view=azure-cli-latest#az-keyvault-certificate-show) 命令來確認憑證是否已完成設定：
...
您的憑證現在會儲存為 `retrieved_certificate`。
...
您可以使用 [az keyvault certificate show](/cli/azure/keyvault/certificate?view=azure-cli-latest#az-keyvault-certificate-show) 命令來確認憑證是否已消失：
