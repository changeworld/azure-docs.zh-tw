...
個人化工具支援字串、數字和布林值類型的特性。 您的應用程式很有可能會使用字串功能，但有一些例外狀況。

### <a name="how-choice-of-feature-type-affects-machine-learning-in-personalizer"></a>功能類型的選擇如何影響個人化工具中的 Machine Learning

* **字串**：對於字串類型，會將每個索引鍵和值的組合視為 One-Hot 的功能 (例如，內容類型： "ScienceFiction" 和內容類型： "加上" 會為機器學習模型建立兩個新的輸入功能。
* **數值**：當數位是應依比例影響個人化結果的量值時，您應該使用數值。 這與案例相依。 在簡化的範例中，例如將零售體驗個人化時，NumberOfPetsOwned 可能是數位的一項功能，因為您可能會想要讓具有2或3個寵物的人員影響個人化結果兩次或三次，最多可有1個寵物。 以數位單位為基礎，但意義不是線性的特徵（例如年齡、溫度或個人高度）最適合編碼為字串。 例如，DayOfMonth 會是 "1"、"2" ... "31" 的字串。 如果您有許多類別，通常可以使用範圍來改善功能品質。 例如，Age 可能會編碼為 "Age"： "0-5"、"Age"： "6-10" 等等。
