...
* 對於分類要素，不需要定義可能的值，也不需要預先定義數值的範圍。

## <a name="supported-feature-types"></a>支援的特性類型

個人化工具支援字串、數字和布林值類型的特性。

### <a name="how-choice-of-feature-type-affects-machine-learning-in-personalizer"></a>功能類型的選擇如何影響個人化中的機器學習

* **字串**：對於字串類型，鍵和值的每個組合都會在個人化機器學習模型中創建新的權重。 
* **數位**：當數位應按比例影響個人化結果時，應使用數值。 這是非常依賴于方案。 在簡化示例中，例如個人化零售體驗時，NumberOfPetsOfs 可能是一個數位功能，因為您可能希望擁有 2 或 3 只寵物的人對個人化結果的影響兩次或三倍于擁有 1 只寵物。 基於數值單位但含義不是線性的要素（如年齡、溫度或人員高度）最好編碼為字串，並且通常可以使用範圍來提高要素品質。 例如，年齡可以編碼為"年齡"："0-5"，"年齡"："6-10"等。
* 發送的值為"false"的**布林**值就像它們根本不發送一樣。
...
您可以遵循您自己的慣例來為特性命名空間命名，但前提是這些特性命名空間必須是有效的 JSON 索引鍵。 命名空間用於將要素組織成不同的集，以及消除具有類似名稱的要素的歧義。 可以將命名空間視為添加到要素名稱的"首碼"。 不能嵌套命名空間。


在下列 JSON 中，`user`、`state` 和 `device` 便是特性命名空間。 

> [!Note]
> 目前，我們強烈建議對基於 UTF-8 的要素命名空間使用名稱，並使用不同的字母開頭。 `user`例如，`state``device`和 開頭`u`，`s`和`d`。 目前具有相同第一個字元的命名空間可能會導致用於機器學習的索引發生衝突。

JSON 物件可以包括嵌套的 JSON 物件和簡單的屬性/值。 僅當陣列項為數字時，才能包含陣列。 
...
### <a name="restrictions-in-character-sets-for-namespaces"></a>命名空間的字元集中的限制

用於命名命名空間的字串必須遵循一些限制： 
* 不能是單碼。
* 可以使用一些帶有代碼< 256 的可列印符號來命名。 
* 不能使用代碼< 32（不可列印）、32（空格）、58（冒號）、124（管道）和 126-140 的代碼符號。
* 它不應以底線"*"開頭，否則該功能將被忽略。
...
位置資訊通常還受益于創建更廣泛的分類。 例如，緯度經座標（如 Lat：47.67402° N，長：122.12154° W）過於精確，並強制模型將緯度和經度作為不同的尺寸來學習。 當您嘗試根據位置資訊進行個人化設置時，它有助於對較大磁區中的位置資訊進行分組。 一個簡單的方法是為 Lat-Long 數位選擇適當的舍入精度，並通過將它們組合成一個字串，將緯度和經度組合成"區域"。 例如，在大約幾公里寬的區域中，表示 47.67402° N、長：122.12154° W 的好方法是"位置"："34.3， 12.1"。


#### <a name="expand-feature-sets-with-extrapolated-information"></a>使用類推的資訊擴大特性集合

您也可以想想有沒有尚未探索到的屬性可以從您已經擁有的資訊來衍生，從而獲得更多的特性。 例如，在虛構的電影清單個人化中，週末與工作日是否引起使用者的不同行為？ 您可以擴充時間來獲得「週末」或「平日」屬性。 與文化有關的國定假日是否會讓人們關注某些電影類型？ 例如，「萬聖節」屬性在與其相關的地點便很有用。 雨天是否可能會對許多人在選擇電影時造成重大影響？ 使用時間和地點，天氣服務便可以提供該資訊，您也可以將其新增為額外的特性。 
...
* 具有_事件_ID。 如果您已經擁有事件 ID，則應提交該 ID。 如果您沒有事件 ID，請不要發送，個人化器會為您創建一個，並在 Rank 請求的回應中返回它。 ID 與 Rank 事件關聯，而不是與使用者關聯。 如果創建 ID，GUID 效果最佳。 
* 都有特性清單。
...
在某些情況下，您的商務邏輯只能在後面才判斷是否要對使用者顯示排名 API 呼叫所產生的「動作」__。 針對這些情況，請使用「非作用中事件」__。
...
「內容」__ 的資訊取決於每個應用程式和使用案例，但一般來說，可能包含的資訊如下：
