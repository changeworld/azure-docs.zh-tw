...
為了讓社會大眾了解充分發揮 AI 的潛能，設計實作時應採適當方式，讓透過 AI 建置的應用程式以及在其應用程式中新增 AI 的使用者能夠信任這些實作。 這些指導方針旨在協助您以有助於在公司和服務中建立信任的方式，來實作個人化工具。 請務必暫停研究、學習及刻意瞭解個人化生活的影響。 如有疑問，請尋求指引。
...
您可以使用下列內容作為入門檢查清單，並根據您的案例加以自訂及精簡。 本檔有兩個主要章節：第一個區段專門用來在選擇個人化工具的案例、功能和報酬時，反白顯示負責的使用考慮。 第二個部分將提供 Microsoft 認為在建置 AI 系統時所應考量的若干價值，並提供可採行的建議，以及評估您使用個人化工具時對系統產生影響的風險。
...
* **教育**：個人化課程和教育機構的排名，其中的建議可能會傳播偏差，並減少使用者對其他選項的認知。
* **民主和市政參與**：為具有影響力意見之目標的使用者個人化內容，是衍生性和操弄。
* **協力廠商報酬評估**：個人化報酬依據協力廠商評估使用者的專案，而不是由使用者本身的行為所產生的報酬。
* **Intolerance 探索**：個人化工具行為可能會造成損害的任何情況。
...
* **使用者人口統計**：關於性別、性別、年齡、競爭、宗教的功能：某些應用程式在某些情況下可能不允許這些功能，基於法規因素，因為個人化會傳播歸納和偏差，所以可能不會對其進行個人化。 例如，工程師徵才公告若刻意不向年長或特定性別的對象顯示，即屬於這類偏見傳播。
* **地區設定資訊**：在世界各地的許多地方，位置資訊（例如郵遞區號、郵遞區號或鄰近名稱）可能會與收入、競爭和宗教高度相互關聯。
* **使用者對公平的認知**：即使在您的應用程式做出合理決策的情況下，也請考慮使用者察覺應用程式中顯示的內容變更，其外觀會與歧視性的功能相互關聯。
* **功能中非預期的偏差**：使用只會影響擴展子集的功能，可能會導入一些偏差類型。 如果以演算法產生特性，則需特別留意，例如，在使用影像分析擷取圖片中的項目時，以及使用文字分析探索文字中的實體時。 請留意您用來建立這些特性的服務有哪些特徵。

選擇要在內容和動作中傳送至個人化工具的特性時，請套用下列實務準則：

* 考量對某些應用程式使用特定特性是否合乎法律和道德，以及看似純正的特性是否可能遭他人轉化為您想要或應該避免的特性。
* 明確告知使用者他們所看到的選項會使用演算法和資料分析進行個人化。
* 問自己：我的使用者是否在意，如果我使用這項資訊來個人化他們的內容，是否會感到滿意？ 我是否能坦然向使用者說明醒目提示或隱藏某些項目的決策是如何制定的？
...
* 請參閱 Microsoft 在2018年1月發行的《 AI 開發責任》的六個準則，[未來計算](https://news.microsoft.com/futurecomputed/)
