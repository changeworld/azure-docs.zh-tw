...
個人化工具會使用機器學習來選取要向使用者顯示的動作（內容）。 根據傳送給服務的資料數量、品質及分佈，選取項目會極為不同。
...
|評分的結果|藉由測量使用者行為的某些層面，並以報酬 _[分數](concept-rewards.md)_ 表示，可以判斷您應用程式的排名選擇如何有效。|
...
|足夠的動作功能|您擁有每個內容選項 (即動作) 的相關資訊，其形式為含有至少 5 個您認為有助於個人化工具做出正確選擇的[特性](concepts-features.md)。|
