...
:::image type="content" source="../media/diagnostics/incident-list.png" alt-text="事件清單" lightbox="../media/diagnostics/incident-list.png":::

**診斷**區段可讓您執行事件的深入分析，以及用來識別根本原因的工具。

:::image type="content" source="../media/diagnostics/diagnose-incident.png" alt-text="診斷事件" lightbox="../media/diagnostics/diagnose-incident.png" :::
...
:::image type="content" source="../media/diagnostics/root-cause-advice.png" lightbox="../media/diagnostics/root-cause-advice.png" alt-text="根本原因建議":::
...
:::image type="content" source="../media/diagnostics/incident-tree.png" alt-text="事件樹狀結構" lightbox="../media/diagnostics/incident-tree.png" :::
...
:::image type="content" source="../media/diagnostics/metric-drilling.png" lightbox="../media/diagnostics/metric-drilling.png" alt-text="度量鑽孔 ":::

[ **維度** ] 設定是事件的維度清單，您可以為每個事件選取其他可用的維度值。 變更維度值之後。 **時間戳記**設定可讓您在不同時間的時間查看目前的事件。
...
:::image type="content" source="../media/diagnostics/drill-down-dimension.png"  lightbox="../media/diagnostics/drill-down-dimension.png" alt-text="向下切入維度":::
...
:::image type="content" source="../media/diagnostics/drill-down-comparison.png" alt-text="向下切入比較" lightbox="../media/diagnostics/drill-down-comparison.png":::
...
:::image type="content" source="../media/diagnostics/drill-down-table.png" alt-text="向下切入資料表" lightbox="../media/diagnostics/drill-down-table.png":::

### <a name="raw-value-visualization"></a>原始值視覺效果
[向下切入] 索引標籤的最後一個部分是原始值的折線圖。 提供此圖表時，不需要流覽至 [度量] 頁面即可查看詳細資料。

:::image type="content" source="../media/diagnostics/drill-down-line-chart.png" alt-text="向下切入折線圖" lightbox="../media/diagnostics/drill-down-line-chart.png":::
...
:::image type="content" source="../media/diagnostics/series-group.png" lightbox="../media/diagnostics/series-group.png"alt-text="數列群組":::

## <a name="compare-time-series"></a>比較時間序列

有時，在特定時間序列上偵測到異常時，在單一視覺效果中，將它與其他多個數列進行比較會很有説明。 按一下 [ **比較工具** ] 索引標籤，然後按一下藍色的 [ **新增** ] 按鈕。 

:::image type="content" source="../media/diagnostics/add-series.png" alt-text="新增要比較的數列" lightbox="../media/diagnostics/add-series.png":::

從您的資料摘要中選取一個數列。 您可以選擇相同的資料細微性或不同的資料細微性。 選取目標維度並載入數列趨勢，然後按一下 **[確定]** ，將它與前一個數列比較。 數列將會一起放在一個視覺效果中。 您可以繼續新增更多數列以進行比較，並取得進一步的見解。 按一下 [ **比較工具** ] 索引標籤頂端的下拉式功能表，以比較時間序列資料在一段時間內的移動。  

> [!Warning]
> 若要啟用移位的比較，您資料的資料細微性必須支援。 例如，如果您的資料是每週，而您使用當天的 **日期** 比較，您將不會得到任何結果。 在此範例中，您會改為使用月份的比較 **月份** 比較。

選取移位的比較之後，您可以選取要比較資料值、差異值或百分比差異。
...
:::image type="content" source="../media/graph/metrics-graph-cross-metrics-analysis.png" alt-text="計量之間的相關事件":::
