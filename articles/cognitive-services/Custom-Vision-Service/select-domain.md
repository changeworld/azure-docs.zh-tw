...
## <a name="compact-domains"></a>Compact 網域

精簡領域所產生的模型可以匯出到本機執行。 模型效能會因選取的定義域而異。 在下表中，我們會報告 Intel 桌面 CPU 和 NVidia GPU 上的模型大小和推斷時間 \[1\]。 
...
|---|---|---|---|---|
|分類|一般（compact）|5 MB|13 毫秒|5 毫秒|
|物件偵測|一般（compact）|45 MB|35毫秒|5 毫秒|
...
>[!IMPORTANT]
>不保證匯出的模型會提供與雲端上的預測 API 完全相同的結果。 執行中平臺或前置處理的實作為些許差異，可能會在模型輸出中產生較大的差異。 如需前置處理邏輯的詳細資料，請參閱[這份檔](python-tutorial.md)。

\[1\] Intel M60 E5-2690 CPU 和 NVIDIA Tesla
