...
description: 瞭解如何使用 Azure 密鑰保管庫使用 Azure 門戶配置客戶管理的密鑰。 客戶管理的金鑰使您能夠創建、旋轉、禁用和撤銷存取控制。
...
您必須使用 Azure 金鑰保存庫來儲存客戶管理的金鑰。 您可以建立自己的密鑰並將其存儲在密鑰保管庫中,也可以使用 Azure 金鑰保管庫 API 產生金鑰。 認知服務資源和密鑰保管庫必須位於同一區域和同一 Azure 活動目錄 (Azure AD) 租戶中,但它們可以位於不同的訂閱中。 有關 Azure 金鑰保管庫的詳細資訊,請參閱[什麼是 Azure 密鑰保管庫?](https://docs.microsoft.com/azure/key-vault/key-vault-overview)
...
使用客戶管理的金鑰需要在密鑰保管庫上設置兩個屬性,**即「軟刪除**」和 **「不清除**」 。 默認情況下,這些屬性不會啟用,但可以使用 PowerShell 或 Azure CLI 在新密鑰保管庫或現有金鑰保管庫上啟用。

> [!IMPORTANT]
> 如果未啟用 **「軟刪除**」和 **「不清除」** 屬性,並且刪除了密鑰,則將無法恢復認知服務資源中的數據。

要瞭解如何在現有金鑰保管庫中啟用這些屬性,請參閱以下文章中標題為 **「啟用軟刪除**和**啟用清除保護**」的部分:

- [如何使用軟刪除與PowerShell。](https://docs.microsoft.com/azure/key-vault/key-vault-soft-delete-powershell)
- [如何使用軟刪除與CLI。](https://docs.microsoft.com/azure/key-vault/key-vault-soft-delete-cli)

Azure 儲存加密僅支援大小為 2048 的 RSA 密鑰。 關於金鑰的詳細資訊,請參考[「關於 Azure 金鑰保管庫金鑰、機密和憑證」](https://docs.microsoft.com/azure/key-vault/about-keys-secrets-and-certificates#key-vault-keys)的**金鑰保管庫金鑰**。
...
1. 導航到認知服務資源。
1. 在認知服務資源的 **「設置」** 邊欄選項卡上,按下 **「加密**」 。 選擇 **「客戶託管金鑰」** 選項,如下圖所示。

    ![顯示如何選擇客戶託管金鑰的螢幕擷取](../media/cognitive-services-encryption/selectcmk.png)

## <a name="specify-a-key"></a>指定鍵

啟用客戶管理的密鑰後,您將有機會指定要與認知服務資源關聯的密鑰。
...
1. 複製提供 URI 的 **「密鑰識別符」** 欄位的值。
    ...
    ![顯示金鑰保存金鑰的網鑰的網形](../media/cognitive-services-encryption/key-uri-portal.png)

1. 在儲存帳戶的**加密**設定中,選擇 **「輸入金鑰 URI」** 選項。
1. 將複製的 URI 貼上到 **「密鑰 URI」** 欄位中。

   ![顯示如何輸入金鑰的網址的螢幕擷取](../media/cognitive-services-encryption/ssecmk2.png)
...
1. 從金鑰保管庫中選擇密鑰。
   ...
   ![顯示客戶託管金鑰選項的螢幕擷取](../media/cognitive-services-encryption/ssecmk3.png)

1. 儲存您的變更。

## <a name="update-the-key-version"></a>更新金鑰版本

建立金鑰的新版本時,請更新認知服務資源以使用新版本。 請遵循下列步驟：
...
禁用客戶管理的金鑰時,您的認知服務資源將使用Microsoft管理的密鑰進行加密。 要關閉客戶管理的金鑰,請按照以下步驟操作:
