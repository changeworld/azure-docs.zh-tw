...
QnA Maker 使用 Azure 搜尋服務的 CMK 支援。 [使用 Azure Key Vault 在 Azure 搜尋服務中設定 CMK](../../search/search-security-manage-encryption-keys.md)。 此 Azure 實例應與 QnA Maker 服務相關聯，使其 CMK 啟用。

# <a name="qna-maker-managed-preview-release"></a>[受控 QnA Maker (預覽版本)](#tab/v2)

QnA Maker 使用 [Azure 搜尋服務中的 CMK 支援](../../search/search-security-manage-encryption-keys.md)，並自動將所提供的 CMK 與加密 azure 搜尋服務索引中儲存的資料產生關聯。
...
1. 建立新的 Azure 搜尋服務實例，並啟用 [Azure 認知搜尋客戶管理的重要必要條件](../../search/search-security-manage-encryption-keys.md#prerequisites)中所述的必要條件。
...
2.  選取 [ **客戶管理的金鑰** ] 選項。 提供 [客戶管理金鑰](../../storage/common/customer-managed-keys-configure-key-vault.md?tabs=portal) 的詳細資料，然後按一下 [ **儲存**]。
...
> [!IMPORTANT]
> 建議您在建立任何知識庫之前，先在全新的 Azure 認知搜尋服務中設定您的 CMK。 如果您在 QnA Maker 服務中以現有的知識庫設定 CMK，您可能會失去其存取權。 深入瞭解如何使用 Azure 認知搜尋中 [的加密內容](../../search/search-security-manage-encryption-keys.md#work-with-encrypted-content) 。
...
* [在 Azure Key Vault 中使用 Cmk 在 Azure 搜尋服務中加密](../../search/search-security-manage-encryption-keys.md)
* [待用資料加密](../../security/fundamentals/encryption-atrest.md)
