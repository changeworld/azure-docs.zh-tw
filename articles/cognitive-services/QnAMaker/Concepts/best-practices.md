...
您的使用者可以輸入有交談風格的文字、`How do I add a toner cartridge to my printer?` 或關鍵字搜尋（例如 `toner cartridge`）的問題。 知識庫應該有兩種問題樣式，才能正確地傳回最佳答案。 如果您不確定客戶輸入的關鍵字，請使用 Application Insights 資料來分析查詢。
...
如果您新增自己的閒聊交談 QnA 配對，請務必新增中繼資料，以便傳回這些答案。 中繼資料名稱/值組是 `editorial:chitchat`。
...
如果您不想要搜尋答案，請使用[`RankerType=QuestionOnly`](#choosing-ranker-type) 。
