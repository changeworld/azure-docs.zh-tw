...
* [隱式反饋](#how-qna-makers-implicit-feedback-works)– 當用戶問題有多個答案且分數非常接近,並將其視為反饋時,排名者會理解。 你不需要做任何事情來做到這一點。
* [明確反饋](#how-you-give-explicit-feedback-with-the-train-api)– 噹從知識庫中返回分數變化不大的多個答案時,客戶端應用程式會詢問使用者哪個問題是正確的問題。 用戶的顯式反饋將隨[訓練 API](../How-to/improve-knowledge-base.md#train-api)發送給 QnA 製造商。

這兩種方法都為排名者提供群集的類似查詢。

## <a name="how-active-learning-works"></a>主動式學習的運作方式

活動學習根據 QnA Maker 返回的前幾個答案的分數觸發。 如果匹配查詢的 QnA 對之間的分數差異位於一個小範圍內,則查詢將被視為每個可能的 QnA 對的可能建議(作為備用問題)。 接受特定 QnA 對的建議問題后,其他對將拒絕該問題。 在接受建議后,您需要記住保存和訓練。
...
* 要檢視建議的備用問題,請使用「編輯」頁上[的「檢視」選項](../How-To/improve-knowledge-base.md#view-suggested-questions)。

## <a name="how-qna-makers-implicit-feedback-works"></a>QnA 製造商的隱式回饋的工作原理

QnA Maker 的隱式反饋使用演算法來確定分數接近程度,然後提出主動學習建議。 判定相近程度的演算法並非簡單的計算。 以下示例中的範圍不是要固定的,但應用作僅瞭解演算法影響的指南。
...
QnA 製造商不知道哪個答案是最好的答案。 使用 QnA Maker 入口的建議清單選擇最佳答案並再次訓練。


## <a name="how-you-give-explicit-feedback-with-the-train-api"></a>如何使用培訓 API 提供明確回饋

QnA Maker 需要明確反饋,瞭解哪些答案是最佳答案。 如何確定最佳答案取決於您,可能包括:

* 用戶反饋,選擇其中一個答案。
* 業務邏輯,例如確定可接受的分數範圍。
* 用戶反饋和業務邏輯的組合。

使用[訓練 API](https://docs.microsoft.com/rest/api/cognitiveservices/qnamakerruntime/runtime/train)在用戶選擇後向 QnA 製造商發送正確的答案。
