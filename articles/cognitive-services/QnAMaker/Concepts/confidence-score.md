...
---
# <a name="the-confidence-score-of-an-answer"></a>答案的置信度分數
...
上表顯示大部分 KB 預期的分數。 但是，由於每個知識庫都不同，並且具有不同類型的單詞、意圖和目標，我們建議您測試並選擇最適合您的門檻。 預設情況下，閾值設置為 0，以便返回所有可能的答案。 建議對大多數 KB 起作用的閾值為**50**。
...
- 如果**涵蓋範圍** (或重新叫用) 更重要，且您想要盡可能回答更多問題 (即使答案與使用者的問題只有部分相關)，則您可以降低閾值。 這表示會有更多答案無法回答使用者實際查詢，但能提供一些其他可能相關之答案的情況。 *例如：* 如果進行閾值**30，** 則可以為查詢提供答案，例如"我在哪裡可以編輯我的知識庫？
...
## <a name="set-threshold"></a>設置閾值

將閾值分數設置為[生成應答 API JSON 正文](../how-to/metadata-generateanswer-usage.md#generateanswer-request-configuration)的屬性。 這意味著您為每個調用將其設置為"生成應答"。

從自動程式框架中，使用[C#](../how-to/metadata-generateanswer-usage.md?#use-qna-maker-with-a-bot-in-c)或[Node.js](../how-to/metadata-generateanswer-usage.md?#use-qna-maker-with-a-bot-in-nodejs)將分數設置為選項物件的一部分。
...
## <a name="confidence-score-differences-between-test-and-production"></a>測試和生產之間的置信度分數差異
在測試和發佈的知識庫版本之間，即使內容相同，不過答案的信賴分數可能會略微變化。 這是因為測試的內容和已發佈的知識庫位於不同的 Azure 認知搜索索引中。

測試索引包含知識庫的所有 QnA 對。 查詢測試索引時，查詢將應用於整個索引，然後結果將限制為該特定知識庫的分區。 如果測試查詢結果對驗證知識庫的能力產生負面影響，您可以：
* 使用以下之一組織您的知識庫：
    * 1 資源限制為 1 KB：將單個 QnA 資源（以及生成的 Azure 認知搜索測試索引）限制為單個知識庫。
    * 2 資源 - 1 用於測試，1 個用於生產：具有兩個 QnA 製造商資源，一個用於測試（有自己的測試和生產指標），另一個用於產品（也有其自己的測試和生產索引）
* 並且，始終使用相同的參數，例如查詢測試和生產知識庫時**[的頂部](../how-to/improve-knowledge-base.md#use-the-top-property-in-the-generateanswer-request-to-get-several-matching-answers)**
...
如果在不同的區域有知識庫，則每個區域都使用其自己的 Azure 認知搜索索引。 由於會使用不同的索引，因此，分數將不會完全相同。


## <a name="no-match-found"></a>未找到相符項目
順位排定程式找不到適當的相符項目時，會傳回 0.0 的信賴分數或「無」，而且預設回應是「在資料庫中找不到適當的相符項目」。 您可以在調用終結點的自動程式或應用程式代碼中重寫此[預設回應](../How-To/metadata-generateanswer-usage.md)。 或者，您也可以在 Azure 中設定覆寫回應，這會變更特定 QnA Maker 服務中部署的所有知識庫預設值。
