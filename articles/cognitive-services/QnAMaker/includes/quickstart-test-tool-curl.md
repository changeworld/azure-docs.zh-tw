...
## <a name="use-curl-with-threshold-and-default-answer"></a>搭配使用 cURL 與閾值和預設答案

您可以針對答案要求最小的閾值。 如果不符合閾值，便傳回預設的答案。

1. 使用下列 cURL 命令，並取代為您自己的資源名稱、知識庫識別碼和端點金鑰，以要求閾值為 80% 以上的 `size` 答案。 因為問題的分數是 71%，所以知識庫應該不會找到該答案，而是會傳回您在建立知識庫時所提供的預設答案。
