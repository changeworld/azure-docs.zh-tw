...
ms.locfileid: "95025220"
---
# <a name="regular-expression-entity"></a>規則運算式實體

規則則運算式實體會根據您所提供的規則運算式模式來擷取實體。

規則運算式最適用於未經處理的語句文字。 這會忽略大小寫並忽略文化特性變體。  在字元等級而非權杖等級的拼字檢查修改之後，才會套用規則運算式比對。 如果規則運算式太複雜 (例如使用許多方括號)，您便無法將運算式新增到模型中。 使用元件，但不是所有 [.Net Regex](/dotnet/standard/base-types/regular-expressions) 程式庫。
