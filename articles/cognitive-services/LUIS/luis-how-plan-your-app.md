...
> [!TIP]
> LUIS 有針對許多常見案例提供[預先建置的領域](./howto-add-prebuilt-models.md)。 請確認您是否能使用預先建置的領域作為應用程式的起點。
...
> [!TIP]
> LUIS 為常見的交談式使用者案例提供預先建立的 [實體](./howto-add-prebuilt-models.md) 。 考慮使用預先建立的實體作為應用程式開發的起點。
