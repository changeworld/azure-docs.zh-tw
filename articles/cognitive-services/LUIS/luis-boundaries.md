...
| 明確清單 | 每個應用程式 50 個|
| 外部實體 | 無限制 |
| [對應方式][intents]|每個應用程式 500 個：499 個自訂意圖，以及所需的_無_意圖。<br>[基於調度](https://aka.ms/dispatch-tool)的應用程式具有相應的 500 個調度源。|
| [清單實體](./luis-concept-entity-types.md) | 父系：50 個項目，子系：20,000 個項目。 正式名稱為*預設字元上限。同義值沒有長度限制。 |
| [機器學習的實體 + 角色](./luis-concept-entity-types.md)：<br> 複合<br>簡單<br>實體角色|100 個父實體或 330 個實體的限制，以使用者首先命中的限制為准。 為了此邊界，角色計為實體。 例如，具有簡單實體的複合，它有 2 個角色：330 個實體中的 4 個複合 = 1 個簡單 + 2 個角色。<br>子元件最多可嵌套 5 個級別。|
|建模為特徵| 可用作特定模型的描述項（特徵）的最大模型數為 10 個模型。 用作特定模型描述項（功能）的片語清單的最大數量為 10 個片語清單。|
...
| [片語清單][phrase-list]|500 個片語清單。 不可互換的片語清單最多包含 5，000 個短語。 可互換片語清單最多包含 50，000 個短語。 每個應用的最大總短語數為 500，000 個短語。|
| [預構建實體](./luis-prebuilt-entities.md) | 沒有限制|
...
資源鍵對創作和終結點有不同的限制。 LUIS 預測查詢終結點金鑰僅適用于終結點查詢。
...
|[情感分析整合](luis-how-to-publish-app.md#enable-sentiment-analysis)|-|-|無需其他 Azure 資源即可添加情緒資訊（包括關鍵短語資料提取）。 |
...
## <a name="website-sign-in-time-period"></a>網站登錄時間段
