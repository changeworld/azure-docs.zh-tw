...
LUIS 具有每月配額和每秒配額，具體取決於 Azure 資源的定價層。 

如果您的 LUIS 應用請求速率超過允許的[配額率](https://azure.microsoft.com/pricing/details/cognitive-services/language-understanding-intelligent-services/)，您可以：

* 將負載擴展到[具有相同應用定義的](#use-multiple-apps-with-same-app-definition)更多 LUIS 應用。 這包括（可選）從[容器](luis-container-howto.md)運行 LUIS。 
* 創建並[分配應用的多個鍵](#assign-multiple-luis-keys-to-same-app)。 
...
訓練這些同級應用時，請確保[使用所有資料進行訓練](luis-how-to-train.md#train-with-all-data)。
...
計畫定期審核（如每兩週一次）的[終結點話語](luis-how-to-review-endpoint-utterances.md)以進行主動學習，然後重新訓練和重新發佈。 
...
假設您正在開發一個具有 500 多個意向的辦公室助理。 如果有 200 個意圖與安排會議有關、200 個與提醒有關、200 個與取得同事相關資訊有關，以及 200 個用於傳送電子郵件，則請將這些意圖分組，讓每個群組都在單一應用程式中，然後建立一個包含每個意圖的最上層應用程式。 使用[調度模型](#dispatch-tool-and-model)生成頂級應用。 然後更改您的自動程式以使用調度[模型教程](https://docs.microsoft.com/azure/bot-service/bot-builder-tutorial-dispatch?view=azure-bot-service-4.0&branch=master&tabs=cs)中所示的級聯調用。 

## <a name="when-you-need-to-combine-several-luis-and-qna-maker-apps"></a>當您需要將數個 LUIS 與 QnA Maker 應用程式結合時
如果您有多個 LUIS 和 QnA 製造商應用需要回應自動程式，請使用[調度模型](#dispatch-tool-and-model)構建頂級應用。  然後更改您的自動程式以使用調度[模型教程](https://docs.microsoft.com/azure/bot-service/bot-builder-tutorial-dispatch?view=azure-bot-service-4.0&branch=master&tabs=cs)中所示的級聯調用。 
...
聊天機器人接收話語，然後發送到父 LUIS 應用進行預測。 父應用中預測的首要意圖決定了下一個調用哪個 LUIS 子應用。 聊天機器人將話語發送到子應用以進行更具體的預測。
...
* [機器人框架 SDK](https://github.com/Microsoft/botframework)
* [調度模型教程](https://docs.microsoft.com/azure/bot-service/bot-builder-tutorial-dispatch?view=azure-bot-service-4.0&branch=master&tabs=cs)
* [調度 CLI](https://github.com/Microsoft/botbuilder-tools)
* 調度模型機器人示例 - [.NET，](https://github.com/microsoft/BotBuilder-Samples/tree/master/samples/csharp_dotnetcore/14.nlp-with-dispatch) [Node.js](https://github.com/microsoft/BotBuilder-Samples/tree/master/samples/javascript_nodejs/14.nlp-with-dispatch)
