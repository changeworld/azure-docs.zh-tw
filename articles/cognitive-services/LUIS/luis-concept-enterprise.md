...
LUIS 具有每月配額以及每秒配額（以 Azure 資源的定價層為基礎）。 

如果您的 LUIS 應用程式要求率超過允許的 [配額費率](https://azure.microsoft.com/pricing/details/cognitive-services/language-understanding-intelligent-services/)，您可以：

* 使用 [相同的應用程式定義](#use-multiple-apps-with-same-app-definition)將負載分散到更多 LUIS 應用程式。 這包括（選擇性）從 [容器](luis-container-howto.md)執行 LUIS。 
...
定型這些同級應用程式時，請務必 [使用所有資料進行定型](luis-how-to-train.md#train-with-all-data)。
...
排程定期審核（例如每兩周），以取得主動式學習的 [端點語句](luis-how-to-review-endpoint-utterances.md) ，然後重新定型並重新發佈。 
