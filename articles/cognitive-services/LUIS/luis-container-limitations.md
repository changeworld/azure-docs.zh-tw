...
LUIS 容器有幾個值得注意的限制。 從不受支援的依賴項到支援的語言子集，本文詳細介紹了這些限制。

## <a name="supported-dependencies-for-latest-container"></a>`latest`容器的支援依賴項

最新的 LUIS 容器發佈于[//build/ 2019，](https://news.microsoft.com/build2019/)將支援：

* [新的預構建域](luis-reference-prebuilt-domains.md)：這些以企業為中心的域包括實體、示例陳述和模式。 擴展這些域供自己使用。

## <a name="unsupported-dependencies-for-latest-container"></a>`latest`容器不受支援的依賴項
...
不支援的應用程式組態|詳細資料|
|--|--|
|不支援的容器文化特性| 荷蘭文`nl-NL`（ ）<br>日語`ja-JP`（ ）<br>德語僅支援[1.0.2 標記器](luis-language-support.md#custom-tokenizer-versions)。|
...
| 語言 | Locale | 預建網域 | 預建實體 | 片語清單建議 | **[文本分析](../text-analytics/language-support.md)<br>(情感和<br>關鍵字)|
|--|--|:--:|:--:|:--:|:--:|
| 美式英文 | `en-US` | ✔️ | ✔️ | ✔️ | ✔️ |
