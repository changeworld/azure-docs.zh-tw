...
ms.locfileid: "96002324"
---
# <a name="language-understanding-luis-container-limitations"></a>Language Understanding (LUIS) 容器限制

LUIS 容器有一些值得注意的限制。 從不支援的相依性到支援的語言子集，本文將詳細說明這些限制。

## <a name="supported-dependencies-for-latest-container"></a>支援的容器相依性 `latest`

最新的 LUIS 容器支援：

* [新的預建網域](luis-reference-prebuilt-domains.md)：這些以企業為主的網域包括實體、範例語句和模式。 擴充這些網域以供您使用。

## <a name="unsupported-dependencies-for-latest-container"></a>不支援的 `latest` 容器相依性

若要 [匯出容器](luis-container-howto.md#export-packaged-app-from-luis)，您必須從您的 LUIS 應用程式中移除不支援的相依性。 當您嘗試匯出容器時，LUIS 入口網站會報告您需要移除的這些不支援功能。

您可以使用 **不含** 下列任何相依性的 LUIS 應用程式：

不支援的應用程式組態|詳細資料|
|--|--|
|不支援的容器文化特性| 荷蘭文 (`nl-NL`) 、日文 (`ja-JP`) 和德文 (`de-DE`) 語言僅支援 [1.0.2 tokenizer](luis-language-support.md#custom-tokenizer-versions)。|
|所有文化特性皆不支援的實體|適用於所有文化特性的 [KeyPhrase](luis-reference-prebuilt-keyphrase.md) 預先建置實體|
|英文 () 文化特性的不支援實體 `en-US`|[GeographyV2](luis-reference-prebuilt-geographyV2.md) 預先建置的實體|
|語音預備|在容器中不支援外部相依性。|
|情感分析|在容器中不支援外部相依性。|
