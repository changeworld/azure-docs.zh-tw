...
* 從已經包含意圖、語句`.lu`和`.json`實體的或檔案匯入 LUIS 應用程式。
...
1. 選擇您的應用程式文化特性，然後選取 [**完成**]。 此時，描述和預測資源是選擇性的。 您可以隨時在入口網站的 [**管理**] 區段中設定。
    ...
    建立應用程式之後，LUIS 入口網站會顯示`None`意圖**清單，** 其中已為您建立意圖。 您現在有一個空的應用程式。 
...
* 將應用程式重新命名
* 使用`.lu`或從檔案匯入`.json`
* 將應用程式`.lu`匯出為（適用于`.json` [LUDown](https://github.com/microsoft/botbuilder-tools/tree/master/packages/Ludown)） `.zip` 、或（適用于[LUIS 容器](luis-container-howto.md)）
* 匯入容器端點記錄以審查端點語句
* 匯出端點記錄作為`.csv`離線分析的
* 刪除應用程式
