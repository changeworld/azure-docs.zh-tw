...
## <a name="what-is-a-feature"></a>什麼是功能

功能是一種區別的特性，可以描述為函式： f （x） = y。 這項功能是用來知道在範例語句中，針對區別特性的外觀。 在建立您的架構時，您對指出特性的範例語句有何瞭解？ 您的答案是建立功能的最佳指南。
...
## <a name="how-you-find-features-in-your-example-utterances"></a>您在範例語句中尋找功能的方式

因為 LUIS 是以語言為基礎的應用程式，所以功能會以文字為基礎。 選擇文字，指出您想要區別的特性。 針對 LUIS，以文字為基礎的最小單位是 token。 對於英文語言，權杖是連續的範圍，不含空格或標點符號，也就是字母和數位。 空格不是 token。

因為空格和標點符號不是 token，所以請將焦點放在您可以用來做為特徵的文字線索上。 請記得包含 word 的變化，如下所示：
* 複數形式
* 動詞時態
* 縮寫
* 拼寫與拼錯
...
以區分特性的形式執行文字，必須：
* 比對完全相同的單字或片語-考慮新增正則運算式實體，或將清單實體當做功能加入至實體或意圖
* 符合知名概念（例如日期、時間或人員名稱）-使用預先建立的實體做為實體或意圖的功能
* 瞭解一段時間的新範例-使用概念的一些範例的片語清單做為實體或意圖的功能

## <a name="combine-features"></a>結合功能

由於特性的描述方式有好幾個選項，因此您可以使用多個有助於描述該特徵或概念的功能。 常見的配對是使用片語清單功能，以及常用來做為特徵的其中一個實體類型：預建實體、正則運算式實體或清單實體。

### <a name="ticket-booking-entity-example"></a>票證預約實體範例

作為第一個範例，請考慮一個應用程式，以使用航班保留意圖和票證預約實體來預約航班。

「票證預約」實體是航班目的地的機器學習實體。 若要協助解壓縮該位置，請使用兩個功能來協助：
* 相關單字的片語清單，例如 `plane` 、 `flight` 、 `reservation` 、`ticket`
* 將 `geographyV2` 實體當做功能預先建立至實體

### <a name="pizza-entity-example"></a>比薩實體範例

另一個範例是，請考慮使用「建立比薩訂單」和「比薩餅」實體訂購比薩的應用程式。

比薩實體是針對比薩詳細資料所學習到的機器實體。 若要協助解壓縮詳細資料，請使用兩個功能來協助：
* 相關單字的片語清單，例如 `cheese` 、 `crust` 、 `pepperoni` 、`pineapple`
* 將 `number` 實體當做功能預先建立至實體

## <a name="a-phrase-list-for-a-particular-concept"></a>特定概念的片語清單

片語清單是封裝特定概念的單字或片語清單，並會在權杖層級套用為不區分大小寫的相符項。
...
片語清單適用于權杖層級，不論大小寫為何。 下圖顯示包含單字的片語清單，如何套用 `Ann` 至該順序中相同字元的變化。


| 的權杖變化`Ann` | 找到權杖時的片語清單相符 |
|--------------------------|---------------------------------------|
| ANN<br>aNN<br>           | 是-token 為`Ann`                  |
| 王小姐                    | 是-token 為`Ann`                  |
| Anne                     | 無-token 為`Anne`                  |
