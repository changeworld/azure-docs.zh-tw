...
---
# <a name="best-practices-for-building-a-language-understanding-luis-app"></a>編譯語言理解 (LUIS) 應用的最佳做法
使用應用程式創作程序產生 LUIS 應用:

* 建構語言模型(意圖與實體)
* 新增一些培訓範例陳述(每個意圖 15-30 個)
* 發佈到終結點
* 從端點測試

[發佈](luis-how-to-publish-app.md)應用后,請使用開發生命週期從終結點添加功能、發佈和測試。 不要透過添加更多範例陳述來開始下一個創作週期,因為這不會讓 LUIS 使用真實使用者陳述瞭解您的模型。
...
|要做的事|不要做的事|
|--|--|
|[定義不同的意圖](#do-define-distinct-intents)<br>[加入描述子](#do-add-descriptors-to-intents) |[將許多範例語句新增至意圖](#dont-add-many-example-utterances-to-intents)<br>[使用很少或簡單的實體](#dont-use-few-or-simple-entities) |
|[找出每個意圖的太攏統與太特定之間的最佳點](#do-find-sweet-spot-for-intents)|[使用 LUIS 作為定型平台](#dont-use-luis-as-a-training-platform)|
|[使用版本反覆運算建構應用](#do-build-your-app-iteratively-with-versions)<br>[產生實體進行模型分解](#do-build-for-model-decomposition)|[新增許多相同格式的範例語句而忽略其他格式](#dont-add-many-example-utterances-of-the-same-format-ignoring-other-formats)|
|[在稍後的反覆運算中加入模式](#do-add-patterns-in-later-iterations)|[混合意圖和實體的定義](#dont-mix-the-definition-of-intents-and-entities)|
|[讓您的語句在所有意圖之間達到平衡](#balance-your-utterances-across-all-intents)，除了 None 意圖之外。<br>[將範例語句新增至 None 意圖](#do-add-example-utterances-to-none-intent)|[建立所有可能值的描述子](#dont-create-descriptors-with-all-the-possible-values)|
|[利用主動式學習的建議功能](#do-leverage-the-suggest-feature-for-active-learning)|[新增太多模式](#dont-add-many-patterns)|
|[透過批次測試測試監控應用的效能](#do-monitor-the-performance-of-your-app)|[新增每一個範例語句都進行定型和發佈](#dont-train-and-publish-with-every-single-example-utterance)|
...
`Book a flight`並使用`Book a hotel`相同的字`book a `彙 。 此格式相同,因此與`flight`和`hotel`提取的實體的不同單詞應具有相同的意圖。

## <a name="do-add-descriptors-to-intents"></a>確實加入描述子

描述符有助於描述意圖的功能。 描述符可以是對該意圖很重要的單詞的短語清單,也可以是該意圖重要的實體。
...
## <a name="do-build-your-app-iteratively-with-versions"></a>使用版本反覆運算建構應用

每個撰寫循環都應該在新[版本](luis-concept-version.md) (從現有版本複製) 之內。

## <a name="do-build-for-model-decomposition"></a>為模型分解進行產生

模型分解的典型過程是:

* 以用戶端應用程式的使用者意圖建立**意圖**
* 從實際使用者輸入新增 15-30 個範例陳述
* 在範例陳述中標註頂級資料概念
* 將資料概念分解為子元件
* 加入子元件加入描述符(功能)
* 向意圖新增描述子(功能)

創建意圖並添加範例陳述後,下面的範例將描述實體分解。

首先確定要在話語中提取的完整數據概念。 這是您的機器學習實體。 然後將短語分解為其部分。 這包括標識子元件(作為實體),以及描述符和約束。

例如,如果要提取位址,可以調用`Address`頂部機器學習實體。 創建位址時,請標識其某些子元件,如街道位址、城市、州和郵政編碼。

通過將郵政編碼**約束**為正則表達式,繼續分解這些元素。 將街道位址分解為街道編號(使用預建編號)、街道名稱和街道類型的部分。 街道類型可以使用**描述符**清單(如大道、圓形、道路和車道)來描述。

V3 創作 API 允許模型分解。

## <a name="do-add-patterns-in-later-iterations"></a>在稍後的反覆運算中加入模式
...
在模型設計開始時添加它們沒有任何壞處,但在使用話語測試模型后,更容易看到每個模式如何更改模型。
...
發佈應用后,僅在開發生命週期過程中添加來自活動學習的話語。 如果語句太類似，則請新增模式。

## <a name="dont-use-few-or-simple-entities"></a>不要使用很少或簡單的實體

實體是為數據提取和預測而構建的。 每個意圖都有機器學習的實體來描述意圖中的數據,這一點很重要。 這有助於 LUIS 預測意圖,即使您的用戶端應用程式不需要使用提取的實體。
...
## <a name="dont-create-descriptors-with-all-the-possible-values"></a>不要建立所有可能值的描述符號

在描述符[短語清單中](luis-concept-feature.md)提供幾個範例,但並不是每個單詞。 LUIS 會將內容一般化並納入考量。
