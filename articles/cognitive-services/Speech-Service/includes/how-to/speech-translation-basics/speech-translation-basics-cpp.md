...
要運行本文中的範例,請在C++代碼檔的頂部`#include``using`包括以下內容和語句。
...
本文中的範例原始碼取決於用於儲存敏感資料的環境變數,例如語音資源訂閱密鑰和地區。 C++代碼檔包含從主機環境變數(即`SPEECH__SUBSCRIPTION__KEY`和`SPEECH__SERVICE__REGION`) 分配的兩個字串值。 這兩個字段都在類作用域中,使它們在類的方法體中可訪問。 關於環境變數的詳細資訊,請參閱[環境變數和應用程式設定](../../../../cognitive-services-security.md#environment-variables-and-application-configuration)。
...
該[`Translations`][translations]詞典可用於從翻譯文本中合成音訊。 反覆運算每個翻譯,並綜合翻譯。 創建`SpeechSynthesizer`實體時,`SpeechConfig`物件需要[`SetSpeechSynthesisVoiceName`][speechsynthesisvoicename]將其 屬性設定為所需的語音。 下面的範例轉換為五種語言,然後每個翻譯都合成到相應的神經語言中的音訊檔。
