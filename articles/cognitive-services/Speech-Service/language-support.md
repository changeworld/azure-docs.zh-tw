...
description: 語音服務支援多種語言進行語音到文本和文字到語音轉換,以及語音翻譯。 本文提供了按服務功能提供語言支援的完整清單。
...
Microsoft 語音 SDK 和 REST API 都支援以下語言(區域設置)。 為了提高準確性,通過上傳"音訊+ 人工標記的腳本"或"相關文本:句子",為語言的子集提供自定義。 發音自訂目前僅適用於`en-US`與`de-DE`。 此處瞭解有關自訂的[更多詳細資訊](how-to-custom-speech.md)。
...
Microsoft 語音 SDK 和 REST API 都支援這些語音,每個語音都支援特定語言和方言,由區域設置標識。
...
| Locale  | Language            | 性別 | 全服務名稱映射                                               | 短語音名稱        |
|---------|---------------------|--------|-------------------------------------------------------------------------|-------------------------|
| `de-DE` | 德文 (德國)    | Female | "微軟伺服器語音文本到語音語音(de-DE,KatjaNeural)"     | "去DE-KatjaNeural"     |
| `en-US` | 英文 (美國)        | Female | "微軟伺服器語音文本到語音語音(en-美國,阿裡亞斯神經)"      | "美國-阿里亞斯神經"      |
...
> [!IMPORTANT]
> 語音`en-US-JessaNeural`已變更為`en-US-AriaNeural`。 如果您以前使用"杰薩",則轉換成"阿里婭"。

要瞭解如何設定和調整神經語音,請參閱[語音合成標記語言](speech-synthesis-markup.md#adjust-speaking-styles)。
...
| Locale | Language | 性別 | 全服務名稱映射 | 簡短名稱 |
|--|--|--|--|--|
...
|  |  | Female | "Microsoft Server Speech Text to Speech Voice (en-IN, PriyaRUS)" | 恩-恩-普里里亞斯" |
...
| `fr-CA` | 法文 (加拿大) | Female | "Microsoft Server Speech Text to Speech Voice (fr-CA, Caroline)" | "fr-CA-卡羅琳" |
...
| `ms-MY` | 馬來文 | Male | "Microsoft Server Speech Text to Speech Voice (ms-MY, Rizwan)" | "ms-MY-里茲萬" |
| `nb-NO` | 挪威文 | Female | "Microsoft Server Speech Text to Speech Voice (nb-NO, HuldaRUS)" | "nb-NO-胡爾達魯斯" |
| `nl-NL` | 荷蘭文 | Female | "Microsoft Server Speech Text to Speech Voice (nl-NL, HannaRUS)" | "nl-NL-漢納魯斯" |
...
> [!IMPORTANT]
> 語音`en-US-Jessa`已變更為`en-US-Aria`。 如果您以前使用"杰薩",則轉換成"阿里婭"。
...
語音`de-DE`自訂可用於`zh-CN``en-US``es-MX``fr-FR``it-IT``pt-BR``en-GB``en-IN`、、、、、、、、、、 和 。 選擇與訓練自定義語音模型時必須的訓練數據相匹配的正確區域設置。 例如,如果您錄製的資料以英文講有英國口音,請`en-GB`選擇 。

> [!NOTE]
> 我們不支援自定義語音的雙語模型培訓,但中英文雙語種除外。 如果你想訓練一個能說英語的漢語語音,請選擇「中英文雙語」。。 所有區域設置中的語音培訓從 2,000 多個話語的數據集開始,`en-US``zh-CN`但 和 位置可以從任何規模的訓練數據開始。

## <a name="speech-translation"></a>語音翻譯

**語音翻譯** API 支援語音轉換語音和語音轉換文字翻譯的不同語言。 源語言必須始終來自"語音到文本"語言表。 可用的目標語言取決於翻譯目標是語音還是文字。 您可以將傳入的語音翻譯為 [60 多種語言](https://www.microsoft.com/translator/business/languages/)。 語言的子集可用於[語音合成](language-support.md#text-languages)。
...
* [取得語音服務試用訂閱](https://azure.microsoft.com/try/cognitive-services/)
