...
測試 Microsoft 語音辨識的正確性或訓練您的自訂模型時，您將需要音訊和文字資料。 在此頁面上，我們將討論自訂語音模型所需的資料類型。

## <a name="data-diversity"></a>資料多樣性

用來測試和定型自訂模型的文字和音訊，必須包含各種不同說話者的樣本，以及您需要模型辨識的案例。
收集自訂模型測試和定型的資料時，請考慮下列因素：

* 您的文字和語音音訊資料必須涵蓋您的使用者在與模型互動時所會進行的各種口頭語句。 例如，可引發並降低溫度需求的模型，可能會對使用者提出要求這類變更的語句進行訓練。
* 您的資料必須包含您的模型需要辨識的所有語音差異。 許多因素可能會改變語音，包括重音、方言、語言混合、年齡、性別、音調、壓力程度，以及一天的時間。
* 您必須在將使用模型的不同環境（室內、戶外、公路雜訊）中包含樣本。
* 音訊必須使用生產系統將使用的硬體裝置進行收集。 如果您的模型需要識別記錄在不同品質的裝置上的語音，則您提供來定型模型的音訊資料也必須代表這些不同的案例。
* 您稍後可以將更多資料新增至模型，但請小心保留資料集的多樣性，並代表您的專案需求。
* 包含*不*在自訂模型辨識需求內的資料可能會損害辨識品質，因此請勿包含您的模型不需要轉譯的資料。

在案例子集上定型的模型，在這些情況下只能順利執行。 請仔細選擇資料，代表您需要自訂模型辨識之案例的完整範圍。

> [!TIP]
> 從小型的樣本資料集開始，符合您的模型會遇到的語言和聲場。
> 例如，在同一個硬體上，記錄一個小型但具代表性的音訊範例，並在您的模型會在生產案例中找到的相同聲場環境中。
> 有代表性資料的小型資料集可能會在您投資收集較大的資料集以進行定型之前，暴露出問題。
