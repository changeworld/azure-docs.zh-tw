...
ms.locfileid: "88185310"
---
# <a name="speech-service-release-notes"></a>語音服務版本資訊

## <a name="new-speech-to-text-locales-2020-august-release"></a>新的語音轉換文字地區設定： 2020-八月發行
語音轉換文字已于8月發行26個新地區設定：2歐洲語言 cs-CZ 和 hu-HU，5個英文地區設定，以及19個西班牙文的地區設定，涵蓋大部分的美國中南部。 以下是新地區設定的清單。 請參閱 [這裡](https://docs.microsoft.com/azure/cognitive-services/speech-service/language-support)的完整語言清單。
...
## <a name="speech-sdk-1130-2020-july-release"></a>語音 SDK 1.13.0： 2020-7 月版本
...
* **神經 tts、15個新的類神經語音**：新增至神經 TTS 組合的新語音會以 `ar-EG` 阿拉伯文 (埃及) Salma、 `ar-SA` 阿拉伯文 (沙烏地阿拉伯) 中的 Zariyah、在加泰羅尼亞文 (西班牙) 、在丹麥 (丹麥) 、英文 (印度) 中的 Christel、 `ca-ES` `da-DK` Neerja `es-IN` `fi-FI` 芬蘭文 (芬蘭) 中的 Noora、印度文中的 Swara、印度文中的 Colette、 `hi-IN` 荷蘭文 `nl-NL` (荷蘭) 、波蘭文 (`pl-PL` 波蘭) 、Fernanda in 葡萄牙文 `pt-PT` (`ru-RU` `sv-SE` `th-TH` `zh-HK` `zh-TW` 葡萄牙) 、印度文中的 Zofia、俄文 (、) 俄羅斯 (、Dariya in 俄羅斯 ()  檢查所有 [支援的語言](https://docs.microsoft.com/azure/cognitive-services/speech-service/language-support#neural-voices)。  
...
* 改善中的 TTS vocoder，以提供更高的精確度和較低的延遲。
    ...
    * 已將中的 Elsa 更新 `it-IT` 為已達到 + 0.464 CMOS 的新 vocoder (的比較表示) 取得語音品質、在合成中提高40%，並在第一個位元組延遲中減少30%。 
...
* 已更新 `de-DE` 和 `ja-JP` 語音模型，讓 TTS 輸出更自然。
    ...
    * 已 `de-DE` 使用最新的韻律模型方法更新中的 Katja，MOS (表示) 增益的意見分數為 + 0.13。 
    * 以 `ja-JP` 新的音調輔色韻律模型更新 Nanami，MOS (表示) 增益的意見分數為 + 0.19;  
...
* 已改善中的讀取位址 `pt-PT` 。
* 已修正 Natasha (`en-AU`) 和 Libby (`en-UK` 在 "for" 和 "四" 一字上) 發音問題。  
* 已修正音訊內容建立工具的 bug
    * 修正第二個段落之後的額外和非預期的暫停。  
    * 「不中斷」功能會從回歸 bug 加回。 
...
- [C \# ](https://github.com/Azure-Samples/cognitive-services-speech-sdk/tree/master/quickstart/csharp/uwp/keyword-recognizer)和[JAVA](https://github.com/Azure-Samples/cognitive-services-speech-sdk/tree/master/quickstart/java/android/keyword-recognizer) (Android) 的關鍵字識別範例。  
...
**新功能**
- Linux：已新增 Red Hat Enterprise Linux (RHEL) /Centos 作為基礎 7 x64 的支援，並提供如何設定系統 for Speech SDK 的 [指示](https://docs.microsoft.com/azure/cognitive-services/speech-service/how-to-configure-rhel-centos-7) 。
...
**新功能**
 ...
 - DialogServiceConnector 現在支援在 BotFrameworkConfig 上使用選擇性的「bot 識別碼」參數。 此參數可讓您使用多個 Direct Line 語音 bot 搭配單一 Azure 語音資源。 若未指定參數，則會使用 [Direct Line 語音通道設定]) 頁面所決定的預設 bot (。
 ...
 - 已更新19個地區設定的語音辨識模型，以減少 18.6% (es 的平均文字錯誤率，es-MX、fr-CA、fr-fr、it-IT、ja-jp、ko、巴西、zh-CN、zh-HK、nb-NO、fi、ru、、pl、、zh-幼圓、第一次、pt 和 tr-TR) 。 新的模型會在多個網域中帶來顯著的改良，包括聽寫、話務中心轉譯和影片索引編制案例。
...
**錯誤修正**
 ...
 - 新增遺漏的 (取得 |將) 屬性方法設定為 AudioConfig
...
- 請參閱 `OpenSSL` 上述重大變更底下的更新。 我們已修正在 Linux 和 JAVA 中高負載) 下 (鎖定爭用的間歇性損毀和效能問題。 
...
- 已針對 JAVA 和 c + + 中的語音辨識 (新增自動來源語言偵測) 
- 已新增 `SourceLanguageConfig` 語音辨識的物件，用來指定 JAVA 和 c + + (的預期來來源語言) 
- `KeywordRecognizer`透過 NuGet 和 Unity 套件，在 Windows (UWP) 、Android 和 iOS 上新增支援
...
- TTS 已新增適用于 JAVA (JRE 和 Android) 、Python、Swift 和目標-C 的系結
...
- 已修正有關 Swift 系結中 null 指定名稱的幾個警告 (可能需要變更較小的程式碼) 
- 已修正在網路負載下導致 websocket 連線關閉強制的錯誤
- 已修正 Android 的問題，有時會導致使用重複的印象識別碼 `DialogServiceConnector`
- 改善跨多回合互動的連線，以及透過事件在發生錯誤時的報告 `Canceled`)  (`DialogServiceConnector`
...
- SDK 現在支援 MP3 和 Opus/OGG 音訊檔案作為串流輸入檔。 這項功能僅適用于 c + + 和 c # 的 Linux，且目前為 Beta (更多 [詳細資料) ](how-to-use-codec-compressed-audio-input-streams.md) 。
...
- 如需其他開發和偵錯工具支援，您可以將 SDK 記錄和診斷資訊重新導向至記錄檔， ([這裡](how-to-use-logging.md)) 的詳細資訊。
- JavaScript：改善音訊處理效能。
...
- `Microsoft.CognitiveServices.Speech.csharp.bindings.dll`先前) 版本中所附的檔案 (不再需要。 此功能現在已整合到 core SDK 中。
...
- `Connection` 物件
  - `Recognizer`您可以從存取 `Connection` 物件。 此物件可讓您明確地起始服務連線，並訂閱連線和中斷連線事件。
     (這項功能尚未從 JavaScript 和 Python 提供。 ) 
- Ubuntu 18.04 支援。
- Android
...
- 已修正在某些情況下，在 JavaScript 中) 的錯誤，導致無法設定 `EndpointId` (。
