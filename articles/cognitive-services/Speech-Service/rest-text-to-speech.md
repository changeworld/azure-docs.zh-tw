---
title: 文本到語音 API 引用 （REST） - 語音服務
titleSuffix: Azure Cognitive Services
description: 瞭解如何使用文本到語音轉換 REST API。 在本文中，您會了解到授權選項、查詢選項，以及如何建構要求與接收回應。
...
ms.locfileid: "80131641"
---
# <a name="text-to-speech-rest-api"></a>文字轉換語音 REST API

語音服務允許您[將文本轉換為合成語音](#convert-text-to-speech)，並使用一組 REST API[獲取區域支援的聲音清單](#get-a-list-of-voices)。 每個可用終結點都與區域相關聯。 需要計畫使用的終結點/區域的訂閱金鑰。
...
## <a name="get-a-list-of-voices"></a>獲取語音清單
...
終結點`v1`允許您使用[語音合成標記語言 （SSML）](speech-synthesis-markup.md)將文本轉換為語音。
...
> [!NOTE]
> 如果您選取的語音和輸出格式具有不同的位元速率，則會視需要重新進行音訊取樣。 但是，24 kHz 語音不支援`audio-16khz-16kbps-mono-siren`和`riff-16khz-16kbps-mono-siren`輸出格式。
...
此 HTTP 要求使用 SSML 指定語音與語言。 如果正文長度很長，並且生成的音訊超過 10 分鐘 - 則將其截斷為 10 分鐘。 換句話說，音訊長度不能超過 10 分鐘。
...
| 415 | 不支援的媒體類型 | 可能提供了錯誤`Content-Type`。 `Content-Type`應設置為`application/ssml+xml`。 |
| 429 | 太多要求 | 您已超出訂用帳戶允許的配額或要求率。 |
| 502 | 錯誤的閘道    | 網路或伺服器端問題。 也可能表示標頭無效。 |
