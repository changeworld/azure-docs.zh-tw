...
在本文中，您可以選擇一個選項，以便輕鬆免費測試語音服務，以便了解該服務可以做什麼，並決定它是否適合您的需要。 根據您的情況和用例選擇以下兩個選項之一：

- [選項 1：](#no-card)立即獲取**免費試用**API 金鑰，而無需提供任何信用卡資訊（您需要具有現有的 Azure 帳戶）。 **免費試用**將持續 30 天，資料在末尾被刪除。 此選項最適合快速試驗服務。
- [選項 2：](#new-resource)使用**免費訂閱**（需要信用卡資訊）在 Azure 中免費創建新的語音資源。 **免費訂閱**主要比付費訂閱具有更嚴格的費率限制。 如果要測試服務，但計畫將來升級到付費訂閱，並且不想遺失資料，則此選項是最佳選擇。

## <a name="try-the-speech-service-without-credit-card-info"></a><a id="no-card"></a>嘗試沒有信用卡資訊的語音服務

完成以下步驟以啟動 30 天的免費試用版並獲得 API 金鑰。 完成以下步驟後，試用期立即開始。

1. 轉到[嘗試認知服務](https://azure.microsoft.com/try/cognitive-services/)。
1. 選取 [語音識別 API]**** 索引標籤。
1. 選擇**獲取 API 金鑰**。

您將看到帳單選項。 選擇免費選項，然後讀取和批准使用者協定。 您將獲得可用於免費試用語音服務的金鑰 30 天。

## <a name="try-the-speech-service-by-creating-an-azure-resource"></a><a id="new-resource"></a>通過創建 Azure 資源嘗試語音服務
...
>
> 如果您使用免費、低容量的語音服務層，即使在免費試用或服務信用額度到期後，您也可以保留此免費訂閱。
>
...
1. 選取入口網站左上方的 [建立資源]****。 如果未看到 **"創建資源"，** 則始終可以通過選擇左上角的折疊功能表來找到它：

   ![折疊導覽按鈕](media/index/collapsed-nav.png)

1. 在 **"新建"** 視窗中，在搜索框中鍵入"語音"，然後按 ENTER。
...
部署新的語音資源需要一些時間。 部署完成後，選擇 **"轉到資源"，** 在左側功能窗格中選擇 **"鍵**"以顯示語音服務訂閱金鑰。 每個訂用帳戶都有兩個金鑰，您可以在應用程式中使用任一個金鑰。 要快速將金鑰複製到代碼編輯器或其他位置，請選擇每個鍵旁邊的複製按鈕，切換視窗將剪貼簿內容粘貼到所需位置。

> [!IMPORTANT]
> 這些訂閱金鑰用於訪問您的認知服務 API。 不要共用您的金鑰。 安全存儲它們* 例如，使用 Azure 金鑰保存庫。 我們還建議定期重新生成這些金鑰。 進行 API 呼叫只需要一個金鑰。 重新生成第一個鍵時，可以使用第二個鍵繼續訪問服務。
...
> [語音 SDK 示例中識別語音](speech-sdk.md#get-the-samples)
