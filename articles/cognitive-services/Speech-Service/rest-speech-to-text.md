...
| `format` | 指定結果格式。 接受的值為 `simple` 和 `detailed`。 簡單的結果包含 `RecognitionStatus`、`DisplayText`、`Offset` 和 `Duration`。 詳細的回應包含四種不同的顯示文字標記法。 預設值是 `simple`。 | 選用 |
...
下列範例程式碼示範如何在 URL 查詢參數中建立發音評估參數：
...
此程式碼範例說明如何以區塊傳送音訊。 只有第一個區塊應該包含音訊檔案的標頭。 `request`是連接`HttpWebRequest`到適當 REST 端點的物件。 `audioFile` 是音訊檔案在磁碟上的路徑。
...
| `PronScore` | 表示指定語音之發音品質的整體分數。 這是從`AccuracyScore`、 `FluencyScore`和加權`CompletenessScore`計算而來。 |
| `ErrorType` | 這個值會指出相較于`ReferenceText`，是否省略、插入或發音不正確的字。 可能的值`None`為（表示此單字上沒有錯誤） `Omission`、 `Insertion`和`Mispronunciation`。 |
