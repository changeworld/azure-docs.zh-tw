...
以滑鼠右鍵按一下 _方案總管_ 中的專案，然後選擇 [管理使用者密碼]。 這樣會開啟名稱為 _secrets.json_ 的檔案。 此檔案不會簽入原始檔控制。 [在此](/aspnet/core/security/app-secrets?tabs=windows)深入了解。 以下列內容取代 secrets.json 的內容，並使用您建立沈浸式閱讀程式資源時提供的值。
