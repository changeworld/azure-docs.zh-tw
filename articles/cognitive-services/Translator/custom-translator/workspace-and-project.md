...
專案是模型、文件和測試的包裝函式。 每個專案都會自動包含上傳到該工作區且具有正確語言組的所有文件。 例如，如果您同時有英文翻譯成西班牙文的專案和西班牙文翻譯成英文的專案，這兩個專案將會包含相同的文件。 每個專案都有其相關聯的 CategoryID，供查詢 [V3 API](../reference/v3-0-translate.md?tabs=curl) 的翻譯時使用。 CategoryID 是一個參數，用來從使用 Custom Translator 建置的自訂系統取得翻譯。
