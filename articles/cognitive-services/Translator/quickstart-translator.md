...
description: 了解如何使用翻譯工具服務來翻譯文字、進行文字音譯、偵測語言等等。 提供的範例適用於 C#、Java、JavaScript 和 Python。
...
在本快速入門中，您將了解如何透過 REST 使用翻譯工具服務。 您將先從基本範例著手，接著再了解一些在開發期間常用的核心設定選項，包括：

* [翻譯](#translate-text)
* [轉換](#transliterate-text)
* [語言識別/偵測](#detect-language)
* [計算句子長度](#get-sentence-length)
* [取得替代翻譯](#dictionary-lookup-alternate-translations)和[句子中的文字使用範例](#dictionary-examples-translations-in-context)

## <a name="prerequisites"></a>必要條件

* Azure 訂用帳戶 - [建立免費帳戶](https://azure.microsoft.com/free/cognitive-services/)
* 擁有 Azure 訂用帳戶之後，請在 Azure 入口網站中[建立翻譯工具資源](https://ms.portal.azure.com/#create/Microsoft.CognitiveServicesTextTranslation)，以取得您的金鑰和端點。 在其部署後，選取 [前往資源]。
  * 您將需要來自資源的金鑰和端點，以將應用程式連線至翻譯工具服務。 您稍後會在快速入門中將金鑰和端點貼到下列程式碼中。
...
* 建立新的專案：`dotnet new console -o your_project_name`
* 將 Program.cs 取代為如下所示的 C# 程式碼。
* 在 Program.cs 中設定訂用帳戶金鑰和端點值。
* [使用 .NET CLI 新增 Newtonsoft.Json](https://www.nuget.org/packages/Newtonsoft.Json/)。
* 從專案目錄執行程式：``dotnet run``

# <a name="go"></a>[Go](#tab/go)

* 在您慣用的程式碼編輯器中，建立新的 Go 專案。
* 新增下方提供的程式碼。
* 以訂用帳戶有效的存取金鑰來取代 `subscriptionKey` 值。
* 以 '.go' 副檔名儲存檔案。
* 在已安裝 Go 的電腦上開啟命令提示字元。
* 建置檔案，例如：'go build example-code.go'。
* 執行檔案，例如：'example-code'。

# <a name="java"></a>[Java](#tab/java)

* 為您的專案建立工作目錄。 例如： `mkdir sample-project` 。
* 使用 Gradle 將專案初始化：`gradle init --type basic`。 出現選擇 **DSL** 的提示時，請選取 [Kotlin]。
* 更新 `build.gradle.kts`。 切記，您必須根據範例更新自己的 `mainClassName`。
  ```java
  plugins {
    java
    application
  ...
  ```
* 建立 Java 檔案，並從提供的範例中複製程式碼。 別忘了新增您的訂用帳戶金鑰。
* 執行範例：`gradle run`。

# <a name="nodejs"></a>[Node.js](#tab/nodejs)

* 在您慣用的 IDE 或編輯器中建立新專案。
* 將其中一個範例中的程式碼複製到您的專案中。
* 設定您的訂用帳戶金鑰。
* 執行程式。 例如： `node Translate.js` 。
...
* 將其中一個範例中的程式碼複製到您的專案中。
* 設定您的訂用帳戶金鑰。
* 執行程式。 例如： `python translate.py` 。

---

## <a name="headers"></a>標題 

透過 REST 呼叫翻譯工具服務時，您必須確定每個要求中都包含下列標頭。 別擔心，我們將在以下各節的範例程式碼中包含標頭。 
...
<table width="100%">
  ...
  <tr>
    ...
    <td><em>選擇項</em>。<br/>用於識別唯一要求的 GUID，由用戶端產生。 若您使用名為 <code>ClientTraceId</code> 的查詢參數在查詢字串中包含追蹤識別碼，您就可以省略此標頭。</td>
  </tr>
</table> 
...
翻譯工具服務的核心作業是翻譯文字。 在本節中，您將建置一個採用單一來源 (`from`) 並提供兩個輸出 (`to`) 的要求。 然後，我們將探討一些可用來調整要求和回應的參數。 
...
如果您確知您將需要翻譯，但不知道將傳送至翻譯工具服務的文字所屬的語言，您可以使用語言偵測作業。 有多種方式可以識別來源文字語言。 在本節中，您將了解如何使用 `translate` 端點和 `detect` 端點進行語言偵測。 

### <a name="detect-source-language-during-translation"></a>在翻譯期間偵測來源語言

如果您的翻譯要求中未包含 `from` 參數，翻譯工具服務將會嘗試偵測來源文字的語言。 在回應中，您會得到偵測到的語言 (`language`) 和信賴分數 (`score`)。 `score` 越接近 `1.0`，偵測正確性的信賴度就越高。
...
使用 `/detect` 端點時，回應會包含替代偵測，且會讓您知道所有偵測到的語言是否支援轉譯和音譯。 呼叫成功後，您應該會看見下列回應： 
...
## <a name="transliterate-text"></a>音譯文字 
...
### <a name="transliterate-during-translation"></a>在翻譯期間進行音譯
...
### <a name="transliterate-without-translation"></a>進行音譯而不翻譯
...
呼叫成功後，您應該會看見下列回應。 不同於呼叫 `translate` 端點， `transliterate` 只會傳回 `script` 和輸出 `text`。 
...
## <a name="get-sentence-length"></a>取得句子長度

透過翻譯工具服務，您可以取得單一句子或一系列句子的字元計數。 回應會以陣列的形式傳回，且每個偵測到的句子都會附有字元計數。 您可以使用 `translate` 和 `breaksentence` 端點來取得句子長度。

### <a name="get-sentence-length-during-translation"></a>在翻譯期間取得句子長度

您可以使用 `translate` 端點，取得來源文字和翻譯輸出的字元計數。 若要傳回句子長度 (`srcSenLen` 和 `transSenLen`)，您必須將 `includeSentenceLength` 參數設定為 `True`。
...
呼叫成功後，您應該會看見下列回應。 除了偵測到的來源語言和翻譯以外，偵測到的每個句子也都會附有來源 (`srcSentLen`) 和轉譯 (`transSentLen`) 的字元計數。
...
### <a name="get-sentence-length-without-translation"></a>取得句子長度而不進行轉譯
...
## <a name="dictionary-lookup-alternate-translations"></a>字典查閱 (替代翻譯)

透過端點，您就可以取得單字或片語的替代翻譯。 例如，將 "shark" 這個字從 `en` 翻譯為 `es`時，此端點會同時傳回 "tiburón" 和 "escualo"。
...
呼叫成功後，您應該會看見下列回應。 我們將加以細分，因為 JSON 比本文中的某些其他範例更為複雜。 `translations` 陣列包含翻譯的清單。 此陣列中的每個物件都包含信賴分數 (`confidence`)、針對使用者顯示進行最佳化的文字 (`displayTarget`)、標準化文字 (`normalizedText`)、語音的部分 (`posTag`)，以及先前翻譯的相關資訊 (`backTranslations`)。 如需回應的詳細資訊，請參閱[字典查閱](reference/v3-0-dictionary-lookup.md)
...
## <a name="dictionary-examples-translations-in-context"></a>字典範例 (上下文的翻譯)

執行字典查閱之後，您可以將來源文字和翻譯傳遞至 `dictionary/examples` 端點，以取得在句子或片語的上下文中顯示雙方字詞的範例清單。 以先前的範例為基礎，您將分別使用字典查閱回應中的 `normalizedText` 和 `normalizedTarget` 作為 `text` 和 `translation`。 您需要來源語言 (`from`) 和輸出目標 (`to`) 參數。 
...
呼叫成功後，您應該會看見下列回應。 如需回應的詳細資訊，請參閱[字典查閱](reference/v3-0-dictionary-examples.md)
...
* [了解 API 如何計算字元](character-counts.md)
* [自訂及改善翻譯](customization.md)

## <a name="see-also"></a>另請參閱

* [翻譯工具 v3 API 參考](reference/v3-0-reference.md)
