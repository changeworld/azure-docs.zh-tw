...
ms.locfileid: "79221393"
---
# <a name="install-and-run-anomaly-detector-containers-preview"></a>安裝並運行異常檢測器容器（預覽版）

異常檢測器具有以下容器功能：

| 函式 | 特性 |
|--|--|
| 異常檢測器 | <li> 即時檢測異常。 <li> 檢測整個資料集的異常情況，作為批次處理。 <li> 推斷資料的預期正常範圍。 <li> 支援異常檢測靈敏度調整，以更好地適合您的資料。 |
...
|異常檢測器資源 |若要使用這些容器，您必須具備：<br><br>Azure_異常檢測器_資源，用於獲取關聯的 API 金鑰和終結點 URI。 這兩個值都可以在 Azure 門戶的**異常檢測器**概述和金鑰頁上使用，並且需要啟動容器。<br><br>**[API_KEY]**：**金鑰**頁上的兩個可用資源鍵之一<br><br>**[ENDPOINT_URI]**：**概述**頁上提供的終結點|
...
### <a name="docker-pull-for-the-anomaly-detector-container"></a>異常檢測器容器的塢取拉
...
[命令的示例可用。](anomaly-detector-container-configuration.md#example-docker-run-commands) `docker run`
...
### <a name="running-multiple-containers-on-the-same-host"></a>在同一主機上運行多個容器
...
* 異常檢測器為 Docker 提供了一個 Linux 容器，通過批次處理與流式處理、預期範圍推理和靈敏度調優封裝異常檢測。
* 容器映射從專用於容器預覽的專用 Azure 容器註冊表下載。
* 容器映像是在 Docker 中執行。
* 通過指定容器的主機 URI，可以使用 REST API 或 SDK 在異常檢測器容器中叫用作業。
* 將容器具現化時，您必須指定帳單資訊。

> [!IMPORTANT]
> 認知服務容器在未連線至 Azure 以進行計量的情況下，將無法被授權以執行。 客戶必須啟用容器以持續與計量服務進行帳單資訊的通訊。 認知服務容器不會向 Microsoft 發送客戶資料（例如，正在分析的時間序列資料）。
