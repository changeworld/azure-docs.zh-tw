...
多語系和表情支援導致 Unicode 編碼，使用一個以上的程式[代碼點](https://wikipedia.org/wiki/Code_point)來表示單一顯示的字元，稱為語素簇。 例如，emoji （如🌷和 👍）可能會使用數個字元，以視覺屬性的其他字元（例如面板色調）來撰寫圖形。 同樣地，印度文單字 `अनुच्छेद` 會編碼為五個字母和三個組合標記。
