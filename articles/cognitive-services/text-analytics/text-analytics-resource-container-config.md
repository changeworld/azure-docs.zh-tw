...
文字分析為每個容器提供了常見的設定架構，因此您可以輕易地設定及管理您容器的儲存體、記錄和遙測，以及安全性設定。 同時也提供數個[範例 docker run 命令](how-tos/text-analytics-how-to-install-containers.md#run-the-container-with-docker-run)。
...
> [!IMPORTANT]
> [`ApiKey`](#apikey-configuration-setting)、和 [`Billing`](#billing-configuration-setting) [`Eula`](#eula-setting) 設定會一起使用，而且您必須為其中三個提供有效的值，否則您的容器將不會啟動。 如需使用這些組態設定來將容器具現化的詳細資訊，請參閱[帳單](how-tos/text-analytics-how-to-install-containers.md#billing)。
