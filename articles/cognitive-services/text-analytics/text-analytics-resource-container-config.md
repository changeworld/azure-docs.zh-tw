...
> [!IMPORTANT]
> [`Billing`](#billing-configuration-setting) [`Eula`](#eula-setting)和[`ApiKey`](#apikey-configuration-setting)設置一起使用，您必須為所有三個設置提供有效值;因此，您必須為所有三個設置提供有效值。否則您的容器將無法啟動。 如需使用這些組態設定來將容器具現化的詳細資訊，請參閱[帳單](how-tos/text-analytics-how-to-install-containers.md#billing)。
...
`Billing` 設定會指定 Azure 上_文字分析_資源的端點 URI，用來對容器計算帳單資訊。 您必須為此配置設置指定值，並且該值必須是 Azure 上 @_文本分析_資源的有效終結點 URI。 容器會每隔 10 到 15 分鐘回報使用量。

此設定可在下列位置找到：

* Azure 門戶：**文本分析**概述，已標記`Endpoint`

|必要| 名稱 | 資料類型 | 描述 |
|--|------|-----------|-------------|
|是| `Billing` | String | 計費終結點 URI。 有關獲取計費 URI 的詳細資訊，請參閱[收集所需的參數](how-tos/text-analytics-how-to-install-containers.md#gathering-required-parameters)。 如需詳細資訊和完整的區域端點清單，請參閱[認知服務的自訂子網域名稱](../cognitive-services-custom-subdomains.md)。 |
...
| 預留位置 | 值 | 格式或範例 |
|-------------|-------|---|
| **[API_KEY]** | Azure`Text Analytics`金鑰頁上可用的`Text Analytics`資源的終結點鍵。 |`xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx`|
| **[ENDPOINT_URI]** | 計費終結點值在 Azure`Text Analytics`概述頁上可用。| 有關顯式示例，請參閱[收集所需的參數](how-tos/text-analytics-how-to-install-containers.md#gathering-required-parameters)。 |
