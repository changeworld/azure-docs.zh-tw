---
title: 快速入門：使用電腦視覺 3.1 REST API Read 作業和 C# 來擷取文字
titleSuffix: Azure Cognitive Services
description: 在本快速入門中，使用電腦視覺 3.1 REST API Read 作業和 C#，將 OCR 套用至影像。
...
在 Visual Studio 中建立範例：

1. 在 Visual Studio 中，使用 Visual C# 主控台應用程式範本建立新的 Visual Studio 解決方案。
2. 安裝 Newtonsoft.Json NuGet 套件。
    1. 在功能表中，按一下 [工具]，選取 [NuGet 套件管理員]，然後選取 [管理解決方案的 NuGet 套件]。
    2. 按一下 [瀏覽] 索引標籤，然後在 [搜尋] 方塊中鍵入 "Newtonsoft.Json"。
    3. 顯示時選取 [Newtonsoft.Json]，按一下專案名稱旁邊的核取方塊，然後按一下 [安裝]。
3. 將下列程式碼複製並貼到解決方案中的 Program.cs 檔案內。
1. 將 `imageFilePath` 設定為您自己的映像路徑。 您可以下載要使用的[範例映像](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/master/articles/cognitive-services/Computer-vision/Images/readsample.jpg)。
4. 執行程式。
