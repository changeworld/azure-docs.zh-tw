---
title: 語言支援
titleSuffix: Azure Cognitive Services
description: Azure 認知服務可讓您建置用來觀察、聆聽、了解使用者以及與其交談的應用程式。 這些服務共支援三十餘種語言，讓使用者能夠自然地與您的應用程式溝通。
services: cognitive-services
author: erhopf
manager: nitinme
ms.service: cognitive-services
ms.topic: conceptual
ms.date: 03/31/2020
ms.author: erhopf
ms.custom: seodec18
ms.openlocfilehash: d103b22338672f031fa4f91349a8376f86b703c8
ms.sourcegitcommit: 58faa9fcbd62f3ac37ff0a65ab9357a01051a64f
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 04/29/2020
ms.locfileid: "81530961"
---
# <a name="natural-language-support-for-azure-cognitive-services"></a>Microsoft 認知服務的自然語言支援

Azure 認知服務可讓您建立應用程式，以查看、聆聽、說話及瞭解您的使用者。 這些服務共支援三十餘種語言，讓使用者能夠自然地與您的應用程式溝通。 使用下列連結以依服務來觀看語言可用性。

這些認知服務與語言無關，而且沒有以人類語言為基礎的限制。

* [異常偵測器 (預覽)](./anomaly-detector/index.yml)
* [自訂視覺](./Custom-Vision-Service/index.yml)
* [臉部](./face/index.yml)
* [個人化工具](./personalizer/index.yml)

## <a name="vision"></a>視覺

* [電腦視覺](./computer-vision/language-support.md)
* [表單辨識器 (預覽)](./form-recognizer/language-support.md)
* [筆跡辨識器 (預覽)](./ink-recognizer/language-support.md)
* [影片索引子](https://docs.microsoft.com/azure/media-services/video-indexer/language-identification-model#guidelines-and-limitations)

## <a name="language"></a>Language

* [沈浸式閱讀程式 (預覽)](./immersive-reader/language-support.md)
* [語言理解 (LUIS)](./luis/luis-supported-languages.md)
* [QnA Maker](./qnamaker/overview/languages-supported.md)
* [文字分析](./text-analytics/text-analytics-supported-languages.md)
* [翻譯文字](./translator/languages.md)

## <a name="speech"></a>語音

* [語音服務：語音轉文字](./speech-service/supported-languages.md#speech-to-text)
* [語音服務：文字轉語音](./speech-service/supported-languages.md#text-to-speech)
* [語音服務：語音翻譯](./speech-service/supported-languages.md#speech-translation)

## <a name="search"></a>搜尋

* [Bing 自訂搜尋](./bing-custom-search/supported-countries-markets.md)
* [Bing 影像搜尋](./bing-image-search/supported-countries-markets.md)
* [Bing 新聞搜尋](./bing-news-search/supported-countries-markets.md)
* [Bing 自動建議](./Bing-Autosuggest/bing-autosuggest-supported-languages.md)
* [Bing 拼字檢查](./bing-spell-check/bing-spell-check-supported-languages.md)
* [Bing 圖像式搜尋](./bing-visual-search/supported-countries-markets.md)
* [Bing Web 搜尋](./bing-web-search/supported-countries-markets.md)

## <a name="decision"></a>決策

* [內容仲裁](./content-moderator/text-moderation-api-languages.md)

## <a name="see-also"></a>請參閱

* [認知服務是什麼？](welcome.md)
* [建立帳戶](cognitive-services-apis-create-account.md)
