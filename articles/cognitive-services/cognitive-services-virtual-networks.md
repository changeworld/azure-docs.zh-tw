...
請在表單`16.17.18.0/24`中使用[CIDR 標記法](https://tools.ietf.org/html/rfc4632)或個別 IP 位址（例如`16.17.18.19`），以提供允許的網際網路位址範圍。

   > [!Tip]
   > 不支援使用 "/31" 或 "/32" 前置詞大小的小型位址範圍。 這些範圍應該使用個別的 IP 位址規則設定。

只有**公用網際網路** IP 位址允許使用 IP 網路規則。 IP 規則中不允許保留私人網路的 IP 位址範圍 (如 [RFC 1918](https://tools.ietf.org/html/rfc1918#section-3) 中所定義)。 專用`10.*`網包括以、 `172.16.*`  -  `172.31.*`和`192.168.*`開頭的位址。
