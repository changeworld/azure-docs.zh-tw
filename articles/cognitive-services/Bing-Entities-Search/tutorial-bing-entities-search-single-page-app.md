...
在本教學課程中，我們只會討論原始程式碼的特定部分。 完整的原始程式碼位於[另一個頁面上]()。 將此程式碼複製並貼到文字編輯器，並儲存為 `bing.html`。
...
Bing 實體搜尋 API [要求您依指定順序顯示結果](../bing-web-search/use-display-requirements.md)。 由於 API 可能傳回兩種不同的回應，因此逐一查看 JSON 回應中的最上層 `Entities` 或 `Places` 集合及顯示這些結果並不夠。 (若您只需要一種結果，請使用 `responseFilter` 查詢參數)。
