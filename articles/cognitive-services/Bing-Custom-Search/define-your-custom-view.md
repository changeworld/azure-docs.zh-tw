...
description: 門戶允許您創建指定 Web 切片的搜索實例;但是，它指定了 Web 的切片。域、子頁和網頁。
...
| 配量名稱 | 描述                                                                                                                                                                                                                                                                                                |
|------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 網域     | 網域配量包括在某個網際網路網域內找到的所有內容。 例如： `www.microsoft.com` 。 省略 `www.` 會使得 Bing 也搜尋該網域的子網域。 例如，如果指定`microsoft.com`， 必應還會返回`support.microsoft.com`或`technet.microsoft.com`中的結果。 |
...
將網頁切片添加到 **"活動"** 清單後，"必應自訂搜索"門戶將在選項卡底部生成網站和子頁面建議。這些切片是必應自訂搜索認為您可能希望包括的。 按一下 [重新整理]**** 以在更新自訂搜尋執行個體的設定之後，取得更新後的建議。 有可用的建議時，才看得見這個區段。
...
您可以使用 **"活動"** 清單中的排名**調整**控制項或使用"預覽"窗格中的"提升"和"降級"控制項來超級提升、提升或降級專案。 服務會在 [使用中] 清單中加入配量，並據此調整順位。
...
門戶還允許您使用 **"固定"** 選項卡將 URL 固定到特定搜索詞的搜尋結果頂部。輸入 URL 和查詢以指定將作為頂部結果顯示的網頁。 請注意，每個搜尋查詢最多可以釘選一個網頁，而且只有已編製索引的網頁會顯示在搜尋中。 釘選結果不適用於影像或影片搜尋。

您有兩種方式可將網頁釘選到頂端：

* 在 **"固定"** 選項卡中，輸入要固定到頂部的網頁的 URL 及其相應的查詢。

* 在 [預覽]**** 窗格中輸入搜尋查詢，然後按一下搜尋。 查找要為查詢固定的網頁，然後按一下 **"固定到頂部**"。 網頁和查詢都會新增至 [已釘選]**** 清單。
