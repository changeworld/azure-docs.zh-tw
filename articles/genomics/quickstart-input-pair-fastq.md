...
description: 如果輸入檔是一對 FASTQ 檔，本文將演示如何向 Microsoft 基因組服務提交工作流。
services: genomics
author: grhuynh
manager: cgronlun
ms.author: grhuynh
...
如果輸入檔是一對 FASTQ 檔，本文將演示如何向 Microsoft 基因組服務提交工作流。 本主題假設您已安裝並執行 `msgen` 用戶端，且熟悉如何使用 Azure 儲存體。 如果您已成功使用提供的示例資料提交工作流，則可以繼續執行本文。 
...
在本文中，您已將一組 FASTQ 檔案上傳到 Azure 儲存體，並已透過 `msgen` Python 用戶端將工作流程提交至 Microsoft Genomics 服務。 要瞭解有關工作流提交和其他命令（您可以使用 Microsoft 基因組學服務）的更多內容，請參閱我們的[常見問題解答](frequently-asked-questions-genomics.md)。 
