...
1. 在快顯視窗中，將 [**裝置識別碼**] 輸入為 `SampleDevKit` ，並將 [**裝置名稱**] 輸入為 `MXChip IoT DevKit - Sample` 。 請確定**模擬**選項為 [關閉]。 然後選取 [建立]。
...
1. 開啟您的網頁瀏覽器，並流覽至 [http://192.168.0.1/](http://192.168.0.1/) 。 以下網頁隨即顯示：
