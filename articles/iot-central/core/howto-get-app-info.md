---
title: 取得 Azure IoT Central 應用程式版本的相關資訊 |Microsoft Docs
description: 如何取得您所使用之 IoT Central 應用程式的相關資訊
services: iot-central
author: dominicbetts
ms.author: dobett
ms.date: 06/26/2020
ms.topic: how-to
ms.service: iot-central
ms.openlocfilehash: 12ba8526be68d2cb15e37510e66f959ab65c6e99
ms.sourcegitcommit: 877491bd46921c11dd478bd25fc718ceee2dcc08
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 07/02/2020
ms.locfileid: "85802012"
---
# <a name="about-your-application"></a>關於您的應用程式

本文說明如何取得 IoT Central 應用程式的相關資訊。 您可能需要：

- 這是您聯繫支援人員的資訊。
- 您的應用程式用來在 Azure 入口網站中尋找帳單資訊的 Azure 訂用帳戶。
- 當您使用 REST API 時，應用程式的識別碼。
- 用來完成工作的應用程式版本，例如新增連接器。

## <a name="get-information-about-your-application"></a>取得應用程式的相關資訊

若要取得 IoT Central 應用程式的相關資訊：

1. 選取頂端功能表上的 [說明 **] 連結。**

1. 選取 [**關於您的應用程式**]。

1. [**關於您的應用**程式] 頁面會顯示應用程式的相關資訊：  

    :::image type="content" source="media/howto-get-app-info/about-your-app.png" alt-text="關於您的應用程式螢幕擷取畫面":::

    使用 [**複製資訊**] 按鈕，將資訊複製到剪貼簿。

## <a name="next-steps"></a>後續步驟

既然您已經知道如何找到 IoT Central 應用程式的版本，建議的下一個步驟是繼續探索系統管理員的操作說明文章：[變更 IoT Central 的應用程式設定](howto-administer.md)。
