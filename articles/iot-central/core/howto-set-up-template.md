...
[IoT 隨插即用（預覽）](../../iot-pnp/overview-iot-plug-and-play.md)可讓 IoT Central 整合裝置，而不需要撰寫任何內嵌的裝置程式碼。 IoT 隨插即用 (預覽) 的核心是說明裝置功能的裝置功能模型結構描述。 在 IoT Central 應用程式中，裝置範本會使用這些 IoT 隨插即用 (預覽) 裝置功能模型。
