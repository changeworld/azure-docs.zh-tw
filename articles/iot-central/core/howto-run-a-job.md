...
1. 選取 **[下一步]** 以移至 [ **傳遞選項** ] 頁面。 [ **傳遞選項** ] 頁面可讓您設定此作業的傳遞選項： **批次** 和 **取消閾值**。

    批次可讓您錯開大量裝置的工作。 作業會分割成多個批次，而且每個批次都會包含裝置的子集。 批次會排入佇列，並依序執行。

    如果錯誤數目超過設定的限制，取消閾值可讓您自動取消作業。 閾值可以套用至作業中的所有裝置，或個別批次。

    :::image type="content" source="media/howto-run-a-job/job-wizard-delivery-options.png" alt-text="作業嚮導 [傳遞選項] 頁面的螢幕擷取畫面":::

1. 選取 **[下一步]** 以移至 [ **排程** ] 頁面。 [ **排程** ] 頁面可讓您啟用排程，以在未來執行作業：

    選擇排程的迴圈選項。 您可以設定要執行的作業：

    * 一次性
    * 每日
    * 每週

    設定排程工作的開始日期和時間。 日期和時間是您的時區特有的，而不是裝置的當地時間。
    ...
    * **之後** 設定執行作業的次數。
    ...
    排程的工作一律會在裝置群組中的裝置上執行，即使裝置群組成員資格會隨著時間而變更。

    :::image type="content" source="media/howto-run-a-job/job-wizard-schedule.png" alt-text="作業嚮導 [排程選項] 頁面的螢幕擷取畫面":::

1. 選取 **[下一步]** 以移至 [ **審核** ] 頁面。 [ **審核** ] 頁面會顯示作業設定詳細資料。 選取 **排程** 以排程作業：

    :::image type="content" source="media/howto-run-a-job/job-wizard-schedule-review.png" alt-text="排程工作嚮導審核頁面的螢幕擷取畫面":::

1. [作業詳細資料] 頁面會顯示排程工作的相關資訊。 當排程工作執行時，您會看到工作實例的清單。 排程工作執行也是 **過去30天** 作業清單的一部分。

    在此頁面上，您可以 **取消排程** 作業或 **編輯** 排程工作。 您可以從排程工作的清單返回排程工作。

    :::image type="content" source="media/howto-run-a-job/job-schedule-details.png" alt-text="[排程工作詳細資料] 頁面的螢幕擷取畫面":::

1. 在 [作業嚮導] 中，您可以選擇不要排程工作，並立即執行。 下列螢幕擷取畫面顯示的作業沒有可立即執行的排程。 選取 [ **執行** ] 以執行作業：

    :::image type="content" source="media/howto-run-a-job/job-wizard-schedule-immediate.png" alt-text="工作 wizard 審核頁面的螢幕擷取畫面":::
