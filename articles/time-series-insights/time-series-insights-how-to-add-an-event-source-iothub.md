---
title: 如何添加 IoT 中心事件源 - Azure 時間序列見解 |微軟文檔
...
* 創建[Azure 時間序列見解環境](time-series-insights-update-create-environment.md)。
...
應用程式會使用取用者群組從 Azure IoT 中樞提取資料。 要可靠地從 IoT 中心讀取資料，請提供專用消費者組，該客戶群體僅由此時間序列見解環境使用。

新增取用者群組至 IoT 中樞：

1. 在[Azure 門戶](https://portal.azure.com)中，查找並打開 IoT 中心。

1. 在 **"設置"** 下，選擇**內置終結點**，然後選擇**事件**終結點。
...
1. 為 [輸入選項]**** 選取一個值：
   ...
   * 如果您的其中一個訂用帳戶已經有 IoT 中樞，請選取 [從可用的訂閱帳戶使用 IoT 中樞]****。 此選項是最簡單的方法。
    ...
    * 下表說明 [從可用的訂用帳戶使用 Iot 中樞]**** 選項的必要屬性：
       ...
       | IoT 中樞原則名稱 | 選取共用存取原則。 您可以在 IoT 中心設置選項卡上找到共用訪問策略。每個共用訪問策略都有一個名稱、您設置的許可權和訪問金鑰。 事件來源的共用存取原則必須** 有**服務連線**權限。 |
       | IoT 中樞原則金鑰 | 索引鍵已預先填入。 |
    ...
    * 如果 IoT 中樞對於您的訂用帳戶是外部，或如果想要選擇進階選項，請選取 [手動提供 IoT 中樞設定]****。
      ...
      下表說明 [手動提供 Iot 中樞設定]**** 的必要屬性：
       ...
       | IoT 中樞原則名稱 | 共用存取原則。 您可以在 IoT 中心設置選項卡上創建共用訪問策略。每個共用訪問策略都有一個名稱、您設置的許可權和訪問金鑰。 事件來源的共用存取原則必須** 有**服務連線**權限。 |
