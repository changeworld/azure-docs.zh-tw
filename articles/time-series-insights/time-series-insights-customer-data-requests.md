---
title: 客戶資料請求功能 - Azure 時間序列見解 |微軟文件
description: 在 Azure 時間序列見解中瞭解客戶數據請求功能。
author: deepakpalled
ms.author: dpalled
manager: cshankar
ms.date: 04/17/2020
...
您也可以使用 REST API，在存取原則上執行刪除作業。 關於詳細資訊,請閱讀[存取策略 - 刪除](https://docs.microsoft.com/rest/api/time-series-insights/management/accesspolicies/delete)。
...
您也可以在提供的 REST API 中使用「依環境列出」作業，在存取原則上執行匯出作業。 關於詳細資訊,請閱讀[存取策略 - 按環境列出](https://docs.microsoft.com/rest/api/time-series-insights/management/accesspolicies/listbyenvironment)。
...
關於詳細資訊,請閱讀[Azure 時間序列的解資源管理員](time-series-insights-explorer.md)。

**刪除資料**

時間序列深入解析目前不支援細微刪除資料。 不過，時間序列深入解析可設定保留原則，以便移除在時間序列深入解析內儲存的客戶資料。 您可以將整個時間序列深入解析環境的保留期限調整為任何天數，以支援您的刪除需求。

關於詳細資訊,請閱讀[時間序列的設定保留](time-series-insights-how-to-configure-retention.md)。

## <a name="next-steps"></a>後續步驟

* 閱讀有關[向 Azure 時間序列見解環境授予數據訪問許可權](./time-series-insights-data-access.md)的更多內容。

* 檢視[Azure 時間序列的解資源管理員](time-series-insights-explorer.md)。

* 在[時間序列的解中設定保留](time-series-insights-how-to-configure-retention.md)。