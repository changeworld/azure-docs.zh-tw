...
description: 瞭解如何使用 Azure 入口網站擴充您的 Azure 時間序列深入解析環境。
...
本文說明如何使用 [Azure 入口網站](https://portal.azure.com)來變更 Azure 時間序列深入解析環境的容量。 容量是套用至輸入速率、儲存體容量以及與選取之 SKU 相關聯成本的乘數。
...
1. 在 Azure 時間序列深入解析環境的功能表中，選取 [ **存放裝置**設定]。
...
1. 確認新容量是否 [足夠，以防止節流](time-series-insights-diagnose-and-solve-problems.md)。

## <a name="next-steps"></a>後續步驟

- 如需詳細資訊，請參閱 [瞭解 Azure 時間序列深入解析中的保留](time-series-insights-concepts-retention.md)。

- 瞭解如何設定 [Azure Azure 時間序列深入解析中的資料保留期](time-series-insights-how-to-configure-retention.md)。
