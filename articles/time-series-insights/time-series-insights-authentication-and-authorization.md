...
1. 選取 [確定] 來儲存原則。
   ...
   > [!TIP]
   > 如需進階的資料存取選項，請參閱[授與資料存取權](./concepts-access-policies.md)。
...
> [!IMPORTANT]
> 如果您使用 [Azure Active Directory Authentication Library (ADAL) ](../active-directory/azuread-dev/active-directory-authentication-libraries.md) 閱讀有關 [遷移至 MSAL](../active-directory/develop/msal-net-migration.md)的資訊。

## <a name="common-headers-and-parameters"></a>一般標頭和參數

本節說明用來對 Azure 時間序列深入解析 Gen1 和 Gen2 Api 進行查詢的一般 HTTP 要求標頭和參數。 API 特定的需求會在 [Azure 時間序列深入解析 REST API 參考檔](/rest/api/time-series-insights/)中更詳細地討論。

> [!TIP]
> 請閱讀 [Azure REST API 參考](/rest/api/azure/) (英文)，以深入了解如何使用 REST API、提出 HTTP 要求，以及處理 HTTP 回應。

### <a name="authentication"></a>驗證

若要對 [AZURE 時間序列深入解析 REST api](/rest/api/time-series-insights/)執行已驗證的查詢，必須使用您選擇的 REST 用戶端，在 [授權標頭](/rest/api/apimanagement/2019-12-01/authorizationserver/createorupdate) 中傳遞有效的 OAuth 2.0 持有人權杖 (Postman、JavaScript、c # ) 。
...
> [!TIP]
> 請在[參考文件](/rest/api/time-series-insights/)中尋找必要和選擇性查詢資訊的詳細資訊。
...
| 選擇性的查詢參數 | 描述 | 版本 |
| --- |  --- | --- |
| `timeout=<timeout>` | HTTP 要求執行的伺服器端逾時。 僅適用於[取得環境事件](/rest/api/time-series-insights/dataaccess(preview)/query/getavailability)和[取得環境彙總](/rest/api/time-series-insights/gen1-query-api#get-environment-aggregates-api) API。 逾時值應採用 ISO 8601 持續期間格式 (例如 `"PT20S"`)，且應在 `1-30 s`範圍內。 預設值為 `30 s`。 | Gen1 |
...
* 如需 API 參考資訊，請參閱[查詢 API 參考](/rest/api/time-series-insights/gen1-query-api)。
