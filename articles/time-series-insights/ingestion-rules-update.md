...
物件的陣列一律會壓平合併，並產生多個事件 | 如果陣列中的物件沒有 () 的 TS 識別碼或時間戳記屬性，則會將物件的陣列全部儲存為動態類型 | `"values": [{"foo" : 140}, {"bar" : 149}]` | `values_foo_long | values_bar_long` | `values_dynamic` |
...
* 所有新的部署都必須符合新的內嵌規則。 例如，如果您的 TS 識別碼是， `telemetry_tagId` 您將需要更新任何 Azure Resource Manager (ARM) 範本或自動部署腳本，以將設定 `telemetry.tagId` 為環境 TS 識別碼。 在 nested JSON 中，事件來源時間戳記也需要進行這項變更。
