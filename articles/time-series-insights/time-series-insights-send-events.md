...
本文說明如何在 Azure 事件中樞中建立和設定事件中樞。 它也會說明如何執行範例應用程式，以將事件推送至事件中樞 Azure 時間序列深入解析。 如果您現有的事件中樞具有 JSON 格式的事件，請略過本教學課程，並在 [Azure 時間序列深入解析](./tutorials-set-up-tsi-environment.md)中查看您的環境。

## <a name="configure-an-event-hub"></a>設定事件中樞

1. 若要瞭解如何建立事件中樞，請閱讀 [事件中樞檔](../event-hubs/index.yml)。
...
1. 如果您尚未建立事件來源，請完成這些步驟以[建立事件來源](./how-to-ingest-data-event-hub.md)。
...
1. 移至 <https://tsiclientsample.azurewebsites.net/windFarmGen.html>。 此 URL 會建立並執行模擬風車裝置。
...
1. 選取 [按一下即可啟動]。
    ...
    > [!TIP]
    > 風車模擬器也會建立 JSON，您可以使用 [AZURE 時間序列深入解析 GA 查詢 api](/rest/api/time-series-insights/gen1-query)作為承載。
...
* 深入瞭解 [IoT 中樞裝置訊息](../iot-hub/iot-hub-devguide-messages-construct.md)