...
* `Location`定義具有父系`states`和子`cities`系的階層。 每`location`個可以有`states`多個，而這可以有`cities`多個。
* `ManufactureDate`定義具有父系`year`和子`month`系的階層。 每`ManufactureDate`個可以有`years`多個，而這可以有`months`多個。
...
假設有一個範例， **H1**其中的`building`階層`floor`H1 具有`room` 、和作為其**instanceFieldNames**定義的一部分：
