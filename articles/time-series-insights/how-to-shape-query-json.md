---
title: 用於塑造 JSON 的最佳實踐 - Azure 時間序列見解查詢 |微軟文檔
...
ms.locfileid: "76989887"
---
# <a name="shape-json-to-maximize-query-performance"></a>形狀 JSON 以最大限度地提高查詢性能 
...
1. 盡可能有效率地透過網路傳送資料。
1. 確保以一種方式存儲資料，以便執行適合您的方案的聚合。
1. 確保您沒有達到時序見解的最大屬性限制：
   - S1 環境為 600 個屬性 (資料行)。
   - S2 環境為 800 個屬性 (資料行)。

> [!TIP]
> 在 Azure 時間序列見解預覽中查看[限制和規劃](time-series-insights-update-plan.md)。

以下指南有助於確保最佳的查詢性能：

1. 不要使用動態屬性（如標記識別項）作為屬性名稱。 此使用有助於達到最大屬性限制。
1. 請勿傳送不必要的屬性。 如果不需要查詢屬性，最好不要發送它。 這樣，可以避免存儲限制。
1. 使用[參考資料](time-series-insights-add-reference-data-set.md)避免通過網路發送靜態資料。
1. 在多個事件之間共用維度屬性，以便更有效地通過網路發送資料。
1. 請勿使用深層陣列巢狀結構。 時間序列見解最多支援包含物件的嵌套陣列的兩個級別。 時間序列見解將消息中的陣列平展到具有屬性值對的多個事件中。
...
以下兩個示例演示如何發送事件以突出顯示前面的建議。 在每個示例之後，您可以查看建議的應用方式。

這些範例是根據多部裝置傳送度量或訊號的案例而來。 測量或信號可以是流速、發動機機油壓力、溫度和濕度。 在第一個範例中，有幾個度量會跨所有裝置。 第二個示例具有許多設備，每個設備都發送許多唯一的測量值。
...
在下面的示例中，有一個 Azure IoT 中心消息，其中外部陣列包含公共維度值的共用部分。 外部陣列使用參考資料來提高訊息的效率。 參考資料包含的設備中繼資料不會隨每個事件發生更改，但它為數據分析提供了有用的屬性。 對通用維度值進行批次處理並採用參考資料，可節省通過導線發送的位元組，從而提高消息的效率。

請考慮使用[IT 設備消息物件](https://docs.microsoft.com/dotnet/api/microsoft.azure.devices.message?view=azure-dotnet)發送到時序到 Azure 雲時發送到時序的時序見解 GA 環境的以下 JSON 有效負載：
...
> - 使用參考資料來減少透過網路傳輸的位元組數目。 使用金鑰屬性**設備 Id**加入這兩個屬性**消息 Id** **和設備位置**。 此資料在入口時間與遙測資料聯接，然後存儲在時間序列見解中以進行查詢。
...
> [!NOTE]
> - 列**設備 Id**和**series.tagId**充當佇列中各種設備和標記的列標題。 使用每個作為自己的屬性將查詢限制為 594（對於 S1 環境）或 794（對於 S2 環境）的總設備與其他六列。
> - 由於第一個示例中引用的原因，避免了不必要的屬性。
> - 參考資料用於通過引入**設備 Id**來減少通過網路傳輸的位元組數，該設備 Id 用於唯一的消息**Id**和對**設備位置**的對。 複合鍵**系列.tagId**用於**唯一的類型**和**單位**對。 複合鍵允許設備**Id**和**series.tagId**對用於引用四個值：**消息 Id、設備位置、類型**和**單位**。 此資料在入口時間與遙測資料聯接。 然後，它存儲在時間序列見解中以進行查詢。
...
對於具有大量可能值的屬性，最好在單個列中作為不同的值發送，而不是為每個值創建新列。 在前兩個範例中：
  ...
  - 在第二個示例中，度量值未指定為單個屬性。 相反，它們是公共系列屬性下的值或度量值陣列。 將發送新的鍵**tagId，** 這將在拼合表中創建新的列**系列。** 使用參考資料創建新的屬性**類型**和**單位**，以便不達到屬性限制。

## <a name="next-steps"></a>後續步驟

- 閱讀有關向雲發送[IoT 中心設備消息](../iot-hub/iot-hub-devguide-messages-construct.md)的資訊。

- 閱讀[Azure 時間序列見解查詢語法](https://docs.microsoft.com/rest/api/time-series-insights/ga-query-syntax)，瞭解有關時序見解資料訪問 REST API 的查詢語法的更多資訊。
