...
description: 瞭解如何將 Azure 事件中樞的事件來源新增至您的 Azure 時間序列深入解析環境。
...
> [!NOTE]
> 本文中所述的步驟適用于 Azure 時間序列深入解析 Gen 1 和 Azure 時間序列深入解析 Gen 2 環境。

## <a name="prerequisites"></a>必要條件

- 建立 Azure 時間序列深入解析環境，如[建立 Azure 時間序列深入解析環境](./time-series-insights-update-create-environment.md)中所述。
...
- 在 Azure 時間序列深入解析環境可以取用的事件中樞建立專用取用者群組。 每個 Azure 時間序列深入解析事件來源都必須有自己的專用取用者群組，而不會與任何其他取用者共用。 如果多個讀取器取用來自相同取用者群組的事件，所有讀取器都可能會出現失敗。 另外也限制每一個事件中樞最多有 20 個取用者群組。 如需詳細資訊，請參閱[事件中樞程式設計指南](../event-hubs/event-hubs-programming-guide.md)。
...
1. 在 [取用者群組]**** 頁面上，在 [名稱]**** 中輸入新的唯一值。  當您在 Azure 時間序列深入解析環境中建立新的事件來源時，請使用此相同名稱。
...
1. 為 [匯入選項]**** 選取適當的值：
   ...
   * 若您的其中一個訂用帳戶中有現有的事件中樞，請選取 [來自可用訂閱的使用事件中樞]****。 此選項是最簡單的方法。
    ...
    *  下表說明 [來自可用訂閱的使用事件中樞]**** 選項的必要屬性：
       ...
       | 事件中樞名稱 | 所需事件中樞實例的名稱。 |
       | 事件中樞原則值 | 選取所需的共用存取原則。 您可以在 [事件中樞**設定**] 索引標籤上建立共用存取原則。每個共用存取原則都會有名稱、您設定的許可權，以及存取金鑰。 事件來源的共用存取原則必須** 有**讀取**權限。 |
...
1. 選取 [建立]。
   ...
   建立事件來源之後，Azure 時間序列深入解析會自動開始將資料串流處理至您的環境。
