...
description: 設定、管理、規劃及部署 Azure 時間序列深入解析 Gen2 環境的最佳做法。
author: deepakpalled
ms.author: dpalled
manager: diviso
...
本文說明如何使用 Azure 時間序列深入解析 Gen2 來規劃和快速入門的最佳作法。
...
Azure 時間序列深入解析採用隨用隨付商務模型。 如需有關費用和容量的詳細資訊，請參閱[Azure 時間序列深入解析定價](https://azure.microsoft.com/pricing/details/time-series-insights/)。
...
> [!NOTE]
> 若要深入瞭解如何查詢您的暖資料，請閱讀[API 參考](https://docs.microsoft.com/rest/api/time-series-insights/dataaccessgen2/query/execute#uri-parameters)。
...
## <a name="review-azure-time-series-insights-gen2-limits"></a>審查 Azure 時間序列深入解析 Gen2 限制
...
您可以驗證您將事件傳送至 Azure 時間序列深入解析的方式。 在理想情況下，您的事件已妥善且有效率地反正規化。
...
- 請參閱[Azure Advisor](../advisor/advisor-overview.md) ，以規劃您的商務復原設定選項。
- 如需深入瞭解[資料](./concepts-ingestion-overview.md)內嵌，請參閱 Azure 時間序列深入解析 Gen2。
- 請參閱 Azure 時間序列深入解析 Gen2 中有關[資料儲存體](./concepts-storage.md)的文章。
- 深入瞭解 Azure 時間序列深入解析 Gen2 中的[資料模型](./concepts-model-overview.md)。
