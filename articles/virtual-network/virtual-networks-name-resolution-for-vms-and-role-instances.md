...
當部署在虛擬網路中的資源需要將網域名稱解析為內部 IP 位址時,可以使用三種方法之一:

* [Azure DNS 專用區域](../dns/private-dns-overview.md)
...
> [!NOTE]
> Azure DNS 專用區域是首選解決方案,使您能夠靈活地管理 DNS 區域和記錄。 如需詳細資訊，請參閱[使用私人網域的 Azure DNS](../dns/private-dns-overview.md)。

> [!NOTE]
> 如果使用 Azure 提供的 DNS,則相應的 DNS 後綴將自動應用於虛擬機器。 對於所有其他選項,您必須使用完全限定的功能變數名稱 (FQDN) 或手動將適當的 DNS 後綴應用於虛擬機器。

| **案例** | **解決方法** | **DNS 尾碼** |
| --- | --- | --- |
| 相同虛擬網路內的 VM 之間或相同雲端服務的 Azure 雲端服務角色執行個體之間所進行的名稱解析。 | [Azure DNS 專用區域](../dns/private-dns-overview.md)或[Azure 提供的名稱解析](#azure-provided-name-resolution) |主機名稱或 FQDN |
| 不同虛擬網路的 VM 之間或不同雲端服務的角色執行個體之間所進行的名稱解析。 |[Azure DNS 專用區域](../dns/private-dns-overview.md)或客戶管理的 DNS 伺服器在虛擬網路之間轉發查詢,以便由 Azure(DNS 代理)解決。 請參考[您自己的 DNS 伺服器的名稱解析](#name-resolution-that-uses-your-own-dns-server)。 |僅 FQDN |
| 從使用虛擬網路整合的 Azure App Service (Web App、Function 或 Bot) 將名稱解析相同虛擬網路中的角色執行個體或 VM。 |客戶管理的 DNS 伺服器將虛擬網路之間的查詢轉送供 Azure (DNS Proxy) 解析。 請參考[您自己的 DNS 伺服器的名稱解析](#name-resolution-that-uses-your-own-dns-server)。 |僅 FQDN |
| 從 App Service Web Apps 將名稱解析到相同虛擬網路中的 VM。 |客戶管理的 DNS 伺服器將虛擬網路之間的查詢轉送供 Azure (DNS Proxy) 解析。 請參考[您自己的 DNS 伺服器的名稱解析](#name-resolution-that-uses-your-own-dns-server)。 |僅 FQDN |
| 從某個虛擬網路的 App Service Web Apps 將名稱解析到不同虛擬網路中的 VM。 |客戶管理的 DNS 伺服器將虛擬網路之間的查詢轉送供 Azure (DNS Proxy) 解析。 請參考[您自己的 DNS 伺服器的名稱解析](#name-resolution-that-uses-your-own-dns-server)。 |僅 FQDN |
| 由 Azure 中的 VM 或角色執行個體解析內部部署電腦及伺服器名稱。 |客戶管理的 DNS 伺服器 (例如，內部部署的網域控制站、本機唯讀網域控制站或使用區域傳輸同步的次要 DNS)。 請參考[您自己的 DNS 伺服器的名稱解析](#name-resolution-that-uses-your-own-dns-server)。 |僅 FQDN |
| 從內部部署電腦解析 Azure 主機名稱。 |將查詢轉送到所對應虛擬網路中客戶管理的 DNS Proxy 伺服器，Proxy 伺服器將查詢轉送給 Azure 進行解析。 請參考[您自己的 DNS 伺服器的名稱解析](#name-resolution-that-uses-your-own-dns-server)。 |僅 FQDN |
| 從內部 IP 還原 DNS。 |[使用自己的 DNS 伺服器,Azure](#name-resolution-that-uses-your-own-dns-server) [DNS 專用區域](../dns/private-dns-overview.md)或[Azure 提供的名稱解析](#azure-provided-name-resolution)或名稱解析。 |不適用 |
...
Azure 提供的名稱解析僅提供基本的權威 DNS 功能。 如果使用此選項,DNS 區域名稱和記錄將自動由 Azure 管理,您將無法控制 DNS 區域名稱或 DNS 記錄的生命週期。 如果需要為虛擬網路提供功能齊全的 DNS 解決方案,則必須使用[Azure DNS 專用區域](../dns/private-dns-overview.md)或[客戶管理的 DNS 伺服器](#name-resolution-that-uses-your-own-dns-server)。

除了公用 DNS 名稱的解析之外，Azure 也提供位於相同虛擬網路或雲端服務內的 VM 和角色執行個體的內部名稱解析。 雲端服務中的虛擬機器和執行個體會共用相同 DNS 尾碼，因此只要主機名稱就已足夠。 但是在使用傳統部署模型所部署的虛擬網路中，不同的雲端服務會有不同的 DNS 尾碼。 在此情況下，您需要 FQDN 才能解析不同雲端服務之間的名稱。 在使用 Azure 資源管理器部署模型部署的虛擬網路中,DNS 後綴在虛擬網路中的所有虛擬機器中是一致的,因此不需要 FQDN。 DNS 名稱可以同時指派給 VM 和網路介面。 雖然 Azure 提供的名稱解析不需要任何設定，但它不適用於所有部署案例，如上表所詳述。
...
* DNS 查找範圍為虛擬網路。 無法從其他虛擬網路解析為一個虛擬網路創建的 DNS 名稱。
* 您無法手動註冊您自己的記錄。
...
* Azure DNS IP 位址為 168.63.129.16。 這是一個靜態 IP 位址,不會更改。

### <a name="reverse-dns-considerations"></a>反向 DNS 注意事項
所有基於 ARM 的虛擬網路都支援反向 DNS。 您可以發出反向 DNS 查詢 (PTR 查詢),以將虛擬機器的 IP 位址映射到虛擬機器的 FQDN。
* 虛擬機器 IP 位址的所有 PTR 查詢都\[傳回表\]單 vmname .internal.cloudapp.net 的 FQDN
* 對表單\[\]vmname.internal.cloudapp.net的 FQDN 的向前查找將解析為分配給虛擬機器的 IP 位址。
* 如果虛擬網路作為註冊虛擬網路連結到[Azure DNS 專用區域](../dns/private-dns-overview.md),則反向 DNS 查詢將返回兩條記錄。 一個紀錄將寫出\[檔 vmname\]。[priednszonename] 和其他將\[的形式 vmname\].internal.cloudapp.net
* 反向 DNS 查找的範圍限定為給定的虛擬網路,即使它被對等到其他虛擬網路也是如此。 對位於對等虛擬網路中的虛擬機器的 IP 位址的反向 DNS 查詢 (PTR 查詢) 將傳回 NXDOMAIN。
* 如果要關閉虛擬網路中的反向 DNS 功能,可以通過使用[Azure DNS 專用區域](../dns/private-dns-overview.md)創建反向查找區域並將此區域連結到虛擬網路來執行此操作。 例如,如果虛擬網路的 IP 位址空間為 10.20.0.0/16,則可以創建一個空專用 DNS 區域 20.10.in-addr.arpa 並將其連結到虛擬網路。 將區域連結到虛擬網路時,應禁用連結上的自動註冊。 此區域將覆蓋虛擬網路的預設反向查找區域,由於此區域為空,您將獲得用於反向 DNS 查詢的 NXDOMAIN。 有關如何創建專用 DNS 區域並將其連結到虛擬網路的詳細資訊,請參閱我們的[快速入門指南](https://docs.microsoft.com/azure/dns/private-dns-getstarted-portal)。

> [!NOTE]
> 如果希望反向 DNS 尋找跨越虛擬網路,則可以建立反向查找區域 (addr.arpa) Azure DNS[專用區域](../dns/private-dns-overview.md)並將其連結到多個虛擬網路。 但是,您必須手動管理虛擬機器的反向 DNS 記錄。
>
...
* **SUSE (使用網路)**:
  ...
  * 編輯 **/etc/sysconfig/網路/設定**,並將*NETCONFIG_DNS_FORWARDER="* 更改為*dnsmasq*。
  * 使用 `netconfig update` 更新 resolv.conf 來設定快取作為本機 DNS 解析程式。
* **CentOS (使用網路管理員):**
  * 使用 `sudo yum install dnsmasq` 安裝 dnsmasq 套件。
  * 使用 `systemctl enable dnsmasq.service` 啟用 dnsmasq 服務。
  * 使用 `systemctl start dnsmasq.service` 啟動 dnsmasq 服務。
  * 將*預端網域名稱伺服器 127.0.0.1;* 增加**到 /etc/dhclient-eth0.conf**。
...
* **SUSE(** 使用網聯):
  1. 新增*逾時:1 嘗試:5*到 **/etc/sysconfig/網路/設定**中的**NETCONFIG_DNS_RESOLVER_OPTIONS="參數**。
  2. 執行 `netconfig update` 以更新。
* **CentOS(** 使用網路管理員):
  1. 將*回音「選項超時:1 嘗試:5」* 新增到 **/etc/網路管理器/調度程式.d/11-dhclient**。
...
* 對於 Azure 資源管理器部署模型中的虛擬網路,後綴可透過[網路介面 REST](https://docs.microsoft.com/rest/api/virtualnetwork/networkinterfaces)API、Get-AzNetwork[介面](/powershell/module/az.network/get-aznetworkinterface)PowerShell cmdlet 和[az 網路 nic 顯示](/cli/azure/network/nic#az-network-nic-show)Azure CLI 命令提供。
...
> [!NOTE]
> 網路連接屬性(如 DNS 伺服器 IP)不應直接在 VM 中編輯。 這是因為當替換虛擬網路介面卡時，它們可能會在服務修復期間遭到清除。 這同時適用於 Windows 和 Linux VM。
...
> [!NOTE]
> 如果更改已部署的虛擬網路或虛擬機器的 DNS 設置,則新 DNS 設置要生效,則必須對虛擬網路中所有受影響的 VM 執行 DHCP 租約續訂。 對於運行 Windows 作業系統的 VM,可以通過`ipconfig /renew`直接在 VM 中鍵入來執行此操作。 步驟因操作系統而異。 請參閱作業系統類型的相關文檔。
...
* [使用網路設定檔設定虛擬網路](virtual-networks-using-network-configuration-file.md)
