...
將允許的儲存體帳戶的資源 URI 儲存在變數中。 執行下列命令之前，請以您的訂用帳戶 ID 的實際值取代 *\<您的訂用帳戶識別碼 >* 。
...
將 Azure 檔案共用裝載至您所建立的目錄。 執行下列命令之前，請將\<的值從 **$saConnectionString 1**取代為*AccountKey*的*儲存體帳戶金鑰 >* 。
...
執行下列命令之前，請將\<的值從 **$saConnectionString 2**取代為*AccountKey*的*儲存體帳戶金鑰 >* 。

```bash
sudo mount --types cifs //notallowedstorageacc.file.core.windows.net/my-file-share /mnt/MyAzureFileShare2 --options vers=3.0,username=notallowedstorageacc,password=<storage-account-key>,dir_mode=0777,file_mode=0777,serverino
```

存取遭到拒絕，而且您收到 `mount error(13): Permission denied` 錯誤，因為此儲存體帳戶不在我們套用至子網的服務端點原則的允許清單中。 
