...
Azure 會自動建立系統路由，並將路由指派給虛擬網路中的每個子網路。 您無法建立系統路由，也無法移除系統路由，但是您可以使用[自訂路由](#custom-routes)覆寫某些系統路由。 當您使用特定 Azure 功能時，Azure 會為每個子網建立預設的系統路由，並將其他[選擇性預設路由](#optional-default-routes)新增至特定子網或每個子網。
...
* **虛擬網路 (VNet) 對等互連**：當您在兩個虛擬網路間建立虛擬網路對等互連時，系統會在每個建立對等連線的虛擬網路位址空間中，為每個位址範圍新增路由。 深入瞭解[虛擬網路對等互連](virtual-network-peering-overview.md)。<br>
...
您可以在 Azure 中建立自訂或使用者定義（靜態）的路由，以覆寫 Azure 的預設系統路由，或將其他路由新增至子網的路由表。 在 Azure 中，您可以建立路由表，然後使路由表與零個或多個虛擬網路子網路產生關聯。 每個子網路可以有零個或一個與其相關聯的路由表。 若要了解您可以新增至路由表的最大路由數目，以及您可以為每個 Azure 訂用帳戶建立的最大使用者定義路由表數目，請參閱 [Azure 限制](../azure-resource-manager/management/azure-subscription-service-limits.md?toc=%2fazure%2fvirtual-network%2ftoc.json#networking-limits)。 如果您建立路由表並讓它與子網路產生關聯，其中的路由會結合或覆寫 Azure 新增至子網路的預設路由。
...
ER 與 VPN 閘道路由傳播，可以透過使用路由表上的屬性來在子網路上停用。 當您使用 BGP 與 Azure 交換路由時，不會將路由新增至已停用虛擬網路閘道路由傳播的所有子網的路由表。 使用下一個躍點類型為*虛擬網路閘道*的[自訂路由](#custom-routes)，即可進行 VPN 連線。 **GatewaySubnet 上不應停用路由傳播。閘道將無法在停用此設定的情況下運作。** 如需詳細資訊，請參閱[如何停用虛擬網路閘道路由傳播](manage-route-table.md#create-a-route-table)。
...
|識別碼  |來源 |State  |位址首碼    |下一個躍點類型          |下一個躍點 IP 位址|使用者定義路由名稱| 
|----|-------|-------|------              |-------                |--------           |--------      |
...
|預設值 |Active |10.0.0.0/8          |無                      |                   |
|預設值 |Active |100.64.0.0/10       |無                      |                   |
|預設值 |Active |192.168.0.0/16      |無                      |                   |
