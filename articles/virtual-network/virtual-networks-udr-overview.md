...
Azure 會自動建立系統路由，並將路由指派給虛擬網路中的每個子網路。 不能創建系統路由，也不能刪除系統路由，但可以使用[自訂路由](#custom-routes)覆蓋某些系統路由。 Azure 為每個子網創建預設系統路由，並在使用特定 Azure 功能時向特定子網或每個子網添加其他[可選預設路由](#optional-default-routes)。
...
ER 與 VPN 閘道路由傳播，可以透過使用路由表上的屬性來在子網路上停用。 使用 BGP 與 Azure 交換路由時，路由不會添加到禁用虛擬網路閘道路由傳播的所有子網的路由表中。 使用下一個躍點類型為*虛擬網路閘道*的[自訂路由](#custom-routes)，即可進行 VPN 連線。 有關詳細資訊，請參閱[如何禁用虛擬網路閘道路由傳播](manage-route-table.md#create-a-route-table)。
...
Subnet2** 的路由表包含所有 Azure 建立的預設路由和選擇性 VNet 對等互連與虛擬網路閘道選擇性路由。 當閘道和對等互連新增至虛擬網路時，Azure 就會將選擇性路由新增至虛擬網路中的所有子網路。 Azure 從*Subnet1*路由表中刪除了 10.0.0.0/8、192.168.0.0/16 和 100.64.0.0/10 位址首碼的路由，此時使用者定義的 0.0.0.0/0 位址首碼已添加到*子網1*。  
