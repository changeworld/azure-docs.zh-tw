...
- 如果使用命令列介面 (CLI) 命令來完成這篇文章中的工作，請在 [Azure Cloud Shell](https://shell.azure.com/bash) \(英文\) 中執行命令，或從您的電腦執行 CLI。 本教學課程需要 Azure CLI 2.0.31 版或更新版本。 執行 `az --version` 來了解安裝的版本。 如果需要安裝或升級，請參閱[安裝 Azure CLI](/cli/azure/install-azure-cli)。 如果您在本機執行 Azure CLI，則也需要執行 `az login` 以建立與 Azure 的連線。
...
4. 在 [建立公用 IP 位址]**** 下方，輸入或選取下列設定的值，然後選取 [建立]****：
   ...
   |名稱（僅當您選擇**兩者**的 IP 版本時可見）|是，如果您選擇**兩個**IP 版本|該名稱必須與此清單中的第一個**名稱**不同。 如果您選擇同時建立 IPv4 和 IPv6 位址，則入口網站會建立兩個個別的公用 IP 位址資源，並各指派一個 IP 位址版本。|
   |IP 位址分配（僅在選擇**兩者**的 IP 版本時可見）|是，如果您選擇**兩個**IP 版本|與上述 IP 位址分配相同的限制|
   |訂用帳戶|是|必須與要關聯公共 IP 的資源位於同一[訂閱](../azure-glossary-cloud-terminology.md?toc=%2fazure%2fvirtual-network%2ftoc.json#subscription)中。|
   |資源群組|是|可以存在於與要關聯公共 IP 的資源相同的資源組中或不同資源[組中](../azure-glossary-cloud-terminology.md?toc=%2fazure%2fvirtual-network%2ftoc.json#resource-group)。|
   |Location|是|必須存在於同一[位置](https://azure.microsoft.com/regions)（也稱為區域）中，作為您將公共 IP 關聯的資源。|
...
雖然入口網站提供建立兩個公用 IP 位址資源 (一個 IPv4 和一個 IPv6) 的選項，但下列的 CLI 和 PowerShell 命令則是會以其中一個 IP 版本的位址建立一個資源。 如果需要兩個公共 IP 位址資源（每個 IP 版本一個），則必須運行該命令兩次，為公共 IP 位址資源指定不同的名稱和 IP 版本。
...
|PowerShell|[獲取 AzPublicIp 位址](/powershell/module/az.network/get-azpublicipaddress)以檢索公共 IP 位址物件並查看其設置，[設置-AzPublicIp位址](/powershell/module/az.network/set-azpublicipaddress)以更新設置;[刪除要刪除的 Az 公開Ip位址](/powershell/module/az.network/remove-azpublicipaddress)|
