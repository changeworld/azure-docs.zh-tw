...
4. 在 [ **新增子網** ] 對話方塊中，輸入下列設定的值：
    ...
    | **網路安全性群組** | 若要篩選子網的輸入和輸出網路流量，您可以將現有的網路安全性群組與子網產生關聯。 網路安全性群組必須與虛擬網路位於相同的訂用帳戶和位置當中。 深入了解[網路安全性群組](./network-security-groups-overview.md)與[如何建立網路安全性群組](tutorial-filter-network-traffic.md)。 |
