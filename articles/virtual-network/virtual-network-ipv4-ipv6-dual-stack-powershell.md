---
title: 部署 IPv6 雙堆疊應用程式 - 基本負載等化器 - PowerShell
...
---
# <a name="deploy-an-ipv6-dual-stack-application-using-basic-load-balancer---powershell-preview"></a>使用基本負載等化器部署 IPv6 雙堆疊應用程式 - 電源外殼（預覽版）

本文介紹如何使用 Azure PowerShell 使用基本負載等化器部署雙堆疊 （IPv4 + IPv6） 應用程式，該應用程式包括雙堆疊虛擬網路和子網、具有雙 （IPv4 + IPv6） 前端配置的基本負載等化器、具有 NIC 的 VM具有雙 IP 配置、網路安全性群組和公共 IP。

要使用標準負載等化器部署雙堆疊 （IPV4 + IPv6） 應用程式，請參閱[使用 Azure PowerShell 使用標準負載等化器部署 IPv6 雙堆疊應用程式](virtual-network-ipv4-ipv6-dual-stack-standard-load-balancer-powershell.md)。
...
使用 [New-AzLoadBalancer](/powershell/module/az.network/new-azloadbalancer) 建立基本負載平衡器。 以下示例使用您在上述步驟中創建的 IPv4 和 IPv6 前端 IP 配置、後端池和負載平衡規則創建名為*myLoad平衡器*的公共基本負載平衡器：
