...
Azure 虛擬網路 (VNet) 是您的網路在雲端中的身分。 它是專屬於您訂用帳戶的 Azure 雲端邏輯隔離。 您可以使用 VNet 在 Azure 中佈建和管理虛擬私人網路 (VPN)，也可以選擇性地連結 VNet 與其他 Azure 中的 VNet，或連結您的內部部署 IT 基礎結構，以便建立混合式或跨單位的解決方案。 您創建的每個 VNet 都有自己的 CIDR 塊，只要 CIDR 塊不重疊，就可以連結到其他 VNet 和本地網路。 針對 VNet 和分割的 VNet 子網路，您也擁有 DNS 伺服器設定的控制項。

您可以使用 VNet：

* 創建專用專用專用專用 VNet。 針對您的解決方案，有時候您不需要跨單位組態。 當您建立 VNet 時，您的服務和 VNet 中的 VM 可以直接且安全地在雲端中彼此通訊。 您仍然可以設定 VM 的端點連線，以及需要網際網路通訊作為解決方案一部分的服務。

* 安全地擴展您的資料中心。 您可以使用 VNet 建置傳統的站台對站台 (S2S) VPN，安全地擴充資料中心容量。 S2S VPN 使用 IPSEC 在您的公司 VPN 閘道與 Azure 之間提供安全連接。

* 啟用混合雲方案。 VNet 可讓您彈性地支援許多混合式雲端案例。 您可以將雲端型應用程式安全地連接到任何類型的內部部署系統，例如大型主機和 Unix 系統。

### <a name="how-do-i-get-started"></a>如何開始使用？
訪問[虛擬網路文檔](https://docs.microsoft.com/azure/virtual-network/)以開始。 本內容提供所有 VNet 功能的概觀和部署資訊。
...
是。 Azure 會在每個子網路中保留 5 個 IP 位址。 These are x.x.x.0-x.x.x.3 and the last address of the subnet. x.x.x.1-x.x.x.3 is reserved in each subnet for Azure services.   
- x.x.x.0： 網路位址
...
受支援的最小 IPv4 子網為 /29，最大為 /8（使用 CIDR 子網定義）。  IPv6 子網的大小必須完全為 /64。  
...
是的，VNet 可以是僅 IPv4 或雙堆疊 （IPv4+IPv6）。  有關詳細資訊，請參閱[Azure 虛擬網路的 IPv6 概述](./ipv6-overview.md)。
...
是。 您可以隨時針對 VNet 變更 DNS 伺服器清單。 如果更改 DNS 伺服器清單，則需要對 VNet 中的所有受影響的 VM 執行 DHCP 租約續訂，以便新的 DNS 設置生效。 對於運行 Windows 作業系統的 VM，可以通過`ipconfig /renew`直接在 VM 上鍵入來執行此操作。 有關其他作業系統類型，請參閱特定作業系統類型的 DHCP 租約續訂文檔。 
...
是。 您可以使用 ASE（應用服務環境）在 VNet 中部署 Web 應用，使用 VNet 集成將應用後端連接到 VNet，並使用服務終結點鎖定對應用的入站流量。 如需詳細資訊，請參閱下列文章：

* [應用服務網路功能](../app-service/networking-features.md)
...
* [應用服務訪問限制](../app-service/app-service-ip-restrictions.md)
...
### <a name="can-i-connect-a-virtual-machine-scale-set-to-a-vnet"></a>是否可以將虛擬機器規模集連接到 VNet？
是。 必須將虛擬機器規模集連接到 VNet。
...
### <a name="how-can-i-restrict-access-to-azure-paas-resources-from-a-vnet"></a>如何限制 VNet 對 Azure PaaS 資源的訪問？

通過某些 Azure PaaS 服務（如 Azure 存儲和 Azure SQL 資料庫）部署的資源可以通過使用虛擬網路服務終結點或 Azure 專用鏈路限制對 VNet 的網路訪問。 有關詳細資訊，請參閱[虛擬網路服務終結點概述](virtual-network-service-endpoints-overview.md)[、Azure 專用連結概述](../private-link/private-link-overview.md)
...
是。 您可以在[Azure 資源管理器](/rest/api/virtual-network)和[經典](https://go.microsoft.com/fwlink/?LinkId=296833)部署模型中對 VNet 使用 REST API。
...
是。 全域 VNet 可讓您對不同區域中的對等 VNet 建立對等互連。 全域 VNet 對等互連在所有 Azure 公共區域、中國雲區域和政府雲區域都可用。 不能全域從 Azure 公共區域到國家雲區域對等。

### <a name="what-are-the-constraints-related-to-global-vnet-peering-and-load-balancers"></a>與全域 VNet 對等互連和負載等化器相關的約束是什麼？
如果通過全域 VNet 對等互連對互連兩個不同區域中的兩個虛擬網路，則無法通過負載等化器的前端 IP 連接到基本負載等化器後面的資源。 此限制對於標準負載等化器不存在。
以下資源可以使用基本負載等化器，這意味著您無法通過負載等化器的前端 IP 通過全域 VNet 對等互連來達到它們。 但是，如果允許，您可以使用全域 VNet 對等互連直接通過其專用 VNet IP 訪問資源。 
- 基本負載等化器背後的 VM
- 具有基本負載等化器的虛擬機器規模集 
- Redis 快取 
- 應用程式閘道 （v1） SKU
...
您可以通過 ExpressRoute 或 VNet 到 VNet 閘道連接到這些資源。
...
如果對等互連連接處於 *"已啟動"* 狀態，這意味著您只創建了一個連結。 必須建立雙向連結，才能建立成功的連線。 例如，若要建立 VNet A 對 VNet B 的對等互連，則必須建立從 VNetA 到 VNetB 和從 VNetB 到 VNetA 的連結。 創建兩個連結將更改狀態到 *"已連接*"。
...
如果您的 VNet 對等互連連接處於*斷開連接*狀態，則表示創建的連結之一已被刪除。 為了重新建立對等互連連接，您需要刪除連結並重新創建它。
...
### <a name="why-is-my-peering-connection-in-a-disconnected-state"></a>為什麼我的對等互連處於*斷開連接*狀態？
...
### <a name="how-can-i-troubleshoot-vnet-peering-issues"></a>如何解決 VNet 對等互連問題？
下面是您可以嘗試的[疑難排解指南](https://support.microsoft.com/en-us/help/4486956/troubleshooter-for-virtual-network-peering-issues)。
...
虛擬網路 TAP 預覽在所有 Azure 區域都可用。 受監視的網路介面、虛擬網路 TAP 資源以及收集器或分析解決方案，都必須部署在相同的區域。
...
受監視的網路介面只能有一個 TAP 設定。 請諮詢各個[合作夥伴解決方案](virtual-network-tap-overview.md#virtual-network-tap-partner-solutions)，瞭解將 TAP 流量的多個副本資料流到您選擇的分析工具的功能。
...
通過服務終結點保護 Azure 服務資源有兩個步驟：
1. 針對 Azure 服務開啟服務端點。
2. 在 Azure 服務上設定 VNet ACL。

第一個步驟是網路端作業，第二個步驟是服務資源端作業。 根據授與系統管理員角色的 RBAC 權限，兩個步驟可以是由相同的或不同的系統管理員來執行。 我們建議您先針對您的虛擬網路開啟服務端點，再於 Azure 服務端設定 VNet ACL。 因此，必須按上面列出的循序執行這些步驟，以設置 VNet 服務終結點。

>[!NOTE]
> 您必須先完成上述兩個作業，才能限制 Azure 服務存取所允許的 VNet 和子網路。 只在網路端針對 Azure 服務開啟服務端點不能提供您受限制的存取。 此外，您也必須在 Azure 服務端設定 VNet ACL。

某些服務（如 SQL 和 CosmosDB）允許通過**忽略缺失VNet服務終結點**標誌對上述序列進行異常。 將標誌設置為**True**後，可以在 Azure 服務端設置 VNet ACL，在網路端設置服務終結點之前。 Azure 服務提供此旗標來協助客戶避免連線中斷，若 IP 防火牆是設定在 Azure 服務上，在網路端開啟服務端點可能會導致連線中斷，因為來源 IP 從公用 IPv4 位址變更為私人位址。 先在 Azure 服務端設定 VNet ACL，再於網路端設定服務端點有助於避免連線中斷。
...
否，並非所有的 Azure 服務都位在客戶的虛擬網路中。 大多數 Azure 資料服務（如 Azure 存儲、Azure SQL 和 Azure Cosmos DB）都是可通過公共 IP 位址訪問的多租戶服務。 您可以在[這裡](virtual-network-for-azure-services.md)深入了解 Azure 服務的虛擬網路整合。 
...
VNet 服務終結點功能（打開網路端的 VNet 服務終結點並在 Azure 服務端設置適當的 VNet ACL）限制 Azure 服務對允許的 VNet 和子網的訪問，從而提供網路級安全性和Azure 服務流量的隔離。 使用 VNet 服務端點的所有流量都流經 Microsoft 骨幹，因此提供與公用網際網路的另一層隔離。 此外，客戶可選擇完全移除公用網際網路對 Azure 服務資源的存取權，僅允許來自其虛擬網路的流量 (透過 IP 防火牆和 VNet ACL 的組合)，進而保護 Azure 服務資源不會被未經授權的使用者存取。      
...
### <a name="can-an-on-premises-devices-ip-address-that-is-connected-through-azure-virtual-network-gateway-vpn-or-expressroute-gateway-access-azure-paas-service-over-vnet-service-endpoints"></a>通過 Azure 虛擬網路閘道 （VPN） 或 ExpressRoute 閘道連接的本地設備的 IP 位址能否通過 VNet 服務終結點訪問 Azure PaaS 服務？
...
### <a name="can-i-use-vnet-service-endpoint-feature-to-secure-azure-service-to-multiple-subnets-within-a-virtual-network-or-across-multiple-virtual-networks"></a>我能否使用 VNet 服務終結點功能將 Azure 服務保護到虛擬網路中的多個子網或跨多個虛擬網路？
要保護 Azure 服務到虛擬網路中的多個子網或跨多個虛擬網路，應單獨在每個子網上啟用網路端的服務終結點，然後通過設置設置 Azure 服務資源來保護所有子網Azure 服務端的相應 VNet ACL。
...
### <a name="what-happens-when-you-access-an-azure-service-account-that-has-a-virtual-network-access-control-list-acl-enabled-from-outside-the-vnet"></a>當您訪問從 VNet 外部啟用了虛擬網路存取控制清單 （ACL） 的 Azure 服務帳戶時會發生什麼情況？
...
### <a name="can-an-azure-service-have-both-a-vnet-acl-and-an-ip-firewall"></a>Azure 服務是否可以同時具有 VNet ACL 和 IP 防火牆？
是的，VNet ACL 和 IP 防火牆可以共存。 這兩個功能彼此互補以確保隔離和安全性。
...
VNet 和子網域的刪除是獨立的作業，即使已針對 Azure 服務開啟服務端點也支援。 如果 Azure 服務已為這些 VNet 和子網設置了 VNet ACL，則當已打開 VNet 服務終結點的 VNet 或子網被刪除時，將禁用與該 Azure 服務關聯的 VNet ACL 資訊。
 
### <a name="what-happens-if-an-azure-service-account-that-has-a-vnet-service-endpoint-enabled-is-deleted"></a>如果已啟用 VNet 服務終結點的 Azure 服務帳戶被刪除，會發生什麼情況？
刪除 Azure 服務帳戶是一項獨立操作，即使在網路端啟用服務終結點並在 Azure 服務端設置 VNet ACL 時，也支援該操作。 
...
當虛擬網路服務端點啟用時，您虛擬網路中的資源用來傳送流量至 Azure 服務的來源 IP 位址，會從使用公用 IPV4 位址切換為使用 Azure 虛擬網路的私人位址。 請注意，這可能導致 Azure 服務上較早以前設置為公共 IPV4 位址的特定 IP 防火牆失敗。 

### <a name="does-the-service-endpoint-route-always-take-precedence"></a>服務終結點路由是否始終優先？
服務終結點添加系統路由，該路由優先于 BGP 路由，並為服務終結點流量提供最佳路由。 服務端點接受的服務流量，一律都是直接從您的虛擬網路到 Microsoft Azure 骨幹網路上的服務。 有關 Azure 如何選擇路由的詳細資訊，請參閱[Azure 虛擬網路流量路由](virtual-networks-udr-overview.md)。
...
擁有虛擬網路寫入權的使用者，可以在虛擬網路上個別設定服務端點。 要保護 Azure 服務資源到 VNet，使用者必須具有要添加子網的**Microsoft.網路/虛擬網路/子網/joinViaService終結點/操作**許可權。 此權限預設會隨附在內建的服務管理員角色中，可藉由建立自訂角色加以修改。 深入了解內建角色以及如何將特定權限指派給[自訂角色](https://docs.microsoft.com/azure/role-based-access-control/custom-roles?toc=%2fazure%2fvirtual-network%2ftoc.json)。
...
### <a name="does-azure-active-directory-azure-ad-support-vnet-service-endpoints"></a>Azure 活動目錄 （Azure AD） 是否支援 VNet 服務終結點？

Azure 活動目錄 （Azure AD） 不支援本機服務終結點。 支援 VNet 服務終結點的 Azure 服務的完整清單[可在此處](https://docs.microsoft.com/azure/virtual-network/virtual-network-service-endpoints-overview)看到。 請注意，支援服務終結點的服務下列出的"Microsoft.AzureActiveDirectory"標記用於支援 ADLS 第 1 代的服務終結點。 對於 ADLS 第 1 代，Azure 資料湖存儲第 1 代的虛擬網路集成利用虛擬網路和 Azure 活動目錄 （Azure AD） 之間的虛擬網路服務終結點安全性在訪問權杖中生成其他安全聲明。 這些宣告隨後會用來對 Data Lake Storage Gen1 帳戶驗證虛擬網路並允許存取。 瞭解有關[Azure 資料湖存儲第 1 代 VNet 集成的更多](../data-lake-store/data-lake-store-network-security.md?toc=%2fazure%2fvirtual-network%2ftoc.json)
...
虛擬網路中的 VNet 服務端點總數沒有限制。 對於 Azure 服務資源（如 Azure 存儲帳戶），服務可能會對用於保護資源的子網數實施限制。 下表顯示一些範例限制： 
