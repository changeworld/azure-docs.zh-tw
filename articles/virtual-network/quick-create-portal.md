...
description: 快速入門：在 Azure 入口網站中建立虛擬網路。 這些網路可讓 Azure 資源（例如 Vm）安全地與彼此和網際網路通訊。
author: KumudD
...
ms.locfileid: "79128489"
---
# <a name="quickstart-create-a-virtual-network-using-the-azure-portal"></a>快速入門：使用 Azure 入口網站建立虛擬網路

在本快速入門中，您將瞭解如何使用 Azure 入口網站建立虛擬網路。 您會部署兩部虛擬機器（Vm）。 接下來，您可以在 Vm 之間安全地進行通訊，並從網際網路連線到 Vm。 虛擬網路是私人網路在 Azure 中的基本建置組塊。 它可讓 Azure 資源（例如 Vm）安全地彼此通訊，以及與網際網路通訊。
...
1. 選取 **[下一步]： [IP 位址]** ，然後針對 [ **IPv4 位址空間**]，輸入*10.1.0.0/16*。

1. 選取 [**新增子網**]，然後輸入子網**名稱**的*myVirtualSubnet*和**子網位址範圍**的*10.1.0.0/24* 。

1. 選取 [**新增**]，然後選取 [**審核 + 建立**]。 將其他項目保留為預設值，然後選取 [建立]。
...
1. 在 [建立虛擬機器 - 基本] 中，輸入或選取這項資訊：
    ...
    | 區域 | 選取 [美國東部]。 |
    | 可用性選項 | 預設為**不需要基礎結構冗余**。 |
    | 影像 | 預設為**Windows Server 2019 Datacenter**。 |
    | 大小 | 預設為**標準 DS1 v2**。 |
    | **系統管理員帳戶** |  |
    | 使用者名稱 | 輸入您選擇的使用者名稱。 |
...
1. 在 [建立虛擬機器 - 網路] 中，選取這項資訊：
    ...
    | 設定 | 值 |
    | ------- | ----- |
    | 虛擬網路 | 預設為**myVirtualNetwork**。 |
    | 子網路 | 預設為**myVirtualSubnet （10.1.0.0/24）** 。 |
    | 公用 IP | 預設為 **（新的） myVm-ip**。 |
    | NIC 網路安全性群組 | 預設為 [**基本**]。 |
    | 公用輸入連接埠 | 預設為**允許選取的埠**。 |
    | 選取輸入連接埠 | 預設為**HTTP**和**RDP**。
...
1. 在 [建立儲存體帳戶] 中，輸入或選取這項資訊：
    ...
    | 帳戶類型 | 預設為**儲存體（一般用途 v1）** 。 |
    | 效能 | 預設為 [**標準**]。 |
    | 複寫 | 預設為**本機-多餘儲存體（LRS）** 。 |

1. 選取 **[確定]** ，然後選取 [**審核 + 建立**]。 您會移至 [檢閱 + 建立] 頁面，其中 Azure 會驗證您的設定。

1. 當您看到 [驗證成功] 訊息時，請選取 [建立]。

### <a name="create-the-second-vm"></a>建立第二個 VM

重複上一節中的程式，以建立另一個虛擬機器。

> [!IMPORTANT]
> 在 [**虛擬機器名稱**] 中，輸入*myVm2*。
>
> 針對**診斷儲存體帳戶**，請確定您選取**myvmstorageaccount**，而不是建立一個。
...
若要深入瞭解虛擬網路設定，請參閱[建立、變更或刪除虛擬網路](manage-virtual-network.md)。

根據預設，Azure 允許在 Vm 之間進行安全通訊。 Azure 只允許從網際網路到 Windows Vm 的輸入遠端桌面連線。 若要深入瞭解 VM 網路通訊的類型，請參閱[篩選網路流量](tutorial-filter-network-traffic.md)。
