...
當您符合需求時，可以依照教學課程：使用 Azure 入口網站建立和設定對等互連來連線[虛擬網路與虛擬網路對等互連](https://docs.microsoft.com/azure/virtual-network/virtual-network-create-peering)。
...
針對 Azure PowerShell，請執行命令[get-azurermvirtualnetworkpeering](https://docs.microsoft.com/powershell/module/azurerm.network/get-azurermvirtualnetworkpeering?view=azurermps-4.1.0)以取得虛擬網路對等互連。 以下是範例：
...
疑難排解文章[： Azure 點對站連線問題](https://docs.microsoft.com/azure/vpn-gateway/vpn-gateway-troubleshoot-vpn-point-to-site-connection-problems)會逐步解說點對站連線的常見問題。
...
Azure 目前使用兩種部署模型：Resource Manager 和傳統。 這兩個模型彼此不完全相容。 若要使用不同的模型來設定多網站連線，請參閱下列文章：
...
4. 選取您想要查看的對等互連。 然後 **，選取 [設定]，以**驗證您已在連線到 expressroute 線路的 VNetA 網路上啟用 [**允許閘道傳輸**]，並使用未連接到 Expressroute 線路之遠端 VNetB 網路上的 [**遠端閘道**]。
