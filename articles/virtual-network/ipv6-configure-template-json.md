---
title: 使用 Azure 虛擬網路中的基本 Load Balancer 部署 IPv6 雙重堆疊應用程式-Resource manager 範本
...
本文提供 IPv6 設定工作的清單，其中包含適用于 Azure Resource Manager VM 範本的部分。 使用本文中所述的範本，透過基本 Load Balancer （包含具有 IPv4 和 IPv6 子網的雙重堆疊虛擬網路）部署雙重堆疊 (IPv4 + IPv6) 應用程式、具有雙重 (IPv4 + IPv6) 前端設定的基本 Load Balancer、具有雙重 IP 設定、網路安全性群組和公用 Ip 的 Nic 的 Vm。

若要使用 Standard Load Balancer 來部署雙協定 (IPV4 + IPv6) 應用程式，請參閱 [使用 Standard Load Balancer 範本部署 IPv6 雙重堆疊應用程式](ipv6-configure-standard-load-balancer-template-json.md)。
