---
title: 在 Azure 虛擬網路中部署具有基本負載等化器的 IPv6 雙堆疊應用程式 - 資源管理器範本（預覽）
...
---
# <a name="deploy-an-ipv6-dual-stack-application-with-basic-load-balancer-in-azure---template-preview"></a>在 Azure - 範本（預覽）中部署具有基本負載等化器的 IPv6 雙堆疊應用程式

本文提供了 IPv6 配置任務的清單，其仲介紹了適用于 Azure 資源管理器 VM 範本的部分。 使用本文中描述的範本，使用基本負載等化器部署雙堆疊 （IPv4 + IPv6） 應用程式，該應用程式包括具有 IPv4 和 IPv6 子網的雙堆疊虛擬網路、具有雙 （IPv4 + IPv6） 前端配置的基本負載等化器、具有 V 的 VM具有雙 IP 配置、網路安全性群組和公共 IP 的 NIC。

要使用標準負載等化器部署雙堆疊 （IPV4 + IPv6） 應用程式，請參閱[使用標準負載等化器部署 IPv6 雙堆疊應用程式 - 範本](ipv6-configure-standard-load-balancer-template-json.md)。
...
要使用 Azure 資源管理器範本在 Azure 虛擬網路中部署具有基本負載等化器的 IPv6 雙堆疊應用程式，[請在此處](https://azure.microsoft.com/resources/templates/ipv6-in-vnet/)查看示例範本。
