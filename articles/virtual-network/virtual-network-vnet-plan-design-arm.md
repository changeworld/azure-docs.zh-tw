...
- 您可以將零或一個網路安全性群組關聯至虛擬網路中的每個子網路。 您可以將相同或不同的網路安全性群組關聯至每個子網路。 每個網路安全性群組都包含規則，能允許或拒絕進出來源與目的地的流量。 瞭解有關[網路安全性群組](#traffic-filtering)的更多資訊。
...
您可以使用[NVA](/azure/architecture/reference-architectures/dmz/secure-vnet-dmz?toc=%2Fazure%2Fvirtual-network%2Ftoc.json)查看在 Azure 和 Internet 之間實現周邊網路（也稱為 DMZ）的示例設計。

### <a name="traffic-routing"></a>流量路由

Azure 會針對來自子網路的輸出流量建立數個預設路由。 您可以透過建立路由表並將它關聯至子網路，來覆寫 Azure 的預設路由。 覆寫 Azure 預設路由的常見原因包括：
- 您想讓子網路之間的流量流經 NVA。 詳細瞭解如何[配置路由表以強制流量通過 NVA](tutorial-create-route-table-portal.md)。
