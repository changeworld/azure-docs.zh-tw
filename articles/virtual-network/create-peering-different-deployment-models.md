...
可以使用 Azure 門戶、Azure[命令列介面](#cli)（CLI）、Azure [PowerShell](#powershell)或 Azure 資源管理器範本創建虛擬網路對等互連。 按一下任何先前的工具連結，直接前往使用您所選工具建立虛擬網路對等互連的步驟。

## <a name="create-peering---azure-portal"></a>建立對等互連 - Azure 入口網站

1. 登錄到 Azure[門戶](https://portal.azure.com)。 您登入時使用的帳戶必須擁有必要的權限，才能建立虛擬網路對等互連。 如需權限清單，請參閱[虛擬網路對等互連權限](virtual-network-manage-peering.md#requirements-and-constraints)。
...
1. 在門戶搜索框中，輸入**我的資源組**。 在搜尋結果中，按一下 **"我的資源組**"。
2. 在 **"我的資源組**"邊欄選項卡上，按一下 **"刪除**"圖示。
3. 要確認刪除，請在 **"鍵入資源組名稱"** 框中輸入 **"資源組**"，然後按一下"**刪除**"。
