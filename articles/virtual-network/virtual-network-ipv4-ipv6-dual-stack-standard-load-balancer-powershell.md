---
title: 部署 IPv6 雙堆疊應用程式 - 標準負載均衡器 - PowerShell
titlesuffix: Azure Virtual Network
description: 本文演示如何使用 Azure Powershell 在 Azure 虛擬網路中使用標準負載均衡器部署 IPv6 雙堆疊應用程式。
...
---
# <a name="deploy-an-ipv6-dual-stack-application-in-azure---powershell"></a>在 Azure 中部署 IPv6 雙堆疊應用程式 - PowerShell

本文介紹如何在 Azure 中使用標準負載均衡器部署雙堆疊 (IPv4 + IPv6) 應用程式,該應用程式包括雙堆疊虛擬網路和子網、具有雙 (IPv4 + IPv6) 前端配置的標準負載均衡器、具有雙 IP 配置的 NIC 的 VM、網路安全組和公共 IP。
...
在本節中,您可以為負載均衡器配置雙前端 IP(IPv4 和 IPv6)和後端位址池,然後創建標準負載均衡器。
...
使用[新負載平衡器](/powershell/module/az.network/new-azloadbalancer)創建標準負載均衡器。 以下範例使用您在上述步驟中建立的 IPv4 和 IPv6 前端 IP 設定、後端池和負載平衡規則建立名為*myLoad平衡器*的公共標準負載平衡器:
...
使用 [New-AzVirtualNetwork](/powershell/module/az.network/new-azvirtualnetwork) 建立虛擬網路。 下面的範例使用*我的子網*建立名為*dsVnet*的虛擬網路:
...
在本文中,您創建了具有雙前端 IP 配置(IPv4 和 IPv6)的標準負載均衡器。 還創建了兩個虛擬機,其中包括具有雙 IP 配置 (IPV4 + IPv6) 的 NIC,這些配置已添加到負載均衡器的後端池中。 要瞭解有關 Azure 虛擬網路中 IPv6 支援的更多資訊,請參閱[什麼是 Azure 虛擬網路的 IPv6?](ipv6-overview.md)
