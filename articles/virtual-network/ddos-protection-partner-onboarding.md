...
- 或者，若要在受保護的公用 IP 有作用中的緩和措施時收到通知，您可以在 [DDoS 攻擊] 下設定計量的 [警示](manage-ddos-protection.md#configure-alerts-for-ddos-protection-metrics) 。
- 此外，您還可以選擇建立其他 DDoS 計量的警示，並 [設定攻擊分析](manage-ddos-protection.md#configure-ddos-attack-analytics) ，以瞭解攻擊的規模、要卸載的流量、攻擊媒介、熱門參與者和其他詳細資料。

![DDoS 計量](./media/ddos-protection-partner-onboarding/ddos-metrics.png)

### <a name="ddos-rapid-response-drr"></a>DDoS 快速回應 (DRR) 
DDoS 保護標準客戶可在主動攻擊期間存取 [快速回應小組](https://azure.microsoft.com/blog/ddos-protection-attack-analytics-rapid-response/) 。 DRR 可協助進行攻擊調查、攻擊期間的自訂緩和措施，以及攻擊後的分析。
...
3. 使用第一個步驟中建立的 DDoS 保護計劃，在具有公用端點的服務的虛擬網路上啟用 Azure DDoS 保護標準。 如需 stpe 逐步指示，請參閱 [啟用 DDoS 標準保護計劃](manage-ddos-protection.md#enable-ddos-for-an-existing-virtual-network)
   > [!IMPORTANT] 
   > 在虛擬網路上啟用 Azure DDoS 保護 Standard 之後，該虛擬網路內的所有公用 Ip 都會自動受到保護。 這些公用 Ip 的來源可以是 Azure (用戶端訂用帳戶) 或 Azure 外部。 
4. （選擇性）將 Azure DDoS 保護標準遙測和攻擊分析整合至應用程式特定的客戶面向儀表板。 如需使用遙測的詳細資訊，請參閱 [使用 DDoS 保護遙測](manage-ddos-protection.md#use-ddos-protection-telemetry)。 如需設定攻擊分析的詳細資訊，請參閱 [設定 DDoS 攻擊分析](manage-ddos-protection.md#configure-ddos-attack-analytics)
