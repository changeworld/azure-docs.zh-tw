---
title: 部署 IPv6 雙堆疊應用程式 - 基本負載等化器 - CLI
...
ms.locfileid: "80239859"
---
# <a name="deploy-an-ipv6-dual-stack-application-using-basic-load-balancer---cli-preview"></a>使用基本負載等化器部署 IPv6 雙堆疊應用程式 - CLI（預覽版）

本文介紹如何使用 Azure CLI 使用基本負載等化器部署雙堆疊 （IPv4 + IPv6） 應用程式，該虛擬網路包括雙堆疊子網的雙堆疊虛擬網路、具有雙 （IPv4 + IPv6） 前端配置的基本負載等化器、具有 NIC 的 VM具有雙 IP 配置、雙網路安全性群組規則和雙公共 IP。

要使用標準負載等化器部署雙堆疊 （IPV4 + IPv6） 應用程式，請參閱[使用 Azure CLI 使用標準負載等化器部署 IPv6 雙堆疊應用程式](virtual-network-ipv4-ipv6-dual-stack-standard-load-balancer-cli.md)。
...
在本節中，您可以為負載等化器配置雙前端 IP（IPv4 和 IPv6）和後端位址集區，然後創建基本負載等化器。

### <a name="create-load-balancer"></a>建立負載平衡器

使用 az 網路 lb 創建基本負載等化器[，創建](https://docs.microsoft.com/cli/azure/network/lb?view=azure-cli-latest)名為 dsLB 的命名**dsLB，** 其中包括名為**dsLbFrontEnd_v4**的前端池，名為**dsLbBackEndPool_v4**的後端池，該後端池與您在前面的步驟中創建的 IPv4 公共 IP 位址**dsPublicIP_v4**相關聯。 
...
在本文中，您創建了具有雙前端 IP 配置（IPv4 和 IPv6）的基本負載等化器。 還創建了兩個虛擬機器，其中包括具有雙 IP 配置 （IPV4 + IPv6） 的 NIC，這些配置已添加到負載等化器的後端池中。 要瞭解有關 Azure 虛擬網路中 IPv6 支援的更多資訊，請參閱[什麼是 Azure 虛擬網路的 IPv6？](ipv6-overview.md)
