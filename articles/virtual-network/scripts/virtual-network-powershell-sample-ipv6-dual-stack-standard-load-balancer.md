---
title: 'Azure PowerShell 腳本範例-使用 Standard Load Balancer (preview 設定 IPv6 前端) '
titlesuffix: Azure Virtual Network
description: 瞭解如何在虛擬網路腳本範例中使用 Standard Load Balancer 設定 IPv6 前端。
...
ms.locfileid: "98223188"
---
# <a name="configure-ipv6-frontend-in-virtual-network-script-sample-with-standard-load-balancerpreview"></a>在虛擬網路腳本範例中使用 Standard Load Balancer (preview 設定 IPv6 前端) 
...
在 Azure 中部署雙重堆疊應用程式之前，您必須使用下列 Azure PowerShell，只針對此預覽功能設定您的訂用帳戶一次：
...
| [New-AzAvailabilitySet](/powershell/module/az.compute/new-azavailabilityset) | 建立可用性設定組。 可用性設定組可確保應用程式的執行時間，方法是將虛擬機器分散到實體資源，如此一來，如果發生失敗，整個集合就不會受到影響。 |
