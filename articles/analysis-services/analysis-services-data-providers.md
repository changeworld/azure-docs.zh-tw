...
ms.locfileid: "81676571"
---
# <a name="client-libraries-for-connecting-to-analysis-services"></a>連線到分析服務的用戶端庫

用戶端程式庫是用戶端應用程式和工具連接到 Analysis Services 伺服器的必備條件。 Microsoft 用戶端應用程式(如 Power BI 桌面、Excel、SQL 伺服器管理工作室 (SSMS) 和分析服務)為 Visual Studio 投影擴展,可安裝所有三個用戶端庫,並定期更新它們。 在某些情況下,您可能需要安裝較新版本的用戶端庫。 自定義用戶端應用程式還需要安裝用戶端庫。
...
分析服務管理物件 (AMO) 與 ADOMD 用戶端函式庫可作為可安裝的套件從[NuGet.org](https://www.nuget.org/)。建議您遷移到 NuGet 引用,而不是使用 Windows 安裝程式。 
...
用於用戶端連線的用戶端程式庫，與從 Azure Analysis Services 伺服器連接到資料來源所需的資料提供者不同。 要瞭解有關資料來源連線的更多情況,請參考[資料來源連線](analysis-services-datasource.md)。
...
2.  右鍵按**下 msolap.dll** > **屬性** > **詳細資訊**。 如果檔名為 msolap140.dll，就會比最新版本還舊，而應進行升級。
