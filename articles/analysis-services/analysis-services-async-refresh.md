...
description: 說明如何使用 Azure Analysis Services REST API 來撰寫模型資料的非同步重新整理程式碼。
author: minewiskan
...
例如，在名為的伺服器上，考慮名為 AdventureWorks 的模型 `myserver` ，位於美國西部 Azure 區域。 伺服器名稱是：
...
若要檢查重新整理作業的狀態，請對重新整理識別碼使用 GET 動詞命令。 以下是回應主體的範例。 如果作業正在進行中， `inProgress` 就會傳回狀態。
...
執行重新整理作業之後，可能需要將新的資料與用於查詢相應放大的複本同步處理。若要執行模型的同步處理作業，請在/sync 函式上使用 POST 動詞命令。 回應中的位置標頭 (Location) 包含重新整理作業的識別碼。
...
程式碼範例會使用 [服務主體](#service-principal) 驗證。
...
1.    在程式碼範例中，尋找 [ **字串授權單位 = ...**]，將 **common** 取代為您組織的租使用者識別碼。
2.    註解/取消註解，以便使用 ClientCredential 類別來具現化認證物件。 確定 \<App ID> 和 \<App Key> 值是以安全的方式存取，或針對服務主體使用以憑證為基礎的驗證。
3.    執行範例。
