...
description: 介紹如何設置 Azure 資源診斷日誌記錄以監視 Azure 分析服務伺服器。
author: minewiskan
...
ms.locfileid: "79266178"
---
# <a name="setup-diagnostic-logging"></a>設定診斷記錄

在任何 Analysis Services 解決方案中，監視您伺服器的執行方式皆是一大重點功能。 使用[Azure 資源日誌](../azure-monitor/platform/platform-logs-overview.md)，可以監視日誌並將其發送到[Azure 存儲](https://azure.microsoft.com/services/storage/)，將它們資料流到 Azure[事件中心](https://azure.microsoft.com/services/event-hubs/)，並將它們匯出到[Azure 監視器日誌](../azure-monitor/azure-monitor-log-hub.md)。
...
"指標"類別將相同的[伺服器指標](analysis-services-monitor.md#server-metrics)記錄到 Azure Metrics 表。 如果使用查詢[橫向擴展](analysis-services-scale-out.md)，並且需要為每個讀取副本分隔指標，請改用 Azure 診斷表，其中**操作名稱**等於**LogMetric**。
...
2. 在 [診斷設定]**** 中，指定下列選項： 
    ...
    * **指標**. 選取此選項可儲存[計量](analysis-services-monitor.md#server-metrics)中的詳細資料。 如果您要封存至儲存體帳戶，可以為診斷記錄選取保留期限。 保留期限過後，就會自動刪除記錄。
...
在查詢建立器中，展開**日誌管理** > **Azure 診斷**。 AzureDiagnostics 包括「引擎」和「服務」事件。 留意到查詢是即時建立的。 EventClass\_s 的欄位包含 xEvent 名稱，如果您曾經使用 xEvents 進行內部部署記錄，這些名稱就可能看起來似曾相似。 按一下 [EventClass\_s]**** 或其中一個事件名稱，Log Analytics 工作區將會繼續建構查詢。 請務必儲存您的查詢，以供日後重複使用。

### <a name="example-queries"></a>查詢範例

#### <a name="example-1"></a>範例 1

以下查詢返回模型資料庫和伺服器的每個查詢結束/刷新結束事件的持續時間。 如果橫向擴展，則結果按副本分解，因為副本編號包含在ServerName_s中。 按RootActivityId_g分組可減少從 Azure 診斷 REST API 檢索的行計數，並有助於保持在[日誌分析速率限制](https://dev.loganalytics.io/documentation/Using-the-API/Limits)中所述的限制範圍內。
...
以下查詢返回伺服器的記憶體和 QPU 消耗。 如果橫向擴展，則結果按副本分解，因為副本編號包含在ServerName_s中。
...
在本快速教學課程中，您可以在與 Analysis Service 伺服器相同的訂用帳戶和資源群組中，建立儲存體帳戶。 然後，使用 Set-Az診斷設置打開診斷日誌記錄，將輸出發送到新的存儲帳戶。
...
要啟用日誌記錄，請使用"設置-Az診斷設置"Cmdlet 以及新存儲帳戶、伺服器帳戶和類別的變數。 執行下列命令，將 **-Enabled** 旗標設為 **$true**：
...
請參閱 PowerShell 説明中的["設置-A診斷設置](https://docs.microsoft.com/powershell/module/az.monitor/set-azdiagnosticsetting)"。
