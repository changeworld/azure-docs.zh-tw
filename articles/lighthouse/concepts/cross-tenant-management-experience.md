...
> [!TIP]
> 如果在使用 Azure CLI 時看不到這些值，請嘗試通過運行`az account clear`後跟`az login --identity`來清除緩存。

我們還提供特定于執行 Azure 委派資源管理工作的 API。 如需詳細資訊，請參閱**參考**一節。
...
- 備份及還原客戶租用戶中的客戶資料
- 使用[備份資源管理器](../../backup/monitor-azure-backup-with-backup-explorer.md)説明查看備份項的操作資訊（包括尚未為備份配置的 Azure 資源）以及委派訂閱的監視資訊（作業和警報）。 備份資源管理器當前僅適用于 Azure VM 資料。
...
- [跟蹤跨多個客戶租戶的攻擊並查看安全警報](https://techcommunity.microsoft.com/t5/azure-sentinel/using-azure-lighthouse-and-azure-sentinel-to-monitor-across/ba-p/1043899)
...
[Azure 網站恢復](../../site-recovery/index.yml)：
...
- 由 Azure Resource Manager 所處理的要求可以使用 Azure 委派的資源管理來執行。 這些要求的作業 URI 會以 `https://management.azure.com` 作為開頭。 但是，Azure 委派的資源管理不支援由資源類型的實例（如 KeyVault 機密訪問或存儲資料訪問）處理的請求。 這些要求的作業 URI 通常會以您執行個體特有的位址作為開頭，例如 `https://myaccount.blob.core.windows.net` 或 `https://mykeyvault.vault.azure.net/`。 此外，後者通常是資料作業，而非管理作業。 
...
- 雖然可以板載使用 Azure 資料塊的訂閱，但管理租戶中的使用者此時無法在委派的訂閱上啟動 Azure 資料塊工作區。
