...
ms.locfileid: "79270624"
---
# <a name="view-and-manage-service-providers"></a>檢視和管理服務提供者

客戶可以使用[Azure 門戶](https://portal.azure.com)中的 **"服務提供者"** 頁來查看有關服務提供者和服務提供者產品/服務的資訊，通過[Azure 委派的資源管理](../concepts/azure-delegated-resource-management.md)委派特定資源，以及購買新的服務提供者產品/服務。 雖然我們在此所述的是服務提供者與其客戶，但管理多個租用戶的企業也可以使用相同的程序來合併其管理體驗。

若要存取 Azure 入口網站中的 [服務提供者]**** 頁面，客戶可選取 [所有服務]****，然後搜尋**服務提供者**並選取它。 他們也可在 Azure 入口網站頂端附近的搜尋方塊中輸入「服務提供者」來找到它。

請記住，"**服務提供者"** 頁僅顯示有關通過 Azure 委派資源管理有權訪問客戶訂閱或資源組的服務提供者的資訊。 如果與客戶合作的其他服務提供者不是使用 Azure 委派的資源管理來存取客戶資源，則那些服務提供者的相關資訊不會在這裡顯示。
...
要查看有關服務提供者的資訊，客戶可以在"服務提供者"頁面的左側選擇 **"供應商"產品****/** 服務。
...
通過頁面頂部的篩選器，您可以對委派資訊進行排序和分組。 您還可以按特定客戶、優惠或關鍵字進行篩選。

> [!NOTE]
> 客戶在[Azure 門戶中或通過 API 查看委派作用域的角色指派資訊](../../role-based-access-control/role-assignments-list-portal.md#list-role-assignments-at-a-scope)時，不會看到這些角色指派，也不會看到被授予這些角色的服務提供者租戶的任何使用者。

## <a name="audit-delegations-in-your-environment"></a>審核環境中的審核授權

客戶可能希望瞭解已委派給服務提供者進行[Azure 委派資源管理的](../concepts/azure-delegated-resource-management.md)訂閱和/或資源組。 這對於訂閱量大或具有許多執行管理工作的使用者的客戶特別有用。

我們提供[Azure 策略內置策略定義](../../governance/policy/samples/built-in-policies.md#lighthouse)，用於審核作用域委派給管理租戶。 您可以將此策略分配給管理組，該管理組包含要審核的所有訂閱。 當您檢查此策略是否符合時，任何委派的訂閱和/或資源組（在策略分配的管理組中）將以不合規狀態顯示。 然後，您可以查看結果並確認沒有意外的代表團。

有關如何分配策略和查看合規性狀態結果的詳細資訊，請參閱[快速入門：創建策略分配](../../governance/policy/assign-policy-portal.md)。
