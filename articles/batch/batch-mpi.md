...
多重執行個體工作可讓您在多個計算節點上同時執行 Azure Batch 工作。 這些工作可以在 Batch 中實現高效能運算案例，例如訊息傳遞介面 (MPI) 應用程式。 在本文中，您將瞭解如何使用 [Batch .NET][api_net] 程式庫來執行多重執行個體工作。
...
多重執行個體工作需要有**已啟用節點間通訊**和**已停用並行工作執行**的集區。 若要停用並行工作執行，請將 [CloudPool.MaxTasksPerComputeNode](/dotnet/api/microsoft.azure.batch.cloudpool) 屬性設定為 1。
...
當您在 Batch 集區中選擇 [支援 RDMA 大小](../virtual-machines/sizes-hpc.md?toc=/azure/virtual-machines/windows/toc.json) (如 A9 計算節點) 時，MPI 應用程式可以利用 Azure 高效能、低延遲的遠端直接記憶體存取 (RDMA) 網路。
...
2. 在 Visual Studio 2019 中開啟 MultiInstanceTasks **解決方案**。 `MultiInstanceTasks.sln` 方案檔位於︰
