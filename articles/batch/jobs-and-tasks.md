...
作業會指定工作執行所在的[集區](nodes-and-pools.md#pools)。 您可以為每個作業建立新的集區，或將集區使用於許多工作。 您可以針對與 [作業排程](#scheduled-jobs)相關聯的每個作業建立集區，或針對與作業排程相關聯的所有作業建立一個集區。

### <a name="job-priority"></a>作業優先順序

您可以將選擇性的作業優先順序指派給您所建立的作業。 Batch 服務會使用作業的優先順序值，判斷工作中所有工作的排程 (的順序) wtihin 每個集區。

若要更新作業的優先順序，請呼叫 [更新作業作業的屬性](/rest/api/batchservice/job/update) (batch REST) ，或修改 [>usestaskdependencies 的優先順序](/dotnet/api/microsoft.azure.batch.cloudjob) (batch .net) 。 優先順序值的範圍從-1000 (最低優先順序) 到 1000 (最高優先順序) 。

在相同的集區中，較高優先順序的作業優先順序高於較低優先順序的作業。 較高優先順序作業中的工作將不會佔用較低優先順序作業中已在執行中的工作。 具有相同優先權層級的作業，可能會有相同的排程，而不會定義工作執行的順序。

在一個集區中執行高優先順序值的作業不會影響在不同集區或不同 Batch 帳戶中執行的作業排程。 作業優先順序不適用於提交工作時所建立的 [autopools](nodes-and-pools.md#autopools)。
...
根據預設，作業內的所有工作都完成時，作業仍會保持作用中狀態。 您可以變更此行為，讓作業在其中的所有工作完成時自動終止。 將 ([>onalltaskscomplete](/dotnet/api/microsoft.azure.batch.cloudjob) in Batch) .net 中的作業 **>onalltaskscomplete** 屬性設定為 `terminatejob` * '，以在其所有工作都處於已完成狀態時，自動終止作業。

Batch 服務會將「沒有」工作的作業視為其所有工作都已完成。 因此，這個選項最常搭配 [作業管理員工作](#job-manager-task)使用。 如果您想要在沒有作業管理員的情況下使用自動作業終止，您應該先將新作業的 **>onalltaskscomplete** 屬性設定為 `noaction` ，然後在完成將工作新增至作業之後，才將它設定為 `terminatejob` * '。
