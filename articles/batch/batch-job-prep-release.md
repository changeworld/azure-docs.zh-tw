---
title: 創建任務以準備&計算節點上完成作業 - Azure 批次處理
...
在執行作業的任務之前，Batch 將在計畫運行任務的每個計算節點上執行作業準備任務。 預設情況下，Batch 在等待作業準備任務完成，然後再運行計畫在節點上執行的任務。 不過，您可以設定服務不要等待。 如果節點重新開機，作業準備任務將再次運行。 您還可以禁用此行為。 如果作業準備任務和作業管理器任務配置了作業，則作業準備任務在作業管理器任務之前運行，就像所有其他任務一樣。 作業準備任務始終首先運行。
...
作業發佈任務最多可以運行 15 分鐘，然後由 Batch 服務終止。 有關詳細資訊，請參閱 REST [API 參考文檔](https://docs.microsoft.com/rest/api/batchservice/job/add#jobreleasetask)。
> 
> 
