...
您可以在建立 Batch 帳戶時或在稍後，建立儲存體帳戶與 Batch 帳戶的關聯。 在選擇儲存體帳戶時，請考慮您的成本和效能需求。 例如，相較於 GPv1，GPv2 和 Blob 儲存體帳戶選項支援更大的[容量和延展性限制](https://azure.microsoft.com/blog/announcing-larger-higher-scale-storage-accounts/)。 （請與 Azure 支援聯繫以請求增加存儲限制。這些帳戶選項可以提高 Batch 解決方案的性能，這些解決方案包含大量從存儲帳戶讀取或寫入存儲帳戶的並行任務。
...
節點可以執行節點作業系統環境所支援的任何可執行檔或指令碼。 可執行檔或腳本包括\*適用于 Windows\*的 .exe、.cmd、.bat\*和 PowerShell 腳本，以及適用于 Linux 的二進位檔案、shell 和 Python 腳本。
...
* **虛擬機器配置**，它指定池由 Azure 虛擬機器組成。 這些 VM 可能會從 Linux 或 Windows 映像加以建立。

    以虛擬機器設定作為基礎建立集區時，您不僅需要指定節點的大小和用來建立這些節點的映像來源，也必須在節點上安裝**虛擬機器映像參考**和 Batch **節點代理程式 SKU**。 如需指定這些集區屬性的詳細資訊，請參閱 [在 Azure Batch 集區中佈建 Linux 計算節點](batch-linux-nodes.md)。 您可以選擇性地將一或多個空的資料磁碟連結至從 Marketplace 映像建立的集區 VM，或是在自訂映像中包含資料磁碟，以建立 VM。 在包含資料磁片時，您需要從 VM 中裝載和格式化磁片才能使用它們。

* **雲服務配置**，它指定池由 Azure 雲服務節點組成。 雲服務*僅*提供 Windows 計算節點。

    雲端服務組態集區可用的作業系統列於 [Azure 客體 OS 版次與 SDK 相容性矩陣](../cloud-services/cloud-services-guestos-update-matrix.md)。 建立包含雲端服務節點的集區時，您需要指定節點大小及其「作業系統系列」**。 雲服務部署到 Azure 的速度比運行 Windows 的虛擬機器更快。 如果您需要 Windows 計算節點的集區，可能會發現雲端服務提供了部署時間方面的效能優勢。
...
當您建立一個集區時，必須選取適當的 **nodeAgentSkuId**，視您 VHD 的基礎映像 OS 而定。 通過調用[清單支援的節點代理 SKU](https://docs.microsoft.com/rest/api/batchservice/list-supported-node-agent-skus)操作，可以獲取可用節點代理 SKU ID 與其 OS 映射引用的映射。

#### <a name="custom-images-for-virtual-machine-pools"></a>適用於虛擬機器集區的自訂映像

要瞭解如何使用自訂映射創建池，請參閱[使用共用圖像庫創建自訂池](batch-sig-images.md)。

或者，您可以使用[託管映射](batch-custom-images.md)資源創建虛擬機器的自訂池。 如需有關從 Azure VM 準備自訂 Linux 映像的資訊，請參閱[如何建立虛擬機器或 VHD 的映像](../virtual-machines/linux/capture-image.md)。 如需有關從 Azure VM 準備自訂 Windows 映像的資訊，請參閱[在 Azure 中建立一般化 VM 的受控映像](../virtual-machines/windows/capture-image-resource.md)。
...
* **低優先順序的計算節點。** 低優先順序的節點會利用 Azure 中剩餘的容量來執行 Batch 工作負載。 低優先順序節點每小時比專用節點便宜，並且支援需要大量計算能力的工作負載。 如需詳細資訊，請參閱[使用低優先順序的 VM 搭配 Batch](batch-low-pri-vms.md)。

    Azure 多餘的容量不足時，可能會佔用低優先順序的計算節點。 如果節點在執行工作時遭到佔用，工作會重新排入佇列並於計算節點再次可用時再次執行。 低優先順序的節點很適合用於作業完成時間有彈性且工作分散於許多節點的工作負載。 在決定為方案使用低優先順序節點之前，請確保因搶佔而丟失的任何工作都將是最小的，並且易於重新創建。

您可以在相同的集區中同時擁有低優先順序和專用的計算節點。 每種節點類型 &mdash; 低優先順序和專用&mdash; 有它自己的目標設定，您可以對其指定所需的節點數目。

計算節點數目稱為「目標」**，因為在某些情況下，您的集區可能無法達到所需的節點數目。 例如，如果集區先達到 Batch 帳戶的[核心配額](batch-quota-limit.md)，它就可能無法達成目標。 或者，如果向池應用了限制最大節點數的自動縮放公式，則池可能無法達到目標。
...
對於動態工作負荷，可以編寫自動[縮放公式](#scaling-compute-resources)並將其應用於池。 Batch 服務將會定期評估您的公式，並根據可以指定的各種集區、作業、和工作參數，調整集區中的節點數目。
...
您還可以指定*填充類型*，該類型確定 Batch 是將任務均勻地跨到池中的所有節點，還是將每個節點與最大任務數打包，然後再將任務分配給另一個節點。
...
啟用節點間通信也會影響節點在群集中的位置，並且由於部署限制，可能會限制池中的最大節點數。 如果應用程式不需要節點之間的通信，Batch 服務可以從許多不同的群集和資料中心向池分配大量潛在的節點，以增強並行處理能力。
...
* 您可以使用作業 **條件約束** 來為作業指定特定的限制：
    ...
    Batch 可以偵測並重試失敗的工作。 您可以指定**工作重試次數上限**作為條件約束，包括要「一律」** 重試工作還是「決不」** 重試工作。 重試任務意味著重新排隊以再次運行任務。
...
* 根據預設，作業內的所有工作都完成時，作業仍會保持作用中狀態。 您可以變更此行為，讓作業在其中的所有工作完成時自動終止。 將作業的 **onAllTasksComplete** 屬性 (在 Batch .NET 中為 [OnAllTasksComplete][net_onalltaskscomplete]) 設定為 *terminatejob*，以在作業的所有工作處於已完成狀態時，自動終止該作業。
    ...
    Batch 服務考慮*沒有*任務完成其所有任務的作業。 因此，這個選項最常搭配 [作業管理員工作](#job-manager-task)使用。 如果您想要使用自動作業終止，而不透過作業管理員，您一開始就應該將新作業的 **onAllTasksComplete** 屬性設定為 noaction**，而只在您完成將工作新增至作業之後，將它設定為 terminatejob**。
...
* 工作的 **命令列**。 這是可在計算節點上執行應用程式或指令碼的命令列。
    ...
    請務必注意，命令列不會在 shell 下運行。 因此，它無法以原生方式利用 Shell 功能，例如[環境變數](#environment-settings-for-tasks)擴充功能 (這包括 `PATH`)。 若要利用這類功能，您必須在命令列中叫用此 Shell，例如藉由在 Windows 節點上啟動 `cmd.exe` 或在 Linux 上啟動 `/bin/sh`︰
...
* **Application packages** 。 [應用程式包](#application-packages)提供任務運行的應用程式的簡化部署和版本控制。 在共用集區的環境中，工作層級的應用程式封裝特別有用：不同的作業會在一個集區上執行，而某項作業完成時並不會刪除該集區。 如果您的作業擁有的工作少於集區中的節點，工作應用程式套件可以減少資料傳輸，因為您的應用程式只會部署至執行工作的節點。
...
[工作相依性](batch-task-dependencies.md)正如其名，可讓您在執行某個工作之前，指定該工作相依於其他工作。 此功能提供下列情況的支援：「下游」工作取用「上游」工作的輸出，或當上游工作執行下游工作所需的某種初始化時。 若要使用這項功能，您必須先在 Batch 作業上啟用工作相依性。 然後，對於依賴于其他任務（或許多其他任務）的每個任務，您可以指定任務所依賴的任務。

利用工作相依性，您可以設定如下所示的案例︰

* taskB** 相依於 taskA** (直到 taskA** 完成，才會開始執行 taskB**)。
* *任務 C*取決於*任務 A*和*任務 B*。
...
* **應用程式**：包含有關安裝在計算節點上的應用程式包的詳細資訊。 工作可藉由參考 `AZ_BATCH_APP_PACKAGE` 環境變數來存取這個目錄。

* **fsmounts**：目錄包含安裝在計算節點上的任何檔案系統。 工作可藉由參考 `AZ_BATCH_NODE_MOUNTS_DIR` 環境變數來存取這個目錄。 有關詳細資訊，請參閱[在批次處理池上裝載虛擬檔案系統](virtual-file-mount.md)。
...
* **易失性**：此目錄用於內部目的。 不能保證此目錄中的任何檔或目錄本身將來會存在。

* **工作項**：此目錄包含作業的目錄及其在計算節點上的任務。

* **任務**：在**工作項**目錄中，為在節點上運行的每個任務創建一個目錄。 它通過引用`AZ_BATCH_TASK_DIR`環境變數進行訪問。
...
例如，作業可能要求您提交大量要執行的任務。 您可以指派調整公式給集區，以根據目前已排入佇列的工作數目和作業中工作的完成率來調整集區中的節點數目。 Batch 服務會定期評估公式，並根據工作負載和其他公式設定來調整集區大小。 服務會在有大量排入佇列的工作時視需要新增節點，並在沒有排入佇列或正在執行時移除節點。
...
間歇性問題還可能導致任務停止回應或執行時間過長。 您可以設定工作的執行間隔上限。 如果超過最大執行間隔，Batch 服務會中斷工作應用程式。
...
* **重新啟動節點** ([REST][rest_reboot] | [.NET][net_reboot])
    ...
    重新啟動節點有時可以清除潛在的問題，例如停滯或損毀的程序。 如果池使用啟動任務或作業使用作業準備任務，則在節點重新開機時執行這些任務。
...
* **停用節點上的工作排程** ([REST][rest_offline] | [.NET][net_offline])
    ...
    這實際上會讓節點離線，以便不再指派任何工作給它，但允許該節點繼續執行並留在集區中。 這可讓您執行進一步的調查以了解失敗原因，卻又不會遺失失敗工作的資料，而且不會讓節點造成額外的工作失敗。 例如，您可以停用節點上的工作排程，然後 [從遠端登入](#connecting-to-compute-nodes) 以檢查節點的事件記錄，或執行其他疑難排解動作。 完成調查後，可以通過啟用任務調度[（REST][rest_online] | [.NET）][net_online]使節點重新連線，或者執行前面討論的其他操作之一。
