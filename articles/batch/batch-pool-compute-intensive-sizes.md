---
title: 將計算密集型 Azure VM 與批次處理一起使用
description: 如何利用 Azure 批次處理池中的 HPC 和 GPU 虛擬機器大小。 瞭解作業系統依賴項，並查看幾個方案示例。
documentationcenter: ''
author: LauraBrenner
manager: evansma
editor: ''
...
只有特定作業系統會支援 Batch 中需要大量計算之大小的 RDMA 或 GPU 功能 （支援的作業系統清單是這些大小創建的虛擬機器支援的作業系統的子集。根據創建 Batch 池的方式，您可能需要在節點上安裝或配置其他驅動程式或其他軟體。 下表摘要說明這些相依性。 如需詳細資訊，請參閱連結的文章。 若要了解用來設定 Batch 集區的選項，請參閱本文稍後的內容。
...
<sup>*</sup>支援 RDMA 的 N 系列尺寸還包括 NVIDIA 特斯拉 GPU
...
## <a name="example-nvidia-gpu-drivers-on-windows-nc-vm-pool"></a>示例：Windows NC VM 池上的 NVIDIA GPU 驅動程式
...
2. 建立該套件的 zip 檔案。
3. 將套件上傳至您的 Batch 帳戶。 如需相關步驟，請參閱[應用程式套件](batch-application-packages.md)指引。 指定應用程式 ID（如*GPUDriver*）和版本（如*411.82）。*
...
## <a name="example-nvidia-gpu-drivers-on-a-linux-nc-vm-pool"></a>示例：Linux NC VM 池上的 NVIDIA GPU 驅動程式
...
3. 按照以下步驟為批次處理創建[共用圖像庫圖像](batch-sig-images.md)。
4. 在支援 NC VM 的區域建立 Batch 帳戶。
5. 使用批次處理 API 或 Azure 門戶，[使用自訂映射](batch-sig-images.md)並使用所需的節點數和縮放創建池。 下表顯示該映像的集區設定範例：
...
1. 按照以下步驟為批次處理創建[共用圖像庫圖像](batch-sig-images.md)。
1. 使用批次處理 API 或 Azure 門戶，[使用共用映射庫](batch-sig-images.md)創建池，並使用所需的節點數和縮放數創建池。 下表顯示該映像的集區設定範例：
...
## <a name="example-intel-mpi-on-a-linux-h16r-vm-pool"></a>示例：Linux H16r VM 池上的英特爾 MPI
