...
| AZ_BATCH_APP_PACKAGE            | 所有應用程式套件環境變數的前置詞。 例如，如果將應用程式 "FOO" 版本 "1" 安裝到集區，則環境變數會 AZ_BATCH_APP_PACKAGE_FOO_1 （在 Linux 上）或 AZ_BATCH_APP_PACKAGE_FOO # 1 （在 Windows 上）。 AZ_BATCH_APP_PACKAGE_FOO_1 指向下載封裝的位置（資料夾）。 使用預設版本的應用程式套件時，請使用不含版本號碼的 AZ_BATCH_APP_PACKAGE 環境變數。 如果在 Linux 中，而應用程式封裝名稱是 "Agent-Linux-x64"，而版本是 "1.1.46.0，則環境名稱實際上是： AZ_BATCH_APP_PACKAGE_agent_linux_x64_1_1_46_0，使用底線和小寫。 詳細資訊請看[這裡](https://docs.microsoft.com/azure/batch/batch-application-packages#execute-the-installed-applications)。 | 與應用程式套件相關聯的任何工作。 如果節點本身有應用程式套件，也適用於所有的工作。 | AZ_BATCH_APP_PACKAGE_FOO_1 （Linux）或 AZ_BATCH_APP_PACKAGE_FOO # 1 （Windows） |
...
| AZ_BATCH_TASK_SHARED_DIR | [多重執行個體工作][multi_instance]的主要工作與每個子工作相同的目錄路徑。 此路徑存在於多重執行個體工作執行所在的每個節點上，且可由在該節點上執行的工作命令 ([協調命令][coord_cmd]與[應用程式命令][app_cmd]) 以讀取/寫入的方式存取。 子工作或在其他節點上執行的主要工作，無法遠端存取此目錄（它不是「共用的」網路目錄）。 | 多重執行個體的主要工作和子工作。 | C:\user\tasks\workitems\multiinstancesamplejob\job-1\multiinstancesampletask |
