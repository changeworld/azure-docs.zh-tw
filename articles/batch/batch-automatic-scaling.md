...
ms.locfileid: "87387467"
---
# <a name="create-an-automatic-formula-for-scaling-compute-nodes-in-a-batch-pool"></a>建立自動公式以調整 Batch 集區中的計算節點

Azure Batch 可以根據您定義的參數自動調整集區，以節省時間和金錢。 使用自動調整功能時，Batch 會在工作需求增加時，以動態方式將節點新增至集區，並隨著工作需求減少而移除計算節點。

若要在計算節點的集區上啟用自動調整，請將集區與您定義的*自動調整公式*產生關聯。 Batch 服務會使用自動調整公式來判斷執行您的工作負載所需的節點數目。 這些節點可以是專用節點或[低優先順序節點](batch-low-pri-vms.md)。 Batch 接著會定期審查服務計量資料，並根據您的公式和您定義的間隔，使用它來調整集區中的節點數目。

您可以在建立集區時啟用自動調整，或將它套用至現有的集區。 Batch 可讓您在將公式指派給集區之前評估公式，以及監視自動調整回合的狀態。 設定具有自動調整的集區之後，您可以稍後再變更公式。

> [!IMPORTANT]
> 當您建立 Batch 帳戶時，您可以指定[集區配置模式](accounts.md)，以決定是否要在 Batch 服務訂用帳戶（預設值）或您的使用者訂用帳戶中組態集區。 如果您使用預設的 Batch 服務設定建立 Batch 帳戶，則您的帳戶會限制為可用於處理的核心數目上限。 Batch 服務只會將計算節點調整為最多達到該核心限制。 基於這個理由，Batch 服務不會達到自動調整公式所指定的目標計算節點數目。 如需檢視和增加帳戶配額的相關資訊，請參閱 [Azure Batch 服務的配額和限制](batch-quota-limit.md) 。
>
>如果您已建立具有使用者訂用帳戶模式的帳戶，則您的帳戶會在訂用帳戶的核心配額中共用。 如需詳細資訊，請參閱 [Azure 訂用帳戶和服務限制、配額與條件約束](../azure-resource-manager/management/azure-subscription-service-limits.md)中的 [虛擬機器限制](../azure-resource-manager/management/azure-subscription-service-limits.md#virtual-machines-limits)。

## <a name="autoscale-formulas"></a>自動調整公式

自動調整公式是您定義的字串值，其中包含一或多個語句。 自動調整公式會指派給集區的 [autoScaleFormula](/rest/api/batchservice/enable-automatic-scaling-on-a-pool) 元素 (Batch REST) 或 [CloudPool.AutoScaleFormula](/dotnet/api/microsoft.azure.batch.cloudpool.autoscaleformula) 屬性 (Batch .NET)。 Batch 集區會使用您的公式來決定集區中可供下一個間隔處理的目標計算節點數目。 公式字串不能超過 8 KB，可以包含最多100個以分號分隔的語句，而且可以包含分行符號和批註。

您可以將自動調整公式視為 Batch 自動調整「語言」。 公式語句是自由格式的運算式，可以包含服務定義的變數（由 Batch 服務所定義）和使用者定義的變數。 公式可以使用內建類型、運算子和函式，對這些值執行各種作業。 例如，陳述式可能會採用下列格式：
...
在您的自動調整公式中包含這些陳述式，以達成目標計算節點數目。 專用節點和低優先順序節點各自有自己的目標設定。 自動調整公式可以包含專用節點目標值、低優先順序節點目標值，或同時包含兩者。
...
使用此自動調整公式，一開始會建立包含單一 VM 的集區。 `$PendingTasks` 計量會定義執行中或已排入佇列的工作數目。 公式會尋找過去 180 秒內的平均擱置中工作數目，並據以設定 `$TargetDedicatedNodes` 變數。 公式會確保目標專用節點數目絕不會超出 25 部 VM。 集區會隨著新工作的提交而自動成長。 當工作完成時，Vm 就會變成可用，而自動調整公式會縮小集區。
...
您將會在本主題稍後深入瞭解[如何建立自動調整公式](#write-an-autoscale-formula)，以及查看其他[範例自動調整公式](#example-autoscale-formulas)。
...
使用者定義的變數是您定義的變數。 在如上所示的範例公式中， `$TargetDedicatedNodes` 和 `$PendingTasks` 是服務定義的變數，而 `startingNumberOfVMs` 和 `maxNumberofVMs` 是使用者定義的變數。
...
| 變數 | 描述 |
| --- | --- |
| $TargetDedicatedNodes |集區之專用計算節點的目標數目。 這會指定為目標，因為集區不一定會達到所需的節點數目。 例如，如果在集區達到初始目標之前，自動調整評估修改了專用節點的目標數目，則集區可能無法到達目標。 <br /><br /> 如果目標超過 Batch 帳戶節點或核心配額，在 Batch 服務模式中建立的帳戶集區可能無法達到其目標。 如果目標超出訂用帳戶的共用核心配額，在使用者訂用帳戶模式中建立的帳戶集區可能無法達到其目標。|
| $TargetLowPriorityNodes |集區之低優先順序計算節點的目標數目。 這個指定為目標，因為集區不一定會達到所需的節點數目。 例如，如果在集區達到初始目標之前，自動調整評估修改了低優先順序節點的目標數目，則集區可能無法到達目標。 如果目標超過 Batch 帳戶節點或核心配額，則集區也可能未達到其目標。 <br /><br /> 如需低優先順序計算節點的詳細資訊，請參閱[使用低優先順序 VM 搭配 Batch](batch-low-pri-vms.md)。 |
| $NodeDeallocationOption |計算節點從集區移除時所發生的動作。 可能的值包括：<ul><li>**requeue**：預設值。 立即結束工作，並將它們放回作業佇列，以便重新排定它們。 此動作可確保儘快達到目標節點數目。 不過，這可能比較不有效率，因為任何執行中的工作都會中斷，然後必須完全重新開機。 <li>**終止**：立即結束工作，並將它們從作業佇列中移除。<li>**taskcompletion**：等候目前正在執行的工作完成，然後從集區中移除該節點。 使用此選項可避免工作中斷和重新排入佇列，並浪費工作已完成的任何工作。<li>**retaineddata**：等待節點上所有本機工作保留的資料在從集區移除節點之前清除。</ul> |
...
您可以取得這些服務定義變數的值，以根據 Batch 服務的計量進行調整。

> [!IMPORTANT]
> 作業釋放工作目前不會包含在提供工作計數的變數中，例如 $ActiveTasks 和 $PendingTasks。 視您的自動調整公式而定，這可能會導致移除節點，而不使用任何節點來執行作業釋放工作。
...
| $ActiveTasks |準備好執行但還未執行的工作數目。 這包括處於作用中狀態且已滿足其相依性的所有工作。 處於作用中狀態但未滿足其相依性的任何工作會從 $ActiveTasks 計數排除。 若為多重執行個體工作，$ActiveTasks 將包含工作上設定的執行個體數目。|
...
> [!TIP]
> 這些唯讀服務定義的變數是提供各種方法來存取與每個相關聯之資料的*物件*。 如需詳細資訊，請參閱本文稍後的[取得範例資料](#obtain-sample-data)。
...
您可以在定義公式時使用資源和工作計量。 您會根據您取得和評估的度量資料來調整集區中專用節點的目標數目。 如需每個度量的詳細資訊，請參閱上方的[變數](#variables)一節。
...
自動調整公式的核心作業是取得工作和資源度量資料（範例），然後根據該資料來調整集區大小。 因此，請務必清楚瞭解自動調整公式與範例的互動方式。

### <a name="methods"></a>方法

自動調整公式會針對 Batch 服務所提供的計量資料範例採取動作。 公式會根據它所取得的值來增加或縮減集區大小。 服務定義的變數是提供方法來存取與該物件相關聯之資料的物件。 例如，下列運算式顯示取得最後五分鐘的 CPU 使用量的要求：
...
| 方法 | 描述 |
| --- | --- |
| GetSample() |`GetSample()` 方法會傳回資料樣本的向量。<br/><br/>一個樣本有 30 秒的度量資料。 換句話說，每隔 30 秒會取得範例。 如下所述，從收集樣本到可用於公式之間會延遲。 因此，並非一段指定時間內的所有樣本都可供公式評估。<ul><li>`doubleVec GetSample(double count)`：指定要從收集到的最新樣本取得的樣本數。 `GetSample(1)` 會傳回最後一個可用的樣本。 不過，對於之類的計量，則不應該 `$CPUPercent` `GetSample(1)` 使用，因為無法得知收集樣本的*時間*。 這可能是最新的，或因為系統問題，可能會更久。 在這種情況下，最好使用時間間隔，如下所示。<li>`doubleVec GetSample((timestamp or timeinterval) startTime [, double samplePercent])`：指定用來收集範例資料的時間範圍。 它也會選擇性地指定在要求的時間範圍內必須可用的樣本數百分比。 例如， `$CPUPercent.GetSample(TimeInterval_Minute * 10)` 如果過去10分鐘的所有樣本都出現在歷程記錄中，則會傳回20個樣本 `CPUPercent` 。 如果歷程記錄的最後一分鐘無法使用，則只會傳回18個樣本。 在此情況下 `$CPUPercent.GetSample(TimeInterval_Minute * 10, 95)` 會失敗，因為只有90% 的樣本可供使用，但 `$CPUPercent.GetSample(TimeInterval_Minute * 10, 80)` 會成功。<li>`doubleVec GetSample((timestamp or timeinterval) startTime, (timestamp or timeinterval) endTime [, double samplePercent])`：指定用來收集資料的時間範圍，同時包含開始時間和結束時間。 如上所述，收集樣本和可供公式使用的時間之間會有延遲。 當您使用 `GetSample` 方法時，請考慮此延遲。 請參閱下文中的 `GetSamplePercent`。 |
...
Batch 服務會定期取得工作和資源計量的樣本，使其可供自動調整公式使用。 Batch 服務會每隔 30 秒記錄一次這些樣本。 不過，記錄樣本的時間與樣本可供自動調整公式使用 (與讀取) 的時間之間有所延遲。 此外，可能因為網路或其他基礎結構問題等因素，而無法針對特定間隔記錄範例。
...
讓我們以 10 分鐘的時間範圍為例。 由於範例會在該10分鐘的時間範圍內每30秒記錄一次，因此 Batch 記錄的樣本總數上限為20個樣本（每分鐘2個）。 不過，由於回報機制固有的延遲，以及 Azure 中的其他問題，可能只有 15 個樣本可供自動調整公式讀取。 因此，舉例來說，在這 10 分鐘的期間內，記錄的樣本總數只有 75% 可供您的公式使用。

### <a name="getsample-and-sample-ranges"></a>GetSample() 和樣本範圍

您的自動調整公式會藉由新增或移除節點來增加和縮減您的集區。 因為節點會產生費用，請確定您的公式使用以足夠的資料為基礎的智慧型分析方法。 我們建議您在公式中使用趨勢型別分析。 此類型會根據所收集樣本的範圍來擴大和縮減集區。
...
因為範例可用性可能會延遲，所以您應該一律指定一個時間範圍，並讓它的回顧開始時間早于一分鐘。 樣本需要花大約一分鐘的時間才能傳播到整個系統，所以通常無法使用 `(0 * TimeInterval_Second, 60 * TimeInterval_Second)` 範圍中的樣本。 同樣地，您可以使用 `GetSample()` 的百分比參數來強制特定樣本百分比需求。
...
您建置自動調整公式的方式是使用上述元件撰寫陳述式，再將這些陳述式結合成完整的公式。 在本節中，我們會建立範例自動調整公式，以執行真實世界的調整決策並進行調整。
...
- 一律以 400 為專用節點的數目上限。
- 減少節點數目時，請勿移除正在執行工作的節點;如有必要，請等到工作完成後再移除節點。

公式中的第一個語句會在高 CPU 使用量期間增加節點的數目。 我們將定義一個語句，其 `$totalDedicatedNodes` 會以目前目標數目的專用節點之110% 的值填入使用者定義的變數（），但僅限於過去10分鐘內的最小平均 CPU 使用率高於70% 時。 否則，它會使用目前專用節點數目的值。
...
若要在低 CPU 使用量期間減少專用節點數目，公式中的下一個語句會將相同的 `$totalDedicatedNodes` 變數設定為目前目標數目的專用節點的90% （如果過去60分鐘內的平均 CPU 使用量低於20%）。 否則，它會使用 `$totalDedicatedNodes` 我們在上述語句中填入的目前值。
...
下列範例會在 .NET 中建立已啟用自動調整的集區。 集區的自動調整公式會將星期一的專用節點目標數目設定為5，在一周中的每隔一天，將設為1。 [自動調整間隔](#automatic-scaling-interval) 設定為 30 分鐘。 在此程式碼片段及本文的其他 C# 程式碼片段中，`myBatchClient` 是 [BatchClient](/dotnet/api/microsoft.azure.batch.batchclient) 類別適當初始化的執行個體。
...
> [!IMPORTANT]
> 當您建立已啟用自動調整的集區時，請勿在對**batchclient.pooloperations.createpool**的呼叫上指定_TargetDedicatedNodes_參數或_targetLowPriorityNodes_參數。 請改為在集區上指定 **AutoScaleEnabled** 和 **AutoScaleFormula** 屬性。 這些屬性的值會判斷每個節點類型的目標數目。
>
> 若要手動調整已啟用自動調整集區的大小（例如，使用[BatchClient. PoolOperations. ResizePoolAsync](/dotnet/api/microsoft.azure.batch.pooloperations.resizepoolasync)），您必須先停用集區上的自動調整，然後調整其大小。

### <a name="python"></a>Python

若要使用 Python SDK 建立已啟用自動調整的集區：
...
- 如果集區上目前已停用自動調整，您必須在發出要求時指定有效的自動調整公式。 您可以選擇性地指定自動調整間隔。 如果您未指定間隔，則會使用預設值15分鐘。
- 如果目前已在集區上啟用自動調整，您可以指定新的公式、新的間隔或兩者。 您必須至少指定其中一個屬性。
  - 如果您指定新的自動調整間隔，就會停止現有的排程，並啟動新的排程。 新排程的開始時間是發出啟用自動調整要求的時間。
  - 如果您省略自動調整公式或間隔，Batch 服務就會繼續使用該設定的目前值。

> [!NOTE]
> 如果您在 .NET 中建立集區時，針對**batchclient.pooloperations.createpool**方法的*targetDedicatedNodes*或*targetLowPriorityNodes*參數指定值，或針對另一種語言的可比較參數，則在評估自動調整公式時，就會忽略這些值。

此 c # 範例使用[Batch .net](/dotnet/api/microsoft.azure.batch)程式庫，在現有的集區上啟用自動調整。
...
您可以先評估公式，再將它套用至集區。 這可讓您先測試公式的結果，再將它放入生產環境中。

您必須先使用有效的公式（例如單行公式）在集區上啟用自動調整，才可以評估自動調整公式 `$TargetDedicatedNodes = 0` 。 然後，使用下列其中之一來評估您想要測試的公式︰
...
若要確保您的公式如預期般執行，我們建議您定期檢查 Batch 對您的集區執行之自動調整執行的結果。 若要這麼做，請取得（或重新整理）集區的參考，然後檢查其上次自動調整執行的屬性。
...
下列 c # 範例會使用 Batch .NET 程式庫來列印有關集區_myPool_上最後一個自動調整執行的資訊。
...
在此 c # 範例中，會根據佇列中的工作數目來調整集區大小。 我們已在公式字串中包含批註和分行符號。
...
這個 c # 範例會根據工作數目來調整集區大小。 此公式也會考慮集區已設定的 [MaxTasksPerComputeNode](/dotnet/api/microsoft.azure.batch.cloudpool.maxtaskspercomputenode) 值。 已在集區上啟用[平行工作執行](batch-parallel-node-tasks.md)的情況下，這個方法很有用。
...
這個範例示範具有自動調整公式的 c # 範例，其會將集區大小設定為初始時間週期內的指定節點數目。 之後，它會根據執行中和作用中的工作數目來調整集區大小。
...
- 瞭解如何在[集區中的計算節點上同時執行多個工作](batch-parallel-node-tasks.md)。 隨著自動調整，這有助於降低某些工作負載的工作持續時間，節省您的成本。
