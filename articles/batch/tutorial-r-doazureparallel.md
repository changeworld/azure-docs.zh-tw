---
title: 教學課程 - 使用 Azure Batch 執行平行 R 模擬
...
* 將 `taskSlotsPerNode` 增加至 2，以利用每個節點上的兩個核心
...
輸出會顯示 doAzureParallel 的「執行背景工作角色」數目。 這個數字是節點數目乘以 `taskSlotsPerNode` 的值。 如果您已如先前所述修改叢集設定，則此數字為 10。
