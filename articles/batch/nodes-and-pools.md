...
當您指定要完成的工作時，可以手動建立集區，或 [由 Batch 服務自動](#autopools) 建立集區。 當您建立集區時，您可以指定下列屬性：
...
> [!IMPORTANT]
> 集區應使用「虛擬機器設定」進行設定，而不是「雲端服務設定」。 「虛擬機器設定」集區支援所有批次功能，並新增新功能。 「雲端服務設定」集區不支援所有功能，而且不會規劃任何新功能。
...
Azure 多餘的容量不足時，可能會佔用低優先順序的節點。 如果節點在執行工作時遭到佔用，工作會重新排入佇列並於計算節點再次可用時再次執行。 低優先順序的節點很適合用於作業完成時間有彈性且工作分散於許多節點的工作負載。 在決定使用適合您案例的低優先順序節點之前，請確定會盡可能降低因優先佔用而遺失的工作數，且可輕鬆重新建立。
...
[Autopool](/rest/api/batchservice/job/add#autopoolspecification)是在提交作業時，Batch 服務所建立的集區，而不是在將在集區中執行的作業之前建立的集區。 Batch 服務會根據您指定的特性來管理 autopool 的存留期。 最常見的情況是，這些集區也會在其工作完成後自動設定為 [刪除]。
