...
1. 選取左側邊列上的 [集區]  ，然後選取搜尋表單上方的 [新增]  按鈕，以建立集區。 
    ...
    1. 啟用啟動工作，並新增命令 `cmd /c "pip install pandas"`。 使用者身分識別可保留為預設 [集區使用者]  。
    1. 選取 [確定]  。
...
1. 遵循[管理 Blob 容器中的 Blob](../vs-azure-tools-storage-explorer-blobs.md#managing-blobs-in-a-blob-container) 中的步驟，使用儲存體總管將 `main.py` 和 `iris.csv` 上傳至輸入容器 `input`。
...
將指令碼儲存為 `main.py`，並將其上傳至 [Azure 儲存體]  容器。 請務必先在本機測試並驗證其功能，再將其上傳至您的 Blob 容器：
