...
## <a name="create-a-pool-from-a-shared-image-using-c"></a>使用 C 從共用映射建立集區#

或者，您可以使用 c # SDK，從共用映射建立集區。
...
* **調整超時大小。** 如果您的集區包含固定數目的節點（如果不自動調整），請`resizeTimeout`根據集區大小來增加集區的屬性。 針對每個 1000 Vm，建議的調整大小時間至少為15分鐘。 例如，使用 2000 Vm 的集區建議大小設定時間至少為30分鐘。
