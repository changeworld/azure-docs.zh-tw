...
|`imageType`|字串|映射的部署方法。 支援的值`virtualMachineConfiguration`為或`cloudServiceConfiguration`|
...
|`targetOSVersion`|字串|安裝在集區中虛擬機器上的 Azure 客體 OS 版本。<br /><br /> 預設值為**\*** ，指定指定系列的最新作業系統版本。<br /><br /> 如需其他允許的值，請參閱[客體 OS 發佈新聞](https://azure.microsoft.com/documentation/articles/cloud-services-guestos-update-matrix/#releases)。|
...
|`nodeAgentId`|字串|佈建在計算節點上的 Batch 節點代理程式其 SKU。|
...
|元素名稱|類型|注意|
|------------------|----------|-----------|
|`enableAutomaticUpdates`|布林值|指出是否已針對自動更新啟用虛擬機器。 如果未指定此屬性，則預設值為 true。|
