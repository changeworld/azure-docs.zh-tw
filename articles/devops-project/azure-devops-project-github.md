---
title: 教學課程：使用 Azure DevOps 入門版為現有的程式碼建立 CI/CD 管線
description: Azure DevOps 入門版可供輕鬆地開始使用 Azure。 DevOps Projects 可透過幾個簡單的步驟，協助您使用自己的程式碼和 GitHub 存放庫，在 Azure 服務上啟動應用程式。
services: vsts
...
Azure DevOps 入門版提供簡化的程序，以便在 Azure 中建立持續整合 (CI) 和持續傳遞 (CD) 管線。 您可以自備現有的程式碼和 Git 存放庫，也可以選取範例應用程式。
...
Azure DevOps 入門版會在 Azure Pipelines 中建立 CI/CD 管線。 您可以建立新的 Azure DevOps 組織或使用現有組織。 Azure DevOps 入門版也會在您選擇的 Azure 訂用帳戶中建立 Azure 資源。
...
1. 在 [Azure 訂用帳戶] 中選取您的訂用帳戶，然後在 [Web 應用程式] 中輸入名稱，或使用預設值。 選取 [位置]，然後選取 [完成]。 在幾分鐘後，Azure 入口網站中便會顯示 DevOps 入門版部署概觀。

1. 選取 [移至資源] 以檢視 DevOps 入門版儀表板。 在右上角將 [專案] 釘選至儀表板以供快速存取。 Azure DevOps 入門版會自動設定 CI 建置和發行觸發程序。 您的程式碼會保留在您的 GitHub 存放庫或其他外部存放庫中，並在 [Azure DevOps 組織] 的存放庫中設定範例應用程式。 Azure DevOps 入門版會執行建置並將應用程式部署至 Azure。
...
1. 從您的 DevOps 入門版儀表板中選取 [存放庫]。 您的 GitHub 存放庫會在新的瀏覽器索引標籤中開啟。變更您的應用程式，然後選取 [認可變更]。

1. 不久後，Azure Pipelines 中就會起始一個建置。 您可以在 DevOps 入門版儀表板中監視建置狀態。 您也可從 DevOps 入門版儀表板中選取 [建置管線]，以在 Azure DevOps 組織中監視建置狀態。

## <a name="examine-the-azure-pipelines-cicd-pipeline"></a>檢查 Azure Pipelines 的 CI/CD 管線

Azure DevOps 入門版會在 Azure Pipelines 中自動設定 CI/CD 管線。 瀏覽管線，並視需要進行自訂。 若要讓自己熟悉建置和發行管線，請執行下列作業：

1. 在 DevOps 入門版儀表板中選取 [建置管線]。
