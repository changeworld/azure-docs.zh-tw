...
1. 選取 [方案**名稱**]、[**計價模式**] 或 [**可用性**]。 *目前，方案僅適用于美國) 的英文 (*。
1. 對方案名稱、描述或物件可用性進行任何變更之後，請選取 [**儲存草稿**]。
1. 當您準備好要發佈更新時，請選取 [**審查併發布**]。 [**[審查與發佈](#review-and-publish-an-updated-offer)**] 頁面隨即開啟，並提供更新的完成狀態。
1. 選取 [**發行**] 以提交已更新的方案來進行發佈。 當更新供應專案的預覽版本可供您審查和登出時，我們會傳送電子郵件給您。

## <a name="offer-a-virtual-machine-plan-at-a-new-price"></a>以新價格提供虛擬機器方案

虛擬機器方案發佈之後，就無法變更其價格。 若要以不同的價格提供相同的方案，您必須隱藏方案，並使用更新的價格建立新的方案。 首先，使用您想要變更的價格來隱藏方案：

1. 在您現有供應專案的 [**供應專案總覽**] 頁面開啟的情況下，選擇您要變更的方案。 如果計畫無法從**計畫總覽**清單存取，請選取 [**查看所有方案**]。
1. 選取 [**隱藏計畫**] 核取方塊。 在繼續之前，請先儲存草稿。

現在您已將方案隱藏成舊價格，請使用更新的價格來建立該方案的複本：

1. 在 [合作夥伴中心] 中，返回 **[計畫總覽**]。
2. 選取 [建立新方案]。 輸入**方案識別碼**和**方案名稱**，然後選取 [**建立**]。
1. 若要從您已隱藏的方案重複使用技術設定，請選取 [**重複使用技術**設定] 核取方塊。 若要深入瞭解，請參閱[計畫總覽](azure-vm-create-offer.md#plan-overview)。
    > [!IMPORTANT]
    > 如果您選取**此方案，會重複使用另一個方案的技術**設定，則稍後將無法停止銷售父計畫。 如果您想要停止銷售父計畫，請勿使用此選項。
...
您可以從供應專案的 **[總覽**] 頁面選取 [**刪除草稿**]，將草稿供應專案從尚未發行的 (中刪除) 。 如果您先前已發佈供應專案，將無法使用此選項。
...
若要移除現有客戶提供的供應專案，請[記錄支援要求](https://aka.ms/marketplacepublishersupport)。 在 [支援主題] 清單中，選取 [**商業 Marketplace**供應專案]  >  **或 [應用程式 Delisting** ]、[移除] 或 [終止] 並提交要求。 支援小組會引導您完成供應專案移除程式。
...
- 使用 [+**新增識別碼**] 或 [將**客戶匯入 (csv) ** ] 按鈕修改一或多個私人方案中的物件，然後儲存變更。
