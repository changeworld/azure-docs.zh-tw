...
|狀況|錯誤|原因|解決方法|
|---|---|---|---|
|1| (VHD) 的虛擬硬碟無效|如果 VHD 頁尾中的指定 cookie 值不正確，VHD 將會被視為無效。|重新建立映射並提交要求。|
...
請勿使用安裝的 SQL server 建立自己的 Windows 映像。 相反地，請使用已核准的 SQL 基底映射，從 Azure Marketplace (企業/標準/web) 。
...
|2|Bash 歷程記錄測試案例|如果提交影像中的 bash 歷程記錄大小超過 1 kb (KB) ，您將會看到錯誤。 大小限制為 1 KB，以確保您的 bash 歷程記錄檔案中不會捕捉到任何可能的機密資訊。|若要解決此問題，請將 VHD 掛接到任何其他工作 VM，並進行任何您想要的變更 (例如，刪除 *bash* 歷程記錄檔案) ，將大小縮減為小於或等於 1 KB。|
...
|15|SNMP 服務|尚未支援簡單的網路管理通訊協定 (SNMP) 服務功能。 應用程式不應該相依于這項功能。|
...
如果以資料磁片提交的任何要求大小大於 1023 gb (GB) ，則不會核准該要求。 此規則適用于 Linux 和 Windows。
...
|250-500 TiB|>200 gib (GiB) blob 大小差異|請洽詢支援小組以取得例外狀況核准。|
...
||RHEL-SAP 7。4|TBD|
||RHEL-SAP 7。5|TBD|
|SLES|包含 SAP) 的 SLES11SP4 (|3.0.101-108.95.2|
...
Azure 上的所有 Vhd 必須具有與 1 mb (MB) 的倍數相符的虛擬大小。 如果您的 VHD 不符合建議的虛擬大小，您的要求可能會遭到拒絕。
...
請參閱下表，以瞭解當您使用共用存取簽章 (SAS) URL 下載 VM 映射時，所發生的任何問題。
...
|7|不正確 VHD 名稱|檢查是否有任何特殊字元（例如百分比符號 (% ) 或引號 ( ") ）是否存在於 VHD 名稱中。|移除特殊字元，以重新命名 VHD 檔案。|
...
|錯誤|原因|解決方法|
|---|---|---|
|`DataDisk- InvalidUrl:`|此錯誤的發生原因可能是在提交供應專案時，為邏輯單元編號指定了不正確數位 (LUN) 。|確認資料磁片的 LUN 編號順序位於 [合作夥伴中心]。|
|`DataDisk- NotFound:`|發生此錯誤的原因可能是資料磁片不在指定的 SAS URL。|確認資料磁片位於要求中指定的 SAS URL。|

## <a name="remote-access-issue"></a>遠端存取問題

如果未啟用 Windows 映像的 [遠端桌面通訊協定 (RDP) ] 選項，您將會收到此錯誤。 

提交 Windows 映像之前，請先啟用 RDP 存取。

## <a name="bash-history-failed"></a>Bash 歷程記錄失敗

如果提交影像中的 bash 歷程記錄大小超過 1 kb (KB) ，您就會看到此錯誤。 大小限制為 1 KB，以確保您的 bash 歷程記錄檔案中不會捕捉到任何可能的機密資訊。

以下是刪除「Bash 歷程記錄」的步驟。

步驟 1： 部署 VM，然後按一下 Azure 入口網站上的 [執行命令] 選項。
![在 Azure 入口網站上執行命令](./media/vm-certification-issues-solutions-3.png)

步驟 2： 選取第一個選項 "RunShellScript"，然後執行下列命令。

命令： "cat/dev/null > ~/. bash_history && 歷程記錄-c" ![ Azure 入口網站上的 bash 歷程記錄命令](./media/vm-certification-issues-solutions-4.png)

步驟 3： 成功執行命令之後，請重新開機 VM。

步驟 4： 將 VM 一般化，接受映射 VHD 並停止 VM。

步驟 5。     重新提交一般化映射。

## <a name="requesting-exceptions-custom-templates-on-vm-images-for-selective-tests"></a>要求例外狀況 (自訂範本) 適用于選擇性測試的 VM 映射

發行者可以向外要求在 VM 認證期間所執行的幾項測試。 在極罕見的情況下，當發行者提供辨識項來支援要求時，會提供例外狀況。
...
* **一或多個測試案例的例外狀況：** 發行者可以連接到 [Marketplace 發行者支援](https://aka.ms/marketplacepublishersupport) 要求例外狀況（適用于測試案例）。 

* 已**鎖定 vm/沒有根存取權：** 少數的「發行者」有一些案例，即 vm 需要鎖定，因為 VM 上已安裝防火牆之類的軟體。 
       在此情況下，發行者可以在這裡下載[認證的測試控管](https://aka.ms/AzureCertificationTestTool)，並在[Marketplace 發行者支援](https://aka.ms/marketplacepublishersupport)提供報告


* **自訂範本：** 有些發行者會發佈需要自訂 ARM 範本來部署 Vm 的 VM 映射。 在此情況下，會要求發行者提供 [Marketplace 發行者支援](https://aka.ms/marketplacepublishersupport) 的自訂範本，讓認證小組可以使用相同的驗證。 

### <a name="information-to-provide-for-exception-scenarios"></a>提供給例外狀況案例的資訊

發行者必須與 [Marketplace 發行者支援](https://aka.ms/marketplacepublishersupport) 人員聯繫，以使用其他下列資訊要求上述案例的例外狀況：

   1.   發行者識別碼–合作夥伴中心入口網站上的發行者識別碼
   2.   供應專案識別碼/名稱–要求例外狀況的供應專案識別碼/名稱 
   3.   SKU/方案識別碼–要求其例外狀況之 VM 供應專案的方案識別碼/sku
   4.    版本–要求其例外狀況的 VM 供應專案版本
   5.   例外狀況類型-測試、鎖定 VM、自訂範本
   6.   要求的原因–此例外狀況的原因，以及要豁免之測試的相關資訊 
   7.   附件-附加任何重要性辨識項檔。 針對鎖定的 Vm，請附加測試報告，並針對自訂範本提供自訂 ARM 範本作為附件。 無法連結已鎖定 Vm 的報告，以及自訂範本的自訂 ARM 範本將導致拒絕要求
