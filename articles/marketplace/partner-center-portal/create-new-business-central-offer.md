---
title: 在 Microsoft 商業 marketplace 中建立新的 Dynamics 365 Business Central 供應專案
description: 如何使用 Microsoft 合作夥伴中心上的商用 Marketplace 入口網站，建立新的 Dynamics 365 Business Central 供應專案，以在 Azure Marketplace、Microsoft AppSource 或透過雲端解決方案提供者（CSP）方案中進行列出或銷售。
author: dsindona
ms.author: dsindona
...
- **供應專案 ID**：您帳戶中每個供應專案的唯一識別碼。 在 marketplace 供應專案的 URL 位址中，客戶會看到此識別碼，而 Azure Resource Manager 範本（如果適用）。 供應專案識別碼必須是小寫的英數位元（包括連字號和底線，但不能是空格）、限制為50個字元，而且在您選取 [**建立**] 之後就無法變更。  例如，如果您在這裡輸入 [*測試-供應專案-1* ]，則供應`https://azuremarketplace.microsoft.com/marketplace/../test-offer-1`專案 URL 會是。
...
[供應專案清單] 頁面可讓您定義供應專案的詳細資料（例如名稱、描述、影像等）。
...
在本節中，您可以提供向客戶顯示供應專案時所要使用的標誌和影像。 所有影像都必須是 .png 格式。 以兩種大小上傳您的供應專案標誌：**小型（48 x 48）** 和**大型（216 x 216）**。
...
如果您選取上述的 **[連線]** ，請在這裡提供您的應用程式安裝 URL。 針對不需要安裝的已聯機服務，請提供服務登陸頁面或註冊頁面的 URL。
...
在 [**試用**產品] 索引標籤下找到的**Marketplace 清單**選項，是您定義試用產品體驗詳細資料的位置。
...
您必須上傳`.pdf`檔案，其中列出您供應專案的金鑰使用案例，並列在檔中（.pdf 格式）。 在我們核准您的 marketplace 供應專案之前，我們的驗證小組可能會驗證此處所列的所有案例。
