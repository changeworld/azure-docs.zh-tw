...
ms.locfileid: "81251941"
---
# <a name="configure-lead-management-using-an-azure-table"></a>使用 Azure 表設定潛在客戶管理

如果合作夥伴中心中沒有明確支援客戶關係管理 (CRM) 系統以接收 Azure 應用商店和 AppSource 潛在顧客,則可以使用 Azure 表來處理這些潛在顧客。 然後,您可以選擇匯出數據並將其導入 CRM 系統。 本文中的說明將引導您完成創建 Azure 儲存帳戶以及該帳戶下的 Azure 表的過程。 此外,您可以使用 Microsoft Flow 創建新流,在產品/服務收到潛在顧客時發送電子郵件通知。

## <a name="configure-azure-table"></a>設定 Azure 表

1. 如果您沒有 Azure 帳戶，您可以[建立免費的試用帳戶](https://azure.microsoft.com/pricing/free-trial/)。
2. Azure 帳號處於活動狀態後,請登入 Azure[門戶](https://portal.azure.com)。
3. 在 Azure 門戶中,使用以下過程創建存儲帳戶。  
    1. 選擇 「在左側功能表欄中**創建資源**」。  "**新建**「窗格(邊欄)將向右顯示。
    2. 在 **「新建」** 窗格中選擇 **「儲存**」。  **右側將顯示"精選**"清單。
    3. 選擇**儲存帳戶**以開始創建帳戶。  按照文章"[創建存儲帳戶](https://docs.microsoft.com/azure/storage/common/storage-quickstart-create-account?tabs=azure-portal)"中的說明進行操作。

        ![建立 Azure 儲存體帳戶的步驟](./media/commercial-marketplace-lead-management-instructions-azure-table/azure-storage-create.png)

        有關儲存帳戶的詳細資訊,請選擇[快速入門教程](https://docs.microsoft.com/azure/storage/)。  如需儲存體定價的詳細資訊，請參閱[儲存體定價](https://azure.microsoft.com/pricing/details/storage/)。

4. 等待存儲帳戶預配,此過程通常需要幾分鐘時間。  然後,通過選擇 **「查看所有資源**」或從 Azure 門戶的左側導航功能表欄中選擇 **「所有資源**」,從 Azure 門戶的**主頁**存取存儲帳戶。

    ![存取 Azure 儲存帳戶](./media/commercial-marketplace-lead-management-instructions-azure-table/azure-storage-access.png)

5. 從儲存帳戶窗格中,選擇 **「存取金鑰」** 並複製金鑰的連接*字串*值。 儲存此值,因為這是存儲*帳戶連接字串*值,您需要在發佈門戶中提供該值才能接收市場產品的潛在顧客。 
...
6. 從儲存帳戶窗格中,選擇 **「表」** 並選擇 **「表**」以建立表。 輸入表的名稱,然後選擇 **"確定**"。 如果要設定 MS Flow 以在收到潛在顧客時接收電子郵件通知,請根據需要保存此值。

    ![Azure 資料表](./media/commercial-marketplace-lead-management-instructions-azure-table/azure-tables.png)

    您可以使用 [Azure 儲存體總管](https://archive.codeplex.com/?p=azurestorageexplorer) \(英文\) 或任何其他工具，來檢視儲存體資料表中的資料。 您還可以在 Azure 表中匯出數據。 

## <a name="optional-use-microsoft-flow-with-an-azure-table"></a>( 選擇性的 )將 Microsoft 串流與 Azure 表一起使用  
...
使用此範例作為指南，以建立簡單的流程，在新的潛在客戶新增至 Azure 資料表時，自動傳送電子郵件通知。 此示例設置一個重複,以便在更新表存儲時每小時發送一次潛在顧客資訊。
...
4. 在下拉清單中,**從空白**中選擇 + 計劃
   ...
   ![我的流 = 計劃 - 從空白*](./media/commercial-marketplace-lead-management-instructions-azure-table/ms-flow-scheduled-from-blank.png)

5.    在 *"生成"下的計劃流*視窗上 *,每個*選擇"1"為間隔,"小時"頻率。 此外,如果您願意,請為流指定一個名稱。 選取 [建立]  。
...
6. 選取 [+ 新步驟] ****。
7. 在「*選擇操作*」視窗「搜尋」過去時間」,然後選擇「在操作下**獲取過去時間**」。
...
在下一組步驟中,您將連接到 Azure 表,並設置處理邏輯以處理新潛在顧客。

9. 在「獲取過去時間」步驟後,選擇 **「新建步驟**」,然後在 *「選擇操作*」視窗中搜索「獲取實體」。
10. 在 **'操作'** 下,選擇**獲取實體(Azure 表存儲)。**
11.    在**Azure 表儲存**視窗中,提供以下欄位的資訊,然後選擇「**建立**:
* *連接名稱*- 為在此流和 Azure 表之間建立的連接提供有意義的名稱。
* *儲存帳戶名稱*- 提供 Azure 表的儲存帳戶的名稱。 您可以在儲存帳戶的訪問**密鑰**頁中找到此項。
* *共用儲存金鑰*- 為 Azure 表的儲存帳戶提供密鑰值。 您可以在儲存帳戶的訪問**密鑰**頁中找到此項。
    ![Azure 表存儲。](./media/commercial-marketplace-lead-management-instructions-azure-table/azure-table-storage.png)

按下"建立"後,您將看到 *「獲取實體」* 視窗。 這裡選擇 **「顯示進階選項**」並提供以下欄位的資訊:
* *表*- 選擇 Azure 儲存的名稱(從有關如何配置 Azure 表的說明的第 6 步)。 下一個螢幕截圖顯示為此範例選擇"市場潛在顧客"表時的提示。
    ![Azure 表獲取實體。](./media/commercial-marketplace-lead-management-instructions-azure-table/azure-table-get-entities.png)

* *篩選器查詢*- 選擇此欄位並將此函數貼上到欄位中:Azure`Timestamp gt datetime'@{body('Get_past_time')}'`![表獲取實體 - 篩選器查詢。](./media/commercial-marketplace-lead-management-instructions-azure-table/azure-table-get-entities-filter-query.png)

12. 現在,您已完成與 Azure 表的連接設定,請選擇 **「新建步驟**」以添加條件以掃描 Azure 表以創建新的潛在顧客。 

13. 在**選擇操作視窗**中,選擇 **'操作**',然後選擇 **'條件" 控制器**。
...
19. 在**Office 365 Outlook**視窗中,提供有關以下欄位的資訊:
    ...
    1. **正文**- 添加要包含在每封電子郵件中的文本(可選),然後貼上到`body('Get_entities')?['value']`正文中。
...
20. 選取 [儲存]**** 以儲存流程。 Microsoft Flow 將會自動測試流程中是否有錯誤。 如果沒有任何錯誤,則流在保存后開始運行。
...
如果未收到任何潛在顧客電子郵件通知,則意味著尚未將新潛在顧客添加到 Azure 表中。 如果有任何流故障,您將收到一封電子郵件,如下一個螢幕捕獲中的示例。

 ![流程失敗電子郵件通知](./media/commercial-marketplace-lead-management-instructions-azure-table/ms-flow-failure-note.png)

## <a name="configure-your-offer-to-send-leads-to-the-azure-table"></a>將產品/服務設定為將潛在顧客傳送到 Azure 表

準備好在發佈門戶中配置產品/服務的首席管理資訊時,請按照以下步驟操作:

1. 導航到產品 **/服務"產品/服務"設置**頁面。
2. 在「潛在顧客管理」部分下選擇 **「連接**」。
3. 在"連接詳細資訊"彈出視窗上,選擇**潛在顧客目標的** **Azure 表**,然後按照前面的步驟粘貼到**儲存帳戶連接字串**欄位,從創建的 Azure 儲存帳戶的連接字串中粘貼。
...
生成潛在顧客時,Microsoft 會將潛在顧客發送到 Azure 表。 如果設定了流,也會將電子郵件發送到您設定的電子郵件位址。

![潛在客戶管理](./media/commercial-marketplace-lead-management-instructions-azure-table/lead-management.png)

![潛在客戶管理 - 連線詳細資訊](./media/commercial-marketplace-lead-management-instructions-azure-table/connection-details.png)

![潛在客戶管理 - 連線詳細資訊儲存帳戶](./media/commercial-marketplace-lead-management-instructions-azure-table/azure-table-connection-details.png)
