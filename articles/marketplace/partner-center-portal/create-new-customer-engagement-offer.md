...
description: 如何為客戶參與創建新的 Dynamics 365 & PowerApps 產品，用於在 Azure 應用商店、AppSource 中或通過雲解決方案供應商 （CSP） 計畫使用 Microsoft 合作夥伴中心的商業應用商店門戶進行上市或銷售。
author: dsindona
ms.author: dsindona
...
選擇 **"+ 新產品/服務**"按鈕，然後選擇**Dynamics 365 以用於客戶參與度& PowerApps**功能表項目。 將顯示"**新建產品/服務**"對話方塊。

### <a name="offer-id-and-alias"></a>優惠 ID 和別名

- **優惠 ID**：帳戶中每個產品/服務的唯一識別碼。 此 ID 將在市場產品/服務的 URL 位址和 Azure 資源管理器範本（如果適用）中向客戶可見。 優惠 ID 必須是小寫字母數位字元（包括連字號和底線，但沒有空格）。 ID 限制為 50 個字元，在選擇"**創建**"後無法更改。 例如，如果您在此處輸入*測試報價 1，* 則產品/服務 URL 將為`https://azuremarketplace.microsoft.com/marketplace/../test-offer-1`。
...
選擇至少一個，最多選擇三個類別。 該**類別**將用於將您的產品/服務放入相應的市場搜索區域。 請務必在產品/服務說明中標注您的產品/服務如何支援這些類別。
...
輸入產品/服務的版本號。 客戶將在產品/服務的詳細資訊頁面上看到此版本。 如果由於市場行銷/描述性更改而僅更新版本號，請選中 **"僅限市場行銷更改"** 框。 此選項允許產品/服務繞過認證和預配階段。
...
您必須在此處至少提供一份（最多三份）相關行銷文檔，如白皮書、摺頁冊、檢查表或演示文稿。 這些檔必須採用 .pdf 格式。
...
此處的選擇僅適用于新的收購;如果有人已經擁有你的應用程式在某一市場，你後來刪除該市場，誰已經在這個市場的報價的人可以繼續使用它，但沒有新的客戶，在這個市場將能夠得到您的報價。
...
### <a name="requires-s2s-outbound-and-crm-secure-store-access"></a>需要 S2S 出站和 CRM 安全存儲訪問

選中此框可啟用 CRM 安全存儲或伺服器到伺服器 （S2S） 出站訪問的配置。 這項功能需要 Dynamics 365 小組在認證階段期間的特殊考量。 Microsoft 會連絡您完成其他步驟，以支援這項功能。

### <a name="application-configuration-url"></a>應用程式佈建 URL

提供可使客戶能夠配置應用的配置網頁的 URL。

### <a name="crm-package"></a>CRM 包

在**包位置欄位的 URL**中，輸入包含上載的 CRM 包 .ZIP 檔案的 Azure 存儲帳戶的 URL。 此 URL 應包括一個唯讀 SAS 金鑰，以允許 Microsoft 拿起您的包進行驗證。

選中標記為"**我的包檔中有多個 CRM 包**"核取方塊（如果適用）。 如果是這樣，請確保在 .ZIP 檔案中包含所有包。

### <a name="crm-package-availability"></a>CRM 套裝軟體可用性
...
預設情況下，您上面輸入**的應用程式佈建 URL**將用於每個區域。 如果您願意，可以為一個或多個特定區域輸入單獨的應用程式佈建 URL。 
...
- **角色名稱**（必需）：提供您在自訂 Dynamics 365 試駕中定義的安全形色名稱。 此安全形色名稱將在使用者試駕期間（例如，測試-磁碟機角色）分配給使用者。
...
- **區域**（必需，單選下拉清單）：目前有 26 個 Azure 支援的區域，可以在其中提供您的試駕。 邏輯應用的資源將部署在您選擇的區域中。 如果邏輯應用在特定區域中存儲了任何自訂資源，請確保在此處選擇該區域。 最好的方法是在門戶中的 Azure 訂閱中本地完全部署邏輯應用，並在進行此選擇之前驗證其是否正常工作。
...
為了代表您部署測試磁碟機，請創建並提供單獨的唯一 Azure 訂閱（Power BI 測試磁碟機不需要）。
...
- 通過選擇 **"提交**"提交要發佈的產品/ 我們將向您發送一封電子郵件，告知您何時可以審核和批准優惠的預覽版本。 返回合作夥伴中心，選擇 **"Go-live"** 以向公眾發佈產品/服務（如果是私人優惠，則將其發佈到私人受眾）。
