...
- [存取合作夥伴中心帳戶設定](#access-your-account-settings)
- [尋找發行者 ID、賽門鐵克 ID、賣家 ID、使用者 ID、MPN ID 和 Azure AD 租戶](#account-details)
- [更新聯絡資訊](#contact-info)
- [設定追蹤 GUID 以監控客戶使用方式](#tracking-guids)
- [管理使用者](#manage-users)
- [管理群組](#manage-groups)
...
- **聯絡資訊**: 發布者顯示名稱、賣家連絡人姓名、電子郵件、電話和位址
- **追蹤 GUID:** 任何追蹤 GUID 與您的帳戶相關聯

### <a name="account-details"></a>帳號詳細資訊
...
在發行者 ID 部份中,您可以看到**您的賽門鐵克 ID、****賣家 ID、****使用者 ID、MPN** **ID**和 Azure **AD 租戶**。 這些值由 Microsoft 分配,以唯一標識您的開發人員帳戶,並且無法編輯。

### <a name="contact-info"></a>連絡資訊

在「聯繫資訊」部分中,您可以看到您的**Publisher 顯示名稱**、**賣家聯絡資訊**(公司賣家的連絡人姓名、電子郵件、電話號碼和位址)**和公司審批人**(有權批准公司決策的個人的姓名、電子郵件和電話號碼)。
...
- **選擇付款條件**:銀行帳戶或PayPal
- **添加付款資訊**:這可能包括選擇帳戶類型(支票或儲蓄)、輸入帳戶持有人姓名、帳號和路由號碼、帳單位址、電話號碼或 PayPal 電子郵件位址。 *有關使用PayPal作為您的帳戶付款條件,並瞭解其是否支援您的市場區域的詳細資訊,請參閱[PayPal 資訊](https://docs.microsoft.com/windows/uwp/publish/setting-up-your-payout-account-and-tax-forms#paypal-info)。
...
檢視您目前的稅務設定檔狀態,確認顯示正確的**實體類型**和**稅務憑證資訊**。 選擇 **「編輯」** 以更新或完成任何必需的表單。
...
如果您在美國以外的地區，可能可以享有稅務協定優惠。 這些福利因國家/地區而異,並可能允許您減少Microsoft扣繳的稅款金額。 您可以完成 W-8BEN 表單的 Part II 來要求稅務協定優惠。 建議您與您國家或地區的適當資源聯繫，以確認您是否適用這些優惠。

[詳細瞭解 Windows 應用程式/遊戲開發人員和 Azure 應用商店發佈商的稅務詳細資訊](https://docs.microsoft.com/windows/uwp/publish/tax-details-for-paid-apps)。

#### <a name="payout-hold-status"></a>支付保留狀態

默認情況下,Microsoft 每月發送一次付款。 但是,您可以選擇暫停付款,這將阻止將付款發送到您的帳戶。 如果您選擇暫停您的支出,我們將繼續記錄您賺取的任何收入,並在**您的付款摘要**中提供詳細資訊。 但是,在您刪除保留之前,我們不會向您的帳戶發送任何付款。 
...
將支付保留狀態設為 [**開啟**] 之後，除非將滑桿切換回 [**關閉**]，否則會保留所有支付。 當您這樣做時,您將在下一個月的支付週期中被包括在內(前提是任何適用的付款閾值都已滿足)。 例如,如果您已暫停付款,但希望 6 月產生支出,請確保在 5 月底之前將付款保留狀態切換為 **"關閉**"。

> [!NOTE]
> **您的"支出保留"狀態**選擇適用於通過 Microsoft 合作夥伴中心支付**的所有**收入來源,包括 Azure 應用商店、AppSource、Microsoft 應用商店、廣告等。 您無法針對個別收入來源選取不同保留狀態。

### <a name="devices"></a>裝置

設備管理設置僅適用於 UWP 發表。 [深入了解](https://docs.microsoft.com/windows/uwp/publish/manage-account-settings-and-profile#additional-settings-and-info)。

### <a name="tracking-guids"></a>追蹤 GUID

全域唯一標識碼 (GUID) 是唯一的參考編號(具有 32 個十六進位數位),可用於追蹤 Azure 使用方式。 
...
如果使用範本部署產品,並且該產品在 Azure 應用商店和 GitHub 上都可用,則可以創建和註冊兩個不同的 GUID:

*    Azure Marketplace 中的產品 A
*    GitHub 上的產品 A

報告是由合作夥伴值 (Microsoft 合作夥伴 ID) 與 GUID 所完成。 您還可以在更精細的級別跟蹤 GUID,以便與產品/服務中的每個計劃保持一致。

有關詳細資訊,請參閱[使用 GUID 追蹤 Azure 客戶使用方式常見問題 。](https://docs.microsoft.com/azure/marketplace/azure-partner-customer-usage-attribution#faq)

## <a name="create-a-billing-profile"></a>建立帳單設定檔

如果您要發布動態[365 用於客戶參與&電源應用](./create-new-customer-engagement-offer.md)或[動態 365 為營運](./create-new-operations-offer.md)產品,您需要填寫您的**帳單設定檔**。
...
1. 轉到**使用者**(在 **'帳戶設定**"下)並選擇「**添加使用者**」 。
2. 從出現的清單中選取一個或多個使用者。 您可以使用搜尋方塊來搜尋特定的使用者。
•如果選擇要添加到合作夥伴中心帳戶的多個使用者,則必須為其分配相同的角色或自定義許可權集。 要添加具有不同角色/許可權的多個使用者,請為每個角色或自定義許可權集重複這些步驟。
3. 完成選擇使用者後,按一下添加**選定的**。
...
1. 轉到**使用者**(在 **'帳戶設定**"下),選擇 **'添加使用者**",然後選擇"**創建新使用者**"。
1. 為每個新用戶輸入名字、姓氏和使用者名。 
1. 如果希望新使用者在組織的目錄中具有全域管理員帳戶,請選擇標記為 **「使此使用者成為 Azure AD 中的全域管理員」複選框,並完全控制所有目錄資源**。 這將可讓使用者完整存取您公司的 Azure AD 中的所有系統管理功能。 他們將能夠添加和管理組織的工作帳戶(Azure AD 租戶)中的使用者,儘管不在合作夥伴中心,除非您授予該帳戶適當的角色/許可權。
1. 如果勾選此方**塊 以使此使用者成為全域管理員**,則需要提供**密碼恢復電子郵件**,以便使用者在必要時恢復其密碼。
1. 在 **「組成員身份**」部分中,選擇要新用戶所屬的任何組。
...
1. 跳到**使用者**(在 **'帳戶設定**"下),選擇 **'新增使用者**",然後選擇**透過電子郵件邀請使用者**。
2. 輸入一個或多個電子郵件地址 (最多十個)，以逗號或分號分隔。
3. 在 **\[角色\]** 區段中，指定使用者的角色或自訂權限。
4. 選取 [儲存]  。

您邀請的用戶將獲得加入合作夥伴中心帳戶的電子郵件邀請。 將在工作帳戶(Azure AD 租戶)中創建新的來賓使用者帳戶。 使用者必須接受邀請才能存取您的帳戶。

如果需要重新發送邀請,請造訪 **「使用者」** 頁面,在使用者清單中尋找邀請,選擇其電子郵件地址(或顯示 *"邀請待定*"的文字)。 然後,在頁面底部,選擇 **「重新發送邀請**」。
...
1. 從 **「使用者」** 頁(在 **「帳戶設定**」下)選擇要編輯的使用者帳戶的名稱。
2. 選擇頁面底部的 **「重置密碼**」按鈕。
...
1. 從 **「使用者」** 頁(在 **「帳戶設定**」下)選擇 **「新增群組**」 。
2. 從出現的清單中選取一個或多個群組。 您可以使用搜尋方塊來搜尋特定的群組。
如果選擇要添加到合作夥伴中心帳戶的多個組,則必須為其分配相同的角色或自定義許可權集。 要添加具有不同角色/許可權的多個組,請為每個角色或一組自定義許可權重複這些步驟。
3. 完成選擇組後,按一下「**添加所選**」。。
...
1. 從 **「使用者」** 頁(在 **「帳戶設定**」下)選擇 **「添加 Azure AD 應用程式**」。
2. 從出現的清單中選取一個或多個 Azure AD 應用程式。 您可以使用搜尋方塊來搜尋特定的 Azure AD 應用程式。 如果選擇要添加到合作夥伴中心帳戶的多個 Azure AD 應用程式,則必須為其分配相同的角色或自定義許可權集。 要添加具有不同角色/許可權的多個 Azure AD 應用程式,請為每個角色或一組自定義許可權重複這些步驟。
...
3. 輸入新 Azure AD 應用程式的**回覆 URL。** 使用者可以從此 URL 登入並使用您的 Azure AD 應用程式 (也稱為 App URL 或登入 URL)。 **回覆 URL** 長度不可超過 256 個字元，而且在您的目錄中必須是唯一的。
4. 輸入新 Azure AD 應用程式的 **\[應用程式識別碼 URI\]**。 這是 Azure AD 應用程式的邏輯識別碼,在將單個登入請求發送到 Azure AD 時顯示。 請注意,對於目錄中的每個 Azure AD 應用程式,**應用 ID URI**必須是唯一的。 此 ID 不能超過 256 個字元。 有關應用程式識別碼的詳細資訊,請參閱[將應用程式與 Azure 的活動目錄整合](https://docs.microsoft.com/azure/active-directory/develop/quickstart-modify-supported-accounts#change-the-application-registration-to-support-different-accounts)。
...
1. 從 **「使用者」** 頁(在 **「帳戶設定**」下)選擇 Azure AD 應用程式的名稱。 您將看到 Azure AD 應用程式的所有活動金鑰,包括創建密鑰的日期和何時過期。 
2. 要刪除不再需要的鍵,請選擇 **"刪除**"。
3. 要添加新鍵,請選擇「**添加新鍵**」。
4. 您將看到一個螢幕,顯示**客戶端 ID**鍵**值**。 請務必列印或複製此資訊,因為離開此頁面后將無法再次訪問此資訊。
5. 如果要創建更多鍵,請選擇「**添加其他鍵**」。。

## <a name="define-user-roles-and-permissions"></a>定義使用者角色和權限

可以為合作夥伴中心上的商業市場計劃分配以下角色和許可權:

- **Manager**
  - 可以存取除稅費和付款設定之外的所有 Microsoft 帳戶功能
  - 可以管理使用者、角色和工作帳戶(租戶)
- **開發人員**
  - 可以管理和發佈優惠
  - 可以檢視某些發行者報告
...
> [!NOTE]
> 對於商業市場計劃,不使用全域管理員、業務貢獻者、財務貢獻者和行銷人員角色。 將這些角色分配給使用者不起作用。 只有管理器和開發人員角色向使用者授予許可權。

有關在合作夥伴中心的其他區域(如 Azure 活動目錄 (AD)、雲端解決方案供應商 (CSP)、控制面板供應商 (CPV)、來賓使用者或 Microsoft 合作夥伴網路 (MPN))中管理角色和許可權的詳細資訊,請參閱[在合作夥伴中心中分配使用者角色和許可權](https://docs.microsoft.com/partner-center/permissions-overview)。

## <a name="manage-tenants"></a>管理租戶

Azure 活動目錄 (AD) 租戶(在本文檔中也稱為"工作帳戶")是您在 Azure 門戶中設置的組織的表示形式,可説明您為內部和外部使用者管理 Microsoft 雲服務的特定實例。 如果您的組織訂閱了 Microsoft 雲服務(如 Azure、Microsoft Intune 或 Office 365),則為您建立了 Azure AD 租戶。

您可以設置多個租戶以與合作夥伴中心一起使用。 在合作夥伴中心帳戶中具有**Manager**角色的任何使用者都可以選擇從帳戶中添加和刪除 Azure AD 租戶。  

### <a name="add-an-existing-tenant"></a>新增現有租戶
...
### <a name="create-a-new-tenant"></a>建立新租戶

要使用合作夥伴中心帳戶創建新的 Azure AD 租戶,請執行:

1. 從 **「租戶」** 頁(在 **「帳戶設定**」下)選擇 **「創建新的 Azure AD 租戶**」。。
2. 輸入新 Azure AD 的目錄資訊︰
    - **功能變數名稱**:我們將用於 Azure AD 網域的唯一名稱,以及".onmicrosoft.com"。 例如,如果輸入"範例",則 Azure AD 網域將為"example.onmicrosoft.com"。
...
有關在 Azure 門戶(而不是透過合作夥伴中心門戶)中創建新租戶的詳細資訊,請參閱在 Azure[活動目錄中創建新租戶](https://docs.microsoft.com/azure/active-directory/fundamentals/active-directory-access-create-new-tenant)的文章。

### <a name="remove-a-tenant"></a>移除租戶

要從合作夥伴中心帳戶中刪除租戶,請在 **「租戶」** 頁上尋找其名稱(在 **「帳戶設置**」中),然後選擇「**刪除**」。。 系統將提示您確認要刪除租戶。 執行此操作後,該租戶中的任何使用者都無法登錄到合作夥伴中心帳戶,並且您為這些使用者配置的任何許可權都將被刪除。
