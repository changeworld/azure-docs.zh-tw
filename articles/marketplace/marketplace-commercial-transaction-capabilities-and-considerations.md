---
title: Microsoft 商業 marketplace 交易功能
description: 本文說明 Microsoft 商業 marketplace 交易選項的定價、計費、發票處理和支出考慮。
author: dsindona
...
具有現有商務功能的發行者可以挑選清單、試用版和自備授權（BYOL）發行選項，以供促銷和使用者取得之用。 使用這些選項時，Microsoft 不會直接參與發行者的軟體授權交易，也不會有相關聯的交易費用。 發行者要負責支援軟體授權交易的所有層面，包含但不限於：訂購、履行、計量、計費、發票開立、付款與收帳。 使用清單和試用版發行選項，發行者保有從客戶收取的發行者軟體授權費用的 100%。
...
Microsoft 可讓客戶訂購、授權及使用發行者軟體，叢集 Microsoft 的商業 Marketplace 和發行者的使用者授權合約的條款及條件。 發行者必須提供其使用者授權合約，或在建立供應專案時選取[標準合約](./standard-contract.md)。
