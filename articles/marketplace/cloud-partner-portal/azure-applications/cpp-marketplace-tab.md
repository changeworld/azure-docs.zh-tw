...
ms.locfileid: "81416302"
---
# <a name="azure-application-marketplace-tab"></a>Azure 應用程式 Marketplace 索引標籤

使用 [Marketplace] 索引標籤來說明 Azure 應用程式並提供行銷資產。 此選項卡包括以下窗體:概述、市場行銷專案、潛在顧客管理和法律。
...
| **行銷識別碼\*** | 與本供應項目相關聯的唯一不重複網址，通常會包含您的組織與解決方案名稱，長度上限是 50 個字元。 為您的服務選擇簡短、易讀的行銷識別碼。 這將用於此供應項目的市集 URL。 例如,如果您的發行者 ID 是「contoso」,而行銷標識符為「示例App」,則 Azure 應用商店中產品/服務的 URL 將為`https://azuremarketplace.microsoft.com/marketplace/apps/contoso.sampleApp`  
| **預覽訂閱代碼\*** | 可新增從 1 到 100 個預覽使用者的訂閱帳戶識別碼。 這些白名單的訂閱將有權訪問您的產品/服務,而優惠在發佈后,在發佈后,在發佈之前,在預覽版中可用。          |
...
|      欄位         |    描述    |
|  ---------------   |  ---------------  |
| **小型\***        | 小徽標:40x40 像素,採用 PNG 格式     |
| **中型\***       | 中等徽標:90x90 像素,採用 PNG 格式    |
| **大\***        | 大徽標:115x115 像素,採用 PNG 格式   |
| **寬\***         | 寬徽標:255x115 像素,採用 PNG 格式    |
| **主圖**           | 可選的英雄徽標:815x290 像素,採用 PNG 格式。 **註:** 英雄圖示在上載後無法刪除。 |
...
| **新增影片**      | 可選,視頻將顯示在您的產品詳細信息頁面上。 相當適合用來以視覺方式傳達應用程式的功能及其運作方式。 若要新增影片： <ul><li>選取 [+ 新增影片]**** 以開啟 [影片] 視窗</li><li>**名稱** - 輸入名稱/標題 (長度上限是 100 個字元)。</li><li>**連結**- 輸入託管視訊的網站的網址(YouTube 或 Vimeo)</li><li>**縮圖**- 上傳縮略圖。 它必須是 PNG 格式，而大小為 533x324 像素。</li></ul>          |
|  |  |
...
>[!TIP]
>選擇資訊圖示以查看此消息:「選擇將存儲潛在顧客的系統。 在此處瞭解如何連接到 CRM[系統。](https://docs.microsoft.com/azure/marketplace/cloud-partner-portal-orig/cloud-partner-portal-get-customer-leads)
