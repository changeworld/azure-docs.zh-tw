---
title: 如何設定您的 Azure 應用程式供應專案清單詳細資料
description: 瞭解如何在合作夥伴中心中設定 Azure 應用程式供應專案的清單詳細資料。
author: aarathin
ms.author: aarathin
...
ms.locfileid: "94370025"
---
# <a name="how-to-configure-your-azure-application-offer-listing-details"></a>如何設定您的 Azure 應用程式供應專案清單詳細資料

您在 Azure 應用程式供應專案的 **供應專案清單** 頁面上提供的資訊，將會顯示在 Microsoft 商業 marketplace 線上商店中。 這包括您的供應專案、螢幕擷取畫面和行銷資產的描述。 若要查看這看起來的樣子，請參閱 [供應專案清單詳細資料](plan-azure-application-offer.md#offer-listing-details)。

> [!NOTE]
> 供應專案清單內容 (例如描述、檔、螢幕擷取畫面及使用規定) 如果供應專案描述的開頭是「非英文語言」，則不需要使用英文版。 您也可以接受提供有用的 _連結_ URL，以不同于供應專案清單內容中所使用的語言提供內容。
...
1. [ **名稱** ] 方塊預先填入您稍早在 [ **新增供應** 專案] 對話方塊中輸入的名稱。 您可以隨時變更名稱。 您在此處輸入的名稱會作為供應項目清單標題向客戶顯示。
1. 在 [ **搜尋結果摘要** ] 方塊中，輸入最多100個字元的文字。 此摘要會在 marketplace 清單搜尋結果中使用。
1. 在 [ **簡短描述** ] 方塊中，輸入最多256個字元的純文字。 此摘要會顯示在供應專案的詳細資料頁面上。
...
1.  (選擇性) 在 [ **搜尋關鍵字** ] 方塊中，輸入最多三個搜尋關鍵字，讓客戶可以用來在商業市場中尋找您的供應專案。 您不需要包含供應專案 **名稱** 和 **描述** ，因為該文字會自動包含在搜尋中。
...
1. 在出現的欄位中，輸入 (最多255個字元的標題) 和連結 (從 `https://`) 開始至線上檔。
...
- **支援連絡人** (必要) - 適用於一般支援問題。
- **工程連絡人** (必要) - 適用於技術問題。 當您的供應專案發生問題時，我們會使用此資訊來與您聯繫，包括認證問題。
- **CSP 方案可聯絡** (選用的) –適用于支援和商務問題。 這項資訊只會顯示給 CSP 合作夥伴。

您將為每個連絡人提供名稱、電話號碼和電子郵件地址， (這些都不會公開) 。 支援連絡人需要支援 URL (這會公開) 。
...
## <a name="add-marketplace-media"></a>新增 marketplace 媒體
...
最多可新增五個螢幕擷取畫面，以示範您的供應專案。 所有影像的大小都必須是 1280 x 720 圖元。PNG 格式。

1. 在 [ **螢幕擷取畫面** ] 下，拖放您的。PNG 檔案到 **螢幕擷取畫面** 方塊。
1. 按一下 [ **新增影像標題** ] 旁的 [編輯] 圖示。
1. 在出現的對話方塊中輸入標題，然後按一下 **[確定]** 。
1. 重複步驟1到3以新增其他螢幕擷取畫面。
...
1. 在出現的方塊中，輸入您影片的名稱和連結。
1. 拖放。PNG 檔 (1280 x 720 圖元) 至灰色 **縮圖** 方塊。
1. 若要新增其他影片，請重複步驟1到3。
...
- [如何為您的 Azure 應用程式供應專案新增預覽物件](create-new-azure-apps-offer-preview.md)
