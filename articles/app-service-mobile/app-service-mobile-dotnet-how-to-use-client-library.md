...
## <a name="setup-and-prerequisites"></a><a name="setup"></a>設定和必要條件
我們假設您已建立並發佈您的行動應用程式後端專案 (至少包含一個資料表)。  在本主題使用的程式碼中，資料表的名稱為 `TodoItem`，且其具有下列資料行：`Id`、`Text` 和 `Complete`。 此表與完成[Azure 移動應用快速啟動][1]時創建的表相同。
...
* **薩馬林工作室**按右鍵您的專案，按一下"**添加**> **NuGet 包**"，`Microsoft.Azure.Mobile.Client`搜索包，然後按一下"**添加包**"。
...
### <a name="how-to-work-with-debug-symbols-in-visual-studio"></a><a name="symbolsource"></a>做法︰使用 Visual Studio 中的偵錯符號
您可以從 [SymbolSource][10] 取得適用於 Microsoft.Azure.Mobile 命名空間的符號。  若要將 SymbolSource 與 Visual Studio 整合，請參閱 [SymbolSource 指示][11]。

## <a name="create-the-mobile-apps-client"></a><a name="create-client"></a>建立 Mobile Apps 用戶端
...
### <a name="how-to-create-a-table-reference"></a><a name="instantiating"></a>作法：建立資料表參考
...
傳回的物件會使用具類型的序列化模型。 也支援不具類型的序列化模型。 下面的示例[創建對未鍵入的表的引用]：
...
### <a name="how-to-query-data-from-your-mobile-app"></a><a name="querying"></a>如何：查詢行動應用程式中的資料
...
### <a name="how-to-filter-returned-data"></a><a name="filtering"></a>如何：篩選返回的資料
下列程式碼說明如何在查詢中包含 `Where` 子句，以篩選資料。 它會從 `todoTable` 傳回其 `Complete` 屬性等於 `false` 的所有項目。 [Where]函數對表的查詢應用行篩選謂詞。
...
### <a name="how-to-sort-returned-data"></a><a name="sorting"></a>作法：排序傳回的資料
...
### <a name="how-to-return-data-in-pages"></a><a name="paging"></a>作法：以分頁方式傳回資料
...
### <a name="how-to-select-specific-columns"></a><a name="selecting"></a>作法：選取特定資料欄
...
### <a name="how-to-look-up-data-by-id"></a><a name="lookingup"></a>作法：按識別碼查詢資料
...
### <a name="how-to-execute-untyped-queries"></a><a name="untypedqueries"></a>如何：執行不具類型的查詢
...
### <a name="how-to-insert-data-into-a-mobile-app-backend"></a><a name="inserting"></a>如何：將資料插入行動應用程式後端
所有用戶端類型都必須包含名為 **Id**的成員，其預設為字串。 執行 CRUD 操作和離線同步需要此**Id。** 以下代碼說明了如何使用[InsertAsync]方法將新行插入到表中。 參數包含要作為 .NET 物件插入的資料。
...
### <a name="how-to-modify-data-in-a-mobile-app-backend"></a><a name="modifying"></a>如何：修改行動應用程式後端中的資料
...
### <a name="how-to-delete-data-in-a-mobile-app-backend"></a><a name="deleting"></a>如何：刪除行動應用程式後端中的資料
...
### <a name="how-to-use-optimistic-concurrency-for-conflict-resolution"></a><a name="optimisticconcurrency"></a>做法：使用開放式並行存取來解決衝突
兩個或多個用戶端可能會同時對相同項目寫入變更。 在沒有偵測到衝突的情況下，最後寫入將覆寫任何先前的更新。 **樂觀併發控制**假定每個事務都可以提交，因此不使用任何資源鎖定。  在認可交易之前，開放式並行存取控制項會驗證沒有其他交易已修改此資料。 如果資料已修改，則會復原認可的交易。
...
### <a name="how-to-bind-mobile-apps-data-to-a-windows-user-interface"></a><a name="binding"></a>如何：將 Mobile Apps 資料繫結至 Windows 使用者介面
...
### <a name="change-the-page-size"></a><a name="pagesize"></a>變更頁面大小
...
## <a name="work-with-offline-tables"></a><a name="#offlinesync"></a>使用離線資料表
...
3. (選擇性)。 對於 Windows 設備，按一下"**引用** > **添加參考..."，** 展開**Windows**資料夾>**擴展**，然後為 Windows SDK 啟用適當的**SQLite**以及 Windows SDK**的視覺化C++ 2013 運行時**。
...
### <a name="syncing-an-offline-table"></a><a name="syncoffline"></a>同步處理離線資料表
...
## <a name="work-with-a-custom-api"></a><a name="#customapi"></a>使用自訂 API
...
## <a name="authenticate-users"></a><a name="authentication"></a>驗證使用者
...
### <a name="client-managed-authentication"></a><a name="clientflow"></a>用戶端管理的驗證
...
#### <a name="authenticate-users-with-the-active-directory-authentication-library"></a><a name="adal"></a>使用 Active Directory Authentication Library 驗證使用者
...
3. 根據您使用的平台，將下列程式碼新增至您的應用程式。 在每個程式碼中，進行下列取代：
   ...
   * 使用 HTTPS 配置，以您網站的 **/.auth/login/done** 端點取代 *INSERT-REDIRECT-URI-HERE* 。 此值應類似于*https://contoso.azurewebsites.net/.auth/login/done*。
...
#### <a name="single-sign-on-using-a-token-from-facebook-or-google"></a><a name="client-facebook"></a>使用來自 Facebook 或 Google 的權杖單一登入
...
### <a name="server-managed-authentication"></a><a name="serverflow"></a>伺服器管理的驗證
...
在伺服器流程中，Azure App Service 透過顯示所選提供者的登入頁面，來管理 OAuth 驗證流程。  在識別提供者傳回後，Azure App Service 會產生 App Service 驗證權杖。 [LoginAsync] 方法 會傳回 [MobileServiceUser]，並提供通過驗證使用者的 [UserId] 和 [MobileServiceAuthenticationToken]，以作為 JSON Web 權杖 (JWT)。 您可以快取並重複使用此權杖，直到它到期為止。 有關詳細資訊，請參閱[緩存身份驗證權杖](#caching)。

### <a name="caching-the-authentication-token"></a><a name="caching"></a>快取驗證權杖
...
Xamarin 應用使用[Xamarin.Auth] API 安全地將憑據存儲在**帳戶**物件中。 如需使用這些 API 的範例，請參閱 [ContosoMoments 照片分享範例](https://github.com/azure-appservice-samples/ContosoMoments)中的 [AuthStore.cs] 程式碼檔案。
...
### <a name="how-to-register-for-push-notifications"></a><a name="register-for-push"></a>如何：註冊推送通知
...
### <a name="how-to-obtain-a-microsoft-store-package-sid"></a><a name="package-sid"></a>如何：取得 Microsoft Store 套件 SID
在 Microsoft Store 應用程式中啟用推播通知需有套件 SID。  若要收到套件 SID，請向 Microsoft Store 註冊應用程式。

若要取得這個值：

1. 在視覺化工作室解決方案資源管理器中，按右鍵 Microsoft 應用商店應用專案，按一下**應用商店** > **關聯應用..."**
...
5. 按一下**應用管理** > **應用標識**，然後向下滾動以查找**您的包 SID**。
...
### <a name="how-to-register-push-templates-to-send-cross-platform-notifications"></a><a name="register-xplat"></a>作法：註冊推送範本以傳送跨平台通知
...
## <a name="miscellaneous-topics"></a><a name="misc"></a>其他主題
### <a name="how-to-handle-errors"></a><a name="errors"></a>作法：處理錯誤
...
### <a name="how-to-customize-request-headers"></a><a name="headers"></a>作法：自訂要求標頭
...
[向應用添加身份驗證]: app-service-mobile-windows-store-dotnet-get-started-users.md
...
[包括總計數]: https://msdn.microsoft.com/library/azure/dn250560(v=azure.10).aspx
...
[訂單按]: https://msdn.microsoft.com/library/azure/dn250572(v=azure.10).aspx
...
[服用]: https://msdn.microsoft.com/library/azure/dn250574(v=azure.10).aspx
[選取]: https://msdn.microsoft.com/library/azure/dn250569(v=azure.10).aspx
[跳]: https://msdn.microsoft.com/library/azure/dn250573(v=azure.10).aspx
[UpdateAsync]: https://msdn.microsoft.com/library/azure/dn250536.(v=azure.10)aspx
[使用者 ID]: https://msdn.microsoft.com/library/windowsazure/microsoft.windowsazure.mobileservices.mobileserviceuser.userid(v=azure.10).aspx
[哪裡]: https://msdn.microsoft.com/library/azure/dn250579(v=azure.10).aspx
