...
## <a name="how-to-create-a-net-mobile-app-backend"></a><a name="create-app"></a>作法：建立 .NET 行動應用程式後端
...
1. 開啟 [新增專案]**** 對話方塊 (從 [檔案]**** >  [新增]**** >  [專案...]****)。
2. 展開**範本** > **視覺化 C#，** 然後選擇**Web**。
3. 選取 [ASP.NET Web 應用程式] ****。
4. 填入專案名稱。 然後按一下 **[確定]**。
5. 在 [ASP.NET 4.5.2 範本]** 底下，選取 [Azure 行動應用程式]****。 核取 [雲端中的主機] **** 以在雲端 (您可以在其中發佈這個專案) 建立行動後端。
6. 按一下 [確定]****。

## <a name="how-to-download-and-initialize-the-sdk"></a><a name="install-sdk"></a>如何：下載並初始化 SDK
SDKNuGet.org[上可用]。此包包括開始使用 SDK 所需的基本功能。 若要初始化 SDK，您需要在 **HttpConfiguration** 物件上執行動作。

### <a name="install-the-sdk"></a>安裝 SDK
若要安裝 SDK，請以滑鼠右鍵按一下 Visual Studio 中的伺服器專案，選取 [管理 NuGet 套件]****，搜尋 [Microsoft.Azure.Mobile.Server] 套件，然後按一下 [安裝]****。

### <a name="initialize-the-server-project"></a><a name="server-project-setup"></a> 初始化伺服器專案
初始化 .NET 後端伺服器專案的方式類似其他 ASP.NET 專案，可藉由包含 OWIN 啟動類別來完成。 請確定您已參考 NuGet 封裝 `Microsoft.Owin.Host.SystemWeb`。 要在 Visual Studio 中添加此類，請按右鍵伺服器專案並選擇"**添加新** >
**專案**"，然後選擇**Web** > **常規** > **OWIN 啟動類**。  隨即產生具有下列屬性的類別：
...
## <a name="how-to-publish-the-server-project"></a><a name="publish-server-project"></a>做法：發佈伺服器專案
...
3. 如果要求選擇發佈目標，請按一下**Microsoft Azure 應用服務** > **下一步**，然後（如果需要）使用 Azure 憑據登錄。
...
## <a name="how-to-define-a-table-controller"></a><a name="define-table-controller"></a> 做法：定義資料表控制器
...
### <a name="how-to-adjust-the-table-paging-size"></a><a name="adjust-pagesize"></a>作法：調整資料表分頁大小
...
1. 在 Visual Studio 中，按右鍵"控制器"資料夾，然後按一下"**添加** > **控制器**"，選擇**Web API 2 控制器&mdash;為空**，然後按一下"**添加**"。
...
* [如何：檢索到經過身份驗證的使用者資訊](#user-info)
* [做法︰限制授權使用者的資料存取](#authorize)

### <a name="how-to-add-authentication-to-a-server-project"></a><a name="add-auth"></a>做法：將驗證新增至伺服器專案
...
### <a name="how-to-use-custom-authentication-for-your-application"></a><a name="custom-auth"></a>做法：針對應用程式使用自訂驗證
> [!IMPORTANT]
...
### <a name="how-to-retrieve-authenticated-user-information"></a><a name="user-info"></a>做法：取出已驗證的使用者資訊
...
### <a name="how-to-restrict-data-access-for-authorized-users"></a><a name="authorize"></a>做法︰限制授權使用者的資料存取
...
## <a name="how-to-enable-targeted-push-using-tags"></a><a name="tags"></a>作法：使用標籤啟用有目標的推播
...
## <a name="how-to-send-push-notifications-to-an-authenticated-user"></a><a name="push-user"></a>做法：將推播通知傳送給已驗證的使用者
...
* [在 Azure 應用服務中啟用診斷日誌記錄](../app-service/troubleshoot-diagnostic-logs.md)
...
1. 按照[啟用應用程式日誌記錄 （Windows） 中的步驟操作](../app-service/troubleshoot-diagnostic-logs.md#enable-application-logging-windows)。
...
5. 下載並評估日誌，如[訪問日誌檔](../app-service/troubleshoot-diagnostic-logs.md#access-log-files)中所述。

### <a name="local-debugging-with-authentication"></a><a name="local-debug"></a>使用驗證進行本機偵錯
...
在本地應用程式佈建中使用*authSigningKey*參數的簽名金鑰。 移動後端現在已具備在本地運行時驗證權杖的功能，用戶端將從基於雲的終結點獲取權杖。
