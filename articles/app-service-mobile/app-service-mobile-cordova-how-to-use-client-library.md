...
## <a name="setup-and-prerequisites"></a><a name="Setup"></a>設置和先決條件
...
## <a name="setting-up-an-ionic-v2-app"></a><a name="ionic"></a>設定 Ionic v2 應用程式
...
## <a name="how-to-authenticate-users"></a><a name="auth"></a>如何：對使用者進行身份驗證
Azure App Service 支援使用各種外部識別提供者 (Facebook、Google、Microsoft 帳戶及 Twitter) 來驗證與授權應用程式使用者。 您可以在資料表上設定權限，以限制僅有通過驗證使用者可以存取特定操作。 您也可以使用通過驗證使用者的身分識別來實作伺服器指令碼中的授權規則。 有關詳細資訊，請參閱[開始使用身份驗證]教程。
...
### <a name="how-to-configure-your-mobile-app-service-for-external-redirect-urls"></a><a name="configure-external-redirect-urls"></a>如何：為外部重定向 URL 配置移動應用服務。
...
7. 尋找「allowedExternalRedirectUrls」元素。  它可能會設定為 null 或值陣列。  將此值變更為下列值︰
    ...
    使用您服務的 URL 來取代 URL。  示例包括`http://localhost:3000`（Node.js 示例服務）或`http://localhost:4400`（對於波紋服務）。  不過，這些 URL 都是範例 - 您的情況 (包括範例中提及的服務) 可能會有差異。
8. 按一下螢幕右上角的 [讀/寫] **** 按鈕。
9. 按一下綠色 [PUT] **** 按鈕。
...
## <a name="how-to-register-for-push-notifications"></a><a name="register-for-push"></a>作法：註冊推播通知
