...
### <a name="create-a-basic-nodejs-back-end-by-using-the-command-line"></a><a name="howto-cmdline-basicapp"></a>使用命令列建立基本 Node.js 後端
...
### <a name="create-a-nodejs-back-end-by-using-visual-studio-2015"></a><a name="howto-vs2015-basicapp"></a>使用 Visual Studio 2015 建立 Node.js 後端
...
1. 在本地運行應用程式（API 在`http://localhost:3000`上提供）或發佈到 Azure。

### <a name="create-a-nodejs-back-end-by-using-the-azure-portal"></a><a name="create-node-backend-portal"></a>使用 Azure 入口網站建立 Node.js 後端
...
### <a name="download-the-nodejs-back-end-quickstart-code-project-by-using-git"></a><a name="download-quickstart"></a>使用 Git 下載 Node.js 後端快速入門程式碼專案
...
### <a name="publish-your-nodejs-back-end-to-azure"></a><a name="howto-publish-to-azure"></a>將您的 Node.js 後端發佈至 Azure
...
### <a name="enable-a-home-page-for-your-application"></a><a name="howto-enable-homepage"></a>啟用應用程式的首頁
...
## <a name="table-operations"></a><a name="TableOperations"></a>資料表作業
...
### <a name="define-tables-by-using-a-dynamic-schema"></a><a name="howto-dynamicschema"></a>使用動態結構描述定義資料表
...
### <a name="define-tables-by-using-a-static-schema"></a><a name="howto-staticschema"></a>使用靜態結構描述定義資料表
...
### <a name="use-sql-server-express-as-a-development-data-store-on-your-local-machine"></a><a name="howto-sqlexpress-setup"></a>以 SQL Server Express 作為本機電腦上的開發資料存放區

Mobile Apps Node.js SDK 提供三種現成可用的資料提供選項：

* 使用**記憶體**驅動程式，提供非持續性的範例存放區。
* 使用 **mssql** 驅動程式，提供可供開發使用的 SQL Server Express 資料存放區。
* 使用**mssql**驅動程式提供用於生產的 Azure SQL 資料庫資料存儲。
...
1. 執行 SQL Server 2014 組態管理員：
   ...
   c. 按右鍵**TCP/IP**並選擇"**啟用**"。 在快顯對話方塊中選取 [確定]****。
...
### <a name="configure-your-project-for-local-development"></a><a name="howto-config-localdev"></a>設定專案以在本機上進行開發
...
此示例**azureMobile.js**檔實現了前面的資料庫設置：
...
我們建議您將**azureMobile.js**添加到 **.gitignore**檔（或其他原始程式碼控制忽略檔），以防止密碼存儲在雲中。 請一律在 [Azure 入口網站]內的 [應用程式設定]**** 中設定生產設定。

### <a name="configure-app-settings-for-your-mobile-app"></a><a name="howto-appsettings"></a>設定行動應用程式的應用程式設定

azureMobile.js 檔案中的大部分設定在 [Azure 入口網站]中都有對等的 [應用程式設定]。 使用以下清單在**應用設置**中配置應用：
...
### <a name="use-sql-database-as-your-production-data-store"></a><a name="howto-use-sqlazure"></a>使用 SQL Database 作為您的生產資料存放區
...
建立資料庫可能需要幾分鐘的時間。 使用 [通知] **** 區域來監視部署的進度。 在資料庫成功部署之前，請勿繼續進行。 資料庫部署完成後，將會在您 Mobile Apps 的應用程式設定中建立 SQL Database 執行個體的連接字串。 您可以在**設置** > **應用程式設定** > **連接字串**中看到此應用程式設定。

### <a name="require-authentication-for-access-to-tables"></a><a name="howto-tables-auth"></a>需經過驗證才能存取資料表
...
### <a name="use-authentication-claims-with-your-tables"></a><a name="howto-tables-getidentity"></a>透過資料表使用驗證宣告
...
### <a name="disable-access-to-specific-table-operations"></a><a name="howto-tables-disabled"></a>停用對特定資料表作業的存取
...
### <a name="adjust-the-query-that-is-used-with-table-operations"></a><a name="howto-tables-query"></a>調整與資料表作業搭配使用的查詢
...
### <a name="configure-a-soft-delete-on-a-table"></a><a name="howto-tables-softdelete"></a>設定資料表上的虛刪除
...
### <a name="seed-your-database-with-data"></a><a name="howto-tables-seeding"></a>在您的資料庫中植入資料
...
### <a name="enable-swagger-support"></a><a name="Swagger"></a>啟用 Swagger 支援
...
## <a name="push-notifications"></a><a name="push"/>推播通知
...
### <a name="send-push-notifications"></a><a name="send-push"></a>傳送推播通知
...
### <a name="send-push-notifications-to-an-authenticated-user-by-using-tags"></a><a name="push-user"></a>使用標記將推播通知傳送給已驗證的使用者
...
## <a name="custom-apis"></a><a name="CustomAPI"></a>自訂 API

### <a name="define-a-custom-api"></a><a name="howto-customapi-basic"></a>定義自訂 API
...
### <a name="require-authentication-for-access-to-a-custom-api"></a><a name="howto-customapi-auth"></a>需經過驗證才能存取自訂 API
...
### <a name="handle-large-file-uploads"></a><a name="howto-customapi-auth"></a>處理大型檔案上傳
...
### <a name="execute-custom-sql-statements"></a><a name="howto-customapi-sql"></a>執行自訂 SQL 陳述式
...
### <a name="debug-diagnose-and-troubleshoot-mobile-apps"></a><a name="howto-diagnostic-logs"></a>為 Mobile Apps 偵錯、診斷和進行疑難排解
...
[阿帕奇科爾多瓦用戶端快速啟動]: app-service-mobile-cordova-get-started.md
[iOS 用戶端快速入門]: app-service-mobile-ios-get-started.md
[Xamarin.iOS 用戶端快速啟動]: app-service-mobile-xamarin-ios-get-started.md
