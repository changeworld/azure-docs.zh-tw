...
如果您尚未設定 Azure Migrate 專案，請在新增工具之前[這麼做](how-to-add-tool-first-time.md)。 如果您已設定專案，請新增工具，如下所示：
...
10. 請檢閱**計算**中的 VM 名稱、大小、OS 磁碟類型和可用性設定 (如果有在上一個步驟中選取)。 VM 必須符合 [Azure 需求](migrate-support-matrix-vmware-migration.md#azure-vm-requirements)。
...
12. 在 [檢閱並啟動複寫] 中檢閱設定，然後按一下 [複寫] 以啟動伺服器的初始複寫。
