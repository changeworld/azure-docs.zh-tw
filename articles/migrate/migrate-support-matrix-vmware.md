...
ms.locfileid: "83849360"
---
# <a name="support-matrix-for-vmware-assessment"></a>VMware 評估的支援矩陣 

本文摘要說明當您使用 [Azure Migrate：伺服器評估](migrate-services-overview.md#azure-migrate-server-assessment-tool)工具來評估是否要將 VMware VM 遷移至 Azure 時的必要條件和支援需求。 如果您想要將 VMware VM 遷移至 Azure，請檢閱[遷移支援矩陣](migrate-support-matrix-vmware-migration.md)。

若要評估 VMware VM，請建立 Azure Migrate 專案，然後在專案中新增伺服器評估工具。 新增工具之後，請部署 [Azure Migrate 設備](migrate-appliance.md)。 設備會持續探索內部部署機器，並將機器的中繼資料和效能資料傳送至 Azure。 探索完成之後，請將探索到的機器收集到群組中，然後為群組執行評估。
...
除了探索機器外，伺服器評估也可以探索在機器上執行的應用程式、角色和功能。 探索應用程式詳細目錄可讓您找出並規劃專為內部部署工作負載量身打造的遷移路徑。 

**支援** | **詳細資料**
--- | ---
**支援的機器** | 應用程式探索目前僅支援 VMware VM。
**探索** | 應用程式探索不需要代理程式。 其會使用機器來賓認證，並使用 WMI 和 SSH 呼叫從遠端存取機器。
**VM 支援** | 應用程式探索支援所有 Windows 和 Linux 版本。
**vCenter 認證** | 應用程式探索需要具有唯讀存取權的 vCenter Server 帳戶，以及為 [虛擬機器] > [來賓作業] 啟用權限。
**VM 認證** | 應用程式探索目前支援為所有 Windows 伺服器使用一個認證，以及為所有 Linux 伺服器使用一個認證。<br/><br/> 請為 Windows VM 建立來賓使用者帳戶，以及為所有 Linux VM 建立常規/一般使用者帳戶 (非 sudo 存取權)。
**VMware 工具** | 您想要探索的 VM 上必須安裝並執行 VMware 工具。 <br/> VMware 工具版本必須為 10.2.0 之後。
...
**VMware** | **詳細資料**
--- | ---
**VMware VM** | 所有 Windows 和 Linux 作業系統都支援評估。
**vCenter Server** | 您想要探索及評估的機器必須由 vCenter Server 5.5、6.0、6.5 或 6.7 版進行管理。
**權限 (評估)** | vCenter Server 唯讀帳戶。
**權限 (應用程式探索)** | 具有唯讀存取權的 vCenter Server 帳戶，以及為 [虛擬機器] > [來賓作業] 啟用權限。
**權限 (相依性視覺效果)** | 具有唯讀存取權的 vCenter Server 帳戶，以及為 [虛擬機器] > [來賓作業] 啟用權限。
...
ESXi 主機 (應用程式探索/無代理程式相依性分析) | 如果您想要執行[應用程式探索](how-to-discover-applications.md)或[無代理程式相依性分析](concepts-dependency-visualization.md#agentless-analysis)，則設備會連線到 TCP 連接埠 443 上的 ESXi 主機、連線到探索應用程式，並在 VM 上執行無代理程式相依性視覺效果。
...
**Windows 帳戶** |  針對相依性分析，Azure Migrate 設備需要有網域系統管理員帳戶或本機系統管理員帳戶才能存取 Windows VM。
