...
本文摘要說明當您使用 [Azure Migrate：伺服器評定] （遷移-服務-總覽 # Azure-遷移-伺服器評估工具）工具來評估要遷移至 Azure 的 VMware Vm 時的必要條件和支援需求。 如果您想要將 VMware Vm 遷移至 Azure，請參閱[遷移支援矩陣](migrate-support-matrix-vmware-migration.md)。
...
**許可權（相依性視覺效果）** | 具有唯讀存取權的中央伺服器帳戶，以及針對**虛擬機器** > **來賓作業**啟用的許可權。
...
**Windows 帳戶** |  針對相依性分析，Azure Migrate 設備需要本機或網域系統管理員帳戶才能存取 Windows Vm。
