...
## <a name="what-geographies-are-supported-for-discovery-and-assessment-with-azure-migrate"></a>Azure 遷移支援哪些地理位置用於發現和評估?
...
- **Hyper-V**: 發現使用超 V 主機認證。 如果 VM 共用同一個 Hyper-V 主機,則當前無法分離發現。  
...
使用本地大小調整時,Azure 遷移不考慮 VM 性能數據進行評估。 Azure 遷移根據本地配置評估 VM 大小。 使用基於性能的尺寸調整,大小調整基於利用率數據。
...
同樣,磁碟大小調整取決於大小調整條件和儲存類型:
- 如果大小調整條件基於性能,並且存儲類型是自動的,則 Azure 遷移在標識目標磁碟類型(標準磁碟或高級磁碟)時,會考慮磁碟的 IOPS 和輸送量值。
- 如果大小調整條件基於性能,並且存儲類型為高級,則 Azure 遷移建議根據本地磁碟的大小使用高級磁碟 SKU。 當大小調整為本地大小調整且儲存類型為"標準"或"高級"時,相同的邏輯應用於磁碟大小調整。
...
對於僅針對基於性能的大小調整,Azure 遷移會收集本地電腦的性能歷史記錄,然後使用它在 Azure 中推薦 VM 大小和磁碟類型:

1. 設備持續對本地環境進行配置檔,以便每 20 秒收集一次即時利用率數據。
1. 產品將收集的 20 秒樣本匯總一次,並使用它們每 15 分鐘創建一個數據點。
1. 要創建數據點,設備將從所有 20 秒的樣本中選擇峰值。
1. 設備將數據點發送到 Azure。

### <a name="utilization"></a>利用

在 Azure 中建立評估時,根據性能持續時間和設置的性能歷史記錄百分位數值,Azure 遷移將計算有效利用率值,然後使用它進行大小調整。

例如,如果將性能持續時間設置為一天,百分位數值設置為第 95 百分位,Azure 遷移會按升序對收集器過去一天發送的 15 分鐘採樣點進行排序。 它選擇第 95 個百分位值作為有效利用。
...
基於導入的評估是使用 CSV 檔導入 Azure 遷移的計算機創建的評估。 導入只需四個字段:伺服器名稱、內核、記憶體和作業系統。 以下是需要注意的一些事項: 
 - 在基於導入的對引導類型參數的評估中,就緒條件不太嚴格。 如果未提供引導類型,則假定電腦具有 BIOS 啟動類型,並且電腦未標記為 **"有條件就緒**"。 在使用發現源作為設備進行的評估中,如果缺少啟動類型,就緒狀態將標記為 **「有條件就緒**」。 此就緒性計算差異是因為,在基於導入的評估完成時,使用者可能沒有有關遷移規劃早期階段計算機的所有資訊。 
 - 基於性能的導入評估使用使用者提供的利用率值進行正確的大小調整計算。 由於利用率值由使用者提供,因此在評估屬性中禁用**性能歷史記錄**和**百分位數利用率**選項。 在使用發現源作為設備進行評估時,所選百分位數值將從設備收集的性能數據中選取。
...
Log Analytics | 不需要。 | Azure 遷移使用[Azure 監視器日誌](https://docs.microsoft.com/azure/log-analytics/log-analytics-overview)中的[服務映射](https://docs.microsoft.com/azure/operations-management-suite/operations-management-suite-service-map)解決方案進行依賴項可視化。 [深入了解](concepts-dependency-visualization.md#agent-based-analysis)。
...
## <a name="do-i-pay-for-dependency-visualization"></a>我為依賴項可視化付費嗎?

否。 瞭解有關[Azure 遷移定價](https://azure.microsoft.com/pricing/details/azure-migrate/)的更多詳細資訊。

## <a name="what-do-i-install-for-agent-based-dependency-visualization"></a>對於基於代理的依賴項可視化,我應安裝什麼?

要使用基於代理的依賴項可視化,請在要評估的每個本地電腦上下載和安裝代理:
...
- 如果您有沒有網路連接的電腦,請下載並安裝日誌分析閘道。

僅當使用基於代理的依賴項可視化效果時,才需要這些代理。

## <a name="can-i-use-an-existing-workspace"></a>我可以使用現有的工作區嗎？

是的,對於基於代理的依賴項可視化,您可以將現有工作區附加到遷移專案,並將其用於依賴項可視化。 

## <a name="can-i-export-the-dependency-visualization-report"></a>是否可以匯出相依性視覺效果報告？

否,無法匯出基於代理的可視化中的依賴項可視化報表。 但是,Azure 遷移使用服務映射,您可以使用[服務映射 REST API](https://docs.microsoft.com/rest/api/servicemap/machines/listconnections)以 JSON 格式檢索依賴項。

## <a name="can-i-automate-agent-installation"></a>我可以自動安裝代理嗎?

對於基於代理的依賴項可視化:

- 使用[文稿安裝相依項代理](../azure-monitor/insights/vminsights-enable-hybrid-cloud.md#installation-script-examples)。
- 對 MMA,[請使用指令列或自動化](../azure-monitor/platform/log-analytics-agent.md#installation-and-configuration),或使用[文稿](https://gallery.technet.microsoft.com/scriptcenter/Install-OMS-Agent-with-2c9c99ab)。
- 除了文本之外,您還可以使用部署工具(如 Microsoft 終結點配置管理器和[Intigua)](https://www.intigua.com/getting-started-intigua-for-azure-migration)來部署代理。
...
- 檢視 MMA[支援的 Linux 作業系統](https://docs.microsoft.com/azure/log-analytics/log-analytics-concept-hybrid#supported-linux-operating-systems)的清單。

## <a name="can-i-visualize-dependencies-for-more-than-one-hour"></a>我可以可視化依賴項超過一小時嗎?

對於基於代理的可視化,您可以可視化依賴項長達一個小時。 您可以追溯到歷史記錄中的特定日期長達一個月,但可視化的最大持續時間為 1 小時。 例如,可以使用依賴項映射中的工期來查看昨天的依賴項,但只能查看一小時視窗的依賴項。 但是,可以使用 Azure 監視器日誌[查詢依賴項數據](https://docs.microsoft.com/azure/migrate/how-to-create-group-machine-dependencies)的時間較長。

對於無代理可視化,您可以查看單個伺服器的依賴項映射,持續時間為 1 小時到 30 天。

## <a name="can-i-visualize-dependencies-for-groups-of-more-than-10-vms"></a>是否可以可視化超過 10 個 VM 的組的依賴項?

您可以視覺化最多具有 10 個 VM 的組[的相依項 。](https://docs.microsoft.com/azure/migrate/how-to-create-group-dependencies) 如果組具有 10 個以上 VM,我們建議您將組拆分為較小的組,然後可視化依賴項。
