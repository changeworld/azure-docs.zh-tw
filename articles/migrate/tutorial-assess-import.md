...
**磁碟讀取作業數總計** | 否 | 所有合併磁碟的每秒讀取作業。 <br/> 如果無法提供磁碟層級的資料，請使用此欄位。 
**磁碟寫入作業數總計** | 否 | 所有合併磁碟的每秒寫入作業。 <br/> 如果無法提供磁碟層級的資料，請使用此欄位。
**磁碟讀取輸送量總計** | 否 | 從所有磁碟讀取的資料量 (MB/秒)。 <br/> 如果無法提供磁碟層級的資料，請使用此欄位。 
**磁碟寫入輸送量總計** | 否 | 寫入所有磁碟的資料量 (MB/秒)。 <br/> 如果無法提供磁碟層級的資料，請使用此欄位。
...
如果無法提供磁碟層級資料，則可以使用下欄欄位提供每部伺服器的磁碟效能資料。 如需每個欄位的詳細資訊，請參閱[本節](#add-server-information)。
- 磁碟讀取作業數總計
- 磁碟寫入作業數總計
- 磁碟讀取輸送量總計
- 磁碟寫入輸送量總計
