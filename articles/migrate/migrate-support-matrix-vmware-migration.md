...
**組合的 Nic** | 不支援。
**IPv6** | 不支援。
**目標磁片** | Vm 只能遷移至受控磁片， (在 Azure 中的標準 HDD、premium SSD) 。
...
### <a name="port-requirements-agentless"></a> (無代理程式) 的埠需求
...
### <a name="vmware-requirements-agent-based"></a> (以代理程式為基礎) 的 VMware 需求
...
**支援** | **詳細資料**
--- | ---
**機器工作負載** | Azure Migrate 支援在支援的電腦上執行任何工作負載 (例如 Active Directory、SQL server 等等 ) 。
...
### <a name="appliance-requirements-agent-based"></a>以代理程式為基礎)  (的設備需求
...
**裝置** | **[連接]**
--- | ---
VM | 在 Vm 上執行的行動服務會與內部部署複寫設備通訊， (設定伺服器) 埠 HTTPS 443 輸入，以進行複寫管理。<br/><br/> VM 會透過輸入連接埠 HTTPS 9443 將複寫資料傳送至處理伺服器 (在設定伺服器電腦上執行)。 您可以修改此連接埠。
...
複寫至 Azure 的所有內部部署 Vm （含無代理程式或代理程式型遷移) ）必須符合此表中摘要說明的 Azure VM 需求。 
