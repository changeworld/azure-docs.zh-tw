---
title: 使用 Azure Migrate 中的伺服器評量來評估 VMware VM 是否可移轉至 Azure VM
description: 了解如何使用伺服器評量來評估 VMware VM 是否可移轉至 Azure VM。
author: rashi-ms
ms.author: rajosh
ms.manager: abhemraj
ms.topic: tutorial
...
ms.locfileid: "96753139"
---
# <a name="tutorial-assess-vmware-vms-for-migration-to-azure-vms"></a>教學課程：評估 VMware VM 以移轉至 Azure VM

在您移轉至 Azure 的過程中，會評估您的內部部署工作負載，以測量雲端整備程度、找出風險，以及預估成本和複雜度。

本文說明如何使用 Azure Migrate 來評估探索到的 VMware 虛擬機器 (VM)，以準備移轉至 Azure VMware 解決方案 (AVS)：伺服器評量工具。
...
10. 在 [評估伺服器] 中，按 [下一步]。
11. 在 [選取要評估的機器] 中，選取 [新建]，然後指定群組名稱。 
12. 選取設備，然後選取您想要新增至群組的 VM。 然後按一下 [下一步]  。

     ![將 VM 新增至群組](./media/tutorial-assess-vmware-azure-vm/assess-group.png)

13. 在 [**檢閱 + 建立評量] 中，檢閱評量詳細資料，然後按一下 [建立評量] 以建立群組並執行評量。
