...
開始進行本教學課程之前，請 [檢閱](migrate-architecture.md) VMware 代理程式型移轉架構。
...
如果您還沒有 Azure Migrate 專案，請立即[加以設定](how-to-add-tool-first-time.md)，然後新增伺服器移轉工具。
...
12. 在 [計算] 中，檢閱 VM 名稱、大小、OS 磁碟類型和可用性設定組。 VM 必須符合 [Azure 需求](migrate-support-matrix-vmware-migration.md#azure-vm-requirements)。
