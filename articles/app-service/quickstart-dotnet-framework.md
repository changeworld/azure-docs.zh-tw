...
3. 在 [設定新專案] 中，將應用程式命名為 myFirstAzureWebApp，然後選取 [建立]。
   ...
   ![設定 Web 應用程式專案](./media/quickstart-dotnet-framework/configure-web-app-project-framework.png)
...
6. 從 Visual Studio 功能表中，選取 **[偵錯]**  >  **[啟動但不偵錯]** 以在本機執行 Web 應用程式。
   ...
   ![在本機執行應用程式](./media/quickstart-dotnet-framework/local-web-app.png)

## <a name="publish-your-web-app"></a>發佈 Web 應用程式<a name="launch-the-publish-wizard"></a>
...
1. 在 [設定主控方案] 對話方塊中，輸入下表中的值，然後選取 [確定]。
   ...
   | 設定 | 建議的值 | 描述 |
   |-|-|-|
   | 主控方案| myAppServicePlan | App Service 方案的名稱。 |
   ...
   ![建立 App Service 方案](./media/quickstart-dotnet-framework/app-service-plan-framework-vs2019.png)
...
3. 在 [發佈] 頁面中，按一下 [發佈]。 Visual Studio 會建立和封裝應用程式，並將其發佈至 Azure，然後在預設瀏覽器中啟動應用程式。
    ...
    ![Azure 中已發佈的 ASP.NET Web 應用程式](./media/quickstart-dotnet-framework/published-azure-web-app.png)

在 [App Service 建立新的] 頁面中指定的應用程式名稱，會作為格式 `http://<app-name>.azurewebsites.net` 中的 URL 前置詞。
...
1. 若要重新部署至 Azure，請在 [方案總管]  中，以滑鼠右鍵按一下 **myFirstAzureWebApp** 專案，然後選取 [發佈]  。 然後，選取 [發佈]。
    ...
    ![Azure 中已更新的 ASP.NET Web 應用程式](./media/quickstart-dotnet-framework/updated-azure-web-app.png)

## <a name="manage-the-azure-app"></a>管理 Azure 應用程式

1. 若要管理 Web 應用程式，請移至 [Azure 入口網站](https://portal.azure.com)，然後搜尋並選取 [應用程式服務]。

   ![選取應用程式服務](./media/quickstart-dotnet-framework/app-services.png)

2. 在 [應用程式服務]  頁面上，選取您的 Web 應用程式名稱。

   ![入口網站瀏覽至 Azure 應用程式](./media/quickstart-dotnet-framework/access-portal-framework-vs2019.png)

   您會看到 Web 應用程式的 [概觀] 頁面。 您可以在這裡進行基本管理，像是瀏覽、停止、啟動、重新啟動及刪除。

   ![Azure 入口網站中的 App Service 概觀](./media/quickstart-dotnet-framework/web-app-general-framework-vs2019.png)
