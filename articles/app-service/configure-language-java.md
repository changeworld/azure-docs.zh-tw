...
ms.locfileid: "80383968"
---
# <a name="configure-a-windows-java-app-for-azure-app-service"></a>為 Azure 應用服務配置 Windows JAVA 應用

Azure 應用服務允許 JAVA 開發人員在完全託管的基於 Windows 的服務上快速構建、部署和擴展其 Tomcat Web 應用程式。 從命令列或編輯器 (例如 IntelliJ、Eclipse 或 Visual Studio Code)，使用 Maven 外掛程式部署應用程式。
...
您可以使用[Maven 的 Azure Web 應用外掛程式](/java/api/overview/azure/maven/azure-webapp-maven-plugin/readme)來部署 .war 檔。 [用於 IntelliJ](/java/azure/intellij/azure-toolkit-for-intellij)的 Azure 工具組或[用於 Eclipse 的 Azure 工具組](/java/azure/eclipse/azure-toolkit-for-eclipse)也支援使用流行的 ID 支援部署。
...
- 要將 .jar 檔部署到 JAVA SE，請使用 Kudu 網站的`/api/zipdeploy/`終結點。 如需有關此 API 的詳細資訊，請參閱[這份文件](https://docs.microsoft.com/azure/app-service/deploy-zip#rest)。
...
使用 Azul JVM 的應用程式服務上的所有 JAVA 運行時都配有祖魯飛行記錄器。 您可以使用它記錄 JVM、系統和 JAVA 級別事件，以監視 JAVA 應用程式中的行為和疑難排解問題。

要進行時序錄製，您需要 JAVA 應用程式的 PID（進程 ID）。 要查找 PID，請以 HTTPs：//<網站名稱>.scm.azure 網站.net/ProcessExplorer/）打開 Web 應用的 SCM 網站的瀏覽器。 此頁顯示 Web 應用中的運行進程。 在表中查找名為"java"的進程並複製相應的 PID（進程 ID）。

接下來，在 SCM 網站的頂部工具列中打開**調試主控台**並運行以下命令。 替換為`<pid>`前面複製的進程 ID。 此命令將啟動 JAVA 應用程式的 30 秒探測器錄製，並生成`timed_recording_example.jfr`目錄中名為`D:\home`的檔。
...
#### <a name="analyze-jfr-files"></a>分析`.jfr`檔
...
Azure 應用服務支援通過 Azure 門戶和 CLI 進行開箱即用的調優和自訂。 查看以下有關非 JAVA 特定 Web 應用配置的文章：

- [進行應用程式設定](configure-common.md#configure-app-settings)
- [設定自訂網域](app-service-web-tutorial-custom-domain.md)
- [配置 SSL 綁定](configure-ssl-bindings.md)
- [新增 CDN](../cdn/cdn-add-to-web-app.md)
- [配置庫杜網站](https://github.com/projectkudu/kudu/wiki/Configurable-settings)

### <a name="set-java-runtime-options"></a>設定 Java 執行階段選項

要設置分配的記憶體或其他 JVM 運行時選項，請創建一個應用`JAVA_OPTS`設置，該[設置](configure-common.md#configure-app-settings)名為這些選項。 應用服務在啟動時將此設置作為環境變數傳遞給 JAVA 運行時。
...
按照[Azure 應用服務中帶有 SSL 綁定的自訂 DNS 名稱](configure-ssl-bindings.md)中的說明進行操作，上載現有 SSL 憑證並將其綁定到應用程式的功能變數名稱。 根據預設，您的應用程式仍然允許 HTTP 連線；請遵循教學課程中的特定步驟，以強制執行 SSL 和 TLS。
...
1. 在[NewRelic.com](https://newrelic.com/signup)創建新的遺物帳戶
2. 從NewRelic下載JAVA代理，它將有一個類似于*newrelic-java-x.x.x.zip*的檔案名。
3. 複製您的授權金鑰，以供稍後設定代理程式時使用。
4. 使用[Kudu 主控台](https://github.com/projectkudu/kudu/wiki/Kudu-console)創建新的目錄 */home/網站/wwwroot/apm*。
5. 上傳解壓縮的新文物JAVA代理檔到目錄下 */home/網站/wwwroot/apm*. 您的代理的檔應位於 */home/網站/wwwroot/apm/newrelic。*
...
3. 使用[Kudu 主控台](https://github.com/projectkudu/kudu/wiki/Kudu-console)創建新的目錄 */home/網站/wwwroot/apm*。
...
最後，我們將將驅動程式 JAR 放在 Tomcat 類路徑中，然後重新開機應用服務。 通過將 JDBC 驅動程式檔放在 */home/tomcat/lib*目錄中，確保它們可供 Tomcat 類載入器使用。 （如果此目錄不存在，請創建該目錄。要將這些檔上載到應用服務實例，請執行以下步驟：
...
## <a name="configuring-tomcat"></a>配置湯姆卡特

要編輯 Tomcat`server.xml`或其他設定檔，請先在門戶中記下 Tomcat 的主要版本。

1. 通過運行`env`該命令查找版本的 Tomcat 主目錄。 搜索以`AZURE_TOMCAT`主要版本開頭並匹配的環境變數。 例如，`AZURE_TOMCAT85_HOME`指向 Tomcat 8.5 的 Tomcat 目錄。
1. 為版本標識 Tomcat 主目錄後，請將配置目錄複寫到`D:\home`。 例如，如果`AZURE_TOMCAT85_HOME`值 為`D:\Program Files (x86)\apache-tomcat-8.5.37`，則複製目錄的新路徑將為`D:\home\apache-tomcat-8.5.37`。

最後，重新啟動 App Service。 您的部署應該和以前`D:\home\site\wwwroot\webapps`一樣。

## <a name="configure-java-se"></a>配置 JAVA SE

運行 時。Windows 上的 JAVA SE`server.port`上的 JAR 應用程式在應用程式啟動時作為命令列選項傳遞。 可以從環境變數 手動解析 HTTP 埠`HTTP_PLATFORM_PORT`。 此環境變數的值將是應用程式應偵聽的 HTTP 埠。 
...
主要版本更新將通過 Windows Azure 應用服務中的新運行時選項提供。 客戶會更新為設定其 App Service 部署的這些新版 Java，以及負責測試並確定主要更新符合其需求。

每年 1 月、4 月、7 月和 10 月都會以季為單位自動修補支援的 JDK。 有關 Azure 上的 JAVA 的詳細資訊，請參閱[此支援文檔](https://docs.microsoft.com/azure/java/jdk/)。
...
在為 Azure 或[Azure 堆疊](https://azure.microsoft.com/overview/azure-stack/)開發具有合格的 Azure 支援計畫時，可通過 Microsoft 獲得[Azure 支援的 Azul Zulu JDK](https://www.azul.com/downloads/azure-only/zulu/) [的產品支援](https://azure.microsoft.com/support/plans/)。
