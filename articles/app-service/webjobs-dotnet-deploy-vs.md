---
title: 使用 VS 開發和部署 Web 作業
description: 瞭解如何在視覺化工作室中開發 Azure Web 作業並將其部署到 Azure 應用服務，包括創建計畫任務。
author: ggailey777
...
本文介紹如何使用 Visual Studio 將主控台應用程式專案作為[Azure WebJob](https://go.microsoft.com/fwlink/?LinkId=390226)將主控台應用程式專案部署到[應用服務](overview.md)中的 Web 應用。 如需如何使用 [Azure 入口網站](https://portal.azure.com)部署 WebJob 的相關資訊，請參閱[使用 WebJob 執行背景工作](webjobs-create.md)。

您可以將多個 Web 作業發佈到單個 Web 應用。 確保 Web 應用中的每個 WebJob 都有唯一的名稱。
...
## <a name="webjobs-as-net-core-console-apps"></a>Web作業作為 .NET 核心主控台應用

使用 Web 作業的版本 3.x 時，您可以創建 Web 作業並將其發佈為 .NET 核心主控台應用。 有關創建 .NET Core 主控台應用程式並將其發佈到 Azure 作為 WebJob 的分步說明，請參閱[Azure Web作業 SDK 開始進行事件驅動的幕後處理](webjobs-sdk-get-started.md)。

> [!NOTE]
> .NET 核心 Web 作業不能與 Web 專案連結。 如果需要使用 Web 應用部署 WebJob，則應[將 WebJob 創建為 .NET 框架主控台應用](#webjobs-as-net-framework-console-apps)。  

### <a name="deploy-to-azure-app-service"></a>部署到 Azure App Service

從 Visual Studio 向應用服務發佈 .NET 核心 WebJob 使用與發佈ASP.NET核心應用相同的工具。

[!INCLUDE [webjobs-publish-net-core](../../includes/webjobs-publish-net-core.md)] 

### <a name="webjob-types"></a>WebJob 類型

預設情況下，從 .NET Core 主控台專案發佈的 WebJob 僅在觸發或按需運行時運行。 您還可以更新專案以[按計劃運行](#scheduled-execution)或連續運行。

[!INCLUDE [webjobs-alwayson-note](../../includes/webjobs-always-on-note.md)]

#### <a name="scheduled-execution"></a>計畫執行

將 .NET Core 主控台應用程式發佈到 Azure 時，將添加新*的設置.job*檔添加到專案中。 使用此檔可為 WebJob 設置執行計畫。 有關詳細資訊，請參閱[計畫觸發的 WebJob](#scheduling-a-triggered-webjob)。

#### <a name="continuous-execution"></a>連續執行

您可以使用 Visual Studio 將 WebJob 更改為在 Azure 中啟用"始終打開"時連續運行。

1. 如果尚未這樣做，則[將專案發佈到 Azure](#deploy-to-azure-app-service)。

1. 在 [方案總管]**** 中，以滑鼠右鍵按一下專案並選取 [發佈]****。

1. 在 **"發佈"** 選項卡中，選擇 **"設置**"。 

1. 在 **"設定檔設置"** 對話方塊中，為**Web 作業類型**選擇 **"連續"，** 然後選擇 **"保存**"。

    ![發佈 Web 作業的設置對話方塊](./media/webjobs-dotnet-deploy-vs/publish-settings.png)

1. 選擇 **"發佈"** 以使用更新的設置重新發佈 WebJob。

## <a name="webjobs-as-net-framework-console-apps"></a>Web作業作為 .NET 框架主控台應用  

當 Visual Studio 部署啟用 WebJobs 的 .NET 框架主控台應用程式專案時，它會將運行時檔案複製到 Web 應用中的相應資料夾（連續 Web 作業*App_Data/作業/連續*進行，並且為計畫或按需 Web 作業*App_Data/作業/觸發*）。
...
* [將 Web作業新專案範本用於獨立的 Web 作業](#createnolink)
...
* [對連結到 Web 專案的 Web 作業使用 Web 作業新專案範本](#createlink)
...
1. 按一下 **"檔** > **新專案**"，然後在 **"新專案**"對話方塊中按一下**雲** > **Azure Web 作業 （.NET 框架）。**
...
### <a name="the-add-azure-webjob-dialog"></a><a id="configure"></a>添加 Azure Web 作業對話方塊
...
> * 如果部署 WebJob，然後決定要更改 WebJob 和重新部署的類型，則需要刪除*Webjob-發佈設置.json*檔。 這樣會讓 Visual Studio 再次顯示發佈選項，您才能夠變更 WebJob 的類型。
...
您可以編輯此檔案目錄，而 Visual Studio 會提供 IntelliSense。 檔架構存儲在，[https://schemastore.org](https://schemastore.org/schemas/json/webjob-publish-settings.json)並可在那裡查看。  
...
您可以編輯此檔案目錄，而 Visual Studio 會提供 IntelliSense。 檔架構存儲在，[https://schemastore.org](https://schemastore.org/schemas/json/webjobs-list.json)並可在那裡查看。
...
WebJob 使用*設置.job*檔來確定 Web 作業何時運行。 使用此檔可為 WebJob 設置執行計畫。 以下示例每小時從上午 9 點到下午 5 點運行：
...
此檔必須位於 WebJob 資料夾的根目錄，沿 WebJob 的腳本（如`wwwroot\app_data\jobs\triggered\{job name}`或`wwwroot\app_data\jobs\continuous\{job name}`） 當您從 Visual Studio 部署 WebJob 時，請將您的 `settings.job` 檔案屬性標示為 [有更新時才複製]****。 

從[Azure 門戶創建 Web 作業](webjobs-create.md)時，將為您創建設置.job 檔。
...
### <a name="settingjob-reference"></a>設置.作業引用

Web 作業支援以下設置：

| **設定** | **類型**  | **描述** |
| ----------- | --------- | --------------- |
| `is_in_place` | 全部 | 允許作業在未首先複製到暫存檔案夾的情況下就地運行。 要瞭解更多資訊，請參閱[Web 作業工作目錄](https://github.com/projectkudu/kudu/wiki/WebJobs#webjob-working-directory)。 |
| `is_singleton` | 連續 | 僅當橫向擴展時，僅在單個實例上運行 Web 作業。要瞭解更多資訊，請參閱[將連續作業設置為單例](https://github.com/projectkudu/kudu/wiki/WebJobs-API#set-a-continuous-job-as-singleton)。 |
