...
當您使用[Azure 流量管理員](/azure/traffic-manager/)對[Azure App Service](overview.md)的流量進行負載平衡時，可以使用** \<流量管理員-端點>** 來存取 App Service 應用程式。 您可以使用 App Service 應用程式來指派自訂功能變數名稱\.（例如 www contoso.com），以便為您的使用者提供更容易辨識的功能變數名稱。
