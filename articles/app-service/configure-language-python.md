...
在您部署 [Git 存放庫](deploy-local-git.md)或 [Zip 套件](deploy-zip.md) (在開啟建置程序的情況下) 時，App Service 部署引擎會自動啟動虛擬環境，並為您執行 `pip install -r requirements.txt`。
...
在 App Service 中，您可以於應用程式的程式碼外部[設定應用程式設定](configure-common.md#configure-app-settings)。 然後，您可以使用標準的 [os.environ](https://docs.python.org/3/library/os.html#os.environ) 模式來存取這些設定。 例如，若要存取稱為 `WEBSITE_SITE_NAME` 的應用程式設定，請使用下列程式碼：
