---
title: 使用流量管理員控制流量
description: 尋找與 Azure 應用服務整合時配置 Azure 流量管理器的最佳做法。
...
要配置應用服務應用流量的控制,請在 Azure 流量管理器中創建一個配置檔,該配置檔使用前面描述的四種負載平衡方法之一,然後將要控制流量的終結點(本例中稱為應用服務)添加到配置檔中。 系統會定期與設定檔溝通您的應用程式狀態 (執行中、已停止或已刪除)，讓 Azure 流量管理員可相應地導向流量。
...
* 您的自訂網域名稱會如預期般運作，但除了將之新增至應用程式外，您還必須設定 DNS 對應以指向流量管理員 URL。 有關如何為應用服務應用設定自訂網域的資訊,請參閱[在 Azure 應用服務中使用流量管理員整合設定自訂網域名稱](configure-domain-traffic-manager.md)。
* 您只能將標準或進階模式的應用程式新增至 Azure 流量管理員設定檔。
* 將應用添加到流量管理器配置檔會導致重新啟動應用。
