...
App Service 資源是區域特定的，無法跨區域移動。 您必須在目的地區域中建立現有 App Service 資源的複本，然後將您的內容移到新的應用程式。 如果您的來源應用程式使用自訂網域，當您完成時，可以將[它遷移至目的地區域中的新應用程式](manage-custom-dns-migrate-domain.md)。
