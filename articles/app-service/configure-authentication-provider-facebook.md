...
1. 在 [**有效的 OAuth 重新導向 uri** ] `https://<app-name>.azurewebsites.net/.auth/login/facebook/callback`欄位中，輸入。 請記得將`<app-name>`取代為您的 Azure App Service 應用程式名稱。
...
1. 您用來註冊應用程式的 Facebook 帳戶是應用程式的系統管理員。 此時，只有系統管理員可以登入此應用程式。
   ...
   若要驗證其他 Facebook 帳戶，請選取 [**應用程式審查**]，並啟用 [將**您的應用程式> 名稱設\<為公用**]，讓一般大眾能夠使用 Facebook 驗證來存取應用程式。
