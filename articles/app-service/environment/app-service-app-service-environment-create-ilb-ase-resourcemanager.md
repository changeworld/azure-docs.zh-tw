---
title: 創建 ILB ASE v1
description: 使用內部負載等化器 （ILB ASE） 創建應用服務環境。 此文檔僅提供給使用舊版 v1 ASE 的客戶。
author: stefsch
...
3. 上傳的 SSL 憑證會明確指派給 ILB ASE 做為其「預設」SSL 憑證。  當應用使用分配給 ASE 的常見根域定址時，此 SSL 憑證將用於 ILB ASE 上應用的 SSL 流量（例如`https://someapp.mycustomrootcomain.com`）
...
* *內部負載平衡模式*：在大多數情況下，將其設置為 3，這意味著埠 80/443 上的 HTTP/HTTPS 流量以及 ASE 上的 FTP 服務偵聽的控制/資料通道埠都將綁定到 ILB 分配的虛擬網路內部位址。  如果此屬性改為設定為 2，則只有 FTP 服務相關的連接埠 (控制和資料通道) 會繫結至 ILB 位址，而 HTTP/HTTPS 流量將保留在公用 VIP 上。
...
* *密碼*：用於保護 .pfx 檔的密碼。
* certificateThumbprint**︰憑證的指紋。  如果您從 Powershell 擷取此值 (例如先前程式碼片段中的 $certificate.Thumbprint ** )，您可以使用現況值。  不過，如果您從 Windows 憑證對話方塊複製此值，請記得去除多餘的空格。  *證書指紋*應如下所示： AF3143EB61D43F6727842115BB7F17BCECAAE
...
完成範本後，可透過 HTTPS 存取 ILB ASE 上的應用程式，並使用預設 SSL 憑證保護連線。  如果 ILB ASE 上的應用程式是使用應用程式名稱加上預設主機名稱的組合來定址，則會使用預設 SSL 憑證。  例如*https://mycustomapp.internal-contoso.com*，將使用 =*.internal-contoso.com 的*預設 SSL 憑證。
