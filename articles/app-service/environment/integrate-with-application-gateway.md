---
title: 與應用程式閘道整合
description: 瞭解如何在此端到端演練中將 ILB 應用服務環境中的應用與應用程式閘道集成。
author: ccompy
...
* ILB App Service Environment 使用的 ILB 位址。 此資訊位於 **"設置** > **IP 位址**"下的"應用服務環境"門戶中：
...
9. 在 ILB App Service Environment 中設定應用程式的自訂網域名稱。 轉到門戶中的應用，並在 **"設置"** 下選擇 **"自訂域**"。
