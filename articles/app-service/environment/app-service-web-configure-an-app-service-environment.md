...
**背景工作**：背景工作是您的應用程式實際執行之處。 當您擴大 App Service 方案時，會用完相關聯背景工作集區中的背景工作。
...
* 將背景工作角色數量縮小為 2。  入口網站中縮小大小最小值為 2。 這將需要幾分鐘時間來解除配置您的執行個體。 
...
* 如果您不再需要那些未使用的執行個體，請縮小第一個背景工作集區。 此作業需要幾分鐘才能完成。
