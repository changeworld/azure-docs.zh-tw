...
![根據「自動調整 - 擴大」規則的工作日 App Service 方案擴大率。][Equation1]

如果是生產 App Service 方案的週末設定檔的「自動調整 - 擴大」規則，公式會解析為：

![根據「自動調整 - 擴大」規則的週末 App Service 方案擴大率。][Equation2]

此值也可以針對相應減少作業計算。

根據生產 App Service 方案的工作日設定檔的「自動調整 - 縮小」規則，這看起來會是如下：

![根據「自動調整 - 縮小」規則的工作日 App Service 方案擴增率。][Equation3]

如果是生產 App Service 方案的週末設定檔的「自動調整 - 縮小」規則，公式會解析為：  

![根據「自動調整 - 縮小」規則的週末 App Service 方案擴增率。][Equation4]
