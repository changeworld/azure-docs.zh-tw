...
在外部 ASE 的情況下，您可以將 IP 位址指派給個別的應用程式。 您無法在 ILB ASE 的情況下那樣做。 如需如何設定您的應用程式以擁有自己的 IP 位址的詳細資訊，請參閱[Azure App Service 中的使用 TLS/SSL 系結保護自訂 DNS 名稱](../configure-ssl-bindings.md)。
