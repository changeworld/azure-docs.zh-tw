---
title: 管理位址
description: 查找用於控制應用服務環境的管理位址。 在路由表中配置它們以避免非對稱路由問題。
author: ccompy
...
應用服務環境 （ASE） 是在 Azure 虛擬網路 （VNet） 中運行的 Azure 應用服務的單個租戶部署。  雖然 ASE 確實在 VNet 中運行，但它仍必須從 Azure 應用服務用於管理服務的多個私人 IP 位址訪問它。  在 ASE 中，管理流量遍歷使用者控制的網路。 如果封鎖或誤傳此流量，ASE 會暫止。 有關 ASE 網路依賴項的詳細資訊，請閱讀[網路注意事項和應用服務環境][networking]。 如需 ASE 的一般資訊，您可以從 [App Service Environment 簡介][intro]開始。

所有 ASE 都有可讓管理流量進入的公用 VIP。 這些位址的連入管理流量會進入 ASE 公用 VIP 上的連接埠 454 和 455。 本文件列出流向 ASE 之管理流量的 App Service 來源位址。 這些位址也位於名為 AppServiceManagement 的 IP 服務標記中。

下面所述的位址可以在路由表中配置，以避免管理流量出現非對稱路由問題。 路由在 IP 級別對流量進行操作，並且沒有流量方向的感知，或者流量是 TCP 回復消息的一部分。 如果 TCP 請求的回復位址與發送到的位址不同，則存在非對稱路由問題。 為了避免 ASE 管理流量出現非對稱路由問題，您需要確保從發送到的同一位址發送回復。 如需深入了解如何設定 ASE，以便在由內部部署軟體傳送輸出流量的環境中運作，請參閱[為您的 ASE 設定強制通道][forcedtunnel]
...
使用網路安全性群組，您無需擔心各個位址或維護自己的配置。 有一個名為 AppServiceManagement 的 IP 服務標記，該標記與所有位址保持最新。 要在 NSG 中使用此 IP 服務標記，請轉到門戶，打開網路安全性群組 UI，然後選擇"入站安全規則"。 如果您有入站管理流量的預先存在的規則，請對其進行編輯。 如果未使用 ASE 建立此 NSG，或者該 NSG 是全新的，則選取 [新增]****。 在來源下拉式清單中，選取 [服務標籤]****。  在"源服務"標記下，選擇 **"應用程式服務管理**"。 將來源連接埠範圍設為 \*、目的地設為 **Any**、目的地連接埠範圍設為 **454-455**、通訊協定設為 **TCP**，以及將動作設為 **Allow**。 如果要制定規則，則需要設定優先權。 
