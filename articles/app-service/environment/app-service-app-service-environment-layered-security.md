---
title: 多層式安全性 v1
description: 瞭解如何在 App Service 環境中執行多層式安全性架構。 本文件僅提供給使用舊版 v1 ASE 的客戶。
author: stefsch
...
得知輸出 IP 位址集後，下一步是要建構網路安全群組。  您可以針對以 Resource Manager 為基礎的虛擬網路以及傳統虛擬網路建立網路安全性群組。  下列範例顯示如何使用 PowerShell 在傳統虛擬網路上建立和設定 NSG。
...
關於 [網路安全性群組](../../virtual-network/network-security-groups-overview.md)的資訊。
