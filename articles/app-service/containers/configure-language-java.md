...
description: 瞭解如何為您的應用程式設定預先建置的 Java 容器。 本文說明最常見的設定工作。
keywords: azure app 服務, web 應用程式, linux, oss, java, java ee, jee, javaee
author: bmitchell287
manager: barbkess
ms.devlang: java
ms.topic: article
...
>[!NOTE]
>在 12 小時後記錄到本機 App Service 檔案系統會停用的功能，僅適用於以 Windows 為基礎的應用程式服務。 僅能使用 [Azure 監視器 (預覽)](/azure/app-service/troubleshoot-diagnostic-logs#send-logs-to-azure-monitor-preview) 來設定以 Linux 為基礎應用程式服務的 Azure Blob 儲存體記錄 
...
若要在 Tomcat 和 Java SE 環境中設定已配置的記憶體或其他 JVM 執行階段選項，請建立[應用程式設定](../configure-common.md?toc=/azure/app-service/containers/toc.json#configure-app-settings)，以這些選項命名為 `JAVA_OPTS`。 App Service Linux 會在啟動時將此設定當成環境變數傳遞至 Java 執行階段。
...
如果您要部署 JAR 應用程式，應將其命名為 *app.jar*，讓內建映像能夠正確地識別應用程式。 (Maven 外掛程式會自動執行此重新命名)。如果您不想要將 JAR 重新命名為 *app.jar*，則可以使用命令上傳殼層指令碼來執行 JAR。 然後，在入口網站的 [設定] 區段中，將此指令碼的完整路徑貼到 [[啟動檔案](app-service-linux-faq.md#built-in-images)] 文字方塊中。 啟動指令碼不會從其放置所在的目錄來執行。 因此，請一律使用絕對路徑在啟動指令碼中參考檔案 (例如： `java -jar /home/myapp/myapp.jar`)。
...
請遵循[使用 Azure App Service 中的 SSL 繫結保護自訂 DNS 名稱](../configure-ssl-bindings.md?toc=/azure/app-service/containers/toc.json)中的指示，上傳現有的 SSL 憑證，並將其繫結至您應用程式的網域名稱。 根據預設，您的應用程式仍然允許 HTTP 連線；請遵循教學課程中的特定步驟，以強制執行 SSL 和 TLS。
...
您可以[開啟 App Service 的 SSH 連線](app-service-linux-ssh-support.md)，然後執行命令 `keytool`，來互動或偵錯 JAVA 金鑰工具。 如需命令清單，請參閱[金鑰工具文件](https://docs.oracle.com/javase/8/docs/technotes/tools/unix/keytool.html)。 如需金鑰儲存區 API 的詳細資訊，請參閱[官方文件](https://docs.oracle.com/javase/8/docs/api/java/security/KeyStore.html)。
...
3. [透過 SSH 連線到您的 App Service 執行個體](app-service-linux-ssh-support.md)，然後建立新目錄 */home/site/wwwroot/apm*。
...
如果要為 JAR 使用不同的名稱，您必須另外提供執行 JAR 檔案的[啟動命令](app-service-linux-faq.md#built-in-images)。 例如： `java -jar my-jar-app.jar` 。 您可以在入口網站中的 [設定] > [一般設定] 底下，或使用名為 `STARTUP_COMMAND` 的應用程式設定，來設定啟動命令的值。
...
新增共用的伺服器層級資料來源，會要求您編輯 Tomcat 的 server .xml。 首先，上傳[啟動指令碼](app-service-linux-faq.md#built-in-images)，並在 [設定] > [啟動命令] 中設定指令碼的路徑。 您可以使用 [FTP](../deploy-ftp.md) 上傳啟動指令碼。
...
您可以設定讓 Tomcat 使用外部工作階段存放區，例如 [Azure Redis 快取](/azure/azure-cache-for-redis/)。 這可讓您在使用者轉移至應用程式的另一個執行個體時 (例如在發生自動調整、重新啟動或容錯移轉的情況下)，保留使用者工作階段狀態 (例如購物車資料)。
...
3. 使用 FTP 將工作階段管理員的 JAR 檔案上傳到 App Service 執行個體，並將它放在 */home/tomcat/lib* 目錄中。 如需詳細資訊，請參閱[使用 FTP/S 將您的應用程式部署至 Azure App Service](https://docs.microsoft.com/azure/app-service/deploy-ftp)。
