...
> [!NOTE]
> 如果在創建 Avere vFXT 群集時未創建新的 Blob 容器，請按照["配置存儲"](avere-vfxt-add-storage.md)中所述添加存儲系統，然後再嘗試裝載用戶端。

在您的用戶端上，``mount`` 命令會將 vFXT 叢集上的虛擬伺服器 (vserver) 對應至本機檔案系統上的路徑。 其格式為 ``mount <vFXT path> <local path> {options}``

裝載命令有三個元素：

* vFXT 路徑 - 群集 9 所述群集上的 IP 位址和命名空間交匯點路徑的組合）
* 本機路徑 - 用戶端上的路徑
* 裝載命令選項 - 在["裝載"命令參數中列出](#mount-command-arguments)

### <a name="junction-and-ip"></a>連接點和 IP

vserver 路徑由其 *IP 位址*和*命名空間連接點*的路徑組合而成。 命名空間連接點是在新增儲存體系統時所定義的虛擬路徑。

如果群集是使用 Blob 存儲創建的，則該容器的命名空間路徑為`/msazure`

範例： ``mount 10.0.0.12:/msazure /mnt/vfxt``

如果在創建群集後添加了存儲，則命名空間交匯點路徑是在創建交匯點時在**命名空間路徑**中設置的值。 例如，如果您使用 ``/avere/files`` 作為命名空間路徑，則用戶端會將 *IP_address*:/ avere/files 掛接至其本機掛接點。
...
安裝用戶端後，可以使用它們將資料複製到群集上的新 Blob 存儲容器。 如果不需要填充新存儲，請閱讀其他連結以瞭解其他設置任務：

* [將資料移動到群集核心檔程式](avere-vfxt-data-ingest.md)- 如何使用多個用戶端和執行緒有效地將資料上載到新的核心檔案伺服器
