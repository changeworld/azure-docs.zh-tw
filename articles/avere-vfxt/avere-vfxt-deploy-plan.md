...
* 群集節點需要多大的強大功能來支援您的工作流?

繼續閱讀以深入了解。

## <a name="learn-the-components-of-the-system"></a>瞭解系統元件

在開始規劃時,瞭解 Azure 系統的 Avere vFXT 的元件會很有説明。

* 叢集節點 - 群集由配置為群集節點的三個或多個 VM 組成。 節點越多,系統輸送量就越高,緩存也更大。

* 快取 - 緩存容量在群集節點之間平均分配。 創建群集時設置每個節點的緩存大小;節點大小將添加為總緩存大小。

* 叢集控制器 -群集控制器是位於與群集節點相同的子網內的其他 VM。 創建群集和持續管理任務需要控制器。

* 後端儲存 - 要快取的資料長期儲存在硬體儲存系統或 Azure Blob 容器中。 可以為 Azure 群集創建 Avere vFXT 後添加存儲,或者透過使用 Blob 儲存可以在創建叢集時添加和配置容器。

* 用戶端 - 使用快取檔案的用戶端電腦使用虛擬檔案路徑連接到群集,而不是直接存取儲存系統。 (在["Avere vFXT"群集中](avere-vfxt-mount-clients.md)閱讀更多內容。

## <a name="subscription-resource-group-and-network-infrastructure"></a>訂閱、資源群組和網路基礎結構
...
* 一起查找 vFXT 群集和群集控制器 VM - 具體來說,它們應該是:
...
* 使用群集創建範本為群集創建所需的大多數基礎結構資源,包括資源組、虛擬網路、子網和存儲帳戶。
  ...
  |----------|-----------|----------|
  | 資源群組 | 是,如果為空 | 必須為空|
  | 儲存體帳戶 | 如果叢集建立後連線現有 Blob 容器,**則為** <br/>  如果在群組建立期間建立新的 Blob 容器,**則否** | 現有 Blob 容器必須為空 <br/> &nbsp; |
  | 虛擬網路 | 是 | 如果建立新的 Azure Blob 容器,則必須包括儲存服務終結點 |
  | 子網路 | 是 | 無法包含其他資源 |
...
* 一個叢集管理 IP 位址。 此位址可以根據需要從群集中的節點移動到一個節點,以便始終可用。 使用此位址可連接到 Avere 控制面板設定工具。
* 對於每個叢集節點：
  * 至少有一個面向用戶端的 IP 位址  (所有面向用戶端的位址都由群集的*vserver*管理,它可以根據需要在節點之間移動 IP 位址。
  * 一個用於叢集通訊的 IP 位址
  * 一個執行個體 IP 位址 (指派給 VM)

如果使用 Azure Blob 儲存,它可能還需要叢集虛擬網路中的 IP 位址:  

* Azure Blob 儲存體帳戶必須至少有五個 IP 位址。 如果找到與群集相同的虛擬網路中的 Blob 儲存,請記住此要求。
* 如果使用群集虛擬網路外的 Azure Blob 儲存,請在虛擬網路內創建儲存服務終結點。 終結點不使用 IP 位址。
...
有關這些 VM 的其他資訊,請閱讀 Microsoft Azure 文件:[記憶體優化的虛擬機器大小](https://docs.microsoft.com/azure/virtual-machines/windows/sizes-memory)
...
後端儲存系統既向群集的快取提供檔,也從快取接收更改的資料。 決定您的工作集是長期存儲在新的 Blob 容器中還是存儲在現有儲存系統(雲端或硬體)中。 這些後端儲存系統稱為*核心檔案伺服器*。

### <a name="hardware-core-filers"></a>硬體核心檔案伺服器

創建群集后,將硬體存儲系統添加到 vFXT 群集。 只要可以從群集的子網到達存儲系統,就可以使用各種流行的硬體系統,包括本地系統。

如需有關如何將現有的儲存體系統新增至 Avere vFXT 叢集的詳細指示，請閱讀[設定儲存體](avere-vfxt-add-storage.md)。

### <a name="cloud-core-filers"></a>雲核心檔案伺服器

Azure 系統的 Avere vFXT 可以使用空 Blob 容器進行後端存儲。 容器在添加到群集時必須為空 - vFXT 系統必須能夠管理其物件存儲,而無需保留現有數據。

> [!TIP]
> 如果要對後端使用 Azure Blob 儲存,請建立一個新容器作為創建 vFXT 群集的一部分。 叢集創建範本可以建立和配置新的 Blob 容器,以便在叢集可用後立即可以使用該容器。 以後添加容器會更加複雜。
>
> 請參閱[建立 Avere vFXT for Azure](avere-vfxt-deploy.md#create-the-avere-vfxt-for-azure)，以了解詳細資料。

將空 Blob 儲存容器添加為核心檔案程式後,可以通過群集將資料複製到該容器。 使用並行的多線程複製機制。 請閱讀[將資料移動到 vFXT 叢集](avere-vfxt-data-ingest.md)以了解如何使用用戶端電腦和 Avere vFXT 快取，將資料有效率地複製到叢集的新容器中。
...
* 跳轉主機 -將公共 IP 位址分配給專用網路中的單獨 VM,並用它來創建到群集節點的 TLS 隧道。

  > [!TIP]
  > 如果您在叢集控制器上設定公用 IP 位址，則此控制器可作為跳板主機。 如需詳細資訊，請參閱[作為跳板主機的叢集控制器](#cluster-controller-as-jump-host)。

* 虛擬專用網路 (VPN) - 在 Azure 中的專用網路和企業網路之間配置點對點或網站到網站 VPN。
...
為了提高具有公共 IP 位址的控制器的安全性,部署腳本會自動創建一個網路安全組,該組僅限制對埠 22 的入站訪問。 若要進一步保護系統，您可以鎖定 IP 來源位址範圍的存取，也就是只允許從要用來存取叢集的機器進行連線。

建立叢集時，您可以選擇是否要在叢集控制器上建立公用 IP 位址。

* 如果創建新**的虛擬網路**或**新子網**,將為群集控制器分配**公共**IP 位址。
* 如果選擇現有的虛擬網路和子網,群集控制器將僅具有**專用**IP 位址。

## <a name="vm-access-roles"></a>VM 存取角色

Azure 使用[基於角色的存取控制](../role-based-access-control/index.yml)(RBAC) 授權群集 VM 執行某些任務。 例如,群集控制器需要授權才能創建和配置群集節點 VM。 叢集節點需要能夠將 IP 位址分配給其他群集節點或重新分配 IP 位址。

Avere vFXT 虛擬機使用兩個內置 Azure 角色:

* 叢集控制器使用內建角色[Avere 參與者](../role-based-access-control/built-in-roles.md#avere-contributor)。
* 叢集節點使用內建角色[Avere 運算子](../role-based-access-control/built-in-roles.md#avere-operator)。

如果需要為 Avere vFXT 元件自定義訪問角色,則必須定義自己的角色,然後在創建這些角色時將其分配給 VM。 不能在 Azure 應用商店中使用部署範本。 通過在 Azure 門戶中打開票證,請參閱 Microsoft 客戶服務和支援,如[獲取系統説明](avere-vfxt-open-ticket.md)中所述。

## <a name="next-steps"></a>後續步驟

[部署概述](avere-vfxt-deploy-overview.md)提供了為 Azure 系統創建 Avere vFXT 並使其準備好提供數據所需的步驟的全域視圖。
