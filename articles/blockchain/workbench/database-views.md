---
title: Azure 區塊鏈工作臺資料庫檢視
description: 可用 Azure 區塊鏈工作臺預覽 SQL DB 資料庫檢視的概述。
ms.date: 09/05/2019
ms.topic: article
...
---
# <a name="azure-blockchain-workbench-database-views"></a>Azure 區塊鏈工作臺資料庫檢視

Azure 區塊鏈工作臺預覽將資料從分散式分類帳傳遞到*鏈外*SQL 資料庫。 鏈外資料庫使得可以使用 SQL 和現有工具（如[SQL Server Management Studio）](https://docs.microsoft.com/sql/ssms/download-sql-server-management-studio-ssms?view=sql-server-2017)與區塊鏈資料進行交互。
...
| UploadedByUserExternalId         | nvarchar(255) | 否          | 應用程式上傳者的使用者外部識別碼。 預設情況下，此 ID 是聯合體 Azure 活動目錄中的使用者。                                                                                                |
...
| UserExternalId             | nvarchar(255) | 否          | 與角色相關聯之使用者的外部識別碼。 預設情況下，此 ID 是聯合體 Azure 活動目錄中的使用者。                                                                     |
...
| UserExternalId           | nvarchar(255) | 否          | 與連線相關聯之使用者的外部識別碼。 預設情況下，此 ID 是聯合體 Azure 活動目錄中的使用者。 |
| UserProvisioningStatus   | int           | 否          |識別使用者佈建程序的目前狀態。 可能的值包括： <br />0 – API 已建立使用者<br />1 – 已在資料庫中建立與使用者相關聯的金鑰<br />2 – 已完整佈建使用者 |
...
| ContractDeployedByUserId                 | int            | 否          | 合約部署者的使用者外部識別碼。 預設情況下，此 ID 是表示使用者 Azure 活動目錄 ID 的 guid。                                                                                                          |
| ContractDeployedByUserExternalId         | nvarchar(255)  | 否          | 合約部署者的使用者外部識別碼。 預設情況下，此 ID 是表示使用者 Azure 活動目錄 ID 的 guid。                                                                                                         |
...
| ContractDeployedByUserFirstName          | nvarchar(50)  | 是         | 合約部署者的使用者名字 |
| ContractDeployedByUserLastName           | nvarchar(255) | 是         | 合約部署者的使用者姓氏 |
| ContractDeployedByUserExternalId         | nvarchar(255) | 否          | 合約部署者的使用者外部識別碼。 預設情況下，此 ID 是在聯合體 Azure 活動目錄中表示其標識的 guid。                                                                                                                                                |
| ContractDeployedByUserEmailAddress       | nvarchar(255) | 是         | 合約部署者的使用者電子郵件地址 |
...
| ContractActionExecutedByUserExternalId   | nvarchar(255) | 是         | 合約動作執行者的使用者外部識別碼 預設情況下，此 ID 是在聯合體 Azure 活動目錄中表示其標識的 guid。 |
...
| TransactionIsWorkbenchTransaction        | bit           | 是         | 標識事務是否為 Azure 區塊鏈工作臺事務位 |
...
| StateName                          | nvarchar(50)  | 是         | 如果此屬性包含協定的狀態，則它是狀態的顯示名稱。 如果與狀態沒有關聯，則值將為 Null。 |
| StateDisplayName                   | nvarchar(255) | 否          | 如果此屬性包含狀態，則它是狀態的顯示名稱。 如果與狀態沒有關聯，則值將為 Null。 |
| StateValue                         | nvarchar(255) | 是         | 如果此屬性包含狀態，則它是狀態值。 如果與狀態沒有關聯，則值將為 Null。 |
...
| StateName                          | nvarchar(50)  | 是         | 如果此屬性包含狀態，則它為狀態的顯示名稱。 如果與狀態沒有關聯，則值將為 Null。 |
| StateDisplayName                   | nvarchar(255) | 否          | 如果此屬性包含狀態，則它是狀態的顯示名稱。 如果與狀態沒有關聯，則值將為 Null。 |
| StateValue                         | nvarchar(255) | 是         | 如果此屬性包含狀態，則它是狀態值。 如果與狀態沒有關聯，則值將為 Null。 |
...
|--------------------|---------------|-------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ID                 | int           | 否          | 使用者的唯一識別碼 |
| ExternalID         | nvarchar(255) | 否          | 使用者的外部識別碼。 預設情況下，此 ID 是表示使用者 Azure 活動目錄 ID 的 guid。 |
...
此檢視會詳細顯示核心工作流程中繼資料以及工作流程的函式和參數。 它專為報告而設計，還包含有關與工作流關聯的應用程式的中繼資料。 此檢視包含來自多個基礎資料表的資料，以利報告工作流程的資料。 對於每個工作流程，此檢視會包含下列資料：
...
此檢視會詳細顯示核心工作流程中繼資料以及工作流程的函式和參數。 它專為報告而設計，還包含有關與工作流關聯的應用程式的中繼資料。 此檢視包含來自多個基礎資料表的資料，以利報告工作流程的資料。 對於每個工作流程函式，此檢視會包含下列資料：
...
| WorkflowFunctionIsConstructor        | bit           | 否          | 標識工作流函數是否為工作流的建構函式 |
| WorkflowFunctionParameterId          | int           | 否          | 函式參數的唯一識別碼 |
| WorkflowFunctionParameterName        | nvarchar(50)  | 否          | 函式參數的名稱 |
| WorkflowFunctionParameterDisplayName | nvarchar(255) | 否          | 要在使用者介面中顯示的函式參數名稱 |
| WorkflowFunctionParameterDataTypeId  | int           | 否          | 與工作流程函式參數相關聯之資料類型的唯一識別碼 |
