---
title: 部署 Azure Blockchain Workbench 預覽版
description: 如何部署 Azure Blockchain Workbench 預覽版
ms.date: 07/16/2020
ms.topic: how-to
...
ms.locfileid: "87077022"
---
# <a name="deploy-azure-blockchain-workbench-preview"></a>部署 Azure Blockchain Workbench 預覽版

Azure Blockchain Workbench 預覽版是使用 Azure Marketplace 的解決方案範本來部署。 該範本可讓您輕鬆部署建立區塊鏈應用程式所需的元件。 部署完成後，Blockchain Workbench 將可讓您存取用戶端應用程式，以建立及管理使用者和區塊鏈應用程式。
...
* Azure 儲存體帳戶 (標準 LRS) 
* 容量為1的虛擬機器擴展集
* 虛擬網路資源群組 (Load Balancer、網路安全性群組、公用 IP 位址、虛擬網路) 
* Azure 區塊鏈服務。 如果您使用先前的 Blockchain Workbench 部署，請考慮重新部署 Azure Blockchain Workbench 以使用 Azure 區塊鏈服務。
...
> [!IMPORTANT]
> Workbench 所要部署到的租用戶，不必和用來註冊 Azure AD 應用程式的租用戶相同。 Workbench 所要部署到的租用戶，必須是您在其中有足夠權限可部署資源的租用戶。 如需 Azure AD 租用戶的詳細資訊，請參閱[如何取得 Active Directory 租用戶](../../active-directory/develop/quickstart-create-new-tenant.md)和[整合應用程式與 Azure Active Directory](../../active-directory/develop/quickstart-register-app.md)。
...
1. 選取**區塊鏈**  >  **Azure Blockchain Workbench (preview) **。
    ...
    | 資料庫和區塊鏈密碼 | 指定要用來存取在部署過程中建立之資料庫的密碼。 密碼必須符合下列四項需求的其中三項：長度必須介於 12 & 72 個字元、1個小寫字元、1個大寫字元、1個數字和1個特殊字元（不是數位記號） ( # ) 、百分比 (% ) 、逗號 (、) 、星星 ( * ) 、雙引號 () 、雙引號 ( ) 、單引號 ( ) 、 \` 雙引號 ( ) 和 semicolumn (; )  |
    | 部署區域 | 指定 Blockchain Workbench 資源的部署位置。 為了達到最佳可用性，這應該符合 **區域** 位置設定。 在預覽期間，並非所有區域都可供使用。 某些區域可能無法使用這些功能。 下列 Azure 區域提供 Azure 區塊鏈資料管理員服務：美國東部和西歐。|
...
1. 在 [進階設定]**** 中，選擇您是否要建立新的區塊鏈網路，或使用現有的權威證明區塊鏈網路。
    ...
    **新建**：
    ...
    [ *建立新* 的] 選項會以預設的基本 sku 部署 Azure 區塊鏈服務仲裁總帳。

    ![適用於新區塊鏈網路的進階設定](media/deploy/advanced-blockchain-settings-new.png)

    | 設定 | 說明  |
    |---------|--------------|
    | Azure 區塊鏈服務定價層 | 選擇用於 Blockchain Workbench 的 **基本** 或 **標準** Azure 區塊鏈服務層 |
    | Azure Active Directory 設定 | 選擇 [稍後再新增]****。</br>注意：如果您選擇了 [預先設定 Azure AD](#azure-ad-configuration) 或重新部署，請選擇 *立即新增*。 |
    | VM 選取項目 | 為您的區塊鏈網路選取慣用的儲存體效能和 VM 大小。 如果您使用服務額度較低的訂用帳戶 (例如，Azure 免費層)，請選擇較小規模的 VM (例如，標準 DS1 v2)**。 |
...
1. 在 App Service **總覽**中，將 **url** 值（代表部署的 Blockchain Workbench 的 web url）複製到。
...
您已完成 Azure Blockchain Workbench 部署。 請參閱 [後續步驟](#next-steps) 以取得開始使用您的部署的建議。
...
1. 登入 [Azure 入口網站](https://portal.azure.com)。
1. 在右上角選取帳戶，並切換至所需的 Azure AD 租用戶。 租使用者應該是訂用帳戶的訂用帳戶管理員租使用者，其中部署了 Azure Blockchain Workbench，而且您有足夠的許可權可以註冊應用程式。
1. 在左側導覽窗格中，選取 [Azure Active Directory]**** 服務。 選取**應用程式註冊**  >  **新的註冊**。

    ![應用程式註冊](media/deploy/app-registration.png)

1. 提供 [顯示 **名稱** ]，並 **只選擇此組織目錄中的帳戶**。
...
1. 資訊清單必須有 GUID。 您可以使用 PowerShell 命令或 Cmdlet 來產生 `[guid]::NewGuid()` GUID `New-GUID` 。 另一個選項是使用 GUID 產生器網站。
1. 針對您註冊的應用程式，請在 [**管理**] 區段中選取 [**資訊清單**]。
1. 接下來，更新資訊清單的 **appRoles** 區段。 取代 `"appRoles": []` 為提供的 JSON。 請務必將欄位的值取代為 `id` 您所產生的 GUID。
...
1. 在 *區塊鏈 api* 應用程式註冊中，選取 [ **api 許可權**]。 依預設，會加入圖形 API **使用者. 讀取** 許可權。
1. 工作臺應用程式需要使用者的基本設定檔資訊的讀取權限。 在 [ *已設定的許可權*] 中，選取 [ **新增許可權**]。 在 **Microsoft api**中，選取 [ **Microsoft Graph**]。
1. 由於工作臺應用程式使用已驗證的使用者認證，因此請選取 [ **委派的許可權**]。
1. 在 [ *使用者* ] 類別中，選擇 [ **>user.readbasic.all]。 [所有** 許可權]。

    ![Azure AD 應用程式註冊設定，顯示新增 Microsoft Graph >user.readbasic.all。所有委派的許可權](media/deploy/add-graph-user-permission.png)

    選取 [新增權限]。

1. 在 [ *設定的許可權*] 中，選取 [授與系統 **管理員同意** 網域]，然後在驗證提示中選取 **[是]** 。
...
1. 在 [ **Advanced 設定** ] 區段中，檢查 **存取權杖** 和 **識別碼權杖**。
    ...
    ![驗證 advanced settings](media/deploy/authentication-advanced-settings.png)
