...
ms.locfileid: "90530432"
---
# <a name="quickstart-use-metamask-to-connect-and-deploy-a-smart-contract"></a>使用 MetaMask 連線並部署智慧型合約
...
您需要 Azure 區塊鏈服務端點位址才能連線至區塊鏈網路。 端點位址和存取金鑰皆位於 Azure 入口網站。

1. 登入 [Azure 入口網站](https://portal.azure.com)。
1. 瀏覽至您的 Azure 區塊鏈服務成員。
1. 選取 [交易節點]**** 和預設交易節點連結。

    ![選取預設交易節點](./media/connect-metamask/transaction-nodes.png)

1. 選取 [連接字串] > [存取金鑰]****。
1. 從 [HTTPS (存取金鑰 1)]**** 複製端點位址。
...
1. 在 [新增網路] > [新增 RPC URL]**** 中，貼上您先前複製的端點位址。
1. 選取 [儲存]****。
...
1. 若要編譯合約，請先選取 Solidity 編譯器窗格，然後選取 [編譯 simple.sol]****。
    ...
    ![螢幕擷取畫面顯示正在編譯的合約。](./media/connect-metamask/compile.png)
...
1. 選取 [簡單] 合約，然後選取 [部署]********。
    ...
    ![螢幕擷取畫面顯示已選取合約和部署的部署及執行交易。](./media/connect-metamask/deploy.png)
...
Remix 的偵錯窗格會顯示與區塊鏈上交易有關的詳細資料：

![偵錯歷程記錄](./media/connect-metamask/debug.png)

您可以查看**簡單**合約的建立情形、**simple.add** 的交易，以及呼叫 **simple.get**。

若要查看 MetaMask 中的交易記錄，請開啟 MetaMask 瀏覽器擴充功能，並查看 [歷程記錄]**** 區段，以取得已部署合約和交易的記錄。
