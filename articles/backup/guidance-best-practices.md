...
雖然您可以輕鬆地在 Azure 上開始保護基礎結構和應用程式，但當您確保基礎 Azure 資源已正確設定並以最佳方式使用時，您就能加快價值的速度。 本文涵蓋設計考慮的簡要總覽，以及最佳設定 Azure 備份部署的指引。 它會檢查核心元件（例如，復原服務保存庫、備份原則）和概念（例如，治理），以及如何將其視為詳細產品檔的連結，以及其功能。
...
Azure 備份可針對各種工作負載（內部部署和雲端）提供資料保護。 它是 Azure 中的安全且可靠的內建資料保護機制。 它可以在多個工作負載之間順暢地調整其保護，而不需要管理額外負荷。 有多個自動化通道也可以啟用此功能（透過 PowerShell、CLI、Azure Resource Manager 範本和 REST Api）。

* **可調整、持久且安全的儲存體-** Azure 備份會使用可靠的 Blob 儲存體搭配內建的安全性和高可用性功能。 您可以針對備份資料選擇 LRS、GRS 或 RA GRS 的儲存體。  

* **原生工作負載整合-** Azure 備份提供與 Azure 工作負載（Vm、SAP Hana、Azure Vm 中的 SQL，甚至 Azure 檔案儲存體）的原生整合，而不需要您管理自動化或基礎結構來部署代理程式、撰寫新的腳本或布建存放裝置。
...
* **快照集管理**– Azure 備份會針對某些 Azure 原生工作負載（vm 和 Azure 檔案儲存體）拍攝快照集、管理這些快照集，並允許從它們進行快速還原。 此選項可大幅減少將資料復原到原始儲存體的時間。
...
由於保存庫的範圍是訂用帳戶，因此符合訂用帳戶設計策略（例如*應用程式類別策略*），其中會根據特定的應用程式或服務或在應用程式 archetype 行中分隔訂閱。 如需詳細資訊，請參閱這篇[文章](https://docs.microsoft.com/azure/cloud-adoption-framework/decision-guides/subscriptions/)。
...
* 如果您的工作負載分散到訂用帳戶，則您可以建立多個保存庫，每個訂用帳戶一或多個。
  ...
  * 如果您需要跨保存庫的一致原則，則可以使用 Azure 原則來跨多個保存庫傳播備份原則。 您可以撰寫自訂的[Azure 原則定義](https://docs.microsoft.com/azure/governance/policy/concepts/definition-structure)，使用[' deployifnotexists '](https://docs.microsoft.com/azure/governance/policy/concepts/effects#deployifnotexists)效果，在多個保存庫之間傳播備份原則。 您指派可以[將](https://docs.microsoft.com/azure/governance/policy/assign-policy-portal)此 Azure 原則定義指派給特定範圍（訂用帳戶或 RG），使其將「備份原則」資源部署到 Azure 原則指派範圍中的所有復原服務保存庫。 備份原則的設定（例如備份頻率、保留等等）應由使用者指定為 Azure 原則指派中的參數。
...
* *儲存體複寫類型*預設會設定為「異地多餘」（GRS）。 一旦您設定備份之後，[修改] 選項就會停用。 請遵循[下列](backup-create-rs-vault.md#set-storage-redundancy)步驟來審查和修改設定。
...
Azure 備份原則有兩個元件：*排程*（何時進行備份）和*保留期*（保留備份的時間長度）。 您可以根據所備份的資料類型、RTO/RPO 需求、操作或法規合規性需求和工作負載類型（例如 VM、資料庫、檔案）來定義原則。 [在這裡深入瞭解](backup-architecture.md#backup-policy-essentials)。
...
* 長期保留：
  * 已規劃（合規性需求）-如果您事先知道資料是目前時間所需的年份，則請使用長期保留。
  * 未規劃的（視需要需求）-如果您事先不知道，則可以使用隨選搭配特定的自訂保留設定（這些自訂保留設定不會受到原則設定的影響）。

* 使用自訂保留的隨選備份-如果您需要進行未透過備份原則排程的備份，則可以使用隨選備份。 這適用于讓備份不符合您的排程備份或進行細微備份（例如，每天執行多個 IaaS VM 備份，因為排程備份每天只允許一次備份）。 請務必注意，在排定的原則中定義的保留原則不適用於隨選備份。
...
* 當您的業務需求變更時，您可能需要延長或減少保留期間。 當您這麼做時，您可以預期下列情況：  
  ...
  * 最新的保留規則適用于所有保留點（不包括隨選保留點）。 因此，如果保留期限已延長（例如100天），則在進行備份時，會接著保留縮減（例如從100天到7天），所有備份資料都會根據最後指定的保留期限（也就是7天）來保留。

* Azure 備份可讓您彈性地*停止保護和管理您的備份*：
  * *停止保護並保留備份資料*。 如果您要淘汰或解除委任您的資料來源（VM、應用程式），但需要保留資料以進行審核或合規性，則可以使用此選項來防止所有未來的備份工作保護您的資料來源，並保留已備份的復原點。 然後，您可以還原或繼續 VM 保護。
  * *停止保護並刪除備份資料*。 此選項會阻止所有未來的備份工作保護您的 VM，並刪除所有復原點。 您將無法還原 VM，也無法使用 [繼續備份] 選項。

  * 如果您繼續保護（資料來源已停止但保留資料），則會套用保留規則。 所有過期的復原點將會移除（在排定的時間）。
...
Azure 備份提供安全性功能，即使在刪除之後，也能協助保護備份資料。 使用虛刪除時，如果使用者刪除備份（VM、SQL Server 資料庫、Azure 檔案共用 SAP Hana 資料庫），備份資料會保留14個額外的天數，允許復原該備份專案，而不會遺失資料。 在「虛刪除」狀態中，備份資料的額外14天保留期不會對您產生任何費用。 [在這裡深入瞭解](backup-azure-security-feature-cloud.md)。
...
Azure 備份服務使用 Microsoft Azure 復原服務 (MARS) 代理程式，將檔案、資料夾，以及磁碟區或系統狀態，從內部部署電腦備份與還原至 Azure。 MARS 現在提供安全性功能：從 Azure 備份下載之後，在上傳和解密之前進行加密的複雜密碼，已刪除的備份資料會保留一天，從刪除日期起算，以及執行重要作業（例如 變更複雜密碼）只能由具有有效 Azure 認證的使用者執行。 [在這裡深入瞭解](backup-azure-security-feature.md)。

## <a name="network-considerations"></a>網路考量

Azure 備份需要將資料從您的工作負載移至復原服務保存庫。 Azure 備份提供數個功能，可防止不慎公開備份資料（例如網路上的攔截式攻擊）。 請參考下列指引：
...
* *混合*式-MARS （Microsoft Azure 復原服務）代理程式需要網路存取所有重要作業-安裝、設定、備份和還原。 MARS 代理程式可以使用公用對等互連（適用于舊線路）和 Microsoft 對等互連（使用[私人端點](install-mars-agent.md#private-endpoints)，或透過[適當的存取控制，透過 proxy/防火牆](install-mars-agent.md#verify-internet-access)），透過[Azure ExpressRoute](install-mars-agent.md#use-azure-expressroute)連接到 Azure 備份服務。

### <a name="private-endpoints-for-azure-backup"></a>Azure 備份的私用端點

Azure[私用端點](https://docs.microsoft.com/azure/private-link/private-endpoint-overview)是一種網路介面，可讓您私下且安全地連線到 Azure 私用連結所支援的服務。 Azure 備份可讓您使用私人端點，安全地備份和還原復原服務保存庫中的資料。

* 當您啟用保存庫的私人端點時，它們只會用來備份和還原 Azure VM 和 MARS 代理程式備份中的 SQL 和 SAP Hana 工作負載。  您也可以使用保存庫來備份其他工作負載（但它們不需要私用端點）。 除了使用 MARS 代理程式來備份 SQL 和 SAP Hana 工作負載和備份之外，在 Azure VM 備份的情況下，也會使用私人端點來執行檔案復原。 [在這裡深入瞭解](private-endpoints.md#recommended-and-supported-scenarios)。

* Azure Active Directory 目前不支援私用端點。 因此，在 Azure Vm 中執行資料庫的備份並使用 MARS 代理程式進行備份時，必須允許 Azure Active Directory 所需的 Ip 和 Fqdn 從受保護的網路進行輸出存取。 您也可以使用 NSG 標籤和 Azure 防火牆標記，以允許 Azure AD 的存取權（如適用）。 在這裡深入瞭解[必要條件](https://docs.microsoft.com/azure/backup/private-endpoints#before-you-start)。

## <a name="governance-considerations"></a>治理考量

Azure 中的治理主要是使用[Azure 原則](https://docs.microsoft.com/azure/governance/policy/overview)和[Azure 成本管理](https://docs.microsoft.com/azure/cost-management-billing/cost-management-billing-overview)來執行。 [Azure 原則](https://docs.microsoft.com/azure/governance/policy/overview)可讓您建立、指派和管理原則定義，以強制執行資源的規則。 此功能可確保這些資源能夠符合公司的標準規範。 針對您的 Azure 資源和其他雲端服務提供者，[Azure 成本管理](https://docs.microsoft.com/azure/cost-management-billing/cost-management-billing-overview)可讓您追蹤雲端使用量和費用。 此外，下列工具（例如[Azure 價格計算機](https://azure.microsoft.com/pricing/calculator/)和[Azure Advisor](https://docs.microsoft.com/azure/advisor/advisor-overview) ）在成本管理流程中扮演重要的角色。

### <a name="azure-backup-support-two-key-scenarios-via-built-in-azure-policy"></a>Azure 備份透過內建 Azure 原則支援兩個主要案例

* 確保新建立的商務關鍵機器會自動以正確的保留設定進行備份。 Azure 備份提供內建原則（使用 Azure 原則），可以指派給訂用帳戶或資源群組中指定位置的所有 Azure Vm。 當此原則指派給指定的範圍時，在該範圍內建立的所有新 Vm 都會自動設定為備份至相同位置和訂用帳戶中的現有保存庫。 使用者可以指定要與備份的 Vm 相關聯的保存庫和保留原則。 [在這裡深入瞭解](backup-azure-auto-enable-backup.md)。

* 確保新建立的保存庫已啟用診斷功能以支援報告。 通常，每個保存庫手動新增診斷設定可能會是一件麻煩的工作。 此外，任何建立的新保存庫都必須啟用診斷設定，讓您可以查看此保存庫的報告。 為了簡化大規模的診斷設定建立（使用 Log Analytics 作為目的地），Azure 備份提供內建 Azure 原則。 此原則會將 LA 診斷設定新增至每個訂用帳戶或資源群組中的所有保存庫。 下列各節提供有關如何使用此原則的指示。 [在這裡深入瞭解](azure-policy-configure-diagnostics.md)。
...
* 優化備份原則
  * 根據工作負載 archetype 優化排程和保留設定（例如任務關鍵性、非關鍵）
  * 優化立即還原的保留設定
  * 選擇正確的備份類型以符合需求，同時取得 Azure 備份考慮的工作負載所支援的備份類型（完整、增量、記錄、差異）。

* 選擇性備份磁片： [排除磁片] （預覽功能）提供有效率且符合成本效益的選擇，以選擇性地備份重要資料。 例如，當您不想要備份連接至 VM 的其餘磁片時，請只備份一個磁片。 當您有多個備份解決方案時，這也很有用。 例如，當您使用工作負載備份解決方案來備份資料庫或資料時（在 Azure VM 備份中 SQL Server 資料庫），而且您想要針對選取的磁片使用 Azure VM 層級備份。
...
* 如果您需要大規模監視操作活動，則**Backup Explorer**會提供整個備份資產的匯總視圖，以進行詳細的向下切入分析和疑難排解。 這是內建的 Azure 監視器活頁簿，提供單一的中央位置，協助您監視 Azure 上整個備份資產的營運活動、跨租使用者、位置、訂用帳戶、資源群組和保存庫。 [在這裡深入瞭解](monitor-azure-backup-with-backup-explorer.md)。
  ...
  * 儀表板會提供過去七天（最大）的操作活動。 如果您需要保留這種資料，您可以匯出成 Excel 檔案並加以保留。
...
* 此外，
  * 您可以將資料（例如作業、原則等等）傳送到**Log Analytics**工作區。 這會啟用 Azure 監視器記錄的功能，讓資料與 Azure 監視器所收集的其他監視資料相互關聯、將多個 Azure 訂用帳戶和租使用者中的記錄專案合併到一個位置以進行分析，並使用記錄查詢來執行複雜的分析，並取得記錄專案的深入見解。 [在這裡深入瞭解](https://docs.microsoft.com/azure/azure-monitor/platform/activity-log#send-to-log-analytics-workspace)。
  * 您可以將資料傳送至事件中樞，以將專案傳送至 Azure 外部（例如，安全性資訊和事件管理）或其他 log analytics 解決方案。 [在這裡深入瞭解](https://docs.microsoft.com/azure/azure-monitor/platform/activity-log#send-to-azure-event-hubs)。
  * 如果您想要保留記錄資料超過90天的時間來進行審核、靜態分析或備份，您可以將資料傳送至 Azure 儲存體帳戶。 如果您只需要保留您的事件90天（或更少），則不需要將封存設定至儲存體帳戶，因為活動記錄事件會在 Azure 平臺中保留90天。 [深入了解](https://docs.microsoft.com/azure/azure-monitor/platform/activity-log#send-to--azure-storage)。
...
* Azure 備份透過電子郵件，為失敗、警告和重要作業提供**內建的警示**通知機制。 您可以指定個別的電子郵件地址或通訊群組清單，以便在產生警示時收到通知。 您也可以選擇是否要取得每個個別警示的通知，或以每小時摘要將其分組，然後收到通知。
  ...
  * 某些工作負載可能會產生高頻率的失敗（例如，每隔15分鐘 SQL Server 一次）。 為了避免在每次失敗發生時產生的警示變滿，會合並警示。 [在這裡深入瞭解](backup-azure-monitoring-built-in-monitor.md#consolidated-alerts)。
  * 內建警示無法自訂，而且僅限於在 Azure 入口網站中定義的電子郵件。

* 如果您需要**建立自訂警示**（例如，成功作業的警示），請使用 Log Analytics。 在 Azure 監視器中，您可以在 Log Analytics 工作區建立自己的警示。 混合式工作負載（DPM/MABS）也可以將資料傳送至 LA，並使用 LA 在 Azure 備份支援的工作負載之間提供常見的警示。
