...
5. MABS 伺服器會在裝載復原資料的磁片區上使用 VSS，以便提供多個陰影複製。 這些陰影複本各自提供一份個別的復原。 VSS 復原點會儲存在 MABS 伺服器上。 在執行 Hyper-v 的伺服器上所做的暫存複本，只會儲存在 MABS 同步處理的持續時間內。
...
5. 在 [選擇資料保護方式] 頁面上，指定保護群組名稱。 選取 [**我想要使用磁片進行短期保護**]，如果您想要使用 Azure 備份服務將資料備份到 Azure，請選取 [**我想要線上保護**]。
