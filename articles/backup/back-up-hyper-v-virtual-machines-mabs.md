...
3. 在 MABS 系統管理員主控台中，按一下 [**保護**  >  **建立保護群組**] 以開啟 [**建立新保護組**嚮導]。
...
9. 在 [一致性檢查選項]**** 頁面上，選取要自動執行一致性檢查的方式。 您可以只在複本資料變得不一致時才檢查，或根據排程來執行檢查。 如果不想設定自動一致性檢查，您可以滑鼠右鍵按一下保護群組，然後選取 [執行一致性檢查]****，以手動方式隨時執行檢查。
