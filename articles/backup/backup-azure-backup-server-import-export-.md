...
>[!NOTE]
> 如果您在 MARS 代理程式的安裝中找不到列出的公用程式（例如*AzureOfflineBackupCertGen*），請寫入AskAzureBackupTeam@microsoft.com來取得它們的存取權。
...
1. 在 [**擁有的應用程式**] 索引標籤上，找出`AzureOfflineBackup _<Azure User Id`具有顯示名稱格式的應用程式。
...
1. 選取 [**管理電腦憑證應用程式** > ] 的 [**個人**] 索引標籤。 `CB_AzureADCertforOfflineSeeding_<ResourceId>`尋找名稱為的憑證。
...
1. 以滑鼠右鍵按一下 [ **CloudBackupProvider**]，並新增名稱`AzureADAppCertThumbprint_<Azure User Id>`為的新字串值。
...
1. 當您有追蹤號碼資訊時，請移至正在等待匯入作業完成的來源電腦。 在提高許可權的命令提示字元中執行下列命令，並以*AzureOfflineBackupDiskPrep*公用程式目錄作為目前的目錄。
   ...
   `*.\AzureOfflineBackupDiskPrep.exe*  u:  s:<*Staging Location Path*>   p:<*Path to AzurePublishSettingsFile*>`
    ...
    | p：&lt;*publishsettingsfile 的路徑的路徑*&gt; | 當命令不是在來源電腦上執行時，會使用此強制輸入。 它可用來提供您在「起始離線備份」一節的工作流程中所輸入的 Azure 發佈設定檔案的路徑。 |
