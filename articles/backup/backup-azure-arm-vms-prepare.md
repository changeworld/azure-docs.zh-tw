...
5. 選取要在其中建立保存庫的 Azure 訂用帳戶、資源群組和地理區域。 然後，按一下 [建立]****。
    * 建立保存庫可能需要一些時間。
    * 請監視入口網站右上方區域中的狀態通知。
...
6. 監視入口網站通知。 您可以在 [保存庫] 儀表板中監視作業進度，>**進行中**的**備份作業** > 。 根據您的 VM 大小，建立初始備份可能需要花一點時間。
