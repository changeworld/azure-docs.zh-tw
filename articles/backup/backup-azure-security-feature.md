...
| 變更複雜密碼 |作業失敗。 識別碼：120002 |**原因：**<br/>啟用安全性設定時，如果您嘗試變更複雜密碼，且您使用的版本不受支援， (在本文第一次記) 中所指定的有效版本，則會出現此錯誤。<br/>**建議的動作：**<br/> 若要變更複雜密碼，您必須先將備份代理程式更新為最低版本2.0.9052 版、Azure 備份伺服器到最小的 update 1，以及/或 DPM 至最小的 DPM 2012 R2 UR12 或 DPM 2016 UR2 (下載以下) 的連結，然後輸入有效的安全性 PIN 碼。 若要取得 PIN，請登入 Azure 入口網站，然後流覽至 [復原服務保存庫] > [設定] > 屬性 > 產生安全性 PIN。 請使用這個 PIN 碼來變更複雜密碼。 |
