...
4. 從用戶端上的代理程式主控台，[設定備份](./backup-windows-with-mars-agent.md#create-a-backup-policy)來指定要備份的內容、何時備份（排程）、備份應保留在 Azure 中多久（保留原則）並開始保護。
...
- **備份 azure 虛擬機器中的特定檔案和資料夾**：備份 azure 虛擬機器（vm）的主要方法是使用 VM 上的 Azure 備份延伸模組。 延伸模組會備份整個 VM。 如果您想要備份 VM 中的特定檔案和資料夾，您可以在 Azure Vm 中安裝 MARS 代理程式。 如需詳細資訊，請參閱[架構：內建的 AZURE VM 備份](./backup-architecture.md#architecture-built-in-azure-vm-backup)。
