...
| **作業系統版本**            | SLES 12 SP2、SP3 和 SP4;SLES 15 with SP0 和 SP1 <br><br>   **預覽**-RHEL 7.4、7.6、7.7 和8。1  <br>     [開始](./tutorial-backup-sap-hana-db.md)使用適用于 RHEL 的 SAP Hana 備份預覽（7.4、7.6、7.7 和8.1）。 如有其他問題，請透過此地址發送電子郵件給我們：[AskAzureBackupTeam@microsoft.com](mailto:AskAzureBackupTeam@microsoft.com)。                |                                             |
...
> [!NOTE]
> 您現在可以在 Azure 入口網站中[監視從 HANA 原生用戶端 (SAP HANA Studio/Cockpit/DBA Cockpit) 觸發的備份和還原](./sap-hana-db-manage.md#monitor-manual-backup-jobs-in-the-portal) (到同一部電腦中)。

## <a name="next-steps"></a>後續步驟

* 了解如何[備份在 Azure VM 上執行的 SAP HANA 資料庫](./backup-azure-sap-hana-database.md) \(部分機器翻譯\)
* 了解如何[還原在 Azure VM 上執行的 SAP HANA 資料庫](./sap-hana-db-restore.md) \(部分機器翻譯\)
