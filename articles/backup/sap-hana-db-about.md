...
您可以使用[AZURE VM 備份](backup-azure-vms-introduction.md)來備份 OS 和其他非資料庫磁片。 VM 備份每天會進行一次，而且它會備份所有磁片 (但 **寫入加速器 (WA) 作業系統磁片** 和 **ultra 磁片**) 除外。 因為正在使用 Azure SAP Hana 備份解決方案來備份資料庫，所以您可以使用適用于 [Azure vm 的選擇性磁片備份和還原](selective-disk-backup-restore.md) 功能，只對 OS 和非資料庫磁片進行檔案一致備份。

若要還原執行 SAP Hana 的 VM，請遵循下列步驟：

* 從最新的復原點，[從 AZURE VM 備份還原新的 vm](backup-azure-arm-restore-vms.md) 。 或建立新的空白 VM，並從最新的復原點連接磁片。
* 如果已排除 WA 磁片，則不會還原它們。 在此情況下，請建立空白的 WA 磁片和記錄區域。
