...
8. 針對 Linux 機器，會產生 python 腳本。 其中一個需要下載腳本，並將它複製到相關/相容的 Linux 伺服器。 您可能必須修改用```chmod +x <python file name>```來執行的許可權。 然後使用```./<python file name>```執行 python 檔案。
...
必須要`download.microsoft.com`有存取權，才能下載用來在執行腳本的電腦與復原點中的資料之間建立安全通道的元件。
