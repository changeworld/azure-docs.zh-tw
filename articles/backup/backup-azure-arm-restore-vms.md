...
- **創建 VM：** 創建新 VM 時，VM 將放置在指定的存儲帳戶中。
- **復原磁碟**：復原磁碟時，磁片將複製到指定的存儲帳戶。 還原作業生成一個範本，您可以下載該範本並用於指定自訂 VM 設置。 此範本放置在指定的存儲帳戶中。
- **替換磁片**：替換現有 VM 上的磁片時，Azure 備份在替換磁片之前獲取現有 VM 的快照。 快照存儲在指定的暫存位置（存儲帳戶）中。 此存儲帳戶用於在還原過程中臨時存儲快照，我們建議您創建一個新帳戶來執行此操作，以後可以輕鬆地刪除該帳戶。
- **存儲帳戶位置**：存儲帳戶必須與保存庫位於同一區域。 僅顯示這些帳戶。 如果位置中沒有存儲帳戶，則需要創建一個存儲帳戶。
- **存儲類型**：不支援 Blob 存儲。
- **存儲冗余**：不支援區域冗余存儲 （ZRS）。 帳戶的複製和冗余資訊顯示在帳戶名稱之後的括弧中。
- **高級存儲**：
...
要還原 VM（創建新 VM），請確保具有還原 VM 操作的正確基於角色的存取控制 （RBAC）[許可權](backup-rbac-rs-vault.md#mapping-backup-built-in-roles-to-backup-management-actions)。

如果沒有許可權，則可以[復原磁碟](#restore-disks)，然後在磁片還原後，可以使用作為還原操作的一部分生成的[範本](#use-templates-to-customize-a-restored-vm)創建新的 VM。

## <a name="select-a-restore-point"></a>選取還原點

1. 在與要還原的 VM 關聯的保存庫中，按一下 **"備份項** > **Azure 虛擬機器**"。
...
1. 在 [還原設定]**** 中，選取還原選項：
    - **創建新**：如果要創建新 VM，請使用此選項。 您可以使用簡單的設定來建立 VM，或還原磁碟並建立自訂 VM。
    - **替換現有**：如果要替換現有 VM 上的磁片，請使用此選項。
...
1. 在**還原配置** > **創建新** > **還原類型**中，選擇 **"創建虛擬機器**"。
2. 在**虛擬機器名稱**中，指定訂閱中不存在的 VM。
...
5. 在**存儲位置**中，指定 VM 的存儲帳戶。 [深入了解](#storage-accounts)。
...
- 使用 PowerShell 從還原的磁片[創建新 VM。](https://docs.microsoft.com/azure/backup/backup-azure-vms-automation#create-a-vm-from-restored-disks)

1. 在**還原配置** > **中 創建新** > **的還原類型**，選擇**復原磁碟**。
...
當虛擬機器使用託管磁片並選擇"**創建虛擬機器**"選項時，Azure 備份不使用指定的存儲帳戶。 在**復原磁碟**和**即時還原**的情況下，存儲帳戶僅用於存儲範本。 託管磁片在指定的資源組中創建。
當虛擬機器使用非託管磁片時，它們將作為 blob 還原到存儲帳戶。
...
3. 在**暫存位置**中，指定在還原過程中應保存當前託管磁片的快照的位置。 [深入了解](#storage-accounts)。
...
作為[還原選項](https://docs.microsoft.com/azure/backup/backup-azure-arm-restore-vms#restore-options)之一，跨區域還原 （CRR） 允許您在次要區域（即 Azure 配對區域）中還原 Azure VM。

要在預覽期間加入該功能，請閱讀["開始之前"部分](https://docs.microsoft.com/azure/backup/backup-create-rs-vault#set-cross-region-restore)。

要查看是否啟用了 CRR，請按照[配置跨區域還原](backup-create-rs-vault.md#configure-cross-region-restore)中的說明操作

### <a name="view-backup-items-in-secondary-region"></a>查看次要區域中的備份項

如果啟用了 CRR，則可以查看次要區域中的備份項。

1. 從門戶轉到**恢復服務保存庫** > **備份專案**
2. 按一下 **"次要區域**"以查看次要區域中的項。

![次要區域中的虛擬機器](./media/backup-azure-arm-restore-vms/secbackedupitem.png)

![選擇次要區域](./media/backup-azure-arm-restore-vms/backupitems-sec.png)

### <a name="restore-in-secondary-region"></a>在次要區域中還原

次要區域還原使用者體驗將類似于主區域還原使用者體驗。 在"還原配置"邊欄選項卡中配置詳細資訊以配置還原時，系統將提示您僅提供次要區域參數。

![選擇要還原的 VM](./media/backup-azure-arm-restore-vms/sec-restore.png)

![選取還原點](./media/backup-azure-arm-restore-vms/sec-rp.png)

![還原配置](./media/backup-azure-arm-restore-vms/rest-config.png)

![觸發正在進行的還原通知](./media/backup-azure-arm-restore-vms/restorenotifications.png)

- 要還原和創建 VM，請參閱[創建 VM](https://docs.microsoft.com/azure/backup/backup-azure-arm-restore-vms#create-a-vm)。
- 要還原為磁片，請參閱[復原磁碟](https://docs.microsoft.com/azure/backup/backup-azure-arm-restore-vms#restore-disks)。

>[!NOTE]
>觸發還原並在資料傳輸階段後，無法取消還原作業。

### <a name="monitoring-secondary-region-restore-jobs"></a>監視次要區域還原作業

1. 從門戶轉到**恢復服務保存庫** > **備份作業**
2. 按一下 **"次要區域**"以查看次要區域中的項。

![篩選的備份作業](./media/backup-azure-arm-restore-vms/secbackupjobs.png)

## <a name="restore-vms-with-special-configurations"></a>使用特殊配置還原 VM
...
**還原單一網域中的多個網域控制站 VM** | 如果可以通過網路到達同一域中的其他網域控制站，則可以像任何 VM 一樣還原網域控制站。 如果該網域控制站是網域內剩餘的最後一個網域控制站，或者您是在隔離的網路中進行復原，請使用[樹系復原](https://docs.microsoft.com/windows-server/identity/ad-ds/manage/ad-forest-recovery-single-domain-in-multidomain-recovery)。
...
**還原具有特殊網路組態的 VM** | 特殊網路組態包含使用內部或外部負載平衡、使用多個 NIC 或多個保留 IP 位址的 VM。 您可以使用[還原磁碟選項](#restore-disks)來還原這些 VM。 此選項將 VHD 的副本複製到指定的存儲帳戶中，然後您可以根據您的配置創建具有[內部](https://azure.microsoft.com/documentation/articles/load-balancer-internal-getstarted/)或[外部](/azure/load-balancer/quickstart-create-standard-load-balancer-powershell)負載等化器、[多個 NICS](../virtual-machines/windows/multiple-nics.md)或[多個保留 IP 位址的](../virtual-network/virtual-network-multiple-ip-addresses-powershell.md)VM。
**NIC/子網上的網路安全性群組 （NSG）** | Azure VM 備份支援在 vnet、子網和 NIC 級別備份和恢復 NSG 資訊。
**區域固定 VM** | Azure 備份支援區域固定 VM 的備份和還原。 [深入了解](https://azure.microsoft.com/global-infrastructure/availability-zones/)
...
- 還原的 VM 不會有可用性設定值組。 如果使用復原磁碟選項，則可以使用提供的範本或 PowerShell 從磁片創建 VM 時[指定可用性集](../virtual-machines/windows/tutorial-availability-sets.md)。
...
- 如果由於 VM 與網域控制站的關係斷開而無法訪問 VM，請按照以下步驟啟動 VM：
  - 將 OS 磁片作為資料磁片附加到恢復的 VM。
  - 如果發現 Azure 代理通過遵循此[連結](https://docs.microsoft.com/azure/virtual-machines/troubleshooting/install-vm-agent-offline)無回應，則手動安裝 VM 代理。
  - 在 VM 上啟用串列主控台訪問，以允許命令列訪問 VM
  ...
  - 使用串列主控台訪問和 CMD 將 VM 與域隔離

    ```cmd
    cmd /c "netdom remove <<MachineName>> /domain:<<DomainName>> /userD:<<DomainAdminhere>> /passwordD:<<PasswordHere>> /reboot:10 /Force"
    ```

- 一旦 VM 分離並重新啟動，您將能夠使用本地管理員憑據成功地將 RDP 重新加入 VM，並成功將 VM 重新加入域。
