...
description: 在本文中,瞭解如何使用 Azure 備份在 Azure 虛擬機上備份 SQL Server 資料庫。
ms.reviewer: vijayts
ms.topic: conceptual
ms.date: 09/11/2019
...
SQL Server 資料庫是需要低恢復點目標 (RPO) 和長期保留的關鍵工作負載。 可以使用[Azure 備份](backup-overview.md)來備份在 Azure 虛擬機器 (VM) 上運行的 SQL Server 資料庫。

本文演示如何將在 Azure VM 上運行的 SQL Server 資料庫備份到 Azure 備份恢復服務保管庫。
...
>[!NOTE]
>**Azure VM 中的 SQL 伺服器軟刪除和 Azure VM 工作負荷中 SAP HANA 的軟刪除**現在在預覽版中可用。<br>
...
1. 標識或與託管 SQL Server 實例的 VM 位於同一區域中的[恢復服務保管庫](backup-sql-server-database-azure-vms.md#create-a-recovery-services-vault)和訂閱。
2. 認證 VM 是否具有[網路連線](backup-sql-server-database-azure-vms.md#establish-network-connectivity)。
...
> [!NOTE]
> 您可以為 Azure VM 以及執行在 VM 上的 SQL Server 資料庫啟用 Azure 備份,而不會發生衝突。
...
**部署 HTTP Proxy 伺服器以路由流量**。 備份 Azure VM 上的 SQL Server 資料庫時,VM 上的備份擴展程式使用 HTTPS API 將管理命令發送到 Azure 備份和數據到 Azure 儲存。 備份延伸模組也會使用 Azure AD 進行驗證。 透過 HTTP Proxy 路由傳送這三項服務的備份延伸模組流量。 Azure 備份沒有通配符域要添加到代理規則的允許清單中。 對於 Azure 提供的這些服務,您需要使用公共 IP 範圍。 延伸模組是唯一為了要存取公用網際網路而設定的元件。
...
使用 HTTP Proxy | VM 的單一網際網路存取點 <br/> | 使用 Proxy 軟體執行 VM 時的額外成本 <br/> 沒有已發布的 FQDN 位址,允許規則將受 Azure IP 位址變更的約束
...
別名可用於不支援的字元,但我們建議避免它們。 如需詳細資訊，請參閱 [了解表格服務資料模型](https://docs.microsoft.com/rest/api/storageservices/Understanding-the-Table-Service-Data-Model)。
...
5. 在 **「備份目標** > **」發現 VM"中**,選擇 **「開始發現」** 以搜尋訂閱中未受保護的 VM。 此搜索可能需要一段時間,具體取決於訂閱中未受保護的 VM 的數量。

   * 在探索之後，未受保護的 VM 應該會出現在清單中，並依照名稱和資源群組列出。
   * 如果 VM 未按預期列出,請參閱它是否已在保管庫中備份。
...
8. Azure 備份會探索 VM 上的所有 SQL Server 資料庫。 在發現期間,以下元素出現在後台:

    * Azure 備份將 VM 註冊到保管庫以進行工作負載備份。 只能備份到此保管庫上已註冊 VM 上的所有資料庫。
    ...
    * 如果您沒有從應用商店創建 VM,或者您在 SQL 2008 和 2008 R2 上,則 VM 可能沒有安裝 SqlIaaS 擴展,並且發現操作在錯誤消息 UserErrorSQLNoSysAdmins 時失敗。 要解決此問題,請按照[「設置 VM 權限」](backup-azure-sql-database.md#set-vm-permissions)下的說明操作。
...
3. 選擇要保護的所有資料庫,然後選擇 **「確定**」。
   ...
   為了將備份負載最佳化，Azure 備份會將每個備份作業的資料庫數目上限設為 50。
     ...
     * 要[啟用](#enable-auto-protection)整個實例或"始終打開的可用性"組,請在 **"AUTOPROTECT"** 下拉列表中選擇"**打開**",然後選擇 **"確定**"。
...
4. 選擇 **「 確定」** 以開啟**備份原則**。
    ...
    ![為「始終開啟的可用性」群組啟用自動保護](./media/backup-azure-sql-database/enable-auto-protection.png)
...
1. 在保管庫中,選擇 **「添加備份策略** > **添加**」。。
2. 在 **'新增**'**中**,在 Azure VM 中選擇 SQL Server 以定義策略類型。
...
5. 在**保留範圍內**,默認情況下選擇所有選項。 清除不需要的任何保留範圍限制,然後設置要使用的間隔。

    * 任何類型的備份(完整備份、差分和日誌)的最短保留期為 7 天。
    * 復原點會根據其保留範圍標記為保留。 例如，如果您選取每日完整備份，每天只會觸發一次完整備份。
    * 根據每周保留範圍和每周保留設置標記和保留特定一天的備份。
    * 每月和每年的保留範圍以類似方式進行。
...
11. 在 [記錄備份]**** 中選取 [啟用]****，然後設定頻率和保留控制項。 日誌備份可以每隔 15 分鐘進行一次,並且可以保留長達 35 天。
12. 選取 [確定]**** 以儲存原則，然後返回主要 [備份原則]**** 功能表。

    ![編輯記錄備份原則](./media/backup-azure-sql-database/log-backup-policy-editor.png)

13. 在 **「備份政策」** 選單上,選擇是否啟用**SQL 備份壓縮**。 預設會停用這個選項。 如果啟用,SQL Server 將向 VDI 發送壓縮備份流。  請注意,Azure 備份會根據此控制件的值,使用壓縮/NO_COMPRESSION子句覆蓋實例級別默認值。

14. 完成備份原則的編輯之後，請選取 [確定]****。

> [!NOTE]
> 每個日誌備份都連結到以前的完整備份,以形成恢復鏈。 此完整備份將保留,直到上次日誌備份的保留過期。 這可能意味著完整備份將保留一段額外的時間,以確保可以恢復所有日誌。 假設使用者每周有一個完整備份、每日差分和 2 小時日誌。 所有保留時間為30天。 但是,每周滿可以真正清理/刪除后,下一個完整備份可用,即,30 + 7 天后。 假設,每週一次的完整備份發生在 11 月 16 日。 根據保留政策,應保留至 12 月 16 日。 此完整的最後一個日誌備份發生在 11 月 22 日下一個計劃的完全之前。 在此日誌可用到 12 月 22 日之前,無法刪除 11 月 16 日的滿。 因此,11月16日的滿滿將保留到12月22日。
...
* 如果您的實例已經包含一些受保護的資料庫,則即使在您打開自動保護后,它們仍將受其各自策略的保護。 以後添加的所有未受保護的資料庫將只有您在啟用自動保護時定義的單個策略,該策略列在 **「配置備份**」下。 但是,您可以稍後更改與自動保護資料庫關聯的策略。  
...
要開啟自動保護:
  ...
  2. 選擇 **「自動保護**」下的下拉清單,選擇 **「打開**」,然後選擇 **「確定**」 。
      ...
      ![在可用性群組啟用自動保護](./media/backup-azure-sql-database/enable-auto-protection.png)

  3. 所有資料庫會一起設定備份，且您可在 [備份作業]**** 中加以追蹤。

如果需要禁用自動保護,請在 **「設定備份**」下選擇實例名稱,然後選擇實例的 **「禁用自動保護**」。。 將繼續備份所有資料庫,但將來的資料庫不會自動保護。

![關閉該實體的自動保護](./media/backup-azure-sql-database/disable-auto-protection.png)

## <a name="next-steps"></a>後續步驟

了解如何：

* [回復備份的 SQL Server 資料庫](restore-sql-database-azure-vm.md)
