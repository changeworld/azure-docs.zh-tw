---
title: 在 Azure VM 上管理及監視 SQL 伺服器 DB
...
本文介紹用於管理和監視在 Azure 虛擬機器 (VM) 上運行並由[Azure 備份](backup-overview.md)服務備份到 Azure 備份恢復服務保管庫的 SQL Server 資料庫的常見任務。 您將學習如何監視作業和警報、停止和恢復資料庫保護、運行備份作業以及從備份中取消註冊 VM。
...
> [!NOTE]
> **備份作業**門戶不顯示計劃的備份作業。 請使用 SQL Server Management Studio 來監視已排程的備份作業，如下一節中所述。
>
...
- 如果您刪除資料來源，但沒有停止備份，新的備份將會失敗。 根據策略,舊恢復點將過期,但最後一個恢復點將始終保留,直到您停止備份並刪除數據。
...
5. 在 **「停止備份」** 選單上,選擇是保留還是刪除數據。 如果需要,請提供理由和註釋。

    ![保留或移除「停止備份」選單上的資料](./media/backup-azure-sql-database/stop-backup-button.png)
...
停止 SQL 資料庫保護時,如果選擇「**保留備份資料」** 選項,則可以稍後恢復保護。 如果不保留備份數據,則無法恢復保護。
...
雖然您需要指定僅複製完整備份的保留期限,但按需完整備份的保留範圍將自動設置為當前時間的 45 天。
...
在關閉保護後在刪除保管庫之前取消註冊 SQL Server 實體:

1. 在保管庫儀錶板上,在 **"管理**"下,選擇 **"備份基礎結構**"。  
...
> [!NOTE]
> 保留期中的任何更改都將追溯應用於除新恢復點之外的所有舊恢復點。

在保管庫儀錶板中,轉到**管理** > **備份策略**並選擇要編輯的策略。

  ![管理備份原則](./media/backup-azure-sql-database/modify-backup-policy.png)

  ![變更備份原則](./media/backup-azure-sql-database/modify-backup-policy-impact.png)
...
只需單擊一下,即可修復所有受影響專案的策略版本:

  ![修復不一致的策略](./media/backup-azure-sql-database/fix-inconsistent-policy.png)

## <a name="re-register-extension-on-the-sql-server-vm"></a>在 SQL Server VM 上重新註冊延伸

有時,VM 上的工作負載擴展可能會由於以下或一個原因而受到影響。 在這種情況下,在 VM 上觸發的所有操作將開始失敗。 然後,您可能需要在 VM 上重新註冊擴展。 **重新註冊**操作在 VM 上重新安裝工作負載備份擴展,以便繼續操作。

謹慎使用此選項;在具有已正常運行的擴展的 VM 上觸發時,此操作將導致擴展重新啟動。 這可能導致所有正在進行的作業失敗。 請在觸發重新註冊操作之前檢查一個或多個[症狀](backup-sql-server-azure-troubleshoot.md#re-registration-failures)。
