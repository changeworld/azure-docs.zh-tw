...
##### <a name="summary"></a>摘要
使用此索引標籤可取得備份資產的高階總覽。 您可以快速概覽備份項目總數、已取用的雲端儲存體總量、受保護的執行個體數目，以及每個工作負載類型的作業成功率。 如需特定備份成品類型的詳細資訊，請移至個別的索引標籤。

   ![摘要索引標籤](./media/backup-azure-configure-backup-reports/summary.png)

##### <a name="backup-items"></a>備份項目
使用此索引標籤可查看在備份專案層級使用之雲端儲存體的資訊和趨勢。 例如，如果您在 Azure VM 備份中使用 SQL，即可查看每個要備份的 SQL 資料庫所取用的雲端儲存體。 您也可選擇查看特定保護狀態的備份項目資料。 例如，選取索引標籤頂端的 [已停止保護] 圖格，可篩選下面所有的 Widget，只針對處於 [已停止保護] 狀態的備份項目顯示資料。

   ![備份項目索引標籤](./media/backup-azure-configure-backup-reports/backup-items.png)

##### <a name="usage"></a>使用量
使用此索引標籤可查看備份的金鑰計費參數。 此索引標籤上顯示的資訊屬於計費實體 (受保護的容器) 層級。 例如，在將 DPM 伺服器備份至 Azure 的情況下，您可檢視受保護的執行個體趨勢，以及 DPM 伺服器所取用的雲端儲存體。 同樣地，如果您在 Azure 備份中使用 SQL 或在 Azure 備份中使用 SAP HANA，此索引標籤會在內含這些資料庫的虛擬機器層級提供使用量相關資訊。
...
##### <a name="jobs"></a>工作
使用此索引標籤可查看作業的長時間執行趨勢，例如每天失敗的工作數，以及作業失敗的最大原因。 您可以同時在彙總層級和備份項目層級檢視此資訊。 在方格中選取特定備份專案，以檢視在所選時間範圍內，針對該備份項目所觸發的每項作業詳細資訊。

   ![作業索引標籤](./media/backup-azure-configure-backup-reports/jobs.png)

##### <a name="policies"></a>原則
使用此索引標籤可查看所有作用中原則的相關資訊，例如關聯的專案數，以及在給定原則下備份的專案所耗用的雲端儲存體總計。 選取特定原則，以檢視每個相關聯備份項目的資訊。

   ![原則索引標籤](./media/backup-azure-configure-backup-reports/policies.png)

##### <a name="optimize"></a>最佳化
使用此索引標籤可讓您查看備份的潛在成本優化機會。 以下是 [優化] 索引標籤目前提供深入解析的案例：

###### <a name="inactive-resources"></a>非使用中的資源
使用此視圖，您可以找出在很長的時間內未成功備份的備份專案。 這可能表示正在備份的基礎電腦已不再存在（因而導致備份失敗），或電腦發生一些問題，導致無法可靠地進行備份。 

若要查看非使用中的資源，請流覽至 [**優化**] 索引標籤，然後按一下 [非使用中**資源**] 磚 按一下此磚會顯示方格，其中包含所選範圍中所有非使用中資源的詳細資料。 根據預設，方格會顯示過去7天內沒有復原點的專案。 若要尋找不同時間範圍的非使用中資源，您可以調整索引標籤頂端的 [**時間範圍**] 篩選準則。

識別非使用中的資源之後，您可以流覽至該資源的 [備份專案] 儀表板或 [Azure 資源] 分頁，以進一步調查問題（任何適用的位置）。 根據您的案例而定，您可以選擇停止電腦的備份（如果它不存在），藉此儲存在受保護的實例成本上，或者您可以修正電腦中的問題，以確保能可靠地執行備份。

![優化索引標籤-非使用中資源](./media/backup-azure-configure-backup-reports/optimize-inactive-resources.png)

###### <a name="backup-items-with-a-large-retention-duration"></a>保留期間較長的備份專案
使用此視圖，您可以找出備份保留時間超過組織所需的專案。 

按一下 [**原則優化**] 磚後面接著 [**保留優化**] 磚，會顯示一個方格，其中包含每日、每週、每月或每年保留點（RP）保留期大於指定值的所有備份專案。 根據預設，方格會顯示所選範圍中的所有備份專案。 您可以使用篩選來進行每日、每週、每月和每年 RP 保留，以進一步篩選格線，並找出保留可能降低以節省備份儲存體成本的專案。

請注意，針對 SQL 和 SAP Hana 之類的資料庫工作負載，方格中所顯示的保留週期會對應到完整備份點的保留期限，而不是差異備份點。 同樣也適用于保留篩選準則。  
...
使用此視圖，您可以識別已針對每日完整備份設定的資料庫工作負載。 經常使用每日差異備份以及每週的完整備份，會更符合成本效益。 

按一下 [**原則優化**] 磚，然後按 [**備份排程優化**] 磚，就會顯示包含所有具有每日完整備份原則之資料庫的方格。 您可以選擇流覽至特定的備份專案，並將原則修改為使用每週完整備份的每日差異備份。

請注意，在索引標籤頂端的 [**備份管理類型**] 篩選器應已選取 [azure **Vm 中的 SQL** ] 和 [**在 azure vm 中 SAP Hana** ]，讓格線能夠如預期般顯示資料庫工作負載。
