...
使用此視圖，您可以找出在很長的時間內未成功備份的備份專案。 這可能表示正在備份的基礎電腦已不再存在 (，因而導致) 備份失敗，或電腦發生一些問題，導致無法可靠地進行備份。 
...
一旦識別出非使用中的資源之後，您可以流覽至 [備份專案] 儀表板或該資源的 [Azure 資源] 分頁，以進一步調查問題， (任何適用的) 。 視您的案例而定，您可以選擇停止電腦 (的備份（如果它已不存在）) 並刪除不必要的備份，藉此節省成本，或您可以修正電腦中的問題，以確保能可靠地執行備份。
...
按一下 [ **原則優化** ] 磚後面接著 [ **保留優化** ] 磚，會顯示一個方格，其中包含每日、每週、每月或每年保留點 (RP) 的保留時間大於指定值的所有備份專案。 根據預設，方格會顯示所選範圍中的所有備份專案。 您可以使用篩選來進行每日、每週、每月和每年 RP 保留，以進一步篩選格線，並找出保留可能降低以節省備份儲存體成本的專案。
