...
> [!NOTE]
> 針對 DPM 工作負載，使用者可能會看到與 [復原服務保存庫總覽] 索引標籤中所顯示的 [使用方式] 值（與 [復原服務保存庫 **總覽** ] 索引標籤中所示的 [匯總使用量] 值相較之下，每 (一部 DPM) 伺服器的 20 MB這項差異的考慮因素是，每一部註冊備份的 DPM 服務器都有相關聯的「中繼資料」資料來源，而不會顯示為報告的構件。
