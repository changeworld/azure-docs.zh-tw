...
ms.locfileid: "88190838"
---
# <a name="whats-new-in-microsoft-azure-backup-server-mabs"></a>Microsoft Azure 備份 Server (MABS 中的新功能) 
...
MABS V2 引進 [Modern Backup Storage](backup-mabs-add-storage.md) (mb) ，改善了儲存空間的使用和效能。 MBS 使用 ReFS 作為基礎的檔案系統，MBS 旨在利用混合式儲存體 (例如階層式儲存體)。

若要以 MB 達到規模和效能，我們建議使用 MABS V3 UR1 做為階層式磁片區的快閃)  (儲存體的整體儲存體) 的小型百分比 (4%，作為與 DPM HDD 儲存體結合的階層式磁片區。 MABS V3 UR1 with 分層式儲存體可提供50-70% 的備份速度。 如需設定階層式儲存區的步驟，請參閱 DPM 文章 [使用分層式儲存體設定 mb](/system-center/dpm/add-storage?view=sc-dpm-2019#set-up-mbs-with-tiered-storage) 。
...
>重大我們已找出備份重復資料刪除 ReFS 磁片區的幾個問題。 我們正在努力修正這些問題，且會在完成修正時更新此區段。 在那之前，我們會移除從 MABSv3 UR1 備份重復資料刪除 ReFS 磁片區的支援。
