...
10. 在 [指定線上備份排程]**** 頁面上，選取您慣用的排程，然後按 [下一步]****。
    ...
    > [!NOTE]
    > MABS 可以從當時可用的最新磁碟備份點開始，為 Azure 最多提供兩個每日備份。 Azure 備份也可以使用[Azure 備份網路節流](backup-windows-with-mars-agent.md#enable-network-throttling)，控制尖峰和離峰時間可用於備份的 WAN 頻寬數量。
    >
    >
