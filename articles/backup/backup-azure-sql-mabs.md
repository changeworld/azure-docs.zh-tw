---
title: 使用 Azure 備份伺服器備份 SQL 伺服器
description: 在本文中，瞭解使用 Microsoft Azure 備份伺服器 （MABS） 備份 SQL Server 資料庫的配置。
ms.topic: conceptual
ms.date: 03/24/2017
...
ms.locfileid: "77505956"
---
# <a name="back-up-sql-server-to-azure-by-using-azure-backup-server"></a>使用 Azure 備份伺服器將 SQL 伺服器備份到 Azure

本文可説明您使用 Microsoft Azure 備份伺服器 （MABS） 設置 SQL Server 資料庫的備份。

要備份 SQL Server 資料庫並從 Azure 恢復它，它：
...
開始之前，請確保已[安裝並準備了 Azure 備份伺服器](backup-azure-microsoft-azure-backup.md)。

## <a name="create-a-backup-policy"></a>建立備份原則  

要保護 Azure 中的 SQL Server 資料庫，請先創建備份策略：

1. 在 Azure 備份伺服器中，選擇**保護**工作區。
1. 選擇 **"新建"** 以創建保護組。

    ![在 Azure 備份伺服器中創建保護組](./media/backup-azure-backup-sql/protection-group.png)
1. 在起始頁上，查看有關創建保護組的指導。 然後選擇 **"下一步**"。
1. 對於保護組類型，選擇 **"伺服器**"。

    ![選擇伺服器保護組類型](./media/backup-azure-backup-sql/pg-servers.png)
1. 展開要備份的資料庫所在的 SQL Server 電腦。 您將看到可以從該伺服器備份的資料來源。 展開**所有 SQL 共用**，然後選擇要備份的資料庫。 在此示例中，我們選擇報表伺服器$MSDPM2012 和報表伺服器$MSDPM2012TempDB。 選取 [下一步]****。
...
1. 選取 [下一步]****。 MABS 顯示可用的總體存儲空間。 它還顯示了潛在的磁碟空間利用率。

    ![在 MABS 中設置磁片分配](./media/backup-azure-backup-sql/pg-storage.png)

    預設情況下，MABS 每個資料來源（SQL Server 資料庫）創建一個卷。 該卷用於初始備份副本。 在此配置中，邏輯磁片管理器 （LDM） 將 MABS 保護限制為 300 個數據源（SQL Server 資料庫）。 若要因應這項限制，請選取 [將資料共置在 DPM 存放集區中]****。 如果使用此選項，MABS 會對多個資料來源使用單個卷。 此設置允許 MABS 保護多達 2，000 個 SQL Server 資料庫。

    如果選擇 **"自動增長卷**"，則 MABS 可以隨著生產資料的增長考慮增加的備份量。 如果不選擇 **"自動增長卷**"，則 MABS 將備份存儲限制為保護組中的資料來源。
1. 如果您是管理員，您可以選擇**通過網路自動**傳輸此初始備份，並選擇傳輸時間。 或者選擇**手動**傳輸備份。 然後選擇 **"下一步**"。

    ![在 MABS 中選擇複本建立方法](./media/backup-azure-backup-sql/pg-manual.png)

    初始備份副本需要傳輸整個資料來源（SQL Server 資料庫）。 備份資料從生產伺服器（SQL Server 電腦）移動到 MABS。 如果此備份較大，則通過網路傳輸資料可能會導致頻寬擁塞。 因此，管理員可以選擇使用卸除式媒體**手動**傳輸初始備份。 或者，他們可以在指定時間**通過網路自動**傳輸資料。
...
1. 選擇何時運行一致性檢查。 然後選擇 **"下一步**"。
    ...
    MABS 可以對備份點的完整性運行一致性檢查。 它計算生產伺服器上的備份檔案的校驗和（本示例中的 SQL Server 電腦）和 MABS 中該檔的備份資料。 如果檢查發現衝突，則假定 MABS 中的備份檔案已損壞。 MABS 通過發送對應于校驗和不匹配的塊來修復備份的資料。 由於一致性檢查是一項性能密集型操作，因此管理員可以選擇計畫一致性檢查或自動運行一致性檢查。
...
1. 如果您是管理員，則可以選擇適合組織策略的備份計畫和保留原則。
    ...
    >
    > 使用資料保護管理器 （DPM） 在本地磁片備份完成後安排 Azure 備份。 遵循這種做法後，最新的磁片備份將複製到 Azure。
    >
