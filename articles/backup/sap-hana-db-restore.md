...
如需有關如何備份 SAP Hana 資料庫的詳細資訊，請參閱在[Azure vm 上備份 SAP Hana 資料庫](./backup-azure-sap-hana-database.md)。
...
1. 接著，依照下列步驟執行：
    ...
    * 還原到某個時間點：
        ...
        * `<DatabaseName@HostName>` - 用其備份來進行還原的資料庫名稱，以及此資料庫所在的**主機**/SAP Hana 伺服器名稱。 `USING SOURCE <DatabaseName@HostName>` 選項會指定資料備份 (用於還原) 源自 SID 或名稱與目標 SAP Hana 機器不同的資料庫。 因此，您不需要針對在備份建立所在的同一 HANA 伺服器上完成的還原進行指定。
