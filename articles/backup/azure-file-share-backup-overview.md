...
Azure 檔案共用備份是以快照集為基礎的解決方案，而快照集所產生的儲存體費用會根據[此處](https://azure.microsoft.com/pricing/details/storage/files/)所述的定價詳細資料，以 Azure 檔案儲存體使用量計費。

不過，利用備份解決方案的受保護實例費用是根據[Azure 檔案儲存體的備份](https://azure.microsoft.com/pricing/details/backup/)一節中所述的定價模式。 目前，只有美國中西部的實際價格已更新。 針對其他區域，將會在某些地區變化之後立即更新確切價格，但使用相同的計價模式。

>[!NOTE]
>現在沒有「受保護的實例費用」，您只需根據[此處](https://azure.microsoft.com/pricing/details/storage/files/)所述的定價向您收費。
