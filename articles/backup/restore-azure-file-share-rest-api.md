...
description: 瞭解如何使用 REST API 從 Azure 備份所建立的還原點還原 Azure 檔案共用或特定檔案
ms.topic: conceptual
ms.date: 02/17/2020
...
ms.locfileid: "88761792"
---
# <a name="restore-azure-file-shares-using-rest-api"></a>使用 REST API 來還原 Azure 檔案共用

本文說明如何使用 REST API，從 [Azure 備份](./backup-overview.md) 所建立的還原點還原整個檔案共用或特定檔案。

在本文結束時，您將瞭解如何使用 REST API 來執行下列作業：
...
我們假設您已經有想要還原的已備份檔案共用。 如果您沒有這麼做，請 [使用 REST API 查看備份 Azure 檔案共用](backup-azure-file-share-rest-api.md) ，以瞭解如何建立一個。
...
針對大部分的還原相關 API 呼叫，您必須傳遞 {容器值} 和 {protectedItemName} URI 參數的值。 在 [GET backupprotectableitems](/rest/api/backup/protecteditems/get) 作業的回應主體中使用 ID 屬性，以取得這些參數的值。 在我們的範例中，我們想要保護的檔案共用識別碼是：
...
若要還原任何已備份的檔案共用或檔案，請先選取復原點以執行還原作業。 您可以使用 [復原點清單](/rest/api/site-recovery/recoverypoints/listbyreplicationprotecteditems) REST API 呼叫來列出備份專案的可用復原點。 它是具有所有相關值的 GET 作業。
...
* {容器}： *storagecontainer; storage; azurefiles; afsaccount*
* {protectedItemName}： *azurefileshare; azurefiles*
...
### <a name="example-response-for-fetch-recovery-points"></a>提取復原點的範例回應
...
復原點是以上述回應中的 {name} 欄位來識別。

## <a name="full-share-recovery-using-rest-api"></a>使用 REST API 的完整共用復原

使用此還原選項可還原原始或替代位置中的完整檔案共用。
觸發還原是 POST 要求，您可以使用 [觸發程式還原](/rest/api/backup/restores/trigger) REST API 執行這項作業。

```http
POST https://management.azure.com/Subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.RecoveryServices/vaults/{vaultName}/backupFabrics/{fabricName}/protectionContainers/{containerName}/protectedItems/{protectedItemName}/recoveryPoints/{recoveryPointId}/restore?api-version=2019-05-13
```

值 {容器名稱} 和 {protectedItemName} 是在 [這裡](#fetch-containername-and-protecteditemname) 設定的，而 recoveryPointID 是上述復原點的 {name} 欄位。
...
如需要求本文的完整定義清單及其他詳細資料，請參閱觸發程式 [還原 REST API 檔](/rest/api/backup/restores/trigger#request-body)。

### <a name="restore-to-original-location"></a>還原至原始位置

#### <a name="request-body-example-for-restore-to-original-location"></a>還原至原始位置的要求主體範例
...
* **targetResourceId**：要還原備份內容的儲存體帳戶。 目標儲存體帳戶必須與保存庫位於相同位置。
* **名稱**：要還原備份內容之目標儲存體帳戶內的檔案共用。
* **targetFolderPath**：要還原資料之檔案共用底下的資料夾。

#### <a name="request-body-example-for-restore-to-alternate-location"></a>還原至替代位置的要求主體範例
...
觸發還原作業是 [非同步作業](../azure-resource-manager/management/async-operations.md)。 這種作業會建立另一個需要個別追蹤的作業。
它會傳回兩個回應： 202 (在建立另一個作業時接受) ，而當該作業完成時，會) 200 (OK。

#### <a name="response-example"></a>回應範例

一旦您提交要觸發還原的 *POST* URI 之後，初始回應會是 202 (已接受的) 具有位置標頭或 Azure 非同步標頭。
...
### <a name="create-request-body-for-item-level-recovery-using-rest-api"></a>使用 REST API 建立專案層級復原的要求主體
...
### <a name="restore-to-original-location-for-item-level-recovery-using-rest-api"></a>使用 REST API 還原至原始位置以進行專案層級復原

下列要求主體是在*afsaccount*儲存體帳戶的*azurefiles*檔案共用中還原*Restoretest.txt*檔案。
...
### <a name="restore-to-alternate-location-for-item-level-recovery-using-rest-api"></a>使用 REST API 還原至專案層級復原的替代位置

下列要求本文會將*afsaccount*儲存體帳戶中*azurefiles*檔案共用中的*Restoretest.txt*檔案還原至*afaccount1*儲存體帳戶中*azurefiles1*檔案共用的*restoredata*資料夾。
...
回應的處理方式應該與上面所述相同，以進行 [完整的共用還原](#full-share-recovery-using-rest-api)。
