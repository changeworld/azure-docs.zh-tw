...
| 計數器 | 狀態 |
| --- | --- |
| 邏輯磁碟 (實體磁碟)--% 閒置 |<li> 100% 閒置至50% 閒置 = 狀況良好</br><li> 49% 閒置到20% 閒置 = 警告或監視</br><li> 19% 閒置至0% 閒置 = 重大或超出規範 |
| 邏輯磁碟 (實體磁碟)--% 平均磁碟的讀取或寫入秒數 |<li> 0.001 毫秒到0.015 毫秒 = 狀況良好</br><li> 0.015 ms 到 0.025 ms = 警告或監視</br><li> 0.026 毫秒或更長 = 重大或超出規範 |
| 邏輯磁碟 (實體磁碟)--(所有執行個體) 目前的磁碟佇列長度 |80 個要求超過 6 分鐘 |
| 記憶體--集區未分頁位元組 |<li> 小於60% 的集區已耗用 = 狀況良好<br><li> 61% 到80% 的已耗用集區 = 警告或監視</br><li> 大於80% 的集區已耗用 = 重大或超出規範 |
| 記憶體--集區分頁位元組 |<li> 小於60% 的集區已耗用 = 狀況良好</br><li> 61% 到80% 的已耗用集區 = 警告或監視</br><li> 大於80% 的集區已耗用 = 重大或超出規範 |
| 記憶體--可用 MB 數 |<li> 50% 的可用記憶體可用或更多 = 狀況良好</br><li> 可用記憶體有 25% = 監視</br><li>可用記憶體有10% 的可用記憶體 = 警告</br><li> 小於 100 MB 或可用記憶體的 5% = 重大或超出規範 |
| 處理器--\% 處理器時間 (所有執行個體) |<li> 小於60% 已耗用 = 狀況良好</br><li> 已耗用61% 到 90% = 監視器或警告</br><li> 91% 到100% 已耗用 = 重大 |
...
如果您嘗試備份大型磁片，則建議使用第一次備份 (初始複寫) 的 [Azure 資料箱](./offline-backup-azure-data-box.md) 。  如果您無法使用資料箱，則在透過網路進行長時間資料傳輸期間，環境中發生的任何暫時性網路問題都會導致備份失敗。  若要避免這些失敗，您可以將幾個資料夾新增至初始備份，並持續以增量方式新增更多資料夾，直到所有資料夾都成功備份至 Azure 為止。  後續的增量備份將會相對較快。
