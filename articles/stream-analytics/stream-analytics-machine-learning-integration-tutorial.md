...
description: 本文說明如何使用使用者定義的函數，快速設定整合 Azure Machine Learning 的簡單 Azure 串流分析作業。
author: mamccrea
ms.author: mamccrea
...
本文說明如何設定簡單的 Azure 串流分析作業，使用 Azure Machine Learning Studio (傳統) 進行情感分析。 您可以使用 Cortana 智慧資源庫中的 Machine Learning 情感分析模型，來分析串流文字資料並判斷情感分數。
...
* 其中有一些 Twitter 資料的 CSV 檔案。 您可以從 [GitHub](https://github.com/Azure/azure-stream-analytics/blob/master/Sample%20Data/sampleinput.csv)下載範例檔案，也可以建立您自己的檔案。 在真實世界的案例中，您會直接從 Twitter 資料流取得資料。

## <a name="create-a-storage-container-and-upload-the-csv-input-file"></a>建立儲存體容器並上傳 CSV 輸入檔

在此步驟中，您會將 CSV 檔案上傳至儲存體容器。

1. 在 [Azure 入口網站中，選取 [**建立資源**] [儲存體] [  >  **儲存體**  >  **帳戶**]。

2. 在 [ *基本* ] 索引標籤中填入下列詳細資料，並保留其餘欄位的預設值：
...
6. 若要驗證情感分析模型是否已備妥可供使用，請選取 [ **測試** ] 按鈕。 提供文字輸入，例如「我喜歡 Microsoft」。
...
7. 在 [ **應用程式** ] 欄中，選取 [ **excel 2010 或舊版活頁簿** ] 連結以下載 excel 活頁簿。 活頁簿包含您稍後設定串流分析作業所需的 API 金鑰和 URL。
...
2. 瀏覽至您的串流分析作業。 然後選取 **[** 函式] 和 [  >  **新增**  >  **Azure ML Studio** ]

3. 在 **Azure Machine Learning** 函式表單中填寫下列值：

   |欄位  |值  |
   |---------|---------|
   | 函式別名 | 使用 [名稱] `sentiment` ，然後選取 [ **手動提供 Azure Machine Learning 函數設定**]，這會提供您輸入 URL 和金鑰的選項。      |
   | URL| 貼上 Web 服務 URL。|
   |答案 | 貼上 API 金鑰。 |
...
3. 輸入下列查詢：
   ...
   此查詢會叫用您稍早建立的函式 (`sentiment`) ，在輸入中的每個推文上執行情感分析。
...
3. 在 [ **啟動作業**] 中，選取 [ **自訂**]，然後選取您將 CSV 檔案上傳至 blob 儲存體之前的一天。 完成時，選取 [啟動]****。  

### <a name="check-the-output"></a>查看輸出

1. 讓工作執行幾分鐘，直到您在 [監視]**** 方塊中看到活動。

2. 如果您有一項通常用來檢查 blob 儲存體內容的工具，請使用該工具來檢查容器。 或者，請在 Azure 入口網站中執行下列步驟：

      1. 在 [Azure 入口網站中，尋找您的儲存體帳戶，然後在帳戶中尋找容器。 您會在容器中看到兩個檔案：包含範例推文的檔案，和串流分析工作所產生的 CSV 檔案。
...
您也能檢視與 Azure Machine Learning 函數相關的度量。 下列與函數相關的度量會顯示在作業總覽的 [ **監視** ] 方塊中：
