...
> [!NOTE]
> 以程式控制方式建立的串流分析工作預設不會啟用監視功能。  您可以在 Azure 入口網站中流覽至作業的 [監視] 頁面，然後按一下 [啟用] 按鈕來手動啟用監視，或是遵循[Azure 串流分析監視器串流分析作業以程式](stream-analytics-monitor-jobs.md)設計方式來執行此動作。
> 
> 
...
如果您指定已經存在且未指定-Force 參數的輸入，Cmdlet 會詢問是否要取代現有的輸入。

如果您指定-Force 參數，並指定現有的輸入名稱，則會取代輸入而不進行確認。
...
如果您指定-Force 參數，並指定現有的作業名稱，則會取代作業定義而不進行確認。
...
如果您指定-Force 參數，並指定現有的輸出名稱，則會取代輸出而不進行確認。
...
如果您指定已存在的轉換，且未指定-Force 參數，此 Cmdlet 會詢問是否要取代現有的轉換。
