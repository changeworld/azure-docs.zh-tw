...
強烈建議為所有作業啟用診斷日誌，因為這將極大地説明調試和監視。
...
> [!NOTE]
> 可以使用 Azure 存儲、Azure 事件中心和 Azure 監視器日誌等服務來分析不符合項資料。 這些服務將會依據各自的計價模式向您收取費用。
...
4. 向下捲動至 JSON 的 [屬性]**** 區段，該區段提供導致作業失敗的錯誤的詳細資料。 在此範例中，失敗是由於超出範圍的緯度值的執行階段錯誤。 流分析工作處理的資料差異會導致資料錯誤。 您可以瞭解不同的[輸入和輸出資料錯誤及其發生的原因](https://docs.microsoft.com/azure/stream-analytics/data-errors)。
...
6. 如果活動日誌中的錯誤訊息對識別根本原因沒有説明，則啟用診斷日誌並使用 Azure 監視器日誌。

## <a name="send-diagnostics-to-azure-monitor-logs"></a>將診斷發送到 Azure 監視器日誌
...
3. 當您的 Stream Analytics 作業啟動時，診斷記錄會路由傳送至 Log Analytics 工作區。 要查看作業的診斷日誌，請在 **"監視**"部分下選擇 **"日誌**"。
   ...
   ![監控下的診斷日誌](./media/stream-analytics-job-diagnostic-logs/diagnostic-logs.png)

4. 流分析提供預定義的查詢，使您能夠輕鬆搜索您感興趣的日誌。 這3個類別是**常規**、**輸入資料錯誤**和**輸出資料錯誤**。 例如，要查看過去 7 天內作業的所有錯誤的摘要，可以選擇 **"運行**適當的預定義查詢"。 

   ![監控下的診斷日誌](./media/stream-analytics-job-diagnostic-logs/logs-categories.png)

   ![日誌結果](./media/stream-analytics-job-diagnostic-logs/logs-result.png)
...
operationName | 記錄的作業名稱。 例如，**發送事件：SQL 輸出寫入失敗到 mysql輸出**。
...
執行記錄包含在執行串流分析作業期間所發生事件的資訊。 屬性的架構因事件是資料錯誤還是泛型事件而異。
...
* [流分析資料錯誤](https://docs.microsoft.com/azure/stream-analytics/data-errors)
