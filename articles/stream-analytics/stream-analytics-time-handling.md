...
description: 瞭解時間處理在 Azure 流分析中的工作方式，例如如何選擇最佳開始時間、如何處理早期事件以及時間處理指標。
author: mamccrea
ms.author: mamccrea
...
- **浮水印**：一個事件時間標記，指示事件已進入流處理器的點。 浮水印讓系統明確指出擷取事件的進度。 依據資料流的特質，內送事件資料永遠不會停止，因此浮水印可指出資料流中某個時間點的進度。
...
2. **應用時間** (也稱為事件時間)
   ...
   產生事件時會指派應用時間，且會成為事件承載的一部分。 要按應用程式時間處理事件，請在選取查詢中使用**時間戳記到**子句。 如果 **Timestamp by** 子句不存在，將按照抵達時間處理事件。
...
- [Azure 流分析事件順序注意事項](stream-analytics-out-of-order-and-late-events.md)
