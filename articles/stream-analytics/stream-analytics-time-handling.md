...
description: 了解時間處理在 Azure 流分析中的工作方式,例如如何選擇最佳開始時間、如何處理早期事件以及時間處理指標。
author: mamccrea
ms.author: mamccrea
...
- **處理時間**:事件到達處理系統並觀察到的時間。 例如，當收費站感應器發現車輛，且電腦系統以所需時間處理資料時。

- **水印**:一個事件時間標記,指示事件已進入流處理器的點。 浮水印讓系統明確指出擷取事件的進度。 依據資料流的特質，內送事件資料永遠不會停止，因此浮水印可指出資料流中某個時間點的進度。
...
2. **應用時間** (也稱為事件時間)
   ...
   產生事件時會指派應用時間，且會成為事件承載的一部分。 要按應用程式時間處理事件,請在選擇查詢中使用**時間戳到**子句。 如果 **Timestamp by** 子句不存在，將按照抵達時間處理事件。
...
然後，串流分析會從查詢規格衍生開始時間。 不過，由於輸入事件的訊息代理程式僅依據抵達時間編製索引，因此系統必須將起始事件時間轉譯為抵達時間。 系統可以從輸入事件訊息代理程式中的那個時間點開始處理事件。 使用早期抵達時間範圍限制時，轉譯是很簡單的。 這是開始事件時間減去 5 分鐘提前到達視窗。 此計算還意味著系統丟棄所有被視為具有事件時間比到達時間更長時間的事件。
...
| **浮水印延遲秒數** | 指出串流資料處理作業的延遲。 請參閱下一節的詳細說明。|
...
在其他串流解決方案中，輸出事件可根據外部情況在不同的觸發點具體化。 在某些解決方案中,給定時間視窗的輸出事件可能會多次生成。 隨著輸入值的調整，彙總結果會變得更精確。 事件一開始可能是推測的，其後再逐漸修訂。 例如，當一個裝置未連接網路時，系統可能會使用預估值。 其後，該裝置連上了網路。 此時，就可將實際的事件資料包含在輸入資料流中。 處理該時間範圍的輸出結果會產生更精確的輸出。
...
1. 下圖顯示浮水印處理這些事件的進程：
   ...
   5. 處理第十二個事件 (device3) 時,到達時間(12:27)比啟用時間(12:21)早 6 分鐘。 此時適用延遲抵達原則。 事件時間調整為 (12:22) 而高於浮水印 (12:21)，因此無需進一步調整。
...
- [Azure 串流分析事件順序注意事項](stream-analytics-out-of-order-and-late-events.md)
