...
2. **應用時間** (也稱為事件時間)
   ...
   產生事件時會指派應用時間，且會成為事件承載的一部分。 若要依應用程式時間處理事件，請在選取查詢中使用**Timestamp by**子句。 如果 **Timestamp by** 子句不存在，將按照抵達時間處理事件。
...
然後，串流分析會從查詢規格衍生開始時間。 不過，由於輸入事件的訊息代理程式僅依據抵達時間編製索引，因此系統必須將起始事件時間轉譯為抵達時間。 系統可以從輸入事件訊息代理程式中的那個時間點開始處理事件。 使用早期抵達時間範圍限制時，轉譯是很簡單的。 這是起始事件時間減去5分鐘的早期抵達時間範圍。 這種計算也表示系統會卸載事件時間5分鐘之前超過抵達時間的所有事件。
...
- [Azure 串流分析事件順序考慮](stream-analytics-out-of-order-and-late-events.md)
