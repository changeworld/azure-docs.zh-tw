...
滑動視窗功能與"翻轉"或"跳躍"視窗不同，**僅在**事件發生時生成輸出。 每個時間範圍都至少會有一個事件，而且時間範圍會持續依據 € (epsilon) 向前移動。 如同跳動時間範圍，事件可以屬於一個以上的滑動時間範圍。
