...
建議使用小於 300 MB 的參考資料集，以獲得最佳效能。 具有6個 su 或更多的作業支援使用大於 300 MB 的參考資料。 這項功能目前為預覽狀態，不能用在生產環境中。 使用非常大的參考資料可能會影響作業的效能。 當查詢的複雜性增加以包含具狀態處理 (例如視窗型彙總、時態性聯結和時態性分析函式) 時，預期可支援的參考資料大小上限會減少。 如果 Azure 串流分析無法載入參考資料，並且執行複雜的作業，則作業將會用完記憶體並發生失敗。 在這種情況下，SU % 使用計量會達到 100%。    

|**串流單位數量**  |**建議的大小**  |
|---------|---------|
|1   |50 MB 或更低   |
|3   |150 MB 或更低   |
|6 和以上   |300 MB 或更低的版本。 預覽中支援使用大於 300 MB 的參考資料，而且可能會影響作業的效能。    |
