...
您的串流分析作業可以根據*事件時間*或*抵達時間*來處理事件。 事件 **/應用時間**是事件裝載中的時間戳記， (事件產生時) 。 **抵達時間**是在輸入來源收到事件時的時間戳記 (事件中樞/IoT 中樞/Blob 儲存體) 。 
...
有時候事件會因為各種原因而延遲抵達。 例如，延遲40秒的事件會有事件時間 = 00:10:00，而抵達時間 = 00:10:40。 如果您將延遲抵達原則設定為15秒，則超過15秒的任何事件都會被捨棄 (不是由串流分析) 處理，或已調整其事件時間。 在上述範例中，當事件抵達40秒 (超過原則設定) 時，其事件時間將會調整為最大延遲抵達原則 00:10:25 (抵達時間-) 的延遲抵達原則值。 預設的延遲抵達原則為5秒。
...
如果事件是根據您已設定的原則，以延遲或不按照順序抵達，您可以卸載這類事件 (不是由串流分析) 處理，或已調整其事件時間。
...
是。 根據預設，依順序排列的原則會設定為零 (00 分鐘和00秒的) 。 如果您變更預設值，則作業的第一個輸出會延遲 (或更大的) 。 
...
## <a name="why-do-i-see-a-delay-of-5-seconds-even-when-my-late-arrival-policy-is-set-to-0"></a>為什麼當我的延遲抵達原則設定為0時，仍會看到5秒的延遲？
當輸入資料分割從未收到任何輸入時，就會發生這種情況。 您可以依資料分割來驗證輸入計量，以驗證此行為。 

當分割區沒有超過設定的延遲抵達閾值的任何資料時，串流分析會將應用程式時間戳記前移，如事件排序考慮一節中所述。 這需要預估的抵達時間。 如果分割區沒有任何資料，則串流分析會將抵達時間預估為*本機時間-5 秒*。 由於此分割區永遠不會有任何資料，因此可能會顯示5秒的浮水印延遲。  
