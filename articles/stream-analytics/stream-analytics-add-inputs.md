...
這些輸入資源可以與流分析作業位於同一 Azure 訂閱中，也可以從其他訂閱中使用。

您可以使用 Azure[門戶](stream-analytics-quick-create-portal.md#configure-job-input)[、Azure PowerShell](https://docs.microsoft.com/powershell/module/az.streamanalytics/New-azStreamAnalyticsInput) [、.NET](https://docs.microsoft.com/dotnet/api/microsoft.azure.management.streamanalytics.inputsoperationsextensions) [API、REST API](https://docs.microsoft.com/rest/api/streamanalytics/stream-analytics-input)和[視覺化工作室](stream-analytics-tools-for-visual-studio-install.md)創建、編輯和測試流分析作業輸入。
...
串流分析也支援稱為「參考資料」** 的輸入。 參考資料是完全靜態或緩慢變更的資料。 這些資料通常用來執行相互關聯和查閱。 比方說，您可能會將資料流輸入中的資料聯結至參考資料中的資料，很像是執行 SQL 聯結來查詢靜態值。 Azure Blob 存儲和 Azure SQL 資料庫當前被視為參考資料的輸入源。 參考資料源 blob 的大小限制高達 300 MB，具體取決於查詢複雜性和分配的流式處理單元（有關詳細資訊，請參閱參考資料文檔[的大小限制](stream-analytics-use-reference-data.md#size-limitation)部分）。
