...
* 安裝[.NET Core CLI](https://docs.microsoft.com/dotnet/core/tools/?tabs=netcore2x)。
...
1. 從網頁瀏覽器，移至 [Twitter For Developers](https://developer.twitter.com/en/apps)，然後選取 [Create an app (建立應用程式)]。 您可能會看到一則訊息，指出您必須申請 Twitter 開發人員帳戶。 您可以隨意執行此動作，在您的應用程式經過核准之後，您應該會看到一封確認電子郵件。 核准開發人員帳戶可能需要幾天的時間。
...
2. 使用文字編輯器來開啟*app.config*檔案。 對 `<appSettings>` 元素進行下列變更：
   ...
   * 將 `oauth_consumer_key` 設定為 Twitter 取用者金鑰（API 金鑰）。 
   * 將 `oauth_consumer_secret` 設定為 Twitter 取用者秘密（API 秘密金鑰）。
   * 將 `oauth_token` 設定為 Twitter 存取權杖。
   * 將 `oauth_token_secret` 設為 Twitter 存取權杖秘密。
...
2. 選取 [ **+&nbsp;新增串流輸入** > **事件中樞**]。 在新的**輸入**表單中填寫下列資訊：
...
2. 在 [**輸出**] 頁面中，按一下 [ **+] &nbsp;新增** **Blob 儲存體/Data Lake Storage Gen2**：
