...
| 元素 | 必要 | 描述 | 
|---------|----------|-------------|
| **時間** | 否 | [ISO 8601 格式](https://en.wikipedia.org/wiki/ISO_8601) \(英文\) 的日期時間字串值，會在基本排程中指定作業第一次啟動的時機。 <p>針對複雜的排程，作業一到 **startTime** 就會啟動。 | 
| **定期** | 否 | 適用於作業執行時機的週期規則。 **recurrence** 物件支援下列元素：**frequency**、**interval**、**schedule**、**count** 及 **endTime**。 <p>如果您使用 **recurrence** 元素，也必須使用 **frequency** 元素，而其他 **recurrence** 元素均為選擇性的。 |
...
| **任務** | 否 | 根據分鐘標記、小時標記、星期幾和每月執行日來定義週期的變更 | 
