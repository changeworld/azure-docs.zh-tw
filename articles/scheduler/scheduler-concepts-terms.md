...
* 當您有現有服務或工作負載的端點時，HTTP 工作（包括支援 TLS 的 HTTPS 作業）
...
| **分鐘** | 否 | 0到59 | 包含分鐘標記的陣列，表示要執行作業的時間 | 
...
| **計數** | 否 | <*沒有*> | 循環次數。 預設為無限制地重複。 您無法同時使用 **count** 和 **endTime**，但會接受先完成的規則。 | 
| **endTime** | 否 | <*沒有*> | 停止循環的日期和時間。 預設為無限制地重複。 您無法同時使用 **count** 和 **endTime**，但會接受先完成的規則。 | 
||||
