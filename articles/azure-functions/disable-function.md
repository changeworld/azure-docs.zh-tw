...
禁用函數的建議方法是使用格式`AzureWebJobs.<FUNCTION_NAME>.Disabled`的應用設置。 您可以通過多種方式創建和修改此應用程式設定，包括使用[Azure CLI](/cli/azure/)和[Azure 門戶](https://portal.azure.com)中的函數**的"管理**"選項卡。 
...
在 Azure CLI 中，[`az functionapp config appsettings set`](/cli/azure/functionapp/config/appsettings#az-functionapp-config-appsettings-set)使用 命令創建和修改應用設置。 以下命令通過創建名為 的應用`QueueTrigger``AzureWebJobs.QueueTrigger.Disabled`設置將其設置為`true`禁用名為 的函數。 
...
雖然建議對所有語言和所有執行階段版本使用應用程式設定方法，但還有幾種其他方法來禁用函數。 這些方法因語言和執行階段版本而異，為向後相容性而維護這些方法。 

### <a name="c-class-libraries"></a>C# 類別庫

在類庫函數中，還可以使用 屬性`Disable`來防止觸發該函數。 您可以使用此屬性 (沒有建構函式參數)，如下列範例所示：
...
在版本 1.x 中，您還可以使用`disabled`*函數.json*檔的屬性告訴運行時不要觸發函數。 此方法僅適用于指令碼語言，如 C# 腳本和 JavaScript。 屬性`disabled`可以設置為`true`應用設置或應用設置的名稱：
...
您可以在 Azure 門戶中編輯檔，或使用函數的 **"管理"** 選項卡上的 **"功能狀態**"開關。門戶交換器的工作原理是更改*函數.json*檔。
