...
1. 保留_身分識別_、_自動關閉_，以及_備份_區段的預設值。
...
1. 在_存取限制_頁面上，您只會看到預設限制。 預設不會對函式應用程式的存取施加任何限制。  選取 [新增規則]，以建立私人網站存取限制組態。
1. 在 [新增存取限制] 窗格中，為新規則提供 [名稱]、[優先順序] 和 [描述]。
1. 從 [類型] 下拉式方塊中選取 [虛擬網路]、選取先前建立的虛擬網路，然後選取 [教學課程] 子網路。 
    > [!NOTE]
    > 可能需要幾分鐘的時間來啟用服務端點。
1. _存取限制_頁面現在會顯示有新的限制。 可能需要幾秒鐘的時間，[端點狀態] 才會從 [已停用] 變更為 [佈建中] 再變更為 [已啟用]。
