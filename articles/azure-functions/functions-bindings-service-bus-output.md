...
description: 瞭解如何從 Azure 函數發送 Azure 服務匯流排消息。
...
ms.locfileid: "79277436"
---
# <a name="azure-service-bus-output-binding-for-azure-functions"></a>Azure 服務匯流排輸出綁定用於 Azure 函數
...
下面的示例演示如何在 Python 中寫入服務匯流排佇列。

服務匯流排綁定定義在*函數.json*中定義，其中*類型*設置為`serviceBus`。
...
在`set`init * \_.py 中，可以通過將值傳遞給 方法來向佇列寫入消息。 _ \_ _ *
...
下面的示例顯示了一個 JAVA 函數，該函數在 HTTP`myqueue`請求觸發時向服務匯流排佇列發送消息。
...
JAVA 函數還可以寫入服務匯流排主題。 下面的示例使用`@ServiceBusTopicOutput`注釋來描述輸出綁定的配置。 
...
和`ServiceBusQueueOutput``ServiceBusTopicOutput`注釋可用於將消息寫入函數輸出。 用這些注釋修飾的參數必須聲明為與`OutputBinding<T>`訊息類型對應`T`的類型所在的參數。

---

## <a name="configuration"></a>組態

下表介紹了您在*函數.json*檔和`ServiceBus`屬性中設置的綁定配置屬性。
...
* `out BrokeredMessage`- 如果參數值在函數退出時為空，則函數不會創建消息（對於函數 1.x）
* `out Message`- 如果參數值在函數退出時為空，則函數不會創建消息（對於函數 2.x 和更高版本）
* `ICollector<T>` 或 `IAsyncCollector<T>` - 適用於建立多個訊息。 當您呼叫 `Add` 方法時，就會建立一則訊息。

使用 C# 函數時：

* 非同步函數需要傳回值或`IAsyncCollector`參數。 `out`

* 要訪問會話 ID，請使用 類型[`Message`](https://docs.microsoft.com/dotnet/api/microsoft.azure.servicebus.message)並使用 屬性`sessionId`。
...
使用`context.bindings.<name from function.json>`訪問佇列或主題。 您可以將字串、位元組陣列或 JavaScript 物件（序列化為 JSON）分配給`context.binding.<name>`。

# <a name="python"></a>[Python](#tab/python)

使用[Azure 服務匯流排 SDK](https://docs.microsoft.com/azure/service-bus-messaging)而不是內置輸出綁定。
...
本節介紹 2.x 和更高版本中可用於此綁定的全域配置設置。 下面的示例 host.json 檔僅包含此綁定的設置。 有關全域配置設置的詳細資訊，請參閱[Azure 函數版本的 host.json 引用](functions-host-json.md)。
