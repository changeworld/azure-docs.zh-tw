...
請考慮此範例，說明 always ready 實例和預先準備就緒的實例如何一起運作。  高階函式應用程式已設定五個永遠就緒的實例，以及一個預熱實例的預設值。  當應用程式閒置且沒有觸發任何事件時，就會在五個實例上布建和執行應用程式。  

第一個觸發程式進入之後，五個永遠就緒的實例就會變成作用中狀態，並配置額外的預先準備就緒實例。  應用程式現在會使用六個已布建的實例來執行：五個目前作用中的 always ready 實例，以及第六個預先準備就緒和非使用中的緩衝區。  如果執行率持續增加，則最後會使用五個使用中的實例。  當平臺決定調整超過五個實例時，會將其調整為預先準備就緒的實例。  發生這種情況時，現在會有六個使用中的實例，而且會立即布建第七個實例，並填入預先準備就緒的緩衝區。  這一系列的調整和預先預熱將繼續進行，直到達到應用程式的最大實例計數為止。  沒有任何實例會預先準備就緒或啟用超過最大值。
...
如果您的應用程式需要的實例超出 always ready 實例，它可以繼續擴充，直到實例數目達到最高高載限制為止。  只有在您的方案大小超過方案規模的情況下，才會向您收取費用。  我們會盡力將您的應用程式相應放大至其定義的上限。
...
建立或調整您的方案時，您可以選擇三種實例大小。  您將會支付每秒耗用的核心總數和記憶體總數。  您的應用程式可以視需要自動相應放大至多個實例。  
