...
您可使用建構函式插入，讓相依性可供函式使用。 使用建構函式插入時無需使用靜態類別。

下列範例示範如何將 `IMyService` 和 `HttpClient` 相依性插入 HTTP 觸發的函式中。 此範例會使用在啟動時註冊 `HttpClient` 所需的 [Microsoft.Extensions.Http](https://www.nuget.org/packages/Microsoft.Extensions.Http/) 套件。
...
- **單一**：單一服務存留期符合主機存留期，並會在該執行個體上跨函式執行重複使用。 建議連線和用戶端使用單一存留期服務，例如 `SqlConnection` 或 `HttpClient` 執行個體。

請前往 GitHub 檢視或下載[不同服務存留期的範本](https://aka.ms/functions/di-sample)。

## <a name="logging-services"></a>記錄服務

如果需要自己的記錄提供者，請將自訂類型註冊為 `ILoggerProvider` 執行個體。 Azure Functions 會自動新增 Application Insights。
...
主機會將 `ILogger<T>` 和 `ILoggerFactory` 服務插入建構函式中。  不過，預設會從函式記錄篩選掉這些新的記錄篩選。  您必須修改 `host.json` 檔案，以加入其他篩選和類別。  下列範例示範如何使用將由主機公開的記錄來新增 `ILogger<HttpTrigger>`。
...
> [!WARNING]
> 避免嘗試從取用方案上的 *local.settings.json* 或 *appsettings.{environment}.json* 等檔案讀取值。 因為裝載基礎結構無法存取設定資訊，所以無法在應用程式調整時從這些與觸發程序連線相關的檔案讀取值。
