...
使用 Core Tools 在本機電腦上開發函式並將其發佈至 Azure，會遵循下列基本步驟：

> [!div class="checklist"]
> * [安裝 Core Tools 和相依性。](#v2)
> * [從特定語言的範本建立函數應用程式專案。](#create-a-local-functions-project)
> * [註冊觸發程式和系結延伸模組。](#register-extensions)
> * [定義儲存體和其他連接。](#local-settings-file)
> * [從觸發程式和特定語言的範本建立函數。](#create-func)
> * [在本機執行函數。](#start)
> * [將專案發佈至 Azure。](#publish)

## <a name="core-tools-versions"></a>Core Tools 版本

有三種版本的 Azure Functions Core Tools。 您使用的版本取決於您的本機開發環境、 [選擇的語言](supported-languages.md)，以及所需的支援層級：
...
3.x/2.x 版的工具會使用以 .NET Core 為基礎的 Azure Functions 執行時間。 .NET Core 支援的所有平臺（包括 [Windows](?tabs=windows#v2)、 [macOS](?tabs=macos#v2)和 [Linux](?tabs=linux#v2)）都支援此版本。 

> [!IMPORTANT]
> 您可以使用 [延伸]模組套件組合來略過安裝 .NET Core SDK 的需求。

# <a name="windows"></a>[Windows](#tab/windows)

下列步驟會使用 Windows installer (MSI) 來安裝 Core Tools v3. x。 如需安裝 Core Tools 2.x 所需的其他套件型安裝程式的詳細資訊，請參閱 [Core tools 讀我檔案](https://github.com/Azure/azure-functions-core-tools/blob/master/README.md#windows)。

1. 根據您的 Windows 版本，下載並執行 Core Tools 安裝程式：

    - [v3. x-建議使用 Windows 64](https://go.microsoft.com/fwlink/?linkid=2135274) 位 (。 [Visual Studio Code 的調試](functions-develop-vs-code.md#debugging-functions-locally) 需要64位。 ) 
    - [v3. x-Windows 32 位](https://go.microsoft.com/fwlink/?linkid=2135275)

1. 如果您不打算使用 [延伸](functions-bindings-register.md#extension-bundles)模組套件組合，請安裝 [適用于 Windows 的 .NET Core 3.x SDK](https://dotnet.microsoft.com/download)。
...
1. 安裝 Core Tools 套件：
    ...
    ##### <a name="v3x-recommended"></a> (建議) 的 v3. x
...
1. 如果您不打算使用 [延伸](functions-bindings-register.md#extension-bundles)模組套件組合，請安裝 [適用于 MacOS 的 .NET Core 3.x SDK](https://dotnet.microsoft.com/download)。
...
1. 安裝 Microsoft 套件存放庫 GPG 金鑰，以驗證套件完整性：
...
1. 檢查檔案中 `/etc/apt/sources.list.d/dotnetdev.list` 列出的其中一個適當的 Linux 版本字串：
...
1. 如果您不打算使用 [延伸](functions-bindings-register.md#extension-bundles)模組套件組合，請安裝 [適用于 Linux 的 .NET Core 3.x SDK](https://dotnet.microsoft.com/download)。
...
>[!IMPORTANT]
> JAVA 使用 Maven 原型來建立本機函式專案，以及您的第一個 HTTP 觸發函式。 使用下列命令來建立您的 JAVA 專案： `mvn archetype:generate -DarchetypeGroupId=com.microsoft.azure -DarchetypeArtifactId=azure-functions-archetype` 。 如需使用 Maven 原型的範例，請參閱 [命令列快速入門](./create-first-function-cli-java.md)。  
...
`func init` 支援下列選項，也就是 2.x/x-only 版，除非另有注明：

| 選項     | 描述                            |
| ------------ | -------------------------------------- |
| **`--csx`** | 將 .NET 函式建立為 c # 腳本，也就是2.x 版的行為。 僅適用于 `--worker-runtime dotnet` 。 |
| **`--docker`** | 使用根據所選的基底映射，建立容器的 Dockerfile `--worker-runtime` 。 如果您要發佈至自訂 Linux 容器，請使用此選項。 |
| **`--docker-only`** |  將 Dockerfile 加入至現有的專案。 如果未指定或未在 local.settings.js中設定，則會提示輸入背景工作執行時間。 當您打算將現有的專案發行至自訂 Linux 容器時，請使用此選項。 |
| **`--force`** | 即使專案中有現有的檔案，仍初始化專案。 此設定會以相同的名稱覆寫現有的檔案。 專案資料夾中的其他檔案不會受到影響。 |
| **`--language`** | 初始化特定語言的專案。 當 `--worker-runtime` 設定為時，目前支援 `node` 。 選項為 `typescript` 和 `javascript` 。 您也可以使用 `--worker-runtime javascript` 或 `--worker-runtime typescript` 。  |
| **`--managed-dependencies`**  | 安裝受管理的相依性。 目前，只有 PowerShell 背景工作執行時間支援此功能。 |
| **`--source-control`** | 控制是否要建立 Git 存放庫。 依預設不會建立存放庫。 設為 `true` 時，就會建立存放庫。 |
| **`--worker-runtime`** | 設定專案的語言執行階段。 支援的值為： `csharp` 、 `dotnet` 、 `javascript` 、 `node` (JavaScript) 、、 `powershell` `python` 和 `typescript` 。 若為 JAVA，請使用 [Maven](functions-reference-java.md#create-java-functions)。未設定時，系統會在初始化期間提示您選擇執行時間。 |
|
> [!IMPORTANT]
> 根據預設，2.x 版和更新版本的 Core Tools 會建立 .NET 執行時間的函式應用程式專案，作為 [c # 類別專案](functions-dotnet-class-library.md) ( .csproj) 。 這些 C# 專案可以與 Visual Studio 或 Visual Studio Code 搭配使用，並在測試期間以及發佈至 Azure 時進行編譯。 如果您想要改為建立和使用在 1.x 版中以及在入口網站中建立的相同 C# 指令碼 (.csx) 檔案，當您建立及部署函式時，必須包含 `--csx` 參數。

## <a name="register-extensions"></a>註冊延伸模組

除了 HTTP 和計時器觸發程式，執行時間2.x 版和更新版本中的函數系結會實作為擴充套件。 HTTP 系結和計時器觸發程式不需要延伸模組。 

為了減少不同擴充功能套件之間的不相容性，函式可讓您參考專案檔 host.js中的延伸模組組合。 如果您選擇不使用延伸模組套件組合，您也需要在本機安裝 .NET Core 2.x SDK，並使用您的函式專案維護副檔名 .csproj。  

在2.x 版和 Azure Functions 執行時間之外，您必須明確地為函式中所使用的系結類型註冊擴充功能。 您可以選擇個別安裝系結延伸模組，也可以將延伸套件組合參考新增至專案檔的 host.js。 延伸模組組合移除使用多個系結類型時，有套件相容性問題的機率。 這是註冊系結延伸模組的建議方法。 延伸模組套件組合也會移除安裝 .NET Core 2.x SDK 的需求。 

### <a name="use-extension-bundles"></a>使用延伸模組套件組合

[!INCLUDE [Register extensions](../../includes/functions-extension-bundles.md)]

若要深入瞭解，請參閱 [註冊 Azure Functions](functions-bindings-register.md#extension-bundles)系結延伸模組。 您應該先將延伸套件組合新增至 host.js，再將系結新增至檔案 function.js。

### <a name="explicitly-install-extensions"></a>明確安裝擴充功能
...
若未設定有效的儲存體連接字串， [`AzureWebJobsStorage`] 且未使用模擬器，則會顯示下列錯誤訊息：
...
在3.x 版或2.x 版中，當您執行時， `func new` 系統會提示您以函式應用程式的預設語言來選擇範本，然後系統也會提示您選擇函數的名稱。 在 1.x 版中，系統也會提示您選擇語言。
...
若要執行 Functions 專案，請執行 Functions 主機。 主機會啟用專案中所有函式的觸發程式。 視您的專案語言而定，啟動命令會有所不同。
...
func start
```
此命令必須 [在虛擬環境中執行](./create-first-function-cli-python.md)。
...
>[!NOTE]
> 本主題中的範例使用 cURL 工具，從終端機或命令提示字元傳送 HTTP 要求。 您可以使用您選擇的工具，將 HTTP 要求傳送至本機伺服器。 以 Linux 為基礎的系統預設會提供捲曲的工具，並 Windows 10 組建17063和更新版本。 在舊版 Windows 上，您必須先下載並安裝 [捲曲的工具](https://curl.haxx.se/)。
...
針對 HTTP 觸發程式和 webhook 和 Event Grid 觸發程式以外的所有函式類型，您可以藉由呼叫管理端點，在本機測試您的函式。 在本機伺服器上使用 HTTP POST 要求來呼叫此端點會觸發函式。 
...
#### <a name="using-the-func-run-command-version-1x-only"></a>使用 `func run` (1.x 版的命令) 

>[!IMPORTANT]
> `func run`只有1.x 版的工具支援此命令。 如需詳細資訊，請參閱[如何設定 Azure Functions 執行階段版本目標](set-runtime-version.md)主題。

在1.x 版中，您也可以使用來直接叫用函數， `func run <FunctionName>` 並提供函數的輸入資料。 此命令類似於使用 Azure 入口網站中的 [測試] 索引標籤執行函式。
...
Azure Functions Core Tools 支援兩種類型的部署：透過 [Zip 部署](functions-deployment-technologies.md#zip-deploy) 和 [部署自訂 Docker 容器](functions-deployment-technologies.md#docker-container)，直接將函式專案檔部署至您的函式應用程式。 您必須已 [在 Azure 訂用帳戶中建立函式應用程式](functions-cli-samples.md#create)，您將在其中部署程式碼。 應建置需要編譯的專案，以便部署二進位檔。

>[!IMPORTANT]
>您必須在本機安裝 [Azure CLI](/cli/azure/install-azure-cli) ，才能從 Core Tools 發佈至 Azure。  

專案資料夾可能包含不應該發行的特定語言檔案和目錄。 排除的專案會列在根專案資料夾的 funcignore 檔案中。     
...
>[!IMPORTANT]
> JAVA 使用 Maven 將您的本機專案發佈至 Azure。 使用下列命令來發佈到 Azure： `mvn azure-functions:deploy` 。 在初始部署期間會建立 Azure 資源。

此命令會發行至 Azure 中的現有函式應用程式。 如果您嘗試發行至 `<FunctionAppName>` 您的訂用帳戶中不存在的，則會收到錯誤。 若要了解如何使用 Azure CLI 從命令提示字元或終端機視窗建立函式應用程式，請參閱[建立無伺服器也可執行的函式應用程式](./scripts/functions-cli-create-serverless.md)。 根據預設，此命令會使用 [遠端組建](functions-deployment-technologies.md#remote-build) ，並將您的應用程式部署為 [從部署套件執行](run-functions-from-deployment-package.md)。 若要停用此建議的部署模式，請使用 `--nozip` 選項。

>[!IMPORTANT]
> 當您在 Azure 入口網站中建立函數應用程式時，預設會使用3.x 版的函式執行時間。 若要讓函式應用程式使用 1.x 版的執行階段，請依照[在 1.x 版上執行](functions-versions.md#creating-1x-apps)中的指示操作。
...
| **`--force`** | 在設定情況下忽略發佈前驗證。 |
| **`--csx`** | 發佈 C# 指令碼 (.csx) 專案。 |
| **`--no-build`** | 專案不會在發行期間建立。 針對 Python，則 `pip install` 不會執行。 |
...
Azure Functions 可讓您在 [自訂 Docker 容器](functions-deployment-technologies.md#docker-container)中部署函數專案。 如需詳細資訊，請參閱[使用自訂映像在 Linux 上建立函式](functions-create-function-linux-custom-image.md)。 自訂容器必須具有 Dockerfile。 若要使用 Dockerfile 建立應用程式，請在上使用--Dockerfile 選項 `func init` 。
...
監視函式執行的建議方式是整合 Azure 應用程式見解。 您也可以將執行記錄串流至您的本機電腦。 若要深入了解，請參閱[監視 Azure Functions](functions-monitoring.md)。

### <a name="application-insights-integration"></a>Application Insights 整合

當您在 Azure 中建立函數應用程式時，應該啟用 Application Insights 整合。 如果您的函式應用程式因為某些原因而未連接到 Application Insights 實例，那麼在 Azure 入口網站中進行這項整合很簡單。 若要深入瞭解，請參閱 [啟用 Application Insights 整合](configure-monitoring.md#enable-application-insights-integration)。
...
這種類型的串流記錄需要針對您的函數應用程式啟用 Application Insights 整合。   


## <a name="next-steps"></a>後續步驟

瞭解如何使用 Azure Functions Core Tools [Microsoft 學習模組](/learn/modules/develop-test-deploy-azure-functions-with-core-tools/) Azure Functions Core Tools 是 [開放原始碼並裝載在 GitHub 上](https://github.com/azure/azure-functions-cli)，以開發、測試及發佈 Azure Functions。  
