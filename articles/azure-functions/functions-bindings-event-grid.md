...
如果您想要的話，可以使用 HTTP 觸發程式來處理事件方格事件;請參閱[將事件接收到 HTTP 端點](../event-grid/receive-events.md)。 目前，當事件在[CloudEvents 架構](../event-grid/cloudevents-schema.md#azure-functions)中傳遞時，您無法在 Azure Functions 應用程式中使用事件方格觸發程式。 相反地，請使用 HTTP 觸發程序。
