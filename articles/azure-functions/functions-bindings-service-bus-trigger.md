...
description: 瞭解如何在建立 Azure 服務匯流排訊息時執行 Azure 函數。
...
當訊息新增至服務匯流排主題時，也可以觸發 JAVA 函數。 下列範例會使用 `@ServiceBusTopicTrigger` 批註來描述觸發程式設定。
...
* [ServiceBusTriggerAttribute](https://github.com/Azure/azure-functions-servicebus-extension/blob/master/src/Microsoft.Azure.WebJobs.Extensions.ServiceBus/ServiceBusTriggerAttribute.cs)
  ...
  由於 `Connection` 未定義屬性，因此函式會尋找名為的應用程式設定 `AzureWebJobsServiceBus` ，這是服務匯流排連接字串的預設名稱。 您也可以設定 `Connection` 屬性，以指定包含要使用之服務匯流排連接字串的應用程式設定名稱，如下列範例所示：
...
|屬性|類型|描述|
|--------|----|-----------|
|`ContentType`|`string`|傳送者和接收者針對應用程式特定邏輯所使用的內容類型識別碼。|
...
|`MessageSession`|`MessageSession`|專門針對啟用會話的佇列和主題的訊息接收者。|
