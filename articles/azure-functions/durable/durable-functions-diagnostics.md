...
您可以在檔案的`logger` （函數1.x）或`logging` （函式2.0）區段`host.json`中，設定發出至 Application Insights 之追蹤資料的詳細資訊。
...
從 Durable Functions 2.0 開始，.NET 協調器函式`ILogger`也可以選擇建立，以在重新執行期間自動篩選出記錄語句。 此自動篩選是使用`IDurableOrchestrationContext.CreateReplaySafeLogger(ILogger)` API 來完成。
...
* **Await**：每當協調`await`器函式中遇到時，它會將控制權傳回給長期工作架構發送器。 如果是第一次遇到特定`await`的，則*永遠不*會繼續相關聯的工作。 因為工作永遠不會繼續，所以不能在 await*上*逐步執行（在 Visual Studio 中為 F10）。 跨越只有在重新執行工作時可行。
...
> [!TIP]
> 在協調器函式中設定中斷點時，如果您只想要在非重新執行的執行上中斷，您可以設定只在`IsReplaying`為`false`時中斷的條件中斷點。
