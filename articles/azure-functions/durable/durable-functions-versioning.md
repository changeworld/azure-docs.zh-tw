...
### <a name="changing-activity-or-entity-function-signatures"></a>變更活動或實體函數簽章

簽章變更是指函式的名稱、輸入或輸出有所變更。 如果對活動或實體函數進行這類變更，它可能會中斷相依于它的任何協調器函式。 如果您更新協調器函式來配合此變更，可能會中斷現有的執行中執行個體。
...
在協調器函式的所有新執行個體中，此變更不會有問題，但會中斷任何執行中的執行個體。 例如，假設協調流程實例呼叫名為的函式 `Foo` 、取得布林值，然後檢查點。 如果此時部署簽章變更，則通過檢查點的執行個體在繼續並重新呼叫 `context.CallActivityAsync<int>("Foo")` 時會立刻失敗。 發生此錯誤是因為歷程記錄資料表中的結果， `bool` 但新的程式碼會嘗試將它還原序列化為 `int` 。

此範例只是簽章變更可能會中斷現有實例的許多不同方式之一。 一般而言，如果協調器需要變更呼叫函式的方式，則變更就很可能會有問題。
...
此變更會在 **Foo** 和 **Bar** 之間新增呼叫 **SendNotification** 函式。 簽章不變。 當現有的執行個體在呼叫 **Bar** 之後繼續時，就發生問題。 在重新執行期間，如果對 **Foo** 的原始呼叫傳回 `true` ，則協調器重新執行會呼叫 **SendNotification**，而不是在其執行歷程記錄中。 結果，「永久性工作架構」會失敗，並傳回 `NonDeterministicOrchestrationException`，因為原本認為應該呼叫 **Bar**，但卻發現呼叫 **SendNotification**。 當您新增任何對「持久」 Api 的呼叫時，可能會發生相同類型的問題，包括 `CreateTimer` 、等等 `WaitForExternalEvent` 。
...
這類失敗是否有問題，取決於您的使用中實例的重要性。 如果您正在進行開發，並不在意執行中的執行個體，這樣可能就很好。 不過，您必須處理診斷管線中的例外狀況和錯誤。 如果想要避免這些事情，請考慮其他版本設定選項。

### <a name="stop-all-in-flight-instances"></a>停止所有執行中的執行個體

另一個選項是停止所有執行中的執行個體。 您可以藉由清除內部 **控制佇列** 和 **工作專案佇列** 佇列的內容，來停止所有實例。 這些實例會永遠停滯在其所在位置，但不會讓您的記錄在失敗訊息中雜亂。 這種方法非常適合用於快速原型開發。
...
* 將所有更新部署為全新的函式，讓現有的函式保持原樣。 這可能很難處理，因為新函式版本的呼叫端也必須依照相同的指導方針進行更新。
* 以不同的儲存體帳戶，將所有更新部署為新的函式應用程式。
* 使用相同的儲存體帳戶來部署函數應用程式的新複本，但使用更新的 `taskHub` 名稱。 建議的方法是使用並存部署。
...
Durable Functions v1. x 的預設值為 `DurableFunctionsHub` 。 從 Durable Functions 2.0 版開始，預設的工作中樞名稱與 Azure 中的函式應用程式名稱相同，或在 `TestHubName` azure 外部執行。

所有 Azure 儲存體實體都是依據 `hubName` 設定值來命名。 只要指定新名稱給工作中樞，就可以確保為應用程式的新版本建立個別的佇列和記錄資料表。 不過，函式應用程式會停止處理在先前的工作中樞名稱下建立之協調流程或實體的事件。
...
> [!NOTE]
> 當您對協調器函式使用 HTTP 和 Webhook 觸發程序時，此策略最理想。 針對非 HTTP 觸發程式（例如佇列或事件中樞），觸發程序定義應衍生自在交換作業中更新的 [應用程式設定](../functions-bindings-expressions-patterns.md#binding-expressions---app-settings) 。
