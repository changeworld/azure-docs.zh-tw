...
在 Azure Functions 中，輸入和輸出繫結會提供宣告式方法，以便從您的程式碼可用之外部服務製作資料。 在本快速入門中，您會使用輸出繫結，在函式由 HTTP 要求觸發時於佇列中建立訊息。 您可以使用 Azure 儲存體容器來查看您的函式所建立的佇列訊息。
...
在本節中，您會使用入口網站 UI 來將佇列儲存體輸出繫結新增至您稍早建立的函式。 這個系結可讓您撰寫最少的程式碼，以在佇列中建立訊息。 您不需要為以下工作撰寫程式碼，例如開啟儲存體連線、建立佇列，或取得佇列的參考。 Azure Functions 執行階段和佇列輸出繫結會為您進行這些工作。

1. 在 Azure 入口網站中，針對您在[從 Azure 入口網站建立您的第一個函式](functions-create-first-azure-function.md)中建立的函式應用程式，開啟函式應用程式分頁。 若要開啟頁面，請搜尋並選取 [**函數應用程式**]。 然後，選取您的函數應用程式。

1. 選取您的函數應用程式，然後選取您在先前的快速入門中建立的函式。

1. 選取 [**整合**]，然後選取 [ **+ 新增輸出**]。

   :::image type="content" source="./media/functions-integrate-storage-queue-output-binding/function-create-output-binding.png" alt-text="建立函數的輸出系結。" border="true":::

1. 選取 [ **Azure 佇列儲存體**] 系結類型，然後依照下列螢幕擷取畫面所示，新增設定： 
...
1. 在您的函式中選取 [程式**代碼 + 測試**]，以在編輯器中顯示函式程式碼。
...
1. 儲存程式碼變更之後，請選取 [**測試**]。
1. 確認您的測試符合下列影像，然後選取 [**執行**]。 

    :::image type="content" source="./media/functions-integrate-storage-queue-output-binding/functions-test-run-function.png" alt-text="測試 Azure 入口網站中的佇列儲存體系結。" border="true":::
...
第一次使用輸出系結時，函數執行時間會在您的儲存體帳戶中建立名為**outqueue**的新佇列。 您將使用儲存體帳戶來驗證佇列和其中的訊息是否已建立。

### <a name="find-the-storage-account-connected-to-azurewebjobsstorage"></a>尋找連線至 AzureWebJobsStorage 的儲存體帳戶


1. 移至您的函式應用程式，**然後選取 [** 設定]。

1. 在 [**應用程式設定**] 下，選取 [ **AzureWebJobsStorage**]。
...
1. 在函式應用程式的資源群組中，選取您在本快速入門中使用的儲存體帳戶。

1. 在 [**佇列服務**] 底下，選取 [**佇列**]，然後選取名為**outqueue**的佇列。 
