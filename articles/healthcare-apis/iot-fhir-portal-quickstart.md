---
title: 快速入門：使用入口網站部署 Azure IoT Connector for FHIR (預覽)
description: 在此快速入門中，您將了解如何使用 Azure 入口網站部署、設定和使用 Azure API for FHIR 的 Azure IoT Connector for FHIR 功能。
...
開啟 [Azure 入口網站](https://portal.azure.com)，並移至您想要為其建立 Azure IoT Connector for FHIR 功能的 **Azure API for FHIR** 資源。
...
## <a name="create-new-azure-iot-connector-for-fhir-preview"></a>建立新的 Azure IoT Connector for FHIR (預覽)
...
輸入新 Azure IoT Connector for FHIR 的設定。 按一下 [建立] 按鈕，並等待 Azure IoT Connector for FHIR 部署。
...
|設定|值|說明 |
|---|---|---|
|連接器名稱|唯一的名稱|輸入名稱以識別您的 Azure IoT Connector for FHIR，此名稱在 Azure API for FHIR 資源中應是唯一的。 名稱只能包含小寫字母、數字及連字號 (-) 字元。 其開頭和結尾必須是字母或數字，而且長度必須介於 3-24 個字元之間。|
|解析類型|查閱或建立|如果您有頻外程序可以在 Azure API for FHIR 中建立[裝置](https://www.hl7.org/fhir/device.html) \(英文\) 和[患者](https://www.hl7.org/fhir/patient.html) \(英文\) FHIR 資源，請選取 [查閱]。 Azure IoT Connector for FHIR 將會在建立[觀察](https://www.hl7.org/fhir/observation.html) \(英文\) FHIR 資源以代表裝置資料時，使用這些資源的參考。 當您希望 Azure IoT Connector for FHIR 使用裝置資料中出現的個別識別碼值，在您的 Azure API for FHIR 中建立準系統裝置和患者資源時，請選取 [建立]。|

安裝完成之後，新建立的 Azure IoT Connector for FHIR 將會顯示在 [IoT 連接器] 頁面上。
...
若要上傳對應範本，請按一下新部署的 Azure IoT Connector for FHIR，以移至 [IoT 連接器] 頁面。
...
IoMT 裝置需要一個連接字串，才能將訊息連線並傳送至 Azure IoT Connector for FHIR。 在新部署的 Azure IoT Connector for FHIR 的 [IoT 連接器] 頁面上，選取 [管理用戶端連線] 按鈕。 
...
您可以使用 Postman，在 Azure API for FHIR 上檢視 Azure IoT Connector for FHIR 所建立的 FHIR 型觀察資源。 設定您的 [Postman 以存取 Azure API for FHIR](access-fhir-postman-tutorial.md)，並向 `https://your-fhir-server-url/Observation?code=http://loinc.org|8867-4` 提出 `GET` 要求，以檢視含有心率值的觀察 FHIR 資源。 
...
若不再需要，您可以移除相關聯的資源群組或相關聯的 Azure API for FHIR 服務，或 Azure IoT Connector for FHIR 執行個體本身，以刪除 IoT 連接器的執行個體。 

若要直接移除 Azure IoT Connector for FHIR 執行個體，請從 [IoT 連接器] 頁面選取執行個體，以移至 [IoT 連接器] 頁面，然後按一下 [刪除] 按鈕。 當系統要求您確認時，請選取 [是]。 
