...
ms.locfileid: "93391835"
---
# <a name="configure-azure-rbac-for-fhir"></a>設定適用于 FHIR 的 Azure RBAC 

在本文中，您將瞭解如何使用 [azure 角色型存取控制 (AZURE RBAC) ](../role-based-access-control/index.yml) 來指派 Azure API for FHIR 資料平面的存取權。 當資料平面使用者是在與您 Azure 訂用帳戶相關聯的 Azure Active Directory 租使用者中進行管理時，Azure RBAC 是指派資料平面存取的慣用方法。 如果您使用外部 Azure Active Directory 租使用者，請參閱 [本機 RBAC 指派參考](configure-local-rbac.md)。
...
如果這些角色不足以滿足您的需求，您也可以 [建立自訂角色](../role-based-access-control/tutorial-custom-role-powershell.md)。
