...
description: 在本教學課程中，您將了解如何透過 Azure IoT Connector for FHIR，啟用從 IoT 中樞到 Azure API for FHIR 的裝置資料路由。
...
ms.locfileid: "94636311"
---
# <a name="tutorial-receive-device-data-through-azure-iot-hub"></a>教學課程：透過 Azure IoT 中樞接收裝置資料

Azure IoT Connector for FHIR (快速健康照護互通資源)* 可讓您將來自醫學物聯網 (IoMT) 裝置的資料內嵌到 Azure API for FHIR 中。 [使用 Azure 入口網站部署 Azure IoT Connector for FHIR (預覽)](iot-fhir-portal-quickstart.md) 快速入門示範了 Azure IoT Central 所管理的裝置如何[將遙測傳送](iot-fhir-portal-quickstart.md#connect-your-devices-to-iot)至 Azure IoT Connector for FHIR 的範例。 Azure IoT Connector for FHIR 也可以使用透過 Azure IoT 中樞佈建和管理的裝置。 本教學課程提供將裝置資料從 Azure IoT 中樞連線並路由至 Azure IoT Connector for FHIR 的程序。
...
## <a name="get-connection-string-for-azure-iot-connector-for-fhir-preview"></a>取得 Azure IoT Connector for FHIR 的連接字串 (預覽)

Azure IoT 中樞需要連接字串，才能安全地與您的 Azure IoT Connector for FHIR 連線。 如[產生連接字串](iot-fhir-portal-quickstart.md#generate-a-connection-string)所述，為您的 Azure IoT Connector for FHIR 建立新的連接字串。 保留此連接字串以便在接下來的步驟中使用。

Azure IoT Connector for FHIR 會在幕後使用 Azure 事件中樞執行個體來接收裝置訊息。 上面建立的連接字串基本上是此基礎事件中樞的連接字串。
...
|EndpointResourceGroup|endpoint-resource-group|Azure IoT Connector for FHIR 所屬 Azure API for FHIR 資源的資源群組名稱。 您可以從 Azure API for FHIR 的 [概觀] 頁面取得此值。|
|EndpointSubscriptionId|endpoint-subscription-id|Azure IoT Connector for FHIR 所屬 Azure API for FHIR 資源的訂用帳戶識別碼。 您可以從 Azure API for FHIR 的 [概觀] 頁面取得此值。|
...
使用您的裝置 (實際或模擬)，將下面顯示的範例心率訊息傳送至 Azure IoT 中樞。 這則訊息會路由傳送至 Azure IoT Connector for FHIR，其中訊息會轉換成 FHIR 的觀測資源，並儲存到 Azure API for FHIR 中。
...
```
> [!IMPORTANT]
> 請務必傳送以 Azure IoT Connector for FHIR 設定，且符合[對應範本](iot-mapping-templates.md)的裝置訊息。

## <a name="view-device-data-in-azure-api-for-fhir"></a>在 Azure API for FHIR 中檢視裝置資料

您可以使用 Postman，在 Azure API for FHIR 上檢視 Azure IoT Connector for FHIR 所建立的 FHIR 觀察資源。 設定您的 [Postman 以存取 Azure API for FHIR](access-fhir-postman-tutorial.md)，並向 `https://your-fhir-server-url/Observation?code=http://loinc.org|8867-4` 提出 `GET` 要求，以在上述範例訊息中，使用已提交的心率值來檢視觀察 FHIR 資源。
...
在本快速入門手冊中，您會設定 Azure IoT 中樞以將裝置資料路由傳送至 Azure IoT Connector for FHIR。 從下面的後續步驟中選取各項主題，以深入了解 Azure IoT Connector for FHIR：
