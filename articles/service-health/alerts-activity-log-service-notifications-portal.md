...
description: 瞭解如何使用 Azure 入口網站設定服務健康情況通知的活動記錄警示，方法是使用 Azure 入口網站。
ms.topic: conceptual
ms.date: 06/27/2019
...
本文說明如何使用 Azure 入口網站來設定服務健康情況通知的活動記錄警示，方法是使用 Azure 入口網站。  

服務健康情況通知會儲存在 [Azure 活動記錄](../azure-monitor/platform/platform-logs-overview.md)中。 由於活動記錄中儲存了大量的資訊，因此有個別的使用者介面可讓您更輕鬆地查看及設定服務健康情況通知的警示。 

您可以在 Azure 傳送服務健康狀態通知到您的 Azure 訂用帳戶時接收警示。 您可以針對下列設定警示：

- 服務健康情況通知的類別 (服務問題、規劃的維護、健康情況諮詢、資訊安全諮詢) 。
- 受影響的訂閱。
- 受影響的服務。
- 受影響的區域。

> [!NOTE]
> 服務健康狀態通知不會傳送資源健康狀態事件的警示。
...
5. 選擇您想要收到警示的 **事件種類** ： *服務問題*、 *規劃的維護*、 *健康情況諮詢*和資訊 *安全諮詢*。
