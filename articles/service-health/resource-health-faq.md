...
## <a name="does-resource-health-differentiate-between-unavailability-caused-by-platform-problems-versus-something-i-did"></a>資源健康狀態是否會區分平臺問題所導致的無法使用，與我所做的不同之處？
...
資源健康狀態 [支援](resource-health-alert-arm-template-guide.md) 以活動記錄為基礎的警示。 活動記錄警示會使用 [動作群組](../azure-monitor/platform/action-groups.md) 來通知使用者已觸發警示。 動作群組支援各種通知通道，例如電子郵件、SMS、webhook 和 ITSM 動作。
