...
有關詳細資訊,請參閱[SSL 中止概述以及使用應用程式閘道端到端 SSL 的概述](ssl-overview.md)
...
SKUStandard_v2應用程式閘道專門支援靜態VIP類型。 這可確保與應用程式閘道相關聯的 VIP 即使在應用程式閘道的存留期結束後也不會變更。
...
有關詳細資訊,請參閱[應用程式閘道設定概述](configuration-overview.md#connection-draining)。
...
應用程式閘道Standard_v2可以設定為自動縮放或固定大小部署。 此 SKU 不提供不同的實例大小。 如需 v2 效能和定價的詳細資訊，請參閱[自動調整 v2 SKU](application-gateway-autoscaling-zone-redundant.md#pricing)。
...
有關應用程式閘道 v1-v2 功能比較,請參閱[自動縮放和區域冗餘應用程式閘道 v2](application-gateway-autoscaling-zone-redundant.md#feature-comparison-between-v1-sku-and-v2-sku)
