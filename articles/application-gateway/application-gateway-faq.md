...
Azure 應用程式閘道提供應用程式交付控制器 （ADC） 作為服務。 它為您的應用程式提供各種第 7 層負載平衡功能。 此服務高度可用、可擴展且完全由 Azure 管理。

### <a name="what-features-does-application-gateway-support"></a>應用程式閘道支援哪些功能？

應用程式閘道支援自動縮放、SSL 卸載和端到端 SSL、Web 應用程式防火牆 （WAF）、基於 Cookie 的會話關聯性、基於 URL 路徑的路由、多網站託管和其他功能。 如需完整的支援功能清單，請參閱[應用程式閘道簡介](application-gateway-introduction.md)。

### <a name="how-do-application-gateway-and-azure-load-balancer-differ"></a>應用程式閘道和 Azure 負載等化器有何不同？

應用程式閘道是第 7 層負載等化器，這意味著它僅適用于 Web 流量（HTTP、HTTPS、WebSocket 和 HTTP/2）。 它支援 SSL 終止、基於 Cookie 的會話相關性和負載平衡流量的迴圈等功能。 負載等化器負載平衡層 4 層（TCP 或 UDP）的流量。
...
### <a name="in-what-regions-is-application-gateway-available"></a>在哪些區域可以使用應用程式閘道？
...
支援重新導向。 請參閱[應用程式閘道重定向概述](application-gateway-redirect-overview.md)。

### <a name="in-what-order-are-listeners-processed"></a>接聽程式的處理順序為何？

請參閱[攔截器處理的順序](https://docs.microsoft.com/azure/application-gateway/configuration-overview#order-of-processing-listeners)。
...
對於 v2 SKU，打開公共 IP 資源並選擇 **"配置**"。 **DNS 名稱標籤（可選）** 欄位可用於配置 DNS 名稱。

### <a name="what-are-the-settings-for-keep-alive-timeout-and-tcp-idle-timeout"></a>保持活動超時和 TCP 空閒超時有哪些設置？

*保持活動超時*控制應用程式閘道在重用或關閉之前等待用戶端在持久連接上發送另一個 HTTP 要求的時間。 *TCP 空閒超時*控制 TCP 連接保持打開狀態的時間，以防無活動。 

應用程式閘道 v1 SKU 中的*保持活動超時*為 120 秒，在 v2 SKU 中為 75 秒。 對於應用程式閘道的 v1 和 v2 SKU 的前端虛擬 IP （VIP）， *TCP 空閒超時*是 4 分鐘的預設值。 

### <a name="does-the-ip-or-dns-name-change-over-the-lifetime-of-the-application-gateway"></a>IP 或 DNS 名稱是否會在應用程式閘道的存留期內更改？

在應用程式閘道 V1 SKU 中，如果停止並啟動應用程式閘道，VIP 可能會更改。 但與應用程式閘道關聯的 DNS 名稱不會在閘道的存留期內更改。 由於 DNS 名稱不會更改，因此應使用 CNAME 別名並將其指向應用程式閘道的 DNS 位址。 在應用程式閘道 V2 SKU 中，您可以將 IP 位址設置為靜態位址，因此 IP 和 DNS 名稱不會在應用程式閘道的存留期內更改。 
...
請參閱[應用程式閘道子網大小注意事項](https://docs.microsoft.com/azure/application-gateway/configuration-overview#size-of-the-subnet)。
...
是，但僅限於特定方案。 有關詳細資訊，請參閱[應用程式閘道配置概述](configuration-overview.md#user-defined-routes-supported-on-the-application-gateway-subnet)。

### <a name="does-application-gateway-support-x-forwarded-for-headers"></a>應用程式閘道是否支援 x-forwarded-for 標頭？

是。 請參閱[對請求的修改](https://docs.microsoft.com/azure/application-gateway/how-application-gateway-works#modifications-to-the-request)。
...
使用 v2 SKU 的大多數部署大約需要 6 分鐘才能進行預配。 但是，根據部署類型，可能需要更長時間。 例如，跨多個可用區域部署多個具有多個實例可能需要超過 6 分鐘的時間。 
...
v2 SKU 會自動確保將新執行個體分散在各個容錯網域和更新網域。 如果選擇區域冗余，則最新的實例也會跨可用性區域分佈，以提供區域故障恢復能力。
...
是，應用程式閘道 v2 SKU 支援自動調整規模。 有關詳細資訊，請參閱[自動縮放和區域冗余應用程式閘道](application-gateway-autoscaling-zone-redundant.md)。
...
是。 您可以設置連接耗盡以更改後端池中的成員，而不會中斷。 有關詳細資訊，請參閱[應用程式閘道的連接耗盡部分](features.md#connection-draining)。
...
是。 應用程式閘道始終部署在虛擬網路子網中。 此子網只能包含應用程式閘道。 有關詳細資訊，請參閱[虛擬網路和子網要求](https://docs.microsoft.com/azure/application-gateway/configuration-overview#azure-virtual-network-and-dedicated-subnet)。
...
只要您具有 IP 連接，應用程式閘道就可以與它所位於的虛擬網路外部的實例進行通信。 應用程式閘道還可以與訂閱外部的實例進行通信。 如果計畫將內部 IP 用作後端池成員，請使用[虛擬網路對等互連](../virtual-network/virtual-network-peering-overview.md)或[Azure VPN 閘道](../vpn-gateway/vpn-gateway-about-vpngateways.md)。

### <a name="can-i-deploy-anything-else-in-the-application-gateway-subnet"></a>是否可以在應用程式閘道子網路中部署其他任何項目？

否。 但是，您可以在子網中部署其他應用程式閘道。

### <a name="are-network-security-groups-supported-on-the-application-gateway-subnet"></a>應用程式閘道子網上支援網路安全性群組嗎？
...
### <a name="can-i-talk-to-on-premises-servers-when-theyre-connected-by-expressroute-or-vpn-tunnels"></a>當本機伺服器通過 ExpressRoute 或 VPN 隧道連接時，我是否可以與它們進行交談？

是，只有流量允許即可。

### <a name="can-one-backend-pool-serve-many-applications-on-different-ports"></a>一個後端池能否在不同的埠上為許多應用程式提供服務？

支援微服務體系結構。 要在不同的埠上探測，您需要配置多個 HTTP 設置。

### <a name="do-custom-probes-support-wildcards-or-regex-on-response-data"></a>自訂探測器是否支援回應資料的萬用字元或正則運算式？
...
### <a name="for-custom-probes-what-does-the-host-field-signify"></a>對於自訂探測器，"主機"欄位表示什麼？

"主機"欄位指定在應用程式閘道上配置多網站時將探測器發送到的名稱。 否則請使用"127.0.0.1"。 此值與虛擬機器主機名稱不同。 其格式是\<協定\>：//\<主機\>：\<埠\>\<路徑\>。

### <a name="can-i-allow-application-gateway-access-to-only-a-few-source-ip-addresses"></a>我是否可以允許應用程式閘道僅訪問幾個源 IP 位址？

是。 請參閱[限制對特定源 IP 的訪問](https://docs.microsoft.com/azure/application-gateway/configuration-overview#allow-application-gateway-access-to-a-few-source-ips)。

### <a name="can-i-use-the-same-port-for-both-public-facing-and-private-facing-listeners"></a>是否可以同時為面向公共的和麵向私有的攔截器使用相同的埠？

否。

### <a name="is-there-guidance-available-to-migrate-from-the-v1-sku-to-the-v2-sku"></a>是否有從 v1 SKU 遷移到 v2 SKU 的指南？

是。 有關詳細資訊，請參閱[將 Azure 應用程式閘道和 Web 應用程式防火牆從 v1 遷移到 v2](migrate-v1-v2.md)。

### <a name="does-application-gateway-support-ipv6"></a>應用程式閘道是否支援 IPv6？

應用程式閘道 v2 當前不支援 IPv6。 它只能使用 IPv4 在雙堆疊 VNet 中運行，但閘道子網必須僅 IPv4。 應用程式閘道 v1 不支援雙堆疊 VNet。 

## <a name="configuration---ssl"></a>配置 - SSL

### <a name="what-certificates-does-application-gateway-support"></a>應用程式閘道支援哪些證書？

應用程式閘道支援自簽章憑證、憑證授權單位 （CA） 證書、擴展驗證 （EV） 證書和萬用字元證書。
...
有關如何自訂 SSL 選項的資訊，請參閱[在應用程式閘道上配置 SSL 策略版本和密碼套件](application-gateway-configure-ssl-policy-powershell.md)。
...
是。 您可以將應用程式閘道配置為拒絕 TLS1.0、TLS1.1 和 TLS1.2。 預設情況下，SSL 2.0 和 3.0 已禁用，並且不可配置。

### <a name="can-i-configure-cipher-suites-and-policy-order"></a>可以設定加密套件和原則的順序嗎？

是。 在應用程式閘道中，您可以[配置密碼套件](application-gateway-ssl-policy-overview.md)。 要定義自訂策略，請至少啟用以下密碼套件之一。 
...
### <a name="how-many-authentication-certificates-for-backend-reencryption-does-application-gateway-support"></a>應用程式閘道支援多少個後端重新加密身份驗證憑證？

應用程式閘道最多支援 100 個身份驗證憑證。

### <a name="does-application-gateway-natively-integrate-with-azure-key-vault"></a>應用程式閘道是否本機與 Azure 金鑰保存庫集成？

是的，應用程式閘道 v2 SKU 支援金鑰保存庫。 有關詳細資訊，請參閱[使用金鑰保存庫證書的 SSL 終止](key-vault-certs.md)。

### <a name="how-do-i-configure-https-listeners-for-com-and-net-sites"></a>如何為 .com 和 .net 網站配置 HTTPS 攔截器？ 

對於多個基於域（基於主機）路由，您可以創建多網站攔截器，設置使用 HTTPS 作為協定的攔截器，並將攔截器與路由規則相關聯。 有關詳細資訊，請參閱[使用應用程式閘道託管多個網站](https://docs.microsoft.com/azure/application-gateway/multiple-site-overview)。

### <a name="can-i-use-special-characters-in-my-pfx-file-password"></a>我可以在我的 .pfx 檔密碼中使用特殊字元嗎？

否，在 .pfx 檔密碼中僅使用字母數位字元。

## <a name="configuration---web-application-firewall-waf"></a>配置 - Web 應用程式防火牆 （WAF）
...
通過診斷日誌記錄監視 WAF。 有關詳細資訊，請參閱[應用程式閘道的診斷日誌記錄和指標](application-gateway-diagnostics.md)。
...
是。 有關詳細資訊，請參閱自訂[WAF 規則組和規則](application-gateway-customize-waf-rules-portal.md)。

### <a name="what-rules-are-currently-available-for-waf"></a>WAF 當前可以使用哪些規則？

WAF 目前支援 CRS [2.2.9](../web-application-firewall/ag/application-gateway-crs-rulegroups-rules.md#owasp229) [、3.0](../web-application-firewall/ag/application-gateway-crs-rulegroups-rules.md#owasp30)和[3.1](../web-application-firewall/ag/application-gateway-crs-rulegroups-rules.md#owasp31)。 這些規則針對開放 Web 應用程式安全專案 （OWASP） 標識的前 10 個漏洞提供了基準安全性： 

* SQL 插入式攻擊保護
* 跨網站腳本保護
* 防止常見的 Web 攻擊，如命令注入、HTTP 要求走私、HTTP 回應拆分和遠端檔包含攻擊
* 防範 HTTP 通訊協定違規
...
是。 您可以在部署應用程式閘道的虛擬網路上啟用 DDoS 保護。 此設置可確保 Azure DDoS 保護服務還保護應用程式閘道虛擬 IP （VIP）。
...
Kubernetes 允許創建`deployment``service`和資源來在群集內部公開一組 pod。 為了在外部公開同一服務，定義了[`Ingress`](https://kubernetes.io/docs/concepts/services-networking/ingress/)提供負載平衡、SSL 終止和基於名稱的虛擬託管的資源。
為了滿足此`Ingress`資源，需要一個入口控制器，用於偵聽`Ingress`對資源的任何更改並配置負載等化器策略。

應用程式閘道入口控制器允許將[Azure 應用程式閘道](https://azure.microsoft.com/services/application-gateway/)用作[Azure 庫伯奈斯服務](https://azure.microsoft.com/services/kubernetes-service/)的入口，也稱為 AKS 群集。
...
* **應用程式閘道訪問日誌**：訪問日誌包含提交到應用程式閘道前端的每個請求。 資料包括調用方的 IP、請求的 URL、回應延遲、返回代碼和進出位元組。訪問日誌每 300 秒收集一次。 它包含每個應用程式閘道的一條記錄。
* **應用程式閘道性能日誌**：性能日誌捕獲每個應用程式閘道的性能資訊。 資訊包括以位元組為單位的輸送量、服務的請求總數、失敗的請求計數以及正常和不正常的後端實例計數。
* **應用程式閘道防火牆日誌**：對於使用 WAF 配置的應用程式閘道，防火牆日誌包含通過檢測模式或預防模式記錄的請求。

有關詳細資訊，請參閱[應用程式閘道的後端運行狀況、診斷日誌和指標](application-gateway-diagnostics.md)。

### <a name="how-do-i-know-if-my-backend-pool-members-are-healthy"></a>如何知道我的後端集區成員狀態良好？

使用 PowerShell Cmdlet`Get-AzApplicationGatewayBackendHealth`或閘戶驗證運行狀況。 有關詳細資訊，請參閱[應用程式閘道診斷](application-gateway-diagnostics.md)。

### <a name="whats-the-retention-policy-for-the-diagnostic-logs"></a>診斷日誌的保留原則是什麼？

診斷日誌流向客戶的存儲帳戶。 客戶可以根據其偏好設置保留原則。 診斷日誌也可以發送到事件中心或 Azure 監視器日誌。 有關詳細資訊，請參閱[應用程式閘道診斷](application-gateway-diagnostics.md)。

### <a name="how-do-i-get-audit-logs-for-application-gateway"></a>如何取得應用程式閘道的稽核記錄？

在門戶中，在應用程式閘道的功能表邊欄選項卡上，選擇**活動日誌**以訪問稽核記錄。 
...
您還可以使用資源管理器範本安裝並運行應用程式閘道訪問日誌的常用[GoAccess](https://goaccess.io/)日誌分析器。 GoAccess 提供有價值的 HTTP 流量統計資訊，如唯一訪問者、請求的檔、主機、作業系統、瀏覽器和 HTTP 狀態碼。 有關詳細資訊，在 GitHub 中，請參閱[資源管理器範本資料夾中的 Readme 檔](https://aka.ms/appgwgoaccessreadme)。

### <a name="what-could-cause-backend-health-to-return-an-unknown-status"></a>是什麼原因導致後端運行狀況返回未知狀態？

通常，當應用程式閘道子網上的網路安全性群組 （NSG）、自訂 DNS 或使用者定義的路由 （UDR） 阻止對後端的訪問時，您會看到未知狀態。 有關詳細資訊，請參閱[應用程式閘道的後端運行狀況、診斷日誌記錄和指標](application-gateway-diagnostics.md)。

### <a name="is-there-any-case-where-nsg-flow-logs-wont-show-allowed-traffic"></a>是否有任何情況下 NSG 流日誌不會顯示允許的流量？

是。 如果您的配置與以下方案匹配，則 NSG 流日誌中看不到允許的流量：
- 已部署應用程式閘道 v2
- 應用程式閘道子網上有一個 NSG
- 您已啟用該 NSG 上的 NSG 流日誌
...
1. 創建具有公共和專用前端 IP 位址的應用程式閘道
2. 不要為公共前端 IP 位址創建任何攔截器。 如果未為其創建攔截器，則應用程式閘道將不會偵聽公共 IP 位址上的任何流量。
3. 按優先順序順序創建並附加具有以下配置的應用程式閘道子網的[網路安全性群組](https://docs.microsoft.com/azure/virtual-network/security-overview)：
    
    a. 允許從源作為**閘道管理器**服務標記的流量，將目標作為**任意**埠和目標埠為**65200-65535**。 Azure 基礎結構通訊需要此連接埠範圍。 這些埠受證書身份驗證的保護（鎖定）。 外部實體（包括閘道使用者管理員）在沒有適當的證書的情況下無法對這些終結點發起更改
    
    b. 允許來自源的流量作為**AzureLoad平衡器**服務標記，目標埠為 **"任意"**
    
    c. 拒絕從源作為**Internet**服務標記的所有入站流量，將目標埠作為**Any**。 在入站規則中給予此規則*最低優先順序*
    
    d. 保留預設規則，如允許虛擬網路入站，以便不會阻止對私人 IP 位址的訪問
    
    e. 無法封鎖輸出網際網路連線。 否則，您將面臨日誌記錄、指標等問題。

僅用於專用 IP 訪問的 NSG![配置示例：僅用於專用 IP 訪問的應用程式閘道 V2 NSG 配置](./media/application-gateway-faq/appgw-privip-nsg.png)

### <a name="does-application-gateway-affinity-cookie-support-samesite-attribute"></a>應用程式閘道關聯 Cookie 是否支援同一網站屬性？
是的，[鉻瀏覽器](https://www.chromium.org/Home) [v80 更新](https://chromiumdash.appspot.com/schedule)引入了沒有 SameSite 屬性的 HTTP Cookie 的任務，應視為 SameSite_Lax。 這意味著瀏覽器不會在協力廠商上下文中發送應用程式閘道關聯 Cookie。 為了支援此方案，應用程式閘道除了注入現有的*應用程式閘道關聯*Cookie 外，還注入了另一個稱為*應用程式閘道 AffinityCORS*的 Cookie。  這些 Cookie 類似，但*應用程式閘道AffinityCORS* Cookie 還有兩個屬性 *：SameSite_無;安全*。 這些屬性即使在跨源請求中也能保持粘滯會話。 有關詳細資訊，請參閱[基於 Cookie 的關聯部分](configuration-overview.md#cookie-based-affinity)。
