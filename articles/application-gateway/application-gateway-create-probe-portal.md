...
1. 登入 [Azure 入口網站](https://portal.azure.com)。 如果您還沒有帳戶，您可以註冊一個[月免費試用](https://azure.microsoft.com/free)
...
3. 選取 [**健康情況探查**]，然後選取 [新增] 以新增健全狀況探查。 **Add**
...
4. 在 [**新增健康情況探查**] 頁面上，填寫探查的必要資訊，然後在完成時選取 **[確定]**。
   ...
   |**主控件**|亦即 contoso.com|此值是虛擬主機的名稱， (與在應用程式伺服器上執行) VM 主機名稱不同。 探查會傳送至/ \<protocol\> / \<host name\> ：\<port\>/\<urlPath\>|
   |**從後端 HTTP 設定挑選主機名稱**|[是] 或 [否]|將探查中的*主機*標頭，設定為與此探查相關聯之 HTTP 設定中的主機名稱。 在多租使用者後端（例如 Azure app service）的情況下特別需要。 [深入了解](https://docs.microsoft.com/azure/application-gateway/configuration-overview#pick-host-name-from-back-end-address)|
   |**從後端 HTTP 設定挑選埠**| [是] 或 [否]|將健康情況探查的*埠*設定為與此探查相關聯之 HTTP 設定的埠。 如果您選擇 [否]，您可以輸入要使用的自訂目的地埠 |
   |**連接埠**| 1-65535 | 要用於健康情況探查的自訂埠 | 
   |**路徑**|/或任何有效的路徑|自訂探查完整 URL 的其餘部分。 有效路徑的開頭為 '/'。 針對 HTTP：/contoso.com 的預設路徑， \/ 只要使用 '/' |
   ...
   |**HTTP 設定**|從下拉式清單選取|探查會與此處選取的 HTTP 設定 () 相關聯，因此，將會監視與所選 HTTP 設定相關聯之後端集區的健康情況。 它會使用與所選 HTTP 設定中所使用的相同的探查要求埠。 您只能選擇與其他任何自訂探查沒有關聯的 (s) 的 HTTP 設定。 <br>請注意，只有 (s) 的 HTTP 設定可用於與此探查設定中所選擇之通訊協定相同的通訊協定，而且具有與 [*從後端 HTTP 設定的挑選主機名稱*] 參數相同狀態的關聯。|
   
   > [!IMPORTANT]
   > 探查只會在與一或多個 HTTP 設定 (s) 相關聯時，才會監視後端的健康狀態。 它會監視那些後端集區的後端資源，這些後端集區會與與此探查關聯的 HTTP 設定 (s) 相關聯。 探查要求將會以 \<protocol\> ：//的形式傳送 \<hostName\> \<port\> / \<urlPath\> 。

### <a name="test-backend-health-with-the-probe"></a>使用探查測試後端健全狀況

輸入探查屬性之後，您可以測試後端資源的健全狀況，以確認探查設定是否正確，以及後端資源是否如預期般運作。

1. 選取 [**測試**] 並記下探查的結果。 應用程式閘道會測試與 HTTP 設定相關聯後端集區中所有後端資源的健全狀況， (s) 用於此探查。 
...
2. 如果有任何狀況不良的後端資源，請檢查 [**詳細資料**] 欄，以瞭解資源狀況不良狀態的原因。 如果資源因為不正確的探查設定而被標示為狀況不良，請選取 [**返回探查**] 連結，然後編輯探查設定。 否則，如果因後端發生問題而將資源標示為狀況不良，請解決後端資源的問題，然後選取 [**回到探查**] 連結並選取 [**測試**]，再次測試後端。
   ...
   > [!NOTE]
   > 您可以選擇儲存探查，即使使用狀況不良的後端資源，但不建議這麼做。 這是因為應用程式閘道不會將要求從後端集區中的後端伺服器轉送到，這是由探查判斷為狀況不良。 萬一後端集區中沒有狀況良好的資源，您將無法存取您的應用程式，而且會收到 HTTP 502 錯誤。
...
4. 在 [**新增健康情況探查**] 分頁上，填寫探查的必要資訊，然後在完成時選取 **[確定]**。
   ...
   |**名稱**|customProbe|此值是可在入口網站中存取之探查的易記名稱。|
   |**通訊協定**|HTTP 或 HTTPS | 健康狀態探查所使用的通訊協定。 |
   |**主控件**|亦即 contoso.com|此值是虛擬主機的名稱， (與在應用程式伺服器上執行) VM 主機名稱不同。 探查會傳送至 (通訊協定) ：// (主機名稱) .. 來自 HTTPsetting 的 (埠) /urlPath。  這適用于在應用程式閘道上設定多網站時。 如果已針對單一網站設定應用程式閘道，請輸入 ' 127.0.0.1 '。|
