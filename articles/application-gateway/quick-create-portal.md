...
1. 在 [基本] 索引標籤上，為下列應用程式閘道設定輸入這些值：
   ...
   - **資源群組**：為資源群組選取 [ **myResourceGroupAG** ]。 如果資源群組不存在，請選取 [新建] 加以建立。
...
2. Azure 需要虛擬網路才能在您所建立的資源之間進行通訊。 您可以建立新的虛擬網路，或使用現有的虛擬網路。 在此範例中，您將會在建立應用程式閘道時，同時建立新的虛擬網路。 在不同的子網路中，建立應用程式閘道執行個體。 在此範例中您會建立兩個子網路：一個用於應用程式閘道，另一個用於後端伺服器。
    ...
    - **子網名稱**（應用程式閘道子網）： [**子**網] 方格會顯示名為*Default*的子網。 將此子網路的名稱變更為 *myAGSubnet*。<br>應用程式閘道子網路只能包含應用程式閘道。 不允許任何其他資源。

    - **子網名稱**（後端伺服器子網）：**在子網方格的**第二列中，于 [**子網名稱**] 資料行中輸入*myBackendSubnet* 。

    - **位址範圍**（後端伺服器子網）：在 [**子網**] 方格的第二列中，輸入不會與*myAGSubnet*位址範圍重迭的位址範圍。 例如，如果 *myAGSubnet* 的位址範圍是 10.0.0.0/24，請針對 *myBackendSubnet* 的位址範圍輸入 *10.0.1.0/24*。
...
2. 在隨即開啟的 [新增後端集區] 視窗中，輸入下列值以建立空的後端集區：
    ...
    - **新增不含目標的後端集**區：選取 **[是]** 以建立沒有目標的後端集區。 您將會在建立應用程式閘道之後再新增後端目標。
...
3. 路由規則需要接聽程式。 在 [新增路由規則] 視窗內的 [接聽程式] 索引標籤上，針對接聽程式輸入下列值：
    ...
    - 接聽程式**名稱**：輸入*myListener*作為接聽項的名稱。
    - **前端 IP**：選取 [**公用**] 以選擇您為前端建立的公用 IP。
