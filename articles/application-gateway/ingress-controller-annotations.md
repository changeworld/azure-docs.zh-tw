...
對於要由 AGIC 觀察的輸入資源，必須以**標注** `kubernetes.io/ingress.class: azure/application-gateway`。 只有 AGIC 才會使用有問題的輸入資源。
...
| [appgw.ingress.kubernetes.io/connection-draining-timeout](#connection-draining) | `int32`表示 | `30` | |
| [appgw.ingress.kubernetes.io/cookie-based-affinity](#cookie-based-affinity) | `bool` | `false` | |
| [appgw.ingress.kubernetes.io/request-timeout](#request-timeout) | `int32`表示 | `30` | |
  ...
  - http:
      ...
      - path: /hello/
        backend:
          ...
          servicePort: 80
```
在上述範例中，我們已定義名為`go-server-ingress-bkprefix`且具有批註`appgw.ingress.kubernetes.io/backend-path-prefix: "/test/"`的輸入資源。 批註會告訴應用程式閘道建立 HTTP 設定，其路徑`/hello`的前置詞會覆寫。 `/test/`
...
`connection-draining-timeout`：此批註可讓使用者指定超時時間，之後應用程式閘道會終止清空後端端點的要求。
...
> [!NOTE]
> * 應用程式閘道不支援相同埠上的多個 Ip （例如：80/443）。 具有批註`appgw.ingress.kubernetes.io/use-private-ip: "false"`的輸入和的`appgw.ingress.kubernetes.io/use-private-ip: "true"`另`HTTP`一個會導致 AGIC 無法更新應用程式閘道。
> * 對於沒有私人 IP 的應用程式閘道，會忽略會輸入 with `appgw.ingress.kubernetes.io/use-private-ip: "true"` 。 這會反映在這些會輸入的控制器記錄和輸入事件中， `NoPrivateIP`並出現警告。
...
此批註可讓我們指定應用程式閘道在與 pod 交談時應該使用的通訊協定。 支援的通訊`http`協定：、`https`

> [!NOTE]
> * 雖然應用程式閘道支援自我簽署憑證，但目前只有在 pod 使用由知名`https` CA 簽署的憑證時，AGIC 才支援。
