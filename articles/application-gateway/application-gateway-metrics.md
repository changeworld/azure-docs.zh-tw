...
- **後端連線時間**
  ...
  這包括網路延遲，以及後端伺服器 TCP 堆疊建立新連線所花費的時間。 如果是 TLS，則會包含交握時所花費的時間。 
...
- **預估計費容量單位**
  ...
  若使用 v2 SKU，則計價模式會以耗用量來驅動。 容量單位會測量固定成本和以耗用量計費的成本。 *預估的計費容量單位*表示用來預估計費的容量單位數。 這會以「目前容量單位數」(對流量進行負載平衡所需的容量單位) 和「固定計費容量單位」(持續佈建的最小容量單位) 之間的較大值進行計算。

- **失敗的要求**

  應用程式閘道由5xx 伺服器錯誤碼提供服務的要求數目。 這包括從應用程式閘道產生的5xx 程式碼，以及從後端產生的5xx 代碼。 您可以進一步篩選要求計數，以顯示每個/特定後端集區的計數-HTTP 設定組合。
...
若要深入瞭解 webhook，以及如何使用它們來搭配警示，請造訪[在 Azure 計量警示上設定 webhook](../azure-monitor/platform/alerts-webhooks.md)。
