...
您可以使用 Azure CLI，在建立[應用程式閘道](./overview.md)時設定 [URL 路徑型路由規則](tutorial-url-route-cli.md)。 在本教學課程中，您可以使用[虛擬機器擴展集](../virtual-machine-scale-sets/overview.md)來建立後端集區。 然後，您可以建立 URL 路由規則，確保 Web 流量會重新導向到適當的後端集區。
