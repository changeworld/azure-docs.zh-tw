...
您可以藉由將[就緒或活動探查](https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-probes/)新增至您的規格來自訂探查屬性 `deployment` / `pod` 。
...
> * `HttpHeaders``InitialDelaySeconds` `SuccessThreshold` 不支援、、。

##  <a name="without-readinessprobe-or-livenessprobe"></a>不含 `readinessProbe` 或`livenessProbe`
如果未提供上述探查，則輸入控制器會假設在針對 `Path` 注釋指定的， `backend-path-prefix` 或在 `path` 服務的定義中指定的可連線到服務 `ingress` 。
