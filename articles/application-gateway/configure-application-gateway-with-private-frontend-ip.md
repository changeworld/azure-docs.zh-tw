...
8. 對於**前端 IP 位址類型**，請選擇 **"專用**"。
   ...
   預設情況下，它是動態 IP 位址分配。 配置子網的第一個可用位址被指定為前端 IP 位址。
   > [!NOTE]
...
17. 對於**攔截器名稱**，鍵入*攔截器-01*。
18. 對於**前端 IP，** 選擇 **"專用**"。
19. 接受剩餘的預設值並選擇 **"後端目標**"選項卡。
20. 對於**目標型別**，選擇**後端池**，然後選擇 **"應用閘道後端池**"。
21. 對於**HTTP 設置**，選擇 **"創建新**"。
22. 對於**HTTP 設置名稱**，鍵入*HTTP-設置-01*。
23. 對於**後端協定**，選擇**HTTP**。
24. 對於**後端埠**，鍵入*80*。
25. 接受剩餘的預設值，然後選擇 **"添加**"。
...
27. 選擇 **"下一步"：標記**。
28. 選擇 **"下一步"：查看 + 創建**。
29. 查看摘要頁上的設置，然後選擇 **"創建**"以創建網路資源和應用程式閘道。 可能需要幾分鐘的時間來建立應用程式閘道。 請等候部署成功完成後，再繼續進行至下一節。

## <a name="add-backend-pool"></a>新增後端集區

後端集區用於將要求路由傳送至可為要求提供服務的後端伺服器。 後端可以由 NIC、虛擬機器規模集、公共 IP 位址、內部 IP 位址、完全限定的功能變數名稱 （FQDN） 和多租戶後端（如 Azure 應用服務）組成。 在此範例中，您會使用虛擬機器作為目標後端。 您可以使用現有的虛擬機器，或建立新的虛擬機器。 在此範例中，您會建立兩個虛擬機器，供 Azure 作為應用程式閘道的後端伺服器。

若要這樣做，請：

1. 創建兩個新的虛擬機器 *，myVM*和*myVM2，* 用作後端伺服器。
...
4. 依虛擬機器輸入這些值：
   - 為**資源組**選擇 *"我的資源組"*
   - *myVM* - 用於**虛擬機器名稱**。
   - 選擇**Windows 伺服器 2019 資料中心****的圖像**。
   - *azureadmin* - 用於**使用者名**。
   - *Azure123456!* 密碼 **。**
5. 接受剩餘的預設值，然後選擇 **"下一步：磁片**"。
6. 接受預設值並選擇 **"下一步：網路**"。
