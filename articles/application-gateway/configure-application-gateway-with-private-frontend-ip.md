...
使用前端私人 IP 位址設定閘道，適用于不會公開至網際網路的內部企業營運應用程式。 對於不會公開至網際網路的安全性界限中的多層式應用程式內的服務和層級也很有用，但仍需要迴圈配置資源負載分配、會話粘性或安全通訊端層（SSL）終止。
...
3. 建立第二個虛擬機器，並使用您剛完成的步驟來安裝 IIS。 輸入 myVM2 作為其名稱，並針對 VMName 在 Sset-azvmextension 中。
