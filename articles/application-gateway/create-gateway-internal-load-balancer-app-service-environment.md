...
---
# <a name="back-end-server-certificate-is-not-allow-listed-for-an-application-gateway-using-an-internal-load-balancer-with-an-app-service-environment"></a>使用具有 App Service 環境的內部 Load Balancer 的應用程式閘道不允許列出後端伺服器憑證

本文會針對下列問題進行疑難排解：當您在 Azure 中使用端對端 TLS 時，使用內部 Load Balancer (ILB) 與後端上的 App Service 環境 (ASE) 建立應用程式閘道時，不允許列出憑證。
...
- **後端健康情況：** 狀況不良–後端伺服器憑證不允許與應用程式閘道一起列出。
...
**502– Web 服務器在作為閘道或 proxy 伺服器時收到不正確回應。**
...
若要減少額外負荷，您可以上傳 HTTP 設定中的 ILB 憑證，以進行探查路徑工作。  (此步驟僅適用于允許清單。 它不會用於 TLS 通訊。 ) 您可以在 HTTPS 上使用瀏覽器的 IP 位址存取 ILB，然後以 Base-64 編碼的 CER 格式匯出 TLS/SSL 憑證，並在個別的 HTTP 設定上傳憑證，來取得 ILB 憑證。
