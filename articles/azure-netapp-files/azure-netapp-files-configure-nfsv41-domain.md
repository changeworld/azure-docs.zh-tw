...
如上述範例所示，的使用者`file1`應該是`root`，但根據預設，它`nobody`會對應至。  本文說明如何將`file1`使用者設定為。 `root`  

## <a name="steps"></a>步驟 

1. 編輯 NFS `/etc/idmapd.conf`用戶端上的檔案。   
    取消批註程式`#Domain`程式碼（也就是`#`從行中移除），並將值`localdomain`變更`defaultv4iddomain.com`為。 
...
4. 在您`rpcbind`的主機上重新開機`service rpcbind restart`服務（），或直接重新開機主機。
5. 視需要掛接 NFS 磁片區。   
...
在下列範例中， `Host1`有三個現有的測試使用者`testuser01`帳戶`testuser02`（ `testuser03`、、）： 
