...
| 資料作業 - 稽核 |可用。 有關詳細資訊，請參閱[此處](data-lake-store-diagnostic-logs.md)。 |可用 |
...
| 大小限制 |帳戶大小、檔案大小或檔案數目沒有限制 |有關特定限制，請參閱[標準存儲帳戶的可伸縮性目標](../storage/common/scalability-targets-standard-account.md)以及[Blob 存儲的可伸縮性和性能目標](../storage/blobs/scalability-targets.md)。 可聯絡 [Azure 支援](https://azure.microsoft.com/support/faq/)以取得較大帳戶的限制 |
| 異地備援 |本機備援 (在一個 Azure 區域中多個資料複本） |本機備援 (LRS)、區域備援 (ZRS)、全域備援 (GRS)、讀取存取全域備援 (RA-GRS)。 有關詳細資訊，請參閱[此處](../storage/common/storage-redundancy.md) |
| 服務狀態 |正式推出 |正式推出 |
| 區域可用性 |[在此處](https://azure.microsoft.com/regions/#services)查看 |所有 Azure 區域均可使用 |
