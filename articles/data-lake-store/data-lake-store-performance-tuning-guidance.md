---
title: Azure Data Lake Storage Gen1-效能調整
description: 瞭解如何在 Azure Data Lake Storage Gen1 中使用所有可用的輸送量，以盡可能平行執行最多的讀取和寫入，以獲得最佳效能。
author: twooley
...
無論您是在 Azure 中使用內部部署機器或 Vm，都應謹慎地選取適當的硬體。 對於來源磁碟硬體，偏好使用 SSD 而非 HDD，並且挑選具有更快磁針的磁碟硬體。 對於來源網路硬體，盡可能使用最快的 NIC。 在 Azure 上，我們建議 Azure D14 Vm 具有適當的強大磁片和網路硬體。

### <a name="network-connectivity-to-data-lake-storage-gen1"></a>Data Lake Storage Gen1 的網路連線能力
...
當資料儲存在 Data Lake Storage Gen1 時，檔案大小、檔案數目和資料夾結構都會影響效能。 下一節說明這些區域的最佳做法。

### <a name="file-size"></a>檔案大小

一般而言，例如 HDInsight 和 Azure Data Lake Analytics 的分析引擎都有每個檔案的額外負荷。 如果您將資料儲存為許多小型檔案，會造成效能的負面影響。

一般情況下，將您的資料組織成較大大小的檔案，以提升效能。 根據經驗法則，在 256 MB 或更大的檔案中組織資料集。 在例如映像和二進位資料的某些情況下，不能夠平行處理。 在這些情況下，建議您保留 2 GB 以下的個別檔案。

有時候，資料管線對具有許多小型檔案的原始資料具有有限的控制權。 建議進行「cooking」程序，該程序會產生較大的檔案，以用於下游應用程式。

### <a name="organize-time-series-data-in-folders"></a>組織資料夾中的時間序列資料

針對 Hive 和 ADLA 工作負載，時間序列資料的資料分割剪除有助於某些查詢僅讀取資料的子集，進而改善效能。

內嵌時間序列資料的管線，通常會使用檔案和資料夾的結構化命名來放置它們的檔案。 以下是我們針對依日期結構化的資料所看到的常見範例： *\dataset\yyyy\mm\dd\ datafile_YYYY_MM_DD tsv*。

請注意，日期時間資訊會同時在資料夾和檔案名稱中顯示。

針對日期和時間，以下是常見的模式： *\dataset\yyyy\mm\dd\hh\mm\ datafile_YYYY_MM_DD_HH_mm tsv*。

同樣地，您對於資料夾和檔案組織的選擇，應該針對較大的檔案大小以及每個資料夾中合理的檔案數目進行最佳化。

## <a name="optimize-io-intensive-jobs-on-hadoop-and-spark-workloads-on-hdinsight"></a>在 HDInsight 上將 Hadoop 和 Spark 工作負載的 i/o 密集型作業優化

作業屬於下列三個類別之一：

* **CPU 密集作業** 這些作業有很長的計算時間和最短的 I/O 時間。 範例包括機器學習和自然語言處理作業。
* **記憶體耗用量。** 這些作業會使用大量記憶體。 範例包括 PageRank 和即時分析作業。
* **I/O 密集作業** 這些作業花費大部分時間執行 I/O。 常見的範例是只執行讀取和寫入作業的複製作業。 其他範例包括讀取大量資料的資料準備工作、執行一些資料轉換，然後將資料寫回存放區。
...
**增加每個 YARN 容器的核心。** 增加配置給每個容器的核心數目，以增加在每個容器中執行的平行工作數目。 這適用于 Spark 之類的應用程式，它會針對每個容器執行多項工作。 針對在每個容器中執行單一線程的應用程式（例如 Hive），最好有更多容器，而不是每個容器有更多核心。
