---
title: Azure VPN 閘道：創建&安裝 VPN 用戶端設定檔 - P2S RADIUS 連接
...
1. [為 P2S 連接設置 Azure VPN 閘道](point-to-site-how-to-radius-ps.md)。
2. [設置 RADIUS 伺服器進行身份驗證](point-to-site-how-to-radius-ps.md#radius)。 
...
### <a name="1-generate-vpn-client-configuration-files"></a><a name="usernamefiles"></a>1. 生成 VPN 用戶端設定檔

可以使用 Azure 門戶或使用 Azure PowerShell 生成 VPN 用戶端設定檔。
...
執行會傳回連結的命令。 複製並粘貼到 Web 瀏覽器的連結以下載**VpnClient配置.zip**。 將該檔案解壓縮以檢視下列資料夾： 
...
### <a name="2-configure-vpn-clients"></a><a name="setupusername"></a>2. 配置 VPN 用戶端
...
1. 選取 Windows 電腦架構所對應的 VPN 用戶端組態檔。 若是 64 位元的處理器架構，請選擇 **VpnClientSetupAmd64** 安裝程式套件。 對於 32 位處理器體系結構，請選擇**VpnClientSetupX86**安裝套裝程式。 
2. 若要安裝，請按兩下套件。 如果您看到智慧螢幕快顯視窗，請選擇 **"更多資訊** > **運行**"。
...
## <a name="certificate-authentication"></a><a name="certeap"></a>憑證驗證
...
### <a name="1-generate-vpn-client-configuration-files"></a><a name="certfiles"></a>1. 生成 VPN 用戶端設定檔
...
1. 選取組態套件並將它安裝在用戶端裝置上。 若是 64 位元的處理器架構，請選擇 **VpnClientSetupAmd64** 安裝程式套件。 對於 32 位處理器體系結構，請選擇**VpnClientSetupX86**安裝套裝程式。 如果您看到智慧螢幕快顯視窗，請選擇 **"更多資訊** > **運行**"。 您也可以儲存要在其他用戶端電腦上安裝的套件。
2. 每個用戶端都需要用戶端憑證才能驗證。 請安裝用戶端憑證。 有關用戶端憑證的資訊，請參閱[點對點的用戶端憑證](vpn-gateway-certificates-point-to-site.md)。 若要安裝所產生的憑證，請參閱[在 Windows 用戶端上安裝憑證](point-to-site-how-to-vpn-client-install-azure-cert.md)。
3. 在用戶端電腦上，瀏覽至 [網路設定]****，然後選取 [VPN]****。 VPN 連線會顯示其連線的虛擬網路名稱。
...
3. 開啟 [網路喜好設定]**** 底下的 [網路]**** 對話方塊。 選擇**+** 該選擇可為 P2S 連接到 Azure 虛擬網路的 P2S 連接創建新的 VPN 用戶端連接設定檔。
...
## <a name="working-with-other-authentication-types-or-protocols"></a><a name="otherauth"></a>與其他驗證類型或通訊協定搭配使用
