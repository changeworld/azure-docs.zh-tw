...
* 為 Resource Manager 和服務管理 (傳統) 安裝 PowerShell Cmdlet。 在本文中，我們會使用 Azure 入口網站和 PowerShell。 需要 PowerShell 才能建立從傳統 VNet 到 Resource Manager VNet 的連線。 有關詳細資訊，請參閱[如何安裝和配置 Azure PowerShell](/powershell/azure/overview)。 

### <a name="example-settings"></a><a name="values"></a>範例設定
...
### <a name="connection-overview"></a><a name="connectoverview"></a>連線概觀
...
## <a name="section-1---configure-the-classic-vnet-settings"></a><a name="classicvnet"></a>第 1 節 - 進行傳統 VNet 設定

在本節中，您會建立傳統 VNet、區域網路 (本機網站) 及虛擬網路閘道。 已提供螢幕擷取畫面做為範例。 請務必將值取代為您自己的值，或使用[範例](#values)值。

### <a name="1-create-a-classic-vnet"></a>1.<a name="classicvnet"></a>創建經典 VNet
...
### <a name="2-configure-the-local-site"></a>2.<a name="local"></a>配置本地網站

1. 瀏覽至 [所有資源]****，並且在清單中找出 **ClassicVNet**。
2. 按一下功能表的 **"設置"** 部分中的 **"閘道**"，然後按一下橫幅以創建閘道。
...
### <a name="3-create-the-virtual-network-gateway"></a><a name="classicgw"></a>3. 創建虛擬網路閘道

1. 在 [新增 VPN 連線]**** 頁面上，選取 [立即建立閘道]**** 核取方塊。
2. 按一下 [選擇性閘道組態]**** 可開啟 [閘道組態]**** 頁面。

   ![打開閘道配置頁](./media/vpn-gateway-connect-different-deployment-models-portal/optionalgatewayconfiguration.png "打開閘道配置頁")
...
### <a name="4-copy-the-virtual-network-gateway-public-ip-address"></a><a name="ip"></a>4. 複製虛擬網路閘道公共 IP 位址
...
## <a name="section-2---configure-the-resource-manager-vnet-settings"></a><a name="rmvnet"></a>第 2 節 - 進行 Resource Manager VNet 設定

在本節中，您會建立 Resource Manager VNet 的虛擬網路閘道和區域網路閘道。 已提供螢幕擷取畫面做為範例。 請務必將值取代為您自己的值，或使用[範例](#values)值。

### <a name="1-create-a-virtual-network"></a>1. 創建虛擬網路
...
### <a name="3-create-a-local-network-gateway"></a><a name="createlng"></a>3. 創建本地網路閘道
...
## <a name="section-3---modify-the-classic-vnet-local-site-settings"></a><a name="modifylng"></a>第 3 節 - 修改傳統 VNet 本機站台設定
...
4. 在 [站對站 VPN 連線]**** 頁面上，按一下站台的名稱。
   ...
   ![網站名稱](./media/vpn-gateway-connect-different-deployment-models-portal/sitetosite3.png "本地網站名稱")
5. 在您本機站台的 [連線] 頁面上，按一下本機站台的名稱以開啟 [本機站台]**** 頁面。

   ![開放本地網站](./media/vpn-gateway-connect-different-deployment-models-portal/openlocal.png "開啟本機網站")
...
### <a name="1-connect-to-your-azure-account"></a>1. 連接到 Azure 帳戶
...
### <a name="3-create-the-connection"></a>3. 創建連接
...
- **-共用金鑰**是您生成和指定的值。 針對此範例，我們使用的是 *abc123*，但是您可以使用更為複雜的值。 重要的是，您在此指定的值必須與您在建立 Resource Manager 到傳統連線時指定的值相同。
...
## <a name="section-6---verify-your-connections"></a><a name="verify"></a>第 6 節 - 驗證您的連線
