...
* 為 Resource Manager 和服務管理 (傳統) 安裝 PowerShell Cmdlet。 在本文中，我們會使用 Azure 入口網站和 PowerShell。 需要 PowerShell 才能建立從傳統 VNet 到 Resource Manager VNet 的連線。 如需詳細資訊，請參閱 [如何安裝及設定 Azure PowerShell](/powershell/azure/)。 
...
如果您沒有傳統 VNet，而且正在執行這些步驟作為練習，您可以使用[這篇文章](/previous-versions/azure/virtual-network/virtual-networks-create-vnet-classic-pportal)和上述的[範例](#values)設定值來建立 VNet。
...
### <a name="3-create-the-virtual-network-gateway"></a><a name="classicgw"></a>3. 建立虛擬網路閘道
...
### <a name="3-create-a-local-network-gateway"></a><a name="createlng"></a>3. 建立局域網路閘道
