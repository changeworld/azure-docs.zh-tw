...
1. 若要連線到您的 VNet，請在用戶端電腦上，移至 Azure 入口網站中的 [ **vpn** 連線]，並找出您建立的 vpn 連線。 VPN 連線的名稱會與虛擬網路相同。 選取 [連接]。 如果出現關於憑證的快顯訊息，請選取 [繼續]**** 以使用較高的權限。
