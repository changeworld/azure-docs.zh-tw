...
在此步驟中，您會建立並設定 TestVNet1。 使用「 [建立閘道」教學](vpn-gateway-tutorial-create-gateway-powershell.md) 課程中的步驟來建立及設定您的 Azure 虛擬網路和 VPN 閘道。 使用下列螢幕擷取畫面中的參考設定。
...
1. 選取 [ **審核 + 建立** ] 以執行驗證。 驗證通過後，選取 [ **建立** ] 以部署 VPN 閘道。 閘道最多可能需要45分鐘的時間才能完整建立和部署。 您可以在閘道的 [總覽] 頁面上看到部署狀態。
...
若要建立跨單位連線，您需要建立 *局域網路閘道* 來代表您的內部部署 VPN 裝置， *並建立連線以連接* VPN 閘道與局域網路閘道，如 [建立站對站](vpn-gateway-howto-site-to-site-resource-manager-portal.md)連線中所述。 本文包含指定 BGP 設定參數所需的其他屬性。
