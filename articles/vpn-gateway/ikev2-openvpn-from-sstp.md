...
ms.locfileid: "80143101"
---
# <a name="transition-to-openvpn-protocol-or-ikev2-from-sstp"></a>從 SSTP 過渡到 OpenVPN 協定或 IKEv2

點對站 (P2S) VPN 閘道連線可讓您建立從個別用戶端電腦到您的虛擬網路的安全連線。 P2S 連線的建立方式是從用戶端電腦開始。 本文適用于資源管理器部署模型，並討論如何通過過渡到 OpenVPN 協定或 IKEv2 來克服 SSTP 的 128 個併發連接限制。
...
* **安全通訊端隧道協定 （SSTP），** 一種基於 SSL 的專有 VPN 協定. SSL VPN 解決方案可以穿透防火牆，因為大多數防火牆都打開 TCP 埠 443 出站，這是 SSL 使用的。 SSTP 僅在 Microsoft 裝置上提供支援。 Azure 支援所有具有 SSTP (Windows 7 及更新版本) 的 Windows 版本。 **SSTP 僅支援多達 128 個併發連接，而不考慮閘道 SKU**。
...
## <a name="migrating-from-sspt-to-ikev2-or-openvpn"></a>從 SSPT 遷移到 IKEv2 或 OpenVPN

在某些情況下，您可能希望支援到 VPN 閘道的 128 個併發 P2S 連接，但使用 SSTP。 在這種情況下，您需要遷移到 IKEv2 或 OpenVPN 協定。

### <a name="option-1---add-ikev2-in-addition-to-sstp-on-the-gateway"></a>選項 1 - 除了閘道上的 SSTP 外添加 IKEv2

這是最簡單的選項。 SSTP 和 IKEv2 可以共存于同一閘道上，並為您提供數量較多的併發連接。 只需在現有閘道上啟用 IKEv2 並重新下載用戶端即可。
...
**IKEv2 使用非標準 UDP 埠，因此您需要確保這些埠不會在使用者的防火牆上被阻止。正在使用的埠是 UDP 500 和 4500。**

要將 IKEv2 添加到現有閘道，只需轉到門戶中虛擬網路閘道下的"點對點配置"選項卡，然後從下拉清單中選擇**IKEv2 和 SSTP （SSL）。**

![點對點](./media/ikev2-openvpn-from-sstp/sstptoikev2.png "IKEv2")


### <a name="option-2---remove-sstp-and-enable-openvpn-on-the-gateway"></a>選項 2 - 刪除 SSTP 並在閘道上啟用 OpenVPN

由於 SSTP 和 OpenVPN 都是基於 TLS 的協定，因此它們不能共存于同一閘道上。 如果您決定從 SSTP 遷移到 OpenVPN，您必須禁用 SSTP 並在閘道上啟用 OpenVPN。 此操作將導致現有用戶端失去與 VPN 閘道的連接，直到用戶端上配置了新設定檔。

如果需要，您可以與 IKEv2 一起啟用 OpenVPN。 OpenVPN 基於 TLS，使用標準 TCP 443 埠。 要切換到 OpenVPN，請轉到門戶中虛擬網路閘道下的"點對點配置"選項卡，然後從下拉清單中選擇**OpenVPN （SSL）** 或**IKEv2 和 OpenVPN （SSL）。**
