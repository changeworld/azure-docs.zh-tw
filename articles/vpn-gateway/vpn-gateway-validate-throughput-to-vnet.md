...
* 本地 VPN 設備（查看[已驗證 VPN 設備](vpn-gateway-about-vpn-devices.md#devicetable)的清單。
* 公共互聯網
* Azure VPN 閘道
* Azure VM
...
1. 判斷您應用程式的基準輸送量需求。
1. 判斷您的 Azure VPN 閘道輸送量限制。 有關説明，請參閱[關於 VPN 閘道](vpn-gateway-about-vpngateways.md#gwsku)的"閘道 SKU"部分。
1. 判斷 VM 大小的 [Azure VM 輸送量指引](../virtual-machines/virtual-machines-windows-sizes.md)。
1. 決定您網際網路服務提供者 (ISP) 的頻寬。
1. 通過佔用 VM、VPN 閘道或 ISP 的最小頻寬來計算預期輸送量;以百萬位元/秒 （/） 除以八 （8） 為單位進行測量。

如果計算的輸送量不符合應用程式的基準輸送量要求，則必須增加您確定為瓶頸的資源的頻寬。 如果要調整 Azure VPN 閘道的大小，請參閱 [變更閘道 SKU](vpn-gateway-about-vpn-gateway-settings.md#gwsku)。 如果調整虛擬機器的大小，請參閱 [調整 VM 的大小](../virtual-machines/virtual-machines-windows-resize-vm.md)。 如果您沒有遇到預期的 Internet 頻寬，您也可以聯繫您的 ISP。
...
此工具不會對磁碟執行任何讀取/寫入作業。 此工具只會產生從一端到另一端自我產生的 TCP 流量。 它基於測量用戶端和伺服器節點之間可用頻寬的實驗生成統計資訊。 在兩個節點之間測試時，一個節點充當伺服器，另一個節點充當用戶端。 完成此測試後，我們建議您反轉節點的角色，以測試兩個節點上的上載和下載輸送量。
...
下載[iPerf](https://iperf.fr/download/iperf_3.1/iperf-3.1.2-win64.zip)。 如需詳細資訊，請參閱 [iPerf 文件](https://iperf.fr/iperf-doc.php)。
 ...
 > [!NOTE]
 > 本文中討論的協力廠商產品由獨立于 Microsoft 的公司製造。 Microsoft 對於這些產品的效能或可靠性不提供任何默示或其他保證。
...
1. 完成上述步驟後，執行相同的步驟，並反轉角色，以便伺服器節點現在將成為用戶端節點，反之亦然。

> [!Note]
> Iperf不是唯一的工具。 [NTTTCP是測試的替代解決方案](https://docs.microsoft.com/azure/virtual-network/virtual-network-bandwidth-testing)。

## <a name="test-vms-running-windows"></a>測試回合 Windows 的 VM
...
在 RECEIVER 上啟動拿鐵.exe（從 CMD 運行，而不是從 PowerShell 運行）：
...
在發送器上啟動拿鐵.exe（從 CMD 運行，而不是從 PowerShell 運行）

`latte -c -a <Receiver IP address>:<port> -i <iterations>`

生成的命令與接收方相同，除非添加"-c"以指示這是"用戶端"或發送方

`latte -c -a 10.0.0.4:5005 -i 65100`

等候結果。 根據 VM 之間的距離，可能需要幾分鐘才能完成。 考慮從較少的反覆運算開始測試成功，然後再運行更長的測試。

## <a name="test-vms-running-linux"></a>測試回合 Linux 的 VM

使用[SockPerf](https://github.com/mellanox/sockperf)測試 VM。

### <a name="install-sockperf-on-the-vms"></a>在 VM 上安裝 SockPerf

在 Linux VM（SENDER 和 RECEIVER）上，運行這些命令以在 VM 上準備 SockPerf：
...
特別是，在這些測試期間，對從用戶端和伺服器並行收集的資料包捕獲跟蹤（Wireshark/網路監視器）進行分析將有助於評估不良性能。 這些跟蹤可能包括資料包丟失、高延遲、MTU 大小。 碎片、TCP 0 視窗、訂單外片段等。

## <a name="address-slow-file-copy-issues"></a>處理檔案複製變慢的問題

即使使用前面的步驟（iPERF/NTTTCP/等）評估的總體輸送量良好，在使用 Windows 資源管理器或通過 RDP 會話進行拖放時，您可能會遇到緩慢的檔應對速度。 此問題一般是因為下列一個或多個因素造成︰

* 如 Windows 檔案總管與 RDP 的檔案應用程式，並未在複製檔案時使用多執行緒。 為了提升效能，請使用如 [Richcopy](https://technet.microsoft.com/magazine/2009.04.utilityspotlight.aspx) 等多執行緒的檔案複製應用程式，以 16 或 32 條執行緒複製檔案。 要更改"富複製"中檔副本的執行緒編號，請按一下**操作** > **複製選項** > **"檔副本**"。

   ![檔案複製變慢的問題](./media/vpn-gateway-validate-throughput-to-vnet/Richcopy.png)<br>

   > [!Note]
   > 並非所有應用程式都工作相同，並非所有應用程式/進程都利用所有線程。 如果運行測試，則可以看到某些執行緒為空，並且無法提供準確的輸送量結果。
   > 要檢查應用程式檔案傳輸性能，請使用多執行緒，連續增加執行緒的 + 或減少，以找到應用程式或檔案傳輸的最佳輸送量。
...
提到您希望 Azure 通過本地網路閘道上的 VPN 訪問本地範圍的子網。 同時，將 Azure 中的 VNET 位址空間定義到本地設備。

* **基於路由的閘道**：基於路由的 VPN 的策略或流量選擇器配置為任意到任意（或萬用字元）。

* **基於策略的閘道**：基於策略的 VPN 根據本地網路和 Azure VNet 之間的位址首碼組合，通過 IPsec 隧道加密和直接資料包。 原則 (或流量選取器) 通常定義為 VPN 組態中的存取清單。

* **使用基於策略的流量選擇器**連接：（"使用基於策略的流量選擇器"以$True連接時將配置 Azure VPN 閘道以在內部連接到基於策略的 VPN 防火牆。 如果啟用策略基於流量選擇器，則需要確保 VPN 設備具有與本地網路（本地網路閘道）首碼的所有組合一起定義的匹配流量選擇器，而不是任何到任何。

配置不當可能導致隧道內頻繁斷開連接、資料包丟棄、輸送量錯誤和延遲。
...
如果您在進入 MS 網路骨幹之前發現任何躍點出現高延遲峰值，則可能需要與 Internet 服務提供者展開進一步調查。
