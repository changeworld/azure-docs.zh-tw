...
若要解決此問題，請[重設 Azure VPN 閘道](./reset-gateway.md)。 若要確保會使用新的路由，成功設定虛擬網路對等互連之後，必須再次下載「點對站」VPN 用戶端。
...
請確定已正確設定 RADIUS 伺服器。 如需詳細資訊，請參閱 [整合 RADIUS 驗證與 Azure AD Multi-Factor Authentication Server](../active-directory/authentication/howto-mfaserver-dir-radius.md)。
