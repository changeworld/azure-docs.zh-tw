---
title: 創建&安裝 P2S VPN 用戶端設定檔：證書身份驗證
...
VPN 用戶端組態檔包含在 ZIP 檔案內。 設定檔提供本機 Windows、Mac IKEv2 VPN 或 Linux 用戶端通過使用本機 Azure 證書身份驗證的點對點連接連接到虛擬網路所需的設置。

用戶端設定檔特定于虛擬網路的 VPN 配置。 如果您在產生 VPN 用戶端組態檔後對「點對站 VPN 組態」進行任何變更 (例如 VPN 通訊協定類型或驗證類型)，請務必為使用者裝置產生新的 VPN 用戶端組態檔。 
...
2. 在虛擬網路閘道頁面上，按一下 [點對站組態]****。
   ...
   ![下載用戶端門戶](./media/point-to-site-vpn-client-configuration-azure-cert/client-configuration-portal.png)
...
2. 對套件按兩下來加以安裝。 如果您看到智慧螢幕快顯視窗，請按一下 **"更多資訊**"，然後**無論如何運行**。
3. 在用戶端電腦上，導航到**網路設置**，然後按一下**VPN**。 VPN 連線會顯示其連線的虛擬網路名稱。 
4. 嘗試連線之前，請確認您已在用戶端電腦上安裝用戶端憑證。 使用原生 Azure 憑證驗證類型時，必須提供用戶端憑證才能通過驗證。 有關生成證書的詳細資訊，請參閱[生成證書](vpn-gateway-howto-point-to-site-resource-manager-portal.md#generatecert)。 有關如何安裝用戶端憑證的資訊，請參閱[安裝用戶端憑證](point-to-site-how-to-vpn-client-install-azure-cert.md)。
...
* **VpnSettings.xml**，它包含伺服器位址和隧道類型等重要設置。 
* **VpnServerRoot.cer**，其中包含在 P2S 連接設置期間驗證 Azure VPN 閘道所需的根憑證。
...
2. 確認您已安裝用戶端憑證，且該憑證由您進行 P2S 設定時上傳到 Azure 的根憑證簽發。 此用戶端憑證與您在上個步驟中安裝的 VPNServerRoot 不同， 此為必要憑證，驗證時需提供。 有關生成證書的詳細資訊，請參閱[生成證書](vpn-gateway-howto-point-to-site-resource-manager-portal.md#generatecert)。 有關如何安裝用戶端憑證的資訊，請參閱[安裝用戶端憑證](point-to-site-how-to-vpn-client-install-azure-cert.md)。
3. 打開 **"網路首選項**"下的 **"網路**"對話方塊，然後按一下 **"+"** 以創建新的 VPN 用戶端連接設定檔，用於 P2S 連接到 Azure 虛擬網路。
...
9. 在 [網路]**** 對話方塊中按一下 [套用]**** 以儲存所有變更。 然後，按一下 **"連接**"以啟動到 Azure 虛擬網路的 P2S 連接。
...
### <a name="install-and-configure"></a><a name="install"></a>安裝和配置

以下說明在 Ubuntu 18.0.4 上創建。 Ubuntu 16.0.10 並不支援 strongSwan GUI。 如果您想要使用 Ubuntu 16.0.10，就必須使用[命令列](#linuxinstallcli)。 視您的 Linux 和 strongSwan 版本而定，下列範例可能會與您看到的畫面不相符。
...
4. 從功能表中選擇**IPsec/IKEv2（強天鵝），** 然後按兩下。 您可以在此步驟中命名連接。

   ![選擇連線類型](./media/point-to-site-vpn-client-configuration-azure-cert/choosetype.png)
5. 從所下載用戶端組態檔中包含的 [Generic]**** 資料夾開啟 **VpnSettings.xml** 檔案。 查找名為**VpnServer**的標記並複製名稱，以"azure 閘道"開頭，以".cloudapp.net"結尾。
...
7. 在連線的 [Client] \(用戶端\)**** 區段中，針對 [Authentication] \(驗證\)****，選取 [Certificate/private key] \(憑證/私密金鑰\)****。 針對 [Certificate] \(憑證\)**** 和 [Private key] \(私密金鑰\)****，選擇稍早建立的憑證和私密金鑰。 在 [Options] \(選項\)**** 中，選取 [Request an inner IP address] \(要求內部 IP 位址\)****. 然後，按一下"**添加**"。
