...
5. 從所下載用戶端組態檔中包含的 [Generic]**** 資料夾開啟 **VpnSettings.xml** 檔案。 尋找名為**VpnServer**的標記，並複製名稱，開頭為 ' azuregateway '，結尾為 '. cloudapp.net '。
...
7. 在連線的 [Client] \(用戶端\)**** 區段中，針對 [Authentication] \(驗證\)****，選取 [Certificate/private key] \(憑證/私密金鑰\)****。 針對 [Certificate] \(憑證\)**** 和 [Private key] \(私密金鑰\)****，選擇稍早建立的憑證和私密金鑰。 在 [Options] \(選項\)**** 中，選取 [Request an inner IP address] \(要求內部 IP 位址\)****. 然後，按一下 [**新增**]。
