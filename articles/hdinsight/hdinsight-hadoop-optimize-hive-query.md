...
例如，選擇 **Interactive Query** 叢集類型，以針對 `ad hoc` 互動式查詢進行優化。 選擇 Apache **Hadoop** 叢集類型，將作為批次程序使用的 Hive 查詢最佳化。 **Spark** 與 **HBase** 叢集類型也可以執行 Hive 查詢。 如需針對不同 HDInsight 叢集類型執行 Hive 查詢的詳細資訊，請參閱[Azure HDInsight 上的 Apache Hive 和 HiveQL 是什麼？](hadoop/hdinsight-use-hive.md)。

根據預設，Hadoop 叢集類型的 HDInsight 叢集並未針對效能優化。 本文說明幾個將 Hive 效能最佳化的最常見方法，您可將這些方法套用於查詢。
