...
此版本適用於 HDInsight 3.6 和 4.0。 HDInsight 版本可在幾天內提供給所有區域。 此處的發佈日期表示第一個區域發佈日期。 如果您沒有看到以下更改,請等待該版本在幾天內在您所在的地區中使用。
...
使用此版本,客戶只能選擇通過公共群集終結點的所有連接加入 TLS 1.2。 為此,引入了新屬性**minSupportTlsVersion,** 可以在群集創建期間指定。 如果未設置該屬性,群集仍支援 TLS 1.0、1.1 和 1.2,這與今天的行為相同。 客戶可以將此屬性的值設置為"1.2",這意味著群集僅支援 TLS 1.2 及以上。 有關詳細資訊,請參閱[傳輸層安全](./transport-layer-security.md)。

### <a name="bring-your-own-key-for-disk-encryption"></a>自帶金鑰進行磁碟加密
HDInsight 中的所有受控磁碟都會使用 Azure 儲存體服務加密 (SSE) 來加以保護。 默認情況下,這些磁碟上的數據由 Microsoft 管理的密鑰加密。 從此版本開始,您可以自帶密鑰 (BYOK) 進行磁碟加密,並使用 Azure 密鑰保管庫對其進行管理。 BYOK 加密是在群集創建期間進行的單步配置,無需額外費用。 只需將 HDInsight 註冊為 Azure 密鑰保管庫的託管標識,並在創建群集時添加加密金鑰。 關於詳細資訊,請參考[客戶管理金鑰磁碟加密](https://docs.microsoft.com/azure/hdinsight/disk-encryption)。
...
頭節點至少需要 4 核 VM,以確保 HDInsight 叢集的高可用性和可靠性。 從 2020 年 4 月 6 日起,客戶只能選擇 4 核或以上 VM 作為新 HDInsight 叢集的頭節點。 現有群集將繼續按預期運行。 

### <a name="esp-spark-cluster-node-size-change"></a>ESP 火花叢集節點大小變更 
在即將發佈的版本中,ESP Spark 群集的最小允許節點大小將更改為Standard_D13_V2。 由於 CPU 和記憶體容量相對較低,A 系列 VM 可能會導致 ESP 群集問題。 A 系列 VM 將被棄用以創建新的 ESP 群集。

### <a name="moving-to-azure-virtual-machine-scale-sets"></a>移至 Azure 虛擬機器縮放集
HDInsight 現在使用 Azure 虛擬機器來預配群集。 在即將發佈的版本中,HDInsight 將改用 Azure 虛擬機縮放集。 查看有關 Azure 虛擬機器縮放集的更多。
...
有關其他說明,請建立[支援要求](https://ms.portal.azure.com/#blade/Microsoft_Azure_Support/HelpAndSupportBlade/overview)。
