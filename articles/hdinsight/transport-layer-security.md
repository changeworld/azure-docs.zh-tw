...
透過公用叢集端點`https://CLUSTERNAME.azurehdinsight.net`連線到 HDInsight 叢集的方式，會經由叢集閘道節點進行代理。 這些連接是使用稱為 TLS 的通訊協定來保護。 在閘道上強制執行較高版本的 TLS 可改善這些連線的安全性。 如需有關為何要使用較新版本 TLS 的詳細資訊，請參閱[解決 tls 1.0 問題](https://docs.microsoft.com/security/solving-tls1-problem)。
