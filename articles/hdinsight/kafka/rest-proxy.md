...
Kafka REST 代理使您能夠通過 REST API 通過 HTTP 與卡夫卡群集進行交互。 這意味著您的 Kafka 用戶端可以位於虛擬網路之外。 此外,用戶端可以進行簡單的 HTTP 調用,以向 Kafka 群集發送和接收消息,而不是依賴 Kafka 庫。 本教學將介紹如何建立啟用 REST 代理的 Kafka 群集,並提供範例代碼,演示如何調用 REST 代理。

## <a name="rest-api-reference"></a>REST API 參考資料

有關 Kafka REST API 支援的動作的完整規範,請參閱[HDInsight Kafka REST 代理 API 參考](https://docs.microsoft.com/rest/api/hdinsight-kafka-rest-proxy)。

## <a name="background"></a>背景

![卡夫卡 REST 代理結構](./media/rest-proxy/rest-proxy-architecture.png)
...
使用 Azure 活動目錄安全組管理對 Kafka REST 代理的訪問。 在啟用 REST 代理的情況下創建 Kafka 群集時,將提供應有權存取 REST 終結點的 Azure 活動目錄安全組。 需要存取 REST 代理的 Kafka 用戶端(應用程式)應由組擁有者註冊到此組。 組擁有者可以通過門戶或通過 PowerShell 執行此操作。

在向 REST 代理終結點發出請求之前,用戶端應用程式應獲取 OAuth 令牌以驗證正確的安全組的成員身份。 尋找下面的[客戶端應用程式範例](#client-application-sample),該示例示範如何取得 OAuth 權杖。 一旦用戶端應用程式具有 OAuth 權杖,它們就必須在提供給 REST 代理的 HTTP 請求中傳遞該權杖。
...
1. 創建 Azure AD 安全組,並將在 Azure AD 中註冊的應用程式作為組的「成員」添加到安全組。 此安全組將用於控制允許哪些應用程式與 REST 代理進行互動。 有關建立 Azure AD 群組的詳細資訊,請參閱[建立基本群組並使用 Azure 活動目錄新增成員](../../active-directory/fundamentals/active-directory-groups-create-azure-portal.md)。

    驗證群組的類型為「安全」![安全組](./media/rest-proxy/rest-proxy-group.png)

    驗證應用程式是否是群組![驗證成員資格的成員](./media/rest-proxy/rest-proxy-membergroup.png)
...
1. 變更代碼部分*設定這些屬性*並更新環境的以下屬性:
    ...
    1.    *用戶端識別碼*= 在安全組中註冊的應用程式的 ID。
    1.    *客戶端金鑰*─您在安全群組中註冊的應用程式的機密
...
1. 從 Azure AD 取得 OAuth 權杖
1. 展示如何向卡夫卡 REST 代理發出要求

有關在 python 中取得 OAuth 權杖的詳細資訊,請參考[Python 身份驗證內容 。](https://docs.microsoft.com/python/api/adal/adal.authentication_context.authenticationcontext?view=azure-python) 您可能會看到延遲,而未通過 Kafka REST 代理創建或刪除的主題將反映在那裡。 此延遲是由於緩存刷新。
