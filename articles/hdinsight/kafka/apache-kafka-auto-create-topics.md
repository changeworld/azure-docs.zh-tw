...
description: 了解如何將 HDInsight 上的 Apache Kafka 設定為自動建立主題。 您可以設定 Kafka，方法`auto.create.topics.enable`是透過 Ambari 將設為 true。 或在叢集建立期間透過 PowerShell 或 Resource Manager 範本。
...
1. 從 [叢集**儀表板**] 中，選取 [ **Ambari home**]。
    ...
    出現提示時，使用叢集的登入 (系統管理員) 認證進行驗證。 相反地，您可以直接從`https://CLUSTERNAME.azurehdinsight.net/` （其中`CLUSTERNAME`是您的 Kafka 叢集名稱）連接到 Amabri。
