...
1. 建立兩個新的使用者： **HDIAdmin**和**HDIUser**。 這兩個使用者會登入 HDInsight 叢集。
    ...
    1. 在 [ **Active Directory 使用者和電腦**] 頁面上，以`HDIFabrikam.com`滑鼠右鍵按一下，然後流覽至 [**新增** > **使用者**]。
    ...
    1. 在 [**新增物件-使用者**] 頁面上`HDIUser` ，輸入作為 [**名字**] 和 [**使用者登入名稱**]。 其他欄位將會自動填入。 然後選取 [下一步]  。
...
1. 建立全域安全性群組。
    ...
    1. 從**Active Directory 使用者和電腦**] 中，以`HDIFabrikam.com`滑鼠右鍵按一下，然後流覽至 [**新增** > **群組**]。

    1. 在`HDIUserGroup` [**組名**] 文字方塊中輸入。
...
1. 將成員新增至**HDIUserGroup**。
    ...
    1. 在 [**輸入要選取的物件名稱**] 文字方塊中， `HDIUserGroup`輸入。 然後在快顯視窗中選取 **[確定]** 和 **[確定]** 。
...
1. 登入 Azure 入口網站。
1. 選取 [**建立資源**]， `directory`然後輸入。 選取 [ **Azure Active Directory** > **建立**]。
1. 在 [**組織名稱**] `HDIFabrikam`下，輸入。
1. 在 [**初始功能變數名稱**] 下`HDIFabrikamoutlook`，輸入。
1. 選取 [建立]  。
...
1. 選取 [ **+ 新增自訂網域**]。
1. 在 [**自訂功能變數名稱**] `HDIFabrikam.com`下，輸入，然後選取 [**新增網域**]。
...
1. 選取 [ **+ 新增群組**]。
1. 在 [**組名**] 文字方塊中， `AAD DC Administrators`輸入。
1. 選取 [建立]  。
...
1. 登入 Azure 入口網站。
1. 選取 [**建立資源**]， `managed identity`然後輸入。 選取 [**使用者指派的受控識別** > ] [**建立**]。
1. 針對 [**資源名稱**]， `HDIFabrikamManagedIdentity`輸入。
1. 選取您的訂用帳戶。
1. 在 [**資源群組**] 下，選取 [ `HDIFabrikam-CentralUS`**建立新**的] 並輸入。
1. 在 [**位置**] 底下，選取 [**美國中部**]。
1. 選取 [建立]  。
...
1. 登入 Azure 入口網站。
1. 選取 **[建立資源**] `Domain services`，輸入，然後選取 [ **Azure AD Domain Services** > **建立**]。
1. 在 [**基本**] 頁面上：
...
1. 為 Azure AD DS 受控網域啟用 LDAPS。
    1. 從 [Azure 入口網站中，選取 [ `HDIFabrikam.com`網域]。
    1. 在 [**管理**] 下，選取 [**安全 LDAP**]。
