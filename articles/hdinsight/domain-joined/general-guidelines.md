...
* 針對 Hive，除了擁有建立/更新/刪除許可權的許可權之外，使用者也應該擁有儲存體和所有子目錄上目錄的 `rwx`許可權。
...
* 同盟使用者需要 `AllowCloudPasswordValidation` 的原則。 由於 HDInsight 會直接使用使用者名稱/密碼來取得來自 Azure AD 的權杖，因此必須為所有同盟使用者啟用此原則。
