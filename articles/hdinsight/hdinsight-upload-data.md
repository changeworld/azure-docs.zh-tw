...
* Azure HDInsight 叢集。 如需指示，請參閱[開始使用 Azure HDInsight](hadoop/apache-hadoop-linux-tutorial-get-started.md)。
* 下列文章的知識：
...
> [!WARNING]  
> 在 Apache HBase 叢集上，寫入資料時所使用的預設區塊大小是 256 KB。 雖然在使用 HBase API 或 REST API 時此大小可正常運作，但使用 `hadoop` 或 `hdfs dfs` 命令寫入大於 ~12 GB 的資料會導致錯誤。 如需詳細資訊，請參閱[在 blob 上寫入的儲存例外](hdinsight-troubleshoot-hdfs.md#storage-exception-for-write-on-blob)狀況。
...
Azure Data Factory 服務是一種完全受控的服務，可用於撰寫資料：儲存、處理和移動服務，成為簡化、可調整且可靠的資料生產管線。
...
Sqoop 是一種專門在 Hadoop 和關聯式資料庫之間傳送資料的工具。 使用它從關係資料庫管理系統（RDBMS）匯入資料，例如 SQL Server、MySQL 或 Oracle。 然後進入 Hadoop 分散式檔案系統（HDFS）。 使用 MapReduce 或 Hive 轉換 Hadoop 中的資料，然後將資料匯出回 RDBMS。
