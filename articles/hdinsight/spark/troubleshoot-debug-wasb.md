---
title: Azure HDInsight 中的 Debug WASB 檔案作業
description: 描述與 Azure HDInsight 叢集互動時，問題的疑難排解步驟和可能的解決方式。
...
ms.locfileid: "92545543"
---
# <a name="debug-wasb-file-operations-in-azure-hdinsight"></a>Azure HDInsight 中的 Debug WASB 檔案作業

有時候您可能會想要瞭解 WASB 驅動程式 Azure 儲存體啟動的作業。 針對用戶端，WASB 驅動程式會在 **DEBUG** 層級為每個檔案系統作業產生記錄檔。 WASB 驅動程式會使用 log4j 來控制記錄層級，而預設值為 **INFO** 層級。 如 Azure 儲存體伺服器端分析記錄，請參閱 [Azure 儲存體分析記錄](../../storage/common/storage-analytics-logging.md)。
...
上述記錄檔應提供檔案系統作業的高階瞭解。 如果上述記錄仍未提供有用的資訊，或您想要調查 blob 儲存體 api 呼叫，請將新增 `fs.azure.storage.client.logging=true` 至 `core-site` 。 這項設定會啟用 wasb 儲存體驅動程式的 java sdk 記錄，並會列印每次對 blob 儲存體伺服器的呼叫。 請在調查後移除設定，因為它可能會快速填滿磁片，而且可能會使進程變慢。
