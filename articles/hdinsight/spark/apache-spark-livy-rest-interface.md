...
了解如何使用可將遠端作業提交至 Azure HDInsight Spark 叢集的 [Apache Livy](https://livy.incubator.apache.org/) (也就是 Apache Spark REST API)。 有關詳細文檔，請參閱[Apache Livy](https://livy.incubator.apache.org/docs/latest/rest-api.html)。
...
* 如果在將作業遠端提交到 Spark 群集後，Livy 服務出現故障，則該作業將繼續在後臺運行。 當 Livy 恢復運作時，它會還原作業的狀態並回報。
...
在本節中，我們將透過範例了解如何使用 Livy Spark 來提交批次作業、監視作業的進度，然後加以刪除。 我們在此範例中使用的應用程式，就是 [建立獨立 Scala 應用程式，並在 HDInsight Spark 叢集上執行](apache-spark-create-standalone-application.md)一文中所開發的應用程式。 此處的步驟假定：

* 您已經通過應用程式 jar 複製到與群集關聯的存儲帳戶。
* 已安裝在嘗試這些步驟的電腦上。

執行下列步驟：

1. 為方便使用，設置環境變數。 此示例基於 Windows 環境，根據需要修改環境所需的變數。 用`CLUSTERNAME`相應的值`PASSWORD`替換 和 。
...
1. 驗證 Livy Spark 是否在群集上運行。 我們可以取得執行中的批次清單，加以確認。 如果首次使用 Livy 運行作業，則輸出應返回零。
...
1. 現在，我們要提交批次作業。 下列程式碼片段會使用輸入檔案 (input.txt) 傳遞 jar 名稱和類別名稱來作為參數。 如果從 Windows 電腦運行這些步驟，則建議使用輸入檔。
...
1. 現在，您可以視需要刪除批次。
    ...
    輸出的最後一行顯示批次已成功刪除。 刪除作業時，在作業運行時也會終止作業。 如果您刪除已完成的作業，無論成功與否，這將會完全刪除作業資訊。
