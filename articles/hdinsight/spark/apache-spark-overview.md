...
| 延展性 | HDInsight 可讓您使用自動調整功能來動態變更叢集節點的數目。 請參閱[自動調整 Azure HDInsight 叢集規模](../hdinsight-autoscale-clusters.md)。 此外，由於所有資料都儲存在 Azure 儲存體或 Data Lake Storage 內，因此您可以在不遺失資料的情況下卸除 Spark 叢集。 |
