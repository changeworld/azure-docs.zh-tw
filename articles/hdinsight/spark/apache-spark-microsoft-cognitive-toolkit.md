---
title: 微軟認知工具組與阿帕奇火花 - Azure HDInsight
...
1. 運行自訂腳本以在 Azure HDInsight Spark 群集上安裝[Microsoft 認知工具組](https://docs.microsoft.com/cognitive-toolkit/)。
...
此解決方案分為本文和您作為本文一部分上傳的 Jupyter 筆記本。 在本文中，您必須完成下列步驟：
...
* 將示例圖像載入到 Spark 彈性分散式資料集或 RDD 中。
  * 載入模組並定義預設值。
  * 在 Spark 叢集上本機下載資料集。
  * 將資料集轉換成 RDD。
...
您可以使用指令碼動作在 Spark 叢集上安裝 Microsoft 辨識工具組。 腳本操作使用自訂腳本在群集上安裝預設情況下不可用的元件。 您可以使用 Azure 門戶中的自訂腳本、使用 HDInsight .NET SDK 或使用 Azure PowerShell。 您也可以使用指令碼，在叢集建立期間安裝工具組，或在叢集已啟動並執行之後加以安裝。
...
有關如何使用 Azure 門戶運行腳本操作的說明，請參閱[使用腳本操作自訂 HDInsight 群集](../hdinsight-hadoop-customize-cluster-linux.md#script-action-during-cluster-creation)。 確定您提供下列輸入來安裝 Microsoft 辨識工具組。 對腳本操作使用以下值：
...
若要使用 Microsoft 辨識工具組搭配 Azure HDInsight Spark 叢集，您必須將 Jupyter Notebook **CNTK_model_scoring_on_Spark_walkthrough.ipynb** 載入至 Azure HDInsight Spark 叢集。 此筆記本可在 GitHub 上[https://github.com/Azure-Samples/hdinsight-pyspark-cntk-integration](https://github.com/Azure-Samples/hdinsight-pyspark-cntk-integration)提供。
...
1. 從 Jupyter 筆記本中，選擇右上角的 **"上傳"，** 然後導航到下載並選擇檔`CNTK_model_scoring_on_Spark_walkthrough.ipynb`。

    ![將 Jupyter 筆記本上載到 Azure HDInsight Spark 群集](./media/apache-spark-microsoft-cognitive-toolkit/hdinsight-microsoft-cognitive-toolkit-load-jupyter-notebook.png "將 Jupyter 筆記本上載到 Azure HDInsight Spark 群集")

1. 再次選擇 **"上傳**"。

1. 上傳筆記本後，按一下筆記本的名稱，然後按照筆記本本身的說明操作如何載入資料集並執行文章。
