...
安裝在 HDInsight 叢集上的元件會相依于協力廠商程式庫。 通常，這些內建元件會參考 Guava 之類的通用模組特定版本。 當您提交具有相依性的應用程式時，可能會導致相同模組的不同版本之間發生衝突。 如果您先在路徑類型中參考的元件版本，內建元件可能會因為版本不相容而擲回例外狀況。 不過，如果內建元件會先將其相依性插入至路徑，您的應用程式可能會擲回 `NoSuchMethod`之類的錯誤。
...
下列範例顯示已更新的檔案 `pom.xml` 使用 maven-陰影-外掛程式來為封裝加上陰影。  XML 區段 `<relocation>…</relocation>` 藉由移動對應的 JAR 檔案專案並重寫受影響的位元組程式碼，將類別從封裝 `com.google.guava` 移至封裝 `com.google.shaded.guava`。

變更 `pom.xml`之後，您可以執行 `mvn package` 來建立陰影 uber。
...
例如，若要為 `com.google.guava`加上陰影，您可以將下列命令新增至 `build.sbt` 檔案：
...
接著，您可以執行 `sbt clean` 和 `sbt assembly` 來建立陰影的 jar 檔案。 
