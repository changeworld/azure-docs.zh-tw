...
3. 若要將 Bind 設定為將名稱解析要求轉寄到內部部署 DNS 伺服器，請使用下列文字做為 `/etc/bind/named.conf.options` 檔案的內容：
    ...
    >
    > 將 `192.168.0.1` 區段中的 `forwarders` 項目取代為內部部署 DNS 伺服器的 IP 位址。 此項目會將 DNS 要求路由到您的內部部署 DNS 伺服器以進行解析。
...
4. 在 SSH 工作階段中，使用下列命令：
    ...
    `icb0d0thtw0ebifqt0g1jycdxd.ex.internal.cloudapp.net` 文字是此虛擬網路的 __DNS 尾碼__。 儲存此值，因為它會在稍後使用。
