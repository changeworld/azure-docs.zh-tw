...
1. 列出目前使用下列命令執行 Yarn 應用程式的所有應用程式識別碼：
    ...
    請記下要下載其`APPLICATIONID`記錄的資料行中的 [應用程式識別碼]。
...
1. 使用下列命令，下載所有應用程式主機的 Yarn 容器記錄。 此步驟會建立以文字格式命名`amlogs.txt`的記錄檔。
...
YARN `ResourceManager` UI 會在叢集前端節點上執行。 它是透過 Ambari web UI 來存取。 請使用下列步驟來檢視 YARN 記錄：
...
3. 從 [**快速連結**] 下拉式清單中，選取其中一個叢集前端節點， **`ResourceManager Log`** 然後選取 []。
