...
ms.locfileid: "87097461"
---
# <a name="configure-virtual-network-service-endpoint-policies-for-azure-hdinsight"></a>設定 Azure HDInsight 的虛擬網路服務端點原則

本文提供如何使用 Azure HDInsight 在虛擬網路上執行服務端點原則的相關資訊。

## <a name="background"></a>背景

Azure HDInsight 可讓您在自己的虛擬網路中建立叢集。 如果您需要允許從虛擬網路到其他 Azure 服務（例如儲存體帳戶）的連出流量，您可以建立[服務端點原則](../virtual-network/virtual-network-service-endpoint-policies-overview.md)。 不過，透過 Azure 入口網站建立的服務端點原則，只允許您為單一帳戶、訂用帳戶中的所有帳戶，或資源群組中的所有帳戶建立原則。

不過，身為受控服務，Azure HDInsight 會從每個區域的特定儲存體帳戶中的每個叢集收集資料和記錄檔。 為了讓此資料能夠從您的虛擬網路連線到 HDInsight，您必須建立服務端點原則，以允許連到 Azure HDInsight 所管理之特定資料收集點的連出流量。

## <a name="service-endpoint-policies-for-hdinsight"></a>HDInsight 的服務端點原則

這些服務端點原則支援下列功能：

- 叢集建立、作業執行和平臺作業（例如調整）的記錄和遙測集合。
- 將虛擬硬碟（Vhd）連接到新建立的叢集節點，以在叢集上布建軟體和程式庫。
...
在建立新叢集之前，請確定已將正確的服務端點原則附加至您的虛擬網路。 否則，叢集建立可能會失敗或導致錯誤。

使用下列程式來建立必要的服務端點原則：

1. 決定您將在其中建立 HDInsight 叢集的區域。
1. 在[服務端點原則資源的清單](https://github.com/Azure-Samples/hdinsight-enterprise-security/blob/main/hdinsight-service-endpoint-policy-resources.json)中查閱該區域，其中會提供 HDInsight 管理儲存體帳戶的所有資源群組。
1. 選取您的區域的資源群組清單。 資源的範例如下 `Canada Central` 所示：
...
1. 將該資源群組清單插入以 Azure CLI 或 Azure PowerShell 撰寫的安裝腳本中。
