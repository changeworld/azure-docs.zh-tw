---
title: 在 Azure HDInsight 中的 Apache Hadoop 叢集上使用空邊緣節點
description: 如何向 HDInsight 群集添加空邊緣節點。 用作用戶端,然後進行測試或託管 HDInsight 應用程式。
...
了解如何將空白邊緣節點新增至 HDInsight 叢集。 空白的邊緣節點是一部 Linux 虛擬機器，其中已安裝及設定和前端節點相同的用戶端工具。 但是,沒有[阿帕奇哈多普](./hadoop/apache-hadoop-introduction.md)服務運行。 您可以使用邊緣節點來存取叢集、測試用戶端應用程式，以及裝載用戶端應用程式。

您可以將空白邊緣節點新增至現有 HDInsight 叢集，以及在建立叢集時新增到新的叢集。 空白邊緣節點是使用 Azure Resource Manager 範本來新增。  以下範例展示如何使用樣本完成:
...
如示例中所示,您可以選擇調用[腳本操作](hdinsight-hadoop-customize-cluster-linux.md)以執行其他配置。 例如在邊緣節點中安裝[Apache 色調](hdinsight-hadoop-hue-linux.md)。 指令碼動作指令碼必須可在網站上公開存取。  例如,如果腳本儲存在 Azure 儲存中,請使用公共容器或公共 Blob。
...
創建邊緣節點後,可以使用 SSH 連接到邊緣節點,並運行用戶端工具以存取 HDInsight 中的 Hadoop 群集。
...
>
> 如果您使用的是 Apache 技術,則[https://apache.org](https://apache.org)可以通過上的 Apache 專案網站(如 Apache [Hadoop](https://hadoop.apache.org/)網站)找到説明。
...
在本節中，您會使用 Resource Manager 範本將邊緣節點新增至現有 HDInsight 叢集。  Resource Manager 範本可在 [GitHub](https://azure.microsoft.com/resources/templates/101-hdinsight-linux-add-edge-node/)中找到。 資源管理器範本調用位於https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/101-hdinsight-linux-add-edge-node/scripts/EmptyNodeSetup.sh的腳本操作。腳本不執行任何操作。  它是演示從資源管理器範本調用腳本操作。
...
1. 檢查**我同意上述條款和條件**,然後選擇 **"購買"** 以創建邊緣節點。
...
1. 檢查**我同意上述條款和條件**,然後選擇 **"購買"** 以創建具有邊緣節點的群集。

## <a name="add-multiple-edge-nodes"></a>新增多個邊緣節點

您可以將多個邊緣節點新增至 HDInsight 叢集。  多個邊緣節點的設定只能使用 Azure Resource Manager 範本來執行。  請參閱本文開頭處的範本範例。  更新**目標實例計數**以反映要創建的邊緣節點數。
...
2. 使用 SSH 連線到邊緣節點後,請使用以下指令開啟 Hive 主控台:
...
4. 右鍵按一下要刪除的邊緣節點,然後選擇 **「刪除**」。
5. 選取 [是] **** 加以確認。

## <a name="next-steps"></a>後續步驟

在本文中,您學習了如何添加邊緣節點以及如何存取邊緣節點。 如需詳細資訊，請參閱下列文章：
