...
在一般叢集上，您只能有一個具有色調的使用者帳戶。 針對多使用者存取，請在叢集上啟用[企業安全性套件](./domain-joined/hdinsight-security-overview.md)。 SSH 通道是在叢集上執行時存取其色調的唯一方法。 透過 SSH 的通道允許直接至在其中執行 Hue 之叢集的前端節點的流量。 叢集完成布建之後，請使用下列步驟在 HDInsight 叢集上使用色調。
...
1. 色調不會瞭解 Apache Tez 作業，這是 Hive 目前的預設值。 如果您想要使用 MapReduce 做為 Hive 執行引擎，請更新指令碼以在您的指令碼中使用下列命令：
