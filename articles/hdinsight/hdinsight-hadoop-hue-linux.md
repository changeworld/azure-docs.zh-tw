...
>
> 自訂元件則獲得商務上合理的支援，協助您進一步疑難排解問題。 如此可能會進而解決問題，或要求您利用可用管道，以找出開放原始碼技術，從中了解該技術的深度專業知識。 例如,有許多社區網站可以使用,例如[:HDInsight 的 MSDN 論壇](https://social.msdn.microsoft.com/Forums/azure/en-US/home?forum=hdinsight)。 [https://stackoverflow.com](https://stackoverflow.com) 此外,Apache 專案也有[https://apache.org](https://apache.org)專案站台,例如: [Hadoop](https://hadoop.apache.org/)。

## <a name="install-hue-using-script-actions"></a>使用指令碼動作安裝 Hue

使用下表中的資訊進行腳本操作。 有關使用文稿操作的具體說明,請參閱[使用文稿操作自訂 HDInsight 叢集](hdinsight-hadoop-customize-cluster-linux.md)。
...
在常規群集上,只能有一個具有 Hue 的使用者帳戶。 對於多使用者存取,在群集上啟用[企業安全包](./domain-joined/hdinsight-security-overview.md)。 SSH 隧道是群集上運行後訪問 Hue 的唯一方法。 透過 SSH 的通道允許直接至在其中執行 Hue 之叢集的前端節點的流量。 叢集完成預配後,請使用以下步驟在 HDInsight 叢集上使用 Hue。
...
1. 使用瀏覽器開啟位於 `http://HOSTNAME:8888` 的 Hue 入口網站。 以您在先前步驟取得的名稱取代 HOSTNAME。
   ...
   > 當您第一次登入時，系統會提示您建立帳戶來登入 Hue 入口網站。 您在此處指定的認證會限制為入口網站，並且與佈建叢集時您指定的系統管理員或 SSH 使用者認證不相關。
    ...
    ![HDInsight 色調門戶登入視窗](./media/hdinsight-hadoop-hue-linux/hdinsight-hue-portal-login.png "為 Hue 門戶指定認證")

### <a name="run-a-hive-query"></a>執行 HIVE 查詢

1. 從 Hue 門戶中,選擇 **「查詢編輯器**」,然後選擇**Hive**以打開 Hive 編輯器。

    ![HDInsight 色調門使用者使用設定單元編輯器](./media/hdinsight-hadoop-hue-linux/hdinsight-hue-portal-use-hive.png "使用 Hive")
...
2. 在 [協助]**** 索引標籤中，於 [資料庫]**** 底下，您應該會看到 **hivesampletable**。 這是 HDInsight 上的所有 Hadoop 叢集隨附的範例資料表。 在右窗格中輸入範例查詢，然後在下方窗格的 [結果] **** 索引標籤中查看輸出，如螢幕擷取畫面所示。
    ...
    ![HDInsight 色調門戶配置單元查詢](./media/hdinsight-hadoop-hue-linux/hdinsight-hue-portal-hive-query.png "執行 Hive 查詢")

    您也可以使用 [圖表] **** 索引標籤來查看結果的視覺表示。

### <a name="browse-the-cluster-storage"></a>瀏覽叢集儲存體

1. 在 Hue 門戶中,在菜單欄的右上角選擇 **「檔案瀏覽器**」。
2. 根據預設，檔案瀏覽器會在 **/user/myuser** 目錄中開啟。 在路徑中的使用者目錄之前選擇正向斜杠,以轉到與群集關聯的 Azure 儲存容器的根目錄。

    ![HDInsight 色調門戶瀏覽器](./media/hdinsight-hadoop-hue-linux/hdinsight-hue-portal-file-browser.png "使用檔案瀏覽器")
...
1. Hue 不理解 Apache Tez 作業,這是 Hive 當前預設的。 如果您想要使用 MapReduce 做為 Hive 執行引擎，請更新指令碼以在您的指令碼中使用下列命令：
...
1. 使用 Linux 叢集，您就可以擁有在主要前端節點上執行服務，在次要前端節點上執行 Resource Manager 的案例。 使用 Hue 以檢視叢集上「執行中」工作的詳細資料時，此類案例可能會導致錯誤 (如下所示)。 不過，您可以在工作完成時檢視工作詳細資料。
   ...
   ![色調門戶錯誤範例訊息](./media/hdinsight-hadoop-hue-linux/hdinsight-hue-portal-error.png "Hue 入口網站錯誤")
