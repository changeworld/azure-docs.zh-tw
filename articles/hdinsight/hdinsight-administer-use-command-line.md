...
登入您的 Azure 訂用帳戶。 如果您打算使用 Azure Cloud Shell，則選取程式碼區塊右上角的 [試試看]  。 或者，請輸入以下命令：
...
使用[az hdinsight show](https://docs.microsoft.com/cli/azure/hdinsight?view=azure-cli-latest#az-hdinsight-show)顯示指定叢集的資訊。 藉由將、和`RESOURCE_GROUP_NAME` `CLUSTER_NAME`相關資訊取代來編輯下面的命令，然後輸入命令：
...
使用[az hdinsight delete](https://docs.microsoft.com/cli/azure/hdinsight?view=azure-cli-latest#az-hdinsight-delete)刪除指定的叢集。 藉由將、和`RESOURCE_GROUP_NAME` `CLUSTER_NAME`相關資訊取代來編輯下面的命令，然後輸入命令：
...
使用[az hdinsight resize](https://docs.microsoft.com/cli/azure/hdinsight?view=azure-cli-latest#az-hdinsight-resize)將指定的 hdinsight 叢集大小調整為指定的大小。 藉由以相關資訊取代`RESOURCE_GROUP_NAME`和`CLUSTER_NAME` ，來編輯下面的命令。 將`WORKERNODE_COUNT`取代為您的叢集所需的背景工作節點數目。 如需調整叢集的詳細資訊，請參閱[調整 HDInsight 叢集規模](./hdinsight-scaling-best-practices.md)。 輸入命令：
