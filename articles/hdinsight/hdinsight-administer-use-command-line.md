...
使用 [az hdinsight list](https://docs.microsoft.com/cli/azure/hdinsight?view=azure-cli-latest#az-hdinsight-list) 列出叢集。 以您的資源組名取代來編輯下列命令 `RESOURCE_GROUP_NAME` ，然後輸入命令：
...
使用 [az hdinsight show](https://docs.microsoft.com/cli/azure/hdinsight?view=azure-cli-latest#az-hdinsight-show) 來顯示指定叢集的資訊。 藉由取代和相關資訊來編輯下列命令 `RESOURCE_GROUP_NAME` `CLUSTER_NAME` ，然後輸入命令：
...
使用 [az hdinsight delete](https://docs.microsoft.com/cli/azure/hdinsight?view=azure-cli-latest#az-hdinsight-delete) 來刪除指定的叢集。 藉由取代和相關資訊來編輯下列命令 `RESOURCE_GROUP_NAME` `CLUSTER_NAME` ，然後輸入命令：
...
使用 [az hdinsight 重設](https://docs.microsoft.com/cli/azure/hdinsight?view=azure-cli-latest#az-hdinsight-resize) 大小，將指定的 hdinsight 叢集大小調整為指定的大小。 藉由取代 `RESOURCE_GROUP_NAME` 和相關資訊來編輯下列命令 `CLUSTER_NAME` 。 取代為您的叢集所 `WORKERNODE_COUNT` 需的背景工作節點數目。 如需調整叢集的詳細資訊，請參閱 [調整 HDInsight](./hdinsight-scaling-best-practices.md)叢集。 輸入命令：
