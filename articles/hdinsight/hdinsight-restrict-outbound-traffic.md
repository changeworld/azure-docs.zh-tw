...
本文提供了使用 Azure 防火牆保護 HDInsight 群集中的出站流量的步驟。 以下步驟假定您正在為現有群集配置 Azure 防火牆。 如果要在防火牆後面部署新群集,請先創建 HDInsight 群集和子網。 然後按照本指南中的步驟操作。

## <a name="background"></a>背景

HDInsight 群集通常部署在虛擬網路中。 群集依賴於該虛擬網路以外的服務。
...
HDInsight 出站流量依賴項幾乎完全使用 FQDN 定義。 其背後沒有靜態 IP 位址。 缺少靜態位址意味著網路安全組 (NSG) 無法鎖定群集中的出站流量。 位址更改頻率足夠大,無法根據當前名稱解析和使用設置規則。

使用防火牆保護出站位址,該防火牆可以基於域名控制出站流量。 Azure 防火牆根據目標或[FQDN 標記](../firewall/fqdn-tags.md)的 FQDN 限制出站流量。
...
1. 在 **'新增應用程式規則集合'** 螢幕上,提供以下資訊:
    ...
    **目標 FQDN 部分**
    ...
    | 名稱 | 來源位址 | `Protocol:Port` | 目標 FQDNS | 注意 |
    | --- | --- | --- | --- | --- |
...
1. 在 **'新增網路規則集合'** 螢幕上,提供以下資訊:
    ...
    **IP 位址部分**
    ...
    | Rule_3 | TCP | * | 資料空間儲存帳戶的 IP 位址 | * | 如果使用 Azure 資料湖儲存,則可以在 IP 位址部分添加網路規則,以解決 ADLS Gen1 和 Gen2 的 SNI 問題。 此選項會將流量路由到防火牆。 這可能導致大型數據載入的成本增加,但流量將在防火牆日誌中記錄和審核。 確定資料湖存儲帳戶的IP位址。 您可以使用 PowerShell 命令`[System.Net.DNS]::GetHostAddresses("STORAGEACCOUNTNAME.blob.core.windows.net")`(例如將 FQDN 解析為 IP 位址)。|
    ...
    **服務標籤部分**
    ...
    | --- | --- | --- | --- | --- | --- |
    | Rule_7 | TCP | * | SQL | 1433 | 在 SQL 的服務標記部分中設定網路規則,以允許您記錄和審核 SQL 流量。 除非您在 HDInsight 子網上為 SQL Server 配置了服務終結點,否則該子網將繞過防火牆。 |
...
1. 在 **"關聯子網"** 螢幕上,選擇群集創建的虛擬網路。 以及用於 HDInsight 叢集的**子網路**。
...
完成紀錄紀錄設定後,如果您使用的是紀錄分析,則可以使用查詢(如:

```Kusto
AzureDiagnostics | where msg_s contains "Deny" | where TimeGenerated >= ago(1h)
```

首次使應用程式正常工作時,將 Azure 防火牆與 Azure 監視器日誌集成非常有用。 特別是當您不知道所有應用程式依賴項時。 您可以從[分析 Azure 監視器中的記錄資料](../azure-monitor/log-query/log-query-overview.md)深入了解 Azure 監視器記錄
...
要使用公共終結點`https://CLUSTERNAME.azurehdinsight.net`( ) 或 ssh`CLUSTERNAME-ssh.azurehdinsight.net`終結點 (), 請確保路由表和 NSG 規則中具有正確的路由,以避免[此處](../firewall/integrate-lb.md)介紹的不對稱路由問題。 此選項, 您必須允許入站 NSG 規則的用戶端 IP 位址,並將其新增到使用者定義的路由表中,下一個躍`internet`點設定為 。 如果路由設置不正確,您將看到超時錯誤。
...
前面的說明可説明您配置 Azure 防火牆,以限制來自 HDInsight 群集的出站流量。 Azure 防火牆會自動配置為允許流量用於許多常見重要方案。 使用另一個網路虛擬設備需要配置許多附加功能。 在設定網路虛擬裝置時,請記住以下因素:
...
| [這裡的](hdinsight-management-ip-addresses.md)IP | 這些 IP 是 HDInsight 服務 |
