...
1. 流覽至 Ambari 入口網站上的 [Hive 元件摘要]，然後按一下 [`Configs`] 索引標籤。

2. 移至 [高級設定] 中的 [`Advanced hive-log4j`] 區段。

3. 將 `log4j.appender.RFA` 參數設定為 RollingFileAppender。 

4. 設定 `log4j.appender.RFA.MaxFileSize` 和 `log4j.appender.RFA.MaxBackupIndex`，如下所示。
...
```
5. 將 `hive.root.logger` 設定為 `INFO,RFA`，如下所示。 預設設定為 DEBUG，這會使記錄檔變得非常大。
