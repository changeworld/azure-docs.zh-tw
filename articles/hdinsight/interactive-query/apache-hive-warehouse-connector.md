...
從互動式查詢叢集 headnode0 上`/etc/hosts`的檔案複製節點資訊，並將資訊串連至 Spark 叢集 headnode0 `/etc/hosts`上的檔案。 此步驟可讓您的 Spark 叢集解析互動式查詢叢集中的節點 IP 位址。 使用`cat /etc/hosts`來查看已更新檔案的內容。 最後的輸出應該看起來像下面螢幕擷取畫面中所顯示的內容。
...
1. 使用`https://LLAPCLUSTERNAME.azurehdinsight.net/#/main/services/HIVE/configs`流覽至叢集的 Apache Ambari Hive 頁面，其中`LLAPCLUSTERNAME`是您的互動式查詢叢集的名稱。

1. 流覽至**Advanced** > **General** > **中繼存放區. uri**並記下值。 值可能類似于： `thrift://iqgiro.rekufuk2y2cezcbowjkbwfnyvd.bx.internal.cloudapp.net:9083,thrift://hn1-iqgiro.rekufuk2y2cezcbowjkbwfnyvd.bx.internal.cloudapp.net:9083`。

1. 流覽至**advanced** > **advanced hive-site** > **hive. zookeeper** ，並記下值。 值可能類似于： `zk0-iqgiro.rekufuk2y2cezcbowjkbwfnyvd.bx.internal.cloudapp.net:2181,zk1-iqgiro.rekufuk2y2cezcbowjkbwfnyvd.bx.internal.cloudapp.net:2181,zk4-iqgiro.rekufuk2y2cezcbowjkbwfnyvd.bx.internal.cloudapp.net:2181`。

#### <a name="from-your-apache-spark-cluster"></a>從您的 Apache Spark 叢集

1. 使用`https://SPARKCLUSTERNAME.azurehdinsight.net/#/main/services/HIVE/configs`流覽至叢集的 Apache Ambari Hive 頁面，其中`SPARKCLUSTERNAME`是 Apache Spark 叢集的名稱。

1. 流覽至**advanced** > **advanced hive-interactive-site** > **hive. llap** ，並記下值。 值可能類似于： `@llap0`。

### <a name="configure-spark-cluster-settings"></a>設定 Spark 叢集設定

從您的 Spark Ambari web UI，流覽至**Spark2** > **CONFIGS** > **的 [自訂 Spark2-預設值**]。
...
|`spark.sql.hive.hiveserver2.jdbc.url`|`jdbc:hive2://LLAPCLUSTERNAME.azurehdinsight.net:443/;user=admin;password=PWD;ssl=true;transportMode=http;httpPath=/hive2`. 設定為 JDBC 連接字串，它會連接到互動式查詢叢集上的 Hiveserver2。 以`LLAPCLUSTERNAME`互動式查詢叢集的名稱取代。 將`PWD`取代為實際密碼。|
|`spark.datasource.hive.warehouse.load.staging.dir`|`wasbs://STORAGE_CONTAINER_NAME@STORAGE_ACCOUNT_NAME.blob.core.windows.net/tmp`. 將設定為適當的 HDFS 相容臨時目錄。 如果您有兩個不同的叢集，則臨時目錄應該是 LLAP 叢集之儲存體帳戶的預備目錄中的資料夾，讓 HiveServer2 有其存取權。  請`STORAGE_ACCOUNT_NAME`以叢集所使用的儲存體帳戶名稱取代，並`STORAGE_CONTAINER_NAME`以儲存體容器的名稱取代。|
|`spark.datasource.hive.warehouse.metastoreUri`|您先前從**hive. 中繼存放區**取得的值。|
|`spark.security.credentials.hiveserver2.enabled`|`false`適用于 YARN 用戶端部署模式。|
...
1. 輸入下列命令以啟動 spark shell：
    ...
    您會看到歡迎使用的訊息， `scala>`以及您可以在其中輸入命令的提示。
...
1. 篩選資料表`hivesampletable` ，其中的資料`state`行`Colorado`等於。 此 Hive 資料表的查詢會以 Spark 資料框架的形式傳回。 然後，資料框架會使用`sampletable_colorado` `write`函數儲存在 Hive 資料表中。
...
1. 然後使用下列命令，將串流資料寫入至新建立的資料表：
    ...
    >[!Important]
    > 因為`metastoreUri` Apache Spark `database`中的已知問題，所以目前必須手動設定和選項。 如需有關此問題的詳細資訊，請參閱[SPARK-25460](https://issues.apache.org/jira/browse/SPARK-25460)。
