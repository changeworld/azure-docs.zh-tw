...
description: 瞭解如何連線到 Apache Beeline 用戶端，以使用 HDInsight 上的 Hadoop 執行 Hive 查詢。 Beeline 是透過 JDBC 與 HiveServer2 搭配作業的公用程式。
...
ms.locfileid: "86076771"
---
# <a name="connect-to-apache-beeline-on-hdinsight-or-install-it-locally"></a>連線到 Apache Beeline on HDInsight 或將它安裝在本機

[Apache Beeline](https://cwiki.apache.org/confluence/display/Hive/HiveServer2+Clients#HiveServer2Clients-Beeline–NewCommandLineShell)是包含在 HDInsight 叢集前端節點上的 Hive 用戶端。 本文說明如何跨不同類型的連線，連線到安裝在 HDInsight 叢集上的 Beeline 用戶端。 它也會討論如何[在本機安裝 Beeline 用戶端](#install-beeline-client)。 
...
將取代為叢集前端 `<headnode-FQDN>` 節點的完整功能變數名稱。 若要找出前端節點的完整網域名稱，請利用[使用 Apache Ambari REST API 管理 HDInsight](../hdinsight-hadoop-manage-ambari-rest-api.md#get-the-fqdn-of-cluster-nodes) 文件中的資訊。

### <a name="to-hdinsight-enterprise-security-package-esp-cluster-using-kerberos"></a>使用 Kerberos 的 HDInsight 企業安全性套件（ESP）叢集

從用戶端連接到已加入至叢集相同領域之電腦上 Azure Active Directory （AAD）-DS 的企業安全性套件（ESP）叢集時，您也必須指定具有叢集存取權的網域 `<AAD-Domain>` 使用者帳戶的功能變數名稱和名稱 `<username>` ：
...
使用公用或私人端點連接到叢集時，您必須提供叢集登入帳戶名稱（預設值 `admin` ）和密碼。 例如，使用 Beeline 從用戶端系統連線到 `clustername.azurehdinsight.net` 位址。 此連接是透過埠進行 `443` ，並使用 TLS/SSL 進行加密。
...
使用的連接字串稍有不同。 而不是包含 `httpPath=/hive2` 它使用的 `httpPath/sparkhive2` 。 將 `clustername` 替換為 HDInsight 叢集的名稱。 將 `admin` 取代為叢集的叢集登入帳戶。 針對 ESP 叢集，請使用完整的 UPN （例如， user@domain.com ）。 將 `password` 取代為叢集登入帳戶的密碼。
...
1. 進一步修改 .bashrc 檔。 您必須識別封存已解壓縮的路徑。 如果使用[適用于 Linux 的 Windows 子系統](https://docs.microsoft.com/windows/wsl/install-win10)，且您已完全遵循步驟，則您的路徑會是 `/mnt/c/Users/user/` ，其中 `user` 是您的使用者名稱。
...
* 如需使用 Beeline 用戶端與 Apache Hive 的範例，請參閱搭配[使用 Apache Beeline 與 Apache Hive](apache-hadoop-use-hive-beeline.md)
