...
[阿帕奇蜂巢](https://hive.apache.org/)是阿帕奇哈多普的資料倉儲系統。 Hive 可執行資料摘要、查詢以及資料分析。 Hive 查詢是以 HiveQL 撰寫而成，這是類似 SQL 的查詢語言。
...
| [蜂巢視圖](../hadoop/apache-hadoop-use-hive-ambari-view.md) |✔ |✔ |任何 (以瀏覽器為基礎) |
...
HiveQL 語言參考可在[語言手冊](https://cwiki.apache.org/confluence/display/Hive/LanguageManual)中提供。
...
* __外部__︰資料會儲存在資料倉儲之外。 資料可以儲存在叢集可存取的任何儲存體上。
    ...
    * 資料也使用於 Hive 之外。 例如，資料檔案由另一個進程（不鎖定檔）更新。
...
|創建外部表|在 Hive 中創建新**的外部**表。 外部資料表只會將資料表定義儲存在 Hive 中。 資料會留在原來的位置，並保持原始格式。|
|行格式|告訴 Hive 如何設定資料格式。 在此情況下，每個記錄中的欄位會以空格隔開。|
|存儲為文字檔位置|告訴 Hive 資料存儲的位置（`example/data`目錄）並將其存儲為文本。 資料可以在目錄的一個檔案中，也可以分散在多個檔案中。|
|SELECT|選取資料行 **t4** 包含 **[ERROR]** 值的所有資料列計數。 這個陳述式會傳回值 **3**，因為有三個資料列包含此值。|
|INPUT__FILE__NAME喜歡".log"|Hive 嘗試將架構應用於目錄中的所有檔。 在這種情況下，目錄包含與架構不匹配的檔。 若要防止結果中出現亂碼資料，此陳述式會告訴 Hive 我們只應該從檔名以 log 結尾的檔案傳回資料。|
...
|引數 |描述 |
|---|---|
|創建表（如果不存在）|如果表不存在，請創建它。 由於不使用**外部**關鍵字，因此此語句將創建一個內部表。 資料表會儲存在 Hive 資料倉儲中，並完全受到 Hive 所管理。|
|存儲為 ORC|以最佳化資料列單欄式 (Optimized Row Columnar, ORC) 格式儲存資料。 ORC 是高度最佳化且有效率的 Hive 資料儲存格式。|
|插入覆蓋...選擇|從 **log4jLogs** 資料表選取包含 **[ERROR]** 的資料列，然後將資料插入 **errorLogs** 資料表。|
...
[Apache Tez](https://tez.apache.org) 是可讓資料高用量應用程式 (例如 Hive)，以大規模而更有效率方式執行作業的架構。 預設情況下啟用 Tez。  [Tez 上的 Apache Hive 設計文件](https://cwiki.apache.org/confluence/display/Hive/Hive+on+Tez)包含實作選擇和調整設定的詳細資料。
