...
瞭解如何使用 Apache Hive WebHCat REST API 在 HDInsight 群集上的 Apache Hadoop 上運行 MapReduce 作業。 Curl 用來示範如何使用原始 HTTP 要求與 HDInsight 互動，以執行 MapReduce 工作。
...
下列任一方法︰
  ...
  * [捲曲](https://curl.haxx.se/)與[jq](https://stedolan.github.io/jq/)
...
1. 要提交 MapReduce 作業，請使用以下命令。 根據需要修改到**jq**的路徑。
   ...
   * **-d** `-G` ： 不使用，因此請求預設為 POST 方法。 `-d` 可指定與要求一起傳送的資料值。
     * **user.name**：執行命令的使用者
...
1. 若要檢查工作的狀態，請使用下列命令。 將 的值`JOBID`替換為上一步驟中返回**的實際**值。 根據需要修改**jq**的位置。
...
1. 為便於使用，設置下面的變數。 替換為`CLUSTERNAME`實際的群集名稱。 執行命令並在提示時輸入群集登錄密碼。
...
1. 使用以下命令驗證是否可以連接到 HDInsight 群集：
...
您可以使用 [Azure CLI](/cli/azure/install-azure-cli) 列出並下載這些檔案。 有關使用 Azure CLI 處理 Azure Blob 存儲的詳細資訊，請參閱[快速入門：使用 Azure CLI 創建、下載和列出 Blob。](../../storage/blobs/storage-quickstart-blobs-cli.md)
