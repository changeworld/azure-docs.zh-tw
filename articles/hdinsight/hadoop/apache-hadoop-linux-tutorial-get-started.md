...
在本快速入門中，您會使用 Azure Resource Manager 範本，在 Azure HDInsight 中建立 [Apache Hadoop](./apache-hadoop-introduction.md) 叢集。 Hadoop 是原始的開放原始碼架構，用於分散式處理和分析叢集上的巨量資料集。 Hadoop 生態系統包含相關的軟體和公用程式，其中包括 Apache Hive、Apache HBase、Spark、Kafka 等等。
...
本快速入門中使用的範本是來自 [Azure 快速入門範本](https://github.com/Azure/azure-quickstart-templates/tree/master/101-hdinsight-linux-ssh-password)。
