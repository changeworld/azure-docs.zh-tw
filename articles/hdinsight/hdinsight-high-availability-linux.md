...
要取得群組的不同節點類型的主機名稱,請使用[Ambari REST API](hdinsight-hadoop-manage-ambari-rest-api.md#get-the-fqdn-of-cluster-nodes)。
...
[ZooKeeper](https://zookeeper.apache.org/) 節點用於前端節點上主要服務的前置選擇。 它們還用於確保服務、數據(輔助節點)節點和閘道知道主服務在哪個頭節點處於活動狀態。 根據預設，HDInsight 會提供三個 ZooKeeper 節點。
...
邊緣節點不積极參與群集中的數據分析。 開發人員或數據科學家在使用 Hadoop 時使用它。 邊緣節點和叢集中的其他節點一樣，存在於相同的 Azure 虛擬網路中，也可直接存取其他所有節點。 使用邊緣節點時，不需從關鍵 Hadoop 服務或分析作業取走資源。
...
|連接埠 |描述 |
|---|---|
|443|用於存取安巴里和其他託管在頭節點上的 Web UI 或 REST API。|
...
此指令傳回類似於以下內容的值,該值包含要`oozie`與 命令一起使用的內部網址:
...
|SSH隧道|如果需要存取託管在未暴露到 Internet 的節點之一上的 Web 服務,則必須使用 SSH 隧道。 如需詳細資訊，請參閱[搭配 HDInsight 使用 SSH 通道](hdinsight-linux-ambari-ssh-tunnel.md)文件。|
|Azure 虛擬網路|如果您的 HDInsight 叢集是「Azure 虛擬網路」的一部分，則任何在相同「虛擬網路」上的資源都可直接存取該叢集內的所有節點。 有關詳細資訊,請參閱[為 HDInsight 文件規劃虛擬網路](hdinsight-plan-virtual-network-deployment.md)。|
...
| 警示名稱                               | 描述                                                                                                                                                                                  |
|------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 指標監控狀態狀態                    | 此警報指示由監視器狀態腳本確定的指標監視器進程的狀態。                                                                                   |
| 安巴里代理心跳                   | 如果伺服器與代理失去聯繫,將觸發此警報。                                                                                                                        |
| 動物園管理員伺服器流程                 | 如果無法確定 ZooKeeper 伺服器進程在網路上啟動並偵聽,則觸發此主機級警報。                                                               |
| IOCache 中繼資料伺服器狀態           | 如果無法確定 IOCache 中繼伺服器是啟動並回應客戶端請求,則會觸發此主機級警報                                                            |
| 記錄節點 Web UI                       | 如果無法訪問 JournalNode Web UI,將觸發此主機級警報。                                                                                                                 |
| Spark2 節儉伺服器                     | 如果無法確定 Spark2 節儉伺服器已啟動,則觸發此主機級警報。                                                                                                |
| 歷史記錄伺服器程序                   | 如果無法將歷史記錄伺服器進程建立為在網路上啟動和偵聽,則觸發此主機級警報。                                                                |
| 歷史記錄伺服器 Web UI                    | 如果無法存取歷史記錄伺服器 Web UI,將觸發此主機級警報。                                                                                                              |
| `ResourceManager`Web UI                   | 如果無法存取 Web`ResourceManager`UI, 將觸發此主機級警報。                                                                                                             |
| 節點管理員執行狀況摘要               | 如果存在不正常的節點管理器,將觸發此服務級別警報                                                                                                                    |
| 套用時間線 Web UI                      | 如果無法存取應用時間軸伺服器 Web UI,將觸發此主機級警報。                                                                                                         |
| 資料節點執行狀況摘要                  | 如果存在不正常的資料節點,將觸發此服務級別警報                                                                                                                       |
| 名稱節點 Web UI                          | 如果 NameNode Web UI 無法存取,將觸發此主機級警報。                                                                                                                    |
| 動物園管理員故障轉移控制器流程    | 如果無法確認 ZooKeeper 故障轉移控制器進程在網路上啟動並偵聽,則觸發此主機級警報。                                                   |
| Oozie 伺服器 Web UI                      | 如果無法訪問 Oozie 伺服器 Web UI,將觸發此主機級警報。                                                                                                                |
| Oozie 伺服器狀態                      | 如果無法確定 Oozie 伺服器是啟動並回應用戶端請求,則觸發此主機級警報。                                                                      |
| 蜂巢中儲存流程                   | 如果無法確定 Hive Metastore 進程在網路上啟動並偵聽,則觸發此主機級警報。                                                                 |
...
| 指標收集器 - HBase 主流程 | 如果無法確認指標收集器的 HBase 主行程在網路上啟動並偵聽配置的關鍵閾值(以秒為單位),則觸發此警報。 |
| 可用的指標百分比監視器       | 如果指標監視器進程未啟動百分比,並且在網路上偵聽配置的警告和關鍵閾值,則觸發此警報。                             |
| 可用節點管理員的百分比           | 如果群集中的下節點管理器數大於配置的關鍵閾值,則觸發此警報。 它聚合節點管理器流程檢查的結果。        |
| 節點管理員執行狀況                       | 此主機級警報檢查 NodeManager 元件提供的節點運行狀況屬性。                                                                                              |
| 節點管理員 Web UI                       | 如果 NodeManager Web UI 無法訪問,將觸發此主機級警報。                                                                                                                 |
| 名稱節點高可用性執行狀況        | 如果活動 NameNode 或備用 NameNode 未執行,則觸發此服務層級警報。                                                                                     |
| 資料節點流程                         | 如果無法將單個 DataNode 進程建立為在網路上啟動和偵聽,則觸發此主機級警報。                                                         |
| 資料節點 Web UI                          | 如果 DataNode Web UI 無法存取,將觸發此主機級警報。                                                                                                                    |
| 可用的紀錄節點百分比           | 如果群集中的日記節點數量大於配置的關鍵閾值,則觸發此警報。 它聚合日記本節點流程檢查的結果。        |
| 可用資料節點的百分比              | 如果群集中的數據節點的向下數據節點數大於配置的關鍵閾值,則觸發此警報。 它聚合 DataNode 過程檢查的結果。              |
...
![阿帕契巴里頭節點主機清單](./media/hdinsight-high-availability-linux/hdinsight-hosts-list.png)

選取其中一個前端節點的連結會顯示該節點上執行的服務與元件。

![阿帕契巴里元件狀態](./media/hdinsight-high-availability-linux/hdinsight-node-services.png)
...
連線後,將顯示提示`sftp>`。 您可以從該提示變更目錄、上傳和下載檔案。 例如：下列命令會將目錄變更至 **/var/log/hadoop/hdfs** 目錄，然後在目錄中下載所有檔案。
...
* **Azure CLI**[`az hdinsight create`](https://docs.microsoft.com/cli/azure/hdinsight?view=azure-cli-latest#az-hdinsight-create):使用 指令時`--headnode-size`,`--workernode-size`可以`--zookeepernode-size`使用 、 和 參數設置頭、輔助角色和 ZooKeeper 節點的大小。

* **Azure PowerShell**:使用[New-AzHDInsightCluster](https://docs.microsoft.com/powershell/module/az.hdinsight/new-azhdinsightcluster) cmdlet`-HeadNodeSize``-WorkerNodeSize`時`-ZookeeperNodeSize`,可以使用 、 和 參數設置頭、輔助角色和 ZooKeeper 節點的大小。
...
* [安裝並設定 Azure CLI](https://docs.microsoft.com//cli/azure/install-azure-cli?view=azure-cli-latest)
* [安裝並設定 Azure PowerShell 模組 Az](/powershell/azure/overview)
