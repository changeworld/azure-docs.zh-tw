...
如需詳細資訊，請參閱[傳輸層安全性](./hdinsight-plan-virtual-network-deployment.md#transport-layer-security)
...
只要叢集存在，您就必須支付節點使用量的費用。 建立叢集後就開始計費，並在叢集刪除後停止計費。 無法取消配置或保留叢集。
