...
| 原因 | 解決方法 |
| --- | --- |
| 您已超過每分鐘 WebHCat 所服務的並行要求數上限（預設值為20） |減少您的工作負載，以確保您不會提交超過並行要求數目上限，或藉由修改`templeton.exec.max-procs`來提高並行要求限制。 如需詳細資訊，請參閱[修改組態](#modifying-configuration) 。 |
...
| 嘗試在 `Fields` 設定為 `*` 時透過 [GET /jobs](https://cwiki.apache.org/confluence/display/Hive/WebHCat+Reference+Jobs) 呼叫擷取所有作業 |不要取出*所有*作業詳細資料。 請`jobid`改用來抓取只大於特定作業識別碼的作業詳細資料。 或者，不要使用`Fields` |
