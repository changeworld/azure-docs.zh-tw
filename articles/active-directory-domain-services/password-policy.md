...
* 已在您的 Azure AD 租用戶中啟用並設定 Azure Active Directory Domain Services 受控網域。
  ...
  * 受控網域必須已使用 Resource Manager 部署模型建立。 如有需要，請[從傳統虛擬網路模型遷移至 Resource Manager][migrate-from-classic]。
...
更細緻的密碼原則（Fgpp）可讓您將密碼和帳戶鎖定原則的特定限制套用到網域中的不同使用者。 例如，若要保護特殊許可權帳戶，您可以套用比一般非特殊許可權帳戶更嚴格的帳戶鎖定設定。 您可以在受控網域中建立多個 Fgpp，並指定優先順序來將它們套用至使用者。
...
針對在受控網域中手動建立的使用者帳戶，也會從預設原則套用下列額外的密碼設定。 這些設定不適用於從 Azure AD 同步處理的使用者帳戶，因為使用者無法直接在 Azure AD DS 中更新其密碼。
...
自訂密碼原則會套用至受控網域中的群組。 此設定會有效覆寫預設原則。

若要建立自訂密碼原則，您可以使用來自已加入網域之 VM 的 Active Directory 系統管理工具。 此 Active Directory 管理中心可讓您在受控網域（包括 Ou）中，查看、編輯和建立資源。
...
1. 開啟 [**系統**] 容器，然後按 [**密碼設定容器**]。
    ...
    會顯示受控網域的內建密碼原則。 您無法修改此內建原則。 相反地，請建立自訂密碼原則來覆寫預設原則。
...
1. 視需要編輯其他密碼原則設定。 請記住下列重點：
    ...
    * 只有手動在受控網域中建立之使用者的密碼複雜性、存留期或到期時間等設定。
    * 帳戶鎖定設定會套用至所有使用者，但只會在受控網域中生效，而不會在 Azure AD 本身中生效。

    ![建立自訂的更細緻密碼原則](./media/password-policy/custom-fgpp.png)
...
1. 輸入您想要套用原則的組名，然後選取 [**檢查名稱**] 以驗證該群組是否存在。
    ...
    ![搜尋並選取要套用 FGPP 的群組](./media/password-policy/fgpp-apply-group.png)
