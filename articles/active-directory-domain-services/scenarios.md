...
Contoso 的 IT 系統管理員想要加入 Azure 中部署的 Vm，讓系統管理更容易，因為使用者可以使用其公司認證進行登入。 加入網域時，也可以使用群組原則物件（Gpo）設定 Vm，以符合所需的安全性基準。 Contoso 不想要在 Azure 中部署、監視和管理自己的網域控制站。
