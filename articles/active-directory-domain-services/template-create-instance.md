...
現在，使用 [New-AzureADGroup][New-AzureADGroup] Cmdlet，建立名為 *AAD DC 管理員* 的 Azure AD 群組。 系統會授與新增至此群組的使用者權限，以在 Azure AD DS 受控網域上執行管理工作。
...
| notificationSettings    | 如果 Azure AD DS 受控網域中產生任何警示，則會送出電子郵件通知。 <br />您可以「啟用」  Azure 租用戶的「全域管理員」  和「AAD DC 管理員」  群組的成員，以便他們可以收到通知。<br /> 如有需要，請變新增其他收件者，以便他們在有警示且需要注意時可收到通知。|
...
下列壓縮的參數定義會顯示這些值的宣告方式。 系統會建立名為 aaddscontoso.com  的使用者樹系，並將所有使用者從 Azure AD 同步處理到 Azure AD DS 受控網域：
...
接著會使用下列已壓縮的 Resource Manager 範本資源類型，來定義和建立 Azure AD DS 受控網域。 Azure 虛擬網路和子網路必須已經存在，或建立為 Resource Manager 範本的一部分。 Azure AD DS 受控網域已連線到此子網路。
...
下列完整的 Resource Manager 範例範本會建立 Azure AD DS 受控網域，以及支援的虛擬網路、子網路和網路安全性群組規則。 必須要有網路安全性群組規則，才能保護受控網域，並確保流量可以正確地流動。 已建立 DNS 名稱為 aaddscontoso.com  的使用者樹系，並從 Azure AD 同步處理所有使用者：
...
若要查看作用中的 Azure AD DS 受控網域，您可以 [將 Windows VM 加入網域][windows-join]、[設定安全 LDAP][tutorial-ldaps]，以及 [設定密碼雜湊同步處理][tutorial-phs]。
