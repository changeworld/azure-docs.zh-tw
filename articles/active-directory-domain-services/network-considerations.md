...
[網路安全性群組（NSG）](../virtual-network/virtual-networks-nsg.md)包含一份規則清單，可允許或拒絕對 Azure 虛擬網路中流量的網路流量。 當您部署 Azure AD DS 時，會建立網路安全性群組，其中包含可讓服務提供驗證和管理功能的一組規則。 此預設網路安全性群組會與您 Azure AD DS 受控網域部署所在的虛擬網路子網相關聯。
