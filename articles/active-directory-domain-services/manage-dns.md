...
在混合式環境中，在其他 DNS 命名空間中設定的 DNS 區域和記錄（例如內部部署 AD DS 環境）不會同步處理至受控網域。 若要解析其他 DNS 命名空間中的已命名資源，請建立並使用指向您環境中現有 DNS 伺服器的條件轉寄站。
...
若要在受控網域中建立及修改 DNS 記錄，您必須安裝 DNS 伺服器工具。 這些工具可以在 Windows Server 中安裝為功能。 如需有關如何在 Windows 用戶端上安裝系統管理工具的詳細資訊，請參閱 install[遠端伺服器管理工具（RSAT）][install-rsat]。
...
1. DNS 主控台會連線到指定的受控網域。 展開 [**正向對應區域**] 或 [**反向對應區域**]，以建立必要的 DNS 專案，或視需要編輯現有的記錄。
...
Azure AD DS DNS 區域應該只包含受控網域本身的區域和記錄。 請勿在受控網域中建立其他區域來解析其他 DNS 命名空間中的已命名資源。 相反地，請使用受控網域中的條件式轉寄站，告訴 DNS 伺服器要前往何處，以便解析這些資源的位址。

條件轉寄站是 DNS 伺服器中的設定選項，可讓您定義 DNS 網域（例如*contoso.com*），以將查詢轉送到。 並不是在本機 DNS 伺服器嘗試解析該網域中的記錄查詢，而是將 DNS 查詢轉送到該網域的已設定 DNS。 此設定可確保傳回正確的 DNS 記錄，因為您不會在受控網域中建立具有重複記錄的本機 DNS 區域來反映這些資源。

若要在您的受控網域中建立條件轉寄站，請完成下列步驟：
...
從連線到受控網域的 Vm，在其他命名空間中的資源名稱解析，現在應能正確解析。 條件轉寄站中設定之 DNS 網域的查詢會傳遞至相關的 DNS 伺服器。
