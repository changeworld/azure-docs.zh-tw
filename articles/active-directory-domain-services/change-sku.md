---
title: 更改 Azure AD 域服務的 SKU |微軟文檔
description: 瞭解在業務需求發生變化時，如何訪問 Azure AD 域服務託管域的 SKU 層
...
---
# <a name="change-the-sku-for-an-existing-azure-ad-domain-services-managed-domain"></a>更改現有 Azure AD 域服務託管域的 SKU
...
* 如果在*高級*SKU 上創建了兩個林信任，則無法向下更改為*標準*SKU。 *標準*SKU 不支援林信任。
...
1. 在 Azure 門戶的頂部，搜索並選擇**Azure AD 域服務**。 從清單中選擇託管域，如*aaddscontoso.com*。
1. 在 Azure AD DS 頁面左側的功能表中，選擇 **"設置"> SKU**。

    ![在 Azure 門戶中選擇 Azure AD DS 託管域的 SKU 功能表選項](media/change-sku/overview-change-sku.png)

1. 在下拉式功能表中，為 Azure AD DS 託管域選擇所需的 SKU。 如果您有資源林，則無法選擇*標準*SKU，因為林信任僅在*企業*SKU 或更高版本上可用。

    從下拉式功能表中選擇您想要的 SKU，然後選擇 **"保存**"。

    ![從 Azure 門戶的下拉式功能表中選擇所需的 SKU](media/change-sku/change-sku-selection.png)

更改 SKU 類型可能需要一兩分鐘。

## <a name="next-steps"></a>後續步驟

如果您有資源林，並希望在 SKU 更改後創建其他信任，請參閱[在 Azure AD DS（預覽）中創建對本地域的出站林信任][create-trust]。
