...
ms.locfileid: "84734838"
---
# <a name="deploy-azure-ad-application-proxy-for-secure-access-to-internal-applications-in-an-azure-active-directory-domain-services-managed-domain"></a>部署 Azure AD 應用程式 Proxy，以安全存取 Azure Active Directory Domain Services 受控網域中的內部應用程式
...
本文說明如何建立和設定 Azure AD 應用程式 Proxy 連接器，以提供受控網域中應用程式的安全存取。
...
若要將流量路由傳送至在您的環境中執行的應用程式，您必須安裝 Azure AD 應用程式 Proxy 連接器元件。 此 Azure AD 應用程式 Proxy 連接器必須安裝在已加入受控網域的 Windows Server 虛擬機器（VM）上。 針對某些應用程式，您可以部署多部已安裝連接器的伺服器。 這個部署選項為您提供更高的可用性，並協助處理更大量的驗證負載。
...
1. [建立自訂 OU](create-ou.md)。 您可以將管理這個自訂 OU 的權限委派給受控網域內的使用者。 Azure AD 應用程式 Proxy 以及執行應用程式的 Vm，都必須是自訂 OU 的一部分，而不是預設的*AAD DC 電腦*OU。
1. 將虛擬機器（執行 Azure AD 應用程式 Proxy 連接器）和執行應用程式的[電腦加入][create-join-windows-vm]至受控網域。 在上一個步驟的自訂 OU 中建立這些電腦帳戶。
...
1. 在安裝期間，系統會提示您在 Azure AD 目錄中向應用程式 Proxy 註冊連接器。
   * 在您的 Azure AD 目錄中提供全域管理員的認證。 Azure AD 的全域系統管理員認證可能與您在入口網站中的 Azure 認證不同
        ...
        >
        > 例如，如果 Azure AD 網域是*aaddscontoso.com*，則全域管理員應該是 `admin@aaddscontoso.com` 或該網域上的另一個有效別名。
...
如果您想要對使用整合式 Windows 驗證（IWA）的應用程式使用單一登入，請授與 Azure AD 應用程式 Proxy 連接器許可權以模擬使用者，並代表其傳送和接收權杖。 若要授與這些許可權，您可以設定連接器的 Kerberos 限制委派（KCD），以存取受控網域上的資源。 因為您在受控網域中沒有網域系統管理員許可權，所以無法在受控網域上設定傳統帳戶層級 KCD。 請改為使用以資源為基礎的 KCD。
