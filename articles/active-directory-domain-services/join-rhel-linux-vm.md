...
## <a name="create-and-connect-to-a-rhel-linux-vm"></a>建立並連接至 RHEL Linux VM

如果您在 Azure 中有現有的 RHEL Linux VM，請使用 SSH 連接到該 VM，然後繼續進行下一個步驟，以 [開始設定 VM](#configure-the-hosts-file)。

如果您需要建立 RHEL Linux VM，或想要建立測試 VM 以便與本文搭配使用，您可以使用下列其中一種方法：
...
VM 需要一些額外的套件才能將 VM 加入受控網域。 若要安裝和設定這些套件，請使用更新和安裝網域加入工具 `yum` 。 RHEL 7.x 與 RHEL 6.x 之間有一些差異，因此請在本文的其餘章節中使用適用于發行版本版本的命令。
...
既然 VM 上已安裝必要的套件，請將 VM 加入受控網域。 同樣地，請針對您的 RHEL 發行版本版本使用適當的步驟。
...
1. 使用 `adcli info` 命令來探索受控網域。 下列範例會探索領域 *ADDDSCONTOSO.COM*。 以全部大寫指定您自己的受控功能變數名稱：
...
1. 首先，使用命令加入網域 `adcli join` ，此命令也會建立 keytab 來驗證電腦。 使用屬於受控網域的使用者帳戶。

    ```console
    sudo adcli join aaddscontoso.com -U contosoadmin
    ```

1. 現在設定 `/ect/krb5.conf` 和建立檔案， `/etc/sssd/sssd.conf` 以使用 `aaddscontoso.com` Active Directory 網域。
   ...
   建立檔案 `/etc/sssd/sssd.conf` ：
    
    ```console
    sudo vi /etc/sssd/sssd.conf
    ```

    更新檔案 `sssd.conf` 以符合下列範例：
...
1. 請確定 `/etc/sssd/sssd.conf` 許可權是600，而且是由根使用者所擁有：
...
1. 用 `authconfig` 來指示 VM 關於 AD Linux 整合：
...
現在請檢查您是否可以使用來查詢使用者 AD 資訊 `getent`
...
1. 若要套用變更並讓使用者使用密碼登入，請重新開機您 RHEL 發行版本版本的 SSH 服務：
...
若要在 RHEL VM 上授與 *AAD DC 系統管理員* 群組系統管理許可權的成員，您可以將專案新增至 */etc/sudoers*。 新增之後， *AAD DC 系統管理員* 群組的成員可以在 `sudo` RHEL VM 上使用命令。
...
1. 從您的主控台建立新的 SSH 連線。 使用屬於受控網域的網域帳戶 `ssh -l` （例如）， `contosoadmin@aaddscontoso.com` 然後輸入 VM 的位址，例如 *rhel.aaddscontoso.com*。 如果您使用 Azure Cloud Shell，請使用 VM 的公用 IP 位址，而不是內部 DNS 名稱。
