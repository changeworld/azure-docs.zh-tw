---
title: 將 RHEL VM 加入 Azure AD 域服務 |微軟文檔
description: 瞭解如何配置紅帽企業 Linux 虛擬機器並將其加入 Azure AD 域服務託管域。
...
## <a name="create-and-connect-to-a-rhel-linux-vm"></a>創建並連接到 RHEL Linux VM

如果在 Azure 中具有現有的 RHEL Linux VM，請使用 SSH 連接到它，然後繼續執行下一步[以開始配置 VM](#configure-the-hosts-file)。

如果需要創建 RHEL Linux VM，或者想要創建用於本文的測試 VM，可以使用以下方法之一：
...
VM 需要一些其他包才能將 VM 加入 Azure AD DS 託管域。 要安裝和配置這些包，請使用`yum`更新和安裝域聯接工具。 RHEL 7.x 和 RHEL 6.x 之間存在一些差異，因此請在本文其餘部分中為您的發行版本使用相應的命令。
...
現在，所需的套裝軟體安裝在 VM 上，請將 VM 加入 Azure AD DS 託管域。 同樣，為您的 RHEL 版版使用相應的步驟。
...
1. 使用`adcli info`命令發現 Azure AD DS 託管域。 下面的示例發現*ADDDSCONTOSO.COM*的境界。 在"所有上寫"中指定您自己的 Azure AD DS 託管功能變數名稱：
...
1. 現在配置`/ect/krb5.conf`並創建`/etc/sssd/sssd.conf`檔以使用`aaddscontoso.com`活動目錄域。
   請確保`AADDSCONTOSO.COM`由您自己的功能變數名稱替換：

    使用編輯器`/ect/krb5.conf`打開檔：

    ```console
    sudo vi /etc/krb5.conf
    ```

    更新`krb5.conf`檔以匹配以下示例：
   ...
   創建`/etc/sssd/sssd.conf`檔：
    
    ```console
    sudo vi /etc/sssd/sssd.conf
    ```

    更新`sssd.conf`檔以匹配以下示例：
...
1. 確保`/etc/sssd/sssd.conf`許可權為 600，並且由根使用者擁有：
...
1. 用於`authconfig`指導 VM 有關 AD Linux 集成：
...
1. 要應用更改並允許使用者使用密碼登錄，請重新開機 RHEL 版版的 SSH 服務：
...
要授予 RHEL VM 上的*AAD DC 管理員*組管理許可權，您需要向 */etc/sudoers*添加一個條目。 添加後 *，AAD DC 管理員*組的成員可以在 RHEL `sudo` VM 上使用該命令。
...
1. 從主控台創建新的 SSH 連接。 使用`ssh -l`命令使用屬於託管域的域帳戶，例如`contosoadmin@aaddscontoso.com`，然後輸入 VM 的位址，如*rhel.aaddscontoso.com*。 如果使用 Azure 雲外殼，請使用 VM 的公共 IP 位址，而不是內部 DNS 名稱。
