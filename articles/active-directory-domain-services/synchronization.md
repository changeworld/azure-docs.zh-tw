...
> [!TIP]
> **使用 UPN 格式登入受控網域***SAMAccountName*屬性（例如`AADDSCONTOSO\driley`）可能會針對 Azure AD DS 受控網域中的某些使用者帳戶自動產生。 使用者自動產生的*SAMAccountName*可能與其 UPN 前置詞不同，因此不一定是可靠的登入方式。
>
