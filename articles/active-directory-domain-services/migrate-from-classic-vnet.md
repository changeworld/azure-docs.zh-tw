...
1. 從 PowerShell 資源庫`Migrate-Aaads`安裝腳本。 [PowerShell Gallery][powershell-script] 此 PowerShell 遷移腳本是由 Azure AD 工程小組以數位方式簽署。
...
1. 現在，請`Migrate-Aadds`使用 *-Prepare*參數執行 Cmdlet。 為您自己的 Azure AD DS 受控網域提供 *-ManagedDomainFqdn* ，例如*aaddscontoso.com*：
...
使用`Migrate-Aadds` *-Commit*參數執行 Cmdlet。 針對您自己在上一節中準備的 Azure AD DS 受控網域提供 *-ManagedDomainFqdn* ，例如*aaddscontoso.com*：
...
使用`Migrate-Aadds` *-Abort*參數執行 Cmdlet。 提供上一節中準備的 Azure AD DS 受控網域（例如*aaddscontoso.com*）和傳統虛擬網路名稱（例如*myClassicVnet*）的 *-ManagedDomainFqdn* ：
