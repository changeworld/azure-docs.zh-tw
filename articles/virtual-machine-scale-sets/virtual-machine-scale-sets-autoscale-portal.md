...
5. 讓我們建立規則，以便平均 CPU 負載在 10 分鐘期間內大於 70% 時，增加擴展集內 VM 執行個體的數目。 觸發此規則時，VM 執行個體數目會增加 20%。 若擴展集內 VM 執行個體的數目很少，您可以將 [作業]**** 設定為 [增加計數]**，然後針對 [執行個體計數]** 指定 *1* 或 *2*。 若擴展集內 VM 執行個體的數目很多，則增加 VM 執行個體數目 10% 或 20% 可能更加適當。
    ...
    | 參數              | 說明                                                                                                         | 值          |
    |------------------------|---------------------------------------------------------------------------------------------------------------------|----------------|
    | *時間聚合*     | 定義應該如何彙總收集的計量以進行分析。                                                | Average        |
    | *指標名稱*          | 要監視並套用擴展集動作的效能計量。                                                   | Percentage CPU |
    | *時間粒紋統計資料* | 定義在每個時間粒紋中所收集的計量，應如何彙總以便進行分析。                             | Average        |
    | *運算元*             | 用以比較計量資料與閾值之間差異的運算子。                                                     | 大於   |
    | *閾 值*            | 讓自動調整規則觸發動作的百分比。                                                 | 70             |
    | *時間*             | 在比較計量與閾值之前監視的時間長短。                                   | 10 分鐘     |
    | *操作*            | 定義擴展集在套用規則時該相應增加或相應減少，以及該增加或減少多少                        | 增加多少百分比 |
    | *實例計數*       | 觸發此規則時，應該變更多少 VM 執行個體百分比。                                            | 20             |
    | *冷卻（分鐘）*  | 再次套用規則前需要等待多久時間，以便讓自動調整動作生效。 | 5 分鐘      |
...
2. 建立規則，以便平均 CPU 負載在 10 分鐘期間內低於 30% 時，減少擴展集內 VM 執行個體的數目。 觸發此規則時，VM 執行個體的數目會減少 20%。
    ...
    | 參數              | 說明                                                                                                          | 值          |
    |------------------------|----------------------------------------------------------------------------------------------------------------------|----------------|
    | *運算元*             | 用以比較計量資料與閾值之間差異的運算子。                                                      | 小於   |
    | *閾 值*            | 讓自動調整規則觸發動作的百分比。                                                 | 30             |
    | *操作*            | 定義擴展集在套用規則時該相應增加或相應減少，以及該增加或減少多少                         | 減少多少百分比 |
    | *實例計數*       | 觸發此規則時，應該變更多少 VM 執行個體百分比。                                             | 20             |
...
在本文中，您已學到如何使用自動調整規則進行水平縮放，以及增加或減少擴展集內 VM 執行個體的「數目」**。 您也可以進行垂直調整以增加或減少 VM 執行個體的*大小*。 有關詳細資訊，請參閱[使用虛擬機器縮放集的垂直自動縮放](virtual-machine-scale-sets-vertical-scale-reprovision.md)。
