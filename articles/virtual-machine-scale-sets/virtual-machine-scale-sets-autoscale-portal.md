...
5. 讓我們建立規則，以便平均 CPU 負載在 10 分鐘期間內大於 70% 時，增加擴展集內 VM 執行個體的數目。 觸發此規則時，VM 執行個體數目會增加 20%。 若擴展集內 VM 執行個體的數目很少，您可以將 [作業]**** 設定為 [增加計數]**，然後針對 [執行個體計數]** 指定 *1* 或 *2*。 若擴展集內 VM 執行個體的數目很多，則增加 VM 執行個體數目 10% 或 20% 可能更加適當。
    ...
    | 參數              | 說明                                                                                                         | 值          |
    |------------------------|---------------------------------------------------------------------------------------------------------------------|----------------|
    | *時間集合*     | 定義應該如何彙總收集的計量以進行分析。                                                | Average        |
    ...
    | *實體計數*       | 觸發此規則時，應該變更多少 VM 執行個體百分比。                                            | 20             |
...
在本文中，您已學到如何使用自動調整規則進行水平縮放，以及增加或減少擴展集內 VM 執行個體的「數目」**。 您也可以進行垂直調整以增加或減少 VM 執行個體的*大小*。 有關詳細資訊,請參閱[使用虛擬機縮放集的垂直自動縮放](virtual-machine-scale-sets-vertical-scale-reprovision.md)。
