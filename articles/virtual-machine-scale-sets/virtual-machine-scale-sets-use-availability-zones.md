...
若要使用最大分配，請將*platformFaultDomainCount*設定為*1*。 若要使用靜態的五個容錯網域分配，請將 *platformFaultDomainCount* 設為 *5*。 在 API 版本*2017-12-01*中，單一區域和跨區域擴展集的*platformFaultDomainCount*預設為*1* 。 目前，地區（非區域）擴展集只支援靜態的五個容錯網域分配。
...
若要使用最佳區域平衡，請將 *zoneBalance* 設為 *false*。 在 API 版本 *2017-12-01* 中，這項設定是預設值。 若要使用嚴格區域平衡，請將*zonebalance 設*設定為*true*。
