...
- 您還可以使用[resources.azure.com](https://resources.azure.com)或特定于語言的[Azure SDK](https://azure.microsoft.com/downloads/)
...
- 具有 az vms 的 Azure CLI[獲取實例視圖](/cli/azure/vmss)

    ```azurecli
    az vmss get-instance-view --resource-group myResourceGroup --name myScaleSet --instance-id instanceId
    ```

- 您還可以使用[resources.azure.com](https://resources.azure.com)或[Azure SDK](https://azure.microsoft.com/downloads/)
...
>[!NOTE]
> 服務結構群集只能使用*自動*模式，但更新的處理方式不同。 有關詳細資訊，請參閱[服務結構應用程式升級](../service-fabric/service-fabric-application-upgrade.md)。

有一種全域擴展集屬性修改並不遵守升級原則。 對規模集 OS 和資料磁片設定檔（如管理員使用者名和密碼）的更改只能在 API 版本*2017-12-01*或更高版本中更改。 這些變更只會套用至變更擴展集模型之後建立的 VM。 若要將現有的 VM 更新至最新狀態，您必須為每個現有的 VM 執行「重新安裝映像」作業。 您可以透過下列方式執行這個重新安裝映像作業：
...
如果透過擴充功能將應用程式部署至擴展集，則更新擴充組態時，會造成應用程式根據升級原則進行更新。 例如，如果您有要在自訂腳本擴展版中運行的腳本的新版本，則可以更新*fileUris*屬性以指向新腳本。 在某些情況下，即使擴充組態未變更 (例如，您已更新指令碼，但未變更指令碼的 URI)，您也可能會想要強制更新。 在這些情況下，您可以修改*強制UpdateTag*以強制更新。 Azure 平台不會解譯這個屬性。 如果您變更此值，並不會影響擴充功能的執行方式。 變更只會強制擴充功能重新執行。 有關*forceUpdateTag*的詳細資訊，請參閱[有關擴展的 REST API 文檔](/rest/api/compute/virtualmachineextensions/createorupdate)。 請注意，forceUpdateTag** 可以搭配所有擴充功能使用，而不只是搭配自訂指令碼擴充功能。
