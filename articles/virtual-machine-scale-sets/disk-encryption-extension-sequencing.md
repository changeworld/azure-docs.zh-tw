...
description: 在本文中，您將瞭解如何為 Linux IaaS Vm 啟用 Microsoft Azure 磁片加密。
author: ju-shim
ms.author: jushiman
ms.topic: how-to
...
Azure 磁片加密等擴充功能可以依指定的順序新增至 Azure 虛擬機器擴展集。 若要這樣做，請使用 [擴充功能排序](virtual-machine-scale-sets-extension-sequencing.md)。 

一般而言，加密應該套用至磁片：

- 在準備磁片或磁片區的延伸模組或自訂腳本之後。
- 在擴充功能或自訂腳本之前，可存取或取用加密磁片或磁片區上的資料。

在任一種情況下， `provisionAfterExtensions` 屬性會指定稍後應在序列中新增的延伸模組。
...
以下是使用 "CustomScriptExtension" 的範例，這個 Powershell 腳本會初始化並格式化 Windows 磁片，後面接著 "AzureDiskEncryption"：
...
如果您想要在另一個擴充功能之前套用 Azure 磁碟加密，請將 `provisionAfterExtensions` 屬性放在擴充功能的區塊中以進行追蹤。

以下是使用 "AzureDiskEncryption"，後面接著 ">microsoft.insights.vmdiagnosticssettings" 的範例，這是在 Windows 型 Azure VM 上提供監視和診斷功能的延伸模組：
