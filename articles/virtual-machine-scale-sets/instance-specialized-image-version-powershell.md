---
title: 從特製化映像建立擴展集
description: 使用共用映射庫中的特製化映射建立擴展集。
author: cynthn
...
ms.locfileid: "98877980"
---
# <a name="create-a-scale-set-from-a-specialized-image-using-powershell"></a>使用 PowerShell 從特製化映射建立擴展集 

使用 Azure PowerShell，從 [共用映射庫](../virtual-machines/shared-image-galleries.md) 中儲存的特製化映射版本建立 VM。 如果想要使用一般化映射版本建立擴展集，請參閱 [從一般化映射版本建立擴展集實例](instance-generalized-image-version-powershell.md)。

當您的資源庫中有特製化映射之後，您就可以使用 [new-azvmss 指令程式](/powershell/module/az.compute/new-azvmss) 來建立虛擬機器擴展集。 
