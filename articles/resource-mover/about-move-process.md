...
**元件** | **詳細資料**
--- | ---
**Resource Mover** |  Resource Mover 會與 [Azure 資源提供者](../azure-resource-manager/management/resource-providers-and-types.md)配合，以協調如何在區域之間移動資源。 Resource Mover 會分析資源相依性，並在移動過程中維護和管理資源的狀態。 
**移動集合** |  移動集合是 [Azure Resource Manager](../azure-resource-manager/management/overview.md) 物件。<br/><br/> 在進行區域移動程序的期間，會針對訂用帳戶中由來源和目標區域組合而成的每個配對建立移動集合。 集合中包含所要移動資源的相關中繼資料和設定資訊。<br/><br/>新增至移動集合的資源必須位於相同的訂用帳戶中，但可以位於不同的資源群組中。 
