...
---
# <a name="enable-safe-deployment-practices-with-azure-deployment-manager-public-preview"></a>使用 Azure 部署管理員啟用安全部署實踐（公共預覽版）
...
Azure 部署管理員處於預覽狀態。 通過提供[回饋](https://aka.ms/admfeedback)來説明我們改進功能。
...
- [教程：將 Azure 部署管理員與資源管理器範本一起使用](./deployment-manager-tutorial.md)。
- [教程：在 Azure 部署管理員 中使用運行狀況檢查](./deployment-manager-tutorial-health-check.md)。
...
您可以定義要在部署作業之前或之後執行的步驟。 目前，只有步驟`wait`和"運行狀況檢查"步驟可用。
...
有關運行狀況檢查步驟的詳細資訊，請參閱[將運行狀況集成推出引入 Azure 部署管理員](./deployment-manager-health-check.md)和[教程：在 Azure 部署管理員 中使用運行狀況檢查](./deployment-manager-tutorial-health-check.md)。
...
>
> [快速入門：只需幾分鐘即可試用 Azure 部署管理員](https://github.com/Azure-Samples/adm-quickstart)
