...
本文將逐步解說建立 Azure 自訂資源提供者端點的需求和最佳作法，以實行自訂資源。 如果您不熟悉 Azure 自訂資源提供者，請參閱 [自訂資源提供者的總覽](overview.md)。

## <a name="how-to-define-a-resource-endpoint"></a>如何定義資源端點

**端點**是指向服務的 URL，此服務會在其與 Azure 之間實行基礎合約。 端點是在自訂資源提供者中定義，而且可以是任何可公開存取的 URL。 下列範例具有由**resourceType** `myCustomResource` 所執行的 resourceType `endpointURL` 。
...
執行**resourceType**的**端點**必須處理 Azure 中新 API 的要求和回應。 建立具有 **resourceType** 的自訂資源提供者時，它會在 Azure 中產生一組新的 api。 在此情況下， **resourceType** 將會為、和產生新的 AZURE 資源 API， `PUT` `GET` `DELETE` 以在單一資源上執行 CRUD，以及 `GET` 取得所有現有的資源：
...
自訂資源的自訂資源提供者提供兩種類型的 **routingTypes**： " `Proxy` " 和 " `Proxy, Cache` "。

### <a name="proxy-routing-type"></a>proxy 路由類型

" `Proxy` " **RoutingType** 會將所有要求方法都 proxy 到自訂資源提供者中指定的 **端點** 。 使用時機 `Proxy` ：
...
" `Proxy, Cache` " **RoutingType** 僅 proxy `PUT` 和 `DELETE` 要求方法至自訂資源提供者中所指定的 **端點** 。 自訂資源提供者會 `GET` 根據其在快取中儲存的內容，自動傳回要求。 如果自訂資源已標示快取，自訂資源提供者也會新增/覆寫回應中的欄位，以使 Api 符合規範。 使用時機 `Proxy, Cache` ：
...
id | *是的* | 自訂資源的資源識別碼。 這應該存在於 **ResourceProvider**
properties | *是的* | 將傳送至 **端點**的要求主體。
...
參數 | 必要 | 描述
---|---|---
id | *是的* | 自訂資源的資源識別碼。 這應該會存在於 **ResourceProvider**中。
...
> [!NOTE]
> 資源要求回應包含來自端點的適當 `id` 、 `name` 和 `type` 。 **endpoint**

Azure Resource Manager 範本需要 `id` `name` `type` 從下游端點正確傳回、和。 傳回的資源回應應該採用下列格式：
...
參數 | 必要 | 描述
---|---|---
resourceTypeName | *是的* | 自訂提供者中定義的**resourceType** **名稱**。
resourceProviderName | *是的* | 自訂資源提供者實例名稱。
