...
「**端點**」（endpoint）是指向服務的 URL，它會在它與 Azure 之間執行基礎合約。 端點會定義在自訂資源提供者中，而且可以是任何可公開存取的 URL。 下列範例中的**resourceType**稱為`myCustomResource` 「實作為`endpointURL`」。
...
執行**resourceType**的**端點**必須處理 Azure 中新 API 的要求和回應。 建立具有**resourceType**的自訂資源提供者時，它會在 Azure 中產生一組新的 api。 在此情況下， **resourceType**會針對`PUT`、 `GET`和`DELETE`產生新的 Azure 資源 API，以在單一資源`GET`上執行 CRUD，以及抓取所有現有的資源：
...
針對自訂資源，自訂資源提供者提供**routingTypes**兩種類型`Proxy`的 routingTypes：`Proxy, Cache`"" 和 ""。
...
"`Proxy, Cache`" 只**routingType** `PUT`會 routingType proxy， `DELETE`並向自訂資源提供者中指定的**端點**要求方法。 自訂資源提供者會根據`GET`它儲存在快取中的內容，自動傳回要求。 如果自訂資源是以快取標記，自訂資源提供者也會在回應中新增/覆寫欄位，使 Api 符合規範。 使用 "`Proxy, Cache`" 的時機：
...
參數 | 必要 | 描述
---|---|---
id | *是的* | 自訂資源的資源識別碼。 這應該存在於**ResourceProvider**中。
...
> [!NOTE]
> 資源要求回應包含來自**端點**的適當`id`、 `name`和`type` 。

Azure Resource Manager 範本需要從`id`下游`name`端點正確`type`傳回、和。 傳回的資源回應應採用下列格式：
