---
title: 使用策略部署關聯
description: 瞭解如何使用 Azure 策略服務部署自訂提供程式的關聯。
author: msHich
ms.topic: conceptual
ms.date: 09/06/2019
ms.author: hich
...
---
# <a name="deploy-associations-for-a-custom-provider-using-azure-policy"></a>使用 Azure 策略部署自訂提供程式的關聯

Azure 策略可用於部署關聯以將資源關聯到自訂提供程式。 在本文中，我們將介紹部署關聯的內置策略以及如何使用該策略。

## <a name="built-in-policy-to-deploy-associations"></a>部署關聯的內置策略

部署自訂提供程式的關聯是一種內置策略，可用於部署關聯以將資源關聯到自訂提供程式。 該策略接受三個參數：

- 自訂提供程式 ID - 此 ID 是需要關聯資源的自訂提供程式的資源識別碼。
- 要關聯的資源類型 - 這些資源類型是要與自訂提供程式關聯的資源類型的清單。 您可以使用同一策略將多個資源類型關聯到自訂提供程式。
- 關聯名稱首碼 - 此字串是要添加到要創建的關聯資源的名稱的首碼。 預設值為"部署策略"。

該策略使用 DeployIfNot 存在評估。 它在資來源提供者處理創建或更新資源請求並返回成功狀態碼後運行。 之後，使用範本部署部署關聯資源。
有關關聯的詳細資訊，請參閱[Azure 自訂提供程式資源載入](./concepts-resource-onboarding.md)

## <a name="how-to-use-the-deploy-associations-built-in-policy"></a>如何使用部署關聯內置策略 

### <a name="prerequisites"></a>Prerequisites
如果自訂提供程式需要對策略範圍的許可權來執行操作，則不授予許可權，關聯資源的策略部署將無法工作。

### <a name="policy-assignment"></a>原則指派
要使用內置策略，請創建策略分配並為自訂提供程式策略分配部署關聯。 然後，策略將標識不合規的資源，並部署這些資源的關聯。

![分配內置策略](media/concepts-built-in-policy/assign-builtin-policy-customprovider.png)
...
- [概念：Azure 自訂提供程式資源載入](./concepts-resource-onboarding.md)
