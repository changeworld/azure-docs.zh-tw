---
title: 自訂資源 Proxy 參考
description: Azure 自訂資源提供者的自訂資源 proxy 參考。 本文將探討執行 proxy 自訂資源的端點需求。
ms.topic: conceptual
ms.author: jobreen
author: jjbfour
ms.date: 06/20/2019
...
本文將探討執行 proxy 自訂資源的端點需求。 如果您不熟悉 Azure 自訂資源提供者，請參閱 [自訂資源提供者的總覽](overview.md)。
...
實作為「Proxy」資源**端點**的**端點**必須處理 Azure 中新 API 的要求和回應。 在此情況下， **resourceType** 將會為、和產生新的 AZURE 資源 API `PUT` `GET` `DELETE` ，以在單一資源上執行 CRUD，以及 `GET` 取得所有現有的資源。

> [!NOTE]
> `id`、 `name` 和 `type` 欄位並非必要，但需要將自訂資源與現有的 Azure 生態系統整合。
