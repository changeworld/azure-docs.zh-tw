...
description: 瞭解如何使用 Azure 原則服務部署受控應用程式的關聯。
author: msHich
ms.topic: conceptual
ms.date: 09/06/2019
ms.author: hich
...
您可以使用 Azure 原則來部署關聯，以將資源與受控應用程式產生關聯。 在本文中，我們會說明部署關聯的內建原則，以及如何使用該原則。

## <a name="built-in-policy-to-deploy-associations"></a>部署關聯的內建原則

部署受控應用程式的關聯是內建原則，可用來部署關聯，以將資源與受控應用程式產生關聯。 原則會接受三個參數：

- 受控應用程式識別碼-此識別碼是資源需要相關聯之受控應用程式的資源識別碼。
- 要建立關聯的資源類型-這些資源類型是要與受控應用程式相關聯的資源類型清單。 您可以使用相同的原則，讓多個資源類型與受控應用程式產生關聯。
- 關聯名稱前置詞-此字串是要加入至所建立關聯資源名稱的前置詞。 預設值為 "DeployedByPolicy"。

原則會使用 DeployIfNotExists 評估。 它會在資源提供者處理所選資源類型的建立或更新資源要求 () s 之後執行，且評估已傳回成功狀態碼。 之後，會使用範本部署來部署關聯資源。
...
如果受控應用程式需要訂用帳戶的許可權來執行動作，則在不授與許可權的情況下，關聯資源的原則部署將無法運作。

### <a name="policy-assignment"></a>原則指派
若要使用內建原則，請建立原則指派，並指派受控應用程式原則的部署關聯。 一旦成功指派原則，原則將會識別不符合規範的資源，並部署這些資源的關聯。
...
- [快速入門：建立自訂資源提供者和部署自訂資源](../custom-providers/create-custom-provider.md)
