...
在本文中，您將了解 Azure Resource Manager 與傳統部署模型。 Resource Manager 和傳統部署模型代表部署和管理 Azure 解決方案的兩個不同方式。 您會透過兩個不同的 API 集使用它們，而所部署的資源可能包含重要的差異。 這兩種型號彼此不相容。 本文將說明這些差異。

為了簡化資源的部署和管理，Microsoft 建議您針對所有新資源使用 Resource Manager。 可能的話，Microsoft 建議您透過 Resource Manager 重新部署現有的資源。

如果您是資源管理器的新功能，則可能需要首先查看[Azure 資源管理器概述](overview.md)中定義的術語。

## <a name="history-of-the-deployment-models"></a>部署模型的歷程記錄

Azure 原本指提供傳統部署模型。 在此模型中，每個資源會獨立存在；沒辦法將相關的資源群組在一起。 因此，您必須手動追蹤哪些資源組成您的解決方案或應用程式，並記得要以協調的方法進行管理。 若要部署解決方案，您必須透過入口網站個別建立每個資源，或建立會以正確的順序部署所有資源的指令碼。 若要刪除解決方案，您必須個別刪除每個資源。 您無法輕鬆應用和更新相關資源的存取控制策略。 最後，您無法將標記應用於資源，以便使用可説明您監視資源和管理計費的條款來標記它們。
...
加入資源管理員時，所有資源會追溯地加入至預設資源群組。 如果現在通過經典部署創建資源，則該資源將自動在該服務的預設資源組中創建，即使您未在部署時指定該資源組。 但是，資源組中存在並不意味著資源已轉換為資源管理器模型。
...
若為虛擬機器、儲存體帳戶和虛擬網路，如果資源是透過傳統部署建立，您必須繼續透過傳統作業來運作。 如果虛擬機器、儲存體帳戶或虛擬網路是透過 Resource Manager 部署建立，您必須繼續使用 Resource Manager 作業。 當您的訂用帳戶包含透過 Resource Manager 和傳統部署建立的各種資源時，此區別可能讓您困惑。 這種資源組合可能會產生意外的結果，因為資源不支援相同的操作。

在某些情況下，Resource Manager 命令可以擷取透過傳統部署建立之資源的相關資訊，或可以執行系統管理工作 (例如將傳統資源移至另一個資源群組)。 但是，這些案例不應給人一種類型支援資源管理器操作的印象。 例如，假設您的資源群組包含使用傳統部署所建立的虛擬機器。 若您執行下列 Resource Manager PowerShell 命令：
...
不過，Resource Manager Cmdlet **Get-AzVM** 只會傳回透過 Resource Manager 部署的虛擬機器。 以下命令不會返回通過經典部署創建的虛擬機器。

```powershell
Get-AzVM -ResourceGroupName ExampleGroup
```

只有透過資源管理員建立的資源支援標記。 不能將標記應用於經典資源。
...
* 虛擬機器引用在網路資來源提供者中定義的特定網路介面卡（必需）和在計算資來源提供者中定義的可用性集（可選）。
* 網路介面卡引用虛擬機器分配的 IP 位址（必需）、虛擬機器虛擬網路的子網（必需）和網路安全性群組（可選）。
* 虛擬網路內的子網路會參考網路安全性群組 (選擇性)。
* 負載等化器實例引用 IP 位址的後端池，其中包括虛擬機器的網路介面卡（可選），並引用負載等化器公共或私人 IP 位址（可選）。
...
* 用來做為裝載虛擬機器 (計算) 之容器所需的雲端服務。 虛擬機器會自動提供網路介面卡和 Azure 分配的 IP 位址。 此外，雲端服務包含外部負載平衡器執行個體、共用 IP 位址及預設端點，以允許 Windows 架構虛擬機器的遠端桌面與遠端 PowerShell 流量，以及 Linux 架構虛擬機器的安全殼層 (SSH) 流量。
* 存儲虛擬機器虛擬硬碟（包括作業系統、臨時資料磁片和其他資料磁片（存儲）所需的存儲帳戶。
* 充當附加容器的可選虛擬網路，您可以在其中創建子網結構並選擇虛擬機器所在的子網（網路）。
...
| 虛擬網路 |虛擬網路是虛擬機器的選用項目。 如果包括，則無法與資源管理器一起部署虛擬網路。 |虛擬機器需要已使用 Resource Manager 部署的虛擬網路。 |
| 儲存體帳戶 |虛擬機器需要存儲存儲作業系統、臨時磁片和其他資料磁片的虛擬硬碟的存儲帳戶。 |虛擬機器需要儲存體帳戶，才能將其磁碟儲存在 Blob 儲存體。 |
...
| 同質群組 |建立虛擬網路時需要同質群組。 但是，隨著區域虛擬網路的引入，這不再需要了。 |簡而言之，透過 Azure Resource Manager 而公開的 API，其實不存在同質群組這種概念。 |
...
| 網路介面 |主要和次要網路介面與其屬性會定義為虛擬機器的網路組態。 |網路介面是 Microsoft.Network 提供者所公開的資源。 網路介面的生命週期不綁定到虛擬機器。 它會參考虛擬機器指派的 IP 位址 (必要)、虛擬機器之虛擬網路的子網路 (必要)，以及網路安全性群組 (選擇性)。 |
...
不支援此配置。 不能使用資源管理器將虛擬機器部署到使用經典部署創建的虛擬網路中。

**我可以使用 Resource Manager 透過使用傳統部署建立的使用者映像，來建立虛擬機器嗎？**

不支援此配置。 但是，可以從使用經典部署模型創建的存儲帳戶複製虛擬硬碟檔，並將它們添加到通過資源管理器創建的新帳戶。
...
可在[Azure 資源管理器快速入門範本](https://azure.microsoft.com/documentation/templates/)上找到一組全面的初學者範本。
