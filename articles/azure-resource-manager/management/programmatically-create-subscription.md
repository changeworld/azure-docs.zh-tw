...
執行下列[get-azsubscription](/powershell/module/az.subscription)命令，並將取代`<enrollmentAccountObjectId>`為第一個`ObjectId`步驟（```747ddfe5-xxxx-xxxx-xxxx-xxxxxxxxxxxx```）中所收集的。 如果您想要指定擁有者，請瞭解[如何取得使用者物件識別碼](grant-access-to-create-subscription.md#userObjectId)。
...
首先，執行以安裝此預覽擴充`az extension add --name subscription`功能。
...
- 每個註冊帳戶的訂閱限制為500。 之後，就只能在 Azure 入口網站中建立帳戶的更多訂閱。 如果您想要透過 API 建立更多訂閱，請建立另一個註冊帳戶。
...
```
使用`displayName`屬性來識別您要建立訂閱的帳單帳戶。 請確定帳戶的 agreeementType 是*MicrosoftCustomerAgreement*。 `name`複製帳戶的。  例如，如果您想要建立`Contoso`帳單帳戶的訂閱，請複製。 `5e98e158-xxxx-xxxx-xxxx-xxxxxxxxxxxx:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_xxxx-xx-xx` 將此值貼在某處，以便您在下一個步驟中使用。
...
| `managementGroupId` | 否      | 字串 | 將新增訂閱的管理群組識別碼。 若要取得管理群組的清單，請參閱[管理群組-清單 API](/rest/api/resources/managementgroups/list)。 使用 API 中的管理群組識別碼。 |
...
```
使用`displayName`屬性來識別您要建立訂閱的帳單帳戶。 請確定帳戶的 agreeementType 是*MicrosoftPartnerAgreement*。 複製帳戶`name`的。 例如，如果您想要建立`Contoso`帳單帳戶的訂閱，請複製。 `99a13315-xxxx-xxxx-xxxx-xxxxxxxxxxxx:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_xxxx-xx-xx` 將此值貼在某處，以便您在下一個步驟中使用。

### <a name="find-customers-that-have-azure-plans"></a>尋找具有 Azure 方案的客戶

進行下列要求，以從`<billingAccountName>`第一個`name`步驟複製的（```5e98e158-xxxx-xxxx-xxxx-xxxxxxxxxxxx:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_xxxx-xx-xx```）來取代，以列出帳單帳戶中可建立 Azure 訂用帳戶的所有客戶。
...
使用`displayName`屬性來識別您要建立訂閱的客戶。 複製客戶`id`的。 例如，如果您想要建立的訂用帳戶`Fabrikam toys`，請複製`/providers/Microsoft.Billing/billingAccounts/99a13315-xxxx-xxxx-xxxx-xxxxxxxxxxxx:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_xxxx-xx-xx/customers/2281f543-xxxx-xxxx-xxxx-xxxxxxxxxxxx`。 將此值貼到某個位置，以便在後續步驟中使用它。
...
提出下列要求，並將`<customerId>`取代為`id`第二個步驟所複製```/providers/Microsoft.Billing/billingAccounts/99a13315-xxxx-xxxx-xxxx-xxxxxxxxxxxx:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_xxxx-xx-xx/customers/2281f543-xxxx-xxxx-xxxx-xxxxxxxxxxxx```的（），以列出適用于客戶的所有轉銷商。
...
```
使用`description`屬性來識別將與訂用帳戶相關聯的轉銷商。 複製轉`resellerId`銷商的。 例如，如果您想要建立關聯`Wingtip`，請複製`3xxxxx`。 將此值貼在某處，以便您在下一個步驟中使用。

### <a name="create-a-subscription-for-a-customer"></a>為客戶建立訂用帳戶

下列範例會建立名為「 *Fabrikam 玩具*的*開發小組訂閱*」的訂用帳戶，並將*Wingtip*轉售商與訂用帳戶建立關聯。 T

提出下列要求，將取代`<customerId>`為第`id`二個步驟所複製的```/providers/Microsoft.Billing/billingAccounts/99a13315-xxxx-xxxx-xxxx-xxxxxxxxxxxx:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_xxxx-xx-xx/customers/2281f543-xxxx-xxxx-xxxx-xxxxxxxxxxxx```（）。 在 API 的要求參數中，傳遞從第二個步驟複製的選擇性*resellerId* 。
