...
Microsoft 負責管理 NSX-t 設備的生命週期，例如 NSX-T Manager 和 NSX-T 邊緣。 Microsoft 也負責啟動網路設定，例如建立第0層閘道，以及啟用北南部路由。 身為您的 AVS 私用雲端的系統管理員，您必須負責 NSX-T SDN 設定，例如網路區段、分散式防火牆規則、第1層閘道和負載平衡器。

> [!IMPORTANT]
> AVS 系統管理員不得修改 NSX-T 邊緣或第0層閘道的設定。 這可能會導致服務遺失。
