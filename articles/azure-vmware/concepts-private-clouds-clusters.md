...
Microsoft 負責管理 NSX-T 設備（例如 NSX-T 管理員和 NSX-T Edge）的生命週期管理。 Microsoft 也負責啟動網路設定，例如建立第0層閘道，以及啟用北南部路由。 以系統管理員身分使用您的 Azure VMware 解決方案私人雲端，您必須負責 NSX-T SDN 設定。 例如，網路區段、分散式防火牆規則、第1層閘道和負載平衡器。
