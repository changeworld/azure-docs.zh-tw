...
ms.locfileid: "82739523"
---
# <a name="networking-checklist-for-azure-vmware-solution-avs"></a>Azure VMware 解決方案 (AVS) 的網路檢查清單

Azure VMware 解決方案提供 VMware 私人雲端環境，可讓使用者和應用程式從內部部署和 Azure 型環境或資源存取。 連線會透過網路服務 (例如 Azure ExpressRoute 和 VPN 連線) 來傳遞，而且需要一些特定的網路位址範圍和防火牆連接埠來啟用服務。 本文提供正確設定網路以與 AVS 搭配運作所需的資訊。
...
AVS 私人雲端會使用 Azure ExpressRoute 連線來連線到您的 Azure 虛擬網路。 此高頻寬、低延遲連線可讓您從私人雲端環境存取在 Azure 訂用帳戶中執行的服務。

對於子網路，AVS 私人雲端需要至少 `/22` CIDR 網路位址區塊，如下所示。 此網路會補足您的內部部署網路。 若要連線到內部部署環境和虛擬網路，這必須是非重疊的網路位址區塊。
...
雲端閘道|      L2C |    TCP (HTTP)  |   443 |   當 L2C 使用與混合式雲端閘道相同的路徑時，將管理指示從雲端閘道傳送至 L2C。
...
準備好適當的網路功能後，請繼續進行下一個教學課程，以建立您的 AVS 私人雲端。

> [!div class="nextstepaction"]
> [教學課程：建立 AVS 私人雲端](tutorial-create-private-cloud.md)
