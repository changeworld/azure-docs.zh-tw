...
某些環境會有防火牆來保護 ExpressRoute 電路。  如果沒有防火牆且沒有進行路由剪除，您可以從內部部署環境偵測您的 Azure VMware 解決方案 vCenter Server 或 NSX-T 區段上的 [VM](deploy-azure-vmware-solution.md#add-a-vm-on-the-nsx-t-network-segment)。

此外，您可以從位於 NSX-T 區段的 VM 觸達內部部署環境中的資源。
