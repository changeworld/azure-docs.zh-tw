...
ms.locfileid: "92495702"
---
# <a name="back-up-azure-vmware-solution-vms-with-azure-backup-server"></a>使用 Azure 備份伺服器備份 Azure VMware 解決方案 Vm

在本文中，我們會將 VMware 虛擬機器 (Vm) 使用 Azure 備份伺服器在 Azure VMware 解決方案上執行的 Vm 進行備份。 首先，請徹底完成 [設定適用于 Azure VMware 解決方案的 Microsoft Azure 備份 Server](set-up-backup-server-for-azure-vmware-solution.md)。
...
1. 在 [ **完成]** 頁面上，檢查設定，然後選取 [ **關閉**]。
   ...
   您會看到在 [實際執行 **伺服器** ] 底下列出的 vCenter server 具有：
   - 類型為 **VMware 伺服器** 
   - 代理程式狀態為 **確定** 
...
1. 在 [ **審核磁碟儲存體配置** ] 頁面上，檢查為 VM 備份提供的磁碟空間。
   ...
   - **資料大小：** 保護群組中的資料大小。
   - **磁碟空間：** 建議的保護群組磁碟空間量。 如果您想要修改此設定，請選取比您估計的每個資料來源數量還大的空間。
...
1. 在 [ **指定線上備份排程** ] 頁面上，指出您要將資料從本機儲存體備份到 Azure 的頻率。 
...
1. 在 [ **指定線上保留原則** ] 頁面上，指出您要將從備份中建立的復原點保留到 Azure 的時間長度。
...
- 在 [ **監視** ] 工作區中：
   - 在 [ **警示**] 底下，您可以監視錯誤、警告和一般資訊。  您可以查看作用中和非作用中警示，並設定電子郵件通知。
   - 在 [ **工作**] 下，您可以針對特定受保護的資料來源或保護群組，查看 Azure 備份伺服器啟動的工作。 您可以追蹤工作進度，或檢查工作所耗用的資源。
...
- **替代位置復原 (ALR) **：當缺少原始 vm，或您不想要擾亂原始 vm 時使用。 提供 ESXi 主機、資源集區、資料夾和儲存體資料存放區和路徑的位置。 為了協助區分還原的 VM 與原始 VM，Azure 備份伺服器會將「 *-已復原* 」附加至 VM 的名稱。
...
1. 使用 [瀏覽] 窗格，瀏覽或篩選以尋找您想要復原的 VM。 選取 VM 或資料夾後，[復原點] 窗格會顯示可用的復原點。

   ![可用的復原點](../backup/media/restore-azure-backup-server-vmware/recovery-points.png)

1. 在 [ **恢復** 點] 窗格中，選取取得復原點的日期。 粗體的行事曆日期具備可用的復原點。 或者，您可以在 VM 上按一下滑鼠右鍵，然後選取 [ **顯示所有復原點** ]，然後從清單中選取復原點。
...
1. 在 [ **指定目的地** ] 畫面上，選取 **[流覽]** 以尋找檔案或資料夾的網路位置。 Azure 備份伺服器會建立所有已復原專案都會複製到其中的資料夾。 資料夾名稱的前置詞 MABS_day-month-year。 當您選取已復原檔案或資料夾的位置時，會提供該位置的詳細資料。
