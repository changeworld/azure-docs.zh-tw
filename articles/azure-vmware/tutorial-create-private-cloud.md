...
使用 `[az group create](/cli/azure/group)` 命令建立資源群組。 Azure 資源群組是在其中部署與管理 Azure 資源的邏輯容器。 下列範例會在 eastus 位置建立名為 myResourceGroup 的資源群組：
...
如果您有不再需要的 Azure VMware 解決方案私人雲端，可以將其刪除。 Azure VMware 解決方案私人雲端包含一個隔離的網路網域、一個或多個佈建在專用伺服器主機上的 vSphere 叢集，和數個虛擬機器。 刪除私人雲端時，會刪除所有的虛擬機器、其資料和叢集。 系統會安全地抹除專用的裸機主機，並將其傳回到可用的集區。 已刪除為客戶佈建的網路網域。  
...
3. 輸入私人雲端的名稱，然後選取 [是]。 刪除程序會在幾個小時內完成。  
