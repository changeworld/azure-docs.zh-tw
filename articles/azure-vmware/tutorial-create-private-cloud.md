...
由於 AVS 不允許您在啟動時使用內部部署 vCenter 來管理私人雲端，所以必須執行額外的組態，並連線到本機 vCenter 執行個體、虛擬網路等等。 本教學課程涵蓋這些程序和相關的必要條件。
...
您可以使用 Azure Cloud Shell 來使用 Azure CLI，而不是使用 Azure 入口網站來建立 AVS 私人雲端。 Azure Cloud Shell 是一個免費的互動式殼層，具有預先安裝和設定的通用 Azure 工具，可與您的帳戶搭配使用。 
...
如果您有不再需要的 AVS 私人雲端，可以將其刪除。 刪除私人雲端時，會刪除所有的叢集及其所有元件。
