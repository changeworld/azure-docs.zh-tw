...
Azure 資料目錄是完全受控的雲端服務，可作為企業資料來源的註冊和探索系統。 換句話說，資料目錄説明人們發現、理解和使用資料來源。 它可説明組織從現有資料中獲取更多價值。 在資料目錄註冊資料來源之後，其中繼資料會由此服務編製索引，讓您可以輕鬆地搜尋以探索所需的資料。
...
查看搜尋結果時，即使啟用了點擊突出顯示，資料資產也並不總是顯而易見。 因為預設會搜尋所有屬性，某個資料資產可能因為符合資料行層級的屬性而被傳回。 由於多個使用者可以使用自己的標記和說明對已註冊的資料資產進行批號，因此並非所有中繼資料都顯示在搜尋結果清單中。
