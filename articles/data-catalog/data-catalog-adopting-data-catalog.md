...
**Azure 資料目錄** 可讓艾德公司的財務小組在關鍵資料來源上進行共同作業，因此每位小組成員都可以輕鬆地找到並使用所需的資料，並且可以與整個小組分享其知識。
...
當您在選擇試驗專案時，請選取其案例最引人注目且能解決現有商務問題的團隊。 例如，商務分析師會從 SQL Server Database 建立報表。 問題在於，他們只會在與數個同事交談之後，才知道資料來源。 最後，在浪費時間嘗試尋找要使用的資料來源之後，他們發現有關 Excel 活頁簿的資訊，其中包含每個資料來源的描述。 雖然 Excel 活頁簿會適當地描述所需的資料表，但如果這些資料來源已在 **Azure 資料目錄**中註冊和批註，則會快速找到這些資料來源。
...
**Azure 資料目錄**所解決的其中一個商務問題是將**資料產生者**連接至**資料取用者**。 其解決方式是作為企業資料來源相關資訊的中央存放庫。 David 使用資料目錄註冊了 Adventure Works 和 SQL Server 資料來源。 除了使用探索到的資料之外，任何探索此資料來源的使用者都可以在資料上共用其意見。 比方說，Nancy 在搜尋目錄後找到資料來源，並分享其關於資料的專業知識。  現在，組織中的其他人可藉由搜尋資料目錄從所分享的知識獲益。
...
在此範例中，商務分析師 Nancy 使用 SQL Server Database 中的資料為其團隊建立報告。 問題在於，他們只會在與數個同事交談之後，才知道資料來源。 如果這些資料來源已在中央位置（例如 **Azure 資料目錄**）進行註冊和批註，他們就能快速找到這些資料來源。
...
* **願意失敗** -如果試驗未達到想要的結果，請重新評估並找出要變更的區域以修正試驗中的問題，然後再移至較大的範圍。
