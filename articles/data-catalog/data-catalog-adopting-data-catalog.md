...
**Azure 資料目錄**使 Adventure Works 財務團隊能夠就關鍵資料來源進行協作，因此每個團隊成員可以輕鬆地查找和使用所需的資料，並可以與整個團隊共用他們的知識。
...
當您在選擇試驗專案時，請選取其案例最引人注目且能解決現有商務問題的團隊。 例如，商務分析師會從 SQL Server Database 建立報表。 問題是，他們只是在與幾個同事交談後才意識到資料來源。 最後，在浪費時間嘗試查找要使用的資料來源後，他們發現了一個 Excel 活頁簿，其中包含每個資料來源的說明。 儘管 Excel 活頁簿充分描述了他們需要的表，但如果這些資料來源在**Azure 資料目錄**中註冊並進行了說明，它們很快就會找到這些資料來源。
...
**Azure 資料目錄**所解決的其中一個商務問題是將**資料產生者**連接至**資料取用者**。 其解決方式是作為企業資料來源相關資訊的中央存放庫。 David 使用資料目錄註冊了 Adventure Works 和 SQL Server 資料來源。 使用眾包，任何發現此資料來源的使用者都可以分享她對資料的看法，以及使用他們發現的資料。 比方說，Nancy 在搜尋目錄後找到資料來源，並分享其關於資料的專業知識。  現在，組織中的其他人可藉由搜尋資料目錄從所分享的知識獲益。
...
在此範例中，商務分析師 Nancy 使用 SQL Server Database 中的資料為其團隊建立報告。 問題是，他們只是在與幾個同事交談後才意識到資料來源。 如果在中央位置（如**Azure 資料目錄**）中註冊並進行註冊和加帶，他們很快就會找到這些資料來源。
...
* **願意失敗**- 如果飛行員沒有達到預期的結果，重新評估並確定要改變的領域 - 在轉移到更大的範圍之前修復試驗中的問題。
