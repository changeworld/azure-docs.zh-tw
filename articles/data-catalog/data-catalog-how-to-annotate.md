...
每個人都有意見。 而這是一件好事。
資料目錄了解不同的使用者對企業資料來源有不同的觀點，而每個觀點都具有潛在的價值。 請考慮下列案例：

* 系統管理員知道服務或託管資料來源之服務的服務等級協定。
* 資料庫管理員知道各資料庫的備份排程，以及 ETL 處理時段。
* 系統擁有者知道使用者要求資料來源存取權的程序。
* 資料負責人知道資料來源中的資產和屬性如何對應至企業資料模型。
* 分析人員知道資料在支援業務流程的上下文中是如何使用的。

上述各觀點都具有價值，而資料目錄針對中繼資料使用群眾外包的方使，讓每個觀點都能夠被擷取並用來提供已註冊資料來源的全局。 使用資料目錄門戶，每個使用者可以添加和編輯自己的批註，同時能夠查看其他使用者提供的批註。
...
| 標籤 (詞彙標籤) |標記可以在資料資產和屬性 / 資料行層級提供。 詞彙標籤是集中定義的詞彙，可用於分類資料資產或使用通用的商務分類法的屬性。 有關詳細資訊[，請參閱如何設置治理標記的業務術語表](data-catalog-how-to-business-glossary.md) |
