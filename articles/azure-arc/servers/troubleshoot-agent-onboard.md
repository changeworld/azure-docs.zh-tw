---
title: 為已啟用 Azure Arc 的伺服器進行疑難排解 (預覽) 代理程式連線問題
description: 本文說明如何在嘗試連線到服務時，使用已啟用 Azure Arc 的伺服器 (預覽) ，針對已啟動的電腦代理程式進行疑難排解並解決問題。
ms.date: 07/20/2020
ms.topic: conceptual
...
ms.locfileid: "88213542"
---
# <a name="troubleshoot-the-connected-machine-agent-connection-issues"></a>針對連線的機器代理程式連接問題進行疑難排解

本文提供疑難排解和解決嘗試設定已啟用 Azure Arc 的伺服器時可能發生的問題的相關資訊 (預覽) 適用于 Windows 或 Linux 的連線電腦代理程式。 包括設定與服務的連線時的互動式和大規模安裝方法。 如需一般資訊，請參閱 [啟用 Arc 的伺服器總覽](./overview.md)。
