...
* **ApplicationId**屬性的值用於 `--service-principal-id` 參數值
...
下列步驟會使用腳本範本，在您的混合式電腦上安裝及設定已連線的電腦代理程式，其會執行將[混合式機器從 Azure 入口網站一文連接到 Azure](onboard-portal.md)中所述的類似步驟。 其差異在於最後一個步驟中，您可以使用 `azcmagent` 命令，透過服務主體建立與 Azure Arc 的連線。 

以下是您設定 `azcmagent` 命令以用於服務主體的設定。

* `tenant-id`：代表 Azure AD 專用實例的唯一識別碼（GUID）。
* `subscription-id`：您要讓機器在其中的 Azure 訂用帳戶的訂用帳戶識別碼（GUID）。
* `resource-group`：您要連線的電腦所屬的資源組名。
...
您可以藉由查看[Azcmagent 參考](azcmagent-reference.md)，深入瞭解 `azcmagent` 命令列工具。
