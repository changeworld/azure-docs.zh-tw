...
![成功的伺服器連線](./media/quickstart-onboard/arc-for-servers-successful-onboard.png)
...
1. 若要從機器解除安裝 Windows 代理程式，請執行下列動作：
    ...
    如果您想要編寫解除安裝指令碼，則可以使用下列範例，其會使用 Msiexec.exe 命令列 `msiexec /x {Product Code}` 來擷取產品代碼並解除安裝該代理程式。 若要這樣做：  
