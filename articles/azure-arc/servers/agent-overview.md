...
* 在安裝期間會建立下列安裝資料夾。
    ...
    |%ProgramData%\GuestConfig |包含 (已套用) Azure 原則相關檔案。|

* 在安裝代理程式期間，會在目標電腦上建立下列 Windows 服務。

    |服務名稱 |顯示名稱 |程序名稱 |描述 |
    |-------------|-------------|-------------|------------|
    |himds |Azure 混合式 Instance Metadata Service |himds.exe |此服務會實作 Azure Instance Metadata service (IMDS) 以追蹤機器。|
...
* 有四個記錄檔可用來進行疑難排解。 如下表中所述。
    ...
    |%ProgramData%\GuestConfig\gc_agent_logs\gc_agent.log |記錄 DSC 服務活動的詳細資料，<br> 特別是 himds 服務和 Azure 原則之間的連線能。|
...
* 在安裝期間會建立下列安裝資料夾。
    ...
    |/var/lib/GuestConfig |包含 (已套用) Azure 原則相關檔案。|

* 在安裝代理程式期間，會在目標電腦上建立下列精靈。

    |服務名稱 |顯示名稱 |程序名稱 |描述 |
    |-------------|-------------|-------------|------------|
    |himdsd.service |Azure 混合式 Instance Metadata Service |/opt/azcmagent/bin/himds |此服務會實作 Azure Instance Metadata service (IMDS) 以追蹤機器。|
