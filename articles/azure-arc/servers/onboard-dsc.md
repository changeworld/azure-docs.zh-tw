...
- `TenantId`：代表 Azure AD 專用實例的唯一識別碼（GUID）。

- `SubscriptionId`：您要讓機器在其中的 Azure 訂用帳戶的訂用帳戶識別碼（GUID）。

- `ResourceGroup`：您要連線的電腦所屬的資源組名。

- `Location`：請參閱[支援的 Azure 區域](overview.md#supported-regions)。 這個位置可以與資源群組的位置相同或不同。

- `Tags`：應套用至已連線電腦資源之標記的字串陣列。

- `Credential`：具有**ApplicationId**和**密碼**的 PowerShell 認證物件，可用來使用[服務主體](onboard-service-principal.md)來大規模註冊電腦。 

1. 在 PowerShell 主控台中，流覽至您儲存 `.ps1` 檔案的資料夾。
...
3. 這會在名為 `C:\dsc`的新資料夾中建立 `localhost.mof file`。
