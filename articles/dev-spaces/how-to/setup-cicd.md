...
ms.topic: conceptual
manager: gwallace
description: 瞭解如何使用 Azure 開發人員空間使用 Azure DevOps 設置連續集成/連續部署
...
當系統提示您選擇父開發空間時，_\<請選擇\>"無_"。

創建開發空間後，需要確定主機尾碼。 使用`azds show-context`命令顯示 Azure 開發空間入口控制器的主機尾碼。
...
在上面的示例中，主機尾碼_fedcba098.eus.azds.io_。 稍後將創建發佈定義時使用此值。
...
1. 選擇創建**新建**管道的選項。
1. 選擇**GitHub**作為源，如有必要，使用 GitHub 帳戶進行授權，並從_開發空間_應用程式範例存儲庫的分叉版本中選擇_azds_updates_分支。
1. 選擇 **"配置"作為代碼**，或作為**YAML**作為範本。
...
您現在擁有一個 CI 解決方案，可以自動建置 *mywebapi* 和 *webfrontend*，以便將任何更新推送到 GitHub 分支的 _azds_updates_ 分支。 您可以通過導航到 Azure 門戶、選擇 Azure 容器註冊表和流覽**存儲庫**選項卡來驗證 Docker 映射已被推送。映射可能需要幾分鐘才能生成並顯示在容器註冊表中。
...
1. 按一下 **"流覽"** 並從專案中選擇`samples/release.json`。
...
1. 啟用**連續部署觸發器**。
1. 將滑鼠懸停在**管道**旁邊的 **"任務"** 選項卡上，然後按一下 _"開發"_ 以編輯_開發_階段任務。
1. 在 **"連線類型**"下選擇"驗證**Azure 資源管理器**"。 您將看到以紅色突出顯示的三個下拉控制項：![發佈定義設置](../media/common/release-setup-tasks.png)
1. 選擇使用 Azure 開發空間的 Azure 訂閱。 您可能還需要按一下 **"授權**"。
1. 選擇使用 Azure 開發空間的資源組和群集。
1. 按一下 **"代理作業**"。
1. 選擇**託管 Ubuntu 1604**下的**代理池**。
1. 將滑鼠懸停在頂部**的"任務"** 選擇器上，按一下_prod_以編輯_prod_階段任務。
1. 在 **"連線類型**"下選擇"驗證**Azure 資源管理器**"。 選擇與 Azure 開發空間一起使用的 Azure 訂閱、資源組和群集。
1. 按一下 **"代理作業**"。
1. 選擇**託管 Ubuntu 1604**下的**代理池**。
1. 按一下 **"變數"** 選項卡以更新版本的變數。
1. 將**DevSpacesHostSuffix**的值從**UPDATE_ME**更新到主機尾碼。 在較早之前運行命令時，`azds show-context`將顯示主機尾碼。
1. 按一下右上方的 [儲存]****，然後按一下 [確定]****。
1. 按一下 [+ 發行]**** ([儲存] 按鈕旁邊)，然後按一下 [建立發行]****。
1. 在 **"專案"** 下，從生成管道中驗證最新生成被選中。
1. 按一下 **[建立]**。

自動化發行處理程序將立即開始，將 *mywebapi* 和 *webfrontend* 圖表部署至 _dev_ 最上層空間中的 Kubernetes 叢集。 您可以在 Azure DevOps Web 門戶上監視發佈進度：
...
> [!TIP]
> 如果您的發行失敗，並顯示類似 *UPGRADE FAILED: timed out waiting for the condition* 的錯誤訊息，請嘗試[使用 Kubernetes 儀表板](../../aks/kubernetes-dashboard.md)檢查叢集中的 Pod。 如果看到 pod 未能從錯誤訊息開始，例如*無法拉取圖像"azdsexample.azurecr.io/mywebapi:122"：rpc 錯誤：代碼 = 未知 desc = 守護進程的錯誤https://azdsexample.azurecr.io/v2/mywebapi/manifests/122:回應：獲得未經授權的身份驗證：需要身份驗證*，可能是因為群集尚未獲得從 Azure 容器註冊表中提取的授權。 請確定您已完成[授權您的 AKS 叢集從 Azure Container Registry 提取](../../aks/cluster-container-registry-integration.md)的先決條件。
...
部署之後，可以使用公用 URL 存取 _dev_ 版本的 *webfrontend*，例如：`http://dev.webfrontend.fedcba098.eus.azds.io`。 您可以通過運行`azds list-uri`命令找到此 URL： 
...
CI/CD 管道的_prod_階段使用負載等化器而不是開發人員空間入口控制器來提供對_prod_服務的訪問。 在_prod_階段部署的服務可作為 IP 位址而不是 DNS 名稱訪問。 在生產環境中，您可以選擇創建自己的入口控制器，以便根據自己的 DNS 配置託管服務。

要確定 Webfrontend 服務的 IP，請按一下 **"列印 Webfrontend 公共 IP"** 步驟以擴展日誌輸出。 使用日誌輸出中顯示的 IP 訪問**Webfrontend**應用程式。
