...
Kubernetes 初始化運算式無法對新的 Pod 套用 PodSpec，因為叢集中「叢集管理員」角色有 RBAC 權限上的變更。 新的 Pod 可能也會有不正確 PodSpec，例如與 Pod 相關聯的服務帳戶已不存在。 若要查看因初始化運算式問題而處於「擱置」狀態的 Pod，請使用 `kubectl get pods` 命令：
