...
> [!Note]
> 除了公用 URL，您也可以使用主控台輸出中顯示的替代 `http://localhost:<portnumber>` URL。 如果您使用 localhost URL，容器可能看起來像在本機執行，但實際是在 AKS 中執行。 Azure Dev Spaces 會使用 Kubernetes 的「連接埠轉送功能將 localhost 連接埠對應至 AKS 中執行的容器。 這可促使您從本機電腦與服務互動。
...
![顯示編輯程式碼、重新整理容器、參閱更新週期的圖表。](media/common/edit-refresh-see.png)
...
![顯示 VS Code 命令託盤中 Azure Dev Spaces 選項的螢幕擷取畫面](media/common/command-palette.png)
...
![VS Code 偵錯功能的螢幕擷取畫面，並以紅色框選 [啟動 Java 程式] 選項。](media/get-started-java/debug-configuration.png)
...
![VS Code 狀態列呈現橙色之後的螢幕擷取畫面。](media/common/vscode-status-bar-url.png)
...
![VS Code 偵錯動作窗格的螢幕擷取畫面，並以紅色框選 [重新啟動] 選項。](media/common/debug-action-refresh.png)
