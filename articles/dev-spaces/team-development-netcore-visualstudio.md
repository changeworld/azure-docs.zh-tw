...
* 其後果就是，執行任何類型的整合測試都變得非常困難。 整合測試實際上只能在認可後發生，這表示您稍後會在開發週期中看到問題。
    ...
    ![說明應用程式服務與其相依性之間關聯性的影像，顯示整合測試的複雜性。](media/common/microservices-challenges.png)
...
1. 在 [空間] 下拉式清單中，選取 **\<Create New Space…\>** 。
1. 確定父空間設定為 **\<none\>** ，然後輸入空間名稱 **dev**。 按一下 [確定]。
...
5. 在 [空間] 下拉式清單中，選取 **\<Create New Space…\>** 。

    ![螢幕擷取畫面，顯示在 Visual Studio 方案總管的「偵錯專案屬性」頁面的空間下拉式清單中，選取「建立新空間」。](media/get-started-netcore-visualstudio/Settings.png)

6. 在 [新增空間] 對話方塊中，將父空間設定為 **dev**，並輸入新空間的名稱。 您可以為新空間使用您的名稱 (例如 "scott")，讓您的同事知道您正在使用哪個空間。 按一下 [確定]。

    ![顯示 [新增空間] 對話方塊的螢幕擷取畫面，用來加入小組開發的新開發人員空間。](media/get-started-netcore-visualstudio/AddSpace.png)

7. 您現在應該會看到專案屬性頁面上選取了您的 AKS 叢集和新的空間。

    ![螢幕擷取畫面，顯示在 Visual Studio 方案總管的「偵錯專案屬性」頁面中，選取了 AKS 叢集 "MyAKS" 和空間 "scott"。](media/get-started-netcore-visualstudio/Settings2.png)
...
![圖表，顯示路徑名稱的差異，以及透過新建立的 "dev/scott" 空間和預設 "dev" 空間的要求路由。](media/common/Space-Routing.png)
