...
本文示範如何使用 Microsoft Azure 和 Visual Studio 建置混合式雲端應用程式。 您可以建立在雲端中使用多個 Azure 資源的應用程式。 本教學課程可協助您瞭解：
...
商務解決方案通常是由自訂程式碼和現有功能的組合所組成。 自訂程式碼 esposito 著手處理新的和獨特的商務需求。 已準備就緒的解決方案和系統會提供現有的功能。

眾多方案架構爭相開始使用雲端，以期能夠更輕鬆地處理擴充需求並降低操作成本。 在這麼做的情況下，他們發現想要使用的現有服務資產做為其解決方案的建立區塊，是在公司防火牆內，雲端解決方案並不容易觸及。 許多內部服務都不是以可在公司網路邊緣輕鬆公開或裝載的方式來建立或託管。

[Azure 轉送](https://azure.microsoft.com/services/service-bus/) 會採用現有的 WCF web 服務，並使那些服務安全地存取公司周邊以外的解決方案，而不需要對公司網路基礎結構進行侵入式變更。 此類轉送服務仍然裝載在其現有環境，但它們會將接聽連入工作階段和要求的工作委派給雲端裝載的轉送服務。 Azure 轉送也會使用 [共用存取簽章 (SAS) ](../service-bus-messaging/service-bus-sas.md) 驗證，來保護這些服務免于未經授權的存取。
...
1. 在 [ **.net** ] 欄中，選擇您要使用的 [Visual Studio](https://www.visualstudio.com) 版本。 本教學課程使用 Visual Studio 2019。
1. 當系統提示您執行或儲存安裝程式時，請選取 [ **執行**]。
1. 在 [ **Web Platform Installer** ] 對話方塊中，選取 [ **安裝** ] 並繼續進行安裝。

安裝完成之後，您就擁有開始開發應用程式所需的一切。 SDK 包含可讓您在 Visual Studio 輕易開發 Azure 應用程式的工具。
...
1. 在 [**建立新專案**] 中，選取 [**主控台應用程式 ( .NET Framework) ** for c #，然後選取 **[下一步]**
...
1. 新增產品連絡人的新類別。 在**方案總管**中，以滑鼠右鍵按一下 **>productsserver**專案，然後選取 [**加入**  >  **類別**]。
...
1. 在 *Program.cs*中，將命名空間定義取代為下列程式碼，此程式碼會為其新增設定檔服務和主機。
...
1. 在 [方案總管] 中按兩下 **App.config**，以在 Visual Studio 編輯器中開啟該檔案。 在元素的底部 `<system.ServiceModel>` ，但仍在中 `<system.ServiceModel>` ，加入下列 XML 程式碼。 請務必以 `yourServiceNamespace` 您的命名空間名稱取代，並 `yourKey` 使用您稍早從入口網站中取出的 SAS 金鑰取代：
    ...
    > [!NOTE]
    > 造成的錯誤 `transportClientEndpointBehavior` 只是警告，而不是此範例的封鎖問題。

1. 仍在 *App.config*的元素中 `<appSettings>` ，以您先前從入口網站取得的連接字串取代連接字串值。
...
1. 選取 Ctrl + Shift + B 或選取 [**組建**  >  **組建方案**]，以建立應用程式，並確認工作到目前為止的精確度。
...
1. 在 Visual Studio 中，選取 [建立新專案]。
1. 在 [**建立新專案**] 中，選取 [ **ASP.NET Web 應用程式 ( .NET Framework**的 c #) ，然後選取 **[下一步]**
1. 將專案命名為 *>productsportal* ，然後選取 [ **建立**]。
1. 在 [**建立新的 ASP.NET Web 應用程式**] 中，**選擇 [** **MVC** ]，然後選取 [**驗證**]。

   ![選取 ASP.NET Web 應用程式][16]

1. 在 [ **變更驗證**] 中，選擇 [ **無驗證** ]，然後選取 **[確定]**。 在本教學課程中，您要部署的應用程式不需要使用者登入。

    ![指定驗證][18]

1. 回到 [ **建立新的 ASP.NET Web 應用程式**]，選取 [ **建立** ] 以建立 MVC 應用程式。
1. 針對新的 web 應用程式設定 Azure 資源。 請依照 [發佈 web 應用程式](../app-service/quickstart-dotnet-framework.md#launch-the-publish-wizard)中的步驟進行。 然後，回到本教學課程並繼續進行下一個步驟。
...
1. 在 **方案總管**中，展開 [ **控制器**]，然後按兩下 [ **HomeController.cs** ]，以 Visual Studio 開啟該檔案。
...
1. 在**方案總管**中， **Views**展開  >  [**共用**的視圖]，然後按兩下 [ **_Layout** ]，在 Visual Studio 編輯器中開啟檔案。
1. 將所有出現的變更 `My ASP.NET Application` 為 *Northwind 商貿產品*。
1. 移除 `Home` 、 `About` 和 `Contact` 連結。 在下列範例中，刪除反白顯示的程式碼。

    ![刪除產生的清單項目][41]

1. 在**方案總管**中，展開 [ **Views**  >  **Home**]，然後按兩下 [ **Index** ]，在 Visual Studio 編輯器中開啟檔案。 將檔案的整個內容取代為下列程式碼：
...
1. 若要確認您的工作到目前為止的精確度，您可以選取 Ctrl + Shift + B 以建立專案。

### <a name="run-the-app-locally"></a>在本機執行應用程式

執行應用程式以驗證它是否能正常運作。

1. 確定 **ProductsPortal** 為作用中專案。 在 **方案總管** 中，以滑鼠右鍵按一下專案名稱，然後選取 [ **設定為啟始專案**]。
...
1. 在**方案總管**中，以滑鼠右鍵按一下 **>productsportal**專案，然後選取 [**加入**  >  **現有專案**]。
1. 從 *ProductsServer* 主控台專案導覽至 **ProductsContract.cs** 檔案。 醒目提示 *ProductsContract.cs*。 選取 [ **新增**] 旁邊的向下箭號，然後選擇 [ **新增為連結**]。

   ![新增為連結][24]

1. 現在會在 Visual Studio 編輯器中開啟 *HomeController.cs* 檔案，並將命名空間定義取代為下列程式碼。 請務必以 `yourServiceNamespace` 您的服務命名空間名稱取代，並以 `yourKey` 您的 SAS 金鑰取代。 此程式碼可讓用戶端呼叫內部部署服務，並傳回呼叫的結果。
...
1. 在 **方案總管**中，以滑鼠右鍵按一下 **>productsportal** 方案。 請務必以滑鼠右鍵按一下方案，而不是專案。 選取 [**加入**  >  **現有專案**]。
1. 導覽至 **ProductsServer** 專案，然後按兩下 *ProductsServer.csproj* 方案檔來新增它。
1. **>productsserver** 必須正在執行，才能在 **>productsportal**上顯示資料。 在 **方案總管**中，以滑鼠右鍵按一下 **>productsportal** 方案，然後選取 [ **屬性** ] 以顯示 **屬性頁**。
1. 選取 [**通用屬性**  >  **啟始專案**]，然後選擇 [**多個啟始專案**]。 確定 **>productsserver** 和 **>productsportal** 會以該順序顯示，且兩者的 **動作** 都是 [ **開始**]。
...
1. 針對 **專案**，選擇 [ **>productsserver**]。 確認未選取 [ **>productsportal** ]，然後選取 **[確定]** 以儲存您的變更。
...
1. 在 **方案總管**中，以滑鼠右鍵按一下 **>productsportal** 專案，然後選取 [ **發行**]。 在 [發佈] 頁面中，選取 [發佈]。

   > [!NOTE]
   > 在部署後自動啟動 **ProductsPortal** Web 專案時，您可在瀏覽器視窗中看到錯誤訊息。 這是預期的，且因為 **ProductsServer** 應用程式尚未執行而出現。
   >

1. 複製已部署之 web 應用程式的 URL。 您稍後將需要 URL。 您也可以從 Visual Studio 中的 [ **Azure App Service 活動** ] 視窗取得此 URL：

   ![已部署應用程式的 URL][9]

1. 關閉瀏覽器視窗以停止執行中的應用程式。

<a name="set-productsportal-as-web-app"></a>在雲端中執行應用程式之前，您必須確定 **>productsportal** 是從 Visual Studio 內以 web 應用程式的形式啟動。

1. 在 Visual Studio 中，以滑鼠右鍵按一下 **>productsportal** 專案，然後選取 [ **屬性**]。
1. 選取 [Web]。 在 [ **起始動作**] 底下，選擇 [ **開始 URL**]。 輸入您先前部署的 web 應用程式的 URL，在此範例中為 `https://productsportal20190906122808.azurewebsites.net/` 。
...
選取 F5 以建立並執行應用程式。 內部部署伺服器（即 **>productsserver** 主控台應用程式）應該會先啟動，然後 **>productsportal** 應用程式應該會在瀏覽器視窗中啟動，如下所示：
...
產品清查會列出從產品服務內部部署系統中取出的資料，並在 web 應用程式中顯示該資料。 檢查 URL，確定 **ProductsPortal** 正在雲端中以 Azure Web 應用程式的形式執行。
   ...
   > [!IMPORTANT]
   > **ProductsServer** 主控台應用程式必須正在執行中，而且能夠提供資料給 **ProductsPortal** 應用程式。 如果瀏覽器顯示錯誤，請等候幾秒鐘，讓 **>productsserver** 載入並顯示下列訊息，然後重新整理瀏覽器。
   >
