...
混合式連線可讓您在兩個網路應用程式之間進行雙向、要求-回應和二進位串流通訊，以及簡單的資料包流程。 任一個或兩個雙方都可以位於 Nat 或防火牆後方。
...
互動模型仰賴其他許多網路 API 所建立的專門用語。 有一個接聽程式會先指出是否已準備好處理連入連線，並在到達時接受這些連接。 而在另一端，用戶端會連線到接聽程式，並預期該連線會被接受，以便能夠建立雙向通訊路徑。 您可在大部分通訊端 API 中發現「連線」、「接聽」、「接受」等同義詞彙。

任何轉送的通訊模型中有任一方對服務端點進行輸出連線。 這會使「接聽程式」也成為口語使用中的「用戶端」，也可能導致其他術語多載。 因此，用於混合式連線的精確術語如下︰

連接兩端的程式稱為「用戶端」，因為它們是用戶端對服務的用戶端。 等候並接受連線的用戶端為「接聽程式」，或稱為擔任「接聽程式角色」。 透過服務對接聽程式起始新連線的用戶端則稱為「傳送者」或擔任「傳送者角色」。
...
服務接受 WebSocket 時，註冊便已完成，所建立的 WebSocket 會保持運作，以作為用來啟用所有後續互動的「控制通道」。 針對一個混合式連線，服務最多允許25個並行接聽程式。 AppHooks 的配額有待決定。
...
編碼資訊的有效期間很短，基本上就是傳送者願意等待兩端建立連線的時間。 採用的上限為 30 秒。 URL 只能用於進行一次成功的連線嘗試。 一旦建立了具有會合 URL 的 WebSocket 連線，此 WebSocket 上的所有後續活動就會由傳送者來回轉送。 此行為會發生，而不會因服務而介入或轉譯。

### <a name="request-message"></a>要求訊息

除了 WebSocket 連線，接聽程式也可以從傳送者接收 HTTP 要求框架 (如果已在混合式連線上明確啟用這項功能)。

透過 HTTP 支援連結至混合式連線的接聽程式必須處理 `request` 舉動。 在連接時，服務可能會封鎖未處理的接聽程式， `request` 因而導致重複的逾時錯誤。

HTTP 框架標題中繼資料會轉譯為 JSON，以便接聽程式架構進行更簡單的處理，也是因為 HTTP 標題剖析程式庫比 JSON 剖析器更少見。 不會轉送與傳送者和轉送 HTTP 閘道之間的關聯性相關的 HTTP 中繼資料（包括授權資訊）。 HTTP 要求主體會以二進位 WebSocket 框架的形式透明傳輸。
...
對於要求，服務會決定是否要透過控制通道路由傳送要求。 這包括但不限於要求超過 64 kB (標頭和本文) 的情況，或者，如果要求是以「 [區塊」傳輸編碼](https://tools.ietf.org/html/rfc7230#section-4.1) 傳送，且服務有理由要求超過 64 kB，或讀取要求的原因不是瞬間。 如果服務選擇透過會合通道傳遞要求，它只會將會合位址傳遞給接聽程式。
...
* **識別碼** – 此連線的唯一識別碼。 如果識別碼是由傳送者用戶端所提供，則為傳送者提供的值，否則為系統產生的值。
...
此參數可讓寄件者用戶端在不可能包含 HTTP 標頭時，將分派引數傳遞至接受的接聽程式。 預期為接聽程式架構會剖析固定的路徑部分和路徑中的已註冊名稱，並提供其餘部分，可能是不含前置詞為 `sb-` 的任何查詢字串引數，可供應用程式決定是否要接受連線。
...
如果沒有主體，則會省略主體框架。 布林值 `body` 屬性工作表示要求訊息中是否存在主體。
...
* **requestTarget** – 字串。 此屬性會保留要求的 ["Request Target" (RFC7230 的 5.3 節)](https://tools.ietf.org/html/rfc7230#section-5.3)。 它包含查詢字串部分，這會移除所有前置 `sb-hc-` 參數。
...
接收者「必須」回應。 在維護連接時重複回應要求的失敗，可能會導致接聽程式遭到封鎖。
...
* **token** –適用于命名空間的有效、URL 編碼的服務匯流排共用存取權杖，或授 **接聽** 許可權的混合式連接。
...
要求會包含任意的額外 HTTP 標題，包括應用程式所定義的標題。 所有提供的標頭，除了直接在 RFC7230 中定義的標頭以外 (查看 [要求訊息](#request-message)) 流向接聽程式，而且可以在 `requestHeader` **要求** 訊息的物件上找到。
...
如果發生錯誤，服務可以回復，如下所示。 不論是來自服務或來自接聽程式的回應，都可以透過 `Via` 標題加以識別。 如果標題存在，則回應是來自接聽程式。
...
| 504  | 閘道逾時 | 要求已路由傳送至接聽程式，但是接聽程式未在需要的時間內確認回條。
