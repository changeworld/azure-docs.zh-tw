...
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| choose    | 根元素。                                                                                                                                                                                                                                                             | 是      |
| when      | 要用於 `if` 原則之 `ifelse` 或 `choose` 組件的條件。 如果 `choose` 原則有多個 `when` 區段，則會依序評估這些區段。 一旦 when 元素的 `condition` 評估為 `true` 後，就不會再評估後面的 `when` 條件。 | 是      |
...
| max-count | {1}整數。{2} 指定允許輸入原則的要求數目上限。           | 是      | N/A     |
