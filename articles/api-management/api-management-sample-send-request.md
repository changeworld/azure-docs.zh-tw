...
或者，如果授權伺服器不包含"活動"欄位以指示權杖是否有效，請使用 Postman 等工具確定在有效權杖中設置的屬性。 例如，如果有效的權杖回應包含名為"expires_in"的屬性，請選中此屬性名稱是否存在於授權伺服器回應中，這樣：

<條件="（（（IResponse））上下文"。變數["權杖狀態"*）。Body.As（）。<JObject>屬性（"expires_in"） = 空）">
