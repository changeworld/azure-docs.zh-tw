...
若要從影響裝載您「API 管理」服務的區域可用性問題復原，請隨時準備好在另一個區域中重新建構您的服務。 根據您的恢復時間目標，您可能希望將備用服務保留在一個或多個區域中。 您還可以嘗試根據復原點目標保持其配置和內容與活動服務同步。 服務備份和還原功能為實施災害復原策略提供了必要的構建基塊。

備份和還原操作還可用於在操作環境（例如開發和暫存）之間複製 API 管理服務配置。 請注意，也會複製運行時資料（如使用者和訂閱），這可能並不總是可取的。

本指南演示如何自動執行備份和還原操作，以及如何確保 Azure 資源管理器成功驗證備份和還原請求。

> [!IMPORTANT]
> 還原操作不會更改目標服務的自訂主機名稱配置。 我們建議對活動服務和備用服務使用相同的自訂主機名稱和 TLS 證書，以便在還原操作完成後，可以通過簡單的 DNS CNAME 更改將流量重定向到備用實例。
>
> 備份操作不捕獲 Azure 門戶中的 Analytics（分析）邊欄選項卡中顯示的報表中使用的預聚合日誌資料。
...
在呼叫產生備份並將其還原的 API 之前，您必須先取得權杖。 下面的示例使用[Microsoft.標識模型.用戶端.ActiveDirectory](https://www.nuget.org/packages/Microsoft.IdentityModel.Clients.ActiveDirectory) NuGet 包來檢索權杖。
...
1. 使用您所建立 Azure Active Directory 應用程式的租用戶識別碼來取代 `{tenant id}`。 您可以通過按一下**應用註冊** -> **終結點**來訪問 ID。
...
-   `api-version`- 替換為`2018-06-01-preview`
...
-   **如果**Azure 存儲帳戶已啟用[防火牆][azure-storage-ip-firewall]，則允許從控制平面訪問 Azure 存儲帳戶。 客戶應在其存儲帳戶上打開[Azure API 管理控制平面 IP 位址][control-plane-ip-address]集，以便備份到或還原。 

> [!NOTE]
> 如果嘗試使用啟用[防火牆][azure-storage-ip-firewall]的存儲帳戶在同一 Azure 區域中執行從 /從 /還原 API 管理服務的備份/還原，則此操作將不起作用。 這是因為對 Azure 存儲的請求不是從計算>（Azure Api 管理控制平面）對公共 IP 的 SNATed。 跨區域存儲請求將進行 SNATed。
