...
委派允許您使用現有網站來處理開發人員登錄/註冊和訂閱產品,而不是在開發人員門戶中使用內置功能。 它使您的網站能夠擁有用戶數據,並自定義執行這些步驟的驗證。

[!INCLUDE [premium-dev-standard-basic.md](../../includes/api-management-availability-premium-dev-standard-basic.md)]

## <a name="delegating-developer-sign-in-and-sign-up"></a><a name="delegate-signin-up"> </a>委派開發人員登入和註冊

要委派開發人員、登錄並註冊現有網站,您需要在網站上創建一個特殊的委派終結點。 它需要充當從 API 管理開發人員門戶啟動的任何此類請求的入口點。

最終工作流程如下所示：

1. 開發人員按下 API 管理開發人員門戶的登入或註冊連結
2. 瀏覽器重新導向至委派端點
3. 委派終結點,返回重定向到或顯示 UI,要求使用者登錄或註冊
4. 成功時，將使用者重新導向回到他們所來自的 API 管理開發人員入口網站頁面

首先，請設定 API 管理透過委派端點來傳遞要求。 在 Azure 門戶中,在 API 管理資源中搜索**安全性**,然後單擊**委派**項。 單擊該複選框可啟用「代理登錄&註冊」。
...
1. 接收下列形式的要求：
   ...
   > *http:\//www.yourwebsite.com/apimdelegation?operation=SignIn&傳回Url_源頁 URL[&salt_字串[&sig_string]*
   > 
   > 
...
3. 驗證您是否收到登入/註冊請求:**操作**查詢參數將設置為「**登錄**」 。
4. 向使用者出示要登入或註冊的 UI
5. 如果使用者要註冊，您必須在 API 管理中為他們建立對應的帳戶。 使用 API Management REST API [建立使用者]。 執行此操作時,請確保將使用者 ID 設定為與使用者儲存中的值或可以追蹤的 ID 相同的值。
6. 成功驗證使用者之後：
...
委派產品訂閱的工作方式類似於委派用戶登錄/上線。 最終工作流程如下所示：

1. 開發人員在 API 管理開發人員門戶中選擇產品,然後單擊「訂閱」按鈕。
2. 瀏覽器被重定向到委派終結點。
...
1. 接收下列形式的要求：
   ...
   > *http:\//www.yourweb.com/apim 委派?操作{操作{操作{&产品 Id_產品訂閱[&用户 Id_使用者 Id_&salt_&s_ssig_string}*
   >
   ...
   * **訂閱代碼**:取消*訂閱*與*續訂*- 產品訂閱的 ID
   * **使用者代碼**:為請求發出要求的使用者的識別碼
   * **salt**：特殊 salt 字串，用於計算安全性雜湊
   * **sig**：已經過計算的安全性雜湊，用於和您已計算的雜湊進行比較
...
## <a name="example-code"></a><a name="delegate-example-code"> </a>範例代碼

這些程式碼範例簡報如何:

* 取得在發佈者門戶的委派螢幕中設置的*委派驗證金鑰*
