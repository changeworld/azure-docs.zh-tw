---
title: 使用 OAuth 2.0 與 AAD 與 API 管理一起保護 API
...
4. 將開發人員主控台配置為使用 OAuth 2.0 使用者授權呼叫 API。
...
1. 轉到[Azure 門戶](https://portal.azure.com)以註冊應用程式。 搜尋並選擇**APP 註冊**。

1. 選取 [新增註冊]****。 

1. 當 **「註冊應用程式」頁**出現時,輸入應用程式的註冊資訊: 
    - 在 **「名稱」** 部份中,輸入一個有意義的應用程式名稱,該名稱將顯示給應用的使用者,如*後端應用*。 
    - 在**支援帳號類型「 部份**中,選擇適合您的方案的選項。 
...
1. 選擇 **「公開 API」 然後**使用預設值設定**應用程式 ID URI。** 記錄此值以便以後。

1. 選擇「**新增範圍**」按鈕以顯示「**新增範圍」** 頁。 然後創建一個由 API 支援的新作用域(`Files.Read`例如, 最後,選擇 **「添加範圍**」按鈕以創建作用域。 重複此步驟以添加 API 支援的所有作用域。

1. 創建作用域時,記下它們,以便隨後的步驟使用。 

## <a name="register-another-application-in-azure-ad-to-represent-a-client-application"></a>在 Azure AD 中註冊另一個應用程式來代表用戶端應用程式

每個呼叫 API 的用戶端應用程式也必須在 Azure AD 中註冊為應用程式。 在此示例中,用戶端應用程式是 API 管理開發人員門戶中的開發人員主控台。 以下說明如何在 Azure AD 中註冊另一個應用程式，來代表開發人員主控台。
...
1. 當 **「註冊應用程式」頁**出現時,輸入應用程式的註冊資訊: 
    - 在 **「名稱」** 部份中,輸入一個有意義的應用程式名稱,該名稱將顯示給應用程式的使用者,如*用戶端應用*。 
    - 在**支援帳戶類型「** 部份中,選擇**任何組織目錄中的帳戶(任何 Azure AD 目錄 - 多租戶)。** 

1. 在 **「重定向 URI」** 部份中`Web``https://contoso5.portal.azure-api.net/signin`,選擇並輸入 URL 。

1. 選取 [註冊]**** 以建立應用程式。 

1. 在應用程式 [概觀]**** 頁面上，尋找 [應用程式 (用戶端) 識別碼]**** 值並將它記下供稍後使用。

現在,創建一個用戶端機密,供此應用程式在後續步驟中使用。

1. 從用戶端應用的頁面清單中,選擇**證書&機密**,然後選擇 **「新用戶端機密**」。

1. 在 **'新增用戶端機密**' 下,提供**描述**。 選擇金鑰何時過期,然後選擇 **"添加**"。
...
1. 轉到[Azure 門戶](https://portal.azure.com)以向用戶端應用程式授予許可權。 搜尋並選擇**APP 註冊**。

1. 選擇客戶端應用。 在應用程式的頁面清單中, 選擇**API 權限**。

1. 選擇 **「添加權限**」。

1. 在 **「選擇 API」** 下,選擇 **「我的 API」,** 然後查找並選擇後端應用。

1. 在 **「委派許可權**」下,選擇要後端應用的適當許可權,然後選擇 **「添加許可權**」。。

1. 或者,在**API 許可權**頁上,選擇 **「授予\<管理員同意 」>** 代表此目錄中的所有使用者授予同意。 
...
1. 在 Azure 門戶中,流覽到 API 管理實例。

1. 選擇**OAuth 2.0** > **新增**。

1. 提供[顯示名稱]**** 和 [描述]****。

1. 針對 [用戶端註冊頁面 URL]****，輸入預留位置值，例如 `http://localhost`。 **用戶端註冊頁面 URL**指向一個頁面,使用者可以用它來為支援此的 OAuth 2.0 提供者創建和配置自己的帳戶。 在此範例中，使用者沒有建立和設定自己的帳戶，因此您可改使預留位置。
...
1. 複製 [OAuth 2.0 授權端點]**** 並貼到 [授權端點 URL]**** 文字方塊中。 在授權請求方法下選擇**POST。**

1. 複製 [OAuth 2.0 權杖端點]**** 並貼到 [權杖端點 URL]**** 文字方塊中。 

    >[!IMPORTANT]
    > 您可以使用**v1**或**v2**終結點。 但是,根據您選擇的版本,下面的步驟將有所不同。 我們建議使用 v2 終結點。 

1. 如果使用**v1**終結點,則加入名為**資源的**正文參數 。 對於此參數的值,請使用後端**應用的應用程式 ID。** 

1. 如果使用**v2**終結點,請使用在 **「預設範圍」** 欄位中為後端應用創建的範圍。 此外,請確保在[`accessTokenAcceptedVersion`](/azure/active-directory/develop/reference-app-manifest#accesstokenacceptedversion-attribute)`2`[應用程式清單](/azure/active-directory/develop/reference-app-manifest)中將屬性的值設定為 。

1. 接著，指定用戶端認證。 這些是用戶端應用程式的認證。

1. 對於**客戶端識別碼,** 請使用用戶端**應用程式 ID。**
...
1. 在 **「重定向 URI」** 下,選擇類型為**Web,** 在**重定向 URI**下粘貼**redirect_url,** 然後保存。
...
2. 選取您要保護的 API。 例如,可以使用`Echo API`。
...
現在,OAuth 2.0 用戶授權在 API 上啟用,開發人員主控台將在調用 API 之前代表使用者獲取訪問權杖。

1. 瀏覽到開發人員門戶中的 API 下的任何操作,然後選擇 **「試用它**」 。。 這將帶您前往 [開發人員主控台]。

2. 請注意 **「授權」** 部分中的新項目,該項與您剛剛添加的授權伺服器相對應。
...
此時，當使用者嘗試從開發人員主控台進行呼叫時，系統會提示使用者登入。 開發人員主控台代表使用者獲取訪問權杖,並在向 API 發出的請求中包括該權杖。

但是,如果有人在沒有權杖或使用無效權杖的情況下調用您的API,該怎麼辦? 例如,嘗試在沒有`Authorization`標頭的情況下調用 API,調用仍將通過。 這是因為 API 管理此時不會驗證存取權杖。 它只會將 `Authorization` 標頭傳遞至後端 API。

您可以使用[驗證 JWT](api-management-access-restriction-policies.md#ValidateJWT) 原則，藉由驗證每個傳入要求的存取權杖來預先授權 API 管理中的要求。 如果要求沒有有效的權杖，則 API 管理會封鎖該要求。 例如,將以下策略添加到`<inbound>`的策略`Echo API`部分。 它會檢查存取權杖中的 audience 宣告，並在權杖無效時傳回錯誤訊息。 如需如何設定原則的資訊，請參閱[設定或編輯原則](set-edit-policies.md)。
...
```
> [!NOTE]
> 此`openid-config`URL 對應於 v1 終結點。 對 v2`openid-config`的`https://login.microsoftonline.com/common/v2.0/.well-known/openid-configuration`終結點, 請使用 。
...
* [建立 API 管理服務實體 。](get-started-create-service-instance.md)
