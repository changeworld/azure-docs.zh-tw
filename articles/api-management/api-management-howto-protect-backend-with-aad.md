...
> [!NOTE]
> 這項功能適用于 API 管理的**開發人員**、**基本**、**標準**和**高階層次。**
...
1. 將開發人員主控台設定為使用 OAuth 2.0 使用者授權呼叫 API。
...
若要使用 Azure AD 來保護 API，請先在代表 API 的 Azure AD 中註冊應用程式。 
...
1. （選擇性）在 [ **API 許可權**] 頁面上，選取 [授與系統**管理員 \<your-tenant-name> 同意**，代表此目錄中的所有使用者同意]。 
...
1. 複製 [OAuth 2.0 權杖端點]**** 並貼到 [權杖端點 URL]**** 文字方塊中。 
   ...
   >[!IMPORTANT]
   > 請使用**v1**或**v2**端點。 不過，根據您選擇的版本而定，下列步驟將會不同。 我們建議使用 v2 端點。 
