...
Azure API 管理支援多區域部署，可讓 API 發行者跨任意數量的支援 Azure 區域散發單一 Azure API 管理服務。 多區域功能有助於減少地理位置分散的 API 取用者所察覺到的要求延遲，並在一個區域離線時改善服務可用性。

新的 Azure API 管理服務一開始只包含單一 Azure 區域（主要區域）中的一個 [單位][unit] 。 您可以將額外的單位新增至主要或次要區域。 API 管理閘道元件會部署到每個選取的主要和次要區域。 傳入的 API 要求會自動導向至最接近的區域。 如果區域離線，則 API 要求會自動路由至失敗的區域到下一個最接近的閘道。

> [!NOTE]
> 只有 API 管理的閘道元件會部署到所有區域。 服務管理元件和開發人員入口網站只裝載在主要區域中。 因此，當主要區域中斷時，存取開發人員入口網站和變更設定的能力 (例如新增 Api、套用原則) 會受到影響，直到主要區域恢復上線為止。 當主要區域離線可用時，次要區域會繼續使用其可用的最新設定來提供 API 流量。
...
2. 按一下頂端列中的 [ **+ 新增** ]。
3. 從下拉式清單中選取位置，並使用滑杆設定單位數目。
4. 按一下 [ **新增** ] 按鈕以確認。
5. 重複此程式，直到您設定所有位置為止。
6. 按一下頂端列中的 [ **儲存** ] 以開始部署程式。
...
> [!TIP]
> 您也可以使用 [Azure 流量管理員](https://azure.microsoft.com/services/traffic-manager/)來前端服務，將 API 呼叫導向至流量管理員，讓它自動解析路由。

## <a name="use-custom-routing-to-api-management-regional-gateways"></a><a name="custom-routing"> </a>使用自訂路由傳送至 API 管理區域閘道

API 管理會根據[最低延遲](../traffic-manager/traffic-manager-routing-methods.md#performance)將要求路由傳送至區域_閘道_。 雖然無法在 API 管理中覆寫此設定，但您可以使用自己的流量管理員搭配自訂路由規則。

1. 建立您自己的 [Azure 流量管理員](https://azure.microsoft.com/services/traffic-manager/)。
1. 如果您使用自訂網域，請 [將它與流量管理員搭配使用](../traffic-manager/traffic-manager-point-internet-domain.md) ，而不是使用 API 管理服務。
...
1. [在流量管理員中設定 API 管理區域狀態端點](../traffic-manager/traffic-manager-monitoring.md)。 例如，區域狀態端點會遵循的 URL 模式 `https://<service-name>-<region>-01.regional.azure-api.net/status-0123456789abcdef` `https://contoso-westus2-01.regional.azure-api.net/status-0123456789abcdef` 。
