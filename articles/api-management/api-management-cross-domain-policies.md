...
## <a name="allow-cross-domain-calls"></a><a name="AllowCrossDomainCalls"></a>允許跨域調用
...
|allowed-methods|如果允許 GET 或 POST 以外的方法，則需要此元素。 包含指定了所支援 HTTP 動詞命令的 `method` 元素。 該值`*`指示所有方法。|否|如果這個區段不存在，則會支援 GET 和 POST。|
...
|名稱|描述|必要|預設|
|----------|-----------------|--------------|-------------|
|allow-credentials|預`Access-Control-Allow-Credentials`檢回應中的標頭將設置為此屬性的值，並影響用戶端在跨域請求中提交憑據的能力。|否|false|
|preflight-result-max-age|預`Access-Control-Max-Age`檢回應中的標頭將設置為此屬性的值，並影響使用者代理緩存飛行前回應的能力。|否|0|
