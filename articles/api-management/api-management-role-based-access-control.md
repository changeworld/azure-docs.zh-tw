...
Azure API 管理依賴 Azure 角色型存取控制（Azure RBAC）來為 API 管理服務和實體（例如 Api 和原則）啟用更細緻的存取管理。 本文章將為您提供 API 管理中內建角色和自訂角色的概觀。 如需有關 Azure 入口網站中的存取管理的詳細資訊，請參閱[開始在 Azure 入口網站中使用存取管理](../role-based-access-control/overview.md)。
