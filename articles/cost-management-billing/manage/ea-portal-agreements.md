...
加成可讓合作夥伴系統管理員將百分比加成新增至其間接的 Enterprise 合約。 百分比加成適用於 Azure EA 入口網站中的所有 Microsoft 第一方服務資訊，例如：計量費率、Azure 預付款和訂單。 當合作夥伴發佈加成之後，客戶會在 Azure EA 入口網站中看到 Azure 成本。 例如，使用量摘要、價格清單和已下載的使用量報表。
...
如果您在 EA 中的所有商業交易上設定相同的加成百分比 (也就是，如果您將預付金資訊、計量費率、訂單資訊等進行加成)， 也就是，如果您加成 Azure 預付款資訊、計量費率、訂單資訊等。

在下列情況下，請勿使用加成功能：
- 您在 Azure 預付款與計量費率之間使用的費率不同。
- 您對不同的計量使用不同的費率。
...
## <a name="resource-prepayment-and-requesting-quota-increases"></a>資源預付款和要求增加配額
...
## <a name="resource-prepayment"></a>資源預付款

Microsoft 將提供服務，最多至少會是您購買每月預付款中所含的相關聯使用量程度 (服務預付款)，但是服務資源使用程度的所有其他增加數量 (例如增加執行中的計算執行個體數目，或提高使用中的儲存體數量) 取決於這些服務資源的可用性。

以上所述的任何配額都不是服務預付款。 為了確認 Microsoft 作為服務預付款一部分所提供的同步小型計算執行個體數量 (或其相等值)，此數量可由承諾在一個月內購買的小型計算執行個體時數除以一年最短月份的時數 (例如二月 – 672 個小時) 決定。
