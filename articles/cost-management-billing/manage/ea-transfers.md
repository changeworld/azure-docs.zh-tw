...
本節僅供資訊參考之用，因為企業管理員無法執行動作。 需要支援要求才能將企業註冊轉移至新的註冊，除非註冊符合 [自動註冊轉移](#auto-enrollment-transfer)的資格。

當您要求將整個企業註冊轉移至另一個註冊時，會發生下列動作：

- 所有 Azure 服務、訂用帳戶、帳戶、部門和整個註冊結構 (包括所有 EA 部門系統管理員) 都會轉移至新的目標註冊。
- 註冊狀態會設定為 [已轉移]。 已轉移的註冊僅限用於報告使用量記錄。
- 您無法將角色或訂用帳戶新增至已轉移的註冊。 已轉移狀態可防止更多使用註冊。
- 合約中任何剩餘的 Azure 預付款餘額都會遺失，其中也包括未來的期限。
-    如果您要轉移的註冊有 RI 購買，RI 購買費用將會保留在來源註冊中，但所有 RI 權益都將轉移至不同的範圍，以供新註冊使用。
-    在舊的註冊上，所有已產生的 marketplace 一次性採購費用和任何每月固定費用都不會轉移到新的註冊。 依使用量的 Marketplace 費用將會轉移。

### <a name="effective-transfer-date"></a>有效的轉移日期

有效的轉移日期可以是目標註冊的開始日期或之後的日期。

來源註冊使用量會依 Azure 預付款或超額部分收費。 有效轉移日期之後發生的使用量會轉移至新的註冊並收費。
...
- 針對註冊轉移有效日期，可以是目標註冊開始日期或其之後的日期。 選擇的日期不會影響已發出之超額發票的使用量。
...
### <a name="auto-enrollment-transfer"></a>自動註冊轉移

您可能會看到註冊有已 **轉移** 的狀態，即使您尚未提交支援票證來要求註冊轉移。 自動註冊傳輸程式的已 **傳輸** 狀態結果。 為了在續約片語期間進行自動註冊轉移程式，新合約中必須包含幾個專案：

- 先前的註冊號碼 (必須存在於 EA 入口網站中) 
- 先前註冊號碼的到期日是新合約的有效開始日期之前的一天
- 新合約的 Azure 預付款訂單有目前的日期或回溯日期
- 新的註冊會在 EA 入口網站中建立

如果先前的註冊與新註冊之間的 EA 入口網站中沒有任何遺失的使用量資料，則您不需要建立轉移支援票證。
