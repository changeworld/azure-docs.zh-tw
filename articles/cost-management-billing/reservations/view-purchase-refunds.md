...
若要深入了解，請參閱[適用於 Enterprise 合約的 Azure 成本管理 Power BI 應用程式](../costs/analyze-cost-data-azure-cost-management-power-bi-template-app.md)。

## <a name="use-apis-to-get-reservation-transactions"></a>使用 API 來取得保留交易

Enterprise 合約 (EA) 和 Microsoft 客戶合約使用者可以使用[保留交易 - 列出 API](/rest/api/consumption/reservationtransactions/list) 來取得保留交易資料。
