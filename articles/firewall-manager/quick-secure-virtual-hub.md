...
在本快速入門中，您可以使用 Resource Manager 範本，利用 Azure 防火牆管理員預覽版來保護您的虛擬中樞。

已部署的防火牆具有允許連線到 `www.microsoft.com` 的應用程式規則。 已部署兩部 Windows Server 2019 虛擬機器來測試防火牆。 一部跳躍伺服器用來連線到工作負載伺服器。 從工作負載伺服器，您只能連線到 `www.microsoft.com`。
...
本快速入門中使用的範本是來自 [Azure 快速入門範本](https://github.com/Azure/azure-quickstart-templates/blob/master/fwm-docs-qs/azuredeploy.json)。
