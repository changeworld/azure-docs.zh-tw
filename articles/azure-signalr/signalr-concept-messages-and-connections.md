...
應用程式伺服器會將 1 KB 的訊息廣播至所有連線的用戶端，從應用程式伺服器到服務的訊息會被視為可用的輸入訊息。 只有從服務傳送至每個用戶端的三個訊息會以輸出訊息的形式收費。

用戶端 A 會將 1 KB 的訊息傳送至另一個用戶端 B，而不需要透過應用程式伺服器。 從用戶端 A 到服務的訊息是可用的輸入訊息。 從服務到用戶端 B 的訊息會以輸出訊息的方式計費。

如果您有三個用戶端和一個應用程式伺服器。 一個用戶端傳送了一則 4 KB 的訊息，讓伺服器廣播到所有用戶端。 計費的訊息計數為8：一則訊息從服務傳送至應用程式伺服器，以及三則訊息從服務傳送至用戶端。 每則訊息分別計為兩則 2 KB 的訊息。

## <a name="how-connections-are-counted"></a>如何計算連線

有 Azure SignalR Service 的伺服器連接和用戶端連接。 根據預設，每個應用程式伺服器一開始會有五個中樞的初始連線，而且每個用戶端都有一個用戶端連線。

Azure 入口網站中顯示的連線計數包括伺服器連線和用戶端連線。

例如，假設您有兩個應用程式伺服器，並且在程式碼中定義五個中樞。 伺服器連線計數將會是50：每個中樞2個應用程式伺服器 * 5 個中樞 * 5 個連接。

ASP.NET SignalR 會以不同的方式計算伺服器連線。 除了您定義的中樞以外，它還包含一個預設中樞。 根據預設，每個應用程式伺服器都需要五個初始伺服器連接。 預設中樞的初始連接計數會與其他中樞保持一致。

服務和應用程式伺服器會持續同步線上狀態，以及調整伺服器連接，以取得更佳的效能和服務穩定性。  因此，您可能會看到伺服器連接號碼隨時變更。

## <a name="how-inboundoutbound-traffic-is-counted"></a>如何計算輸入/輸出流量

傳送至服務的訊息是輸入訊息。 從服務傳送的訊息是輸出訊息。 流量會以位元組為單位計算。

## <a name="related-resources"></a>相關資源

- [Azure 監視器中的彙總類型](../azure-monitor/platform/metrics-supported.md#microsoftsignalrservicesignalr )
