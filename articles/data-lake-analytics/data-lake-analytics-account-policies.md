...
帳戶原則可協助您控制如何使用 Azure Data Lake Analytics 帳戶的資源。 這些原則可讓您控制使用 Azure Data Lake Analytics 的成本。 例如，透過這些原則，您可以限制帳戶可同時使用的 au 數目，以避免非預期的成本尖峰。 # # 帳戶層級原則

這些原則會套用至 Data Lake Analytics 帳戶中的所有作業。 # # # Data Lake Analytics 帳戶中的 au 數目上限原則會控制您的 Data Lake Analytics 帳戶可使用的分析單位（au）總數。 根據預設，此值設定為 250。 例如，如果此值設為 250 AU，您可以有一項已指派 250 AU 的執行中作業，或 10 項各有 25 AU 的執行中作業。 其他已提交的作業會排入佇列，直到執行中的作業完成為止。 當執行中的作業完成時，會釋出 AU 以便執行已排入佇列的作業。
...
Data Lake Analytics 有兩個您可以在作業層級設定的原則： ***每項作業的 AU 限制**：使用者只能提交最多可達此 AU 數的作業。 根據預設，此限制與帳戶的 AU 上限相同。
...
* [作業層級原則 blog 文章](https://blogs.msdn.microsoft.com/azuredatalake/2017/06/08/managing-your-azure-data-lake-analytics-compute-resources-job-level-policy/)# # 後續步驟
