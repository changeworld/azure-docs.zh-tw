...
description: 瞭解如何使用帳戶原則來控制 Data Lake Analytics 帳戶的使用量，例如最大 au 和最大作業。
...
ms.locfileid: "87127701"
---
# <a name="manage-azure-data-lake-analytics-using-account-policies"></a>使用帳戶原則來管理 Azure Data Lake Analytics

帳戶原則可協助您控制如何使用 Azure Data Lake Analytics 帳戶的資源。 這些原則可讓您控制使用 Azure Data Lake Analytics 的成本。 例如，您可以使用這些原則，藉由限制帳戶可同時使用的 au 數目，來避免非預期的成本尖峰。 # # 帳戶層級原則

這些原則適用于 Data Lake Analytics 帳戶中的所有作業。 # # # Data Lake Analytics 帳戶中的 au 數目上限-原則會控制 Data Lake Analytics 帳戶可以使用的分析單位總數)  (au。 根據預設，此值設定為 250。 例如，如果此值設為 250 AU，您可以有一項已指派 250 AU 的執行中作業，或 10 項各有 25 AU 的執行中作業。 其他已提交的作業會排入佇列，直到執行中的作業完成為止。 當執行中的作業完成時，會釋出 AU 以便執行已排入佇列的作業。
...
此原則會限制可同時執行的作業數目。 根據預設，此值設定為 20。 如果 Data Lake Analytics 有 AU 可用，則會安排立即執行新作業，直到執行中的作業總數達到此原則的值為止。 當您達到可同時執行的作業數目上限時，後續作業會以優先權順序排入佇列，直到一或多個執行中的作業完成為止 (視可用的 au) 而定。
...
當您的使用者執行 U SQL 工作時，Data Lake Analytics 服務會保留所有相關檔案。 這些檔案包括 U SQL 腳本、U SQL 腳本中所參考的 DLL 檔案、已編譯的資源和統計資料。 這些檔案位於預設 Azure Data Lake 儲存體帳戶的 /system/ 資料夾中。 此原則控制這些資源在自動刪除前的儲存時間長度 (預設值是 30 天)。 您可以使用這些檔案進行偵錯，以及未來將重新執行之作業的效能微調。
...
Data Lake Analytics 有兩個您可以在作業層級設定的原則： * **每項作業的 AU 限制**：使用者只能提交最多此 AU 的作業。 根據預設，此限制與帳戶的 AU 上限相同。
...
5. 新原則會列在 [預設]**** 原則資料表的 [作業提交限制]**** 之下。 # # # 刪除或編輯現有的原則
...
* [作業層級原則的 blog 文章](https://blogs.msdn.microsoft.com/azuredatalake/2017/06/08/managing-your-azure-data-lake-analytics-compute-resources-job-level-policy/)# # 後續步驟
