...
了解如何調整並提高 Azure Data Lake Analytics (ADLA) 帳戶中的配額和限制。 瞭解這些限制將説明您瞭解 U-SQL 作業行為。 所有配額限制並非強制，因此您可以與 Azure 支援連絡來提高上限。
...
* 如果您已經有四個執行中的作業，而每個都使用 8 AU，當您送出第五個需要 8 AU 的作業時，它會在作業佇列中等到有 8 AU 可使用。
    ...
    ![Azure 資料湖分析限制和配額頁](./media/data-lake-analytics-quota-limits/adjust-quota-limits.png)

**每個作業的最大分析單位 （A） 數量：250**個，預設 32 個

這是每個作業可以在您的帳戶中分配的最大數量。 除非提交者受可提供每項作業更多 AU 的計算原則 (作業提交限制) 影響，否則系統會拒絕已指派超過此限制的作業。 此值的上限是帳戶的 AU 限制。
