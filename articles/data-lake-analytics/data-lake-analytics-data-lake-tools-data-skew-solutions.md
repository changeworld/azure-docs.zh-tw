---
title: 解決資料偏斜 - 用於視覺化工作室的 Azure 資料湖工具
...
在前面的示例中，如果只想檢查國家/地區的稅務審核工作負載，則可以通過選擇 ID 號作為金鑰來改進資料分佈。 有時候，選擇不同的分割區或散發索引鍵可以更平均地散發資料，但您必須確保此舉不會影響商務邏輯。 比方說，為了計算各州的總稅額，您可以指定_州別_做為資料分割索引鍵。 如果您仍然遇到這個問題，請嘗試使用選項 3。
...
如果您針對分割區和散發找不到適合的索引鍵，便可以嘗試使用循環配置資源散發。 循環配置資源散發會平等對待所有資料列，並將資料列隨機放到對應的貯體。 資料會平均地散發，但將會失去位置資訊，這項缺點也會減少部分作業的作業效能。 此外，如果您原本便打算針對扭曲索引鍵進行彙總，資料扭曲的問題會持續存在。 若要深入了解循環配置資源散發，請參閱[CREATE TABLE (U-SQL)：使用結構描述建立資料表](/u-sql/ddl/tables/create/managed/create-table-u-sql-creating-a-table-with-schema#dis_sch)中的＜U-SQL 資料表散發＞一節。
...
- SqlUser 定義組合器（模式=合併模式.Full）：每個輸出行可能依賴于具有相同鍵值的左、右所有輸入行。
