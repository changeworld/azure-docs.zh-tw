...
- [AKS](../aks/kubernetes-walkthrough.md)叢集，並 `kubectl` 設定 AKS 叢集[認證](/cli/azure/aks#az-aks-get-credentials)。

- [Azure Container Registry （ACR）](../container-registry/container-registry-get-started-azure-cli.md)登錄、ACR 登入伺服器名稱，以及設定為要向 ACR 登錄[進行驗證](../aks/cluster-container-registry-integration.md)的 AKS 叢集。

- 安裝並設定 Azure CLI 2.0.46 版或更新版本。 執行  `az --version` 以尋找版本。 如果您需要安裝或升級，請參閱 [安裝 Azure CLI](/cli/azure/install-azure-cli)。

- [Docker 安裝](https://docs.docker.com/install/)在您的開發系統上

- GitHub 帳戶、 [github 個人存取權杖](https://help.github.com/articles/creating-a-personal-access-token-for-the-command-line/)，以及安裝在您的開發系統上的 Git 用戶端

- 如果您提供自己的 Jenkins 實例，而不是此範例腳本方式來部署 Jenkins，則您的 Jenkins 實例必須[安裝並設定 Docker](https://docs.docker.com/install/)並[kubectl](https://kubernetes.io/docs/tasks/tools/install-kubectl/)。
...
這會提取所需的基底映像並建置應用程式容器。 接著，您可以使用[docker images](https://docs.docker.com/engine/reference/commandline/images/)命令來查看所建立的映射。 已下載或建立三個映像。 `azure-vote-front` 映像含有應用程式，並使用 `nginx-flask` 映像做為基礎映像。 `redis` 映像可用來啟動 Redis 執行個體：
...
您必須先使用[az acr list](/cli/azure/acr#az-acr-list)命令取得 acr 登入伺服器，才能將*azure 投票-前端*容器映射推送至 acr。 下列範例會取得名為 *myResourceGroup* 之資源群組中某個登錄的 ACR 登入伺服器位址：

```azurecli
az acr list --resource-group myResourceGroup --query "[].{acrLoginServer:loginServer}" --output table
```

使用[docker tag](https://docs.docker.com/engine/reference/commandline/tag/)命令，以 ACR 登入伺服器名稱和 `v1`的版本號碼來標記映射。 提供在上一個步驟中所取得您自己的 `<acrLoginServer>` 名稱：
...
接下來，使用[kubectl apply](https://kubernetes.io/docs/reference/generated/kubectl/kubectl-commands#apply)命令將應用程式部署至您的 AKS 叢集：

```console
kubectl apply -f azure-vote-all-in-one-redis.yaml
```

系統會建立一個 Kubernetes 負載平衡器服務，以將應用程式公開至網際網路。 此程序需要數分鐘的時間。 若要監視負載平衡器部署的進度，請使用[kubectl get service](https://kubernetes.io/docs/reference/generated/kubectl/kubectl-commands#get)命令搭配 `--watch` 引數。 當 *EXTERNAL-IP* 位址從 *pending* 變成一個 *IP 位址*之後，請使用 `Control + C` 來停止 kubectl 監看式流程。
...
![在 AKS 中執行的 Azure 範例投票應用程式](media/jenkins-continuous-deployment/azure-vote.png)
...
首先，使用[az ad sp 建立-rbac 命令來](/cli/azure/ad/sp#az-ad-sp-create-for-rbac)建立服務主體：
...
使用[az ACR show](/cli/azure/acr#az-acr-show)命令取得 ACR 登錄的資源識別碼，並將其儲存為變數。 提供您的資源群組名稱和 ACR 名稱：
...
![使用服務主體資訊來建立 Jenkins 認證物件](media/jenkins-continuous-deployment/acr-credentials.png)
...
1. 在 [Bindings] \(繫結\) 底下，選取 [Add] \(新增\) > [Username and password (separated)] \(使用者名稱和密碼 (分隔)\)
   - 針對 [Username Variable] \(使用者名稱變數\)`ACR_ID`**輸入**，並針對 [Password Variable] \(密碼變數\)`ACR_PASSWORD`**輸入**
     ...
     ![Jenkins 繫結](media/jenkins-continuous-deployment/bindings.png)
...
![Jenkins 測試組建](media/jenkins-continuous-deployment/test-build.png)
...
![Jenkins 組建作業所更新的 AKS 中範例 Azure 投票](media/jenkins-continuous-deployment/azure-vote-updated.png)
