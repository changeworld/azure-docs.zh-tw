---
title: Azure VMware 解決方案（AVS）-在 Azure 中使用 VM 範本建立虛擬機器
description: 說明如何使用適用于您的 AVS 私人雲端之 VMware 基礎結構上的 VM 範本，在 Azure 中建立虛擬機器
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/16/2019
ms.topic: article
...
您可以在您的 AVS 系統管理員已為您的訂用帳戶啟用的 VMware 基礎結構上，使用 VM 範本在 Azure 入口網站中建立虛擬機器。

## <a name="sign-in-to-azure"></a>登入 Azure

登入 [Azure 入口網站](https://portal.azure.com)。

## <a name="create-avs-virtual-machine"></a>建立 AVS 虛擬機器
...
4. 輸入基本資訊按 **[下一步：大小]** 。
    ...
    > [!NOTE]
    > 在 Azure 上建立的 AVS 虛擬機器需要 VM 範本。 此 VM 範本應該存在於您的私用雲端 vCenter 上。 從具有所需作業系統和設定的 vCenter UI，在私人雲端上建立虛擬機器。 使用在[VSphere Web 用戶端中將虛擬機器複製到範本](https://docs.vmware.com/en/VMware-vSphere/6.5/com.vmware.vsphere.vm_admin.doc/GUID-FE6DE4DF-FAD0-4BB0-A1FD-AFE9A40F4BFE_copy.html)中的指示，建立範本。
...
## <a name="view-list-of-avs-virtual-machines"></a>查看 AVS 虛擬機器的清單
...
3. 選取您的私用雲端建立所在的。
    ...
    ![AVS 虛擬機器的清單](media/list-cloudsimple-virtual-machines.png)

AVS 虛擬機器清單包含從 Azure 入口網站建立的虛擬機器。  在對應的 vCenter 資源集區中，于私人雲端 vCenter 上建立的虛擬機器將會顯示在清單中。  
