---
title: Azure VMware 解決方案（AVS）-設定適用于 AVS 私用雲端的 DNS
description: 說明如何設定 DNS 名稱解析，以從內部部署工作站存取 AVS 私人雲端上的 vCenter server
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/14/2019
ms.topic: article
...
若要從內部部署工作站存取 AVS 私人雲端上的 vCenter server，您必須設定 DNS 位址解析，讓 vCenter server 可由主機名稱和 IP 位址定址。

## <a name="obtain-the-ip-address-of-the-dns-server-for-your-avs-private-cloud"></a>取得您的 AVS 私人雲端的 DNS 伺服器 IP 位址

1. 登入[AVS 入口網站](access-cloudsimple-portal.md)。

2. 流覽至 [**資源**] > [AVS] [**私人**雲端]，然後選取您想要連線的 AVS 私人雲端。

3. 在 [AVS 私人雲端] [**基本資訊**] 底下的 [**摘要**] 頁面上，複製 [avs 私人雲端 DNS 伺服器 IP 位址]。
...
6. 針對您從 AVS 入口網站取得的您的 AVS 私人雲端，輸入 DNS 伺服器的 IP 位址。
...
3. 在 AVS 私人雲端中輸入 dns 網域和 DNS 伺服器的 IP 位址，然後按一下 **[確定]** 。
