---
title: Azure VMware 解決方案（AVS）-為 Oracle RAC 優化您的 AVS 私用雲端
...
---
# <a name="optimize-your-avs-private-cloud-for-installing-oracle-rac"></a>優化您的 AVS 私人雲端以安裝 Oracle RAC

您可以在您的 AVS 私用雲端環境中部署 Oracle Real Application 叢集（RAC）。 本指南說明如何部署新的叢集，並將 Oracle RAC 解決方案的 VM 優化。 完成本主題中的步驟之後，您就可以安裝和設定 Oracle RAC。
...
下列各節說明如何為 Oracle RAC 設定您的 AVS 私用雲端環境。

1. 磁片設定的最佳做法
2. 部署 AVS 私用雲端 vSphere 叢集
...
## <a name="deploy-avs-private-cloud-vsphere-cluster"></a>部署 AVS 私用雲端 vSphere 叢集

若要在您的 AVS 私人雲端上部署 vSphere 叢集，請遵循此程式：

1. 從 AVS 入口網站[建立 Avs 私用雲端](create-private-cloud.md)。 AVS 會在新建立的 AVS 私人雲端中，建立名為 ' cloudowner ' 的預設 vCenter 使用者。 如需預設的 AVS 私用雲端使用者和許可權模型的詳細資訊，請參閱[瞭解 AVS 私用雲端許可權模型](learn-private-cloud-permissions.md)。 此步驟會為您的 AVS 私人雲端建立主要管理叢集。

2. 從 AVS 入口網站，展開具有新叢集[的 AVS 私用雲端](expand-private-cloud.md)。 此叢集將用來部署 Oracle RAC。 根據所需的容錯（最少三個節點）選取節點的數目。

## <a name="set-up-networking-for-oracle-rac"></a>設定 Oracle RAC 的網路功能

1. 在您的 AVS 私人雲端中，[建立兩個 vlan](create-vlan-subnet.md)，一個用於 oracle 公用網路，一個用於 oracle 私人網路，並指派適當的子網 CIDRs。
2. 建立 Vlan 之後，請[在 AVS 私用雲端 vCenter 上建立分散式通訊埠群組](create-vlan-subnet.md#use-vlan-information-to-set-up-a-distributed-port-group-in-vsphere)。
3. 在您的管理叢集上，為 Oracle 環境設定[DHCP 和 DNS 伺服器虛擬機器](dns-dhcp-setup.md)。
4. 在安裝于 AVS 私用雲端[的 dns 伺服器上設定 dns 轉送](on-premises-dns-setup.md#create-a-conditional-forwarder)。

## <a name="set-up-vsan-storage-policies"></a>設定 vSAN 儲存原則

vSAN 原則會針對儲存在 VM 磁片上的資料，定義容許的失敗和磁片等量。 建立 VM 時，必須在 VM 磁片上套用所建立的儲存體原則。

1. 登入您的 AVS 私人雲端[的 vSphere 用戶端](https://docs.azure.cloudsimple.com/vsphere-access)。
2. 從頂端功能表中，選取 [**原則和設定檔**]。
...
1. 在 AVS 入口網站中，提升 cloudowner 使用者的[許可權](escalate-private-cloud-privileges.md)。
2. 登入您的 AVS 私人雲端[的 vSphere 用戶端](https://docs.azure.cloudsimple.com/vsphere-access)。
