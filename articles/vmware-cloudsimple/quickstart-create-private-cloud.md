...
description: 瞭解如何使用 Azure VMware 解決方案 by CloudSimple 來建立及設定私人雲端
author: sharaths-cs
ms.author: dikamath
ms.date: 08/16/2019
ms.topic: article
...
私人雲端是透過 CloudSimple 入口網站進行管理。 他們在自己的管理網域中有自己的 vCenter server。 堆疊會在專用節點上執行，並隔離裸機硬體節點。
...
10. 輸入 vSphere/vSAN 子網的 CIDR 範圍。 請確定 CIDR 範圍未與任何內部部署或其他 Azure 子網重迭 (虛擬網路) 或使用閘道子網。
    ...
    **CIDR 範圍選項：** /24、/23、/22 或/21。 /24 CIDR 範圍最多可支援26個節點，/23 個 CIDR 範圍最多可支援58個節點，而/22 和/21 個 CIDR 範圍支援64節點， (私人雲端) 的節點數目上限。  若要深入瞭解以及 Vlan 和子網，請參閱 [vlan 和子網總覽](cloudsimple-vlans-subnets.md)。
...
您可以從 Azure 入口網站存取 CloudSimple 入口網站。  CloudSimple 入口網站將會使用您的 Azure 登入認證來啟動，並使用單一 Sign-On (SSO) 。  存取 CloudSimple 入口網站時，您需要授權 **CloudSimple Service 授權** 應用程式。  如需授與許可權的詳細資訊，請參閱 [同意 CloudSimple 服務授權應用程式](access-cloudsimple-portal.md#consent-to-cloudsimple-service-authorization-application)。

1. 選取 [所有服務]。
2. 搜尋 **CloudSimple 服務**。
3. 選取您要在其上建立私人雲端的 CloudSimple 服務。
4. 從總覽中，按一下 **[移至 CloudSimple 入口網站** ] 以開啟 CloudSimple 入口網站的新瀏覽器索引標籤。  如果出現提示，請使用您的 Azure 登入認證登入。  

    ![啟動 CloudSimple 入口網站](media/launch-cloudsimple-portal.png)

## <a name="create-point-to-site-vpn"></a>建立點對站 VPN

點對站 VPN 連線是從電腦連線到私人雲端最簡單的方式。 如果您是從遠端連線到私人雲端，請使用點對站 VPN 連線。  若要快速存取您的私人雲端，請遵循下列步驟。  您可以使用 [站對站 VPN](vpn-gateway.md) 或 [Azure ExpressRoute](on-premises-connection.md)來存取內部部署網路中的 CloudSimple 區域。
...
4. 在 [ **閘道**設定] 中，指定下列設定，然後按 **[下一步]**。
    ...
    * 指定點對站閘道的用戶端子網。  當您連線時，將會從這個子網指定 DHCP 位址。

5. 針對 [ **連接/使用者**]，指定下列設定，然後按 **[下一步]**。

    * 若要自動允許所有目前及未來的使用者透過這個點對站閘道存取私人雲端，請選取 [ **自動新增所有使用者**]。 當您選取此選項時，系統會自動選取使用者清單中的所有使用者。 您可以藉由取消挑選清單中的個別使用者來覆寫自動選項。
    * 若只要選取個別使用者，請按一下 [使用者] 清單中的核取方塊。

6. [Vlan/子網] 區段可讓您指定閘道和連線的管理和使用者 Vlan/子網。

    * **自動新增選項會**設定此閘道的全域原則。 這些設定適用于目前的閘道。 您可以在 **選取** 區域中覆寫這些設定。
...
建立私人雲端之後，請建立您將在其中部署工作負載/應用程式 Vm 的 VLAN。
...
將會建立 VLAN/子網。  您現在可以使用此 VLAN 識別碼，在您的私人雲端 vCenter 上建立分散式埠群組。

## <a name="connect-your-environment-to-an-azure-virtual-network"></a>將您的環境連線至 Azure 虛擬網路

CloudSimple 為您的私人雲端提供 ExpressRoute 線路。 您可以將 Azure 上的虛擬網路連線到 ExpressRoute 線路。 如需設定連接的完整詳細資訊，請遵循 [使用 ExpressRoute 的 Azure 虛擬網路](./cloudsimple-azure-network-connection.md)連線中的步驟。

## <a name="sign-in-to-vcenter"></a>登入 vCenter

您現在可以登入 vCenter 來設定虛擬機器和原則。

1. 若要存取 vCenter，請從 CloudSimple 入口網站開始。 在首頁的 [ **一般**工作] 底下，按一下 [ **啟動 vSphere 用戶端**]。  選取私人雲端，然後按一下 [在私人雲端上 **啟動 VSphere 用戶端** ]。

    ![啟動 vSphere 用戶端](media/launch-vcenter-from-cloudsimple-portal.png)

2. 選取您慣用的 vSphere 用戶端以存取 vCenter，並使用您的使用者名稱和密碼登入。  預設為：
...
* 存留期上限：密碼必須每隔365天變更一次
* 限制重複使用：使用者無法重複使用先前的五個密碼
* 長度： 8-20 個字元
* 特殊字元：至少一個特殊字元
...
* 相同的相鄰字元數上限：三
    ...
    範例：密碼的一部分可以接受 CC 或 CCC，但是 CCCC 則否。

如果您設定的密碼不符合需求：

* 如果您使用 vSphere Flash 用戶端，它會報告錯誤
* 如果您使用 HTML5 用戶端，則不會報告錯誤。 用戶端不接受變更，舊密碼仍繼續運作。

## <a name="access-nsx-manager"></a>存取 NSX 管理員

您可以使用預設密碼來部署 NSX manager。 

* 使用者名稱：系統 **管理員**
* Password： **CloudSimple123！**

您可以在 CloudSimple 入口網站上，找到 (FQDN) 和 NSX manager IP 位址的完整功能變數名稱。

1. 啟動 CloudSimple 入口網站，然後選取 [ **資源**]。
2. 按一下您想要使用的私人雲端。
3. 選取 **vSphere 管理網路**
4. 使用 [ **NSX 管理員** ] 的 FQDN 或 IP 位址，並使用網頁瀏覽器進行連線。
...
1. 遵循 [VSphere 網路指南](https://docs.vmware.com/en/VMware-vSphere/6.5/vsphere-esxi-vcenter-server-65-networking-guide.pdf)中「新增分散式通訊埠群組」中的指示。
2. 設定分散式埠群組時，請提供在 [為您的工作負載 Vm 建立 vlan](#create-a-vlan-for-your-workload-vms)中建立的 vlan ID。
