---
title: Azure VMware 解決方案（AVS）快速入門-建立 AVS 私用雲端
description: 瞭解如何使用 Azure VMware 解決方案（AVS）來建立及設定 AVS 私用雲端
...
ms.locfileid: "77018562"
---
# <a name="quickstart---configure-an-avs-private-cloud-environment"></a>快速入門-設定 AVS 私人雲端環境

在本文中，您將瞭解如何建立 AVS 私用雲端，並設定您的 AVS 私用雲端環境。
...
AVS 私用雲端是一種隔離的 VMware 堆疊，可支援 ESXi 主機、vCenter、vSAN 和 NSX。

AVS 私人雲端是透過 AVS 入口網站來管理。 他們在自己的管理網域中有自己的 vCenter server。 堆疊會在專用節點和隔離的裸機硬體節點上執行。
...
10. 輸入 vSphere/vSAN 子網的 CIDR 範圍。 請確定 CIDR 範圍不會與您的任何內部部署或其他 Azure 子網（虛擬網路）或閘道子網重迭。
    ...
    **CIDR 範圍選項：** /24、/23、/22 或/21。 A/24 CIDR 範圍最多支援26個節點，/23 CIDR 範圍最多支援58個節點，而/22 和/21 CIDR 範圍支援64個節點（AVS 私人雲端中的節點數目上限）。 若要深入瞭解 Vlan 和子網，請參閱[vlan 和子網總覽](cloudsimple-vlans-subnets.md)。
...
## <a name="launch-avs-portal"></a>啟動 AVS 入口網站

您可以從 Azure 入口網站存取 AVS 入口網站。 您將使用單一登入（SSO），透過您的 Azure 登入認證來啟動 AVS 入口網站。 存取 AVS 入口網站需要您授權**Avs 服務授權**應用程式。 如需授與許可權的詳細資訊，請參閱[同意 AVS 服務授權應用程式](access-cloudsimple-portal.md#consent-to-avs-service-authorization-application)。

1. 選取 [所有服務]。
2. 搜尋 [ **AVS 服務**]。
3. 選取您要在其上建立您的 AVS 私人雲端的 AVS 服務。
4. 在 [總覽] 中，按一下 **[前往 avs 入口網站**] 以開啟 [avs 入口網站] 的新瀏覽器索引標籤。 若出現提示，請使用您的 Azure 登入認證登入。 

    ![啟動 AVS 入口網站](media/launch-cloudsimple-portal.png)

## <a name="create-point-to-site-vpn"></a>建立點對站 VPN

點對站 VPN 連線是從電腦連線到您的 AVS 私人雲端最簡單的方式。 如果您是從遠端連線到 AVS 私人雲端，請使用點對站 VPN 連線。 若要快速存取您的 AVS 私用雲端，請遵循下列步驟。 您可以使用[站對站 VPN](vpn-gateway.md)或[Azure ExpressRoute](on-premises-connection.md)來存取您內部部署網路中的 AVS 區域。
...
建立 AVS 私用雲端之後，請建立您要部署工作負載/應用程式 Vm 的 VLAN。
...
將會建立 VLAN/子網。 您現在可以使用此 VLAN ID，在您的 AVS 私用雲端 vCenter 上建立分散式通訊埠群組。

## <a name="connect-your-environment-to-an-azure-virtual-network"></a>將您的環境連線至 Azure 虛擬網路

AVS 為您的 AVS 私人雲端提供 ExpressRoute 線路。 您可以將 Azure 上的虛擬網路連線到 ExpressRoute 線路。 如需設定連線的完整詳細資料，請遵循[使用 ExpressRoute 的 Azure 虛擬網路](https://docs.azure.cloudsimple.com/cloudsimple-azure-network-connection/)連線中的步驟。

## <a name="sign-in-to-vcenter"></a>登入 vCenter

您現在可以登入 vCenter 以設定虛擬機器和原則。

1. 若要存取 vCenter，請從 AVS 入口網站啟動。 在首頁的 [**一般**工作] 底下，按一下 [**啟動 vSphere 用戶端**]。 選取 [AVS 私用雲端]，然後按一下 [在 AVS 私人雲端上**啟動 VSphere 用戶端**]。

    ![啟動 vSphere 用戶端](media/launch-vcenter-from-cloudsimple-portal.png)

2. 選取您慣用的 vSphere 用戶端來存取 vCenter，並使用您的使用者名稱和密碼登入。 預設值為：
    * 使用者名稱： **CloudOwner@AVS.local**
    * 密碼： **AVS123！**  
...
AVS 建議您在第一次登入 vCenter 時變更密碼。 您所設定的密碼必須符合下列需求：
...
您可以在 AVS 入口網站上找到 NSX 管理員的完整功能變數名稱（FQDN）和 IP 位址。

1. 啟動 AVS 入口網站，然後選取 [**資源**]。
2. 按一下您要使用的 AVS 私人雲端。
3. 選取**vSphere 管理網路**
