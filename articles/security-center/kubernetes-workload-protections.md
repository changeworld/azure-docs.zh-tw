...
1. 附加元件安裝完成後約30分鐘，資訊安全中心會針對下列建議顯示叢集的健全狀況狀態，每個都出現在相關的安全性控制中，如下所示：
    ...
    | 建議名稱                                                         | 安全性控制                         | 需要設定 |
    |-----------------------------------------------------------------------------|------------------------------------------|------------------------|
    | 應強制執行容器 CPU 和記憶體限制                          | 保護應用程式免于遭受 DDoS 攻擊 | 否                     |
    | 應避免具有特殊許可權的容器                                     | 管理存取權和許可權            | 否                     |
    | 不可變的 (唯讀) 根目錄檔案系統應強制執行容器     | 管理存取權和許可權            | 否                     |
    | 應避免具有許可權擴大的容器                       | 管理存取權和許可權            | 否                     |
    | 應避免以根使用者的形式執行容器                           | 管理存取權和許可權            | 否                     |
    | 應避免共用機密主機命名空間的容器              | 管理存取權和權限            | 否                     |
    | 應為容器強制執行最低許可權的 Linux 功能       | 管理存取權和許可權            | **是**                |
    | Pod HostPath 磁片區裝載的使用應限制為已知清單    | 管理存取權和權限            | **是**                |
    | 容器應只接聽允許的埠                              | 限制未經授權的網路存取     | **是**                |
    | 服務應該只在允許的埠上接聽                                | 限制未經授權的網路存取     | **是**                |
    | 應限制使用主機網路和埠                     | 限制未經授權的網路存取     | **是**                |
    | 應限制容器的覆寫或停用 AppArmor 設定檔 | 修復安全性設定        | **是**                |
