...
當您啟用 Azure 中的資訊安全中心時，已針對每個 Azure 訂用帳戶開啟資料收集。 您也可以在 Azure 資訊安全中心的 [安全性原則] 區段中開啟訂用帳戶的資料收集。 當 [資料收集] 開啟時，Azure 資訊安全中心會在所有現有支援的 Azure 虛擬機器上布建 Log Analytics 代理程式，以及建立任何新的。
Log Analytics 代理程式會掃描各種安全性相關設定，並將其事件放入[Windows 事件追蹤](https://msdn.microsoft.com/library/windows/desktop/bb968803.aspx)（ETW）追蹤。 此外，作業系統會在執行機器的過程中引發事件記錄檔事件。 這類資料的範例包括︰作業系統類型和版本、作業系統記錄 (Windows 事件記錄)、執行中程序、電腦名稱、IP 位址、已登入的使用者和租用戶識別碼。 Log Analytics 代理程式會讀取事件記錄專案和 ETW 追蹤，並將它們複製到您的工作區進行分析。 Log Analytics 代理程式也會將損毀傾印檔案複製到您的工作區、啟用進程建立事件，並啟用命令列的審核。
