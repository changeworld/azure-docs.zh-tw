...
當您啟用 Azure 中的資訊安全中心時，已針對每個 Azure 訂用帳戶開啟資料收集。 您也可以在 Azure 資訊安全中心的 [安全性原則] 區段中開啟訂用帳戶的資料收集。 打開數據收集後,Azure 安全中心在所有現有受支援的 Azure 虛擬機器和創建的任何新虛擬機器上提供日誌分析代理。
日誌分析代理將掃描各種與安全相關的配置,並將其事件掃描到 Windows (ETW)[追蹤的事件](https://msdn.microsoft.com/library/windows/desktop/bb968803.aspx)追蹤中。 此外，作業系統會在執行機器的過程中引發事件記錄檔事件。 這類資料的範例包括︰作業系統類型和版本、作業系統記錄 (Windows 事件記錄)、執行中程序、電腦名稱、IP 位址、已登入的使用者和租用戶識別碼。 日誌分析代理讀取事件日誌條目和 ETW 跟蹤並將其複製到工作區進行分析。 日誌分析代理還會將崩潰轉儲檔複製到工作區,啟用進程創建事件,並啟用命令行審核。
...
* **Azure 活動**:所有安全警報、已批准的安全中心[即時](https://docs.microsoft.com/azure/security-center/security-center-just-in-time)請求以及[自適應應用程式控制程式](https://docs.microsoft.com/azure/security-center/security-center-adaptive-application)生成的所有警報。
* **Azure 監視器日誌**:所有安全警報。
