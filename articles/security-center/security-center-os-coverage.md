...
資訊安全中心取決於[Log Analytics 代理程式](../azure-monitor/platform/agents-overview.md#log-analytics-agent)。 請確定您的電腦正在執行此代理程式的其中一個支援作業系統，如下列頁面所述：
...
此外，請確定您的 Log Analytics 代理程式已[正確設定，可將資料傳送至資訊安全中心](security-center-enable-data-collection.md#manual-agent)
...
虛擬機器也會在客戶訂用帳戶中建立為部分 Azure 管理的服務，例如 Azure Kubernetes （AKS）、Azure Databricks 等等。 資訊安全中心也會探索這些虛擬機器，並在支援的 OS 可供使用時，安裝和設定 Log Analytics 代理程式。
...
也支援保護位於 Azure Stack 中的 Vm。 如需資訊安全中心與 Azure Stack 整合的詳細資訊，請參閱[將您的 Azure Stack 虛擬機器上架到資訊安全中心](https://docs.microsoft.com/azure/security-center/quick-onboard-azure-stack)。

## <a name="next-steps"></a>後續步驟

- 瞭解[資訊安全中心如何使用 Log Analytics 代理程式來收集資料](security-center-enable-data-collection.md)。
