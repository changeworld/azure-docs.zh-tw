...
升級至資訊安全中心標準版，讓您的混合式雲端工作負載能充分享有強化的安全性管理和威脅防護。 您可以免費試用「標準版」。 有關詳細資訊，請參閱安全中心[定價頁面](https://azure.microsoft.com/pricing/details/security-center/)。
...
您可以將整個 Azure 訂閱升級到標準層，該層由訂閱中的所有受支援資源繼承。 將標準層應用於工作區將應用於向工作區報告的所有資源。
...
3.  按一下 [啟動試用版]**** 來啟動所選訂用帳戶上的試用版。
  ![選取訂用帳戶](./media/security-center-onboarding/select-subscription.png)
   ...
   > [!NOTE]
   > 安全中心的免費功能僅應用於 Azure VM 和 VMSS。 免費版功能不會套用至非 Azure 電腦。 如果選擇"標準"，則標準功能將應用於報告工作區的所有 Azure VM、VM 比例集和非 Azure 電腦。 我們建議您套用「標準版」，為您的 Azure 與非 Azure 資源提供進階的安全性。
   >
...
2. 在 **"安全和審核**"下，選擇**OMS 工作區**以創建新工作區。
   > [!NOTE]
