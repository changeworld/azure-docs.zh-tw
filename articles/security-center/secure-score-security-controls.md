...
- 在 [專用 **安全分數** ] 頁面中，您可以看到訂用帳戶和管理群組的安全分數：

    :::image type="content" source="./media/secure-score-security-controls/score-on-dedicated-dashboard.png" alt-text="資訊安全中心的安全分數頁面上訂用帳戶的安全分數":::

    :::image type="content" source="./media/secure-score-security-controls/secure-score-management-groups.png" alt-text="資訊安全中心的 [安全分數] 頁面上的 [管理群組的安全分數] 頁面":::

    > [!NOTE]
    > 任何您沒有足夠許可權的管理群組都會顯示其分數為「受限制」。 
...
:::image type="content" source="./media/secure-score-security-controls/power-bi-secure-score-dashboard.png" alt-text="隨時間變化的選擇性安全分數 Power BI 儀表板，以追蹤一段時間的安全分數並調查變更":::
