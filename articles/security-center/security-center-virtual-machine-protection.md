...
## <a name="view-the-security-of-your-compute-and-apps-resources"></a>查看計算和應用資源的安全性
...
* [**VM 和伺服器**](#vms-and-computers)：列出對 VM、電腦和每個伺服器的當前安全狀態的建議

* [**VM 規模集**](#vmscale-sets)：列出規模集的建議， 

* [**雲服務**](#cloud-services)：列出安全中心監視的 Web 和輔助角色的建議

* [**應用服務**](#app-services)：列出應用服務環境的建議以及每個環境的當前安全狀態

* [**容器**](#containers)：列出容器的建議及其配置的安全評估

* **計算資源**：列出計算資源的建議，如服務結構群集和事件中心

### <a name="whats-in-each-tab"></a>每個選項卡中有什麼？

每個選項卡有多個部分，在每個部分中，您可以向下切入以查看有關顯示的專案的其他詳細資訊。
...
"VM 和電腦"部分概述了 VM 和電腦的所有安全建議。 包括四種類型的機器：
...
![從工作區識別的 VM](./media/security-center-virtual-machine-recommendations/security-center-monitoring-icon4.png) 僅只從已檢視訂用帳戶中的工作區識別的 VM。 這包括報告為此訂閱中的工作區的其他訂閱的 VM，以及使用操作管理器直接代理安裝且沒有資源識別碼 的 VM。
...
### <a name="virtual-machine-scale-sets"></a><a name="vmscale-sets"></a>虛擬機器縮放集
安全中心會自動探索您是否具有縮放集，並建議您在擴展集上安裝 Microsoft 監視代理。
...
要設置新的規模集以自動安裝 Microsoft 監視代理，請執行以下規定：
1. 移至 Azure 原則，然後按一下 [定義]****。

1. 搜索 Windows**虛擬機器縮放集的策略"部署日誌分析代理**"，然後按一下它。
...
在有建議的情況下，請按照建議中的步驟更新作業系統。 當更新可用時，您將有一個警報（紅色或橙色 - 取決於問題的嚴重性）。 有關此建議的完整說明，請按一下 **"描述"** 列下的 **"更新作業系統"版本**。
...
如果選擇 Web 應用程式，將打開一個摘要視圖，其中包含三個選項卡：
...
打開 **"容器"** 選項卡時，根據您的環境，您可能會看到三種類型的資源中的任何一種：

![容器主機](./media/security-center-virtual-machine-recommendations/icon-container-host-rec.png) 容器主機 - 運行 Docker 的 VM 
...
有關如何使用容器安全功能的說明，請參閱[監視容器的安全性](monitor-container-security.md)。

[此處](azure-container-registry-integration.md)介紹了 Azure 容器註冊表捆綁包的優點
...
要查看清單中特定資源的建議，請按一下該資源。

#### <a name="visibility-into-container-registries"></a>容器註冊表的可見度
...
按一下運行 Docker 的 VM 之一時，您將看到詳細資訊頁，其中包含與電腦上的容器相關的資訊，例如 Docker 版本和主機上運行的圖像數。

![運行 Docker 的 VM 建議](./media/security-center-virtual-machine-recommendations/docker-recommendation.png)
