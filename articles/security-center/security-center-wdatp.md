...
資訊安全中心的 Microsoft Defender ATP 支援 Windows Server 2016、2012 R2 和 2008 R2 SP1 的偵測。 針對 Azure Vm，您需要標準層訂用帳戶，而針對非 Azure Vm，您只需要在工作區層級中使用標準層。
...
2. 如果您看不到所有的工作區，可能是因為許可權不足，請確定您的工作區設定為 Azure 安全性標準定價層。 如需詳細資訊，請參閱 [升級為資訊安全中心的標準層以增強安全性](security-center-pricing.md)。
    
3. 選取 [ **新增伺服器** ]，以查看如何安裝 Log Analytics 代理程式的指示。 

4. 上架之後，您可以在 [ **計算和應用程式**] 底下監視電腦。
