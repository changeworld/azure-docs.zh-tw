...
Azure 安全中心正在通過與[Microsoft 防禦者高級威脅防護](https://www.microsoft.com/microsoft-365/windows/microsoft-defender-atp)（ATP） 集成來擴展其雲工作負載保護平台產品。
...
- **先進的漏洞後檢測感應器**：微軟衛士ATP感應器為Windows伺服器收集大量的行為信號。
...
- **自動載入**：微軟防禦者 ATP 感應器自動啟用到 Azure 安全中心板載的 Windows 伺服器。

- **單個窗格**：Azure 安全中心主控台顯示 Microsoft Defender ATP 警報。

- **詳細的機器調查**：Azure 安全中心客戶可以使用 Microsoft Defender ATP 主控台進行詳細調查，以發現違規範圍。
...
> [!NOTE]
> 當您使用 Azure 安全中心監視伺服器時，將自動創建 Microsoft Defender ATP 租戶，預設情況下，Microsoft Defender ATP 資料將存儲在歐洲。 如果需要將資料移動到其他位置，則需要聯繫 Microsoft 支援以重置租戶。 已為 Office 365 GCC 客戶禁用了使用此集成的伺服器終結點監視。
...
1. 在 **"入職"** 區域中，選擇或創建一個工作區，用於存儲資料。 <br>
2. 如果您看不到所有的工作區，可能是因為權限不足，請確定您的工作區設定為 Azure 安全性標準層。 有關詳細資訊，請參閱[升級到安全中心的標準層，以提高安全性](security-center-pricing.md)。
    
3. 選擇 **"添加伺服器**"以查看有關如何安裝 Microsoft 監視代理的說明。 

4. 入職後，您可以在 **"計算"和"應用程式**"下監視電腦。

   ![上線的電腦](media/security-center-wdatp/onboard-computers.png)

## <a name="enable-microsoft-defender-atp-integration"></a>啟用微軟後衛ATP集成

要查看是否啟用了 Microsoft Defender ATP 集成，請選擇**安全中心** > **定價&設置**>按一下您的訂閱。
您可以在這裡查看目前已啟用的整合內容。

  ![啟用 Microsoft 防禦者 ATP 集成的 Azure 安全中心威脅檢測設置頁面](media/security-center-wdatp/enable-integrations.png)
...
- 如果您已通過 Microsoft Defender ATP 登上伺服器：
  - 參閱文件，了解[如何下架伺服器機器](https://go.microsoft.com/fwlink/p/?linkid=852906)的指南。
  - 將伺服器上架到 Azure 資訊安全中心。

## <a name="access-to-the-microsoft-defender-atp-portal"></a>訪問微軟後衛 ATP 門戶
...
3. 如果命令成功，您將在 Azure 安全中心儀表板和 Microsoft Defender ATP 門戶上看到新的警報。 此警示可能需要幾分鐘才會顯示。

4. 要查看安全中心的警報，請訪問**安全警報** > **可疑電源 Shell 命令列**。

5. 從調查視窗，選擇要轉到 Microsoft Defender ATP 門戶的連結。
