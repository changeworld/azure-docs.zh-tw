...
若要從機器收集資料，您應該安裝 Log Analytics 代理程式。 代理程式的安裝可以自動完成（建議選項），也可以手動安裝代理程式。 預設會關閉自動布建。

當自動布建為 on 時，資訊安全中心會在所有支援的 Azure Vm 和任何新建立的虛擬機器上部署 Log Analytics 代理程式。 建議使用自動布建，但如有必要，您可以手動安裝代理程式（請參閱[手動安裝 Log Analytics 代理程式](#manual-agent)）。


啟用 Log Analytics 代理程式的自動佈建：
1. 從入口網站資訊安全中心的功能表中，選取 [**定價] & 設定**]。
2. 選取相關的訂用帳戶。
...
5. 選取 [儲存]  。 代理程式會在15分鐘內部署在所有 Vm 上。 

>[!TIP]
> 如果需要布建工作區，代理程式安裝可能需要最多25分鐘的時間。
...
資訊安全中心收集的資料會儲存在 Log Analytics 工作區。 您的資料可以從儲存在資訊安全中心所建立的工作區中的 Azure Vm，或在您建立的現有工作區中收集。 
...
4. 選取 [儲存]**** 之後，系統會詢問您是否要重新設定先前連線到預設工作區的受監控虛擬機器。
   ...
   - 如果您想要將新的工作區設定僅套用在新的 Vm 上，請選取 [**否**]。 新的工作區設定僅適用于新的代理程式安裝;未安裝 Log Analytics 代理程式的新探索到的 Vm。
...
### <a name="automatic-provisioning-in-cases-of-a-pre-existing-agent-installation"></a>在預先存在的代理程式安裝情況下自動布建<a name="preexisting"></a> 
...
[安全性中心] 會將 Log Analytics 代理程式延伸模組並存安裝到現有的 Operations Manager。 現有的 Operations Manager 代理程式會繼續正常地向 Operations Manager 伺服器報告。 Operations Manager agent 和 Log Analytics 代理程式會共用通用的執行時間程式庫，這將會在此程式期間更新為最新版本。 如果已安裝 Operations Manager 代理程式版本 2012 **，請不要啟用**自動布建。<br>
...
3. 在 [**安全性原則-資料收集**] 頁面的 [**自動**布建] 下，選取 [**關閉**]。
4. 選取 [儲存]  。
...
- 若要找出未受監視的 Vm 和電腦問題：
    ...
    如果機器未執行 Log Analytics 代理程式擴充功能，資訊安全中心不會監視 VM 或電腦。 機器可能已安裝本機代理程式，例如 OMS 直接代理程式或 System Center Operations Manager 代理程式。 具有這些代理程式的機器會識別為未受監視，因為資訊安全中心無法完整支援這些代理程式。 為了充分利用資訊安全中心的所有功能，需要 Log Analytics 代理程式延伸模組。

    如需資訊安全中心無法成功監視已初始化進行自動布建之 Vm 和電腦的原因詳細資訊，請參閱[監視代理程式健康情況問題](security-center-troubleshooting-guide.md#mon-agent)。


## <a name="next-steps"></a>後續步驟
本文說明資料收集和自動佈建如何在資訊安全中心運作。 若要深入瞭解資訊安全中心，請參閱下列頁面：
