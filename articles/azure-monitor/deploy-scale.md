...
[診斷設定](platform/diagnostic-settings.md)會將資源記錄和計量從 Azure 資源收集到多個位置，通常是在 log Analytics 工作區，可讓您使用[記錄查詢](log-query/log-query-overview.md)和[記錄警示](platform/alerts-log.md)來分析資料。 每當您建立資源時，請使用原則自動建立診斷設定。
...
|名稱 |描述 |
|-----|------------|
|Audit Log Analytics 代理程式部署–未列出的 VM 映射 (OS)  |如果 VM 映射 (OS) 未定義于清單中，且未安裝代理程式，則會將 Vm 回報為不相容。 |
|部署適用於 Linux VM 的 Log Analytics 代理程式 |部署適用于 Linux Vm 的 Log Analytics 代理程式（如果 VM 映射 (OS) 是在清單中定義，而且代理程式未安裝。 |
|部署適用於 Windows VM 的 Log Analytics 代理程式 |部署適用于 Windows Vm 的 Log Analytics 代理程式（如果 VM 映射 (OS) 是在清單中定義，而且代理程式未安裝。 |
...
|在虛擬機器擴展集中進行 Audit Dependency agent 部署– VM 映射 (OS) 未列出 |如果 VM 映射 (OS) 未定義于清單中，且未安裝代理程式，則將虛擬機器擴展集報告為不相容。 |
|在虛擬機器擴展集中進行 Audit Log Analytics 代理程式部署–未列出 VM 映射 (OS)  |如果 VM 映射 (OS) 未定義于清單中，且未安裝代理程式，則將虛擬機器擴展集報告為不相容。 |
|為 Linux 虛擬機器擴展集部署 Log Analytics 代理程式 |部署適用于 Linux 虛擬機器擴展集的 Log Analytics 代理程式（如果 VM 映射 (OS) 是在清單中定義，而且代理程式未安裝。 |
