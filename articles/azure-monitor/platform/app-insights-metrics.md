---
title: Azure 應用程式見解記錄型計量 |Microsoft Docs
description: 本文列出支援的匯總和維度的 Azure 應用程式 Insights 計量。 記錄型計量的詳細資料包含基礎 Kusto 查詢語句。
author: vgorbenko
...
Application Insights 記錄型計量可讓您分析受監視應用程式的健康情況、建立功能強大的儀表板，以及設定警示。 計量有兩種：

* 場景後方以[記錄為基礎的計量](../app/pre-aggregated-metrics-log-metrics.md#log-based-metrics)會從預存事件轉譯成 Kusto 的[查詢](/azure/kusto/query/)。
* [標準計量](../app/pre-aggregated-metrics-log-metrics.md#pre-aggregated-metrics) 會儲存為預先匯總的時間序列。

由於 *標準計量* 會在收集期間預先匯總，因此在查詢時會有較佳的效能。 這讓它們成為儀表板和即時警示的較佳選擇。 以 *記錄為基礎的度量* 有更多維度，讓它們成為資料分析和臨機操作診斷的絕佳選項。 使用 [命名空間選取器](metrics-getting-started.md#create-your-first-metric-chart) ，在 [計量瀏覽器](metrics-getting-started.md)中的記錄式和標準計量之間切換。

## <a name="interpret-and-use-queries-from-this-article"></a>解讀和使用本文中的查詢

本文列出支援的匯總和維度的度量。 記錄型計量的詳細資料包含基礎 Kusto 查詢語句。 為了方便起見，每個查詢都會使用時間細微性、圖表類型，以及有時分割維度的預設值，以簡化在 Log Analytics 中使用查詢的方式，而不需要修改。
...
### <a name="users-authenticated-usersauthenticated"></a>使用者、已驗證的 (使用者/已驗證) 
