---
title: Azure 監視器中視圖設計器部件的參考指南 |微軟文檔
description: 通過在 Azure 監視器中使用視圖設計器，可以創建顯示在 Azure 門戶中的自訂視圖，並在日誌分析工作區中包含各種資料視覺化效果。 此文章是自訂檢視中可用的視覺效果組件設定的參考指南。
ms.subservice: logs
ms.topic: conceptual
author: bwren
ms.author: bwren
...
ms.locfileid: "77658552"
---
# <a name="reference-guide-to-view-designer-visualization-parts-in-azure-monitor"></a>在 Azure 監視器中查看設計器視覺化部件的參考指南
通過在 Azure 監視器中使用視圖設計器，可以在 Azure 門戶中創建各種自訂視圖，這些視圖可説明您在日誌分析工作區中視覺化資料。 此文章是自訂檢視中可用的視覺效果組件設定的參考指南。
...
| 檢視類型 | 描述 |
|:--- |:--- |
| [查詢清單](#list-of-queries-part) |顯示日誌查詢的清單。 您可以選取每個查詢以顯示其結果。 |
| [數字與清單](#number-and-list-part) |標頭顯示單個數位，顯示日誌查詢中的記錄計數。 清單會顯示查詢的前 10 筆結果，並包含表示數值資料行的相對值或一段時間內之變更的圖表。 |
| [兩個數字與清單](#two-numbers-and-list-part) |標頭顯示兩個數字，顯示來自不同日誌查詢的記錄計數。 清單會顯示查詢的前 10 筆結果，並包含表示數值資料行的相對值或一段時間內之變更的圖表。 |
...
> [!NOTE]
> 視圖中的部件基於日誌分析工作區中的[日誌查詢](../log-query/log-query-overview.md)。 它們當前不支援[跨資源查詢](../log-query/cross-workspace-query.md)以從應用程式見解檢索資料。

## <a name="list-of-queries-part"></a>查詢清單部分
查詢清單部分顯示日誌查詢的清單。 您可以選取每個查詢以顯示其結果。 根據預設，檢視會包含單一查詢，您可以選取 [+ 查詢]**** 來新增額外的查詢。
...
標頭顯示單個數位，顯示日誌查詢中的記錄計數。 清單會顯示查詢的前 10 筆結果，並包含表示數值資料行的相對值或一段時間內之變更的圖表。
...
標頭有兩個數字，顯示來自不同日誌查詢的記錄計數。 清單會顯示查詢的前 10 筆結果，並包含表示數值資料行的相對值或一段時間內之變更的圖表。
...
| 單位 |指定要查詢傳回之值的單位。 這項資訊用來顯示圖表上的標籤，指出值的類型，並選擇性地用來轉換值。 *單位*類型指定單位的分類，並定義可用的*目前的單位*類型值。 如果您選取*轉換成*中的值，則數值會從*目前單位*類型轉換為*轉換成*類型。 |
...
名稱和值分隔符號是單一字元分隔符號，用來將來自清單查詢的文字屬性剖析成多個值。 如果指定分隔符號，則可以為每個欄位提供名稱，由 **"名稱"** 框中的同一分隔符號分隔。
...
點選瀏覽會定義當您按一下標頭或檢視中的清單項目時，要採取哪個動作。  這將在[日誌分析](../../azure-monitor/log-query/portals.md)中打開查詢或啟動另一個視圖。

下表描述點選瀏覽的設定。

| 設定           | 描述 |
|:--|:--|
| 記錄搜尋 (自動) | 在選擇標頭項時，將運行的日誌查詢。  這是項所基於的相同日誌查詢。
| 記錄搜尋        | 在清單中選擇專案時，將查詢記錄以運行。  將查詢輸入 [瀏覽查詢]**** 方塊。   使用 *{selected item}* 來包含使用者所選項目的語法。  例如，如果查詢具有名為 *Computer* 的資料行且瀏覽查詢是 *{selected item}*，則會在您選取電腦時執行 *Computer="MyComputer"* 之類的查詢。 如果瀏覽查詢是 *Type=Event {selected item}*，則會執行 *Type=Event Computer="MyComputer"*。 |
...
### <a name="thresholds"></a><a name="thresholds"></a>臨界值
...
* 瞭解[日誌查詢](../log-query/log-query-overview.md)以支援視覺化部分中的查詢。
