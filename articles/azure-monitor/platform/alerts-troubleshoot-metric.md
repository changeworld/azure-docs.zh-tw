...
- 如果您收到 `Metric not found` 錯誤：
   ...
   - 針對自訂計量：請確定已發出計量， (您無法針對尚未存在) 的自訂計量建立警示規則，而且您要提供自訂計量的命名空間 (請參閱[這裡](./alerts-metric-create-templates.md#template-for-a-static-threshold-metric-alert-that-monitors-a-custom-metric)的 ARM 範本範例) 
