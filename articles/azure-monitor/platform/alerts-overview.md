...
**操作**:觸發警報時執行的特定操作。 有關詳細資訊,請參閱[操作群組](../../azure-monitor/platform/action-groups.md)。

## <a name="what-you-can-alert-on"></a>您可以發出警示的對象

您可以對指標和日誌發出警報,如[監視數據源](../../azure-monitor/platform/data-sources.md)中所述。 包含但不限於：
...
您可以設定警示的狀態來指定警示在解決流程中的位置。 當滿足警示規則中指定的條件時,將建立或觸發警報,並且其狀態為*New*。 當您認可警示並將它關閉時，您可以變更狀態。 任何狀態變更都會儲存在警示的記錄中。
...
智慧組是基於機器學習演演演算法的警報聚合,可説明降低警報噪音並説明排除故障。 [深入了解智慧群組](https://aka.ms/smart-groups)和[如何管理智慧群組](https://aka.ms/managing-smart-groups)。


## <a name="alerts-experience"></a>警示體驗 
默認警報頁提供在特定時間範圍內創建的警報的摘要。 它顯示每個嚴重性的總警報,列用於標識每個嚴重性狀態中的警報總數。 選取任何嚴重性以開啟依照該嚴重性篩選的 [所有警示](#all-alerts-page) 頁面。

或您可以使用[REST API 以程式設計方式的 enalt 的警示檔 。](#manage-your-alert-instances-programmatically)
...
![警示頁面的螢幕擷取](media/alerts-overview/alerts-page.png)

您可以通過在頁面頂部的下拉菜單中選擇值來篩選此視圖。

| 資料行 | 描述 |
|:---|:---|
| 訂用帳戶 | 選擇要為其查看警報的 Azure 訂閱。 您可以選擇選擇所有訂閱。 檢視中僅包含您有權訪問的選定訂閱中的警報。 |
...
要顯示 **「規則」** 頁,請選擇「**管理警報規則**」。 "規則"頁是管理 Azure 訂閱中所有警報規則的單個位置。 它會列出所有警示規則，並可根據目標資源、資源群組、規則名稱或狀態來排序。 您還可以編輯、啟用或禁用此頁面的警報規則。  
 ...
 ![規則頁面的螢幕擷取](./media/alerts-overview/alerts-preview-rules.png)


## <a name="create-an-alert-rule"></a>建立警示規則
無論監視服務或信號類型如何,您都可以以一致的方式編寫警報規則。
...
![所有警示頁面的螢幕擷圖](media/alerts-overview/all-alerts-page.png)

您可以透過在頁面頂部的下拉選單中選擇以下值來篩選檢視:
...
| Severity | 選擇警報嚴重性,或選擇 **「全部**」以包含所有嚴重性警報。 |
...
## <a name="alert-details-page"></a>警示詳細資訊頁面
選擇警報時,此頁面提供警報的詳細資訊,並使您能夠更改其狀態。

![警示詳細資訊頁面的螢幕截圖](media/alerts-overview/alert-detail2.png)
...
| 診斷 | 有關包含警報的智慧組的資訊。 「警示計數」** 是指智慧群組中包含的警示數目。 包括過去 30 天內創建的同一智慧組中的其他警報,而不考慮警報列表頁中的時間篩選器。 選取警示以檢視其詳細資料。 |

## <a name="role-based-access-control-rbac-for-your-alert-instances"></a>警示實體的基於角色的存取控制 (RBAC)

警示實例的消耗和管理要求使用者具有[監視參與者](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#monitoring-contributor)或[監視讀取器](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#monitoring-reader)的內建 RBAC 角色。 這些角色在任何 Azure 資源管理器作用域中都受支援,從訂閱級別到資源級別的粒度分配。 例如,如果使用者僅對虛擬機`ContosoVM1`具有監視參與者訪問許可權,則該使用者可以僅使用和管理在`ContosoVM1`上生成的警報。

## <a name="manage-your-alert-instances-programmatically"></a>以程式設計管理警示實體

您可能希望以程式設計方式查詢針對訂閱生成的警報。 這可能是在 Azure 門戶之外創建自定義視圖,或分析警報以識別模式和趨勢。
...
以下對資源圖 REST API 的範例請求傳回一個訂閱中的警示計數:
...
您還可以在門戶中通過 Azure 資源圖資源管理員查看此資源圖查詢的結果[:portal.azure.com](https://portal.azure.com/?feature.customportal=false#blade/HubsExtension/ArgQueryBlade/query/AlertsManagementResources%20%7C%20where%20type%20%3D~%20%27Microsoft.AlertsManagement%2Falerts%27%20%7C%20summarize%20count())
