...
5. 取得資料來源：`Query`
    ...
    2. 訂戶`Use default subscriptions`
    3. 在查詢控制項中，新增此程式碼片段
    ```kusto
...
[Azure Resource Graph 文件](https://docs.microsoft.com/azure/governance/resource-graph/overview)
...
5. 取得資料來源：`JSON`
    ...
    ```
    2. 點擊藍色的 [_更新_] 按鈕。
6. 選擇性地將設定 `Include only resource types` 為_Application Insights_
7. 從工具列選擇 [儲存] 以建立參數。
