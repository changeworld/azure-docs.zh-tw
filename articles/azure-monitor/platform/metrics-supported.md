...
|CurrentConnections|連接：目前的連接|[計數]|Average|目前已建立的用戶端連線數。|ServerResourceType|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|TotalRequests|閘道要求總數（已淘汰）|[計數]|總數|閘道要求數目-請改用具有 GatewayResponseCodeCategory 維度的多維度要求度量|Location、Hostname|
|SuccessfulRequests|成功的閘道要求（已淘汰）|[計數]|總數|成功的閘道要求數目-請改用具有 GatewayResponseCodeCategory 維度的多維度要求度量|Location、Hostname|
|UnauthorizedRequests|未經授權的閘道要求（已淘汰）|[計數]|總數|未授權的閘道要求數目-請改用具有 GatewayResponseCodeCategory 維度的多維度要求度量|Location、Hostname|
|FailedRequests|失敗的閘道要求（已淘汰）|[計數]|總數|閘道要求中的失敗次數-請改用具有 GatewayResponseCodeCategory 維度的多維度要求度量|Location、Hostname|
|OtherRequests|其他閘道要求（已淘汰）|[計數]|總數|其他閘道要求數目-請改用具有 GatewayResponseCodeCategory 維度的多維度要求度量|Location、Hostname|
|持續期間|閘道要求的整體持續期間|毫秒|Average|閘道要求的整體持續期間，以毫秒為單位|Location、Hostname|
|BackendDuration|後端要求的持續時間|毫秒|Average|後端要求的持續時間（以毫秒為單位）|Location、Hostname|
|容量|容量|百分比|Average|ApiManagement 服務的使用率衡量標準|位置|
|EventHubTotalEvents|EventHub 事件總數|[計數]|總數|傳送至 EventHub 的事件數目|位置|
|EventHubSuccessfulEvents|成功的 EventHub 事件|[計數]|總數|成功的 EventHub 事件數目|位置|
|EventHubTotalFailedEvents|失敗的 EventHub 事件|[計數]|總數|失敗的 EventHub 事件數目|位置|
|EventHubRejectedEvents|已拒絕 EventHub 事件|[計數]|總數|已拒絕的 EventHub 事件數（錯誤設定或未經授權）|位置|
|EventHubThrottledEvents|已節流的 EventHub 事件|[計數]|總數|已節流的 EventHub 事件數目|位置|
|EventHubTimedoutEvents|已超時的 EventHub 事件|[計數]|總數|已超時的 EventHub 事件數|位置|
|EventHubDroppedEvents|已卸載 EventHub 事件|[計數]|總數|因為達到佇列大小上限而略過的事件數目|位置|
|EventHubTotalBytesSent|EventHub 事件的大小|位元組|總數|EventHub 事件的總大小（以位元組為單位）|位置|
|要求|要求|[計數]|總數|具有多個維度的閘道要求計量|Location、Hostname、LastErrorReason、BackendResponseCode、GatewayResponseCode、BackendResponseCodeCategory、GatewayResponseCodeCategory|
...
|AppMemoryMax|應用程式記憶體最大值|位元組|最大需求|可用於記憶體管理的最大記憶體數量（以位元組為單位）|AppName、Pod|
...
|OldGenPromotedBytes|升級為舊的產生資料大小|位元組|最大需求|在 GC 到 GC 之後，舊的世代記憶體集區大小正增加的計數|AppName、Pod|
|YoungGenPromotedBytes|升階為年輕產生資料大小|位元組|最大需求|在一個 GC 到下一個之前，會增加年輕世代記憶體集區的大小|AppName、Pod|
|GCPauseTotalCount|GC 暫停計數|[計數]|總數|GC 暫停計數|AppName、Pod|
|GCPauseTotalTime|GC 暫停總時間|毫秒|總數|GC 暫停總時間|AppName、Pod|
|TomcatSentBytes|Tomcat 總傳送位元組數|位元組|總數|Tomcat 總傳送位元組數|AppName、Pod|
|TomcatReceivedBytes|Tomcat 接收的位元組總數|位元組|總數|Tomcat 接收的位元組總數|AppName、Pod|
|TomcatRequestTotalTime|Tomcat 要求總次數|毫秒|總數|Tomcat 要求總次數|AppName、Pod|
|TomcatRequestTotalCount|Tomcat 要求總計數|[計數]|總數|Tomcat 要求總計數|AppName、Pod|
|TomcatResponseAvgTime|Tomcat 要求平均時間|毫秒|Average|Tomcat 要求平均時間|AppName、Pod|
|TomcatRequestMaxTime|Tomcat 要求最大時間|毫秒|最大需求|Tomcat 要求最大時間|AppName、Pod|
|TomcatErrorCount|Tomcat 全域錯誤|[計數]|總數|Tomcat 全域錯誤|AppName、Pod|
|TomcatSessionActiveMaxCount|Tomcat 會話最大使用中計數|[計數]|總數|Tomcat 會話最大使用中計數|AppName、Pod|
|TomcatSessionAliveMaxTime|Tomcat 會話最大的存留時間|毫秒|最大需求|Tomcat 會話最大的存留時間|AppName、Pod|
|TomcatSessionActiveCurrentCount|Tomcat 會話的生存計數|[計數]|總數|Tomcat 會話的生存計數|AppName、Pod|
|TomcatSessionCreatedCount|Tomcat 會話已建立計數|[計數]|總數|Tomcat 會話已建立計數|AppName、Pod|
|TomcatSessionExpiredCount|Tomcat 會話過期計數|[計數]|總數|Tomcat 會話過期計數|AppName、Pod|
|TomcatSessionRejectedCount|Tomcat 會話已拒絕計數|[計數]|總數|Tomcat 會話已拒絕計數|AppName、Pod|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|TotalJob|工作總數|[計數]|總數|工作總數|Runbook，狀態|
|TotalUpdateDeploymentRuns|更新部署執行總計|[計數]|總數|軟體更新部署執行總計|SoftwareUpdateConfigurationName，狀態|
|TotalUpdateDeploymentMachineRuns|更新部署電腦的執行總計|[計數]|總數|在軟體更新部署執行中執行的軟體更新部署電腦總數|SoftwareUpdateConfigurationName、Status、TargetComputer、SoftwareUpdateConfigurationRunId|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|CoreCount|專用核心計數|[計數]|總數|Batch 帳戶中的專用核心總數|無|
|TotalNodeCount|專用節點計數|[計數]|總數|Batch 帳戶中的專用節點總數|無|
|LowPriorityCoreCount|LowPriority 核心計數|[計數]|總數|Batch 帳戶中的低優先順序核心總數|無|
|TotalLowPriorityNodeCount|低優先順序節點計數|[計數]|總數|Batch 帳戶中的低優先順序節點總數|無|
|CreatingNodeCount|建立節點計數|[計數]|總數|正在建立的節點數目|無|
|StartingNodeCount|啟動節點計數|[計數]|總數|啟動的節點數目|無|
|WaitingForStartTaskNodeCount|等候啟動工作節點計數|[計數]|總數|等待啟動工作完成的節點數目|無|
|StartTaskFailedNodeCount|啟動工作失敗的節點計數|[計數]|總數|啟動工作失敗的節點數目|無|
|IdleNodeCount|閒置的節點計數|[計數]|總數|閒置節點的數目|無|
|OfflineNodeCount|離線節點計數|[計數]|總數|離線節點的數目|無|
|RebootingNodeCount|重新啟動節點計數|[計數]|總數|重新啟動節點的數目|無|
|RebootingNodeCount|重新安裝映像節點計數|[計數]|總數|重新安裝映像的節點數目|無|
|RunningNodeCount|執行節點計數|[計數]|總數|執行節點的數目|無|
|LeavingPoolNodeCount|離開集區節點計數|[計數]|總數|離開集區的節點數目|無|
|UnusableNodeCount|無法使用的節點計數|[計數]|總數|無法使用的節點數目|無|
|PreemptedNodeCount|先占節點計數|[計數]|總數|先占節點的數量|無|
|TaskStartEvent|工作啟動的事件|[計數]|總數|已啟動的工作總次數|無|
|TaskCompleteEvent|工作完成事件|[計數]|總數|已完成的工作總次數|無|
|TaskFailEvent|工作失敗事件|[計數]|總數|失敗狀態中已完成的工作總數|無|
|PoolCreateEvent|集區建立事件|[計數]|總數|已建立集區的總數|無|
|PoolResizeStartEvent|調整集區大小啟動事件|[計數]|總數|已啟動的調整集區大小總次數|無|
|PoolResizeCompleteEvent|調整集區大小完成事件|[計數]|總數|已完成的調整集區大小總次數|無|
|PoolDeleteStartEvent|集區刪除啟動事件|[計數]|總數|已啟動的集區刪除總次數|無|
|PoolDeleteCompleteEvent|集區刪除完成事件|[計數]|總數|已完成的集區刪除總次數|無|
|JobDeleteCompleteEvent|作業刪除完成事件|[計數]|總數|已成功刪除的作業總數。|無|
|JobDeleteStartEvent|作業刪除啟動事件|[計數]|總數|已要求要刪除的作業總數。|無|
|JobDisableCompleteEvent|作業停用完成事件|[計數]|總數|已成功停用的作業總數。|無|
|JobDisableStartEvent|作業停用啟動事件|[計數]|總數|已要求要停用的作業總數。|無|
|JobStartEvent|作業啟動事件|[計數]|總數|已成功啟動的作業總數。|無|
|JobTerminateCompleteEvent|作業終止完成事件|[計數]|總數|已成功終止的作業總數。|無|
|JobTerminateCompleteEvent|作業終止啟動事件|[計數]|總數|已要求要終止的作業總數。|無|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|已提交作業|已提交作業|[計數]|總數|已提交的作業數目|案例，ClusterName|
|工作已完成|工作已完成|[計數]|總數|已完成的作業數目|案例，ClusterName，ResultType|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|CpuUsagePercentageInDouble|CPU 使用量百分比|百分比|最大需求|CPU 使用量百分比|節點|
...
|IOReadBytes|IO 讀取位元組|位元組|總數|IO 讀取位元組|節點|
|IOWriteBytes|IO 寫入位元組|位元組|總數|IO 寫入位元組|節點|
|ConnectionAccepted|接受的連接|[計數]|總數|接受的連接|節點|
|ConnectionHandled|已處理的連接|[計數]|總數|已處理的連接|節點|
|ConnectionActive|作用中的連線|[計數]|Average|作用中的連線|節點|
|RequestHandled|已處理的要求|[計數]|總數|已處理的要求|節點|
|ProcessedBlocks|已處理的區塊|[計數]|總數|已處理的區塊|節點|
|ProcessedTransactions|已處理的交易|[計數]|總數|已處理的交易|節點|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|connectedclients|Connected Clients|[計數]|最大需求||ShardId|
|totalcommandsprocessed|總作業數|[計數]|總數||ShardId|
|cachehits|快取點擊|[計數]|總數||ShardId|
|cachemisses|快取遺漏|[計數]|總數||ShardId|
|cachemissrate|快取遺漏率|百分比|cachemissrate||ShardId|
|getcommands|取得|[計數]|總數||ShardId|
|setcommands|集合|[計數]|總數||ShardId|
|operationsPerSecond|每秒的作業數|[計數]|最大需求||ShardId|
|evictedkeys|收回的金鑰|[計數]|總數||ShardId|
|totalkeys|索引鍵總計|[計數]|最大需求||ShardId|
|expiredkeys|到期的金鑰|[計數]|總數||ShardId|
|usedmemory|Used Memory|位元組|最大需求||ShardId|
|usedmemorypercentage|已用的記憶體百分比|百分比|最大需求||ShardId|
|usedmemoryRss|已用的記憶體 RSS|位元組|最大需求||ShardId|
|serverLoad|伺服器負載|百分比|最大需求||ShardId|
|cacheWrite|快取寫入|每秒位元組|最大需求||ShardId|
|cacheRead|快取讀取|每秒位元組|最大需求||ShardId|
|percentProcessorTime|CPU|百分比|最大需求||ShardId|
|cacheLatency|快取延遲 (毫秒) (預覽)|[計數]|Average||ShardId|
|錯誤|錯誤|[計數]|最大需求||ShardId、ErrorType|
|connectedclients0|連線的用戶端 (分區 0)|[計數]|最大需求||無|
|totalcommandsprocessed0|總作業數 (分區 0)|[計數]|總數||無|
|cachehits0|快取點擊 (分區 0)|[計數]|總數||無|
|cachemisses0|快取遺漏 (分區 0)|[計數]|總數||無|
|getcommands0|取得 (分區 0)|[計數]|總數||無|
|setcommands0|設定 (分區 0)|[計數]|總數||無|
|operationsPerSecond0|每秒的作業數 (分區 0)|[計數]|最大需求||無|
|evictedkeys0|收回的金鑰 (分區 0)|[計數]|總數||無|
|totalkeys0|金鑰總數 (分區 0)|[計數]|最大需求||無|
|expiredkeys0|到期的金鑰 (分區 0)|[計數]|總數||無|
|usedmemory0|已用的記憶體 (分區 0)|位元組|最大需求||無|
|usedmemoryRss0|已用的記憶體 RSS (分區 0)|位元組|最大需求||無|
|serverLoad0|伺服器負載 (分區 0)|百分比|最大需求||無|
|cacheWrite0|快取寫入 (分區 0)|每秒位元組|最大需求||無|
|cacheRead0|快取讀取 (分區 0)|每秒位元組|最大需求||無|
|percentProcessorTime0|CPU (分區 0)|百分比|最大需求||無|
|connectedclients1|連線的用戶端 (分區 1)|[計數]|最大需求||無|
|totalcommandsprocessed1|總作業數 (分區 1)|[計數]|總數||無|
|cachehits1|快取點擊 (分區 1)|[計數]|總數||無|
|cachemisses1|快取遺漏 (分區 1)|[計數]|總數||無|
|getcommands1|取得 (分區 1)|[計數]|總數||無|
|setcommands1|設定 (分區 1)|[計數]|總數||無|
|operationsPerSecond1|每秒的作業數 (分區 1)|[計數]|最大需求||無|
|evictedkeys1|收回的金鑰 (分區 1)|[計數]|總數||無|
|totalkeys1|金鑰總數 (分區 1)|[計數]|最大需求||無|
|expiredkeys1|到期的金鑰 (分區 1)|[計數]|總數||無|
|usedmemory1|已用的記憶體 (分區 1)|位元組|最大需求||無|
|usedmemoryRss1|已用的記憶體 RSS (分區 1)|位元組|最大需求||無|
|serverLoad1|伺服器負載 (分區 1)|百分比|最大需求||無|
|cacheWrite1|快取寫入 (分區 1)|每秒位元組|最大需求||無|
|cacheRead1|快取讀取 (分區 1)|每秒位元組|最大需求||無|
|percentProcessorTime1|CPU (分區 1)|百分比|最大需求||無|
|connectedclients2|連線的用戶端 (分區 2)|[計數]|最大需求||無|
|totalcommandsprocessed2|總作業數 (分區 2)|[計數]|總數||無|
|cachehits2|快取點擊 (分區 2)|[計數]|總數||無|
|cachemisses2|快取遺漏 (分區 2)|[計數]|總數||無|
|getcommands2|取得 (分區 2)|[計數]|總數||無|
|setcommands2|設定 (分區 2)|[計數]|總數||無|
|operationsPerSecond2|每秒的作業數 (分區 2)|[計數]|最大需求||無|
|evictedkeys2|收回的金鑰 (分區 2)|[計數]|總數||無|
|totalkeys2|金鑰總數 (分區 2)|[計數]|最大需求||無|
|expiredkeys2|到期的金鑰 (分區 2)|[計數]|總數||無|
|usedmemory2|已用的記憶體 (分區 2)|位元組|最大需求||無|
|usedmemoryRss2|已用的記憶體 RSS (分區 2)|位元組|最大需求||無|
|serverLoad2|伺服器負載 (分區 2)|百分比|最大需求||無|
|cacheWrite2|快取寫入 (分區 2)|每秒位元組|最大需求||無|
|cacheRead2|快取讀取 (分區 2)|每秒位元組|最大需求||無|
|percentProcessorTime2|CPU (分區 2)|百分比|最大需求||無|
|connectedclients3|連線的用戶端 (分區 3)|[計數]|最大需求||無|
|totalcommandsprocessed3|總作業數 (分區 3)|[計數]|總數||無|
|cachehits3|快取點擊 (分區 3)|[計數]|總數||無|
|cachemisses3|快取遺漏 (分區 3)|[計數]|總數||無|
|getcommands3|取得 (分區 3)|[計數]|總數||無|
|setcommands3|設定 (分區 3)|[計數]|總數||無|
|operationsPerSecond3|每秒的作業數 (分區 3)|[計數]|最大需求||無|
|evictedkeys3|收回的金鑰 (分區 3)|[計數]|總數||無|
|totalkeys3|金鑰總數 (分區 3)|[計數]|最大需求||無|
|expiredkeys3|到期的金鑰 (分區 3)|[計數]|總數||無|
|usedmemory3|已用的記憶體 (分區 3)|位元組|最大需求||無|
|usedmemoryRss3|已用的記憶體 RSS (分區 3)|位元組|最大需求||無|
|serverLoad3|伺服器負載 (分區 3)|百分比|最大需求||無|
|cacheWrite3|快取寫入 (分區 3)|每秒位元組|最大需求||無|
|cacheRead3|快取讀取 (分區 3)|每秒位元組|最大需求||無|
|percentProcessorTime3|CPU (分區 3)|百分比|最大需求||無|
|connectedclients4|連線的用戶端 (分區 4)|[計數]|最大需求||無|
|totalcommandsprocessed4|總作業數 (分區 4)|[計數]|總數||無|
|cachehits4|快取點擊 (分區 4)|[計數]|總數||無|
|cachemisses4|快取遺漏 (分區 4)|[計數]|總數||無|
|getcommands4|取得 (分區 4)|[計數]|總數||無|
|setcommands4|設定 (分區 4)|[計數]|總數||無|
|operationsPerSecond4|每秒的作業數 (分區 4)|[計數]|最大需求||無|
|evictedkeys4|收回的金鑰 (分區 4)|[計數]|總數||無|
|totalkeys4|金鑰總數 (分區 4)|[計數]|最大需求||無|
|expiredkeys4|到期的金鑰 (分區 4)|[計數]|總數||無|
|usedmemory4|已用的記憶體 (分區 4)|位元組|最大需求||無|
|usedmemoryRss4|已用的記憶體 RSS (分區 4)|位元組|最大需求||無|
|serverLoad4|伺服器負載 (分區 4)|百分比|最大需求||無|
|cacheWrite4|快取寫入 (分區 4)|每秒位元組|最大需求||無|
|cacheRead4|快取讀取 (分區 4)|每秒位元組|最大需求||無|
|percentProcessorTime4|CPU (分區 4)|百分比|最大需求||無|
|connectedclients5|連線的用戶端 (分區 5)|[計數]|最大需求||無|
|totalcommandsprocessed5|總作業數 (分區 5)|[計數]|總數||無|
|cachehits5|快取點擊 (分區 5)|[計數]|總數||無|
|cachemisses5|快取遺漏 (分區 5)|[計數]|總數||無|
|getcommands5|取得 (分區 5)|[計數]|總數||無|
|setcommands5|設定 (分區 5)|[計數]|總數||無|
|operationsPerSecond5|每秒的作業數 (分區 5)|[計數]|最大需求||無|
|evictedkeys5|收回的金鑰 (分區 5)|[計數]|總數||無|
|totalkeys5|金鑰總數 (分區 5)|[計數]|最大需求||無|
|expiredkeys5|到期的金鑰 (分區 5)|[計數]|總數||無|
|usedmemory5|已用的記憶體 (分區 5)|位元組|最大需求||無|
|usedmemoryRss5|已用的記憶體 RSS (分區 5)|位元組|最大需求||無|
|serverLoad5|伺服器負載 (分區 5)|百分比|最大需求||無|
|cacheWrite5|快取寫入 (分區 5)|每秒位元組|最大需求||無|
|cacheRead5|快取讀取 (分區 5)|每秒位元組|最大需求||無|
|percentProcessorTime5|CPU (分區 5)|百分比|最大需求||無|
|connectedclients6|連線的用戶端 (分區 6)|[計數]|最大需求||無|
|totalcommandsprocessed6|總作業數 (分區 6)|[計數]|總數||無|
|cachehits6|快取點擊 (分區 6)|[計數]|總數||無|
|cachemisses6|快取遺漏 (分區 6)|[計數]|總數||無|
|getcommands6|取得 (分區 6)|[計數]|總數||無|
|setcommands6|設定 (分區 6)|[計數]|總數||無|
|operationsPerSecond6|每秒的作業數 (分區 6)|[計數]|最大需求||無|
|evictedkeys6|收回的金鑰 (分區 6)|[計數]|總數||無|
|totalkeys6|金鑰總數 (分區 6)|[計數]|最大需求||無|
|expiredkeys6|到期的金鑰 (分區 6)|[計數]|總數||無|
|usedmemory6|已用的記憶體 (分區 6)|位元組|最大需求||無|
|usedmemoryRss6|已用的記憶體 RSS (分區 6)|位元組|最大需求||無|
|serverLoad6|伺服器負載 (分區 6)|百分比|最大需求||無|
|cacheWrite6|快取寫入 (分區 6)|每秒位元組|最大需求||無|
|cacheRead6|快取讀取 (分區 6)|每秒位元組|最大需求||無|
|percentProcessorTime6|CPU (分區 6)|百分比|最大需求||無|
|connectedclients7|連線的用戶端 (分區 7)|[計數]|最大需求||無|
|totalcommandsprocessed7|總作業數 (分區 7)|[計數]|總數||無|
|cachehits7|快取點擊 (分區 7)|[計數]|總數||無|
|cachemisses7|快取遺漏 (分區 7)|[計數]|總數||無|
|getcommands7|取得 (分區 7)|[計數]|總數||無|
|setcommands7|設定 (分區 7)|[計數]|總數||無|
|operationsPerSecond7|每秒的作業數 (分區 7)|[計數]|最大需求||無|
|evictedkeys7|收回的金鑰 (分區 7)|[計數]|總數||無|
|totalkeys7|金鑰總數 (分區 7)|[計數]|最大需求||無|
|expiredkeys7|到期的金鑰 (分區 7)|[計數]|總數||無|
|usedmemory7|已用的記憶體 (分區 7)|位元組|最大需求||無|
|usedmemoryRss7|已用的記憶體 RSS (分區 7)|位元組|最大需求||無|
|serverLoad7|伺服器負載 (分區 7)|百分比|最大需求||無|
|cacheWrite7|快取寫入 (分區 7)|每秒位元組|最大需求||無|
|cacheRead7|快取讀取 (分區 7)|每秒位元組|最大需求||無|
|percentProcessorTime7|CPU (分區 7)|百分比|最大需求||無|
|connectedclients8|連線的用戶端 (分區 8)|[計數]|最大需求||無|
|totalcommandsprocessed8|總作業數 (分區 8)|[計數]|總數||無|
|cachehits8|快取點擊 (分區 8)|[計數]|總數||無|
|cachemisses8|快取遺漏 (分區 8)|[計數]|總數||無|
|getcommands8|取得 (分區 8)|[計數]|總數||無|
|setcommands8|設定 (分區 8)|[計數]|總數||無|
|operationsPerSecond8|每秒的作業數 (分區 8)|[計數]|最大需求||無|
|evictedkeys8|收回的金鑰 (分區 8)|[計數]|總數||無|
|totalkeys8|金鑰總數 (分區 8)|[計數]|最大需求||無|
|expiredkeys8|到期的金鑰 (分區 8)|[計數]|總數||無|
|usedmemory8|已用的記憶體 (分區 8)|位元組|最大需求||無|
|usedmemoryRss8|已用的記憶體 RSS (分區 8)|位元組|最大需求||無|
|serverLoad8|伺服器負載 (分區 8)|百分比|最大需求||無|
|cacheWrite8|快取寫入 (分區 8)|每秒位元組|最大需求||無|
|cacheRead8|快取讀取 (分區 8)|每秒位元組|最大需求||無|
|percentProcessorTime8|CPU (分區 8)|百分比|最大需求||無|
|connectedclients9|連線的用戶端 (分區 9)|[計數]|最大需求||無|
|totalcommandsprocessed9|總作業數 (分區 9)|[計數]|總數||無|
|cachehits9|快取點擊 (分區 9)|[計數]|總數||無|
|cachemisses9|快取遺漏 (分區 9)|[計數]|總數||無|
|getcommands9|取得 (分區 9)|[計數]|總數||無|
|setcommands9|設定 (分區 9)|[計數]|總數||無|
|operationsPerSecond9|每秒的作業數 (分區 9)|[計數]|最大需求||無|
|evictedkeys9|收回的金鑰 (分區 9)|[計數]|總數||無|
|totalkeys9|金鑰總數 (分區 9)|[計數]|最大需求||無|
|expiredkeys9|到期的金鑰 (分區 9)|[計數]|總數||無|
|usedmemory9|已用的記憶體 (分區 9)|位元組|最大需求||無|
|usedmemoryRss9|已用的記憶體 RSS (分區 9)|位元組|最大需求||無|
|serverLoad9|伺服器負載 (分區 9)|百分比|最大需求||無|
|cacheWrite9|快取寫入 (分區 9)|每秒位元組|最大需求||無|
|cacheRead9|快取讀取 (分區 9)|每秒位元組|最大需求||無|
|percentProcessorTime9|CPU (分區 9)|百分比|最大需求||無|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|WebApplicationFirewallRequestCount|Web 應用程式防火牆要求計數|[計數]|總數|Web 應用程式防火牆處理的用戶端要求數|PolicyName、RuleName、Action|
...
|Network In|Network In|位元組|總數|虛擬機器 (連入流量) 在所有網路介面上接收到的位元組數目。|無|
|Network Out|Network Out|位元組|總數|虛擬機器 (連出流量) 在所有網路介面上送出的位元組數目。|無|
...
|Network In|Network In|位元組|總數|虛擬機器 (連入流量) 在所有網路介面上接收到的位元組數目。|RoleInstanceId|
|Network Out|Network Out|位元組|總數|虛擬機器 (連出流量) 在所有網路介面上送出的位元組數目。|RoleInstanceId|
...
|交易|交易|[計數]|總數|向儲存體服務或所指定 API 作業傳送的要求數。 此數目包括成功與失敗的要求，以及產生錯誤的要求。 使用 ResponseType 維度來取得不同類型回應的數目。|ResponseType、GeoType、ApiName、Authentication|
|輸入|輸入|位元組|總數|輸入資料量 (位元組)。 此數目包括從外部用戶端輸入到 Azure 儲存體與 Azure 內的輸入。|GeoType、ApiName、Authentication|
|輸出|輸出|位元組|總數|輸出資料量 (位元組)。 此數目包括從外部用戶端輸出到 Azure 儲存體與 Azure 內的輸出。 因此，此數目未反映可收費的輸出。|GeoType、ApiName、Authentication|
...
|交易|交易|[計數]|總數|向儲存體服務或所指定 API 作業傳送的要求數。 此數目包括成功與失敗的要求，以及產生錯誤的要求。 使用 ResponseType 維度來取得不同類型回應的數目。|ResponseType、GeoType、ApiName、Authentication、檔案共用|
|輸入|輸入|位元組|總數|輸入資料量 (位元組)。 此數目包括從外部用戶端輸入到 Azure 儲存體與 Azure 內的輸入。|GeoType、ApiName、Authentication、檔案共用|
|輸出|輸出|位元組|總數|輸出資料量 (位元組)。 此數目包括從外部用戶端輸出到 Azure 儲存體與 Azure 內的輸出。 因此，此數目未反映可收費的輸出。|GeoType、ApiName、Authentication、檔案共用|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|TotalCalls|呼叫總數|[計數]|總數|呼叫總數。|ApiName、OperationName、Region|
|SuccessfulCalls|成功的呼叫|[計數]|總數|成功的呼叫數。|ApiName、OperationName、Region|
|TotalErrors|錯誤總數|[計數]|總數|具有錯誤回應 (HTTP 回應碼 4xx 或 5xx) 的呼叫總數。|ApiName、OperationName、Region|
|BlockedCalls|已封鎖的呼叫|[計數]|總數|超過速率或配額限制的呼叫數目。|ApiName、OperationName、Region|
|ServerErrors|伺服器錯誤|[計數]|總數|具有服務內部錯誤 (HTTP 回應碼 5xx) 的呼叫數目。|ApiName、OperationName、Region|
|ClientErrors|用戶端錯誤|[計數]|總數|具有用戶端錯誤 (HTTP 回應碼 4xx) 的呼叫數目。|ApiName、OperationName、Region|
|DataIn|資料輸入|位元組|總數|內送資料大小，以位元組為單位。|ApiName、OperationName、Region|
|DataOut|資料輸出|位元組|總數|輸出資料大小，以位元組為單位。|ApiName、OperationName、Region|
|延遲|延遲|毫秒|Average|延遲，以毫秒為單位。|ApiName、OperationName、Region|
|CharactersTranslated|轉譯字元數|[計數]|總數|傳入文字要求中的字元總數。|ApiName、OperationName、Region|
|CharactersTrained|已定型的字元|[計數]|總數|已定型的字元總數。|ApiName、OperationName、Region|
|SpeechSessionDuration|語音工作階段持續時間|秒|總數|語音工作階段的總持續時間 (秒)。|ApiName、OperationName、Region|
|TotalTransactions|交易數總計|[計數]|總數|交易總數。|無|
|TotalTokenCalls|呼叫權杖總計|[計數]|總數|呼叫權杖次數總計。|ApiName、OperationName、Region|
...
|Network In|計費中的網路（已淘汰）|位元組|總數|虛擬機器在所有網路介面上接收的可計費位元組數（連入流量）（已淘汰）|無|
|Network Out|網路輸出計費（已淘汰）|位元組|總數|虛擬機器（連出流量）在所有網路介面上傳出的可計費位元組數（已淘汰）|無|
|Disk Read Bytes|Disk Read Bytes|位元組|總數|在監視期間從磁片讀取的位元組數|無|
|Disk Write Bytes|Disk Write Bytes|位元組|總數|在監視期間寫入磁片的位元組數|無|
|Disk Read Operations/Sec|Disk Read Operations/Sec|每秒計數|Average|磁碟讀取 IOPS|無|
|Disk Write Operations/Sec|Disk Write Operations/Sec|每秒計數|Average|磁碟寫入 IOPS|無|
...
|網路總計|網路總計|位元組|總數|虛擬機器 (連入流量) 在所有網路介面上接收到的位元組數目|無|
|網路輸出總計|網路輸出總計|位元組|總數|虛擬機器 (連出流量) 在所有網路介面上送出的位元組數目|無|
...
|Network In|計費中的網路（已淘汰）|位元組|總數|虛擬機器在所有網路介面上接收的可計費位元組數（連入流量）（已淘汰）|VMName|
|Network Out|網路輸出計費（已淘汰）|位元組|總數|虛擬機器（連出流量）在所有網路介面上傳出的可計費位元組數（已淘汰）|VMName|
|Disk Read Bytes|Disk Read Bytes|位元組|總數|在監視期間從磁片讀取的位元組數|VMName|
|Disk Write Bytes|Disk Write Bytes|位元組|總數|在監視期間寫入磁片的位元組數|VMName|
...
|網路總計|網路總計|位元組|總數|虛擬機器 (連入流量) 在所有網路介面上接收到的位元組數目|VMName|
|網路輸出總計|網路輸出總計|位元組|總數|虛擬機器 (連出流量) 在所有網路介面上送出的位元組數目|VMName|
...
|RunDuration|執行持續期間|毫秒|總數|執行持續時間（毫秒）|無|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|SuccessfullRequests|成功的要求|[計數]|總數|自訂提供者提出的成功要求|HttpMethod、CallPath、StatusCode|
|FailedRequests|失敗的要求|[計數]|總數|取得自訂資源提供者的可用記錄|HttpMethod、CallPath、StatusCode|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|FailedRuns|失敗的執行|[計數]|總數||pipelineName、activityName|
|SuccessfulRuns|成功的執行|[計數]|總數||pipelineName、activityName|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|PipelineFailedRuns|失敗的管線執行計量|[計數]|總數||FailureType、Name|
|PipelineSucceededRuns|成功的管線執行計量|[計數]|總數||FailureType、Name|
|PipelineCancelledRuns|已取消的管線執行計量|[計數]|總數||FailureType、Name|
|ActivityFailedRuns|失敗的活動執行計量|[計數]|總數||ActivityType、PipelineName、FailureType、Name|
|ActivitySucceededRuns|成功的活動執行計量|[計數]|總數||ActivityType、PipelineName、FailureType、Name|
|ActivityCancelledRuns|已取消的活動執行計量|[計數]|總數||ActivityType、PipelineName、FailureType、Name|
|TriggerFailedRuns|失敗的觸發程序執行計量|[計數]|總數||Name、FailureType|
|TriggerSucceededRuns|成功的觸發程序執行計量|[計數]|總數||Name、FailureType|
|TriggerCancelledRuns|已取消的觸發程式會執行計量|[計數]|總數||Name、FailureType|
...
|MaxAllowedResourceCount|允許的實體數目上限|[計數]|最大需求||無|
|MaxAllowedFactorySizeInGbUnits|允許的 factory 大小上限（GB 單位）|[計數]|最大需求||無|
|ResourceCount|總實體數|[計數]|最大需求||無|
|FactorySizeInGbUnits|總廠大小（GB 單位）|[計數]|最大需求||無|

## <a name="microsoftdatalakeanalyticsaccounts"></a>Microsoft.DataLakeAnalytics/accounts

|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|JobEndedSuccess|成功的作業|[計數]|總數|成功作業計數。|無|
|JobEndedFailure|失敗的作業|[計數]|總數|失敗作業計數。|無|
|JobEndedCancelled|取消的作業|[計數]|總數|取消作業計數。|無|
|JobAUEndedSuccess|成功 AU 時間|秒|總數|成功作業的 AU 時間總計。|無|
|JobAUEndedFailure|失敗 AU 時間|秒|總數|失敗作業的 AU 時間總計。|無|
|JobAUEndedCancelled|取消的 AU 時間|秒|總數|取消作業的 AU 時間總計。|無|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|TotalStorage|儲存體總計|位元組|最大需求|儲存在帳戶中的總資料量。|無|
|DataWritten|寫入的資料|位元組|總數|寫入帳戶的總資料量。|無|
|DataRead|讀取的資料|位元組|總數|從帳戶讀取的總資料量。|無|
|WriteRequests|寫入要求|[計數]|總數|帳戶的資料寫入要求計數。|無|
|ReadRequests|讀取要求|[計數]|總數|帳戶的資料讀取要求計數。|無|
...
|storage_used|已使用儲存體|位元組|Average|已使用儲存體|無|
|storage_limit|儲存體限制|位元組|最大需求|儲存體限制|無|
...
|connections_failed|失敗的連線|[計數]|總數|失敗的連線|無|
|seconds_behind_master|複寫延遲 (秒)|[計數]|最大需求|複寫延遲 (秒)|無|
|backup_storage_used|已使用的備份儲存體|位元組|Average|已使用的備份儲存體|無|
|network_bytes_egress|Network Out|位元組|總數|跨作用中連線的網路輸出|無|
|network_bytes_ingress|Network In|位元組|總數|跨作用中連線的網路輸入|無|
...
|serverlog_storage_percent|伺服器記錄儲存體百分比|百分比|Average|伺服器記錄儲存體百分比|無|
|serverlog_storage_usage|使用的伺服器記錄儲存體|位元組|Average|使用的伺服器記錄儲存體|無|
|serverlog_storage_limit|伺服器記錄儲存體限制|位元組|最大需求|伺服器記錄儲存體限制|無|
|active_connections|作用中的連線|[計數]|Average|作用中的連線|無|
|connections_failed|失敗的連線|[計數]|總數|失敗的連線|無|
|seconds_behind_master|複寫延遲 (秒)|[計數]|最大需求|複寫延遲 (秒)|無|
|backup_storage_used|已使用的備份儲存體|位元組|Average|已使用的備份儲存體|無|
...
|network_bytes_egress|Network Out|位元組|總數|跨作用中連線的網路輸出|無|
|network_bytes_ingress|Network In|位元組|總數|跨作用中連線的網路輸入|無|
|pg_replica_log_delay_in_seconds|複本延隔時間|秒|最大需求|複本延遲（秒）|無|
|pg_replica_log_delay_in_bytes|複本之間的最大延隔時間|位元組|最大需求|最延遲複本的延遲（以位元組為單位）|無|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|d2c.telemetry.ingress.allProtocol|遙測訊息傳送嘗試|[計數]|總數|要嘗試傳送至您 IoT 中樞的裝置到雲端遙測訊息數目|無|
|d2c.telemetry.ingress.success|已傳送的遙測訊息|[計數]|總數|成功傳送至您 IoT 中樞的裝置到雲端遙測訊息數目|無|
|c2d.commands.egress.complete.success|已完成 C2D 訊息傳遞|[計數]|總數|裝置已成功完成的雲端到裝置訊息傳遞數|無|
|c2d.commands.egress.abandon.success|已放棄的 C2D 訊息|[計數]|總數|裝置放棄的雲端到裝置訊息數|無|
|c2d.commands.egress.reject.success|拒絕的 C2D 訊息|[計數]|總數|裝置拒絕的雲端到裝置訊息數目|無|
|C2DMessagesExpired|C2D 郵件已過期（預覽）|[計數]|總數|已過期的雲端到裝置訊息數目|無|
|devices.totalDevices|裝置總計 (已淘汰)|[計數]|總數|向 IoT 中樞註冊的裝置數目|無|
|devices.connectedDevices.allProtocol|連接的裝置 (已淘汰) |[計數]|總數|連接至 IoT 中樞的裝置數目|無|
|d2c.telemetry.egress.success|路由：已傳遞的遙測訊息|[計數]|總數|使用 IoT 中樞路由成功地將訊息傳遞到所有端點的次數。 如果將訊息路由至多個端點，這個值會為每一次成功傳遞加 1。 如果將訊息多次傳遞到同一個端點，這個值會為每一次成功傳遞加 1。|無|
|d2c.telemetry.egress.dropped|路由：已捨棄的遙測訊息 |[計數]|總數|IoT 中樞路由因為端點無效而捨棄訊息的次數。 這個值不會計算傳遞到後援路由的訊息，因為捨棄的訊息不會傳遞到那裡。|無|
|d2c.telemetry.egress.orphaned|路由：已遺棄的遙測訊息 |[計數]|總數|IoT 中樞路由因為訊息不符合任何路由規則 (包括後援規則) 而遺棄訊息的次數。 |無|
|d2c.telemetry.egress.invalid|路由：不相容的遙測訊息|[計數]|總數|IoT 中樞路由因為與端點不相容而無法傳遞訊息的次數。 這個值不包含重試次數。|無|
|d2c.telemetry.egress.fallback|路由：傳遞至後援的訊息|[計數]|總數|IoT 中樞路由傳遞訊息至與後援路由相關聯之端點的次數。|無|
|d2c.endpoints.egress.eventHubs|路由：傳遞至事件中樞的訊息|[計數]|總數|IoT 中樞路由成功將訊息傳遞至事件中樞端點的次數。|無|
|d2c.endpoints.latency.eventHubs|路由：事件中樞的訊息延遲|毫秒|Average|訊息輸入到 IoT 中樞與訊息輸入到事件中樞端點之間的平均延遲 (毫秒)。|無|
|d2c.endpoints.egress.serviceBusQueues|路由：傳遞至服務匯流排佇列的訊息|[計數]|總數|IoT 中樞路由成功將訊息傳遞至服務匯流排佇列端點的次數。|無|
|d2c.endpoints.latency.serviceBusQueues|路由：服務匯流排佇列的訊息延遲|毫秒|Average|訊息輸入到 IoT 中樞與遙測訊息輸入到服務匯流排佇列端點之間的平均延遲 (毫秒)。|無|
|d2c.endpoints.egress.serviceBusTopics|路由：傳遞至服務匯流排主題的訊息|[計數]|總數|IoT 中樞路由成功將訊息傳遞至服務匯流排主題端點的次數。|無|
|d2c.endpoints.latency.serviceBusTopics|路由：服務匯流排主題的訊息延遲|毫秒|Average|訊息輸入到 IoT 中樞與遙測訊息輸入到服務匯流排佇列端點之間的平均延遲 (毫秒)。|無|
|d2c.endpoints.egress.builtIn.events|路由：傳遞至訊息/事件的訊息|[計數]|總數|IoT 中樞路由成功將訊息傳遞至內建端點 (訊息/事件) 的次數。|無|
|d2c.endpoints.latency.builtIn.events|路由：訊息/事件的訊息延遲|毫秒|Average|訊息輸入到 IoT 中樞與遙測訊息輸入到內建端點 (訊息/事件) 之間的平均延遲 (毫秒)。|無|
|d2c.endpoints.egress.storage|路由：傳遞至儲存體的訊息|[計數]|總數|IoT 中樞路由成功將訊息傳遞至儲存體端點的次數。|無|
|d2c.endpoints.latency.storage|路由：儲存體的訊息延遲|毫秒|Average|訊息輸入到 IoT 中樞與遙測訊息輸入到儲存體端點之間的平均延遲 (毫秒)。|無|
|d2c.endpoints.egress.storage.bytes|路由：傳遞至儲存體的資料|位元組|總數|IoT 中樞路由傳遞至儲存體端點的資料量 (位元組)。|無|
|d2c.endpoints.egress.storage.blobs|路由：傳遞至儲存體的 BLOB|[計數]|總數|IoT 中樞路由將 BLOB 傳遞至儲存體端點的次數。|無|
|EventGridDeliveries|事件方格傳遞（預覽）|[計數]|總數|已發佈至事件方格的 IoT 中樞事件數目。 使用 [結果] 維度來取得成功和失敗的要求數目。 [事件種類] 維度會顯示事件的類型（ https://aka.ms/ioteventgrid)。|ResourceId、結果、事件識別碼|
...
|d2c.twin.read.success|裝置的成功對應項讀取|[計數]|總數|裝置起始的所有成功對應項讀取的計數。|無|
|d2c.twin.read.failure|裝置的失敗對應項讀取|[計數]|總數|裝置起始的所有失敗對應項讀取的計數。|無|
|d2c.twin.read.size|裝置的對應項讀取回應大小|位元組|Average|裝置起始的所有成功對應項讀取的平均值、最小值和最大值。|無|
|d2c.twin.update.success|裝置的成功對應項更新|[計數]|總數|裝置起始的所有成功對應項更新的計數。|無|
|d2c.twin.update.failure|裝置的失敗對應項更新|[計數]|總數|裝置起始的所有失敗對應項更新的計數。|無|
|d2c.twin.update.size|裝置的對應項更新大小|位元組|Average|裝置起始的所有成功對應項更新的大小平均值、最小值和最大值。|無|
|c2d.methods.success|成功直接方法叫用|[計數]|總數|所有成功直接方法呼叫的計數。|無|
|c2d.methods.failure|失敗直接方法叫用|[計數]|總數|所有失敗直接方法呼叫的計數。|無|
...
|c2d.twin.read.success|後端的成功對應項讀取|[計數]|總數|後端起始的所有成功對應項讀取的計數。|無|
|c2d.twin.read.failure|後端的失敗對應項讀取|[計數]|總數|後端起始的所有失敗對應項讀取的計數。|無|
|c2d.twin.read.size|後端的對應項讀取回應大小|位元組|Average|後端起始的所有成功對應項讀取的平均值、最小值和最大值。|無|
|c2d.twin.update.success|後端的成功對應項更新|[計數]|總數|後端起始的所有成功對應項更新的計數。|無|
|c2d.twin.update.failure|後端的失敗對應項更新|[計數]|總數|後端起始的所有失敗對應項更新的計數。|無|
|c2d.twin.update.size|後端的對應項更新大小|位元組|Average|後端起始的所有成功對應項更新的大小平均值、最小值和最大值。|無|
|twinQueries.success|成功對應項查詢|[計數]|總數|所有成功對應項查詢的計數。|無|
|twinQueries.failure|失敗對應項查詢|[計數]|總數|所有失敗對應項查詢的計數。|無|
|twinQueries.resultSize|對應項查詢結果大小|位元組|Average|所有成功對應項查詢的結果大小平均值、最小值和最大值。|無|
|jobs.createTwinUpdateJob.success|成功建立的對應項更新作業|[計數]|總數|所有成功建立的對應項更新作業計數。|無|
|jobs.createTwinUpdateJob.failure|建立失敗的對應項更新作業|[計數]|總數|所有建立失敗的對應項更新作業計數。|無|
|jobs.createDirectMethodJob.success|成功建立的方法叫用作業|[計數]|總數|所有成功建立的直接方法叫用作業計數。|無|
|jobs.createDirectMethodJob.failure|建立失敗的方法叫用作業|[計數]|總數|所有建立失敗的直接方法叫用作業計數。|無|
|jobs.listJobs.success|成功呼叫列出作業|[計數]|總數|所有成功呼叫列出作業的計數。|無|
|jobs.listJobs.failure|呼叫列出作業失敗|[計數]|總數|所有呼叫列出作業失敗的計數。|無|
|jobs.cancelJob.success|成功取消作業|[計數]|總數|所有成功呼叫取消作業的計數。|無|
|jobs.cancelJob.failure|取消作業失敗|[計數]|總數|所有呼叫取消作業失敗的計數。|無|
|jobs.queryJobs.success|成功作業查詢|[計數]|總數|所有成功呼叫查詢作業的計數。|無|
|jobs.queryJobs.failure|失敗作業查詢|[計數]|總數|所有呼叫查詢作業失敗的計數。|無|
|jobs.completed|已完成的工作|[計數]|總數|所有已完成的作業計數。|無|
|jobs.failed|失敗作業|[計數]|總數|所有失敗作業計數。|無|
|d2c.telemetry.ingress.sendThrottle|節流錯誤數目|[計數]|總數|因裝置輸送量節流而導致的節流錯誤數目|無|
|dailyMessageQuotaUsed|已使用的訊息總數|[計數]|Average|今日已使用的總訊息數|無|
|deviceDataUsage|裝置資料使用量總計|位元組|總數|傳輸至任何已連線至 IotHub 之裝置或從該裝置傳出的位元組數|無|
|deviceDataUsageV2|裝置資料使用量總計 (預覽)|位元組|總數|傳輸至任何已連線至 IotHub 之裝置或從該裝置傳出的位元組數|無|
|totalDeviceCount|裝置總計 (預覽)|[計數]|Average|向 IoT 中樞註冊的裝置數目|無|
|connectedDeviceCount|連接的裝置 (預覽)|[計數]|Average|連接至 IoT 中樞的裝置數目|無|
|組態|設定計量|[計數]|總數|設定作業的計量|無|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|RegistrationAttempts|註冊嘗試數|[計數]|總數|嘗試的裝置註冊數|ProvisioningServiceName、IotHubName、Status|
|DeviceAssignments|已指派的裝置數|[計數]|總數|已指派到 IoT 中樞的裝置數|ProvisioningServiceName、IotHubName|
|AttestationAttempts|證明嘗試數|[計數]|總數|嘗試的裝置證明數|ProvisioningServiceName，狀態，通訊協定|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|AddRegion|已新增區域|[計數]|[計數]|已新增區域|區域|
|AvailableStorage|[可用存放裝置]|位元組|總數|在5分鐘資料細微性報告的可用儲存空間總計|CollectionName、DatabaseName、Region|
|CassandraConnectionClosures|Cassandra 連線終止|[計數]|總數|已關閉的 Cassandra 連接數，以1分鐘的資料細微性回報|APIType、Region、ClosureReason|
|CassandraRequestCharges|Cassandra 要求費用|[計數]|總數|針對提出的 Cassandra 要求所耗用的 ru|APIType、DatabaseName、CollectionName、Region、OperationType、ResourceType|
...
|DataUsage|資料使用量|位元組|總數|5分鐘資料細微性報告的總數據使用量|CollectionName、DatabaseName、Region|
|DeleteAccount|帳戶已刪除|[計數]|[計數]|帳戶已刪除|無|
|DocumentCount|文件計數|[計數]|總數|在5分鐘資料細微性報告的檔計數總計|CollectionName、DatabaseName、Region|
|DocumentQuota|文件配額|位元組|總數|5分鐘資料細微性報告的總儲存體配額|CollectionName、DatabaseName、Region|
|IndexUsage|索引使用量|位元組|總數|5分鐘資料細微性報告的索引使用量總計|CollectionName、DatabaseName、Region|
...
|MongoRequestCharge|Mongo 要求收費|[計數]|總數|已使用的 Mongo 要求單位|DatabaseName、CollectionName、Region、CommandName、ErrorCode|
...
|ProvisionedThroughput|佈建的輸送量|[計數]|最大需求|佈建的輸送量|DatabaseName、CollectionName|
|RegionFailover|區域已故障切換|[計數]|[計數]|區域已故障切換|無|
|RemoveRegion|區域已移除|[計數]|[計數]|區域已移除|區域|
...
|TotalRequestUnits|要求單位總計|[計數]|總數|已使用的要求單位|DatabaseName、CollectionName、Region、StatusCode、OperationType|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|PublishSuccessCount|已發佈的事件|[計數]|總數|發佈至本主題的事件總數|主題|
|PublishFailCount|發行失敗的事件|[計數]|總數|發佈至本主題失敗的事件總數|主題、ErrorType、錯誤|
|PublishSuccessLatencyInMs|發佈成功延遲|毫秒|總數|發佈成功延遲（以毫秒為單位）|無|
|MatchedEventCount|相符的事件|[計數]|總數|與此事件訂閱相符的事件總數|主題、EventSubscriptionName、DomainEventSubscriptionName|
|DeliveryAttemptFailCount|傳遞失敗的事件|[計數]|總數|傳遞到此事件訂閱失敗的事件總數|主題、EventSubscriptionName、DomainEventSubscriptionName、Error、ErrorType|
|DeliverySuccessCount|已傳遞的事件|[計數]|總數|已傳遞到此事件訂閱的事件總數|主題、EventSubscriptionName、DomainEventSubscriptionName|
...
|DroppedEventCount|卸除的事件|[計數]|總數|與此事件訂閱相符的已卸除事件總數|主題、EventSubscriptionName、DomainEventSubscriptionName、DropReason|
|DeadLetteredCount|失效信件事件|[計數]|總數|與此事件訂閱相符的失效信件事件總數|主題、EventSubscriptionName、DomainEventSubscriptionName、DeadLetterReason|

## <a name="microsofteventgridtopics"></a>Microsoft.EventGrid/topics

|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|PublishSuccessCount|已發佈的事件|[計數]|總數|發佈至本主題的事件總數|無|
|PublishFailCount|發行失敗的事件|[計數]|總數|發佈至本主題失敗的事件總數|ErrorType，錯誤|
|UnmatchedEventCount|不相符的事件|[計數]|總數|不符合本主題中任何事件訂閱的事件總數|無|
|PublishSuccessLatencyInMs|發佈成功延遲|毫秒|總數|發佈成功延遲（以毫秒為單位）|無|
|MatchedEventCount|相符的事件|[計數]|總數|與此事件訂閱相符的事件總數|EventSubscriptionName|
|DeliveryAttemptFailCount|傳遞失敗的事件|[計數]|總數|傳遞到此事件訂閱失敗的事件總數|Error、ErrorType、EventSubscriptionName|
|DeliverySuccessCount|已傳遞的事件|[計數]|總數|已傳遞到此事件訂閱的事件總數|EventSubscriptionName|
|DestinationProcessingDurationInMs|目的端處理持續期間|毫秒|Average|目的端處理持續期間，以毫秒為單位|EventSubscriptionName|
|DroppedEventCount|卸除的事件|[計數]|總數|與此事件訂閱相符的已卸除事件總數|DropReason,EventSubscriptionName|
|DeadLetteredCount|失效信件事件|[計數]|總數|與此事件訂閱相符的失效信件事件總數|DeadLetterReason,EventSubscriptionName|

## <a name="microsofteventgrideventsubscriptions"></a>Microsoft.EventGrid/eventSubscriptions

|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|MatchedEventCount|相符的事件|[計數]|總數|與此事件訂閱相符的事件總數|無|
|DeliveryAttemptFailCount|傳遞失敗的事件|[計數]|總數|傳遞到此事件訂閱失敗的事件總數|Error、ErrorType|
|DeliverySuccessCount|已傳遞的事件|[計數]|總數|已傳遞到此事件訂閱的事件總數|無|
|DestinationProcessingDurationInMs|目的端處理持續期間|毫秒|Average|目的端處理持續期間，以毫秒為單位|無|
|DroppedEventCount|卸除的事件|[計數]|總數|與此事件訂閱相符的已卸除事件總數|DropReason|
|DeadLetteredCount|失效信件事件|[計數]|總數|與此事件訂閱相符的失效信件事件總數|DeadLetterReason|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|SuccessfulRequests|成功的要求|[計數]|總數|Microsoft.EventHub 的成功要求。|EntityName、OperationResult|
|ServerErrors|伺服器錯誤。|[計數]|總數|Microsoft.EventHub 的伺服器錯誤。|EntityName、OperationResult|
|UserErrors|使用者錯誤。|[計數]|總數|Microsoft.EventHub 的使用者錯誤。|EntityName、OperationResult|
|QuotaExceededErrors|超出配額的錯誤。|[計數]|總數|Microsoft.EventHub 超出配額的錯誤。|EntityName、OperationResult|
|ThrottledRequests|節流的要求。|[計數]|總數|Microsoft.EventHub 的節流要求。|EntityName、OperationResult|
|IncomingRequests|傳入的要求|[計數]|總數|Microsoft.EventHub 的傳入要求。|EntityName|
|IncomingMessages|內送訊息|[計數]|總數|Microsoft.EventHub 的傳入訊息。|EntityName|
|OutgoingMessages|外寄訊息|[計數]|總數|Microsoft.EventHub 的傳出訊息。|EntityName|
|IncomingBytes|傳入位元組數。|位元組|總數|Microsoft.EventHub 的傳入位元組數。|EntityName|
|OutgoingBytes|傳出位元組數。|位元組|總數|Microsoft.EventHub 的傳出位元組數。|EntityName|
...
|CaptureBacklog|擷取待辦項目。|[計數]|總數|Microsoft.EventHub 的擷取待辦項目。|EntityName|
|CapturedMessages|已擷取的訊息。|[計數]|總數|Microsoft.EventHub 的已擷取訊息。|EntityName|
|CapturedBytes|已擷取的位元組。|位元組|總數|Microsoft.EventHub 的已擷取位元組。|EntityName|
|大小|大小|位元組|Average|EventHub 的大小 (以位元組為單位)。|EntityName|
|INREQS|傳入要求（已淘汰）|[計數]|總數|命名空間的連入傳送要求總數（已淘汰）|無|
|SUCCREQ|成功的要求（已淘汰）|[計數]|總數|命名空間的成功要求總數（已淘汰）|無|
|FAILREQ|失敗的要求（已淘汰）|[計數]|總數|命名空間的失敗要求總計（已淘汰）|無|
|SVRBSY|伺服器忙碌錯誤（已淘汰）|[計數]|總數|命名空間的伺服器忙碌錯誤總計（已淘汰）|無|
|INTERR|內部伺服器錯誤（已淘汰）|[計數]|總數|命名空間的內部伺服器錯誤總數（已淘汰）|無|
|MISCERR|其他錯誤（已淘汰）|[計數]|總數|命名空間的失敗要求總計（已淘汰）|無|
|INMSGS|傳入訊息（已過時）（已淘汰）|[計數]|總數|命名空間的傳入訊息總數。 此計量已過時。 請改用傳入訊息計量（已淘汰）|無|
|EHINMSGS|連入訊息數 (已過時)|[計數]|總數|命名空間的傳入訊息總計（已淘汰）|無|
|OUTMSGS|外寄訊息（已過時）（已被取代）|[計數]|總數|命名空間的傳出訊息總數。 此計量已過時。 請改用外寄訊息度量（已淘汰）|無|
|EHOUTMSGS|傳出訊息數 (已過時)|[計數]|總數|命名空間的傳出訊息總計（已淘汰）|無|
|EHINMBS|傳入位元組數（已過時）（已被取代）|位元組|總數|命名空間的事件中樞傳入訊息輸送量。 此計量已過時。 請改用傳入位元組度量（已淘汰）|無|
|EHINBYTES|傳入位元組數 (已過時)|位元組|總數|命名空間的事件中樞傳入訊息輸送量（已淘汰）|無|
|EHOUTMBS|傳出位元組數（已過時）（已被取代）|位元組|總數|命名空間的事件中樞傳出訊息輸送量。 此計量已過時。 請改用傳出位元組計量（已淘汰）|無|
|EHOUTBYTES|傳出位元組數 (已過時)|位元組|總數|命名空間的事件中樞傳出訊息輸送量（已淘汰）|無|
|EHABL|封存待處理專案訊息（已淘汰）|[計數]|總數|命名空間的待處理專案中的事件中樞封存訊息（已淘汰）|無|
|EHAMSGS|封存訊息（已淘汰）|[計數]|總數|命名空間中的事件中樞封存訊息（已淘汰）|無|
|EHAMBS|封存訊息輸送量（已淘汰）|位元組|總數|命名空間中的事件中樞封存訊息輸送量（已淘汰）|無|

## <a name="microsofteventhubclusters"></a>Microsoft.EventHub/clusters

|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|SuccessfulRequests|成功的要求|[計數]|總數|Microsoft.EventHub 的成功要求。|OperationResult|
|ServerErrors|伺服器錯誤。|[計數]|總數|Microsoft.EventHub 的伺服器錯誤。|OperationResult|
|UserErrors|使用者錯誤。|[計數]|總數|Microsoft.EventHub 的使用者錯誤。|OperationResult|
|QuotaExceededErrors|超出配額的錯誤。|[計數]|總數|Microsoft.EventHub 超出配額的錯誤。|OperationResult|
|ThrottledRequests|節流的要求。|[計數]|總數|Microsoft.EventHub 的節流要求。|OperationResult|
|IncomingRequests|傳入的要求|[計數]|總數|Microsoft.EventHub 的傳入要求。|無|
|IncomingMessages|內送訊息|[計數]|總數|Microsoft.EventHub 的傳入訊息。|無|
|OutgoingMessages|外寄訊息|[計數]|總數|Microsoft.EventHub 的傳出訊息。|無|
|IncomingBytes|傳入位元組數。|位元組|總數|Microsoft.EventHub 的傳入位元組數。|無|
|OutgoingBytes|傳出位元組數。|位元組|總數|Microsoft.EventHub 的傳出位元組數。|無|
|ActiveConnections|ActiveConnections|[計數]|Average|Microsoft.EventHub 的使用中連線總數。|無|
...
|CaptureBacklog|擷取待辦項目。|[計數]|總數|Microsoft.EventHub 的擷取待辦項目。|無|
|CapturedMessages|已擷取的訊息。|[計數]|總數|Microsoft.EventHub 的已擷取訊息。|無|
|CapturedBytes|已擷取的位元組。|位元組|總數|Microsoft.EventHub 的已擷取位元組。|無|
|CPU|CPU|百分比|最大需求|事件中樞叢集的 CPU 使用率 (百分比)|角色|
|AvailableMemory|可用的記憶體|百分比|最大需求|事件中樞叢集的可用記憶體（以總記憶體的百分比表示）。|角色|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|GatewayRequests|閘道要求數|[計數]|總數|閘道器要求數目|HttpStatus|
|CategorizedGatewayRequests|已分類的閘道要求數|[計數]|總數|依類別 (1xx/2xx/3xx/4xx/5xx) 區分的閘道要求數目|HttpStatus|
|NumActiveWorkers|作用中背景工作角色的數目|[計數]|最大需求|作用中背景工作角色的數目|MetricName|
...
|ScaleActionsInitiated|已起始的調整規模動作數|[計數]|總數|調整規模作業的方向。|ScaleDirection|
...
|IngestionVolumeInMB|內嵌磁片區（以 MB 為單位）|[計數]|總數|擷取到叢集的資料總量 (以 MB 為單位)|資料庫|
|IngestionLatencyInSeconds|內嵌延遲（以秒為單位）|秒|Average|從來源 (例如，訊息位於 EventHub 中) 擷取到叢集的擷取時間，以秒為單位|無|
|EventsProcessedForEventHubs|已處理的事件（適用于事件/IoT 中樞）|[計數]|總數|從事件/IoT 中樞內嵌時，由叢集處理的事件數目|EventStatus|
|IngestionResult|內嵌結果|[計數]|[計數]|擷取作業數|IngestionResultDetails|
|CPU|CPU|百分比|Average|CPU 使用率等級|無|
|ContinuousExportNumOfRecordsExported|連續匯出–匯出記錄的數目|[計數]|總數|針對匯出作業期間寫入的每個儲存體成品所匯出的記錄數目|無|
|ExportUtilization|匯出使用率|百分比|最大需求|匯出使用率|無|
|ContinuousExportPendingCount|連續匯出暫止計數|[計數]|最大需求|準備好執行的暫止連續匯出作業數|無|
|ContinuousExportMaxLatenessMinutes|連續匯出最大延遲分鐘數|[計數]|最大需求|所有連續匯出暫止且準備好執行的最大延遲（以分鐘為單位）|無|
...
|TotalNumberOfConcurrentQueries|並行查詢的總數|[計數]|總數|並行查詢的總數|無|
|TotalNumberOfThrottledQueries|已節流的查詢總數|[計數]|總數|已節流的查詢總數|無|
|TotalNumberOfThrottledCommands|已節流的命令總數|[計數]|總數|已節流的命令總數|CommandType|
|TotalNumberOfExtents|範圍總數|[計數]|總數|資料範圍總數|無|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|RunsStarted|執行已啟動|[計數]|總數|已啟動的工作流程執行數目。|無|
|RunsCompleted|完成的執行|[計數]|總數|已完成的工作流程執行數目。|無|
|RunsSucceeded|已成功的執行|[計數]|總數|已成功的工作流程執行數目。|無|
|RunsFailed|失敗的執行|[計數]|總數|已失敗的工作流程執行數目。|無|
|RunsCancelled|取消的執行|[計數]|總數|已取消的工作流程執行數目。|無|
|RunLatency|執行延遲|秒|Average|已完成的工作流程執行延遲。|無|
|RunSuccessLatency|執行成功的延遲|秒|Average|已成功的工作流程執行延遲。|無|
|RunThrottledEvents|執行節流的事件|[計數]|總數|工作流程動作或觸發節流的事件數目。|無|
|RunStartThrottledEvents|執行啟動節流事件|[計數]|總數|工作流程執行開始節流事件的數目。|無|
|RunFailurePercentage|執行失敗百分比|百分比|總數|已失敗的工作流程執行百分比。|無|
|ActionsStarted|啟動的動作 |[計數]|總數|已啟動的工作流程動作數目。|無|
|ActionsCompleted|完成的動作 |[計數]|總數|已完成的工作流程動作數目。|無|
|ActionsSucceeded|成功的動作 |[計數]|總數|已成功的工作流程動作數目。|無|
|ActionsFailed|動作的失敗 |[計數]|總數|已失敗的工作流程動作數目。|無|
|ActionsSkipped|略過的動作 |[計數]|總數|已略過的工作流程動作數目。|無|
|ActionLatency|動作延遲 |秒|Average|已完成的工作流程動作延遲。|無|
|ActionSuccessLatency|動作成功延遲 |秒|Average|已成功的工作流程動作延遲。|無|
|ActionThrottledEvents|動作節流的事件|[計數]|總數|工作流程動作已節流的事件數目。|無|
|TriggersStarted|啟動的觸發程序 |[計數]|總數|已啟動的工作流程觸發程序數目。|無|
|TriggersCompleted|完成的觸發程序 |[計數]|總數|已完成的工作流程觸發程序數目。|無|
|TriggersSucceeded|成功的觸發程序 |[計數]|總數|已成功的工作流程觸發程序數目。|無|
|TriggersFailed|失敗的觸發程序 |[計數]|總數|已失敗的工作流程觸發程序數目。|無|
|TriggersSkipped|略過的觸發程序|[計數]|總數|已略過的工作流程觸發程序數目。|無|
|TriggersFired|引發的觸發程序 |[計數]|總數|已引發的工作流程觸發程序數目。|無|
...
|TriggerThrottledEvents|觸發程序節流的事件|[計數]|總數|工作流程觸發程序節流的事件數目。|無|
|BillableActionExecutions|可計費的動作執行|[計數]|總數|計費的工作流程動作執行數目。|無|
|BillableTriggerExecutions|可計費的觸發程序執行|[計數]|總數|計費的工作流程觸發程序執行數目。|無|
|TotalBillableExecutions|可計費的總執行次數|[計數]|總數|計費的工作流程執行數目。|無|
|BillingUsageNativeOperation|為使用原生作業執行計費|[計數]|總數|要計費的原生作業執行數。|無|
|BillingUsageStandardConnector|為使用標準連接器執行計費|[計數]|總數|要計費的標準連接器執行數。|無|
|BillingUsageStorageConsumption|為使用儲存體使用量執行計費|[計數]|總數|要計費的儲存體使用量執行數。|無|
|BillingUsageNativeOperation|為使用原生作業執行計費|[計數]|總數|要計費的原生作業執行數。|無|
|BillingUsageStandardConnector|為使用標準連接器執行計費|[計數]|總數|要計費的標準連接器執行數。|無|
|BillingUsageStorageConsumption|為使用儲存體使用量執行計費|[計數]|總數|要計費的儲存體使用量執行數。|無|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|已完成的執行|已完成的執行|[計數]|總數|已成功完成此工作區的執行數目|案例，RunType，PublishedPipelineId，ComputeType，PipelineStepType|
|開始執行|開始執行|[計數]|總數|此工作區已開始執行的數目|案例，RunType，PublishedPipelineId，ComputeType，PipelineStepType|
|失敗的執行|失敗的執行|[計數]|總數|此工作區的執行失敗次數|案例，RunType，PublishedPipelineId，ComputeType，PipelineStepType|
|模型註冊成功|模型註冊成功|[計數]|總數|在此工作區中成功的模型註冊數目|狀況|
|模型註冊失敗|模型註冊失敗|[計數]|總數|在此工作區中失敗的模型註冊數目|案例，StatusCode|
|模型部署已開始|模型部署已開始|[計數]|總數|在此工作區中啟動的模型部署數目|狀況|
|模型部署成功|模型部署成功|[計數]|總數|在此工作區中成功的模型部署數|狀況|
|模型部署失敗|模型部署失敗|[計數]|總數|在此工作區中失敗的模型部署數|案例，StatusCode|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|使用方式|使用方式|[計數]|[計數]|API 呼叫的計數|ApiCategory、ApiName、ResultType、ResponseCode|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|輸出|輸出|位元組|總數|輸出資料量（以位元組為單位）。|OutputFormat|
|SuccessE2ELatency|成功的端對端延遲|毫秒|Average|成功要求的平均延遲（以毫秒為單位）。|OutputFormat|
|要求|要求|[計數]|總數|對串流端點的要求。|OutputFormat、HttpStatusCode、ErrorCode|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|BytesSentRate|傳送的位元組數|位元組|總數|網路介面傳送的位元組數|無|
|BytesReceivedRate|已接收的位元組數|位元組|總數|網路介面接收的位元組數|無|
|PacketsSentRate|傳送的封包|[計數]|總數|網路介面傳送的封包數|無|
|PacketsReceivedRate|接收的封包|[計數]|總數|網路介面接收的封包數|無|
...
|ByteCount|位元組計數|[計數]|總數|在期間內傳輸的位元組總數|FrontendIPAddress、FrontendPort、Direction|
|PacketCount|封包計數|[計數]|總數|在期間內傳輸的封包總數|FrontendIPAddress、FrontendPort、Direction|
|SYNCount|SYN 計數|[計數]|總數|在期間內傳輸的 SYN 封包總數|FrontendIPAddress、FrontendPort、Direction|
|SnatConnectionCount|SNAT 連線計數|[計數]|總數|在期間內建立的新 SNAT 連線總數|FrontendIPAddress、BackendIPAddress、ConnectionState|
|AllocatedSnatPorts|配置的 SNAT 連接埠 (預覽)|[計數]|總數|在期間內配置的 SNAT 連接埠總數|FrontendIPAddress、BackendIPAddress、ProtocolType、IsAwaitingRemoval|
|UsedSnatPorts|使用的 SNAT 連接埠 (預覽)|[計數]|總數|在期間內使用的 SNAT 連接埠總數|FrontendIPAddress、BackendIPAddress、ProtocolType、IsAwaitingRemoval|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|QueryVolume|查詢磁碟區|[計數]|總數|為 DNS 區域儲存的查詢數目|無|
|RecordSetCount|記錄集計數|[計數]|最大需求|DNS 區域中的記錄集數目|無|
|RecordSetCapacityUtilization|記錄集容量使用率|百分比|最大需求|DNS 區域所使用的記錄集容量百分比|無|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|PacketsInDDoS|傳入封包 DDoS|每秒計數|最大需求|傳入封包 DDoS|無|
|PacketsDroppedDDoS|傳入封包捨棄 DDoS|每秒計數|最大需求|傳入封包捨棄 DDoS|無|
|PacketsForwardedDDoS|傳入封包轉寄 DDoS|每秒計數|最大需求|傳入封包轉寄 DDoS|無|
|TCPPacketsInDDoS|傳入 TCP 封包 DDoS|每秒計數|最大需求|傳入 TCP 封包 DDoS|無|
|TCPPacketsDroppedDDoS|傳入 TCP 封包捨棄 DDoS|每秒計數|最大需求|傳入 TCP 封包捨棄 DDoS|無|
|TCPPacketsForwardedDDoS|傳入 TCP 封包轉送 DDoS|每秒計數|最大需求|傳入 TCP 封包轉送 DDoS|無|
|UDPPacketsInDDoS|傳入 UDP 封包 DDoS|每秒計數|最大需求|傳入 UDP 封包 DDoS|無|
|UDPPacketsDroppedDDoS|傳入 UDP 封包捨棄 DDoS|每秒計數|最大需求|傳入 UDP 封包捨棄 DDoS|無|
|UDPPacketsForwardedDDoS|傳入 UDP 封包轉送 DDoS|每秒計數|最大需求|傳入 UDP 封包轉送 DDoS|無|
|BytesInDDoS|傳入位元組數 DDoS|每秒位元組|最大需求|傳入位元組數 DDoS|無|
|BytesDroppedDDoS|傳入位元組數捨棄 DDoS|每秒位元組|最大需求|傳入位元組數捨棄 DDoS|無|
|BytesForwardedDDoS|傳入位元組數轉送 DDoS|每秒位元組|最大需求|傳入位元組數轉送 DDoS|無|
|TCPBytesInDDoS|傳入 TCP 位元組數 DDoS|每秒位元組|最大需求|傳入 TCP 位元組數 DDoS|無|
|TCPBytesDroppedDDoS|傳入 TCP 位元組數捨棄 DDoS|每秒位元組|最大需求|傳入 TCP 位元組數捨棄 DDoS|無|
|TCPBytesForwardedDDoS|傳入 TCP 位元組數轉送 DDoS|每秒位元組|最大需求|傳入 TCP 位元組數轉送 DDoS|無|
|UDPBytesInDDoS|傳入 UDP 位元組數 DDoS|每秒位元組|最大需求|傳入 UDP 位元組數 DDoS|無|
|UDPBytesDroppedDDoS|傳入 UDP 位元組數捨棄 DDoS|每秒位元組|最大需求|傳入 UDP 位元組數捨棄 DDoS|無|
|UDPBytesForwardedDDoS|傳入 UDP 位元組數轉送 DDoS|每秒位元組|最大需求|傳入 UDP 位元組數轉送 DDoS|無|
|IfUnderDDoSAttack|是否正遭受 DDoS 攻擊|[計數]|最大需求|是否正遭受 DDoS 攻擊|無|
|DDoSTriggerTCPPackets|傳入 TCP 封包數以觸發 DDoS 緩和|每秒計數|最大需求|傳入 TCP 封包數以觸發 DDoS 緩和|無|
|DDoSTriggerUDPPackets|傳入 UDP 封包數以觸發 DDoS 緩和|每秒計數|最大需求|傳入 UDP 封包數以觸發 DDoS 緩和|無|
|DDoSTriggerSYNPackets|用以觸發 DDoS 風險降低措施的輸入 SYN 封包數|每秒計數|最大需求|用以觸發 DDoS 風險降低措施的輸入 SYN 封包數|無|
|VipAvailability|資料路徑可用性|[計數]|Average|每個持續時間 IP 位址可用性的平均值|Port|
|ByteCount|位元組計數|[計數]|總數|在期間內傳輸的位元組總數|埠，方向|
|PacketCount|封包計數|[計數]|總數|在期間內傳輸的封包總數|埠，方向|
|SynCount|SYN 計數|[計數]|總數|在期間內傳輸的 SYN 封包總數|埠，方向|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|ApplicationRuleHit|應用程式規則計數|[計數]|總數|遇到應用程式規則的次數|狀態、原因、通訊協定|
|NetworkRuleHit|網路規則計數|[計數]|總數|點擊網路規則的次數|狀態、原因、通訊協定|
|FirewallHealth|防火牆健全狀況狀態|百分比|Average|防火牆健全狀況狀態|狀態、原因|
|DataProcessed|已處理的資料量|位元組|總數|防火牆處理的總數據量|無|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|Throughput|Throughput|每秒位元組|Average|應用程式閘道每秒已服務的位元組數目|無|
...
|TotalRequests|要求總數|[計數]|總數|應用程式閘道已服務的成功要求計數|BackendSettingsPool|
|AvgRequestCountPerHealthyHost|每部狀況良好主機的每分鐘要求數|[計數]|Average|集區中每個狀況良好後端主機每分鐘的平均要求計數|BackendSettingsPool|
|FailedRequests|失敗的要求|[計數]|總數|應用程式閘道已服務的失敗要求計數|BackendSettingsPool|
|ResponseStatus|回應狀態|[計數]|總數|應用程式閘道傳回的 Http 回應狀態|HttpStatusGroup|
|CurrentConnections|目前連線數|[計數]|總數|目前與應用程式閘道建立的連線計數|無|
...
|BackendResponseStatus|後端回應狀態|[計數]|總數|後端成員所產生的 HTTP 回應碼數目。 這不包含任何由應用程式閘道產生的回應碼。|BackendServer、BackendPool、BackendHttpSetting、HttpStatusGroup|
|TlsProtocol|用戶端 TLS 通訊協定|[計數]|總數|與應用程式閘道建立連線的用戶端所起始的 TLS 和非 TLS 要求數目。 若要查看 TLS 通訊協定散發，請依維度 TLS 通訊協定進行篩選。|接聽程式，TlsProtocol|
|BytesSent|傳送的位元組數|位元組|總數|應用程式閘道傳送給用戶端的位元組總數|接聽程式|
|BytesReceived|已接收的位元組數|位元組|總數|應用程式閘道從用戶端接收的位元組總數|接聽程式|
...
|MatchedCount|Web 應用程式防火牆的規則散發總數|[計數]|總數|連入流量的 Web 應用程式防火牆總規則分佈|RuleGroup、RuleId|
|BlockedCount|Web 應用程式防火牆封鎖要求規則散發|[計數]|總數|Web 應用程式防火牆封鎖要求規則散發|RuleGroup、RuleId|
|BlockedReqCount|Web 應用程式防火牆封鎖的要求計數|[計數]|總數|Web 應用程式防火牆封鎖的要求計數|無|
...
|P2SConnectionCount|P2S 連線計數|[計數]|最大需求|閘道的點對站連線計數|通訊協定|
|TunnelAverageBandwidth|通道頻寬|每秒位元組|Average|通道的平均頻寬 (位元組/秒)|ConnectionName、RemoteIP|
|TunnelEgressBytes|通道輸出位元組數|位元組|總數|通道的傳出位元組數|ConnectionName、RemoteIP|
|TunnelIngressBytes|通道輸入位元組數|位元組|總數|通道的傳入位元組數|ConnectionName、RemoteIP|
|TunnelEgressPackets|通道輸出封包數|[計數]|總數|通道的傳出封包計數|ConnectionName、RemoteIP|
|TunnelIngressPackets|通道輸入封包數|[計數]|總數|通道的傳入封包計數|ConnectionName、RemoteIP|
|TunnelEgressPacketDropTSMismatch|通道輸出 TS 不相符封包捨棄|[計數]|總數|因與通道不符而從流量選取器捨棄的傳出封包計數|ConnectionName、RemoteIP|
|TunnelIngressPacketDropTSMismatch|通道輸入 TS 不符合封包捨棄|[計數]|總數|因與通道不符而從流量選取器捨棄的傳入封包計數|ConnectionName、RemoteIP|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|QpsByEndpoint|傳回的依端點查詢數|[計數]|總數|在指定時間範圍內傳回 Traffic Manager 端點的次數|EndpointName|
|ProbeAgentCurrentEndpointStateByProfileResourceId|依端點的端點狀態|[計數]|最大需求|若端點探查狀態為「已啟用」則為 1，否則為 0。|EndpointName|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|RequestCount|要求計數|[計數]|總數|HTTP/S Proxy 服務的用戶端要求數|HttpStatus、HttpStatusGroup、ClientRegion、ClientCountry|
|RequestSize|要求大小|位元組|總數|從用戶端傳送到 HTTP/S Proxy 的要求的位元組數|HttpStatus、HttpStatusGroup、ClientRegion、ClientCountry|
|ResponseSize|回應大小|位元組|總數|從 HTTP/S Proxy 傳送到用戶端的回應的位元組數|HttpStatus、HttpStatusGroup、ClientRegion、ClientCountry|
|BillableResponseSize|可計費回應大小|位元組|總數|當做回應從 HTTP/S proxy 傳送至用戶端的可計費位元組數（每個要求最少的2KB）。|HttpStatus、HttpStatusGroup、ClientRegion、ClientCountry|
|BackendRequestCount|後端要求計數|[計數]|總數|從 HTTP/S Proxy 傳送至後端的要求數|HttpStatus、HttpStatusGroup、後端|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|registration.all|註冊作業數|[計數]|總數|所有成功註冊作業 (建立、更新、查詢和刪除) 的計數。 |無|
|registration.create|註冊建立作業|[計數]|總數|所有成功註冊建立的計數。|無|
|registration.update|註冊更新作業|[計數]|總數|所有成功註冊更新的計數。|無|
|registration.get|註冊讀取作業|[計數]|總數|所有成功註冊查詢的計數。|無|
|registration.delete|註冊刪除作業|[計數]|總數|所有成功註冊刪除的計數。|無|
|incoming|內送訊息|[計數]|總數|所有成功傳送 API 呼叫的計數。 |無|
|incoming.scheduled|傳送已排程的推播通知|[計數]|總數|取消已排程的推播通知|無|
|incoming.scheduled.cancel|取消已排程的推播通知|[計數]|總數|取消已排程的推播通知|無|
|scheduled.pending|暫止已排程的通知|[計數]|總數|暫止已排程的通知|無|
|installation.all|安裝管理作業|[計數]|總數|安裝管理作業|無|
|installation.get|取得安裝作業|[計數]|總數|取得安裝作業|無|
|installation.upsert|建立或更新安裝作業|[計數]|總數|建立或更新安裝作業|無|
|installation.patch|修補安裝作業|[計數]|總數|修補安裝作業|無|
|installation.delete|刪除安裝作業|[計數]|總數|刪除安裝作業|無|
|outgoing.allpns.success|成功通知|[計數]|總數|所有成功通知的計數。|無|
|outgoing.allpns.invalidpayload|承載錯誤|[計數]|總數|因為 PNS 傳回錯誤承載錯誤而失敗的推播計數。|無|
|outgoing.allpns.pnserror|外部通知系統錯誤|[計數]|總數|因為與 PNS 通訊發生問題 (不包括驗證問題) 而失敗的推播計數。|無|
|outgoing.allpns.channelerror|通道錯誤|[計數]|總數|因為通道無效、未與正確的應用程式相關聯、已節流處理或過期，導致失敗的推播計數。|無|
|outgoing.allpns.badorexpiredchannel|錯誤或過期的通道錯誤|[計數]|總數|因為註冊中的通道/權杖/registrationId 過期或無效，導致失敗的推播計數。|無|
|outgoing.wns.success|WNS 成功通知|[計數]|總數|所有成功通知的計數。|無|
|outgoing.wns.invalidcredentials|WNS 授權錯誤 (無效認證)|[計數]|總數|因為 PNS 不接受提供的認證或已封鎖認證，導致失敗的推播計數。 (Windows Live 無法識別認證)。|無|
|outgoing.wns.badchannel|WNS 不正確的通道錯誤|[計數]|總數|因為無法識別註冊中的 ChannelURI (WNS 狀態︰404 找不到)，導致失敗的推播計數。|無|
|outgoing.wns.expiredchannel|WNS 過期通道錯誤|[計數]|總數|因為 ChannelURIi 過期 (WNS 狀態︰ 410 不存在) 而失敗的推播計數。|無|
|outgoing.wns.throttled|WNS 節流通知|[計數]|總數|因為 WNS 正在節流此應用程式，導致失敗的推播計數 (WNS 狀態︰406 無法接受)。|無|
|outgoing.wns.tokenproviderunreachable|WNS 授權錯誤 (無法連線)|[計數]|總數|無法連線到 Windows Live。|無|
|outgoing.wns.invalidtoken|WNS 授權錯誤 (無效權杖)|[計數]|總數|提供給 WNS 的權杖無效 (WNS 狀態︰ 401 未經授權)。|無|
|outgoing.wns.wrongtoken|WNS 授權錯誤 (錯誤權杖)|[計數]|總數|提供給 WNS 的權杖有效，但卻是給另一個應用程式 (WNS 狀態︰403 禁止)。 如果註冊中的 ChannelURI 與另一個應用程式相關聯，可能會發生這種情形。 請檢查用戶端應用程式相關聯的應用程式，就是認證位於通知中樞的同一個應用程式。|無|
|outgoing.wns.invalidnotificationformat|WNS 無效通知格式|[計數]|總數|通知的格式無效 (WNS 狀態︰400)。 請注意，WNS 不會拒絕所有無效承載。|無|
|outgoing.wns.invalidnotificationsize|WNS 無效通知大小錯誤|[計數]|總數|通知承載太大 (WNS 狀態︰413)。|無|
|outgoing.wns.channelthrottled|WNS 通道已節流處理|[計數]|總數|因為註冊中的 ChannelURI 已節流處理而捨棄通知 (WNS 回應標頭︰X-WNS-NotificationStatus:channelThrottled)。|無|
|outgoing.wns.channeldisconnected|WNS 通道已中斷連線|[計數]|總數|因為註冊中的 ChannelURI 已節流處理而捨棄通知 (WNS 回應標頭︰X-WNS-DeviceConnectionStatus: disconnected)。|無|
|outgoing.wns.dropped|WNS 已捨棄通知|[計數]|總數|因為註冊中的 ChannelURI 已節流處理而捨棄通知 (X-WNS-NotificationStatus: dropped 但不是 X-WNS-DeviceConnectionStatus: disconnected)。|無|
|outgoing.wns.pnserror|WNS 錯誤|[計數]|總數|因為與 WNS 通訊錯誤而未傳遞通知。|無|
|outgoing.wns.authenticationerror|WNS 驗證錯誤|[計數]|總數|因為與 Windows Live 通訊錯誤、無效認證或不正確的權杖，導致未傳遞通知。|無|
|outgoing.apns.success|APNS 成功通知|[計數]|總數|所有成功通知的計數。|無|
|outgoing.apns.invalidcredentials|APNS 授權錯誤|[計數]|總數|因為 PNS 不接受提供的認證或已封鎖認證，導致失敗的推播計數。|無|
|outgoing.apns.badchannel|APNS 不正確的通道錯誤|[計數]|總數|因為權杖無效而失敗的推播計數 (APNS 狀態碼︰8)。|無|
|outgoing.apns.expiredchannel|APNS 過期通道錯誤|[計數]|總數|由 APNS 意見反應通道宣告失效的權杖計數。|無|
|outgoing.apns.invalidnotificationsize|APNS 無效通知大小錯誤|[計數]|總數|因為承載太大而失敗的推播計數 (APNS 狀態碼︰7)。|無|
|outgoing.apns.pnserror|APNS 錯誤|[計數]|總數|因為與 APNS 通訊錯誤而失敗的推播計數。|無|
|outgoing.gcm.success|GCM 成功通知|[計數]|總數|所有成功通知的計數。|無|
|outgoing.gcm.invalidcredentials|GCM 授權錯誤 (無效認證)|[計數]|總數|因為 PNS 不接受提供的認證或已封鎖認證，導致失敗的推播計數。|無|
|outgoing.gcm.badchannel|GCM 不正確的通道錯誤|[計數]|總數|因為無法識別註冊中的 registrationId 而失敗的推播計數 (GCM 結果：無效註冊)。|無|
|outgoing.gcm.expiredchannel|GCM 過期通道錯誤|[計數]|總數|因為註冊中的 registrationId 過期而失敗的推播計數 (GCM 結果：NotRegistered)。|無|
|outgoing.gcm.throttled|GCM 已將通知節流處理|[計數]|總數|因為 GCM 已將此應用程式節流處理而失敗的推播計數 (GCM 狀態碼︰501-599 或結果：Unavailable)。|無|
|outgoing.gcm.invalidnotificationformat|GCM 無效通知格式|[計數]|總數|因為承載未正確格式化而失敗的推播計數 (GCM 結果︰InvalidDataKey 或 InvalidTtl)。|無|
|outgoing.gcm.invalidnotificationsize|GCM 無效通知大小錯誤|[計數]|總數|因為承載太大而失敗的推播計數 (GCM 結果︰MessageTooBig)。|無|
|outgoing.gcm.wrongchannel|GCM 不正確的通道錯誤|[計數]|總數|因為註冊中的 registrationId 未與目前的應用程式相關聯，導致失敗的推播計數 (GCM 結果：InvalidPackageName)。|無|
|outgoing.gcm.pnserror|GCM 錯誤|[計數]|總數|因為與 GCM 通訊錯誤而失敗的推播計數。|無|
|outgoing.gcm.authenticationerror|GCM 驗證錯誤|[計數]|總數|因為 PNS 不接受提供的認證、已封鎖認證，或應用程式中未正確設定 SenderId，導致失敗的推播計數 (GCM 結果︰MismatchedSenderId)。|無|
|outgoing.mpns.success|MPNS 成功通知|[計數]|總數|所有成功通知的計數。|無|
|outgoing.mpns.invalidcredentials|MPNS 無效認證|[計數]|總數|因為 PNS 不接受提供的認證或已封鎖認證，導致失敗的推播計數。|無|
|outgoing.mpns.badchannel|MPNS 不正確的通道錯誤|[計數]|總數|因為無法識別註冊中的 ChannelURI 而失敗的推播計數 (MPNS 狀態︰404 找不到)。|無|
|outgoing.mpns.throttled|MPNS 已將通知節流處理|[計數]|總數|因為 MPNS 正在節流此應用程式，導致失敗的推播計數 (MPNS 狀態︰406 無法接受)。|無|
|outgoing.mpns.invalidnotificationformat|MPNS 無效通知格式|[計數]|總數|因為通知的承載太大而失敗的推播計數。|無|
|outgoing.mpns.channeldisconnected|MPNS 通道已中斷連線|[計數]|總數|因為註冊中的 ChannelURI 中斷連線而失敗的推播計數 (MPNS 狀態︰412 找不到)。|無|
|outgoing.mpns.dropped|MPNS 已捨棄通知|[計數]|總數|MPNS 已捨棄的推播計數 (MPNS 回應標頭︰X-NotificationStatus: QueueFull 或 Suppressed)。|無|
|outgoing.mpns.pnserror|MPNS 錯誤|[計數]|總數|因為與 MPNS 通訊錯誤而失敗的推播計數。|無|
|outgoing.mpns.authenticationerror|MPNS 驗證錯誤|[計數]|總數|因為 PNS 不接受提供的認證或已封鎖認證，導致失敗的推播計數。|無|
|notificationhub.pushes|所有外寄通知|[計數]|總數|通知中樞的所有外寄通知|無|
|incoming.all.requests|所有傳入要求|[計數]|總數|針對通知中樞傳入的要求總數|無|
|incoming.all.failedrequests|所有傳入的失敗要求|[計數]|總數|針對通知中樞傳入的失敗要求總數|無|
...
|Average_% Free Space|可用空間百分比|[計數]|Average|Average_% Free Space|Computer、ObjectName、InstanceName、CounterPath、及 sourcesystem|
...
|Average_Total Bytes|位元組總數|[計數]|Average|Average_Total Bytes|Computer、ObjectName、InstanceName、CounterPath、及 sourcesystem|
...
|Average_Avg. 的集合規則|Avg. Disk sec/Read|[計數]|Average|Average_Avg. 的集合規則|Computer、ObjectName、InstanceName、CounterPath、及 sourcesystem|
...
|Average_Avg. 的集合規則|Avg. Disk sec/Write|[計數]|Average|Average_Avg. 的集合規則|Computer、ObjectName、InstanceName、CounterPath、及 sourcesystem|
...
|Average_% Idle Time|% 閒置時間|[計數]|Average|Average_% Idle Time|Computer、ObjectName、InstanceName、CounterPath、及 sourcesystem|
...
|Average_% Processor Time|% 處理器時間|[計數]|Average|Average_% Processor Time|Computer、ObjectName、InstanceName、CounterPath、及 sourcesystem|
...
|Average_Processes|處理序|[計數]|Average|Average_Processes|Computer、ObjectName、InstanceName、CounterPath、及 sourcesystem|
...
|Average_Uptime|運作時間|[計數]|Average|Average_Uptime|Computer、ObjectName、InstanceName、CounterPath、及 sourcesystem|
|Average_Users|使用者|[計數]|Average|Average_Users|Computer、ObjectName、InstanceName、CounterPath、及 sourcesystem|
|Average_Avg. 的集合規則|Avg. Disk sec/Read|[計數]|Average|Average_Avg. 的集合規則|Computer、ObjectName、InstanceName、CounterPath、及 sourcesystem|
|Average_Avg. 的集合規則|Avg. Disk sec/Write|[計數]|Average|Average_Avg. 的集合規則|Computer、ObjectName、InstanceName、CounterPath、及 sourcesystem|
...
|活動訊號|活動訊號|[計數]|總數|活動訊號|Computer、OSType、Version、SourceComputerId|
|更新|更新|[計數]|Average|更新|電腦、產品、分類、UpdateState、選擇性、已核准|
|事件|事件|[計數]|Average|事件|Source、EventLog、Computer、EventCategory、EventLevel、EventLevelName、EventID|
...
|qpu_high_utilization_metric|QPU 高使用率|[計數]|總數|前一分鐘的 QPU 高使用率，1 代表 QPU 高使用率，否則為 0|無維度|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|ListenerConnections-Success|ListenerConnections-Success|[計數]|總數|Microsoft.Relay 的成功 ListenerConnections。|EntityName、OperationResult|
|ListenerConnections-ClientError|ListenerConnections-ClientError|[計數]|總數|Microsoft.Relay 的 ListenerConnections 上發生的 ClientError。|EntityName、OperationResult|
|ListenerConnections-ServerError|ListenerConnections-ServerError|[計數]|總數|Microsoft.Relay 的 ListenerConnections 上發生的 ServerError。|EntityName、OperationResult|
|SenderConnections-Success|SenderConnections-Success|[計數]|總數|Microsoft.Relay 的成功 SenderConnections。|EntityName、OperationResult|
|SenderConnections-ClientError|SenderConnections-ClientError|[計數]|總數|Microsoft.Relay 的 SenderConnections 上發生的 ClientError。|EntityName、OperationResult|
|SenderConnections-ServerError|SenderConnections-ServerError|[計數]|總數|Microsoft.Relay 的 SenderConnections 上發生的 ServerError。|EntityName、OperationResult|
|ListenerConnections-TotalRequests|ListenerConnections-TotalRequests|[計數]|總數|Microsoft.Relay 的 ListenerConnections 總計。|EntityName|
|SenderConnections-TotalRequests|SenderConnections-TotalRequests|[計數]|總數|Microsoft.Relay 的 SenderConnections 要求總計。|EntityName|
|ActiveConnections|ActiveConnections|[計數]|總數|Microsoft.Relay 的 ActiveConnections 總計。|EntityName|
|ActiveListeners|ActiveListeners|[計數]|總數|Microsoft.Relay 的 ActiveListeners 總計。|EntityName|
|BytesTransferred|BytesTransferred|[計數]|總數|Microsoft.Relay 的 BytesTransferred 總計。|EntityName|
|ListenerDisconnects|ListenerDisconnects|[計數]|總數|Microsoft.Relay 的 ListenerDisconnects 總計。|EntityName|
|SenderDisconnects|SenderDisconnects|[計數]|總數|Microsoft.Relay 的 SenderDisconnects 總計。|EntityName|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|SuccessfulRequests|成功的要求|[計數]|總數|命名空間的成功要求總數|EntityName、OperationResult|
|ServerErrors|伺服器錯誤。|[計數]|總數|Microsoft.ServiceBus 的伺服器錯誤。|EntityName、OperationResult|
|UserErrors|使用者錯誤。|[計數]|總數|Microsoft.ServiceBus 的使用者錯誤。|EntityName、OperationResult|
|ThrottledRequests|節流的要求。|[計數]|總數|Microsoft.ServiceBus 的節流要求。|EntityName、OperationResult|
|IncomingRequests|傳入的要求|[計數]|總數|Microsoft.ServiceBus 的傳入要求。|EntityName|
|IncomingMessages|內送訊息|[計數]|總數|Microsoft.ServiceBus 的傳入訊息。|EntityName|
|OutgoingMessages|外寄訊息|[計數]|總數|Microsoft.ServiceBus 的傳出訊息。|EntityName|
|ActiveConnections|ActiveConnections|[計數]|總數|Microsoft.ServiceBus 的使用中連線總數。|無|
...
|NamespaceCpuUsage|CPU|百分比|最大需求|服務匯流排 premium 命名空間 CPU 使用量計量。|複本|
|NamespaceMemoryUsage|記憶體使用量|百分比|最大需求|服務匯流排 premium 命名空間記憶體使用量計量。|複本|
|CPUXNS|CPU （已淘汰）|百分比|最大需求|服務匯流排 premium 命名空間 CPU 使用量計量。 此度量為淘汰。 請改用 CPU 度量（NamespaceCpuUsage）。|複本|
|WSXNS|記憶體使用量（已淘汰）|百分比|最大需求|服務匯流排 premium 命名空間記憶體使用量計量。 此計量已過時。 請改用記憶體使用量（NamespaceMemoryUsage）度量。|複本|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|ConnectionCount|連線計數|[計數]|最大需求|使用者連線數量。|端點|
|MessageCount|訊息計數|[計數]|總數|訊息總量|無|
|InboundTraffic|輸入流量|位元組|總數|服務的輸入流量|無|
|OutboundTraffic|輸出流量|位元組|總數|服務的輸出流量|無|
|UserErrors|使用者錯誤|百分比|最大需求|使用者錯誤的百分比|無|
|SystemErrors|系統錯誤|百分比|最大需求|系統錯誤的百分比|無|
...
|儲存|使用的資料空間|位元組|最大需求|使用的資料空間。 不適用於資料倉儲。|無|
|connection_successful|成功的連線|[計數]|總數|成功的連線|無|
|connection_failed|失敗的連線|[計數]|總數|失敗的連線|無|
|blocked_by_firewall|遭到防火牆封鎖|[計數]|總數|遭到防火牆封鎖|無|
|死結|死結|[計數]|總數|發生. 不適用於資料倉儲。|無|
|storage_percent|使用的資料空間百分比|百分比|最大需求|已使用的資料空間百分比。 不適用於資料倉儲或超大規模資料庫資料庫。|無|
...
|dwu_limit|DWU 限制|[計數]|最大需求|DWU 限制。 僅適用于資料倉儲。|無|
|dwu_consumption_percent|DWU 百分比|百分比|最大需求|DWU 百分比。 僅適用于資料倉儲。|無|
|dwu_used|已使用 DWU|[計數]|最大需求|已使用 DWU。 僅適用于資料倉儲。|無|
|cache_hit_percent|快取命中的百分比|百分比|最大需求|快取命中百分比。 僅適用于資料倉儲。|無|
|cache_used_percent|已用快取的百分比|百分比|最大需求|快取已使用百分比。 僅適用于資料倉儲。|無|
|sqlserver_process_core_percent|SQL Server 處理常式核心百分比|百分比|最大需求|SQL Server 進程的 CPU 使用量百分比（由作業系統測量）。 目前僅適用于無伺服器資料庫。|無|
|sqlserver_process_memory_percent|SQL Server 處理常式記憶體百分比|百分比|最大需求|SQL Server 進程的記憶體使用量百分比（由作業系統測量）。 目前僅適用于無伺服器資料庫。|無|
|tempdb_data_size|Tempdb 資料檔案大小 Kb|[計數]|最大需求|Tempdb 資料檔案大小（Kb）。 不適用於資料倉儲。|無|
|tempdb_log_size|Tempdb 記錄檔大小 Kb|[計數]|最大需求|Tempdb 記錄檔大小（Kb）。 不適用於資料倉儲。|無|
|tempdb_log_used_percent|使用的 Tempdb 百分比記錄|百分比|最大需求|使用的 Tempdb 百分比記錄。 不適用於資料倉儲。|無|
|local_tempdb_usage_percent|本機 tempdb 百分比|百分比|Average|本機 tempdb 百分比。 僅適用于資料倉儲。|無|
|app_cpu_billed|應用程式 CPU 計費|[計數]|總數|應用程式 CPU 計費。 適用于無伺服器資料庫。|無|
...
|memory_usage_percent|記憶體百分比|百分比|最大需求|記憶體百分比。 僅適用于資料倉儲。|無|
|full_backup_size_bytes|完整備份儲存體大小|位元組|最大需求|累計完整備份儲存體大小。 適用于以 vCore 為基礎的資料庫。 不適用於超大規模資料庫資料庫。|無|
|diff_backup_size_bytes|差異備份儲存體大小|位元組|最大需求|累計差異備份儲存體大小。 適用于以 vCore 為基礎的資料庫。 不適用於超大規模資料庫資料庫。|無|
|log_backup_size_bytes|記錄備份儲存體大小|位元組|最大需求|累計記錄備份儲存體大小。 適用于以 vCore 為基礎的資料庫。 不適用於超大規模資料庫資料庫。|無|
...
|sqlserver_process_core_percent|SQL Server 處理常式核心百分比|百分比|最大需求|CPU 使用量（以 SQL DB 進程的百分比表示）。 適用于彈性集區。|無|
|sqlserver_process_memory_percent|SQL Server 處理常式記憶體百分比|百分比|最大需求|記憶體使用量，以 SQL DB 進程的百分比表示。 適用于彈性集區。|無|
|tempdb_data_size|Tempdb 資料檔案大小 Kb|[計數]|最大需求|Tempdb 資料檔案大小 Kb|無|
|tempdb_log_size|Tempdb 記錄檔大小 Kb|[計數]|最大需求|Tempdb 記錄檔大小 Kb|無|
|tempdb_log_used_percent|使用的 Tempdb 百分比記錄|百分比|最大需求|使用的 Tempdb 百分比記錄|無|
...
|allocated_data_storage_percent|已配置的資料空間百分比|百分比|最大需求|已配置的資料空間百分比|無|
...
|運作時間|運作時間|[計數]|Average|快取和監視系統之間的連線能力測試布林值結果。|無|
...
|StorageSyncSyncSessionAppliedFilesCount|檔案已同步|[計數]|總數|已同步處理的檔案計數|SyncGroupName、ServerEndpointName、SyncDirection|
|StorageSyncSyncSessionPerItemErrorsCount|檔案無法同步|[計數]|總數|無法同步的檔案計數|SyncGroupName、ServerEndpointName、SyncDirection|
|StorageSyncBatchTransferredFileBytes|同步的位元組|位元組|總數|同步會話的已傳輸檔案大小總計|SyncGroupName、ServerEndpointName、SyncDirection|
|StorageSyncServerHeartbeat|伺服器線上狀態|[計數]|最大需求|每次 resigtered 伺服器順利記錄與雲端端點的心跳時，其值會記錄1的度量|伺服器名稱|
|StorageSyncRecallIOTotalSizeBytes|雲端階層處理重新叫用|位元組|總數|伺服器回收的資料大小總計|伺服器名稱|
|StorageSyncRecalledTotalNetworkBytes|雲端階層處理重新叫用大小|位元組|總數|重新叫用的資料大小|SyncGroupName、ServerName|
|StorageSyncRecallThroughputBytesPerSecond|雲端階層處理重新叫用輸送量|每秒位元組|Average|資料重新叫用輸送量的大小|SyncGroupName、ServerName|
|StorageSyncRecalledNetworkBytesByApplication|雲端階層處理重新叫用應用程式的大小|位元組|總數|應用程式重新叫用的資料大小|SyncGroupName、ServerName、ApplicationName|

## <a name="microsoftstoragesyncstoragesyncservicessyncgroups"></a>microsoft.storagesync/storageSyncServices/syncGroups

|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|SyncGroupSyncSessionAppliedFilesCount|檔案已同步|[計數]|總數|已同步處理的檔案計數|SyncGroupName、ServerEndpointName、SyncDirection|
|SyncGroupSyncSessionPerItemErrorsCount|檔案無法同步|[計數]|總數|無法同步的檔案計數|SyncGroupName、ServerEndpointName、SyncDirection|
|SyncGroupBatchTransferredFileBytes|同步的位元組|位元組|總數|同步會話的已傳輸檔案大小總計|SyncGroupName、ServerEndpointName、SyncDirection|

## <a name="microsoftstoragesyncstoragesyncservicessyncgroupsserverendpoints"></a>microsoft.storagesync/storageSyncServices/syncGroups/serverEndpoints

|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|ServerEndpointSyncSessionAppliedFilesCount|檔案已同步|[計數]|總數|已同步處理的檔案計數|ServerEndpointName、SyncDirection|
|ServerEndpointSyncSessionPerItemErrorsCount|檔案無法同步|[計數]|總數|無法同步的檔案計數|ServerEndpointName、SyncDirection|
|ServerEndpointBatchTransferredFileBytes|同步的位元組|位元組|總數|同步會話的已傳輸檔案大小總計|ServerEndpointName、SyncDirection|

## <a name="microsoftstoragesyncstoragesyncservicesregisteredservers"></a>microsoft.storagesync/storageSyncServices/registeredServers

|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|ServerHeartbeat|伺服器線上狀態|[計數]|最大需求|每次 resigtered 伺服器順利記錄與雲端端點的心跳時，其值會記錄1的度量|ServerResourceId、ServerName|
|ServerRecallIOTotalSizeBytes|雲端階層處理重新叫用|位元組|總數|伺服器回收的資料大小總計|ServerResourceId、ServerName|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|ResourceUtilization|SU % 使用率|百分比|最大需求|SU % 使用率|LogicalName、PartitionId|
|InputEvents|輸入事件|[計數]|總數|輸入事件|LogicalName、PartitionId|
|InputEventBytes|輸入事件位元組|位元組|總數|輸入事件位元組|LogicalName、PartitionId|
|LateInputEvents|延遲輸入事件|[計數]|總數|延遲輸入事件|LogicalName、PartitionId|
|OutputEvents|輸出事件|[計數]|總數|輸出事件|LogicalName、PartitionId|
|ConversionErrors|資料轉換錯誤|[計數]|總數|資料轉換錯誤|LogicalName、PartitionId|
|錯誤|執行階段錯誤|[計數]|總數|執行階段錯誤|LogicalName、PartitionId|
|DroppedOrAdjustedEvents|次序錯誤事件|[計數]|總數|次序錯誤事件|LogicalName、PartitionId|
|AMLCalloutRequests|函式要求|[計數]|總數|函式要求|LogicalName、PartitionId|
|AMLCalloutFailedRequests|失敗的函式要求|[計數]|總數|失敗的函式要求|LogicalName、PartitionId|
|AMLCalloutInputEvents|函式事件|[計數]|總數|函式事件|LogicalName、PartitionId|
|DeserializationError|輸入還原序列化錯誤|[計數]|總數|輸入還原序列化錯誤|LogicalName、PartitionId|
|EarlyInputEvents|早期輸入事件|[計數]|總數|早期輸入事件|LogicalName、PartitionId|
|OutputWatermarkDelaySeconds|浮水印延遲秒數|秒|最大需求|浮水印延遲秒數|LogicalName、PartitionId|
|InputEventsSourcesBacklogged|待處理輸入事件數|[計數]|最大需求|待處理輸入事件數|LogicalName、PartitionId|
|InputEventsSourcesPerSecond|收到的輸入來源數|[計數]|總數|收到的輸入來源數|LogicalName、PartitionId|

## <a name="microsofttimeseriesinsightsenvironments"></a>Microsoft.TimeSeriesInsights/environments

|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|IngressReceivedMessages|輸入接收的訊息|[計數]|總數|從所有事件中樞或 IoT 中樞事件來源讀取的訊息計數|無|
|IngressReceivedInvalidMessages|輸入接收的無效訊息|[計數]|總數|從所有事件中樞或 IoT 中樞事件來源讀取的無效訊息計數|無|
|IngressReceivedBytes|輸入接收的位元組|位元組|總數|從所有事件來源讀取的位元組計數|無|
|IngressStoredBytes|輸入儲存的位元組|位元組|總數|已成功處理且可供查詢的事件總大小|無|
|IngressStoredEvents|輸入儲存的事件|[計數]|總數|已成功處理且可供查詢的壓平合併事件計數|無|
|IngressReceivedMessagesTimeLag|輸入接收的訊息時間延遲|秒|最大需求|訊息在事件來源中加入佇列的時間與在輸入中處理的時間之間的差異|無|
|IngressReceivedMessagesCountLag|輸入接收的訊息計數延遲|[計數]|Average|事件來源資料分割中最後排入佇列之訊息的序號與輸入中所處理訊息的序號之間的差異|無|
|WarmStorageMaxProperties|暖儲存空間上限屬性|[計數]|最大需求|適用于 S1/S2 SKU 的環境所允許的屬性數目上限，以及 PAYG SKU 的暖存放區所允許的最大屬性數目|無|
|WarmStorageUsedProperties|暖儲存體使用的屬性 |[計數]|最大需求|適用于 S1/S2 SKU 的環境所使用的屬性數目，以及 PAYG SKU 的暖存放區所使用的屬性數目|無|

## <a name="microsofttimeseriesinsightsenvironmentseventsources"></a>Microsoft.TimeSeriesInsights/environments/eventsources

|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|IngressReceivedMessages|輸入接收的訊息|[計數]|總數|從事件來源讀取的訊息計數|無|
|IngressReceivedInvalidMessages|輸入接收的無效訊息|[計數]|總數|從事件來源讀取的無效訊息計數|無|
|IngressReceivedBytes|輸入接收的位元組|位元組|總數|從事件來源讀取的位元組計數|無|
|IngressStoredBytes|輸入儲存的位元組|位元組|總數|已成功處理且可供查詢的事件總大小|無|
|IngressStoredEvents|輸入儲存的事件|[計數]|總數|已成功處理且可供查詢的壓平合併事件計數|無|
|IngressReceivedMessagesTimeLag|輸入接收的訊息時間延遲|秒|最大需求|訊息在事件來源中加入佇列的時間與在輸入中處理的時間之間的差異|無|
|IngressReceivedMessagesCountLag|輸入接收的訊息計數延遲|[計數]|Average|事件來源資料分割中最後排入佇列之訊息的序號與輸入中所處理訊息的序號之間的差異|無|
...
|Disk Read Bytes|Disk Read Bytes|位元組|總數|由於在取樣期間讀取作業的總磁片輸送量。|無|
|Disk Write Bytes|Disk Write Bytes|位元組|總數|因取樣期間的寫入作業而造成的總磁片輸送量。|無|
|DiskReadOperations|磁片讀取作業|[計數]|總數|上一個範例期間的 IO 讀取作業數目。 請注意，這些作業可能是可變大小的。|無|
|DiskWriteOperations|磁片寫入作業|[計數]|總數|上一個範例期間的 IO 寫入作業數目。 請注意，這些作業可能是可變大小的。|無|
...
|Network In|Network In|位元組|總數|已接收流量的網路輸送量總計。|無|
|Network Out|Network Out|位元組|總數|傳輸流量的網路輸送量總計。|無|
...
|PercentageCpuReady|CPU 已就緒百分比|毫秒|總數|「就緒時間」是等待 CPU 在過去的更新間隔中變成可用的時間。|無|
...
|DiskQueueLength|Disk Queue Length|[計數]|Average|Disk Queue Length|執行個體|
|HttpQueueLength|Http 佇列長度|[計數]|Average|Http 佇列長度|執行個體|
|BytesReceived|資料輸入|位元組|總數|資料輸入|執行個體|
|BytesSent|資料輸出|位元組|總數|資料輸出|執行個體|
...
|公制|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|CpuTime|CPU 時間|秒|總數|CPU 時間|執行個體|
|要求|要求|[計數]|總數|要求|執行個體|
|BytesReceived|資料輸入|位元組|總數|資料輸入|執行個體|
|BytesSent|資料輸出|位元組|總數|資料輸出|執行個體|
|Http101|Http 101|[計數]|總數|Http 101|執行個體|
|Http2xx|Http 2xx|[計數]|總數|Http 2xx|執行個體|
|Http3xx|Http 3xx|[計數]|總數|Http 3xx|執行個體|
|Http401|Http 401|[計數]|總數|Http 401|執行個體|
|Http403|Http 403|[計數]|總數|Http 403|執行個體|
|Http404|Http 404|[計數]|總數|Http 404|執行個體|
|Http406|Http 406|[計數]|總數|Http 406|執行個體|
|Http4xx|Http 4xx|[計數]|總數|Http 4xx|執行個體|
|Http5xx|Http 伺服器錯誤|[計數]|總數|Http 伺服器錯誤|執行個體|
...
|AppConnections|連接|[計數]|Average|連接|執行個體|
|處理|控制碼計數|[計數]|Average|控制碼計數|執行個體|
|執行緒|執行緒計數|[計數]|Average|執行緒計數|執行個體|
|PrivateBytes|私用位元組|位元組|Average|私用位元組|執行個體|
|IoReadBytesPerSecond|每秒的 IO 讀取位元組數|每秒位元組|總數|每秒的 IO 讀取位元組數|執行個體|
|IoWriteBytesPerSecond|每秒的 IO 寫入位元組數|每秒位元組|總數|每秒的 IO 寫入位元組數|執行個體|
|IoOtherBytesPerSecond|每秒的 IO 其他位元組數|每秒位元組|總數|每秒的 IO 其他位元組數|執行個體|
|IoReadOperationsPerSecond|每秒的 IO 讀取作業數|每秒位元組|總數|每秒的 IO 讀取作業數|執行個體|
|IoWriteOperationsPerSecond|每秒的 IO 寫入作業數|每秒位元組|總數|每秒的 IO 寫入作業數|執行個體|
|IoOtherOperationsPerSecond|每秒的 IO 其他作業數|每秒位元組|總數|每秒的 IO 其他作業數|執行個體|
...
|Gen0Collections|Gen 0 記憶體回收|[計數]|總數|Gen 0 記憶體回收|執行個體|
|Gen1Collections|Gen 1 記憶體回收|[計數]|總數|Gen 1 記憶體回收|執行個體|
|Gen2Collections|Gen 2 記憶體回收|[計數]|總數|Gen 2 記憶體回收|執行個體|
...
|MemoryWorkingSet|記憶體工作集|位元組|Average|記憶體工作集|執行個體|
|AverageMemoryWorkingSet|平均記憶體工作集|位元組|Average|平均記憶體工作集|執行個體|
|FunctionExecutionUnits|函式執行單位|MB/毫秒|總數|[函式執行單位](https://github.com/Azure/Azure-Functions/wiki/Consumption-Plan-Cost-Billing-FAQ#how-can-i-view-graphs-of-execution-count-and-gb-seconds)|執行個體|
|FunctionExecutionCount|函式執行計數|[計數]|總數|函式執行計數|執行個體|
|PrivateBytes|私用位元組|位元組|Average|私用位元組|執行個體|
|IoReadBytesPerSecond|每秒的 IO 讀取位元組數|每秒位元組|總數|每秒的 IO 讀取位元組數|執行個體|
|IoWriteBytesPerSecond|每秒的 IO 寫入位元組數|每秒位元組|總數|每秒的 IO 寫入位元組數|執行個體|
|IoOtherBytesPerSecond|每秒的 IO 其他位元組數|每秒位元組|總數|每秒的 IO 其他位元組數|執行個體|
...
|FunctionExecutionUnits|函式執行單位|[計數]|總數|函式執行單位|執行個體|
|FunctionExecutionCount|函式執行計數|[計數]|總數|函式執行計數|執行個體|
|AppConnections|連接|[計數]|Average|連接|執行個體|
|處理|控制碼計數|[計數]|Average|控制碼計數|執行個體|
|執行緒|執行緒計數|[計數]|Average|執行緒計數|執行個體|
...
|DiskQueueLength|Disk Queue Length|[計數]|Average|Disk Queue Length|執行個體|
|HttpQueueLength|Http 佇列長度|[計數]|Average|Http 佇列長度|執行個體|
|ActiveRequests|使用中的要求|[計數]|總數|使用中的要求|執行個體|
