...
|mashup_engine_private_bytes_metric|M 引擎私人位元組|位元組|Average|混搭引擎進程的私用位元組使用量。|ServerResourceType|
...
|EventHubRejectedEvents|已拒絕的 EventHub 事件|計數|總計|已拒絕的 EventHub 事件數（錯誤設定或未經授權）|Location|
|EventHubThrottledEvents|已節流的 EventHub 事件|計數|總計|已節流的 EventHub 事件數目|Location|
|EventHubTimedoutEvents|已逾時的 EventHub 事件|計數|總計|已超時的 EventHub 事件數|Location|
|EventHubDroppedEvents|已卸除的 EventHub 事件|計數|總計|因為達到佇列大小上限而略過的事件數目|Location|
|EventHubTotalBytesSent|EventHub 事件的大小|位元組|總計|EventHub 事件的總大小（以位元組為單位）|Location|
|Requests|Requests|計數|總計|具有多個維度的閘道要求計量|Location、Hostname、LastErrorReason、BackendResponseCode、GatewayResponseCode、BackendResponseCodeCategory、GatewayResponseCodeCategory|
|NetworkConnectivity|資源的網路線上狀態（預覽）|計數|總計|來自 API 管理服務之相依資源類型的網路線上狀態|Location、ResourceType|
...
|HttpIncomingRequestDuration|HttpIncomingRequestDuration|計數|Average|Http 要求的延遲。|StatusCode|
...
|AppMemoryCommitted|已指派的應用程式記憶體|位元組|Average|指派給 JVM 的記憶體（位元組）|AppName、Pod|
|AppMemoryUsed|已使用的應用程式記憶體|位元組|Average|使用的應用程式記憶體（位元組）|AppName、Pod|
...
|OldGenMemoryPoolBytes|舊世代資料大小|位元組|Average|完整 GC 之後的舊層代記憶體集區大小|AppName、Pod|
...
|作用中節點|作用中節點|計數|Average|執行節點的數目|案例，ClusterName|
|閒置節點|閒置節點|計數|Average|閒置節點的數目|案例，ClusterName|
|無法使用的節點|無法使用的節點|Count|Average|無法使用的節點數目|案例，ClusterName|
|先佔節點|先佔節點|計數|Average|先占節點的數量|案例，ClusterName|
|正在離開的節點|正在離開的節點|計數|Average|離開節點的數目|案例，ClusterName|
|總核心數|總核心數|計數|Average|核心總數|案例，ClusterName|
|作用中核心|作用中核心|計數|Average|使用中核心數目|案例，ClusterName|
|閒置核心|閒置核心|計數|Average|閒置核心數|案例，ClusterName|
|無法使用的核心|無法使用的核心|計數|Average|無法使用的核心數目|案例，ClusterName|
|先佔節點|先佔節點|計數|Average|搶先核心數|案例，ClusterName|
|正在離開的核心|正在離開的核心|計數|Average|離開核心數|案例，ClusterName|
|配額使用率百分比|配額使用率百分比|計數|Average|使用的配額百分比|案例，ClusterName，VmFamilyName，VmPriority|
...
|StorageUsage|儲存體使用量|位元組|Average|儲存體使用量|節點|
...
|PendingTransactions|擱置交易|計數|Average|擱置交易|節點|
|QueuedTransactions|已排入佇列的交易|計數|Average|已排入佇列的交易|節點|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|CPU 百分比|Percentage CPU|百分比|Average|虛擬機器目前使用的配置計算單位百分比。|無|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|CPU 百分比|Percentage CPU|百分比|Average|虛擬機器目前使用的配置計算單位百分比。|RoleInstanceId|
...
|異動|交易|Count|總計|向儲存體服務或所指定 API 作業傳送的要求數。 此數目包括成功與失敗的要求，以及產生錯誤的要求。 使用 ResponseType 維度來取得不同類型回應的數目。|ResponseType、GeoType、ApiName、Authentication|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|BlobCapacity|Blob 容量|位元組|Average|儲存體帳戶 Blob 服務所使用的儲存體量 (位元組)。|BlobType，層|
|BlobCount|Blob 計數|計數|Average|儲存體帳戶 Blob 服務中的 Blob 數目。|BlobType，層|
|ContainerCount|Blob 容器計數|計數|Average|儲存體帳戶 Blob 服務中的容器數目。|無|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|TableCapacity|資料表容量|位元組|Average|儲存體帳戶資料表服務所使用的儲存體量 (位元組)。|無|
|TableCount|資料表計數|計數|Average|儲存體帳戶資料表服務中的資料表數目。|無|
|TableEntityCount|資料表實體計數|Count|Average|儲存體帳戶資料表服務中的資料表實體數目。|無|
|異動|交易|Count|總計|向儲存體服務或所指定 API 作業傳送的要求數。 此數目包括成功與失敗的要求，以及產生錯誤的要求。 使用 ResponseType 維度來取得不同類型回應的數目。|ResponseType、GeoType、ApiName、Authentication|
|輸入|輸入|位元組|總計|輸入資料量 (位元組)。 此數目包括從外部用戶端輸入到 Azure 儲存體與 Azure 內的輸入。|GeoType、ApiName、Authentication|
|輸出|輸出|位元組|總計|輸出資料量 (位元組)。 此數目包括從外部用戶端輸出到 Azure 儲存體與 Azure 內的輸出。 因此，此數目未反映可收費的輸出。|GeoType、ApiName、Authentication|
|SuccessServerLatency|成功伺服器延遲|毫秒|Average|Azure 儲存體用來處理成功要求的延遲（以毫秒為單位）。 此值不包括在 SuccessE2ELatency 中指定的網路延遲。|GeoType、ApiName、Authentication|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|FileCapacity|檔案容量|位元組|Average|儲存體帳戶的檔案服務所使用的儲存體數量（以位元組為單位）。|FileShare|
...
|FileShareCapacityQuota|檔案共用配額大小|位元組|Average|Azure 檔案儲存體服務可使用的儲存體數量上限（以位元組為單位）。|FileShare|
|異動|交易|Count|總計|向儲存體服務或所指定 API 作業傳送的要求數。 此數目包括成功與失敗的要求，以及產生錯誤的要求。 使用 ResponseType 維度來取得不同類型回應的數目。|ResponseType、GeoType、ApiName、Authentication、檔案共用|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|QueueCapacity|佇列容量|位元組|Average|儲存體帳戶佇列服務所使用的儲存體量 (位元組)。|無|
|QueueCount|佇列計數|計數|Average|儲存體帳戶佇列服務中的佇列數目。|無|
|QueueMessageCount|佇列訊息計數|Count|Average|儲存體帳戶佇列服務中的佇列訊息大約數目。|無|
|異動|交易|Count|總計|向儲存體服務或所指定 API 作業傳送的要求數。 此數目包括成功與失敗的要求，以及產生錯誤的要求。 使用 ResponseType 維度來取得不同類型回應的數目。|ResponseType、GeoType、ApiName、Authentication|
|輸入|輸入|位元組|總計|輸入資料量 (位元組)。 此數目包括從外部用戶端輸入到 Azure 儲存體與 Azure 內的輸入。|GeoType、ApiName、Authentication|
|輸出|輸出|位元組|總計|輸出資料量 (位元組)。 此數目包括從外部用戶端輸出到 Azure 儲存體與 Azure 內的輸出。 因此，此數目未反映可收費的輸出。|GeoType、ApiName、Authentication|
|SuccessServerLatency|成功伺服器延遲|毫秒|Average|Azure 儲存體用來處理成功要求的延遲（以毫秒為單位）。 此值不包括在 SuccessE2ELatency 中指定的網路延遲。|GeoType、ApiName、Authentication|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|CPU 百分比|Percentage CPU|百分比|Average|目前虛擬機器正在使用中的已配置計算單位百分比|無|
|Network In|計費網路流入量 (已淘汰)|位元組|總計|虛擬機器在所有網路介面上接收的可計費位元組數（連入流量）（已淘汰）|無|
|Network Out|計費網路流出量 (已淘汰)|位元組|總計|虛擬機器（連出流量）在所有網路介面上傳出的可計費位元組數（已淘汰）|無|
|Disk Read Bytes|Disk Read Bytes|位元組|總計|在監視期間從磁片讀取的位元組數|無|
|Disk Write Bytes|Disk Write Bytes|位元組|總計|在監視期間寫入磁片的位元組數|無|
|Disk Read Operations/Sec|Disk Read Operations/Sec|每秒計數|Average|磁碟讀取 IOPS|無|
|Disk Write Operations/Sec|Disk Write Operations/Sec|每秒計數|Average|磁碟寫入 IOPS|無|
|剩餘 CPU 信用額度|剩餘 CPU 信用額度|Count|Average|可用於高載的信用額度總數|無|
|CPU Credits Consumed|CPU Credits Consumed|Count|Average|由虛擬機器取用的信用額度總數|無|
...
|每一磁碟 QD|資料磁片 QD [（已淘汰）](portal-disk-metrics-deprecation.md)] （portal-disk-metrics-deprecation.md）|Count|Average|資料磁碟佇列深度 (或佇列長度)|SlotId|
...
|OS 每一磁碟 QD|OS 磁片 QD [（已淘汰）](portal-disk-metrics-deprecation.md)|Count|Average|OS 磁碟佇列深度 (或佇列長度)|無|
...
|OS 磁碟佇列深度|OS 磁碟佇列深度 (預覽)|Count|Average|OS 磁碟佇列深度 (或佇列長度)|無|
|連入流量|輸入流量|Count|Average|輸入流量是傳入方向的目前流量數（進入 VM 的流量）|無|
|連出流量|連出流量|Count|Average|輸出流程是輸出方向的目前流量數（從 VM 傳出的流量）|無|
|輸入流量最大建立速率|輸入流量最大建立速率|每秒計數|Average|輸入流量的最大建立速率（進入 VM 的流量）|無|
|輸出流量最大建立速率|輸出流量最大建立速率|每秒計數|Average|輸出流量的最大建立速率（從 VM 傳出的流量）|無|
|進階資料磁碟快取讀取命中|進階資料磁碟快取讀取命中 (預覽)|百分比|Average|進階資料磁碟快取讀取命中|LUN|
|進階資料磁碟快取讀取遺漏|進階資料磁碟快取讀取遺漏 (預覽)|百分比|Average|進階資料磁碟快取讀取遺漏|LUN|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|CPU 百分比|Percentage CPU|百分比|Average|目前虛擬機器正在使用中的已配置計算單位百分比|VMName|
|Network In|計費網路流入量 (已淘汰)|位元組|總計|虛擬機器在所有網路介面上接收的可計費位元組數（連入流量）（已淘汰）|VMName|
|Network Out|計費網路流出量 (已淘汰)|位元組|總計|虛擬機器（連出流量）在所有網路介面上傳出的可計費位元組數（已淘汰）|VMName|
...
|每一磁碟讀取作業/秒|資料磁片讀取作業數/秒[（已淘汰）](portal-disk-metrics-deprecation.md)|每秒計數|Average|在監視期間從單一磁片讀取 IOPS|SlotId|
|每一磁碟寫入作業/秒|資料磁片寫入作業數/秒[（已淘汰）](portal-disk-metrics-deprecation.md)|每秒計數|Average|在監視期間從單一磁片寫入 IOPS|SlotId|
|每一磁碟 QD|資料磁片 QD [（已淘汰）](portal-disk-metrics-deprecation.md)|Count|Average|資料磁碟佇列深度 (或佇列長度)|SlotId|
|OS 每一磁碟讀取位元組/秒|OS 磁片讀取位元組數/秒[（已淘汰）](portal-disk-metrics-deprecation.md)|每秒計數|Average|OS 磁片在監視期間從單一磁片讀取的位元組/秒|無|
|OS 每一磁碟寫入位元組/秒|OS 磁片寫入位元組數/秒[（已淘汰）](portal-disk-metrics-deprecation.md)|每秒計數|Average|OS 磁片在監視期間寫入單一磁片的位元組/秒|無|
|OS 每一磁碟讀取作業/秒|OS 磁片讀取作業數/秒[（已淘汰）](portal-disk-metrics-deprecation.md)|每秒計數|Average|在 OS 磁片監視期間從單一磁片讀取 IOPS|無|
|OS 每一磁碟寫入作業/秒|OS 磁片寫入作業數/秒[（已淘汰）](portal-disk-metrics-deprecation.md)|每秒計數|Average|針對 OS 磁片在監視期間從單一磁片寫入 IOPS|無|
...
|連入流量|輸入流量|Count|Average|輸入流量是傳入方向的目前流量數（進入 VM 的流量）|VMName|
|連出流量|連出流量|Count|Average|輸出流程是輸出方向的目前流量數（從 VM 傳出的流量）|VMName|
|輸入流量最大建立速率|輸入流量最大建立速率|每秒計數|Average|輸入流量的最大建立速率（進入 VM 的流量）|VMName|
|輸出流量最大建立速率|輸出流量最大建立速率|每秒計數|Average|輸出流量的最大建立速率（從 VM 傳出的流量）|VMName|
|進階資料磁碟快取讀取命中|進階資料磁碟快取讀取命中 (預覽)|百分比|Average|進階資料磁碟快取讀取命中|LUN，VMName|
|進階資料磁碟快取讀取遺漏|進階資料磁碟快取讀取遺漏 (預覽)|百分比|Average|進階資料磁碟快取讀取遺漏|LUN，VMName|
|進階 OS 磁碟快取讀取命中|進階 OS 磁碟快取讀取命中 (預覽)|百分比|Average|進階 OS 磁碟快取讀取命中|VMName|
|進階 OS 磁碟快取讀取遺漏|進階 OS 磁碟快取讀取遺漏 (預覽)|百分比|Average|進階 OS 磁碟快取讀取遺漏|VMName|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|TotalPullCount|提取總數|Count|Average|總映射提取次數|無|
|SuccessfulPullCount|成功提取計數|Count|Average|成功的映射提取次數|無|
|TotalPushCount|推送總數|Count|Average|映射推播總數總計|無|
|SuccessfulPushCount|成功推送計數|Count|Average|成功的映射推送數目|無|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|kube_node_status_allocatable_cpu_cores|受控叢集中可用的 cpu 核心總數|Count|Average|受控叢集中可用的 cpu 核心總數|無|
...
|BytesUploadedToCloudPerShare|上傳的雲端位元組數 (共用)|位元組|Average|報告期間從共用上傳至 Azure 的位元組總數。|共用|
|TotalCapacity|容量總計|位元組|Average|容量總計|無|
...
|BytesUploadedToCloud|上傳的雲端位元組數 (裝置)|位元組|Average|報告期間從裝置上傳至 Azure 的位元組總數。|無|
|HyperVVirtualProcessorUtilization|Edge 計算 - 百分比 CPU|百分比|Average|CPU 使用量百分比|InstanceName|
...
|IntegrationRuntimeQueueLength|整合執行階段佇列長度|計數|Average||IntegrationRuntimeName|
...
|JobStage|階段中的工作|計數|總計|每個階段中的工作數目。|無|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|ShareCount|已傳送的共用|計數|最大值|帳戶中已傳送的共用數|ShareName|
|ShareSubscriptionCount|已接收的共用|計數|最大值|帳戶中接收的共用數|ShareSubscriptionName|
...
|cpu_percent|CPU 百分比|百分比|Average|CPU 百分比|無|
|memory_percent|記憶體百分比|百分比|Average|記憶體百分比|無|
|iops|IOPS|Count|Average|每秒的 IO 作業數|無|
|storage_percent|儲存體百分比|百分比|Average|儲存體百分比|無|
|storage_used|已使用儲存體|位元組|Average|已使用儲存體|無|
|active_connections|作用中的連線|計數|Average|作用中的連線|無|
|network_bytes_egress|Network Out|位元組|總計|跨作用中連線的網路輸出|無|
...
|maximum_used_transactionIDs|使用的交易識別碼上限|計數|Average|使用的交易識別碼上限|無|
...
|RoutingDeliveries|路由傳遞（預覽）|毫秒|總計|IoT 中樞嘗試使用路由將訊息傳遞至所有端點的次數。 若要查看成功或失敗的嘗試次數，請使用 [結果] 維度。 若要查看失敗的原因，例如 [無效]、[已捨棄] 或 [孤立]，請使用 [FailureReasonCategory] 維度。 您也可以使用端點和 EndpointType 維度來瞭解有多少訊息傳遞至不同的端點。 計量值會針對每個傳遞嘗試增加一個，包括訊息是否傳遞至多個端點，或訊息是否多次傳遞至相同的端點。|ResourceId、EndpointType、端點、FailureReasonCategory、Result、RoutingSource|
|RoutingDeliveryLatency|路由傳遞延遲（預覽）|毫秒|Average|訊息輸入到 IoT 中樞的平均延遲（毫秒）和遙測訊息輸入到端點。 您可以使用端點和 EndpointType 維度來瞭解不同端點的延遲。|ResourceId、EndpointType、終結點、RoutingSource|
...
|CassandraTableThroughputUpdate|已更新 Cassandra 資料表輸送量|Count|計數|已更新 Cassandra 資料表輸送量|ChildResourceName、ApiKind、ApiKindResourceType、IsThroughputRequest|
|CassandraTableUpdate|Cassandra 資料表已更新|Count|計數|Cassandra 資料表已更新|ChildResourceName、ApiKind、ApiKindResourceType、IsThroughputRequest|
|CreateAccount|帳戶已建立|Count|計數|帳戶已建立|無|
|DataUsage|資料使用量|位元組|總計|5分鐘資料細微性報告的總數據使用量|CollectionName、DatabaseName、Region|
|DeleteAccount|帳戶已刪除|Count|計數|帳戶已刪除|無|
...
|GremlinDatabaseDelete|已刪除 Gremlin 資料庫|Count|計數|已刪除 Gremlin 資料庫|ApiKind、ApiKindResourceType、OperationType|
|GremlinDatabaseThroughputUpdate|已更新 Gremlin 資料庫輸送量|Count|計數|已更新 Gremlin 資料庫輸送量|ApiKind、ApiKindResourceType、IsThroughputRequest|
|GremlinDatabaseUpdate|已更新 Gremlin 資料庫|Count|計數|已更新 Gremlin 資料庫|ApiKind、ApiKindResourceType、IsThroughputRequest|
|GremlinGraphDelete|已刪除 Gremlin 圖形|Count|計數|已刪除 Gremlin 圖形|ChildResourceName、ApiKind、ApiKindResourceType、OperationType|
|GremlinGraphThroughputUpdate|已更新 Gremlin 圖形輸送量|Count|計數|已更新 Gremlin 圖形輸送量|ChildResourceName、ApiKind、ApiKindResourceType、IsThroughputRequest|
|GremlinGraphUpdate|Gremlin 圖已更新|Count|計數|Gremlin 圖已更新|ChildResourceName、ApiKind、ApiKindResourceType、IsThroughputRequest|
|IndexUsage|索引使用量|位元組|總計|5分鐘資料細微性報告的索引使用量總計|CollectionName、DatabaseName、Region|
|MetadataRequests|中繼資料要求|Count|計數|中繼資料要求計數。 Cosmos DB 會維護每個帳戶的系統中繼資料集合，可讓您免費列舉集合、資料庫等及其設定。|DatabaseName、CollectionName、Region、StatusCode、Role|
|MongoCollectionDelete|已刪除 Mongo 集合|Count|計數|已刪除 Mongo 集合|ChildResourceName、ApiKind、ApiKindResourceType、OperationType|
|MongoCollectionThroughputUpdate|已更新 Mongo 集合輸送量|Count|計數|已更新 Mongo 集合輸送量|ChildResourceName、ApiKind、ApiKindResourceType、IsThroughputRequest|
|MongoCollectionUpdate|已更新 Mongo 集合|Count|計數|已更新 Mongo 集合|ChildResourceName、ApiKind、ApiKindResourceType、IsThroughputRequest|
|MongoDBDatabaseUpdate|已更新 Mongo 資料庫|Count|計數|已更新 Mongo 資料庫|ApiKind、ApiKindResourceType、IsThroughputRequest|
|MongoDatabaseDelete|已刪除 Mongo 資料庫|Count|計數|已刪除 Mongo 資料庫|ApiKind、ApiKindResourceType、OperationType|
|MongoDatabaseThroughputUpdate|已更新 Mongo 資料庫輸送量|Count|計數|已更新 Mongo 資料庫輸送量|ApiKind、ApiKindResourceType、IsThroughputRequest|
|MongoRequestCharge|Mongo 要求收費|計數|總計|已使用的 Mongo 要求單位|DatabaseName、CollectionName、Region、CommandName、ErrorCode、Status|
|MongoRequests|Mongo 要求|Count|計數|已提出的 Mongo 要求數目|DatabaseName、CollectionName、Region、CommandName、ErrorCode、Status|
...
|MongoRequestsInsert|Mongo 插入要求率|每秒計數|Average|每秒的 Mongo 插入計數|DatabaseName、CollectionName、Region、CommandName、ErrorCode|
...
|RegionFailover|區域已故障切換|Count|計數|區域已故障切換|無|
|RemoveRegion|區域已移除|Count|計數|區域已移除|區域|
...
|SqlContainerDelete|Sql 容器已刪除|Count|計數|Sql 容器已刪除|ChildResourceName、ApiKind、ApiKindResourceType、OperationType|
|SqlContainerThroughputUpdate|Sql 容器輸送量已更新|Count|計數|Sql 容器輸送量已更新|ChildResourceName、ApiKind、ApiKindResourceType、IsThroughputRequest|
|SqlContainerUpdate|已更新 Sql 容器|Count|計數|已更新 Sql 容器|ChildResourceName、ApiKind、ApiKindResourceType、IsThroughputRequest|
|SqlDatabaseDelete|Sql Database 已刪除|Count|計數|Sql Database 已刪除|ApiKind、ApiKindResourceType、OperationType|
|SqlDatabaseThroughputUpdate|Sql 資料庫輸送量已更新|Count|計數|Sql 資料庫輸送量已更新|ApiKind、ApiKindResourceType、IsThroughputRequest|
|SqlDatabaseUpdate|Sql Database 已更新|Count|計數|Sql Database 已更新|ApiKind、ApiKindResourceType、IsThroughputRequest|
|TableTableDelete|已刪除 AzureTable 資料表|Count|計數|已刪除 AzureTable 資料表|ApiKind、ApiKindResourceType、OperationType|
|TableTableThroughputUpdate|已更新 AzureTable 資料表輸送量|Count|計數|已更新 AzureTable 資料表輸送量|ApiKind、ApiKindResourceType、IsThroughputRequest|
|TableTableUpdate|AzureTable 資料表已更新|Count|計數|AzureTable 資料表已更新|ApiKind、ApiKindResourceType、IsThroughputRequest|
|TotalRequestUnits|要求單位總計|Count|總計|已使用的要求單位|DatabaseName、CollectionName、Region、StatusCode、OperationType、Status|
|TotalRequests|要求總數|Count|計數|進行的要求數目|DatabaseName、CollectionName、Region、StatusCode、OperationType、Status|
|UpdateAccountKeys|帳戶金鑰已更新|Count|計數|帳戶金鑰已更新|KeyType|
|UpdateAccountNetworkSettings|已更新帳戶網路設定|Count|計數|已更新帳戶網路設定|無|
|UpdateAccountReplicationSettings|已更新帳戶複寫設定|Count|計數|已更新帳戶複寫設定|無|
|UpdateDiagnosticsSettings|已更新帳戶診斷設定|Count|計數|已更新帳戶診斷設定|DiagnosticSettingsName、ResourceGroupName|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|TransactionCount|交易計數|Count|計數|總交易計數|TransactionCount|
|SuccessCount|成功計數|Count|計數|成功的交易計數|SuccessCount|
|FailureCount|失敗計數|Count|計數|失敗的交易計數|FailureCount|
...
|INREQS|傳入的要求 (已淘汰)|Count|總計|命名空間的連入傳送要求總數（已淘汰）|無|
|SUCCREQ|成功的要求 (已淘汰)|Count|總計|命名空間的成功要求總數（已淘汰）|無|
|FAILREQ|失敗的要求 (已淘汰)|Count|總計|命名空間的失敗要求總計（已淘汰）|無|
|SVRBSY|伺服器忙線錯誤 (已淘汰)|Count|總計|命名空間的伺服器忙碌錯誤總計（已淘汰）|無|
|INTERR|內部伺服器錯誤 (已淘汰)|Count|總計|命名空間的內部伺服器錯誤總數（已淘汰）|無|
|MISCERR|其他錯誤 (已淘汰)|Count|總計|命名空間的失敗要求總計（已淘汰）|無|
|INMSGS|傳入的訊息 (過時) (已淘汰)|Count|總計|命名空間的傳入訊息總數。 此計量已過時。 請改用傳入訊息計量（已淘汰）|無|
|EHINMSGS|連入訊息數 (已過時)|Count|總計|命名空間的傳入訊息總計（已淘汰）|無|
|OUTMSGS|傳出的訊息 (過時) (已淘汰)|Count|總計|命名空間的傳出訊息總數。 此計量已過時。 請改用外寄訊息度量（已淘汰）|無|
|EHOUTMSGS|傳出訊息數 (已過時)|Count|總計|命名空間的傳出訊息總計（已淘汰）|無|
|EHINMBS|傳入位元組數 (過時) (已淘汰)|位元組|總計|命名空間的事件中樞傳入訊息輸送量。 此計量已過時。 請改用傳入位元組度量（已淘汰）|無|
|EHINBYTES|傳入位元組數 (已過時)|位元組|總計|命名空間的事件中樞傳入訊息輸送量（已淘汰）|無|
|EHOUTMBS|傳出位元組數 (過時) (已淘汰)|位元組|總計|命名空間的事件中樞傳出訊息輸送量。 此計量已過時。 請改用傳出位元組計量（已淘汰）|無|
|EHOUTBYTES|傳出位元組數 (已過時)|位元組|總計|命名空間的事件中樞傳出訊息輸送量（已淘汰）|無|
|EHABL|封存待處理項目訊息 (已淘汰)|Count|總計|命名空間的待處理專案中的事件中樞封存訊息（已淘汰）|無|
...
|availabilityResults/count|可用性集合|Count|計數|可用性測試計數|availabilityResult/name、availabilityResult/location、availabilityResult/success|
...
|dependencies/count|相依性呼叫|Count|計數|應用程式對外部資源的呼叫計數。|dependency/type、dependency/performanceBucket、dependency/success、dependency/target、dependency/resultCode、operation/綜合、雲端/roleInstance、雲端/擁有項|
...
|dependencies/failed|相依性呼叫失敗|Count|計數|應用程式對外部資源進行相依性呼叫的失敗計數。|dependency/type、dependency/performanceBucket、dependency/success、dependency/target、dependency/resultCode、operation/綜合、雲端/roleInstance、雲端/擁有項|
|pageViews/count|頁面檢視|Count|計數|頁面檢視計數。|作業/綜合、雲端/擁有|
...
|requests/count|伺服器要求|Count|計數|已完成 HTTP 要求的計數。|request/performanceBucket、request/resultCode、operation/綜合、雲端/roleInstance、要求/成功、雲端/擁有|
|requests/failed|失敗的要求|Count|計數|標示為失敗的 HTTP 要求計數。 在大多數情況下，這些要求的回應碼會 >= 400，且不等於 401。|request/performanceBucket、request/resultCode、request/success、operation/綜合、cloud/roleInstance、雲端/擁有|
|要求/速率|伺服器要求率|每秒計數|Average|每秒伺服器要求的速率|request/performanceBucket、request/resultCode、operation/綜合、雲端/roleInstance、要求/成功、雲端/擁有|
|exceptions/count|例外狀況|Count|計數|所有未攔截的例外狀況合併計數。|雲端/擁有的雲端/roleInstance，用戶端/類型|
|exceptions/browser|瀏覽器例外狀況|Count|計數|在瀏覽器中擲回的未攔截例外狀況計數。|用戶端/isServer、雲端/擁有|
|exceptions/server|伺服器例外狀況|Count|計數|伺服器應用程式中擲回但未攔截的例外狀況計數。|用戶端/isServer、雲端/擁有群、雲端/roleInstance|
|traces/count|追蹤|Count|計數|追蹤文件計數|追蹤/severityLevel、作業/綜合、雲端/擁有的雲端/roleInstance|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|connectedDeviceCount|已連線的裝置總數|計數|Average|連線到 IoT Central 的裝置數目|無|
|c2d。屬性. 讀取. 成功|成功的裝置屬性從 IoT Central 讀取|計數|總計|從 IoT Central 起始的所有成功屬性讀取計數|無|
|c2d. 屬性。讀取失敗|失敗的裝置屬性從 IoT Central 讀取|計數|總計|從 IoT Central 起始的所有失敗屬性讀取計數|無|
|d2c。屬性. 讀取. 成功|裝置的成功屬性讀取|計數|總計|從裝置起始的所有成功屬性讀取計數|無|
|d2c. 屬性。讀取失敗|裝置的裝置屬性讀取失敗|計數|總計|從裝置起始的所有失敗屬性讀取計數|無|
|c2d。屬性. update. success|從 IoT Central 成功的裝置屬性更新|計數|總計|從 IoT Central 起始的所有成功屬性更新計數|無|
|c2d. 屬性. update. 失敗|IoT Central 的裝置屬性更新失敗|計數|總計|從 IoT Central 起始的所有失敗屬性更新計數|無|
|d2c。屬性. update. success|裝置的成功屬性更新|計數|總計|從裝置起始的所有成功屬性更新計數|無|
|d2c. 屬性. update. 失敗|裝置的裝置屬性更新失敗|計數|總計|從裝置起始的所有失敗屬性更新計數|無|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|ServiceApiHit|服務 API 點擊次數總計|Count|計數|服務 API 點擊次數總數|ActivityType、ActivityName|
|ServiceApiLatency|整體服務 API 延遲|毫秒|Average|服務 API 要求的整體延遲|ActivityType、ActivityName、StatusCode、StatusCodeClass|
|ServiceApiResult|服務 API 結果總計|Count|計數|服務 API 結果總數|ActivityType、ActivityName、StatusCode、StatusCodeClass|
...
|QueryDuration|查詢持續時間|毫秒|Average|查詢的持續時間（秒）|QueryStatus|
|IngestionUtilization|擷取使用率|百分比|Average|在叢集中使用的擷取位置的比率|無|
...
|EventsProcessedForEventHubs|已處理的事件 (針對事件/IoT 中樞)|計數|總計|從事件/IoT 中樞內嵌時，由叢集處理的事件數目|EventStatus|
|IngestionResult|擷取結果|計數|計數|擷取作業數|IngestionResultDetails|
|CPU|CPU|百分比|Average|CPU 使用率等級|無|
|ContinuousExportNumOfRecordsExported|連續匯出–匯出記錄的數目|計數|總計|針對匯出作業期間寫入的每個儲存體成品所匯出的記錄數目|ContinuousExportName，資料庫|
|ExportUtilization|匯出使用率|百分比|最大值|匯出使用率|無|
|ContinuousExportPendingCount|連續匯出擱置計數|計數|最大值|準備好執行的暫止連續匯出作業數|無|
|ContinuousExportMaxLatenessMinutes|連續匯出最大延遲|計數|最大值|叢集中的連續匯出作業所報告的延遲（以分鐘為單位）|無|
...
|SteamingIngestRequestRate|串流內嵌要求率|Count|RateRequestsPerSecond|串流內嵌要求速率（每秒的要求數）|無|
...
|InstanceCount|執行個體計數|計數|Average|總實例計數|無|
...
|錯誤|Errors|Count|總計|此工作區中的執行錯誤數目|情節|
|警告|警告|Count|總計|此工作區中的執行警告數目|情節|
|成功的模型註冊|成功的模型註冊|Count|總計|在此工作區中成功的模型註冊數目|情節|
|失敗的模型註冊|失敗的模型註冊|Count|總計|在此工作區中失敗的模型註冊數目|案例，StatusCode|
|已啟動的模型部署|已啟動的模型部署|Count|總計|在此工作區中啟動的模型部署數目|情節|
|成功的模型部署|成功的模型部署|Count|總計|在此工作區中成功的模型部署數|情節|
...
|作用中節點|作用中節點|計數|Average|Acitve 節點的數目。 這些是目前正在執行作業的節點。|案例，ClusterName|
...
|先佔節點|先佔節點|計數|Average|已搶先節點的數目。 這些節點是從可用節點集區中取出的低優先順序節點。|案例，ClusterName|
|正在離開的節點|正在離開的節點|計數|Average|離開節點的數目。 離開節點是剛完成作業並將進入閒置狀態的節點。|案例，ClusterName|
|總核心數|總核心數|計數|Average|核心總數|案例，ClusterName|
|作用中核心|作用中核心|計數|Average|使用中核心數目|案例，ClusterName|
|閒置核心|閒置核心|計數|Average|閒置核心數|案例，ClusterName|
|無法使用的核心|無法使用的核心|計數|Average|無法使用的核心數目|案例，ClusterName|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|使用量|使用量|Count|計數|API 呼叫的計數|ApiCategory、ApiName、ResultType、ResponseCode|
...
|StreamingPolicyQuota|串流原則配額|Count|Average|目前媒體服務帳戶允許的串流原則數目|無|
...
|VolumeLogicalSize|磁片區已耗用大小|位元組|Average|磁碟區的邏輯大小 (已使用的位元組)|無|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|VolumePoolAllocatedUsed|配置給磁片區大小的集區|位元組|Average|集區的已使用配置大小|無|
...
|FirewallHealth|防火牆健全狀態|百分比|Average|防火牆健全狀態|狀態、原因|
|DataProcessed|已處理的資料量|位元組|總計|防火牆處理的總數據量|無|
|SNATPortUtilization|SNAT 連接埠使用率|百分比|Average|SNAT 連接埠使用率|無|
...
|AvgRequestCountPerHealthyHost|每個健康情況主機每分鐘的要求|計數|Average|集區中每個狀況良好後端主機每分鐘的平均要求計數|BackendSettingsPool|
...
|NewConnectionsPerSecond|每秒的新連線數|每秒計數|Average|以應用程式閘道建立的每秒新連線數|無|
|CpuUtilization|CPU 使用率|百分比|Average|應用程式閘道目前的 CPU 使用率|無|
|CapacityUnits|目前的容量單位|計數|Average|耗用的容量單位|無|
|FixedBillableCapacityUnits|固定計費容量單位|計數|Average|將收取的最小容量單位|無|
|EstimatedBilledCapacityUnits|預估的計費容量單位|計數|Average|預估的容量單位將收取費用|無|
...
|MatchedCount|Web 應用程式防火牆 v1 規則散發總數|計數|總計|連入流量的 Web 應用程式防火牆 v1 總規則分佈|RuleGroup、RuleId|
|BlockedCount|Web 應用程式防火牆 v1 已封鎖要求規則散發|計數|總計|Web 應用程式防火牆 v1 已封鎖要求規則散發|RuleGroup、RuleId|
|BlockedReqCount|Web 應用程式防火牆 v1 已封鎖的要求計數|計數|總計|Web 應用程式防火牆 v1 已封鎖的要求計數|無|
...
|BillableResponseSize|可計費的回應大小|位元組|總計|當做回應從 HTTP/S proxy 傳送至用戶端的可計費位元組數（每個要求最少的2KB）。|HttpStatus、HttpStatusGroup、ClientRegion、ClientCountry|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|QueryVolume|查詢磁碟區|計數|總計|為私人 DNS 區域提供服務的查詢數目|無|
...
|活動訊號|Heartbeat|Count|總計|活動訊號|Computer、OSType、Version、SourceComputerId|
...
|NumberOfAssetsWithClassifications|具有至少一個分類的資產數目|計數|Average|指出至少有一個標記分類的資產數目。|ResourceId|
...
|DeadletteredMessages|佇列/主題中的無效訊息計數。|計數|Average|佇列/主題中的無效訊息計數。|EntityName|
...
|dtu_limit|DTU 限制|Count|Average|DTU 限制。 適用于以 DTU 為基礎的資料庫。|無|
|dtu_used|已使用 DTU|計數|Average|使用的 DTU。 適用于以 DTU 為基礎的資料庫。|無|
|cpu_limit|CPU 限制|計數|Average|CPU 限制。 適用于以 vCore 為基礎的資料庫。|無|
|cpu_used|已使用的 CPU|計數|Average|使用的 CPU。 適用于以 vCore 為基礎的資料庫。|無|
...
|app_cpu_billed|已計費應用程式 CPU|計數|總計|應用程式 CPU 計費。 適用于無伺服器資料庫。|無|
...
|eDTU_limit|eDTU 限制|Count|Average|eDTU 限制。 適用于以 DTU 為基礎的彈性集區。|無|
|storage_limit|資料大小上限|位元組|Average|資料大小上限|無|
|eDTU_used|已使用 eDTU|Count|Average|使用的 eDTU。 適用于以 DTU 為基礎的彈性集區。|無|
|database_eDTU_used|已使用 eDTU|Count|Average|已使用 eDTU|DatabaseResourceId|
|storage_used|已使用的資料空間|位元組|Average|已使用的資料空間|無|
|database_storage_used|已使用的資料空間|位元組|Average|已使用的資料空間|DatabaseResourceId|
|xtp_storage_percent|記憶體內部 OLTP 儲存體百分比|百分比|Average|記憶體內部 OLTP 儲存體百分比|無|
|cpu_limit|CPU 限制|Count|Average|CPU 限制。 適用于以 vCore 為基礎的彈性集區。|無|
|database_cpu_limit|CPU 限制|Count|Average|CPU 限制|DatabaseResourceId|
|cpu_used|已使用的 CPU|Count|Average|使用的 CPU。 適用于以 vCore 為基礎的彈性集區。|無|
|database_cpu_used|已使用的 CPU|Count|Average|已使用的 CPU|DatabaseResourceId|
...
|BlobCapacity|Blob 容量|位元組|Average|儲存體帳戶 Blob 服務所使用的儲存體量 (位元組)。|BlobType，層|
|BlobCount|Blob 計數|計數|Average|儲存體帳戶 Blob 服務中的 Blob 數目。|BlobType，層|
|ContainerCount|Blob 容器計數|Count|Average|儲存體帳戶 Blob 服務中的容器數目。|無|
|IndexCapacity|索引容量|位元組|Average|ADLS Gen2 （階層）索引所使用的儲存體數量（以位元組為單位）。|無|
|異動|交易|Count|總計|向儲存體服務或所指定 API 作業傳送的要求數。 此數目包括成功與失敗的要求，以及產生錯誤的要求。 使用 ResponseType 維度來取得不同類型回應的數目。|ResponseType、GeoType、ApiName、Authentication|
|輸入|輸入|位元組|總計|輸入資料量 (位元組)。 此數目包括從外部用戶端輸入到 Azure 儲存體與 Azure 內的輸入。|GeoType、ApiName、Authentication|
|輸出|輸出|位元組|總計|輸出資料量 (位元組)。 此數目包括從外部用戶端輸出到 Azure 儲存體與 Azure 內的輸出。 因此，此數目未反映可收費的輸出。|GeoType、ApiName、Authentication|
...
|QueueCapacity|佇列容量|位元組|Average|儲存體帳戶佇列服務所使用的儲存體量 (位元組)。|無|
|QueueCount|佇列計數|計數|Average|儲存體帳戶佇列服務中的佇列數目。|無|
|QueueMessageCount|佇列訊息計數|計數|Average|儲存體帳戶佇列服務中的佇列訊息大約數目。|無|
|異動|交易|Count|總計|向儲存體服務或所指定 API 作業傳送的要求數。 此數目包括成功與失敗的要求，以及產生錯誤的要求。 使用 ResponseType 維度來取得不同類型回應的數目。|ResponseType、GeoType、ApiName、Authentication|
|輸入|輸入|位元組|總計|輸入資料量 (位元組)。 此數目包括從外部用戶端輸入到 Azure 儲存體與 Azure 內的輸入。|GeoType、ApiName、Authentication|
|輸出|輸出|位元組|總計|輸出資料量 (位元組)。 此數目包括從外部用戶端輸出到 Azure 儲存體與 Azure 內的輸出。 因此，此數目未反映可收費的輸出。|GeoType、ApiName、Authentication|
|SuccessServerLatency|成功伺服器延遲|毫秒|Average|Azure 儲存體用來處理成功要求的平均延遲 (毫秒)。 此值不包括在 AverageE2ELatency 中指定的網路延遲。|GeoType、ApiName、Authentication|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|ClientIOPS|用戶端 IOPS 總計|Count|Average|快取處理的用戶端檔案作業的速率。|無|
...
|StorageTargetHealth|儲存目標健康狀態|Count|Average|快取和儲存體目標之間的連線能力測試布林值結果。|無|
|Uptime|Uptime|Count|Average|快取和監視系統之間的連線能力測試布林值結果。|無|
|StorageTargetIOPS|StorageTarget IOPS 總計|Count|Average|快取傳送至特定 StorageTarget 之所有檔案作業的速率。|StorageTarget|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|ServerSyncSessionResult|同步工作階段結果|計數|Average|每次伺服器端點成功完成與雲端端點的同步會話時，會將值記錄為1的度量|SyncGroupName、ServerEndpointName、SyncDirection|
|StorageSyncSyncSessionAppliedFilesCount|同步的檔案|計數|總計|已同步處理的檔案計數|SyncGroupName、ServerEndpointName、SyncDirection|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|SyncGroupSyncSessionAppliedFilesCount|同步的檔案|計數|總計|已同步處理的檔案計數|SyncGroupName、ServerEndpointName、SyncDirection|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|ServerEndpointSyncSessionAppliedFilesCount|同步的檔案|計數|總計|已同步處理的檔案計數|ServerEndpointName、SyncDirection|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|OrchestrationPipelineRunsEnded|管線執行結束|計數|總計|成功、失敗或已取消之協調流程管線執行的計數|結果、FailureType、管線|
|OrchestrationActivityRunsEnded|活動執行結束|計數|總計|成功、失敗或已取消之協調流程活動的計數|Result、FailureType、Activity、ActivityType、Pipeline|
|OrchestrationTriggersEnded|觸發程式已結束|計數|總計|成功、失敗或已取消之協調流程觸發程式的計數|Result、FailureType、Trigger|
|SQLOnDemandLoginAttempts|登入嘗試|計數|總計|成功或失敗的登入嘗試計數|結果|
|SQLOnDemandQueriesEnded|查詢已結束|計數|總計|成功、失敗或已取消的查詢計數|結果|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|SparkJobsEnded|結束的應用程式|計數|總計|已結束的應用程式計數|JobType、Jobresult.txt|
...
|ConnectionsBlockedByFirewall|防火牆封鎖的連線|計數|總計|防火牆規則封鎖的連線計數。 若計數很高，請重新流覽 SQL 集區的存取控制原則，並監視這些連接|無|
...
|WLGActiveQueries|工作負載群組使用中查詢|計數|總計|工作負載群組內的使用中查詢。 使用未篩選的此計量和解除會顯示在系統上執行的所有使用中查詢|IsUserDefined、WorkloadGroup|
|WLGActiveQueriesTimeouts|工作負載群組查詢逾時|計數|總計|查詢已超時的工作負載群組。此計量所報告的查詢超時，只有在查詢開始執行之後（不包括因鎖定或資源等候而造成的等候時間）|IsUserDefined、WorkloadGroup|
...
|WLGQueuedQueries|已排入佇列的工作負載群組查詢|計數|總計|達到最大並行限制之後，佇列的要求累計計數|IsUserDefined、WorkloadGroup|
...
|DiskReadOperations|磁碟讀取作業|計數|總計|上一個範例期間的 IO 讀取作業數目。 請注意，這些作業可能是可變大小的。|無|
...
|MemoryUsed|已使用的記憶體|位元組|Average|VM 使用的電腦記憶體數量。|無|
|MemoryGranted|已授與的記憶體|位元組|Average|主機授與 VM 的記憶體數量。 記憶體在觸及一次之後才會授與給主機，而且如果 VMkernel 需要記憶體，則會交換或膨脹。|無|
|MemoryActive|作用中記憶體|位元組|Average|VM 在過去小型時間範圍內使用的記憶體數量。 這是 VM 目前所需記憶體數量的 "true"。 其他、未使用的記憶體可能會交換或膨脹，而不會影響到來賓的效能。|無|
|CPU 百分比|Percentage CPU|百分比|Average|CPU 使用率。 此值是以100% 回報，代表系統上的所有處理器核心。 例如，使用四核心系統50% 的雙向 VM，完全使用兩個核心。|無|
...
|TcpSynSent|已傳送的 TCP 同步|計數|Average|已傳送的 TCP 同步|執行個體|
|TcpSynReceived|已接收的 TCP 同步|計數|Average|已接收的 TCP 同步|執行個體|
|TcpEstablished|已建立的 TCP|計數|Average|已建立的 TCP|執行個體|
|TcpFinWait1|TCP 完成等候 1|計數|Average|TCP 完成等候 1|執行個體|
|TcpFinWait2|TCP 完成等候 2|計數|Average|TCP 完成等候 2|執行個體|
|TcpClosing|正在關閉的 TCP|計數|Average|正在關閉的 TCP|執行個體|
|TcpCloseWait|等候關閉的 TCP|計數|Average|等候關閉的 TCP|執行個體|
|TcpLastAck|TCP 上次認可|計數|Average|TCP 上次認可|執行個體|
...
|HealthCheckStatus|健康情況檢查狀態|計數|Average|健康情況檢查狀態|執行個體|
...
|TotalFrontEnds|前端總數|Count|Average|前端總數|無|
|SmallAppServicePlanInstances|小型 App Service 方案背景工作角色|Count|Average|小型 App Service 方案背景工作角色|無|
|MediumAppServicePlanInstances|中型 App Service 方案背景工作角色|Count|Average|中型 App Service 方案背景工作角色|無|
|LargeAppServicePlanInstances|大型 App Service 方案背景工作角色|Count|Average|大型 App Service 方案背景工作角色|無|

## <a name="microsoftwebhostingenvironmentsworkerpools"></a>Microsoft.Web/hostingEnvironments/workerPools

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|WorkersTotal|背景工作角色總數|Count|Average|背景工作角色總數|無|
|WorkersAvailable|可用的背景工作角色|Count|Average|可用的背景工作角色|無|
|WorkersUsed|已使用的背景工作角色|Count|Average|已使用的背景工作角色|無|
|CpuPercentage|CPU 百分比|百分比|Average|CPU 百分比|執行個體|
|MemoryPercentage|記憶體百分比|百分比|Average|記憶體百分比|執行個體|
