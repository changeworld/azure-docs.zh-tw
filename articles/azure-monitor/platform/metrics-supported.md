...
>
> 有關可通過診斷設置匯出的平臺指標清單，請參閱[本文](metrics-supported-export-diagnostic-settings.md)。
...
|mashup_engine_private_bytes_metric|M 引擎專用位元組|位元組|Average|由 mashup 引擎進程使用專用位元組。|ServerResourceType|
|mashup_engine_virtual_bytes_metric|M 引擎虛擬位元組|位元組|Average|按 mashup 引擎進程進行的虛擬位元組使用。|ServerResourceType|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|TotalRequests|閘道請求總數（已棄用）|Count|總計|閘道請求數 - 使用多維請求指標，改為使用閘道回應代碼類別維度|位置，主機名稱|
|SuccessfulRequests|成功的閘道請求（已棄用）|Count|總計|成功閘道請求數 - 使用多維請求指標，改為使用閘道回應代碼類別維度|位置，主機名稱|
|UnauthorizedRequests|未經授權的閘道請求（已棄用）|Count|總計|未經授權的閘道請求數 - 使用多維請求指標，改為使用閘道回應代碼類別維度|位置，主機名稱|
|FailedRequests|失敗的閘道請求（已棄用）|Count|總計|閘道請求中的故障數 - 使用多維請求指標，改為使用閘道回應代碼類別維度|位置，主機名稱|
|OtherRequests|其他閘道請求（已棄用）|Count|總計|其他閘道請求數 - 使用多維請求指標，改為使用閘道回應代碼類別維度|位置，主機名稱|
|Duration|閘道要求的整體持續期間|毫秒|Average|閘道要求的整體持續期間，以毫秒為單位|位置，主機名稱|
|後端持續時間|後端請求的持續時間|毫秒|Average|後端請求的持續時間（以毫秒為單位）|位置，主機名稱|
|Capacity|Capacity|百分比|Average|ApiManagement 服務的使用率衡量標準|Location|
|活動中心總活動|事件中心事件總數|Count|總計|發送到事件 Hub 的事件數|Location|
|活動中心成功活動|成功事件中心事件|Count|總計|成功事件中心事件數|Location|
|事件Hub完全失敗事件|失敗的事件中心事件|Count|總計|失敗的事件Hub事件數|Location|
|事件Hub拒絕事件|已拒絕的事件中心事件|Count|總計|被拒絕的事件事件數（配置錯誤或未授權）|Location|
|事件HubHubd事件|限制事件中心事件|Count|總計|受限制的事件Hub 事件數|Location|
|事件HubTimedout事件|超時事件中心事件|Count|總計|超時事件 Hub 事件數|Location|
|事件Hub放棄事件|丟棄的事件中心事件|Count|總計|由於已達到佇列大小限制而跳過的事件數|Location|
|事件中心總位元組已發送|事件中心事件的大小|位元組|總計|事件中心事件的總大小（以位元組為單位）|Location|
|Requests|Requests|Count|總計|具有多個維度的閘道請求指標|位置、主機名稱、LastError、後端回應代碼、閘道回應代碼、後端回應代碼類別、閘道回應代碼類別|


## <a name="microsoftappconfigurationconfigurationstores"></a>微軟.應用程式佈建/配置商店

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|Http 傳入請求計數|Http 傳入請求計數|Count|Count|傳入 HTTP 請求的總數。|None|
|失敗的 HttpRequestCount|失敗的 HttpRequestCount|Count|Count|失敗的 HTTP 請求。|None|
|Http 傳入請求持續時間|Http 傳入請求持續時間|Count|Average|HTTP 請求上的延遲。|None|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|系統 Cpu 使用百分比|系統 CPU 使用率|百分比|Average|整個系統最近的 cpu 使用方式|應用程式名稱，Pod|
|應用Cpu使用百分比|應用 CPU 使用率|百分比|Average|應用 JVM CPU 使用率|應用程式名稱，Pod|
|應用記憶體已提交|應用記憶體分配|位元組|Average|分配給 JVM 的記憶體（以位元組為單位）|應用程式名稱，Pod|
|應用記憶體已使用|使用的應用程式記憶體|位元組|Average|以位元組為單位使用的應用記憶體|應用程式名稱，Pod|
|應用程式記憶體最大值|應用記憶體最大值|位元組|最大值|可用於記憶體管理的最大記憶體量（以位元組為單位）|應用程式名稱，Pod|
|最大記憶池位元組|最大可用舊代資料大小|位元組|Average|舊代記憶體池的最大大小|應用程式名稱，Pod|
|舊記憶體池位元組|舊代資料大小|位元組|Average|完整 GC 後舊代記憶體池的大小|應用程式名稱，Pod|
|舊基因提升位元組|提升為舊代資料大小|位元組|最大值|GC 之前舊代記憶體池的大小正增長計數到 GC 之後|應用程式名稱，Pod|
|楊根升點位元組|提升為年輕一代資料大小|位元組|最大值|增加一個 GC 後年輕代記憶體池的大小，以在下一個 GC 之前增加|應用程式名稱，Pod|
|GC暫停總計數|GC 暫停計數|Count|總計|GC 暫停計數|應用程式名稱，Pod|
|GC暫停總時間|GC 暫停總時間|毫秒|總計|GC 暫停總時間|應用程式名稱，Pod|
|托姆卡特森特斯|湯姆卡特總傳送的位元組|位元組|總計|湯姆卡特總傳送的位元組|應用程式名稱，Pod|
...
|湯姆卡特請求最大時間|湯姆卡特請求最大時間|毫秒|最大值|湯姆卡特請求最大時間|應用程式名稱，Pod|
|湯姆卡特錯誤計數|湯姆卡特全域錯誤|Count|總計|湯姆卡特全域錯誤|應用程式名稱，Pod|
|湯姆卡特會話活動最大值Count|Tomcat 會話 最大活動計數|Count|總計|Tomcat 會話 最大活動計數|應用程式名稱，Pod|
|湯姆卡特會話活動最大時間|Tomcat 會話最大啟用時間|毫秒|最大值|Tomcat 會話最大啟用時間|應用程式名稱，Pod|
|湯姆卡特會話活動電流Count|Tomcat 會話活動計數|Count|總計|Tomcat 會話活動計數|應用程式名稱，Pod|
|湯姆卡特會話創建計數|Tomcat 會話創建計數|Count|總計|Tomcat 會話創建計數|應用程式名稱，Pod|
|湯姆卡特會話過期計數|Tomcat 會話過期計數|Count|總計|Tomcat 會話過期計數|應用程式名稱，Pod|
|湯姆卡特會話拒絕計數|Tomcat 會話已拒絕計數|Count|總計|Tomcat 會話已拒絕計數|應用程式名稱，Pod|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|TotalJob|工作總數|Count|總計|工作總數|運行簿，狀態|
|總更新部署運行|更新部署執行總計|Count|總計|軟體更新部署運行總數|軟體更新配置名稱、狀態|
|總更新部署電腦運行|更新部署機器執行總計|Count|總計|軟體更新部署電腦在軟體更新部署運行中運行的總|軟體更新配置名稱、狀態、目的電腦、軟體更新配置RunId|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|提交作業|提交作業|Count|總計|提交的作業數|方案，群集名稱|
|已完成作業|已完成作業|Count|總計|已完成的作業數|方案、群集名稱、結果類型|
|節點總數|節點總數|Count|Average|總節點數|方案，群集名稱|
|活動節點|活動節點|Count|Average|執行節點的數目|方案，群集名稱|
|空閒節點|空閒節點|Count|Average|閒置節點的數目|方案，群集名稱|
|無法使用的節點|無法使用的節點|Count|Average|無法使用的節點數目|方案，群集名稱|
|搶佔節點|搶佔節點|Count|Average|先占節點的數量|方案，群集名稱|
|離開節點|離開節點|Count|Average|離開節點數|方案，群集名稱|
|核心總數|核心總數|Count|Average|總內核數|方案，群集名稱|
|活動核心|活動核心|Count|Average|活動內核數|方案，群集名稱|
|空閒核心|空閒核心|Count|Average|空閒內核數|方案，群集名稱|
|不可用的內核|不可用的內核|Count|Average|不可用內核數|方案，群集名稱|
|搶佔型核心|搶佔型核心|Count|Average|搶佔型內核數|方案，群集名稱|
|離開核心|離開核心|Count|Average|離開內核的數量|方案，群集名稱|
|配額利用率百分比|配額利用率百分比|Count|Average|使用的配額百分比|方案，群集名稱，Vm 家族名稱，Vm 優先|
...
|存儲使用|存儲使用方式|位元組|Average|存儲使用方式|節點|
|IORead 位元組|IO 讀取位元組|位元組|總計|IO 讀取位元組|節點|
|IOWrite 位元組|IO 寫入位元組|位元組|總計|IO 寫入位元組|節點|
|連接已接受|已接受連接|Count|總計|已接受連接|節點|
|連接處理|已處理的連接|Count|總計|已處理的連接|節點|
|連接活動|作用中的連線|Count|Average|作用中的連線|節點|
|請求處理|已處理的請求|Count|總計|已處理的請求|節點|
|已處理的塊|已處理的塊|Count|總計|已處理的塊|節點|
|已處理的交易|已處理的交易|Count|總計|已處理的交易|節點|
|待定交易|待處理事務|Count|Average|待處理事務|節點|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|WebApplicationFirewallRequestCount|Web 應用程式防火牆要求計數|Count|總計|Web 應用程式防火牆處理的用戶端要求數|策略名稱、規則名稱、操作|
...
|輸入|輸入|位元組|總計|輸入資料量 (位元組)。 此數目包括從外部用戶端輸入到 Azure 儲存體與 Azure 內的輸入。|地理類型，ApiName，身份驗證|
|輸出|輸出|位元組|總計|輸出資料量 (位元組)。 此數目包括從外部用戶端輸出到 Azure 儲存體與 Azure 內的輸出。 因此，此數目未反映可收費的輸出。|地理類型，ApiName，身份驗證|
|SuccessServerLatency|成功伺服器延遲|毫秒|Average|Azure 存儲用於處理成功請求的延遲（以毫秒為單位）。 此值不包括在 SuccessE2ELatency 中指定的網路延遲。|地理類型，ApiName，身份驗證|
|SuccessE2ELatency|成功 E2E 延遲|毫秒|Average|向存儲服務或指定的 API 操作成功請求的端到端延遲（以毫秒為單位）。 此值包括 Azure 儲存體內讀取要求、傳送回應及接收回應認可的必要處理時間。|地理類型，ApiName，身份驗證|
|可用性|可用性|百分比|Average|儲存體服務或所指定 API 作業的可用性百分比。 可用性的計算方式是將 TotalBillableRequests 值除以適用要求數目，包括產生意外錯誤的要求。 所有意外錯誤都會導致儲存體或所指定 API 作業的可用性降低。|地理類型，ApiName，身份驗證|
...
|IndexCapacity|索引容量|位元組|Average|ADLS Gen2（分層）索引（以位元組為單位）使用的存儲量。|None|
|交易|交易|Count|總計|向儲存體服務或所指定 API 作業傳送的要求數。 此數目包括成功與失敗的要求，以及產生錯誤的要求。 使用 ResponseType 維度來取得不同類型回應的數目。|回應類型、地理類型、ApiName、身份驗證|
|輸入|輸入|位元組|總計|輸入資料量 (位元組)。 此數目包括從外部用戶端輸入到 Azure 儲存體與 Azure 內的輸入。|地理類型，ApiName，身份驗證|
|輸出|輸出|位元組|總計|輸出資料量 (位元組)。 此數目包括從外部用戶端輸出到 Azure 儲存體與 Azure 內的輸出。 因此，此數目未反映可收費的輸出。|地理類型，ApiName，身份驗證|
|SuccessServerLatency|成功伺服器延遲|毫秒|Average|Azure 存儲用於處理成功請求的延遲（以毫秒為單位）。 此值不包括在 SuccessE2ELatency 中指定的網路延遲。|地理類型，ApiName，身份驗證|
...
## <a name="microsoftclassicstoragestorageaccountsfileservices"></a>微軟.經典存儲/存儲帳戶/檔服務

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|FileCapacity|檔案容量|位元組|Average|存儲帳戶的檔服務（以位元組為單位）使用的存儲量。|FileShare|
|FileCount|檔案計數|Count|Average|存儲帳戶的檔服務中的檔數。|FileShare|
|FileShareCount|檔案共用計數|Count|Average|存儲帳戶的檔服務中的檔共用數。|None|
|檔共用快照計數|檔共用快照計數|Count|Average|存儲帳戶檔服務中共用上存在的快照數。|FileShare|
|檔共用快照大小|檔共用快照大小|位元組|Average|存儲帳戶的檔服務中的快照使用的存儲量（以位元組為單位）。|FileShare|
|檔共用配額|檔共用配額大小|位元組|Average|Azure 檔服務（以位元組為單位）可以使用的存儲量的上限。|FileShare|
|交易|交易|Count|總計|向儲存體服務或所指定 API 作業傳送的要求數。 此數目包括成功與失敗的要求，以及產生錯誤的要求。 使用 ResponseType 維度來取得不同類型回應的數目。|回應類型、地理類型、ApiName、身份驗證、檔共用|
|輸入|輸入|位元組|總計|輸入資料量 (位元組)。 此數目包括從外部用戶端輸入到 Azure 儲存體與 Azure 內的輸入。|地理類型，ApiName，身份驗證，檔共用|
|輸出|輸出|位元組|總計|輸出資料量 (位元組)。 此數目包括從外部用戶端輸出到 Azure 儲存體與 Azure 內的輸出。 因此，此數目未反映可收費的輸出。|地理類型，ApiName，身份驗證，檔共用|
|SuccessServerLatency|成功伺服器延遲|毫秒|Average|Azure 存儲用於處理成功請求的延遲（以毫秒為單位）。 此值不包括在 SuccessE2ELatency 中指定的網路延遲。|地理類型，ApiName，身份驗證，檔共用|
|SuccessE2ELatency|成功 E2E 延遲|毫秒|Average|向存儲服務或指定的 API 操作成功請求的端到端延遲（以毫秒為單位）。 此值包括 Azure 儲存體內讀取要求、傳送回應及接收回應認可的必要處理時間。|地理類型，ApiName，身份驗證，檔共用|
|可用性|可用性|百分比|Average|儲存體服務或所指定 API 作業的可用性百分比。 可用性的計算方式是將 TotalBillableRequests 值除以適用要求數目，包括產生意外錯誤的要求。 所有意外錯誤都會導致儲存體或所指定 API 作業的可用性降低。|地理類型，ApiName，身份驗證，檔共用|
...
|Disk Read Bytes|Disk Read Bytes|位元組|總計|監視期間從磁片讀取的位元組|None|
|Disk Write Bytes|Disk Write Bytes|位元組|總計|監視期間寫入磁片的位元組|None|
|Disk Read Operations/Sec|Disk Read Operations/Sec|每秒計數|Average|磁碟讀取 IOPS|None|
|Disk Write Operations/Sec|Disk Write Operations/Sec|每秒計數|Average|磁碟寫入 IOPS|None|
...
|每一磁碟讀取位元組/秒|資料磁片讀取位元組/秒（已棄用）|每秒計數|Average|監視期間從單個磁片讀取位元組/秒|SlotId|
|每一磁碟寫入位元組/秒|資料磁片寫入位元組/秒（已棄用）|每秒計數|Average|在監視期間寫入單個磁片的位元組/秒數|SlotId|
|每一磁碟讀取作業/秒|資料磁片讀取操作/秒（已棄用）|每秒計數|Average|在監視期間從單個磁片讀取 IOPS|SlotId|
|每一磁碟寫入作業/秒|資料磁片寫入操作/秒（已棄用）|每秒計數|Average|在監視期間從單個磁片寫入 IOPS|SlotId|
|每一磁碟 QD|資料磁片 QD（已棄用）|Count|Average|資料磁碟佇列深度 (或佇列長度)|SlotId|
|OS 每一磁碟讀取位元組/秒|OS 磁片讀取位元組/秒（已棄用）|每秒計數|Average|在作業系統磁片的監視期間從單個磁片讀取位元組/秒|None|
|OS 每一磁碟寫入位元組/秒|OS 磁片寫入位元組/秒（已棄用）|每秒計數|Average|在作業系統磁片的監視期間寫入單個磁片的位元組/秒|None|
|OS 每一磁碟讀取作業/秒|OS 磁片讀取操作/秒（已棄用）|每秒計數|Average|在作業系統磁片的監視期間從單個磁片讀取 IOPS|None|
|OS 每一磁碟寫入作業/秒|OS 磁片寫入操作/秒（已棄用）|每秒計數|Average|在作業系統磁片的監視期間從單個磁片寫入 IOPS|None|
|OS 每一磁碟 QD|OS 磁片 QD（已棄用）|Count|Average|OS 磁碟佇列深度 (或佇列長度)|None|
|資料磁片讀取位元組/秒|資料磁碟讀取位元組/秒 (預覽)|每秒計數|Average|監視期間從單個磁片讀取位元組/秒|LUN|
|資料磁片寫入位元組/秒|資料磁碟寫入位元組/秒 (預覽)|每秒計數|Average|在監視期間寫入單個磁片的位元組/秒數|LUN|
|資料磁片讀取操作/秒|資料磁碟讀取作業/秒 (預覽)|每秒計數|Average|在監視期間從單個磁片讀取 IOPS|LUN|
|資料磁片寫入操作/秒|資料磁碟寫入作業/秒 (預覽)|每秒計數|Average|在監視期間從單個磁片寫入 IOPS|LUN|
|資料磁碟佇列深度|資料磁片佇列深度（預覽）|Count|Average|資料磁碟佇列深度 (或佇列長度)|LUN|
|OS 磁片讀取位元組/秒|OS 磁碟讀取位元組/秒 (預覽)|每秒計數|Average|在作業系統磁片的監視期間從單個磁片讀取位元組/秒|None|
|作業系統磁片寫入位元組/秒|OS 磁碟寫入位元組/秒 (預覽)|每秒計數|Average|在作業系統磁片的監視期間寫入單個磁片的位元組/秒|None|
|作業系統磁片讀取操作/秒|OS 磁碟讀取作業/秒 (預覽)|每秒計數|Average|在作業系統磁片的監視期間從單個磁片讀取 IOPS|None|
|作業系統磁片寫入操作/秒|OS 磁碟寫入作業/秒 (預覽)|每秒計數|Average|在作業系統磁片的監視期間從單個磁片寫入 IOPS|None|
|OS 磁碟佇列深度|作業系統磁片佇列深度（預覽）|Count|Average|OS 磁碟佇列深度 (或佇列長度)|None|
|連入流量|連入流量|Count|Average|入站流是入站方向的電流數（進入 VM 的流量）|None|
|出站流|出站流|Count|Average|出站流是出站方向的電流數（從 VM 流出的流量）|None|
|入站流最大創建速率|入站流最大創建速率（預覽）|每秒計數|Average|入站流的最大創建速率（進入 VM 的流量）|None|
|出站流最大創建速率|出站流最大創建速率（預覽）|每秒計數|Average|出站流的最大創建速率（從 VM 流出的流量）|None|
|高級資料磁片緩存讀取命中|高級資料磁片緩存讀取命中（預覽）|百分比|Average|高級資料磁片緩存讀取命中|LUN|
|高級資料磁片緩存讀取誤區|高級資料磁片緩存讀取誤讀（預覽）|百分比|Average|高級資料磁片緩存讀取誤區|LUN|
|高級作業系統磁片緩存讀取命中|高級作業系統磁片緩存讀取命中（預覽）|百分比|Average|高級作業系統磁片緩存讀取命中|None|
|高級 OS 磁片緩存讀取誤區|高級作業系統磁片緩存讀取誤讀（預覽）|百分比|Average|高級 OS 磁片緩存讀取誤區|None|
...
|Disk Read Bytes|Disk Read Bytes|位元組|總計|監視期間從磁片讀取的位元組|VMName|
|Disk Write Bytes|Disk Write Bytes|位元組|總計|監視期間寫入磁片的位元組|VMName|
...
|OS 每一磁碟寫入作業/秒|OS 磁片寫入操作/秒（已棄用）|每秒計數|Average|在作業系統磁片的監視期間從單個磁片寫入 IOPS|None|
|OS 每一磁碟 QD|OS 磁片 QD（已棄用）|Count|Average|OS 磁碟佇列深度 (或佇列長度)|None|
|資料磁片讀取位元組/秒|資料磁碟讀取位元組/秒 (預覽)|每秒計數|Average|監視期間從單個磁片讀取位元組/秒|LUN，VM 名稱|
|資料磁片寫入位元組/秒|資料磁碟寫入位元組/秒 (預覽)|每秒計數|Average|在監視期間寫入單個磁片的位元組/秒數|LUN，VM 名稱|
|資料磁片讀取操作/秒|資料磁碟讀取作業/秒 (預覽)|每秒計數|Average|在監視期間從單個磁片讀取 IOPS|LUN，VM 名稱|
|資料磁片寫入操作/秒|資料磁碟寫入作業/秒 (預覽)|每秒計數|Average|在監視期間從單個磁片寫入 IOPS|LUN，VM 名稱|
|資料磁碟佇列深度|資料磁片佇列深度（預覽）|Count|Average|資料磁碟佇列深度 (或佇列長度)|LUN，VM 名稱|
|OS 磁片讀取位元組/秒|OS 磁碟讀取位元組/秒 (預覽)|每秒計數|Average|在作業系統磁片的監視期間從單個磁片讀取位元組/秒|VMName|
|作業系統磁片寫入位元組/秒|OS 磁碟寫入位元組/秒 (預覽)|每秒計數|Average|在作業系統磁片的監視期間寫入單個磁片的位元組/秒|VMName|
|作業系統磁片讀取操作/秒|OS 磁碟讀取作業/秒 (預覽)|每秒計數|Average|在作業系統磁片的監視期間從單個磁片讀取 IOPS|VMName|
|作業系統磁片寫入操作/秒|OS 磁碟寫入作業/秒 (預覽)|每秒計數|Average|在作業系統磁片的監視期間從單個磁片寫入 IOPS|VMName|
|OS 磁碟佇列深度|作業系統磁片佇列深度（預覽）|Count|Average|OS 磁碟佇列深度 (或佇列長度)|VMName|
|連入流量|連入流量|Count|Average|入站流是入站方向的電流數（進入 VM 的流量）|VMName|
|出站流|出站流|Count|Average|出站流是出站方向的電流數（從 VM 流出的流量）|VMName|
|入站流最大創建速率|入站流最大創建速率（預覽）|每秒計數|Average|入站流的最大創建速率（進入 VM 的流量）|VMName|
|出站流最大創建速率|出站流最大創建速率（預覽）|每秒計數|Average|出站流的最大創建速率（從 VM 流出的流量）|VMName|
|高級資料磁片緩存讀取命中|高級資料磁片緩存讀取命中（預覽）|百分比|Average|高級資料磁片緩存讀取命中|LUN，VM 名稱|
|高級資料磁片緩存讀取誤區|高級資料磁片緩存讀取誤讀（預覽）|百分比|Average|高級資料磁片緩存讀取誤區|LUN，VM 名稱|
|高級作業系統磁片緩存讀取命中|高級作業系統磁片緩存讀取命中（預覽）|百分比|Average|高級作業系統磁片緩存讀取命中|VMName|
|高級 OS 磁片緩存讀取誤區|高級作業系統磁片緩存讀取誤讀（預覽）|百分比|Average|高級 OS 磁片緩存讀取誤區|VMName|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|總拉計數|總拉取計數|Count|Average|圖像拉取總數|None|
|成功拉計數|成功拉取計數|Count|Average|成功圖像提取次數|None|
|總推送計數|總推送計數|Count|Average|圖像推送總數|None|
|成功推送計數|成功推送計數|Count|Average|成功圖像推送數|None|
|運行持續時間|執行持續期間|毫秒|總計|以毫秒為單位運行持續時間|None|
...
## <a name="microsoftcustomprovidersresourceproviders"></a>微軟.自訂供應商/資源供應商

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|成功滿請求|成功的要求|Count|總計|自訂提供程式發出的成功請求|HttpMethod，呼叫路徑，狀態碼|
|FailedRequests|失敗的要求|Count|總計|獲取自訂資來源提供者的可用日誌|HttpMethod，呼叫路徑，狀態碼|
...
|雲上傳輸送量PerShare|雲上傳輸送量（共用）|每秒位元組|Average|報告期間從共用上載到 Azure 的輸送量。|共用|
...
|位元組上傳到雲|上傳雲位元組（設備）|位元組|Average|報告期間從設備上載到 Azure 的位元組總數。|None|
|超V虛擬處理器利用|邊緣計算 - CPU 百分比|百分比|Average|CPU 使用率|InstanceName|
|超V記憶體利用|邊緣計算 - 記憶體使用|百分比|Average|正在使用的 RAM 量|InstanceName|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|FailedRuns|失敗的執行|Count|總計||管道名稱，活動名稱|
|SuccessfulRuns|成功的執行|Count|總計||管道名稱，活動名稱|
...
|管道取消運行|已取消的管道運行指標|Count|總計||失敗類型，名稱|
|ActivityFailedRuns|失敗的活動執行計量|Count|總計||活動類型、管道名稱、失敗類型、名稱|
|ActivitySucceededRuns|成功的活動執行計量|Count|總計||活動類型、管道名稱、失敗類型、名稱|
|活動已取消運行|已取消的活動運行指標|Count|總計||活動類型、管道名稱、失敗類型、名稱|
|TriggerFailedRuns|失敗的觸發程序執行計量|Count|總計||名稱，失敗類型|
|TriggerSucceededRuns|成功的觸發程序執行計量|Count|總計||名稱，失敗類型|
|觸發器取消運行|已取消的觸發器運行指標|Count|總計||名稱，失敗類型|
|IntegrationRuntimeCpuPercentage|整合執行階段 CPU 使用率|百分比|Average||集成運行時名稱，節點名稱|
|IntegrationRuntimeAvailableMemory|整合執行階段可用記憶體|位元組|Average||集成運行時名稱，節點名稱|
|集成運行時平均任務提取延遲|集成運行時佇列持續時間|秒|Average||集成運行時名稱|
|集成運行時排隊長度|集成運行時佇列長度|Count|Average||集成運行時名稱|
|集成運行時可用節點編號|集成運行時可用節點計數|Count|Average||集成運行時名稱|
...
|backup_storage_used|使用的備份存儲|位元組|Average|使用的備份存儲|None|
...
|network_bytes_egress|Network Out|位元組|總計|跨作用中連線的網路輸出|None|
|network_bytes_ingress|Network In|位元組|總計|跨作用中連線的網路輸入|None|
|pg_replica_log_delay_in_seconds|複本延隔時間|秒|最大值|複寫延遲（以秒為單位）|None|
|pg_replica_log_delay_in_bytes|複本之間的最大延隔時間|位元組|最大值|滯後副本的位元組數中延遲|None|
...
|cpu_percent|CPU 百分比|百分比|Average|CPU 百分比|None|
|memory_percent|記憶體百分比|百分比|Average|記憶體百分比|None|
|伊奧普斯|IOPS|Count|Average|每秒 IO 操作|None|
|storage_percent|儲存體百分比|百分比|Average|儲存體百分比|None|
|storage_used|已使用儲存體|位元組|Average|已使用儲存體|None|
|active_connections|作用中的連線|Count|Average|作用中的連線|None|
|network_bytes_egress|Network Out|位元組|總計|跨作用中連線的網路輸出|None|
...
|事件網格交付|事件網格交付（預覽）|Count|總計|發佈到事件網格的 IoT 中心事件數。 對成功和失敗的請求數使用"結果"維度。 事件種類維度顯示事件種類 （。https://aka.ms/ioteventgrid)|資源 Id、結果、事件種類|
|事件網格延遲|事件網格延遲（預覽）|毫秒|Average|從生成 Iot 中心事件到將事件發佈到事件網格的平均延遲（毫秒）。 此數位是所有事件種類之間的平均值。 使用事件種類維度查看特定類型的事件的延遲。|資源 Id，事件種類|
...
|DeviceAssignments|已指派的裝置數|Count|總計|已指派到 IoT 中樞的裝置數|預配服務名稱，IotHub 名稱|
|AttestationAttempts|證明嘗試數|Count|總計|嘗試的裝置證明數|預配服務名稱、狀態、協定|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|添加區域|已添加的區域|Count|Count|已添加的區域|區域|
|AvailableStorage|可用的儲存體|位元組|總計|在 5 分鐘細微性下報告的可用存儲總數|集合名稱、資料庫名稱、區域|
|CassandraConnectionClosures|Cassandra 連線終止|Count|總計|已關閉的 Cassandra 連接數，以 1 分鐘細微性報告|API 類型、區域、封閉原因|
|CassandraRequestCharges|Cassandra 要求費用|Count|總計|為卡珊多拉請求使用的 R|API 類型、資料庫名稱、集合名稱、區域、操作類型、資源類型|
|CassandraRequests|Cassandra 要求|Count|Count|提出的卡珊多拉請求數量|API 類型、資料庫名稱、集合名稱、區域、操作類型、資源類型、錯誤代碼|
|創建帳戶|已創建帳戶|Count|Count|已創建帳戶|None|
|DataUsage|資料使用量|位元組|總計|在 5 分鐘細微性下報告的總數據使用量|集合名稱、資料庫名稱、區域|
|刪除帳戶|帳戶已刪除|Count|Count|帳戶已刪除|None|
|DocumentCount|文件計數|Count|總計|以 5 分鐘細微性報告的文檔計數總數|集合名稱、資料庫名稱、區域|
|DocumentQuota|文件配額|位元組|總計|在 5 分鐘細微性下報告的總存儲配額|集合名稱、資料庫名稱、區域|
|IndexUsage|索引使用量|位元組|總計|在 5 分鐘細微性下報告的總索引使用方式|集合名稱、資料庫名稱、區域|
|MetadataRequests|中繼資料要求|Count|Count|中繼資料要求計數。 Cosmos DB 會維護每個帳戶的系統中繼資料集合，可讓您免費列舉集合、資料庫等及其設定。|資料庫名稱、集合名稱、區域、狀態碼、角色|
|MongoRequestCharge|Mongo 要求收費|Count|總計|已使用的 Mongo 要求單位|資料庫名稱、集合名稱、區域、命令名稱、錯誤代碼|
|MongoRequests|Mongo 要求|Count|Count|已提出的 Mongo 要求數目|資料庫名稱、集合名稱、區域、命令名稱、錯誤代碼|
|蒙戈請求計數|蒙戈請求率|每秒計數|Average|蒙戈請求每秒計數|資料庫名稱、集合名稱、區域、命令名稱、錯誤代碼|
|蒙戈請求刪除|蒙戈刪除請求率|每秒計數|Average|每秒刪除 Mongo 請求|資料庫名稱、集合名稱、區域、命令名稱、錯誤代碼|
|蒙戈請求插入|蒙戈插入請求率|每秒計數|Average|蒙戈每秒插入計數|資料庫名稱、集合名稱、區域、命令名稱、錯誤代碼|
|蒙戈請求查詢|蒙戈查詢請求率|每秒計數|Average|每秒蒙戈查詢請求|資料庫名稱、集合名稱、區域、命令名稱、錯誤代碼|
|蒙戈請求更新|蒙戈更新要求率|每秒計數|Average|每秒蒙戈更新要求|資料庫名稱、集合名稱、區域、命令名稱、錯誤代碼|
|ProvisionedThroughput|佈建的輸送量|Count|最大值|佈建的輸送量|資料庫名稱，集合名稱|
|區域容錯移轉|區域故障結束|Count|Count|區域故障結束|None|
|刪除區域|已刪除區域|Count|Count|已刪除區域|區域|
|ReplicationLatency|P99 複寫延遲|毫秒|Average|異地複寫啟用的帳戶其跨來源和目標區域的 P99 複寫延遲|源區域，目的地區域|
|ServiceAvailability|服務可用性|百分比|Average|帳戶請求可用性，時間為一小時、一天或每月細微性|None|
|TotalRequestUnits|要求單位總計|Count|總計|已使用的要求單位|資料庫名稱、集合名稱、區域、狀態碼、操作類型|
|TotalRequests|要求總數|Count|Count|進行的要求數目|資料庫名稱、集合名稱、區域、狀態碼、操作類型|
|更新帳戶金鑰|帳戶金鑰已更新|Count|Count|帳戶金鑰已更新|KeyType|
|更新帳戶網路設置|帳戶網路設置已更新|Count|Count|帳戶網路設置已更新|None|
|更新帳戶複製設置|帳戶複製設置已更新|Count|Count|帳戶複製設置已更新|None|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|事務計數|事務計數|Count|Count|總事務計數|事務計數|
|SuccessCount|成功計數|Count|Count|成功事務計數|SuccessCount|
|故障計數|失敗計數|Count|Count|失敗的交易計數|故障計數|
...
|PublishFailCount|發佈失敗事件|Count|總計|發佈至本主題失敗的事件總數|主題， 錯誤類型， 錯誤|
...
|PublishFailCount|發佈失敗事件|Count|總計|發佈至本主題失敗的事件總數|錯誤類型，錯誤|
...
|DroppedEventCount|卸除的事件|Count|總計|與此事件訂閱相符的已卸除事件總數|刪除原因，事件訂閱名稱|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|SuccessfulRequests|成功的要求|Count|總計|Microsoft.EventHub 的成功要求。|機構名稱，操作結果|
|ServerErrors|伺服器錯誤。|Count|總計|Microsoft.EventHub 的伺服器錯誤。|機構名稱，操作結果|
|UserErrors|使用者錯誤。|Count|總計|Microsoft.EventHub 的使用者錯誤。|機構名稱，操作結果|
|QuotaExceededErrors|超出配額的錯誤。|Count|總計|Microsoft.EventHub 超出配額的錯誤。|機構名稱，操作結果|
|ThrottledRequests|節流的要求。|Count|總計|Microsoft.EventHub 的節流要求。|機構名稱，操作結果|
...
|INREQS|傳入請求（已棄用）|Count|總計|命名空間的傳入發送請求總數（已棄用）|None|
|SUCCREQ|成功請求（已棄用）|Count|總計|命名空間成功請求總數（已棄用）|None|
|FAILREQ|失敗的請求（已棄用）|Count|總計|命名空間的失敗請求總數（已棄用）|None|
|SVRBSY|伺服器忙錯誤（已棄用）|Count|總計|命名空間的伺服器忙錯誤總數（已棄用）|None|
|INTERR|內部伺服器錯誤（已棄用）|Count|總計|命名空間的內部伺服器錯誤總數（已棄用）|None|
|MISCERR|其他錯誤（已棄用）|Count|總計|命名空間的失敗請求總數（已棄用）|None|
|INMSGS|傳入消息（已過時）（已棄用）|Count|總計|命名空間的傳入訊息總數。 此計量已過時。 請改為使用傳入消息指標（已棄用）|None|
|EHINMSGS|連入訊息數 (已過時)|Count|總計|命名空間的傳入消息總數（已棄用）|None|
|OUTMSGS|傳出消息（已過時）（已棄用）|Count|總計|命名空間的傳出訊息總數。 此計量已過時。 請改為使用傳出消息指標（已棄用）|None|
|EHOUTMSGS|傳出訊息數 (已過時)|Count|總計|命名空間的總傳出消息（已棄用）|None|
|EHINMBS|傳入位元組（已過時）（已棄用）|位元組|總計|命名空間的事件中樞傳入訊息輸送量。 此計量已過時。 請改用傳入位元組指標（已棄用）|None|
|EHINBYTES|傳入位元組數 (已過時)|位元組|總計|命名空間的事件中心傳入消息輸送量（已棄用）|None|
|EHOUTMBS|傳出位元組（過時）（已棄用）|位元組|總計|命名空間的事件中樞傳出訊息輸送量。 此計量已過時。 請改用傳出位元組指標（已棄用）|None|
|EHOUTBYTES|傳出位元組數 (已過時)|位元組|總計|命名空間的事件中心傳出消息輸送量（已棄用）|None|
|EHABL|存檔積壓工作郵件（已棄用）|Count|總計|事件中心存檔名稱空間積壓工作中的消息（已棄用）|None|
|EHAMSGS|存檔消息（已棄用）|Count|總計|事件中心在命名空間中存檔的消息（已棄用）|None|
|EHAMBS|存檔消息輸送量（已棄用）|位元組|總計|事件中心存檔的消息輸送量在命名空間（已棄用）|None|
...
|AvailableMemory|可用的記憶體|百分比|最大值|事件中心群集的可用記憶體占總記憶體的百分比。|角色|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|可用性 結果/可用性百分比|可用性|百分比|Average|成功完成的可用性測試的百分比|可用性結果/名稱、可用性結果/位置|
...
|dependencies/count|相依性呼叫|Count|Count|應用程式對外部資源的呼叫計數。|依賴項/類型、依賴項/性能桶、依賴項/成功、依賴項/目標、操作/綜合、雲/角色實例、雲/角色名稱|
|dependencies/duration|相依性持續時間|毫秒|Average|應用程式對外部資源的呼叫持續時間。|依賴項/類型、依賴項/性能桶、依賴項/成功、依賴項/目標、操作/綜合、雲/角色實例、雲/角色名稱|
|dependencies/failed|依賴項調用失敗|Count|Count|應用程式對外部資源進行相依性呼叫的失敗計數。|依賴項/類型、依賴項/性能桶、依賴項/成功、依賴項/目標、操作/綜合、雲/角色實例、雲/角色名稱|
...
|requests/duration|伺服器回應時間|毫秒|Average|從接收 HTTP 要求直至完成傳送回應的經過時間。|請求/性能存儲桶、請求/結果代碼、操作/綜合、雲/角色實例、請求/成功、雲/角色名稱|
|requests/count|伺服器要求|Count|Count|已完成 HTTP 要求的計數。|請求/性能存儲桶、請求/結果代碼、操作/綜合、雲/角色實例、請求/成功、雲/角色名稱|
|requests/failed|失敗的要求|Count|Count|標示為失敗的 HTTP 要求計數。 在大多數情況下，這些要求的回應碼會 >= 400，且不等於 401。|請求/性能存儲桶、請求/結果代碼、請求/成功、操作/綜合、雲/角色實例、雲/角色名稱|
|請求/速率|伺服器請求率|每秒計數|Average|每秒伺服器請求的速率|請求/性能存儲桶、請求/結果代碼、操作/綜合、雲/角色實例、請求/成功、雲/角色名稱|
...
|traces/count|追蹤|Count|Count|追蹤文件計數|跟蹤/嚴重性級別、操作/合成、雲/角色名稱、雲/角色實例|
...
|ServiceApiLatency|整體服務 API 延遲|毫秒|Average|服務 API 要求的整體延遲|活動類型、活動名稱、狀態碼、狀態碼類|
|ServiceApiResult|服務 API 結果總計|Count|Count|服務 API 結果總數|活動類型、活動名稱、狀態碼、狀態碼類|
|飽和鞋盒|整體保存庫飽和度|百分比|Average|使用的保險庫容量|活動類型、活動名稱、交易記錄類型|
|可用性|總體保險庫可用性|百分比|Average|保存庫請求可用性|活動類型、活動名稱、狀態碼、狀態碼類|

## <a name="microsoftkustoclusters"></a>Microsoft.Kusto/Clusters

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|緩存利用率|緩存利用率|百分比|Average|在叢集範圍的使用率等級|None|
...
|連續匯出記錄|連續匯出 = 匯出記錄數|Count|總計|匯出的記錄數，對於匯出操作期間寫入的每個存儲工件已觸發的記錄數|None|
|匯出利用率|出口利用率|百分比|最大值|出口利用率|None|
|連續匯出掛起計數|連續匯出掛起計數|Count|最大值|待執行的掛起連續匯出作業數|None|
|連續出口最大完成分鐘數|連續匯出最大延遲分鐘數|Count|最大值|所有連續匯出的最長延遲（分鐘數）待定並可供執行|None|
|連續匯出結果|連續匯出結果|Count|Count|指示連續匯出是成功還是失敗|連續匯出名稱、結果、資料庫|
|流式處理持續時間|資料流引入持續時間|毫秒|Average|以毫秒為單位資料流引入持續時間|None|
|資料流資料速率|資料流引入資料速率|Count|Average|資料流引入資料速率（MB/秒）|None|
|蒸化請求率|資料流引入請求速率|Count|速率請求每秒|資料流引入請求速率（每秒請求數）|None|
|流式處理結果|資料流引入結果|Count|Average|資料流攝錄結果|結果|
|併發查詢總數|併發查詢總數|Count|總計|併發查詢總數|None|
|查詢總數|限制查詢的總數|Count|總計|限制查詢的總數|None|
|已限制的指令總數|被限制命令的總數|Count|總計|被限制命令的總數|CommandType|
|範圍總數|擴展盤區總數|Count|總計|資料範圍總數|None|
...
|運行啟動限制事件|運行啟動限制事件|Count|總計|工作流運行數啟動限制事件。|None|
...
|TriggerSuccessLatency|觸發程序成功延遲 |秒|Average|已成功的工作流程觸發程序延遲。|None|
|TriggerThrottledEvents|觸發程序節流的事件|Count|總計|工作流程觸發程序節流的事件數目。|None|
|整合服務環境工作流處理器使用|整合服務環境的工作流處理器使用方式|百分比|Average|整合服務環境的工作流處理器使用方式。|None|
|整合服務環境工作流記憶體使用|整合服務環境的工作流記憶體使用方式|百分比|Average|整合服務環境的工作流記憶體使用方式。|None|
|整合服務環境連接器處理器使用|整合服務環境的連接器處理器使用方式|百分比|Average|整合服務環境的連接器處理器使用方式。|None|
|整合服務環境連接器記憶體使用|整合服務環境的連接器記憶體使用方式|百分比|Average|整合服務環境的連接器記憶體使用方式。|None|

## <a name="microsoftmachinelearningservicesworkspaces"></a>Microsoft.MachineLearningServices/workspaces

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|已完成運行|已完成運行|Count|總計|為此工作區成功完成的運行數|方案、運行類型、已發佈的管道 Id、計算類型、管道步進類型|
|已啟動運行|已啟動運行|Count|總計|為此工作區啟動的運行次數|方案、運行類型、已發佈的管道 Id、計算類型、管道步進類型|
|失敗的執行|失敗的執行|Count|總計|此工作區失敗的運行次數|方案、運行類型、已發佈的管道 Id、計算類型、管道步進類型|
|模型註冊成功|模型註冊成功|Count|總計|在此工作區中成功的模型註冊數|狀況|
|模型註冊失敗|模型註冊失敗|Count|總計|在此工作區中失敗的模型註冊數|方案，狀態碼|
|模型部署已啟動|模型部署已啟動|Count|總計|在此工作區中啟動的模型部署數|狀況|
|模型部署成功|模型部署成功|Count|總計|在此工作區中成功的模型部署數|狀況|
|模型部署失敗|模型部署失敗|Count|總計|在此工作區中失敗的模型部署數|方案，狀態碼|
|節點總數|節點總數|Count|Average|總節點數。 此總計包括一些活動節點、空閒節點、不可用節點、搶佔節點、離開節點|方案，群集名稱|
|活動節點|活動節點|Count|Average|Acitve 節點數。 這些是主動運行作業的節點。|方案，群集名稱|
|空閒節點|空閒節點|Count|Average|空閒節點數。 空閒節點是未運行任何作業但可以接受新作業（如果可用）的節點。|方案，群集名稱|
|無法使用的節點|無法使用的節點|Count|Average|不可用節點數。 由於某些無法解決的問題，無法使用的節點無法正常工作。 Azure 將回收這些節點。|方案，群集名稱|
|搶佔節點|搶佔節點|Count|Average|搶佔節點數。 這些節點是從可用節點池中獲取的低優先順序節點。|方案，群集名稱|
|離開節點|離開節點|Count|Average|離開節點的數量。 離開節點是剛剛完成工作處理並將轉到空閒狀態的節點。|方案，群集名稱|
|核心總數|核心總數|Count|Average|總內核數|方案，群集名稱|
|活動核心|活動核心|Count|Average|活動內核數|方案，群集名稱|
|空閒核心|空閒核心|Count|Average|空閒內核數|方案，群集名稱|
|不可用的內核|不可用的內核|Count|Average|不可用內核數|方案，群集名稱|
...
|離開核心|離開核心|Count|Average|離開內核的數量|方案，群集名稱|
|配額利用率百分比|配額利用率百分比|Count|Average|使用的配額百分比|方案，群集名稱，Vm 家族名稱，Vm 優先|
|Cpu 利用|Cpu 利用|Count|Average|CPU (預覽)|方案，運行 Id，節點 Id，創建時間|
|Gpu 利用|Gpu 利用|Count|Average|GPU（預覽版）|方案，運行 Id，節點 Id，創建時間，設備 Id|
...
|可用性|可用性|百分比|Average|API 的可用性|Api 分類，ApiName|

## <a name="microsoftmediamediaservicesstreamingendpoints"></a>微軟.媒體/媒體服務/流媒體終端

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|輸出|輸出|位元組|總計|出入口資料量（以位元組為單位）。|OutputFormat|
...
|內容關鍵策略配額|內容關鍵策略配額|Count|Average|當前媒體服務帳戶允許多少內容金鑰監管|None|
|內容關鍵策略計數|內容金鑰策略計數|Count|Average|當前媒體服務帳戶中已創建多少個內容金鑰策略|None|
|內容關鍵策略配額使用百分比|內容 關鍵策略配額使用百分比|百分比|Average|當前媒體服務帳戶中的內容金鑰策略使用百分比|None|
|流式處理策略配額|流式處理策略配額|Count|Average|當前媒體服務帳戶允許多少流策略|None|
|流式處理策略計數|流策略計數|Count|Average|當前媒體服務帳戶中已創建多少流策略|None|
|流式處理策略配額使用百分比|流策略配額使用百分比|百分比|Average|當前媒體服務帳戶中的流策略使用百分比|None|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|PingMesh平均往返|Ping 到 VM 的往返時間|毫秒|Average|發送到目標 VM 的 Ping 的往返時間|源客戶位址，目標客戶位址|
|PingMeshProbe 失敗百分比|對 VM 的故障 Ping|百分比|Average|故障 Ping 到目標 VM 的總發送 Ping 的百分比|源客戶位址，目標客戶位址|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|應用程式規則Hit|應用程式規則命中計數|Count|總計|命中應用程式規則的次數|狀態、原因、協定|
|網路規則Hit|網路規則命中計數|Count|總計|命中網路規則的次數|狀態、原因、協定|
|防火牆運行狀況|防火牆運行狀況狀態|百分比|Average|防火牆運行狀況狀態|狀態，原因|
|資料處理|已處理的資料量|位元組|總計|防火牆處理的資料總量|None|
|SNATPort 利用|SNAT 埠利用率|百分比|Average|SNAT 埠利用率|None|
...
|Cpu 利用|CPU 使用率|百分比|Average|應用程式閘道的當前 CPU 利用率|None|
|容量單位|當前容量單位|Count|Average|消耗的容量單位|None|
|計算單位|當前計算單位|Count|Average|消耗的計算單位|None|
|後端回應狀態|後端回應狀態|Count|總計|後端成員生成的 HTTP 回應代碼數。 這不包括應用程式閘道生成的任何回應代碼。|後端伺服器、後端池、後端Http設置、HTTPStatusGroup|
|特爾斯協定|用戶端 TLS 協定|Count|總計|與應用程式閘道建立連接的用戶端啟動的 TLS 和非 TLS 請求數。 要查看 TLS 協定分佈，按維度 TLS 協定進行篩選。|監聽器，TlsProtocol|
|BytesSent|傳送的位元組數|位元組|總計|應用程式閘道發送到用戶端的位元組總數|接聽程式|
|BytesReceived|接收的位元組數|位元組|總計|應用程式閘道從用戶端接收的位元組總數|接聽程式|
|用戶端Rtt|用戶端 RTT|毫秒|Average|用戶端和應用程式閘道之間的平均往返時間。 此指標指示建立連接和返回確認所需的時間|接聽程式|
|應用程式閘道總時間|應用程式閘道總時間|毫秒|Average|處理請求及其回應的平均時間。 計算此值的時間從應用程式閘道接收 HTTP 要求的第一個位元組到回應發送操作完成的時間的間隔平均值計算。 請務必注意，這通常包括應用程式閘道處理時間、請求和回應資料包通過網路傳輸的時間以及後端伺服器進行回應的時間。|接聽程式|
|後端連線時間|後端連線時間|毫秒|Average|與後端伺服器建立連接所花費的時間|攔截器、後端伺服器、後端池、後端Http設置|
|後端第一位元組回應時間|後端第一位元組回應時間|毫秒|Average|建立與後端伺服器的連接開始到接收回應標頭的第一個位元組之間的時間間隔，近似後端伺服器的處理時間|攔截器、後端伺服器、後端池、後端Http設置|
|後端上位元組回應時間|後端上一個位元組回應時間|毫秒|Average|建立與後端伺服器的連接開始到接收回應正文的最後一個位元組之間的時間間隔|攔截器、後端伺服器、後端池、後端Http設置|
|匹配計數|Web 應用程式防火牆 v1 總規則分發|Count|總計|Web 應用程式防火牆 v1 傳入流量的總規則分發|規則組，規則 Id|
|阻止計數|Web 應用程式防火牆 v1 阻止請求規則分發|Count|總計|Web 應用程式防火牆 v1 阻止的請求規則分發|規則組，規則 Id|
|已阻止的 ReqCount|Web 應用程式防火牆 v1 阻止的請求計數|Count|總計|Web 應用程式防火牆 v1 阻止的請求計數|None|
...
|TunnelAverageBandwidth|通道頻寬|每秒位元組|Average|通道的平均頻寬 (位元組/秒)|連接名稱，遠端 IP|
|TunnelEgressBytes|通道輸出位元組數|位元組|總計|通道的傳出位元組數|連接名稱，遠端 IP|
|TunnelIngressBytes|通道輸入位元組數|位元組|總計|通道的傳入位元組數|連接名稱，遠端 IP|
|TunnelEgressPackets|通道輸出封包數|Count|總計|通道的傳出封包計數|連接名稱，遠端 IP|
|TunnelIngressPackets|通道輸入封包數|Count|總計|通道的傳入封包計數|連接名稱，遠端 IP|
|TunnelEgressPacketDropTSMismatch|通道輸出 TS 不相符封包捨棄|Count|總計|因與通道不符而從流量選取器捨棄的傳出封包計數|連接名稱，遠端 IP|
|TunnelIngressPacketDropTSMismatch|通道輸入 TS 不符合封包捨棄|Count|總計|因與通道不符而從流量選取器捨棄的傳入封包計數|連接名稱，遠端 IP|


## <a name="microsoftnetworkexpressrouteports"></a>微軟.網路/快速路由埠

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|RxLight 級別|RxLight 級別|Count|Average|rx 光電平（dBm）|連結， 萊恩|
|TxLight 級別|TxLight 級別|Count|Average|tx 光電平（dBm）|連結， 萊恩|
|管理狀態|管理狀態|Count|Average|埠的管理狀態|連結|
|線協定|線協定|Count|Average|埠的線路協定狀態|連結|
|波特比辛第二|BitsInPerSecond|每秒計數|Average|每秒輸入到 Azure 的位元數|連結|
|埠出點秒|BitsOutPerSecond|每秒計數|Average|每秒從 Azure 輸出的位元數|連結|
...
|全域覆蓋位秒|全域覆蓋位秒|每秒計數|Average|每秒輸入到 Azure 的位元數|對等電路金鑰|
|全域覆蓋位出每秒|全域覆蓋位出每秒|每秒計數|Average|每秒從 Azure 輸出的位元數|對等電路金鑰|
|Bgp可用性|Bgp 可用性|百分比|Average|從 MSEE 到所有對等體的 BGP 可用性。|對等類型，對等|
|arp可用性|Arp 可用性|百分比|Average|從 MSEE 到所有對等體的 ARP 可用性。|對等類型，對等|
|QosDropBitper秒|落點每秒|每秒計數|Average|每秒丟棄的資料入口位|None|
|QosDropBitOutPer秒|已退出的位數每秒|每秒計數|Average|每秒丟棄的資料出口位|None|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|ErGateway 連接位位秒|BitsInPerSecond|每秒計數|Average|每秒輸入到 Azure 的位元數|連接名稱|
|ErGateway連接位出每秒|BitsOutPerSecond|每秒計數|Average|每秒從 Azure 輸出的位元數|連接名稱|
...
|檢查失敗百分比|檢查失敗百分比（預覽）|百分比|Average|連接監控檢查失敗的百分比|源位址、源名、源資源 Id、源類型、協定、目標位址、目標名稱、目標資源 Id、目標型別、目標埠、測試組名稱、測試組態名稱|
|往返時間|往返時間 （ms） （預覽）|毫秒|Average|連接監控檢查的往返時間（以毫秒為單位）|源位址、源名、源資源 Id、源類型、協定、目標位址、目標名稱、目標資源 Id、目標型別、目標埠、測試組名稱、測試組態名稱|
...
|計費回應大小|計費回應大小|位元組|總計|從 HTTP/S 代理髮送到用戶端的回應時發送的計費位元組數（每個請求最少 2KB）。|HttpStatus，HTTPStatusGroup，用戶端區域，用戶端國家|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|Average_% Free Inodes|% Free Inodes|Count|Average|Average_% Free Inodes|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_% Free Space|% Free Space|Count|Average|Average_% Free Space|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_% Used Inodes|% Used Inodes|Count|Average|Average_% Used Inodes|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_% Used Space|% Used Space|Count|Average|Average_% Used Space|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Disk Read Bytes/sec|Disk Read Bytes/sec |Count|Average|Average_Disk Read Bytes/sec|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Disk Reads/sec|Disk Reads/sec |Count|Average|Average_Disk Reads/sec|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Disk Transfers/sec|Disk Transfers/sec|Count|Average|Average_Disk Transfers/sec|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Disk Write Bytes/sec|Disk Write Bytes/sec|Count|Average|Average_Disk Write Bytes/sec|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Disk Writes/sec|Disk Writes/sec|Count|Average|Average_Disk Writes/sec|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Free Megabytes|Free Megabytes|Count|Average|Average_Free Megabytes|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Logical Disk Bytes/sec|Logical Disk Bytes/sec|Count|Average|Average_Logical Disk Bytes/sec|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_% Available Memory|% Available Memory|Count|Average|Average_% Available Memory|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_% Available Swap Space|% Available Swap Space|Count|Average|Average_% Available Swap Space|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_% Used Memory|% Used Memory|Count|Average|Average_% Used Memory|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_% Used Swap Space|% Used Swap Space|Count|Average|Average_% Used Swap Space|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Available MBytes Memory|Available MBytes Memory|Count|Average|Average_Available MBytes Memory|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Available MBytes Swap|Available MBytes Swap|Count|Average|Average_Available MBytes Swap|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Page Reads/sec|Page Reads/sec|Count|Average|Average_Page Reads/sec|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Page Writes/sec|Page Writes/sec|Count|Average|Average_Page Writes/sec|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Pages/sec|Pages/sec|Count|Average|Average_Pages/sec|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Used MBytes Swap Space|Used MBytes Swap Space|Count|Average|Average_Used MBytes Swap Space|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Used Memory MBytes|Used Memory MBytes|Count|Average|Average_Used Memory MBytes|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Total Bytes Transmitted|Total Bytes Transmitted|Count|Average|Average_Total Bytes Transmitted|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Total Bytes Received|Total Bytes Received|Count|Average|Average_Total Bytes Received|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Total Bytes|Total Bytes|Count|Average|Average_Total Bytes|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Total Packets Transmitted|Total Packets Transmitted|Count|Average|Average_Total Packets Transmitted|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Total Packets Received|Total Packets Received|Count|Average|Average_Total Packets Received|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Total Rx Errors|Total Rx Errors|Count|Average|Average_Total Rx Errors|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Total Tx Errors|Total Tx Errors|Count|Average|Average_Total Tx Errors|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Total Collisions|Total Collisions|Count|Average|Average_Total Collisions|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Avg. Disk sec/Read|平均磁片秒/讀取|Count|Average|Average_Avg. Disk sec/Read|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Avg. Disk sec/Transfer|平均磁片秒/傳輸|Count|Average|Average_Avg. Disk sec/Transfer|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Avg. Disk sec/Write|平均磁片秒/寫入|Count|Average|Average_Avg. Disk sec/Write|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Physical Disk Bytes/sec|Physical Disk Bytes/sec|Count|Average|Average_Physical Disk Bytes/sec|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Pct Privileged Time|Pct Privileged Time|Count|Average|Average_Pct Privileged Time|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Pct User Time|Pct User Time|Count|Average|Average_Pct User Time|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Used Memory kBytes|Used Memory kBytes|Count|Average|Average_Used Memory kBytes|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Virtual Shared Memory|Virtual Shared Memory|Count|Average|Average_Virtual Shared Memory|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_% DPC Time|% DPC Time|Count|Average|Average_% DPC Time|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_% Idle Time|% Idle Time|Count|Average|Average_% Idle Time|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_% Interrupt Time|% Interrupt Time|Count|Average|Average_% Interrupt Time|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_% IO Wait Time|% IO Wait Time|Count|Average|Average_% IO Wait Time|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_% Nice Time|% Nice Time|Count|Average|Average_% Nice Time|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_% Privileged Time|% Privileged Time|Count|Average|Average_% Privileged Time|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_% Processor Time|% Processor Time|Count|Average|Average_% Processor Time|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_% User Time|% User Time|Count|Average|Average_% User Time|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Free Physical Memory|Free Physical Memory|Count|Average|Average_Free Physical Memory|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Free Space in Paging Files|Free Space in Paging Files|Count|Average|Average_Free Space in Paging Files|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Free Virtual Memory|Free Virtual Memory|Count|Average|Average_Free Virtual Memory|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Processes|處理序|Count|Average|Average_Processes|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Size Stored In Paging Files|Size Stored In Paging Files|Count|Average|Average_Size Stored In Paging Files|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Uptime|Uptime|Count|Average|Average_Uptime|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Users|使用者|Count|Average|Average_Users|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Avg. Disk sec/Read|平均磁片秒/讀取|Count|Average|Average_Avg. Disk sec/Read|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Avg. Disk sec/Write|平均磁片秒/寫入|Count|Average|Average_Avg. Disk sec/Write|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Current Disk Queue Length|Current Disk Queue Length|Count|Average|Average_Current Disk Queue Length|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Disk Reads/sec|Disk Reads/sec |Count|Average|Average_Disk Reads/sec|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Disk Transfers/sec|Disk Transfers/sec|Count|Average|Average_Disk Transfers/sec|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Disk Writes/sec|Disk Writes/sec|Count|Average|Average_Disk Writes/sec|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Free Megabytes|Free Megabytes|Count|Average|Average_Free Megabytes|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_% Free Space|% Free Space|Count|Average|Average_% Free Space|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Available MBytes|可用的 MB|Count|Average|Average_Available MBytes|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_% Committed Bytes In Use|% Committed Bytes In Use|Count|Average|Average_% Committed Bytes In Use|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Bytes Received/sec|Bytes Received/sec|Count|Average|Average_Bytes Received/sec|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Bytes Sent/sec|Bytes Sent/sec|Count|Average|Average_Bytes Sent/sec|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Bytes Total/sec|Bytes Total/sec|Count|Average|Average_Bytes Total/sec|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_% Processor Time|% Processor Time|Count|Average|Average_% Processor Time|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Average_Processor Queue Length|處理器佇列長度|Count|Average|Average_Processor Queue Length|電腦， 物件名稱， 實例名稱， 反路徑， 源系統|
|Heartbeat|Heartbeat|Count|總計|Heartbeat|電腦， OSType， 版本， 源電腦 Id|
|更新|更新|Count|Average|更新|電腦， 產品， 分類， 更新狀態， 可選， 已批准|
|事件|事件|Count|Average|事件|源、事件日誌、電腦、事件類別、事件級別、事件級別名稱、事件ID|

## <a name="microsoftpeeringpeeringservices"></a>微軟.對等/對等服務

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|首碼延遲|首碼延遲|毫秒|Average|中位首碼延遲|首碼名稱|

## <a name="microsoftpeeringpeerings"></a>微軟.對等互連/對等

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|會話可用性V4|會話可用性 V4|百分比|Average|V4 會話的可用性|ConnectionId|
|會話可用性V6|會話可用性 V6|百分比|Average|V6 會話的可用性|ConnectionId|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|ListenerConnections-Success|ListenerConnections-Success|Count|總計|Microsoft.Relay 的成功 ListenerConnections。|機構名稱，操作結果|
|ListenerConnections-ClientError|ListenerConnections-ClientError|Count|總計|Microsoft.Relay 的 ListenerConnections 上發生的 ClientError。|機構名稱，操作結果|
|ListenerConnections-ServerError|ListenerConnections-ServerError|Count|總計|Microsoft.Relay 的 ListenerConnections 上發生的 ServerError。|機構名稱，操作結果|
|SenderConnections-Success|SenderConnections-Success|Count|總計|Microsoft.Relay 的成功 SenderConnections。|機構名稱，操作結果|
|SenderConnections-ClientError|SenderConnections-ClientError|Count|總計|Microsoft.Relay 的 SenderConnections 上發生的 ClientError。|機構名稱，操作結果|
|SenderConnections-ServerError|SenderConnections-ServerError|Count|總計|Microsoft.Relay 的 SenderConnections 上發生的 ServerError。|機構名稱，操作結果|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|SuccessfulRequests|成功的要求|Count|總計|命名空間的成功要求總數|機構名稱，操作結果|
|ServerErrors|伺服器錯誤。|Count|總計|Microsoft.ServiceBus 的伺服器錯誤。|機構名稱，操作結果|
|UserErrors|使用者錯誤。|Count|總計|Microsoft.ServiceBus 的使用者錯誤。|機構名稱，操作結果|
|ThrottledRequests|節流的要求。|Count|總計|Microsoft.ServiceBus 的節流要求。|機構名稱，操作結果|
...
|計畫消息|佇列/主題中計畫的消息計數。|Count|Average|佇列/主題中計畫的消息計數。|EntityName|
|命名空間Cpu使用|CPU|百分比|最大值|服務匯流排高級命名空間 CPU 使用率指標。|複本|
|命名空間記憶體使用|記憶體使用量|百分比|最大值|服務匯流排高級命名空間記憶體使用方式指標。|複本|
|CPUXNS|CPU（已棄用）|百分比|最大值|服務匯流排高級命名空間 CPU 使用率指標。 此指標已取消。 請改用 CPU 指標（命名空間CpuUse）。|複本|
|WSXNS|記憶體使用（已棄用）|百分比|最大值|服務匯流排高級命名空間記憶體使用方式指標。 此計量已過時。 請使用記憶體使用方式（命名空間記憶體使用）指標。|複本|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|已分配 Cpu|已分配 Cpu|Count|Average|以毫核方式分配給此容器的 Cpu|應用程式名稱、服務名稱、代碼包名稱、服務副本名稱|
|已分配記憶體|已分配記憶體|位元組|Average|以 MB 表示分配給此容器的記憶體|應用程式名稱、服務名稱、代碼包名稱、服務副本名稱|
|實際Cpu|實際Cpu|Count|Average|以毫芯為單位的實際 CPU 使用率|應用程式名稱、服務名稱、代碼包名稱、服務副本名稱|
|實際記憶體|實際記憶體|位元組|Average|以 MB 表示的實際記憶體使用量|應用程式名稱、服務名稱、代碼包名稱、服務副本名稱|
|Cpu 利用|Cpu 利用|百分比|Average|此容器 CPU 的利用率（占已分配 Cpu 的百分比）|應用程式名稱、服務名稱、代碼包名稱、服務副本名稱|
|記憶體利用|記憶體利用|百分比|Average|此容器 CPU 的利用率（占已分配 Cpu 的百分比）|應用程式名稱、服務名稱、代碼包名稱、服務副本名稱|
|應用程式狀態|應用程式狀態|Count|Average|服務結構網格應用的狀態|應用程式名稱，狀態|
|ServiceStatus|ServiceStatus|Count|Average|服務結構網格應用程式中服務的運行狀況|應用程式名稱、狀態、服務名稱|
|服務複製狀態|服務複製狀態|Count|Average|服務結構網格應用程式中服務副本的運行狀況|應用程式名稱、狀態、服務名稱、服務副本名稱|
|容器狀態|容器狀態|Count|Average|服務結構網格應用程式中容器的狀態|應用程式名稱、服務名稱、代碼包名稱、服務副本名稱、狀態|
|重新開機計數|重新開機計數|Count|Average|在服務結構網格應用程式中重新開機容器計數|應用程式名稱、狀態、服務名稱、服務副本名稱、代碼包名稱|
...
|deadlock|死結|Count|總計|僵局。 不適用於資料倉儲。|None|
|storage_percent|使用的資料空間百分比|百分比|最大值|資料空間使用百分比。 不適用於資料倉儲或超大規模資料庫。|None|
|xtp_storage_percent|記憶體內部 OLTP 儲存體百分比|百分比|Average|記憶體中 OLTP 存儲百分比。 不適用於資料倉儲。|None|
|workers_percent|背景工作角色百分比|百分比|Average|工人百分比。 不適用於資料倉儲。|None|
...
|cpu_limit|CPU 限制|Count|Average|CPU 限制。 適用于基於 vCore 的資料庫。|None|
|cpu_used|使用的 CPU|Count|Average|使用的 CPU。 適用于基於 vCore 的資料庫。|None|
|dwu_limit|DWU 限制|Count|最大值|DWU 限制。 僅適用于資料倉儲。|None|
|dwu_consumption_percent|DWU 百分比|百分比|最大值|DWU 百分比。 僅適用于資料倉儲。|None|
|dwu_used|已使用 DWU|Count|最大值|使用了 DWU。 僅適用于資料倉儲。|None|
|cache_hit_percent|快取命中的百分比|百分比|最大值|緩存命中百分比。 僅適用于資料倉儲。|None|
|cache_used_percent|已用快取的百分比|百分比|最大值|緩存使用的百分比。 僅適用于資料倉儲。|None|
|sqlserver_process_core_percent|SQL 伺服器進程核心百分比|百分比|最大值|SQL Server 進程的 CPU 使用率百分比，由作業系統測量。|None|
|sqlserver_process_memory_percent|SQL 伺服器進程記憶體百分比|百分比|最大值|SQL Server 進程的記憶體使用百分比，由作業系統測量。|None|
|tempdb_data_size|Tempdb 資料檔案大小 千位元組|Count|最大值|Tempdb 資料檔案大小千位元組。 不適用於資料倉儲。|None|
|tempdb_log_size|Tempdb 日誌檔案大小 千位元組|Count|最大值|Tempdb 日誌檔案大小千位元組。 不適用於資料倉儲。|None|
|tempdb_log_used_percent|使用的 Tempdb 百分比日誌|百分比|最大值|使用百分比日誌。 不適用於資料倉儲。|None|
|local_tempdb_usage_percent|本機 tempdb 百分比|百分比|Average|本地溫度百分比。 僅適用于資料倉儲。|None|
|app_cpu_billed|應用程式 CPU 計費|Count|總計|應用 CPU 計費。 適用于無伺服器資料庫。|None|
|app_cpu_percent|應用 CPU 百分比|百分比|Average|應用 CPU 百分比。 適用于無伺服器資料庫。|None|
|app_memory_percent|應用記憶體百分比|百分比|Average|應用記憶體百分比。 適用于無伺服器資料庫。|None|
|allocated_data_storage|已配置的資料空間|位元組|Average|已分配的資料存儲。 不適用於資料倉儲。|None|
|memory_usage_percent|記憶體百分比|百分比|最大值|記憶體百分比。 僅適用于資料倉儲。|None|
|full_backup_size_bytes|完全備份存儲大小|位元組|最大值|累積完整備份存儲大小。 適用于基於 vCore 的資料庫。 不適用於超大規模資料庫。|None|
|diff_backup_size_bytes|差分備份存儲大小|位元組|最大值|累積差異備份存儲大小。 適用于基於 vCore 的資料庫。 不適用於超大規模資料庫。|None|
|log_backup_size_bytes|記錄備份存儲大小|位元組|最大值|累積記錄備份存儲大小。 適用于基於 vCore 的資料庫。 不適用於超大規模資料庫。|None|
...
|dtu_consumption_percent|DTU 百分比|百分比|Average|DTU 百分比。 適用于基於 DTU 的彈性池。|None|
...
|eDTU_limit|eDTU 限制|Count|Average|eDTU 限制。 適用于基於 DTU 的彈性池。|None|
|storage_limit|資料大小上限|位元組|Average|資料大小上限|None|
|eDTU_used|已使用 eDTU|Count|Average|使用了 eDTU。 適用于基於 DTU 的彈性池。|None|
...
|cpu_limit|CPU 限制|Count|Average|CPU 限制。 適用于基於 vCore 的彈性池。|None|
|database_cpu_limit|CPU 限制|Count|Average|CPU 限制|DatabaseResourceId|
|cpu_used|使用的 CPU|Count|Average|使用的 CPU。 適用于基於 vCore 的彈性池。|None|
|database_cpu_used|使用的 CPU|Count|Average|使用的 CPU|DatabaseResourceId|
|sqlserver_process_core_percent|SQL 伺服器進程核心百分比|百分比|最大值|SQL Server 進程的 CPU 使用率百分比，由作業系統測量。 適用于彈性池。|None|
|sqlserver_process_memory_percent|SQL 伺服器進程記憶體百分比|百分比|最大值|SQL Server 進程的記憶體使用百分比，由作業系統測量。 適用于彈性池。|None|
|tempdb_data_size|Tempdb 資料檔案大小 千位元組|Count|最大值|Tempdb 資料檔案大小 千位元組|None|
|tempdb_log_size|Tempdb 日誌檔案大小 千位元組|Count|最大值|Tempdb 日誌檔案大小 千位元組|None|
|tempdb_log_used_percent|使用的 Tempdb 百分比日誌|百分比|最大值|使用的 Tempdb 百分比日誌|None|
...
|allocated_data_storage_percent|分配的資料空間百分比|百分比|最大值|分配的資料空間百分比|None|
...
|輸入|輸入|位元組|總計|輸入資料量 (位元組)。 此數目包括從外部用戶端輸入到 Azure 儲存體與 Azure 內的輸入。|地理類型，ApiName，身份驗證|
|輸出|輸出|位元組|總計|輸出資料量 (位元組)。 此數目包括從外部用戶端輸出到 Azure 儲存體與 Azure 內的輸出。 因此，此數目未反映可收費的輸出。|地理類型，ApiName，身份驗證|
|SuccessServerLatency|成功伺服器延遲|毫秒|Average|Azure 儲存體用來處理成功要求的平均延遲 (毫秒)。 此值不包括在 AverageE2ELatency 中指定的網路延遲。|地理類型，ApiName，身份驗證|
|SuccessE2ELatency|成功 E2E 延遲|毫秒|Average|向儲存體服務或所指定 API 作業發出之成功要求的平均端對端延遲 (毫秒)。 此值包括 Azure 儲存體內讀取要求、傳送回應及接收回應認可的必要處理時間。|地理類型，ApiName，身份驗證|
|可用性|可用性|百分比|Average|儲存體服務或所指定 API 作業的可用性百分比。 可用性的計算方式是將 TotalBillableRequests 值除以適用要求數目，包括產生意外錯誤的要求。 所有意外錯誤都會導致儲存體或所指定 API 作業的可用性降低。|地理類型，ApiName，身份驗證|
...
|輸入|輸入|位元組|總計|輸入資料量 (位元組)。 此數目包括從外部用戶端輸入到 Azure 儲存體與 Azure 內的輸入。|地理類型，ApiName，身份驗證，檔共用|
|輸出|輸出|位元組|總計|輸出資料量 (位元組)。 此數目包括從外部用戶端輸出到 Azure 儲存體與 Azure 內的輸出。 因此，此數目未反映可收費的輸出。|地理類型，ApiName，身份驗證，檔共用|
|SuccessServerLatency|成功伺服器延遲|毫秒|Average|Azure 儲存體用來處理成功要求的平均延遲 (毫秒)。 此值不包括在 AverageE2ELatency 中指定的網路延遲。|地理類型，ApiName，身份驗證，檔共用|
|SuccessE2ELatency|成功 E2E 延遲|毫秒|Average|向儲存體服務或所指定 API 作業發出之成功要求的平均端對端延遲 (毫秒)。 此值包括 Azure 儲存體內讀取要求、傳送回應及接收回應認可的必要處理時間。|地理類型，ApiName，身份驗證，檔共用|
|可用性|可用性|百分比|Average|儲存體服務或所指定 API 作業的可用性百分比。 可用性的計算方式是將 TotalBillableRequests 值除以適用要求數目，包括產生意外錯誤的要求。 所有意外錯誤都會導致儲存體或所指定 API 作業的可用性降低。|地理類型，ApiName，身份驗證，檔共用|
...
## <a name="microsoftstoragecachecaches"></a>微軟.存儲緩存/緩存

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|用戶端|客戶 IOPS 總數|Count|Average|緩存處理的用戶端檔操作的速率。|None|
|用戶端延遲|平均用戶端延遲|毫秒|Average|用戶端檔操作對存儲緩存的平均延遲。|None|
|用戶端閱讀|用戶端讀取 IOPS|每秒計數|Average|用戶端每秒讀取操作。|None|
|用戶端讀取輸送量|平均緩存讀取輸送量|每秒位元組|Average|用戶端讀取資料傳輸速率。|None|
|用戶端寫入|用戶端寫入 IOPS|每秒計數|Average|用戶端每秒寫入操作。|None|
|用戶端寫入輸送量|平均緩存寫入輸送量|每秒位元組|Average|用戶端寫入資料傳輸速率。|None|
|用戶端中繼資料讀取|用戶端中繼資料讀取 IOPS|每秒計數|Average|發送到緩存的用戶端檔操作的速率（不包括資料讀取）不修改持久狀態。|None|
|用戶端中繼資料寫入|用戶端中繼資料寫入 IOPS|每秒計數|Average|發送到緩存的用戶端檔操作的速率（不包括資料寫入）來修改持久狀態。|None|
|用戶端鎖定|用戶端鎖定 IOPS|每秒計數|Average|每秒用戶端檔鎖定操作。|None|
|存儲目標運行狀況|存儲目標運行狀況|Count|Average|緩存和存儲目標之間的連接測試結果。|None|
|Uptime|Uptime|Count|Average|緩存和監控系統之間的連接測試結果。|None|
|存儲目標|總存儲目標 IOPS|Count|Average|緩存發送到特定存儲目標的所有檔操作的速率。|存儲目標|
|存儲目標寫入IOPS|存儲目標寫入 IOPS|Count|Average|緩存發送到特定存儲目標的檔寫入操作的速率。|存儲目標|
|存儲目標同步寫入輸送量|存儲目標非同步寫入輸送量|每秒位元組|Average|緩存非同步將資料寫入特定存儲目標的速率。 這些是機會型寫入，不會導致用戶端阻塞。|存儲目標|
|存儲目標同步寫入輸送量|存儲目標同步寫入輸送量|每秒位元組|Average|緩存同步將資料寫入特定存儲目標的速率。 這些是導致用戶端阻止的寫入。|存儲目標|
|存儲目標總寫入輸送量|存儲目標總寫入輸送量|每秒位元組|Average|緩存將資料寫入特定存儲目標的總速率。|存儲目標|
|存儲目標延遲|存儲目標延遲|毫秒|Average|緩存發送到分段存儲目標的所有檔操作的平均往返延遲。|存儲目標|
|存儲目標中繼資料讀取IOPS|存儲目標中繼資料讀取 IOPS|每秒計數|Average|緩存發送到特定存儲目標的檔操作不修改持久狀態並排除讀取操作的速率。|存儲目標|
|存儲目標中繼資料寫入IOPS|存儲目標中繼資料寫入 IOPS|每秒計數|Average|緩存發送到特定存儲目標的檔操作的速率，這些操作修改持久狀態並排除寫入操作。|存儲目標|
|存儲目標讀取|存儲目標讀取 IOPS|每秒計數|Average|緩存發送到特定存儲目標的檔讀取操作的速率。|存儲目標|
|存儲目標讀取前通量|存儲目標提前讀取輸送量|每秒位元組|Average|緩存機會性地從存儲目標讀取資料的速率。|存儲目標|
|存儲目標填充輸送量|存儲目標填充輸送量|每秒位元組|Average|緩存從 StorageTarget 讀取資料以處理緩存錯過的速率。|存儲目標|
|存儲目標總讀取輸送量|存儲目標總讀取輸送量|每秒位元組|Average|緩存從特定存儲目標讀取資料的總速率。|存儲目標|

## <a name="microsoftstoragesyncstoragesyncservices"></a>微軟.存儲同步/存儲同步服務

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|伺服器同步會話結果|同步會話結果|Count|Average|每次伺服器終結點成功完成與雲終結點的同步會話時記錄值 1 的指標|同步組名稱、伺服器終結點名稱、同步方向|
|存儲同步會話應用檔計數|檔已同步|Count|總計|已同步的檔計數|同步組名稱、伺服器終結點名稱、同步方向|
|存儲同步會話Per專案錯誤計數|檔案無法同步|Count|總計|檔計數未能同步|同步組名稱、伺服器終結點名稱、同步方向|
|存儲同步批次轉移檔位元組|同步的位元組|位元組|總計|為同步會話傳輸的檔案大小總計|同步組名稱、伺服器終結點名稱、同步方向|
|存儲同步伺服器活動訊號|伺服器線上狀態|Count|最大值|每次重新被重新列印的伺服器成功記錄與雲終結點的活動訊號時記錄值 1 的指標|ServerName|
|存儲同步召回總大小位元組|雲端階層處理重新叫用|位元組|總計|伺服器召回的資料的總大小|ServerName|
|存儲同步已調用的總網路位元組|雲分層召回大小|位元組|總計|重新叫用的資料大小|同步組名稱，伺服器名稱|
|存儲同步存儲輸送量位元組每秒|雲分層召回輸送量|每秒位元組|Average|資料召回輸送量的大小|同步組名稱，伺服器名稱|
|存儲同步調用網路位元組應用程式|按應用程式劃分的雲分層召回大小|位元組|總計|應用程式調用的資料大小|同步組名稱、伺服器名稱、應用程式名稱|

## <a name="microsoftstoragesyncstoragesyncservicessyncgroups"></a>微軟.存儲同步/存儲同步服務/同步組

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|同步組同步應用檔計數|檔已同步|Count|總計|已同步的檔計數|同步組名稱、伺服器終結點名稱、同步方向|
|同步組會話項錯誤計數|檔案無法同步|Count|總計|檔計數未能同步|同步組名稱、伺服器終結點名稱、同步方向|
|同步組批次轉移檔位元組|同步的位元組|位元組|總計|為同步會話傳輸的檔案大小總計|同步組名稱、伺服器終結點名稱、同步方向|

## <a name="microsoftstoragesyncstoragesyncservicessyncgroupsserverendpoints"></a>微軟.存儲同步/存儲同步服務/同步組/伺服器終結點

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|伺服器端點同步應用檔計數|檔已同步|Count|總計|已同步的檔計數|伺服器終結點名稱，同步方向|
|伺服器端點同步會話項錯誤計數|檔案無法同步|Count|總計|檔計數未能同步|伺服器終結點名稱，同步方向|
|伺服器終結點批量傳輸檔位元組|同步的位元組|位元組|總計|為同步會話傳輸的檔案大小總計|伺服器終結點名稱，同步方向|

## <a name="microsoftstoragesyncstoragesyncservicesregisteredservers"></a>微軟.存儲同步/存儲同步服務/註冊伺服器

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|伺服器檢測|伺服器線上狀態|Count|最大值|每次重新被重新列印的伺服器成功記錄與雲終結點的活動訊號時記錄值 1 的指標|伺服器資源 Id，伺服器名稱|
|伺服器撤回總大小位元組|雲端階層處理重新叫用|位元組|總計|伺服器召回的資料的總大小|伺服器資源 Id，伺服器名稱|
...
|輸入事件源回記錄|待處理輸入事件數|Count|最大值|待處理輸入事件數|邏輯名稱，分區 Id|
|輸入事件源每秒|收到的輸入來源數|Count|總計|收到的輸入來源數|邏輯名稱，分區 Id|
...
|IngressReceivedMessagesCountLag|輸入接收的訊息計數延遲|Count|Average|事件源分區中最後一個排隊消息的序號與在入口中處理的消息的序號之間的差異|None|
|暖存儲最大值屬性|暖存儲最大屬性|Count|最大值|環境允許 S1/S2 SKU 的最大屬性數和 PAYG SKU 暖存儲允許的最大屬性數|None|
|熱存儲已用屬性|暖存儲使用屬性 |Count|最大值|環境用於 S1/S2 SKU 的屬性數和暖存儲用於 PAYG SKU 的屬性數|None|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|磁片讀取位元組秒|Disk Read Bytes/Sec|每秒位元組|Average|示例時間段內讀取操作導致的平均磁片輸送量。|None|
|磁片寫入位元組秒數|Disk Write Bytes/Sec|每秒位元組|Average|示例期間寫入操作時導致的平均磁片輸送量。|None|
|Disk Read Bytes|Disk Read Bytes|位元組|總計|示例時間段內讀取操作導致的總磁片輸送量。|None|
|Disk Write Bytes|Disk Write Bytes|位元組|總計|示例期間寫入操作時導致的總磁片輸送量。|None|
|磁片讀取操作|磁片讀取操作|Count|總計|上一個採樣期間中的 IO 讀取運算元。 請注意，這些操作的大小可能是可變的。|None|
|磁片寫入操作|磁片寫入操作|Count|總計|上一個示例期間中的 IO 寫入運算元。 請注意，這些操作的大小可能是可變的。|None|
|Disk Read Operations/Sec|Disk Read Operations/Sec|每秒計數|Average|上一個採樣期間的平均 IO 讀取運算元。 請注意，這些操作的大小可能是可變的。|None|
|Disk Write Operations/Sec|Disk Write Operations/Sec|每秒計數|Average|上一個採樣期間的平均 IO 寫入運算元。 請注意，這些操作的大小可能是可變的。|None|
|磁片讀取延遲|磁碟讀取延遲|毫秒|Average|讀取延遲總數。 設備和內核讀取延遲的總和。|None|
|磁片寫入延遲|磁碟寫入延遲|毫秒|Average|總寫入延遲。 設備和內核寫入延遲的總和。|None|
|網路位元組秒|以位元組/秒為單位的網路|每秒位元組|Average|接收流量的平均網路輸送量。|None|
|網路外位元組秒|網路出位元組/秒|每秒位元組|Average|傳輸流量的平均網路輸送量。|None|
|Network In|Network In|位元組|總計|接收流量的總網路輸送量。|None|
|Network Out|Network Out|位元組|總計|傳輸流量的總網路輸送量。|None|
|記憶體已使用|使用的記憶體|位元組|Average|VM 正在使用的電腦記憶體量。|None|
|記憶體授予|記憶體已授予|位元組|Average|主機授予 VM 的記憶體量。 在觸摸主機一次之前，不會授予記憶體，如果 VMkernel 需要記憶體，則授予的記憶體可能會被交換或膨脹。|None|
|記憶體活動|記憶體活動|位元組|Average|VM 過去小視窗使用的記憶體量。 這是 VM 當前需要多少記憶體的"真"數。 其他未使用的記憶體可能會被交換或膨脹，不會影響來賓的性能。|None|
...
|TcpSynsent|TCP 合成發送|Count|Average|TCP 合成發送|執行個體|
...
|TcpClose等待|TCP 關閉等待|Count|Average|TCP 關閉等待|執行個體|
...
|運行狀況檢查狀態|運行狀況檢查狀態|Count|Average|運行狀況檢查狀態|執行個體|
