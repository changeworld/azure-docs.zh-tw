...
系統會根據價格方案來儲存及保留已編製索引的記錄檔搜尋資料。 有關詳細資訊，請參閱[日誌分析定價](https://azure.microsoft.com/pricing/details/log-analytics/)。
...
## <a name="1-sign-up-for-log-analytics-and-collect-data"></a>1. 註冊日誌分析並收集資料
...
使用註冊金鑰註冊所有代理類型，並使用基於證書的身份驗證和帶埠 443 的 TLS 在代理和日誌分析服務之間建立安全連線。 Log Analytics 會使用秘密存放區來產生及維護金鑰。 私密金鑰每 90 天會輪替一次，其儲存在 Azure 中，並由遵守嚴格法規與相容性作法的 Azure 作業人員管理。
...
如上所述，來自管理伺服器或直接連接的代理的資料通過 TLS 發送到 Microsoft Azure 資料中心。 (選擇性) 您可以使用 ExpressRoute 為資料提供額外的安全性。 ExpressRoute 可供直接從現有 WAN 網路 (例如網路服務提供者所提供的多重通訊協定標籤交換 (MPLS) VPN) 連線至 Azure。 如需詳細資訊，請參閱 [ExpressRoute](https://azure.microsoft.com/services/expressroute/)。

## <a name="3-the-log-analytics-service-receives-and-processes-data"></a>3. 日誌分析服務接收和處理資料
...
儲存在資料庫中已收集資料的保留期，會取決於所選的定價方案。 對於「免費」** 層，收集的資料可使用七天。 對於「付費」** 層，收集的資料根據預設可供使用 31 天，但可以延長為 730 天。 資料會以待用加密的形式儲存在 Azure 儲存體，以確保資料機密性，並使用本地備援儲存體 (LRS) 在本地區域內複寫資料。 最近兩周的資料也存儲在基於 SSD 的緩存中，並且此緩存已加密。

## <a name="4-use-log-analytics-to-access-the-data"></a>4. 使用日誌分析訪問資料
