...
1. **可擴展警報**– 動態閾值警報規則可以一次為數百個指標系列創建量身定制的閾值，同時提供在單個指標上定義警報規則的同樣容易性。 它們可為您提供更少的警報來創建和管理。 可以使用 Azure 門戶或 Azure 資源管理器 API 來創建它們。 在處理指標維度或應用於多個資源（如所有訂閱資源）時，可擴充方法特別有用。  [深入了解如何使用範本設定具有動態閾值的計量警示](alerts-metric-create-templates.md)。

1. **智慧指標模式識別**– 使用我們的 ML 技術，我們能夠自動檢測指標模式並適應指標隨時間的變化，這通常包括季節性（每小時/每天/每週）。 適應指標隨時間而過的行為，並根據偏離其模式的警報減輕了了解每個指標的"正確"閾值的負擔。 動態閾值中使用的 ML 演算法可防止不具備預期模式的雜訊 (低精確度) 或廣泛 (低召回率) 閾值。
...
**故障期間**- 動態閾值還允許您配置"觸發警報的數量衝突"，即系統在一定時間範圍內提高警報所需的最小偏差數（預設時間視窗為 20 分鐘內的四個偏差）。 使用者可以設定失敗期間，並藉由變更失敗期間和時間範圍來選擇要警示的標的。 這項功能可減少暫時性尖峰所產生的警示雜訊。 例如：
...
您可以通過按一下電子郵件或文本消息中的連結或瀏覽器查看 Azure 門戶中的警示檢視，流覽警示檢視中觸發的警報實例。 [瞭解有關警示檢視的更多](alerts-overview.md#alerts-experience)。

警示檢視顯示：

- 觸發動態閾值警報時的所有指標詳細資訊。
- 觸發警報的時間段的圖表，包括該時間點使用的動態閾值。
- 能夠提供有關動態閾值警報和警示檢視體驗的回饋，從而改進未來的檢測。
...
首次創建警報規則時，圖表中顯示的閾值將基於足夠的歷史資料計算，以計算小時或每日季節模式（10 天）。 創建警報規則後，動態閾值將使用所有可用的所需歷史資料，並將根據新資料不斷學習和調整，以使閾值更準確。 這意味著，在此計算後，圖表還將顯示每週模式。

## <a name="how-much-data-is-needed-to-trigger-an-alert"></a>觸發警報需要多少資料？

如果您有新資源或缺少指標資料，則動態閾值不會在三天或 30 個指標資料樣本可用之前觸發警報，以確保閾值準確。
...
1. 在[Azure 門戶](https://portal.azure.com)中，按一下**監視器**。 [監視] 檢視會將您的所有監視設定與資料合併在一個檢視中。
...
![瞭解有關如何配置指標警報的更多](media/alerts-dynamic-thresholds/threshold-picture-8bit.png)

使用以下資訊解釋上一個圖表。

- **藍線**- 隨時間而測量的實際指標。
- **藍色陰影區域**- 顯示指標的允許範圍。 只要指標值保持在此範圍內，將不會發生警報。
- **藍點**- 如果向左按一下圖表的一部分，然後懸停在藍線上，則會在游標下方顯示一個藍點，顯示單個聚合指標值。
- **帶藍點的快顯視窗**- 顯示測量的指標值（藍點）以及允許範圍的上限和下部值。  
