...
我們 *不建議* 使用「自動調整」設定（例如下列範例）搭配「out」和「條件」相同或類似的臨界值：
...
4. 則在相應減少之前，自動調整會嘗試評估相應縮小後的最終狀態。 例如，575 x 3 (目前的執行個體計數) = 1,725 / 2 (相應減少後的最終執行個體數) = 862.5 個執行緒。 這表示自動調整必須在相應縮小之後，立即相應放大，如果平均執行緒計數保持不變，或甚至只落在很小的數量。 但自動調整若再相應增加，整個程序將會重複執行，進行產生無限迴圈。
5. 為了避免這種「不穩定」的狀況，自動調整根本不會相應減少， 而會在下次執行服務的工作時略過並重新評估條件。 Flapping 狀態可能會讓許多人混淆，因為平均執行緒計數為575時，自動調整不會正常運作。
...
3. 當總訊息計數達到 100 時，
4. 由於相應放大動作，因此會新增第三個儲存體佇列實例。  除非佇列中的總訊息計數達到 150 (因為 150/3=50)，否則不會執行下一個相應放大動作。
...
下圖顯示的自動調整設定中，其預設設定檔的最小執行個體數 = 2，最大執行個體數 = 10。 在此範例中，當佇列中的訊息計數大於10時，規則會設定為相應放大，當佇列中的訊息計數小於3時，則會相應放大。 因此我們現在可以將資源調整為 2 到 10 個執行個體。
...
- [建立活動記錄警示，以監視訂用帳戶上的所有自動調整引擎作業。](https://github.com/Azure/azure-quickstart-templates/tree/master/monitor-autoscale-alert)
