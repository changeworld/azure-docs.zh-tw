...
如果僅使用組合的一部分，自動縮放將只在單個方向（橫向擴展或中）執行操作，直到達到設定檔中定義的最大實例計數或最小實例計數。 這不是最佳作法，理想的狀態是讓資源在高使用量時擴展以確保可用性。 同樣地，您在低使用量時需要資源縮減，因此可實現成本節約。
...
*我們不建議*自動縮放設置，如以下示例，其閾值相同或相似，適用于 out 和條件：

* 當執行緒計數 >= 600 時，將實例增加 1 計數
* 當執行緒計數<= 600 時，將實例減少 1 計數
...
4. 則在相應減少之前，自動調整會嘗試評估相應縮小後的最終狀態。 例如，575 x 3 (目前的執行個體計數) = 1,725 / 2 (相應減少後的最終執行個體數) = 862.5 個執行緒。 這意味著，即使擴展，自動縮放也必須立即擴展，即使其擴展，如果平均執行緒數保持不變，甚至僅下降少量。 但自動調整若再相應增加，整個程序將會重複執行，進行產生無限迴圈。
5. 為了避免這種「不穩定」的狀況，自動調整根本不會相應減少， 而會在下次執行服務的工作時略過並重新評估條件。 抖動狀態會使許多人感到困惑，因為當平均執行緒數為 575 時，自動縮放似乎不起作用。
...
3. 當總訊息計數達到 100 時，
4. 由於橫向擴展操作，將添加第三個存儲佇列實例。  除非佇列中的總訊息計數達到 150 (因為 150/3=50)，否則不會執行下一個相應放大動作。
...
下圖顯示的自動調整設定中，其預設設定檔的最小執行個體數 = 2，最大執行個體數 = 10。 在此示例中，規則配置為在佇列中的消息計數大於 10 時橫向擴展，當佇列中的消息計數小於 3 時，將縮放到。 因此我們現在可以將資源調整為 2 到 10 個執行個體。
...
有些情況可能需要您在設定檔中設定多項規則。 自動縮放引擎在設置多個規則時使用以下自動縮放規則。

在*橫向擴展*時，如果滿足任何規則，自動縮放將運行。
...
* 如果 CPU 為 76%，記憶體為 50%，則我們橫向擴展。
* 如果 CPU 為 50%，記憶體為 76%，則我們橫向擴展。
...
預設實例計數很重要，因為自動縮放將服務縮放到指標無法接通的計數。 因此，請選取對您工作負載而言最安全的預設執行個體計數。
...
- [創建活動日誌警報以監視訂閱上的所有自動縮放引擎操作。](https://github.com/Azure/azure-quickstart-templates/tree/master/monitor-autoscale-alert)
