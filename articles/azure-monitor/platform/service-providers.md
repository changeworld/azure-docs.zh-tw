...
針對屬於[雲端解決方案提供者（CSP）](https://partner.microsoft.com/Solutions/cloud-reseller-overview)方案一部分的合作夥伴和服務提供者，Azure 監視器中的 Log Analytics 是 azure CSP 訂用帳戶中可用的其中一個 azure 服務。
...
1. 集中式工作區：服務提供者可以在其租用戶中建立工作區，並使用可搭配[資料收集 API](https://dev.loganalytics.io/) 使用[查詢 API](../../azure-monitor/platform/data-collector-api.md) 的指令碼，將不同工作區的資料帶到這個集中位置。 指令碼以外的另一個選項是使用 [Azure Logic Apps](https://docs.microsoft.com/azure/logic-apps/logic-apps-overview)。
