...
如[先前所宣佈](monitoring-classic-retirement.md)，Azure 監視器中的傳統警示已淘汰，但仍受限於尚未支援新警示的資源。 作為淘汰程式的一部分，Azure 入口網站提供[遷移工具](alerts-using-migration-tool.md)，讓客戶自行觸發遷移。
...
- 從**2019 年9月 1**日開始，客戶將無法建立任何新的傳統警示規則，但不包括[特定計量](alerts-understand-migration.md#manually-migrating-classic-alerts-to-newer-alerts)。
- 針對例外狀況，客戶可以繼續建立新的傳統警示規則，並使用其傳統警示，直到進一步公告為止。
- 從**2019 年9月 1**日開始，任何具有傳統警示的客戶都會以批次方式觸發傳統警示的遷移。
- 如同自發的遷移工具，某些未可移轉的傳統警示規則會保持不被使用。 這些傳統警示規則會繼續受到支援，直到進一步公告為止。 不過，任何不正確傳統警示規則將會遭到刪除，因為它們無法正常運作。
...
- 如果客戶需要採取動作，例如暫時停用資源鎖定或變更原則指派，客戶將需要解決任何這類問題。 如果未解決問題，則無法保證傳統警示的成功遷移。
