...
[Azure Advisor](../../advisor/advisor-overview.md)警告您有關此行為。 系統會針對 Azure Advisor 上的 [高可用性] 類別下的 [使用中度影響] 和 [修復您的記錄警示規則以確保監視] 的描述，新增特定記錄警示規則的建議。 如果在 Azure Advisor 提供了七天的建議之後，記錄警示規則中的警示查詢不會受到修復，Azure 監視器將會停用記錄警示，並確保當規則無法在可調整的期間（例如一周）內持續執行時，不會以不必要的方式計費。
