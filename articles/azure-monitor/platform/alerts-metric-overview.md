...
- 計量：CPU 百分比
- 條件類型：靜態
- 時間彙總 (針對未經處理的計量值執行的統計資料。 支援的時間聚合為最小、最大值、平均值、總計、計數：平均值
...
一段時間後，"myVM"上的用法會恢復正常（低於閾值）。 警示規則會再監視該條件兩次，以傳送解決通知。 當連續三個期間的警示條件都不符合時，警示規則會傳送已解決/停用訊息，以減少不穩定情況下的雜訊。
...
此功能當前支援以下 Azure 雲中以下服務的平臺指標（不是自訂指標）：
...
您可以通過以下三種方式之一指定單個指標警報規則的監視範圍。 例如，對於虛擬機器，您可以將作用域指定為：  
