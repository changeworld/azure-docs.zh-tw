...
- 計量：CPU 百分比
- 條件類型:靜態
- 時間彙總 (針對未經處理的計量值執行的統計資料。 支援的時間集合為最小、最大值、平均值、總計、計數:平均值
- 期間(檢查指標值的回望視窗):在過去 5 分鐘中
- 頻率(如果滿足條件,指標警報檢查的頻率):1 分鐘
- 運算符:大於
- 閾值：70
...
在一個規則中使用多個條件時,規則"和"一起將條件"放在一起。  也就是說,當警報中的所有條件都評估為 true 時,警報將觸發,並在其中一個條件不再為 true 時解析。 當"CPU 高於 90%"和"佇列長度超過 300 項"時,此類警報的示例將處於警報狀態。 
...
一段時間后,"myVM"上的用法會恢復正常(低於閾值)。 警示規則會再監視該條件兩次，以傳送解決通知。 當連續三個期間的警示條件都不符合時，警示規則會傳送已解決/停用訊息，以減少不穩定情況下的雜訊。
...
到目前為止，您已經了解如何使用單一計量警示來監視與單一 Azure 資源相關的一或多個計量時間序列。 很多時候，您可能想將相同的警示規則套用至許多資源。 Azure 監視器還支援使用一個指標警報規則監視同一 Azure 區域中存在的資源的多個資源(相同類型)。 

此功能目前支援以下 Azure 雲中以下服務的平台指標(不是自訂指標):
...
您可以通過以下三種方式之一指定單個指標警報規則的監視範圍。 例如,對於虛擬機,您可以將作用域指定為:  

- 訂閱中一個 Azure 區域中的虛擬機器清單
- 作為訂用帳戶中一或多個資源群組的所有虛擬機器 (位於一個 Azure 區域)
- 作為訂用帳戶中的所有虛擬機器 (位於一個 Azure 區域)

建立監視多個資源的計量警示規則，類似於[建立監視單一資源的任何其他計量警示](alerts-metric.md)。 唯一的區別是您會選取要監視的所有資源。 您也可以透過 [Azure Resource Manager 範本](../../azure-monitor/platform/alerts-metric-create-templates.md#template-for-a-metric-alert-that-monitors-multiple-resources)建立這些規則。 您將收到每個受監視資源的單個通知。
...
你可以[在本文中找到](../../azure-monitor/platform/alerts-metric-near-real-time.md#metrics-and-dimensions-supported)受支援的資源類型的完整清單。


## <a name="next-steps"></a>後續步驟

- [瞭解如何在 Azure 中建立、檢視和管理指標警報](alerts-metric.md)
- [了解如何使用 Azure Resource Manager 範本部署計量警示](../../azure-monitor/platform/alerts-metric-create-templates.md)
- [瞭解有關操作群組詳細資訊](action-groups.md)
