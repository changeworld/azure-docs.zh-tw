---
title: Azure 監視器中的自訂欄位（預覽） |微軟文檔
description: Azure 監視器的自訂欄位功能允許您從日誌分析工作區中添加到已收集記錄的屬性的記錄中創建自己的可搜索欄位。  本文說明用來建立自訂欄位的程序，並透過範例事件提供詳細的逐步解說。
ms.subservice: logs
ms.topic: conceptual
author: bwren
ms.author: bwren
...
---
# <a name="create-custom-fields-in-a-log-analytics-workspace-in-azure-monitor-preview"></a>在 Azure 監視器中的日誌分析工作區中創建自訂欄位（預覽）

> [!NOTE]
> 本文介紹如何在收集日誌分析工作區中分析文本資料。 我們建議在按照[Azure 監視器 中的 Parse 文本資料](../log-query/parse-text.md)中描述的指南收集查詢篩選器中的文本資料後對其進行分析。 與使用自訂欄位時，它提供了幾個優點。

Azure 監視器的**自訂欄位**功能允許您通過添加自己的可搜索欄位來擴展日誌分析工作區中的現有記錄。  自訂欄位會自動填入擷取自同一筆記錄中其他屬性的資料。

![總覽](media/custom-fields/overview.png)

例如，以下範例記錄在事件描述中埋藏了有用的資料。 將此資料提取到單獨的屬性中，可用於排序和篩選等操作。

![樣品提取](media/custom-fields/sample-extract.png)

> [!NOTE]
> 在預覽版中，工作區限制只能有 100 個自訂欄位。  這項功能正式上市時，此限制數量將會擴大。

## <a name="creating-a-custom-field"></a>建立自訂欄位
當您在建立自訂欄位時，Log Analytics 必須了解要用來填入其值的資料。  它會使用來自 Microsoft Research 稱為 FlashExtract 的技術快速識別此資料。  Azure 監視器不需要提供顯式說明，而是從提供的示例中瞭解要提取的資料。

下列各節提供用於建立自訂欄位的程序。  本文最後會逐步解說範例擷取作業。

> [!NOTE]
> 自訂欄位填充為與指定條件匹配的記錄添加到日誌分析工作區，因此它只會顯示在創建自訂欄位後收集的記錄上。  建立自訂欄位時便已存在於資料存放區的記錄不會新增自訂欄位。
> 

### <a name="step-1--identify-records-that-will-have-the-custom-field"></a>步驟 1 – 識別將會具有自訂欄位的記錄
第一個步驟是識別會取得自訂欄位的記錄。  從[標準日誌查詢](../log-query/log-query-overview.md)開始，然後選擇記錄作為 Azure 監視器將學習的模型。  當您指定您要將資料擷取到自訂欄位時，便會開啟 [欄位擷取精靈] **** 供您驗證及精簡準則。

1. 轉到**Logs**並使用[查詢檢索](../log-query/log-query-overview.md)將具有自訂欄位的記錄。
2. 選取 Log Analytics 將用來做為模型以擷取資料來填入自訂欄位的記錄。  您會識別您想要從這筆記錄擷取的資料，Log Analytics 會使用這項資訊來判斷要為所有類似記錄填入自訂欄位的邏輯。
3. 展開記錄屬性，按一下記錄頂部屬性左側的橢圓，然後從 中選擇 **"提取"欄位**。
4. 欄位**提取嚮導**將打開，您選擇的記錄將顯示在 **"主示例"** 列中。  這些記錄的自訂欄位將會以所選屬性中的相同值來定義。  
...
1. 在範例記錄中醒目提示您想要填入自訂欄位的文字。  然後，將顯示一個對話方塊，以提供欄位的名稱和資料類型，並執行初始提取。  字元**\_CF**將自動追加。
...
下面的章節會逐步解說建立自訂欄位的完整範例。  這個範例會擷取 Windows 事件中指出變更狀態之服務的服務名稱。  這依賴于服務控制管理員在 Windows 電腦上的系統啟動期間創建的事件。  如果您想要跟隨此範例，您必須 [收集系統記錄檔的資訊事件](data-sources-windows-events.md)。

我們輸入下列查詢，以從服務控制管理員傳回事件識別碼為 7036，且為指出服務正在啟動或停止之事件的所有事件。

![查詢](media/custom-fields/query.png)

然後，我們使用事件 ID 7036 選擇並展開任何記錄。

![來源記錄](media/custom-fields/source-record.png)

我們通過按一下頂部屬性旁邊的橢圓來定義自訂欄位。
...
我們醒目提示 [RenderedDescription]**** 屬性中的服務名稱，然後使用 [Service]**** 來識別服務名稱。  自訂欄位將會稱為 **Service_CF**。 在這種情況下，欄位類型是字串，因此我們可以保持該字串不變。

![欄位標題](media/custom-fields/field-title.png)

我們看到某些記錄已正確識別服務名稱，但某些記錄則未如此。   [搜尋結果]**** 顯示 **WMI Performance Adapter** 有部分名稱未能選取到。  摘要**顯示**，一個記錄標識**的模組安裝程式**，而不是**Windows模組安裝程式**。  
...
我們現在可以運行一個查詢，以驗證已創建但尚未添加到任何記錄**Service_CF。** 這是因為自訂欄位不能針對現有記錄，因此我們需要等待收集新記錄。
...
* 瞭解[日誌查詢](../log-query/log-query-overview.md)，以便使用條件自訂欄位生成查詢。
