...
* 請流覽 Log Analytics 意見反應頁面，以查看已提交的想法和 bug， [https://aka.ms/opinsightsfeedback](https://aka.ms/opinsightsfeedback) 或提出新的問題。 
...
- 執行下列查詢，以確認代理程式正在傳送心跳至其設定報告的工作區。 `<ComputerName>`以電腦的實際名稱取代。
...
- 依**事件來源**健全狀況服務模組]、[HealthService] 和 [服務連接器] 篩選*Operations Manager*事件記錄檔，  -  *Health Service Modules*並依**事件層級***警告*和*錯誤*篩選，以確認它是否已寫入下表中的事件。 *HealthService* *Service Connector* 如果是，請檢查每個可能事件所包含的解決步驟。
    ...
    |2129 |健全狀況服務模組 |失敗的連接/失敗的 TLS 協調 |檢查您的網路介面卡 TCP/IP 設定和代理程式 proxy 設定。|
    ...
    |4002 |服務連接器 |服務傳回 HTTP 狀態碼 403 以回應查詢。 請洽詢服務系統管理員以取得服務的健全狀況。 稍後將重試查詢。 |此錯誤是在代理程式的初始註冊階段所撰寫，您會看到類似下列的 URL： *HTTPs:// \<workspaceID> . oms.opinsights.azure.com/AgentService.svc/AgentTopologyRequest*。 錯誤碼403表示禁止，而且可能是因為輸入錯誤的工作區識別碼或金鑰，或是電腦上的資料和時間不正確所造成。 如果時間為自目前時間起的 + /-15 分鐘，則上架失敗。 若要修正此錯誤，請更新 Windows 電腦的日期和/或時區。|
...
1. 在電腦上開啟提升許可權的命令提示字元，然後輸入來重新開機 agent 服務 `net stop healthservice && net start healthservice` 。
