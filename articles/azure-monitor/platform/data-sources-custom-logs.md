...
>[!IMPORTANT]
>自訂日誌收集要求編寫日誌檔的應用程式定期將日誌內容刷新到磁片。 這是因為自訂日誌集合依賴于要跟蹤的日誌檔的檔案系統更改通知。
...
記錄將位於 *C:\MyApp\Logs*。  每一天都會建立一個新檔案，且其名稱中包括「appYYYYMMDD.log」 ** 模式的日期。  此日誌的足夠模式是*\\\*C：\MyApp_Logs .log*。
...
我們使用*MyApp_CL*的簡單查詢來從收集的日誌返回所有記錄。
