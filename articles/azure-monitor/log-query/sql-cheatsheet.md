...
下表有助於熟悉 SQL 的使用者了解 Kusto 查詢語言，以在 Azure 監視器中撰寫記錄查詢。 查看 T-SQL 命令，以解決 Azure 監視器日誌查詢中的常見方案和等效方案。
...
按度量值衡量的前 n 條記錄                |`SELECT TOP 100 name, COUNT(*) as Count FROM dependencies GROUP BY name ORDER BY Count asc`        |<code>dependencies <br>&#124; summarize Count=count() by name <br>&#124; top 100 by Count asc</code>
...
- 複習有關[在 Azure 監視器 中編寫日誌查詢的課程](get-started-queries.md)。
