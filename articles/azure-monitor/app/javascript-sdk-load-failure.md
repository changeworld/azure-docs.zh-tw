...
為了盡可能減少間歇性的網路連線失敗，我們已在所有 CDN 檔案上執行快取控制標頭，因此，一旦使用者的瀏覽器下載目前版本的 SDK 之後，瀏覽器就不需要再次下載，而且瀏覽器將會重複使用先前取得的複本（請參閱快取的[運作方式](../../cdn/cdn-how-caching-works.md)）。 如果快取檢查失敗或已有新的版本，則使用者的瀏覽器將需要下載更新的版本。 因此，您可能會在檢查失敗的情況中看到「_雜訊_」的背景層級，或在發生新發行且已正式推出（部署至 CDN）時，出現暫時性尖峰。
