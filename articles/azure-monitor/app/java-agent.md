---
title: Java Web 應用效能監視 - Azure 應用程式見解
...
---
# <a name="monitor-dependencies-caught-exceptions-and-method-execution-times-in-java-web-apps"></a>監視 Java Web 應用中的相依項、擷取的例外狀況


如果您已[使用 Application Insights 檢測您的 Java Web 應用程式][java]，您可以使用 Java 代理程式獲得更深入的見解，而不需變更任何程式碼：

* **相依性** ：您的應用程式對其他元件呼叫的相關資料，包括：
  * 通過 Apache HTTPClient、OkHTTP`java.net.HttpURLConnection`進行傳出 HTTP**調用**並捕獲。
  * 通過絕地客戶端撥打**的雷迪斯電話**被捕獲。
  * **JDBC 查詢**- 對於 MySQL 和 PostgreSQL,如果調用時間超過 10 秒,代理將報告查詢計劃。

* **應用程式紀錄記錄:** 擷取應用程式紀錄並將其與 HTTP 要求和其他遙測資料相關聯
  * **日誌4j 1.2**
  * **紀錄4j2**
  * **紀錄**
...
## <a name="additional-config-spring-boot"></a>附加設定 (彈簧啟動)
...
代理必須打包為專案中的資源,以便它最終位於 D:/home/網站/wwwroot/目錄中。 您可以通過存**取開發工具** > **進階工具** > **除錯主控台**並檢查網站目錄的內容來確認代理位於正確的應用服務目錄中。    

* 儲存設定並重新啟動您的應用程式。 (這些步驟僅適用於在 Windows 上運行的應用服務。
...
在「應用程式見解」資源中,聚合的遠端依賴項和方法執行時間將顯示在[效能磁貼下][metrics]。
