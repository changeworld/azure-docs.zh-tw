...
description: 開始使用 Application Insights 代理程式的詳細指示。 無須重新部署網站，即可監視網站效能。 適用于內部部署、Vm 或 Azure 中裝載的 ASP.NET web 應用程式。
ms.topic: conceptual
...
包含的最常見參數是您開始使用時所需的參數。
如果您沒有網際網路存取，我們也提供手動下載指示。

## <a name="get-an-instrumentation-key"></a>取得檢測金鑰

若要開始使用，您需要檢測金鑰。 如需詳細資訊，請參閱 [建立 Application Insights 資源](create-new-resource.md#copy-the-instrumentation-key)。
...
PowerShell 需要系統管理員層級的許可權，才能對您的電腦進行變更。
### <a name="execution-policy"></a>執行原則
- 描述：預設會停用執行中的 PowerShell 腳本。 建議僅針對目前的範圍允許 RemoteSigned 腳本。
...
藉由執行命令來審核您的 PowerShell 實例 `$PSVersionTable` 。
此命令會產生下列輸出：
...
這些指示是在執行 Windows 10 的電腦和上面所列版本的電腦上撰寫和測試。
...
1. 以具有更高許可權的執行原則，以系統管理員身分執行 PowerShell。
2. 安裝 NuGet 封裝提供者。
    - 描述：您需要此提供者，才能與 NuGet 型存放庫（例如 PowerShell 資源庫）進行互動。
...
3. 將 PowerShell 資源庫設定為受信任的存放庫。
    ...
    - 參考： [設定-PSRepository](/powershell/module/powershellget/set-psrepository?view=powershell-6)。
    - 命令： `Set-PSRepository -Name "PSGallery" -InstallationPolicy Trusted` 。
    - 選擇性參數：
        - `-Proxy`. 指定要求的 proxy 伺服器。

    如果 PowerShell 資源庫不受信任，您將會收到這個提示：
    ...
    您可以藉由執行命令來確認這項變更，並審核所有 Psrepository `Get-PSRepository` 。

4. 安裝最新版的 PowerShellGet。
    - 描述：此模組包含用來從 PowerShell 資源庫取得其他模組的工具。 版本1.0.0.1 隨附于 Windows 10 和 Windows Server。 需要版本1.6.0 或更高版本。 若要判斷已安裝的版本，請執行 `Get-Command -Module PowerShellGet` 命令。
    ...
    如果您不是使用最新版的 PowerShellGet，您將會收到此錯誤：
...
5. 重新開機 PowerShell。 您無法在目前的會話中載入新的版本。 新的 PowerShell 會話將會載入最新版本的 PowerShellGet。
...
1. 確定已符合 PowerShell 資源庫的所有必要條件。
2. 以具有更高許可權的執行原則，以系統管理員身分執行 PowerShell。
3. 安裝 Az. ApplicationMonitor 模組。
    - 參考： [Install 模組](/powershell/module/powershellget/install-module?view=powershell-6)。
...
如果基於任何原因而無法連接到 PowerShell 模組，您可以手動下載並安裝 ApplicationMonitor 模組。
...
2. 在 [ **版本歷程記錄** ] 資料表中，選取最新版本的檔案。
3. 在 [ **安裝選項**] 下，選取 [ **手動下載**]。

### <a name="option-1-install-into-a-powershell-modules-directory"></a>選項1：安裝至 PowerShell 模組目錄
將手動下載的 PowerShell 模組安裝到 PowerShell 目錄，讓 PowerShell 會話可以探索到它。
如需詳細資訊，請參閱 [安裝 PowerShell 模組](/powershell/scripting/developer/module/installing-a-powershell-module)。


#### <a name="unzip-nupkg-as-a-zip-file-by-using-expand-archive-v1010"></a>使用 Expand-Archive (v 1.0.1.0 版將 nupkg 解壓縮為 zip 檔案) 

- 描述：1.0.1.0 版的基底版本) 無法解壓縮 nupkg (檔案。 以 .zip 副檔名重新命名檔案。
- 參考： [Expand-Archive](/powershell/module/microsoft.powershell.archive/expand-archive?view=powershell-6)。
- 命令：
...
#### <a name="unzip-nupkg-by-using-expand-archive-v1100"></a>使用 Expand-Archive (v 1.1.0.0 解壓縮 nupkg) 

- 描述：使用目前版本的 Expand-Archive 將 nupkg 檔案解壓縮，而不變更副檔名。
- 參考： [Expand-archive](/powershell/module/microsoft.powershell.archive/expand-archive?view=powershell-6)和[Microsoft.](https://www.powershellgallery.com/packages/Microsoft.PowerShell.Archive/1.1.0.0)
- 命令：
...
如果您要將模組安裝到任何其他目錄，請使用 [import-module](/powershell/module/microsoft.powershell.core/import-module?view=powershell-6)手動匯入模組。

> [!IMPORTANT] 
> Dll 會透過相對路徑安裝。
> 將套件的內容儲存在您想要的執行時間目錄，並確認存取權限允許讀取但不寫入。

1. 將延伸模組變更為 ".zip"，然後將套件的內容解壓縮到您想要的安裝目錄中。
2. 尋找 Az.ApplicationMonitor.psd1 的檔案路徑。
3. 以具有更高許可權的執行原則，以系統管理員身分執行 PowerShell。
...
從 PowerShell 資源庫中下載並安裝 Az. ApplicationMonitor 的 PowerShell 命令支援 `-Proxy` 參數。
當您撰寫安裝腳本時，請參閱上述指示。

Application Insights SDK 必須將應用程式的遙測資料傳送給 Microsoft。 建議您在 web.config 檔案中，為您的應用程式設定 proxy 設定。 如需詳細資訊，請參閱 [APPLICATION INSIGHTS 常見問題： Proxy 傳遞](../faq.md#proxy-passthrough)。
