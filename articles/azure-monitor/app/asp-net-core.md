...
3. 選取 [**開始**使用]。 根據您的 Visual Studio 版本而定，此選取的文字可能會有所不同。 某些較舊版本則改為使用 [**開始免費**] 按鈕。
...
或者，您可以`FullScript`從`ScriptBody` SDK v 2.14 開始使用。 如果您需要控制`<script>`標記來設定內容安全性原則，請使用此動作：
...
將任何新`TelemetryInitializer`的`DependencyInjection`新增至容器，如下列程式碼所示。 SDK 會自動挑選已新增`TelemetryInitializer`至`DependencyInjection`容器的任何。
...
預設會顯示遙測初始化運算式。 若要移除所有或特定的遙測初始化運算式，請在*after*呼叫`AddApplicationInsightsTelemetry()`之後使用下列範例程式碼。
...
您可以使用上`TelemetryConfiguration` `AddApplicationInsightsTelemetryProcessor` `IServiceCollection`的擴充方法，將自訂遙測處理器加入至。 您在[advanced 篩選案例](https://docs.microsoft.com/azure/azure-monitor/app/api-filtering-sampling#itelemetryprocessor-and-itelemetryinitializer)中使用遙測處理器。 請使用下列範例。
...
使用函式插入`TelemetryClient`來取得的實例，並在其上`TrackXXX()`呼叫所需的方法。 我們不建議在 ASP.NET Core `TelemetryClient`應用程式中建立新的實例。 的單一實例已經`TelemetryClient`在`DependencyInjection`容器中註冊，而這會與`TelemetryConfiguration`其餘的遙測資料共用。 只有當您`TelemetryClient`需要的設定與其余的遙測資料不同時，才建議建立新的實例。
...
雖然仍然支援擴充`UseApplicationInsights()`方法，但是在 Application Insights SDK 版本2.8.0 中，它會標示為過時。 它將在 SDK 的下一個主要版本中移除。 啟用 Application Insights 遙測的建議方式是使用`AddApplicationInsightsTelemetry()` ，因為它會提供多載來控制某些設定。 此外，在 ASP.NET Core 3.x 應用程式中， `services.AddApplicationInsightsTelemetry()`是啟用 application insights 的唯一方法。
...
不可以。 [狀態監視器](https://docs.microsoft.com/azure/azure-monitor/app/monitor-performance-live-website-now)和[狀態監視器 v2](https://docs.microsoft.com/azure/azure-monitor/app/status-monitor-v2-overview)目前僅支援 ASP.NET 4.x。
