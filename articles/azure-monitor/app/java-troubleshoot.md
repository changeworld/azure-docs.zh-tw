...
* [通過在](#debug-data-from-the-sdk)應用程式 Insights.xml`<SDKLogger />`檔中的根節點下添加元素（在專案中的資源資料夾中），打開日誌記錄，並檢查以 AI 開頭的條目：INFO/WARN/ERROR 是否存在任何可疑日誌。 
...
* 開啟 [使用量和估計成本] 頁面，並檢查[取樣](../../azure-monitor/app/sampling.md)是否正在運作。 （100% 傳輸意味著採樣未運行。可以將應用程式見解服務設置為僅接受從應用到達的遙測資料的一小部分。 這有助於您維持在每月的遙測配額內。
...
### <a name="spring-boot-starter"></a>彈簧啟動器

要使用應用程式見解彈簧啟動啟動器啟用使用 Spring 啟動應用的 SDK 日誌記錄`application.properties`，請向檔添加以下內容：
...
要啟用 JVM 代理日誌記錄，請更新[AI 代理.xml 檔](java-agent.md)：
...
**我正在查看 Azure[門戶](https://portal.azure.com)。地圖是否告訴我有關我的應用的內容？**
