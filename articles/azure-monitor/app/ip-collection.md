...
若要啟用 IP 收集和儲存，Application Insights 元件`DisableIpMasking`的屬性必須設定為`true`。 這個屬性可以透過 Azure Resource Manager 範本或藉由呼叫 REST API 來設定。 
...
6. 部署完成後，就會記錄新的遙測資料。
    ...
    如果您再次選取並編輯範本，您只會看到預設範本，而且看不到新加入的屬性及其相關聯的值。 如果您看不到 IP 位址資料，而且想要`"DisableIpMasking": true`確認已設定。 執行下列 PowerShell：（將取代`Fabrikam-dev`為適當的資源和資源組名。）
