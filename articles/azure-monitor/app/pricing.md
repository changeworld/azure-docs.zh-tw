...
> [!NOTE]
> 本文介紹如何理解和控制應用程序見解的成本。  相關文章"[監視使用方式和估計成本](https://docs.microsoft.com/azure/azure-monitor/platform/usage-estimated-costs)「介紹了如何查看不同定價模型的多個 Azure 監視功能的使用方式和估計成本。

應用程式見解旨在獲取監視 Web 應用程式的可用性、性能和使用方式所需的一切,無論這些應用程式託管在 Azure 上還是本地。 應用程式見解支援常用語言和框架,如 .NET、JAVA 和 Node.js,並與 DevOps 行程和工具(如 Azure DevOps、Jira 和 PagerDuty)整合。 瞭解哪些決定監視應用程式的成本非常重要。 在本文中,我們將回顧是什麼驅動您的應用程序監控成本,以及如何主動監視和控制它們。
...
[Azure 應用程式見解][start]的定價是基於引入的數據量的**即用即付**模型,並可選擇用於延長數據保留時間。 每項 Application Insights 資源都是個別計費的服務，並且會計入到您的 Azure 訂用帳戶帳單。 資料量是以 Application Insights 從應用程式接收的未壓縮 JSON 資料套件大小來測量。 使用[實時指標流](../../azure-monitor/app/live-stream.md)不收取數據量。
...
如果尚未使用應用程式見解,則可以使用[Azure 監視器定價計算機](https://azure.microsoft.com/pricing/calculator/?service=monitor)來估計使用應用程式見解的成本。 首先在「搜索」框中輸入「Azure 監視器」,然後單擊生成的 Azure 監視器磁貼。 向下滾動頁面到 Azure 監視器,然後從"類型"下拉清單中選擇應用程式見解。  在這裡,您可以輸入您每月要收集的 GB 資料數,因此問題是應用程式見解將收集多少數據來監視您的應用程式。

有兩種方法可以解決此問題:使用預設監視和自適應採樣(這在ASP.NET SDK 中可用,或者根據其他類似客戶所看到的情況估計可能的數據引入。

### <a name="data-collection-when-using-sampling"></a>使用採樣時的資料收集

通過ASP.NET SDK 的[自適應採樣](sampling.md#adaptive-sampling),數據量會自動調整,以保持在指定的最大流量範圍內,以便默認應用程式見解監控。 如果應用程式生成的遙測量較低(例如調試時或由於使用率低),則只要捲低於每秒配置的事件級別,採樣處理器就不會刪除專案。 對於高容量應用程式,默認閾值為每秒五個事件,自適應採樣將限制每日事件數為 432,000 個。 使用典型的平均事件大小為 1 KB,這相當於每個託管應用程式的節點每個 31 天月 13.4 GB 的遙測數據(因為採樣是本地執行的每個節點的)。 

對於不支援自我調整的 SDK,可以使用[引入取樣,](https://docs.microsoft.com/azure/azure-monitor/app/sampling#ingestion-sampling)應用程式見解根據要保留的資料百分比接收資料時進行取樣,或者[對 ASP.NET、ASP.NET核心和 Java 網站進行固定利率採樣](sampling.md#fixed-rate-sampling),以減少從 Web 伺服器和 Web 瀏覽器傳送的流量

### <a name="learn-from-what-similar-customers-collect"></a>瞭解類似客戶收集的內容

在應用程式見解的 Azure 監視定價計算機中,如果啟用"基於應用程式活動估計數據量"功能,則可以提供有關應用程式的輸入(每月請求和每月頁面流覽次數,以防收集客戶端遙測),然後計算機將告訴您類似應用程式收集的數據的中位數和 90%。 這些應用程式涵蓋應用程式見解配置的範圍(例如,有些應用程式具有預設[採樣](../../azure-monitor/app/sampling.md),有些沒有採樣等),因此您仍具有使用採樣將所引入的數據量減少到遠低於中位數水準的控制項。 但這是瞭解其他類似客戶所看到的一個起點。
...
(請注意,本文屏幕截圖中顯示的所有價格僅供參考。 有關貨幣和地區當前的價格,請參閱[應用程式見解定價][pricing].)
...
要瞭解有關資料卷的更多資訊,請為應用程式見解資源選擇**指標**,請添加新圖表。 對圖表指標,**在基於紀錄的指標**下,選擇**資料點卷**。 按下 **「應用分割**」,然後按**`Telemetryitem`類型**選擇組。

![使用指標檢視資料量](./media/pricing/10-billing.png)

### <a name="queries-to-understand-data-volume-details"></a>瞭解資料詳細資訊查詢

有兩種方法可以調查應用程式見解的數據卷。 第一個使用表中的`systemEvents`聚合資訊,第二個`_BilledSize`使用 屬性,該屬性在每個引入的事件上都可用。

#### <a name="using-aggregated-data-volume-information"></a>使用集合資料資料資訊

例如,可以使用表`systemEvents`查看過去 24 小時內隨查詢引入的數據卷:
...
或者,要查看過去 30 天按數據類型顯示的資料量圖表(以位元組為單位),可以使用:
...
請注意,此查詢可用於[Azure 日誌警報](https://docs.microsoft.com/azure/azure-monitor/platform/alerts-unified-log),以設置數據卷的警報。  

要瞭解有關遙測數據更改的更多情況,我們可以使用查詢按類型獲取事件計數:
...
例如,要查看過去 30 天內哪些操作生成的數據量最多,我們可以為所有依賴項`_BilledSize`事件求 和:
...
## <a name="viewing-application-insights-usage-on-your-azure-bill"></a>在 Azure 帳單上檢視應用程式來解使用方式

Azure 在[Azure 成本管理和計費](https://docs.microsoft.com/azure/cost-management/quick-acm-cost-analysis?toc=/azure/billing/TOC.json)中心中提供了大量有用的功能。 例如,「成本分析」功能使您能夠查看 Azure 資源的支出。 按資源類型添加篩選器(到Microsoft.insights/應用程式見解元件)將允許您追蹤支出。
...
在下載的電子表格中,每天可以看到每個 Azure 資源的使用方式。 在此 Excel 電子表格中,可以通過首先在「儀表類別」列上篩選以顯示「應用程式見解」和「日誌分析」,然後在「實例 ID」列上添加篩選器(即"包含 Microsoft.insights/元件")中找到應用程式見解資源的使用方式。  大多數應用程式見解使用方式都報告在具有日誌分析儀表類別的儀錶上,因為所有 Azure 監視器元件都有一個日誌後端。  只有舊版定價層和多步驟 Web 測試的應用程式見解資源報告應用程式見解表類別的應用程式見解。  用法顯示在「消耗量」列中,每個條目的單位顯示在「度量單位」列中。  有關詳細資訊,可説明您瞭解 Microsoft [Azure 帳單](https://docs.microsoft.com/azure/billing/billing-understand-your-bill)。
...
* **限制Ajax呼叫**:您可以限制可在每個頁面檢視中[報告的Ajax調用數](../../azure-monitor/app/javascript.md#configuration),或關閉Ajax報告。

* **關閉不需要的模組**:[編輯應用程式Insights.config](../../azure-monitor/app/configuration-with-applicationinsights-config.md)以關閉不需要的集合模組。 例如，您可能會決定效能計數器或相依性資料是不必要的。

* **預聚合指標**:如果在應用中調用 TrackMetric,則可以使用接受計算一批測量的平均和標準偏差的過載來減少流量。 或者，您也可以使用[預先彙總套件](https://www.myget.org/gallery/applicationinsights-sdk-labs)。
...
* **節流**：節流會將資料速率限制在每秒 32,000 個事件 (每個檢測金鑰以 1 分鐘計算平均值)。 系統會每分鐘評估應用程式傳送的資料量。 如果每秒速率超過每分鐘平均值，伺服器會拒絕部分要求。 SDK 會緩衝處理資料，然後嘗試重新傳送它。 它會在數分鐘的期間內散佈大量資料。 如果應用程式始終以高於節流速率的方式傳送資料，有些資料會就遭到捨棄。 (ASP.NET、JAVA 和 JAVAScript SDK 嘗試以這種方式重新發送數據;其他 SDK 可能只是丟棄受限制的數據。如果發生限制,將發出通知警告,提醒您已發生這種情況。
...
查看應用程式見解使用方式和估計成本,以瞭解數據引入趨勢以及要定義的每日交易量上限。 應該謹慎考慮它,因為達到限制后您將無法監視資源。

### <a name="set-the-daily-cap"></a>設定每日上限

要更改每日上限,請在「**使用和估計成本**」頁中的「設定應用程式見解」資源的「**設定**」部分中,選擇 **「每日上限**」 。

![調整每日遙測資料量上限](./media/pricing/pricing-003.png)

要[透過 Azure 資源管理員變更每日上限](../../azure-monitor/app/powershell.md),要變更`dailyQuota`的屬性是 。  透過 Azure 資源管理員,您`dailyQuotaResetTime`還可以設定`warningThreshold`與一個上限 。

### <a name="create-alerts-for-the-daily-cap"></a>為每日上限建立警報

當引入的數據卷達到警告級別或每日上限級別時,應用程式見解每日上限會在 Azure 活動日誌中創建事件。  您可以[從這些活動紀錄事件建立警報](https://docs.microsoft.com/azure/azure-monitor/platform/alerts-activity-log#create-with-the-azure-portal)。 這些事件的信號名稱稱為:
...
要變更保留,請從應用程式見解資源轉到 **"使用方式和估計成本**"頁,然後選擇 **「資料保留**」選項:
...
將數據發送到應用程式見解可能會導致數據頻寬費用。 如 Azure[頻寬定價頁](https://azure.microsoft.com/pricing/details/bandwidth/)中所述,位於兩個區域中的 Azure 服務之間的數據傳輸按正常速率作為出站數據傳輸。 入站數據傳輸是免費的。 但是,此費用非常小(很少%)與應用程式見解日誌數據引入的成本相比。 因此,控制日誌分析的成本需要專注於您的引入資料量,我們有指導來説明[理解這一點](https://docs.microsoft.com/azure/azure-monitor/app/pricing#managing-your-data-volume)。
...
* 為「每個節點」層中的任何應用發送遙測數據的每個節點付費。
  * *節點*是承載應用的物理或虛擬伺服器計算機或平臺即服務角色實例。
...
* 每個節點 (資料粒度為小時) 偵測每日 200 MB 的資料量配置。 未使用的資料配置不會累計到隔天。
  * 如果選擇「每個節點」定價層,則每個訂閱會根據向該訂閱中的應用程式見解資源發送遙測的節點數獲取每日數據量。 因此，如果您有 5 個節點全天候傳送資料，您會有合起來 1GB 的資料額度，套用至該訂用帳戶中的所有 Application Insights 資源。 由於內含資料會在所有節點之間共用，因此特定節點所傳送的資料是否超過其他節點並無妨。 如果在給定的某一天,應用程式見解資源接收的數據多於此訂閱的每日數據分配中包含的數據,則每 GB 超額數據費用將收取。 
