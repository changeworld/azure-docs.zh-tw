...
選取 [ **移至詳細資料** ] 以探索端對端交易體驗，其可向下提供向下查看至呼叫堆疊層級。
...
應用程式對應會使用 [ **雲端角色名稱** ] 屬性來識別地圖上的元件。 手動設定或覆寫雲端角色名稱，並變更應用程式對應上顯示的內容：

> [!NOTE]
> Application Insights SDK 或代理程式會自動將雲端角色名稱屬性新增至 Azure App Service 環境中元件發出的遙測。
...
**ASP.NET apps：將初始化運算式載入至使用中的 TelemetryConfiguration**
...
**ASP.NET Core apps：將初始化運算式載入至 TelemetryConfiguration**
...
針對 [JAVA agent 3.0](./java-in-process-agent.md) ，雲端角色名稱的設定如下：
...
如果您使用 SDK （從 Application Insights JAVA SDK 2.5.0 開始），您可以藉由新增至檔案來指定雲端角色名稱 `<RoleName>` `ApplicationInsights.xml` ，例如
...
彈簧開機 starter 會自動將雲端角色名稱指派給您在 spring.application.name 屬性中輸入的值。
...
至於如何思考 **雲端角色名稱**，查看具有多個雲端角色名稱的應用程式對應可能會很有説明：

![應用程式對應螢幕擷取畫面](media/app-map/cloud-rolename.png)

在應用程式對應中，綠色方塊中的每個名稱都是此特定分散式應用程式不同層面的雲端角色名稱值。 因此對於此應用程式，其角色包含： `Authentication` 、 `acmefrontend` 、 `Inventory Management` 、a `Payment Processing Worker Role` 。 

在此應用程式的案例中，每個雲端角色名稱也都代表不同的唯一 Application Insights 資源，並具有自己的檢測金鑰。 由於此應用程式的擁有者可以存取這四個不同 Application Insights 資源，因此應用程式對應能夠將基礎關聯性的地圖拼接在一起。

若為 [正式定義](https://github.com/Microsoft/ApplicationInsights-dotnet/blob/39a5ef23d834777eefdd72149de705a016eb06b0/Schema/PublicSchema/ContextTagKeys.bond#L93)：
...
如需如何使用遙測初始化運算式覆寫 [雲端角色名稱] 屬性的詳細資訊，請參閱 [新增屬性： ITelemetryInitializer](api-filtering-sampling.md#addmodify-properties-itelemetryinitializer)。
...
3. 如果您是使用 Azure Functions 搭配 c #，請升級至函式 [V2](../../azure-functions/functions-versions.md)。

4. 確認 [雲端角色名稱](#set-or-override-cloud-role-name) 已正確設定。
...
應用程式對應會針對存在於要求遙測中的每個唯一雲端角色名稱，以及相依性遙測中類型、目標和雲端角色名稱的每個唯一組合，建立應用程式節點。 如果您的遙測中有10000個以上的節點，應用程式對應將無法提取所有節點和連結，因此您的對應將不會完成。 如果發生這種情況，則在觀看地圖時將會出現警告訊息。

此外，應用程式對應最多隻支援一次轉譯最多1000個已取消群組的節點。 應用程式對應會將具有相同類型和呼叫端的相依性群組在一起，以減少視覺效果複雜度，但如果您的遙測具有太多唯一的雲端角色名稱或太多相依性類型，則該群組將會不足，而且地圖將無法呈現。

若要修正此問題，您必須變更檢測，以適當地設定雲端角色名稱、相依性類型和相依性目標欄位。

* 相依性目標應代表相依性的邏輯名稱。 在許多情況下，它相當於相依性的伺服器或資源名稱。 例如，在 HTTP 相依性的案例中，它會設定為主機名。 它不應該包含從一個要求變更為另一個要求的唯一識別碼或參數。

* 相依性類型應代表相依性的邏輯類型。 例如，HTTP、SQL 或 Azure Blob 都是典型的相依性類型。 不應包含唯一識別碼。

* [上述章節](#set-or-override-cloud-role-name)將說明雲端角色名稱的用途。
...
* 如需 ASP.NET Core 和 ASP.NET 中的先進相互關聯案例，請參閱 [追蹤自訂作業](custom-operations-tracking.md) 文章。