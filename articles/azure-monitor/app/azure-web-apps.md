...
* 藉由安裝 Application Insights SDK，**透過程式碼手動檢測應用程式**。
    ...
    * 如果您需要進行自訂 API 呼叫來追蹤預設不是以代理程式為基礎所捕捉的事件/相依性，則必須使用此方法。 若要深入瞭解，請參閱[自訂事件和計量的 API 一文](https://docs.microsoft.com/azure/azure-monitor/app/api-custom-events-metrics)。 這也是 Linux 型工作負載目前唯一支援的選項。
