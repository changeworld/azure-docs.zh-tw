...
您可以使用 Azure PowerShell Cmdlet `New-AzResourceGroupDeployment`或 Azure CLI，透過提供的 Azure Resource Manager 範本來部署解決方案。
...
若要先識別`workspaceResourceId` **containerSolutionParams**中參數值所需之 Log Analytics 工作區的完整資源識別碼，請執行下列步驟，然後執行 PowerShell Cmdlet 或 Azure CLI 命令來新增解決方案。
...
* 驗證所有 Linux 背景工作角色節點`kubernetes.io/role=agent`都有標籤可排程 rs pod。 如果不存在，請將它加入。
* 驗證`cAdvisor secure port:10250`或`unsecure port: 10255`在叢集中的所有節點上開啟。
