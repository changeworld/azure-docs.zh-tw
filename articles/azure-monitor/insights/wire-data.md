---
title: Azure 監視器中的線路資料解決方案 |Microsoft Docs
...
除了 Log Analytics 代理程式，Wire Data 解決方案還會使用您在 IT 基礎結構的電腦上所安裝的 Microsoft 相依性代理程式。 相依性代理程式會監視往返於您電腦傳送的網路資料 (屬於 [OSI 模型](https://en.wikipedia.org/wiki/OSI_model)中的網路層級 2-3)，包括使用的各種通訊協定和連接埠。 然後，會使用代理程式將資料傳送至 Azure 監視器。  

>[!NOTE]
>網路資料解決方案已取代為 [服務對應的解決方案](service-map.md)。  兩者都會使用 Log Analytics 代理程式和相依性代理程式，將網路連線資料收集 Azure 監視器。 
> 
>使用網路資料解決方案的現有客戶可以繼續使用。 我們將發佈移至服務對應的遷移時程表指引。
>
>新客戶應該安裝 [服務對應的解決方案](service-map.md) 或 [適用於 VM 的 Azure 監視器](vminsights-overview.md)。  服務對應資料集相當於網路資料。  適用於 VM 的 Azure 監視器包含服務對應資料集，以及額外的效能資料和功能可供分析。 


根據預設，Azure 監視器會將 CPU、記憶體、磁片和網路效能資料的資料，從 Windows 和 Linux 內建的計數器，以及您可以指定的其他效能計數器記錄。 針對每個代理程式，都是即時收集網路和其他資料，包括電腦使用的子網路和應用程式層級通訊協定。  Wire Data 會查看應用程式層級的網路資料，而不會往下查看 TCP 傳輸層。 解決方案不會查看個別的 ACK 和 SYN。 交握完成後，即會將它視為即時連接，並且標示為已連接。 只要兩端同意通訊端為開啟，該連接會維持為即時，且可以來回傳遞資料。 一旦任一端關閉連接，它就會標示為已中斷連線。  因此，它只會計算已成功完成封包的頻寬，而不會回報重新傳送或失敗的封包。
...
不過，因為是檢視中繼資料，在深入的疑難排解中不見得實用。 Azure 監視器中的連線資料不是完整的網路資料捕捉。  不適用於封包層級的深入疑難排解。 相較于其他收集方法，使用代理程式的優點是您不需要安裝設備、重新設定網路交換器，或執行複雜的設定。 連線資料是以代理程式為基礎—在電腦上安裝代理程式，它就會監視自己的網路流量。 另一個優點是您想要監視雲端提供者、主機服務提供者或 Microsoft Azure 中執行的工作負載，而其中使用者未擁有網狀架構層級。

## <a name="connected-sources"></a>連接的來源

Wire Data 會從 Microsoft 相依性代理程式取得其資料。 Dependency Agent 相依于 Log Analytics 代理程式，以供其 Azure 監視器的連線。 這表示，伺服器必須先安裝 Log Analytics 代理程式，並設定 Dependency Agent。 下表描述 Wire Data 解決方案支援的連線來源。

| **連線的來源** | **支援** | **說明** |
| --- | --- | --- |
| Windows 代理程式 | 是 | Wire Data 會分析並收集來自 Windows 代理程式電腦的資料。 <br><br> 除了 [適用于 windows 的 Log Analytics 代理程式](../platform/agent-windows.md)，windows 代理程式還需要 Microsoft Dependency agent。 如需作業系統版本的完整清單，請參閱 [支援的作業系統](vminsights-enable-overview.md#supported-operating-systems) 。 |
| Linux 代理程式 | 是 | Wire Data 會分析並收集來自 Linux 代理程式電腦的資料。<br><br> 除了 [適用于 linux 的 Log Analytics 代理程式](../learn/quick-collect-linux-computer.md)以外，linux 代理程式還需要 Microsoft Dependency agent。 如需作業系統版本的完整清單，請參閱 [支援的作業系統](vminsights-enable-overview.md#supported-operating-systems) 。 |
| System Center Operations Manager 管理群組 | 是 | Wire Data 會在連線的 [System Center Operations Manager 管理群組](../platform/om-agents.md)中，分析並收集來自 Windows 和 Linux 代理程式的資料。 <br><br> 需要從 System Center Operations Manager 代理程式電腦直接連接到 Azure 監視器。 |
| Azure 儲存體帳戶 | 否 | Wire Data 會收集來自代理程式電腦的資料，因此沒有要從 Azure 儲存體收集的資料。 |

在 Windows 上，System Center Operations Manager 和 Azure 監視器都會使用 Microsoft Monitoring Agent (MMA) 來收集和傳送資料。 視內容而定，此代理程式可稱為 System Center Operations Manager 代理程式、Log Analytics 代理程式、MMA 或直接代理程式。 System Center Operations Manager 和 Azure 監視器會提供稍微不同的 MMA 版本。 這些版本可以每個報表 System Center Operations Manager、Azure 監視器或兩者。

在 Linux 上，適用于 Linux 的 Log Analytics 代理程式會收集資料，並將資料傳送至 Azure 監視器。 您可以在代理程式直接連線到 Azure 監視器，或透過 System Center Operations Manager 管理群組連線到 Azure 監視器的伺服器上使用網路資料。

相依性代理程式不會傳輸任何資料，而且不需要對防火牆或埠進行任何變更。 連線資料中的資料一律會由 Log Analytics 代理程式傳輸至 Azure 監視器，不論是直接或透過 Log Analytics 閘道。

![代理程式圖表](./media/wire-data/agents.png)

如果您是管理群組連線到 Azure 監視器的 System Center Operations Manager 使用者：

- 當您的 System Center Operations Manager 代理程式可以存取網際網路以連接到 Azure 監視器時，不需要進行其他設定。
- 當您的 System Center Operations Manager 代理程式無法透過網際網路存取 Azure 監視器時，您需要設定 Log Analytics 閘道以搭配 System Center Operations Manager。

如果您的 Windows 或 Linux 電腦無法直接連線到服務，您必須將 Log Analytics 代理程式設定為使用 Log Analytics 閘道連接到 Azure 監視器。 您可以從 [Microsoft 下載中心](https://www.microsoft.com/download/details.aspx?id=52666)下載 Log Analytics 閘道。
...
- 需要有系統管理員許可權，才能安裝或卸載 Dependency agent。
- 相依性代理程式必須安裝在具有64位作業系統的電腦上。

### <a name="operating-systems"></a>作業系統

下列各節列出相依性代理程式支援的作業系統。 Wire Data 不支援任何 32 位元架構的作業系統。
...
1. 從 [Azure Marketplace](https://azuremarketplace.microsoft.com/marketplace/apps/Microsoft.WireData2OMS?tab=Overview) 啟用活動記錄分析解決方案，或使用 [從方案庫新增監視解決方案](./solutions.md)中所述的程式。
2. 在您要取得資料的每部電腦上安裝 Dependency agent。 相依性代理程式可以監視立即相鄰鄰居的連接，因此您可能不需要在每部電腦上都有代理程式。
...
相依性代理程式是透過 InstallDependencyAgent-Windows.exe 安裝在執行 Windows 的電腦上。 如果您在執行此可執行檔時不使用任何選項，則會啟動以互動方式安裝的精靈。

使用下列步驟，在執行 Windows 的每部電腦上安裝 Dependency agent：

1. 依照[為混合式環境中的 Windows 電腦設定 Log Analytics 代理程式](../platform/agent-windows.md)一文中的步驟，安裝 Log Analytics 代理程式。
...
相依性代理程式是透過 Installdependencyagent-linux64.bin (（具有自我解壓縮二進位檔的 shell 腳本）安裝在 Linux 電腦上。 您可以使用 _sh_ 來執行檔案，或對檔案本身新增執行權限。

請使用下列步驟在每一部 Linux 電腦上安裝 Dependency Agent：

1. 依照[為混合式環境中的 Linux 電腦設定 Log Analytics 代理程式](../learn/quick-collect-linux-computer.md#obtain-workspace-id-and-key)一文中的步驟，安裝 Log Analytics 代理程式。
...
若要一次輕鬆地在多部伺服器上部署相依性代理程式，可協助使用腳本。 您可以使用下列腳本範例，在 Windows 或 Linux 上下載並安裝 Dependency agent。
...
若要透過 Desired State Configuration 部署相依性代理程式，您可以使用 xPSDesiredStateConfiguration 模組和一些程式碼，如下所示：
...
#### <a name="uninstall-the-dependency-agent-on-windows"></a>在 Windows 上卸載 Dependency agent
...
若要完全卸載 Linux 上的相依性代理程式，您必須移除代理程式本身，以及自動與代理程式一起安裝的連接器。 您可以使用下列單一命令同時解除安裝這兩者：
...
在 Log Analytics 工作區中啟動 Wire Data 時，會將 300 KB 的管理組件傳送至該工作區中的所有 Windows 伺服器。 如果您是在[連線的管理群組](../platform/om-agents.md)中使用 System Center Operations Manager 代理程式，則會從 System Center Operations Manager 部署相依性監視管理組件。 如果代理程式是直接連接的，Azure 監視器提供管理元件。
...
- 使用 [從方案庫新增監視解決方案](solutions.md)中所述的程式，將網路資料解決方案新增至 Log Analytics 工作區。 不需要進一步的組態。
...
![在 [Wire Data 2.0] 儀表板中的代理程式 [正在捕捉網路流量] 分頁的螢幕擷取畫面，其中顯示每部電腦所耗用的網路頻寬。](./media/wire-data/log-search-example01.png)

同樣地，您可以使用 [區域子網路數]**** 刀鋒視窗，來判斷透過子網路移動的網路流量。 使用者通常會為其應用程式定義重要區域周圍的子網路。 此刀鋒視窗會提供這些區域的檢視。

![Wire Data 2.0 儀表板中 [本機子網] 分頁的螢幕擷取畫面，其中顯示每個 LocalSubnet 所使用的網路頻寬。](./media/wire-data/log-search-example02.png)

[應用程式層級通訊協定數]**** 刀鋒視窗可協助了解使用中的通訊協定，因此很有用。 例如，您可能預期 SSH 不會用於網路環境中。 藉由檢視刀鋒視窗中可用的資訊，即可快速確認或否認您的預期。

![[應用層級通訊 Wire Data 2.0 協定] 分頁的螢幕擷取畫面，其中顯示每個通訊協定所耗用的網路頻寬。](./media/wire-data/log-search-example03.png)
...
- [搜尋記錄](../log-query/log-query-overview.md)以檢視詳細的連線資料搜尋記錄。
