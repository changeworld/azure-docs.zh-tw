...
Azure 中的代理程式健全狀況解決方案可協助您瞭解在 Azure 監視器中直接向 Log Analytics 工作區回報的所有代理程式，或連接到 Azure 監視器的 System Center Operations Manager 管理群組，而這些代理程式沒有回應且無法提交運算元據。  您可以也追蹤已部署的代理程式數目，其散佈地區，並執行其他查詢，以留意 Azure、其他雲端環境或內部部署中部署之代理程式的散佈情形。    
...
| System Center Operations Manager 管理群組 | 是 | 每隔60秒向管理群組報告的代理程式都會收集心跳事件，然後將其轉送至 Azure 監視器。 不需要從 Operations Manager 代理程式直接連接到 Azure 監視器。 從管理群組轉送至 Log Analytics 工作區的訊號事件資料。|
...
| 依管理群組分配 | 您環境中不同 Operations Manager 管理群組的磁碟分割。|
| 代理程式的地理位置 | 您擁有代理程式之不同國家/地區的磁碟分割，以及已在每個國家/地區中安裝的代理程式數目總計數。|
...
| `ComputerIP` | 電腦的公用 IP 位址。 在 Azure Vm 上，這會顯示公用 IP （如果有的話）。 針對使用私人 ip 的 Vm，這會顯示 Azure SNAT 位址， (不是私人 IP 位址) 。 |
...
每個向 Operations Manager 管理伺服器報告的代理程式都會傳送兩個信號，而 SCAgentChannel 屬性的值將會包含 **Direct** 和 **>scmanagementserver** ，這取決於您在訂用帳戶中啟用的資料來源和監視解決方案。 如果您還記得，解決方案的資料會從 Operations Manager 管理伺服器直接傳送到 Azure 監視器，或由於代理程式上收集的資料量，會直接從代理程式傳送到 Azure 監視器。 若為具有 **SCManagementServer** 值的活動訊號事件，ComputerIP 值為管理伺服器的 IP 位址，因為資料實際上由它上傳。  若為 SCAgentChannel 設為 [Direct]**** 的活動訊號，此值為代理程式的公用 IP 位址。  
