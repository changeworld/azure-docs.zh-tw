...
>[!NOTE]
>範本必須部署在與啟用的虛擬機器或虛擬機器擴展集相同的資源群組中。


您可以從 GitHub 存放庫[下載](https://aka.ms/VmInsightsARMTemplates)的封存檔案（.zip）中提供 Azure Resource Manager 範本。 檔案的內容包括代表每個部署案例的資料夾，以及範本和參數檔案。 在執行之前，請修改參數檔案，並指定所需的值。 
