...
適用于容器的 Azure 監視器可為 Azure Kubernetes Service （AKS）和 AKS 引擎叢集提供豐富的監視體驗。 本文說明如何針對裝載于[Azure Red Hat OpenShift](../../openshift/intro-openshift.md) 1.x 版的 Kubernetes 叢集啟用監視，以達到類似的監視體驗。
...
容器的 Azure 監視器支援監視 Azure Red Hat OpenShift v4，如[容器的 Azure 監視器總覽](container-insights-overview.md)中所述，但下列功能除外：
...
- 若要啟用及存取容器 Azure 監視器中的功能，您至少必須具備 Azure 訂用帳戶中的 Azure*參與者*角色，以及 log analytics 工作區中的[*log analytics 參與者*](../platform/manage-access.md#manage-access-using-azure-permissions)角色（設定容器的 Azure 監視器）。
...
## <a name="enable-monitoring-for-an-existing-cluster"></a>啟用現有叢集的監視

若要使用所提供的 Bash 腳本，針對部署在 Azure 中的 Azure Red Hat OpenShift 第4版或更新版本的叢集啟用監視，請執行下列動作：
...
1. 藉由執行下列命令，下載並儲存到本機資料夾，此腳本會使用監視增益集來設定您的叢集：
...
在本節中，您會使用您稍早下載的 Bash 腳本來啟用叢集的監視。 若要與現有的 Log Analytics 工作區整合，請從識別參數所需之 Log Analytics 工作區的完整資源識別碼開始，然後 `logAnalyticsWorkspaceResourceId` 執行命令以針對指定的工作區啟用監視增益集。

如果您沒有要指定的工作區，您可以跳至 [[與預設工作區整合](#integrate-with-the-default-workspace)] 區段，然後讓腳本為您建立新的工作區。
...
啟用監視之後，可能需要約15分鐘的時間，才能查看叢集的健全狀況計量。

### <a name="integrate-with-the-default-workspace"></a>與預設工作區整合

在本節中，您會使用您下載的 Bash 腳本來啟用 Azure Red Hat OpenShift v4. x 叢集的監視。

在此範例中，您不需要預先建立或指定現有的工作區。 此命令會在叢集訂用帳戶的預設資源群組中建立預設工作區，以簡化您的程式（如果區域中尚未存在）。

所建立的預設工作區的格式為*DefaultWorkspace- \<GUID> - \<Region> *。  
...
容器的 Azure 監視器中的多重叢集視圖會反白顯示未**在 [未受監視的**叢集] 索引標籤下啟用監視的 Azure Red Hat OpenShift 叢集。叢集旁的 [**啟用**] 選項不會從入口網站起始監視的上架。 系統會將您重新導向至本文，以依照本文稍早所述的步驟手動啟用監視。

1. 登入 [Azure 入口網站](https://portal.azure.com)。

1. 在左窗格或首頁上，選取 [ **Azure 監視器**]。

1. 在 [**深入**解析] 區段中，選取 [**容器**]。

1. 在 [**監視-容器**] 頁面上，選取 [未受**監視**的叢集]。

1. 在未受監視的叢集清單中，選取叢集，然後選取 [**啟用**]。

    您可以藉由尋找 [叢集**類型**] 資料行中的**ARO**值，識別清單中的結果。 選取 [**啟用**] 之後，系統會將您重新導向至這篇文章。

## <a name="next-steps"></a>後續步驟

- 既然您已啟用監視來收集 RedHat OpenShift 4.x 叢集的健康情況和資源使用率，以及在其上執行的工作負載，請瞭解[如何使用](container-insights-analyze.md)容器的 Azure 監視器。

- 根據預設，容器化代理程式會收集所有命名空間（kube 除外）中執行之所有容器的*stdout*和*stderr*容器記錄。 若要設定特定命名空間或命名空間專屬的容器記錄檔集合，請參閱[Container Insights 代理程式](container-insights-agent-config.md)設定，以設定您想要用於*ConfigMap*設定檔案的資料收集設定。
