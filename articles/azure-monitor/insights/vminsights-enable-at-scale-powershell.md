---
title: 使用 PowerShell 或範本啟用適用於 VM 的 Azure 監視器
...
本文說明如何使用 Azure PowerShell 或 Azure Resource Manager 範本來啟用 Azure 虛擬機器或虛擬機器擴展集的適用於 VM 的 Azure 監視器。 在此程式結束時，您將已成功開始監視所有虛擬機器，並瞭解是否有任何效能或可用性問題。
...
若要啟用多個 Vm 或虛擬機器擴展集的適用於 VM 的 Azure 監視器，請使用 PowerShell 腳本[Install-VMInsights.ps1](https://www.powershellgallery.com/packages/Install-VMInsights)。 它可以從 Azure PowerShell 資源庫取得。 此腳本會逐一查看：
