...
使用適用于容器的 Azure 監視器，視圖部署（預覽）功能會公開`kubeclt get deployments`和`kubectl describe deployment {your deployment}`命令，即時模擬 Kubernetes 部署物件的直接存取。 
...
部署（預覽）功能會對部署端點`/apis/apps/v1/deployments`執行一次（可重新整理）負載。 它可讓您選取指定的部署，並針對部署端點`/apis/apps/v1/namespaces/${nameSpace}/deployments/${deploymentName}`載入該特定部署的描述詳細資料。 
...
此視圖會顯示所有正在執行的部署清單，以及命名空間和其他詳細資訊，以模擬命令`kubectl get deployments –all-namespaces`的執行。 您可以選取任何一個資料行來排序結果。 

![部署屬性窗格詳細資料](./media/container-insights-livedata-deployments/deployment-properties-pane-details.png)

當您從清單中選取部署時，[屬性] 窗格會自動顯示在頁面的右側。 它會顯示您執行命令`kubectl describe deployment {deploymentName}`時所要查看之所選部署的相關資訊。 您可能已注意到描述資訊缺少一些詳細資料。 最值得注意的是**範本**遺失。 選取 [**原始**] 索引標籤可讓您流覽至未剖析的描述詳細資料。  
