---
title: 如何從 Azure 監視器查詢 VM 日誌
description: VM 的 Azure 監視器解決方案收集指標和記錄資料，本文介紹記錄並包含依例查詢。
ms.subservice: ''
ms.topic: conceptual
author: bwren
ms.author: bwren
...
ms.locfileid: "80283798"
---
# <a name="how-to-query-logs-from-azure-monitor-for-vms"></a>如何從適用於 VM 的 Azure 監視器查詢記錄

VM 的 Azure 監視器收集性能和連接指標、電腦和流程清單資料以及運行狀況狀態資訊，並將其轉發到 Azure 監視器中的日誌分析工作區。  此資料可用於 Azure 監視器中的[查詢](../../azure-monitor/log-query/log-query-overview.md)。 您可以將此資料套用至各種案例，包括移轉規劃、容量分析、探索和隨選效能疑難排解。
...
連接指標功能在 Azure 監視器日誌中引入了兩個新表 - VMConnect 和 VMBundPort。 這些表提供有關電腦（入站和出站）的連接以及打開/處於活動狀態的伺服器埠的資訊。 連接指標也通過 API 公開，這些 API 提供了在時間視窗期間獲取特定指標的方法。 偵聽通訊端上*接受*產生的 TCP 連接是入站連接，而通過*連接到*給定 IP 和埠創建的 TCP 連接是出站連接。 連線的方向會透過 Direction 屬性來表示，此屬性可設為 **inbound** 或 **outbound**。 

這些表中的記錄是從依賴項代理報告的資料生成的。 每個記錄表示 1 分鐘時間間隔的觀測值。 TimeGenerated 屬性表示時間間隔的開始時間。 每筆記錄均包含資訊來識別個別的實體 (也就是連線或連接埠)，以及與該實體相關聯的計量。 目前只會報告透過 IPv4 使用 TCP 而發生的網路活動。 

#### <a name="common-fields-and-conventions"></a>公共欄位和約定 

以下欄位和約定適用于 VMConnection 和 VMBundPort： 

- 電腦：報告機完全限定的功能變數名稱 
- 代理 Id：具有日誌分析代理的電腦的唯一識別碼  
- 電腦：服務映射公開的電腦的 Azure 資源管理器資源的名稱。 它是*m-{GUID}* 表單，其中*GUID*與 AgentId 相同  
- 進程：服務映射公開的進程的 Azure 資源管理器資源的名稱。 它是*p-x 字串表單中的。* 流程在機器範圍內是唯一的，用於跨電腦生成唯一的流程 ID，併合並機器和流程欄位。 
- 流程名稱：報告流程的可執行名稱。
...
4. 僅在特定介面上綁定的埠將 IsWildcardBind 設置為*False*。
...
電腦上主動接受傳入流量或可能接受流量，但在報告時間視窗期間處於空閒狀態的埠將寫入 VMBoundPort 表。  

VMBoundPort 中的每個記錄都由以下欄位標識： 
...
|連接埠 |埠號 |
|通訊協定 | 協定。  例如 *，tcp*或*udp（* 當前僅支援*tcp）。*|
 
埠的標識派生自上述五個欄位，並存儲在 PortId 屬性中。 此屬性可用於跨時間快速查找特定埠的記錄。 
...
- 若要減少詳細資訊和資料量，如果有具特定 IP 位址的相符記錄 (適用於相同的處理序、連接埠和通訊協定)，將略過具有萬用字元 IP 的記錄。 當省略萬用字元 IP 記錄時，具有特定 IP 位址的記錄的*IsWildcardBind*屬性將設置為*True*。  這表示埠在報告機的每個介面上公開。 
- 僅在特定介面上綁定的埠將 IsWildcardBind 設置為*False*。 

### <a name="vmcomputer-records"></a>虛擬機器記錄

具有*VMComputer*類型的記錄具有具有依賴項代理的伺服器的清單資料。 這些記錄具有下表中的屬性：
...
|Ipv4 位址 | IPv4 位址陣列 | 
|Ipv4 子網路遮罩 | IPv4 子網路遮罩陣列（順序與 Ipv4 位址相同）。 |
|Ipv4 預設閘道 | IPv4 閘道陣列 | 
|Ipv6 位址 | IPv6 位址的陣列 | 
|Mac 位址 | MAC 位址陣列 | 
|Dns名稱 | 與電腦關聯的 DNS 名稱陣列。 |
|依賴項代理版本 | 在電腦上運行的依賴項代理的版本。 | 
|作業系統系列 | *Linux*，*視窗* |
|作業系統全名 | 作業系統的完整名稱 | 
|實體記憶體MB | 以百萬位元組為單位的實體記憶體 | 
|Cpu | 處理器數目 | 
|Cpu速度 | CPU 速度 (MHz) | 
|虛擬機器類型 | *hyperv*， *vmware*， *xen* |
|虛擬機器本機 Id | VM 識別碼 (由其 Hypervisor 指派) | 
|虛擬機器本機名稱 | VM 的名稱 |
|虛擬機器虛擬機器虛擬機器管理Id | 託管 VM 的虛擬機器管理程式的唯一識別碼 |
|虛擬機器管理程式類型 | *超v* |
|虛擬機器管理 | 虛擬機器管理程式的唯一 ID | 
|託管供應商 | *蔚藍* |
|_ResourceId | Azure 資源的唯一識別碼 |
|AzureSubscriptionId | 標識訂閱的全球唯一識別碼 | 
|Azure 資源組 | 電腦是 其成員的 Azure 資源組的名稱。 |
|Azure 資源名稱 | Azure 資源的名稱 |
|Azure 定位 | Azure 資源的位置 |
|Azure 更新域 | Azure 更新域的名稱 |
|AzureFaultDomain | Azure 容錯域的名稱 |
|AzureVmId | Azure 虛擬機器的唯一識別碼 |
|Azure 大小 | Azure VM 的大小 |
|Azure 圖像發行者 | Azure VM 發行者的名稱 |
|Azure 圖像提供 | Azure VM 產品/服務類型的名稱 | 
|Azure 圖像庫 | Azure VM 映射的 SKU | 
|Azure 圖像版本 | Azure VM 映射的版本 | 
|Azure 雲服務名稱 | Azure 雲服務的名稱 |
|Azure 雲服務部署 | 雲服務的部署 ID |
|Azure 雲服務角色名稱 | 雲服務角色名稱 |
|Azure 雲服務角色類型 | 雲服務角色類型：*輔助角色*或*Web* |
|Azure 雲服務實例 Id | 雲服務角色實例 ID |
|AzureVmScaleSet 名稱 | 虛擬機器縮放集的名稱 |
|AzureVmScaleSet部署 | 虛擬機器規模集部署 ID |
|AzureVmScaleSet 資源 Id | 虛擬機器縮放集資源的唯一識別碼。|
|AzureVmScaleSetinstanceId | 虛擬機器規模集的唯一識別碼 |
|Azure 服務Fabric集群 ID | Azure 服務結構群集的唯一標識器 | 
|Azure 服務 Fabric 群集名稱 | Azure 服務結構群集的名稱 |

### <a name="vmprocess-records"></a>VMProcess 記錄

具有*VMProcess*類型的記錄具有具有依賴項代理的伺服器上 TCP 連接進程的清單資料。 這些記錄具有下表中的屬性：
...
|AgentId | 日誌分析代理的唯一 ID |
|電腦 | 服務映射公開的電腦的 Azure 資源管理器資源的名稱。 它是*m-{GUID}* 表單，其中*GUID*與 AgentId 相同。 | 
|Process | 服務映射過程的唯一識別碼。 它的形式是*p_GUID}*。 
|可執行名稱 | 處理序可執行檔的名稱 | 
|DisplayName | 進程顯示名稱 |
...
## <a name="sample-map-queries"></a>示例地圖查詢
...
### <a name="list-computer-name-dns-ip-and-os"></a>列出電腦名稱稱、DNS、IP 和作業系統
...
### <a name="number-of-open-ports-across-machines"></a>跨電腦打開的埠數
...
### <a name="score-processes-in-your-workspace-by-the-number-of-ports-they-have-open"></a>按工作區中打開的埠數對進程進行評分
...
## <a name="performance-records"></a>性能記錄
具有*InsightsMetrics*類型的記錄具有來自虛擬機器客體作業系統的效能資料。 這些記錄具有下表中的屬性：
...
|AgentId | 每台電腦代理的唯一識別碼 |
|類型 | *見解指標* |
|_資源 Id_ | 虛擬機器的資源識別碼 |

當前收集到*的見解指標*表中的效能計數器列在下表中：

| 命名空間 | 名稱 | 描述 | 單位 | Tags |
|:---|:---|:---|:---|:---|
| 電腦    | Heartbeat             | 電腦心跳                        | | |
| 記憶體      | 可用 MB           | 記憶體可用位元組                    | 位元組          | 記憶體大小MB - 總記憶體大小|
| 網路     | WriteBytesPerSecond   | 每秒網路寫入位元組數            | 每秒位元組 | 網路設備 Id - 設備的 ID<br>位元組 - 發送的位元組總數 |
| 網路     | ReadBytesPerSecond    | 每秒網路讀取位元組數             | 每秒位元組 | 網路設備 Id - 設備的 ID<br>位元組 - 接收的位元組總數 |
| 處理器   | 利用率百分比 | 處理器利用率百分比          | 百分比        | 總 CPU - 總 CPU |
| LogicalDisk | WritesPerSecond       | 每秒邏輯磁片寫入            | 每秒計數 | 安裝 Id - 設備的安裝 ID |
| LogicalDisk | 寫入延遲M        | 邏輯磁片寫入延遲毫秒    | 毫秒   | 安裝 Id - 設備的安裝 ID |
| LogicalDisk | WriteBytesPerSecond   | 邏輯磁片每秒寫入位元組數       | 每秒位元組 | 安裝 Id - 設備的安裝 ID |
| LogicalDisk | TransfersPerSecond    | 每秒邏輯磁片傳輸         | 每秒計數 | 安裝 Id - 設備的安裝 ID |
| LogicalDisk | 傳輸延遲M     | 邏輯磁片傳輸延遲毫秒 | 毫秒   | 安裝 Id - 設備的安裝 ID |
| LogicalDisk | ReadsPerSecond        | 每秒邏輯磁片讀取             | 每秒計數 | 安裝 Id - 設備的安裝 ID |
| LogicalDisk | 讀取延遲M         | 邏輯磁片讀取延遲毫秒     | 毫秒   | 安裝 Id - 設備的安裝 ID |
| LogicalDisk | ReadBytesPerSecond    | 邏輯磁片每秒讀取位元組數        | 每秒位元組 | 安裝 Id - 設備的安裝 ID |
| LogicalDisk | 可用空間百分比   | 邏輯磁片可用空間百分比        | 百分比        | 安裝 Id - 設備的安裝 ID |
| LogicalDisk | 自由空間MB           | 邏輯磁片可用空間位元組             | 位元組          | 安裝 Id - 設備的安裝 ID<br>磁片大小 - 總磁片大小 |
| LogicalDisk | 每秒位元組        | 每秒邏輯磁片位元組數             | 每秒位元組 | 安裝 Id - 設備的安裝 ID |
