...
- 結合 Azure 資源群組中的資源，以完整了解組成不同應用程式的所有資源。 [適用於資源群組的 Azure 監視器](./insights/resource-group-insights.md)提供了一種簡單的方式，來追蹤整個完整堆疊應用程式的健全狀況和效能，並可以向下切入個別元件以進行任何調查或偵錯。
...
- 在不同的部署環境 (如 Dev、Test，Canary 和 Prod) 之間[維護個別的監視執行個體](./app/separate-resources.md)。這可確保收集的資料與相關聯的應用程式和基礎結構相關。 如果需要跨環境將資料相互關聯，可以[在計量瀏覽器中使用多資源圖表](./platform/metrics-charts.md)，或[在 Azure 監視器中建立跨資源查詢](log-query/cross-workspace-query.md)。


## <a name="create-actionable-alerts-with-actions"></a>使用動作建立可採取動作的警示
監視的一個重要層面是主動向系統管理員通知任何目前的問題和已預測的問題。 

- 根據記錄和計量[在 Azure 監視器中建立警示](./platform/alerts-overview.md)，以識別可預測的失敗狀態。 您的目標應該是讓所有的警示可採取動作，這表示它們代表實際的重大情況，並設法減少誤判。 使用[動態閾值](platform/alerts-dynamic-thresholds.md)即可自動計算計量資料的基準，而不是定義您自己的靜態閾值。 
...
- 根據所收集的計量，使用[自動調整](./learn/tutorial-autoscale-performance-schedule.md)來動態增加和減少計算資源。
...
- 根據組織中不同角色的常用計量和記錄準備[自訂儀表板](./learn/tutorial-app-dashboards.md)。 儀表板可以結合來自所有 Azure 資源的資料。
- 準備[活頁簿](./platform/workbooks-overview.md)以確保開發與作業之間的知識共享。 這些可以準備為包含計量圖表和記錄查詢的動態報告，或者甚至作為開發人員準備的疑難排解指引，協助客戶支援或作業處理基本問題。
...
- 使用 Application Insights 中的工具[追蹤使用者行為和參與度](./learn/tutorial-users.md)。
- 使用[影響分析](./app/usage-impact.md)來協助您排定要關注哪些領域的優先順序，以推動重要的 KPI。
