...
本文提供如何使用 [Azure Resource Manager 範本](../../azure-resource-manager/templates/template-syntax.md)在 Azure 監視器中設定[計量警示規則](../platform/alerts-metric-near-real-time.md)的範例。 每個範例都包含範本檔案和參數檔案，且附有要提供給範本的範例值。
...
## <a name="single-criteria-static-threshold"></a>單一準則、靜態閾值
下列範例會使用單一準則和靜態閾值來建立計量警示規則。
...
## <a name="single-criteria-dynamic-threshold"></a>單一準則、動態閾值
下列範例會使用單一準則和動態閾值來建立計量警示規則。
...
## <a name="multiple-criteria-static-threshold"></a>多個準則、靜態閾值
計量警示支援多維度計量警示，而且每個警示規則可具有最多 5 個準則。 下列範例會建立維度計量的計量警示規則，並指定多個準則。

在包含多個準則的警示規則中使用維度時，會套用下列限制：
...
## <a name="multiple-dimensions-static-threshold"></a>多個維度、靜態閾值
單一警示規則可以一次監視多個計量時間序列，如此可管理較少的警示規則。 下列範例會建立維度計量的靜態計量警示規則。

在此範例中，警示規則會監視 **ResponseType** 的維度值組合，以及 **Transactions** 計量的 **ApiName** 維度：
...
## <a name="multiple-dimensions-dynamic-thresholds"></a>多個維度、動態閾值
單一動態閾值警示規則可一次為數百個計量時間序列 (甚至不同類型) 建立合適的閾值，讓需要管理的警示規則變少。 下列範例會建立維度計量的動態閾值計量量警示規則。
...
## <a name="custom-metric-static-threshold"></a>自訂計量、靜態閾值
...
針對存在於相同 Azure 區域中的資源，Azure 監視器支援使用單一計量警示規則來監視多個相同類型的資源。 目前只有 Azure 公用雲端才支援這項功能，而且僅適用於虛擬機器、SQL 伺服器資料庫、SQL 伺服器彈性集區和 Data Box Edge 裝置。 此外，這項功能僅適用於平台計量，且不支援自訂計量。
...
此範例會為一個或多個資源群組中的所有虛擬機器 (位於一個 Azure 區域)，建立可監視 CPU 百分比的動態閾值計量警示規則。
...
此範例會為一個訂用帳戶中的所有虛擬機器 (位於一個 Azure 區域)，建立可監視 CPU 百分比的靜態閾值計量警示規則。
...
此範例會為一個訂用帳戶中的所有虛擬機器 (位於一個 Azure 區域)，建立可監視 CPU 百分比的動態閾值計量警示規則。
...
此範例會為一個訂用帳戶中的一系列虛擬機器 (位於一個 Azure 區域)，建立可監視 CPU 百分比的靜態閾值計量警示規則。
...
此範例會為一個訂用帳戶中的一系列虛擬機器 (位於一個 Azure 區域)，建立可監視 CPU 百分比的動態閾值計量警示規則。
