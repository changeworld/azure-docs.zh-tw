...
若要查看您的工作內容 (您據以執行命令的訂用帳戶)，請使用下列命令：
...
> [!NOTE]
> `Get-AzLog` 只提供 15 天的歷程記錄。 使用 **-MaxRecords** 參數可讓您查詢 15 天以前的最後 N 個事件。 若要存取 15 天前的事件，請使用 REST API 或 SDK (使用 SDK 的 C# 範例)。 如果您未包含 **StartTime**，則預設值是 **EndTime** 減去一小時。 如果您未包含 **EndTime**，則預設值是目前的時間。 所有時間都是採用 UTC 格式。
> 
> 
