...
| 主機 | Hive 伺服器的 IP 位址或主機名稱，由";" 分隔多個主機（僅在啟用服務發現模式時）。  | 是 |
...
有關可用於定義資料集的節和屬性的完整清單，請參閱[資料集](concepts-datasets-linked-services.md)一文。 本節提供 Hive 資料集所支援的屬性清單。

若要從 Hive 複製資料，請將資料集的類型屬性設定為 **HiveObject**。 以下是支援的屬性：

| 屬性 | 描述 | 必要 |
|:--- |:--- |:--- |
| type | 資料集的類型屬性必須設置為 **：HiveObject** | 是 |
| 結構描述 | 結構描述的名稱。 |否 (如果已指定活動來源中的「查詢」)  |
| 資料表 | 資料表的名稱。 |否 (如果已指定活動來源中的「查詢」)  |
| tableName | 表的名稱，包括架構部件。 此屬性支援向後相容性。 對於新的工作負載，請使用`schema``table`和 。 | 否 (如果已指定活動來源中的「查詢」) |
...
若要從 Hive 複製資料，請將複製活動中的來源類型設定為 **HiveSource**。 複製活動**源**部分支援以下屬性：
