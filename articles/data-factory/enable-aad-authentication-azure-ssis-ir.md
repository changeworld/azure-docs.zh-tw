---
title: 為 Azure SSIS 集成運行時啟用 AAD
...
本文介紹如何使用 Azure 資料工廠 （ADF） 的託管標識啟用 Azure 活動目錄 （Azure AD） 身份驗證，並將其而不是傳統的身份驗證方法（如 SQL 身份驗證）用於：

- 創建 Azure-SSIS 集成運行時 （IR），該運行時將反過來代表您在 Azure SQL 資料庫伺服器/託管實例中預配 SSIS 目錄資料庫 （SSISDB）。

- 在 Azure-SSIS IR 上運行 SSIS 包時連接到各種 Azure 資源。

有關 ADF 的託管標識的詳細資訊，請參閱[資料工廠的託管標識](https://docs.microsoft.com/azure/data-factory/data-factory-service-identity)。
...
>-  如果您已經使用 SQL 身份驗證創建了 SSIS IR，則無法此時將其重新配置為通過 PowerShell 使用 Azure AD 身份驗證，但可以通過 Azure 門戶/ADF 應用執行此操作。 
...
3.  將 ADF 的受控識別新增至群組。 您可以按照["資料工廠託管標識"](https://docs.microsoft.com/azure/data-factory/data-factory-service-identity)一文獲取主託管標識物件識別碼（例如 765ad4ab-XXXX-XXXX-XXXX-XXXX-51ed985819dc，但不為此使用託管標識應用程式 ID）。
...
2. 在"**連接到伺服器"** 對話方塊中，在 **"伺服器名稱**"欄位中輸入 Azure SQL 資料庫伺服器名稱。

3. 在 **"身份驗證**"欄位中，選擇 **"活動目錄 - 通用"支援 MFA（** 您還可以使用其他兩種活動目錄身份驗證類型，請參閱[使用 SQL 配置和管理 Azure AD 身份驗證](https://docs.microsoft.com/azure/sql-database/sql-database-aad-authentication-configure)）。

4. 在 **"使用者名"** 欄位中，輸入您作為伺服器管理員設置的 Azure AD 帳戶的名稱，例如 。 testuser@xxxonline.com
...
10. 如果 SSISDB 是使用 SQL 身份驗證創建的，並且希望切換到使用 Azure-SSIS IR 的 Azure AD 身份驗證來訪問它，請按右鍵**SSISDB**資料庫並選擇 **"新建查詢**"。
...
2.  使用**系統管理員**的 SQL Server 帳戶連接到託管實例。 這是一個臨時限制，一旦 Azure SQL 資料庫託管實例的 Azure AD 伺服器主體（登錄）成為 GA，將刪除該限制。 如果您嘗試使用 Azure AD 管理員帳戶創建登錄名：Msg 15247、級別 16、狀態 1、行 1 使用者無權執行此操作，您將看到以下錯誤。

3.  在 [物件總管]**** 中，展開 [資料庫]****[系統資料庫] -> **** 資料夾。

4.  在 [master]**** 資料庫上按一下滑鼠右鍵，然後選取 [新增查詢]****。

5.  在查詢視窗中，執行以下 T-SQL 腳本以添加 ADF 作為使用者的託管標識
...
6.  如果 SSISDB 是使用 SQL 身份驗證創建的，並且希望切換到使用 Azure-SSIS IR 的 Azure AD 身份驗證來訪問它，請按右鍵**SSISDB**資料庫並選擇 **"新建查詢**"。
...
## <a name="run-ssis-packages-with-managed-identity-authentication"></a>使用託管標識身份驗證運行 SSIS 包

在 Azure-SSIS IR 上運行 SSIS 包時，可以使用託管標識身份驗證連接到各種 Azure 資源。 目前，我們已經在以下連線管理員中支援託管身份身份驗證。
...
- [ADO.NET連線管理員](https://docs.microsoft.com/sql/integration-services/connection-manager/ado-net-connection-manager#managed-identities-for-azure-resources-authentication)

- [Azure 存儲連線管理員](https://docs.microsoft.com/sql/integration-services/connection-manager/azure-storage-connection-manager#managed-identities-for-azure-resources-authentication)
