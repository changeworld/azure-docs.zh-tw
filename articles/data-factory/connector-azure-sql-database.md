---
title: 複製並轉換 Azure SQL 資料庫中的資料
description: 瞭解如何將資料複製到 Azure SQL 資料庫中,以及使用 Azure 數據工廠轉換 Azure SQL 資料庫中的數據。
...
- 使用 SQL 認證和 Azure 活動目錄 (Azure AD) 應用程式權杖身份驗證使用 Azure 資源的服務主體或託管識別複製資料。
- 作為源,使用 SQL 查詢或儲存過程檢索數據。
- 作為接收器,將資料追加到目標表或在複製期間使用自定義邏輯調用存儲過程。

>[!NOTE]
>此連接器現在不支援 Azure SQL 資料庫["始終加密](https://docs.microsoft.com/sql/relational-databases/security/encryption/always-encrypted-database-engine?view=azuresqldb-current)"。 為了解決這些問題,您可以通過自託管的整合時使用[通用 ODBC 連接器](connector-odbc.md)和 SQL Server ODBC 驅動程式。 有關 ODBC 驅動程式下載和連接字串配置,請按照[本指南](https://docs.microsoft.com/sql/connect/odbc/using-always-encrypted-with-the-odbc-driver?view=azuresqldb-current)操作。

> [!IMPORTANT]
> 如果使用 Azure 資料工廠整合執行時複製數據,請配置 Azure [SQL Server 防火牆](https://docs.microsoft.com/azure/sql-database/sql-database-firewall-configure),以便 Azure 服務可以存取伺服器。
> 如果使用自託管的整合執行時複製資料,請將 Azure SQL Server 防火牆配置為允許適當的 IP 範圍。 此範圍包括用於連接到 Azure SQL 資料庫的電腦 IP。

## <a name="get-started"></a>開始使用

[!INCLUDE [data-factory-v2-connector-get-started](../../includes/data-factory-v2-connector-get-started.md)]

以下各節提供有關用於定義特定於 Azure SQL 資料庫連接器的 Azure 數據工廠實體的屬性的詳細資訊。
...
| 屬性 | 描述 | 必要 |
|:--- |:--- |:--- |
| type | **類型**屬性必須設定為**AzureSqlDatabase**。 | 是 |
| connectionString | 指定連接到**連接String**屬性的 Azure SQL 資料庫實例所需的資訊。 <br/>您還可以在 Azure 密鑰保管庫中放置密碼或服務主體密鑰。 如果是 SQL 身份驗證,則`password`從連接 字串中拔出配置。 有關詳細資訊,請參閱[Azure 密鑰保管庫中表和存儲認證](store-credentials-in-key-vault.md)據之後的 JSON 範例。 | 是 |
| servicePrincipalId | 指定應用程式的用戶端識別碼。 | 是,當您使用具有服務主體的 Azure AD 身份驗證時 |
| servicePrincipalKey | 指定應用程式的金鑰。 將此欄位標記為**SecureString,** 以將安全性儲存在 Azure 資料工廠中,或[參考記憶體在 Azure 金鑰保存庫中的機密](store-credentials-in-key-vault.md)。 | 是,當您使用具有服務主體的 Azure AD 身份驗證時 |
| tenant | 指定應用程式所在的租戶資訊,如功能變數名稱或租戶 ID。 通過將滑鼠懸停在 Azure 門戶的右上角來檢索它。 | 是,當您使用具有服務主體的 Azure AD 身份驗證時 |
| connectVia | 用來連線到資料存放區的[整合執行階段](concepts-integration-runtime.md)。 如果數據存儲位於專用網路中,則可以使用 Azure 整合執行時或自託管整合時。 如果未指定,則使用預設的 Azure 整合執行時。 | 否 |

針對不同的驗證類型，請分別參閱下列有關先決條件和 JSON 範例的章節：

- [SQL 認證](#sql-authentication)
- [Azure AD 應用程式權杖身份驗證服務主體](#service-principal-authentication)
...
2. 如果尚未這樣做,則在 Azure 門戶上為 Azure SQL Server[預先定義 Azure 活動目錄管理員](../sql-database/sql-database-aad-authentication-configure.md#provision-an-azure-active-directory-administrator-for-your-azure-sql-database-server)。 Azure AD 系統管理員必須是 Azure AD 使用者或 Azure AD 群組，但不能是服務主體。 此步驟必須完成，如此您才可以在下一個步驟中使用 Azure AD 身分識別，為服務主體建立自主資料庫使用者。

3. 為服務主體[建立自主資料庫使用者](../sql-database/sql-database-aad-authentication-configure.md#create-contained-database-users-in-your-database-mapped-to-azure-ad-identities)。 使用 SQL 伺服器管理工作室等工具,使用至少具有 ALTER 任何用戶許可權的 Azure AD 標識,從或連接到要複製數據的資料庫。 執行下列 T-SQL： 
...
1. 如果尚未這樣做,則在 Azure 門戶上為 Azure SQL Server[預先定義 Azure 活動目錄管理員](../sql-database/sql-database-aad-authentication-configure.md#provision-an-azure-active-directory-administrator-for-your-azure-sql-database-server)。 Azure AD 管理員可以是 Azure AD 使用者或 Azure AD 組。 如果您授與受控識別系統管理員角色，請略過步驟 3 和 4。 管理員對資料庫具有完全訪問許可權。

2. 為 Azure 資料工廠管理辨識[器建立包含的資料庫使用者](../sql-database/sql-database-aad-authentication-configure.md#create-contained-database-users-in-your-database-mapped-to-azure-ad-identities)。 使用 SQL 伺服器管理工作室等工具,使用至少具有 ALTER 任何用戶許可權的 Azure AD 標識,從或連接到要複製數據的資料庫。 執行下列 T-SQL： 
  
    ```sql
    CREATE USER [your Data Factory name] FROM EXTERNAL PROVIDER;
    ```

3. 授予數據工廠託管標識所需的許可權,就像您通常對 SQL 使用者和其他人所做的一樣。 執行下列程式碼。 如需更多選項，請參閱[此文件](https://docs.microsoft.com/sql/relational-databases/system-stored-procedures/sp-addrolemember-transact-sql?view=sql-server-2017)。
...
| 屬性 | 描述 | 必要 |
|:--- |:--- |:--- |
| type | 資料集**的類型**屬性必須設定為**AzureSqlTable**。 | 是 |
...
要從 Azure SQL 資料庫複製資料,複製活動**來源**部分支援以下屬性:

| 屬性 | 描述 | 必要 |
|:--- |:--- |:--- |
| type | 必須複製活動來源**的類型**屬性設定為**AzureSqlSource**。 對於向後相容性,仍然支援"SqlSource"類型。 | 是 |
...
| storedProcedureParameters | 預存程序的參數。<br/>允許的值為名稱或值組。 參數的名稱和大小寫必須與存儲過程參數的名稱和大小寫匹配。 | 否 |
...
- 如果為**AzureSqlSource**指定**了 sqlReaderQuery,** 則複製活動將針對 Azure SQL 資料庫來源執行此查詢以獲取數據。 如果預存程序接受參數，您也可以藉由指定 **sqlReaderStoredProcedureName** 和 **storedProcedureParameters** 來指定預存程序。
- 如果不指定**sqlReaderQuery**或**sqlReader 儲存過程名稱**,則數據集 JSON 的「結構」部分中定義的列用於構造查詢。 查詢`select column1, column2 from mytable`針對 Azure SQL 資料庫運行。 如果資料集定義沒有 "structure"，則會從資料表中選取所有資料行。
...
> [!TIP]
> 詳細瞭解將[資料載入到 Azure SQL 資料庫的最佳實務](#best-practice-for-loading-data-into-azure-sql-database)中支援的寫入行為、配置和最佳實務。

要將資料複製到 Azure SQL 資料庫,複製活動**接收器**部分支援以下屬性:

| 屬性 | 描述 | 必要 |
|:--- |:--- |:--- |
| type | 必須複製活動接收器**的類型**屬性設定為**AzureSqlSink**。 對於向後相容性,仍然支援"SqlSink"類型。 | 是 |
| writeBatchSize | *要插入到 SQL 表中的行數。*<br/> 允許的值為**整數** (資料列數目)。 默認情況下,Azure 數據工廠會根據行大小動態確定適當的批處理大小。 | 否 |
...
| 禁用"指標收集" | 數據工廠收集 Azure SQL 資料庫 DTD 等指標,以便進行複製性能優化和建議。 如果您關注此行為,請指定`true`將其關閉。 | 否 (預設值為 `false`) |
...
## <a name="best-practice-for-loading-data-into-azure-sql-database"></a>將資料載入 Azure SQL 資料庫的最佳做法
...
請參閱有關如何在 Azure 數據工廠中配置的相應部分以及最佳做法。

### <a name="append-data"></a>附加資料

追加資料是此 Azure SQL 資料庫接收器連接器的默認行為。 Azure 數據工廠執行批量插入以高效地寫入表。 您可以在複製活動中相應地配置源和接收器。
...
例如,在 Azure 資料工廠中,您可以建立具有「**複製」 活動**與**儲存過程活動**連結的管道。 前者將源存儲中的數據複製到 Azure SQL 資料庫臨時表(例如 **,##UpsertTempTable)** 作為數據集中的表名。 然後,後者調用存儲過程將臨時表的源數據合併到目標表中並清理臨時表。
...
下列範例示範如何使用預存程序，對 Azure SQL Database 中的資料表執行 upsert。 假設輸入資料與接收器**行銷**表各有三列:**設定檔案 ID、****狀態**與**類別**。 根據**ProfileID**列執行 upsert,並且僅將其應用於名為"ProductA"的特定類別。
...
在映射資料流中轉換數據時,可以從 Azure SQL 資料庫讀取和寫入表。 關於詳細資訊,請參考映射資料串流的[來源轉換](data-flow-source.md)與[接收器轉換](data-flow-sink.md)。

### <a name="source-transformation"></a>來源轉換

特定於 Azure SQL 資料庫的設定可在源轉換的 **「源選項**」選項卡中提供。 
...
特定於 Azure SQL 資料庫的設定可在接收器轉換的 **「設定」** 選項卡中提供。
...
此處選擇作為鍵的列名稱將由 ADF 用作後續更新、upsert、刪除的一部分。 因此,必須選取「接收器」映射中存在的列。 如果不希望將值寫入此鍵列,請按下"跳過寫入鍵列"。
...
從 Azure SQL 資料庫複製資料或複製到 Azure SQL 資料庫時,將使用以下映射從 Azure SQL 資料庫數據類型到 Azure 數據工廠臨時資料類型。 若要了解複製活動如何將來源結構描述和資料類型對應至接收，請參閱[結構描述和資料類型對應](copy-activity-schema-and-type-mapping.md)。
...
>[!NOTE]
> 針對對應至 Decimal 過渡期類型的資料類型，Azure Data Factory 目前支援最多 28 個有效位數。 如果數據精度大於 28,請考慮在 SQL 查詢中轉換為字串。
...
有關 Azure 資料工廠複製活動支援為來源及接收器的資料儲存清單,請參考[資料儲存和格式](copy-activity-overview.md#supported-data-stores-and-formats)。
