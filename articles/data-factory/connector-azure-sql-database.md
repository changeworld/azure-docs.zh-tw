...
>[!NOTE]
> 此連接器目前不支援 Azure SQL Database [Always Encrypted](/sql/relational-databases/security/encryption/always-encrypted-database-engine) 。 若要解決這個問題，您可以透過自我裝載的整合執行時間，使用 [一般 odbc 連接器](connector-odbc.md) 和 SQL Server ODBC 驅動程式。 若要深入瞭解，請 [使用 Always Encrypted](#using-always-encrypted) 一節。 

> [!IMPORTANT]
> 如果您使用 Azure integration runtime 複製資料，請設定 [伺服器層級防火牆規則](../azure-sql/database/firewall-configure.md) ，讓 Azure 服務可以存取伺服器。
...
4. 如同您一般對 SQL 使用者或其他人所做的一樣，將所需的權限授與服務主體。 執行下列程式碼。 如需更多選項，請參閱[此文件](/sql/relational-databases/system-stored-procedures/sp-addrolemember-transact-sql)。
...
3. 依照您平常為 SQL 使用者和其他人所做的動作，授與 Data Factory 受控識別所需的許可權。 執行下列程式碼。 如需更多選項，請參閱[此文件](/sql/relational-databases/system-stored-procedures/sp-addrolemember-transact-sql)。
...
如需可用來定義資料集之區段和屬性的完整清單，請參閱 [資料集](./concepts-datasets-linked-services.md)。
...
**選項1：** 當您要複製大量資料時，可以使用複製活動將所有記錄大量載入至臨時表，然後執行預存程式活動，以在一次中套用 [MERGE](/sql/t-sql/statements/merge-transact-sql) 或 INSERT/UPDATE 語句。 
...
當您將資料複製到 Azure SQL Database 時，您也可以使用來源資料表的每個批次上的其他參數來設定及叫用使用者指定的預存程式。 預存程式功能會利用 [資料表值參數](/dotnet/framework/data/adonet/sql/table-valued-parameters)。
...
您可以將此處使用的索引鍵資料行參數化，以更新您的目標 Azure SQL Database 資料表。 如果您有多個資料行的複合索引鍵，請按一下 [自訂表格達式]，然後您就可以使用 ADF 資料流程運算式語言來加入動態內容，它可以包含一個字串陣列，其中包含複合索引鍵的資料行名稱。
...
寫入至 Azure SQL DB 時，某些資料列可能會因為目的地所設定的限制而失敗。 常見的錯誤包括：

*    資料表中的字串或二進位資料會被截斷
*    無法將 Null 值插入資料行
*    INSERT 語句與檢查條件約束衝突

根據預設，資料流程執行會在其取得的第一個錯誤時失敗。 您可以選擇 **在發生錯誤時繼續** 進行，即使個別的資料列發生錯誤也會讓您的資料流程完成。 Azure Data Factory 提供不同的選項供您處理這些錯誤資料列。

**交易認可：** 選擇您的資料是以單一交易或批次來撰寫。 單一交易會提供較糟的效能，但在交易完成之前，不會對其他人顯示任何寫入的資料。  

**輸出已拒絕的資料：** 啟用時，您可以將錯誤資料列輸出到 Azure Blob 儲存體中的 csv 檔案，或您選擇的 Azure Data Lake Storage Gen2 帳戶。 這會以三個額外的資料行來寫入錯誤資料列： SQL 作業（例如 INSERT 或 UPDATE）、資料流程錯誤碼，以及資料列上的錯誤訊息。

**報告發生錯誤時成功：** 啟用時，即使找到錯誤資料列，資料流程也會標示為成功。 

![處理資料列時發生錯誤](media/data-flow/sql-error-row-handling.png "處理資料列時發生錯誤")
...
當您從 [Always Encrypted](/sql/relational-databases/security/encryption/always-encrypted-database-engine)Azure SQL Database 複製資料時，請使用 [一般 ODBC 連接器](connector-odbc.md) ，並透過自我裝載的 Integration Runtime SQL Server ODBC 驅動程式。 此 Azure SQL Database 連接器目前不支援 Always Encrypted。 

具體而言：

1. 設定自我裝載的 Integration Runtime （如果沒有的話）。 如需詳細資訊，請參閱自我裝載的 [Integration Runtime](create-self-hosted-integration-runtime.md) 文章。

2. 從 [這裡](/sql/connect/odbc/download-odbc-driver-for-sql-server)下載適用于 SQL Server 的64位 ODBC 驅動程式，並在 Integration Runtime 電腦上安裝。 深入瞭解此驅動程式如何 [使用 Always Encrypted 搭配 ODBC driver for SQL Server](/sql/connect/odbc/using-always-encrypted-with-the-odbc-driver#using-the-azure-key-vault-provider)。
