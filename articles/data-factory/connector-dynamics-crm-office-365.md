...
本文概述如何使用 Azure Data Factory 中的「複製活動」，從 Microsoft Dynamics 365 或 Microsoft Dynamics CRM 複製資料以及複製資料至 Microsoft Dynamics 365 或 Microsoft Dynamics CRM。 它基於["複製活動概述](copy-activity-overview.md)"一文，其仲介紹了複製活動的一般概述。
...
此 Dynamics 連接器支援適用于連線或本地的 Dynamics 版本 7.x 到 9.x。 具體而言：

- 版本 7.x 映射到動態 CRM 2015
...
| 搭配 IFD 的 Dynamics 365 內部部署版 <br> 搭配 IFD 的 Dynamics CRM 2016 內部部署版 <br> 搭配 IFD 的 Dynamics CRM 2015 內部部署版 | IFD | [與 IFD 和 IFD auth 的本地動態](#dynamics-365-and-dynamics-crm-on-premises-with-ifd) |
...
| 屬性 | 描述 | 必要 |
|:--- |:--- |:--- |
| type | 類型屬性必須設置為**動態**、**動態Crm**或**通用資料服務為應用**。 | 是 |
...
| servicePrincipalId | 指定 Azure Active Directory 應用程式的用戶端識別碼。 | 使用`AADServicePrincipal`身份驗證時是 |
| 服務主體證書類型 | 指定要用於服務主體驗證的認證類型。 允許的值是：**服務主金鑰**或服務**主體證書**。 | 使用`AADServicePrincipal`身份驗證時是 |
| 服務主體證書 | 指定服務主體憑據。 <br>當用作`ServicePrincipalKey`憑據類型時，`servicePrincipalCredential`可以是字串（ADF 將在連結的服務部署時對其進行加密）或對 AKV 中機密的引用。 <br>用作`ServicePrincipalCert`憑據時，`servicePrincipalCredential`應引用 AKV 中的證書。 | 使用`AADServicePrincipal`身份驗證時是 | 
| username | 指定要連線到 Dynamics 的使用者名稱。 | 使用`Office365`身份驗證時是 |
| 密碼 | 指定您為 username 指定之使用者帳戶的密碼。 將此欄位標記為 SecureString，將它安全地儲存在 Data Factory 中，或[參考 Azure Key Vault 中儲存的祕密](store-credentials-in-key-vault.md)。 | 使用`Office365`身份驗證時是 |
...
| connectVia | 用於連接到資料存儲的[集成運行時](concepts-integration-runtime.md)。 如果未指定，就會使用預設的 Azure Integration Runtime。 | 否 (來源)；是 (接收) |
...
要從 Dynamics 複製資料，請支援以下屬性。

| 屬性 | 描述 | 必要 |
|:--- |:--- |:--- |
| type | 資料集的類型屬性必須設置為**動態實體**、**動態CrmEntity**或**通用資料服務為AppsEntity**。 |是 |
...
要從 Dynamics 複製資料，複製活動**源**部分中支援以下屬性。

| 屬性 | 描述 | 必要 |
|:--- |:--- |:--- |
| type | 複製活動源的類型屬性必須設置為**動態源**、**動態CrmSource**或**通用資料服務ForAppsSource。** | 是 |
| 查詢 | FetchXML 是在 Dynamics (線上版和內部部署版) 中使用的專屬查詢語言。 請參閱下列範例。 要瞭解更多資訊，請參閱[使用 FetchXML 生成查詢](https://msdn.microsoft.com/library/gg328332.aspx)。 | 否 (如果已指定資料集中的 "entityName") |

>[!NOTE]
>即使您在 FetchXML 查詢中設定的資料行投影未包含 PK 資料行，還是一律會複製 PK 資料行。

> [!IMPORTANT]
>- 從 Dynamics 複製資料時，從 Dynamics 到接收器的顯式列映射是可選的，但高度重排序，以確保確定性複製結果。
>- 在創作 UI 時，ADF 通過從 Dynamics 查詢結果中採樣頂部行來初始化源列清單來推斷架構，在這種情況下，將省略頂部行中沒有值的列。 如果沒有顯式映射，則相同的行為適用于複製執行。 您可以查看映射並將更多列添加到映射中，這將在複製運行時得到遵守。
...
| 屬性 | 描述 | 必要 |
|:--- |:--- |:--- |
| type | 複製活動接收器的類型屬性必須設置為**動態Sink、****動態CrmSink**或**通用資料服務ForAppsSink**。 | 是 |
...
>[!NOTE]
>"**寫入BatchSize"** 的接收器的預設值和 Dynamics 接收器的複製活動"**[並行副本](copy-activity-performance.md#parallel-copy)**"均為 10。 因此，100 個記錄會同時提交到 Dynamics。
...
**"writeBatchSize"** 和"**並行副本**"的最佳組合取決於實體的架構，例如列數、行大小、連接到這些調用的外掛程式/工作流/工作流活動的數量等。根據 Dynamics 服務，10 個 writeBatchSize = 10 並行副本的預設設置是建議，對於大多數 Dynamics 實體來說，這適用于大多數 Dynamics 實體，但可能不是最佳性能。 您可以藉由調整複製活動設定中的組合來微調效能。
...
> [!NOTE]
> 不支援動態資料類型屬性類型屬性類型.日曆規則、屬性類型、多選擇挑選清單和屬性類型。党清單不受支援。
