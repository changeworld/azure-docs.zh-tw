...
Azure 函式的傳回型別必須是有效的 `JObject`。 （請記住， [JArray](https://www.newtonsoft.com/json/help/html/T_Newtonsoft_Json_Linq_JArray.htm) *不*是`JObject`。）以外的任何傳回型`JObject`別都會失敗，而且會引發使用者錯誤*回應內容，不是有效的 JObject*。
...
| 函數名稱  | 此活動會在 Azure 函式應用程式中呼叫的函式名稱 | 字串 | 是 |
...
Azure 函式活動也支援**查詢**。 查詢必須包含在中`functionName`。 例如，當函`HttpTriggerCSharp`式名稱是，而您想要包含的查詢是`name=hello`時，您可以`functionName`在 Azure 函式活動中將當做建立`HttpTriggerCSharp?name=hello`為。 此函式可以參數化，因此可以在執行時間決定值。
