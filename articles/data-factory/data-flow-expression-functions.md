...
___
### <code>fromUTC</code>
<code><b>fromUTC(<i>&lt;value1&gt;</i> : timestamp, [<i>&lt;value2&gt;</i> : string]) => timestamp</b></code><br/><br/> 會轉換成 UTC 格式的時間戳記。 您可以用 'GMT'、'PST'、'UTC'、'America/Cayman' 的形式選擇性傳遞時區。 它預設為目前 timezo mpleDateFormat 的類別，以 ne. Refer Java's `S i 取得可用的格式。 https://docs.oracle.com/javase/8/docs/api/java/text/SimpleDateFormat.html.  
...
___
### <code>hour</code>
<code><b>hour(<i>&lt;value1&gt;</i> : timestamp, [<i>&lt;value2&gt;</i> : string]) => integer</b></code><br/><br/> 取得時間戳記的小時值。 您可以用 'GMT'、'PST'、'UTC'、'America/Cayman' 的格式傳遞選擇性的時區。 本地時區是用 lt. Refer Java's `S i 來做為可用格式的 & l mpleDateFormat」類別。 https://docs.oracle.com/javase/8/docs/api/java/text/SimpleDateFormat.html.  
...
<code><b>millisecond(<i>&lt;value1&gt;</i> : timestamp, [<i>&lt;value2&gt;</i> : string]) => integer</b></code><br/><br/> 取得日期的毫秒值。 您可以用 'GMT'、'PST'、'UTC'、'America/Cayman' 的格式傳遞選擇性的時區。 本地時區是用 lt. Refer Java's `S i 來做為可用格式的 & l mpleDateFormat」類別。 https://docs.oracle.com/javase/8/docs/api/java/text/SimpleDateFormat.html.  
...
___
### <code>minus</code>
<code><b>minus(<i>&lt;value1&gt;</i> : any, <i>&lt;value2&gt;</i> : any) => any</b></code><br/><br/> 減去數位。 從日期減去天數。 從時間戳記減去持續時間。 減去兩個時間戳記來取得差異 (以毫秒為單位)。 與相同 the - operator.  
* ``minus(20, 10) -> 10``  
* ``20 - 10 -> 10``  
...
___
### <code>minute</code>
<code><b>minute(<i>&lt;value1&gt;</i> : timestamp, [<i>&lt;value2&gt;</i> : string]) => integer</b></code><br/><br/> 取得時間戳記的分鐘值。 您可以用 'GMT'、'PST'、'UTC'、'America/Cayman' 的格式傳遞選擇性的時區。 本地時區是用 lt. Refer Java's `S i 來做為可用格式的 & l mpleDateFormat」類別。 https://docs.oracle.com/javase/8/docs/api/java/text/SimpleDateFormat.html.  
...
___
### <code>normalize</code>
<code><b>normalize(<i>&lt;String to normalize&gt;</i> : string) => string</b></程式碼><br/><br/> 將字串值正規化以分隔重音符號code characters.  
...
___
### <code>null</code>
<code><b>null() => null</b></程式碼><br/><br/> 會傳回 Null 值。 Use the function `如果有一個名為 ' null ' 的資料行，則語法 (null ( # A2 # B3 '。 使用的任何作業將會 r esult in a NULL.  
...
___
### <code>second</code>
<code><b>second(<i>&lt;value1&gt;</i> : timestamp, [<i>&lt;value2&gt;</i> : string]) => integer</b></code><br/><br/> 取得日期的第二個值。 您可以用 'GMT'、'PST'、'UTC'、'America/Cayman' 的格式傳遞選擇性的時區。 本地時區是用 lt. Refer Java's `S i 來做為可用格式的 & l mpleDateFormat」類別。 https://docs.oracle.com/javase/8/docs/api/java/text/SimpleDateFormat.html.  
...
___
### <code>subDays</code>
<code><b>subDays(<i>&lt;date/timestamp&gt;</i> : datetime, <i>&lt;days to subtract&gt;</i> : integral) => datetime</b></code><br/><br/> 從日期或時間戳記減去天數。 與-op 相同erator for date.  
...
___
### <code>toDate</code>
<code><b>toDate(<i>&lt;string&gt;</i> : any, [<i>&lt;date format&gt;</i> : string]) => date</b></程式碼><br/><br/> 將輸入日期字串轉換為日期，並使用選擇性的輸入日期表單 at. Refer Java's `S i mpleDateFormat ' 類別來取得可用的格式。 如果省略輸入日期格式，則預設格式為 yyyy-[M]M-[d]d。 接受的格式為： [yyyy，yyyy-[M] M，yyyy-[M] M-[d] d，yyyy-[M]M-[d]dT* ].  
...
<code><b>toTimestamp(<i>&lt;string&gt;</i> : any, [<i>&lt;timestamp format&gt;</i> : string], [<i>&lt;time zone&gt;</i> : string]) => timestamp</b></code><br/><br/> 會將字串轉換為指定的時間戳記格式的時間戳記。 如果省略時間戳記，則會使用預設模式 yyyy-[M] M-[d] d hh： mm： ss [f. ...]。 您可以用 'GMT'、'PST'、'UTC'、'America/Cayman' 的格式傳遞選擇性的時區。 時間戳記支援最高毫秒的精確度，而且值為 9 99. Refer Java's `S i mpleDateFormat ' 類別可用的格式。 https://docs.oracle.com/javase/8/docs/api/java/text/SimpleDateFormat.html.  
...
___
### <code>toUTC</code>
<code><b>toUTC(<i>&lt;value1&gt;</i> : timestamp, [<i>&lt;value2&gt;</i> : string]) => timestamp</b></code><br/><br/> 會將時間戳記轉換為 UTC。 您可以用 'GMT'、'PST'、'UTC'、'America/Cayman' 的格式傳遞選擇性的時區。 它預設為目前 timezo mpleDateFormat 的類別，以 ne. Refer Java's `S i 取得可用的格式。 https://docs.oracle.com/javase/8/docs/api/java/text/SimpleDateFormat.html.  
...
___
### <code>true</code>
<code><b>true() => boolean</b></code><br/><br/> 一律會傳回 true 值。 Use the function `語法 (true ( # A2 # B3 ' （如果有資料行）mn named 'true'.  
* ``(10 + 20 == 30) -> true``  
* ``(10 + 20 == 30) -> true()``  
___
### <code>typeMatch</code>
