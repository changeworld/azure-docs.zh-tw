---
title: 從銷售人員服務雲複製資料
description: 瞭解如何使用資料工廠管道中的複製活動將數據從 Salesforce 服務雲端複製到受支援的接收器數據存儲或受支援的源數據儲存到 Salesforce 服務雲端。
...
ms.locfileid: "81416664"
---
# <a name="copy-data-from-and-to-salesforce-service-cloud-by-using-azure-data-factory"></a>使用 Azure 資料工廠從 Salesforce 服務雲端複製資料
[!INCLUDE[appliesto-adf-asa-md](includes/appliesto-adf-asa-md.md)]

本文概述了如何使用 Azure 數據工廠中的複製活動從 Salesforce 服務雲複製數據。 本文是根據[複製活動概觀](copy-activity-overview.md)一文，該文提供複製活動的一般概觀。

## <a name="supported-capabilities"></a>支援的功能

此 Salesforce 服務雲端連線器支援用於以下活動:

- 使用[支援的來源/接收器矩陣](copy-activity-overview.md)[複製活動](copy-activity-overview.md)
- [尋找活動](control-flow-lookup-activity.md)

您可以將資料從 Salesforce 服務雲端複製到任何受支援的接收體資料儲存。 您還可以將數據從任何受支援的源數據儲存複製到 Salesforce 服務雲端。 有關"複製"活動支援為源或接收體的數據儲存的清單,請參閱[支持數據存儲](copy-activity-overview.md#supported-data-stores-and-formats)表。
...
Salesforce 連接器建置在 Salesforce REST/Bulk API 之上。 預設情況下,連接器使用[v45](https://developer.salesforce.com/docs/atlas.en-us.218.0.api_rest.meta/api_rest/dome_versions.htm)從 Salesforce 複製數據,並使用[v40](https://developer.salesforce.com/docs/atlas.en-us.208.0.api_asynch.meta/api_asynch/asynch_api_intro.htm)將數據複製到 Salesforce。 您還可以顯示式設定用於透過[`apiVersion`](#linked-service-properties)連結服務中的屬性讀取/寫入資料的 API 版本。
...
以下各節提供有關用於定義特定於 Salesforce 服務雲端連接器的資料工廠實體的屬性的詳細資訊。
...
| 屬性 | 描述 | 必要 |
|:--- |:--- |:--- |
| type |類型屬性必須設定為**SalesforceServiceCloud**。 |是 |
...
| securityToken |指定使用者帳戶的安全性權杖。 <br/><br/>若要整體了解安全性權杖，請參閱[安全性和 API](https://developer.salesforce.com/docs/atlas.en-us.api.meta/api/sforce_api_concepts_security.htm)。 僅當將整合式執行時的 IP 添加到 Salesforce 上的[受信任 IP 位址清單](https://developer.salesforce.com/docs/atlas.en-us.securityImplGuide.meta/securityImplGuide/security_networkaccess.htm)時,才能跳過安全權杖。 使用 Azure IR 時,請參考[Azure 整合執行時 IP 位址](azure-integration-runtime-ip-addresses.md)。<br/><br/>有關如何獲取和重置安全權杖的說明,請參閱[獲取安全權杖](https://help.salesforce.com/apex/HTViewHelpDoc?id=user_security_token.htm)。 將此欄位標記為 SecureString，將它安全地儲存在 Data Factory 中，或[參考 Azure Key Vault 中儲存的祕密](store-credentials-in-key-vault.md)。 |否 |
| apiVersion | 指定要使用的 Salesforce REST/Bulk API`48.0`版本,例如 。 預設情況下,連接器使用[v45](https://developer.salesforce.com/docs/atlas.en-us.218.0.api_rest.meta/api_rest/dome_versions.htm)從 Salesforce 複製數據,並使用[v40](https://developer.salesforce.com/docs/atlas.en-us.208.0.api_asynch.meta/api_asynch/asynch_api_intro.htm)將數據複製到 Salesforce。 | 否 |
...
>[!IMPORTANT]
>將資料複製到 Salesforce 服務雲端時,預設的 Azure 整合執行時不能用於執行複製。 換句話說,如果源連結的服務沒有指定的整合執行時,請顯式創建具有 Salesforce 服務雲端服務實例附近位置的[Azure 整合執行時](create-azure-integration-runtime.md#create-azure-ir)。 按以下範例關聯 Salesforce 服務雲端連結服務。
...
如需可用來定義資料集的區段和屬性完整清單，請參閱[資料集](concepts-datasets-linked-services.md)一文。 本節提供 Salesforce 服務雲數據集支援的屬性清單。

要將數據從 Salesforce 服務雲複製,請支援以下屬性。

| 屬性 | 描述 | 必要 |
|:--- |:--- |:--- |
| type | 類型屬性必須設定為**SalesforceServiceCloudObject**。  | 是 |
...
如需可用來定義活動的區段和屬性完整清單，請參閱[管線](concepts-pipelines-activities.md)一文。 本節提供 Salesforce 服務雲端源和接收器支援的屬性清單。

### <a name="salesforce-service-cloud-as-a-source-type"></a>銷售隊伍服務雲作為源類型

要從 Salesforce 服務雲端複製數據,複製活動**來源**部分中支援以下屬性。

| 屬性 | 描述 | 必要 |
|:--- |:--- |:--- |
| type | 複製活動來源類型屬性必須設定為**SalesforceServiceCloudSource**。 | 是 |
| 查詢 |使用自訂查詢來讀取資料。 您可以使用 [Salesforce 物件查詢語言 (SOQL)](https://developer.salesforce.com/docs/atlas.en-us.soql_sosl.meta/soql_sosl/sforce_api_calls_soql.htm) 查詢或 SQL-92 查詢。 請參閱[查詢秘訣](#query-tips)一節中的秘訣。 如果未指定查詢,將檢索資料集中「objectApiName」中指定的 Salesforce 服務雲端服務物件的所有資料。 | 否 (如果在資料集中指定 "objectApiName") |
...
要將資料複製到 Salesforce 服務雲端,複製活動**接收器**部分支援以下屬性。

| 屬性 | 描述 | 必要 |
|:--- |:--- |:--- |
| type | 複製活動接收器的類型屬性必須設定為**SalesforceServiceCloudSink**。 | 是 |
| writeBehavior | 作業的寫入行為。<br/>允許的值為 **Insert** 和 **Upsert**。 | 否 (預設為 Insert) |
| externalIdFieldName | upsert 作業的外部識別碼欄位名稱。 指定欄位必須在 Salesforce 服務雲端物件中定義為「外部 ID 欄位」。 對應的輸入資料中不能有 NULL 值。 | 是 (用於 upsert) |
| writeBatchSize | 每個批次中寫入 Salesforce 服務雲端的數據的行計數。 | 否 (預設值為 5,000) |
...
通過將查詢指定為`{call "<report name>"}`,可以從 Salesforce 服務雲報表中檢索數據。 例如 `"query": "{call \"TestReport\"}"`。

### <a name="retrieve-deleted-records-from-the-salesforce-service-cloud-recycle-bin"></a>從 Salesforce 服務雲回收站檢索已刪除的記錄

要從 Salesforce 服務雲回收站查詢已刪除的軟紀錄,`readBehavior`可以`queryAll`指定為 。 

### <a name="difference-between-soql-and-sql-query-syntax"></a>SOQL 和 SQL 查詢語法的差異

從 Salesforce 服務雲端複製數據時,可以使用 SOQL 查詢或 SQL 查詢。 請注意，這兩個查詢具有不同的語法和功能支援，不可混用。 建議您使用 Salesforce 服務雲端本機支援的 SOQL 查詢。 下表列出主要差異：
...
如果遇到"MALFORMED_QUERY:截斷"的錯誤,通常是由於數據中具有 JunctionIdList 類型列,Salesforce 在支援大量行中支援此類數據有限制。 要緩解,請嘗試排除 JunctionIdList 列或限制要複製的行數(您可以將其分區為多個複製活動運行)。

## <a name="data-type-mapping-for-salesforce-service-cloud"></a>銷售人員服務雲的資料類型對應

從 Salesforce 服務雲複製數據時,將使用以下映射從 Salesforce 服務雲端資料類型到資料工廠臨時資料類型。 若要了解複製活動如何將來源結構描述和資料類型對應至接收，請參閱[結構描述和資料類型對應](copy-activity-schema-and-type-mapping.md)。
