...
此範本需要源資料庫的架構包含時間戳記資料行或遞增索引鍵，以識別新的或更新的資料列。

>[!NOTE]
> 如果您的源資料庫中有時間戳記資料行，以識別新的或更新的資料列，但您不想要建立用於 delta 複製的外部控制資料表，則可以改用 [Azure Data Factory 資料複製工具](copy-data-tool.md) 來取得管線。 該工具會使用觸發程式排程的時間做為變數，以從源資料庫讀取新的資料列。

## <a name="about-this-solution-template"></a>關於此解決方案範本

此範本會先抓取舊的浮水印值，並將它與目前的水位線值進行比較。 之後，它會根據兩個水位線值之間的比較，只複製源資料庫的變更。 最後，它會將新的高水位線值儲存至外部控制資料表，以在下次進行差異資料載入時使用。

範本包含四個活動：
- **Lookup** 會抓取舊的高水位線值（儲存在外部控制資料表中）。
- 另一個 **查閱** 活動會從源資料庫抓取目前的高水位線值。
- **複製** 只會將源資料庫的變更複製到目的地存放區。 識別源資料庫中之變更的查詢類似于「SELECT * FROM Data_Source_Table，其中 TIMESTAMP_Column > 「上次高水位線」和 TIMESTAMP_Column <= 「目前的高水位線」」。
- **SqlServerStoredProcedure** 會將目前的高水位線值寫入至外部控制資料表，以在下次進行 delta 複製時使用。
...
2. 在 SQL Server 或 Azure SQL Database 中建立控制資料表，以儲存用於差異資料載入的高水位線值。 在下列範例中，控制資料表的名稱是 *watermarktable*。 在此表中， *WatermarkValue* 是儲存高水位線值的資料行，且其類型為 *datetime*。
...
3. 在您用來建立控制資料表的相同 SQL Server 或 Azure SQL Database 實例中建立預存程式。 預存程式是用來將新的高水位線值寫入至外部控制資料表，以在下次進行差異資料載入時使用。
...
13. 您可以在您的來源資料表中建立新的資料列。 以下是用來建立新資料列的 SQL 語言範例：
