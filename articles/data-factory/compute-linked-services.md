...
| [Azure Batch](#azure-batch-linked-service)                   | [自 定義](transform-data-using-dotnet-custom-activity.md)     |
...
| [Azure 機器學習](#azure-machine-learning-linked-service) | [Azure 機器學習執行管道](transform-data-machine-learning-service.md) |
| [Azure 機器學習](#azure-machine-learning-linked-service) | [Azure 機器學習執行管道](transform-data-machine-learning-service.md) |
...
> [!IMPORTANT]
> HDInsight 群集在 JSON（**連結服務名稱**）中指定的 blob 存儲中創建**預設容器**。 HDInsight 不會在刪除叢集時刪除此容器。 這是設計的行為。 在使用 HDInsight 隨選連結服務時，除非有現有的即時叢集 (**timeToLive**)，否則每當需要處理配量時，就會建立 HDInsight 叢集，並在處理完成時予以刪除。 
>
...
| clusterSshUserName         | SSH 的使用者名遠端連線到群集的節點（對於 Linux）。 | 否       |
...
| connectVia        | 將活動分派到此連結服務所用的整合執行階段。 您可以使用 Azure 整合執行階段或自我裝載整合執行階段。 如果未指定，就會使用預設的 Azure Integration Runtime。 <br />對於企業安全包 （ESP） 啟用的 HDInsight 群集，使用自託管集成運行時，該運行時對群集具有視線，或者應將其部署在同一虛擬網路中的 ESP HDInsight 群集。 | 否       |
...
創建 Azure 機器學習工作室連結服務，以將機器學習批次處理評分終結點註冊到資料工廠。
...
| updateResourceEndpoint | 用於使用已訓練的模型檔更新預測 Web 服務的 Azure 機器學習 Web 服務終結點的更新資源 URL | 否                                       |
...
創建 Azure 機器學習連結服務以將 Azure 機器學習工作區連接到資料工廠。
...
您可以創建**Azure 資料磚塊連結服務**來註冊資料磚工作區，用於運行 DataBRICKS 工作負荷（筆記本、jar、python）。 
> [!IMPORTANT]
> 資料磚連結的服務支援[實例池](https://aka.ms/instance-pools)。 
...
| newClusterNodeType   | 此欄位透過單一值，將對此叢集中每個 Spark 節點可使用的資源編碼。 例如，可以為記憶體或計算密集型工作負載預配和優化 Spark 節點。 新群集需要此欄位                | 否               |
...
| 新的集群腳本| 新群集的一組可選的使用者定義初始化腳本。 指定到 init 腳本的 DBFS 路徑。 | 否  |
...
創建 Azure 函數連結服務並將其與 Azure[函式活動](control-flow-azure-function-activity.md)一起使用，以在資料工廠管道中運行 Azure 函數。 Azure 函式的傳回型別必須是有效的 `JObject`。 （請記住[，JArray](https://www.newtonsoft.com/json/help/html/T_Newtonsoft_Json_Linq_JArray.htm) *不是* `JObject`. ）任何返回類型，除了`JObject`失敗和引發使用者錯誤*回應內容不是有效的JObject。*
