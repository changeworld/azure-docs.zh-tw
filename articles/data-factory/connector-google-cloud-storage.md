...
本文概述了如何從 Google 雲存儲複製數據。 若要了解 Azure Data Factory，請閱讀[簡介文章](introduction.md)。

## <a name="supported-capabilities"></a>支援的功能

以下活動支援此 Google 雲端儲存連線器:
...
您的 Google 雲儲存帳戶需要設定以下設定:

1. 為您的 Google 雲儲存帳戶啟用互通性
2. 設定包含要複製資料的預設項目
3. 創建訪問金鑰。

![檢索 Google 雲儲存的存取金鑰](media/connector-google-cloud-storage/google-storage-cloud-settings.png)

## <a name="required-permissions"></a>所需的權限

若要從 Google Cloud Storage 複製資料，請確定您已獲得下列權限︰

- **對於複製活動執行:**`s3:GetObject``s3:GetObjectVersion`和 物件操作。
- **對於資料工廠 GUI**創作`s3:ListAllMyBuckets``s3:ListBucket`/`s3:GetBucketLocation`, 和儲存桶操作操作需要額外的操作,如測試連接和瀏覽/導航檔路徑。 如果您不想要授與這些權限，請跳過已連結的服務建立頁面中的測試連線，然後直接在資料集設定中指定路徑。
...
| 屬性 | 描述 | 必要 |
|:--- |:--- |:--- |
| type | 類型屬性必須設定為**GoogleCloud 儲存**。 | 是 |
| accessKeyId | 密碼存取金鑰的識別碼。 要尋找存取金鑰和機密,請參閱[先決條件](#prerequisites)。 |是 |
...
| serviceUrl | 將自訂 S3**`https://storage.googleapis.com`** 終結點 指定為 。 | 是 |
...
在基於格式的資料集設定`location`下 ,Google 雲端儲存支援以下屬性:
...
在基於格式的副本源中的設定下`storeSettings`,Google 雲端儲存支援以下屬性:
...
| prefix                   | 在數據集中配置的給定存儲桶下的 S3 物件鍵的前置碼,以篩選源物件。 系統會選取索引鍵以此前置詞開頭的物件。 僅適用於未指定`wildcardFolderPath``wildcardFileName`和 屬性時。 |                                                             |
