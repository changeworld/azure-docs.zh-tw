...
- **針對複製活動執行：**： `s3:GetObject`以及`s3:GetObjectVersion`物件作業。
- **針對 Data Factory GUI 撰寫**： `s3:ListAllMyBuckets`針對`s3:ListBucket` / `s3:GetBucketLocation` [測試連線] 和 [流覽/流覽檔案路徑] 之類的作業，另外還需要「值區作業」許可權的和。 如果您不想要授與這些權限，請跳過已連結的服務建立頁面中的測試連線，然後直接在資料集設定中指定路徑。
...
| serviceUrl | 將自訂 S3 端點指定**`https://storage.googleapis.com`** 為。 | 是 |
...
下列屬性在以格式為基礎之資料集`location`的設定下支援 Google 雲端儲存體：
...
下列屬性在以格式為基礎之複製來源`storeSettings`的設定下支援 Google 雲端儲存體：
