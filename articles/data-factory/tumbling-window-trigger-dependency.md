...
![相依項目建立](media/tumbling-window-trigger-dependency/tumbling-window-dependency01.png "相依項目建立")
...
| **屬性名稱** | **說明**  | **型別** | **必要** |
|---|---|---|---|
| type  | 所有現有的輪轉視窗觸發程序都會顯示在此下拉式清單中。 選擇要採取相依性的觸發程序。  | 翻捲視窗觸發器相依引用或自相依項視窗觸發器參考 | 是 |
...
> [!NOTE]
> 翻轉視窗觸發器可能最多取決於另外兩個觸發器。

## <a name="tumbling-window-self-dependency-properties"></a>輪轉視窗自我相依性屬性

在觸發器在成功完成上一個視窗之前不應轉到下一個視窗的情況下,生成自依賴關係。 相依於前一 hr 中自身早期執行成功的自我依賴觸發器將具有以下屬性:
...
```
## <a name="usage-scenarios-and-examples"></a>使用機制與範例

下面是翻倒窗口依賴項屬性的方案和使用方式的插圖。

### <a name="dependency-offset"></a>相依性偏移

![位移示例](media/tumbling-window-trigger-dependency/tumbling-window-dependency02.png "位移示例")
...
![相依項範例](media/tumbling-window-trigger-dependency/tumbling-window-dependency05.png "相依項範例")

### <a name="dependency-on-itself"></a>對其本身的相依性

作業輸出資料流中沒有間隔的每日作業：

![自依賴範例](media/tumbling-window-trigger-dependency/tumbling-window-dependency06.png "自依賴範例")

有關如何使用翻滾視窗觸發器在 Azure 資料工廠建立從屬管道的演示,請觀看以下影片:
