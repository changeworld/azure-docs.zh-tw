---
title: 使用 Azure Data Factory 從 IBM Informix 複製資料及將資料複製到其中
description: 瞭解如何使用 Azure Data Factory 管線中的複製活動，從 IBM Informix 複製資料及將資料複製到其中。
...
---
# <a name="copy-data-from-and-to-ibm-informix-using-azure-data-factory"></a>使用 Azure Data Factory 從 IBM Informix 複製資料及將資料複製到其中
...
您可以將資料從 Informix 來源複製到任何支援的接收資料存放區，或從任何支援的來源資料存放區複製到 Informix 接收器。 如需複製活動所支援作為來源/接收器的資料存放區清單，請參閱[支援的資料存放區](copy-activity-overview.md#supported-data-stores-and-formats)表格。

## <a name="prerequisites"></a>Prerequisites

若要使用此 Informix 連接器，您需要：

- 設定一個「自我裝載 Integration Runtime」。 如需詳細資訊，請參閱自我裝載的 [Integration Runtime](create-self-hosted-integration-runtime.md) 文章。
- 在 Integration Runtime 機上安裝資料存放區的 Informix ODBC 驅動程式。 例如，您可以使用「IBM INFORMIX Informix DRIVER (64-bit) 」驅動程式。

## <a name="getting-started"></a>開始使用

[!INCLUDE [data-factory-v2-connector-get-started](../../includes/data-factory-v2-connector-get-started.md)]

下列各節提供屬性的相關詳細資料，這些屬性是用來定義 Informix 連接器專屬 Data Factory 實體。
...
若要從 Informix 複製資料，[複製活動 **來源** ] 區段支援下列屬性：

| 屬性 | 說明 | 必要 |
|:--- |:--- |:--- |
| type | 複製活動來源的 type 屬性必須設定為： **InformixSource** | 是 |
...
### <a name="informix-as-sink"></a>Informix 為接收

若要將資料複製到 Informix，[複製活動 **接收** ] 區段支援下列屬性：

| 屬性 | 說明 | 必要 |
|:--- |:--- |:--- |
| type | 複製活動接收的 type 屬性必須設定為： **InformixSink** | 是 |
