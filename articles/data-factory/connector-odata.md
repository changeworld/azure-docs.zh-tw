...
- OData 3.0 和 4.0 版。
- 使用以下身份驗證之一複製資料：**匿名**身份驗證、**基本**身份驗證 **、Windows**和**AAD 服務主體**。
...
| authenticationType | 用來連線到 OData 來源的驗證類型。 允許的值是**匿名**、**基本**、**視窗**和**AadService 原則**。 不支援基於使用者的 OAuth。 | 是 |
...
要從 OData 複製資料，在"複製活動**源**"部分中支援以下屬性：

| 屬性 | 描述 | 必要 |
|:--- |:--- |:--- |
| type | 複製活動源**的類型**屬性必須設置為**ODataSource**。 | 是 |
