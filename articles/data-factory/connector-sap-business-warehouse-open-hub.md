...
- SAP 業務倉庫**版本 7.01 或更高版本(在 2015 年之後發布的 SAP 支援包堆疊中)。**
...
SAP BW Open Hub Destination (OHD) 會定義轉送 SAP 資料的目標。 SAP 資料傳輸流程 (DTP) 支援的任何物件都可以用作開放集線器資料來源,例如 DSO、InfoCube、DataSource 等。開啟集線器目標類型 (其中存儲中繼資料 ) 可以是資料庫表(本地或遠端)和平面檔。 此 SAP BW Open Hub 連接器支援從 BW 中的 OHD 本機資料表複製資料。 如果您使用其他類型，您可以使用其他連接器直接連線到資料庫或檔案系統。

![SAP BW Open Hub](./media/connector-sap-business-warehouse-open-hub/sap-bw-open-hub.png)

## <a name="delta-extraction-flow"></a>遞增流程

ADF SAP BW 開放式集線`excludeLastRequest`器`baseRequestId`連接器 提供 兩個可選屬性:可用於處理來自打開集線器的增量負載。 
...
1. **SAP BW 資料傳輸流程 (DTP)** 此步驟將資料從 SAP BW 資訊提供者複製到 SAP BW 開放中心表 

1. **ADF 資料複本**在此步驟中,開啟集線器表由 ADF 連接器讀取 

![遞增流程](media/connector-sap-business-warehouse-open-hub/delta-extraction-flow.png)

在第一步中,將執行 DTP。 每次執行都會創建新的 SAP 請求 ID。 請求 ID 儲存在打開中心表中,然後由 ADF 連接器用於標識增量。 這兩個步驟以異步方式運行:DTP 由 SAP 觸發,ADF 數據拷貝通過 ADF 觸發。 

默認情況下,ADF 不會從打開中心表讀取最新的增量(選項"排除最後請求"為 true)。 因此,ADF 中的數據不是 100% 最新的,與打開中心表中的數據(缺少最後一個增量)一起。 作為回報,此過程可確保非同步提取不會丟失任何行。 即使 ADF 正在讀取打開中心表,而 DTP 仍在寫入同一表,它也能正常工作。 

通常,ADF 在暫存資料儲存(如上圖中的 Azure Blob)中,將複製的請求 ID 在上次執行時的最大複製請求 ID 儲存。 因此,ADF 不會在後續運行中第二次讀取同一請求。 同時,請注意,數據不會自動從打開中心表中刪除。

為了進行正確的增量處理,不允許在同一個打開中心表中具有來自不同 DTP 的請求 IP。 因此,您不得為每個打開中心目標 (OHD) 創建多個 DTP。 當需要從同一資訊提供者提取完整和增量時,應為同一資訊提供者創建兩個 OHD。 
...
> [!TIP]
>
> 有關使用 SAP BW 開放集線器連接器的演練,請參閱[使用 Azure 資料工廠載入來自 SAP 商務主目錄 (BW) 的資料](load-sap-bw-data.md)。
...
如需可用來定義資料集的區段和屬性完整清單，請參閱[資料集](concepts-datasets-linked-services.md)一文。 本節提供 SAP BW 開放中心數據集支援的屬性清單。
...
如果設置`excludeLastRequest``baseRequestId`和數據集中,它仍然支援它,同時建議您今後在活動源中使用新模型。
...
要從 SAP BW 開放中心複製資料,複製活動**來源**部分支援以下屬性:

| 屬性 | 描述 | 必要 |
|:--- |:--- |:--- |
| type | 複製活動來源**的類型**屬性必須設定為**SapOpenHubSource**。 | 是 |
...
要加快數據載入速度,可以設置[`parallelCopies`](copy-activity-performance-features.md#parallel-copy)複製活動以並行從 SAP BW 開放集線器載入資料。 例如,如果設置為`parallelCopies`4,則數據工廠同時執行四個 RFC 調用,並且每個 RFC 調用從由 DTP 請求 ID 和包 ID 分區的 SAP BW 開放中心表檢索一部分數據。 當唯一 DTP 請求 ID + 包`parallelCopies`ID 的數量大於的值 時,這適用。 將資料複製到基於檔案的資料儲存中時,還會重新命令以多個檔形式寫入資料夾(僅指定資料夾名稱),在這種情況下,性能優於寫入單個檔。
...
**症狀:** 如果在 HANA 上運行 SAP BW,並且僅觀察到僅使用 ADF 複製活動(100 萬行)複製數據子集,則可能的原因是您在 DTP 中啟用"SAP HANA 執行"選項,在這種情況下,ADF 只能檢索第一批數據。

**解析度:** 禁用 DTP 中的「SAP HANA 執行」選項,重新處理數據,然後嘗試再次執行複製活動。
