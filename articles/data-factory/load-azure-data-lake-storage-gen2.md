...
Azure 資料工廠 （ADF） 是一個完全託管的基於雲的資料整合服務。 您可以使用此服務，在建置分析解決方案時，於 Lake 中置入來自豐富的內部部署集合和雲端式資料存放區的資料，並節省時間。 如需受支援連接器的詳細清單，請參閱[支援的資料存放區](copy-activity-overview.md#supported-data-stores-and-formats)資料表。
...
* 啟用了資料存儲庫第 2 代的 Azure 存儲帳戶：如果沒有存儲帳戶，[請創建一個帳戶](https://ms.portal.azure.com/#create/Microsoft.StorageAccount-ARM)。
...
9. 在 **"選擇輸出檔案或資料夾**"頁中，將**froms3 中的複製**作為輸出檔案夾名稱，然後選擇 **"下一步**"。 ADF 將在複製期間創建相應的 ADLS Gen2 檔案系統和子資料夾（如果不存在）。
...
13. 請注意，系統會自動選取左側的 [監視]**** 索引標籤。 "**操作"** 列包含用於查看活動運行詳細資訊和重新運行管道的連結：
