...
不支援[SQL Server Express LocalDB](/sql/database-engine/configure-windows/sql-server-express-localdb) 。
...
當您將資料複製到 SQL Server 資料庫時，您也可以使用來源資料表的每個批次上的其他參數來設定及叫用使用者指定的預存程式。 預存程式功能會利用 [資料表值參數](/dotnet/framework/data/adonet/sql/table-valued-parameters)。
...
當您從 [Always Encrypted](/sql/relational-databases/security/encryption/always-encrypted-database-engine)SQL Server 複製資料時，請使用 [一般 ODBC 連接器](connector-odbc.md) ，並透過自我裝載的 Integration Runtime SQL Server ODBC 驅動程式。 此 SQL Server 連接器目前不支援 Always Encrypted。 
