...
> * [目前版本](concepts-pipelines-activities.md)
[!INCLUDE[appliesto-adf-asa-md](includes/appliesto-adf-asa-md.md)]

本文協助您了解 Azure Data Factory 中的管線和活動，並使用這些項目來為您的資料移動和資料處理案例建構端對端的資料導向工作流程。

## <a name="overview"></a>概觀
資料處理站可以有一或多個管線。 管線是一起執行某個工作的活動所組成的邏輯群組。 例如,管道可以包含一組引入和清理日誌數據的活動,然後啟動映射數據流以分析日誌數據。 管道允許您將活動作為一個集而不是每個單獨的集進行管理。 您可以獨立部署和計畫管道,而不是活動。

管線中的活動會定義要在資料上執行的動作。 例如，您可以使用複製活動將資料從內部部署 SQL Server 複製到「Azure Blob 儲存體」。 然後,使用數據流活動或 Databricks 筆記本活動來處理數據並將其轉換為 Azure Synapse 分析池,在此基礎上構建了商業智慧報告解決方案。

資料工廠有三個群組的活動:[資料移動活動](copy-activity-overview.md)、[資料轉換活動](transform-data.md)[與控制活動](control-flow-web-activity.md)。 作用可以取得零個或多個輸入[資料集](concepts-datasets-linked-services.md),並產生一個或多個輸出[資料集](concepts-datasets-linked-services.md)。 下圖顯示 Data Factory 中管線、活動及資料集之間的關聯性：
...
控制活動 | 描述
---------------- | -----------
[附加變數](control-flow-append-variable-activity.md) | 向現有陣列變數添加值。
[執行導管](control-flow-execute-pipeline-activity.md) | 「執行管道」活動可讓 Data Factory 管道叫用另一個管道。
[Filter](control-flow-filter-activity.md) | 將篩選器運算式應用於輸入陣列
...
[If 條件活動](control-flow-if-condition-activity.md) | 「If 條件」可用於根據評估為 true 或 false 的條件來分支。 If Condition 活動所提供的功能，與 If 陳述式在程式設計語言中提供的功能相同。 當條件評估到`true`時,它評估一組活動,當條件評估為`false.`
...
[驗證活動](control-flow-validation-activity.md) | 確保管道僅在存在引用數據集、滿足指定條件或已達到超時時繼續執行。
[Wait 活動](control-flow-wait-activity.md) | 在管道中使用 Wait 活動時,管道將等待指定時間,然後再繼續執行後續活動。
[網路活動](control-flow-web-activity.md) | 使用 Web 活動可以從 Data Factory 管線呼叫自訂的 REST 端點。 您可以傳遞資料集和連結服務，以供活動取用和存取。
[Webhook 活動](control-flow-webhook-activity.md) | 使用 Webhook 活動,調用終結點,並傳遞回調 URL。 管道運行等待調用回調,然後再繼續下一個活動。
...
Tag | 描述 | 類型 | 必要
--- | ----------- | ---- | --------
NAME | 管線的名稱。 指定代表管線所執行之動作的名稱。 <br/><ul><li>字元數目上限︰140</li><li>開頭必須為字母、數字或底線 (\_)</li><li>不允許使用以下字元:".".","*"、"\""/","<"、">"、""%"、"&"、":","\" </li></ul> | String | 是
...
Tag | 描述 | 必要
--- | ----------- | ---------
NAME | 活動的名稱。 指定代表活動所執行之動作的名稱。 <br/><ul><li>字元數目上限︰55</li><li>必須以字母編號或下劃線開頭\_( )</li><li>不允許使用以下字元:".".","*"、"\""/","<"、">"、""%"、"&"、":","\" | 是</li></ul>
...
原則 | 會影響活動之執行階段行為的原則。 此屬性包括超時和重試行為。 如果未指定,則使用預設值。 如需詳細資訊，請參閱[活動原則](#activity-policy)一節。 | 否
...
secureOutput | 設置為 true 時,活動輸出被視為安全,不會記錄以進行監視。 | Boolean | 否。 預設值為 false。
...
Tag | 描述 | 必要
--- | ----------- | --------
NAME | 活動的名稱。 指定代表活動所執行之動作的名稱。<br/><ul><li>字元數目上限︰55</li><li>開頭必須為字母、數字或底線 (\_)</li><li>不允許使用以下字元:".".","*"、"\""/","<"、">"、""%"、"&"、":","\" | 是</li><ul>
description | 說明活動用途的文字 | 是
...
typeProperties | typeProperties 區段中的屬性視每一種活動而定。 若要查看活動的類型屬性，請按一下先前小節中的活動連結。 | 否
dependsOn | 這個屬性用來定義活動相依性，以及後續活動如何相依於先前活動。 有關詳細資訊,請參閱[活動相依項](#activity-dependency)。 | 否

### <a name="activity-dependency"></a>活動相依性
活動依賴項定義後續活動如何依賴於以前的活動,以確定是否繼續執行下一個任務的條件。 一個活動可以根據不同的相依性條件，而相依於一或多個先前活動。
...
- 活動 B 具有活動 A 的依賴項**條件,如果**活動 A 的最終狀態為跳過,則活動 B 將運行。 「略過」發生於活動 X -> 活動 Y -> 活動 Z 的情節中，每個活動只有在前一個活動成功時才會執行。 如果活動 X 失敗,則活動 Y 的狀態為"跳過",因為它永遠不會執行。 同樣,活動 Z 的狀態也為"跳過"。
...
- 在 **typeProperties** 區段中，來源類型指定為 **BlobSource**，接收類型指定為 **SqlSink**。 在[「資料移動活動](#data-movement-activities)」部分中,按一下要用作源或接收體的資料儲存,瞭解有關將資料移至/從該資料儲存移動到/從該資料儲存的詳細資訊。
...
- Hive 文稿檔案**saweblogs.hql**儲存在 Azure 儲存帳戶(由文本連結服務指定,稱為 Azure`adfgetstarted`儲存連結服務)和容器 中的腳本資料夾中。
...
管道由觸發程序排程。 有不同類型的觸發器(計劃觸發器,它允許按掛鐘計劃觸發管道,以及手動觸發器,它觸發按需管道)。 如需觸發程序的詳細資訊，請參閱[管道執行和觸發程序](concepts-pipeline-execution-triggers.md)一文。

若要讓觸發程序啟動管道執行，您必須在觸發程序定義中包含特定管道的管道參考。 管道和觸發程序具有 n-m 關聯性。 多個觸發器可以啟動單個管道,而同一觸發器可以啟動多個管道。 定義觸發程序之後，您必須啟動觸發程序，才會開始觸發管道。 如需觸發程序的詳細資訊，請參閱[管道執行和觸發程序](concepts-pipeline-execution-triggers.md)一文。

例如,假設您有一個計劃程式觸發器「觸發器 A」,我希望啟動我的管道「MyCopyPipeline」。。 定義觸發器,如以下範例所示:
