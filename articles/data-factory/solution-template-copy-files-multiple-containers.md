...
本文描述的解決方案範本可用來從檔案存放區之間的多個容器複製檔案。 例如，您可以使用它將您的 data lake 從 AWS S3 遷移至 Azure Data Lake 存放區。 或者，您可以使用此範本，將所有專案從一個 Azure Blob 儲存體帳戶複寫到另一個帳戶。
...
- **ForEach** 會從 **GetMetadata** 活動取得容器清單，然後逐一查看清單，並將每個容器傳遞給複製活動。
- **複製** 會將每個容器從來源儲存體存放區複製到目的地存放區。

範本會定義下列參數：
- *SourceFileFolder* 是您資料來源存放區的資料夾路徑，您可以在其中取得容器清單。 路徑是根目錄，其中包含多個容器檔案夾。 此參數的預設值為 `sourcefolder`。
- *SourceFileDirectory* 是資料來源存放區根目錄下的子資料夾路徑。 此參數的預設值為 `subfolder`。
- *DestinationFileFolder* 是將檔案複製到目的地存放區中的資料夾路徑。 此參數的預設值為 `destinationfolder`。
- *DestinationFileDirectory* 是將檔案複製到目的地存放區中的子資料夾路徑。 此參數的預設值為 `subfolder`。
