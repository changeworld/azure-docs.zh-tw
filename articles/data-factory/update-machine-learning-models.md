...
本文補充主要 Azure Data Factory-Azure Machine Learning Studio (傳統) 整合文章： [使用 Azure Machine Learning Studio (傳統) 和 Azure Data Factory 建立預測管線](transform-data-using-machine-learning.md)。 如果您尚未檢閱主要文件，請在閱讀這篇文章之前先這麼做。

## <a name="overview"></a>概觀
在運用 Azure Machine Learning Studio (傳統) 模型的過程中，您的模型會進行定型和儲存。 接著，使用它來建立預測性 Web 服務。 接著才能在網站、儀表板及行動應用程式取用 Web 服務。

使用 Azure Machine Learning Studio (傳統) 所建立的模型通常不是靜態的。 因為當有新資料或 API 取用者有自己的資料時，模型就必須重新定型。 

重新定型可能經常會發生。 您可以使用批次執行活動和更新資源活動，讓 Azure Machine Learning Studio (傳統) 模型重新定型，並使用 Data Factory 更新預測性 Web 服務。
...
| linkedServiceName             | Azure Machine Learning Studio (包含 updateResourceEndpoint 屬性的傳統) 連結服務。 | 是      |
...
若要讓上述的端對端工作流程正常運作，您必須建立兩個 Azure Machine Learning Studio (傳統) 連結服務：

1. Azure Machine Learning Studio (傳統) 連結服務到定型 web 服務，則批次執行活動會使用此連結服務，其方式與 [使用 Azure Machine Learning Studio (傳統) 和 Data Factory 批次執行活動建立預測管線](transform-data-using-machine-learning.md)所述的方式相同。 差異在於訓練 web 服務的輸出是 .Ilearner 檔案，可供更新資源活動用來更新預測性 web 服務。
2. Azure Machine Learning Studio (傳統) 連結服務到預測性 web 服務的更新資源端點。 更新資源活動會使用此連結服務，以利用上述步驟傳回的 iLearner 檔案，更新預測 Web 服務。

針對第二 Azure Machine Learning Studio (傳統) 連結服務，當 Azure Machine Learning Studio (傳統) Web 服務是傳統 Web 服務或新的 Web 服務時，此設定會有所不同。 差異會在下列各節分別進行討論。
...
下列案例提供更多詳細資料， 它包含從 Azure Data Factory 管線重新定型和更新 Azure Machine Learning Studio (傳統) 模型的範例。
...
下列 JSON 程式碼片段會定義 Azure Machine Learning Studio (傳統) 連結服務，以指向定型 web 服務的預設端點。
