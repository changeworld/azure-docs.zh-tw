...
當您將資料庫工作負載從內部部署 SQL Server 遷移至 Azure 資料庫服務（也就是 Azure SQL Database 或 Azure SQL 受控執行個體）時，SQL Server Integration Services 上的 ETL 工作負載 (SSIS) 做為其中一個主要值新增的服務，也必須進行遷移。
...
-  (SSISDB) 的 SSIS 目錄。 這是在 SQL Server 2012 中引進，並包含一組用來處理 SSIS 專案/封裝的預存程式、視圖和資料表值函式。
- 檔案系統。
