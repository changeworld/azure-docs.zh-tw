...
| 類型 | 用來讀取/寫入 JSON 檔案的壓縮編解碼器。 <br>允許的值為**bzip2**、 **gzip**、 **deflate**、 **ZipDeflate**、 **snappy**或**lz4**。 表示在儲存檔案時使用。 預設值為 [未壓縮]。<br>**注意：** 目前的複製活動不支援 "snappy" & "lz4"，而且對應資料流程不支援 "ZipDeflate"。<br>**請注意**，使用複製活動來解壓縮**ZipDeflate**檔案並寫入以檔案為基礎的接收資料存放區時，預設會將檔案解壓縮至資料夾： `<path specified in dataset>/<folder named as source zip file>/` ， `preserveZipFileNameAsFolder` 在[複製活動來源](#json-as-source)上使用，以控制是否要將 ZIP 檔案名保留為資料夾結構。 | 否。  |
| 等級 | 壓縮比例。 <br>允許的值為**最佳**或**最快**。<br>- **最快速：** 即使產生的檔案未以最佳方式壓縮，壓縮作業也應該儘快完成。<br>- **最佳**：即使作業需要較長的時間才能完成，壓縮作業也應以最佳方式壓縮。 如需詳細資訊，請參閱 [壓縮層級](https://msdn.microsoft.com/library/system.io.compression.compressionlevel.aspx) 主題。 | No       |
...
| 屬性      | 說明                                                  | 必要 |
| ------------- | ------------------------------------------------------------ | -------- |
| type          | 複製活動來源的類型屬性必須設定為**JSONSource**。 | Yes      |
| formatSettings | 屬性的群組。 請參閱下面的**JSON 讀取設定**表格。 | No       |
...
| 屬性      | 說明                                                  | 必要 |
| ------------- | ------------------------------------------------------------ | -------- |
| type          | FormatSettings 的類型必須設定為**JsonReadSettings**。 | Yes      |
...
| 屬性      | 說明                                                  | 必要 |
| ------------- | ------------------------------------------------------------ | -------- |
| type          | 複製活動來源的類型屬性必須設定為**JSONSink**。 | Yes      |
| formatSettings | 屬性的群組。 請參閱下面的**JSON 寫入設定**表格。 | No       |
...
| 屬性      | 說明                                                  | 必要                                              |
| ------------- | ------------------------------------------------------------ | ----------------------------------------------------- |
| type          | FormatSettings 的類型必須設定為**JsonWriteSettings**。 | Yes                                                   |
| filePattern |表示每個 JSON 檔案中儲存的資料模式。 允許的值為： **setOfObjects** （JSON 行）和**arrayOfObjects**。 **預設**值為**setOfObjects**。 關於這些模式的詳細資訊，請參閱 [JSON 檔案模式](#json-file-patterns)一節。 |No |
...
| 單一檔 | 對應資料流程從每個檔案讀取一個 JSON 檔 | 不可以 | `true` 或 `false` | singleDocument |
| 不具引號的資料行名稱 | 如果選取了不具引號的資料**行名稱**，則對應資料流程會讀取未以引號括住的 JSON 資料行。 | 不可以 | `true` 或 `false` |  unquotedColumnNames
| 有批註 | 如果 JSON 資料具有 C 或 c + + 樣式批註，請選取 [**具有批註**] | 不可以 | `true` 或 `false` | asComments |
| 單引號 | 讀取未以引號括住的 JSON 資料行 | 不可以 | `true` 或 `false` | singleQuoted |
| 反斜線已轉義 | 如果使用反斜線來轉義 JSON 資料中的字元，請選取 [**反斜線**（已轉義）] | 不可以 | `true` 或 `false` | backslashEscape |
