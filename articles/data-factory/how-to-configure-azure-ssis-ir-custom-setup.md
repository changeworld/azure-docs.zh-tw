...
藉由使用自訂安裝，您可以將預設的作業設定或環境變更為，例如，啟動其他 Windows 服務、保存檔案共用的存取認證，或使用強式密碼編譯/更安全的網路通訊協定（TLS 1.2）。 或者，您可以在 Azure SSIS IR 的每個節點上安裝其他元件，例如元件、驅動程式或擴充功能。
...
> [!IMPORTANT]
> 若要從未來的增強功能中獲益，建議您搭配自訂安裝，使用適用于 Azure SSIS IR 的 v3 或較新的節點系列。
...
- 如果您想要參考腳本中的子資料夾， *msiexec*不支援使用`.\`標記法來參考根資料夾。 使用之類的`msiexec /i ".\MySubfolder\MyInstallerx64.msi" ...`命令， `msiexec /i "MySubfolder\MyInstallerx64.msi" ...`而不是。
...
1. 準備您的自訂安裝指令碼和及相關聯的檔案 (例如 .bat、.cmd、.exe、.dll、.msi 或 .ps1 檔案)。
   ...
   * 如果您想要將其他工具（例如*msiexec*）產生的其他記錄上傳至您的容器，請在腳本中指定預先定義的`CUSTOM_SETUP_SCRIPT_LOG_DIR`環境變數，做為記錄檔資料夾（例如， *msiexec/i xxx .msi/quiet/lv% CUSTOM_SETUP_SCRIPT_LOG_DIR% \ install .log*）。

1. 下載、安裝和開啟[Azure 儲存體總管](https://storageexplorer.com/)。 操作方法：
...
1. 當您使用 data factory UI 來設定或重新設定 Azure SSIS IR 時，可以在 [**整合執行時間設定**] 窗格的 [**高級設定**] 區段中選取 [**使用額外的系統組態/元件安裝自訂您的 azure ssis Integration Runtime** ] 核取方塊，以新增或移除自訂設定。 
   ...
   * 如果您選取 [**安裝授權] 元件**類型，則可以在 [**元件名稱**] 下拉式清單中，從我們的 ISV 合作夥伴選取整合元件：
     ...
     * 如果您選取**Theobald Software 的擷取 X) 是**元件，則可以從 AZURE SSIS IR 上的 Theobald 軟體，拖曳 & 將您購買的產品授權檔案拖放到 [**授權**檔案] 方塊中，以安裝[擷取 X) 是](https://theobald-software.com/en/xtract-is/)SAP 系統（ERP、s/4HANA、BW）的連接器套件。 目前的整合版本是**6.1.1.3**。
...
1. 當您使用 PowerShell 安裝或重新設定 Azure SSIS IR 時，您可以執行`Set-AzDataFactoryV2IntegrationRuntime` Cmdlet 來新增或移除自訂設定，然後再啟動您的 AZURE ssis ir。
...
1. 若要查看標準自訂設置的一些範例，請使用 Azure 儲存體總管連接到我們的公用預覽容器。
   ...
   e. 按兩下 [ *UserScenarios* ] 資料夾以尋找下列專案：
      ...
      * *.NET FRAMEWORK 3.5*資料夾，其中包含自訂安裝腳本（*main .cmd*），用以安裝舊版的 .NET Framework，在您的 Azure SSIS IR 的每個節點上可能需要此版本的自訂群組件。

      * *BCP*資料夾，其中包含自訂安裝腳本（*main .cmd*），可在您的 Azure SSIS IR 的每個節點上安裝 SQL Server 命令列公用程式（*MsSqlCmdLnUtils*），包括大量複製程式（*BCP*）。

      * *EXCEL*資料夾，其中包含自訂安裝腳本（*main .cmd*），用以安裝 c # 元件和程式庫，您可以在腳本工作中用來在您的 Azure SSIS IR 的每個節點上動態讀取及寫入 EXCEL 檔案。 
      ...
      * *儲存體*資料夾，其中包含自訂安裝腳本（*main .cmd*），可在您的 Azure SSIS IR 的每個節點上安裝 Azure PowerShell。 此設定可讓您部署及執行執行 PowerShell 腳本的 SSIS 套件，[以操作您的 Azure 儲存體帳戶](https://docs.microsoft.com/azure/storage/blobs/storage-how-to-use-blobs-powershell)。 
      ...
      * *TLS 1.2*資料夾，其中包含自訂安裝腳本（*main .cmd*），以使用強式密碼編譯/更安全的網路通訊協定（TLS 1.2），並在您的 Azure SSIS IR 的每個節點上停用較舊的 SSL/TLS 版本。
   ...
   f. 若要嘗試這些自訂安裝範例，請將所選資料夾中的內容複寫到您的容器。
      ...
      當您使用 PowerShell 安裝或重新設定 Azure SSIS IR 時，請以您`Set-AzDataFactoryV2IntegrationRuntime`容器的 SAS URI 作為參數的值來`SetupScriptContainerSasUri`執行 Cmdlet。
