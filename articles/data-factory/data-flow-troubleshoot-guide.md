...
### <a name="error-code-validation-error-at-source-with-multiline-csv-files"></a>錯誤碼：來源有多行 CSV 檔案的驗證錯誤 

- **Message**：您可能會看到下列其中一個錯誤訊息：
   - 最後一個資料行是 null 或遺漏。
   - 來源的架構驗證失敗。
   - 架構匯入無法在 UX 中正確顯示，且最後一個資料行的名稱中有新的行字元。
- **原因**：在對應資料流程中，目前的多行 CSV 來源無法與 \r\n 作為資料列分隔符號一起使用。 有時，在換行時，會傳回中斷來源值。 
- **建議**：在來源產生檔案，並以 \n 作為資料列分隔符號，而不是 \r\n。 或者，您也可以使用複製活動，將 CSV 檔案（具有 \r\n）轉換成 \n 作為資料列分隔符號。
