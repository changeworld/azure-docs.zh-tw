...
description: 瞭解如何使用 Azure 資料工廠從 SFTP 伺服器複製資料。
...
> * [當前版本](connector-sftp.md)

本文概述了如何將資料從 SFTP 伺服器複製到 SFTP 伺服器。 若要了解 Azure Data Factory，請閱讀[簡介文章](introduction.md)。
...
- 使用**基本**或**SshPublicKey**身份驗證從/複製到 SFTP 複製檔。
...
在基於格式的資料集中的設置下`location`，SFTP 支援以下屬性：

| 屬性   | 描述                                                  | 必要 |
| ---------- | ------------------------------------------------------------ | -------- |
| type       | 資料集中的類型`location`屬性必須設置為**SftpLocation**。 | 是      |
...
在基於格式的副本源中的設置下`storeSettings`，SFTP 支援以下屬性：

| 屬性                 | 描述                                                  | 必要                                      |
| ------------------------ | ------------------------------------------------------------ | --------------------------------------------- |
| type                     | 下`storeSettings`的類型屬性必須設置為**SftpReadSettings**。 | 是                                           |
...
### <a name="sftp-as-sink"></a>SFTP 作為接收器

[!INCLUDE [data-factory-v2-file-formats](../../includes/data-factory-v2-file-formats.md)] 

在基於格式的副本接收器中的設置下`storeSettings`，SFTP 支援以下屬性：

| 屬性                 | 描述                                                  | 必要 |
| ------------------------ | ------------------------------------------------------------ | -------- |
| type                     | 下`storeSettings`的類型屬性必須設置為**SftpWriteSettings**。 | 是      |
...
| 使用TempFile重命名 | 指示是上載到暫存檔案並重命名，還是直接寫入目的檔案夾/檔位置。 預設情況下，ADF 首先寫入暫存檔案，然後在上載完成後進行檔重命名，以便 1） 避免衝突寫入導致損壞的檔，如果您有其他進程寫入同一檔，和 2） 確保檔的原始版本存在期間整個轉移。 如果您的 SFTP 伺服器不支援重命名操作，請禁用此選項並確保沒有併發寫入目的檔案。 請參閱下表下方的故障排除提示。 | 否。 預設值為 true。 |
| 操作超時 | 每個寫入請求到 SFTP 伺服器前的等待時間超時。預設值為 60 分鐘（01：00：00）。|否 |

>[!TIP]
>如果在將資料寫入 SFTP 時遇到"使用者錯誤SftpPathNotfound"、"使用者錯誤"或"Sftp操作失敗"的錯誤，並且您使用的 SFTP 使用者確實具有適當的許可權，請檢查您的 SFTP 伺服器是否支援檔重命名操作 - 如果沒有，請禁用"使用暫存檔案上載"（）`useTempFileRename`選項，然後重試。 從上表中瞭解有關此屬性的更多內容。 如果使用自託管集成運行時進行複製，請確保使用版本 4.6 或以上。
