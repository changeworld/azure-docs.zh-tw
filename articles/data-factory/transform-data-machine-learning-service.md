...
description: 瞭解如何在 Azure Data Factory 管線中執行 Azure Machine Learning 管線。
...
以 Azure Data Factory 管線中的步驟執行 Azure Machine Learning 管線。 Machine Learning 執行管線活動可進行批次預測案例，例如找出可能的貸款預設值、判斷情感，以及分析客戶行為模式。
...
mlPipelineId | 已發佈的 Azure Machine Learning 管線識別碼 | 字串 (或含有字串之 resultType 的運算式) | 是
experimentName | Machine Learning 管線執行的執行歷程記錄實驗名稱 | 字串 (或含有字串之 resultType 的運算式) | 否
mlPipelineParameters | 要傳遞至已發佈 Azure Machine Learning 管線端點的索引鍵、值組。 索引鍵必須符合已發佈 Machine Learning 管線中定義之管線參數的名稱 | 具有索引鍵值組的物件 (或具有 resultType 物件的運算式)  | 否
mlParentRunId | 父 Azure Machine Learning 管線執行識別碼 | 字串 (或含有字串之 resultType 的運算式) | 否
continueOnStepFailure | 是否要在步驟失敗時，繼續執行 Machine Learning 管線執行中的其他步驟 | boolean | 否

> [!NOTE]
> 若要填入 Machine Learning 管線名稱和識別碼中的下拉式清單專案，使用者必須有列出 ML 管線的許可權。 ADF UX 會使用已登入的使用者認證，直接呼叫 AzureMLService Api。  
