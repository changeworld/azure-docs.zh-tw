...
> [!NOTE]
> 這些指令碼是使用自我裝載整合執行時間中[記載的命令列公用程式](./create-self-hosted-integration-runtime.md#set-up-an-existing-self-hosted-ir-via-local-powershell)所建立。 如有需要，可根據自動化需求以適當地自訂這些指令碼。
