...
* **事件中樞**：將記錄串流至 Azure 事件中樞。 記錄會變成合作夥伴服務/自訂分析解決方案的輸入，如 Power BI。
...
1. 為設定命名，選取 [**傳送至 Log analytics**]，然後從**Log analytics 工作區**中選取工作區。
    ...
    * 在_資源特定_模式中，Azure Data Factory 的診斷記錄會流入下列資料表：
      - _ADFActivityRun_
      - _ADFPipelineRun_
      - _ADFTriggerRun_
      ...
      您可以選取與您的工作負載相關的各種記錄，以傳送至 Log Analytics 資料表。 例如，如果您根本不使用 SQL Server Integration Services (SSIS) ，則不需要選取任何 SSIS 記錄。 如果您想要記錄 SSIS Integration Runtime (IR) 啟動/停止/維護作業，您可以選取 [SSIS IR 記錄檔]。 如果您在 SQL Server Management Studio 上透過 T-sql 叫用 SSIS 套件執行 (SSMS) 、SQL Server Agent 或其他指定的工具，則可以選取 [SSIS 封裝記錄檔]。 如果您透過 ADF 管線中的 [執行 SSIS 套件] 活動叫用 SSIS 套件執行，則可以選取 [所有記錄]。

    * 如果您選取 [ _AllMetrics_]，將會提供各種 adf 計量讓您監視或引發警示，包括 ADF 活動、管線和觸發程式執行的計量，以及 ssis IR 作業和 ssis 封裝執行。
...
1. **Azure Data Factory 分析 (預覽**的詳細資料) 
   ...
   ![有關「Azure Data Factory 分析 (Preview) 」的詳細資訊](media/data-factory-monitor-oms/monitor-oms-image4.png)
...
| PipelineCancelledRuns                 | 已取消的管線執行計量           | Count    | 總計                | 在一分鐘時間範圍內取消的管線執行總數。 |
| PipelineFailedRuns                   | 失敗的管線執行計量             | Count    | 總計                | 在一分鐘時間範圍內失敗的管線執行總數。 |
| PipelineSucceededRuns                | 成功的管線執行計量          | Count    | 總計                | 在一分鐘時間範圍內成功執行的管線總次數。 |
| TriggerCancelledRuns                  | 已取消的觸發程序執行計量            | Count    | 總計                | 在一分鐘時間範圍內取消的觸發程式執行總數。 |
...
| SSISIntegrationRuntimeStartCancelled  | 已取消 SSIS integration runtime 啟動計量           | Count    | 總計                | 在一分鐘時間範圍內取消的 SSIS integration runtime 啟動總數。 |
| SSISIntegrationRuntimeStartFailed    | 失敗的 SSIS integration runtime 啟動計量             | Count    | 總計                | 在一分鐘時間範圍內失敗的 SSIS integration runtime 啟動總數。 |
| SSISIntegrationRuntimeStartSucceeded | 成功的 SSIS 整合執行時間啟動計量          | Count    | 總計                | 在一分鐘時間範圍內成功啟動的 SSIS 整合執行時間總數。 |
| SSISIntegrationRuntimeStopStuck      | 停滯 SSIS integration runtime 停止計量               | Count    | 總計                | 在一分鐘時間範圍內停滯的 SSIS integration runtime 停止總數。 |
| SSISIntegrationRuntimeStopSucceeded  | 成功的 SSIS 整合執行時間停止計量           | Count    | 總計                | SSIS 整合執行時間的總次數會在一分鐘的時間範圍內停止。 |
| SSISPackageExecutionCancelled         | 已取消 SSIS 封裝執行計量  | Count    | 總計                | 在一分鐘時間範圍內取消的 SSIS 封裝執行總數。 |
...
> [!NOTE]
> 只有來自已完成、觸發的活動和管線執行的事件才會發出。 進行中和**不**會發出 debug 執行。 另一方面，**所有**SSIS 封裝執行的事件都會發出，包括已完成和進行中的事件（不論其調用方法為何）。 例如，您可以透過 SSMS 上的 T-sql、SQL Server Agent 或其他指定的工具，在已啟用 Azure 的)  (SQL Server Data Tools 上叫用封裝，以及在 ADF 管線中執行 SSIS 套件活動的觸發或偵錯工具。
...
* 保留原則會每天套用。 日期之間的界限會在國際標準時間 (UTC) 的午夜發生。 一天結束時，會刪除保留原則以外的天數記錄。 例如，如果您的保留原則是一天，則在今天的一開始，會刪除昨天之前的記錄。
...
* 將授權標頭設定為您從 Azure Active Directory (Azure AD) 中所獲得的 JSON web 權杖。 如需詳細資訊，請參閱[驗證要求](../active-directory/develop/authentication-scenarios.md)。
...
以下是與 SSIS IR 上 SSIS 封裝執行所產生之事件訊息相關的條件記錄屬性。 它們會將類似的資訊傳達為[ssis 目錄 (SSISDB) 事件訊息內容資料表或查看](https://docs.microsoft.com/sql/integration-services/system-views/catalog-event-message-context?view=sql-server-ver15)，其中顯示許多 SSIS 封裝屬性的運行時間值。 當您選取 `Basic/Verbose` 記錄層級，並適用于檢查/合規性檢查時，就會產生它們。
...
| **integrationRuntimeName** | String | SSIS IR 的名稱                                             | `MySSISIR` |
| **等級**                  | String | 診斷記錄的層級                                         | `Informational` |
| **operationId**            | String | 在 SSISDB 中用來追蹤特定作業的唯一識別碼          | `1` (1 表示與**未**儲存于 SSISDB/透過 t-sql 叫用之套件相關的作業)  |
| **coNtextDepth**           | String | 事件訊息內容的深度                              | `0` (0 表示封裝執行開始之前的內容，1表示發生錯誤時的內容，而且會隨著內容從錯誤而增加)  |
| **packagePath**            | String | 封裝物件的路徑，做為您的事件訊息內容來源      | `\Package` |
| **coNtextType**            | String | 封裝物件的類型，做為您的事件訊息內容來源      | `60` (看到[更多內容類型](https://docs.microsoft.com/sql/integration-services/system-views/catalog-event-message-context?view=sql-server-ver15#remarks))  |
...
以下是 ssis IR 上 SSIS 封裝執行所產生之事件訊息的記錄屬性。 它們會將類似的資訊傳達為[SSISDB 事件訊息資料表或查看](https://docs.microsoft.com/sql/integration-services/system-views/catalog-event-messages?view=sql-server-ver15)，以顯示事件訊息的詳細文字/中繼資料。 它們是在任何記錄層級產生，但除外 `None` 。
...
| **messageType**            | String | 事件訊息的類型                                     | `70` (看到[更多訊息類型](https://docs.microsoft.com/sql/integration-services/system-views/catalog-operation-messages-ssisdb-database?view=sql-server-ver15#remarks))  |
| **messageSourceType**      | String | 事件訊息來源的類型                              | `20` (看到[更多訊息來源類型](https://docs.microsoft.com/sql/integration-services/system-views/catalog-operation-messages-ssisdb-database?view=sql-server-ver15#remarks))  |
...
| **executionPath**          | String | 從父封裝到執行元件的完整路徑            | `\Transformation\Data Flow Task` (此路徑也會捕捉元件反覆運算)  |
...
以下是 ssis IR 上 SSIS 封裝執行所產生的可執行統計資料記錄屬性，其中可執行檔是封裝控制流程中的容器或工作。 它們會將類似的資訊傳達為每個執行中可執行檔（包括其反覆運算）的[SSISDB 可執行檔統計資料表或 view](https://docs.microsoft.com/sql/integration-services/system-views/catalog-executable-statistics?view=sql-server-ver15) 。 它們是在任何記錄層級產生 `None` 的，但對於識別工作層級瓶頸/失敗而言很有用。
...
| **integrationRuntimeName** | String | SSIS IR 的名稱                                         | `MySSISIR` |
| **等級**                  | String | 診斷記錄的層級                                     | `Informational` |
| **executionId**            | String | 在 SSISDB 中用來追蹤特定執行的唯一識別碼      | `1` (1 表示與**未**儲存于 SSISDB/透過 t-sql 叫用之套件相關的執行作業)  |
| **executionPath**          | String | 從父封裝到執行元件的完整路徑          | `\Transformation\Data Flow Task` (此路徑也會捕捉元件反覆運算)  |
...
以下是 SSIS IR 上 SSIS 封裝執行所產生之資料流程元件的執行時間統計資料的記錄屬性。 它們會將類似的資訊傳達為[SSISDB 執行元件階段資料表或視圖](https://docs.microsoft.com/sql/integration-services/system-views/catalog-execution-component-phases?view=sql-server-ver15)，以顯示資料流程元件在其所有執行階段所花費的時間。 當您選取 `Performance/Verbose` 記錄層級，並適用于捕捉資料流程執行統計資料時，就會產生它們。
...
以下是在 SSIS IR 上 SSIS 封裝執行所產生的每個資料流程管線階段（從上游到下游元件）之資料移動的記錄屬性。 它們會將類似的資訊傳達為[SSISDB 執行資料統計資料表或查看](https://docs.microsoft.com/sql/integration-services/system-views/catalog-execution-data-statistics?view=sql-server-ver15)，以顯示透過資料流程工作移動的資料列計數。 當您選取 `Verbose` 記錄層級，並適用于計算資料流程輸送量時，就會產生這些資料。
...
若要提升 & 轉移 SSIS 工作負載，您可以在支援的[ADF 中布建 SSIS IR](https://docs.microsoft.com/azure/data-factory/tutorial-deploy-ssis-packages-azure) ：
...
布建之後，您可以[使用 Azure PowerShell 或 ADF 入口網站的**監視器**中樞來檢查 SSIS IR 操作狀態](https://docs.microsoft.com/azure/data-factory/monitor-integration-runtime#azure-ssis-integration-runtime)。 使用專案部署模型時，SSIS 封裝執行記錄會儲存在 SSISDB 內部資料表或 views 中，因此您可以使用 SSMS 等指定的工具來查詢、分析並以視覺化方式呈現它們。 使用封裝部署模型，SSIS 封裝執行記錄可以儲存在檔案系統中，或 Azure 檔案儲存體為 CSV 檔案，您仍然需要使用其他指定的工具進行剖析和處理，然後才可以查詢、分析和呈現其視覺效果。

現在有了[Azure 監視器](https://docs.microsoft.com/azure/azure-monitor/platform/data-platform)整合，您就可以在 Azure 入口網站上查詢、分析並以視覺化方式呈現從 ssis IR 作業和 ssis 套件執行產生的所有計量和記錄。 此外，您也可以對其提出警示。

### <a name="configure-diagnostic-settings-and-workspace-for-ssis-operations"></a>設定 SSIS 作業的診斷設定和工作區

若要將從 SSIS IR 作業和 SSIS 封裝執行產生的所有計量和記錄傳送到 Azure 監視器，您需要[設定 ADF 的診斷設定和工作區](https://docs.microsoft.com/azure/data-factory/monitor-using-azure-monitor#configure-diagnostic-settings-and-workspace)。

### <a name="ssis-operational-metrics"></a>SSIS 操作計量

SSIS 操作[計量](https://docs.microsoft.com/azure/azure-monitor/platform/data-platform-metrics)是效能計數器或數值，描述 ssis IR 啟動和停止作業的狀態，以及在特定時間點的 ssis 封裝執行。 它們是[Azure 監視器中 ADF 計量](https://docs.microsoft.com/azure/data-factory/monitor-using-azure-monitor#data-factory-metrics)的一部分。
...
SSIS 操作[記錄](https://docs.microsoft.com/azure/azure-monitor/platform/data-platform-logs)是 ssis IR 作業和 ssis 封裝執行所產生的事件，可針對任何已識別的問題提供足夠的內容，而且對於根本原因分析很有用。 

當您在 Azure 監視器上設定 ADF 的診斷設定和工作區時，您可以選取相關的 SSIS 操作記錄，並將它們傳送至以 Azure 資料總管為基礎的 Log Analytics。 在那裡，他們將可[使用豐富的查詢語言](https://docs.microsoft.com/azure/azure-monitor/log-query/log-query-overview)、 [Azure 儀表板上的呈現](https://docs.microsoft.com/azure/azure-monitor/learn/tutorial-app-dashboards)，以及[近乎即時的警示](https://docs.microsoft.com/azure/azure-monitor/platform/alerts-log)來進行分析。

![命名您的設定，並選取 log analytics 工作區](media/data-factory-monitor-oms/monitor-oms-image2.png)

Azure 監視器和 Log Analytics 中 SSIS 封裝執行記錄的架構和內容，類似 SSISDB 內部資料表或 views 的架構。
...
您所選取的 SSIS 套件執行記錄一律會傳送到 Log Analytics，不論其調用方法為何。 例如，您可以在啟用 Azure 的 SSDT 上叫用封裝，透過 SSMS 上的 T-sql、SQL Server Agent 或其他指定的工具，以及在 ADF 管線中執行 SSIS 套件活動的觸發或 debug 回合。

查詢記錄分析上的 SSIS IR 作業記錄時，您可以分別使用**OperationName**和**ResultType**屬性設定為 `Start/Stop/Maintenance` 和 `Started/InProgress/Succeeded/Failed` 。 

![查詢 Log Analytics 上的 SSIS IR 作業記錄](media/data-factory-monitor-oms/log-analytics-query.png)

查詢記錄分析上的 SSIS 套件執行記錄時，您可以使用**OperationId** / **ExecutionId** / **CorrelationId**屬性來聯結它們。 **OperationId** /**ExecutionId** `1` 對於與**不**是透過 t-sql 所叫用的所有套件相關的所有作業/執行，ExecutionId 一律會設定為。
