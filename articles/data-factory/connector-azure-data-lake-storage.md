...
>[!IMPORTANT]
>如果您使用 PolyBase 將資料從 Data Lake Storage Gen2 載入 SQL 資料倉儲，在使用 Data Lake Storage Gen2 的受控識別驗證時，請確定您也遵循[本指引](../sql-database/sql-database-vnet-service-endpoint-rule-overview.md#impact-of-using-vnet-service-endpoints-with-azure-storage)中的步驟1和2，以 1) 向 Azure Active Directory (Azure AD) 註冊您的 SQL Database 伺服器，然後 2) 將儲存體 Blob 資料參與者角色指派給您的 SQL Database 伺服器；其餘部分則由 Data Factory 處理。 如果已設定您的 Data Lake Storage Gen2 搭配 Azure 虛擬網路端點使用，若要使用 PolyBase 來載入資料，您必須使用 PolyBase 所要求的受控識別驗證。
...
| maxConcurrentConnections | 可同時連線到儲存體存放區的連線數目。 只有在想要限制同時連線到資料存放區的連線數目時，才需指定此項。 | 否                                            |
...
| blockSizeInMB | 指定用於將資料寫入 ADLS Gen2 的區塊大小 (以 MB 為單位)。 深入了解[區塊 Blob](https://docs.microsoft.com/rest/api/storageservices/understanding-block-blobs--append-blobs--and-page-blobs#about-block-blobs)。 <br/>允許的值**介於 4 到 100 MB之間**。 <br/>根據預設，ADF 會根據您的來源存放區類型和資料，自動決定區塊大小。 對於非二進位複製到 ADLS Gen2 中，預設區塊大小為 100 MB，因此最多可容納 4.95 TB 的資料。 當您的資料不大時，尤其是當您使用網路不佳的自我裝載 Integration Runtime 時，這可能不是最佳大小，因而導致作業超時或效能問題。 您可以明確指定區塊大小，同時確保 blockSizeInMB*50000 夠大足以儲存資料，否則複製活動執行會失敗。 | 否 |
| maxConcurrentConnections | 可同時連線到資料存放區的連線數目。 只有在想要限制同時連線到資料存放區的連線數目時，才需指定此項。 | 否       |
...
| ------------------------------------------------------------ | --------------------------------------------------------- | ------------------------------------------------------------ |
| filesystem<br/>&nbsp;&nbsp;&nbsp;&nbsp;FolderA<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**File1.csv**<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;File2.json<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Subfolder1<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**File3.csv**<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;File4.json<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**File5.csv**<br/>&nbsp;&nbsp;&nbsp;&nbsp;Metadata<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;FileListToCopy.txt | File1.csv<br>Subfolder1/File3.csv<br>Subfolder1/File5.csv | **資料集中：**<br>- 檔案系統：`filesystem`<br>- 資料夾路徑：`FolderA`<br><br>**複製活動來源中：**<br>- 檔案清單路徑：`filesystem/Metadata/FileListToCopy.txt` <br><br>檔案清單路徑指向同個資料存放區內的文字檔，內有所要複製的檔案清單，每行一個檔案，使用資料集內設定路徑的相對路徑。 |
...
當您將檔案從 Amazon S3/Azure Blob/Azure Data Lake Storage Gen2 複製到 Azure Data Lake Storage Gen2/Azure Blob 時，可以選擇保留檔案中繼資料和資料。 深入了解[保留中繼資料](copy-activity-preserve-metadata.md#preserve-metadata)。
...
在對應資料流程中轉換資料時，您可以讀取和寫入 Azure Data Lake Storage Gen2 中 JSON、Avro、分隔文字或 Parquet 格式的檔案。 如需詳細資訊，請參閱對應資料流程功能中的[來源轉換](data-flow-source.md)和[接收轉換](data-flow-sink.md)。
...
**萬用字元路徑：** 使用萬用字元模式會指示 Azure Data Factory 在單一來源轉換中，迴圈處理每個相符的資料夾和檔案。 這是在單一流程內處理多個檔案的有效方式。 當滑鼠停留在現有的萬用字元模式上時，使用顯示的 + 符號可新增多個萬用字元符合模式。

從您的來源容器中，選擇符合模式的一系列檔案。 在資料集內只能指定容器。 因此，您的萬用字元路徑也必須包含根資料夾中的資料夾路徑。
...
* ```/data/sales/2004/*/12/[XY]1?.csv``` 取得 2004 年 12 月中以兩位數字開頭且第一個字元為 X 或 Y 的所有 csv 檔案

**分割區根路徑：** 如果您的檔案來源中有 ```key=value``` 格式的分割區資料夾 (例如 year = 2019)，則可以將該分割區資料夾樹狀結構的最上層指派給資料流程資料流中的資料行名稱。

首先，設定萬用字元以包含所有分割區資料夾的路徑，加上您想要讀取的分葉檔案。

![分割區來源檔案設定](media/data-flow/partfile2.png "分割區檔案設定")

使用 [分割區根路徑] 設定來定義資料夾結構的最上層。 當您透過資料預覽檢視資料的內容時，會看到 Azure Data Factory 新增在您的每個資料夾層級中找到的已解析分割區。

![分割區根路徑](media/data-flow/partfile1.png "分割區根路徑預覽")

**檔案清單：** 這是檔案的集合。 建立文字檔，包含要處理的相對路徑檔案清單。 指向這個文字檔。

**要儲存檔案名稱的資料行：** 將來源檔案的名稱儲存在資料行中。 在此輸入新的資料行名稱以儲存檔案名稱字串。

**完成後：** 選擇在資料流程執行後，不要對來源檔案執行任何動作、刪除來源檔案、或移動來源檔案。 移動的路徑都是相對路徑。

若要在處理後將來源檔案移到另一個位置，請先選取 [移動] 進行檔案操作。 然後，設定「來源」目錄。 如果您的路徑中未使用任何萬用字元，則「來源」設定將會與您的來源資料夾位於相同的資料夾。

如果的來源路徑有萬用字元的，您的語法看起來會像下面這樣：
...
在此情況下，所有 /data/sales 中的檔案都會移至 /backup/priorSales。

> [!NOTE]
> 只有當您從管線執行 (管線偵錯工具或執行回合) 啟動資料流程，且該管線執行在管線中使用「執行資料流程」活動時，檔案作業才會執行。 檔案作業「不會」在資料流程偵錯模式中執行。

**依上次修改時間篩選：** 您可以指定上次修改日期範圍來篩選所處理的檔案。 所有日期時間均為國際標準時間。 
...
**檔案名稱選項：** 決定目的地檔案在目的地資料夾中的命名方式。 檔案名稱的選項有：
   * **預設值**：可讓 Spark 根據 PART 預設值來命名檔案。
   * **模式**：輸入一個模式，列舉每個分割區的輸出檔案。 例如，**loans[n].csv** 會建立 loans1 .csv、loans2，依此類推。
   * **每個分割區**：每個分割區輸入一個檔案名稱。
   * **作為資料行中的資料**：將輸出檔案設定為資料行的值。 路徑為相對於資料集容器的路徑，不是目的地資料夾。 如果您的資料集內有資料夾路徑，將會被覆蓋。
   * **輸出至單一檔案**：將分割的輸出檔案結合成單一的指名檔案。 路徑為相對於資料集資料夾的路徑。 請注意，「合併」作業可能會失敗，取決於節點的大小。 此選項不建議用於大型資料集。
...
>[!NOTE]
>下列模型仍依原狀支援，以提供回溯相容性。 建議您往後使用以上各節所述的新模型，製作 UI 的 ADF 已切換為產生新模型。
