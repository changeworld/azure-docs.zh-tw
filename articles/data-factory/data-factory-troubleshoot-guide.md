---
title: 排除 Azure 資料工廠故障 |微軟文件
description: 瞭解如何在 Azure 數據工廠中排除外部控制活動。
...
ms.locfileid: "81416563"
---
# <a name="troubleshoot-azure-data-factory"></a>排除 Azure 資料工廠故障
[!INCLUDE[appliesto-adf-asa-md](includes/appliesto-adf-asa-md.md)]

本文探討 Azure 數據工廠中外部控制活動的常見故障排除方法。

## <a name="connector-and-copy-activity"></a>連接器與複製活動

對連線器問題(例如,使用複製活動遇到錯誤)請參閱對[Azure 資料工廠連接器進行故障排除](connector-troubleshoot-guide.md)。
...
- **建議**:默認情況下,Azure 數據磚塊訪問權杖的有效期為90天。 建立新權杖並更新連結的服務。
...
- **建議**:為工作區定址方案或`dbfs:/folder/subfolder/foo.py`存儲在數據磚塊檔案系統中的檔指定絕對路徑。
...
- **建議**:檢查使用此數據塊工作區的所有管道的作業創建率。  如果啟動的管道在聚合中運行了過多的 Databricks,則將某些管道遷移到新的工作區。
...
- **消息**`User: `: 簡單使用者上下文[用戶user@company.comId]...` is not   authorized to access cluster.`

- **原因**:不允許生成訪問權杖的使用者存取連結服務中指定的Databricks群集。
...
- **原因**:錯誤消息指示各種問題,如意外的群集狀態或特定活動。 通常根本不顯示錯誤消息。
...
- **建議**:通過更改數據工廠觸發器和活動併發設置,減少向數據湖分析提交的作業數。 或增加數據湖分析的限制。
...
- **原因**:服務主體或證書無法存取存儲中的檔案。

- **建議**:確保使用者為數據湖分析作業提供的服務主體或證書可以訪問數據湖分析帳戶和來自根資料夾中的默認數據湖存儲實例。
...
- **原因**:U-SQL 檔的路徑錯誤,或者鏈接的服務認證未存取權限。
...
- **建議**:類似示例的錯誤表示作業已提交到數據湖分析,並且那裡的腳本失敗。 在數據湖分析中調查。 在門戶中,轉到數據湖分析帳戶,並使用數據工廠活動運行 ID(而不是管道運行 ID)查找作業。 那裡的作業提供有關錯誤的詳細資訊,並將説明您排除故障。 如果解決方案不明確,請聯絡數據湖分析支援團隊並提供作業 URL,其中包括您的帳戶名稱和工作 ID。
...
- **原因**:Azure 函數活動不支援在活動負載中指定的 Http 方法。
...
- **原因**:調用的 Azure 函數未在回應中返回 JSON 有效負載。 ADF Azure 函數活動僅支援 JSON 回應內容。

- **建議**:更新 Azure 函數以返回有效的 JSON 有效負載,例如 C# 函數\"可能會返回(操作結果)新的 OkObjectResult("*ID\":123\"\"}");
...
- **建議**:請檢查輸入 Azure 函數活動 JSON 定義具有名為"函數鍵" 的屬性。
...
- **建議**:請檢查輸入 Azure 函數活動 JSON 定義具有名為"函數 AppUrl"的屬性。
...
- **建議**:請檢查輸入 Azure 函數活動 JSON 定義具有名為"方法"的屬性。
...
- **建議**:請轉到 Azure 門戶,查找存儲,複製連接字串並貼上到連結的服務中,然後重試。
...
- **原因**:由於基礎問題(如網路連接、DNS 失敗、伺服器證書驗證或超時),請求失敗。
...
- **建議**:確保可執行檔存在。 如果程式啟動,請確保*stdout.txt*和*stderr.txt*已上傳到儲存帳戶。 最好在代碼中發出大量日誌以進行調試。
...
- **原因**:自定義活動的資料夾 Path 中的檔太多。 資源檔的總大小不能超過 32,768 個字元。
...
- **原因**:同一名稱的多個文件位於資料夾 Path 的不同子資料夾中。

- **建議**:自定義活動在資料夾路徑下平展資料夾結構。  如果需要保留資料夾結構,請使用解壓縮命令壓縮檔案並將其提取到 Azure Batch 中。 例如,使用`powershell.exe -nologo -noprofile   -command "& { Add-Type -A 'System.IO.Compression.FileSystem';   [IO.Compression.ZipFile]::ExtractToDirectory($zipFile, $folder); }" ;   $folder\yourProgram.exe`
...
- **原因**: 批次處理網址 必須類似於`https://mybatchaccount.eastus.batch.azure.com`
...
- **建議**:錯誤消息應有助於解決問題。 如果沒有足夠的資訊,請聯絡 ADF 支援人員尋求進一步説明。
...
- **建議**:嘗試其他存儲。 如果這不是可接受的解決方案,請聯絡 ADF 支援團隊以獲得進一步説明。
...
- **原因**:嘗試讀取服務主體或實例化 MSI 身份驗證時出現內部錯誤。

- **建議**:請考慮提供具有在提供的訂閱中創建 HDInsight 叢集的許可權的服務主體,然後重試。 如果這不是可接受的解決方案,請聯絡 ADF 支援團隊以獲得進一步説明。
...
- **原因**:當錯誤消息包含類似於"無法解析遠端名稱"的消息時,這可能意味著提供的群集 URI 無效。


- **建議**:確保群集尚未刪除,並且提供的URI正確。 在瀏覽器中打開URI時,應看到Ambari UI。 如果群集位於虛擬網路中,則URI應為專用URI。 要打開它,請使用屬於同一虛擬網路的 VM。 如需詳細資訊，請參閱 [this](https://docs.microsoft.com/azure/hdinsight/hdinsight-plan-virtual-network-deployment#directly-connect-to-apache-hadoop-services)。
                  

<br>

- **原因**:當錯誤消息包含類似於"任務已取消"的消息時,這意味著作業提交超時。

- **建議**: 問題可能是一般 HDInsight 連接或網路連接。 首先確認 HDInsight Ambari UI 可從任何瀏覽器獲得。 確認您的憑據仍然有效。 如果使用自託管整合時 (IR),請確保在安裝自託管 IR 的 VM 或電腦中執行此操作。 然後嘗試再次嘗試從數據工廠提交作業。 如果仍然失敗,請與數據工廠團隊聯繫以尋求支援。

<br>

- **原因**:當錯誤消息包含類似於"使用者管理員鎖定在 Ambari 中"或"未授權:Ambari 使用者名或密碼不正確"的消息時,這意味著 HDInsight 的認證不正確或已過期。

- **建議**:更正憑據並重新部署連結的服務。 首先,通過在任何瀏覽器上打開群集 URI 並嘗試登錄,確保憑據在 HDInsight 上正常工作。 如果憑據不起作用,可以從 Azure 門戶重置它們。

<br>

- **原因**:當錯誤消息包含類似於"502 - Web 伺服器在充當閘道或代理伺服器時收到無效回應"的消息時,HDInsight 服務將返回此錯誤。


- **建議**: 檢視 Azure HDInsighthttps://hdinsight.github.io/ambari/ambari-ui-502-error.htmlhttps://hdinsight.github.io/spark/spark-thriftserver-errors.htmlhttps://docs.microsoft.com/azure/application-gateway/application-gateway-troubleshooting-502故障排除文件, 例如 , 。
                  

<br>

- **原因**:當錯誤消息包含類似於"無法為提交作業請求提供服務"或"由於 templeton 服務忙於提交作業請求太多"或"佇列 root.joblauncher 已包含 500 個申請,無法接受提交申請"的消息時,這意味著同時向 HDInsight 提交的作業太多。

- **建議**:考慮限制提交到 HDInsight 的併發作業數。 如果同一活動正在提交作業,請參閱數據工廠活動併發。 更改觸發器,以便併發管道運行隨時間而分散。 請參閱 HDInsight 文件來調整坦普頓.parallellism.job.提交,如錯誤所示。
...
- **建議**:當 ADF 在嘗試獲取正在運行的作業的狀態時未從 HDInsight 群集獲得回應時,將發生此錯誤。 這可能是群集本身出現問題的原因,或者 HDInsight 服務可能有中斷。 請參閱 HDInsight 故障排除https://docs.microsoft.com/azure/hdinsight/hdinsight-troubleshoot-guide文檔, 或聯絡其支援以獲得進一步説明。
...
- **建議**:按照活動運行輸出中的 Yarn 日誌連結尋找 HDI 輸出中的錯誤。 如果需要,請聯繫 HDInsight 團隊尋求支援。
...
- **建議**:按照活動運行輸出中的 Yarn 日誌連結尋找 HDI 輸出中的錯誤。 請重試,或在需要時聯繫 HDInsight 團隊尋求支援。
...
- **原因**:HDI 連結服務或 HDI 活動中使用的儲存連結服務配置了不支援的 MSI 身份驗證。

- **建議**:請為 HDI 連結服務或 HDI 活動中使用的儲存帳戶提供完整的連接字串。
...
- **建議**:錯誤消息應有助於識別問題。 請修復 json 配置,然後重試。 有關詳細資訊https://docs.microsoft.com/azure/data-factory/compute-linked-services#azure-hdinsight-on-demand-linked-service,請查看。
...
- **原因**:ADF 嘗試使用 Livy API(livy/batch)在 Spark 群集上創建批處理,但收到錯誤。

- **建議**:請按照錯誤消息來解決此問題。 如果沒有足夠的資訊來解決問題,請與 HDI 團隊聯繫,並為他們提供可在活動運行 ADF 監視頁中的輸出中找到的批處理 ID 和作業 ID。
...
- **建議**:請按照活動運行輸出中的連結在 ADF 監視頁中對 HDInsight Spark 群集上的運行進行故障排除。 請聯繫 HDInsight 支援團隊以獲得進一步説明。
...
- **建議**:對 HDInsight Spark 群集上的批處理進行故障排除。 請聯繫 HDInsight 支援,以獲得進一步説明。 
...
- **原因**:HDInsightOnDemand 鏈接服務不支援透過自託管 IR 執行。
...
- **原因**:提供的認證對群集是錯誤的,或者存在網路配置或連接問題,或者 IR 連接到群集時出現問題。

- **建議**：  
      1. 通過在瀏覽器中打開 HDInsight 群集的 Ambari UI,驗證認證認證是否正確。
      ...
      2. 如果群集在 VNet 中,則使用自承載的 IR,並且使用了專用 URL,並且連接仍然失敗,則安裝 IR 的 VM 連接到 HDI 時出現問題。 連接到安裝 IR 的 VM,並在瀏覽器中打開 Ambari UI。 使用群集的專用 URL。 此連接應在瀏覽器中工作。 如果沒有,請聯繫 HDInsight 支援團隊以獲得進一步説明。
      3. 如果未使用自承載 IR,則應公開訪問 HDI 群集。 在瀏覽器中打開 Ambari UI 並確保它打開。 如果群集或其上的服務存在任何問題,請聯絡 HDInsight 支援團隊尋求説明。
...
- **建議**:錯誤消息和錯誤代碼應有助於解決 ODBC 連接錯誤。 如果它們不足以解決問題,請聯絡 Azure HDInsight 團隊尋求支援。
...
- **原因**:ADF 通過 ODBC 連接向 HDI 群集提交了用於執行的配置單元腳本,並且該文本在 HDI 上出現故障。

- **建議**:在 HDI 群集上執行 hive 文稿失敗,錯誤消息和錯誤代碼應有助於故障排除。 如果它們不足以解決問題,請聯絡 Azure HDInsight 團隊尋求支援。
...
- **建議**:HDI 活動的主儲存連結服務中僅支援完整連接字串。 請確保您沒有使用 MSI auth 或應用程式。
...
- **建議**:當 ADF 為 HDI 活動執行準備步驟時,將發生此錯誤。 在將作業提交到 HDI 之前,它嘗試將檔案複製到主儲存。 確保檔存在於提供的位置,存儲連接正確。 ADF HDI 活動不支援與 HDI 活動相關的儲存帳戶上的 MSI 身份驗證,因此請確保這些連結的服務具有完整金鑰或使用 Azure 金鑰保管庫。
...
- **建議**:請檢查該檔是否確實存在,並且指向此檔的連接資訊的連結服務具有正確的認證。
...
- **建議**:請確保所選連結的服務具有活動支持的類型之一。 HDI 活動支援 Azure Blob 儲存和 Azure BlobFS 儲存連結的服務。
...
- **建議**：  
      \"請確保提供的值類似於: 命令\"環境\": [ 變數Name_\"變數值 ] 每個變數只出現在清單中一次。
...
- **原因**:群集創建失敗,ADF 未從 HDInsight 服務傳回錯誤。

- **建議**:打開 Azure 門戶,並嘗試查找具有提供名稱的 HDI 資源,並檢查預配狀態。 請聯繫 HDInsight 支援團隊以獲得進一步説明。
...
- **原因**:提供的額外存儲不是 Azure Blob 儲存。

- **建議**:提供 Azure Blob 儲存帳戶作為 HDInsight 按需連結服務的額外儲存。
...
要使用 Fiddler 建立受監視 Web 應用程式的 HTTP 工作階段,請執行以下操作:

1. 下載、安裝與開啟[Fiddler](https://www.telerik.com/download/fiddler)。
...
1. 如果您的應用程式使用 TLS/SSL 憑證,請將 Fiddler 憑證添加到您的裝置。 跳到**工具** > **Fiddler 選項** > **HTTPS** > **操作** > **將根憑證匯出到桌面**。

1. 通過訪問**檔案** > **捕獲流量**關閉捕獲。 或按**F12**。

1. 清除瀏覽器的快取,以便刪除所有快取的項目,並且必須再次下載。
