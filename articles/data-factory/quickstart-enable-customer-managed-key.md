...
下圖顯示 Data Factory 如何透過 Azure Active Directory 和 Azure Key Vault 使用客戶管理的金鑰提出要求：
  ...
  ![此圖顯示客戶管理的金鑰在 Azure Data Factory 中的運作方式](media/quickstart-enable-customer-managed-key/encryption-customer-managed-keys-diagram.png)
...
如果您要透過 Azure 入口網站建立新的 Azure Key Vault，可以依照下列方式啟用__虛刪除__和__不要清除__：
  ...
  ![在建立 Key Vault 時啟用虛刪除和清除保護的螢幕擷取畫面](media/quickstart-enable-customer-managed-key/01-enable-purge-protection.png)
...
請確定 Azure Key Vault 與 Azure Data Factory 位於相同的 Azure Active Directory (Azure AD) 租用戶和_相同區域_中。 透過 Azure Key Vault 存取控制，為資料處理站的受控服務識別 (MSI) 授與下列權限：_取得_、_解除包裝金鑰_和_包裝金鑰_。 必須要有這些權限，才能在 Data Factory 中啟用客戶管理的金鑰。
  ...
  ![讓 Data Factory 有權存取 Key Vault 的螢幕擷取畫面](media/quickstart-enable-customer-managed-key/02-access-policy-factory-msi.png)
...
您可以建立自己的金鑰並將其儲存在金鑰保存庫中，或是使用 Azure Key Vault API 來產生金鑰。 Data Factory 加密僅支援 2048 位元的 RSA 金鑰。 如需詳細資訊，請參閱[關於金鑰、祕密和憑證](../key-vault/general/about-keys-secrets-certificates.md)。
  ...
  ![產生客戶管理的金鑰的螢幕擷取畫面](media/quickstart-enable-customer-managed-key/03-create-key.png)
...
1. 複製 [金鑰識別碼] 欄位的值，以提供 URI
    ...
    ![從 Key Vault 取得金鑰 URI 的螢幕擷取畫面](media/quickstart-enable-customer-managed-key/04-get-key-uri.png)

1. 啟動 Azure Data Factory 入口網站，並使用左側的導覽列跳至 Data Factory 首頁

1. 按一下 [客戶管理的金鑰] 圖示

    ![在 Data Factory 中啟用客戶管理的金鑰的螢幕擷取畫面](media/quickstart-enable-customer-managed-key/05-cmk-configuration.png)
