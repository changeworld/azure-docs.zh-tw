...
2. 允許 Azure 服務存取 SQL Server。 確認已為執行 SQL Database 的伺服器啟用 [允許 Azure 服務和資源存取此伺服器] 設定。 此設定可讓 Data Factory 將資料寫入您的資料庫執行個體。 若要確認並開啟此設定，請移至邏輯 SQL Server > [安全性] > [防火牆與虛擬網路] > 將 **[允許 Azure 服務和資源存取此伺服器]** 選項設定為 **[開啟]** 。
