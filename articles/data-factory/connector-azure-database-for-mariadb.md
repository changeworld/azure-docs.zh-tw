---
title: 從 Azure 資料庫複製 MariaDB 的資料
...
以下活動支援 MariaDB 連接器的此 Azure 資料庫：
...
| 屬性 | 描述 | 必要 |
|:--- |:--- |:--- |
| type | 類型屬性必須設置為 **：Azure MariaDB** | 是 |
...
有關可用於定義資料集的節和屬性的完整清單，請參閱[資料集](concepts-datasets-linked-services.md)一文。 本節提供適用於 MariaDB 的 Azure 資料庫資料集所支援的屬性清單。

要從 MariaDB 的 Azure 資料庫複製資料，支援以下屬性：

| 屬性 | 描述 | 必要 |
|:--- |:--- |:--- |
| type | 資料集的類型屬性必須設置為 **：AzureMariaDBTable** | 是 |
...
要從 MariaDB 的 Azure 資料庫複製資料，複製活動**源**部分支援以下屬性：

| 屬性 | 描述 | 必要 |
|:--- |:--- |:--- |
| type | 必須將複製活動源的類型屬性設置為 **：AzureMariaDBSource** | 是 |
