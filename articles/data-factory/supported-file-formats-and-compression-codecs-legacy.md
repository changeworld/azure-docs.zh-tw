...
---
# <a name="supported-file-formats-and-compression-codecs-in-azure-data-factory-legacy"></a>Azure 資料工廠支援的檔案格式和壓縮編解碼器(舊版)
...
>[!IMPORTANT]
>資料工廠引入了新的基於格式的數據集模型,請參閱相應的格式文章,其中詳細介紹了: <br>- [Avro 格式](format-avro.md)<br>- [二進位格式](format-binary.md)<br>- [分隔文字格式](format-delimited-text.md)<br>- [JSON 格式](format-json.md)<br>- [ORC 格式](format-orc.md)<br>- [鑲木地板格式](format-parquet.md)<br>本文中提到的其他配置仍支持作為向後相容性的配置。 建議您今後使用新模型。 

## <a name="text-format-legacy"></a><a name="text-format"></a>文字格式(舊版)

>[!NOTE]
>從[分隔文字格式](format-delimited-text.md)文章中瞭解新模型。 基於檔案的資料儲存資料集上的以下配置仍支援按"按"進行反向相容性。 建議您今後使用新模型。
...
| 屬性 | 描述 | 允許的值 | 必要 |
| --- | --- | --- | --- |
| columnDelimiter |用來分隔檔案中的資料行的字元。 您可以考慮使用資料中不太可能存在的罕見不可列印字元。 例如，指定 "\u0001"，這代表「標題開頭」(SOH)。 |只允許一個字元。 **默認值**為**逗號(",")。** <br/><br/>若要使用 Unicode 字元，請參考 [Unicode 字元 (英文)](https://en.wikipedia.org/wiki/List_of_Unicode_characters) 以取得其對應的代碼。 |否 |
| rowDelimiter |用來分隔檔案中資料列的字元。 |只允許一個字元。 **默認值**是讀取的以下任意值 **:"\r\n"、"\r"、"\n"*和****"\r\n"。** |否 |
| escapeChar |用來逸出輸入檔內容中資料行分隔符號的特殊字元。 <br/><br/>您無法為資料表同時指定 escapeChar 和 quoteChar。 |只允許一個字元。 無預設值。 <br/><br/>範例:如果您有逗號 (',') 作為列分隔符,但您希望在文本中具有逗號字元(例如:"你好,世界"),則可以將"$"定義為轉義字元,並在源中使用字串"Hello$$,world"。 |否 |
...
| encodingName |指定編碼名稱。 |有效的編碼名稱。 請參考[編碼.編碼名稱屬性](https://msdn.microsoft.com/library/system.text.encoding.aspx)。 例如：windows-1250 或 shift_jis。 **預設值**為**UTF-8**。 |否 |
...
>[!NOTE]
>從[JSON 格式](format-json.md)文章中瞭解新模型。 基於檔案的資料儲存資料集上的以下配置仍支援按"按"進行反向相容性。 建議您今後使用新模型。
...
>[!NOTE]
>對於將陣列中的數據交叉應用於多個行的情況[(JsonFormat 示例中](#jsonformat-example)為範例 1->示例 2),只能選擇使用 屬性`jsonNodeReference`展開單個陣列。
...
具有**JsonFormat**類型的輸入數據集定義如下:(僅包含相關部件的部分定義)。 具體而言：
...
>[!NOTE]
>從[Parquet 格式](format-parquet.md)文章中瞭解新模型。 基於檔案的資料儲存資料集上的以下配置仍支援按"按"進行反向相容性。 建議您今後使用新模型。
...
對於在具有 Parquet 檔序列化/反序列化的自承載 IR 上運行的副本,ADF*`(SOFTWARE\JavaSoft\Java Runtime Environment\{Current Version}\JavaHome)`* 首先檢查註冊表 以查找 JRE(如果未找到*`JAVA_HOME`*)來定位 JAVA 運行時 ,其次檢查 OpenJDK 的系統變數。
...
>[!NOTE]
>從[ORC 格式](format-orc.md)文章中瞭解新模型。 基於檔案的資料儲存資料集上的以下配置仍支援按"按"進行反向相容性。 建議您今後使用新模型。
...
>[!NOTE]
>從[Avro 格式](format-avro.md)文章中瞭解新模型。 基於檔案的資料儲存資料集上的以下配置仍支援按"按"進行反向相容性。 建議您今後使用新模型。
...
使用 Hive 表中使用 Avro 格式,可以參考[Apache Hive 的教學](https://cwiki.apache.org/confluence/display/Hive/AvroSerDe)。

請注意下列幾點：

* [不支援複雜的數據類型](https://avro.apache.org/docs/current/spec.html#schema_complex)(記錄、枚舉、陣列、映射、聯合和固定)。
...
* 從 Azure blob 讀取 GZIP 壓縮資料，將其解壓縮，並將結果資料寫入到 Azure SQL 資料庫。 將輸入 Azure Blob 資料`compression``type`集的屬性 定義為 GZIP。
* 從來自內部部署檔案系統之純文字檔案讀取資料、使用 GZip 格式加以壓縮並將壓縮的資料寫入到 Azure blob。 將輸出 Azure Blob`compression``type`資料集的屬性定義為 GZip。
* 從 FTP 伺服器讀取.zip 檔、將它解壓縮以取得其中的檔案，並將這些檔案放入 Azure Data Lake Store。 將輸入 FTP 資料集`compression``type`定義為 ZipDeflate。
* 從 Azure blob 讀取 GZIP 壓縮資料，將其解壓縮、使用 BZIP2 將其壓縮，並將結果資料寫入到 Azure blob。 定義輸入 Azure Blob`compression``type`資料集 ,設定為 GZIP,將輸出數據集`compression``type`設置為 BZIP2。
...
* **類型:** 壓縮編解碼器,可以是GZIP、Deflate、BZIP2或**ZipDeflate。** **GZIP** **Deflate** **BZIP2** 注意使用複製活動來解壓縮 ZipDeflate 檔案並寫入檔案的接收器資料儲存時,檔案將擷取到資料夾`<path specified in dataset>/<folder named as source zip file>/`: 。
* **層級：** 壓縮比，它可以是**最佳**或**最快**。
...
## <a name="unsupported-file-types-and-compression-formats"></a>不支援的檔案型態和壓縮格式

可以使用 Azure 資料工廠的擴充功能來轉換不支援的檔案。
兩個選項包括使用 Azure 批次處理的 Azure 函數和自訂任務。

可以看到使用 Azure 函數[提取 tar 檔的內容](https://github.com/Azure/Azure-DataFactory/tree/master/SamplesV2/UntarAzureFilesWithAzureFunction)的範例。 有關詳細資訊,請參閱[Azure 函數活動](https://docs.microsoft.com/azure/data-factory/control-flow-azure-function-activity)。

您還可以使用自定義點網活動構建此功能。 更多資訊[可在此處](https://docs.microsoft.com/azure/data-factory/transform-data-using-dotnet-custom-activity)取得

## <a name="next-steps"></a>後續步驟

從[支援的檔案格式與壓縮中瞭解最新的支援的檔案格式和壓縮](supported-file-formats-and-compression-codecs.md)。
