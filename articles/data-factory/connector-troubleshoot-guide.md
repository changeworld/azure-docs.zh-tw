...
description: 瞭解如何在 Azure 數據工廠中解決連接器問題。
...
- **建議**:詳細檢查錯誤。 請參考 blob 說明https://docs.microsoft.com/rest/api/storageservices/blob-service-error-codes文件 : 如果需要説明,請聯繫儲存團隊。
...
### <a name="error-code--azureblobnotsupportmultiplefilesintosingleblob"></a>錯誤代碼:AzureBlob 不支援多個檔案到單一 Blob

- **訊息**:`Transferring multiple files into a single Blob is not supported. Currently only single file source is supported.`


### <a name="error-code--azurestorageoperationfailedconcurrentwrite"></a>錯誤代碼:Azure 儲存操作失敗並發寫入
...
- **原因**:Cosmos DB 將單個請求的大小限制為 2 MB。 公式為,請求大小 = 單個文檔大小 = 寫入批次處理大小。 如果文檔大小較大,則默認行為將導致請求大小過大。 您可以調整寫入批次處理大小。

- **解析度**:在複製活動接收器中,減小"寫入批處理大小"值(預設值為 10000)。

### <a name="error-message-unique-index-constraint-violation"></a>錯誤訊息:唯一索引約束衝突

- **錯誤 :** 將資料複製到 Cosmos DB 時,您發生以下錯誤:
...
- **原因**:有兩個可能的原因:
    ...
    - 如果使用**Insert**作為寫入行為,則此錯誤意味著源資料具有具有相同 ID 的行/物件。

    - 如果使用**Upsert**作為寫入行為,並且為容器設置了另一個唯一鍵,則此錯誤意味著源數據具有具有不同 ID 但定義的唯鑰相同值的行/物件。
...
- **原因**:使用的請求單位大於 Cosmos DB 中配置的可用 RU。 瞭解宇宙資料庫[如何從這裏](../cosmos-db/request-units.md#request-unit-considerations)計算RU。
...
### <a name="column-missing-in-column-mapping"></a>列對應中缺少欄位

- **症狀**:當您為Cosmos DB匯入用於列映射的架構時,某些列將丟失。 

- **原因**:ADF 從前 10 個 Cosmos DB 文件中推斷出架構。 如果某些列/屬性在這些文件中沒有值,則 ADF 不會檢測到它們,因此不會顯示它們。
...
- **原因**: 在 BSON 中表示 UUID 的方法有兩種 - UuidStarard 和 UuidLegacy。 默認情況下,UuidLegacy 用於讀取數據。 如果蒙戈DB中的 UUID 數據是 Uuid標準,則您將遭遇錯誤。
...
- **建議**:檢查 ADLS Gen2 引發的詳細錯誤訊息。 如果是由瞬態故障引起的,請重試。 如果您需要進一步説明,請聯絡 Azure 儲存支援,並在錯誤消息中提供請求 ID。

- **原因**:當錯誤訊息包含"禁止"時,您使用的服務主體或託管標識可能沒有足夠的許可權來訪問 ADLS Gen2。

- **建議**: 請參考文件: https://docs.microsoft.com/azure/data-factory/connector-azure-data-lake-storage#service-principal-authentication.

- **原因**:當錯誤消息包含"內部伺服器錯誤"時,錯誤由 ADLS Gen2 返回。

- **建議**:可能是由暫時性故障引起的,請重試。 如果問題仍然存在,請聯絡 Azure 儲存支援,並在錯誤消息中提供請求 ID。


### <a name="error-code--adlsgen2invalidurl"></a>錯誤代碼: AdlsGen2 無效網址
...
### <a name="error-code--adlsgen2operationfailedconcurrentwrite"></a>錯誤代碼:AdlsGen2操作失敗並發寫入

- **訊息**:`Error occurred when trying to upload a file. It's possible because you have multiple concurrent copy activities runs writing to the same file '%name;'. Check your ADF configuration.`


### <a name="error-code--adlsgen2timeouterror"></a>錯誤代碼:AdlsGen2超時錯誤
...
- **原因**:一個可能的原因是您使用的服務主體或託管標識沒有訪問特定資料夾/檔案的許可權。

- **解決方法**:對需要複製的所有資料夾和子資料夾授予相應的許可權。 請參考[此文件](connector-azure-data-lake-store.md#linked-service-properties)。

### <a name="error-message-failed-to-get-access-token-by-using-service-principal-adal-error-service_unavailable"></a>錯誤消息:無法使用服務主體獲取訪問權杖。 ADAL 錯誤:service_unavailable
...
- **原因**:如果錯誤消息包含"SqlException",SQL 資料庫將引發錯誤,指示某些特定操作失敗。

- **建議**:請在此參考文件中按 SQL 錯誤碼搜尋更多https://docs.microsoft.com/sql/relational-databases/errors-events/database-engine-events-and-errors詳細資訊: 。 如果您需要其他説明,請與 Azure SQL 支援部門聯繫。

- **原因**:如果錯誤訊息包含"具有 IP 位址的用戶端"...'不允許造訪伺服器",並且您嘗試連接到 Azure SQL 資料庫,通常是由 Azure SQL 資料庫防火牆問題引起的。

- **建議**:在 Azure SQL Server 防火牆配置中,啟用"允許 Azure 服務和資源存取此伺服器"選項。 參考文件: https://docs.microsoft.com/azure/sql-database/sql-database-firewall-configure.


### <a name="error-code--sqloperationfailed"></a>錯誤代碼:Sql 操作失敗

- **訊息**:`A database operation failed. Please search error to get more details.`

- **原因**:如果錯誤消息包含"SqlException",SQL 資料庫將引發錯誤,指示某些特定操作失敗。

- **建議**:如果 SQL 錯誤不明確,請嘗試將資料庫更改為最新的相容性級別「150」。。 它可以引發最新版本的 SQL 錯誤。 請參考詳細資訊文件: https://docs.microsoft.com/sql/t-sql/statements/alter-database-transact-sql-compatibility-level?view=sql-server-ver15#backwardCompat。
        有關解決 SQL 問題的疑難排解,請在此參考文件中搜尋 SQL 錯誤代碼https://docs.microsoft.com/sql/relational-databases/errors-events/database-engine-events-and-errors以瞭解更多詳細資訊: 。 如果您需要其他説明,請與 Azure SQL 支援部門聯繫。

- **原因**:如果錯誤消息包含「Pdw管理到本機InteropException」,通常是由於源列和接收器列大小不匹配造成的。

- **建議**:檢查源列和接收器列的大小。 如果您需要其他説明,請與 Azure SQL 支援部門聯繫。

- **原因**:如果錯誤消息包含"無效操作異常",則通常是由無效輸入數據引起的。

- **建議**:要確定遇到問題的行,請在複製活動上啟用容錯功能,該功能可以將有問題的行重定向到存儲以進行進一步調查。 參考文件: https://docs.microsoft.com/azure/data-factory/copy-activity-fault-tolerance.


### <a name="error-code--sqlunauthorizedaccess"></a>錯誤代碼:Sql 未經授權存取
...
### <a name="error-code--sqlopenconnectiontimeout"></a>錯誤代碼:SqlOpen 連接逾時

- **訊息**:`Open connection to database timeout after '%timeoutValue;' seconds.`

- **原因**:可能是 SQL 資料庫瞬態故障。

- **建議**:請重試更新具有較大連接超時值的連結服務連接字串。
...
- **原因**:如果問題發生在 SQL 源上,並且錯誤與 SqlDateTime 溢出有關,則數據值超過邏輯類型範圍(1/1/1753 12:00:00 AM - 12/31/9999 11:59:59 PM)。

- **建議**:在源 SQL 查詢或複製活動列映射中將類型轉換為字串,將列類型更改為"String"。

- **原因**:如果問題發生在 SQL 接收器上,並且錯誤與 SqlDateTime 溢出有關,則數據值超過接收器表中允許的範圍。

- **建議**:將相應的列類型更新為接收器表中的"datetime2"類型。


### <a name="error-code--sqlinvaliddbstoredprocedure"></a>錯誤代碼:SqlInvalidDb 儲存程序
...
- **原因**:指定的存儲過程無效。 這可能是由於存儲過程不返回任何數據造成的。

- **建議**:透過 SQL 工具驗證儲存過程。 確保存儲過程可以返回數據。
...
- **原因**:指定的 SQL 查詢無效。 這可能是由於查詢不返回任何數據造成的

- **建議**:按 SQL 工具驗證 SQL 查詢。 確保查詢可以返回數據。
...
- **建議**:驗證查詢中的列、數據集中的「結構」和活動中的「映射」。


### <a name="error-code--sqlcolumnnamemismatchbycasesensitive"></a>錯誤代碼:SqlColumnName 不符合使用區分大小寫
...
- **原因**:如果異常詳細信息不斷告訴事務超時,則集成運行時和資料庫之間的網路延遲高於預設閾值 30 秒。

- **建議**:更新 Sql 連結的服務連接字串,其"連接超時"值等於 120 或更高,然後重新運行活動。
...
### <a name="error-message-conversion-failed-when-converting-from-a-character-string-to-uniqueidentifier"></a>錯誤訊息:從字串轉換為唯一識別元時轉換失敗

- **症狀**:當您使用暫存複本和 PolyBase 將資料從表格資料來源(如 SQL Server)複製到 Azure SQL 資料倉儲時,您會遭遇以下錯誤:
...
- **解析度**:在「複製活動接收器」中,在「多邊基」設定下,將「**使用類型預設值**」選項設定為 false。

### <a name="error-message-expected-data-type-decimalxx-offending-value"></a>錯誤訊息:預期資料類型:DECIMAL(x,x),違規值

- **症狀**:當您使用暫存複本和 PolyBase 將資料從表格資料來源(如 SQL Server)複製到 SQL DW 時,您會遭遇以下錯誤:
...
- **原因**:可能的原因是架構(總列寬度)過大(大於 1 MB)。 以新增所有欄的大小來檢查目標 SQL DW 表的架構:
...
- **症狀**:使用 SQL 查詢從 Azure SQL 資料倉儲取資料,並命中以下錯誤:

    ```
    ...StorageException: The condition specified using HTTP conditional header(s) is not met...
    ```

- **原因**:Azure SQL 資料倉庫在 Azure 存儲中查詢外部表時出現問題。
...
### <a name="error-code--delimitedtextcolumnnamenotallownull"></a>錯誤代碼:分隔文字列名不允許無效
...
### <a name="error-code--delimitedtextmorecolumnsthandefined"></a>錯誤代碼:分隔文字更多列符,已定義
...
- **原因**:有問題的行的列計數大於第一行的列計數。 這可能是由數據問題或不正確的列分隔符/報價字元設置引起的。

- **建議**:請在錯誤消息中獲取行計數,檢查行的列並修復數據。

- **原因**:如果錯誤消息中的預期列計數為"1",則可能是您指定的壓縮或格式設置錯誤,從而導致 ADF 錯誤地分析檔。

- **建議**:檢查格式設置,以確保它與源檔匹配。

- **原因**:如果源是資料夾,則指定資料夾下的檔案可能具有不同的架構。

- **建議**:確保給定資料夾下的檔具有相同的架構。


### <a name="error-code--delimitedtextincorrectrowdelimiter"></a>錯誤代碼:分隔文字不正確的行限制器

- **訊息**:`The specified row delimiter %rowDelimiter; is incorrect. Cannot detect a row after parse %size; MB data.`


### <a name="error-code--delimitedtexttoolargecolumncount"></a>錯誤代碼:分隔文字太大欄位
...
### <a name="error-code--delimitedtextinvalidsettings"></a>錯誤代碼:分隔文字無效設定
...
### <a name="error-code--dynamicscreateserviceclienterror"></a>錯誤代碼:動態建立服務客戶端錯誤
...
### <a name="error-code--jsonconflictpartitiondiscoveryschema"></a>錯誤代碼:Json衝突分割區資料架構

- **訊息**:`Conflicting partition column names detected.'%schema;', '%partitionDiscoverySchema;'`


### <a name="error-code--jsoninvaliddataformat"></a>錯誤代碼:Json 不合法資料格式

- **訊息**:`Error occurred when deserializing source JSON file '%fileName;'. Check if the data is in valid JSON object format.`


### <a name="error-code--jsoninvaliddatamixedarrayandobject"></a>錯誤代碼: Json 不合法的數量
...
### <a name="error-code--parquetjavainvocationexception"></a>錯誤代碼:ParquetJavain呼叫異常

- **訊息**:`An error occurred when invoking java, message: %javaException;.`

- **原因**:當錯誤消息包含"java.lang.OutMemory"、"Java 堆空間"和"雙容量"時,通常是舊版本的集成運行時的記憶體管理問題。

- **建議**:如果您使用的是自託管集成運行時,並且版本早於 3.20.7159.1,建議升級到最新版本。

- **原因**:當錯誤消息包含"java.lang.OutMemory"時,集成運行時沒有足夠的資源來處理檔。

- **建議**:限制整合式執行時的併發運行。 對於自託管整合時,可擴展到記憶體等於或大於 8 GB 的強大電腦。

- **原因**:當錯誤消息包含"NullPointer 參考"時,可能是暫時性錯誤。

- **建議**:請重試。 如果問題仍然存在,請聯繫支持人員。


### <a name="error-code--parquetinvalidfile"></a>錯誤代碼:Parquet 無效檔案
...
- **原因**:Azure 數據工廠不支援鑲木地板格式。

- **建議**:仔細檢查源數據。 請參考文件: https://docs.microsoft.com/azure/data-factory/supported-file-formats-and-compression-codecs。


### <a name="error-code--parquetmisseddecimalprecisionscale"></a>錯誤代碼:鑲木地板十進製精度刻度
...
### <a name="error-code--parquetinvaliddecimalprecisionscale"></a>錯誤代碼:鑲木地板無效十進製精度刻度
...
### <a name="error-code--parquetcolumnnotfound"></a>錯誤代碼: ParquetColumn 找不到
...
### <a name="error-code--parquetinvaliddataformat"></a>錯誤代碼:Parquet 不合法資料格式

- **訊息**:`Incorrect format of %srcValue; for converting to %dstType;.`

- **原因**:資料無法轉換為對應對應的類型。

- **建議**:在複製活動列映射中仔細檢查源數據或指定此列的正確數據類型。 請參考文件: https://docs.microsoft.com/azure/data-factory/supported-file-formats-and-compression-codecs。


### <a name="error-code--parquetdatacountnotmatchcolumncount"></a>錯誤代碼:ParquetData( C) CountNotMatchColumnCount
...
- **訊息**: 資料型態 %srcType;與給定欄型態 %dstType 不匹配;在列「%列索引;」。

- **原因**:來源資料無法轉換為接收器中定義的類型化資料
...
### <a name="error-code--parquetunsupportfilelevelcompressionoption"></a>錯誤代碼:Parquet 不支援檔案等級壓縮選項
...
- **建議**:檢查整合式執行時環境,參考文件:https://docs.microsoft.com/azure/data-factory/format-parquet#using-self-hosted-integration-runtime


### <a name="error-code--wildcardpathsinknotsupported"></a>錯誤代碼:不支援通配子路徑

- **訊息**:`Wildcard in path is not supported in sink dataset. Fix the path: '%setting;'.`

- **原因**:接收器數據集不支援通配符。

- **建議**:檢查接收器數據集並修復沒有通配符值的路徑。


### <a name="error-code--mappinginvalidpropertywithemptyvalue"></a>錯誤代碼:對應的屬性與空值
...
### <a name="error-code--mappinginvalidordinalforsinkcolumn"></a>錯誤代碼:映射無效的「無效」,用於尋找問題柱
