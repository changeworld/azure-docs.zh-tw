...
| enableServerCertificateValidation | 指定當您使用透過 SSL/TLS 的 FTP 通道時，是否要啟用伺服器 TLS/SSL 憑證驗證。<br/>允許的值為：**true** (預設值)、**false**。 | 否 |
...
在以格式為基礎的資料集`location`的設定下，FTP 支援下列屬性：
...
在以格式為基礎的複製來源`storeSettings`的設定下，FTP 支援下列屬性：

| 屬性                 | 描述                                                  | 必要                                      |
| ------------------------ | ------------------------------------------------------------ | --------------------------------------------- |
| type                     | 底下的 type 屬性`storeSettings`必須設定為**FtpReadSettings**。 | 是                                           |
