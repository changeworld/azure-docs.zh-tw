...
Azure Data Lake Storage Gen2 是[Azure Blob 儲存體](../storage/blobs/storage-blobs-introduction.md)內建的大量資料分析專用的一組功能。 其可讓您使用檔案系統和物件儲存範例連接您的資料。
...
2. 在 [**新增資料**處理站] 頁面上，提供下圖所示欄位的值： 
    ...
    * **Name**：輸入 Azure 資料處理站的全域唯一名稱。 如果您收到「資料處理站名稱 \"LoadADLSDemo\" 無法使用」的錯誤，請為資料處理站輸入其他名稱。 例如，使用**您的名稱****ADFTutorialDataFactory**。 再次建立資料處理站。 如需 Data Factory 成品的命名規則，請參閱 [Data Factory 命名規則](naming-rules.md)。
...
14. 請注意，系統會自動選取左側的 [監視] 索引標籤。 [**動作**] 資料行會包含可查看活動執行詳細資料的連結，以及重新執行管線。
