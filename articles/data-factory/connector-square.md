...
| connectionProperties | 定義如何連接到正方形的屬性群組。 | 是 |
...
| accessToken | 從正方形取得的存取權杖。 藉由向經過驗證的使用者要求明確許可權，授與對正方形帳戶的有限存取權。 OAuth 存取權杖會在發行後30天過期，但重新整理權杖不會過期。 重新整理權杖可以重新整理存取權杖。<br>將此欄位標記為 SecureString，將它安全地儲存在 Data Factory 中，或[參考 Azure Key Vault 中儲存的祕密](store-credentials-in-key-vault.md)。  | 是 |
...
正方形支援兩種類型的存取權杖： **個人** 和 **OAuth**。

- 個人存取權杖是用來取得您自己的正方形帳戶中資源的無限制連接 API 存取。
- OAuth 存取權杖可用來取得任何方形帳戶的經過驗證和限定範圍的 Connect API 存取。 當您的應用程式代表帳戶擁有者存取其他正方形帳戶中的資源時，請使用它們。 OAuth 存取權杖也可以用來存取您自己的正方形帳戶中的資源。

在 Data Factory 中，透過個人存取權杖的驗證只需要 `accessToken` ，而透過 OAuth `accessToken` 進行驗證需要和 `refreshToken` 。 從 [這裡](https://developer.squareup.com/docs/build-basics/access-tokens)瞭解如何取出存取權杖。
