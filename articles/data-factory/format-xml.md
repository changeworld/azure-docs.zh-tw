...
| validationMode | 指定是否要驗證 XML 架構。<br>允許的值為**none** (預設、沒有驗證) 、 **XSD** (使用 xsd) 進行驗證、 **dtd** (使用 dtd) 進行驗證。 | 否 |
...
下表列出 XML 來源所支援的屬性。 您可以在 [**來源選項**] 索引標籤中編輯這些屬性。深入瞭解[XML 連接器行為](#xml-connector-behavior)。 使用內嵌資料集時，您會看到其他檔案設定，與[資料集屬性](#dataset-properties)一節中所述的屬性相同。 
...
| 驗證模式 | 指定是否要驗證 XML 架構。 | 否 | `None` (預設值，則不會驗證) <br>`xsd` (使用 XSD) 進行驗證<br>`dtd` (使用 DTD) 進行驗證。 | validationMode |
