...
有關可用於定義資料集的節和屬性的完整清單，請參閱[資料集](concepts-datasets-linked-services.md)一文。 本節提供 PayPal 資料集所支援的屬性清單。

若要從 PayPal 複製資料，請將資料集的 type 屬性設定為 **PayPalObject**。 以下是支援的屬性：

| 屬性 | 描述 | 必要 |
|:--- |:--- |:--- |
| type | 資料集的類型屬性必須設置為 **：PayPalObject** | 是 |
...
若要從 PayPal 複製資料，請將複製活動中的來源類型設定為 **PayPalSource**。 複製活動**源**部分支援以下屬性：

| 屬性 | 描述 | 必要 |
|:--- |:--- |:--- |
| type | 複製活動來源的 type 屬性必須設定為：**PayPalSource** | 是 |
| 查詢 | 使用自訂 SQL 查詢來讀取資料。 例如：`"SELECT * FROM Payment_Experience"`。 | 否 (如果已指定資料集中的 "tableName") |
