...
您可以使用 **AzDataFactoryV2IntegrationRuntime** PowerShell Cmdlet 建立 Integration Runtime。 若要建立 Azure IR，請指定命令的名稱、位置和類型。 以下是使用位置設定為 "West Europe" (歐洲西部) 來建立 Azure IR 的範例命令：
...
1. 在左窗格上選取 [整合執行階段]，然後選取 [+ 新增]。
   ...
   ![在左窗格中反白顯示整合執行時間和 [+ 新增] 按鈕的螢幕擷取畫面。](media/doc-common-process/manage-new-integration-runtime.png)
