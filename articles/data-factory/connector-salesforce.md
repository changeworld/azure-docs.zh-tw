...
您可以將資料從 Salesforce 複製到任何支援的接收資料存放區。 您也可以從任何支援的來源資料存放區將資料複製到 Salesforce。 有關"複製"活動支援為源或接收體的數據儲存的清單,請參閱[支持數據存儲](copy-activity-overview.md#supported-data-stores-and-formats)表。
