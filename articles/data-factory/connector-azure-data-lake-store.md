...
| 選項 3：檔案清單<br>- fileListPath | 表示複製指定的檔案集。 指向包含所要複製檔案清單的文字檔，每一行一個檔案，為資料集中所設定路徑的相對路徑。<br/>使用此選項時，請勿指定資料集中的檔案名稱。 請參閱[檔案清單範例](#file-list-examples)中更多的範例。 |否 |
| ***其他設定：*** |  | |
...
| ------------------------------------------------------------ | --------------------------------------------------------- | ------------------------------------------------------------ |
| root<br/>&nbsp;&nbsp;&nbsp;&nbsp;FolderA<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**File1.csv**<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;File2.json<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Subfolder1<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**File3.csv**<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;File4.json<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**File5.csv**<br/>&nbsp;&nbsp;&nbsp;&nbsp;Metadata<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;FileListToCopy.txt | File1.csv<br>Subfolder1/File3.csv<br>Subfolder1/File5.csv | **在資料集中：**<br>- 資料夾路徑：`root/FolderA`<br><br>**在複製活動來源中：**<br>- 檔案清單路徑：`root/Metadata/FileListToCopy.txt` <br><br>檔案清單路徑指向包含所要複製檔案清單的同一個資料存放區，為資料集中所設定路徑的相對路徑，每一行一個檔案。 |
...
在對應資料流中轉換資料時，您可以讀取和寫入 Azure Data Lake Storage Gen1 中 JSON、Avro、分隔文字或 Parquet 格式的檔案。 如需詳細資訊，請參閱對應資料流功能中的[來源轉換](data-flow-source.md)和[接收轉換](data-flow-sink.md)。

### <a name="source-transformation"></a>來源轉換

在「來源」轉換中，您可以從 Azure Data Lake Storage Gen1 中讀取容器、資料夾或個別檔案。 [來源選項] 索引標籤可讓您管理檔案的讀取方式。 
...
>[!NOTE]
>下列模型仍依原狀支援，以提供回溯相容性。 建議您繼續使用以上各節中提到的新模型，撰寫 UI 的 Azure Data Factory 已切換以產生新模型。
