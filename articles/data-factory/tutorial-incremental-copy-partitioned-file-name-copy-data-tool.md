---
title: 根據時間分區的檔案名增量複製新檔
description: 創建 Azure 資料工廠，然後使用"複製資料"工具僅根據時間分區的檔案名增量載入新檔。
...
ms.locfileid: "79501658"
---
# <a name="incrementally-copy-new-files-based-on-time-partitioned-file-name-by-using-the-copy-data-tool"></a>使用"複製資料"工具，根據時間分區的檔案名增量複製新檔

在這個教學課程中，您會使用 Azure 入口網站來建立資料處理站。 然後，使用"複製資料"工具創建一個管道，該管道根據從 Azure Blob 存儲到 Azure Blob 存儲的時間分區檔案名增量複製新檔。
...
1. 創建名為**源**的容器。  在容器中將資料夾路徑創建為**2020/03/17/03。** 創建空文字檔，並將其命名為**file1.txt**。 將 file1.txt 上載到存儲帳戶中的資料夾路徑**源/2020/03/17/03。**  您可以使用各種工具來執行這些工作，例如 [Azure 儲存體總管](https://storageexplorer.com/)。
...
2. 創建名為**目標的**容器。 您可以使用各種工具來執行這些工作，例如 [Azure 儲存體總管](https://storageexplorer.com/)。
...
5. 在 [版本]**** 下，選取 [V2]**** 作為版本。
6. 在**位置**下，選擇資料工廠的位置。 只有受到支援的位置會顯示在下拉式清單中。 資料處理站所使用的資料存放區 (例如 Azure 儲存體和 SQL Database) 和計算 (例如 Azure HDInsight) 可位於其他地區和區域。
7. 選取 [建立]****。
...
4. 在 [選擇輸入檔案或資料夾]**** 頁面上，執行以下步驟︰
    ...
    b. 在 **"檔載入行為**"下，選擇**增量載入：時間分區資料夾/檔案名**。

    c. 將動態資料夾路徑寫成**源/[年]/{每月}/{day}/{小時}，** 並更改格式，如下圖所示。 檢查**二進位副本**，然後按一下 **"下一步**"。

    ![選擇輸入檔案或資料夾](./media/tutorial-incremental-copy-partitioned-file-name-copy-data-tool/check-binary-copy.png)     

5. 在 **"目標資料存儲"** 頁上，選擇**AzureBlob 存儲**，這是與資料來源存儲相同的存儲帳戶，然後按一下"**下一步**"。
...
6. 在 **"選擇輸出檔案"或"資料夾**"頁上，執行以下步驟：

    a. 流覽並選擇**目標**資料夾，然後按一下"**選擇**"。
...
10. 請注意，系統會自動選取左側的 [監視]**** 索引標籤。  當管道自動觸發時（大約一小時後），您需要等待管道的運行。 運行時，按一下管道名稱連結**DeltaCopyFromBlobPipeline**以查看活動運行詳細資訊或重新運行管道。 選取 [重新整理]**** 可重新整理清單。

    ![監視管線回合](./media/tutorial-incremental-copy-partitioned-file-name-copy-data-tool/monitor-pipeline-runs1.png)
11. 管線中只有一個活動 (複製活動)，所以您只會看到一個項目。 調整**源**列**和目標**列的列寬度（如有必要）以顯示更多詳細資訊，您可以看到原始檔案（file1.txt）已從*源/2020/03/17/03/* 目標/具有相同檔案名*複製到目標/2020/03/17/03。* 

    ![監視管線回合](./media/tutorial-incremental-copy-partitioned-file-name-copy-data-tool/monitor-pipeline-runs2.png)

    還可以通過使用 Azure 存儲資源管理器（掃描檔）https://storageexplorer.com/)來驗證相同的內容。

    ![監視管線回合](./media/tutorial-incremental-copy-partitioned-file-name-copy-data-tool/monitor-pipeline-runs3.png)

12. 創建另一個空文字檔，新名稱為**file2.txt**。 將 file2.txt 檔上載到存儲帳戶中的資料夾路徑**源/2020/03/17/04。** 您可以使用各種工具來執行這些工作，例如 [Azure 儲存體總管](https://storageexplorer.com/)。

    > [!NOTE]
    > 您可能知道需要創建新的資料夾路徑。 請用您的 UTC 時間調整資料夾名稱。  例如，如果當前 UTC 時間是 2020 年 3 月 17 日淩晨 4：20，則可以按 **[年]/{月]/天/{小時}的規則**將資料夾路徑創建為**源/2020/03/17/04/。**

13. 要返回 **"管道運行"** 視圖，請選擇 **"所有管道運行**"，然後等待同一管道在一小時後再次自動觸發。  

    ![監視管線回合](./media/tutorial-incremental-copy-partitioned-file-name-copy-data-tool/monitor-pipeline-runs5.png)

14. 選擇新的**DeltaCopyFromBlobPipeline**連結，以便第二個管道在管道運行時運行，並執行相同的操作來查看詳細資訊。 您將看到原始檔案 （file2.txt） 已從**源/2020/03/17/04/** 複製到**目標/2020/03/17/04/** 具有相同的檔案名。 還可以通過使用 Azure 存儲資源管理器（掃描**目標**容器中https://storageexplorer.com/)的檔）來驗證相同的內容。
