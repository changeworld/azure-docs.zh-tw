...
資料流程會在執行時間啟動的 Spark 叢集上執行。 所用叢集的設定是在 (IR) 活動的整合執行時間中定義。 定義您的整合執行時間時，需要進行三個效能考慮：叢集類型、叢集大小和存留時間。
...
建立具有多個資料流程的複雜管線時，您的邏輯流程對計時和成本可能會有很大的影響。 本節涵蓋不同架構策略的影響。

### <a name="executing-data-flows-in-parallel"></a>平行執行資料流程

如果您以平行方式執行多個資料流程，ADF 會針對每個活動加速個別的 Spark 叢集。 這可讓每個工作以平行方式隔離並執行，但會導致多個叢集同時執行。

如果您的資料流程平行執行，建議不要啟用 [Azure IR 存留時間] 屬性，因為這會導致多個未使用的暖集區。

> [!TIP]
> 不是在中針對每個活動多次執行相同的資料流程，而是在 data lake 中暫存您的資料，並使用萬用字元路徑來處理單一資料流程中的資料。

### <a name="execute-data-flows-sequentially"></a>循序執行資料流程

如果您依序執行資料流程活動，建議您在 Azure IR 設定中設定 TTL。 ADF 會重複使用計算資源，進而加快叢集啟動的時間。 每個活動仍會隔離，每次執行時都會收到新的 Spark 內容。

循序執行工作可能會花費最長的時間來執行端對端，但會提供邏輯作業的清楚分隔。

### <a name="overloading-a-single-data-flow"></a>多載單一資料流程

如果您將所有邏輯放在單一資料流程中，ADF 將會在單一 Spark 實例上執行整個作業。 雖然這看起來像是降低成本的方法，但它會將不同的邏輯流程混合在一起，而且可能很難以監視和進行調試。 如果其中一個元件失敗，工作的其他所有部分也會失敗。 Azure Data Factory 小組建議以獨立的商務邏輯流程來組織資料流程程。 如果您的資料流程變得太大，請將它分割成分隔元件，讓監視和偵錯工具變得更容易。 雖然資料流程中的轉換數目沒有硬性限制，但有太多的時間會使作業變得複雜。
