...
我們建議針對生產環境的大部分工作負載使用**記憶體優化**。 您將能夠將更多的資料儲存在記憶體中，並將記憶體不足的錯誤降到最低。 記憶體優化比計算優化的每個核心具有更高的價格點，但可能會導致更快速的轉換速度和更成功的管線。 如果您在執行資料流程時遇到記憶體不足的錯誤，請切換至記憶體最佳化的 Azure IR 設定。

**計算優化**可以滿足有限資料列數目的 debug 和 data preview。 計算優化可能也不會與生產工作負載一起執行。
...
在每次轉換時，您都可以在 [最佳化] 索引標籤中設定要讓資料處理站使用的資料分割配置。比較好的方式是在保留預設分割和最佳化的情況下，先測試檔案型的接收。 將資料分割保留在檔案目的地接收中的「目前分割」，可讓 Spark 為您的工作負載設定適當的預設資料分割。 預設資料分割會針對每個分割區使用128Mb。
...
## <a name="join-and-lookup-performance"></a>聯結和查閱效能
...
對於具有許多聯結作業的資料流程，建議的設定是將優化設為「自動」，以進行「廣播」，並使用***記憶體優化***Azure Integration Runtime 設定。 如果您在資料流程執行期間遇到記憶體不足的錯誤或廣播逾時，您可以關閉廣播最佳化。 不過，這會造成資料流程的執行速度變慢。 (選擇性) 您可以指示資料流程只下推聯結的左側或右側 (或兩者)。
