...
description: 瞭解如何使用 Azure Data Factory 管線中的複製活動，從 Microsoft Access 複製資料及將資料複製到其中。
...
---
# <a name="copy-data-from-and-to-microsoft-access-using-azure-data-factory"></a>使用 Azure Data Factory 從 Microsoft Access 複製資料及將資料複製到其中
...
下列活動支援此 Microsoft Access connector：

- 含[支援來源/接收器矩陣](copy-activity-overview.md)的[複製活動](copy-activity-overview.md)
- [查閱活動](control-flow-lookup-activity.md)

您可以將資料從 Microsoft Access 來源複製到任何支援的接收資料存放區，或從任何支援的來源資料存放區複製到 Microsoft Access sink。 如需複製活動所支援作為來源/接收器的資料存放區清單，請參閱[支援的資料存放區](copy-activity-overview.md#supported-data-stores-and-formats)表格。

## <a name="prerequisites"></a>Prerequisites

若要使用此 Microsoft Access connector，您需要：

- 設定一個「自我裝載 Integration Runtime」。 如需詳細資訊，請參閱自我裝載的 [Integration Runtime](create-self-hosted-integration-runtime.md) 文章。
- 在 Integration Runtime 電腦上安裝資料存放區的 Microsoft Access ODBC 驅動程式。

>[!NOTE]
>Microsoft Access 2016 版的 ODBC 驅動程式不適用於此連接器。 請改用驅動程式版本2013或2010。

## <a name="getting-started"></a>開始使用

[!INCLUDE [data-factory-v2-connector-get-started](../../includes/data-factory-v2-connector-get-started.md)]

下列各節提供屬性的相關詳細資料，這些屬性是用來定義 Microsoft Access connector 專用 Data Factory 實體。

## <a name="linked-service-properties"></a>連結服務屬性

以下是 Microsoft Access 已連結服務支援的屬性：

| 屬性 | 說明 | 必要 |
|:--- |:--- |:--- |
| type | Type 屬性必須設定為： **MicrosoftAccess** | 是 |
| connectionString | 排除認證部分的 ODBC 連接字串。 您可以指定連接字串，或使用系統 DSN (資料來源名稱) 您在 Integration Runtime 機上設定 (您仍然需要在連結服務中，據此指定認證部分) 。<br> 您也可以將密碼放在 Azure Key Vault 中，並  `password`   從連接字串中提取設定。如需詳細資訊，請參閱 [Azure Key Vault 中的儲存認證](store-credentials-in-key-vault.md)   。| 是 |
...
若要從 Microsoft Access 複製資料，[複製活動 **來源** ] 區段支援下列屬性：

| 屬性 | 說明 | 必要 |
|:--- |:--- |:--- |
| type | 複製活動來源的 type 屬性必須設定為： **MicrosoftAccessSource** | 是 |
...
若要將資料複製到 Microsoft Access，複製活動 **接收** 區段支援下列屬性：

| 屬性 | 說明 | 必要 |
|:--- |:--- |:--- |
| type | 複製活動接收的 type 屬性必須設定為： **MicrosoftAccessSink** | 是 |
