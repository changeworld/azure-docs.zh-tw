...
2. 在 [新增資料處理站]**** 頁面中，為下圖所示的欄位提供值：
    ...
    * **名稱**：輸入 Azure 資料處理站的全域唯一名稱。 如果收到錯誤「數據工廠名稱*LoadFromOffice365Demo*不可用」,請輸入數據工廠的其他名稱。 例如，您可以使用_**yourname**_**LoadFromOffice365Demo**。 請嘗試再次建立資料處理站。 如需 Data Factory 成品的命名規則，請參閱 [Data Factory 命名規則](naming-rules.md)。
...
3. 現在,您位於「複製活動配置」選項卡中。按一下 Office 365 資料集旁邊的 **「編輯」** 按鈕以繼續數據配置。

    ![設定 Office 365 資料集的一般資訊](./media/load-office-365-data/transition-to-edit-dataset.png)
 
4. 您會看到已為 Office 365 資料集開啟一個新的索引標籤。 在"屬性"視窗底部的 **"常規"選項卡**中,輸入名稱的"SourceOffice365資料集數據集"。
 
5. 跳到屬性的視窗的**連線' 選項 。** 在 [已連結的服務] 文字方塊旁，按一下 [+ 新增]****。
...
7. 在連結服務建立後，您會回到資料集設定。 在**表**旁邊,選擇向下箭頭以展開可用 Office 365 數據集的清單,然後選擇"BasicDataSet_v0。下拉清單中的Message_v0":

    ![設定 Office 365 資料集資料表](./media/load-office-365-data/edit-dataset.png)

8. 現在返回**管道** > **源選項卡**,繼續配置 Office 365 資料提取的其他屬性。  使用者範圍和使用者範圍篩選器是可選的謂詞,您可以定義這些謂詞來限制要從 Office 365 中提取的數據。 有關如何配置這些設置,請參閱[Office 365 資料集屬性](https://docs.microsoft.com/azure/data-factory/connector-office-365#dataset-properties)部分。

9. 您需要選擇日期篩選器並提供開始時間和結束時間值。

10. 按一下「**導入架構」** 選項卡以導入消息數據集的架構。
...
2. 在"新建數據集"視窗中,請注意,從 Office 365 複製時,僅選擇支持的目標。 選擇**Azure Blob 儲存**,選擇二進位格式,然後選擇「**繼續**」 。  在此教學課程中，您會將 Office 365 資料複製到「Azure Blob 儲存體」中。

3. 按下 Azure Blob 儲存資料集旁邊的 **「編輯」** 按鈕以繼續資料配置。

4. 在 [屬性] 視窗的 [一般]**** 索引標籤上，於 [名稱] 中輸入 "OutputBlobDataset"。

5. 跳到屬性的視窗的**連線' 選項 。** 在 [連結服務] 文字方塊旁，選取 [+ 新增]****。

6. 在"新建連結服務"視窗中,輸入"AzureStorageLinkService"作為名稱,從身份驗證方法的下拉清單中選擇"服務主體",填寫服務終結點、租戶、服務主體 ID 和服務主體密鑰,然後選擇"保存"以部署鏈接服務。  如需了解如何設定「Azure Blob 儲存體」的服務主體驗證，請參閱[這裡](connector-azure-blob-storage.md#service-principal-authentication)。
...
移至左側的 [監視]**** 索引標籤。 您會看到手動觸發程序所觸發的管線執行。 您可以使用 **「操作」** 欄中的連結查看活動詳細資訊並重新執行管道。
...
如果這是您第一次請求此上下文的數據(正在訪問的哪個數據表的組合,哪個目標帳戶是要載入到的數據,哪個使用者標識正在發出資料訪問請求),您將看到複製活動狀態為 **"正在進行**",並且只有在單擊"操作"下的"詳細資訊"連結時,您才會看到狀態為 **"重新設置同意**"。  資料存取核准者群組的成員必須先在 Privileged Access Management 中核准該要求，資料擷取才能繼續。
...
一旦提供了同意,數據提取將繼續,並在一段時間后,管道運行將顯示為成功。

![監視管線 - 成功](./media/load-office-365-data/pipeline-monitoring-succeeded.png) 

現在轉到目標 Azure Blob 儲存,並驗證 Office 365 資料是否以二進位格式提取。
