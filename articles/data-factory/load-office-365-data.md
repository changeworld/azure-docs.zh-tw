...
2. 在 [新增資料處理站] 頁面中，為下圖所示的欄位提供值：
    ...
    * **Name**：輸入 Azure 資料處理站的全域唯一名稱。 如果您收到「Data factory 名稱 *>loadfromoffice365demo* 無法使用」錯誤，請為資料處理站輸入不同的名稱。 例如，您可以使用_**yourname**_**LoadFromOffice365Demo**。 請嘗試再次建立資料處理站。 如需 Data Factory 成品的命名規則，請參閱 [Data Factory 命名規則](naming-rules.md)。
...
8. 現在回到 [**管線**  >  **來源]** 索引標籤，繼續設定 Office 365 資料解壓縮的其他屬性。  使用者範圍和使用者範圍篩選器是選擇性的述詞，您可以定義此述詞來限制您想要從 Office 365 取出的資料。 如需如何設定這些設定的詳細資料，請參閱 [Office 365 資料集屬性](https://docs.microsoft.com/azure/data-factory/connector-office-365#dataset-properties) 一節。
