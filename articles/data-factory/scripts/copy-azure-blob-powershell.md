...
* 在 Blob 儲存體中建立一個 **Blob 容器**，在容器中建立一個輸入**資料夾**，然後上傳一些檔案到資料夾中。 您可以使用 [Azure 儲存體總管](https://azure.microsoft.com/features/storage-explorer/) 之類的工具來連線至 Azure blob 儲存體、建立 Blob 容器、上傳輸入檔案，以及驗證輸出檔。
