...
13. 若要切換回 [管線執行] 視圖，請選取頂端的 [ **所有管線執行** ] 連結。 選取 [重新整理] 即可重新整理清單。

    ![監視活動回合](./media/load-azure-sql-data-warehouse/activity-monitoring.png)

14. 若要監視每個複製活動的執行詳細資料，請選取 [活動執行] 眼鏡中 [ **活動名稱** ] 下的 [ **詳細資料** ] 連結 (圖示) 。 您可以監視詳細資料，例如從來源複製到接收的資料量、資料輸送量、執行步驟與對應的持續時間，以及使用的設定。
