...
Data Factory 提供三種類型的 Integration Runtime （IR），您應該選擇最適合資料整合功能的類型，以及您要尋找的網路環境需求。  這三種類型為：
...
活動分派是將活動路由傳送至目標計算服務的輕量作業，因此不需要相應增加此案例的計算大小。
...
- 針對內部部署或 Azure 虛擬網路中的計算資源，分派下列轉換活動： HDInsight Hive 活動（BYOC-攜帶您自己的叢集）、HDInsight Pig 活動（BYOC）、hdinsight MapReduce 活動（BYOC）、HDInsight Spark 活動（BYOC）、HDInsight 串流活動（BYOC）、Machine Learning 批次執行活動、Machine Learning 更新資源活動、預存程式活動、Data Lake Analytics U-SQL 活動、自訂活動（在 Azure Batch 上執行）、查閱活動，以及取得中繼資料活動。
...
如果您想要在私人網路環境中安全地執行資料整合，而這種方式無法從公用雲端環境直接進行連線，您可以在公司防火牆後方或虛擬私人網路內安裝自我裝載的 IR 內部部署環境。  自我裝載整合執行階段對於開放網際網路，只會建立以 HTTP 為基礎的輸出連線。
...
當客戶建立 data factory 實例時，他們需要指定資料處理站的位置。 Data Factory 位置中儲存資料處理站的中繼資料，也是觸發管道的源頭。 Factory 的中繼資料只會儲存在客戶選擇的區域中，而且不會儲存在其他區域中。
...
- 針對複製活動，ADF 會盡力自動偵測接收資料存放區的位置，然後在相同的區域中使用 IR （如果有的話），或位於相同地理位置中最接近的一處;如果無法偵測接收資料存放區的區域，則會使用 data factory 區域中的 IR 做為替代。
  ...
  - 將資料複製到美國西部的 Azure Blob 時，如果 ADF 成功偵測到該 Blob 位於美國西部，則複製活動會在美國西部的 IR 上執行;如果區域偵測失敗，則會在美國東部的 IR 上執行複製活動。
...
- 針對查閱/GetMetadata/刪除活動執行（也稱為管線活動）、轉換活動分派（也稱為外部活動）和撰寫作業（測試連線、流覽資料夾清單和資料表清單、預覽資料），ADF 會使用 data factory 區域中的 IR。
...
- 在**兩個雲端資料來源之間複製**：當來源和接收連結服務都使用 Azure IR 時，ADF 會使用區域 Azure IR （如果您指定），或如果您選擇「自動解析 IR」（預設值）（如[Integration runtime 位置](#integration-runtime-location)一節中所述），則會自動決定 Azure IR 的位置。
