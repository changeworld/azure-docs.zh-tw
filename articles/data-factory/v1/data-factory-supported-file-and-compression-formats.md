...
- `jsonPathDefinition` 指定每個資料行的 JSON 路徑，以指出從哪裡擷取資料。 要從陣列複製資料，可以使用**陣列_x_屬性**從 x 物件中提取給定屬性的值，也可以使用**陣列\屬性**從包含此類屬性的任何物件中查找值。
...
* **類型：** 壓縮編解碼器，可以是GZIP、Deflate、BZIP2或**ZipDeflate。** **GZIP** **Deflate** **BZIP2**  
* **層級：** 壓縮比，它可以是**最佳**或**最快**。
...
* 從 Azure blob 讀取 GZIP 壓縮資料，將其解壓縮，並將結果資料寫入到 Azure SQL 資料庫。 使用`compression``type`JSON 屬性將輸入 Azure Blob 資料集定義為 GZIP。
* 從來自內部部署檔案系統之純文字檔案讀取資料、使用 GZip 格式加以壓縮並將壓縮的資料寫入到 Azure blob。 將輸出 Azure Blob 資料集與`compression``type`JSON 屬性定義為 GZip。
* 從 FTP 伺服器讀取.zip 檔、將它解壓縮以取得其中的檔案，並將這些檔案放入 Azure Data Lake Store。 將輸入 FTP 資料集定義為`compression``type`ZipDeflate。
* 從 Azure blob 讀取 GZIP 壓縮資料，將其解壓縮、使用 BZIP2 將其壓縮，並將結果資料寫入到 Azure blob。 在這種情況下，將輸入 Azure Blob`compression``type`資料集設置為 GZIP，並將輸出`compression``type`資料集設置為 BZIP2。   


## <a name="next-steps"></a>後續步驟
如需 Azure Data Factory 所支援的檔案型資料存放區，請參閱下列文章：

- [Azure Blob 存儲](data-factory-azure-blob-connector.md)
- [Azure 資料湖存儲](data-factory-azure-datalake-connector.md)
