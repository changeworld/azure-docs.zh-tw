---
title: Azure 資料工廠中具有資料管理閘道的高可用性
...
具有多個節點的閘道會讓所有節點的資料存放區認證保持同步。 如果存在節點到節點連接問題，則憑據可能不同步。為使用閘道的本地資料存儲設置憑據時，它會在調度程式/輔助節點上保存憑據。 發送器節點會與其他背景工作節點進行同步處理。 此過程稱為**憑據同步**。節點之間的通信通道可以通過公共 SSL/TLS 證書**進行加密**。 
...
2. 瀏覽至 Azure 入口網站。 啟動您資料處理站的 [Data Factory]**** 頁面。 按一下 [已連結的服務] 圖格來啟動 [已連結的服務]**** 頁面。 選擇閘道以啟動**閘道頁面**。 按一下並啟用 [預覽功能]****，如下圖所示： 
...
- 支援 Windows Server 2012 R2 支援 TLS/SSL 憑證的任何金鑰大小。
- 不支援使用 CNG 金鑰的憑證。
...
並行作業 (執行中/限制) | 每個節點上執行的作業或工作數目。 這個值是近乎即時的快照集。 限制表示每個節點的最大並行作業數。 這個值會根據機器大小來定義。 在 CPU/記憶體/網路利用率不足，但活動超時的高級方案中，可以增加擴展併發作業執行的限制。此功能也可用於單節點閘道（即使未啟用可伸縮性和可用性功能）。 如需詳細資訊，請參閱[調整考量](#scale-considerations)一節。 
...
- 單一邏輯閘道目前最多可以有四個實體閘道節點。 出於性能原因，如果您需要四個以上節點，請發送電子郵件至[DMGHelp@microsoft.com](mailto:DMGHelp@microsoft.com)。
- 您無法從另一個邏輯閘道使用驗證金鑰來重新註冊閘道節點，以跳脫目前的邏輯閘道。 若要重新註冊，請從節點解除安裝閘道、重新安裝閘道，然後使用驗證金鑰向其他邏輯閘道進行註冊。 
- 如果所有閘道節點都需要 HTTP 代理，請將代理設置在 diahost.exe.config 和 diawp.exe.config 中，並使用伺服器管理員確保所有節點具有相同的 diahost.exe.config 和 diawip.exe.config。有關詳細資訊，請參閱[配置代理設置](data-factory-data-management-gateway.md#configure-proxy-server-settings)部分。 
- 若要在閘道組態管理員中變更節點間通訊的加密模式，請在入口網站中只留下一個節點而刪除其他所有節點。 然後，於加密模式變更後重新新增節點。
- 如果您選擇加密節點到節點的通信通道，請使用官方 TLS 證書。 自我簽署憑證可能會造成連線問題，因為相同的憑證可能不會受到其他機器上的憑證授權單位清單所信任。 
