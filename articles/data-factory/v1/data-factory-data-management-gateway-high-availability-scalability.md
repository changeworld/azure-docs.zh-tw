---
title: Azure Data Factory 中的資料管理閘道的高可用性
...
具有多個節點的閘道會讓所有節點的資料存放區認證保持同步。 如果有節點對節點連線問題，認證可能不同步。當您為使用閘道的內部部署資料存放區設定認證時，它會將認證儲存在發送器/背景工作節點上。 發送器節點會與其他背景工作節點進行同步處理。 此程式稱為 **認證同步** 處理。節點之間的通道可以使用公用 SSL/TLS 憑證來 **加密** 。 
...
3. 如果您選擇 [快速設定]
    ...
    2. 遵循[這些指示](data-factory-data-management-gateway.md#configuration-manager)來啟動「資料管理組態管理員」。 您會看到閘道名稱、節點名稱、狀態等資訊。
        ...
        ![顯示您可以在何處查看閘道名稱、節點名稱和狀態的螢幕擷取畫面。](media/data-factory-data-management-gateway-high-availability-scalability/data-factory-gateway-installation-success.png)
4. 如果您選擇 [手動設定]：
...
6. 在 Azure 入口網站啟動 [閘道] 頁面： 
    1. 在入口網站中的資料處理站首頁上，按一下 [已連結的服務]。
        ...
        ![醒目顯示 [已連結的服務] 磚的螢幕擷取畫面。](media/data-factory-data-management-gateway-high-availability-scalability/data-factory-home-page.png)
    2. 選取 **閘道** 以查看 [閘道] 頁面：
...
2. 瀏覽至 Azure 入口網站。 啟動您資料處理站的 [Data Factory] 頁面。 按一下 [已連結的服務] 圖格來啟動 [已連結的服務] 頁面。 選取要啟動 **閘道頁面** 的閘道。 按一下並啟用 [預覽功能]，如下圖所示： 
...
並行作業 (執行中/限制) | 每個節點上執行的作業或工作數目。 這個值是近乎即時的快照集。 限制表示每個節點的最大並行作業數。 這個值會根據機器大小來定義。 您可以增加限制，在先進的案例中擴大並行工作執行，其中 CPU/記憶體/網路使用率過低，但活動已過期。這項功能也適用于單一節點閘道， (即使未啟用) 的擴充性和可用性功能也是一樣。 如需詳細資訊，請參閱[調整考量](#scale-considerations)一節。 
...
- 單一邏輯閘道目前最多可以有四個實體閘道節點。 如果您基於效能考慮需要四個以上的節點，請傳送電子郵件至 [DMGHelp@microsoft.com](mailto:DMGHelp@microsoft.com) 。
- 您無法從另一個邏輯閘道使用驗證金鑰來重新註冊閘道節點，以跳脫目前的邏輯閘道。 若要重新註冊，請從節點解除安裝閘道、重新安裝閘道，然後使用驗證金鑰向其他邏輯閘道進行註冊。 
- 如果所有閘道節點都需要 HTTP proxy，請在 diahost.exe.config 中設定 proxy 並 diawp.exe.config，然後使用伺服器管理員來確定所有節點都有相同的 diahost.exe.config 和 diawip.exe.config。如需詳細資訊，請參閱 [設定 proxy 設定](data-factory-data-management-gateway.md#configure-proxy-server-settings) 一節。 
- 若要在閘道組態管理員中變更節點間通訊的加密模式，請在入口網站中只留下一個節點而刪除其他所有節點。 然後，於加密模式變更後重新新增節點。
- 如果您選擇加密節點對節點通道，請使用官方的 TLS 憑證。 自我簽署憑證可能會造成連線問題，因為相同的憑證可能不會受到其他機器上的憑證授權單位清單所信任。 
