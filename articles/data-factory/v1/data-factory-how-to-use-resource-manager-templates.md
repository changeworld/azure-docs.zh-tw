...
假設某個組織在全球各地有 10 個製造工廠。 每個工廠中的記錄都會儲存在不同的內部部署 SQL Server 資料庫中。 該公司希望在雲中構建單個數據倉庫,以便進行特殊分析。 它也想要具有相同的邏輯，但開發、測試和生產環境的組態不同。
