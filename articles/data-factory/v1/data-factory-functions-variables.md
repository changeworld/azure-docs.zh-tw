...
> [!IMPORTANT]
> 在另一個函式中使用函式時，您不需要**$$** 針對內部函式使用前置詞。 例如：$$Text.Format('PartitionKey eq \\'my_pkey_filter_value\\' and RowKey ge \\'{0: yyyy-MM-dd HH:mm:ss}\\'', Time.AddHours(SliceStart, -6)). 在此範例中，請**$$** 注意**time.addhours**函式不會使用前置詞。 
