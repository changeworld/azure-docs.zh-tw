...
(選擇性) 若要深入了解 Batch，請參閱 [Batch 文件](../../batch/index.yml)。
...
(選擇性) 若要深入了解 Data Factory，請參閱 [Data Factory 文件](/rest/api/datafactory/v1/data-factory-data-factory)。
...
1. 逐一查看輸入集合的成員可在 [Microsoft.WindowsAzure.Storage.Blob](/java/api/com.microsoft.azure.storage.blob) 命名空間中找到。 若要逐一查看 Blob 集合，您必須使用 **BlobContinuationToken** 類別。 基本上，您必須搭配使用 do-while 迴圈和權杖做為結束迴圈的機制。 如需詳細資訊，請參閱[從 .NET 使用 Blob 儲存體](../../storage/blobs/storage-quickstart-blobs-dotnet.md)。 基本迴圈如下所示：
    ...
    ```
   如需詳細資訊，請參閱 [ListBlobsSegmented](/java/api/com.microsoft.azure.storage.blob.cloudblobcontainer.listblobssegmented) 方法的文件。
...
* [開始使用 Power BI Desktop](/power-bi/fundamentals/desktop-getting-started)
