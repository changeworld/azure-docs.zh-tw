...
> [!NOTE]
> 本文適用於 Data Factory 第 1 版。 如果您使用目前版本的 Data Factory 服務，請參閱[第 2 版中的 Cassandra 連接器](../connector-cassandra.md)。

本文說明如何使用 Azure Data Factory 中的「複製活動」，從內部部署的 Cassandra 資料庫移動資料。 它基於[「資料移動活動」](data-factory-data-movement-activities.md)一文,其中概述複製活動的資料移動。

您可以將資料從內部部署的 Cassandra 資料存放區複製到任何支援的接收資料存放區。 有關複製活動支援為接收器支援的數據儲存的清單,請參閱[支援資料存儲](data-factory-data-movement-activities.md#supported-data-stores-and-formats)表。 Data Factory 目前只支援將資料從 Cassandra 資料存放區移到其他資料存放區，而不支援將資料從其他資料存放區移到 Cassandra 資料存放區。
...
- 建立導管的最簡單方法是使用**複製精靈**。 如需使用複製資料精靈建立管線的快速逐步解說，請參閱 [教學課程︰使用複製精靈建立管線](data-factory-copy-data-wizard-tutorial.md) 。
...
| 屬性 | 描述 | 必要 |
| --- | --- | --- |
| type |類型屬性必須設為： **OnPremisesCassandra** |是 |
...
**TypeProperties**部分對於每種類型的數據集都不同,並提供有關數據存儲中數據位置的資訊。 **CassandraTable** 類型資料集的 typeProperties 區段有下列屬性
...
此示例提供了使用[Visual Studio](data-factory-copy-activity-tutorial-using-visual-studio.md)或[Azure PowerShell](data-factory-copy-activity-tutorial-using-powershell.md)創建管道的範例 JSON 定義。 它示範如何將資料從內部部署的 Cassandra 資料庫複製到「Azure Blob 儲存體」。 不過，您可以在 Azure Data Factory 中使用複製活動，將資料複製到 [這裡](data-factory-data-movement-activities.md#supported-data-stores-and-formats) 所說的任何接收器。
...
* 每個集合資料行的 **虛擬資料表** ，以展開巢狀資料。 表示集合的虛擬表使用真實表的名稱、分隔符 *「vt」* 和列的名稱命名。
...
從關聯式資料存放區複製資料時，請將可重複性謹記在心，以避免產生非預期的結果。 在 Azure Data Factory 中，您可以手動重新執行配量。 您也可以為資料集設定重試原則，使得在發生失敗時，重新執行配量。 以上述任一方式重新執行配量時，您必須確保不論將配量執行多少次，都會讀取相同的資料。 請參考[從關聯原始讀取的可重複讀取](data-factory-repeatable-copy.md#repeatable-read-from-relational-sources)。
