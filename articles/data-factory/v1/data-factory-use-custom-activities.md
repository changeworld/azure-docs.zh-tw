...
- **linkedservices.json 和 datasets.json**。 這個屬性是活動之輸入/輸出資料集所參考的資料存放區連結服務的可列舉清單。
...
2. 使用下列 JSON 指令碼取代右窗格中的 JSON︰
   ...
   * **AssemblyName**會設定為 dll 的名稱： **MyDotnetActivity**。
...
3. 如果輸入配量未設定為 [**就緒**]，請確認輸入資料夾結構正確，而且輸入資料夾中有 **.txt** 。
