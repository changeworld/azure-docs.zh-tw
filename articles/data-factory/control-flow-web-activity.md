...
> [!NOTE]
> Web 活動也支援叫用裝載于私人虛擬網路中的 Url，並利用自我裝載整合執行時間。 整合執行時間應該可以看到 URL 端點。 
...
connectVia | 用來連線到資料存放區的[整合執行階段](https://docs.microsoft.com/azure/data-factory/concepts-integration-runtime)。 您可以使用 Azure integration runtime 或自我裝載整合執行時間（如果您的資料存放區位於私人網路中）。 如果未指定此屬性，服務就會使用預設的 Azure integration runtime。 | 整合執行時間參考。 | 否 
