...
1. 在左側選取 [**連接**]。 在畫面頂端，您會看到您的 ASN 與 Microsoft 之間的對等互連連線摘要，橫跨 metro 內的不同設施。 您也可以藉由選取窗格中央的 [**連接**]，從 [**總覽**] 頁面存取 [連線摘要]，如下所示。

    > [!div class="mx-imgBorder"]
    > ![對等互連資源連線](../media/setup-direct-get-connectionssummary.png)

    * 連線**狀態**對應至對等互連連線設定的狀態。 此欄位中顯示的狀態會遵循[直接對等逐步](../walkthrough-direct-all.md)解說中所示的狀態圖表。
