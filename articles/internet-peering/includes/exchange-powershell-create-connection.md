...
---
下面的示例演示如何在西雅圖的 Equinix 互聯網交易所創建 Exchange 連接。 如果您使用不同的提供者和不同的設定，請在您提出要求時替換成該資訊。

使用 PowerShell Cmdlet**新-Az 對等互連連線物件**創建 PowerShell 連線物件，該物件將用於生成新的對等請求：
...
> [!IMPORTANT]
> Microsoft 將開始預配請求的對等互連，`ConnectionState`並將反映進度。
> 有關預配相關步驟，請參閱[Exchange 對等演練文檔](../walkthrough-exchange-all.md)。
