...
1. 在「**建立對等」** 頁上的 **「設定」** 選項卡上,填寫如下所示的框。
    ...
    * 選擇要將對等互連轉換為 Azure 資源的**Metro**位置。 如果在選擇的**Metro**位置中具有未轉換為 Azure 資源的對等連接,則這些連接將列在 **「對等連接**」部分中,如圖所示。 現在,您可以將這些對等連接轉換為 Azure 資源。
        ...
        > [!div class="mx-imgBorder"]
        > ![對等互連連線清單](../media/setup-exchange-legacy-conf-tab.png)

        > [!NOTE]
        > 不能修改舊體對等互連連接的設置。 如果要在所選**的 Metro**位置添加與 Microsoft 的其他對等互連連接,請選擇「**創建新**」。 有關詳細資訊,請參閱[使用門戶 建立或修改 Exchange 對等互連](../howto-exchange-portal.md)。
        >
...
1. 消息更改為*驗證后*,驗證您的資訊。 通過選擇 **「創建**」提交請求。 如果需要修改請求,請選擇 **「上一步」** 並重複這些步驟。

    > [!div class="mx-imgBorder"]
    > ![對等提交](../media/setup-exchange-review-tab-submit.png)

1. 提交請求後,等待部署完成。 如果部署失敗,請與[微軟對等互連](mailto:peering@microsoft.com)。 成功部署將顯示為圖所示。
