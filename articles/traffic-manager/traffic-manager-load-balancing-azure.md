...
* **流量管理員**提供全域 DNS 負載平衡。 它會查看傳入 DNS 要求並根據客戶所選取的路由原則，以狀況良好的端點回應。 路由方法的選項如下︰
  ...
  * 多值路由，可讓您在單一 DNS 回應中傳送多個應用程式端點的 IP 位址。
  ...
  用戶端直接連接到流量管理員傳回的端點。 Azure 流量管理員會在偵測到端點狀況不良時，將用戶端重新導向至另一個狀況良好的執行個體。 請參閱[Azure 流量管理器文件](traffic-manager-overview.md)以瞭解有關服務的詳細資訊。
* **應用程式閘道**提供應用程式提供控制器 (ADC) 作為服務,為您的應用程式提供各種第 7 層負載平衡功能。 它允許客戶透過 CPU 密集型 TLS 連接安裝到應用程式閘道來最佳化 Web 伺服器場的工作效率。 其他第 7 層路由功能包括循環配置連入流量、以 Cookie 為基礎的工作階段同質、URL 路徑型路由，以及在單一應用程式閘道背後代管多個網站的能力。 應用程式閘道可以設定為連結網際網路的閘道、內部專用閘道或兩者混合。 應用程式閘道完全由 Azure 管理、可調整且可用性極高。 它提供一組豐富的診斷和記錄功能，很好管理。
* **負載均衡器**是 Azure SDN 堆疊的組成部分,為所有 UDP 和 TCP 協定提供高性能、低延遲的第 4 層負載平衡服務。 它會管理輸入及輸出連線。 您可以設定公用和內部負載平衡端點，並使用 TCP 和 HTTP 健全狀況探查選項定義規則，將輸入連線對應至後端集區目的地，以管理服務可用性。
...
> [!NOTE]
> 這個範例只是 Azure 所提供的許多可能的負載平衡服務設定的其中一個。 Azure 流量管理員、應用程式閘道和負載平衡器可以混合使用及配對，以提供最符合您負載平衡的需求。 例如,如果不需要 TLS 卸載或第 7 層處理,則可以使用負載均衡器代替應用程式閘道。

## <a name="setting-up-the-load-balancing-stack"></a>設定負載平衡堆疊

### <a name="step-1-create-a-traffic-manager-profile"></a>步驟 1︰建立流量管理員設定檔

1. 在 Azure 門戶中,按下「**建立資源** > **網路** > **流量管理員」 設定檔** > **「 創建**」。
2. 輸入下列基本資訊：
...
1. 在 Azure 門戶中,在左側窗格中,按下「**創建資源** > **網路** > **應用程式閘道**」。。
2. 輸入下列關於應用程式閘道的基本資訊︰
...
1. 在 Azure 門戶中,在左側窗格中,按兩下「**創建資源** > **網路** > **負載均衡器**」。
2. 為負載平衡器選擇名稱。
3. 將 **"類型"** 設定為**內部**,並選擇適當的虛擬網路和子網,以便負載均衡器駐留在其中。
4. 在**IP 位址配置**下,選擇**動態**或**靜態**。
5. 在 [資源群組]**** 下，選擇負載平衡器的資源群組。
6. 在 **"位置**"下,為負載均衡器選擇適當的區域。
7. 按一下 [建立]**** 以產生負載平衡器。
...
6. 在**不正常閾值**下,指定後端 VM 被視為不正常時必須發生的連續探測失敗數。
7. 按下 **「確定」** 以創建探測器。
...
2. 輸入負載平衡規則的 [名稱]****。
3. 選擇負載均衡器、**協定**與**連接埠**的**前端 IP 位址**。
4. 在**後埠**下,指定要在後端池中使用的埠。
5. 選取**後端集區**和在先前步驟中所建立以對其套用規則的**探查**。
6. 在 **「工作階段持久性」** 下,選擇您希望會話持久化的方式。
7. 在 **「空閒超時」** 下,指定空閒超時前的分鐘數。
