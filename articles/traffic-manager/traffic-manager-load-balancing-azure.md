...
> [!NOTE]
> 這個範例只是 Azure 所提供的許多可能的負載平衡服務設定的其中一個。 Azure 流量管理員、應用程式閘道和負載平衡器可以混合使用及配對，以提供最符合您負載平衡的需求。 例如，如果不需要進行 TLS 卸載或第7層處理，您可以使用 Load Balancer 來取代應用程式閘道。
...
6. 在 [**狀況不良臨界值**] 下，指定要將後端 VM 視為狀況不良時，必須發生的連續探查失敗次數。
7. 按一下 **[確定]** 以建立探查。
