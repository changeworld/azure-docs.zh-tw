...
1. 在 Azure 門戶的左上角，選擇 **"創建資源** > **計算** > **Windows 伺服器 2016 VM**"。
...
在本節中，您將 IIS 伺服器安裝在兩個 VM 上 - *myIISVMEastUS*  & *myIISVMWEurope，* 然後更新預設網站頁面。 自訂的網站頁面會顯示當您從網頁瀏覽器瀏覽網站時，您所連線的 VM 名稱。
...
6. 在伺服器桌面上，導航到**Windows 管理工具**>**伺服器管理員**。
...
流量管理員會根據服務端點的 DNS 名稱來路由傳送使用者流量。 在本節中，您可以為 IIS 伺服器配置 DNS 名稱 - *myIISVMEastUS*和*myIISVMWEurope*。
...
1. 在螢幕的左上角，選擇 **"創建資源** > **網路** > **流量管理器設定檔** > **創建**"。
...
添加運行 IIS 伺服器的兩個 VM - *myIISVMEastUS* & *myIISVMWEurope，* 以便根據使用者查詢的子網路由使用者流量。
...
3. 輸入或選擇以下資訊，接受其餘設置的預設值，然後選擇 **"確定**" ：
    ...
    | 目標資源類型           | 公用 IP 位址                          |
    | 目標資源          | **選擇公共 IP 位址**以顯示同一訂閱下具有公共 IP 位址的資源清單。 在 [資源]**** 中，選取名為 *myIISVMEastUS-ip* 的公用 IP 位址。 這是美國東部 IIS 伺服器 VM 的公用 IP 位址。|
...
1. 在門戶的搜索欄中，搜索您在上一節中創建的**流量管理器設定檔**名稱。 在顯示的結果中，按一下流量管理員設定檔。
1. 按一下 [概觀]****。
...
4. 選取 [確定]****。
5. 您可能會在登入過程中收到憑證警告。 如果您收到警告，請選取 [是]**** 或 [繼續]**** 以繼續進行連線。
1. 在 VM *myVMEastUS* 的網頁瀏覽器中，輸入流量管理員設定檔的 DNS 名稱，以檢視您的網站。 由於 VM *myVMEastUS* IP 位址與終結點*myIISVMEastUS*相關聯，因此 Web 瀏覽器啟動測試網站伺服器 - *myIISVMEastUS*。

   ![測試流量管理員設定檔](./media/traffic-manager-subnet-routing-method/test-traffic-manager.png)

2. 接下來，使用步驟 1-5 來連線到位於**西歐**的 VM *myVMWestEurope*，並從這個 VM 瀏覽到流量管理員設定檔網域名稱。 由於 VM *myVMWestEurope* IP 位址與端點*myIISVMEastUS*相關聯，因此 Web 瀏覽器啟動測試網站伺服器 - *myIISVMWEurope*。
