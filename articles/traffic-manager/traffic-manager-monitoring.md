...
* **協定**. 選擇 HTTP、HTTPS 或 TCP 作為通訊協定，流量管理員會在探查端點以檢查其健康情況時使用。 HTTPS 監視不會驗證 TLS/SSL 憑證是否有效,它僅檢查證書是否存在。
* **連接埠**。 選擇要求所使用的連接埠。
...
1. **取得**。 對於每個端點而言，流量管理員監視系統會對監視設定中指定的路徑執行 GET 要求。
...
* [我在流量管理器配置檔中停止了 Web 應用程式終結點,但即使在重新啟動它後,我也沒有收到任何流量。如何解決此問題?](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-faqs#i-stopped-an-web-application-endpoint-in-my-traffic-manager-profile-but-i-am-not-receiving-any-traffic-even-after-i-restarted-it-how-can-i-fix-this)
