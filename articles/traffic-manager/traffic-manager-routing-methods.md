...
* **[優先順序](#priority-traffic-routing-method)：** 如果您想要針對所有流量使用主要服務端點，請選取 [**優先順序**]，並提供備份，以免主要或備份端點無法使用。
* **[加權](#weighted)：** 當您想要將流量分散到一組端點（不論是平均或根據您定義的權數）時，請選取 [**加權**]。
* **[效能](#performance)：** 當您的端點位於不同的地理位置，而您想要讓使用者使用「最靠近」的端點（根據最低網路延遲）時，請選取 [**效能**]。
...
流量管理員會讀取 DNS 查詢的來源 IP 位址，判斷查詢的來源地理區域。 然後尋找是否有此地理區域對應的端點。 此查閱會從最低的資料細微性層級開始（州/省的支援位置，否則位於國家/地區層級），一直到最高層級，也就是**世界**。 經過此周遊找到的第一個相符項目，就指定為要在查詢回應中傳回的端點。 符合「巢狀」類型端點時，則會根據路由方法，傳回該子設定檔中的端點。 此行為有下列幾個特點︰
