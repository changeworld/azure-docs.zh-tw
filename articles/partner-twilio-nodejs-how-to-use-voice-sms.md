...
Twilio 提供了一個可將任何桌面 Web 瀏覽器、iOS 應用程式或 Android 應用程式轉換為 VoIP 電話的用戶端 SDK。 在本文中，我們僅討論如何在瀏覽器中使用 VoIP 通話。 除了在瀏覽器中執行的 *Twilio JAVASCRIPT SDK* 之外， (node.js 應用程式的伺服器端應用程式) 必須用來將「功能權杖」發出至 JavaScript 用戶端。 如需透過 node.js 使用 VoIP 的詳細資訊，請參閱 [Twilio 開發人員部落格][voipnode]。
