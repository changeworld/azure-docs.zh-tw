...
如果藍圖分配使用**系統分配的託管標識**，則引用的金鑰保存庫_必須_存在於藍圖定義分配給的同一訂閱中。

如果藍圖分配使用**使用者分配的託管標識**，則引用的金鑰保存庫_可能_存在於集中式訂閱中。 在分配藍圖之前，必須授予託管標識在金鑰保存庫上的適當許可權。

> [!IMPORTANT]
> 在這兩種情況下，金鑰保存庫都必須具有"**訪問策略**"頁上配置**的範本部署的 Azure 資源管理器存取權限**。 如需如何啟用此功能的相關指示，請參閱 [Key Vault - 啟用範本部署](../../../azure-resource-manager/managed-applications/key-vault-access.md#enable-template-deployment)。
...
1. 按一下現有藍圖，然後按一下 **"編輯藍圖**"或按一下"**創建藍圖"** 並填寫 **"基礎知識"** 選項卡上的資訊。
...
1. 以滑鼠右鍵按一下您想要指派的藍圖。 選擇 **"分配藍圖**"或按一下要分配的藍圖，然後按一下"**分配藍圖"** 按鈕。
