...
ms.locfileid: "98918325"
---
# <a name="quickstart-define-and-assign-a-blueprint-in-the-portal"></a>在入口網站中定義和指派藍圖

在了解如何建立及指派藍圖後，您即可根據 Resource Manager 範本 (ARM 範本)、原則、安全性等要素定義常用模式，以開發可重複使用並可快速部署的組態。 在本教學課程中，您將了解如何使用 Azure 藍圖在您的組織中處理藍圖的建立、發佈和指派等常見工作。 這些工作包括：
...
1. 在頁面左側選取 [藍圖定義]，然後選取頁面頂端的 [+ 建立藍圖] 按鈕。
   ...
   :::image type="content" source="./media/create-blueprint-portal/create-blueprint-button.png" alt-text="[藍圖定義] 頁面上 [建立藍圖] 按鈕的螢幕擷取畫面。" border="false":::
...
1. 確認資訊正確無誤。 [藍圖名稱] 和 [定義位置] 欄位無法在之後變更。 按一下頁面底端的 [下一步：成品] 或頁面頂端的 [成品]。
...
1. 新增訂用帳戶層級的角色指派：
   ...
   :::image type="content" source="./media/create-blueprint-portal/add-role-assignment.png" alt-text="用於新增至藍圖定義的角色指派成品選項螢幕擷取畫面。" border="false":::

   > [!NOTE]
   > 大部分的成品均支援參數。 在藍圖建立期間獲得指派值的參數，是 _靜態參數_。 如果是在藍圖指派期間受指派的參數，則為 _動態參數_。 如需詳細資訊，請參閱[藍圖參數](./concepts/parameters.md)。
...
1. 在資源群組下新增範本：
   ...
   1. 在編輯器方塊的 [範本] 索引標籤中，貼上下列 ARM 範本。 貼上範本後，請選取 [參數] 索引標籤，並請留意已偵測到範本參數 **storageAccountType** 和 **位置**。 系統會自動偵測並填入每個參數，但將其設定為動態參數。
   ...
   1. 清除 **storageAccountType** 核取方塊，並留意下拉式清單中僅包含 ARM 範本的 **allowedValues** 下方所含的值。 選取方塊，將其設回動態參數。

   1. 選取 [新增] 以將此成品新增至藍圖。

   :::image type="content" source="./media/create-blueprint-portal/add-resource-manager-template.png" alt-text="用於新增至藍圖定義的 Resource Manager 範本成品選項螢幕擷取畫面。" border="false":::

1. 完成的藍圖應該如下所示： 請注意，每個成品的 [參數] 資料行中都會出現 **已填入 _x_ 個參數，共 _y_ 個**。 在每次指派藍圖時都會設定動態參數。

   :::image type="content" source="./media/create-blueprint-portal/completed-blueprint.png" alt-text="具有每個成品類型的已完成藍圖定義螢幕擷取畫面。" border="false":::
...
1. 在資源群組下新增角色指派：
   ...
   :::image type="content" source="./media/create-blueprint-portal/add-role-assignment-2.png" alt-text="用於新增至藍圖定義的第二個角色指派成品選項螢幕擷取畫面。" border="false":::

1. 完成的藍圖應該如下所示： 請注意，新增的角色指派會顯示 **已填入 1 個參數，共 1 個**。 這表示它是靜態參數。

   :::image type="content" source="./media/create-blueprint-portal/completed-blueprint-2.png" alt-text="螢幕擷取畫面：具有其他角色指派成品的第二個已完成的藍圖定義。" border="false":::
...
1. 在 [指派藍圖] 頁面的 [訂用帳戶] 下拉式清單中，選取要部署此藍圖的訂用帳戶。
   ...
   如果 [Azure 計費](../../cost-management-billing/index.yml)中有支援的企業供應項目，則會在 [訂用帳戶] 方塊下方啟動 [新建] 連結。 請遵循下列步驟：
   ...
   1. 選取頁面底部的 [建立]。
      ...
      :::image type="content" source="./media/create-blueprint-portal/assignment-create-subscription.png" alt-text="建立訂用帳戶視窗和新訂用帳戶選項的螢幕擷取畫面。" border="false":::
...
1. [鎖定指派] 請保留 [不要鎖定] 的預設值。 如需詳細資訊，請參閱[藍圖資源鎖定](./concepts/resource-locking.md)。
   ...
   :::image type="content" source="./media/create-blueprint-portal/assignment-locking-mi.png" alt-text="藍圖指派的 [鎖定指派] 和 [受控識別] 選項的螢幕擷取畫面。" border="false":::
...
1. 在 ARM 範本 **StorageAccount** 中，選取 **Standard_GRS** 作為 **storageAccountType** 參數。
...
1. 在藍圖清單中，以滑鼠右鍵按一下您先前指派的藍圖，然後選取 [檢視指派詳細資料]。
   ...
   :::image type="content" source="./media/create-blueprint-portal/view-assignment-details.png" alt-text="藍圖指派捷徑功能表的螢幕擷取畫面，其中已選取 [檢視指派詳細資料] 選項。" border="false":::
