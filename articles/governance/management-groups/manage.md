...
> [!IMPORTANT]
> Azure Resource Manager 的使用者權杖和管理群組快取會持續30分鐘，才能強制重新整理。 執行任何動作（例如移動管理群組或訂用帳戶）之後，最多可能需要30分鐘的時間才會顯示。 若要更快查看更新，您必須重新整理瀏覽器、登入和登出，或要求新的權杖，以更新您的權杖。  
...
1. 選取頁面頂端的 [重新命名群組]**** 選項。
   ...
   :::image type="content" source="./media/detail_action_small.png" alt-text="[管理群組] 頁面上的 [重新命名群組] 選項" border="false":::

1. 功能表開啟時，輸入您要顯示的新名稱。

   :::image type="content" source="./media/rename_context.png" alt-text="重新命名群組窗格以重新命名管理群組" border="false":::
...
1. 您需要管理群組（「擁有者」、「參與者」或「管理群組參與者」）的寫入權限。 若要查看您有哪些權限，請選取管理群組，然後選取 **IAM**。 若要深入瞭解 RBAC 角色，請參閱  
   [使用 RBAC 管理存取權與許可權](../../role-based-access-control/overview.md)。
...
1. 在清單中選取具有正確識別碼的訂用帳戶。
   ...
   :::image type="content" source="./media/add_context_sub.png" alt-text="要新增至管理群組的可用訂用帳戶" border="false":::
...
1. 針對清單中您需要移動的訂用帳戶資料列，選取資料列結尾省略符號。
   ...
   :::image type="content" source="./media/move_small.png" alt-text="在管理群組上移動選項" border="false":::
...
1. 在開啟的功能表中，選取是要新的管理群組還是使用現有管理群組。
   ...
   :::image type="content" source="./media/add_context_MG.png" alt-text="將管理群組移至新的或現有的群組" border="false":::
...
:::image type="content" source="./media/al-mg.png" alt-text="包含管理群組的活動記錄" border="false":::
...
使用此路徑的範例，就是在 PowerShell 中指派新的角色指派給管理群組：
