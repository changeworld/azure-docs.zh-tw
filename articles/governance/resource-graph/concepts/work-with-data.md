...
> [!NOTE]
> **第一次**使用時，建議您至少使用`asc`或`desc`的一個資料行來排序結果。 如果沒有排序，傳回的結果會是隨機的，而且無法重複。
...
> [!NOTE]
> 使用**Skip**時，建議您至少使用`asc`或`desc`的一個資料行來排序結果。 如果沒有排序，傳回的結果會是隨機的，而且無法重複。
...
**resultTruncated** 是一個布林值，如果回應中有其他未傳回的記錄，則會通知取用者。 在 **count** 屬性小於 **totalRecords** 屬性時，也可識別出此條件。 **totalRecords** 會定義符合查詢的記錄筆數。
 ...
 當任何**true**一個分頁已停用或不可能是因為沒有`id`資料行，或當查詢所要求的資源不足時， **resultTruncated**就是 true。 當**resultTruncated**為**true**時，不會設定 **$skipToken**屬性。
