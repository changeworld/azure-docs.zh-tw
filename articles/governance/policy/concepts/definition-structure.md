...
ms.locfileid: "80239996"
---
# <a name="azure-policy-definition-structure"></a>Azure 原則定義結構

Azure 策略為資源建立約定。 策略定義描述資源合規性[條件](#conditions)以及滿足條件時要達到的效果。 條件將資源屬性[欄位](#fields)與所需值進行比較。 使用[別名](#aliases)訪問資源屬性欄位。 資源屬性欄位是單值欄位或多個值的[陣列](#understanding-the--alias)。 條件計算在陣列上是不同的。
...
策略定義架構如下所示：[https://schema.management.azure.com/schemas/2019-06-01/policyDefinition.json](https://schema.management.azure.com/schemas/2019-06-01/policyDefinition.json)
...
**根據**策略的目標是 Azure 資源管理器屬性或資來源提供者屬性，配置模式。

### <a name="resource-manager-modes"></a>資源管理器模式
...
例如，資源`Microsoft.Network/routeTables`支援標記和位置，並在這兩種模式下進行評估。 但是，無法`Microsoft.Network/routeTables/routes`標記資源，並且無法在`Indexed`模式下進行評估。
...
建立會強制執行標籤或位置的原則時，應該使用 `indexed`。 雖然並非必要，但它可防止不支援標籤和位置的資源在合規性結果中顯示為不符合規範。 有一個例外，就是**資源群組**。 原則如果會在資源群組上強制執行位置或標籤，就應該將 **mode** 設定為 `all`，並明確地以 `Microsoft.Resources/subscriptions/resourceGroups` 類型作為目標。 如需範例，請參閱[強制執行資源群組標籤](../samples/enforce-tag-rg.md)。 有關支援標記的資源清單，請參閱[標記對 Azure 資源的支援](../../../azure-resource-manager/management/tag-support.md)。

### <a name="resource-provider-modes-preview"></a><a name="resource-provider-modes" />資來源提供者模式（預覽）

預覽期間當前支援以下資來源提供者模式：

- `Microsoft.ContainerService.Data`用於管理[Azure 庫伯內斯服務](../../../aks/intro-kubernetes.md)上的入場控制器規則。 使用此資來源提供者模式的策略**必須**使用[強制重新策略](./effects.md#enforceregopolicy)效果。
- `Microsoft.Kubernetes.Data`用於在 Azure 上管理自管理的 AKS 引擎庫伯奈斯群集。
  使用此資來源提供者模式的策略**必須**使用[強制OPA約束](./effects.md#enforceopaconstraint)效果。
- `Microsoft.KeyVault.Data`用於在[Azure 金鑰保存庫](../../../key-vault/key-vault-overview.md)中管理保存庫和證書。

> [!NOTE]
> 資來源提供者模式僅支援內置策略定義，並且在預覽版中不支援計畫。
...
- `metadata`：定義主要由 Azure 門戶用於顯示方便使用資訊的子屬性：
  - `description`：參數用於什麼的說明。 能用來提供可接受值的範例。
  - `displayName`：參數門戶中顯示的易記名稱。
  - `version`：（可選）跟蹤有關策略定義內容版本的詳細資訊。

    > [!NOTE]
    > Azure 策略服務使用`version``preview`和`deprecated`屬性來表示對內置策略定義或計畫與狀態的更改級別。 的`version`格式是： `{Major}.{Minor}.{Patch}`。 特定狀態（如_棄用_或_預覽_）作為`version`**布林斯**追加到屬性或其他屬性中。

  - `category`：（可選）確定在 Azure 門戶中顯示策略定義的類別。
  - `strongType`：（可選）通過門戶分配策略定義時使用。 提供內容感知清單。 如需詳細資訊，請參閱 [strongType](#strongtype)。
  - `assignPermissions`：（可選） 設置為_true，_ 以便 Azure 門戶在策略分配期間創建角色指派。 如果您希望在分配範圍之外分配許可權，則此屬性很有用。 策略中每個角色定義有一個角色指派（或計畫中的所有策略中每個角色定義）。 參數值必須是有效的資源或作用域。
...
在 `metadata` 屬性內，您可以使用 **strongType** 在 Azure 入口網站內提供可複選的選項清單。 **強型別**可以是受支援_的資源類型_或允許的值。 要確定_資源類型_是否對**強型別**有效，請使用[Get-AzResource提供程式](/powershell/module/az.resources/get-azresourceprovider)。

**支援 Get-AzResourceProvider**未返回的某些_資源類型_。 它們是：
...
> [!NOTE]
> 在創建或更新策略定義期間 **，ID**、**類型**和**名稱**由 JSON 外部的屬性定義，並且 JSON 檔中不需要。 通過 SDK 獲取策略定義將返回**ID、type**和**名稱**屬性作為 JSON 的一部分，但每個屬性都是與策略定義的唯讀資訊。 **type**
...
使用**匹配****和不匹配**條件時，提供`#`匹配數位、`?`字母、`.`匹配任何字元和任何其他字元以匹配該實際字元。 雖然**匹配**和**不匹配**區分大小寫，但評估_字串Value_的所有其他條件都不區分大小寫。 不會區分大小寫的替代項目，可在 **matchInsensitively** 和 **notMatchInsensitively** 中取得。

** \[在\*別名\]** 陣列欄位值中，陣列中的每個元素都使用邏輯**和**元素之間單獨計算。 有關詳細資訊，請參閱[評估\[\*\]別名](../how-to/author-policies-for-arrays.md#evaluating-the--alias)。
...
在下列範例中，`concat` 被用來建立針對名稱為 **tagName** 參數值之標籤的標籤欄位查閱。 如果該標記不存在，則**修改**效果用於使用`resourcegroup()`look 查找函數使用審核的資源父資源組上設置的相同命名標記的值委任標記。
...
在**值**中使用_範本函數_允許使用許多複雜的嵌套函數。 如果「範本函式」__ 的結果為錯誤，則原則評估會失敗。 失敗的評估隱含著**拒絕**的意思。 在某些情況下失敗**的值**的示例：
...
上面的示例策略規則使用[子字串（）](../../../azure-resource-manager/templates/template-functions-string.md#substring)將**名稱**的前三個字元與**abc**進行比較。 如果**名稱**短于三個字元，則`substring()`函數會導致錯誤。 此錯誤會導致策略成為**拒絕**效果。

相反，使用[if（）](../../../azure-resource-manager/templates/template-functions-logical.md#if)函數檢查**名稱**的前三個字元是否等於**abc，** 而**不允許名稱短**于三個字元導致錯誤：
...
使用修訂後的策略規則，`if()`在嘗試獲取少於三個字元`substring()`的值之前，檢查**名稱**的長度。 如果**名稱**太短，則傳回值"不從 abc 開始"，並將其與**abc**進行比較。 名稱短且不以**abc**開頭的資源仍然失敗策略規則，但在評估期間不再導致錯誤。

### <a name="count"></a>Count

可以使用**count**運算式來表示計算資源負載中陣列滿足條件運算式的成員數的條件。 常見方案是檢查陣列成員是否滿足條件，"至少一個"，"確切地說，全部"或"沒有一個"。 **count**計算條件運算式的每個[\[\*\]別名](#understanding-the--alias)陣列成員並求和真實結果，然後將_該結果_與運算式運算子進行比較。
...
- **count.field（** 必需）：包含陣列的路徑，並且必須是陣列別名。 如果缺少陣列，則運算式計算為_false，_ 而不考慮條件運算式。
- **計數.where（** 可選）：用於單獨計算**count.field**的每個[\[\*\]別名](#understanding-the--alias)陣列成員的條件運算式。 如果未提供此屬性，則所有路徑為"欄位"的陣列成員將計算為_true_。 任何[條件](../concepts/definition-structure.md#conditions)都可以在此屬性內使用。
  [邏輯運算子](#logical-operators)可用於此屬性內創建複雜的評估要求。
- 條件（必需）：該值與滿足**count.where**條件運算式的項數進行比較。 ** \> \< ** 應使用數值[條件](../concepts/definition-structure.md#conditions)。

#### <a name="count-examples"></a>計算示例
...
示例 6：使用**內部值的欄位**檢查所有陣列成員是否符合條件運算式**value**
...
示例 7：檢查至少一個陣列成員是否與條件運算式中的多個屬性匹配
...
- **審核IfNot存在**：如果相關資源不存在，則在活動日誌中生成警告事件
...
有關每個效果、評估順序、屬性和示例的完整詳細資訊，請參閱[瞭解 Azure 策略效果](effects.md)。

### <a name="policy-functions"></a>原則函式

除以下函數和使用者定義的函數外，所有[資源管理器範本函數](../../../azure-resource-manager/templates/template-functions.md)都可以在策略規則中使用：
...
以下函數可用於策略規則，但與 Azure 資源管理器範本中的使用不同：

- `utcNow()`- 與資源管理器範本不同，可以在預設值之外使用。
  - 返回在通用 ISO 8601 DateTime 格式"yyyyy-MM-ddTHH：mm：s.fffffZ"中設置為當前日期和時間的字串
...
- `addDays(dateTime, numberOfDaysToAdd)`
  ...
  - **要添加的天數**： [必需] 整數 - 要添加的天數
- `field(fieldName)`
  - **欄位名稱**： [必需] 字串 - 要檢索的[欄位](#fields)的名稱
  - 從"If 條件"正在評估的資源中返回該欄位的值
...
- `requestContext().apiVersion`
  - 返回觸發策略評估的請求的 API 版本（例如： `2019-09-01`。 這將是 PUT/PATCH 請求中使用的 API 版本，用於評估資源創建/更新。 在現有資源的合規性評估期間，始終使用最新的 API 版本。
...
- 視覺化工作室代碼的 Azure 策略擴展（推薦）

  使用[視覺化工作室代碼的 Azure 策略擴展](../how-to/extension-for-vscode.md)來查看和發現資源屬性的別名。

  ![視覺化工作室代碼的 Azure 策略擴展](../media/extension-for-vscode/extension-hover-shows-property-alias.png)
...
"正常"別名將欄位表示為單個值。 此欄位用於完全符合比較方案，當整個值集必須完全按照定義、不再和更少時。

** \[別名\*** 可以比較陣列中每個元素的值和每個元素的特定屬性。 此方法可以比較元素屬性的"如果沒有"，"如果任何"或"如果所有"方案。 對於更複雜的方案，請使用[計數](#count)條件運算式。 使用**\[\*ipRules，** 一個示例將驗證每個_操作_都是_Deny，_ 但不擔心存在多少規則或 IP_值_是什麼。
此示例規則檢查**\[\*\]ipRules .value**到**10.0.4.1**的任何匹配項，並且僅在找不到至少匹配項時應用**效果類型**：
...
> [!NOTE]
> 一旦分配了計畫，則無法更改計畫級別參數。 因此，建議在定義參數時設置**預設值**。
