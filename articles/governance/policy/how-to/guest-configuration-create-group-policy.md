---
title: 如何從 Windows 群組原則基準建立來賓設定原則定義
...
在稽核 Windows 時，客體設定會使用[預期狀態設定](/powershell/scripting/dsc/overview/overview) (DSC) 資源模組來建立設定檔。 DSC 設定會定義電腦所應處的條件。 如果設定的評估不**符合規範**，就會觸發原則效果*auditIfNotExists* 。 [Azure 原則來賓](../concepts/guest-configuration.md)設定只會審核計算機內的設定。

> [!IMPORTANT]
> 具有「來賓設定」的自訂原則定義是一項預覽功能。
>
...
DSC 社區已發佈[BaselineManagement 模組](https://github.com/microsoft/BaselineManagement)，以將匯出的群組原則範本轉換成 DSC 格式。 使用 GuestConfiguration Cmdlet，BaselineManagement 模組會從群組原則內容建立 Windows Azure 原則來賓設定套件。 如需使用 BaselineManagement 模組的詳細資訊，請參閱[快速入門：將群組原則轉換成 DSC](/powershell/scripting/dsc/quickstarts/gpo-quickstart)一文。 

在本指南中，我們將逐步解說從群組原則物件（GPO）建立 Azure 原則來賓設定套件的程式。 雖然本逐步解說概述 Windows Server 2019 安全性基準的轉換，但相同的程式也適用于其他 Gpo。  
...
若要在 PowerShell 中安裝**DSC**、 **GuestConfiguration**、**基準管理**和相關的 Azure 模組：
...
接下來，我們會使用來賓設定和基準管理模組，將下載的伺服器2019基準轉換成來賓設定套件。 
...
1. 建立原則內容套件之前，重新命名、重新格式化及執行已轉換的腳本。
...
> [!IMPORTANT]
> 來賓設定原則定義必須**一律**使用結合_AuditIfNotExists_和_DeployIfNotExists_原則的計畫來指派。 如果僅指派 _AuditIfNotExists_ 原則，則不會部署先決條件，而原則一律會顯示「0」部伺服器符合規範。
