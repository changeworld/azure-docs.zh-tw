...
description: 瞭解如何使用 Visual Studio Code 的 Azure 原則延伸模組來查閱 Azure Resource Manager 別名。
ms.date: 06/16/2020
ms.topic: how-to
...
1. 從 Azure 原則延伸模組或命令選擇區登入 Azure。
   ...
   - 命令選擇區
     ...
     從功能表列中，移至 [**查看**] [命令選擇區]  >  ** **，然後輸入**Azure： Sign In**。
...
1. 從命令選擇區或視窗頁尾啟動 [訂用帳戶] 命令。
   ...
   - 命令選擇區： 
     ...
     從功能表列中，移至 [**查看**] [命令選擇區]  >  ** **，然後輸入**Azure： Select 訂閱**。
...
根據預設，此延伸模組會依據現有的資源和具有原則別名的資源來篩選「資源提供者」部分。 在 [**設定**延伸模組] Azure 原則中變更此行為  >  **Extensions**  >  **Azure Policy** ，以查看所有資源提供者而不進行篩選。
...
1. 從 Azure 原則延伸模組或命令選擇區啟動搜尋介面。
   ...
   - 命令選擇區：
     ...
     從功能表列中，移至 [**流覽**] [命令選擇區] > ** **，然後輸入**資源：搜尋資源**。
...
選取資源時，不論是透過搜尋介面或在 treeview 中選取，Azure 原則延伸模組都會開啟代表該資源及其所有 Azure Resource Manager 屬性值的 JSON 檔案。
...
1. 從 Azure 原則延伸模組或命令選擇區啟動搜尋介面。
   ...
   - 命令選擇區：
     ...
     從功能表列中，移至 [**流覽**] [命令選擇區] > ** **，然後輸入**原則：搜尋原則**。
