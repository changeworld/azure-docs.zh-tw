...
- 當前策略分配上次評估的合規性狀態和時間戳記
- 資源不合規_的原因_清單

> [!IMPORTANT]
> 由於_不合規_資源的符合性詳細資訊顯示該資源上屬性的當前值，因此使用者必須對資源**類型**具有**讀取**操作。 例如，如果_不符合要求_的資源是**Microsoft.Compute/虛擬機器**，則使用者必須具有**Microsoft.計算/虛擬機器/讀取**操作。 如果使用者沒有所需的操作，將顯示訪問錯誤。
...
1. 在 **"策略合規性**"頁的 **"資源合規性**"選項卡下，按右鍵或選擇_處於不符合_的**合規性狀態**的資源的省略號。 然後選擇 **"查看合規性詳細資訊**"。

   ![查看合規性詳細資訊選項](../media/determine-non-compliance/view-compliance-details.png)

1. "**合規性詳細資訊**"窗格顯示從資源的最新評估到當前策略分配的資訊。 在此示例中，發現欄位**Microsoft.Sql/伺服器/版本**為_12.0，_ 而策略定義預期_為 14.0_。 如果由於多種原因資源不符合要求，則每個資源都列在此窗格中。
...
|當前值必須位於目標值中。 |在**或不**在 |
|當前值必須與目標值類似。 |喜歡或不**喜歡** |
|當前值必須區分大小寫與目標值匹配。 |匹配與否**匹配** |
|當前值必須不區分大小寫與目標值匹配。 |匹配不**敏感或不匹配**不敏感 |
|當前值不能將目標值作為鍵包含。 |不包含金鑰或**不包含**金鑰|
|當前值不能包含目標值。 |不包含或**不包含** |
|當前值不能等於目標值。 |不**等於或不等於** |
|當前值必須不存在。 |**不存在**  |
|當前值不能位於目標值中。 |不在或**不在** |
|當前值不能與目標值類似。 |不喜歡或**不喜歡** |
|當前值不能區分大小寫與目標值匹配。 |不匹配或**不匹配** |
|當前值不能不區分大小寫與目標值匹配。 |不匹配**敏感或不匹配**不敏感 |
|沒有任何相關資源與策略定義中的效果詳細資訊匹配。 |**在**** ** |

## <a name="compliance-details-for-guest-configuration"></a>來賓配置的合規性詳細資訊

對於"_來賓配置_"類別中的_審核IfNot存在_策略，VM 內部可能有多個設置，您需要查看每個設置的詳細資訊。 例如，如果您正在審核密碼原則清單，並且其中只有一個策略的狀態_不符合要求_，則需要知道哪些特定的密碼原則不符合要求以及原因。
...
在 **"合規性詳細資訊"** 窗格視圖中按一下連結 **"上次評估的資源**"。

   ![查看審核不存在定義詳細資訊](../media/determine-non-compliance/guestconfig-auditifnotexists-compliance.png)

來賓**分配**頁顯示所有可用的合規性詳細資訊。 視圖中的每一行都表示在電腦內執行的評估。 在 **"原因"** 列中，將顯示一個短語，說明來賓分配_不符合_的原因。 例如，如果要審核密碼原則，**則"原因"** 列將顯示包含每個設置的當前值的文本。

![查看合規性詳細資訊](../media/determine-non-compliance/guestconfig-compliance-details.png)
...
您還可以在電腦範圍內為來賓分配輸出符合性歷史記錄。 此命令的輸出包括 VM 的每個報表的詳細資訊。

> [!NOTE]
> 輸出可能會返回大量資料。 建議將輸出存儲在變數中。
...
要簡化此視圖，請使用 **"顯示更改"** 參數。 此命令的輸出僅包括合規性狀態更改後的報告。
...
1. 選取 [資源合規性]**** 頁面上的 [變更歷程記錄 (預覽)]**** 索引標籤。 偵測到的變更清單 (如果有的話) 會隨即顯示。
   ...
   ![Azure 策略更改歷史記錄選項卡位於資源合規性頁上](../media/determine-non-compliance/change-history-tab.png)

1. 選取其中一個偵測到的變更。 資源的_可視差異_顯示在 **"更改歷史記錄**"頁上。

   ![Azure 策略更改更改歷史記錄在更改歷史記錄頁上的可視差異](../media/determine-non-compliance/change-history-visual-diff.png)

_視覺化差異_有助於識別資源的變更。 檢測到的更改可能與資源的當前符合性狀態無關。

更改歷史記錄資料由[Azure 資源圖](../../resource-graph/overview.md)提供。 要在 Azure 門戶之外查詢此資訊，請參閱[獲取資源更改](../../resource-graph/how-to/get-resource-changes.md)。
