...
以下是 **CIS Microsoft Azure 基礎基準測試**控制項的對應。 使用右側的導覽區可直接跳到特定的**合規性網域**。 許多控制項都是以 [Azure 原則](../overview.md)方案定義進行實作的。 若要檢閱完整方案定義，請在 Azure 入口網站中開啟 [原則]，然後選取 [定義] 頁面。
然後，找出並選取**稽核 CIS Microsoft Azure 基礎基準測試 1.1.0 建議，及部署特定的支援 VM 延伸模組**法規合規性內建方案定義。

> [!IMPORTANT]
> 下列每個控制措施都與一或多個 [Azure 原則](../overview.md)定義相關聯。
> 這些原則可協助您使用工具[存取合規性](../how-to/get-compliance-data.md)；不過，控制措施和一或多個原則之間，通常不是 1：1 或完整對應。 因此，Azure 原則中的**符合規範**只是指原則定義本身，這不保證您符合控制措施所有需求的規範。 此外，合規性標準包含目前未由任何 Azure 原則定義解決的控制措施。 因此，Azure 原則中的合規性只是整體合規性狀態的部分觀點。 此合規性標準的合規性網域、控制項與 Azure 原則定義之間的關聯，可能會隨著時間而改變。 若要檢視變更歷程記錄，請參閱 [GitHub 認可歷程記錄](https://github.com/Azure/azure-policy/commits/master/built-in-policies/policySetDefinitions/Regulatory%20Compliance/CISv1_1_0_audit.json) \(英文\)。
...
### <a name="ensure-that-email-service-and-co-administrators-is-enabled"></a>確定 [電子郵件服務和共同管理員] 為 [已啟用]

**識別碼**：CIS Azure 4.7 **擁有權**：客戶
...
### <a name="ensure-that-send-alerts-to-is-set"></a>確定已設定 [傳送警示給]

**識別碼**：CIS Azure 4.6 **擁有權**：客戶
...
### <a name="ensure-that-threat-detection-types-is-set-to-all"></a>確定 [威脅偵測類型] 設定為 [全部]

**識別碼**：CIS Azure 4.5 **擁有權**：客戶
...
|名稱<br /><sub>(Azure 入口網站)</sup> |描述 |效果 |版本<br /><sub>(GitHub)</sub> |
|---|---|---|---|---|
|[自訂訂用帳戶擁有者角色不應存在](https://portal.azure.com/#blade/Microsoft_Azure_Policy/PolicyDetailBlade/definitionId/%2Fproviders%2FMicrosoft.Authorization%2FpolicyDefinitions%2F10ee2ea2-fb4d-45b8-a7e9-a2e770044cd9) |此原則可確保不存在自訂訂用帳戶擁有者角色。 |Audit, Disabled |[1.0.0](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policyDefinitions/General/CustomSubscription_OwnerRole_Audit.json) |
