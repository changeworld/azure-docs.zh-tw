---
title: 使用 Azure Site Recovery 設定以 Azure 為目標的 Azure VMware 解決方案 VM 災害復原
description: 了解如何使用 Azure Site Recovery 為 Azure VMware 解決方案 VM 設定災害復原至 Azure。
author: Harsha-CS
manager: rochakm
...
ms.locfileid: "92370452"
---
# <a name="set-up-disaster-recovery-to-azure-for-azure-vmware-solution-vms"></a>針對 Azure VMware 解決方案 VM 設定以 Azure 為目標的災害復原

本文說明如何針對 Azure VMware 解決方案 VM 啟用複寫功能，以便使用 [Azure Site Recovery](site-recovery-overview.md) 服務進行 Azure 的災害復原。

這是一系列中的第三個教學課程，說明如何為 Azure VMware 解決方案 VM 設定 Azure 的災害復原。 在上一個教學課程中，我們[準備了 Azure VMware 解決方案環境](avs-tutorial-prepare-avs.md)以便進行 Azure 的災害復原。
...
> [!div class="checklist"]
>
> * 設定來源複寫設定，以及 Azure VMware 解決方案私人雲端中的 Site Recovery 設定伺服器
> * 設定複寫目標設定。
> * 建立複寫原則。
> * 啟用適用於 VMware VM 的複寫。
...
3. 本教學課程將示範如何複寫單一 VM。 如果您要部署多個 VM，則應該使用[部署規劃工具](https://aka.ms/asr-deployment-planner)。 [深入了解](site-recovery-deployment-planner.md) 此工具。
...
- **設定伺服器** ：設定伺服器會協調 Azure VMware 解決方案私人雲端與 Azure 之間的通訊，以及管理資料複寫。
- **處理序伺服器** ：處理序伺服器可作為複寫閘道。 負責接收複寫資料，以快取、壓縮和加密進行最佳化，然後將其傳送至 Azure 中的快取儲存體帳戶。 處理序伺服器也會在您要複寫的 VM 上安裝行動服務代理程式，並且在 Azure VMware 解決方案 VM 上執行自動探索。
- **主要目標伺服器** ：主要目標伺服器會在從 Azure 容錯回復期間，處理複寫資料。


這些元件會全部一起安裝在單一 Azure VMware 解決方案機器 (也稱為「組態伺服器」) 上。 根據預設，對於 Azure VMware 解決方案災害復原，我們會將組態伺服器設定為高可用性 VMware VM。 若要這麼做，請下載備妥的開放式虛擬化應用程式 (OVA) 範本，然後將範本匯入 VMware 以建立 VM。
...
> [執行災害復原演練](avs-tutorial-dr-drill-azure.md)