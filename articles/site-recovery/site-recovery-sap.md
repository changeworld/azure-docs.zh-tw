...
大部分的大型與中型 SAP 部署都有某種形式的災害復原解決方案。 隨著移至應用程式 (如 SAP) 的核心商務程序愈來愈多，強固且可測試的災害復原解決方案愈形重要。 Azure Site Recovery 已經過測試並與 SAP 應用程式整合。 Site Recovery 超過大部分內部部署嚴重損壞修復解決方案的功能，且擁有的擁有權總成本低於競爭解決方案。
...
* 至少一個在 Azure 中執行的 Azure Active Directory 網域控制站和 DNS 伺服器

我們建議您先建立此基礎結構，再部署 Site Recovery。

## <a name="reference-sap-application-deployment"></a>參考 SAP 應用程式部署

此參考架構會在具有高可用性的 Azure 上的 Windows 環境中執行 SAP NetWeaver。 此架構會與特定虛擬機器一起部署 (VM) 大小，您可以變更這些大小以符合您組織的需求。

![典型 SAP 部署模式的圖表](./media/site-recovery-sap/sap-netweaver_latest.png)

## <a name="disaster-recovery-considerations"></a>災害復原考量

針對嚴重損壞修復，您必須能夠容錯移轉至次要區域。 每一層都會使用不同的策略來提供嚴重損壞修復保護。

#### <a name="vms-running-sap-web-dispatcher-pools"></a>執行 SAP Web 發送器集區的 Vm

Web 發送器元件可做為 sap 應用程式伺服器之間 SAP 流量的負載平衡器。 為了達到 Web Dispatcher 元件的高可用性，Azure Load Balancer 會實行平行 Web 發送器設定。 Web 發送器會針對 HTTP (，使用迴圈配置資源設定，) 平衡器集區中可用的 Web 發送器之間的流量分配。

#### <a name="vms-running-application-servers-pools"></a>執行應用程式伺服器集區的 Vm
SMLG 交易會管理 ABAP 應用程式伺服器的登入群組。 它會在中央服務的訊息伺服器中使用負載平衡功能，在 SAP 應用程式伺服器集區之間分散工作負載，以進行 SAPGUIs 和 RFC 流量。 您可以使用 Site Recovery 來複寫此管理。
...
若要執行高可用性解決方案，您可以使用共用磁片叢集或檔案共用叢集。 若要設定共用磁碟叢集的虛擬機器，請使用 Windows Server 容錯移轉叢集。 我們建議您使用雲端見證作為仲裁見證。
 ...
 > [!NOTE]
 > 因為 Site Recovery 不會複寫雲端見證，所以建議您在損毀修復區域中部署雲端見證。

為了支援容錯移轉叢集環境， [SIOS DataKeeper Cluster Edition](https://azuremarketplace.microsoft.com/marketplace/apps/sios_datakeeper.sios-datakeeper-8) 會執行叢集共用磁片區功能。 在函數中，SIOS DataKeeper 叢集會複寫叢集節點所擁有的獨立磁片。 因為 Azure 原本就不支援共用磁片，所以需要由 SIOS 提供的解決方案。

您也可以藉由執行檔案共用叢集來處理叢集。 [SAP](https://blogs.sap.com/2018/03/19/migration-from-a-shared-disk-cluster-to-a-file-share-cluster) 最近修改了中央服務部署模式，來透過 UNC 路徑存取 /sapmnt 全域目錄。 我們仍然建議您確保/sapmnt UNC 共用具有高可用性。 您可以檢查中央服務實例。 使用 Windows Server 容錯移轉叢集搭配 Scale Out 檔案伺服器 (SOFS) 以及 Windows Server 2016 中的儲存空間直接存取 (S2D) 功能。
...
您可以使用 Site Recovery 來協調跨 Azure 區域的完整 SAP 部署的容錯移轉。
以下是設定嚴重損壞修復的步驟：
...
1. 新增應用程式伺服器、web 發送器和 SAP Central services Vm，以建立復原方案。
1. 選取 [ **自訂** ] 以將 vm 分組。 依預設，所有 Vm 都屬於群組1。

### <a name="add-scripts-to-the-recovery-plan"></a>將指令碼新增至復原計畫
若要讓您的應用程式正確運作，您可能需要在 Azure 虛擬機器上進行某些作業。 在容錯移轉之後或在測試容錯移轉期間執行這些作業。 您也可以自動化某些容錯移轉後的作業。 例如，更新 DNS 專案，並將對應的腳本新增至復原方案來變更系結和連接。
...
1. 將動作前腳本新增至群組1，以容錯移轉 SQL Server 可用性群組。 使用在範例腳本中發佈的 ASR-SQL-Asr-sql-failoverag 腳本。 遵循腳本中的指引，並適當地在腳本中進行必要的變更。
1. 新增動作後腳本，以將負載平衡器連結至 Web 層的容錯移轉虛擬機器 (群組 1) 。 使用在範例腳本中發佈的 ASR-AddSingleLoadBalancer 腳本。 遵循腳本中的指引，並視需要在腳本中進行必要的變更。
