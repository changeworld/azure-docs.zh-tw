...
本文概述 [Azure Site Recovery](site-recovery-overview.md)中的復原方案。

復原方案會針對容錯移轉的目的，將機器收集到復原群組。 復原方案可藉由建立您可容錯移轉的小型獨立單位，協助您定義系統性復原程序。 單位通常代表您環境中的應用程式。

- 復原方案會定義機器容錯移轉的方式，以及機器在容錯移轉之後啟動的順序。
- 復原方案可用於容錯移轉至 Azure，以及從 Azure 容錯回復。
- 最多可將100個受保護的實例新增至一個復原方案。
- 您可以將順序、指示及工作新增至方案來加以自訂。
- 定義方案之後，您可以在該方案上執行容錯移轉。
- 您可以在多個復原方案中參考機器，如果先前已使用另一個復原方案來部署機器，後續的計畫將會略過電腦的部署/啟動。
...
- 在指定順序之處，就會使用排序。 動作會以平行方式執行，以改善應用程式復原的 RTO。
- 單一群組中的機器會以平行方式容錯移轉。
...
## <a name="automate-tasks-in-recovery-plans"></a>將復原方案中的工作自動化
...
- 如果您發現遺漏了某個東西，請觸發清除，然後重新執行測試容錯移轉。 
- 請執行測試容錯移轉多次，直到您確定應用程式可順暢復原為止。
- 因為每個應用程式都是唯一的，所以您必須建置針對每個應用程式自訂的復原方案，並在每個復原方案上執行測試容錯移轉。
- 應用程式和其相依性會經常變更。 為確保復原方案是最新的，請每季執行每個應用程式的測試容錯移轉。

    ![Site Recovery 方案中範例復原方案的螢幕擷取畫面](./media/recovery-plan-overview/rptest.png)

## <a name="watch-a-recovery-plan-video"></a>觀看復原方案影片

觀看快速範例影片，其中顯示兩層式 WordPress 應用程式之復原方案的按一下容錯移轉。
