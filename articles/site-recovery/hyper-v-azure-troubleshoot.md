---
title: 使用 Azure 網站恢復解決 Hyper-V 災害復原問題
...
4. 檢查 Hyper-V-VMMS\Admin 登入虛擬機器時出現的問題。 此日誌位於**應用程式和服務日誌** > **微軟** > **Windows**中。
5. 在客體 VM 上，確認 WMI 已啟用並可供存取。
   ...
   - [解決](https://technet.microsoft.com/library/ff406382.aspx#H22)WMI 腳本和服務的問題。
...
4. 檢查 Hyper-V 伺服器與 Azure 之間的連線。 若要檢查連線能力，請在 Hyper-V 主機上開啟工作管理員。 在 **"性能"** 選項卡上，按一下 **"打開資源監視器**"。 在 [網路]**** 索引標籤 > [網路活動的程序]**** 上，查看 cbengine.exe 是否正在傳送大量資料 (以 MB 為單位)。
...
5. 確認備份服務已啟用。 驗證它在**Hyper-V 設置** > **整合服務**中是否啟用。
...
1. 檢查事件記錄檔中是否有 VSS 錯誤和建議：
    - 在 Hyper-V 主機伺服器上，打開超級 V 管理員事件日誌的事件**檢視器** > **應用程式和服務日誌** > **Microsoft** > **Windows** > **Hyper-V** > **管理員**。
...
2. 若要產生 VM 的 VSS 快照集，請確認已在 VM 上安裝 Hyper-V Integration Services，而且已啟用備份 (VSS) Integration Service。
    ...
    - 您可以使用命令**Get-VM\<整合服務 -VMName VMName>-名稱 VSS**從 Hyper-V 主機上的提升 PowerShell 會話中檢查此資訊，您也可以通過登錄到來賓 VM 獲取此資訊。 [深入了解](https://docs.microsoft.com/windows-server/virtualization/hyper-v/manage/manage-hyper-v-integration-services)。
...
所有 Hyper-V 複製事件都記錄在位於**應用程式和服務日誌** > **Microsoft** > **Windows**中的 Hyper-V-VMMS_Admin 日誌中。 此外，您還可以啟用 Hyper-V 虛擬機器管理服務的分析記錄，如下所示：

1. 讓 [分析] 與 [偵錯] 記錄可在 [事件檢視器] 中檢視。 要使日誌可用，請在事件檢視器中按一下 **"查看** > **顯示分析和調試日誌"。** [分析] 記錄會出現在 **Hyper-V-VMMS** 下。
