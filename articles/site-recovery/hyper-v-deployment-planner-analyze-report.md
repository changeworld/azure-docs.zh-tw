...
**所需的 RPO （分鐘）**：預設復原點目標，或在報告產生時針對 "DesiredRPO" 參數所傳遞的值，以預估所需的頻寬。
...
**VM 相容性**：值為 **[是]** 和 **[是**]\*。 **是** \*適用于 VM 適合[Premium ssd](../virtual-machines/windows/disks-types.md)的實例。 相較於對應到磁碟的大小，這裡剖析的高變換或 IOPS 磁碟適合較大的進階磁碟大小。 儲存體帳戶會根據磁碟大小，決定磁碟要對應至的進階儲存體大小類型： 
* <128 GB 為 P10。
...
**磁片區上為了進行差異複寫所需的儲存體（GB）**： VM 磁片區上所需的可用儲存空間，以進行差異複寫。
...
**批次的估計初始複寫時間（HH： MM）**：以小時：分鐘為單位的估計初始複寫時間。
