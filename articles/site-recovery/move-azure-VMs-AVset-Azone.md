...
description: 了解如何使用 Site Recovery 將 VM 移至不同區域中的可用性區域
...
ms.locfileid: "93393127"
---
# <a name="move-azure-vms-into-availability-zones"></a>將 Azure VM 移至可用性區域中

這篇文章說明如何將 Azure VM 移至不同區域中的可用性區域。 如果您想要移至相同區域中的不同區域，請[參閱這篇文章](./azure-to-azure-how-to-enable-zone-to-zone-disaster-recovery.md)。


Azure 中的可用性區域有助於讓應用程式和資料不受資料中心故障的影響。 每個可用性區域由一或多個資料中心組成，配備了電力、冷卻系統及網路系統。 若要確保復原能力，在所有已啟用的地區中都至少要有三個個別的區域。 某個地區內可用性區域的實體區隔可保護應用程式和資料不受資料中心故障影響。 搭配可用性區域後，Azure 提供虛擬機器 (VM) 運作時間達 99.99% 的服務等級協定 (SLA)。 可用性區域可在選定區域中受到支援，如[支援可用性區域的地區](../availability-zones/az-region.md)中所述。
...
> [!IMPORTANT]
> 若要將 Azure VM 移至不同區域的可用性區域，我們現在建議使用 [Azure Resource Mover](../resource-mover/move-region-availability-zone.md)。 Resource Mover 處於公開預覽狀態，並且提供：
> - 用於在區域之間移動資源的單一中樞。
...
- 檢查目標區域是否[支援可用性區域](../availability-zones/az-region.md)。 確認您選擇的[來源/目標區域組合可受到支援](./azure-to-azure-support-matrix.md#region-support)。 在目標區域上做出明智的決策。
...
1. 如果您想要使用 Site Recovery 將 VM 移至可用性區域，這些 VM 應使用受控磁碟。 您可以將使用非受控磁碟的現有 Windows VM 轉換成使用受控磁碟。 請遵循[將 Windows 虛擬機器從非受控磁碟轉換成受控磁碟](../virtual-machines/windows/convert-unmanaged-to-managed-disks.md)上的步驟。 請確定可用性設定組已設定為「受控」。
...
6. 確認 [VM 的輸出連線需求](azure-to-azure-tutorial-enable-replication.md#set-up-vm-connectivity)。
...
3. 為來源網路配置中識別的每個元件建立目標資源。 此動作可確保在完全移轉至目標區域後，您的 VM 具備您在來源中擁有的所有功能和特性。
   ...
   如需其他網路元件的資訊，請參閱網路[文件](../index.yml?pivot=products&panel=network)。
    ...
    > [!IMPORTANT]
    > 請務必在目標中建立區域備援負載平衡器。 您可以在 [Standard Load Balancer 和可用性區域](../load-balancer/load-balancer-standard-availability-zones.md)中參閱更多資訊。

4. 如果您想要先測試組態再完全移轉到目標區域，請在目標區域中手動[建立非生產網路](../virtual-network/quick-create-portal.md)。 我們建議使用此方法，因為這樣對生產網路產生的干擾最少。
...
2. 在 [作業] 中，選取 [災害復原]。
3. 在 [設定災害復原] > [目標區域] 中，選取要作為複寫目的地的目標區域。 請確定此區域[支援](../availability-zones/az-region.md)可用性區域。
4. 完成時，選取 下一步:**進階設定**。
