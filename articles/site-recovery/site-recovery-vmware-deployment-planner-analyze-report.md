---
title: 使用 Azure 網站恢復分析 VMware 災害復原的部署規劃器報告
description: 本文介紹如何使用 Azure 網站恢復分析 VMware 災害復原恢復恢復恢復恢復部署規劃程式生成的報表。
...
---
# <a name="analyze-the-deployment-planner-report-for-vmware-disaster-recovery-to-azure"></a>分析 VMware 災害復原到 Azure 的部署規劃器報告
...
**所需的 RPO**：您部署的復原點目標。 預設會計算 RPO 值 15、30 和 60 分鐘所需的網路頻寬。 根據選取項目，在工作表上更新受影響的值。 如果在生成報表時使用了 *"所需 RPOinMin"* 參數，則該值將顯示在所需的 RPO 結果中。
...
下列圖表顯示保護所有相容 VM 所需的儲存體帳戶 (標準和進階) 總數。 若要了解每個 VM 所要使用的儲存體帳戶，請參閱「VM 儲存體放置」一節。 如果使用部署規劃器的 v2.5，則建議僅顯示覆制所需的標準緩存存儲帳戶數，因為資料直接寫入託管磁片。
...
>[!Note]
>部署規劃器 v2.5 以後建議將直接複製到託管磁片的電腦的存儲位置。

![VM 儲存體放置](media/site-recovery-vmware-deployment-planner-analyze-report/vm-storage-placement-v2a.png)

**複製存儲類型**：標準磁片或高級託管磁片，用於複製 VM 中提及的所有相應**VM。**

**日誌存儲帳戶類型**：所有複製日誌都存儲在標準存儲帳戶中。

**存儲帳戶的建議首碼**：可用於命名存儲存儲帳戶的建議三字元首碼。 您可以使用自己的前置詞，但工具的建議會遵循[儲存體帳戶的磁碟分割命名慣例](https://aka.ms/storage-performance-checklist)。

**建議的日誌帳戶名稱**：包含建議首碼後存儲帳戶名稱。 以您的自訂輸入取代括弧內的名稱 (< 和 >)。

**放置摘要**：按存儲類型保護 VM 所需的磁片摘要。 它包括 VM 的總數、所有磁片的總預配大小和磁片總數。
...
**VM 相容性**：值為 **[是]** 和 **[是\*]**。 **是**\*，對於 VM 適合[高級 SSD](../virtual-machines/windows/disks-types.md)的實例。 在此情況下，剖析的高變換 / IOPS 磁碟符合 P20 或 P30 類別，但磁碟大小會導致它向下對應至 P10 或 P20。 儲存體帳戶會根據磁碟大小，決定磁碟所要對應至的進階儲存體大小類型。 例如：
* <128 GB 為 P10。
...
**為複製創建的 Asrseed 磁片（託管磁片）：** 啟用複製時創建的磁片的名稱。 它將資料存儲及其快照在 Azure 中。
...
* 資料磁片大小錯誤或 OS 磁片大小錯誤。 [查看](vmware-physical-azure-support-matrix.md#azure-vm-requirements)支援限制。 
...
* 對於磁片的平均 I/O 大小，平均資料改動超過支援的網站恢復資料改動限制 20 MB/s。
...
**作業系統類型**：它是 VM 的作業系統類型。 根據在建立 VM 時從 VMware vSphere 所選的範本，可以是 Windows 或 Linux 或其他類型。
...
**複製存儲目標** | **平均源磁片 I/O 大小** |**平均源磁片資料改動** | **每天源磁片資料總改動**
---|---|---|---
