...
**Azure 網站恢復許可證**：所有相容 VM 的網站恢復許可證成本。 如果您已在詳細的成本分析表格中手動輸入 VM，則 Site Recovery 授權成本也包含該 VM。
...
**目的地區域**：為 DR 指定的 Azure 區域。 報告中針對計算、儲存體、網路和授權使用的價格是以該地區的 Azure 價格為基礎。 
