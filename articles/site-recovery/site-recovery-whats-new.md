...
[匯總套件47](https://support.microsoft.com/help/4570609/) | 9.34.5634.1 | 5.1.6100.0 | 9.34.5634.1 | 5.1.6100.0 | HyperV 機器-2.0.9183。0 <br> VMware 機器-2.0.9177。0
...
> [!NOTE]
> 更新彙總套件48的已知問題，為使用 ADE 加密的 Linux 機器啟用複寫。 [深入了解](./azure-to-azure-troubleshoot-errors.md#enable-protection-failed-as-the-installer-is-unable-to-find-the-root-disk-error-code-151137)。
...
## <a name="updates-june-2020"></a>更新 (2020 年6月) 
...
## <a name="updates-january-2020"></a>2020年1月 (更新) 
...
**更新** | **詳細資料**
--- | ---
**提供者和代理程式** | Site Recovery 代理程式和提供者的更新 (如匯總中所詳述) 
**問題修正/改進** |  (如匯總中所述的一些修正和改善) 


## <a name="updates-november-2019"></a>2019年11月 (更新) 
...
**Linux** | Site Recovery 現在支援執行 Linux 並具有 Azure 磁碟加密 (ADE) 的 Azure Vm。
**第 2 代** | 所有第2代 Azure Vm 現在都支援損毀修復。
...
- 從適用于 Azure Vm) 的行動服務 (延伸模組的版本9.30.5407.1，以及適用于 VMware/實體機器的行動服務代理程式 () ，某些電腦作業系統必須執行服務堆疊更新和 SHA-1。 下表顯示詳細資料。
- 請根據連結的 KB 安裝 update 和 SHA-1。 2019 年 9 月起不支援 SHA-1，而且如果未啟用 SHA-2 程式碼簽署，代理程式擴充功能將不會如預期般安裝/升級。
...
**復原計畫** | 復原方案現在限制為100個 Vm，以確保容錯移轉的可靠性。
...
## <a name="updates-september-2019"></a>2019年9月 (更新) 
...
## <a name="updates-august-2019"></a>2019年8月 (更新) 
...
## <a name="updates-may-2019"></a>更新 (2019) 
...
**更新** | **詳細資料**
--- | ---
**提供者和代理程式** | Site Recovery 代理程式和提供者的更新 (如匯總) 中所述。
**問題修正/改進** | 匯總) 中 (的一些修正和改善。
...
**儲存空間直接存取** | 您可以使用[儲存空間直接存取](/windows-server/storage/storage-spaces/storage-spaces-direct-overview)以取得高可用性，針對在 Azure VM 應用程式上執行的應用程式設定嚴重損壞修復。  使用儲存空間直接存取 (S2D) 搭配 Site Recovery 可提供 Azure VM 工作負載的完整保護。 S2D 可讓您在 Azure 中裝載來賓叢集。 當 VM 裝載重要的應用程式（例如 SAP ASCS 層、SQL Server 或向外延展檔案伺服器）時，這特別有用。
...
**儲存空間直接存取** | Site Recovery 使用儲存空間直接存取 (S2D) 來支援 Azure Vm。
...
**功能** | **詳細資料**
--- | ---
**Linux 支援** |  已針對 Oracle Linux 6.8、Oracle Linux 6.9 和 Oracle Linux 7.0 與 Red Hat 相容核心，以及 Unbreakable Enterprise Kernel (UEK) 第5版）新增支援。
...
### <a name="automatic-updates-for-the-mobility-service-azure-vms"></a> (Azure Vm) 的行動服務自動更新
...
**支援磁片加密** | 針對使用 Azure 磁碟加密 (ADE) 以 Azure AD 應用程式加密的 Azure Vm 的嚴重損壞修復新增支援。 [深入了解](azure-to-azure-how-to-enable-replication-ade-vms.md)。
...
## <a name="updates-august-2018"></a>2018年8月 (更新) 
...
**功能** | **詳細資料**
--- | ---
**Linux 支援** | 已針對 RedHat Enterprise Linux 6.10、CentOS 6.10 新增支援。<br/><br/> 現在支援使用 GUID 磁碟分割表格 (GPT) 磁碟分割樣式的 Linux 型 Vm。 如需詳細資訊，請參閱[AZURE VM 常見問題](../virtual-machines/linux/faq-for-disks.md)。
**遷移後 Vm 的嚴重損壞修復** | 支援針對遷移至 Azure 的內部部署 VMware VM，對次要區域啟用嚴重損壞修復，而不需要先卸載 VM 上的行動服務，再啟用複寫。
**Windows Server 2008** | 支援將執行 Windows Server 2008 R2/2008 64 位和32位的電腦進行遷移。<br/><br/> 僅 (複寫和容錯移轉) 進行遷移。 不支援容錯回復。
