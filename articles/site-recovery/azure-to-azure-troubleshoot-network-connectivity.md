...
ms.locfileid: "87835879"
---
# <a name="troubleshoot-azure-to-azure-vm-network-connectivity-issues"></a>針對 Azure 至 Azure VM 網路連線問題進行疑難排解

本文說明當您將 Azure 虛擬機器從一個區域 (VM) 複寫和復原到另一個區域時，與網路連線相關的常見問題。 如需網路需求的詳細資訊，請參閱複寫[Azure vm 的連線能力需求](azure-to-azure-about-networking.md)。
...
無法建立 Site Recovery 端點的連線，因為網域名稱系統 (DNS) 解析失敗。 當您已故障處理 VM，但無法從嚴重損壞修復 (DR) 區域連線到 DNS 伺服器時，此問題更常見。
...
> [!NOTE]
> 如果 Vm 位於**標準**內部負載平衡器後方，則根據預設，它無法存取 Office 365 ip，例如 `login.microsoftonline.com` 。 將它變更為 [**基本**內部負載平衡器類型] 或 [建立輸出存取]，如[使用 Azure CLI 在 Standard Load Balancer 中設定負載平衡和輸出規則](../load-balancer/quickstart-load-balancer-standard-public-cli.md?tabs=option-1-create-load-balancer-standard#create-outbound-rule-configuration)一文所述。
...
### <a name="issue-4-azure-to-azure-replication-failed-when-the-network-traffic-goes-through-on-premises-proxy-server-151072"></a>問題4：當網路流量通過內部部署 proxy 伺服器時，Azure 到 Azure 複寫失敗 (151072) 

#### <a name="possible-cause"></a>可能的原因

自訂 proxy 設定無效，而且 Azure Site Recovery 行動服務代理程式未從 Internet Explorer (IE) 自動偵測 proxy 設定。
