---
title: 使用 Azure 網站恢復為 IIS Web 應用設定災難復原
...
重要的 Web 應用程式通常會設定為多層式應用程式：Web、資料庫及應用程式分別位於不同層上。 除了會分散於各層外，應用程式也可在每一層中使用多部伺服器來平衡流量負載。 此外，各層之間和 Web 伺服器上的對應可能會以靜態 IP 位址為基礎。 在容錯移轉時，需要更新其中一些對應，尤其是如果在 Web 伺服器上設定了多個網站。 如果 Web 應用程式使用 TLS,則必須更新證書綁定。
...
如果您使用靜態 IP 位址，您可以指定您想要虛擬機器使用的 IP 位址。 要設置 IP 位址,請造訪 **「計算和網路設定** > **目標 IP」。。**
...
![顯示設定 TLS/SSL 結合螢幕擷取](./media/site-recovery-iis/sslbinding.png)
...
#### <a name="tlsssl-certificate-binding-for-an-https-connection"></a>HTTPS 連線的 TLS/SSL 憑證繫結
網站可能具有關聯的 TLS/SSL 憑證,有助於確保 Web 伺服器和用戶瀏覽器之間的安全通信。 如果網站具有 HTTPS 連接,並且具有與具有 TLS/SSL 憑證綁定的 IIS 伺服器 IP 位址綁定的關聯 HTTPS 網站,則必須為證書添加新網站綁定,該證書具有 IIS 虛擬機故障轉移後的 IP 位址。

TLS/SSL 憑證可以針對以下元件的發行:

* 網站的完整網域名稱。
* 伺服器的名稱。
* 網域名稱的萬用字元憑證。  
* IP 位址。 如果針對 IIS 伺服器的 IP 位址頒發的 TLS/SSL 憑證,則需要針對 Azure 網站上的 IIS 伺服器的 IP 位址頒發另一個 TLS/SSL 憑證。 需要為此證書創建其他 TLS 綁定。 因此,我們建議不要使用針對 IP 位址頒發的 TLS/SSL 證書。 此選項較不常用，而且很快就會按照新的憑證授權單位/瀏覽器論壇變更而被取代。
