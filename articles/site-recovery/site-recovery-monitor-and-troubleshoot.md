...
在本文中，瞭解如何使用網站恢復內置監視來監視 Azure[網站恢復](site-recovery-overview.md)。  您可以監視：
...
您可能需要在開始之前查看[常見的監視問題](monitoring-common-questions.md)。

## <a name="monitor-in-the-dashboard"></a>儀表板中的監視器

1. 在保存庫中，按一下 **"概述**"。 恢復服務儀表板將保存庫的所有監視資訊合併到單個位置。 網站恢復和 Azure 備份服務都有頁面，您可以在它們之間切換。
...
3. 在 **"複製專案"** 中，按一下"**全部查看"** 以查看保存庫中的所有伺服器。
4. 按一下每個部分中的狀態詳細資訊以向下切入。
5. 在**基礎結構視圖中**，按要複製的電腦類型對監視資訊進行排序。

## <a name="monitor-replicated-items"></a>監視複寫的項目

在 **"複製"項**中，監視啟用複製的保存庫中所有電腦的運行狀況。
...
- 建議您每六個月就至少對複寫的機器執行一次測試容錯移轉。 這是一種在不中斷生產環境的情況下檢查容錯移轉是否按預期工作的方法。 
...
在**錯誤摘要中**，監視可能影響保存庫中伺服器複製的當前活動錯誤症狀，並監視受影響的電腦數。

- 顯示影響本地基礎結構元件的錯誤是本節的開頭。 例如，未從本地佈建服務器上的 Azure 網站恢復提供程式或 Hyper-V 主機接收活動訊號。
- 接著會顯示影響已複寫伺服器的複寫錯誤症狀。
- 資料表項目會先依錯誤嚴重性遞減排序，再依受影響機器的計數遞減排序。
- 受影響的伺服器計數是瞭解單個基礎問題是否會影響多台電腦的有用方法。 例如，網路故障可能會影響複製到 Azure 的所有電腦。 
- 單一伺服器上可能會發生多個複寫錯誤。 在此情況下，每個錯誤症狀都會將該伺服器計入受此症狀影響的伺服器清單中。 問題修復後，複寫參數會改善，錯誤也會從該機器中清除。

## <a name="monitor-the-infrastructure"></a>監視基礎結構。

在**基礎結構視圖中**，監視複製中涉及的基礎結構元件以及伺服器和 Azure 服務之間的連接運行狀況。

- 綠線表示連線狀況良好。
- 帶有疊加錯誤圖示的紅線表示存在一個或多個影響連接的錯誤症狀。
...
[瞭解如何](monitor-log-analytics.md)使用 Azure 監視器監視網站恢復。
