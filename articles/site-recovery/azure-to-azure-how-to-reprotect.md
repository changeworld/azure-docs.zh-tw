...
1. 在 [保存**庫** > ] [已複寫的**專案**] 中，以滑鼠右鍵按一下故障的 VM，然後選取 [**重新保護**]。 重新保護的顯示方向應該是從次要到主要。
...
1. 如果目標儲存體帳戶 (主要區域中的原始儲存體帳戶) 不存在，即會建立新的帳戶。 指派的儲存體帳戶名稱是次要 VM 所使用的儲存體帳戶名稱，加`asr`上尾碼。
...
1. 如果 VM 使用受控磁片，則會以`-ASRReplica`尾碼建立原始磁片的複本。 原始磁碟則會被刪除。 這些`-ASRReplica`複本會用於複寫。
...
1. 如果 VM 使用受控磁片，則會建立具有尾碼的`-ASRReplica`複本磁片。 這些`-ASRReplica`複本會用於複寫。
