---
title: 在自訂策略中定義 OAuth2 技術設定檔
titleSuffix: Azure AD B2C
description: 在 Azure 活動目錄 B2C 中的自訂策略中定義 OAuth2 技術設定檔。
...
---
# <a name="define-an-oauth2-technical-profile-in-an-azure-active-directory-b2c-custom-policy"></a>在 Azure 活動目錄 B2C 自訂策略中定義 OAuth2 技術設定檔

[!INCLUDE [active-directory-b2c-advanced-audience-warning](../../includes/active-directory-b2c-advanced-audience-warning.md)]

Azure 活動目錄 B2C（Azure AD B2C）支援 OAuth2 協定標識提供程式。 OAuth2 是授權和委派身份驗證的主要協定。 如需詳細資訊，請參閱 [RFC 6749 The OAuth 2.0 授權架構](https://tools.ietf.org/html/rfc6749)。 借助 OAuth2 技術設定檔，您可以與基於 OAuth2 的標識供應商（如 Facebook）聯合。 與標識提供程式聯合允許使用者使用其現有社交或企業標識登錄。
...
| scope | 否 | 根據 OAuth2 標識提供程式規範定義的請求範圍。 例如，`openid`、`profile` 和 `email`。 |
...
| 屬性 | 必要 | 描述 |
| --------- | -------- | ----------- |
| client_secret | 是 | 識別提供者應用程式的用戶端密碼。 只有在 **response_types** 中繼資料設為 `code` 時，才需要密碼編譯金鑰。 在此情況下，Azure AD B2C 會進行另一次呼叫，以交換存取權杖的授權碼。 如果中繼資料設置為`id_token`，則可以省略加密金鑰。 |
