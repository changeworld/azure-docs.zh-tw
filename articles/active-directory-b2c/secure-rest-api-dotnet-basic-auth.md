...
在 [Azure AD B2C 相關文章](rest-api-claims-exchange-dotnet.md)中，您在未經驗證的情況下，即建立了與 Azure Active Directory B2C (Azure AD B2C) 使用者旅程圖整合的 RESTful 服務 (Web API)。
...
完成[將 REST API 宣告交換整合到 Azure AD B2C 使用者旅程圖中](rest-api-claims-exchange-dotnet.md)文章的步驟。
...
1. 以滑鼠右鍵按一下 App_Start 資料夾，選取 [新增]，然後選取 [類別]。
   ...
   ![在 App_Start 資料夾中新增 ClientAuthMiddleware.cs 類別](./media/secure-rest-api-dotnet-basic-auth/rest-api-netfw-secure-basic-OWIN-startup-auth1.png)

2. 在 [名稱] 方塊中，輸入 **ClientAuthMiddleware.cs**。

   ![在的 [ C#加入新專案] 對話方塊中建立新的類別 Visual Studio](./media/secure-rest-api-dotnet-basic-auth/rest-api-netfw-secure-basic-OWIN-startup-auth2.png)
...
1. 在專案上按一下滑鼠右鍵，選取 [新增] > [新增項目]，然後搜尋 **OWIN**。
   ...
   ![在 Visual Studio 中的 [新增專案] 對話方塊中建立 OWIN 啟動類別](./media/secure-rest-api-dotnet-basic-auth/rest-api-netfw-secure-basic-OWIN-startup.png)
...
![將授權標記新增至控制器](./media/secure-rest-api-dotnet-basic-auth/rest-api-netfw-secure-basic-authorize.png)
...
5. 在關閉 `<Metadata>` 元素之後，立即新增下列 XML 程式碼片段：
    ...
    ![將基本驗證 XML 元素新增至 TechnicalProfile](./media/secure-rest-api-dotnet-basic-auth/rest-api-netfw-secure-basic-add-1.png)
...
3. 在 [名字] 方塊中，輸入 **Test** 來測試流程。
    ...
    ![在您的身分識別 API 中測試指定的名稱輸入驗證](./media/secure-rest-api-dotnet-basic-auth/rest-api-netfw-test.png)
...
* 在完成[開始使用自訂原則](custom-policy-get-started.md)逐步解說之後，建議您使用自己的自訂原則檔案來建置您的情節。 我們已提供[範例原則檔案](https://github.com/Azure-Samples/active-directory-b2c-custom-policy-starterpack/tree/master/scenarios/aadb2c-ief-rest-api-netfw-secure-basic)，供您參考。
...
* [使用用戶端憑證來保護您的 RESTful API](secure-rest-api-dotnet-certificate-auth.md)
