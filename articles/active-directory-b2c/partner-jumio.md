...
3. 遵循 [本文中的](../app-service/configure-ssl-certificate.md#upload-a-private-certificate)指示，將憑證匯入 Azure App Service。
...
您可以 [在 Azure App Service 中設定應用程式設定](../app-service/configure-common.md#configure-app-settings)。 使用這個方法，您可以安全地進行設定，而不需要將它們簽入存放庫。 您必須提供下列設定給 Rest API：
...
2. 請遵循 [這篇文章](./custom-policy-get-started.md?tabs=applications#custom-policy-starter-pack) 來下載 [LocalAccounts 入門套件](https://github.com/Azure-Samples/active-directory-b2c-custom-policy-starterpack/tree/master/LocalAccounts)。
