...
| Description | 0:1 | 可向使用者顯示的技術設定檔描述。 |
...
| 屬性 | 必要 | 說明 |
| --------- | -------- | ----------- |
| Name | Yes | Azure AD B2C 所支援的有效通訊協定名稱，可用來作為技術設定檔的一部分。 可能的值： `OAuth1` 、 `OAuth2` 、 `SAML2` 、 `OpenIdConnect` 、 `Proprietary` 或 `None` 。 |
| 處理常式 | No | 當通訊協定名稱設為 `Proprietary` 時，請指定 Azure AD B2C 用於判斷通訊協定處理常式之組件的完整名稱。 |
...
| 屬性 | 必要 | 說明 |
| --------- | -------- | ----------- |
| Id | No | 原則檔中的其他元素所參考之特定金鑰組的唯一識別碼。 |
...
| DefaultValue | No | 當 ClaimTypeReferenceId 指示的宣告不存在時，此預設值可用於建立宣告，以便產生的宣告可由技術設定檔用來作為 InputClaim。 |
| PartnerClaimType | No | 指定的原則宣告類型對應之外部夥伴的宣告類型識別碼。 如果未指定 PartnerClaimType 屬性，則指定的原則宣告類型會對應至相同名稱的夥伴宣告類型。 當您的宣告類型名稱不同於其他合作對象時，請使用這個屬性。 例如，第一個宣告名稱是 'givenName'，而夥伴會使用名稱為 'first_name' 的宣告。 |
...
| 屬性 | 必要 | 說明 |
| --------- | -------- | ----------- |
| ClaimTypeReferenceId | No | 已在原則檔或父代原則檔之 ClaimsSchema 區段中定義的宣告類型識別碼。 |
| DisplayControlReferenceId | No | 已在原則檔或父原則檔的 ClaimsSchema 區段中定義的[顯示控制項](display-controls.md)識別碼。 |
| 必要 | No | 指出是否需要顯示宣告。 |
