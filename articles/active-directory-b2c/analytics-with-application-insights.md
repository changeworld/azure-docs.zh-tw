---
title: 使用應用程式見解跟蹤使用者行為
titleSuffix: Azure AD B2C
description: 瞭解如何使用自訂策略從 Azure AD B2C 使用者旅程中啟用應用程式見解中的事件日誌。
...
[應用程式見解](application-insights-technical-profile.md)技術設定檔定義 Azure AD B2C 的事件。 這個設定檔會指定事件的名稱、所記錄的宣告及檢測金鑰。 要發佈事件，技術設定檔將添加為[使用者旅程](userjourneys.md)中的業務流程步驟。
...
聲明在 Azure AD B2C 策略執行期間提供臨時資料存儲。 [聲明架構](claimsschema.md)是聲明聲明的位置。
...
| 技術設定檔 | Task |
| ----------------- | -----|
| 應用見解-通用 | 要包含在所有 Azure Insights 技術設定檔中的通用參數集。 |
| 應用見解-登錄請求 | 收到登錄`SignInRequest`請求時，使用一組聲明記錄事件。 |
| 應用見解-使用者簽名 | 記錄使用者`UserSignUp`在註冊/登錄旅程中觸發註冊選項時的事件。 |
| 應用見解-簽名完成 | 記錄成功`SignInComplete`完成身份驗證時的事件，當令牌已發送到依賴方應用程式時。 |
...
## <a name="optional-collect-more-data"></a>[可選]收集更多資料

將宣告類型和事件新增至您的使用者旅程圖，以符合您的需求。 您可以使用[聲明解析器](claim-resolver-overview.md)或任何字串聲明類型，通過將**輸入聲明**元素添加到應用程式見解事件或 AppInsights-通用技術設定檔來添加聲明。
...
- 在 IEF 參考中瞭解有關[應用程式見解](application-insights-technical-profile.md)技術設定檔的更多詳細資訊。 