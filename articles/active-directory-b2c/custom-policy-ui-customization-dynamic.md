...
本文著重於如何使用自訂原則，自訂具有「動態內容」的 Azure AD B2C 使用者介面。 一開始，請參閱[自訂原則中的 UI 自訂](custom-policy-ui-customization.md)。

>[!NOTE]
>Azure AD B2C 的[設定自訂原則中的 UI 自訂](custom-policy-ui-customization.md)一文討論下列基本概念：
> * 頁面使用者介面 (UI) 自訂功能。
...
在[在自訂原則中設定 UI 自訂](custom-policy-ui-customization.md)一文中，您會將 HTML5 檔案上傳至 Azure Blob 儲存體。 這些 HTML5 檔案都是靜態，並且對每個要求呈現相同的 HTML 內容。
...
3. 為應用程式命名 (例如 Contoso.AADB2C.UI)，然後選取 [確定]。
    ...
    ![建立新的 Visual Studio 專案](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-create-project1.png)

4. 選取 [Web 應用程式] 範本。

5. 將驗證設為 [沒有驗證]。

    ![選取 Web 應用程式範本](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-create-project2.png)
...
1. 以滑鼠右鍵按一下 [檢視]/[首頁] 資料夾，然後按一下 [新增] > [新增項目]。
    ...
    ![Visual Studio 中的 [加入新專案] 功能表項目](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-view1.png)
...
5. 選取 [新增]。
    ...
    ![反白顯示 MVC 視圖頁面的 Visual Studio 中的 [新增專案] 對話方塊](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-view2.png)
...
10. 複製檔案的內容，並將它在版面配置定義下貼上。 您的程式碼看起來應該類似：
    ...
    ![新增 HTML5 之後的 unified.cshtml 檔案](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-edit-view1.png)
...
![具有自訂 background_background_image src 值的 img 元素](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-static-background.png)
...
1. 開啟 **Controllers\HomeController.cs**，然後新增下列方法：
    ...
    ![變更控制器來呈現檢視](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-controller-view.png)
...
1. 在 [方案總管] 中，以滑鼠右鍵按一下 **Contoso.AADB2C.UI** 專案，然後選取 [發佈]。
    ...
    ![發佈至 Microsoft Azure App Service](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish1.png)

2. 選取 [Microsoft Azure App Service] 圖格，然後選取 [發佈]。

    ![建立新的 Microsoft Azure App Service](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish2.png)
...
4. 選取 [建立] 開始建立 Azure 資源。
    ...
    ![提供 App Service 屬性](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish3.png)
...
1. 從 [Azure 入口網站](https://portal.azure.com/)中，選取 [應用程式服務]，然後選取 API 應用程式的名稱。
    ...
    ![在 Azure 入口網站中選取 API 應用程式](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS1.png)

2. 在 [設定] 區段的 [API] 區段之下，選取 [CORS]。

    ![在 Azure 入口網站的 App Service 功能表中反白顯示的 CORS 功能表項目](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS2.png)
...
4. 選取 [儲存]。
    ...
    ![在允許的原始來源中反白顯示星號的 CORS 設定頁面](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS3.png)
...
6. 將 `LoadUri` 的值從 ~/tenant/default/unified 變更為 https://<app_name>.azurewebsites.net/home/unified。
    ...
    ![已反白顯示 LoadUri 元素的範例 XML 程式碼片段](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-content-definition.png)
...
1. 選取 [Azure AD B2C 設定]，然後選取 [識別體驗架構]。
    ...
    >[!NOTE]
    >[立即執行] 需要在租用戶上至少預先註冊一個應用程式。 若要了解如何註冊應用程式，請參閱 Azure AD B2C [開始使用](tutorial-create-tenant.md)一文或[應用程式註冊](tutorial-register-applications.md)一文。

2. 開啟 **B2C_1A_signup_signin** (此為您上傳的信賴憑證者 (RP) 自訂原則)，然後選取 [立即執行]。
    您應該能夠看到您的自訂 HTML5 具有您稍早建立的背景。

    ![您的註冊或登入原則](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo1.png)
...
2. 找出具有識別碼 `background_background_image` 的 `<img>` 元素，並以 `@ViewData["background"]` 取代 `src` 值。
    ...
    ![已反白顯示 src 值的 img 元素 ](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-dynamic-background.png)
...
5. 將 campaignId 查詢字串參數新增至此 URI。 例如，新增 `&campaignId=hawaii`，如下圖所示：
    ...
    ![已反白顯示 campaignId 查詢字串參數的 URI](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-campaignId-param.png)

6. 選取 **Enter** 以顯示夏威夷背景影像。

    ![使用夏威夷影像自訂背景的註冊登入頁面](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo2.png)

7. 將值變更為 Tokyo，然後選取 **Enter**。
    瀏覽器會顯示東京背景。

    ![具有東京影像自訂背景的註冊登入頁面](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo3.png)
...
* 在完成[開始使用自訂原則](custom-policy-get-started.md)逐步解說之後，建議您使用自己的自訂原則檔案來建置您的情節。 我們已提供[範例原則檔案](https://github.com/Azure-Samples/active-directory-b2c-custom-policy-starterpack/tree/master/scenarios/aadb2c-ief-ui-customization)，供您參考。
