...
在`SocialAndLocalAccounts\` **`TrustFrameworkExtensions.xml`** 自訂原則入門套件中開啟。

找出`<ClaimsProviders>`區段並加入下列 XML 程式碼片段。
...
1. 將原則`PolicyId`的`PublicPolicyUri`和變更為_B2C_1A_signup_signin_saml_ ， `http://tenant-name.onmicrosoft.com/B2C_1A_signup_signin_saml`如下所示。
...
1. 在`<RelyingParty>`專案之前加入下列 XML 程式碼片段。 此 XML 會覆寫_signuporsignin.xml_使用者旅程圖的協調流程步驟號碼7。 如果您從入門套件中的不同資料夾啟動，或藉由新增或移除協調流程步驟來自訂您的使用者旅程圖，請確定該`order`數位（在專案中）與權杖簽發者步驟的使用者旅程圖中所指定的數目一致（例如，在其他入門套件資料夾中`LocalAccounts`，第4個步驟， `SocialAccounts`適用于 9 `SocialAndLocalAccountsWithMfa`個）。
...
1. 使用下列技術`<TechnicalProfile>`設定檔 XML `<RelyingParty>`來取代元素中的整個元素。
...
1. 使用`tenant-name`您的 Azure AD B2C 租使用者名稱進行更新。
...
Azure AD B2C 原則 IDP 中繼資料是 SAML 通訊協定中用來公開 SAML 識別提供者設定的資訊。 中繼資料會定義服務的位置，例如，登入和登出、憑證、登入方法，以及其他項目。 Azure AD B2C 的原則中繼資料可在下列 URL 取得。 請`tenant-name`以您的 Azure AD B2C 租使用者名稱取代， `policy-name`並以原則的名稱（ID）取代：
...
1. 在 [重新**導向 URI**] 底下，選取 [ `https://localhost` **Web**]，然後輸入。 您稍後會在應用程式註冊的資訊清單中修改此值。
1. 選取 [註冊]  。
...
`identifierUris`是包含使用者定義 URI 的字串集合，可唯一識別其 Azure AD B2C 租使用者內的 Web 應用程式。 您的服務提供者必須在 SAML 要求`Issuer`的元素中設定此值。
...
針對使用 SAML 測試應用程式的本教學課程，請使用下列值`samlMetadataUrl`：
...
如果您未提供中繼資料 URI，可以明確指定回復 URL。 這個選擇性屬性`AssertionConsumerServiceUrl`代表（`SingleSignOnService`服務提供者中繼資料中的 URL）， `BindingType`而則假設為`HTTP POST`。

如果您選擇在應用程式資訊清單中設定 [回復 URL] 和 [登出 URL]，而不使用服務提供者中繼資料，Azure AD B2C 將不會驗證 SAML 要求籤章，也不會加密 SAML 回應。

在此教學課程中，您會使用 SAML 測試應用程式，將`url`的屬性`replyUrlsWithType`設定為下列 JSON 程式碼片段中所示的值。
...
此選擇性`Logout`屬性代表 url （`SingleLogoutService`信賴憑證者中繼資料中的 url），而`BindingType`這個的會假設為。 `Http-Redirect`

針對使用 SAML 測試應用程式的本教學課程，請`logoutUrl`將設定`https://samltestapp2.azurewebsites.net/logout`為：
...
選取 **[登**入]，您應該會看到一個使用者登入畫面。 登入時，會將 SAML 判斷提示回傳給範例應用程式。
...
- 從[Azure AD B2C GitHub 社區](https://github.com/azure-ad-b2c/saml-sp-tester)存放庫取得 SAML 測試 web 應用程式。
