...
Azure Active Directory B2C (Azure AD B2C) 會在處理每個驗證流程時發出數種安全性權杖。 SAML 權杖簽發者的技術設定檔會發出傳回給信賴憑證者應用程式（服務提供者）的 SAML 權杖。 此技術設定檔通常是使用者旅程圖中的最後一個協調流程步驟。
...
若要設定信賴憑證者應用程式（專案的屬性`UseTechnicalProfileForSessionManagement` ）之間的 Azure AD B2C SAML 會話，請參考[SamlSSOSessionProvider](custom-policy-reference-sso.md#samlssosessionprovider) SSO 會話。
