...
本文提供了在 Azure 活動目錄 B2C （Azure AD B2C） 中使用標識體驗框架架構架構的字串集合聲明轉換的示例。 如需詳細資訊，請參閱 [ClaimsTransformations](claimstransformations.md)。
...
| OutputClaim | collection | stringCollection | 已調用此聲明轉換後生成的聲明類型，並在輸入聲明中指定值。 |
...
檢查 StringCollection 聲明類型是否包含元素
...
下面的示例檢查`roles`字串集合聲明類型是否包含**管理員**的值。
