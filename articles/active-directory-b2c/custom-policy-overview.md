...
自訂策略是定義 Azure 活動目錄 B2C （Azure AD B2C） 租戶行為的設定檔。 對於最常用的身分識別工作，Azure AD B2C 入口網站中預先定義了使用者流程。 身分識別開發人員可以完全編輯自訂原則，以完成許多不同的工作。

## <a name="comparing-user-flows-and-custom-policies"></a>比較使用者流程與自訂原則

| | 使用者流程 | 自訂原則 |
|-|-------------------|-----------------|
| 目標使用者 | 所有具備或不具備身分識別專業知識的應用程式開發人員。 | 身分識別專業人員、系統整合人員、顧問和內部身分識別小組。 他們熟悉 OpenID Connect 流，並瞭解標識提供程式和基於聲明的身份驗證。 |
...
| 識別提供者 | 預定義的本地或社交提供程式和大多數 OIDC 標識提供程式，例如與 Azure 活動目錄租戶的聯合。 | 標準式 OIDC、OAUTH 和 SAML。  通過使用與 REST API 的集成，也可以進行身份驗證。 |
...
身份體驗框架是一個完全可配置、策略驅動的基於雲的 Azure 平臺，以標準協定格式（如 OpenID Connect、OAuth、SAML）和一些非標準協定格式（例如 REST）協調實體之間的信任基於 API 的系統到系統聲明交換。 此架構會建立方便使用、白色標籤體驗，支援 HTML 和 CSS。
...
> [開始使用自訂策略](custom-policy-get-started.md)
