...
| 將現有應用程式遷移至 b2clogin.com | Login.microsoftonline.com 的取代將于2020年12月的所有 Azure AD B2C 租使用者生效。 [進一步瞭解](b2clogin.md)。 |
| 使用 Identity Protection 和條件式存取 | 使用這些功能可大幅提高對具風險驗證和存取原則的控制。 需要 Azure AD B2C Premium P2。 [進一步瞭解](conditional-access-identity-protection-overview.md)。 |
...
| 瞭解如何針對 Azure AD B2C 進行疑難排解 | 瞭解如何在開發期間針對 [自訂原則進行疑難排解](https://docs.microsoft.com/azure/active-directory-b2c/troubleshoot-custom-policies?tabs=applications) 。 瞭解一般驗證流程的外觀，並使用工具來探索異常和錯誤。 例如，使用 [Application Insights](troubleshoot-with-application-insights.md) 來檢查使用者旅程的輸出記錄。 |
...
| 畫筆測試 | 在使用您的解決方案之前，請執行滲透測試練習來確認所有元件都是安全的，包括任何協力廠商相依性。 確認您已使用存取權杖保護您的 Api，並針對您的應用程式案例使用正確的驗證通訊協定。 深入瞭解 [滲透測試](https://docs.microsoft.com/azure/security/fundamentals/pen-testing) 和 [Microsoft Cloud 整合滲透測試的參與規則](https://www.microsoft.com/msrc/pentest-rules-of-engagement?rtc=1)。 |
