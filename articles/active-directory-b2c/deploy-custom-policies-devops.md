...
> [!IMPORTANT]
> 使用 Azure 管線管理 Azure AD B2C 自訂原則目前會**preview**使用 Microsoft Graph API `/beta`端點上提供的預覽作業。 不支援在生產應用程式中使用這些 API。 如需詳細資訊，請參閱[Microsoft Graph REST API Beta 版端點參考](https://docs.microsoft.com/graph/api/overview?toc=./ref/toc.json&view=graph-rest-beta)。
...
如果您已經有用於自動化工作的應用程式註冊，請確定它已被授與在應用程式註冊之**API 許可權**內的**Microsoft Graph** > **原則** > **TrustFramework**許可權。
...
如果工作順利完成，請針對每個自訂原則檔案執行上述步驟，以新增部署工作。 修改每`-PolicyId`個`-PathToFile`原則的和引數值。

`PolicyId`是在 TrustFrameworkPolicy 節點中的 XML 原則檔案開頭找到的值。 例如，下列原則`PolicyId` XML 中的*B2C_1A_TrustFrameworkBase*：
