...
若要以正規化宣告的形式讀取**主體**中的 SAML 判斷提示**NameId** ，請將**PartnerClaimType**的宣告設定`SPNameQualifier`為屬性的值。 如果未`SPNameQualifier`提供屬性，請將宣告**PartnerClaimType**設定為`NameQualifier`屬性的值。 
...
如果 SAML `SPNameQualifier`判斷`NameQualifier`提示中未顯示這兩個或屬性，請將**PartnerClaimType**宣告 PartnerClaimType `assertionSubjectName`設定為。 請確定 **NameId** 是判斷提示 XML 中的第一個值。 如果您定義一個以上的判斷提示，Azure AD B2C 會挑選最後一個判斷提示中的主體值。
