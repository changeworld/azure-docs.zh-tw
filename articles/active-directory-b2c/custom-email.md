...
1. 展開左側的 [**設定**]，針對 [**電子郵件**主旨`{{subject}}`]，輸入。
1. 選取 [**儲存範本**]。
...
在您的原則中，將下列宣告類型新增`<ClaimsSchema>`至內`<BuildingBlocks>`的元素。
...
將下列宣告轉換新增至內`<ClaimsTransformations>` `<BuildingBlocks>`的元素。 對宣告轉換 XML 進行下列更新：

* 使用您`template_id`稍早在[建立 SendGrid 範本](#create-sendgrid-template)中建立的 SendGrid 交易式範本的識別碼來更新 InputParameter 值。
* 更新`from.email`位址值。 請使用有效的電子郵件地址，協助防止驗證電子郵件被標示為垃圾郵件。
* 以適用于您組織`personalizations.0.dynamic_template_data.subject`的主旨行，更新主旨行輸入參數的值。
...
在內`<BuildingBlocks>`的宣告轉換底下，新增下列[ContentDefinition](contentdefinitions.md)以參考版本2.0.0 資料 URI：
...
1. 從使用者`email`收集位址宣告類型。
1. 等待使用者以傳送給使用者的`verificationCode`程式碼來提供宣告類型。
1. `email`回到具有此顯示控制項參考的自我判斷技術設定檔。
...
在 [內容定義] 下`<BuildingBlocks>`，仍在中，[將下列類型](display-controls.md)的[VerificationControl](display-control-verification.md)新增至您的原則。
...
在最後一個步驟中，新增您所建立之控制項的參考。 如果您使用`LocalAccountSignUpWithLogonEmail`舊版的 Azure AD B2C 原則，請以下列程式碼取代現有的自我判斷技術設定檔。 此技術設定檔`DisplayClaims`會使用具有加入控制項的參考。
...
若要將電子郵件當地語系化，您必須將當地語系化的字串傳送至 SendGrid 或您的電子郵件提供者。 例如，將電子郵件主旨、本文、您的程式碼訊息或電子郵件簽章當地語系化。 若要這樣做，您可以使用[GetLocalizedStringsTransformation](string-transformations.md)宣告轉換，將當地語系化的字串複製到宣告類型。 在產生`GenerateSendGridRequestBody` JSON 承載的宣告轉換中，會使用包含當地語系化字串的輸入宣告。
