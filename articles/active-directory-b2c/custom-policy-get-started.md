...
1. 在 [重新**導向 URI**] 底下，選取 [ `https://your-tenant-name.b2clogin.com/your-tenant-name.onmicrosoft.com` **Web**]，然後輸入，其中`your-tenant-name`是您的 Azure AD B2C 租使用者功能變數名稱。
...
1. 在**SocialAndLocalAccounts**目錄中的所有檔案中，將字串`yourtenant`取代為 Azure AD B2C 租使用者的名稱。
    ...
    例如，如果您的 B2C 租使用者名稱是*contosotenant*，的`yourtenant.onmicrosoft.com`所有實例就會`contosotenant.onmicrosoft.com`變成。
...
> [!TIP]
> 如果您的 XML 編輯器支援驗證，請針對位於入門`TrustFrameworkPolicy_0.3.0.0.xsd`套件根目錄中的 xml 架構來驗證檔案。 在上載之前，XML 結構描述驗證會識別錯誤。
...
1. 在檔案中，將的值取代`client_id`為 Facebook 應用程式識別碼： `SocialAndLocalAccounts/` **`TrustFrameworkExtensions.xml`**
