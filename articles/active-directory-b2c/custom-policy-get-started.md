...
description: 瞭解如何在 Azure 活動目錄 B2C 中開始自定義策略。
...
[自定義策略](custom-policy-overview.md)是定義 Azure 活動目錄 B2C (Azure AD B2C) 租戶行為的配置檔。 在此文章中，您會建立自訂原則，使用電子郵件地址與密碼來支援本機帳戶註冊或登入。 您也會準備環境以新增識別提供者。

## <a name="prerequisites"></a>Prerequisites

- 如果還沒有,請建立連結到 Azure 訂閱的[Azure AD B2C 租戶](tutorial-create-tenant.md)。
- 在創建的租戶中[註冊應用程式](tutorial-register-applications.md),以便它可以與 Azure AD B2C 通訊。
- 使用 Facebook[帳戶完成設置註冊和登錄](identity-provider-facebook.md)中的步驟,以配置 Facebook 應用程式。 儘管使用自定義策略不需要 Facebook 應用程式,但本演練中使用它來演示自定義策略中的啟用社交登錄。
...
將 Facebook 應用程式[的應用金鑰](identity-provider-facebook.md)添加為策略密鑰。 您可以使用作為本文先決條件一部分創建的應用程式的應用機密。
...
1. 針對 [名稱]  輸入 `IdentityExperienceFramework`。
1. 在**支援帳號類型**下,**選擇「 僅在此組織目錄中的帳戶**」 。
1. 在**重定向 URI**下,選擇`your-tenant-name`**Web** `https://your-tenant-name.b2clogin.com/your-tenant-name.onmicrosoft.com`,然後輸入 Azure AD B2C 租戶功能變數名稱所在的 位置。
...
1. 在 [管理]**** 底下，選取 [公開 API]****。
1. 選擇 **「新增作用域**」,然後選擇 **「保存並繼續**接受預設應用程式 ID URI」。。
1. 輸入以下值以建立允許在 Azure AD B2C 租戶執行自訂策略的範圍:
...
1. 針對 [應用程式類型]**** 選擇 [原生]****。
1. 對**一個信件的 URI,** 請`myapp://auth`輸入 。
1. 選取 [建立]  。 建立之後，複製應用程式識別碼，並儲存它以供日後使用。
1. 選擇 **「設定**」,然後選擇 **「必需權限**」,然後選擇「**新增**」 。
1. 選擇 **「選擇 API」 搜尋**並選擇 **「身份體驗框架**」,然後按下「**選擇**」 。
1. 選取 [存取 IdentityExperienceFramework]**** 旁的核取方塊、按一下 [選取]****，然後按一下 [完成]****。
1. 選擇 **「授予權限**」,然後選擇 **「 是**」進行確認。
...
1. 在 [管理]**** 底下，選取 [驗證]****。
1. 選取 [試用全新體驗]**** (若顯示的話)。
1. 在 **「進階設定**」 下,啟用**將應用程式視為公共用戶端**(選擇 **" 是**" 。
1. 選取 [儲存]  。
...
## <a name="custom-policy-starter-pack"></a>自訂策略啟動套件

自定義策略是一組 XML 檔,用於定義技術設定檔和使用者旅程。 我們提供入門包與幾個預先構建的策略,讓您快速前進。 每個初學者包都包含實現上述方案所需的最少數量的技術配置檔和使用者旅程:

- **本地帳戶**─ 僅啟用本地帳戶。
- **社交帳戶**- 僅啟用社交(或聯合)帳戶。
- **社交和本地帳戶**- 支援使用本地帳戶和社會帳戶。
- **社交和本地帳戶與MFA** - 啟用社交、本地和多重身份驗證選項。

每個入門套件均包含：

- **基本檔**- 基本檔不需要修改。 範例:*信任框架基礎.xml*
- **擴展檔案**- 此檔是進行大多數配置更改的地方。 範例:*信任框架延伸.xml*
- **相依方檔案**- 應用程式呼叫的工作特定檔。 範例:*註冊或Signin.xml,**設定檔編輯.xml,**密碼重置.xml*

在本文中,您可以在**社交和本地帳戶**初學者包中編輯 XML 自定義策略檔。 如果您需要 XML 編輯器,請嘗試[可視化工作室代碼](https://code.visualstudio.com/download),一個輕量級的跨平臺編輯器。

### <a name="get-the-starter-pack"></a>抓取入門包

從 GitHub 獲取自訂策略初學者包,然後使用 Azure AD B2C 租戶名稱更新社交和本地帳戶初學者包中的 XML 檔。

1. [下載 .zip 檔案](https://github.com/Azure-Samples/active-directory-b2c-custom-policy-starterpack/archive/master.zip)或複製儲存函式庫:
...
1. 在**社交和本地帳戶**目錄中的所有檔中,將字串`yourtenant`替換為 Azure AD B2C 租戶的名稱。
    ...
    例如,如果 B2C 租戶的名稱是*contoso 租戶*,則的所有`yourtenant.onmicrosoft.com`實體都`contosotenant.onmicrosoft.com`變為 。

### <a name="add-application-ids-to-the-custom-policy"></a>將應用程式識別碼新增至自訂原則

將應用程式識別碼新增至擴充檔案 *TrustFrameworkExtensions.xml*。

1. 開啟`SocialAndLocalAccounts/`**`TrustFrameworkExtensions.xml`** 並尋找元素`<TechnicalProfile Id="login-NonInteractive">`。
1. 將`IdentityExperienceFrameworkAppId`中的兩個實體取代為您之前創建的標識體驗框架應用程式的應用程式 ID。
1. 將中的`ProxyIdentityExperienceFrameworkAppId`兩個實體取代為之前創建的代理身份體驗框架應用程式的應用程式 ID。
1. 儲存檔案。

## <a name="upload-the-policies"></a>上傳原則

1. 在 Azure 門戶中的 B2C 租戶中選擇 **「身份體驗框架」** 功能表項。
1. 選擇 **「上載自定義策略**」。
1. 依此順序上載策略檔:
...
上載檔時,Azure 會將首`B2C_1A_`碼 添加到每個檔。

> [!TIP]
> 如果 XML 編輯器支援驗證,請根據位於`TrustFrameworkPolicy_0.3.0.0.xsd`初學者 包根目錄中的 XML 架構驗證檔。 在上載之前，XML 結構描述驗證會識別錯誤。

## <a name="test-the-custom-policy"></a>測試自訂原則

1. 在**自定義策略**下,選擇**B2C_1A_signup_signin。**
1. 有關在自定義策略的概述頁上**選擇應用程式**,請選擇以前註冊的名為*Webapp1*的 Web 應用程式。
1. 變更**回覆網址**為`https://jwt.ms`。
1. 選取 [立即執行]****。
1. 使用電子郵件地址註冊。
1. 再次選擇 **「立即運行**」。
...
如[先決條件](#prerequisites)中所述,Facebook*不需要*使用自定義策略,但在此處用於演示如何在自定義策略中啟用聯合社交登錄。

1. 在`SocialAndLocalAccounts/`**`TrustFrameworkExtensions.xml`** 檔案中,將`client_id`的值 替換為 Facebook 應用程式 ID:
...
1. 在**自定義策略**下,選擇**B2C_1A_signup_signin。**
1. 選擇 **「立即運行**」,選擇 Facebook 以使用 Facebook 登入並測試自訂策略。

## <a name="next-steps"></a>後續步驟

接下來,嘗試添加 Azure 活動目錄 (Azure AD) 作為標識提供程式。 此入門指南中使用的基本檔已包含添加其他標識提供者(如 Azure AD)所需的一些內容。

有關將 Azure AD 設定為與識別提供應用程式的資訊,請參閱[使用活動目錄 B2C 自訂政策使用 Azure 的目錄帳戶設定註冊和登入](identity-provider-azure-ad-single-tenant-custom.md)。
