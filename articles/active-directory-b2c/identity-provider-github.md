...
若要在 Azure Active Directory B2C （Azure AD B2C）中使用 GitHub 帳戶做為[識別提供者](authorization-code-flow.md)，您需要在代表它的租使用者中建立應用程式。 如果您還沒有 GitHub 帳戶，您可以在[https://www.github.com/](https://www.github.com/)註冊。
...
1. 在 [授權回呼 URL]`https://your-tenant-name.b2clogin.com/your-tenant-name.onmicrosoft.com/oauth2/authresp`**中輸入**。 將 `your-tenant-name` 取代為您的 Azure AD B2C 租用戶名稱。 即使租用戶在 Azure AD B2C 中是使用大寫字母來定義的，仍請在輸入租用戶名稱時，全部使用小寫字母。
