...
description: 使用 Azure Active Directory B2C 中的任何 OpenID Connect 識別提供者 (IdP) 來設定註冊和登入。
...
[OpenID Connect](openid-connect.md) 是建置於 OAuth 2.0 之上的驗證通訊協定，可用於安全使用者登入。 Azure AD B2C 中支援使用此通訊協定的大部分識別提供者。 本文說明如何將自訂 OpenID Connect 識別提供者新增至使用者流程中。
...
> [!NOTE]
> 用戶端祕密為選擇性項目。 但是，如果您想要使用 [授權碼流程](https://openid.net/specs/openid-connect-core-1_0.html#CodeFlowAuth)（使用密碼來交換權杖的程式碼），則必須輸入用戶端密碼。
