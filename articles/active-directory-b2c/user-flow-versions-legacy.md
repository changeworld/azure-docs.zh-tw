...
ms.locfileid: "87505779"
---
# <a name="legacy-user-flow-versions-in-azure-active-directory-b2c"></a>Azure Active Directory B2C 中的舊版使用者流程版本

> [!IMPORTANT]
> 本文說明使用者流程的舊版版本控制方法，其提供 V1 （生產環境就緒）版本，以及 v1.1 和 V2 （預覽）版本的使用者流程。 Azure 公用雲端以外的環境將繼續使用此舊版版本控制方法。 在 Azure 公用雲端中，此方法會由[新的**建議**和**預覽**版本](user-flow-versions.md)所取代。
> 
Azure Active Directory B2C （Azure AD B2C）中的使用者流程，可協助您設定完整描述客戶身分識別體驗的一般[原則](user-flow-overview.md)。 這些體驗包括註冊、登入、密碼重設或設定檔編輯。

在下表中，除非使用者流程已依照**建議**識別，否則會被視為處於*預覽*狀態。 您應該只針對生產環境應用程式使用建議的使用者流程。
...
| 使用者流程 | 建議 | 說明 |
| --------- | ----------- | ----------- |
| 密碼重設 v1。1 | 否 | 允許使用者在驗證其電子郵件之後選擇新密碼（可用的新頁面配置）。 使用此使用者流程，您可以設定： <ul><li>[多重要素驗證](custom-policy-multi-factor-authentication.md)</li><li>權杖相容性設定</li><li>[密碼複雜性需求](user-flow-password-complexity.md)</li></ul> |
