---
title: 使用 Mailjet 進行自訂電子郵件驗證
titleSuffix: Azure AD B2C
description: 瞭解如何整合 Mailjet，以自訂在使用者註冊以使用您的已啟用 Azure AD B2C 的應用程式時，傳送給客戶的驗證電子郵件。
...
如果您還沒有帳戶，請從設定 Mailjet 帳戶開始， (Azure 客戶可以將6000電子郵件解除鎖定，限制為每日) 200 個電子郵件。 

1. 遵循 [建立 Mailjet 帳戶](https://www.mailjet.com/guides/azure-mailjet-developer-resource-user-guide/enabling-mailjet/)的安裝指示。
...
2. 流覽至 [ [API 金鑰管理] 頁面](https://app.mailjet.com/account/api_keys)。 記錄 **API 金鑰** 和 **秘密金鑰** ，以便在稍後的步驟中使用。 當您的帳戶建立時，這兩個金鑰都會自動產生。  

## <a name="create-azure-ad-b2c-policy-key"></a>建立 Azure AD B2C 原則金鑰

接下來，將 Mailjet API 金鑰儲存在您要參考之原則的 Azure AD B2C 原則金鑰中。
...
1. 在 [ **密碼**] 中，輸入您先前記錄的 Mailjet **API 金鑰** 。
1. 針對 [金鑰使用方法] 選取 [簽章]。
1. 選取 [建立]。
1. 選取 [原則金鑰]，然後選取 [新增]。
1. 在 [ **選項**] 中選擇 [ **手動**]。
1. 輸入原則金鑰的 [名稱]。 例如： `MailjetSecretKey` 。 金鑰名稱前面會自動新增前置詞 `B2C_1A_`。
1. 在 [ **密碼**] 中，輸入您先前記錄的 Mailjet **秘密金鑰** 。
1. 針對 [金鑰使用方法] 選取 [簽章]。
1. 選取 [建立]。
...
* `Messages.0.TemplateID`使用您稍早在[建立 Mailjet 範本](#create-a-mailjet-template)中所建立之 Mailjet 交易範本的識別碼，更新 >inputparameter 值。
* 更新 `Messages.0.From.Email` 位址值。 請使用有效的電子郵件地址，以協助防止驗證電子郵件被標示為垃圾郵件。
* `Messages.0.Subject`使用適用于您組織的主旨行，更新主旨行輸入參數的值。
...
在內的宣告轉換下方 `<BuildingBlocks>` ，新增下列 [ContentDefinition](contentdefinitions.md) 以參考2.1.0 資料 URI 版本：
...
1. 將 `email` 設為具有此顯示控制項參考的自我判斷技術設定檔。
...
此 REST API 技術設定檔會使用) 的 Mailjet 格式產生電子郵件內容 (。 如需 RESTful 技術設定檔的詳細資訊，請參閱 [定義 RESTful 技術設定檔](restful-technical-profile.md)。
...
若要將電子郵件當地語系化，您必須將當地語系化的字串傳送至 Mailjet 或您的電子郵件提供者。 例如，您可以當地語系化電子郵件主旨、主體、程式碼訊息或電子郵件的簽章。 若要這樣做，您可以使用 [GetLocalizedStringsTransformation](string-transformations.md) 宣告轉換，將當地語系化的字串複製到宣告類型。 `GenerateEmailRequestBody`宣告轉換會產生 JSON 承載，並使用包含當地語系化字串的輸入宣告。
...
1. 更新您的 Mailjet 範本，以使用動態參數取代將由 Azure AD B2C 當地語系化的所有字串。
...
## <a name="optional-localize-the-ui"></a>參數將 UI 當地語系化

Localization 元素可讓您在使用者旅程圖的適用原則中支援多個地區設定或語言。 原則中的當地語系化支援可讓您針對 [驗證顯示控制項使用者介面](localization-string-ids.md#verification-display-control-user-interface-elements)專案提供特定語言的字串，以及 [一次密碼錯誤訊息](localization-string-ids.md#one-time-password-error-messages)。 將下列 >localizedstring 新增至您的 LocalizedResources。 
...
新增當地語系化的字串之後，請從 LocalAccountSignUpWithLogonEmail 和 LocalAccountDiscoveryUsingEmailAddress 技術設定檔中移除 OTP 驗證錯誤訊息中繼資料。
