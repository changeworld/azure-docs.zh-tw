...
為協助您為應用程式設定最常見的身分識別工作，Azure AD B2C 入口網站包含預先定義、可設定的原則，稱為 **使用者流程**。 使用者流程可讓您決定當使用者進行登入、註冊、編輯設定檔或重設密碼之類的動作時，使用者如何與您的應用程式互動。 透過使用者流程，您可以控制下列功能：
...
Azure AD B2C 包含數種類型的使用者流程：

- **註冊和登入** - 透過單一組態來處理註冊與登入體驗。 系統會視內容而定，將使用者引導到正確的路徑。 也包含個別的 **註冊** 或登 **入** 使用者流程。 但我們通常會建議結合的註冊和登入使用者流程。
- **設定檔編輯** - 可讓使用者編輯其設定檔資訊。
...
> [!IMPORTANT]
> 如果您之前曾在 Azure AD B2C 中使用過使用者流程，您會發現我們已變更參考使用者流程版本的方式。 以前，我們提供了 V1 (可用於生產環境) 版本，以及 V1.1 和 V2 (預覽) 版本。 現在，我們已將使用者流程合併為兩個版本：
>
