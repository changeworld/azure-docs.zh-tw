...
| `AADB2C90006` | 在要求中提供{0}的重新導向 URI ' ' 未針對用戶端識別碼 '{1}' 註冊。 |
| `AADB2C90007` | 與用戶端識別碼 '{0}' 相關聯的應用程式沒有已註冊的重新導向 uri。 |
| `AADB2C90008` | 要求未包含用戶端識別碼參數。 |
| `AADB2C90010` | 要求未包含範圍參數。 |
| `AADB2C90011` | 要求中提供的{0}用戶端識別碼 ' ' 與原則中註冊的客戶{1}端識別碼 ' ' 不相符。 |
| `AADB2C90012` | 不支援要求{0}中提供的範圍 ' '。 |
| `AADB2C90013` | 不支援要求中提供{0}的要求回應類型 ' '。 |
| `AADB2C90014` | 不支援要求中提供{0}的要求回應模式 ' '。 |
| `AADB2C90016` | 要求的用戶端判斷提示{0}類型 ' ' 不符合預期的類型{1}' '。 |
| `AADB2C90017` | 要求中提供的用戶端判斷提示無效：{0} |
| `AADB2C90018` | 在要求中指定{0}的用戶端識別碼 ' ' 未在租使用者 '{1}' 中註冊。 |
| `AADB2C90019` | 租使用者 '{1}' 中識別碼{0}為 ' ' 的金鑰容器沒有有效的金鑰。 原因： {2}。 |
| `AADB2C90021` | 技術設定檔 '{0}' 不存在於租使用者 '{1}{2}' 的原則 ' ' 中。 |
| `AADB2C90022` | 無法傳回租使用者 '{0}{1}' 中原則 ' ' 的中繼資料。 |
| `AADB2C90023` | 設定檔{0}' ' 未包含必要的中繼資料索引{1}鍵 ' '。 |
| `AADB2C90025` | 租使用者{0}'{1}{2}' 中原則 ' ' 的設定檔 ' ' 未包含必要的密碼編譯{3}金鑰 ' '。 |
...
| `AADB2C90035` | 這項服務暫時無法使用。 請稍候幾分鐘後再試一次。 |
| `AADB2C90036` | 要求未包含 URI，可將使用者重新導向至登出後。 在 post_logout_redirect_uri 參數欄位中指定 URI。 |
| `AADB2C90037` | 處理這個要求時發生錯誤。 請洽詢您嘗試存取之網站的系統管理員。 |
| `AADB2C90039` | 要求包含用戶端判斷提示，但在租使用者 '{0}{1}' 中提供的原則 ' ' 遺漏了 RelyingPartyPolicy 中的 client_secret。 |
| `AADB2C90040` | 使用者旅程圖{0}' ' 未包含傳送宣告步驟。 |
| `AADB2C90043` | 包含在要求中的提示包含不正確值。 必須是 ' none '、' login '、' 同意 ' 或 ' select_account '。 |
| `AADB2C90044` | 宣告解析程式{0}'{1}' 不支援宣告 ' '。 |
...
| `AADB2C90055` | 要求中提供{0}的範圍 ' ' 必須指定資源，例如 'https://example.com/calendar.read'。 |
| `AADB2C90057` | 提供的應用程式未設定為允許 OAuth 隱含流程。 |
| `AADB2C90058` | 提供的應用程式未設定為允許公用用戶端。 |
| `AADB2C90067` | 登出後重新導向 URI '{0}' 的格式無效。 請指定 HTTPs 型 URL，例如 'https://example.com/return'，或原生用戶端使用 IETF NATIVE client URI ' urn： IETF： wg： oauth：2.0： oob '。 |
| `AADB2C90068` | 提供識別碼為 '{0}' 的應用程式對此服務無效。 請使用透過 B2C 入口網站建立的應用程式，然後再試一次。 |
| `AADB2C90075` | {0}步驟 '{1}' 中指定的宣告交換 ' ' 傳回 HTTP 錯誤回應，錯誤碼為{2}' '，原因{3}為 ' '。 |
| `AADB2C90077` | 使用者沒有現有的會話，且要求提示參數的值為 '{0}'。 |
| `AADB2C90079` | 用戶端必須在兌換機密授與時傳送 client_secret。 |
| `AADB2C90080` | 提供的授與已過期。 請重新驗證，然後再試一次。 目前時間： {0}，授與發出時間{1}：，授與滑動視窗到期{2}時間：。 |
...
| `AADB2C90086` | 不支援提供的{0}grant_type []。 |
| `AADB2C90087` | 未針對此版本的通訊協定端點發行提供的授與。 |
| `AADB2C90088` | 未針對此端點發行提供的授與。 實際值： {0}和預期值：{1} |
| `AADB2C90092` | 已為租使用者 '{0}{1}' 停用提供識別碼為 ' ' 的應用程式。 請啟用應用程式，然後再試一次。 |
...
| `AADB2C90117` | 不支援要求{0}中提供的範圍 ' '。 |
| `AADB2C90118` | 使用者忘記其密碼。 |
| `AADB2C90120` | 在要求中指定的{0}最大存留期參數 ' ' 無效。 最大存留期必須是介於 '{1}' 到 '{2}' （含）之間的整數。 |
| `AADB2C90122` | 在要求中{0}收到的 ' ' 輸入，HTTP 要求驗證失敗。 請確定輸入不包含 < 或 & 之類的字元。 |
| `AADB2C90128` | 與此授與相關聯的帳戶已不存在。 請重新驗證，然後再試一次。 |
| `AADB2C90129` | 已撤銷提供的授與。 請重新驗證，然後再試一次。 |
| `AADB2C90145` | 找不到未驗證的電話號碼，且原則不允許使用者輸入的號碼。 |
| `AADB2C90146` | 要求中提供{0}的範圍 ' ' 指定了一個以上的存取權杖資源，這是不支援的。 |
| `AADB2C90149` | 無法載入{0}腳本 ' '。 |
...
| `AADB2C90165` | 在狀態中找不到識別碼{0}為 ' ' 的 SAML 起始訊息。 |
| `AADB2C90168` | HTTP 重新導向要求未包含已簽署要求所需的{0}參數 ' '。 |
...
| `AADB2C90194` | 為持有{0}人權杖所指定的宣告 ' ' 不存在於可用的宣告中。 可用的宣告{1}' '。 |
...
| `AADB2C90261` | 步驟 '{1}' 中{0}指定的宣告交換 ' ' 傳回了無法剖析的 HTTP 錯誤回應。 |
...
| `AADB2C90276` | 要求與原則{0}'{1}{2}{3}' 租使用者 '{4}' 的 technicalProfile ' ' 中的控制項設定 ' '： ' ' 不一致。 |
| `AADB2C90277` | 原則 '{2}' 的{0}使用者旅程圖 '{1}' 的協調流程步驟 ' ' 未包含內容定義參考。 |
| `AADB2C90279` | 提供的用戶端識別碼{0}' ' 不符合發出授與的用戶端識別碼。 |
| `AADB2C90284` | 識別碼為 '{0}' 的應用程式未獲得同意，而且無法用於本機帳戶。 |
| `AADB2C90285` | 找不到識別碼為{0}' ' 的應用程式。 |
| `AADB2C90288` | {0}TechnicalProfile '{1}' 中參考的識別碼為 ' ' 的 UserJourney，對租使用者 '{2}' 的重新整理權杖兌換不{3}存在於原則 ' ' 或其任何基底原則中。 |
| `AADB2C90289` | 連接到識別提供者時發生錯誤。 請稍後再試一次。 |
| `AADB2C90296` | 應用程式未正確設定。 請洽詢您嘗試存取之網站的系統管理員。 |
| `AADB2C99005` | 要求包含不正確範圍參數，其中包含不合法的字元{0}' '。 |
| `AADB2C99006` | Azure AD B2C 找不到應用程式識別碼為 '{0}' 的擴充功能應用程式。 如需https://go.microsoft.com/fwlink/?linkid=851224詳細資訊，請造訪。 |
| `AADB2C99011` | 未在原則 '{0}{1}{2}' 的 TechnicalProfile ' ' 中指定中繼資料值 ' '。 |
| `AADB2C99013` | 不支援提供的{0}grant_type [] 和{1}token_type [] 組合。 |
| `AADB2C99015` | 租使用者{0}'{2}' 中原則{1}' ' 的設定檔 ' ' 遺失資源擁有者密碼認證流程所需的所有 InputClaims。 |
