...
| `AADB2C90007` | 與用戶端識別碼 ' ' 相關聯的應用程式沒有已註冊的重新 {0} 導向 uri。 |
| `AADB2C90008` | 要求未包含用戶端識別碼參數。 |
| `AADB2C90010` | 要求未包含範圍參數。 |
| `AADB2C90011` | 要求中提供的用戶端識別碼 ' {0} ' 與 {1} 原則中註冊的用戶端識別碼 ' ' 不相符。 |
| `AADB2C90012` | {0}不支援要求中提供的範圍 ' '。 |
| `AADB2C90013` | {0}不支援要求中提供的要求回應類型 ' '。 |
| `AADB2C90014` | {0}不支援要求中提供的要求回應模式 ' '。 |
...
| `AADB2C90021` | 技術設定檔 ' ' 不 {0} 存在於租使用者 ' ' 的原則 ' {1} ' 中 {2} 。 |
| `AADB2C90022` | 無法傳回 {0} 租使用者 ' ' 中原則 ' ' 的中繼資料 {1} 。 |
| `AADB2C90023` | 設定檔 ' {0} ' 未包含必要的中繼資料索引鍵 ' {1} '。 |
| `AADB2C90025` | 租使用者 {0} ' ' 中原則 ' ' 的設定檔 ' ' {1} {2} 未包含必要的密碼編譯金鑰 ' {3} '。 |
| `AADB2C90027` | 為 ' ' 指定的基本認證 {0} 無效。 請檢查認證是否正確，以及資源是否已授與存取權。 |
| `AADB2C90028` | 為 ' ' 指定的用戶端憑證 {0} 無效。 檢查憑證是否正確、包含私密金鑰，以及資源是否已授與存取權。 |
...
| `AADB2C90039` | 要求包含用戶端判斷提示，但在租使用者 ' ' 中提供的原則 ' {0} ' 遺漏了 {1} RelyingPartyPolicy 中的 client_secret。 |
...
| `AADB2C90068` | 提供識別碼為 ' ' 的應用程式 {0} 對此服務無效。 請使用透過 B2C 入口網站建立的應用程式，然後再試一次。 |
| `AADB2C90075` | {0}步驟 ' ' 中指定的宣告交換 ' ' {1} 傳回 HTTP 錯誤回應，錯誤碼為 ' {2} '，原因為 ' {3} '。 |
...
| `AADB2C90092` | 已為 {0} 租使用者 ' ' 停用提供識別碼為 ' ' 的應用程式 {1} 。 請啟用應用程式，然後再試一次。 |
| `AADB2C90107` | 識別碼為 ' ' 的應用程式 {0} 無法取得識別碼權杖，可能是因為要求中未提供 openid 範圍，或應用程式未獲授權。 |
...
| `AADB2C90117` | {0}不支援要求中提供的範圍 ' '。 |
| `AADB2C90118` | 使用者忘記其密碼。 |
| `AADB2C90120` | 在要求中指定的最大存留期參數 ' {0} ' 無效。 最大存留期必須是介於 ' {1} ' 到 ' {2} ' （含）之間的整數。 |
| `AADB2C90122` | {0}在要求中收到的 ' ' 輸入，HTTP 要求驗證失敗。 請確定輸入不包含 < 或 & 之類的字元。 |
| `AADB2C90128` | 與此授與相關聯的帳戶已不存在。 請重新驗證，然後再試一次。 |
| `AADB2C90129` | 已撤銷提供的授與。 請重新驗證，然後再試一次。 |
| `AADB2C90145` | 找不到未驗證的電話號碼，且原則不允許使用者輸入的號碼。 |
| `AADB2C90146` | {0}要求中提供的範圍 ' ' 指定了一個以上的存取權杖資源，這是不支援的。 |
...
| `AADB2C90168` | HTTP 重新導向要求未包含已簽署要求所需的參數 ' {0} '。 |
...
| `AADB2C90188` | SAML 技術設定檔 ' {0} ' 指定了 ' ' 的 PARTNERENTITY URL {1} ，但提取中繼資料失敗，原因為 ' {2} '。 |
| `AADB2C90194` | 為 {0} 持有人權杖所指定的宣告 ' ' 不存在於可用的宣告中。 可用的宣告 ' {1} '。 |
...
| `AADB2C90220` | 租使用者 ' {0} ' 中具有儲存體識別碼 ' ' 的金鑰容器 {1} 存在，但不包含有效的憑證。 憑證可能已過期，或您的憑證未來可能會變成作用中（nbf）。 |
...
| `AADB2C90255` | 技術設定檔 ' ' 中指定的宣告交換未 {0} 如預期般完成。 您可能想要從頭開始嘗試啟動您的會話。 |
| `AADB2C90261` | {0}步驟 ' ' 中指定的宣告交換 ' ' {1} 傳回了無法剖析的 HTTP 錯誤回應。 |
...
| `AADB2C90276` | 要求與 {0} {1} {2} 原則 ' {3} ' 租使用者 ' ' 的 technicalProfile ' ' 中的控制項設定 ' '： ' ' 不一致 {4} 。 |
| `AADB2C90277` | 原則 ' ' 的使用者旅程圖 ' ' 的協調流程步驟 ' ' {0} {1} {2} 未包含內容定義參考。 |
| `AADB2C90279` | 提供的用戶端識別碼 ' {0} ' 不符合發出授與的用戶端識別碼。 |
| `AADB2C90284` | 識別碼為 ' ' 的應用程式 {0} 未獲得同意，而且無法用於本機帳戶。 |
| `AADB2C90285` | 找不到識別碼為 ' ' 的應用程式 {0} 。 |
| `AADB2C90288` | TechnicalProfile ' ' 中參考的識別碼為 ' ' 的 UserJourney，對租使用者 ' ' 的重新整理 {0} {1} 權杖兌換不 {2} 存在於原則 ' {3} ' 或其任何基底原則中。 |
...
| `AADB2C99011` | {0}未在 {1} 原則 ' ' 的 TechnicalProfile ' ' 中指定中繼資料值 ' ' {2} 。 |
| `AADB2C99013` | 不支援提供的 grant_type [ {0} ] 和 token_type [ {1} ] 組合。 |
| `AADB2C99015` | 租使用者 {0} ' ' 中原則 ' ' 的設定檔 ' ' {1} {2} 遺失資源擁有者密碼認證流程所需的所有 InputClaims。 |
