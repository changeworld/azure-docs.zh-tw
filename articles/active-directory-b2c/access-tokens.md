...
範圍提供一種方式來管理受保護資源的許可權。 要求存取權杖時，用戶端應用程式必須在要求的**範圍**參數中指定所需的許可權。 例如，若要為**應用程式識別碼 URI**為 `https://contoso.onmicrosoft.com/api`的 API 指定 `read` 的**範圍值**，則會 `https://contoso.onmicrosoft.com/api/read`範圍。
...
如果 `/authorize` 要求中的**response_type**參數包含 `token`，**範圍**參數必須包含至少一個要授與的 `openid` 和 `offline_access` 以外的資源範圍。 否則，`/authorize` 要求將會失敗。

## <a name="request-a-token"></a>要求權杖

若要要求存取權杖，您需要授權碼。 以下是授權碼的 `/authorize` 端點要求的範例。 不支援使用自訂網域搭配存取權杖。 在要求 URL 中使用您的 tenant-name.onmicrosoft.com 網域。
