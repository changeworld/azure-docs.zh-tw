---
title: 使用 SendGrid 自訂電子郵件驗證
titleSuffix: Azure AD B2C
description: 瞭解如何與 SendGrid 整合，以自訂在您的客戶註冊使用已啟用 Azure AD B2C 的應用程式時，所傳送的驗證電子郵件。
...
ms.locfileid: "85397919"
---
# <a name="custom-email-verification-with-sendgrid"></a>使用 SendGrid 自訂電子郵件驗證

使用 Azure Active Directory B2C （Azure AD B2C）中的自訂電子郵件，將自訂電子郵件傳送給註冊使用您的應用程式的使用者。 藉由使用[DisplayControls](display-controls.md) （目前為預覽狀態）和協力廠商電子郵件提供者 SendGrid，您可以使用自己的電子郵件範本，並*從：* 位址和主旨，以及支援當地語系化和自訂的單次密碼（OTP）設定。

自訂電子郵件驗證需要使用協力廠商電子郵件提供者，例如[SendGrid](https://sendgrid.com)、 [Mailjet](https://Mailjet.com)或[SparkPost](https://sparkpost.com)、自訂 REST API，或任何以 HTTP 為基礎的電子郵件提供者（包括您自己的）。 本文說明如何設定使用 SendGrid 的解決方案。
...
1. 選取 [程式**代碼編輯器**]，然後 [**繼續**]。
1. 在 [HTML 編輯器] 中，貼上下列 HTML 範本或使用您自己的範本。 `{{otp}}`和 `{{email}}` 參數將會以單次密碼值和使用者電子郵件地址動態取代。
...
1. 展開左側的 [**設定**]，針對 [**電子郵件**主旨]，輸入 `{{subject}}` 。
1. 選取 [**儲存範本**]。
...
* `template_id`使用您稍早在[建立 SendGrid 範本](#create-sendgrid-template)中建立的 SendGrid 交易式範本的識別碼來更新 InputParameter 值。
* 更新 `from.email` 位址值。 請使用有效的電子郵件地址，協助防止驗證電子郵件被標示為垃圾郵件。
* `personalizations.0.dynamic_template_data.subject`以適用于您組織的主旨行，更新主旨行輸入參數的值。
...
1. 回到 `email` 具有此顯示控制項參考的自我判斷技術設定檔。
...
若要將電子郵件當地語系化，您必須將當地語系化的字串傳送至 SendGrid 或您的電子郵件提供者。 例如，您可以將電子郵件主旨、主體、程式碼訊息或電子郵件簽章當地語系化。 若要這樣做，您可以使用[GetLocalizedStringsTransformation](string-transformations.md)宣告轉換，將當地語系化的字串複製到宣告類型。 會 `GenerateEmailRequestBody` 產生 JSON 承載的宣告轉換會使用包含當地語系化字串的輸入宣告。
...
1. 更新您的 SendGrid 範本，以使用動態參數來取代所有將以 Azure AD B2C 當地語系化的字串。
