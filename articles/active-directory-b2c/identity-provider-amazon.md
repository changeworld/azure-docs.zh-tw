...
若要在 Azure Active Directory B2C （Azure AD B2C）中使用 Amazon 帳戶做為[識別提供者](authorization-code-flow.md)，您需要在代表它的租使用者中建立應用程式。 如果您還沒有 Amazon 帳戶，您可以在[https://www.amazon.com/](https://www.amazon.com/)註冊。
...
1. 在 [Web 設定] 區段中，選取 [編輯]，然後在 [允許的 JavaScript 原始來源]`https://your-tenant-name.b2clogin.com`**中，輸入**，並在 [允許的傳回 URL]`https://your-tenant-name.b2clogin.com/your-tenant-name.onmicrosoft.com/oauth2/authresp`**中，輸入**。 以您的租用戶名稱取代 `your-tenant-name`。 即使租用戶在 Azure AD B2C 中是使用大寫字母來定義的，您還是需要在輸入租用戶名稱時，全部使用小寫字母。
