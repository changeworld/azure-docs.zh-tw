---
title: 使用自訂原則來設定以 LinkedIn 帳戶登入
titleSuffix: Azure AD B2C
description: 在 Azure Active Directory B2C 中使用自訂原則來設定以 LinkedIn 帳戶進行登入。
...
本文說明如何使用 Azure Active Directory B2C (Azure AD B2C) 中的 [自訂原則](custom-policy-overview.md) ，讓使用者能夠從 LinkedIn 帳戶登入。

## <a name="prerequisites"></a>必要條件

- 完成在 [Azure Active Directory B2C 中開始使用自訂原則](custom-policy-get-started.md)中的步驟。
- LinkedIn 帳戶-如果您還沒有帳戶，請 [建立一個帳戶](https://www.linkedin.com/start/join)。
- LinkedIn 頁面-您需要 [Linkedin 頁面](https://www.linkedin.com/help/linkedin/answer/710/creating-a-linkedin-company-page) ，以與您在下一節建立的 linkedin 應用程式產生關聯。
...
1.  (選擇性) 輸入 **隱私權原則 URL**。 它必須是有效的 URL，但不需要是可連線的端點。
1. 輸入 **公司電子郵件**。
...
1. 記錄 **用戶端識別碼**。
1. 顯示並記錄 **用戶端密碼**。
1. 在 [ **OAuth 2.0 設定**] 下，新增下列重新 **導向 URL**。 以您的租用戶名稱取代 `your-tenant`。 即使在 Azure AD B2C 中以大寫字母定義，也請將 **所有小寫字母** 用於租使用者名稱。
...
2. 將 LinkedIn 存取權杖儲存到宣告中。 [請參閱此處的指示](idp-pass-through-custom.md)。
3. 新增下列宣告提供者，以對 LinkedIn 的 API 提出要求 `/emailAddress` 。 為了授權此要求，您需要 LinkedIn 存取權杖。
...
4. 在您的使用者旅程圖中新增下列協調流程步驟，以便在使用者使用 LinkedIn 登入時觸發 API 宣告提供者。 請務必 `Order` 適當地更新號碼。 在觸發 LinkedIn 技術設定檔的協調流程步驟之後，立即新增此步驟。
