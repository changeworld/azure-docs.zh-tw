...
完成[開始使用自訂原則](custom-policy-get-started.md)一文中的步驟。
...
1. 在 [名稱] 方塊中，輸入應用程式的名稱 (例如 *Contoso.AADB2C.API*)，然後選取 [確定]。
    ...
    ![在 Visual Studio 中建立新的 Visual Studio 專案](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-create-project.png)

1. 在 [新的 ASP.NET Web 應用程式] 視窗中，選取 [Web API] 或 [Azure API 應用程式] 範本。

    ![在 Visual Studio 中選取 Web API 範本](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-select-web-api.png)
...
1. 在 [方案總管] 中，以滑鼠右鍵按一下 **Models** 資料夾，選取 [新增]，然後選取 [類別]。
    ...
    ![在 Visual Studio 中選取 [新增類別] 功能表項目](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-add-model.png)
...
1. 在 [方案總管] 中，以滑鼠右鍵按一下 **Controllers** 資料夾，選取 [新增]，然後選取 [控制器]。
    ...
    ![在 Visual Studio 中新增控制器](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-add-controller-1.png)

1. 在 [新增 Scaffold] 視窗中，選取 [Web API 控制器 - 空白]，然後選取 [新增]。

    ![選取 Visual Studio 中的 Web API 2 控制器-空白](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-add-controller-2.png)

1. 在 [新增控制器]視窗中，將控制器命名為 **IdentityController**，然後選取 [新增]。

    ![在 Visual Studio 中輸入控制器名稱](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-add-controller-3.png)
...
1. 在 [方案總管] 中，以滑鼠右鍵按一下 **Contoso.AADB2C.API** 專案，然後選取 [發佈]。
    ...
    ![使用 Visual Studio 發行至 Microsoft Azure App Service](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-publish-to-azure-1.png)

1. 在 [發佈] 視窗中，選取 [Microsoft Azure App Service]，然後選取 [發佈]。

    ![使用 Visual Studio 建立新的 Microsoft Azure App Service](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-publish-to-azure-2.png)
...
1. 在 [Web 應用程式名稱] 方塊中，輸入唯一的應用程式名稱 (有效字元為 a-z、0-9 及連字號 (-))。 Web 應用程式的 URL 是 http://<app_name>.azurewebsites.NET，其中 *app_name* 是 Web 應用程式的名稱。 您可以接受自動產生的名稱 (這是唯一的)。
    ...
    ![設定 App Service 屬性](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-publish-to-azure-3.png)
...
2. 開啟 **B2C_1A_signup_signin** (此為您上傳的信賴憑證者 (RP) 自訂原則)，然後選取 [立即執行]。
    ...
    ![Azure 入口網站中的 [B2C_1A_signup_signin 自訂原則] 頁面](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-run.png)

3. 在 [名字] 方塊中，輸入 **Test** 來測試流程。
    Azure AD B2C 會在視窗頂端顯示錯誤訊息。

    ![測試註冊登入頁面上的指定名稱輸入驗證](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-test.png)
...
* 在完成[開始使用自訂原則](custom-policy-get-started.md)逐步解說之後，建議您使用自己的自訂原則檔案來建置您的情節。 我們已提供[範例原則檔案](https://github.com/Azure-Samples/active-directory-b2c-custom-policy-starterpack/tree/master/scenarios/aadb2c-ief-rest-api-netfw)，供您參考。
