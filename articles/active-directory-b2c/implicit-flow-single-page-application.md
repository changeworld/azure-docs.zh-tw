...
description: 瞭解如何使用 Azure 活動目錄 B2C 使用 OAuth 2.0 隱式流添加單頁登錄。
...
---
# <a name="single-page-sign-in-using-the-oauth-20-implicit-flow-in-azure-active-directory-b2c"></a>使用 Azure 活動目錄 B2C 中的 OAuth 2.0 隱式流登錄

許多現代應用程式具有一個單頁應用程式前端，主要用 JavaScript 編寫。 通常，應用程式是通過使用像反應、角或 Vue.js 這樣的框架編寫的。 主要在瀏覽器上執行的單一頁面應用程式和其他 JavaScript 應用程式，在驗證時會面臨一些額外的挑戰：

- 這些應用程式的安全特性不同于傳統的基於伺服器的 Web 應用程式。
- 許多授權伺服器與識別提供者不支援跨原始來源資源共用 (CORS) 要求。
- 整頁瀏覽器重定向遠離應用程式可能會侵入使用者體驗。

為了支援這些應用程式，Azure Active Directory B2C (Azure AD B2C) 使用 OAuth 2.0 隱含流程。 如需 OAuth 2.0 授權隱含授權流程的說明，請參閱 [OAuth 2.0 規格的 4.2 節](https://tools.ietf.org/html/rfc6749) 。 在隱含流程中，應用程式會直接從 Azure Active Directory (Azure AD) 授權端點接收權杖，而不需執行任何伺服器對伺服器交換。 所有身份驗證邏輯和會話處理都完全在 JavaScript 用戶端中完成，其中有頁面重定向或彈出框。

Azure AD B2C 會擴充標準的 OAuth 2.0 隱含流程，功能更強大，而不僅止於簡單的驗證與授權。 Azure AD B2C 導入了[原則參數](user-flow-overview.md)。 利用原則參數，您可以使用 OAuth 2.0 來為應用程式新增原則，例如註冊、登入和設定檔管理使用者流程。 在本文中的 HTTP 要求示例中 **，[租戶].onmicrosoft.com**用作示例。 如果有`{tenant}`租戶的名稱，並且還創建了使用者流，請替換為租戶的名稱。

隱含登入流程看起來像下圖。 本文稍後將詳細說明每個步驟。

![顯示 OpenID 連接隱式流的泳道樣式圖](./media/implicit-flow-single-page-application/convergence_scenarios_implicit.png)

## <a name="send-authentication-requests"></a>傳送驗證要求

當 Web 應用程式需要對使用者進行身份驗證並運行使用者流時，它可以將使用者定向到`/authorize`終結點。 使用者根據使用者流執行操作。

在此請求中，用戶端指示需要從`scope`參數中的使用者獲取的許可權和運行的使用者流。 要瞭解請求的工作原理，請嘗試將請求粘貼到瀏覽器中並運行它。 將 `{tenant}` 取代為您的 Azure AD B2C 租用戶名稱。 替換為`90c0fe63-bcf2-44d5-8fb7-b8bbc0b29dc6`以前在租戶中註冊的應用程式的應用 ID。 替換為`{policy}`您在租戶中創建的策略的名稱，例如`b2c_1_sign_in`。
...
| prompt | 否 | 需要的使用者互動類型。 目前唯一支援的值為 `login`。 此參數強制使用者在該請求上輸入其憑據。 單一登入無效。 |

此時會要求使用者完成原則的工作流程。 使用者可能必須輸入其使用者名和密碼、使用社交身份登錄、註冊目錄或任何其他步驟數。 使用者動作取決於使用者流程的定義方式。
...
| id_token | 應用程式所要求的識別碼權杖。 您可以使用識別碼權杖來確認使用者的身分識別，然後開始與使用者的工作階段。 如需識別碼權杖及其內容的詳細資料，請參閱 [Azure AD B2C 權杖參考](tokens-overview.md)。 |
...
| 參數 | 描述 |
| --------- | ----------- |
| error | 用於對發生的錯誤類型進行分類的代碼。 |
| error_description | 可協助您識別驗證錯誤根本原因的特定錯誤訊息。 |
| state | 如果要求中包含 `state` 參數，回應中就應該出現相同的值。 應用程式應該驗證要求和回應中的 `state` 值完全相同。|

## <a name="validate-the-id-token"></a>驗證識別碼權杖

收到識別碼權杖並不足以驗證使用者。 驗證 ID 權杖的簽名，並根據應用的要求驗證權杖中的聲明。 Azure AD B2C 使用 [JSON Web Tokens (JWT)](https://self-issued.info/docs/draft-ietf-oauth-json-web-token.html) 和公開金鑰加密編譯來簽署權杖及驗證其是否有效。
...
若要判斷使用了哪個使用者流程來簽署識別碼權杖 (以及可從何處擷取中繼資料)，您有兩個選項。 首先，使用者流程名稱包含於 `id_token` 的 `acr` 宣告中。 如需如何剖析識別碼權杖中的宣告相關資訊，請參閱 [Azure AD B2C 權杖參考](tokens-overview.md)。 另一個選項是當您發出要求時，在 `state` 參數的值中將使用者流程編碼。 然後將 `state` 參數解碼，以判斷使用了哪個使用者流程。 任一種方法都有效。

當您從 OpenID Connect 中繼資料端點取得中繼資料文件之後，就可以使用 RSA-256 公開金鑰 (位於此端點) 來驗證識別碼權杖的簽章。 此端點可能隨時會列出多個金鑰，每個都由 `kid` 所識別。 `id_token` 的標頭也包含 `kid` 宣告。 它指出使用了這其中哪個金鑰來簽署識別碼權杖。 如需詳細資訊 (包括了解如何[驗證權杖](tokens-overview.md))，請參閱 [Azure AD B2C 權杖參考](tokens-overview.md)。
...
您應該執行的其他幾個驗證將在[OpenID 連接核心規範](https://openid.net/specs/openid-connect-core-1_0.html)中詳細描述。您可能還需要驗證其他聲明，具體取決於您的方案。 一些常見的驗證包括：
...
如果您的 Web 應用程式只需要執行使用者流程，則可以略過接下來的幾節。 以下各節中的資訊僅適用于需要對 Web API 進行身份驗證調用且受 Azure AD B2C 保護的 Web 應用。

現在，您已將使用者登錄到單頁應用，可以獲取用於調用 Azure AD 保護的 Web API 的訪問權杖。 即使您已經使用 `token` 回應類型收到權杖，仍可使用此方法來取得其他資源的權杖，而不需重新導向使用者進行再次登入。

在典型的 Web 應用流中，您將向`/token`終結點發出請求。 但是，終結點不支援 CORS 請求，因此進行 AJAX 調用以獲取刷新權杖不是一個選項。 相反地，您可以在隱藏的 HTML iframe 元素中使用隱含流程，為其他 Web API 取得新權杖。 以下是範例 (內含換行符號以利閱讀)：
...
| response_mode |建議 |指定將產生之權杖送回到應用程式要使用的方法。 對於隱式流，`fragment`請使用 。 可以指定另外兩種模式，`query`並且`form_post`不起作用隱式流。 |
...
| login_hint |必要 |若要重新整理並取得隱藏 iframe 中的權杖，請在此提示中包含使用者的使用者名稱，以區分使用者在特定時間點可能具有的多個工作階段。 您可以使用`preferred_username`聲明從較早的登錄中提取使用者名（接收`profile``preferred_username`聲明需要作用域）。 |
| domain_hint |必要 |可以是 `consumers` 或 `organizations`。  對於在隱藏 iframe 中刷新和獲取權杖，請在`domain_hint`請求中包括該值。  從較早`tid`的登入識別碼 權杖中提取聲明，以確定要使用的值（接收`profile``tid`聲明需要作用域）。 如果 `tid` 宣告值是 `9188040d-6c67-4c5b-b112-36a304b66dad`，請使用 `domain_hint=consumers`。  否則，使用 `domain_hint=organizations`。 |

一旦設定 `prompt=none` 參數，此要求就會立即成功或失敗，並返回您的應用程式。  藉由使用 `response_mode` 參數中指定的方法，以指定的重新導向 URI 將成功的回應傳送至您的應用程式。

### <a name="successful-response"></a>成功回應
使用`response_mode=fragment`成功回應如下所示：
...
### <a name="error-response"></a>錯誤回應
錯誤回應也能傳送到重新導向 URI，以便讓應用程式能夠適當地處理它們。  對於`prompt=none`，預期錯誤類似于以下示例：
...
如果要將使用者登出應用，請將使用者重定向到 Azure AD 以登出。如果不重定向使用者，他們也許能夠重新驗證到你的應用，而無需再次輸入其憑據，因為他們具有有效的單個登錄會話與 Azure AD。
...
| post_logout_redirect_uri | 否 | 成功登出後，應重定向到使用者的 URL。如果未包含，Azure AD B2C 會向使用者顯示一條通用消息。 |
| state | 否 | 如果要求中包含 `state` 參數，回應中就應該出現相同的值。 應用程式應驗證請求和回應`state`中的值是否相同。 |


> [!NOTE]
> 將使用者導向至 `end_session_endpoint`，會利用 Azure AD B2C 清除使用者的部分單一登入狀態。 不過，它不會將使用者登出使用者的社交身分識別提供者工作階段。 如果使用者在後續登錄期間選擇相同的標識提供程式，則使用者將重新進行身份驗證，而無需輸入其憑據。 舉例來說，如果使用者想要登出您的 Azure AD B2C 應用程式，不一定代表他們想要完全登出自己的 Facebook 帳戶。 不過，如果使用本機帳戶，使用者的工作階段將會正確地結束。
>

## <a name="next-steps"></a>後續步驟

### <a name="code-sample-azure-ad-b2c-with-microsoft-authentication-library-for-javascript"></a>代碼示例：Azure AD B2C，帶有用於 JavaScript 的 Microsoft 身份驗證庫

[使用 msal.js 構建的 Azure AD B2C][github-msal-js-example] （GitHub） 的單頁應用程式

GitHub 上的此示例旨在説明您在使用[msal.js][github-msal-js]構建並使用彈出式身份驗證構建的簡單 Web 應用程式中開始使用 Azure AD B2C。
