...
在本快速入門中，您會使用 Resource Manager 範本搭配網路規則和應用程式規則中使用的範例 IP 群組，以部署 Azure 防火牆。

IP 群組是最上層資源，可讓您定義 IP 位址、範圍和子網定義，並將其群組為單一物件。 這適用於管理 Azure 防火牆規則中的 IP 位址。 您可手動輸入 IP 位址，或從檔案進行匯入。
...
本快速入門中使用的範本是來自 [Azure 快速入門範本](https://github.com/Azure/azure-quickstart-templates/blob/master/101-azurefirewall-create-with-ipgroups-and-linux-jumpbox/azuredeploy.json)
