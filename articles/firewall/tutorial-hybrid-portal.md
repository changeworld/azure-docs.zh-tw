...
2. 完成時，選取 [下一步:IP 位址]。
3. 針對 [IPv4 位址空間]，輸入 **10.5.0.0/16** 。
6. 在 [子網路名稱] 下，選取 [預設]。
7. 針對 [名稱]，輸入 **AzureFirewallSubnet** 。 防火牆會在此子網路中，且子網路名稱 **必須** 是 AzureFirewallSubnet。
8. 在 [位址範圍] 中，輸入 **10.5.0.0/26** 。 
9. 選取 [儲存]。
10. 選取 [檢閱 + 建立]。
11. 選取 [建立]。
...
2. 針對 [區域]，選取 [(美國) 美國東部]。
3. 完成時，選取 [下一步:IP 位址]。
4. 針對 [IPv4 位址空間]，輸入 **10.6.0.0/16** 。
6. 在 [子網路名稱] 下，選取 [預設]。
7. 針對 [名稱] 中，輸入 **SN-Workload** 。
8. 在 [位址範圍] 中，輸入 **10.6.0.0/24** 。 
9. 選取 [儲存]。
10. 選取 [檢閱 + 建立]。
11. 選取 [建立]。
...
2. 針對 [區域]，選取 [(美國) 美國東部]。
3. 選取 下一步：IP 位址
4. 針對 [IPv4 位址空間]，輸入 **192.168.0.0/16** 。
5. 在 [子網路名稱] 下，選取 [預設]。
7. 針對 [名稱]，輸入 **SN-Corp** 。
8. 在 [位址範圍]  中，輸入 **192.168.1.0/24** 。 
9. 選取 [儲存]。
10. 選取 [檢閱 + 建立]。
11. 選取 [建立]。
