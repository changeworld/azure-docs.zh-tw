...
您可以在 Azure 防火牆上設定 NAT 規則、網路規則和應用程式規則。 系統會根據規則類型的優先順序來處理規則集合，將較低的數位從100到65000。 規則集合名稱只能有字母、數位、底線、句號或連字號。 它必須以字母或數位開頭，並以字母、數位或底線結尾。 最大名稱長度為80個字元。

最好是一開始先以100遞增（100、200、300等等）來為規則集合的優先順序編號加上空間，讓您有足夠的空間可以視需要新增更多規則集合。
...
|名稱  |通訊協定  |來源類型  |來源  |目的地類型  |目的地址  |目的地連接埠|
|---------|---------|---------|---------|----------|----------|--------|
