...
description: 您可以使用 [DNS 伺服器] 和 [DNS proxy] 設定來設定 Azure 防火牆。
services: firewall
author: vhorne
ms.service: firewall
ms.topic: how-to
...
### <a name="configure-custom-dns-servers-preview"></a>設定自訂 DNS 伺服器（預覽）

1. 在 [Azure 防火牆**設定**] 底下，選取 [ **DNS 設定**]。
2. 在 [ **DNS 伺服器**] 底下，您可以輸入或新增先前已在虛擬網路中指定的現有 DNS 伺服器。
3. 選取 [儲存]。
4. 防火牆現在會將 DNS 流量導向至指定的 DNS 伺服器，以進行名稱解析。
