---
title: 教學課程：使用 Azure 入口網站部署 & 設定 Azure 防火牆
...
在本教學課程中，您會建立包含三個子網路的簡易單一 VNet，以進行簡單的部署。 針對生產環境部署，建議使用[中樞和輪輻模型](https://docs.microsoft.com/azure/architecture/reference-architectures/hybrid-networking/hub-spoke)。 防火牆位於自己的 VNet 中。 工作負載伺服器位於相同區域中的對等互連 VNet，其中包含一個或多個子網路。
...
6. 接受其他預設值，然後選取 **[下一步：磁片]** 。
7. 接受磁片預設值，然後選取 **[下一步：網路]** 。
...
4. 在 [優先順序] 中，鍵入 **200**。
5. 在 [動作] 中，選取 [允許]。
6. 在 [**規則**]、[ **IP 位址**] 下，針對 [**名稱**] 輸入**Allow-DNS**。
7. 在 [通訊協定] 中，選取 [UDP]。
9. 針對 [**來源類型**]，選取 [ **IP 位址**]。
