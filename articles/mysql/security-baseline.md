...
或者，您可以使用虛擬網路服務終結點來保護和限制對 Azure 資料庫的網路訪問，以便實現 MySQL 實現。 虛擬網路規則是一項防火牆安全性功能，可控制適用於 MySQL 的 Azure 資料庫伺服器是否接受從虛擬網路中特定子網路所傳來的通訊。
...
如何為 MySQL 配置 Azure 資料庫的專用連結：https://docs.microsoft.com/azure/mysql/howto-configure-privatelink-portal

如何在 MySQL 的 Azure 資料庫中創建和管理 VNet 服務終結點和 VNet 規則：https://docs.microsoft.com/azure/sql-database/sql-database-vnet-service-endpoint-rule-overview
...
**指南**：當 MySQL 實例的 Azure 資料庫安全到專用終結點時，可以在同一虛擬網路中部署虛擬機器。 您可以使用網路安全性群組 （NSG） 來降低資料洩露的風險。 啟用 NSG 流日誌並將日誌發送到存儲帳戶以進行流量審核。 您還可以將 NSG 流日誌發送到日誌分析工作區，並使用流量分析提供有關 Azure 雲中的流量流的見解。 流量分析的一些優點是能夠視覺化網路活動並識別熱點、識別安全威脅、瞭解流量模式和查明網路錯誤配置。
...
**指南**：對 MySQL 的 Azure 資料庫使用高級威脅保護。 高級威脅防護可檢測異常活動，指示訪問或利用資料庫的異常和潛在有害嘗試。

在與 MySQL 資料庫關聯的虛擬網路上啟用 DDoS 保護標準，以防止 DDoS 攻擊。 使用 Azure 安全中心集成威脅智慧拒絕與已知惡意或未使用的 Internet IP 位址的通信。

如何為 MySQL 配置 Azure 資料庫的高級威脅保護：https://docs.microsoft.com/azure/mysql/howto-database-threat-protection-portal
...
**指南**：當 MySQL 實例的 Azure 資料庫安全到專用終結點時，可以在同一虛擬網路中部署虛擬機器。 然後，您可以配置網路安全性群組 （NSG）以降低資料洩露的風險。 啟用 NSG 流日誌並將日誌發送到存儲帳戶以進行流量審核。 您還可以將 NSG 流日誌發送到日誌分析工作區，並使用流量分析提供有關 Azure 雲中的流量流的見解。 流量分析的一些優點是能夠視覺化網路活動並識別熱點、識別安全威脅、瞭解流量模式和查明網路錯誤配置。
...
**指南**：對於需要訪問 MySQL 實例的 Azure 資料庫的資源，請使用虛擬網路服務標記在網路安全性群組或 Azure 防火牆上定義網路訪問控制項。 建立安全性規則時，您可以使用服務標籤取代特定的 IP 位址。 通過指定服務標記名稱（例如 SQL。WestUs） 在規則的相應源或目標欄位中，您可以允許或拒絕相應服務的流量。 Microsoft 管理服務標記包含的位址首碼，並在位址更改時自動更新服務標記。
...
**指南**：使用 Azure 策略為 MySQL 實例定義和實施與 Azure 資料庫關聯的網路設置和網路資源的標準安全配置。 在"Microsoft.DBforMySQL"和"Microsoft.Network"命名空間中使用 Azure 策略別名來創建自訂策略，以審核或強制執行 MySQL 實例的 Azure 資料庫的網路設定。 您還可以使用與網路相關的內置策略定義或 MySQL 實例的 Azure 資料庫，例如：
...
**指南**：對 MySQL 實例的 Azure 資料庫的網路安全和流量流相關的資源使用標記來提供中繼資料和邏輯組織。
...
**指南**：Microsoft 維護用於 Azure 資源的時間源，例如日誌中時間戳記的 MySQL 的 Azure 資料庫。
...
**指南**：啟用診斷設置和伺服器日誌以及引入日誌以聚合由 Azure 資料庫為 MySQL 實例生成的安全資料。 在 Azure 監視器中，使用日誌分析工作區來查詢和執行分析，並使用 Azure 存儲帳戶進行長期/存檔存儲。 或者，您可以將資料啟用到 Azure Sentinel 或協力廠商 SIEM。

瞭解 MySQL Azure 資料庫的伺服器日誌：https://docs.microsoft.com/azure/mysql/concepts-monitoring#server-logs
...
**指南**：在 Azure 資料庫上為 MySQL 實例啟用診斷設置，以訪問審核、慢速查詢和 MySQL 指標日誌。 確保專門啟用 MySQL 稽核記錄。 活動日誌自動可用，包括事件源、日期、使用者、時間戳記、源位址、目標位址和其他有用元素。 您還可以啟用 Azure 活動日誌診斷設置，並將日誌發送到相同的日誌分析工作區或存儲帳戶。

瞭解 MySQL Azure 資料庫的伺服器日誌：https://docs.microsoft.com/azure/mysql/concepts-monitoring#server-logs

如何為 MySQL 配置和訪問 Azure 資料庫的慢速查詢日誌：https://docs.microsoft.com/azure/mysql/howto-configure-server-logs-in-portal

如何為 MySQL 配置和訪問 Azure 資料庫的稽核記錄：https://docs.microsoft.com/azure/mysql/howto-configure-audit-logs-portal
...
**指南**：在 Azure 監視器中，對於用於保存 MySQL 日誌的 Azure 資料庫的日誌分析工作區，請根據組織的合規性法規設置保留期。 使用 Azure 存儲帳戶進行長期/存檔存儲。
...
**指南**：分析和監視 Azure 資料庫中的 MySQL 實例的日誌異常行為。 使用 Azure 監視器的日誌分析查看日誌並執行日誌資料的查詢。 或者，您可以將資料啟用並車載到 Azure Sentinel 或協力廠商 SIEM。
...
如何為 MySQL 的 Azure 資料庫啟用高級威脅保護（預覽）：https://docs.microsoft.com/azure/mysql/howto-database-threat-protection-portal
...
**指南**：維護對 MySQL實例 Azure 資料庫的管理平面（例如 Azure 門戶）具有管理存取權限的使用者帳戶的清單。 此外，維護有權訪問 MySQL 實例 Azure 資料庫的資料平面（在資料庫本身內）的管理帳戶的清單。 （創建 MySQL 伺服器時，為管理員使用者提供憑據。 此管理員可用於創建其他 MySQL 使用者。

MySQL 的 Azure 資料庫不支援基於內置角色的存取控制，但可以基於特定資來源提供者選項創建自訂角色。

瞭解 Azure 訂閱的自訂角色：https://docs.microsoft.com/azure/role-based-access-control/custom-roles 

瞭解 MySQL 資來源提供者操作的 Azure 資料庫：https://docs.microsoft.com/azure/role-based-access-control/resource-provider-operations#microsoftdbformysql

瞭解 MySQL Azure 資料庫的訪問管理：https://docs.microsoft.com/azure/mysql/concepts-security#access-management
...
為 MySQL 資源本身創建 Azure 資料庫後，Azure 將強制創建具有強式密碼的管理使用者。 但是，創建 MySQL 實例後，您可以使用您創建的第一個伺服器管理員帳戶來創建其他使用者並授予對它們的管理存取權限。 創建這些帳戶時，請確保為每個帳戶配置不同的強式密碼。

如何為 MySQL 創建 Azure 資料庫的其他帳戶：https://docs.microsoft.com/azure/mysql/howto-create-users
...
**指南**：圍繞使用有權訪問 MySQL 實例的 Azure 資料庫的專用管理帳戶創建標準操作過程。 使用 Azure 安全中心標識和訪問管理監視管理帳戶數。

瞭解 Azure 安全中心標識和訪問：https://docs.microsoft.com/azure/security-center/security-center-identity-access

瞭解如何在 Azure 資料庫中為 MySQL 創建管理員使用者：https://docs.microsoft.com/azure/mysql/howto-create-users
...
**指南**：支援使用直接在資料庫中配置的使用者名/密碼，以及使用 Azure 活動目錄 （AD） 標識並使用 Azure AD 權杖進行連接，都支援登錄到 MySQL 的 Azure 資料庫。 使用 Azure AD 權杖時，支援不同方法，例如 Azure AD 使用者、Azure AD 組或連接到資料庫的 Azure AD 應用程式。

另外，MySQL 的控制平面訪問可通過 REST API 獲得，並支援 SSO。 要進行身份驗證，請將請求的授權標頭設置為從 Azure 活動目錄獲取的 JSON Web 權杖。
...
對於 MySQL 登錄到 Azure 資料庫，建議使用 Azure AD 並使用 Azure AD 權杖進行連接。 使用 Azure AD 權杖時，支援不同方法，例如 Azure AD 使用者、Azure AD 組或連接到資料庫的 Azure AD 應用程式。 

Azure AD 憑據還可用於管理平面級別（例如 Azure 門戶）的管理，以控制 MySQL 管理帳戶。
...
**指導**：查看 Azure 活動目錄日誌，以説明發現陳舊的帳戶，這些帳戶可以包括具有 MySQL 管理角色的 Azure 資料庫的帳戶。 此外，使用 Azure 標識訪問審核可有效地管理組成員身份、訪問可用於訪問 MySQL Azure 資料庫的企業應用程式以及角色指派。 使用者存取權限應定期審查，例如每 90 天一次，以確保只有正確的使用者才能繼續訪問。
...
**指南**：使用標記來説明跟蹤 MySQL 實例或存儲或處理敏感資訊的相關資源的 Azure 資料庫。
...
如何為 MySQL 配置 Azure 資料庫的專用連結：https://docs.microsoft.com/azure/mysql/concepts-data-access-security-private-link
...
**指南**：當使用 Azure VM 訪問 MySQL 實例的 Azure 資料庫時，請使用專用鏈路、MySQL 網路設定、網路安全性群組和服務標記來降低資料外泄的可能性。

Microsoft 管理 MySQL Azure 資料庫的基礎基礎結構，並實施了嚴格的控制，以防止客戶資料丟失或暴露。

如何緩解 MySQL Azure 資料庫的資料滲漏：https://docs.microsoft.com/azure/mysql/concepts-data-access-security-private-link#data-exfiltration-prevention
...
**指南**： MySQL 的 Azure 資料庫支援使用安全通訊端層 （SSL） 將 MySQL 伺服器連接到用戶端應用程式。 在您的資料庫伺服器和用戶端應用程式之間強制使用 SSL 連線，可將兩者之間的資料流加密，有助於抵禦「中間人」攻擊。 在 Azure 門戶中，確保預設情況下為 MySQL 實例的所有 Azure 資料庫啟用"強制 SSL 連線"。

目前，MySQL Azure 資料庫支援的 TLS 版本為 TLS 1.0、TLS 1.1、TLS 1.2。

如何為 MySQL 配置 Azure 資料庫的傳輸加密：https://docs.microsoft.com/azure/mysql/concepts-ssl-connection-security
...
**指南**：MySQL 的 Azure 資料庫尚未提供資料標識、分類和損失預防功能。 如果需要符合性，則實施協力廠商解決方案。
...
**指導**：使用基於 Azure 角色的存取控制 （RBAC） 來控制對 MySQL 控制平面（例如 Azure 門戶）的 Azure 資料庫的訪問。 對於資料平面訪問（在資料庫本身中），使用 SQL 查詢創建使用者並配置使用者許可權。 RBAC 不會影響資料庫中的使用者許可權。

如何在 Azure 中配置 RBAC：https://docs.microsoft.com/azure/role-based-access-control/role-assignments-portal

如何使用適用于 MySQL 的 Azure 資料庫的 SQL 配置使用者訪問：https://docs.microsoft.com/azure/mysql/howto-create-users
...
**指南**： MySQL 服務的 Azure 資料庫使用 FIPS 140-2 驗證加密模組對靜態資料進行存儲加密。 資料（包括備份）在磁片上加密，但執行查詢時創建的暫存檔案除外。 該服務使用包含在 Azure 儲存體加密中的 AES 256 位元加密，且金鑰是由系統進行管理。 儲存體加密會一律啟用，且無法停用。

使用 MySQL Azure 資料庫的客戶管理金鑰進行資料加密使您能夠自帶金鑰 （BYOK） 以進行靜態資料保護。 此時，您必須請求訪問才能使用此功能。 為此，請聯繫：

AskAzureDBforMySQL@service.microsoft.com

瞭解 MySQL Azure 資料庫的靜態加密：https://docs.microsoft.com/azure/mysql/concepts-security

如何為 MySQL 配置 Azure 資料庫的客戶託管金鑰：https://docs.microsoft.com/azure/mysql/concepts-data-encryption-mysql
...
**指南**：將 Azure 監視器與 Azure 活動日誌一起用於創建警報，用於當 MySQL 和其他關鍵或相關資源的 Azure 資料庫的生產實例發生更改時。"
...
**指南**：將標記應用於 MySQL 實例和其他相關資源的 Azure 資料庫，從而提供中繼資料，以邏輯方式將它們組織到分類中。
...
**指南**：使用 Azure 條件訪問通過為"Microsoft Azure 管理"應用配置"阻止訪問"來限制使用者與 Azure 資源管理器交互的能力。 這可以防止在高安全性環境中創建和更改資源，例如包含敏感資訊的 MySQL 的 Azure 資料庫實例。
...
**指南**：使用 Azure 策略為 MySQL 實例定義和實現 Azure 資料庫的標準安全配置。 在"Microsoft.DBforMySQL"命名空間中使用 Azure 策略別名創建自訂策略，以審核或強制執行 MySQL 實例的 Azure 資料庫的網路設定。 您還可以使用與 MySQL 實例的 Azure 資料庫相關的內置策略定義，例如：
...
**指南**：如果對 MySQL 實例和相關資源的 Azure 資料庫使用自訂 Azure 策略定義，請使用 Azure 存儲庫安全地存儲和管理代碼。
...
**指南**：使用"Microsoft.DBforMySQL"命名空間中的 Azure 策略別名創建自訂策略以警報、審核和強制執行系統組態。 此外，開發用於管理原則異常的流程和管道。
...
**指南**：使用"Microsoft.DBforMySQL"命名空間中的 Azure 策略別名創建自訂策略以警報、審核和強制執行系統組態。 使用 Azure 策略 [審核]、[拒絕]和 [部署（如果不存在））自動為 MySQL 實例和相關資源強制實施 Azure 資料庫的配置。
...
**指南**：對於在 Azure 應用服務上運行的 Azure 虛擬機器或 Web 應用程式，用於訪問 MySQL 實例的 Azure 資料庫，請使用 Azure 金鑰保存庫的託管服務標識來簡化和保護 Azure 資料庫以進行 MySQL 金鑰管理。 確保啟用金鑰保存庫虛刪除。
...
**指南**： MySQL 實例的 Azure 資料庫支援 Azure 活動目錄身份驗證（預覽版）以訪問資料庫。  為 MySQL 實例創建 Azure 資料庫時，可以為管理員使用者提供憑據。 此管理員可用於創建其他資料庫使用者。  

對於在 Azure 應用服務上運行的 Azure 虛擬機器或 Web 應用程式，用於訪問 MySQL 實例的 Azure 資料庫，請使用 Azure 金鑰保存庫的託管服務標識來存儲和檢索 MySQL 的 Azure 資料庫憑據實例。 確保啟用金鑰保存庫虛刪除。
...
預掃描上載到非計算 Azure 資源的任何內容，如應用服務、資料湖存儲、Blob 存儲、MySQL 的 Azure 資料庫等。在這種情況下，Microsoft 無法訪問您的資料。
...
**指南**：MySQL 的 Azure 資料庫獲取資料檔案和事務日誌的備份。 根據支援的最大存儲大小，我們要麼進行完整備份和差分備份（最多 4 TB 存儲伺服器）或快照備份（最多 16 TB 的存儲伺服器）。 在您設定的備份保留期限內，這些備份可讓您將伺服器還原至任何時間點。 預設的備份保留期限是七天。 可選擇設定的期限最多為 35 天。 所有備份皆會使用 AES 256 位元加密進行加密。
...
**指南**：MySQL 的 Azure 資料庫根據使用者的選擇自動創建伺服器備份並將其存儲在本地冗余存儲或異地冗余存儲中。 備份可以用來將伺服器還原至某個時間點。 備份和還原可保護資料免於意外損毀或刪除，是商務持續性策略中不可或缺的一部分。 

如果使用 Azure 金鑰保存庫為 MySQL 實例存儲 Azure 資料庫的憑據，請確保定期自動備份金鑰。 
...
**指南**：MySQL 的 Azure 資料庫需要完整、差異和事務記錄備份。 在您設定的備份保留期限內，這些備份可讓您將伺服器還原至任何時間點。 預設的備份保留期限是七天。 可選擇設定的期限最多為 35 天。 所有備份皆會使用 AES 256 位元加密進行加密。 確保啟用金鑰保存庫虛刪除。
