...
description: 瞭解適用於 MySQL 的 Azure 資料庫中的預存程式對於協助您設定資料複寫、設定時區和終止查詢有何説明。
author: ajlam
ms.author: andrela
ms.service: mysql
ms.topic: conceptual
...
ms.locfileid: "91542473"
---
# <a name="azure-database-for-mysql-management-stored-procedures"></a>適用於 MySQL 的 Azure 資料庫管理預存程式

適用於 MySQL 的 Azure 資料庫伺服器上可以使用預存程式來協助管理您的 MySQL 伺服器。 這包括管理伺服器的連線、查詢及設定資料輸入複寫。  
...
若要在適用於 MySQL 的 Azure 資料庫中的來源和複本之間設定資料輸入複寫，請參閱 [如何設定資料輸入複寫](howto-data-in-replication.md)。

## <a name="other-stored-procedures"></a>其他已儲存的程序

下列預存程式可在適用於 MySQL 的 Azure 資料庫中用來管理您的伺服器。
