---
title: 專用連結 - Azure CLI - MySQL 的 Azure 資料庫
description: 瞭解如何從 Azure CLI 為 MySQL 配置 Azure 資料庫的專用連結
author: kummanish
ms.author: manishku
ms.service: mysql
ms.topic: conceptual
...
---
# <a name="create-and-manage-private-link-for-azure-database-for-mysql-using-cli"></a>使用 CLI 為 MySQL 創建和管理 Azure 資料庫的專用連結

私人端點是 Azure 中私人連結的基本要素。 其可讓 Azure 資源 (例如虛擬機器 (VM)) 與私人連結資源進行私密通訊。 在本文中，您將學習如何使用 Azure CLI 在 Azure 虛擬網路中創建 VM，以及使用 Azure 專用終結點為 MySQL 伺服器創建 Azure 資料庫。
...
使用 az mysql 伺服器創建命令為 MySQL 創建 Azure 資料庫。 請記住，MySQL Server 的名稱必須在 Azure 中是唯一的，因此請將括弧中的預留位置值替換為您自己的唯一值： 
...
請注意，MySQL 伺服器 ID ```/subscriptions/subscriptionId/resourceGroups/myResourceGroup/providers/Microsoft.DBforMySQL/servers/servername.```類似于您將在下一步中使用 MySQL 伺服器 ID。 

## <a name="create-the-private-endpoint"></a>建立私人端點 
為虛擬網路中的 MySQL 伺服器創建專用終結點： 
```azurecli-interactive
...
為 MySQL 伺服器域創建專用 DNS 區域，並與虛擬網路創建關聯連結。 
```azurecli-interactive
...
4. 在 **"新建連接**"中，輸入或選擇此資訊：
    ...
    | ------- | ----- |
    | 連線名稱| 選擇您選擇的連接名稱。|
    | 主機名稱 | 選擇*mydemoserver.privatelink.mysql.database.azure.com* |
    | 使用者名稱 | 輸入 MySQL 伺服器創建期間提供的使用者名*username@servername*。 |
