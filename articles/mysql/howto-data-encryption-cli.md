...
description: 瞭解如何使用 Azure CLI 來設定和管理適用於 MySQL 的 Azure 資料庫的資料加密。
author: mksuni
ms.author: sumuth
ms.service: mysql
ms.topic: how-to
...
* 金鑰必須具有下列屬性，才能做為客戶管理的金鑰：
  ...
  * 未停用
  * 執行 **取得**、 **包裝**、 **解除** 包裝作業
  * recoverylevel 屬性設定為 **可** 復原 (這需要啟用的虛刪除（保留期限設為90天）) 
  * 已啟用清除保護
...
1. 有兩種方式可取得適用於 MySQL 的 Azure 資料庫的受控識別。
   ...
   ### <a name="create-an-new-azure-database-for-mysql-server-with-a-managed-identity"></a>使用受控識別建立新的適用於 MySQL 的 Azure 資料庫伺服器。

   ```azurecli-interactive
   az mysql server create --name -g <resource_group> --location <locations> --storage-size size>  -u <user>-p <pwd> --backup-retention <7> --sku-name <sku name> -geo-redundant-backup <Enabled/Disabled>  --assign-identity
   ```

   ### <a name="update-an-existing-the-azure-database-for-mysql-server-to-get-a-managed-identity"></a>更新現有的適用於 MySQL 的 Azure 資料庫伺服器以取得受控識別。
...
1. 使用 Azure Key Vault 中建立的金鑰來啟用適用於 MySQL 的 Azure 資料庫的資料加密。
...
## <a name="additional-capability-for-the-key-being-used-for-the-azure-database-for-mysql"></a>用於適用於 MySQL 的 Azure 資料庫的金鑰的額外功能
...
除了 Azure 入口網站之外，您也可以針對新的和現有的伺服器使用 Azure Resource Manager 範本，在適用於 MySQL 的 Azure 資料庫伺服器上啟用資料加密。
...
此 Azure Resource Manager 範本會建立適用於 MySQL 的 Azure 資料庫伺服器，並使用傳遞做為參數的 **KeyVault** 和 **金鑰** ，在伺服器上啟用資料加密。
