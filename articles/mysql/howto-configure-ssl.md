---
title: 設定 SSL - MySQL 的 Azure 資料庫
...
從下載透過 SSL 與 MySQL 伺服器的[https://www.digicert.com/CACerts/BaltimoreCyberTrustRoot.crt.pem](https://www.digicert.com/CACerts/BaltimoreCyberTrustRoot.crt.pem)Azure 資料庫通訊所需的憑證,並將憑證檔儲存到本地驅動器(例如,本教程使用 c:\ssl)。
...
有關特定的程式設計語言連接字串,請參閱下面的[範例代碼](howto-configure-ssl.md#sample-code)。
...
請參閱 MySQL 服務的 Azure 資料庫支援的[相容驅動程式](concepts-compatibility.md)清單。
...
```
### <a name="php-using-pdo"></a>PHP(使用PDO)
```phppdo
$options = array(
...
```
### <a name="java-mysql-connector-for-java"></a>Java (Java 的MySQL連接器)
```java
...
```
### <a name="java-mariadb-connector-for-java"></a>Java(用於 Java 的 MariaDB 連接器)
```java
# generate truststore and keystore in code
String importCert = " -import "+
    " -alias mysqlServerCACert "+
