...
description: 瞭解如何使用 Azure 門戶為 MySQL 為 Azure 資料庫設置和管理資料加密。
author: kummanish
ms.author: manishku
ms.service: mysql
ms.topic: conceptual
...
2. 選擇 **"金鑰許可權**"，然後選擇 **"獲取**、**換行**、**解包**"和 **"主體**"，這是 MySQL 伺服器的名稱。 如果在現有主體清單中找不到伺服器主體，則需要註冊它。 首次嘗試設置資料加密時，系統會提示您註冊伺服器主體，但該加密失敗。

   ![訪問策略概述](media/concepts-data-access-and-security-data-encryption/access-policy-wrap-unwrap.png)

3. 選取 [儲存]****。

## <a name="set-data-encryption-for-azure-database-for-mysql"></a>為 MySQL 為 Azure 資料庫設置資料加密

1. 在 MySQL 的 Azure 資料庫中，選擇**資料加密**以設置客戶管理的金鑰。
...
2. 您可以選擇金鑰保存庫和金鑰組，也可以輸入金鑰識別碼。
   ...
   ![MySQL Azure 資料庫的螢幕截圖，突出顯示資料加密選項](media/concepts-data-access-and-security-data-encryption/setting-data-encryption.png)
...
使用存儲在金鑰保存庫中的客戶託管金鑰組 MySQL 的 Azure 資料庫進行加密後，還將加密任何新創建的伺服器複本。 您可以通過本地或異地還原操作或通過副本（本地/跨區域）操作製作此新副本。 因此，對於加密的 MySQL 伺服器，可以使用以下步驟創建加密還原的伺服器。
...
1. 在伺服器上，選擇 **"概述** > **還原**"。
   ...
   ![MySQL Azure 資料庫的螢幕截圖，並突出顯示覆制](media/concepts-data-access-and-security-data-encryption/mysql-replica.png)

2. 還原操作完成後，創建的新伺服器將使用主伺服器的金鑰進行加密。 但是，伺服器上的功能和選項被禁用，並且無法訪問伺服器。 這可以防止任何資料操作，因為新伺服器的標識尚未被授予訪問金鑰保存庫的許可權。

   ![MySQL Azure 資料庫的螢幕截圖，突出顯示不可訪問狀態](media/concepts-data-access-and-security-data-encryption/show-restore-data-encryption.png)
...
除了 Azure 門戶之外，您還可以使用新伺服器和現有伺服器的 Azure 資源管理器範本在 MySQL 伺服器上啟用 Azure 資料庫上的資料加密。

### <a name="for-a-new-server"></a>對於新伺服器

使用預先創建的 Azure 資源管理器範本之一為伺服器預配啟用了資料加密：[資料加密示例](https://github.com/Azure/azure-mysql/tree/master/arm-templates/ExampleWithDataEncryption)

此 Azure 資源管理器範本為 MySQL 伺服器創建 Azure 資料庫，並使用作為參數傳遞的**KeyVault**和**金鑰**在伺服器上啟用資料加密。

### <a name="for-an-existing-server"></a>對於現有伺服器
此外，可以使用 Azure 資源管理器範本在 MySQL 伺服器的現有 Azure 資料庫上啟用資料加密。
