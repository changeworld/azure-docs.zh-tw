...
4. 檢查複寫狀態。
   ...
   如果的狀態為「是」， `Slave_IO_Running` `Slave_SQL_Running` 而的值 `Seconds_Behind_Master` 為 "0"，則複寫運作良好。 `Seconds_Behind_Master` 可指定複本的延遲時間。 如果值不是 "0"，表示複本正在處理更新。
