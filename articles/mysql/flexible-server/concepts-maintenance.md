---
title: 排程維護-適用於 MySQL 的 Azure 資料庫彈性的伺服器
description: 本文說明適用於 MySQL 的 Azure 資料庫彈性的伺服器中的排程維護功能。
author: niklarin
ms.author: nlarin
ms.service: mysql
ms.topic: conceptual
...
ms.locfileid: "90934394"
---
# <a name="scheduled-maintenance-in-azure-database-for-mysql--flexible-server"></a>適用於 MySQL 的 Azure 資料庫中的排程維護 – 彈性伺服器

適用於 MySQL 的 Azure 資料庫彈性的伺服器會執行定期維護，讓您的受控資料庫保持安全、穩定且最新。 在維護期間，伺服器會取得新的功能、更新和修補程式。

> [!IMPORTANT]
> 適用於 MySQL 的 Azure 資料庫彈性的伺服器處於預覽階段。

## <a name="select-a-maintenance-window"></a>選取維護時段
...
在罕見的情況下，系統可能會取消維護事件，或可能無法順利完成。 在此情況下，系統會分別建立有關已取消或失敗之維護事件的通知。 下次嘗試執行維護時，將會根據目前的排程設定排程，您將會在五天前收到通知。
