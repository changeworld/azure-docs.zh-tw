---
title: 管理防火牆規則-Azure 入口網站-適用於 MySQL 的 Azure 資料庫-彈性伺服器
description: 使用 Azure 入口網站建立及管理適用於 MySQL 的 Azure 資料庫彈性伺服器的防火牆規則
author: ambhatna
ms.author: ambhatna
ms.service: mysql
ms.topic: how-to
...
> [!IMPORTANT]
> 適用於 MySQL 的 Azure 資料庫彈性的伺服器目前處於公開預覽狀態。
...
在本文中，我們將著重于建立具有 **公用存取 (允許的 IP 位址) ** 使用 Azure 入口網站的 MySQL 伺服器，並在建立具彈性的伺服器之後，將會提供管理防火牆規則的總覽。 使用 *公用存取 (允許的 ip 位址) *，MySQL 伺服器的連線會限制為僅允許的 ip 位址。 防火牆規則中必須允許用戶端 IP 位址。 若要深入瞭解，請參閱 [公用存取 (允許的 IP 位址) ](./concepts-networking.md#public-access-allowed-ip-addresses)。 您可以在建立伺服器時定義防火牆規則 (建議) 但稍後也可以新增。 在本文中，我們將概述如何使用公用存取 (允許的 IP 位址) 來建立和管理防火牆規則。
...
6. 在 [連線 **方法**] 中，選取 [ *公用存取 (允許的 IP 位址]) *。 若要建立 **防火牆規則**，請指定防火牆規則名稱和單一 IP 位址，或位址範圍。 如果您想要將規則限制為單一 IP 位址，請在 [起始 IP 位址] 和 [結束 IP 位址] 欄位中輸入相同的位址。 開啟防火牆可讓系統管理員、使用者和應用程式存取 MySQL 伺服器上具有有效認證的任何資料庫。
   > [!Note]
   > 適用於 MySQL 的 Azure 資料庫彈性的伺服器會在伺服器層級建立防火牆。 它會防止外部應用程式和工具連線到伺服器及伺服器上的任何資料庫，除非您建立規則以針對特定 IP 位址開啟防火牆。
...
1. 在 [ [Azure 入口網站](https://portal.azure.com/)中，選取您要新增防火牆規則適用於 MySQL 的 Azure 資料庫具彈性的伺服器。
...
資源不需要位於相同的虛擬網路中 (VNet) 或資源群組，防火牆規則就可以啟用這些連線。 如果不允許連線嘗試，則要求不會到達適用於 MySQL 的 Azure 資料庫彈性的伺服器。
...
- [使用 Azure CLI 建立及管理適用於 MySQL 的 Azure 資料庫防火牆規則](./how-to-manage-firewall-cli.md)。