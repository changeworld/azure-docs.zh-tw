...
2. 選取 **[一般用途** ] 或 [ **記憶體優化**] （視您要調整的內容而定）。
   ...
   ![在適用於 MySQL 的 Azure 資料庫中選擇基本、一般用途或記憶體優化層 Azure 入口網站的螢幕擷取畫面](./media/howto-create-manage-server-portal/change-pricing-tier.png)
...
2. 將滑桿移至所需的值來變更 [vCore]**** 設定。
    ...
    ![適用於 MySQL 的 Azure 資料庫中選擇 [vCore] 選項 Azure 入口網站的螢幕擷取畫面](./media/howto-create-manage-server-portal/scaling-compute.png)
...
2. 將滑杆移至您想要的值，以變更 **儲存體** 設定。
   ...
   ![在適用於 MySQL 的 Azure 資料庫中選擇儲存體規模 Azure 入口網站的螢幕擷取畫面](./media/howto-create-manage-server-portal/scaling-storage.png)
...
1. 在 Azure 入口網站中選取您的伺服器。 在 [ **總覽** ] 視窗中，選取 [ **重設密碼**]。
   ...
   ![在適用於 MySQL 的 Azure 資料庫中重設密碼 Azure 入口網站的螢幕擷取畫面](./media/howto-create-manage-server-portal/overview-reset-password.png)

2. 輸入新密碼並確認密碼。 文字方塊會提示您輸入密碼複雜性需求。

   ![Azure 入口網站的螢幕擷取畫面，以重設密碼並儲存適用於 MySQL 的 Azure 資料庫](./media/howto-create-manage-server-portal/reset-password.png)
...
1. 在 Azure 入口網站中選取您的伺服器。 在 [ **總覽** ] 視窗中，選取 [ **刪除**]。
   ...
   ![適用於 MySQL 的 Azure 資料庫中刪除伺服器 Azure 入口網站的螢幕擷取畫面](./media/howto-create-manage-server-portal/overview-delete.png)

2. 在 [輸入] 方塊中輸入伺服器的名稱，確認這是您要刪除的伺服器。

   ![在適用於 MySQL 的 Azure 資料庫中確認伺服器刪除 Azure 入口網站的螢幕擷取畫面](./media/howto-create-manage-server-portal/confirm-delete.png)
