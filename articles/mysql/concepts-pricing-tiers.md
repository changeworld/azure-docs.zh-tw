...
description: 瞭解適用於 MySQL 的 Azure 資料庫的各種定價層，包括計算世代、儲存體類型、儲存體大小、虛擬核心、記憶體和備份保留期限。
author: savjani
ms.author: pariks
ms.service: mysql
ms.topic: conceptual
...
適用於 MySQL 的 Azure 資料庫可提供高達 100% 的已佈建伺服器儲存體作為備份儲存體，且不須支付額外費用。 您使用超過此數量的任何備份儲存體都會依每月 GB 計費。 例如，如果您布建的伺服器具有 250 GB 的儲存體，您可以免費使用 250 GB 的額外儲存體來進行伺服器備份。 超過 250 GB 的備份儲存體會依 [定價模型](https://azure.microsoft.com/pricing/details/mysql/)收費。 若要瞭解影響備份儲存體使用量、監視及控制備份儲存體成本的因素，您可以參閱 [備份檔案](concepts-backup.md)。
