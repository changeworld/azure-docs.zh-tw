...
| Azure Synapse 分析 (先前的 Azure SQL DW)  |公開預覽 | |
| Azure 資料總管 | |✓ |
...
| Azure Data Lake Storage Gen2 | ✓ | | ✓ ||
| Azure SQL Database | ✓ | | ✓ | ✓ | ✓ ||
| Azure Synapse 分析 (先前的 Azure SQL DW)  | ✓ | | ✓ | ✓ | ✓ ||
| Azure 資料總管 |||||| ✓ |
...
Azure 資料共用支援從 Azure SQL Database 和 Azure Synapse 分析 (先前的 Azure SQL DW) 共用資料表或觀點。 資料取用者可以選擇將資料接受 Azure Data Lake 存放區 Gen2，或 Azure Blob 儲存體 csv 或 parquet 檔案。 請注意，根據預設，檔案格式為 csv。 資料取用者可以視需要選擇接收 parquet 格式的資料。 這可以在接收資料時，于資料集對應設定中完成。 
...
若要存取共用資料庫，資料取用者必須有自己的 Azure 資料總管叢集。 資料取用者的 Azure 資料總管叢集必須在與資料提供者的 Azure 資料總管叢集相同的 Azure 資料中心內尋找。 建立共用關聯性時，Azure 資料共用會在提供者和取用者的 Azure 資料總管叢集之間建立符號連結。 使用批次模式內嵌至來源 Azure 資料總管叢集的資料會在幾秒內顯示于目標叢集上。
