...
1. 設定**Azure 資源的存取管理**切換回 [**否**]。 由於這是專屬於每個使用者的設定，因此您必須以用來提高存取權的相同使用者身分來等入。
    ...
    如果您嘗試移除 [存取控制（IAM）] 窗格上的 [使用者存取系統管理員] 角色指派，將會看到下列訊息。 若要移除角色指派，您必須將開關設定回 [**否**]，或使用 [Azure PowerShell]、[Azure CLI] 或 [REST API]。
