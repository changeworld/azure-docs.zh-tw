...
1. 在 **'管理**' 的**屬性**。
   ...
   ![選擇 Azure 的目錄屬性屬性 ─ 螢幕擷取](./media/elevate-access-global-admin/azure-active-directory-properties.png)
...
1. 在 [Azure 資源的存取管理]**** 下，將切換開關設為 [是]****。
   ...
   將切換設定為 **「是**」時,將為其在 Azure RBAC 中的根作用域 (/) 中分配使用者存取管理員角色。 這會授與您權限，讓您可以在所有與 Azure AD 目錄相關聯的 Azure 訂用帳戶和管理群組中指派角色。 只有在 Azure AD 中獲派全域管理員角色的使用者可使用此切換開關。
   ...
   當您將切換開關設定為 [否]**** 時，Azure RBAC 中的使用者存取管理員角色會從使用者帳戶中移除。 您將無法在所有與 Azure AD 目錄相關聯的 Azure 訂用帳戶和管理群組中指派角色。 您只能檢視和管理已取得其存取權的 Azure 訂用帳戶和管理群組。
    ...
    > [!NOTE]
    > 如果使用[Azure AD 特權識別管理 (PIM),](../active-directory/privileged-identity-management/pim-configure.md)停用角色分配不會將此切換更改為 **「否**」 。 為了保持最低特權訪問許可權,我們建議您在停用角色分配之前將此切換設置為 **"否**"。
...
1. 登出再重新登入，以重新整理您的存取權。
    ...
    您現在應該有目錄中所有訂用帳戶和管理群組的存取權。 查看存取控制件 (IAM) 窗格時,您會注意到已在根作用域中分配了「使用者存取管理員」角色。
...
要刪除根作用域中的「使用者存取管理員」角色`/`分配 (),請按照以下步驟操作。

1. 與用於提升訪問許可權的同一用戶登錄。

1. 在瀏覽清單中，按一下 [Azure Active Directory]****，然後按一下 [屬性]****。

1. 將**Azure 資源的存取管理**設定回**No**。 由於這是專屬於每個使用者的設定，因此您必須以用來提高存取權的相同使用者身分來等入。

    如果嘗試刪除「訪問控制 (IAM)」窗格上的「訪問管理員」角色分配,您將看到以下消息。 要刪除角色分配,必須將切換設定為 **「否**」或使用 Azure PowerShell、Azure CLI 或 REST API。

    ![移除具有根範圍的角色分配](./media/elevate-access-global-admin/iam-root-remove.png)

## <a name="azure-powershell"></a>Azure PowerShell

[!INCLUDE [az-powershell-update](../../includes/updated-for-az.md)]

### <a name="list-role-assignment-at-root-scope-"></a>在根範圍(/)列出角色分配

要列出根作用域 ()`/`上的使用者的使用者存取管理員角色分配,請使用[Get-AzRole 分配](/powershell/module/az.resources/get-azroleassignment)命令。
...
1. 以可移除所提高存取權的使用者身分登入。 這可以是用於提升訪問許可權的同一使用者,也可以是根作用域中具有提升訪問許可權的另一個全域管理員。
...
要列出根作用域 ()`/`上的使用者的使用者存取管理員角色分配,請使用[az 角色分配清單](/cli/azure/role/assignment#az-role-assignment-list)命令。
...
1. 使用[az 角色分配刪除](/cli/azure/role/assignment#az-role-assignment-delete)命令刪除使用者存取管理員角色分配。
...
1. 使用 REST`elevateAccess`呼叫 ,它授予在根作用`/`域 () 中的使用者存取管理員角色。

   ```http
   POST https://management.azure.com/providers/Microsoft.Authorization/elevateAccess?api-version=2016-07-01
   ```

1. 建立[角色指派](/rest/api/authorization/roleassignments)以在任何範圍的指派任何角色。 下面的範例顯示在根範圍 () 中分配 {角色定義`/`ID} 角色的屬性 ( :
...
1. 在使用者存取管理員時,您還可以刪除根作用域(`/`) 中的角色分配。

1. 移除您的「使用者存取系統管理員」權限，直到再次需要這些權限為止。

### <a name="list-role-assignments-at-root-scope-"></a>在根範圍(/)列出角色分配

您可以在根作用域 ()`/`列出使用者的所有角色分配。
...
### <a name="list-deny-assignments-at-root-scope-"></a>在根範圍(/) 上列出拒絕分配
...
呼叫`elevateAccess`時,您為自己創建角色分配,因此要撤銷這些許可權,您需要在根作用域 ()`/`中刪除自己的使用者存取管理員角色分配。
