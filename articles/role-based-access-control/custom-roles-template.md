...
description: 瞭解如何使用 Azure Resource Manager 範本和 Azure 角色型存取控制來建立 Azure 自訂角色 (Azure RBAC) 。
...
如果 [Azure 內建角色](built-in-roles.md) 不符合您組織的特定需求，您可以建立自己的 [自訂角色](custom-roles.md)。 本文說明如何使用 Azure Resource Manager 範本來建立自訂角色。
...
若要建立自訂角色，請指定角色名稱、許可權，以及可使用角色的位置。 在本文中，您會建立名為「自訂角色-RG 讀取者」的角色，其具有可在訂用帳戶範圍或更低範圍指派的資源許可權。

### <a name="review-the-template"></a>檢閱範本

本文中使用的範本是來自 [Azure 快速入門範本](https://github.com/Azure/azure-quickstart-templates/tree/master/subscription-deployments/create-role-def)。 範本有四個參數和一個 resources 區段。 這四個參數為：

- 預設值為 ["Microsoft .Resources/訂閱/resourceGroups/read"] 的動作陣列
