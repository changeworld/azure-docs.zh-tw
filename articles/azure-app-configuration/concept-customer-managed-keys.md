...
2. `GET` `WRAP` `UNWRAP` 在目標 Key Vault 的存取原則中，授與身分識別、和許可權。
...
1. 啟用 Key Vault 的虛刪除和清除保護。 替代 `contoso-vault` `contoso-resource-group` 在步驟1中建立的 Key Vault （）和資源群組（）的名稱。

    ```azurecli
    az keyvault update --name contoso-vault --resource-group contoso-resource-group --enable-purge-protection --enable-soft-delete
    ```
    
1. 建立 Key Vault 金鑰。 提供此索引 `key-name` 鍵的唯一，並替代 `contoso-vault` 步驟1中所建立的 Key Vault （）名稱。 指定您偏好 `RSA` 或 `RSA-HSM` 加密。
