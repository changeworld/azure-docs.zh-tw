...
授權步驟需要將一或多個 RBAC 角色指派給安全性主體。 Azure 應用程式組態提供 RBAC 角色，其中包含應用程式組態資源的權限集。 指派給安全性主體的角色會決定提供給主體的權限。 如需 RBAC 角色的詳細資訊，請參閱[適用于 Azure 應用程式組態的 Azure 內建角色](#azure-built-in-roles-for-azure-app-configuration)。 
