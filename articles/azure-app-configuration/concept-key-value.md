...
金鑰會作為索引鍵/值的識別碼，並用來儲存和抓取對應的值。 常見的方法是使用字元分隔符號 (例如 `/` 或 `:`)，來將索引鍵組織到階層命名空間。 使用最適合您應用程式的慣例。 應用程式設定會將索引鍵視為一個整體。 其不會剖析索引鍵，以找出索引鍵名稱的結構，或對索引鍵強制執行任何規則。

以下是以元件服務為基礎之階層結構中的索引鍵名稱範例：
...
在應用程式架構內使用設定資料時，可能會指示索引鍵/值的特定命名配置。 例如，JAVA 的春天雲端架構會定義 `Environment` 可提供設定給春季應用程式的資源。  這些是由包含*應用程式名稱*和*設定檔*的變數參數化。 Spring Cloud 相關設定資料的索引鍵通常會以這兩個元素開頭，並以分隔符號隔開。
...
除了之外，您可以在索引鍵名稱中使用任何 unicode 字元 `%` 。 索引鍵名稱不能是 `.` 或 `..` 其中之一。 索引鍵/值上有 10 KB 的結合大小限制。 此限制包括索引鍵中的所有字元、索引鍵的值及所有相關聯的選擇性屬性。 在此限制中，您可以有許多索引鍵階層層級。
...
應用程式設定中的索引鍵/值可選擇性地使用標籤屬性。 標籤會用來區分具有相同索引鍵的索引鍵值。 具有標籤 A** 和 B** 的索引鍵 app1**，會在應用程式組態存放區中形成兩個個別的索引鍵。 根據預設，索引鍵/值沒有標籤。 若要明確參考不含標籤的索引鍵/值，請使用 `\0` 編碼為) 的 (URL `%00` 。
...
使用標籤作為建立多個版本的索引鍵/值的方式。 例如，您可以在標籤內輸入應用程式版本號碼或 Git 認可識別碼，以識別與特定軟體組建相關聯的索引鍵/值。

> [!NOTE]
> 如果您要尋找變更版本，應用程式組態會在過去一段時間內，自動保留索引鍵/值的所有變更。 如需詳細資訊，請參閱[時間點快照](./concept-point-time-snapshot.md)集。

### <a name="query-key-values"></a>查詢索引鍵/值

透過索引鍵和標籤 (可以是 `\0`)，即可識別每個唯一的索引鍵/值。 您可以藉由指定模式來查詢索引鍵/值的應用程式組態存放區。 應用程式組態存放區會傳回符合模式的所有索引鍵/值，包括其對應的值和屬性。 在對應用程式設定發出的 REST API 呼叫中，請使用下列索引鍵模式：
...
> [!NOTE]
> `*`、 `,` 和 `\` 是查詢中的保留字元。 如果您的索引鍵名稱或標籤中使用了保留字元，您必須在查詢中使用來將它換成 `\{Reserved Character}` 。

## <a name="values"></a>值

指派給索引鍵的值也是 Unicode 字串。 您可以使用所有 Unicode 字元來作為值。

### <a name="use-content-type"></a>使用內容類型
應用程式組態中的每個索引鍵/值都有 content-type 屬性。 您可以選擇性地使用這個屬性，將數值型別的相關資訊儲存在索引鍵/值中，協助應用程式正確地處理它。 您可以使用任何格式做為內容類型。 應用程式組態使用[媒體類型]( https://www.iana.org/assignments/media-types/media-types.xhtml) (也稱為 MIME 類型) 用於內建資料類型，例如功能旗標、Key Vault 參考和 JSON 索引鍵/值。
