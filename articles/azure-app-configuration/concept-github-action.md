...
若要開始使用此 GitHub 動作，請移至您的存放庫，然後選取 [Actions] \(動作\) 索引標籤。依序按一下 [New workflow] \(新的工作流程\)、[Set up a workflow yourself] \(自行設定工作流程\)。 最後，在 Marketplace 中搜尋「Azure App Configuration Sync」。
