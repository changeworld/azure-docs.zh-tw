...
## <a name="install-issues"></a>安裝問題
  ...
  > [!NOTE]
  > 如果由於錯誤而重新啟動安裝,請確保在重新觸發安裝之前刪除資源組或從資源組中刪除所有資源。
...
- 使用以下步驟附加部署失敗的紀錄檔:
    ...
    1. 導航到 Azure 門戶中的資源組。
    2. 在左側的「設置」部分下選擇」部署」。
    3. 對於顯示「失敗」的每個部署,按一下以訪問詳細資訊並下載部署詳細資訊。 將此檔附加到郵件。
...
**糾正措施**:檢查您嘗試為其添加角色分配的電子郵件 ID。 電子郵件ID必須與該ID完全匹配,該ID是在活動目錄中為該用戶註冊的。 如果錯誤仍然存在,請通過錯誤消息/日誌與我們聯繫。
...
- 使用以下步驟使用正確的使用者名/密碼更新 FarmBeats 並重試作業。

    *更新哨兵使用者名稱*
    1. 登入[Azure 門戶](https://portal.azure.com)。
    2. 在 **「搜尋」** 框中,搜尋伺服器場節拍 Datahub 資源組。
    3. 選擇儲存帳戶儲存* ->容器 ->批处理准备文件 ->to_vm -> 配置.ini
    4. 按一下 [編輯]
    ...
    *更新哨兵密碼*
    ...
    4. 在「設定」下選擇存取原則
    5. 按下"添加訪問策略"
    6. 使用「機密管理」從範本進行配置,並將自己添加到主體
    7. 按下"添加",然後單擊"訪問策略"頁上的"保存"
    8. 單擊"設置下的秘密"
    9. 點選哨兵密碼
    10. 建立此值的新版本並開啟它
