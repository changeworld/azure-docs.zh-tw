...
ms.locfileid: "86187773"
---
# <a name="get-weather-data-from-weather-partners"></a>從氣象合作夥伴取得天氣資料

Azure FarmBeats 可協助您使用以 docker 為基礎的連接器架構，從您的氣象資料提供者 () 中帶入氣象資料。 使用此架構，氣象資料提供者會執行可與 FarmBeats 整合的 docker。 目前支援下列天氣資料提供者：
...
5. 現在，您的 FarmBeats 實例有作用中的氣象資料合作夥伴，您可以執行作業來要求特定位置的天氣資料， (緯度/經度) 和日期範圍。 JobType (s) 會詳細說明執行天氣作業所需的參數。

   例如，針對 DTN，將會建立下列 JobType (s) ：
...
1. 在您的 FarmBeats 資料中樞 swagger (中 https://yourdatahub.azurewebsites.net/swagger) ，流覽至/WEATHERDATALOCATION API 並提出 GET 要求。 回應將會有/WeatherDataLocation 物件 (s) 為在作業執行過程中指定的位置 (的緯度/經度) 建立。 記下 (s) 之物件的**識別碼**和**weatherDataModelId** 。
...
在上述範例中，回應中有兩個時間戳記的資料，以及量值名稱 ( 「溫度」 ) 和兩個時間戳記中報告的天氣資料值。 您必須參考相關聯的氣象資料模型 (如上述步驟 2) 所述，以解讀所報告值的類型和單位。

## <a name="troubleshoot-job-failures"></a>針對作業失敗進行疑難排解

若要針對作業失敗進行疑難排解，您可以檢查作業記錄。 請遵循[這裡的步驟](troubleshoot-azure-farmbeats.md#weather-data-job-failures)來進行相同的工作。
...
|        Partner   |  詳細資料   |
| ------- | -------             |
|     DockerDetails-imageName         |          Docker 映射名稱。 例如，docker.io/mydockerimage hub.docker.com) 中的 (影像，或 Azure Container Registry) 中的 myazureacr.azurecr.io/mydockerimage (影像等等。 如果未提供登錄，則預設值為 hub.docker.com      |
...
|    DockerDetails - azureBatchVMDetails - dedicatedComputerNodes   |  不正確。 batch 集區的專用電腦節點。 預設值為 1。 |
