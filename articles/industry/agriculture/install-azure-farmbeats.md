...
在您想要安裝 Azure FarmBeats 的 Azure 租使用者中，您將需要下列許可權-
...
4. AAD 腳本需要大約2分鐘的時間來執行，並將值輸出到螢幕以及相同目錄中的 json 檔案。 如果您有其他人執行此腳本，請要求他們與您共用此輸出。
...
5. 提供電子郵件地址，在 [ **FarmBeats 服務警示**] 區段中，應該會收到與 Azure FarmBeats 相關的任何服務警示。 按一下頁面底部的 下一**步]，移至 [相依**性 索引標籤。 ![基本 索引標籤](./media/install-azure-farmbeats/create-azure-farmbeats-basics.png)

6. 將[aad 腳本](#create-an-aad-application)輸出中的個別專案複製到 aad 應用程式區段中的輸入。

7. 在 [Sentinel 帳戶] 區段中，輸入[sentinel 帳戶](#create-sentinel-account)的使用者名稱和密碼。 按 [下一步] 移至 [審核] 和 [**建立** 索引標籤 ![相依性](./media/install-azure-farmbeats/create-azure-farmbeats-dependencies.png)]

8. 一旦輸入的詳細資料通過驗證，請選取 **[確定]** 。 [使用規定] 頁面隨即出現。 檢查條款，然後選取 [**建立**] 以開始安裝。 系統會自動將您重新導向至可以追蹤安裝進度的頁面。
...
輸入 json 檔案的路徑是選擇性的。 如果未指定，腳本會要求您提供所有必要的輸入。 升級應該會在大約30分鐘後完成。
