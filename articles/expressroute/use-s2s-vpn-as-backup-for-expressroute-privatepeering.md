...
透過 ExpressRoute 線路的次要連線的次要 CE 路由器的內部部署路由通告，如下 (Junos 命令) 所示：
...
內部部署路由會由防火牆公告至 VPN 閘道的主要和次要 BGP 對等互連。 路由公告如下 (Junos) ：
...
我們注意到，當指定的內部部署路由透過 ExpressRoute 和 S2S VPN 來公告時，Azure 會偏好使用 ExpressRoute 路徑。 若要強制 Azure 使用 S2S VPN 在並存 ExpressRoute 上的路徑，您需要透過 VPN 連線，以較大的子網) 遮罩 (更多的首碼來公告更特定的路由。 我們的目標是使用 VPN 連線作為「僅備份」。 因此，Azure 的預設路徑選取行為會隨著我們的目標而行。 

我們負責確保從內部部署目的地到 Azure 的流量也偏好透過 S2S VPN 的 ExpressRoute 路徑。 在我們的內部部署設定中，CE 路由器和防火牆的預設本機喜好設定為100。 因此，藉由設定透過 ExpressRoute 私用對等互連（大於100）所接收之路由的本機喜好設定 (假設 150) ，我們可以讓目的地為 Azure 的流量偏好處于穩定狀態的 ExpressRoute 線路。
...
內部部署防火牆的路由表會確認 (如下所示) 對於目的地為 Azure 的內部部署流量，慣用路徑是在穩定狀態的 ExpressRoute 上。
...
如上所示，VPN 閘道具有 VPN 閘道的主要和次要 BGP 對等端所接收的路由。 它也可以看到透過主要和次要 ExpressRoute 連線接收到的路由， (以 12076) 開頭的路徑。 若要確認透過 VPN 連線接收的路由，我們需要知道連線的內部部署 BGP 對等互連 IP。 在我們的安裝程式中，它是192.168.11.88，而我們會看到從它接收的路由。

接下來，讓我們來驗證 Azure VPN 閘道向內部部署防火牆 BGP 對等 (192.168.11.88) 所通告的路由。
...
既然我們已確認透過 VPN 連線的成功路由交換 (控制平面) ，我們會將流量設定為 (資料平面) 從 ExpressRoute 連線切換到 VPN 連線能力。 
...
容錯移轉切換時間取決於 BGP 聚合時間。 在我們的設定中，容錯移轉參數需要幾秒鐘的時間， (不到 10) 。 在切換之後，重複的追蹤路由會顯示下列路徑：
