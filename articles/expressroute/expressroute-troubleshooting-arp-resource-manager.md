...
> [!NOTE]
> 如果第 3 層由服務提供者提供,並且下面的門戶/輸出中的 ARP 表為空,請使用門戶上的刷新按鈕刷新電路配置。 此作業將套用在您線路上的正確路由組態。 
>
>
...
如果本地或連接提供程式出現問題,您可能會看到 ARP 表中只會顯示一個條目,或者本地 MAC 位址將顯示不完整。 並為 Microsoft 端所使用的 MAC 位址與 IP 位址顯示其間的對應。 
