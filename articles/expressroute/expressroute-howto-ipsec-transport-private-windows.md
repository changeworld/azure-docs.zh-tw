...
ms.locfileid: "98017267"
---
# <a name="configure-ipsec-transport-mode-for-expressroute-private-peering"></a>設定 ExpressRoute 私人對等互連的 IPsec 傳輸模式

本文可協助您透過 ExpressRoute 私用對等互連，在傳輸模式中建立 IPsec 通道。 通道是在執行 Windows 的 Azure Vm 與內部部署 Windows 主機之間建立。 本文中針對此設定的步驟會使用群組原則物件。 雖然您可以在不使用組織單位的情況下建立此設定， (Ou) 和群組原則物件 (Gpo) 。 Ou 和 Gpo 的組合將有助於簡化安全性原則的控制，並可讓您快速擴大。 本文中的步驟假設您已經有 Active Directory 的設定，而且您已經熟悉 Ou 和 Gpo 的使用方式。

## <a name="about-this-configuration"></a>關於此組態

下列步驟中的設定會使用單一 Azure 虛擬網路 (VNet) 搭配 ExpressRoute 私人對等互連。 不過，此設定可跨越其他 Azure Vnet 和內部部署網路。 本文將協助您定義 IPsec 加密原則，您可以將其套用至 Azure Vm 或內部部署主機群組。 這些 Azure Vm 或內部部署主機都屬於相同的 OU。 您可以僅針對目的地連接埠為 8080 的 HTTP 流量設定 Azure VM (vm1 和 vm2) 與內部部署 host1 之間的加密。 您可以根據自己的需求建立不同類型的 IPsec 原則。
...
* **篩選清單：** 篩選清單是一或多個篩選條件的組合。 一份清單可包含多個篩選條件。 篩選準則會根據下列準則來定義是否封鎖、允許或保護通訊： IP 位址範圍、通訊協定或甚至特定埠。 每個篩選條件分別會比對一組特定的條件；例如，透過特定目的地連接埠從特定子網路傳送至特定電腦的封包。 當網路條件相符其中一或多個篩選條件時，就會啟動篩選清單。 每個篩選條件都會定義在特定的篩選清單中。 不同的篩選清單之間不可共用篩選條件。 不過，給定的篩選清單可以併入數個 IPsec 原則中。 
...
1. 開啟群組原則管理嵌入式管理單元，建立連結至 OU 的新 GPO。 然後找出要連結 GPO 的 OU。 在此範例中，OU 的名稱為 **IPSecOU**。 

   [![9]][9]
2. 在群組原則管理嵌入式管理單元中選取 OU，然後按一下滑鼠右鍵。 在下拉式清單中，選取 [**在這個網域中建立 GPO 並連結到**]。

   [![10]][10]
3. 為 GPO 指定直覺式名稱，以便後續能輕鬆找出。 選取 **[確定]** 以建立並連結 GPO。
...
1. 在下拉式清單中，以滑鼠右鍵按一下 **Active Directory 上的 [Ip 安全性原則**]，然後選取 [ **管理 ip 篩選器清單和篩選器動作**...]。

   [![15]][15]
2. 在 [**管理篩選器動作**] 索引標籤上，選取 [ **新增**]。

   [![16]][16]

3. 在 [ **IP 安全性篩選器動作]** 中，選取 **[下一步**]。

   [![17]][17]
4. 為篩選動作指定直覺式名稱，以方便後續尋找。 在此範例中，篩選動作的名稱為 **myEncryption**。 您也可以新增描述。 然後，選取 [下一步]  。

   [![18]][18]
5. **交涉安全性** 可讓您定義無法對另一部電腦建立 IPsec 時的行為。 選取 [ **協調安全性**]，然後選取 **[下一步]**。

   [![19]][19]
6. 在 [ **與不支援 IPsec 的電腦通訊** ] 頁面上，選取 [ **不允許不安全的通訊**]，然後選取 **[下一步]**。

   [![20]][20]
7. 在 [ **IP 流量和安全性** ] 頁面上，選取 [ **自訂**]，然後選取 [ **設定 ...**]。

   [![21]][21]
8. 在 [自訂安全性方法設定] 頁面上，選取 [資料完整性和加密 (ESP)：SHA1、3DES]。 然後選取 [確定]  。

   [![22]][22]
9. 在 [管理篩選動作] 頁面上，您會看到 **myEncryption** 篩選條件已成功新增。 選取 [關閉]。
...
1. 若要限定哪些類型的流量必須加密，請使用 **IP 篩選清單**。 在 [ **管理 IP 篩選器清單** ] 索引標籤中 **，選取 [新增]** 以新增 IP 篩選器清單。

   [![24]][24]
2. 在 [名稱] 欄位中，輸入 IP 篩選清單的名稱。 例如 **azure-onpremises-HTTP8080**。 然後，選取 [ **新增**]。

   [![25]][25]
3. 在 [IP 篩選描述和鏡像屬性] 頁面上，選取 [鏡像]。 鏡像設定會比對雙向傳輸、而支援雙向通訊的封包。 然後選取 [下一步]。

   [![26]][26]
4. 在 [IP 流量來源] 頁面上，從 [來源位址：] 下拉式清單中選擇 [特定的 IP 位址或子網路]。 

   [![27]][27]
5. 指定來源位址的 ip **位址或子網：** ip 流量，然後選取 **[下一步]**。

   [![28]][28]
6. 指定 [目的地位址：] 下方的 IP 位址或子網路。 然後，選取 [下一步]  。

   [![29]][29]
7. 在 [IP 通訊協定類型] 上，選取 [TCP]。 然後，選取 [下一步]  。

   [![30]][30]
8. 在 [IP 通訊協定連接埠] 頁面上，選取 [從任何連接埠] 和 [到此連接埠：]。 在文字方塊中輸入 **8080**。 這些設定會指定只有目的地連接埠 8080 的 HTTP 流量會加密。 然後，選取 [下一步]  。
...
若要將相同類型的流量從內部部署主機加密至 Azure VM，您需要第二個 IP 篩選器。 遵循您用來設定第一個 IP 篩選器的相同步驟，並建立新的 IP 篩選器。 兩者只有來源子網路和目的地子網路不相同。

1. 若要將新的 IP 篩選條件新增至 IP 篩選清單，請選取 [編輯]。

   [![33]][33]
2. 在 [ **IP 篩選器清單** ] 頁面上，選取 [ **新增**]。
...
如果內部部署位置和 Azure 子網之間需要加密，以保護應用程式。 您可以新增 IP 篩選器清單，而不是修改現有的 IP 篩選器清單。 將兩個或多個 IP 篩選清單與相同的 IPsec 原則建立關聯，可提供更多的彈性。 您可以修改或移除 IP 篩選清單，而不會影響其他 IP 篩選器清單。
...
1. 選取與 OU 相關聯的 **Active directory 上的 IP 安全性原則**。 按一下滑鼠右鍵，然後選取 [建立 IP 安全性原則]。
   ...
   [![37]][37]
2. 為安全性原則命名。 例如 **policy-azure-onpremises**。 然後，選取 [下一步]  。

   [![38]][38]
3. 選取 [ **下一步]** ，但不選取核取方塊。

   [![39]][39]
4. 確認已選取 [ **編輯屬性** ] 核取方塊，然後選取 **[完成]**。
...
3. 規則會提供用來定義 IPsec 模式的選項：通道模式或傳輸模式。
   ...
   * 傳輸模式只會對承載和 ESP 結尾加密；原始封包的 IP 標頭不會加密。 在傳輸模式中，封包的 IP 來源和 IP 目的地不會變更。
   ...
   選取 **此規則不會指定** 通道，然後選取 **[下一步]**。

   [![43]][43]
4. [網路類型] 會定義要與安全性原則相關聯的網路連線。 選取 [ **所有網路** 連線]，然後選取 **[下一步]**。

   [![44]][44]
5. 選取您先前建立的 IP 篩選器清單（  **azure 內部部署->azure-onpremises-HTTP8080**），然後選取 **[下一步]**。

   [![45]][45]
6. 選取您先前建立的現有篩選動作 **myEncryption**。

   [![46]][46]
7. Windows 支援四種不同類型的驗證：Kerberos、憑證、NTLMv2 和預先共用金鑰。 由於我們正在使用已加入網域的主機，請選取 **Active Directory 預設 (Kerberos V5 通訊協定)**，然後選取 **[下一步]**。

   [![47]][47]
8. 新的原則會建立安全性規則：**azure-onpremises-HTTP8080**。 選取 [確定]。

   [![48]][48]

IPsec 原則會要求目的地連接埠 8080 上的所有 HTTP 連線使用 IPsec 傳輸模式。 由於 HTTP 是純文字通訊協定，因此啟用安全性原則，可確保資料會在透過 ExpressRoute 私人對等互連傳輸時加密。 針對 Active Directory 的 IPsec 原則比 [具有 Advanced Security 的 Windows 防火牆] 設定更複雜。 但是，它允許進一步自訂 IPsec 連線。
...
PowerShell 腳本會以 HTTP 用戶端的形式運作，以在埠8080上產生 HTTP 要求：
...
如果您在內部部署環境中執行 PowerShell 腳本 (HTTP 用戶端) ，Azure VM 中的網路捕捉就會顯示類似的追蹤。
