...
Office 365 服務（如交換連線、SharePoint 線上和 Skype 業務）以及 Azure 服務（如存儲和 SQL DB）可通過 Microsoft 對等互連訪問。 當 Microsoft 對等互連在 ExpressRoute 線路中設定時，與這些服務相關的所有前置詞都會透過建立的 BGP 工作階段進行公告。 BGP 社群值附加至每個前置詞，來識別透過前置詞提供的服務。 如需 BGP 社群值和它們對應之服務的清單，請參閱 [BGP 社群](expressroute-routing.md#bgp)。
...
您可以選取建立新資源的選項來建立路由篩選。 按一下"**創建資源** > **網路** > **路由篩選器**"，如下圖所示：
...
* 有關路由器配置示例的資訊，請參閱[路由器配置示例以設置和管理路由](expressroute-config-samples-routing.md)。 
