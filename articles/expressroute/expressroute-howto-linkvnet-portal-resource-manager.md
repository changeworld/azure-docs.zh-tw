---
title: ExpressRoute：將 VNet 連結至線路： Azure 入口網站
description: 瞭解如何使用 Azure 入口網站來建立連線，以將虛擬網路連結到 Azure ExpressRoute 線路。
...
1. 確認正確設定您的 ExpressRoute 電路和 Azure 私人對等互連。 遵循 [建立 expressroute 線路](expressroute-howto-circuit-arm.md) 中的指示，並 [建立和修改 expressroute 線路的對等互連](expressroute-howto-routing-arm.md)。 ExpressRoute 線路看起來應該像下圖：
   ...
   [![刪除 ExpressRoute 線路螢幕擷取畫面](./media/expressroute-howto-linkvnet-portal-resource-manager/routing1.png "查看電路")](./media/expressroute-howto-linkvnet-portal-resource-manager/routing1-exp.png#lightbox)
2. 您現在可以開始佈建將虛擬網路閘道連結至 ExpressRoute 線路的連線。 按一下 [**連接**  >  **新增**] 以開啟 [**加入連接**] 頁面，然後設定這些值。
...
6. 輸入**授權金鑰**和**對等線路 URI**，並提供連線名稱。 按一下 [確定]。 **對等電路 URI**是 expressroute 線路 (的資源識別碼，您可以在 expressroute 線路) 的 [屬性] 設定窗格中找到此識別碼。
