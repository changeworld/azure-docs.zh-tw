...
description: ExpressRoute 的 Azure 安全性基準
author: msmbaldwin
ms.service: security
ms.topic: conceptual
ms.date: 06/22/2020
...
**指導**方針：不適用;ExpressRoute 可作為重複的線路配對，以確保高可用性。 ExpressRoute 連線不會經過公用網際網路。
...
**指導**方針：不適用;使用閘道子網時，您應該避免將網路安全性群組（NSG）與閘道子網產生關聯。 將網路安全性群組與此子網產生關聯，可能會導致您的虛擬網路閘道（VPN、ExpressRoute 閘道）停止如預期般運作。
...
**指導**方針：不適用;每個客戶的 ExpressRoutes 都會包含在自己的路由網域中，並以通道傳送至其自己的虛擬網路。 雖然 ExpressRoutes 是隔離的，若要額外保護共用相同虛擬網路的其他資源，您可能會啟用 DDoS 保護標準來防範 DDoS 攻擊。

* [瞭解 Azure ExpressRoute 的安全性控制](https://docs.microsoft.com/azure/expressroute/expressroute-security-controls)
...
**指導**方針：不適用;只有 IaaS 計算資源（Azure 虛擬機器）支援套件捕捉。
...
**指導**方針：不適用;每個客戶的 ExpressRoutes 都會包含在自己的路由網域中，並以通道傳送至其自己的虛擬網路。
...
**指導**方針：不適用;Azure ExpressRoute 本身並不是您可以使用服務標籤或網路安全性群組來篩選或允許流量的端點。
...
**指導**方針：使用 Azure 原則定義和執行 Azure ExpressRoute 的標準安全性設定。 使用 "Microsoft. Network" 命名空間中 Azure 原則別名來建立自訂原則，以審核或強制執行 ExpressRoute 的網路設定。
...
**指引**：使用 Azure ExpressRoute 實例的標記來提供中繼資料和邏輯組織。
...
**指引**：使用 Azure 活動記錄來監視網路資源設定，並偵測與您的 ExpressRoute 連線相關的網路資源變更。 在 Azure 監視器中建立警示，以在重大資源的變更發生時觸發。
...
**指導**方針：不適用;Microsoft 會針對記錄檔中的時間戳記，維護用於 Azure 資源的時間來源，例如 Azure Sentinel。
...
**指引**：啟用 Azure 活動記錄診斷設定，並將記錄傳送到 Log Analytics 工作區、Azure 事件中樞或 Azure 儲存體帳戶進行封存。 活動記錄可讓您深入瞭解在控制平面層級的 Azure ExpressRoute 資源上執行的作業。 使用 Azure 活動記錄資料，您可以判斷針對 ExpressRoute 資源在控制平面層級執行的任何寫入作業（PUT、POST、DELETE）的「內容、物件和時間」。
...
**指引**：在 Azure 監視器中，根據貴組織的合規性規定，設定與您的 Azure ExpressRoute 資源相關聯之 log Analytics 工作區的記錄保留週期。
...
**指引**：啟用 Azure 活動記錄診斷設定，並將記錄傳送到 Log Analytics 工作區。 在 Log Analytics 中執行查詢，以搜尋詞彙、識別趨勢、分析模式，並根據可能針對 Azure ExpressRoute 收集的活動記錄資料，提供許多其他深入解析。
...
**指導**方針：您可以設定，以根據與您的 Azure ExpressRoute 資源相關的計量和活動記錄來接收警示。 Azure 監視器可讓您設定警示，以傳送電子郵件通知、呼叫 webhook，或叫用 Azure 邏輯應用程式。
...
**指導**方針：不適用;Azure ExpressRoute 不會處理或產生與反惡意程式碼相關的記錄。
...
**指引**：維護具有 Azure ExpressRoute 資源控制層（例如 Azure 入口網站）之系統管理存取權的使用者帳戶清查。

您可以使用訂用帳戶的 Azure 入口網站中的 [身分識別] 和 [存取控制（IAM）] 窗格來設定角色型存取控制（RBAC）。 這些角色會套用至 Active Directory 中的使用者、群組、服務主體和受控識別。

此外，使用 ExpressRoute 合作夥伴 Resource Manager API 的合作夥伴可以將以角色為基礎的存取控制套用至 expressRouteCrossConnection 資源。 這些控制項可以定義許可權，讓使用者帳戶可以修改 expressRouteCrossConnection 資源，以及新增/更新/刪除對等互連設定。
...
**指導**方針：不適用;當使用者登入 Azure Active Directory （AD）中的自訂應用程式時，單一登入（SSO）可增加安全性和便利性。 Azure ExpressRoute 控制平面（例如 Azure 入口網站）的存取已與 Azure Active Directory 整合，而且可透過 Azure 入口網站以及 Azure Resource Manager REST API 來存取。
...
**指引**：使用 AZURE ACTIVE DIRECTORY （AD）做為 Azure Sentinel 實例的中央驗證和授權系統。 Azure AD 會對待用資料和傳輸中資料使用增強式加密，以保護資料安全。 Azure AD 也會對使用者認證進行 Salt 處理、雜湊處理並安全儲存資料。
...
**指引**：使用 AZURE ACTIVE DIRECTORY （AD）做為 Azure ExpressRoute 資源的中央驗證和授權系統。 Azure AD 會對待用資料和傳輸中資料使用增強式加密，以保護資料安全。 Azure AD 也會對使用者認證進行 Salt 處理、雜湊處理並安全儲存資料。
...
**指引**： Microsoft 會管理 Azure ExpressRoute 線路和相關資源的基礎結構，並已實作為嚴格的控制，以避免遺失或公開客戶資料。
...
**指導**方針： IPSEC 是 IETF 標準。 它會以網際網路通訊協定（IP）層級或網路層3來加密資料。 您可以使用 IPsec 來加密內部部署網路與 Azure 上的虛擬網路（VNET）之間的端對端連線。

* [如何透過 ExpressRoute 設定網站對網站 IPSEC](https://docs.microsoft.com/azure/expressroute/site-to-site-vpn-over-microsoft-peering)
...
Azure ExpressRoute 也具有線路擁有者和線路使用者角色。 線路使用者是虛擬網路閘道的擁有者，與 ExpressRoute 線路位於不同的訂用帳戶內。 電路擁有者能夠隨時修改及撤銷授權。 如果撤銷授權，則在存取權遭撤銷的訂用帳戶中，所有連結連線均會被刪除。 電路使用者可以兌換授權 (每個虛擬網路一個授權)。
...
**指引**：不適用，這項建議主要用於計算資源。 Microsoft 會管理 Azure Sentinel 的基礎結構，並已實行嚴格的控制，以避免遺失或公開客戶資料。
...
**指導**方針： MACSEC 是 IEEE 標準。 它會將媒體存取控制（MAC）層級或網路層2上的資料加密。 當您透過 ExpressRoute Direct 連接到 Microsoft 時，您可以使用 MACsec 來加密您的網路裝置與 Microsoft 網路裝置之間的實體連結。 根據預設，ExpressRoute Direct 埠上的 MACsec 已停用。 您會攜帶自己的 MACsec 金鑰進行加密，並將它儲存在 Azure Key Vault 中。 您決定何時要旋轉金鑰。

* [瞭解 Azure ExpressRoute 中的點對點加密](https://docs.microsoft.com/azure/expressroute/expressroute-about-encryption)

* [如何設定 ExpressRoute Direct 埠上的 MACsec](https://docs.microsoft.com/azure/expressroute/expressroute-howto-macsec)
...
**指導**方針：使用 Azure 監視器搭配 azure 活動記錄，以針對 azure ExpressRoute 的生產實例和其他重要或相關資源進行變更時，建立警示。
...
**指導**方針：當您透過 ExpressRoute Direct 連線到 microsoft 時，您可以使用 MACsec 來加密您的網路裝置與 microsoft 網路裝置之間的實體連結。 根據預設，ExpressRoute Direct 埠上的 MACsec 已停用。 您會攜帶自己的 MACsec 金鑰進行加密，並將它儲存在 Azure Key Vault 中。 您決定何時要旋轉金鑰。
...
**指引**：在支援 azure 服務（例如 azure ExpressRoute）的基礎主機上，已啟用 Microsoft 反惡意程式碼，但不會在客戶內容上執行。
...
**指導**方針：如果使用 Azure Key Vault 來儲存您的 MACsec 秘密，請確保金鑰的定期自動備份。
...
* [如何在設定 MACsec 後測試 Azure ExpressRoute 線路的連線能力](https://docs.microsoft.com/azure/expressroute/expressroute-howto-macsec)
...
**指引**：在 Key Vault 中啟用虛刪除，以防止意外或惡意刪除的金鑰。
