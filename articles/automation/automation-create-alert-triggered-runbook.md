...
|警示  |描述|承載結構描述  |
|---------|---------|---------|
|[常見警報](../azure-monitor/platform/alerts-common-schema.md?toc=%2fazure%2fautomation%2ftoc.json)|當前在 Azure 中標準化警報通知的消耗體驗的通用警報架構。|常見警報有效負載架構|
...
4. 在 **"添加運行簿"** 頁上，為 Runbook 名稱輸入**停止 AzureVm 回應VM警報**。 針對 Runbook 類型，選取 [PowerShell]****。 然後，選取 [Create]**** \(建立\)。  
...
## <a name="create-the-alert"></a>創建警報

警報使用操作組，操作組是由警報觸發的操作的集合。 Runbook 只是可與動作群組搭配使用的眾多動作之一。

1. 在自動化帳戶中，選擇 **"監視**下的**警報**"。
1. 選取 [+ 新增警示規則]****。
1. 按一下 **"在****資源**下選擇"。 在"**選擇資源**"頁上，選擇 VM 以發出警報，然後按一下"**完成**"。
1. 按一下"**在條件**下**添加條件**" 選擇要使用的信號，例如**CPU 百分比**，然後按一下 **"完成**"。
1. 在 **"配置信號邏輯**"頁上，在 **"警報"邏輯**下輸入**閾值**，然後按一下 **"完成**"。
1. 在 **"操作組**"下，選擇 **"新建**"。
...
1. 在 **"警報詳細資訊"** 下，添加警報規則名稱和說明，然後按一下"**創建警報規則**"。
