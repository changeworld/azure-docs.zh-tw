...
description: 如何使用 Azure 自動化狀態設定設定用於管理的電腦
services: automation
...
Azure 自動化狀態配置是任何雲端或本地資料中心中所需狀態配置 (DSC) 節點的配置管理服務。 通過在**配置管理**下選擇**狀態配置 (DSC),** 在 Azure 門戶中訪問它。 

此服務可在中央、安全的位置快速輕鬆地跨數千台計算機進行可擴充性。 您可以輕鬆地對計算機進行板載,為其分配聲明性配置,並查看顯示每台電腦是否符合您指定的所需狀態的報告。
...
- Azure 虛擬機器
- Azure 虛擬機器 (傳統)
- 本地或 Azure 以外的雲端(包括 AWS EC2 實體)的物理/虛擬 Windows 電腦
- 位於內部部署、Azure 或 Azure 以外之雲端中的實體/虛擬 Linux 機器

如果尚未準備好從雲端管理電腦配置,則可以使用 Azure 自動化狀態配置作為僅報告終結點。 此功能允許您透過 DSC 設定(推送)配置,並在 Azure 自動化中查看報告詳細資訊。
...
Azure 自動化狀態設定允許您使用 Azure 門戶、Azure 資源管理器範本或 PowerShell 輕鬆將 Azure VM 上傳進行配置管理。 在引擎蓋下,無需管理員遠端連接到 VM,Azure VM 所需的狀態配置擴展將 VM 註冊為 Azure 自動化狀態配置。 由於 Azure 擴展以非同步方式運行,因此在本文的故障[排除 Azure 虛擬機載入「](#troubleshooting-azure-virtual-machine-onboarding)部分中提供了追蹤其進度或故障排除的步驟。

> [!NOTE]
>將 DSC 部署到 Linux 節點使用 **/tmp**資料夾。 在將模組`nxautomation`(如模組安裝到其適當位置)之前臨時下載以進行驗證。 為確保模組正確安裝,Linux 的日誌分析代理需要 **/tmp**資料夾的讀/寫許可權。<br><br>
>Linux 的日誌分析代理以使用者`omsagent`身份 運行。 要向`omsagent`使用者授予>寫入權限,請`setfacl -m u:omsagent:rwx /tmp`執行指令 。

### <a name="onboard-a-vm-using-azure-portal"></a>使用 Azure 門戶的 VM 板載

透過[Azure 門戶](https://portal.azure.com/)將 Azure VM 與 Azure 自動化狀態配置一起:

1. 導航到 Azure 自動化帳戶,該帳戶將 VM 板載。 

2. 在「狀態設定」頁上,選擇 **「節點」** 選項卡,然後按下「**添加**」。

3. 選擇要上載的 VM。

4. 如果電腦未安裝 PowerShell 所需的狀態擴展,並且電源狀態正在運行,請單擊「**連接**」。。

5. 在 **「註冊」 ,** 輸入用例所需的[PowerShell DSC 本地設定管理員值](/powershell/scripting/dsc/managing-nodes/metaConfig)。 或者,您可以輸入要分配給 VM 的節點配置。

![上架](./media/automation-dsc-onboarding/DSC_Onboarding_6.png)

### <a name="onboard-a-vm-using-azure-resource-manager-templates"></a>使用 Azure 資源管理員樣本在 VM 上

您可以使用 Azure 資源管理器樣本將 VM 部署到 Azure 自動化狀態配置並將其重新上傳。 有關將現有 VM 板出 Azure 自動化狀態設定的範例樣本[,請參考由所需狀態設定服務管理的伺服器](https://azure.microsoft.com/resources/templates/101-automation-configuration/)。 如果要管理 VM 縮放集,請參閱 Azure 自動化[管理的 VM 規模集配置](https://azure.microsoft.com/resources/templates/201-vmss-automation-dsc/)中的範例範本。
...
要尋找在樣本中用參數的註冊金鑰和註冊網址,請參考本文中的[「 使用註冊」 的安全性地載入](#onboarding-securely-using-registration)。

## <a name="onboarding-physicalvirtual-windows-machines-on-premises-or-in-a-cloud-other-than-azure-including-aws-ec2-instances"></a>在本地或 Azure 以外的雲(包括 AWS EC2 實體)中安裝物理/虛擬 Windows 電腦

您可以將在本地或其他雲端環境執行的 Windows 伺服器連接到 Azure 自動化狀態配置。 伺服器必須具有[對 Azure 的出站存取權限](automation-dsc-overview.md#network-planning)。

1. 確保最新版本的[WMF 5](https://aka.ms/wmf5latest)安裝在要裝機到 Azure 自動化狀態配置的電腦上。 此外,WMF 5 必須安裝在您用於載入操作的電腦上。
1. 按照[生成DSC配置的資料中](#generating-dsc-metaconfigurations)包含包含所需DSC配置的資料夾。 
1. 使用以下 cmdlet 將 PowerShell DSC 配置遠端應用於要上載的電腦。 

   ```powershell
   Set-DscLocalConfigurationManager -Path C:\Users\joe\Desktop\DscMetaConfigs -ComputerName MyServer1, MyServer2
   ```

1. 如果無法遠端應用 PowerShell DSC 配置,請將**中設定**資料夾複製到要載入的電腦。 然後添加代碼在電腦上`Set-DscLocalConfigurationManager`本機上啟動。
1. 使用 Azure 門戶或 cmdlet,驗證要上傳的電腦是否顯示為在 Azure 自動化帳戶中註冊的狀態配置節點。

## <a name="onboarding-physicalvirtual-linux-machines-on-premises-or-in-a-cloud-other-than-azure"></a>在本地或 Azure 以外的雲端中安裝實體/虛擬 Linux 電腦
...
2. 如果[PowerShell DSC 本地設定管理員預設值](/powershell/scripting/dsc/managing-nodes/metaConfig4)與您的用例符合,並且您希望在電腦上板,以便它們同時從電腦中提取並報告到 Azure 自動化狀態設定:

   - 在每台 Linux 電腦上,使用 PowerShell DSC`Register.py`本地配置管理器預設值進行板載。

     `/opt/microsoft/dsc/Scripts/Register.py <Automation account registration key> <Automation account registration URL>`

   - 要查找自動化帳戶的註冊密鑰和註冊 URL,請參閱本文的[註冊部分的載入安全](#onboarding-securely-using-registration)版。

3. 如果 PowerShell DSC 本地設定管理員 (LCM) 預設值與您的用例不匹配,或者您希望將僅向 Azure 自動化狀態設定報告的電腦板載,請按照步驟 4-7 執行。 否則,直接執行步驟 7。

4. 按照[生成DSC配置的資料](#generating-dsc-metaconfigurations)包含所需DSC配置的資料夾。

5. 確保最新版本的[WMF 5](https://aka.ms/wmf5latest)安裝在用於載入的電腦上。

6. 按照如下方式添加代碼,將 PowerShell DSC 配置遠端應用於要上載的電腦。
...
7. 如果無法遠端應用 PowerShell DSC 配置,請將與遠端電腦對應的元設定從步驟 4 中描述的資料夾中複製到 Linux 電腦。

8. 將要在每個`Set-DscLocalConfigurationManager.py`Linux 電腦上本機呼叫的代碼添加到 Azure 自動化狀態配置的板載。

   `/opt/microsoft/dsc/Scripts/SetDscLocalConfigurationManager.py -configurationmof <path to metaconfiguration file>`

9. 使用 Azure 門戶或 cmdlet,請確保要上傳的電腦現在顯示為 Azure 自動化帳戶中註冊的 DSC 節點。

## <a name="generating-dsc-metaconfigurations"></a>產生 DSC 中繼設定

要將任何電腦連線到 Azure 自動化狀態設定,可以產生[DSC 中設定](/powershell/scripting/dsc/managing-nodes/metaConfig)。 此配置告訴 DSC 代理從和/或向 Azure 自動化狀態配置提取和/或報告。 您可以使用 PowerShell DSC 配置或 Azure 自動化 PowerShell cmdlet 為 Azure 自動化狀態配置生成 DSC 元配置。

> [!NOTE]
> DSC 中繼設定包含將電腦上架至進行管理之自動化帳戶的機密資料。 請務必適當地保護您所建立的任何 DSC 中繼設定，或在使用後將它們刪除。

對元配置的代理支援由 LCM 控制,LCM 是 Windows PowerShell DSC 引擎。 LCM 在所有目標節點上運行,並負責調用 DSC 元配置腳本中包含的配置資源。 通過在`ConfigurationRepositoryWeb``ResourceRepositoryWeb`中 根據需要包括代理 URL 的定義和代理認證,可以在元配置中包括`ReportServerWeb`代理支援。 請參考[設定本地端設定管理員](https://docs.microsoft.com/powershell/scripting/dsc/managing-nodes/metaconfig?view=powershell-7)。

### <a name="generate-dsc-metaconfigurations-using-a-dsc-configuration"></a>使用 DSC 設定產生 DSC 中設定
...
1. 填寫您自動化帳戶的註冊金鑰和 URL，以及要上架的電腦名稱。 所有其他參數都是選擇性的。 要尋找自動化帳戶的註冊金鑰和註冊網址,請參閱[使用註冊功能安全地載入](#onboarding-securely-using-registration)。

1. 如果希望電腦向 Azure 自動化狀態配置報告 DSC 狀態資訊,但不希望拔取配置`ReportOnly`或 PowerShell 模組,則將參數設置為 true。
...
### <a name="generate-dsc-metaconfigurations-using-azure-automation-cmdlets"></a>使用 Azure 自動化 cmdlet 產生 DSC 中設定
...
計算機可以通過 WMF 5 DSC 註冊協定安全地登上 Azure 自動化帳戶。 此協定允許 DSC 節點對 PowerShell DSC 拉取伺服器或報表伺服器進行身份驗證,包括 Azure 自動化狀態配置。 節點在註冊 URL 上向伺服器註冊並使用註冊密鑰進行身份驗證。 在註冊期間,DSC 節點和 DSC 拉/報告伺服器協商一個唯一的證書,用於節點在註冊後對伺服器進行身份驗證。 此過程可防止板載節點相互類比,例如,如果節點受到威脅並惡意行為。 註冊后,註冊金鑰不再用於身份驗證,並從節點中刪除。
...
- 註冊網址是"金鑰"頁上的網址欄位。
- 註冊鍵是 **「密鑰」頁上的主訪問鍵**欄位或**輔助存取鍵**欄位的值。 可以使用這兩個金鑰。

為了增加安全性,您可以隨時在「密鑰」頁上重新生成自動化帳戶的主訪問密鑰和輔助存取密鑰。 金鑰重新生成可防止將來的節點註冊使用以前的密鑰。

## <a name="re-registering-a-node"></a>重新註冊節點

在 Azure 自動化狀態配置中將電腦註冊為 DSC 節點後,可能需要在將來重新註冊該節點有幾個原因。

- **證書續訂。** 對於 Windows Server 2019 之前的 Windows 伺服器版本,每個節點會自動協商唯一的身份驗證證書,該證書將在一年後過期。 如果憑證過期而不續訂,則節點無法與 Azure 自動化通訊並標`Unresponsive`記為 。 目前,PowerShell DSC 註冊協議無法在證書即將到期時自動續訂證書,您必須在一年後重新註冊節點。 在重新註冊之前,請確保每個節點都運行 WMF 5 RTM。 
...
1. 在 Azure 門戶中,導航到正在上傳的 VM。
2. 按下 **「設定」** 下的 **「擴展**」。 
3. 現在選擇**DSC**或**DSCForLinux,** 這取決於你的作業系統。 
4. 如需詳細資訊，您可以按一下 [檢視詳細狀態] ****。

有關故障排除的詳細資訊,請參閱 Azure[自動化所需狀態設定 (DSC) 的故障排除問題](./troubleshoot/desired-state-configuration.md)。
