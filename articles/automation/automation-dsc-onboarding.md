...
您可以使用 Azure 資源管理器樣本將 VM 部署到 Azure 自動化狀態配置並將其重新上傳。 有關將現有 VM 板出 Azure 自動化狀態設定的範例樣本[,請參考由所需狀態設定服務管理的伺服器](https://azure.microsoft.com/resources/templates/101-automation-configuration/)。 如果要管理虛擬機規模集,請參閱[由 Azure 自動化管理的虛擬機縮放集配置](https://azure.microsoft.com/resources/templates/201-vmss-automation-dsc/)中的範例範本。
...
## <a name="onboarding-physicalvirtual-windows-machines"></a>載入物理/虛擬 Windows 電腦

您可以將在本地或其他雲端環境(包括 AWS EC2 實例)執行的 Windows 伺服器連接到 Azure 自動化狀態配置。 伺服器必須具有[對 Azure 的出站存取權限](automation-dsc-overview.md#network-planning)。
...
## <a name="onboarding-physicalvirtual-linux-machines"></a>載入物理/虛擬 Linux 電腦
