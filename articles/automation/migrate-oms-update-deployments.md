...
Operations Management Suite (OMS) 入口網站已被[取代](../azure-monitor/platform/oms-portal-transition.md)。 更新管理的 OMS 入口網站中提供的所有功能，都可透過 Azure 監視器記錄在 Azure 入口網站中取得。 本文提供遷移至 Azure 入口網站所需的資訊。
...
從您的 OMS 工作區，按一下 [在 Azure 中開啟]****。 此選項會流覽至 OMS 已使用的 Log Analytics 工作區。
...
在 [Azure 入口網站中，選取 [**所有服務**] 底下的 [**自動化帳戶**]。 

在 [**管理工具**] 底下，選取適當的自動化帳戶，然後按一下 [**更新管理**]。
...
針對**要更新的電腦**，選取現有 OMS 部署所使用的已儲存搜尋。

| 屬性 | 描述 |
| --- | --- |
|名稱 |用以識別更新部署的唯一名稱。 |
|作業系統| 選取 [Linux]**** 或 [Windows]****。|
|要更新的機器 |選取已儲存的搜尋、已匯入的群組，或從下拉式清單中選擇 [機器]，然後選取個別的機器。 如果您選擇 [機器]****，機器的整備程度會顯示於 [更新代理程式整備程度]**** 欄中。</br> 若要瞭解在 Azure 監視器記錄中建立電腦群組的不同方法，請參閱[Azure 監視器記錄中的電腦群組](../azure-monitor/platform/computer-groups.md) |
...
若要深入瞭解 Azure 中的更新管理，請參閱[更新管理](automation-update-management.md)。
