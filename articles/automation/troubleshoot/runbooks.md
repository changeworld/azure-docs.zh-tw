...
此錯誤可能是因為在 runbook 中使用不完整的 AzureRM 遷移至 Az 模組所造成。 這可能會導致 Azure 自動化只使用 AzureRM 模組來啟動 runbook 作業，然後只使用 Az 模組啟動另一個作業，導致沙箱損毀。 
...
不完整地將您的 runbook 模組從 AzureRM 遷移至 Az，可能會導致沙箱損毀和 runbook 失敗。 請參閱在[runbook 中使用模組](../automation-runbook-execution.md#using-modules-in-your-runbooks)。
