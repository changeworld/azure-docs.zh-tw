...
在更新管理中，您的機器不會顯示為 [就緒（狀況良好）] 的原因有很多。 在更新管理中，您可以檢查混合式 Runbook 背景工作角色代理程式的健康情況，以判斷根本問題。 本文討論如何在[離線案例](#troubleshoot-offline)中，從 Azure 入口網站和非 azure 機器執行 Azure 機器的疑難排解員。 
...
> [!NOTE]
> VM 必須處於執行中狀態，才能進行檢查。 如果 VM 不在執行中，您會看到 **[啟動 vm** ] 按鈕。
