...
您可用於上線的資源群組數目受限於 [Resource Manager 部署限制](../azure-resource-manager/templates/cross-resource-group-deployment.md)。 資源管理器部署（不要與更新部署混淆）僅限於每個部署 5 個資源組。 為了確保上線的完整性，系統會保留其中 2 個資源群組來設定 Log Analytics 工作區、自動化帳戶和相關資源。 這讓您有 3 個資源群組可選取以供部署。 此限制僅適用于同時載入，不適用於由自動化解決方案管理的資源組數。

您還可以使用 Runbook 進行載入，有關詳細資訊，請參閱[板載更新並將跟蹤解決方案更改為 Azure 自動化](automation-onboard-solutions.md)。
...
**解決方案**：將虛擬機器遷移到資源管理器部署模型。 若要了解如何執行這項操作，請參閱[遷移傳統部署模型資源](../virtual-machines/windows/migration-classic-resource-manager-overview.md)。
...
現在，已為虛擬機器啟用了該解決方案，請訪問更新管理概述一文，瞭解如何為電腦創建**更新部署**。
