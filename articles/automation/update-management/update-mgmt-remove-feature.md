...
description: 本文說明如何停止使用更新管理，並將自動化帳戶從 Log Analytics 工作區取消連結。
services: automation
ms.date: 07/28/2020
ms.topic: conceptual
ms.custom: mvc
...
當您使用 Azure 自動化更新管理在虛擬機器上啟用管理更新之後，您可能會決定停止使用它，並從帳戶和連結的 Log Analytics 工作區中移除設定。  本文會告訴您如何從受管理的 Vm、您的自動化帳戶和 Log Analytics 工作區中完全移除更新管理。
...
移除更新管理之前，您必須先停止管理您的 Vm。 請參閱[從更新管理移除 vm](update-mgmt-remove-vms.md) ，將它們從功能中取消註冊。

## <a name="remove-updatemanagement-solution"></a>移除 UpdateManagement 解決方案

您必須遵循下列步驟，才能將自動化帳戶從工作區取消連結，才能完全移除更新管理。 您將從工作區中移除**更新**解決方案。

1. 登入 [Azure 入口網站](https://portal.azure.com)。

2. 在 Azure 入口網站中，選取 [所有服務]。 在資源清單中輸入 **Log Analytics**。 當您開始輸入時，清單會根據您的輸入來篩選建議。 選取 [Log Analytics]。

3. 在您的 Log Analytics 工作區清單中，選取當您啟用更新管理時所選擇的工作區。
...
當它嘗試取消連結 Log Analytics 工作區時，您可以在功能表的 [**通知**] 底下追蹤進度。
...
當它嘗試取消連結自動化帳戶時，您可以在功能表的 [**通知**] 底下追蹤進度。
...
如果更新管理已設定為支援舊版的 Azure SQL 監視，則功能的設定可能已建立您應該移除的自動化資產。 為了進行更新管理，可能需要移除下列不再需要的項目：
   ...
   * 為更新管理建立的混合式背景工作角色群組，其名稱類似于*machine1，com_9ceb8108-26c9-4051-名稱-227600d715c8）*。
