...
移除更新管理之前，您必須先停止管理您的 Vm。 請參閱 [從更新管理移除 vm](remove-vms.md) ，以從功能中將其取消註冊。
...
若要重新啟用這項功能，請參閱 [從自動化帳戶啟用更新管理](enable-from-automation-account.md)、 [流覽 Azure 入口網站來啟用更新管理](enable-from-portal.md)、 [從 runbook 啟用更新管理](enable-from-runbook.md)，或 [從 Azure VM 啟用更新管理](enable-from-vm.md)。
