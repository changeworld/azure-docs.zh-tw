...
您可以使用 Azure 自動化中的更新管理，在 Azure、內部部署環境或其他雲端環境中，管理對 Windows 和 Linux 機器的作業系統更新。 您可以快速評估所有代理程式機器上可用更新的狀態，並管理為伺服器安裝必要更新的程序。

您可以透過下列方式來啟用 VM 的更新管理：

* 從適用於一或多部 Azure 電腦的 [Azure 自動化帳戶](automation-onboard-solutions-from-automation-account.md)啟用。
* 針對非 Azure 電腦手動啟用。
...
* 針對[多部 Azure VM](manage-update-multi.md)，透過從 Azure 入口網站的虛擬機器頁面中選取這些 VM 的方式來啟用。
...
更新管理所管理的機器會使用下列設定來執行評量和更新部署：

* 適用於 Windows 或 Linux 的 Log Analytics 代理程式
* 適用於 Linux 的 PowerShell 預期狀態組態 (DSC)
* 自動化 Hybrid Runbook Worker
* 適用於 Windows 機器的 Microsoft Update 或 Windows Server Update Services (WSUS)

下圖說明更新管理如何評估安全性更新，並將其套用至工作區中所有已連線的 Windows Server 和 Linux 機器：

![更新管理工作流程](./media/automation-update-management/update-mgmt-updateworkflow.png)

更新管理可用來在相同租用戶的多個訂用帳戶中以原生方式部署機器。

針對 Linux 機器，發行套件之後，需要 2 到 3 小時的時間，才會顯示修補程式以進行評量。 針對 Windows 機器，在發行之後，需要 12 到 15 小時的時間，才會顯示修補程式以進行評量。

在機器完成更新合規性掃描之後，代理程式會將資訊大量轉送到 Azure 監視器記錄。 在 Windows 機器上，合規性掃描預設每 12 小時執行一次。

除了掃描排程，如果在更新安裝之前與更新安裝之後重新啟動 Log Analytics 代理程式，則會在 15 分鐘內啟動更新合規性掃描。

針對 Linux 機器，合規性掃描預設每小時執行一次。 如果重新啟動 Log Analytics 代理程式，則會在 15 分鐘內啟動合規性掃描。
...
* 每部 Windows 機器：更新管理會針對每部機器每天執行兩次掃描。 其每隔 15 分鐘就會查詢一次 Windows API 來取得上次更新時間，以判斷狀態是否已變更。 如果狀態已變更，更新管理就會啟動合規性掃描。
...
建議您使用定義例外狀況時所列出的位址。 針對 IP 位址，您可以下載 [Microsoft Azure 資料中心 IP 範圍](https://www.microsoft.com/download/details.aspx?id=41653)。 此檔案會每週更新，並反映目前已部署的範圍及任何即將進行的 IP 範圍變更。

遵循[在無法存取網際網路的情況下連線電腦](../azure-monitor/platform/gateway.md)中的指示，來設定無法存取網際網路的機器。
...
已投資 Microsoft Endpoint Configuration Manager 來管理電腦、伺服器和行動裝置的客戶，也需仰賴 Configuration Manager 的強度和成熟度，以協助管理軟體更新。 若要了解如何整合更新管理與 Configuration Manager，請參閱[整合更新管理與 Windows Endpoint Configuration Manager](updatemgmt-mecmintegration.md)。
...
Azure [Resource Manager 範本](automation-update-management-deploy-template.md)可用來協助您將更新管理部署到新的或現有的自動化帳戶及訂用帳戶中的 Azure 監視器 Log Analytics 工作區。 其不會設定應管理的機器範圍，這會在使用範本之後以個別步驟執行。
...
* 如需使用更新管理的詳細資訊，請參閱[管理 Azure VM 的更新和修補](automation-tutorial-update-management.md)。
