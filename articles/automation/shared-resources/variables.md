...
Azure 自動化會安全地儲存每個加密的變數。 無法使用隨附在 Azure PowerShell 模組中的[AzAutomationVariable](https://docs.microsoft.com/powershell/module/az.automation/get-azautomationvariable?view=azps-3.5.0) Cmdlet 來抓取它的值。 取得加密值的唯一方法是使用 runbook 或 DSC 設定中的 `Get-AutomationVariable` 活動。
...
變數不限於指定的資料類型。 如果您想要指定不同類型的值，則必須使用 Windows PowerShell 來設定變數。 如果您指出**未定義**，變數的值會設定為 Null，而且您必須使用[AzAutomationVariable](https://docs.microsoft.com/powershell/module/az.automation/set-azautomationvariable?view=azps-3.5.0)指令，或 `Set-AutomationVariable` 活動來設定此值。
...
下表中的活動是用來存取 runbook 和 DSC 設定中的變數。 本文開頭的加密變數會說明 `Get-AzAutomationVariable` 和 `Get-AutomationVariable` 之間的差異。
...
腳本會使用 `New-AzAutomationVariable` Cmdlet 來建立新的變數，並設定其初始值。 然後，它可以使用 `Get-AzAutomationVariable`來抓取值。 如果值是簡單型別，則會抓取相同的型別。 如果它是複雜型別，則會抓取 `PSCustomObject` 型別。
...
使用 [`Set-AutomationVariable`] 活動可設定 PowerShell runbook 或 DSC 設定中自動化變數的值，以及用來取得它的 `Get-AutomationVariable`。 您不應該在 runbook 或 DSC 設定中使用 `Set-AzAutomationVariable` 和 `Get-AzAutomationVariable` Cmdlet 或其 AzureRM 模組對等專案，因為它們比工作流程活動更有效率。 

請注意，您無法使用 `Get-AzAutomationVariable` 或其對等的 AzureRM 模組來取得安全變數的值。 

從 runbook 或 DSC 設定中建立新變數的唯一方式是使用 `New-AzAutomationVariable` Cmdlet。
...
在圖形化 runbook 中，您可以新增 `Get-AutomationVariable` 或 `Set-AutomationVariable` 活動。 只要以滑鼠右鍵按一下圖形化編輯器 [程式庫] 窗格中的變數，然後選取您想要的活動即可。
