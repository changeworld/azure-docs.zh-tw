...
本教學課程將逐步引導您在 Azure 自動化中建立 [PowerShell 工作流程 Runbook](automation-runbook-types.md#powershell-workflow-runbooks)。 從您測試和發佈的簡單 Runbook 開始,同時瞭解如何跟蹤 Runbook 作業的狀態。 然後修改 Runbook 以實際管理 Azure 資源,通過啟動 Azure 虛擬機進行說明。 最後,通過添加 Runbook 參數使 Runbook 更加健壯。
...
1. 通過選擇 **「創建執行簿」 建立新的 Runbook。**
1. 將 Runbook 命名為「MyFirstRunbook-Workflow」 ****。
1. 在這種情況下,您會建立[PowerShell 工作流執行簿](automation-runbook-types.md#powershell-workflow-runbooks)。 為**Runbook 類型**選擇**PowerShell 工作流**。
1. 按一下 [建立] **** 來建立 Runbook 並開啟文字式編輯器。

## <a name="step-2---add-code-to-the-runbook"></a>步驟 2 - 將程式碼加入至 runbook

您可以直接在 Runbook 中鍵入代碼,也可以從庫控制項中選擇 cmdlet、Runbook 和資產,並將它們添加到具有任何相關參數的 Runbook 中。 在本教學中,您將直接在 Runbook 中鍵入代碼。

1. Runbook 當前為空,只有`Workflow`必需的關鍵字、Runbook 的名稱以及包含整個工作流的大括弧。
...
您建立的 Runbook 仍處於草稿模式。 必須先發佈它,然後才能在生產中運行它。 當您發佈 Runbook 時，您會使用草稿版本覆寫現有的已發佈版本。 在此情況下，因為您剛剛建立 Runbook，所以還沒有已發佈的版本。

1. 按一下 [發佈]**** 來發佈 Runbook，然後出現提示時按一下 [是]****。
1. 向左滾動以檢視 Runbook 頁中的**Runbook,** 並注意「**創作狀態」** 欄位設定為 **「已發布**」 。
...
1. 執行簿狀態顯示"已完成"後,按一下「**輸出**」。 將開啟「輸出」頁,您可以在其中查看`Hello World`消息。
...
1. 按下 **「資源**下的**作業**」以打開此 Runbook 的作業頁面。 此頁列出 Runbook 創建的所有作業。 您應該只看到列出的一個作業,因為您只運行該作業一次。
...
您已測試並發行您的 Runbook，但是到目前為止，它似乎並不實用。 您想要讓它管理 Azure 資源。 除非使用訂閱的認證,否則無法執行此操作。 身份驗證使用[連接-AzAccount](https://docs.microsoft.com/powershell/module/az.accounts/connect-azaccount?view=azps-3.7.0) cmdlet。
...
1. 導覽到 MyFirstRunbook-工作流頁面,然後單擊 **「編輯**」打開文字編輯器。
2. 刪除列`Write-Output`。
3. 將游標放在大括弧之間的空白行。
...
1. 按一下 [開始] **** 以開始測試。 完成後,您應該會看到類似於以下內容的輸出,顯示帳戶中的基本資訊。 此操作確認憑據有效。
...
Runbook 目前啟動在 Runbook 中硬編碼的 VM。 如果可以在啟動 Runbook 時指定 VM,則該 VM 將更有用。 讓我們向 Runbook 添加輸入參數以提供該功能。

1. 將`VMName``ResourceGroupName`和 參數的變數添加到 Runbook,並將變數`Start-AzVM`與 cmdlet 一起使用,如下所示。
