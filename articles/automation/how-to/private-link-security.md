...
ms.locfileid: "86207277"
---
# <a name="use-azure-private-link-to-securely-connect-networks-to-azure-automation-preview"></a>使用 Azure 私人連結，安全地將網路連線至 Azure 自動化 (preview) 
...
本文涵蓋時機，以及如何使用您的自動化帳戶來設定私人端點 (預覽) 。

![Azure 自動化的私用連結概念總覽](./media/private-link-security/private-endpoints-automation.png)

>[!NOTE]
> Azure 自動化 (preview) 的私人連結支援僅適用于 Azure 商業和 Azure 美國政府雲端。
...
Azure 自動化私用連結會將一或多個私人端點連接 (，因此，它們包含在您的自動化帳戶資源) 中的虛擬網路。 這些端點是使用 webhook 來啟動 runbook 的電腦、裝載混合式 Runbook 背景工作角色的機器，以及 DSC 節點。
...
狀態設定提供 Azure 設定管理服務，可讓您針對任何雲端或內部部署資料中心內的節點，撰寫、管理和編譯 PowerShell Desired State Configuration (DSC) 設定。
...
在設定您的自動化帳戶資源之前，請考慮您的網路隔離需求。 評估您的虛擬網路對公用網際網路的存取權，以及您自動化帳戶的存取限制 (包括將私人連結群組範圍設定為 Azure 監視器記錄（如果與您的自動化帳戶整合) ）。 也請在您的計畫中納入自動化服務[DNS 記錄](./automation-region-dns-records.md)的審查，以確保支援的功能不會有任何問題。
...
同樣地， (FQDN) 的唯一完整功能變數名稱是針對狀態設定 (agentsvc) 以及混合式 Runbook 背景工作角色作業執行時間 (jrds) 所建立。 系統會為每個使用者指派 VNet 中的個別 IP，且**線上狀態**會顯示為 [**已核准**]。
...
您可以設定自動化帳戶來拒絕所有公用設定，並只允許透過私人端點的連線，以進一步增強網路安全性。 如果您想要限制只能從 VNet 記憶體取自動化帳戶，而不允許從公用網際網路存取，您可以將屬性設定 `publicNetworkAccess` 為 `$true` 。

當 [**拒絕公用網路存取**] 設定設為時 `true` ，只允許透過私人端點的連線，而且所有透過公用端點的連線都會遭到拒絕，並出現錯誤訊息。
