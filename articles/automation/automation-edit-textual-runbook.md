...
Azure 自動化中的每個 Runbook 有兩個版本，「草稿」和「已發佈」。 您編輯 Runbook 的「草稿」版本然後發佈，讓它可以執行。 無法編輯「已發佈」版本。 有關詳細資訊,請參閱發佈[Runbook](manage-runbooks.md#publishing-a-runbook)。

本文提供執行與此編輯器不同的功能的詳細步驟。 這些不適用於[圖形執行簿](automation-runbook-types.md#graphical-runbooks)。 要使用這些 Runbook,請參考[Azure 自動化的圖像創作](automation-graphical-authoring-intro.md)。
...
2. 在 **「行程自動化」** 下,選擇**Runbook**以打開 Runbook 清單。
3. 選擇要編輯的 Runbook,然後單擊「**編輯**」。
4. 編輯 Runbook。
5. 編輯完成時，按一下 [儲存]****。
6. 如果要發佈 Runbook 的最新草稿版本,請單擊 **「發布**」。
...
4. 右鍵按一下 cmdlet 名稱以插入並選擇「**新增到畫布**」 。。 如果 cmdlet 具有多個參數集,則編輯器將添加預設集。 您也以展開 Cmdlet 以選取不同的參數集合。
5. 請注意,cmdlet 的代碼將插入其整個參數清單。
6. 為任何必需的參數提供適當的值,以代替由角括弧(<>)括起來的值。 刪除不需要的任何參數。

### <a name="insert-code-for-a-child-runbook-into-a-runbook"></a>將子執行簿的代碼插入到 Runbook 中
...
2. 在程式庫控制項中，展開 [資產] **** 節點。
3. 展開所需資產類型的節點。
4. 右鍵按一下要插入的資產名稱,然後選擇 **「添加到畫布**」。 對於[變數資產](automation-variables.md),選擇 **「獲取變數」以畫布**或**添加「設定變數」以畫布**,具體取決於您是想要獲取還是設置變數。
5. 請注意,資產的代碼將插入到 Runbook 中。

## <a name="editing-an-azure-automation-runbook-using-windows-powershell"></a>使用 Windows PowerShell 編輯 Azure 自動化執行簿

要使用 Windows PowerShell 編輯 Runbook,請使用您選擇的編輯器並將 Runbook 儲存到 **.ps1**檔。 您可以使用[匯出-AzAutomationRunbook](/powershell/module/Az.Automation/Export-AzAutomationRunbook) cmdlet 檢索 Runbook 的內容。 您可以使用[匯入-AzAutomationRunbook](/powershell/module/Az.Automation/import-azautomationrunbook) cmdlet 將現有的草稿執行簿替換為修改後的計畫執行簿。
...
下列範例命令顯示如何以指令碼檔案內容取代現有 Runbook 的內容。 這個範例程序與[使用 Windows PowerShell 從指令碼檔案匯入 Runbook](manage-runbooks.md#importing-a-runbook) 的相同。
...
* [Azure 自動化的圖像創作](automation-graphical-authoring-intro.md)
