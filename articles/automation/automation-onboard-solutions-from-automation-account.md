...
在登錄 Azure。 https://portal.azure.com

## <a name="enable-solutions"></a>啟用解決方案

導覽到您的自動化帳戶,並在**配置管理**下選擇 **「庫存**」或 **「更改追蹤**」。

選擇日誌分析工作區和自動化帳戶,然後單擊 **「啟用」** 以啟用解決方案。 啟用解決方案最多需要 15 分鐘。
...
當更改追蹤和清單解決方案入入通知完成後,選擇**更新管理**下的 **「更新管理**」。

更新管理解決方案允許您管理 Azure 和混合 VM 的更新和修補程式。 您可以評估可用更新的狀態、計劃安裝所需更新以及查看部署結果以驗證更新已成功應用於這些更新。
...
每個解決方案都會使用工作區中的範圍設定，來設定取得解決方案的電腦。 範圍設定是一或多個已儲存搜尋的群組，用以將解決方案的範圍限定於特定電腦。 要存取範圍設定,請在「**資源**」下的自動化帳戶中選擇**工作區**。 然後在**工作區數據源**下的工作區中,選擇 **「範圍配置**」。。
...
導覽到日誌分析工作區,並在 **「常規**」下選擇 **「已保存的搜索**」。 下表顯示這些解決方案所使用的兩個已儲存搜尋：
...
從您的自動化帳戶中選擇 **「設定管理**下的**庫存**」或 **「更改追蹤**」,或**選擇「更新管理**下的**更新管理**」 。
...
您必須手動加入不在 Azure 中的機器。 從您的自動化帳戶中選擇 **「設定管理**下的**庫存**」或 **「更改追蹤**」,或**選擇「更新管理**下的**更新管理**」 。

按一下 [加入非 Azure 電腦]****。 此操作將打開一個新的瀏覽器視窗,其中包含[安裝和設定 Windows 的日誌分析代理的說明](../azure-monitor/platform/log-analytics-agent.md),以便電腦可以開始向解決方案報告。 如果您正在進入當前由系統中心操作管理員管理的電腦,則不需要新代理,並且工作區資訊將輸入到現有代理中。

## <a name="onboard-machines-in-the-workspace"></a>讓工作區中的機器上線

您必須將手動安裝的機器或已向您工作區回報的機器新增至 Azure 自動化，才能啟用解決方案。 從您的自動化帳戶中選擇 **「設定管理**下的**庫存**」或 **「更改追蹤**」,或**選擇「更新管理**下的**更新管理**」 。
...
您可以透過新增初始儲存的搜尋來添加範圍設定。 有關詳細資訊,請參閱[儲存的搜尋](#saved-searches)。

### <a name="selected-machines"></a>選取的機器

要新增一台或多台電腦啟用解決方案,請選擇 **「在選擇電腦上啟用」 選單**擊要新增到解決方案的每台電腦旁邊**新增**。 此工作會為解決方案將所選取機器的名稱加入至電腦群組儲存的搜尋查詢。
