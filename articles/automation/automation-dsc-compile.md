...
您可以使用[啟動-Az自動化編譯作業](/powershell/module/az.automation/start-azautomationdsccompilationjob)開始使用 Windows PowerShell 編譯。 以下範例代碼開始編譯稱為範例配置的 DSC 配置。

```powershell
Start-AzAutomationDscCompilationJob -ResourceGroupName 'MyResourceGroup' -AutomationAccountName 'MyAutomationAccount' -ConfigurationName 'SampleConfig'
```

**啟動-AzAutomationDsc編譯作業**返回可用於跟蹤作業狀態的編譯作業物件。 然後,您可以使用此編譯作業物件與[Get-AzAutomationDsc 編譯作業](/powershell/module/az.automation/get-azautomationdsccompilationjob)來確定編譯作業的狀態,[然後獲取-AzAutomationD編譯作業輸出](/powershell/module/az.automation/get-azautomationdscconfiguration)以查看其流(輸出)。 以下範例開始編譯 SampleConfig 設定,等待它完成,然後顯示其流。
...
下列範例 DSC 組態會透過 $ConfigurationData 和 $AllNodes 關鍵字來使用 **ConfigurationData**。 對於此範例,您還需要[xWeb 管理模組](https://www.powershellgallery.com/packages/xWebAdministration/)。
...
您可以使用 Windows PowerShell 編譯前面的 DSC 配置。 以下文稿將兩個節點配置添加到 Azure 自動化狀態配置拉取服務:設定數據示例.MyVM1 和配置數據示例.MyVM3。
...
Azure 自動化中的 DSC 配置可以使用**獲取自動化 PS認證來源資訊,** 引用自動化認證為雲端的資產。 如果配置具有指定**PS認證**的參數,請使用**獲取自動化PS認證,** 透過將 Azure 自動化認證資產的字串名稱傳遞給 cmdlet 來檢索認證。 然後,將該物件用於需要**PSCredential**物件的參數。 會在背景中取出具有該名稱的 Azure 自動化認證資產，並傳遞至設定。 下面的示例顯示了此方案在操作中。
...
您可以使用 PowerShell 來編譯上述 DSC 設定。 以下 PowerShell 代碼將兩個節點配置添加到 Azure 自動化狀態配置拉取伺服器:憑據範例.MyVM1 和認證例.MyVM2。
...
> [!NOTE]
> 編譯完成後,您可能會收到錯誤消息"'Microsoft.PowerShell.Management'"模組未導入,因為"Microsoft.PowerShell.管理"的卡入式已導入。 您可以放心忽略這個訊息。
...
- 有關 PowerShell cmdlet 引用,請參閱[Azure 自動化狀態設定 cmdlet](/powershell/module/az.automation)。
