...
1. 在您的自動化帳戶中，按一下 **[State configuration （DSC）**]。
...
`Start-AzAutomationDscCompilationJob`傳回可用來追蹤工作狀態的編譯工作物件。 接著，您可以使用此編譯工作物件搭配[AzAutomationDscCompilationJob](/powershell/module/az.automation/get-azautomationdsccompilationjob)來判斷編譯工作的狀態，並[取得 AzAutomationDscCompilationJobOutput](/powershell/module/az.automation/get-azautomationdscconfiguration)以查看其資料流程（輸出）。 下列範例會開始編譯 SampleConfig 設定，等待它完成，然後顯示其資料流程。
...
下列範例會使用`FeatureName`和`IsPresent`參數，來判斷在編譯期間產生的**parametersexample.sample**節點設定中的屬性值。
...
如需將物件`PSCredential`傳遞為參數的相關資訊，請參閱[認證資產](#credential-assets)。
...
`ConfigurationData`是內建的 DSC 參數，可讓您在使用 PowerShell DSC 時，將結構化設定與任何環境特定的設定分開。 如需詳細資訊，請參閱[在 POWERSHELL DSC 中分隔 "What" 與 "Where"](https://devblogs.microsoft.com/powershell/separating-what-from-where-in-powershell-dsc/)。

> [!NOTE]
> 在 Azure 自動化狀態設定中進行編譯時，您`ConfigurationData`可以在 Azure PowerShell 中使用，但不能用於 Azure 入口網站中。

下列範例 DSC 設定會透過`ConfigurationData` `$ConfigurationData`和`$AllNodes`關鍵字來使用。 在此範例中，您也需要[請參閱 xwebadministration 模組](https://www.powershellgallery.com/packages/xWebAdministration/)。
...
Azure 自動化中的 DSC 組態可以使用 `Get-AutomationPSCredential` Cmdlet 參考自動化認證資產。 如果設定具有指定`PSCredential`物件的參數，請使用，將`Get-AutomationPSCredential` Azure 自動化認證資產的字串名稱傳遞給 Cmdlet，以取得認證。 然後將該物件用於需要`PSCredential`物件的參數。 會在背景中取出具有該名稱的 Azure 自動化認證資產，並傳遞至設定。 下列範例示範此案例的實際運作方式。
...
您可以告訴 PowerShell DSC，使用設定資料在產生的節點設定 Mof 中以純文字輸出認證是正常的。 您應該`ConfigurationData`針對`PSDscAllowPlainTextPassword = $true`每個出現在 DSC 設定中的節點區塊名稱傳遞，並使用認證。
...
> [!NOTE]
> 當編譯完成時，您可能會收到錯誤訊息`The 'Microsoft.PowerShell.Management' module was not imported because the 'Microsoft.PowerShell.Management' snap-in was already imported.` ，您可以放心地忽略此訊息。

## <a name="compiling-your-dsc-configuration-in-windows-powershell"></a>在 Windows PowerShell 中編譯 DSC 設定

您也可以匯入在 Azure 外部編譯的節點設定（MOF 檔案）。 匯入包含從開發人員工作站或服務（如[Azure DevOps](https://dev.azure.com)）進行編譯。 這個方法有多項優點，包括效能和可靠性。

Windows PowerShell 中的編譯也提供簽署設定內容的選項。 DSC 代理程式會在受管理節點上本機驗證已簽署的節點設定。 驗證可確保套用至節點的設定來自于已授權的來源。

> [!NOTE]
> 節點設定檔案必須不超過 1 MB，才能讓 Azure 自動化匯入它。
...
您可以從開發人員工作站或在組建服務中執行此程式，例如[Azure DevOps](https://dev.azure.com)。 接著，您可以將編譯設定所產生的 MOF 檔案匯入到 Azure 狀態設定服務。

### <a name="import-a-node-configuration-in-the-azure-portal"></a>匯入 Azure 入口網站中的節點設定

1. 在您的自動化帳戶中，按一下 [設定**管理**] 底下的 **[STATE configuration （DSC）** ]。
1. 在 [State configuration （DSC **）] 頁面**上，按一下 [設定] 索引標籤，然後按一下 [**新增**]。
1. 在 [匯入] 頁面上，按一下 [**節點設定檔**] 欄位旁的資料夾圖示，以流覽本機電腦上的節點設定 MOF 檔案。
