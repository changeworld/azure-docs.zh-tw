...
4. 為 runbook 命名「MyFirstRunbook-圖形化」。
...
1. 在 [程式庫] 控制項中，按一下 [搜尋] 欄位並輸入「寫入輸出」。 搜尋結果如下圖所示： <br> ![Microsoft.PowerShell.Utility](media/automation-first-runbook-graphical/search-powershell-cmdlet-writeoutput.png)
...
1. [**標籤**] 欄位的預設值為 Cmdlet 的名稱，但您可以將它變更為更容易瞭解的專案。 將它變更為「將 Hello World 寫入輸出」。
...
1. [**資料來源**] 下拉式功能表會提供您可以用來填入參數值的來源。 在此功能表中，選取 [ **PowerShell 運算式**]。 
   ...
   您可以使用來自這類來源的輸出作為另一個活動、自動化資產或 PowerShell 運算式。 在此情況下，輸出只是「Hello World」。 您可以使用 PowerShell 運算式，並指定字串。<br>
...
1. 請注意，會建立[runbook 工作](automation-runbook-execution.md)，並在窗格中顯示其狀態。
   ...
   作業狀態會以「已排入佇列」開始，表示作業正在等候雲端中的 runbook 背景工作可供使用。 當背景工作宣告作業時，狀態會變更為 [正在啟動]。 最後，當 runbook 實際開始執行時，狀態會變成 [正在執行]。

1. 當 runbook 作業完成時，[測試 pagn] 會顯示其輸出。 在此情況下，您會看到「Hello World」。<br> ![Hello World](media/automation-first-runbook-graphical/runbook-test-results.png)
1. 關閉 [測試] 窗格以返回畫布。
...
1. 向右移動以查看 MyFirstRunbook 的頁面-圖形化。
...
1. 已建立的 runbook 作業會開啟 [作業] 窗格。 確認 [**作業狀態**] 顯示為 [已完成]。
1. 按一下 [**輸出**] 以開啟 [輸出] 頁面，您可以在其中看到顯示「Hello World」。
1. 關閉 [輸出] 頁面。
...
1. 關閉 [資料流程] 窗格和 [作業] 窗格，以返回 MyFirstRunbook-圖形化頁面。
...
1. 流覽至您的 runbook，並在 MyFirstRunbook-圖形化頁面上選取 [**編輯**]。
1. 您不再需要「寫入 Hello World 來輸出」專案。 只要按一下省略號，然後選取 [**刪除**]。
...
1. 在畫布上，選取 [ **Disconnect-azaccount]** 。 在 [設定控制] 窗格的 [**標籤**] 欄位中，輸入「登入 Azure」。
...
1. 在 [參數值] 頁面上，進行下列設定，然後按一下 **[確定]** 。
   ...
   * **資料來源**--選取**活動輸出**。
   * 資料來源清單-選取 [**取得執行**身分連線]。
   * **欄位路徑**--輸入 "ApplicationId"。 您要指定欄位路徑的屬性名稱，因為活動會輸出具有多個屬性的物件。
...
1. 按一下 [ **TENANTID**]，然後在 [參數值] 頁面上進行下列設定。 完成後，請按兩次 **[確定]** 。
    ...
    * 資料來源清單-選取 [**取得執行**身分連線]。
    * **欄位路徑**--輸入 "TenantId"。 
1. 在 [程式庫] 控制項的 [搜尋] 欄位中輸入 "Set-azcoNtext"。
1. 將**set-azcoNtext**新增至畫布。
1. 選取畫布上的 [ **set-azcoNtext** ]。 在 [設定控制] 窗格的 [**標籤**] 欄位中，輸入「指定訂用帳戶識別碼」。
...
現在您必須新增**開始 update-azvm**活動，以啟動虛擬機器。 您可以在 Azure 訂用帳戶中挑選任何 VM，而現在您會將該名稱硬式編碼到[update-azvm](https://docs.microsoft.com/powershell/module/az.compute/start-azvm?view=azps-3.5.0) Cmdlet。
...
1. 選取 [ResourceGroupNameParameterSetName] 參數集。 欄位**ResourceGroupName**和**名稱**旁邊有驚嘆號，表示它們是必要參數。 請注意，這兩個欄位都預期為字串值。
1. 選取 [Start-AzureRmVM]。 針對 [**資料來源**] 欄位選取 [ **PowerShell 運算式**]。 針對您用來啟動此 runbook 的 VM，請輸入以雙引號括住的電腦名稱稱。 按一下 [確定]。
1. 選取 [ResourceGroupName]。 針對 [**資料來源**] 欄位使用 [PowerShell 運算式] 值，然後輸入以雙引號括住的資源組名。 按一下 [確定]。
1. 按一下 [測試] 窗格，您便可測試 Runbook。
1. 按一下 [開始] 以開始測試。 完成後，請確定 VM 已啟動。 
...
1. 按一下 [MyFirstRunbook-圖形化] 窗格上的 [**編輯**] 來開啟圖形化編輯器。
...
1. 針對**InputObject**，將 [**資料來源**] 變更為 [ **PowerShell 運算式**]，然後在 [已成功啟動 $VMName] 運算式中輸入。
1. 在第二個**寫入輸出**控制項上，按一下 [**參數**]，然後將 [**標籤**] 值變更為 [**通知 VM 啟動失敗**]。
1. 針對**InputObject**，將**資料來源**變更為**PowerShell 運算式**，然後在運算式 "$VMName 無法啟動" 中輸入。
