...
* 如有必要,請根據使用 cmdlet[匯入 Azure 模組](shared-resources/modules.md)或[更新模組](automation-update-azure-modules.md)。

## <a name="differences-from-powershell-workflow-runbooks"></a>與 PowerShell 工作流程執行簿差異

PowerShell 運行簿與 PowerShell 工作流執行簿具有相同的生命週期、功能和管理。 但是,存在一些差異和限制。

| 特性  | 電源殼執行簿 | 電源外殼工作流執行簿 |
| ------ | ----- | ----- |
| 速度 | 運行速度快,因為它們不使用編譯步驟。 | 運行得更慢。 |
| 檢查點 | 不支持檢查點。 PowerShell 運行簿只能從一開始就恢復操作。 | 使用檢查點,允許工作簿從任何點恢復操作。 |
| 命令執行 | 僅支援串行執行。 | 支援串行和並行執行。|
...
5. 在這種情況下,您會建立[PowerShell 執行簿](automation-runbook-types.md#powershell-runbooks)。 為**Runbook 類型**選擇**PowerShell。**
6. 按一下 [建立] **** 來建立 Runbook 並開啟文字式編輯器。

## <a name="step-2---add-code-to-the-runbook"></a>步驟 2 - 將程式碼加入至 runbook

您可以直接將程式碼輸入到 runbook 中，或從程式庫控制項選取 cmdlet、runbook 和資產，並利用任何相關的參數將它們加入至 runbook。 在本教學中,您將直接在 Runbook 中鍵入代碼。

1. 您的 Runbook 目前為空。 鍵入`Write-Output "Hello World"`腳本正文。

   ![Hello World](media/automation-first-runbook-textual-powershell/automation-helloworld.png)

2. 按一下 [儲存] **** 來儲存 Runbook。

## <a name="step-3---test-the-runbook"></a><a name="step-3---test-the-runbook"> </a>步驟 3 - 測試執行簿
...
3. 請注意,將創建[Runbook 作業](automation-runbook-execution.md),其狀態將顯示在窗格中。
   ...
   作業狀態以「排隊」開頭,指示作業正在等待雲中的 Runbook 工作人員變為可用。 當工作人員聲明作業時,狀態更改為"開始"。 最後,當 Runbook 實際開始運行時,狀態變為"
...
1. 向後滾動到右側以查看**MyFirstRunbook-PowerShell**的頁面。
...
1. Runbook 狀態顯示"已完成"後,按一下「**輸出」** 以打開「`Hello World`輸出」頁,您可以在其中顯示顯示。
...
1. 在 **「詳細資訊**」下,按下 **「作業**」以打開此 Runbook 的作業頁面。 此頁列出 Runbook 創建的所有作業。 您應該只看到列出的一個作業,因為您只運行該作業一次。
...
> [!NOTE]
> 對於 PowerShell`Add-AzAccount``Add-AzureRMAccount`執行簿`Connect-AzAccount`,並且是 的別名。 您可以使用這些 cmdlet,也可以將自動化帳戶中的[模組更新](automation-update-azure-modules.md)到最新版本。 即使您剛剛創建了新的自動化帳戶,您也可能需要更新模組。
...
1. 按一下 [開始] **** 以開始測試。 完成後,您應該會看到類似於以下內容的輸出,顯示帳戶中的基本資訊。 此輸出會確認該執行身分帳戶有效。
...
由於您的 Runbook 正在驗證您的 Azure 訂用帳戶，所以您可以管理資源。 讓我們添加一個命令來啟動虛擬機器。 您可以選擇 Azure 訂閱中的任何虛擬機,並且只需在 Runbook 中立即輸入名稱的硬編碼。

1. 添加到 runbook 文稿,添加[啟動 AzVM](https://docs.microsoft.com/powershell/module/Az.Compute/Start-AzVM?view=azps-3.5.0) cmdlet 以啟動虛擬機器。 如下所示,cmdlet 啟動具有`VMName`名稱 且資源組`ResourceGroupName`名為 的虛擬機。
...
1. 按下 **「開始」** 以開始測試。 完成後,請確保虛擬機器已啟動。

## <a name="step-7---add-an-input-parameter"></a>步驟 7 - 新增輸入參數

Runbook 當前啟動在 Runbook 中硬編碼的虛擬機器。 如果在啟動 Runbook 時指定虛擬機,則 Runbook 更有用。 讓我們向 Runbook 添加輸入參數以提供該功能。

1. 在文字編輯器中`Start-AzVM`,修改 cmdlet 以`VMName`使用`ResourceGroupName`參數和的變數。 
...
1. 鍵入要啟動的虛擬機器的**VMNAME**和**RESOURCEGROUPNAME**的值,然後按下 **「 確定**」。<br><br> ![傳遞參數](media/automation-first-runbook-textual-powershell/automation-pass-params.png)<br>
1. 運行簿完成後,請確保虛擬機已啟動。
