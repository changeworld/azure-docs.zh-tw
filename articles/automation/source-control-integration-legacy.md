...
如果您已經有想要連結到 Azure 自動化的 GitHub 帳戶和存放庫，則登入您現有的帳戶並從下面的步驟 2 開始執行。 否則，導航到[GitHub，](https://github.com/)註冊新帳戶，然後[創建新的存儲庫](https://help.github.com/articles/create-a-repo/)。
...
2. [原始檔控制]**** 分頁隨即開啟，您可以在其中設定您的 GitHub 帳戶詳細資料。 以下是要設定的參數清單：  
   ...
   | Runbook 資料夾路徑 |Runbook 資料夾路徑可指定 GitHub 儲存機制中的路徑，以便您從中推送或提取程式碼。 它必須以 **/foldername/subfoldername**格式輸入。 只有 Runbook 資料夾路徑中的 Runbook 會同步處理至您的自動化帳戶。 Runbook 資料夾路徑之子資料夾中的 Runbook **不會** 進行同步處理。 用於**/** 同步存儲庫下的所有 Runbook。 |
...
4. 設定參數之後，它們會顯示在 [設定原始檔控制]**** 分頁。  
    ...
    ![顯示設定的源控制項頁](media/source-control-integration-legacy/automation-SourceControlConfigure.png)
5. 按一下 [確定]**** 後，原始檔控制整合現已針對您的自動化帳戶設定，而且應以您的 GitHub 資訊進行更新。 您現在可以按一下此部分來檢視所有原始檔控制同步處理工作歷程記錄。  

    ![當前配置的原始程式碼管理配置的值](media/source-control-integration-legacy/automation-RepoValues.png)
6. 設置原始程式碼管理後，在自動化帳戶中創建兩個[變數資產](automation-variables.md)。 此外，授權應用程式將添加到您的 GitHub 帳戶中。
   ...
   * **Microsoft.Azure.Automation.SourceControl.OauthToken**變數包含 OAuthToken 的安全加密值。  
     ...
     ![顯示源控制變數的視窗](media/source-control-integration-legacy/automation-Variables.png)  

   * **自動化原始檔控制** 已做為已授權的應用程式加入至您的 GitHub 帳戶。 要查看應用程式：從 GitHub 主頁導航到**設定檔** > **設置** > **應用程式**。 此應用程式可讓 Azure 自動化將 GitHub 儲存機制同步至自動化帳戶。  
...
2. 編輯 Runbook 之後，加以儲存並按一下 [編輯]**** 分頁中的 [簽入]****。  
    ...
    ![顯示簽入 GitHub 按鈕的視窗](media/source-control-integration-legacy/automation-CheckinButton.png)
...
3. 當您按一下 [簽入]**** 時，將會出現一個確認訊息，按一下 [是]**** 繼續進行。  
    ...
    ![確認簽入源控制的對話方塊](media/source-control-integration-legacy/automation-CheckinMessage.png)
...
6. 在工作完成時重新整理您的 GitHub 儲存機制，即可檢視變更。  存儲庫中應該有一個提交消息，其中有一條提交消息 **：Azure 自動化中更新*的 Runbook 名稱*。**  
...
1. 從設置原始程式碼控制的自動化帳戶中，打開**原始程式碼管理集成/存儲庫同步**頁面，然後按一下 **"同步**"。 系統會提示您收到確認訊息，按一下"**是**"以繼續。  

    ![同步按鈕，消息確認所有 Runbook 都將同步](media/source-control-integration-legacy/automation-SyncButtonwithMessage.png)
...
2. 同步處理會啟動 Runbook： **Sync-MicrosoftAzureAutomationAccountFromGitHubV1**。 此 Runbook 會連接到 GitHub 並將儲存機制中的變更提取至 Azure 自動化。 您應該會在此動作的 [存放庫同步處理]**** 分頁看到新作業。 若要檢視同步處理工作的詳細資料，按一下以開啟作業詳細資料分頁。  
    ...
    ![顯示 GitHub 存儲庫上同步作業同步結果的視窗](media/source-control-integration-legacy/automation-SyncRunbook.png)

    > [!NOTE]
    > 從原始檔控制進行的同步處理會針對目前在原始檔控制中的 **所有** Runbook，覆寫目前存在於您的自動化帳戶中的 Runbook 草稿版本。 要同步處理的 Git 對等命令列指示為 **git pull**

![顯示掛起的原始程式碼管理同步作業的所有日誌的視窗](media/source-control-integration-legacy/automation-AllLogs.png)
...
若要中斷與 GitHub 帳戶的連線，請開啟 [存放庫同步處理] 分頁，然後按一下 [中斷連線]****。 一旦中斷與原始檔控制的連線，先前同步處理的 Runbook 仍會保留在您的自動化帳戶中，但不會啟用 [存放庫同步處理] 分頁。  
  ...
  ![顯示斷開按鈕以斷開源控制的視窗](media/source-control-integration-legacy/automation-Disconnect.png)
