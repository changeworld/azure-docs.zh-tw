...
1. 執行下列 PowerShell Cmdlet，以每個先前步驟中的*ResourceId*值取代 `<AutomationResourceId>` 和 `<WorkspaceResourceId>`。
...
* 篩選*DscResourceStatusData* ，以傳回套用至該資源的節點設定中呼叫的每個 DSC 資源的作業。 
* 篩選*DscResourceStatusData* ，以傳回任何失敗之 DSC 資源的錯誤資訊。
...
1. 在查詢欄位中鍵入下列搜尋內容來為您的警示建立記錄搜尋查詢：`Type=AzureDiagnostics Category='DscNodeStatus' NodeName_s='DSCTEST1' OperationName='DscNodeStatusData' ResultType='Failed'`。
   ...
   如果您已將來自多個自動化帳戶或訂用帳戶的記錄設定到您的工作區，就能依訂用帳戶或自動化帳戶來將警示分組。 從 DscNodeStatusData 搜尋中的 [資源] 欄位，衍生自動化帳戶名稱。
...
| SourceSystem | Azure 監視器記錄檔收集資料的方式。 適用于 Azure 診斷的一律為 "Azure"。 |
