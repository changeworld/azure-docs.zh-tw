...
description: 本文說明如何安裝 Azure 自動化混合式 Runbook 背景工作角色的資訊，以便您在本機資料中心或雲端環境內的 Linux 電腦上執行 Runbook。
services: automation
...
若要在 Linux 電腦上安裝和設定混合式 Runbook 背景工作角色，請遵循直接進行的手動程序。 您必須在 Azure Log Analytics 工作區中啟用混合式 Runbook 背景工作角色，然後執行一組命令將電腦註冊為背景工作角色，再將它新增至群組。
...
1. 使用下列其中一個方法，在 Azure 中啟用混合式 Runbook 背景工作角色：

   * 使用 [將 Azure 監視器記錄新增至您的工作區](../log-analytics/log-analytics-add-solutions.md)中的程式，將混合式 Runbook 背景工作角色新增至您的訂用帳戶。
   * 執行下列 Cmdlet：
...
> [!NOTE]
> 如果您針對 Azure VM 使用適用於 Linux 的 Azure 監視器虛擬機器擴充功能，建議您將 `autoUpgradeMinorVersion` 設定為 false，因為自動升級版本可能會導致混合式 Runbook 背景工作角色發生問題。 若要瞭解如何手動升級延伸模組，請參閱 [Azure CLI 部署](../virtual-machines/extensions/oms-linux.md#azure-cli-deployment)。
...
> [!NOTE]
> 此程式碼不會從電腦移除適用於 Linux 的 Log Analytics 代理程式。 它只會移除混合式 Runbook 背景工作角色的功能和設定。
