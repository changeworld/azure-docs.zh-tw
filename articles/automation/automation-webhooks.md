...
如果您在建立 webhook 時`WebhookData`指定的值，則會在 webhook 使用來自用戶端 POST 要求的資料來啟動 runbook 時遭到覆寫。 即使應用程式未在要求主體中包含任何資料，也會發生這種情況。 

如果您使用 webhook 以外的機制`WebhookData`來啟動定義的 runbook，則可以提供 runbook 可辨識`WebhookData`的值。 這個值應該是具有與`WebhookData`參數相同[屬性](#webhook-properties)的物件，如此一來，runbook 就可以使用它，就像它適用于`WebhookData` webhook 所傳遞的實際物件一樣。
...
針對下一個 runbook 範例，我們將定義下列屬性`WebhookData`：
...
下列範例顯示在的`RequestBody`屬性中，可供 runbook 使用的要求主體。 `WebhookData` 此值會以 JSON 格式格式化，以與要求主體中包含的格式相容。
