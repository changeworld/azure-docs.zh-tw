...
- **裝置 SDK 類型**。 如果您選取 [CMake] 做為 [專案類型]，這就是在中設定產生的程式碼將如何包含 Azure IoT C 裝置 SDK 的步驟 `CMakeLists.txt` ：
...
1. 在 VS Code 中，使用**Ctrl + Shift + P**開啟命令選擇區，輸入並選取 [ **C/c + +：編輯設定（JSON）** ]，以開啟檔案**c_cpp_properties.js** 。

1. 在區段中新增裝置 SDK 的路徑 `includePath` ：
...
1. 使用命令來安裝**GCC**、 **Git**、 `cmake` 和所有相依性 `apt-get` ：
...
1. 在 `cmake` 資料夾中建立子目錄，其中包含產生的程式碼 stub，並流覽至該資料夾：
...
若要使用 Visual Studio CMake 與 Windows 上的裝置 C SDK 建立裝置程式碼，請在命令列中，參閱[IoT 隨插即用快速入門](./quickstart-create-pnp-device-windows.md)。 下列步驟示範如何在 Visual Studio 中，將裝置程式碼與裝置 C SDK Vcpkg 做為 CMake 專案一起建立。
...
1. 開啟 Visual Studio，選擇 [檔案] **> 開啟 > CMake ...** ]，以開啟 `CMakeLists.txt` 資料夾中包含所產生程式碼的。
...
1. 使用[Homebrew](https://brew.sh)來安裝所有相依性：
...
1. `cmake`在包含所產生程式碼的資料夾底下，建立名為的資料夾，然後流覽至該資料夾。
...
1. 程式碼產生器會使用您設定的先前設定，並重新產生程式碼。 不過，它不會覆寫可能包含使用者程式碼（例如和）的檔案 `main.c` `{project_name}_impl.c` 。

> [!NOTE]
> 如果您在介面檔案中更新 URN 識別碼，則會將它視為新的介面。 當您重新產生程式碼時，程式碼產生器會產生介面的程式碼，但不會覆寫檔案中的原始檔案 `{project_name}_impl.c` 。
