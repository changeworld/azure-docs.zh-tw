...
當您使用完整功能變數名稱（FQDN）連接到私人連結資源做為連接字串的一部分時，請務必正確地設定您的 DNS 設定，以解析為配置的私人 IP 位址。 現有的 Microsoft Azure 服務在透過公用端點連線時，可能已經有 DNS 設定可供使用。 必須覆寫此設定，才能使用您的私人端點來連線。 
...
| Azure SQL Database （Microsoft .Sql/伺服器）/SQL Server | privatelink.database.windows.net | database.windows.net |
...
| Azure Web Apps （Microsoft Web/sites）/網站 | privatelink.azurewebsites.net | azurewebsites.net |
