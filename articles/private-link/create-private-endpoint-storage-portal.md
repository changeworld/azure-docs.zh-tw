...
description: 瞭解如何使用專用終結點將專用連線到 Azure 中的存儲帳戶。
...
在此快速入門中，您將學習如何在 Azure 虛擬網路上創建 VM，該虛擬網路是使用 Azure 門戶使用專用終結點的存儲帳戶。 然後，您可以安全地從 VM 訪問存儲帳戶。
...
## <a name="create-your-private-endpoint"></a>創建專用終結點
在本節中，您將使用專用終結點創建專用存儲帳戶。 

1. 在 Azure 門戶中螢幕的左上角，選擇 **"創建資源** > **存儲** > **存儲帳戶**"。

1. 在 **"創建存儲帳戶 - 基礎知識"中**，輸入或選擇此資訊：
    ...
    | 資源群組 | 選擇**我的資源組**。 您已在上一節中建立此項目。|
    | **實例詳細資訊** |  |
    | 儲存體帳戶名稱  | 輸入*我的存儲帳戶*。 如果此名稱已被使用，請建立唯一名稱。 |
    | 區域 | 選取 [WestCentralUS]****。 |
    | 效能| 保留預設值 [標準]****。 |
    | 帳戶類型 | 保留預設**存儲（通用 v2）。** |
    | 複寫 | 選擇**讀取存取異地冗余存儲 （RA-GRS）。** |
    |||
  
3. 選擇**下一個：網路**。
4. 在**創建存儲帳戶 - 網路**，連接方法中，選擇**專用終結點**。
5. 在 **"創建存儲帳戶 - 網路**"中，選擇 **"添加專用終結點**"。 
6. 在 **"創建專用終結點**"中，輸入或選擇此資訊：
    ...
    | 私人 DNS 區域  | 將預設 **（新建） privatelink.blob.core.windows.net**。 |
    |||
7. 選取 [確定]****。 
8. 選擇 **"審閱" = 創建**。 您會移至 [檢閱 + 建立]**** 頁面，其中 Azure 會驗證您的設定。 
9. 當您看到**驗證傳遞**的消息時，選擇 **"創建**"。 
10. 流覽到您剛剛創建的存儲帳戶資源。
11. 從左側內容功能表中選擇 **"便捷鍵**"。
12. 選擇鍵1的連接字串上的 **"複製**"。
...
## <a name="access-storage-account-privately-from-the-vm"></a>從 VM 私下訪問存儲帳戶

在本節中，您將使用專用終結點私下連接到存儲帳戶。

1. 在 myVm** 的遠端桌面中，開啟 PowerShell。
2. 輸入`nslookup mystorageaccount.blob.core.windows.net`您將收到類似于此消息的消息：
    ```azurepowershell
    Server:  UnKnown
...
6. 選擇 **"使用連接字串**"。
7. 選取 [下一步]****。
8. 通過粘貼以前複製的資訊來輸入連接字串。
9. 選取 [下一步]****。
10. 選取 [連接]****。
11. 從我的存儲帳戶流覽 Blob 容器 
12. （可選）創建資料夾和/或將檔上載到*我的存儲帳戶*。 
13. 關閉遠端桌面連線到*myVM。* 

訪問存儲帳戶的其他選項：
- Microsoft Azure 存儲資源管理器是 Microsoft 的一個獨立免費應用，使您能夠在 Windows、macOS 和 Linux 上直觀地處理 Azure 存儲資料。 您可以安裝應用程式以私下流覽存儲帳戶內容。 
 
- AzCopy 實用程式是 Azure 存儲高性能可腳本資料傳輸的另一個選項。 請使用 AzCopy 在 Blob、檔案和表格儲存體之間傳輸資料。 


## <a name="clean-up-resources"></a>清除資源 
使用專用終結點、存儲帳戶和 VM 完成後，請刪除資源組及其包含的所有資源： 
