---
title: '使用 Azure 私人端點將私人連線至 web 應用程式 (預覽版) '
description: 本文說明如何使用 Azure 私人端點 (預覽版) 私下連線至 web 應用程式。
author: ericgre
...
ms.locfileid: "90054529"
---
# <a name="connect-privately-to-a-web-app-by-using-azure-private-endpoint-preview"></a>使用 Azure 私人端點將私人連線至 web 應用程式 (預覽版) 

Azure 私人端點 (預覽版) 是 Azure Private Link 的基本組建區塊。 使用私人端點時，您可以私下連線至 web 應用程式。 在本文中，您將瞭解如何使用私人端點部署 web 應用程式，然後從 (VM) 的虛擬機器連線至 web 應用程式。

如需詳細資訊，請參閱 [使用 Azure web 應用程式的私人端點][privateendpointwebapp]。

> [!Note]
> 私人端點 (預覽) 適用于 >premiumv2 層 Windows web 應用程式、Linux web 應用程式，以及 Azure Functions Premium 方案 (有時稱為「彈性高階方案) 」的公用區域。 
...
1. 在 web 應用程式內容的 [ **設定**] 底下，選取 [ **網路**]，然後在 [私人端點連線] ** (預覽) **下，選取 [ **設定您的私人端點**連線]。

   > [!div class="mx-imgBorder"]
   > ![Web 應用程式網路功能窗格上 [設定您的私人端點連線] 連結的螢幕擷取畫面。][7]

1. 在 [ **私人端點連線] (預覽) ** wizard] 中，選取 [ **新增**]。

   > [!div class="mx-imgBorder"]
   > ![[私人端點連線] (預覽) ] wizard 中 [新增] 按鈕的螢幕擷取畫面。][8]

1. 在 [ **訂**用帳戶]、[ **虛擬網路**] 和 [ **子網** ] 下拉式清單中選取正確的資訊，然後選取 **[確定]**。

   > [!div class="mx-imgBorder"]
   > ![[新增私人端點 (預覽) ] 窗格的螢幕擷取畫面。][9]
...
1. 在 [ **myVM** ] 窗格中，確認 web 應用程式無法透過公用 IP 存取。 若要這樣做，請開啟瀏覽器並貼上 web 應用程式名稱。 頁面應該會顯示「錯誤 403-禁止」訊息。
   ...
   > [!Important]
   > 因為這項功能處於預覽狀態，所以您需要手動管理功能變數名稱服務 (DNS) 專案。
...
若要深入瞭解私人端點 (預覽版) ，請參閱 [什麼是 Azure 私人端點？][privateendpoint]。
