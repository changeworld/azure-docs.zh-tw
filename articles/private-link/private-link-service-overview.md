...
私人連結服務可讓您透過「可見度」設定來控制服務的公開。 您可以將服務設為私用，以從您擁有的不同 Vnet 取用（僅適用于 RBAC 許可權）、限制暴露于您信任的一組有限訂用帳戶，或將其設為公開，讓所有 Azure 訂用帳戶都可以要求私人連結服務上的連接。 您的可見度設定會決定取用者是否可以連接到您的服務。 
