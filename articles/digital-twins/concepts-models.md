...
Azure 數位 Twins 的模型是使用數位 Twins 定義語言（DTDL）來定義。 DTDL 是以 JSON-LD 為基礎，而且與程式設計語言無關。 DTDL 不是 Azure 數位 Twins 專屬的，但也可用來代表其他 IoT 服務中的裝置資料，例如[iot 隨插即用](../iot-pnp/overview-iot-plug-and-play.md)。 Azure 數位 Twins 使用 DTDL*第2版*。
...
如需一般 DTDL 的詳細資訊，請參閱 GitHub 中的規格檔：[數位 Twins 定義語言（DTDL）-第2版](https://github.com/Azure/opendigitaltwins-dtdl/blob/master/DTDL/v2/dtdlv2.md)。
...
有一個範例可用於驗證模型檔，以確保 DTDL 有效。 它是建置於 DTDL 剖析器程式庫上，並與語言無關。 請在此尋找： [DTDL 驗證程式範例](https://docs.microsoft.com/samples/azure-samples/dtdl-validator/dtdl-validator)。

或者，如需剖析器程式庫的詳細資訊，包括如何直接使用它的範例，請參閱[如何：剖析和驗證模型](how-to-use-parser.md)。
