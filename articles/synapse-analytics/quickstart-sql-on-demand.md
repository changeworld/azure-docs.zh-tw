...
建立您自己的資料庫以供示範之用。 這是您用來建立檢視的資料庫。 請在本文的範例查詢中使用此資料庫。
...
修改下列程式碼片段，以建立適用於 CSV、JSON 和 Parquet 容器的認證：
...
> [!IMPORTANT]
> 我們會將整個 JSON 檔案讀取為單一資料列/資料行，因此 FIELDTERMINATOR、FIELDQUOTE 和 ROWTERMINATOR 會設定為0x0b，因為我們不會在檔案中尋找此項目。
