---
title: 教學課程：使用 AutoML 訓練機器學習模型
description: 如何使用 AutoML 在 Azure Synapse 中訓練機器學習模型的教學課程。
...
ms.locfileid: "96463640"
---
# <a name="tutorial-code-free-machine-learning-model-training-in-azure-synapse-with-automl-preview"></a>教學課程：無需程式碼，使用 AutoML 在 Azure Synapse 中訓練機器學習模型 (預覽)

了解如何使用[在 Azure Machine Learning 中 AutoML](https://docs.microsoft.com/azure/machine-learning/concept-automated-ml) 訓練的新機器學習模型，輕鬆地擴充 Spark 資料表中的資料。  Synapse 中的使用者可以直接在 Azure Synapse 工作區中選取 Spark 資料表，作為用來在無需程式碼體驗中建立機器學習模型的訓練資料集。

在本教學課程中，您將了解如何：

> [!div class="checklist"]
> - 使用在 Azure ML 中使用自動化 ML 的 Azure Synapse Studio 中的無需程式碼體驗，將機器學習模型定型。 您所定型的模型類型取決於您嘗試解決的問題。
...
## <a name="launch-automl-wizard-to-train-a-model"></a>啟動 AutoML 精靈將模型定型

以滑鼠右鍵按一下在上一個步驟中建立的 Spark 資料表。 選取 [機器學習 -> 使用新模型擴充] 並開啟精靈。
![啟動 AutoML 精靈](media/tutorial-automl-wizard/tutorial-automl-wizard-00d.png)

組態面板隨即出現，系統會要求您提供組態詳細資料，以在 Azure Machine Learning 中建立 AutoML 實驗執行。 此執行會將多個模型定型，而成功執行的最佳模型將會在 Azure ML 模型登錄中註冊：

![設定執行步驟 1](media/tutorial-automl-wizard/tutorial-automl-wizard-configure-run-00a.png)

- **Azure ML 工作區**：建立 AutoML 實驗執行時，必須要有 Azure ML 工作區。 您也需要使用[連結服務](quickstart-integrate-azure-machine-learning.md)，將 Azure Synapse 工作區連結至 Azure ML 工作區。 一旦擁有所有的必要條件，就可以指定要用於此 AutoML 執行的 Azure ML 工作區。

- **實驗名稱**：指定實驗名稱。 當您提交 AutoML 執行時，必須提供一個實驗名稱。 執行的資訊會儲存在 Azure ML 工作區的該實驗底下。 此體驗預設會建立新的實驗，並產生建議的名稱，但您也可以提供現有實驗的名稱。

- **最佳模型**：從 AutoML 執行指定最佳模型的名稱。 最佳模型會使用此名稱，並在執行後將其自動儲存在 Azure ML 模型登錄中。 AutoML 執行會建立許多機器學習模型。 根據您將在稍後步驟中選取的主要度量，您可以比較這些模型並選出最佳模型。

- **目標資料行**：此為將接受訓練進行預測的模型。 選擇要預測的資料行。

- **Spark 集區**：要用於 AutoML 實驗執行的 Spark 集區。 系統會在您指定的集區上執行計算。
...
- **主要計量**：用來測量模型執行程度的計量。 這是用來比較 AutoML 執行中所建立之不同模型的計量，並判斷哪一個模型最適合執行。

- 訓練作業時間 (小時)：實驗執行和訓練模型的時間上限，以小時為單位。 請注意，您也可以提供小於 1 的值。 例如 `0.5` 。

- **並行反覆項目上限**：表示平行執行的最大反覆運算數目。

- **ONNX 模型相容性**：若已啟用，由 AutoML 定型的模型會轉換成 ONNX 格式。 如果要在 Azure Synapse SQL 集區中使用模型評分，此項目特別相關。
...
完成所有必要的組態之後，您就可以開始執行 AutoML。

有兩種方式可以開始在 Azure Synapse 中執行 AutoML。 如需無程式碼體驗，可以直接選擇 [建立執行]。 如果偏好使用程式碼，可以選取 [在筆記本中開啟]，這可讓您查看建立執行和執行筆記本的程式碼。

### <a name="create-run-directly"></a>直接建立執行

按一下 [開始執行]，直接開始執行 AutoML。 系統將會出現通知，指出正在啟動 AutoML 執行。

成功啟動 AutoML 執行後，您會看到另一個成功的通知。 您也可以按一下 [通知] 按鈕，檢查執行提交的狀態。
按一下成功通知中的連結查看 Azure ML。
...
這也讓您有機會將其他設定新增至 AutoML 執行。

![開啟筆記本](media/tutorial-automl-wizard/tutorial-automl-wizard-configure-run-00e.png)

成功提交筆記本的執行之後，在筆記本輸出的 Azure ML 工作區中將會有實驗執行的連結。 您可以按一下連結，以在 Azure ML 中監視您的 AutoML 執行。
