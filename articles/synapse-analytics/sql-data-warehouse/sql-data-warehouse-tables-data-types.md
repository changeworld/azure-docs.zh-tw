...
如果您要從另一個 SQL 資料庫遷移您的資料庫，您可能會在 SQL 集區中找到不支援的資料類型。 使用下列查詢，在您現有的 SQL 架構中探索不支援的資料類型：
...
下列清單顯示 SQL 集區不支援的資料類型，並為不支援的資料類型提供有用的替代方案。
