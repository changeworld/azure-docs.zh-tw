...
* 已預先載入 AdventureWorksDW 範例資料的 SQL 集區。 若要進行布建，請參閱[建立 SQL 集](create-data-warehouse-portal.md)區並選擇載入範例資料。 如果您已經有資料倉儲但沒有範例資料，您可以[手動載入範例資料](load-data-from-azure-blob-storage-using-polybase.md)。
...
我們將會以 80-20 的比例分割資料：80% 訓練機器學習模型以及 20% 測試模型。 我們會針對此二元分類問題使用「雙類別」演算法。
