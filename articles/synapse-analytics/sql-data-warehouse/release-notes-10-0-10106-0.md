...
|**使用複製命令進行 Parquet 檔案的自動架構偵測（預覽）**|[COPY 命令](https://docs.microsoft.com/sql/t-sql/statements/copy-into-transact-sql?view=azure-sqldw-latest)現在支援在載入 Parquet 檔案時進行自動結構描述偵測。 此命令會自動偵測 Parquet 檔案結構描述，並且在載入之前建立資料表。 請前往下列電子郵件通訊群組清單，以啟用此功能： sqldwcopypreview@service.microsoft.com 。 |
|**使用複製命令載入複雜的 Parquet 資料類型（預覽）**|[COPY 命令](https://docs.microsoft.com/sql/t-sql/statements/copy-into-transact-sql?view=azure-sqldw-latest)現在支援載入複雜的 Parquet 類型。 您可以將例如「地圖」和「清單」的複雜類型載入至字串資料行。  請前往下列電子郵件通訊群組清單，以啟用此功能： sqldwcopypreview@service.microsoft.com 。 |
|**使用 COPY 命令進行 Parquet 檔案的自動壓縮偵測**|[COPY 命令](https://docs.microsoft.com/sql/t-sql/statements/copy-into-transact-sql?view=azure-sqldw-latest)現在支援 Parquet 檔案的壓縮方法自動偵測。 請前往下列電子郵件通訊群組清單，以啟用此功能： sqldwcopypreview@service.microsoft.com 。|
