...
- [常見錯誤訊息](sql-data-warehouse-troubleshoot-connectivity.md#common-error-messages)

## <a name="check-service-availability"></a>檢查服務可用性

檢查該服務是否可用。 在 Azure 門戶中，轉到嘗試連接的 SQL 分析資料庫。 在左側 TOC 面板中，按一下 **"診斷並解決問題**"。

![選擇資源運行狀況](./media/sql-data-warehouse-troubleshoot-connectivity/diagnostics-link.png)

SQL 分析的狀態將在此處顯示。 如果服務未顯示為 **"可用"，** 請檢查其他步驟。

![服務可用](./media/sql-data-warehouse-troubleshoot-connectivity/resource-health.png)

如果"資源"運行狀況顯示 SQL Analytics 實例已暫停或縮放，請按照該指南繼續實例。
...
檢查門戶以查看 SQL 分析實例是否已暫停或縮放。

![服務暫停](./media/sql-data-warehouse-troubleshoot-connectivity/overview-paused.png)

如果您看到服務已暫停或擴展，請檢查服務不在維護計畫期間。 在 SQL 分析*概述*的門戶上，您將看到已選擇維護計畫。

![概述維護計畫](./media/sql-data-warehouse-troubleshoot-connectivity/overview-maintance-schedule.png)

否則，請諮詢 IT 管理員以驗證此維護是否不是計畫事件。 要恢復 SQL 分析實例，請按照[此處](https://docs.microsoft.com/azure/sql-data-warehouse/pause-and-resume-compute-portal#resume-compute)概述的步驟操作。

## <a name="check-your-firewall-settings"></a>檢查防火牆設定

SQL 分析資料庫通過埠 1433 進行通信。如果您嘗試從公司網路內進行連線，您網路的防火牆可能不允許透過連接埠 1433 的輸出流量。 在此情況下，除非 IT 部門開啟連接埠 1433，否則您無法連線至 Azure SQL Database 伺服器。 有關防火牆配置的其他資訊，請參閱[此處](https://docs.microsoft.com/azure/sql-database/sql-database-firewall-configure#create-and-manage-ip-firewall-rules)。

## <a name="check-your-vnetservice-endpoint-settings"></a>檢查 VNet/服務端點設定

如果您收到錯誤 40914 和 40615，請參閱[此處的錯誤描述和解決方法](https://docs.microsoft.com/azure/sql-database/sql-database-vnet-service-endpoint-rule-overview?toc=/azure/sql-data-warehouse/toc.json#errors-40914-and-40615)。

## <a name="check-for-the-latest-drivers"></a>檢查最新的驅動程式

### <a name="software"></a>軟體

檢查以確保您使用最新的工具連接到 SQL 分析資料庫：
...
進行檢查以了解伺服器上是否因為有大量已排入佇列的要求而讓負載太高。 您可能需要擴展 SQL 分析實例以獲取其他資源。

## <a name="common-error-messages"></a>常見的錯誤訊息

錯誤 40914 和 40615，請參閱[此處的錯誤描述和解決方法](https://docs.microsoft.com/azure/sql-database/sql-database-vnet-service-endpoint-rule-overview?toc=/azure/sql-data-warehouse/toc.json#errors-40914-and-40615)。
