...
description: 在 Azure 突觸分析（以前的 SQL DW）中開發 SQL 分析解決方案的建議和最佳實踐。
...
ms.locfileid: "80350875"
---
# <a name="best-practices-for-sql-analytics-in-azure-synapse-analytics-formerly-sql-dw"></a>Azure 突觸分析（以前的 SQL DW）中 SQL 分析的最佳做法

本文是一系列最佳實踐，可説明您從[SQL Analytics](sql-data-warehouse-overview-what-is.md#sql-analytics-and-sql-pool-in-azure-synapse)部署中獲得最佳性能。  本文的目的是為您提供一些基本指導，並突出重要的重點領域。  每個部分都向您介紹一個概念，然後向您介紹更深入地介紹該概念的更詳細的文章。 主題的順序按重要性排列。 
...
Azure SQL 資料倉儲可以配置為自動檢測和創建列上的統計資訊。  優化程式創建的查詢計劃僅與可用統計資訊一樣好。  我們建議您為資料庫啟用AUTO_CREATE_STATISTICS，並每天或每次載入後更新統計資料，以確保查詢中使用的列統計資訊始終保持最新。 
...
## <a name="use-polybase-to-load-and-export-data-quickly"></a>使用 PolyBase 將資料快速載入及匯出
 ...
 SQL Analytics 支援通過多個工具（包括 Azure 資料工廠、PolyBase 和 BCP）載入和匯出資料。  若是小量的資料，效能不是那麼重要，任何工具都可以滿足您的需求。  不過，當您要載入或匯出大量資料，或者需要快速的效能時，PolyBase 是最佳選擇。  
 
 PolyBase 旨在利用 MPP（大規模並行處理）架構，並將載入和匯出資料比任何其他工具快。  您可使用 CTAS 或 INSERT INTO 來執行 PolyBase 載入。  **使用 CTAS 可以減少交易記錄，是載入資料最快的方法。** 
 
  Azure Data Factory 也支援 PolyBase 載入，並且可以達到與 CTAS 類似的效能。  PolyBase 支援各種不同的檔案格式，包括 Gzip 檔案。  **要最大化使用 gzip 文字檔時的輸送量，請將檔分解為 60 個或更多檔，以最大化負載的並行性。**   如需更快的總輸送量，請考慮同時載入資料。

另請參閱[載入資料][Load data]、[使用 PolyBase][Guide for using PolyBase]指南[、SQL 池載入模式和策略][Azure SQL Data Warehouse loading patterns and strategies]、[使用 Azure 資料工廠載入資料][Load Data with Azure Data Factory]、[使用 Azure 資料工廠移動資料][Move data with Azure Data Factory]、[創建外部檔案格式][CREATE EXTERNAL FILE FORMAT]、[創建表作為選擇 （CTAS）][Create table as select (CTAS)]

## <a name="load-then-query-external-tables"></a>載入並查詢外部資料表
雖然 Polybase (也稱為外部資料表) 可能是載入資料最快的方法，卻並非最適合查詢。 Polybase 表目前僅支援 Azure blob 檔和 Azure 資料湖存儲。 這些檔案沒有任何支援的計算資源。  

因此，SQL Analytics 無法卸載此工作，因此必須通過將其載入到 tempdb 來讀取整個檔才能讀取資料。  所以，如果您有數個將會查詢此資料的查詢，最好能一次載入此資料，並讓查詢使用本機資料表。
...
雖然分區資料可以通過分區切換或通過消除分區來優化掃描來有效地維護資料，但分區過多可能會減慢查詢速度。  通常，高細微性分區策略（在 SQL Server 中可能運行良好）在 SQL 分析中可能不起作用。  

如果每個資料分割的資料列少於 1 百萬，太多個資料分割也會減少叢集資料行存放區索引的效率。  請記住，在後臺，SQL Analytics 會將您的資料分區到 60 個資料庫中，因此，如果您創建一個包含 100 個分區的表，這實際上會導致 6000 個分區。  
...
例如，如果您有一個"插入"，如果可能，您需要 1 小時，請將 INSERT 分成四個部分，每個部分將在 15 分鐘內運行。  利用特殊的最低限度記錄案例，像是 CTAS、TRUNCATE、DROP TABLE 或 INSERT 空資料表，來降低回復的風險。  
...
對於未分區的表，請考慮使用 CTAS 將要保存在表中的資料寫入，而不是使用 DELETE。  如果 CTAS 需要相同的時間，則運行該操作更安全，因為它具有最少的事務日誌記錄，如果需要，可以快速取消。
...
臨時登陸資料時，您可能會發現使用堆表會使整個進程更快。  如果您只要在執行其他轉換之前暫存載入的資料，則將資料表載入堆積資料表遠快於將資料載入叢集資料行存放區資料表。  
...
群集列存儲索引是將資料存儲在 SQL Analytics 中的最有效方法之一。  預設情況下，SQL 分析中的表創建為群集列存儲。  為了讓資料行存放區資料表的查詢獲得最佳效能，良好的區段品質很重要。  
...
由於高品質的列存儲段很重要，因此最好使用大中型資源類中的使用者 IT 來載入資料。 使用較低的[資料倉儲單位](what-is-a-data-warehouse-unit-dwu-cdwu.md)，表示您想要將更大型的資源類別指派給正在載入的使用者。

由於列存儲表通常不會將資料推送到壓縮列存儲段，直到每個表有 100 多萬行，並且每個 SQL Analytics 表被分區為 60 個表，因此，根據經驗，列存儲表不會使查詢受益，除非該表有 6000 多萬行。  小於 6 千萬列的資料表使用資料行存放區索引似乎不太合理，  但也無傷大雅。  
...
我們會主動監看這個論壇，以確保您的問題有其他使用者或是我們回答。  如果您喜歡在堆疊溢位上提問，我們也有一個 Azure[突觸堆疊溢位論壇][Azure SQL Data Warehouse Stack Overflow Forum]。

最後，請使用 Azure[突觸回饋][Azure SQL Data Warehouse Feedback]頁發出功能請求。  加入您的要求或票選其他要求確實可協助我們決定功能的優先順序。
