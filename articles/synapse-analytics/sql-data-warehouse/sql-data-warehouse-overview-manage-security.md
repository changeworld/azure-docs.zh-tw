...
本文將逐步引導您瞭解保護 Synapse SQL 集區的基本概念。 尤其是，本文會讓您開始使用資源，以限制存取、保護資料，以及監視使用 SQL 集區布建之資料庫上的活動。
...
最佳做法是，您應該盡可能限制可透過伺服器層級防火牆所允許的 IP 位址範圍。  若要從本機電腦存取 SQL 集區，請確定您的網路和本機電腦上的防火牆允許 TCP 埠1433上的連出通訊。  
...
SQL 集區的連接預設會加密。  停用加密的修改連線設定會被忽略。

## <a name="authentication"></a>驗證

「驗證」是指連線到資料庫時如何證明身分識別。 SQL 集區目前支援使用使用者名稱和密碼 SQL Server 驗證，以及使用 Azure Active Directory。
...
然後，使用您的伺服器管理員登入連接到您的 **SQL 集區資料庫** ，並根據您所建立的伺服器登入建立資料庫使用者。
