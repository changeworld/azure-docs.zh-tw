...
| 問題                                                        | 解決方案                                                   |
| :----------------------------------------------------------- | :----------------------------------------------------------- |
| 使用者 'NT AUTHORITY\ANONYMOUS LOGON' 登入失敗。 (Microsoft SQL Server，錯誤：18456) | 當 AAD 使用者嘗試連線到主要資料庫，但主要資料庫中沒有使用者時，就會發生此錯誤。  要更正此問題，請指定要在連接時連接到的 SQL 池，或者將使用者添加到主資料庫。  有關詳細資訊，請參閱[安全概述](sql-data-warehouse-overview-manage-security.md)文章。 |
| 伺服器主體 "MyUserName" 在目前的資訊安全內容下無法存取「主要」資料庫。 無法開啟使用者預設資料庫。 登入失敗。 使用者 'MyUserName' 登入失敗。 (Microsoft SQL Server，錯誤：916) | 當 AAD 使用者嘗試連線到主要資料庫，但主要資料庫中沒有使用者時，就會發生此錯誤。  要更正此問題，請指定要在連接時連接到的 SQL 池，或者將使用者添加到主資料庫。  有關詳細資訊，請參閱[安全概述](sql-data-warehouse-overview-manage-security.md)文章。 |
...
| 無法與工具或驅動程式連線                           | SQL Analytics 建議使用[SSMS](/sql/ssms/download-sql-server-management-studio-ssms?view=sql-server-ver15) [、SSDT 用於視覺化工作室](sql-data-warehouse-install-visual-studio.md)，或[使用 sqlcmd](sql-data-warehouse-get-started-connect-sqlcmd.md)來查詢您的資料。 有關驅動程式和連接到 Azure 突觸的詳細資訊，請參閱 Azure[突觸的驅動程式](sql-data-warehouse-connection-strings.md)和[連接到 Azure 突觸](sql-data-warehouse-connect-overview.md)的文章。 |

## <a name="tools"></a>工具
| 問題                                                        | 解決方案                                                   |
| :----------------------------------------------------------- | :----------------------------------------------------------- |
| Visual Studio 物件總管中遺漏 AAD 使用者           | 這是已知的問題。  解決方法是在 [sys.database_principals](https://docs.microsoft.com/sql/relational-databases/system-catalog-views/sys-database-principals-transact-sql?view=sql-server-ver15) 中檢視使用者。  請參閱[對 Azure Synaps 的身份驗證](sql-data-warehouse-authentication.md)，瞭解有關將 Azure 活動目錄與 SQL 分析一起使用的更多資訊。 |
| 手動編寫腳本、使用腳本嚮導或通過 SSMS 進行連線速度緩慢、無法回應或建置錯誤 | 請確定已在主要資料庫中建立使用者。 在腳本選項中，還要確保引擎版本設置為"Microsoft Azure SQL 資料倉儲版本"，引擎類型為"Microsoft Azure SQL 資料庫"。 |
| 無法在 SSMS 中產生指令碼                               | 如果選項"為從屬物件生成腳本"選項設置為"True"，則為 SQL Analytics 生成腳本將失敗。 作為解決方法，使用者必須手動轉到**工具 -> 選項 ->SQL Server 物件資源管理器 ->生成從屬選項的腳本，並將其設置為 false** |

## <a name="performance"></a>效能
| 問題                                                        | 解決方案                                                   |
| :----------------------------------------------------------- | :----------------------------------------------------------- |
| 查詢效能疑難排解                            | 如果您正試著針對特定查詢進行疑難排解，請從[了解如何監視查詢](sql-data-warehouse-manage-monitor.md#monitor-query-execution)開始。 |
| TempDB 空間問題 | [監視 TempDB](sql-data-warehouse-manage-monitor.md#monitor-tempdb)空間使用方式。  臨時資料庫空間不足的常見原因是：<br>- 分配給查詢的資源不足，導致資料溢出到 TempDB。  請參閱[工作負載管理](resource-classes-for-workload-management.md) <br>- 統計資訊丟失或過期，導致資料過度移動。  有關如何創建統計資訊的詳細資訊，請參閱[維護表統計資訊](sql-data-warehouse-tables-statistics.md)<br>- TempDB 空間按服務等級分配。  [將 SQL 池縮放](sql-data-warehouse-manage-compute-overview.md#scaling-compute)到更高的 DWU 設置會分配更多的 TempDB 空間。|
...
| 如何實作最佳作法                              | 開始學習如何提高查詢性能的最佳地點是 SQL Analytics[最佳實踐](sql-data-warehouse-best-practices.md)一文。 |
