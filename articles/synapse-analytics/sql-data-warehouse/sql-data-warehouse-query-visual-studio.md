...
在短短幾分鐘內，使用 Visual Studio 在 Azure Synapse 中查詢專用的 SQL 集區。 這個方法會在 Visual Studio 2019 中使用 SQL Server Data Tools (SSDT) 擴充功能。 
...
* 完整的 SQL 伺服器名稱。 若要尋找此資訊，請參閱 [連接到專用的 SQL 集](../sql/connect-overview.md)區。
...
若要為 Azure Active Directory 驗證設定您的環境，請參閱 [驗證專用的 SQL 集](sql-data-warehouse-authentication.md)區。
