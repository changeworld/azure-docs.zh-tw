...
A：Azure Synapse 是適用於 BI、AI 和持續型智能的整合式資料平台。 其會透過單一平台連線各種分析執行階段 (例如 SQL 和 Apache Spark)，以提供統一的方式來執行下列動作：

- 保護您的分析資源 (包括網路)，以及管理集區、資料和開發成品的單一登入存取。
- 輕鬆地進行監視，並快速地對工作區活動在任何層級上發生的事件進行最佳化、反應和偵錯。
- 跨引擎管理您的中繼資料。 建立無伺服器 Apache Spark 集區資料表，這會在您的 Azure Synapse 資料庫中自動提供。
...
- 分析功能會透過專用 SQL 集區或無伺服器 SQL 集區 (預覽) 提供。
- 完全支援 Scala、Python、Spark、SQL 和 C# 的無伺服器 Apache Spark 集區 (預覽)
- 提供無程式碼巨量資料轉換體驗的資料流程
...
A：Azure Synapse 分析是 Azure SQL 資料倉儲進入分析平台的演進，其中包含作為資料倉儲解決方案的專用 SQL 集區。 此平台結合資料探索、擷取、轉換、準備和服務分析層。
...
### <a name="q-what-is-a-good-use-case-for-dedicated-sql-pool"></a>問：對專用 SQL 集區而言，何謂良好的使用案例

A：專用SQL 集區是您資料倉儲需求的核心。 對於資料倉儲的[價格/效能](https://azure.microsoft.com/services/sql-data-warehouse/compare/)來說，這是居於領導地位的解決方案。 專用 SQL 集區是領先業界的雲端資料倉儲解決方案，因為您可以：

- 處理大量且混合的各種工作負載，而不會影響效能，因為其可以高度並行，並且區隔工作負載
- 透過進階功能 (從網路安全性到精密存取皆涵蓋在內)，輕鬆保護您的資料
- 受益於各式各樣的生態系統

### <a name="q-what-is-a-good-use-case-for-serverless-apache-spark-pool-in-azure-synapse"></a>問：對 Azure Synapse 中無伺服器 Apache Spark 集區而言，何謂良好的使用案例

A：我們的第一個目標是提供絕佳的資料工程經驗，以透過批次或串流的方式轉換湖上的資料。 其可以緊密但簡單地與我們的資料協調流程整合，讓您開發工作可以簡單明瞭地執行。

Apache Spark 的另一個使用案例是讓資料科學家可以執行下列作業：

- 擷取特徵
- 探索資料
- 將模型定型

### <a name="q-what-is-a-good-use-case-for-serverless-sql-pool-in-azure-synapse"></a>問：對 Azure Synapse 中無伺服器 SQL 集區而言，何謂良好的使用案例

A：無伺服器 SQL 集區是針對資料湖資料進行的查詢服務。 其藉由提供熟悉的 T-SQL 語法來就地查詢資料 (不需要將資料複製或載入至特製存放區)，將您所有資料的存取權普及化。
...
### <a name="q-what-is-a-good-use-case-for-data-flow-in-azure-synapse"></a>問：對 Azure Synapse 中的資料流程而言，何謂良好的使用案例
...
A：端對端單一登入體驗是 Synapse Analytics 中的重要驗證程序。 透過完整 Azure AD 整合來管理和傳遞身分識別是必須的。
...
A：是，您可以使用協力廠商商務應用程式 (例如 Tableau 和 Power BI) 連線到專用 SQL 集區和無伺服器 SQL 集區。 Spark 支援 IntelliJ。

## <a name="next-steps"></a>後續步驟

- [建立工作區](quickstart-create-workspace.md)
- [使用 Synapse Studio](quickstart-synapse-studio.md)
- [建立專用 SQL 集區](quickstart-create-sql-pool-portal.md)
