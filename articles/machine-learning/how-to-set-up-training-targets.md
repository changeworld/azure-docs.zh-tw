...
定型時，通常在本機電腦上啟動，然後在不同的計算目標上執行該定型指令碼。 使用 Azure 機器學習,您可以在各種計算目標上運行文稿,而無需更改文稿。

您只需在**執行配置**中為每個計算目標定義環境。  然後，當您想要在不同的計算目標上執行定型實驗時，為該計算指定回合組態。 有關指定環境並將其綁定為執行設定的詳細資訊,請參閱[建立和管理用於培訓和部署的環境](how-to-use-environments.md)。

深入了解本文末尾的[提交實驗](#submit)。

## <a name="whats-an-estimator"></a>什麼是估算器?

為了方便使用常用框架進行模型培訓,Azure 機器學習 Python SDK 提供了一種替代的更高級抽象(估計器類)。  此類允許您輕鬆建構運行配置。 您可以建立和使用通用[估計器](https://docs.microsoft.com/python/api/azureml-train-core/azureml.train.estimator?view=azure-ml-py)提交使用您選擇的任何學習框架(如 scikit 學習)的培訓文稿。 我們建議使用評估器進行培訓,因為它會自動為您建構嵌嵌入的物件(如環境或 Run配置物件)。 如果希望對這些物件的創建方式有更多的控制,並指定為實驗運行安裝的程式包,請按照[以下步驟](#amlcompute)使用 Azure 機器學習計算上的 Run配置物件提交訓練實驗。
...
借助 ML 管道,您可以簡化、快速、可移植和重用來優化工作流。 使用 Azure 機器學習構建管道時,可以專注於專業知識、機器學習,而不是基礎結構和自動化。

ML 管道由多個**步驟**構造,這些步驟是管道中不同的計算單元。 每個步驟都可以獨立運行並使用隔離的計算資源。 這允許多個數據科學家在同一管道上同時工作,而不會對計算資源進行過重的徵稅,並且可以輕鬆地為每個步驟使用不同的計算類型/大小。

> [!TIP]
> 在訓練模型時,ML 管道可以使用運行配置或估計器。

雖然 ML 管道可以培訓模型,但它們也可以在培訓前準備數據,並在培訓後部署模型。 管道的主要用例之一是批處理評分。 有關詳細資訊,請參閱[管道:優化機器學習工作流](concept-ml-pipelines.md)。
...
1. **創建和附加**:無需建立或附加計算目標,將本地電腦用作訓練環境。  

1. **設定**:當您使用本地電腦作為計算目標時,訓練代碼將執行在[開發環境中](how-to-configure-environment.md)。  如果該環境已經有您需要的 Python 封裝，請使用使用者管理的環境。

 [!code-python[](~/aml-sdk-samples/ignore/doc-qa/how-to-set-up-training-targets/local.py?name=run_local)]

現在,您已經附加了計算並設定了運行,下一步是[提交訓練執行](#submit)。
...
1.  **創建、附加和配置**:基於執行的創建執行所有必要的步驟,使用運行配置創建、附加和配置計算目標。  
...
1. **建立並附加**:要在 Python 中創建持久的 Azure 機器學習計算資源,請指定**vm_size**和**max_nodes**屬性。 Azure Machine Learning 接著會對於其他屬性使用智慧型預設值。 未使用時，計算會自動向下調整為零節點。   視需要建立專用的虛擬機器以執行您的作業。
    
    * **vm_size**: Azure 機器學習計算建立的節點的 VM 系列。
    * **max_nodes**: 在 Azure 機器學習計算上執行作業時,要自動縮放的最大節點數。
...
1. **附加**: 要將現有虛擬機器附加到計算目標,必須為虛擬機器提供資源 ID、使用者名稱和密碼。 可以使用以下字串格式使用訂閱 ID、資源群組名稱和 VM 名稱建構 VM 的資源代碼:`/subscriptions/<subscription_id>/resourceGroups/<resource_group>/providers/Microsoft.Compute/virtualMachines/<vm_name>`
...
1. **配置**:為 DSVM 計算目標建立執行設定。 Docker 和 Conda 用來建立和設定 DSVM 上的定型環境。
...
1. **附加**: 要將 HDInsight 叢集附加到計算目標,必須為 HDInsight 群集提供資源 ID、使用者名稱和密碼。 可以使用以下字串格式使用訂閱 ID、資源群組名稱和 HDInsight 叢集名稱建構 HDInsight 叢集的資源代碼:`/subscriptions/<subscription_id>/resourceGroups/<resource_group>/providers/Microsoft.HDInsight/clusters/<cluster_name>`
...
Azure Batch 用於在雲端中高效地執行大規模並行和高性能計算 (HPC) 應用程式。 AzureBatchStep 可用於 Azure 機器學習管道,將作業提交到電腦的 Azure 批處理池。

要將 Azure 批次處理附加到計算目標,必須使用 Azure 機器學習 SDK 並提供以下資訊:

-    **Azure 批次處理計算名稱**:用於工作區內計算的友好名稱
-    **Azure 批次處理帳號名稱**:Azure 批次處理帳戶名稱
-    **資源組**:包含 Azure 批次處理帳戶的資源群組。

以下代碼展示如何將 Azure 批次附加到計算目標:
...
## <a name="set-up-in-azure-machine-learning-studio"></a>在 Azure 機器學習工作室中設定
...
1. 導覽[到 Azure 機器學習工作室](https://ml.azure.com)。
...
您可以使用 Azure 機器學習的[CLI 擴充名](reference-azure-machine-learning-cli.md)訪問與工作區關聯的計算目標。  您可以使用 CLI 執行下列操作：
...
## <a name="set-up-with-vs-code"></a>使用 VS 代碼設定

您可以使用 Azure 機器學習的[VS 代碼擴展來](tutorial-train-deploy-image-classification-model-vscode.md#configure-compute-targets)存取、創建和管理與工作區關聯的計算目標。

## <a name="submit-training-run-using-azure-machine-learning-sdk"></a><a id="submit"></a>使用 Azure 機器學習 SDK 提交培訓執行
...
> [!IMPORTANT]
> 提交訓練運行時,將創建包含訓練文本的目錄快照並將其發送到計算目標。 它還作為實驗的一部分存儲在工作區中。 如果更改檔並再次提交運行,則僅上傳已更改的檔。
>
...
* **source_directory**: 包含訓練文稿的來源目錄
* **文稿**: 確定訓練文稿
* **run_config**: 執行設定,進而定義訓練將發生的位置。
...
> [!TIP]
> 此示例預設僅使用計算目標的一個節點進行訓練。 要使用多個節點,可以將`node_count`運行配置設置為所需的節點數。 例如,以下代碼將用於定型的節點數設定為四個:
>
> ```python
...
* 提交 HyperDrive 執行以[進行超參數調優](how-to-tune-hyperparameters.md)。
* 通過[VS 代碼擴展](tutorial-train-deploy-image-classification-model-vscode.md#train-the-model)提交實驗。

有關詳細資訊,請參閱[文本配置](https://docs.microsoft.com/python/api/azureml-core/azureml.core.scriptrunconfig?view=azure-ml-py)和[運行配置](https://docs.microsoft.com/python/api/azureml-core/azureml.core.runconfiguration?view=azure-ml-py)文檔。

## <a name="create-run-configuration-and-submit-run-using-azure-machine-learning-cli"></a>使用 Azure 機器學習 CLI 建立執行設定並提交執行
...
建立執行設定的最簡單方法是導覽包含機器學習 Python 文稿的資料夾,並使用 CLI 命令

```azurecli
az ml folder attach
```

此命令創建一個子`.azureml`資料夾,其中包含針對不同計算目標的範本運行配置檔。 您可以複製和編輯這些檔以自定義設定,例如添加 Python 包或更改 Docker 設定。  

### <a name="structure-of-run-configuration-file"></a>執行設定檔結構

執行設定檔採用 YAML 格式包含以下部份
 * 要執行的文稿及其參數
 ...
 * 環境部分。 有關本節中欄位的詳細資訊,請參閱[創建和管理有關培訓和部署的環境](how-to-use-environments.md)。
   * 要指定要為執行安裝的 Python 套件,請建立[conda 環境檔](https://docs.conda.io/projects/conda/en/latest/user-guide/tasks/manage-environments.html#create-env-file-manually),並設定__conda 依賴檔案__欄位。
 * 執行歷史記錄詳細資訊以指定日誌檔資料夾,並啟用或禁用輸出集合並運行歷史記錄快照。
 * 特定於所選框架的配置詳細資訊。
 * 數據引用和數據存儲詳細資訊。
 * 特定於機器學習計算的配置詳細資訊,用於創建新群集。

有關完整執行設定架構,請參閱[範例 JSON 檔](https://github.com/microsoft/MLOps/blob/b4bdcf8c369d188e83f40be8b748b49821f71cf2/infra-as-code/runconfigschema.json)。
...
### <a name="hyperdrive-run"></a>超級磁碟機執行
...
將此檔與運行設定檔一起添加。 然後,使用以下功能提交 HyperDrive 執行:
```azurecli
az ml run submit-hyperdrive -e <experiment> -c <runconfig> --hyperdrive-configuration-name <hdconfig> my_train.py
```

請注意"超磁碟器配置"中的 runconfig 和*參數空間*中的*參數*部分。它們包含要傳遞給訓練腳本的命令列參數。 每次反覆運算運行時配置中的值保持不變,而 HyperDrive 配置中的範圍則反覆運算。 不要在兩個檔中指定相同的參數。

有關這些```az ml```CLI 指令和完整參數集的詳細資訊,請參考[文件](reference-azure-machine-learning-cli.md)。
