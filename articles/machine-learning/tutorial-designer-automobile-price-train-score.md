...
>本教學課程完成後，將可作為範例管線。
>
>若要尋找此範例，請移至工作區中的設計工具。 在 [**新增管線**] 區段中，選取 [**範例 1-回歸：汽車價格預測（基本）** ]。
...
既然您的管線已全部設定，您可以提交管線執行來訓練您的機器學習服務模型。 在設計工具中建立管線時，您可以在任何時間點提交管線執行。 您可以在執行時檢查您的工作，並如預期般驗證您的管線功能。
...
* **平均絕對錯誤（MAE）** ：絕對錯誤的平均值。 誤差是指預測值與實際值之間的差異。
* **根平均平方誤差（RMSE）** ：對測試資料集所做之預測的平方誤差平均值的平方根。
* **相對絕對誤差**：相對於實際值與所有實際值之平均值之間的絕對差異的絕對誤差平均值。
* **相對平方誤差**：相對於實際值與所有實際值之平均值之間的平方差異的平方誤差平均值。
* **決定係數**：也稱為 R 平方值，此統計計量表示模型適合資料的程度。
