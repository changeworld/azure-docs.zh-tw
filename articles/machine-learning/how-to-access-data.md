...
- Azure Machine Learning 工作區。
  ...
  [建立 Azure Machine Learning 工作區](how-to-manage-workspace.md)，或透過 Python SDK 使用現有的工作區。 
   ...
   ```
    ...
    當您建立工作區時，Azure blob 容器和 Azure 檔案共用會自動註冊為工作區的資料存放區。 這兩者分別會命名為 `workspaceblobstore` 和 `workspacefilestore`。 `workspaceblobstore`會用來儲存工作區成品和您的機器學習實驗記錄。 它也會設定為 **預設資料** 存放區，而且無法從工作區刪除。 `workspacefilestore`是用來儲存透過[計算實例](./concept-compute-instance.md#accessing-files)授權的筆記本和 R 腳本。
...
| 儲存體類型&nbsp; | 驗證類型&nbsp; | [Azure&nbsp;Machine&nbsp;Learning Studio](https://ml.azure.com/) | [Azure&nbsp;Machine&nbsp;Learning&nbsp; Python SDK](/python/api/overview/azure/ml/intro?preserve-view=true&view=azure-ml-py) |  [Azure&nbsp;Machine&nbsp;Learning CLI](reference-azure-machine-learning-cli.md) | [Azure&nbsp;Machine&nbsp;Learning&nbsp; Rest API](/rest/api/azureml/) | VS 程式碼
---|---|---|---|---|---|---
[Azure&nbsp;Blob&nbsp;儲存體](../storage/blobs/storage-blobs-overview.md)| 帳戶金鑰 <br> SAS 權杖 | ✓ | ✓ | ✓ |✓ |✓
[Azure&nbsp;檔案共用](../storage/files/storage-files-introduction.md)&nbsp;| 帳戶金鑰 <br> SAS 權杖 | ✓ | ✓ | ✓ |✓|✓
[Azure&nbsp;Data Lake&nbsp;Storage Gen&nbsp;1](../data-lake-store/index.yml)| 服務主體| ✓ | ✓ | ✓ |✓|
[Azure&nbsp;Data Lake&nbsp;Storage Gen&nbsp;2](../storage/blobs/data-lake-storage-introduction.md)| 服務主體| ✓ | ✓ | ✓ |✓|
[Azure&nbsp;SQL&nbsp;Database](../azure-sql/database/sql-database-paas-overview.md)| SQL 驗證 <br>服務主體| ✓ | ✓ | ✓ |✓|
[Azure&nbsp;PostgreSQL](../postgresql/overview.md) | SQL 驗證| ✓ | ✓ | ✓ |✓|
[適用於&nbsp;MySQL&nbsp;的 Azure&nbsp;資料庫](../mysql/overview.md) | SQL 驗證|  | ✓* | ✓* |✓*|
[Databricks&nbsp;檔案系統](/azure/databricks/data/databricks-file-system)&nbsp;| 不需要驗證 | | ✓** | ✓ ** |✓** |

\*只有管線[DataTransferStep](/python/api/azureml-pipeline-steps/azureml.pipeline.steps.datatransferstep?preserve-view=true&view=azure-ml-py)支援 MySQL<br />
\*\*只有管線[DatabricksStep](/python/api/azureml-pipeline-steps/azureml.pipeline.steps.databricks_step.databricksstep?preserve-view=true&view=azure-ml-py)支援 Databricks
...
針對 Azure blob 容器和 Azure Data Lake Gen 2 儲存體，請確定您的驗證認證具有 **儲存體 Blob 資料讀取器** 存取權。 深入瞭解 [儲存體 Blob 資料讀取器](../role-based-access-control/built-in-roles.md#storage-blob-data-reader)。 帳戶 SAS 權杖預設為沒有許可權。 針對資料讀取權限，您的驗證認證必須具有容器和物件的最小清單和讀取權限。 若要進行資料寫入存取，也需要 [寫入] 和 [新增] 許可權。
...
* [Azure Data Lake Storage Generation 2](#azure-data-lake-storage-generation-2)
 ...
 若要為其他支援的儲存體服務建立資料存放區，請參閱 [適用 `register_azure_*` 方法的參考檔](/python/api/azureml-core/azureml.core.datastore.datastore?preserve-view=true&view=azure-ml-py#&preserve-view=truemethods)。
...
若要使用您的服務主體，您必須 [註冊您的應用程式](../active-directory/develop/app-objects-and-service-principals.md) ，並將服務主體授與 **儲存體 Blob 資料讀取器** 存取權。 深入了解 [ADLS Gen 2 的存取控制設定](../storage/blobs/data-lake-storage-access-control.md)。 
...
若要取得在目前的工作區中註冊的特定資料存放區，請對 `Datastore` 類別使用 [`get()`](/python/api/azureml-core/azureml.core.datastore%28class%29?preserve-view=true&view=azure-ml-py#&preserve-view=trueget-workspace--datastore-name-) 靜態方法：

```Python
# Get a named datastore from the current workspace
datastore = Datastore.get(ws, datastore_name='your datastore name')
```
若要取得向指定工作區註冊的資料存放區清單，您可以對工作區物件使用 [`datastores`](/python/api/azureml-core/azureml.core.workspace%28class%29?preserve-view=true&view=azure-ml-py#&preserve-view=truedatastores) 屬性：
...
| 方法 | 資料存放區存取 | 描述 |
| ----- | :-----: | ----- |
| [批次預測](./tutorial-pipeline-batch-scoring-classification.md) | ✔ | 以非同步的方式對大量資料進行預測。 |
