...
description: 瞭解儲存輸入和輸出檔案的位置，以防止儲存體限制錯誤和實驗延遲。
...
* **將您的檔案儲存在 Azure Machine Learning [資料](/python/api/azureml-core/azureml.data?preserve-view=true&view=azure-ml-py)存放區中。** 這可防止實驗延遲問題，並具有從遠端計算目標存取資料的優點，這表示驗證和裝載是由 Azure Machine Learning 管理。 深入瞭解如何將資料存放區指定為來原始目錄，並將檔案上傳至您的資料存放區，以 [從您的資料存放區文章存取資料](how-to-access-data.md) 。
