...
這個範例會將包含 `Gryphon` 資料行中之文字的所有資料列放入第一個資料集 `Text`中。 其會將其他資料列放入**分割資料**的第二個輸出中。
...
這個範例會在 dataset 的第二個數據行內的任何位置尋找指定的字串。 位置在這裡是以索引值1來表示。 比對會區分大小寫。
