...
ms.locfileid: "90905198"
---
# <a name="resnet"></a>ResNet

本文描述如何在 Azure Machine Learning 設計工具中使用 **ResNet** 模組，以使用 ResNet 演算法建立影像分類模型。  

此分類演算法是受監督的學習方法，而且需要已加上標籤的資料集。 如需如何取得已加上標籤的影像目錄的詳細資訊，請參閱 [轉換成映射目錄](convert-to-image-directory.md) 模組。 您可以將模型和加上標籤的影像目錄提供為 [定型 Pytorch 模型](train-pytorch-model.md)的輸入，藉此定型模型。 然後，您可以使用定型的模型，利用 [計分影像模型](score-image-model.md)來預測新輸入範例的值。
