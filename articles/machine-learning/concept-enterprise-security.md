...
如果內建角色不符合您的需求，您可以建立自訂角色。 僅有工作區上的作業和 Machine Learning Compute 支援自訂角色。 自訂角色可以具有工作區以及該工作區中之計算資源的讀取、寫入或刪除權限。 您可以在特定工作區層級、特定的資源群組層級或特定的訂用帳戶層級上，讓角色可供使用。 如需詳細資訊，請參閱[管理 Azure Machine Learning 工作區中的使用者和角色](how-to-assign-roles.md)。
...
* 使用訂用帳戶的參與者權限，對 Machine Learning 應用程式 (在 [身分識別與存取管理] 中) 進行授權。
    ...
    ![在入口網站的 [身分識別與存取管理] 中，授權 [Azure Machine Learning 應用程式]](./media/concept-enterprise-security/authorize-azure-machine-learning.png)
