---
title: 使用 Azure AD 身分識別與您的 web 服務
titleSuffix: Azure Machine Learning
description: 在您 Azure Kubernetes Service 的 web 服務中使用 Azure AD 身分識別，以在評分期間存取雲端資源。
...
1. 遵循 Azure AD Pod 身分識別專案頁面中顯示的步驟，[在 Azure 上建立身分識別](https://azure.github.io/aad-pod-identity/docs/demo/standard_walkthrough/#2-create-an-identity-on-azure)。

1. 遵循 Azure AD Pod 身分識別專案] 頁面中所示的步驟來[部署 AzureIdentity](https://azure.github.io/aad-pod-identity/docs/demo/standard_walkthrough/#3-deploy-azureidentity) 。

1. 遵循 Azure AD Pod 身分識別專案] 頁面中所示的步驟來[部署 AzureIdentityBinding](https://azure.github.io/aad-pod-identity/docs/demo/standard_walkthrough/#5-deploy-azureidentitybinding) 。

1. 如果在上一個步驟中建立的 Azure 身分識別不在 AKS 叢集的相同節點資源群組中，請遵循 Azure AD Pod 身分識別專案] 頁面中顯示的 [角色指派](https://azure.github.io/aad-pod-identity/docs/getting-started/role-assignment/#user-assigned-identities-that-are-not-within-the-node-resource-group) 步驟。

## <a name="assign-azure-identity-to-web-service"></a>將 Azure 身分識別指派給 web 服務
...
藉由編輯部署規格，將 Azure 身分識別選取器標籤新增至您的部署。選取器值應該是您在 [部署 AzureIdentityBinding](https://azure.github.io/aad-pod-identity/docs/demo/standard_walkthrough/#5-deploy-azureidentitybinding)的步驟5中定義的值。
...
若要確認已正確新增標籤，請執行下列命令。 您也應該會看到新建立的 pod 的狀態。
...
## <a name="assign-roles-to-your-azure-identity"></a>將角色指派給您的 Azure 身分識別
...
## <a name="use-azure-identity-with-your-web-service"></a>使用 Azure 身分識別搭配您的 web 服務
