...
1. 選取您想要的資料存放區：
    ...
    - *檢查唯讀設定*。
    - *修改*。 變更驗證類型和認證。 支援的驗證類型包括帳戶金鑰和 SAS 權杖。
...
1. 在提示字元中：
    ...
    1. 從清單中選取 VM 大小。
    1. 選擇是否要啟用 SSH 存取。
        1. 如果您啟用 SSH 存取，您也必須提供公開 SSH 金鑰或包含金鑰的檔案。 如需詳細資訊，請參閱在 [Azure 上建立和使用 SSH 金鑰的指南](https://docs.microsoft.com/azure/virtual-machines/linux/mac-create-ssh-keys)。
