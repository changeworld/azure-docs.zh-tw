...
若要將此函 `CustomAddRows` 式公開為 Azure Machine Learning Studio （傳統）模組，必須建立 XML 定義檔以指定**自訂加入資料列**模組的外觀與行為。 
...
接下來，將包含 ' Customaddrows.zip '、' CustomAddRows.xml ' 和 ' Removedupnarows.r ' 的 zip 檔案上傳為自訂 R 模組。
