...
description: 使用本地 SQL Server 資料庫中的資料使用 Azure 機器學習工作室（經典）執行高級分析。
...
---
# <a name="perform-analytics-with-azure-machine-learning-studio-classic-using-an-on-premises-sql-server-database"></a>使用本地 SQL Server 資料庫使用 Azure 機器學習工作室（經典）執行分析

[!INCLUDE [Notebook deprecation notice](../../../includes/aml-studio-notebook-notice.md)]

使用內部部署資料的企業通常會想要針對他們的機器學習服務工作負載來利用雲端的範圍和靈活度。 但是他們又不想因為將內部部署資料移至雲端而中斷目前的商務程序和工作流程。 Azure 機器學習工作室（經典版）現在支援從本地 SQL Server 資料庫中讀取資料，然後使用此資料訓練和評分模型。 您不再需要手動複製和同步處理雲端與內部部署伺服器之間的資料。 相反，Azure 機器學習工作室中的**導入資料**模組（經典版）現在可以直接從本地 SQL Server 資料庫中讀取，以便進行培訓和評分作業。

本文概述了如何將本地 SQL 伺服器資料引入 Azure 機器學習工作室（經典）。 它假定您熟悉 Studio（經典）概念，如工作區、模組、資料集、實驗*等*。
...
要訪問 Azure 機器學習工作室（經典）中的本地 SQL Server 資料庫，需要下載並安裝資料工廠自託管集成運行時（以前稱為資料管理閘道）。 在機器學習工作室（經典版）中配置連接時，您有機會使用下面描述的**下載和註冊資料閘道**對話方塊下載並安裝集成運行時 （IR）。
...
* 如果主機休眠，IR 將不會回應資料請求。 因此，安裝 IR 之前，請先在電腦上設定適當的電源計劃。 如果電腦已設定為休眠，安裝 IR 時會顯示訊息。
...
* 一次只為一個工作區配置一個 IR。 目前，無法跨工作區共用 I。
* 您可以為單一工作區設定多個 IR。 例如，您可能希望在開發期間使用連接到測試資料來源的 IR，在準備操作時使用生產 IR。
* IR 不一定要在與資料來源相同的電腦上。 但是，愈接近資料來源，可縮短閘道連線到資料來源的時間。 建議在不同的電腦上安裝 IR 及裝載內部部署資料來源，這樣閘道和資料來源才不會爭搶資源。
* 如果電腦上已安裝了 IR，請為 Power BI 或 Azure 資料工廠方案提供服務，請在另一台電腦上安裝 Azure 機器學習工作室（經典）的單獨 IR。

  > [!NOTE]
  > 您無法在同一部電腦上執行 Data Factory 自我裝載整合執行階段和 Power BI Gateway。
  >
  >
* 您需要使用 Azure 機器學習工作室的資料工廠自託管集成運行時（經典），即使您對其他資料使用 Azure ExpressRoute 也是如此。 即使使用 ExpressRoute，您也應該將資料來源視為內部部署資料來源 (亦即在防火牆後面)。 使用 Data Factory 自我裝載整合執行階段建立 Machine Learning 與資料來源之間的連線。

您可以在 [Data Factory 中的整合執行階段](../../data-factory/concepts-integration-runtime.md)一文中，找到安裝必要條件、安裝步驟和疑難排解秘訣的詳細資訊。

## <a name="span-idusing-the-data-gateway-step-by-step-walk-classanchorspan-id_toc450838866-classanchorspanspaningress-data-from-your-on-premises-sql-server-database-into-azure-machine-learning"></a><span id="using-the-data-gateway-step-by-step-walk" class="anchor"><span id="_Toc450838866" class="anchor"></span></span>從本地 SQL 伺服器資料庫進入 Azure 機器學習
在本演練中，您將在 Azure 機器學習工作區中設置 Azure 資料工廠集成運行時，對其進行配置，然後從本地 SQL Server 資料庫中讀取資料。

> [!TIP]
> 在開始之前，請禁用瀏覽器的快顯視窗阻止程式`studio.azureml.net`。 如果您使用 Google Chrome 瀏覽器，請下載並安裝數個可在 Google Chrome 線上應用程式商店 [Click Once 應用程式擴充功能](https://chrome.google.com/webstore/search/clickonce?_category=extensions)上取得的外掛程式之一。
>
> [!NOTE]
...
1. 登錄到[Azure 機器學習工作室（經典），](https://studio.azureml.net/Home/)然後選擇要在其中工作的工作區。
...
12. 在 Microsoft 資料管理閘道組態管理員中，將切換到**證書**選項卡。此選項卡上指定的證書用於加密/解密您在門戶中指定的本地資料存儲的憑據。 此憑證是預設的憑證。 Microsoft 建議將此憑證變更為您自己在憑證管理系統中備份的憑證。 按一下 [變更] **** 改為使用您自己的憑證。

    ![變更閘道憑證](./media/use-data-from-an-on-premises-sql-server/data-gateway-configuration-manager-certificate.png)
13. (選擇性) 如果您想啟用詳細資訊記錄功能來為閘道問題進行疑難排解，請在 [Microsoft 資料管理閘道組態管理員] 中切換到 [診斷]**** 索引標籤，然後選取 [啟用詳細資訊記錄以進行疑難排解]**** 選項。 日誌記錄資訊可以在**應用程式和服務日誌** -&gt;**資料管理閘道**節點下的 Windows 事件檢視器中找到。 您也可以使用 [診斷] **** 索引標籤，使用閘道來測試與內部部署資料來源的連線。

    ![啟用詳細資訊記錄](./media/use-data-from-an-on-premises-sql-server/data-gateway-configuration-manager-verbose-logging.png)

這將完成 Azure 機器學習工作室（經典）中的閘道設置過程。
您現在可以開始使用內部部署資料。

您可以在 Studio 中為每個工作區創建和設置多個閘道（經典）。 例如，您可能想要有一個可在開發期間連接到測試資料來源的閘道器，以及另一個適用於生產資料來源的閘道器。 Azure 機器學習工作室（經典版）使您能夠根據企業環境靈活地設置多個閘道。 目前，您無法在工作區之間共用閘道，並且只能在一台電腦上安裝一個閘道。 如需詳細資訊，請參閱[利用資料管理閘道在內部部署來源和雲端之間移動資料](../../data-factory/tutorial-hybrid-copy-portal.md)。
...
1. 在機器學習工作室（經典版）中，選擇 **"實驗"** 選項卡，按一下左下角的 **"新"，** 然後選擇 **"空白實驗**"（或選擇幾個可用的示例實驗之一）。
2. 找出 **匯入模型** 模組，並將它拖曳到實驗畫布。
3. 按一下畫布下方的 [另存新檔] **** 。 為實驗名稱輸入"Azure 機器學習工作室（經典）本地 SQL 伺服器教程"，選擇工作區，然後按一下 **"確定**"核取記號。
...
7. 按一下 [使用者名稱和密碼]**** 下方的 [輸入值]****，然後輸入資料庫認證。 根據您內部部署 SQL Server 的設定方式而定，您可以使用 Windows 整合式驗證或 SQL Server 驗證。
   ...
   「需要值」訊息會變更為「已設定值」並具有綠色核取記號。 除非資料庫資訊或密碼變更，否則您只需輸入認證一次。 Azure 機器學習工作室（經典版）使用安裝閘道時提供的證書來加密雲中的憑據。 Azure 永遠不會在沒有加密的情況下儲存內部部署認證。

   ![匯入資料模組屬性](./media/use-data-from-an-on-premises-sql-server/import-data-properties-entered.png)
8. 按一下 [執行] **** 來執行實驗。

實驗完成運行後，可以通過按一下**導入資料**模組的輸出埠並選擇 **"視覺化**"來視覺化從資料庫導入的資料。
