...
description: 使用 Azure 機器學習工作室（經典）開發用於分析和評分客戶流失的集成模型的案例研究。
...
---
# <a name="analyze-customer-churn-using-azure-machine-learning-studio-classic"></a>使用 Azure 機器學習工作室分析客戶客戶流失（經典）

[!INCLUDE [Notebook deprecation notice](../../../includes/aml-studio-notebook-notice.md)]

## <a name="overview"></a>總覽
本文介紹了使用 Azure 機器學習工作室（經典）構建的客戶客戶客戶客戶流失分析專案的參考實現。 在本文中將討論關聯的一般模型，可全面性地解決產業客戶流失的問題。 對於以機器學習建立的模型，我們也衡量其正確性，同時評定進一步開發的方向。  

### <a name="acknowledgements"></a>通知
此實驗由微軟首席資料科學家謝爾蓋·伯傑和羅傑·巴爾加（前微軟 Azure 機器學習工作室產品經理）（經典）開發和測試。 Azure 文件小組高度認可其專業知識，並感謝他們分享這份白皮書。
...
## <a name="implementing-the-modeling-archetype-in-machine-learning-studio-classic"></a>在機器學習工作室中實現建模原型（經典）
根據說明的問題，實作一套整合的模型化和評分方法的最佳方式為何？ 在本節中，我們將演示如何使用 Azure 機器學習工作室（經典）來實現此目的。  

針對客戶流失來設計整體原型時，多重模型方法不可或缺。 即使是作法中的評分 (預測) 部分也應該為多重模型。  

下圖顯示了我們創建的原型，該原型在機器學習工作室（經典）中使用了四個評分演算法來預測流失率。 使用多重模型方法的理由不只是為了建立集成分類器來提高準確性，也是為了避免過度訓練，同時改善制式的特徵選擇。  

![描繪具有許多互連模組的複雜工作室（經典）工作區的螢幕截圖](./media/azure-ml-customer-churn-scenario/churn-3.png)

*圖 5：客戶流失模型建構方法的原型*  

以下各節提供了有關我們使用機器學習工作室（經典）實現的原型評分模型的更多詳細資訊。  
...
模型的特徵選擇取決於預測變數組的初步顯著計分，包含在利用隨機森林模組的程序中。 對於機器學習工作室（經典）中的實現，我們計算了代表性功能的平均值、中位數和範圍。 舉例來說，我們加上定性資料的總合，例如使用者活動的最小值和最大值。

我們也取得最近 6 個月的暫時資訊。 我們分析一年的資料後，發現即使存在統計顯著趨勢，對客戶流失的效應也會在六個月後遞減。  

最重要的一點是，整個過程（包括 ETL、功能選擇和建模）是在機器學習工作室（經典）中使用 Microsoft Azure 中的資料來源實現的。   
...
*圖 8：在機器學習工作室中創建模型（經典）*  

### <a name="scoring-methods"></a>評分方法
我們使用已標示的訓練資料集來對這四種模型評分。  

我們也提交評分資料集給使用 SAS Enterprise Miner 12 桌上型版本建立的比較模型。 我們測量了 SAS 模型和所有四個機器學習工作室（經典）模型的準確性。  

## <a name="results"></a>結果
在本節中，我們根據評分資料集，提出關於模型正確性的調查結果。  

### <a name="accuracy-and-precision-of-scoring"></a>評分的正確性和準確度
通常，Azure 機器學習工作室（經典）中的實現在精度上落後 SAS 約 10-15%（曲線下區域或 AUC）。  
...
* SAS 分類誤判率為 10-15%。
* 機器學習工作室（經典）錯誤分類率為 15-20%， 前 200-300 名流失者。  

在電信業中，只提供特別禮遇或其他特殊待遇給最可能流失的客戶是很重要的。 在這方面，機器學習工作室（經典）實現實現的結果與 SAS 模型相同。  
...
我們使用機器學習工作室（經典）模型和使用 SAS 企業礦工 12.1 的桌上出版創建的可比模型比較了資料評分的速度。  
...
機器學習工作室（經典）託管的模型在執行速度方面優於 SAS 15-25%，但準確性基本相當。  
...
但是，使用機器學習工作室（經典）進行自助服務分析的承諾是，按部門或部門分級的四類資訊成為機器學習有關流失的寶貴來源。  

Azure 機器學習工作室（經典版）中的另一個令人興奮的功能是能夠將自訂模組添加到已可用的預定義模組存儲庫中。 這項功能基本上讓人有機會針對垂直市場選取程式庫和建立範本。 它是 Azure 機器學習工作室（經典）在市場上的重要區別。  

我們希望未來繼續探討這個主題，特別是關於巨量資料分析。
  

## <a name="conclusion"></a>結論
本文說明一套實用的方法，採取通用的架構來處理客戶流失這個普遍的問題。 我們考慮了評分模型的原型，並使用 Azure 機器學習工作室（經典）實現了它。 最後，我們依據 SAS 中可比較的演算法，評估原型解決方案的正確性和表現。  

 

## <a name="references"></a>參考
[1] 預測分析：超出預測範圍，W. McKnight，資訊管理，2011 年 7 月/8 月，第 18-20 頁。  

[2] 維琪百科文章：[準確性和精度](https://en.wikipedia.org/wiki/Accuracy_and_precision)
