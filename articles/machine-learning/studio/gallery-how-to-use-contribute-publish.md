...
* **實驗**- 庫包含在 Azure 機器學習工作室（經典）中開發的各種實驗。 小至示範特定機器學習服務技術的快速概念證明實驗，大至用於解決複雜機器學習服務問題的完整開發解決方案，都在這些實驗的涵蓋範圍內。
* **教學課程**：此資源中提供了許多教學課程來引導您了解機器學習服務的技術和概念，或說明用來解決各種機器學習服務問題的進階方法。
* **集合**：集合可讓您將用於處理特定解決方案或概念的實驗、API 和其他資源庫項目群組在一起。
* **自訂模組**- 您可以將自訂模組下載到 Studio（經典）工作區中，以用於您自己的實驗。
...
您可以將**實驗**、**自訂模組**和**Jupyter 筆記本電腦**下載到您自己的機器學習工作室（經典）工作區中，用於開發您自己的解決方案。
...
1. 將滑鼠指向資源，然後選取 [在 Studio 中開啟]****。
    ![從機器學習工作室內部打開庫實驗（經典）](./media/gallery-how-to-use-contribute-publish/open-experiment-from-studio.png)

資源進入工作區後，您可以自訂和使用它，就像您在 Studio 中創建的任何內容一樣（經典）。
...
為了演示分析技術，或為其他人提供解決方案的入門，您可以貢獻您在 Studio 中開發**的實驗**（經典）。
...
1. 在資源庫中，輸入 [名稱]**** 和描述性 [標記]****。 醒目提示您所使用的技術或您正在解決的實際問題。 描述性實驗標題的一個示例是"二進位分類：Twitter 情緒分析"。
...
若有需要，您可以在您發佈到 Gallery 的實驗中變更工作流程 (模組、參數等等)。 在機器學習工作室（經典）中，對實驗進行任何更改，然後再次發佈。 系統會以您所做的變更來更新發佈的實驗。
...
**我在實驗中使用的資料集會發生什麼情況？資料集是否也在庫中發佈？**
...
**我有一個實驗，使用導入資料模組從 Azure HDInsight 或 SQL Server 中提取資料。它使用我的憑據來檢索資料。我可以發佈這種實驗嗎？如何確保我的憑據不會共用？**
