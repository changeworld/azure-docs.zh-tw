...
在此方法中，定型資料會儲存在 Azure blob 儲存體中。 Azure Data Factory 管線會從輸入 blob 容器中提取資料、進行轉換，並將資料儲存至輸出 blob 容器。 此容器可作為 Azure Machine Learning 服務的[資料存放區](concept-data.md)。 備妥資料之後，Data Factory 管線會叫用定型 Machine Learning 管線來定型模型。 在此特定範例中，資料轉換是由在 Azure Databricks 叢集上執行的 Python 筆記本進行。 
...
資料工程師會在 IDE 中的本機（例如[Visual Studio Code](https://code.visualstudio.com)），或直接在 Databricks 工作區中使用 Python 筆記本原始碼。 後者可讓您在開發環境上進行程式碼的偵錯工具。 在任何情況下，程式碼都會依照分支原則合併到存放庫。 強烈建議將程式碼儲存在檔案中`.py` ，而不是`.ipynb` Jupyter 筆記本格式。 它可以改善程式碼的可讀性，並在 CI 進程中啟用自動程式碼品質檢查。
