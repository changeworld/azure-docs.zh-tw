...
要查看自動機器學習實驗的示例，請參閱[教程：使用自動機器學習訓練分類模型](tutorial-auto-train-models.md)，或在[雲中使用自動機器學習培訓模型](how-to-auto-train-remote.md)。
...
如果您更喜歡沒有代碼體驗，還可以在 Azure[機器學習工作室中創建自動機器學習實驗](how-to-use-automated-ml-for-ml-models.md)。
...
[DNN 分類器](https://www.tensorflow.org/api_docs/python/tf/estimator/DNNClassifier)|[DNN 回歸器](https://www.tensorflow.org/api_docs/python/tf/estimator/DNNRegressor) | [DNN 回歸器](https://www.tensorflow.org/api_docs/python/tf/estimator/DNNRegressor)|
[DNN 線性分類器](https://www.tensorflow.org/api_docs/python/tf/estimator/LinearClassifier)|[線性回歸器](https://www.tensorflow.org/api_docs/python/tf/estimator/LinearRegressor)|[線性回歸器](https://www.tensorflow.org/api_docs/python/tf/estimator/LinearRegressor)
[貝氏機率分類](https://scikit-learn.org/stable/modules/naive_bayes.html#bernoulli-naive-bayes)|[快速線性回歸器](https://docs.microsoft.com/python/api/nimbusml/nimbusml.linear_model.fastlinearregressor?view=nimbusml-py-latest)|[自動 ARIMA](https://www.alkaline-ml.com/pmdarima/modules/generated/pmdarima.arima.auto_arima.html#pmdarima.arima.auto_arima)
[隨機梯度下降 (SGD)](https://scikit-learn.org/stable/modules/sgd.html#sgd)|[線上漸變下降回歸器](https://docs.microsoft.com/python/api/nimbusml/nimbusml.linear_model.onlinegradientdescentregressor?view=nimbusml-py-latest)|[先知](https://facebook.github.io/prophet/docs/quick_start.html)
...
使用構造`task`函數中的`AutoMLConfig`參數指定實驗類型。
...
自動化機器學習支援位於本機桌面或雲端中 (例如 Azure Blob 儲存體) 所包含的資料。 資料可以讀取到**熊貓資料框架**或**Azure 機器學習表格資料集**中。  [瞭解有關資料集的更多。](how-to-create-register-datasets.md)
...
對於遠端執行，必須從遠端計算訪問訓練資料。 SDK[`Datasets`](https://docs.microsoft.com/python/api/azureml-core/azureml.core.dataset.dataset?view=azure-ml-py)中的類將功能公開給：

* 輕鬆將資料從靜態檔或 URL 源傳輸到工作區
* 在雲計算資源上運行時，將資料提供給訓練腳本
...
可以直接在`AutoMLConfig`建構函式中指定單獨的訓練與驗證集。
...
### <a name="monte-carlo-cross-validation-repeated-random-sub-sampling"></a>蒙特卡羅交叉驗證（重複隨機子採樣）
...
如果不接受隨機拆分（通常是時間序列資料或不平衡資料），請使用自訂驗證資料集。 您可以指定自己的驗證資料集。 系統會根據指定的驗證資料集來評估模型，而不是隨機的資料集。
...
* 雲端中的遠端機器 – [Azure Machine Learning 受控計算](concept-compute-target.md#amlcompute)是一項受控服務，能夠在 Azure 虛擬機器叢集上定型機器學習模型。
  ...
  有關具有本地和遠端計算目標的筆記本的示例，請參閱此[GitHub 網站](https://github.com/Azure/MachineLearningNotebooks/tree/master/how-to-use-azureml/automated-machine-learning)。

* Azure 訂閱中的 Azure 資料磚塊群集。 您可以在此處找到更多詳細資訊 -[為自動 ML 設置 Azure 資料塊群集](how-to-configure-environment.md#azure-databricks)

  有關具有 Azure 資料磚塊的筆記本的示例，請參閱此[GitHub 網站](https://github.com/Azure/MachineLearningNotebooks/tree/master/how-to-use-azureml/azure-databricks/automl)。
...
1. 使用 AUC 加權作為主要指標的分類實驗，實驗超時分鐘設置為 30 分鐘，2 個交叉驗證折疊。
   ...
   ```
2. 下面是一個回歸實驗示例，該實驗設置為在 60 分鐘後結束，有五個驗證交叉折疊。
...
### <a name="data-featurization"></a>資料壯舉

在每個自動機器學習實驗中，您的資料[都會自動縮放和正常化](concept-automated-ml.md#preprocess)，以説明*某些*對不同比例的要素敏感的演算法。  但是，您還可以啟用其他任務化，例如缺少值歸因、編碼和轉換。 [詳細瞭解包含哪些壯舉](how-to-use-automated-ml-for-ml-models.md#featurization)。

配置實驗時，可以啟用高級設置`featurization`。 下表顯示了[`AutoMLConfig`類](https://docs.microsoft.com/python/api/azureml-train-automl/azureml.train.automl.automlconfig?view=azure-ml-py)中用於執行的已接受設置。

|技術化配置 | 描述 |
| ------------- | ------------- |
|`"featurization":`&nbsp;`'FeaturizationConfig'`| 指示應使用自訂的壯舉步驟。 [瞭解如何自訂壯舉](how-to-configure-auto-train.md#customize-feature-engineering)。|
|`"featurization": 'off'`| 指示不應自動執行壯舉步驟。|
|`"featurization": 'auto'`| 指示作為預處理的一部分，[將自動執行資料護欄和壯舉步驟](how-to-use-automated-ml-for-ml-models.md#advanced-featurization-options)。|

> [!NOTE]
> 自動化機器學習壯舉步驟（功能規格化、處理缺失資料、將文本轉換為數字等）成為基礎模型的一部分。 使用模型進行預測時，訓練期間應用的相同壯舉步驟將自動應用於輸入資料。

### <a name="time-series-forecasting"></a>時間序列預測
時間序列`forecasting`任務需要在設定物件中執行其他參數：

1. `time_column_name`：在訓練資料中定義包含有效時間序列的列名稱的必需參數。
1. `max_horizon`：根據訓練資料的循環定義要預測的時間長度。 例如，如果您有具有每日時粒的訓練資料，則可以定義您希望模型訓練天數的遠數。
1. `grain_column_names`：定義在訓練資料中包含單個時間序列資料的列的名稱。 例如，如果要按商店預測特定品牌的銷售，則可以將商店和品牌列定義為粒標。 將為每個顆粒/分組創建單獨的時間序列和預測。 

有關下面使用的設置的示例，請參閱[示例筆記本](https://github.com/Azure/MachineLearningNotebooks/blob/master/how-to-use-azureml/automated-machine-learning/forecasting-orange-juice-sales/auto-ml-forecasting-orange-juice-sales.ipynb)。
...
### <a name="ensemble-configuration"></a><a name="ensemble"></a>組合配置

預設情況下啟用組合模型，並在自動機器學習運行中顯示為最終運行反覆運算。 當前支援的合奏方法是投票和堆疊。 投票使用加權平均值實現為軟投票，堆疊實現使用兩層實現，其中第一層具有與投票組相同的模型，第二層模型用於查找第一層的模型。 如果您使用的是 ONNX 模型，**或者**啟用了模型解釋性，則將禁用堆疊，並且僅使用投票。

可以像在物件中一`kwargs``AutoMLConfig`樣提供多個預設參數來更改預設堆疊集合行為。

* `stack_meta_learner_type`：元學習者是訓練在單個異構模型輸出上的模型。 預設元學員`LogisticRegression`用於分類任務（或`LogisticRegressionCV`啟用了交叉驗證）和`ElasticNet`回歸/預測任務（或者如果`ElasticNetCV`啟用了交叉驗證）。 此參數可以是`LogisticRegression`以下字串之一： `LogisticRegressionCV`、 、 `LightGBMClassifier` `ElasticNet`、 `ElasticNetCV` `LightGBMRegressor`、 `LinearRegression`、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、 、
* `stack_meta_learner_train_percentage`：指定為培訓元學員保留的訓練集的比例（選擇訓練和驗證培訓類型時）。 預設值為 `0.2`。
* `stack_meta_learner_kwargs`：可選參數，以傳遞給元學習者的初始化器。 這些參數和參數類型反映相應模型建構函式的參數和參數類型，並轉發給模型建構函式。

以下代碼顯示了在物件中指定自訂集合行為的示例`AutoMLConfig`。
...
預設情況下啟用組合訓練，但可以使用`enable_voting_ensemble`和`enable_stack_ensemble`布林參數禁用它。
...
對於自動 ML，您將創建一`Experiment`個物件，該物件是用於運行實驗`Workspace`的命名物件。
...
請參閱預處理和[自動功能工程](concept-automated-ml.md#preprocess)的清單，這些工程發生在 時`"featurization": 'auto'`。

請思考此範例：
+ 有四個輸入功能：A（數位）、B（數位）、C（數位）、D（日期時間）
+ 數位要素 C 被丟棄，因為它是具有所有唯一值的 ID 列
+ 數值特徵 A 和 B 缺少值，因此按平均值進行歸因
+ DateTime 功能 D 被加工成 11 種不同的工程功能

在擬合模型的第一步使用這兩個 API 來瞭解更多。  請參閱[此示例筆記本](https://github.com/Azure/MachineLearningNotebooks/tree/master/how-to-use-azureml/automated-machine-learning/forecasting-energy-demand)。

+ API 1：`get_engineered_feature_names()`返回工程特徵名稱的清單。
...
+ API 2：`get_featurization_summary()`返回所有輸入功能的壯舉摘要。
...
|--|--|
|列用途更新|覆蓋指定列的功能類型。|
|變壓器參數更新 |更新指定變壓器的參數。 目前支援 Putputer（平均值、最頻繁的&中位數）和雜湊OneHotEncodeer。|
|刪除列 |要從壯舉中掉落的列。|
|塊變壓器| 用於壯舉過程的塊變壓器。|
...
### <a name="scalingnormalization-and-algorithm-with-hyperparameter-values"></a>具有超參數值的縮放/正常化和演算法：

要瞭解管道的縮放/正常化和演算法/超參數值，請使用 fitted_model。"步驟"。 [瞭解有關縮放/正常化的更多](concept-automated-ml.md#preprocess)。 以下是範例輸出：

```
[('RobustScaler', RobustScaler(copy=True, quantile_range=[10, 90], with_centering=True, with_scaling=True)), ('LogisticRegression', LogisticRegression(C=0.18420699693267145, class_weight='balanced', dual=False, fit_intercept=True, intercept_scaling=1, max_iter=100, multi_class='multinomial', n_jobs=1, penalty='l2', random_state=None, solver='newton-cg', tol=0.0001, verbose=0, warm_start=False))
```

要獲取更多詳細資訊，請使用[此示例筆記本](https://github.com/Azure/MachineLearningNotebooks/blob/master/how-to-use-azureml/automated-machine-learning/classification/auto-ml-classification.ipynb)中顯示的此説明器功能。
...
以下示例輸出適用于使用特定演算法的管道（本例中採用 RobustScalr 的 Logistic回歸）。
...
使用自動 ML 生成的模型都有包裝物件，這些物件可鏡像開源源類中的功能。 自動 ML 返回的大多數分類模型包裝物件實現`predict_proba()`函數，該函數接受要素的陣列狀或疏鬆陣列資料樣本（X 值），並返回每個樣本的 n 維陣列及其相應的類概率。

假設您使用上面相同的調用檢索了最佳運行和擬合模型，則可以直接從擬合型號調用`predict_proba()`，根據型號類型以適當的格式提供`X_test`樣本。
...
如果基礎模型不支援`predict_proba()`函數或格式不正確，則將引發特定于模型類的異常。 有關如何為不同的模型類型實現此函數的示例，請參閱[隨機林分類器](https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.RandomForestClassifier.html#sklearn.ensemble.RandomForestClassifier.predict_proba)和[XGBoost](https://xgboost.readthedocs.io/en/latest/python/python_api.html)參考文檔。
