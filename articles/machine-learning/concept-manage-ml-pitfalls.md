...
因為分類演算法通常會透過正確性來評估，所以檢查模型其正確性分數是識別該模型是否受到不平衡資料影響的好方法。 對於某些類別而言，模型的正確性非常高或非常低？
...
- **加權資料行**：自動化 ML 支援使用加權資料行作為輸入，讓資料中的資料列向上或向下加權，以此增加或減少類別的「重要性」。
...
- 使用效能指標來更適當地處理不平衡資料。 例如，F1 分數是精確度與重新叫用率的加權平均值。 精確度會測量分類器的精確程度 (低精確度表示具有大量誤判)，而重新叫用率則會測量分類器的完整性 (低重新叫用率表示具有大量漏判)。 
