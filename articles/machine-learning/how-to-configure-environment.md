...
description: 瞭解如何為 Azure 機器學習配置開發環境。 使用 Conda 環境、創建設定檔並配置您自己的基於雲的筆記本伺服器、Jupyter 筆記本、Azure 資料塊、IE、代碼編輯器和資料科學虛擬機器。
...
ms.locfileid: "79472421"
---
# <a name="configure-a-development-environment-for-azure-machine-learning"></a>設定 Azure Machine Learning 的開發環境
[!INCLUDE [applies-to-skus](../../includes/aml-applies-to-basic-enterprise-sku.md)]

在本文中，您將瞭解如何配置開發環境以使用 Azure 機器學習。 Azure 機器學習與平臺無關。 開發環境的唯一硬要求是 Python 3。 還建議使用孤立環境，如 Anaconda 或 Virtualenv。

下表顯示了本文涵蓋的每個開發環境，以及優缺點。

| 環境 | 優點 | 缺點 |
| --- | --- | --- |
| [基於雲的 Azure 機器學習計算實例（預覽）](#compute-instance) | 入門最簡單的方法。 整個 SDK 已安裝在工作區 VM 中，筆記本教程已預克隆並可供運行。 | 對開發環境和依賴項缺乏控制。 Linux VM 產生的額外成本（在不使用時可以停止 VM 以避免費用）。 請參閱[定價詳細資訊](https://azure.microsoft.com/pricing/details/virtual-machines/linux/)。 |
| [本機環境](#local) | 完全控制您的開發環境和依賴關係。 使用您選擇的任何生成工具、環境或 IDE 運行。 | 需要更長的時間才能開始。 必須安裝必要的 SDK 包，如果還沒有，則必須安裝環境。 |
| [Azure Databricks](#aml-databricks) | 非常適合在可擴展的 Apache Spark 平臺上運行大型密集型機器學習工作流。 | 用於實驗性機器學習或小規模實驗和工作流的超殺。 Azure 資料塊產生的額外費用。 請參閱[定價詳細資訊](https://azure.microsoft.com/pricing/details/databricks/)。 |
| [資料科學虛擬機器 （DSVM）](#dsvm) | 與基於雲的計算實例類似（已預先安裝 Python 和 SDK），但預先安裝了其他流行的資料科學和機器學習工具。 易於擴展，並與其他自訂工具和工作流結合。 | 與基於雲的計算實例相比，入門體驗較慢。 |

本文還提供了以下工具的其他使用提示：

* [朱皮特筆記本](#jupyter)：如果您已經在使用 Jupyter 筆記本，SDK 會安裝一些附加功能。

* [視覺化工作室代碼](#vscode)：如果使用視覺化工作室代碼[，Azure 機器學習擴展](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-ai)包括對 Python 的廣泛語言支援以及使使用 Azure 機器學習更加方便和高效的功能。

## <a name="prerequisites"></a>Prerequisites

Azure Machine Learning 工作區。 要創建工作區，請參閱[創建 Azure 機器學習工作區](how-to-manage-workspace.md)。 工作區是使用自己的[基於雲的筆記本伺服器](#compute-instance)[、DSVM](#dsvm)或[Azure 資料磚入門所需的全部功能](#aml-databricks)。

要為[本地電腦](#local)[、Jupyter 筆記本伺服器](#jupyter)或[視覺化工作室代碼](#vscode)安裝 SDK 環境，您還需要：
...
## <a name="your-own-cloud-based-compute-instance"></a><a id="compute-instance"></a>您自己的基於雲的計算實例

Azure 機器學習[計算實例（預覽）](concept-compute-instance.md)是一個安全的基於雲的 Azure 工作站，為數據科學家提供 Jupyter 筆記本電腦伺服器、JupyterLab 和完全準備好的 ML 環境。

計算實例無需安裝或配置任何內容。  隨時在 Azure 機器學習工作區中創建一個。 僅提供名稱並指定 Azure VM 類型。 現在試試本教程[：設置環境和工作區](tutorial-1st-experiment-sdk-setup.md)。
...
1. 要將 DSVM 配置為使用 Azure 機器學習工作區，請參閱[創建工作區設定檔](#workspace)部分。
...
1. 在新環境中運行以下命令，以啟用特定于環境的 IPython 內核。 這將確保在 Anaconda 環境中使用 Jupyter 筆記本時的預期內核和包導入行為：
...
1. 使用以下命令安裝包：
    ...
    此命令使用筆記本和其他`automl`功能安裝基本 Azure 機器學習 SDK。 額外的`automl`是大型安裝，如果您不打算運行自動機器學習實驗，則可以從支架中刪除。 額外的`automl`還包括 Azure 機器學習資料準備 SDK 預設情況下作為依賴項。
...
### <a name="jupyter-notebooks"></a><a id="jupyter"></a>Jupyter 筆記本
...
1. 克隆一組示例筆記本的[GitHub 存儲庫](https://aka.ms/aml-notebooks)。

    ```bash
    git clone https://github.com/Azure/MachineLearningNotebooks.git
    ```

1. 使用以下命令啟動 Jupyter 筆記本伺服器：

    ```bash
    jupyter notebook
    ```

1. 要驗證 Jupyter 筆記本是否可以使用 SDK，請**創建新筆記本，** 選擇**Python 3**作為內核，然後在筆記本儲存格中運行以下命令：
...
Visual Studio Code 是一款非常受歡迎的跨平臺代碼編輯器，通過[Visual Studio 市場](https://marketplace.visualstudio.com/vscode)提供的擴展支援一組廣泛的程式設計語言和工具。 [Azure 機器學習擴展](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-ai)將安裝用於在所有類型的 Python 環境（虛擬、Anaconda 等）進行編碼的[Python 擴展](https://marketplace.visualstudio.com/items?itemName=ms-python.python)。 此外，它還提供了使用 Azure 機器學習資源和運行 Azure 機器學習實驗的便捷功能，而無需離開 Visual Studio 代碼。
...
1. 瞭解如何將 Visual Studio 代碼用於任何類型的 Python 開發，請參閱[在 VSCode 中開始使用 Python。](https://code.visualstudio.com/docs/python/python-tutorial)
    ...
    - 輸入__Python：選擇解譯器__，然後選擇適當的環境

1. 要驗證是否可以使用 SDK，請創建一個包含以下代碼的新 Python 檔 （.py）：
    ...
    azureml.core.VERSION
    ```
    按一下"運行儲存格"CodeLens 或簡單地按移位輸入來運行此代碼。
...
### <a name="set-up-your-databricks-cluster"></a>設置資料磚塊群集

創建[資料磚塊群集](https://docs.microsoft.com/azure/azure-databricks/quickstart-create-databricks-workspace-portal)。 僅當在 Databricks 上安裝用於自動機器學習的 SDK 時，才會應用某些設置。
**建立叢集將需要幾分鐘的時間。**
...
| Databricks 執行階段 |always|非 ML 運行時 6.0 （scala 2.11， 火花 2.4.3） |
| Python 版本 |always| 3 |
| 背景工作角色 |always| 2 個以上 |
| 背景工作節點 VM 類型 <br>（確定併發反覆運算的最大 #） |自動化 ML<br>向| 建議使用已記憶體最佳化的 VM |
| 啟用自動調整 |自動化 ML<br>向| 取消選取 |

請靜候至叢集運作，再繼續操作。

### <a name="install-the-correct-sdk-into-a-databricks-library"></a>將正確的 SDK 安裝到資料磚塊庫中
群集運行後，[創建一個庫](https://docs.databricks.com/user-guide/libraries.html#create-a-library)，將相應的 Azure 機器學習 SDK 包附加到群集。

1. 按右鍵要存儲庫的當前工作區資料夾。 選擇 **"創建** > **庫**"。

1. **僅選擇一個選項**（不支援其他 SDK 安裝）

   |SDK&nbsp;&nbsp;包附加功能|來源|皮皮&nbsp;名稱&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;|
   |----|---|---|
   |對於資料磚| 上傳 Python Egg 或 PyPI | azureml-sdk[databricks]|
   |對於資料磚 - 與<br> 自動 ML 功能| 上傳 Python Egg 或 PyPI | azureml-sdk[自動ml]|
...
1. 監視錯誤，直到狀態更改為**附加**，這可能需要幾分鐘時間。  如果此步驟失敗：
   ...
   還要考慮：
   + 在自動ML配置中，使用 Azure 資料塊時添加以下參數：
       1. ```max_concurrent_iterations```基於群集中的工作節點數。
        2. ```spark_context=sc```基於預設火花上下文。
   + 或者，如果您有舊的 SDK 版本，請將其從群集安裝的 libs 中取消選擇，然後移動到垃圾堆中。 安裝新版 SDK，並重新啟動叢集。 如果重新開機後出現問題，請分離並重新連接群集。

如果安裝成功，導入的庫應類似于以下庫之一：

資料磚**_SDK，無需_** 自動機器學習![Azure 機器學習 SDK 資料磚](./media/how-to-configure-environment/amlsdk-withoutautoml.jpg)

資料磚 SDK**與**自動機器學習![SDK 一起安裝在資料磚上，並安裝了自動機器學習](./media/how-to-configure-environment/automlonadb.png)

### <a name="start-exploring"></a>開始探索

現在就試試看：
+ 雖然許多示例筆記本可用，**但只有[這些示例筆記本](https://github.com/Azure/MachineLearningNotebooks/blob/master/how-to-use-azureml/azure-databricks)使用 Azure 資料塊。**

+ 直接從工作區導入這些示例。 請參閱以下內容：![選擇導入](./media/how-to-configure-environment/azure-db-screenshot.png)
![面板](./media/how-to-configure-environment/azure-db-import.png)

+ 瞭解如何[創建使用資料磚塊作為訓練計算的管道](how-to-create-your-first-pipeline.md)。

## <a name="create-a-workspace-configuration-file"></a><a id="workspace"></a>建立工作區組態檔

工作區設定檔是一個 JSON 檔，該檔告訴 SDK 如何與 Azure 機器學習工作區進行通信。 檔案名稱為 *config.json*，其格式如下：
...
* **下載檔案**：在[Azure 門戶](https://ms.portal.azure.com)中，從工作區的 **"概述"** 部分選擇 **"下載 config.json"。**

     ![Azure 入口網站](./media/how-to-configure-environment/configure.png)

* **以程式設計方式創建檔**：在以下程式碼片段中，通過提供訂閱 ID、資源組和工作區名稱連接到工作區。 接著，它會將工作區組態儲存至檔案：
    ...
    此代碼將設定檔寫入 *.azureml/config.json*檔。
