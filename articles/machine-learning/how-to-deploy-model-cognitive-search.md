...
ms.locfileid: "91250890"
---
# <a name="deploy-a-model-for-use-with-cognitive-search"></a>部署用於認知搜尋的模型


本文會教您如何使用 Azure Machine Learning 部署模型，以搭配 [Azure 認知搜尋](../search/search-what-is-azure-search.md)使用。
...
>
> 如需如何設定認知搜尋以使用已部署模型的相關資訊，請參閱 [使用 Azure Machine Learning 教學課程建立和部署自訂技能](../search/cognitive-search-tutorial-aml-custom-skill.md) 。
>
...
部署模型以搭配 Azure 認知搜尋使用時，部署必須符合下列需求：

* 使用 Azure Kubernetes Service 裝載模型以進行推斷。
* 為 Azure Kubernetes Service 啟用傳輸層安全性 (TLS) 。 TLS 是用來保護認知搜尋和已部署模型之間的 HTTPS 通訊。
* 專案腳本必須使用 `inference_schema` 封裝來產生服務的 OpenAPI (Swagger) 架構。
* 專案腳本也必須接受 JSON 資料作為輸入，並產生 JSON 作為輸出。
...
Azure Machine Learning 工作區提供集中式位置，可搭配使用 Azure Machine Learning 時所建立的所有成品。 工作區會保留所有定型執行的歷程記錄，包括記錄、計量、輸出，以及腳本的快照。
...
當您將模型從 Azure Machine Learning 部署至 Azure Kubernetes Service 時，將其裝載為 web 服務所需的模型和所有資產都會封裝到 Docker 容器中。 然後，此容器會部署到叢集上。

下列程式碼示範如何為您的工作區建立新的 Azure Kubernetes Service (AKS) 叢集：

> [!TIP]
> 您也可以將現有的 Azure Kubernetes Service 連結至 Azure Machine Learning 工作區。 如需詳細資訊，請參閱 [如何將模型部署至 Azure Kubernetes Service](how-to-deploy-azure-kubernetes-service.md)。

> [!IMPORTANT]
> 請注意，程式碼會使用 `enable_ssl()` 方法來啟用叢集 (TLS) 的傳輸層安全性。 當您計畫從認知服務使用已部署的模型時，這是必要的。
...
專案腳本會接收提交至 web 服務的資料，並將其傳遞至模型，並傳回評分結果。 下列腳本會在啟動時載入模型，然後使用模型來評分資料。 有時會呼叫此檔案 `score.py` 。

> [!TIP]
> 這是模型特定的輸入指令碼。 例如，腳本必須知道要搭配您的模型、資料格式等使用的架構。

> [!IMPORTANT]
> 當您計畫使用 Azure 認知服務中已部署的模型時，您必須使用 `inference_schema` 套件來啟用部署的架構產生。 此封裝提供的裝飾專案可讓您定義 web 服務的輸入和輸出資料格式，以使用模型來執行推斷。
...
如需有關輸入腳本的詳細資訊，請參閱 [如何和部署的位置](how-to-deploy-and-where.md)。

## <a name="define-the-software-environment"></a>定義軟體環境

環境類別是用來定義服務的 Python 相依性。 它包含模型和專案腳本所需的相依性。 在此範例中，它會從一般 pypi 索引，以及從 GitHub 儲存機制安裝套件。 
...
> [!TIP]
> 如果您不確定部署的記憶體、CPU 或 GPU 需求，可以流量分析來瞭解這些需求。 如需詳細資訊，請參閱 [部署模型的方式和位置](how-to-deploy-and-where.md)。
...
將模型部署到您的 AKS 叢集，並等候它建立您的服務。 在此範例中，系統會從登錄載入兩個已註冊的模型，並將其部署至 AKS。 部署之後，部署中的檔案會 `score.py` 載入這些模型，並使用它們來執行推斷。
...
下列範例會使用 `aks_service` 先前的程式碼區段所儲存在變數中的部署資訊。 它會使用此變數來取得與服務通訊所需的評分 URL 和驗證權杖：
...
如需從認知搜尋使用此模型的詳細資訊，請參閱使用 Azure Machine Learning 教學課程 [建立和部署自訂技能](../search/cognitive-search-tutorial-aml-custom-skill.md) 。

## <a name="clean-up-the-resources"></a>清除資源

如果您已特別針對此範例建立 AKS 叢集，請在使用認知搜尋完成測試之後刪除您的資源。
...
* [使用 Azure Machine Learning 建立和部署自訂技能](../search/cognitive-search-tutorial-aml-custom-skill.md)
