...
:::image type="content" source="media/ubuntu_upgrade/create-snapshot-options.png" alt-text="顯示 [建立快照集] 選項的螢幕擷取畫面":::
...
:::image type="content" source="media/ubuntu_upgrade/remote-host-warning.png" alt-text="顯示遠端主機識別已變更警告的 PowerShell 螢幕擷取畫面":::

若要這樣做，請在您的本機電腦上執行下列命令：
...
:::image type="content" source="media/ubuntu_upgrade/portal-disks-search.png" alt-text="顯示 [搜尋磁片] 頁面和 [新增] 按鈕 Azure 入口網站的螢幕擷取畫面":::

2. 將 **訂** 用帳戶、 **資源群組** 和 **區域** 設定為 VM 快照集的值。 選擇要建立之磁片的 **名稱** 。

3. 選取 [ **來源類型** ] 作為 [ **快照** ]，然後選取 VM 快照集作為 **來源快照** 集。 檢查並建立磁片。 

:::image type="content" source="media/ubuntu_upgrade/disk-create-options.png" alt-text="顯示選項之 [磁片建立] 對話方塊的螢幕擷取畫面":::

### <a name="create-a-new-ubuntu-data-science-virtual-machine"></a>建立新的 Ubuntu 資料科學虛擬機器

使用 [Azure 入口網站](https://portal.azure.com) 或 [ARM 範本](./dsvm-tutorial-resource-manager.md)建立新的 Ubuntu 資料科學虛擬機器。 
...
如需詳細資訊，請參閱 [快速入門：設定 Linux (Ubuntu) 的資料科學虛擬機器 ](./dsvm-ubuntu-intro.md)。
...
:::image type="content" source="media/ubuntu_upgrade/attach-data-disk.png" alt-text="顯示磁片附件選項的 [DSVM 選項] 頁面螢幕擷取畫面":::
...
2. 執行下列命令，確認您已從舊 VM 的快照集連接所建立的磁片：
    ...
    :::image type="content" source="media/ubuntu_upgrade/lsblk-results.png" alt-text="Lsblk 輸出的螢幕擷取畫面，其中顯示已卸載的資料磁片磁碟機":::
...
如需詳細資訊，請參閱 [使用入口網站將資料磁片連接至 LINUX VM](../../virtual-machines/linux/attach-disk-portal.md#connect-to-the-linux-vm-to-mount-the-new-disk)。
...
:::image type="content" source="media/ubuntu_upgrade/ssh-os-release.png" alt-text="顯示作業系統版本資料之 Ubuntu 終端機的螢幕擷取畫面":::

版本的變更也會顯示在 Azure 入口網站中。

:::image type="content" source="media/ubuntu_upgrade/portal-showing-os-version.png" alt-text="入口網站的螢幕擷取畫面，其中顯示包含作業系統版本的 DSVM 屬性":::

## <a name="next-steps"></a>後續步驟

- [Azure 中的資料科學與 Ubuntu 資料科學機器](./linux-dsvm-walkthrough.md)
- [Azure 資料科學虛擬機器上包含哪些工具](./tools-included.md) \(部分機器翻譯\)