...
ms.locfileid: "85660768"
---
# <a name="secure-code-best-practices-with-azure-machine-learning"></a>使用 Azure Machine Learning 保護程式碼最佳做法

在 Azure Machine Learning 中，您可以從任何來源上傳檔案和內容。 Jupyter 筆記本或腳本中的內容可能會從您的會話讀取資料、在 Azure 中存取組織內的資料，或代表您執行惡意流程。
...
使用 Azure Machine Learning 的開發通常牽涉到 web 架構開發環境（筆記本 & Azure ML studio）。 使用 web 架構的開發環境時，可能的威脅包括：
...
* [跨網站偽造要求（CSRF）](https://owasp.org/www-community/attacks/csrf)：此攻擊可能會將影像或連結的 url 取代為惡意腳本或 API 的 url。 載入影像或按一下連結時，會對 URL 進行呼叫。
