...
|影像|函式：<br>`run.log_image(name, path=None, plot=None)`<br><br>範例：<br>`run.log_image("ROC", plot=plt)` | 將映像記錄到執行記錄中。 使用 log_image 來記錄。執行的 PNG 影像檔案或 matplotlib 圖。  這些映像會顯示在執行記錄中，並可供比較。|
