...
如果您遵循[設定適用於進階分析的 Azure 虛擬機器](../../machine-learning/data-science-virtual-machine/overview.md)中所提供的指示建立了 Azure 虛擬機器，應該已將這個指令碼檔案下載至虛擬機器上的 *C:\\Users\\\<使用者名稱\>\\Documents\\Data Science Scripts* 目錄中。 這些 Hive 查詢只會要求您在適當的欄位中提供資料結構描述和 Azure blob 儲存體設定，以準備提交。
...
* **解壓縮** 為 .csv 檔案，然後
* 將它們**上傳**到 Azure 儲存體帳戶的預設（或適當的容器）;這類帳戶的選項會出現在 [[使用 Azure 儲存體搭配 Azure HDInsight](../../hdinsight/hdinsight-hadoop-use-blob-storage.md)叢集] 主題。 請參閱此 [頁面](hive-walkthrough.md#upload)，以了解將 .csv 檔案上傳至儲存體帳戶上之預設容器的程序。
...
#### <a name="submit-hive-queries-in-hql-files"></a>在 ' hql ' 檔案中提交 Hive 查詢
若 Hive 查詢更複雜且有多行，則在命令列或 Hive 命令主控台中編輯查詢並不實際。 另一個替代方式是在 Hadoop 叢集的前端節點中使用文字編輯器，將 Hive 查詢儲存在前端節點的本機目錄中的 '. hql ' 檔案。 然後，您可以使用`-f`引數來提交 '. hql ' 檔案中的 Hive 查詢，如下所示：
...
您也可以使用查詢主控台（Hive 編輯器），方法是在網頁瀏覽器中輸入*HTTPs\//\<： Hadoop 叢集名稱>* 格式的 URL。 您必須登入才能看到此主控台，因此您在這裡需要 Hadoop 叢集認證。
...
* **資料庫名稱\>：您想要建立的資料庫\<** 名稱。 如果您只想要使用預設資料庫，則可以省略「*建立資料庫 ...*」查詢。
* **資料表名稱\>：您要在指定的資料庫中建立的資料表\<** 名稱。 如果您想要使用預設資料庫，則資料表* \<名稱\> *不能直接參考資料表，而不\<需要資料庫\>名稱。
* **欄位分隔符號\>：分隔資料檔案中要上傳至 Hive 資料表之欄位的\<** 分隔符號。
* **行分隔符號\>：分隔資料檔中各行的\<** 分隔符號。
* **儲存位置\>：用來儲存 Hive 資料表資料的 Azure 儲存體\<** 位置。 如果您未指定 *LOCATION\< 儲存體位置\>*，資料庫和資料表預設會儲存在 Hive 叢集之預設容器的 *hive/warehouse/* 目錄中。 如果您想要指定儲存體位置，儲存體位置必須位於資料庫和資料表的預設容器內。 此位置必須以 *' wasb:///\<directory 1>/'* 或 *'\<wasb:///directory 1>/\<directory 2>/'* 等格式，參照叢集預設容器的位置。執行查詢之後，會在預設容器內建立相對目錄。
...
* * \< \> * ** \>blob 資料的路徑：如果要上傳至 Hive 資料表的 blob 檔案是在 HDInsight Hadoop 叢集的預設容器中，則 blob 資料的路徑應為 ' wasb://directory （在此容器中）的格式，> \< ** * \</\<blob 檔案名> '*。 Blob 檔案也可以位於 HDInsight Hadoop 叢集的其他容器中。 在此情況下， * \< \> blob 資料路徑*的格式應該是 *' wasb://\<container name>\<儲存體帳戶名稱>. blob.core.windows.net/\<blob 檔案名> '*。
...
查詢資料分割資料表時，建議您在`where`子句**開頭**加入資料分割條件，以改善搜尋效率。
...
> [!NOTE]
> 如果 TEXTFILE 資料表* \<資料庫名稱\>，則\<為。外部 textfile 資料表名稱\> *具有分割區，在步驟3中`SELECT * FROM <database name>.<external textfile table name>` ，命令會選取資料分割變數做為傳回的資料集中的欄位。 將它插入* \<資料庫名稱\>。\<ORC 資料表名稱\> *因為* \< \>資料庫名稱而失敗\< 。ORC 資料表名稱\> *沒有資料分割變數做為資料表架構中的欄位。 在此情況下，您需要特別選取要插入* \<資料庫名稱\>的欄位。\<ORC 資料表名稱\> * ，如下所示：
>
>
...
在所有資料都已插入* \< \> * * \<資料庫名稱\>之後，使用下列查詢時，可以安全地卸載外部文字檔資料表名稱\< 。ORC 資料表名稱\>*：
