...
description: 將資料從一般檔案（CSV 或 TSV 格式）或儲存在 SQL Server 中的資料移至 Azure SQL Database。
...
本文概述從一般檔案（CSV 或 TSV 格式）或儲存在 SQL Server 中的資料，將資料移至 Azure SQL Database 的選項。 將資料移到雲端的這些工作是 Team Data Science Process 的一部分。

如需概述將資料移至 SQL Server 以進行 Machine Learning 之選項的主題，請參閱[將資料移至 Azure 虛擬機器上的 SQL Server](move-sql-server-virtual-machine.md)。
...
您可以將上述程序調整為自己的資料集，或者遵循上述步驟使用 NYC 計程車資料集。 若要將 NYC 計程車資料集上傳至 SQL Server 資料庫，請遵循[大量匯入資料到 SQL Server 資料庫](sql-walkthrough.md#dbload)中所述的程式。
...
如果來源資料儲存在 SQL Server 中，將資料移至 Azure SQL Database 有多種可能的情況：
...
瞭解如何在本主題中使用 Azure Data Factory （ADF）將資料移至 Azure SQL Database，[使用 Azure Data Factory 將資料從 SQL Server 移至 SQL Azure](move-sql-azure-adf.md)。 本主題說明如何使用 ADF，透過 Azure Blob 儲存體將資料從 SQL Server 資料庫移至 Azure SQL Database。
