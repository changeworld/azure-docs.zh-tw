...
description: 展示 HDInsight Spark 上 Spark MLlib 工具組的資料探索和模型化功能。
...
- [Spark 1.6 筆記本](https://github.com/Azure/Azure-MachineLearning-DataScience/blob/master/Misc/Spark/pySpark/Spark1.6/)：提供如何藉由數種不同的演算法來執行資料探索、模型化和評分的相關資訊。
- [Spark 2.0 筆記本](https://github.com/Azure/Azure-MachineLearning-DataScience/blob/master/Misc/Spark/pySpark/Spark2.0/)：提供如何執行迴歸與分類工作的相關資訊。 資料集可能有所不同，但步驟和概念適用於各種資料集。
...
如需 Jupyter 筆記本核心和預先定義的「秘訣」的詳細資訊，請參閱[在 HDInsight 上搭配 HDInsight Spark Linux 叢集使用的 Jupyter 筆記本核心](../../hdinsight/spark/apache-spark-jupyter-notebook-kernels.md)。
...
* **SQL magic (`%%sql`)** HDInsight PySpark 核心支援針對 sqlContext 進行簡單的內嵌 HiveQL 查詢。 「-o VARIABLE_NAME」引數會將 SQL 查詢的輸出，保存為 Jupyter 伺服器上的 Pandas 資料框架。 此設定可讓輸出以本機模式提供。
...
### <a name="create-a-random-subsampling-of-the-data-and-split-it-into-training-and-testing-sets"></a>建立資料的隨機子取樣，並將資料分割成訓練集和測試集

此程式碼會建立隨機取樣資料 (這裡使用 25%)。 雖然您不需要這個範例 (因為資料集的大小)，我們將在這裡示範如何取樣，讓您了解如何在需要時使用它來自行解決問題。 若樣本龐大，取樣可以在訓練模型時節省大量時間。 接下來我們將範例分割成訓練部分 (這裡為 75%) 和測試部分 (這裡為 25%)，以便在分類和迴歸模型化中使用。
...
## <a name="train-a-binary-classification-model"></a>將二進位分類模型訓練
...
## <a name="train-a-regression-model"></a>定型迴歸模型
