...
將資料模型化前的第一個步驟就是特徵設計。 其在資料科學程序中的角色[說明在此](https://docs.microsoft.com/azure/machine-learning/team-data-science-process/create-features)。 「特徵」是模型的預測性屬性 - 例如溫度、壓力、震動等。 針對 PdM，功能工程牽涉到將機器的健康情況，與在可調整的持續時間內收集的歷程記錄資料抽象化。 在該意義而言，它與其對等項目不同，例如遠端監視、異常偵測和故障偵測。 
