...
- 在下一個層次，企業會練習「預防性維護」__，以判斷零件的使用年限，並且在故障前進行維護或更換。 預防性維護可避免事先未料到和重大的失敗。 但是,在使用壽命期間,計劃停機、元件利用率不足以及人工成本仍然很高。
...
運作中設備的感應器 (或其他) 串流資料是很重要的資料來源。 PdM 中的一個關鍵假設是,計算機的運行狀況在日常操作期間會隨時間而降低。 預計資料會包含隨時間變化的特徵，以擷取此老化模式以及任何會導致降低的異常狀況。 演算法需要資料的時態層面，才能了解一段時間的故障和非故障模式。 演算法會根據這些資料點，學習預測機器在故障前可以繼續運作多少個時間單位。
...
| 使用案例 | 相關資料的範例 |
|:---------|---------------------------|
|航班延誤和取消__ | 航段和頁面記錄形式的航班路線資訊。 航班航段數據包括航線詳細資訊,如出發/到達日期、時間、機場、中途航等。頁面日誌包括地面維護人員記錄的一系列錯誤和維護代碼。|
|飛機引擎零件故障__ | 從飛機中可提供各種零件狀況相關資訊的感應器收集而來的資料。 維護記錄有助於識別零件發生故障和更換的時間。|
|ATM 故障__ | 每筆交易 (現金/支票存款) 的感應器讀數和現金的發放。 有關注釋之間的間隙測量、註釋厚度、註釋到達距離、檢查屬性等的資訊。上次重新裝注現金分配器時提供錯誤代碼、維修信息的維護記錄。|
|風力渦輪機故障__ | 感測器監控渦輪機條件,如溫度、風向、發電、發電機速度等。數據來自位於不同地區的風力發電場的多個風力渦輪機。 一般而言，每台渦輪機會有多個在固定時間間隔接力測量的感應器讀數。|
|斷路器故障__ | 包含矯正性、預防性及系統化動作的維護記錄。 傳送至斷路器的運作資料 (包含開啟和關閉等動作的自動和手動命令)。 設備元數據,如製造日期、位置、型號等。斷路器規格,如電壓級別、地理位置、環境條件。|
|電梯門故障__| 電梯中繼資料，例如電梯類型、製造日期、維護頻率、建築類型等。 運作資訊，例如車門循環次數、車門平均關閉時間。 包含原因的故障歷程記錄。|
|車輪故障__ | 測量車輪加速、制動實例、行駛距離、速度等的感測器數據。車輪上的靜態資訊,如製造商,製造日期。 從零件訂單資料庫推斷的故障資料，用以追蹤訂單日期和數量。|
...
將資料模型化前的第一個步驟就是特徵設計。 其在資料科學程序中的角色[說明在此](https://docs.microsoft.com/azure/machine-learning/team-data-science-process/create-features)。 「特徵」__ 是模型的預測性屬性 - 例如溫度、壓力、震動等。 對於 PdM,功能工程涉及將機器的運行狀況抽象到在相當持續時間內收集的歷史數據上。 在該意義而言，它與其對等項目不同，例如遠端監視、異常偵測和故障偵測。 
...
對於資產的每個標記記錄,定義_大小為<sub>W-k</sub>_ 的視窗,其中_k_是大小_W_的視窗數。然後,聚合在_k__翻滾視窗_ _W-k、W-(k-1)<sub>(k-1)</sub>、...、 W-<sub>2、W-1</sub><sub>1</sub>_ 上創建,用於記錄時間戳之前的期間。 _k_ 若為較小數字可擷取短期的效果，若為較大數字則可擷取長期的降低模式。 (請參閱圖 2)。
...
> - Dong, G. Liu, H. (編輯), 機器學習和數據分析功能工程 (查普曼 & 霍爾/CRC 數據挖掘和知識發現系列), CRC出版社, 2018.
...
二進位類別用於_預測裝置在未來時間段內發生故障的概率_- 稱為_未來地平線週期 X_。X 由業務問題和手頭的數據決定,並與領域專家協商。 範例包括：
...
![圖 5. 多類別的故障時間預測標籤](./media/predictive-maintenance-playbook/labelling-for-multiclass-classification-for-failure-time-prediction.png)
...
假設有一連串的時間戳記事件，例如來自各種感應器的測量資料。 針對包含多個事件的時間範圍，定義訓練和測試範例的特徵及標籤。 例如,對於二進位類別,基於過去的事件創建要素,並基於未來"X"時間單位內的未來事件創建標籤(請參閱有關[功能工程和](#feature-engineering)建模技術的部分)。 因此，範例的標記時間範圍會晚於其特徵的時間範圍。
...
| # | Title | 描述 |
|--:|:------|-------------|
| 2 | [Azure 預測性維護解決方案範本](https://github.com/Azure/AI-PredictiveMaintenance) | 一個開源解決方案樣本,用於展示 Azure ML 建模和能夠支援 IoT 遠端監視上下文中的預測性維護方案的完整 Azure 基礎結構。 |
