...
- 在下一個層次，企業會練習「預防性維護」__，以判斷零件的使用年限，並且在故障前進行維護或更換。 預防性維護可避免事先未料到和重大的失敗。 但是，在使用壽命期間，計畫停機、元件利用率不足以及人工成本仍然很高。
...
|**能源** |                          |
|_風力渦輪機故障_：風力渦輪機是環境責任國家/地區的主要能源，資本成本高。 風力渦輪機的一個關鍵部件是發電機電機，其故障使渦輪機失效。 其修理成本也極為昂貴。|預測 KPI (例如 MTTF (平均失效時間))，可協助能源公司預防渦輪機故障，並確保最少的停機時間。 故障機率可讓技術人員知道要監視可能很快會故障的渦輪機，並安排以時間為基礎的維護制度。 預測性模型可提供各種造成故障的因素見解，進而協助技術人員進一步了解問題的根本原因。|
...
有關故障歷程記錄資料的兩個常見問題：(1) 「需要多少個故障事件才能訓練模型？」 （2） "有多少記錄被視為"足夠"？沒有確切的答案，只有經驗法則。 針對問題 (1)，故障事件數目愈多，模型愈佳。 針對問題 (2)，確切的故障事件數目取決於資料和所要解決之問題的內容。 但另一方面，如果機器太常故障，企業就會換掉它，以減少故障情況。 再次重申，領域專家的指導方針很重要。 不過，有一些方法可以處理「罕見事件」__ 的問題。 [處理不平衡的資料](#handling-imbalanced-data)一節會討論這些方法。
...
資產的維護歷史記錄包含有關更換的部件、執行的維修活動等的詳細資訊。這些事件記錄降級模式。 訓練資料中缺少這項重要資訊，可能會導致誤導的模型結果。 在維護歷程記錄中也可以找到故障歷程記錄，其以特殊錯誤碼或零件的訂單日期呈現。 領域專家應調查和提供影響故障模式的其他資料來源。

#### <a name="machine-operating-conditions"></a>機器運作狀況
運作中設備的感應器 (或其他) 串流資料是很重要的資料來源。 PdM 中的一個關鍵假設是，電腦的運行狀況在日常操作期間會隨時間而降低。 預計資料會包含隨時間變化的特徵，以擷取此老化模式以及任何會導致降低的異常狀況。 演算法需要資料的時態層面，才能了解一段時間的故障和非故障模式。 演算法會根據這些資料點，學習預測機器在故障前可以繼續運作多少個時間單位。
...
| 使用案例 | 相關資料的範例 |
|:---------|---------------------------|
|航班延誤和取消__ | 航段和頁面記錄形式的航班路線資訊。 航班航段資料包括航線詳細資訊，如出發/到達日期、時間、機場、中途航等。頁面日誌包括地面維護人員記錄的一系列錯誤和維護代碼。|
|飛機引擎零件故障__ | 從飛機中可提供各種零件狀況相關資訊的感應器收集而來的資料。 維護記錄有助於識別零件發生故障和更換的時間。|
|ATM 故障__ | 每筆交易 (現金/支票存款) 的感應器讀數和現金的發放。 有關注釋之間的間隙測量、注釋厚度、注釋到達距離、檢查屬性等的資訊。上次重新裝注現金分配器時提供錯誤代碼、維修資訊的維護記錄。|
|風力渦輪機故障__ | 感應器監控渦輪機條件，如溫度、風向、發電、發電機速度等。資料來自位於不同地區的風力發電場的多個風力渦輪機。 一般而言，每台渦輪機會有多個在固定時間間隔接力測量的感應器讀數。|
|斷路器故障__ | 包含矯正性、預防性及系統化動作的維護記錄。 傳送至斷路器的運作資料 (包含開啟和關閉等動作的自動和手動命令)。 設備中繼資料，如製造日期、位置、型號等。斷路器規格，如電壓級別、地理位置、環境條件。|
|電梯門故障__| 電梯中繼資料，例如電梯類型、製造日期、維護頻率、建築類型等。 運作資訊，例如車門循環次數、車門平均關閉時間。 包含原因的故障歷程記錄。|
|車輪故障__ | 測量車輪加速、制動實例、行駛距離、速度等的感應器資料。車輪上的靜態資訊，如製造商，製造日期。 從零件訂單資料庫推斷的故障資料，用以追蹤訂單日期和數量。|
...
將資料模型化前的第一個步驟就是特徵設計。 其在資料科學程序中的角色[說明在此](https://docs.microsoft.com/azure/machine-learning/team-data-science-process/create-features)。 「特徵」__ 是模型的預測性屬性 - 例如溫度、壓力、震動等。 對於 PdM，功能工程涉及將機器的運行狀況抽象到在相當持續時間內收集的歷史資料上。 在該意義而言，它與其對等項目不同，例如遠端監視、異常偵測和故障偵測。 
...
![圖 1。 滾動彙總特徵](./media/predictive-maintenance-playbook/rolling-aggregate-features.png)
...
- _飛機發動機零件故障_：滾動手段、標準差、過去一天、一周的總和等。此指標應與業務領域專家一起確定。
...
對於資產的每個標記記錄，定義_大小為<sub>W-k</sub>_ 的視窗，其中_k_是大小_W_的視窗數。然後，聚合在_k__翻滾視窗_ _W-k、W-（k-1）<sub>(k-1)</sub>、...、W-<sub>2、W-1</sub><sub>1</sub>_ 上創建，用於記錄時間戳記之前的期間。 _k_ 若為較小數字可擷取短期的效果，若為較大數字則可擷取長期的降低模式。 (請參閱圖 2)。
...
> - Dong， G. Liu， H. （編輯）， 機器學習和資料分析功能工程 （查普曼 & 霍爾/CRC 資料採礦和知識發現系列）， CRC出版社， 2018.
...
二進位分類用於_預測設備在未來時間段內發生故障的概率_- 稱為_未來地平線週期 X_。X 由業務問題和手頭的資料決定，並與領域專家協商。 範例包括：
...
以下列出某些使用案例的標記策略範例。
- _航班延誤_：X可以選擇為一天，以預測未來24小時的延誤。 接著，在故障前 24 小時內的所有航班都已標記為 1。
...
![圖 5. 多類分類的故障時間預測標籤](./media/predictive-maintenance-playbook/labelling-for-multiclass-classification-for-failure-time-prediction.png)
...
![圖 6. 多類分類的根本原因預測標籤](./media/predictive-maintenance-playbook/labelling-for-multiclass-classification-for-root-cause-prediction.png)
...
建議方法是以「時間相依」__ 方式將範例分割成訓練和驗證集，讓所有驗證範例的時間晚於所有訓練範例。 對於每組超參數值，透過訓練資料集來訓練演算法。 測量模型在同一驗證集中的性能。 選擇可顯示最佳效能的超參數值。 訓練/驗證分割所選的超參數值會導致未來模型效能優於交叉驗證隨機選擇的值。
...
假設有一連串的時間戳記事件，例如來自各種感應器的測量資料。 針對包含多個事件的時間範圍，定義訓練和測試範例的特徵及標籤。 例如，對於二進位分類，基於過去的事件創建要素，並基於未來"X"時間單位內的未來事件創建標籤（請參閱有關[功能工程和](#feature-engineering)建模技術的部分）。 因此，範例的標記時間範圍會晚於其特徵的時間範圍。
...
要達到此平衡的方法有好幾種。 若要緩和不平等損失的問題，請將高成本指派給少數類別歸類錯誤，並嘗試將整體成本降到最低。 有些演算法 (例如 SVM (支援向量機器)__) 原本就會使用此概念，其允許在訓練期間指定肯定和否定範例的成本。 同樣地，「促進式決策樹」__ 等提升 (Boosting) 方法通常會在資料不平衡的情況下展現良好效能。
...
- [準確性](https://en.wikipedia.org/wiki/Accuracy_and_precision)是用於描述分類器性能的最常見指標。 但精確度易受資料分配影響，而且對於資料集不平衡的案例而言是不平衡的量值。 改用其他計量。 [混淆矩陣](https://en.wikipedia.org/wiki/Confusion_matrix)等工具用來計算及推論模型的精確度。
...
| # | Title | 描述 |
|--:|:------|-------------|
| 2 | [Azure 預測性維護解決方案範本](https://github.com/Azure/AI-PredictiveMaintenance) | 一個開源解決方案範本，用於演示 Azure ML 建模和能夠支援 IoT 遠端監視上下文中的預測性維護方案的完整 Azure 基礎結構。 |
...
| 6 | [適用於 IoT Edge 的 Azure AI 工具組](https://github.com/Azure/ai-toolkit-iot-edge) | 使用 TensorFlow 在 IoT 邊緣中的 AI;工具組在 Azure IoT 邊緣相容 Docker 容器中打包深度學習模型，並將這些模型公開為 REST API。
...
| [微軟AI YouTube網路研討會](https://www.youtube.com/watch?v=NvrH7_KKzoM&t=4s) | 公開 |
