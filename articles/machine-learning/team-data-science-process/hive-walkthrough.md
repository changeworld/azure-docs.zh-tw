...
根據資料分析，判斷您想要進行的預測類型，以協助闡明所需的處理工作。 以下是我們在本逐步解說中所討論的三個預測問題範例，全都根據*tip\_數量*而定：
...
* ***\<Hadoop 叢集的預設容器 >*** 您的叢集所使用的預設容器。 預設容器的名稱通常與叢集本身的名稱相同。 例如，如果叢集稱為 "abc123.azurehdinsight.net"，預設容器即為 abc123。
...
以下是**C:\temp\sample\_hive 的內容\_建立\_db\_和\_資料表的 hql**檔案，該檔案會建立 hive 資料庫**nyctaxidb**，以及資料表的**往返**和**費用**。
...
查詢會直接套用標準 Hive 函式，以從「 **pickup\_日期時間**」欄位產生下列時間參數：
- 時
- 年中的週
