...
ms.locfileid: "93322009"
---
# <a name="move-data-to-or-from-azure-blob-storage-using-ssis-connectors"></a>使用 SSIS 連接器從 Azure Blob 儲存體來回移動資料
[SQL Server Integration Services Feature Pack for Azure](/sql/integration-services/azure-feature-pack-for-integration-services-ssis) 中的元件可供連線至 Azure、在 Azure 與內部部署資源來源之間傳輸資料，以及處理儲存在 Azure 中的資料。

[!INCLUDE [blob-storage-tool-selector](../../../includes/machine-learning-blob-storage-tool-selector.md)]

一旦客戶將內部部署資料移至雲端後，就可以從任何 Azure 服務存取其資料，以充分利用 Azure 技術套件的完整功能。 之後可能會使用資料，例如 Azure Machine Learning 或 HDInsight 叢集上。
...
若要執行本文中所述的工作，您必須設定 Azure 訂用帳戶和 Azure 儲存體帳戶。 您需要 Azure 儲存體帳戶名稱和帳戶金鑰，才能上傳或下載資料。

* 若要設定 **Azure 訂用帳戶** ，請參閱 [免費試用一個月](https://azure.microsoft.com/pricing/free-trial/)。
* 如需有關建立 **儲存體帳戶** ，以及取得帳戶和金鑰資訊的指示，請參閱 [關於 Azure 儲存體帳戶](../../storage/common/storage-account-create.md)。

若要使用 **SSIS 連接器** ，您必須下載:

* **SQL Server 2014 或 2016 標準版 (或更新版本)** ：安裝包含 SQL Server 整合服務。
* **適用于 Azure 的 Microsoft SQL Server 2014 或 2016 Integration Services Feature Pack** ：可以分別從 [SQL Server 2014 Integration Services](https://www.microsoft.com/download/details.aspx?id=47366) 和 [SQL Server 2016 Integration Services](https://www.microsoft.com/download/details.aspx?id=49492) 頁面下載這些連接器。
...
如需如何使用 SISS 啟動與執行，以建置簡單擷取、轉換與載入 (ETL) 封裝的相關資訊，請參閱 [SSIS 教學課程：建立簡易 ETL 封裝](/sql/integration-services/ssis-how-to-create-an-etl-package)。

## <a name="download-nyc-taxi-dataset"></a>下載紐約計程車資料集
此處描述的範例使用供大眾使用的資料集 -- [紐約計程車路線](https://www.andresmh.com/nyctaxitrips/) 資料集。 此資料集收集了 2013 年紐約 1 億 7 千 3 百萬筆計程車行程資料。 資料有兩種：路線詳細資料及車費資料。 由於每個月都有一個檔案，因此我們有24個檔案，每個檔案都有大約 2 GB 的未壓縮檔。

## <a name="upload-data-to-azure-blob-storage"></a>將資料上傳至 Azure Blob 儲存體
為了使用 SSIS 功能封裝將資料從內部部署移至 Azure Blob 儲存體，我們使用 [**Azure Blob 上傳工作**](/sql/integration-services/control-flow/azure-blob-upload-task)執行個體，如這裡所示：
...
| 欄位 | 描述 |
| --- | --- |
| **AzureStorageConnection** |指定現有的 Azure 儲存體連線管理員或建立參考指向裝載 blob 檔案之 Azure 儲存體帳戶的新。 |
| **BlobContainer** |指定將上傳的檔案保存為 blob 的 blob 容器名稱。 |
...
若要透過 SSIS 將資料從 Azure Blob 儲存體下載到內部部署儲存體，請使用 [Azure Blob 下載工作](/sql/integration-services/control-flow/azure-blob-download-task)的執行個體。
...
* 若要使用 SSIS 在 Azure HDInsight 叢集上執行 Hive 指令碼，請使用 [Azure HDInsight Hive 工作](/sql/integration-services/control-flow/azure-hdinsight-hive-task)。
* 若要使用 SSIS 在 Azure HDInsight 叢集上執行 Pig 指令碼，請使用 [Azure HDInsight Pig 工作](/sql/integration-services/control-flow/azure-hdinsight-pig-task)。