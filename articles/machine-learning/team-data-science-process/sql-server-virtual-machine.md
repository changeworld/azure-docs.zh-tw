...
description: 探索 + 處理資料，並在 Azure 上的 SQL Server 虛擬機器中使用 Python 或 SQL 來產生功能。
...
### <a name="count-based-feature-generation"></a><a name="sql-countfeature"></a>以計數為基礎的功能產生
下列範例示範兩種產生計數功能的方法。 第一種方法會使用條件式加總，而第二種方法會使用 'where' 子句。 然後，這些結果可能會與原始資料表聯結 (使用主鍵資料行，) 讓計數功能與原始資料一起使用。
...
以下是有關經緯度位置資料的簡短入門指南 (源自 stackoverflow [如何測量經度和緯度的準確性？](https://gis.stackexchange.com/questions/8650/how-to-measure-the-accuracy-of-latitude-and-longitude))。 本指南在將位置納入為一或多個功能之前，會很有用：

* 正負號告訴我們是否位於地球的北方或南方、東方或西方。
* 非零的數百個位數告訴我們使用的是經度，而不是緯度！
* 數十個位數可提供大約 1,000 公里的位置。 它會為我們提供身處哪個大陸或海洋的實用資訊。
* 單位數 (一個十進位度數) 提供一個最多可達 111 公里 (60 海浬，大約 69 英哩) 的位置。 它可以告訴您您所處的州、國家或地區。
...
當資料位於 SQL Server 時，使用 Python 來瀏覽資料與產生特徵，類似於使用 Python 來處理 Azure Blob 中的資料，如[在資料科學環境中處理 Azure Blob 資料](data-blob.md)中所述。 將資料庫中的資料載入至 pandas 資料框架以進行更多處理。 我們將在本節中說明連接到資料庫以及將資料載入資料框架的程序。
