...
* [梯度提升樹](https://spark.apache.org/docs/latest/ml-classification-regression.html#gradient-boosted-trees-gbts) (GBT) 是一群決策樹。 GBT 會反覆訓練決策樹，以最小化遺失函數。 GBT 用於迴歸和分類，可處理分類特徵，不需要調整特徵，還能夠擷取非線性和特徵互動。 它們也可用於多類別分類設定。
...
本節包含的程式碼示範如何將分類的文字資料索引做為標示點資料類型以及如何編碼。 這項轉換會準備文字資料，以用於訓練和測試 MLlib 羅吉斯回歸和其他分類模型。 標示點物件是彈性分散式資料集 (RDD)，其格式化成適合 MLlib 中大部分的 ML 演算法的輸入資料。 [標示點](https://spark.apache.org/docs/latest/mllib-data-types.html#labeled-point) 是本機向量 (密集或疏鬆)，與標籤/回應相關聯。
...
### <a name="create-a-random-subsampling-of-the-data-and-split-it-into-training-and-testing-sets"></a>建立資料的隨機次取樣，並分割成訓練集和測試集
...
**輸出模型檔案的路徑，指向要在取用筆記本中使用的模型檔案。 **若要耗用與評分獨立資料集，您需要在「耗用量 Notebook」中複製並貼上這些檔案名稱。
