...
```
如果您未指定 \<的基底分支名稱 >，新的分支會以 `master`為基礎。 
...
在 [**新增提取要求**] 畫面上，如有必要，流覽至您想要合併變更的 Git 存放庫和分支。 新增或變更您想要的任何其他資訊。 在 [**審核者**] 底下，新增審核者的名稱，然後選取 [**建立**]。 
...
當您回到左側導覽中的 [**存放庫**] 時，您會看到已切換至主要分支，因為 `script` 分支已刪除。
