---
title: 準備 ML Studio (傳統) 的資料-Team Data 科學流程
...
前置處理和清除工作，例如資料探索工作，可以在各種不同的環境中執行，例如 SQL 或 Hive 或 Azure Machine Learning Studio (傳統) ，以及使用各種工具和語言（例如 R 或 Python），取決於您資料的儲存位置和格式。 由於 TDSP 本質上是反覆的，所以這些工作可以在程序工作流程中的各個步驟進行。

本文介紹各種資料處理概念和工作，可在將資料擷取至 Azure Machine Learning Studio (傳統) 之前或之後進行。

如需在 Azure Machine Learning Studio (傳統) 內進行資料探索和預先處理的範例，請參閱 [前置處理資料](https://azure.microsoft.com/documentation/videos/preprocessing-data-in-azure-ml-studio/) 影片。
...
* **不一致的資料記錄**。 檢查是否允許值範圍。 例如，如果資料包含學生 GPA (等級點平均) ，請檢查 GPA 是否在指定的範圍中，例如 0 ~ 4。

當您發現資料有問題時，需要 **處理步驟** ，這通常牽涉到清除遺漏值、資料正規化、離散化、移除和/或取代可能影響資料對齊之內嵌字元的文字處理、一般欄位中的混合資料類型，以及其他專案。

**Azure Machine Learning 會使用格式正確的表格式資料**。  如果資料已是表格形式，則可以直接在 Machine Learning 中使用 Azure Machine Learning Studio (傳統) 來執行資料前置處理。  如果資料的格式不是表格式，假設是 XML，就可能需要進行剖析，才能將資料的格式轉換成表格式。  
...
**表格式資料中的文字欄位** 可能包含會影響資料行對齊和/或記錄界限的字元。 例如，以 tab 分隔的檔案中的內嵌索引標籤會造成資料行對齊，而內嵌的分行符號會中斷記錄行。 寫入或讀取文字時，不正確的文字編碼處理會導致資訊遺失，不慎引進無法讀取的字元 (例如 null) ，而且可能也會影響文字剖析。 您可以需要仔細地進行剖析和編輯，以清除文字欄位來取得正確的對齊方式，及 (或) 從非結構化或半結構化的文字資料中擷取結構化資料。

**資料探索** 可讓您檢視早期資料。 在此步驟中可以發現一些資料問題，然後套用對應的方法以解決這些問題。  請務必提出問題，例如，問題的來源是什麼，以及問題可能是如何引發的。 此程式也可協助您決定需要採取的資料處理步驟來解決這些問題。 識別最終的使用案例和角色也可以用來排列資料處理工作的優先順序。
