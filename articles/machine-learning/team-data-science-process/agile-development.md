...
- **功能**：對應至專案參與的功能。 與用戶端的不同合作是不同的功能，因此最好將專案的不同階段視為不同的功能。 如果您選擇架構（例如） *\<ClientName>-\<EngagementName>* 來命名您的功能，您可以輕鬆地辨識專案的內容，並從名稱本身進行參與。
...
1. 從您的專案頁面中 **，選取**  >  **Backlogs**左側導覽中的 [面板待辦專案]。 
...
若要將工作新增至使用者案例，請選取 [ **+** 使用者故事] 專案旁邊的，然後**Task**選取 [工作]。 填寫工作中的標題和其他資訊。
...
1. 在新建立的專案中，**選取**  >  **Backlogs**左側導覽中的 [面板待辦專案]。
...
1. 繼續選取 **+** [工作專案] 旁的符號，以加入新的 TDSP 階段、階段和工作。 
