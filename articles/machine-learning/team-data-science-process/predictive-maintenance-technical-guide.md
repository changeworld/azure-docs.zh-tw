...
description: 航太、公用事業和運輸中預測性維護解決方案範本的技術指南。
...
---
# <a name="technical-guide-to-the-solution-template-for-predictive-maintenance-in-aerospace"></a>航太中預測性維護解決方案範本的技術指南
...
解決方案範本的設計是要加速建立 E2E 示範的程式。 已部署的範本會以必要的元件布建您的訂用帳戶，然後建立它們之間的關聯性。 它也會在資料管線內植入資料產生器應用程式的範例資料，供您在部署解決方案範本之後下載並安裝到本機電腦上。 產生器的資料會產生資料管線，並開始產生機器學習服務預測，然後就可以在 Power BI 儀表板上將它們視覺化。
...
當您部署解決方案時，它會啟用 Azure 服務，包括事件中樞、串流分析、HDInsight、Data Factory 和 Machine Learning。 架構圖顯示為航太解決方案範本建構預測性維護的方式。 您可以按一下在部署解決方案時所建立的解決方案範本圖表上的服務 (HDInsight 除外，因為 HDInsight 是在需要執行相關管線活動，且會在執行之後刪除的情況下，才能依需求佈建)。
...
針對此範本，使用的資料來源是從下載的桌面應用程式產生，您可以在成功部署後於本機執行。
...
使用 HDInsight 來執行[Hive](https://blogs.msdn.com/b/bigdatasupport/archive/2013/11/11/get-started-with-hive-on-hdinsight.aspx)腳本（由 Azure Data Factory 協調），以提供使用 Azure 串流分析資源封存之原始事件的匯總。
...
使用 Azure 串流分析資源來提供近乎即時的分析，方法是從資料串流讀取並輸出資料至任意數目的來源。
...
[Azure Data Factory](https://azure.microsoft.com/documentation/services/data-factory/) 服務會協調資料的移動和處理。 在航太解決方案的預測性維護範本中，data factory 是由三個[管線](../../data-factory/concepts-pipelines-activities.md)所組成，可使用各種技術來移動和處理資料。  請開啟隨解決方案部署所建立的解決方案範本圖表底端的 Data Factory 節點，以存取您的 Data Factory。 如果在資料集底下看到錯誤，這是因為在啟動資料產生器之前即已部署 Data Factory。 這些錯誤可以忽略，而且不會防止您的 data factory 運作。
...
* 管線的最後一個步驟是將資料 (例如來自機器學習的預測) 寫入至 SQL Database。 您可能必須等候最多三個小時，資料才會出現在 SQL Database。 監視您的 SQL Database 中有多少資料可用的其中一個方法是透過 [Azure 入口網站](https://portal.azure.com/)。 在左面板中，找出 [ ![sql 資料庫](./media/predictive-maintenance-technical-guide/icon-SQL-databases.png) ] sql 圖示，然後按一下它。 接著找到您的資料庫 **pmaintenancedb** 並按一下它。 在下一個頁面的底部，按一下 [管理]。
...
Power BI 會連接到 Azure SQL Database 做為其資料來源，並在其中儲存預測結果。 

注意: 
1.    在部署您的解決方案時，資料庫會在3小時內出現預測。 產生器下載隨附的 pbix 檔案包含一些種子資料，因此您可以立即建立 Power BI 儀表板。 
2.    在此步驟中，必要條件是下載並安裝免費的軟體[Power BI 桌上型電腦](https://powerbi.microsoft.com/documentation/powerbi-desktop-get-the-desktop/)。
...
1. 取得資料庫認證。
   ...
   * 您會看到一個新的瀏覽器索引標籤/視窗，其中顯示 [Azure 入口網站] 頁面。 按一下左側面板上的 [資源群組] **** 。
...
2. 使用 Power BI 桌面版來更新冷路徑報告檔案的資料來源。
   ...
   * 在下一個快顯視窗中，您會在左側窗格上看到兩個選項 ([Windows]**** 和 [資料庫]****)。 按一下 [**資料庫**]，填入您的 [使用者**名稱**] 和 [**密碼**] （您第一次部署解決方案並建立 Azure SQL Database 時輸入的使用者名稱和密碼）。 在 [選取要套用這些設定的層級]****** 中，請勾選資料庫層級選項。 然後按一下 **[連線]**。
...
3. (選擇性) 將冷路徑儀表板發佈至 [Power BI 線上版](https://www.powerbi.com/)。 此步驟需要 Power BI 帳戶（或 Office 365 帳戶）。
   ...
   * 若要建立新的儀表板： **+** 按一下左窗格中 [**儀表板**] 區段旁邊的 [符號]。 為這個新儀表板輸入名稱「預測性維護示範」。
   * 開啟報告後，按一下 ![PIN icon](./media/predictive-maintenance-technical-guide/icon-pin.png)，將所有視覺效果釘選至儀表板。 若要尋找詳細的指示，請參閱 [從報告將磚釘選至 Power BI 儀表板](https://support.powerbi.com/knowledgebase/articles/430323-pin-a-tile-to-a-power-bi-dashboard-from-a-report)。
     ...
     <br/>
   * 若要排程資料的重新整理，請將滑鼠停留**在 predictivemaintenanceaerospace.pbix**資料集![上，](./media/predictive-maintenance-technical-guide/icon-elipsis.png)按一下省略號圖示，然後選擇 [**排程**重新整理]。
     <br/>
     > [!NOTE]
     > 如果您看到一則警告訊息，請按一下 [**編輯認證**]，並確定您的資料庫認證與步驟1中所述相同。
     <br/>
...
1. 以下是建立上述其中一個磚的一些步驟–「感應器11與閾值48.26 的車隊觀點」磚：
   ...
   * 將滑鼠停留在儀表板上的此磚上，按一下右上角的 [編輯] 圖示，將其標題變更為「感應器11與臨界值48.26 的車隊觀點」，並將副標題設為「一段時間的平均分佈」。
