...
description: 用於航空航太、公用事業和運輸預測性維護的解決方案範本技術指南。
...
ms.locfileid: "79477165"
---
# <a name="technical-guide-to-the-solution-template-for-predictive-maintenance-in-aerospace"></a>航空航太預測性維護解決方案範本技術指南
...
解決方案範本旨在加快構建 E2E 演示的過程。 已部署的範本使用必要的元件為訂閱提供配置，然後構建它們之間的關係。 它也會在資料管線內植入資料產生器應用程式的範例資料，供您在部署解決方案範本之後下載並安裝到本機電腦上。 產生器的資料會產生資料管線，並開始產生機器學習服務預測，然後就可以在 Power BI 儀表板上將它們視覺化。
...
部署解決方案時，它會啟動 Azure 服務，包括事件中心、流分析、HDInsight、資料工廠和機器學習。 架構圖顯示為航太解決方案範本建構預測性維護的方式。 您可以按一下在部署解決方案時所建立的解決方案範本圖表上的服務 (HDInsight 除外，因為 HDInsight 是在需要執行相關管線活動，且會在執行之後刪除的情況下，才能依需求佈建)。
...
對於此範本，使用的資料來源是從成功部署後在本地運行的已下載桌面應用程式生成的。
...
使用 HDInsight 運行[Hive](https://blogs.msdn.com/b/bigdatasupport/archive/2013/11/11/get-started-with-hive-on-hdinsight.aspx)腳本（由 Azure 資料工廠協調），以提供使用 Azure 流分析資源存檔的原始事件的聚合。
...
使用[Azure SQL 資料庫](https://azure.microsoft.com/services/sql-database/)存儲 Azure 機器學習接收的預測，然後在 Power [BI](https://powerbi.microsoft.com)儀表板中使用這些預測。
...
使用 Azure 流分析資源通過從資料流程讀取資料並將資料輸入任意數量的源來提供近乎即時的分析。
...
[Azure Data Factory](https://azure.microsoft.com/documentation/services/data-factory/) 服務會協調資料的移動和處理。 在航空航太解決方案範本的預測維護中，資料工廠由三[個管道](../../data-factory/concepts-pipelines-activities.md)組成，這些管道使用各種技術移動和處理資料。  請開啟隨解決方案部署所建立的解決方案範本圖表底端的 Data Factory 節點，以存取您的 Data Factory。 如果在資料集底下看到錯誤，這是因為在啟動資料產生器之前即已部署 Data Factory。 這些錯誤可以忽略，不會阻止資料工廠運行。
...
#### <a name="aggregateflightinfopipeline"></a>**
...
#### <a name="copyscoredresultpipeline"></a>**
...
* 管線的最後一個步驟是將資料 (例如來自機器學習的預測) 寫入至 SQL Database。 您可能必須等候最多三個小時，資料才會出現在 SQL Database。 監視您的 SQL Database 中有多少資料可用的其中一個方法是透過 [Azure 入口網站](https://portal.azure.com/)。 在左側面板上，找到 SQL![資料庫](./media/predictive-maintenance-technical-guide/icon-SQL-databases.png)SQL 圖示並按一下它。 接著找到您的資料庫 **pmaintenancedb** 並按一下它。 在底部的下一頁，按一下"管理"。
...
Power BI 作為資料來源連接到 Azure SQL 資料庫，其中存儲預測結果。 

注意: 
1.    部署解決方案時，將在 3 小時內在資料庫中顯示預測。 產生器下載隨附的 pbix 檔案包含一些種子資料，因此您可以立即建立 Power BI 儀表板。 
2.    在此步驟中，先決條件是下載並安裝免費軟體 Power [BI 桌面](https://powerbi.microsoft.com/documentation/powerbi-desktop-get-the-desktop/)。
...
1. 取得資料庫認證。
   ...
   * 您將看到顯示 Azure 門戶頁的新瀏覽器選項卡/視窗。 按一下左側面板上的 [資源群組] **** 。
...
2. 使用 Power BI 桌面版來更新冷路徑報告檔案的資料來源。
   ...
   * 在下一個快顯視窗中，您會在左側窗格上看到兩個選項 ([Windows]**** 和 [資料庫]****)。 按一下 **"資料庫"，** 填寫 **"使用者名"** 和 **"密碼"（** 首次部署解決方案並創建 Azure SQL 資料庫時輸入的使用者名和密碼）。 在 [選取要套用這些設定的層級]****** 中，請勾選資料庫層級選項。 然後按一下 **"連接"。**
...
3. (選擇性) 將冷路徑儀表板發佈至 [Power BI 線上版](https://www.powerbi.com/)。 此步驟需要 Power BI 帳戶（或 Office 365 帳戶）。
   ...
   * 要創建新儀表板，請按一下左側窗格的**+****"儀表板"** 部分旁邊的符號。 為這個新儀表板輸入名稱「預測性維護示範」。
   * 開啟報告後，按一下 ![PIN icon](./media/predictive-maintenance-technical-guide/icon-pin.png)，將所有視覺效果釘選至儀表板。 若要尋找詳細的指示，請參閱 [從報告將磚釘選至 Power BI 儀表板](https://support.powerbi.com/knowledgebase/articles/430323-pin-a-tile-to-a-power-bi-dashboard-from-a-report)。
     ...
     <br/>
   * 要計畫刷新資料，請將滑鼠懸停在**預測維護航空航太**資料集上，按一下![橢圓圖示](./media/predictive-maintenance-technical-guide/icon-elipsis.png)，然後選擇 **"計畫刷新**"。
     <br/>
     > [!NOTE]
     > 如果看到警告訊息，請按一下 **"編輯憑據"，** 並確保資料庫憑據與步驟 1 中描述的憑據相同。
     <br/>
...
1. 在 Azure 串流分析 (ASA) 中加入 Power BI 輸出。
   ...
   * ASA 查詢有三個輸出，即**飛機監控、****飛機警報**和**飛行時間**。 您可以通過按一下查詢選項卡來查看查詢。對應于每個表，您需要向 ASA 添加輸出。 當您新增第一個輸出 (**aircraftmonitor**) 時，請確定**輸出別名**、**資料集名稱**和**資料表名稱**皆相同 (**aircraftmonitor**)。 重複這些步驟來新增 **aircraftalert** 和 **flightsbyhour** 的輸出。 新增所有三個輸出資料表並啟動 ASA 作業後，應該會出現確認訊息 (「啟動串流分析作業 maintenancesa02asapbi 成功」)。
...
1. 下面是創建上面的一個磁貼的一些步驟 - "感應器 11 的 Fleet 視圖與閾值 48.26"磁貼：
   ...
   * 按一下此折線圖右上角的 **"針圖視覺"** 圖示。 可能出現「釘選至儀表板」視窗供您選擇儀表板。 選取 [預測性維護示範]，然後按一下 [釘選]。
   * 將滑鼠懸停在儀表板上的此磁貼上，按一下右上角的"編輯"圖示將其標題更改為"感應器 11 的 Fleet 視圖與閾值 48.26"，並將副標題更改為"跨車隊隨時間的平均值"。
