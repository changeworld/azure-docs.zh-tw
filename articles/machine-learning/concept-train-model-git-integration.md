...
當您從 Python SDK 或 Machine Learning CLI 提交定型回合時，定型模型所需的檔案會上傳至您的工作區。 如果命令`git`可在您的開發環境中使用，則上傳程式會使用它來檢查檔案是否儲存在 git 存放庫中。 若是如此，則您 git 存放庫中的資訊也會上傳做為定型執行的一部分。 這項資訊會儲存在定型回合的下列屬性中：
...
如果您的`git`定型檔案不是位於開發環境的 git 存放庫中，或命令無法使用，則不會追蹤任何 git 相關資訊。
...
> ```
>
> 如果已安裝，且在路徑中，您會收到類似的`git version 2.4.1`回應。 如需在開發環境上安裝 git 的詳細資訊，請參閱[git 網站](https://git-scm.com/)。
...
1. 選取 [__記錄__]，然後展開 [__記錄__] 和 [ __azureml__ ] 專案。 選取以__ ### \_azure__開頭的連結。
...
提交定型回合之後，會傳回[執行](https://docs.microsoft.com/python/api/azureml-core/azureml.core.run%28class%29?view=azure-ml-py)物件。 此`properties`物件的屬性包含已記錄的 git 資訊。 例如，下列程式碼會抓取認可雜湊：
...
`az ml run` CLI 命令可以用來從執行中取出屬性。 例如，下列命令會在名為`train-on-amlcompute`的實驗中傳回最後一次執行的屬性：
