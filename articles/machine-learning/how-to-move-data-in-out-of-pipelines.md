...
1. 使用 `TabularDataset.as_named_inputs()` 或 `FileDataset.as_named_input()` (no ' at end) 來建立 `DatasetConsumptionConfig` 物件
...
雖然 `Dataset` 物件只代表持續性資料，但 [`OutputFileDatasetConfig`](https://docs.microsoft.com/python/api/azureml-core/azureml.data.outputfiledatasetconfig?view=azure-ml-py) 物件 (s) 可用於管線步驟 **和** 持續性輸出資料的暫存資料輸出。 
