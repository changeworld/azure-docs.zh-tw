...
description: Azure ML 入門系列的第 2 部分，說明如何將簡單的 "hello world" Python 指令碼提交至雲端。
...
ms.locfileid: "90929334"
---
# <a name="tutorial-run-hello-world-python-script-part-2-of-4"></a>教學課程：執行 "Hello World" Python 指令碼 (第 2 部，共 4 部)

在本教學課程中，您將了解如何使用 Azure Machine Learning Python SDK 來提交和執行 Python "Hello World" 指令碼。

本教學課程是**四部分教學課程系列的第二部分**，您會在其中了解 Azure Machine Learning 的基本概念，以及在 Azure 中完成作業型機器學習工作。 本教學課程會以您在[教學課程第 1 部分：設定本機電腦以執行 Azure Machine Learning](
tutorial-1st-experiment-sdk-setup-local.md)完成的工作為基礎。

在本教學課程中，您將：

> [!div class="checklist"]
> * 在本機建立和執行 "Hello World" Python 指令碼
...
- 如果您還沒有 Azure Machine Learning 工作區，請完成[教學課程第 1 部分的本機電腦設定](tutorial-1st-experiment-sdk-setup-local.md)。
...
您可以使用您慣用的 IDE 或透過終端機在本機執行程式碼，其具有互動式程式碼偵錯的優點：
...
「控制指令碼」可讓您在雲端中執行 `hello.py` 指令碼。  控制指令碼可讓您控制機器學習程式碼的執行方式和位置。  

在您的教學課程目錄中，建立名為 `03-run-hello.py` 的新 Python 檔案，然後複製下列程式碼並貼到該檔案中：
...
:::row:::
   ...
   :::column-end:::
   :::column span="2":::
       提交您的指令碼。 此提交行為稱為[回合 (Run)](https://docs.microsoft.com/python/api/azureml-core/azureml.core.run(class)?view=azure-ml-py&preserve-view=true)。  一個回合會封裝程式碼的單次執行。 使用回合來監視指令碼進度、擷取輸出、分析結果、將計量視覺化等等。
   :::column-end:::
:::row-end:::
:::row:::
   :::column span="":::
...
## <a name="monitor-your-code-in-the-cloud-using-studio"></a>使用 Studio 在雲端中監視您的程式碼

輸出會包含 Azure Machine Learning Studio 的連結，看起來像這樣：`https://ml.azure.com/experiments/hello-world/runs/<run-id>?wsid=/subscriptions/<subscription-id>/resourcegroups/<resource-group>/workspaces/<workspace-name>`。

遵循連結，然後瀏覽至 [輸出 + 記錄] 索引標籤。您可以看到如下所示的 `70_driver_log.txt` 檔案：
...
`70_driver_log.txt` 檔案包含回合的標準輸出。 在雲端中偵錯遠端回合時，此檔案很有用。
