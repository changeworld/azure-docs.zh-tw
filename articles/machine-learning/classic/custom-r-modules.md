...
description: 瞭解如何在 ML Studio (傳統) 中撰寫和部署自訂 R 模組。
...
ms.locfileid: "93322785"
---
# <a name="define-custom-r-modules-for-machine-learning-studio-classic"></a>定義適用于 Machine Learning Studio (傳統) 的自訂 R 模組
...
**自訂模組** 是一種使用者定義的模組，可上傳至您的工作區，並作為 Azure Machine Learning Studio (傳統) 實驗的一部分執行。 **自訂 R 模組** 是執行使用者定義之 R 函數的自訂模組。 **R** 是一種適用於統計運算和圖形的程式設計語言，由實作演算法的統計學家和資料科學家廣泛使用。 目前，R 是自訂模組中唯一支援的語言，但未來版本將新增更多語言的支援。

自訂模組在 Azure Machine Learning Studio (傳統) 中具有最高層級的 **狀態** ，因為它們可以像任何其他模組一樣使用。 它們可以與已發行實驗或視覺效果中包含的其他模組一起執行。 您可以控制模組實作的演算法、要使用的輸入與輸出連接埠、模型參數，以及其他多種執行階段行為。 包含自訂模組的實驗也可以發佈至 Azure AI 資源庫以輕鬆共用。
...
若要將此函式公開 `CustomAddRows` 為 Azure Machine Learning Studio (傳統) 模組，必須建立 XML 定義檔，以指定 **自訂加入資料列** 模組的外觀和行為。 
...
若要在 Machine Learning 工作區中註冊它們，請移至 Azure Machine Learning Studio (傳統) 中的工作區，按一下底部的 [ **+ 新增** ] 按鈕，然後 **從 ZIP 套件選擇 [模組->** ]，以上傳新的 [新增資料 **列** ] 模組。
...
模組的結果可能具決定性或不具決定性。** 依預設，所有模組都視為具決定性。 亦即，假設有一組不變的輸入參數和資料，模組應該會傳回相同的結果 Rand 或函式的執行時間。 由於這個行為，Azure Machine Learning Studio (傳統) 只有在參數或輸入資料已變更時，才會將標示為決定性的模組重新運行。 傳回快取的結果也會讓實驗的執行速度加快許多。
...
然後以 ' CustomAddRows. R ' 中的正確順序，傳回清單中的物件清單：
...
如同輸入和輸出，每個參數都必須有相關聯的唯一識別碼值。 在我們的快速入門範例中，會 *交換* 相關聯的識別碼/參數。
...
**ColumnPicker** ：資料行選取參數。 此類型在 UX 中會轉譯成資料行選擇器。 這裡使用 **Property** 元素來指定要從中選取資料行之埠的識別碼，其中目標埠類型必須是 *DataTable* 。 資料行選取的結果會傳遞至 R 函式，做為包含所選取資料行名稱的字串清單。 
...
* *必要屬性* ： **PortId** -符合輸入專案的識別碼（類型為 *DataTable* ）。
* *選擇性屬性* ：
...
* *選擇性屬性* ：
  * **預設** 值-預設屬性的值必須與其中一個 **專案** 元素的識別碼值相對應。

### <a name="auxiliary-files"></a>輔助檔案
放在自訂模組 ZIP 檔案中的所有檔案在執行期間都可供使用。 所有存在的目錄結構皆會保留。 這表示檔案來源在本機和 Azure Machine Learning Studio 中的運作方式， (傳統) 執行。 
...
例如，假設您想要從資料集移除具有 NAs 的任何資料列，並移除任何重複的資料列，然後再將它輸出到 CustomAddRows 中，而且您已經撰寫一個在 Removedupnarows.r 檔案中執行該動作的 R 函數：
...
接下來，將包含 ' CustomAddRows '、' CustomAddRows.xml ' 和 ' Removedupnarows.r ' 的 zip 檔案上傳為自訂 R 模組。
