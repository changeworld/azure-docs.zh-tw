...
1. 登入 Azure Machine Learning Studio (傳統) Web 服務入口網站 [https://services.azureml.net](https://services.azureml.net) 。 若為傳統 web 服務，您也可以在 Studio (傳統) 的 Machine Learning Studio (傳統) Web 服務] 頁面上，按一下 [ **新的 Web 服務體驗** ] 以進入入口網站。
...
> [!TIP]
>
> 若為傳統 web 服務，Machine Learning Studio 中的 Web 服務儀表板 (傳統) 也有啟用記錄的參數。 不過，由於記錄現在是透過 Web 服務入口網站所管理，您需要透過入口網站來啟用記錄，如本文所述。 如果您已經在 Studio (傳統) 中啟用記錄，請在 Web 服務入口網站中停用記錄，然後再重新啟用。
