...
3. 它會將 _Web 服務輸入_ 和 _輸出_ 模組插入實驗中的預設位置 (這些模組會接受並傳回使用者資料)。
...
更新 Web 服務的一個選擇是以程式設計方式重新定型模型。 如需詳細資訊，請參閱以程式設計方式重新定型 [Machine Learning Studio (傳統) 模型](./retrain-machine-learning-model.md)。
