---
title: ML Studio (傳統) ：使用 API 管理管理 web 服務-Azure
...
* 一個 Azure 帳戶。
* AzureML 帳戶。
* 部署為 Web 服務之 AzureML 實驗的工作區、服務和 API 金鑰。 如需如何建立 AzureML 實驗的詳細資料，請參閱 [Studio 快速入門](create-experiment.md)。 如需如何將 Studio (傳統版) 實驗部署為 Web 服務的資訊，請參閱 [Studio 部署操作說明](deploy-a-machine-learning-web-service.md)以取得如何將 AzureML 實驗部署為 Web 服務的詳細資料。 此外，附錄 A 中的指示說明如何建立及測試簡單的 AzureML 實驗，並將其部署為 Web 服務。
...
![將書籍評論資料集模組連線至專案資料行模組](./media/manage-web-service-endpoints-using-api-management/project-columns.png)

按一下 [選取資料集中的資料行]，然後按一下 [啟動資料行選取器] 並選取 [Col2]。 按一下核取記號以套用這些變更。

![使用資料行名稱來選取資料行](./media/manage-web-service-endpoints-using-api-management/select-columns.png)
