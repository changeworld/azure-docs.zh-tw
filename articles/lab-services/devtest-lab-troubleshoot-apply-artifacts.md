...
1. 登入有問題的虛擬機器。
2. 將腳本複製到虛擬機器的本機，或在下`C:\Packages\Plugins\Microsoft.Compute.CustomScriptExtension\<version>`的虛擬機器上找到它。 這是用來下載成品腳本的位置。
...
- **檢查預設實驗室的儲存體帳戶**（也就是建立實驗室時所建立的第一個儲存體帳戶，其名稱通常以字母 "a" 開頭，並以多位數數位（也就是\<labname\>#）結尾。
    1. 流覽至實驗室的資源群組。
...
如果這些錯誤皆未發生，而您仍然無法套用成品，您可以將 Azure 支援事件。 移至 [ [Azure 支援] 網站](https://azure.microsoft.com/support/options/)，然後選取 [**取得支援**]。
