...
創建實驗室時，它將在虛擬網路的子網中創建。  根據預設，建立這個子網路時會將 [啟用共用公用 IP]**** 設為 [是]**。  此設定會為整個子網路建立一個公用 IP 位址。  如需設定虛擬網路和子網路的詳細資訊，請參閱[設定 Azure DevTest Labs 中的虛擬網路](devtest-lab-configure-vnet.md)。
...
此實驗室中建立任何 VM 會預設為使用共用 IP。  創建 VM 時，可以在**IP 位址配置**下的 **"高級設置"** 頁中觀察到此設置。
...
- **私人：** 您建立的每個 VM 使用私人 IP 位址。 您無法通過遠端桌面直接從 Internet 連接到此 VM。
