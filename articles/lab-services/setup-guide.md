...
針對 Windows VM 的 RDP，建議學生使用 [Microsoft 遠端桌面用戶端](/windows-server/remote/remote-desktop-services/clients/remote-desktop-clients)。 遠端桌面用戶端支援 Mac、Chromebook 和 Windows。
...
如需詳細資訊，請參閱下列文章：
- [Microsoft 小組內的 Azure 實驗室服務](./lab-services-within-teams-overview.md)
...
1. **建立實驗室。** 請參閱建立實驗室的教學課程：
    - [建立教室實驗室](./tutorial-setup-classroom-lab.md#create-a-classroom-lab) 以取得相關指示。
    - [從 Teams 建立實驗室](./how-to-get-started-create-lab-within-teams.md)

    > [!NOTE]
    > 如果課程需要巢狀的虛擬化，請參閱[啟用巢狀虛擬化](./how-to-enable-nested-virtualization-template-vm.md)中的步驟。
...
1. **自訂映像與發佈實驗室 VM。** 連線至稱為範本 VM 的特殊 VM。 請參閱下列指南中的步驟：
    ...
    > [!NOTE]
    > 如果使用的是 Windows，建議另行參閱[準備 Windows 範本 VM](./how-to-prepare-windows-template.md) 中的指示。 這些指示包括供學生使用的 OneDrive 和 Office 設定步驟。
...
1. **新增與管理實驗室使用者**。 若要將使用者新增至實驗室，請參閱下列教學課程中的步驟：
   ...
   - [從小組管理實驗室服務使用者清單](./how-to-manage-user-lists-within-teams.md)
    ...
    如需學生可使用的帳戶類型相關資訊，請參閱[學生帳戶](./how-to-configure-student-usage.md#student-accounts) (機器翻譯)。
...
1. **設定成本控制項。** 若要控制實驗室的成本，請設定排程、配額及自動關機。 請參閱下列教學課程：
   ...
   - [設定使用者的配額](./how-to-configure-student-usage.md#set-quotas-for-users) (機器翻譯) 和 [為特定使用者設定額外的配額](./how-to-configure-student-usage.md#set-additional-quotas-for-specific-users) (機器翻譯)
   ...
   - [在中斷連線時啟用 VM 的自動關機](./how-to-enable-shutdown-disconnect.md)
    ...
    - [在小組內建立及管理實驗室服務排程](./how-to-create-schedules-within-teams.md) 

1. **使用儀表板。** 如需指示，請參閱[使用實驗室的儀表板](./use-dashboard.md) (機器翻譯)。
