...
角色定義 ID 是現有角色定義的字串識別碼。 角色識別碼的格式`/subscriptions/{subscription-id}/providers/Microsoft.Authorization/roleDefinitions/{role-definition-id}`為。 

訂用帳戶識別碼是使用`subscription().subscriptionId`範本函式取得。  

您需要取得內`DevTest Labs User`建角色的角色定義。 若要取得[DevTest Labs 使用者](../role-based-access-control/built-in-roles.md#devtest-labs-user)角色的 GUID，您可以使用 REST API 或[get-azroledefinition 指令程式](/powershell/module/az.resources/get-azroledefinition?view=azps-1.8.0)的[角色指派](/rest/api/authorization/roleassignments)。

```powershell
$dtlUserRoleDefId = (Get-AzRoleDefinition -Name "DevTest Labs User").Id
```

角色識別碼會定義在 variables 區段中，並命名`devTestLabUserRoleId`為。 在範本中，角色識別碼會設定為：111111111-0000-0000-11111111111111111。 
...
### <a name="scope"></a>影響範圍
範圍指定應該套用角色指派的資源或資源群組。 針對資源，範圍的格式為： `/subscriptions/{subscription-id}/resourceGroups/{resource-group-name}/providers/{provider-namespace}/{resource-type}/{resource-name}`。 此範本會使用`subscription().subscriptionId`函式來填入`subscription-id`元件和`resourceGroup().name`樣板函式，以填入`resource-group-name`部分。 使用這些函式表示您要為其指派角色的實驗室，必須存在於目前的訂用帳戶中，以及部署範本所使用的相同資源群組。 最後一個部分`resource-name`是實驗室的名稱。 此值是透過此範例中的樣板參數所接收。 
...
如簡介中所述，您會建立新的 Azure 角色指派，以將使用者新增至實驗室的**DevTest Labs 使用者**角色。 在 PowerShell 中，您可以使用[new-azurermroleassignment](/powershell/module/azurerm.resources/new-azurermroleassignment?view=azurermps-6.13.0) Cmdlet 來執行此動作。 此 Cmdlet 有許多選擇性參數可讓您彈性。 可以`ObjectId`將`SigninName`、或`ServicePrincipalName`指定為要授與許可權的物件。  
...
若要指定要授與許可權的`ResourceName`資源，可以透過、 `ResourceType` `ResourceGroup`或的`scope`組合來指定。 使用任何組合的參數時，請提供足夠的資訊給 Cmdlet，以唯一識別 Active Directory 物件（使用者、群組或服務主體）、範圍（資源群組或資源）和角色定義。
...
要授與存取權的物件可以由`objectId`、 `signInName`、 `spn`參數指定。 物件被`scope`授與存取權的實驗室可以透過 url 或`resource-name`、 `resource-type`和`resource-group`參數的組合來識別。
