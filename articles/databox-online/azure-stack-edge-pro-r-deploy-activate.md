---
title: 在 Azure 入口網站中啟用 Azure Stack Edge Pro R 裝置的教學課程 | Microsoft Docs
description: 部署 Azure Stack Edge Pro R 的教學課程將說明如何啟動實體裝置。
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
ms.locfileid: "96463629"
---
# <a name="tutorial-activate-azure-stack-edge-pro-r-device"></a>教學課程：啟動 Azure Stack Edge Pro R 裝置

本教學課程說明如何使用本機 Web UI 來啟動 Azure Stack Edge Pro R 裝置。
...
* 對於實體裝置： 
    ...
    - 您已依照[設定網路、計算網路 Web Proxy](azure-stack-edge-pro-r-deploy-configure-network-compute-web-proxy.md) 中的詳細說明，設定網路和計算網路設定
    - 如果您已透過 **裝置** 頁面變更裝置名稱或 DNS 網域，則您已在裝置上傳自己或產生的裝置憑證。 如果尚未完成此步驟，您會在裝置啟用期間看到錯誤，而且系統會封鎖啟用。 如需詳細資訊，請移至[設定憑證](azure-stack-edge-placeholder.md)。
    
* 您具有先前建立用以管理 Azure Stack Edge Pro R 裝置的 Azure Stack Edge 服務適用的啟用金鑰。 如需詳細資訊，請移至[準備部署 Azure Stack Edge Pro R](azure-stack-edge-pro-r-deploy-prep.md)。
...
2. 在 **啟用** 圖格上，選取 [啟動]。 
    ...
    ![本機 Web UI 的「雲端詳細資料」頁面 1](./media/azure-stack-edge-pro-r-deploy-activate/activate-1.png)
...
5. 首先，該裝置會啟動。 系統接著會提示您下載金鑰檔案。
    ...
    ![本機 Web UI 的「雲端詳細資料」頁面 3](./media/azure-stack-edge-pro-r-deploy-activate/activate-3.png)
...
若要了解如何使用 Azure Stack Edge Pro R 裝置來傳輸資料，請參閱：

> [!div class="nextstepaction"]
> [使用 Azure Stack Edge Pro R 傳輸資料](./azure-stack-edge-j-series-deploy-add-shares.md)
