...
ms.locfileid: "90985641"
---
# <a name="what-is-the-azure-stack-edge-mini-r"></a>Azure Stack Edge Mini R 是什麼？

Azure Stack Edge Mini R 是一種硬體即服務解決方案。 Microsoft 提供一個堅固的雲端受控便攜裝置，該裝置具有內建的視覺處理單元 (VPU) 可啟用加速的 AI 推斷，並具備網路儲存體閘道的所有功能。 這些裝置適合在最嚴峻的環境中使用，並已針對 AI、分析和無伺服器運算最佳化。
...
|支援的通訊協定     |支援使用標準 SMB、NFS 與 REST 通訊協定擷取資料。 <br> 如需支援版本的詳細資訊，請移至 [ 系統需求](azure-stack-edge-gpu-system-requirements.md)。|
...
* **Azure Stack Edge Mini R 實體裝置** - 備有電池的便攜式裝置，搭配 Microsoft 提供的堅固外殼，可設定將資料傳送至 Azure。 裝置重量加上電池低於 7 磅。
...
* **Azure Stack Edge Mini R 本機 Web UI** - 使用本機 Web UI 對 Azure Stack Edge Mini R 裝置執行起始裝置設定、執行診斷、關機和重新啟動、檢視複製記錄，以及連絡 Microsoft 支援服務以提出服務要求。
