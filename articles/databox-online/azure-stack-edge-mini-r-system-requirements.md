---
title: Microsoft Azure Stack Edge 迷你 R 系統需求 |Microsoft Docs
description: 瞭解 Azure Stack Edge 迷你 R 的軟體和網路需求
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
ms.locfileid: "96466276"
---
# <a name="azure-stack-edge-mini-r-system-requirements"></a>Azure Stack Edge 迷你 R 系統需求

本文說明 Microsoft Azure Stack Edge 迷你 R 解決方案以及連接至 Azure Stack Edge 迷你 R 之用戶端的重要系統需求。建議您先仔細檢查資訊，再部署 Azure Stack Edge 迷你 R。您可以視需要在部署和後續作業期間回頭參考此資訊。

Azure Stack Edge 迷你 R 的系統需求包括：
...
當您連接到本機 Azure Resource Manager 時，這些儲存體帳戶會透過裝置本機 Api 來進行。 以下是支援的儲存體帳戶：
...
### <a name="port-requirements-for-azure-stack-edge-mini-r"></a>Azure Stack Edge 迷你 R 的埠需求

下表列出必須在您的防火牆中開啟的連接埠，以允許 SMB、雲端或管理流量。 在這個資料表中，in 或 inbound 指的是輸入用戶端要求存取裝置的方向。 *輸出* 或 *輸出* 是指您的 Azure Stack Edge 迷你 R 裝置在外部傳送資料的方向，例如，輸出到網際網路。
...
網路系統管理員通常可以根據 URL 模式設定進階防火牆規則，來篩選輸入和輸出流量。 您 Azure Stack Edge 迷你 R 裝置和服務取決於其他 Microsoft 應用程式，例如 Azure 服務匯流排、Azure Active Directory 存取控制、儲存體帳戶和 Microsoft Update 伺服器。 與這些應用程式相關聯的 URL 模式可以用來設定防火牆規則。 請務必了解與這些應用程式相關聯的 URL 模式可以變更。 這些變更需要網路系統管理員視需要監視和更新您 Azure Stack Edge 迷你 R 的防火牆規則。

建議您根據 Azure Stack Edge 迷你 R 固定 IP 位址來設定輸出流量的防火牆規則，在大部分情況下壓縮。 不過，您可以使用下列資訊設定建立安全環境所需的進階防火牆規則。
...
在開發和測試您的解決方案時使用您的體驗，以確保您的 Azure Stack Edge 迷你 R 裝置上有足夠的容量，而且您可以從裝置獲得最佳效能。
...
- 可透過裝置的 PowerShell 介面使用的監視命令，例如：
    ...
    如需可用命令的詳細資訊，請移至 [ Debug Kubernetes 問題](azure-stack-edge-gpu-connect-powershell-interface.md#debug-kubernetes-issues-related-to-iot-edge)。

最後，請務必在您的資料集上驗證您的解決方案，並在生產環境中進行部署之前，先量化 Azure Stack Edge 迷你 R 的效能。
