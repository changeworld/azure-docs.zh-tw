---
title: Microsoft Azure Stack 邊緣技術規格和合規性 |Microsoft Docs
description: 瞭解您 Azure Stack 邊緣的技術規格和合規性
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
ms.locfileid: "82929004"
---
# <a name="azure-stack-edge-technical-specifications"></a>Azure Stack Edge 技術規格

Microsoft Azure Stack 邊緣裝置的硬體元件會遵循本文中所述的技術規格和法規標準。 技術規格描述電源供應器（Psu）、存放裝置容量、主機殼和環境標準。 
...
欄位可程式化閘道陣列（FPGA）包含在啟用 Machine Learning （ML）案例的每個 Azure Stack 邊緣裝置上。 
...
Azure Stack Edge 裝置具有具有高效能風扇的兩個 100-240 V 電源供應器（Psu）。 這兩個 Psu 提供了多餘的電源設定。 如果 PSU 失敗，裝置會繼續在另一個 PSU 上正常運作，直到更換故障的模組為止。 下表列出 Psu 的技術規格。
...
您的 Azure Stack Edge 裝置有6個網路介面 PORT1-PORT6。
...
Azure Stack Edge 裝置有 9 X 2.5 "NVMe Ssd，每個都有 1.6 TB 的容量。 在這些 Ssd 中，1是作業系統磁片，另8則是資料磁片。 裝置的可用容量總計約為 12.5 TB。 下表包含裝置儲存體容量的詳細資料。
...
裝置套件會權衡61磅。 和需要兩個人員來處理它。 裝置的權數視主機殼的設定而定。
...
|-----------------------|--------------------------------------|--------------------------------------|---------------------------|
|    作業        |    10° C-35 ° C （50° F-86 ° F）         |    10%-80% 非冷凝。         |    29°C (84°F)            |
