---
title: 在 Azure 入口網站中設定搭配 GPU 的 Azure Stack Edge 裝置憑證的教學課程 | Microsoft Docs
description: 部署搭配 GPU 的 Azure Stack Edge GPU 的教學課程將說明如何在實體裝置上設定憑證。
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
ms.locfileid: "89268171"
---
# <a name="tutorial-configure-certificates-for-your-azure-stack-edge-with-gpu"></a>教學課程：為您搭配 GPU 的 Azure Stack Edge 設定憑證

本教學課程說明如何使用本機 Web UI 來設定搭配上線 GPU 的 Azure Stack Edge 裝置憑證。
...
2. 根據您是否已在**裝置設定**圖格中變更裝置名稱或 DNS 網域，可以為憑證選擇下列其中一個選項。
    ...
    - 如果您變更了裝置名稱或 DNS 網域，則若要成功啟用您的裝置，可以選擇下列其中一個選項： 
    
        - **產生所有裝置憑證**。 裝置憑證僅供測試之用，不適用於生產工作負載。** 如需詳細資訊，請移至[在您的 Azure Stack Edge上產生裝置憑證](#generate-device-certificates)。

        - **攜帶您自己的憑證**。 您可以攜帶自己的已簽署端點憑證和對應的簽署鏈結。 您必須先新增簽署鏈結，才能上傳端點憑證。 **建議您一律攜帶自己的憑證以供生產工作負載使用。** 如需詳細資訊，請移至[攜帶您自己的憑證以供 Azure Stack Edge 裝置使用](#bring-your-own-certificates)。
    
        - 您可以攜帶一些自己的憑證，同時產生一些裝置憑證。 **產生憑證**選項只會重新產生裝置憑證。

    - 如果您變更了裝置名稱或 DNS 網域，但未產生憑證或攜帶您自己的憑證，則會封鎖啟用。
...
使用這些步驟來重新產生並下載 Azure Stack Edge 裝置憑證：
...
3. 在**憑證**頁面上，您會看到系統現在已填入**下載**欄，且可以使用下載重新產生的憑證連結。 
...
若要在用來存取裝置的用戶端上匯入及安裝這些憑證，請遵循[在存取您 Azure Stack Edge 裝置的用戶端上匯入憑證](azure-stack-edge-j-series-manage-certificates.md#import-certificates-on-the-client-accessing-the-device)中的步驟。 
...
若要了解如何啟動您的 Azure Stack Edge 裝置，請參閱：

> [!div class="nextstepaction"]
> [啟動 Azure Stack Edge 裝置](./azure-stack-edge-gpu-deploy-activate.md)
