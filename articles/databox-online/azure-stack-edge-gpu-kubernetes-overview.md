---
title: 概述 Microsoft Azure Stack Edge 裝置上的 Kubernetes 叢集 |Microsoft Docs
description: 描述如何在 Azure Stack Edge 裝置上執行 Kubernetes。
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
在您的 Azure Stack Edge 裝置上，您可以藉由設定計算來建立 Kubernetes 叢集。 設定計算角色時，系統會為您部署並設定包含主要和背景工作節點的 Kubernetes 叢集。 此叢集接著會用於透過 `kubectl` 、IoT Edge 或 Azure Arc 的工作負載部署。

Azure Stack Edge 裝置可作為構成基礎結構叢集的1節點設定。 Kubernetes 叢集不同于基礎結構叢集，並部署在基礎結構叢集之上。 基礎結構叢集會為您的 Azure Stack Edge 裝置提供持續性儲存體，而 Kubernetes 叢集僅負責應用程式協調流程。 
...
下圖說明在1個節點 Azure Stack Edge 裝置上執行 Kubernetes 的程式。 1個節點的裝置不具高可用性，而且如果單一節點失敗，則裝置會關閉。 Kubernetes 叢集也會停止運作。

![Azure Stack Edge 裝置的1節點架構 Kubernetes](media/azure-stack-edge-gpu-kubernetes-overview/kubernetes-architecture-1-node.png)
...
若要支援應用程式工作負載，您可以在 Azure Stack Edge 裝置共用上掛接存放磁片區，以取得持續性資料。 可以使用靜態與動態磁碟區。 
...
當新的 Kubernetes 版本可供使用時，您的叢集可以使用 Azure Stack Edge 裝置可用的標準更新進行升級。 如需有關如何升級的步驟，請參閱套用 [Azure Stack Edge 的更新](azure-stack-edge-gpu-install-update.md)。

## <a name="access-monitoring"></a>存取，監視

Azure Stack Edge 裝置上的 Kubernetes 叢集允許 (RBAC) 的角色型存取控制。 如需詳細資訊，請參閱 [Azure Stack Edge 裝置上的 Kubernetes 叢集角色型存取控制](azure-stack-edge-gpu-kubernetes-rbac.md)。

您也可以透過 Kubernetes 儀表板來監視叢集和資源的健康情況。 也可以使用容器記錄。 如需詳細資訊，請參閱 [使用 Kubernetes 儀表板來監視您 Azure Stack Edge 裝置上的 Kubernetes 叢集健康](azure-stack-edge-gpu-monitor-kubernetes-dashboard.md)情況。
...
在 Azure Stack Edge 裝置上建立 Kubernetes 叢集之後，您可以透過下列任一方法來管理此叢集上部署的應用程式：
...
如需部署 Kubernetes 叢集的詳細資訊，請移至 [Azure Stack Edge 裝置上的部署 Kubernetes](azure-stack-edge-gpu-create-kubernetes-cluster.md)叢集。 如需管理的相關資訊，請移至 [使用 kubectl 來管理 Azure Stack Edge 裝置上的 Kubernetes](azure-stack-edge-gpu-create-kubernetes-cluster.md)叢集。


### <a name="kubernetes-and-iot-edge"></a>Kubernetes 和 IoT Edge

Kubernetes 也可以與 Azure Stack Edge 裝置上 IoT Edge 工作負載整合，其中 Kubernetes 會提供規模調整，而生態系統和 IoT 則提供以 IoT 為主的生態系統。 Kubernetes 層用來作為基礎結構層，以部署 Azure IoT Edge 工作負載。 模組存留期和網路負載平衡是由 Kubernetes 管理，而 edge 應用程式平臺則是由 IoT Edge 管理。

如需透過 IoT Edge 在您的 Kubernetes 叢集上部署應用程式的詳細資訊，請移至： 

- 透過[IoT Edge 公開 Azure Stack Edge 裝置上的無狀態應用程式](azure-stack-edge-gpu-deploy-stateless-application-iot-edge-module.md)。
...
- 深入瞭解 [Azure Stack Edge 裝置](azure-stack-edge-gpu-kubernetes-storage.md)上的 Kubernetes 儲存體。
- 瞭解 [Azure Stack Edge 裝置](azure-stack-edge-gpu-kubernetes-networking.md)上的 Kubernetes 網路模型。
