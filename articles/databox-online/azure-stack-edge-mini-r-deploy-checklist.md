---
title: 部署 Azure Stack Edge 迷你 R 裝置的預先部署檢查清單
description: 本文說明在您部署 Azure Stack Edge 迷你 R 裝置之前，可以搜集的資訊。
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
ms.locfileid: "96466395"
---
# <a name="deployment-checklist-for-your-azure-stack-edge-mini-r-device"></a>Azure Stack Edge 迷你 R 裝置的部署檢查清單  

本文說明可在 Azure Stack Edge 迷你 R 裝置的實際部署之前搜集的資訊。 

使用下列檢查清單，以確保您在下一 Azure Stack Edge 迷你 R 裝置的訂單，以及在收到裝置之前，擁有這項資訊。 

## <a name="deployment-checklist"></a>部署檢查清單 

| 階段                             | 參數                                                                                                                                                                                                                           | 詳細資料                                                                                                           |
|-----------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------|
| 裝置管理               | <li>Azure 訂用帳戶</li><li>已註冊資源提供者</li><li>Azure 儲存體帳戶</li>|<li>啟用 Azure Stack Edge 迷你 R/Data Box Gateway、擁有者或參與者存取權。</li><li>在 Azure 入口網站中，前往 **Home > 訂用帳戶 > 訂用帳戶 > 資源提供者**。 搜尋 `Microsoft.DataBoxEdge` 並註冊。 如果要 `Microsoft.Devices` 部署 IoT 工作負載，請重複執行。</li><li>需要存取認證</li> |
...
| 啟用  | 需要 Azure Stack Edge 資源的啟用金鑰。    | 一旦產生，金鑰會在3天內到期。 |
...
準備部署您的 [Azure Stack Edge 迷你 R 裝置](azure-stack-edge-gpu-deploy-prep.md)。
