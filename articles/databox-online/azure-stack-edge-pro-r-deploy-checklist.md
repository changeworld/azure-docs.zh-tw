---
title: 部署 Azure Stack Edge Pro R 裝置的預先部署檢查清單 |Microsoft Docs
description: 本文說明在您部署 Azure Stack Edge Pro R 裝置之前，可以搜集的資訊。
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
ms.locfileid: "96466334"
---
# <a name="deployment-checklist-for-your-azure-stack-edge-pro-r-device"></a>Azure Stack Edge Pro R 裝置的部署檢查清單  

本文說明可在 Azure Stack Edge Pro R 裝置的實際部署之前搜集的資訊。 

使用下列檢查清單，以確保在您將 Azure Stack Edge Pro R 裝置的訂單，以及收到裝置之前，有這項資訊。 
...
|  (選用) 憑證  | 如果您自備憑證 (包括簽署鏈結)，請以適當的格式[新增憑證](azure-stack-edge-pro-r-deploy-configure-certificates-vpn-encryption.md#bring-your-own-certificates)。| 只有當您變更裝置名稱和（或） DNS 網域時，才需要設定憑證。 |
| VPN  | <!--Need VPN certificate, VPN gateway, firewall setup in Azure,  passphrase and region info VPN scripts. -->   | |
| 待用資料加密  | 建議使用自動產生的加密金鑰。   |如果使用您自己的金鑰，則需要32個字元的完整 Base-64 編碼金鑰。  |
...
準備部署 [Azure Stack Edge Pro 裝置](azure-stack-edge-pro-r-deploy-prep.md)。
