...
Azure Cache for Redis Standard 和 Premium 層提供內建的虛擬機器，方法是將每個快取裝載在兩部專用的虛擬機器上， (Vm) 。 雖然這些 Vm 位於不同的 [Azure 容錯和更新網域](../virtual-machines/manage-availability.md) ，且具有高可用性，但它們很容易發生資料中心層級的失敗。 Azure Cache for Redis 也支援其進階層中的區域冗余。 區域冗余快取會在分散到多個 [可用性區域](../virtual-machines/manage-availability.md#use-availability-zones-to-protect-from-datacenter-level-failures)的 vm 上執行。 它提供更高的復原能力和可用性。
