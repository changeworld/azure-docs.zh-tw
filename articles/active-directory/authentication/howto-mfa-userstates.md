---
title: 每使用者多重身份驗證 ─ Azure 的動作目錄
description: 通過更改 Azure 多重身份驗證中的用戶狀態啟用 MFA。
...
**由條件存取策略啟用**- 這是為使用者啟用兩步驗證的最靈活方法。 啟用"使用條件存取"策略僅適用於雲中的 Azure MFA,是 Azure AD 的進階功能。 如需這個方法的詳細資訊，請參閱[部署雲端式 Azure Multi-Factor Authentication](howto-mfa-getstarted.md)。

**由 Azure AD Identity Protection 啟用** - 這個方法使用 Azure AD Identity Protection 風險原則，要求所有雲端應用程式進行只根據登入風險的雙步驟驗證。 這個方法需要 Azure Active Directory P2 授權。 有關此方法的詳細資訊,請參閱[Azure 活動目錄識別保護](../identity-protection/howto-sign-in-risk-policy.md)
...
> [!IMPORTANT]
> 通過條件存取策略啟用 Azure MFA 不會更改用戶的狀態。 不要驚慌用戶顯示為禁用。 條件訪問不會更改狀態。 **如果使用者正在使用條件訪問策略,則組織不應啟用或強制使用者。**
...
3. 選擇**多重身份認證**。 您可能需要向右滾動才能看到此功能表選項。 選擇下面的範例螢幕擷取以檢視完整的 Azure 門戶視窗和選單位置:[![](media/howto-mfa-userstates/selectmfa-cropped.png "從 Azure AD 中的「使用者」視窗選擇多重身份驗證")](media/howto-mfa-userstates/selectmfa.png#lightbox)
4. 隨即開啟新的頁面，以顯示使用者狀態。
...
4. 在右邊的**快速步驟**下，選擇 [啟用]**** 或 [停用]****。
   ![點選「快速步驟」選單上的「啟用」啟用選取的使用者](./media/howto-mfa-userstates/user1.png)
