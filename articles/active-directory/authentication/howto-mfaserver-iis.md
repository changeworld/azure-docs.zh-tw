---
title: IIS 身份驗證和 Azure MFA 伺服器 - Azure 活動目錄
...
![MFA 伺服器中的 IIS 身份驗證](./media/howto-mfaserver-iis/iis.png)
...
2. 按一下 [表單架構]**** 索引標籤。
3. 按一下 **[新增]**。
4. 要自動檢測使用者名、密碼和域變數，請在"自動設定基於表單的網站`https://localhost/contoso/auth/login.aspx`"對話方塊中輸入登錄 URL（如 ），然後按一下"**確定**"。
...
10. 如果所有使用者都已或即將導入伺服器並接受多重要素驗證，則選中 **"需要 Azure 多重要素驗證"使用者匹配**框。 如果有大量使用者尚未匯入伺服器及/或將免除多重要素驗證，請勿核取此方塊。
...
2. 按一下 [HTTP]**** 索引標籤。
3. 按一下 **[新增]**。
4. 在"添加基本 URL"對話方塊中，輸入執行 HTTP 身份驗證的網站的 URL（<http://localhost/owa>如 ），並提供應用程式名稱（可選）。 應用程式名稱會出現在 Azure Multi-Factor Authentication 報表中，而且可能顯示在簡訊或行動應用程式驗證訊息內。
...
1. 如果在 IIS 6 上運行，請按一下**ISAPI**選項卡。選擇 Web 應用程式運行下的網站（例如預設網站），以啟用該網站的 Azure 多重要素驗證 ISAPI 篩選器外掛程式。
2. 如果在 IIS 7 或更高版本上運行，請按一下**本機模組**選項卡。選擇伺服器、網站或應用程式以在所需級別啟用 IIS 外掛程式。
...
2. 按一下 **[新增]**。
3. 當"添加受信任的 IP"對話方塊出現時，選擇 **"單個 IP、IP****範圍**"或 **"子網**"選項按鈕。
