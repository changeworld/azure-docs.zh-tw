...
2. 按一下 [表單架構]**** 索引標籤。
3. 按一下 [加入]  。
4. 若要自動偵測使用者名稱、密碼和網域變數，請在 [自動`https://localhost/contoso/auth/login.aspx`設定表單架構網站] 對話方塊中輸入登入 URL （例如），然後按一下 **[確定]**。
...
8. 選取正確的 [要求格式]。 這會設定為 [ **POST] 或 [取得**]，用於大部分的 web 應用程式。
9. 輸入 [使用者名稱變數]、[密碼變數] 和 [網域變數] \(如果出現在登入頁面上)。 若要尋找輸入方塊的名稱，請在網頁瀏覽器中瀏覽至登入頁面，在該頁面上按一下滑鼠右鍵，然後選取 [檢視原始檔]****。
10. 如果所有使用者都已或將匯入至伺服器，且受限於多重要素驗證，請核取 [**需要 Azure 多因素驗證使用者**比對] 方塊。 如果有大量使用者尚未匯入伺服器及/或將免除多重要素驗證，請勿核取此方塊。
...
2. 按一下 [HTTP]**** 索引標籤。
3. 按一下 [加入]  。
4. 在 [加入基底 URL] 對話方塊中，輸入執行 HTTP 驗證之網站的 URL （例如<http://localhost/owa>），並提供應用程式名稱（選擇性）。 應用程式名稱會出現在 Azure Multi-Factor Authentication 報表中，而且可能顯示在簡訊或行動應用程式驗證訊息內。
...
2. 按一下 [加入]  。
3. 當 [新增信任的 Ip] 對話方塊出現時，請選取 [**單一 ip**]、[ **ip 範圍**] 或 [**子網**] 選項按鈕。
