---
title: 設定 Azure 多重身份認證 ─ Azure 的動作目錄
...
您可以通過瀏覽 Azure**活動目錄** > **安全** > **MFA**從 Azure 門戶訪問與 Azure 多重身份驗證相關的設置。
...
| [封鎖/解除封鎖使用者](#block-and-unblock-users) | 用於阻止特定使用者接收多重身份驗證請求。 系統會自動拒絕已封鎖使用者的任何驗證嘗試。 使用者自封鎖的時刻起，將持續封鎖 90 天。 |
| [欺詐警報](#fraud-alert) | 設定與使用者報告詐騙性認證要求的能力相關的設定 |
...
2. 瀏覽到**Azure 的目錄** > **安全** > **MFA** > **封鎖/取消封鎖使用者**。
3. 選取 [新增]**** 以封鎖使用者。
4. 選取 [複寫群組]****。 輸入被封鎖使用者的**使用者名稱,\@並使用使用者名稱domain.com**。 在 [原因]**** 欄位中輸入註解。
5. 選取 [新增]**** 以完成封鎖使用者。
...
* **初始問候語期間以代碼報告詐騙**：當使用者收到電話以執行雙步驟驗證時，他們一般會按下 **#** 來確認登入。 要報告欺詐,使用者在按**#** 之前輸入代碼。 根據預設，此代碼是 **0**，但您可以自訂。
...
2. 選擇**Azure 的目錄** > **登入**。欺詐報告現在是標準 Azure AD 登錄報告的一部分。

## <a name="notifications"></a>通知

在此處為將收到欺詐警報電子郵件的使用者配置電子郵寄地址。
...
在美國,如果您尚未配置 MFA 呼叫者 ID,則來自 Microsoft 的語音呼叫來自以下號碼:+1 (866) 539 4191、+1 (855) 330 8653 和 +1 (877) 668 6536。 如果使用垃圾郵件篩檢程式,請確保排除這些數位。
...
1. 以管理者登入[Azure 門戶](https://portal.azure.com)。
1. 瀏覽到**Azure 的目錄** > **安全** > **MFA** > **電話來呼叫設定**。
1. 選取 [新增問候語]****。
1. 選擇問候語的類型。
1. 選擇語言。
...
### <a name="custom-voice-message-defaults"></a>自訂語音信件預設值
...
| 問候文(標準) | 感謝您使用微軟的登錄驗證系統。 請按英鎊鍵完成驗證。 |
| 問候文 (PIN) | 感謝您使用微軟的登錄驗證系統。 請輸入您的 PIN,後跟英鎊金鑰以完成驗證。 |
...
1. 以管理者登入[Azure 門戶](https://portal.azure.com)。
2. 瀏覽到**Azure 活動目錄** > **安全** > **MFA** > **一次性繞過**。
3. 選取 [新增]  。
4. 如有必要，請選取此許可的複寫群組。
5. 輸入使用者名稱作為**使用者\@名 domain.com**。 輸入許可需持續的秒數。 輸入許可的原因。
...
Azure Multi-Factor Authentication 的應用程式密碼、受信任的 IP、驗證選項和成員多重要素驗證等設定，位於服務設定中。 可以通過瀏覽 Azure**活動目錄** > **安全** > **MFA** > **開始** > **配置** > **其他基於雲的 MFA 設置**從 Azure 門戶訪問服務設置。
...
>[!NOTE]
>應用密碼不適用於基於條件訪問的多重身份驗證策略和現代身份驗證。
...
> [!NOTE]
> MFA 受信任的 IP 和條件存取命名位置僅適用於 IPV4 位址。
...
### <a name="enable-named-locations-by-using-conditional-access"></a>使用條件存取檔案位置

1. 登入 [Azure 入口網站](https://portal.azure.com)。
2. 在左邊,選擇**Azure 的目錄** > **安全** > **條件存取** > **命名位置**。
3. 選取 [新增位置]****。
4. 輸入位置的名稱。
5. 選擇 **「標記為受信任位置**」 。
...
### <a name="enable-the-trusted-ips-feature-by-using-conditional-access"></a>使用條件存取存取信任的 IP 功能
...
4. 在 [服務設定]**** 頁面的 [信任的 IP]**** 下方，選擇下列兩個選項中的任一項：
   ...
   * **適用於從特定公用 IP 範圍提出的要求**：若要選擇此選項，請在文字方塊中，使用 CIDR 表示法輸入 IP 位址。
      ...
      * 對於單個 IP 位址,**請使用 xxx.xxx.xxx.xxx/32**等符號。
...
非瀏覽器應用程式上不會顯示 [X 天內不要再問我]**** 選項，不論應用程式是否支援新式驗證。 這些套用每小時提供新的存取權杖_的刷新權杖_。 當已驗證重新整理權杖時，Azure AD 會檢查在指定天數內發生的最後一次雙步驟驗證。
...
>
>**記住多重身份驗證**功能與 B2B 使用者不相容,在登錄到受邀的租戶時,B2B 使用者將不可見。
>
