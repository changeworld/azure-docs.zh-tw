---
title: 配置 Azure 多重要素驗證 - Azure 活動目錄
...
您可以通過流覽 Azure**活動目錄** > **安全** > **MFA**從 Azure 門戶訪問與 Azure 多重要素驗證相關的設置。
...
| [封鎖/解除封鎖使用者](#block-and-unblock-users) | 用於阻止特定使用者接收多重要素驗證請求。 系統會自動拒絕已封鎖使用者的任何驗證嘗試。 使用者自封鎖的時刻起，將持續封鎖 90 天。 |
| [欺詐警報](#fraud-alert) | 配置與使用者報告欺詐性驗證請求的能力相關的設置 |
...
2. 流覽到**Azure 活動目錄** > **安全** > **MFA** > **阻止/取消阻止使用者**。
3. 選取 [新增]**** 以封鎖使用者。
4. 選取 [複寫群組]****。 輸入被阻止使用者的**使用者名作為使用者名\@domain.com**。 在 [原因]**** 欄位中輸入註解。
5. 選取 [新增]**** 以完成封鎖使用者。
...
* **初始問候語期間以代碼報告詐騙**：當使用者收到電話以執行雙步驟驗證時，他們一般會按下 **#** 來確認登入。 要報告欺詐，使用者在按 之前**#** 輸入代碼。 根據預設，此代碼是 **0**，但您可以自訂。
...
2. 選擇**Azure 活動目錄** > **登錄**。欺詐報告現在是標準 Azure AD 登錄報告的一部分。

## <a name="notifications"></a>通知

在此處為將收到欺詐警報電子郵件的使用者配置電子郵件地址。

![通知欺詐警報電子郵件示例](./media/howto-mfa-mfasettings/multi-factor-authentication-fraud-alert-email.png)
...
在美國，如果您尚未配置 MFA 呼叫者 ID，則來自 Microsoft 的語音電話來自以下號碼：+1 （866） 539 4191、+1 （855） 330 8653 和 +1 （877） 668 6536。 如果使用垃圾郵件篩檢程式，請確保排除這些數位。
...
1. 以管理員身份登錄到[Azure 門戶](https://portal.azure.com)。
1. 流覽到**Azure 活動目錄** > **安全** > **MFA** > **撥打電話設置**。
1. 選取 [新增問候語]****。
1. 選擇問候語的類型。
1. 選擇語言。
...
### <a name="custom-voice-message-defaults"></a>自訂語音信箱預設值
...
| --- | --- |
| 身份驗證成功 | 您的登錄已成功驗證。 再見。 |
| 擴展提示 | 感謝您使用微軟的登錄驗證系統。 請按磅鍵繼續。 |
| 欺詐確認 | 已提交欺詐警報。 要取消阻止您的帳戶，請聯繫您公司的 IT 説明台。 |
| 欺詐問候語（標準） | 感謝您使用微軟的登錄驗證系統。 請按英鎊鍵完成驗證。 如果您未啟動此驗證，則可能有人試圖訪問您的帳戶。 請按零磅提交欺詐警報。 這將通知公司的 IT 團隊並阻止進一步的驗證嘗試。 |
| 欺詐報告 已提交欺詐警報。 | 要取消阻止您的帳戶，請聯繫您公司的 IT 説明台。 |
| 啟用 | 感謝您使用微軟的登錄驗證系統。 請按英鎊鍵完成驗證。 |
| 身份驗證被拒絕重試 | 驗證被拒絕。 |
| 重試（標準） | 感謝您使用微軟的登錄驗證系統。 請按英鎊鍵完成驗證。 |
| 問候語（標準） | 感謝您使用微軟的登錄驗證系統。 請按英鎊鍵完成驗證。 |
| 問候語 （PIN） | 感謝您使用微軟的登錄驗證系統。 請輸入您的 PIN，後跟英鎊金鑰以完成驗證。 |
| 欺詐問候語 （PIN） | 感謝您使用微軟的登錄驗證系統。  請輸入您的 PIN，後跟英鎊金鑰以完成驗證。 如果您未啟動此驗證，則可能有人試圖訪問您的帳戶。 請按零磅提交欺詐警報。 這將通知公司的 IT 團隊並阻止進一步的驗證嘗試。 |
| 重試（PIN） | 感謝您使用微軟的登錄驗證系統。 請輸入您的 PIN，後跟英鎊金鑰以完成驗證。 |
| 數位後擴展提示 | 如果已在此擴展處，請按磅鍵繼續。 |
| 身份驗證被拒絕 | 對不起，我們現在不能簽你。 請稍後再試一次。 |
| 啟動問候語（標準） | 感謝您使用微軟的登錄驗證系統。 請按英鎊鍵完成驗證。 |
| 啟動重試（標準） | 感謝您使用微軟的登錄驗證系統。 請按英鎊鍵完成驗證。 |
| 啟動問候語 （PIN） | 感謝您使用微軟的登錄驗證系統。 請輸入您的 PIN，後跟英鎊金鑰以完成驗證。 |
| 數位前的擴展提示 | 感謝您使用微軟的登錄驗證系統。 請轉接此呼叫到分機... |
...
1. 以管理員身份登錄到[Azure 門戶](https://portal.azure.com)。
2. 流覽到**Azure 活動目錄** > **安全** > **MFA** > **一次性繞過**。
3. 選取 [加入]****。
4. 如有必要，請選取此許可的複寫群組。
5. 輸入使用者名作為**使用者名\@domain.com**。 輸入許可需持續的秒數。 輸入許可的原因。
6. 選取 [加入]****。 時間限制會立即生效。 使用者必須在單次許可到期之前登入。
...
Azure Multi-Factor Authentication 的應用程式密碼、受信任的 IP、驗證選項和成員多重要素驗證等設定，位於服務設定中。 可以通過流覽 Azure**活動目錄** > **安全** > **MFA** > **開始** > **配置** > **其他基於雲的 MFA 設置**從 Azure 門戶訪問服務設置。
...
>[!NOTE]
>應用密碼不適用於基於條件訪問的多重要素驗證策略和現代身份驗證。
...
> [!NOTE]
> MFA 受信任的 IP 和條件訪問命名位置僅適用于 IPV4 位址。

如果您的組織部署 NPS 延伸模組以提供 MFA 給內部部署應用程式，請注意來源 IP 位址一律會顯示為驗證嘗試流經的 NPS 伺服器。

| Azure AD 租用戶類型 | 信任的 IP 功能選項 |
|:--- |:--- |
| 受控 |**特定 IP 位址的範圍**：對於從公司內部網路登入的使用者，管理員會指定允許略過雙步驟驗證的 IP 位址範圍。 最多可以配置 50 個受信任的 IP 範圍。|
...
### <a name="enable-named-locations-by-using-conditional-access"></a>使用條件訪問啟用命名位置

1. 登錄到 Azure[門戶](https://portal.azure.com)。
2. 在左側，選擇**Azure 活動目錄** > **安全** > **條件訪問** > **命名位置**。
3. 選取 [新增位置]****。
4. 輸入位置的名稱。
5. 選擇 **"標記為受信任位置**"。
...
### <a name="enable-the-trusted-ips-feature-by-using-conditional-access"></a>使用條件訪問啟用受信任的 IP 功能
...
4. 在 [服務設定]**** 頁面的 [信任的 IP]**** 下方，選擇下列兩個選項中的任一項：
   ...
   * **適用於從特定公用 IP 範圍提出的要求**：若要選擇此選項，請在文字方塊中，使用 CIDR 表示法輸入 IP 位址。
      ...
      * 對於單個 IP 位址，**請使用xxx.xxx.xxx.xxx/32**等符號。
...
| 電話簡訊 |傳送包含驗證碼的簡訊。 系統會提示使用者在登入介面中輸入這個驗證碼。 此程序稱為「單向 SMS」。 雙向 SMS 表示使用者必須以簡訊回傳特定驗證碼。 雙向 SMS 已淘汰，且在 2018 年 11 月 14 之後不支援。 管理員應該為以前使用雙向 SMS 的使用者啟用另一種方法。|
...
>[!IMPORTANT]
>若帳戶或裝置遭到入侵，請記住受信任裝置的 Multi-Factor Authentication 可能會影響安全性。 如果公司帳戶被洩露或受信任設備丟失或被盜，則應[撤銷 MFA 會話](howto-mfa-userdevicesettings.md)。
>
...
非瀏覽器應用程式上不會顯示 [X 天內不要再問我]**** 選項，不論應用程式是否支援新式驗證。 這些應用使用每小時提供新訪問權杖_的刷新權杖_。 當已驗證重新整理權杖時，Azure AD 會檢查在指定天數內發生的最後一次雙步驟驗證。

此功能會減少 web 應用程式上的驗證次數，一般每次皆會提示。 此功能會增加新式驗證用戶端的驗證次數 (一般會每隔 90 天提示一次)。 與條件訪問策略結合使用時，還可以增加身份驗證數。
...
>
>**記住多重要素驗證**功能與 B2B 使用者不相容，在登錄到受邀的租戶時，B2B 使用者將不可見。
>
