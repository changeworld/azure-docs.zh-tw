...
智慧鎖定有助於鎖定那些試圖猜測使用者密碼或使用暴力方法進入的不良行為者。 此外也會辨識來自有效使用者的登入，並將其視為不同於攻擊者和其他不明來源所進行的登入。 智慧鎖定可在鎖定攻擊者的同時，讓您的使用者仍可繼續存取其帳戶並保有生產力。
...
任何 Azure AD 客戶只要採用兼具適當安全性和可用性的預設設定，智慧鎖定就一律會啟用。 自定義智慧鎖定設定(具有特定於組織的值)需要為使用者付費的 Azure AD 許可證。

使用智慧鎖定並不保證真正的用戶永遠不會被鎖定。當智慧鎖定鎖定使用者帳戶時,我們會盡力不鎖定正版使用者。 鎖定服務會嘗試確保不良參與者無法訪問正版使用者帳戶。  
...
> [!IMPORTANT]
> 目前,如果使用者的雲帳戶已被智慧鎖定功能鎖定,則無法解鎖這些帳戶。 系統管理員必須等待鎖定持續期間結束。 但是,使用者可以使用來自受信任設備或位置的自助密碼重置 (SSPR) 進行解鎖。
...
3. 瀏覽到**電腦設定** > **政策** > **視窗設定** > **安全性設定** > **帳號政策** > **帳戶鎖定原則**。
4. 驗證**您的帳戶鎖定閾值**和**重置帳戶鎖定計數器後**的值。

![變更本地端目錄帳戶鎖定原則](./media/howto-password-smart-lockout/active-directory-on-premises-account-lockout-policy.png)

## <a name="manage-azure-ad-smart-lockout-values"></a>管理 Azure AD 智慧鎖定值

根據您的組織需求，智慧鎖定值可能需要進行自訂。 自定義智慧鎖定設定(具有特定於組織的值)需要為使用者付費的 Azure AD 許可證。

若要檢查或修改組織的智慧鎖定值，請使用下列步驟：

1. 登入 [Azure 入口網站](https://portal.azure.com)。
1. 搜尋並選取 [Azure Active Directory]**。 選擇**安全性** > **認證方法** > **密碼保護**。
