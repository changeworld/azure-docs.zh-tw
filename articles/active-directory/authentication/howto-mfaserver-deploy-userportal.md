...
![MFA 伺服器使用者門戶登錄頁](./media/howto-mfaserver-deploy-userportal/portal.png)
...
如果您對在 IIS 伺服器上配置 TLS/SSL 憑證有疑問，請參閱["如何在 IIS 上設置 SSL"](https://docs.microsoft.com/iis/manage/configuring-security/how-to-set-up-ssl-on-iis)一文。

Web 服務 SDK 必須使用 TLS/SSL 憑證進行保護。 自我簽署憑證適用於這項用途。 將證書導入使用者門戶 Web 服務器上本地電腦帳戶的"可信根憑證授權單位"存儲，以便在啟動 TLS 連接時信任該證書。
...
3. 將 TLS/SSL 憑證綁定到 IIS 中的網站
   ...
   > [!NOTE]
   > 此 TLS/SSL 憑證通常是公開簽名的 TLS/SSL 憑證。

4. 從任何電腦打開 Web 瀏覽器並導航到安裝使用者門戶的 URL（示例： `https://mfa.contoso.com/MultiFactorAuth`）。 確定未出現任何憑證警告或錯誤。
...
如果運行 Azure 多重要素驗證伺服器的伺服器不是面向 Internet 的，則應在**單獨的面向 Internet 的伺服器上**安裝使用者門戶。
...
* 使用 TLS/SSL 憑證保護使用者門戶。
* 使用 TLS/SSL 憑證保護 Azure 多重要素驗證 Web 服務 SDK。
* 確保使用者門戶可以通過 TLS/SSL 連線到 Azure 多重要素驗證 Web 服務 SDK。
...
5. 在 [記事本] 中編輯 Web.Config 檔案
    ...
    * 查找值**https://www.contoso.com/MultiFactorAuthWebServiceSdk/PfWsSdk.asmx**並將此預留位置 URL 更改為我們在步驟 2 中安裝的 Web 服務 SDK URL。
    * 儲存 Web.Config 檔案並關閉 [記事本]。

6. 從任何電腦打開 Web 瀏覽器並導航到安裝使用者門戶的 URL（示例： `https://mfa.contoso.com/MultiFactorAuth`）。 確定未出現任何憑證警告或錯誤。
...
3. 在 **"管理員"** 選項卡上定義應管理員。您可以使用"添加/編輯"框中的核取方塊和下拉創建精細的管理許可權。
...
如果希望使用者登錄並註冊，則必須在"設置"選項卡下選擇"**允許使用者登錄**"和 **"允許使用者註冊**"選項。請記住，您選擇的設置會影響使用者登錄體驗。
...
- [部署 Azure 多重要素驗證伺服器移動應用 Web 服務](howto-mfaserver-deploy-mobileapp.md)
