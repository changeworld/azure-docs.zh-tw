---
title: 管理使用者和裝置 Azure MFA - Azure 活動目錄
...
當管理員分配了身份驗證管理員角色時,您可以要求使用者重置其密碼、重新註冊 MFA 或從其使用者物件撤消現有的 MFA 作業階段。
...
1. 選擇要對其執行操作的使用者,然後選擇**認證方法**。
   - **重置密碼**將重置使用者的密碼,並分配必須在下一個登錄時更改的臨時密碼。
   - **要求重新註冊 MFA**將使用戶下次登錄時,將請求他們設置新的 MFA 身份驗證方法。
