...
5. Azure 多因素驗證會與 Azure Active Directory 通訊、抓取使用者的詳細資料，並使用使用者所設定的方法（行動電話、文字訊息或行動應用程式）來執行次要驗證。
...
5. 在 [**原則**] 底下，選取 [**網路原則**]。 您應該會看到類似下圖的虛擬私人網路 (VPN) 連線原則：
...
_在 hklm\software\microsoft\azuremfa 建立中_建立名為 REQUIRE_USER_MATCH 的新字串值，並將值設定為*True*或*False*。
...
若要使用此指令碼，請為擴充功能提供您的 Azure Active Directory 系統管理認證以及您之前複製的 Azure Active Directory 租用戶識別碼。 帳戶必須與您想要啟用的延伸模組位於相同的 Azure AD 租使用者中。 在安裝了 NPS 擴充功能的每個 NPS 伺服器上執行此指令碼。
...
3. 在下一個命令提示字元中，輸入 **.\AzureMfaNpsExtnConfigSetup.ps1**，然後選取 enter。 此指令碼會檢查您是否已安裝 Azure AD PowerShell 模組。 如果尚未安裝此模組，指令碼就會為您安裝。
    ...
    如果您因為 TLS 而收到安全性錯誤，請從 PowerShell 提示字元使用`[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12`命令來啟用 TLS 1.2。
