...
第一個安裝的 MFA Server 是 Azure MFA 服務預設啟用時的主要 MFA Server。 主要 MFA server 具有 PhoneFactor. phonefactor.pfdata 資料庫的可寫入複本。 MFA Server 執行個體的後續安裝稱為從屬項目。 MFA 從屬項目具有 PhoneFactor.pfdata 資料庫的唯讀複寫複本。 MFA Server 使用遠端程序呼叫 (RPC) 複寫資訊。 所有 MFA Server 必須共同加入網域或必須是獨立，才能複寫資訊。

當需要雙因素驗證時，MFA 主要和次級 MFA 伺服器都會與 MFA 服務通訊。 例如，當使用者嘗試存取需要雙因素驗證的應用程式，使用者會先經過識別提供者驗證，例如 Active Directory (AD)。

向 AD 成功驗證之後，MFA Server 將會與 MFA 服務通訊。 MFA Server 會等候來自 MFA 服務的通知，以允許或拒絕使用者存取應用程式。

如果 MFA 主伺服器離線，仍然可以處理驗證，但無法處理需要變更 MFA 資料庫的作業。 (範例包括：新增使用者、自助 PIN 變更、變更使用者資訊，或存取使用者入口網站)
...
* **需要手動升階 MFA 從屬項目**。 如果主要 Azure MFA 伺服器離線，次要 Azure MFA 伺服器會繼續處理 MFA 要求。 不過，在主要 MFA 伺服器可供使用之前，系統管理員無法新增使用者或修改 MFA 設定，使用者也無法使用使用者入口網站進行變更。 將 MFA 次級升級為主要角色一律是手動程式。
...
1. 這兩部 Azure MFA Server (MFA1 和 MFA2) 已經過負載平衡 (mfaapp.contoso.com)，並設定為使用靜態連接埠 (4443) 以複寫 PhoneFactor.pfdata 資料庫。 Web 服務 SDK 會安裝在每部 MFA Server 上，以允許透過 TCP 連接埠 443 與 ADFS 伺服器通訊。 MFA Server 會部署在無狀態負載平衡設定中。 不過，如果您想要透過簡訊使用 OTP，您必須使用具狀態負載平衡。
   ...
   > [!NOTE]
   > 因為 RPC 使用動態連接埠，所以不建議在 RPC 可能使用的動態連接埠範圍內開啟防火牆。 如果您的 MFA 應用程式伺服器 **之間** 有防火牆，您應該將 mfa Server 設定為在從屬與主伺服器之間的複寫流量的靜態埠上進行通訊，並在防火牆上開啟該埠。 您可以在 ```HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Positive Networks\PhoneFactor``` 建立稱為 ```Pfsvc_ncan_ip_tcp_port``` 的 DWORD 登錄值，並將該值設定為可用的靜態連接埠，來強制使用靜態連接埠。 連接一律會由次級 MFA Server 起始至主要複本，只有在主要複本上才需要靜態埠，但由於您可以隨時將次級升級為主要複本，因此您應該在所有 MFA Server 上設定靜態埠。
