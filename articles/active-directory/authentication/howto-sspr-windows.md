---
title: Windows 裝置的自助式密碼重設-Azure Active Directory
...
自助式密碼重設（SSPR）可讓使用者 Azure Active Directory （Azure AD）變更或重設其密碼的能力，而不需要系統管理員或服務台的介入。 使用者通常會在另一部裝置上開啟網頁瀏覽器，以存取[SSPR 入口網站](https://aka.ms/sspr)。 若要改善執行 Windows 7、8、8.1 和10之電腦的體驗，您可以讓使用者在 Windows 登入畫面重設其密碼。

![顯示具有 SSPR 連結的範例 Windows 7 和 10 登入畫面](./media/howto-sspr-windows/windows-reset-password.png)

> [!IMPORTANT]
> 本教學課程說明如何為企業中的 Windows 裝置啟用 SSPR。
>
> 如果您的 IT 小組尚未啟用從您的 Windows 裝置使用 SSPR 的功能，或登入期間發生問題，請與您的技術服務人員聯繫以取得其他協助。
...
- 這項功能不適用於已部署 802.1 x 網路驗證的網路，以及 [使用者登入前立即執行] 選項。 針對已部署 802.1 x 網路驗證的網路，建議使用電腦驗證來啟用此功能。
...
- 已知下列設定會干擾在 Windows 10 裝置上使用和重設密碼的能力：
    - 如果在 v1809 之前的 Windows 10 版本中，原則需要 Ctrl + Alt + Del，**重設密碼**將無法使用。
    - 如果鎖定螢幕通知已關閉，[**重設密碼**] 將無法使用。
    - *HideFastUserSwitching*設定為 enabled 或1
    - *DontDisplayLastUserName*設定為 enabled 或1
    - *NoLockScreen*設定為 enabled 或1
    - 已在裝置上設定*EnableLostMode*
...
- 使用者必須先註冊 SSPR，才能使用此功能[https://aka.ms/ssprsetup](https://aka.ms/ssprsetup)
    - 並非唯一使用 SSPR 從 Windows 登入畫面，所有使用者都必須先提供驗證連絡人資訊，才能重設其密碼。
- 網路 proxy 需求：
...
使用 Intune 從登入畫面部署設定變更以啟用 SSPR 是最具彈性的方法。 Intune 可讓您將組態變更部署至您所定義的特定機器群組。 必須完成裝置的 Intune 註冊，才能使用此方法。

#### <a name="create-a-device-configuration-policy-in-intune"></a>在 Intune 中建立裝置設定原則

1. 登入[Azure 入口網站](https://portal.azure.com)並選取 [ **Intune**]。
1. 前往 [**裝置**設定配置  >  **檔**]，然後選取 [ **+ 建立配置**檔]，以建立新的裝置設定檔
   - 針對**平臺**選擇*Windows 10 和更新版本*
   - 針對 [**配置檔案類型**]，選擇 [*自訂*]
1. 選取 [**建立**]，然後為設定檔提供有意義的名稱，例如*Windows 10 登入畫面 SSPR*

    （選擇性）提供設定檔的有意義描述，然後選取 **[下一步]**。
1. 在 [*設定*] 底下，選取 [**新增**]，並提供下列 oma-uri 設定以啟用 [重設密碼] 連結：
      - 提供有意義的名稱來說明設定的功能，例如 [*新增 SSPR 連結*]。
      - 選擇性地提供設定的有意義描述。
    ...
    選取 [**新增** **]、[下一步]**。
1. 原則可以指派給特定的使用者、裝置或群組。 視需要為您的環境指派設定檔，最好是先對裝置進行測試群組，然後選取 **[下一步]**。

    如需詳細資訊，請參閱[在 Microsoft Intune 中指派使用者和裝置設定檔](/mem/intune/configuration/device-profile-assign)。

1. 為您的環境設定所需的適用性規則，例如，*如果作業系統版本是 Windows 10 企業版，則指派設定檔*，然後選取 **[下一步]**。
1. 檢查您的設定檔，然後選取 [**建立**]。
...
如果您在 Windows 登入畫面中使用 SSPR 時發生問題，Azure AD audit 記錄檔會包含發生密碼重設之 IP 位址和*ClientType*的相關資訊，如下列範例輸出所示：

![Azure AD 稽核記錄中的範例 Windows 7 密碼重設](media/howto-sspr-windows/windows-7-sspr-azure-ad-audit-log.png)

當使用者從 Windows 10 裝置的登入畫面重設其密碼時，會建立名為的低許可權暫存帳戶 `defaultuser1` 。 此帳戶用來保護密碼重設程序。

帳戶本身具有隨機產生的密碼，並不會顯示裝置登入，而且會在使用者重設其密碼之後自動移除。 可能會存在多個 `defaultuser` 設定檔，但您可以放心忽略。
...
針對 Windows 7、8和8.1，電腦上必須安裝小型元件，才能在登入畫面上啟用 SSPR。 若要安裝此 SSPR 元件，請完成下列步驟：

1. 下載適用於所要啟用 Windows 版本的安裝程式。

    您可以在 Microsoft 下載中心取得軟體安裝程式，網址為[https://aka.ms/sspraddin](https://aka.ms/sspraddin)
1. 登入要安裝的電腦，並執行安裝程式。
1. 安裝之後，強烈建議您重新開機。
...
您可以使用下列命令來安裝或卸載 SSPR 元件，而不會出現提示：
...
如果您在 Windows 登入畫面中使用 SSPR 時發生問題，則會在電腦和 Azure AD 記錄事件。 Azure AD 事件包含發生密碼重設之 IP 位址和 ClientType 的相關資訊，如下列範例輸出所示：

![Azure AD 稽核記錄中的範例 Windows 7 密碼重設](media/howto-sspr-windows/windows-7-sspr-azure-ad-audit-log.png)

如果需要額外的記錄，則可變更電腦上的登錄機碼來啟用詳細資訊記錄。 僅使用下列登錄機碼值，為疑難排解目的啟用詳細資訊記錄：
...
當使用者嘗試登入時，他們會看到 [**重設密碼**] 或 [**忘記密碼**] 連結，可在登入畫面開啟自助式密碼重設體驗。 這項功能可讓使用者重設其密碼，而不需使用另一個裝置來存取網頁瀏覽器。

如需使用這項功能之使用者的詳細資訊，請參閱[重設您的工作或學校密碼](../user-help/active-directory-passwords-update-your-own-password.md)

## <a name="next-steps"></a>後續步驟

若要簡化使用者註冊體驗，您可以[預先填入 SSPR 的使用者驗證連絡人資訊](howto-sspr-authenticationdata.md)。
