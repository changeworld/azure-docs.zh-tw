---
title: RADIUS 和 Azure MFA 伺服器 - Azure 活動目錄
...
> [!IMPORTANT]
> 本文僅適用于 Azure MFA 伺服器的使用者。 如果使用基於雲的 Azure MFA，則瞭解如何與 Azure [MFA 的 RADIUS 身份驗證集成](howto-mfa-nps-extension.md)。
>
...
![MFA 伺服器中的半徑身份驗證](./media/howto-mfaserver-dir-radius/radius.png)
...
1. 按一下 **"目標"** 選項卡。
   * 如果 Azure MFA 伺服器安裝在活動目錄環境中的域加入伺服器上，請選擇 Windows**域**。
   * 如果使用者應針對 LDAP 目錄進行身份驗證，請選擇**LDAP 綁定**。
      選取 [目錄整合] 圖示，然後編輯 [設定] 索引標籤上的 LDAP 組態，以便讓伺服器能夠繫結至您的目錄。 有關配置 LDAP 的說明，請參閱[LDAP 代理配置指南](howto-mfaserver-dir-ldap.md)。
