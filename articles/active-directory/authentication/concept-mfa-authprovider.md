---
title: Azure 多因素認證提供者 - Azure 活動目錄
...
> [!IMPORTANT]
> 自 2018 年 9 月 1 日起，不再建立新的驗證提供者。 現有身份驗證提供者可以繼續使用和更新,但遷移不再可能。 多重要素驗證將繼續為 Azure AD Premium 授權中的可用功能。
...
### <a name="removing-an-authentication-provider"></a>移除認證提供者

> [!CAUTION]
> 刪除身份驗證提供程式時沒有確認。 選擇 **「刪除**」是一個永久過程。

認證提供者可以在**Azure 門戶** > **Azure 的活動目錄** > **安全** > **MFA** > **提供程式中找到**。 按一下列出的提供者以查看與該提供程式關聯的詳細資訊和配置。

在刪除身份驗證提供者之前,請注意提供程式中配置的任何自定義設置。 決定需要從供應商遷移到常規 MFA 設置的設置,並完成這些設置的遷移。 

連接到提供者的 Azure MFA 伺服器需要使用**Azure 入口網站** > **Azure 活動目錄** > **安全** > **MFA** > **伺服器設定**下生成的認證重新啟動。 在重新啟動之前,必須從環境中 Azure`\Program Files\Multi-Factor Authentication Server\Data\`MFA 伺服器上的目錄中移除以下檔案:
...
![從 Azure 門戶中移除認證提供者](./media/concept-mfa-authprovider/authentication-provider-removal.png)

確認已遷移所有設定後,可以瀏覽到 Azure**門戶** > **Azure 活動目錄** > **安全** > **Delete** **MFA** > **提供者**並選擇省略號 **...**

> [!WARNING]
> 刪除身份驗證提供程式將刪除與該提供程式關聯的任何報告資訊。 您可能需要在刪除提供程式之前儲存活動報告。

> [!NOTE]
> 具有舊版本的 Microsoft 驗證器應用和 Azure MFA 伺服器的使用者可能需要重新註冊其應用。
