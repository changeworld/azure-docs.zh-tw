...
description: 瞭解不同的 Azure 活動目錄自助服務密碼重置策略選項
...
ms.locfileid: "81393041"
---
# <a name="self-service-password-reset-policies-and-restrictions-in-azure-active-directory"></a>Azure 活動目錄中的自助服務密碼重置策略和限制
...
**微軟為任何 Azure 管理員角色強制實施強預設*雙門*密碼重置策略**。 此策略可能與為使用者定義的策略不同,並且無法更改此策略。 請一律要以未指派任何 Azure 系統管理員角色的使用者身分測試密碼重設功能。
...
* 如果試用版訂用帳戶已經過 30 天；或
* 已為 Azure AD 租戶設定了自訂網域,例如*contoso.com*。或
...
* 尚未為 Azure AD 租戶設定自訂網域,因此使用預設的 **.onmicrosoft.com*。 不建議預設 =*.onmicrosoft.com*域用於生產;和
...
每個需要登入 Azure AD 的使用者帳戶都必須具有與其帳戶相關聯的唯一使用者主體名稱 (UPN) 屬性值。 下表概述了適用於同步到雲端的本地活動目錄網域服務使用者帳戶和僅雲端使用者帳戶的策略:
...
下表描述了應用於在 Azure AD 建立與管理的使用者帳戶的密碼政策設定:
...
| 密碼過期持續時間 (最大密碼期限) |<ul><li>默認值 **:90**天。</li><li>可透過適用於 Windows PowerShell 的 Azure Active Directory 模組使用 `Set-MsolPasswordPolicy` Cmdlet 設定該值。</li></ul> |
| 密碼過期通知(當使用者收到密碼過期通知時) |<ul><li>默認值 **:14**天(密碼過期之前)。</li><li>可使用 `Set-MsolPasswordPolicy` Cmdlet 設定此值。</li></ul> |
...
1. 執行下列其中一個命令：
   ...
   * 要檢視單個使用者的密碼是否設定為永不過期,請使用 UPN(例如*\@,aprilr contoso.onmicrosoft.com)* 或要檢查的使用者的使用者 ID 執行以下 cmdlet:
...
要開始使用 SSPR,請參閱[教學:允許使用者使用 Azure Active Directory 自助服務密碼重置解鎖其帳戶或重置密碼](tutorial-enable-sspr.md)。
