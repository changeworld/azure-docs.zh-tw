...
1. 在 Azure 多重要素驗證伺服器中，按一下左側功能表中的**IIS 身份驗證**圖示。
2. 按一下 [表單架構]**** 索引標籤。
3. 按一下 **[新增]**。

   ![MFA 伺服器 IIS 身份驗證視窗](./media/howto-mfaserver-adfs-2/setup1.png)

4. 要自動檢測使用者名、密碼和域變數，請在"自動設定基於表單的網站"`https://sso.contoso.com/adfs/ls`對話方塊中輸入登錄 URL（如 ），然後按一下"**確定**"。
...
7. 在"添加基於表單的網站"對話方塊中，在"提交 URL"欄位中輸入 AD FS 登錄頁的`https://sso.contoso.com/adfs/ls`URL（如 ），然後輸入應用程式名稱（可選）。 應用程式名稱會出現在 Azure Multi-Factor Authentication 報表中，而且可能顯示在簡訊或行動應用程式驗證訊息內。
...
10. 如果所有使用者都已經或將要匯入到「伺服器」，且必須接受雙步驟驗證，請核取 [需要進行 Azure Multi-Factor Authentication 使用者比對]**** 方塊。 如果有大量使用者尚未匯入伺服器及/或將免除雙步驟驗證，請勿核取此方塊。
    ...
    ![將基於表單的網站添加到 MFA 伺服器](./media/howto-mfaserver-adfs-2/manual.png)
...
12. 因為 AD FS Proxy 伺服器不可能加入網域，您可以使用 LDAP 連接到您的網域控制站，以便使用者匯入和預先驗證。 在"基於表單的網站高級"對話方塊中，按一下 **"主身份驗證"** 選項卡，並為預身份驗證類型選擇**LDAP 綁定**。
...
2. 在 [設定] 索引標籤上，選取 [使用特定 LDAP 設定]**** 選項按鈕。
   ...
   ![為特定 LDAP 設置配置 LDAP 設置](./media/howto-mfaserver-adfs-2/ldap1.png)

3. 按一下 **[編輯]**。
4. 在 [編輯 LDAP 設定] 對話方塊的欄位中，填入連接到 AD 網域控制站所需的資訊。 欄位說明包含在 Azure Multi-Factor Authentication Server 說明檔中。
5. 按一下 [測試]**** 按鈕來測試 LDAP 連接。

   ![在 MFA 伺服器中測試 LDAP 配置](./media/howto-mfaserver-adfs-2/ldap2.png)
...
![登錄編輯程式用於配置公司設置](./media/howto-mfaserver-adfs-2/reg.png)
...
2. 按一下 [HTTP]**** 索引標籤。
3. 按一下 **[新增]**。
4. 在"添加基本 URL"對話方塊中，在"基本 URL"欄位中輸入 AD FS`https://sso.domain.com/adfs/ls/auth/integrated`網站的 URL，其中執行 HTTP 身份驗證（如 ） 然後，輸入應用程式名稱 (選擇性)。 應用程式名稱會出現在 Azure Multi-Factor Authentication 報表中，而且可能顯示在簡訊或行動應用程式驗證訊息內。
...
![將受信任的 IP 配置為 MFA 伺服器](./media/howto-mfaserver-adfs-2/trusted.png)
