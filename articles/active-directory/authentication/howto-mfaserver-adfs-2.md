...
7. 在 [新增表單架構網站] 對話方塊中，于 [提交 URL] 欄位中輸入 AD FS 登入頁面的 URL （例如 `https://sso.contoso.com/adfs/ls` ），然後輸入應用程式名稱（選擇性）。 應用程式名稱會出現在 Azure Multi-Factor Authentication 報表中，而且可能顯示在簡訊或行動應用程式驗證訊息內。
...
2. 按一下 [HTTP] 索引標籤。
3. 按一下 [新增] 。
4. 在 [新增基底 URL] 對話方塊中，于 [基底 URL] 欄位中輸入執行 HTTP 驗證的 AD FS 網站 URL （例如 `https://sso.domain.com/adfs/ls/auth/integrated` ）。 然後，輸入應用程式名稱 (選擇性)。 應用程式名稱會出現在 Azure Multi-Factor Authentication 報表中，而且可能顯示在簡訊或行動應用程式驗證訊息內。
