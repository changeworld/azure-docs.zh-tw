...
description: 瞭解如何在 Azure 入口網站中設定 Azure MFA Server 的設定
...
本文可協助您管理 Azure 入口網站中的 Azure MFA Server 設定。
...
| [一次性略過](#one-time-bypass) | 允許使用者在有限時間執行多重要素驗證的情況下進行驗證。 |
...
[單次許可] 功能可讓使用者驗證一次，而不需要執行多重要素驗證。 許可只是暫時性，經過指定的秒數之後就會到期。 在行動裝置應用程式或電話沒有收到通知或來電的情況下，您可以啟用單次許可，讓使用者能夠存取所需的資源。
...
1. 選取 [新增]。
1. 如有必要，請選取此許可的複寫群組。
1. 輸入 `username\@domain.com` 作為使用者名稱。 輸入許可的最後時間和略過原因的秒數。
1. 選取 [新增]。 時間限制會立即生效。 使用者必須在單次許可到期之前登入。

您也可以從這個相同的視窗中，查看一次性略過報表。
...
您可以從 MFA Server 本身的 web 主控台取得其他 MFA Server 設定選項。 您也可以[設定 AZURE MFA Server 以獲得高可用性](howto-mfaserver-deploy-ha.md)。
