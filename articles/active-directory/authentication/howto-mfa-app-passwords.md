---
title: 設定 Azure AD Multi-Factor Authentication 的應用程式密碼-Azure Active Directory
description: 瞭解如何在 Azure AD Multi-Factor Authentication 中設定和使用繼承應用程式的應用程式密碼
...
ms.locfileid: "94839262"
---
# <a name="enable-and-use-azure-ad-multi-factor-authentication-with-legacy-applications-using-app-passwords"></a>使用應用程式密碼來啟用和使用 Azure AD Multi-Factor Authentication 搭配繼承應用程式

某些舊版的非瀏覽器應用程式（例如 Office 2010 或更早版本）和 iOS 11 之前的 Apple Mail 不了解驗證程式中的暫停或中斷。 如果使用者已啟用 Azure AD Multi-Factor Authentication，並嘗試使用其中一個舊版的非瀏覽器應用程式，他們就無法成功進行驗證。 若要以安全的方式使用這些應用程式，並啟用使用者帳戶的 Azure AD Multi-Factor Authentication，您可以使用應用程式密碼。 這些應用程式密碼會取代您的傳統密碼，讓應用程式略過多重要素驗證並正常運作。
...
啟用 Azure AD Multi-Factor Authentication 的使用者帳戶時，一般登入提示會因要求進行額外驗證而中斷。 某些較舊的應用程式不了解登入程式中的此中斷，因此驗證會失敗。 若要維護使用者帳戶安全性，並讓 Azure AD Multi-Factor Authentication 啟用，可以使用應用程式密碼，而不是使用者的一般使用者名稱和密碼。 在登入期間使用應用程式密碼時，不會有其他驗證提示，因此驗證會成功。
...
* 在使用者的帳戶上啟用 Azure AD Multi-Factor Authentication 之後，應用程式密碼便可用於大部分的非瀏覽器用戶端，例如 Outlook 和 Microsoft 商務用 Skype。 不過，您無法透過非瀏覽器應用程式（例如 Windows PowerShell）使用應用程式密碼來執行系統管理動作。 即使使用者具有系統管理員帳戶，仍無法執行動作。
...
Azure AD 支援搭配內部部署 Active Directory Domain Services (AD DS) 的同盟或單一登入 (SSO) 。 如果您的組織與 Azure AD 同盟，且您使用 Azure AD Multi-Factor Authentication，則適用下列應用程式密碼考慮：
...
* 您使用 Exchange online。
* 您使用商務用 Skype 內部部署。
* 您使用 Azure AD Multi-Factor Authentication。
...
當使用者完成 Azure AD Multi-Factor Authentication 的初始註冊時，有一個選項可在註冊程式結束時建立應用程式密碼。

使用者也可以在註冊後建立應用程式密碼。 如需使用者的詳細資訊和詳細步驟，請參閱 [Azure AD Multi-Factor Authentication 中的應用程式密碼為何？](../user-help/multi-factor-authentication-end-user-app-passwords.md)

## <a name="next-steps"></a>後續步驟

如需如何讓使用者快速註冊 Azure AD Multi-Factor Authentication 的詳細資訊，請參閱 [結合的安全性資訊註冊總覽](concept-registration-mfa-sspr-combined.md)。
