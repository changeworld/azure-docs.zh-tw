...
1. 使用者選取 [無法存取帳戶]**** 連結或直接移至 [https://aka.ms/sspr](https://passwordreset.microsoftonline.com)。
   ...
   * 若要以不同的當地語系化語言來查看密碼重設入口網站，請將 "？ mkt =" 附加至密碼重設 URL 的結尾，並使用遵循當地語系化為西班牙文的範例 [https://passwordreset.microsoftonline.com/?mkt=es-us](https://passwordreset.microsoftonline.com/?mkt=es-us) 。
...
> [!IMPORTANT]
> 自2019年3月起，Azure AD 租使用者的免費/試用版中，將無法使用通話選項進行 MFA 和 SSPR。 手機簡訊不會受到這項變更的影響。 付費 Azure AD 租用戶中的使用者將可繼續使用通話功能。 這項變更只會影響免費/試用版 Azure AD 租用戶。
...
使用者在註冊自助式密碼重設時，無法選擇註冊其行動應用程式 [https://aka.ms/ssprsetup](https://aka.ms/ssprsetup) 。 使用者可以註冊其行動裝置應用程式 [https://aka.ms/mfasetup](https://aka.ms/mfasetup) ，或在的新安全性資訊註冊預覽版中 [https://aka.ms/setupsecurityinfo](https://aka.ms/setupsecurityinfo) 。
