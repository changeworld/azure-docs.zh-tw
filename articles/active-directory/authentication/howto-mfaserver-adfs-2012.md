---
title: Windows 伺服器中帶有 AD FS 的 Azure MFA 伺服器 - Azure 活動目錄
...
![編輯全域身份驗證策略](./media/howto-mfaserver-adfs-2012/global.png)
...
2. 將 **WebServiceSdkUrl** 的值設定為 Multi-Factor Authentication Web 服務 SDK 的 URL。 例如 *：HTTPs：contoso.com/\/\/\<證書名稱>/多因數AuthWebServiceSdk/PfWsSdk.asmx，* 其中*\<證書名稱>* 是證書的名稱。  
3. 通過添加到`-ConfigurationFilePath &lt;path&gt;``Register-AdfsAuthenticationProvider`命令的末尾編輯寄存器多因素身份驗證 AdfsAdapter.ps1 腳本，其中*&lt;路徑&gt;* 是多因素身份驗證 AdfsAdapter.config 檔的完整路徑。
...
3. 以滑鼠右鍵按一下 [Microsoft Office 365 身分識別平台]****，然後選取 [編輯宣告規則...]****
   ...
   ![在 ADFS 主控台中編輯聲明規則](./media/howto-mfaserver-adfs-2012/trustedip1.png)
...
5. 在 [新增轉換宣告規則精靈] 上，從下拉式清單選取 [通過或篩選傳入宣告]****，然後按 [下一步]****。
   ...
   ![添加轉換聲明規則嚮導](./media/howto-mfaserver-adfs-2012/trustedip3.png)
