---
title: 使用 NPS - Azure 活動目錄提供 Azure MFA 功能
...
當您安裝擴充功能時，您的 Azure AD 租用戶需要目錄識別碼和系統管理員認證。 您可以在 [Azure 入口網站](https://portal.azure.com)中找到您的目錄識別碼。 以系統管理員身分登入。 搜索並選擇**Azure 活動目錄**，然後選擇**屬性**。 複製 [目錄識別碼]**** 方塊中的 GUID，然後儲存。 當您安裝 NPS 擴充功能時，將使用此 GUID 作為租用戶識別碼。
...
此外，需要連接到以下 URL 才能[使用提供的 PowerShell 腳本完成配接器的設置](#run-the-powershell-script)
...
1. 在 RADIUS 用戶端 (VPN、Netscaler 伺服器或其他) 與 NPS 伺服器之間使用的密碼加密演算法。
   - **PAP**支援雲中 Azure MFA 的所有身份驗證方法：撥打電話、單向短信、移動應用通知、OATH 硬體權杖和移動應用驗證碼。
   - **CHAPV2**和**EAP**支援撥打電話和移動應用通知。

      > [!NOTE]
      > 當您部署 NPS 擴充時，使用這些因素來評估哪些方法可供您的使用者使用。 如果您的 RADIUS 用戶端支援 PAP，但用戶端 UX 沒有驗證碼的輸入欄位，則通話和行動裝置應用程式通知是兩個支援的選項。
      >
      > 此外，如果您的 VPN 用戶端 UX 支援輸入欄位，並且您已配置了網路訪問策略 - 身份驗證可能會成功，但是網路原則中配置的 RADIUS 屬性都不會應用於網路訪問設備，像RRAS伺服器，也不是VPN用戶端。 因此，VPN 用戶端可能具有比預期更多的存取權限，或者對無法訪問的訪問更少。
      >
...
2. 遵循提示來設定驗證方法。
3. [創建條件訪問策略](howto-mfa-getstarted.md#create-conditional-access-policy)以要求測試帳戶的多重要素驗證。
...
1. 從 Microsoft 下載中心[下載 NPS 擴展](https://aka.ms/npsmfa)。
2. 將二進位檔複製到您要設定的網路原則伺服器。
3. 運行*安裝程式.exe*並按照安裝說明操作。 如果您遇到錯誤，請根據必要條件一節再次檢查兩個已成功安裝的程式庫。

#### <a name="upgrade-the-nps-extension"></a>升級 NPS 擴展

升級現有 NPS 擴展安裝時，為了避免重新開機基礎伺服器，請完成以下步驟：
...
如果以前的電腦證書已過期，並且已生成新證書，則應刪除任何過期的證書。 證書過期可能會導致 NPS 擴展啟動出現問題。
...
1. 在 NPS 伺服器上打開**登錄編輯程式**。
1. 瀏覽至 `HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\AzureMfa`。 設置以下鍵值：
...
`AzureMfaNpsExtnConfigSetup.ps1`腳本創建的證書的有效期為 2 年。 IT 組織應監視證書的過期情況。 NPS 擴展證書放置在"個人"下的本地電腦憑證存放區中，並頒發給提供給腳本的租戶 ID。
...
- NPS 擴展使用本地活動目錄中的 UPN 來標識 Azure MFA 上的使用者以執行輔助 Auth。可以將擴展配置為使用其他識別碼，如備用登入識別碼 或自訂活動目錄欄位（UPN） 以外的欄位。 如需詳細資訊，請參閱 [NPS 擴充功能的 Multi-Factor Authentication 所適用的進階設定選項](howto-mfa-nps-extension-advanced.md)一文。
...
### <a name="nps-extension-health-check-script"></a>NPS 擴展運行狀況檢查腳本
...
由*AzureMfaNpsExttttttconfigSetup.ps1*腳本生成的自簽章憑證也有兩年的有效性存留期。 驗證是否安裝了證書時，還應檢查證書是否尚未過期。
...
運行此命令後，轉到 C 磁碟機，找到該檔並按兩下該檔。 移至 [詳細資料] 並向下捲動至 [指紋]，將安裝在伺服器上的憑證指紋與此指紋進行比較。 這兩個憑證指紋應該相符。
...
要檢查您是否具有有效的證書，請使用 MMC 檢查本地電腦帳戶的憑證存放區，並確保證書尚未超過其到期日期。 要生成新有效的證書，應重新運行"[運行 PowerShell 腳本](#run-the-powershell-script)"一節下的步驟
...
其他故障排除指南和可能的解決方案可以在"[解決來自 Azure 多重要素驗證的 NPS 副檔名中的錯誤訊息](howto-mfa-nps-extension-errors.md)"一文中找到。

## <a name="next-steps"></a>後續步驟

- [Windows 伺服器網路原則伺服器的概述和配置](https://docs.microsoft.com/windows-server/networking/technologies/nps/nps-top)
