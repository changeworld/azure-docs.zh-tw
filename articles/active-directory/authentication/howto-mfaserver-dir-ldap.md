---
title: LDAP 認證與 Azure MFA 伺服器 - Azure 活動目錄
...
2. 核取 [啟用 LDAP 驗證]**** 核取方塊。
   ...
   ![MFA 伺服器中的 LDAP 認證](./media/howto-mfaserver-dir-ldap/ldap2.png)

3. 在「用戶端」選項卡上,如果 Azure 多重身份驗證 LDAP 服務應綁定到非標準埠以偵聽 LDAP 請求,請更改 TCP 埠和 SSL (TLS) 埠。
4. 如果計劃使用從用戶端到 Azure 多重身份驗證伺服器的 LDAPS,則必須在與 MFA 伺服器相同的伺服器上安裝 TLS/SSL 證書。 按下 **「瀏覽**」SSL (TLS) 憑證框旁邊,然後選擇用於安全連接的證書。
5. 按一下 **[新增]**。
...
7. 按下 **「篩選器」** 選項卡。伺服器已預先配置為從活動目錄載入容器、安全組和使用者。 如果繫結至不同 LDAP 目錄，您可能需要編輯顯示的篩選。 如需篩選的詳細資訊，請按一下 [說明]**** 連結。
8. 按下屬性**選項**卡。伺服器已預先配置為從活動目錄映射屬性。
...
已選取 [使用 LDAP 唯一識別碼屬性來比對使用者名稱]**** 選項按鈕時，Azure Multi-factor Authentication Server 會嘗試將每個使用者名稱解析為 LDAP 目錄中的唯一識別碼。 LDAP 搜尋在目錄整合 -> 屬性選項卡中定義的使用者名屬性上執行。當使用者進行身份驗證時,用戶名將解析為 LDAP 目錄中的唯一標識符。 唯一識別碼可用來比對 Azure Multi-Factor Authentication 資料檔中的使用者。 這可進行不區分大小寫的比較，且允許完整或簡短的使用者名稱格式。
...
* 將 LDAP 超時設定為 30-60 秒,以便有時間使用 LDAP 目錄驗證使用者的認證、執行第二步驗證、接收其回應並回應 LDAP 訪問請求。
* 如果使用 LDAPS,則執行 LDAP 查詢的設備或伺服器必須信任 Azure 多重身份驗證伺服器上安裝的 TLS/SSL 證書。
