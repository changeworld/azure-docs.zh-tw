---
title: LDAP 驗證和 Azure MFA 伺服器 Azure Active Directory
...
8. 按一下 [ **屬性** ] 索引標籤。伺服器已預先設定為對應 Active Directory 的屬性。
...
已選取 [使用 LDAP 唯一識別碼屬性來比對使用者名稱]**** 選項按鈕時，Azure Multi-factor Authentication Server 會嘗試將每個使用者名稱解析為 LDAP 目錄中的唯一識別碼。 系統會在 [目錄整合-> 屬性] 索引標籤中定義的使用者名稱屬性上執行 LDAP 搜尋。當使用者進行驗證時，使用者名稱會解析為 LDAP 目錄中的唯一識別碼。 唯一識別碼可用來比對 Azure Multi-Factor Authentication 資料檔中的使用者。 這可進行不區分大小寫的比較，且允許完整或簡短的使用者名稱格式。
...
* 將 LDAP timeout 設定為30-60 秒，以便有時間使用 LDAP 目錄驗證使用者的認證、執行第二步驗證、接收其回應，以及回應 LDAP 存取要求。
* 如果使用 LDAPS，則進行 LDAP 查詢的設備或伺服器必須信任安裝在 Azure Multi-Factor Authentication Server 上的 TLS/SSL 憑證。
