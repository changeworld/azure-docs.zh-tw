...
如果未指派這些權限，回寫雖看似已正確設定，但使用者會在從雲端管理其內部部署密碼時遇到錯誤。
...
內部部署 AD DS 環境中的密碼原則可能會導致密碼重設無法正確進行處理。 若要讓密碼回寫正確運作，[密碼最短使用期限]  的群組原則必須設定為 0。 此設定可在 `gpedit.msc` 內的 > [電腦設定] > [原則] > [Windows 設定] > [安全性設定] > [帳戶原則]  底下找到。
