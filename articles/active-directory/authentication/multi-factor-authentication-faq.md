...
* [Azure 多因素驗證服務器如何處理使用者資料？](#how-does-azure-multi-factor-authentication-server-handle-user-data)
...
使用多重要素驗證服務器時，使用者資料只會儲存在內部部署伺服器上。 雲端中不會儲存任何持續性的使用者資料。 使用者執行雙步驟驗證時，Multi-Factor Authentication Server 會將資料傳送到 Azure Multi-Factor Authentication 雲端服務以供驗證。 Multi-Factor Authentication Server 與 Multi-Factor Authentication 雲端服務之間的通訊會透過傳出連接埠 443，使用安全通訊端層 (SSL) 或傳輸層安全性 (TLS) 通訊。

驗證要求傳送至雲端服務時，會收集資料以用於驗證和使用方式報告。 下列資料欄位包含在雙步驟驗證記錄中：
...
您可以參考[多因素驗證定價頁面](https://azure.microsoft.com/pricing/details/multi-factor-authentication/)或[Azure 多因素驗證版本和取用方案](concept-mfa-licensing.md)的檔，來回答大部分的帳單問題。

* [我的組織是否需要支付用來進行驗證的通話和文字訊息的費用？](#is-my-organization-charged-for-sending-the-phone-calls-and-text-messages-that-are-used-for-authentication)
* [每位使用者的計費模式是否會向我收取所有已啟用使用者的費用，或只針對執行雙步驟驗證的使用者付費？](#does-the-per-user-billing-model-charge-me-for-all-enabled-users-or-just-the-ones-that-performed-two-step-verification)
* [多重要素驗證計費如何運作？](#how-does-multi-factor-authentication-billing-work)
* [是否有免費版本的 Azure 多重要素驗證？](#is-there-a-free-version-of-azure-multi-factor-authentication)
* [我的組織是否可以隨時在每位使用者和每次驗證耗用量計費模式之間切換？](#can-my-organization-switch-between-per-user-and-per-authentication-consumption-billing-models-at-any-time)
* [我的組織可以隨時在使用計費和訂用帳戶之間切換（以授權為基礎的模型）嗎？](#can-my-organization-switch-between-consumption-based-billing-and-subscriptions-a-license-based-model-at-any-time)
* [我的組織是否必須使用和同步處理身分識別才能使用 Azure 多重要素驗證？](#does-my-organization-have-to-use-and-synchronize-identities-to-use-azure-multi-factor-authentication)
...
否，您不需支付個別電話通話或透過 Azure 多重要素驗證傳送給使用者的文字訊息。 如果您使用每個驗證 MFA 提供者，則會針對每個驗證計費，但不會針對所使用的方法收費。
...
當您建立每個使用者或每個授權的 MFA 提供者時，貴組織的 Azure 訂用帳戶會以使用方式每個月計費作為基礎。 此計費模式類似于 Azure 如何計費虛擬機器和 Web Apps 的使用量。
...
可以在 Azure AD Free 層中啟用安全性預設值。 使用安全性預設值時，會使用 Microsoft Authenticator 應用程式，為所有使用者啟用多重要素驗證。 您無法使用文字訊息或電話驗證搭配安全性預設值，只要 Microsoft Authenticator 應用程式就可以了。
...
如果您的目錄有*每個驗證*的 Azure 多因素驗證提供者，只要 MFA 提供者連結至您的訂用帳戶，您就一律會針對每個驗證付費。 您可以將 MFA 授權指派給使用者，但您仍需要為每次雙步驟驗證要求支付費用，無論發出此要求的人是否擁有指派的 MFA 授權。
...
如果貴組織使用根據使用量計費的模型，則 Azure Active Directory 為選擇性項目，並非必要。 如果您的 MFA 提供者未連結至 Azure AD 租使用者，您就只能在內部部署環境部署 Azure 多因素驗證服務器。
...
* [如果我的使用者在電話上未收到回應，該怎麼辦？](#what-should-i-tell-my-users-to-do-if-they-dont-receive-a-response-on-their-phone)
* [如果我的其中一個使用者無法進入他們的帳戶，該怎麼辦？](#what-should-i-do-if-one-of-my-users-cant-get-in-to-their-account)
...
* [我的使用者說，有時候他們不會收到文字訊息或驗證時間。](#my-users-say-that-sometimes-they-dont-receive-the-text-message-or-the-verification-times-out)
* [我可以在系統超時前，變更使用者必須從文字訊息輸入驗證碼的時間長度嗎？](#can-i-change-the-amount-of-time-my-users-have-to-enter-the-verification-code-from-a-text-message-before-the-system-times-out)
* [我可以搭配使用硬體權杖與 Azure 多因素驗證服務器嗎？](#can-i-use-hardware-tokens-with-azure-multi-factor-authentication-server)
* [我可以使用 Azure 多重要素驗證服務器來保護終端機服務嗎？](#can-i-use-azure-multi-factor-authentication-server-to-secure-terminal-services)
* [我已在 MFA Server 中設定呼叫者識別碼，但我的使用者仍收到來自匿名呼叫端的多重要素驗證呼叫。](#i-configured-caller-id-in-mfa-server-but-my-users-still-receive-multi-factor-authentication-calls-from-an-anonymous-caller)
* [為什麼我的使用者會收到註冊安全性資訊的提示？](#why-are-my-users-being-prompted-to-register-their-security-information)

### <a name="what-should-i-tell-my-users-to-do-if-they-dont-receive-a-response-on-their-phone"></a>如果我的使用者在電話上未收到回應，該怎麼辦？

讓您的使用者在5分鐘內最多嘗試五次，以取得電話或 SMS 以進行驗證。 Microsoft 會使用多個提供者提供電話及簡訊。 如果這種方法無法解決問題，請開啟支援案例進行進一步的疑難排解。

協力廠商安全性應用程式也可能會封鎖驗證碼的文字訊息或通話。 如果使用協力廠商安全性應用程式，請嘗試停用保護，然後要求傳送另一個 MFA 驗證碼。

如果上述步驟沒有作用，請檢查使用者是否已設定一個以上的驗證方法。 請嘗試重新登入，但在登入頁面上選取不同的驗證方法。

如需詳細資訊，請參閱[使用者疑難排解指南](../user-help/multi-factor-authentication-end-user-troubleshoot.md)。
...
>
> 只有不支援最新驗證方式的應用程式需要應用程式密碼。 Office 2013 用戶端支援最新的驗證通訊協定，但需要設定。 執行 Office 2013 的2015年3月或更新版本的任何客戶都可以使用新式驗證。 如需詳細資訊，請參閱 blog 文章[更新的 Office 365 新式驗證](https://www.microsoft.com/microsoft-365/blog/2015/11/19/updated-office-365-modern-authentication-public-preview/)。

### <a name="my-users-say-that-sometimes-they-dont-receive-the-text-message-or-the-verification-times-out"></a>我的使用者說，有時候他們不會收到文字訊息或驗證時間。

不保證 SMS 訊息的傳遞，因為有可能會影響服務可靠性的無法控制因素。 這些因素包括目的地國家或地區、行動電話貨運公司，以及信號強度。

協力廠商安全性應用程式也可能會封鎖驗證碼的文字訊息或通話。 如果使用協力廠商安全性應用程式，請嘗試停用保護，然後要求傳送另一個 MFA 驗證碼。

如果您的使用者經常會遇到可哥靠接收文字訊息的問題，請告知他們改用 Microsoft Authenticator 應用程式或通話方法。 Microsoft Authenticator 可以透過行動電話通訊和 Wi-fi 連線接收通知。 此外，即使裝置收不到訊號，行動應用程式可以產生驗證碼。 Microsoft Authenticator 應用程式適用于[Android](https://go.microsoft.com/fwlink/?Linkid=825072)、 [iOS](https://go.microsoft.com/fwlink/?Linkid=825073)和[Windows Phone](https://www.microsoft.com/p/microsoft-authenticator/9nblgggzmcj6)。
...
針對在雲端中使用 Azure MFA 的單向 SMS （包括 AD FS 介面卡或網路原則伺服器擴充功能），您無法設定 [超時] 設定。 Azure AD 會儲存驗證程式碼達 180 秒。
...
如果您使用的是 Azure 多因素驗證服務器，您可以匯入協力廠商的 Open Authentication （OATH）以時間為基礎的一次性密碼（TOTP）權杖，然後將它們用於雙步驟驗證。
...
當透過公共電話網路發出 Multi-Factor Authentication 通話時，有時會透過不支援來電顯示的電信業者路由傳送。 由於此電訊廠商的行為，因此不保證呼叫者識別碼，即使多因素驗證系統一律會傳送它也一樣。
...
- 使用者存取具有條件式存取原則的應用程式需要 MFA，而且先前尚未註冊 MFA。
- 使用者正在向 Azure AD 註冊裝置（包括 Azure AD Join），而您的組織需要 MFA 進行裝置註冊，但使用者先前尚未註冊 MFA。
- 使用者正在 Windows 10 中產生 Windows Hello 企業版（需要 MFA），而且先前尚未註冊 MFA。
...
### <a name="what-should-users-do-if-they-see-an-authentication-request-is-not-for-an-activated-account-error-message-when-using-mobile-app-notifications"></a>如果使用者在使用行動代理程式更新時看到「驗證要求不適用於已啟用的帳戶」錯誤訊息，該怎麼辦？
...
當您嘗試登入的非瀏覽器應用程式（安裝在本機電腦上）無法使用需要雙步驟驗證的帳戶時，就會發生*0x800434D4L*錯誤。
