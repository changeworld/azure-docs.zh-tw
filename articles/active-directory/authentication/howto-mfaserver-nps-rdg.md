---
title: 使用 RADIUS RDG 和 Azure MFA 伺服器 - Azure 活動目錄
...
通常，遠端桌面 （RD） 閘道使用本地[網路原則服務 （NPS）](https://docs.microsoft.com/windows-server/networking/core-network-guide/core-network-guide#BKMK_optionalfeatures)對使用者進行身份驗證。 本文說明如何將 RADIUS 要求從遠端桌面閘道器 (透過本機 NPS) 傳送至 Multi-Factor Authentication Server。 Azure MFA 和 RD 閘道的組合表示使用者可以從任何地方存取其工作環境，同時執行強式驗證。
...
2. 選取 [TS 閘道伺服器群組]****。
3. 移至 [負載平衡]**** 索引標籤。
4. 在**認為請求被丟棄之前更改沒有回應的秒數**，以及伺服器在 30 到 60 秒之間**被標識無法接通請求之間的秒數**。 (如果您在驗證期間發現伺服器仍然逾時，您可以回到這裡並增加秒數。)
...
![MFA 伺服器中的半徑身份驗證](./media/howto-mfaserver-nps-rdg/radius.png)
