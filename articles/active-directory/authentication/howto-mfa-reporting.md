---
title: Azure MFA 的訪問和使用報告 - Azure 活動目錄
...
| 內部部署元件的使用方式 | Azure AD >安全> MFA >活動報告 | 透過 NPS 擴充功能、ADFS 和 MFA 伺服器提供 MFA 整體使用量的相關資訊。 |
| 已略過的使用者歷程記錄 | Azure AD >安全> MFA >一次性繞過 | 提供針對使用者許可 Multi-Factor Authentication 之要求的歷程記錄。 |
| 伺服器狀態 | Azure AD >安全> MFA >伺服器狀態 | 顯示與帳戶相關聯之 Multi-Factor Authentication Server 的狀態。 |

## <a name="view-mfa-reports"></a>檢視 MFA 報告

1. 登錄到 Azure[門戶](https://portal.azure.com)。
2. 在左側，選擇**Azure 活動目錄** > **安全** > **MFA**。
3. 選取您要檢視的報告。

   ![Azure 門戶中的 MFA 伺服器伺服器狀態報表](./media/howto-mfa-reporting/report.png)
...
此資料可通過 Azure[門戶](https://portal.azure.com)和報告[API](../reports-monitoring/concept-reporting-api.md)獲得。

![Azure 門戶中的 Azure AD 登錄報告](./media/howto-mfa-reporting/sign-in-report.png)
...
下表可用於使用多重要素驗證活動報告的下載版本對多重要素驗證進行故障排除。 它們不會直接顯示在 Azure 門戶中。

| 呼叫結果 | 描述 | 廣泛描述 |
| --- | --- | --- |
| SUCCESS_WITH_PIN | 已輸入 PIN | 使用者已輸入 PIN。 如果驗證成功，表示使用者已輸入正確的 PIN。 如果拒絕身份驗證，則輸入了不正確的 PIN 或使用者設置為標準模式。 |
| SUCCESS_NO_PIN | 僅 = 已輸入 | 如果使用者設定為 PIN 模式且驗證遭到拒絕，這表示使用者未輸入 PIN，只輸入 #。  如果使用者設定為標準模式且驗證成功，這表示使用者只輸入 #，而在標準模式下這是正確的動作。 |
...
| SUCCESS_USED_CACHE | 已使用快取 | 身份驗證成功，無需進行多重要素驗證調用，因為之前對同一使用者名的成功身份驗證是在配置的緩存時間範圍內發生的。 |
| SUCCESS_BYPASSED_AUTH | 已略過驗證 | 使用針對使用者起始之單次許可的驗證已成功。 如需許可的詳細資訊，請參閱許可的使用者歷程記錄報告。 |
| SUCCESS_USED_IP_BASED_CACHE | 已使用基於 IP 的緩存 | 身份驗證成功，無需進行多重要素驗證調用，因為之前在配置的緩存時間範圍內成功進行了對相同使用者名、身份驗證類型、應用程式名稱和 IP 的身份驗證。 |
| SUCCESS_USED_APP_BASED_CACHE | 已使用基於應用程式的緩存 | 身份驗證成功，無需進行多重要素驗證調用，因為之前在配置的緩存時間範圍內成功驗證了相同的使用者名、身份驗證類型和應用程式名稱。 |
| SUCCESS_INVALID_INPUT | 電話輸入無效 | 電話傳來的回應無效。 這可能是來自傳真機或數據機，或者使用者可能已輸入 * 作為其 PIN 的一部分。 |
| SUCCESS_USER_BLOCKED | 已封鎖使用者 | 已封鎖使用者的電話號碼。 在身份驗證調用期間，使用者可以啟動阻止的號碼，也可以由使用 Azure 門戶的管理員啟動。 <br> 注：被阻止的號碼也是欺詐警報的副產品。 |
...
| FAILED_INVALID_PHONENUMBER | 電話號碼格式無效 | 電話號碼中有無效的格式。 電話號碼必須是數位，並且國家代碼 #1（美國&加拿大）必須為 10 位數位。 |
...
| FAILED_SERVER_ERROR | 無法撥打電話 | 多重要素驗證服務無法發出呼叫。 |
| FAILED_SMS_NOT_SENT | 無法傳送簡訊 | 無法發送文本消息。 驗證遭到拒絕。 |
| FAILED_SMS_OTP_INCORRECT | 簡訊 OTP 不正確 | 使用者從收到的短信中輸入了不正確的一次性密碼 （OTP）。 驗證遭到拒絕。 |
| FAILED_SMS_OTP_PIN_INCORRECT | 簡訊 OTP + PIN 不正確 | 使用者輸入的一次性密碼 （OTP） 和/或不正確的使用者 PIN。 驗證遭到拒絕。 |
| FAILED_SMS_MAX_OTP_RETRY_REACHED | 超過最大短信 OTP 嘗試次數 | 使用者已超過一次性密碼 （OTP） 嘗試的最大次數。 |
...
| FAILED_OATH_TOKEN_TIMEOUT | OATH 代碼結果超時 | 使用者輸入 OATH 代碼的時間太長，多重要素驗證嘗試已超時。 |
| FAILED_SECURITY_QUESTIONS_TIMEOUT | 安全問題結果超時 | 使用者花了很長時間才輸入對安全問題的回答，多重要素驗證嘗試已超時。 |
| FAILED_AUTH_RESULT_TIMEOUT | Auth 結果超時 | 使用者完成多重要素驗證嘗試的時間太長。 |
| FAILED_AUTHENTICATION_THROTTLED | 身份驗證已限制 | 多因素身份驗證嘗試被服務限制。 |
