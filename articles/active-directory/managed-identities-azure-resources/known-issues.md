---
title: 常見問題解答和託管標識的已知問題 - Azure AD
...
否，Azure 資源適用受控識別尚未與 ADAL 或 MSAL 整合。 有關使用 REST 終結點為 Azure 資源獲取託管標識權杖的詳細資訊，請參閱如何在[Azure VM 上對 Azure 資源使用託管標識來獲取訪問權杖](how-to-use-vm-token.md)。
...
### <a name="should-i-use-the-managed-identities-for-azure-resources-imds-endpoint-or-the-vm-extension-endpoint"></a>我應該對 Azure 資源 IMDS 終結點或 VM 擴展終結點使用託管標識？

將 Azure 資源的託管標識與 VM 一起使用時，我們建議使用 IMDS 終結點。 Azure 執行個體中繼資料服務是透過 Azure Resource Manager建立之所有 IaaS VM 可存取的 REST 端點。 
...
- 可以將 1000 個使用者分配的託管標識分配給單個 VM。 

Azure 資源 VM 擴展的託管標識仍然可用;因此，該標識仍可用。但是，我們不再開發新功能。 我們建議切換以使用 IMDS 終結點。 

使用 VM 擴展終結點的一些限制包括：
- 對 Linux 發行版本的有限支援： CoreOS 穩定， CentOS 7.1， 紅帽 7.2， Ubuntu 15.04， Ubuntu 16.04
- 只能向 VM 分配 32 個使用者分配的託管標識。


注意：Azure 資源 VM 擴展的託管標識將在 2019 年 1 月退出支援。 
...
### <a name="what-azure-rbac-permissions-are-required-to-managed-identity-on-a-resource"></a>在資源上管理標識需要哪些 Azure RBAC 許可權？ 

- 系統分配的託管標識：您需要對資源的寫入權限。 例如，針對虛擬機器，您需要 Microsoft.Compute/virtualMachines/write。 此操作包含在特定于資源的內置角色中，如[虛擬機器參與者](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#virtual-machine-contributor)。
- 使用者分配的託管標識：您需要對資源的寫入權限。 例如，針對虛擬機器，您需要 Microsoft.Compute/virtualMachines/write。 除了[託管標識操作員](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#managed-identity-operator)在託管標識上的角色指派外。
...
已移動到其他目錄的訂閱中的託管標識的解決方法：

 - 若為系統指派的受控識別：停用然後重新啟用。 
 - 若為使用者指派的受控識別：加以刪除、重新建立，然後重新連結至所需的資源 (例如虛擬機器)

### <a name="moving-a-user-assigned-managed-identity-to-a-different-resource-groupsubscription"></a>將使用者分配的託管標識移動到其他資源組/訂閱

將使用者指派的受控身分識別移至不同的資源群組會導致身分識別中斷。 因此，使用該標識的資源（例如 VM）將無法為其請求權杖。 
