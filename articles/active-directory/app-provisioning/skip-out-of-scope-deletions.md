...
本文說明如何使用 Microsoft Graph API 和 Microsoft Graph API explorer 來設定旗標 ***SkipOutOfScopeDeletions*** ，以控制移出範圍的帳戶處理。 
* 如果 ***SkipOutOfScopeDeletions*** 設定為 0 (false) ，則會在目標中停用超出範圍的帳戶。
* 如果 ***SkipOutOfScopeDeletions*** 設定為 1 (true) ，則不會在目標中停用超出範圍的帳戶。 此旗標設定于布建 *應用* 層級，而且可以使用圖形 API 來設定。 
...
在 Graph Explorer 中，執行下列命令以使用 ***SkipOutOfScopeDeletions*** 旗標更新秘密端點。 

在下列 URL 中，將 [servicePrincipalId] 取代為從[步驟 1](#step-1-retrieve-your-provisioning-app-service-principal-id-object-id)中解壓縮的**servicePrincipalId** 。 
