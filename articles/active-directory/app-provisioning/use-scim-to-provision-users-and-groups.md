...
|OAuth 授權碼授與|存取權杖的存留期遠比密碼短，且具有長期持有人權杖所沒有的自動化重新整理機制。  在初始授權期間必須有實際的使用者存在，因而增加了一層責任。 |必須要有使用者存在。 如果使用者離職，權杖就會失效，而必須重新完成授權。|支援資源庫應用程式，但不支援非資源庫應用程式。 非資源庫的支援在我們的待處理專案中。|
