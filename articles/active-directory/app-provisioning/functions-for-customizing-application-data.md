...
[附錄](#append)&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;位與&nbsp;CBool&nbsp;組合&nbsp;轉換到Base64 轉換到UTF8hex 計數 Cstr 日期從num 格式日期 Guid IIF instr isnullornull 是存在是存在是 字串 專案 加入左 中間規範化音調號不 刪除重複 替換選擇唯一值&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[CStr](#cstr)[IIF](#iif)[IsPresent](#ispresent)[FormatDateTime](#formatdatetime)&nbsp;[InStr](#instr)&nbsp;&nbsp;&nbsp;&nbsp;[BitAnd](#bitand)&nbsp;[Join](#join)[Mid](#mid)[Left](#left)[Coalesce](#coalesce)&nbsp;[Not](#not)[Replace](#replace)[Count](#count)&nbsp;[Item](#item)[Guid](#guid)[RemoveDuplicates](#removeduplicates)[CBool](#cbool)&nbsp;&nbsp;&nbsp;&nbsp;[IsNull](#isnull)&nbsp;[DateFromNum](#datefromnum)&nbsp;[NormalizeDiacritics](#normalizediacritics)[IsString](#isstring)[IsNullOrEmpty](#isnullorempty)[ConvertToBase64](#converttobase64)&nbsp;&nbsp;&nbsp;[SelectUniqueValue](#selectuniquevalue)[ConvertToUTF8Hex](#converttoutf8hex)&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;單應用配置&nbsp;分割&nbsp;條帶&nbsp;空間&nbsp;切換到 「下 轉到 上部 」&nbsp; &nbsp; [ ](#switch) [ ](#split) &nbsp; [ ](#singleapproleassignment) &nbsp; &nbsp; [ ](#toupper) [ ](#word) [ ](#stripspaces) &nbsp; [ ](#tolower)
...
**描述:**<br> CBool 傳回基於計算表達式的布林。 如果表達式計算為非零值,則 CBool 返回 True,否則返回 False。
...
**功能:**<br> 合併 (來源 1,源 2,...,預設值)

**描述:**<br> 返回第一個不是 NULL 的源值。 如果所有參數都為 NULL,並且存在預設值,則將返回預設值。 如果所有參數都是 NULL,並且不存在預設值,則合併傳回 NULL。

**參數:**<br> 

| 名稱 | 必要 / 重複 | 類型 | 注意 |
| --- | --- | --- | --- |
| **來源1 ...來源N** | 必要 | String |必需的可變次數。 通常為 source 物件的屬性名稱。 |
...
| 名稱 | 必要 / 重複 | 類型 | 注意 |
| --- | --- | --- | --- |
| **value** |必要 | 數值、參考或布林 | 可以是數值、參考屬性或布林值。 |
...
| 名稱 | 必要 / 重複 | 類型 | 注意 |
| --- | --- | --- | --- |
| **value** |必要 | Date | 要轉換日期時間類型的 AD 日期 |
...
**功能:**<br> InStr(value 1,值 2,開始,比較類型)
...
| **值1** |必要 |String |要搜尋的字串 |
| **值2** |必要 |String |要找到的字串 |
| **start** |選用 |整數 |開始位置以尋找子字串|
| **比較類型** |選用 |例舉 |可以是 vb 文字比較或 vbBinary 比較 |
...
| 名稱 | 必要 / 重複 | 類型 | 注意 |
| --- | --- | --- | --- |
| **expression** |必要 |expression |要計算的運算式 |
...
| **來源1 ...來源N** |必要，變動次數 |String |要聯結在一起的字串值。 |
...
| --- | --- | --- | --- |
| **字串** |必要 |屬性 | 要從中傳回字元的字串 |
| **納姆查理斯** |必要 |整數 | 識別從字串開頭(左)傳回的字元數的數字|
...
**描述:**<br> NumFromDate 函數將 DateTime 值轉換為活動目錄格式,該格式是設置[帳戶過期](https://docs.microsoft.com/windows/win32/adschema/a-accountexpires)等屬性所必需的。 使用此函數可將從雲HR應用(如工作日和成功因素)收到的DateTime值轉換為其等效的AD表示形式。 

**參數:**<br> 

| 名稱 | 必要 / 重複 | 類型 | 注意 |
| --- | --- | --- | --- |
| **value** |必要 | String | 受支援格式的日期時間字串。 如需支援的格式，請參閱https://msdn.microsoft.com/library/8kb3ddd4%28v=vs.110%29.aspx。 |

**範例:**<br>
* 工作日範例 <br>
  假設您要將屬性*ContractEndDate*從工作日(以格式*為 2020-12-31-08:00*映射到 AD 中的*帳戶過期*欄位)進行映射,下面介紹如何使用此函數並更改時區偏移以匹配區域設置。 
  `NumFromDate(Join("", FormatDateTime([ContractEndDate], "yyyy-MM-ddzzz", "yyyy-MM-dd"), "T23:59:59-08:00"))`

* 成功因素範例 <br>
  假設您要映射屬性*endDate*從成功因數這是在格式*M/d/yyyyhhhhh:ss tt*帳號*過期*欄位在 AD 中, 下面是如何使用此函數和更改時區偏移以符合您的區域設定.
...
| 名稱 | 必要 / 重複 | 類型 | 注意 |
| --- | --- | --- | --- |
| **屬性** |必要 |多值屬性 |將刪除重複項目的多值屬性|

**範例:**<br>
RemoveDuplicates([proxyAddresses])                                                                                                       
傳回已移除所有重複值的已清除代理地址屬性
...
| **樣本** |選用 |String |當提供**範本**值時,我們將在範本中查找**舊值**並將其替換為**源**值。 |
...
**描述:**<br> 若**來源**值符合 **key**，則傳回該 **key** 的 **value**。 若**來源**值不符合任何 key，則傳回 **defaultValue**.。  **key** 和 **value** 參數必須永遠成對出現。 函式必須要有偶數數目的參數。 該函數不應用於引用屬性(如管理員)。 
...
| --- | --- | --- | --- |
| **字串** |必要 |多值屬性 |要返回單詞的字串。|
| **字數** |必要 | 整數 | 識別回應的字型號的數字|
| **分隔符** |必要 |String| 表示應用於識別單字的分隔符號的字串|
...
* **輸入** (PreferredFirstName)："John"
* **輸入** (PreferredLastName)："Smith"
* **輸出**John.Smith@contoso.com: 如果目錄中John.Smith@contoso.com不存在的 UPN 值
* **輸出**J.Smith@contoso.com: 如果目錄中已John.Smith@contoso.com存在的 UPN 值
* **輸出**Jo.Smith@contoso.com:""如果目錄中已存在上述兩個 UPN 值

### <a name="flow-mail-value-if-not-null-otherwise-flow-userprincipalname"></a>串流郵件值(如果不是 NULL),否則流使用者主名稱
如果郵件屬性存在,則希望將其流式傳遞。 如果不是,您希望改為流用戶主體名稱的值。
...
* [帳號預先通知](../app-provisioning/user-provisioning.md)
