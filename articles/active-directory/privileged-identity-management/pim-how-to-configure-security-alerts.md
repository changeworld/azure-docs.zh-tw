...
| **防止** | 只將特殊許可權角色指派給具有商業理由的使用者。 </br>排程定期[存取權檢閱](pim-how-to-start-security-review.md)，以確認使用者仍然需要其存取權。 |
...
| **防止** | 針對每一個角色[要求 MFA](pim-how-to-change-default-settings.md)。  |
...
| **防止** | 確定共用帳戶會在知道密碼的使用者有所變更時，輪替使用強式密碼。 </br>使用[存取權審查](pim-how-to-start-security-review.md)和移除不再需要的角色指派，定期審查具有特殊許可權角色的帳戶。 |
| **入口網站內風險降低措施** | 從其特殊權限角色中移除帳戶。 |
...
| **防止** | 調查使用者在 Privileged Identity Management 外部指派特殊許可權角色的位置，並禁止未來指派。 |
...
| **防止** | 請確定為他們的特殊權限角色設定的[啟用持續時間](pim-how-to-change-default-settings.md)夠長，足以讓使用者執行他們的工作。</br>針對具有多個系統管理員共用帳戶的特殊許可權角色，[需要多重要素驗證](pim-how-to-change-default-settings.md)。 |
| **入口網站內風險降低措施** | N/A |
