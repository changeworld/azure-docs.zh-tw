...
- 指派為符合 Azure AD 的使用者或使用 PIM 管理的 Azure 角色的使用者
- 指派為合格成員或特殊許可權存取群組擁有者的使用者
- 可在 PIM 中核准或拒絕啟用要求的使用者
- 指派給存取權檢閱的使用者
- 執行存取權檢閱的使用者

下列工作**不**需要 Azure AD Premium P2 授權：

- 設定 PIM、設定原則、接收警示及設定存取權評論的使用者不需要任何授權。
...
| 狀況 | 計算 | 授權數目 |
| --- | --- | --- |
| Woodgrove Bank 有10位不同部門的系統管理員，以及設定和管理 PIM 的全域系統管理員。 他們讓五位系統管理員都符合資格。 | 適用于系統管理員的五個授權 | 5 |
| 圖形設計局有25個系統管理員，其中14個是透過 PIM 來管理。 角色啟用需要核准，且組織中有三個不同的使用者可核准啟用。 | 14個符合資格的角色授權，以及三個核准者 | 17 |
| Contoso 有50的系統管理員，其中42是透過 PIM 來管理。 角色啟用需要核准，且組織中有五個不同的使用者可核准啟用。 Contoso 也會對指派給系統管理員角色和審核者的使用者，進行每月的評論，也就是使用者的主管，其中六位不是 PIM 所管理的系統管理員角色。 | 適用于合格角色的42授權 + 五位核准者 + 六名審核者 | 53 |

## <a name="when-a-license-expires"></a>授權到期時

如果 Azure AD Premium P2、EMS E5 或試用版授權過期，您的目錄中將無法再使用 Privileged Identity Management 功能：

- Azure AD 角色的永久角色指派不會受到影響。
- Azure 入口網站中的 Privileged Identity Management 服務以及 Privileged Identity Management 的圖形 API Cmdlet 和 PowerShell 介面，將無法再供使用者啟用特殊許可權角色、管理特殊許可權存取，或執行特殊許可權角色的存取權檢查。
- Azure AD 角色的合格角色指派將會遭到移除，因為使用者將無法再啟動特殊權限的角色。
- Azure AD 角色的任何進行中存取權檢查將會結束，而且 Privileged Identity Management 的設定將會移除。
- Privileged Identity Management 將不再傳送有關角色指派變更的電子郵件。
