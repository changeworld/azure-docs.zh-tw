---
title: 使用 Azure 監視器整合記錄與 ArcSight |Microsoft Docs
...
4. 最後，完成設定指南**部署後設定**中的部署後步驟。 本節說明如何執行其他設定，以防止函 App Service 式應用程式在經過一段時間後進入閒置狀態、設定從事件中樞串流處理資源記錄檔，以及更新 SysLog NG Daemon SmartConnector 金鑰儲存區憑證，以將它與新建立的儲存體帳戶產生關聯。
