...
有了 Azure Active Directory (Azure AD) 報表，您就可以取得有關所有寫入作業的詳細資料，方法是以您的方向 (audit 記錄) 和驗證資料 (登入記錄) 。 雖然可以使用 MS 圖形 API 來取得資訊，但現在您可以使用 Azure AD PowerShell Cmdlet 來抓取相同的資料，以進行報告。
...
| 以上皆是              | AzureADAuditDirectoryLogs-Filter "initiatedBy/app/displayName eq ' Azure AD 雲端同步處理 ' 和分類 eq ' ApplicationManagement ' 和 activityDateTime gt 2019-04-18"|
...
| 建立日期時間              | AzureADAuditSignInLogs-Filter "createdDateTime gt 2019-04-18T17：30： 00.0 Z" (自4/18 年 5:30 pm 起的所有專案)  |
