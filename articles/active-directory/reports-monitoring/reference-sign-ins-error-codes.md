...
|50194|應用程式 ' {appId} ' （{appName}）未設定為多租使用者應用程式。 在 ' {time} ' 之後建立的這類應用程式不支援使用/common 端點。 使用租使用者特定端點，或將應用程式設定為多租使用者。|
...
|53032|已封鎖帳戶，因為 Azure AD Identity Protection 的原則。|
...
|500014|資源 ' {identifier} ' 已停用。|
|500021| 租使用者受到公司 proxy 的限制。 拒絕資源存取。|
|500121| 驗證在強式驗證要求期間失敗。|
|500133| Assert 陳述式不在其有效時間範圍內。 先確認存取權杖未過期，再將它用於使用者 Assert 陳述式，或要求新權杖。|
|500172|' {Issuer} ' 所簽發的憑證 ' {name} ' 無效。 目前時間： ' {curTime} '。 憑證 NotBefore： ' {startTime} '。 憑證 NotAfter： ' {endTime} '。|
|501291|用戶端應用程式是 Mam 應用程式，裝置未註冊，且會使用 broker 傳送要求。 您必須先完成工作位置聯結，才能存取應用程式。|
|530003|您的裝置必須受到管理才能存取此資源。|
...
|700051|應用程式未啟用 Response_type ' token '。 應用程式要求的回應類型不受支援，原因如下：應用程式未啟用 response_type ' token '。 應用程式擁有者應該移至 Azure 入口網站或呼叫 MS Graph，以啟用隱含存取權杖授與。|
