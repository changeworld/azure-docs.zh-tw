...
* 使用**Azure AD Premium P2 版本**時，您會取得有關所有基礎偵測的最詳細資訊。 
...
> [!NOTE]
> 如果登入使用者名稱和密碼不相符，登入將會失敗，且不會進行風險偵測。 從不熟悉的位置登入風險偵測只會在成功登入時觸發。
...
不熟悉的位置可以強烈指出攻擊者可能使用遭竊的身分識別。 當使用者正在移動、試用新裝置或使用新的 VPN 時，可能會發生誤判。 由於這些誤報，此事件種類的風險層級為「**中**」。
