...
- **使用者**-登入作業期間所使用的使用者名稱。
- **Ip** -用來連接到 Azure Active Directory 之裝置的 ip 位址。
- **Location** -用來連接到 Azure Active Directory 的位置。 這是根據追蹤、登錄資料、反向查詢及其他資訊，儘可能取得的近似值。
- **登入時間** - 執行登入的時間
- **狀態** - 登入狀態
...
- 已偵測到之[風險偵測類型](concept-risk-events.md)的相關匯總資訊。 使用**Azure AD Premium P1 版本**時，您的授權未涵蓋的偵測會顯示為偵測**到有額外風險**的「風險偵測」登入。 使用**Azure AD Premium P2 版本**時，您會取得有關所有基礎偵測的最詳細資訊。
