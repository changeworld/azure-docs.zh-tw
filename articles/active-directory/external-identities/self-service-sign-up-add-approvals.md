...
9. 在 [ **API 許可權**] 頁面上，選取 **[授與系統管理員同意] (您的租使用者名稱) **，然後選取 **[是]**。
...
- **要求核准**-在使用者完成 [屬性集合] 頁面後，但在建立使用者帳戶之前，傳送對核准系統的呼叫，以要求核准。 核准要求可以自動授與或手動審核。 「要求核准」 API 連接器的範例。 
...
3. 核准系統會使用受邀使用者的識別碼來更新使用者帳戶，並將所收集的使用者屬性 (選擇性) 。
