...
description: 使用 Azure Active Directory (Azure AD) API 連接器，使用 web Api 來自訂和擴充您的自助式註冊使用者流程。
...
在使用者向身分識別提供者驗證 (Google、Facebook、Azure AD) 之後，就會立即叫用註冊程式中此步驟的 API 連接器。 此步驟會在 [***屬性集合] 頁面***之前，這是向使用者呈現以收集使用者屬性的表單。 以下是您可能會在此步驟啟用的 API 連接器案例範例：
