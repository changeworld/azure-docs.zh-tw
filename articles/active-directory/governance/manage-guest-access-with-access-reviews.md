...
> [!NOTE]
> 本文件著重於檢閱來賓使用者的存取權。 如果您想檢閱所有使用者的存取權 (不只來賓)，請參閱[使用存取權檢閱管理使用者存取權](manage-user-access-with-access-reviews.md)。 如果要查看使用者在管理角色（如全域管理員）中的成員身份，請參閱在[Azure AD 特權標識管理中啟動訪問審核](../privileged-identity-management/pim-how-to-start-security-review.md)。

## <a name="prerequisites"></a>Prerequisites

- Azure AD Premium P2

有關詳細資訊，[許可證要求](access-reviews-overview.md#license-requirements)。

## <a name="create-and-perform-an-access-review-for-guests"></a>建立和執行來賓的存取權檢閱

首先，作為全域管理員或使用者管理員，請轉到["標識治理"頁](https://portal.azure.com/#blade/Microsoft_AAD_ERM/DashboardBlade/)，以確保訪問審核已為您的組織做好準備。
...
1. 要為組創建訪問審閱，請選擇審核以僅包括來賓使用者成員，並且成員自行審閱。 有關詳細資訊，請參閱[創建組或應用程式的訪問審閱](create-access-review.md)。

2. 要求每個來賓檢閱他們自己的成員資格。 根據預設，已接受邀請的每個來賓都會收到 Azure AD 的電子郵件，其中包含存取權檢閱的連結。 Azure AD 為來賓提供了有關如何[查看對組或應用程式的訪問](perform-access-review.md)的說明。

3. 在檢閱者提供輸入後，停止存取權檢閱並套用變更。 有關詳細資訊，請參閱[完成組或應用程式的訪問審閱](complete-access-review.md)。
...
1. 要為組創建訪問審閱，請選擇審核以僅包括來賓使用者成員。 然後指定一或多個檢閱者。 有關詳細資訊，請參閱[創建組或應用程式的訪問審閱](create-access-review.md)。

2. 要求檢閱者提供意見。 預設情況下，他們各自會收到一封來自 Azure AD 的電子郵件，其中帶有指向訪問面板的連結，其中他們[查看對組或應用程式的訪問](perform-access-review.md)。
...
1. 要為應用程式創建訪問審閱，請選擇審核以僅包括來賓，以及使用者查看自己的存取權限。 有關詳細資訊，請參閱[創建組或應用程式的訪問審閱](create-access-review.md)。

2. 要求每個來賓檢閱其自己的應用程式存取權。 根據預設，已接受邀請的每個來賓都會收到 Azure AD 的電子郵件。 該電子郵件有一個連結可連至貴組織存取面板中的存取權檢閱。 Azure AD 為來賓提供了有關如何[查看對組或應用程式的訪問](perform-access-review.md)的說明。
...
1. 要為應用程式創建訪問審閱，請選擇審核以僅包括來賓。 然後指定一或多個使用者作為檢閱者。 有關詳細資訊，請參閱[創建組或應用程式的訪問審閱](create-access-review.md)。
...
2. 要為該組創建訪問審閱，請選擇檢閱者作為成員本身。 有關詳細資訊，請參閱[創建組或應用程式的訪問審閱](create-access-review.md)。

3. 要求每個來賓檢閱他們自己的成員資格。 根據預設，已接受邀請的每個來賓會收到 Azure AD 的電子郵件，其中的連結可連至您組織的存取面板以執行存取權檢閱。 Azure AD 為來賓提供了有關如何[查看對組或應用程式的訪問](perform-access-review.md)的說明。  未接受邀請的來賓會在檢閱結果中顯示為「未通知到」。

4. 在檢閱者提供輸入後，停止存取權檢閱。 有關詳細資訊，請參閱[完成組或應用程式的訪問審閱](complete-access-review.md)。
