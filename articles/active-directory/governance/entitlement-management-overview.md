...
- 任何人都不知道其他組織目錄中的所有特定人員都能夠邀請他們
- 即使他們可以邀請這些使用者，組織中也不會有任何人記得以一致的方式管理所有使用者的存取權

Azure AD 權利管理有助於解決這些挑戰。  若要深入瞭解客戶如何使用 Azure AD 權利管理，您可以閱讀[Avanade 案例研究](https://customers.microsoft.com/story/avanade-professional-services-azure-canada)和[Centrica 案例研究](https://customers.microsoft.com/story/757467-centrica-energy-azure)。  這段影片提供權利管理和其價值的總覽：
...
- 員工對於特定工作需要有時間限制的存取權。  例如，您可以使用以群組為基礎的授權和動態群組，以確保所有員工都擁有 Exchange Online 信箱，然後在員工需要額外存取權的情況下使用存取套件，例如從另一個部門讀取部門資源。
...
特製化雲端（例如 Azure 德國和 Azure 中國世紀）目前無法供使用。

### <a name="how-many-licenses-must-you-have"></a>您必須擁有多少個授權？

請確定您的目錄至少有您所擁有的 Azure AD Premium P2 授權數目：
