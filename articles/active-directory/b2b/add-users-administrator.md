---
title: 在 Azure 門戶 - Azure AD 中添加 B2B 協作使用者
...
作為分配任何有限管理員目錄角色的使用者，可以使用 Azure 門戶邀請 B2B 協作使用者。 您可以將來賓使用者邀請到目錄、群組或應用程式。 透過上述任一方法邀請使用者之後，受邀使用者的帳戶就會新增至 Azure Active Directory (Azure AD)，且使用者類型為「來賓」**。 來賓使用者接著必須兌換其邀請才能存取資源。 使用者的邀請不會過期。
...
確保組織的外部協作設置已配置為允許您邀請來賓。 預設情況下，所有使用者和管理員都可以邀請來賓。 但是，您的組織的外部協作策略可能配置為防止某些類型的使用者或管理員邀請來賓。 要瞭解如何查看和設置這些策略，請參閱[啟用 B2B 外部協作並管理誰可以邀請來賓](delegate-invitations.md)。

## <a name="add-guest-users-to-the-directory"></a>將來賓使用者新增到目錄

若要將 B2B 共同作業使用者新增至目錄，請遵循下列步驟：

1. 以分配有限管理員目錄角色或來賓邀請者角色的使用者身份登錄到[Azure 門戶](https://portal.azure.com)。
2. 從任何頁面搜索並選擇**Azure 活動目錄**。
3. 在 [管理]**** 底下選取 [使用者]****。
4. 選取 [新增來賓使用者]****。
...
5. 在 [新增使用者]**** 頁面上選取 [邀請使用者]****，然後新增來賓使用者的資訊。 
   ...
   - **組**：您可以將來賓使用者添加到一個或多個現有組，也可以稍後執行此操作。
   - **目錄角色**：如果需要使用者的 Azure AD 管理許可權，則可以將它們添加到 Azure AD 角色。 
...
如果需要手動將 B2B 協作使用者添加到組，請按照以下步驟操作：
