---
title: 排除 B2B 協作故障 - Azure 活動目錄 |微軟文檔
...
如果收到通知您沒有邀請使用者的許可權，請驗證您的使用者帳戶是否有權在 Azure 活動目錄>使用者設置下邀請外部使用者>外部使用者>管理外部協作設置：

![顯示外部使用者設置的螢幕截圖](media/troubleshoot/external-user-settings.png)
...
![聲明租戶的錯誤不允許通過電子郵件發送已驗證的使用者](media/troubleshoot/allow-email-verified-users.png)
...
## <a name="you-receive-an-aadsts65005-error-when-you-try-to-log-in-to-an-azure-resource"></a>當您嘗試登錄到 Azure 資源時，您會收到"AADSTS65005"錯誤

具有來賓帳戶的使用者無法登錄，並且正在收到以下錯誤訊息：

    AADSTS65005: Using application 'AppName' is currently not supported for your organization contoso.com because it is in an unmanaged state. An administrator needs to claim ownership of the company by DNS validation of contoso.com before the application AppName can be provisioned.

使用者具有 Azure 使用者帳戶，並且是已放棄或非託管的病毒租戶。 此外，租戶中沒有全域或公司管理員。

要解決此問題，您必須接管已放棄的租戶。 請參閱[在 Azure 活動目錄中以管理員身份接管非託管目錄](https://docs.microsoft.com/azure/active-directory/users-groups-roles/domains-admin-takeover)。 您還必須訪問相關域尾碼的面向 Internet 的 DNS，以便提供直接證據證明您控制了命名空間。 在租戶返回到託管狀態後，請與客戶討論離開使用者和驗證的功能變數名稱是否是其組織的最佳選擇。
...
自 2019 年 11 月 18 日起，目錄中的來賓使用者（定義為**使用者類型**屬性等於**來賓**的使用者帳戶）被阻止使用 AzureAD PowerShell V1 模組。 今後，使用者需要是成員使用者（**使用者類型**等於**成員**），或者使用 AzureAD PowerShell V2 模組。

## <a name="in-an-azure-us-government-tenant-i-cant-invite-a-b2b-collaboration-guest-user"></a>在 Azure 美國政府租戶中，我無法邀請 B2B 協作來賓使用者

在 Azure 美國政府雲中，B2B 協作目前僅在 Azure 美國政府雲中同時支援並且都支援 B2B 協作的租戶之間提供支援。 如果邀請不屬於 Azure 美國政府雲或尚不支援 B2B 協作的租戶中的使用者，則將出錯。 有關詳細資訊和限制，請參閱[Azure 活動目錄高級 P1 和 P2 變體](https://docs.microsoft.com/azure/azure-government/documentation-government-services-securityandidentity#azure-active-directory-premium-p1-and-p2)。
