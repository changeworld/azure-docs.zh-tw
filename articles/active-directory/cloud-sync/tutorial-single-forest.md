...
description: 本主題說明雲端同步處理的先決條件和硬體需求。
...
ms.locfileid: "98611900"
---
# <a name="tutorial-integrate-a-single-forest-with-a-single-azure-ad-tenant"></a>教學課程：整合單一樹系與單一 Azure AD 租用戶

本教學課程會逐步引導您使用 Azure Active Directory (Azure AD) Connect cloud sync 來建立混合式身分識別環境。

![建立](media/tutorial-single-forest/diagram-2.png)

您可以使用您在本教學課程中建立的環境來進行測試，或更熟悉雲端同步處理。
...
3. 在左側選取 [ **Azure Active Directory**]，按一下 [ **Azure AD Connect**]，然後在中央選取 [ **管理雲端同步**]。

   ![Azure 入口網站](media/how-to-install/install-6.png)

4. 按一下 [下載代理程式]。
5. 執行 Azure AD Connect 佈建代理程式。
6. 在啟動顯示畫面上 **接受** 授權條款，然後按一下 [安裝]。

   ![此螢幕擷取畫面顯示 [Microsoft Azure AD Connect 佈建代理程式套件] 啟動顯示畫面。](media/how-to-install/install-1.png)
...
8. 在 [連接 Active Directory] 畫面上，按一下 [新增目錄]，然後以您的 Active Directory 網域系統管理員帳戶登入。  注意：網域系統管理員帳戶應該不會有密碼變更需求。 如果密碼過期或變更，您必須使用新的認證重新設定代理程式。 此作業將會新增您的內部部署目錄。  按 [下一步] 。
   ...
   ![[連線 Active Directory] 畫面的螢幕擷取畫面。](media/how-to-install/install-3a.png)

9. 在 [設定完成] 畫面上，按一下 [確認]。  此作業將會註冊並重新啟動代理程式。

   ![顯示 [組態完成] 畫面的螢幕擷取畫面。](media/how-to-install/install-4a.png)
...
- [什麼是 Azure AD Connect 雲端佈建？](what-is-cloud-sync.md)