---
title: 將短信設置為驗證方法 - Azure AD
description: 如何設置安全資訊（預覽）頁面，以使用短信作為驗證方法驗證您的身份。
...
---
# <a name="set-up-text-messaging-as-your-verification-method"></a>將短信設置為驗證方法
...
5. 將透過簡訊傳送給您的代碼輸入到行動裝置內，然後選取 [下一步]****。
    ...
    ![成功通知、連接電話號碼、接收短信的選擇以及您的帳戶](media/security-info/securityinfo-myprofile-phonetext-success.png)
...
如果希望文本消息是使用雙因素驗證或密碼重設請求登錄工作或學校帳戶時使用的預設方法，則可以從 **"安全資訊**"頁設置它。
...
2. 從可用方法的下拉式清單中選取 [電話 - 傳簡訊到 (your_phone_number)]***__***，然後選取 [確認]****。

    ![選擇預設登入方法](media/security-info/securityinfo-myprofile-phonetext-changeddefault.png)

    用於登入的預設方法會變更為 [電話 - 傳簡訊到 (your_phone_number)]***__***。
