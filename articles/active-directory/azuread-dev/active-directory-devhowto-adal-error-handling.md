...
- [原生用戶端](../develop/developer-glossary.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json#native-client) 應用程式 (IOS、Android、.net Desktop 或 Xamarin) 
...
如果您正在建置原生用戶端應用程式，則需考量幾個錯誤處理案例，而這些案例與網路問題、暫時性失敗和其他平台特有錯誤相關。 在大多數情況下，應用程式不應該執行立即重試，而是等候使用者互動以提示登入。 
...
|  |  |
|------|-------------|
| **案例 1**：<br>無法重試的錯誤 (大部分的案例) | 1. 請勿嘗試立即重試。 根據叫用重 (試的特定錯誤顯示終端使用者 UI，例如，「嘗試重新登入」或「下載 Azure AD broker 應用程式」 ) 。 |
| **案例 2**：<br>可重試的錯誤 | 1. 執行單一重試，因為終端使用者可能已進入會導致成功的狀態。<br><br>2. 如果重試失敗，請根據叫用重試的特定錯誤顯示終端使用者 UI ( 「再次嘗試登入」、「下載 Azure AD broker 應用程式」等 ) 。 |
...
|  |  |
|------|-------------|
| **案例 1**：<br>可透過互動式要求解決 | 1. 如果登入 ( # A1 失敗，請勿立即執行重試。 只在使用者動作提示重試之後重試。|
| **案例 2**：<br>無法透過互動式要求解決。 錯誤可重試。 | 1. 執行單一重試，因為終端使用者的主要進入成功的狀態。<br><br>2. 如果重試失敗，請根據可叫用重試 ( 「再次嘗試登入」 ) 的特定錯誤，向終端使用者顯示動作。 |
...
### <a name="logging-personal-identifiable-information--organizational-identifiable-information"></a>記錄個人識別資訊 & 組織識別資訊 
依預設，ADAL 記錄不會捕捉或記錄任何個人識別資訊或組織標識資訊。 文件庫可讓應用程式開發人員透過記錄器類別的 setter 來開啟此選項。 藉由記錄個人識別資訊或組織識別資訊，應用程式會負責安全地處理高度敏感的資料，並遵守任何法規需求。
...
使用者使用 Web 檢視時可能會在登入時造成 iOS 錯誤 (驗證特性)。 這可能是因為 TLS 錯誤、超時或網路錯誤等狀況所造成：
