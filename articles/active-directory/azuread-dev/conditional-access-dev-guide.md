...
讓我們逐步解說使用條件式存取案例的範例。 使用者只進入在網站上，而且沒有會話。 我們執行 `login()` 呼叫時，無需進行多重要素驗證即可取得 ID 權杖。 然後使用者會叫用按鈕，要求應用程式向 web API 要求資料。 應用程式嘗試進行`acquireToken()`呼叫但失敗，因為使用者尚未執行多重要素驗證，而且必須符合條件式存取原則。
