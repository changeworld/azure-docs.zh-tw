...
許多 Web Apps 不僅需要將使用者登入，同時需要使用 OAuth 代表使用者來存取 Web 服務。 這個案例會合併 OpenID Connect 以進行使用者驗證，同時使用 `authorization_code`OAuth 授權碼流程`access_tokens`取得 [，藉此取得 ](v1-protocols-oauth-code.md#use-the-authorization-code-to-request-an-access-token)。
...
* 深入了解 [`id_token` 和宣告](../develop/id-tokens.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json)。
