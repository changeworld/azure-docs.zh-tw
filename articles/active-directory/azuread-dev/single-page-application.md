...
單頁應用程式 （SPA） 通常構造為在瀏覽器中運行的 JavaScript 展示層（前端）和在伺服器上運行並實現應用程式的業務邏輯的 Web API 後端。要瞭解有關隱式授權授予的更多資訊，並説明您確定它是否適合您的應用程式方案，請參閱[在 Azure 活動目錄中瞭解 OAuth2 隱式授予流](v1-oauth2-implicit-grant-flow.md)。
...
1. 使用者在登入頁面上登入。
1. 如果身份驗證成功，Azure AD 將創建一個 ID 權杖，並將其作為 URL 片段 （#） 返回到應用程式的 Reply URL。 對於實際執行應用程式，此回覆 URL 應該為 HTTPS。 傳回的權杖包含應用程式驗證權杖所需的使用者與 Azure AD 宣告。
1. 在瀏覽器中運行的 JavaScript 用戶端代碼從回應中提取權杖，用於保護對應用程式的 Web API 後端的調用。
1. 瀏覽器使用授權標頭中的 ID 權杖調用應用程式的 Web API 後端。 Azure AD 驗證服務會發出識別碼權杖，如果資源與用戶端識別碼相同，即可用來作為持有人權杖 (在此情況下，這是因為 Web API 是應用程式本身的後端)。
...
註冊應用程式之後，它必須設定為使用 OAuth 2.0 隱含授權通訊協定。 根據預設，應用程式都停用此通訊協定。 要為應用程式啟用 OAuth2 隱式授予協定，請從 Azure 門戶編輯其應用程式清單，並將"oauth2Allow隱式流"值設置為 true。 如需詳細資訊，請參閱[應用程式資訊清單](../develop/reference-app-manifest.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json)。
...
成功驗證之後，Azure AD 會將 Cookie 寫入使用者的瀏覽器以建立工作階段。 請注意，工作階段存在於使用者與 Azure AD 之間，而不是在使用者與 Web 應用程式之間。 當權杖過期時，ADAL.js 會使用此工作階段來自動取得另一個權杖。 ADAL.js 會使用隱藏的 iFrame 並透過 OAuth 隱含授權通訊協定，來傳送和接收要求。 ADAL.js 也可以使用相同的機制靜默方式獲取應用程式調用的其他 Web API 資源的訪問權杖，只要這些資源支援跨源資源分享 （CORS），在使用者的目錄中註冊，並且任何必需的同意都是使用者在登錄期間提供。
