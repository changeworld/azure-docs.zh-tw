...
當服務收到 Azure AD 租使用者所發行的權杖時，必須使用在同盟元資料檔案中發行的簽署金鑰來驗證權杖的簽章。 同盟中繼資料包含租用戶用於權杖簽署之憑證的公開部分。 憑證原始位元組會出現在 `KeyDescriptor` 項目中。 當 `use` 屬性值為 `signing` 時，權杖簽署憑證僅能適用於簽署。
