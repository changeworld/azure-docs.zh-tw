---
title: 適用于開發人員 (v1.0) 的 Azure AD |蔚藍
description: 瞭解 (v1.0) 的開發人員 Azure AD 的驗證基本概念，例如應用程式模型、API、布建，以及最常見的驗證案例。
...
適用于開發人員的 Azure Active Directory (v1.0)  (Azure AD) 透過提供身分識別即服務來簡化應用程式開發人員的驗證，並支援業界標準的通訊協定（例如 OAuth 2.0 和 OpenID Connect），以及適用于不同平臺的開放原始碼程式庫，協助您快速開始撰寫程式碼。
...
* 想要將驗證外包給 Azure AD 的應用程式，必須在 Azure Active Directory (Azure AD) 中註冊。 Azure AD 會在目錄中註冊並唯一識別該應用程式。
...
* 使用者經過驗證之後，應用程式必須驗證使用者的安全性權杖，以確定驗證成功。 您可以找到以各種不同語言和架構提供的快速入門、教學課程和程式碼範例，它們會說明應用程式必須執行哪些作業。
...
* 驗證程序的要求和回應流程是由您所使用的驗證通訊協定決定，例如 OAuth 2.0、OpenID Connect、WS-同盟或 SAML 2.0。 如需通訊協定的詳細資訊，請參閱檔的 **> 驗證通訊協定的概念** 一節。
...
視用戶端的建立方式而定，它可以使用一 (或多個 Azure AD 所支援的驗證流程) 。 這些流程可能會產生各種權杖 (id_tokens、重新整理權杖、存取權杖) 以及授權碼，而且需要不同的權杖才能使其運作。 此圖表提供概觀：
