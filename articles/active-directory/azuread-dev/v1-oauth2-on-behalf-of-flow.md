...
這些步驟會構成代理者流程：![顯示 OAuth2.0 代理者流程中的步驟](./media/v1-oauth2-on-behalf-of-flow/active-directory-protocols-oauth-on-behalf-of-flow.png)
...
1. 在文字欄位中輸入中介層服務的名稱。
1. 選擇 [**選取許可權**]，然後選取 [**存取 \<service name> **]。
...
下列 HTTP POST 會要求提供 https://graph.microsoft.com Web API 的存取權杖。 `client_id` 會識別要求存取權杖的服務。
...
下列 HTTP POST 會使用憑證要求提供 https://graph.microsoft.com Web API 的存取權杖。 `client_id` 會識別要求存取權杖的服務。
...
Azure AD 權杖端點在嘗試取得已設定條件式存取原則 (例如多重要素驗證) 之下游 API 的存取權杖時，會傳回錯誤回應。 中介層服務應該向用戶端應用程式呈現此錯誤，以便用戶端應用程式可以提供使用者互動，以滿足條件式存取原則。
