...
本指南中的許多建議可以使用 Azure AD 免費實現或根本沒有許可證。 若需要授權，我們會陳述至少需要哪一個授權才能完成工作。
...
## <a name="phase-1-build-a-foundation-of-security"></a>第 1 階段：建立安全基礎
...
| [創建特定于組織的自訂禁止密碼清單](../authentication/howto-password-ban-bad-configure.md) | 防止使用者從您的組織或區域建立包含常見字組或片語的密碼。 | |
...
| [使用條件訪問策略部署 Azure AD 多重要素驗證](../authentication/howto-mfa-getstarted.md) | 要求使用者在使用條件訪問策略訪問敏感應用程式時執行兩步驗證。 | Azure AD Premium P1 |
...
| [使用風險檢測觸發多重要素驗證和密碼更改](../authentication/tutorial-risk-based-sspr-mfa.md) | 啟用可觸發以下事件的自動化功能：例如多重要素驗證、密碼重設，以及根據風險封鎖登入。 | Azure AD Premium P2 |
...
## <a name="phase-2-import-users-enable-synchronization-and-manage-devices"></a>第 2 階段：導入使用者、啟用同步和管理設備
...
| [在組織中部署 Windows Hello 企業版](/windows/security/identity-protection/hello-for-business/hello-manage-in-organization) | 使用 Windows Hello 準備無密碼身份驗證 | |
| [為使用者部署無密碼身份驗證方法](../authentication/concept-authentication-passwordless.md) | 為使用者提供方便的無密碼身份驗證方法 | Azure AD Premium P1 |

## <a name="phase-3-manage-applications"></a>第 3 階段：管理應用程式
...
## <a name="phase-4-audit-privileged-identities-complete-an-access-review-and-manage-user-lifecycle"></a>第 4 階段：審核特權標識、完成訪問審核和管理使用者生命週期
