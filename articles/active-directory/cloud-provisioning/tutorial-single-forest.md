---
title: 教程 - 將單個林與單個 Azure AD 租戶集成
...
ms.locfileid: "80332242"
---
# <a name="tutorial-integrate-a-single-forest-with-a-single-azure-ad-tenant"></a>教程：將單個林與單個 Azure AD 租戶集成

本教程將引導您使用 Azure 活動目錄 （Azure AD） 連接雲資源調配創建混合標識環境。

![建立](media/tutorial-single-forest/diagram1.png)

您可以使用本教程中創建的環境進行測試或更熟悉雲資源調配。
...
1. 識別運行 Windows Server 2012 R2 或更高域的主機伺服器，至少運行 4 GB RAM 和 .NET 4.7.1+ 運行時 
...
2. 如果您的伺服器和 Azure AD 之間有防火牆，請設定下列項目：
   ...
   - 如果您的防火牆或代理允許您指定安全尾碼，則將連接 t 添加到**\*.msappproxy.net**和**\*.servicebus.windows.net**。 如果不允許建立，請允許存取每週更新的 [Azure 資料中心 IP 範圍](https://www.microsoft.com/download/details.aspx?id=41653)。
...
3. 在左側，選擇**Azure 活動目錄**，按一下**Azure AD 連接**，並在中心選擇 **"管理預配（預覽）"。**

   ![Azure 入口網站](media/how-to-install/install6.png)

4. 按一下 **"下載代理**"。
5. 運行 Azure AD 連接預配代理。
...
8. 在 [連接 Active Directory]**** 畫面上，按一下 [新增目錄]****，然後以您的 Active Directory 網域系統管理員帳戶登入。  注： 域管理員帳戶不應有密碼更改要求。 如果密碼過期或變更，您必須使用新的認證重新設定代理程式。 此作業將會新增您的內部部署目錄。  按 [下一步]****。
...
10. 此操作完成後，您應該會看到一條通知：**您的代理配置已成功驗證。**  您可以按一下 [結束]****。</br>
...
1. 流覽到[https://myapps.microsoft.com](https://myapps.microsoft.com)
