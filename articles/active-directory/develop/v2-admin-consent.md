...
|`scope`        | 必要      | 定義應用程式請求的許可權集。 這可以是靜態（使用 /.default）或動態作用域。  這可以包括 OIDC 作用域`openid`（、 `profile` `email`。 。 | 
...
- 瞭解在[授權代碼授予流期間，OAuth 2.0 協定層如何支援同意](v2-oauth2-auth-code-flow.md#request-an-authorization-code)。
