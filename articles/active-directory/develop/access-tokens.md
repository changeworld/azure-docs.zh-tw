...
> [!IMPORTANT]
> 存取權杖是根據權杖的*物件*來建立，這表示在權杖中擁有範圍的應用程式。  這是[應用程式資訊清單](reference-app-manifest.md#manifest-reference)中的資源設定`2` `accessTokenAcceptedVersion`如何允許呼叫 v1.0 端點的用戶端接收 v2.0 存取權杖。  同樣地，這也是為什麼針對您的用戶端變更存取權杖[選擇性宣告](active-directory-optional-claims.md) `user.read`，並不會變更在要求權杖時所收到的存取權杖，這是由資源所擁有。
...
如果有值可填入宣告時，宣告才會存在。 因此，您的應用程式不應該相依于目前存在的宣告。 範例包括`pwd_exp` （不是每個租使用者都需要密碼`family_name`到期），或（用戶端[認證（v1.0](../azuread-dev/v1-oauth2-client-creds-grant-flow.md)，v2.0）流程代表不具有名稱的應用[程式）。](v2-oauth2-client-creds-grant-flow.md) 用於驗證存取權杖的宣告一律會存在。
...
為確保權杖大小不會超過 HTTP 標頭大小限制，Azure AD 會限制在群組宣告中包含的物件識別碼數目。 如果使用者隸屬于超額限制的群組（SAML 權杖為150，JWT 權杖為200），則 Azure AD 不會在權杖中發出群組宣告。 相反地，它會在權杖中包含超額宣告，指示應用程式查詢 Microsoft Graph API 以取得使用者的群組成員資格。
...
您可以使用 [ `BulkCreateGroups.ps1` [應用程式建立腳本](https://github.com/Azure-Samples/active-directory-aspnetcore-webapp-openidconnect-v2/tree/master/5-WebApp-AuthZ/5-2-Groups/AppCreationScripts)] 資料夾中提供的來協助測試超額案例。
...
我們會提供程式庫和程式碼範例，以示範如何處理權杖驗證。 下列資訊專門提供給想要了解基礎程序的人。 另外還有數個協力廠商開放原始碼程式庫可用於 JWT 驗證-幾乎每個平臺和語言都有至少一個選項。 如需有關 Azure AD 驗證程式庫和程式碼範例的詳細資訊，請參閱 [v1.0 驗證程式庫](../azuread-dev/active-directory-authentication-libraries.md)和 [v2.0 驗證程式庫](reference-v2-libraries.md)。

### <a name="validating-the-signature"></a>驗證簽章

JWT 包含三個區段 (以 `.` 字元分隔)。 第一個區段稱為**標頭**、第二個稱為**主體**，而第三個稱為**簽章**。 簽章區段可用來驗證權杖的真實性，您的應用程式才得以信任。

Azure AD 所簽發的權杖是使用業界標準非對稱式加密演算法（例如 RS256）來簽署。 JWT 標頭包含用來簽署權杖的金鑰和加密方法相關資訊：
...
`alg`宣告會指出用來簽署權杖的演算法，而`kid`宣告則表示用來驗證權杖的特定公開金鑰。
...
> [!TIP]
> 請在瀏覽器中嘗試此[URL](https://login.microsoftonline.com/common/v2.0/.well-known/openid-configuration) ！

此中繼資料文件：

* 是 JSON 物件，其中包含數個有用的資訊片段，例如執行 OpenID Connect 驗證所需的各種端點位置。
* 包含 `jwks_uri`，其提供用來簽署權杖的公用金鑰組位置。 位於的`jwks_uri` JSON Web 金鑰（JWK）包含在該特定時間點使用的所有公開金鑰資訊。  JWK 格式如[RFC 7517](https://tools.ietf.org/html/rfc7517)中所述。  您的應用程式可以使用 JWT 標頭中的 `kid` 宣告選取本文件中已用來簽署特定權杖的公開金鑰。 然後，它可以使用正確的公開金鑰和指定的演算法來執行簽章驗證。
...
如果您的應用程式具有自訂簽署金鑰做為使用[宣告對應](active-directory-claims-mapping.md)功能的結果，您必須附加`appid`包含應用程式識別碼的查詢參數，才能`jwks_uri`取得指向應用程式簽署金鑰資訊的，這應該用於驗證。 例如： `https://login.microsoftonline.com/{tenant}/.well-known/openid-configuration?appid=6731de76-14a6-49ae-97bc-6eba6914391e`包含`jwks_uri`的。 `https://login.microsoftonline.com/{tenant}/discovery/keys?appid=6731de76-14a6-49ae-97bc-6eba6914391e`
...
* 僅限應用程式的權杖不會`scp`有宣告，而可以改為`roles`使用宣告。 這是會記錄應用程式權限 (而不是委派的權限) 的位置。 如需委派和應用程式許可權的詳細資訊，請參閱許可權和[同意（v1.0](../azuread-dev/v1-permissions-consent.md) [、v2.0](v2-permissions-and-consent.md)）。
...
使用[權杖存留期](active-directory-configurable-token-lifetimes.md)設定，可以更改重新整理權杖的存留期。  這是正常的，而且在某些權杖不使用的情況下（例如，使用者沒有在3個月內開啟應用程式），因此會過期。  應用程式會遇到登入伺服器因為其存留期而拒絕重新整理權杖的案例。 
...
重新整理權杖可由伺服器撤銷，因為認證變更，或因為使用或系統管理動作。  重新整理權杖可分為兩個類別：發給機密用戶端（最右邊的資料行），以及發行給公用用戶端（所有其他資料行）的類別。   
...
| 管理員透過[PowerShell](https://docs.microsoft.com/powershell/module/azuread/revoke-azureaduserallrefreshtoken)撤銷使用者的所有重新整理權杖 | 已撤銷 | 已撤銷 |已撤銷 | 已撤銷 | 已撤銷 |
...
>
> 當重新整理權杖用來提取新的存取權杖和重新整理權杖時，不會使其失效或撤銷。  不過，您的應用程式應該在使用時捨棄舊的帳戶，並將其取代為新權杖，因為新權杖在其中有新的到期時間。 

## <a name="next-steps"></a>後續步驟

* 深入瞭解 Azure AD。 [ `id_tokens` ](id-tokens.md)
