...
---
# <a name="microsoft-identity-platform-access-tokens"></a>微軟識別平台存取權杖

存取權杖可讓用戶端安全地呼叫受 Azure 保護的 API。 微軟標識平台訪問令牌是[JWTs,Base64](https://tools.ietf.org/html/rfc7519)編碼的JSON物件由Azure簽名。 用戶端應將存取權杖視為不透明字串，因為權杖的內容僅會用於存取資源。 若要用於驗證及偵錯，開發人員可以使用 [jwt.ms](https://jwt.ms) 等網站來解碼 JWT。 用戶端可以使用各種協定從 v1.0 終結點或 v2.0 終結點獲取訪問權杖。

當用戶端請求訪問權杖時,Azure AD 還會返回有關訪問權杖的某些元資料,供應用使用。 此資訊包括存取權杖的到期時間以及其有效範圍。 此資料可讓應用程式執行存取權杖的智慧型快取，而不需剖析存取權杖本身。
...
> [!IMPORTANT]
> 訪問權杖基於權杖的訪問*群體*創建,這意味著在權杖中擁有作用域的應用程式。  這是[應用清單](reference-app-manifest.md#manifest-reference)中的`2`資源`accessTokenAcceptedVersion`設置 ,用於允許調用 v1.0 終結點的用戶端接收 v2.0 訪問權杖的方式。  同樣,這就是為什麼更改客戶端的訪問權[杖 可選聲明](active-directory-optional-claims.md)`user.read`不會更改請求 的令牌時收到的訪問權杖,該權杖歸資源所有。
> 出於同樣的原因,在使用個人帳戶(如hotmail.com或outlook.com)測試客戶端應用程式時,您可能會發現用戶端接收的訪問權杖是不透明字串。 這是因為正在訪問的資源請求了已加密且客戶端無法理解的舊 MSA(Microsoft 帳戶)票證。
...
如果有值可填入宣告時，宣告才會存在。 因此,你的應用不應依賴存在的聲明。 範例包括`pwd_exp`(不是每個租戶都需要密碼過期)`family_name`或 (用戶端認證[(v1.0,](../azuread-dev/v1-oauth2-client-creds-grant-flow.md) [v2.0)](v2-oauth2-client-creds-grant-flow.md)串流代表沒有名稱的應用程式)。 用於驗證存取權杖的宣告一律會存在。
...
| 宣告 | [格式] | 描述 |
|-----|--------|-------------|
| `aud` | 字串，應用程式識別碼 URI | 識別權杖的預定接收者。 在 ID 權中,訪問群體是應用的應用程式 ID,分配給 Azure 門戶中的應用。 您的應用程式應驗證此值，並拒絕值不相符的權杖。 |
...
| `azp` | 字串，GUID | 只在 v2.0 權杖中存在`appid`,取代 。 使用權杖的用戶端應用程式識別碼。 應用程式代表本身或使用者行事。 應用程式識別碼通常代表應用程式物件，但也可以代表 Azure AD 中的服務主體物件。 |
| `azpacr` | "0"、"1" 或 "2" | 只在 v2.0 權杖中存在`appidacr`,取代 。 指出如何驗證用戶端。 若為公用用戶端，此值為 "0"。 如果使用用戶端識別碼和用戶端密碼，此值為 "1"。 如果已使用用戶端憑證進行驗證，則值為 "2"。 |
...
| `hasgroups` | Boolean | 如果有的話，一律為 `true`，表示使用者在至少一個群組中。 如果完整 groups 宣告會將 URI 片段延伸超出 URL 長度限制 (目前為 6 個或更多群組)，則使用於取代隱含授與流程中 JWT 的 `groups` 宣告。 指示客戶端應使用 Microsoft 圖形 API 來`https://graph.microsoft.com/v1.0/users/{userID}/getMemberObjects`確定使用者的群組 ( 。 |
...
為了確保權杖大小不超過 HTTP 標頭大小限制,Azure AD 會限制它在組聲明中包括的物件 ID 數。 如果使用者是多個組的成員,而不是超額限制(SAML 令牌為 150,JWT 權杖為 200),則 Azure AD 不會在權杖中發出組聲明。 相反,它在權杖中包括一個超額聲明,該聲明指示應用程式查詢Microsoft圖形API以檢索使用者的組成員身份。
...
您可以使用`BulkCreateGroups.ps1`[應用創建腳本](https://github.com/Azure-Samples/active-directory-aspnetcore-webapp-openidconnect-v2/tree/master/5-WebApp-AuthZ/5-2-Groups/AppCreationScripts)資料夾中提供的內容來幫助測試超額方案。
...
若要驗證 id_token 或 access_token，您的應用程式應該驗證權杖的簽章和宣告。 要驗證訪問權杖,你的應用還應驗證頒發者、訪問群體和簽名權杖。 這些都需要對 OpenID 探索文件中的值進行驗證。 例如,與租戶無關的文件版本位於[https://login.microsoftonline.com/common/.well-known/openid-configuration](https://login.microsoftonline.com/common/.well-known/openid-configuration)。
...
我們提供庫和代碼示例,演示如何處理令牌驗證。 下列資訊專門提供給想要了解基礎程序的人。 還有幾個第三方開源庫可用於 JWT 驗證 - 幾乎每個平臺和語言都至少有一個選項。 如需有關 Azure AD 驗證程式庫和程式碼範例的詳細資訊，請參閱 [v1.0 驗證程式庫](../azuread-dev/active-directory-authentication-libraries.md)和 [v2.0 驗證程式庫](reference-v2-libraries.md)。

### <a name="validating-the-signature"></a>驗證簽章

JWT 包含三個區段 (以 `.` 字元分隔)。 第一個區段稱為**標頭**、第二個稱為**主體**，而第三個稱為**簽章**。 簽章區段可用來驗證權杖的真實性，您的應用程式才得以信任。

Azure AD 頒發的權杖使用產業標準非對稱加密演演演算法(如 RS256)進行簽名。 JWT 標頭包含用來簽署權杖的金鑰和加密方法相關資訊：
...
宣告`alg`指示用於對權杖進行簽名的演`kid`演算法, 而聲明指示用於驗證權杖的特定公開金鑰。
...
您可以使用位於以下位置的[OpenID Connect 中繼資料文件](v2-protocols-oidc.md#fetch-the-openid-connect-metadata-document)取得驗證簽章的簽章的金鑰資料:
...
* 包含 `jwks_uri`，其提供用來簽署權杖的公用金鑰組位置。 位於的`jwks_uri`JSON Web 金鑰 (JWK) 包含該特定時刻正在使用的所有公開金鑰資訊。  JWK 格式在[RFC 7517 中](https://tools.ietf.org/html/rfc7517)描述。  您的應用程式可以使用 JWT 標頭中的 `kid` 宣告選取本文件中已用來簽署特定權杖的公開金鑰。 然後,它可以使用正確的公鑰和指示的演算法進行簽名驗證。

> [!NOTE]
> v1.0 端點會傳回 `x5t` 和 `kid` 宣告，而 v2.0 端點只會以 `kid` 宣告進行回應。 往後，建議您使用 `kid` 宣告來驗證權杖。

執行簽章驗證不在本文件的範圍之內 - 有許多開源庫可用於在必要時説明您執行此操作。  但是,Microsoft 標識平台有一個權杖簽名擴展到標準 - 自定義簽名密鑰。

如果應用由於使用[聲明映射](active-directory-claims-mapping.md)功能而具有自定義簽名密鑰,則必須`appid`追加包含應用 ID 的查詢`jwks_uri`參數,以便獲取指向應用的簽名密鑰資訊,這些資訊應用於驗證。 例如:`https://login.microsoftonline.com/{tenant}/.well-known/openid-configuration?appid=6731de76-14a6-49ae-97bc-6eba6914391e`包含`jwks_uri``https://login.microsoftonline.com/{tenant}/discovery/keys?appid=6731de76-14a6-49ae-97bc-6eba6914391e`的 。

### <a name="claims-based-authorization"></a>以宣告為基礎的授權

您應用程式的商務邏輯會左右此步驟，以下列出一些常見的授權方法。

* 檢查`scp``roles`或 聲明以驗證所有當前作用域是否與 API 公開的作用域匹配,並允許用戶端執行請求的操作。
* 請確定呼叫用戶端可以使用 `appid` 宣告來呼叫您的 API。
* 使用`appidacr`驗證呼叫客戶端的身份驗證狀態 ─ 如果不允許公共用戶端呼叫 API,則不應為 0。
* 對照過去的`nonce`聲明清單進行檢查,以驗證權杖未重播。
...
* 如果已請求訪問權杖中的`roles``groups`或聲明,請驗證使用者是否位於允許執行此操作的組中。
...
您的應用程式可能會代表使用者(通常的流)或直接從應用程式接收令牌(通過用戶端認證串[流 (v1.0,](../azuread-dev/v1-oauth2-client-creds-grant-flow.md) [v2.0)。](v2-oauth2-client-creds-grant-flow.md) 這些應用程式專用的權杖表示此呼叫來自應用程式，而且沒有使用者的支援。 這些權杖的處理大部分都相同，但其間有些差異：
...
* 許多特定於人類的索賠將丟失,`name`例如`upn`。
* 和`sub``oid`聲明將相同。

## <a name="token-revocation"></a>權杖撤銷

由於不同的原因,刷新令牌可以隨時失效或吊銷。 這主要可分成兩大類：逾時和撤銷。

### <a name="token-timeouts"></a>權杖逾時

使用[權杖生存期配置](active-directory-configurable-token-lifetimes.md),可以更改刷新權杖的存留期。  這是正常的,預期一些令牌不使用(例如,使用者3個月不打開應用),因此過期。  應用會遇到登錄伺服器由於刷新權杖的年齡而拒絕刷新令牌的情況。 

* MaxInActiveTime:如果在 MaxInActiveTime 規定的時間內未使用刷新令牌,則刷新令牌將不再有效。
...
* 範例：
  * 租戶的 MaxInActiveTime 為 5 天,並且使用者休假一周,因此 Azure AD 在 7 天內未看到使用者發出新的令牌請求。 下次使用者請求新令牌時,他們將發現其刷新令牌已被吊銷,並且必須再次輸入其憑據。
  * 敏感應用程式具有一天的 MaxAgeSession 單因素。 如果使用者在星期一和周二登錄(25 小時后),則需要重新進行身份驗證。

### <a name="revocation"></a>撤銷

由於憑據更改或由於使用或管理操作,伺服器可以吊銷刷新令牌。  刷新權杖分為兩個類 - 向機密用戶端(最右側列)發出的類和頒發給公共用戶端(所有其他列)的類。   
...
| 管理員[透過 PowerShell](https://docs.microsoft.com/powershell/module/azuread/revoke-azureaduserallrefreshtoken)復原使用者的所有刷新權杖 | 已撤銷 | 已撤銷 |已撤銷 | 已撤銷 | 已撤銷 |
...
> [!NOTE]
> 「非密碼型」登入是讓使用者不需輸入密碼就能取得它的方式。 例如,將你的臉與 Windows Hello、FIDO2 密鑰或 PIN 一起使用。
>
> Windows 10 上的主刷新權杖 (PRT) 根據認證進行隔離。 例如,Windows Hello 和密碼具有彼此隔離的各自的 PRT。 當使用者使用 Hello 認證(PIN 或生物辨識)登錄然後更改密碼時,以前獲得的基於密碼的 PRT 將被吊銷。 使用密碼重新登錄會使舊 PRT 無效,並請求新的 PRT。
>
> 當用於獲取新的訪問權杖和刷新權杖時,刷新權杖不會失效或吊銷。  但是,應用應在使用舊令牌后立即棄置舊令牌,並將其替換為新令牌,因為新令牌中具有新的過期時間。 
