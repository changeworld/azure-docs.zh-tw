---
title: Azure 活動目錄條件存取的開發人員指南
description: Azure AD 條件存取和Microsoft識別平台的開發人員指南和方案。
...
Azure 活動目錄 (Azure AD) 中的條件存取功能提供了可用於保護應用和保護服務的幾種方法之一。 條件存取開發人員和企業客戶能夠以多種方式保護服務,包括:

* Multi-Factor Authentication
* 只允許已註冊 Intune 的裝置存取特定服務
* 限制使用者位置及 IP 範圍

有關條件存取的所有功能的詳細資訊,請參閱[Azure 的目錄中的條件存取](../active-directory-conditional-access-azure-portal.md)。
...
## <a name="how-does-conditional-access-impact-an-app"></a>條件訪問如何影響應用?

### <a name="app-types-impacted"></a>受影響的應用程式類型

在大多數情況下,條件訪問不會更改應用的行為或需要開發人員進行任何更改。僅當應用直接或間接或默默請求服務令牌時,應用才需要更改代碼來處理條件訪問"挑戰"。這有如執行互動式登入要求一樣簡單。
...
條件存取策略可以應用於應用,也可以應用於應用存取的 Web API。 要瞭解有關如何配置條件存取策略的更多資訊,請參閱[快速入門:對於具有 Azure 活動目錄條件存取的特定應用需要 MFA。](../conditional-access/app-based-mfa.md)

根據方案,企業客戶可以隨時應用和刪除條件訪問策略。 若要在套用新原則時讓您的應用程式繼續運作，您必須實作「挑戰」處理。 下列範例說明挑戰處理。

### <a name="conditional-access-examples"></a>條件存取範例
...
Azure AD 條件存取是[Azure AD 高級版](https://docs.microsoft.com/azure/active-directory/active-directory-whatis)中包含的功能。 擁有 [Microsoft 365 商務版授權](/office365/servicedescriptions/microsoft-365-service-descriptions/microsoft-365-business-service-description)的客戶也有條件式存取功能的存取權。
...
下列各節會討論更複雜的常見案例。 核心操作原則是條件訪問策略在請求應用條件訪問策略的服務時令牌時進行評估。
...
> [!NOTE]
> 在這種情況下,它是一個多重身份驗證錯誤描述,但與條件訪問相關的`interaction_required`範圍很廣。
...
假設我們應用了 Web 服務 A 和 B,Web 服務 B 應用了條件存取策略。 雖然初始互動式 auth 請求需要兩個服務的同意,但條件訪問策略並非在所有情況下都是必需的。 如果應用程式要求 web 服務 B 的權杖，就會叫用原則，且 web 服務 A 的後續要求也會成功，如下所示。

![存取多個服務的應用程式流程圖](./media/v2-conditional-access-dev-guide/app-accessing-multiple-services-scenario.png)

或者,如果應用最初請求 Web 服務 A 的權杖,最終使用者不會呼叫條件存取策略。 這允許應用開發人員控制最終用戶體驗,而不是強制在所有情況下調用條件訪問策略。 棘手的情況是,如果應用隨後請求 Web 服務 B 的權杖。此時,最終使用者需要遵守條件訪問策略。 當應用程式嘗試 `acquireToken` 時，可能會產生下列錯誤 (如下列圖表所示)：
...
如果應用正在使用 MSAL 庫,則始終以交互方式重試獲取令牌失敗。 發生此互動式請求時,最終使用者有機會遵守條件訪問。 這是事實,除非請求為`AcquireTokenSilentAsync``PromptBehavior.Never`或 在這種情況下,應用需要執行```AcquireToken```互動式 請求,以便最終使用者有機會遵守策略。

## <a name="scenario-single-page-app-spa-using-msaljs"></a>配置:使用 MSAL.js 的單頁應用 (SPA)

在這種情況下,我們將演練使用 MSAL.js 調用條件訪問受保護的 Web API 的單頁應用 (SPA) 的情況。 這是一個簡單的體系結構,但在圍繞條件訪問進行開發時需要考慮一些細微差別。

在`loginPopup()`MSAL.js 中,有幾個函數取得權杖`acquireTokenSilent(...)`:`acquireTokenPopup(…)``acquireTokenRedirect(…)`、 與 。

* `loginPopup()`通過互動式登錄請求獲取 ID 權杖,但不獲取任何服務(包括條件存取受保護的 Web API)的存取權杖。
...
讓我們通過條件訪問方案來演練一個示例。 使用者只需登陸網站，且沒有工作階段。 我們執行 `loginPopup()` 呼叫時，無需進行多重要素驗證即可取得 ID 權杖。 然後使用者會叫用按鈕，要求應用程式向 web API 要求資料。 應用嘗試執行調用,`acquireTokenSilent()`但失敗,因為使用者尚未執行多重身份驗證,並且需要遵守條件訪問策略。
...
* 有關更多 Azure AD 代碼範例,請參考[範例](sample-v2-code.md)。
* 關於 MSAL SDK 與造訪參考文件的詳細資訊,請參閱 Microsoft[身份驗證函式庫概述](msal-overview.md)。
* 若要深入了解多租用戶情節，請參閱[如何使用多租用戶模式將使用者登入](howto-convert-app-to-be-multi-tenant.md)。
* 瞭解有關[條件存取和保護對IoT應用的存取](/azure/architecture/example-scenario/iot-aad/iot-aad)的更多資訊。
