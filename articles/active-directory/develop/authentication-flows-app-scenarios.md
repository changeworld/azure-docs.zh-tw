...
Microsoft 身分識別平台 (v2.0) 端點支援各種新式應用程式架構的驗證。 所有架構都是以業界標準通訊協定 [OAuth 2.0 和 OpenID Connect](active-directory-v2-protocols.md) 為基礎。 藉由使用[Microsoft 身分識別平臺的驗證程式庫](reference-v2-libraries.md)，應用程式會驗證識別，並取得權杖以存取受保護的 api。
...
安全性權杖可以由多種類型的應用程式取得。 這些應用程式通常會分成下列三種類別。 每種都會搭配不同的程式庫和物件使用。

- **單頁應用程式**（也稱為 spa），這些是 web 應用程式，其中的權杖是由在瀏覽器中執行的 JavaScript 或 TypeScript 應用程式取得。 許多現代化應用程式在前端都有單一頁面應用程式，主要是以 JavaScript 撰寫。 此應用程式通常使用 Angular、React 或 Vue 之類的架構。 MSAL.js 是唯一支援單頁應用程式的 Microsoft 驗證程式庫。

- **公用用戶端應用程式**：此類別中的應用程式（例如下列類型）一律會登入使用者：
  - 代表已登入的使用者呼叫 web Api 的傳統型應用程式
  - 行動應用程式
  - 在沒有瀏覽器的裝置上執行的應用程式，就像在 IoT 上執行的一樣
...
可用的驗證流程會根據登入對象而有所不同。 有些流程僅適用於公司或學校帳戶。 其他則適用于公司或學校帳戶，以及個人 Microsoft 帳戶。
...
許多新式 web 應用程式都是以用戶端單一頁面應用程式的形式建立。 這些應用程式會使用 JavaScript 或類似角度、Vue 和回應 .js 的架構。 這些應用程式會在網頁瀏覽器中執行。

單一頁面應用程式與傳統的伺服器端 web 應用程式不同，就驗證特性而言。 使用 Microsoft 身分識別平台，單頁應用程式即可登入使用者，並取得權杖來存取後端服務或 Web API。
...
- 如果您在 .NET 中進行開發，請使用 ASP.NET 或 ASP.NET Core 搭配 ASP.NET OpenID Connect 中介軟體。 保護資源牽涉到驗證安全性權杖，這是由[適用于 .net 的 microsoft.identitymodel 擴充](https://github.com/AzureAD/azure-activedirectory-identitymodel-extensions-for-dotnet/wiki)功能，而不是 MSAL 程式庫所完成。

- 如果您以 .Node.js 進行開發，您會使用 Passport.js。

如需詳細資訊，請參閱[登入使用者的 Web 應用程式](scenario-web-app-sign-user-overview.md)。

### <a name="web-app-that-signs-in-a-user-and-calls-a-web-api-on-behalf-of-the-user"></a>登入使用者並代表使用者呼叫 Web API 的 Web 應用程式

![呼叫 Web API 的 Web 應用程式](media/scenarios/web-app.svg)

若要代表使用者從 web 應用程式呼叫 Web API，請使用授權碼流程，並在權杖快取中儲存取得的權杖。 如有需要，MSAL 會重新整理權杖，而控制器會以無訊息方式從快取中取得權杖。

如需詳細資訊，請參閱[呼叫 Web api 的 web 應用程式](scenario-web-app-call-api-overview.md)。

### <a name="desktop-app-that-calls-a-web-api-on-behalf-of-a-signed-in-user"></a>代表已登入的使用者呼叫 Web API 的傳統型應用程式

若要讓桌面應用程式呼叫登入使用者的 Web API，請使用 MSAL 的互動式權杖取得方法。 您可以使用這些互動式方法來控制登入 UI 體驗。 MSAL 會在此互動中使用網頁瀏覽器。
...
雖然我們不建議您使用它，但在公用用戶端應用程式中會提供使用者[名稱/密碼流程](scenario-desktop-acquire-token.md#username-and-password)。 在某些案例 (例如 DevOps) 中，仍然需要此流程。

使用使用者名稱/密碼流程會限制您的應用程式。 例如，應用程式無法登入需要使用多重要素驗證的使用者，或 Azure AD 中的條件式存取工具。 您的應用程式也不會受益於單一登入。 使用者名稱/密碼流程的驗證會遵循新式驗證的原則，並僅為舊版的原因提供。

在桌面應用程式中，如果您想要保留權杖快取，您可以自訂權杖快取[序列化](scenario-desktop-acquire-token.md#file-based-token-cache)。 藉由執行[雙重權杖](scenario-desktop-acquire-token.md#dual-token-cache-serialization-msal-unified-cache--adal-v3)快取序列化，您可以使用與舊版相容和向前相容的權杖快取。 這些權杖支援先前的驗證程式庫層代。 特定程式庫包括適用於 .NET 的 Azure AD 驗證程式庫 (ADAL.NET) 第 3 版和第 4 版。
...
類似于桌面應用程式，行動應用程式會呼叫 MSAL 的互動式權杖取得方法，以取得用來呼叫 Web API 的權杖。
...
您可以使用 Microsoft 身分識別平臺端點來保護 web 服務，例如應用程式的 RESTful Web API。 受保護的 Web API 是透過存取權杖來呼叫。 權杖可協助保護 API 的資料，並驗證傳入的要求。 Web API 的呼叫者會在 HTTP 要求的授權標頭尾部附加存取權杖。

如果您想要保護您的 ASP.NET 或 ASP.NET Core Web API，您需要驗證存取權杖。 在此驗證中，您會使用 ASP.NET JWT 中介軟體。 驗證是由[適用於 .NET 的 IdentityModel 擴充功能](https://github.com/AzureAD/azure-activedirectory-identitymodel-extensions-for-dotnet/wiki)程式庫 (而不是 MSAL.NET) 完成。
...
若要讓受保護的 Web API 代表使用者呼叫另一個 Web API，您的應用程式需要取得下游 Web API 的權杖。 這類呼叫有時稱為「*服務對服務*呼叫」。 呼叫其他 web Api 的 web Api 必須提供自訂快取序列化。

![呼叫另一個 Web API 的 Web API](media/scenarios/web-api.svg)

如需詳細資訊，請參閱[呼叫 Web API 的 Web API](scenario-web-api-call-api-overview.md)。

### <a name="daemon-app-that-calls-a-web-api-in-the-daemons-name"></a>在守護程式的名稱中呼叫 Web API 的 daemon 應用程式

應用程式如果具有長時間執行的程序，或其運作方式不需要使用者互動，就也需要一個存取受保護的 Web API 的方法。 這類應用程式可以使用應用程式的身分識別來驗證和取得權杖。 應用程式會使用用戶端秘密或憑證來證明其身分識別。

您可以使用 MSAL 中的[用戶端認證](scenario-daemon-acquire-token.md#acquiretokenforclient-api)取得方法，撰寫為呼叫應用程式取得權杖的這類守護程式應用程式。 這些方法需要您在 Azure AD 中新增至應用程式註冊的用戶端密碼。 然後應用程式會與所呼叫的精靈共用此秘密。 這類秘密的範例包括應用程式密碼、憑證判斷提示和用戶端判斷提示。
...
<table>
 ...
 <tbody>
  ...
  <tr>
   ...
   <td>透過 PKCE 使用<a href="v2-oauth2-auth-code-flow.md">授權碼</a>進行互動</td>
   <td>公司或學校帳戶、個人帳戶和 Azure AD B2C</td>
 </tr>
 ...
 <tr>
  ...
  <tr>
   ...
   <td>僅限應用程式的許可權，不含使用者，而且只能在 Azure AD 組織中使用</td>
 </tr>
