---
title: 註冊在用戶中簽名的 Web 應用 - 微軟身份平臺 |蔚藍
...
---
# <a name="web-app-that-signs-in-users-app-registration"></a>登入使用者的 Web 應用:應用程式註冊
...
> [!NOTE]
> 要使用的門戶因應用程式是在Microsoft Azure公共雲中運行還是在國家雲或主權雲中運行而異。 有關詳細資訊,請參閱[國家雲](./authentication-national-cloud.md#app-registration-endpoints)。


1. 使用工作或學校帳戶或個人 Microsoft 帳戶登入[Azure 門戶](https://portal.azure.com)。 或者,登錄到國家雲的首選 Azure 門戶。
...
1. 當 **「註冊應用程式**」頁出現時,輸入應用程式的註冊資訊:
   1. 為應用程式選擇受支援的帳戶類型。 ( 請參考[支援的帳號類型](./v2-supported-account-types.md)。
   1. 在 [名稱]**** 區段中，輸入將對應用程式使用者顯示、且有意義的應用程式名稱。 例如,輸入**AspNetCore-WebApp**。
   1. 對於**重定向URI**,添加應用程式類型和URI目標,這些目標將在成功身份驗證後接受返回的權杖回應。 例如,輸入**https://localhost:44321**。 接著，選取 [註冊] ****。
1. 選取 [驗證]**** 功能表，然後新增下列資訊：
   1. 對於**回覆網址,** 新增**https://localhost:44321/signin-oidc**型態**的 Web**。
...
1. 當 **「註冊應用程式」頁**出現時,輸入應用程式的註冊資訊:
   ...
   1. 在**重定向 URI(選擇性的)** 部份中,在組合框中選擇**Web**並輸入以下**https://localhost:44326/** 重定向 URI: 。
1. 選取 [註冊]**** 以建立應用程式。
1. 選擇 **「身份驗證」** 選單。
1. 在 **「進階設定** | **」隱式授予**部分中,選擇 ID**權杖**。 此示例要求啟用[隱式授予流](v2-oauth2-implicit-grant-flow.md)才能登錄使用者。
1. 選取 [儲存]  。

# <a name="java"></a>[Java](#tab/java)

1. 當 **「註冊應用程式」頁**出現時,輸入應用程式的顯示名稱。 例如,輸入**java-webapp**。
1. 選擇**任何組織目錄與個人 Microsoft 帳號中的帳號(例如 Skype、Xbox、Outlook.com),** 然後為**應用程式類型**選擇**Web 應用/ API** 。
1. 選擇 **「註冊」** 以註冊應用程式。
...
1. 當 **「註冊應用程式」頁**出現時,輸入應用程式的註冊資訊:
   ...
   1. 在**重定向 URI(選擇性的)** 部份中,在組合框中選擇**Web**並輸入以下**http://localhost:5000/getAToken**重定向 URI: 。
1. 選取 [註冊]**** 以建立應用程式。
