---
title: 如何與 Azure 活動目錄整合
description: 瞭解將應用程式與 Azure 活動目錄整合的好處,並取得簡化登入、標識管理、多重身份驗證和存取控制等功能的資源。
...
在本文中,您將瞭解將應用程式與 Azure 活動目錄 (Azure AD) 整合的好處,並獲取用於整合的資源。 Azure AD 為組織提供雲應用程式的企業級標識管理。 Azure AD 整合提供使用者簡化的登入程序，並可協助您的應用程式符合 IT 原則。

## <a name="how-to-integrate"></a>如何整合

整合應用程式與 Azure AD 的方式有幾種。 充分利用適合您應用程式使用的這些案例。

### <a name="support-azure-ad-as-a-way-to-sign-in-to-your-application"></a>支援 Azure AD 作為登入應用程式的方法
...
### <a name="browse-for-users-manage-user-provisioning-and-control-access-to-your-application"></a>瀏覽使用者、管理使用者預先配和控制軟體的存取

**瀏覽以尋找目錄中的使用者。**  使用 Microsoft 圖形 API 可幫助使用者在邀請其他人或授予訪問許可權時搜索和瀏覽其組織中的其他人,而不是要求他們鍵入電子郵件位址。 使用者可以使用常見的通訊錄樣式介面進行瀏覽，包括檢視組織階層的詳細資料。 瞭解有關[微軟圖形 API](https://docs.microsoft.com/graph/overview)的更多資訊。
...
**使用 Azure AD 進行角色型存取控制。**   系統管理員和應用程式擁有者可以將使用者和群組指派至您在 Azure AD 中註冊應用程式時所定義的角色。 角色資訊以登入權杖發送到應用程式,也可以使用Microsoft圖形API讀取。 深入了解 [使用 Azure AD 進行授權](https://cloudblogs.microsoft.com/enterprisemobility/2014/12/18/azure-active-directory-now-with-group-claims-and-application-roles/)。

### <a name="get-access-to-users-profile-calendar-email-contacts-files-and-more"></a>存取使用者的個人資料、行事曆、電子郵件、連絡人、檔等
...
### <a name="secure-device-to-service-and-service-to-service-communication"></a>安全設備到服務和服務到服務通訊
...
### <a name="integration-with-enterprise-identity-management"></a>與企業識別管理整合
...
### <a name="advanced-security-features"></a>進階安全功能

**多重身份驗證。**   Azure AD 提供原生多因素驗證。 IT 系統管理員可以要求多因素驗證才能存取應用程式，因此您無需自行撰寫這項支援的程式碼。 瞭解有關[多重身份驗證](https://azure.microsoft.com/documentation/services/multi-factor-authentication/)的更多。
...
**業界標準通訊協定。**   Microsoft 致力於支援業界標準。 Microsoft 識別平台支援產業標準 OAuth 2.0 和 OpenID Connect 1.0 協定。 瞭解有關[微軟身份平台身份驗證協定的更多資訊](active-directory-v2-protocols.md)。
...
[使用 Microsoft 識別平台登入使用者](authentication-scenarios.md)
