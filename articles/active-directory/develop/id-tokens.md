...
---
# <a name="microsoft-identity-platform-id-tokens"></a>微軟標識平臺 ID 權杖
...
ID 權杖應用於驗證使用者是否屬於他們聲稱的身份，並獲取有關它們的其他有用資訊 - 它不應用於授權以代替[訪問權杖](access-tokens.md)。 它提供的聲明可用於應用程式內的 UX，作為資料庫中的金鑰，並提供對用戶端應用程式的訪問。  為資料庫創建金鑰時，`idp`不應使用，因為它會破壞來賓方案。  鍵控應`sub`單獨完成（始終是唯一的），如果需要，`tid`用於路由。  如果需要`oid`+`sub`+`tid`跨服務共用資料，將工作，因為多個服務都得到相同的`oid`。
...
此清單顯示預設情況下處于大多數id_tokens的聲明（除非另有說明）。  但是，你的應用可以使用[可選聲明](active-directory-optional-claims.md)在id_token中請求其他聲明。  這些範圍從`groups`聲明到有關使用者名的資訊。
...
|`exp` |  整數，UNIX 時間戳記 | "exp" (到期時間) 宣告會識別到期時間，等於或晚於此時間都不得接受 JWT 以進行處理。  請務必注意，如果此時資源也可能拒絕權杖 -例如，如果需要更改身份驗證或檢測到權杖吊銷。 |
...
|`oid` | 字串，GUID | 物件在 Microsoft 身分識別系統中的不可變識別碼，在此案例為使用者帳戶。 此識別碼可跨應用程式唯一識別使用者，同一位使用者登入兩個不同的應用程式會在 `oid` 宣告中收到相同的值。 Microsoft Graph 會傳回這個識別碼做為指定使用者帳戶的 `id` 屬性。 由於`oid`允許多個應用關聯使用者，`profile`因此需要作用域才能接收此聲明。 請注意，如果單個使用者存在於多個租戶中，則使用者將在每個租戶中包含不同的物件識別碼 - 即使使用者登錄到具有相同憑據的每個帳戶，它們也被視為不同的帳戶。 索賠`oid`是 GUID，不能重複使用。 |
...
|`unique_name` | String | 提供人類看得懂的值，用以識別權杖的主體。 此值在任何給定時間點都是唯一的，但由於電子郵件和其他識別碼可以重複使用，因此此值可以重新出現在其他帳戶上，因此應僅用於顯示目的。 僅在 v1.0 `id_tokens` 中發出。 |
|`uti` | 不透明字串 | Azure 用來重新驗證權杖的內部宣告。 應該予以忽略。 |
|`ver` | 字串，1.0 或 2.0 | 表示 id_token 的版本。 |


> [!NOTE]
> 從上面的例子中可以看出，v1 和 v2 id_token在所攜帶的資訊量上存在差異。 該版本基本上指定從發佈位置的 Azure AD 平臺終結點。 [Azure AD Oauth 實現](https://docs.microsoft.com/azure/active-directory/develop/about-microsoft-identity-platform)已經過多年的發展。 目前，我們為 AzureAD 應用程式提供了兩個不同的 oAuth 終結點。 您可以使用任何分類為 v2 的新終結點或稱為 v1 的舊終結點。 兩者中的 Oauth 終結點是不同的。 V2 終結點是較新的終結點，我們嘗試遷移 v1 終結點的所有功能，並建議新開發人員使用 v2 終結點。 
> - V1：Azure 活動目錄終結點：`https://login.microsoftonline.com/common/oauth2/authorize`
> - V2：微軟身份平臺終結點：`https://login.microsoftonline.com/common/oauth2/v2.0/authorize`

## <a name="validating-an-id_token"></a>驗證 id_token

驗證 與`id_token`[驗證訪問權杖的第一](access-tokens.md#validating-tokens)步類似 - 用戶端應驗證正確的頒發者是否已發送回權杖，並且該權杖未被篡改。 因為 `id_tokens` 一律是 JWT，所以有許多程式庫可用來驗證這些權杖 - 我們建議您使用其中一個程式庫，而不需自己執行此作業。
...
* 瞭解[訪問權杖](access-tokens.md)
* 使用[可選聲明](active-directory-optional-claims.md)自訂id_token中的聲明。
