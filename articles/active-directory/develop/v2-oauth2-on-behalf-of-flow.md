...
| `assertion` | 必要 | 已傳送至中介層 API 的存取權杖。  此權杖必須具有應用程式的物件（ `aud` ）宣告，以進行此 OBO 要求（由欄位表示的應用程式 `client-id` ）。 應用程式無法兌換不同應用程式的權杖（例如，如果用戶端傳送的 API 是適用于 MS Graph 的權杖，則 API 無法使用 OBO 兌換它。  它應該會改為拒絕 token）。  |
