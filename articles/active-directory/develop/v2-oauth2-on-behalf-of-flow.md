...
> [!NOTE]
> 上述存取權杖是 v1.0 格式的權杖。 這是因為提供的權杖會以要存取的**資源**為依據。 既然 Microsoft Graph 已設為接受 v1.0 權杖，所以當用戶端要求 Microsoft Graph 權杖時，Microsoft 身分識別平台即會產生 v1.0 存取權杖。 只有應用程式應該檢查存取權杖。 用戶端**不得**檢查存取權杖。
