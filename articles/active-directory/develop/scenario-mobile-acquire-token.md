...
定義範圍最簡單的方式，就是將所需的 Web API `App ID URI` 與範圍結合 `.default` 。 此定義會告訴 Microsoft 身分識別平臺，您的應用程式需要在入口網站中設定的所有範圍。
...
MSAL 可讓應用程式以無訊息和互動方式取得權杖。 當您呼叫 `AcquireTokenSilent()` 或時 `AcquireTokenInteractive()` ，MSAL 會傳回所要求範圍的存取權杖。 正確的模式是提出無訊息要求，然後切換回互動式要求。
...
- `Consent`可讓您提示使用者同意，即使之前已授與同意亦然。 在此情況下，MSAL 會將 `prompt=consent` 傳送給識別提供者。
    ...
    您可能想要 `Consent` 在以安全性為主的應用程式中使用常數，其中組織治理會要求使用者在每次使用應用程式時看到同意對話方塊。
...
`WithExtraScopeToConsent`在您想要讓使用者提前同意數個資源的 advanced 案例中使用修飾詞。 當您不想要使用累加式同意時，可以使用這個修飾詞，這通常與 MSAL.NET 或 Microsoft 身分識別平臺2.0 搭配使用。 如需詳細資訊，請參閱[將數個資源進行預先同意](scenario-desktop-production.md#have-the-user-consent-upfront-for-several-resources)。
