...
您可以使用 [Azure 入口網站](https://portal.azure.com)來定義應用程式角色。 當使用者登入應用程式時，Azure AD `roles` 會針對使用者已個別授與使用者和其群組成員資格的每個角色發出宣告。
...
1. 在 [ **建立應用程式角色** ] 窗格中，輸入角色的設定。 影像後面的資料表會描述每個設定及其參數。
    ...
    | **描述** | 更詳細描述在系統管理員應用程式指派和同意體驗期間所顯示的應用程式角色。 | `Writers can create surveys.` |
    | **要啟用此應用程式角色嗎？** | 指定是否啟用應用程式角色。 若要刪除應用程式角色，請取消選取此核取方塊，並在嘗試刪除操作之前套用變更。 | *已檢查* |
