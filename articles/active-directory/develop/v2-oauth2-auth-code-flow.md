...
## <a name="setup-required-for-single-page-apps"></a>單頁應用程式所需的設定

單頁應用程式的授權碼流程需要一些額外的設定。  在[建立應用程式](howto-create-service-principal-portal.md)時，您必須將應用程式的重新導向 URI 標示為 `spa` 重新導向 URI。 這會導致登入伺服器允許您應用程式的 CORS (跨原始資源分享)。  這是使用 XHR 兌換程式碼的必要條件。
...
> [!NOTE]
> 單頁應用程式可能會收到 `invalid_request` 錯誤，指出只允許「單頁應用程式」用戶端類型的跨原始來源權杖兌換。  這表示用來要求權杖的重新導向 URI 尚未標示為 `spa` 重新導向 URI。  關於如何啟用此流程，請參閱[應用程式註冊步驟](#setup-required-for-single-page-apps)。
