...
如果您想要建立第一個可攜 (ASP.NET Core) web 應用程式登入使用者，請遵循此快速入門：
...
- 在應用程式註冊期間，如果您將應用程式部署至數個位置) 回復 Uri，就必須提供一或多個 (。 在某些情況下 (ASP.NET 和 ASP.NET Core) ，您必須啟用 ID token。 最後，您會想要設定登出 URI，讓您的應用程式對登出使用者做出回應。
- 在應用程式的程式碼中，您必須提供 web 應用程式委派登入的許可權。 在) 的合作夥伴案例中，您可能會想要自訂權杖驗證 (。
