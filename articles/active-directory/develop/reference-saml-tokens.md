...
> | 名稱 | 對等的 JWT 宣告 | 說明 | 範例 |
> | --- | --- | --- | ------------|
> |觀眾 | `aud` |權杖的預定接收者。 接收權杖的應用程式必須確認對象值正確無誤，並拒絕任何適用於不同對象的權杖。 | `<AudienceRestriction>`<br>`<Audience>`<br>`https://contoso.com`<br>`</Audience>`<br>`</AudienceRestriction>`  |
