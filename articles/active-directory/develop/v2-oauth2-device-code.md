...
description: 在沒有瀏覽器的情況下登錄使用者。 使用設備授權授予構建嵌入式身份驗證流和無瀏覽器身份驗證流。
...
ms.locfileid: "77159726"
---
# <a name="microsoft-identity-platform-and-the-oauth-20-device-authorization-grant-flow"></a>微軟身份平臺和 OAuth 2.0 設備授權授予流

Microsoft 標識平臺支援[設備授權授予](https://tools.ietf.org/html/rfc8628)，它允許使用者登錄到受輸入約束的設備，如智慧電視、IoT 設備或印表機。  若要啟用此流程，裝置會讓使用者在其他裝置的瀏覽器中瀏覽網頁，以執行登入程序。  使用者登入後，裝置可取得所需的存取權杖和重新整理權杖。  
...
> [!NOTE]
> Microsoft 標識平臺終結點不支援所有 Azure 活動目錄方案和功能。 要確定是否應使用 Microsoft 標識平臺終結點，請閱讀有關[Microsoft 標識平臺限制](active-directory-v2-limitations.md)。
...
用戶端必須首先與身份驗證服務器查詢用於啟動身份驗證的設備使用者代碼。 用戶端從 `/devicecode` 端點收集此要求。 在此要求中，用戶端也應一併附上必須向使用者索取的權限。 自發出要求算起，使用者只有 15 分鐘可以登入 (`expires_in` 的一般值)，因此請在使用者已準備好登入的情況下，才發出要求。
...
| 參數 | 條件 | 描述 |
| --- | --- | --- |
| `tenant` | 必要 | 可以是 /公共、/消費者或 /組織。  它還可以是要以 GUID 或易記名稱格式請求許可權的目錄租戶。  |
...
|`user_code`       | String | 向使用者顯示用於標識輔助設備上的會話的短字串。|
...
收到`user_code`和`verification_uri`後，用戶端向使用者顯示這些，指示他們使用行動電話或 PC 瀏覽器登錄。
...
設備代碼流是輪詢協定，因此用戶端必須在使用者完成身份驗證之前收到錯誤。  
...
您可以使用刷新權杖使用[OAuth 代碼流文檔中](v2-oauth2-auth-code-flow.md#refresh-the-access-token)記錄的相同流獲取新的訪問權杖和刷新權杖。  
