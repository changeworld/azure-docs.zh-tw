...
| `scope` | 建議 | 您想要使用者同意的[範圍](v2-permissions-and-consent.md)清單（以空格分隔）。  |
...
收到`user_code`和`verification_uri`之後，用戶端會向使用者顯示這些專案，指示他們使用其行動電話或電腦瀏覽器登入。
...
| `bad_verification_code`| 無法`device_code`辨識傳送至`/token`端點的。 | 確認用戶端是否在要求中傳送正確的 `device_code`。 |
