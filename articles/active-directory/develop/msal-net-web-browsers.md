...
如果您想要使用系統 web 瀏覽器，讓應用程式能與在瀏覽器中執行的應用程式啟用 SSO，但擔心 Android 裝置的使用者體驗不具有自訂索引標籤支援的瀏覽器，您可以選擇`IsSystemWebViewAvailable()`呼叫中`IPublicClientApplication`的方法來決定。 `true`如果 PackageManager 偵測到自訂索引標籤， `false`而且未在裝置上偵測到，則此方法會傳回。
