...
只要萬用字元符合外部 URL，您就可以使用萬用字元憑證。 [萬用字元應用程式](application-proxy-wildcard.md)必須使用萬用字元憑證。 如果您想要使用憑證來同時存取子域，您必須在相同的憑證中新增子域萬用字元做為主體的替代名稱。 例如， * \*adventure-works.com*的憑證將無法用於* \*. apps.adventure-works.com* ，除非您新增* \*apps.adventure-works.com*做為主體的替代名稱。 
