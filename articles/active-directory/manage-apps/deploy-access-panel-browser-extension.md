...
---
# <a name="how-to-deploy-the-access-panel-extension-for-internet-explorer-using-group-policy"></a>如何：使用群組原則部署 Internet 資源管理器的訪問面板擴展
...
## <a name="step-1-create-the-distribution-point"></a>步驟 1：創建發佈點
...
1. 轉到"**共用"** 選項卡。然後按一下 **"** > **任務"""新共用..."...**

    ![螢幕截圖顯示從"任務"螢幕中查找"新共用"的位置](./media/deploy-access-panel-browser-extension/shares.png)
...
## <a name="step-2-create-the-group-policy-object"></a>第 2 步：創建群組原則物件

1. 登錄到承載活動目錄域服務 （AD DS） 安裝的伺服器。
1. 在伺服器管理員中，轉到**工具** > **群組原則管理**。
...
1. 在 [群組原則管理] **** 視窗的左窗格中，檢視您的組織單位 (OU) 階層並決定您想要套用群組原則的範圍。 例如，您可能決定針對測試挑選小型 OU 以部署到少數使用者，或者您可能會挑選最上層 OU 以部署到整個組織。
   ...
   > [!NOTE]
   > 如果要創建或編輯組織單位 （O），請切換回伺服器管理員，然後轉到**工具** > **活動目錄使用者和電腦**。

1. 一旦您選取 OU，以滑鼠右鍵按一下它然後選取 [在這個網域中建立 GPO 並連結到...]****

    ![螢幕截圖顯示創建新的 GPO 選項](./media/deploy-access-panel-browser-extension/create-gpo.png)

1. 在 [新增 GPO] **** 提示中，輸入新的群組原則物件的名稱。
1. 以滑鼠右鍵按一下您建立的群組原則物件，然後選取 [編輯]****。

## <a name="step-3-assign-the-installation-package"></a>步驟 3：分配安裝包
...
1. 移至共用資料夾，該資料夾包含[步驟 1：建立發佈點](#step-1-create-the-distribution-point)的安裝程式套件，選取 .msi 檔案，然後按一下 [開啟]****。
   ...
   > 如果共用位於相同的伺服器上，確認您是透過網路檔案路徑存取此 .msi，，而不是本機檔案路徑。
    ...
    ![從共用資料夾中選擇安裝包](./media/deploy-access-panel-browser-extension/select-package.png)
...
## <a name="step-4-auto-enable-the-extension-for-internet-explorer"></a>第 4 步：自動啟用 Internet 資源管理器的擴展
...
1. 以滑鼠右鍵按一下 [附加元件清單]****，然後選取 [編輯]****。
    ...
    ![按右鍵"載入清單"並選擇"編輯"](./media/deploy-access-panel-browser-extension/edit-add-on-list.png)
...
1. 在 [顯示內容] **** 視窗中，執行下列步驟：
   ...
   1. 按一下 [確定]**** 關閉 [顯示內容]**** 視窗。
      ...
      ![填寫上一步中指定的值](./media/deploy-access-panel-browser-extension/show-contents.png)
...
## <a name="step-5-optional-disable-remember-password-prompt"></a>步驟 5（可選）：禁用"記住密碼"提示

當使用者登入使用存取面板擴充功能的網站時，Internet Explorer 可能會顯示下列提示詢問「是否要儲存密碼？」

![顯示"您要存儲密碼..."提示](./media/deploy-access-panel-browser-extension/remember-password-prompt.png)
...
1. 尋找名為 [開啟表單上使用者名稱和密碼的自動完成功能] **** 的設定。
   ...
   > 舊版的 Active Directory 可能會列出這項設定，但名為 [不允許以自動完成儲存密碼]****。 該設定的組態不同於本教學課程中所描述的設定。
    ...
    ![請記住在"使用者設置"下查找此內容](./media/deploy-access-panel-browser-extension/disable-auto-complete.png)

1. 以滑鼠右鍵按一下上述設定，然後選取 [編輯]****。
1. 在標題為 [開啟表單上使用者名稱和密碼的自動完成功能]**** 的視窗中選取 [停用]****。

    ![為自動完成功能的"已禁用"選項選擇"已禁用"選項](./media/deploy-access-panel-browser-extension/disable-passwords.png)
...
## <a name="step-6-testing-the-deployment"></a>第 6 步：測試部署
...
1. 群組原則更改可能需要幾個登錄才能使用此電腦完全更新。 若要強制更新，開啟 [命令提示字元]**** 視窗，然後執行下列命令：`gpupdate /force`
...
1. 在 [管理附加元件]**** 視窗中，確認 [存取面板擴充功能]**** 已安裝且其 [狀態]**** 已設為 [已啟用]****。
   ...
   ![驗證"訪問面板擴展"是否安裝並啟用](./media/deploy-access-panel-browser-extension/verify-install.png)
