...
請注意，如果您已針對 SP 起始的 SAML 型單一登入設定應用程式，並將 SSO 模式變更為停用，使用者還是可以在 MyApps 入口網站以外的應用程式中進行簽署。 若要達到此目的，您必 [停用使用者登入的能力](disable-user-sign-in-portal.md)
