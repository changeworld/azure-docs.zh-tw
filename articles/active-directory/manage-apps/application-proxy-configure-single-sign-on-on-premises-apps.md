...
下列通訊協定圖表描述服務提供者起始 (SP 起始的) 流程和識別提供者起始的 (IdP 起始) 流程的單一登入順序。 應用程式 Proxy 會透過在內部部署應用程式中快取 SAML 要求和回應，來與 SAML SSO 搭配運作。
  ...
  ![下圖顯示應用程式、應用程式 Proxy、用戶端和 Azure A D 的互動。](./media/application-proxy-configure-single-sign-on-on-premises-apps/saml-sp-initiated-flow.png)

  ![圖表顯示應用程式、應用程式 Proxy、用戶端和 Azure A D 的互動，以 D 表示 I d P 起始的單一登入。](./media/application-proxy-configure-single-sign-on-on-premises-apps/saml-idp-initiated-flow.png)
