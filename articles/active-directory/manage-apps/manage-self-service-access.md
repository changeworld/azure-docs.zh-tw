...
> [!NOTE]
> 使用者需要 Azure 活動目錄高級版 (P1 或 P2) 許可證才能請求加入自助服務應用,並且擁有者需要批准或拒絕請求。 如果沒有 Azure 活動目錄高級許可證,使用者無法添加自助服務應用。
...
1. 以全域管理員登入[Azure 門戶](https://portal.azure.com)。

2. 選取 **Azure Active Directory**。 在左邊導航選單中,選擇**企業應用程式**。

3. 從清單中選擇應用程式。 如果看不到應用程式,請開始在搜尋框中鍵入其名稱。 或者使用篩選器控制件選擇應用程式類型、狀態或可見性,然後選擇 **"應用**"。

4. 在左邊導航選單中,選擇**自助服務**。

5. 若要啟用此應用程式的自助式應用程式存取，請將 [要允許使用者要求此應用程式的存取權嗎?]**** 切換開關切換為 [是]****。

6. 旁邊**應向哪個組分配使用者?** **Select group** 選擇組,然後單擊 **「選擇**」。。 當使用者的請求獲得批准時,他們將被添加到此組。 查看此組的成員身份時,您將能夠看到誰已被授予通過自助服務訪問訪問應用程式。
  
    > [!NOTE]
    > 此設定不支援從本地同步的組。

7. **選擇:** 要在允許使用者訪問之前需要業務批准,請先在**授予對此應用程式訪問許可權之前設置"需要審批"?** 切換為 **"是**"。

8. **選擇密碼單一符號的應用程式,** 允許業務審批者為已批准的使用者指定發送到此應用程式的密碼,請設定 **「允許審批者」以為此應用程式設定使用者的密碼?** **Yes**

9. **選擇:** 要指定允許批准存取此應用程式的業務審批人,請在**允許誰批准訪問此應用程式旁邊,** 按兩下 **「選擇審批者**」,然後選擇最多 10 個單獨的業務審批者。 然後按一下 [選取] ****。

    >[!NOTE]
    >不支援群組。 您最多可以選擇 10 個單獨的業務審批人。 如果指定多個審批人,則任何單個審批人都可以批准訪問請求。

10. **可選:****對於公開角色的應用程式**,要將自助服務批准的使用者分配給角色,**在此應用程式中應分配使用者到哪個角色旁邊?,** 按兩下 **「選擇角色**」,然後選擇這些使用者應為其分配的角色。 然後按一下 [選取] ****。

11. 按一下窗格頂端的 [儲存]**** 按鈕以完成此動作。

完成自助服務應用程式配置後,用戶可以導航到其[「我的應用」訪問面板](https://myapps.microsoft.com/),然後單擊「**添加自助服務應用」** 按鈕,以查找透過自助服務訪問啟用的應用。 業務審批者還會在其[「我的應用」訪問面板](https://myapps.microsoft.com/)中看到通知。 您可以啟用電子郵件，通知他們有使用者已要求存取應用程式，需要他們核准。
