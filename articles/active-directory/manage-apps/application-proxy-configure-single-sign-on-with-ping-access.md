...
1. 使用新應用程式的相關資訊填寫必要的欄位。 使用下方的指導方針進行設定。
   ...
   1. **內部 URL**：當您在公司網路上時，通常會提供將您帶到應用程式登入頁面的 url。 在此案例中，連接器需要將 PingAccess proxy 視為應用程式的首頁。 使用此格式︰`https://<host name of your PingAccess server>:<port>`。 連接埠預設為 3000，但您可以在 PingAccess 中設定它。
      ...
      > [!WARNING]
      > 針對此類型的單一登入，內部 URL 必須使用 `https` 且無法使用 `http` 。
...
1. 選取 [新增]。 PingAccess 索引鍵會顯示在用戶端密碼的表格中，並在 [ **值** ] 欄位中會自動填入隨機字串。
...
選擇性宣告可讓您新增每個使用者和租使用者所擁有的標準預設但不包含預設的宣告。 您可以藉由修改應用程式資訊清單來設定應用程式的選擇性宣告。 如需詳細資訊，請參閱[瞭解 Azure AD 應用程式資訊清單文章](https://docs.microsoft.com/azure/active-directory/develop/reference-app-manifest/)。
...
[宣告對應原則 (預覽) ](https://docs.microsoft.com/azure/active-directory/develop/active-directory-claims-mapping#claims-mapping-policy-properties) AzureAD 中不存在的屬性。 宣告對應可讓您藉由新增 ADFS 或使用者物件所支援的額外自訂宣告，將舊的內部內部部署應用程式遷移至雲端
...
- [針對 Azure AD 設定 PingAccess，以保護使用 Microsoft Azure AD 應用程式 Proxy 發佈的應用程式](https://support.pingidentity.com/s/document-item?bundleId=pingaccess-52&topicId=agents/azure/pa_c_PAAzureSolutionOverview.html)
