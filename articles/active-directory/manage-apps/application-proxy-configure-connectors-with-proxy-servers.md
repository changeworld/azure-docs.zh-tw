...
要關閉連接器的出站代理使用,請編輯 C:_程式檔案\Microsoft AAD 應用程式代理連接器\應用程式代理連接器.exe.config 檔案,並添加此代碼範例中顯示的*system.net*部分:
...
為此,請編輯 C:_程式檔案\微軟 AAD 應用程式代理連接器\應用程式代理連接器.exe.config 檔案,並添加此代碼範例中顯示的*system.net*部分。 更改*代理伺服器:8080*以反映本地代理伺服器名稱或 IP 位址,以及它正在偵聽的埠。 即使您使用的是 IP 位址，該值也必須具有前置詞 http://。
...
| mscrl.microsoft.com:80<br>crl.microsoft.com:80<br>ocsp.msocsp.com:80<br>www.microsoft.com:80 | 連線器使用這些網址驗證憑證 |
| login.windows.net<br>secure.aadcdn.microsoftonline-p.com<br>*.microsoftonline.com<br>*.microsoftonline-p.com<br>*.msauth.net<br>*.msauthimages.net<br>*.msecnd.net<br>*.msftauth.net<br>*.msftauthimages.net<br>*.phonefactor.net<br>enterpriseregistration.windows.net<br>management.azure.com<br>policykeyservice.dc.ad.msft.net<br>ctdl.windowsupdate.com:80 | 連接器會在註冊程序進行期間使用這些 URL。 |

如果您的防火牆或代理允許您配置 DNS 允許清單,則可以允許\*連接到\*.msappproxy.net 和 .servicebus.windows.net。 若非如此，您需要允許存取 [Azure 資料中心的 IP 範圍](https://www.microsoft.com/download/details.aspx?id=41653)。 IP 範圍會每週更新。
...
不要對連接器流量使用 TLS 檢查,因為它會導致連接器流量出現問題。 連接器使用證書對應用程式代理服務進行身份驗證,並且該證書可能會在TLS檢查期間丟失。

## <a name="configure-using-a-proxy-between-the-connector-and-backend-application"></a>使用連接器與後端應用程式之間的代理進行設定
在某些環境中,對後端應用程式的通信使用轉發代理可能是一項特殊要求。
要啟用此功能,請按照以下步驟操作:

### <a name="step-1-add-the-required-registry-value-to-the-server"></a>步驟 1: 向伺服器新增所需的註冊表值
1. 要啟用使用預設代理,請在「HKEY_LOCAL_MACHINE_軟體_微軟_微軟`UseDefaultProxyForBackendRequests = 1`AAD 應用程式代理連接器」中向連接器設定註冊表項添加以下註冊表值 (DWORD)。

### <a name="step-2-configure-the-proxy-server-manually-using-netsh-command"></a>第二步:使用 netsh 指令手動設定代理伺服器
1.  啟用組策略,使每台計算機設置代理。 這見於:計算機配置\策略\管理範本\Windows元件_Internet 資源管理器。 這需要設置,而不是將此策略設置為每個使用者。
2.  在`gpupdate /force`伺服器上運行或重新啟動伺服器,以確保它使用更新的組策略設置。
3.  啟動具有管理權限的提升指令提示符`control inetcpl.cpl`並輸入 。
4.  配置所需的代理設置。 

這些設置使連接器使用相同的轉發代理來與 Azure 和後端應用程式通信。 如果 Azure 通訊的連接器不需要轉發代理或其他轉發代理,則可以通過修改檔案應用程式代理連接器服務.exe.config 來設置,如繞過出站代理或使用出站代理伺服器部分所述。

連接器更新程式服務也將使用電腦代理。 可以通過修改檔案應用程式代理連接器更新服務.exe.config 來更改此行為。
...
1. 停止網路擷取。
   ...
   ![螢幕截圖顯示「停止網路擷取」按鈕](./media/application-proxy-configure-connectors-with-proxy-servers/stop-trace.png)
