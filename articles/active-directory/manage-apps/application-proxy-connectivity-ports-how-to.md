...
-   確定所有適用於您連接器的 Proxy 或防火牆也已套用至這個頁面。 這可以在 internet Explorer 中前往 [**設定** - &gt; ] [**網際網路選項** - &gt; **Connections**  - &gt; ] [連線] [ **LAN 設定**] 來完成。 在此頁面上，您會看到 [為您的 LAN 使用 Proxy 伺服器] 欄位。 選取此方塊，然後將 Proxy 位址放入 [位址] 欄位。
