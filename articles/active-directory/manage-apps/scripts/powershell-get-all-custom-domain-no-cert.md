---
title: PowerShell 示例 - 沒有證書的應用程式代理應用
description: PowerShell 示例列出了使用自訂域但未上載有效 TLS/SSL 憑證的所有 Azure 活動目錄 （Azure AD） 應用程式代理應用程式。
...
---
# <a name="get-all-azure-ad-proxy-application-apps-published-with-no-certificate-uploaded"></a>獲取發佈的所有 Azure AD 代理應用程式應用程式，無需上載證書

此 PowerShell 腳本示例列出了使用自訂域但未上載有效 TLS/SSL 憑證的所有 Azure 活動目錄 （Azure AD） 應用程式代理應用。
