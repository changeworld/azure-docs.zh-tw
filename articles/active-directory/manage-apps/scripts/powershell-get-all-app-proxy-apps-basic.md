...
description: PowerShell 範例，列出 Azure Active Directory (Azure AD) 應用程式 Proxy 應用程式以及應用程式識別碼 (AppId)、名稱 (DisplayName) 及物件識別碼 (ObjId)。
...
ms.locfileid: "96859346"
---
# <a name="get-all-application-proxy-apps-and-list-basic-information"></a>取得所有應用程式 Proxy 應用程式及列出基本資訊

此 PowerShell 指令碼範例列出所有 Azure Active Directory (Azure AD) 應用程式 Proxy 應用程式的相關資訊，包括應用程式識別碼 (AppId)、名稱 (DisplayName) 及物件識別碼 (ObjId)。
