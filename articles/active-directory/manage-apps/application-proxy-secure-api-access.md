...
ms.locfileid: "85206439"
---
# <a name="secure-access-to-on-premises-apis-with-azure-ad-application-proxy"></a>使用 Azure AD 應用程式 Proxy 來保護內部部署 Api 的存取

您可能有在內部部署執行的商務邏輯 Api，或裝載在雲端的虛擬機器上。 您的原生 Android、iOS、Mac 或 Windows 應用程式必須與 API 端點互動，才能使用資料或提供使用者互動。 Azure AD 應用程式 Proxy 和[Microsoft 驗證程式庫（MSAL）](/azure/active-directory/develop/active-directory-authentication-libraries)可讓您的原生應用程式安全地存取您的內部部署 api。 Azure Active Directory 應用程式 Proxy 是更快速且更安全的解決方案，而不是在應用層開啟防火牆埠和控制驗證和授權。
...
Azure AD 應用程式 Proxy 會形成解決方案的骨幹、做為 API 存取的公用端點，以及提供驗證和授權。 您可以使用[Microsoft 驗證程式庫（MSAL）](/azure/active-directory/develop/active-directory-authentication-libraries)程式庫，從大量的平臺存取您的 api。
...
1. 在 [**註冊應用程式**] 頁面上：
   ...
   1. 在 [重新**導向 URL**] 底下，下拉並選取 **[公用用戶端（行動 & 桌面）**]，然後輸入 *https://login.microsoftonline.com/common/oauth2/nativeclient* 。
...
最後一個步驟是設定原生應用程式。 Nativeclient-android 範例應用程式中*Form1.cs*檔案的下列程式碼片段會使 MSAL 程式庫取得要求 API 呼叫的權杖，並將其附加為應用程式標頭的持有人。
...
若要將原生應用程式設定為連接到 Azure Active Directory 並呼叫 API 應用程式 Proxy，請使用來自 Azure AD 的值，更新 Nativeclient-android 範例應用程式之*App.config*檔案中的預留位置值：

- 在欄位中貼上**目錄（租使用者）識別碼** `<add key="ida:Tenant" value="" />` 。 您可以從任何一個應用程式的 [**總覽**] 頁面，尋找並複製此值（GUID）。

- 在欄位中貼上 AppProxyNativeAppSample**應用程式（用戶端）識別碼** `<add key="ida:ClientId" value="" />` 。 您可以從 [AppProxyNativeAppSample] [**總覽**] 頁面尋找並複製此值（GUID）。

- 在欄位中貼上 AppProxyNativeAppSample 重新**導向 URI** `<add key="ida:RedirectUri" value="" />` 。 您可以從 [AppProxyNativeAppSample **Authentication** ] 頁面尋找並複製此值（URI）。

- 在欄位中貼上 SecretAPI**應用程式識別碼 URI** `<add key="todo:TodoListResourceId" value="" />` 。 您可以從 [SecretAPI] [**公開 API** ] 頁面中找到並複製此值（URI）。

- 在欄位中貼上 SecretAPI**首頁 URL** `<add key="todo:TodoListBaseAddress" value="" />` 。 您可以從 [SecretAPI**商標**] 頁面尋找並複製此值（URL）。
