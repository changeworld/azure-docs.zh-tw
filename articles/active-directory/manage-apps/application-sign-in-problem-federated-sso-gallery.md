---
title: 登入同盟單一登入資源庫應用程式的問題 |Microsoft Docs
...
請確認 `AssertionConsumerServiceURL` SAML 要求中的值符合 Azure AD 中設定的 [回復 URL] 值。 如果您使用 Azure 入口網站中的 [測試體驗](../azuread-dev/howto-v1-debug-saml-sso-issues.md) 搭配我的應用程式安全瀏覽器延伸模組，則不需要手動執行這些步驟。
...
1.  開啟 [**Azure 入口網站**](https://portal.azure.com/) ，然後以 **全域系統管理員**身分登入。
...
1.  從應用程式清單中，選取您要指派給使用者的應用程式。

1.  應用程式載入之後，請從應用程式的左側導覽功能表中選取 [ **使用者和群組** ]。
...
1. 將滑鼠停留在清單中的**使用者**上方，以顯示**核取方塊**。 按一下使用者設定檔相片或標誌旁的核取方塊，將使用者新增至 **選取** 的清單。

1. **選用：** 如果您想要 **新增多個使用者**，請在 [ **依名稱或電子郵件地址** 搜尋] 方塊中輸入另一個完整名稱或電子郵件地址，然後按一下核取方塊，將使用者新增至 **選取** 的清單。

1. 當您完成選取使用者時，請按一下 [ **選取** ] 按鈕，將他們新增到要指派給應用程式的使用者和群組清單。

1. **選用：** 按一下 [**新增指派**] 窗格中的 [**選取角色**選取器]，以選取要指派給您已選取之使用者的角色。

1. 按一下 [指派] 按鈕，將應用程式指派給選取的使用者。

經過一小段時間之後，您選取的使用者將能夠使用解決方案描述一節中所述的方法來啟動這些應用程式。
