...
1.  請確定內部 URL 是包含該應用程式所有相關連結的根目錄。 這可讓所有的連結解析為在相同應用程式內發佈的內容。
    ...
    如果您變更內部 URL，但不想要變更使用者的登陸頁面，請將首頁 URL 變更為先前發佈的內部 URL。 若要這麼做，請前往 [Azure Active Directory]-&gt; [應用程式&gt;註冊]-選取&gt;應用程式品牌。 在 [商標] 區段中，您會看到 [首頁 URL] 欄位，您可以將其調整為所需的登陸頁面。 如果您仍在使用舊版應用程式註冊體驗，[屬性] 索引標籤會顯示 [首頁 URL] 詳細資料。 
