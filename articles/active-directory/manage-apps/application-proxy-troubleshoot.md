...
在排除應用程式代理問題時，我們建議您從檢查故障排除流、[調試應用程式代理連接器問題](application-proxy-debug-connectors.md)開始，以確定應用程式代理連接器是否配置正確。 如果連接到應用程式時仍有問題，請按照[調試應用程式代理應用程式中](application-proxy-debug-apps.md)的故障排除流操作。

如果在存取已發佈的應用程式或發佈應用程式時發生錯誤，請檢查下列選項以查看 Microsoft Azure AD 應用程式 Proxy 是否運作正常︰

* 打開 Windows 服務主控台。 驗證**Microsoft AAD 應用程式代理連接器**服務是否啟用並運行。 您也可以查看應用程式 Proxy 服務屬性頁面，如下圖所示：  
  ![[Microsoft AAD 應用程式 Proxy 連接器屬性] 視窗螢幕擷取畫面](./media/application-proxy-troubleshoot/connectorproperties.png)
* 打開事件檢視器，並在**應用程式和服務日誌** > **Microsoft** > **AadApplicationProxy** > **連接器** > **管理員**中查找應用程式代理連接器事件。
...
| 錯誤 | 建議的步驟 |
| ----- | ----------------- |
| 連接器註冊失敗︰確定您已在 Azure 管理入口網站中啟用應用程式 Proxy，並已正確地輸入您的 Active Directory 使用者名稱和密碼。 錯誤︰「發生一或多個錯誤。」 | 如果您在未登入 Azure AD 的情況下關閉註冊視窗，請再次執行連接器精靈，並註冊連接器。 <br><br> 如果註冊視窗打開，然後立即關閉，而不允許您登錄，您可能會收到此錯誤。 您的系統有網路錯誤時，就會發生此錯誤。 確保可以從瀏覽器連接到公共網站，並且埠在[應用程式代理先決條件](application-proxy-add-on-premises-application.md#prepare-your-on-premises-environment)中指定處於打開狀態。 |
| 清除錯誤會顯示在註冊視窗中。 無法繼續 | 如果您看到這個錯誤且視窗隨後關閉，表示您輸入錯誤的使用者名稱或密碼。 請再試一次。 |
| 連接器註冊失敗︰確定您已在 Azure 管理入口網站中啟用應用程式 Proxy，並已正確地輸入您的 Active Directory 使用者名稱和密碼。 錯誤︰「AADSTS50059: 在要求中找不到租用戶識別資訊，或任何提供的認證均未隱含租用戶識別資訊，而且依服務主體 URI 的搜尋已失敗。 | 您嘗試使用 Microsoft 帳戶登錄，而不是使用您嘗試訪問的目錄的組織 ID 的域登錄。 確定系統管理員屬於與租用戶網域相同的網域名稱，例如，若 Azure AD 網域是 contoso.com，則系統管理員應該是 admin@contoso.com。 |
| 無法擷取目前的執行原則以供執行 PowerShell 指令碼。 | 如果連接器安裝失敗，請檢查以確保未禁用 PowerShell 執行策略。 <br><br>1. 打開群組原則編輯器。<br>2. 轉到**電腦配置** > **管理範本** > **Windows 元件** > **Windows PowerShell**和按兩下**打開腳本執行**。<br>3. 執行策略可以設置為 **"未配置**"或 **"已啟用**"。 如果是設定為 [已啟用]****，請確定 [選項] 底下的 [執行原則] 是設定為 [允許本機指令碼和遠端已簽署的指令碼]**** 或 [允許所有指令碼]****。 |
...
| 連接器註冊失敗：請確保您是活動目錄的應用程式管理員來註冊連接器。 錯誤︰「註冊要求被拒絕。」 | 您嘗試用以登入的別名不是此網域的系統管理員。 您的連接器永遠都會針對擁有使用者網域的目錄進行安裝。 確保嘗試登錄的管理員帳戶至少具有 Azure AD 租戶的應用程式管理員許可權。 |
| 由於網路問題，連接器無法連接到服務。 連接器嘗試訪問以下 URL。 | 連接器無法連接到應用程式代理雲服務。 如果防火牆規則阻止連接，則可能發生此情況。 請確保您已允許訪問[應用程式代理先決條件](application-proxy-add-on-premises-application.md#prepare-your-on-premises-environment)中列出的正確埠和 URLS。 |
...
| 錯誤 | 建議的步驟 |
| ----- | ----------------- |
| 無法擷取目前的執行原則以供執行 PowerShell 指令碼。 | 如果連接器安裝失敗，請檢查以確定未停用 PowerShell 執行原則。<br><br>1. 打開群組原則編輯器。<br>2. 轉到**電腦配置** > **管理範本** > **Windows 元件** > **Windows PowerShell**和按兩下**打開腳本執行**。<br>3. 執行策略可以設置為 **"未配置**"或 **"已啟用**"。 如果是設定為 [已啟用]****，請確定 [選項] 底下的 [執行原則] 是設定為 [允許本機指令碼和遠端已簽署的指令碼]**** 或 [允許所有指令碼]****。 |
...
| 網站無法顯示頁面。 | 如果應用程式是 OWA 應用程式，則使用者在嘗試存取此應用程式時可能會發生此錯誤。 這可能是由下列下列其中一項所造成︰<br><li>為此應用程式定義的 SPN 不正確。 確定為此應用程式設定的 SPN 正確無誤。</li><li>嘗試存取應用程式的使用者使用 Microsoft 帳戶，而不是使用適當的公司帳戶進行登入，或使用者是來賓使用者。 確定使用者是使用符合已發佈應用程式之網域的公司帳戶進行登入。 Microsoft 帳戶使用者和來賓無法存取 IWA 應用程式。</li><li>嘗試訪問應用程式的使用者未在本地端為此應用程式正確定義。 請確保此使用者具有為本地電腦上的此後端應用程式定義的適當許可權。 |
| 無法存取此公司應用程式。 您未獲得授權存取此應用程式。 授權失敗。 務必將此應用程式的存取權指派給使用者。 | 如果使用者使用 Microsoft 帳戶而不是其公司帳戶登錄，則使用者在嘗試訪問您發佈的應用時可能會收到此錯誤。 來賓使用者也可能會發生此錯誤。 Microsoft 帳戶使用者和來賓無法存取 IWA 應用程式。 確定使用者是使用符合已發佈應用程式之網域的公司帳戶進行登入。<br><br>您可能尚未對此應用程式指派使用者。 移至 [應用程式]**** 索引標籤，在 [使用者和群組]**** 底下，將此使用者或使用者群組指派給此應用程式。 |
| 無法立即存取此公司應用程式。 請稍後再試...連接器已逾時。 | 如果使用者未在本地端正確定義此應用程式，則使用者在嘗試訪問您發佈的應用時可能會收到此錯誤。 確保使用者具有為本地電腦上的此後端應用程式定義的適當許可權。 |
| 無法存取此公司應用程式。 您未獲得授權存取此應用程式。 授權失敗。 確保使用者具有 Azure 活動目錄高級版許可證。 | 如果使用者的管理員未使用高級許可證顯式分配應用，則使用者在嘗試訪問您發佈的應用時可能會收到此錯誤。 轉到訂閱者的活動目錄**許可證**選項卡，並確保為此使用者或使用者組分配了高級許可證。 |
| 找不到具有指定主機名稱的伺服器。 | 如果應用程式的自訂域配置不正確，則使用者在嘗試訪問您發佈的應用時可能會收到此錯誤。 按照[Azure AD 應用程式代理中處理自訂域](application-proxy-configure-custom-domain.md)的步驟，確保已上載域的證書並正確配置 DNS 記錄 |
