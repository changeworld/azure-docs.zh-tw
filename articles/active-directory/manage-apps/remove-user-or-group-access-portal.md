---
title: 從 Azure AD 中的應用中刪除使用者或組分配
...
很容易從 Azure 活動目錄 （Azure AD） 中的企業應用程式的分配存取權限中刪除使用者或組。 您需要適當的許可權來管理企業應用。 而且，您必須是目錄的全域管理員。
...
1. 在**Azure 活動目錄 -*目錄名稱*** 頁上（即要管理的目錄的 Azure AD 頁），選擇**企業應用程式**。
1. 在 **"企業版應用程式 - 所有應用程式**"頁上，您將看到您可以管理的應用程式的清單。 選取應用程式。
1. 在***應用名稱***概述頁上（即標題中包含所選應用名稱的頁面）中，選擇 **"使用者&組**"。
