...
1.  打開[Azure 門戶](https://portal.azure.com)並作為**全域管理員**或**共同管理員**登錄
...
8. 在 [使用者屬性]**** 區段下，從 [使用者識別碼]**** 下拉式清單選取使用者的唯一識別碼。 所選的選項必須符合應用程式中預期的值，才能驗證使用者。
   ...
   >[!NOTE] 
   >Azure AD 會根據應用程式在 SAML AuthRequest 中選取的值或要求的格式，來選取 NameID 屬性 (使用者識別碼) 的格式。 有關詳細資訊，請訪問"名稱 ID 策略"一節中的["單一登入 SAML 協定](https://docs.microsoft.com/azure/active-directory/develop/active-directory-single-sign-on-protocol-reference#authnrequest)"。
   >
   >
...
9. [將使用者分配給應用程式](#how-to-assign-a-user-to-an-application-directly)。
...
7.  在 [名稱]**** 文字方塊中輸入應用程式的名稱。 選擇 **"添加"。**

稍候片刻，您便能看見應用程式的設定窗格。

#### <a name="configure-the-application-for-password-single-sign-on"></a><a name="configure-the-application-for-password-single-sign-on-1"></a>配置應用程式以進行密碼單一登入
...
5.  按一下 [所有應用程式]****，以檢視所有應用程式的清單。
    ...
    1.  如果您沒有看到要在此處顯示的應用程式，請使用 **"所有應用程式清單**"頂部的 **"篩選器"** 控制項，並將 **"顯示**"選項設置為 **"所有應用程式"。**
...
10. [將使用者分配給應用程式](#how-to-assign-a-user-to-an-application-directly)。
...
-   [如何將使用者直接指派至應用程式](#how-to-assign-a-user-to-an-application-directly)
...
[將新使用者添加到 Azure 活動目錄](./../fundamentals/add-users-azure-active-directory.md)
