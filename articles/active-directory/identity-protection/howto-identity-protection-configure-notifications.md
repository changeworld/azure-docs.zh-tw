...
為了避免電子郵件超載，您只會在5秒的期間內收到一位具有風險的使用者偵測電子郵件。 這表示，如果多個使用者在相同的5秒期間內移至指定的風險層級，我們將會匯總並傳送一封電子郵件，以代表所有這些使用者的風險層級變更。
