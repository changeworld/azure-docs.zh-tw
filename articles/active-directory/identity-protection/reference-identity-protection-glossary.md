...
Azure Active Directory 的安全性模組，可針對風險偵測以及影響組織身分識別的潛在弱點，提供合併的觀點。

### <a name="conditional-access"></a>條件式存取
用來保護資源存取的原則。 條件式存取規則會儲存在 Azure Active Directory 中，並在授與資源的存取權之前，由 Azure AD 進行評估。  範例規則包括根據使用者位置、裝置健康狀態或使用者驗證方法來限制存取。
...
### <a name="false-positive-risk-detection"></a>假-正 (風險偵測) 
Identity Protection 使用者手動設定的風險偵測狀態，表示已調查風險偵測，並將錯誤標示為風險偵測。
...
當偵測到相同使用者的兩個登入（其中至少有一個來自非典型登入位置），以及登入之間的時間比實際在這些位置之間移動所需的最短時間短時，就會觸發風險偵測。  

### <a name="investigation"></a>調查
檢查活動、記錄和其他與風險偵測相關的資訊的程式，以決定是否需要補救或緩和步驟，瞭解身分識別是否及如何遭到入侵，以及瞭解如何使用遭盜用的身分識別。

### <a name="leaked-credentials"></a>認證外洩
偵測到目前的使用者認證 (使用者名稱和密碼) ，在我們的研究人員于深色網站公開張貼時，會觸發風險偵測。

### <a name="mitigation"></a>降低
此動作可限制或消除攻擊者利用遭到入侵的身分識別或裝置的能力，但不需將身分識別或裝置還原至安全的狀態。 風險降低不會解決先前與身分識別或裝置相關聯的風險偵測。
...
安全性原則的一部分，說明會觸發原則的情況，以及觸發原則時所採取的動作。

### <a name="prevention"></a>預防
預防藉由不當使用疑似或已知遭到入侵的身分識別或裝置來傷害組織的動作。 預防動作不會保護裝置或身分識別，也不會解決先前的風險偵測。

### <a name="privileged-user"></a>特殊權限 (使用者)
在風險偵測時，具有 Azure Active Directory 中一或多個資源之永久或暫時性系統管理員許可權的使用者，例如全域管理員、計費管理員、服務管理員、使用者系統管理員和密碼管理員。 
...
由 Identity Protection 使用者手動設定的風險偵測狀態，表示使用者在 Identity Protection 之外採取適當的補救動作，以及應將風險偵測視為已關閉。

### <a name="risk-detection-status"></a>風險偵測狀態
風險偵測的屬性，指出事件是否為作用中，如果關閉，則是關閉它的原因。

### <a name="risk-detection-type"></a>風險偵測類型
風險偵測的類別，表示導致事件被視為有風險的異常類型。

### <a name="risk-level-risk-detection"></a>風險層級 (風險偵測) 
指出風險偵測的嚴重性 (高、中或低) ，可協助 Identity Protection 使用者排定他們採取的動作，以降低組織的風險。 
...
從已識別為匿名 proxy IP 位址的 IP 位址成功登入之後所觸發的風險偵測。

### <a name="sign-in-from-infected-device"></a>從受感染的裝置登入
當登入來自 IP 位址（已知由一或多個遭入侵的裝置使用，但主動嘗試與 bot 伺服器通訊）時，就會觸發風險偵測。

### <a name="sign-in-from-ip-address-with-suspicious-activity"></a>從具有可疑活動的 IP 位址登入
從 IP 位址成功登入，並在一小段時間內跨多個使用者帳戶成功登入，就會觸發風險偵測。

### <a name="sign-in-from-unfamiliar-location"></a>從不熟悉的位置登入
當使用者從新位置成功登入 (IP、緯度/經度和 ASN) 時，會觸發風險偵測。
...
條件式存取原則，可考慮登入，並根據預先定義的條件和規則來套用緩和措施。

### <a name="users-flagged-for-risk"></a>標示有風險的使用者
有風險偵測的使用者，也就是作用中或已補救
