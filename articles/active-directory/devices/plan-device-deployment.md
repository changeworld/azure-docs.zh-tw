...
* 將[裝置加入](concept-azure-ad-join.md)至 Azure AD (僅限雲端) 或
...
* 提高生產力–透過 Azure AD，您的使用者可以對內部部署和雲端資源執行順暢的登[入 (SSO) ](./azuread-join-sso.md) ，讓他們能夠隨時隨地保持生產力。
...
* 簡化部署和管理–裝置身分識別管理可簡化將裝置帶入[Windows Autopilot](https://docs.microsoft.com/windows/deployment/windows-autopilot/windows-10-autopilot)、[大量](https://docs.microsoft.com/mem/intune/enrollment/windows-bulk-enroll)布建和自助式服務 Azure AD 的程式[： (OOBE) 的全新體驗](../user-help/user-help-join-device-on-network.md)。 您可以使用行動裝置管理來管理這些裝置， (MDM) 工具，例如[Microsoft Intune](https://docs.microsoft.com/mem/intune/fundamentals/what-is-intune)，以及其在[Azure 入口網站](https://portal.azure.com/)中的身分識別。
...
您的組織可以在單一 Azure AD 租使用者中使用多個裝置整合方法。 目標是選擇)  (的方法，以便在 Azure AD 中安全地管理您的裝置。 有許多參數會驅動此決策，包括擁有權、裝置類型、主要物件，以及您組織的基礎結構。
...
> [!NOTE]
> 此圖中不會顯示個人或自備裝置 (BYOD) 案例。 它們一律會導致 Azure AD 註冊。
...
| Windows 舊版裝置 (Windows 8.1 或 Windows 7) | | | ![核取](./media/plan-device-deployment/check.png) |
|**登入選項**| | |  |
...
條件式存取 <br> (需要混合式 Azure AD 加入的裝置) | | | ![核取](./media/plan-device-deployment/check.png)
...
Azure AD 已註冊的裝置支援將您自己的裝置 (BYOD) 和公司擁有的裝置，提供給雲端資源的 SSO。 資源的存取是以套用至裝置和使用者的 Azure AD [CA 原則](../conditional-access/require-managed-devices.md)為基礎。
...
大部分的組織已加入網域的裝置，並透過群組原則或 System Center Configuration Manager (SCCM) 來管理它們。 在這種情況下，我們建議您設定混合式 Azure AD 聯結，以在運用現有投資的同時開始獲得效益。
...
* [尋找裝置](device-management-azure-portal.md#locate-devices)

* [執行裝置身分識別管理工作](device-management-azure-portal.md#device-identity-management-tasks)
...
| - | - | - | - |
| [ (MDM) 的行動裝置管理](https://docs.microsoft.com/windows/client-management/mdm/azure-active-directory-integration-with-mdm) <br>範例： Microsoft Intune| ![核取](./media/plan-device-deployment/check.png)| ![核取](./media/plan-device-deployment/check.png)| ![核取](./media/plan-device-deployment/check.png)|  |
...
| [群組原則](https://docs.microsoft.com/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/hh831791(v=ws.11))<br>僅 (Windows) | | | ![核取](./media/plan-device-deployment/check.png)|  |



 我們建議您針對已註冊的 iOS 或 Android 裝置，考慮 Microsoft Intune 的行動[應用程式管理 (MAM) ](https://docs.microsoft.com/mem/intune/apps/app-management) ，或不使用裝置管理。

 系統管理員也可以[部署虛擬桌面基礎結構 (](howto-device-identity-virtual-desktop-infrastructure.md)在其組織中裝載 Windows 作業系統的 VDI) 平臺，以透過匯總和集中化資源來簡化管理並降低成本。 
