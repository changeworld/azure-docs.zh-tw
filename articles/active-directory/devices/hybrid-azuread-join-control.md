...
> [!NOTE]
> 如果您無法在 AD FS 伺服器上設定用戶端 SCP，則會將裝置身分識別的來源視為內部部署。 ADFS 接著會在 ADFS 裝置註冊的屬性 "MaximumInactiveDays" 中定義約定期限之後，開始刪除內部部署目錄中的裝置物件。 您可以使用 [set-adfsdeviceregistration Cmdlet](/powershell/module/adfs/get-adfsdeviceregistration?view=win10-ps)找到 ADFS 裝置註冊物件。
