...
當您在存取面板中按一下 [EasySSO for Bamboo] 圖格時，應該會自動登入您已設定 SSO 的 Bamboo 執行個體。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](../user-help/my-apps-portal-end-user-access.md)。
...
- [如何使用進階可見性和控制項保護 EasySSO for Bamboo](/cloud-app-security/proxy-intro-aad)