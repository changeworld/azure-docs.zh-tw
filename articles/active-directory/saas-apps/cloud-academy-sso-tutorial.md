...
若要深入了解 SaaS 應用程式與 Azure AD 的整合，請參閱[什麼是單一登入？](../manage-apps/what-is-single-sign-on.md)。
...
當您在存取面板中選取 [Cloud Academy - SSO] 圖格時，應該會自動登入您設定 SSO 的 Cloud Academy - SSO 執行個體。 如需詳細資訊，請參閱[存取面板簡介](../user-help/my-apps-portal-end-user-access.md)。
...
- [如何使用進階可見性和控制項保護 Cloud Academy - SSO](/cloud-app-security/proxy-intro-aad)