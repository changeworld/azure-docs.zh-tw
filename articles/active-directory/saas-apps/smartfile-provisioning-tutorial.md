...
description: 瞭解如何設定 Azure Active Directory，以將使用者帳戶自動布建和取消布建至 SmartFile。
...
ms.locfileid: "91286109"
---
# <a name="tutorial-configure-smartfile-for-automatic-user-provisioning"></a>教學課程：設定 SmartFile 來自動布建使用者

本教學課程的目的是要示範在 SmartFile 中執行的步驟，以及 Azure Active Directory (Azure AD) 將 Azure AD 設定為自動布建和解除布建使用者和/或群組至 SmartFile。
...
## <a name="setup-smartfile-for-provisioning"></a>設定 SmartFile 以進行布建

使用 Azure AD 設定 SmartFile 來自動布建使用者之前，您必須在 SmartFile 上啟用 SCIM 布建，並收集所需的其他詳細資料。

1. 登入您的 SmartFile 管理主控台。 流覽至 SmartFile 管理主控台的右上角。 選取 [ **產品金鑰**]。

    ![SmartFile 管理主控台](media/smartfile-provisioning-tutorial/login.png)

2. 若要產生持有人權杖，請複製 **產品金鑰** 和 **產品密碼**。 將它們貼入具有冒號的記事本中。
    
     ![產品金鑰區段的螢幕擷取畫面，其中包含稱為 [產品金鑰] 和 [產品密碼] 文字方塊。](media/smartfile-provisioning-tutorial/auth.png)

    ![純文字的螢幕擷取畫面，其中顯示產品金鑰和產品密碼（以冒號分隔）。](media/smartfile-provisioning-tutorial/key.png)
...
4. 在搜尋方塊中，輸入 **SmartFile**，在 [結果] 面板中選取 [ **SmartFile** ]，然後按一下 [ **新增** ] 按鈕以新增應用程式。
...
本節將引導您逐步設定 Azure AD 布建服務，以根據 Azure AD 中的使用者和/或群組指派，在 SmartFile 中建立、更新及停用使用者和/或群組。

> [!TIP]
> 您也可以選擇啟用 SmartFile 的 SAML 型單一登入，請遵循 [SmartFile 單一登入教學](SmartFile-tutorial.md)課程中提供的指示。 您可以獨立設定自動使用者布建的單一登入，雖然這兩個功能彼此補充

### <a name="to-configure-automatic-user-provisioning-for-smartfile-in-azure-ad"></a>若要在 Azure AD 中為 SmartFile 設定自動使用者布建：
...
5.  在 [ **管理員認證** ] 區段下的 [ `https://<SmartFile sitename>.smartfile.com/ftp/scim` **租使用者 URL**] 中輸入。 範例看起來會像這樣 `https://demo1test.smartfile.com/ftp/scim` 。 輸入您稍早在**秘密權杖**中取出的**持有人權杖**值 (ProductKey： ProductPassword) 。 按一下 [ **測試連接** ] 以確保 Azure AD 可以連線至 SmartFile。 如果連接失敗，請確定您的 SmartFile 帳戶具有系統管理員許可權，然後再試一次。
...
9. 在 [ **屬性對應** ] 區段中，檢查從 Azure AD 同步處理到 SmartFile 的使用者屬性。 選取為 [比對 **] 屬性的屬性會** 用來比對 SmartFile 中的使用者帳戶以進行更新作業。 選取 [儲存] 按鈕以認可所有變更。
...
11. 在 [ **屬性對應** ] 區段中，檢查從 Azure AD 同步處理到 SmartFile 的群組屬性。 選取為 [比對 **] 屬性的屬性會** 用來比對 SmartFile 中的群組以進行更新作業。 選取 [儲存] 按鈕以認可所有變更。

    ![SmartFile 群組屬性](media/smartfile-provisioning-tutorial/groupattribute.png)

12. 若要設定範圍篩選，請參閱[範圍篩選教學課程](../app-provisioning/define-conditional-rules-for-provisioning-user-accounts.md)中提供的下列指示。

13. 若要啟用 SmartFile Azure AD 的布建服務，請在 [**設定**] 區段中，將 [布建**狀態**] 變更為 [**開啟**]。
...
15. 當您準備好要佈建時，按一下 [儲存]。
    ...
    此作業會對在 [設定]**** 區段的 [範圍]**** 中定義的所有使用者和/或群組，啟動首次同步處理。 初始同步處理會比後續同步處理花費更多時間執行，只要 Azure AD 佈建服務正在執行，這大約每 40 分鐘便會發生一次。 您可以使用 [ **同步處理詳細資料** ] 區段來監視進度，並遵循連結來布建活動報告，該報告描述 SmartFile 上的 Azure AD 布建服務所執行的所有動作。
