...
若要設定將 AWS ClientVPN 整合到 Azure AD 中，您需要從資源庫將 AWS ClientVPN 新增到受控 SaaS 應用程式清單。
...
3. 您可以使用 Microsoft 存取面板。 當您按一下存取面板中的 [AWS ClientVPN] 圖格時，將會重新導向至 AWS ClientVPN 登入 URL。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
