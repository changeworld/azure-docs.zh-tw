...
4. 在 [基本 SAML 組態] 對話方塊中，完成下列步驟。
    ...
    > [!NOTE]
    > 這些值都是預留位置。 您需要使用實際的登入 URL 和識別碼。 請連絡 [Predictix Price Reporting 支援小組](https://www.infor.com/company/customer-center/)以取得此值。 您也可以參考 Azure 入口網站中的 [基本 SAML 組態] 對話方塊所顯示的模式。
...
若要在 Predictix Price Reporting 端設定單一登入，您必須將從 Azure 入口網站下載的憑證和複製的 URL 傳送給 [Predictix Price Reporting 支援小組](https://www.infor.com/company/customer-center/)。 此小組會確定兩端的 SAML SSO 連線都已正確設定。
