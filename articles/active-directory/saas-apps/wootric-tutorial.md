---
title: 教學課程：Azure Active Directory 單一登入 (SSO) 與 Wootric 整合 | Microsoft Docs
description: 了解如何設定 Azure Active Directory 與 Wootric 之間的單一登入。
...
在此教學課程中，您將會了解如何整合 Wootric 與 Azure Active Directory (Azure AD)。 在整合 Wootric 與 Azure AD 時，您可以︰

* 在 Azure AD 中控制可存取 Wootric 的人員。
...
若要設定將 Wootric 整合到 Azure AD 中，您需要將 Wootric 從資源庫新增到受控 SaaS 應用程式清單。
...
## <a name="configure-and-test-azure-ad-sso-for-wootric"></a>設定及測試 Wootric 的 Azure AD SSO

以名為 **B.Simon** 的測試使用者，設定及測試與 Wootric 搭配運作的 Azure AD SSO。 若要讓 SSO 能夠運作，您必須建立 Azure AD 使用者與 Wootric 中相關使用者之間的連結關聯性。

若要設定及測試與 Wootric 搭配運作的 Azure AD SSO，請完成下列建置組塊：

1. **[設定 Azure AD SSO](#configure-azure-ad-sso)** - 讓您的使用者能夠使用此功能。
    1. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 B.Simon 測試 Azure AD 單一登入。
    1. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 B.Simon 能夠使用 Azure AD 單一登入。
1. **[設定 Wootric SSO](#configure-wootric-sso)** - 在應用程式端設定單一登入設定。
    1. **[建立 Wootric 測試使用者](#create-wootric-test-user)** - 使 Wootric 中對應的 B.Simon 連結到該使用者在 Azure AD 中的代表項目。
...
1. Wootric 應用程式需要特定格式的 SAML 判斷提示，需要您加入自訂屬性對應到您的 SAML 權杖屬性設定。 以下螢幕擷取畫面顯示預設屬性清單。

    ![image](common/default-attributes.png)

1. 除了上述屬性外，Wootric 應用程式還需要 SAML 回應中多傳回幾個屬性，如下所示。 這些屬性也會預先填入，但您可以根據您的需求來檢閱這些屬性。
...
1. 在 [設定 Wootric] 區段上，根據您的需求複製適當的 URL。
...
在此節中，您會將 Wootric 的存取權授與 B.Simon，讓其能夠使用 Azure 單一登入。

1. 在 Azure 入口網站中，選取 [企業應用程式]，然後選取 [所有應用程式]。
1. 在應用程式清單中，選取 [Wootric]。
...
## <a name="configure-wootric-sso"></a>設定 Wootric SSO
...
在此節中，會在 Wootric 中建立名為 B.Simon 的使用者。 Wootric 支援依預設啟用的 Just-In-Time 使用者佈建。 在這一節沒有您需要進行的動作項目。 如果 Wootric 中還沒有任何使用者存在，在驗證之後就會建立新的使用者。
