...
1. 將 [佈建模式] 設定為 [自動]。 
    ...
    ![螢幕擷取畫面：Azure 入口網站中 GoToMeeting 的 [佈建] 索引標籤。 [佈建模式] 設定為 [自動]，且已醒目提示 [管理員使用者名稱]、[密碼] 和 [測試連線]。](./media/citrixgotomeeting-provisioning-tutorial/provisioning.png)
