...
5. 在 [**管理員認證**] 區段底下`https://<subdomain>.officespacesoftware.com/api/scim/v2/`的 [**租使用者 url**] 中輸入 URL 格式。 例如 `https://contoso.officespacesoftware.com/api/scim/v2/` 。 輸入稍早在**秘密權杖**中所取得的**SCIM Authentication Token**值。 按一下 [**測試連接**] 以確保 Azure AD 可以連接到 officespace software 軟體。 如果連線失敗，請確定您的 Officespace software 軟體帳戶具有系統管理員許可權，然後再試一次。
