---
title: 教學課程：Azure Active Directory 單一登入 (SSO) 與 Netvision Compas 整合 | Microsoft Docs
description: 了解如何設定 Azure Active Directory 與 Netvision Compas 之間的單一登入。
...
在本教學課程中，您將了解如何整合 Netvision Compas 與 Azure Active Directory (Azure AD)。 在整合 Netvision Compas 與 Azure AD 時，您可以︰

* 在 Azure AD 中控制可存取 Netvision Compas 的人員。
* 讓使用者使用其 Azure AD 帳戶自動登入 Netvision Compas。
...
* 已啟用 Netvision Compas 單一登入 (SSO) 的訂用帳戶。

## <a name="scenario-description"></a>案例描述

在本教學課程中，您會在測試環境中設定和測試 Azure AD SSO。

* Netvision Compas 支援由 **SP 和 IDP** 起始的 SSO
* 設定 Netvision Compas 後，您可以強制執行工作階段控制項，以即時防止組織的敏感資料遭到外洩和滲透。 工作階段控制項會從條件式存取延伸。 [了解如何使用 Microsoft Cloud App Security 來強制執行工作階段控制項](/cloud-app-security/proxy-deployment-aad)


## <a name="adding-netvision-compas-from-the-gallery"></a>從資源庫新增 Netvision Compas

若要進行將 Netvision Compas 整合到 Azure AD 中的設定，您必須從資源庫將 Netvision Compas 新增至受控 SaaS 應用程式清單。

1. 使用公司或學校帳戶或個人的 Microsoft 帳戶登入 [Azure 入口網站](https://portal.azure.com)。
1. 在左方瀏覽窗格上，選取 [Azure Active Directory]  服務。
1. 巡覽至 [企業應用程式]  ，然後選取 [所有應用程式]  。
1. 若要新增應用程式，請選取 [新增應用程式]  。
1. 在 [從資源庫新增]  區段的搜尋方塊中，輸入 **Netvision Compas** 。
1. 從結果面板中選取 [Netvision Compas]  ，然後新增應用程式。 當應用程式新增至您的租用戶時，請等候幾秒鐘。


## <a name="configure-and-test-azure-ad-single-sign-on-for-netvision-compas"></a>設定及測試 Netvision Compas 的 Azure AD 單一登入

以名為 **B.Simon** 的測試使用者，設定及測試與 Netvision Compas 搭配運作的 Azure AD SSO。 若要讓 SSO 能夠運作，您必須建立 Azure AD 使用者與 Netvision Compas 中相關使用者之間的連結關聯性。

若要設定及測試與 Netvision Compas 搭配運作的 Azure AD SSO，請完成下列建置組塊：
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Netvision Compas]  應用程式整合頁面上，尋找 [管理]  區段並選取 [單一登入]  。
...
1. 在 [基本 SAML 設定]  區段上，如果您想要以 **IDP** 起始模式設定應用程式，請輸入下列欄位的值：
    ...
    a. 在 [識別碼]  文字方塊中，使用下列模式來輸入 URL：`https://<TENANT>.compas.cloud/Identity/Saml20`

    b. 在 [回覆 URL]  文字方塊中，使用下列模式來輸入 URL：`https://<TENANT>.compas.cloud/Identity/Auth/AssertionConsumerService`

1. 如果您想要以 **SP** 起始模式設定應用程式，請按一下 [設定其他 URL]  ，然後執行下列步驟：

    在 [登入 URL]  文字方塊中，以下列模式輸入 URL︰`https://<TENANT>.compas.cloud/Identity/Auth/AssertionConsumerService`
...
在本節中，您會將 Netvision Compas 的存取權授與 B.Simon，讓其能夠使用 Azure 單一登入。

1. 在 Azure 入口網站中，選取 [企業應用程式]  ，然後選取 [所有應用程式]  。
1. 在應用程式清單中，選取 [Netvision Compas]  。
...
當您在存取面板中按一下 [Netvision Compas] 圖格時，應該會自動登入您已設定 SSO 的 Netvision Compas。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](../user-help/my-apps-portal-end-user-access.md)。
...
- [嘗試搭配 Azure AD 使用 Netvision Compas](https://aad.portal.azure.com/)
