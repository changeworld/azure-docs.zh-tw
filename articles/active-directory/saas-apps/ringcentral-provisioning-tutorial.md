...
description: 瞭解如何從 Azure AD 將使用者帳戶自動布建和取消布建至 RingCentral。
...
ms.locfileid: "94352787"
---
# <a name="tutorial-configure-ringcentral-for-automatic-user-provisioning"></a>教學課程：設定 RingCentral 來自動布建使用者

本教學課程說明在 RingCentral 和 Azure Active Directory (Azure AD) 設定自動使用者布建時所需執行的步驟。 當設定時，Azure AD 會使用 Azure AD 布建服務，自動將使用者和群組布建和取消布建至 [RingCentral](https://www.ringcentral.com/office/plansandpricing.html) 。 如需此服務的用途、運作方式和常見問題等重要詳細資訊，請參閱[使用 Azure Active Directory 對 SaaS 應用程式自動佈建和取消佈建使用者](../app-provisioning/user-provisioning.md)。 


## <a name="capabilities-supported"></a>支援的功能
> [!div class="checklist"]
> * 在 RingCentral 中建立使用者
> * 當使用者不再需要存取權時，請移除 RingCentral 中的使用者
> * Azure AD 與 RingCentral 之間保持使用者屬性同步
> * RingCentral (建議的[單一登入](./ringcentral-tutorial.md)) 
...
3. 判斷要 [在 Azure AD 與 RingCentral 之間對應](../app-provisioning/customize-application-attributes.md)的資料。 

## <a name="step-2-configure-ringcentral-to-support-provisioning-with-azure-ad"></a>步驟 2： 設定 RingCentral 以支援 Azure AD 的布建
...
2.  選擇 [ **選取目錄提供者** ] 下的 [ **SCIM** ]。  (未來將會有一個稱為 Azure Active Directory) 的選項。 按一下 [ **啟用 SCIM 服務** ]。

    ![RingCentral 新增 SCIM](media/ringcentral-provisioning-tutorial/scim.png)

3.  請聯絡 RingCentral 支援小組，以 matthew.hunt@ringcentral.com 取得 **SCIM Authentication 權杖** 。 在 Azure 入口網站的 RingCentral 應用程式的 [布建] 索引標籤中，將會在 [秘密權杖] 欄位中輸入此值。
...
從 Azure AD 應用程式資源庫新增 RingCentral，以開始管理布建至 RingCentral。 如果您先前已設定 SSO 的 RingCentral，您可以使用相同的應用程式。 不過，建議您在一開始測試整合時，建立個別的應用程式。 [在此](../manage-apps/add-application-portal.md)深入了解從資源庫新增應用程式。 
...
### <a name="to-configure-automatic-user-provisioning-for-ringcentral-in-azure-ad"></a>若要在 Azure AD 中為 RingCentral 設定自動使用者布建：
...
9. 在 [ **屬性對應** ] 區段中，檢查從 Azure AD 同步處理到 RingCentral 的使用者屬性。 選取為 [比對 **] 屬性的屬性會** 用來比對 RingCentral 中的使用者帳戶以進行更新作業。 如果您選擇變更相符的 [目標屬性](../app-provisioning/customize-application-attributes.md)，您將必須確定 RingCentral API 支援根據該屬性篩選使用者。 選取 [儲存] 按鈕以認可所有變更。
...
11. 若要啟用 RingCentral Azure AD 的布建服務，請在 [ **設定** ] 區段中，將 [布建 **狀態** ] 變更為 [ **開啟** ]。
