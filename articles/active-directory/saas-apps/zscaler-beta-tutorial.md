...
* Zscaler Beta 支援 **SP** 起始的 SSO。
* Zscaler **Beta 支援即時** 使用者布建。
...
1. 在 [ **從資源庫新增** ] 區段的 [搜尋] 方塊中輸入 Zscaler 搶鮮 **版（Beta** ）。
1. 從結果面板選取 [ **Zscaler Beta** ]，然後新增應用程式。 當應用程式新增至您的租用戶時，請等候幾秒鐘。

## <a name="configure-and-test-azure-ad-sso-for-zscaler-beta"></a>設定及測試 Azure AD SSO for Zscaler Beta

使用名為 **b. Simon** 的測試使用者，設定及測試與 Zscaler Beta 搭配的 Azure AD SSO。 若要讓 SSO 能夠運作，您必須建立 Azure AD 使用者與 Zscaler Beta 中相關使用者之間的連結關聯性。

若要設定及測試與 Zscaler Beta Azure AD SSO，請執行下列步驟：


1. **[設定 Azure AD SSO](#configure-azure-ad-sso)** - 讓您的使用者能夠使用此功能。
    1. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 B.Simon 測試 Azure AD 單一登入。
    1. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 B.Simon 能夠使用 Azure AD 單一登入。
1. **[設定 Zscaler BETA SSO](#configure-zscaler-beta-sso)** -在應用程式端設定單一 Sign-On 設定。
    1. **[建立 Zscaler Beta 測試使用者](#create-zscaler-beta-test-user)** -使 Zscaler Beta 中 Simon 的對應使用者連結到該使用者在 Azure AD 的代表身分。
...
1. 在 Azure 入口網站的 [ **Zscaler Beta** 應用程式整合] 頁面上，尋找 [ **管理** ] 區段並選取 [ **單一登入**]。
...
1. 在 [基本 SAML 組態]  區段上，輸入下列欄位的值：
    ...
    在 [登入 **url** ] 方塊中，輸入使用者用來登入 Zscaler Beta Beta 應用程式的 URL。
...
在本節中，您會將 Zscaler Beta 的存取權授與 b. Simon，讓她能夠使用 Azure 單一登入。
...
## <a name="configure-zscaler-beta-sso"></a>設定 Zscaler Beta SSO
...
* 在 Azure 入口網站中按一下 [測試此應用程式]。 這會重新導向至 Zscaler Beta 登入 URL，您可以在其中起始登入流程。 
...
設定 Zscaler Beta 之後，您可以強制執行會話控制項，以即時防止組織的敏感性資料遭到外泄和滲透。 工作階段控制項會從條件式存取延伸。 [了解如何使用 Microsoft Cloud App Security 來強制執行工作階段控制項](/cloud-app-security/proxy-deployment-any-app)。