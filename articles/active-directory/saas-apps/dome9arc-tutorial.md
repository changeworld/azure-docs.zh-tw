...
1. 除了以上屬性之外，Check Point CloudGuard Dome9 Arc 應用程式還預期 SAML 回應中會再多傳回幾個屬性，如下所示。 這些屬性也會預先填入，但您可以根據您的需求來檢閱這些屬性。
    ...
    >[!NOTE]
    >請按一下[此處](./apptio-tutorial.md)來了解如何在 Azure AD 中建立角色。
...
當您在存取面板中按一下 [Check Point CloudGuard Dome9 Arc] 圖格時，應該會自動登入您已設定 SSO 的 Check Point CloudGuard Dome9 Arc。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](../user-help/my-apps-portal-end-user-access.md)。
