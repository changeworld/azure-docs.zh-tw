...
> * Fuze (建議的[單一登入](./fuze-tutorial.md)) 
...
3. 判斷要 [在 Azure AD 與 Fuze 之間對應](../app-provisioning/customize-application-attributes.md)的資料。 
...
從 Azure AD 應用程式資源庫新增 Fuze，以開始管理布建至 Fuze。 如果您先前已設定 SSO 的 Fuze，您可以使用相同的應用程式。 不過，建議您在一開始測試整合時，建立個別的應用程式。 [在此](../manage-apps/add-application-portal.md)深入了解從資源庫新增應用程式。
