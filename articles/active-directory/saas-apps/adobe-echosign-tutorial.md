...
1. 在 [SAML 設定]  區段中，執行下列步驟：
   ...
   ![醒目提示 SAML 設定 (包括 [SAML 強制]) 的螢幕擷取畫面。](./media/adobe-echosign-tutorial/ic789521.png "SAML 設定")
...
當您在存取面板中按一下 [Adobe Sign] 圖格時，應該會自動登入您已設定 SSO 的 Adobe Sign。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](../user-help/my-apps-portal-end-user-access.md)。
