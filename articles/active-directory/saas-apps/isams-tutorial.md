---
title: 教學課程：Azure Active Directory 單一登入 (SSO) 與 iSAMS 整合 | Microsoft Docs
description: 了解如何設定 Azure Active Directory 與 iSAMS 之間的單一登入。
...
在本教學課程中，您會了解如何將 iSAMS 與 Azure Active Directory (Azure AD) 進行整合。 在整合 iSAMS 與 Azure AD 時，您可以︰

* 在 Azure AD 中控制可存取 iSAMS 的人員。
...
若要設定將 iSAMS 整合到 Azure AD 中，您需要從資源庫將 iSAMS 新增到受控 SaaS app 清單。
...
## <a name="configure-and-test-azure-ad-sso-for-isams"></a>設定和測試 iSAMS 的 Azure AD SSO

以名為 **B.Simon** 的測試使用者，設定及測試與 iSAMS 搭配運作的 Azure AD SSO。 若要讓 SSO 能夠運作，您必須建立 Azure AD 使用者與 iSAMS 中相關使用者之間的連結關聯性。

若要設定及測試與 iSAMS 搭配運作的 Azure AD SSO，請完成下列建置組塊：

1. **[設定 Azure AD SSO](#configure-azure-ad-sso)** - 讓您的使用者能夠使用此功能。
    1. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 B.Simon 測試 Azure AD 單一登入。
    1. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 B.Simon 能夠使用 Azure AD 單一登入。
1. **[設定 iSAMS SSO](#configure-isams-sso)** - 在應用程式端設定單一登入設定。
    1. **[建立 iSAMS 測試使用者](#create-isams-test-user)** - 使 iSAMS 中對應的 B.Simon 連結到該使用者在 Azure AD 中的代表項目。
...
1. 在 [基本 SAML 設定] 區段上，如果您想要以 **IDP** 起始模式設定應用程式，請輸入下列欄位的值：
    ...
    a. 在 [識別碼] 文字方塊中，使用下列模式來輸入 URL：`https://<SUBDOMAIN>.isams.cloud/main/sso/saml2`

    b. 在 [回覆 URL] 文字方塊中，使用下列模式來輸入 URL：`https://<SUBDOMAIN>.isams.cloud/main/sso/saml2/acs`
...
在本節中，您會將 iSAMS 的存取權授與 B.Simon，讓其能夠使用 Azure 單一登入。
...
## <a name="configure-isams-sso"></a>設定 iSAMS SSO
...
1. 從右側的功能表中，選取 [識別提供者]
    ...
    ![組態](./media/isams-tutorial/click-identity-provider.png)

1. 選取 [新增提供者]。

    ![組態](./media/isams-tutorial/add-identity-provider.png)


1. 在下列頁面上，執行下列步驟：

    ![組態](./media/isams-tutorial/configure-isams.png)
...
### <a name="create-isams-test-user"></a>建立 iSAMS 測試使用者

1. 以管理員身分登入 iSAMS。

2.  移至**控制台首頁** -> **安全性和權限** -> **使用者帳戶** -> **使用者選項和工作** -> **修改使用者屬性**

    ![測試使用者](./media/isams-tutorial/modify-user-properties.png)


3. 在產生的快顯視窗中，選取**帳戶詳細資料**索引標籤，然後將**授權**變更為新建立的識別提供者。

    ![測試使用者](./media/isams-tutorial/account-details.png)
