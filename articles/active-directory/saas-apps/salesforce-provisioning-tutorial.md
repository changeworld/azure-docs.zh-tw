...
本節將 Azure AD 連接到[Salesforce 的使用者帳戶預配 API - v40，](https://developer.salesforce.com/docs/atlas.en-us.208.0.api.meta/api/implementation_considerations.htm)並根據 Azure AD 中的使用者和組分配配置預配服務來創建、更新和禁用 Salesforce 中分配的使用者帳戶。
...
> [!NOTE]
> 在 Salesforce 應用程式中預配使用者後，管理員需要為其配置特定于語言的設置。 有關語言配置的更多詳細資訊，請參閱[本文](https://help.salesforce.com/articleView?id=setting_your_language.htm&type=5)。
...
* 如果您在授權訪問 Salesforce 時遇到問題，請確保：
    * 使用的憑據具有對 Salesforce 的管理存取權限。
    * 您使用的 Salesforce 版本支援 Web 訪問（例如開發人員、企業版、沙箱版和無限版的 Salesforce）。
    * 為使用者啟用 Web API 訪問。
* Azure AD 預配服務支援使用者的預配語言、地區設定和時區。 這些屬性位於預設屬性映射中，但沒有預設源屬性。 請確保選擇預設源屬性，並且源屬性採用 SalesForce 預期的格式。 例如，英語（美國）的地區設定是en_US。 查看[此處](https://help.salesforce.com/articleView?id=setting_your_language.htm&type=5)提供的指南，以確定正確的地區設定 SidKey 格式。 語言LocaleKey格式可以[在這裡](https://help.salesforce.com/articleView?id=faq_getstart_what_languages_does.htm&type=5)找到。 除了確保格式正確之外，您可能需要確保為使用者啟用語言，如[此處](https://help.salesforce.com/articleView?id=setting_your_language.htm&type=5)所述。 
* **銷售力量許可證限制超過：** 無法在目標應用程式中創建使用者，因為此使用者沒有可用的許可證。 為目標應用程式獲取其他許可證，或者查看使用者分配和屬性對應配置，以確保使用正確的屬性分配正確的使用者。
* **銷售隊伍重複使用者名：** 無法預配使用者，因為它具有在另一個Salesforce.com租戶中複製的Salesforce.com"使用者名"。在Salesforce.com中，"使用者名"屬性的值在所有Salesforce.com租戶中都必須是唯一的。預設情況下，Azure 活動目錄中的使用者主要名稱將成為其Salesforce.com中的"使用者名"。您有兩個選項。一個選項是查找和重命名使用者與重複的"使用者名"在其他Salesforce.com租戶，如果你管理另一個租戶。另一個選項是從 Azure 活動目錄使用者中刪除對與其集成目錄的Salesforce.com租戶的訪問。 我們將在下一次同步嘗試時重試此操作。 
* **銷售隊伍需要現場缺失：** Salesforce 要求使用者上存在某些屬性才能成功創建或更新使用者。 此使用者缺少所需的屬性之一。 確保在要預配到 Salesforce 的所有使用者上填充了電子郵件和別名等屬性。 您可以使用[基於屬性的範圍篩選器](https://docs.microsoft.com/azure/active-directory/manage-apps/define-conditional-rules-for-provisioning-user-accounts)來限定沒有這些屬性的使用者。 
* 預配到 Salesforce 的預設屬性映射包括單一AppRole分配運算式，用於在 Azure AD 中將應用Role分配映射到 Salesforce 中的設定檔名稱。 確保使用者在 Azure AD 中沒有多個應用角色指派，因為屬性對應僅支援預配一個角色。 
* Salesforce 要求在更改之前手動批准電子郵件更新。 因此，您可能會在預配日誌中看到多個條目來更新使用者的電子郵件（直到電子郵件更改獲得批准）。
...
* [配置單點登錄](https://docs.microsoft.com/azure/active-directory/active-directory-saas-salesforce-tutorial)
