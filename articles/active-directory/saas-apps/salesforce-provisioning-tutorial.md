...
* **SalesforceRequiredFieldMissing：** Salesforce 要求使用者必須有某些屬性，才能成功建立或更新使用者。 此使用者缺少其中一個必要的屬性。 請確定在您想要布建到 Salesforce 的所有使用者上，都已填入電子郵件和別名等屬性。 您可以使用以 [屬性為基礎的範圍篩選準則](https://docs.microsoft.com/azure/active-directory/manage-apps/define-conditional-rules-for-provisioning-user-accounts)，將沒有這些屬性的使用者設定為範圍。 
