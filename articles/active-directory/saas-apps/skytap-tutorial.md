...
* 設定 Single Sign-on for Skytap 後，您可以強制執行工作階段控制項，以即時防止組織的敏感資料遭到外洩和滲透。 工作階段控制項會從條件式存取延伸。 [了解如何使用 Microsoft Cloud App Security 來強制執行工作階段控制項](https://docs.microsoft.com/cloud-app-security/proxy-deployment-any-app)。

## <a name="adding-single-sign-on-for-skytap-from-the-gallery"></a>從資源庫新增 Single Sign-on for Skytap
...
以名為 **B.Simon** 的測試使用者，設定及測試與 Single Sign-on for Skytap 搭配運作的 Azure AD SSO。 若要讓 SSO 能夠運作，您必須建立 Azure AD 使用者與 Single Sign-on for Skytap 中相關使用者之間的連結關聯性。

若要設定及測試與 Single Sign-on for Skytap 搭配運作的 Azure AD SSO，請完成下列構成要素：

1. **[設定 Azure AD SSO](#configure-azure-ad-sso)** - 讓您的使用者能夠使用此功能。
    1. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 B.Simon 測試 Azure AD 單一登入。
    1. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 B.Simon 能夠使用 Azure AD 單一登入。
1. **[設定 Single Sign-on for Skytap 單一登入](#configure-single-sign-on-for-skytap-sso)** - 在應用程式端設定單一登入設定。
    1. **[建立 Single Sign-on for Skytap 測試使用者](#create-single-sign-on-for-skytap-test-user)** - 使 Single Sign-on for Skytap 中對應的 B.Simon 連結到該使用者在 Azure AD 中的代表項目。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Single Sign-on for Skytap]  應用程式整合頁面上，尋找 [管理]  區段並選取 [單一登入]  。
...
1. 在 [設定 Single Sign-on for Skytap]  區段上，根據您的需求複製適當的 URL。
...
在本節中，您要在 Single Sign-on for Skytap 中建立名為 Britta Simon 的使用者。 與 [Single Sign-on for Skytap 用戶端支援小組合作](mailto:support@skytap.com)，在 Single Sign-on for Skytap 平台中新增使用者。 您必須先建立和啟動使用者，然後才能使用單一登入。

## <a name="test-sso"></a>測試 SSO 

在本節中，您會使用存取面板來測試您的 Azure AD 單一登入設定。

當您在存取面板中按一下 [Single Sign-on for Skytap] 圖格時，應該會自動登入您設定 SSO 的 Single Sign-on for Skytap。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
