...
5. 在 [**管理員認證**] 區段下，輸入 [ `https://www.figma.com/scim/v2/<TenantID>` **租使用者 URL** ]，其中**TenantID**是您稍早從 Figma 中取出的值。 在 [**秘密權杖**] 中輸入**API token**值。 按一下 [**測試連接**] 以確保 Azure AD 可以連接到 Figma。 如果連線失敗，請確定您的 Figma 帳戶具有系統管理員許可權，然後再試一次。
