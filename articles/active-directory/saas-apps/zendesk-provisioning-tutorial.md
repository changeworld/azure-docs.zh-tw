...
description: 瞭解如何設定 Azure Active Directory，以將使用者帳戶自動布建和解除布建至 Zendesk。
...
本教學課程示範在 Zendesk 中執行的步驟，以及 Azure Active Directory (Azure AD) 設定 Azure AD，以將使用者和群組自動布建和解除布建至 Zendesk。
...
* Azure AD 租用戶。
* 已啟用 Professional 方案或更佳的 Zendesk 租使用者。
* Zendesk 中具有系統管理員許可權的使用者帳戶。

## <a name="add-zendesk-from-the-azure-marketplace"></a>從 Azure Marketplace 新增 Zendesk

使用 Azure AD 設定 Zendesk 來自動布建使用者之前，請從 Azure Marketplace 將 Zendesk 新增至受控 SaaS 應用程式清單。
...
設定並啟用自動使用者布建之前，請先決定 Azure AD 中的哪些使用者或群組需要存取 Zendesk。 若要將這些使用者或群組指派給 Zendesk，請依照 [將使用者或群組指派給企業應用程式](../manage-apps/assign-user-or-group-access-portal.md)中的指示進行。

### <a name="important-tips-for-assigning-users-to-zendesk"></a>將使用者指派給 Zendesk 的重要秘訣

* 現在，Zendesk 角色會在 Azure 入口網站 UI 中自動且動態地填入。 將 Zendesk 角色指派給使用者之前，請確定已針對 Zendesk 完成初始同步處理，以取得 Zendesk 租使用者中的最新角色。

* 建議您將單一 Azure AD 使用者指派給 Zendesk，以測試您的初始自動使用者布建設定。 您稍後可以在測試成功後指派其他使用者或群組。

* 當您將使用者指派給 Zendesk 時，請在 [指派] 對話方塊中選取任何有效的應用程式特有角色（如果有的話）。 具有**預設存取**角色的使用者會從佈建中排除。

## <a name="configure-automatic-user-provisioning-to-zendesk"></a>設定自動使用者布建至 Zendesk 

本節將逐步引導您設定 Azure AD 佈建服務。 您可以使用它，根據 Azure AD 中的使用者或群組指派，在 Zendesk 中建立、更新和停用使用者或群組。

> [!TIP]
> 您也可以啟用 Zendesk 的 SAML 型單一登入。 遵循 [Zendesk 單一登入教學](zendesk-tutorial.md)課程中的指示。 您可以獨立設定自動使用者布建的單一登入，雖然這兩個功能彼此互補。

### <a name="configure-automatic-user-provisioning-for-zendesk-in-azure-ad"></a>在 Azure AD 中設定 Zendesk 的自動使用者布建

1. 登入 [Azure 入口網站](https://portal.azure.com)。 選取**Enterprise applications**  >  **所有應用程式**  >  **Zendesk**的企業應用程式。
...
5. 在 [系統 **管理員認證** ] 區段下，輸入您的 Zendesk 帳戶的系統管理員使用者名稱、秘密權杖和網域。 這些值的範例如下：

   * 在 [系統 **管理員使用者名稱** ] 方塊中，填入 Zendesk 租使用者中系統管理員帳戶的使用者名稱。 例如 admin@contoso.com。

   * 在 [ **秘密權杖** ] 方塊中，填入秘密權杖，如步驟6所述。

   * 在 [ **網域** ] 方塊中，填入 Zendesk 租使用者的子域。 例如，針對租使用者 URL 為的帳戶 `https://my-tenant.zendesk.com` ，您的子域就是 **我的租**使用者。

6. 您 Zendesk 帳戶的秘密權杖位於**管理**  >  **API**  >  **設定**中。 請確定 [ **權杖存取** ] 設定為 [ **已啟用**]。
...
7. 在您填入步驟5所示的方塊之後，請選取 [ **測試連接** ] 以確定 Azure AD 可以連線至 Zendesk。 如果連接失敗，請確定您的 Zendesk 帳戶具有系統管理員許可權，然後再試一次。
...
11. 在 [ **屬性** 對應] 區段中，檢查從 Azure AD 同步至 Zendesk 的使用者屬性。 選取為 [比對]**** 屬性的屬性會用來比對 Zendesk 中的使用者帳戶以進行更新作業。 若要儲存任何變更，請選取 [儲存]。
...
13. 在 [ **屬性** 對應] 區段中，檢查從 Azure AD 同步至 Zendesk 的群組屬性。 選取為 [比對]**** 屬性的屬性會用來比對 Zendesk 中的群組以進行更新作業。 若要儲存任何變更，請選取 [儲存]。
...
15. 若要啟用 Zendesk Azure AD 布建服務，請在 [ **設定** ] 區段中，將 [布建 **狀態** ] 變更為 [ **開啟**]。
...
您可以使用 [同步處理詳細資料] 區段來監視進度，並循著連結取得佈建活動報告。 報表會描述 Zendesk 上 Azure AD 布建服務所執行的所有動作。

如需如何讀取 Azure AD 佈建記錄的詳細資訊，請參閱[關於使用者帳戶自動佈建的報告](../app-provisioning/check-status-user-account-provisioning.md)。

## <a name="connector-limitations"></a>連接器限制

* Zendesk 僅支援使用具有 **代理程式** 角色的使用者群組。 如需詳細資訊，請參閱 [Zendesk 檔](https://support.zendesk.com/hc/en-us/articles/203661966-Creating-managing-and-using-groups)集。

* 將自訂角色指派給使用者或群組時，Azure AD 的自動使用者布建服務也會指派預設角色 **代理程式**。 只有代理程式才能指派自訂角色。 如需詳細資訊，請參閱 [ZENDESK API 檔](https://developer.zendesk.com/rest_api/docs/support/users#json-format-for-agent-or-admin-requests)。 
