...
4. 在 [基本 SAML 組態]  區段上，執行下列步驟：
    ...
    > [!NOTE]
    > 這些都不是真正的值。 請使用實際的「登入 URL」及「識別碼」來更新這些值。 請連絡 [客戶支援小組](https://www.performio.co/contact-us)以取得這些值。 您也可以參考 Azure 入口網站中 **基本 SAML 組態** 區段所示的模式。
...
當您在存取面板中按一下 PerformanceCentre 圖格時，應該會自動登入您設定 SSO 的 PerformanceCentre。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](../user-help/my-apps-portal-end-user-access.md)。
