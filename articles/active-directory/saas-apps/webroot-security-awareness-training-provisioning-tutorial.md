---
title: 教學課程：使用 Azure Active Directory 設定 Webroot 安全性認知訓練以進行自動使用者布建 |Microsoft Docs
description: 瞭解如何從 Azure AD 自動布建和取消布建使用者帳戶，以 Webroot 安全性認知訓練。
...
> [!div class="checklist"]
> * 在 Webroot 安全性認知訓練中建立使用者
> * 當使用者不再需要存取權時，移除 Webroot 安全性認知訓練中的使用者
> * 在 Azure AD 和 Webroot 安全性認知訓練之間保持使用者屬性同步處理
> * 在 Webroot 安全性認知訓練中布建群組和群組成員資格
...
## <a name="step-2-configure-webroot-security-awareness-training-to-support-provisioning-with-azure-ad"></a>步驟 2： 設定 Webroot 安全性認知訓練以支援以 Azure AD 布建

### <a name="obtain-a-secret-token"></a>取得秘密權杖

若要將您的網站連線到 Azure AD，您必須在 Webroot 管理主控台中取得該網站的**秘密權杖**。

1. 登入您的[Webroot 管理主控台](https://identity.webrootanywhere.com/v1/Account/login#tab_customers)

2. 從 [**網站**] 索引標籤中，按一下您想要與 Azure AD 連線之網站的 [安全性認知訓練] 欄中的齒輪圖示。
...
## <a name="step-3-add-webroot-security-awareness-training-from-the-azure-ad-application-gallery"></a>步驟 3： 從 Azure AD 應用程式庫新增 Webroot 安全性認知訓練

從 Azure AD 應用程式庫新增 Webroot 安全性認知訓練，以開始管理布建以 Webroot 安全性認知訓練。 [在此](https://docs.microsoft.com/azure/active-directory/manage-apps/add-gallery-app)深入了解從資源庫新增應用程式。 
...
* 指派使用者和群組以 Webroot 安全性認知訓練時，您必須選取 [**預設存取**] 以外的角色。 具有預設存取角色的使用者會從佈建中排除，而且會在佈建記錄中被標示為沒有效率。 如果應用程式上唯一可用的角色是 [預設存取] 角色，您可以[更新應用程式資訊清單](https://docs.microsoft.com/azure/active-directory/develop/howto-add-app-roles-in-azure-ad-apps) \(部分機器翻譯\) 以新增其他角色。 
...
## <a name="step-5-configure-automatic-user-provisioning-to-webroot-security-awareness-training"></a>步驟 5。 設定自動使用者布建以 Webroot 安全性認知訓練 

此節將引導您逐步設定 Azure AD 佈建服務，以根據 Azure AD 中的使用者和/或群組指派，在 TestApp 中建立、更新和停用使用者和/或群組。

### <a name="to-configure-automatic-user-provisioning-for-webroot-security-awareness-training-in-azure-ad"></a>若要在 Azure AD 中設定自動使用者布建以進行 Webroot 安全性認知訓練：

1. 登入 [Azure 入口網站](https://portal.azure.com)。 選取 [企業應用程式]，然後選取 [所有應用程式]。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，選取 [ **Webroot 安全性認知訓練**]。
...
9. 在 [**屬性對應**] 區段中，檢查從 Azure AD 同步處理的使用者屬性，以 Webroot 安全性認知訓練。 選取為 [比對] 屬性**的屬性會**用來比對 Webroot 安全性認知訓練中的使用者帳戶，以進行更新作業。 如果您選擇變更相符的[目標屬性](https://docs.microsoft.com/azure/active-directory/manage-apps/customize-application-attributes)，您將需要確保 Webroot 安全性認知訓練 API 支援根據該屬性來篩選使用者。 選取 [儲存] 按鈕以認可所有變更。
...
11. 在 [**屬性對應**] 區段中，檢查從 Azure AD 同步處理到 Webroot 安全性認知訓練的群組屬性。 選取為 [比對] 屬性**的屬性會**用來比對 Webroot 安全性認知訓練中的群組以進行更新作業。 選取 [儲存] 按鈕以認可所有變更。
...
13. 若要啟用 Azure AD 布建服務以進行 Webroot 安全性認知訓練，請在 [**設定**] 區段中將 [布建**狀態**] 變更為 [**開啟**]。

    ![佈建狀態已切換為開啟](common/provisioning-toggle-on.png)

14. 在 [**設定**] 區段的 [**範圍**] 中選擇所需的值，以定義您想要布建的使用者和/或群組，以 Webroot 安全性認知訓練。
