...
若要設定及測試與 8x8 搭配運作的 Azure AD SSO，請執行下列步驟：
...
1. 在 Azure 入口網站的 [8x8] 應用程式整合頁面上，尋找 [管理] 區段並選取 [單一登入]。
...
* 對於使用 Configuration Manager 進行管理的 8x8 版本和 X 系列客戶，請參閱[設定 8x8 管理主控台](#configure-8x8-admin-console)。
* 若為使用帳戶管理員來進行系統管理的 Virtual Office 客戶，請參閱[設定 8x8 帳戶管理員](#configure-8x8-account-manager)。

### <a name="configure-8x8-admin-console"></a>設定 8x8 管理主控台
...
1. 如果您想要手動設定 8x8，請以系統管理員身分登入 8x8 [管理主控台](https://admin.8x8.com/)。
...
1. 將 Azure AD 中 [設定使用 SAML 的單一登入] 頁面內的三個 URL 和簽署憑證，複製到 8x8 管理主控台中的 [Microsoft Azure AD SAML 設定] 區段。

    ![8x8 管理主控台](./media/8x8virtualoffice-tutorial/configure3.png)
...
* 在 Azure 入口網站中按一下 [測試此應用程式]。 這會重新導向您可以在其中起始登入流程的 8x8 登入 URL。  

* 直接移至 8x8 登入 URL，然後從該處起始登入流程。

#### <a name="idp-initiated"></a>IDP 起始：

* 在 Azure 入口網站中按一下 [測試此應用程式]，您應該會自動登入您已設定 SSO 的 8x8 

您也可以使用 Microsoft 存取面板，以任何模式測試應用程式。 當您按一下存取面板中的 8x8 圖格時，如果是在 SP 模式中設定，您會重新導向至 [應用程式登入] 頁面來起始登入流程，如果在 IDP 模式中設定，則應該會自動登入已設定 SSO 的 8x8。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。


## <a name="next-steps"></a>後續步驟

設定 8x8 後，您可以強制執行工作階段控制項，以即時防止組織的敏感資料遭到外洩和滲透。 工作階段控制項會從條件式存取延伸。 [了解如何使用 Microsoft Cloud App Security 來強制執行工作階段控制項](/cloud-app-security/proxy-deployment-any-app)。
