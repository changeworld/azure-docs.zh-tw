...
若要設定及測試與 Meraki Dashboard 搭配運作的 Azure AD SSO，請完成下列建構元素：
...
1. 除了上述屬性外，Meraki Dashboard 應用程式還需要在 SAML 回應中多傳回幾個屬性，如下所示。 這些屬性也會預先填入，但您可以根據您的需求來檢閱這些屬性。
    ...
    > [!NOTE]
    > 若要了解如何在 Azure AD 中設定角色，請參閱[此文章](../develop/active-directory-enterprise-app-role-management.md)。
...
1. 將您從 Azure 入口網站複製的 [指紋]  值，貼到 [X.590 憑證 SHA1 指紋]  文字方塊中。 然後按一下 [儲存]  。 儲存之後，取用者 URL 就會顯示。 複製 [取用者 URL] 值，並將其貼到 Azure 入口網站中 [基本 SAML 設定]  區段中的 [回覆 URL]  文字方塊。
    ...
    ![Meraki Dashboard 設定](./media/meraki-dashboard-tutorial/configure4.png)
...
當您在存取面板中按一下 Meraki Dashboard 圖格時，應該會自動登入您設定 SSO 的 Meraki Dashboard。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](../user-help/my-apps-portal-end-user-access.md)。
