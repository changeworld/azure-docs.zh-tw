---
title: 教學課程：LinkedIn Elevate 的使用者佈建 - Azure AD
...
4. 按一下 [+ 新增 SCIM 組態]，遵循程序填寫每個欄位。
    ...
    ![螢幕擷取畫面：顯示 LinkedIn 帳戶中心的全域設定。](./media/linkedinelevate-provisioning-tutorial/linkedin_elevate1.PNG)

    > [!NOTE]
    > 若啟用自動指派授權，您需要記下應用程式執行個體及授權類型。 授權的指派採取先到先拿原則，一直到所有授權都指派完為止。

    ![螢幕擷取畫面：顯示 [SCIM 設定] 頁面。](./media/linkedinelevate-provisioning-tutorial/linkedin_elevate2.PNG)
...
10. 將 [佈建模式]  設定為 [自動]  。
    ...
    ![螢幕擷取畫面：顯示 [LinkedIn Elevate 佈建] 頁面。](./media/linkedinelevate-provisioning-tutorial/linkedin_elevate3.PNG)
...
14. 在 [屬性對應] 區段中，檢閱將從 Azure AD 同步至 LinkedIn Elevate 的使用者和群組屬性。 請注意，選取為 [比對] 屬性的屬性會用來比對 LinkedIn Elevate 中的使用者帳戶和群組以進行更新作業。 選取 [儲存] 按鈕以認可任何變更。
    ...
    ![顯示 [對應] 的螢幕擷取畫面，包括屬性對應。](./media/linkedinelevate-provisioning-tutorial/linkedin_elevate4.PNG)
