---
title: 教學課程：Azure Active Directory 單一登入 (SSO) 與 Coda 整合 | Microsoft Docs
description: 了解如何設定 Azure Active Directory 與 Coda 之間的單一登入。
...
在本教學課程中，您將了解如何整合 Coda 與 Azure Active Directory (Azure AD)。 在整合 Coda 與 Azure AD 時，您可以︰

* 在 Azure AD 中控制可存取 Coda 的人員。
* 讓使用者使用其 Azure AD 帳戶自動登入 Coda。
...
* Coda 支援由 **IDP** 起始的 SSO

* Coda 支援 **Just In Time** 使用者佈建

* 設定 Coda 後，您可以強制執行工作階段控制項，以即時防止組織的敏感資料遭到外洩和滲透。 工作階段控制項會從條件式存取延伸。 [了解如何使用 Microsoft Cloud App Security 來強制執行工作階段控制項](https://docs.microsoft.com/cloud-app-security/proxy-deployment-any-app)。

## <a name="adding-coda-from-the-gallery"></a>從資源庫新增 Coda

若要進行將 Coda 整合到 Azure AD 中的設定，您必須從資源庫將 Coda 新增至受控 SaaS 應用程式清單。

1. 使用公司或學校帳戶或個人的 Microsoft 帳戶登入 [Azure 入口網站](https://portal.azure.com)。
1. 在左方瀏覽窗格上，選取 [Azure Active Directory]  服務。
1. 巡覽至 [企業應用程式]  ，然後選取 [所有應用程式]  。
1. 若要新增應用程式，請選取 [新增應用程式]  。
1. 在 [從資源庫新增]  區段的搜尋方塊中，輸入 **Coda**。
1. 從結果面板中選取 [Coda]  ，然後新增應用程式。 當應用程式新增至您的租用戶時，請等候幾秒鐘。


## <a name="configure-and-test-azure-ad-single-sign-on-for-coda"></a>設定及測試 Coda 的 Azure AD 單一登入

以名為 **B.Simon** 的測試使用者，設定及測試與 Coda 搭配運作的 Azure AD SSO。 若要讓 SSO 能夠運作，您必須建立 Azure AD 使用者與 Coda 中相關使用者之間的連結關聯性。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Coda]  應用程式整合頁面上，尋找 [管理]  區段並選取 [單一登入]  。
...
在本節中，您會將 Coda 的存取權授與 B.Simon，讓其能夠使用 Azure 單一登入。
...
當您在存取面板中按一下 [Coda] 圖格時，應該會自動登入您已設定 SSO 的 Coda。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
...
- [嘗試搭配 Azure AD 使用 Coda](https://aad.portal.azure.com/)

- [什麼是 Microsoft Cloud App Security 中的工作階段控制項？](https://docs.microsoft.com/cloud-app-security/proxy-intro-aad)

- [如何使用進階可見性和控制項保護 Coda](https://docs.microsoft.com/cloud-app-security/proxy-intro-aad)
