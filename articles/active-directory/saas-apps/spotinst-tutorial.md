---
title: 教學課程：Azure Active Directory 單一登入 (SSO) 與 Spotinst 整合 | Microsoft Docs
...
在本教學課程中，您將了解如何整合 Spotinst 與 Azure Active Directory (Azure AD)。 在整合 Spotinst 與 Azure AD 時，您可以︰

* 在 Azure AD 中控制可存取 Spotinst 的人員。
...
以名為 **B.Simon** 的測試使用者，設定及測試與 Spotinst 搭配運作的 Azure AD SSO。 若要讓 SSO 能夠運作，您必須建立 Azure AD 使用者與 Spotinst 中相關使用者之間的連結關聯性。

若要設定及測試與 Spotinst 搭配運作的 Azure AD SSO，請完成下列建置組塊：

1. **[設定 Azure AD SSO](#configure-azure-ad-sso)** - 讓您的使用者能夠使用此功能。
    * **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 B.Simon 測試 Azure AD 單一登入。
    * **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 B.Simon 能夠使用 Azure AD 單一登入。
1. **[設定 Spotinst SSO](#configure-spotinst-sso)** - 在應用程式端設定單一登入設定。
    * **[建立 Spotinst 測試使用者](#create-spotinst-test-user)** - 使 Spotinst 中對應的 B.Simon 連結到該使用者在 Azure AD 中的代表項目。
...
1. Spotinst 應用程式需要特定格式的 SAML 判斷提示，因此您必須將自訂屬性對應至 SAML 權杖屬性設定。 以下螢幕擷取畫面顯示預設屬性清單。

    ![image](common/default-attributes.png)

1. 除了上述屬性外，Spotinst 應用程式還需要在 SAML 回應中多傳回幾個屬性，如下所示。 這些屬性也會預先填入，但您可以根據您的需求來檢閱這些屬性。
...
## <a name="configure-spotinst-sso"></a>設定 Spotinst SSO
