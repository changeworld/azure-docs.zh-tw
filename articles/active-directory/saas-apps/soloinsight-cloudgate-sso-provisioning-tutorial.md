---
title: 教學課程：使用 Azure Active Directory 設定 Soloinsight-CloudGate SSO 來自動布建使用者Microsoft Docs
description: 瞭解如何設定 Azure Active Directory，以將使用者帳戶自動布建和取消布建至 Soloinsight-CloudGate SSO。
...
本教學課程的目的是要示範在 Soloinsight-CloudGate SSO 和 Azure Active Directory (Azure AD) 中執行的步驟，以將 Azure AD 設定為自動布建和解除布建使用者和/或群組至 Soloinsight-CloudGate SSO。
...
* Azure AD 租用戶
* [Soloinsight-CloudGate SSO 租使用者](https://www.soloinsight.com/)
* Soloinsight-CloudGate SSO 中具有系統管理員許可權的使用者帳戶。
...
在設定並啟用自動使用者布建之前，您應該決定 Azure AD 中的哪些使用者及/或群組需要存取 Soloinsight-CloudGate SSO。 一旦決定後，您可以遵循此處的指示，將這些使用者和/或群組指派給 Soloinsight-CloudGate SSO：
...
使用 Azure AD 設定 Soloinsight-CloudGate SSO 來自動布建使用者之前，您需要從 Azure AD 應用程式庫將 Soloinsight-CloudGate SSO 新增至受控 SaaS 應用程式清單。
...
## <a name="configuring-automatic-user-provisioning-to-soloinsight-cloudgate-sso"></a>設定自動使用者布建至 Soloinsight-CloudGate SSO 

本節將引導您逐步設定 Azure AD 布建服務，以根據 Azure AD 中的使用者和/或群組指派，在 Soloinsight-CloudGate SSO 中建立、更新及停用使用者和/或群組。

> [!TIP]
> 您也可以選擇啟用 Soloinsight-CloudGate SSO 的 SAML 型單一登入，請遵循 [>soloinsight-cloudgate-CLOUDGATE SSO 單一登入教學](https://docs.microsoft.com/azure/active-directory/saas-apps/soloinsight-cloudgate-sso-tutorial)課程中提供的指示。 您可以獨立設定自動使用者布建的單一登入，雖然這兩個功能彼此補充

### <a name="to-configure-automatic-user-provisioning-for-soloinsight-cloudgate-sso-in-azure-ad"></a>若要在 Azure AD 中設定 Soloinsight-CloudGate SSO 的自動使用者布建：
...
9. 在 [ **屬性對應** ] 區段中，檢查從 Azure AD 同步至 Soloinsight-CloudGate SSO 的使用者屬性。 選取為 [比對] 屬性 **的屬性會** 用來比對 Soloinsight-CloudGate SSO 中的使用者帳戶以進行更新作業。 選取 [儲存] 按鈕以認可所有變更。
...
11. 在 [ **屬性對應** ] 區段中，檢查從 Azure AD 同步至 Soloinsight-CloudGate SSO 的群組屬性。 選取為 [比對] 屬性 **的屬性會** 用來比對 Soloinsight-CloudGate SSO 中的群組以進行更新作業。 選取 [儲存] 按鈕以認可所有變更。
...
此作業會對在 [設定]**** 區段的 [範圍]**** 中定義的所有使用者和/或群組，啟動首次同步處理。 初始同步處理會比後續同步處理花費更多時間執行，只要 Azure AD 佈建服務正在執行，這大約每 40 分鐘便會發生一次。 您可以使用 [ **同步處理詳細資料** ] 區段來監視進度，並遵循連結來布建活動報告，該報告描述 Soloinsight-CloudGate SSO 上的 Azure AD 布建服務所執行的所有動作。
