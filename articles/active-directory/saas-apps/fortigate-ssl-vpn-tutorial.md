...
1. **[設定 FortiGate SSL VPN SSO](#configure-fortigate-ssl-vpn-sso)** - 在應用程式端設定單一登入設定。
    1. **[建立 FortiGate SSL VPN 測試使用者](#create-fortigate-ssl-vpn-test-user)** - 使 FortiGate SSL VPN 中具有與 Azure AD 中的 B.Simon 連結的相對應使用者。
...
若要在 **FortiGate SSL VPN** 端設定單一登入，請遵循[此](https://aka.ms/AA9avum)文件。

> [!NOTE]
> 如需設定 FortiGate SSL VPN 的詳細資訊，請參閱[此](https://docs.fortinet.com/document/fortigate/6.4.0/new-features/558169/saml-sp-for-vpn-authentication)連結。

### <a name="create-fortigate-ssl-vpn-test-user"></a>建立 FortiGate SSL VPN 測試使用者

在本節中，您會在 FortiGate SSL VPN 中建立名為 B.Simon 的使用者。 請與  [FortiGate SSL VPN 支援小組](mailto:tac_amer@fortinet.com)合作，在 FortiGate SSL VPN 平台中新增使用者。 您必須先建立和啟動使用者，然後才能使用單一登入。
