...
description: 瞭解如何設定 Azure Active Directory，以將使用者帳戶自動布建和取消布建至 Airstack。
...
ms.locfileid: "88542865"
---
# <a name="tutorial-configure-airstack-for-automatic-user-provisioning"></a>教學課程：設定 Airstack 來自動布建使用者

本教學課程的目的是要示範在 Airstack 中執行的步驟，以及 Azure Active Directory (Azure AD) 將 Azure AD 設定為自動布建和解除布建使用者和/或群組至 Airstack。
...
## <a name="setup-airstack-for-provisioning"></a>設定 Airstack 以進行布建
...
2.  在位於畫面左側的功能表上流覽至 **Azure** 設定。

    ![Airstack 新增 SCIM](media/airstack-provisioning-tutorial/azureconfig.png)

3.  按一下 [ **產生** ] 按鈕。 複製 **Azure 的秘密權杖**。 在 Azure 入口網站的 Airstack 應用程式的 [布建] 索引標籤中，將會在 [秘密權杖] 欄位中輸入此值。
...
使用 Azure AD 設定 Airstack 來自動布建使用者之前，您必須從 Azure AD 應用程式資源庫將 Airstack 新增至受控 SaaS 應用程式清單。
...
4. 在搜尋方塊中，輸入 **Airstack**，在 [結果] 面板中選取 [ **Airstack** ]，然後按一下 [ **新增** ] 按鈕以新增應用程式。
...
5. 在 [ **管理員認證** ] 區段下的 [ `https://api-as.lenovosoftware.com/0/as/common/scim` **租使用者 URL**] 中輸入。 輸入稍早在**秘密權杖**中取出的**SCIM Authentication 權杖**值。 按一下 [ **測試連接** ] 以確保 Azure AD 可以連線至 Airstack。 如果連接失敗，請確定您的 Airstack 帳戶具有系統管理員許可權，然後再試一次。
...
11. 若要啟用 Airstack Azure AD 的布建服務，請在 [**設定**] 區段中，將 [布建**狀態**] 變更為 [**開啟**]。
