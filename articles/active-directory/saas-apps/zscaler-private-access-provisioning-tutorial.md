...
> [!TIP]
> 您也可以選擇啟用 Zscaler Private Access (ZPA) 的 SAML 型單一登入，方法是遵循 [Zscaler 私用存取 (ZPA) 單一登入教學](https://docs.microsoft.com/azure/active-directory/saas-apps/zscalerprivateaccess-tutorial)課程中提供的指示。 您可以獨立設定自動使用者布建的單一登入，雖然這兩個功能彼此互補。
