---
title: 教學課程：Azure Active Directory 單一登入 (SSO) 與 Visibly 整合 | Microsoft Docs
description: 了解如何設定 Azure Active Directory 與 Visibly 之間的單一登入。
...
在本教學課程中，您會了解如何將 Visibly 與 Azure Active Directory (Azure AD) 進行整合。 在整合 Visibly 與 Azure AD 時，您可以︰

* 在 Azure AD 中控制可存取 Visibly 的人員。
...
若要設定將 Visibly 整合到 Azure AD 中，您需要從資源庫將 Visibly 新增到受控 SaaS 應用程式清單。
...
## <a name="configure-and-test-azure-ad-sso-for-visibly"></a>設定和測試 Visibly 的 Azure AD SSO

以名為 **B.Simon** 的測試使用者，設定及測試與 Visibly 搭配運作的 Azure AD SSO。 若要讓 SSO 能夠運作，您必須建立 Azure AD 使用者與 Visibly 中相關使用者之間的連結關聯性。

若要設定及測試與 Visibly 搭配運作的 Azure AD SSO，請完成下列建置組塊：

1. **[設定 Azure AD SSO](#configure-azure-ad-sso)** - 讓您的使用者能夠使用此功能。
    1. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 B.Simon 測試 Azure AD 單一登入。
    1. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 B.Simon 能夠使用 Azure AD 單一登入。
1. **[設定 Visibly SSO](#configure-visibly-sso)** - 在應用程式端設定單一登入設定。
    1. **[建立 Visibly 測試使用者](#create-visibly-test-user)** - 使 Visibly 中對應的 B.Simon 連結到該使用者在 Azure AD 中的代表項目。
...
1. 在 [基本 SAML 組態] 區段上，輸入下列欄位的值：
    ...
    a. 在 [登入 URL] 文字方塊中，輸入 URL：`https://app.visibly.io/`

    b. 在 [回覆 URL] 文字方塊中，輸入 URL：`https://api.visibly.io/api/v1/verifyResponse`


1. Visibly 應用程式需要特定格式的 SAML 判斷提示，需要您加入自訂屬性對應到您的 Visibly 權杖屬性設定。 以下螢幕擷取畫面顯示預設屬性清單。

    ![image](common/default-attributes.png)

1. 除了上述屬性外，Visibly 應用程式還需要在 SAML 回應中多傳回幾個屬性，如下所示。 這些屬性也會預先填入，但您可以根據您的需求來檢閱這些屬性。
...
1. 在 [設定 Visibly] 區段上，依據您的需求複製適當的 URL。
...
在本節中，您會將 Visibly 的存取權授與 B.Simon，讓其能夠使用 Azure 單一登入。

1. 在 Azure 入口網站中，選取 [企業應用程式]，然後選取 [所有應用程式]。
1. 在應用程式清單中，選取 [Visibly]。
...
## <a name="configure-visibly-sso"></a>設定 Visibly SSO
...
### <a name="create-visibly-test-user"></a>建立 Visibly 測試使用者

本節會在 Visibly 中建立名為 B.Simon 的使用者。 Visibly 支援預設會啟用的 Just-In-Time 使用者佈建。 在這一節沒有您需要進行的動作項目。 如果 Visibly 中還沒有任何使用者存在，在驗證之後就會建立新的使用者。
