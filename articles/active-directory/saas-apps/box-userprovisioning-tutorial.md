...
4. 將 [佈建模式]  設定為 [自動]  。 
    ...
    ![螢幕擷取畫面：Azure 入口網站中 Box 的 [佈建] 索引標籤。 [佈建模式] 已設定為 [自動]，並已在 [管理員認證] 中醒目提示 [授權]。](./media/box-userprovisioning-tutorial/provisioning.png)

5. 在 [管理員認證] 區段底下，按一下 [授權] 以在新的瀏覽器視窗中開啟 Box 登入對話方塊。

6. 在 [登入以授與 Box 存取權] 頁面上，提供必要的認證，然後按一下 [授權]。 
   
    ![[登入以授權 Box 存取權] 畫面的螢幕擷取畫面，其中顯示電子郵件和密碼的項目，以及 [授權] 按鈕。](./media/box-userprovisioning-tutorial/IC769546.png "啟用自動使用者佈建")

7. 按一下 [授與 Box 存取權]  ，以授權進行此作業並返回 Azure 入口網站。 
   
    ![Box 中 [授權存取] 畫面的螢幕擷取畫面，其中顯示說明訊息和 [授與 Box 存取權] 按鈕。](./media/box-userprovisioning-tutorial/IC769549.png "啟用自動使用者佈建")
