...
1. 在 [基本 SAML 組態]  區段上，輸入下列欄位的值：
    ...
    > [!NOTE]
    > 識別碼值不是實際值。 請使用實際的「識別碼」來更新此值。 請連絡 [Trend Micro Web Security (TMWS) 用戶端支援小組](https://success.trendmicro.com/contact-support-north-america)以取得識別碼值。 您也可以參考 Azure 入口網站中**基本 SAML 組態**區段所示的模式。
...
若要在 **Trend Micro Web Security (TMWS)** 端設定單一登入，您必須將從 Azure 入口網站下載的 [憑證 (Base64)]  和所複製的適當 URL 傳送給 [Trend Micro Web Security (TMWS) 支援小組](https://success.trendmicro.com/contact-support-north-america)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
