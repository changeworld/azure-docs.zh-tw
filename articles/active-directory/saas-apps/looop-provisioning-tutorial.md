...
description: 瞭解如何設定 Azure Active Directory，以將使用者帳戶自動布建和取消布建至 Looop。
...
ms.locfileid: "94356819"
---
# <a name="tutorial-configure-looop-for-automatic-user-provisioning"></a>教學課程：設定 Looop 來自動布建使用者

本教學課程的目的是要示範在 Looop 中執行的步驟，以及 Azure Active Directory (Azure AD) 將 Azure AD 設定為自動布建和解除布建使用者和/或群組至 Looop。
...
## <a name="set-up-looop-for-provisioning"></a>設定布建的 Looop

使用 Azure AD 設定 Looop 來自動布建使用者之前，您必須從 Looop 取得一些布建資訊。
...
2. 按一下 [ **SCIM 整合** ] 下的 [ **重設權杖** ]，以產生新的權杖。
    ...
    :::image type="content" source="media/looop-provisioning-tutorial/resettoken.png" alt-text="Looop 管理主控台頁面中 [S C I M 整合] 區段的螢幕擷取畫面。[重設權杖] 按鈕會反白顯示。" border="false":::
...
4. 在搜尋方塊中，輸入 **Looop** ，在 [結果] 面板中選取 [ **Looop** ]。 
...
6. 由於 Looop 是 OpenIDConnect 應用程式，請選擇使用您的 Microsoft 公司帳戶登入 Looop。

    ![Looop OIDC 登入](media/looop-provisioning-tutorial/msftlogin.png)

7. 驗證成功之後，請接受同意頁面的同意提示。 然後，應用程式會自動新增至您的租使用者，而系統會將您重新導向至您的 Looop 帳戶。
...
本節將引導您逐步設定 Azure AD 布建服務，以根據 Azure AD 中的使用者和/或群組指派，在 Looop 中建立、更新及停用使用者和/或群組。

### <a name="to-configure-automatic-user-provisioning-for-looop-in-azure-ad"></a>若要在 Azure AD 中為 Looop 設定自動使用者布建：
...
9. 在 [ **屬性對應** ] 區段中，檢查從 Azure AD 同步處理到 Looop 的使用者屬性。 選取為 [比對 **] 屬性的屬性會** 用來比對 Looop 中的使用者帳戶以進行更新作業。 選取 [儲存]  按鈕以認可所有變更。
...
11. 若要啟用 Looop Azure AD 的布建服務，請在 [ **設定** ] 區段中，將 [布建 **狀態** ] 變更為 [ **開啟** ]。
...
此作業會對在 [設定]  區段的 [範圍]  中定義的所有使用者和/或群組，啟動首次同步處理。 初始同步處理會比後續同步處理花費更多時間執行，只要 Azure AD 佈建服務正在執行，這大約每 40 分鐘便會發生一次。 您可以使用 [ **同步處理詳細資料** ] 區段來監視進度，並遵循連結來布建活動報告，該報告描述 Looop 上的 Azure AD 布建服務所執行的所有動作。
