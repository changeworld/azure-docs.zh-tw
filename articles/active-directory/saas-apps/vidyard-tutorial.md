...
2. 從 Vidyard 儀表板中，選取 [Group] \(群組\)   > [Security] \(安全性\) 
    ...
    ![螢幕擷取畫面：顯示已從 Vidyard Software 網站中的 [群組] 選取 [安全性]。](./media/vidyard-tutorial/configure1.png)
...
5. 從 [Single Sign On] \(單一登入\) 索引標籤中，選取現有設定檔旁邊的 [Assign] \(指派\) 
    ...
    ![螢幕擷取畫面：顯示 Azure AD SSO 設定檔的 [指派] 按鈕。](./media/vidyard-tutorial/configure4.png)
...
6. 選取您的組織群組，此群組會顯示在 [Groups Available to Assign] \(可指派的群組\)  中。
    ...
    ![螢幕擷取畫面：顯示 [將 SAML 設定指派給組織] 區段，您可以在其中選取群組。](./media/vidyard-tutorial/configure5.png)

7. 您可以在 [Groups Currently Assigned] \(目前指派的群組\)  底下看到所指派的群組。 根據您的組織選取群組的角色，然後按一下 [Confirm] \(確認\)  。

    ![螢幕擷取畫面：顯示 [將 SAML 設定指派給組織] 區段，您可以在其中選取角色。](./media/vidyard-tutorial/configure6.png)
...
當您在存取面板中按一下 [Vidyard] 圖格時，應該會自動登入您設定 SSO 的 Vidyard。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](../user-help/my-apps-portal-end-user-access.md)。
