---
title: 教學課程：使用 Azure Active Directory 設定 TeamViewer 來自動布建使用者 |Microsoft Docs
description: 瞭解如何將使用者帳戶從 Azure AD 自動布建和取消布建至 TeamViewer。
...
ms.locfileid: "91361546"
---
# <a name="tutorial-configure-teamviewer-for-automatic-user-provisioning"></a>教學課程：設定 TeamViewer 來自動布建使用者

本教學課程說明您需要在 TeamViewer 和 Azure Active Directory (Azure AD) 中執行的步驟，以設定自動使用者布建。 當設定時，Azure AD 會使用 Azure AD 布建服務，自動將使用者和群組布建並取消布建至 [TeamViewer](https://www.teamviewer.com/buy-now/) 。 如需此服務的用途、運作方式和常見問題等重要詳細資訊，請參閱[使用 Azure Active Directory 對 SaaS 應用程式自動佈建和取消佈建使用者](../manage-apps/user-provisioning.md)。 


## <a name="capabilities-supported"></a>支援的功能
> [!div class="checklist"]
> * 在 TeamViewer 中建立使用者
> * 當 TeamViewer 中的使用者不再需要存取權時，請將其移除
> * 在 Azure AD 與 TeamViewer 之間保持使用者屬性同步
> * 使用 TeamViewer ([單一登入](https://docs.microsoft.com/azure/active-directory/saas-apps/teamviewer-tutorial)) 
...
* 適用于 TeamViewer 的有效 [Tensor 授權](https://www.teamviewer.com/de/teamviewer-tensor/) 。
...
3. 判斷要 [在 Azure AD 與 TeamViewer 之間對應](https://docs.microsoft.com/azure/active-directory/manage-apps/customize-application-attributes)的資料。 

## <a name="step-2-configure-teamviewer-to-support-provisioning-with-azure-ad"></a>步驟 2： 設定 TeamViewer 以支援 Azure AD 的布建
...
2.  流覽至 [ **應用程式**]。 按一下 [ **建立腳本標記**]。

    ![TeamViewer 建立權杖](./media/teamviewer-provisioning-tutorial/createtoken.png)

3.  提供腳本權杖的名稱。 按一下 [ **儲存** ] 按鈕。

    ![TeamViewer 權杖名稱](./media/teamviewer-provisioning-tutorial/tokenname.png)

4. 複製 **權杖** ，然後按一下 **[確定]**。 在 Azure 入口網站中，會在您 TeamViewer 應用程式的 [ **秘密權杖** ] 欄位中輸入此值。

    ![TeamViewer 權杖](./media/teamviewer-provisioning-tutorial/token.png)

## <a name="step-3-add-teamviewer-from-the-azure-ad-application-gallery"></a>步驟 3： 從 Azure AD 應用程式資源庫新增 TeamViewer

從 Azure AD 應用程式資源庫新增 TeamViewer，以開始管理對 TeamViewer 的布建。 如果您先前已設定適用于 SSO 的 TeamViewer，您可以使用相同的應用程式。 不過，建議您在一開始測試整合時，建立個別的應用程式。 [在此](https://docs.microsoft.com/azure/active-directory/manage-apps/add-gallery-app)深入了解從資源庫新增應用程式。 
...
* 將使用者和群組指派給 TeamViewer 時，您必須選取 **預設存取**以外的角色。 具有預設存取角色的使用者會從佈建中排除，而且會在佈建記錄中被標示為沒有效率。 如果應用程式上唯一可用的角色是 [預設存取] 角色，您可以[更新應用程式資訊清單](https://docs.microsoft.com/azure/active-directory/develop/howto-add-app-roles-in-azure-ad-apps) \(部分機器翻譯\) 以新增其他角色。 
...
5. 在 [系統 **管理員認證** ] 區段的 `ttps://webapi.teamviewer.com/scim/v2`  [ **Tentant URL** ] 欄位中輸入，然後輸入稍早在 **秘密權杖**中建立的腳本權杖。 按一下 [ **測試連接** ] 以確保 Azure AD 可以連線至 TeamViewer。 如果連接失敗，請確定您的 TeamViewer 帳戶具有系統管理員許可權，然後再試一次。

    ![螢幕擷取畫面顯示 [管理認證] 對話方塊，您可以在其中輸入租使用者 U R L 和秘密權杖。](./media/teamViewer-provisioning-tutorial/provisioning.png)
...
9. 在 [ **屬性對應** ] 區段中，檢查從 Azure AD 同步處理至 TeamViewer 的使用者屬性。 選取為 [比對] 屬性 **的屬性會** 用來比對 TeamViewer 中的使用者帳戶，以進行更新作業。 如果您選擇變更相符的 [目標屬性](https://docs.microsoft.com/azure/active-directory/manage-apps/customize-application-attributes)，您將必須確定 TeamViewer API 支援根據該屬性篩選使用者。 選取 [儲存] 按鈕以認可所有變更。
...
11. 若要啟用 TeamViewer 的 Azure AD 布建服務，請在 [**設定**] 區段中，將布建**狀態**變更為 [**開啟**]。

    ![佈建狀態已切換為開啟](common/provisioning-toggle-on.png)

12. 在 [**設定**] 區段的 [**範圍**] 中選擇所需的值，以定義您想要布建至 TeamViewer 的使用者和/或群組。
