...
從 Azure AD 應用程式庫新增 TeamViewer，以開始管理對 TeamViewer 的布建。 如果您先前已設定適用于 SSO 的 TeamViewer，您可以使用相同的應用程式。 不過，建議您在一開始測試整合時，建立個別的應用程式。 [在此](https://docs.microsoft.com/azure/active-directory/manage-apps/add-gallery-app)深入了解從資源庫新增應用程式。 
