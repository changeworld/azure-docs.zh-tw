...
1.  若要取得所需的**秘密權杖**，請聯絡support@miro.comMiro 支援小組，網址為。 此值將會在 Azure 入口網站中 Miro 應用程式的 [布建] 索引標籤的 [秘密權杖] 欄位中輸入。
...
5. 在 [**管理員認證**] 區段下`https://miro.com/api/v1/scim` ，輸入 [**租使用者 URL**]。 輸入稍早在**秘密權杖**中所取得的**SCIM Authentication Token**值。 按一下 [**測試連接**] 以確保 Azure AD 可以連接到 Miro。 如果連線失敗，請確定您的 Miro 帳戶具有系統管理員許可權，然後再試一次。
