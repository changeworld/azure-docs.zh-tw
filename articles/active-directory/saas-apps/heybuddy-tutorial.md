...
1. 除了上述屬性外，EZOfficeInventory 應用程式還需要在 SAML 回應中多傳回幾個屬性，如下所示。 這些屬性也會預先填入，但您可以根據您的需求來檢閱這些屬性。
    ...
    > [!NOTE]
    > 請參閱此[連結](../develop/active-directory-enterprise-app-role-management.md)，以了解如何設定及安裝應用程式的角色。
...
當您在存取面板中按一下 [HeyBuddy] 圖格時，應該會自動登入您已設定 SSO 的 HeyBuddy。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](../user-help/my-apps-portal-end-user-access.md)。
