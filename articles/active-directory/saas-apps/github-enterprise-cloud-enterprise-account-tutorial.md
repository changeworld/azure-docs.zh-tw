...
* 在 Azure AD 中控制可存取 GitHub Enterprise Cloud - 企業帳戶的人員。
* 讓使用者使用其 Azure AD 帳戶自動登入 GitHub Enterprise Cloud - 企業帳戶。
...
* 已啟用 GitHub Enterprise Cloud - 企業帳戶單一登入 (SSO) 的訂用帳戶。
...
* 設定 GitHub Enterprise Cloud - 企業帳戶後，您可以強制執行工作階段控制項，以即時防止組織的敏感資料遭到外洩和滲透。 工作階段控制項會從條件式存取延伸。 [了解如何使用 Microsoft Cloud App Security 來強制執行工作階段控制項](https://docs.microsoft.com/cloud-app-security/proxy-deployment-any-app)。
...
1. **[設定 GitHub Enterprise Cloud - 企業帳戶 SSO](#configure-github-enterprise-cloud-enterprise-account-sso)** - 在應用程式端設定單一登入設定。
    1. **[建立 GitHub Enterprise Cloud - 企業帳戶測試使用者](#create-github-enterprise-cloud-enterprise-account-test-user)** - 使 GitHub Enterprise Cloud - 企業帳戶中對應的 B.Simon 連結到該使用者在 Azure AD 中的代表項目。
...
1. 如果您想要以 **SP** 起始模式設定應用程式，請按一下 [設定其他 URL]，然後執行下列步驟：
    ...
    > [!NOTE]
    > 這些都不是真正的值。 請使用實際的 [登入 URL]、[回覆 URL] 和 [識別碼] 來更新這些值。 請連絡 [GitHub Enterprise Cloud - 企業帳戶用戶端支援小組](mailto:support@github.com)來取得這些值。 您也可以參考 Azure 入口網站中**基本 SAML 組態**區段所示的模式。
...
在本節中，您會將 GitHub Enterprise Cloud - 企業帳戶 的存取權授與 B.Simon，讓其能夠使用 Azure 單一登入。
...
## <a name="configure-github-enterprise-cloud-enterprise-account-sso"></a>設定 GitHub Enterprise Cloud - 企業帳戶 SSO

若要設定 **GitHub Enterprise Cloud - 企業帳戶**端的單一登入，您必須將從 Azure 入口網站下載的 [憑證 (Base64)] 和複製的適當 URL 傳送給 [GitHub Enterprise Cloud - 企業帳戶支援小組](mailto:support@github.com)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。

### <a name="create-github-enterprise-cloud-enterprise-account-test-user"></a>建立 GitHub Enterprise Cloud - 企業帳戶使用者

本節會在 GitHub Enterprise Cloud - 企業帳戶中建立名為 B.Simon 的使用者。 GitHub Enterprise Cloud - 企業帳戶支援依預設啟用的 Just-In-Time 使用者佈建。 在這一節沒有您需要進行的動作項目。 如果 GitHub Enterprise Cloud - 企業帳戶中還沒有任何使用者存在，在驗證之後就會建立新的使用者。

## <a name="test-sso"></a>測試 SSO 

在本節中，您會使用存取面板來測試您的 Azure AD 單一登入設定。

當您在存取面板中按一下 [GitHub Enterprise Cloud - 企業帳戶] 圖格時，應該會自動登入您已設定 SSO 的 GitHub Enterprise Cloud - 企業帳戶。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
...
- [如何使用進階可見性和控制項保護 GitHub Enterprise Cloud - 企業帳戶](https://docs.microsoft.com/cloud-app-security/proxy-intro-aad)