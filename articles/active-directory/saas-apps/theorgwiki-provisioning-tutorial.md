...
4. 按一下 [ **+ 服務帳戶**]。 在 [**服務帳戶類型**] 底下，選取 [**權杖**型]。 按一下 [檔案] 。
...
5. 在 [**管理員認證**] 區段下，輸入 [ `https://<TheOrgWiki Subdomain        value>.theorgwiki.com/api/v2/scim/v2/` **租使用者 URL**]。 
...
您可以使用 [**目前狀態**] 區段來監視進度，並遵循 [布建活動報告] 的連結，其中描述 TheOrgWiki 上的 Azure AD 布建服務所執行的所有動作。 如需詳細資訊，請參閱[檢查使用者佈建的狀態](../app-provisioning/application-provisioning-when-will-provisioning-finish-specific-user.md)。 若要讀取 Azure AD 布建記錄，請參閱[關於自動使用者帳戶](../app-provisioning/check-status-user-account-provisioning.md)布建的報告。
