...
5. 在 [**管理員認證**] 區段下`https://scim.smartsheet.com/v2/` ，輸入 [**租使用者 URL**]。 輸入您先前從 Smartsheet 的 [**秘密權杖**] 中抓取並儲存的值。 按一下 [**測試連接**] 以確保 Azure AD 可以連接到 Smartsheet。 如果連線失敗，請確定您的 Smartsheet 帳戶具有系統管理員（SysAdmin）許可權，然後再試一次。
