...
1. 除了上述屬性外，ColorTokens ZTNA 應用程式還需要在 SAML 回應中多傳回幾個屬性，如下所示。 這些屬性也會預先填入，但您可以根據您的需求來檢閱這些屬性。
    ...
    > [!NOTE]
    > 請按一下[此處](../develop/active-directory-enterprise-app-role-management.md)來了解如何在 Azure AD 中建立角色。
...
當您在存取面板中按一下 [ColorTokens ZTNA] 圖格時，應該會自動登入您設定 SSO 的 ColorTokens ZTNA。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](../user-help/my-apps-portal-end-user-access.md)。
...
- [如何使用進階可見性和控制項保護 ColorTokens ZTNA](/cloud-app-security/proxy-intro-aad)