...
5. 在 [**管理員認證**] 區段下，于 [租使用者 URL] 中輸入 `https://api.federated.directory/v2/`。 輸入您先前從 [**秘密權杖**] 中的 [同盟目錄] 中抓取並儲存的值。 按一下 [**測試連接**] 以確保 Azure AD 可以連接到同盟目錄。 如果連線失敗，請確定您的同盟目錄帳戶具有系統管理員許可權，然後再試一次。
