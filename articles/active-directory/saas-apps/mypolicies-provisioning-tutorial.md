...
description: 瞭解如何設定 Azure Active Directory，以將使用者帳戶自動布建和取消布建至 myPolicies。
...
本教學課程的目的是要示範在 myPolicies 中執行的步驟，以及 Azure Active Directory (Azure AD) 將 Azure AD 設定為自動布建和解除布建使用者和/或群組至 myPolicies。
...
## <a name="setup-mypolicies-for-provisioning"></a>設定 myPolicies 以進行布建

使用 Azure AD 設定 myPolicies 來自動布建使用者之前，您必須在 myPolicies 上啟用 SCIM 布建。

1. 前往您的 myPolicies 代表 **support@mypolicies.com** ，以取得設定 SCIM 布建所需的秘密權杖。

2.  儲存 myPolicies 代表所提供的權杖值。 在 Azure 入口網站的 myPolicies 應用程式的 [布建] 索引標籤中，將會在 [ **秘密權杖** ] 欄位中輸入此值。
...
4. 在搜尋方塊中，輸入 **myPolicies**，在 [結果] 面板中選取 [ **myPolicies** ]，然後按一下 [ **新增** ] 按鈕以新增應用程式。
...
本節將引導您逐步設定 Azure AD 布建服務，以根據 Azure AD 中的使用者和/或群組指派，在 myPolicies 中建立、更新及停用使用者和/或群組。
...
### <a name="to-configure-automatic-user-provisioning-for-mypolicies-in-azure-ad"></a>若要在 Azure AD 中為 myPolicies 設定自動使用者布建：
...
6. 在 [ **秘密權杖**] 中，輸入稍早取出的權杖值。 按一下 [ **測試連接** ] 以確保 Azure AD 可以連線至 myPolicies。 如果連接失敗，請確定您的 myPolicies 帳戶具有系統管理員許可權，然後再試一次。
...
12. 若要啟用 myPolicies Azure AD 的布建服務，請在 [**設定**] 區段中，將 [布建**狀態**] 變更為 [**開啟**]。
...
此作業會對在 [設定]**** 區段的 [範圍]**** 中定義的所有使用者和/或群組，啟動首次同步處理。 初始同步處理會比後續同步處理花費更多時間執行，只要 Azure AD 佈建服務正在執行，這大約每 40 分鐘便會發生一次。 您可以使用 [ **同步處理詳細資料** ] 區段來監視進度，並遵循連結來布建活動報告，該報告描述 myPolicies 上的 Azure AD 布建服務所執行的所有動作。
