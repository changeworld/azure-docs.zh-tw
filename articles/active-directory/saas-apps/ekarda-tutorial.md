...
* 已啟用 Ekarda 單一登入 (SSO) 的訂用帳戶。

## <a name="scenario-description"></a>案例描述

在本教學課程中，您會在測試環境中設定和測試 Azure AD SSO。

* Ekarda 支援由 **SP 和 IDP** 起始的 SSO
* Ekarda 支援 **Just In Time** 使用者佈建

* 設定 Ekarda 後，您可以強制執行工作階段控制項，以即時防止組織的敏感資料遭到外洩和滲透。 工作階段控制項會從條件式存取延伸。 [了解如何使用 Microsoft Cloud App Security 來強制執行工作階段控制項](https://docs.microsoft.com/cloud-app-security/proxy-deployment-any-app)。

## <a name="adding-ekarda-from-the-gallery"></a>從資源庫新增 Ekarda

若要設定將 Ekarda 整合到 Azure AD 中，您需要從資源庫將 Ekarda 新增到受控 SaaS 應用程式清單。
...
以名為 **B.Simon** 的測試使用者，設定及測試與 Ekarda 搭配運作的 Azure AD SSO。 若要讓 SSO 能夠運作，您必須建立 Azure AD 使用者與 Ekarda 中相關使用者之間的連結關聯性。
...
1. **[設定 Ekarda SSO](#configure-ekarda-sso)** - 在應用程式端設定單一登入設定。
    1. **[建立 Ekarda 測試使用者](#create-ekarda-test-user)** - 使 Ekarda 中對應的 B.Simon 連結到該使用者在 Azure AD 中的代表項目。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Ekarda] 應用程式整合頁面上，尋找 [管理] 區段並選取 [單一登入]。
...
1. 如果您有**服務提供者中繼資料檔案**，請在 [基本 SAML 設定] 區段上執行下列步驟：
    ...
    c. 在成功上傳中繼資料檔案之後，會自動在 [Ekarda] 區段文字方塊中填入 [識別碼] 和 [回覆 URL] 值：
...
1. 在 [基本 SAML 設定] 區段上，如果您想要以 **IDP** 起始模式設定應用程式，請輸入下列欄位的值 (如果您沒有**服務提供者中繼資料檔案**的話)：
    ...
    a. 在 [識別碼] 文字方塊中，使用下列模式來輸入 URL：`https://my.ekarda.com/users/saml_metadata/<COMPANY_ID>`

    b. 在 [回覆 URL] 文字方塊中，使用下列模式來輸入 URL：`https://my.ekarda.com/users/saml_acs/<COMPANY_ID>`
...
1. 如果您想要以 **SP** 起始模式設定應用程式，請按一下 [設定其他 URL]，然後執行下列步驟：
    ...
    > [!NOTE]
    > 這些都不是真正的值。 請使用實際的「識別碼」、「回覆 URL」及「登入 URL」來更新這些值。 請連絡 [Ekarda 用戶端支援小組](mailto:contact@ekarda.com)以取得這些值。 您也可以參考 Azure 入口網站中**基本 SAML 組態**區段所示的模式。

1. 在 [以 SAML 設定單一登入] 頁面上的 [SAML 簽署憑證] 區段中，按一下複製按鈕以複製**憑證 (Base64)** ，並將其儲存在電腦上。

    ![憑證下載連結](common/certificatebase64.png)

1. 在 [設定 Ekarda] 區段上，根據您的需求複製適當的 URL。
...
1. 在下列頁面上，執行下列步驟：
    ...
    c. 在 [IDP 實體識別碼] 文字方塊中，貼上您從 Azure 入口網站複製的 [實體識別碼] 值。

    d. 在 [IDP 登入 URL] 文字方塊中，貼上您從 Azure 入口網站複製的 [登入 URL] 值。

    e. 在 [IDP 登出 URL] 文字方塊中，貼上您從 Azure 入口網站複製的 [登出 URL] 值。

    f. 從 Azure 入口網站將所下載的**憑證(Base64)** 以記事本開啟，然後將內容貼至 [IDP x509 憑證] 文字方塊。

    g. 在 [選項] 區段中，選取 [啟用 SLO]。

    h. 按一下 [更新]。

### <a name="create-ekarda-test-user"></a>建立 Ekarda 測試使用者

本節會在 Ekarda 中建立名為 B.Simon 的使用者。 Ekarda 支援依預設啟用的 Just-In-Time 使用者佈建。 在這一節沒有您需要進行的動作項目。 如果 Ekarda 中還沒有任何使用者存在，在驗證之後就會建立新的使用者。

## <a name="test-sso"></a>測試 SSO 

在本節中，您會使用存取面板來測試您的 Azure AD 單一登入設定。

當您在存取面板中按一下 [Ekarda] 圖格時，應該會自動登入您設定 SSO 的 Ekarda。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
