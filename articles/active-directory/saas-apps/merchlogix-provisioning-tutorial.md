...
description: 瞭解如何設定 Azure Active Directory 以自動布建和取消布建使用者帳戶至 MerchLogix。
...
本教學課程的目的是要示範要在 MerchLogix 和 Azure Active Directory （Azure AD）中執行的步驟，以設定 Azure AD 自動布建和取消布建使用者和/或群組至 MerchLogix。
...
* Azure AD 租用戶
* MerchLogix 租使用者
* MerchLogix 的技術連絡人，他可以提供 SCIM 端點 URL 和使用者布建所需的秘密權杖

## <a name="adding-merchlogix-from-the-gallery"></a>從資源庫新增 MerchLogix

將 MerchLogix 設定為使用 Azure AD 自動布建使用者之前，您需要從 Azure AD 應用程式庫將 MerchLogix 新增至受控 SaaS 應用程式清單。
...
5. 在結果面板中，選取 [ **MerchLogix**]，然後按一下 [**新增**] 按鈕，將 MerchLogix 新增至您的 SaaS 應用程式清單。
...
本節將引導您逐步設定 Azure AD 布建服務，以根據 Azure AD 中的使用者和/或群組指派，在 MerchLogix 中建立、更新和停用使用者和/或群組。
...
### <a name="to-configure-automatic-user-provisioning-for-merchlogix-in-azure-ad"></a>若要在 Azure AD 中設定 MerchLogix 的自動使用者布建：
...
5. 在 [**管理員認證**] 區段底下：
    ...
    * 在 [**秘密權杖**] 欄位中，輸入您的 MerchLogix 技術連絡人所提供的秘密權杖。

6. 填入步驟5所示的欄位之後，按一下 [**測試連接**] 以確保 Azure AD 可以連接到 MerchLogix。 如果連線失敗，請確定您的 MerchLogix 帳戶具有系統管理員許可權，然後再試一次。
