---
title: 教學課程：設定 Netskope 系統管理員主控台以 Azure Active Directory 的自動使用者布建 |Microsoft Docs
description: 瞭解如何設定 Azure Active Directory，以將使用者帳戶自動布建和解除布建到 Netskope 系統管理員主控台。
...
ms.locfileid: "88554544"
---
# <a name="tutorial-configure-netskope-administrator-console-for-automatic-user-provisioning"></a>教學課程：設定 Netskope 系統管理員主控台來自動布建使用者

本教學課程的目的是要示範在 Netskope 系統管理員主控台中執行的步驟，並 Azure Active Directory (Azure AD) ，將 Azure AD 設定為自動布建和解除布建使用者和/或群組至 Netskope 系統管理員主控台。
...
* Netskope 系統管理員主控台中具有系統管理員許可權的使用者帳戶。
...
* 將使用者指派給 Netskope 系統管理員主控台時，您必須在 [指派] 對話方塊中選取任何有效的應用程式特定角色 (可用的) 。 具有**預設存取**角色的使用者會從佈建中排除。

## <a name="set-up-netskope-administrator-console-for-provisioning"></a>設定 Netskope 系統管理員主控台以進行布建

1. 登入您的 [Netskope 系統管理員主控台管理主控台](https://netskope.goskope.com/)。 流覽至 **Home > 設定**。
...
3.  複製 **SCIM 伺服器 URL** 和 **權杖**。 這些值將會分別輸入至 [租使用者 URL] 和 [秘密權杖] 欄位中 Netskope 系統管理員主控台應用程式的 [布建] 索引標籤中 Azure 入口網站。
...
在設定 Netskope 系統管理員主控台以 Azure AD 自動布建使用者之前，您必須從 Azure AD 應用程式資源庫將 Netskope 系統管理員主控台新增至受控 SaaS 應用程式清單。
...
4. 在搜尋方塊中，輸入 **Netskope 系統管理員主控台**，在 [結果] 面板中選取 [ **Netskope] 系統管理員主控台** ，然後按一下 [ **新增** ] 按鈕以新增應用程式。
...
本節將引導您逐步設定 Azure AD 布建服務，以根據 Azure AD 中的使用者和/或群組指派，在 Netskope 系統管理員主控台中建立、更新及停用使用者和/或群組。

> [!TIP]
> 您也可以遵循 [Netskope 系統管理員主控台單一登入教學](https://docs.microsoft.com/azure/active-directory/saas-apps/netskope-cloud-security-tutorial)課程中提供的指示，選擇啟用 Netskope 系統管理員主控台的 SAML 型單一登入。 您可以獨立設定自動使用者布建的單一登入，雖然這兩個功能彼此互補。
...
5. 在 [系統**管理員認證**] 區段下，于 [**租使用者 url**] 中稍早抓取的輸入**SCIM 伺服器 URL**值。 輸入稍早在**秘密權杖**中取出的**權杖**值。 按一下 [ **測試連接** ] 以確保 Azure AD 可以連線到 Netskope 系統管理員主控台。 如果連接失敗，請確定您的 Netskope 系統管理員主控台帳戶具有系統管理員許可權，然後再試一次。
...
10. **在 [對應**] 區段下，選取 [**同步處理 Azure Active Directory 群組] 以 Netskope 系統管理員主控台**。
...
此作業會對在 [設定]**** 區段的 [範圍]**** 中定義的所有使用者和/或群組，啟動首次同步處理。 初始同步處理會比後續同步處理花費更多時間執行，只要 Azure AD 佈建服務正在執行，這大約每 40 分鐘便會發生一次。 您可以使用 [ **同步處理詳細資料** ] 區段來監視進度，並遵循連結來布建活動報告，該報告描述 Netskope 系統管理員主控臺上 Azure AD 布建服務所執行的所有動作。
