---
title: 教學課程：Azure Active Directory 單一登入 (SSO) 與 Segment 整合 | Microsoft Docs
description: 了解如何設定 Azure Active Directory 與 Segment 之間的單一登入。
...
在本教學課程中，您將了解如何整合 Segment 與 Azure Active Directory (Azure AD)。 在整合 Segment 與 Azure AD 後，您可以︰

* 在 Azure AD 中控制可存取 Segment 的人員。
...
若要設定將 Segment 整合到 Azure AD 中，您需要從資源庫將 Segment 新增到受控 SaaS 應用程式清單。
...
## <a name="configure-and-test-azure-ad-single-sign-on-for-segment"></a>設定及測試 Segment 的 Azure AD 單一登入

以名為 **B. Simon** 的測試使用者，設定及測試與 Segment 搭配運作的 Azure AD SSO。 若要讓 SSO 能夠運作，您必須建立 Azure AD 使用者與 Segment 中相關使用者之間的連結關聯性。

若要設定及測試與 Segment 搭配運作的 Azure AD SSO，請完成下列建構元素：

1. **[設定 Azure AD SSO](#configure-azure-ad-sso)** - 讓您的使用者能夠使用此功能。
    1. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 B.Simon 測試 Azure AD 單一登入。
    1. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 B.Simon 能夠使用 Azure AD 單一登入。
1. **[設定 Segment SSO](#configure-segment-sso)** - 在應用程式端設定單一登入設定。
    1. **[建立 Segment 測試使用者](#create-segment-test-user)** - 使 Segment 中對應的 B.Simon 連結到該使用者在 Azure AD 中的代表項目。
...
1. 在 [基本 SAML 設定] 區段上，如果您想要以 **IDP** 起始模式設定應用程式，請輸入下列欄位的值：
    ...
    a. 在 [識別碼] 文字方塊中，使用下列模式來輸入 URL：`urn:auth0:segment-prod:samlp-<CUSTOMER_VALUE>`

    b. 在 [回覆 URL] 文字方塊中，使用下列模式來輸入 URL：`https://segment-prod.auth0.com/login/callback?connection=<CUSTOMER_VALUE>`

1. 如果您想要以 **SP** 起始模式設定應用程式，請按一下 [設定其他 URL]，然後執行下列步驟：

    在 [登入 URL] 文字方塊中，輸入 URL：`https://app.segment.com`
...
1. 在 [設定 Segment] 區段上，根據您的需求複製適當的 URL。
...
在本節中，您會將 Segment 的存取權授與 B.Simon，讓其能夠使用 Azure 單一登入。

1. 在 Azure 入口網站中，選取 [企業應用程式]，然後選取 [所有應用程式]。
1. 在應用程式清單中，選取 [Segment]。
...
## <a name="configure-segment-sso"></a>設定 Segment SSO

1. 在新的網頁瀏覽器視窗中，以系統管理員身分登入您的 Segment 公司網站。
...
1. 在下列頁面上，執行下列步驟：
    ...
    b. 複製 [對象 URL] 值，並將其貼至 Azure 入口網站中 [基本 SAML 設定] 對話方塊中的 [識別碼 URL] 方塊內。
...
### <a name="create-segment-test-user"></a>建立 Segment 測試使用者

本節會在 Segment 中建立名為 B.Simon 的使用者。 Segment 支援依預設啟用的 Just-In-Time 使用者佈建。 在這一節沒有您需要進行的動作項目。 如果 Segment 中還沒有任何使用者存在，在驗證之後就會建立新的使用者。
