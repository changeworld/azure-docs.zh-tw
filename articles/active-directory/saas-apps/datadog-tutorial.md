...
在本教學課程中，您將了解如何整合 Datadog 與 Azure Active Directory (Azure AD)。 在整合 Datadog 與 Azure AD 時，您可以︰

* 在 Azure AD 中控制可存取 Datadog 的人員。
* 讓使用者使用其 Azure AD 帳戶自動登入 Datadog。
...
* 已啟用 Datadog 單一登入 (SSO) 的訂用帳戶。

## <a name="scenario-description"></a>案例描述

在本教學課程中，您會在測試環境中設定和測試 Azure AD SSO。

* Datadog 支援由 **SP 和 IDP** 起始的 SSO
* 設定 Datadog 後，您可以強制執行工作階段控制項，以即時防止組織的敏感資料遭到外洩和滲透。 工作階段控制項會從條件式存取延伸。 [了解如何使用 Microsoft Cloud App Security 來強制執行工作階段控制項](/cloud-app-security/proxy-deployment-aad)


## <a name="adding-datadog-from-the-gallery"></a>從資源庫新增 Datadog

若要進行將 Datadog 整合到 Azure AD 中的設定，您必須從資源庫將 Datadog 新增至受控 SaaS app 清單。

1. 使用公司或學校帳戶或個人的 Microsoft 帳戶登入 [Azure 入口網站](https://portal.azure.com)。
1. 在左方瀏覽窗格上，選取 [Azure Active Directory]  服務。
1. 巡覽至 [企業應用程式]  ，然後選取 [所有應用程式]  。
1. 若要新增應用程式，請選取 [新增應用程式]  。
1. 在 [從資源庫新增]  區段的搜尋方塊中，輸入 **Datadog** 。
...
若要設定及測試與 Datadog 搭配運作的 Azure AD SSO，請完成下列建置組塊：

1. **[設定 Azure AD SSO](#configure-azure-ad-sso)** - 讓您的使用者能夠使用此功能。
    1. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 B.Simon 測試 Azure AD 單一登入。
    1. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 B.Simon 能夠使用 Azure AD 單一登入。
1. **[設定 Datadog SSO](#configure-datadog-sso)** - 在應用程式端設定單一登入設定。
    1. **[建立 Datadog 測試使用者](#create-datadog-test-user)** - 使 Datadog 中對應的 B.Simon 連結到該使用者在 Azure AD 中的代表項目。
...
1. 在 [設定 Datadog]  區段上，根據您的需求複製適當的 URL。
...
在本節中，您會將 Datadog 的存取權授與 B.Simon，讓其能夠使用 Azure 單一登入。
...
當您在存取面板中按一下 [Datadog] 圖格時，應該會自動登入您已設定 SSO 的 Datadog。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](../user-help/my-apps-portal-end-user-access.md)。
...
- [嘗試搭配 Azure AD 使用 Datadog](https://aad.portal.azure.com/)
