...
ms.locfileid: "92456131"
---
# <a name="tutorial-configure-cinode-for-automatic-user-provisioning"></a>教學課程：設定 Cinode 來自動布建使用者

本教學課程說明在 Cinode 和 Azure Active Directory (Azure AD) 設定自動使用者布建時所需執行的步驟。 當設定時，Azure AD 會使用 Azure AD 布建服務，自動將使用者和群組布建和取消布建至 [Cinode](https://cinode.com/) 。 如需此服務的用途、運作方式和常見問題等重要詳細資訊，請參閱[使用 Azure Active Directory 對 SaaS 應用程式自動佈建和取消佈建使用者](../app-provisioning/user-provisioning.md)。 
...
3. 判斷要 [在 Azure AD 與 Cinode 之間對應](../app-provisioning/customize-application-attributes.md)的資料。
...
從 Azure AD 應用程式資源庫新增 Cinode，以開始管理布建至 Cinode。 如果您先前已設定 SSO 的 Cinode，您可以使用相同的應用程式。 不過，建議您在一開始測試整合時，建立個別的應用程式。 [在此](../manage-apps/add-application-portal.md)深入了解從資源庫新增應用程式。 
...
9. 在 [ **屬性對應** ] 區段中，檢查從 Azure AD 同步處理到 Cinode 的使用者屬性。 選取為 [比對 **] 屬性的屬性會** 用來比對 Cinode 中的使用者帳戶以進行更新作業。 如果您選擇變更相符的 [目標屬性](../app-provisioning/customize-application-attributes.md)，您將必須確定 Cinode API 支援根據該屬性篩選使用者。 選取 [儲存] 按鈕以認可所有變更。
