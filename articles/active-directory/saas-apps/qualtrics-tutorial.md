...
* 已啟用 SAP Qualtrics 單一登入 (SSO) 的訂用帳戶。
...
* 設定 SAP Qualtrics 後，您可以強制執行工作階段控制項，以即時防止組織的敏感資料遭到外洩和滲透。 工作階段控制項會從條件式存取延伸。 [了解如何使用 Microsoft Cloud App Security 來強制執行工作階段控制項](https://docs.microsoft.com/cloud-app-security/proxy-deployment-any-app)。

## <a name="adding-sap-qualtrics-from-the-gallery"></a>從資源庫新增 SAP Qualtrics
...
1. 從結果面板選取 [SAP Qualtrics]  ，然後新增應用程式。 當應用程式新增至您的租用戶時，請等候幾秒鐘。
...
1. **[設定 SAP Qualtrics SSO](#configure-sap-qualtrics-sso)** - 在應用程式端設定單一登入設定。
    1. **[建立 SAP Qualtrics 測試使用者](#create-sap-qualtrics-test-user)** - 在 SAP Qualtrics 中建立 B.Simon 的對應項目，此項目須與 Azure AD 中代表該使用者的項目連結。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [SAP Qualtrics]  應用程式整合頁面上，尋找 [管理]  區段並選取 [單一登入]  。
...
1. 如果您想要以 **SP** 起始模式設定應用程式，請按一下 [設定其他 URL]  ，然後執行下列步驟：
    ...
    > [!NOTE]
    > 這些都不是真正的值。 使用實際的登入 URL、識別碼、回覆 URL 和轉送狀態來更新這些值。 請連絡 [Qualtrics 用戶端支援小組](https://www.qualtrics.com/support/) \(英文\) 以取得這些值。 您也可以參考 Azure 入口網站中**基本 SAML 組態**區段所示的模式。
...
若要在 **SAP Qualtrics** 端設定單一登入，您必須將從 Azure 入口網站複製的 [應用程式同盟中繼資料 URL]  傳送給 [SAP Qualtrics 支援小組](https://www.qualtrics.com/support/)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。

### <a name="create-sap-qualtrics-test-user"></a>建立 SAP Qualtrics 測試使用者

本節會在 SAP Qualtrics 中建立名為 Britta Simon 的使用者。 SAP Qualtrics 支援依預設啟用的 Just-In-Time 使用者佈建。 在這一節沒有您需要進行的動作項目。 如果 SAP Qualtrics 中還沒有任何使用者存在，在驗證之後就會建立新的使用者。

## <a name="test-sso"></a>測試 SSO 

在本節中，您會使用存取面板來測試您的 Azure AD 單一登入設定。

當您在存取面板中按一下 [SAP Qualtrics] 圖格時，應該會自動登入您已設定 SSO 的 SAP Qualtrics。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
...
- [如何使用進階可見性和控制項保護 SAP Qualtrics](https://docs.microsoft.com/cloud-app-security/proxy-intro-aad)
