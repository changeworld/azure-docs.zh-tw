...
5. 在 [**管理員認證**] 區段下，輸入 [ `https://<RFPIO tenant instance>.rfpio.com/rfpserver/scim/v2 ` **租使用者 URL**]。 範例值為 `https://Azure-test1.rfpio.com/rfpserver/scim/v2` 。 輸入稍早在**秘密權杖**中所取得的**SCIM API Token**值。 按一下 [**測試連接**] 以確保 Azure AD 可以連接到 RFPIO。 如果連線失敗，請確定您的 RFPIO 帳戶具有系統管理員許可權，然後再試一次。
