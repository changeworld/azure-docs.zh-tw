...
1. 除了上述屬性外，Dotcom-Monitor 應用程式還需要在 SAML 回應中多傳回幾個屬性，如下所示。 這些屬性也會預先填入，但您可以根據您的需求來檢閱這些屬性。
    ...
    > [!NOTE]
    > 您可以在[這裡](../develop/active-directory-enterprise-app-role-management.md) \(機器翻譯\) 找到更多如何在 Azure AD 中建立自訂角色的指導方針。
...
當您在存取面板中按一下 [Dotcom-Monitor] 圖格時，應該會自動登入您已設定 SSO 的 Dotcom-Monitor。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](../user-help/my-apps-portal-end-user-access.md)。
