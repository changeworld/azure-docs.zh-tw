...
在 Azure 入口網站中建立安全性群組或使用 Azure AD PowerShell 時，只有群組的擁有者可以更新成員資格。 「[存取面板](https://account.activedirectory.windowsazure.com/r#/joinGroups)」中的自助式所建立的安全性群組和所有 Office 365 群組都可以加入所有使用者，無論是擁有者核准或自動核准。 在 [存取面板] 中，您可以在建立群組時變更成員資格選項。
...
1. 如果您將**使用者設定為可在 azure 入口網站中建立安全性群組**，或**使用者可以在 azure 入口網站中建立 Office 365 群組**來
...
> [!NOTE]
> 使用者必須要有 Azure Active Directory Premium （P1 或 P2）授權，才能要求加入安全性群組或 Office 365 群組，並讓擁有者核准或拒絕成員資格要求。 若沒有 Azure Active Directory Premium 授權，使用者仍然可以在存取面板中管理其群組，但他們無法在存取面板中建立需要擁有者核准的群組，而且他們無法要求加入群組。 
