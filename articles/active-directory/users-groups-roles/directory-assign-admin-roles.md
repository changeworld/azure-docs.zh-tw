---
title: 管理員角色描述和許可權 - Azure AD |微軟文檔
...
ms.locfileid: "80346954"
---
# <a name="administrator-role-permissions-in-azure-active-directory"></a>Azure Active Directory 中的系統管理員角色權限

使用 Azure 活動目錄 （Azure AD），可以指定有限的管理員以較低許可權的角色管理標識任務。 可以為管理員分配，以便添加或更改使用者、分配管理角色、重置使用者密碼、管理使用授權和管理功能變數名稱。 只能在 Azure AD 中的使用者設定中變更預設使用者權限。

## <a name="limit-the-use-of-global-administrator"></a>限制全域管理員的使用

分配給全域管理員角色的使用者可以讀取和修改 Azure AD 組織中的每個管理設置。 預設情況下，註冊 Azure 訂閱的人員將為其為 Azure AD 組織分配全域管理員角色。 只有全域管理員和特權角色管理員才能委派管理員角色。 為了降低企業的風險，我們建議您將此角色指派給組織中可能最少的人員。

作為最佳實踐，我們建議您將此角色指派給組織中少於五個人。 如果您的組織中已分配了五個以上的管理員到全域管理員角色，以下是減少其使用的一些方法。

### <a name="find-the-role-you-need"></a>查找所需的角色

如果從多個角色清單中找到所需的角色令人沮喪，Azure AD 可以基於角色類別顯示角色子集。 查看[Azure AD 角色和管理員](https://portal.azure.com/#blade/Microsoft_AAD_IAM/ActiveDirectoryMenuBlade/RolesAndAdministrators)的新**類型**篩選器，僅顯示所選類型中的角色。

### <a name="a-role-exists-now-that-didnt-exist-when-you-assigned-the-global-administrator-role"></a>現在存在的角色在分配全域管理員角色時不存在

可能將角色或角色添加到 Azure AD 中，該角色或角色提供了更精細的許可權，而當您將某些使用者提升到全域管理員時，這些許可權不是選項。 隨著時間的推移，我們將推出其他角色，以完成隻有全域管理員角色才能完成的任務。 您可以看到這些反映在以下[可用角色中](#available-roles)。
...
應用程式管理員可以管理應用程式憑據，以允許他們類比應用程式。 因此，分配給此角色的使用者可以僅管理未分配給任何 Azure AD 角色的應用程式或僅分配給以下管理員角色的應用程式的應用程式憑據：
* 應用程式系統管理員
* 應用程式開發人員
* 雲端應用程式系統管理員
* 目錄讀取器
...
此角色還授予_同意_委派許可權和應用程式許可權的許可權的許可權的許可權，但 Microsoft 圖形 API 上的許可權除外。

> [!IMPORTANT]
> 此例外情況意味著您仍然可以同意_其他_應用（例如已註冊的協力廠商應用或應用）的許可權，但不能同意對 Azure AD 本身的許可權。 您仍然可以在應用註冊時_請求_這些許可權，但_授予_（即同意）這些許可權需要 Azure AD 管理員。這意味著惡意使用者無法輕鬆提升其許可權，例如，通過創建和同意可以寫入整個目錄的應用，並通過該應用的許可權提升自己，成為全域管理員。

### <a name="application-developer"></a>[應用程式開發人員](#application-developer-permissions)

將「使用者可以註冊應用程式」設定設為「否」時，此角色中的使用者可以建立應用程式註冊。 當"使用者可以同意代表他們訪問公司資料的應用"設置設置為"否"時，此角色還授予代表自己同意的許可權。 創建新應用程式註冊或企業應用程式時，分配給此角色的使用者將作為擁有者添加。

### <a name="authentication-administrator"></a>[身份驗證管理員](#authentication-administrator-permissions)

此角色的使用者可以設置或重置非密碼憑據、更新密碼、要求根據現有非密碼憑據（例如 MFA 或 FIDO）重新註冊，並撤銷設備上記住非管理員或僅分配以下角色的使用者**的 MFA：：**
* 驗證系統管理員
* 目錄讀取器
* 來賓邀請者
* 訊息中心讀取者
...
具有此角色的使用者可以在 Azure 門戶中創建和管理 B2C 使用者流（也稱為"內置"策略）。通過創建或編輯使用者流，這些使用者可以更改使用者體驗的 html/CSS/javascript 內容、更改每個使用者流的 MFA 要求、更改權杖中的聲明以及調整租戶中所有策略的會話設置。 另一方面，此角色不包括查看使用者資料或更改租戶架構中包含的屬性的能力。對身份體驗框架（也稱為自訂）策略的更改也不屬於此角色的範圍。

### <a name="b2c-user-flow-attribute-administrator"></a>[B2C 使用者流屬性管理員](#b2c-user-flow-attribute-administrator-permissions)

具有此角色的使用者添加或刪除租戶中所有使用者流可用的自訂屬性。因此，具有此角色的使用者可以更改或向最終使用者架構添加新元素，並影響所有使用者流的行為，並間接導致最終使用者可能要求哪些資料更改，並最終作為聲明發送到應用程式。此角色無法編輯使用者流。

### <a name="b2c-ief-keyset-administrator"></a>[B2C IEF 金鑰集管理員](#b2c-ief-keyset-administrator-permissions)

使用者可以創建和管理權杖加密、權杖簽名和聲明加密/解密的策略金鑰和機密。通過向現有金鑰容器添加新金鑰，此受限管理員可以根據需要滾動機密，而不會影響現有應用程式。此使用者可以查看這些機密的全部內容及其到期日期，即使在創建之後也是如此。

> [!IMPORTANT]
> 這是一個敏感的角色。在預生產和生產期間，應仔細審核和分配金鑰集管理員角色。

### <a name="b2c-ief-policy-administrator"></a>[B2C IEF 策略管理員](#b2c-ief-policy-administrator-permissions)

此角色中的使用者可以創建、讀取、更新和刪除 Azure AD B2C 中的所有自訂策略，因此可以完全控制相關 Azure AD B2C 租戶中的標識體驗框架。 通過編輯策略，此使用者可以與外部標識提供程式建立直接聯合，更改目錄架構，更改面向使用者的的所有內容（HTML、CSS、JavaScript）、更改要求以完成身份驗證、創建新使用者、發送使用者資料到外部系統，包括完全遷移，並編輯所有使用者資訊，包括敏感欄位，如密碼和電話號碼。 相反，此角色無法更改加密金鑰或編輯用於租戶中聯合使用的秘密。

> [!IMPORTANT]
> B2 IEF 策略管理員是一個高度敏感的角色，應在非常有限的基礎上為生產中的租戶分配。應仔細審核這些使用者的活動，尤其是對於生產中的租戶。
...
雲應用程式管理員可以管理應用程式憑據，允許他們類比應用程式。 因此，分配給此角色的使用者可以僅管理未分配給任何 Azure AD 角色的應用程式或僅分配給以下管理員角色的應用程式的應用程式憑據：
* 應用程式開發人員
* 雲端應用程式系統管理員
* 目錄讀取器
...
### <a name="cloud-device-administrator"></a>[雲裝置管理員](#cloud-device-administrator-permissions)
...
具備此角色的使用者有權限管理 Microsoft 365 合規性中心、Microsoft 365 系統管理中心、Azure 和 Office 365 安全性與合規性中心中的合規性相關功能。 受讓人還可以為 Azure 和 Microsoft 365 創建支援票證。 如需詳細資訊，請參閱[關於 Office 365 管理員角色](https://support.office.com/article/About-Office-365-admin-roles-da585eea-f576-4f55-a1e0-87090b6aaa9d)。
...
[Office 365 安全性與合規性中心](https://support.office.com/article/About-Office-365-admin-roles-da585eea-f576-4f55-a1e0-87090b6aaa9d) | 管理資料治理<br>執行法律和資料的調查<br>管理資料主體要求<br><br>此角色具有與 Office 365 安全&合規性中心基於角色的存取控制中的[合規性管理員角色組](https://docs.microsoft.com/microsoft-365/security/office-365-security/permissions-in-the-security-and-compliance-center#permissions-needed-to-use-features-in-the-security--compliance-center)相同的許可權。
...
具有此角色的使用者具有在 Microsoft 365 合規性中心、Microsoft 365 管理中心和 Azure 中跟蹤資料的許可權。 使用者還可以跟蹤 Exchange 管理中心、合規性管理器和 Skype 業務管理中心&團隊內的合規性資料，並為 Azure 和 Microsoft 365 創建支援票證。

在 | 可以執行
----- | ----------
[Microsoft 365 合規性中心](https://protection.office.com) | 監控 Microsoft 365 服務中與合規性相關的策略<br>管理合規性警示
[合規性管理員](https://docs.microsoft.com/office365/securitycompliance/meet-data-protection-and-regulatory-reqs-using-microsoft-cloud) | 追蹤、指派和確認您組織的法規合規性活動
[Office 365 安全性與合規性中心](https://support.office.com/article/About-Office-365-admin-roles-da585eea-f576-4f55-a1e0-87090b6aaa9d) | 管理資料治理<br>執行法律和資料的調查<br>管理資料主體要求<br><br>此角色具有與 Office 365 安全&合規性中心基於角色的存取控制中的[合規性資料管理員角色組](https://docs.microsoft.com/microsoft-365/security/office-365-security/permissions-in-the-security-and-compliance-center#permissions-needed-to-use-features-in-the-security--compliance-center)相同的許可權。
[Intune](https://docs.microsoft.com/intune/role-based-access-control) | 檢視所有的 Intune 稽核資料
[Cloud App Security](https://docs.microsoft.com/cloud-app-security/manage-admins) | 具有唯讀權限，並可管理警示<br>可建立和修改檔案原則，並允許檔案治理動作<br>可檢視 [資料管理] 下的所有內建報告

### <a name="conditional-access-administrator"></a>[條件訪問管理員](#conditional-access-administrator-permissions)

具有此角色的使用者能夠管理 Azure 活動目錄條件訪問設置。
> [!NOTE]
> 要在 Azure 中部署 Exchange ActiveSync 條件訪問策略，使用者還必須是全域管理員。

### <a name="customer-lockbox-access-approver"></a>[客戶密碼箱訪問審批人](#customer-lockbox-access-approver-permissions)
...
此角色中的使用者可以管理桌面分析和辦公室自訂&策略服務。 對於桌面分析，這包括查看資產清單、創建部署計畫、查看部署和運行狀況。 對於 Office 自訂&策略服務，此角色使使用者能夠管理 Office 策略。
...
* 授予一組特定的來賓使用者讀取存取許可權，而不是將其授予所有來賓使用者。
* 將"僅限制對 Azure AD 門戶的訪問限制為管理員"設置為"是"時，授予一組特定的非管理員使用者訪問 Azure 門戶的許可權。
* 授予服務主體對目錄的存取權限，其中目錄.Read.All 不是一個選項。
...
> [!NOTE]
> 在 Microsoft 圖形 API 和 Azure AD PowerShell 中，此角色標識為"Exchange 服務管理員"。 它是[Azure 門戶](https://portal.azure.com)中的"交換管理員"。 它是[交換管理中心](https://go.microsoft.com/fwlink/p/?LinkID=529144)中的"交換線上管理員"。

### <a name="external-identity-provider-administrator"></a>[外部標識提供程式管理員](#external-identity-provider-administrator-permissions)

此管理員管理 Azure 活動目錄租戶和外部標識提供程式之間的聯合。通過此角色，使用者可以添加新標識提供程式並配置所有可用設置（例如身份驗證路徑、服務 ID、分配的金鑰容器）。此使用者可以使租戶信任來自外部標識提供程式的身份驗證。由此產生的對最終使用者體驗的影響取決於租戶的類型：

* Azure 活動目錄租戶的員工和合作夥伴：添加聯合（例如使用 Gmail）將立即影響尚未兌換的所有來賓邀請。 請參閱[添加 Google 作為 B2B 來賓使用者的身份供應商](https://docs.microsoft.com/azure/active-directory/b2b/google-federation)。
* Azure 活動目錄 B2C 租戶：添加聯合（例如，使用 Facebook 或其他 Azure AD 組織）不會立即影響最終使用者流，直到標識提供程式作為使用者流中的選項添加（也稱為內置政策）。 有關示例[，請參閱將 Microsoft 帳戶配置為標識提供程式](https://docs.microsoft.com/azure/active-directory-b2c/active-directory-b2c-setup-msa-app)。要更改使用者流，需要"B2C 使用者流管理員"的有限角色。
...
此角色中的使用者可以讀取 Microsoft 365 服務的設置和管理資訊，但不能執行管理操作。 全域讀取器是全域管理員的唯讀對應器。 分配全域讀取器而不是全域管理員進行規劃、審核或調查。 將全域讀取器與其他有限的管理員角色（如 Exchange Admin）結合使用，使其在未分配全域管理員角色的情況下更輕鬆地完成工作。 全球閱讀器與 Microsoft 365 管理中心、交換管理中心、團隊管理中心、安全中心、合規性中心、Azure AD 管理中心和裝置管理管理中心合作。
...
>- [M365 管理中心](https://admin.microsoft.com/Adminportal/Home#/homepage)- 全域讀取器無法讀取客戶密碼箱請求。 在 M365 管理中心左側窗格中的 **"支援"** 下找不到 **"客戶密碼箱請求**"選項卡。
>- [M365 安全中心](https://security.microsoft.com/homepage)- 全域讀取器無法讀取靈敏度和保留標籤。 在 M365 安全中心的左側窗格中找不到 **"敏感度"標籤**、**保留標籤**和**標籤分析**選項卡。
>- [辦公室安全&合規性中心](https://sip.protection.office.com/homepage)- 全域讀者無法讀取 SCC 稽核記錄、執行內容搜索或查看安全分數。
>- [團隊管理中心](https://admin.teams.microsoft.com)- 全球閱讀器不能讀取**團隊生命週期**，**分析&報告****，IP電話裝置管理和****應用程式目錄**。
>- [特權訪問管理 （PAM）](https://docs.microsoft.com/office365/securitycompliance/privileged-access-management-overview)不支援全域讀取器角色。
...
此角色中的使用者可以創建/管理組及其設置，如命名和過期策略。 請務必瞭解，將使用者分配給此角色，使他們能夠管理租戶中除 Outlook 之外的各種工作負載（如 Teams、SharePoint、Yammer）中的所有組。 此外，使用者將能夠管理各種監管中心（如 Microsoft 管理中心、Azure 門戶）以及各種組設置，以及特定于工作負荷的組設置，如團隊和 SharePoint 管理中心。
...
具備此角色的使用者可以變更密碼、讓重新整理權杖失效、管理服務要求，以及監視服務健康情況。 讓重新整理權杖失效會強制使用者重新登入。 説明台管理員可以重置密碼，使非管理員或僅分配以下角色的其他使用者的刷新權杖無效：
...
此角色以前在[Azure 門戶](https://portal.azure.com/)中稱為"密碼管理員"。 Azure AD 中的"説明台管理員"名稱現在與其名稱在 Azure AD PowerShell 和 Microsoft 圖形 API 中匹配。
...
此角色可以創建和管理所有安全性群組。 但是，Intune 管理員對 Office 組沒有管理員許可權。 這意味著管理員無法更新租戶中所有 Office 組的擁有者或成員身份。 但是，他/她可以管理他創建的 Office 組，該組是其最終使用者許可權的一部分。 因此，他/她創建的任何 Office 組（不是安全性群組）都應計入其配額 250。
...
具有此角色的使用者具有全域許可權，可在存在服務時管理 Microsoft Kaizala 中的設置，以及管理支援票證和監視服務運行狀況的功能。 此外，使用者可以訪問與採用 &組織成員使用 Kaizala 相關的報告，以及使用 Kaizala 操作生成的業務報告。
...
### <a name="message-center-privacy-reader"></a>[消息中心隱私閱讀器](#message-center-privacy-reader-permissions)

此角色中的使用者可以監視消息中心中的所有通知，包括資料隱私消息。 郵件中心隱私讀者收到電子郵件通知，包括與資料隱私相關的通知，他們可以使用郵件中心首選項取消訂閱。 只有全域管理員和郵件中心隱私讀取器才能讀取資料隱私消息。 此外，此角色包含查看組、域和訂閱的功能。 此角色無權查看、創建或管理服務請求。
...
此角色中的使用者可以管理 Office 365 應用的雲設置。 這包括管理雲策略、自助服務下載管理和查看 Office 應用相關報告的能力。 此角色還授予管理支援票證和監視主管理中心中的服務運行狀況的能力。 分配給此角色的使用者還可以管理 Office 應用中新功能的通信。 
...
### <a name="power-platform-administrator"></a>[電源平臺管理員](#power-platform-administrator-permissions)

此角色中的使用者可以創建和管理環境的所有方面，PowerApps、流、資料丟失防護策略。 此外，具有此角色的使用者能夠管理支援票證並監視服務運行狀況。

### <a name="privileged-authentication-administrator"></a>[特權身份驗證管理員](#privileged-authentication-administrator-permissions)

具有此角色的使用者可以為所有使用者（包括全域管理員）設置或重置非密碼憑據，並可以更新所有使用者的密碼。 特權身份驗證管理員可以強制使用者根據現有非密碼憑據（例如 MFA、FIDO）重新註冊，並撤銷"記住設備上的 MFA"，從而在所有使用者的下一次登錄時提示 MFA。
...
具有此角色的使用者可以在 Microsoft 365 管理中心查看使用方式報告資料和報告儀表板，並在 Power BI 中查看採用上下文包。 此外，此角色還可讓使用者存取 Azure AD 中的登入報告與活動，以及 Microsoft Graph 報告 API 所傳回的資料。 獲指派「報告讀者」角色的使用者只能存取相關的使用情況和採用計量。 他們並不具備任何系統管理權限，因此無法進行設定或存取產品特定的系統管理中心 (例如 Exchange)。 這個角色沒有檢視、建立或管理支援票證的存取權。

### <a name="search-administrator"></a>[搜索管理員](#search-administrator-permissions)

此角色中的使用者可以完全訪問 Microsoft 365 管理中心中的所有 Microsoft 搜索管理功能。 搜索管理員可以將搜索管理員和搜索編輯器角色委派給使用者，並創建和管理內容，如書簽、Q&As 和位置。 此外，這些使用者可以查看消息中心、監視服務運行狀況和創建服務請求。

### <a name="search-editor"></a>[搜索編輯器](#search-editor-permissions)

此角色中的使用者可以在 Microsoft 365 管理中心創建、管理和刪除 Microsoft 搜索的內容，包括書簽、Q&As 和位置。
...
[特權身份管理](https://docs.microsoft.com/azure/active-directory/privileged-identity-management/pim-configure) | 「安全性讀取者」角色的所有權限<br>**無法**管理 Azure AD 角色指派或設置
...
具有此角色的使用者可以管理警報，並具有安全相關功能的全域唯讀存取權限，包括 Microsoft 365 安全中心、Azure 活動目錄、標識保護、特權標識管理和 Office 365 安全&合規性中心中的所有資訊。 關於 Office 365 權限的詳細資訊可在 [Office 365 安全性與法規遵循中心的權限](https://docs.microsoft.com/office365/securitycompliance/permissions-in-the-security-and-compliance-center)中取得。

在 | 可以執行
--- | ---
[Microsoft 365 資訊安全中心](https://protection.office.com) | 「安全性讀取者」角色的所有權限<br>查看、調查和回應安全威脅警報
身分識別防護中心 | 「安全性讀取者」角色的所有權限<br>此外，還能夠執行除了重設密碼以外的所有身分識別防護中心作業
[特權身份管理](https://docs.microsoft.com/azure/active-directory/privileged-identity-management/pim-configure) | 「安全性讀取者」角色的所有權限
[Office 365 安全性與合規性中心](https://support.office.com/article/About-Office-365-admin-roles-da585eea-f576-4f55-a1e0-87090b6aaa9d) | 「安全性讀取者」角色的所有權限<br>查看、調查和回應安全警報
Windows Defender ATP 和 EDR | 「安全性讀取者」角色的所有權限<br>查看、調查和回應安全警報
...
[特權身份管理](https://docs.microsoft.com/azure/active-directory/privileged-identity-management/pim-configure) | 已對 Azure AD 特權標識管理中顯示的所有資訊的唯讀存取權限：Azure AD 角色指派和安全審查的策略和報告。<br>**無法**註冊 Azure AD 特權標識管理或對其進行任何更改。 在特權身份監管中心或通過 PowerShell 中，如果使用者有資格擔任這些角色，則此角色中的某人可以啟動其他角色（例如，全域管理員或特權角色管理員）。
...
Windows Defender ATP 和 EDR | 查看和調查警報。 當您在 Windows Defender ATP 中打開基於角色的訪問控制項時，具有唯讀許可權（如 Azure AD 安全讀取器角色）的使用者將無法訪問，直到他們被分配到 Windows Defender ATP 角色。
...
具有此角色的使用者可以使用 Microsoft 打開 Azure 和 Office 365 服務的支援請求，並在[Azure 門戶](https://portal.azure.com)和[Microsoft 365 管理中心](https://admin.microsoft.com)中查看服務儀表板和消息中心。 有關詳細資訊[，有關管理員角色](https://support.office.com/article/About-Office-365-admin-roles-da585eea-f576-4f55-a1e0-87090b6aaa9d)。
...
### <a name="teams-communications-administrator"></a>[團隊通信管理員](#teams-communications-administrator-permissions)

此角色的使用者可以管理 Microsoft Teams 在語音和電話語音相關工作負載的各個層面。 這包括電話號碼指派管理工具、語音和會議原則，以及呼叫分析工具組的完整存取權。

### <a name="teams-communications-support-engineer"></a>[團隊通信支援工程師](#teams-communications-support-engineer-permissions)
...
### <a name="teams-communications-support-specialist"></a>[團隊通信支援專家](#teams-communications-support-specialist-permissions)
...
具有此角色的使用者可以創建使用者，並管理具有某些限制的使用者的所有方面（見下文），並可以更新密碼過期策略。 此外，具有此角色的使用者可以建立與管理所有群組。 此角色也包含建立和管理使用者檢視、管理支援票證，以及監視服務健康情況的能力。 使用者管理員無權管理大多數管理員角色中使用者的某些使用者屬性。 具有此角色的使用者沒有管理 MFA 的子項。 下表列出了對此限制的例外角色。

| | |
| --- | --- |
|一般權限|<p>建立 [使用者和群組]</p><p>建立和管理使用者檢視</p><p>建立 Office 支援票證<p>更新密碼過期策略|
...
### <a name="application-administrator-permissions"></a>應用程式管理員許可權
...
| 微軟.目錄/應用程式/應用程式代理Url設置/更新 | 在 Azure 活動目錄中更新應用程式代理內部和外部 URLS。 |
...
| 微軟.目錄/應用程式/創建 | 在 Azure Active Directory 中建立應用程式。 |
...
| 微軟.目錄/連接器組/創建 | 在 Azure 活動目錄中創建應用程式代理連接器組。 |
| 微軟.目錄/連接器組/刪除 | 刪除 Azure 活動目錄中的應用程式代理連接器組。 |
| 微軟.目錄/連接器/一切/讀取 | 讀取 Azure 活動目錄中的所有應用程式代理連接器屬性。 |
| 微軟.目錄/連接器/創建 | 在 Azure 活動目錄中創建應用程式代理連接器。 |
| 微軟.目錄/策略/應用程式佈建/基本/讀取 | 讀取 Azure Active Directory 中的 policies.applicationConfiguration 屬性。 |
| 微軟.目錄/策略/應用程式佈建/基本/更新 | 更新 Azure Active Directory 中的 policies.applicationConfiguration 屬性。 |
| 微軟.目錄/策略/應用程式佈建/創建 | 在 Azure Active Directory 中建立原則。 |
| 微軟.目錄/策略/應用程式佈建/刪除 | 刪除 Azure Active Directory 中的原則。 |
| 微軟.目錄/策略/應用程式佈建/擁有者/讀取 | 讀取 Azure Active Directory 中的 policies.applicationConfiguration 屬性。 |
| 微軟.目錄/策略/應用程式佈建/擁有者/更新 | 更新 Azure Active Directory 中的 policies.applicationConfiguration 屬性。 |
| microsoft.目錄/策略/應用程式佈建/策略應用於/讀取 | 讀取 Azure Active Directory 中的 policies.applicationConfiguration 屬性。 |
...
可以獨立于"使用者可以註冊應用程式"設置創建應用程式註冊。

| **動作** | **描述** |
| --- | --- |
| 微軟.目錄/應用程式/創建擁有者 | 在 Azure Active Directory 中建立應用程式。 建立者會新增為第一個擁有者，而建立的物件會算在建立者的 250 個建立物件配額中。 |
| 微軟.目錄/應用程式角色分配/創建擁有者 | 在 Azure Active Directory 中建立 appRoleAssignments。 建立者會新增為第一個擁有者，而建立的物件會算在建立者的 250 個建立物件配額中。 |
...
### <a name="authentication-administrator-permissions"></a>身份驗證管理員許可權
...
| 微軟.目錄/使用者/增強式驗證/更新 | 更新 MFA 認證資訊等增強式驗證屬性。 |
| microsoft.azure.serviceHealth/allEntities/allTasks | 讀取及設定 Azure 服務健康情況。 |
| microsoft.azure.supportTickets/allEntities/allTasks | 建立和管理 Azure 支援票證。 |
| microsoft.office365.webPortal/allEntities/basic/read | 讀取 microsoft.office365.webPortal 中所有資源的基本屬性。 |
| microsoft.office365.serviceHealth/allEntities/allTasks | 讀取及設定 Office 365 服務健康情況。 |
| microsoft.office365.supportTickets/allEntities/allTasks | 建立和管理 Office 365 支援票證。 |
| 微軟.目錄/使用者/密碼/更新 | 更新 Office 365 組織中所有使用者的密碼。 如需詳細資訊，請參閱線上文件。 |

### <a name="azure-devops-administrator-permissions"></a>Azure 開發人員管理員許可權
...
### <a name="azure-information-protection-administrator-permissions"></a>Azure 資訊保護管理員許可權
...
### <a name="b2c-user-flow-administrator-permissions"></a>B2C 使用者流管理員許可權
...
### <a name="b2c-user-flow-attribute-administrator-permissions"></a>B2C 使用者流屬性管理員許可權
...
### <a name="b2c-ief-keyset-administrator-permissions"></a>B2C IEF 金鑰集管理員許可權

在身份體驗框架中管理聯合和加密的機密。

| **動作** | **描述** |
| --- | --- |
| microsoft.aad.b2c/信任框架/金鑰集/所有任務 | 讀取和配置 Azure 活動目錄 B2C 中的金鑰集。 |

### <a name="b2c-ief-policy-administrator-permissions"></a>B2C IEF 策略管理員許可權

在身份體驗框架中創建和管理信任框架策略。

| **動作** | **描述** |
| --- | --- |
| microsoft.aad.b2c/信任框架/策略/所有任務 | 在 Azure 活動目錄 B2C 中讀取和配置自訂策略。 |
...
### <a name="cloud-application-administrator-permissions"></a>雲應用程式管理員許可權
...
### <a name="cloud-device-administrator-permissions"></a>雲裝置管理員許可權
...
| 微軟.目錄/設備/刪除 | 刪除 Azure Active Directory 中的 devices。 |
| 微軟.目錄/設備/禁用 | 停用 Azure Active Directory 中的 devices。 |
| 微軟.目錄/設備/啟用 | 啟用 Azure Active Directory 中的裝置。 |
| 微軟.目錄/簽名報告/所有屬性/讀取 | 讀取 Azure Active Directory 中的 signInReports 所包含的所有屬性 (包括特殊權限的屬性)。 |
| microsoft.azure.serviceHealth/allEntities/allTasks | 讀取及設定 Azure 服務健康情況。 |
| microsoft.office365.serviceHealth/allEntities/allTasks | 讀取及設定 Office 365 服務健康情況。 |

### <a name="company-administrator-permissions"></a>公司管理員許可權
...
| 微軟.目錄/合同/所有屬性/所有任務 | 建立和刪除合約，以及讀取與更新 Azure Active Directory 中的所有屬性。 |
| 微軟.目錄/設備/所有屬性/所有任務 | 建立和刪除裝置，以及在 Azure Active Directory 中讀取和更新所有屬性。 |
| microsoft.directory/目錄角色/所有屬性/所有任務 | 建立和刪除 directoryRoles，以及在 Azure Active Directory 中讀取和更新所有屬性。 |
| 微軟.目錄/目錄角色範本/所有屬性/所有任務 | 建立和刪除 directoryRoleTemplates，以及在 Azure Active Directory 中讀取和更新所有屬性。 |
| 微軟.目錄/域/所有屬性/所有任務 | 建立和刪除 domains，以及在 Azure Active Directory 中讀取和更新所有屬性。 |
| 微軟.目錄/組/所有屬性/所有任務 | 建立和刪除 groups，以及讀取與更新 Azure Active Directory 中的所有屬性。 |
| 微軟.目錄/組設置/所有屬性/所有任務 | 建立和刪除 groupSettings，以及讀取與更新 Azure Active Directory 中的所有屬性。 |
| microsoft.目錄/組設置範本/所有屬性/所有任務 | 建立和刪除 groupSettingTemplates，以及讀取與更新 Azure Active Directory 中的所有屬性。 |
...
| microsoft.目錄/角色指派/所有屬性/所有任務 | 建立與刪除 roleAssignments，以及讀取與更新 Azure Active Directory 中的所有屬性。 |
...
| 微軟.目錄/服務操作/禁用目錄功能 | 可以在 Azure Active Directory 中執行 Disabledirectoryfeature 服務動作 |
| 微軟.目錄/服務操作/啟用目錄功能 | 可以在 Azure Active Directory 中執行 Enabledirectoryfeature 服務動作 |
| 微軟.目錄/服務操作/獲取可用屬性 | 可以在 Azure Active Directory 中執行 Getavailableextentionproperties 服務動作 |
...
### <a name="compliance-administrator-permissions"></a>合規性管理員許可權
...
| **動作** | **描述** |
| --- | --- |
| microsoft.aad.cloudAppSecurity/allEntities/allTasks | 讀取和配置 Microsoft 雲應用安全性。 |
| microsoft.azure.informationProtection/allEntities/allTasks | 管理 Azure 資訊保護的所有層面。 |
| microsoft.azure.serviceHealth/allEntities/allTasks | 讀取及設定 Azure 服務健康情況。 |
| microsoft.azure.supportTickets/allEntities/allTasks | 建立和管理 Azure 支援票證。 |
| microsoft.office365.webPortal/allEntities/basic/read | 讀取 microsoft.office365.webPortal 中所有資源的基本屬性。 |
...
### <a name="conditional-access-administrator-permissions"></a>條件訪問管理員許可權

可以管理條件訪問功能。

| **動作** | **描述** |
| --- | --- |
| microsoft.目錄/策略/條件訪問/基本/讀取 | 讀取 Azure Active Directory 中的 policies.conditionalAccess 屬性。 |
| microsoft.目錄/策略/條件訪問/基本/更新 | 更新 Azure Active Directory 中的 policies.conditionalAccess 屬性。 |
| microsoft.目錄/策略/條件訪問/創建 | 在 Azure Active Directory 中建立原則。 |
| microsoft.目錄/策略/條件訪問/刪除 | 刪除 Azure Active Directory 中的原則。 |
| microsoft.目錄/策略/條件訪問/擁有者/讀取 | 讀取 Azure Active Directory 中的 policies.conditionalAccess 屬性。 |
| microsoft.目錄/策略/條件訪問/擁有者/更新 | 更新 Azure Active Directory 中的 policies.conditionalAccess 屬性。 |
| microsoft.目錄/策略/條件訪問/策略應用於/讀取 | 讀取 Azure Active Directory 中的 policies.conditionalAccess 屬性。 |
| microsoft.目錄/策略/條件訪問/租戶預設/更新 | 更新 Azure Active Directory 中的 policies.conditionalAccess 屬性。 |
...
### <a name="customer-lockbox-access-approver-permissions"></a>客戶密碼箱訪問核准者許可權
...
### <a name="desktop-analytics-administrator-permissions"></a>桌面分析管理員許可權

可以管理桌面分析和辦公室自訂&策略服務。 對於桌面分析，這包括查看資產清單、創建部署計畫、查看部署和運行狀況。 對於 Office 自訂&策略服務，此角色使使用者能夠管理 Office 策略。
...
### <a name="device-administrators-permissions"></a>裝置管理員許可權
...
### <a name="directory-readers-permissions"></a>目錄閱讀器許可權
...
| 微軟.目錄/設備/基本/讀取 | 讀取 Azure Active Directory 中 devices 的基本屬性。 |
| 微軟.目錄/設備/成員/讀取 | 讀取 Azure Active Directory 中的 devices.memberOf 屬性。 |
| 微軟.目錄/設備/註冊擁有者/閱讀 | 讀取 Azure Active Directory 中的 devices.registeredOwners 屬性。 |
| 微軟.目錄/設備/註冊使用者/閱讀 | 讀取 Azure Active Directory 中的 devices.registeredUsers 屬性。 |
...
| 微軟.目錄/組/設置/讀取 | 讀取 Azure Active Directory 中的 groups.settings 屬性。 |
| 微軟.目錄/組設置/基本/讀取 | 讀取 Azure Active Directory 中 groupSettings 的基本屬性。 |
| 微軟.目錄/組設置範本/基本/讀取 | 讀取 Azure Active Directory 中 groupSettingTemplates 的基本屬性。 |
...
| 微軟.目錄/角色指派/基本/讀取 | 讀取 Azure Active Directory 中 roleAssignments 的基本屬性。 |
...
| 微軟.目錄/使用者/擁有設備/讀取 | 讀取 Azure Active Directory 中的 users.ownedDevices 屬性。 |
...
### <a name="directory-synchronization-accounts-permissions"></a>目錄同步帳戶許可權
...
| 微軟.目錄/策略/創建 | 在 Azure Active Directory 中建立原則。 |
...
### <a name="directory-writers-permissions"></a>目錄編寫器許可權

可讀取和寫入基本目錄資訊。 用來授與應用程式的存取權，不適用於使用者。

| **動作** | **描述** |
| --- | --- |
| 微軟.目錄/組/創建 | 在 Azure Active Directory 中建立 groups。 |
| microsoft.目錄/組/創建擁有者 | 在 Azure Active Directory 中建立 groups。 建立者會新增為第一個擁有者，而建立的物件會算在建立者的 250 個建立物件配額中。 |
...
| 微軟.目錄/組設置/創建 | 在 Azure Active Directory 中建立 groupSettings。 |
| 微軟.目錄/組設置/刪除 | 在 Azure Active Directory 中刪除 groupSettings。 |
| 微軟.目錄/使用者/應用程式角色分配/更新 | 更新 Azure Active Directory 中的 users.appRoleAssignments 屬性。 |
| 微軟.目錄/使用者/分配許可證 | 管理 Azure Active Directory 中的使用者授權。 |
...
### <a name="exchange-service-administrator-permissions"></a>交換服務管理員許可權
...
### <a name="external-identity-provider-administrator-permissions"></a>外部標識提供程式管理員許可權

配置標識提供程式以用於直接聯合。

| **動作** | **描述** |
| --- | --- |
| microsoft.aad.b2c/身份提供者/所有任務 | 讀取和配置 Azure 活動目錄 B2C 中的標識提供程式。 |

### <a name="global-reader-permissions"></a>全域讀取器許可權
...
| 微軟.目錄/使用者/擁有的物件/讀取    | 讀取 Azure Active Directory 中的 users.ownedObjects 屬性。 |
| 微軟.目錄/使用者/註冊設備/讀取    | 讀取 Azure Active Directory 中的 users.registeredDevices 屬性。 |
| 微軟.目錄/使用者/增強式驗證/讀取    | 讀取增強式驗證屬性，如 MFA 憑據資訊。 |
| 微軟.office365.交換/所有實體/讀    | 閱讀線上交換的所有方面。 |
| microsoft.office365.messageCenter/messages/read    | 讀取 microsoft.office365.messageCenter 中的訊息。 |
| microsoft.office365.messageCenter/securityMessages/read    | 讀取 microsoft.office365.messageCenter 中的 securityMessages。 |
...
### <a name="groups-administrator-permissions"></a>組管理員許可權
...
### <a name="helpdesk-administrator-permissions"></a>説明台管理員許可權
...
| 微軟.目錄/連絡人/創建 | 在 Azure Active Directory 中建立 contacts。 |
| 微軟.目錄/連絡人/刪除 | 刪除 Azure Active Directory 中的 contacts。 |
| 微軟.目錄/設備/基本/更新 | 更新 Azure Active Directory 中 devices 的基本屬性。 |
| 微軟.目錄/設備/位鎖修復金鑰/讀取 | 讀取 Azure Active Directory 中的 devices.bitLockerRecoveryKeys 屬性。 |
| 微軟.目錄/設備/創建 | 在 Azure Active Directory 中建立 devices。 |
| 微軟.目錄/設備/刪除 | 刪除 Azure Active Directory 中的 devices。 |
| 微軟.目錄/設備/註冊擁有者/更新 | 更新 Azure Active Directory 中的 devices.registeredOwners 屬性。 |
| 微軟.目錄/設備/註冊使用者/更新 | 更新 Azure Active Directory 中的 devices.registeredUsers 屬性。 |
| 微軟.目錄/組/應用程式角色分配/更新 | 更新 Azure Active Directory 中的 groups.appRoleAssignments 屬性。 |
| 微軟.目錄/組/基本/更新 | 更新 Azure Active Directory 中 groups 的基本屬性。 |
| 微軟.目錄/組/創建 | 在 Azure Active Directory 中建立 groups。 |
| microsoft.目錄/組/創建擁有者 | 在 Azure Active Directory 中建立 groups。 建立者會新增為第一個擁有者，而建立的物件會算在建立者的 250 個建立物件配額中。 |
...
### <a name="kaizala-administrator-permissions"></a>凱紮拉管理員許可權
...
### <a name="message-center-privacy-reader-permissions"></a>資訊中心隱私閱讀器許可權

可以讀取消息中心帖子、資料隱私消息、組、域和訂閱。
...
### <a name="message-center-reader-permissions"></a>消息中心讀取器許可權
...
### <a name="office-apps-administrator-permissions"></a>辦公室應用 管理員許可權
...
| microsoft.office365.serviceHealth/allEntities/allTasks | 讀取及設定 Office 365 服務健康情況。 |
| microsoft.office365.supportTickets/allEntities/allTasks | 建立和管理 Office 365 支援票證。 |
| 微軟.office365.使用者通信/所有身份/所有任務 | 閱讀和更新"新增郵件"可見度。 |
| microsoft.office365.webPortal/allEntities/basic/read | 讀取 microsoft.office365.webPortal 中所有資源的基本屬性。 |

### <a name="partner-tier1-support-permissions"></a>合作夥伴第 1 層支援許可權
...
| 微軟.目錄/使用者/經理/更新 | 更新 Azure Active Directory 中的 users.manager 屬性。 |
| 微軟.目錄/使用者/密碼/更新 | 在 Azure Active Directory 中更新所有使用者的密碼。 如需詳細資訊，請參閱線上文件。 |
| 微軟.目錄/使用者/恢復 | 還原 Azure Active Directory 中已刪除的使用者。 |
| 微軟.目錄/使用者/使用者主名/更新 | 更新 Azure Active Directory 中的 users.userPrincipalName 屬性。 |
| microsoft.azure.serviceHealth/allEntities/allTasks | 讀取及設定 Azure 服務健康情況。 |
| microsoft.azure.supportTickets/allEntities/allTasks | 建立和管理 Azure 支援票證。 |
| microsoft.office365.webPortal/allEntities/basic/read | 讀取 microsoft.office365.webPortal 中所有資源的基本屬性。 |
...
### <a name="partner-tier2-support-permissions"></a>合作夥伴第 2 層支援許可權
...
| 微軟.目錄/連絡人/創建 | 在 Azure Active Directory 中建立 contacts。 |
| 微軟.目錄/連絡人/刪除 | 刪除 Azure Active Directory 中的 contacts。 |
| 微軟.目錄/域/所有任務 | 建立和刪除 domains，以及在 Azure Active Directory 中讀取和更新標準屬性。 |
| 微軟.目錄/組/創建 | 在 Azure Active Directory 中建立 groups。 |
| 微軟.目錄/組/刪除 | 刪除 Azure Active Directory 中的 groups。 |
| 微軟.目錄/組/成員/更新 | 更新 Azure Active Directory 中的 groups.members 屬性。 |
| 微軟.目錄/組/恢復 | 還原 Azure Active Directory 中的 groups。 |
...
### <a name="power-bi-service-administrator-permissions"></a>電源 BI 服務管理員許可權
...
### <a name="power-platform-administrator-permissions"></a>電源平臺管理員許可權
...
### <a name="privileged-authentication-administrator-permissions"></a>特權身份驗證管理員許可權
...
### <a name="privileged-role-administrator-permissions"></a>特權角色管理員許可權

可以管理 Azure AD 中的角色指派以及特權標識管理的所有方面。
...
| microsoft.目錄/管理單位/所有屬性/所有任務 | 創建和管理管理單位（包括成員） |
| microsoft.目錄/角色指派/所有屬性/所有任務 | 創建和管理角色指派。 |
| 微軟.目錄/角色定義/所有屬性/所有任務 | 創建和管理角色定義。 |

### <a name="reports-reader-permissions"></a>報告讀取器許可權
...
### <a name="search-administrator-permissions"></a>搜索管理員許可權
...
### <a name="search-editor-permissions"></a>搜索編輯器許可權

可以創建和管理編輯內容，如書簽、問答、位置、平面圖。
...
### <a name="security-administrator-permissions"></a>安全管理員許可權
...
### <a name="security-operator-permissions"></a>安全操作員許可權
...
| microsoft.windows.defenderAdvancedThreatProtection/allEntities/read | 讀取和配置 Windows 防禦器高級威脅保護。 |

### <a name="security-reader-permissions"></a>安全讀取器許可權
...
### <a name="service-support-administrator-permissions"></a>服務支援管理員許可權
...
### <a name="sharepoint-service-administrator-permissions"></a>共用點服務管理員許可權
...
### <a name="teams-communications-administrator-permissions"></a>團隊通信管理員許可權
...
### <a name="teams-communications-support-engineer-permissions"></a>團隊通信支援工程師許可權
...
### <a name="teams-communications-support-specialist-permissions"></a>團隊通信支援專家許可權
...
### <a name="teams-service-administrator-permissions"></a>團隊服務管理員許可權
...
### <a name="user-administrator-permissions"></a>使用者管理員許可權
...
| 微軟.目錄/使用者/分配許可證 | 管理 Azure Active Directory 中的使用者授權。 |
| 微軟.目錄/使用者/基本/更新 | 更新 Azure Active Directory 中 users 的基本屬性。 |
| 微軟.目錄/使用者/創建 | 在 Azure Active Directory 中建立 users。 |
| 微軟.目錄/使用者/刪除 | 刪除 Azure Active Directory 中的 users。 |
| microsoft.directory/使用者/無效所有刷新權杖 | 使 Azure Active Directory 中的所有使用者重新整理權杖失效。 |
| 微軟.目錄/使用者/經理/更新 | 更新 Azure Active Directory 中的 users.manager 屬性。 |
| 微軟.目錄/使用者/密碼/更新 | 在 Azure Active Directory 中更新所有使用者的密碼。 如需詳細資訊，請參閱線上文件。 |
...
B2C IEF 策略管理員 | B2C IEF 策略管理員 | 3edaf663-341e-4475-9f94-5c398ef6c070
...
加入裝置 | 設備聯接 | 9c094953-4995-41c8-84c8-3ebb9b32c93f
...
外部標識提供程式管理員 | 外部標識提供程式管理員 | be2f45a1-457d-42af-a067-6ec1fa63bc45
...
消息中心隱私閱讀器 | 消息中心隱私讀取器 | ac16e43d-7b2d-40e0-ac05-243ff356ab5b
訊息中心讀取者 | 消息中心閱讀器 | 790c1fb9-7f7d-4f88-86a1-ef1f95c05c1b
...
電源平臺管理員 | 電源平臺管理員 | 11648597-926c-4cf3-9c36-bcebb0ba8dcc
特權身份驗證管理員 | 特權身份驗證管理員 | 7be44c8a-adaf-4e2a-84d6-ab2649e08a13
特殊權限角色管理員 | 特殊權限角色管理員 | e8611ab8-c189-46e8-94e1-60213ab1f814
報告讀者 | 報表讀者 | 4a5d8f65-41da-4de4-8968-e035b65339cf
搜索管理員 | 搜索管理員 | 0964bb5e-9bdb-4d7b-ac29-58e794862a40
搜索編輯器 | 搜索編輯器 | 8835291a-918c-4fd7-a9ce-faa49f0cf7d9
...
加入工作場所裝置 | 工作場所設備加入 | c34f683f-4d5a-4403-affd-6615e00e3a7f
