...
設定群組命名原則時，會將原則套用至使用者所建立的新 Office 365 群組。 命名原則不適用於特定目錄角色，例如全域管理員或使用者系統管理員（如需從群組命名原則豁免的完整角色清單，請參閱下文）。 針對現有的 Office 365 群組，原則不會在設定時立即套用。 一旦群組擁有者編輯這些群組的組名，就會強制執行命名原則。
