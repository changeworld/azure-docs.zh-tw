---
title: 基於組的許可附加方案 - Azure AD |微軟文檔
...
並非所有位置都可使用某些 Microsoft 服務。 系統管理員必須指定使用者的 [使用位置]**** 屬性，才能將授權指派給使用者。 [在 Azure 門戶中](https://portal.azure.com)，可以在**使用者**&gt;**設定檔**&gt;**設置**中指定使用位置。

如果是群組授權指派，未指定使用位置的任何使用者會繼承目錄的位置。 如果使用者位於多個位置，請確保在將使用者添加到具有許可證的組之前，在使用者資源中正確反映這一點。
...
您可以對任何安全性群組使用群組型授權，這表示可與 Azure AD 動態群組結合。 動態組針對使用者資源屬性運行規則，以自動從組中添加和刪除使用者。
...
- 選擇*E3*許可證將顯示更多詳細資訊，包括組許可證分配為使用者啟用哪些服務的資訊。
...
最初，使用者僅從*E3 基本服務*組繼承許可證，該組啟用四個服務方案。

1. 選擇 **"分配**"可直接向使用者分配 E3 許可證。 在此案例中，您準備停用 Yammer Enterprise 以外的所有服務方案。

    因此，使用者仍然只使用 E3 產品的一個授權。 但是，直接指派只會針對該使用者啟用 Yammer Enterprise 服務。 您可以看到組成員身份與直接分配啟用的服務。

1. 使用直接指派時允許下列作業︰

   - Yammer 企業可以直接關閉使用者資源。 在圖中，這項服務已啟用 [開啟/關閉]**** 切換，不同於其他服務切換。 因為此服務直接在使用者上啟用，所以可以修改。
...
當 Microsoft 將新服務添加到產品許可證計畫時，預設情況下，在您已為其分配產品許可證的所有組中啟用該服務。 您租用戶中訂閱產品變更相關通知的使用者會事先收到電子郵件，通知他們即將新增服務。
...
4. 按一下您要檢閱的群組 (在本例中為「O365 E5 - 僅 Exchange」**)。 這將打開 **"許可證"** 選項卡。按一下 E5 許可證將打開一個刀片式伺服器，列出所有已啟用的服務。
   > [!NOTE]
...
3. 挑選清單中的專案以查看已更改的內容的詳細資訊。 在 [已修改的屬性]** 下會同時列出授權指派的新舊值。
...
- [Microsoft 365 管理中心](https://admin.microsoft.com)當前不支援基於組的許可。 如果使用者從群組繼承授權，此授權在 Office 系統管理入口網站中會顯示為一般使用者授權。 如果您嘗試修改該授權或嘗試移除授權，入口網站會傳回錯誤訊息。 無法直接修改使用者繼承的群組授權。
