...
**問：[在微軟 Azure 德國雲](https://www.microsoft.de/cloud-deutschland)和 Microsoft Azure[政府雲](https://azure.microsoft.com/features/gov/)中是否提供無縫 SSO？**

否。 只有全球版的 Azure AD 執行個體有提供「無縫 SSO」功能。

**問：哪些應用程式利用了無縫`domain_hint`SSO 的優勢或`login_hint`參數功能？**
...
| Office 365 入口網站 | HTTPs：\//portal.office.com？domain_hint_contoso.com， HTTPs：\//www.office.com？domain_hint_contoso.com |

此外，如果應用程式向 Azure AD 設置為租戶的終結點（即 HTTPs：/login.microsoftonline.com/contoso.com/<..> 或 HTTPs：/login.microsoftonline.com/<tenant_ID>/<）\/\/發送登錄請求，則使用者可以獲得靜默登錄體驗。> - 而不是 Azure AD 的常見終結點 - 即\/HTTPs： /login.microsoftonline.com/common/<...>。 以下是一份不完整的應用程式清單，列出會提出這類登入要求的應用程式。
...
**問：無縫 SSO`Alternate ID`支援作為使用者名，而不是`userPrincipalName`？**
...
**問[：Azure AD 聯接](../active-directory-azureadjoin-overview.md)和無縫 SSO 提供的單一登入體驗之間的區別是什麼？**
...
**問：我想使用 Azure AD 註冊非 Windows 10 設備，而無需使用 AD FS。我可以改用無縫 SSO 嗎？**

是，此案例需要 2.1 版或更新版本的[加入工作場所用戶端](https://www.microsoft.com/download/details.aspx?id=53554)。

**問：如何滾動電腦帳戶的 Kerberos 解密金鑰`AZUREADSSOACC`？**
...
請在執行 Azure AD Connect 的內部部署伺服器上依照下列步驟操作：
   ...
   **步驟 2.更新設置它的每個 AD 林上的 Kerberos 解密金鑰**
   ...
   >[!IMPORTANT]
   >確定您「並未」__ 執行 `Update-AzureADSSOForest` 命令一次以上。 否則，此功能會停止運作，直到使用者的 Kerberos 票證過期並由您的內部部署 Active Directory 重新發出為止。
...
**問：如何禁用無縫 SSO？**
   ...
   **步驟 3.手動從您看到`AZUREADSSOACCT`列出的每個 AD 林中刪除電腦帳戶。**

## <a name="next-steps"></a>後續步驟

- [**快速入門**](how-to-connect-sso-quick-start.md)- 啟動並運行 Azure AD 無縫 SSO。
