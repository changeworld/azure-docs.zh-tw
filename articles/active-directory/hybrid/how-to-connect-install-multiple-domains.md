...
description: 本檔說明如何使用 Microsoft 365 和 Azure AD 來設定多個最上層網域。
...
![顯示多個最上層網域的螢幕擷取畫面](./media/how-to-connect-install-multiple-domains/domains.png)
...
因此，在 Azure AD 或 Microsoft 365 驗證期間，會使用使用者權杖中的 IssuerUri 元素，在 Azure AD 中尋找網域。 如果找不到相符項目，驗證將會失敗。
...
4. 在已安裝[適用於 Windows PowerShell 的 Microsoft Azure Active Directory 模組](/previous-versions/azure/jj151815(v=azure.100))的機器上執行下列命令：`$cred=Get-Credential`。  
...
1. 在已安裝[適用於 Windows PowerShell 的 Microsoft Azure Active Directory 模組](/previous-versions/azure/jj151815(v=azure.100))的機器上執行下列命令：`$cred=Get-Credential`。  
...
因此，假設我有 bmcontoso.com，並接著加入 corp.bmcontoso.com。  Corp.bmcontoso.com 使用者的 IssuerUri 將必須是 **`http://bmcontoso.com/adfs/services/trust`** 。  不過，上述針對 Azure AD 所執行的標準規則，將會產生具有簽發者的權杖作為 **`http://corp.bmcontoso.com/adfs/services/trust`** 。  的權杖，這與網域所需的值不符，因此驗證將會失敗。
