...
- 停用同步排程器。 排程器 預設會每隔 30 分鐘執行一次。 當您進行變更並針對新規則進行疑難排解時，請確定它並未啟動。 若要暫時停用排程器，請啟動 PowerShell `Set-ADSyncScheduler -SyncCycleEnabled $false`，然後執行。
...
>[!NOTE] 
> 在此情況下，請使用`AuthoritativeNull`而不是。 `Null` 這是因為非 null 值會取代 null 值，即使它具有較低的優先順序（規則中較高的數值）。 `AuthoritativeNull`相反地，其他規則不會以非 null 值來取代。 
...
我們先將部門從來源（Active Directory）轉換成小寫。 然後，使用`Left`函式，我們只會取得前三個字元，並與`hrd`它進行比較。 如果相符，則值會設定為`True`，否則`NULL`為。 將值設為 null 時，較低優先順序的其他規則（較高的數位值）可以使用不同的條件來寫入它。 在一個物件上執行預覽，以驗證同步處理規則，如[驗證同步處理規則](#validate-sync-rule)一節中所述。
...
請注意，在修改過的規則`msExchMailboxGuid`中，屬性會變更為**運算式**類型，而不是**直接**。 此外，此值也會變更為**Null**和**ExecuteOnce**選項。 您可以忽略已識別和優先順序的差異。 
