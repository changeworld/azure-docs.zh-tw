...
>[!NOTE] 
> `AuthoritativeNull`在此情況下，請使用而不是 `Null` 。 這是因為非 null 值會取代 null 值，即使它具有較低的優先順序（規則中較高的數值）。 `AuthoritativeNull`相反地，其他規則不會以非 null 值來取代。 
...
我們先將部門從來源（Active Directory）轉換成小寫。 然後，使用函式 `Left` ，我們只會取得前三個字元，並與它進行比較 `hrd` 。 如果相符，則值會設定為 `True` ，否則為 `NULL` 。 將值設為 null 時，較低優先順序的其他規則（較高的數位值）可以使用不同的條件來寫入它。 在一個物件上執行預覽，以驗證同步處理規則，如[驗證同步處理規則](#validate-sync-rule)一節中所述。
