...
* [重複或不正確屬性會防止 Microsoft 365 中的目錄同步作業](https://support.microsoft.com/kb/2647098)
...
* Microsoft 365 中建立已啟用郵件的安全性群組。 系統管理員會在內部部署 AD (中加入新的使用者或連絡人，而這些使用者或連絡人未同步處理至 Azure AD 但) 具有與 Microsoft 365 群組相同的 ProxyAddresses 屬性值。

#### <a name="example-case"></a>範例案例
1. 系統管理員會在 Microsoft 365 中為稅務部門建立新的已啟用郵件安全性群組，並提供電子郵件地址 tax@contoso.com 。 此群組已被指派 ProxyAddresses 屬性值 **smtp：稅金 \@ contoso.com**
2. 新的使用者會加入 Contoso.com，並為內部部署使用者建立帳戶，並以 proxyAddress 為 **smtp：稅務 \@ contoso.com**
...
#### <a name="related-articles"></a>相關文章
* [準備透過目錄同步作業將使用者布建至 Microsoft 365](https://support.office.com/article/Prepare-to-provision-users-through-directory-synchronization-to-Office-365-01920974-9e6f-4331-a370-13aea4e82b3e)
...
2. Bob 移至不同的 Contoso.com 部門，稱為 Fabrikam.com，其 UserPrincipalName 變更為 bob@fabrikam.com
...
#### <a name="related-articles"></a>相關文章
* [當您將使用者帳戶的 UPN 變更為使用不同的同盟網域之後，Azure Active Directory 同步工具不會同步變更](https://support.microsoft.com/help/2669550/changes-aren-t-synced-by-the-azure-active-directory-sync-tool-after-you-change-the-upn-of-a-user-account-to-use-a-different-federated-domain)
...
Azure AD Connect 不允許大致比對內部部署 AD 中的使用者物件與 Azure AD 中獲派系統管理角色的使用者物件。  如需詳細資訊，請參閱 [Azure AD UserPrincipalName 人口](plan-connect-userprincipalname.md)
...
1. 從所有系統管理員角色中移除 (擁有者) 的 Azure AD 帳戶。 
2. **實刪除** 雲端中的隔離物件。 
3. 下一個同步處理週期將會負責將內部部署使用者與雲端帳戶進行軟比對 (，因為雲端使用者現在已不再是全域 GA) 。 
4. 還原擁有者的角色成員資格。 
...
* [在 Active Directory 管理中心找出 Active Directory 物件](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/dd560661(v=ws.10))
* [如何使用 Azure Active Directory PowerShell 查詢 Azure Active Directory 中的物件](/previous-versions/azure/jj151815(v=azure.100))