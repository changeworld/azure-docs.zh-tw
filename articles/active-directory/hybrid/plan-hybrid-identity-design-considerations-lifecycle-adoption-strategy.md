...
Azure AD RBAC 使公司能夠限制個人訪問 Azure 門戶後可以執行的運算元。 使用 RBAC 來控制存取入口網站時，IT 系統管理員可以利用下列存取管理方法來委派存取：

* **基於組的角色指派**：您可以分配對可以從本地活動目錄同步的 Azure AD 組的訪問。 這可讓您運用組織目前在群組管理工具和程序方面所做的投資。 您也可以使用 Azure AD Premium 的委派群組管理功能。
* **運用 Azure 中內建的角色**：您可以使用三個角色 — 擁有者、參與者和讀者，以確保使用者和群組只擁有他們執行工作所需的權限。
* **細微性訪問資源**：您可以將角色指派給特定訂閱、資源組或單個 Azure 資源（如網站或資料庫）的使用者和組。 如此一來，您可以確保使用者能夠存取他們需要的資源，但不能存取他們不需要管理的資源。
...
Azure Active Directory 為數千個 SaaS 應用程式和內部部署 Web 應用程式提供單一登入和增強的應用程式存取安全性。 有關詳細資訊，請參閱[將應用程式與 Azure 活動目錄集成](../develop/quickstart-v1-integrate-apps-with-azure-ad.md)
...
* 管理員可以根據應用程式資源、設備和使用者標識、網路位置和多重要素驗證提供條件訪問。
