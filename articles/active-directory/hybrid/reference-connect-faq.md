...
**問：我可以將 Azure AD Connect 資料庫從本機資料庫移到遠端 SQL Server 實例嗎？**   
是，下列步驟將提供如何執行這項操作的一般指引。 我們目前正在製作更詳細的文件。
1. 備份 LocalDB ADSync 資料庫。
執行此操作最簡單的方式是使用與 Azure AD Connect 安裝在同一部機器上的 SQL Server Management Studio。 連接到 *（LocalDb） .\ADSync*，然後備份 ADSync 資料庫。
...
3. 針對現有的[遠端 SQL 資料庫](how-to-connect-install-existing-database.md)安裝 Azure AD Connect。
   本文將示範如何遷移至使用本機 SQL 資料庫。 如果您要遷移至使用遠端 SQL 資料庫，則在此程序的步驟 5 中，您也必須輸入現有服務帳戶作為 Windows 同步服務執行時的依據。 此同步引擎服務帳戶的描述如下：
      ...
      **使用現有的服務帳戶**：根據預設，Azure AD Connect 會使用虛擬服務帳戶，以供同步處理服務使用。 如果您是使用遠端 SQL Server 執行個體或需要驗證的 Proxy，請使用受控服務帳戶，或使用網域中知道密碼的服務帳戶。 在這類情況下，請輸入要使用的帳戶。 請確定執行安裝的使用者是 SQL 中的系統管理員，以便建立服務帳戶的登入認證。 如需詳細資訊，請參閱[Azure AD Connect 帳戶和許可權](reference-connect-accounts-permissions.md#adsync-service-account)。 
...
不可以。  為了讓 Azure AD Connect 自動建立 Azure AD 連接器帳戶，電腦必須已加入網域。  
...
不可以。 變更伺服器名稱會使同步引擎無法連線到 SQL 資料庫執行個體，並且無法啟動此服務。
...
不可以。 此選項不會擷取所有組態設定，因此不應使用。 應改用精靈在第二部伺服器上建立基底組態，並使用同步處理規則編輯器產生 PowerShell 指令碼，以在伺服器之間移動任何自訂規則。 如需詳細資訊，請參閱[變換移轉](how-to-upgrade-previous-version.md#swing-migration)。
...
若要確認您的伺服器上所安裝的 Azure AD Connect 版本，請移至 [控制台]，然後選取 [**程式** > ] [**程式和功能**] 來查詢已安裝的 Microsoft Azure AD Connect 版本，如下所示：
