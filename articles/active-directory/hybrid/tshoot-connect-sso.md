...
- 如果無縫 SSO 成功，使用者就沒有機會選取 [讓我保持登入]****。 由於此行為[，SharePoint 和 OneDrive 映射方案](https://support.microsoft.com/help/2616712/how-to-configure-and-to-troubleshoot-mapped-network-drives-that-connec)不起作用。
...
- 將 Azure AD 服務`https://autologon.microsoftazuread-sso.com`URL （ ） 添加到受信任的網站區域，而不是本地 Intranet 區域*阻止使用者登錄*。
- 無縫 SSO 支援 Kerberos 的AES256_HMAC_SHA1、AES128_HMAC_SHA1和RC4_HMAC_MD5加密類型。 建議將 AzureADSSOAcc$ 帳戶的加密類型設置為AES256_HMAC_SHA1，或 AES 類型與 RC4 的加密類型之一，以增加安全性。 加密類型存儲在活動目錄中帳戶的 msDS 支援的加密類型屬性上。  如果 AzureADSSOAcc$ 帳戶加密類型設置為RC4_HMAC_MD5，並且要將其更改為 AES 加密類型之一，請確保首先滾動 AzureADSSOAcc$ 帳戶的 Kerberos 解密金鑰，如常見問題文檔中在相關問題下的[常見問題文檔中](how-to-connect-sso-faq.md)所述，否則不會發生無縫 SSO。
...
流覽到[Azure 活動目錄管理中心](https://aad.portal.azure.com/)中的**Azure 活動目錄** > **登錄**，然後選擇特定使用者的登錄活動。 尋找 [登入錯誤碼]**** 欄位。 使用下表，將該欄位的值對應至失敗原因和解決方式：
...
- 確保 Azure AD URL`https://autologon.microsoftazuread-sso.com`（ ） 是使用者的 Intranet 地區設定的一部分。
