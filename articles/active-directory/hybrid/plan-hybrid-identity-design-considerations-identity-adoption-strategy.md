...
在這項工作中，您將定義要使用的多重要素驗證策略。  Azure AD Multi-Factor Authentication 有兩個不同的版本。  其中一個是雲端型，另一個是使用 Azure MFA Server 的內部部署型。  根據您以上所做的評估，您可以判斷何者是策略的正確解決方案。  使用下表來判斷哪一個設計選項最符合公司的安全性需求：
