...
* **設定 Azure AD Connect 伺服器**：如果您使用[傳遞驗證](how-to-connect-pta.md)作為登入方法，不需要進行額外的必要條件檢查。 如果您使用[密碼雜湊同步處理](how-to-connect-password-hash-synchronization.md)作為登入方法，而且 Azure AD Connect 與 Azure AD 之間有防火牆，請確定︰
   ...
   - 如果您的防火牆或代理允許 DNS 白名單，請將連接到**\*.msappproxy.net** URL 的連接列入埠 443。 如果不允許建立，請允許存取每週更新的 [Azure 資料中心 IP 範圍](https://www.microsoft.com/download/details.aspx?id=41653)。 只有啟用此功能時，此必要條件才適用。 不需要實際的使用者登入。

    >[!NOTE]
    >Azure AD Connect 版本 1.1.557.0、1.1.558.0、1.1.561.0 和 1.1.614.0 具有與密碼雜湊同步處理相關的問題。 如果您_不打算_將密碼雜湊同步與直通身份驗證結合使用，請閱讀[Azure AD 連接版本資訊](https://docs.microsoft.com/azure/active-directory/connect/active-directory-aadconnect-version-history#116470)以瞭解更多資訊。
...
>[!IMPORTANT]
> 無縫 SSO 在每個 AD`AZUREADSSOACC`林中創建在本地活動目錄 （AD） 中命名的電腦帳戶。 出於`AZUREADSSOACC`安全原因，需要對電腦帳戶進行強力保護。 只有域管理員才能管理電腦帳戶。 確保禁用電腦帳戶上的 Kerberos 委派，並且 Active Directory 中沒有其他帳戶對`AZUREADSSOACC`電腦帳戶具有委派許可權。 將電腦帳戶存儲在組織單位 （OU），其中電腦帳戶可以免受意外刪除，並且只有域管理員有權訪問。

>[!NOTE]
> 如果在本地環境中使用"傳遞雜湊"和"憑據防盜緩解"體系結構，請進行適當的更改，以確保`AZUREADSSOACC`電腦帳戶不會最終進入隔離容器。 
...
3. 流覽到**使用者配置** > **策略** > **管理範本** > **Windows 元件** > **Internet 資源管理器** > **互聯網控制台** > **安全頁面**。 然後選取 [指派網站到區域清單]****。
...
6. 流覽到**使用者配置** > **策略** > **管理範本** > **Windows 元件** > Internet**資源管理器** > **互聯網控制台** > **安全頁面** > **內聯網區域**。 然後選取 [允許透過指令碼更新狀態列]****。
...
1. 開啟群組原則管理編輯器工具。
2. 編輯套用至部分或所有使用者的群組原則。 此範例使用**預設網域原則**。
3. 流覽到**使用者配置** > **首選項** > **視窗設置** > **註冊表** > **新** > **登錄機碼**。
...
如果已覆蓋環境中的[AuthNegotiate委派允許清單](https://docs.microsoft.com/DeployEdge/microsoft-edge-policies#authnegotiatedelegateallowlist)或[AuthServerAllowlist](https://docs.microsoft.com/DeployEdge/microsoft-edge-policies#authserverallowlist)策略設置，請確保向它們添加 Azure AD 的 URL`https://autologon.microsoftazuread-sso.com`（）。

#### <a name="microsoft-edge-based-on-chromium-macos-and-other-non-windows-platforms"></a>基於鉻（macOS 和其他非 Windows 平臺）的微軟邊緣

對於基於 Mac OS 和其他非 Windows 平臺上的鉻的 Microsoft Edge，請參閱[基於鉻策略清單的 Microsoft Edge，](https://docs.microsoft.com/DeployEdge/microsoft-edge-policies#authserverallowlist)瞭解如何將 Azure AD URL 添加到允許清單的集成身份驗證。
...
無縫 SSO 無法在 Firefox 和 Microsoft Edge 瀏覽器的隱私瀏覽模式中運作。 如果瀏覽器是在「增強保護」模式中執行，它也無法在 Internet Explorer 上運作。 對於基於鉻的下一個版本的 Microsoft Edge，它將不會在設計中在 InPrivate 和 Guest 模式下工作。
