...
**問：我管理多個 Azure AD 目錄。如何切換到具有 Azure 活動目錄高級版的目錄？**

若要在不同的 Azure AD 租用戶之間切換，請在右上角選取目前登入的 [使用者名稱]****，然後選擇適當的帳戶。 如果此處未列出該帳戶，請選擇 **"登出**"，然後使用啟用 Azure 活動目錄高級版登錄的目錄的全域管理員憑據。
...
閱讀有關[為運行狀況代理配置 HTTP 代理](how-to-connect-health-agent-install.md#configure-azure-ad-connect-health-agents-to-use-http-proxy)的更多資訊。

如果需要在代理程式註冊期間設定 Proxy，您可能需要預先修改 Internet Explorer 的 Proxy 設定。

1. 打開互聯網瀏覽器>**設置** > **互聯網選項** > **連接** > **局域網設置**。
2. 選擇**使用代理伺服器進行 LAN。**
3. 如果 HTTP 和 HTTPS/安全具有不同的代理埠，請選擇 **"高級**"。
...
有關防火牆埠和其他連接要求的清單，請參閱[要求部分](how-to-connect-health-agent-install.md#requirements)。

**問：為什麼在 Azure AD 連接運行狀況門戶中看到兩個同名伺服器？**
...
* 因為防火牆封鎖流量，代理程式無法與所需端點通訊。 在 Web 應用程式 Proxy 伺服器上尤其常見。 請確定您已允許針對所需端點和連接埠的輸出通訊。 有關詳細資訊，請參閱[需求部分](how-to-connect-health-agent-install.md#requirements)。
* 出站通信接受網路層的 TLS 檢查。 這會導致代理程式所使用的憑證遭到檢查伺服器/實體所取代，完成代理程式註冊的步驟便會失敗。
...
**問：我收到警報，"測試身份驗證請求（綜合交易）未能獲取權杖。如何疑難排解？**
...
**問：我收到一封電子郵件，指示我的機器沒有修補最近發生的勒索軟體攻擊。為什麼我收到此電子郵件？**
