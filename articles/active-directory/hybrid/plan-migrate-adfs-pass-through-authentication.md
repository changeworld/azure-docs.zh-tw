---
title: Azure AD 連接：從聯合遷移到 Azure AD 的 PTA
...
> [!NOTE]
> 更改身份驗證方法需要規劃、測試和潛在的停機時間。 [分階段推出](how-to-connect-staged-rollout.md)提供了一種使用傳遞身份驗證測試和逐步從識別身分同盟遷移到雲身份驗證的替代方法。
...
對大多數客戶來說，二或三個驗證代理程式即足以提供高可用性和必要的容量。 一個租用戶最多可註冊 12 個代理程式。 第一個代理程式一律安裝於 Azure AD Connect 伺服器本身。 要瞭解代理限制和代理部署選項，請參閱[Azure AD 直通身份驗證：當前限制](https://docs.microsoft.com/azure/active-directory/connect/active-directory-aadconnect-pass-through-authentication-current-limitations)。

### <a name="plan-the-migration-method"></a>規劃移轉方法

要從同盟身分識別管理移轉至傳遞驗證和無縫單一登入 (SSO)，您可以從兩種方法中擇一使用。 所應使用的方法取決於您 AD FS 執行個體原本的設定方式。

* **Azure AD 連接**。 如果您原本使用 Azure AD Connect 來設定 AD FS，則「必須」** 使用 Azure AD Connect 精靈來變更為傳遞驗證。
...
3. 在 **"管理聯合>其他任務**下，滾動到**活動目錄聯合服務 （AD FS）。**<br />

   * 如果此區段中出現 AD FS 設定，您就可以放心假設 AD FS 原本是使用 Azure AD Connect 進行設定的。 您可以使用 Azure AD Connect 的 [變更使用者登入]**** 選項，將網域從同盟身分識別轉換為受控識別。 有關該過程的詳細資訊，請參閱**選項 A：使用 Azure AD 連接配置直通身份驗證**。
   * 如果目前的設定未列出 AD FS，則必須手動使用 PowerShell 將網域從同盟身分識別轉換為受控識別。 有關此過程的詳細資訊，請參閱**選項 B：使用 Azure AD 連接和 PowerShell 從聯合切換到傳遞身份驗證**。
...
### <a name="step-1-prepare-for-seamless-sso"></a>第 1 步：為無縫 SSO 做好準備
...
### <a name="step-2-change-the-sign-in-method-to-pass-through-authentication-and-enable-seamless-sso"></a>第 2 步：將登錄方法更改為傳遞身份驗證並啟用無縫 SSO

您可以透過兩個選項將登入方法變更成傳遞驗證，並啟用無縫 SSO。

#### <a name="option-a-configure-pass-through-authentication-by-using-azure-ad-connect"></a>選項 A：使用 Azure AD 連接配置直通身份驗證
...
6. 在 [已可設定]**** 頁面上，確定已選取 [在設定完成時開始同步處理程序]**** 核取方塊。 然後，選擇 **"配置**"。<br />
...
1. 在 Azure 門戶中，轉到**Azure 活動目錄** > **Azure AD 連接**，然後選擇**傳遞身份驗證**。
...
> [!IMPORTANT]
> 跳過選項**B 部分：使用 Azure AD 連接和 PowerShell 從聯合切換到傳遞身份驗證**。 如果您選擇了選項 A，將登入方法變更成傳遞驗證並啟用無縫 SSO，則不適用該節提供的步驟。 

#### <a name="option-b-switch-from-federation-to-pass-through-authentication-by-using-azure-ad-connect-and-powershell"></a>選項 B：使用 Azure AD 連接和 PowerShell 從聯合切換到傳遞身份驗證
