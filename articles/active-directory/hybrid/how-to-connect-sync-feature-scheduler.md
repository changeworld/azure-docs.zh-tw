...
本主題介紹 Azure AD 連接同步（同步引擎）中的內置計畫程式。
...
* **維護任務**。 為密碼重設和「裝置註冊服務」(DRS) 更新金鑰和憑證。 清除作業記錄檔中的舊項目。
...
* **自訂同步週期間隔**。 如果您想要讓排程器以預設值 30 分鐘以外的任何其他頻率執行，那麼您要設定此設定。 在上圖中，已將排程器改為設定成每小時執行一次。 如果您將此設定設定成低於 AllowedSyncInterval 的值，則會使用後者。
* **下一個同步週期策略類型**。 Delta (差異) 或 Initial (初始)。 定義下一次運行是否僅應處理增量更改，或者下一次運行是否應執行完整匯入和同步。後者還將重新處理任何新的或更改的規則。
* **NextSyncCycleStartTimeInUTC**。 排程器下一次啟動下一個同步處理循環的時間。
* **清除運行歷史間隔**。 應該保留作業記錄的時間。 您可以在同步處理服務管理員中檢閱這些記錄。 這些記錄預設會保留 7 天。
...
### <a name="full-sync-cycle"></a>完整同步處理循環
完整同步處理循環包含下列步驟：
...
如果需要手動運行同步週期，則從 PowerShell 運行`Start-ADSyncSyncCycle -PolicyType Delta`。

若要起始完整同步處理循環，請從 PowerShell 提示字元執行 `Start-ADSyncSyncCycle -PolicyType Initial` 。   

運行完全同步週期可能非常耗時，請閱讀下一節以瞭解如何優化此過程。

### <a name="sync-steps-required-for-different-configuration-changes"></a>同步不同配置更改所需的步驟
不同的配置更改需要不同的同步步驟，以確保更改正確應用於所有物件。

- 添加更多要從原始目錄導入的物件或屬性（通過添加/修改同步規則）
    - 該原始目錄的連接器上需要完整匯入
- 對同步處理規則進行變更
    - 更改的同步規則需要連接器上的完整同步
- 變更 [篩選](how-to-connect-sync-configure-filtering.md) 以納入不同數目的物件
    - 每個 AD 連接器的連接器上都需要完整匯入，具體取決於已導入同步引擎的屬性，具體取決於屬性

### <a name="customizing-a-sync-cycle-run-the-right-mix-of-delta-and-full-sync-steps"></a>自訂同步週期可運行增量和完全同步步驟的正確組合
為了避免運行完全同步週期，您可以標記特定的連接器以使用以下 Cmdlet 運行完整步驟。
...
示例：如果對連接器"AD 林 A"的同步規則進行了更改，並且不需要導入任何新屬性，則運行以下 Cmdlet 以運行增量同步週期，該週期還對該連接器執行完全同步步驟。

`Set-ADSyncSchedulerConnectorOverride -ConnectorName “AD Forest A” -FullSyncRequired $true`

`Start-ADSyncSyncCycle -PolicyType Delta`

示例：如果對連接器"AD 林 A"的同步規則進行了更改，以便它們現在需要導入新屬性，則運行以下 Cmdlet 以運行增量同步週期，該週期還對該連接器執行完整匯入、完全同步步驟。
...
2. 如果您使用 1.1.281 之前的組件，停止排程器並不會阻止目前的連接器進行其目前的工作。 若要強制停止連接器，請採取下列動作：![StopAConnector](./media/how-to-connect-sync-feature-scheduler/stopaconnector.png)
   * 從開始功能表開始**同步服務**。 轉到**連接器**，突出顯示狀態**為"正在運行的**連接器"，然後從"操作 **"中選擇"停止**"。
