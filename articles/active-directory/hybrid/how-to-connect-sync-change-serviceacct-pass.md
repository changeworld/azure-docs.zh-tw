---
title: Azure AD 連接同步：更改 ADSync 服務帳戶 |微軟文檔
...
---
# <a name="changing-the-adsync-service-account-password"></a>更改 ADSync 服務帳戶密碼
如果您更改 ADSync 服務帳戶密碼，同步服務將無法正確啟動，直到您放棄加密金鑰並重新初始化 ADSync 服務帳戶密碼。 

作為同步服務的一部分，Azure AD 連接使用加密金鑰來存儲 AD DS 連接器帳戶和 ADSync 服務帳戶的密碼。  這些帳戶會先加密再儲存到資料庫中。 

所使用的加密金鑰會使用 [Windows 資料保護 (DPAPI)](https://msdn.microsoft.com/library/ms995355.aspx) 來提供保護。 DPAPI 使用**ADSync 服務帳戶**保護加密金鑰。 

如果需要更改服務帳戶密碼，可以使用[放棄 ADSync 服務帳戶加密金鑰](#abandoning-the-adsync-service-account-encryption-key)中的過程來實現此目的。  如果您基於任何原因而需要放棄加密金鑰，您也應該使用這些程序。
...
- 在 Windows 服務控制管理員下，如果您嘗試啟動同步服務，但無法檢索加密金鑰，則失敗，並且錯誤<strong>"Windows 無法在本地電腦上啟動 Microsoft Azure AD 同步。有關詳細資訊，請查看系統事件日誌。如果這是非 Microsoft 服務，請與服務供應商聯繫，並參考特定于服務的錯誤代碼 -21451857952</strong>。
- 在 Windows 事件檢視器下，應用程式事件日誌包含事件**ID 6028**和錯誤訊息 *"無法訪問伺服器加密金鑰"的錯誤。*

為確保不會收到這些錯誤，請按照[放棄 ADSync 服務帳戶加密金鑰](#abandoning-the-adsync-service-account-encryption-key)的過程更改密碼。
...
由於資料庫中存儲的現有密碼無法再解密，您需要向同步服務提供 AD DS 連接器帳戶的密碼。 同步處理服務會使用新的加密金鑰來將密碼加密︰
