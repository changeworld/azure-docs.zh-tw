---
title: 維修通知
description: 在 Azure 中運行的虛擬機器的維護通知概述。
author: shants123
...
---
# <a name="handling-planned-maintenance-notifications"></a>處理計畫的維護通知

為提升虛擬機器之主機基礎結構的可靠性、效能和安全性，Azure 會定期執行更新。 更新會變更，例如裝載環境的修補或將硬體升級與解除委任。 大多數更新都已完成，對託管虛擬機器沒有任何影響。 不過，更新在某些情況下確實會造成影響：

- 如果維護不需要重新開機，Azure 會在主機更新時使用就地移轉來暫停 VM。 這些類型的維護操作按容錯域應用容錯域。 如果收到任何警告健康情況訊號，就會停止進度。

- 如果維護需要重新開機，您會在規劃維護時收到通知。 您有大約 35 天的時間視窗，您可以在維護工作時自行開始維護。


預定進行的維護作業若需要重新開機，會排定在不同波段。 每一波段有不同的範圍 (區域)。

- 波段開始時會傳送通知給客戶。 預設情況下，通知將發送給服務管理員和共同管理員。 您可以使用[活動日誌警報](../service-health/alerts-activity-log-service-notifications.md)添加更多收件者和消息選項，如電子郵件、短信和網路掛號。  
- 通知發出後，自助服務*視窗*將可用。 在此視窗期間，您可以查詢哪些虛擬機器受到影響，並根據您自己的計畫需求開始維護。 自助服務視窗通常約為 35 天。
...
對於使用**可用性集**的部署，不建議進行自助服務維護。 可用性集一次只能更新一個更新域。 

- 讓 Azure 觸發維護工作。 對於需要重新開機的維護，將通過更新域完成維護。 更新域不一定按順序接收維護，更新域之間有 30 分鐘的暫停。 
- 如果某些容量（1 更新域）暫時丟失是一個問題，則可以在維護期間添加實例。 
...
- 兩個更新域 （UD） 之間需要超過 30 分鐘的 VM 恢復時間。 若要控制更新網域之間的時間，您必須在虛擬機器上觸發維護 (一次一個更新網域 (UD))。
...
有關高可用性的詳細資訊，請參閱[Azure 中的虛擬機器的可用性](./linux/availability.md)。

**問：我如何取得規劃維護的通知？**

**答：** 規劃的維護是從對一或多個 Azure 區域設定排程開始。 不久之後，將通過電子郵件通知發送給訂閱管理員（每個訂閱一封電子郵件）。 可以使用「活動記錄警示」來設定此通知的其他通道和收件者。 如果您將虛擬機器部署到已排程規劃的維護之區域，您不會收到通知，但是需要檢查 VM 的維護狀態。

**問：在門戶、Powershell 或 CLI 中，我看不到任何計畫維護的跡象。怎麼了？**
...
**答：** 計畫維護現在可用於虛擬機器規模集。 有關如何啟動自助服務維護的說明，請參閱[虛擬機器規模集文檔的計畫維護](../virtual-machine-scale-sets/virtual-machine-scale-sets-maintenance-notifications.md)。
...
您可以使用[Azure CLI、Azure](maintenance-notifications-cli.md) [PowerShell](maintenance-notifications-powershell.md)或[門戶](maintenance-notifications-portal.md)來處理計畫維護。
