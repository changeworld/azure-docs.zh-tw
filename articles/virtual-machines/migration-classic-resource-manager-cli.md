...
description: 本文將逐步解說平臺支援的資源從傳統遷移至 Azure Resource Manager，方法是使用 Azure CLI。
author: tanmaygore
manager: vashan
...
* 將 [不支援的組態或功能清單](migration-classic-resource-manager-overview.md)看一遍。 如果您的虛擬機器使用不支援的組態或功能，建議您等到宣布支援該功能/組態之後，再進行移轉。 或者，您可以移除該功能或移出該組態，以利移轉進行 (如果這麼做符合您的需求)。
