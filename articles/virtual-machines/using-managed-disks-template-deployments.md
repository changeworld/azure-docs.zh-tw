---
title: 使用 Azure Resource Manager 範本部署磁片
description: 詳細說明如何在 Azure Vm 的 Azure Resource Manager 範本中使用受控和非受控磁片。
documentationcenter: ''
author: jboeshart
manager: ''
...
ms.locfileid: "91976654"
---
# <a name="using-disks-in-azure-resource-manager-templates"></a>使用 Azure Resource Manager 範本中的磁片
...
若要建立具有受控磁片的 VM，您不再需要建立儲存體帳戶資源。 參考以下的範本範例，與先前的非受控磁片範例有一些差異要注意：

- `apiVersion`是支援受控磁片的版本。
- `osDisk` 而且 `dataDisks` 不再參考 VHD 的特定 URI。
- 部署時若未指定其他屬性，磁片將會使用以 VM 大小為基礎的儲存體類型。 例如，如果您使用支援 premium 儲存體的 VM 大小 (其名稱中有 "s" 的大小，例如 Standard_D2s_v3) 則會預設設定高階磁片。 您可以使用磁片的 sku 設定來指定儲存體類型，以變更此設定。
- 如果未指定磁片的名稱，則會採用 `<VMName>_OsDisk_1_<randomstring>` OS 磁片和 `<VMName>_disk<#>_<randomstring>` 每個資料磁片的格式。
  - 如果從自訂映射建立 VM，則會從自訂映射資源中定義的磁片屬性抓取儲存體帳戶類型和磁片名稱的預設設定。 您可以藉由在範本中指定這些值來覆寫這些值。
- 預設會停用 Azure 磁片加密。
- 依預設，磁碟快取是 OS 磁片的讀取/寫入，而不是資料磁片的磁碟快取。
- 在下列範例中，仍有儲存體帳戶相依性，但這只是為了儲存診斷，且不需要磁片儲存體。
...
* 如需如何在 Azure 虛擬機器擴展集內使用受控磁碟的相關資訊，請瀏覽[使用資料磁碟搭配擴展集](../virtual-machine-scale-sets/virtual-machine-scale-sets-attached-disks.md)文件。