...
1. 選取 [+ 新增] 。
    ...
    :::image type="content" source="media/virtual-machines-disks-double-encryption-at-rest-portal/double-encryption-add-disk-encryption-set.png" alt-text="主要 Azure 入口網站的螢幕擷取畫面，在搜尋列中會反白顯示磁片加密集。":::
...
1. 填寫剩餘的資訊。
    ...
    :::image type="content" source="media/virtual-machines-disks-double-encryption-at-rest-portal/double-encryption-create-disk-encryption-set-blade.png" alt-text="主要 Azure 入口網站的螢幕擷取畫面，在搜尋列中會反白顯示磁片加密集。":::
...
1. 選取 Azure Key Vault 和金鑰，或視需要建立一個新的金鑰。
    ...
    :::image type="content" source="media/virtual-machines-disks-double-encryption-at-rest-portal/double-encryption-select-key-vault.png" alt-text="主要 Azure 入口網站的螢幕擷取畫面，在搜尋列中會反白顯示磁片加密集。":::

1. 選取 [建立]****。
1. 流覽至您所建立的磁片加密集，然後選取所顯示的錯誤。 這會將您的磁片加密集設定為可運作。

    :::image type="content" source="media/virtual-machines-disks-double-encryption-at-rest-portal/double-encryption-disk-set-error.png" alt-text="主要 Azure 入口網站的螢幕擷取畫面，在搜尋列中會反白顯示磁片加密集。":::
...
1. 選取 [儲存]。
    ...
    :::image type="content" source="media/virtual-machines-disks-double-encryption-at-rest-portal/double-encryption-enable-disk-blade.png" alt-text="主要 Azure 入口網站的螢幕擷取畫面，在搜尋列中會反白顯示磁片加密集。":::
