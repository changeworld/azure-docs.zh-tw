...
description: 概述 Azure 中 VM 的生命週期，包括 VM 隨時可以處於的各種狀態的說明。
...
&#42; 部分 Azure 資源（例如磁片和網路）會產生費用。 執行個體上的軟體授權不會產生費用。
...
:::row:::
   ...
   **OSProvisioningComplete**
   ...
   短暫的狀態。 除非需要安裝任何擴充功能，否則 VM 會快速過渡為**成功**狀態。 安裝擴充功能需要一些時間。
...
執行個體檢視 API 會提供 VM 的執行狀態資訊。 如需詳細資訊，請參閱[虛擬機器 - 執行個體檢視](/rest/api/compute/virtualmachines/instanceview) API。
