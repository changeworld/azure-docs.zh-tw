...
大部分的散發套件都會提供 `fstab` **nofail** 或 **nobootwait** 參數。 即使磁碟在啟動時無法掛接，這些參數也能讓系統開機。 請檢查散發版本的文件，以取得這些參數的相關資訊。 如需如何在您新增資料磁碟時設定 Linux VM 以使用 UUID 的詳細資訊，請參閱[連接到 Linux VM 以掛接新磁碟](../linux/add-disk.md#format-and-mount-the-disk)。

在 VM 上安裝 Azure Linux 代理程式時，代理程式會使用 Udev 規則，在 /dev/disk/azure 路徑下方建構一組符號連結。 應用程式與指令碼會使用 Udev 規則，來識別連接至 VM 的磁碟，以及磁碟類型與磁碟 LUN。

如果您已編輯 fstab，使 VM 無法開機，且您無法透過 SSH 連線至您的 VM，您可以使用 [VM 序列主控台](./serial-console-linux.md) 來進入 [單一使用者模式](./serial-console-grub-single-user-mode.md) ，並修改您的 fstab。
