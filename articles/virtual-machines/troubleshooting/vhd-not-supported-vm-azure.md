...
description: 本文可協助您在 Microsoft Azure 中執行虛擬機器時修正 VHD 錯誤。
...
ms.locfileid: "88653489"
---
# <a name="vhd-is-not-supported-when-you-create-a-virtual-machine-in-azure"></a>當您在 Azure 中建立虛擬機器時，不支援 VHD

本文可協助您在 Windows 或 Linux 中執行虛擬機器時修正 VHD 錯誤。

## <a name="symptoms"></a>徵兆

當您使用上傳的 VHD 在 Microsoft Azure 中建立虛擬機器時，部署會失敗並傳回下列錯誤訊息： 
...
> [!NOTE]
> 若要執行下列修正，客戶必須在將 VHD 上傳至 Azure 之前，先執行這些步驟。

若要解決此問題，請調整磁片大小以符合 1 MB 的對齊：

- 若要解決 Windows 的問題，請使用重 [設大小 VHD PowerShell Cmdlet](/powershell/module/hyper-v/resize-vhd)。 請注意，重 **設大小 VHD** 不是 Azure PowerShell Cmdlet。
...
繼續問題可能表示 VHD 損毀。 在此情況下，我們建議您從頭重建 VHD。

如需詳細資訊，請參閱下列文章：

- [關於 VHD](../managed-disks-overview.md)
