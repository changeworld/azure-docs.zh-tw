...
1. 選擇您修復的新磁片，然後輸入 VM 的名稱以確認變更。 如果您在清單中看不到磁片，請在從疑難排解 VM 卸離磁片之後等候 10 ~ 15 分鐘。 也請確定磁片與 VM 位於相同的位置。
1. 選取 [確定]。
