...
有關可用性集的詳細資訊，請參閱管理[VM 的可用性](../windows/manage-availability.md)
...
- [如何計畫 Azure VM 上的計畫維護](../windows/classic/planned-maintenance-schedule.md)
...
如果從 Azure 門戶、Azure PowerShell、命令列介面或 REST API 執行重新開機，則可以在[Azure 活動日誌](../../azure-monitor/platform/platform-logs-overview.md)中找到該事件。
...
當 I/O 要求因為每秒 I/O 作業次數 (IOPS) 超出磁碟的 I/O 限制而一直節流時，VM 可能會暫時關閉。 （標準磁片存儲限制為 500 IOPS。要緩解此問題，請使用磁片條帶化或配置來賓 VM 內的存儲空間，具體取決於工作負載。 如需詳細資訊，請參閱[設定 Azure VM 以達到最佳的儲存體效能](https://blogs.msdn.com/b/mast/archive/2014/10/14/configuring-azure-virtual-machines-for-optimal-storage-performance.aspx)。
