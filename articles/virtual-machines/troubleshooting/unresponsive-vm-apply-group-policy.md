...
description: 本文提供的步驟可解決在 Azure VM 中啟動期間套用原則時，負載畫面沒有回應的問題。
...
本文提供的步驟可解決當 Azure 虛擬機器（VM）在啟動期間套用原則時，負載畫面沒有回應的問題。
...
1. [啟用序列主控台和記憶體傾印集合](#step-3-enable-serial-console-and-memory-dump-collection)
...
1. 找出**HKEY_LOCAL_MACHINE**的金鑰， **File**然後  >  從功能表中選取 [檔案] [**載入 Hive** ]。
    ...
    - 您可以使用 [載入 Hive] 從離線系統載入登錄機碼。 在此情況下，系統是連接到修復 VM 的中斷磁片。
...
1. 請移至 BROKENSOFTWARE，並檢查載入的 hive 中是否有 Cleanupprofile 機密鑰。

    1. 如果金鑰存在，則會設定 Cleanupprofile 機原則。 其值代表以天為單位的保留原則。 繼續刪除機碼。
...
### <a name="step-3-enable-serial-console-and-memory-dump-collection"></a>步驟3：啟用序列主控台和記憶體傾印集合
...
1. 確認 OS 磁片上的可用空間是否至少等於 VM 的記憶體大小（RAM）。
    ...
    如果 OS 磁片上沒有足夠的空間，請變更記憶體傾印位置，並將它參考到連接的資料磁片，並提供足夠的可用空間。 若要變更位置，請在下列命令中使用資料磁片的磁碟機號（例如 "F："）來取代 "% SystemRoot%"。
