...
* 針對[以 Linux 為基礎的 Azure 虛擬機器安全殼層 (SSH) 連線進行疑難排解](troubleshoot-ssh-connection.md)。
...
## <a name="step-2-access-application-from-another-vm-in-the-same-virtual-network"></a><a id="step2"></a>步驟 2︰從相同虛擬網路中的其他 VM 存取應用程式
...
## <a name="step-3-access-application-from-outside-the-virtual-network"></a><a id="step3"></a>步驟 3︰ 從虛擬網路外部存取應用程式
