...
1. 透過 RDP 連線到您的虛擬機器。 
1. 使用 [檔案**瀏覽器**] `c:\windows\system32\drivers\etc`移至。 
1. 以滑鼠右鍵按一下**hosts**檔案，然後使用 [**記事本**] （或任何其他文字編輯器）開啟檔案。
1. 在檔案結尾，以`IP Address, DNS Suffix #comment` like 的形式新增每個節點、可用性群組和接聽程式的專案： 
...
1. 在其中一個節點上，開啟**容錯移轉叢集管理員**，選取叢集，以滑鼠右鍵按一下 [叢集**核心資源**] 底下的 [ ** \<名稱： ClusterNam>** ]，然後選取 [**屬性**]。 
...
在此步驟中，建立 SQL 登入在加密端點上使用的憑證。 在每個節點上建立資料夾來保存憑證備份，例如`c:\certs`。 

若要設定第一個節點，請遵循下列步驟： 

1. 開啟**SQL Server Management Studio**並連接到您的第一個節點， `AGNode1`例如。 
...
1. 使用 [檔案**瀏覽器**] 移至您的憑證所在的檔案位置， `c:\certs`例如。 
1. 手動從第一個節點建立憑證的複本（ `AGNode1Cert.crt`例如），並將它傳輸到第二個節點上的相同位置。 

若要設定第二個節點，請遵循下列步驟： 

1. 使用**SQL Server Management Studio**（例如）連接到第二個`AGNode2`節點。 
...
1. 使用 [檔案**瀏覽器**] 移至您的憑證所在的檔案位置， `c:\certs`例如。 
1. 手動從第二個節點複製一份憑證`AGNode2Cert.crt`，例如，然後將它傳輸到第一個節點上的相同位置。 
...
在此步驟中，請設定您的可用性群組，並在其中新增您的資料庫。 此時請勿建立接聽程式。 如果您不熟悉這些步驟，請參閱[可用性群組教學](virtual-machines-windows-portal-sql-availability-group-tutorial.md#create-the-availability-group)課程。 請務必起始容錯移轉和容錯回復，以確認所有專案都正常運作。 
   ...
   > [!NOTE]
   > 如果在同步處理過程中發生失敗，您可能需要授`NT AUTHORITY\SYSTEM`與系統管理員許可權，以便在第一個節點上建立叢集資源， `AGNode1`例如暫時。 
