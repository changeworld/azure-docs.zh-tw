...
> [!NOTE]
> 自動備份依賴**SQL Server IaaS 代理程式擴充**功能。 目前的 SQL 虛擬機器資源庫映像預設會新增這項擴充。 如需詳細資訊，請參閱 [SQL Server IaaS 代理程式擴充](virtual-machines-windows-sql-server-agent-extension.md)。
...
若要啟用加密，請修改先前的腳本，將**EnableEncryption**參數和密碼（安全字串）傳遞給**CertificatePassword**參數。 下列指令碼會啟用上述範例中的自動備份設定，並新增加密。
