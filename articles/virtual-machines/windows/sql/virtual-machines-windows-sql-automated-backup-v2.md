...
> * [SQL 伺服器 2016/2017](virtual-machines-windows-sql-automated-backup-v2.md)
...
- 目標資料庫必須位於預設 SQL Server 實例或[正確安裝](virtual-machines-windows-sql-server-iaas-faq.md#administration)的命名實例上。 

> [!NOTE]
> 自動備份依賴于 SQL **Server IaaS 代理擴展**。 目前的 SQL 虛擬機器資源庫映像預設會新增這項擴充。 如需詳細資訊，請參閱 [SQL Server IaaS 代理程式擴充](virtual-machines-windows-sql-server-agent-extension.md)。
...
| **保留期** | 1-30 天 (30 天) | 保留備份的天數。 |
| **存儲帳戶** | Azure 儲存體帳戶 | 將自動備份檔案儲存在 Blob 儲存體中時，所使用的 Azure 儲存體帳戶。 這個位置會建立一個容器來儲存所有備份檔案。 備份檔案命名慣例包括日期、時間和資料庫 GUID。 |
| [加密]**** |啟用/停用 (已停用) | 啟用或停用加密。 啟用加密時，用來還原備份的憑證會放在指定的儲存體帳戶中。 它使用相同的**自動備份**容器和相同的命名約定。 如果密碼變更，就會以該密碼產生新的憑證，但是舊的憑證還是會保留，以還原先前的備份。 |
...
| **備份計畫** | 手動/自動化 (自動化) | 預設會根據記錄的成長情況自動決定備份排程。 手動備份排程可讓使用者指定備份的時間範圍。 在此情況下，只會以指定的頻率且在特定一天的指定時間範圍期間進行備份。 |
| **完全備份頻率** | 每天/每週 | 完整備份的頻率。 在這兩種情況下，完整備份都會在下一個排定的時間範圍期間開始進行。 選取 [每週] 時，備份可以跨多天，直到所有資料庫都已順利備份為止。 |
| **完整備份開始時間** | 00:00 – 23:00 (01:00) | 可進行完整備份的特定一天開始時間期間。 |
| **完全備份時間視窗** | 1 – 23 小時 (1 小時) | 可進行完整備份的特定一天時間範圍持續時間。 |
...
在**SQL 伺服器設置**選項卡中，選擇 **"自動備份**下**啟用**"。 下列的 Azure 入口網站螢幕擷取畫面顯示 [SQL 自動備份]**** 設定。
...
對於現有的 SQL Server 虛擬機器，導航到[SQL 虛擬機器資源](virtual-machines-windows-sql-manage-portal.md#access-the-sql-virtual-machines-resource)，然後選擇 **"備份**"以配置自動備份。

![現有 VM 的 SQL 自動備份](./media/virtual-machines-windows-sql-automated-backup-v2/sql-server-configuration.png)


完成後，按一下 **"備份**設置"頁底部的 **"應用**"按鈕以保存更改。
...
首先，為備份檔案選擇或創建存儲帳戶。 以下指令碼會選取或建立儲存體帳戶 (如果不存在)。
...
要啟用加密，請修改以前的腳本以傳遞**啟用加密**參數以及**證書密碼**參數的密碼（安全字串）。 下列指令碼會啟用上述範例中的自動備份設定，並新增加密。
...
首先，您可以藉由呼叫 [msdb.managed_backup.sp_get_backup_diagnostics](https://docs.microsoft.com/sql/relational-databases/system-stored-procedures/managed-backup-sp-get-backup-diagnostics-transact-sql) 來輪詢狀態。 或者查詢[msdb.managed_backup.fn_get_health_status](https://docs.microsoft.com/sql/relational-databases/system-functions/managed-backup-fn-get-health-status-transact-sql)表值函數。
