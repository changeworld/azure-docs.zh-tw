...
| 層 | 描述 |
| --- | --- |
| **展示** |*展示層* (Web 層，亦為前端層) 是使用者與應用程式互動所在的分層。 |
| **商務** |*商業層* (中間層) 是展示層與資料層用來彼此聯繫的分層，具有系統的核心功能。 |
| **資料** |*資料層*基本上是應用程式資料存放所在的伺服器 (例如，執行 SQL Server 的伺服器)。 |
...
建議您將屬於同一層的虛擬機器放在相同雲端服務中，並使用相同的可用性設定組。 例如，請將一組 Web 伺服器放在 **CloudService1** 和 **AvailabilitySet1** 中，並將一組資料庫伺服器放在 **CloudService2** 和 **AvailabilitySet2** 中。 Azure 中的可用性設定組可讓您將高可用性節點放入分隔的容錯網域和升級網域。
...
此外，我們建議您使用 Azure 虛擬網路，為虛擬機器設定私人網路。 這可讓虛擬機器透過私人 IP 位址彼此通訊。 有關詳細資訊，請參閱[Azure 虛擬網路](../../../virtual-network/virtual-networks-overview.md)。
