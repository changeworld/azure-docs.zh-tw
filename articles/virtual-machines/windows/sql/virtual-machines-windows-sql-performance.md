...
* **NTFS 配置單位大小**：格式化資料磁碟時，建議您針對資料/記錄檔案和 TempDB，採用 64 KB 的配置單位大小。 如果將 TempDB 放在暫存磁片上（D:\磁片磁碟機）利用此磁片磁碟機所獲得的效能，超過64K 配置單位大小的需求。 
