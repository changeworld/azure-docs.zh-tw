...
| **Azure 金鑰保存庫整合** |讓您在 SQL Server VM 上自動安裝和設定 Azure 金鑰保存庫。 如需詳細資訊，請參閱[在 Azure 虛擬機器上設定 SQL Server 的 Azure Key Vault 整合（Resource Manager）](virtual-machines-windows-ps-sql-keyvault.md)。 |
...
* [下載並設定最新的 Azure PowerShell 命令](/powershell/azure/overview)
