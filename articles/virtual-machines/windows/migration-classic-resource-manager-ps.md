---
title: 使用 PowerShell 遷移到資源管理器
description: 本文通過使用 Azure PowerShell 命令，介紹支援平臺的 IaaS 資源（如虛擬機器 （VM）、虛擬網路和存儲帳戶從經典遷移到 Azure 資源管理器的遷移過程
...
ms.locfileid: "79249967"
---
# <a name="migrate-iaas-resources-from-classic-to-azure-resource-manager-by-using-powershell"></a>使用 PowerShell 將 IaaS 資源從經典資源遷移到 Azure 資源管理器
...
* [查看最常見的遷移錯誤](migration-classic-resource-manager-errors.md)。
...
以下是在評估是否將 IaaS 資源從經典遷移到資源管理器時，我們建議的一些最佳實踐：
...
> [!IMPORTANT]
> 應用程式閘道當前不受從經典到資源管理器的遷移支援。 要使用應用程式閘道遷移虛擬網路，請先在運行"準備"操作以移動網路之前刪除閘道。 在完成移轉之後，於 Azure Resource Manager 中重新連接閘道。
>
...
## <a name="step-2-install-the-latest-version-of-powershell"></a>第 2 步：安裝最新版本的 PowerShell
...
## <a name="step-3-ensure-that-youre-an-administrator-for-the-subscription"></a>第 3 步：確保您是訂閱的管理員
要執行此遷移，必須添加為[Azure 門戶](https://portal.azure.com)中的訂閱的共同管理員。

1. 登錄到 Azure[門戶](https://portal.azure.com)。
2. 在 **"中心"** 功能表上，選擇 **"訂閱**"。 如果您沒有看到，請選取 [所有服務]****。
3. 查找相應的訂閱條目，然後查看 **"MY ROLE"** 欄位。 對於共同管理員，該值應為_帳戶管理員_。

如果您無法添加共同管理員，請與訂閱的服務管理員或共同管理員聯繫以進行添加。

## <a name="step-4-set-your-subscription-and-sign-up-for-migration"></a>第 4 步：設置訂閱並註冊遷移
首先，開啟 PowerShell 提示字元。 對於遷移，請為經典資源管理器和資源管理器設置環境。
...
* [在雲服務中遷移 VM（不在虛擬網路中）](#step-51-option-1---migrate-virtual-machines-in-a-cloud-service-not-in-a-virtual-network)
...
### <a name="step-51-option-1---migrate-virtual-machines-in-a-cloud-service-not-in-a-virtual-network"></a>步驟 5.1：選項 1 - 在雲服務中遷移虛擬機器（不在虛擬網路中）
...
* **選項 1：將 VM 遷移到平臺創建的虛擬網路。**
    ...
    Move-AzureService -Prepare -ServiceName $serviceName `
        -DeploymentName $deploymentName -CreateNewVirtualNetwork
    ```
* **選項 2：遷移到資源管理器部署模型中的現有虛擬網路。**

    本示例將資源組名稱設置為**myResourceGroup，** 將虛擬網路名稱設為 **"我的虛擬網路**"，並將子網名稱設到**我的 SubNet**。 將此範例中的名稱取代為您自己的資源名稱。
...
### <a name="step-51-option-2---migrate-virtual-machines-in-a-virtual-network"></a>步驟 5.1：選項 2 - 在虛擬網路中遷移虛擬機器

若要移轉虛擬網路中的虛擬機器，您將需要移轉虛擬網路。 虛擬機器會自動隨著虛擬網路移轉。 選取您想要移轉的虛擬網路。
> [!NOTE]
> 通過使用虛擬機器的 VHD（OS 和資料）檔，使用經典部署模型創建新的資源管理器虛擬機器（具有託管磁片）遷移使用經典部署模型創建的[單個](migrate-single-classic-to-resource-manager.md)虛擬機器。
<br>

> [!NOTE]
> 虛擬網路名稱可能與新門戶中顯示的名稱不同。 新的 Azure 門戶將名稱顯示為`[vnet-name]`，但實際虛擬網路名稱的類型`Group [resource-group-name] [vnet-name]`為 。 在開始遷移之前，請使用 命令查找實際的虛擬網路名稱，或在舊的`Get-AzureVnetSite | Select -Property Name`Azure 門戶中查看該名稱。 
...
遷移虛擬機器後，在遷移存儲帳戶之前執行以下先決條件檢查。

> [!NOTE]
> 如果您的存儲帳戶沒有關聯的磁片或 VM 資料，則可以直接跳到"驗證存儲帳戶並開始遷移"部分。

* 先決條件檢查您是否遷移了任何 VM 或存儲帳戶具有磁片資源：
    * 遷移磁片存儲在存儲帳戶中的虛擬機器。

        以下命令返回存儲帳戶中所有 VM 磁片的 RoleName 和 DiskName 屬性。 RoleName 是磁碟所連線的虛擬機器名稱。 如果此命令返回磁片，請確保在遷移存儲帳戶之前遷移這些磁片所附加到的虛擬機器。
        ```powershell
        ...
        ```
    * 刪除存儲在存儲帳戶中的未連接 VM 磁片。

        使用以下命令在存儲帳戶中查找未連接的 VM 磁片：
