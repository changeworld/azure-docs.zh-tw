...
1. 開啟虛擬機器的頁面。
1. 在左側功能表中，選取 [ **大小**]。
1. 從可用大小清單中選取新的大小，然後選取 [重 **設**大小]。


如果虛擬機器目前正在執行，變更其大小將會重新開機。 停止虛擬機器可能會顯示其他的大小。

## <a name="use-powershell-to-resize-a-vm-not-in-an-availability-set"></a>使用 PowerShell 調整不在可用性設定組中的 VM 大小
...
如果您要的大小未列出，請執行以下命令以解除配置 VM、調整其大小，然後重新啟動 VM。 取代 **\<newVMsize>** 為您想要的大小。
...
## <a name="use-powershell-to-resize-a-vm-in-an-availability-set"></a>使用 PowerShell 調整可用性設定組中的 VM 大小
...
如需更多的擴充性，請執行多個 VM 實例和相應放大。如需詳細資訊，請參閱 [自動調整虛擬機器擴展集中的 Windows 機器](../../virtual-machine-scale-sets/tutorial-autoscale-powershell.md)。