---
title: 移動 Azure 中的 Windows VM 資源
...
本文將逐步引導您了解如何在資源群組或訂用帳戶之間移動 Windows 虛擬機器 (VM)。 如果您原本在個人訂用帳戶中建立 VM，而現在想要將它移至您的公司訂用帳戶以繼續工作，在訂用帳戶之間移動會很方便。 您不需要停止 VM 以便移動 VM，它應該會在移動期間繼續執行。
...
您可以使用前一個命令的輸出來建立以逗號分隔的資源識別碼清單，以 [移動-get-azresource](/powershell/module/az.resources/move-azresource) 以將每個資源移到目的地。
