---
title: 使用 Azure 網站回復將 Windows VM 移至 Azure 進階儲存
...
2. 選擇 **「創建資源** > **管理** > **備份和網站恢復 (OMS)」。** 或您可以選擇**瀏覽** > **服務保存服務** > **Add**。
   >[!NOTE]
...
2. 跳到**修復服務保管庫** > **設定** > **網站復原** > **步驟 1:準備基礎結構** > **保護目標**。
...
選擇 **「準備基礎結構** > **目標**」,並指定故障轉移後要用於 VM 的部署模型。 視您的案例而定，您可以選擇 [傳統]**** 或 [Resource Manager]****。
...
2. 啟用複寫，如下所示︰
   1. 選擇**複製應用程式** > **來源**。 第一次啟用複寫之後，請選取保存庫中的 [+複寫]****，以對其他機器啟用複寫。
...
在設計 Azure 儲存體環境時，建議您為可用性設定組中的每個 VM 使用不同的儲存體帳戶。 我們建議您遵循儲存層中的最佳做法,[為每個可用性集使用多個儲存帳戶](../linux/manage-availability.md)。 將 VM 磁碟分散到多個儲存體帳戶有助於增進儲存體可用性，並將 I/O 分散到整 Azure 儲存體基礎結構。
...
要檢查複製是否完成,請選擇「站點恢復」實例,然後選擇 **「設定** > **複製專案**」。。 您將會看到複寫程序的狀態和百分比。 
...
您可以在 **「設定** > **作業** > *YOUR_FAILOVER_PLAN_NAME」* 中查看測試故障轉移的狀態。 在窗格中，您會看到分解步驟和成功/失敗的結果。 如果測試容錯移轉在任一步驟失敗，請選取該步驟來檢查錯誤訊息。 
...
* [建立 Windows 伺服器 VHD 並將其上傳到 Azure](upload-generalized-managed.md)
