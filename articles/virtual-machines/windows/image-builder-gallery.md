...
請等待`RegistrationState` ， `Registered`再移至下一個步驟。

檢查您的提供者註冊。 請確定每個`Registered`都會傳回。
...
影像產生器將會使用提供的[使用者身分識別](https://docs.microsoft.com/azure/active-directory/managed-identities-azure-resources/how-to-manage-ua-identity-powershell)，將映射插入 Azure 共用映射庫（SIG）。 在此範例中，您將建立一個 Azure 角色定義，其具有細微動作可執行將映射散發至 SIG。 角色定義接著會指派給使用者身分識別。
...
### <a name="assign-permissions-for-identity-to-distribute-images"></a>指派身分識別的許可權以發佈映射
