...
---
# <a name="preview-deploy-spot-vms-using-azure-powershell"></a>預覽：使用 Azure PowerShell 部署 Spot VM
...
使用[New-AzVmConfig 創建](/powershell/module/az.compute/new-azvmconfig)點 VM 以創建配置。 包括`-Priority Spot`並設置為`-MaxPrice`：
- `-1`因此，VM 不會根據價格被逐出。
...
您還可以使用[Azure CLI](../linux/spot-cli.md)或[範本](../linux/spot-template.md)創建 Spot VM。
