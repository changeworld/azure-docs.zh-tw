...
5. 在 [指定複本] 頁面上方會列出多個索引標籤：[複本]、[端點]、[備份喜好設定] 以及 [接聽程式]。 從 [複本] 索引標籤按一下 [新增 Azure 複本...] 以啟動 [加入 Azure 複本精靈]。
...
除了使用 [加入 Azure 複本精靈]  將您的 Always On 可用性群組延伸至 Azure 之外，您也可以將部分 SQL Server 工作負載完全移動至 Azure。 若要開始進行，請參閱 [在 Azure 上佈建 SQL Server 虛擬機器](../../../azure-sql/virtual-machines/windows/create-sql-vm-portal.md)。

如需在 Azure VM 中執行 SQL Server 的其他相關主題，請參閱 [Azure 虛擬機器上的 SQL Server](../../../azure-sql/virtual-machines/windows/sql-server-on-azure-vm-iaas-what-is-overview.md)。
