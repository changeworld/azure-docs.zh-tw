...
如果您使用內部部署機器執行 SQL Server，則[可遵循一些步驟來從內部部署 SQL Server 電腦存取 Azure Key Vault](https://msdn.microsoft.com/library/dn198405.aspx)。 但是對於 Azure VM 中的 SQL server，您可以使用 *Azure 金鑰保存庫整合* 功能來節省時間。 使用一些 Azure PowerShell Cmdlet 來啟用這項功能，您可以自動化 SQL VM 存取您的金鑰保存庫所需的組態。
