---
title: 適用于 Windows VM 的 Azure AD 的 Azure 磁片加密（上一版本）
...
---
# <a name="azure-disk-encryption-with-azure-ad-for-windows-vms-previous-release"></a>適用于 Windows VM 的 Azure AD 的 Azure 磁片加密（上一版本）

**新版本的 Azure 磁片加密消除了提供 Azure AD 應用程式參數以啟用 VM 磁片加密的要求。使用新版本時，不再需要在啟用加密步驟期間提供 Azure AD 憑據。使用新版本，必須在沒有 Azure AD 應用程式參數的情況下對所有新 VM 進行加密。要查看使用新版本啟用 VM 磁片加密的說明，請參閱 Windows [VMS 的 Azure 磁片加密](disk-encryption-windows.md)。仍支援已使用 Azure AD 應用程式參數加密的 VM，應繼續使用 AAD 語法進行維護。**
...
3. 在您部署此範本之後，請使用您慣用的方法來確認 VM 加密狀態：
     ...
     - 使用[獲取-AzVm磁片加密狀態](/powershell/module/az.compute/get-azvmdiskencryptionstatus)Cmdlet 與 Azure PowerShell 進行驗證。 
     ...
     -  選取 VM，然後按一下 [設定]**** 標題之下的 [磁碟]****，以在入口網站中驗證加密狀態。 在圖中的 [加密]**** 之下，您會看到它是否已啟用。 
           ![Azure 門戶 - 已啟用磁片加密](../media/disk-encryption/disk-encryption-fig2.png)
...
## <a name="new-iaas-vms-created-from-customer-encrypted-vhd-and-encryption-keys"></a><a name="bkmk_VHDpre"> </a>使用客戶加密 VHD 和加密金鑰創建的新 IaaS VM
...
* [準備預加密的 Windows VHD](disk-encryption-sample-scripts.md#prepare-a-pre-encrypted-windows-vhd)
...
-  **使用用戶端金鑰加密正在運行的 VM：** 下面的腳本初始化變數並運行 Set-AzVM 磁片加密擴展 Cmdlet。 資源群組、VM、金鑰保存庫、AAD 應用程式和用戶端祕密應該已經建立為必要條件。 用您的價值觀替換 MyKeyVault 資源組、我的虛擬電腦資源組、MySecureVM、MySecureVault、My-AAD 用戶端 ID 和 My-AAD 用戶端機密。 此範例會對 -VolumeType 參數使用 "All"，其同時包含 OS 和資料磁碟區。 如果您只想要加密 OS 磁碟區，則請對 -VolumeType 參數使用 "OS"。 
...
- **使用資源管理器範本禁用加密：** 
    ...
    1. 按一下在運行 Windows [VM 範本時從禁用磁片加密](https://github.com/Azure/azure-quickstart-templates/tree/master/201-decrypt-running-windows-vm)**部署到 Azure。**
