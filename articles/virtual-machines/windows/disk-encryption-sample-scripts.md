...
description: 本文是適用於 Windows VM 的 Microsoft Azure 磁碟加密的附錄。
author: msmbaldwin
...
```
列出用於加密金鑰保存庫中 VM 的所有磁碟加密機密:

```azurepowershell-interactive
Get-AzKeyVaultSecret -VaultName $KeyVaultName | where {$_.Tags.ContainsKey('DiskEncryptionKeyFileName')} | format-table @{Label="MachineName"; Expression={$_.Tags['MachineName']}}, @{Label="VolumeLetter"; Expression={$_.Tags['VolumeLetter']}}, @{Label="EncryptionKeyURL"; Expression={$_.Id}}
```

## <a name="the-azure-disk-encryption-prerequisites-scripts"></a>Azure 磁碟加密先決條件文稿
...
- [在現有或正在執行的 Windows VM 上啟用磁碟加密](https://github.com/Azure/azure-quickstart-templates/tree/master/201-encrypt-running-windows-vm-without-aad)  
- [關閉正在執行的 Windows VM 上的加密](https://github.com/Azure/azure-quickstart-templates/tree/master/201-decrypt-running-windows-vm-without-aad) 

### <a name="encrypt-or-decrypt-vms-with-an-azure-ad-app-previous-release"></a>加密或解密具有 Azure AD 應用程式 (舊版) 的 VM 
 
- [在現有或正在執行的 Windows VM 上啟用磁碟加密](https://github.com/Azure/azure-quickstart-templates/tree/master/201-encrypt-running-windows-vm)    
- [關閉正在執行的 Windows VM 上的加密](https://github.com/Azure/azure-quickstart-templates/tree/master/201-decrypt-running-windows-vm) 
...
設定 **BitLocker 磁碟機加密**的 BitLocker 群組原則設定，其位於此路徑**本機電腦原則** > **電腦設定** > **系統管理範本** > **Windows 元件**。 將此設定更改為**作業系統驅動程式** >  > **需要在啟動時進行額外的身份驗證****,允許 BitLocker 沒有相容的 TPM**,如下圖所示:
...
使用[`manage-bde`](https://technet.microsoft.com/library/ff829849.aspx)指令使用外部金鑰保護器在引導卷上啟用加密。 也將外部金鑰 (.bek 檔案) 放置於外部磁碟機或磁碟區。 下次重新開機後，會在系統/開機磁碟區上啟用加密。
...
以前獲得的磁碟加密密鑰必須作為密鑰保管庫中的機密上載。  這需要向將上載機密的帳戶授予集機密許可權和包裝密鑰許可權。
