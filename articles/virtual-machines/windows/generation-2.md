...
* **我有一個來自本地第 2 代 VM 的 .vhd 檔。我能否使用 .vhd 檔在 Azure 中創建第 2 代 VM？**
  可以，您可以將第 2 代 .vhd 檔帶到 Azure，並用它來創建第 2 代 VM。 使用以下步驟執行此操作：
    ...
    創建第 2 代 VM 時，可以選擇通用此 VM 映射。 通過對映射進行一般化，可以使用它創建多個 VM。

* **如何增加作業系統磁片大小？**  
  大於 2 TB 的 OS 磁片是第 2 代 VM 的新磁片。 預設情況下，對於第 2 代 VM，OS 磁片小於 2 TB。 您可以將磁片大小增加到建議的最大 4 TB。 使用 Azure CLI 或 Azure 門戶增加 OS 磁片大小。 有關如何以程式設計方式擴展磁片的資訊，請參閱[調整磁片的大小](expand-os-disk.md)。
...
* 瞭解如何準備[VHD](prepare-for-upload-vhd-image.md)以從本地系統上載到 Azure。
