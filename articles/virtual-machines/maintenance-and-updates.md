...
本頁說明 Azure 如何執行這兩種類型的維護。 如需非計劃性事件 (中斷) 的詳細資訊，請參閱 [管理 Windows VM 的可用性](./manage-availability.md)或 [Linux](./manage-availability.md) 的對應文章。
...
若要對所有維護活動 (包括無影響的更新和不必重新開機的更新) 進行更好的控制，您可以使用維護控制功能。 您必須使用 [Azure 專用主機](./dedicated-hosts.md)或[隔離的 VM](../security/fundamentals/isolation-choices.md)。 維護控制可讓您選擇略過所有平台更新，並在 35 天的滾動時間範圍內擇時套用更新。 如需詳細資訊，請參閱[使用維護控制和 Azure CLI 來控制更新](maintenance-control.md)。
...
如需如何設定 VM 以獲得高可用性的詳細資訊，請參閱 [管理 Windows VM 的可用性](./manage-availability.md)或 [Linux](./manage-availability.md) 的對應文章。
