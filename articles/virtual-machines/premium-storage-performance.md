...
開始之前，如果您不熟悉進階儲存體，請先閱讀[為 IaaS VM 選取 Azure 磁碟類型](./linux/disks-types.md) \(英文\) 和[進階儲存體帳戶的延展性目標](~/articles/storage/blobs/scalability-targets-premium-page-blobs.md)。
...
* 針對 Linux： [選取磁片類型](./linux/disks-types.md)
* 針對 Windows： [選取磁片類型](./windows//disks-types.md)

若為 SQL Server 使用者，請參閱「SQL Server 的效能最佳作法」文章：

* [Azure 虛擬機器中的 SQL Server 效能最佳作法](~/articles/azure-sql/virtual-machines/windows/performance-guidelines-best-practices.md)
