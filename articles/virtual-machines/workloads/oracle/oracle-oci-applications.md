...
如需高可用性，您可以在相同的可用性設定組或不同的可用性區域中，設定不同伺服器的重複實例。 可用性區域可讓您達到99.99% 的執行時間 SLA，而可用性設定組可讓您在區域內達到99.95% 的執行時間 SLA。 這篇文章中所顯示的範例架構會部署在兩個可用性區域中。
...
*圖4： JD Edwards EnterpriseOne 僅限 Azure 的架構*
...
此層的元件如下所示：
 ...
 - 布建**伺服器**-此伺服器用於應用程式之不同元件的端對端部署。 它會與其他層中的實例進行通訊，包括資料庫層中的實例（透過埠22）。 它會主控 JD Edwards EnterpriseOne 的伺服器管理員主控台。
