...
3. 透過執行下列命令格式化磁碟 /dev/sdc，並透過下列項目回答提示：
   ...
   使用上面提供的答案，命令的輸出 `fdisk` 看起來應該如下所示：
...
1. 從 [Oracle ASM 下載頁面](https://www.oracle.com/database/technologies/oracle19c-linux-downloads.html)下載 Oracle Grid Infrastructure。 
...
6. 在用戶端系統上，啟動 PuTTY。 在 [**類別目錄**] 窗格中，移至 [**連接**  >  **SSH**  >  **驗證**]。在 [**驗證的私密金鑰** 檔案] 方塊中，流覽至您稍早產生的金鑰。
...
5. 在 [建立磁碟群組] 對話方塊中：
   ...
   ![[建立磁片群組] 對話方塊的螢幕擷取畫面，並反白顯示 [外部 (無]) 選項。](./media/oracle-asm/asm04.png)
