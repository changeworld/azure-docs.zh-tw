...
Sles 12 SP3 for hpc、SLES 12 SP3 for hpc (Premium) 、SLES 12 SP1 for HPC、SLES 12 SP1 for HPC (Premium) 、SLES 12 SP4 以及 Marketplace 中的 SLES 15 VM 映射。 這些 VM 映射已預先載入 RDMA 和 Intel MPI 5.1 版的網路直接驅動程式。 深入瞭解如何在 Vm 上[設定 MPI](setup-mpi.md) 。
...
如需功能或效能的必要，您可以在支援的 OS 散發版本上安裝或更新[Linux Integration Services (.lis) 驅動程式](../../linux/endorsed-distros.md)，特別是使用自訂映射或較舊的 OS 版本（例如 CENTOS/RHEL 6.x 或較早版本的7.x）進行部署。
