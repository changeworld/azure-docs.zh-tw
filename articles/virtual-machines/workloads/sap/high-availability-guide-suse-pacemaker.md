---
title: 在 Azure 中的 SLES 上設置起搏器 |微軟文件
...
SBD 裝置至少需要一部額外的虛擬機器，作為 iSCSI 目標伺服器並且提供 SBD 裝置。 不過，這些 iSCSI 目標伺服器可以與其他 Pacemaker 叢集共用。 使用 SBD 裝置的優點是更快的故障轉移時間,如果您在本地使用 SBD 設備,則不需要更改您操作心臟起搏器群集的方式。 您可以對一個 Pacemaker 叢集使用最多三個 SBD 裝置，以容許在 iSCSI 目標伺服器的 OS 修補期間 (舉例來說)，有一個 SBD 裝置無法使用。 如果您想要使用一個以上的 SBD 裝置每 Pacemaker，請務必部署多部 iSCSI 目標伺服器，並從每部 iSCSI 目標伺服器連線一個 SBD。 我們建議使用一個或三個 SBD 裝置。 如果您只設定兩個 SBD 裝置，而且其中一個無法使用，Pacemaker 將無法自動隔離叢集節點。 如果您希望能夠在一部 iSCSI 目標伺服器關閉時隔離，您必須使用三個 SBD 裝置，因此會有三部 iSCSI 目標伺服器。

如果不想投資一個額外的虛擬機,也可以使用 Azure Fence 代理。 缺點是如果資源停止容錯或叢集節點無法再彼此進行通訊，則容錯移轉需要 10 到 15 分鐘。

![SLES 上的 Pacemaker 概觀](./media/high-availability-guide-suse-pacemaker/pacemaker.png)

>[!IMPORTANT]
> 規劃和部署 Linux Pacemaker 叢集節點與 SBD 裝置時，對於完整叢集設定的整體可靠性而言，最重要的是相關 VM 與裝載 SBD 裝置的 VM 之間，其路由傳送不會通過 [NVA](https://azure.microsoft.com/solutions/network-appliances/) 等任何其他裝置。 否則，與 NVA 相關的問題與維護事件，會對整體叢集設定的穩定性與可靠性造成負面影響。 為了避免此類障礙,在規劃和部署 Linux 起搏器群集節點和 SBD 設備時,不要定義透過 NVA 和類似裝置在群集節點和 SBD 設備之間路由流量的 NVA 或[使用者定義路由規則的路由規則](https://docs.microsoft.com/azure/virtual-network/virtual-networks-udr-overview)。 
>
...
1. 部署新的 SLES 12 SP1 或更新版本的虛擬機器，並透過 SSH 連線到這些機器。 機器不需要大。 諸如 Standard_E2s_v3 或 Standard_D2s_v3 的虛擬機器大小就已足夠。 請務必使用 OS 磁碟的進階儲存體。
...
**`nfs`** 用於識別 NFS 群集, **ascsnw1**用於識別**NW1**的 ASCS 群集 **,dbnw1**用於標識**NW1**的資料庫集群 **,nfs-0**和**nfs-1**是 NFS 集群節點的主機名稱 **,nw1-xscs-0**和**nw1-xscs-1**是**NW1** ASCS集群節點的主機名 **,nw1-db-0**和**nw1-db-1**是資料庫的主機名。 使用您的叢集節點主機名稱和 SAP 系統 SID 取代它們。
...
以下項目預置**為 [A]** - 適用於所有節點 **[1]-** 僅適用於節點 1 或 **[2]** - 僅適用於節點 2。
...
1. **[A]** 安裝叢集資源所需的 azure 磅元件

   <pre><code>sudo zypper in resource-agents
   </code></pre>

   > [!NOTE]
   > 檢查套件資源代理的版本,並確保滿足最低版本要求:  
...
1. **[A]** 為 HA 群集設定雲網配置-azure

   >[!NOTE]
   > 通過運行**zypper 資訊雲-netconfig-azure**來檢查包**雲-netconfig-azure**的已安裝版本。 如果環境中的版本為 1.3 或更高版本,則不再需要通過雲網路外掛程式禁止管理網路介面。 如果版本低於 1.3,我們建議您將包**雲網配置-azure**更新為最新版本。  

   更改網路介面的配置檔,如下所示,以防止雲端網路外掛程式刪除虛擬 IP 位址(Pacemaker 必須控制 VIP 分配)。 有關詳細資訊,請參閱[SUSE KB 7023633](https://www.suse.com/support/kb/doc/?id=7023633)。 
...
1. **[A]** 安裝柵欄代理程式
   ...
   >[!IMPORTANT]
   > 如果將 Suse Linux 企業伺服器用於 SAP 15,請注意,您需要啟動其他模組並安裝其他元件,這是使用 Azure 圍欄代理的先決條件。 要瞭解有關 SUSE 模組和擴充的更多,請參閱[解釋的模組和擴展](https://www.suse.com/documentation/sles-15/singlehtml/art_modules/art_modules.html)。 按照說明波紋管安裝 Azure Python SDK。 

   以下有關如何安裝 Azure Python SDK 的說明僅適用於 SAP **15**的 Suse 企業伺服器。  

    - 如果您使用的是自帶訂閱,請按照以下說明操作  
...
1. 開啟 [Azure Active Directory] 刀鋒視窗  
   移至 [屬性]，並記下目錄識別碼。 這是租戶**代碼**。
1. 按一下 [應用程式註冊]
1. 單擊"新註冊"
1. 輸入名稱,選擇"僅在此組織目錄中的帳戶" 
2. 選擇應用程式類型"Web",輸入登錄 URL(例如 http:/\/本地主機),然後單擊"添加"  
   登入 URL 並未使用，而且可以是任何有效的 URL
1. 選擇證書和機密,然後單擊"新用戶端機密"
1. 輸入新密鑰的說明,選擇"永不過期",然後單擊"添加"
1. 記下值。 它用作服務主體的**密碼**
1. 選取 [概觀]。 記下應用程式識別碼。 它用作服務主體的使用者名稱(以下步驟中的**登入 ID)**

### <a name="1-create-a-custom-role-for-the-fence-agent"></a>**[1]** 為柵欄代理程式建立自訂角色

默認情況下,服務主體沒有訪問 Azure 資源的許可權。 您需要為服務主體提供權限來啟動和停止 (解除配置) 叢集的所有虛擬機器。 如果您尚未建立自訂角色，您可以使用 [PowerShell](https://docs.microsoft.com/azure/role-based-access-control/custom-roles-powershell#create-a-custom-role) 或 [Azure CLI](https://docs.microsoft.com/azure/role-based-access-control/custom-roles-cli) 來建立
...
將在上一章中建立的自訂角色「Linux 柵欄代理程式角色」指派給服務主體。 不要再使用所有者角色了!

1. 跳到(G)[https://portal.azure.com](https://portal.azure.com)
1. 開啟 [所有資源] 刀鋒視窗
1. 選取第一個叢集節點的虛擬機器
1. 選取 [存取控制 (IAM)]
1. 按一下 [新增角色指派]
...
## <a name="pacemaker-configuration-for-azure-scheduled-events"></a>Azure 計劃事件的起搏器設定

Azure 提供[計畫的事件](https://docs.microsoft.com/azure/virtual-machines/linux/scheduled-events)。 計劃事件通過中繼服務提供,並允許應用程式有時間為 VM 關機、VM 重新部署等事件做好準備。資源代理**[Azure 事件](https://github.com/ClusterLabs/resource-agents/pull/1161)** 監視計畫的 Azure 事件。 如果檢測到事件,代理將嘗試停止受影響的 VM 上的所有資源,並將它們移動到群集中的另一個節點。 為了實現這一目標,必須配置額外的起搏器資源。 

1. **[A]** 確保**Azure 事件**代理的包已安裝並更新。 
...
sudo crm configure property maintenance-mode=false
   ...
   > [!NOTE]
   > 為 Azure 事件代理設定起搏器資源後,將群集置於維護模式或置於維護模式外時,可能會收到警告訊息,例如:  
     警告: cib-開機選項:未知屬性「hostName_<strong>主機名稱</strong>」  
     警告: cib 引導選項:未知屬性"azure-events_globalPullState"  
     警告: cib-開機選項:未知屬性「hostName_<strong>主機名稱</strong>」  
   > 您可以忽略這些警告訊息。
