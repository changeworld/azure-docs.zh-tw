...
1. **[A]** 設定適用于 HA 叢集的雲端 netconfig-azure
   ...
   >[!NOTE]
   > 執行**zypper info cloud-netconfig**，檢查已安裝的 package **netconfig-azure**版本。 如果您環境中的版本是1.3 或更高版本，則不再需要透過雲端網路外掛程式來隱藏網路介面的管理。 如果版本低於1.3，建議您將套件**雲端 netconfig**更新至最新的可用版本。  
...
1. 記下值。 它會用來做為服務主體的**密碼**
1. 選取 [概觀]  。 記下應用程式識別碼。 它是用來做為服務主體的使用者名稱（以下步驟中的**登入識別碼**）
