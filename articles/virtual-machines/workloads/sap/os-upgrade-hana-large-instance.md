...
>[!NOTE]
>作業系統升級是客戶的責任，Microsoft 運營支援可以引導您到升級期間需要注意的關鍵領域。 規劃升級之前，請先諮詢您的作業系統廠商。
...
## <a name="sap-hana-large-instance-type-i-recommended-configuration"></a>SAP HANA 大型實例（I 型）推薦配置

由於修補、系統升級和客戶所做的更改，作業系統配置可能會隨著時間的推移而偏離建議的設置。 此外，Microsoft 還會識別現有系統所需的更新，以確保它們以最佳配置，以實現最佳性能和彈性。 按照說明概述瞭解決網路性能、系統穩定性和最佳 HANA 性能的建議。

### <a name="compatible-enicfnic-driver-versions"></a>相容 eNIC/fNIC 驅動程式版本
  為了具有適當的網路性能和系統穩定性，建議確保安裝特定于作業系統的 eNIC 和 fNIC 驅動程式的適當版本，如下相容性表所示。 伺服器以相容版本交付給客戶。 請注意，在某些情況下，在 OS/Kernel 修補期間，驅動程式可以回滾到預設驅動程式版本。 確保適當的驅動程式版本在運行後 OS/內核修補操作。
       
      
  |  作業系統供應商    |  作業系統包版本     |  韌體版本  |  eNIC 驅動程式 |  fNIC 驅動程式 | 
  |---------------|-------------------------|--------------------|--------------|--------------|
...
### <a name="commands-for-driver-upgrade-and-to-clean-old-rpm-packages"></a>用於驅動程式升級和清潔舊 rpm 包的命令
```
...
*   執行`multipath -ll`命令。
*   獲取其大小約為 50G 的 LUN ID 或使用以下命令：`fdisk -l | grep mapper`
*   使用`/etc/default/grub_installdevice`行`/dev/mapper/<LUN ID>`更新檔 。 示例： /dev/映射器/3600a09803830372f483f495242534a56
>[!NOTE]
>LUN ID 因伺服器而異。


### <a name="disable-edac"></a>禁用 EDAC 
   錯誤偵測和更正 （EDAC） 模組有助於檢測和更正記憶體錯誤。 但是，Azure 大型實例（類型 I）上的 SAP HANA 的基礎硬體已經執行了相同的功能。 在硬體和作業系統 （OS） 級別啟用相同的功能可能會導致衝突，並可能導致伺服器偶爾意外、計畫外停機。 因此，建議從作業系統禁用模組。
...
lsmod | grep -i edac 
```
* 通過將以下行追加到檔中來禁用模組`/etc/modprobe.d/blacklist.conf`
```
blacklist sb_edac
blacklist edac_core
```
需要重新開機才能進行更改。 執行`lsmod`命令並驗證輸出中不存在模組。


### <a name="kernel-parameters"></a>核心參數
   請確保`transparent_hugepage`應用 了 、`numa_balancing``processor.max_cstate``ignore_ce``intel_idle.max_cstate`和 的正確設置。
...
* 將這些參數添加到檔中的`GRB_CMDLINE_LINUX`行`/etc/default/grub`
```
