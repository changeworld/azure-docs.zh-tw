...
如需 Azure 中 Linux Pacemaker 的詳細資訊，請參閱在[azure 中的 SUSE Linux Enterprise Server 上設定 Pacemaker](./high-availability-guide-suse-pacemaker.md)。
...
1. 將 [db2 使用者的 shell 環境] 變更 \<sid> 為 [ */bin/ksh*]。 建議您使用 Yast 工具。 
...
1. 在左側畫面中，選擇 [**安全性存放區**]。
1. 在右邊的畫面中，選擇 [jdbc/pool//url.] 這個關鍵字 \<SAPSID>
...
- [SUSE Linux Enterprise Server 上的 Azure Vm 上的 SAP NetWeaver 高可用性與適用于 SAP 應用程式的 Azure NetApp Files](./high-availability-guide-suse-netapp-files.md)
...
- [在 Azure 中的 SUSE Linux Enterprise Server 上設定 Pacemaker](./high-availability-guide-suse-pacemaker.md)
