...
> [!NOTE]
> 本指南說明如何將 SAP HANA 部署到 Azure VM。 如需如何將 SAP Hana 部署到 HANA 大型實例的相關資訊，請參閱[如何在 Azure 上安裝和設定 SAP Hana （大型實例）](./hana-installation.md)。
...
* 中記載的概念和程式：
   * 在 Azure 上規劃 SAP 部署，其中包括 Azure 虛擬網路規劃和 Azure 儲存體使用量。 請參閱[Azure 上的 SAP NetWeaver 虛擬機器-規劃與實施指南](./planning-guide.md)
   * 部署原則及在 Azure 中部署 VM 的方式。 請參閱[適用于 SAP 的 Azure 虛擬機器部署](./deployment-guide.md)
   * [Azure 虛擬機器的 SAP Hana 高可用性](./sap-hana-availability-overview.md)中所述 SAP Hana 的高可用性概念

## <a name="step-by-step-before-deploying"></a>部署前的逐步解說
在本節中，會列出不同的步驟，您必須在開始安裝 Azure 虛擬機器中的 SAP Hana 之前執行。 順序會列舉出來，因此應遵循列舉：

1. Azure 不支援所有可能的部署案例。 因此，您應該查看[Azure 虛擬機器支援案例上的 SAP 工作負載](./sap-planning-supported-configurations.md)檔，以瞭解您的 SAP Hana 部署所需考慮的案例。 如果未列出案例，您必須假設它尚未經過測試，因此不受支援。
2. 假設您大致瞭解 SAP Hana 部署的記憶體需求，您需要尋找適合的 Azure VM。 並非所有經 SAP NetWeaver 認證的 Vm （如[sap 支援附注 #1928533](https://launchpad.support.sap.com/#/notes/1928533)中所述）都 SAP Hana 認證。 SAP Hana 認證的 Azure Vm 的真實來源是[SAP Hana 硬體目錄](https://www.sap.com/dmc/exp/2014-09-02-hana-hardware/enEN/iaas.html#categories=Microsoft%20Azure)的網站。 以**S**開頭的單位是[HANA 大型實例](./hana-overview-architecture.md)單位，而不是 Azure vm。
...
3. 決定 VM 的網路設定。 您可以在[Azure 上 SAP Hana 基礎結構設定和作業](./hana-vm-operations.md)檔中閱讀詳細資訊。 請記住，您不能將網路輸送量配額指派給 Azure 中的虛擬網路卡。 因此，將流量導向不同 Vnic 的唯一目的是根據安全性考慮。 我們信任您在透過多個 Vnic 的流量路由複雜性與安全性層面強制執行的需求之間，找到可支援性的危害。
...
1. 選取適用于 SAP Hana 的 Azure 儲存體類型。 在此步驟中，您必須決定用於 SAP Hana 安裝的儲存體配置。 您將使用已連接的 Azure 磁片或原生 Azure NFS 共用。 您可以在[SAP Hana azure 虛擬機器儲存體](./hana-vm-operations-storage.md)設定中，記錄或支援的 azure 儲存體類型，以及可使用的不同 azure 儲存體類型組合。 將設定記錄為起始點。 對於非生產系統，您可能可以設定較低的輸送量或 IOPS。 基於生產目的，您可能需要設定更多的輸送量和 IOPS。
2. 當您使用 M 系列或 Mv2 系列 Vm 時，請確定您已為包含 DBMS 交易記錄或重做記錄的磁片區設定[Azure 寫入加速器](../../linux/how-to-enable-write-accelerator.md)。 請留意所記載寫入加速器的限制。
...
如需使用 Azure 進階儲存體或 Ultra 磁片的直接連接磁片 SAP Hana 相應放大設定，請參閱[azure 上 SAP Hana 基礎結構設定和作業](./hana-vm-operations.md#configuring-azure-infrastructure-for-sap-hana-scale-out)檔中的詳細資訊
