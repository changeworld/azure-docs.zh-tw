---
title: Azure VM SLES 上的 SAP NetWeaver 高可用性 |微軟文檔
...
在示例配置、安裝命令等中。使用 ASCS 實例編號 00、ERS 實例編號 02 和 SAP 系統 ID NW1。 範例中資源 (例如虛擬機器、虛擬網路) 的名稱會假設您已使用[交集範本][template-converged]與 SAP 系統識別碼 NW1 來建立資源。
...
NFS 伺服器、SAP NetWeaver ASCS、SAP NetWeaver SCS、SAP NetWeaver ERS 和 SAP HANA 資料庫會使用虛擬主機名稱和虛擬 IP 位址。 在 Azure 上必須有負載平衡器才能使用虛擬 IP 位址。 我們建議使用[標準負載等化器](https://docs.microsoft.com/azure/load-balancer/quickstart-load-balancer-standard-public-portal)。 下列清單顯示 (A)SCS 和 ERS 負載平衡器的組態。
...
1. 打開 Azure 門戶上的[ASCS/SCS 多 SID 範本][template-multisid-xscs]或[融合範本][template-converged]。 
   ASCS/SCS 範本僅為 SAP NetWeaver ASCS/SCS 和 ERS（僅限 Linux）實例創建負載平衡規則，而融合範本還會為資料庫創建負載平衡規則（例如 Microsoft SQL Server 或 SAP HANA）。 如果您打算安裝 SAP NetWeaver 架構的系統，而且也想要在同一部電腦上安裝資料庫，請使用[交集範本][template-converged]。
1. 輸入下列參數
...
您必須先為此 NFS 叢集建立虛擬機器。 之後，您將創建一個負載等化器，並在後端池中使用虛擬機器。
...
1. 創建負載等化器（內部標準）：  
   ...
   1. 負載平衡規則
      1. ASCS 的負載平衡規則
         ...
         1. 輸入新的負載等化器規則的名稱（例如**nw1-lb-ascs）**
         1. 選擇您之前創建的前端 IP 位址、後端池和運行狀況探測（例如**nw1-ascs 前端****、nw1 後端**和**nw1-ascs-hp）**
         1. 選擇**HA 埠**
         1. 將閒置逾時增加為 30 分鐘
         1. **務必啟用浮動 IP**
         1. Click OK
         * 重複上述步驟，為 ERS 創建負載平衡規則（例如**nw1 磅）**
1. 或者，如果您的方案需要基本的負載等化器（內部），請按照以下步驟操作：  
   1. 建立前端 IP 位址
      1. 針對 ASCS 是 IP 位址 10.0.0.7
         ...
         1. Click OK
      1. 針對 ASCS ERS 是 IP 位址 10.0.0.8
         * 重複上述步驟，為 ERS 創建 IP 位址（例如**10.0.0.8**和**nw1-aers 前端**）
   1. 建立後端集區
      1. 開啟負載平衡器，選取後端集區，然後按一下 [新增]
      1. 輸入新後端集區的名稱 (例如 **nw1-backend**)
      1. 按一下 [新增虛擬機器]。
...
1. **[A]** 安裝 SUSE 連接器
   ...
   > [!NOTE]
   > 在主機名稱中使用破折號的已知問題已修復為包**sap-suse-群集連接器**的**3.1.1**版本。 如果使用主機名稱中帶有破折號的叢集節點，請確保至少使用包 sap-suse 群集連接器的 3.1.1 版本。 否則叢集將無法運作。 
...
如果使用排隊伺服器 2 體系結構[（ENSA2](https://help.sap.com/viewer/cff8531bc1d9416d91bb6781e628d4e0/1709%20001/en-US/6d655c383abf4c129b0e5c8683e7ecd8.html)），請按照如下方式定義資源：
...
在此範例中，SAP NetWeaver 安裝在 SAP Hana 上。 您可以針對此安裝使用每個支援的資料庫。 如需如何在 Azure 中安裝 SAP Hana 的詳細資訊，請參閱 [Azure 虛擬機器 (VM) 上的 SAP Hana 高可用性][sap-hana-ha]。 有關支援的資料庫清單，請參閱[SAP 注釋 1928533][1928533]。
...
1. 測試以手動方式重新啟動 ASCS 執行個體
   ...
   例如，透過編輯交易 su01 中的使用者來建立佇列鎖定。 以 \<sapsid>adm 身份在執行 ASCS 執行個體的節點上執行下列命令。 這些命令會停止 ASCS 執行個體，並重新啟動它。 如果使用排隊伺服器 1 體系結構，則在此測試中，排隊鎖將丟失。 如果使用排隊伺服器 2 體系結構，則將保留該佇列。 
...
* [用於 SAP 應用程式多 SID 指南的 SLES 上的 Azure VM 上的 SAP NW 的 HA](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-multi-sid)
