---
title: SAP HANA Azure 虛擬機器儲存配置 |微軟文件
description: 已部署 SAP HANA 的 VM 的儲存建議。
...
Azure 提供不同類型的存儲,適用於運行 SAP HANA 的 Azure VM。 **SAP HANA 認證的 Azure 儲存類型**,可考慮用於 SAP HANA 部署清單,如下所示: 

- Azure 進階 SSD  
- [Ultra 磁碟](https://docs.microsoft.com/azure/virtual-machines/linux/disks-enable-ultra-ssd)
- [Azure NetApp Files](https://azure.microsoft.com/services/netapp/) 

要瞭解這些磁碟類型,請參閱文章[「選擇磁碟類型](https://docs.microsoft.com/azure/virtual-machines/linux/disks-types)」
...
> [!IMPORTANT]
> 與選擇的 Azure 儲存類型無關,SAP 需要支援用於該存儲的檔案系統,用於特定的作業系統和 DBMS。 [SAP 支援說明#405827](https://launchpad.support.sap.com/#/notes/405827)列出支援用於不同作業系統和資料庫(包括 SAP HANA)的檔案系統。 這適用於 SAP HANA 可能造訪的所有卷,用於讀取和寫入任何任務。 具體使用 Azure 上的 NFS 進行 SAP HANA,NFS 版本的其他限制適用於本文後面所述 


不同儲存類型的最低 SAP HANA 認證條件為: 

- Azure 進階 SSD - /hana/log 需要使用 Azure[寫入加速器](https://docs.microsoft.com/azure/virtual-machines/linux/how-to-enable-write-accelerator)進行快取。 /hana/資料卷可以放置在進階 SSD 上,無需 Azure 寫入加速器或超磁碟
- Azure 超級磁碟至少適用於 /hana/日誌卷。 /hana/資料卷可以放置在進階 SSD 上,無需 Azure 寫入加速器,也可以用於更快地重新啟動超級磁碟
- **NFS v4.1**卷在 Azure NetApp 檔之上的 /hana/log**和**/hana/資料。 /hana/shared 的卷可以使用 NFS v3 或 NFS v4.1 協定。 NFS v4.1 協議對於 /hana/數據和/哈納/日誌卷是必需的。

可以組合某些存儲類型。 例如,可以將 /hana/資料放在高級存儲上,/hana/log 可以放在超磁碟存儲上,以獲得所需的低延遲。 但是,如果您使用基於 ANF 的 NFS v4.1 卷進行 /hana/資料,則需要使用基於 ANF 的 /hana/log 的另一個 NFS v4.1 卷。 **不支援**將 NFS 用於 ANF 的頂部,用於其中一個卷(如 /hana/資料)和 Azure 高級存儲或超磁碟,用於其他卷(如 /hana/log)。
...
指定低儲存體延遲對於 DBMS 系統相當重要，即使是像 SAP HANA 等 DBMS 將資料保存在記憶體內部也一樣。 儲存體中的關鍵路徑通常是在 DBMS 系統之交易記錄寫入的周圍。 但像是寫入儲存點或是在損毀復原之後載入記憶體內部資料之類的作業也很重要。 因此,**必須**利用 Azure 高級磁碟進行 **/hana/資料和** **/hana/log**卷。 為了達到如 SAP 想要的 **/hana/log** 和 **/hana/data** 最小輸送量，您必須使用 MDADM 或 LVM 在多個 Azure 進階儲存體磁碟上建置 RAID 0。 然後使用 RAID 磁碟區作為 **/hana/data** 和 **/hana/log** 磁碟區。 
...
> [!IMPORTANT]
> /hana/data 的條帶大小從之前要求 64 KB 或 128 KB 的建議更改為 256 KB,具體取決於具有較新 Linux 版本的客戶體驗。 256 KB 的大小提供略佳的效能
...
Linux 有數個不同的 I/O 排程模式。 通過 Linux 供應商和 SAP 的常見建議是重新配置磁碟卷的 I/O 調度程式模式,從**mq 截止時間**或**kyber**模式到**noop(** 非多佇列)或**無**(多佇列)模式。 詳細資訊在[SAP 說明#1984787](https://launchpad.support.sap.com/#/notes/1984787)中引用。 


## <a name="solutions-with-premium-storage-and-azure-write-accelerator-for-azure-m-series-virtual-machines"></a>適用於 Azure M 系列虛擬機器的進階儲存和 Azure 寫入加速器的解決方案
...
> [!IMPORTANT]
> 使用 Azure 進階儲存時,必須使用 Azure[寫入加速器](https://docs.microsoft.com/azure/virtual-machines/linux/how-to-enable-write-accelerator)或 /hana/log 卷。 寫入加速器僅適用於進階儲存和 M 系列和 Mv2 系列 VM。
...
**建議:由於 SAP HANA 觀察到的 I/O 模式,使用 Azure 進階儲存的不同卷的緩存設定應類似:**

- **/hana/data**：無快取
- **/hana/log** - 無緩存 - M-和 Mv2 系列例外,其中應啟用寫入加速器而不進行讀取緩存。 
- **/哈納/共用**- 讀取緩存

### <a name="production-recommended-storage-solution"></a>生產推薦的儲存解決方案
...
這些建議通常超出了本文前面所述的 SAP 最低要求。 列出的建議是 SAP 建議的大小建議與不同 VM 類型提供的最大儲存輸送量之間的折衷。
...
檢查不同建議的卷的存儲輸送量是否符合要運行的工作負荷。 如果工作負載需要更多 **/hana/data** 和 **/hana/log** 磁碟區，您需要增加 Azure 進階儲存體 VHD 的數目。 使用比列出的多 VHD 的卷進行大小調整會增加 Azure 虛擬機器類型限制範圍內的 IOPS 和 I/O 輸送量。
...
**建議:您需要對形成 /hana/log 卷的磁碟使用寫入加速器**


### <a name="cost-conscious-azure-storage-configuration"></a>節省成本的 Azure 儲存體組態
下表顯示了客戶也用於在 Azure VM 上託管 SAP HANA 的 VM 類型的配置。 可能有一些 VM 類型可能不符合 SAP HANA 的所有最低儲存條件,或者 SAP 未正式支援 SAP HANA。 但是目前這些虛擬機器似乎可以針對非生產案例正常執行。 **/hana/資料和** **/hana/log**組合為一個卷。 因此,Azure 寫入加速器的使用可能成為IOPS方面的限制。

> [!NOTE]
> 對於 SAP 支援的方案,檢查 SAP[在 IAAS](https://www.sap.com/dmc/exp/2014-09-02-hana-hardware/enEN/iaas.html)的 SAP 文件中是否支援 SAP HANA 的特定 VM 類型。

> [!NOTE]
> 與以前關於成本意識解決方案的建議的一個變化是,從[Azure 標準硬碟磁碟](https://docs.microsoft.com/azure/virtual-machines/windows/disks-types#standard-hdd)轉向性能更好、更可靠的[Azure 標準 SSD 磁碟](https://docs.microsoft.com/azure/virtual-machines/windows/disks-types#standard-ssd)
...
## <a name="azure-ultra-disk-storage-configuration-for-sap-hana"></a>適用於 SAP HANA 的 Azure 超磁碟儲存配置
微軟正在推出一種新的 Azure 儲存類型,稱為[Azure 超磁碟](https://docs.microsoft.com/azure/virtual-machines/windows/disks-types#ultra-disk)。 到目前為止提供的 Azure 儲存與 Ultra 磁碟之間的顯著區別是磁碟功能不再綁定到磁碟大小。 為客戶,您可以為 Ultra 磁碟定義這些功能:

- 磁碟大小範圍從 4 GiB 到 65,536 GiB
- IOPS 範圍從 100 IOPS 到 160K IOPS(最大值也取決於 VM 類型)
- 儲存輸送量從 300 MB/秒到 2,000 MB/秒

Ultra 磁碟使您能夠定義滿足大小、IOPS 和磁碟輸送量範圍的單個磁碟。 而不是在 Azure 進階儲存之上使用 LVM 或 MDADM 等邏輯卷管理器來建構滿足 IOPS 和儲存輸送量要求的卷。 您可以在超磁碟和高級存儲之間運行配置群組。 因此,您可以將 Ultra 磁碟的使用限制為性能關鍵/哈納/資料和 /hana/log 卷,並覆蓋使用 Azure 進階儲存的其他卷

與高級存儲相比,Ultra 磁碟的其他優點可能是更好的讀取延遲。 當您希望減少 HANA 啟動時間和數據後續載入到記憶體中時,更快的讀取延遲可能具有優勢。 當 HANA 編寫儲存點時,也可以感受到超磁碟存儲的優勢。 由於 /hana/資料的高級儲存磁碟通常不寫入加速器緩存,因此與 Ultra 磁碟相比,進階儲存上對 /hana/資料的寫入延遲更高。 可以預期,使用Ultra磁碟的儲存點寫入在Ultra磁碟上性能更好。

> [!IMPORTANT]
> 超級磁碟尚未存在於所有 Azure 區域中,並且還不支援下面列出的所有 VM 類型。 有關 Ultra 磁碟可用以及支援哪些 VM 系列的詳細資訊,請查看[「Azure 中有哪些磁碟類型可用?](https://docs.microsoft.com/azure/virtual-machines/windows/disks-types#ultra-disk)

### <a name="production-recommended-storage-solution-with-pure-ultra-disk-configuration"></a>使用純超磁碟配置生產推薦的儲存解決方案
在這裡設定中,您可以單獨保留 /hana/資料和 /hana/log 卷。 建議的值派生自 SAP 必須根據[SAP TDI 儲存白皮書](https://www.sap.com/documents/2015/03/74cdb554-5a7c-0010-82c7-eda71af511fa.html)中的建議對 SAP HANA 和儲存配置的 VM 類型進行驗證的 KPI。
...
列出的值旨在作為起點,需要根據實際需求進行評估。 Azure Ultra 磁碟的優點是可以調整 IOPS 和輸送量的值,而無需關閉 VM 或停止應用於系統的工作負載。   

> [!NOTE]
> 到目前為止,使用Ultra磁碟儲存的儲存快照不可用。 這會阻止使用 Azure 備份服務使用 VM 快照

### <a name="cost-conscious-storage-solution-with-pure-ultra-disk-configuration"></a>具有純超磁碟配置的注重成本的儲存解決方案
在這裡設定中,您在同一磁碟上的 /hana/資料和 /hana/log 卷。 建議的值派生自 SAP 必須根據[SAP TDI 儲存白皮書](https://www.sap.com/documents/2015/03/74cdb554-5a7c-0010-82c7-eda71af511fa.html)中的建議對 SAP HANA 和儲存配置的 VM 類型進行驗證的 KPI。 
...
## <a name="nfs-v41-volumes-on-azure-netapp-files"></a>Azure NetApp 檔案上的 NFS v4.1 卷
Azure NetApp 檔案提供本機 NFS 共用,可用於 /hana/共用、//哈納/數據和 /hana/log 卷。 對 /hana/資料和 /hana/log 卷使用基於 ANF 的 NFS 共用需要使用 v4.1 NFS 協定。 在基於 ANF 時,不支援使用 /hana/資料和 /hana/log 卷的 NFS 協定 v3。 

> [!IMPORTANT]
> 不支援在 Azure NetApp 檔上實現的 NFS v3 協定用於 /hana/數據**not**和 /hana/log。 從功能角度來看,NFS 4.1 對於 /hana/資料和 /hana/log 卷是必需的。 而對於 /hana/共用卷,可以從功能角度使用 NFS v3 或 NFS v4.1 協定。
...
- 最小容量池為 4 TiB。  
- 最小體積大小為 100 GiB
- Azure NetApp 檔案和將載入 Azure NetApp 檔案卷的所有虛擬機器必須位於同一 Azure 虛擬網路或同一區域中的[對等虛擬網路中](https://docs.microsoft.com/azure/virtual-network/virtual-network-peering-overview)。  
- 所選虛擬網路必須具有委派給 Azure NetApp 檔案的子網。
- Azure NetApp 卷的輸送量是卷配額和服務級別的函數,如[Azure NetApp 檔的服務級別](https://docs.microsoft.com/azure/azure-netapp-files/azure-netapp-files-service-levels)中所述。 調整 HANA Azure NetApp 卷的量量時,請確保生成的輸送量滿足 HANA 系統要求。  
- Azure NetApp 檔案提供[匯出策略](https://docs.microsoft.com/azure/azure-netapp-files/azure-netapp-files-configure-export-policy):您可以控制允許的用戶端、存取類型(讀取&寫入、僅讀取等)。 
- Azure NetApp 檔功能尚未了解區域。 當前,Azure NetApp 檔功能未部署在 Azure 區域中的所有可用性區域中。 請注意某些 Azure 區域的潛在延遲影響。  
- 將虛擬機部署在靠近 Azure NetApp 儲存的位置以降低延遲非常重要。 
- 虛擬機上的<b>sid</b>dm 的`sapsys`使用者 ID 和虛擬機器上的組 ID 必須與 Azure NetApp 檔中的配置匹配。 

> [!IMPORTANT]
> 對於 SAP HANA 工作負載,低延遲至關重要。 與 Microsoft 代表合作,確保虛擬機器和 Azure NetApp 檔卷在接近的地方部署。  

> [!IMPORTANT]
> 如果<b>sid</b>adm 的使用者 ID 與虛擬`sapsys`機器和 Azure NetApp 設定之間的群組 ID 不符合,則安裝在虛擬機器上的 Azure NetApp 卷上的檔案權限將顯示為`nobody`。 在[將新系統輸入](https://forms.office.com/Pages/ResponsePage.aspx?id=v4j5cvGGr0GRqy180BHbRxjSlHBUxkJBjmARn57skvdUQlJaV0ZBOE1PUkhOVk40WjZZQVJXRzI2RC4u)Azure NetApp 檔時,`sapsys`請確保為 sid <b>adm</b>指定正確的使用者 ID 和 組的 ID。

### <a name="sizing-for-hana-database-on-azure-netapp-files"></a>Azure NetApp 檔案上的 HANA 資料庫的大小

Azure NetApp 卷的輸送量是卷大小和服務級別的函數,如[Azure NetApp 檔的服務級別](https://docs.microsoft.com/azure/azure-netapp-files/azure-netapp-files-service-levels)中所述。 

在 Azure 中為 SAP 設計基礎結構時,應瞭解 SAP 的一些最低儲存輸送量要求,這轉化為以下最低輸送量特徵:
...
> [!IMPORTANT]
> 與您在單個 NFS 卷上部署的容量無關,輸送量預計將在虛擬機中的消費者利用的 1.2-1.4 GB/sec 頻寬範圍內穩定下來。 這與 ANF 產品提供的基礎體系結構和 NFS 的相關 Linux 會話限制有關。 在文章[「Azure NetApp 檔案的性能基準測試結果](https://docs.microsoft.com/azure/azure-netapp-files/azure-netapp-files-performance-benchmarks)」中記錄的性能和輸送量編號是針對具有多個用戶端 VM 的一個共用 NFS 卷進行的,因此具有多個作業階段。 這種情況不同於我們在 SAP 中測量的方案。 我們根據 NFS 卷測量單個 VM 的輸送量。 託管在 ANF 上。

為了滿足 SAP 資料和日誌的最低輸送量要求,並根據`/hana/shared`的準則,建議的大小如下所示:
...
> [!NOTE]
> 此處列出的 Azure NetApp 檔大小調整建議旨在滿足 SAP 對其基礎結構提供者表示的最低要求。 在實際的客戶部署和工作負載方案中,這可能是不夠的。 將這些建議作為起點,並根據特定工作負載的要求進行調整。  

因此,您可以考慮為已列出的超磁碟儲存的 ANF 卷部署類似的輸送量。 還要考慮為不同的 VM SKU 列出的大小的大小,就像 Ultra 磁碟錶中所做的那樣。

> [!TIP]
> 您可以動態調整 Azure NetApp 檔卷的大小,`unmount`而無需卷 、停止虛擬機或停止 SAP HANA。 這允許靈活性,以滿足您的應用程式的預期和不可預見的輸送量需求。

有關如何使用 ANF 中託管的 NFS v4.1 卷使用備用節點部署 SAP HANA 橫向擴充設定的文件在[SAP HANA 橫向擴充中發表,在 Azure VM 上使用在 SUSE Linux 企業伺服器上具有 Azure NetApp 檔案](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-hana-scale-out-standby-netapp-files-suse)。
