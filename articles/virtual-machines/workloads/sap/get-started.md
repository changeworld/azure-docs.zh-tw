---
title: 開始在 Azure VM 上使用 SAP |微軟文件
description: 瞭解在 Microsoft Azure 中的虛擬機器 (VM) 上執行的 SAP 解決方案
...
在 Azure 中託管 SAP 工作負載方案還可以創建標識集成和單一登錄的要求。 當您使用 Azure 活動目錄 (Azure AD) 連接不同的 SAP 元件和 SAP 軟體即服務 (SaaS) 或平臺即服務 (PaaS) 產品時,可能會出現這種情況。 與 Azure AD 和 SAP 實體的此類整合和單一登錄方案的清單在「AAD SAP 識別整合和單一登錄」一節中描述和記錄。

## <a name="changes-to-the-sap-workload-section"></a>對 SAP 工作負荷部份的變更
本文末尾列出了對 Azure 工作負荷上的 SAP 中文件的更改。 更改日誌中的條目將保留大約 180 天。

## <a name="you-want-to-know"></a>你想知道
如果您有具體問題,我們將在起始頁的這一部分中指向特定文檔或流。 您想知道:

- 支援哪些 SAP 軟體版本和哪些作業系統版本發布的 Azure VM 和 HANA 大型實例單元。 閱讀文件[Azure 部署支援哪些 SAP 軟體](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-supported-product-on-azure)以尋找答案以及搜尋資訊的過程
- Azure VM 和 HANA 大型實例支援哪些 SAP 部署方案。 有關支援的方案的資訊,請參閱文檔:
    - [Azure 虛擬機器支援的專案上的 SAP 工作負載](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-planning-supported-configurations)
    - [HANA 大型實體受支援的機制](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-supported-scenario)
- 哪些 Azure 服務、Azure VM 類型和 Azure 儲存服務在不同的 Azure 區域中可用,請[按區域](https://azure.microsoft.com/global-infrastructure/services/)檢查網站產品 

 
## <a name="sap-hana-on-azure-large-instances"></a>SAP HANA on Azure (大型執行個體)

一系列文檔引導您透過 Azure(大型實體)上的 SAP HANA,或者對於簡短的 HANA 大型實例。 有關 HANA 大型實體的資訊,從[Azure(大型實體)上的 SAP HANA 文件概述和體系結構開始,](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-overview-architecture)然後流覽 HANA 大型實體部分中的相關文件



## <a name="sap-hana-on-azure-virtual-machines"></a>Azure 虛擬機器上的 SAP HANA
文件的本節涵蓋 SAP HANA 的不同層面。 作為先決條件,您應該熟悉 Azure 提供 Azure IaaS 基本服務的主要服務。 因此,您需要瞭解 Azure 計算、儲存和網路。 其中許多主題在 SAP NetWeaver 相關的 Azure[規劃指南](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/planning-guide)中處理。 
...
本節列出了 AZURE 上的 SAP NetWeaver 和業務一體的規劃和部署文檔。 本文檔重點介紹 Azure 上具有 SAP 工作負荷的非 HANA 資料庫的基礎知識和使用。 高可用性的文件和文章也是 Azure 中 HANA 高可用性的基礎,例如:
...
- [在 Azure VM 上部署 SAP MaxDB、即時快取和內容伺服器](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/dbms_guide_maxdb)
...
本文件指向各種其他體系結構和方案文檔。 在以後的方案文檔中,提供了指向詳細技術文檔的鏈接,這些文檔解釋了不同高可用性方法的部署和配置。 演示如何為 SAP NetWeaver 工作負載建立和配置高可用性的不同文檔涵蓋 Linux 和 Windows 作業系統。


有關 Azure 活動目錄 (Azure AD) 和 SAP 服務和單一登入之間的整合的資訊,請參閱:
...
- 2020 年 3 月 26 日[:SAP 應用程式在 SLES 上的 Azure VM 上的 SAP NetWeaver 的高可用性](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse)變化[、SLES 上的 Azure VM 上的 SAP NetWeaver 的高可用性以及用於 SAP 應用程式的 Azure NetApp 檔案](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-netapp-files)[、SLES 上 Azure VM 上的 NFS 高可用性](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-nfs)[、SAP Net 的高可用性 SLES 多 SID 指南上的 Azure VM 上的 Weaver、SAP](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-multi-sid)應用程式[RHEL 上 Azure VM 上的 SAP NetWeaver 的高可用性](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-rhel)以及[RHEL 上的 Azure VM 上的 SAP NetWeaver 的高可用性,以及用於 SAP 應用程式的 Azure NetApp 檔以](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-rhel-netapp-files)更新圖表並闡明 Azure 負載均衡器後端池建立說明
- 03/19/2020:文檔[快速入門的主要修訂:在 Azure 虛擬機上手動安裝單實例 SAP HANA,](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-get-started)以[在 Azure 虛擬機器上安裝 SAP HANA](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-get-started)
- 03/17/2020:在[Azure 中的 SUSE Linux 企業伺服器上設定起搏器](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-pacemaker)以刪除不再需要的 SBD 配置設定
...
- 03/10/2020:更改[SAP HANA Azure 虛擬機儲存配置](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-vm-operations-storage),以明確 ANF 的實際現有輸送量限制
- 03/09/2020:SAP [NetWeaver 在 SUSE Linux 企業伺服器上的 Azure VM 上為 SAP 應用程式](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse)而高[可用性的變化,SUSE Linux 企業伺服器上的 Azure VM 上的 SAP NetWeaver 的高可用性,以及用於 SAP 應用程式的 Azure NetApp 檔](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-netapp-files),在[SUSE Linux 企業伺服器上的 Azure VM 上的 NFS 的高可用性](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-nfs)、[設定 步調 在 Azure 中的 SUSE Linux 企業伺服器上](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-pacemaker)[,IBM Db2 LUW 在 SUSE Linux 企業伺服器上的 Azure VM 上高可用性](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/dbms-guide-ha-ibm),[在 SUSE Linux 企業伺服器上的 Azure VM 上高可用性](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-hana-high-availability),以及[SLES 多 SID 指南上的 SAP NetWeaver 在 Azure VM 上的高可用性](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-multi-sid),以便使用資源代理 azure-lb 更新群集資源 
...
- 03/01/2020:Azure[虛擬機上的 SAP HANA](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-hana-backup-guide)返工備份指南,以包括 Azure 備份服務。 在檔等級減少和壓縮[SAP HANA Azure 備份](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-hana-backup-file-level)中的內容,並刪除了處理通過磁碟快照進行備份的第三個文檔。 內容在 Azure 虛擬機器上的 SAP HANA 備份指南中處理 
...
- 02/26/2020:更改[SAP HANA Azure 虛擬機器儲存配置](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-vm-operations-storage),以闡明 Azure 上 HANA 的檔案系統選擇
...
- 2020 年 2 月 26 日[:SAP 應用程式在 SLES 上的 Azure VM 上的 SAP NW 的高可用性](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse)變化[、SLES 上 Azure VM 上的 SAP NW 高可用性以及 SAP 應用程式的 ANF、RH SAP](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-netapp-files)[NetWeaver 的高可用性](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-rhel)以及[RHEL 上 SAP NetWeaver 的 Azure NetWeaver 高可用性以及 Azure NetApp 檔案](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-rhel-netapp-files),以刪除不支援多 SID ASCS/ERS 叢集的聲明
...
- 02/25/2020:[更改高可用性體系結構和方案,以便 SAP](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-high-availability-architecture-scenarios)新增指向較新的 HA 文章的連結
- 02/25/2020:IBM [Db2 LUW 在 SUSE Linux 企業伺服器上的 Azure VM 上的高可用性變化,使用起搏器](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/dbms-guide-ha-ibm)指向描述使用標準 Azure 負載均衡器訪問公共終結點的文檔
- 02/21/2020:完成對文章[SAP ASE Azure 虛擬機 DBMS 部署的 SAP 工作負荷](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/dbms_guide_sapase)的完整修訂
- 02/21/2020:更改[SAP HANA Azure 虛擬機器儲存配置](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-vm-operations-storage),以表示 /hana/資料條帶大小的新建議,並添加 I/O 計劃程式的設定
- 02/21/2020:HANA 大型實例文件的更改,以表示新認證的 S224 和 S224m SKU
- 2020 年 2 月 21 日:使用 Azure NetApp 檔案[在 RHEL 上更改 SAP NetWeaver 的 AZURE NetWeaver 的高可用性](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-rhel)和[Azure VM 的高可用性,用於使用 Azure NetApp 檔在 RHEL 上進行 SAP NetWeaver 的高可用性](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-rhel-netapp-files),以調整排隊伺服器複製 2 體系結構 (ENSA2) 的群集約束
- 02/20/2020:在[SLES 多 SID 指南上的 Azure VM 上 SAP NetWeaver 的高可用性](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-multi-sid)變化,以新增指向 SUSE 多 SID 叢集指南的連結
- 02/13/2020:對[Azure 虛擬機器的更改規劃和實現,以便 SAP NetWeaver](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/planning-guide)實施指向新文件的連結
- 02/13/2020:[在 Azure 虛擬機器支援的方案上](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-planning-supported-configurations)新增了新的文檔 SAP 工作負載
- 02/13/2020: 新增了新文件[Azure 部署支援哪些 SAP 軟體](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-supported-product-on-azure)
- 02/13/2020: [IBM Db2 LUW 在紅帽企業 Linux 伺服器上的 Azure VM 上的高可用性](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-rhel-ibm-db2-luw)變化,以指向描述使用標準 Azure 負載均衡器存取公共終結點的文檔
- 02/13/2020:將新的 VM 類型新增到[在 Microsoft Azure 執行的 SAP 認證和設定](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-certifications)
- 02/13/2020: 新增新的 SAP 支援說明[Azure 上的 SAP 工作負載:規劃和部署檢查表](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-deployment-checklist)
...
- 02/07/2020:在[SAP HA 方案中使用 Azure 標準 ILB 更改 VM 的公共終結點連接](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-standard-load-balancer-outbound-connections),以更新範例 NSG 螢幕截圖
- 02/03/2020:SAP[應用程式在 SLES 上的 Azure VM 上的 SAP NW 的高可用性](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse)變化,[在 SLES 上的 Azure VM 上對 SAP NW 的高可用性,用於 SAP 應用程式,](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-netapp-files)用於刪除有關在 SLES 上的群集節點主機名中使用破折號的警告
...
- 01/17/2020:使用[SAP 應用程式更改 Azure 接近放置組以獲得最佳網路延遲](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-proximity-placement-scenarios),以更改將現有 VM 移至鄰近放置元件
...
- 01/16/2020:更改[如何在 Azure 安裝與設定 SAP HANA(大型實例),](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-installation)讓作業系統版本適應 HANA IaaS 硬體目錄
- 2020 年 1 月 16 日:使用佇列服務 (ENSA2) 在[SLES 多 SID 指南上的 Azure VM 上的 SAP NetWeaver 的高可用性](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-multi-sid)更改,以新增 SAP 系統的說明
...
- 2019 年 11 月 15 日[:SUSE Linux 企業伺服器上 SAP NetWeaver 的高可用性略有變化,其中 Azure NetApp 檔適用於 SAP 應用程式](high-availability-guide-suse-netapp-files.md),而[在紅帽企業 Linux 上,SAP NetWeaver 具有用於 SAP 應用程式的 Azure NetApp 檔](high-availability-guide-rhel-netapp-files.md),以澄清容量池大小限制並刪除僅支援 NFSv3 版本的聲明。
- 2019/11/12:使用[Azure NetApp 檔案 (SMB) 在 Windows 上發表 SAP NetWeaver 的高可用性版本](high-availability-guide-windows-netapp-files-smb.md)
- 2019/11/08:在[SUSE Linux 企業伺服器上的 Azure VM 上設置 SAP HANA](sap-hana-high-availability.md)系統高可用性的變化,在 Azure 虛擬機[(VM) 上設置 SAP HANA 系統複製](sap-hana-high-availability-rhel.md),[在 SUSE Linux 企業伺服器上為 SAP NetWeaver 提供高可用性的 Azure](high-availability-guide-suse.md)虛擬機,以及[SUSE Linux 企業伺服器上 SAP NetWeaver 的 Azure 虛擬機高可用性。 Azure NetApp 檔案](high-availability-guide-suse-netapp-files.md), Azure 虛擬機器在[紅帽企業 Linux 上為 SAP NetWeaver 提供高可用性](high-availability-guide-rhel.md), [Azure 虛擬機器在紅帽企業 Linux 上高可用性 SAP NetWeaver 與 Azure Net App 檔案](high-availability-guide-rhel-netapp-files.md)、 [SUSE Linux 企業伺服器上的 Azure VM 上的 NFS 的高可用性](high-availability-guide-suse-nfs.md),[紅帽企業 Linux 上 Azure VM 上的 GlusterFS, SAP NetWeaver](high-availability-guide-rhel-glusterfs.md)推薦 Azure 標準負載均衡器  
- 2019/11/08:SAP[工作負載規劃和部署清單](sap-deployment-checklist.md)的更改,以澄清加密建議  
- 2019/11/04:在[Azure 中的 SUSE Linux 企業伺服器上設定起搏器](high-availability-guide-suse-pacemaker.md)以直接使用單播設定創建群集的更改  
- 2019 年 10 月 29 日:[在 SAP 高可用性方案中使用 Azure 標準負載均衡器釋放虛擬機器的公共終結點連接](high-availability-guide-standard-load-balancer-outbound-connections.md)
- 2019 年 10 月 25 日[:SAP HANA Azure 虛擬機器儲存設定](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-vm-operations-storage)的更改和[SAP HANA 擴充,在 Azure VM 上使用 Azure NetApp 檔在 SUSE Linux 企業伺服器上使用備用節點](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-hana-scale-out-standby-netapp-files-suse),以闡明 /hana/共用卷的 NFS 協定
- 2019/10/22:SAP [NetWeaver 在 SUSE Linux 企業伺服器上的 Azure VM 上為 SAP 應用程式的高可用性](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse)變化[,SUSE Linux 企業伺服器上的 Azure VM 上的 SAP NetWeaver 的高可用性,以及用於 SAP 應用程式的 Az NetApp 檔案](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-netapp-files)[、Azure VM 上的 NFS 高可用性 SUSE Linux 企業伺服器](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-nfs):[在 Azure 中的 SUSE Linux 企業伺服器上設置起搏器](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-pacemaker); 在[SUSE Linux 企業伺服器上的 Azure VM 上使用起搏器的高可用性 IBM Db2 LUW",](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/dbms-guide-ha-ibm)以及[SUSE Linux 企業伺服器上的 Azure VM 上用於](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-hana-high-availability)Azure 負載均衡器檢測強化的高可用性 SAP HANA
- 在[SAP HANA Azure 虛擬機器儲存設定](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-vm-operations-storage)中變更 ANF 部份和標頭部分
...
- 2019 年 10 月 11 日:更改超磁碟儲存配置,並在[SAP HANA Azure 虛擬機器儲存配置](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-vm-operations-storage)中引入 ANF
...
- 08/22/2019:在[Azure 中的 SUSE Linux 企業伺服器上設定起搏器](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-pacemaker)以更新自定義角色創建的網址的更改  
- 08/16/2019:在[Azure 中的紅帽企業 Linux 上設置起搏器的](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-rhel-pacemaker)更改,以提醒客戶在自定義角色中更新操作(如果更新到新版本的 Azure 圍欄代理)  
- 08/15/2019:SAP [HANA Azure 虛擬機儲存配置的變化](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-vm-operations-storage),以反映超磁碟(以前超 SSD)的通用可用性
- 08/01/2019:在[Azure 中的 SUSE Linux 企業伺服器上設置起搏器](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-pacemaker)的更改,以整合專為 SLES 15 所做的更改 
