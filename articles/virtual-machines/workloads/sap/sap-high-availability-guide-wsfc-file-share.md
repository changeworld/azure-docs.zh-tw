...
SAP 叢集 \<SID\> 角色不包含叢集共用磁片或一般檔案共用叢集資源。
...
您可以使用自己的 SAP 叢集角色，在一個叢集中部署 SAP ASCS/SCS 實例 \<SID\> 。 在此情況下，您要使用其他叢集角色，在另一個叢集上設定向外延展檔案共用。
...
* [部署雙節點儲存空間直接存取向外延展檔案伺服器以在 Azure 中儲存 UPD][deploy-sofs-s2d-in-azure]
