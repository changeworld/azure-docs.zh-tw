---
title: SLES 上的 Azure VM 上 SAP HANA 的高可用性 |微軟文件
...
* SAP Note [2205917]已為 SAP 應用程式的 SUSE Linux 企業伺服器推薦了作業系統設置。
* SAP 說明[1944799]具有 SAP HANA 指南,適用於 SAP 應用程式的 SUSE Linux 企業伺服器。
...
1. 輸入下列參數：
    ...
    - **堆疊類型**:(此參數僅在使用收斂範本時才適用。選擇 SAP NetWeaver 堆疊類型。
    - **OS 類型**：選取其中一個 Linux 發行版本。 這個選項, 選擇**SLES 12**。
    - **DB 類型**：選取 **HANA**。
    - **SAP 系統大小**：輸入新系統要提供的 SAP 數量。 如果您不確定系統需要多少 SAP，請詢問您的 SAP 技術合作夥伴或系統整合者。
    - **系統可用性**選取 [HA]****。
    - **管理員使用者名和密碼**:將創建一個新使用者,可用於登錄到計算機。
...
1. 建立可用性設定組。
   - 設定更新網域上限。
1. 建立負載平衡器 (內部)。 我們建議[使用標準負載均衡器](https://docs.microsoft.com/azure/load-balancer/load-balancer-standard-overview)。
   - 選取步驟 2 所建立的虛擬網路。
1. 建立虛擬機器 1。
...
1. 建立虛擬機器 2。
   ...
   - 選取步驟 3 所建立的可用性設定組。 
1. 新增資料磁碟。
1. 如果使用標準負載均衡器,請按照以下配置步驟操作:
   1. 首先，建立前端 IP 集區：
   
      1. 開啟負載平衡器，選取 [前端 IP 集區]****，然後選取 [新增]****。
      1. 輸入新的前端 IP 池的名稱(例如,**哈納前端**)。
      1. 將 **「分配分配」** 設定為**靜態**並輸入 IP 位址(例如 **,10.0.0.13**)。
      1. 選取 [確定]  。
   ...
   1. 接下來，建立後端集區：
      ...
      1. 輸入新後端池的名稱(例如 **,hana後端**)。
      1. 選擇**虛擬網路**。
      1. 選取 [新增虛擬機器]****。
      1. 選擇 [虛擬機]。
      1. 選擇 SAP HANA 群集的虛擬機器及其 IP 位址。
      1. 選取 [新增]  。
   
   1. 接下來，建立健康情況探查：
   
      1. 開啟負載平衡器，選取 [健康情況探查]****，然後選取 [新增]****。
      1. 輸入新運行狀況探測的名稱(例如 **,hana-hp)。**
   ...
   1. 接下來,建立負載平衡規則:
      ...
      1. 輸入新的負載均衡器規則的名稱(例如 **,hana-lb**)。
      1. 選擇前端 IP 位址、後端介面和您之前建立的運行狀況探測(例如,**哈納前端**、**哈納後端**和**hana-hp)。**
      1. 選擇**HA 連接埠**。
      1. 將 [閒置逾時]**** 增加為 30 分鐘。
      1. 請確保**啟用浮動 IP**。
      1. 選取 [確定]  。
   ...
   > [!Note]
   > 當沒有公共 IP 位址的 VM 放置在內部(無公共 IP 位址)標準 Azure 負載均衡器的後端池中時,將沒有出站網路連接,除非執行其他配置以允許路由到公共終結點。 有關如何實現出站連接的詳細資訊,請參閱[在 SAP 高可用性方案中使用 Azure 標準負載均衡器的虛擬機器的公共終結點連接](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-standard-load-balancer-outbound-connections)。  

1. 或者,如果方案要求使用基本負載均衡器,請按照以下配置步驟操作:
   1. 首先，建立前端 IP 集區：
   ...
   1. 針對 SAP HANA 1.0，建立負載平衡規則：
      ...
      1. 開啟負載平衡器，選取 [負載平衡規則]****，然後選取 [新增]****。
      1. 輸入新的負載均衡器規則的名稱(例如,hana-lb-3**03**15)。
   ...
   1. 若為 SAP Hana 2.0，請針對系統資料庫建立負載平衡規則：
      ...
      1. 開啟負載平衡器，選取 [負載平衡規則]****，然後選取 [新增]****。
      1. 輸入新的負載均衡器規則的名稱(例如,hana-lb-3**03**13)。
      1. 選取您稍早建立的前端 IP 位址、後端集區及健康情況探查 (例如 **hana-frontend**)。
   ...
   1. 若為 SAP Hana 2.0，請先針對租用戶資料庫建立負載平衡規則：
      ...
      1. 開啟負載平衡器，選取 [負載平衡規則]****，然後選取 [新增]****。
      1. 輸入新的負載均衡器規則的名稱(例如,hana-lb-3**03**40)。
      1. 選擇您之前創建的前端 IP 位址、後端池和運行狀況探測(例如 **,hana 前端**)。
...
> [!IMPORTANT]
> 請勿在位於 Azure 負載均衡器後面的 Azure VM 上啟用 TCP 時間戳。 啟用 TCP 時間戳將導致運行狀況探測失敗。 將參數**net.ipv4.tcp_timestamps**設定為**0**。 有關詳細資訊,請參閱[負載均衡器執行狀況探測器](https://docs.microsoft.com/azure/load-balancer/load-balancer-custom-probe-overview)。
> 另請參閱 SAP 註釋[2382421](https://launchpad.support.sap.com/#/notes/2382421)。 
...
1. **[A]** 設定磁碟配置：**邏輯磁碟區管理員 (LVM)**。
   ...
   建立邏輯磁碟區。 當您使用 `lvcreate` 卻未搭配 `-i` 參數時，會建立線性磁碟區。 我們建議您創建條帶化卷以更好的 I/O 性能,並將條帶大小與[SAP HANA VM 儲存配置](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-vm-operations-storage)中記錄的值對齊。 參數`-i`應為基礎物理卷的數量`-I`, 並且參數為條帶大小。 本文件會使用 2 個實體磁碟區來作為資料磁碟區，因此 `-i` 參數引數會設定為 **2**。 數據卷的條帶大小為**256KiB。** 日誌卷使用一個物理卷,因此不會顯式用於`-i`日誌`-I`卷命令或開關。  

   > [!IMPORTANT]
   > 當您要對每個資料、記錄或共用磁碟區使用多個實體磁碟區時，請使用 `-i` 參數，並將其值設定為基礎實體磁碟區的數目。 建立條`-I`帶卷時,使用開關指定條帶大小。  
   > 有關建議的儲存設定(包括條帶大小和磁碟數),請參閱[SAP HANA VM 儲存配置](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-vm-operations-storage)。  
...
1. **[1]** 建立租用戶資料庫。
   ...
   如果您使用 SAP HANA 2.0 或 MDC，請為您的 SAP NetWeaver 系統建立租用戶資料庫。 將**NW1**替換為 SAP 系統的 SID。
...
1. **[2]** 在第二個節點上設定系統複寫：
   ...
   註冊第二個節點，以啟動系統複寫。 將以下指令執行為 <\>哈納西德 adm :
...
1. **[1]** 創建所需的使用者。
   ...
   將以下命令作為根運行。 請確定將粗體字串(HANA 系統 ID **HN1**和實體編號**03)** 取代為 SAP HANA 安裝的值:
...
> [!IMPORTANT]
> 最近的測試揭示了 Netcat 由於積壓和僅處理一個連接的限制而停止回應請求的情況。 netcat 資源停止偵聽 Azure 負載均衡器請求,浮動 IP 將不可用。  
...
>
> 請注意,更改將需要短暫的停機時間。  
> 對於現有的起搏器群集,如果配置已更改為使用[Azure 負載均衡器檢測強化](https://www.suse.com/support/kb/doc/?id=7024128)中所述的 socat,則無需立即切換到 Azure-lb 資源代理。
...
1. 測試 1：在節點 1 上停止主要資料庫
   ...
   在節點 hn1-db-0\>上<hanasid adm 執行以下指令:
...
1. 測試 2：在節點 2 上停止主要資料庫
   ...
   在節點 hn1-db-1\>上<hanasid adm 執行以下指令:
