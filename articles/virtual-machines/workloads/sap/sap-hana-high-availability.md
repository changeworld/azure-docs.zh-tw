---
title: SLES 上的 Azure VM 上 SAP HANA 的高可用性 |微軟文檔
...
* SAP 說明[1944799]具有 SAP HANA 指南，適用于 SAP 應用程式的 SUSE Linux 企業伺服器。
...
1. 輸入下列參數：
    ...
    - **OS 類型**：選取其中一個 Linux 發行版本。 在此示例中，選擇**SLES 12**。
    - **DB 類型**：選取 **HANA**。
    - **SAP 系統大小**：輸入新系統要提供的 SAP 數量。 如果您不確定系統需要多少 SAP，請詢問您的 SAP 技術合作夥伴或系統整合者。
    - **系統可用性**選取 [HA]****。
    - **管理員使用者名和密碼**：將創建一個新使用者，可用於登錄到電腦。
...
1. **[2]** 在第二個節點上設定系統複寫：
   ...
   註冊第二個節點，以啟動系統複寫。 將以下命令運行為 <哈\>納西德 adm ：
...
1. 測試 1：在節點 1 上停止主要資料庫
   ...
   在節點 hn1-db-0\>上<hanasid adm 運行以下命令：
...
1. 測試 2：在節點 2 上停止主要資料庫
   ...
   在節點 hn1-db-1\>上<hanasid adm 運行以下命令：
