---
title: SAP HANA 擴展,在 RHEL 上使用 Azure NetApp 檔進行待機。微軟文件
description: 紅帽企業 Linux 上的 SAP NetWeaver 高可用性指南,用於 SAP 應用程式的 Azure NetApp 檔案
...
ms.locfileid: "80754162"
---
# <a name="deploy-a-sap-hana-scale-out-system-with-standby-node-on-azure-vms-by-using-azure-netapp-files-on-red-hat-enterprise-linux"></a>使用紅帽企業 Linux 上的 Azure NetApp 檔案,在 Azure VM 上部署具有備用節點的 SAP HANA 橫向擴充系統 
...
本文介紹如何通過將[Azure NetApp 檔案](https://docs.microsoft.com/azure/azure-netapp-files/azure-netapp-files-introduction/)用於共用儲存卷,在 Azure 紅帽企業 Linux 虛擬機器 (VM) 上具有待機狀態的橫向擴充配置中部署高可用性 SAP HANA 系統。  

在範例設定、安裝命令等中,HANA 實體為**03,HANA**系統 ID 為**HN1**。 這些範例基於 SAP 7.6 的 HANA 2.0 SP4 和紅帽企業 Linux。 
...
* SAP 說明 [2002167] 已建議為紅帽企業 Linux 設定作業系統設定
...
對於此範例設定,子網是:  
  ...
  - `storage`10.9.3.0/26  
  - `hana`10.9.2.0/26  
  - `anf`10.9.0.0/26(委派子網到 Azure NetApp 檔案)
...
5. 按照為[Azure NetApp 檔案建立 NFS 卷](https://docs.microsoft.com/azure/azure-netapp-files/azure-netapp-files-create-volumes)中的說明部署 Azure NetApp 檔卷。  
   ...
   請記住,Azure NetApp 檔案資源和 Azure VM 必須位於同一 Azure 虛擬網路或對等 Azure 虛擬網路中。 例如 **,HN1**-data-mnt00001、HN1 -log-mnt00001 等,是卷名稱,nfs://10.9.0.4/ ** ** **HN1**-數據-mnt00001、nfs://10.9.0.4/**HN1**-log-mnt00001 等,是 Azure NetApp 檔卷的文件路徑。  

   * 捲**HN1**-資料 mnt00001 (nfs://10.9.0.4/**HN1**-資料 mnt00001)
   * 捲**HN1**-資料 mnt00002 (nfs://10.9.0.4/**HN1**-資料 mnt00002)
   ...
   * 卷**HN1**-共用 (nfs://10.9.0.4/**HN1**-共用)
   ...
   在此示例中,我們為每個 HANA 數據和日誌卷使用了單獨的 Azure NetApp 檔卷。 要在較小或非生產性系統上進行成本優化的配置,可以將所有數據裝載放在單個卷上,並且所有日誌都安裝在不同的單個卷上。  

### <a name="important-considerations"></a>重要考量︰

在建立 Azure NetApp 檔案以使用備用節點方案進行 SAP HANA 橫向延伸時,請注意以下重要注意事項:
...
- 讀取 16 MB 和 64 MB I/O 大小的 /hana/數據至少 400 MB/s 的活動。  
- 使用 16 MB 和 64 MB I/O 大小為 /hana/數據寫入至少 250 MB/s 的活動。 
...
3. **[A]** 添加網络路由,以便與 Azure NetApp 檔的通信通過存儲網路介面進行。  

   在此範例中,將用於`Networkmanager`配置其他網路路由。 以下說明假定儲存網路介面為`eth1`。  
   首先,確定設備`eth1`的連接名稱。 這個功能中,裝置的`eth1`連線名稱稱為`Wired connection 1`。  
   ...
   然後透過`eth1`配置到 Azure NetApp 檔委派網路的其他路由。  
...
3. **[A]** 準備作業系統,以便使用 NFS 在 Azure NetApp 上運行 SAP HANA,如[使用 Azure NetApp 檔案在 Microsoft Azure 上的 NetApp SAP 應用程式][anf-sap-applications-azure]中所述。 為 NetApp 設定設定建立設定檔 */etc/sysctl.d/netapp-hana.conf。*  
...
5. **[A]** 使用 Azure NetApp 檔調整 Sunrpc 設置,如[Microsoft Azure 上的 NetApp SAP 應用程式][anf-sap-applications-azure]中的建議。  
...
6. **[A]** 用於 HANA 配置的紅帽。
    ...
    設定 RHEL,如 SAP 說明<https://access.redhat.com/solutions/2447641>2292690、2455582、2593824 和 。 [2292690] [2455582] [2593824]

    > [!NOTE]
    > 如果安裝 HANA 2.0 SP04,`compat-sap-c++-7`則需要按照 SAP 說明[2593824]中所述安裝套件,然後才能安裝 SAP HANA。 
...
4. **[A]**`nfs4_disable_idmapping`驗證 。 應將設定為**Y**。要創建位於的位置`nfs4_disable_idmapping`的目錄結構,請執行裝載命令。 您將無法在 /sys/模組下手動創建目錄,因為造訪是為內核/驅動程式保留的。  
   ...
   有關如何更改`nfs4_disable_idmapping`參數的更多詳細資訊,請參https://access.redhat.com/solutions/1749883閱 。
