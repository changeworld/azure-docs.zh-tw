---
title: RHEL 上的 Azure VM 上 SAP HANA 的高可用性 |微軟文件
...
* [Linux 上為 SAP 部署的 Azure 虛擬機器(本文)][deployment-guide]
...
1. 輸入下列參數：
    ...
    * **OS 類型**：選取其中一個 Linux 發行版本。 這個範例中,選擇**RHEL 7**。
    * **DB 類型**：選取 **HANA**。
    * **SAP 系統大小**：輸入新系統要提供的 SAP 數量。 如果您不確定系統需要多少 SAP，請詢問您的 SAP 技術合作夥伴或系統整合者。
    * **系統可用性**選取 [HA]****。
    * **管理員使用者名、管理員密碼或 SSH 金鑰**:創建了一個新使用者,可用於登錄到電腦。
    * **子網 ID**:如果要將 VM 部署到已定義子網的現有 VNet 中,應為其分配 VM,則命名該特定子網的 ID。 識別碼通常如下所示：**/subscriptions/\<訂用帳戶識別碼>/resourceGroups/\<資源群組名稱>/providers/Microsoft.Network/virtualNetworks/\<虛擬網路名稱>/subnets/\<子網路名稱>**。 如果您想要建立新的虛擬網路，請保留空白
...
1. **[2]** 在第二個節點上設定系統複寫：
   ...
   註冊第二個節點，以啟動系統複寫。 將以下指令執行為<個 hanasid\>adm:
...
1. **[1]** 檢查複寫狀態
   ...
   檢查複製狀態並等待所有資料庫同步。如果狀態保持未知狀態,請檢查防火牆設置。
...
> [!NOTE]
> 上述配置中的超時只是示例,可能需要適應特定的 HANA 設置。 例如,如果啟動 SAP HANA 資料庫需要更長的時間,則可能需要增加啟動超時。  
...
有關如何模擬網路故障的說明,請參閱[紅帽知識庫文章 79523。](https://access.redhat.com/solutions/79523) 在此範例中，我們會使用 net_breaker 指令碼來封鎖所有對網路的存取。
