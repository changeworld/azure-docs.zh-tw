---
title: RHEL 上的 Azure VM 上 SAP HANA 的高可用性 |微軟文檔
...
1. 輸入下列參數：
    ...
    * **OS 類型**：選取其中一個 Linux 發行版本。 在此示例中，選擇**RHEL 7**。
    * **DB 類型**：選取 **HANA**。
...
1. 建立可用性設定組。  
   設定更新網域上限。
1. 建立負載平衡器 (內部)。 我們建議[使用標準負載等化器](https://docs.microsoft.com/azure/load-balancer/load-balancer-standard-overview)。
   * 選取步驟 2 所建立的虛擬網路。
1. 建立虛擬機器 1。  
...
1. 建立虛擬機器 2。  
   至少使用 Red Hat Enterprise Linux for SAP HANA 7.4。 這個範例會使用 Red Hat Enterprise Linux for SAP HANA 7.4 的映像 <https://portal.azure.com/#create/RedHat.RedHatEnterpriseLinux75forSAP-ARM> 選取在步驟 3 建立的可用性設定組。
1. 新增資料磁碟。
1. 如果使用標準負載等化器，請按照以下配置步驟操作：
   1. 首先，建立前端 IP 集區：

      1. 開啟負載平衡器，選取 [前端 IP 集區]****，然後選取 [新增]****。
      1. 輸入新的前端 IP 池的名稱（例如，**哈納前端**）。
      1. 將 **"分配分配"** 設置為**靜態**並輸入 IP 位址（例如 **，10.0.0.13**）。
      1. 選取 [確定]****。
   ...
   1. 接下來，創建負載平衡規則：
      ...
      1. 輸入新的負載等化器規則的名稱（例如 **，hana-lb**）。
      1. 選擇前端 IP 位址、後端池和您之前創建的運行狀況探測（例如，**哈納前端**、**哈納後端**和**hana-hp）。**
      1. 選擇**HA 埠**。
      1. 將 [閒置逾時]**** 增加為 30 分鐘。
      1. 請確保**啟用浮動 IP**。
      1. 選取 [確定]****。
...
1. 或者，如果方案要求使用基本負載等化器，請按照以下配置步驟操作：
   ...
   1. 針對 SAP HANA 1.0，建立負載平衡規則：
      ...
      1. 開啟負載平衡器，選取 [負載平衡規則]****，然後選取 [新增]****。
      1. 輸入新的負載等化器規則的名稱（例如，hana-lb-3**03**15）。
   ...
   1. 若為 SAP Hana 2.0，請針對系統資料庫建立負載平衡規則：
      ...
      1. 開啟負載平衡器，選取 [負載平衡規則]****，然後選取 [新增]****。
      1. 輸入新的負載等化器規則的名稱（例如，hana-lb-3**03**13）。
      1. 選取您稍早建立的前端 IP 位址、後端集區及健康情況探查 (例如 **hana-frontend**)。
   ...
   1. 若為 SAP Hana 2.0，請先針對租用戶資料庫建立負載平衡規則：
      ...
      1. 開啟負載平衡器，選取 [負載平衡規則]****，然後選取 [新增]****。
      1. 輸入新的負載等化器規則的名稱（例如，hana-lb-3**03**40）。
...
> 另請參閱 SAP 注釋[2382421](https://launchpad.support.sap.com/#/notes/2382421)。 
...
1. **[2]** 在第二個節點上設定系統複寫：
   ...
   註冊第二個節點，以啟動系統複寫。 將以下命令運行為<個 hanasid\>adm：
...
1. **[1]** 創建所需的使用者。
   ...
   將以下命令作為根運行。 請確保將粗體字串（HANA 系統 ID **HN1**和實例編號**03）** 替換為 SAP HANA 安裝的值：
...
有關如何類比網路故障的說明，請參閱[紅帽知識庫文章 79523。](https://access.redhat.com/solutions/79523) 在此範例中，我們會使用 net_breaker 指令碼來封鎖所有對網路的存取。
