...
2.  技術設計檔。 本檔應包含：
    ...
    - 高可用性和嚴重損壞修復架構。
        ...
        - 若要瞭解區域內的高可用性，請檢查所需的 DBMS 在 Azure 中提供的功能。 大部分 DBMS 封裝都提供同步熱待命的同步方法，我們建議用於生產系統。 此外，請檢查適用于不同資料庫的 SAP 相關檔，從適用于 [sap 工作負載的 Azure 虛擬機器 DBMS 部署](./dbms_guide_general.md) 和相關檔的考慮開始。
           不支援使用 Windows Server 容錯移轉叢集搭配 DBMS 層的共用磁片設定，例如 [SQL Server 所述](/sql/sql-server/failover-clusters/windows/always-on-failover-cluster-instances-sql-server?view=sql-server-2017)。 相反地，請使用類似下列的解決方案：
