...
本文說明如何在 Azure 虛擬機器（Vm）上，使用適用于共用存放磁片區的[Azure NetApp Files](../../../azure-netapp-files/azure-netapp-files-introduction.md) ，在向外延展設定中部署高可用性 SAP Hana 系統。  
...
5. 遵循[建立適用于 Azure Netapp files 的 NFS 磁片](../../../azure-netapp-files/azure-netapp-files-create-volumes.md)區中的指示，部署 Azure netapp files 磁片區。  
   ...
   當您要部署磁片區時，請務必選取**nfsv 4.1**版本。 目前，必須將 NFSv 4.1 的存取權新增至允許清單。 在指定的 Azure NetApp Files [子網路](/rest/api/virtualnetwork/subnets)中部署磁碟區。 Azure NetApp 磁碟區的 IP 位址會自動指派。 
...
| HANA/shared | 每4個背景工作節點最大值（512 GB、1xRAM） | 每4個背景工作節點最大值（512 GB、1xRAM） | v3 或 4.1 版 |
