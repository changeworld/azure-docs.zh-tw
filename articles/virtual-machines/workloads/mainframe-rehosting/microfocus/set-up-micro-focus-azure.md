...
description: 使用 Azure 虛擬機器上的微焦點開發和測試環境 (Vm) ，來重新裝載您的 IBM z/OS 大型主機工作負載。
...
企業伺服器的對應開發環境是企業開發人員，它會在 Microsoft Visual Studio 2017 或更新版本上執行，Visual Studio Community (免費下載) 或 Eclipse。 本文說明如何使用已安裝 Visual Studio 2017 或更新版本的 Windows Server 2016 虛擬機器來部署它。
...
2.  建立虛擬機器。 從 [Azure Marketplace] 中，選取您想要的虛擬機器和作業系統。 以下是建議的設定：
    ...
    -   **企業伺服器：** 選取 **ES2 V3 VM** (，其中包含2個 vcpu 和 16 GB 記憶體) （已安裝 Windows Server 2016 和 SQL Server 2017）。 此映射可從 Azure Marketplace 取得。 企業伺服器也可以使用 Azure SQL Database。

    -   **企業開發人員：** 選取 [ **B2MS VM** (，其中包含2個 vcpu 和 8 GB 記憶體) 並已安裝 Windows 10 和 Visual Studio。 此映射可從 Azure Marketplace 取得。
...
6.  建立虛擬機器之後，開啟適用于 HTTP 和**3389**的輸入埠**9003、86**和**80** ，以在企業伺服器電腦上遠端桌面通訊協定 (RDP) ，以及開發人員電腦上的**3389** 。
...
8.  在 RDP 會話中，載入下列兩個檔案。 因為您使用的是 Windows，所以可以將檔案拖放到 RDP 會話：
    ...
    -   `es\_50.exe`，也就是企業伺服器安裝檔案。

    -   `mflic`，對應的授權檔案—企業伺服器不會在沒有它的情況下載入。
...
2.  選取 [ **啟動** \> **微焦點授權管理員** \> **授權管理**]，然後按一下 [ **安裝** ] 索引標籤。選擇要上傳的授權格式類型：授權檔案或16個字元的授權碼。 例如，針對檔案，在 [ **授權檔案**] 中，流覽至 `mflic` 先前上傳至 VM 的 * 檔案，然後選取 [ **安裝授權**]。
...
3.  如果您想要) ，請從 RDP 會話載入下列兩個檔案 (拖放動作：

    -   `edvs2017.exe`，也就是企業伺服器安裝檔案。

    -   `mflic`， (企業開發人員的對應授權檔案不會) 載入。
...
7.  選擇要上傳的授權格式類型：授權檔案或16字元授權碼。 例如，針對檔案，在 [ **授權檔案**] 中，流覽至 `mflic` 先前上傳至 VM 的檔案，然後選取 [  **安裝授權**]。
