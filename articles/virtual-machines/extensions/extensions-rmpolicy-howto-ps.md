---
title: 使用 Azure 原則 (Windows) 限制 VM 延伸模組的安裝
...
若要測試原則，請嘗試使用「VM 存取」延伸模組。 下列內容應該會失敗，並顯示「原則不允許資源 ' Myvmaccess」訊息 '」訊息。
