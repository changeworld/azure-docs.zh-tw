---
title: 適用于 Windows 的 Azure 自訂腳本擴展
...
ms.locfileid: "80066882"
---
# <a name="custom-script-extension-for-windows"></a>Windows 的自訂指令碼延伸模組

「自訂指令碼擴充功能」會在 Azure 虛擬機器上下載並執行指令碼。 此擴展可用於部署後配置、軟體安裝或任何其他配置或管理工作。 您可以從 Azure 儲存體或 GitHub 下載指令碼，或是在擴充功能執行階段將指令碼提供給 Azure 入口網站。 自訂腳本擴展與 Azure 資源管理器樣板集成，可以使用 Azure CLI、PowerShell、Azure 門戶或 Azure 虛擬機器 REST API 運行。
...
Windows 的自訂腳本擴展將在受擴展支援擴展擴展作業系統上運行，有關詳細資訊，請參閱此[Azure 擴展支援的作業系統](https://support.microsoft.com/help/4078134/azure-extension-supported-operating-systems)。

### <a name="script-location"></a>指令碼位置

可以將擴展配置為使用 Azure Blob 存儲憑據訪問 Azure Blob 存儲。 只要 VM 可以路由到該端點（如 GitHub 或內部檔案伺服器），腳本位置可以位於任何位置。

### <a name="internet-connectivity"></a>網際網路連線

如果需要從外部下載腳本（如從 GitHub 或 Azure 存儲）下載腳本，則需要打開其他防火牆和網路安全性群組埠。 例如，如果腳本位於 Azure 存儲中，則可以使用 Azure NSG 服務標記進行[存儲](../../virtual-network/security-overview.md#service-tags)。

如果腳本位於本機伺服器上，則可能還需要打開其他防火牆和網路安全性群組埠。
...
* 如果腳本會導致重新開機，則安裝應用程式並運行腳本，則可以使用 Windows 計畫任務計畫重新開機，或使用 DSC、Chef 或 Puppet 擴展等工具。
...
* 自訂腳本副檔名不支援代理伺服器，但您可以使用支援腳本中的代理伺服器的檔案傳輸工具，如*Curl*
...
> [!NOTE]
> 託管識別屬性**不得**與存儲帳戶名稱或存儲帳戶金鑰屬性結合使用

> [!NOTE]
> 一個時間點只能在 VM 上安裝一個版本的擴展，在同一資源管理器範本中為同一 VM 指定自訂腳本兩次將失敗。

> [!NOTE]
> 我們可以在虛擬機器資源中使用此架構，也可以用作獨立資源。 如果此擴展在 ARM 範本中用作獨立資源，則資源的名稱必須在此格式為"虛擬機器名稱/副檔名稱"。 
...
| 託管身份（例如） | [ ] 或 " 用戶端 Id"：" 31b403a-c364-4240-a7ff-d85fb6cd7232" * 或 { "objectId"：" 12dd289c-0583-46e3-b9b4-115d5c19ef4b" | | json 物件 |
...
* `managedIdentity`：（可選，json 物件）用於下載檔案的[託管標識](https://docs.microsoft.com/azure/active-directory/managed-identities-azure-resources/overview)
  * `clientId`：（可選，字串）託管標識的用戶端 ID
  * `objectId`：（可選，字串）託管標識的物件識別碼
...
####  <a name="property-managedidentity"></a>屬性：託管標識

CustomScript（版本 1.10 以後）支援從"fileUris"設置中提供的 URL 下載檔案的[託管標識](https://docs.microsoft.com/azure/active-directory/managed-identities-azure-resources/overview)。 它允許 CustomScript 訪問 Azure 存儲專用 Blob 或容器，而無需使用者傳遞 SAS 權杖或存儲帳戶金鑰等機密。

要使用此功能，使用者必須向預期運行 CustomScript 的 VM 或 VMSS 添加[系統分配](https://docs.microsoft.com/azure/app-service/overview-managed-identity?tabs=dotnet#add-a-system-assigned-identity)或[使用者分配的](https://docs.microsoft.com/azure/app-service/overview-managed-identity?tabs=dotnet#add-a-user-assigned-identity)標識，並[授予對 Azure 存儲容器或 blob 的託管標識存取權限](https://docs.microsoft.com/azure/active-directory/managed-identities-azure-resources/tutorial-vm-windows-access-storage#grant-access)。
...
要在目標 VM/VMSS 上使用使用者分配的標識，請使用用戶端 ID 或託管標識的物件識別碼 配置"託管標識"欄位。
...
也可以使用 Azure Resource Manager 範本部署 Azure VM 擴充功能。 JSON 架構（上一節中詳細介紹）可用於 Azure 資源管理器範本，用於在部署期間運行自訂腳本擴展。 下列範例會說明如何使用自訂指令碼擴充功能：
...
在此示例中，您有三個腳本用於構建伺服器。 **命令ToExecute**調用第一個腳本，然後你有關于如何調用其他腳本的選項。 例如，您可以有一個主腳本來控制執行，並具有正確的錯誤處理、日誌記錄和狀態管理。 腳本將下載到本地電腦以進行運行。 例如，在`1_Add_Tools.ps1`中，您可以通過`2_Add_Features.ps1`添加到`.\2_Add_Features.ps1`腳本來調用，並在 中`$settings`定義的其他腳本重複此過程。
...
在此示例中，您可能希望將本地 SMB 伺服器用於腳本位置。 通過執行此操作，您不需要提供任何其他設置，但**命令ToExecute**。
...
或者，您可以將[ForceUpdateTag](/dotnet/api/microsoft.azure.management.compute.models.virtualmachineextension.forceupdatetag)屬性設置為**true**。

### <a name="using-invoke-webrequest"></a>使用調用 Web 請求

如果在腳本中使用[Invoke-WebRequest，](/powershell/module/microsoft.powershell.utility/invoke-webrequest)則必須指定參數`-UseBasicParsing`，否則在檢查詳細狀態時將收到以下錯誤：
...
要在規模集中部署自訂腳本擴展，請參閱添加[AzVms 擴展](https://docs.microsoft.com/powershell/module/az.compute/add-azvmssextension?view=azps-3.3.0)

## <a name="classic-vms"></a>傳統 VM

[!INCLUDE [classic-vm-deprecation](../../../includes/classic-vm-deprecation.md)]

要在經典 VM 上部署自訂腳本擴展，可以使用 Azure 門戶或經典 Azure PowerShell Cmdlet。

### <a name="azure-portal"></a>Azure 入口網站

導航到經典 VM 資源。 在 **"設置"** 下選擇**擴展**。

按一下 **+ 添加**，並在資源清單中選擇**自訂腳本擴展**。

在 **"安裝副檔名**"頁上，選擇本地 PowerShell 檔，並填寫任何參數，然後按一下"**確定**"。

### <a name="powershell"></a>PowerShell

使用[集-AzureVM 自訂腳本擴展](/powershell/module/servicemanagement/azure/set-azurevmcustomscriptextension)Cmdlet 可用於將自訂腳本擴展添加到現有虛擬機器。
...
擴展輸出記錄到目標虛擬機器上的以下資料夾下的檔。

```cmd
C:\WindowsAzure\Logs\Plugins\Microsoft.Compute.CustomScriptExtension
```

指定的檔將下載到目標虛擬機器上的以下資料夾中。
...
保存第一個 URI 段後，通過`fileUris`屬性清單下載的檔的路徑資訊。  如下表所示，下載的檔案會對應到下載的子目錄，以反映 `fileUris` 值結構。  
...
<sup>1</sup>絕對目錄路徑在 VM 的存留期內更改，但在自訂腳本擴展的單個執行中更改。

### <a name="support"></a>支援

如果本文中的任何一點都需要更多説明，則可以在[MSDN Azure 和堆疊溢位論壇](https://azure.microsoft.com/support/forums/)上聯繫 Azure 專家。 您也可以提出 Azure 支援事件。 轉到[Azure 支援網站](https://azure.microsoft.com/support/options/)並選擇"獲取支援"。 如需使用 Azure 支援的資訊，請參閱 [Microsoft Azure 支援常見問題集](https://azure.microsoft.com/support/faq/)。
