...
如果您想要的話，也可以使用 [Azure CLI](migration-classic-resource-manager-cli.md)來遷移資源。
...
如果您無法新增共同管理員，請洽詢訂用帳戶的服務管理員或共同管理員，以加入您自己的帳戶。
...
> [!NOTE]
> 使用虛擬機器的 VHD (作業系統和資料) 檔，藉由使用受控磁碟建立新的 Resource Manager 虛擬機器，以使用傳統部署模型來遷移建立的[單一虛擬](./windows/create-vm-specialized-portal.md)機。
<br>
