...
目前，Azure 雲端提供者不適用於 Azure Stack。 基於這個理由，您無法使用磁碟連結在 Azure Stack 中永續儲存。 相反地，您可以設定其他儲存選項，例如 NFS、iSCSI、GlusterFS 等。或者，您可以啟用 CN 並使用 GlusterFS 來保存永久儲存體。 如果已啟用 CN，將會部署額外三個節點，而且有額外的儲存體可供 GlusterFS 使用。
