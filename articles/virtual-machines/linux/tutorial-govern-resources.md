...
建立新群組或找到現有群組後，請使用 [az role assignment create](/cli/azure/policy/assignment#az_policy_assignment_create) 命令，將新的 Azure Active Directory 群組指派給資源群組的虛擬機器參與者角色。
...
[Azure 原則](../../governance/policy/overview.md)可協助您確認訂用帳戶中的所有資源均符合公司標準。 您的訂用帳戶已經有數個原則定義。 若要查看可用的原則定義，請使用 [az policy definition list](/cli/azure/policy/definition#az_policy_definition_list) 命令：
...
若要鎖定虛擬機器和網路安全性群組，請使用 [az lock create](/cli/azure/resource/lock#az_resource_lock_create) 命令：
...
若要將標籤套用至虛擬機器，請使用 [az resource tag](/cli/azure/resource#az_resource_list) 命令。 資源上的任何現有標記都不會保留。
...
您無法刪除已鎖定的網路安全性群組，除非移除鎖定。 若要移除鎖定，請擷取鎖定的識別碼，並將這些識別碼提供給 [az lock delete](/cli/azure/resource/lock#az_resource_lock_delete) 命令：
