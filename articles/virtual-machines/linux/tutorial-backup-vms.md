...
根據預設，Azure 備份會為 Linux VM 建立檔案系統一致備份，但可以將它設定成建立[應用程式一致備份 (使用備份前指令碼和備份後指令碼架構)](../../backup/backup-azure-linux-app-consistent.md)。 Azure 備份服務擷取快照集之後，資料會傳輸至保存庫。 為了能更有效率，服務只會找出並傳輸自上次備份之後有變更的資料區塊。
