...
根據預設，Image Builder 不會變更映像的大小，其會使用來源映像的大小。 您 **只能** 將 OS 磁片的大小增加 (Win 和 Linux) ，這是選擇性的，值為0表示保留與來源映射相同的大小。 您無法將 OS 磁片大小減少為小於來源映射的大小。
...
#### <a name="support-for-market-place-plan-information"></a>支援市場方案資訊
您也可以指定方案資訊，例如：
```json
    "source": {
        "type": "PlatformImage",
...
> [!NOTE]
> 來源受控映射必須是支援的 OS，且映射必須與您的 Azure 映射建立器範本相同。 
...
> [!NOTE]
> 來源受控映射必須是受支援的 OS，且映射必須與您的 Azure 映射產生器範本相同區域，如果不是，請將映射版本複寫至映射產生器範本區域。
...
> [!NOTE]
> 您可以在影像範本定義中查看內嵌命令，並在協助支援案例時 Microsoft 支援服務。 如果您有機密資訊，則應該將它移至 Azure 儲存體中的腳本，存取需要驗證。
...
> [!NOTE]
> 內嵌命令會儲存為映射範本定義的一部分，您可以在傾印映射定義時看到這些專案，而且在針對疑難排解用途的支援案例中，Microsoft 支援服務也可以看到這些專案。 如果您有敏感性的命令或值，強烈建議將這些命令移至腳本，並使用使用者身分識別來驗證 Azure 儲存體。
...
> [!NOTE]
> 如果有任何未處理的 Windows 重新開機，或仍在執行中的應用程式安裝，Windows Update 自訂者可能會失敗，通常您可能會在自訂 .log 中看到此錯誤 `System.Runtime.InteropServices.COMException (0x80240016): Exception from HRESULT: 0x80240016` 。 強烈建議您考慮在 Windows 重新開機中新增，以及/或讓應用程式有足夠的時間來完成其安裝，請使用 https://docs.microsoft.com/powershell/module/microsoft.powershell.utility/start-sleep?view=powershell-7) 內嵌命令或腳本中的 [睡眠] 或 [等候命令] (，然後再執行 Windows Update。
...
您可以在相同的設定中，將映射散發至這兩個目標型別。

> [!NOTE]
> 預設的 AIB sysprep 命令不包含 "/mode： vm"，不過當建立將安裝 HyperV 角色的映射時，這可能是必要的。 如果您需要新增此命令引數，您必須覆寫 sysprep 命令。
...
Distribute 屬性：
- **type** – managedImage 
- **imageId** –目的地影像的資源識別碼，格式應為：/Subscriptions/ \<subscriptionId> /resourceGroups/ \<destinationResourceGroupName> /providers/Microsoft.Compute/images/\<imageName>
- **location**- 受控映像的位置。  
...
- **type** - sharedImage  
- **galleryImageId** –共用映射庫的識別碼，這可以用兩種格式來指定：
    * 自動版本控制：影像產生器會為您產生單純版本號碼，當您想要保留相同範本的映射時，這會很有用：格式為： `/subscriptions/<subscriptionId>/resourceGroups/<resourceGroupName>/providers/Microsoft.Compute/galleries/<sharedImageGalleryName>/images/<imageGalleryName>` 。
    * 明確的版本控制-您可以傳遞您希望映射產生器使用的版本號碼。 格式為： `/subscriptions/<subscriptionID>/resourceGroups/<rgName>/providers/Microsoft.Compute/galleries/<sharedImageGalName>/images/<imageDefName>/versions/<version e.g. 1.1.1>`

- **runOutputName**– 用於識別散發的唯一名稱。  
- **artifactTags** - 選擇性使用者指定的索引鍵/值組標記。
- **replicationRegions** - 用於複寫的區域陣列。 其中一個區域必須是映像庫部署所在的區域。 新增區域會增加組建時間，因為組建不會在複寫完成之前完成。
- **excludeFromLatest** (選擇性) 這可讓您將所建立的映射版本標示為 SIG 定義中的最新版本，預設值為 ' false '。
- **>storageaccounttype** (選擇性) AIB 支援為要建立的映射版本指定這些類型的儲存體：
    * "Standard_LRS"
    * "Standard_ZRS"


> [!NOTE]
> 如果影像範本和參考的 `image definition` 位置不在相同位置，您會看到建立映射的額外時間。 影像產生器目前沒有 `location` 映射版本資源的參數，我們會從其父系取得參數 `image definition` 。 例如，如果映射定義在 westus 中，而您想要將映射版本複寫到 eastus，則會將 blob 複製到 westus，從這個版本會建立 westus 中的映射版本資源，然後再複寫到 eastus。 若要避免額外的複寫時間，請確定 `image definition` 和影像範本位於相同的位置。
...
若要啟動組建，您必須在映射範本資源上叫用 ' Run '，命令範例 `run` 如下：
