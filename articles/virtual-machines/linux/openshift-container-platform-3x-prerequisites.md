---
title: 在 Azure 先決條件中打開Shift容器平臺 3.11
...
由於所有虛擬機器 (VM) 是透過 Azure Resource Manager 範本部署的，因此會使用相同的公開金鑰存取所有 VM。 相應的私密金鑰必須位於執行所有行動手冊的 VM 上。 要安全地執行此操作，使用 Azure 金鑰保存庫將私密金鑰傳遞到 VM 中。

如果容器有永續性儲存體的需求，則需要永久性磁碟區。 OpenShift 支援持久卷的 Azure 虛擬硬碟 （VHD），但必須先將 Azure 配置為雲提供程式。

在此模型中，OpenShift 將會：

- 在 Azure 存儲帳戶或託管磁片中創建 VHD 物件。
...
使用 [az group create](/cli/azure/group) 命令來建立資源群組。 Azure 資源群組是在其中部署與管理 Azure 資源的邏輯容器。 應使用專用資源組來承載金鑰保存庫。 此群組與 OpenShift 叢集資源部署所在的資源群組分開。

下面的示例在*東部*位置創建名為*keyvaultrg*的資源組：
...
使用 [az keyvault create](/cli/azure/keyvault) 命令來建立要儲存叢集之 SSH 金鑰的金鑰保存庫。 金鑰保存庫名稱必須全域唯一，並且必須為範本部署啟用，否則部署將失敗，出現"KeyVault 參數引用秘密檢索失敗"錯誤。
...
下面的示例創建一個服務主體，並將其參與者許可權分配給名為*openshiftrg*的資源組。

首先，創建名為*openshiftrg*的資源組 ：
...
## <a name="prerequisites-applicable-only-to-resource-manager-template"></a>僅適用于資源管理器範本的先決條件

需要為 SSH 私**鑰 （sshPrivateKey）、** Azure AD 用戶端金鑰 （**aadClientSecret）、** OpenShift 管理員密碼 （**開放移位密碼**） 和紅帽訂閱管理器密碼或啟用金鑰 **（rhsmPasswordOr 啟用金鑰**） 創建秘密。  此外，如果使用自訂 SSL 憑證，則需要創建另外六個機密 -**路由快取檔案**、**路由檔**、**路由金鑰檔**、**母版檔**、**母版檔**和**主金鑰檔**。  這些參數將更詳細地解釋。

範本引用特定的機密名稱，因此**必須**使用上面列出的粗體名稱（區分大小寫）。

### <a name="custom-certificates"></a>自訂證書

預設情況下，範本將使用 OpenShift Web 主控台和路由域的自簽章憑證部署 OpenShift 群集。 如果要使用自訂 SSL 憑證，可以將"路由證書類型"設置為"自訂"，將"主證書類型"設置為"自訂"。  證書需要以 .pem 格式的 CA、Cert 和 Key 檔。  可以將自訂證書用於其中一個，但不能對另一個證書使用。

您需要將這些檔存儲在金鑰保存庫機密中。  使用與私密金鑰相同的金鑰保存庫。  範本無需為機密名稱增加 6 個輸入，而是硬編碼，可為每個 SSL 憑證檔使用特定機密名稱。  使用下表中的資訊存儲證書資料。
