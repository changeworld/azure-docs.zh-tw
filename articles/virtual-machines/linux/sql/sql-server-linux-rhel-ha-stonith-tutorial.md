...
使用下列命令在區域中建立資源群組 `<resourceGroupName>`。 請將 `<resourceGroupName>` 取代為您選擇的名稱。 在本教學課程中，我們將使用 `East US 2`。 如需詳細資訊，請參閱下列[快速入門](../quick-create-cli.md)。
...
1. 使用下列命令，在可用的群組 `ag1` 中建立資源 `ag_cluster`。
