...
長期以來，企業一直使用傳統的關係資料庫管理系統（RDBMS）平臺來滿足其線上交易處理（OLTP）需求。 如今，有許多企業都已將其大型主機型資料庫環境移轉到 Azure 做為延伸處理能力、降低成本及維持穩定之作業成本結構的方式。 移轉通常是將傳統平台現代化的第一個步驟。 

最近，一個企業客戶將其在 z/OS 上運行的 IBM DB2 環境重新託管到 Azure 上的 IBM DB2 純標。 Db2 pureScale 資料庫群集解決方案在 Linux 作業系統上提供高可用性和可擴充性。 在安裝 Db2 pureScale 之前，客戶在 Azure 上的一個大型擴展系統中作為獨立擴展實例成功運行 Db2。 

Linux 上的 IBM DB2 pureScale 雖然與原始環境不同，但針對在大型主機上之 Parallel Sysplex 設定中執行的 z/OS 提供與 IBM DB2 類似的高可用性與延展性。 在這種情況下，群集通過 iSCSI 連接到共用存儲群集。 我們使用 GlusterFS 檔案系統，這是一個針對雲存儲專門優化的免費、可擴展的開源分散式檔案系統。 但是，IBM 不再支援此解決方案。 要維護 IBM 的支援，您需要使用支援的 iSCSI 相容檔案系統。 微軟提供存儲空間直接 （S2D） 作為一個選項
...
![顯示存儲和網路的 Azure 虛擬機器上的 DB2 純標量](media/db2-purescale-on-azure/pureScaleArchitecture.png "顯示存儲和網路的 Azure 虛擬機器上的 DB2 純標量")


此圖說明 DB2 pureScale 叢集所需的邏輯層。 這些包括適用於用戶端、適用於管理、適用於快取、適用於資料庫引擎與適用於共用儲存體的虛擬機器。 

除了資料庫引擎節點之外，此圖也包括兩個用於叢集快取工具 (CF) 的節點。 資料庫引擎本身至少使用兩個節點。 屬於 pureScale 叢集的 DB2 伺服器稱為成員。 

群集通過 iSCSI 連接到三節點共用存儲群集，以提供橫向擴展存儲和高可用性。 DB2 pureScale 是安裝在執行 Linux 的 Azure 虛擬機器上。
...
-   高效能共用儲存體 (在圖中以 P30 大小顯示)。 每個節點都使用此存儲。
...
-   使用運行 Linux 的標準\_DS4\_v2 虛擬機器的共用存儲群集。

-   管理跳轉盒是運行\_Linux 的標準\_DS2 v2 虛擬機器。  另一種選擇是 Azure Bastion，該服務為虛擬網路中的所有 VM 提供安全的 RDP/SSH 體驗。

-   用戶端是執行 v 的標準 \_DS3\_v2 虛擬機器 (用於測試)。

-   *選擇項*。 見證伺服器。 這僅適用于某些早期版本的 Db2 pureScale。 此示例使用運行 Linux\_的標準\_DS3 v2 虛擬機器（用於 DB2 純尺規）。
...
DB2 pureScale 使用共用所有項目架構，其中資料可供所有叢集節點存取。 高級存儲必須在多個實例之間共用，無論是按需共用還是在專用實例上。

大型的 DB2 pureScale 叢集會需要 200 TB 以上的進階共用儲存體 (具有 100,000 的 IOPS)。 DB2 pureScale 支援您可在 Azure 上使用的 iSCSI 區塊介面。 iSCSI 介面需要一個共用存儲群集，您可以使用 S2D 或其他工具實現該群集。 此類型的解決方案會在 Azure 中建立虛擬儲存區域網路 (vSAN) 裝置。 DB2 pureScale 會使用 vSAN 來安裝用來在虛擬機器之間共用資料的叢集檔案系統。
