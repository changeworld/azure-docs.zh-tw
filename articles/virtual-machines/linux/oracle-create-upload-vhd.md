...
本文假設您已將 Oracle Linux 作業系統安裝到虛擬硬碟。 有多個工具可用來建立 .vhd 檔案，例如，像是 Hyper-V 的虛擬化解決方案。 如需指示，請參閱[安裝 Hyper-v 角色和設定虛擬機器](https://technet.microsoft.com/library/hh846766.aspx)。
...
* 確定已啟用 `Addons` 儲存機制。 編輯`/etc/yum.repos.d/public-yum-ol6.repo`檔案（Oracle Linux 6）或`/etc/yum.repos.d/public-yum-ol7.repo`（Oracle Linux 7），並將此檔案中`enabled=0` `enabled=1` **[ol6_addons]** 或 **[ol7_addons]** 底下的一行變更為。
...
3. 執行下列命令以解除安裝 NetworkManager：
    ...
    **注意：** 如果尚未安裝封裝，此命令將會失敗，並出現錯誤訊息。 這是預期的結果。
...
14. 執行下列命令，以取消佈建虛擬機器，並準備將它佈建於 Azure 上：
        ...
        # export HISTSIZE=0
        # logout
15. 按一下 [動作]-在 [Hyper-v 管理員] 中 **> 關閉**]。 您現在可以將 Linux VHD 上傳至 Azure。
