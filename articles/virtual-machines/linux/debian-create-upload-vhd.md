...
description: 瞭解如何為 Azure 中的 VM 部署創建 Debian VHD 映射。
author: gbowerman
...
* Azure 不支援較新的 VHDX 格式。 您可以使用 Hyper-V 管理器或**轉換 vhd** Cmdlet 將磁片轉換為 VHD 格式。
...
有可用於生成 Azure 的 Debian VHD 的工具，例如來自[Credativ](https://www.credativ.com/)[的 azure 管理](https://github.com/credativ/azure-manage)腳本。 若不想從頭建立映像，建議採用此方法。 例如，若要建立 Debian 8 VHD，請執行下列命令以下載 `azure-manage` 公用程式 (和相依性)，並執行 `azure_build_image` 指令碼︰
...
10. 按一下 **"操作**->在超 V 管理器中關閉。 您現在可以將 Linux VHD 上傳至 Azure。
