---
title: 使用雲 init 將使用者添加到 Azure 上的 Linux VM
...
本文會示範如何在 Azure 佈建期間，使用 [cloud-init](https://cloudinit.readthedocs.io) 在虛擬機器 (VM) 上或虛擬機器擴展集 (VMSS) 上新增使用者。 一旦 Azure 佈建資源，此 cloud-init 指令碼就會在初次開機時執行。 有關雲 init 如何在 Azure 中本機工作和支援的 Linux 發行版本的詳細資訊，請參閱[雲 init 概述](using-cloud-init.md)。
