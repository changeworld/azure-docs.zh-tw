...
| 產品/服務 | 發行項 |
| --------------- | ------- |
| **電腦信任邊界** | <ul><li>[確保配置了適當的 ACL 以限制對設備上資料的未授權訪問](#acl-restricted-access)</li><li>[確保敏感使用者特定的應用程式內容存儲在使用者設定檔目錄中](#sensitive-directory)</li><li>[確保已部署的應用程式是以最低權限執行](#deployed-privileges)</li></ul> |
| **Web 應用程式** | <ul><li>[在處理業務邏輯流時強制執行順序步進順序](#sequential-logic)</li><li>[實作速率限制機制以防止列舉](#rate-enumeration)</li><li>[確保適當的授權到位，並遵循最低特權原則](#principle-least-privilege)</li><li>[業務邏輯和資源訪問授權決策不應基於傳入請求參數](#logic-request-parameters)</li><li>[確保無法透過強迫瀏覽來列舉或存取內容和資源](#enumerable-browsing)</li></ul> |
| **資料庫** | <ul><li>[確保使用最低權限的帳戶連線到資料庫伺服器](#privileged-server)</li><li>[實現行級安全 RLS，以防止租戶訪問彼此的資料](#rls-tenants)</li><li>[系統管理員角色只能具備有效的必要使用者](#sysadmin-users)</li></ul> |
| **IoT 雲端閘道** | <ul><li>[使用最低權限的權杖連線到雲端閘道](#cloud-least-privileged)</li></ul> |
| **Azure 事件中心** | <ul><li>[使用僅限傳送權限 SAS 金鑰來產生裝置權杖](#sendonly-sas)</li><li>[請勿使用可供直接存取事件中樞的存取權杖](#access-tokens-hub)</li><li>[使用具有所需最低權限的 SAS 金鑰來連線到事件中樞](#sas-minimum-permissions)</li></ul> |
...
| **Service Fabric 信任邊界** | <ul><li>[使用 RBAC 限制用戶端對群集操作的訪問](#cluster-rbac)</li></ul> |
| **Dynamics CRM** | <ul><li>[執行安全建模，並在需要時使用現場級別安全性](#modeling-field)</li></ul> |
| **Dynamics CRM 入口網站** | <ul><li>[執行入口網站帳戶的安全性模型化，並記住，入口網站的安全性模型與 CRM 的其餘部分不同](#portal-security)</li></ul> |
| **Azure 存儲** | <ul><li>[在 Azure 表格儲存體中授與某個實體範圍的精細權限](#permission-entities)</li><li>[使用 Azure Resource Manager 啟用 Azure 儲存體帳戶的角色型存取控制 (RBAC)](#rbac-azure-manager)</li></ul> |
| **行動用戶端** | <ul><li>[實作隱含的越獄或 Root 權限入侵偵測](#rooting-detection)</li></ul> |
| **WCF** | <ul><li>[WCF 中的弱類引用](#weak-class-wcf)</li><li>[WCF-實現授權控制](#wcf-authz)</li></ul> |
| **Web API** | <ul><li>[在ASP.NET Web API 中實施適當的授權機制](#authz-aspnet)</li></ul> |
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [SQL Database 權限階層](https://msdn.microsoft.com/library/ms191465)、[SQL Database 安全性實體](https://msdn.microsoft.com/library/ms190401) |
| **步驟** | 應該使用最低權限的帳戶連線到資料庫。 應用程式登入應受限於資料庫，應該只能執行所選的程序。 應用程式的登入應該沒有直接的資料表存取權。 |

## <a name="implement-row-level-security-rls-to-prevent-tenants-from-accessing-each-others-data"></a><a id="rls-tenants"></a>實現行級安全 RLS，以防止租戶訪問彼此的資料
...
| **引用**              | [SQL Server 資料列層級安全性 (RLS)](https://msdn.microsoft.com/library/azure/dn765131.aspx) |
...
| **引用**              | [Iot 集線器存取控制](https://azure.microsoft.com/documentation/articles/iot-hub-devguide/#Security) |
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [使用角色指派來管理 Azure 訂用帳戶資源的存取權](https://azure.microsoft.com/documentation/articles/role-based-access-control-configure/)  |
...
| **適用的技術** | 泛型 |
| **屬性**              | 環境 - Azure |
| **引用**              | [角色型存取控制 (適用於 Service Fabric 用戶端)](https://azure.microsoft.com/documentation/articles/service-fabric-cluster-security-roles/) |
...
| **適用的技術** | 泛型 |
| **屬性**              | StorageType - 資料表 |
| **引用**              | [如何使用 SAS 將存取權委派給 Azure 儲存體帳戶中的物件](https://azure.microsoft.com/documentation/articles/storage-security-guide/#_data-plane-security) |
| **步驟** | 在特定商務案例中，Azure 表格儲存體可能必須儲存能滿足不同合作夥伴需求的敏感性資料。 例如，與不同國家/地區相關的敏感性資料。 在這種情況下，可以通過指定分區和行鍵範圍來構造 SAS 簽名，以便使用者可以訪問特定于特定國家/地區的資料。| 
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [如何使用角色型存取控制 (RBAC) 保護儲存體帳戶](https://azure.microsoft.com/documentation/articles/storage-security-guide/#management-plane-security) |
...
| **屬性**              | N/A  |
| **引用**              | [MSDN](https://msdn.microsoft.com/library/ff648500.aspx)、[Fortify Kingdom](https://vulncat.fortify.com/en/detail?id=desc.config.dotnet.wcf_misconfiguration_weak_class_reference) |
...
| **引用**              | [ASP.NET Web API 中的驗證與授權](https://www.asp.net/web-api/overview/security/authentication-and-authorization-in-aspnet-web-api) |
