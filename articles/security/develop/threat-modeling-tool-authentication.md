...
| 產品/服務 | 發行項 |
| --------------- | ------- |
| **Web 應用程式**    | <ul><li>[考慮使用標準身份驗證機制對 Web 應用程式進行身份驗證](#standard-authn-web-app)</li><li>[應用程式必須安全地處理失敗的驗證案例](#handle-failed-authn)</li><li>[啟用升步或自我調整身份驗證](#step-up-adaptive-authn)</li><li>[確保已適當地鎖定系統管理介面](#admin-interface-lockdown)</li><li>[安全地實現忘記的密碼功能](#forgot-pword-fxn)</li><li>[確保實施密碼和帳戶原則](#pword-account-policy)</li><li>[實現控制項以防止使用者名枚舉](#controls-username-enum)</li></ul> |
| **資料庫** | <ul><li>[如果可能，請使用 Windows 身份驗證連接到 SQL 伺服器](#win-authn-sql)</li><li>[如果可能，請使用 Azure 活動目錄身份驗證連接到 SQL 資料庫](#aad-authn-sql)</li><li>[使用 SQL 驗證模式時，確保在 SQL Server 上強制執行帳戶和密碼原則](#authn-account-pword)</li><li>[請勿在包含的資料庫中使用 SQL 身份驗證](#autn-contained-db)</li></ul> |
| **Azure 事件中心** | <ul><li>[使用採用 SaS 權杖的每一裝置驗證認證](#authn-sas-tokens)</li></ul> |
| **Azure 信任邊界** | <ul><li>[啟用 Azure 系統管理員適用的 Azure Multi-Factor Authentication](#multi-factor-azure-admin)</li></ul> |
| **Service Fabric 信任邊界** | <ul><li>[限制對服務交換矩陣群集的匿名存取](#anon-access-cluster)</li><li>[確保服務結構用戶端到節點證書不同于節點到節點證書](#fabric-cn-nn)</li><li>[使用 AAD 對用戶端進行身份驗證以服務結構群集](#aad-client-fabric)</li><li>[確保從經過核准的憑證授權單位 (CA) 取得 Service Fabric 憑證](#fabric-cert-ca)</li></ul> |
| **Identity Server** | <ul><li>[使用標識伺服器支援的標準身份驗證方案](#standard-authn-id)</li><li>[使用可擴展的替代方案覆蓋預設標識伺服器權杖緩存](#override-token)</li></ul> |
| **電腦信任邊界** | <ul><li>[確保已數位簽署所部署應用程式的二進位檔](#binaries-signed)</li></ul> |
| **WCF** | <ul><li>[在 WCF 中連接到 MSMQ 佇列時啟用身份驗證](#msmq-queues)</li><li>[WCF - 請勿將訊息 clientCredentialType 設定為 none](#message-none)</li><li>[WCF-不將傳輸用戶端憑據類型設置為無](#transport-none)</li></ul> |
| **Web API** | <ul><li>[確保使用標準身份驗證技術來保護 Web API](#authn-secure-api)</li></ul> |
| **Azure AD** | <ul><li>[使用 Azure Active Directory 所支援的標準驗證案例](#authn-aad)</li><li>[使用可擴展的替代方案覆蓋預設的 ADAL 權杖緩存](#adal-scalable)</li><li>[確保使用 TokenReplayCache 來防止重新執行 ADAL 驗證權杖](#tokenreplaycache-adal)</li><li>[使用 ADAL 庫管理從 OAuth2 用戶端到 AAD（或本地 AD）的權杖請求](#adal-oauth2)</li></ul> |
| **IoT 現場閘道** | <ul><li>[驗證連接到現場閘道的設備](#authn-devices-field)</li></ul> |
| **IoT 雲端閘道** | <ul><li>[確保會驗證連線到雲端閘道的裝置](#authn-devices-cloud)</li><li>[使用每設備身份驗證憑據](#authn-cred)</li></ul> |
| **Azure 存儲** | <ul><li>[確保只有必要的容器和 blob 會取得匿名讀取權限](#req-containers-anon)</li><li>[使用 SAS 或 SAP 對 Azure 儲存體中的物件授與有限的存取權](#limited-access-sas)</li></ul> |
...
| **屬性**              | N/A  |
| **引用**              | N/A  |
| 詳細資料 | <p>必須實作符合組織原則和最佳作法的密碼和帳戶原則。</p><p>若要防止暴力密碼破解及以字典為基礎的猜測︰必須實作強式密碼原則，以確保使用者建立複雜的密碼 (例如，長度最少 12 個字元、英數字元和特殊字元)。</p><p>帳戶鎖定原則可能會以下列方式實作︰</p><ul><li>**軟式鎖定︰** 這是保護使用者以對抗暴力密碼破解攻擊的理想選項。 例如，每當使用者輸入錯誤密碼三次時，應用程式可以鎖定帳戶一分鐘，以減慢暴力強制其密碼使攻擊者無法繼續操作的過程。 如果要對此示例實施硬鎖定對策，則可以通過永久鎖定帳戶來實現"DoS"。 或者，應用程式可產生 OTP (一次性密碼) 並以頻外方式 (透過電子郵件、簡訊等) 將它傳送給使用者。 另一種方法則是在達到嘗試失敗次數閾值之後，實作 CAPTCHA。</li><li>**硬鎖定：** 每當檢測到攻擊應用程式的使用者時，都應應用這種類型的鎖定，並通過永久鎖定其帳戶來反擊它們，直到回應團隊有時間進行取證。 此過程後，您可以決定將使用者重新提供帳戶，或針對他們採取進一步的法律行動。 這種方法可防止攻擊者進一步入侵您的應用程式和基礎結構。</li></ul><p>若要防禦對於預設和可預測帳戶的攻擊，請確認所有金鑰和密碼均可取代，而且會在安裝階段後產生或取代。</p><p>如果應用程式必須自動產生密碼，請確保所產生的密碼是隨機的且具有高熵。</p>|
...
| **適用的技術** | OnPrem |
| **屬性**              | SQL 版本 - 全部 |
| **引用**              | [SQL Server - 選擇驗證模式](https://msdn.microsoft.com/library/ms144284.aspx) |
...
| **適用的技術** | SQL Azure |
| **屬性**              | SQL 版本 - V12 |
| **引用**              | [使用 Azure 活動目錄身份驗證連接到 SQL 資料庫](https://azure.microsoft.com/documentation/articles/sql-database-aad-authentication/) |
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [SQL Server 密碼原則](https://technet.microsoft.com/library/ms161959(v=sql.110).aspx) |
...
| **引用**              | [自主資料庫的安全性最佳做法](https://msdn.microsoft.com/library/ff929055.aspx) |
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [事件中樞驗證和安全性模型概觀](https://azure.microsoft.com/documentation/articles/event-hubs-authentication-and-security-model-overview/) |
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [什麼是 Azure Multi-Factor Authentication？](https://azure.microsoft.com/documentation/articles/multi-factor-authentication/) |
...
| **適用的技術** | 泛型 |
| **屬性**              | 環境 - Azure  |
| **引用**              | [Service Fabric 叢集安全性案例](https://azure.microsoft.com/documentation/articles/service-fabric-cluster-security) |
...
| **適用的技術** | 泛型 |
| **屬性**              | 環境 - Azure、環境 - 獨立 |
| **引用**              | [Service Fabric 用戶端對節點憑證安全性](https://azure.microsoft.com/documentation/articles/service-fabric-cluster-security/#_client-to-node-certificate-security)、[使用用戶端憑證連線到安全的叢集](https://azure.microsoft.com/documentation/articles/service-fabric-connect-to-secure-cluster/) |
...
| **適用的技術** | 泛型 |
| **屬性**              | 環境 - Azure |
| **引用**              | [叢集安全性案例 - 安全性建議](https://azure.microsoft.com/documentation/articles/service-fabric-cluster-security/#security-recommendations) |
...
| **適用的技術** | 泛型 |
| **屬性**              | 環境 - Azure |
| **引用**              | [X.509 憑證和 Service Fabric](https://azure.microsoft.com/documentation/articles/service-fabric-cluster-security/#x509-certificates-and-service-fabric) |
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [IdentityServer3 - 概觀](https://identityserver.github.io/Documentation/docsv2/overview/bigPicture.html) |
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [Identity Server 部署 - 快取](https://identityserver.github.io/Documentation/docsv2/advanced/deployment.html) |
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [ASP.NET Web API 中的驗證和授權](https://www.asp.net/web-api/overview/security/authentication-and-authorization-in-aspnet-web-api)、[外部驗證服務與 ASP.NET Web API (C#)](https://www.asp.net/web-api/overview/security/external-authentication-services) |
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [Modern Authentication with Azure Active Directory for Web Applications](https://blogs.msdn.microsoft.com/microsoft_press/2016/01/04/new-book-modern-authentication-with-azure-active-directory-for-web-applications/)、[使用 Redis 作為 ADAL 權杖快取](https://blogs.msdn.microsoft.com/mrochon/2016/09/19/using-redis-as-adal-token-cache/)  |
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [Modern Authentication with Azure Active Directory for Web Applications](https://blogs.msdn.microsoft.com/microsoft_press/2016/01/04/new-book-modern-authentication-with-azure-active-directory-for-web-applications/) |
...
| **引用**              | N/A，Azure [IoT 中心與 .NET](https://azure.microsoft.com/documentation/articles/iot-hub-csharp-csharp-getstarted/)，[開始使用 IoT 中心和節點 JS](https://azure.microsoft.com/documentation/articles/iot-hub-node-node-getstarted)，[使用 SAS 和證書保護 IoT，Git](https://azure.microsoft.com/documentation/articles/iot-hub-sas-tokens/)[存儲庫](https://github.com/Azure/azure-iot-sdks/tree/master/node) |
...
| **引用**              | [Azure IoT 中樞安全性權杖](https://azure.microsoft.com/documentation/articles/iot-hub-sas-tokens/) |
...
| **適用的技術** | 泛型 |
| **屬性**              | StorageType - Blob |
| **引用**              | [管理對容器與 Blob 的匿名讀取權限](https://azure.microsoft.com/documentation/articles/storage-manage-access-to-resources/)、[共用存取簽章，第 1 部分：了解 SAS 模型](https://azure.microsoft.com/documentation/articles/storage-dotnet-shared-access-signature-part-1/) |
