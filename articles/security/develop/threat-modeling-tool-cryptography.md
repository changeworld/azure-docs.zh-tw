...
| 產品/服務 | 發行項 |
| --------------- | ------- |
| **Web 應用程式** | <ul><li>[僅使用經批准的對稱塊密碼和鍵長度](#cipher-length)</li><li>[針對對稱編碼器使用核准的區塊編碼器模式和初始化向量](#vector-ciphers)</li><li>[使用核准的非對稱演算法、金鑰長度和填補](#padding)</li><li>[使用核准的亂數產生器](#numgen)</li><li>[不要使用對稱流密碼](#stream-ciphers)</li><li>[使用經批准的 MAC/HMAC/鍵控雜湊演算法](#mac-hash)</li><li>[僅使用已批准的加密雜湊函數](#hash-functions)</li></ul> |
| **資料庫** | <ul><li>[使用增強式加密演算法加密資料庫中的資料](#strong-db)</li><li>[SSIS 套裝軟體應加密和數位簽章](#ssis-signed)</li><li>[將數位簽章添加到關鍵資料庫安全功能](#securables-db)</li><li>[使用 SQL 伺服器 EKM 保護加密金鑰](#ekm-keys)</li><li>[如果不應向資料庫引擎顯示加密金鑰，請使用"始終加密"功能](#keys-engine)</li></ul> |
| **IoT 裝置** | <ul><li>[在 IoT 設備上安全地存儲加密金鑰](#keys-iot)</li></ul> | 
| **IoT 雲端閘道** | <ul><li>[生成足夠長度的隨機對稱金鑰，以便對 IoT 中心進行身份驗證](#random-hub)</li></ul> | 
| **Dynamics CRM 行動用戶端** | <ul><li>[確保裝置管理策略到位，需要使用 PIN 並允許遠端擦除](#pin-remote)</li></ul> | 
| **Dynamics CRM Outlook 用戶端** | <ul><li>[確保裝置管理策略到位，需要 PIN/密碼/自動鎖定並加密所有資料（例如 BitLocker）](#bitlocker)</li></ul> | 
| **Identity Server** | <ul><li>[使用標識伺服器時，確保滾動簽名金鑰](#rolled-server)</li><li>[確保在身份伺服器中使用加密強用戶端 ID、用戶端金鑰](#client-server)</li></ul> | 
...
| **屬性**              | N/A  |
| **引用**              | N/A  |
| **步驟** | <p>產品必須使用 SHA-2 系列的雜湊演算法 (SHA256、SHA384 及 SHA512)。 如果需要較短的雜湊，例如 128 位元的輸出長度，以配合設計時預設使用較短 MD5 雜湊的資料結構，產品小組可截斷其中一個 SHA2 雜湊 (通常是 SHA256)。 請注意，SHA384 是 SHA512 的截斷版本。 不允許基於安全性考量而將密碼編譯雜湊截斷為少於 128 位元。 新的程式碼不得使用 MD2、MD4、MD5、SHA-0、SHA-1 或 RIPEMD 雜湊演算法。 這些演算法在計算時可能會發生雜湊衝突，而結果便是打斷演算法。</p><p>為了實現受控密碼編譯靈活性所允許的 .NET 雜湊演算法 (依偏好順序)：</p><ul><li>SHA512Cng (符合 FIPS 規範)</li><li>SHA384Cng (符合 FIPS 規範)</li><li>SHA256Cng (符合 FIPS 規範)</li><li>SHA512 託管（不符合 FIPS）（在調用 HashAlgorithm 時使用 SHA512 作為演算法名稱。</li><li>SHA384 管理（不符合 FIPS）（在調用 HashAlgorithm 時使用 SHA384 作為演算法名稱。</li><li>SHA256 託管（不符合 FIPS）（在調用 HashAlgorithm 時使用 SHA256 作為演算法名稱。</li><li>SHA512CryptoServiceProvider (符合 FIPS 規範)</li><li>SHA256CryptoServiceProvider (符合 FIPS 規範)</li><li>SHA384CryptoServiceProvider (符合 FIPS 規範)</li></ul>| 
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [選擇加密演算法](https://technet.microsoft.com/library/ms345262(v=sql.130).aspx) |
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [使用數位簽章識別套件的來源](https://msdn.microsoft.com/library/ms141174.aspx)、[威脅和弱點風險降低 (整合服務)](https://msdn.microsoft.com/library/bb522559.aspx) |
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [ADD SIGNATURE (Transact-SQL)](https://msdn.microsoft.com/library/ms181700) |
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [SQL Server 可延伸金鑰管理 (EKM)](https://msdn.microsoft.com/library/bb895340)、[使用 Azure Key Vault 的可延伸金鑰管理 (SQL Server)](https://msdn.microsoft.com/library/dn198405) |
...
| **引用**              | [始終加密（資料庫引擎）](https://msdn.microsoft.com/library/mt163865) |
...
| **引用**              | [Windows IoT 核心版上的 TPM](https://developer.microsoft.com/windows/iot/docs/tpm)、[設定 Windows IoT 核心版上的 TPM](https://docs.microsoft.com/windows/iot-core/secure-your-device/setuptpm)、[Azure IoT 裝置 SDK TPM](https://github.com/Azure/azure-iot-hub-vs-cs/wiki/Device-Provisioning-with-TPM) |
...
## <a name="ensure-a-device-management-policy-is-in-place-that-requires-a-pinpasswordauto-lock-and-encrypts-all-data-eg-bitlocker"></a><a id="bitlocker"></a>確保裝置管理策略到位，需要 PIN/密碼/自動鎖定並加密所有資料（例如 BitLocker）
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [標識伺服器 - 金鑰、簽名和加密](https://identityserver.github.io/Documentation/docsv2/configuration/crypto.html) |
| **步驟** | 確定在使用 Identity Server 時會變換簽署金鑰。 [參考] 區段中的連結會說明該如何進行規劃，而不會造成仰賴 Identity Server 的應用程式發生中斷。 |

## <a name="ensure-that-cryptographically-strong-client-id-client-secret-are-used-in-identity-server"></a><a id="client-server"></a>確保在身份伺服器中使用加密強用戶端 ID、用戶端金鑰
