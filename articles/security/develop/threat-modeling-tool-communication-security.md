...
| **身分識別伺服器** | <ul><li>[確定對身分識別伺服器的所有流量都是透過 HTTPS 連線](#identity-https)</li></ul> |
| **Web 應用程式** | <ul><li>[驗證用來驗證 SSL、TLS 及 DTLS 連線的 X.509 憑證](#x509-ssltls)</li><li>[在 Azure App Service 中設定自訂網域的 TLS/SSL 憑證](#ssl-appservice)</li><li>[強制所有前往 Azure App Service 的流量透過 HTTPS 連線來進行](#appservice-https)</li><li>[啟用 HTTP Strict Transport Security (HSTS)](#http-hsts)</li></ul> |
| **Database** | <ul><li>[確認 SQL server 連線加密和憑證驗證](#sqlserver-validation)</li><li>[強制加密與 SQL Server 的通訊](#encrypted-sqlserver)</li></ul> |
| **Azure 儲存體** | <ul><li>[確定對 Azure 儲存體的通訊是透過 HTTPS](#comm-storage)</li><li>[如果無法啟用 HTTPS，請在下載 blob 之後驗證 MD5 雜湊](#md5-https)</li><li>[使用 SMB 3.0 相容用戶端來確保傳輸中的資料加密至 Azure 檔案共用](#smb-shares)</li></ul> |
...
| **Azure Cache for Redis** | <ul><li>[確定與 Azure Cache for Redis 的通訊是透過 TLS 進行](#redis-ssl)</li></ul> |
...
## <a name="use-data-management-gateway-while-connecting-on-premises-sql-server-to-azure-data-factory"></a><a id="sqlserver-factory"></a>將內部部署 SQL Server 連接至 Azure Data Factory 時使用資料管理閘道
...
## <a name="configure-tlsssl-certificate-for-custom-domain-in-azure-app-service"></a><a id="ssl-appservice"></a>在 Azure App Service 中設定自訂網域的 TLS/SSL 憑證
...
| **步驟** | 根據預設，Azure 已使用 *.azurewebsites.net 網域的萬用字元憑證來啟用每一個應用程式的 HTTPS。 但是，就像所有萬用字元網域一樣，這並不如使用自訂網域搭配自己的憑證那麼安全。[參閱](https://casecurity.org/2014/02/26/pros-and-cons-of-single-domain-multi-domain-and-wildcard-certificates/)。 建議為已部署的應用程式將透過其存取的自訂網域啟用 TLS|
...
| **步驟** | 啟用 TLS 加密可增加在 SQL Server 和應用程式實例之間傳輸的資料安全性。 |
...
| **步驟** | <p>憑證釘選可防止攔截式 (MITM) 攻擊。 釘選程序會將主機與其預期的 X509 憑證或公開金鑰相關聯。 在主機已知或已看到憑證或公開金鑰後，憑證或公開金鑰便已關聯或「釘選」到主機。 </p><p>因此，當敵人嘗試進行 TLS MITM 攻擊時，在 TLS 交握期間，來自攻擊者伺服器的金鑰會與釘選的憑證金鑰不同，而要求將會被捨棄，因而無法藉由執行 ServicePointManager 的`ServerCertificateValidationCallback`委派來進行 MITM 憑證釘選。</p>|
...
| **步驟** | <ul><li>**說明︰** 保護層級設定為「無」時，它會停用訊息保護。 機密性和完整性要有適當的設定層級才能實現。</li><li>**相關**<ul><li>當 `Mode=None` 時 - 停用訊息保護</li><li>當 `Mode=Sign` 時 - 會簽署但不加密訊息；若您認為資料完整性很重要，便應使用</li><li>當 `Mode=EncryptAndSign` 時 - 簽署並加密訊息</li></ul></li></ul><p>若您只需要驗證資訊的完整性而不必擔心機密性，請考慮關閉加密功能，只要簽署訊息即可。 對於需要驗證原始傳送者，但不會傳輸敏感性資料的作業或服務合約，這會非常實用。 減少保護層級時，請小心訊息不包含任何個人資料。</p>|
...
```
將此篩選準則新增至任何需要 TLS 的 Web API 動作： 
```csharp
...
## <a name="ensure-that-communication-to-azure-cache-for-redis-is-over-tls"></a><a id="redis-ssl"></a>確定與 Azure Cache for Redis 的通訊是透過 TLS 進行
...
| **步驟** | Redis 伺服器不支援現成的 TLS，但 Azure Cache for Redis 會執行。 如果您要連線到 Azure Cache for Redis，而您的用戶端支援 TLS （例如 Stackexchange.redis. Redis），則您應該使用 TLS。 針對新的 Azure Cache for Redis 實例，預設會停用非 TLS 埠。 除非有 redis 用戶端的 TLS 支援相依性，否則請確定安全的預設值不會變更。 |
