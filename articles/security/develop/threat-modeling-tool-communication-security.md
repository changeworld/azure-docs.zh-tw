...
| **Azure Data Factory** | <ul><li>[將本地 SQL 伺服器連接到 Azure 資料工廠時使用資料管理閘道](#sqlserver-factory)</li></ul> |
| **身分識別伺服器** | <ul><li>[確保到識別伺服器的所有流量都透過 HTTPS 連線](#identity-https)</li></ul> |
...
| **Database** | <ul><li>[確保 SQL 伺服器連線加密與憑證驗證](#sqlserver-validation)</li><li>[強制加密與 SQL Server 的通訊](#encrypted-sqlserver)</li></ul> |
| **Azure 儲存體** | <ul><li>[確定對 Azure 儲存體的通訊是透過 HTTPS](#comm-storage)</li><li>[如果無法開啟 HTTPS,則在下載 blob 後驗證 MD5 哈希](#md5-https)</li><li>[使用 SMB 3.0 相容客戶端確保對 Azure 檔案分享的傳輸中資料加密](#smb-shares)</li></ul> |
| **行動用戶端** | <ul><li>[提供憑證固定](#cert-pinning)</li></ul> |
...
| **SDL 階段**               | 部署 |  
| **適用的技術** | 泛型 |
| **屬性**              | 連結服務型態 - Azure 和本地端 |
| **參考**              |[在本地端與 Azure 資料工廠之間移動資料](https://azure.microsoft.com/documentation/articles/data-factory-move-data-between-onprem-and-cloud/#create-gateway),[資料管理閘道](https://azure.microsoft.com/documentation/articles/data-factory-data-management-gateway/) |
...
| **步驟** | <ul><li>**說明︰** 保護層級設定為「無」時，它會停用訊息保護。 機密性和完整性要有適當的設定層級才能實現。</li><li>**建議:**<ul><li>當 `Mode=None` 時 - 停用訊息保護</li><li>當 `Mode=Sign` 時 - 會簽署但不加密訊息；若您認為資料完整性很重要，便應使用</li><li>當 `Mode=EncryptAndSign` 時 - 簽署並加密訊息</li></ul></li></ul><p>若您只需要驗證資訊的完整性而不必擔心機密性，請考慮關閉加密功能，只要簽署訊息即可。 對於需要驗證原始傳送者，但不會傳輸敏感性資料的作業或服務合約，這會非常實用。 降低保護級別時,請注意消息不包含任何個人數據。</p>|
