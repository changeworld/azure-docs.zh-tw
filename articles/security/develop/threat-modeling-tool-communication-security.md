...
ms.locfileid: "80294141"
---
# <a name="security-frame-communication-security--mitigations"></a>安全性架構︰通訊安全性 | 風險降低 
| 產品/服務 | 發行項 |
| --------------- | ------- |
| **Azure 事件中心** | <ul><li>[使用 SSL/TLS 保護與事件中樞的通訊](#comm-ssltls)</li></ul> |
| **Dynamics CRM** | <ul><li>[檢查服務帳戶權限，並確認自訂服務或 ASP.NET 網頁採用 CRM 的安全性](#priv-aspnet)</li></ul> |
| **Azure 資料工廠** | <ul><li>[將本地 SQL 伺服器連接到 Azure 資料工廠時使用資料管理閘道](#sqlserver-factory)</li></ul> |
| **Identity Server** | <ul><li>[確保到標識伺服器的所有流量都通過 HTTPS 連接](#identity-https)</li></ul> |
...
| **資料庫** | <ul><li>[確保 SQL 伺服器連接加密和證書驗證](#sqlserver-validation)</li><li>[強制加密與 SQL Server 的通訊](#encrypted-sqlserver)</li></ul> |
| **Azure 存儲** | <ul><li>[確定對 Azure 儲存體的通訊是透過 HTTPS](#comm-storage)</li><li>[如果無法啟用 HTTPS，則在下載 blob 後驗證 MD5 雜湊](#md5-https)</li><li>[使用 SMB 3.0 相容用戶端確保對 Azure 檔共用的傳輸中資料加密](#smb-shares)</li></ul> |
| **行動用戶端** | <ul><li>[實現證書固定](#cert-pinning)</li></ul> |
...
| **用於雷瑞斯的 Azure 緩存** | <ul><li>[確定對 Azure Cache for Redis 的通訊是透過 SSL](#redis-ssl)</li></ul> |
| **IoT 現場閘道** | <ul><li>[安全設備到現場閘道通信](#device-field)</li></ul> |
...
## <a name="use-data-management-gateway-while-connecting-on-premises-sql-server-to-azure-data-factory"></a><a id="sqlserver-factory"></a>將本地 SQL 伺服器連接到 Azure 資料工廠時使用資料管理閘道
...
| **適用的技術** | 泛型 |
| **屬性**              | 連結服務類型 - Azure 和本地 |
| **引用**              |[在本地和 Azure 資料工廠之間移動資料](https://azure.microsoft.com/documentation/articles/data-factory-move-data-between-onprem-and-cloud/#create-gateway)，[資料管理閘道](https://azure.microsoft.com/documentation/articles/data-factory-data-management-gateway/) |
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [IdentityServer3 - 金鑰、簽章和密碼編譯](https://identityserver.github.io/Documentation/docsv2/configuration/crypto.html)、[IdentityServer3 - 部署](https://identityserver.github.io/Documentation/docsv2/advanced/deployment.html) |
...
## <a name="configure-ssl-certificate-for-custom-domain-in-azure-app-service"></a><a id="ssl-appservice"></a>在 Azure App Service 中設定自訂網域的 SSL 憑證
...
| **引用**              | [針對 Azure App Service 中的 App 啟用 HTTPS](../../app-service/configure-ssl-bindings.md) |
...
| **適用的技術** | 泛型 |
| **屬性**              | EnvironmentType - Azure |
| **引用**              | [在 Azure App Service 上強制使用 HTTPS](../../app-service/configure-ssl-bindings.md#enforce-https) |
...
| **適用的技術** | SQL Azure  |
| **屬性**              | SQL 版本 - V12 |
| **引用**              | [為 SQL Database 撰寫安全連接字串的最佳作法](https://social.technet.microsoft.com/wiki/contents/articles/2951.windows-azure-sql-database-connection-security.aspx#best) |
...
| **引用**              | [啟用資料庫引擎的加密連接](https://msdn.microsoft.com/library/ms191192)  |
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [Azure 儲存體傳輸層級加密 – 使用 HTTPS](https://azure.microsoft.com/documentation/articles/storage-security-guide/#_encryption-in-transit) |
...
| **適用的技術** | 泛型 |
| **屬性**              | StorageType - Blob |
| **引用**              | [Windows Azure Blob MD5 概觀](https://blogs.msdn.microsoft.com/windowsazurestorage/2011/02/17/windows-azure-blob-md5-overview/) |
...
| **適用的技術** | 泛型 |
| **屬性**              | StorageType - 檔案 |
| **引用**              | [Azure 檔案儲存體](https://azure.microsoft.com/blog/azure-file-storage-now-generally-available/#comment-2529238931)、[Windows 用戶端的 Azure 檔案儲存體 SMB 支援](https://azure.microsoft.com/documentation/articles/storage-dotnet-how-to-use-files/#_mount-the-file-share) |
...
| **屬性**              | N/A  |
| **引用**              | [憑證和公開金鑰釘選](https://www.owasp.org/index.php/Certificate_and_Public_Key_Pinning#.Net) |
...
| **屬性**              | N/A  |
| **引用**              | [MSDN](https://msdn.microsoft.com/library/ff648500.aspx)、[Fortify Kingdom](https://vulncat.fortify.com/en/detail?id=desc.config.dotnet.wcf_misconfiguration_transport_security_enabled) |
...
| **步驟** | <ul><li>**說明︰** 保護層級設定為「無」時，它會停用訊息保護。 機密性和完整性要有適當的設定層級才能實現。</li><li>**建議：**<ul><li>當 `Mode=None` 時 - 停用訊息保護</li><li>當 `Mode=Sign` 時 - 會簽署但不加密訊息；若您認為資料完整性很重要，便應使用</li><li>當 `Mode=EncryptAndSign` 時 - 簽署並加密訊息</li></ul></li></ul><p>若您只需要驗證資訊的完整性而不必擔心機密性，請考慮關閉加密功能，只要簽署訊息即可。 對於需要驗證原始傳送者，但不會傳輸敏感性資料的作業或服務合約，這會非常實用。 降低保護層級時，請注意消息不包含任何個人資料。</p>|
...
| **適用的技術** | MVC5、MVC6 |
| **屬性**              | N/A  |
| **引用**              | [在 Web API 控制器中強制執行 SSL](https://www.asp.net/web-api/overview/security/working-with-ssl-in-web-api) |
...
## <a name="ensure-that-communication-to-azure-cache-for-redis-is-over-ssl"></a><a id="redis-ssl"></a>確定對 Azure Cache for Redis 的通訊是透過 SSL
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [Azure Redis SSL 支援](https://azure.microsoft.com/documentation/articles/cache-faq/#when-should-i-enable-the-non-ssl-port-for-connecting-to-redis) |
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [選擇您的通訊協定](https://azure.microsoft.com/documentation/articles/iot-hub-devguide/#messaging) |
