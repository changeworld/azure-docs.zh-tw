...
| 產品/服務 | 發行項 |
| --------------- | ------- |
| **電腦信任邊界** | <ul><li>[如果二進位檔案包含敏感資訊，請確保它們被混淆](#binaries-info)</li><li>[考慮使用加密檔案系統 （EFS） 來保護特定于使用者的機密資料](#efs-user)</li><li>[確定應用程式儲存在檔案系統上的敏感性資料已加密](#filesystem)</li></ul> | 
| **Web 應用程式** | <ul><li>[確保敏感內容未緩存在瀏覽器上](#cache-browser)</li><li>[加密包含敏感性資料的 Web 應用程式組態檔區段](#encrypt-data)</li><li>[在敏感表單和輸入中顯式禁用自動完成 HTML 屬性](#autocomplete-input)</li><li>[確定使用者畫面上顯示的敏感性資料已遮罩](#data-mask)</li></ul> | 
| **資料庫** | <ul><li>[實施動態資料遮罩，以限制敏感性資料洩露非特權使用者](#dynamic-users)</li><li>[確保密碼以鹽漬雜湊格式存儲](#salted-hash)</li><li>[確保資料庫列中的敏感性資料已加密](#db-encrypted)</li><li>[確保啟用資料庫級加密 （TDE）](#tde-enabled)</li><li>[確保資料庫備份已加密](#backup)</li></ul> | 
| **Web API** | <ul><li>[確保與 Web API 相關的敏感性資料不會存儲在瀏覽器的存儲中](#api-browser)</li></ul> | 
| Azure Document DB | <ul><li>[將儲存在 Azure Cosmos DB 中的敏感性資料加密](#encrypt-docdb)</li></ul> | 
| **Azure IaaS VM 信任邊界** | <ul><li>[使用 Azure 磁片加密加密虛擬機器使用的磁片](#disk-vm)</li></ul> | 
| **Service Fabric 信任邊界** | <ul><li>[加密服務交換矩陣應用程式中的秘密](#fabric-apps)</li></ul> | 
| **Dynamics CRM** | <ul><li>[執行安全性模型化並視需要使用業務單位/團隊](#modeling-teams)</li><li>[最小化對關鍵實體上的共用功能的訪問](#entities)</li><li>[培訓使用者有關 Dynamics CRM 共用功能和良好安全實踐的風險](#good-practices)</li><li>[包括一個開發標準規則，禁止在異常管理中顯示配置詳細資訊](#exception-mgmt)</li></ul> | 
| **Azure 存儲** | <ul><li>[針對待用資料使用 Azure 儲存體服務加密 (SSE) (預覽)](#sse-preview)</li><li>[使用用戶端加密在 Azure 儲存體中儲存敏感性資料](#client-storage)</li></ul> | 
| **行動用戶端** | <ul><li>[將寫入電話本機儲存體的敏感性資料或 PII 資料加密](#pii-phones)</li><li>[在分發到最終使用者之前，對生成的二進位檔案進行模糊處理](#binaries-end)</li></ul> | 
| **WCF** | <ul><li>[將用戶端憑據類型設置為證書或 Windows](#cert)</li><li>[未啟用 WCF 安全性模式](#security)</li></ul> | 
...
## <a name="consider-using-encrypted-file-system-efs-is-used-to-protect-confidential-user-specific-data"></a><a id="efs-user"></a>考慮使用加密檔案系統 (EFS) 來保護機密的使用者特定資料
...
## <a name="ensure-that-sensitive-data-stored-by-the-application-on-the-file-system-is-encrypted"></a><a id="filesystem"></a>確定應用程式儲存在檔案系統上的敏感性資料已加密
...
## <a name="ensure-that-sensitive-content-is-not-cached-on-the-browser"></a><a id="cache-browser"></a>確定瀏覽器上不會快取敏感性內容
...
| **屬性**              | N/A  |
| **引用**              | N/A  |
| **步驟** | 瀏覽器可以儲存資訊以便進行快取和記錄歷程。 這些快取的檔案會儲存在資料夾中，例如若是 Internet Explorer，則會儲存在 Temporary Internet Files 資料夾內。 當您再次參閱這些頁面時，瀏覽器就會顯示快取內的這些頁面。 如果向使用者顯示敏感資訊（如其位址、信用卡詳細資訊、社會保險號或使用者名），則此資訊可以存儲在瀏覽器的緩存中，因此可以通過檢查瀏覽器的緩存或通過只需按下瀏覽器的"後退"按鈕。 將緩存控制回應標頭值設置為所有頁面的"無存儲"。 |
...
## <a name="encrypt-sections-of-web-apps-configuration-files-that-contain-sensitive-data"></a><a id="encrypt-data"></a>加密包含敏感性資料的 Web 應用程式組態檔區段
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [作法：在 ASP.NET 2.0 中使用 DPAPI 加密組態區段](https://msdn.microsoft.com/library/ff647398.aspx)、[指定受保護的組態提供者](https://msdn.microsoft.com/library/68ze1hb2.aspx)、[使用 Azure Key Vault 來保護應用程式的機密資訊](https://azure.microsoft.com/documentation/articles/guidance-multitenant-identity-keyvault/) |
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [MSDN︰自動完成屬性](https://msdn.microsoft.com/library/ms533486(VS.85).aspx)、[在 HTML 中使用自動完成](https://msdn.microsoft.com/library/ms533032.aspx)[HTML 清理弱點](https://technet.microsoft.com/security/bulletin/MS10-071)、[再次自動完成？！](https://blog.mindedsecurity.com/2011/10/autocompleteagain.html) |
...
## <a name="ensure-that-sensitive-data-displayed-on-the-user-screen-is-masked"></a><a id="data-mask"></a>確定使用者畫面上顯示的敏感性資料已遮罩
...
## <a name="implement-dynamic-data-masking-to-limit-sensitive-data-exposure-non-privileged-users"></a><a id="dynamic-users"></a>實作動態資料遮罩來限制敏感性資料暴露非特殊權限使用者
...
| **引用**              | [動態資料遮罩](https://msdn.microsoft.com/library/mt130841) |
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [使用 .NET Crypto API 的密碼雜湊](https://docs.asp.net/en/latest/security/data-protection/consumer-apis/password-hashing.html) |
...
## <a name="ensure-that-sensitive-data-in-database-columns-is-encrypted"></a><a id="db-encrypted"></a>確定資料庫資料行中的敏感性資料已加密
...
| **適用的技術** | 泛型 |
| **屬性**              | SQL 版本 - 全部 |
| **引用**              | [加密 SQL Server 中的敏感性資料](https://technet.microsoft.com/library/ff848751(v=sql.105).aspx)、[加密 SQL Server 中的一欄資料](https://msdn.microsoft.com/library/ms179331)、[以憑證加密](https://msdn.microsoft.com/library/ms188061) |
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [了解 SQL Server 的透明資料加密 (TDE)](https://technet.microsoft.com/library/bb934049(v=sql.105).aspx) |
...
| **引用**              | [SQL Database 備份加密](https://msdn.microsoft.com/library/dn449489) |
...
## <a name="encrypt-sensitive-data-stored-in-cosmos-db"></a><a id="encrypt-docdb"></a>將儲存在 Cosmos DB 中的敏感性資料加密
...
## <a name="use-azure-disk-encryption-to-encrypt-disks-used-by-virtual-machines"></a><a id="disk-vm"></a>使用 Azure 磁碟加密來加密虛擬機器所使用的磁碟
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [使用 Azure 磁碟加密來加密虛擬機器所使用的磁碟](https://azure.microsoft.com/documentation/articles/storage-security-guide/#_using-azure-disk-encryption-to-encrypt-disks-used-by-your-virtual-machines) |
...
| **適用的技術** | 泛型 |
| **屬性**              | 環境 - Azure |
| **引用**              | [管理 Service Fabric 應用程式中的密碼](https://azure.microsoft.com/documentation/articles/service-fabric-application-secret-management/) |
...
## <a name="minimize-access-to-share-feature-on-critical-entities"></a><a id="entities"></a>將共用重要實體功能的存取權最小化
...
## <a name="train-users-on-the-risks-associated-with-the-dynamics-crm-share-feature-and-good-security-practices"></a><a id="good-practices"></a>訓練使用者以讓其了解與 Dynamics CRM 共用功能相關聯的風險以及正確的安全性作法
...
## <a name="include-a-development-standards-rule-proscribing-showing-config-details-in-exception-management"></a><a id="exception-mgmt"></a>包含開發標準規則來禁止在例外狀況管理中顯示組態詳細資料
...
## <a name="use-azure-storage-service-encryption-sse-for-data-at-rest-preview"></a><a id="sse-preview"></a>針對待用資料使用 Azure 儲存體服務加密 (SSE) (預覽)
...
| **適用的技術** | 泛型 |
| **屬性**              | StorageType - Blob |
| **引用**              | [待用資料的 Azure 儲存體服務加密 (預覽)](https://azure.microsoft.com/documentation/articles/storage-service-encryption/) |
...
| **適用的技術** | 泛型、Xamarin  |
| **屬性**              | N/A  |
| **引用**              | [使用 Microsoft Intune 原則管理裝置上的設定和功能](https://docs.microsoft.com/mem/intune/configuration/)、[金鑰鏈貼身](https://components.xamarin.com/view/square.valet) |
...
## <a name="obfuscate-generated-binaries-before-distributing-to-end-users"></a><a id="binaries-end"></a>先對進行模糊處理再散發給使用者
...
| **適用的技術** | 泛型 |
| **屬性**              | N/A  |
| **引用**              | [適用於 .Net 的 CryptoObfuscator](https://www.ssware.com/cryptoobfuscator/obfuscator-net.htm) |
| **步驟** | 產生的二進位檔 (apk 內的組件) 應該進行模糊處理，以阻止組件遭到進行反向工程。`CryptoObfuscator` 之類的工具可用於此用途。 |

## <a name="set-clientcredentialtype-to-certificate-or-windows"></a><a id="cert"></a>將 clientCredentialType 設定為憑證或 Windows
...
| **屬性**              | N/A  |
| **引用**              | [Fortify](https://vulncat.fortify.com/en/detail?id=desc.config.dotnet.wcf_misconfiguration_weak_token) |
...
## <a name="wcf-security-mode-is-not-enabled"></a><a id="security"></a>未啟用 WCF 安全性模式
...
| **引用**              | [MSDN](https://msdn.microsoft.com/library/ff648500.aspx)、[Fortify Kingdom](https://vulncat.fortify.com/en/detail?id=desc.config.dotnet.wcf_misconfiguration_weak_class_reference)、[WCF 安全性基礎概念 CoDe Magazine](https://www.codemag.com/article/0611051) |
