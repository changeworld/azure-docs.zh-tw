...
將所有輸入視為不受信任，以保護您的應用程式免于出現最常見的 web 應用程式弱點。 不受信任的資料是插入式攻擊的車輛。 應用程式的輸入包括 URL 中的參數、使用者的輸入、資料庫中的資料或 API，以及使用者可能操作的任何傳遞內容。 應用程式應該在應用程式以任何方式（包括向使用者顯示資料）之前，[驗證](https://owasp.org/www-project-proactive-controls/v3/en/c5-validate-inputs)資料在語法上和語義上都有效。
