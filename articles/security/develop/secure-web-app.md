...
您應該按順序按照本文中描述的步驟操作,以確保正確配置應用程式元件。 資料庫、Azure 應用服務、Azure 密鑰保管庫實例和 Azure 應用程式閘道實例彼此依賴。

部署腳本設置基礎結構。 運行部署腳本後,需要在 Azure 門戶中執行一些手動配置,以將元件和服務連結在一起。

示例應用面向在 Azure 上開發應用程式,希望在其應用程式中實施安全措施的初學者。

在開發並部署此應用程式時,您將學習如何:

- 創建 Azure 金鑰保管庫實例,存儲和檢索其機密。
- 為 PostgreSQL 部署 Azure 資料庫,設定安全密碼並授權訪問它。
...
- [Linux 的 Azure Web 應用程式](../../app-service/containers/app-service-linux-intro.md)。 提供在 Linux 環境中執行 Python 應用的容器運行時。
- [Azure 金鑰保存函式庫](../../key-vault/index.yml)。 存儲和加密應用的機密,並管理圍繞其創建訪問策略。
- [PostgreSQL](https://azure.microsoft.com/services/postgresql/)的 Azure 資料庫 。 安全地存儲我們應用的數據。
- [Azure 安全中心與](../../security-center/index.yml) [Azure 應用程式來見解](../../azure-monitor/app/app-insights-overview.md)。 提供有關應用程式操作的監視和警報。

## <a name="threat-model"></a>威脅模型

威脅建模是識別業務和應用程式的潛在安全威脅,然後確保制定適當的緩解計劃的過程。

此示例使用[Microsoft 威脅建模工具](threat-modeling-tool.md)為安全示例應用實現威脅建模。 通過繪製元件和數據流圖,可以在開發過程的早期識別問題和威脅。 這樣可以節省時間和金錢。
...
例如,透過清理使用者輸入和使用 Azure 資料庫中的儲存函數進行 PostgreSQL 來緩解上述威脅模型輸出中的 SQL 注入。 此緩解措施可防止在數據讀取和寫入期間任意執行查詢。

開發人員通過緩解威脅模型輸出中的每個威脅來提高系統的整體安全性。

## <a name="deployment"></a>部署

以下選項允許您在 Azure 應用服務執行 Linux:

- 從 Azure 上已建構的預建構 Microsoft 容器清單中選擇一個容器,該容器已使用支援技術(Python、Ruby、PHP、Java、Node.js、.NET Core)創建。
- 使用自定義容器。 選擇您自己的容器註冊表作為映射的源,並基於支援 HTTP 的許多可用技術進行構建。

在此範例中,您將運行部署文稿,該文本將 Webapp 部署到應用服務並創建資源。

該應用程式可以使用以下的不同部署模型:

![部署資料串流圖](./media/secure-web-app/deployment.png)
...
範例應用對流入和流出網路的傳輸中資料使用端到端TLS/SSL加密。 閘道配置了自簽名證書。
> [!IMPORTANT]
> 此演示使用自簽名證書。 在生產環境中,應從經過驗證的證書頒發機構 (CA) 獲取證書。
...
要登錄到門戶,範例應用對已分配對資源的訪問許可權的 Azure 活動目錄 (Azure AD) 管理員使用多重身份驗證。
範例應用使用託管標識從 Azure 密鑰保管庫獲取讀取和檢索機密的許可權,確保應用不需要硬編碼認證和權杖即可讀取機密。 Azure AD 會自動創建應用在使用託管標識時需要讀取和修改機密的服務主體。
...
PostgreSQL 資料庫中的數據由 PostgreSQL 的 Azure 資料庫自動加密靜態數據。 資料庫授權應用服務 IP 位址,以便只有已部署的應用服務 Web 應用才能使用正確的身份驗證認證認證資料庫資源。

### <a name="logging-and-auditing"></a>記錄與稽核

應用透過使用應用程式見解來追蹤發生的指標、日誌和異常,從而實現日誌記錄。 此日誌記錄提供了足夠的應用元數據,可通知開發人員和操作團隊成員有關應用的狀態。 它還提供了足夠的數據,以便在發生安全事件時回溯。
...
3. 文稿資料夾中有特定於您正在使用的平臺(Windows 或 Linux)的檔。 安裝 Azure CLI 後,請透過執行此 Azure CLI 命令在指令提示符下登入 Azure 帳號:

   ```azurecli-interactive
   az login
   ```

瀏覽器將打開,使用您的憑據登錄。 登錄后,可以從命令提示符開始部署資源。

部署文稿`deploy-powershell.ps1``deploy-bash.sh`並包含部署整個應用程式的代碼。
要部署解決方案:

1. 如果在 PowerShell`deploy-powershell.ps1`上執行 檔案,`./deploy-powershell.ps1 REGION RESOURCE_GROUP_NAME`則鍵入 將區域和資源群組名稱取代為合適的 Azure 區域和資源群組的名稱
2. 如果您在 Linux 上透過`deploy-bash.sh``/deploy-bash.sh REGION RESOURCE_GROUP_NAME`鍵來執行此檔案,則可能需要在鍵入檔案執行`chmod +x deploy-bash.sh`

以下範例示範了關鍵元件的代碼段。 您可以透過執行部署檔單獨部署範例,也可以與元件的其餘部分一起部署這些範例。
...
要使用 Azure CLI 部署 Azure 金鑰保管庫,請使用:
...
PostgreSQL 的 Azure 資料庫的工作方式如下,首先創建資料庫伺服器,然後創建用於儲存架構和數據的資料庫。

完成部署後,在 Azure 上運行 PostgreSQL 伺服器和資料庫。

要使用 Azure CLI 部署後格雷SQL的 Azure 資料庫,請使用:

1. 使用 Azure CLI 和 Azure 訂閱設定打開終端。
2. 生成用於訪問資料庫的安全使用者名和密碼組合。 (這些應存儲在 Azure 密鑰保管庫中,用於使用它們的應用。
3. 創建 PostgreSQL 伺服器實例。
4. 在步驟 3 中創建的伺服器實例上創建資料庫。
5. 在 PostgreSQL 實例上運行 PostgreSQL 文稿。

以下代碼依賴於上面部署 KeyVault 步驟中儲存在 Azure KeyVault 中的 PGUSERNAME 和 PGPASSWORD 機密。
...
部署資料庫後,需要將其認證和連接字串儲存在 Azure 密鑰保管庫中。
在文稿資料夾中,有一個`functions.sql`檔包含 PL/pgSQL 代碼,在執行時創建儲存的函數。 運行此檔案參數化輸入以限制 SQL 注入。

PostgreSQL 與一個稱為`psql`用於 連接到資料庫的工具捆綁在一起。 要運行`functions.sql`,需要從本地電腦連接到 PostgreSQL 實例的 Azure 資料庫,並從那裡運行它。 psql 工具的安裝包含在每個作業系統 PostgreSQL 的預設安裝中。
有關詳細資訊,請參閱[psql 文件](https://www.postgresql.org/docs/9.3/app-psql.html)。

Azure 雲外殼還包括該`psql`工具 。 您可以通過選擇雲外殼圖示直接從 Azure 門戶使用雲外殼。

要啟用對 PostgreSQL 實體的遠端存取,您需要在 PostgreSQL 中授權 IP 位址。
通過存取**連線安全**選項卡、選擇 **「添加用戶端 IP」** 以及保存新設定來啟用此存取權限。

![授權用戶端 IP](./media/secure-web-app/add-client-ip-postgres.png)

如果使用雲外殼而不是本地 psql 工具,請選擇「**允許存取 Azure 服務**」並將其值更改為**ON**以允許雲端的殼存取。

然後,通過運行以下 psql 命令與 Azure 門戶上的 PostgreSQL 實例**的連接字串**選項卡的連接字串參數連接到實例。
將空大括弧替換為資料庫的連接字串邊欄選項卡中的參數,用 Azure 密鑰保管庫的密碼替換密碼。

```shell
psql "host={} port=5432 dbname=hellodb user={} password=PGPASSWORD sslmode=require"
```

在確保已連接到資料庫後,請運行以下 PL/pgSQL 文稿。 該文本創建用於將數據插入資料庫的存儲函數。
...
有關如何為 PostgreSQL 設定 TLS 和憑證發行機構 (CA) 驗證的詳細資訊,請參閱[為 PostgreSQL 在 Azure 資料庫中設定 TLS 連接](/azure/postgresql/concepts-ssl-connection-security)。
...
閱讀有關如何為 PostgreSQL 設定 TLS 安全的更多內容,[在此設定 TLS 連線安全性](/azure/postgresql/concepts-ssl-connection-security)。

#### <a name="deploy-azure-web-apps-on-linux"></a>Linux 部署 Azure Web 應用

在 Azure 應用服務的基礎上輕鬆建構 Linux 服務,因為 Azure 為廣泛使用的語言(如 Python、Ruby、C# 和 Java)提供了一組預建構的容器和映射。 Azure 還支援自定義容器,它允許在 Azure 應用服務平臺上運行幾乎所有程式設計語言。

正在部署的應用程式是一個簡單的Python應用程式,運行在最新的UbuntuLinux發行版。 它分別連接到在前幾節中為憑據管理和數據存儲創建的 Azure 密鑰保管庫和 PostgreSQL 實例。

以下 Docker 檔案在應用程式的根資料夾中提供:
...
上面的 Dockerfile 用於生成託管在 Azure 容器`mcr.microsoft.com/samples/basic-linux-app`註冊表上的 容器。
...
1. 在 Azure 上建立新的自簽名證書。
2. 將自簽名證書下載為 base64 編碼檔。
3. 為自簽名證書生成密碼。
4. 將證書匯出為使用密碼簽名的 PFX 檔。
5. 將證書的密碼儲存在 Azure 金鑰保管庫中。
...
1. 要啟用此功能,請在 Azure 應用服務實體邊欄選項卡上,**在「設定」** 下選擇 **「網路**」 。 在右邊窗格中,在**VNet 整合**下,選擇 **「按下此處配置**」 。
...
1. 在下一個功能表上,選擇在以`hello-vnet`開始的部署中創建的虛擬網路。 您可以建立新子網或選擇現有子網。
...
4. 在閘道 NSG 的出站規則中,透過建立以服務`AppService`標記 為目標的規則,加入允許出站連接到應用服務實例的規則:
...
7. 在出站安全規則中,創建一個新的出站安全規則,允許應用服務實例與 PostgreSQL 資料庫進行通信。 像這樣設定它:

   ![允許出站後 SQL 連線的規則](./media/secure-web-app/nsg-outbound-postgresql.png)

   *新增規則以允許出站 PostgreSQL 連線*

要限制攻擊面,請修改應用服務網路設置,以僅允許應用程式閘道存取應用程式。
通過進入應用服務網路選項卡、選擇 **「IP 限制」** 選項卡以及創建只允許應用程式閘道的 IP 直接存取服務的允許規則來執行此操作。

可以從閘道的概述頁檢索閘道的 IP 位址。 IP**位址 CIDR**選項卡上,輸入此`<GATEWAY_IP_ADDRESS>/32`格式的 IP 位址: 。

![只允許閘道](./media/secure-web-app/app-allow-gw-only.png)

*只允許閘道 IP 存取應用服務*

#### <a name="implement-azure-active-directory-oauth"></a>完整下載 Azure 的目錄 OAuth

在範例 Web 應用程式時分發的 Azure 文件是我們應用中可能需要保護的資源。 可以使用 Azure 活動目錄 (Azure AD) 使用不同的身份驗證流實現 Web、桌面和行動應用程式的身份驗證。
該應用程式使用**與 Microsoft 登錄**,允許應用讀取已添加到單租戶 Azure AD 使用者清單中的使用者的配置檔。

在 Azure 門戶中,將應用配置為使用所需的認證:

1. 選擇**Azure 活動目錄**,或使用搜尋框搜尋它。
...
3. 在下一頁上,輸入應用名稱。 在**支援帳號類型**下,**選擇「 僅在此組織目錄中的帳戶**」 。
    在**重定向URI**下,輸入應用將運行的基本域加上帶有令牌終結點的一個域。 例如 *:GATEWAY_HASH*.cloudapp.net/token。

   ![設定 Azure AD 應用程式註冊](./media/secure-web-app/ad-auth-type.png)

   *設定 Azure AD 應用程式註冊*

4. 將顯示顯示已註冊應用及其信息的螢幕。 您需要將此資訊添加到 Azure 金鑰保管庫實例中。
   1. 複製應用程式 (用戶端)ID 並將儲存在金鑰保存庫中為`CLIENTID`。
   2. 複製您在上一步中輸入的重定到 URI 並將其另`REDIRECTURI`存為 。
   3. 複製 Azure AD 預設目錄名稱 (其格式*名稱*.microsoftonline.com,並將儲存在金鑰保`TENANT`管庫中為 。
   4. 跳到以前創建的 Azure AD 應用的 **「證書&機密**」選項卡,然後選擇 **「新建用戶端機密**」,如下圖所示。 設定到期日期,然後複製產生的值並將儲存在金鑰保管庫中為`CLIENTSECRET`。

      ![Azure AD 授權金鑰](./media/secure-web-app/ad-auth-secrets.png)

      *Azure AD 授權金鑰*

   5. 使用任何命令列/連線工具生成安全的隨機密鑰。 將其另存到密鑰保管`FLASKSECRETKEY`庫中。 應用程式框架使用此金鑰創建作業階段。
        要瞭解如何生成密鑰,請參閱[「跳蚤會話](http://flask.pocoo.org/docs/1.0/quickstart/#sessions)」。
...
5. 配置登錄後,需要將使用者添加到 Azure AD 連結,以允許他們登錄到資源。 要添加它們,請轉到 Azure AD 中的 **「使用者」** 選項卡,選擇 **「所有使用者**」,然後選擇 **「新使用者**」或 **「新來賓使用者**」 。。 對於測試,您可以添加來賓使用者並邀請使用者進入目錄。 或者,如果應用正在運行的域已過驗證,則可以添加新使用者。 在此示例中,只能註冊在 Azure AD 租戶中註冊的使用者才能進行訪問。 有關多租戶登錄訪問的資訊,請參閱文檔。
   ...
   ![將使用者加入預設網域](./media/secure-web-app/ad-auth-add-user.png)

   *將使用者新增到預設的 Azure 的目錄網域*
...
現在,應用已部署並工作,您需要處理應用中發生的錯誤以及日誌記錄和跟蹤數據收集。
日誌記錄和跟蹤數據收集提供了對應用中發生的審核事件的檢視。

應用程式見解是收集使用者或系統可以生成的日誌的服務。

要創建應用程式見解實例,

1. 使用 Azure 門戶中的搜尋框搜尋**應用程式的見解**。
2. 選取 [Application Insights] ****。 提供此處顯示的詳細資訊以創建實例。

   ![建立應用程式見解實例](./media/secure-web-app/app-insights-data.png)

部署完成後,您有一個應用程式見解實例。

創建應用程式見解實例後,需要使應用瞭解允許其向雲發送日誌的檢測密鑰。 為此,請檢索應用程式見解密鑰,並在 Azure 為應用程式見解提供的應用程式庫中使用它。 最佳做法是在 Azure 密鑰保管庫中儲存密鑰和機密,以確保它們的安全。

對於基本範例應用程式,在創建應用程式見解實例後,需要使應用瞭解允許其向雲發送日誌的檢測密鑰。
在密鑰保管庫中,設置`APPINSIGHTSKEY`機密並將其值設置為檢測密鑰。 這樣做允許應用向應用程式見解發送日誌和指標。
...
2. 在安全類別下,選擇條件訪問。 您會看到這個畫面：
   ...
   ![條件存取 - 政策](./media/secure-web-app/ad-mfa-conditional-add.png)
...
2. 選擇 Azure AD 進階**免費試用版**連結以訂閱免費試用版。
   ...
   ![Azure AD 進階免費試用版](./media/secure-web-app/ad-trial-premium.png)
...
2. **選擇立即使用政策**。
   ...
   ![立即選擇使用政策](./media/secure-web-app/ad-mfa-conditional-enable.png)
...
1. 在 Azure 門戶中的搜尋框中,搜尋**紀錄分析**。 選取 [Log Analytics 工作區]****。
   ...
   ![搜尋紀錄分析工作區](./media/secure-web-app/sentinel-log-analytics.png)
...
4. 選擇 **'添加**",然後選擇之前創建的日誌分析工作區。
   ...
   ![新增紀錄分析工作區](./media/secure-web-app/sentinel-workspace-add.png)

    *新增紀錄分析工作區*

5. 在**Azure 哨兵 - 資料連接器**頁上,在**配置**下,選擇 **「數據連接器**」。 您將看到一系列 Azure 服務,可以連結到 Azure Sentinel 中的日誌分析儲存實例進行分析。

   ![紀錄分析資料連接器](./media/secure-web-app/sentinel-connectors.png)
   ...
   4. 在 **「診斷設定」** 頁上,選擇您建立的紀錄分析工作區,然後選擇要收集的所有指標並將其發送到 Azure Sentinel。 選取 [儲存]  。
        ...
        ![Azure 哨兵連線器設定](./media/secure-web-app/sentinel-connector-settings.png)

        *Azure 哨兵連線器設定*

  來自資源的指標位於 Azure Sentinel 中,您可以在其中查詢和調查它們。

   在 Azure 金鑰保管庫、公共 IP 位址、PostgreSQL Azure 資料庫以及帳戶中支援診斷日誌的任何服務的診斷設置中添加相同的指標。

設置指標後,Azure Sentinel 具有要分析的數據。
...
對於示例應用,使用靜態分析工具進行驗證涉及使用污點檢查和數據流分析等技術查找應用代碼中的漏洞。 Python 靜態分析工具讓您更有信心你的應用是安全的。
...
修復林廷工具發現的某些代碼錯誤后,您更有信心代碼不容易出錯。 修復錯誤可顯著降低將代碼部署到生產環境時可能出現的安全風險。
