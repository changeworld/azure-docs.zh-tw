...
ms.locfileid: "80548310"
---
# <a name="azure-best-practices-for-network-security"></a>網路安全的 Azure 最佳實作
本文討論了增強網路安全的 Azure 最佳實務的集合。 這些最佳作法衍生自我們的 Azure 網路經驗和客戶的經驗。
...
這些最佳實務基於共識,Azure 平臺功能和功能集,因為它們在撰寫本文時存在。 意見和技術會隨著時間改變，這篇文章將會定期進行更新以反映這些變更。
...
- 管理核心網路功能,如 ExpressRoute、虛擬網路和子網預配以及 IP 尋址。
- 管理網路安全元素,如網路虛擬設備功能,如 ExpressRoute、虛擬網路和子網預配以及 IP 定址。

如果使用一組通用的管理工具來監視網路和網路的安全性,則可以清楚地瞭解這兩種功能。 簡單、統一的安全策略減少了錯誤,因為它提高了人類的理解和自動化的可靠性。

## <a name="logically-segment-subnets"></a>以邏輯方式分割子網路
Azure 虛擬網路與本地網路上的 LAN 類似。 Azure 虛擬網路背後的理念是,您可以基於單個專用 IP 位址空間創建網路,您可以在其中放置所有 Azure 虛擬機器。 可用的私人 IP 位址空間位於類別 A (10.0.0.0/8)、類別 B (172.16.0.0/12) 和類別 C (192.168.0.0/16) 範圍中。

以邏輯方式分割子網路的最佳做法包括：

**最佳實務**:不要分配範圍廣的允許規則(例如,允許 0.0.0.0 到 255.255.255)。  
...
**最佳做法**：建立子網路之間的網路存取控制。 子網路由會自動發生,您無需手動配置路由表。 默認情況下,在 Azure 虛擬網路上創建的子網之間沒有網路存取控制項。   
**詳細資訊**:使用[網路安全組](/azure/virtual-network/virtual-networks-nsg)防止未經請求的流量進入 Azure 子網。 網路安全組是簡單、有狀態的資料包檢測設備,使用 5 元組方法(源 IP、源埠、目標 IP、目標埠和第 4 層協定)為網路流量創建允許/拒絕規則。 您可以允許或拒絕單一 IP 位址、多個 IP 位址或整個子網路的輸入或輸出流量。

在子網之間使用網路安全組進行網路訪問控制時,可以將屬於同一安全區域的資源或角色放在自己的子網中。

**最佳實踐**:避免小型虛擬網路和子網,以確保簡單性和靈活性。   
**詳細資訊**:大多數組織添加的資源比最初計劃的資源多,重新分配位址是勞動密集型的。 使用小型子網會增加有限的安全值,並將網路安全組映射到每個子網會增加開銷。 廣泛定義子網,以確保您具有增長的靈活性。

**最佳實務**:透過定義[應用程式安全組](https://azure.microsoft.com/blog/applicationsecuritygroups/)來簡化網路安全組規則管理。  
**詳細資訊**:為您認為將來可能會更改或跨多個網路安全組使用的 IP 位址列表定義應用程式安全組。 請務必明確命名應用程式安全組,以便其他人能夠瞭解其內容和目的。

## <a name="adopt-a-zero-trust-approach"></a>採用零信任方法
基於週邊的網路運行假設網路中的所有系統都可以信任。 但是,當今的員工在各種設備和應用程式上從任何位置訪問其組織的資源,這使得週邊安全控制變得無關緊要。 只關注誰可以訪問資源的訪問控制策略是不夠的。 為了掌握安全性和工作效率之間的平衡,安全管理員還需要考慮*如何*訪問資源。

網路需要從傳統防禦中發展,因為網路可能容易受到破壞:攻擊者可能會破壞受信任邊界內的單個終結點,然後在整個網路中快速擴展立足點。 [零信任](https://www.microsoft.com/security/blog/2018/06/14/building-zero-trust-networks-with-microsoft-365/)網路消除了基於週邊網路位置的信任概念。 相反,零信任體系結構使用設備和使用者信任聲明來對組織數據和資源的訪問進行門禁。 對於新計劃,採用零信任方法,在訪問時驗證信任。

最佳做法包括:

**最佳實務**:根據設備、標識、保證、網路位置等對資源進行條件訪問。  
**詳細資訊**[:Azure AD 條件存取](/azure/active-directory/conditional-access/overview)允許您根據所需的條件實現自動存取控制決策,從而應用正確的存取控制。 有關詳細資訊,請參閱[使用條件存取管理 Azure 管理的存取](../../role-based-access-control/conditional-access-azure-management.md)。

**最佳做法**:僅在工作流批准後啟用埠訪問。  
**詳細資訊**:您可以使用[Azure 安全中心中的即時 VM 訪問](../../security-center/security-center-just-in-time.md)來鎖定 Azure VM 的入站流量,從而減少攻擊風險,同時在需要時輕鬆存取 VM。

**最佳做法**:授予執行特權任務的臨時許可權,以防止惡意或未經授權的使用者在許可權過期后獲取訪問許可權。 只有當使用者需要時才會獲得存取權。  
**詳細資訊**:在 Azure AD 特權識別管理或第三方解決方案中使用即時訪問許可權,以授予執行特權任務的許可權。

零信任是網路安全的下一個演變。 網路攻擊狀態促使組織採取「假定違約」的心態,但這種方法不應受到限制。 零信託網路保護企業數據和資源,同時確保組織能夠通過使用技術構建現代工作場所,使員工能夠隨時隨地、以任何方式提高工作效率。
...
網路安全組和使用者定義的路由可以為[OSI 模型](https://en.wikipedia.org/wiki/OSI_model)的網路和傳輸層提供一定的網路安全。 但在某些情況下，您會想要或需要在堆疊的高層級啟用安全性。 在這類情況下，建議您部署 Azure 合作夥伴所提供的虛擬網路安全性應用裝置。
...
要查找可用的 Azure 虛擬網路安全設備,請造訪[Azure 應用商店](https://azure.microsoft.com/marketplace/)並搜尋「安全性」和「網路安全」。
...
周邊網路非常實用，因為您可以將網路存取控制管理、監視、記錄和報告的重點放在位於 Azure 虛擬網路邊緣的裝置。 外圍網路是您通常啟用分散式拒絕服務 (DDoS) 防護、入侵檢測/入侵防禦系統 (IDS/IPS)、防火牆規則和策略、Web 篩選、網路反惡意軟體等功能。 網路安全性裝置位於網際網路與您的 Azure 虛擬網路之間，具有兩個網路均適用的介面。

雖然這是週邊網路的基本設計,但有很多不同的設計,如背對背、三套和多家式設計。

基於前面提到的零信任概念,我們建議您考慮對所有高安全性部署使用外圍網路,以提高 Azure 資源的網路安全和訪問控制級別。 您可以使用 Azure 或第三方解決方案在資產和 Internet 之間提供額外的安全層:

- Azure 本機控件。 [Azure 防火牆](/azure/firewall/overview)和[應用程式閘道中的 Web 應用程式防火牆](../../application-gateway/features.md#web-application-firewall)提供基本安全性,以完全狀態的防火牆為服務、內置高可用性、不受限制的雲端可伸縮性、FQDN 篩選、對 OWASP 核心規則集的支援以及簡單的設置和配置。
- 第三方產品。 在[Azure 應用商店](https://azuremarketplace.microsoft.com/)中搜索下一代防火牆 (NGFW) 和其他提供熟悉安全工具和顯著增強的網路安全級別的第三方產品。 配置可能更為複雜,但第三方產品可能允許您使用現有功能和技能集。

## <a name="avoid-exposure-to-the-internet-with-dedicated-wan-links"></a>通過專用廣域網連結避免接觸互聯網
...
* [網站間 VPN](../../vpn-gateway/vpn-gateway-howto-multi-site-to-site-resource-manager-portal.md)： 這是一個值得信賴的,可靠的,成熟的技術,但連接發生在互聯網上。 帶寬被限制到最大約 1.25 Gbps。 在某些情況下,網站到網站 VPN 是理想的選擇。
* **Azure 快速路由**。 我們建議您針對跨單位連線改用 [ExpressRoute](../../expressroute/expressroute-introduction.md)。 ExpressRoute 可讓您透過連線提供者所提供的私人連線，將內部部署網路延伸至 Microsoft 雲端。 使用 ExpressRoute,您可以建立與 Microsoft 雲服務(如 Azure、Office 365 和 Dynamics 365)的連接。 ExpressRoute 是本地位置或Microsoft Exchange託管供應商之間的專用 WAN 鏈路。 因為這是一個電信連接,您的數據不會通過互聯網傳播,所以它不會接觸到互聯網通信的潛在風險。

ExpressRoute 連接的位置可能會影響防火牆容量、可擴充性、可靠性和網路流量可見性。 您需要確定在現有(本地)網路中終止 ExpressRoute 的位置。 您可以：

- 如果需要對流量進行可見性,則需要繼續隔離資料中心的現有做法,或者僅將外聯網資源放在 Azure 上,則在防火牆外部終止(週邊網路範例)。
...
**負載平衡選項**：使用 [Azure 應用程式閘道](/azure/application-gateway/application-gateway-introduction) (HTTP Web 流量負載平衡器)。 應用程式閘道支援閘道的端到端 TLS 加密與[TLS 端接](/azure/application-gateway/application-gateway-introduction)。 網頁伺服器可以不用再承受加密和解密的負荷，且流量未經加密就流向後端伺服器。

**案例**：您需要將來自網際網路的傳入連線，進行負載平衡分散到位在 Azure 虛擬網路中的伺服器之間。 案例如當您：

- 有接受來自網際網路傳入要求的無狀態應用程式時。
- 不需要粘性會話或 TLS 卸載。 黏性工作階段是搭配應用程式負載平衡，以達到伺服器親和性的方法。
...
**選項**：[站對站 VPN](/azure/vpn-gateway/vpn-gateway-site-to-site-create) 透過網際網路將整個網路連線到另一個網路。 您可以使用站對站 VPN 將內部部署網路連線到 Azure 虛擬網路。 您內部部署網路上的使用者是透過站對站 VPN 連線，使用 RDP 或 SSH 通訊協定來連線。 您不必允許透過網路直接存取 RDP 或 SSH。
...
有關使用 Azure 設計、部署和管理雲端解決方案時要使用的更多安全最佳實務,請參閱[Azure 安全最佳實務和模式](best-practices-and-patterns.md)。
