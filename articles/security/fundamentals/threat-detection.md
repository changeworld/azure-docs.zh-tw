...
Azure 通過 Azure 活動目錄 （Azure AD）、Azure 監視器日誌和 Azure 安全中心等服務提供內置的高級威脅檢測功能。 這個安全性服務和功能集合提供簡單且快速的方式，來了解您 Azure 部署內的一舉一動。

Azure 提供各種選項來設定和自訂安全性，以符合您應用程式部署的需求。 本文討論如何滿足這些需求。

## <a name="azure-active-directory-identity-protection"></a>Azure Active Directory Identity Protection

[Azure AD 標識保護](../../active-directory/identity-protection/overview.md)是[Azure 活動目錄高級 P2](../../active-directory/active-directory-whatis.md)版本功能，提供可能影響組織標識的風險檢測和潛在漏洞的概述。 標識保護使用通過[Azure AD 異常活動報告](../../active-directory/active-directory-reporting-azure-portal.md)提供的現有 Azure AD 異常檢測功能，並引入新的風險檢測類型來檢測即時異常。

![Azure AD Identity Protection 圖表](./media/threat-detection/azure-threat-detection-fig1.png)

身份保護使用自我調整機器學習演算法和啟發式方法來檢測可能指示身份已遭到破壞的異常和風險檢測。 使用此資料，標識保護將生成報告和警報，以便您可以調查這些風險檢測並採取適當的補救或緩解措施。

Azure Active Directory Identity Protection 不只是監視和報告工具而已。 根據風險檢測，身份保護會為每個使用者計算使用者風險級別，以便您可以配置基於風險的策略以自動保護組織的身份。

除了 Azure 活動目錄和[EMS](../../active-directory/active-directory-conditional-access-azure-portal.md)提供的其他[條件訪問控制項](../../active-directory/active-directory-conditional-access-azure-portal.md)外，這些基於風險的策略還可以自動阻止或提供自我調整修正操作，包括密碼重設和多重要素驗證實施。

### <a name="identity-protection-capabilities"></a>Identity Protection 功能

Azure Active Directory Identity Protection 不只是監視和報告工具而已。 若要保護您組織的身分識別，您可以設定以風險為基礎的原則，當達到指定風險層級時自動回應偵測到的問題。 除了 Azure 活動目錄和 EMS 提供的其他條件訪問控制項外，這些策略還可以自動阻止或啟動自我調整修正操作，包括密碼重設和多重要素驗證實施。

Azure Identity Protection 可用以協助保護您的帳戶和身分識別的一些方法範例包括：

[檢測風險檢測和風險帳戶](../../active-directory/identity-protection/overview.md)
-   使用機器學習和啟發式規則檢測六種風險檢測類型。
-   計算使用者風險層級。
-   提供自訂建議，藉由將弱點醒目提示來改善整體安全性狀態。

[調查風險檢測](../../active-directory/identity-protection/overview.md)
-   發送風險檢測通知。
-   使用相關和上下文資訊調查風險檢測。
-   提供基本工作流程來追蹤調查。
-   讓您輕鬆存取補救動作 (例如重設密碼)。
...
[Azure 監視器日誌](../../azure-monitor/index.yml)是 Microsoft 基於雲的 IT 管理解決方案，可説明您管理和保護本地和雲基礎結構。 由於 Azure 監視器日誌是作為基於雲的服務實現的，因此只需對基礎結構服務的投資最少，即可快速啟動和運行日誌。 會自動提供新的安全性功能，以節省持續維護和升級成本。

除了自行提供有價值的服務外，Azure 監視器日誌還可以與系統中心元件（如[系統中心操作管理器](https://blogs.technet.microsoft.com/cbernier/2013/10/23/monitoring-windows-azure-with-system-center-operations-manager-2012-get-me-started/)）集成，以便將現有的安全管理投資擴展到雲中。 系統中心和 Azure 監視器日誌可以協同工作，以提供完整的混合管理體驗。

### <a name="holistic-security-and-compliance-posture"></a>整體安全性與合規性狀態

[Log Analytics 安全性與稽核儀表板](../../security-center/security-center-intro.md)針對值得您注意的問題，使用內建的搜尋查詢，為您組織的 IT 安全性狀態提供全面檢視。 安全和審核儀表板是 Azure 監視器日誌中與安全性相關的一切內容的主畫面。 它可讓您深入了解您的電腦的安全性狀態。 您也可以檢視過去 24 小時、7 天或任何其他自訂時間範圍內的所有事件。

Azure 監視器日誌可説明您快速輕鬆地瞭解任何環境的整體安全狀況，所有這些都在 IT 操作的上下文中，包括軟體更新評估、反惡意軟體評估和配置基線。 可立即存取安全性記錄資料，以簡化安全性與合規性稽核程序。
...
在 Azure[監視器日誌](../../log-analytics/log-analytics-queries.md)的中心是由 Azure 託管的存儲庫。

![見解和分析圖表](./media/threat-detection/azure-threat-detection-fig4.png)

您可以藉由設定資料來源並將解決方案新增至訂用帳戶，將資料從連線的來源收集到存放庫。

![Azure 監視器日誌儀表板](./media/threat-detection/azure-threat-detection-fig5.png)

資料來源和解決方案各會建立具有其專屬屬性集的不同記錄類型，但仍能一起分析來查詢存放庫。 您可以使用相同的工具和方法，來處理各種來源所收集的各種資料。


與 Azure 監視器日誌的大部分交互都通過 Azure 門戶進行，該門戶在任何瀏覽器中運行，並允許您訪問配置設置和多個工具來分析和處理收集的資料。 從入口網站中，您可以使用：
...
解決方案向 Azure 監視器日誌添加功能。 它們主要在雲中運行，並提供日誌分析存儲庫中收集的資料分析。 解決方案還可以定義要收集的新記錄類型，這些類型可以通過日誌搜索或使用解決方案在日誌分析儀表板中提供的其他使用者介面進行分析。
...
部分範例包括：
-   **可疑進程執行**：攻擊者使用多種技術在未檢測到的情況下執行惡意軟體。 例如，攻擊者可能會讓惡意程式碼具有與合法系統檔案相同的名稱，但會將這些檔案放在替代位置、使用類似良性檔案名稱的名稱，或為檔案的真正副檔名加上遮罩。 資訊安全中心會建立處理序行為的模型，並監視處理序執行以偵測這類極端值。

-   **隱藏的惡意軟體和開發嘗試**： 複雜的惡意軟體可以通過從不寫入磁片或加密存儲在磁片上的軟體元件來逃避傳統的反惡意軟體產品。 不過，可以使用記憶體分析來偵測這類惡意程式碼，因為惡意程式碼必須在記憶體中留下蹤跡才能運作。 當軟體損毀時，損毀傾印會在損毀時擷取部分的記憶體。 藉由分析損毀傾印中的記憶體，Azure 資訊安全中心可以偵測到用來惡意探索軟體中的弱點、存取機密資料，以及暗中保存於遭入侵電腦的技術，而不會影響您電腦的效能。
...
-   **檢測調優**：演算法針對真實的客戶資料集運行，安全研究人員與客戶合作驗證結果。 真肯定和誤判可用來縮小機器學習演算法的範圍。
...
適用于 Azure 的[Microsoft 反惡意軟體](antimalware.md)是應用程式和租戶環境的單代理解決方案，旨在在後臺運行，無需人工干預。 您可依據應用程式工作負載需求，選擇預設的基本安全性或進階的自訂組態 (包括反惡意程式碼監視) 來部署保護。 Azure Antimalware 是自動安裝在所有 Azure PaaS 虛擬機器之 Azure 虛擬機器的安全性選項。
...
-   **靈活、可靠的檢測**：異常檢測模型允許使用者配置靈敏度設置並檢測季節性和非季節性資料集中的異常情況。 使用者可以根據自己的需求來調整異常偵測模型，以降低或提高偵測 API 的敏感度。 這表示會在包含和不含季節性模式的資料中偵測較不常見或較常見的異常。

-   **可擴展和及時的檢測**：使用專家的域知識設置的當前閾值進行的傳統監控方式是昂貴的，不能擴展到數百萬動態變化的資料集。 會學習此 API 中的異常偵測模型，而且會從歷程記錄和即時資料自動調整模型。

-   **主動和可操作檢測**：慢趨勢和電平變化檢測可應用於早期異常檢測。 偵測到的早期異常訊號可用來引導使用者調查及處理問題區域。 此外，還可以在此異常偵測 API 服務上，開發根本原因分析模型和警示工具。
...
- [回應當今的威脅](../../security-center/security-center-alerts-overview.md#respond-threats)：説明識別針對 Azure 資源的活動威脅，並提供快速回應所需的見解。
