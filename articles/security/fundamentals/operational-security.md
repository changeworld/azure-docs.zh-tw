...
description: 瞭解 Microsoft Azure 監視器日誌、其服務及其工作原理。
services: security
...
6.  [Azure 活動目錄](../../active-directory/fundamentals/active-directory-whatis.md)


## <a name="microsoft-azure-monitor-logs"></a>微軟 Azure 監視器日誌

Microsoft Azure 監視器日誌是混合雲的 IT 管理解決方案。 Azure 監視器日誌單獨使用或擴展現有系統中心部署，可為您提供基礎架構基於雲的管理的最大靈活性和控制力。

![Azure 監視器記錄](./media/operational-security/azure-operational-security-fig1.png)

借助 Azure 監視器日誌，您可以以比競爭解決方案更低的成本管理任何雲中的任何實例，包括本地、Azure、AWS、Windows 伺服器、Linux、VMware 和 OpenStack。 Azure 監視器日誌專為雲優先世界構建，為企業管理提供了一種新的方法，是應對新業務挑戰和適應新工作負載、應用程式和雲環境的最快、最具成本效益的方法。

### <a name="azure-monitor-services"></a>Azure 監視器服務

Azure 監視器日誌的核心功能由在 Azure 中運行的一組服務提供。 每個服務會提供特定的管理功能，您可以合併服務以達成不同的管理案例。
...
[Azure 備份](https://azure.microsoft.com/documentation/services/backup)提供資料備份和還原服務，是 Azure 監視器產品和服務套件的一部分。
...
[Azure 監視器](../../security-center/security-center-intro.md)是 Microsoft 基於雲的 IT 管理解決方案，可説明您管理和保護本地和雲基礎結構。


[管理解決方案](../../monitoring/monitoring-solutions.md)是預打包的邏輯集，使用一個或多個 Azure 監視器服務實現特定的管理方案。 Microsoft 和合作夥伴提供了不同的解決方案，您可以輕鬆添加到 Azure 訂閱中，以提高 Azure 監視器的投資價值。 作為合作夥伴，您可以創建自己的解決方案來支援應用程式和服務，並通過 Azure 應用商店或快速入門範本向使用者提供它們。


![管理解決方案](./media/operational-security/azure-operational-security-fig4.png)

[更新管理解決方案](../../automation/automation-update-management.md)就是使用多項服務來提供其他功能的良好解決方案範例。 此解決方案使用 Windows 和 Linux 的[Azure 監視器日誌](../../log-analytics/log-analytics-queries.md)代理來收集有關每個代理上所需更新的資訊。 它將此資料寫入 Azure 監視器日誌存儲庫，您可以在其中使用包含的儀表板對其進行分析。
...
[Azure 監視器日誌安全和](../../security-center/security-center-monitoring.md)審核解決方案使 IT 能夠主動監視所有資源，這有助於將安全事件的影響降至最低。 Azure 監視器日誌安全和審核具有可用於監視資源的安全域。 安全性網域提供特定選項的快速存取方式，並基於安全性監視目的，更詳盡地涵蓋下列網域：
...
-   **<a href="https://docs.microsoft.com/azure/network-watcher/network-watcher-next-hop-overview">下一個躍點</a>**- 確定 Azure 網路結構中路由的資料包的下一個躍點，使您能夠診斷任何配置錯誤的使用者定義的路由。

-   **<a href="https://docs.microsoft.com/azure/network-watcher/network-watcher-security-group-view-overview">安全性群組視圖</a>**- 獲取應用於 VM 的有效和應用的安全規則。
...
[Azure 存儲分析](https://docs.microsoft.com/rest/api/storageservices/fileservices/storage-analytics)執行日誌記錄，並為存儲帳戶提供指標資料。 您可以使用此資料來追蹤要求、分析使用量趨勢，以及診斷儲存體帳戶的問題。 儲存體分析記錄適用於 [Blob、佇列及表格服務](../../storage/common/storage-introduction.md)。 儲存體分析會記錄對儲存體服務之成功和失敗要求的詳細資訊。
...
> [!Note]
> 有關存儲帳戶限制的詳細資訊，請參閱[標準存儲帳戶的可伸縮性目標](../../storage/common/scalability-targets-standard-account.md)。
...
-   使用 Azure AD 多重要素驗證和條件訪問提高應用程式安全性。
...
- [Azure 監視器日誌 |安全&合規性](https://www.microsoft.com/cloud-platform/security-and-compliance)
