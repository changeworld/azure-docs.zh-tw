...
對於想要強制執行資料存取安全性原則的組織來說，限制存取是必須做的事。 您可以使用角色型存取控制 (RBAC) 將權限指派給特定範圍的使用者、群組和應用程式，例如，需要知道並擁有最低權限安全性原則。 若要深入瞭解如何將應用程式的存取權授與使用者，請參閱 [什麼是 AZURE RBAC)  (azure 角色型存取控制 ](../../role-based-access-control/overview.md)。
