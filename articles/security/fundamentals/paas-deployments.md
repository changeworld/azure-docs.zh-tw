...
description: 瞭解在 Azure 上設計、建立和管理安全雲端應用程式的最佳作法，並瞭解 PaaS 與其他雲端服務模型的安全性優勢。
services: security
...
[在 Azure 上開發安全的應用程式](https://azure.microsoft.com/resources/develop-secure-applications-on-azure/) 是在開發雲端應用程式時，在軟體發展生命週期的每個階段所應考慮的安全性問題和控制項的一般指南。

## <a name="cloud-security-advantages"></a>雲端安全性優點
請務必瞭解您與 Microsoft 之間的 [職責劃分](shared-responsibility.md) 。 在內部部署環境中，您擁有整個堆疊，但是當您移到雲端時，部分責任就會轉移給 Microsoft。

[在雲端中有安全性優勢](shared-responsibility.md#cloud-security-advantages)。 在內部部署環境中，組織可能責任重大但可投資在安全性上的資源卻相當有限，導致創造出一種攻擊者能夠利用所有層級弱點的環境。
...
在堆疊頂端的資料控管和權限管理，您需承擔一項可由金鑰管理降低的風險。  (金鑰管理涵蓋于最佳作法中。 ) 雖然金鑰管理是額外的責任，但您在 PaaS 部署中有一些區域不再需要管理，因此您可以將資源移至金鑰管理。
...
**詳細資料**：遺失金鑰和認證是相當常見的問題。 您可以使用集中式解決方案，將金鑰和秘密儲存在硬體安全性模組中， (Hsm) 。 [Azure Key Vault](../../key-vault/general/overview.md) 使用受 hsm 保護的金鑰來加密驗證金鑰、儲存體帳戶金鑰、資料加密金鑰、.pfx 檔案和密碼，藉此保護您的金鑰和密碼。
...
Microsoft [安全性開發週期](https://www.microsoft.com/en-us/sdl)指定小組應在進行設計階段時，參與稱為威脅模型化的程序。 為了加快此程序，Microsoft 已建立 [SDL Threat Modeling Tool](../develop/threat-modeling-tool.md)。 將應用程式設計模型化，並跨所有信任界限列舉 [STRIDE](https://docs.google.com/viewer?a=v&pid=sites&srcid=ZGVmYXVsdGRvbWFpbnxzZWN1cmVwcm9ncmFtbWluZ3xneDo0MTY1MmM0ZDI0ZjQ4ZDMy) 威脅，可以盡早攔截設計錯誤。
...
| 資訊洩漏 | 機密性 | 使用[服務憑證](/rest/api/appservice/certificates)將待用的敏感性資料加密。 |
...
**詳細資料**：對於想要強制執行資料存取安全性原則的組織來說，限制存取是必須做的事。 您可以使用 Azure RBAC 將許可權指派給特定範圍的使用者、群組和應用程式。 若要深入了解授與使用者的應用程式存取權，請參閱[開始使用存取管理](../../role-based-access-control/overview.md)。

**最佳做法**：保護您的金鑰。   
**詳細資料**：Azure Key Vault 可協助保護雲端應用程式和服務所使用的密碼編譯金鑰和祕密。 您可以使用金鑰保存庫加密金鑰和密碼 (例如驗證金鑰、儲存體帳戶金鑰、資料加密金鑰、.PFX 檔案和密碼)，方法是使用受硬體安全模組 (HSM) 保護的金鑰。 為了加強保證，您可以在 HSM 中匯入或產生金鑰。 若要深入了解，請參閱 [Azure Key Vault](../../key-vault/general/overview.md)。 您可以使用 Key Vault 藉由自動更新管理 TLS 憑證。
...
**詳細資料**：使用 Azure 資訊安全中心來監視 App Service 環境。 當資訊安全中心發現潛在的安全性弱點時會建立[建議](../../security-center/asset-inventory.md)，引導您完成設定所需控制項的程序。
...
[Web 應用程式防火牆 (WAF)](../../web-application-firewall/afds/afds-overview.md) 是一個應用程式閘道功能，可提供 Web 應用程式的集中式保護，免於遭遇常見的攻擊和弱點。 WAF 會根據 [Open Web Application Security Project (OWASP) 核心規則集](https://www.owasp.org/index.php/Category:OWASP_ModSecurity_Core_Rule_Set_Project) 3.0 或 2.2.9 中的規則提供保護。
...
驗證安全性防禦與測試任何其他功能一樣重要。 進行 [滲透測試](pen-testing.md) ，使其成為組建和部署程式的標準部分。 在已部署應用程式上排程定期安全性測試和弱點掃描，並監視開啟連接埠、端點和攻擊。
...
- [Azure SQL Database 和 Azure Synapse Analytics](paas-applications-using-sql.md)
- [Azure 儲存體](paas-applications-using-storage.md)
- Azure Cache for Redis
- Azure 服務匯流排
- Web 應用程式防火牆

在開發雲端的應用程式時，請參閱在 [Azure 上開發安全的應用程式](https://azure.microsoft.com/resources/develop-secure-applications-on-azure/) ，以瞭解您應該在軟體發展生命週期的每個階段考慮的安全性問題和控制項。
