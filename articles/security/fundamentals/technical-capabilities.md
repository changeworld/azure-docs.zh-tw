...
[Azure AD Multi-Factor Authentication (MFA) ](../../active-directory/authentication/concept-mfa-howitworks.md) 是一種驗證方法，需要使用一個以上的驗證方法，並在使用者登入和交易中新增重要的第二層安全性。 [MFA 有助於保護](../../active-directory/authentication/concept-mfa-howitworks.md) 對資料和應用程式的存取，同時滿足使用者對簡單登入程式的需求。 它可以透過一些驗證選項 (例如電話、文字訊息，或行動應用程式通知或驗證代碼，以及第三方 OAuth 權杖) 來提供強大的驗證功能。
