...
-   Service Fabric 以角色為基礎的存取控制 (Service Fabric RBAC) ：此案例針對存取叢集的每個系統管理員和使用者用戶端角色使用不同的身分識別 (憑證、Azure AD 等等。 當您建立叢集時，可以指定角色身分識別。
...
>[!NOTE]
> 如需在 Service Fabric 中使用角色的詳細資訊，請參閱 [Service Fabric Service Fabric 用戶端的角色型存取控制](../../service-fabric/service-fabric-cluster-security-roles.md)。
...
- [Service Fabric 安全性檢查清單](../../service-fabric/service-fabric-best-practices-security.md)
