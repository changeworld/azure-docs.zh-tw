...
日誌分析噴嘴（噴嘴）是雲鑄造廠 （CF） 元件，該元件將指標從[雲鑄造日誌](https://docs.cloudfoundry.org/loggregator/architecture.html)器火管轉發到 Azure 監視器日誌。 在 Nozzle 的協助之下，您可以收集、檢視及分析多個部署的系統健康情況和效能計量。

在本文中，您將瞭解如何將噴嘴部署到 CF 環境，然後從 Azure 監視器日誌主控台訪問資料。
...
### <a name="1-deploy-a-cf-or-pivotal-cloud-foundry-environment-in-azure"></a>1. 在 Azure 中部署 CF 或 Pivotal 雲鑄造環境
...
### <a name="2-install-the-cf-command-line-tools-for-deploying-the-nozzle"></a>2. 安裝用於部署噴嘴的 CF 命令列工具
...
在設置 UAA 命令列用戶端之前，請確保安裝了 RubyGems。

### <a name="3-create-a-log-analytics-workspace-in-azure"></a>3. 在 Azure 中創建日誌分析工作區

您可以手動方式或使用範本來建立 Log Analytics 工作區。 該範本將為 Azure 監視器日誌主控台部署預配置的 KPI 視圖和警報的設置。 

#### <a name="to-create-the-workspace-manually"></a>若要手動建立工作區：

1. 在 Azure 門戶中，搜索 Azure 應用商店中的服務清單，然後選擇日誌分析工作區。
2. 選取 [建立]****，然後選取下列項目的選項：
...
有關詳細資訊，請參閱使用[Azure 監視器日誌入門](https://docs.microsoft.com/azure/log-analytics/log-analytics-get-started)。
...
如果您已透過 Marketplace 範本部署監視解決方案，請前往 Azure 入口網站並尋找該解決方案。 您可以在資源群組中找到您在範本中指定的解決方案。 按一下解決方案，流覽到"日誌分析主控台"，列出預配置的視圖，並列出頂級雲鑄造系統 KPI、應用程式資料、警報和 VM 運行狀況指標。 

如果您已手動建立 Log Analytics 工作區，請依照下列步驟來建立檢視和警示：

### <a name="1-import-the-oms-view"></a>1. 導入 OMS 視圖

從 OMS 門戶中，流覽到**查看設計器** > **導入** > **流覽**，然後選擇其中一個 omsview 檔。 例如，選取 Cloud Foundry.omsview**，然後儲存檢視。 圖格隨即會顯示在 [概觀]**** 頁面上。 選取以查看視覺化的計量。
...
### <a name="2-create-alert-rules"></a>2. 創建警報規則
...
| Type=CF_ValueMetric_CL Name_s=slowConsumerAlert                               | 結果數目 > 0   | 當噴嘴收到來自日誌聚合器的慢速消費者警報時，它將**慢速消費者警報**值Metric發送到 Azure 監視器日誌。 |
...
1. 登入 Ops Manager。
2. 查找**PCF 磁貼的 Microsoft Azure 日誌分析噴嘴**。
3. 選取回收圖示並確認刪除。
...
如果您移除 Nozzle，OMS 入口網站中的資料並不會自動移除。 它根據 Azure 監視器日誌保留設置過期。
...
從 PCF2.0，VM 性能指標通過系統指標轉寄站傳輸到 Azure 日誌分析噴嘴，並集成到日誌分析工作區中。 您不再需要 Log Analytics 代理程式，就能取得 VM 效能計量。 不過，您仍然可以使用 Log Analytics 代理程式來收集 Syslog 資訊。 Log Analytics 代理程式會以 Bosh 附加元件的形式安裝至您的 CF VM。 
