---
title: 解決連接問題 - MariaDB 的 Azure 資料庫
description: 瞭解如何解決與 MariaDB Azure 資料庫的連接問題，包括需要重試的暫時性錯誤、防火牆問題和中斷。
author: jan-eng
ms.author: janeng
ms.service: mariadb
...
* 使用者錯誤：例如，連接參數鍵入錯誤，例如連接字串中的伺服器名稱或使用者名中缺少*\@的伺服器名稱*尾碼。

### <a name="steps-to-resolve-persistent-connectivity-issues"></a>解決永久性連線問題的步驟

1. 設置[防火牆規則](howto-manage-firewall-portal.md)以允許用戶端 IP 位址。 (僅適用於臨時性的測試目的) 請使用 0.0.0.0 作為起始 IP 位址並使用 255.255.255.255 作為結束 IP 位址來設定防火牆規則。 這樣會開放伺服器供所有 IP 位址存取。 若這樣可解決您的連線問題，請移除此規則並針對已適當限制的 IP 位址或位址範圍建立防火牆規則。
...
4. 檢查儀表板中的服務健康情況。 如果認為存在區域中斷，請參閱[MariaDB Azure 資料庫的業務連續性概述](concepts-business-continuity.md)，瞭解恢復到新區域的步驟。
