...
description: 瞭解如何使用 PowerShell 來設定和管理適用於 MariaDB 的 Azure 資料庫中的讀取複本。
author: ajlam
ms.author: andrela
ms.service: mariadb
ms.topic: how-to
...
在本文中，您將瞭解如何使用 PowerShell 來建立和管理適用於 MariaDB 的 Azure 資料庫服務中的讀取複本。 若要深入了解讀取複本，請參閱[概觀](concepts-read-replicas.md)。
...
若要刪除主伺服器，您可以執行 `Remove-AzMariaDbServer` Cmdlet。
...
> [使用 PowerShell 重新開機適用於 MariaDB 的 Azure 資料庫 server](howto-restart-server-powershell.md)
