---
title: 防火牆規則 - MariaDB 的 Azure 資料庫
description: 瞭解如何使用防火牆規則為 MariaDB 伺服器啟用與 Azure 資料庫的連接。
author: ajlam
ms.author: andrela
ms.service: mariadb
ms.topic: conceptual
...
如果固定傳出 IP 位址不適用於 Azure 服務，則可以考慮啟用來自所有 Azure 資料中心 IP 位址的連接。 可以通過將"**允許訪問 Azure 服務"** 選項設置為"**從連接安全**窗格**中打開**"並點擊 **"保存**"，從 Azure 門戶啟用此設置。 從 Azure CLI 中，具有開始和結束位址等於 0.0.0.0 的防火牆規則設置等效。 如果不允許連線嘗試，要求就不會到達適用於 MariaDB 的 Azure 資料庫伺服器。
...
要從 VNet 安全地連接到 MariaDB 伺服器的 Azure 資料庫，請考慮使用[VNet 服務終結點](./concepts-data-access-security-vnet.md)。 

## <a name="programmatically-managing-firewall-rules"></a>以程式設計方式管理防火牆規則
除了 Azure 入口網站之外，防火牆規則還可以程式設計方式使用 Azure CLI 來管理。 

另請參閱[使用 Azure CLI 為 MariaDB 防火牆規則創建和管理 Azure 資料庫](./howto-manage-firewall-cli.md)。
...
* **伺服器的 IP 似乎是公開的：** 與 MariaDB 伺服器的 Azure 資料庫的連接通過可公開訪問的 Azure 閘道路由。 不過，實際的伺服器 IP 會受到防火牆保護。 有關詳細資訊，請訪問[連接體系結構文章](concepts-connectivity-architecture.md)。 
...
- [MariaDB Azure 資料庫中的 VNet 服務終結點](./concepts-data-access-security-vnet.md)
