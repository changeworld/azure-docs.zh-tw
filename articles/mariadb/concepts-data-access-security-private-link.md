...
當此設定設為 *[是]* 時，只允許透過私人端點的連線到您的適用於 MariaDB 的 Azure 資料庫。 當此設定設為 [*否*] 時，用戶端可以根據您的防火牆或 VNet 服務端點設定連線到您的適用於 MariaDB 的 Azure 資料庫。 此外，一旦設定了 [私人網路存取] 的值，您就無法新增和/或更新現有的防火牆和 VNet 服務端點規則。
