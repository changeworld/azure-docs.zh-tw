...
**指導**方針：為具有私人端點的適用於 MariaDB 的 Azure 資料庫設定私用連結。 Private Link 可讓您透過私人端點連線到 Azure 中的各種 PaaS 服務。 Azure Private Link 基本上會將 Azure 服務帶入私人虛擬網路 (VNet) 內部。 您的虛擬網路與適用于 mariadb 實例之間的流量會傳送到 Microsoft 骨幹網路。
...
**指導**方針：使用適用於 MariaDB 的 Azure 資料庫的 Advanced 威脅防護。 進階威脅防護會偵測異常活動，指出有不尋常及可能有害的活動試圖存取或惡意探索資料庫。

在與您適用於 MariaDB 的 Azure 資料庫實例相關聯的虛擬網路上啟用 DDoS 保護標準，以防範 DDoS 攻擊。 使用 Azure 資訊安全中心的整合式威脅情報，以拒絕與已知為惡意或未使用的網際網路 IP 位址通訊。
...
如需使用服務標記的詳細資訊： https://docs.microsoft.com/azure/virtual-network/service-tags-overview 瞭解適用於 MariaDB 的 Azure 資料庫的服務標記使用方式：https://docs.microsoft.com/azure/mariadb/concepts-data-access-security-vnet#terminology-and-description
...
**指引**：使用 Azure 活動記錄來監視網路資源設定，並偵測與適用於 MariaDB 的 Azure 資料庫實例相關的網路資源變更。 在 Azure 監視器中建立警示，以在重要網路資源發生變更時觸發。
...
**指引**：啟用診斷設定和伺服器記錄檔，並內嵌記錄來匯總適用於 MariaDB 的 Azure 資料庫實例所產生的安全性資料。 在 Azure 監視器中，使用 Log Analytics 工作區來查詢和執行分析，並使用 Azure 儲存體帳戶進行長期/封存儲存。 或者，您也可啟用 Azure Sentinel 或第三方 SIEM，並讓資料上線。
如何設定和存取伺服器記錄以進行適用於 MariaDB 的 Azure 資料庫：https://docs.microsoft.com/azure/mariadb/concepts-server-logs

如何設定及存取適用於 MariaDB 的 Azure 資料庫的 audit 記錄： https://docs.microsoft.com/azure/mariadb/howto-configure-audit-logs-portal 如何將 Azure Sentinel 上架：https://docs.microsoft.com/azure/sentinel/quickstart-onboard
...
**指引**：在您的適用於 MariaDB 的 Azure 資料庫實例上啟用診斷設定，以存取 audit、security 和診斷記錄。 請確定您已明確啟用適用于 mariadb Audit 記錄檔。 活動記錄 (自動提供) 包含事件來源、日期、使用者、時間戳記、來源位址、目的地位址，以及其他有用的項目。 您也可以啟用 Azure 活動記錄診斷設定，並將記錄傳送到相同的 Log Analytics 工作區或儲存體帳戶。
...
**指引**：在 Azure 監視器中，針對用來保存適用於 MariaDB 的 Azure 資料庫記錄的 Log Analytics 工作區，請根據貴組織的合規性法規來設定保留週期。 使用 Azure 儲存體帳戶進行長期/封存儲存。
如何設定 Log Analytics 工作區的記錄保留參數：將 https://docs.microsoft.com/azure/azure-monitor/platform/manage-cost-storage#change-the-data-retention-period 資源記錄儲存在 Azure 儲存體帳戶中：https://docs.microsoft.com/azure/azure-monitor/platform/resource-logs-collect-storage
...
**指引**：分析和監視來自適用于 mariadb 實例的記錄，以瞭解異常行為。 使用 Azure 監視器的 Log Analytics 工作區來審查記錄，並對記錄資料執行查詢。 或者，您也可啟用 Azure Sentinel 或第三方 SIEM，並讓資料上線。
...
此外，您可以啟用伺服器記錄和診斷設定，以適用于 mariadb 並將記錄傳送到 Log Analytics 工作區。 請將 Log Analytics 工作區在 Azure Sentinel 上線，因為其可提供安全性協調流程自動化回應 (SOAR) 解決方案。 如此可建立劇本 (自動化解決方案)，並用於修復安全性問題。
...
建立適用于 mariadb 資源本身時，Azure 會強制建立具有強式密碼的系統管理使用者。 不過，一旦建立適用于 mariadb 實例，您就可以使用您所建立的第一個伺服器系統管理員帳戶來建立其他使用者，並授與其系統管理存取權。 建立這些帳戶時，請確定每個帳戶都設定了不同的強式密碼。
...
**指引**：使用可存取您的適用于 mariadb 實例的專用系統管理帳戶，建立相關的標準操作程式。 使用 Azure 資訊安全中心身分識別與存取管理來監視系統管理帳戶的數目。
...
**指導**方針：對適用于 mariadb 的資料平面存取是由儲存在資料庫內的身分識別所控制，且不支援 SSO。 適用于 mariadb 的控制平面存取可透過 REST API 取得，並支援 SSO。 若要驗證，請將要求的授權標頭設定為從 Azure Active Directory 取得的 JSON Web 權杖。
...
**指導**方針：請參閱 Azure Active Directory 記錄，以協助探索過時的帳戶，其中可能包含具有適用于 mariadb 系統管理角色的帳戶。 此外，您可以使用 Azure 身分識別存取審查來有效率地管理群組成員資格、存取可用於存取適用于 mariadb 的企業應用程式，以及角色指派。 請定期檢閱使用者的存取權 (例如每 90 天)，以確定只有適當的使用者具有持續存取權。
...
**指引**：針對開發、測試和生產，實作不同的訂用帳戶及/或管理群組。 使用私用連結、服務端點和/或適用于 mariadb 防火牆規則的組合，來隔離和限制對您適用于 mariadb 實例的網路存取。
...
**指導**方針：適用於 MariaDB 的 Azure 資料庫支援使用傳輸層安全性（TLS）將您的適用於 MariaDB 的 Azure 資料庫伺服器連接到用戶端應用程式（先前稱為安全通訊端層（SSL））。 在資料庫伺服器與用戶端應用程式之間強制執行 TLS 連線，藉由將伺服器與應用程式之間的資料流加密來協助抵禦「中間人」攻擊。 在 Azure 入口網站中，請確定已針對您的所有適用于 mariadb 實例啟用 [強制執行 SSL 連線]。
...
**指導**方針：使用 Azure AD RBAC 來控制適用于 mariadb 管理平面（Azure 入口網站/Azure Resource Manager）對 Azure 資料庫的存取。 針對資料平面存取 (在該資料庫本身內)，請使用 SQL 查詢來建立使用者和設定使用者權限。
...
**指導**方針：適用於 MariaDB 的 Azure 資料庫服務會使用 FIPS 140-2 驗證的密碼編譯模組，以進行待用資料的儲存體加密。 資料 (包含備份) 會在磁碟上加密，但不包括執行查詢時所建立的暫存檔案。 該服務使用包含在 Azure 儲存體加密中的 AES 256 位元加密，且金鑰是由系統進行管理。 儲存體加密會一律啟用，且無法停用。
...
**指引**：使用 Azure Resource Graph 來查詢和探索訂用帳戶內的所有資源（包括適用於 MariaDB 的 Azure 資料庫伺服器）。 確保租用戶中有適當的 (讀取) 權限，且能列舉所有 Azure 訂用帳戶以及訂用帳戶中的資源。
...
**指引**.. 適當地使用標記、管理群組和個別的訂用帳戶來組織和追蹤適用於 MariaDB 的 Azure 資料庫伺服器和相關資源。 請定期調節清查，並確保會及時刪除訂用帳戶中未經授權的資源。
...
**指引**：使用 Azure 條件式存取，藉由設定「Microsoft Azure 管理」應用程式的「封鎖存取」，限制使用者與 Azure Resource Manager 互動的能力。 這可能會防止在高安全性環境（例如包含機密資訊的適用於 MariaDB 的 Azure 資料庫伺服器）中建立和變更資源。
...
**指導**方針：使用 "DBforMariaDB" 命名空間中的 Azure 原則別名來建立自訂原則，以警示、審查和強制執行系統組態。 此外，開發流程和管線以管理原則例外狀況。
...
**指引**：針對在用來存取適用於 MariaDB 的 Azure 資料庫伺服器的 Azure App Service 上執行的 Azure 虛擬機器或 web 應用程式，請搭配使用受控服務識別與 Azure Key Vault 以簡化和保護適用於 MariaDB 的 Azure 資料庫伺服器密碼管理。 確保已啟用 Key Vault 虛刪除。
...
針對在用來存取適用於 MariaDB 的 Azure 資料庫伺服器的 Azure App Service 上執行的 Azure 虛擬機器或 web 應用程式，請使用受控服務識別搭配 Azure Key Vault 來儲存和取出適用於 MariaDB 的 Azure 資料庫伺服器的認證。  確保已啟用 Key Vault 虛刪除。
...
**指引**：在適用於 MariaDB 的 Azure 資料庫中，從源伺服器的備份執行還原，以定期測試備份。 有兩種類型的還原可使用：時間點還原和異地還原。 時間點還原可搭配任一備份備援選項使用，並在原始伺服器所在區域中建立新的伺服器。 異地還原只能在將伺服器設定為使用異地備援儲存體時使用，這可供將伺服器還原至不同的區域。
