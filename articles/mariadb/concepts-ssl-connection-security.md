...
在某些情況下，應用程式需要從受信任的憑證授權單位單位（CA）憑證檔案產生的本機憑證檔案，才能安全地連接。 目前客戶**只能使用**預先定義的憑證來連接到位於的適用於 MariaDB 的 Azure 資料庫伺服器 https://www.digicert.com/CACerts/BaltimoreCyberTrustRoot.crt.pem 。 
