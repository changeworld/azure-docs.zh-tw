...
description: 瞭解 MariaDB Azure 資料庫的各種定價層,包括計算生成、儲存類型、儲存大小、vCore、記憶體和備份保留期。
author: jasonwhowell
ms.author: jasonh
ms.service: mariadb
ms.topic: conceptual
...
您可以在 Azure 入口網站或使用 Azure CLI 命令來監視 I/O 耗用量。 要監視的相關指標是[儲存限制、儲存百分比、使用的儲存和 IO 百分比](concepts-monitoring.md)。

### <a name="reaching-the-storage-limit"></a>到達儲存體限制

如果可用儲存小於預配儲存大小的 5%,則將標記為唯讀 100 GB 預配儲存的伺服器。 若伺服器中佈建的儲存體超過 100 GB，則只有在可用儲存空間小於 5 GB 時，伺服器所才會標示為唯讀。

例如,如果已預配了 110 GB 的存儲空間,並且實際利用率超過 105 GB,則伺服器將標記為唯讀。 或者,如果您預配了 5 GB 的存儲,則當可用儲存達到小於 256 MB 時,伺服器將標記為唯讀。

當服務嘗試讓伺服器變為唯讀時，會封鎖所有新的寫入交易要求，而現有的使用中交易會繼續執行。 當伺服器設為唯讀時，所有後續的寫入作業和交易認可都會失敗。 讀取查詢將會繼續運作，不會中斷。 當您增加佈建的儲存體之後，伺服器就可以再次接受寫入交易。

我們建議您打開儲存自動增長或設置警報,以便在伺服器存儲接近閾值時通知您,以便避免進入唯讀狀態。 如需詳細資訊，請參閱[如何設定警示](howto-alert-metric.md)的文件。

### <a name="storage-auto-grow"></a>儲存自動成長

存儲自動增長可防止伺服器耗盡存儲並變為唯讀。 如果啟用了存儲自動增長,則存儲會自動增長,而不會影響工作負載。 對於配置儲存小於 100 GB 的伺服器,當可用儲存低於預配儲存的 10% 時,預配儲存大小將增加 5 GB。 對於具有 100 GB 以上預配儲存的伺服器,當可用儲存空間低於預配儲存大小的 10 GB 時,預配儲存大小將增加 5%。 上述最大存儲限制適用。

例如,如果已預配了 1000 GB 的存儲,並且實際利用率超過 990 GB,則伺服器存儲大小將增加到 1050 GB。 或者,如果您預配了 10 GB 的存儲,則當少於 1 GB 的存儲空間可用時,儲存大小將增加到 15 GB。

請記住,存儲只能放大,不能縮小。

## <a name="backup"></a>Backup

服務會自動採用伺服器的備份。 您可以選擇 7 到 35 天的保留期。 通用和記憶體優化伺服器可以選擇具有用於備份的異地冗餘存儲。 在[概念文章中瞭解有關備份的更多內容](concepts-backup.md)。
