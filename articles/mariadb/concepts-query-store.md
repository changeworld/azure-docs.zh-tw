...
3. 搜尋 query_store_capture_mode 參數。
4. 將值設為 [ALL]，然後 **儲存**。
...
使用下列檢視和函式來檢視和管理查詢存放區。 [選取權限公用角色](howto-create-users.md#create-more-admin-users)中的任何人都可以使用這些檢視，查看查詢存放區中的資料。 這些檢視僅適用於 **mysql** 資料庫。

移除常值和常數之後，查看查詢結構，其會呈現標準化。 如果兩個查詢除了常值之外還相同，則會有相同的雜湊。
...
此檢視會傳回查詢存放區中的等候事件資料。 每個不同的資料庫識別碼、使用者識別碼、查詢識別碼和事件都有一個資料列。
