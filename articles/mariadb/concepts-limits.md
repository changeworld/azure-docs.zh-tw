...
您可以從 MySQL 命令列或 MySQL 工作臺之類`mysql.az_load_timezone`的工具呼叫預存程式來填入時區資料表。 請參閱[Azure 入口網站](howto-server-parameters.md#working-with-the-time-zone-parameter)或[Azure CLI](howto-configure-server-parameters-cli.md#working-with-the-time-zone-parameter)文章，以瞭解如何呼叫預存程式，以及設定全域或工作階段層級的時區。

## <a name="storage-engine-support"></a>儲存引擎支援

### <a name="supported"></a>支援
- [InnoDB](https://mariadb.com/kb/en/library/xtradb-and-innodb/) \(英文\)
- [快閃記憶體](https://mariadb.com/kb/en/library/memory-storage-engine/)
...
- [存檔](https://mariadb.com/kb/en/library/archive/)
...
- 超級許可權：同樣地，[超級許可權](https://mariadb.com/kb/en/library/grant/#global-privileges)也受到限制。
