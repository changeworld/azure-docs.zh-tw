---
title: 快速入門：建立自訂警示
description: 了解、建立及指派適用於 IoT 安全性服務的 Azure Defender 的自訂裝置警示。
...
對於徹底了解其裝置預期行為的客戶，適用於 IoT 的 Defender 可讓您將這方面的了解轉化為裝置行為原則，並對不在所預期正常行為內的任何偏差舉動發出警示。
...
適用於 IoT 的 Defender 提供了大量警示，您可以根據本身的特定需求加以自訂。 請檢閱[可自訂的警示資料表](concept-customizable-security-alerts.md)，以了解警示嚴重性、資料來源、描述，以及您在收到每個警示時可參考的建議補救步驟。
