...
ms.locfileid: "80057276"
---
# <a name="what-are-availability-zones-in-azure"></a>什麼是 Azure 中的可用性區域？
「可用性區域」是高可用性供應項目，可保護您的應用程式和資料不受資料中心故障影響。 「可用性區域」是 Azure 地區內獨特的實體位置。 每個區域皆由一或多個配備獨立電力、冷卻系統及網路的資料中心所組成。 若要確保復原，所有已啟用的地區中至少要有三個不同的區域。 地區內「可用性區域」的實體區隔可保護應用程式和資料不受資料中心故障影響。 區域備援服務會將應用程式和資料複寫至所有「可用性區域」，以防出現單一失敗點。 使用「可用性區域」時，Azure 可提供業界最佳的 99.99% VM 執行時間 SLA。 完整 [Azure SLA](https://azure.microsoft.com/support/legal/sla/virtual-machines/) 說明保證的 Azure 整體可用性。
...
- **分區服務**– 將資源固定到特定區域（例如，虛擬機器、託管磁片、標準 IP 位址）或
...
> [!IMPORTANT]
> 可用性區域識別碼（上圖中的數位 1、2 和 3）以邏輯方式映射到每個訂閱的實際物理區域。 這意味著給定訂閱中的可用性區域 1 可能引用與不同訂閱中的可用性區域 1 不同的物理區域。 因此，建議不要依賴不同訂閱中的可用區域 ID 進行虛擬機器放置。

## <a name="services-support-by-region"></a>按區域提供的服務支援
...
## <a name="services-resiliency"></a>服務恢復能力
所有 Azure 管理服務都設計為從區域級故障中具有彈性。 在故障範圍內，與整個區域故障相比，區域內的一個或多個可用性區域故障的故障半徑較小。 Azure 可以從區域或其他 Azure 區域中的管理服務的區域級故障中恢復。 Azure 在區域內一次執行一個區域的關鍵維護，以防止任何影響跨區域可用性區域部署的客戶資源的失敗。
...
- [創建虛擬機器](../virtual-machines/windows/create-portal-availability-zone.md)
...
- [SQL 資料庫](../sql-database/sql-database-high-availability.md#zone-redundant-configuration)
...
- [為 Azure 宇宙資料庫添加區域冗余區域](../cosmos-db/high-availability.md#availability-zone-support)
- [入門 Azure 緩存，用於雷瑞斯可用性區域](https://aka.ms/redis/az/getstarted)
- [建立 Azure Active Directory Domain Services 執行個體](../active-directory-domain-services/tutorial-create-instance.md)
- [創建使用可用性區域的 Azure 庫伯奈斯服務 （AKS） 群集](../aks/availability-zones.md)
