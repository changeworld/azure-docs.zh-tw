...
1. 執行 PUT Https 方法，將新資源新增 `ServiceRunner` 至實驗室，如下列範例所示。 服務執行器資源是用來管理和控制 DevTest Labs 中受控識別的 proxy 資源。 服務執行器名稱可以是任何有效的名稱，但建議您使用受控識別資源的名稱。 
