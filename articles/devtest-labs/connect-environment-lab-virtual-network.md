...
以下是連接實驗室子網的簡單環境範本。 在此範例中， `DTLSubnetId` 參數代表實驗室所在之子網的識別碼。 它會指派給： `$(LabSubnetId)` ，這會由 DevTest Labs 自動解析為實驗室子網的識別碼。 在此定義中，VM**網路介面**的**子網**屬性會設定為， `DTLSubnetId` 使其加入相同的子網。 
