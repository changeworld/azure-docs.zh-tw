...
description: 瞭解如何使用 Azure DevTest Labs，透過 Azure PowerShell 來建立和管理虛擬機器。
ms.topic: article
ms.date: 06/26/2020
...
ms.locfileid: "85484106"
---
# <a name="create-a-virtual-machine-with-devtest-labs-using-azure-powershell"></a>使用 Azure PowerShell 建立具有 DevTest Labs 的虛擬機器
本文說明如何使用 Azure PowerShell 在 Azure DevTest Labs 中建立虛擬機器。 您可以使用 PowerShell 腳本，在 Azure DevTest Labs 中自動建立實驗室中的虛擬機器。 

## <a name="prerequisites"></a>必要條件
開始之前：

- 如果您不想要使用現有的實驗室來測試本文中的腳本或命令，請[建立實驗室](devtest-lab-create-lab.md)。 
- [安裝 Azure PowerShell](/powershell/azure/install-az-ps?view=azps-1.7.0) 或使用整合至 Azure 入口網站的 Azure Cloud Shell。 

## <a name="powershell-script"></a>PowerShell 指令碼
本節中的範例腳本會使用 [AzResourceAction](/powershell/module/az.resources/invoke-azresourceaction?view=azps-1.7.0) Cmdlet。  此 Cmdlet 會採用實驗室的資源識別碼、執行 () 的動作名稱， `createEnvironment` 以及必要的參數來執行該動作。 這些參數是在雜湊表中，其中包含所有虛擬機器描述屬性。 
...
上述腳本中的虛擬機器屬性，可讓我們建立以 Windows Server 2016 DataCenter 作為 OS 的虛擬機器。 針對每一種類型的虛擬機器，這些屬性將會稍有不同。 [ [定義虛擬機器](#define-virtual-machine) ] 區段會說明如何判斷要在此腳本中使用的屬性。

下列命令提供執行檔案名中所儲存之腳本的範例： Create-LabVirtualMachine.ps1。 
...
本節說明如何取得您想要建立之虛擬機器類型的特定屬性。 

### <a name="use-azure-portal"></a>使用 Azure 入口網站
在 Azure 入口網站中建立 VM 時，您可以產生 Azure Resource Manager 範本。 您不需要完成建立 VM 的流程。 您只需要遵循這些步驟，直到看到範本為止。 如果您還沒有建立實驗室 VM，這是取得必要 JSON 描述的最佳方式。 

1. 瀏覽至 [Azure 入口網站](https://portal.azure.com)。
2. 選取左側導覽功能表上的 [ **所有服務** ]。
3. 從服務清單中搜尋並選取 **DevTest Labs** 。 
4. 在 [ **DevTest labs** ] 頁面的實驗室清單中，選取您的實驗室。
5. 在實驗室的首頁上，選取工具列上的 [ **+ 新增** ]。 
6. 選取 VM 的 **基底映射** 。 
...
在此範例中，您會瞭解如何取得 Azure marketplace 映射的定義。 您可以使用相同的方式取得自訂映射、公式或環境的定義。 新增虛擬機器所需的任何成品，並設定所需的任何 advanced 設定。 提供必要欄位的值和任何選用欄位之後，再選取 [ **自動化選項** ] 按鈕。

### <a name="use-azure-rest-api"></a>使用 Azure REST API
下列程式可讓您使用 REST API 取得映射屬性的步驟：這些步驟僅適用于實驗室中的現有 VM。 

1. 流覽至 [ [虛擬機器-清單](/rest/api/dtl/virtualmachines/list) ] 頁面，選取 [ **試試看** ] 按鈕。 
2. 選取您的 **Azure 訂用帳戶**。
3. 輸入 **實驗室的資源群組**。
4. 輸入 **實驗室的名稱**。 
5. 選取 [執行]****。
6. 您會看到 **映射的屬性** （VM 的建立來源）。 

## <a name="set-expiration-date"></a>設定到期日期
在訓練、示範和試用等案例中，您可能會想要建立虛擬機器，並在固定期間內自動刪除它們，以免產生不必要的成本。 您可以使用 PowerShell 建立 VM 的到期日，如範例 [powershell 腳本](#powershell-script) 一節所示。

以下是 PowerShell 腳本範例，可針對實驗室中的所有現有 Vm 設定到期日：
