...
建立 Azure 地圖服務帳戶之後，就會產生主要和次要金鑰。 當您 [使用共用金鑰驗證來呼叫 Azure 地圖服務](./azure-maps-authentication.md#shared-key-authentication)時，建議您使用主要金鑰作為訂用帳戶金鑰。 您可以在迴圈金鑰變更之類的案例中使用次要金鑰。 如需詳細資訊，請參閱 [Azure 地圖服務中的驗證](./azure-maps-authentication.md)。

### <a name="securely-store-shared-key"></a>安全地儲存共用金鑰

主要和次要金鑰允許對 Maps 帳戶的所有 Api 進行授權。 應用程式應該將金鑰儲存在安全的存放區（例如 Azure Key Vault）。 應用程式必須將共用金鑰取出為 Azure Key Vault 秘密，以避免在應用程式設定中將共用金鑰儲存為純文字。 若要瞭解如何設定 Azure Key Vault，請參閱 [Azure Key Vault 開發人員指南](../key-vault/general/developers-guide.md)。
...
> [!Tip]
> 如果應用程式裝載于 Azure 環境中，您應該執行受控識別，以降低管理秘密以驗證 Azure Key Vault 的成本和複雜度。 請參閱下列 Azure Key Vault [教學課程，以透過受控識別進行](../key-vault/general/tutorial-net-create-vault-azure-web-app.md)連線。
...
請參閱 [受控識別的總覽](../active-directory/managed-identities-azure-resources/overview.md) ，讓應用程式能夠存取受控識別。
...
5. 請完成下列步驟，以建立用戶端密碼或設定憑證。
    ...
    * 選取 [ **新增** ] 之後，複製秘密並安全地儲存在服務中，例如 Azure Key Vault。 請參閱 [Azure Key Vault 開發人員指南](../key-vault/general/developers-guide.md) ，以安全地儲存憑證或秘密。 您將使用此秘密從 Azure AD 取得權杖。
...
3. 選取內建的 Azure 地圖服務角色定義，例如 **Azure 地圖服務資料讀取器** 或 **Azure 地圖服務資料參與者** 。 在 [  。 選取主體。 然後選取 [儲存]。
...
針對裝載資源設定受控識別之後，請使用 Azure SDK 或 REST API 取得 Azure 地圖服務的權杖，請參閱 [取得存取權杖](../active-directory/managed-identities-azure-resources/how-to-use-vm-token.md)的詳細資料。 遵循本指南之後，預期會傳回存取權杖，可用於 REST API 要求。
...
如需詳細的範例，請參閱 [Azure AD 的驗證案例](../active-directory/develop/authentication-vs-authorization.md)。
