...
Read 函式的結果是 `SpatialDataSet` 物件。 此物件會擴充 GeoJSON FeatureCollection 類別。 它可以輕鬆地依原樣傳遞至 `DataSource`，以便在地圖上轉譯其功能。 `SpatialDataSet` 不僅包含功能資訊，還可能包含 KML 的基礎重迭、處理計量，以及下表中所述的其他詳細資料。
...
下列程式碼示範如何讀取簡單的空間資料集，並使用 `SimpleDataLayer` 類別在地圖上轉譯它。 程式碼會使用 URL 所指向的 .GPX 檔案。

<br/>

<iframe height='500' scrolling='no' title='簡單載入空間資料' src='//codepen.io/azuremaps/embed/yLNXrZx/?height=500&theme-id=0&default-tab=js,result&embed-version=2&editable=true' frameborder='no' allowtransparency='true' allowfullscreen='true' style='width: 100%;'>請參閱<a href='https://codepen.io'>CodePen</a>上的畫筆<a href='https://codepen.io/azuremaps/pen/yLNXrZx/'>載入空間資料 Simple</a> by Azure 地圖服務（<a href='https://codepen.io/azuremaps'>@azuremaps</a>）。
</iframe>
...
<iframe height='500' scrolling='no' title='將 KML 載入至對應' src='//codepen.io/azuremaps/embed/XWbgwxX/?height=500&theme-id=0&default-tab=js,result&embed-version=2&editable=true' frameborder='no' allowtransparency='true' allowfullscreen='true' style='width: 100%;'>請參閱<a href='https://codepen.io'>CodePen</a>上的 Azure 地圖服務（<a href='https://codepen.io/azuremaps'>@azuremaps</a>），將畫筆<a href='https://codepen.io/azuremaps/pen/XWbgwxX/'>載入 KML 至地圖</a>。
</iframe>

您可以選擇性地提供 proxy 服務，以存取可能未啟用 CORs 的跨網域資產。 這段程式碼顯示您可以提供 proxy 服務：
...
下列最後一個示範顯示如何讀取分隔的檔案，並將其轉譯在地圖上。 在此情況下，程式碼會使用具有空間資料行的 CSV 檔案。

<br/>

<iframe height='500' scrolling='no' title='新增分隔的檔案' src='//codepen.io/azuremaps/embed/ExjXBEb/?height=500&theme-id=0&default-tab=js,result&embed-version=2&editable=true' frameborder='no' allowtransparency='true' allowfullscreen='true' style='width: 100%;'>請參閱<a href='https://codepen.io'>CodePen</a>上的使用畫筆 Azure 地圖服務（<a href='https://codepen.io/azuremaps'>@azuremaps</a>）<a href='https://codepen.io/azuremaps/pen/ExjXBEb/'>新增分隔</a>的檔案。
</iframe>

## <a name="write-spatial-data"></a>寫入空間資料

空間 IO 模組中有兩個主要寫入功能。 `atlas.io.write` 函式會產生字串，而 `atlas.io.writeCompressed` 函數會產生壓縮的 zip 檔案。 壓縮的 zip 檔案會包含具有空間資料的文字型檔案。 這兩個函式都會傳回將資料新增至檔案的承諾。 而且，兩者都可以寫入下列任何資料： `SpatialDataSet`、`DataSource`、`ImageLayer`、`OgcMapLayer`、功能集合、功能、幾何，或這些資料類型的任何組合陣列。 使用其中一個函式撰寫時，您可以指定想要的檔案格式。 如果未指定檔案格式，則會將資料寫入為 KML。

下列工具示範可搭配 `atlas.io.write` 函式使用的大部分寫入選項。

<br/>

<iframe height='700' scrolling='no' title='空間資料寫入選項' src='//codepen.io/azuremaps/embed/YzXxXPG/?height=700&theme-id=0&default-tab=result&embed-version=2&editable=true' frameborder='no' allowtransparency='true' allowfullscreen='true' style='width: 100%;'>請參閱<a href='https://codepen.io'>CodePen</a>上的 Azure 地圖服務（<a href='https://codepen.io/azuremaps'>@azuremaps</a>）的畫筆<a href='https://codepen.io/azuremaps/pen/YzXxXPG/'>空間資料寫入選項</a>。
</iframe>
...
<iframe height='700' scrolling='no' title='將空間檔案拖放到對應上' src='//codepen.io/azuremaps/embed/zYGdGoO/?height=700&theme-id=0&default-tab=result&embed-version=2&editable=true' frameborder='no' allowtransparency='true' allowfullscreen='true' style='width: 100%;'>在<a href='https://codepen.io'>CodePen</a>上，請參閱將<a href='https://codepen.io/azuremaps/pen/zYGdGoO/'>空間檔案拖放到地圖</a>上的 Azure 地圖服務（<a href='https://codepen.io/azuremaps'>@azuremaps</a>）畫筆。
</iframe>
...
## <a name="read-and-write-well-known-text-wkt"></a>讀寫知名的文字（WKT）

[已知的文字](https://en.wikipedia.org/wiki/Well-known_text_representation_of_geometry)（WKT）是將空間幾何表示為文字的開放地理空間協會（OGC）標準。 許多地理空間系統都支援 WKT，例如使用 PostGIS 外掛程式的 Azure SQL 和 Azure 于 postgresql。 如同大部分的 OGC 標準，座標會格式化為「經度緯度」，以配合「x y」慣例。 例如，可使用 WKT 格式，將經度-110 和緯度45的點寫成 `POINT(-110 45)`。

已知的文字可以使用 `atlas.io.ogc.WKT.read` 函式來讀取，並使用 `atlas.io.ogc.WKT.write` 函數撰寫。

## <a name="examples-of-reading-and-writing-well-known-text-wkt"></a>讀取和寫入已知文字（WKT）的範例

下列程式碼示範如何讀取已知的文字字串 `POINT(-122.34009 47.60995)`，並使用反升圖層在地圖上呈現。

<br/>

<iframe height='500' scrolling='no' title='讀取知名文字' src='//codepen.io/azuremaps/embed/XWbabLd/?height=500&theme-id=0&default-tab=result&embed-version=2&editable=true' frameborder='no' allowtransparency='true' allowfullscreen='true' style='width: 100%;'>請在<a href='https://codepen.io'>CodePen</a>上 Azure 地圖服務（<a href='https://codepen.io/azuremaps'>@azuremaps</a>），查看手寫筆的<a href='https://codepen.io/azuremaps/pen/XWbabLd/'>已知文字</a>。
</iframe>
...
<iframe height='700' scrolling='no' title='讀寫知名的文字' src='//codepen.io/azuremaps/embed/JjdyYav/?height=700&theme-id=0&default-tab=result&embed-version=2&editable=true' frameborder='no' allowtransparency='true' allowfullscreen='true' style='width: 100%;'>請參閱<a href='https://codepen.io'>CodePen</a>上的「手寫筆<a href='https://codepen.io/azuremaps/pen/JjdyYav/'>讀取和寫入已知的文字</a>」 Azure 地圖服務（<a href='https://codepen.io/azuremaps'>@azuremaps</a>）。
</iframe>
...
- `isAxisOrderLonLat` 選項-座標 "緯度，經度" 或 "經度，緯度" 的軸順序可能會因資料集而異，而且不一定是妥善定義的。 根據預設，GML 讀取器會將座標資料讀取為「緯度，經度」，但將此選項設定為 true 會將其讀取為「經度，緯度」。
...
`atlas.io.read` 函式會在偵測到輸入資料為 XML 時，預設為 `atlas.io.core.GmlReader.read` 函式，但資料不是其他支援空間 XML 格式的其中一個。
