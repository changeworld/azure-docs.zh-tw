...
## <a name="how-to-draw-shapes"></a>如何繪製圖案
 ...
 在可以繪製任何圖形之前，請先`drawingMode`將 [繪圖管理員] 的選項設定為 [支援的繪圖] 設定。 這項設定可以進行程式設計，或按工具列上的其中一個 [繪製] 按鈕叫用。 即使在繪製圖形之後，繪圖模式仍會保持啟用狀態，讓您更輕鬆地繪製相同類型的其他圖形。 以程式設計方式將繪圖模式設定為閒置狀態。 或者，按一下工具列上的 [目前繪製模式] 按鈕，切換到閒置狀態。
