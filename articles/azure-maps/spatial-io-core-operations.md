...
- `getNextRow` 函式會讀取分隔資料集中的每一行文字，並傳回字串的陣列，代表該行資料集內的所有儲存格。 在處理下一個資料列之前，使用者可以處理資料列，並處置該資料列中任何不需要的記憶體。 因此，函式的記憶體效率較高。

根據預設，讀取器會使用逗號字元做為分隔符號。 不過，分隔符號可以變更為任何單一字元，或設定為 `'auto'`。 當設定為 [`'auto'`] 時，讀取器會分析字串中的第一行文字。 然後，它會從下表中選取最常見的字元來當做分隔符號使用。
...
`atlas.io.core.CsvWriter` 會將物件的陣列當做分隔字串寫入。 任何單一字元都可以用來做為分隔符號或文字辨識符號。 預設的分隔符號是逗號（`','`），而預設的文字辨識符號是引號（`'"'`）字元。

若要使用此類別，請遵循下列步驟：

- 建立類別的實例，並選擇性地設定自訂分隔符號或文字辨識符號。
- 使用 `write` 函數或 `writeRow` 函數，將資料寫入類別。 針對 `write` 函式，傳遞代表多個資料列和資料格的二維物件陣列。 若要使用 `writeRow` 函數，請傳遞物件的陣列，代表具有多個資料行的資料列。
- 呼叫 `toString` 函式，以取出分隔字串。 
...
在剖析 XML 檔案時，`atlas.io.core.SimpleXmlReader` 類別比 `DOMParser`更快。 不過，`atlas.io.core.SimpleXmlReader` 類別需要 XML 檔案的格式正確。 格式不正確的 XML 檔案（例如遺漏結束記號）可能會導致錯誤。
