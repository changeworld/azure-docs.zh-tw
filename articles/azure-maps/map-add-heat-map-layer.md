...
> [!TIP]
> 熱度圖圖層預設會轉譯資料來源中所有幾何的座標。 若要限制圖層，使其只呈現點幾何特徵，請`filter`將圖層的屬性`['==', ['geometry-type'], 'Point']`設為。 如果您也想要包含 MultiPoint 功能，請將圖`filter`層的屬性設為`['any', ['==', ['geometry-type'], 'Point'], ['==', ['geometry-type'], 'MultiPoint']]`。
...
若要將點的資料來源轉譯為熱度圖，請將資料來源傳遞至`HeatMapLayer`類別的實例，並將它加入至對應。
...
* `color`：指定如何以色彩標示熱度圖。 色彩漸層是熱度圖的常見功能。 您可以使用`interpolate`運算式來達到效果。 您也可以使用`step`運算式來上色熱度圖，以視覺化方式將密度分解成類似等高線或雷達圖樣式地圖的範圍。 這些色彩調色盤會定義最小密度值到最大密度值的顏色。 

  您可以指定熱度圖的色彩值做為`heatmap-density`值的運算式。 沒有資料的區域色彩定義于「插補」運算式的索引0，或「階」運算式的預設色彩。 您可以使用這個值來定義背景色彩。 這個值通常會設定為透明或半透明的黑色。 
...
調整半徑，使其每個縮放層級加倍時，會建立一個在所有縮放層級上看起來一致的熱度圖。 若要套用此調整， `zoom`請使用搭配基`exponential interpolation`底2運算式，並針對最小縮放層級設定圖元半徑，並針對計算`2 * Math.pow(2, minZoom - maxZoom)`的最大縮放層級調整半徑，如下列範例所示。 縮放地圖以查看熱度圖如何隨著縮放層級調整。
