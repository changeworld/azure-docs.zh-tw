...
description: 與 Azure 地圖服務、位置型服務和 GIS 相關之常用詞彙的詞彙。
author: rbrundritt
ms.author: richbrun
ms.date: 09/18/2018
ms.topic: conceptual
...
下列清單描述與 Azure 地圖服務服務搭配使用的常見字組。

## <a name="a"></a>A

<a name="address-validation"></a> **Address validation** \(地址驗證\)：驗證地址是否存在的程序。

<a name="advanced-routing"></a>**Advanced routing**：使用道路路線資料執行前進作業的服務集合;例如，計算可到達的範圍（等時線）、距離矩陣，以及批次路由要求。

<a name="aerial-imagery"></a> **Aerial imagery** \(空照圖影像\)：請參閱[衛星影像](#satellite-imagery)。 

<a name="along-a-route-search"></a>**沿著路由搜尋**：在指定的繞道時間內尋找資料的空間查詢，或與路由路徑的距離。

<a name="altitude"></a> **Altitude** \(高度\)：位於參考平面上之某點的高度或垂直相對高度。 高度的測量是以指定的參考數據為基礎，例如平均海平面。 請參閱＜相對高度＞。

<a name="ambiguous"></a> **Ambiguous** \(不明確\)：資料分類中的不確定狀態，適用於針對指定屬性可以適當地為物件指派兩個或以上之值的情況。 例如，當地理編碼「CA」時，會傳回兩個不明確的結果：「加拿大」和「加州」。 「CA」是國家/地區和州碼，分別代表「加拿大」和「加州」。 

<a name="annotation"></a> **Annotation** \(註釋\)：顯示在地圖上以為使用者提供資訊的文字或圖形。 註釋可以識別或描述特定的地圖實體、提供地圖上某個區域的一般資訊，或是提供有關地圖本身的資訊。

<a name="antimeridian"></a>**Antimeridian**：也稱為 180<sup>th</sup>經線。 這是-180 度和180度的經度符合點。 這與地球上的質數經線相反。
...
<a name="bearing"></a> **Bearing** \(方位\)：某點與另一個點之間的相對水平方向。 這會表示為相對於北部的角度，以順時針方向從0度到360度。 
...
<a name="camera"></a> **Camera** \(觀景窗\)：以互動式地圖控制的內容來說，觀景窗會定義地圖的視野。 相機的視口是根據數個地圖參數來決定：置中、縮放層級、音調、軸承。 

<a name="centroid"></a> **Centroid** \(距心\)：特徵的幾何中心。 線條的距心將會是該線條的中間點，而多邊形的距心則會是其區域的中心點。

<a name="choropleth-map"></a>**分級著色圖地圖**：主題地圖，其中的區域會與統計變數的測量比例成正比。 此統計變數會顯示在地圖上。 例如，依據每個美國州份相對於其他州的人口，以各種色彩標示其界限。
...
<a name="elevation"></a>提高**許可權**：某個點的垂直距離，或高於或低於參考介面或 datum 的物件。 一般而言，參考介面是平均海洋層級。 相對高度通常指的是土地的垂直高度。

<a name="envelope"></a> **Envelope** \(外殼\)：請參閱[週框方塊](#bounding-box)。

<a name="extended-postal-code"></a> **Extended postal code** \(擴充郵遞區號\)：可能包含額外資訊的郵遞區號。 例如，在美國，郵遞區號有五位數。 但擴充郵遞區號（也稱為 zip + 4）包含四個額外的數位。 這些額外的數位是用來識別五位數傳遞區域內的地理區段，例如 city 組塊、公寓群組或郵局盒。 瞭解地理區段有助於有效率地進行郵件排序和傳遞。
...
<a name="fleet-management"></a> **Fleet management** \(車隊管理\)：商用車輛 (例如汽車、卡車、船隻及飛機) 的管理。 車隊管理可以包含數種功能，例如動產融資、維護、車載資通訊 (追蹤與診斷)，以及駕駛員、速度、燃料及健康與安全管理。 車隊管理是一種由依賴業務運輸的公司所使用的流程。 公司想要將風險降至最低，並降低其整體運輸和員工成本，同時確保符合政府法規。
...
<a name="landsat"></a>**Landsat**： Multispectral，地球軌道由 NASA 所開發的衛星，可收集土地的影像。 這種影像用於許多產業，例如農業、林業和一套。
...
<a name="multipolygon"></a> **MultiPolygon**：代表 Polygon 物件集合的幾何。 例如，若要顯示夏威夷的界限，每個島都會以多邊形框住。 因此，夏威夷的界限會是 MultiPolygon。
...
<a name="postal-code"></a>**郵遞區號**：一系列以特定格式的字母或數位（或兩者）。 國家/地區的郵政服務會使用郵遞區號來將地理區域分割成區域，以簡化郵件的傳遞。

<a name="primary-key"></a>**主要金鑰**：為 Azure 地圖服務共用金鑰驗證所提供的兩個訂用帳戶金鑰中的第一個。 請參閱[共用金鑰驗證](#shared-key-authentication)。

<a name="prime-meridian"></a> **Prime meridian** \(本初子午線\)：代表經度 0 度的經度線。 一般來說，在 westerly 方向上移動時，經度值會減少到180度，而在 easterly 方向到-180-度時則會增加。 
...
<a name="quadtree"></a> **Quadtree** \(四元樹\)：其中的每個節點都具有四個子系的資料結構。 Azure 地圖服務中使用的磚系統會使用 quadtree 結構，如此一來，當使用者放大一個層級時，每個地圖底圖就會分成四個 subtiles。  如需詳細資訊，請參閱[縮放層級和圖格格線](zoom-levels-and-tile-grid.md)文件。
...
<a name="rest-service"></a> **REST service** \(REST 服務\)："Representational State Transfer" \(具象狀態傳輸\) 的縮寫。 用來在非集中式分散環境中的對等之間交換資訊的架構。 REST 可讓不同電腦上的程式與作業系統或平臺分開通訊。 服務可以將超文字傳輸通訊協定（HTTP）要求傳送至統一資源定位器（URL），並取回資料。
...
<a name="spatial-reference"></a> **Spatial reference** \(空間參考\)：以座標為基礎的當地、區域或全球系統，用來精確地找出地理實體。 它能定義用來將地圖座標與真實世界位置相關聯的座標系統。 空間參考可確保來自不同層或來源的空間資料可以進行整合，以進行精確的觀看或分析。 Azure 地圖服務會使用 [EPSG:3857](https://epsg.io/3857) \(英文\) 座標參考系統，並針對輸入幾何資料使用 WGS 84。
...
<a name="synchronous-request"></a> **Synchronous request** \(同步要求\)：會開啟連線並等候回應的 HTTP 要求。 瀏覽器會限制可從某個頁面提出之並行 HTTP 要求的數目。 如果同時進行多個長時間執行的同步要求，則可以達到此限制。 要求將會延遲，直到其中一個其他要求完成為止。
...
<a name="thematic-maps"></a> **Thematic maps** \(主題地圖\)：主題地圖是用來反映關於某地理區域之某個主題的簡易地圖。 這種對應類型的常見案例是根據某些資料計量，將系統管理區域（例如國家/地區）設為色彩。

<a name="tile-layer"></a> **Tile layer** \(圖格圖層\)：透過將地圖圖格 (矩形區段) 組合成連續圖層來顯示的圖層。 圖格可能是點陣影像圖格或向量圖格。 通常會預先轉譯點陣磚圖層，並將它們儲存為伺服器上的影像。 點陣磚圖層可能會使用大型的儲存空間。 向量磚圖層會在用戶端應用程式中以近乎即時的方式呈現。 因此，向量磚圖層的伺服器端儲存需求較小。

<a name="time-zone"></a> **Time zone** \(時區\)：地球上的某個區域，基於法律、商業和社會目的而遵循統一的標準時間。 時區通常會遵循國家/地區及其子分割的界限。

<a name="transaction"></a> **Transaction** \(交易\)：Azure 地圖服務會使用交易式授權模型，其中：

- 每要求 15 個地圖或流量圖格，便會建立一個交易。
- Azure 地圖服務中的其中一個服務，會針對每個 API 呼叫建立一個交易。 搜尋和路由是 Azure 地圖服務服務的範例。
...
<a name="trilateration"></a>**Trilateration**：測量所有三個點之間的距離，以判斷地球表面上某個點的位置（關於兩個其他點）。
...
<a name="vehicle-routing-problem-vrp"></a>**車輛路由問題（VRP）**：一類問題，其中會計算一組車輛的已排序路由，同時將其視為一組條件約束。 這些條件約束可能包括傳遞時間視窗、多個路由容量和行進持續時間限制。
...
<a name="waypoint-optimization"></a> **Waypoint optimization** \(導航點最佳化\)：對一系列導航點進行重新排序的程序，以最小化通過所有所提供之導航點所需的旅行時間或距離。 視優化的複雜度而定，這項優化通常稱為「[旅遊 Salesmen problem 問題](#traveling-salesmen-problem-tsp)」或「[車輛路線](#vehicle-routing-problem-vrp)」的問題。
...
<a name="web-mercator"></a>**Web Mercator**：也稱為球形 Mercator 投射。 這是 Mercator 投射的輕微變化，主要用於 Web 架構的對應程式。 它所使用的公式，與標準麥卡托投影用於較小規模地圖的公式相同。 不過，Web Mercator 會使用所有規模的球面公式，但大規模的 Mercator 地圖通常會使用投影的橢圓體形式。 這項差異是以全球規模 imperceptible，但它會使當地區域的對應與真正的橢圓體 Mercator 對應有些許的偏差。
...
<a name="Zoom level"></a> **Zoom level** \(縮放層級\)：指定詳細資料的層級，以及地圖的可見程度。 將所有方法縮放至層級0時，會經常顯示完整的世界地圖。 但是，地圖會顯示有限的詳細資料，例如國家/地區名稱、框線和海洋名稱。 放大到層級 17 時，地圖將會顯示涵蓋數個城市街區的區域，並提供詳細的道路資訊。 在 azure 地圖服務中，最高的縮放層級為22。 如需詳細資訊，請參閱[縮放層級和圖格格線](zoom-levels-and-tile-grid.md)檔。
