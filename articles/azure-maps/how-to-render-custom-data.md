...
ms.locfileid: "96906075"
---
# <a name="render-custom-data-on-a-raster-map"></a>在點陣地圖上轉譯自訂資料

本文說明如何使用 [靜態映射服務](/rest/api/maps/render/getmapimage)（具有影像組合功能），以允許在地圖上進行重迭。 影像組合可讓您使用其他資料（例如自訂圖釘、標籤和幾何重迭）來重新顯示點陣磚。

若要呈現自訂圖釘、標籤和幾何重迭，您可以使用 Postman 應用程式。 您可以使用 Azure 地圖服務 [資料服務 api](/rest/api/maps/data) 來儲存和轉譯重迭。
...
您也可以使用 [資料上傳 API](/rest/api/maps/data/uploadpreview)來取得路徑和 pin 位置資訊。 遵循下列步驟來上傳路徑和圖釘資料。
...
您可以使用樣式修飾詞搭配[路徑參數](/rest/api/maps/render/getmapimage#uri-parameters)來修改多邊形的外觀。
...
您可以藉由新增樣式修飾詞來修改 pin 的外觀。 例如，若要使圖釘和其標籤變大或較小，請使用 `sc` "scale style" 修飾詞。 此修飾詞會採用大於零的值。 1 的值是標準比例。 大於 1 的值會使圖釘變大，小於 1 的值會使它們變小。 如需樣式修飾詞的詳細資訊，請參閱 [靜態映射服務路徑參數](/rest/api/maps/render/getmapimage#uri-parameters)。
...
* 探索 [Azure 地圖服務 Get Map Image API](/rest/api/maps/render/getmapimage) \(英文\) 文件。
* 若要深入瞭解 Azure 地圖服務資料服務 (Preview) ，請參閱 [服務檔](/rest/api/maps/data)。