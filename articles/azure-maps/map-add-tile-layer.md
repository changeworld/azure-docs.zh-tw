...
圖格圖層會在伺服器的磚中載入。 這些映射可以預先轉譯或以動態方式呈現。 預先轉譯的影像會以圖格圖層瞭解的命名慣例，與伺服器上的任何其他影像儲存起來一樣。 動態轉譯的影像會使用服務，以接近即時的方式載入映射。 Azure 地圖服務的 [>tilelayer](/javascript/api/azure-maps-control/atlas.layer.tilelayer) 類別支援三種不同的磚服務命名慣例： 
...
> [!TIP]
> [TileLayer](/javascript/api/azure-maps-control/atlas.layer.tilelayer) 是視覺化地圖上大型資料集的好方法。 您不僅可以從影像產生圖格圖層，也可以將向量資料轉譯為圖格圖層。 藉由將向量資料轉譯為圖格圖層，地圖控制項只需要載入比它們所代表的向量資料小的檔案大小的圖格。 這項技術通常用來在地圖上呈現數百萬個數據列的資料。
