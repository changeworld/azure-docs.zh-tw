...
description: 如何設定無瀏覽器的應用程式，以支援登入 Azure AD 和呼叫 Azure 地圖服務 REST Api。
author: anastasia-ms
ms.author: v-stharr
ms.date: 06/12/2020
ms.topic: how-to
...
本指南將討論如何保護無法安全地儲存秘密或接受瀏覽器輸入的公用應用程式或裝置。 這些類型的應用程式屬於 IoT 或物聯網的類別。 這些應用程式的一些範例可能包括：智慧型電視裝置或發出應用程式的感應器資料。 
...
以下是使用中心點和半徑上傳表示為圓形幾何之簡單地理柵欄的範例要求主體。
