...
description: 瞭解如何在 Azure 地圖服務中設定縮放層級。 瞭解如何將地理座標轉換成圖元座標、圖格座標和 quadkeys。 查看程式碼範例。
author: anastasia-ms
ms.author: v-stharr
ms.date: 07/14/2020
ms.topic: conceptual
...
ms.locfileid: "92913886"
---
# <a name="zoom-levels-and-tile-grid"></a>縮放層級和圖格格線

Azure 地圖服務使用麥卡托圓球投影座標系統 (EPSG：3857)。 投射是用來將球形球形轉換成平面地圖的數學模型。 球形 Mercator 投射會在兩極伸展地圖，以建立正方形圖。 這項投射會大幅扭曲地圖的尺規和區域，但是有兩個重要的屬性超過此扭曲：

- 這是蘭伯特正投射，這表示它會保留相對較小物件的形狀。 在顯示空拍影像時，保留小型物件的形狀特別重要。 例如，我們想要避免將建築物的形狀扭曲。 正方形建築應該會顯示為正方形，而不是矩形。
- 它是一種圓柱投射。 北和南一律為向上和向下，而 west 和東一律為左方和右邊。 

為了將地圖抓取和顯示的效能優化，地圖會分成正方形磚。 Azure 地圖服務 SDK 的磚使用大小為 512 x 512 圖元的磚，以及適用于衛星影像的較小 256 x 256 圖元。 Azure 地圖服務為23個縮放層級（編號為0到22）提供點陣和向量圖格。 在縮放層級 0，整個世界剛好放進單一圖格裡：

:::image type="content" source="./media/zoom-levels-and-tile-grid/world0.png" alt-text="世界地圖磚":::

縮放層級 1 使用四個圖格來呈現世界：2 x 2 個正方形

:::image type="content" source="./media/zoom-levels-and-tile-grid/map-2x2-tile-layout.png" alt-text="世界地圖磚":::

每個額外的縮放層級會將前一個圖格的圖格分成四個圖格，以建立 2<sup>縮放</sup>x<sup>2 縮放的方格。</sup> 縮放層級 22 是 格線 2<sup>22</sup> x 2<sup>22</sup>，或 4,194,304 x 4,194,304 個圖格 (總計 17,592,186,044,416 個圖格)。

Web 和 Android Azure 地圖服務的互動式地圖控制項支援25個縮放層級，編號0到24。 雖然只有當磚可用時，才會在的縮放層級提供道路資料。

下表提供縮放層級值的完整清單，其中磚大小為512圖元正方形（位於緯度0）：
...
在每個縮放層級上選擇要使用的投射和調整，我們可以將地理座標轉換成圖元座標。 特定縮放層級之世界地圖影像的全圖元寬度和高度的計算方式如下：
...
給定緯度和經度的角度和詳細資料層級，圖元 XY 座標的計算方式如下：
...
緯度和經度值會假設在 WGS 84 基準上。 雖然 Azure 地圖服務使用球形投影，但請務必將所有地理座標轉換成通用的基準。 WGS 84 是選取的基準。 經度值會假設為從-180 度到 + 180 度的範圍，而且緯度值必須裁剪成從-85.05112878 到85.05112878 的範圍。 遵循這些值可避免兩極的 singularity，並確保投射的地圖是正方形圖形。

## <a name="tile-coordinates"></a>磚座標

為了將地圖抓取和顯示的效能優化，轉譯的地圖會剪下為磚。 每個縮放層級的圖元數目和圖格數目不同：
...
假設有一組圖元的 XY 座標，您就可以輕鬆地判斷包含該圖元之磚的圖格 XY 座標：
...
一旦決定縮放層級，就可以計算 x 和 y 值。 每個縮放格線的左上圖格是 x = 0、y = 0;右下方的磚位於 x = 2<sup>zoom-1</sup>、y = 2<sup>zoom-1</sup>。

以下是縮放層級 1 的縮放格線：

:::image type="content" border="false" source="./media/zoom-levels-and-tile-grid/api_x_y.png" alt-text="世界地圖磚":::

## <a name="quadkey-indices"></a>Quadkey 索引

某些對應平臺會使用 `quadkey` 索引命名慣例，將磚 ZY 座標合併為單一維度的字串，稱為索引 `quadtree` 鍵或 `quadkeys` 簡短。 每個都能 `quadkey` 唯一識別特定詳細資料層級上的單一磚，並且可以用來做為一般資料庫 B 型樹狀目錄索引中的索引鍵。 `quadkey`除了[新增圖格圖層](map-add-tile-layer.md)檔中所述的其他命名慣例之外，Azure 地圖服務 sdk 還支援覆迭使用命名慣例的磚圖層。

> [!NOTE]
> `quadkeys`命名慣例僅適用于一或多個縮放層級。 Azure 地圖服務 SDK 的支援縮放層級0，這是整個世界的單一地圖磚。 

若要將磚座標轉換成 `quadkey` ，Y 和 X 座標的位是交錯的，且結果會轉譯為以4為基底的數位 (前置零會維持) 並轉換成字串。 比方說，假設圖格的 XY 座標為 (3，5) 在層級3，則 `quadkey` 決定如下：
...
`Qquadkeys` 有幾個有趣的屬性。 首先， (位數的長度 `quadkey`) 等於對應磚的縮放層級。 其次，任何磚的會以 `quadkey` `quadkey` 其父磚的開頭， (上一個層級) 包含的磚。 如下列範例所示，磚2是磚20到23的父系：

:::image type="content" border="false" source="./media/zoom-levels-and-tile-grid/quadkey-tile-pyramid.png" alt-text="世界地圖磚":::

最後， `quadkeys` 提供一維索引鍵，通常會在 XY 空間中保留磚的相近。 換言之，兩個具有鄰近 XY 座標的圖格通常 `quadkeys` 會有相當接近的圖格。 這對優化資料庫效能很重要，因為連續的磚通常會在群組中要求，且最好將這些磚保留在相同的磁片區上，以便將磁片讀取次數降至最低。

## <a name="tile-math-source-code"></a>磚數學來源程式碼

下列範例程式碼說明如何執行本檔中所述的函式。 您可以視需要輕鬆地將這些函式轉譯成其他程式設計語言。
