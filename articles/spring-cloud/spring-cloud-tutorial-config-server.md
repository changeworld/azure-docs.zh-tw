...
3. 在左側流覽窗格中選取 [ **Config Server** ]。

4. 在 [預設存放庫] 區段中，將 [URI] 設定為 [https://github.com/Azure-Samples/piggymetrics-config ]。

5. 按一下 **[驗證]** 。

    ![流覽至 config server](media/spring-cloud-quickstart-launch-app-portal/portal-config.png)

6. 驗證完成時，按一下 [套用] 以儲存 **您的變更** 。

    ![正在驗證 config server](media/spring-cloud-quickstart-launch-app-portal/validate-complete.png)

7. 更新設定可能需要幾分鐘的時間。
 
    ![正在更新 config server](media/spring-cloud-quickstart-launch-app-portal/updating-config.png) 
