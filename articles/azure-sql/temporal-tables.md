...
ms.locfileid: "85985477"
---
# <a name="getting-started-with-temporal-tables-in-azure-sql-database-and-azure-sql-managed-instance"></a>Azure SQL Database 和 Azure SQL 受控執行個體中的時態表入門
[!INCLUDE[appliesto-sqldb-sqlmi](includes/appliesto-sqldb-sqlmi.md)]

時態表是 Azure SQL Database 和 Azure SQL 受控執行個體的可程式性功能，可讓您追蹤和分析資料中變更的完整歷程記錄，而不需要自訂程式碼。 時態表會將資料與時間內容緊密相關，讓儲存的事實只能在特定期間內解讀為有效。 時態表的這個屬性可讓您以有效率的時間為基礎進行分析，並從資料演進取得見解。

## <a name="temporal-scenario"></a>時態性案例

本文說明在應用程式案例中使用時態表的步驟。 假設您想要從頭開始追蹤正在開發的新網站上的使用者活動，或您想要使用使用者活動分析擴充的現有網站上的使用者活動。 在這個簡化的範例中，我們假設在一段時間內流覽過的網頁數目是一個指標，必須在裝載于 Azure SQL Database 或 Azure SQL 受控執行個體的網站資料庫中進行攔截和監視。 使用者活動歷史分析的目標是要獲得重新設計網站的意見，並為訪客提供更好的經驗。
...
幸運的是，您不需要在您的 app 上花太多精力，就可以維護此活動資訊。 使用時態表，此程式會自動化，讓您在網站設計期間擁有完整的彈性，以及更多時間專注于資料分析。 您只需要確保將 **WebSiteInfo** 資料表設定為 [時態系統設定版本](/sql/relational-databases/tables/temporal-tables#what-is-a-system-versioned-temporal-table)。 在此案例中使用時態表的確切步驟如下所述。

## <a name="step-1-configure-tables-as-temporal"></a>步驟 1︰將資料表設定為時態表

根據您要開始新的開發工作，還是升級現有的應用程式，您將會建立時態表，或透過新增時態屬性來修改現有的資料表。 在一般情況下，您的案例可能會混用這兩個選項。 請使用[SQL Server Management Studio](/sql/ssms/download-sql-server-management-studio-ssms) （SSMS）、 [SQL Server Data Tools](/sql/ssdt/download-sql-server-data-tools-ssdt) （SSDT）、 [Azure Data Studio](/sql/azure-data-studio/download-azure-data-studio)或任何其他 transact-sql 開發工具來執行這些動作。
...
在此特殊案例中，我們的目標是針對一段較長的資料歷程記錄以及較大的資料集，執行以時間為基礎的趨勢分析，因此歷程記錄表格的儲存體選擇為叢集資料行存放區索引。 叢集資料行存放區為分析查詢提供很好的壓縮和效能。 時態表可讓您彈性地在目前和時態表上完全獨立設定索引。

> [!NOTE]
> 資料行存放區索引可在「商務關鍵性」、「一般用途」和「高階」層級，以及在「標準」層、S3 和更新版本中
...
時態表在物件總管中會以特定的圖示來表示，以方便識別，而其記錄資料表會顯示為子節點。
...
時態表的主要優點是您不需要以任何方式變更或調整您的網站來執行變更追蹤。 一旦建立之後，每次您對資料進行修改時，時態表就會以透明方式保存先前的資料列版本。
...
請務必注意，更新查詢不需要知道實際作業進行的時間，也不需要知道如何保留歷史資料以供未來分析之用。 Azure SQL Database 和 Azure SQL 受控執行個體會自動處理這兩個層面。 下圖說明如何在每次更新時產生歷程記錄資料。
...
一般而言，您必須在開發 app 的同時，變更時態表結構描述。 為此，只要執行一般的 ALTER TABLE 語句，Azure SQL Database 或 Azure SQL 受控執行個體適當地將變更傳播到歷程記錄資料表。 下列指令碼示範如何新增要追蹤的其他屬性︰
...
透過系統設定版本的時態表，歷程記錄資料表可以將資料庫大小增加到超過一般資料表。 對於不斷成長的大型歷程記錄資料表來說，單是儲存成本和加諸於時態查詢之上的效能負擔就可能會造成問題。 因此，開發資料保留原則來管理歷程記錄資料表中的資料是規劃及管理每個時態表生命週期的重要環節。 透過 Azure SQL Database 和 Azure SQL 受控執行個體，您可以使用下列方法來管理時態表中的歷程記錄資料：
...
- 如需有關時態表的詳細資訊，請參閱簽出[時態表](/sql/relational-databases/tables/temporal-tables)。
