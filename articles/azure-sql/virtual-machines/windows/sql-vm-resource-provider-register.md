...
description: 向 SQL VM 資源提供者註冊 Azure SQL Server 虛擬機器，以啟用部署在 Azure Marketplace 外 SQL Server VM 的功能、實現合規性，以及改良管理能力。
...
ms.locfileid: "84039829"
---
# <a name="register-a-sql-server-virtual-machine-in-azure-with-the-sql-vm-resource-provider"></a>向 SQL VM 資源提供者註冊 Azure 中的 SQL Server 虛擬機器
[!INCLUDE[appliesto-sqlvm](../../includes/appliesto-sqlvm.md)]

這篇文章說明如何向 SQL VM 資源提供者註冊 Azure 中的 SQL Server 虛擬機器 (VM)。 向資源提供者註冊會在訂用帳戶中建立 **SQL 虛擬機器**_資源_，這是與虛擬機器資源不同的資源。 向資源提供者取消註冊 SQL Server VM 會移除 **SQL 虛擬機器**的_資源_ ，但不會卸載實際的虛擬機器。 
...
- **免費管理**：在這三種管理模式下向 SQL VM 資源提供者註冊完全免費。 與資源提供者或變更管理模式都不會產生任何額外費用。 
...
若要利用 SQL VM 資源提供者，您必須先[向資源提供者註冊訂用帳戶](#register-subscription-with-rp)，讓資源提供者能夠在該特定訂用帳戶內建立資源。
...
如果 VM 上未安裝[SQL Server IaaS 代理程式擴充功能](sql-server-iaas-agent-extension-automate-management.md)，建議您在輕量模式下向 SQL VM 資源提供者註冊。 這會以[輕量模式](#management-modes)安裝 SQL IaaS 擴充功能，並防止 SQL Server 服務重新啟動。 然後，您可以隨時升級至完整模式，但是這麼做會重新啟動 SQL Server 服務，因此建議您等到排程的維護時段。 
...
容錯移轉叢集執行個體和多重執行個體部署只能以輕量模式向 SQL VM 資源提供者註冊。 
...
如果已手動將 SQL IaaS 擴充功能安裝至 VM，則可以在完整模式中註冊 SQL Server VM，而不需要重新啟動 SQL Server 服務。 **不過，如果尚未安裝 SQL IaaS 擴充功能，以完整模式註冊會以完整模式安裝 SQL IaaS 擴充功能，然後重新啟動 SQL Server 服務。請謹慎執行。**
...
使用 Azure CLI 在 NoAgent 模式中註冊 SQL Server 2008 VM： 
...
使用 Azure CLI 在 NoAgent 模式中註冊 SQL Server 2008 R2 VM： 
...
已安裝*輕量* IaaS 擴充功能 SQL Server VM 可以使用 Azure 入口網站、Azure CLI 或 PowerShell，升級為_完整_模式。 在 OS 升級為 Windows 2008 R2 和更新版本後，_NoAgent_ 模式中的 SQL Server VM 就可以升級為_完整_模式。 無法降級 - 若要執行此操作，您需要從 SQL VM 資源提供者[取消註冊](#unregister-vm-from-rp) SQL Server VM。 這麼做會移除 **SQL 虛擬機器**_資源_，但不會刪除實際的虛擬機器。 
...
1. 登入 [Azure 入口網站](https://portal.azure.com)。 
1. 前往您的 [SQL Server 虛擬機器](manage-sql-vm-portal.md)
...
如果要向 SQL VM 資源提供者取消註冊您的 SQL Server VM，請使用 Azure 入口網站或 Azure CLI 刪除 SQL 虛擬機器*資源*。 刪除 SQL 虛擬機器*資源* 不會刪除 SQL Server VM。 不過，請謹慎使用並遵循步驟，因為在嘗試移除 *資源*時，可能會不小心刪除虛擬機器。 
...
1. 輸入 SQL 虛擬機器的名稱，然後**清除虛擬機器旁的核取方塊**。
...
1. 選取 [刪除]，以確認刪除 SQL 虛擬機器*資源*，而不是 SQL Server 虛擬機器。 

### <a name="command-line"></a>命令列

# <a name="azure-cli"></a>[Azure CLI](#tab/azure-cli)
若要使用 Azure CLI 從資源提供者取消註冊 SQL Server 虛擬機器，請使用 [az SQL VM delete](/cli/azure/sql/vm?view=azure-cli-latest#az-sql-vm-delete) 命令。 這樣做會移除 SQL Server 虛擬機器*資源*，但不會刪除虛擬機器。 
...
# <a name="powershell"></a>[PowerShell](#tab/azure-powershell)
若要使用 Azure CLI 從資源提供者取消註冊您的 SQL Server 虛擬機器，請使用 [AzSqlVM](/powershell/module/az.sqlvirtualmachine/new-azsqlvm)命令。 這樣做會移除 SQL Server 虛擬機器*資源*，但不會刪除虛擬機器。 
...
如果 SQL Server VM 是自行安裝，而不是從 Azure Marketplace 中的 SQL Server 映像佈建，則應該向 SQL VM 資源提供者註冊，即使已安裝 SQL Server IaaS 延伸模組也一樣。 向 SQL VM 資源提供者註冊會建立 Microsoft.SqlVirtualMachines 類型的新資源。 安裝 SQL Server IaaS 擴充功能並不會建立該資源。
...
**向 SQL Server VM 資源提供者註冊會在 VM 上重新啟動 SQL Server 嗎？**
...
若要變更完整管理模式，請卸除 SQL Server *資源*以[取消註冊](#unregister-vm-from-rp) SQL Server 虛擬機器，並以不同的管理模式向 SQL VM 資源提供者重新註冊 SQL Server VM。
