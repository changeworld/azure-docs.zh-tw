...
description: 了解如何連線到 Azure 上的 SQL Server 虛擬機器。 本主題使用傳統部署模型。 案例會視網路組態和用戶端的位置而有所不同。
...
本主題說明如何連線到 Azure 虛擬機器 (VM) 上的 SQL。 它涵蓋了部分[一般連線能力案例](#connection-scenarios)，然後提供[在入口網站中變更連線設定的步驟](#change)。 如果您需要疑難排解或設定入口網站以外的連線，請參閱本主題結尾的[手動設定](#manual)。 

如需有關佈建和連線能力的完整逐步解說，請參閱[在 Azure 上佈建 SQL Server 虛擬機器](create-sql-vm-portal.md)。
...
![佈建期間的公用 SQL 連線能力選項](./media/ways-to-connect-to-sql/sql-vm-portal-connectivity.png)
...
> [!NOTE]
> 當您透過網際網路查詢 VM 上的 SQL Server 時，所有從 Azure 資料中心傳出的資料都會以一般[輸出資料傳輸價格](https://azure.microsoft.com/pricing/details/data-transfers/)計費。
...
私用連線能力通常與[虛擬網路](../../../virtual-network/virtual-networks-overview.md)搭配使用，可進行數個情節。 您可以連接在相同虛擬網路中的 VM，即使這些 VM 位於不同的資源群組也可以。 [站對站 VPN](../../../vpn-gateway/tutorial-site-to-site-portal.md)可讓您建立能將 VM 連接至內部部署網路和電腦的混合式架構。
...
## <a name="change-sql-connectivity-settings"></a><a id="change"></a>變更 SQL 連線能力設定
...
4. 將 **SQL 連線能力層級** 變更為必要的設定。 可選擇性地使用此區域來變更 SQL Server 連接埠或 SQL 驗證設定。
   ...
   ![變更 SQL 連線能力](./media/ways-to-connect-to-sql/sql-vm-portal-connectivity-change.png)
...
## <a name="manual-configuration-and-troubleshooting"></a><a id="manual"></a>手動設定和疑難排解
...
| [建立 SQL 登入](/sql/relational-databases/security/authentication-access/create-a-login) | 如果您使用的是 SQL 驗證，則需要包含使用者名稱和密碼，且具備目標資料庫權限的 SQL 登入。 |
...
若要查看佈建指示以及連線步驟，請參閱[在 Azure 上佈建 SQL Server 虛擬機器](create-sql-vm-portal.md)。

如需在 Azure VM 上執行 SQL Server 的其他相關主題，請參閱 [Azure 虛擬機器上的 SQL Server](sql-server-on-azure-vm-iaas-what-is-overview.md)。