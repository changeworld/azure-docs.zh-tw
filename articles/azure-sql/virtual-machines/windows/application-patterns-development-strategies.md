...
## <a name="summary"></a>摘要：
決定要在 Azure 環境中用於 SQL Server 型應用程式的應用程式模式或模式是一項重要的設計決策，而且需要瞭解 Azure 的 SQL Server 和每個基礎結構元件如何搭配使用。 透過 Azure 基礎結構服務中的 SQL Server，您可以輕鬆地將 Windows Server 上建立的現有 SQL Server 應用程式遷移、維護及監視到 Azure 中的虛擬機器（Vm）。
...
* 您想要利用 Azure 環境的功能，但 Azure SQL Database 不支援您應用程式需要的所有功能。 這可能包括下列部分：
  ...
  * **HIPAA 合規性**：醫療保健客戶和獨立軟體廠商（isv）可能會選擇[azure 虛擬機器上的 SQL Server](sql-server-on-azure-vm-iaas-what-is-overview.md) ，而不是[Azure SQL Database](../../database/sql-database-paas-overview.md) ，因為 HIPAA 業務關聯協定（BAA）涵蓋 azure 虛擬機器上的 SQL Server。 如需法規遵循的資訊，請參閱 [Microsoft Azure 信任中心：合規性](https://azure.microsoft.com/support/trust-center/compliance/)。
...
在此應用程式模式中，您會將「展示層」（web 伺服器）和「商務層」（應用程式伺服器）元件散發到多部虛擬機器，以將2層或3層資料庫應用程式部署至 Azure 虛擬機器。 此外，您會在 Azure 虛擬機器中，為您的資料庫執行高可用性和嚴重損壞修復（HADR）解決方案。
