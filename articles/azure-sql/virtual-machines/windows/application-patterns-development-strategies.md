---
title: 在 Vm 上 SQL Server 應用程式模式 |Microsoft Docs
...
## <a name="summary"></a>摘要：
決定要在 Azure 環境中用於 SQL Server 型應用程式的應用程式模式或模式是一項重要的設計決策，而且需要瞭解 Azure 的 SQL Server 和每個基礎結構元件如何搭配使用。 透過 Azure 基礎結構服務中的 SQL Server，您可以輕鬆地將 Windows Server 上建立的現有 SQL Server 應用程式遷移、維護及監視到 Azure 中 (Vm) 的虛擬機器。
...
**技術審核者：** Corey Sanders、已繪製 McDaniel、Narayan Annamalai、Nir Mashkowski、Sanjay Mishra、Silvano Coriani、Stefan Schackow、Tim Hickey、Tim Wieman、Xin 金
...
開始閱讀本文之前，您應該先了解 SQL Server 和 Azure 的基本概念。 如需相關資訊，請參閱[Azure 虛擬機器](sql-server-on-azure-vm-iaas-what-is-overview.md)和[Azure.com](https://azure.microsoft.com/)上的[SQL Server 線上叢書](https://msdn.microsoft.com/library/bb545450.aspx)、SQL Server。

本文會說明一些應用程式模式，它們適用於簡單的應用程式以及複雜度高的企業應用程式。 在詳述每個模式之前，建議您先熟悉 Azure 中可用的資料儲存體服務，例如[azure 虛擬機器中](sql-server-on-azure-vm-iaas-what-is-overview.md)的[Azure 儲存體](../../../storage/common/storage-introduction.md)、 [Azure SQL Database](../../database/sql-database-paas-overview.md)和 SQL Server。 若要為應用程式做出最佳設計決策，您必須清楚地了解何時要使用哪一種資料儲存體服務。

### <a name="choose-sql-server-on-azure-virtual-machines-when"></a>選擇 Azure 虛擬機器上 SQL Server，時機：
...
* 您想要利用 Azure 環境的功能，但 Azure SQL Database 不支援您應用程式需要的所有功能。 這可能包括下列部分：
  ...
  * **HIPAA 合規性**： (isv) 的醫療保健客戶和獨立軟體廠商可能會選擇[azure 虛擬機器上的 SQL Server](sql-server-on-azure-vm-iaas-what-is-overview.md) ，而不是[Azure SQL Database](../../database/sql-database-paas-overview.md) ，因為 HIPAA 業務關聯合約 SQL Server BAA 虛擬機器會涵蓋 azure (上的) 。 如需法規遵循的資訊，請參閱 [Microsoft Azure 信任中心：合規性](https://azure.microsoft.com/support/trust-center/compliance/)。
...
![具有商業層向外延展的應用程式模式](./media/application-patterns-development-strategies/IC728011.png)

## <a name="2-tier-and-3-tier-with-presentation-and-business-tiers-scale-out-and-hadr"></a>展示層和商務層向外延展的 2 層和 3 層以及 HADR
在此應用程式模式中，您會將2層或3層資料庫應用程式部署至 Azure 虛擬機器，方法是將展示層 (web 伺服器) 和商務層 (應用程式伺服器) 元件散發到多部虛擬機器。 此外，您還可以在 Azure 虛擬機器中，為您的資料庫執行高可用性和嚴重損壞修復 (HADR) 解決方案。
...
簡而言之，如果您不想要產生大量的展示層/商業層管理工作，且應用程式不需要對軟體或作業系統做任何複雜的設定，請使用 Azure 雲端服務。 如果 Azure SQL Database 不支援您要尋找的所有功能，請使用 Azure 虛擬機器中的 SQL Server 作為資料層。 在 Azure 雲端服務上執行應用程式，並將資料儲存在 Azure 虛擬機器中，可結合這兩個服務的優點。 如需詳細的比較，請參閱本主題的章節： [比較 Azure 中的開發策略](#comparing-web-development-strategies-in-azure)。
...
3. 設定公司網路內部部署和 [Azure 虛擬網路](../../../virtual-network/virtual-networks-overview.md)之間的網路連線。 若要設定公司網路內部部署與 Azure 虛擬機器之間的連線，請使用下列兩種方法之一：
   ...
   2. 透過 Azure 虛擬私人網路 (VPN) 通道，在內部部署與 Azure 之間建立連線。 此方法可讓您將網域原則延伸至 Azure 中的虛擬機器。 此外，您可以設定防火牆規則，並在虛擬機器中使用 Windows 驗證。 Azure 目前支援安全的網站間 VPN 和點對站 VPN 連線：
      ...
      * 您可以透過安全的點對站連線，在 Azure 虛擬網路與在任何一處執行的電腦之間，建立網路連線。 此方法多半建議用於開發和測試。
      ...
      如需如何連接至 Azure 中 SQL Server 的相關資訊，請參閱連線[至 azure 上的 SQL Server 虛擬機器](ways-to-connect-to-sql.md)。
4. 設定排定的工作和警示，以在 Azure 的虛擬機器磁碟中備份內部部署資料。 如需詳細資訊，請參閱[使用 Azure Blob 儲存體服務 SQL Server 備份和還原](https://msdn.microsoft.com/library/jj919148.aspx)和[azure 虛擬機器上 SQL Server 的備份與還原](../../../azure-sql/virtual-machines/windows/backup-restore.md)。
...
* 在 Azure 中設定傳統 web 伺服器 (IIS Internet Information Services) ，並在 Azure 虛擬機器上存取 SQL Server 中的資料庫。
* 實作雲端服務並將其部署至 Azure。 然後，請確定此雲端服務可以在 Azure 虛擬機器的 SQL Server 中存取資料庫。 雲端服務可以包含多個 Web 角色和背景工作角色。

下表提供傳統 網頁程式開發與 azure 雲端服務和 azure Web Apps （相對於 Azure 虛擬機器上的 SQL Server）的比較。 此表格包含 Azure Web Apps，因為在 Azure VM 中可透過其公用虛擬 IP 位址或 DNS 名稱使用 SQL Server 作為 Azure Web Apps 的資料來源。

|  | Azure 虛擬機器中的傳統 Web 開發 | Azure 中的雲端服務 | 使用 Azure Web Apps 的 Web 主機 |
| --- | --- | --- | --- |
...
如需在 Azure 虛擬機器上執行 SQL Server 的詳細資訊，請參閱[azure 上的 SQL Server 虛擬機器總覽](sql-server-on-azure-vm-iaas-what-is-overview.md)。
