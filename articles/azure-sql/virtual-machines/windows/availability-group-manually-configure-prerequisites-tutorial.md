...
ms.locfileid: "84197133"
---
# <a name="prerequisites-for-creating-always-on-availability-groups-on-sql-server-on-azure-virtual-machines"></a>在 Azure 虛擬機器中的 SQL Server 上建立 Always On 可用性群組的先決條件
[!INCLUDE[appliesto-sqlvm](../../includes/appliesto-sqlvm.md)]

本教學課程示範如何完成[在 Azure虛擬機器 (VM) 上建立 SQL Server Always On 可用性群組](availability-group-manually-configure-tutorial.md)的必要條件。 完成必要條件之後，您會在單一資源群組中有一個網域控制站、兩個 SQL Server VM 及一部見證伺服器。
...
17. 在 [部署組態] 下，選取 [將網域控制站新增至現有網域]。
    ![部署組態](./media/availability-group-manually-configure-prerequisites-tutorial-/28-deploymentconfig.png)
18. 按一下 [選取]。
...
> [!NOTE]
> 此處建議的機器大小是為了在 Azure VM 中測試可用性群組。 為了在生產工作負載上獲得最佳效能，請參閱 [Azure 虛擬機器中 SQL Server 的效能最佳做法](performance-guidelines-best-practices.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)中關於 SQL Server 虛擬機器大小與組態的建議。
>
>
