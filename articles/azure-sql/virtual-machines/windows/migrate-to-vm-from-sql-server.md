...
| [執行備份至 URL 並從 URL 還原到 Azure 虛擬機器](#backup-to-a-url-and-restore) |SQL Server 2012 SP1 CU2 或更新版本 | SQL Server 2012 SP1 CU2 或更新版本 | < 12.8 TB 適用於 SQL Server 2016，否則為 < 1 TB | 此法是使用 Azure 儲存體將備份檔案移至 VM 的另一種方式。 |
...
## <a name="backup-to-a-url-and-restore"></a>備份至 URL 和還原

除了備份至本機檔案，您可以使用[備份至 URL](https://msdn.microsoft.com/library/dn435916.aspx) 並從 URL 還原至 VM。 SQL Server 2016 支援等量分割備份組。 建議用來提升效能，並且在超過每個 Blob 的大小限制時為必備條件。 對於大型資料庫，建議使用 [Windows 匯入/匯出服務](../../../storage/common/storage-import-export-service.md) 。
