...
description: 本教學課程會示範如何在 SQL Server Management Studio 中使用 Always Encrypted wizard，以資料加密來保護資料庫中的敏感性資料 Azure SQL Database。
...
ms.locfileid: "84190514"
---
# <a name="configure-always-encrypted-by-using-azure-key-vault"></a>使用 Azure Key Vault 設定 Always Encrypted 

[!INCLUDE[appliesto-sqldb](../includes/appliesto-sqldb-sqlmi.md)]

本文說明如何使用[SQL Server Management Studio （SSMS）](/sql/ssms/sql-server-management-studio-ssms)中的[Always Encrypted wizard](/sql/relational-databases/security/encryption/always-encrypted-wizard) ，以資料加密來保護資料庫中的敏感性資料 Azure SQL Database。 它也包含示範如何將每個加密金鑰儲存在「Azure 金鑰保存庫」中的指示。

Always Encrypted 是一種資料加密技術，可在用戶端與伺服器之間移動時，以及在資料使用中時，協助保護伺服器上待用的敏感性資料。 「一律加密」可確保機密資料在資料庫系統內一律不會以純文字顯示。 設定資料加密之後，只有具備金鑰存取權的用戶端應用程式或應用程式伺服器才可以存取純文字資料。 如需詳細資訊，請參閱 [一律加密 (資料庫引擎)](https://msdn.microsoft.com/library/mt163865.aspx)。

將資料庫設定為使用「一律加密」之後，您將使用 Visual Studio 以 C# 建立用戶端應用程式來使用加密資料。

請遵循這篇文章中的步驟，並瞭解如何在 Azure SQL Database 或 SQL 受控執行個體中設定資料庫的 Always Encrypted。 在本文章中，您將學習到如何執行下列工作：
...
您必須藉由設定 Azure Active Directory （Azure AD）應用程式，並複製驗證應用程式所需的*應用程式識別碼*和*金鑰*，讓您的用戶端應用程式能夠在 SQL Database 中存取您的資料庫。
...
您可以藉**由展開 [** 實務] [  >  **安全性**]  >  **Always Encrypted 金鑰**，來確認 SSMS 中的金鑰建立。
...
3. 前往 [**工具**] [  >  **NuGet 套件管理員**] [  >  **套件管理員主控台**]，以安裝下列 NuGet 套件。
...
2. 按一下 [**連接**  >  **資料庫引擎**] 以開啟 [連線**到伺服器**] 視窗，然後按一下 [**選項**]。
...
在您的資料庫設定為使用 Always Encrypted 之後，您可能會想要執行下列動作：
