...
Azure SQL Database 和 Azure SQL 受控執行個體所使用的虛擬核心（vCore）購買模型提供數個優點：
...
VCore 模型中的服務層選項包括一般用途、業務關鍵和超大規模資料庫。 服務層通常會定義儲存體架構、空間和 i/o 限制，以及與可用性和嚴重損壞修復相關的商務持續性選項。
...
- [選擇超大規模資料庫服務層級的時機](service-tier-hyperscale.md#who-should-consider-the-hyperscale-service-tier)
...
[無伺服器計算層](serverless-tier-overview.md)會根據工作負載活動自動調整計算資源，並以每秒使用的計算量來計費。
...
在 Azure 入口網站中，您可以在建立時選取資料庫或 SQL Database 集區的硬體世代，也可以變更現有資料庫或集區的硬體世代。
...
從 [SQL 受控執行個體] 頁面中，選取放置在 [設定] 區段底下的 [**定價層**] 連結
...
第4代硬體即將[淘汰](https://azure.microsoft.com/updates/gen-4-hardware-on-azure-sql-database-approaching-end-of-life-in-2020/)，不再適用于新的部署。 所有新的資料庫都必須部署在第5代硬體上。
...
- 集區[Azure SQL Database 的 vCore 為基礎的資源限制](resource-limits-vcore-elastic-pools.md)。
