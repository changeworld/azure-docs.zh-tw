...
VCore 模型中的硬體世代選項包括 Gen 4/5、M 系列和 Fsv2 系列。 硬體世代通常會定義計算和記憶體限制，以及影響工作負載效能的其他特性。
...
只有在業務關鍵層中才支援 M 系列，且不支援區域冗余。  訂用帳戶必須是付費供應專案類型，包括隨用隨付或 Enterprise 合約 (EA) 。 如需 M 系列的可用區域，請參閱 [m 系列可用性](#m-series-1)。
