...
1. [彈性資料庫分割合併工具教學課程](elastic-scale-configure-deploy-split-and-merge.md)
2. [分割合併安全性設定](elastic-scale-split-merge-security-configuration.md)
3. [分割合併安全性考量](elastic-scale-split-merge-security-configuration.md)
4. [分區對應管理](elastic-scale-shard-map-management.md)
5. [將現有的資料庫移轉到相應放大的資料庫](elastic-convert-to-use-elastic-tools.md)
6. [彈性資料庫功能概觀](elastic-scale-introduction.md)
7. [彈性資料庫工具字彙](elastic-scale-glossary.md)

## <a name="why-use-the-split-merge-tool"></a>為何使用分割合併工具？

- **彈性**

  應用程式需要在 Azure SQL Database 中，彈性延展超過單一資料庫的限制。 使用工具依需要將資料移至新的資料庫，同時保有完整性。
...
- **合併以縮減**
  ...
  容量因為業務的季節特性需要縮減。 當業務減緩時，此工具可讓您相應減少至較少的縮放單位。 彈性調整分割合併服務中的「合併」功能涵蓋了這項需求。
...
- **客戶主控式服務**
  ...
  分割合併提供為客戶主控式服務。 您必須將服務部署並裝載於 Microsoft Azure 訂用帳戶中。 您從 NuGet 下載的封裝包含設定範本，可加入您特定部署的資訊而使之完備。 如需詳細資訊，請參閱 [分割合併教學課程](elastic-scale-configure-deploy-split-and-merge.md) 。 因為服務是在您的 Azure 訂用帳戶中執行，您可以控制和設定服務的大部分安全性層面。 預設範本包含設定 TLS、以憑證為基礎的用戶端驗證、儲存的認證加密、DoS 保護和 IP 限制的選項。 您可以在下列文件中找到安全性層面的詳細資訊： [分割合併安全性組態](elastic-scale-split-merge-security-configuration.md)。
...
- **分區地圖**
  ...
  下一節的要求參數討論分區對應和主控分區對應之資料庫的相關資訊。 特別是，您必須提供裝載 shardmap 的伺服器和資料庫名稱、連接到分區對應資料庫的認證，以及分區對應的名稱。 目前，此作業只接受一組認證。 這些認證必須有足夠權限來變更分區對應及分區上的使用者資料。
...
- **目標分區（分割）**
  ...
  在您提供分割作業來源的相關資訊之後，您需要提供目標的伺服器和資料庫名稱，以定義要將資料複製到何處。
...
一般而言，較高、更有效的服務層級會有更好的效能。 越高的服務層級使用越高的 IO、CPU 和記憶體配置，有利於分割合併服務使用的大量複製和刪除作業。 基於這個理由，請只針對這些資料庫，在一段已定義的有限期間內增加服務層級。
...
1. 請遵循 [部署分割-合併服務](elastic-scale-configure-deploy-split-and-merge.md)中的步驟執行。
