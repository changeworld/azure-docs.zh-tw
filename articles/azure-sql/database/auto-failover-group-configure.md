...
description: 瞭解如何使用 Azure 入口網站、Azure CLI 和 PowerShell，為 Azure SQL Database （單一和集區）和 SQL 受控執行個體設定自動容錯移轉群組。
...
ms.locfileid: "85987124"
---
# <a name="configure-a-failover-group-for-azure-sql-database"></a>設定 Azure SQL Database 的容錯移轉群組
[!INCLUDE[appliesto-sqldb-sqlmi](../includes/appliesto-sqldb-sqlmi.md)]

本主題將教您如何為 Azure SQL Database 和 Azure SQL 受控執行個體設定[自動容錯移轉群組](auto-failover-group-overview.md)。
...
1. 選取您想要新增到容錯移轉群組的資料庫。
1. 在 [**伺服器名稱**] 底下選取伺服器的名稱，以開啟伺服器的設定。

   ![單一 db 的開啟伺服器](./media/auto-failover-group-configure/open-sql-db-server.png)

1. 在 [**設定**] 窗格下選取 [**容錯移轉群組**]，然後選取 [**新增群組**] 以建立新的容錯移轉群組。

   ![加入新的容錯移轉群組](./media/auto-failover-group-configure/sqldb-add-new-failover-group.png)
...
1. 在 [**總覽**] 窗格中，選取 [**伺服器名稱**] 下的伺服器名稱，以開啟伺服器的設定。
   ...
   ![開啟適用于彈性集區的伺服器](./media/auto-failover-group-configure/server-for-elastic-pool.png)
...
使用 Azure 入口網站或 PowerShell，在 Azure SQL 受控執行個體中的兩個受控實例之間建立容錯移轉群組。

您將需要設定[ExpressRoute](../../expressroute/expressroute-howto-circuit-portal-resource-manager.md)或為每個 SQL 受控執行個體的虛擬網路建立閘道、連接兩個閘道，然後建立容錯移轉群組。
...
1. 選取 [**設定**] 底下的 [**子網**]，然後選取以新增新的**閘道子網**。 保留預設值。
   ...
   ![為主要受控實例新增閘道](./media/auto-failover-group-configure/add-subnet-gateway-primary-vnet.png)
...
使用 Azure 入口網站，為您的 SQL 受控實例建立容錯移轉群組。
...
1. 選取 [**容錯移轉**]，然後在關於 TDS 會話中斷連線的警告上選取 **[是]** 。
   ...
   ![容錯移轉群組](./media/auto-failover-group-configure/failover-mi-failover-group.png)
...
一旦設定您的容錯移轉群組，請將應用程式的連接字串更新為接聽程式端點。 這會讓您的應用程式連接到容錯移轉群組接聽程式，而不是主資料庫、彈性集區或實例資料庫。 如此一來，您就不需要在每次資料庫實體故障時手動更新連接字串，而會將流量路由傳送至目前主要的任何實體。
...
- 移除單一或集區資料庫的容錯移轉群組並不會停止複寫，而且不會刪除複寫的資料庫。 如果您想要在移除容錯移轉群組後，將單一或集區資料庫重新加入至該群組，您必須手動停止異地複寫，並從次要伺服器刪除該資料庫。 `The operation cannot be performed due to multiple errors`當嘗試將資料庫新增到容錯移轉群組時，可能會導致類似的錯誤。
