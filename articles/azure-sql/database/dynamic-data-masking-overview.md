---
title: 動態資料遮罩
description: 動態資料遮罩會對 Azure SQL Database、Azure SQL 受控執行個體和 Azure Synapse 分析的非特殊許可權使用者進行遮罩，以限制敏感性資料的暴露程度。
...
Azure SQL Database、Azure SQL 受控執行個體和 Azure Synapse 分析支援動態資料遮罩。 動態資料遮罩會對不具權限的使用者遮罩機密資料，從而限制其曝光。 
...
| **隨機數字** |**遮罩方法會產生一個隨機數字**，其根據為選取的界限與實際資料類型。 如果指定的邊界相等，則遮罩函數是常數。<br/><br/>![導覽窗格](./media/dynamic-data-masking-overview/1_DDM_Random_number.png) |
