...
分割合併工具可讓您在分區化資料庫之間移動資料。 請參閱 [在相應放大的雲端資料庫之間移動資料](elastic-scale-overview-split-and-merge.md)
...
如需有關設定服務安全性的詳細指示，請參閱 [分割合併安全性設定](elastic-scale-split-merge-security-configuration.md)。
...
請注意，在實際執行部署中，CA、加密、伺服器憑證和用戶端憑證應該使用個別憑證。 如需詳細指示，請參閱 [安全性設定](elastic-scale-split-merge-security-configuration.md)。
...
執行範例 PowerShell 腳本時，您可能會看到下列訊息：
