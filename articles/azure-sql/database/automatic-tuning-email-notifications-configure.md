...
ms.locfileid: "85982868"
---
# <a name="email-notifications-for-automatic-tuning"></a>針對自動調整的電子郵件通知
[!INCLUDE[appliesto-sqldb-sqlmi](../includes/appliesto-sqldb-sqlmi.md)]


Azure SQL Database[自動調整](automatic-tuning-overview.md)會產生 Azure SQL Database 微調建議。 此解決方案會持續監視和分析資料庫的工作負載，為每個與索引建立、索引刪除和優化查詢執行計畫相關的個別資料庫提供自訂的微調建議。

Azure SQL Database 自動調整建議可以在[Azure 入口網站](database-advisor-find-recommendations-portal.md)中查看，並使用[REST API](https://docs.microsoft.com/rest/api/sql/databaserecommendedactions/listbydatabaseadvisor)呼叫來抓取，或使用[t-sql](https://azure.microsoft.com/blog/automatic-tuning-introduces-automatic-plan-correction-and-t-sql-management/)和[PowerShell](https://docs.microsoft.com/powershell/module/az.sql/get-azsqldatabaserecommendedaction)命令。 本文使用 PowerShell 指令碼來擷取自動調整建議。

[!INCLUDE [updated-for-az](../../../includes/updated-for-az.md)]
> [!IMPORTANT]
> Azure SQL Database 仍然支援 PowerShell Azure Resource Manager 模組，但所有未來的開發都是針對 Az .Sql 模組。 如需這些 Cmdlet，請參閱[AzureRM](https://docs.microsoft.com/powershell/module/AzureRM.Sql/)。 Az 模組和 AzureRm 模組中命令的引數本質上完全相同。

## <a name="automate-email-notifications-for-automatic-tuning-recommendations"></a>自動化自動調整建議的電子郵件通知
...
請遵循下列步驟，透過從 Azure Marketplace 選取並設定自動化應用程式的方法來建立 Azure 自動化帳戶：
...
在數個訂用帳戶的案例中，您可以將它們新增為以逗號分隔，並以逗點分隔到腳本標頭中的 "$subscriptions" 屬性。
...
在上述步驟中，將會在 Azure 自動化中載入用來抓取自動調整建議的 PowerShell 腳本。 下一步是自動化和排程電子郵件傳送工作。
...
此步驟的必要條件是註冊[Microsoft Flow](https://flow.microsoft.com)帳戶並登入。 一旦進入解決方案，請遵循下列步驟來設定**新的流程**：
...
2. 建立動作，以從已執行的 PowerShell 腳本取出輸出
...
> [!TIP]
> 若要傳送自動化電子郵件給不同的收件者，請建立個別流程。 在這些額外的流程中，變更 [收件者] 欄位中的收件者電子郵件地址，以及 [主旨] 欄位中的電子郵件主旨行。 使用自訂的 PowerShell 腳本（例如變更 Azure 訂用帳戶識別碼）在 Azure 自動化中建立新的 runbook，可進一步自訂自動化案例，例如，針對個別訂用帳戶自動調整建議的電子郵件個別收件者。
...
[流程分析] 窗格有助於監視作業執行是否成功，以及是否需要進行疑難排解。  在進行疑難排解的案例中，您也可能想要檢查可透過 Azure 自動化應用程式存取的 PowerShell 腳本執行記錄。
...
- 若要手動檢查並套用自動調整建議，請參閱[尋找和套用效能建議](database-advisor-find-recommendations-portal.md)。
