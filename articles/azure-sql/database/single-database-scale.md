...
> [!NOTE]
> 此外，如果是標準 (S2 S12) 和一般用途的資料庫，在彈性集區中移入/移出，或在彈性集區之間移動資料庫的延遲會與資料庫大小成正比，如果資料庫使用 Premium 檔案共用 ([PFS](https://docs.microsoft.com/azure/storage/files/storage-files-introduction)) 儲存體。
>
...
- 您可以使用 Azure 入口網站、 [transact-sql](https://docs.microsoft.com/sql/t-sql/statements/alter-database-transact-sql#examples-1)、 [PowerShell](/powershell/module/az.sql/set-azsqldatabase)、 [Azure CLI](/cli/azure/sql/db#az-sql-db-update)或 [REST API](https://docs.microsoft.com/rest/api/sql/databases/update)來增加其大小上限，以布建單一資料庫的額外儲存空間。
