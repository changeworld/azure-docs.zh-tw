...
description: 本檔說明 Azure SQL Database 和 Azure Synapse 分析的傳輸層安全性（TLS）版本選擇和 Proxy 與重新導向設定
...
ms.locfileid: "87132971"
---
# <a name="azure-sql-connectivity-settings"></a>Azure SQL 連線能力設定
[!INCLUDE[appliesto-sqldb-asa](../includes/appliesto-sqldb-asa.md)]

本文介紹可控制伺服器連線以進行 Azure SQL Database 和 Azure Synapse 分析的設定。 這些設定適用于與伺服器相關聯的**所有**SQL Database 和 Azure Synapse 資料庫。

> [!IMPORTANT]
> 本文*不*適用**Azure SQL 受控執行個體**

您可以從 [**防火牆] 和 [虛擬網路**] 畫面存取連線設定，如下列螢幕擷取畫面所示：
...
當 [**拒絕公用網路存取**] 設定設為 **[是]** 時，只允許透過私人端點的連接。 當此設定設為 [**否**] （預設值）時，用戶端可以使用公用端點（以 IP 為基礎的防火牆規則、VNET 型防火牆規則）或私人端點（使用私人連結）進行連線，如[網路存取總覽](network-access-controls-overview.md)中所述。 
...
當 [**拒絕公用網路存取**] 設定設為 **[是]** 時，只允許透過私人端點的連線，而且所有透過公用端點的連線都會遭到拒絕，並顯示類似下列的錯誤訊息：  
...
當 [**拒絕公用網路存取**] 設定設為 **[是]** 時，將會拒絕任何新增或更新防火牆規則的嘗試，並顯示類似下列的錯誤訊息：
...
「最小[傳輸層安全性（TLS）](https://support.microsoft.com/help/3135244/tls-1-2-support-for-microsoft-sql-server) 」版本設定可讓客戶控制其 Azure SQL Database 所使用的 TLS 版本。

目前，我們支援 TLS 1.0、1.1 和1.2。 設定最低的 TLS 版本可確保支援後續的較新 TLS 版本。 例如，選擇大於1.1 的 TLS 版本。 表示只接受 TLS 1.1 和1.2 的連線，而 TLS 1.0 遭到拒絕。 在測試確認您的應用程式支援它之後，建議您將最小的 TLS 版本設定為1.2，因為它包含舊版中找到的弱點修正程式，而且是 Azure SQL Database 中支援的最高 TLS 版本。

> [!IMPORTANT]
> 最低 TLS 版本的預設值是允許所有版本。 不過，一旦您強制執行 TLS 版本，就無法還原為預設值。
...
下列 PowerShell 腳本示範如何 `Get` 和 `Set` 邏輯伺服器層級的**最低 TLS 版本**屬性：
...
## <a name="change-connection-policy"></a>變更連線原則

連線[原則](connectivity-architecture.md#connection-policy)會決定用戶端連接到 Azure SQL Database 的方式。
...
- 如需 Azure SQL Database 中的連線能力的總覽，請參閱連線[架構](connectivity-architecture.md)
- 如需如何變更伺服器連線原則的詳細資訊，請參閱[conn-policy](https://docs.microsoft.com/cli/azure/sql/server/conn-policy)。
