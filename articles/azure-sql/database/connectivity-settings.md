...
目前，我們支援 TLS 1.0、1.1 和1.2。 設定最短的 TLS 版本，可確保支援較新的 TLS 版本。 例如，選擇大於1.1 的 TLS 版本表示只接受具有 TLS 1.1 和1.2 的連線，而且會拒絕與 TLS 1.0 的連接。 在您進行測試以確認您的應用程式支援它之後，建議您將最基本的 TLS 版本設定為1.2。 此版本包含舊版弱點的修正程式，而且是 Azure SQL Database 中支援的最高 TLS 版本。

> [!IMPORTANT]
> 最基本 TLS 版本的預設值是允許所有版本。 在您強制執行 TLS 版本之後，就無法還原為預設值。

如果客戶的應用程式依賴舊版的 TLS，建議您根據應用程式的需求來設定最基本的 TLS 版本。 針對依賴使用未加密連線來連接應用程式的客戶，建議您不要設定任何基本的 TLS 版本。
