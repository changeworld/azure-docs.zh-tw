...
description: 監視及管理多租使用者 SaaS 應用程式中 Azure SQL Database 的效能
...
ms.locfileid: "86042810"
---
# <a name="monitor-and-manage-performance-of-azure-sql-database-in-a-multi-tenant-saas-app"></a>監視及管理多租使用者 SaaS 應用程式中 Azure SQL Database 的效能
...
[Azure 入口網站](https://portal.azure.com)提供大部分資源的內建監視與警示功能。 監視和警示適用于資料庫和集區。 這個內建的監視與警示功能是資源特定，因此針對少數資源使用很方便，但是搭配許多資源使用時則不是很方便。

在您處理許多資源的大量案例中，可以使用[Azure 監視器記錄](../../sql-database/saas-dbpertenant-log-analytics.md)。 這是個別的 Azure 服務，可針對在 Log Analytics 工作區中收集的已發出記錄提供分析。 Azure 監視器記錄檔可以從許多服務收集遙測，並用來查詢和設定警示。
...
1. 設定 **$DemoScenario**  =  **1**，布建**一批**租使用者
1. 按 **F5** 以執行指令碼。
...
1. 在[Azure 入口網站](https://portal.azure.com)中開啟*Pool1* （在*tenants1- \<user\> tenants1-dpt user-* 伺服器上）。
...
1. 設定 *$DemoScenario*  =  **3**，_針對每個資料庫產生較長且更頻繁的高載負載_，以增加集區的匯總負載強度，而不變更每個資料庫所需的尖峰負載。
1. 按下 **F5** 將負載套用到您的所有租用戶資料庫。
...
回到**Pool1**  >  **總覽**以觀看監視圖表。 監視為集區提供更多資源的影響 (雖然有一些資料庫和隨機負載，但是在您執行一段時間前，也不容易明確地看出)。 當您查看圖表時，請記住上方圖表上的 100% 現在代表 100 個 eDTU，而下方圖表上的 100% 仍是 50 個 eDTU，因為每個資料庫最大值仍是 50 個 eDTU。
...
流覽至**Pool2** （在*tenants1-tenants1-dpt user- \<user\> *伺服器上）以開啟集區，並監視其效能。 如果您沒有看到它，請等候新的集區佈建完成。
...
1. 在 [ [Azure 入口網站](https://portal.azure.com)中，流覽至*tenants1-tenants1-dpt user- \<user\> *伺服器上的資料庫清單。 
