...
在您處理許多資源的大量案例中，可以使用 [Azure 監視器記錄](./saas-dbpertenant-log-analytics.md)。 這是個別的 Azure 服務，可針對 Log Analytics 工作區中收集的發出記錄提供分析。 Azure 監視器記錄可以收集來自許多服務的遙測並用來查詢及設定警示。
