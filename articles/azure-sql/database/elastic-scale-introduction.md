...
傳統上，這類案例的解決方式是投資更大規模的伺服器以支援應用程式。 不過，此選項在雲端有其限制，因為所有處理都發生在預先定義的商用硬體上。 相反地，將資料和處理分散至許多相同結構的資料庫 (一種稱為「分區化」的相應放大模式)，不論在成本或彈性上，都是超越傳統相應增加方法的另一種選擇。
...
垂直調整指的是增加或減少個別資料庫的計算大小，也稱為「相應增加」。

大部分雲端級別的資料庫應用程式都採用這些兩種策略的組合。 例如，「軟體即服務」應用程式可能會使用水準調整來布建新的終端客戶和垂直調整，以允許每個終端客戶的資料庫視工作負載需求成長或縮減資源。
...
*分區化* 是一種將大量相同結構化資料分散到多個獨立資料庫的技術。 為一般客戶或企業建立軟體即服務 (SAAS) 供應項目的開發人員尤其愛用。 這些終端客戶通常稱為「租使用者」。 需要使用分區化可能有各種原因：  
