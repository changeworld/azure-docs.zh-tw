...
|[sys.database_service_objectives (Azure SQL Database)](/sql/relational-databases/system-catalog-views/sys-database-service-objectives-azure-sql-database)|針對 SQL Database 或 Azure Synapse Analytics 中的資料庫，傳回 (服務層級) 、服務目標 (定價層) 和彈性集區名稱（如果有的話）的版本。 如果登入伺服器中的 master 資料庫，則會傳回所有資料庫的資訊。 針對 Azure Synapse Analytics，您必須連接到 master 資料庫。|
