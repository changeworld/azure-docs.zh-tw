...
當 [ **允許存取 Azure 服務** ] 設為 [ **關閉**] 時，[匯入匯出服務] 無法運作。 不過，您可以 [手動從 AZURE VM 執行 sqlpackage.exe，或](https://docs.microsoft.com/azure/sql-database/import-export-from-vm) 使用 DACFx API 直接在程式碼中執行匯出，來解決此問題。
