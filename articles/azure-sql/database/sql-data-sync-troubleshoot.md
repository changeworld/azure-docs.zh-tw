...
description: 瞭解如何在 Azure 中識別、疑難排解和解決 SQL 資料同步的常見問題。
...
- [我看到這個訊息：「無法將 Null 值插入資料行 \<column> 。資料行不允許 null。」這代表什麼意思，如何修正此問題？](#sync-nulls)

- [資料同步如何處理迴圈參考？也就是說，在多個同步群組中同步處理相同的資料時，會繼續變更結果嗎？](#sync-circ)
...
- **解決方法** ： 確定用戶端代理程式已上線，然後再試一次。

- **原因** 。 用戶端代理程式已解除安裝或遺失。

- **解決方法** ： 如果用戶端代理程式已解除安裝或以別的方式遺失：
...
- **解決方法** ： 重新啟動 SQL 資料同步服務。

    1. 在 [ **開始** ] 功能表中，搜尋 [ **服務** ]。
    1. 在搜尋結果中，選取 [ **服務** ]。
    1. 尋找 **SQL 資料同步** 服務。
    1. 如果服務狀態為 [ **已停止** ]，請在服務名稱上按一下滑鼠右鍵，然後選取 [ **啟動** ]。

> [!NOTE]
> 如果上述資訊未讓您的同步群組脫離處理中狀態，Microsoft 支援人員可以重設您的同步群組的狀態。 若要重設您的同步群組狀態，請在 [Microsoft 問&Azure SQL Database 的問題頁面](/answers/topics/azure-sql-database.html)上，建立貼文。 在文章中，新增您的訂用帳戶識別碼和需要重設之群組的同步群組識別碼。 Microsoft 支援工程師會回應您的文章，並在已重設狀態時告訴您。
...
- **解決方法** ： 確定同步牽涉的資料表名稱彼此不同，即使資料裝屬於資料庫中的不同結構描述。
...
- **解決方法** ： 若要避免這個問題，請確認主要索引鍵資料行中的資料都未變更。 在發生問題之後，若要修正此問題，請從同步群組中的所有端點刪除具有不一致資料的資料列。 然後，重新插入資料列。
...
- **解決方法** ： 最佳的修正方式就是預防。 確保您的同步群組中沒有循環參考。 由某個同步群組同步的任何資料列不能同步至另一個同步群組。

### <a name="i-see-this-message-cannot-insert-the-value-null-into-the-column-column-column-does-not-allow-nulls-what-does-this-mean-and-how-can-i-fix-it"></a><a name="sync-nulls"></a> 我看到這個訊息：「無法將 Null 值插入資料行 \<column> 。 資料行不允許 Null。」 這是什麼意思，該如何修正？ 
...
若要針對用戶端代理程式的問題進行疑難排解，請參閱 [Data Sync Agent 問題疑難排解](sql-data-sync-agent-overview.md#agent-tshoot)。
...
- [我無法取消註冊 SQL Server 資料庫](#setup-unreg)
...
- **解決方法** ： 手動刪除 %temp% 資料夾 (`del \*sync\* /s`) 中的同步檔案。 然後，刪除 %temp% 資料夾中的子目錄。
...
- **解決方法** ： 等候佈建或同步程序完成，然後重試刪除同步群組。

### <a name="i-cant-unregister-a-sql-server-database"></a><a name="setup-unreg"></a> 我無法取消註冊 SQL Server 資料庫

- **原因** 。 最有可能的原因是您正在嘗試取消註冊已刪除的資料庫。

- **解決方法** ： 若要取消註冊 SQL Server 資料庫，請選取資料庫，然後選取 [ **強制刪除** ]。
  ...
  1. 停止然後再重新啟動用戶端代理程式主機服務：  
    ...
    如 檢查資料庫是否仍註冊。 如果它不再為已註冊，則作業完成。 否則，請繼續下一個步驟。
...
- **解決方法** ： 授與以服務方式登入的服務認證給使用者帳戶：

  1. 移至 [ **開始** ] 主控台 [系統  >  **Control Panel**  >  **管理工具**  >  **本機安全性原則** ]  >  **本機原則**  >  **使用者 Rights Management** 。
  1. 選取 [以服務方式登入]  。
...
- **解決方法** ： 您可以藉由確定沒有任何資料庫離線 45 天以上來避免 **過期** 狀態。
...
- **解決方法** ： 若要避免同步群組的 **過期** 狀態，請定期在歷程記錄檢視器中檢查同步工作的結果。 調查並解決無法套用的任何變更。
...
-   更新同步結構描述
    -   使用 Transact-sql-[將 Azure 中 SQL 資料同步架構變更](sql-data-sync-update-sync-schema.md)的複寫自動化
