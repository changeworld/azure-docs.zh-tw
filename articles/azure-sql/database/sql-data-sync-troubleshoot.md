...
- [我看到此訊息：「無法將 Null 值插入資料行 \<column> 。資料行不允許 null。」這是什麼意思，以及如何修正此問題？](#sync-nulls)
...
> [!NOTE]
> 如果上述資訊未讓您的同步群組脫離處理中狀態，Microsoft 支援人員可以重設您的同步群組的狀態。 若要重設同步處理群組狀態，請在[Microsoft Q&Azure SQL Database 的問題頁面](https://docs.microsoft.com/answers/topics/azure-sql-database.html)中，建立文章。 在文章中，新增您的訂用帳戶識別碼和需要重設之群組的同步群組識別碼。 Microsoft 支援工程師會回應您的文章，並在已重設狀態時告訴您。
...
### <a name="i-see-this-message-cannot-insert-the-value-null-into-the-column-column-column-does-not-allow-nulls-what-does-this-mean-and-how-can-i-fix-it"></a><a name="sync-nulls"></a>我看到此訊息：「無法將 Null 值插入資料行 \<column> 。 資料行不允許 Null。」 這是什麼意思，該如何修正？ 
...
- **解決**方式。 若要取消註冊 SQL Server 資料庫，請選取資料庫，然後選取 [**強制刪除**]。
...
- **解決**方式。 授與以服務方式登入的服務認證給使用者帳戶：
  ...
  1. 前往 [ **Start**控制台] [系統  >  **Control Panel**  >  **管理工具**] [  >  **本機安全性原則**] [  >  **本機原則**] [  >  **使用者 Rights Management**]。
  1. 選取 [以服務方式登入]****。
