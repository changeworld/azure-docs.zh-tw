...
description: 瞭解如何使用動態管理檢視來監視 Microsoft Azure SQL Database 和 Azure SQL 受控執行個體，以偵測並診斷常見的效能問題。
...
Microsoft Azure SQL Database 和 Azure SQL 受控執行個體啟用動態管理檢視的子集，以診斷效能問題，這可能是因為封鎖或長時間執行的查詢、資源瓶頸、不良的查詢計劃等所造成。 本主題提供有關如何使用動態管理檢視來偵測常見效能問題的資訊。
...
如需動態管理檢視的詳細資訊，請參閱[動態管理檢視和函數（transact-sql）](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/system-dynamic-management-views)。
...
在 Azure SQL 受控執行個體中，查詢動態管理檢視需要**VIEW SERVER STATE**許可權。 如需詳細資訊，請參閱[系統動態管理檢視](/sql/relational-databases/system-dynamic-management-views/system-dynamic-management-views#required-permissions)。

在 SQL Server 和 Azure SQL 受控執行個體的實例中，動態管理檢視會傳回伺服器狀態資訊。 在 Azure SQL Database 中，它們只會傳回目前邏輯資料庫的相關資訊。
...
使用下列查詢識別資料及記錄 IO 使用量。 如果資料或記錄 IO 高於80%，則表示使用者已使用 SQL Database 服務層級的可用 IO。
...
### <a name="identify-high-memory-consuming-statements"></a>識別高記憶體耗用量語句
...
您可以使用 [ [dm_exec_connections](/sql/relational-databases/system-dynamic-management-views/sys-dm-exec-connections-transact-sql) ] 視圖來抓取與特定伺服器和受控實例建立之連線的相關資訊，以及每個連線的詳細資料。 此外，[sys.dm_exec_sessions](/sql/relational-databases/system-dynamic-management-views/sys-dm-exec-sessions-transact-sql) 檢視有助於擷取所有作用中使用者的連接資訊和內部工作。
...
您可以使用[SQL Database 查詢效能深入解析](query-performance-insight-use.md)來監視 Azure SQL Database 資源使用量。 針對 Azure SQL Database 和 Azure SQL 受控執行個體，您可以使用[查詢存放區](https://msdn.microsoft.com/library/dn817826.aspx)進行監視。
...
您可以在每個資料庫中使用[sys.databases dm_db_resource_stats](https://msdn.microsoft.com/library/dn800981.aspx)視圖。 **sys.dm_db_resource_stats** 檢視可顯示相對於服務層級的最新資源使用量資料。 每隔 15 秒鐘就會記錄一次 CPU、資料 IO、記錄檔寫入和記憶體的平均百分比，並且會維持 1 小時。
...
您可以使用[sys. server_resource_stats](/sql/relational-databases/system-catalog-views/sys-server-resource-stats-azure-sql-database)來傳回 Azure SQL 受控執行個體的 CPU 使用量、IO 和儲存體資料。 於五分鐘間隔內收集及彙總資料。 每隔15秒報告就會有一個資料列。 傳回的資料包括 CPU 使用量、儲存體大小、IO 使用率和受控實例 SKU。 歷程記錄資料大約會保留 14 天。
...
**Master**資料庫中的[sys.databases resource_stats](https://msdn.microsoft.com/library/dn269979.aspx)視圖具有其他資訊，可協助您監視資料庫在其特定服務層級和計算大小的效能。 這項資料每隔 5 分鐘就會收集一次，並且會維持大約 14 天。 如果您的資料庫使用資源的時間較長，則此視圖適用于更長期的歷程記錄分析。
...
資料庫引擎會在每個伺服器的**master**資料庫的**resource_stats**視圖中，公開每個作用中資料庫的已使用資源資訊。 資料表中的資料會以 5 分鐘的間隔彙總。 利用基本、標準和進階服務層級，資料可能要花 5 分鐘以上的時間才會出現在資料表中，因此這項資料比較適合進行歷程記錄分析而不是近乎即時的分析。 查詢 **sys.resource_stats** 檢視可查看資料庫的近期歷程記錄，並驗證所選的保留是否會在必要時提供所需的效能。

> [!NOTE]
> 在 Azure SQL Database 上，您必須連接到**master**資料庫才能查詢**sys.databases，resource_stats**在下列範例中。
...
下一個範例顯示您可以使用**resource_stats**目錄檢視的不同方式，以取得資料庫如何使用資源的相關資訊：
...
3. 利用這項有關各資源度量平均值和最大值的資訊，您可以評估您的工作負載與您所選之計算大小的符合程度。 通常，來自 **sys.resource_stats** 的平均值可提供您對目標大小所使用的理想基準。 它應該是您主要的量尺。 例如，您可能使用標準服務層級搭配 S2 計算大小。 CPU 以及 IO 讀取和寫入的平均使用量百分比低於 40%，背景工作角色平均數目低於 50，而且工作階段平均數目低於 200。 您的工作負載可能符合 S1 計算大小。 要看到您的資料庫是否符合背景工作和工作階段限制範圍內非常容易。 若要查看資料庫在 CPU、讀取和寫入方面是否符合較低的計算大小，請將較低計算大小的 DTU 數目除以目前計算大小的 DTU 數目，然後將結果乘以 100：
   ...
   | 24.5 |100.00 |
    ...
    如果此查詢針對三個資源維度傳回的值小於99.9%，請考慮移到下一個較高的計算大小，或使用應用程式微調技術來減少資料庫的負載。
...
如果您要分析 SQL Server 的工作負載，請修改查詢，將焦點放在特定的資料庫上。 如果您考慮將資料庫移至 Azure，此查詢可協助您判斷其可能的會話需求。
...
您可以藉由查詢[resource_stats sys.databases](/sql/relational-databases/system-catalog-views/sys-resource-stats-azure-sql-database)來取得會話的歷程記錄統計資料，並查看 [ **active_session_count** ] 資料行。
...
速度慢或長時間執行的查詢會造成過度的資源耗用，並且會導致封鎖查詢的後果。 導致封鎖的原因可能是不佳的應用程式設計、不良的查詢計畫、缺乏有用的索引等等。 您可以使用 [sys.databases dm_tran_locks] 視圖，取得資料庫中目前鎖定活動的相關資訊。 如需範例程式碼，請參閱[sys.databases dm_tran_locks （transact-sql）](https://msdn.microsoft.com/library/ms190345.aspx)。
