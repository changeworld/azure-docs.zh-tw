...
Azure SQL Database 中的擴充事件功能集，是 SQL Server 和 Azure SQL 受控執行個體上功能的強大子集。

*XEvents* 是非正式暱稱，有時在部落格或其他非正式位置用於「擴充事件」。

有關擴充事件的其他資訊可在下列位置取得：

- [快速入門： SQL Server 中的擴充事件](/sql/relational-databases/extended-events/quick-start-extended-events-in-sql-server)
...
- 關於擴充事件的大量檔適用于 SQL Server、Azure SQL Database 和 Azure SQL 受控執行個體。

當選擇事件檔案做為 [目標](#AzureXEventsTargets)時，事先公開下列項目很有幫助：

- [Azure 儲存體服務](https://azure.microsoft.com/services/storage/)

- [使用 Azure 儲存體的 Azure PowerShell](/powershell/module/az.storage/)
...
- 當您在 SQL Server 上執行 [CREATE EVENT SESSION](/sql/t-sql/statements/create-event-session-transact-sql) 命令時，您使用 **ON SERVER** 子句。 但是在 Azure SQL Database 您改為使用**ON Database**子句。
...
在 Microsoft SQL Server 中，類似的目錄檢視會以不含名稱的* \_ 資料庫*部分來命名，例如：

- **sys.dm_xe_sessions**，而不是<br/>**dm_xe_database_sessions**。

### <a name="dmvs-common-to-both"></a>兩者通用的 DMV

針對擴充事件，有 Azure SQL Database、Azure SQL 受控執行個體和 Microsoft SQL Server 通用的其他 Dmv：
...
## <a name="targets-for-your-azure-sql-database-event-sessions"></a>Azure SQL Database 事件會話的目標

以下是可以在 Azure SQL Database 上從事件會話中捕獲結果的目標：
...
Azure SQL Database 上的擴充事件無法使用[Windows 事件追蹤（ETW）](https://msdn.microsoft.com/library/ms751538.aspx) API。

## <a name="restrictions"></a>限制

Azure SQL Database 的雲端環境中，有幾個與安全性相關的差異這：
...
有大量使用擴充事件會累積超出整體系統狀況良好所允許的更多作用中記憶體的案例。 因此 Azure SQL Database 會動態設定並調整事件會話可以累積的使用中記憶體數量限制。 許多因素會列入動態計算。
...
**事件檔案** 目標在將資料保存到 Azure 儲存體 Blob 時可能會遇到網路延遲或失敗。 Azure SQL Database 中的其他事件可能會在等候網路通訊完成時發生延遲。 此延遲會降低您的工作負載。

- 若要減輕這個效能風險，請避免在您的事件工作階段定義中將 **EVENT_RETENTION_MODE** 選項設為 **NO_EVENT_LOSS**。

## <a name="related-links"></a>相關連結

- 搭配[Azure 儲存體使用 Azure PowerShell](/powershell/module/az.storage/)。
