...
## <a name="monitoring-dashboard-for-all-your-sync-groups"></a>監視所有同步群組的儀表板 

您不再需要個別查看每個同步處理群組的記錄來尋找問題。 您可以使用自訂的 Azure 監視器 view，在同一個位置監視任何訂用帳戶的所有同步群組。 此檢視會呈現 SQL 資料同步客戶重視的相關資訊。

![資料同步監視儀表板](./media/sql-data-sync-monitor-sync/sync-monitoring-dashboard.png)

## <a name="automated-email-notifications"></a>自動電子郵件通知

您再也不用在 Azure 入口網站中手動檢查記錄檔，也不用透過 PowerShell 或 REST API 來進行。 使用 [Azure 監視器記錄](https://docs.microsoft.com/azure/log-analytics/log-analytics-overview)檔時，您可以建立警示，直接前往需要在發生錯誤時查看這些使用者的電子郵件地址。
...
執行下列動作，在不到一小時的時間內針對 SQL 資料同步執行自訂 Azure 監視器記錄監視解決方案：

您必須設定三個元件：

-   PowerShell runbook，可將 SQL 資料同步記錄資料記錄到 Azure 監視器記錄檔。
...
使用 Azure 自動化裝載的 PowerShell runbook 來提取 SQL 資料同步記錄資料，並將其傳送至 Azure 監視器記錄檔。 隨附範例指令碼。 必要條件是您必須具備 Azure 自動化帳戶。 然後您需要建立 Runbook 並排程執行。 
...
10. 針對您的帳戶和 SQL 資料同步設定，進行所需的變更。 (如需詳細資訊，請參閱範例指令碼。)
    ...
    3.  Azure 監視器記錄資訊。 在 Azure 入口網站中尋找這項資訊 |設定 |連線的來源。 如需將資料傳送至 Azure 監視器記錄的詳細資訊，請參閱 [使用 HTTP 資料收集器 API 將資料傳送至 Azure 監視器記錄 (預覽) ](../../azure-monitor/platform/data-collector-api.md)。
...
5.  將 [週期]**** 設為 [週期性] 並設定所要的間隔。 請在這裡、腳本及 Azure 監視器記錄檔中使用相同的間隔。
...
## <a name="create-an-azure-monitor-reader-alert-for-email-notifications"></a>建立電子郵件通知的 Azure 監視器讀者警示

若要建立使用 Azure 監視器記錄的警示，請執行下列動作。 先決條件是，您必須有與 Log Analytics 工作區連結的 Azure 監視器記錄。
...
4.  在 [產生警示的依據]**** 下方，選取 [計量量值]****。
    ...
    2.  在 [大於]**** 之後，輸入要在經過多久閾值後收到通知。 資料同步中預期會發生暫時性錯誤。若要減少雜訊，請將閾值設為5。

5.  在 [動作]**** 下方，將 [電子郵件通知]**** 設為 [是]。 輸入所需的電子郵件收件者。

6.  按一下 [檔案]  。 現在，指定的收件者即可在發生錯誤時，收到電子郵件通知。

## <a name="create-an-azure-monitor-view-for-monitoring"></a>建立用於監視的 Azure 監視器視圖

此步驟會建立 Azure 監視器視圖，以視覺方式監視所有指定的同步群組。 這個檢視包含數個元件：

-   概觀圖格，其中顯示所有同步群組的錯誤次數、成功次數和警告次數。

-   所有同步群組的圖格，其會顯示每個同步群組的錯誤和警告計數。 此圖格不會顯示沒有問題的群組。

-   每個同步處理群組的磚，會顯示錯誤、成功和警告的數目，以及最近的錯誤訊息。

若要設定 Azure 監視器 view，請執行下列動作：

1.  在 Log Analytics 工作區的首頁上，選取左側的加號以開啟 [ **視圖設計**工具]。
...
**Azure 監視器記錄：** 根據您的使用量而定，可能會有與 Azure 監視器記錄相關聯的成本。 免費層包含一天 500 MB 的內嵌資料。 在大部分情況下，這個解決方案一天只會內嵌 500 MB 以內的資料量。 若要減少使用量，可使用 Runbook 隨附的僅限失敗篩選功能。 如果您每天使用超過 500 MB，請升級至付費層，以避免在達到限制時停止分析的風險。 如需詳細資訊，請參閱 [Azure 監視器記錄定價](https://azure.microsoft.com/pricing/details/log-analytics/)。
