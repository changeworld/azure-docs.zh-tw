...
- 具有[伺服器層級防火牆規則](firewall-create-server-level-portal-quickstart.md)的 [Azure SQL Database 資料庫](single-database-create-quickstart.md)
...
1. 使用 `SELECT` 陳述式查詢資料表。
    ...
    **結果**
    ...
    ![RTestData 資料表的內容](./media/r-script-create-quickstart/select-rtestdata.png)
...
1. 請執行下列 R 指令碼。 它會使用 `SELECT` 陳述式來擷取資料表中的資料、透過 R 執行階段傳遞，然後傳回資料做為資料框架。 `WITH RESULT SETS` 子句會定義為 SQL Database 傳回的資料表所具備的結構描述，並新增資料行名稱 NewColName。
    ...
    **結果**
    ...
    ![從資料表傳回資料的 R 指令碼輸出](./media/r-script-create-quickstart/r-output-rtestdata.png)
...
1. 您也可以在無輸入資料的情況下 (`@input_data_1` 設為空白)，只使用 R 指令碼產生值。
    ...
    **結果**
    ...
    ![使用 @script 做為輸入查詢結果](./media/r-script-create-quickstart/r-data-generated-output.png)
...
![R 中已安裝的套件](./media/r-script-create-quickstart/r-installed-packages.png)
...
> [使用 Azure SQL Database 機器學習服務 (預覽) 在 R 中建立和定型預測模型](r-train-score-model-create-quickstart.md)
