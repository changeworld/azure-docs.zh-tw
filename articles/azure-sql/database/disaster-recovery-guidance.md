---
title: 災害復原
description: 瞭解如何使用 Azure SQL Database 主動式異地複寫和異地還原功能，從區域資料中心中斷或失敗復原資料庫。
...
使用 [取得可復原資料庫](/previous-versions/azure/reference/dn800985(v=azure.100)) ( *LastAvailableBackupDate* ) 以取得異地複寫的最近還原點。
...
如果您應用程式的停機可能會導致商務責任，您應該使用容錯移轉群組。 這可讓應用程式在發生中斷時，在不同的區域中快速還原可用性。 如需教學課程，請參閱[實作異地分散式資料庫](geo-distributed-application-configure-tutorial.md)。
...
如需變更連接字串的詳細資訊，請參閱 [連線庫](connect-query-content-reference-guide.md#libraries)的適當開發語言。

### <a name="configure-firewall-rules"></a>設定防火牆規則

您需要確認伺服器和資料庫上設定的防火牆規則符合主要伺服器與主要資料庫上設定的防火牆規則。 如需詳細資訊，請參閱 [如何：進行防火牆設定 (Azure SQL Database)](firewall-configure.md)。
...
如需有關資料庫警示規則的詳細資訊，請參閱[接收警示通知](../../azure-monitor/platform/alerts-overview.md)和[追蹤服務健全狀況](../../service-health/service-notifications.md)。

### <a name="enable-auditing"></a>啟用稽核

如果需要稽核才能存取您的資料庫，則您必須在資料庫復原之後啟用稽核。 如需詳細資訊，請參閱[資料庫稽核 (英文)](../../azure-sql/database/auditing-overview.md)。

## <a name="next-steps"></a>後續步驟

- 若要了解 Azure SQL Database 自動備份，請參閱 [SQL Database 自動備份](automated-backups-overview.md)
- 若要了解商務持續性設計及復原案例，請參閱 [持續性案例](business-continuity-high-availability-disaster-recover-hadr-overview.md)
- 若要了解如何使用自動備份進行復原，請參閱 [從服務起始的備份還原資料庫](recovery-using-backups.md)