...
本文會逐步解說開發人員在撰寫程式碼以連接到 Azure 中的資料庫時，應注意的基本考慮。 本文適用于 Azure SQL Database 和 Azure SQL 受控執行個體。

## <a name="language-and-platform"></a>語言和平台

您可以使用各種[程式設計語言與平台](connect-query-content-reference-guide.md)連線及查詢 Azure SQL Database。 您可以找到可用來連接到資料庫的[範例應用程式](https://azure.microsoft.com/resources/samples/?service=sql-database&sort=0)。
...
Azure SQL Database 的存取受到登入及防火牆的保護。 Azure SQL Database 同時支援 SQL Server 和[Azure Active Directory 驗證](authentication-aad-overview.md)使用者和登入。 Azure Active Directory 登入僅適用于 SQL 受控執行個體。 
...
Azure SQL Database 為雲端服務，因此基礎結構或雲端實體之間的通訊可能會發生暫時性錯誤。 雖然 Azure SQL Database 會從可轉移的基礎結構失敗中復原，但這些失敗可能會影響您的連線。 當連接到 SQL Database 發生暫時性錯誤時，您的程式碼應該[重試呼叫](troubleshoot-common-connectivity-issues.md)。 我們建議重試邏輯使用輪詢邏輯，使其不會同時重試多個用戶端而造成服務的負荷。 重試邏輯取決於 [SQL Database 用戶端程式的錯誤訊息](troubleshoot-common-errors-issues.md)。

如需如何準備 Azure SQL Database 上預定維護事件的詳細資訊，請參閱[規劃 Azure SQL Database 中的 Azure 維護事件](planned-maintenance.md)。
...
- 如果您的用戶端程式是在 Azure 虛擬機器 (VM) 上執行，而用戶端程式會連線至 SQL Database，您就必須開啟該 VM 上特定的連接埠範圍。 詳細資訊： [ADO.NET 4.5 和 SQL Database 的埠超過 1433](adonet-v12-develop-direct-route-ports.md)。
