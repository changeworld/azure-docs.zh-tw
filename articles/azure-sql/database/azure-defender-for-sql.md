...
啟用適用于 SQL 的 Azure Defender 一次，以啟用所有這些包含的功能。 只要按一下，您就可以針對 Azure 中 [伺服器](logical-servers.md) 上或 SQL 受控執行個體中的所有資料庫啟用 Azure Defender。 啟用或管理 Azure Defender 設定需要屬於 [SQL 安全性管理員](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#sql-security-manager) 角色，或其中一個資料庫或伺服器管理員角色。
