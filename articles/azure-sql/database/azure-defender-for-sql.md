...
:::image type="content" source="media/azure-defender-for-sql/enable-azure-defender.png" alt-text="啟用 Azure Defender":::
...
:::image type="content" source="media/azure-defender-for-sql/security-server-settings.png" alt-text="安全性伺服器設定":::

## <a name="manage-azure-defender-settings-for-a-database"></a>管理資料庫的 Azure Defender 設定

若要覆寫特定資料庫的 Azure Defender 設定，請核取 [資料庫 **安全性中心** 設定] 中的 [在 **資料庫層級啟用 AZURE defender for SQL** ] 核取方塊。 只有當您有特定的需求，可針對個別資料庫接收個別的 Advanced 威脅防護警示或弱點評定結果，而不是針對伺服器或受控實例上的所有資料庫接收警示和結果時，才使用此選項。

選取此核取方塊之後，您就可以設定此資料庫的相關設定。

:::image type="content" source="media/azure-defender-for-sql/enable-for-database-level.png" alt-text="在資料庫層級啟用 Azure Defender":::

您的伺服器或受控實例的 azure Defender for SQL 設定也可以從 Azure Defender 資料庫窗格連線。 按一下主要 [安全性中心] 窗格中的 [ **設定** ]，然後按一下 [ **View AZURE Defender for SQL server 設定**]。
