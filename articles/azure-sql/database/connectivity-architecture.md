...
description: 本檔說明從 Azure 內或從 Azure 外部進行資料庫連接的 Azure SQL Database 連線架構。
...
本文說明將網路流量導向 Azure SQL Database 或 Azure Synapse 分析中之伺服器的各種元件架構。 它也會說明不同的連線原則，以及它如何影響從 Azure 內部連線的用戶端，以及從 Azure 外部連線的用戶端。
...
下列步驟說明如何建立連接以 Azure SQL Database：
...
如果您從 Azure 內部連線，該連線預設的連線原則為 `Redirect`。 的原則 `Redirect` 表示在建立 TCP 會話以 Azure SQL Database 之後，會將用戶端會話重新導向至正確的資料庫叢集，並變更目的地虛擬 IP 與叢集的 Azure SQL Database 閘道。 此後，所有後續封包會略過 Azure SQL Database 閘道，直接流經叢集。 下圖說明此流量。
...
下表依區域列出閘道的 IP 位址。 若要連線到 SQL Database 或 Azure Synapse，您需要允許進出區域**所有**閘道的網路流量。

如何將流量遷移至特定區域中新閘道的詳細資料，請參閱下列文章： [Azure SQL Database 流量遷移至較新的閘道](gateway-migration.md)
...
| 美國西部 2            | 13.66.226.202, 40.78.240.8  |
|                      |                    |

## <a name="next-steps"></a>後續步驟

- 如需如何變更伺服器 Azure SQL Database 連線原則的相關資訊，請參閱[conn-policy](https://docs.microsoft.com/cli/azure/sql/server/conn-policy)。
