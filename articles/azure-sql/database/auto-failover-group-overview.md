...
您可以在兩個不同的訂用帳戶中，建立 SQL 受控實例之間的容錯移轉群組。 使用 PowerShell API 時，您可以指定次要 SQL 受控執行個體的參數來執行此動作 `PartnerSubscriptionId` 。 使用 REST API 時，包含在參數中的每個實例識別碼都 `properties.managedInstancePairs` 可以有自己的 subscriptionID。
