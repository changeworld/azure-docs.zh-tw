...
| [BACKUP 命令](https://docs.microsoft.com/sql/t-sql/statements/backup-transact-sql) | 否，僅限系統起始的自動備份 - 請參閱[自動備份](automated-backups-overview.md) | 是，使用者已起始僅限複製的 Azure Blob 儲存體備份 (自動系統備份無法由使用者) 啟動-請參閱[備份差異](../managed-instance/transact-sql-tsql-differences-sql-server.md#backup) |
...
| [擴充事件 (XEvent) ](https://docs.microsoft.com/sql/relational-databases/extended-events/extended-events) | 部分請參閱 [SQL Database 中的擴充事件](xevent-db-diff-from-svr.md) | 是 - 請參閱[擴充事件差異](../managed-instance/transact-sql-tsql-differences-sql-server.md#extended-events) |
| [擴充預存程式](https://docs.microsoft.com/sql/relational-databases/extended-stored-procedures-programming/creating-extended-stored-procedures) | 否 | 否 |
...
| [登入和使用者](https://docs.microsoft.com/sql/relational-databases/security/authentication-access/principals-database-engine) | 可以，但是 `CREATE` 和 `ALTER` login 語句不會提供 (Windows 和伺服器層級 Azure Active Directory 登入) 的所有選項。 `EXECUTE AS LOGIN`不受支援-請改用 `EXECUTE AS USER` 。  | 是，但有一些[差異](../managed-instance/transact-sql-tsql-differences-sql-server.md#logins-and-users)。 Windows 登入不受支援，而且應該以 Azure Active Directory 登入來取代。 |
...
| [SQL Server Agent](https://docs.microsoft.com/sql/ssms/agent/sql-server-agent) | 否-請參閱[彈性作業 (預覽) ](elastic-jobs-overview.md) | 是 - 請參閱 [SQL Server Agent 差異](../managed-instance/transact-sql-tsql-differences-sql-server.md#sql-server-agent) |
...
| 自動調整規模 | 是，但僅適用于[無伺服器模型](serverless-tier-overview.md)。 在非伺服器模型中，服務層級的變更 (vCore、儲存體或 DTU) 的變更會快速且上線。 服務層級的變更需要最少或無停機時間。 | 否，您必須選擇保留的計算和儲存體。 服務層 (vCore 或最大儲存體) 的變更已上線，需要最少或無停機時間。 |
...
| 資源集區 | 是，身為[彈性](elastic-pool-overview.md)集區 | 是。 SQL 受控執行個體的單一實例可以有多個共用相同資源集區的資料庫。 此外，您可以將多個 SQL 受控執行個體實例部署在實例集區中， [ (預覽) ](../managed-instance/instance-pools-overview.md)可以共用資源。 |
| 線上)  (相應增加或相應減少 | 是，您可以在最短的停機時間內變更 DTU 或保留的虛擬核心或最大儲存體。 | 是，您可以在最短的停機時間內變更保留的虛擬核心或最大儲存體。 |
