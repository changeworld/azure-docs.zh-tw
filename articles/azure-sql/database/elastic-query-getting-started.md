...
ms.locfileid: "85956892"
---
# <a name="report-across-scaled-out-cloud-databases-preview"></a>跨相應放大雲端資料庫報告 (預覽)
[!INCLUDE[appliesto-sqldb](../includes/appliesto-sqldb.md)]

您可以使用[彈性查詢](elastic-query-overview.md)，從單一連接點建立多個資料庫的報表。 資料庫必須水平分割 (也稱為「分區化」)。

如果您有現有的資料庫，請參閱 [將現有的資料庫移轉到相應放大的資料庫](elastic-convert-to-use-elastic-tools.md)。

若要了解查詢所需的 SQL 物件，請參閱 [跨水平分割資料庫查詢](elastic-query-horizontal-partitioning.md)。

## <a name="prerequisites"></a>必要條件

下載並執行 [彈性資料庫工具範例入門](elastic-scale-get-started.md)。
...
1. 遵循[下載並執行範例應用](elastic-scale-get-started.md#download-and-run-the-sample-app-1)程式一節中的步驟，建立並執行**開始使用彈性資料庫工具**範例應用程式。 完成所有步驟之後，您會看到下列命令提示字元：
...
2. 在與分區安裝程式相同的伺服器中，于 Azure SQL Database 建立新的資料庫。 將資料庫命名為 "ElasticDBQuery"。
...
2. 連接至 ElasticDBQuery 資料庫，並執行下列 T-SQL 命令：
    ...
    「使用者名稱」和「密碼」應該與在**開始使用彈性資料庫工具**一文中[下載及執行範例應用程式](elastic-scale-get-started.md#download-and-run-the-sample-app)一節的步驟3中所使用的登入資訊相同。
