...
若要讓兩個受控實例參與容錯移轉群組，必須在兩個受控實例的虛擬網路之間設定 ExpressRoute 或閘道，以允許網路通訊。 如果您選擇設定 [ExpressRoute](../../expressroute/expressroute-howto-circuit-portal-resource-manager.md) ，而不是連接兩個 VPN 閘道，請直接跳至 [步驟 7](#create-a-failover-group)。  
...
1. 在 [ **設定** ] 頁面上，選取或輸入下列值，然後選取 **[確定]**：
    ...
    1. 選擇 **第二個虛擬網路閘道**的 [次要網路閘道]，例如 `secondaryGateway` 。 
    1. 選取 [ **建立雙向連接**] 旁的核取方塊。 
...
1. 在 [ **審查 + 建立** ] 頁面上，檢查雙向連線的設定，然後選取 **[確定]** 以建立您的連線。 
...
1. 移至新的 _次要_ 受控實例，然後再次選取 [ **容錯移轉** ]，將主要實例容錯回復至主要角色。 
