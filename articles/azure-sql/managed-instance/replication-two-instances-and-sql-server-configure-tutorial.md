...
1. 為 DNS 區域提供任意名稱，例如 `repldns.com`。
   ...
   ![建立私人 DNS 區域](./media/replication-two-instances-and-sql-server-configure-tutorial/create-private-dns-zone.png)
...
1. 提供 SQL Server VM 的名稱，以及私人內部 IP 位址。
   ...
   ![設定 A 記錄](./media/replication-two-instances-and-sql-server-configure-tutorial/configure-a-record.png)
...
1. 核取 [啟用自動註冊] 旁的方塊。
   ...
   ![建立 VNet 連結](./media/replication-two-instances-and-sql-server-configure-tutorial/configure-vnet-link.png)
...
1. 在 [代理程式安全性] 頁面上，選取 [安全性設定...]。提供要用於快照集代理程式以及連線至發行者的 SQL Server 登入認證。 選取 [確定]，以關閉 [快照集代理程式安全性] 頁面。 選取 [下一步] 。
   ...
   ![設定快照集代理程式安全性](./media/replication-two-instances-and-sql-server-configure-tutorial/snapshot-agent-security.png)
...
請參閱[什麼是 Azure SQL 受控執行個體功能？](sql-managed-instance-paas-overview.md#advanced-security-and-compliance)一文，以取得完整的資料庫保護方式清單。 將會討論的安全性功能如下：
