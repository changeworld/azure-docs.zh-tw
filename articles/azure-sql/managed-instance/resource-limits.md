...
|   | **Gen4** | **Gen5** |
| --- | --- | --- |
| **硬體** | Intel E5 2673 v3 (Haswell) 2.4-GHz 處理器，附加 SSD 虛擬核心 = 1 PP (實體核心) | Intel E5-2673 v4 (Broadwell) 2.3-GHz 和 Intel SP-8160 (Skylake) 處理器，快速 NVMe SSD，vCore = 1 LP (超執行緒)  |
| **虛擬核心數目** | 8 個、16 個、24 個虛擬核心 | 4、8、16、24、32、40、64、80虛擬核心 |
| **記憶體 (記憶體/核心比例) 上限** | 每個虛擬核心 7GB<br/>新增更多虛擬核心以取得更多記憶體。 | 每個虛擬核心 5.1 GB<br/>新增更多虛擬核心以取得更多記憶體。 |
...
> [!Important]
> 商務關鍵服務層級提供 SQL 受控執行個體的額外內建複本 (次要複本) ，可用於唯讀工作負載。 如果您可以分隔讀寫查詢和唯讀/分析/報告查詢，就會得到相同價格的兩倍虛擬核心和記憶體。 次要複本可能會在主要實例後的幾秒鐘後延遲，因此它的設計是要卸載不需要資料的確切目前狀態的報告/分析工作負載。 在下表中，**唯讀查詢**是在次要複本上執行的查詢。
...
|  (保留) 的實例儲存體大小上限 | -2 TB 適用于4虛擬核心 (僅限第5代) <br/>-適用于其他大小的 8 TB | 第4代： 1 TB <br/> 第 5 代： <br/>-1 TB 適用于4、8、16虛擬核心<br/>- 2 TB (適用於 24 個虛擬核心)<br/>- 4 TB (適用於 32、40、64、80 個虛擬核心) |
| 資料庫大小上限 | 根據虛擬核心) 數目，最高可達目前可用的實例大小 (最多 2 TB-8 TB。 | 根據虛擬核心) 數目，最多可達目前可用的實例大小 (上限為 1 TB-4 TB。 |
| 最大 tempDB 大小 | 受限於 24 GB/vCore (96-1920 GB) 和目前可用的實例儲存體大小。<br/>新增更多虛擬核心以取得更多 TempDB 空間。<br/> 記錄檔大小限制為 120 GB。| 最高可達目前可用的實例儲存體大小。 |
...
|  (每個實例的記錄寫入輸送量限制)  | 每個虛擬核心 3 MB/秒<br/>最大值 22 MB/秒 | 每個 vCore 4 MB/秒<br/>最大 48 MB/秒 |
...
| [唯讀複本](../database/read-scale-out.md) | 0 | 1 (包含在價格)  |
...
- 最大實例 IOPS 取決於工作負載的檔案配置和散發。 例如，如果您建立 7 x 1TB 檔案，其中每個和7個小型檔案 (小於 128 GB) 且每個都有 500 IOPS，則您可以取得每個實例 38500 IOPS (7x5000 + 7x500) 如果您的工作負載可以使用所有檔案。 請注意，某些 IOPS 也會用於自動備份。
...
此外，最大記錄寫入輸送量的實例層級限制 (是 22 MB/s) ，因此您可能無法在整個記錄檔上連線到 max 檔案，因為您已達到實例輸送量限制。
...
> [!Note]
> 如需訂用帳戶的區域可用性的最新資訊，請先參閱[官方 COVID-19 blog 文章](https://aka.ms/sqlcapacity)。

支援的訂用帳戶類型可包含有限的每一區域資源數目。 根據訂用帳戶類型的類型，SQL 受控執行個體在每個 Azure 區域中都有兩個預設限制 (可隨需增加，方法是在 Azure 入口網站中建立特殊的[支援要求](../database/quota-increase-request.md)：
...
下表顯示支援的訂用帳戶類型的**預設地區限制** (預設限制可以使用以下所述的支援要求來延伸) ：

|訂用帳戶類型| SQL 受控執行個體子網的數目上限 | VCore 單位數目上限 * |
| :---| :--- | :--- |
|隨用隨付|3|320|
|CSP |在某些區域中為 8 (15 * * ) |在某些區域中，960 (1440 * * ) |
|隨用隨付開發/測試|3|320|
|Enterprise 開發/測試|3|320|
|EA|在某些區域中為 8 (15 * * ) |在某些區域中，960 (1440 * * ) |
|Visual Studio 企業版|2 |64|
|Visual Studio Professional 和 MSDN 平臺|2|32|

\*在規劃部署中，請考慮商務關鍵性 (BC) 服務層級所需的時間比一般用途 (GP) 服務層級多四 (4) 倍。 例如：1個 GP vCore = 1 個 vCore unit 和1個 BC vCore = 4 個 vCore 單位。 為了簡化您對預設限制的耗用量分析，請摘要說明部署 SQL 受控執行個體區域中所有子網的 vCore 單位，並將結果與訂用帳戶類型的實例單位限制進行比較。 **VCore 單位限制的最大數目**適用于區域中的每個訂用帳戶。 每個個別子網沒有限制，不同之處在于跨多個子網部署的所有虛擬核心總和必須小於或等於**vCore 單位的最大數目**。
