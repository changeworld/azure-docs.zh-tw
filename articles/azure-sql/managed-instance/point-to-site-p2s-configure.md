...
description: 從內部部署用戶端電腦使用點對站連線，以使用 SQL Server Management Studio (SSMS) 連接到 Azure SQL 受控執行個體。
...
ms.locfileid: "91325074"
---
# <a name="quickstart-configure-a-point-to-site-connection-to-azure-sql-managed-instance-from-on-premises"></a>快速入門：設定從內部部署至 Azure SQL 受控執行個體的點對站連線
[!INCLUDE[appliesto-sqlmi](../includes/appliesto-sqlmi.md)]

本快速入門示範如何透過點對站連線，在內部部署用戶端電腦上使用 [SQL Server Management Studio](https://docs.microsoft.com/sql/ssms/sql-server-management-studio-ssms) (SSMS) 來連線至 Azure SQL 受控執行個體。 如需點對站連線的詳細資訊，請參閱 [關於點對站 VPN](../../vpn-gateway/point-to-site-about.md)。

## <a name="prerequisites"></a>Prerequisites

此快速入門：

- 使用建立  [受控實例](instance-create-quickstart.md) 中建立的資源作為起點。
...
- 需要最新版的內部部署用戶端電腦上的 [SQL Server Management Studio](https://docs.microsoft.com/sql/ssms/sql-server-management-studio-ssms) 。

## <a name="attach-a-vpn-gateway-to-a-virtual-network"></a>將 VPN 閘道連結至虛擬網路

1. 在您的內部部署用戶端電腦上開啟 PowerShell。

2. 複製此 PowerShell 指令碼。 此腳本會將 VPN 閘道附加至您在 [建立受控實例](instance-create-quickstart.md) 快速入門中建立的 SQL 受控執行個體虛擬網路。 此腳本會使用 Azure PowerShell Az 模組，並針對 Windows 或 Linux 主機進行下列動作：

   - 在用戶端電腦上建立並安裝憑證
   - 計算未來的 VPN 閘道子網 IP 範圍
   - 建立閘道子網
   - 部署將 VPN 閘道附加至 VPN 子網的 Azure Resource Manager 範本
...
3. 在 PowerShell 視窗中貼上指令碼，並提供必要的參數。 `<subscriptionId>`、和的值 `<resourceGroup>` `<virtualNetworkName>` 應與您用於[建立受控實例](instance-create-quickstart.md)快速入門的值相符。 `<certificateNamePrefix>` 的值可以是您自己選擇的字串。
...
## <a name="create-a-vpn-connection"></a>建立 VPN 連接
...
4. 在您的內部部署用戶端電腦上，從 zip 檔案解壓縮檔案，然後以解壓縮檔案開啟資料夾。
...
8. 在參考虛擬網路的對話方塊中，選取 **[是]** ，為您的虛擬網路安裝 VPN 用戶端。

## <a name="connect-to-the-vpn-connection"></a>連線到 VPN 連線

1. 移至內部部署用戶端電腦上**網路 & 網際網路**中的**VPN** ，然後選取您的 SQL 受控執行個體虛擬網路以建立與此 VNet 的連線。 在下圖中，VNet 的名稱為 **MyNewVNet**。
...
4. 當系統提示您連線管理員需要較高的許可權來更新您的路由表時，請選擇 [ **繼續**]。
5. 在 [使用者帳戶控制] 對話方塊中，按選取 [是]**** 以繼續。

   您已建立連至 SQL 受控執行個體 VNet 的 VPN 連線。
...
2. 在 [**連接到伺服器**] 對話方塊中，于 [**伺服器名稱**] 方塊中輸入受控實例的完整**主機名稱**。
3. 選取 [SQL Server 驗證]****，提供您的使用者名稱和密碼，然後選取 [連線]****。

    ![SSMS 連接](./media/point-to-site-p2s-configure/ssms-connect.png)  
