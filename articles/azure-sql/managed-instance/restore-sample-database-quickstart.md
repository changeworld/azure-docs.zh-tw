...
description: 在本快速入門中，了解如何使用 SQL Server Management (SSMS) 將資料庫備份還原至 Azure SQL SQL 受控執行個體。
...
ms.locfileid: "84267574"
---
# <a name="quickstart-restore-a-database-to-an-azure-sql-managed-instance-with-ssms"></a>快速入門：使用 SSMS 將資料庫還原至 Azure SQL 受控執行個體
...
> [!NOTE]
> 如需使用 Azure 資料庫移轉服務 (DMS) 進行移轉的詳細資訊，請參閱[使用 DMS 的 SQL 受控執行個體移轉](../../dms/tutorial-sql-server-to-managed-instance.md)。
...
- 使用[建立 SQL 受控執行個體](instance-create-quickstart.md)快速入門的資源。
- 需要已安裝最新版的 [SQL Server Management Studio (SSMS)](https://docs.microsoft.com/sql/ssms/sql-server-management-studio-ssms)。
- 需要使用 SSMS 以連線到您的 SQL 受控執行個體。 請參閱這些快速入門以了解如何連線：
...
在 SQL Server Management Studio (SSMS) 中，遵循這些步驟以將 Wide World Importers 資料庫還原至您的 SQL 受控執行個體。 資料庫備份檔案儲存在預先設定的 Azure Blob 儲存體帳戶中。

1. 開啟 SSMS 並連線到您的 SQL 受控執行個體。
2. 在 [物件總管] 中，以滑鼠右鍵按一下您的 SQL 受控執行個體，並選取 [新增查詢] 以開啟新的查詢視窗。
3. 執行下列 SQL 指令碼，其會使用預先設定的儲存體帳戶和 SAS 金鑰，在您的 SQL 受控執行個體中[建立認證](https://docs.microsoft.com/sql/t-sql/statements/create-credential-transact-sql)。
...
5. 執行下列程式碼以還原 Wide World Importers 資料庫。
   ...
   ```
    ...
    ![還原](./media/restore-sample-database-quickstart/restore.png)
