---
title: Azure 雲端服務 (延伸支援) Def WebRole 架構 |Microsoft Docs
description: '雲端服務 web 角色的相關資訊 (延伸支援) '
ms.topic: article
...
ms.locfileid: "98744324"
---
# <a name="azure-cloud-services-extended-support-definition-webrole-schema"></a>Azure 雲端服務 (延伸支援) 定義 WebRole 架構
...
|vmsize|字串|選擇性。 設定此值可變更對角色所配置的虛擬機器大小。 預設值是 `Small`。 如需詳細資訊，請參閱[雲端服務的虛擬機器大小](available-sizes.md)。|  
...
|sizeInMb|int|選擇性。 想要配置給本機存放區的儲存體空間數量，以 MB 為單位。 若未指定，則預設配置的儲存體空間是 100 MB。 可配置的儲存體空間最小數量為 1 MB。<br /><br /> 本機資源最大的大小取決於虛擬機器大小。 如需詳細資訊，請參閱[雲端服務的虛擬機器大小](available-sizes.md)。|  
...
`Certificates` 元素會說明 Web 角色之憑證的集合。 此元素是 `Certificate` 元素的父代。 一個角色可以有任意數目的相關聯憑證。 如需使用 certificates 元素的詳細資訊，請參閱[使用憑證修改服務定義檔](../cloud-services/cloud-services-configure-ssl-certificate-portal.md#step-2-modify-the-service-definition-and-configuration-files)。
...
| 屬性 | 類型 | 描述 |  
| --------- | ---- | ----------- |  
|moduleName|字串|必要。 要匯入之模組的名稱。 有效的匯入模組為：<br /><br /> -   RemoteAccess<br />-   RemoteForwarder<br />-   Diagnostics<br /><br /> RemoteAccess 和 RemoteForwarder 模組可讓您設定遠端桌面連線的角色執行個體。 如需詳細資訊，請參閱 [擴充](extensions.md)功能。<br /><br /> 診斷模組可讓您收集角色執行個體的診斷資料。|  
...
| 屬性 | 類型 | 描述 |  
| --------- | ---- | ----------- |  
|xpath|字串|選擇性。 執行個體之部署設定的位置路徑。 如需詳細資訊，請參閱[組態變數與 XPath](../cloud-services/cloud-services-role-config-xpath.md)。<br /><br /> 您必須包含值屬性或 `RoleInstanceValue` 元素。|  
...
##  <a name="site"></a><a name="Site"></a> 網站  
...
|physicalDirectory|字串|網站根目錄之內容目錄的位置。 您可以將位置指定為絕對路徑或相對於已進行的位置。|  
...
`Bindings` 元素會說明網站之繫結的集合。 它是 `Binding` 元素的父代元素。 每個 `Site` 元素都需要此元素。 如需如何設定端點的詳細資訊，請參閱[啟用角色執行個體的通訊](../cloud-services/cloud-services-enable-communication-role-instances.md)。
...
`Startup` 元素會說明角色啟動時所執行之工作的集合。 此元素可以是 `Variable` 元素的父代。 如需如何使用角色啟動工作的詳細資訊，請參閱[如何設定啟動工作](../cloud-services/cloud-services-startup-tasks.md)。 這是選用元素，一個角色只能有一個啟動區塊。
