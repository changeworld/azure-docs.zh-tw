...
> [!NOTE]
> 有關如何處理通信堆疊的詳細資訊，請參閱與 ASP.NET[核心的服務通信](service-fabric-reliable-services-communication-aspnetcore.md)
...
選擇 **.NET 核心 2.0 ->有狀態服務**，並將其命名為*HelloWorldState。* 按一下 [確定]****。
...
具狀態服務與無狀態服務具有相同的進入點。 主要區別是狀態提供程式的可用性，該*提供程式*可以可靠地存儲狀態。 Service Fabric 附帶一個稱為["可靠集合](service-fabric-reliable-services-reliable-collections.md)"的狀態提供程式實現，它允許您通過可靠狀態管理員創建複製的資料結構。 具狀態可靠服務預設會使用此狀態供應器。
...
* 當您在可靠的集合上認可交易時，物件會複寫以獲得高可用性。 儲存在可靠集合中的物件會保留在服務中的本機記憶體。 這表示您有物件的本機參考。
   ...
   很重要的一點是，您不要改變那些物件的本機執行個體而不在交易中的可靠集合上執行更新作業。 這是因為不會自動複寫對本機物件執行個體所做的變更。 必須重新將物件重新插入字典，或使用*字典上的更新*方法之一。
...
除了語言集成查詢 （LINQ）之外`System.Collections.Generic`，`System.Collections.Concurrent`可靠集合具有許多與其和對口相同的操作。 可靠的集合上的作業是非同步的。 這是因為具備可靠集合的寫入作業執行 I/O 作業以將資料複寫並保存至磁碟。
