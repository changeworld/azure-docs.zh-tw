...
|GetCodePackageActivationContextTimeout|時間範圍，預設值為 Common::TimeSpan::FromSeconds(120)|動態|以秒為單位指定時間範圍。 CodePackageActivationContext 呼叫的逾時值。 這不適用於臨時服務。 |
...
|PartitionCount |整數，預設值為 3 |不允許|要建立之命名服務存放區的資料分割數目。 每個分區都擁有一個與其索引對應的分區鍵;所以分區鍵 [0;分區計數* 存在。 增加命名服務資料分割數目會增加命名服務可執行的級別，其方法是減少任何備用複本集所保有的平均資料量，代價則是資源的使用率會增加 (因為必須維持 PartitionCount*ReplicaSetSize 個服務複本)。|
...
|MoveNextFabricUpgradeDomain |字串，預設值為 "Admin" |動態| 使用顯式升級域恢復群集升級的安全配置。 |
