---
title: Azure 服務結構 CLI-sfctl 節點
description: 瞭解 sfctl，Azure 服務結構命令列介面。 包括用於管理叢集節點的命令清單。
author: jeffj6123
ms.topic: reference
ms.date: 1/16/2020
ms.author: jejarry
...
## <a name="sfctl-node-add-configuration-parameter-overrides"></a>sfctl 節點附加配置參數覆蓋
在指定的節點上添加配置覆蓋的清單。

此 api 允許在指定的節點上添加所有現有配置覆蓋。

### <a name="arguments"></a>引數

|引數|描述|
| --- | --- |
| --配置參數覆蓋清單 [必需] | 用於添加配置覆蓋清單的說明。 |
| --節點名稱 [必需] | 節點的名稱。 |
| --force | 強制添加配置覆蓋指定節點。 |
...
## <a name="sfctl-node-get-configuration-overrides"></a>sfctl 節點獲取配置覆蓋
獲取指定節點上的配置覆蓋清單。

此 api 允許在指定的節點上獲取所有現有配置覆蓋。
...
## <a name="sfctl-node-remove-configuration-overrides"></a>sfctl 節點刪除配置覆蓋
刪除指定節點上的配置覆蓋。

此 api 允許刪除指定節點上的所有現有配置覆蓋。
...
這意謂著無法復原該節點的永續性狀態。 通常如果已將硬碟抹除乾淨或硬碟損毀，就會發生這種情況。 節點必須停止運作，此作業才能成功。 此作業可讓 Service Fabric 知道該節點上的複本已不存在，而 Service Fabric 應該停止等候這些複本恢復運作。 如果節點上的狀態尚未移除，而節點能夠在其狀態保持不變的情況下恢復運作，則請勿執行此 Cmdlet。 從 Service Fabric 6.5 開始，為了將此 API 用於種子節點，請將種子節點更改為常規（非種子）節點，然後調用此 API 以刪除節點狀態。 如果群集在 Azure 上運行，則種子節點關閉後，Service Fabric 將嘗試將其自動更改為非種子節點。 為此，請確保主節點類型中的非種子節點數不小於向下種子節點的數量。 如有必要，向主節點類型添加更多節點以實現此目的。 對於獨立群集，如果"向下"種子節點不能完整地恢復其狀態，請從群集中刪除該節點，請參閱 HTTPs\:/docs.microsoft.com/en-us/azure/service-fabric/service-fabric-cluster-windows-server-add-remove-nodes。
