...
> [!NOTE]
> 基本 SKU 負載平衡器的限制可防止新增額外的擴展集。 建議改為使用標準 SKU 負載平衡器。 如需詳細資訊，請參閱 [這兩個 sku 的比較](/azure/load-balancer/skus)。
...
若要部署更新的設定，您必須先取得 Key Vault 中儲存的叢集憑證的幾個參考。 尋找這些值最簡單的方式是透過 Azure 入口網站。 您將需要：
