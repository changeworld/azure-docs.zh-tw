...
在 Windows Server 獨立叢集上，您可以使用*RunAs*原則，以*群組受管理的服務帳戶*（gMSA）來執行服務。  根據預設，Service Fabric 應用程式會在執行`Fabric.exe`進程的帳戶下執行。 即使在共用主控環境中，以不同帳戶執行應用程式能避免彼此干擾。 使用 gMSA，就不需將密碼或加密的密碼儲存於應用程式資訊清單中。  您也可以利用 [Active Directory 使用者或群組](service-fabric-run-service-as-ad-user-or-group.md)身分執行服務。
