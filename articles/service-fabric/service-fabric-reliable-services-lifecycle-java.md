---
title: Azure 服務結構可靠服務生命週期
description: 使用 JAVA 進行有狀態和無狀態服務，瞭解 Azure 服務結構可靠服務應用程式中的生命週期事件。
...
Service Fabric 引發的異常可以是永久[（`FabricException`）](https://docs.microsoft.com/java/api/system.fabric.exception)或瞬態[（`FabricTransientException`） 。](https://docs.microsoft.com/java/api/system.fabric.exception.fabrictransientexception) 應該記錄並擲回永久例外狀況。 可以根據重試邏輯重試暫時性例外狀況。
