...
### <a name="design-time-service-type-service-package-and-manifest-application-type-application-package-and-manifest"></a>設計階段：服務類型、服務包和清單、應用程式類型、應用程式包和清單
服務類型是指派給服務的程式碼封裝、資料封裝及組態封裝的名稱/版本。 這在 ServiceManifest.xml 檔中定義。 服務類型由可執行代碼和服務配置設置（在運行時載入）和服務配置設置以及服務使用的靜態資料組成。

服務封裝是一個磁碟目錄，其中包含服務類型的 ServiceManifest.xml 檔案，此檔案會參考服務類型的程式碼、靜態資料及組態封裝。 例如，服務套件可能會參考構成資料庫服務的程式碼、靜態資料和組態封裝。

應用程式類型是指派給服務類型集合的名稱/版本。 這在應用程式清單.xml 檔中定義。

![Service Fabric 應用程式類型和服務類型][cluster-imagestore-apptypes]

應用程式包是一個磁碟目錄，它包含應用程式類型的應用程式類型的應用程式清單.xml 檔，該檔引用構成應用程式類型的每個服務類型的服務包。 例如，電子郵件應用程式類型的應用程式封裝可能包含指向佇列服務封裝、前端服務封裝、資料庫服務封裝的參考。  

應用程式封裝目錄中的檔案會複製到 Service Fabric 叢集的映像存放區。 您可以接著從這個應用程式類型建立一個具名應用程式，然後在叢集內執行該應用程式。 創建命名應用程式後，可以從應用程式類型的一種服務類型創建命名服務。 
...
[Service Fabric 群集](service-fabric-deploy-anywhere.md)是一組網路連接的虛擬或物理電腦，用於部署和管理微服務。 叢集可擴充至數千部機器。
...
[分區](service-fabric-concepts-partitioning.md)對於服務結構並非唯一。 資料分割是一種知名的資料分割形式，也稱為分區化。 含有大量狀態的具狀態服務，會跨資料分割切割其資料。 每個資料分割會負責服務完整狀態的一部分。 
...
在分割內，無狀態的具名服務會有執行個體，而具狀態的具名服務則有複本。 通常，無狀態具名服務只會有 1 個分割，因為它們有沒有內部狀態。 分區實例提供[可用性](service-fabric-availability-services.md)。 若某個執行個體失敗，其他執行個體會繼續正常運作，接著 Service Fabric 會建立新的執行個體。 具狀態的具名服務會在複本中維持其狀態，且每個資料分割都有自己的複本集。 讀取與寫入作業會在單一複本上執行 (稱為「主要複本」)。 從寫入作業對狀態進行的變更，會複寫至多個其他複本 (稱為「作用中次要複本」)。 複本失敗失敗時，Service Fabric 會從現有複本建立新的複本。
...
Service Fabric 的一個關鍵區別是，它專注于構建有狀態服務，無論是內置[程式設計模型](service-fabric-choose-framework.md)還是容器化狀態服務。 [應用程式案例](service-fabric-application-scenarios.md)說明使用具狀態服務的案例。
...
[可靠服務](service-fabric-reliable-services-introduction.md)是一個羽量級框架，用於編寫與 Service Fabric 平臺集成的服務，並受益于全套平臺功能。 Reliable Services 可以是無狀態的 (類似大多數服務平台，例如「Azure 雲端服務」中的 Web 伺服器或「背景工作角色」)，其中狀態會保存在外部解決方案中，例如 Azure DB 或「Azure 資料表儲存體」。 Reliable Services 也可以是具狀態的，其狀態是使用 Reliable Collections 直接保存在服務本身中。 狀態透過複寫變得[高度可用](service-fabric-availability-services.md)，並透過[資料分割](service-fabric-concepts-partitioning.md)散發，全由 Service Fabric 自動管理。

### <a name="reliable-actors"></a>Reliable Actors
["可靠參與者"](service-fabric-reliable-actors-introduction.md)框架建立在可靠服務之上，是一個應用程式框架，它基於執行元件設計模式實現虛擬執行元件模式。 Reliable Actor 架構使用獨立的計算單位，和以單一執行緒方式執行稱為動作項目的狀態。 Reliable Actor 架構提供動作項目的內建通訊，以及預先設定狀態持續性和相應放大組態。
...
為了建立真正雲端級別的服務，確保應用程式和服務可以禁得起真實世界失敗考驗便至關重要。 錯誤分析服務的設計用意，是測試建置於 Service Fabric 的服務。 使用[故障分析服務](service-fabric-testability-overview.md)，您可以引發有意義的故障，並針對應用程式運行完整的測試方案。 這些錯誤及案例會在受控制、安全且一致的情況下，執行及驗證服務在其生命週期會發生的各種狀態和轉換情形。
...
[方案](service-fabric-testability-scenarios.md)是由一個或多個操作組成的複雜操作。 錯誤分析服務提供兩個內建的完整案例︰
...
我們可藉由在 Azure 上執行 Service Fabric 叢集，與其他的 Azure 功能和服務整合，而能夠更加輕鬆可靠地操作及管理叢集。 叢集為 Azure Resource Manager 資源，因此您可如同 Azure 中的其他任何資源般建立叢集模型。 Resource Manager 亦可輕鬆管理由叢集以單一單位方式使用的所有資源。 Azure 上的群集與 Azure 診斷和 Azure 監視器日誌集成。 叢集節點類型是[虛擬機器擴展集](/azure/virtual-machine-scale-sets/index)，因此內建自動調整功能。
...
獨立叢集是由您完全擁有的資源。 由您負責修補基礎 OS 和起始網狀架構升級。 如果群集可以連接到[https://www.microsoft.com/download](https://www.microsoft.com/download)，則可以將群集設置為自動下載和預配新的 Service Fabric 運行時包。 接著您即會起始升級。 如果群集無法訪問[https://www.microsoft.com/download](https://www.microsoft.com/download)，則可以從 Internet 連接的電腦手動下載新的運行時包，然後啟動升級。 如需詳細資訊，請參閱[升級獨立 Service Fabric 叢集](service-fabric-cluster-upgrade-windows-server.md)。
...
Service Fabric 元件會針對叢集中的所有實體，提供現成的報告。 [系統運行狀況報告](service-fabric-understand-and-troubleshoot-with-system-health-reports.md)提供群集和應用程式功能的可見度，並通過運行狀況標記問題。 系統健康狀態報告會針對應用程式和服務來確認實體是否已實作，並從 Service Fabric 執行階段的角度來確認其是行為是否正確。 這些報告不提供對服務的業務邏輯的任何運行狀況監視，也不檢測已停止回應的進程。 若要將特定的健康狀態資訊新增至服務的邏輯，請在服務中[實作自訂健康狀態報告](service-fabric-report-health.md)。

Service Fabric 提供多種[健康狀態報告檢視](service-fabric-view-entities-aggregated-health.md)方式，且會將此報告彙總於健康狀態資料存放區：
* [服務結構資源管理器](service-fabric-visualizing-your-cluster.md)或其他視覺化檢視。
