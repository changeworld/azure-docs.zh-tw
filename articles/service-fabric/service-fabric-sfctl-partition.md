...
description: 瞭解 Azure Service Fabric 命令列介面的 sfctl。 包含用來管理服務之分割區的命令清單。
author: jeffj6123
ms.topic: reference
ms.date: 1/16/2020
ms.author: jejarry
...
- PartialDataLoss：只會移除複本的仲裁，並針對分割區觸發 OnDataLoss，但實際資料遺失取決於是否有進行中的複寫。  
- Fulldataloss 將：移除所有複本，因此會遺失所有資料，並觸發 OnDataLoss。 應該只使用具狀態服務作為目標來呼叫此 API。 不建議使用系統服務作為目標來呼叫此 API。
