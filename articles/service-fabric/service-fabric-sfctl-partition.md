---
title: Azure 服務結構 CLI-sfctl 分區
description: 瞭解 sfctl，Azure 服務結構命令列介面。 包括用於管理服務分區的命令清單。
author: jeffj6123
ms.topic: reference
ms.date: 1/16/2020
ms.author: jejarry
...
- 部分資料丟失：僅刪除副本的仲裁，並為分區觸發 OnDataLoss，但實際資料丟失取決於動態複製的存在。  
- 完全資料丟失：所有副本都將被刪除，因此所有資料都將丟失並觸發 OnDataLoss。 應該只使用具狀態服務作為目標來呼叫此 API。 不建議使用系統服務作為目標來呼叫此 API。
