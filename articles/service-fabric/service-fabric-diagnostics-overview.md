...
* EventStore - EventStore 是平台所提供的功能，提供可在 Service Fabric Explorer 中和透過 REST API 取得的 Service Fabric 平台事件。 您可以看到快照集檢視，其中顯示每個實體 (例如節點、服務、應用程式) 在叢集中發生什麼情況，並根據事件的時間進行查詢。 您還可以在[事件商店概述中閱讀有關事件存儲的詳細資訊](service-fabric-diagnostics-eventstore.md)。    

![EventStore](media/service-fabric-diagnostics-overview/eventstore.png)

所提供的診斷預設便是以一組詳盡事件的形式提供。 這些[服務結構事件](service-fabric-diagnostics-events.md)說明了平臺在不同實體（如節點、應用程式、服務、分區等）上執行的操作。在上述最後一個方案中，如果節點關閉，平臺將發出事件`NodeDown`，並且可以通過您選擇的監視工具立即通知您。 其他常見的範例包括容錯移轉期間的 `ApplicationUpgradeRollbackStarted` 或 `PartitionReconfigured`。 **Windows 和 Linux 叢集上都會提供相同的事件。**

這些事件會透過 Windows 和 Linux 上的標準通道傳送，並可由任何支援這些通道的監視工具讀取。 Azure 監視器解決方案是 Azure 監視器日誌。 請隨意閱讀有關 Azure[監視器日誌集成](service-fabric-diagnostics-event-analysis-oms.md)的更多內容，該集成包括群集的自訂操作儀表板以及一些依例查詢，從中可以創建警報。 如需更多叢集監視概念，請參閱[平台層級事件和記錄產生](service-fabric-diagnostics-event-generation-infra.md)。
...
Service Fabric 還為可靠服務和參與者程式設計模型提供了一組效能計數器。 如果您使用上述其中一種模型，這些效能計數器可以提供資訊，以協助確保您的動作項目正確向上和向下微調，或者您的可靠服務要求處理的速度夠快。 如需詳細資訊，請參閱[可靠服務遠端的監視](service-fabric-reliable-serviceremoting-diagnostics.md#performance-counters)和 [Reliable Actors 的效能監視](service-fabric-reliable-actors-diagnostics.md#performance-counters)。 

收集這些日誌的 Azure 監視器解決方案是 Azure 監視器日誌，類似于平臺級別的監視。 應使用[日誌分析代理](service-fabric-diagnostics-oms-agent.md)收集相應的效能計數器，並在 Azure 監視器日誌中查看它們。
...
* 使用[診斷代理](service-fabric-diagnostics-event-aggregation-wad.md)和[Azure 監視器日誌](service-fabric-diagnostics-oms-setup.md)進行群集監視
* 使用[Azure 監視器日誌](service-fabric-diagnostics-oms-agent.md)進行基礎結構監視
...
儘管我們建議的兩種解決方案[Azure 監視器日誌](service-fabric-diagnostics-event-analysis-oms.md)和[應用程式見解](service-fabric-diagnostics-event-analysis-appinsights.md)已內置與 Service Fabric 集成，但許多事件都是通過 ETW 提供程式編寫的，並且與其他日誌記錄解決方案一起擴展。 您也應該研究 [Elastic Stack](https://www.elastic.co/products) (尤其是當您考慮在離線環境中執行叢集時)、[Dynatrace](https://www.dynatrace.com/)，或您偏好的其他任何平台。 我們在[這裡](service-fabric-diagnostics-partners.md)提供一份整合的合作夥伴清單。

不論選擇何種平台，重點在於您對於其所提供的使用者介面、查詢能力、可用的自訂視覺化與儀表板，以及可強化監視體驗的其他工具所抱持的感受。 

## <a name="next-steps"></a>後續步驟

* 有關開始檢測應用程式，請參閱[應用程式級事件和日誌生成](service-fabric-diagnostics-event-generation-app.md)。
...
* 詳細瞭解如何監視平臺和 Service Fabric 在[平臺級事件和日誌生成](service-fabric-diagnostics-event-generation-infra.md)中為您提供的事件。
* 在[為群集設置 Azure 監視器日誌](service-fabric-diagnostics-oms-setup.md)時配置 Azure 監視器日誌與服務結構集成
