...
### <a name="overview"></a>概觀
Service Fabric 叢集具有 [[可靠性層級](./service-fabric-cluster-capacity.md#reliability-characteristics-of-the-cluster)] 屬性，可用來決定在叢集的主要節點類型上執行的系統服務複本數目。 必要的複本數目將決定必須在叢集的主要節點類型中維護的節點數目下限。 如果主要節點類型中的節點數目低於可靠性層級的最低需求，叢集將會變得不穩定。  
...
* 如果可靠性層級為銅，請遵循下列[步驟](./service-fabric-cluster-scale-in-out.md#manually-remove-vms-from-a-node-typevirtual-machine-scale-set)，正常地在您的叢集中進行調整。
