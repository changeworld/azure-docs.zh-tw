...
description: 瞭解 Azure Service Fabric 命令列介面的 sfctl。 包含用來管理叢集的命令清單。
author: jeffj6123
ms.topic: reference
ms.date: 1/16/2020
ms.author: jejarry
...
取得 Service Fabric 叢集資訊清單。 叢集資訊清單包含叢集的屬性，其中包含叢集上不同的節點類型、安全性設定、錯誤和升級網域拓撲等等。部署獨立叢集時，會將這些屬性指定為檔案 ClusterConfig.JS的一部分。 不過，叢集資訊清單中大部分的資訊都會由服務網狀架構在於其他部署案例 (例如使用 Azure 入口網站時) 中進行叢集部署的期間，從內部產生。 叢集資訊清單的內容僅供參考之用，使用者不應該依賴檔案內容或其解釋的格式。
...
|引數|說明|
| --- | --- |
| --state-filter | 針對使用者引發的作業，用來依據 OperationState 進行篩選。 -65535-全選-1-選取執行-2-選取 RollingBack-8-選取已完成-16-選取錯誤-32-選取已取消-64-選取 ForceCancelled。  預設值\:65535。 |
| --timeout -t | 用來執行作業的伺服器超時（以秒為單位）。 此超時時間指定用戶端願意等候要求的作業完成的持續時間。 此參數的預設值為60秒。  預設值\: 60。 |
...
傳送 Service Fabric 叢集上的健康情況報告。 此報告必須包含健康情況報告來源及報告所針對屬性的相關資訊。 此報告會傳送給 Service Fabric 閘道節點，再由其轉送給健康狀態資料存放區。 閘道可能會接受此報告，但健康狀態資料存放區可能在進行額外驗證後會予以拒絕。 例如，健康狀態資料存放區可能會因為參數無效 (例如序號過時) 而拒絕此報告。 若要查看健康狀態存放區中是否已套用此報告，請執行 GetClusterHealth，並檢查報表是否出現在 [HealthEvents] 區段中。
...
如果連線到安全的叢集，請指定憑證 (.crt) 和金鑰檔案 (.key) 的絕對路徑，或包含兩者之單一檔案 (.pem) 的絕對路徑。 請勿指定兩者。 您也可以在連線到安全的叢集時，視需要一併指定 CA 組合檔案的絕對路徑，或是受信任 CA 憑證目錄的絕對路徑。  沒有先執行此命令的叢集連接，包括與 localhost 的連接。 但是，不需要明確的端點就能連接到本機叢集。  如果使用自我簽署的憑證，或其他未由知名 CA 簽署的憑證，請傳入--CA 參數，以確保通過驗證。 如果不在生產環境叢集中，若要略過用戶端驗證 (適用于自我簽署或不知名的 CA 簽署) ，請使用--no-verify 選項。 雖然可能，但不建議用於生產叢集。 否則可能會產生憑證驗證錯誤。
...
| --ca | 要視為有效 CA 憑證目錄的絕對路徑，或 CA 組合檔案的絕對路徑。 如果使用 CA 憑證的目錄，OpenSSL 提供的 `c_rehash <directory>` 必須先執行以計算憑證雜湊，並建立適當的 symbolics 連結。 這是用來驗證叢集所傳回的憑證是否有效。 |
| --cert | 用戶端憑證檔案的絕對路徑。 |
| --端點 | 叢集端點 URL，包括連接埠和 HTTP 或 HTTPS 前置詞。 一般而言，端點看起來會像是 HTTPs \: //<您的 url>\: 19080。 如果沒有指定端點，則會預設為 HTTP \: //localhost \: 19080。  預設 \: HTTP \: //localhost \: 19080。 |
| --key | 用戶端憑證金鑰檔案的絕對路徑。 |
