...
> | MaxPercentUnhealthyDeployedApplications|PS, VS |預設值和建議值為 0。 指定應用程式被視為不健康和更新失敗之前，應用程式不健康之已部署的應用程式數目上限 (請參閱 [健康狀態章節](service-fabric-health-introduction.md))。 這個參數定義節點上的應用程式健康狀態，協助在升級期間偵測問題。 應用程式複本通常會負載平衡至其他節點，讓應用程式健康狀態良好，以便繼續升級。 藉由指定嚴格的 *MaxPercentUnhealthyDeployedApplications* 健全狀況，Service Fabric 可以迅速偵測出應用程式封裝的問題，並協助產生失敗的快速升級。 |
...
| 參數 | 描述 |
| --- | --- |
default-service-health-policy | 預設用來評估服務類型健康情況之健康原則的[JSON](/rest/api/servicefabric/sfclient-model-servicetypehealthpolicy)編碼規格。 對應預設是空的。 |
...
upgrade-domain-timeout | 執行 *FailureAction* 之前，每個升級網域必須完成的時間量。 它會先解譯為代表 ISO 8601 持續時間的字串。 如果失敗，則會解譯為代表總毫秒數的數字。 預設值為永不 (無限)，您應該針對您的應用程式適當地加以自訂。 預設值：P10675199DT02H48M05.4775807S。 |
upgrade-timeout | 執行 *FailureAction* 之前，每個升級網域必須完成的時間量。 它會先解譯為代表 ISO 8601 持續時間的字串。 如果失敗，則會解譯為代表總毫秒數的數字。 預設值為永不 (無限)，您應該針對您的應用程式適當地加以自訂。 預設值：P10675199DT02H48M05.4775807S。|
