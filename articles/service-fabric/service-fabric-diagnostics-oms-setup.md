---
title: 使用 Azure 監視器日誌設置監視
description: 瞭解如何設置 Azure 監視器日誌以視覺化和分析事件以監視 Azure 服務結構群集。
author: srrengar
ms.topic: conceptual
ms.date: 02/20/2019
ms.author: srrengar
...
---
# <a name="set-up-azure-monitor-logs-for-a-cluster"></a>為群集設置 Azure 監視器日誌

建議您使用 Azure 監視器記錄來監視叢集層級事件。 您可以透過 Azure Resource Manager、PowerShell 或 Azure Marketplace 來設定 Log Analytics 工作區。 如果維護部署的更新資源管理器範本供將來使用，請使用同一範本來設置 Azure 監視器日誌環境。 如果已經在啟用診斷的情況下部署叢集，則透過 Marketplace 部署會更輕鬆。 如果您要用來部署的帳戶中沒有訂用帳戶層級的存取權，請使用 PowerShell 或 Resource Manager 範本進行部署。

> [!NOTE]
> 要將 Azure 監視器日誌設置為監視群集，需要啟用診斷才能查看群集級或平臺級事件。 請參閱[如何在 Windows 叢集中設定診斷功能](service-fabric-diagnostics-event-aggregation-wad.md)和[如何在 Linux 叢集中設定診斷功能](service-fabric-diagnostics-oms-syslog.md)，以取得詳細資訊
...
如果使用 Windows，請繼續執行以下步驟，將 Azure 監視器日誌連接到存儲群集事件的存儲帳戶。 

>[!NOTE]
>服務結構分析解決方案僅支援 Windows 群集。 對於 Linux 群集，請查看我們有關如何[為 Linux 群集設置 Azure 監視器日誌](service-fabric-diagnostics-oms-syslog.md)的文章。  
...
7. 選取 [確定]**** 以將您的工作區連線至您的叢集記錄。
    ...
    ![將存儲帳戶日誌添加到 Azure 監視器日誌](media/service-fabric-diagnostics-event-analysis-oms/add-storage-account.png)
...
## <a name="deploy-azure-monitor-logs-with-azure-resource-manager"></a>使用 Azure 資源管理器部署 Azure 監視器日誌
...
可以使用 Azure PowerShell 模組中的`New-AzResourceGroupDeployment`API 將範本作為資源管理器升級部署到群集。 範例命令可能像這樣：
...
## <a name="deploy-azure-monitor-logs-with-azure-powershell"></a>使用 Azure PowerShell 部署 Azure 監視器日誌

您還可以使用 命令通過 PowerShell`New-AzOperationalInsightsWorkspace`部署日誌分析資源。 若要使用此方法，請確定您已安裝 [Azure PowerShell](https://docs.microsoft.com/powershell/azure/install-Az-ps)。 請使用此指令碼建立新的 Log Analytics 工作區，並將 Service Fabric 解決方案新增至該工作區： 
...
完成後，按照上一節中的步驟將 Azure 監視器日誌連接到相應的存儲帳戶。

您也可以新增其他解決方案，或使用 PowerShell 對 Log Analytics 工作區進行其他修改。 要瞭解更多資訊，請參閱[使用 PowerShell 管理 Azure 監視器日誌](../azure-monitor/platform/powershell-workspace-configuration.md)。
...
* [使用視圖設計器在 Azure 監視器日誌中創建自訂視圖](../azure-monitor/platform/view-designer.md)
