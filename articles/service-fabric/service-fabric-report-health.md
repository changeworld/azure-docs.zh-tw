...
* 在 Service Fabric 節點上執行但 *未* 實作為 Service Fabric 服務的內部監視程式。
* 從 Service Fabric 叢集 *外部* 探查資源的外部監視程式 (例如，監視服務，例如 Gomez) 。
...
健康情況報告會透過存在於網狀架構用戶端的健康狀態用戶端傳送給健全狀況管理員。 健全狀況管理員會將報告儲存在健康狀態存放區中。 可以使用下列設定來設定健康情況用戶端：

* **HealthReportSendInterval**：將報表加入至用戶端的時間與傳送到健全狀況管理員的時間之間的延遲。 用於將報告批次處理為單一訊息，而不是針對每份報告傳送一則訊息。 批次處理可改善效能。 預設值：30 秒。
* **HealthReportRetrySendInterval**： health 用戶端將累積的健康情況報告重新傳送至健康狀態管理員的間隔。 預設值：30秒，最小值：1秒。
* **HealthOperationTimeout**：傳送至健康狀態管理員之報表訊息的超時時間。 如果訊息超時，健全狀況用戶端會重試，直到健全狀況管理員確認報表已處理為止。 預設值：兩分鐘。

> [!NOTE]
> 批次處理報告時，網狀架構用戶端必須至少保持運作長達 HealthReportSendInterval，以確保報告已傳送。 如果訊息遺失或健全狀況管理員因為暫時性錯誤而無法套用它們，網狀架構用戶端必須保持運作時間，讓它有機會重試。
> 
> 

用戶端上的緩衝會將報告的唯一性納入考量。 例如，如果特定的錯誤報告程式在相同實體的相同屬性上每秒產生 100 個報告，則會以最後一個版本取代報告。 最多只有一份這類報告存在於用戶端佇列中。 如果設定了批次處理，則傳送到健全狀況管理員的報告數目只是每個傳送間隔一個。 這是最後新增的報告，可反映實體的最新狀態。
...
一旦確定監視程式詳細資料，您應決定可識別它的唯一來源 ID。 如果叢集中有多個相同類型的看門狗在運作，它們必須報告不同的實體，如果它們報告相同的實體，請使用不同的來源 ID 或屬性。 如此一來，其報告才可並存。 健康狀態報告的屬性應該能擷取受監視的條件。  (上述範例中，可以 **ShareSize** 屬性。 ) 如果有多個報表套用至相同的條件，則此屬性應該包含一些可允許報表並存的動態資訊。 例如，如果需要監視多個共用，該屬性名稱可以是 **ShareSize-sharename**。
...
[Service Fabric 健康情況監視簡介](service-fabric-health-introduction.md)
