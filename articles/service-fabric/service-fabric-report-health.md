...
Azure Service Fabric 引入了 [健康狀態模型](service-fabric-health-introduction.md) ，主要用於在特定實體上標示狀況不良的叢集和應用程式條件。 運行狀況模型使用**運行狀況報告器**（系統元件和監視器）。 目標為輕易迅速的診斷並修復問題。 服務寫入器必須預先考慮到健康狀態。 任何可能會影響到健康狀態的條件都需加以回報，尤其是如果它有助標示出接近根目錄的問題。 健康情況資訊將可有效減少偵錯和調查工作所需的時間和心力。 一旦在雲端 (私人或 Azure) 大規模啟動並執行服務，效益特別明顯。
...
* 在服務結構節點上運行但不作為服務交換矩陣*服務實現的內部*監視器。
* 從 Service Fabric 群集*外部*探測資源的外部監視程式（例如，監視服務（如 Gomez）。
...
運行狀況報告通過運行狀況用戶端發送給運行狀況管理器，該用戶端位於結構用戶端內部。 運行狀況管理器在運行狀況存儲中保存報告。 可以使用下列設定來設定健康情況用戶端：

* **運行狀況報告發送間隔**：從將報表添加到用戶端的時間到發送到運行狀況管理器的時間之間的延遲。 用於將報告批次處理為單一訊息，而不是針對每份報告傳送一則訊息。 批次處理可改善效能。 預設值：30 秒。
* **運行狀況報告重試發送間隔**：運行狀況用戶端向運行狀況管理器重新發送累積運行狀況報告的時間間隔。 預設值：30 秒，最小：1 秒。
* **運行狀況操作超時**：發送給運行狀況管理器的報告消息的超時時間。 如果消息超時，運行狀況用戶端將重試它，直到運行狀況管理器確認已處理報告。 預設值：兩分鐘。

> [!NOTE]
> 批次處理報告時，網狀架構用戶端必須至少保持運作長達 HealthReportSendInterval，以確保報告已傳送。 如果消息丟失或運行狀況管理器由於暫時性錯誤而無法應用它們，則結構用戶端必須保持活動更長時間，以便有機會重試。
> 
> 

用戶端上的緩衝會將報告的唯一性納入考量。 例如，如果特定的錯誤報告程式在相同實體的相同屬性上每秒產生 100 個報告，則會以最後一個版本取代報告。 最多只有一份這類報告存在於用戶端佇列中。 如果配置了批次處理，則發送到運行狀況管理器的報告數僅為每個發送間隔的一個。 這是最後新增的報告，可反映實體的最新狀態。
...
一旦確定監視程式詳細資料，您應決定可識別它的唯一來源 ID。 如果叢集中有多個相同類型的看門狗在運作，它們必須報告不同的實體，如果它們報告相同的實體，請使用不同的來源 ID 或屬性。 如此一來，其報告才可並存。 健康狀態報告的屬性應該能擷取受監視的條件。 （對於上面的示例，屬性可以是**共用大小**。）如果多個報表應用於同一條件，則屬性應包含一些允許報表共存的動態資訊。 例如，如果需要監視多個共用，該屬性名稱可以是 **ShareSize-sharename**。
...
[服務結構運行狀況監測簡介](service-fabric-health-introduction.md)
