...
這些服務的預設**服務包啟動模式**是**共用進程**,在這種情況下,服務結構僅在給定應用程式的節點上啟動一個*服務包*副本。  這意謂著一個節點上只會執行一份服務程式碼。 如果您想要在一個節點上執行多份服務程式碼，請在建立服務時將 **ServicePackageActivationMode** 指定為 **ExclusiveProcess**。 例如，您可以在建立 ServiceType** (指定於 ServiceManifest** 中) 的多個服務 (Service1** 至 ServiceN**) 時這麼做，也可以在您的服務是多重分割時這麼做。 
...
- 來自不同*ServiceType*的複本需要共用一些具有高初始化或記憶體成本的常見數據。
- 您有免費的服務供應項目，而您想要透過將服務的所有複本都放在同一個處理序中，為資源使用量設定限制。

專屬處理序主控模型與每一 ServicePackage** 有多個 ServiceType** 的應用程式模型並不相同。 這是因為每個*ServicePackage*的多個*ServiceType*旨在實現副本之間的更高資源分享,並啟用每個進程更高的副本密度。 專屬處理序模型的設計目的是要實現不同結果。
...
- "我的代碼包",它註冊*服務類型*"我的服務類型A"。
- "我的代碼包B",它註冊*服務類型*"我的服務類型B"。
