...
此值會定義屬性值的保護。 它可能是 SecretsStoreRef/Encrypted/純文字。
如果設定為 SecretsStoreRef，此值會參考在叢集的 SecureStore 中加密儲存的秘密，而此密碼會在啟用時解除參考和解密。
...
此值會定義您在 ' Password ' 屬性中指定的密碼類型。 它可以是 SecretsStoreRef/Encrypted/純文字。
如果設定為 SecretsStoreRef，我們會從 SecretStore 中取出參考值。
如果設定為加密，應用程式開發人員會負責建立憑證，並使用 Invoke-ServiceFabricEncryptSecret Cmdlet 來加密機密資訊。
