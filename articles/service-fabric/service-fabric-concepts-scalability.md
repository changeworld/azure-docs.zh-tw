...
description: 瞭解 Azure 服務結構中的縮放以及用於擴展應用程式的各種技術。
author: masnider
ms.topic: conceptual
ms.date: 08/26/2019
ms.author: masnider
...
可以在服務的忙碌程度提高或降低時，建立 (或移除) 新的具名服務執行個體。 這樣可讓要求散佈到多個服務執行個體，通常會使現有服務上的負載降低。 在建立服務時，Service Fabric 叢集 Resource Manager 會以分散式方式將服務置於叢集中。 確切決策是依叢集和其他放置規則中的[計量](service-fabric-cluster-resource-manager-metrics.md)來控管。 服務可以通過幾種不同的方式創建，但最常見的是通過諸如某人調用[`New-ServiceFabricService`](https://docs.microsoft.com/powershell/module/servicefabric/new-servicefabricservice?view=azureservicefabricps)等管理操作，或者通過代碼調用[`CreateServiceAsync`](https://docs.microsoft.com/dotnet/api/system.fabric.fabricclient.servicemanagementclient.createserviceasync?view=azure-dotnet)。 甚至可以從叢集中執行的其他服務內呼叫 `CreateServiceAsync`。
...
解決方案是為要跟蹤的工作流的每個不同實例創建此服務的實例。這是一個偉大的模式，無論服務是無狀態還是有狀態，都有效。 若要運作此模式，通常有另一項服務可作為「工作負載管理員服務」。 這項服務的作業會接收要求，並將這些要求路由至其他服務。 管理員可以在其接收訊息時動態建立工作負載服務的執行個體，然後再將要求傳遞至這些服務。 當指定的工作流程服務完成其作業時，管理員服務可能也會收到回呼。 管理員收到這些回呼時，它可以刪除工作流程服務的執行個體，或是如果有多個呼叫，就會將它保留。 
...
![具有三個節點的分區佈局](./media/service-fabric-concepts-scalability/layout-three-nodes.png)
</center>
...
![具有四個節點的分區佈局](./media/service-fabric-concepts-scalability/layout-four-nodes.png)
</center>
...
由於作業系統之間的實現差異，選擇將 Service Fabric 與 Windows 或 Linux 一起使用可能是擴展應用程式的重要組成部分。 一個潛在的障礙是如何執行暫存日誌記錄的。 Windows 上的服務結構使用內核驅動程式進行每台電腦的一個日誌，該日誌在有狀態的服務副本之間共用。 此日誌重約 8 GB。 另一方面，Linux 為每個副本使用 256 MB 暫存日誌，因此對於希望最大化在給定節點上運行的羽量級服務副本數量的應用程式來說，它不太理想。 臨時存儲要求的這些差異可能會為服務結構群集部署所需的平臺提供資訊。
...
2. 當客戶註冊您的服務時，管理員服務就負責查看他們的資訊。 然後，根據這項資訊而定，管理員服務會建立「只針對該客戶」__ 建立「實際」__ 連絡人儲存體服務的執行個體。 如果他們需要特定組態、隔離或升級，您也可以決定針對此客戶加速應用程式執行個體。 
