...
* *閒置的超時時間*。 這是動作項目維持未使用 (閒置) 所需的時間長度，過此時間後即可停用和進行記憶體回收。 預設值為 60 分鐘。
...
```
對於每個作用中動作項目，動作項目執行階段會持續追蹤動作項目已閒置 (亦即未使用) 的時間。 動作專案執行時間會檢查每個動作專案， `ScanIntervalInSeconds` 以查看是否可進行垃圾收集，並在它已閒置時加以標記 `IdleTimeoutInSeconds` 。
