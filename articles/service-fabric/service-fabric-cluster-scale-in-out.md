...
> [!NOTE]
> 在規模的案例中，除非您的節點類型具有黃金或銀[級的耐久性等級][durability]，否則您必須使用適當的節點名稱呼叫[remove-servicefabricnodestate 指令程式](https://docs.microsoft.com/powershell/module/servicefabric/remove-servicefabricnodestate)。 針對銅級持久性，不建議一次相應縮小一個以上的節點。
> 
> 
...
當您相應放大叢集時，Service Fabric Explorer 會反映屬於叢集的節點數目（虛擬機器擴展集實例）。  不過，當您在中調整叢集時，除非您使用適當的節點名稱呼叫[remove-servicefabricnodestate cmd](https://docs.microsoft.com/powershell/module/servicefabric/remove-servicefabricnodestate) ，否則會看到顯示為狀況不良狀態的已移除節點/VM 實例。   

以下是這種行為的說明。

Service Fabric Explorer 列出的節點會反映出 Service Fabric 系統服務 (特別是 FM) 所知叢集曾經有過/現有擁有的節點數目。 當您調整中的虛擬機器擴展集時，會刪除 VM，但 FM 系統服務仍然認為節點（對應至已刪除的 VM）將會返回。 所以 Service Fabric Explorer 會繼續顯示該節點 (雖然健全狀況狀態可能是錯誤或未知)。

為確保移除 VM 時也移除節點，您有兩個選項︰

1. 為叢集中的節點類型選擇 Gold 或 Silver 持久性層級，提供您基礎結構的整合。 這會在您相應縮小時，自動從我們的系統服務（FM）狀態中移除節點。
請參閱 [持久性層級的詳細資訊](service-fabric-cluster-capacity.md)

2. 一旦將 VM 實例相應縮小，您必須呼叫[remove-servicefabricnodestate 指令程式](https://docs.microsoft.com/powershell/module/servicefabric/remove-servicefabricnodestate)。
