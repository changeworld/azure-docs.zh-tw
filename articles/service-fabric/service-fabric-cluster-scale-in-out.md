---
title: 相應縮小或放大 Service Fabric 叢集
...
如果您的叢集有多個節點類型，請針對您想要相應縮小或放大 (的每個節點類型/虛擬機器擴展集重複此步驟) 。 請先考慮一定要有的節點數目，再設定自動調整規模。 主要節點類型一定要有的節點數目下限是由您已選擇的可靠性層級決定。 深入了解 [可靠性層級](service-fabric-cluster-capacity.md)。

> [!NOTE]
> 將主要節點類型相應縮小為小於最小數目，會使叢集不穩定或甚至關閉。 這可能導致應用程式和系統服務資料遺失。
> 
> 

自動調整規模功能目前不是由應用程式可能向 Service Fabric 報告的負載所驅動。 所以，您現在取得的自動調整只由每個虛擬機器擴展集執行個體所發出的效能計數器驅動。  

遵循這些指示 [來設定每個虛擬機器擴展集的自動調整規模](../virtual-machine-scale-sets/virtual-machine-scale-sets-autoscale-overview.md)。
...
> [!NOTE]
> 新增 Vm 需要一些時間，因此不會預期新增會立即生效。 規劃預先增加容量，允許超過10分鐘的時間，讓複本/服務實例可以使用 VM 容量。
> 
...
## <a name="manually-remove-vms-from-a-node-typevirtual-machine-scale-set"></a>從節點類型/虛擬機器擴展集手動移除 Vm
當您調整節點類型時，會從擴展集移除 VM 實例。 如果節點類型是銅級持久性層級，Service Fabric 不知道發生什麼事，以及報告節點已遺失。 Service Fabric 接著會報告叢集狀況不良的狀態。 若要避免這種錯誤狀態，您必須明確地從叢集中移除節點，並移除節點狀態。

Service fabric 系統服務會在叢集中的主要節點類型中執行。 在主要節點類型中進行調整時，永遠不會將實例數目調整為小於 [可靠性層級](service-fabric-cluster-capacity.md) 所保證的數目。 
...
當您向外延展叢集時，Service Fabric Explorer 會反映屬於叢集的虛擬機器擴展集實例) 的節點數目 (。  不過，當您調整叢集中的叢集時，除非您以適當的節點名稱呼叫 [>remove-servicefabricnodestate cmd](/powershell/module/servicefabric/remove-servicefabricnodestate) ，否則會看到已移除的節點/VM 實例會顯示為狀況不良的狀態。   

以下是這種行為的說明。

Service Fabric Explorer 列出的節點會反映出 Service Fabric 系統服務 (特別是 FM) 所知叢集曾經有過/現有擁有的節點數目。 當您調整中的虛擬機器擴展集時，已刪除 VM，但 FM 系統服務仍然認為對應至已刪除之 VM 的節點 () 將會回來。 所以 Service Fabric Explorer 會繼續顯示該節點 (雖然健全狀況狀態可能是錯誤或未知)。
