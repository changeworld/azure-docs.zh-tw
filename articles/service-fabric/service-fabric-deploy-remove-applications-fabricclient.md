...
在群集中部署應用程式並運行實例後，可以刪除應用程式實例及其應用程式類型。 按照以下步驟從群集中完全刪除應用程式：

1. 移除 (或刪除) 執行中的應用程式執行個體
2. 取消註冊不再需要的應用程式類型

如果您在本機開發叢集上使用 Visual Studio 來針對應用程式進行部署和偵錯，則先前所有步驟都會透過 PowerShell 指令碼自動處理。  此腳本位於應用程式專案的*腳本*資料夾中。 本文提供了該腳本執行的操作的背景，以便您可以在 Visual Studio 之外執行相同的操作。 
 
## <a name="connect-to-the-cluster"></a>連線至叢集
執行本文中任何程式碼範例之前，請先建立 [FabricClient](/dotnet/api/system.fabric.fabricclient) 執行個體來連線到叢集。 針對連線至本機開發叢集或遠端叢集，或是連線至使用 Azure Active Directory、X509 憑證或 Windows Active Directory 保護的叢集，如需相關的範例，請參閱[連線至安全的叢集](service-fabric-connect-to-secure-cluster.md#connect-to-a-cluster-using-the-fabricclient-apis)。 要連接到本地開發群集，請運行以下示例：
...
假設您在 Visual Studio 中組建並封裝名為 *MyApplication* 的應用程式。 根據預設，ApplicationManifest.xml 中列出的應用程式類型名稱會是 "MyApplicationType"。  應用程式包包含必要的應用程式清單、服務清單和代碼/配置/資料包，位於*C：[使用者\&lt;&gt;使用者名]文檔_Visual Studio 2019_專案_我的應用程式\pkg_調試*。

上傳應用程式套件會將它放在內部 Service Fabric 元件可以存取的位置。 Service Fabric 會在應用程式套件的註冊期間驗證應用程式套件。 但是，如果要在本地驗證應用程式包（即在上載之前），請使用[測試服務Fabric應用程式包](/powershell/module/servicefabric/test-servicefabricapplicationpackage?view=azureservicefabricps)Cmdlet。
...
您可以使用 [CreateServiceAsync](/dotnet/api/system.fabric.fabricclient.servicemanagementclient.createserviceasync) API，從服務類型將服務具現化。  如果服務宣告為應用程式資訊清單中的預設服務，服務會在應用程式具現化時進行具現化。  為已具現化的服務調用[CreateServiceAsync](/dotnet/api/system.fabric.fabricclient.servicemanagementclient.createserviceasync) API 將返回 FabricException 類型的異常。 該異常將包含一個錯誤代碼，其值為 FabricErrorCode.Service 已存在。
...
不再需要特定版本的應用程式類型時，您應該使用 [Unregister-ServiceFabricApplicationType](/dotnet/api/system.fabric.fabricclient.applicationmanagementclient.unprovisionapplicationasync) API 將該特定版本的應用程式類型取消註冊。 取消註冊未使用的應用程式類型版本會釋放映像存放區已使用的儲存空間。 只要沒有應用程式具現化針對該應用程式類型的版本，就可以取消註冊應用程式類型的版本。 此外，應用程式類型不能沒有掛起的應用程式升級引用該應用程式類型的該版本。
...
問題：上載包成功完成，但[預配應用程式同步](/dotnet/api/system.fabric.fabricclient.applicationmanagementclient.provisionapplicationasync)API 超時。嘗試：
- 複製到映像存放區之前[壓縮封裝](service-fabric-package-apps.md#compress-a-package)。
壓縮會減少檔案的大小和數目，而後者則可減少資料傳輸量和 Service Fabric 必須執行的工作。 上載操作可能較慢（特別是包括壓縮時間），但註冊和取消註冊應用程式類型的速度更快。
...
下面的示例將應用程式包複製到映射存儲並規定應用程式類型。 然後，該示例創建一個應用程式實例並創建一個服務實例。 最後，該示例刪除應用程式實例，取消應用程式類型，並從映射存儲中刪除應用程式包。
