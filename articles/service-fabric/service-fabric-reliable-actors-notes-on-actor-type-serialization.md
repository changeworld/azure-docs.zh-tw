...
所有方法的引數、動作專案介面中每個方法所傳回之工作的結果類型，以及動作專案的狀態管理員中儲存的物件，都必須是[資料合約可](/dotnet/framework/wcf/feature-details/types-supported-by-the-data-contract-serializer)序列化。 這也適用於在 [動作項目事件介面](service-fabric-reliable-actors-events.md)中定義的方法引數。 (動作項目事件介面方法一律會傳回無效)。
...
Reliable Actor 架構會使用 DataContract 序列化。 因此，自訂資料物件及其成員必須分別以**DataContract**和**DataMember**屬性來標注。
