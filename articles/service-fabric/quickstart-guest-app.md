...
開始之前，請確定您已 [設定開發環境](service-fabric-get-started.md)。 其中包括安裝服務結構 SDK 和視覺化工作室 2019 或 2015。
...
如果沒有 Azure 訂閱，請創建[一個免費帳戶][create-account]。
...
| 設定                   | 值 |
| ------------------------- | ------ |
| 程式碼封裝資料夾       | _&lt;帶有 Node.js 應用的資料夾&gt;_ |
| 程式碼封裝行為     | 將資料夾內容複製到專案 |
| 程式                   | node.exe |
| 引數                 | server.js |
| 工作資料夾            | CodePackage |
...
應用程式專案 （**MyGuestApp**） 不直接包含任何代碼。 專案引用一組服務專案。 此外，它包含三種其他類型的內容：
...
開啟專案中的 **ServiceManifest.xml** 檔案。 在清單的底部，有一`<Resources> \ <Endpoints>`個已定義的條目。 修改該項目以新增 `Port`、`Protocol` 和 `Type`。 
...
如果按**F5**並運行專案，則該專案將部署到本地群集。 不過，讓我們改為將它部署到 Azure。
...
在 [連線端點]**** 之下，選取要部署 Service Fabric 叢集的位置。 如果沒有，請選擇**&lt;"創建新群集..."&gt;** 打開 Web 瀏覽器視窗到 Azure 門戶。 如需詳細資訊，請參閱[在入口網站中建立叢集](service-fabric-cluster-creation-via-portal.md#create-cluster-in-the-azure-portal)。 

當您建立 Service Fabric 叢集時，請務必將 [自訂端點]**** 設定設為 **80**。

![自訂端點的 Service fabric 節點類型組態][custom-endpoint]

建立新的 Service Fabric 叢集需要一些時間才能完成。 創建後，返回發佈對話方塊並選擇**&lt;"刷新&gt;**"。 新的叢集會列在下拉式方塊中，請加以選取。
...
請檢查服務位址的 [概觀] 刀鋒視窗。 使用 [用戶端連線端點]__ 屬性中的網域名稱。 例如： `http://mysvcfab1.westus2.cloudapp.azure.com` 。

![Azure 入口網站上的 Service Fabric 概觀刀鋒視窗][overview]

導航到此位址，`HELLO WORLD`您將看到回應。

## <a name="delete-the-cluster"></a>選取叢集

不要忘記刪除為此快速入門創建的所有資源，因為您需要為這些資源付費。
