---
title: Azure 服務結構 CLI-sfctl 應用程式
description: 瞭解 sfctl，Azure 服務結構命令列介面。 包括用於管理應用程式的命令清單。
author: jeffj6123
ms.topic: reference
ms.date: 1/16/2020
ms.author: jejarry
...
| provision | 使用外部存儲中的".sfpkg"包或在映射存儲中使用應用程式包向群集提供或註冊 Service Fabric 應用程式類型。 |
...
使用外部存儲中的".sfpkg"包或在映射存儲中使用應用程式包向群集提供或註冊 Service Fabric 應用程式類型。

向叢集佈建 Service Fabric 應用程式類型。 在具現化任何新應用程式之前，需要提供此規定。 預配操作可以在相對 PathInImageStore 指定的應用程式包上執行，也可以使用外部".sfpkg"的 URI。 除非已設定 --external-provision，否則此命令將會預期要有映像存放區佈建。
...
| --- | --- |
| --路徑 [必需] | 本機應用程式套件的路徑。 |
| --壓縮 | 僅適用于服務結構應用程式包。 創建包含壓縮應用程式包的新資料夾，以預設位置或壓縮位置參數指定的位置，然後上載新創建的資料夾。 <br><br> 如果 sfctl 已生成壓縮檔，則如果設置了此標誌，該檔將被覆蓋。 如果目錄不是應用程式包，則返回錯誤。 如果它已經是壓縮的應用程式包，資料夾將複製為現在。 預設情況下，新創建的壓縮應用程式包將在成功上載後刪除。 如果上傳不成功，請根據需要手動清理壓縮包。 如果壓縮位置參數引用不存在的目錄，則刪除不會刪除可能已創建的任何空轉。 |
| --壓縮位置 | 放置壓縮應用程式包的位置。 <br><br> 如果未提供任何位置，則壓縮包將放置在路徑參數中指定的父目錄下的新創建的名為sfctl_compressed_temp的資料夾下。 例如，如果路徑參數的值為 C\:/FolderA/AppPkg，則壓縮的包將添加到 C\:/FolderA/sfctl_compressed_temp/AppPkg 中。 |
| --imagestore-string | 將應用程式套件上傳至其中的目的地映像存放區。  預設值：fabric\:ImageStore。 <br><br> 要上載到檔位置，請用"檔\:"啟動此參數。 否則，該值應為圖像存儲連接字串，如預設值。 |
| --保持壓縮 | 是否在成功上載完成時保留生成的壓縮包。 <br><br> 如果未設置，則在成功完成時，將刪除壓縮的應用包。 如果上載不成功，則應用程式包將始終保存在輸出目錄中以重新上載。 |
| --show-progress | 顯示大型套件的檔案上傳進度。 |
| --timeout -t | 總超時（以秒為單位）。 上載將失敗，並在上載超時持續時間過後返回錯誤。 此超時適用于整個應用程式包，單個檔超時將等於剩餘的超時持續時間。 超時不包括壓縮應用程式包所需的時間。  預設\:300。 |
