...
9. 若要設定容器隔離模式，請參閱[設定隔離模式]( ./service-fabric-get-started-containers.md#configure-isolation-mode)。 Windows 支援兩種容器隔離模式：分別為處理序和 Hyper-V。 下列程式碼片段顯示如何在應用程式資訊清單檔中指定隔離模式。
...
> [!NOTE] 
> 根據預設，Service Fabric 的應用程式可存取 Service Fabric 執行時間，其形式為接受應用程式特定要求的端點。 當應用程式裝載未受信任的程式碼時，請考慮停用此存取。 如需詳細資訊，請參閱 [Service Fabric 中的安全性最佳作法](service-fabric-best-practices-security.md#platform-isolation)。 若要停用 Service Fabric 執行時間的存取權，請在對應至已匯入之服務資訊清單的應用程式資訊清單的 [原則] 區段中新增下列設定，如下所示：
>
```xml
  <Policies>
