...
> [!NOTE] 
> 從 API 版本開始，支援部署具有受控識別的 Service Fabric 應用程式 `"2019-06-01-preview"` 。 您也可以針對應用程式類型、應用程式類型版本和服務資源使用相同的 API 版本。 支援的最小 Service Fabric 執行時間為 6.5 CU2。 在 additoin 中，組建/封裝環境也應該具有 CU2 或更高版本的 SF .Net SDK
...
2. 在對應至要指派受控識別之服務的**ServiceManifestImport**區段中，新增**IdentityBindingPolicy**元素，如下所示：
    ...
    **ApplicationManifest.xml**
    ...
    這個元素會將應用程式的身分識別指派給服務;若沒有此指派，服務將無法存取應用程式的身分識別。 在上述程式碼片段中，身分 `SystemAssigned` 識別（也就是保留關鍵字）會對應至易記名稱下的服務定義 `WebAdmin` 。

3. 更新服務資訊清單，以在**Resources**區段內新增**microsoft.managedidentity**元素，其名稱符合 `ServiceIdentityRef` `IdentityBindingPolicy` 應用程式資訊清單中定義的設定值：
    ...
    **ServiceManifest.xml**
    ...
    ```
    這是與上述服務的身分識別對應，但從服務定義的觀點來看。 這裡會依 `WebAdmin` 應用程式資訊清單中宣告的易記名稱（）來參考此身分識別。
