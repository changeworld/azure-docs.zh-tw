...
description: 瞭解有狀態服務副本的配置以及重新配置 Service Fabric 用於在更改期間保持一致性和可用性的過程。
author: appi101
ms.topic: conceptual
ms.date: 01/10/2018
ms.author: aprameyr
...
「重新設定」** 是指將某個設定移至另一個設定的流程。 這會變更具狀態服務其分割區的複本集。 舊配置稱為*上一個配置 （PC），* 新配置稱為*當前配置 （CC）。* Azure Service Fabric 中的重新設定通訊協定在複本集進行任何變更期間，會保持一致性並維持可用性。
...
- 主要複本變更所在的重新設定：
    - **容錯移轉**：容錯移轉是重新配置，以回應正在運行的主伺服器的故障。
...
- **階段 2**：在此階段，Service Fabric 可確保當前配置的大多數副本中都有所有資料可用。
...
- [系統運行狀況報告](service-fabric-understand-and-troubleshoot-with-system-health-reports.md)
