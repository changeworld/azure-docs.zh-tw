...
description: 深入瞭解具狀態服務複本的設定，以及重新設定 Service Fabric 用來在變更期間維持一致性和可用性的處理常式。
author: appi101
ms.topic: conceptual
ms.date: 01/10/2018
ms.author: aprameyr
...
「重新設定」** 是指將某個設定移至另一個設定的流程。 這會變更具狀態服務其分割區的複本集。 舊的設定稱為先前的設定 * (電腦) *，而新的設定則稱為目前的設定 * (副本) *。 Azure Service Fabric 中的重新設定通訊協定在複本集進行任何變更期間，會保持一致性並維持可用性。
...
- **階段 2**：在這個階段期間，Service Fabric 可確保所有資料都可在目前設定的大部分複本中使用。
