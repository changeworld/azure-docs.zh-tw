...
Azure Service Fabric 簡化了撰寫和管理無狀態和具狀態服務的流程。 本主題涵蓋︰

* 無狀態和具狀態之服務的 Reliable Services 程式設計模型。
* 撰寫 Reliable Services 時必須進行的選擇。
* Reliable Services 使用時機及撰寫方式的一些案例和範例。

*Reliable Services* 是 Service Fabric 上可用的其中一種程式設計模型。 另一個是 *可靠* 的動作專案程式設計模型，它會在 Reliable Services 模型之上提供 [虛擬](https://research.microsoft.com/en-us/projects/orleans/) 動作專案應用程式架構。 如需 Reliable Actors 的相關詳細資訊，請參閱 [Service Fabric Reliable Actors 簡介](service-fabric-reliable-actors-introduction.md)。
...
* Service Fabric Api 的存取權。 不同于模型為 [來賓可執行檔](service-fabric-guest-executables-introduction.md)的 Service Fabric 服務，Reliable Services 可以直接使用 Service Fabric api。 如此可讓服務︰
  * 查詢系統
  * 報告叢集中實體的健康情況
  * 接收關於設定和程式碼變更的通知
  * 尋找其他服務並與之通訊，
  * 使用 [可靠的集合](service-fabric-reliable-services-reliable-collections.md)
  * 使用數種程式設計語言的頂級程式設計模型，存取許多其他功能。
...
## <a name="what-makes-reliable-services-different"></a>Reliable Services 會有何不同

Reliable Services 與您之前撰寫的服務不同，因為 Service Fabric 會提供：
...
* **延展性** - 服務與特定硬體分離，可以視需要透過加入或移除硬體或其他資源而成長或壓縮。 服務可以輕鬆分割 (特別是在具狀態的情況下)，以確保服務可以調整和處理局部失敗。 您可以透過程式碼動態建立和刪除服務，讓更多實例可以在必要時啟動，例如回應客戶的要求。 最後，Service Fabric 建議使用輕量型的服務。 Service Fabric 允許在單一處理序內佈建數千個服務，而不需要或讓整個作業系統的執行個體或處理序專用於服務的單一執行個體。
* **一致性** ：儲存在可靠服務中的任何資訊都可保證一致。 即使是在服務內多個可靠的集合中，也是如此。 利用交易不可部分完成的方式，即可在服務內跨集合進行變更。

## <a name="service-lifecycle"></a>服務生命週期

無論您的服務是具狀態還是無狀態，Reliable Services 會提供簡單的生命週期，可讓您快速插入您的程式碼，並開始著手。  若要啟動並執行新的服務，您需要執行兩種方法：
...
讓我們進一步瞭解 Reliable Services 模型如何搭配無狀態和具狀態服務運作。
...
具 *狀態服務* 是一種必須保持一致狀態的部分，而且必須存在才能讓服務運作的服務。 假設有一個服務不斷地根據某個值收到的更新，計算它的滾動平均。 若要這樣做，它必須擁有目前需要處理的連入要求集，以及目前的平均。 擷取、處理並將資訊儲存在外部存放區 (例如現在的 Azure Blob 或資料表存放區) 的任何服務都是具狀態。 它只將狀態保留在外部狀態存放區中。

現在的大部分服務會在外部儲存其狀態，因為外部存放區為該狀態提供可靠性、可用性、延展性和一致性。 在 Service Fabric 中，服務不需要將狀態儲存在外部。 Service Fabric 會為服務程式碼和服務狀態來處理這些需求。

假設我們想要撰寫一個處理映像的服務。 在做法上，這個服務需要取得映像和要在該映像上執行的一系列轉換。 此服務會傳回一個通訊接聽程式 (假設是 WebAPI)，其中公開一個像 `ConvertImage(Image i, IList<Conversion> conversions)` 的 API。 當它收到要求時，服務會將它儲存在中 `IReliableQueue` ，並將某個識別碼傳給用戶端，讓它可以追蹤要求。
...
雖然這項服務聽起來像是一般的 .NET 服務，但不同之處在于使用的資料結構 (`IReliableQueue` 和 `IReliableDictionary`) 是由 Service Fabric 提供，且高度可靠、可用且一致。

## <a name="when-to-use-reliable-services-apis"></a>使用 Reliable Services API 的時機

如果有下列情況，請考慮 Reliable Services Api：

* 您希望您的服務程式代碼 (，並選擇性地將) 設為高可用性和可靠的狀態。
