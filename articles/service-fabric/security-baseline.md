...
**指導**方針：針對與 Azure Service Fabric 叢集相關的網路資源，定義及執行標準安全性設定。 使用 "ServiceFabric" 和 "Microsoft" 命名空間中 Azure 原則別名來建立自訂原則，以審核或強制執行 Azure Service Fabric 叢集的網路設定。
...
**指引**：維護在 Azure Service Fabric 叢集布建期間建立的本機系統管理客戶紀錄，以及您所建立的任何其他帳戶。 此外，如果使用 Azure AD 整合，Azure AD 具有必須明確指派，因此可查詢的內建角色。 使用 Azure AD PowerShell 模組執行臨機操作查詢，以探索屬於系統管理群組成員的帳戶。
...
**指導**方針：整合 Azure Service Fabric 的驗證與 Azure Active Directory。 建立有關使用專用系統管理帳戶的原則和程式。
...
針對 Microsoft 管理的基礎平台，Microsoft 會將所有客戶內容視為敏感性資訊，並竭盡全力防範客戶資料外洩和暴露。 為了確保 Azure 中的客戶資料保持安全，Microsoft 已實行並維護一套強大的資料保護控制措施，以及
...
**指引**：加密傳輸中的所有機密資訊。 請確定任何連線到您 Azure 資源的用戶端都能協商 TLS 1.2 或更高版本。
...
**指引**：若要讓 Azure Service Fabric 叢集儲存或處理敏感資訊，請使用標記，將叢集和相關資源標示為機密。 Azure 儲存體或計算資源尚無法使用資料識別、分類和遺失防護功能。 若需要達到合規性目標，請實作協力廠商解決方案。
...
**指引**：定期執行 Service Fabric 錯誤分析服務和混亂服務，以模擬整個叢集的錯誤，以評估服務的穩定性和可靠性。

請遵循在 Azure 虛擬機器和容器映射上執行弱點評定 Azure 資訊安全中心中的建議。
...
**指引**：在 Azure Service Fabric 叢集的虛擬機器擴展集上啟用自動 OS 映射升級。
...
* [Azure 自動化更新管理概觀](https://docs.microsoft.com/azure/automation/automation-update-management)
...
**指引**：以一致的間隔匯出掃描結果，並比較結果以確認是否已補救弱點。 當您使用 Azure 資訊安全中心建議的弱點管理建議時，您可以將資料透視到選取的解決方案入口網站，以查看歷程記錄掃描資料。
...
**指導**方針：針對 Azure Service Fabric 叢集節點，請執行協力廠商解決方案，以防止未授權的檔案類型執行。
...
**指引**： Azure Service Fabric 作業系統映射是由 Microsoft 所管理和維護。 負責為叢集節點的作業系統執行安全設定的客戶。
...
**指引**： Azure Service Fabric 由 Microsoft 所管理和維護的叢集作業系統映射。 負責執行 OS 層級狀態設定的客戶。
...
使用 Azure Key Vault 自動輪替 Service Fabric 的叢集憑證。
...
**指導**方針：根據預設，windows Server 2016 上安裝了 windows Defender 防毒軟體。 某些 SKU 上會預設安裝使用者介面，但並不需要。
...
**指引**： Service Fabric 備份還原服務的備份會使用您訂用帳戶中的 Azure 儲存體帳戶。 Azure 儲存體會加密待用儲存體帳戶中的所有資料。 根據預設，資料是以使用 Microsoft 管理的金鑰加密。 若要進一步控制加密金鑰，您可以提供客戶管理的金鑰來加密儲存體資料。
...
* [適用于 IT 計畫和功能的測試、訓練及練習程式的 NIST 指南](https://nvlpubs.nist.gov/nistpubs/Legacy/SP/nistspecialpublication800-84.pdf)
