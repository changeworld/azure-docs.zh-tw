...
* *傳輸協議*. 您要使用 HTTP 以提供更優異的互通性，還是自訂的二進位通訊協定，以應付最大的輸送量？
...
* 可連接的各項服務都有「位址」 ** 可接聽 (例如 HTTP 或 WebSockets)。 當服務執行個體或分割移動時，其位址端點會變更。 （它移動到具有不同 IP 位址的不同節點。如果您使用的是內置通信元件，它們將為您處理重新解析服務位址。
