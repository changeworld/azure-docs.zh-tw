---
title: 在 Linux 中進行 Azure Service Fabric 應用程式的偵錯工具
...
對於 Java 應用程式，有 [多個記錄架構](https://en.wikipedia.org/wiki/Java_logging_framework) 可用。 因為 `java.util.logging` 是 JRE 的預設選項，所以它也會用於 [GitHub 中的程式碼範例](https://github.com/Azure-Samples/service-fabric-java-getting-started)。 下列討論說明如何設定 `java.util.logging` 架構。
...
有多個架構適用於追蹤 Linux 上的 CoreCLR 應用程式。 如需詳細資訊，請參閱 [適用于記錄的 .Net 擴充功能](https://github.com/dotnet/extensions/tree/master/src/Logging)。  因為 C# 開發人員熟悉 EventSource，本文使用 EventSource 來追蹤 Linux 上的 CoreCLR 範例。
