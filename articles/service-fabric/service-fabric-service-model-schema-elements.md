...
宣告資料夾，並以 Name 屬性命名，並使用包含 Settings.xml 檔案的 PackageRoot。 此檔案包含程序可以在執行階段讀回的使用者定義、成對的索引鍵/值設定等區段。 在升級期間，如果只有 ConfigPackage 版本已變更，則不會重新啟動執行中程序。 相反地，回呼會通知程序組態設定已變更，因此它們可以動態方式重新載入。
...
這個值會定義您在 [值] 屬性中指定的數值型別。 它可以是 SecretsStoreRef/Encrypted/純文字。
如果設定為 SecretsStoreRef，我們會從 SecretStore 中取出參考值。
如果設定為 [已加密]，應用程式開發人員會負責建立憑證，並使用 Invoke-servicefabricencryptsecret Cmdlet 來加密機密資訊。
