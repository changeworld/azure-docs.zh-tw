...
聲明一個資料夾，由 Name 屬性命名，包 Root 包含 Settings.xml 檔。 此檔案包含程序可以在執行階段讀回的使用者定義、成對的索引鍵/值設定等區段。 在升級期間，如果只有 ConfigPackage 版本已變更，則不會重新啟動執行中程序。 相反地，回呼會通知程序組態設定已變更，因此它們可以動態方式重新載入。
...
|屬性|值|
|---|---|
|type|[圖像類型](service-fabric-service-model-schema-complex-types.md#imagetype-complextype)|
|content|0 個元素，0 個屬性|
...
|屬性|值|
|---|---|
|type|[參數類型](service-fabric-service-model-schema-complex-types.md#parametertype-complextype)|
|content|0 個元素，0 個屬性|
...
指定帳戶類型：本地使用者、域使用者、網路服務、本機服務、託管服務帳戶或本地系統。  預設值為 LocalUser。 本機使用者帳戶會建立在應用程式部署所在的電腦上。 根據預設，這些帳戶不會具有這裡指定的相同名稱。 相反地，它們會動態產生並具有隨機密碼。 支援的本機系統帳戶類型為 LocalUser、NetworkService、LocalService 和 LocalSystem。 有 Azure Active Directory 可供使用的 Windows Server 部署可支援網域帳戶。
