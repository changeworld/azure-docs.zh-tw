---
title: 建立和管理變數來儲存和傳遞值
description: 瞭解如何在您使用建立的自動化工作和工作流程中，使用變數來儲存、管理、使用及傳遞值 Azure Logic Apps
services: logic-apps
...
本文說明如何建立及使用您用來將值儲存在邏輯應用程式中的變數。 例如，變數可協助您追蹤迴圈執行的次數。 若要反覆運算陣列或檢查特定專案的陣列，您可以使用變數來參考每個陣列專案的索引編號。
...
* 為變數指派不同的值。
* 將變數的值插入或 *附加* 至字串或陣列中的最後一個專案。

變數只存在於建立它們的邏輯應用程式執行個體內，並且是全域性。 此外，它們只會在邏輯應用程式執行個體內跨任何迴圈反覆項目存留。 當您參考變數時，請使用變數的名稱做為標記，而不是動作的名稱，這是參考動作輸出的一般方式。
...
1. 在 [Azure 入口網站](https://portal.azure.com) 或 Visual Studio 中，在邏輯應用程式設計工具中開啟邏輯應用程式。
...
1. 在邏輯應用程式中您想要新增變數的步驟底下，依照下列其中一個步驟進行操作： 
   ...
   * 若要在最後一個步驟底下新增動作，請選取 [ **新增步驟**]。
     ...
     ![顯示在 [邏輯應用程式設計工具] 頁面上選取 [新增步驟] 動作的螢幕擷取畫面。](./media/logic-apps-create-variables-store-values/add-action.png)
...
1. 提供變數的相關資訊，如下所述：
   ...
   | **名稱** | 是 | <*變數名稱*> | 要遞增的變數名稱 |
   | **型別** | 是 | <*變數類型*> | 變數的資料類型 |
   | **ReplTest1** | 否 | <*開始-值*> | 您的變數初始值 <p><p>**提示**：雖然此值為選擇性，但最佳做法是設定此值，如此您便一律知道變數的開始值。 |
   |||||
...
1. 現在繼續新增您想要的動作。 完成時，請在設計工具的工具列上，選取 [儲存]。

如果您從設計工具切換到程式碼設計編輯器，以下是 [ **初始化變數** ] 動作在邏輯應用程式定義中出現的方式，也就是 JAVASCRIPT 物件標記法 (JSON) 格式：
...
例如，這個運算式會使用函數，從 [先前在本文中建立](#append-value) 的陣列變數取得專案 `variables()` 。 `string()`函數會以字串格式傳回變數的內容：`"1, 2, 3, red"`
...
若要以常數值增加或 *遞增* 變數，請將 **遞增變數** 動作新增至邏輯應用程式。 此動作僅適用於整數和浮點數變數。

1. 在邏輯應用程式設計工具中，于您想要增加現有變數的步驟下，選取 [ **新增步驟**]。 
...
1. 在搜尋方塊中，輸入 "increment variable" 作為篩選條件。 在 [動作] 清單中選取 [ **遞增變數**]。
...
如果您從設計工具切換到程式碼設計編輯器，以下是 **遞增變數** 動作在邏輯應用程式定義內出現的方式，也就是 JSON 格式：
...
1. 在觸發程式中，若要檢查是否有附件，並將這些附件傳遞至邏輯應用程式的工作流程，請針對這些屬性選取 **[是]** ：
...
1. 新增 [[初始化變數]**** 動作](#create-variable)。 建立名為的整數變數 `Count` ，其起始值為零。

   ![新增 [初始化變數] 的動作](./media/logic-apps-create-variables-store-values/initialize-variable.png)

1. 若要迴圈處理每個附件，請新增 *for each* 迴圈。

   1. 在 [ **初始化變數** ] 動作底下，選取 [ **新增步驟**]。

   1. 在 [選擇動作] 底下，選取 [內建]。 在搜尋方塊中，輸入 `for each` 做為您的搜尋篩選準則，然後 **針對每個**篩選準則進行選取。
...
1. 如果您的邏輯應用程式未啟用，請在邏輯應用程式功能表上選取 **[總覽**]。 在工具列上，選取 [ **啟用**]。

1. 在 [邏輯應用程式設計工具] 工具列上，選取 [ **執行**]。 此步驟會手動啟動您的邏輯應用程式。

1. 將一封含有一或多個附件的電子郵件傳送給您在此範例中使用的電子郵件帳戶。

   此步驟會觸發邏輯應用程式的觸發程序，它會為邏輯應用程式的工作流程建立並執行一個執行個體。 因此，邏輯應用程式會傳送訊息或電子郵件給您，當中顯示您所傳送電子郵件中的附件數目。

如果您從設計工具切換到程式碼設計編輯器，以下是 **For each** 迴圈的顯示方式，以及邏輯應用程式定義內的 **遞增變數** 動作（採用 JSON 格式）。
...
若要以常數值減少或 *遞減* 變數，請依照下列步驟來 [增加變數](#increment-value) ，但您會發現並選取 **遞減變數** 動作。 此動作僅適用於整數和浮點數變數。
...
如果您從設計工具切換到程式碼設計編輯器，以下是 **遞減變數** 動作在邏輯應用程式定義內出現的方式，也就是 JSON 格式。
...
>
> 1. 在迴圈的右上角選取省略號 (**...**) 按鈕，然後選取 [ **設定**]。
> 
> 2. 在 [並行控制]**** 底下，將 [覆寫預設]**** 設定變更為 [開啟]****。
>
> 3. 將 [平行處理原則的程度]**** 滑桿拖曳至 [1]****。

如果您從設計工具切換到程式碼設計編輯器，以下是 [ **設定變數** ] 動作在邏輯應用程式定義內出現的方式，也就是 JSON 格式。 此範例會將 `Count` 變數的目前值變更為另一個值。
...
如果您從設計工具切換到程式碼設計編輯器，以下是「 **附加至陣列變數** 」動作在邏輯應用程式定義內出現的方式，也就是 JSON 格式。 此範例會建立一個陣列變數，然後將另一個值新增為陣列中的最後一個項目。 結果會是一個含有下列陣列的已更新變數：`[1,2,3,"red"]`
