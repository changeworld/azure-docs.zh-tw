...
1. 提供您變數的相關資訊，如下所述：
   ...
   | 屬性 | 必要 | 值 |  描述 |
   |----------|----------|-------|--------------|
   | **名稱** | Yes | <*變數名稱*> | 要遞增的變數名稱 |
   | **型別** | Yes | <*變數類型*> | 變數的資料類型 |
   | **ReplTest1** | No | <*開始-值*> | 您的變數初始值 <p><p>**提示**：雖然此值為選擇性，但最佳做法是設定此值，如此您便一律知道變數的開始值。 |
   |||||
...
例如，此運算式會從[先前在本文中](#append-value)使用函式建立的陣列變數取得專案 `variables()` 。 函式會 `string()` 以字串格式傳回變數的內容：`"1, 2, 3, red"`
...
1. 提供下列資訊來遞增您的變數：
   ...
   |----------|----------|-------|--------------|
   | **名稱** | Yes | <*變數名稱*> | 要遞增的變數名稱 |
   | **ReplTest1** | No | <*增量-值*> | 用來遞增變數的值。 預設值為一。 <p><p>**提示**：雖然此值為選擇性，但最佳做法是設定此值，如此您便一律知道用來遞增變數的特定值。 |
   ||||
...
| 屬性 | 必要 | 值 |  描述 |
|----------|----------|-------|--------------|
| **名稱** | Yes | <*變數名稱*> | 要遞減的變數名稱 | 
| **ReplTest1** | No | <*增量-值*> | 用來遞減變數的值。 預設值為一。 <p><p>**提示**：雖然此值為選擇性，但最佳做法是設定此值，如此您便一律知道用來遞減變數的特定值。 |
||||| 
