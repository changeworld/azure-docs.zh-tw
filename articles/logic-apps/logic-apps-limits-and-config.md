...
本節只會列出 Azure Logic Apps 服務的輸入 IP 位址。 為協助降低建立安全性規則時的複雜度，您可以選擇性地使用[服務](../virtual-network/service-tags-overview.md)標籤**LogicAppsManagement**，而不是指定每個區域的輸入 Logic Apps IP 位址首碼。 此標記可在可用 Logic Apps 服務的區域之間運作。 如果您有 Azure Government，請參閱[Azure Government 輸入的 IP 位址](#azure-government-inbound)。
...
此區段會列出 Azure Logic Apps 服務和受控連接器的輸出 IP 位址。 為協助降低建立安全性規則時的複雜度，您可以選擇性地使用[服務](../virtual-network/service-tags-overview.md)標籤**LogicApps**，而不是指定每個區域的輸出 Logic Apps IP 位址首碼。 此標記可在可用 Logic Apps 服務的區域之間運作。 針對受控連接器，請使用 IP 位址。 如果您有 Azure Government，請參閱[Azure Government 輸出的 IP 位址](#azure-government-outbound)。
