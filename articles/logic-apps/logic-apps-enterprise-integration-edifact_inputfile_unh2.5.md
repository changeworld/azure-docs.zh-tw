...
如果 EDIFACT 檔中有 UNH 2.5 區段，則會使用區段進行架構查閱。 例如，在此範例 EDIFACT 訊息中，UNH 欄位是 `EAN008` ：
...
若要處理訊息，您必須部署具有 UNH 2.5 根節點名稱的架構。 例如，範例 UNH 欄位的架構根名稱是 `EFACT_D03B_ORDERS_EAN008` 。 針對每個 `D03B_ORDERS` 具有不同 unh 2.5 區段的，您必須部署個別的架構。
