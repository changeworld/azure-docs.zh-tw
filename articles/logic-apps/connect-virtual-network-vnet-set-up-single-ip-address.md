...
1. 在集合中，新增規則以允許目的地系統的流量。
   ...
   例如，假設您有一個在 ISE 中執行的邏輯應用程式，而且需要與 SFTP 伺服器通訊。 您會建立名為的網路規則集合 `LogicApp_ISE_SFTP_Outbound` ，其中包含名為的網路規則 `ISE_SFTP_Outbound` 。 此規則允許使用您的防火牆私人 IP 位址，從您的 ISE 在虛擬網路中執行的任何子網的 IP 位址，到目的地 SFTP 伺服器的流量。
