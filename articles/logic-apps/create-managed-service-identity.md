...
1. 在邏輯應用程式功能表的 [設定] 下，選取 [身分識別]。 在 > **儲存** **上**，選取 **系統指派**的 > 。 當 Azure 提示您確認時，請選取 **[是]** 。
   ...
   | 屬性 | 值 | 描述 |
   |----------|-------|-------------|
   | **物件識別碼** | <*identity-resource-ID*> | 全域唯一識別碼（GUID），代表 Azure AD 租使用者中邏輯應用程式的系統指派身分識別 |
   ||||
...
1. 在[Azure 入口網站](https://portal.azure.com)的任何頁面上的 [搜尋] 方塊中，輸入 `managed identities`，然後選取 [**受控**識別]。
...
1. 提供受控識別的相關資訊，然後選取 [**建立**]，例如：
   ...
   | 屬性 | 必要項 | 值 | 描述 |
   |----------|----------|-------|-------------|
   | **資源名稱** | 是 | <*使用者指派的身分識別名稱*> | 要提供給使用者指派身分識別的名稱。 這個範例會使用「Fabrikam-使用者指派的身分識別」。 |
   | **訂用帳戶** | 是 | <*Azure-subscription-name*> | 要使用的 Azure 訂用帳戶的名稱 |
   | **資源群組** | 是 | <*Azure-resource-group-name*> | 要使用之資源群組的名稱。 建立新的群組，或選取現有的群組。 這個範例會建立名為「fabrikam 管理-身分識別-RG」的新群組。 |
   | **位置** | 是 | <*Azure-region*> | Azure 區域，用來儲存資源的相關資訊。 此範例使用「美國西部」。 |
   |||||

   現在您可以將使用者指派的身分識別新增至邏輯應用程式。 您無法將多個使用者指派的身分識別新增至您的邏輯應用程式。

1. 在 Azure 入口網站中，尋找並開啟邏輯應用程式設計工具中的邏輯應用程式。
...
* `type` 屬性設定為的 `identity` 物件 `UserAssigned`
...
| 屬性（JSON） | 值 | 描述 |
|-----------------|-------|-------------|
| `principalId` | <*principal-ID*> | Azure AD 租使用者中使用者指派受控識別的全域唯一識別碼（GUID） |
| `clientId` | <*client-ID*> | 邏輯應用程式新身分識別的全域唯一識別碼（GUID），用於執行時間期間的呼叫 |
||||

如果您的範本也包含受控識別的資源定義，您可以將 `identity` 物件參數化。 這個範例會顯示子 `userAssignedIdentities` 物件如何參考您在範本的 `variables` 區段中定義的 `userAssignedIdentity` 變數。 此變數會參考使用者指派之身分識別的資源識別碼。
...
| 屬性（JSON） | 值 | 描述 |
|-----------------|-------|-------------|
| `tenantId` | <*Azure-AD-tenant-ID*> | 全域唯一識別碼（GUID），代表使用者指派的身分識別現在是成員的 Azure AD 租使用者。 在 Azure AD 租使用者內，服務主體的名稱與使用者指派的身分識別名稱相同。 |
| `principalId` | <*principal-ID*> | Azure AD 租使用者中使用者指派受控識別的全域唯一識別碼（GUID） |
| `clientId` | <*client-ID*> | 邏輯應用程式新身分識別的全域唯一識別碼（GUID），用於執行時間期間的呼叫 |
||||
...
1. 在邏輯應用程式功能表的 [**設定**] 底下，選取 [身分**識別**]，然後依照您的身分識別步驟執行：
   ...
   * 在 > **儲存** **上**，選取 **系統指派**的 > 。 當 Azure 提示您確認時，請選取 **[是]** 。
