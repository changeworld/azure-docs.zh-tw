...
如果您在這裡找不到您想要的動作，請嘗試流覽 Azure Logic Apps 提供的許多不同 [資料操作函數](../logic-apps/workflow-definition-language-functions-reference.md) 。
...
| [**選擇**](#select-action) | 從不同陣列中所有項目的指定屬性建立陣列。 |
||| 
...
若要從多個輸入建立單一輸出（例如 JSON 物件），您可以使用 **撰寫** 動作。 您的輸入可具有多種不同類型，例如整數、布林值、陣列、JSON 物件，以及 Azure Logic Apps 所支援的任何其他原生類型，例如二進位檔與 XML。 接著，您可以在**撰寫**動作之後的動作中使用其輸出。 **撰寫**動作也可讓您免於在建立邏輯應用程式的工作流程時重複輸入相同的內容。
...
1. 在 [Azure 入口網站](https://portal.azure.com)或 Visual Studio 的「邏輯應用程式設計工具」中，開啟邏輯應用程式。
   ...
   ![正在啟動「撰寫」動作的範例邏輯應用程式](./media/logic-apps-perform-data-operations/sample-starting-logic-app-compose-action.png)

1. 在您要建立輸出的邏輯應用程式中，依照下列其中一個步驟操作： 

   * 若要在最後一個步驟底下新增動作，請選取 [ **新增步驟**]。

     ![針對 [撰寫] 動作選取 [新步驟]](./media/logic-apps-perform-data-operations/add-compose-operation-action.png)

   * 若要在步驟之間新增動作，請將滑鼠移至連接箭號上方，以 **+** 顯示加號 () 。 選取加號，然後選取 [ **新增動作**]。

1. 在 [選擇動作] 底下的搜尋方塊中，輸入 `compose` 作為篩選條件。 從 [動作] 清單中選取 [ **撰寫** ] 動作。
...
若要建立逗點分隔值 (CSV) 資料表，其中具有陣列中 JavaScript 物件標記法 (JSON) 物件的屬性和值，請使用 [ **建立 CSV 資料表** ] 動作。 接著，您可以在**建立 CSV 資料表**動作之後的動作中使用產生的資料表。
...
1. 在您要建立 CSV 資料表的邏輯應用程式中，依照下列其中一個步驟操作： 
   ...
   * 若要在最後一個步驟底下新增動作，請選取 [ **新增步驟**]。
     ...
     ![針對 [建立 CSV 資料表] 動作選取 [新步驟]](./media/logic-apps-perform-data-operations/add-create-table-action.png)
...
依預設，[資料 **行** ] 屬性會設定為根據陣列專案自動建立資料表資料行。 若要指定自訂標頭和值，請遵循下列步驟：
...
例如，假設您想要只有屬性值的資料表資料行，而不是陣列中的屬性名稱。 若只要傳回這些值，請依照下列步驟在設計工具或程式碼視圖中工作。 以下是此範例傳回的結果：
...
在動作中，將 **標頭資料** 行保留空白。 在 [值] 資料行中的每個資料列上， **取值** 為每個您想要的陣列屬性。 **Value**底下的每個資料列都會傳回指定陣列屬性的所有值，並成為資料表中的資料行。

1. 在 [ **值**] 底下，于您想要的每個資料列上，按一下編輯方塊，使動態內容清單出現。

1. 在動態內容清單中，選取 [運算式]。

1. 在 [運算式編輯器] 中，輸入此運算式，以指定您想要的陣列屬性值，然後選取 **[確定]**。
   ...
   * `item()?['Product_ID']`
   ...
   !["Create CSV table" 的取值 "Description"](./media/logic-apps-perform-data-operations/csv-table-expression.png)

1. 針對您想要的每個陣列屬性重複上述步驟。 當您完成時，您的動作看起來會像這個範例：

   !["Create CSV table" 中的 "item ( # A1" 函數](./media/logic-apps-perform-data-operations/finished-csv-expression.png)

1. 若要將運算式解析成更描述性的版本，請切換至 [程式碼設計]，然後再重新開啟折迭的動作：
...
在動作的 JSON 定義中，于 `columns` 陣列內將屬性設定 `header` 為空字串。 針對每個 `value` 屬性，取值為每個您想要的陣列屬性。

1. 在設計工具的工具列上，選取 [程式 **代碼視圖**]。

1. 在程式碼編輯器的動作 `columns` 陣列中， `header` `value` 為您想要的陣列值的每個資料行加入空的屬性和這個運算式：
...
1. 切換回設計工具視圖，然後重新開啟折迭的動作。
   ...
   [ **建立 CSV 資料表** ] 動作現在看起來就像這個範例，而運算式已解析成更具描述性的版本：

   ![「建立 CSV 資料表」-已解析的運算式和沒有標頭](./media/logic-apps-perform-data-operations/resolved-csv-expression.png)
...
1. 在該動作中，按一下您想要顯示結果的任何位置。 動態內容清單開啟時，請在**建立 CSV 資料表**動作下方選取 [輸出]****。 
   ...
   此範例使用 Office 365 Outlook **傳送電子郵件** 動作，並在電子郵件的本文中包含 **輸出** 欄位：
...
若要建立 HTML 資料表，此資料表具有陣列中 JavaScript 物件標記法 (JSON) 物件的屬性和值，請使用 [ **建立 HTML 資料表** ] 動作。 接著，您可以在**建立 HTML 資料表**動作之後的動作中使用產生的資料表。
...
1. 在您要建立 HTML 資料表的邏輯應用程式中，依照下列其中一個步驟操作：
   ...
   * 若要在最後一個步驟底下新增動作，請選取 [ **新增步驟**]。
     ...
     ![針對 [建立 HTML 資料表] 動作選取 [新步驟]](./media/logic-apps-perform-data-operations/add-create-table-action.png)
...
1. 在 [來源]**** 方塊中，提供您要用來建立資料表的陣列或運算式。
   ...
   !["Create HTML table" 的完成範例](./media/logic-apps-perform-data-operations/finished-create-html-table-action.png)
...
若要從陣列傳回值，您可以使用[ `item()` 函數](../logic-apps/workflow-definition-language-functions-reference.md#item)搭配**Create HTML table**動作。 在 `For_each` 迴圈中，您可以使用[ `items()` 函數](../logic-apps/workflow-definition-language-functions-reference.md#items)。
...
在動作中，將 **標頭資料** 行保留空白。 在 [值] 資料行中的每個資料列上， **取值** 為每個您想要的陣列屬性。 [ **值** ] 底下的每個資料列都會傳回指定之屬性的所有值，並且成為資料表中的資料行。
...
1. 針對您想要的每個陣列屬性重複上述步驟。 當您完成時，您的動作看起來會像這個範例：
   ...
   !["Create HTML table" 中的 "item ( # A1" 函式](./media/logic-apps-perform-data-operations/finished-html-expression.png)
...
1. 切換回設計工具視圖，然後重新開啟折迭的動作。
   ...
   [ **建立 HTML 資料表** ] 動作現在看起來就像這個範例，而運算式已解析成更具描述性的版本：

   ![「建立 HTML 資料表」-已解析的運算式和沒有標頭](./media/logic-apps-perform-data-operations/resolved-html-expression.png)
...
若要從現有的陣列建立具有符合特定準則之專案的較小陣列，請使用 **篩選陣列** 動作。 接著，您可以在**篩選陣列**動作之後的動作中使用已篩選的陣列。
...
> 否則， **篩選陣列** 動作只能讀取回應主體，而不是 JSON 承載的結構。
...
1. 在您要建立已篩選陣列的邏輯應用程式中，依照下列其中一個步驟操作： 
   ...
   * 若要在最後一個步驟底下新增動作，請選取 [ **新增步驟**]。
     ...
     ![針對「篩選陣列」動作選取 [新步驟]](./media/logic-apps-perform-data-operations/add-filter-array-action.png)

   * 若要在步驟之間新增動作，請將滑鼠移至連接箭號上方，以 **+** 顯示加號 () 。 選取加號，然後選取 [ **新增動作**]。

1. 在搜尋方塊中，輸入 `filter array` 作為篩選條件。 從 [動作] 清單中選取 [ **篩選陣列** ] 動作。
...
1. 針對條件，請指定要比較的陣列項目、選取比較運算子，並指定比較值。
   ...
   此範例會使用 `item()` 函數來存取陣列中的每個專案，而 **篩選陣列** 動作會搜尋值大於1的陣列專案：
...
1. 在該動作中，按一下您想要顯示結果的任何位置。 當動態內容清單開啟時，請選取 [ **運算式**]。 若要取得**篩選陣列**動作的陣列輸出，請輸入此運算式，並在其中包含**篩選陣列**動作的名稱：

   `@actionBody('Filter_array')`

   此範例會使用 Office 365 Outlook **傳送電子郵件** 動作，並在電子郵件的本文中包含來自 **>actionbody ( ' Filter_array ' ) ** 運算式的輸出：
...
若要建立一個字串，其中包含陣列中的所有專案，並以特定分隔符號分隔這些專案，請使用 **聯結** 動作。 接著，您可以在**聯結**動作之後的動作中使用該字串。
...
1. 在 [Azure 入口網站](https://portal.azure.com)或 Visual Studio 的「邏輯應用程式設計工具」中，開啟邏輯應用程式。
   ...
   ![正在啟動「聯結」動作的範例邏輯應用程式](./media/logic-apps-perform-data-operations/sample-starting-logic-app-join-action.png)

1. 在您要從陣列建立字串的邏輯應用程式中，依照下列其中一個步驟操作：

   * 若要在最後一個步驟底下新增動作，請選取 [ **新增步驟**]。

     !["Join" 動作的選取 "New step"](./media/logic-apps-perform-data-operations/new-step-add-join-action.png)
...
1. 在該動作中，按一下您想要顯示結果的任何位置。 動態內容清單開啟時，請在**聯結**動作下方選取 [輸出]****。 
   ...
   ![[聯結] 動作的 [輸出] 欄位](./media/logic-apps-perform-data-operations/send-email-join-action.png)
...
若要參考或存取 JavaScript 物件標記法 (JSON) 內容中的屬性，您可以使用 **PARSE JSON** 動作為這些屬性建立易記的欄位或權杖。 如此，您即可在指定邏輯應用程式的輸入時從動態內容清單中選取這些屬性。 針對此動作，您可以提供 JSON 結構描述，或從範例 JSON 內容或承載產生 JSON 結構描述。
...
1. 在 [Azure 入口網站](https://portal.azure.com)或 Visual Studio 的「邏輯應用程式設計工具」中，開啟邏輯應用程式。
   ...
   ![正在啟動「剖析 JSON」動作的範例邏輯應用程式](./media/logic-apps-perform-data-operations/sample-starting-logic-app-parse-json-action.png)

1. 在您要剖析 JSON 內容的邏輯應用程式中，依照下列其中一個步驟操作：

   * 若要在最後一個步驟底下新增動作，請選取 [ **新增步驟**]。

     ![針對「剖析 JSON」動作選取 [新步驟]](./media/logic-apps-perform-data-operations/add-parse-json-action.png)

   * 若要在步驟之間新增動作，請將滑鼠移至連接箭號上方，以 **+** 顯示加號 () 。 選取加號，然後選取 [ **新增動作**]。

1. 在搜尋方塊中，輸入 `parse json` 作為篩選條件。 從 [動作] 清單中選取 [ **剖析 JSON** ] 動作。
...
1. 輸入 JSON 結構描述以說明您要剖析的 JSON 內容。
   ...
   1. 在 [ **輸入或貼上範例 json**承載] 下，提供 JSON 內容，然後選取 [ **完成**]。
...
1. 在該動作中，按一下您想要顯示結果的任何位置。 現在，當動態內容清單開啟時，您可以在**剖析 JSON** 動作下方，從已剖析的 JSON 內容中選取屬性。
   ...
   此範例使用 Office 365 Outlook **傳送電子郵件** 動作，並在電子郵件的本文中包含 **FirstName**、 **LastName**和 **email** 欄位：

   ![[傳送電子郵件] 動作中的 JSON 屬性](./media/logic-apps-perform-data-operations/send-email-parse-json-action.png)

   以下是完成的電子郵件動作：

   ![完成的電子郵件動作範例](./media/logic-apps-perform-data-operations/send-email-parse-json-action-2.png)
...
若要建立一個陣列，其中包含從現有陣列中的值建立的 JSON 物件，請使用 **Select** 動作。 例如，您可以為整數陣列中的每個值建立 JSON 物件，只要指定每個 JSON 物件所必須具有的屬性，並指定如何將來源陣列中的值對應至這些屬性即可。 此外，雖然您可以變更這些 JSON 物件中的元件，但輸出陣列的項目數一律會與來源陣列相同。
...
1. 在 [Azure 入口網站](https://portal.azure.com)或 Visual Studio 的「邏輯應用程式設計工具」中，開啟邏輯應用程式。
   ...
   ![正在啟動「選取」動作的範例邏輯應用程式](./media/logic-apps-perform-data-operations/sample-starting-logic-app-select-action.png)

1. 在您要建立陣列的邏輯應用程式中，依照下列其中一個步驟操作： 

   * 若要在最後一個步驟底下新增動作，請選取 [ **新增步驟**]。

     ![針對 [選取] 動作選取 [新步驟]](./media/logic-apps-perform-data-operations/add-select-operation-action.png)

   * 若要在步驟之間新增動作，請將滑鼠移至連接箭號上方，以 **+** 顯示加號 () 。 選取加號，然後選取 [ **新增動作**]。

1. 在 [選擇動作] 底下，選取 [內建]。 在搜尋方塊中，輸入 `select` 作為篩選條件。 從 [動作] 清單中選取 [ **選取** ] 動作。
...
1. 在 [對應]**** 方塊左側的資料行中，提供您要在來源陣列中指派各個值的屬性名稱。 在右側的資料行中指定運算式，代表您要指派給屬性的值。
   ...
   這個範例會指定 "Product_ID" 作為屬性名稱，藉由 `item()` 在存取每個陣列專案的運算式中使用函式，來指派整數陣列中的每個值。 

   ![指定 JSON 物件屬性和值以建立陣列](./media/logic-apps-perform-data-operations/configure-select-action-2.png)
...
1. 在該動作中，按一下您想要顯示結果的任何位置。 當動態內容清單開啟時，請選取 [ **運算式**]。 若要取得**選取**動作的陣列輸出，請輸入此運算式，並在其中包含**選取**動作的名稱：

   `@actionBody('Select')`

   此範例使用 Office 365 Outlook **傳送電子郵件** 動作，並 `@actionBody('Select')` 在電子郵件的本文中包含來自運算式的輸出：
