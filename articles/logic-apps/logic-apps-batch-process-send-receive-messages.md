...
若要以特定方式以群組的形式傳送和處理訊息，您可以建立批次處理解決方案。 此解決方案會將訊息收集到*批次*中，並等到您指定的準則符合您的條件，然後再釋放和處理批次處理的訊息。 批次處理可降低您的邏輯應用程式處理訊息的頻率。

本文說明如何藉由在相同 Azure 訂用帳戶、Azure 區域內建立兩個邏輯應用程式，並依照下列順序來建立批次處理解決方案：

1. [「批次接收者」](#batch-receiver)邏輯應用程式，會接受訊息並收集到批次中，直到符合指定準則而可釋出和處理這些訊息。 請確定您先建立此批次接收者，以便稍後在建立批次傳送者時選取批次目的地。
...
* 若要使用 Visual Studio 而不是 Azure 入口網站，請確定您已[設定使用 Logic Apps 的 Visual Studio](../logic-apps/quickstart-create-logic-apps-with-visual-studio.md)。
...
1. 在邏輯應用程式設計工具中，新增**批次**觸發程式，以啟動您的邏輯應用程式工作流程。 在搜尋方塊中，輸入 `batch` ，然後選取此觸發程式：**批次訊息**
...
1. 現在新增可處理每個批次的一或多個動作。
   ...
   1. 在搜尋方塊中，輸入 `send email` 作為篩選條件。 根據您的電子郵件提供者，選取電子郵件連接器。
      ...
      例如，如果您有工作或學校帳戶，例如 @fabrikam.com 或 @fabrikam.onmicrosoft.com ，請選取 [ **Microsoft 365 Outlook**連接器]。 如果您有個人帳戶，例如 @outlook.com 或 @hotmail.com ，請選取**Outlook.com**連接器。 這個範例會使用 Microsoft 365 Outlook 連接器。
...
1. 設定所新增動作的屬性。
   ...
   * 在 [**主體**] 方塊中，當動態內容清單出現時，請選取 [**訊息識別碼**] 欄位。
     ...
     邏輯應用程式設計工具會自動在傳送電子郵件動作前後新增**for each**迴圈，因為該動作會將上一個動作的輸出視為集合，而不是批次。
...
1. 儲存您的邏輯應用程式。 您現在已建立批次接收者。
   ...
   > [!IMPORTANT]
   > 如果您使用 Visual Studio，在繼續下一節之前，請先確定您已先將[批次接收者邏輯應用程式*部署*至 Azure](../logic-apps/quickstart-create-logic-apps-with-visual-studio.md#deploy-logic-app-to-azure)。 否則，當您建立批次傳送者時，將無法選取批次接收者。
...
* 請確定您先前已[建立並部署您的批次接收者](#batch-receiver)，因此當您建立批次傳送者時，可以選取現有的批次接收者作為目的地批次。 雖然批次接收者不需要知道批次傳送者的任何訊息，但批次傳送者必須知道訊息的傳送目的地。

* 請確定您的批次接收者和批次傳送者共用相同的 Azure 區域*和*azure 訂用帳戶。 如果未共用，您就無法在建立批次傳送者時選取批次接收者，原因是兩者並無法看到彼此。

1. 使用下列名稱建立另一個邏輯應用程式：`BatchSender`
...
1. 新增動作來將訊息傳送至批次。
   ...
   1. 在 [**迴圈**] 觸發程式下，選取 [**新增步驟**]。

   1. 在 [搜尋] 方塊中，輸入 `batch` 做為您的篩選準則，然後選取此動作：**選擇包含批次觸發程式的 Logic Apps 工作流程**

      ![選取 [選擇包含批次觸發程序的 Logic Apps 工作流程]](./media/logic-apps-batch-process-send-receive-messages/send-messages-batch-action.png)

      清單隨即出現，而且只會顯示具有批次觸發程式且存在於相同 Azure 區域*和*Azure 訂用帳戶作為 batch 傳送者邏輯應用程式的邏輯應用程式。

   1. 從 [邏輯應用程式] 清單中，選取您先前建立的批次接收者邏輯應用程式。

      ![選取您的批次接收者邏輯應用程式](./media/logic-apps-batch-process-send-receive-messages/batch-sender-select-batch-receiver.png)

      > [!IMPORTANT]
      > 如果您使用 Visual Studio，而且沒有看到任何要選取的批次接收者，請檢查您先前建立的批次接收者並將其部署至 Azure。 如果您尚未這麼做，請瞭解[如何將您的批次接收者邏輯應用程式部署至 Azure](../logic-apps/quickstart-create-logic-apps-with-visual-studio.md#deploy-logic-app-to-azure)。

   1. 從 [動作] 清單中，選取此動作： **Batch_messages-<*您的邏輯-應用程式名稱* > **
...
1. 設定批次傳送者的屬性：
   ...
   > [!NOTE]
   > **觸發程式名稱**和**工作流程**屬性值會自動填入您選取的邏輯應用程式。
   ...
   1. 輸入運算式 `utcnow()` ，然後選取 **[確定]**。
      ...
      ![在 [訊息內容] 中，選取 [運算式]，輸入 "utcnow （）"，然後選取 [確定]。](./media/logic-apps-batch-process-send-receive-messages/batch-sender-details.png)

1. 現在設定批次的資料分割。 在 `BatchReceiver` 動作中，開啟 [**加入新的參數**] 清單，然後選取下列屬性：
...
> [!IMPORTANT]
> 當您完成測試時，請務必停用 `BatchSender` 邏輯應用程式以停止傳送訊息，並避免多載您的收件匣。
