...
1. 在 [搜尋] 方塊中，輸入 `when a http request`，然後選取 [**收到 HTTP 要求時**] 作為觸發程式。
...
1. 在 [**要**解碼的訊息] 屬性中，輸入您想要讓 AS2 動作解碼的輸入，這是 HTTP 要求觸發程式所接收的 `body` 內容。 您可以透過多種方式，將此內容指定為輸入，不論是從動態內容清單或運算式：
   ...
   * 若要輸入參考觸發程式之 `body` 輸出的運算式，請在 [**要**解碼的訊息] 方塊內按一下。 動態內容清單出現之後，選取 [**運算式**]。 在 [運算式編輯器] 中，于此處輸入運算式，然後選取 **[確定]** ：
...
1. 針對 [**訊息標頭**] 屬性，輸入 AS2 動作所需的任何標頭，這會由 HTTP 要求觸發程式所接收的 `headers` 內容描述。

   若要輸入參考觸發程式之 `headers` 輸出的運算式，請按一下 [**訊息標頭**] 方塊內部。 動態內容清單出現之後，選取 [**運算式**]。 在 [運算式編輯器] 中，于此處輸入運算式，然後選取 **[確定]** ：
...
1. 現在指定要評估的條件。 在 [**選擇值**] 方塊中，輸入此運算式：
   ...
   在中間方塊中，確定比較作業設定為 [`is equal to`]。 在右側方塊中，輸入 `Expected`的值。 若要取得要解析為此標記的運算式，請在設計工具和程式碼視圖之間切換。

   ![具有決策路徑的條件圖形](./media/logic-apps-enterprise-integration-b2b/expression-for-evaluating-condition.png)

1. 現在指定回應，以傳回**AS2**解碼動作是否成功。

   1. 針對**AS2**解碼動作成功的情況，請在 [**若為 true** ] 圖形中，選取 [**新增動作**]。 在 [**選擇動作**] 下的 [搜尋] 方塊中，輸入 `response`，然後選取 [**回應**]。
   ...
   1. 針對**AS2**解碼動作失敗的情況，請在 [**若為 false** ] 圖形中選取 [**新增動作**]。 在 [**選擇動作**] 下的 [搜尋] 方塊中，輸入 `response`，然後選取 [**回應**]。 設定**回應**動作，以傳回您想要的狀態和錯誤。
...
* [使用 SSH 和 Azure Logic Apps 來監視、建立及管理 SFTP 檔案](../connectors/connectors-sftp-ssh.md)
