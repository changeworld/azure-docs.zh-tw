...
1. 針對 [**要**解碼的訊息] 屬性，輸入您要讓 AS2 動作解碼的輸入，這是 `body` HTTP 要求觸發程式所收到的內容。 您可以透過多種方式，將此內容指定為輸入，不論是從動態內容清單或運算式：
   ...
   * 若要輸入參考觸發程式輸出的運算式 `body` ，請在 [要解碼的**訊息**] 方塊內按一下。 動態內容清單出現之後，選取 [**運算式**]。 在 [運算式編輯器] 中，于此處輸入運算式，然後選取 **[確定]**：
...
1. 針對 [**訊息標頭**] 屬性，輸入 AS2 動作所需的任何標頭，由 `headers` HTTP 要求觸發程式收到的內容描述。

   若要輸入參考觸發程式輸出的運算式 `headers` ，請按一下 [**訊息標頭**] 方塊內部。 動態內容清單出現之後，選取 [**運算式**]。 在 [運算式編輯器] 中，于此處輸入運算式，然後選取 **[確定]**：
