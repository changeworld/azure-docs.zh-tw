...
| senderQualifier | Yes | String | 傳送夥伴辨識符號 |
| senderIdentifier | Yes | String | 傳送夥伴識別碼 |
| receiverQualifier | Yes | String | 接收夥伴辨識符號 |
| receiverQualifier | Yes | String | 接收合作夥伴識別碼 |
| agreementName | 否 | String | 據以解析訊息的 X12 合約名稱 |
| direction | Yes | 列舉 | 訊息流程的方向，也就是 `receive` 或`send` |
...
| segmentsCount | No | 整數 | X12 交易集中的區段數目 |
|||||
...
| segmentsCount | Yes | 列舉 | 具有下列允許值的認可狀態碼： `Accepted` 、 `Rejected` 和`AcceptedWithErrors` |
| processingStatus | Yes | 列舉 | 具有下列允許值的確認處理狀態： `Received` 、 `Generated` 和`Sent` |
| CorrelationMessageId | 否 | String | 相互關聯訊息識別碼，這是 {AgreementName} {*GroupControlNumber*} {TransactionSetControlNumber} 的組合 |
| isMessageFailed | 是 | Boolean | X12 訊息是否失敗 |
...
| isMessageFailed | 是 | Boolean | X12 訊息是否失敗 |
| StatusCode | Yes | 列舉 | 交換通知狀態碼與下列允許的值： `Accepted` 、 `Rejected` 和`AcceptedWithErrors` |
| processingStatus | Yes | 列舉 | 具有下列允許值的認可狀態： `Received` 、 `Generated` 和`Sent` |
| ta102 | 否 | String | 交換日期 |
| ta103 | 否 | String | 交換時間 |
...
| direction | Yes | 列舉 | 訊息流程的方向，可以是接收或傳送 |
| interchangeControlNumber | 否 | String | 交換控制編號 |
| functionalGroupControlNumber | 否 | String | 功能控制編號 |
...
| StatusCode | Yes | 列舉 | 具有下列允許值的認可狀態碼： `Accepted` 、 `Rejected` 和`AcceptedWithErrors` |
| processingStatus | Yes | 列舉 | 具有下列允許值的確認處理狀態： `Received` 、 `Generated` 和`Sent` |
