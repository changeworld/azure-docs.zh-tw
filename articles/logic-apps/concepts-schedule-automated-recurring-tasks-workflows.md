...
| 觸發程序 | 週期性 | 間隔 | 頻率 | 開始時間 | 在這幾天內 | 在這幾小時內 | 在這幾分鐘內 | 注意 |
|---------|------------|----------|-----------|------------|---------------|----------------|------------------|------|
...
| 週期性 | 在您儲存邏輯應用程式時，每天上午8點*和*分鐘標記執行 | 1 | Day | {無} | {無法使用} | 8 | {無} | 如果沒有開始日期和時間，此排程會根據您儲存邏輯應用程式（PUT 作業）的時間來執行。 |
| 週期性 | 每天上午8:00 執行（有開始日期和時間） | 1 | Day | *startDate*T08:00:00Z | {無法使用} | {無} | {無} | 此排程不會「早於」** 指定的開始日期和時間開始。 未來每日在上午8:00 執行一次。 | 
| 週期性 | 每天上午8:00 執行（沒有開始日期和時間） | 1 | Day | {無} | {無法使用} | 8 | 00 | 此排程會在每天上午8:00 執行。 |
| 週期性 | 每天上午8:00 和 4:00 PM 執行 | 1 | Day | {無} | {無法使用} | 8, 16 | 0 | |
| 週期性 | 每天上午8:30、8:45 AM、4:30 PM 和 4:45 PM 執行 | 1 | Day | {無} | {無法使用} | 8, 16 | 30, 45 | |
| 週期性 | 在每個星期六下午5:00 執行（沒有開始日期和時間） | 1 | 週 | {無} | "Saturday" | 17 | 0 | 此排程在每個星期六的下午 5:00 執行。 |
| 週期性 | 在每個星期六下午5:00 執行（有開始日期和時間） | 1 | 週 | *startDate*T17:00:00Z | "Saturday" | {無} | {無} | 此排程不會「早於」** 指定的開始日期和時間開始，在此案例中是 2017 年 9 月 9 日下午 5:00。 未來會在每個星期六的下午 5:00 重複執行。 |
