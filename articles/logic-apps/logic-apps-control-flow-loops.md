...
若要重複執行動作直到條件符合或狀態改變，您可以建立 ["Until" 迴圈](#until-loop)。 邏輯應用首先運行迴圈內的所有操作，然後檢查條件或狀態。 如果符合條件，則迴圈會停止。 否則，迴圈會重複。 如需一個邏輯應用程式回合中可擁有的 "Until" 迴圈數上限，請參閱[限制和設定](../logic-apps/logic-apps-limits-and-config.md)。 
...
* "Foreach"迴圈中的操作使用[`@item()`](../logic-apps/workflow-definition-language-functions-reference.md#item) 
運算式以引用和處理陣列中的每個項。 如果您指定的資料不是在陣列中，則邏輯應用程式工作流程將會失敗。 
...
2. 在 RSS 觸發程序和傳送電子郵件動作之間，新增 "Foreach" 迴圈。 
   ...
   1. 在搜尋方塊下方，選擇 [全部]****。 在搜尋方塊中，輸入 "for each" 作為篩選條件。 從動作清單中選擇此操作：**每個操作 - 控制項**
...
要運行和重複操作，直到條件得到滿足或狀態更改，請將這些操作放在"直到"迴圈中。 邏輯應用首先運行迴圈中的任何和所有操作，然後檢查條件或狀態。 如果符合條件，則迴圈會停止。 否則，迴圈會重複。
...
1. 在搜尋方塊下方，選擇 [全部]****。 搜索"直到"，然後選擇此操作：直到 **- 控制**
...
1. 在搜尋方塊下方，選擇 [全部]****。 搜索"變數"，然後選擇此操作：**遞增變數 - 變數**
...
1. 設定電子郵件動作的屬性。 在主旨上新增**限制**變數。 這樣一來，您可以確認變數的目前值符合您指定的條件，例如：
      ...
      | 屬性 | 值 | 描述 |
      | -------- | ----- | ----------- | 
      | **自** | *\<電子郵件地址\@域>* | 收件者的電子郵件地址。 若要進行測試，請使用自己的電子郵件地址。 | 
      | **主旨** | [限制] 目前的值是**限制** | 指定電子郵件主旨。 在此範例中，請確定您已包含**限制**變數。 | 
      | **內文** | <*電子郵件內容*> | 指定您想要傳送的電子郵件訊息內容。 針對此範例，輸入任何您喜歡的文字。 | 
      |||| 
...
| **超時** | PT1H | 迴圈結束前可執行迴圈的最大時間。 預設值是一小時，並以 ISO 8601 格式指定。 <p>逾時值的評估會以每個迴圈循環為基礎。 如果迴圈中有任何動作所花費的時間超過逾時限制，目前的循環並不會停止。 但不會啟動下一個循環，因為不符合限制條件。 | 
|||| 
