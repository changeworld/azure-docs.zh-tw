...
以下是有關運算式中函數的一些其他注意事項：

* 函數參數會由左至右評估。

* 在參數定義的語法中，參數後方出現問號 (？) 表示此參數是選擇性的。 例如，請參閱 [getFutureTime()](#getFutureTime)。

下列各節根據其一般用途來組織函式，或者您可以[依字母順序](#alphabetical-list)流覽這些函數。
...
| 轉換函式 | 工作 |
| ------------------- | ---- |
| [數列](../logic-apps/workflow-definition-language-functions-reference.md#array) | 從單一指定輸入傳回的陣列。 如需多個輸入，請參閱[createArray](../logic-apps/workflow-definition-language-functions-reference.md#createArray)。 |
...
檢查運算式是 True 或 False。 根據結果，傳回指定的值。 參數會由左至右評估。
