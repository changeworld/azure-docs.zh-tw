...
| 屬性 | 必要 | 類型 | 描述 |
|----------|----------|------|-------------|
| senderPartnerName | 否 | 字串 | AS2 訊息寄件者的夥伴名稱 |
| receiverPartnerName | 否 | 字串 | AS2 訊息接收者的夥伴名稱 |
| as2To | 是 | String | As2 訊息的標頭中的 AS2 訊息接收者名稱 |
| as2From | 是 | String | As2 訊息的標頭中的 AS2 訊息寄件者名稱 |
| agreementName | 否 | 字串 | 據以解析訊息的 AS2 合約名稱 |
| direction | 是 | String | 訊息流程的方向，也就是`receive`或`send` |
| messageId | 否 | 字串 | As2 訊息標頭的 AS2 訊息識別碼 |
| dispositionType | 否 | 字串 | 訊息處置通知（MDN）配置類型值 |
| fileName | 否 | 字串 | AS2 訊息標頭中的檔案名 |
...
| correlationMessageId | 否 | 字串 | AS2 訊息識別碼，用來將訊息與 Mdn 相互關聯 |
...
| agreementName | 否 | 字串 | 據以解析訊息的 AS2 合約名稱 |
| direction | 是 | String | 訊息流程的方向，也就是`receive`或`send` |
| messageId | 否 | 字串 | AS2 訊息識別碼 |
| originalMessageId | 否 | 字串 | AS2 原始訊息識別碼 |
| dispositionType | 否 | 字串 | MDN 配置類型值 |
| isMessageFailed | 是 | Boolean | AS2 訊息是否失敗 |
| isMessageSigned | 是 | Boolean | 是否已簽署 AS2 訊息 |
| isNrrEnabled | 是 | Boolean | 如果不知道值，是否要使用預設值 |
...
| correlationMessageId | 否 | 字串 | 相互關聯識別碼，這是已設定 MDN 之原始訊息的識別碼 |
