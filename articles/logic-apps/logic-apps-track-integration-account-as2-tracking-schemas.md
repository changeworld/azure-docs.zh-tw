...
| as2To | 是 | String | As2 訊息接收者的名稱（來自 AS2 訊息的標頭） |
| as2From | 是 | String | As2 訊息寄件者的名稱（來自 AS2 訊息的標頭） |
| agreementName | 否 | String | 據以解析訊息的 AS2 合約名稱 |
| direction | 是 | String | 訊息流程的方向，也就是 `receive` 或 `send` |
| messageId | 否 | String | AS2 訊息標頭中的 AS2 訊息識別碼 |
| dispositionType | 否 | String | 訊息處置通知 (MDN) 處置類型值 |
| fileName | 否 | String | 來自 AS2 訊息標頭的檔案名 |
...
| incomingHeaders | 否 | JToken 的字典 | 傳入的 AS2 訊息標頭詳細資料 |
| outgoingHeaders | 否 | JToken 的字典 | 外寄 AS2 訊息標頭詳細資料 |
| isNrrEnabled | 是 | Boolean | 如果值未知，是否要使用預設值 |
| isMdnExpected | 是 | Boolean | 如果值未知，是否要使用預設值 |
| mdnType | 是 | 列舉 | 允許的值： `NotConfigured` 、 `Sync` 和 `Async` |
|||||
...
| senderPartnerName | 否 | String | AS2 訊息寄件者的夥伴名稱 |
| receiverPartnerName | 否 | String | AS2 訊息接收者的夥伴名稱 |
| as2To | 是 | String | 接收 AS2 訊息的夥伴名稱 |
| as2From | 是 | String | 傳送 AS2 訊息的夥伴名稱 |
| agreementName | 否 | String | 據以解析訊息的 AS2 合約名稱 |
| direction | 是 | String | 訊息流程的方向，也就是 `receive` 或 `send` |
...
| isMessageSigned | 是 | Boolean | AS2 訊息是否已簽署 |
| isNrrEnabled | 是 | Boolean | 如果值未知，是否要使用預設值 |
| statusCode | 是 | 列舉 | 允許的值： `Accepted` 、 `Rejected` 和 `AcceptedWithErrors` |
| micVerificationStatus | 是 | 列舉 | 允許的值： `NotApplicable` 、 `Succeeded` 和 `Failed` |
| correlationMessageId | 否 | String | 相互關聯識別碼，也就是已設定 MDN 之原始訊息的識別碼。 |
| incomingHeaders | 否 | JToken 的字典 | 傳入訊息標頭詳細資料 |
| outgoingHeaders | 否 | JToken 的字典 | 外寄訊息標頭詳細資料 |
|||||
