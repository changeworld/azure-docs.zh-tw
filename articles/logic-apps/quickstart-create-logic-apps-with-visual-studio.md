...
在本快速入門中，您會使用 Visual Studio 建立與 Azure 入口網站快速入門中相同的邏輯應用程式。 您也可以了解如何[在 Visual Studio Code中建立範例應用程式](quickstart-create-logic-apps-visual-studio-code.md)，並[透過 Azure 命令列介面 (Azure CLI) 建立和管理邏輯應用程式](quickstart-logic-apps-azure-cli.md)。此邏輯應用程式會監視網站的 RSS 摘要，並針對該摘要中的每個新專案傳送電子郵件。 您已完成的邏輯應用程式看起來像以下高階工作流程：
