---
title: 使用視覺化工作室 （ASP.NET） 開始使用 Azure 表存儲
...
* [Azure 存儲帳戶](../storage/common/storage-account-create.md)
...
1. 在 **CreateTable** 方法內，取得 **CloudStorageAccount** 物件，其代表您的儲存體帳戶資訊。 使用以下代碼從 Azure 服務配置獲取存儲連接字串和存儲帳戶資訊：（將*&lt;存儲帳戶名稱>* 更改為要訪問的 Azure 存儲帳戶的名稱。
...
1. 運行應用程式，然後選擇 **"創建表"** 以查看類似于以下螢幕截圖的結果：
...
*實體*使用派生自\#**表實體**的自訂類映射到 C 物件。 若要將實體新增至資料表，請建立一個類別來定義實體的屬性。 在本節中，您將了解如何定義一個使用客戶名字作為資料列索引鍵、並使用姓氏作為資料分割索引鍵的實體類別。 實體的資料分割索引鍵和資料列索引鍵共同唯一識別資料表中的實體。 查詢具有相同分割區索引鍵的實體，其速度快於查詢具有不同分割區索引鍵的實體，但使用不同的資料分割索引鍵可提供更佳的延展性或平行作業。 應該儲存在資料表服務中的任何屬性，都必須是公開設定和擷取值之支援類型的公用屬性。
實體類別必須** 宣告公用的無參數建構函式。
...
1. 在 **AddEntity** 方法內，取得 **CloudStorageAccount** 物件，其代表您的儲存體帳戶資訊。 使用以下代碼從 Azure 服務配置獲取存儲連接字串和存儲帳戶資訊：（將*&lt;存儲帳戶名稱>* 更改為要訪問的 Azure 存儲帳戶的名稱。
...
1. 運行應用程式，然後選擇 **"添加實體**"以查看類似于以下螢幕截圖的結果：
...
除了能夠[一次向表添加一個實體](#add-an-entity-to-a-table)外，還可以大量新增實體。 在批次中新增實體可減少程式碼與 Azure 資料表服務之間的來回行程次數。 下列步驟說明如何使用單一插入作業將多個實體新增至資料表：
...
1. 在 **GetSingle** 方法內，取得 **CloudStorageAccount** 物件，其代表您的儲存體帳戶資訊。 使用以下代碼從 Azure 服務配置獲取存儲連接字串和存儲帳戶資訊：（將*&lt;存儲帳戶名稱>* 更改為要訪問的 Azure 存儲帳戶的名稱。
...
1. 建立擷取作業物件，其採用衍生自 **TableEntity** 的實體物件。 第一個參數是 partitionKey**，而第二個參數是 rowKey**。 使用"將[一批實體添加到表](#add-a-batch-of-entities-to-a-table)"一節中提供的**CustomerEntity**類和資料，以下程式碼片段查詢具有*分區鍵*值為"Smith"和 *"Ben"的行鍵*值**的客戶實體**的表：
...
1. 運行應用程式，然後選擇 **"獲取單"** 以查看類似于以下螢幕截圖的結果：
...
1. 在 **GetPartition** 方法內，取得 **CloudStorageAccount** 物件，其代表您的儲存體帳戶資訊。 使用以下代碼從 Azure 服務配置獲取存儲連接字串和存儲帳戶資訊：（將*&lt;存儲帳戶名稱>* 更改為要訪問的 Azure 存儲帳戶的名稱。
...
1. 具現化 **TableQuery** 物件，以在 **Where** 子句中指定查詢。 使用"將[一批實體添加到表](#add-a-batch-of-entities-to-a-table)"一節中提供的**CustomerEntity**類和資料，以下程式碼片段查詢**分區鍵**（客戶姓氏）的值為"Smith"的所有實體的表：
...
1. 運行應用程式，然後選擇 **"獲取分區**"以查看類似于以下螢幕截圖的結果：
...
1. 在 **DeleteEntity** 方法內，取得 **CloudStorageAccount** 物件，其代表您的儲存體帳戶資訊。 使用以下代碼從 Azure 服務配置獲取存儲連接字串和存儲帳戶資訊：（將*&lt;存儲帳戶名稱>* 更改為要訪問的 Azure 存儲帳戶的名稱。
...
1. 運行應用程式，然後選擇 **"刪除實體**"以查看類似于以下螢幕截圖的結果：
...
如需了解 Azure 中的其他資料儲存選項，請檢視更多功能指南。
  ...
  * [開始使用 Azure Blob 存儲和視覺化工作室連接服務（ASP.NET）](../storage/vs-storage-aspnet-getting-started-blobs.md)
