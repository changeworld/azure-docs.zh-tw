...
ms.locfileid: "87321729"
---
# <a name="create-external-stream-transact-sql"></a>CREATE EXTERNAL STREAM (Transact-SQL)

外部資料流物件的輸入和輸出資料流程都具有雙重用途。 它可以做為輸入，以查詢來自事件內嵌服務（例如 Azure 事件中樞、Azure IoT 中樞（或 Edge 中樞）或 Kafka）的串流資料，或可作為輸出以指定從串流查詢儲存結果的位置和方式。

您也可以指定和建立 EXTERNAL STREAM 來作為事件中樞或 Blob 儲存體等服務的輸出和輸入。 這可促進串流查詢將結果保存到外部資料流做為輸出，以及從相同的外部資料流讀取另一個串流查詢做為輸入的連結案例。
...
| Kafka | Y | N | 用來從 Kafka 主題讀取串流資料的資料來源。 Azure SQL Edge 的 ARM64 版本不提供 Kafka 支援。|
...
- **LOCATION**：指定資料來源中實際資料或位置的名稱。 
   - 針對 Edge 中樞或 Kafka stream 物件，location 會指定要讀取或寫入的 Edge 中樞或 Kafka 主題的名稱。
   - 若為 SQL 資料流程物件（SQL Server、Azure SQL Database 或 Azure SQL Edge）位置，則指定資料表的名稱。 如果資料流程建立在與目的地資料表相同的資料庫和架構中，則只是資料表名稱尾碼。 否則，您必須完整限定（<database_name. schema_name. table_name）資料表名稱。
   - 針對 Azure Blob 儲存體資料流程物件位置指的是要在 Blob 容器內使用的路徑模式。 如需這項功能的詳細資訊，請參閱（/articles/stream-analytics/stream-analytics-define-outputs.md # blob-storage-和-azure-data-lake-gen2）

- **INPUT_OPTIONS**：為串流查詢輸入的服務（例如 Kafka、IoT Edge 中樞）指定選項做為索引鍵/值組
    - 分割區：為主題定義的資料分割數目
      - 適用于 Kafka 輸入資料流程
    - CONSUMER_GROUP：事件中樞和 IoT 中樞會限制一個取用者群組可擁有的讀者數量 (最多 5 個)。 將此欄位保留空白則會使用 '$Default' 取用者群組。
      - 保留供未來使用。 不適用於 Azure SQL Edge。  
...
- **OUTPUT_OPTIONS**：針對輸出至串流查詢的受支援服務，將選項指定為索引鍵/值組 
  ...
  - MINIMUM_ROWS：  
    ...
    - 適用於所有受支援的輸出 
  - MAXIMUM_TIME：  
    每個批次的等候時間上限（分鐘）。 在此時間後，即使未符合資料列下限需求，也會將批次寫入輸出。 
    - 適用於所有受支援的輸出 
