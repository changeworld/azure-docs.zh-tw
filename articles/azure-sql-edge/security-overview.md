...
- *SQL 驗證*：
    ...
    如需有關在 SQL Edge 中建立和管理登入和使用者的詳細資訊，請參閱 [建立登](/sql/relational-databases/security/authentication-access/create-a-login) 入和 [建立資料庫使用者](/sql/relational-databases/security/authentication-access/create-a-database-user)。

### <a name="authorization"></a>授權   

授權是指指派給 Azure SQL Edge 資料庫中之使用者的許可權，並決定允許使用者執行的動作。 許可權的控制方式是將使用者帳戶加入至 [資料庫角色](/sql/relational-databases/security/authentication-access/database-level-roles) ，並將資料庫層級許可權指派給這些角色，或授與使用者某些 [物件層級許可權](/sql/relational-databases/security/permissions-database-engine)。 如需詳細資訊，請參閱登入 [和使用者](../azure-sql/database/logins-create-manage.md)。
...
|如需下列資訊|請參閱|  
|---------------------------|---------|  
|伺服器與資料庫使用者、角色和處理序|[主體資料庫引擎](/sql/relational-databases/security/authentication-access/principals-database-engine)|  
|伺服器與資料庫物件安全性|[安全性實體](/sql/relational-databases/security/securables)|
| &nbsp; | &nbsp; |
...
|如需下列資訊|請參閱|  
|---------------------------|---------|  
|實作安全連接|[加密連接](/sql/linux/sql-server-linux-encrypted-connections)|  
|加密函數|[密碼編譯函數 &#40;Transact-SQL&#41;](/sql/t-sql/functions/cryptographic-functions-transact-sql)|
|待用資料加密|[透明資料加密](/sql/relational-databases/security/encryption/transparent-data-encryption)|
|Always Encrypted|[一律加密](/sql/relational-databases/security/encryption/always-encrypted-database-engine)|
| &nbsp; | &nbsp; |

> [!NOTE]
> 針對 [Linux 上的 SQL Server](/sql/linux/sql-server-linux-security-overview) 所述的安全性限制也適用于 Azure SQL Edge。 
...
與 Azure SQL 和 Microsoft SQL Server 類似，Azure SQL Edge 也提供相同的機制來建立和使用憑證，以增強物件和連線安全性。 如需詳細資訊，請參閱 [建立 (transact-sql) 的憑證 ](/sql/t-sql/statements/create-certificate-transact-sql)。
...
Azure SQL Edge 安全性最佳作法包括撰寫安全的用戶端應用程式。 如需如何在網路層維護用戶端應用程式安全的詳細資訊，請參閱 [用戶端網路組態](/sql/database-engine/configure-windows/client-network-configuration)。
...
|函數和 views|連結|  
|---------------------------|---------|  
|安全性目錄檢視，會傳回資料庫層級和伺服器層級許可權、主體、角色等等的相關資訊。 此外，另有若干目錄檢視提供了加密金鑰、憑證與認證的相關資訊。|[安全性目錄檢視 &#40;Transact-SQL&#41;](/sql/relational-databases/system-catalog-views/security-catalog-views-transact-sql)|  
|安全性函數，傳回目前使用者、許可權和架構的相關資訊。|[安全性函數 &#40;Transact-SQL&#41;](/sql/t-sql/functions/security-functions-transact-sql)|  
|安全性動態管理的觀點。|[安全性相關的動態管理檢視和函數 &#40;Transact-SQL&#41;](/sql/relational-databases/system-dynamic-management-views/security-related-dynamic-management-views-and-functions-transact-sql)|  
| &nbsp; | &nbsp; |
...
- [適用於 IoT 的 Azure 資訊安全中心](../defender-for-iot/overview.md)