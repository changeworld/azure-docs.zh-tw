...
description: 本檔將探討 Azure IoT 中樞裝置布建服務 (DPS) Sdk 以進行開發時所提供的工具
author: wesmc7777
ms.author: wesmc
ms.date: 04/09/2018
ms.topic: conceptual
...
[X.509 憑證](./concepts-x509-attestation.md#x509-certificates)可用來作為證明機制，以調整生產環境並簡化裝置佈建。  有[數種方式](../iot-hub/iot-hub-x509ca-overview.md#how-to-get-an-x509-ca-certificate)可取得 X.509 憑證：
...
6. 使用下列命令來執行工具：
   ...
   ```
7. 出現提示時，您可以選擇輸入憑證的 _一般名稱_。
8. 此工具會在本機產生 **用戶端憑證**、**用戶端憑證私密金鑰**、**中繼憑證** 及 **根憑證**。

**用戶端憑證** 是裝置上的分葉憑證。  **用戶端憑證** 和相關聯的 **用戶端憑證私密金鑰** 都是裝置用戶端中所需的項目。 根據您選擇的語言而定，將此項目置於用戶端應用程式中的機制可能有所不同。  如需詳細資訊，請參閱關於使用 X.509 建立模擬裝置的[快速入門](./quick-create-simulated-device-x509.md)。

根憑證或中繼憑證均可[以程式設計方式](./how-to-manage-enrollments-sdks.md)或使用[入口網站](./how-to-manage-enrollments.md)來建立註冊群組或個別註冊。
