---
title: 在 Linux VM 上使用虛擬 TPM 預配設備 - Azure IoT 邊緣
...
---
# <a name="create-and-provision-an-iot-edge-device-with-a-virtual-tpm-on-a-linux-virtual-machine"></a>在 Linux 虛擬機器上使用虛擬 TPM 創建和預配 IoT 邊緣設備

Azure IoT 邊緣設備可以使用[設備預配服務](../iot-dps/index.yml)自動預配。 如果您不熟悉自動預配過程，請查看[自動預配概念](../iot-dps/concepts-auto-provisioning.md)，然後再繼續。
...
> [!TIP]
> 本文介紹如何使用 TPM 模擬器測試 DPS 預配，但大部分都適用于物理 TPM 硬體，如用於 IoT 設備的 Azure 認證的[Itfineon&trade; OPTIGA TPM。](https://catalog.azureiotsolutions.com/details?title=OPTIGA-TPM-SLB-9670-Iridium-Board)
>
...
在本節中，您將在 Hyper-V 上創建新的 Linux 虛擬機器。 使用類比 TPM 配置此虛擬機器，以便使用它測試自動預配如何與 IoT Edge 配合使用。
...
2. 再次在 Hyper-V 管理器中，在 **"操作"** 功能表中選擇 **"新** > **虛擬機器**"。

3. 使用下列特定組態完成 [新增虛擬機器精靈]****：

   1. **指定世代**：選取 [第 2 代]****。 第 2 代虛擬機器已啟用嵌套虛擬化，這是在虛擬機器上運行 IoT Edge 所必需的。
...
創建 VM 後，請打開其設置以啟用虛擬受信任的平臺模組 （TPM），該模組允許您自動預配設備。
...
在虛擬機器中，生成一個工具，可用於檢索設備的**註冊 ID**和**背書金鑰**。

1. 啟動虛擬機器並連接到虛擬機器。

1. 按照虛擬機器中的提示完成安裝過程並重新啟動電腦。

1. 登錄到 VM，然後按照[設置 Linux 開發環境](https://github.com/Azure/azure-iot-sdk-c/blob/master/doc/devbox_setup.md#linux)中的步驟安裝和構建用於 C 的 Azure IoT 設備 SDK。

   >[!TIP]
   >在本文中，您將從虛擬機器複製和粘貼，這很難通過 Hyper-V 管理器連接應用程式。 您可能希望通過 Hyper-V 管理器連接到虛擬機器一次，以檢索其 IP 位址： `ifconfig`。 然後，您可以使用 IP 位址通過 SSH 進行連接`ssh <username>@<ipaddress>`： 。

1. 運行以下命令以構建 SDK 工具，該工具從 TPM 模擬器檢索設備預配資訊。
...
1. 從命令視窗導航到`azure-iot-sdk-c`目錄並運行 TPM 模擬器。 它會透過連接埠 2321年和 2322 上的通訊端接聽。 不要關閉此命令視窗;因此，不要關閉此命令視窗。您將需要保持此模擬器運行。
   ...
   從目錄中`azure-iot-sdk-c`運行以下命令以啟動模擬器：

   ```bash
   ./provisioning_client/deps/utpm/tools/tpm_simulator/Simulator.exe
   ```

1. 使用 Visual Studio，打開在名為`cmake`的`azure_iot_sdks.sln`目錄中生成的解決方案，並使用 **"生成"** 功能表上的**生成解決方案**命令生成它。

1. 在 Visual Studio 的 [方案總管]**** 窗格中，瀏覽至 **Provision\_Tools** 資料夾。 以滑鼠右鍵按一下 **tpm_device_provision** 專案，然後選取 [設為起始專案]****。

1. 使用 **"調試"** 功能表上的"**開始"** 命令之一運行解決方案。 輸出視窗顯示 TPM 模擬器的**註冊 ID**和**背書金鑰**，稍後在設備創建個人註冊時應複製該金鑰，您可以在其中關閉此視窗（使用註冊 ID 和背書金鑰），但保持 TPM 模擬器視窗運行。

## <a name="retrieve-provisioning-information-from-a-physical-device"></a>從物理設備檢索預配資訊

在設備上，生成可用於檢索設備的預配資訊的工具。

1. 按照[設置 Linux 開發環境](https://github.com/Azure/azure-iot-sdk-c/blob/master/doc/devbox_setup.md#linux)中的步驟安裝和構建用於 C 的 Azure IoT 設備 SDK。

1. 運行以下命令以構建 SDK 工具，該工具從 TPM 設備檢索設備預配資訊。
...
1. 複製**註冊 ID**和**背書金鑰**的值。 您可以使用這些值為 DPS 中的裝置建立個別的註冊。
