---
title: 代理和集線器模組孿生的屬性 - Azure IoT 邊緣
...
| runtime.settings.registryCredentials<br>.{registryId}.username | 容器登錄的使用者名稱。 Azure Container Registry 的使用者名稱通常是登錄名稱。<br><br> 註冊表憑據是任何專用模組映射所必需的。 | 否 |
...
當前所需屬性的副本可用於判斷設備是否應用了最新部署或仍在運行以前的部署清單。
...
| clients.{device or moduleId}.lastDisconnectTime | 上次設備或模組斷開連接時。 |
