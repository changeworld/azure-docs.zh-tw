...
IoT Edge 代理程式和 IoT Edge 中樞是構成 IoT Edge 執行階段的兩個模組。 如需每個執行時間模組職責的詳細資訊，請參閱 [瞭解 Azure IoT Edge 執行時間及其架構](iot-edge-runtime.md)。

本文提供執行階段模組對應項的所需屬性和報告屬性。 如需如何在 IoT Edge 裝置上部署模組的詳細資訊，請參閱 [瞭解如何在 IoT Edge 中部署模組及建立路由](module-composition.md)。

模組對應項包含：

* **所需屬性**。 解決方案後端可以設定所需的屬性，而模組可以讀取它們。 模組也可以接收所需屬性變更的通知。 所需的屬性會搭配報告屬性使用，以便同步處理模組的設定或條件。

* **報告屬性**。 模組可以設定報告屬性，而解決方案後端可以讀取和查詢它們。 報告屬性會搭配所需屬性使用，以便同步處理模組設定或條件。
...
| runtime.settings.registryCredentials<br>.{registryId}.username | 容器登錄的使用者名稱。 Azure Container Registry 的使用者名稱通常是登錄名稱。<br><br> 任何私用模組映射都需要登錄認證。 | 否 |
...
目前所需屬性的複本很適合用來分辨裝置是否已套用最新的部署，或仍在執行先前的部署資訊清單。
...
| lastDesiredStatus.code | 此狀態碼是指 IoT Edge 代理程式看到的最後預期屬性。 允許的值：`200` 成功、`400` 無效的設定、`412` 無效的結構描述版本、`417` 預期屬性是空的、`500` 失敗 |
...
| lastDesiredStatus.code | 狀態碼會參考 IoT Edge 中樞看到的最後預期屬性。 允許的值：`200` 成功、`400` 無效的設定、`500` 失敗 |
| lastDesiredStatus.description | 狀態的文字描述。 |
