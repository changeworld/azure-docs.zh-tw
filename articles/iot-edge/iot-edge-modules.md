...
Azure IoT Edge 允許您以*模組*的形式在邊緣部署和管理業務邏輯。 Azure IoT Edge 模組是 IoT Edge 管理的最小計算單位，可以包含 Azure 服務 (例如 Azure 串流分析) 或您自己的解決方案特定程式碼。 要瞭解如何開發、部署和維護模組，請考慮模組的四個概念元素：
...
當 IoT Edge 運行時創建新的模組實例時，它將獲得相應的模組標識。 模組標識存儲在 IoT 中心中，並用作該模組實例的所有本地和雲通信的定址和安全範圍。
...
Azure IoT 邊緣模組在與 IoT 中心同步至少一次後，可以無限期離線運行。 IoT 邊緣設備還可以將此離線功能擴展到其他 IoT 設備。 如需詳細資訊，請參閱[了解適用於 IoT Edge 裝置、模組及子裝置的擴充離線功能](offline-capabilities.md)。

## <a name="next-steps"></a>後續步驟

* [瞭解開發 IoT 邊緣模組的要求和工具](module-development.md)
