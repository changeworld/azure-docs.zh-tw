...
如果您要建立多層式部署，您可能會在其他部署中設定以相同裝置為目標的模組。 若要更新模組對應項而不覆寫其他版本，請開啟 [模組對應項**設定**] 索引標籤。在模組對應項的所需屬性中，為子區段建立新`properties.desired.settings`的模組對應項**屬性**，例如。 如果您只在`properties.desired`欄位中定義屬性，它會覆寫任何較低優先順序部署中所定義之模組的所需屬性。
