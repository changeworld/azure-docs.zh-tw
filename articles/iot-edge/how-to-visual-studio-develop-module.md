...
因為本文使用 Visual Studio 2019 作為主要開發工具，所以請安裝 Visual Studio。 請確定您在 Visual Studio 2019 安裝中包含 **使用 c + +** 工作負載進行 **Azure 開發** 和桌面開發。 您可以 [修改 Visual Studio 2019](/visualstudio/install/modify-visual-studio?view=vs-2019&preserve-view=true) 來新增所需的工作負載。
...
- [Azure Container Registry](../container-registry/index.yml) 或 [Docker 中樞](https://docs.docker.com/docker-hub/repos/#viewing-repository-tags)。
...
Visual Studio 中的 Azure IoT Edge 專案範本可建立可以在 Azure IoT 中樞中部署到 Azure IoT Edge 裝置的專案。 首先，您會建立 Azure IoT Edge 解決方案，然後在該解決方案中產生第一個模組。 每個 IoT Edge 解決方案都可以包含多個模組。
...
1. 按一下 [重新整理] 按鈕，以查看與 **SimulatedTemperatureSensor** 模組搭配運作的新模組，以及 **$edgeAgent** 和 **$edgeHub**。
