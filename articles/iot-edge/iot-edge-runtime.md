...
description: 瞭解 IoT Edge 執行時如何管理裝置上的模組、安全性、通訊和報告
author: kgremban
manager: philmea
ms.author: kgremban
ms.date: 11/01/2019
...
IoT Edge 執行階段的責任分為兩類：通訊和模組管理。 這兩個角色由作為 IoT Edge 運行時一部分的兩個元件執行。*IoT Edge 中心*負責通訊,而*IoT Edge代理*部署和監視模組。

IoT Edge 中樞和 IoT Edge 代理程式都是模組，就像 IoT Edge 裝置上執行的任何其他模組。 它們有時稱為*執行時模組*。
...
IoT 邊緣中心不是本地運行的IoT中心的完整版本。 IoT Edge 中心將一些任務悄悄地委派給IoT中心。 例如，當裝置第一次嘗試連線時，IoT Edge 中樞會將驗證要求轉送到 IoT 中樞。 建立第一個連線之後，IoT Edge 中樞就會在本機快取安全性資訊。 允許將來從該設備連接,而無需再次對雲進行身份驗證。

為了減少 IoT Edge 解決方案使用的頻寬,IoT Edge 中心優化了與雲端的實際連接數。 IoT Edge 中心從模組或下游設備獲取邏輯連接,並合併這些連接以進行與雲端的單個實體連接。 此程序的詳細資料對解決方案的其餘部分而言是透明的。 用戶端認為它們有自己的連線可連至雲端，即使它們全部都會透過相同連線來傳送。

![IoT Edge 中樞是實體裝置與 IoT 中樞之間的閘道](./media/iot-edge-runtime/Gateway.png)

IoT Edge 中樞可以判斷是否已連線到 IoT 中樞。 如果連線中斷，IoT Edge 中樞就會在本機儲存訊息或對應項更新。 一旦連線重新建立之後，就會將所有資料同步。 用於此臨時緩存的位置由 IoT Edge 中心模組孿生的屬性確定。 快取的大小不會受限，而且只要裝置還有儲存體容量就會持續成長。有關詳細資訊,請參閱[離線功能](offline-capabilities.md)。

### <a name="module-communication"></a>模組通訊

IoT Edge 中樞可促進模組對模組的通訊。 使用 IoT Edge 中樞作為訊息代理程式，讓模組彼此保持獨立。 模組只需指定它們要在其中接受訊息的輸入，以及要將訊息寫入其中的輸出。 解決方案開發人員可以將這些輸入和輸出拼接在一起,以便模組按該解決方案的特定順序處理數據。

![IoT Edge 中樞可促進模組對模組的通訊](./media/iot-edge-runtime/module-endpoints.png)

為了將資料傳送至 IoT Edge 中樞，模組會呼叫 SendEventAsync 方法。 第一個引數會指定在哪個輸出上傳送訊息。 以下偽程式碼在**輸出1**上傳送訊息:
...
部署清單中的每個項都包含有關模組的特定資訊,IoT Edge 代理用於控制模組的生命週期。 以下提供一些更令人感興趣的屬性：
...
* **設定.createOptions** = 在啟動模組的容器時直接傳遞到 Moby 容器守護行程的字串。 在此屬性中添加選項允許進階設定,如埠轉寄或將捲安裝到模組的容器中。  
...
* **restartPolicy**：IoT Edge 代理程式重新啟動模組的方式。 可能的值包括：
  ...
  * `never`• IoT 邊緣代理永遠不會重新啟動模組。
  * `on-failure`- 如果模組崩潰,IoT 邊緣代理將重新啟動它。 如果模組乾淨地關閉,IoT Edge 代理不會重新啟動它。
  * `on-unhealthy`- 如果模組崩潰或被視為不正常,IoT Edge 代理將重新啟動它。
  * `always`- 如果模組崩潰、被視為不正常或以任何方式關閉,IoT Edge 代理將重新啟動它。

* **圖像拉取策略**- IoT 邊緣代理是否嘗試自動提取模組的最新映射。 如果不指定值,則預設值為 *"創建*"。 可能的值包括：

  * `on-create`- 啟動模組或更新基於新部署清單的模組時,IoT Edge 代理將嘗試從容器註冊錶中提取模組映射。
  * `never`- IoT 邊緣代理永遠不會嘗試從容器註冊表中提取模組映射。 使用此設定,您負責將模組映射連接到設備並管理任何映射更新。
...
IoT Edge 代理程式在 IoT Edge 裝置的安全性中扮演了關鍵角色。 例如,它在啟動模組映射之前執行驗證模組映射等操作。
