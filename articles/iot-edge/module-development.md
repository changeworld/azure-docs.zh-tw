...
撰寫模組時，您可以使用 [Azure IoT 裝置 SDK](../iot-hub/iot-hub-devguide-sdks.md) 連線至 IoT Edge 中樞，並使用上述功能，如同使用 IoT 中樞搭配裝置應用程式時所做的一樣。 IoT Edge 模組和 IoT 裝置應用程式之間的唯一差異在於，您必須參考模組身分識別，而不是裝置身分識別。
...
IoT Edge 模組（而不是一般的 IoT 中樞裝置應用程式）可以接收由本機 IoT Edge 中樞進行 proxy 處理的裝置到雲端訊息。
