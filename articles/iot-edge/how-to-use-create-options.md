...
如果您使用適用于 Visual Studio 或 Visual Studio Code 的 Azure IoT Tools 擴充功能，您可以在**deployment. template. json**檔案中撰寫 json 格式的 create 選項。 然後，當您使用此擴充功能來建立 IoT Edge 解決方案或產生部署資訊清單時，它會以 IoT Edge 執行時間所預期的格式為您 json.stringify JSON。 例如：

```json
"createOptions": "{\"HostConfig\":{\"PortBindings\":{\"5671/tcp\":[{\"HostPort\":\"5671\"}],\"8883/tcp\":[{\"HostPort\":\"8883\"}],\"443/tcp\":[{\"HostPort\":\"443\"}]}}}"
```

撰寫建立選項的秘訣之一是使用`docker inspect`命令。 在您的開發過程中，請使用`docker run <container name>`在本機執行模組。 當模組以您想要的方式運作時，請執行`docker inspect <container name>`。 此命令會以 JSON 格式輸出模組詳細資料。 尋找您設定的參數，並複製 JSON。 例如：
...
然後，使用[Docker 容器 [建立選項](https://docs.docker.com/engine/api/v1.32/#operation/ContainerCreate)] 的 [ **HostConfig** ] 群組中的 [**即可**] 設定，將模組中已公開的埠對應至主機裝置上的埠。 例如，如果您在模組內公開端口8080，而且想要將其對應至主機裝置的埠80，則範本. json 檔案中的 create 選項看起來會像下列範例：
