...
---
# <a name="caching-with-azure-front-door"></a>使用 Azure 前門進行緩存
下列文件會利用已啟用快取的路由規則來指定 Front Door 的行為。 前門是一個現代的內容交付網路 （CDN），因此，隨著動態網站加速和負載平衡，它還支援緩存行為，就像任何其他 CDN 一樣。

## <a name="delivery-of-large-files"></a>傳遞大型檔案
Azure 前門提供大檔，檔案大小上沒有上限。 Front Door 會使用稱為物件區塊化的技術。 要求大型檔案時，Front Door 會從後端擷取較小的檔案片段。 接收到完整或位元組範圍檔案要求之後，Front Door 環境就會向後端要求檔案 (以 8 MB 的區塊為單位)。
...
- [布羅特利](https://en.wikipedia.org/wiki/Brotli)
...
1. **單路徑清除**：通過指定具有檔副檔名的資產的完整路徑（沒有協定和域）清除單個資產，例如/圖片/斯特拉斯堡.png;
2. **萬用字元清除**︰星號 (\*) 可作為萬用字元。 清除具有 /\*的終結點下的所有資料夾、子資料夾和檔，或通過指定資料夾後跟 /（\*例如/圖片/）\*來清除特定資料夾下的所有子資料夾和檔。
...
緩存控制回應標頭指示不會緩存回應，例如緩存控制：專用、緩存控制：無緩存和緩存控制：不受存儲。 不過，如果 POP 中有多個針對相同 URL 的要求，它們可能共用回應。 如果不存在緩存控制，則預設行為是 AFD 將緩存資源的 X 時間，其中 X 在 1 到 3 天之間隨機選取。
