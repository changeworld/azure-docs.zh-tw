...
這些服務被分類為兩個類別：
1. **全域負載平衡服務**（例如流量管理員和 Front Door）會在跨雲端或甚至是混合式內部部署服務的區域後端中，散發您的終端使用者流量。 全域負載平衡會將您的流量路由傳送到距離您最接近的服務後端，並對服務可靠性或效能的變更做出反應，以為您的使用者維持永遠可用的最大化效能。 
...
| --------------- | ------------------------ |
|**任何通訊協定**：由於流量管理員是在 DNS 層運作，因此您可以路由傳送任何類型的網路流量 (HTTP、TCP、UDP 等)。 | **HTTP 加速：** 使用 Front Door 後流量會在 Microsoft 網路的邊緣進行 proxy 處理。  因此，HTTP （S）要求會看到延遲和輸送量的改善，以減少 TLS 協調的延遲，並使用從 AFD 到您應用程式的熱連接。|
...
我們建議所有網站、API 與服務皆應進行異地備援處理，並從距離它們最接近 (延遲最低) 的位置將流量傳遞給其使用者。  結合來自流量管理員、Front Door、應用程式閘道和 Load Balancer 的服務，可讓您建立地理位置和 zonally 冗余，以最大化可靠性、規模及效能。
