...
ms.locfileid: "89399458"
---
# <a name="load-balancing-with-azures-application-delivery-suite"></a>使用 Azure 的應用程式傳遞套件進行負載平衡

## <a name="introduction"></a>簡介
Microsoft Azure 提供多個全域和區域服務來管理網路流量的分散和負載平衡：流量管理員、Front Door、應用程式閘道和 Load Balancer。  除了 Azure 的眾多區域 (region) 與區域 (zonal) 架構之外，搭配使用這些服務也能讓您建置強健且可調整的高效能應用程式。

![應用程式傳遞套件 ][1]
 
這些服務被分類為兩個類別：
1. **全域負載平衡服務** （例如流量管理員和 Front Door）會將來自您的終端使用者的流量分散至整個區域後端、跨雲端或甚至混合式內部部署服務。 全域負載平衡會將您的流量路由傳送到距離您最接近的服務後端，並對服務可靠性或效能的變更做出反應，以為您的使用者維持永遠可用的最大化效能。 
2. **區域負載平衡服務** （例如 Standard Load Balancer 或應用程式閘道）可讓 (您將虛擬網路內的流量分散到虛擬網路中， (vnet) 區域內的 vm) 或區域性服務端點。

在您的應用程式中結合全域和區域服務，可提供端對端可靠、高效能且安全的方式，將流量路由傳送至您的 IaaS、PaaS 或內部部署服務，以及將流量路由傳送到您的使用者。 在下一節中，我們會逐一描述這些服務。
...
| --------------- | ------------------------ |
|**任何通訊協定**：由於流量管理員是在 DNS 層運作，因此您可以路由傳送任何類型的網路流量 (HTTP、TCP、UDP 等)。 | **HTTP 加速：** 使用 Front Door 流量是在 Microsoft 網路的邊緣進行 proxy 處理。  基於這個原因，HTTP (S) 要求看到延遲和輸送量改進，可減少 TLS 協商的延遲，以及使用從 AFD 到您應用程式的熱連接。|
|**內部部署路由：** 使用 DNS 層的路由時，流量一律會從點到點。  從您的分公司路由傳送至內部部署資料中心可以採用直接路徑;甚至是在您自己的網路上使用流量管理員。 | **獨立的可調整姓**：由於 Front Door 能搭配 HTTP 要求運作，因此可以根據規則及每個應用程式微服務的健康情況，將針對不同 URL 路徑的要求路由傳送到不同的後端/區域服務集區 (微服務)。|
...
我們建議所有網站、API 與服務皆應進行異地備援處理，並從距離它們最接近 (延遲最低) 的位置將流量傳遞給其使用者。  結合流量管理員、Front Door、應用程式閘道和 Load Balancer 的服務，可讓您建立地理位置和 zonally 冗余，以將可靠性、規模和效能最大化。
