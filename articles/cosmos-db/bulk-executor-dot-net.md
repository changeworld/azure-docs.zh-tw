...
> [!NOTE]
> 這篇文章中所述的大量執行程式程式庫是針對使用 .NET SDK 2.x 版的應用程式進行維護。 針對新的應用程式，您可以使用[.NET SDK 版本](tutorial-sql-api-dotnet-bulk-import.md)3.x 直接提供的**大量支援**，而不需要任何外部程式庫。 

> 如果您目前正在使用大量執行程式程式庫，並打算遷移至較新 SDK 上的大量支援，請使用[遷移指南](how-to-migrate-from-bulk-executor-library.md)中的步驟來遷移您的應用程式。

本教學課程說明如何使用大量執行程式 .NET 程式庫將文件匯入並更新至 Azure Cosmos 容器。 若要深入瞭解大量執行程式程式庫，以及它如何協助您利用大量的輸送量和儲存體，請參閱[大量執行程式程式庫總覽](bulk-executor-overview.md)一文。 在本教學課程中，您會看到將隨機產生的檔大量匯入至 Azure Cosmos 容器的範例 .NET 應用程式。 匯入之後，應用程式會說明如何將修補程式指定為可在特定文件欄位上執行的作業，來大量更新匯入的資料。
...
複製的存放庫包含兩個範例 "BulkImportSample" 和 "BulkUpdateSample"。 您可以開啟其中一個範例應用程式，使用您 Azure Cosmos DB 帳戶的連接字串更新 App.config 檔案中的連接字串、建立解決方案，然後加以執行。
...
2. Azure Cosmos DB 的連接字串會從 App.config 檔案抓取，如下列程式碼所示：  
   ...
   大量匯入工具會使用資料庫名稱、容器名稱和在 App.config 檔案中指定的輸送量值來建立新的資料庫和容器。
...
2. 定義更新專案以及對應的欄位更新作業。 在此範例中，您將使用 `SetUpdateOperation` 來更新 `Name` 欄位，並 `UnsetUpdateOperation` `Description` 從所有檔中移除欄位。 您也可以執行其他作業，例如透過指定值來遞增文件欄位、將特定值推送至陣列欄位中，或是從陣列欄位中移除特定值。 若要了解大量更新 API 提供的不同方法，請參閱 [API 文件](https://docs.microsoft.com/dotnet/api/microsoft.azure.cosmosdb.bulkexecutor.bulkupdate?view=azure-dotnet)。
...
* 建議您在 `BulkExecutor` 對應至特定 Azure Cosmos 容器的單一虛擬機器中，為整個應用程式具現化單一物件。  
