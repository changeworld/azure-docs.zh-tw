...
複製的存放庫包含兩個範例 "BulkImportSample" 和 "BulkUpdateSample"。 您可以開啟其中一個範例應用程式，使用 Azure Cosmos DB 帳戶的連接字串更新 App.config 檔案中的連接字串，建立方案並加以執行。
...
2. 會從 App.config 檔案抓取 Azure Cosmos DB 的連接字串，如下列程式碼所示：  
...
2. 定義更新專案以及對應的欄位更新作業。 在此範例中，您將`SetUpdateOperation`使用來更新`Name`欄位， `UnsetUpdateOperation`並從所有`Description`檔中移除欄位。 您也可以執行其他作業，例如透過指定值來遞增文件欄位、將特定值推送至陣列欄位中，或是從陣列欄位中移除特定值。 若要了解大量更新 API 提供的不同方法，請參閱 [API 文件](https://docs.microsoft.com/dotnet/api/microsoft.azure.cosmosdb.bulkexecutor.bulkupdate?view=azure-dotnet)。
