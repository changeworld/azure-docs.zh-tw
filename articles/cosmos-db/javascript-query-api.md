---
title: 在 Azure Cosmos DB 預存程序和觸發器中使用 JavaScript 集成查詢 API
...
ms.locfileid: "76901830"
---
# <a name="javascript-query-api-in-azure-cosmos-db"></a>Azure Cosmos DB 中的 JavaScript 查詢 API

除了在 Azure Cosmos DB 中使用 SQL API 發出查詢外[，Cosmos DB 伺服器端 SDK](https://azure.github.io/azure-cosmosdb-js-server/)還提供 JavaScript 介面，用於在 Cosmos DB 預存程序和觸發器中執行優化的查詢。 不必一定要懂得 SQL 語言才能使用此 JavaScript 介面。 JavaScript 的查詢 API 使用 ECMAScript5 陣列內建和受歡迎的 JavaScript 程式庫如 lodash 所熟悉的語法，將述詞函式依序傳遞至函式呼叫，藉此以程式設計方式建立查詢。 JavaScript 執行階段會剖析查詢，並使用 Azure Cosmos DB 的索引，有效執行該查詢。
...
- [使用 Azure Cosmos DB 預存程序、觸發器和使用者定義的函數](stored-procedures-triggers-udfs.md)
