---
title: 優化成本和 RU/s 以在 Azure Cosmos DB 中執行查詢
...
您還可以使用 SDK 以程式設計方式取得查詢的成本。 若要測量任何作業 (如建立、更新或刪除) 的額外負荷，請在使用 REST API 時檢查 `x-ms-request-charge` 標頭。 如果使用 .NET 或 JAVA SDK，`RequestCharge`則該屬性是獲取請求費用的等效屬性，此屬性存在於資源回應或 FeedResponse 中。
...
查詢的要求單位取決於許多因素。 例如，載入/返回的 Azure Cosmos 項數、針對索引的查找數、查詢編譯時間等詳細資訊。 Azure Cosmos DB 保證在相同資料上執行時，相同的查詢將一律使用相同數量的要求單位，即使重複執行也是如此。 使用查詢執行計量的查詢設定檔，可讓您清楚了解要求單位的使用情況。  
...
由查詢、使用者定義函數 (UDF) 耗用的效能和輸送量主要取決於函式主體。 找出 UDF 中查詢執行花費的時間以及消耗的 RU 數目，最簡單的方式是啟用查詢計量。 如果使用 .NET SDK，下面是 SDK 返回的依例查詢指標：
...
* 瞭解有關[Azure 宇宙 DB 保留容量](cosmos-db-reserved-capacity.md)的更多
