...
`CosmosClientOptions.RequestTimeout` (或 `ConnectionPolicy.RequestTimeout` For SDK V2) 設定可讓您設定會影響每個個別網路要求的時間。  使用者啟動的作業可以跨越多個網路要求 (例如，可能會進行節流) ，而這項設定會在重試時套用到每個網路要求。 這不是端對端作業要求超時。
...
為了達到最佳延遲，建議 CPU 使用量大約為40%。 建議使用10秒做為間隔，以監視平均) CPU 使用率的最大 (。 跨分割區查詢，CPU 尖峰較常見，它可能會對單一查詢執行多個連接。
...
如果您是在 Azure Vm 上執行，請遵循[SNAT 埠耗盡指南](troubleshoot-dot-net-sdk.md#snat)。
...
Azure Cosmos DB 會將整體布建的輸送量平均分散到實體分割區。 當有熱資料分割時，實體分割區上的一或多個邏輯分割區索引鍵會耗用所有實體分割區的 RU/秒，而其他實體磁碟分割上的 RU/秒則不會使用。 就徵兆而言，所耗用的 RU/秒總數會小於資料庫或容器上的整體布建 RU/秒，但您仍會在對最忙碌邏輯分割區索引鍵的要求上看到節流 (429s) 。 使用 [[正規化 RU 耗用量](monitor-normalized-request-units.md)] 計量，查看工作負載是否遇到熱資料分割。 
