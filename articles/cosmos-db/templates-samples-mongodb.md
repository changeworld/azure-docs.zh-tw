...
本文僅提供適用於 MongoDB 的 Azure Cosmos DB API 範例，若要尋找其他 API 類型帳戶的範例，請參閱：搭配使用 Azure Resource Manager 範本與適用於 [Cassandra](templates-samples-cassandra.md)、[Gremlin](templates-samples-gremlin.md)、[SQL](templates-samples-sql.md)、[資料表](templates-samples-table.md)的 Azure Cosmos DB API 文章。
