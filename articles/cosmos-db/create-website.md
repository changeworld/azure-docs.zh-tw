...
4. 按一下 [編輯參數]****，提供每個必要參數的值，然後按一下 [確定]****。  參數如下所示：
   ...
   1. SITENAME：指定 App Service web 應用程式名稱，並用來建立用來存取 web 應用程式的 URL （例如，如果您指定 "mydemodocdbwebsite"，則您用來存取 web 應用程式的 URL 為`mydemodocdbwebapp.azurewebsites.net`）。
...
8. 若要使用應用程式，請流覽至 web 應用程式 URL （在上述範例中，URL 會`http://mydemodocdbwebapp.azurewebsites.net`是）。  您會看到下列的 Web 應用程式：
