...
| **requestResourceId** | **Id** | 關於要求的 resourceId。 視執行的作業而定，這個值可能會指向 `databaseRid` 、 `collectionRid` 或 `documentRid` 。|
...
|度量 (計量顯示名稱) |單位 (匯總類型)  |說明|維度| 時間資料粒度| 舊版計量對應 | 使用量 |
|---|---|---|---| ---| ---| ---|
...
|度量 (計量顯示名稱) |單位 (匯總類型) |說明|維度| 時間資料粒度| 使用量 |
|---|---|---|---| ---| ---|
...
|度量 (計量顯示名稱)  |單位 (匯總類型) |說明| 時間資料粒度| 舊版計量對應 | 使用量 |
|---|---|---|---| ---| ---|
