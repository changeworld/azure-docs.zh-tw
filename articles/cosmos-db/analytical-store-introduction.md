...
考慮事項：
*   流量分析 TTL 值啟用分析存放區之後，稍後可以將其更新為不同的有效值 
*   雖然交易 TTL 可以在容器或專案層級設定，但分析 TTL 只能在容器層級設定
*   您可以在容器層級設定分析 TTL >= 交易式 TTL，以在分析存放區中達到較長的運算元據保留期
*   藉由設定分析 TTL = 交易式 TTL，可以建立分析存放區以鏡像交易式存放區

當您在容器上啟用 anaytical 存放區時：
 * 使用 Azure 入口網站時，分析 TTL 會設定為預設值-1。 您可以藉由流覽至 [資料總管] 下的 [容器設定]，將此值變更為 [n] 秒。 
 
 * 您可以使用 Azure SDK 或 Powershell 或 CLI 來啟用分析 TTL，方法是將它設定為-1 或 ' n '。 
