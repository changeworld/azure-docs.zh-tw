...
Azure Cosmos DB 是 Microsoft 的全域分散式多模型資料庫服務。 當多模型意味著 Azure Cosmos DB 支援多個 API 和多個資料模型時，不同的 API 使用不同的資料格式進行存儲和有線協定。 例如，SQL使用JSON，蒙哥DB使用BSON，表使用EDM，卡珊多拉使用CQL，格雷姆林使用圖形SON。 因此，我們建議對給定帳戶中的資料的所有訪問使用相同的 API。
