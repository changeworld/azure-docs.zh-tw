...
description: 瞭解如何建立無伺服器 SQL 集區資料庫，並透過 Synapse 連結來進行 Azure Cosmos DB、查詢 Azure Cosmos DB 容器，然後使用這些視圖的 Power BI 建立模型。
...
* 將產品資料載入至 Azure Cosmos 容器，如此 [批次資料](https://github.com/Azure-Samples/Synapse/blob/main/Notebooks/PySpark/Synapse%20Link%20for%20Cosmos%20DB%20samples/Retail/spark-notebooks/pyspark/1CosmoDBSynapseSparkBatchIngestion.ipynb) 內嵌筆記本中所述。
...
下列腳本示範如何在每個容器上建立 views。 為了簡單起見，讓我們在啟用 Synapse 連結的容器上使用無伺服器 SQL 集區的 [自動架構推斷](analytical-store-introduction.md#analytical-schema) 功能：
