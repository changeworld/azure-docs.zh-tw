...
如果容器的大小預計存儲超過 20 GB，或者您希望每秒分配超過 10，000 個請求單位 （RUs），**則需要分區**。 [Azure Cosmos DB 分區機制](partition-data.md)中的相同一般原則適用于下面介紹的一些特定于圖形的優化。

![圖形分區。](./media/graph-partitioning/graph-partitioning.png)

## <a name="graph-partitioning-mechanism"></a>圖形分區機制

以下指南描述了 Azure Cosmos DB 中的分區策略如何操作：

- **頂點和邊緣會儲存為 JSON 文件**。

- **端點需要資料分割索引鍵**。 此索引鍵會決定頂點將透過雜湊演算法儲存在哪個資料分割中。 分區鍵屬性名稱在創建新容器時定義，並且其格式為： `/partitioning-key-name`。

- **邊緣將會連同其來源頂點一起儲存**。 換句話說，對每個頂點而言，其分割區金鑰將會定義其本身及其外傳邊緣的儲存位置。 在圖形查詢中使用基數時，`out()`進行此優化是為了避免跨分區查詢。

- **邊包含指向 的頂點的引用**。 所有邊都存儲著它們指向的頂點的分區鍵和指示的 I。 此計算使所有`out()`方向查詢始終為作用域分區查詢，而不是盲跨分區查詢。 
...
- **圖形查詢必須指定資料分割索引鍵**。 若要在 Azure Cosmos DB 中充分利用水平資料分割，則應在選取單一端點時盡可能指定資料分割索引鍵。 以下是在資料分割的圖表中選取一或多個頂點的查詢：
    ...
    - 選擇一組帶有其 ID 的頂點，並**指定分區鍵值的清單**： 
    
        ```java
        g.V('vertex_id0', 'vertex_id1', 'vertex_id2', …).has('partitionKey', within('partitionKey_value0', 'partitionKey_value01', 'partitionKey_value02', …)
        ```

    - 在查詢開始時使用**分區策略**，並為 Gremlin 查詢的其餘部分的範圍指定分區： 
...
- **在查詢頂點時務必要指定資料分割索引鍵值**。 從已知資料分割取得頂點，是可達到效能的方法。 所有後續鄰接操作將始終限定為分區，因為"邊緣"包含其目標頂點的參考 ID 和分區鍵。

- **查詢邊緣時應盡可能使用外傳方向**。 如前所述，邊緣會連同其外傳方向的來源頂點一起儲存。 依此模式為原則設計資料和查詢時，將可盡量降低進行跨資料分割查詢的可能性。 相反，`in()`查詢將始終是一個昂貴的扇出查詢。

- **選擇一個分區鍵，它將在分區之間均勻分佈資料**。 此決策高度仰賴解決方案的資料模型。 請閱讀 [Azure Cosmos DB 中的資料分割和調整](partition-data.md)，以深入了解如何建立適當的資料分割索引鍵。
