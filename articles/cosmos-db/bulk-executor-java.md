---
title: 在 Azure Cosmos DB 中使用批量執行器 JAVA 庫執行大量匯入和更新操作
description: 使用批量執行器 JAVA 庫大量匯入和更新 Azure Cosmos DB 文檔
author: tknandu
...
本教學課程說明如何使用 Azure Cosmos DB 大量執行程式的 Java 程式庫來匯入和更新 Azure Cosmos DB 文件。 若要深入了解大量執行程式程式庫，以及它如何協助您利用大量輸送量與儲存體，請參閱[大量執行程式程式庫概觀](bulk-executor-overview.md)一文。 在本教程中，您將構建一個 JAVA 應用程式，該應用程式生成隨機文檔，並將它們大量匯入到 Azure Cosmos 容器中。 匯入之後，您會大量更新文件的某些屬性。 
...
4. 調用生成隨機文檔以大量匯入 Azure Cosmos 容器的 importAll API。 您可以設定 CmdLineConfiguration.java 檔案內的命令列組態。
   ...
   **大量匯入回應物件定義**大量匯入 API 呼叫的結果包含下列 get 方法：
   ...
   |列出\<異常>獲取錯誤（） |  如果提供給大量匯入 API 呼叫的批次中有部分文件無法插入，則會取得錯誤清單。       |
   |清單\<物件>獲取錯誤輸入文檔（）  |    格式錯誤而未成功匯入大量匯入 API 呼叫的文件清單。 使用者應該修正傳回的文件，然後再次嘗試匯入。 格式錯誤的文件包含其識別碼值不是字串 (Null 或任何其他視為無效的資料類型) 的文件。     |
...
2. 調用更新所有生成隨機文檔的 API，然後大量匯入到 Azure Cosmos 容器中。 您可以設定要傳遞到 CmdLineConfiguration.java 檔案內的命令列組態。
   ...
   **大量匯入回應物件定義**大量匯入 API 呼叫的結果包含下列 get 方法：
   ...
   |列出\<異常>獲取錯誤（）   |    如果提供給大量更新 API 呼叫的批次中有部分文件無法插入，則會取得錯誤清單。      |
...
* 建議在對應于特定 Azure Cosmos 容器的單個虛擬機器中具現化整個應用程式的單個 DocumentBulk 執行器物件。  
