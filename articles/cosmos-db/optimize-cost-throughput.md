...
一開始，您可以使用最小的輸送量 400 RU/秒，之後再擴大為每秒數千萬個要求以上。 您針對 Azure Cosmos 容器或資料庫所發出的每個要求 (例如，讀取要求、寫入要求、查詢要求)，預存程序都會有對應的成本，並且會從您所佈建的輸送量扣除。 如果您佈建 400 RU/秒，並發出成本為 40 RU 的查詢，則每秒能夠發出 10 個這樣的查詢。 超出該輸送量的要求，其速率會受限，這時您就應該重試要求。 如果您使用用戶端驅動程式，其可支援自動重試邏輯。
...
4. 您可以視需要彈性地擴大和減少所佈建的輸送量，以因應工作負載需求。 
