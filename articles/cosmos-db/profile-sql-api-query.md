...
ms.locfileid: "85262543"
---
# <a name="get-sql-query-execution-metrics-and-analyze-query-performance-using-net-sdk"></a>使用 .NET SDK 取得 SQL 查詢執行計量和分析查詢效能

本文說明如何在 Azure Cosmos DB 上分析 SQL 查詢效能。 您可以使用從 .NET SDK 抓取的來完成這 `QueryMetrics` 種分析，詳細資訊請參閱這裡。 [QueryMetrics](https://msdn.microsoft.com/library/microsoft.azure.documents.querymetrics.aspx)是具有後端查詢執行相關資訊的強型別物件。 這些計量在[微調查詢效能](https://docs.microsoft.com/azure/cosmos-db/documentdb-sql-query-metrics)一文中有更詳細的記錄。
...
在上一節中，請注意，有多個[ExecuteNextAsync](https://msdn.microsoft.com/library/azure/dn850294.aspx)方法呼叫。 每個呼叫都會傳回一個 `FeedResponse` 具有字典的物件， `QueryMetrics` 一個用於查詢的每個接續。 下列範例顯示如何使用 LINQ 匯總這些 `QueryMetrics` ：
...
您可以 `QueryMetrics` 依據分割區識別碼來分組。 依據分割區識別碼分組可讓您查看特定分割區是否會造成效能問題（相較于其他分割區）。 下列範例顯示如何 `QueryMetrics` 使用 LINQ 進行分組：
...
您也可以 `FeedResponse` 使用方法，從 LINQ 查詢取得 `AsDocumentQuery()` ：
...
您可以捕捉每個查詢所耗用的要求單位，以調查耗用高輸送量的昂貴查詢或查詢。 您可以使用中的[RequestCharge](https://msdn.microsoft.com/library/azure/dn948712.aspx)屬性來取得要求費用 `FeedResponse` 。 若要深入瞭解如何使用 Azure 入口網站和不同的 Sdk 取得要求費用，請參閱[尋找要求單位費用](find-request-unit-charge.md)一文。
...
計算執行用戶端查詢所需的時間時，請確定您只包含呼叫方法的時間， `ExecuteNextAsync` 而不是程式碼基底的其他部分。 只有這些呼叫可協助您計算查詢執行所花的時間，如下列範例所示：
...
若要優化此範例查詢，請避免在篩選準則中使用上限。 相反地，當建立或更新檔時，這些 `c.description` 值必須全部以大寫字元插入。 查詢就會變成： 
...
## <a name="references"></a><a id="References"></a>參考資料
