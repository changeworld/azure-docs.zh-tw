...
您可以使用 Cosmos DB Spark 連接器使用存儲在 Azure Cosmos DB 中的資料運行[Spark](https://spark.apache.org/)作業。 Cosmos 可用於批次處理和流處理，並作為低延遲訪問的服務層。

您可以將連接器與 Azure[資料塊](https://azure.microsoft.com/services/databricks)或[Azure HDInsight](https://azure.microsoft.com/services/hdinsight/)一起使用 ，後者在 Azure 上提供託管 Spark 群集。 下表顯示了支援的 Spark 版本。
...
> 對於宇宙 DB 卡珊多拉 API，請使用[卡珊多拉火花連接器](https://github.com/datastax/spark-cassandra-connector)。
>

## <a name="quickstart"></a>快速入門

* 按照 JAVA [SDK 入門](sql-api-async-java-get-started.md)的步驟設置 Cosmos DB 帳戶，並填充一些資料。
* 按照 Azure[資料磚塊入門](/azure/azure-databricks/quickstart-create-databricks-workspace-portal)的步驟來設置 Azure 資料塊工作區和群集。
* 您現在可以創建新的筆記本，並導入 Cosmos DB 連接器庫。 跳轉到[使用 Cosmos DB 連接器](#bk_working_with_connector)，詳細瞭解如何設置工作區。
* 以下部分包含有關如何使用連接器讀取和寫入的程式碼片段。

### <a name="batch-reads-from-cosmos-db"></a>從宇宙 DB 進行批次處理讀取

以下程式碼片段演示如何創建 Spark 資料框架，以便從 PySpark 中的 Cosmos DB 讀取。
...
### <a name="batch-writes-to-cosmos-db"></a>批次處理寫入 Cosmos DB

以下程式碼片段演示如何在 PySpark 中將資料框寫入 Cosmos DB。
...
### <a name="streaming-reads-from-cosmos-db"></a>從宇宙 DB 資料流讀取

以下程式碼片段演示如何連接到 Azure Cosmos DB 更改源並將其讀取。
...
### <a name="streaming-writes-to-cosmos-db"></a>資料流寫入 Cosmos DB
...
.start()
```
更多片段和端到端示例，請參閱[Jupyter](https://github.com/Azure/azure-cosmosdb-spark/tree/master/samples/notebooks)。

## <a name="working-with-the-connector"></a><a name="bk_working_with_connector"></a>使用連接器

您可以在 GitHub 中的源中構建連接器，或在下面的連結中從 Maven 下載優步罐。
...
按照 Azure 資料磚塊指南中的指南>[使用 Azure Cosmos DB Spark 連接器](https://docs.azuredatabricks.net/spark/latest/data-sources/azure/cosmosdb-connector.html)"中的指南"創建使用 Databricks 工作區的庫

> [!NOTE]
> 請注意，"**使用 Azure 宇宙 DB 火花連接器"** 頁當前不是最新的。 而不是將六個單獨的罐子下載到六個不同的庫中，你可以從 maven 下載https://search.maven.org/artifact/com.microsoft.azure/azure-cosmosdb-spark_2.4.0_2.11/1.4.0/jar)優步罐，並安裝這個 jar/庫。
> 

### <a name="using-spark-cli"></a>使用火花-cli

要使用火花-cli（即`spark-shell`，、、）`pyspark``spark-submit`使用連接器，可以將`--packages`參數與連接器的[maven 座標](https://mvnrepository.com/artifact/com.microsoft.azure/azure-cosmosdb-spark_2.4.0_2.11)一起使用。
...
如果您在 HDInsight 中使用 Jupyter 筆記本，則可以使用火花魔法`%%configure`儲存格來指定連接器的 maven 座標。
...
### <a name="build-the-connector"></a>構建連接器

目前，此連接器專案使用`maven`這種生成來生成，無需依賴項，您可以運行：
...
[Cosmos DB Spark GitHub 存儲庫](https://github.com/Azure/azure-cosmosdb-spark)具有以下示例筆記本和腳本，您可以嘗試這些筆記本和腳本。
...
* 使用`azure-cosmosdb-spark`**[將 Azure 資料塊連接到 Azure 宇宙資料庫](https://docs.databricks.com/spark/latest/data-sources/azure/cosmosdb-connector.html)**。  此處連結的還有即時[飛行性能筆記本](https://github.com/dennyglee/databricks/tree/master/notebooks/Users/denny%40databricks.com/azure-databricks)的 Azure 資料磚塊版本。
* **[使用 Azure Cosmos DB 和 HDInsight （Apache Spark）](https://github.com/Azure/azure-cosmosdb-spark/blob/master/samples/lambda/readme.md)** 的 Lambda 體系結構：您可以使用 Cosmos DB 和 Spark 減少維護大資料管道的操作開銷。
...
* [火花連接器配置](https://github.com/Azure/azure-cosmosdb-spark/wiki/Configuration-references)
* [火花到宇宙 DB 連接器設置](https://github.com/Azure/azure-documentdb-spark/wiki/Spark-to-Cosmos-DB-Connector-Setup)（正在進行中）
* [通過 Apache Spark （HDI） 將 Power BI 直接查詢配置為 Azure 宇宙資料庫](https://github.com/Azure/azure-cosmosdb-spark/wiki/Configuring-Power-BI-Direct-Query-to-Azure-Cosmos-DB-via-Apache-Spark-(HDI))

### <a name="troubleshooting"></a>疑難排解

* [使用宇宙資料庫聚合](https://github.com/Azure/azure-documentdb-spark/wiki/Troubleshooting:-Using-Cosmos-DB-Aggregates)
...
* [使用 Azure 宇宙 DB 更改源和 Apache Spark 進行流處理更改](https://github.com/Azure/azure-cosmosdb-spark/wiki/Stream-Processing-Changes-using-Azure-Cosmos-DB-Change-Feed-and-Apache-Spark)
...
* [使用應用程式見解監視 Spark 作業](https://github.com/Azure/azure-cosmosdb-spark/tree/2.3/samples/monitoring)
