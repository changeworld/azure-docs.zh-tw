...
本教學課程說明如何使用 Azure Cosmos DB 的大量執行程式 .NET 程式庫，將圖形物件匯入並更新至 Azure Cosmos DB Gremlin API 容器中。 此程序會使用[大量執行程式程式庫](./bulk-executor-overview.md)中的圖形類別，以程式設計方式建立頂點和邊緣物件，繼而為個別的網路要求插入其中多項。 此行為可透過大量執行程式程式庫進行設定，以充分運用資料庫和本機記憶體資源。
...
[大量執行程式程式庫](/dotnet/api/microsoft.azure.cosmosdb.bulkexecutor.graph?preserve-view=true&view=azure-dotnet)包含 `Microsoft.Azure.CosmosDB.BulkExecutor.Graph` 命名空間，可提供建立和匯入圖形物件的功能。 
...
3. 使用 [Graph BulkImportAsync 方法](/dotnet/api/microsoft.azure.cosmosdb.bulkexecutor.graph.graphbulkexecutor.bulkimportasync?preserve-view=true&view=azure-dotnet)，將圖形物件插入集合中。
...
* 透過 **無限制集合** 建立 Azure Cosmos DB Gremlin API 資料庫。 本指南將說明如何開始使用 [.NET 中的 Azure Cosmos DB Gremlin API](./create-graph-dotnet.md)。
...
`AuthorizationKey`|這是您的 Azure Cosmos DB 帳戶下所列的主要或次要金鑰。 請深入了解[保護 Azure Cosmos DB 資料的存取](./secure-access-to-data.md#primary-keys)
...
* 參考[效能祕訣](./bulk-executor-dot-net.md#performance-tips)以進一步最佳化大量執行程式的使用方式。
* 檢閱 [BulkExecutor.Graph 參考文章](/dotnet/api/microsoft.azure.cosmosdb.bulkexecutor.graph?preserve-view=true&view=azure-dotnet)，以取得與此命名空間中定義的類別和方法有關的詳細資訊。