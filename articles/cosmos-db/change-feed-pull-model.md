...
> [!NOTE]
> 變更摘要提取模型目前僅適用於 [Azure Cosmos DB .NET SDK 中的預覽](https://www.nuget.org/packages/Microsoft.Azure.Cosmos/3.9.0-preview)。 預覽尚未提供給其他 SDK 版本使用。

## <a name="consuming-an-entire-containers-changes"></a>使用整個容器的變更

您可以使用提取模型建立 `FeedIterator` 來處理變更摘要。 當您一開始建立 `FeedIterator` 時，可以在 `ChangeFeedRequestOptions` 內指定選擇性的 `StartTime`。 若未指定，則 `StartTime` 將會是目前的時間。
...
使用 `FeedRange`，您就可以建立 `FeedIterator`，將變更摘要的處理平行處理至多部電腦或執行緒。 不同於先前示範如何取得整個容器單一 `FeedIterator` 的範例，您可以使用 `FeedRange` 來取得可平行處理變更摘要的多個 FeedIterators。
