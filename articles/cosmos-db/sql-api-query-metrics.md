...
| `MaxDegreeOfParallelism` | 在 Azure Cosmos 資料庫服務中的並行查詢執行期間獲取或設置運行用戶端的併發運算元。 正值的屬性值會將並行操作次數限制為設定的值。 如果將它設為小於 0，系統就會自動決定要執行的並行操作次數。 |
...
預設情況下，Azure Cosmos DB 將對所有資料應用自動索引。 對於高性能插入方案，請考慮排除路徑，因為這將降低每個插入操作的 RU 成本。 
