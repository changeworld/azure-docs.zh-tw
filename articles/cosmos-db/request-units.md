...
3. **自動調整模式**：在此模式中，您可以根據資料庫或容器的使用量，自動並立即調整其輸送量 (RU/s) ，而不會影響工作負載的可用性、延遲、輸送量或效能。 此模式非常適合具有變數或無法預期流量模式的任務關鍵性工作負載，而且需要高效能和規模的 Sla。 若要深入瞭解，請參閱「 [自動調整輸送量](provision-throughput-autoscale.md) 」一文。 
