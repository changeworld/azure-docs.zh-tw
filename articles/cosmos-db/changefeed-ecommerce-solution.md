...
description: 本文介紹零售公司如何使用更改源來瞭解使用者模式、執行即時資料分析和視覺化
author: SnehaGunda
...
Azure Cosmos DB 更改源是一種機制，用於在創建或修改這些記錄時從 Azure Cosmos 容器獲取連續和增量的記錄源。 變更摘要可藉由接聽容器是否有任何變更來支援工作。 然後變更摘要會輸出已排序的文件清單，這些文件已依其修改的順序變更過。 若要深入了解變更摘要，請參閱[使用變更摘要](change-feed.md)一文。 
...
2. **宇宙資料庫：** 生成的資料存儲在 Azure Cosmos 容器中。  

3. **更改源：** 更改源將偵聽對 Azure Cosmos 容器的更改。 每當有新的文件新增至集合時 (也就是發生使用者檢視某個項目、將某項目新增至其購物車或購買某項目之類的事件時)，變更摘要就會觸發 [Azure 函式](../azure-functions/functions-overview.md)。  
...
1. 轉到[Azure 門戶](https://portal.azure.com/)，查找範本部署創建的**Azure Cosmos DB 帳戶**。  
...
3. 選擇**根管理共用訪問金鑰**。 將 [[連接字串 – 主要金鑰]]**** 複製到 [記事本]，或是您在整個實驗室中都可存取的其他文件。 您應將其標示為 [事件中樞命名空間]**** 連接字串。 您稍後必須將此字串複製到您的程式碼中，因此請將其記下，並記住其儲存位置。

## <a name="set-up-azure-function-to-read-the-change-feed"></a>設定用來讀取變更摘要的 Azure 函式

創建新文檔或在 Cosmos 容器中修改當前文檔時，更改源會自動將修改的文檔添加到其集合歷史記錄中。 現在，您將建置並執行可處理變更摘要的 Azure 函式。 在您已建立的集合中建立或修改文件時，變更摘要將會觸發 Azure 函式。 然後，Azure 函式會將修改過的文件傳送至事件中樞。
...
7. 如果導航到[Azure 門戶](https://portal.azure.com/)，則導航到資源組中的 Cosmos DB 帳戶，然後導航到**資料資源管理器**，您將看到在**更改源實驗室集合**中導入的亂數據。
...
1. 從[Azure 門戶](https://portal.azure.com/)導航到資源組，然後導航到**流作業 1（** 您在預實驗室中創建的流分析作業）。  
...
6. 選擇 **= 添加**。 然後，從下拉式功能表中選取 [Power BI]****。  
...
6. 在 [視覺效果類型]**** 欄位中，從下拉式功能表中選擇 [群組橫條圖]****。 在 [軸]**** 下方新增動作。 略過 [圖例]**** 而不新增任何項目。 然後，在下一節"**值**"下，添加**vg**。選擇 **"下一步**"，然後標題圖表，然後選擇 **"應用**"。 您應該會在儀表板上看到新的圖表！  
...
現在，您將觀察如何使用新的資料分析工具與實際的電子商務網站連線。 為了構建電子商務網站，請使用 Azure Cosmos 資料庫來存儲產品類別（女性、男性、Unisex）、產品目錄和最受歡迎的專案清單的清單。

1. 導航回[Azure 門戶](https://portal.azure.com/)，然後導航到**Cosmos DB 帳戶**，然後導航到**資料資源管理器**。  

   在具有固定存儲容量**的更改源實驗室** - **產品和****類別**下添加兩個集合。
...
要刪除在本實驗期間創建的資源，請導航到[Azure 門戶](https://portal.azure.com/)上的資源組，然後從頁面頂部的功能表**中選擇"刪除資源組**"，然後按照提供的說明進行操作。
