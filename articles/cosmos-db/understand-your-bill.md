...
使用 Azure Cosmos DB 免費層時，您可以在帳戶層級上免費取得您帳戶中的前 400 RU/秒和 5 GB 的儲存體。 任何 RU/秒和超過 400 RU/秒及 5 GB 的儲存體，將會依定價頁面的一般價格費率計費。 在帳單上，您將不會看到免費的 400 Ru/秒和 5 GB 的收費或明細專案，僅限免費層涵蓋的 RU/秒和儲存體。 400 RU/秒適用于任何類型的 RU/秒布建輸送量、autopilot （預覽）和多宿主。  

### <a name="billing-example---container-or-database-with-provisioned-throughput"></a>計費範例-具有已布建輸送量的容器或資料庫
- 假設我們以 400 RU/秒和 5 GB 儲存體的免費層帳戶來建立資料庫或容器。
- 您的帳單不會顯示此資源的任何費用。 您的每小時和每月成本將為 $0。
- 現在，我們假設在相同的帳戶中，我們會新增另一個具有 1000 RU/秒和 10 GB 儲存空間的資料庫或容器。
- 您的帳單現在會顯示 1000 RU/秒和 10 GB 儲存空間的費用。 

### <a name="billing-example---container-or-database-with-autopilot-throughput-preview"></a>計費範例-具有 autopilot 輸送量的容器或資料庫（預覽）
- 我們假設在免費層帳戶中，我們會建立已啟用 autopilot 的資料庫或容器，其最大 RU/秒為 4000 RU/秒。 此資源會自動調整為 400 RU/秒-4000 RU/秒。 
- 假設在每小時1到1小時10日，資源的最小值為 400 RU/秒。 在小時11期間，資源會相應增加到 1000 RU/秒，然後在一小時內向下調整至 400 RU/秒。
- 在1到10小時內，您將以 $0 為輸送量計費，因為 400 RU/秒是由免費層所涵蓋。 
- 在小時11中，將會向您收取有效的 1000 RU/秒-400 RU/秒 = 600 RU/秒，因為這是一小時內最高的 RU/秒。 這會是每小時 100 RU/秒的6個單位，因此該小時的總輸送量成本會是6個單位 * $0.012 = $0.072。 
- 超過前 5 GB 的任何儲存體都會以一般儲存體費率計費。 

### <a name="billing-example---multi-region-single-write-region-account"></a>計費範例-多重區域、單一寫入區域帳戶
- 假設在免費層帳戶中，我們會使用 1200 RU/秒和 10 GB 的儲存體來建立資料庫或容器。 我們會將帳戶複寫到3個區域，而我們有單一主機（單一寫入區域）帳戶。
- 總共，在沒有免費層的情況下，我們會以 3 * 1200 RU/秒 = 3600 RU/秒和 3 * 10 GB = 30 GB 的儲存體來計費。
- 在移除 400 RU/秒和 5 GB 的儲存體之後，我們將以單一寫入區域的速率和 25 GB 的儲存體，針對布建輸送量的有效 3200 RU/秒（32單位）計費。
- RU/秒的每月成本會是：32單位 * $0.008 * 24 小時 * 31 天 = $190.46。 儲存體的每月成本會是： 25 GB * 0.25/GB = $6.25。 總成本會是 $190.46 + $6.25 = $196.71。
- 注意：如果 RU/秒或儲存體的單價因區域而異，免費層 400 RU/秒和 5 GB 會反映區域建立所在帳戶的費率。

### <a name="billing-example---multi-region-multi-master-multiple-write-region-account"></a>計費範例-多個區域、多宿主（多個寫入區域）帳戶

此範例會反映在2019年12月1日之後建立的帳戶的[多宿主定價](https://azure.microsoft.com/pricing/details/cosmos-db/)。 
- 假設在免費層帳戶中，我們會使用 1200 RU/秒和 10 GB 的儲存體來建立資料庫或容器。 我們會將帳戶複寫到3個區域，而我們有多宿主（多個寫入區域）帳戶。 
- 總共，在沒有免費層的情況下，我們會以 3 * 1200 RU/秒 = 3600 RU/秒和 3 * 10 GB = 30 GB 的儲存體來計費。
- 在移除 400 RU/秒和 5 GB 的儲存體之後，我們會以多個寫入區域的速率和 25 GB 的儲存體，針對布建輸送量的有效 3200 RU/秒（32單位）計費。
- RU/秒的每月成本會是：32單位 * $0.016 * 24 小時 * 31 天 = $380.93。 儲存體的每月成本會是： 25 GB * 0.25/GB = $6.25。 總成本會是 $380.93 + $6.25 = $387.18。
