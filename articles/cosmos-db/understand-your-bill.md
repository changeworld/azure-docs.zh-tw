...
假設您在美國西部擁有一個 Azure Cosmos 容器。 您建立了具備 10K RU/秒輸送量的容器，並在這個月儲存 1 TB 資料。 假設您在自己的 Azure Cosmos 帳戶新增了三個區域 (美國東部、歐洲北部與東亞)，每個區域的儲存體與輸送量皆相同。 您的每月帳單總計會是 (假設一個月 30 天)。 您的帳單會是： 
...
假設您在美國西部建立一個 Azure Cosmos 容器。 您建立了具備 10K RU/秒輸送量的容器，並在這個月儲存 1 TB 資料。 假設您新增三個區域 - 美國東部、歐洲北部與東亞，每個區域都有相同的儲存體與輸送量，且希望能寫入與您 Azure Cosmos 帳戶相關聯所有區域中的容器。 您每月帳單總計會是 (假設一個月 30 天)：

|**Item** |**使用量 (月)**|**費率** |**每個月的成本** |
|---------|---------|---------|-------|
|美國西部容器的輸送量計費 (所有區域都可寫入)       | 10K RU/秒 * 24 * 30    |每小時每 100 RU/秒 $0.016    |$1,152 |
|3 個新增區域 (美國東部、歐洲北部與東亞) 的輸送量計費 (所有區域都可寫入)        | (3 + 1) * 10K RU/秒 * 24 * 30    |每小時每 100 RU/秒 $0.016   |$4,608 |
|美國西部容器的儲存體計費      | 250 GB    |$0.25/GB  |$62.50|
|其他 3 個區域的儲存體計費 - 美國東部、北歐和東亞      | 3 * 250 GB    |$0.25/GB  |$187.50|
|**總計**     |     |  |**$6,010**|
...
* 我們建立了一個三區域 Azure Cosmos 帳戶 (美國西部、美國東部、歐洲北部)，且所有區域都可寫入 
