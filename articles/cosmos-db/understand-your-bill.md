...
ms.locfileid: "80258017"
---
# <a name="understand-your-azure-cosmos-db-bill"></a>了解您的 Azure Cosmos DB 帳單

Azure Cosmos DB 是一個完全受控的雲端原生資料庫服務，透過只針對已佈建輸送量與已取用儲存體收費以簡化計費流程。 與本地或 IaaS 託管的替代方案相比，沒有額外的許可費、硬體、公用事業成本或設施成本。 考慮 Azure Cosmos DB 的多區域功能時，與現有的本地或 IaaS 解決方案相比，資料庫服務可大幅降低成本。
...
* 在 720 小時（24 小時 = 30 天）的月份中，如果 500 小時的預配輸送量為 1，200 RU/秒，而其餘 220 小時的預配輸送量為 22，200 RU/秒，則每月帳單顯示：500 x 0.096 美元/小時 = 220 x 1.776 美元/小時 = 438.72 美元/月。
...
* 然後，您的帳單將更改為：1，550 = $0.008 = 12.40 美元/小時。  

* 在 720 小時的一個月中，如果 300 小時的預配輸送量為 120-K RU/秒，其餘 420 小時的預配輸送量為 155-K RU/秒，則每月帳單將顯示：300 x 9.60 美元/小時 = 420 x 12.40 美元/小時 = 2，880 美元 = 5，208 美元 = 8，088 美元/月。 
...
您可以隨時將 Azure Cosmos 資料庫帳戶添加到/刪除世界任何地方的 Azure 區域。 已為各種 Azure Cosmos 資料庫和容器配置的輸送量將保留在與 Azure Cosmos 資料庫帳戶關聯的每個 Azure 區域中。 如果您的 Azure Cosmos 資料庫帳戶內所有資料庫與容器所設定的佈建輸送量 (RU/秒) 總和 (每小時佈建量) 為 T，而與您資料庫帳戶相關聯的 Azure 區域數目為 N，那麼您 Azure Cosmos 資料庫帳戶已知小時的總佈建輸送量分別為，(a) 設定為單一寫入區域等於T x N RU/秒，以及 (b) 設定為能夠處理寫入的所有區域等於 T x (N+1) RU/秒。 佈建的輸送量 (單一寫入區域) 費用為每 100 RU/秒 $0.008/小時，多個可寫入區域 (多重主機設定) 佈建的輸送量費用為每 100 RU/秒 $0.016/小時 (請參閱[定價頁面](https://azure.microsoft.com/pricing/details/cosmos-db/))。 不論您選擇單一寫入區域或多重寫入區域，Azure Cosmos DB 都能讓您從任何區域讀取資料。
...
*我們還假定您每月從美國西部的容器出口 100 GB 的資料，將資料複製到美國東部、北歐和東亞。根據資料傳輸速率，您需要按出口計費。*
...
### <a name="billing-example-azure-cosmos-account-with-multi-master-database-level-throughput-including-dedicated-throughput-mode-for-some-containers"></a>計費示例：Azure Cosmos 帳戶具有多主資料庫級輸送量，包括某些容器的專用輸送量模式
...
| | |其他 2 個區域的輸送量帳單：美國東部、北歐（所有區域都是可寫）  |`(2 + 1) * (60 K RU/sec /100 * $0.016) * 100 hours = $2,880`  |$2,880  |
...
| | |其他 2 個區域的輸送量帳單：美國東部、北歐（所有區域都是可寫）  |`(2 + 1) * (120 K RU/sec /100 * $0.016) * 100 hours = $5,760`  |$5,760  |
...
| | |其他 2 個區域的輸送量帳單：美國東部、北歐（所有區域都是可寫）  |`(2 + 1) * (140 K RU/sec /100 * $0.016-) * 100 hours = $6,720` |$6,720 |
...
| | |其他 2 個區域的輸送量帳單：美國東部、北歐（所有區域都是可寫）  |`(1 + 1) * (90 K RU/sec /100 * $0.016) * 100 hours = $2,880`  |$2,880  |
...
| | |其他 2 個區域的輸送量帳單：美國東部、北歐（所有區域都是可寫）  |`(1 + 1) * (40 K RU/sec /100 * $0.016) * 100 hours = $1,280`  |$1,280  |
...
| | |其他 2 個區域的輸送量帳單：美國東部、北歐（所有區域都是可寫）  |`(1 + 1) * (120 K RU/sec /100 * $0.016) * 200 hours = $1,280`  |$7,680  |
...
| | |其他 2 個區域的輸送量帳單：美國東部、北歐（所有區域都是可寫）  |`(1 + 1) * (70 K RU/sec /100 * $0.016) * 20 hours = $448`  |$224  |
|| |**每月總成本**  | |**$38,688**   |

## <a name="billing-examples-with-free-tier-accounts"></a>使用免費套餐帳戶計費示例
使用 Azure Cosmos DB 免費套餐，您將免費獲得帳戶中的第一個 400 RU/s 和 5 GB 的存儲空間，這些存儲空間在帳戶級別應用。 任何 RU/s 和超過 400 RU/s 和 5 GB 的存儲都將按定價頁面的常規定價費率計費。 在帳單上，您不會看到免費 400 Ru/s 和 5 GB 的費用或行專案，只有 RU/s 和超出免費套餐範圍的存儲。 400 RU/s 適用于任何類型的 RU/s - 預配輸送量、自動駕駛儀（預覽）和多主機。  

### <a name="billing-example---container-or-database-with-provisioned-throughput"></a>計費示例 - 具有預配輸送量的容器或資料庫
- 假設我們在具有 400 RU/s 和 5 GB 存儲空間的免費層帳戶中創建資料庫或容器。
- 您的帳單不會顯示此資源的任何費用。 您的每小時和每月費用將為 $0。
- 現在，讓我們假設在同一帳戶中，我們添加另一個資料庫或容器，其中有 1000 RU/s 和 10 GB 的存儲空間。
- 您的帳單現在將顯示 1000 RU/s 和 10 GB 存儲的費用。 

### <a name="billing-example---container-or-database-with-autopilot-throughput-preview"></a>計費示例 - 具有自動駕駛儀輸送量的容器或資料庫（預覽）
- 假設在一個免費的層帳戶中，我們創建一個資料庫或容器，啟用了自動駕駛儀，最大 RU/s 為 4000 RU/s。 此資源將自動在 400 RU/s - 4000 RU/s 之間縮放。 
- 假設在第 1 小時到 1 小時 10 中，資源至少為 400 RU/s。 在 11 小時期間，資源將擴展到 1000 RU/s，然後在小時內返回 400 RU/s。
- 在小時 1 到 10 中，輸送量將收取 $0，因為 400 RU/s 由免費套餐覆蓋。 
- 在 11 小時中，您將獲得有效 1000 RU/s - 400 RU/s = 600 RU/s 的費用，因為這是一小時內最高的 RU/s。 對於每小時，這將是 6 個單位的 100 RU/s，因此每小時的總輸送量成本將為 6 個單位 = $0.012 = 0.072。 
- 超過前 5 GB 的任何存儲都將以正常存儲速率計費。 

### <a name="billing-example---multi-region-single-write-region-account"></a>計費示例 - 多區域、單寫入區域帳戶
- 假設在一個免費的層帳戶中，我們創建一個資料庫或容器，具有 1200 RU/s 和 10 GB 的存儲空間。 我們將帳戶複製到 3 個區域，並且我們有一個單主（單寫入區域）帳戶。
- 總共，如果沒有免費套餐，我們將為 3 = 1200 RU/s = 3600 RU/s 和 3 = 10 GB = 30 GB 的存儲空間付費。
- 通過免費套餐折扣，在刪除 400 RU/s 和 5 GB 存儲後，我們將以單寫入區域速率和 25 GB 的存儲空間為有效 3200 RU/s （32 台）預配輸送量付費。
- RU/s 的每月成本為：32 個單位 = 0.008 美元 + 24 小時 = 31 天 = 190.46 美元。 每月的存儲成本是：25 GB = 0.25 / GB = 6.25 美元。 總成本為190.46美元=6.25美元=196.71美元。
- 注意：如果 RU/s 或存儲的單價在區域中有所不同，則免費套餐 400 RU/s 和 5 GB 將反映帳戶在其中創建的區域的速率。

### <a name="billing-example---multi-region-multi-master-multiple-write-region-account"></a>計費示例 - 多區域、多主控（多個寫入區域）帳戶

此示例反映 2019 年 12 月 1 日之後創建的帳戶的[多主定價](https://azure.microsoft.com/pricing/details/cosmos-db/)。 
- 假設在一個免費的層帳戶中，我們創建一個資料庫或容器，具有 1200 RU/s 和 10 GB 的存儲空間。 我們將帳戶複製到 3 個區域，並且我們有一個多主控（多個寫入區域）帳戶。 
- 總共，如果沒有免費套餐，我們將為 3 = 1200 RU/s = 3600 RU/s 和 3 = 10 GB = 30 GB 的存儲空間付費。
- 通過免費套餐折扣，在刪除 400 RU/s 和 5 GB 存儲後，我們將以多個寫入區域速率和 25 GB 的存儲空間為有效 3200 RU/s （32 單位）的預配輸送量付費。
- RU/s 的每月成本為：32 個單位 = 0.016 美元 + 24 小時 = 31 天 = 380.93 美元。 每月的存儲成本是：25 GB = 0.25 / GB = 6.25 美元。 總成本為 $380.93 = $6.25 = $387.18。
...
|**輸送量成本** | | | |
|----|----|----|----|
|作業類型| 個要求/秒| 平均 RU/請求| 必要 RU|
|寫入| 100 | 5 | 500|
|讀取| 400| 1| 400|

總 RU/秒： 500 × 400 = 900 每小時成本： 900/100 = $0.008 = $0.072 預期每月輸送量成本 （假設 31 天）： $0.072 = 24 = 31 = $53.57

**每月總成本**

每月成本總計 = 儲存體每月成本 + 輸送量每月成本每月總成本 = $25.00 + $53.57 = $78.57

*定價可能因地區而異。有關最新定價，請參閱[定價頁面](https://azure.microsoft.com/pricing/details/cosmos-db/)。*

## <a name="billing-with-azure-cosmos-db-reserved-capacity"></a>Azure Cosmos DB 保留容量計費

Azure Cosmos DB 預留容量使您能夠提前購買預配輸送量（預留容量或預留），這些輸送量可應用於所有 Azure 區域中的所有 Azure Cosmos 資料庫和容器（對於任何 API 或資料模型）。 因為每個區域的佈建輸送量價格各不相同，所以您可以將保留容量視為以折扣價購得的現金點數，而您可以從中提領，用於每個區域依其各自價格佈建的輸送量。 例如，假設您有一個具備單一容器的 Azure Cosmos 帳戶，容器中佈建 50 K RU/秒與全域複寫兩個區域 - 美國東部與日本東部。 如果選擇隨用隨付選項，您必須支付：  
...
|**地區**| **每 100 RU/秒的每小時價格**|**單位 (RU/秒)**|**計費金額 (每小時)**| **計費金額 (每月)**|
|----|----|----|----|----|
...
* 吞吐成本（即用即付）：100，000 RU/秒/100 = 0.008 美元/小時 = 一年 8760 小時 = 70，080 美元 
