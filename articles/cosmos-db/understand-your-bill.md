...
使用 Azure Cosmos DB 免費層時，您可以在帳戶層級上免費取得您帳戶中的前 400 RU/秒和 5 GB 的儲存體。 任何 RU/秒和超過 400 RU/秒及 5 GB 的儲存體，將會依定價頁面的一般價格費率計費。 在帳單上，您將不會看到免費的 400 Ru/秒和 5 GB 的收費或明細專案，僅限免費層涵蓋的 RU/秒和儲存體。 400 RU/秒適用于任何類型的 RU/秒布建輸送量、自動調整和多宿主。  
...
### <a name="billing-example---container-or-database-with-autoscale-throughput"></a>計費範例-具有自動調整輸送量的容器或資料庫
- 假設在免費層帳戶中，我們會建立已啟用自動調整的資料庫或容器，其最大 RU/秒為 4000 RU/秒。 此資源會自動調整為 400 RU/秒-4000 RU/秒。 
...
- 注意：如果 RU/秒或儲存體的單價與區域不同，免費層 400 RU/秒和 5 GB 會反映帳戶建立所在區域的費率。
