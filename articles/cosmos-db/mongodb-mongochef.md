...
使用 Studio 3T 連接到蒙戈DB的 Azure Cosmos DB API,您必須:
...
要將 Azure Cosmos 帳號加入 Studio 3T 連接管理員,請使用以下步驟:

1. 使用[「將 MongoDB 應用程式連接到 Azure Cosmos DB」](connect-mongodb-account.md)文章中的說明檢索 Azure Cosmos DB MongoDB 帳戶的 API 的連線資訊。

    ![連接字串頁面的螢幕截圖](./media/mongodb-mongochef/ConnectionStringBlade.png)

2. 按一下 [連接]**** 以開啟 [連接管理員]，然後按一下 [新增連線]****

    ![工作室 3T 連接管理員的螢幕截圖](./media/mongodb-mongochef/ConnectionManager.png)
...
4. 在 [新增連線]**** 視窗中，請在 [驗證]**** 索引標籤上選擇驗證模式 [基本 (MONGODB-CR 或 SCARM-SHA-1)]****，並輸入使用者名稱和密碼。  接受預設的驗證資料庫 (管理員)，或提供您自己的值。
    ...
    ![Studio 3T 連線管理員驗證選項卡的螢幕截圖](./media/mongodb-mongochef/ConnectionManagerAuthenticationTab.png)
5. 在 [新增連接]**** 視窗的 [SSL]**** 索引標籤上，勾選 [使用 SSL 通訊協定連接]**** 核取方塊和 [接受自我簽署的 SSL 憑證]**** 選項按鈕。

    ![工作室 3T 連接管理器 SSL 選項卡的螢幕截圖](./media/mongodb-mongochef/ConnectionManagerSSLTab.png)
...
1. 在 [連接管理員]**** 中，反白顯示連接，然後按一下 [連接]****。
    ...
    ![工作室 3T 連接管理員的螢幕截圖](./media/mongodb-mongochef/ConnectToAccount.png)
2. 以滑鼠右鍵按一下主機，然後選擇 [新增資料庫]****。  提供資料庫名稱，然後按一下 [確定] ****。

    ![工作室 3T 新增資料庫選項的螢幕截圖](./media/mongodb-mongochef/AddDatabase1.png)
...
4. 按一下 [集合]**** 功能表項目，然後按一下 [新增文件]****。
    ...
    ![工作室 3T 添加文件選單的螢幕截圖](./media/mongodb-mongochef/AddDocument1.png)
