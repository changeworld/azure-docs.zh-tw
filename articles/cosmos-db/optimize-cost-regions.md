...
假設您在美國西部有一個容器設定為單一區域寫入，而布建的輸送量為 10K RU/秒，並在本月儲存 0.5 TB 的資料。 讓我們假設您使用相同的儲存體和輸送量來新增區域美國東部，而且您想要能夠從您的應用程式寫入這兩個區域中的容器。 您新的每月帳單 (假設一個月730小時) 將如下所示：

|**Item**|**使用量 (每月)**|**費率**|**每個月的成本**|
|----|----|----|----|
|美國西部容器的輸送量帳單 (單一寫入區域)  |10K RU/秒 * 730 小時 |每小時 $0.008 每 100 RU/秒 |$584 |
|2個區域中容器的輸送量計費-美國西部 & 美國東部 (多個寫入區域)  |2 * 10K RU/秒 * 730 小時 |每小時 $0.016 (每 100 RU/秒) |$2336 |
|美國西部容器的儲存體計費 |0.5 TB (或 512 GB)  |$0.25/GB |$128 |
|2個區域中容器的儲存體帳單-美國西部 & 美國東部 |2 * 0.5 TB (或 1024 GB)  |$0.25/GB |$256 |
