...
本文說明如何建立資料表、儲存您的資料，以及對資料執行 CRUD 作業。 選擇 Azure 資料表服務或 Azure Cosmos DB 資料表 API。 本文中說明的範例是以 Ruby 撰寫的，並且使用 [適用於 Ruby 的 Azure 儲存體資料表用戶端程式庫](https://github.com/azure/azure-storage-ruby/tree/master/table)。 涵蓋的案例包括建立資料表、刪除資料表、插入實體，以及查詢資料表中的實體。
...
## <a name="add-access-to-azure-storage-or-azure-cosmos-db"></a>新增對 Azure 儲存體或 Azure Cosmos DB 的存取權
