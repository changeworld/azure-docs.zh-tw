---
title: 使用 Azure Cosmos DB 和 Azure Functions 的無伺服器資料庫計算
...
* 為 Cosmos DB 建立事件驅動的 **Azure Functions 觸發**程式。 此觸發程式依賴 [變更](change-feed.md) 摘要串流來監視您的 Azure Cosmos 容器是否有變更。 對容器進行任何變更時，變更摘要串流會傳送至觸發程序，其叫用 Azure Function。
* 或者，使用 **輸入**系結將 azure 函數系結至 azure Cosmos 容器。 函式執行時，輸入繫結會從容器讀取資料。
* 使用 **輸出**系結將函數系結至 Azure Cosmos 容器。 函式完成時，輸出繫結會將資料寫入容器。

> [!NOTE]
> 目前，Azure Functions 觸發程式、輸入系結，以及 Cosmos DB 的輸出系結僅支援搭配 SQL API 使用。 對於其他所有的 Azure Cosmos DB API，您應對您的 API 使用靜態用戶端，以從函式存取資料庫。
...
Azure Cosmos DB 的 Azure Functions 觸發程式、輸入系結和輸出系結，可用於下列組合：

* 適用于 Cosmos DB 的 Azure Functions 觸發程式可與不同 Azure Cosmos 容器的輸出系結搭配使用。 在函式對變更摘要中的項目執行操作後，您可以將其寫入另一個容器 (將其寫入與來源相同的容器將有效地建立遞迴迴圈)。 或者，您可以使用 Cosmos DB 的 Azure Functions 觸發程式，使用輸出系結，將所有已變更的專案從一個容器有效率地遷移至不同的容器。
* Azure Cosmos DB 的輸入繫結與輸出繫結可用於相同的 Azure Function。 這也在適用於修改後，當您想要利用輸入繫結尋找特定資料、在 Azure Function 中加以修改，然後儲存到相同的容器或不同的容器的情況下。
* Azure Cosmos 容器的輸入系結可用於與 Cosmos DB 的 Azure Functions 觸發程式相同的函式中，也可以搭配或不搭配輸出系結使用。 您可以使用此組合，將最新的貨幣兌換資訊 (使用兌換容器的輸入繫結提取) 套用至購物車服務中新訂單的變更摘要。 已更新的購物車總計，以及目前套用的貨幣轉換，可以使用輸出繫結寫入第三個容器。

## <a name="use-cases"></a>使用案例

下列使用案例示範幾個方法，您可以藉由將您的資料連接至事件驅動的 Azure Functions 來充分利用 Azure Cosmos DB 資料。

### <a name="iot-use-case---azure-functions-trigger-and-output-binding-for-cosmos-db"></a>適用于 Cosmos DB 的 IoT 使用案例 Azure Functions 觸發程式和輸出系結

在 IoT 實作中，您可以在檢查已連線汽車中顯示的引擎燈時，叫用函式。

**實作為：** 針對 Cosmos DB 使用 Azure Functions 觸發程式和輸出系結

1. **Cosmos DB 的 Azure Functions 觸發**程式可用來觸發與汽車警示相關的事件，例如在連接的汽車中進入的檢查引擎燈。
2. 檢查引擎燈時，感應器資料會傳送到 Azure Cosmos DB。
3. Azure Cosmos DB 建立或更新新的感應器資料檔案時，這些變更會串流至 Cosmos DB 的 Azure Functions 觸發程式。
4. 在感應器資料收集每次資料變更時叫用觸發程序，因為所有變更會透過變更摘要串流處理。
5. 閾值條件用於函式中以將感應器資料傳送至保修部門。
6. 如果溫度亦超過特定值，也會向擁有者發送警報。
7. 函數的 **輸出** 系結會更新另一個 Azure Cosmos 容器中的汽車記錄，以儲存檢查引擎事件的相關資訊。
...
1. 使用 [計時器觸發](../azure-functions/functions-bindings-timer.md)程式時，您可以使用 **輸入**系結，以定期間隔取出儲存在 Azure Cosmos 容器中的銀行帳戶餘額資訊。
...
在遊戲中建立新的使用者時，您可以使用 [Azure Cosmos DB Gremlin API](graph-introduction.md) 搜尋可能知道的其他使用者。 然後，您可以將結果寫入 [Azure Cosmos DB 或 SQL 資料庫]() ，以便輕鬆地進行抓取。

**實作為：** 針對 Cosmos DB 使用 Azure Functions 觸發程式和輸出系結

1. 使用 Azure Cosmos DB [graph 資料庫](graph-introduction.md) 來儲存所有使用者，您可以使用 Cosmos DB 的 Azure Functions 觸發程式來建立新的函式。 
...
**實作為：** Cosmos DB 接聽一個容器的多個 Azure Functions 觸發程式

1. 您可以藉由新增 Azure Functions 觸發程式來建立多個 Azure Functions，這些 Cosmos DB 觸發程式全都會接聽相同的購物車資料變更摘要。 請注意，若有多個函式接聽相同的變更摘要，則每個函式皆需要新的租用集合。 如需有關租用集合的詳細資訊，請參閱[了解變更摘要處理器程式庫](change-feed-processor.md)。
2. 每當新的項目新增至使用者的購物車時，會根據購物車容器的變更摘要單獨叫用每個函式。
   * 某個函式可以使用目前購物籃的內容來變更使用者可能感興趣之其他項目的顯示。
   * 另一個函式可更新存貨總計。
   * 另一個函式可將特定產品的客戶資訊傳送至行銷部門，向他們傳送促銷郵件。 

     任何部門都可以藉由接聽變更摘要來建立 Cosmos DB 的 Azure Functions，並確定它們不會延遲進程中的重要訂單處理事件。

在這些使用案例中，由於函式已將應用程式本身分離，因此您不需要隨時啟動新的應用程式實例。 相反地，Azure Functions 會啟動個別函式，視需要完成離散程序。

## <a name="tooling"></a>Tooling

Azure Cosmos DB 與 Azure Functions 之間的原生整合可在 Azure 入口網站和 Visual Studio 2019 中取得。
...
* 在 Azure Cosmos DB 入口網站中，您可以將 Cosmos DB 的 Azure Functions 觸發程式新增至相同資源群組中的現有 Azure 函數應用程式。
