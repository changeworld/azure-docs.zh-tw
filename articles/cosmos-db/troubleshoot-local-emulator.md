---
title: 針對使用 Azure Cosmos DB 模擬器時的問題進行疑難排解
description: 瞭解如何在使用 Azure Cosmos DB 模擬器時，對服務無法使用、憑證、加密和版本控制問題進行疑難排解。
...
ms.locfileid: "96030742"
---
# <a name="troubleshoot-issues-when-using-the-azure-cosmos-db-emulator"></a>針對使用 Azure Cosmos DB 模擬器時的問題進行疑難排解
[!INCLUDE[appliesto-all-apis](includes/appliesto-all-apis.md)]

Azure Cosmos DB 模擬器提供一個模擬 Azure Cosmos DB 服務的本機環境做為開發之用。 使用本文中的秘訣，協助您針對安裝或使用 Azure Cosmos DB 模擬器時所遇到的問題進行疑難排解。 

如果您已安裝新版本的模擬器但發生了錯誤，請務必重設您的資料。 您可以用滑鼠右鍵按一下系統匣上的 Azure Cosmos DB 模擬器] 圖示，然後按一下 [重設資料 ...]，來重設資料。 如果無法修正錯誤，您可以卸載模擬器和任何舊版模擬器（如果找到的話），移除 *C:\Program files\Azure Cosmos DB 模擬器* 目錄，然後重新安裝模擬器。 如需相關指示，請參閱[解除安裝本機模擬器](local-emulator.md#uninstall)。 或者，如果重設資料沒有作用，請流覽至 `%LOCALAPPDATA%\CosmosDBEmulator` 位置並刪除資料夾。

## <a name="troubleshoot-corrupted-windows-performance-counters"></a>針對損毀的 windows 效能計數器進行疑難排解

* 如果 Azure Cosmos DB 模擬器損毀，請從資料夾收集傾印檔案、壓縮檔案， `%LOCALAPPDATA%\CrashDumps` 然後從 [Azure 入口網站](https://portal.azure.com)開啟支援票證。
...
1. 關閉模擬器並監看系統匣，確定程式已關閉。 可能需要一分鐘的時間才能完成。 您也可以在 Azure Cosmos DB 模擬器使用者介面 **中選取 [** 結束]。
