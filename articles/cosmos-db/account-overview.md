...
description: 本文介紹如何創建和使用 Azure Cosmos 帳戶。 它還顯示 Azure Cosmos 帳戶中元素的層次結構
author: markjbrown
ms.author: mjbrown
...
要安全地管理對 Azure Cosmos 帳戶中所有資料的訪問，可以使用與帳戶關聯的[主金鑰](secure-access-to-data.md)。 為了進一步安全訪問資料，可以在 Azure Cosmos 帳戶上配置[VNET 服務終結點](vnet-service-endpoint.md)和[IP 防火牆](firewall-support.md)。 

## <a name="elements-in-an-azure-cosmos-account"></a>Azure Cosmos 帳戶中的元素

Azure Cosmos 容器是可伸縮性的基本單位。 您在容器上可佈建的輸送量 (RU/秒) 和儲存體幾乎不受限制。 Azure Cosmos DB 會使用您指定的邏輯分割區索引鍵以透明的方式分割您的容器，以彈性地調整您佈建的輸送量和儲存體。 如需詳細資訊，請參閱[使用 Azure Cosmos 容器和項目](databases-containers-items.md)。
...
![Azure Cosmos 帳戶的層次結構](./media/account-overview/hierarchy.png)
