...
*運算子深度* | **400** |  周遊中的特有步驟總數。 例如， ```g.V().out()```的運算子計數為2： V （）和 out （）， ```g.V('label').repeat(out()).times(100)```運算子深度為3： V （）、重複（）和 out （），因為```.times(100)```是```.repeat()```運算子的參數。
...
*每小時的資源權杖* | **100** | Gremlin 用戶端用來連線到區域中 Gremlin 帳戶的唯一資源權杖數目。 當應用程式超過每小時唯一的權杖`"Exceeded allowed resource token limit of 100 that can be used concurrently"`限制時，將會在下一次驗證要求時傳回。
