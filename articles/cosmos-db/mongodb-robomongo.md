...
4. 在 [連接設定]**** 視窗中，選擇名稱。 然後，從步驟 1 的連接資訊中，找出 [主機]**** 和 [連接埠]****，分別輸入至 [位址]**** 和 [連接埠]****。
    ...
    :::image type="content" source="./media/mongodb-robomongo/manageconnections.png" alt-text="Robomongo 管理連線的螢幕擷取畫面":::
5. 在 [驗證]**** 索引標籤上，按一下 [執行驗證]****。 然後，輸入您的 [資料庫] \(預設值是 *Admin*)、[使用者名稱]**** 和 [密碼]****。
在步驟 1 的連接資訊中，可以找到 [使用者名稱]**** 和 [密碼]****。

    :::image type="content" source="./media/mongodb-robomongo/authentication.png" alt-text="[Robomongo Authentication] 索引標籤的螢幕擷取畫面":::
6. 在 [SSL]**** 索引標籤上，勾選 [使用 SSL 通訊協定]****，然後將 [驗證方法]**** 變更為 [自我簽署憑證]****。

    :::image type="content" source="./media/mongodb-robomongo/SSL.png" alt-text="[Robomongo SSL] 索引標籤的螢幕擷取畫面":::
