...
1. **連線模式：使用 DirectHttps**
   ...
   2. [DirectHttps](https://docs.microsoft.com/java/api/com.microsoft.azure.documentdb.connectionmode)
      ...
      Java SDK 使用 HTTPS 作為傳輸通訊協定。 HTTPS 使用 TLS 進行初始身份驗證和加密流量。 當使用 Java SDK 時，只需要開啟 HTTPS 連接埠 443。 
...
2. **在應用程式存留期內使用單一 Azure Cosmos DB 用戶端**
    ...
    每個[DocumentClient](https://docs.microsoft.com/java/api/com.microsoft.azure.documentdb.documentclient)實體都是線程安全的,在直接模式下運行時執行高效的連接管理和位址快取。 若要藉由 DocumentClient 獲得有效率的連接管理和更佳的效能，建議在應用程式存留期內對每個 AppDomain 使用單一 DocumentClient 執行個體。
...
4. **微調分割之集合的平行查詢**
    ...
    (a) ***微調 setMaxDegreeOfParallelism\:*** 平行查詢的運作方式是以平行方式查詢多個分割。 不過，對於查詢會以循序方式擷取來自個別分割集合的資料。 因此,使用[setMaxAToParallelism](https://docs.microsoft.com/java/api/com.microsoft.azure.documentdb.feedoptions.setmaxdegreeofparallelism)設置具有最大實現最性能查詢機會的分區數,前提是所有其他系統條件保持不變。 如果您不知道分割數目，您可以使用 setMaxDegreeOfParallelism 設定為較高的數字，然後系統會選擇最小值 (分割數目、使用者提供的輸入) 作為平行處理原則的最大刻度。 
...
8. **調整查詢/讀取摘要的頁面大小以獲得更好的效能**
    ...
    使用讀取來源功能(例如,[讀取文檔](https://docs.microsoft.com/java/api/com.microsoft.azure.documentdb.documentclient.readdocuments))或發出 SQL 查詢執行文件批量讀取時,如果結果集太大,則以分段方式返回結果。 根據預設，會以 100 個項目或 1 MB 的區塊傳回結果 (以先達到的限制為準)。
...
1. **從索引中排除未使用的路徑,以便加快寫入速度**

    Azure Cosmos DB 的索引原則允許您透過利用索引路徑([設定包含路徑](https://docs.microsoft.com/java/api/com.microsoft.azure.documentdb.indexingpolicy.setincludedpaths)和[設定排除路徑](https://docs.microsoft.com/java/api/com.microsoft.azure.documentdb.indexingpolicy.setexcludedpaths))指定要包括或排除索引的文件路徑。 在事先知道查詢模式的案例中，使用檢索路徑可改善寫入效能並降低索引儲存空間，因為檢索成本與檢索的唯一路徑數目直接相互關聯。  例如，以下程式碼示範如何將文件的整個區段 (也稱為 樹狀子目錄) 自索引編製作業中排除 (透過使用 "*" 萬用字元)。
...
1. **測量和調整較低的要求單位/秒使用量**
    ...
    要測量任何操作(創建、更新或刪除)的開銷,請檢查[x-ms 請求-請求-費用](https://docs.microsoft.com/rest/api/cosmos-db/common-cosmosdb-rest-response-headers)標頭(或[\<資源回應 T>](https://docs.microsoft.com/java/api/com.microsoft.azure.documentdb.resourceresponse)或[FeedResponse\<T>](https://docs.microsoft.com/java/api/com.microsoft.azure.documentdb.feedresponse)中的等效 RequestCharge 屬性,以測量這些操作使用的請求單位數。
...
1. **處理速率限制/要求速率太大**
    ...
    當用戶端嘗試超過帳戶保留的輸送量時，伺服器的效能不會降低，而且不會使用超過保留層級的輸送量容量。 伺服器將先發制人地使用 RequestRateTooLarge(HTTP 狀態代碼 429)結束請求,並返回[x-ms-retry 後 ms](https://docs.microsoft.com/rest/api/cosmos-db/common-cosmosdb-rest-response-headers)標頭,指示使用者在重試請求之前必須等待的時間量(以毫秒為單位)。
    ...
    如果多個用戶端累積操作始終高於請求速率,則用戶端當前在內部設置為 9 的預設重試計數可能不足;在這種情況下,用戶端向應用程式拋出具有狀態代碼 429 的[DocumentClientException。](https://docs.microsoft.com/java/api/com.microsoft.azure.documentdb.documentclientexception) 可以使用[連接策略](https://docs.microsoft.com/java/api/com.microsoft.azure.documentdb.connectionpolicy)實例上的[「重試選項」](https://docs.microsoft.com/java/api/com.microsoft.azure.documentdb.connectionpolicy.setretryoptions)更改預設重試計數。 根據預設，如果要求繼續以高於要求速率的方式運作，則會在 30 秒的累計等候時間後傳回 DocumentClientException (狀態碼 429)。 即使目前的重試計數小於最大重試計數 (預設值 9 或使用者定義的值)，也會發生這種情況。
