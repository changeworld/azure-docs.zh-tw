...
2. **在應用程式存留期內使用單一 Azure Cosmos DB 用戶端**
    ...
    每個[DocumentClient](https://docs.microsoft.com/java/api/com.microsoft.azure.documentdb.documentclient)實例都是執行緒安全的，在直接模式下運行時執行高效的連接管理和位址緩存。 若要藉由 DocumentClient 獲得有效率的連接管理和更佳的效能，建議在應用程式存留期內對每個 AppDomain 使用單一 DocumentClient 執行個體。
...
8. **調整查詢/讀取摘要的頁面大小以獲得更好的效能**
    ...
    使用讀取源功能（例如，[讀取文檔](https://docs.microsoft.com/java/api/com.microsoft.azure.documentdb.documentclient.readdocuments)）或發出 SQL 查詢執行文檔批量讀取時，如果結果集太大，則以分段方式返回結果。 根據預設，會以 100 個項目或 1 MB 的區塊傳回結果 (以先達到的限制為準)。
...
1. **處理速率限制/要求速率太大**
    ...
    當用戶端嘗試超過帳戶保留的輸送量時，伺服器的效能不會降低，而且不會使用超過保留層級的輸送量容量。 伺服器將先發制人地使用 RequestRateTooLarge（HTTP 狀態碼 429）結束請求，並返回[x-ms-retry 後 ms](https://docs.microsoft.com/rest/api/cosmos-db/common-cosmosdb-rest-response-headers)標頭，指示使用者在重試請求之前必須等待的時間量（以毫秒為單位）。
