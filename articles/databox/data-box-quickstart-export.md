...
1. 如果使用 Windows 用戶端，請使用 SMB 相容的檔案複製工具，例如 Robocopy。 針對 NFS 主機，使用 `cp` 命令或 `rsync` 來複製資料。 如需有關如何使用 Robocopy 來複製資料的詳細資訊，請移至 [Robocopy](/previous-versions/technet-magazine/ee851678(v=msdn.10))。
