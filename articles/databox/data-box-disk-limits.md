...
## <a name="data-box-service-limits"></a>資料箱服務限制
 ...
 - 資料框服務在["區域可用性](data-box-disk-overview.md#region-availability)"中列出的 Azure 區域中可用。
 - 資料箱磁碟支援單一儲存體帳戶。
...
- [阻止 Blob 和頁面 blob 約定](https://docs.microsoft.com/rest/api/storageservices/understanding-block-blobs--append-blobs--and-page-blobs)
...
- 請勿直接將資料複製到磁碟中。 將資料複製到預先創建的塊*BlockBlob**Blob、PageBlob*和*AzureFile*資料夾。
- *BlockBlob* 和 *PageBlob* 下的資料夾是容器。 例如，容器會建立為 *BlockBlob/container* 和 *PageBlob/container*。
- 如果雲中現有 Azure 物件（如 Blob）與正在複製的物件同名，則資料盒磁片將在雲中將檔重命名為檔（1）。
...
- 將資料上載到 Azure 檔時，不會保留檔中繼資料和 NTFS 許可權。 例如，複製資料時，不會保留檔的 *"上次修改*"屬性。
...
|-------------------|-----------------------------------------------------------|
| 區塊 Blob        | ~ 4.75 TiB                                                 |
| 分頁 Blob         | 8 TiB <br> （以頁面 Blob 格式上載的每個檔必須對齊 512 個位元組，否則上載失敗。 <br> VHD 和 VHDX 均對齊 512 位元組。 |
|Azure 檔案        | 1 TiB <br> 最大 份額大小為 5 TiB     |
| 受控磁碟     |4 TiB <br> 有關大小和限制的詳細資訊，請參閱： <li>[託管磁片的可伸縮性目標](../virtual-machines/windows/disk-scalability-targets.md#managed-virtual-machine-disks)</li>|
...
| 單位                                       | 慣例                                                                                                                                                                                                                                                                                                               |
|----------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 區塊 Blob 和分頁 Blob 的容器名稱 <br> Azure 檔的檔共用名稱稱 | 必須是長度介於 3 到 63 個字元的有效 DNS 名稱。 <br>  必須以字母或數字開頭。 <br> 只能包含小寫字母、數字和連字號 (-)。 <br> 每個連字號 (-) 的前後都必須緊鄰字母或數字。 <br> 名稱中不允許連續的連字號。 |
| Azure 檔案的目錄和檔案名稱     |<li> 保留大小寫、不區分大小寫而且長度不得超過 255 個字元。 </li><li> 不能以正斜線 (/) 結尾。 </li><li>如果有的話，則會自動移除。 </li><li> 不允許使用以下字元：<code>" \\ / : \| < > * ?</code></li><li> 保留的 URL 字元必須正確逸出。 </li><li> 不允許使用不合法的 URL 路徑字元。 uE000 等\\代碼點不是有效的 Unicode 字元。 某些 ASCII 或 Unicode 字元（如控制字元（0x00 到\\0x1F、u0081 等）也不允許。 如需在 HTTP/1.1 中控管 Unicode 字串的規則，請參閱 RFC 2616 第 2.2 節：基本規則和 RFC 3987。 </li><li> 不允許下列檔案名稱：LPT1、LPT2、LPT3、LPT4、LPT5、LPT6、LPT7、LPT8、LPT9、COM1、COM2、COM3、COM4、COM5、COM6、COM7、COM8、COM9、PRN、AUX、NUL、CON、CLOCK$、點字元 (.) 和雙點字元 (..)。</li>|
...
## <a name="managed-disk-naming-conventions"></a>託管磁片命名約定

| 單位 | 慣例                                             |
|-------------------|-----------------------------------------------------------|
| 託管磁片名稱       | <li> 名稱必須為 1 到 80 個字元長。 </li><li> 名稱必須以字母或數位開頭，以字母、數位或底線結尾。 </li><li> 名稱可能僅包含字母、數位、底線、句點或連字號。 </li><li>   名稱不應具有空格或`/`。                                              |

## <a name="next-steps"></a>後續步驟

- 查看[資料盒磁片系統要求](data-box-disk-system-requirements.md)
