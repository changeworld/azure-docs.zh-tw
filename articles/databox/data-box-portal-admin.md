...
對於匯入和匯出訂單，您只能在訂單處理之前取消訂單。 處理訂單並備妥資料箱裝置之後，就無法取消訂單。
...
2. 按一下 [下載訂單記錄]****。 在下載的記錄中，您會看到貨運公司追蹤記錄的記錄。 Data Box Heavy 上會有兩組對應到兩個節點的記錄。 如果您捲動到此記錄的底部，將可看到下列項目的連結：
   ...
   - **複製記錄**-具有從資料箱資料複製到您的 Azure 儲存體帳戶期間所錯誤的檔案清單 (匯入順序) 或*反向* (匯出順序) 。
   - **Audit logs** -包含有關如何在 Azure 資料中心外開啟和存取資料箱上之共用的資訊。
   - 匯**入順序中的 BOM**檔案-具有檔案清單 (也稱為檔案資訊清單) ，您可以在**寄送準備**期間下載該檔案，並具有檔案名、檔案大小和檔案總和檢查碼。
   - [詳細資訊記錄] （**匯出順序**）-具有檔案名、檔案大小和總和檢查碼計算的檔案清單（當資料從 Azure 儲存體帳戶複製到資料箱時）。
...
|訂單狀態 |描述 |
|---------|---------|
|訂購時間     | 已成功放置匯出訂單。 <br>如果裝置可供使用，Microsoft 會識別要寄送的裝置，並準備該裝置。 <br> 如果裝置無法立即提供使用，系統將會在裝置可供使用時處理該訂單。 訂單的處理可能需花費數天到數個月的時間。 如果無法在 90 天內完成訂單，系統將會取消該訂單並通知您。         |
|已取消            |訂單便會取消。 <br> 您已取消訂單 (您只能在訂單處理之前取消) 或發生錯誤，且服務已取消訂單。 如果無法在 90 天內完成訂單，系統也將會取消該訂單並通知您。     |
|已處理     | 訂單處理已完成。 根據您的訂單，裝置已準備好進行資料中心的資料複製。 建立裝置共用。         |
|資料複製進行中     | 從指定的 Azure 儲存體帳戶複製到裝置的資料正在進行中。 在 Azure 入口網站中追蹤您訂單的複製進度。 <br> 等候資料複製完成。 |
|複製完成     | 從指定的 Azure 儲存體帳戶複製到裝置的資料已完成。 詳細資訊記錄檔 (如果已在 order) 中啟用此選項，並在您的儲存體帳戶中建立複製記錄檔。 詳細資訊記錄檔包含複製到裝置之所有檔案 (名稱、路徑、計算總和檢查碼) 的相關資訊。 複製記錄包含複製程式的摘要，包括因任何錯誤而無法複製的檔案清單。<br> 儲存體帳戶資料會保持不變。 |
|複製已完成，但發生錯誤| 資料複製已完成，但在複製期間發生錯誤。 <br> 使用 Azure 入口網站中提供的路徑來檢查 Azure 儲存體帳戶中的複製記錄檔。 請參閱[下載完成但發生錯誤的複製記錄範例](https://docs.microsoft.com/azure/databox/data-box-logs#upload-completed-with-errors)。   |
|複製完成，但出現警告| Azure 儲存體帳戶的資料複製已完成，但資料有不嚴重的錯誤。 <br> 使用 Azure 入口網站中提供的路徑來檢閱複製記錄。 記下非嚴重的錯誤。 [在下載完成但出現警告時，請參閱複製記錄檔的範例](https://docs.microsoft.com/azure/databox/data-box-logs#upload-completed-with-warnings)。   |
|複製失敗，發生錯誤| Azure 儲存體帳戶的資料複製失敗，訂單會終止。 將不會寄送裝置。<br> 使用 Azure 入口網站中提供的路徑來檢查 Azure 儲存體帳戶中的複製記錄檔。 請參閱[下載失敗但發生錯誤時複製記錄檔的範例](https://docs.microsoft.com/azure/databox/data-box-logs#upload-completed-with-errors)。   |
|已分派     |訂單已出貨。 在入口網站中使用顯示在訂單上的追蹤識別碼來追蹤運輸狀態。        |
|已傳遞     |裝置已寄送至訂單中指定的地址。        |
|已取貨     |您寄回的裝置已經由快遞公司收取並掃描。         |
|已收到     | 您的裝置已由 Azure 資料中心收到並掃描。 <br> 已檢查出貨。      |
|已完成           |訂單已完成。     |
|清除 | 裝置上的資料已清除。 當訂單記錄在 Azure 入口網站中可供下載時，即視為已完成裝置清除。|

> [!NOTE]
> 如果將資料從 Azure 儲存體帳戶匯出至資料箱完成但發生錯誤或警告的複製作業，裝置仍會寄送。 只有在複製失敗的情況下，訂單才會終止，裝置也不會寄出。


如果使用自我管理的出貨，則在複製完成且在您收到裝置之前，您會看到下列狀態 (，而不是上表中所述的) ：

|訂單狀態 |描述 |
|---------|---------|
|準備好在 Azure 資料中心收取      |裝置已準備好在 Azure 資料中心上收取。        |
|已取貨    |您已挑選裝置。         |
|準備好在 Azure 資料中心接收     |裝置已準備好接收到 Azure 資料中心。        |
|已收到     |Azure 資料中心會收到該裝置。      |
