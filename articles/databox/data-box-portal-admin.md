...
|訂單狀態 |描述 |
|---------|---------|
|訂購時間     | 已成功下單。 <br>如果裝置可供使用，Microsoft 會識別要寄送的裝置，並準備該裝置。 <br> 如果裝置無法立即使用，則會在裝置變成可用時處理訂單。 訂單的處理可能需花費數天到數個月的時間。 如果無法在90天內完成訂單，就會取消訂單並通知您。         |
...
|已取消            |訂單便會取消。 <br> 您已取消訂單，或服務在發生錯誤之後取消了訂單。 如果無法在90天內完成訂單，則訂單也會取消，而您會收到通知。     |
