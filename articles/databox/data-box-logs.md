...
- 您的資料箱會以鎖定狀態抵達內部部署。 您可以使用 Azure 入口網站中提供的裝置認證來取得您的訂單。  
    ...
    設定資料箱時，您可能需要知道誰已存取裝置認證。 若要找出存取 [**裝置認證**] 分頁的人員，您可以查詢活動記錄。  涉及存取**裝置詳細資料 > 認證** 分頁的任何動作都會以 `ListCredentials` 動作的形式，記錄到活動記錄中。
...
`storage-account-name/databoxcopylog/ordername_device-serial-number_CopyLog_guid.xml`第 1 課：建立 Windows Azure 儲存體物件{2}。
...
容器的新唯一名稱採用 `DataBox-GUID` 格式，而容器的資料會放入新重新命名的容器中。 複製記錄檔會為容器指定舊的和新的容器名稱。
...
`copylog` 會指定舊的和新的 blob 或檔案名，以及 Azure 中的路徑。
