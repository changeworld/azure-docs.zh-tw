...
只有 Azure AD 系統管理員使用者可以建立/啟用以 Azure AD 為基礎之驗證的使用者。 我們建議您不要使用 Azure AD 系統管理員進行一般資料庫作業，因為它具有更高的使用者權限（例如 CREATEDB）。
...
> [!IMPORTANT]
> 設定系統管理員時，會將新的使用者新增至具有完整系統管理員許可權的適用於 PostgreSQL 的 Azure 資料庫伺服器。 適用於 PostgreSQL 的 Azure 資料庫中的 Azure AD 系統管理使用者會擁有該`azure_ad_admin`角色。
...
使用`psql`命令列用戶端時，必須透過`PGPASSWORD`環境變數傳遞存取權杖，因為存取權杖超出`psql`可直接接受的密碼長度：
...
3. 在適用於 PostgreSQL 的 Azure 資料庫`<user>@yourtenant.onmicrosoft.com`中建立角色。
4. 將`<user>@yourtenant.onmicrosoft.com`角色的成員 azure_ad_user。 這必須僅提供給 Azure AD 使用者。
...
如果您現有的使用者已經符合 Azure AD 的使用者名稱，您可以授與`azure_ad_user`角色給他們，以便 Azure AD 驗證：
