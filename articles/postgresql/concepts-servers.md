---
title: 伺服器 - Azure 資料庫，用於後格雷SQL - 單伺服器
description: 本文提供了為 PostgreSQL - 單伺服器配置和管理 Azure 資料庫的注意事項和指南。
author: rachel-msft
ms.author: raagyema
...
本文提供了使用 Azure 資料庫的注意事項和指南，用於 PostgreSQL - 單伺服器。

## <a name="what-is-an-azure-database-for-postgresql-server"></a>什麼是適用於 PostgreSQL 的 Azure 資料庫伺服器？
Azure 資料庫中的伺服器用於 PostgreSQL - 單伺服器部署選項是多個資料庫的中心管理點。 這與您可能已在內部部署領域中熟悉的 PostgreSQL 伺服器建構相同。 具體來說，PostgreSQL 服務會受到管理、提供效能保證、公開伺服器層級的存取和功能。
...
- 可在多個版本中使用。 有關詳細資訊，請參閱支援的[PostgreSQL 資料庫版本](concepts-supported-versions.md)。
...
|||
|:--|:--|
| **身份驗證和授權** | 適用於 PostgreSQL 的 Azure 資料庫伺服器支援原生的 PostgreSQL 驗證。 您可以利用伺服器的系統管理員登入來連接和驗證伺服器。 |
| **協定** | 此服務支援 PostgreSQL 所使用的訊息架構通訊協定。 |
...
可以使用[Azure 門戶](https://portal.azure.com)或[Azure CLI](/cli/azure/postgres)管理 PostgreSQL 伺服器的 Azure 資料庫。
...
- 有關服務的概述，請參閱 Azure [資料庫，用於 PostgreSQL 概述](overview.md)。
