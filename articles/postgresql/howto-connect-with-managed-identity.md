---
title: 使用受控識別連線 - 適用於 PostgreSQL 的 Azure 資料庫 - 單一伺服器
description: 了解如何使用受控識別進行連線和驗證，以利用適用於 PostgreSQL 的 Azure 資料庫進行驗證
author: lfittl-msft
ms.author: lufittl
...
- 您需要已設定 [Azure AD 驗證](howto-configure-sign-in-aad-authentication.md)的適用於 PostgreSQL 的 Azure 資料庫資料庫伺服器
- 若要遵循此 C# 範例，請先完成如何[與 C# 連線](connect-csharp.md)的指南
...
## <a name="creating-a-postgresql-user-for-your-managed-identity"></a>為您的受控識別建立 PostgreSQL 使用者

現在，以 Azure AD 系統管理員使用者身分連線到您的 PostgreSQL 資料庫，然後執行下列 SQL 陳述式：
...
針對測試目的，您可以在殼層中執行下列命令。 請注意，您必須安裝 `curl`、`jq` 和 `psql` 用戶端。
...
本節將說明如何使用 VM 使用者指派的受控識別來取得存取權杖，以用來呼叫適用於 PostgreSQL 的 Azure 資料庫。 適用於 PostgreSQL 的 Azure 資料庫原生支援 Azure AD 驗證，因此可以直接接受使用 Azure 資源的受控識別所取得的存取權杖。 建立與 PostgreSQL 的連線時，您將存取權杖傳遞至密碼欄位。

以下是使用存取權杖來開啟與 PostgreSQL 連線的 .NET 程式碼範例。 此程式碼必須在 VM 上執行，才能夠存取 VM 使用者指派的受控識別端點。 必須要有 .NET Framework 4.6 (或更新版本) 或 .NET Core 2.2 (或更新版本)，才能使用存取權杖方法。 取代 HOST、USER、DATABASE 和 CLIENT_ID 的值。
...
* 檢閱[使用適用於 PostgreSQL 的 Azure 資料庫進行 Azure Active Directory 驗證](concepts-aad-authentication.md)的整體概念
