...
`Get-AzPostgreSQLReplica` 命令需要下列參數：
...
### <a name="stop-a-replica-server"></a>停止複本伺服器

停止讀取複本伺服器會將讀取複本升級為獨立的伺服器。 您可以藉由執行 `Update-AzPostgreSqlServer` Cmdlet，並將 ReplicationRole 值設為，來完成此操作 `None` 。
