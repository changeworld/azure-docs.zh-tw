...
ms.locfileid: "88545432"
---
# <a name="use-java-and-jdbc-with-azure-database-for-postgresql"></a>搭配使用 Java 和 JDBC 與適用於 PostgreSQL 的 Azure 資料庫

本主題示範如何建立使用 Java 和 [JDBC](https://en.wikipedia.org/wiki/Java_Database_Connectivity) 的範例應用程式，以在[適用於 PostgreSQL 的 Azure 資料庫](https://docs.microsoft.com/azure/postgresql/)中儲存及擷取資訊。
...
> [!NOTE]
> 您可以在[使用 Azure 入口網站建立適用於 PostgreSQL 的 Azure 資料庫伺服器](/azure/postgresql/quickstart-create-server-database-portal)中，閱讀更多關於如何建立 PostgreSQL 伺服器的詳細資訊。
