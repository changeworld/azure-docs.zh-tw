---
title: 專用連結 - Azure 門戶 - 用於 PostgreSQL 的 Azure 資料庫 - 單個伺服器
description: 瞭解如何為 Azure 門戶中的 PostgreSQL- 單一伺服器配置 Azure 資料庫的專用連結
author: kummanish
ms.author: manishku
...
ms.locfileid: "79370911"
---
# <a name="create-and-manage-private-link-for-azure-database-for-postgresql---single-server-using-portal"></a>創建和管理 Azure 資料庫的專用連結，用於 PostgreSQL - 使用門戶創建單個伺服器

私人端點是 Azure 中私人連結的基本要素。 其可讓 Azure 資源 (例如虛擬機器 (VM)) 與私人連結資源進行私密通訊。  在本文中，您將學習如何使用 Azure 門戶在 Azure 虛擬網路中創建 VM，以及使用 Azure 專用終結點的 PostgreSQL 單一伺服器的 Azure 資料庫。
...
在本節中，您將創建虛擬網路和子網來承載用於訪問專用連結資源的 VM（Azure 中的 PostgreSQL 伺服器）。
...
## <a name="create-an-azure-database-for-postgresql-single-server"></a>為後格雷SQL單一伺服器創建 Azure 資料庫

在本節中，您將在 Azure 中為 PostgreSQL 伺服器創建 Azure 資料庫。 

1. 在 Azure 門戶中螢幕的左上角，選擇**為 PostgreSQL****創建資源** > **資料庫** > Azure 資料庫 。

1. 在**Azure 資料庫的 PostgreSQL 部署選項**中，選擇**單個伺服器**並提供以下資訊：
    ...
    | Location | 選擇要希望 PostgreSQL 伺服器駐留的 Azure 區域。 |
    |版本  | 選擇所需的 PostgreSQL 伺服器的資料庫版本。|
...
在本節中，您將創建 PostgreSQL 伺服器，並添加專用終結點。 
...
6. 在 [建立私人端點 - 資源]**** 中，輸入或選取這項資訊：
    ...
    | 訂用帳戶| 選取您的訂用帳戶。 |
    | 資源類型 | 選擇**微軟.DBforPostgreSQL/伺服器**。 |
    | 資源 |選取 [myServer]**|
    |目標子資源 |選擇*後格雷sqlServer*|
    |||
7. 選擇 **"下一步"：配置**。
8. 在 **"創建專用終結點 - 配置**"中，輸入或選擇此資訊：
    ...
    |**私人 DNS 整合**||
    |與私人 DNS 區域整合 |選取 [是]****。 |
    |私人 DNS 區域 |選擇 *（新）私人連結.postgres.資料庫.azure.com* |
    |||

1. 選擇 **"審閱" = 創建**。 您會移至 [檢閱 + 建立]**** 頁面，其中 Azure 會驗證您的設定。 
2. 當您看到**驗證傳遞**的消息時，選擇 **"創建**"。 

    ![已創建專用連結](media/concepts-data-access-and-security-private-link/show-postgres-private-link.png)
...
## <a name="access-the-postgresql-server-privately-from-the-vm"></a>從 VM 私下訪問 PostgreSQL 伺服器
...
3. 使用任何可用的用戶端測試 PostgreSQL 伺服器的專用鏈路連接。 在下面的示例中，我使用[Azure 資料工作室](https://docs.microsoft.com/sql/azure-data-studio/download?view=sql-server-ver15)來執行此操作。
...
4. 在 **"新建連接**"中，輸入或選擇此資訊：
    ...
    | [使用者名稱] | 輸入在username@servernamePostgreSQL 伺服器創建期間提供的使用者名。 |
...
7. （可選）從後格雷SQL伺服器創建或查詢資訊。

8. 關閉遠端桌面連線到 myVm。

## <a name="clean-up-resources"></a>清除資源
使用專用終結點 PostgreSQL 伺服器和 VM 完成後，請刪除資源組及其包含的所有資源：
...
在此執行中，您可以在虛擬網路上創建了 VM、為 PostgreSQL - 單伺服器創建 Azure 資料庫以及用於專用訪問的專用終結點。 您從 Internet 連接到一個 VM，並使用專用鏈路安全地連接到 PostgreSQL 伺服器。 要瞭解有關私有終結點的更多內容，請參閱[什麼是 Azure 專用終結點](https://docs.microsoft.com/azure/private-link/private-endpoint-overview)。
