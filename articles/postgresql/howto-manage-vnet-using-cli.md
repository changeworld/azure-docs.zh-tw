---
title: 使用虛擬網路規則 - Azure CLI - 用於後格雷SQL的 Azure 資料庫 - 單個伺服器
description: 本文介紹如何使用 Azure CLI 命令列為 PostgreSQL 創建和管理 Azure 資料庫的 VNet 服務終結點和規則。
author: bolzmj
ms.author: mbolz
...
---
# <a name="create-and-manage-vnet-service-endpoints-for-azure-database-for-postgresql---single-server-using-azure-cli"></a>使用 Azure CLI 為後格雷SQL - 單伺服器創建和管理 Azure 資料庫的 VNet 服務終結點
...
如果您選擇在本機安裝和使用 CLI，本文會要求您執行 Azure CLI 2.0 版或更新版本。 若要查看所安裝的版本，請執行 `az --version` 命令。 如果需要安裝或升級，請參閱[安裝 Azure CLI](/cli/azure/install-azure-cli)。 
