---
title: 定價層 - Azure 資料庫,用於 PostgreSQL - 單一伺服器
description: 本文介紹了 Azure 資料庫中的 PostgreSQL - 單伺服器的計算和儲存選項。
author: jasonwhowell
ms.author: jasonh
...
建立伺服器之後，虛擬核心數目、硬體世代和定價層 (基本層的來回除外) 可在幾秒內上下變動。 您可以也單獨調高儲存體的數量及延長或縮短備份保留期限，而無須中斷應用程式。 但您無法在建立伺服器之後，變更備份儲存體類型。 有關詳細資訊,請參閱[縮放資源](#scale-resources)部分。

## <a name="compute-generations-and-vcores"></a>計算世代和虛擬核心

計算資源會以 vCore 的形式提供，vCore 代表了基礎硬體的邏輯 CPU。 中國東1號、中國北1、美國DoD中央和美國DoD東使用基於英特爾E5-2673 v3(哈斯韋爾)2.4-GHz處理器的4代邏輯CPU。 所有其他區域都使用基於英特爾 E5-2673 v4 (Broadwell) 2.3-GHz 處理器的 5 代邏輯 CPU。
...
| IOPS | 變數 |3 IOPS/GB<br/>最小值為 100 IOPS<br/>最多 20,000 IOPS | 3 IOPS/GB<br/>最小值為 100 IOPS<br/>最多 20,000 IOPS |

> [!NOTE]
> 以下區域支援高達 16TB 和 20,000 IOPS 的存儲:美國東部、美國東部 2、美國中部、美國西部、美國中北部、美國中南部、北歐、西歐、英國南部、英國西部、東南亞、東亞、日本東部、日本西部、韓國中部、韓國南部、澳大利亞東部、澳大利亞東南部。
>
> 所有其他區域都支援高達 4TB 的儲存空間和 6000 IOPS。
>
...
如果可用儲存小於 512MB 或預先儲存大小的 5%,則將標記為僅讀 100 GB 預配儲存的伺服器。 若伺服器中佈建的儲存體超過 100 GB，則只有在可用儲存空間小於 5 GB 時，伺服器所才會標示為唯讀。

例如,如果已預配了 110 GB 的存儲空間,並且實際利用率超過 105 GB,則伺服器將標記為唯讀。 或者,如果您預配了 5 GB 的存儲,則當可用儲存達到小於 512 MB 時,伺服器將標記為唯讀。
...
我們建議您打開儲存自動增長或設置警報,以便在伺服器存儲接近閾值時通知您,以便避免進入唯讀狀態。 如需詳細資訊，請參閱[如何設定警示](howto-alert-on-metric.md)的文件。

### <a name="storage-auto-grow"></a>儲存自動成長

存儲自動增長可防止伺服器耗盡存儲並變為唯讀。 如果啟用了存儲自動增長,則存儲會自動增長,而不會影響工作負載。 對於配置儲存小於 100 GB 的伺服器,一旦可用儲存低於 1 GB 或預配儲存的 10%, 則預配儲存大小將增加 5 GB。 對於具有 100 GB 以上預配儲存的伺服器,當可用儲存空間低於 10 GB 或預配儲存大小的 5% 時,預配儲存大小將增加 5%。 上述最大存儲限制適用。

例如,如果預配了 1000 GB 的儲存空間,並且實際利用率超過 950 GB,則伺服器存儲大小將增加到 1050 GB。 或者,如果您預配了 10 GB 的存儲,則當少於 1 GB 的存儲空間可用時,儲存大小將增加到 15 GB。
...
- 瞭解如何[在門戶中建立 PostgreSQL 伺服器](tutorial-design-database-using-azure-portal.md)。
