---
title: 適用于適用於 PostgreSQL 的 Azure 資料庫單一伺服器的 Azure 安全性基準
description: 適用于適用於 PostgreSQL 的 Azure 資料庫單一伺服器的 Azure 安全性基準
author: msmbaldwin
ms.service: security
ms.topic: conceptual
ms.date: 03/23/2020
...
適用于適用於 PostgreSQL 的 Azure 資料庫單一伺服器的 Azure 安全性基準包含可協助您改善部署安全性狀態的建議。
...
**指導**方針：為具有私人端點的適用於 PostgreSQL 的 Azure 資料庫設定私用連結。 Private Link 可讓您透過私人端點連線到 Azure 中的各種 PaaS 服務。 Azure 私用連結基本上會將 Azure 服務帶入您的私用虛擬網路（VNet）內。 您的虛擬網路與于 postgresql 實例之間的流量會傳送到 Microsoft 骨幹網路。

或者，您可以使用虛擬網路服務端點來保護和限制對您適用於 PostgreSQL 的 Azure 資料庫的執行的網路存取。 「虛擬網路規則」是一項防火牆功能，可控制適用於 PostgreSQL 伺服器的 Azure 資料庫是否接受虛擬網路中特定子網路所傳來的通訊。

您也可以使用防火牆規則來保護您的適用於 PostgreSQL 的 Azure 資料庫伺服器。 伺服器防火牆會防止對您資料庫伺服器的所有存取，直到您指定哪些電腦擁有許可權為止。 若要設定您的防火牆，您可以建立防火牆規則，指定可接受的 IP 位址範圍。 您可以在伺服器層級建立防火牆規則。

如何設定適用於 PostgreSQL 的 Azure 資料庫的私人連結：https://docs.microsoft.com/azure/postgresql/howto-configure-privatelink-portal

如何在適用於 PostgreSQL 的 Azure 資料庫中建立和管理 VNet 服務端點和 VNet 規則：https://docs.microsoft.com/azure/postgresql/howto-manage-vnet-using-portal

如何設定適用於 PostgreSQL 的 Azure 資料庫防火牆規則：https://docs.microsoft.com/azure/postgresql/howto-manage-firewall-using-portal
...
**指引**：當您的適用於 PostgreSQL 的 Azure 資料庫實例受到私人端點的安全保護時，您可以在相同的虛擬網路中部署虛擬機器。 您可以使用網路安全性群組（NSG）來降低資料外泄的風險。 啟用 NSG 流量記錄，並將記錄傳送到儲存體帳戶以進行流量審核。 您也可以將 NSG 流量記錄傳送到 Log Analytics 工作區，並使用「分析」來提供 Azure 雲端中流量的深入解析。 流量分析的一些優點是能夠將網路活動視覺化，並識別作用點、識別安全性威脅、瞭解流量模式，以及找出網路錯誤配置。
...
**指導**方針：使用適用於 PostgreSQL 的 Azure 資料庫的 Advanced 威脅防護。 先進的威脅防護會偵測異常活動，指出不尋常且可能有害的嘗試存取或惡意探索資料庫。

在與您適用於 PostgreSQL 的 Azure 資料庫實例相關聯的虛擬網路上啟用 DDoS 保護標準，以防範 DDoS 攻擊。 使用 Azure 資訊安全中心整合式威脅情報來拒絕與已知惡意或未使用的網際網路 IP 位址的通訊。

如何設定適用於 PostgreSQL 的 Azure 資料庫的先進威脅防護：https://docs.microsoft.com/azure/postgresql/howto-database-threat-protection-portal
...
**指引**：當您的適用於 PostgreSQL 的 Azure 資料庫實例受到私人端點的安全保護時，您可以在相同的虛擬網路中部署虛擬機器。 接著，您可以設定網路安全性群組（NSG），以降低資料外泄的風險。 啟用 NSG 流量記錄，並將記錄傳送到儲存體帳戶以進行流量審核。 您也可以將 NSG 流量記錄傳送到 Log Analytics 工作區，並使用「分析」來提供 Azure 雲端中流量的深入解析。 流量分析的一些優點是能夠將網路活動視覺化，並識別作用點、識別安全性威脅、瞭解流量模式，以及找出網路錯誤配置。
...
**指導**方針：對於需要存取適用於 PostgreSQL 的 Azure 資料庫實例的資源，請使用虛擬網路服務標籤來定義網路安全性群組或 Azure 防火牆上的網路存取控制。 建立安全性規則時，您可以使用服務標籤取代特定的 IP 位址。 藉由指定服務標記名稱（例如 SQL）。WestUs）在規則的適當 [來源] 或 [目的地] 欄位中，您可以允許或拒絕對應服務的流量。 Microsoft 會管理服務標籤所包含的位址前置詞，並隨著位址變更自動更新服務標記。

注意：適用於 PostgreSQL 的 Azure 資料庫使用 "Microsoft .Sql" 服務標記。

如需使用服務標記的詳細資訊：https://docs.microsoft.com/azure/virtual-network/service-tags-overview

瞭解適用於 PostgreSQL 的 Azure 資料庫的服務標記使用方式：https://docs.microsoft.com/azure/postgresql/concepts-data-access-and-security-vnet#terminology-and-description
...
**指導**方針：使用 Azure 原則定義和執行與您的適用於 PostgreSQL 的 Azure 資料庫實例相關聯的網路設定和網路資源的標準安全性設定。 使用 "DBforPostgreSQL" 和 "Microsoft" 命名空間中 Azure 原則別名來建立自訂原則，以審核或強制執行適用於 PostgreSQL 的 Azure 資料庫實例的網路設定。 您也可以使用與網路或適用於 PostgreSQL 的 Azure 資料庫實例相關的內建原則定義，例如：
...
**指導**方針：使用標記來取得與適用於 PostgreSQL 的 Azure 資料庫實例的網路安全性和流量流程相關的資源，以提供中繼資料和邏輯組織。
...
**指引**：使用 Azure 活動記錄來監視網路資源設定，並偵測與適用於 PostgreSQL 的 Azure 資料庫實例相關的網路資源變更。 在 Azure 監視器中建立警示，以在重大網路資源的變更發生時觸發。
...
**指引**： Microsoft 會維護 Azure 資源所使用的時間來源，例如記錄中時間戳記的適用於 PostgreSQL 的 Azure 資料庫。
...
**指引**：啟用診斷設定和伺服器記錄檔，並內嵌記錄來匯總適用於 PostgreSQL 的 Azure 資料庫實例所產生的安全性資料。 在 Azure 監視器中，使用 Log Analytics 工作區來查詢和執行分析，並將 Azure 儲存體帳戶用於長期/封存儲存體。 或者，您可以啟用和內部資料，以 Azure Sentinel 或協力廠商 SIEM。

如何設定和存取伺服器記錄以進行適用於 PostgreSQL 的 Azure 資料庫：https://docs.microsoft.com/azure/postgresql/howto-configure-server-logs-in-portal

如何設定及存取適用於 PostgreSQL 的 Azure 資料庫的 audit 記錄：https://docs.microsoft.com/azure/postgresql/concepts-audit
...
**指引**：在您的適用於 PostgreSQL 的 Azure 資料庫實例上啟用診斷設定，以存取 audit、security 和 resource 記錄。 請確定您已明確啟用于 postgresql Audit 記錄檔。 活動記錄（可自動取得）包含事件來源、日期、使用者、時間戳記、來源位址、目的地位址，以及其他實用的元素。 您也可以啟用 Azure 活動記錄診斷設定，並將記錄傳送到相同的 Log Analytics 工作區或儲存體帳戶。
...
**指引**：在 Azure 監視器中，針對用來保存適用於 PostgreSQL 的 Azure 資料庫記錄的 Log Analytics 工作區，請根據貴組織的合規性法規來設定保留週期。 針對長期/封存儲存體使用 Azure 儲存體帳戶。
...
**指引**：分析和監視來自適用於 PostgreSQL 的 Azure 資料庫實例的記錄，以瞭解異常行為。 使用 Azure 監視器的 Log Analytics 來審查記錄，並對記錄資料執行查詢。 或者，您可以啟用和內部資料，以 Azure Sentinel 或協力廠商 SIEM。
...
**指導**方針：啟用適用於 PostgreSQL 的 Azure 資料庫的 [先進的威脅防護]。 先進的威脅防護會偵測異常活動，指出不尋常且可能有害的嘗試存取或惡意探索資料庫。

此外，您可以啟用伺服器記錄和診斷設定，以于 postgresql 並將記錄傳送到 Log Analytics 工作區。 將您的 Log Analytics 工作區上架到 Azure Sentinel，因為它提供安全性協調流程自動化回應（攀升情況）解決方案。 這可讓您建立和使用腳本來修復安全性問題。

如何為適用於 PostgreSQL 的 Azure 資料庫啟用先進的威脅防護：https://docs.microsoft.com/azure/postgresql/howto-database-threat-protection-portal
...
**指導**方針：不適用;適用於 PostgreSQL 的 Azure 資料庫不會處理或產生與反惡意程式碼相關的記錄檔。
...
**指導**方針：不適用;適用於 PostgreSQL 的 Azure 資料庫不會處理或產生 DNS 相關的記錄。
...
**指引**：維護具有適用於 PostgreSQL 的 Azure 資料庫實例控制平面（例如 Azure 入口網站）之系統管理存取權的使用者帳戶清查。 此外，請維護可存取適用於 PostgreSQL 的 Azure 資料庫實例之資料平面（在資料庫本身內）的系統管理帳戶清查。 （建立于 postgresql 伺服器時，您會提供系統管理員使用者的認證。 此系統管理員可以用來建立其他于 postgresql 使用者）。

適用於 PostgreSQL 的 Azure 資料庫不支援內建的角色型存取控制，但是您可以根據特定的資源提供者作業來建立自訂角色。
...
**指導**方針： Azure Active Directory 和適用於 PostgreSQL 的 Azure 資料庫沒有預設密碼的概念。

在建立適用於 PostgreSQL 的 Azure 資料庫資源本身時，Azure 會強制建立具有強式密碼的系統管理使用者。 不過，一旦建立于 postgresql 實例，您就可以使用您所建立的第一個伺服器系統管理員帳戶來建立其他使用者，並授與其系統管理存取權。 建立這些帳戶時，請確定您為每個帳戶設定了不同的強式密碼。

如何建立適用於 PostgreSQL 的 Azure 資料庫的其他帳戶：https://docs.microsoft.com/azure/postgresql/howto-create-users

如何更新管理密碼：https://docs.microsoft.com/azure/postgresql/howto-create-manage-server-portal#update-admin-password
...
**指導**方針：使用可存取您適用於 PostgreSQL 的 Azure 資料庫實例的專用系統管理帳戶，建立相關的標準操作程式。 使用 Azure 資訊安全中心的身分識別與存取管理來監視系統管理帳戶的數目。 

瞭解 Azure 資訊安全中心身分識別和存取：https://docs.microsoft.com/azure/security-center/security-center-identity-access 

瞭解如何在適用於 PostgreSQL 的 Azure 資料庫中建立系統管理員使用者：https://docs.microsoft.com/azure/postgresql/howto-create-users#the-server-admin-account
...
**指引**.. 登入適用於 PostgreSQL 的 Azure 資料庫的支援方式是使用直接在資料庫中設定的使用者名稱/密碼，以及使用 AZURE ACTIVE DIRECTORY （AD）身分識別並利用 Azure AD token 來連接。 使用 Azure AD token 時，支援不同的方法，例如 Azure AD 使用者、Azure AD 群組，或連接到資料庫的 Azure AD 應用程式。

于 postgresql 的控制平面存取會分別透過 REST API 提供，並支援 SSO。 若要進行驗證，請將要求的授權標頭設定為您從 Azure Active Directory 取得的 JSON Web 權杖。

使用 Azure Active Directory 以適用於 PostgreSQL 的 Azure 資料庫進行驗證：https://docs.microsoft.com/azure/postgresql/howto-configure-sign-in-aad-authentication

瞭解適用於 PostgreSQL 的 Azure 資料庫 REST API：https://docs.microsoft.com/rest/api/postgresql/
...
**指導**方針：為適用於 PostgreSQL 的 Azure 資料庫啟用 [先進的威脅防護]，以產生可疑活動的警示。
...
若要登入適用於 PostgreSQL 的 Azure 資料庫，建議使用 Azure AD，並使用 Azure AD token 來連接。 使用 Azure AD token 時，支援不同的方法，例如 Azure AD 使用者、Azure AD 群組，或連接到資料庫的 Azure AD 應用程式。

Azure AD 認證也可以用於管理平面層級（例如 Azure 入口網站）的系統管理，以控制于 postgresql 的系統管理員帳戶。
...
**指導**方針：請參閱 Azure Active Directory 記錄，以協助探索過時的帳戶，其中可能包含具有適用於 PostgreSQL 的 Azure 資料庫系統管理角色的帳戶。 此外，您可以使用 Azure 身分識別存取審查來有效率地管理群組成員資格、存取可用來存取適用於 PostgreSQL 的 Azure 資料庫的企業應用程式，以及角色指派。 應定期檢查使用者存取，例如每隔90天，以確保只有適當的使用者可以繼續存取。
...
**指導**方針：啟用適用於 PostgreSQL 的 Azure 資料庫和 Azure Active Directory 的診斷設定，並將所有記錄傳送到 Log Analytics 工作區。 在 Log Analytics 中設定所需的警示（例如失敗的驗證嘗試）。
...
如何觀看 Azure AD 有風險的登入：https://docs.microsoft.com/azure/active-directory/reports-monitoring/concept-risk-events
...
**指導**方針：目前無法使用;適用於 PostgreSQL 的 Azure 資料庫尚不支援客戶加密箱。
...
**指引**：使用標記來協助追蹤適用於 PostgreSQL 的 Azure 資料庫實例或儲存或處理敏感資訊的相關資源。
...
**指引**：在開發、測試和生產環境中，執行不同的訂用帳戶和/或管理群組。 使用私人連結、服務端點和（或）防火牆規則的組合，來隔離及限制適用於 PostgreSQL 的 Azure 資料庫實例的網路存取。
...
如何設定適用於 PostgreSQL 的 Azure 資料庫防火牆規則：https://docs.microsoft.com/azure/postgresql/concepts-firewall-rules
...
**指引**：使用 Azure 虛擬機器來存取適用於 PostgreSQL 的 Azure 資料庫實例時，請使用私人連結、于 postgresql 網路設定、網路安全性群組和服務標籤，以降低資料外泄的可能性。

Microsoft 會管理適用於 PostgreSQL 的 Azure 資料庫的基礎結構，並已實行嚴格的控制，以避免遺失或公開客戶資料。

如何緩和適用於 PostgreSQL 的 Azure 資料庫的資料外泄：https://docs.microsoft.com/azure/postgresql/concepts-data-access-and-security-private-link
...
如何在傳輸中設定適用於 PostgreSQL 的 Azure 資料庫的加密：https://docs.microsoft.com/azure/postgresql/concepts-ssl-connection-security
...
**指導**方針：適用於 PostgreSQL 的 Azure 資料庫尚無法使用資料識別、分類和遺失防護功能。 視需要執行協力廠商解決方案，以符合合規性目的。
...
**指引**：使用 Azure 角色型存取控制（RBAC）來控制適用於 PostgreSQL 的 Azure 資料庫控制平面（例如 Azure 入口網站）的存取權。 對於資料平面存取（在資料庫本身），請使用 SQL 查詢來建立使用者並設定使用者權限。 RBAC 不會影響資料庫內的使用者權限。

如何在 Azure 中設定 RBAC：https://docs.microsoft.com/azure/role-based-access-control/role-assignments-portal

如何使用 SQL 為適用於 PostgreSQL 的 Azure 資料庫設定使用者存取權：https://docs.microsoft.com/azure/postgresql/howto-create-users
...
**指導**方針：適用於 PostgreSQL 的 Azure 資料庫服務會使用 FIPS 140-2 驗證的密碼編譯模組，以進行待用資料的儲存體加密。 包含備份在內的資料會在磁片上進行加密，但執行查詢時所建立的暫存檔案除外。 該服務使用包含在 Azure 儲存體加密中的 AES 256 位元加密，且金鑰是由系統進行管理。 儲存體加密會一律啟用，且無法停用。

適用於 PostgreSQL 的 Azure 資料庫單一伺服器的資料加密與客戶管理的金鑰（CMK），可讓您攜帶自己的金鑰（BYOK）來保護待用資料。 此時，您必須要求存取權才能使用這項功能。 若要這麼做，請聯絡：

AskAzureDBforPostgreSQL@service.microsoft.com.

瞭解適用於 PostgreSQL 的 Azure 資料庫的待用加密：https://docs.microsoft.com/azure/postgresql/concepts-security

瞭解使用客戶管理的金鑰進行適用於 PostgreSQL 的 Azure 資料庫的待用加密：https://docs.microsoft.com/azure/postgresql/concepts-data-encryption-postgresql
...
**指導**方針：使用 Azure 監視器搭配 Azure 活動記錄，以針對適用於 PostgreSQL 的 Azure 資料庫的生產實例和其他重要或相關資源進行變更時，建立警示。
...
**指導**方針：目前無法使用;Azure 資訊安全中心尚未支援適用於 PostgreSQL 的 Azure 資料庫的弱點評估。
...
**指導**方針： Microsoft 會在支援適用於 PostgreSQL 的 Azure 資料庫的基礎系統上執行弱點管理。
...
**指引**：使用 Azure Resource Graph 來查詢和探索訂用帳戶內的所有資源（包括適用於 PostgreSQL 的 Azure 資料庫實例）。 請確定您的租使用者中有適當的（讀取）許可權，而且能夠列舉所有的 Azure 訂用帳戶以及您訂用帳戶內的資源。
...
**指引**：將標籤套用至適用於 PostgreSQL 的 Azure 資料庫實例和其他相關資源，以邏輯方式將它們組織成分類法。
...
**指引**：適當地使用標記、管理群組和個別訂用帳戶來組織和追蹤適用於 PostgreSQL 的 Azure 資料庫實例和相關資源。 定期協調清查，並確保未經授權的資源會及時從訂用帳戶中刪除。
...
**指引**：使用 Azure 條件式存取來限制使用者與 Azure Resource Manager 的互動能力，方法是設定「Microsoft Azure 管理」應用程式的「封鎖存取」。 這可能會防止在高安全性環境（例如包含機密資訊的適用於 PostgreSQL 的 Azure 資料庫實例）中建立和變更資源。
...
**指導**方針：使用 "DBforPostgreSQL" 命名空間中的 Azure 原則別名來建立自訂原則，以警示、審查和強制執行系統組態。 此外，開發處理常式和管線來管理原則例外狀況。
...
**指引**：針對在用來存取適用於 PostgreSQL 的 Azure 資料庫實例 Azure App Service 上執行的 Azure 虛擬機器或 web 應用程式，請搭配使用受控服務識別與 Azure Key Vault 以簡化和保護適用於 PostgreSQL 的 Azure 資料庫秘密管理。 請確定已啟用 Key Vault 虛刪除。
...
**指導**方針：適用於 PostgreSQL 的 Azure 資料庫 server 支援 Azure Active Directory authentication （預覽版）來存取資料庫。  建立適用於 PostgreSQL 的 Azure 資料庫伺服器時，您會提供系統管理員使用者的認證。 此系統管理員可以用來建立額外的資料庫使用者。  

針對在用來存取適用於 PostgreSQL 的 Azure 資料庫伺服器的 Azure App Service 上執行的 Azure 虛擬機器或 web 應用程式，請使用受控服務識別搭配 Azure Key Vault 來儲存和取出適用於 PostgreSQL 的 Azure 資料庫伺服器的認證。 請確定已啟用 Key Vault 虛刪除。
...
**指導**方針：支援 Azure 服務的基礎主機（例如適用於 PostgreSQL 的 Azure 資料庫）已啟用 Microsoft 反惡意程式碼，但不會在客戶內容上執行。

預先掃描要上傳至非計算 Azure 資源的任何內容，例如 App Service、Data Lake Storage、Blob 儲存體、適用於 PostgreSQL 的 Azure 資料庫等等。Microsoft 無法存取您在這些實例中的資料。
...
支援 Azure 服務的基礎主機（例如適用於 PostgreSQL 的 Azure 資料庫）會啟用 Microsoft 反惡意程式碼，但不會在客戶內容上執行。
...
**指導**方針：適用於 PostgreSQL 的 Azure 資料庫取得資料檔案和交易記錄檔的備份。 視支援的最大儲存體大小而定，我們會採用完整和差異備份（4 TB 的儲存體伺服器）或快照集備份（最多 16 TB 的儲存體伺服器）。 在您設定的備份保留期限內，這些備份可讓您將伺服器還原至任何時間點。 預設的備份保留期限是七天。 可選擇設定的期限最多為 35 天。 所有備份皆會使用 AES 256 位元加密進行加密。

如何在適用於 PostgreSQL 的 Azure 資料庫中備份伺服器：https://docs.microsoft.com/azure/postgresql/howto-restore-server-portal

瞭解適用於 PostgreSQL 的 Azure 資料庫初始設定：https://docs.microsoft.com/azure/postgresql/tutorial-design-database-using-azure-portal
...
**指導**方針：適用於 PostgreSQL 的 Azure 資料庫會根據使用者的選擇，自動建立伺服器備份，並將它們儲存在本機-多餘或異地多餘的儲存體中。 備份可以用來將伺服器還原至某個時間點。 備份和還原可保護資料免於意外損毀或刪除，是商務持續性策略中不可或缺的一部分。

如果使用 Azure Key Vault 來儲存適用於 PostgreSQL 的 Azure 資料庫實例的認證，請確保金鑰的定期自動備份。
...
**指引**：在適用於 PostgreSQL 的 Azure 資料庫中，執行還原會從源伺服器的備份建立新的伺服器。 有兩種可用的還原類型：時間點還原和異地還原。 時間點還原可搭配任一備份備援選項使用，並在您原始伺服器所在區域中建立新的伺服器。 異地還原只能您將伺服器設定為使用異地備援儲存體時使用，這可讓您將伺服器還原至不同的區域。

預估的復原時間取決於數個因素，包括資料庫大小、交易記錄大小、網路頻寬，以及在相同區域中同時進行復原的資料庫總數。 復原時間通常不到 12 小時。

定期測試適用於 PostgreSQL 的 Azure 資料庫實例的還原。
...
**指導**方針：適用於 PostgreSQL 的 Azure 資料庫會取得完整、差異和交易記錄備份。 在您設定的備份保留期限內，這些備份可讓您將伺服器還原至任何時間點。 預設的備份保留期限是七天。 可選擇設定的期限最多為 35 天。 所有備份皆會使用 AES 256 位元加密進行加密。

瞭解適用於 PostgreSQL 的 Azure 資料庫中的備份和還原：https://docs.microsoft.com/azure/postgresql/concepts-backup
