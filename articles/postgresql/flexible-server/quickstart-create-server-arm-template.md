...
Azure Resource Manager 是 Azure 的部署和管理服務。 其提供管理層，可讓您建立、更新和刪除您 Azure 帳戶中的資源。 您可以使用存取控制、鎖定和標記等管理功能，在部署後保護及組織您的資源。 若要了解 Azure Resource Manager 範本，請參閱[範本部署概觀](../../azure-resource-manager/templates/overview.md)。
...
適用於 PostgresSQL 的 Azure 資料庫伺服器是區域內一或多個資料庫的父資源， 提供適用於其資料庫的管理原則範圍︰登入、防火牆、使用者、角色和組態。
...
1. 在資料庫清單中，選取您的新伺服器以檢視 **概觀** 頁面來管理伺服器。

# <a name="powershell"></a>[PowerShell](#tab/PowerShell)

您必須輸入新伺服器的名稱，才能檢視適用於 PostgreSQL 的 Azure 資料庫彈性伺服器詳細資料。
...
您必須輸入新伺服器的名稱和資源群組，才能檢視適用於 PostgreSQL 的 Azure 資料庫彈性伺服器詳細資料。
