...
安裝及設定程序可能需要大約 10 分鐘的時間才能完成。 本文中的資訊僅適用於部署 StorSimple Virtual Array。 關於 StorSimple 8000 系列裝置的部署，請參閱︰[部署執行 Update 2 的 StorSimple 8000 系列裝置](./storsimple-8000-deployment-walkthrough-u2.md)。
...
10. (可省略) 設定裝置的時間設定，例如時區，以及主要和次要 NTP 伺服器。 NTP 伺服器是必要的，因為您的裝置必須讓時間同步，才能與您的雲端服務提供者進行驗證。
    ...
    ![[時間設定] 頁面的螢幕擷取畫面。 時區和主要 N T P 伺服器會填入。 次要 N T P 伺服器是空白的。](./media/storsimple-virtual-array-deploy3-fs-setup/image10.png)
...
3. 按一下命令列上的 [設定]。 這會開啟 [設定] 刀鋒視窗。 在 [設定] 刀鋒視窗中，執行下列動作：
   ...
   4. 按一下 [進行必要的設定]，以指定要用於裝置的儲存體帳戶認證。 如果未設定儲存體帳戶認證，請按一下 [新增]。 **確定您使用的儲存體帳戶支援區塊 blob。不支援分頁 blob。** 關於 [區塊 Blob 和分頁 Blob](/rest/api/storageservices/understanding-block-blobs--append-blobs--and-page-blobs) 的其他資訊。
