---
title: 管理存儲帳戶憑據，StorSimple 8000 系列設備
...
* **名稱** – 建立時指派給帳戶的唯一名稱。
* **啟用 SSL** = 是否啟用 TLS，以及設備到雲的通信是否通過安全通道。
* **使用者** – 使用儲存體帳戶的磁碟區數目。
...
* **服務訂用帳戶中的儲存體帳戶** – 這些帳戶是 Azure 儲存體帳戶，與和服務相同的訂用帳戶相關聯。 若要深入了解如何建立這些儲存體帳戶，請參閱 [關於 Azure 儲存體帳戶](../storage/common/storage-create-storage-account.md)。 
* **服務訂用帳戶以外的儲存體帳戶** – 這些是與您的服務沒有相關聯，且可能在服務建立之前即存在的 Azure 儲存體帳戶。

## <a name="add-a-storage-account"></a>新增儲存體帳戶

您可以提供唯一的易記名稱以及與儲存體帳戶(搭配指定的雲端服務提供者) 連結的存取認證來新增儲存體帳戶。 您還可以選擇啟用傳輸層安全 （TLS） 模式（以前稱為安全通訊端層 （SSL） 模式，以創建設備與雲之間的網路通信安全通道。
...
4. 在 [新增儲存體帳戶認證]**** 刀鋒視窗中，執行下列步驟︰
    ...
    4. 要啟用 TLS，請按一下"**啟用**"按鈕，為 StorSimple 裝置管理員服務和雲之間的網路通信創建安全通道。 只有當您在私人雲端內操作時，才按一下 [停用]**** 按鈕。
...
3. 當提示確認時，按一下"**是**"繼續刪除。 表格式清單會更新以反映所做的變更。
