...
| 元件 | Module | 類型 | 位置 | 現場可更換單位 (FRU)？ | 描述 |
| --- | --- | --- | --- | --- | --- |
| 插槽 [0-11] 中的磁碟機 |磁碟機 |實體 |Shared |是 |主要機箱中的每個 SSD 或 HDD 磁碟機都表示為一行。 |
| 周圍溫度感應器 |機箱 |實體 |Shared |否 |測量底座內溫度。 |
| 中間板溫度感應器 |機箱 |實體 |Shared |否 |測量中間板的溫度。 |
| 有聲警報器 |機箱 |實體 |Shared |否 |指出底座內的有聲警報器子系統是否正常運作。 |
| 機箱 |機箱 |實體 |Shared |是 |指出底座的目前狀態。 |
| 機箱設定 |機箱 |實體 |Shared |否 |指底座的前板。 |
| 電源線電壓感應器 |PCM |實體 |Shared |否 |許多電源線電壓感應器都會顯示其狀態，指出測量的電壓是否在容許範圍內。 |
| 電源線電流感應器 |PCM |實體 |Shared |否 |許多電源線電流感應器都會顯示其狀態，指出測量的電流是否在容許範圍內。 |
| PCM 中的溫度感應器 |PCM |實體 |Shared |否 |許多溫度感應器 (例如入口和熱點感應器) 都會顯示其狀態，指出測量的溫度是否在容許範圍內。 |
| 電源供應器 [0-1] |PCM |實體 |Shared |是 |位於裝置背面的兩個 PCM 中的每個電源供應器都表示為一行。 |
| 冷卻 [0-1] |PCM |實體 |Shared |是 |位於兩個 PCM 中的四部冷卻風扇都表示為一行。 |
| 電池 [0-1] |PCM |實體 |Shared |是 |位於 PCM 中的每個備份電池模組都表示為一行。 |
| Metis |N/A |邏輯 |Shared |N/A |顯示電池狀態：是否需要充電和生命週期是否即將結束。 |
| 叢集 |N/A |邏輯 |Shared |N/A |顯示兩個整合式控制器模組之間建立的叢集狀態。 |
| 叢集節點 |N/A |邏輯 |Shared |N/A |指出做為叢集一部分的控制器狀態。 |
| 叢集仲裁 |N/A |邏輯 | |N/A |指出 HDD 儲存體集區中大部分磁碟成員資格的目前狀態。 |
| HDD 資料空間 |N/A |邏輯 |Shared |N/A |儲存空間可用於硬碟 (HDD) 儲存體集區中的資料。 |
| HDD 管理空間 |N/A |邏輯 |Shared |N/A |管理工作的 HDD 儲存體集區中保留的空間。 |
| HDD 仲裁空間 |N/A |邏輯 |Shared |N/A |叢集仲裁的 HDD 儲存體集區中保留的空間。 |
| HDD 更換空間 |N/A |邏輯 |Shared |N/A |控制器更換的 HDD 儲存體集區中保留的空間。 |
| SSD 資料空間 |N/A |邏輯 |Shared |N/A |儲存空間可用於固態硬碟 (SSD) 存放集區中的資料。 |
| SSD NVRAM 空間 |N/A |邏輯 |Shared |N/A |SSD 儲存體集區中 NVRAM 邏輯專用的儲存空間。 |
| HDD 儲存體集區 |N/A |邏輯 |Shared |N/A |顯示從裝置 HDD 建立之邏輯儲存體集區的狀態。 |
| SSD 儲存體集區 |N/A |邏輯 |Shared |N/A |顯示從裝置 SSD 建立之邏輯儲存體集區的狀態。 |
...
| 元件 | Module | 類型 | 位置 | FRU？ | 描述 |
| --- | --- | --- | --- | --- | --- |
| 插槽 [0-11] 中的磁碟機 |磁碟機 |實體 |Shared |是 |EBOD 機箱前方的每個 HDD 磁碟機都表示為一行。 |
| 周圍溫度感應器 |機箱 |實體 |Shared |否 |測量底座內溫度。 |
| 中間板溫度感應器 |機箱 |實體 |Shared |否 |測量中間板的溫度。 |
| 有聲警報器 |機箱 |實體 |Shared |否 |指出底座內的有聲警報器子系統是否正常運作。 |
| 機箱 |機箱 |實體 |Shared |是 |指出底座的目前狀態。 |
| 機箱設定 |機箱 |實體 |Shared |否 |指 OPS 或底座的前板。 |
| 電源線電壓感應器 |PCM |實體 |Shared |否 |許多電源線電壓感應器都會顯示其狀態，指出測量的電壓是否在容許範圍內。 |
| 電源線電流感應器 |PCM |實體 |Shared |否 |許多電源線電流感應器都會顯示其狀態，指出測量的電流是否在容許範圍內。 |
| PCM 中的溫度感應器 |PCM |實體 |Shared |否 |許多溫度感應器 (例如入口和熱點感應器) 都會顯示其狀態，其指出測量的溫度是否在容許範圍內。 |
| 電源供應器 [0-1] |PCM |實體 |Shared |是 |位於裝置背面的兩個 PCM 中的每個電源供應器都表示為一行。 |
| 冷卻 [0-1] |PCM |實體 |Shared |是 |位於兩個 PCM 中的四部冷卻風扇都表示為一行。 |
| 本機儲存體 [HDD] |N/A |邏輯 |Shared |N/A |顯示從裝置 HDD 建立之邏輯儲存體集區的狀態。 |
