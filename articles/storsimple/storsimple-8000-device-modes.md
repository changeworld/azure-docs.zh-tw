...
> [!NOTE]
> **僅在正常運行的設備上支援維護模式。在一個或兩個控制器無法正常工作的設備上不支援它。**
...
如果您的系統進入復原模式，您應該連絡 Microsoft 支援服務來請示後續步驟。 有關詳細資訊，請轉到聯繫[Microsoft 支援](storsimple-8000-contact-microsoft-support.md)。
...
> [!IMPORTANT]
> 進入維護模式之前，請在 Azure 入口網站中，存取您裝置的 [裝置設定] > [硬體健康狀態]****，以確認兩個裝置控制器的健康情況良好。 如果一個或兩個控制器的健康情況不好，請連絡 Microsoft 支援服務以進行後續步驟。 有關詳細資訊，請轉到聯繫[Microsoft 支援](storsimple-8000-contact-microsoft-support.md)。
