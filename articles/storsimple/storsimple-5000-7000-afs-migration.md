...
在這裡，您將找到舊版 5000 或 7000 系列設備的遷移先決條件到 Azure 檔同步。開始之前，請確保您具有：
...
- 擁有者級對將用於部署 Azure 檔同步的 Azure 訂閱的存取權限。如果沒有擁有者或管理員級別許可權，則為同步組創建雲終結點時可能會遇到問題。
...
1.  在掛接 StorSimple 磁碟區的相同 Windows Server 主機上執行下列步驟，或以不同的系統執行。 
    - [準備 Windows 伺服器與 Azure 檔同步一起使用](https://docs.microsoft.com/azure/storage/files/storage-sync-files-deployment-guide#prepare-windows-server-to-use-with-azure-file-sync)。
    - [安裝 Azure 檔同步代理](https://docs.microsoft.com/azure/storage/files/storage-sync-files-deployment-guide#install-the-azure-file-sync-agent)。
    - [部署存儲同步服務](https://docs.microsoft.com/azure/storage/files/storage-sync-files-deployment-guide#deploy-the-storage-sync-service)。 
    - [使用存儲同步服務註冊 Windows 伺服器](https://docs.microsoft.com/azure/storage/files/storage-sync-files-deployment-guide#register-windows-server-with-storage-sync-service)。 
...
2.  等候檔案伺服器同步完成。 針對給定同步群組中的每個伺服器，請確定：
    ...
    - 上傳和下載的 [檔案無法同步]**** 皆為 0。
    有關伺服器同步何時完成的詳細資訊，請訪問["對 Azure 檔同步進行故障排除](https://docs.microsoft.com/azure/storage/files/storage-sync-files-troubleshoot?tabs=portal1%2Cportal#how-do-i-know-if-my-servers-are-in-sync-with-each-other)"。同步可能需要幾個小時到幾天，具體取決於您的資料大小和頻寬。 同步完成後，您所有的資料都會安全地放在 Azure 檔案共用中。 
