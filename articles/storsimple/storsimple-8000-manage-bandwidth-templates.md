...
**問題**. 當您在排程之間時，頻寬控制會發生什麼事？ (一個排程已結束而另一個排程尚未開始)。

**A**. 在這種情況下，不會採用任何頻寬控制。 這表示將資料分層至雲端時，裝置可以使用無限制的頻寬。

**問題**. 您是否可以修改離線裝置上的頻寬範本？

**A**. 如果對應的裝置已離線，您無法修改磁碟區容器上的頻寬範本。

**問題**. 您是否可以在相關聯的磁碟區離線時，編輯和磁碟區容器相關聯的頻寬範本？
...
**問題**. 要是裝置上多個磁碟區容器的排程重疊，但是每個排程都套用不同的限制會發生什麼事？
