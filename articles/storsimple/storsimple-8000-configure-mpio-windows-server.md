...
Microsoft 為 Windows Server 中的多重路徑 I/O (MPIO) 功能建立支援，協助建立高可用性、容錯的 iSCSI 網路組態。 MPIO 使用備援實體路徑元件 — 配接器、纜線以及交換器 — 以在伺服器與存放裝置之間建立邏輯路徑。 如果元件故障而導致邏輯路徑失敗，多重路徑邏輯使用替代的 I/O 路徑，讓應用程式仍然可以存取其資料。 此外，依照您的設定，MPIO 也能藉由重新平衡所有路徑的負載來改善效能。 有關詳細資訊，請參閱[MPIO 概述](https://technet.microsoft.com/library/cc725907.aspx "MPIO 概述和功能")。
...
3. 在 [新增角色及功能]**** 精靈中，執行下列步驟：
   ...
   6. 在 **"確認安裝選擇"** 頁上，確認選擇，然後**根據需要自動選擇"重新開機目標伺服器**"，如下所示。 按一下 **[安裝]**。
...
2. 在 [MPIO 屬性]**** 對話方塊中，選取 [探索多重路徑]**** 索引標籤。
3. 選取 [新增 iSCSI 裝置的支援]****，然後按一下 [新增]****。  
   ![MPIO 內容探索多重路徑](./media/storsimple-configure-mpio-windows-server/IC741003.png)
4. 當系統提示您將伺服器重新開機。
5. 在**MPIO 屬性**對話方塊中，按一下**MPIO 設備**選項卡。按一下"**添加**"。
...
5. 選取 [iSCSI 啟動器屬性]**** 對話方塊中的 [目標]**** 索引標籤。 您應該會在 [探索到的目標] **** 下看到 StorSimple 裝置目標 IQN。
...
7. 在 [連接到目標]**** 對話方塊中，選取 [啟用多重路徑]**** 核取方塊。 按一下 [進階]****。
...
11. 在 [進階設定]**** 對話方塊中：
    ...
    1. 在**本地配接器**下拉清單中，選擇 Microsoft iSCSI 開始器。
    2. 在 [啟動器 IP]**** 下拉式清單中，選取對應至主機的 IP 位址。 在此情況下，您是將裝置上的兩個網路介面連線到主機上的單一網路介面。 因此，這個介面會和第一個工作階段提供的相同。
...
14. 在 [磁碟管理]**** 下，以滑鼠右鍵按一下 [磁碟]****，然後選取 [屬性]****。
15. 在 StorSimple 型號 #### [多重路徑磁碟裝置屬性]**** 對話方塊中，按一下 [MPIO]**** 索引標籤。
    
    ![StorSimple 8100 多重路徑磁碟 DeviceProp。](./media/storsimple-configure-mpio-windows-server/IC741009.png)
16. 在 [DSM 名稱]**** 區段中按一下 [詳細資料]****，確認參數已設為預設參數。 預設參數如下：
...
> [!IMPORTANT]
> **我們建議您不要混合 1 GbE 和 10 GbE 網路介面。如果使用兩個網路介面，則兩個介面的類型應相同。**
...
3. 按一下 [確定]****，返回 [iSCSI 啟動器屬性]**** 對話方塊。
4. 在 [iSCSI 啟動器屬性]**** 對話方塊中，選取 [目標]**** 索引標籤、反白顯示探索到的目標，然後按一下 [連接]****。 [連接到目標]**** 對話方塊隨即出現。
5. 在 [連接到目標]**** 對話方塊中：
...
8. 在 [連接到目標]**** 對話方塊中，選取 [啟用多重路徑]**** 核取方塊，然後按一下 [進階]****。
9. 在 [進階設定]**** 對話方塊中：
   
   1. 在 [本機介面卡]**** 下拉式清單中，選取 [Microsoft iSCSI 啟動器]****。
   2. 在 [啟動器 IP]**** 下拉式清單中，選取對應到主機上第二個 iSCSI 介面的 IP 位址。
   3. 在 [目標入口 IP] **** 下拉式清單中，為裝置上啟用的第二個資料介面選取 IP 位址。
   4. 按一下 [確定]****，返回 [iSCSI 啟動器屬性]**** 對話方塊。 您現已完成將第二個工作階段新增到目標。
10. 重複步驟 8-10，將其他工作階段 (路徑) 新增到目標。 主機上有兩個介面且裝置上也有兩個，您總共可以新增四個工作階段。
11. 新增所需的工作階段 (路徑) 後，在 [iSCSI 啟動器屬性]**** 對話方塊中選取目標，然後按一下 [屬性]****。 在 [內容] **** 對話方塊的 [工作階段] 索引標籤中，針對可能的路徑排列組合記下其對應的四個工作階段識別碼。 若要取消工作階段，請選取工作階段識別碼旁的核取方塊，然後按一下 [中斷連線]****。
12. 要查看會話中顯示的設備，請選擇"**設備**"選項卡。要為所選設備配置 MPIO 策略，請按一下**MPIO**。 [裝置詳細資料]**** 對話方塊便會隨即出現。 在 [MPIO]**** 索引標籤上，您可以選取適當的 [負載平衡原則]**** 設定。 您也可以檢視 [作用中]**** 或 [備用]**** 路徑類型。
