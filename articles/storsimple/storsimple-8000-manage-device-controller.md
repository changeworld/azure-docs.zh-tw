...
**A.** 如果裝置上的兩個控制器皆狀況良好且已開啟，您會收到確認提示。 您可以選擇：
...
**A.** 您可以在 [控制器] 刀鋒視窗上檢查控制器狀態。 控制器狀態會指出控制器是否正在重新啟動或關閉。 此外，如果控制器已重新啟動或關閉，[警示]**** 刀鋒視窗會包含資訊警示。 控制器重新啟動和關閉作業也會記錄在活動記錄中。 如需有關活動記錄的詳細資訊，請移至[檢視活動記錄](storsimple-8000-service-dashboard.md#view-the-activity-logs)。

**問。** 控制器容錯移轉會不會對 I/O 造成任何影響？

**A.**  啟動器和主動控制器之間的 TCP 連接將會因為控制器容錯移轉而重設，但會在被動控制器繼續作業時重新建立。 在這項作業的過程中，啟動器與裝置之間的 I/O 活動中可能會有暫時的 (少於 30 秒) 暫停。

**問。**  如何在控制器關閉並遭移除後，將控制器傳回給服務？

**A.** 若要將控制器傳回給服務，您必須依照 [更換 StorSimple 裝置上的控制器模組](storsimple-8000-controller-replacement.md)。
