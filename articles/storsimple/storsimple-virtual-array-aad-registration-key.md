...
| 如果您的裝置正在執行  | 請採取下列動作                                    |
|----------------------------|--------------------------------------------------------------|
| Update 1.0 或更新版本，且裝置已離線。 <br> 您會看到表示 URL 未列入允許清單的警示。| 1. 修改防火牆規則以包括身份驗證 URL。 請參閱[驗證 URL](#url-changes-for-aad-authentication)。 <br> 2.[從服務獲取 AAD 註冊金鑰](#aad-based-registration-keys)。 <br> 3. 執行步驟 1-5 以[連接到虛擬陣列的 Windows PowerShell 介面](storsimple-virtual-array-deploy2-provision-hyperv.md#step-2-provision-a-virtual-array-in-hypervisor)。<br> 4.`Invoke-HcsReRegister`使用 Cmdlet 通過 Windows PowerShell 註冊設備。 提供您在上一個步驟中取得的金鑰。|
| Update 1.0 或更新版本，且裝置已上線。| 您不需要執行任何動作。                                       |
| Update 0.6 或更早版本，且裝置已離線。 | 1.[通過目錄伺服器下載更新 1.0](storsimple-virtual-array-install-update-1.md#download-the-update-or-the-hotfix)。<br>2.[通過本地 Web UI 應用更新 1.0](storsimple-virtual-array-install-update-1.md#install-the-update-or-the-hotfix)。<br>3.[從服務獲取 AAD 註冊金鑰](#aad-based-registration-keys)。 <br>4. 執行步驟 1-5 以[連接到虛擬陣列的 Windows PowerShell 介面](storsimple-virtual-array-deploy2-provision-hyperv.md#step-2-provision-a-virtual-array-in-hypervisor)。<br>5.`Invoke-HcsReRegister`使用 Cmdlet 通過 Windows PowerShell 註冊設備。 提供您在上一個步驟中取得的金鑰。|
