...
| 警示文字 | 事件 | 詳細資訊 / 建議的動作 |
|:--- |:--- |:--- |
| 復原作業無法還原此服務的所有設定。 部分裝置的裝置設定資料處於不一致的狀態。 |災害復原之後偵測到資料不一致。 |服務上與裝置上加密的資料不同步。 從 StorSimple 裝置管理員授權裝置 <裝置名稱> 以啟動同步處理程序。 使用 Windows PowerShell Interface for StorSimple 在裝置 <裝置名稱>`Restore-HcsmEncryptedServiceData`*Cmdlet 上執行*，並提供舊密碼作為此 Cmdlet 的輸入，以還原安全性設定檔。 接著執行 `Invoke-HcsmServiceDataEncryptionKeyChange` Cmdlet 以更新服務資料加密金鑰。 採取適當的動作之後，請從警示頁面清除此警示。 |
