...
您可以提供唯一的易記名稱和連結至儲存體帳戶的存取認證，將儲存體帳戶認證新增至 StorSimple 裝置管理員服務設定。 您還可以選擇啟用傳輸層安全 （TLS） 模式（以前稱為安全通訊端層 （SSL） 模式，以創建設備與雲之間的網路通信安全通道。
...
您可以編輯裝置所使用的儲存體帳戶認證。 如果編輯當前正在使用的存儲帳戶憑據，則可供修改的欄位是存儲帳戶憑據的訪問金鑰和 TLS 模式。 您可以提供新的儲存體存取金鑰，或是修改 [啟用 SSL 模式] **** 的選取項目，並儲存已更新的設定。
...
4. 在儲存體帳戶認證的 [屬性]**** 刀鋒視窗中，執行下列動作︰
   ...
   1. 按一下 [刪除] **** 來刪除認證。
   2. 當提示確認時，按一下"**是**"繼續刪除。 表格式清單會更新以反映變更。
