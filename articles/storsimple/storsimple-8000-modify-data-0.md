...
1. 在序列主控台功能表中，選擇選項 1 ([以完整存取權限登入])。 出現提示時，請提供 **裝置系統管理員密碼**。 預設密碼為 `Password1`。
2. 在命令提示字元中，輸入：
...
1. 在序列主控台功能表中，選擇選項 1 ([以完整存取權限登入])。 出現提示時，提供裝置系統管理員密碼。 預設密碼為 `Password1`。
2. 在命令提示字元中，輸入：
   ...
   * 控制器 1 的固定的 IPv4 位址
     ...
     如需使用此 Cmdlet 的詳細資訊，請移至 [Windows PowerShell for StorSimple Cmdlet 參考](/previous-versions/windows/powershell-scripting/dn688161(v=wps.630))。

## <a name="next-steps"></a>後續步驟
* 若要設定 DATA 0 以外的網路介面，您可以使用[在 Azure 入口網站中設定網路設定](storsimple-8000-modify-device-config.md)。 
* 如果您在設定您的網路介面時遇到任何問題，請參閱 [疑難排解部署問題](./storsimple-8000-troubleshoot-deployment.md)。