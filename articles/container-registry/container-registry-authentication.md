...
直接與您的登錄搭配運作時（例如將映射提取至您建立的映射，並將其從開發工作站推送至您所建立的登錄）時，請使用您的個別 Azure 身分識別進行驗證。 在[Azure CLI](/cli/azure/install-azure-cli)中執行[az acr login](/cli/azure/acr?view=azure-cli-latest#az-acr-login)命令：
