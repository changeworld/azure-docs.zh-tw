...
`acr purge` 命令目前在公用容器映像 (`mcr.microsoft.com/acr/acr-cli:0.4`) 中散發，是從 GitHub [acr-cli](https://github.com/Azure/acr-cli) 存放庫中的原始碼建構而來。
...
* `--concurrency` -指定同時處理 x 清除工作。 如果未提供此參數，則會使用預設值。
