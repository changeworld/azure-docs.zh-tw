...
如果 `FROM` 語句中所指定的基底映射位於其中一個位置，則 ACR 工作會加入勾點，以確保在其基底更新時重建映射。
...
* **適用于基**底映射的穩定標籤-若要在基底映射更新時觸發工作，基底映射必須具有*穩定*的標記，例如 `node:9-alpine`。 這個標籤通常適用於隨著 OS 和架構修補程式更新到最新穩定版本的基底映像。 如果基底映像是隨著新版本標籤更新，它就不會觸發工作。 如需有關映像標籤的詳細資訊，請參閱[最佳做法指引](container-registry-image-tag-version.md)。 
