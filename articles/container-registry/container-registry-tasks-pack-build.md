...
Azure CLI 命令`az acr pack build`會使用[`pack`](https://github.com/buildpack/pack) [Buildpacks](https://buildpacks.io/)中的 CLI 工具來建立應用程式，並將其映射推送至 Azure container registry。 這項功能可讓您從您的應用程式原始程式碼快速地建立容器映射，而不需要定義 Dockerfile。
...
* 適用于您應用程式的 Buildpack 產生器映射名稱。 Azure Container Registry 快取產生器映射`cloudfoundry/cnb:0.0.34-cflinuxfs3` ，例如，以加快組建的速度。  
...
下列範例會使用`cloudfoundry/cnb:0.0.34-cflinuxfs3`產生器，從[Azure 範例/nodejs 檔-hello-world](https://github.com/Azure-Samples/nodejs-docs-hello-world)存放庫中的 node.js 應用程式建立容器映射。 此產生器是由 Azure Container Registry 快取， `--pull`因此不需要參數：
...
此範例會建立`node-app`具有`1.0`標記的映射，並將其推送至*myregistry*容器登錄。 在此範例中，目標登錄名稱會明確地加到映射名稱前面。 如果未指定，登錄登入伺服器名稱會自動加在映射名稱前面。
...
下列範例會使用`heroku/buildpacks:18`產生器，從[buildpack/sample-java 應用](https://github.com/buildpack/sample-java-app)程式存放庫中的 java 應用程式建立容器映射。 `--pull`參數會指定命令應提取最新的產生器映射。 
