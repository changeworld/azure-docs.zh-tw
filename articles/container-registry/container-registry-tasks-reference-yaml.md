...
| -------- | ---- | -------- | ----------- | ------------------ | ------------- |
| `version` | 字串 | Yes | 「ACR 工作」服務所剖析的 `acr-task.yaml` 檔案版本。 在「ACR 工作」努力維持回溯相容性的同時，此值則可讓「ACR 工作」在已定義的版本內維持相容性。 如果未指定，則預設為最新版本。 | 否 | None |
| `stepTimeout` | 整數 (秒) | Yes | 步驟的可執行秒數上限。 如果在工作上指定屬性，則會設定 `timeout` 所有步驟的預設屬性。 如果在 `timeout` 步驟上指定屬性，它會覆寫工作所提供的屬性。 | Yes | 600 (10 分鐘) |
| `workingDirectory` | 字串 | Yes | 容器在執行時間期間的工作目錄。 如果在工作上指定屬性，則會設定 `workingDirectory` 所有步驟的預設屬性。 如果在步驟上指定，它會覆寫工作所提供的屬性。 | Yes | `/workspace` |
| `env` | [字串, 字串, ...] | Yes |  格式的字串陣列 `key=value` ，定義工作的環境變數。 如果在工作上指定屬性，則會設定 `env` 所有步驟的預設屬性。 如果在步驟上指定，它會覆寫任何繼承自工作的環境變數。 | None |
| `secrets` | [秘密，秘密，...] | Yes | [秘密](#secret)物件的陣列。 | None |
| `networks` | [network，network，...] | Yes | [網路](#network)物件的陣列。 | None |
...
| `keyvault` | 字串 | Yes | Azure Key Vault 秘密 URL。 | None |
| `clientID` | 字串 | Yes | 適用于 Azure 資源之[使用者指派受控識別](container-registry-tasks-authentication-managed-identity.md)的用戶端識別碼。 | None |
...
| `driver` | 字串 | Yes | 用來管理網路的驅動程式。 | None |
...
| `context` | 傳遞給 `docker build` 的根目錄。 每個工作的根目錄都會設定為共用的 [workingDirectory](#task-step-properties)，並且包含相關 Git 複製目錄的根目錄。 | No |
...
| 屬性 | 類型 | 選用 | 描述 | 預設值 |
| -------- | ---- | -------- | ----------- | ------- |
| `detach` | bool | Yes | 執行時是否應將容器中斷連結。 | `false` |
| `disableWorkingDirectoryOverride` | bool | Yes | 是否要停用覆 `workingDirectory` 寫功能。 將此與搭配使用， `workingDirectory` 即可完整控制容器的工作目錄。 | `false` |
| `entryPoint` | 字串 | Yes | 覆寫步驟容器的 `[ENTRYPOINT]`。 | None |
| `env` | [字串, 字串, ...] | Yes | `key=value` 格式的字串陣列，用來定義步驟的環境變數。 | None |
| `expose` | [字串, 字串, ...] | Yes | 從容器公開的埠陣列。 |  None |
| [`id`](#example-id) | 字串 | Yes | 可唯一識別工作內的步驟。 工作內的其他步驟可以參考步驟的 `id`，例如使用 `when`進行相依性檢查。<br /><br />`id` 同時也是執行中容器的名稱。 舉例來說，在工作內其他容器中執行的程序可以參考 `id` 作為其 DNS 主機名稱，或藉由 docker logs [id] 來存取它。 | `acb_step_%d`，其中 `%d` 是 YAML 檔案中步驟上階的以零為起始的索引。 |
| `ignoreErrors` | bool | Yes | 不論容器執行期間是否發生錯誤，是否將步驟標記為成功。 | `false` |
| `isolation` | 字串 | Yes | 容器的隔離等級。 | `default` |
| `keep` | bool | Yes | 在執行後是否應保留步驟的容器。 | `false` |
| `network` | 物件 (object) | Yes | 識別執行容器的網路。 | None |
| `ports` | [字串, 字串, ...] | Yes | 從容器發佈到主機的埠陣列。 |  None |
| `pull` | bool | Yes | 是否要先強制提取容器，再執行它以防止任何快取行為。 | `false` |
| `privileged` | bool | Yes | 是否要以特殊許可權模式執行容器。 | `false` |
...
| `secret` | 物件 (object) | Yes | 識別 Azure 資源的 Azure Key Vault 秘密或[受控識別](container-registry-tasks-authentication-managed-identity.md)。 | None |
| `startDelay` | 整數 (秒) | Yes | 延遲容器執行的秒數。 | 0 |
| `timeout` | 整數 (秒) | Yes | 終止步驟前可允許步驟執行的秒數上限。 | 600 |
| [`when`](#example-when) | [字串, 字串, ...] | Yes | 設定步驟與工作內一或多個其他步驟的相依性。 | None |
| `user` | 字串 | Yes | 容器的使用者名稱或 UID | None |
| `workingDirectory` | 字串 | Yes | 設定步驟的工作目錄。 「ACR 工作」預設會建立根目錄作為工作目錄。 不過，如果您的組建含有數個步驟，則可藉由指定相同的工作目錄，讓較前面的步驟與較後面的步驟共用成品。 | `/workspace` |
