...
## <a name="enclave-validation-work-flow"></a>記憶體保護區驗證工作流程

以下是典型 SGX 記憶體保護區證明工作流程中的一般步驟 (使用 Azure 證明)：

1. 用戶端會從記憶體保護區收集辨識項。 辨識項是記憶體保護區環境和在記憶體保護區內執行之用戶端程式庫的相關資訊。
1. 用戶端具有參考 Azure 證明執行個體的 URI。 用戶端會向 Azure AD 進行驗證並取得存取權杖。
1. 用戶端會將辨識項連同存取權杖一起傳送至 Azure 證明。 提交給提供者的確切資訊取決於記憶體保護區類型。
...
![記憶體保護區驗證流程](./media/validation-flow.png)
