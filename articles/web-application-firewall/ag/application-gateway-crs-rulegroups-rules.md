...
---
# <a name="web-application-firewall-crs-rule-groups-and-rules"></a>Web 應用程式防火牆 CRS 規則組和規則

應用程式閘道 web 應用程式防火牆 (WAF) 會保護 web 應用程式免於一般安全性弱點和攻擊。 這是通過基於 OWASP 核心規則集 3.1、3.0 或 2.2.9 定義的規則來完成的。 這些規則可以逐條規則禁用。 本文包含提供的當前規則和規則集。

## <a name="core-rule-sets"></a>核心規則集

預設情況下，應用程式閘道 WAF 預配置了 CRS 3.0。 但是，您可以選擇使用 CRS 3.1 或 CRS 2.2.9。 CRS 3.1 提供了新的規則集，可抵禦 JAVA 感染、初始檔上載檢查集、修復誤報等。 與 CRS 2.2.9 相比，CRS 3.0 可提供減少的誤報。 您還可以[自訂規則以滿足您的需要](application-gateway-customize-waf-rules-portal.md)。
...
- 其他常見攻擊，如命令注入、HTTP 要求走私、HTTP 回應拆分和遠端檔包含
- 違反 HTTP 協定
- HTTP 協定異常，例如缺少主機使用者代理和接受標頭
- Bot、編目程式及掃描程式攻擊
- 常見應用程式佈建錯誤（例如，Apache 和 IIS）

### <a name="owasp-crs-31"></a>OWASP CRS 3.1

CRS 3.1 包括 13 個規則組，如下表所示。 每個組包含多個規則，可以禁用這些規則。
...
|**[REQUEST-920-PROTOCOL-ENFORCEMENT](application-gateway-crs-rulegroups-rules.md#crs920-31)**|防止協定和編碼問題|
|**[REQUEST-921-PROTOCOL-ATTACK](#crs921-31)**|防止頭注入、請求走私和回應拆分|
|**[REQUEST-930-APPLICATION-ATTACK-LFI](#crs930-31)**|防止檔和路徑攻擊|
|**[REQUEST-931-APPLICATION-ATTACK-RFI](#crs931-31)**|防止遠端檔包含 （RFI） 攻擊|
|**[REQUEST-932-APPLICATION-ATTACK-RCE](#crs932-31)**|再次保護遠端代碼執行攻擊|
|**[REQUEST-933-APPLICATION-ATTACK-PHP](#crs933-31)**|防止 PHP 注入攻擊|
|**[REQUEST-941-APPLICATION-ATTACK-XSS](#crs941-31)**|防止跨網站腳本攻擊|
|**[REQUEST-942-APPLICATION-ATTACK-SQLI](#crs942-31)**|防止 SQL 注入攻擊|
|**[REQUEST-943-APPLICATION-ATTACK-SESSION-FIXATION](#crs943-31)**|防止會話固定攻擊|
|**[請求-944-應用程式-攻擊-會話-JAVA](#crs944-31)**|防範 JAVA 攻擊|

### <a name="owasp-crs-30"></a>OWASP CRS 3.0

CRS 3.0 包括 12 個規則組，如下表所示。 每個組包含多個規則，可以禁用這些規則。
...
|**[REQUEST-920-PROTOCOL-ENFORCEMENT](application-gateway-crs-rulegroups-rules.md#crs920-30)**|防止協定和編碼問題|
|**[REQUEST-921-PROTOCOL-ATTACK](#crs921-30)**|防止頭注入、請求走私和回應拆分|
|**[REQUEST-930-APPLICATION-ATTACK-LFI](#crs930-30)**|防止檔和路徑攻擊|
|**[REQUEST-931-APPLICATION-ATTACK-RFI](#crs931-30)**|防止遠端檔包含 （RFI） 攻擊|
|**[REQUEST-932-APPLICATION-ATTACK-RCE](#crs932-30)**|再次保護遠端代碼執行攻擊|
|**[REQUEST-933-APPLICATION-ATTACK-PHP](#crs933-30)**|防止 PHP 注入攻擊|
|**[REQUEST-941-APPLICATION-ATTACK-XSS](#crs941-30)**|防止跨網站腳本攻擊|
|**[REQUEST-942-APPLICATION-ATTACK-SQLI](#crs942-30)**|防止 SQL 注入攻擊|
|**[REQUEST-943-APPLICATION-ATTACK-SESSION-FIXATION](#crs943-30)**|防止會話固定攻擊|

### <a name="owasp-crs-229"></a>OWASP CRS 2.2.9

CRS 2.2.9 包括 10 個規則組，如下表所示。 每個組包含多個規則，可以禁用這些規則。

|規則群組|描述|
|---|---|
|**[crs_20_protocol_violations](#crs20)**|防止協定衝突（如無效字元或帶有請求正文的 GET）|
|**[crs_21_protocol_anomalies](#crs21)**|防止不正確的標頭資訊|
|**[crs_23_request_limits](#crs23)**|防止參數或檔超出限制|
|**[crs_30_http_policy](#crs30)**|防止受限制的方法、標頭和檔案類型|
|**[crs_35_bad_robots](#crs35)**|防止網路抓取器和掃描器|
|**[crs_40_generic_attacks](#crs40)**|防止泛型攻擊（如會話固定、遠端檔包含和 PHP 注入）|
|**[crs_41_sql_injection_attacks](#crs41sql)**|防止 SQL 注入攻擊|
|**[crs_41_xss_attacks](#crs41xss)**|防止跨網站腳本攻擊|
|**[crs_42_tight_security](#crs42)**|防止路徑遍歷攻擊|
|**[crs_45_trojans](#crs45)**|防止後門特洛伊木馬|

在應用程式閘道上使用 Web 應用程式防火牆時，可以使用以下規則組和規則。
...
|RuleId|描述|
|---|---|
|932100|遠端命令執行：Unix 命令注入|
|932105|遠端命令執行：Unix 命令注入|
|932106|遠端命令執行：Unix 命令注入|
|932110|遠端命令執行：Windows 命令注入|
|932115|遠端命令執行：Windows 命令注入|
...
|933111|PHP注入攻擊：找到PHP指令檔上傳|
|933120|PHP 插入式攻擊 = 找到組態指示詞|
|933130|PHP 插入式攻擊 = 找到變數|
|933131|PHP 注入攻擊：找到變數|
|933140|PHP 注入攻擊：找到 I/O 流|
|933150|PHP 插入式攻擊 = 找到高風險 PHP 函式名稱|
|933151|PHP 注入攻擊：找到中風險 PHP 函數名稱|
|933160|PHP 插入式攻擊 = 找到高風險 PHP 函式呼叫|
|933161|PHP注入攻擊：找到低值PHP函式呼叫|
|933170|PHP 注入攻擊：序列化物件注入|
|933180|PHP 插入式攻擊 = 找到變數函式呼叫|
|933190|PHP 注入攻擊：找到 PHP 關閉標記|
...
|941170|NoScript XSS 注入檢查器：屬性注入|
|941180|節點狀態驗證封鎖清單關鍵字|
|941190|使用樣式表的 XSS|
|941200|使用 VML 幀的 XSS|
|941210|使用模糊化 JAVAscript 的 XSS|
|941220|使用模糊 VB 腳本的 XSS|
|941230|使用"嵌入"標記的 XSS|
|941240|使用"導入"或"實現"屬性的 XSS|
|941250|IE XSS 篩檢程式 - 檢測到攻擊|
|941260|使用"元"標記的 XSS|
|941270|使用"連結"href的 XSS|
|941280|使用"基本"標記的 XSS|
|941290|使用"小程式"標籤的 XSS|
|941300|使用"物件"標記的 XSS|
...
|942110|SQL 注入攻擊：檢測到常見注入測試|
|942130|SQL 注入攻擊：檢測到 SQL 陶體學。|
|942140|SQL 插入式攻擊 = 偵測到常用的 DB 名稱|
|942150|SQL 插入式攻擊|
|942160|偵測到使用 sleep() 或 benchmark() 的盲人 sqli 測試。|
|942170|偵測到 SQL 基準測試與睡眠插入式嘗試包括條件式查詢|
|942180|檢測基本 SQL 身份驗證旁路嘗試 1/3|
|942190|偵測到 MSSQL 程式碼執行和資訊收集嘗試次數|
|942200|偵測到 MySQL 註解/空間模糊化的插入和反引號終止|
|942210|檢測鏈式 SQL 注入嘗試 1/2|
|942220|查找整數溢出攻擊，這些攻擊取自跳過魚，但 3.0.00738585072 除外|
|942230|偵測到條件式 SQL 插入嘗試|
|942240|檢測 MySQL 字元集開關和 MSSQL DoS 嘗試|
|942250|檢測匹配、合併和執行即時噴射|
|942251|偵測到 HAVING 插入|
|942260|偵測到基本 SQL 驗證略過嘗試 2/3|
|942270|尋找基本的 SQL 插入。 mysql oracle 和其他常用攻擊字串|
|942280|檢測 Postgres pg_sleep注入、等待延遲攻擊和資料庫關閉嘗試|
|942290|尋找基本 MongoDB SQL 插入嘗試|
|942300|偵測到 MySQL 註解、條件和 ch(a)r 插入|
|942310|檢測鏈式 SQL 注入嘗試 2/2|
...
|942361|檢測基於關鍵字更改或聯合的基本 SQL 注入|
|942370|偵測到傳統 SQL 插入探測 2/2|
|942380|SQL 插入式攻擊|
|942390|SQL 插入式攻擊|
|942400|SQL 插入式攻擊|
|942410|SQL 插入式攻擊|
|942420|受限 SQL 字元異常檢測 （cookies）： 超過特殊字元的 * （8）|
|942421|受限 SQL 字元異常檢測 （cookies）： 超過特殊字元的 * （3）|
|942430|受限制的 SQL 字元異常偵測 (args)：超出的特殊字元數目 (12)|
|942431|受限 SQL 字元異常檢測 （args）： 超過特殊字元的 * （6）|
|942432|受限 SQL 字元異常檢測 （args）： 超過特殊字元的 * （2）|
|942440|偵測到 SQL 註解順序。|
|942450|識別到 SQL 十六進位編碼|
|942460|中繼字元異常偵測警示 - 重複性的非文字字元|
|942470|SQL 插入式攻擊|
|942480|SQL 插入式攻擊|
|942490|檢測經典 SQL 注入探測 3/3|
...
### <a name="p-x-ms-format-detectionnonerequest-944-application-attack-session-javap"></a><a name="crs944-31"></a> <p x-ms-format-detection="none">請求-944-應用程式-攻擊-會話-JAVA</p>

|RuleId|描述|
|---|---|
|944120|可能的有效負載執行和遠端命令執行|
|944130|可疑的JAVA類|
|944200|利用 JAVA 反序列化阿帕奇公域|
...
- [使用 Azure 門戶自訂 Web 應用程式防火牆規則](application-gateway-customize-waf-rules-portal.md)
