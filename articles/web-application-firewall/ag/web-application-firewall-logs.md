...
description: 瞭解如何啟用和記錄管理和 Azure Web 應用程式防火牆
...
您可以使用記錄來監視 Web 應用程式防火牆資源。 您可以儲存效能、存取和其他資料，或從資源取用以進行監視。
...
3. [ **診斷設定** ] 頁面提供資源記錄的設定。 在此範例中，Log Analytics 會儲存記錄。 您也可以使用事件中樞和儲存體帳戶來儲存資源記錄。
...
|sslProtocol| 使用的 TLS 通訊協定 (是否已啟用 TLS) 。|
