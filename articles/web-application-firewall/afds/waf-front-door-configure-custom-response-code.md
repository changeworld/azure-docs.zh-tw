...
ms.locfileid: "94563676"
---
# <a name="configure-a-custom-response-for-azure-web-application-firewall-waf"></a>設定 Azure Web 應用程式防火牆 (WAF) 的自訂回應

根據預設，當 WAF 因為符合規則而封鎖要求時，會傳回403狀態碼，並 **顯示要求被封鎖的** 訊息。 預設訊息也包含可用於連結至要求之 [記錄專案](./waf-front-door-monitor.md) 的追蹤參考字串。  您可以針對您的使用案例，設定自訂的回應狀態碼和含有參考字串的自訂訊息。 本文說明如何在 WAF 封鎖要求時設定自訂回應頁面。
...
您可以在本機電腦上安裝 [Azure PowerShell](/powershell/azure/)，並將其用於任何 PowerShell 工作階段。 請遵循頁面上的指示，使用 Azure 認證進行登入，並安裝 Az PowerShell 模組。
