...
description: 瞭解與線上遷移至 Azure SQL 受控執行個體相關的已知問題/遷移限制。
...
從 SQL Server 到 Azure SQL 受控執行個體的線上遷移相關的已知問題和限制如下所述。
...
- **記憶體內部資料表**
    ...
    記憶體內部 OLTP 適用于 SQL 受控執行個體的 Premium 和商務關鍵層級;一般用途層不支援記憶體內部 OLTP。

## <a name="migration-resets"></a>遷移重設

- **部署**

    SQL 受控執行個體是一種 PaaS 服務，具有自動修補和版本更新。 在您的 SQL 受控執行個體遷移期間，會保留最多36小時的非重大更新。 之後（針對重大更新），如果遷移中斷，進程就會重設為完整還原狀態。
