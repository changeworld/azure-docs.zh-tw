...
* 確保您的虛擬網路網路安全性群組 （NSG） 規則不會阻止以下通訊連接埠 443、53、9354、445、12000。 有關虛擬網路 NSG 流量篩選的更多詳細資訊，請參閱文章["使用網路安全性群組篩選網路流量](https://docs.microsoft.com/azure/virtual-network/virtual-networks-nsg)"。
...
* 啟用 TCP/IP 通訊協定，在 SQL Server Express 安裝期間預設會停用，方法是遵循[啟用或停用伺服器網路通訊協定](https://docs.microsoft.com/sql/database-engine/configure-windows/enable-or-disable-a-server-network-protocol#SSMSProcedure)一文中的指示。
    ...
    > [!IMPORTANT]
    > 創建 Azure 資料庫移轉服務的實例需要訪問通常不在同一資源組中的虛擬網路設置。 因此，創建 DMS 實例的使用者需要訂閱級別的許可權。 要創建所需的角色（可以根據需要分配這些角色），請運行以下腳本：
    >
    > ```
    >
    > $readerActions = `
...
* 創建 Azure SQL 資料庫實例的實例，通過按照文章"在 Azure[門戶中創建 Azure SQL 資料庫](https://docs.microsoft.com/azure/sql-database/sql-database-get-started-portal)"中的詳細資訊進行操作。
...
* 針對 Azure SQL Database 伺服器建立伺服器層級的[防火牆規則](https://docs.microsoft.com/azure/sql-database/sql-database-firewall-configure)，以允許 Azure 資料庫移轉服務存取目標資料庫。 提供用於 Azure 資料庫移轉服務的虛擬網路的子網範圍。
...
## <a name="prerequisites-for-migrating-sql-server-to-an-azure-sql-database-managed-instance"></a>將 SQL 伺服器遷移到 Azure SQL 資料庫託管實例的先決條件

* 通過按照"[在 Azure 門戶中創建 Azure SQL 資料庫託管實例](https://aka.ms/sqldbmi)"一文中的詳細資訊創建 Azure SQL 資料庫託管實例。
...
* 記下在您先前建立的網路共用上具有完整控制權限的 Windows 使用者 (和密碼)。 Azure 資料庫移轉服務類比使用者憑據以將備份檔案上載到 Azure 存儲容器以進行還原操作。
