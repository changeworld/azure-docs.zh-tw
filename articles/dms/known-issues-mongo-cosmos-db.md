...
description: 使用 Azure 資料庫遷移服務瞭解從 MongoDB 遷移到 Azure Cosmos DB 的已知問題和遷移限制。
...
| 原因         | 解決方案 |
| ------------- | ------------- |
| 在 Azure 資料庫遷移服務中使用自簽名證書可能會導致遷移失敗,因為 SSL Cert 不正確。錯誤消息可能包括「根據驗證過程遠程證書無效」。 | 使用來自 CA 的正版證書。  自簽名證書通常僅用於內部測試。 從 CA 頒發機構安裝正版證書時,可以在 Azure 資料庫遷移服務中無問題地使用 SSL(與 Cosmos DB 的連接透過 Mongo API 使用 SSL)。<br><br> |

## <a name="unable-to-get-the-list-of-databases-to-map-in-dms"></a>無法取得 DMS 中對應的資料庫清單

* **症狀**:在 **「選擇源」** 邊欄選項卡上使用**Azure 儲存模式的數據**時,無法取得**資料庫設定**邊欄選項卡上的資料庫清單。

| 原因         | 解決方案 |
| ------------- | ------------- |
| 儲存帳戶連接字串缺少 SAS 資訊,因此無法進行身份驗證。 | 在儲存資源管理員中的 blob 容器上創建 SAS,並將具有容器 SAS 資訊的 URL 用作源詳細資訊連接字串。<br><br> |
...
| 原因         | 解決方案 |
| ------------- | ------------- |
| 您試著從不支援的 MongoDB 版本移轉到 Azure 宇宙 DB。 | 隨著 MongoDB 的新版本發表,將測試它們以確保與 Azure 資料庫遷移服務的相容性,並且該服務將定期更新以接受最新版本。 如果迫切需要遷移,則可以將資料庫/集合匯出到 Azure 存儲,然後將源指向生成的轉儲。 在儲存資源管理員中的 blob 容器上創建 SAS,然後將具有容器 SAS 資訊的網址 用作源詳細資訊連接字串。<br><br> |
