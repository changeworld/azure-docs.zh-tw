...
- 適用於 MySQL 的 Azure 資料庫支援：
  - MySQL 社群版
  - InnoDB 引擎
- 相同版本移轉。 不支援將 MySQL 5.6 移轉到適用於 MySQL 5.7 的 Azure 資料庫。 不支援與 MySQL 8.0 之間的移轉。
...
- **錯誤：** 目標資料庫 {database} 是空的。 請遷移結構描述。
  ...
  **限制**：如果目標適用於 MySQL 的 Azure 資料庫資料庫沒有必要的架構，就會發生此錯誤。 需要進行架構遷移，才能將資料移轉到您的目標。
  ...
  因應 **措施：將** 架構從源資料庫 [遷移](./tutorial-mysql-azure-mysql-online.md#migrate-the-sample-schema)至目標資料庫。
