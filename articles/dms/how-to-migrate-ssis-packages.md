...
如果您使用 2012 以前的 SSIS 版本，則在將 SSIS 專案/套件重新部署至專案部署模型之前，您必須先使用 Integration Services 專案轉換精靈 (亦可從 SSMS 啟動) 進行轉換。 如需詳細資訊，請參閱[將專案轉換為專案部署模型](/sql/integration-services/packages/deploy-integration-services-ssis-projects-and-packages?view=sql-server-2017#convert)一文。
...
* 您必須在 Azure Data Factory (ADF) 中布建 SSIS，其中包含 () 所裝載的目的地 SSISDB 的 Azure-SSIS Integration Runtime SQL Database IR (，如本文中的 Azure-SSIS Integration Runtime Azure Data Factory [中](../data-factory/tutorial-deploy-ssis-packages-azure.md) 所述。
