...
description: 瞭解如何在 IoT Edge 上部署適用於 IoT 的 Azure 資訊安全中心的安全性代理程式。
...
使用下列步驟來部署 IoT Edge 的適用於 IoT 的 Azure 資訊安全中心安全性模組。

### <a name="prerequisites"></a>先決條件

1. 在 IoT 中樞中，確定您的裝置已 [註冊為 IoT Edge 裝置](https://docs.microsoft.com/azure/iot-edge/how-to-register-device-portal)。

1. 適用於 IoT 的 Azure 資訊安全中心 Edge 模組需要在 IoT Edge 裝置上安裝 [AuditD framework](https://linux.die.net/man/8/auditd) 。
...
1. 選取 **物聯網**，然後搜尋 **適用於 IoT 的 Azure 資訊安全中心** 並加以選取。
   ...
   ![選取適用於 IoT 的 Azure 資訊安全中心](media/howto/edge-onboarding-8.png)
...
完成每個步驟，以完成適用於 IoT 的 Azure 資訊安全中心的 IoT Edge 部署。
...
1. 在 [ **模組** 對應項設定] 索引標籤上，新增下列設定：
   ...
   如需設定代理程式的詳細資訊，請參閱 [設定安全性代理](https://docs.microsoft.com/azure/asc-for-iot/how-to-agent-configuration)程式。
