...
粗略 relocalization 的運作方式是將各種裝置感應器讀數與建立和查詢錨點產生關聯。 針對戶外案例，感應器資料通常是裝置全球定位系統) 位置的 GPS (。 當 GPS 無法使用或不可靠 (例如室內) 時，感應器資料是由 WiFi 存取點和範圍內的藍牙信號所組成。 所有收集的感應器資料都有助於維護空間索引。這是 Azure 空間錨點所使用的，以快速判斷在您的裝置大約100計量內的錨點。

粗略 relocalization 所啟用的快速查閱錨點可簡化應用程式的開發，這是由世界級的集合所支援的 (例如，數百萬個地理分佈) 錨點。 錨點管理的複雜性全都隱藏起來，讓您更專注于絕佳的應用程式邏輯。 所有的錨點繁重動作都是由 Azure 空間錨點在幕後進行。
...
取用者等級的 GPS 裝置通常不精確。 [Zandenbergen 和 Barbeau (2011][6]的研究) 報告具有輔助 gps 的行動電話的中位數準確度 (gps) 大約7米，這相當於要忽略的大數值！ 為了考慮這些度量錯誤，服務會將錨點視為 GPS 空間中的機率分佈。 因此，錨點現在就是最可能 (的空間區域，而且有超過95% 的信賴) 包含其真實、未知的 GPS 位置。
...
| **Wlan**        | 以每3秒大約一次掃描的速率提供支援 | 支援。 從 API 層級28開始，每隔2分鐘就會將 WiFi 掃描節流至4個呼叫。 在 Android 10 中，您可以從 [開發人員設定] 功能表停用節流。 如需詳細資訊，請參閱[Android 檔][5]。 | N/A-沒有公用 API |
