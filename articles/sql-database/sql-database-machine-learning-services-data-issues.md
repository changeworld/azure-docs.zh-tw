---
title: 使用 R 與 SQL 資料類型與物件
titleSuffix: Azure SQL Database Machine Learning Services (preview)
description: 瞭解如何使用機器學習服務(預覽)使用 Azure SQL 資料庫使用 R 中的數據類型和數據物件,包括可能會遇到的常見問題。
...
本文討論了在[機器學習服務(使用 R)中的機器學習服務(使用 R)](sql-database-machine-learning-services-overview.md)中在 R 和 SQL 資料庫之間行動數據時可能會遇到的一些常見問題。 透過此練習所獲得的體驗，可提供您在使用自己指令碼資料時的基本背景。
...
## <a name="working-with-a-data-frame"></a>使用資料盒

當文本將結果從 R 返回到 SQL 時,它必須將數據作為**數據返回。** 您在指令碼中產生的任何其他類型物件 (不論是清單、因數、向量或二進位資料)，如果您想要輸出它以做為預存程序結果的一部分，就必須將它轉換為資料框架。 幸好有多個 R 函數可用來支援將其他物件變更為資料框架。 您甚至可以序列化二進位模型並將其返回資料幀中,本文稍後將執行此操作。
...
如您所見，稍微變化一下 R 語法，就會對結果的結構描述產生極大的影響。 對於所有的細節,R數據類型的差異在[哈德利·韋翰的"高級R"](http://adv-r.had.co.nz)中*的數據結構*部分中詳細解釋。

現在只需注意，您需要在將 R 物件強制轉型為資料框架時檢查預期的結果。

> [!TIP]
> 您還可以使用 R 標識函數`is.matrix`(`is.vector`如 ) 來返回有關內部數據結構的資訊。
...
例如，下列指令碼定義長度為 6 的數值陣列，並將它儲存在 R 變數 `df1` 中。 然後,數位陣列與 RTestData 表(上面建立的)的整數組合,該表包含三 (3) 個值,以`df2`建立新的資料框 。
...
請記住,數據框僅看起來像一個表,但實際上是向量的清單。

## <a name="cast-or-convert-sql-data"></a>強制轉換 SQL 資料

R 和 SQL 不使用相同的資料類型,因此,當您在 SQL 中執行查詢以獲取數據,然後將該查詢傳遞到 R 執行時時,通常會發生某些類型的隱式轉換。 當您將數據從 R 返回到 SQL 時,將發生另一組轉換。

- SQL 將數據從查詢推送到 R 進程並將其轉換為內部表示形式,以提高效率。
- R 執行階段會將資料載入 data.frame 變數，並對資料執行自己的作業。
- 資料庫引擎使用安全的內部連接將數據返回到 SQL,並按 SQL 數據類型顯示數據。
- 通過使用用戶端或網路庫連接到 SQL 來獲取數據,該庫可以發出 SQL 查詢並處理表格數據集。 此用戶端應用程式可能會以其他方式影響資料。
...
> [!NOTE]
> 您可以使用任何版本的 AdventureWorks，或使用您自己的資料庫建立不同查詢。 關鍵是嘗試處理包含文本、日期時間和數值的某些數據。

現在,請嘗試使用此查詢作為存儲過程的輸入。
...
有關受支援和不支援的 R 資料類型的詳細資訊,請參閱[R 函式庫和資料類型](/sql/advanced-analytics/r/r-libraries-and-data-types)。
