...
description: 了解如何使用查詢編輯器對 Azure SQL Database 執行 Transact-SQL (T-SQL) 查詢。
...
查詢編輯器是 Azure 入口網站中的一項工具，可針對您的 Azure SQL 資料庫或 Azure SQL 資料倉儲執行 SQL 查詢。 

在本快速入門中，您將使用查詢編輯器對 Azure SQL 資料庫執行 Transact-SQL (T-SQL) 查詢。


## <a name="prerequisites"></a>Prerequisites

完成本快速入門需要 AdventureWorksLT 範例資料庫。 如果您沒有 AdventureWorksLT SQL 資料庫的工作複本，下列快速入門會快速建立一個：

- [快速入門：使用 Azure 入口網站、PowerShell 和 Azure CLI，在 Azure SQL 資料庫中建立單一 Azure SQL 資料庫](sql-database-single-database-get-started.md) 
...
> [!IMPORTANT]
> 查詢編輯器會使用連接埠 443 和 1443 來進行通訊。 請確定您已在這些連接埠上啟用輸出 HTTPS 流量。 您也必須[將輸出 IP 位址新增至伺服器的允許防火牆規則](sql-database-server-level-firewall-rule.md)，以存取您的資料庫和資料倉儲。


## <a name="open-the-sql-database-query-editor"></a>開啟 SQL Database 查詢編輯器

1. 登入 [Azure 入口網站](https://portal.azure.com/) 並選取您想要查詢的 SQL 資料庫。
...
即使您已登入入口網站，您仍然需要提供認證來存取 SQL 資料庫。 您可以使用 SQL 驗證或 Azure Active Directory 來連線到您的資料庫。
...
設定 Azure Active Directory (Azure AD) 系統管理員，可讓您使用單一身分識別來登入 Azure 入口網站和您的 SQL 資料庫。 若要使用 Azure AD 連線到您的資料庫，請遵循下列步驟來為您的 SQL 伺服器設定 Azure AD 系統管理員。
...
#### <a name="set-an-active-directory-admin-for-the-database-server"></a>設定資料庫伺服器的 Active Directory 系統管理員

1. 在 Azure 入口網站中選取您的 SQL 伺服器。

2. 在 [SQL 伺服器]  功能表上，選取 [Active Directory 系統管理員]  。

3. 在 SQL 伺服器的 [Active Directory 系統管理員]  頁面工具列中，選取 [設定系統管理員]  ，然後選擇使用者或群組作為您的 Azure AD 系統管理員。
...
6. 在 [SQL Server]  功能表中，選取 [SQL 資料庫]  ，然後選取您的 SQL 資料庫。

7. 在 [SQL 資料庫]  功能表中，選取 [查詢編輯器 (預覽)]  。 在 [登入]  頁面的 [Active Directory 驗證]  標籤下方，如果您是 Azure AD 系統管理員，即會顯示一則訊息，指出您已登入。然後選取 [繼續使用  \<您的使用者或群組識別碼>]  按鈕。 如果頁面指出您尚未成功登入，您可能需要重新整理頁面。

## <a name="query-a-sql-database"></a>查詢 SQL 資料庫
...
若要深入了解 Azure SQL 資料庫中支援的 Transact-SQL (T-SQL)，請參閱[解析移轉至 SQL 資料庫時的 Transact-SQL 差異](sql-database-transact-sql-information.md)。
