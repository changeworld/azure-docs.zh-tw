...
- 若要將現有的 SQL Server 資料庫從內部部署還原到受控執行個體，您可以使用[Azure 資料庫移轉服務（DMS）進行遷移](../dms/tutorial-sql-server-to-managed-instance.md)，或使用[t-sql restore 命令](sql-database-managed-instance-get-started-restore.md)從資料庫備份檔案還原。
