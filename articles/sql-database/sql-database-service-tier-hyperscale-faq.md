...
| **功耗** | 單一資料庫 | 每個虛擬核心 500 IOPS，且 IOPS 上限為 7000 | 超大規模資料庫是多層式架構，在多個層級進行快取。 有效的 IOPS 將視工作負載而定。 | 5000 IOPS，IOPS 上限為 200,000|
...
- **相應縮小/相應放大**
  ...
  這些額外計算複本的布建可在固定時間內完成，而且是一項線上作業。 您可以將連接字串的引數設定為`ApplicationIntent` `ReadOnly`，以連線至這些額外的唯讀計算複本。 任何與`ReadOnly`應用程式意圖的連接都會自動路由傳送至其中一個額外的唯讀計算複本。
...
### <a name="does-my-tempdb-scale-as-my-database-grows"></a>我的`tempdb`資料庫規模是否成長

您`tempdb`的資料庫位於本機 SSD 儲存體，且大小會按比例調整為您布建的計算大小。 您`tempdb`的已優化，可提供最大的效能優勢。 `tempdb`大小不是可設定的，而且會為您管理。
...
### <a name="does-the-size-of-my-tempdb-database-also-grow-as-the-compute-is-scaled-up"></a>當計算相應增加時`tempdb` ，我的資料庫大小也會隨之成長

是。 當`tempdb`計算成長時，資料庫將會自動相應增加。  
...
您可以將連接字串的引數設定為`ApplicationIntent` `ReadOnly`，以連線至這些額外的唯讀計算複本。 任何標示為的`ReadOnly`連線都會自動路由至其中一個額外的唯讀計算複本。  
...
如果您連接`READ_ONLY`到唯讀次要複本，而且`READ_WRITE`您已連接到主要複本，則結果會是。 請注意，資料庫內容必須設定為超大規模資料庫資料庫的名稱，而非`master`資料庫。

### <a name="can-i-create-a-dedicated-endpoint-for-a-read-scale-out-replica"></a>我可以為讀取相應放大複本建立專用端點嗎

否。 您只能藉由指定`ApplicationIntent=ReadOnly`來連接到讀取相應放大複本。
...
### <a name="do-i-get-different-tempdb-sizing-for-my-primary-compute-and-my-additional-secondary-compute-replicas"></a>我的主要計算`tempdb`和其他次要計算複本的大小是否不同

否。 您`tempdb`的資料庫是根據計算大小布建而設定的，您的次要計算複本與主要計算的大小相同。
