...
- [設定異地複本的安全性](sql-database-geo-replication-security-config.md)。
...
- [跨不同架構的資料表進行查詢](sql-database-elastic-query-vertical-partitioning.md)。

## <a name="elastic-database-jobs"></a>彈性資料庫作業

- [建立和管理](elastic-jobs-powershell.md)使用 PowerShell 彈性資料庫作業。
...
- [遷移分區化 DB](sql-database-elastic-convert-to-use-elastic-tools.md)。
