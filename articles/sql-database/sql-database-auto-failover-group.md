...
當您使用自動容錯移轉群組與自動容錯移轉原則時，影響群組中一或多個資料庫的任何中斷，都會導致自動容錯移轉。 這些事件通常無法由內建的自動高可用性作業自行緩和。 容錯移轉觸發程式的範例包括由於數個計算節點上的作業系統核心記憶體流失而導致 SQL 租使用者通道或控制環已關閉的事件，或因一或多個租使用者通道因為不正確的網路纜線在例行硬體解除委任期間而中斷而造成的事件。  如需詳細資訊，請參閱[SQL Database 高可用性](sql-database-high-availability.md)。
...
- **初始植入** 
  ...
  將資料庫、彈性集區或受控實例新增至容錯移轉群組時，會在資料複寫開始之前產生初始植入階段。 初始植入階段是最長且最耗費資源的作業。 初始植入完成後，資料會進行同步處理，然後只複寫後續的資料變更。 初始種子完成所需的時間取決於您的資料大小、複寫的資料庫數目，以及容錯移轉群組中的實體之間的連結速度。 在正常情況下，一般植入速度是單一資料庫或彈性集區的 50-500 GB 一小時，而受控實例的 18-35 GB 一小時。 會平行執行所有資料庫的植入。 您可以使用指定的植入速度，以及資料庫的數目和資料大小總計，來估計初始植入階段在資料複寫開始之前要花費的時間。

  針對受控實例，當估計初始植入階段的時間時，也必須考慮兩個實例之間的快速路由連結速度。 如果兩個實例之間的連結速度比所需還慢，則種子的時間可能會受到影響。 您可以使用指定的植入速度、資料庫數目、資料大小總計，以及用來估計初始植入階段在資料複寫開始之前所需時間的連結速度。 例如，針對單一 100 GB 資料庫，如果連結能夠每小時推送 35 GB，則初始種子階段會從 2.8-5.5 小時起。 如果連結每小時只能傳輸 10 GB，則植入 100 GB 資料庫將需要約10小時。 如果有多個要複寫的資料庫，會以平行方式執行植入，而當結合緩慢的連結速度時，初始植入階段可能會花費更長的時間，特別是當平行植入所有資料庫中的資料超過可用的連結頻寬時。 如果兩個實例之間的網路頻寬受到限制，而您將多個受控實例新增至容錯移轉群組，請考慮依序逐一將多個受控實例新增到容錯移轉群組。
...
- **資料遺失的寬限期**
  ...
  因為主要和次要資料庫是使用非同步複寫進行同步處理，容錯移轉可能會導致資料遺失。 您可以自訂自動容錯移轉原則，以反映應用程式對資料遺失的容錯程度。 藉由`GracePeriodWithDataLossHours`設定，您可以控制系統在起始可能造成資料遺失的容錯移轉之前等待的時間長度。
...
如果您有容忍某些過時資料的邏輯隔離唯讀工作負載，則可以使用應用程式中的次要資料庫。 針對唯讀工作階段，請使用 `<fog-name>.secondary.database.windows.net` 作為伺服器 URL，連線會自動導向至次要伺服器。 也建議您使用`ApplicationIntent=ReadOnly`，在連接字串讀取意圖中指出。 如果您想要確保唯讀工作負載可以在容錯移轉之後重新連線，或在次要伺服器離線時，請務必設定容錯移轉原則`AllowReadOnlyFailoverToPrimary`的屬性。
...
如果偵測到中斷，SQL 會等待您指定的期間`GracePeriodWithDataLossHours`。 預設值為 1 小時。 如果您無法承受資料遺失，請務必將設定`GracePeriodWithDataLossHours`為夠大的數位，例如24小時。 使用手動群組容錯移轉，從次要伺服器容錯回復到主要伺服器。
