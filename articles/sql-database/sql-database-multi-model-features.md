...
- 您想要套用[記憶體內部技術](sql-database-in-memory.md)的資料庫功能，以改善 NoSQL 資料結構的分析或處理效能、使用[異動複寫](sql-database-managed-instance-transactional-replication.md)或[可讀取的複本](sql-database-read-scale-out.md)，在另一個位置建立資料的複本，並從主資料庫卸載一些分析工作負載。
