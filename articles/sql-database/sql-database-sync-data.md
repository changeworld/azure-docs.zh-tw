...
- **混合式資料同步處理：** 使用資料同步，您可以讓內部部署資料庫與 Azure SQL 資料庫之間的資料保持同步，以啟用混合式應用程式。 此功能對於考慮移轉至雲端，而且想要將部分應用程式放在 Azure 的客戶很有吸引力。
...
| 缺點 | - 5 分鐘或更多的延遲<br/>- 無交易一致性<br/>- 更高的效能影響 | -無法從 Azure SQL Database 的單一資料庫或集區資料庫發行<br/>- 高維護成本 |
