...
---
# <a name="creating-and-using-active-geo-replication"></a>創建和使用活動異地複製

活動異地複製是 Azure SQL 資料庫功能，允許您在同一或不同的資料中心（區域）中的 SQL 資料庫伺服器上創建單個資料庫的可讀次要資料庫。
...
- **資料庫移轉**：您可以使用活動異地複製將資料庫從一台伺服器遷移到另一台連線伺服器，但停機時間最小。
...
> [!IMPORTANT]
> 可以使用異地複製在與主資料庫相同的區域中創建次要資料庫。 您可以使用此次要資料庫來負載平衡同一區域中的唯讀工作負載。 但是，同一區域中的次要資料庫不提供額外的故障恢復能力，因此不是災害復原的適當容錯移轉目標。 它還不能保證可用性區域隔離。 使用具有[區域冗余配置](sql-database-high-availability.md#zone-redundant-configuration)的業務關鍵或高級服務層來實現可用性區域隔離。   
>

- **計劃性容錯移轉**

  完成完全同步後，計畫容錯移轉將切換主資料庫和次要資料庫的角色。 它是一個線上操作，不會導致資料丟失。 操作的時間取決於需要同步的主資料庫上的事務日誌的大小。 計畫容錯移轉專為以下方案而設計：（a） 在資料丟失不可接受時在生產中執行 DR 鑽取;（a） 在生產中執行 DR 鑽取;（a） 在生產中執行 DR 鑽取。（a） 在生產中執行 DR 鑽取。（a） 在生產中執行 DR 鑽取。（b） 將資料庫遷至另一個區域;和 （c） 在中斷緩解（容錯回復）後將資料庫返回到主區域。

- **計畫外容錯移轉**

  非計劃性或強制容錯移轉會立即將次要資料庫切換為主要角色，而不會與主要資料庫進行任何同步處理。 提交到主事務但未複製到次要資料庫的任何事務都將丟失。 當主資料庫無法訪問時，此操作設計為在中斷期間的恢復方法，但必須快速還原資料庫可用性。 當原始主資料庫重新連線時，它將自動重新連接並成為新的次要資料庫。 容錯移轉之前的所有不同步交易都將保留在備份檔案中，但不會與新主事務同步以避免衝突。 這些事務必須手動與主資料庫的最新版本合併。
...
## <a name="preparing-secondary-database-for-failover"></a>為容錯移轉準備次要資料庫

為確保應用程式可以在容錯移轉後立即訪問新的主資料庫，請確保正確配置了次要伺服器和資料庫的身份驗證要求。 如需詳細資訊，請參閱 [災害復原後的 SQL Database 安全性](sql-database-geo-replication-security-config.md)。 為了保證容錯移轉後的合規性，請確保次要資料庫上的備份保留原則與主資料庫的備份保留原則匹配。 這些設置不是資料庫的一部分，並且不會複製。 預設情況下，次要伺服器的預設 PITR 保留期為 7 天。 如需詳細資訊，請參閱 [SQL Database 自動備份](sql-database-automated-backups.md)。

> [!IMPORTANT]
> 如果您的資料庫是容錯移轉組的成員，則無法使用異地複製分程式命令啟動其容錯移轉。 請考慮對組使用容錯移轉命令。 如果需要對單個資料庫進行容錯移轉，必須首先將其從容錯移轉組中刪除。 有關詳細資訊，請參閱[容錯移轉組](sql-database-auto-failover-group.md)。 


## <a name="configuring-secondary-database"></a>配置次要資料庫

主要和次要資料庫必須有相同的服務層級。 此外也強烈建議您使用與主要資料庫相同的計算大小 (DTU 或虛擬核心) 來建立次要資料庫。 如果主資料庫遇到繁重的寫入工作負載，則計算大小較低的次要資料庫可能無法跟上它。 這將導致輔助延遲和潛在不可用。 如果需要強制容錯移轉，滯後于主資料庫的次要資料庫也面臨大量資料丟失的風險。 為了減輕這些風險，有效的活動異地複製將限制主的日誌速率，使其次要資料庫能夠趕上。 二次配置不平衡的另一個後果是，容錯移轉後，由於新主伺服器的計算能力不足，應用程式的性能將受到影響。 需要將其升級到更高的計算級別，這在緩解中斷之前是不可能的。 


> [!IMPORTANT]
> 除非次要資料庫配置與主資料庫相同的計算大小，否則無法保證已發佈的 RPO = 5 秒。 


如果您決定建立具有較低計算大小的次要資料庫，您可以利用 Azure 入口網站上的記錄 IO 百分比圖表，來預估次要資料庫承受複寫負載所需的最低計算大小。 例如，如果您的主要資料庫是 P6 (1000 DTU) 和其記錄 IO 百分比為 50%，則次要資料庫必須至少是 P4 (500 DTU)。 您也可以使用 [sys.resource_stats](/sql/relational-databases/system-catalog-views/sys-resource-stats-azure-sql-database) 或 [sys.dm_db_resource_stats](/sql/relational-databases/system-dynamic-management-views/sys-dm-db-resource-stats-azure-sql-database) 資料庫檢視來擷取記錄 IO 資料。  在[sys.dm_exec_requests](/sql/relational-databases/system-dynamic-management-views/sys-dm-exec-requests-transact-sql)和[sys.dm_os_wait_stats](/sql/relational-databases/system-dynamic-management-views/sys-dm-os-wait-stats-transact-sql)資料庫檢視中，限制報告為HADR_THROTTLE_LOG_RATE_MISMATCHED_SLO等候狀態。 
...
執行更改的用戶端需要對主伺服器進行網路訪問。 儘管必須將用戶端的相同 IP 位址添加到次要伺服器上的允許清單中，但嚴格來說，與次要伺服器的網路連接並不嚴格。 
...
1. 創建專用於設置活動異地複製的登錄名（並根據需要調整憑據）：

   ```sql
   create login geodrsetup with password = 'ComplexPassword01'
   ```

1. 創建相應的使用者並將其分配給 dbManager 角色： 
...
1. 使用相同的使用者名密碼創建與主伺服器上相同的登錄名，以及 SID： 
...
> [!IMPORTANT]
> 容錯移轉組中的主資料庫無法擴展到更高層，除非首先將次要資料庫縮放到較高層。 如果在縮放次要資料庫之前嘗試縮放主資料庫，可能會收到以下錯誤：
>
...
要監視與 RPO 有關滯後，請使用主資料庫上的*replication_lag_sec*列[的 sys.dm_geo_replication_link_status。](/sql/relational-databases/system-dynamic-management-views/sys-dm-geo-replication-link-status-azure-sql-database) 它顯示在主資料庫上提交和在輔助級上保留的事務之間的延遲（以秒為單位）。 例如 如果滯後值為 1 秒，則意味著如果主資料庫此時受到中斷的影響並啟動容錯移轉，則不會保存 1 秒的最新轉換。 

要衡量對已應用於次要資料庫的主資料庫（即可從次要資料庫讀取）的更改的延遲，請將次要資料庫上的*last_commit*時間與主資料庫上的值相同進行比較。

> [!NOTE]
> 有時 *，* 主資料庫上的replication_lag_sec具有 Null 值，這意味著主資料庫當前不知道次要資料庫有多遠。   這通常在進程重新開機後發生，並且應該是暫時性情況。 如果*replication_lag_sec*在較長時間內返回 Null，請考慮提醒應用程式。 這將指示次要資料庫由於永久連接故障而無法與主資料庫通信。 還有一些條件可能導致次要資料庫和主資料庫上*last_commit*時間之間的差異變大。 例如 如果在長時間未更改後在主資料庫上進行提交，則差異將跳升到較大的值，然後快速返回到 0。 當這兩個值之間的差異長時間保持較大時，請考慮為錯誤條件。
...
### <a name="t-sql-manage-failover-of-single-and-pooled-databases"></a>T-SQL：管理單個資料庫和池資料庫的容錯移轉
...
| Command | 描述 |
| --- | --- |
| [更改資料庫](https://docs.microsoft.com/sql/t-sql/statements/alter-database-transact-sql?view=azuresqldb-current) |使用 ADD SECONDARY ON SERVER 引數，針對現有資料庫建立次要資料庫並開始資料複寫 |
| [更改資料庫](https://docs.microsoft.com/sql/t-sql/statements/alter-database-transact-sql?view=azuresqldb-current) |使用 FAILOVER 或 FORCE_FAILOVER_ALLOW_DATA_LOSS，將次要資料庫切換為主要資料庫以便開始容錯移轉 |
| [更改資料庫](https://docs.microsoft.com/sql/t-sql/statements/alter-database-transact-sql?view=azuresqldb-current) |使用 REMOVE SECONDARY ON SERVER，來終止 SQL Database 和指定次要資料庫間的資料複寫。 |
...
### <a name="powershell-manage-failover-of-single-and-pooled-databases"></a>PowerShell：管理單個資料庫和池資料庫的容錯移轉
...
### <a name="rest-api-manage-failover-of-single-and-pooled-databases"></a>REST API：管理單個資料庫和池資料庫的容錯移轉
...
- 如需範例指令碼，請參閱：
  ...
  - [使用活動異地複製配置和容錯移轉池資料庫](scripts/sql-database-setup-geodr-and-failover-pool-powershell.md)
