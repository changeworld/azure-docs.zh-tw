...
- **IO 問題**
  ...
  查詢可能正在等候頁面寫入資料或記錄檔。 在此情況下，請`INSTANCE_LOG_RATE_GOVERNOR`檢查`WRITE_LOG`DMV 中`PAGEIOLATCH_*`的、或等候統計資料。 請參閱使用 Dmv 來[識別 IO 效能問題](sql-database-monitoring-with-dmvs.md#identify-io-performance-issues)。
- **TempDB 問題**
