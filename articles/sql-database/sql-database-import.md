...
---
# <a name="quickstart-import-a-bacpac-file-to-a-database-in-azure-sql-database"></a>快速入門:將 BACPAC 檔案匯入 Azure SQL 資料庫中的資料庫
...
觀看此視訊,瞭解如何從 Azure 門戶中的 BACPAC 檔案匯入,或繼續閱讀以下內容:
...
目前不支援使用 Azure PowerShell 從 BACPAC 檔案將資料庫移至[託管實例](sql-database-managed-instance.md)。 請使用 SQL 伺服器管理工作室或 SQL 包。

> [!NOTE]
> 處理透過 Azure 門戶或 PowerShell 提交的匯入/匯出請求的電腦需要儲存 BACPAC 檔案以及資料層應用程式架構 (DacFX) 生成的臨時檔。 在大小相同的資料庫之間所需的磁碟空間差異很大,可能需要高達資料庫大小的 3 倍的磁碟空間。 運行導入/匯出請求的計算機僅具有 450GB 的本地磁碟空間。 因此,某些請求可能會失敗與錯誤`There is not enough space on the disk`。 在這種情況下,解決方法是在具有足夠本地磁碟空間的計算機上運行 sqlpackage.exe。 我們鼓勵使用 SqlPackage 導入/匯出大於 150GB 的資料庫以避免此問題。
...
## <a name="using-sqlpackage"></a>使用 Sql 套件

若要使用 [SqlPackage](https://docs.microsoft.com/sql/tools/sqlpackage) 命令列公用程式匯入 SQL Server 資料庫，請參閱[匯入參數和屬性](https://docs.microsoft.com/sql/tools/sqlpackage#import-parameters-and-properties)。 用於[可視化工作室的](https://msdn.microsoft.com/library/mt204009.aspx) [SQL 伺服器管理工作室](https://docs.microsoft.com/sql/ssms/download-sql-server-management-studio-ssms)和 SQL Server 數據工具包括 SqlPackage。 您也可以從 Microsoft 下載中心下載最新的 [SqlPackage](https://www.microsoft.com/download/details.aspx?id=53876)。
...
> [!NOTE]
> [託管實例](sql-database-managed-instance.md)當前不支援使用 Azure PowerShell 從 BACPAC 檔案將資料庫遷移到實例資料庫中。 若要匯入至受控執行個體，請使用 SQL Server Management Studio 或 SQLPackage。

> [!NOTE]
> 處理透過門戶或 Powershell 提交的導入/匯出請求的電腦需要儲存 bacpac 檔案以及資料層應用程式架構 (DacFX) 生成的暫存檔案。 在大小相同的 DB 中所需的磁碟空間差異很大,並且可能需要高達資料庫大小的 3 倍。 運行導入/匯出請求的計算機僅具有 450GB 的本地磁碟空間。 因此,某些請求可能會失敗,出現"磁碟上空間不足"錯誤。 在這種情況下,解決方法是在具有足夠本地磁碟空間的計算機上運行 sqlpackage.exe。 導入/匯出大於 150GB 的資料庫時,請使用 SqlPackage 避免此問題。
...
您可以使用[獲取-AzSql 資料庫導入狀態](/powershell/module/az.sql/get-azsqldatabaseimportexportstatus)cmdlet 檢查導入的進度。 在要求後立即執行 cmdlet 通常`Status: InProgress`會傳回 。 當您看到`Status: Succeeded`時,導入已完成。
...
使用[az-sql-db 匯入](/cli/azure/sql/db#az-sql-db-import)命令向 Azure SQL 資料庫服務提交匯入資料庫請求。 匯入可能需要一些時間才能完成，視資料庫大小而定。
...
- 將 Azure 服務的允許存取設定為 OFF 時,導入匯出服務不起作用。 但是,您可以通過從 Azure VM 手動運行 sqlpackage.exe 或使用 DACFx API 直接在代碼中執行導出來解決此問題。
...
- [SQL 伺服器匯入與匯出精靈](https://docs.microsoft.com/sql/integration-services/import-export-data/start-the-sql-server-import-and-export-wizard)。

## <a name="next-steps"></a>後續步驟

- 要瞭解如何連線到與查詢匯入的 SQL 資料庫,請參閱[快速入門:Azure SQL 資料庫:使用 SQL 伺服器管理工作室連接和查詢資料](sql-database-connect-query-ssms.md)。
