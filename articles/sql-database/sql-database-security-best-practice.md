...
本指南的預期受眾是面臨有關如何保護 Azure SQL 資料庫的問題的客戶。 對本最佳實務文章感興趣的角色包括但不限於:
...
- 如果無法避免密碼或機密,請將使用者密碼和應用程式機密存儲在 Azure 密鑰保管庫中,並通過金鑰保管庫存取策略管理訪問。 
...
- 不要將需要靜態加密的數據存儲在主資料庫中。 主資料庫不能使用 TDE 加密。
