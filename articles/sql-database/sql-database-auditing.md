...
- 要按照[Azure 存儲提供的說明](https://docs.microsoft.com/azure/storage/blobs/storage-blob-immutability-policies-manage#enabling-allow-protected-append-blobs-writes)為伺服器或資料庫級審核事件配置不可變日誌存儲（請確保在配置不可變 Blob 存儲時已選擇 **"允許其他追加服務**"）。
...
## <a name="manage-azure-sql-server-and-database-auditing"></a><a id="manage-auditing"></a>管理 Azure SQL 伺服器和資料庫審核
