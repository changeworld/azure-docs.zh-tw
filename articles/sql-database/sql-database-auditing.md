...
Azure SQL[資料庫](sql-database-technical-overview.md)和[Azure 同步分析](../synapse-analytics/sql-data-warehouse/sql-data-warehouse-overview-what-is.md)的審核可追蹤資料庫事件並將其寫入 Azure 儲存帳戶、日誌分析工作區或事件中心中的審核日誌。 

稽核也具備下列功能：

- 協助您保持法規遵循、了解資料庫活動，以及深入了解可指出商務考量或疑似安全違規的不一致和異常。

- 啟用及推動遵循法規標準，但不保證符合法規。 有關支持標準合規性的 Azure 程式的詳細資訊,請參閱[Azure 信任中心](https://gallery.technet.microsoft.com/Overview-of-Azure-c1be3942),您可以在其中查找最新的 SQL 資料庫合規性認證清單。

> [!NOTE] 
> 本主題適用於 Azure SQL 資料庫和 Azure 同步分析資料庫。 為簡單起見,SQL 資料庫在引用 Azure SQL 資料庫和 Azure 突觸分析時使用。
...
#### <a name="auditing-limitations"></a>稽核限制

- **不支援****進階儲存體**。
- **Azure 資料空間儲存 Gen2 儲存帳號**的**分割命名空間**目前**不支援**。
- 不支援對已暫停的**Azure SQL 資料倉庫**啟用審核。 要啟用審核,請恢復數據倉庫。
...
5. 有多種選項可用於配置將寫入審核日誌的位置。 您可以將日誌寫入 Azure 儲存帳戶、日誌分析工作區以供 Azure 監視器日誌(預覽)使用,也可以將日誌寫入事件中心以使用事件中心(預覽)。 您可以設定這些選項的任何組合，並將稽核記錄寫入至每個組合。
...
- 保留期的預設值為 0(無限制保留)。 在配置存儲帳戶進行審核時,可以通過在**存儲設置**中移動**保留(天)** 滑塊來更改此值。
  - 如果將保留期從 0(無限制保留)更改為任何其他值,請注意,保留僅適用於在保留值更改後編寫的日誌(即使在啟用保留期後,保留保留期間編寫的日誌)。

  ![storage account](./media/sql-database-auditing-get-started/auditing_select_storage.png)

#### <a name="remarks"></a>備註

- 稽核紀錄寫入 Azure 訂閱上的 Azure Blob 儲存中的**新增 Blob**
- 要按照[Azure 儲存提供的說明](https://docs.microsoft.com/azure/storage/blobs/storage-blob-immutability-policies-manage#enabling-allow-protected-append-blobs-writes)為伺服器或資料庫級審核事件配置不可變日誌儲存(請確保在配置不可變 Blob 儲存時已選擇 **「允許其他追加服務**」) 。
- 可以將審核日誌寫入 VNet 或防火牆後面的 Azure 儲存帳戶。 有關具體說明,請參閱[將審核寫入 VNet 和防火牆後面的儲存帳戶](create-auditing-storage-account-vnet-firewall.md)。
...
- 然後,您有兩種方法來查看日誌:
    ...
    按一下**審核記錄**頁頂部的**日誌分析**將在日誌分析工作區中打開日誌檢視,您可以在其中自訂時間範圍和搜索查詢。
    
    ![在紀錄分析工作區中開啟](./media/sql-database-auditing-get-started/auditing-log-analytics.png)

    按下 **「審核記錄**」頁頂部的 **「查看儀錶板**」將打開一個儀表板,顯示審核日誌資訊,您可以在其中深入瞭解安全見解、訪問敏感資料等。 此儀錶板旨在説明您獲得數據的安全見解。
    您還可以自定義時間範圍和搜索查詢。 
    ![檢視紀錄分析儀表板](media/sql-database-auditing-get-started/auditing-view-dashboard.png)

    ![紀錄分析儀表板](media/sql-database-auditing-get-started/auditing-log-analytics-dashboard.png)
...
- 或者，您也可以從 Log Analytics 刀鋒視窗存取稽核記錄。 開啟 Log Analytics 工作區，然後在 [一般]**** 區段下，按一下 [記錄]****。 您可以從簡單的查詢開始，例如：「搜尋 "SQLSecurityAuditEvents"」** 以檢視稽核記錄。
    在此處,您還可以使用[Azure 監視器日誌](../log-analytics/log-analytics-log-search.md)對審核日誌數據運行進階搜索。 Azure 監視器日誌使用整合搜尋和自訂儀表板為您提供即時操作見解,以便輕鬆分析所有工作負荷和伺服器的數百萬條記錄。 有關 Azure 監視器紀錄搜尋語言與指令的其他有用資訊,請參閱[Azure 監視器紀錄搜尋參考](../log-analytics/log-analytics-log-search.md)。
...
- 稽核記錄會在您於設定期間選擇的帳戶中彙總。 可以使用[Azure 儲存資源管理員](https://storageexplorer.com/)等工具流覽審核日誌。 在 Azure 儲存體中，稽核記錄是以 Blob 檔案集合的方式儲存在名為 **sqldbauditlogs** 的容器內。 有關儲存資料夾的層次結構、命名約定和日誌格式的詳細資訊,請參閱[SQL 資料庫稽核紀錄格式](https://go.microsoft.com/fwlink/?linkid=829599)。
...
- 通過門戶或使用[Azure 儲存資源管理員](https://storageexplorer.com/)等工具從 Azure 儲存 Blob 容器下載日誌檔。
...
- [建立或更新資料庫稽核原則 (集-AzSql 資料庫審核)](/powershell/module/az.sql/set-azsqldatabaseaudit)
- [建立或更新伺服器稽核原則 (設定-AzSqlServer審核)](/powershell/module/az.sql/set-azsqlserveraudit)
- [取得資料庫稽核原則 (取得 AzSql 資料庫審核)](/powershell/module/az.sql/get-azsqldatabaseaudit)
- [取得伺服器稽核原則 (取得 AzSqlServer 稽核)](/powershell/module/az.sql/get-azsqlserveraudit)
- [刪除資料庫稽核原則 (刪除-AzSql 資料庫審核)](/powershell/module/az.sql/remove-azsqldatabaseaudit)
- [刪除伺服器稽核原則 (移除-AzSqlServer審核)](/powershell/module/az.sql/remove-azsqlserveraudit)
...
- [建立或更新資料庫*延伸稽核*原則](/rest/api/sql/database%20extended%20auditing%20settings/createorupdate)
- [建立或更新伺服器*擴充*原則](/rest/api/sql/server%20auditing%20settings/createorupdate)
...
- [部署啟用稽核的 Azure SQL 伺服器,將稽核紀錄寫入 Azure Blob 儲存帳戶](https://github.com/Azure/azure-quickstart-templates/tree/master/201-sql-auditing-server-policy-to-blob-storage)
...
> [!NOTE]
> 連結的範例位於外部公共存儲庫中,提供"有效",無保修,並且任何 Microsoft 支援計畫/服務都不支援。
