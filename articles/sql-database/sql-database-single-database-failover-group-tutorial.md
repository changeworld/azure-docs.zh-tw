...
[容錯移轉群組](sql-database-auto-failover-group.md)是宣告式抽象層，可讓您將多重異地複寫資料庫分組。 瞭解如何設定 Azure SQL Database 單一資料庫的容錯移轉群組，並使用 Azure 入口網站、PowerShell 或 Azure CLI 測試容錯移轉。  在本教學課程中，您將學會如何：

> [!div class="checklist"]
> - 建立 Azure SQL Database 單一資料庫。
> - 在兩個邏輯 SQL server 之間建立單一資料庫的容錯移轉群組。
> - 測試容錯移轉。
...
1. 選取在第1節中建立的單一資料庫， `mySampleDatabase`例如。 
