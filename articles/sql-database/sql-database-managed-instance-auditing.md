...
2. 建立用來儲存稽核記錄的 Azure 儲存體**容器**。
  ...
  > [!IMPORTANT]
  > 希望為其伺服器或資料庫級審核事件配置不可變日誌存儲的客戶應遵循[Azure 存儲提供的說明](https://docs.microsoft.com/azure/storage/blobs/storage-blob-immutability-policies-manage#enabling-allow-protected-append-blobs-writes)（請確保在配置不可變 blob 存儲時已選擇 **"允許其他追加"**
  
3. 在為稽核記錄建立容器之後，有兩個方法可將該容器設定為稽核記錄的目標：[使用 T-SQL](#blobtsql) 或[使用 SQL Server Management Studio (SSMS) UI](#blobssms)：
   ...
   - <a id="blobtsql"></a>使用 T-SQL 來設定稽核記錄的 Blob 儲存體：
     ...
     1. 產生 Azure 儲存體 **SAS 權杖**來將受控執行個體稽核存取權限授予儲存體帳戶：
        ...
        - 請依照下列方式設定 SAS：
          ...
          - **開始日期**：為了避免時區相關問題，建議使用昨天的日期

          - **結束日期**：選擇此 SAS 權杖過期的日期
   ...
   - <a id="blobssms"></a>使用 SQL Server Management Studio (SSMS) 18 (預覽) 來設定稽核記錄的 Blob 儲存體：
     ...
     1. 從下拉式清單中選取訂用帳戶、儲存體帳戶和 Blob 容器，或者，藉由按一下 [建立]**** 來建立您自己的容器。 當您完成後，請按一下 [確定]****：
        ...
        ![選擇 Azure 訂閱、存儲帳戶和 Blob 容器](./media/sql-managed-instance-auditing/13_mi_SSMS_select_subscription_account_container.png)

     1. 在 [建立稽核] 對話方塊中按一下 [確定]****。

4. <a id="createspec"></a>將 Blob 容器配置為稽核記錄的目標後，創建並啟用伺服器審核規範或資料庫審核規範，就像 SQL Server 一樣：
...
- [審核 Azure SQL 資料庫中的單個資料庫、彈性池和託管實例之間的差異以及 SQL Server 中的資料庫](#auditing-differences-between-databases-in-azure-sql-database-and-databases-in-sql-server)
- [CREATE SERVER AUDIT](https://docs.microsoft.com/sql/t-sql/statements/create-server-audit-transact-sql)
- [ALTER SERVER AUDIT](https://docs.microsoft.com/sql/t-sql/statements/alter-server-audit-transact-sql)

## <a name="set-up-auditing-for-your-server-to-event-hub-or-azure-monitor-logs"></a>將伺服器審核設置為事件中心或 Azure 監視器日誌

來自託管實例的稽核記錄可以發送到偶等中心或 Azure 監視器日誌。 本節說明如何進行此設定：
...
5. 為審核事件選擇目標 - 事件中心、Azure 監視器日誌或兩者。 為每個目標設定必要的參數 (例如 Log Analytics 工作區)。
...
9. 創建並啟用伺服器審核規範或資料庫審核規範，就像 SQL Server 一樣：
...
- 可以使用[Azure 存儲資源管理器](https://azure.microsoft.com/features/storage-explorer/)等工具流覽稽核記錄。 在 Azure 儲存體中，稽核記錄會以 Blob 檔案集合的方式儲存在定義來儲存稽核記錄的容器內。 如需有關儲存體資料夾階層、命名慣例、記錄格式的進一步詳細資訊，請參閱 [Blob 稽核記錄格式參考](https://go.microsoft.com/fwlink/?linkid=829599)。
...
### <a name="consume-and-analyze-logs-stored-in-azure-monitor-logs"></a>使用和分析存儲在 Azure 監視器日誌中的日誌

如果稽核記錄寫入 Azure 監視器日誌，則日誌日誌在日誌分析工作區中可用，您可以在該工作區中對審核資料運行高級搜索。 作為起點，導航到日誌分析工作區，並在 *"常規*"部分下按一下 *"日誌"* 並輸入一個簡單的查詢，例如`search "SQLSecurityAuditEvents"`：查看稽核記錄。  

Azure 監視器日誌使用集成搜索和自訂儀表板為您提供即時操作見解，以便輕鬆分析所有工作負荷和伺服器的數百萬條記錄。 有關 Azure 監視器日誌搜索語言和命令的其他有用資訊，請參閱[Azure 監視器日誌搜索引用](https://docs.microsoft.com/azure/azure-monitor/log-query/log-query-overview)。
...
- 提供了新的語法`TO EXTERNAL MONITOR`，以啟用"均勻中心"和"Azure 監視器"日誌目標。
...
- 如需稽核記錄耗用方法的完整清單，請參閱[開始使用 SQL 資料庫稽核](sql-database-auditing.md)。
- 有關支援標準合規性的 Azure 程式的詳細資訊，請參閱[Azure 信任中心](https://gallery.technet.microsoft.com/Overview-of-Azure-c1be3942)，您可以在其中查找最新的 SQL 資料庫合規性認證清單。
