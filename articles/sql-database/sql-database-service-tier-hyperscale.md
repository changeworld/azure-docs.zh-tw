...
- **存放裝置**：
  ...
  您設定超大規模資料庫時，不需要指定資料大小上限。 在超大規模層中,您需要根據實際分配為資料庫收取存儲費用。 儲存自動分配在 40 GB 和 100 TB 之間,以 10 GB 的增量 10 GB 分配。 如果需要,多個數據檔可以同時增長。 創建起始大小為 10 GB 的超大規模資料庫,每 10 分鐘開始增加 10 GB,直到達到 40 GB 的大小。
...
1. 如果目標區域中尚未有適當的伺服器,則在目標區域中創建 SQL 資料庫伺服器。  此伺服器應由與原始(源)伺服器相同的訂閱擁有。
...
| 變更追蹤 | 更改追蹤當前處於公共預覽版中,可在新資料庫或現有超大規模資料庫上啟用。 |
