...
| [**快照**](https://docs.microsoft.com/sql/relational-databases/replication/snapshot-replication) | 是 (僅作為訂閱者) | 是|
...
| &nbsp; | &nbsp; | &nbsp; |
  ...
  ### <a name="supportability-matrix-for-instance-databases-and-on-premises-systems"></a>實例資料庫和本地系統的可支援性矩陣
...
- 埠 445（TCP 出站）需要在託管實例子網的安全規則中打開才能訪問 Azure 檔共用。 
- 如果發佈伺服器/分發伺服器位於託管實例上且訂閱伺服器位於本地，則需要打開端口 1433（TCP 出站）。
- 所有類型的複製參與者（發佈伺服器、分發伺服器、拉取訂閱伺服器和發送訂閱伺服器）都可以放置在託管實例上，但發行者和分發伺服器必須同時位於雲中或同時位於本地。
- 如果發佈商、分發伺服器和/或訂閱者存在於不同的虛擬網路中，則必須在每個實體之間建立 VPN 對等互連，以便發行者和分發伺服器之間存在 VPN 對等互連，並且/或分發伺服器和訂閱者之間存在 VPN 對等互連。 


>[!NOTE]
> - 如果分發伺服器是實例資料庫且訂閱者位於本地時，出站網路安全性群組 （NSG） 埠 445 被阻止，則連接到 Azure 存儲檔時可能會遇到錯誤 53。 [更新 vNet NSG](/azure/storage/files/storage-troubleshoot-windows-file-connection-problems)以解決此問題。 
...
### <a name="publisher-with-local-distributor-on-a-managed-instance"></a>在託管實例上具有本地分發伺服器的發佈伺服器
...
### <a name="publisher-with-remote-distributor-on-a-managed-instance"></a>在託管實例上具有遠端分發伺服器的發佈伺服器

在此配置中，一個託管實例發佈對放置在另一個託管實例上的分發器的更改，該實例可以服務許多源託管實例，並將更改分發到託管實例、單個資料庫、池資料庫或Sqlserver。

![發行者和散發者使用個別的執行個體](media/replication-with-sql-database-managed-instance/02-separate-instances-asdbmi-pubdist.png)

在兩個受控執行個體上設定發行者和散發者。 此配置存在一些約束： 
...
## <a name="with-failover-groups"></a>帶容錯移轉組

如果在[容錯移轉組中](sql-database-auto-failover-group.md)的**發行者**或**分發商**實例上啟用了異地複製，則託管實例管理員必須清理舊主伺服器上的所有發佈，並在容錯移轉發生後在新主伺服器上重新配置它們。 在這種情況下，需要以下活動：

1. 停止在資料庫上運行的所有複製作業（如果有）。
2. 通過在發行者資料庫上運行以下腳本從發行者中刪除訂閱中繼資料：

   ```sql
   EXEC sp_dropsubscription @publication='<name of publication>', @article='all',@subscriber='<name of subscriber>'
   ```             
 
1. 從訂閱伺服器刪除訂閱中繼資料。 在訂閱伺服器上的訂閱資料庫上運行以下腳本：
...
1. 通過在已發佈的資料庫中運行以下腳本，強制從發行者中刪除所有複製物件：

   ```sql
   EXEC sp_removedbreplication
   ```

1. 強制將舊分發伺服器從原始主實例中刪除（如果失敗返回以前具有分發伺服器的舊主資料庫）。 在舊分發伺服器託管實例的主資料庫上運行以下腳本：

   ```sql
   EXEC sp_dropdistributor 1,1
   ```

如果在容錯移轉組中的**訂戶**實例上啟用了異地複製，則應將發佈配置為連接到訂閱伺服器託管實例的容錯移轉組攔截器終結點。 在發生容錯移轉時，託管實例管理員的後續操作取決於發生的容錯移轉的類型： 

- 對於沒有資料丟失的容錯移轉，複製將在容錯移轉後繼續工作。 
- 對於具有資料丟失的容錯移轉，複製也將有效。 它將再次複製丟失的更改。 
- 對於具有資料丟失的容錯移轉，但資料丟失超出散發資料庫保留期，託管實例管理員將需要重新初始化訂閱資料庫。 

## <a name="next-steps"></a>後續步驟

- [配置 MI 發行者和訂閱者之間的複製](replication-with-sql-database-managed-instance.md)
- [配置 MI 發行者、MI 分發伺服器和 SQL Server 訂閱者之間的複製](sql-database-managed-instance-configure-replication-tutorial.md)
...
- [使用 MI 和容錯移轉組的複製](sql-database-managed-instance-transact-sql-information.md#replication)
- [複寫至 SQL Database](replication-to-sql-database.md)
- [複製到託管實例](replication-with-sql-database-managed-instance.md)
- [創建出版物](https://docs.microsoft.com/sql/relational-databases/replication/publish/create-a-publication)
