...
在 Azure 中，您可以選擇使用[角色型存取控制 (RBAC)][rbac-what-is-813s] 來建立單一自訂安全性角色，而且只給予一部分必要的能力。 可以使用自訂角色，而不是涉及網路系統管理員或資料庫管理員。如果將使用者添加到自訂角色，則將使用者添加到其他兩個主要管理員角色時，安全暴露的表面區域較低。
...
> [!IMPORTANT]
> Azure SQL 資料庫仍然支援 PowerShell Azure 資源管理器模組，但所有後續開發都針對 Az.Sql 模組。 AzureRM 模組將繼續接收錯誤修復，至少直到 2020 年 12 月。  Az 模組和 AzureRm 模組中命令的參數基本相同。 有關其相容性的更多資訊，請參閱[介紹新的 Azure PowerShell Az 模組](/powershell/azure/new-azureps-module-az)。

#### <a name="steps"></a>步驟

1. 在 PowerShell 中，使用 Azure 活動目錄 （AAD） 註冊託管 Azure SQL 資料倉儲實例的**Azure SQL 伺服器**：
...
1. 請瀏覽至您儲存體帳戶之下的 [存取控制 \(IAM\)]****，然後按一下 [新增角色指派]****。 將**存儲 Blob 資料參與者**RBAC 角色指派給託管 Azure SQL 資料倉儲的 Azure SQL 伺服器，該倉庫已註冊到 Azure 活動目錄 （AAD）， 如步驟#1。
...
1. **Polybase 連線至 Azure 儲存體帳戶：**
   ...
   1. 使用 PolyBase`abfss://`創建具有連接到通用 v2 存儲帳戶的方案的外部資料源：
       ...
       > [!NOTE]
       > - 如果您已有與一般用途 v1 或 Blob 儲存體帳戶相關聯的外部資料表，請先卸除這些外部資料表，再卸除對應的外部資料來源。 然後創建外部資料源，方案`abfss://`連接到通用 v2 存儲帳戶，並使用此新的外部資料源重新創建所有外部表。 您可以使用[產生和發佈指令碼精靈](https://docs.microsoft.com/sql/ssms/scripting/generate-and-publish-scripts-wizard)，輕鬆地為所有的外部資料表產生建立指令碼。
       > - 有關`abfss://`方案的更多資訊，請參閱[本指南](https://docs.microsoft.com/azure/storage/data-lake-storage/introduction-abfs-uri)。
...
腳本還可以使用 PowerShell Cmdlet **New-AzSqlServer 虛擬網路規則**或[az 網路 vnet 創建創建虛擬網路](/cli/azure/network/vnet#az-network-vnet-create)規則。 如有興趣，請參閱[使用 PowerShell 建立 Azure SQL Database 的虛擬網路服務端點和規則][sql-db-vnet-service-endpoint-rule-powershell-md-52d]。
...
1. 登錄到 Azure[門戶][http-azure-portal-link-ref-477t]。

2. 搜索並選擇**SQL 伺服器**，然後選擇伺服器。 在 **"安全"** 下，選擇**防火牆和虛擬網路**。

3. 將 [允許存取 Azure 服務]**** 控制項設為 [關閉]。

    > [!IMPORTANT]
    > 如果將控制項集保留為 ON，Azure SQL 資料庫伺服器將接受來自 Azure 邊界內任何子網的通信，即來自識別為 Azure 資料中心定義範圍內的 IP 位址之一的通信。 就安全性觀點而言，讓此控制項保持 [開啟] 可能使存取過多。 Microsoft Azure 虛擬網路服務端點功能，連同 SQL Database 的虛擬網路規則功能，可縮小安全性曝露面。
...
- [使用 PowerShell 創建虛擬網路服務終結點，然後創建 Azure SQL 資料庫的虛擬網路規則。][sql-db-vnet-service-endpoint-rule-powershell-md-52d]
