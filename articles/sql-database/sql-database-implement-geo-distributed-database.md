...
>
> 如需詳細資訊，請參閱[建立資料庫層級防火牆規則](/sql/relational-databases/system-stored-procedures/sp-set-database-firewall-rule-azure-sql-database)或若要判斷用於電腦的伺服器層級防火牆規則的 IP 位址，請參閱[建立伺服器層級防火牆](sql-database-server-level-firewall-rule.md)。  
...
> [使用 DMS 將 SQL Server 遷移至 Azure SQL database 受控實例](../dms/tutorial-sql-server-to-managed-instance.md)
