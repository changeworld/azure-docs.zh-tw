...
高級數據安全 (ADS) 提供一組高級 SQL 安全功能,包括數據發現&分類、漏洞評估和高級威脅防護。

- [數據發現&分類](sql-database-data-discovery-and-classification.md)提供了內置於 Azure SQL 資料庫中的功能,用於發現、分類和標記&報告資料庫中的敏感數據。 它可用來讓您檢視資料庫分類狀態、追蹤對資料庫內敏感性資料的存取，並具有其他多方面的用途。
- [漏洞評估](sql-vulnerability-assessment.md)是一種易於配置的服務,可以發現、跟蹤並説明您修復潛在的資料庫漏洞。 它可讓您檢視安全性狀態，且包含解決安全性問題和增強資料庫防護性的可行步驟。
...
只要啟用 SQL ADS，其中包含的所有功能即會一併啟用。 只要按一下按鍵，即可對 SQL Database 伺服器或受控執行個體上的所有資料庫啟用 ADS。 啟用或管理 ADS 設置需要屬於[SQL 安全管理員](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#sql-security-manager)角色、SQL 資料庫管理員角色或 SQL 伺服器管理員角色。 
...
通過在 SQL 資料庫伺服器或託管實例**的安全**標題下導航到**進階資料安全**,啟用 ADS。 若要對資料庫伺服器或受控執行個體上的所有資料庫啟用 ADS，請按一下 [在伺服器上啟用進階資料安全性]****。
...
## <a name="2-start-classifying-data-tracking-vulnerabilities-and-investigating-threat-alerts"></a>2. 開始對資料進行分類、追蹤漏洞和調查威脅警報

按一下 [資料探索與分類]**** 卡可查看建議要分類的敏感性資料行，以及使用持續敏感性標籤為資料分類。 按一下 [弱點評估]**** 卡可檢視和管理弱點掃描和報告，以及追蹤您的安全性水準。 如果已收到安全警報,請單擊**高級威脅保護**卡以查看警報的詳細資訊,並通過 Azure 安全中心安全警報頁面查看有關 Azure 訂閱中所有警報的整合報告。

## <a name="3-manage-ads-settings-on-your-sql-database-server-or-managed-instance"></a>3. 管理 SQL 資料庫伺服器或託管實例上的 ADS 設定

若要檢視及管理 ADS 設定，請針對您的 SQL Database 伺服器或受控執行個體，瀏覽至 [安全性]**** 標題下的 [進階資料安全性]****。 在此頁上,您可以啟用或禁用 ADS,並修改整個 SQL 資料庫伺服器或託管實例的漏洞評估和進階威脅保護設定。
...
若要覆寫特定資料庫的 ADS 設定，請勾選 [啟用資料庫層級的進階資料安全性]**** 核取方塊。 僅當有特定要求為各個資料庫接收單獨的高級威脅保護警報或漏洞評估結果,以代替或除了資料庫伺服器或託管實例上所有資料庫收到的警報和結果時,才使用此選項。

選中該複選框後,即可配置此資料庫的相關設置。
...
- 瞭解有關[資料發現&分類](sql-database-data-discovery-and-classification.md) 
- 瞭解有關[漏洞評估](sql-vulnerability-assessment.md)的更多 
