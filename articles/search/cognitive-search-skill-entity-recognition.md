...
|includeTypelessEntities | 如果您`true`想要辨識出不符合目前類別的知名實體，請設定為。 已辨識的`entities`實體會在複雜輸出欄位中傳回。 例如，「Windows 10」是知名實體（產品），但由於「產品」不是支援的分類，因此此實體會包含在 [實體] 輸出欄位中。 預設為 `false` |
...
> [!NOTE]
> 並非所有的實體類別都支援所有語言。 上述`"Person"`語言`"Location"`的完整`"Organization"`清單支援、和實體類別目錄類型。 只有_de_、 _en_、 _es_、 _fr_和_zh-hans_支援`"Quantity"`、 `"Datetime"`、 `"URL"`和`"Email"`類型的解壓縮。 如需詳細資訊，請參閱[文字分析 API 的語言和區域支援](https://docs.microsoft.com/azure/cognitive-services/text-analytics/language-support)。  
