...
記錄的大小上限應為50000個字元，如所測量[`String.Length`](https://docs.microsoft.com/dotnet/api/system.string.length)。 如果您需要在將資料傳送至技能之前先將其分解，請考慮使用[文字分割技能](cognitive-search-skill-textsplit.md)。
...
| minimumPrecision | 介於0.0 和1.0 之間的值。 如果信賴分數（在`piiEntities`輸出中）低於設定`minimumPrecision`值，則不會傳回或遮罩該實體。 預設值為 0.0。 |
| maskingMode | 提供各種方式來遮罩輸入文字中偵測到之 PII 的參數。 支援下列選項： <ul><li>`none`（預設值）：這表示不會執行任何遮罩，也`maskedText`不會傳回輸出。 </li><li> `redact`：此選項會從輸入文字中移除偵測到的實體，而不會將其取代為任何專案。 請注意，在此情況下， `piiEntities`輸出中的位移會與原始文字相關，而不是遮罩文字。 </li><li> `replace`：此選項會將偵測到的實體取代為`maskingCharacter`參數中指定的字元。  字元將會重複為偵測到之實體的長度，因此位移會正確對應至輸入文字和輸出`maskedText`。</li></ul> |
| maskingCharacter | 如果`maskingMode`參數設定為`replace`，將用來遮罩文字的字元。 支援下列選項： `*` （預設）、 `#`、。 `X` 只有在未設定為`null` `replace`時`maskingMode` ，這個參數才可以是。 |
...
| maskedText | 如果`maskingMode`設定為以外的值`none`，則此輸出將會是在輸入文字上執行之遮罩的字串結果，如選取`maskingMode`的所述。  如果`maskingMode`設定為`none`，則不會出現此輸出。 |
