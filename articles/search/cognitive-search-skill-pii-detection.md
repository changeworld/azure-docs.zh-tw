...
description: 在 Azure 認知搜尋中，從擴充管線中的文字解壓縮及遮罩個人識別資訊。 此技能目前為公開預覽狀態。
manager: nitinme
author: careyjmac
ms.author: chalton
...
**PII 偵測**技能可從輸入文字中取出個人識別資訊，並可讓您選擇以各種方式遮罩該文字。 這項技能會使用認知服務中[文字分析](../cognitive-services/text-analytics/overview.md)所提供的機器學習模型。
...
記錄的大小上限應該是 50,000 個字元 (以 [`String.Length`](/dotnet/api/system.string.length) 為測量單位)。 如果您需要先分割資料，再將其傳送至技能，請考慮使用 [文字分割技能](cognitive-search-skill-textsplit.md)。
...
| `maskingMode` | 參數，提供各種方法來遮罩輸入文字中偵測到的 PII。 支援下列選項： <ul><li>`none` (預設) ：這表示不會執行任何遮罩，也 `maskedText` 不會傳回輸出。 </li><li> `redact`：此選項會從輸入文字中移除偵測到的實體，而不會將它們取代為任何專案。 請注意，在此情況下，輸出中的位移 `piiEntities` 會相對於原始文字，而不是遮罩文字。 </li><li> `replace`：此選項會使用參數中指定的字元來取代偵測到的實體 `maskingCharacter` 。  字元將會重複為所偵測到之實體的長度，因此位移會正確地對應至輸入文字和輸出 `maskedText` 。</li></ul> |
...
如果技能傳回警告，則輸出 `maskedText` 可能是空的。  這表示，如果您預期輸出會存在於未來的技能中，它將無法如預期般運作。 撰寫您的技能集定義時，請記住這一點。
