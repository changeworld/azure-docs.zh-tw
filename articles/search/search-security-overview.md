...
針對搜尋服務內部處理的資料，下表描述 [資料加密模型](../security/fundamentals/encryption-models.md)。 某些功能（例如知識存放區、累加擴充和以索引子為基礎的索引、讀取或寫入其他 Azure 服務中的資料結構）。 這些服務有自己的加密支援層級，與 Azure 認知搜尋不同。
