...
本文適用于需要更深入瞭解技能集概念和組合的開發人員，並假設您熟悉 AI 擴充流程。 如果您不熟悉這個概念，請從 [Azure 認知搜尋的 AI 擴充](cognitive-search-concept-intro.md)開始。

## <a name="introducing-skillsets"></a>技能集簡介

技能集是附加至索引子的 Azure 認知搜尋中可重複使用的資源，並指定在編制索引期間用來分析、轉換及豐富文字或影像內容的技能集合。 技能具有輸入和輸出，而某項技能的輸出通常會在一或多個處理常式中變成另一個技能的輸入。

技能集有三個主要屬性：

+ `skills`是一組未排序的技能集合，平臺會根據每項技能所需的輸入來決定執行順序。
+ `cognitiveServices`是認知服務資源的索引鍵，可針對包含內建技能的技能集執行影像和文字處理。
+ `knowledgeStore`， (選擇性的) 將會投射擴充檔的 Azure 儲存體帳戶。 搜尋索引也會耗用擴充的檔。
...
+ 技能 #1 是 [文字分割技能](cognitive-search-skill-textsplit.md) ，可接受 "reviews_text" 欄位的內容做為輸入，並將該內容分割為5000個字元的「頁面」作為輸出。
+ 技能 #2 是 [情感偵測技能](cognitive-search-skill-sentiment.md) 接受「頁面」作為輸入，並產生稱為 "情感" 的新欄位作為輸出，其中包含情感分析的結果。
...
```
> [!NOTE]
> 您可以使用 [條件式技能](cognitive-search-skill-conditional.md) 來建立運算式，以利用迴圈和分支來建立複雜的技能集。 語法是以 [JSON 指標](https://tools.ietf.org/html/rfc6901) 路徑標記法為基礎，但有幾項修改可識別擴充樹狀結構中的節點。 `"/"` 會在樹狀結構中向下周遊一個層級，而 `"*"` 則會作為內容中的 for-each 運算子。 本文中的許多範例都會說明語法。 

### <a name="enrichment-tree"></a>擴充樹狀結構

在 [擴充管線的步驟](cognitive-search-concept-intro.md#enrichment-steps)進展中，內容處理會遵循 *檔破解* 階段，其中文字和影像會從來源中解壓縮。 然後，您可以將影像內容路由傳送至指定影像處理的技能，而文字內容會排入文字處理的佇列。 對於包含大量文字的來源文件，您可以在索引子上設定 *剖析模式* ，以將文字分成較小的區塊，以提供更佳的處理。 
...
+ 輸入的圖形。 對於多層集合，將內容設定為父系集合會影響技能輸入的圖形。 例如，如果您有一個具有國家/地區清單的擴充樹狀結構，每個樹狀結構都會以包含郵遞區號清單的狀態清單進行擴充。
...
使用 [旅館評論技能集](https://github.com/Azure-Samples/azure-search-sample-data/blob/master/hotelreviews/HotelReviews_skillset.json) 作為參考點，我們將探討：

+ 擴充樹狀結構如何隨著每項技能的執行而演進
+ 內容和輸入如何運作，以判斷技能執行多少次
+ 輸入的圖形是以內容為基礎

擴充流程中的「檔」代表在 hotel_reviews.csv 原始程式檔中， (飯店審核) 的單一資料列。

### <a name="skill-1-split-skill"></a>技能 #1：分割技能

當來源內容包含大量文字時，將它分成較小的元件，以提供更高的語言、情感和關鍵字組偵測的精確度，會很有説明。 有兩個可用的粒紋：頁面和句子。 頁面包含大約5000個字元。
...
在的技能內容 `"/document/reviews_text"` 中，分割技能會針對執行一次 `reviews_text` 。 技能會輸出為一份清單，將 `reviews_text` 分成 5000 個字元區段。 分割技能的輸出會命名為 `pages` ，並新增至擴充樹狀結構。 `targetName` 功能可讓您先將技能輸出重新命名，再新增至擴充樹狀結構。
...
旅館評論檔包含以多種語言表示的客戶意見反應。 語言偵測技能會決定所使用的語言。 然後，結果會傳遞至關鍵字組的解壓縮和情感偵測，並在偵測情感和片語時，考慮採用語言。
...
在 Azure 認知搜尋中，索引子會儲存其所建立的輸出。 其中一個輸出一律是可搜尋的 [索引](search-what-is-an-index.md)。 指定索引是一項需求，而當您附加技能集時，索引所內嵌的資料就會包含擴充的物質。 通常，特定技能（例如關鍵字組或情感分數）的輸出會內嵌到針對該目的所建立之欄位中的索引。

索引子也可以選擇性地將輸出傳送至 [知識存放區](knowledge-store-concept-intro.md) ，以供其他工具或進程取用。 知識存放區會定義為技能集的一部分。 它的定義會決定是否將擴充的檔投射為 (檔案或 blob) 的資料表或物件。 表格式投影適用于 Power BI 等工具中的互動式分析，而檔案和 blob 通常用於資料科學或類似的進程。 在本節中，您將瞭解技能集組合如何塑造您要投影的資料表或物件。

### <a name="projections"></a>投影

針對以知識存放區為目標的內容，您會想要考慮內容的結構。 *投射* 是從擴充樹狀目錄中選取節點，並在知識存放區中建立這些節點之實體運算式的流程。 投影是文件 (內容和擴充) 的自訂圖形，可以作為表格或物件投影的輸出。 若要深入了解如何使用投影，請參閱[使用投影](knowledge-store-projection-overview.md)。

![欄位對應選項](./media/cognitive-search-working-with-skillsets/field-mapping-options.png "擴充管線的欄位對應選項")

### <a name="sourcecontext"></a>SourceContext

`sourceContext`元素只用于技能輸入和預測。 它是用來建構多層級的巢狀物件。 您可能需要建立新的物件，以將其傳遞為對技能的輸入或投影至知識存放區。 由於擴充節點在擴充樹狀結構中可能不是有效的 JSON 物件，且參考樹狀結構中的節點只會傳回該節點在建立時的狀態，您會需要建立格式正確的 JSON 物件才能使用擴充內容作為技能輸入或投影。 此 `sourceContext` 可讓您建構階層式匿名型別物件，如果您只使用內容，就需要多個技能。 
...
+ 您可以使用文字整形器技能來建立新節點，這是您所投射之所有擴充的根節點。 然後，在您的投影中，您只會參考塑形器技能的輸出。

+ 在投射定義本身的投影中使用內嵌圖形。

塑形器方法比內嵌塑形冗長，但可確保擴充樹狀結構的所有變動都包含在技能內，而且輸出是可以重複使用的物件。 相反地，內嵌成形可讓您建立所需的形狀，但它是一個匿名物件，只適用于其定義所在的投射。 這些方法可以一起使用或分開使用。 在入口網站工作流程中為您建立的技能集包含這兩者。 它將塑形器技能用於資料表投影，但也使用內嵌塑形來投影關鍵片語資料表。
