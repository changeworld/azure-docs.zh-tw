...
description: 在與 Azure 認知搜索集成的搜索應用程式中添加用於自導篩選的分面導航。
manager: nitinme
author: HeidiSteen
ms.author: heidist
...
---
# <a name="how-to-implement-faceted-navigation-in-azure-cognitive-search"></a>如何在 Azure 認知搜索中實現分面導航

多面向導覽是一個篩選機制，它在搜尋應用程式中提供自動導向的向下鑽研導覽。 「多面向導覽」一詞可能讓您感到陌生，但您可能早已使用過它。 如下列範例所示，多面向導覽其實就是用來篩選結果的類別。

 ![Azure 認知搜索作業門戶演示](media/search-faceted-navigation/azure-search-faceting-example.png "Azure 認知搜索作業門戶演示")

多面向導覽是替代的搜尋進入點。 它提供了方便的替代功能，讓您不必手動輸入複雜搜尋運算式。 方面可以説明您找到您要查找的內容，同時確保不會獲得零結果。 作為開發人員，分面允許您公開導航搜索索引的最有用的搜尋條件。 在線上零售應用程式中，分面導航通常建立在品牌、部門（兒童鞋）、規模、價格、受歡迎程度和評級之上。 

實作多面向導覽會因各搜尋技術而不同。 在 Azure 認知搜索中，分面導航是在查詢時構建的，使用以前在架構中屬性化的欄位。
...
- 在[Azure 認知搜索作業門戶演示](https://aka.ms/azjobsdemo)中線上查看和測試工作演示。

- 從 [GitHub 上的 Azure 範例儲存機制](https://github.com/Azure-Samples/search-dotnet-asp-net-mvc-jobs)下載程式碼。

## <a name="get-started"></a>開始使用
如果您不了解搜尋開發，最好的方法就是把多面向導覽想成它會顯示自動導向搜尋的可能選項。 它是一種基於預定義篩選器的向下切入搜索體驗，用於通過點擊點和按一下操作快速縮小搜尋結果範圍。 

### <a name="interaction-model"></a>互動模型

分面導航的搜索體驗是反覆運算的，因此讓我們首先將其理解為回應使用者操作展開的查詢序列。

起點是一個提供多面向導覽 (通常位在側邊) 應用程式頁面。 多面向導覽通常是樹狀結構，且每個值有核取方塊，或可點選的文字。 

1. 發送到 Azure 認知搜索的查詢通過一個或多個分面查詢參數指定分面導航結構。 例如，查詢可能包括 `facet=Rating`，可能還有 `:values` 或 `:sort` 選項，以進一步精簡展示。
...
面向是查詢參數，但不要將它與查詢輸入混淆了。 它在查詢中不是當作選取準則。 反之，將面向查詢參數想成是，返回的回應中所包含對導覽結構的輸入。 對於提供的每個面查詢參數，Azure 認知搜索評估每個分面值的部分結果中有多少個文檔。
...
在 Azure 認知搜索中，請求通過一個或多個查詢參數指定（請參閱[搜索文檔](https://docs.microsoft.com/rest/api/searchservice/Search-Documents)，瞭解每個查詢參數的說明）。 沒有任何一個查詢參數是必要的，但您至少要有一個才能讓查詢有效。
...
這兩個查詢都有效，但如果您正在尋找在西雅圖有停車位的非汽車旅館，則第二個問題是優越的。
...
在生成搜索請求的應用程式代碼中，使用 Azure 認知搜索實現分面導航。 多面向導覽會依賴您先前在結構描述中定義的元素。

`Facetable [true|false]` 索引屬性預先定義在您的搜尋索引中，且設定在所選取的欄位，以便在多面向導覽結構中啟用或停用他們。 若沒有 `"Facetable" = true`，該欄位就不能在多面向導覽中使用。

您程式碼中的展示層提供使用者經驗。 它應該列示多面向導覽的組成部份，例如標籤、值、核取方塊和計數。 Azure 認知搜索 REST API 與平臺無關，因此請使用所需的任何語言和平臺。 重要的是要包含支援累加式重新整理的 UI 元素，當選取額外的面向便會更新 UI 狀態。 
...
Azure 認知搜索基於您輸入的一個或多個字詞以及對分面導航結構的更新返回搜尋結果。 在 Azure 認知搜索中，分面導航是單級構造，具有分面值，並計算每個層找到的結果數。
...
根據預設，所有可能用於多面向導覽的欄位類型為 `Facetable` 。 此類欄位類型包括`Edm.String` `Edm.DateTimeOffset`、和所有數值欄位類型（基本上，除 外`Edm.GeographyPoint`，所有欄位類型都是可面的，不能用於分面導航）。 

在建置索引時，多面向導覽的最佳作法是關閉不應做為面向之欄位的多面向功能。  尤其是單一值的字串欄位 (例如識別碼或產品名稱)，應該將它們設為 `"Facetable": false` 以防止意外被用於多面向導覽。 在不需要的地方關閉面，有助於減小索引的大小，並且通常可以提高性能。
...
如您在示例架構中看到的，`Facetable`對於不應用作分面（如 ID 值）的字串欄位，將關閉。 在不需要的地方關閉面，有助於減小索引的大小，並且通常可以提高性能。
...
對於 Web 應用程式，展示層通常是使用 AJAX，因為它可讓您重新整理累加的變更。 您還可以使用ASP.NET MVC 或任何其他視覺化平臺，這些平臺可以通過 HTTP 連接到 Azure 認知搜索服務。 本文中引用的應用程式範例 （ **Azure 認知搜索作業門戶演示**） 恰好是一個ASP.NET MVC 應用程式。
...
面向查詢參數已設定至欄位，且根據資料類型能夠由包含 `count:<integer>`、`sort:<>`、`interval:<integer>` 與 `values:<list>` 的逗號分隔清單進一步參數化。 當設定範圍時，針對數值資料支援值清單。 有關使用方式詳細資訊[，請參閱搜索文檔（Azure 認知搜索 API）。](https://docs.microsoft.com/rest/api/searchservice/Search-Documents)
...
如附註所說明，階層中沒有直接支援巢狀面向。 預設情況下，Azure 認知搜索中的分面導航僅支援一級篩選器。 不過有因應措施。 您可以在 `Collection(Edm.String)` 中利用各階層各一個進入點來編碼階層面向。 此因應措施的實作不是本文的討論範圍。 
...
**使用篩選條件提高搜尋精準度**
 ...
 使用篩選條件。 如果僅依賴搜索運算式，則詞幹可能會導致返回的文檔，該文檔在其任何欄位中沒有精確的分面值。
...
* `&facet=city,count:5` 指定只有最高排序結果中的前 5 個城市會傳回為面向結果。 考慮一個搜索詞為"機場"和 32 個匹配項的依例查詢。 若查詢指定 `&facet=city,count:5`，則只有搜尋結果中包含最多文件的前五個特定城市會包含在面向結果中。
...
> [!NOTE]
> 當有一種類型以上時討論 `count` 可能會讓人混淆。 下表簡要概述了如何在 Azure 認知搜索 API、示例代碼和文檔中使用該術語。 
...
* "`@odata.count`"<br/>
   在查詢回應中，此值表示搜尋結果中相符項目的數目。 平均而言，它大於所有分面結果的總和，因為存在與搜索詞匹配但沒有分面值匹配的專案。

**取得面向結果中的計數**

新增篩選條件至多面向查詢時，您可能希望保留面向陳述式 (例如，`facet=Rating&$filter=Rating ge 4`)。 從技術上講，不需要分面_評級，但保留它返回評級 4 和更高等級的分面值計數。 例如，如果您按一下 "4" 且查詢包含大於或等於 "4" 的篩選條件，則會針對等於或大於 4 的各分級傳回計數。  

**確實取得精確的面向計數**

在某些情況下，您可能會發現面計數與結果集不匹配（請參閱[Azure 認知搜索（論壇帖子）中的分面導航](https://social.msdn.microsoft.com/Forums/azure/06461173-ea26-4e6a-9545-fbbd7ee61c8f/faceting-on-azure-search?forum=azuresearch)）。

面向計數可能會因為分區結構而不正確。 每個搜尋索引都有多個分區，且每個分區都會依照文件計數報告前 N 個面向，然後結合為單一結果。 如果一些分區有許多相符值，而一些有較少相符值，您可能發現結果中有一些面向值遺失或短少。

儘管此行為隨時可能更改，但如果您今天遇到此行為，可以通過人為地將計數膨脹來解決此問題：>\<到大量，以強制從每個分片進行完整報告。 如果計數: 的值大於或等於欄位中唯一值的數目，您就能保證結果正確。 不過，當文件計數很高的時候，則會有效能的負面影響，因此請謹慎使用此選項。

### <a name="user-interface-tips"></a>使用者介面秘訣
**針對多面向導覽中的各欄位新增標籤**

標籤通常以 HTML 或表單定義 (應用程式範例中為 `index.cshtml`)。 Azure 認知搜索中沒有針對分面導航標籤或任何其他中繼資料的 API。

<a name="rangefacets"></a>

## <a name="filter-based-on-a-range"></a>根據範圍進行篩選
透過值範圍進行面向化是常見的搜尋應用程式需求。 範圍支援數值資料與日期時間值。 您可以在[搜索文檔（Azure 認知搜索 API）](https://docs.microsoft.com/rest/api/searchservice/Search-Documents)中瞭解有關每種方法的更多資訊。

Azure 認知搜索提供了兩種計算範圍的方法，從而簡化了範圍構造。 對於這兩種方法，Azure 認知搜索會根據您提供的輸入創建適當的範圍。 例如，如果您指定 10|20|30 的值範圍，它會自動建立 0-10、10-20、20-30 的範圍。 您的應用程式可以選擇性地移除任何空白間隔。 
...
已使用 0 做為起始點，清單中的值做為結束點建置各範圍，並且修剪之前的範圍來建立離散間隔。 Azure 認知搜索作為分面導航的一部分執行這些操作。 您不需要撰寫程式碼來建構各個間隔。
...
通常可以看到篩選器，這些篩選器可説明您根據商店、餐廳或目的地的鄰近位置來選擇它。 雖然這種類型的篩選器可能看起來像分面導航，但它只是一個篩選器。 對於那些特別要尋找特定設計問題之實作建議的使用者，我們會在此處說明。

Azure 認知搜索、**地理.距離**和**地理相交**中有兩個地理空間函數。
...
您可以在[OData 運算式語法（Azure 認知搜索）](query-odata-filter-orderby-syntax.md)中找到篩選器示例。

<a name="tryitout"></a>

## <a name="try-the-demo"></a>試用示範
Azure 認知搜索作業門戶演示包含本文中引用的示例。
...
觀看[Azure 認知搜索深度潛水](https://channel9.msdn.com/Events/TechEd/Europe/2014/DBI-B410)。 在 45:25 時有示範如何實作多面向。

如需多面向導覽設計原則的深入見解，推薦您下列連結：

* [設計模式：多面向導覽](https://alistapart.com/article/design-patterns-faceted-navigation)
* [實現分面搜索時的前端問題 – 第 1 部分](https://articles.uie.com/faceted_search2/)
