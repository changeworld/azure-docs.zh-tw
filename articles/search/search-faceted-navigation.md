...
根據預設，所有可能用於多面向導覽的欄位類型為 `Facetable` 。 這類欄位類型包括 `Edm.String`、`Edm.DateTimeOffset`和所有數值欄位類型（基本上，除了不能用於多面向導覽的 `Edm.GeographyPoint`以外，所有欄位類型都會 facetable。 
...
如您在範例架構中所見，針對不應當做 facet 使用的字串欄位（例如識別碼值），會關閉 `Facetable`。 關閉不需要的 facet 可協助您將索引的大小保持在最小，而且通常會改善效能。
