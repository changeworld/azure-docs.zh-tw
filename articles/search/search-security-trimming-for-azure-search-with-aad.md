...
本文示範如何使用 Azure Active Directory (AAD) 安全性身分識別以及 Azure 認知搜尋中的篩選準則，以根據使用者群組成員資格來修剪搜尋結果。
...
### <a name="step-1-create-aad-group"></a>步驟 1：建立 [AAD 群組](/graph/api/group-post-groups?view=graph-rest-1.0) 
```csharp
...
### <a name="step-2-create-aad-user"></a>步驟 2：建立 [AAD 使用者](/graph/api/user-post-users?view=graph-rest-1.0)
```csharp
User user = new User()
{
...
(選擇性) 若要降低網路延遲，您可以快取使用者群組關聯，以便在發出搜尋要求時從快取傳回群組，從而省下 AAD 的往返。 您可以使用 [AAD BATCH API](/graph/json-batching) 傳送具有多位使用者的單一 Http 要求，並建立快取。
...
如果使用者的群組尚快取或是快取已過期，請發出[群組](/graph/api/directoryobject-getmembergroups?view=graph-rest-1.0)要求
```csharp
