...
description: 在 Azure 認知搜索中的擴充管道中使用光學字元辨識 （OCR） 從影像檔中提取文本。
manager: nitinme
author: luiscabrer
ms.author: luisca
...
**光學字元辨識 （OCR）** 技能可識別影像檔中的列印和手寫文本。 這項技能會使用認知服務中[電腦視覺](https://docs.microsoft.com/azure/cognitive-services/computer-vision/home)所提供的機器學習模型。 **OCR** 技能對應至下列功能：

+ ["OCR"API](../cognitive-services/computer-vision/concept-recognizing-text.md#ocr-optical-character-recognition-api)用於英語以外的語言。 
+ 對於英語，使用新的["讀取"API。](../cognitive-services/computer-vision/concept-recognizing-text.md#read-api)
...
|行結束 | 每個檢測到的行之間要使用的值。 可能的值："空間"，"運輸返回"，"線饋送"。  預設值為"空格" |

以前，有一個參數稱為"文本提取演算法"，用於指定技能是否應提取"列印"或"手寫"文本。  此參數被棄用，不再需要，因為最新的讀取 API 演算法能夠同時提取這兩種類型的文本。  如果技能定義已包含此參數，則無需將其刪除，但將不再使用它，並且無論設置為什麼，都將提取這兩種類型的文本。
