...
我們的第一個範例會使用`knowledgeStore`此技能集，並以其 null 做為基礎`knowledgeStore` ，填滿物件，並以建立可在其他案例中使用之表格式資料結構的投影來設定。 
...
假設我們想要建立一個儀表板，讓我們可以將從檔中解壓縮的關鍵字組以單字雲端呈現。 若要建立正確的資料結構，我們可以在技能集中加入塑造器技能，以建立具有檔特定詳細資料和關鍵字組的自訂圖形。 在`document`根節點上會呼叫`pbiShape`自訂圖形。
...
在此範例中，「自訂」圖形會結合中繼資料和已識別的實體和主要片語。 物件被呼叫`pbiShape` ，而且在下`/document`是父代。 

> [!IMPORTANT] 
> 成形的一個目的是要確保所有擴充節點都以格式正確的 JSON 表示，這是投射到知識存放區所需的。 當擴充樹狀結構包含格式不正確的 JSON 時（例如，當擴充的父系為類似字串的基本類型）時，更是如此。
>
> 請注意最後兩個節點`KeyPhrases` ： `Entities`和。 這些會使用包裝成有效的 JSON 物件`sourceContext`。 這是必要的`keyphrases` ， `entities`而且會在基本專案上擴充，而且必須先轉換成有效的 JSON，才可以進行預測。
>
...
現在，我們已擁有將專案投影到資料表所需的所有資料，請使用資料表定義來更新 knowledgeStore 物件。 在此範例中，我們有三個數據表，並藉`tableName`由`source`設定`generatedKeyName` 、和屬性來定義。
...
切割會```generatedKeyName```在父資料表與子資料工作表之間隱含產生關聯性，並在子資料工作表中使用來建立具有相同名稱的資料行。 

### <a name="naming-relationships"></a>命名關聯性

和```generatedKeyName``` ```referenceKeyName```屬性是用來關聯資料表之間或甚至是跨投射類型的資料。 子資料工作表/投射中的每個資料列都有指向父系的屬性。 子系中的資料行或屬性名稱是來自父系```referenceKeyName```的。 ```referenceKeyName```當未提供時，服務會預設為父代的```generatedKeyName``` 。 

Power BI 依賴這些產生的索引鍵來探索資料表中的關聯性。 如果您需要以不同方式命名的子資料工作表中的資料行```referenceKeyName``` ，請在父資料表上設定屬性。 其中一個範例是將 pbiDocument 資料表```generatedKeyName```上的 as ID 設定為，並```referenceKeyName```將設為 DocumentID。 這會導致 pbiEntities 和 pbiKeyPhrases 資料表中的資料行包含名為 DocumentID 的檔識別碼。

## <a name="projecting-to-objects"></a>投射至物件

物件投射的限制與資料表投影不相同，而且較適合用來投射大型檔。 在此範例中，我們會將整份檔投影至物件投射。 物件投射僅限於容器中的單一投影，無法進行切割。

為了定義物件投射，我們將在投影中```objects```使用陣列。 您可以使用造型技能或使用物件投射的內嵌成形，產生新的圖形。 雖然資料表範例示範了建立形狀和配量的方法，但此範例示範內嵌成形的用法。 

內嵌成形是在投影的輸入定義中建立新圖形的能力。 內嵌成形所建立的匿名物件，與塑造者的技能會產生的一樣（在我們的`pbiShape`案例中為）。 如果您要定義不打算重複使用的圖形，內嵌成形就很有用。
...
為了產生檔案投射，我們使用投射物件`files`中的陣列。 這個範例會將所有從檔解壓縮的影像投射到名`samplefile`為的容器。
...
若要取得這些預測所需的圖形，請從新增可建立名`crossProjection`為之形狀物件的造型技能開始。 
...
這個範例也會反白顯示投影的另一個功能。 藉由在相同的投射物件內定義多個投射類型，在不同類型（資料表、物件、檔案）內和之間有一個關聯性，可讓您從檔的資料表資料列開始，並在物件投射中尋找該檔內影像的所有 OCR 文字。 
...
+ 不會將字串擴充成形成有效的 JSON。 擴充字串（例如`merged_content` ，使用主要片語擴充）時，擴充的屬性會表示為擴充樹狀結構`merged_content`內的子系。 預設標記法的 JSON 格式不正確。 因此在投射時間，請務必將擴充轉換成具有名稱和值的有效 JSON 物件。

+ ```/*```省略來源路徑結尾的。 如果投射的來源是`/document/pbiShape/keyPhrases`，則主要片語陣列會投射為單一物件/資料列。 相反地，請將來源路徑`/document/pbiShape/keyPhrases/*`設定為，以針對每個主要片語產生單一資料列或物件。
