...
ms.locfileid: "85130801"
---
# <a name="query-types-and-composition-in-azure-cognitive-search"></a>Azure 認知搜尋中的查詢類型和組合

在 Azure 認知搜尋中，查詢是來回作業的完整規格。 在要求上，有一些參數可提供引擎的執行指示，以及可塑造回應的參數。 未指定（ `search=*` ），沒有符合條件並使用 null 或 default 參數，查詢會針對所有可搜尋的欄位執行，以做為全文檢索搜尋作業，以任意順序傳回計分的結果集。

下列範例是[REST API](https://docs.microsoft.com/rest/api/searchservice/search-documents)中所建立的代表性查詢。 此範例以[飯店示範索引](search-get-started-portal.md)為目標，並包含一般參數，讓您可以瞭解查詢的外觀。
...
+ **`search`** 提供比對準則，通常是整個詞彙或片語，但通常會伴隨布林運算子。 單一的獨立字詞是「字詞」** 查詢。 以引號括住的多部分查詢是*片語*查詢。 搜尋可以是未定義的，如同中的 **`search=*`** ，但沒有符合的準則，結果集是由任意選取的檔所組成。

+ **`searchFields`** 將查詢執行限制為特定欄位。 在索引*架構中屬性*化為可搜尋的任何欄位，都是此參數的候選項。

回應也會依照您在查詢中包含的參數來塑造：

+ **`select`** 指定要在回應中傳回的欄位。 只有在*索引中標示為可*抓取的欄位，才可以在 select 語句中使用。

+ **`top`** 傳回指定數目的最相符檔。 在此範例中，只會傳回10個命中。 您可以使用 [靠上] 和 [略過] （未顯示）來分頁結果。

+ **`count`** 告訴您整個索引中的多少檔整體相符，這可能會超過傳回的數目。 

+ **`orderby`** 如果您想要依值排序結果（例如評等或位置），則會使用。 否則，預設值是使用相關性分數來排序結果。

在 Azure 認知搜尋中，查詢執行一律會針對一個索引，並使用要求中提供的 api 金鑰進行驗證。 在 REST 中，則會於要求標頭中同時提供這兩者。

### <a name="how-to-run-this-query"></a>如何執行這項查詢

在撰寫任何程式碼之前，您可以使用查詢工具來學習語法，並使用不同的參數進行實驗。 最快的方法是內建的入口網站工具，也就是[搜尋瀏覽器](search-explorer.md)。

如果您遵循本[快速入門來建立飯店示範索引](search-get-started-portal.md)，您可以將此查詢字串貼入 explorer 的搜尋列中，以執行您的第一個查詢：`search=+"New York" +restaurant&searchFields=Description, Address/City, Tags&$select=HotelId, HotelName, Description, Rating, Address/City, Tags&$top=10&$orderby=Rating desc&$count=true`
...
下列範例可說明這點：相同的查詢，但有不同的 queryType 設定，會產生不同的結果。 在第一個查詢中， `^3` 會將 after `historic` 視為搜尋詞彙的一部分。 此查詢的排名最高的結果是 "Marquis Plaza & suite"，其描述中有*海洋*

```
queryType=simple&search=ocean historic^3&searchFields=Description, Tags&$select=HotelId, HotelName, Tags, Description&$count=true
```

使用完整 Lucene 剖析器的相同查詢會解讀 `^3` 為現場詞彙增強功能。 切換剖析器會變更排名，其中包含的結果*會移至*最上層。
...
| 查詢類型 | 使用方式 | 範例和詳細資訊 |
|------------|--------|-------------------------------|
| 自由格式文字檢索 | 搜尋參數和任一剖析器| 全文檢索搜尋會掃描索引中所有「可搜尋」** 欄位內的一或多個字詞，而且運作方式就如同您對 Google 或 Bing 等搜尋引擎的期待。 簡介中的範例便是全文檢索搜尋。<br/><br/>全文檢索搜尋會使用標準 Lucene 分析器（根據預設）對所有詞彙進行詞法分析，移除 "the" 這類的停用字詞。 您可以使用[非英文分析器](index-add-language-analyzers.md#language-analyzer-list)或用於修改詞法分析的[特定語言無關分析器](index-add-custom-analyzers.md#AnalyzerTable)，來覆寫預設值。 其中一個範例是[關鍵字](https://lucene.apache.org/core/6_6_1/analyzers-common/org/apache/lucene/analysis/core/KeywordAnalyzer.html)，其會將欄位的整個內容視為單一語彙基元。 這適合用於郵遞區號、識別碼和產品名稱等資料。 | 
...
+ 將 **`searchMode=any`** （預設值）變更為 **`searchMode=all`** ，以要求符合所有準則，而非任何準則。 此做法在查詢中包含布林運算子時特別有用。

+ 如果文字或語彙分析是必要的，但查詢類型無法進行語言處理，請變更查詢技術。 在全文檢索搜尋中，文字或詞彙分析會更正拼寫錯誤、單複數單字形式，甚至是不規則的動詞或名詞。 對於某些查詢（例如模糊或萬用字元搜尋），詞法分析不是查詢剖析管線的一部分。 在某些情況下，規則運算式是既有的因應措施。 
...
如果您想要讓 Azure 認知搜尋傳回以搜尋分數以外的值排序的結果，您可以使用 **`orderby`** 搜尋參數。 您可以指定參數的值， **`orderby`** 以包含功能變數名稱和函式的呼叫， [** `geo.distance()` 以**](query-odata-filter-orderby-syntax.md)取得地理空間值。 每個運算式後面都可以接著， `asc` 以指示以遞增順序要求結果，並 **`desc`** 指出以遞減順序要求結果。 預設排名為遞增順序。


### <a name="hit-highlighting"></a>搜尋結果醒目提示
在 Azure 認知搜尋中，您可以使用 **`highlight`** 、和參數，強調符合搜尋查詢之搜尋結果的確切部分 **`highlightPreTag`** **`highlightPostTag`** 。 您可以指定哪些可搜尋欄位應該強調其*相符的文字*，以及指定要附加到 Azure 認知搜尋傳回的相符文字開頭和結尾的確切字串標記。
