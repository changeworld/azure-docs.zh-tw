...
* 在您搜尋服務上託管的 [Hotels-sample 索引](search-get-started-portal.md)
...
1. 向下捲動。 在 [要求] 中選取 [+ 從範例匯入] 按鈕，以設定搜尋服務的查詢要求：
   ...
   * 在 [標頭] 中輸入 `Content-Type`。 您會在稍後的步驟中，將值設定為 `application/json`。
...
1. 切換回精靈並返回 [3.要求] 步驟。 向下捲動到 [回應] 區段。 按一下 [新增預設回應]。 這很重要，因為這可協助 Power Apps 了解回應的結構描述。 
...
1. 在清單中尋找您的連接器 (在本教學課程中為 "AzureSearchQuery")。
