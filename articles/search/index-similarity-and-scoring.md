...
搜尋分數會根據資料和查詢的統計屬性來計算。 Azure 認知搜尋會尋找符合搜尋字詞的文件 (部分或全部，視 [searchMode](/rest/api/searchservice/search-documents#searchmodeany--all-optional) 而定)，優先列出包含多個搜尋字詞例項的文件。 如果字詞在資料索引間很少見，但在文件內很常見，搜尋分數會更高。 這種計算相關性的方法基礎稱為 *TF-IDF* (或 term frequency-inverse document frequency)。
...
如果您想要打破重複分數之間的僵局，您可以新增 **$orderby** 子句，首先依分數排序，然後再依另一個可排序欄位排序 (例如 `$orderby=search.score() desc,Rating desc`)。 如需詳細資訊，請參閱 [$orderby](./search-query-odata-orderby.md) \(部分機器翻譯\)。
...
如果您偏好根據所有分區的統計屬性來計算分數，您可以將 *scoringStatistics=global* 加入為[查詢參數](/rest/api/searchservice/search-documents) (或將 *"scoringStatistics": "global"* 加入為[查詢要求](/rest/api/searchservice/search-documents)的主體參數)。
...
[搜尋檔](/rest/api/searchservice/preview-api/search-documents) 要求具有新的 [featuresMode](/rest/api/searchservice/preview-api/search-documents#featuresmode) 參數，可在欄位層級提供相關性的其他詳細資料。 `@searchScore`針對檔的計算方式，是在此) 查詢的內容中 (相關的檔，而您可以透過 featuresMode 取得個別欄位的相關資訊（如結構中所表示） `@search.features` 。 此結構包含查詢中所使用的所有欄位 (在查詢中透過 **searchFields** 的特定欄位，或在索引) 中 **屬性為可** 搜尋的所有欄位。 針對每個欄位，您會取得下列值：

+ 在欄位中找到的唯一標記數目
+ 相似性分數，或相對於查詢詞彙的欄位內容相似之處的量值
+ 字詞頻率，或在欄位中找到查詢詞彙的次數

針對以「描述」和「標題」欄位為目標的查詢，包含的回應 `@search.features` 可能如下所示：
...
您可以在 [自訂評分解決方案](https://github.com/Azure-Samples/search-ranking-tutorial) 中使用這些資料點，或使用此資訊來偵測搜尋相關性問題。


## <a name="see-also"></a>另請參閱

 [評分設定檔](index-add-scoring-profiles.md) [REST API 參考](/rest/api/searchservice/)[搜尋檔 API](/rest/api/searchservice/search-documents) [Azure 認知搜尋 .net SDK](/dotnet/api/overview/azure/search)