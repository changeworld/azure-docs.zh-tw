...
description: '瞭解如何使用 c # 和 Microsoft，在 .NET 應用程式中建立和管理搜尋物件。搜尋 (版本 10) 的 .NET SDK。 程式碼片段會示範如何連接到服務、建立索引和查詢。'
manager: nitinme
author: brjohnstmsft
ms.author: brjohnst
ms.devlang: dotnet
...
ms.locfileid: "92681488"
---
# <a name="how-to-use-microsoftazuresearch-in-a-c-net-application"></a>如何使用 Microsoft Azure。在 c # .NET 應用程式中搜尋

本文說明如何使用 c # 和舊版用戶端程式庫（在 Azure SDK for .NET 中 [**搜尋**](/dotnet/api/overview/azure/search/client10) (版本 10) ）來建立和管理搜尋物件。 

第10版是最新版的 Microsoft Azure. Search 套件。 從 Azure SDK 團隊 [**Azure.Search.Documents**](/dotnet/api/overview/azure/search.documents-readme) 中，新功能將在未來推出。

> [!NOTE]
> 如果您有現有或傳遞的開發專案，您可以繼續使用第10版。 針對新的專案，或使用新功能，您應該轉換至新的連結 [庫](/dotnet/api/overview/azure/search.documents-readme)。

## <a name="about-version-10"></a>關於第10版
...
.NET SDK 的目標版本是[ `2019-05-06` Azure 認知搜尋 REST API](https://github.com/Azure/azure-rest-api-specs/tree/master/specification/search/data-plane/Microsoft.Azure.Search.Data/stable/2019-05-06)。 此版本包含對 Azure Blob 編制索引時的 [複雜類型](search-howto-complex-data-types.md)、 [AI 擴充](cognitive-search-concept-intro.md)、 [自動完成](/rest/api/searchservice/autocomplete)和 [JsonLines 剖析模式](search-howto-index-json-blobs.md) 的支援。 

此 SDK 不支援[管理作業](/rest/api/searchmanagement/)，例如建立及調整搜尋服務和管理 API 金鑰。 如果您需要從 .NET 應用程式管理搜尋資源，您可以使用 [Azure 認知搜尋 .Net 管理 SDK](/dotnet/api/overview/azure/search/management)。

## <a name="upgrade-to-v10"></a>升級至 v10
