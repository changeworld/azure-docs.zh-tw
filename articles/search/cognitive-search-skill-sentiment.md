...
description: 在 Azure 認知搜尋的 AI 擴充管線中，從文字中解壓縮正面負的情感分數。
manager: nitinme
author: luiscabrer
ms.author: luisca
...
**情感**技能會依循正負面連續性評估非結構化文字，並且針對每個記錄傳回介於 0 與 1 之間的數值分數。 接近 1 的分數表示正面情感；接近 0 的分數表示負面情感。 這項技能會使用認知服務中[文字分析](../cognitive-services/text-analytics/overview.md)所提供的機器學習模型。
...
記錄的大小上限應為5000個字元（以測量） [`String.Length`](/dotnet/api/system.string.length) 。 如果您需要先分割資料，再將該資料傳送至情感分析器，請使用[文字分割技能](cognitive-search-skill-textsplit.md)。
...
| 參數名稱 | 描述 |
|----------------|----------------------|
| `defaultLanguageCode` | (選用) 要套用至未明確指定語言之文件的語言代碼。 <br/> 請參閱[支援語言的完整清單](../cognitive-services/text-analytics/language-support.md) |
