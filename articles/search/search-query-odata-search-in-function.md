...
---
# <a name="odata-searchin-function-in-azure-cognitive-search"></a>Azure `search.in`認知搜尋中的 OData 函式
...
> [!NOTE]
> 4.01 版的 OData 標準最近引進了[ `in`運算子](https://docs.oasis-open.org/odata/odata/v4.01/cs01/part2-url-conventions/odata-v4.01-cs01-part2-url-conventions.html#_Toc505773230)，其行為與 Azure 認知搜尋中的`search.in`函數類似。 不過，Azure 認知搜尋不支援此運算子，因此您必須改為`search.in`使用函數。
...
`search.in`函式會測試指定的字串欄位或範圍變數是否等於指定的值清單中的其中一個。 變數與清單中每個值之間的相等，是以區分大小寫的方式來決定，與`eq`運算子的方式相同。 因此，像 `search.in(myfield, 'a, b, c')` 這樣的運算式會等同於 `myfield eq 'a' or myfield eq 'b' or myfield eq 'c'`，差別在於 `search.in` 所產生的效果會好得多。
...
| `valueList` | `Edm.String` | 字串，包含要比對`variable`參數之值的分隔清單。 如果未`delimiters`指定參數，預設分隔符號會是空格和逗號。 |
