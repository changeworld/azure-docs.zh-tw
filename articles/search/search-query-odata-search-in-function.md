...
函式 `search.in` 會測試指定的字串欄位或範圍變數是否等於指定的值清單中的其中一個。 變數與清單中每個值之間的相等，是以區分大小寫的方式來決定，與運算子的方式相同 `eq` 。 因此，像 `search.in(myfield, 'a, b, c')` 這樣的運算式會等同於 `myfield eq 'a' or myfield eq 'b' or myfield eq 'c'`，差別在於 `search.in` 所產生的效果會好得多。
...
如果您使用 `search.in`，當第二個參數所含的清單有數百或數千個值時，預期的回應時間應該不到一秒。 `search.in`雖然您仍然受到最大要求大小的限制，但您可以傳遞至的專案數沒有明確限制。 不過，當值的數目增加時，延遲也會隨之增長。
