...
description: 完整的 Lucene 查詢語法的引用，如 Azure 認知搜索中用於萬用字元、模糊搜索、RegEx 和其他高級查詢構造。
manager: nitinme
author: brjohnstmsft
ms.author: brjohnst
...
ms.locfileid: "79283130"
---
# <a name="lucene-query-syntax-in-azure-cognitive-search"></a>Azure 認知搜索中的 Lucene 查詢語法

您可以基於豐富的[Lucene 查詢解析器](https://lucene.apache.org/core/6_6_1/queryparser/org/apache/lucene/queryparser/classic/package-summary.html)語法編寫針對 Azure 認知搜索的查詢，用於專用查詢表單：萬用字元、模糊搜索、鄰近搜尋、正則運算式就是一些示例。 除了通過`$filter`運算式在 Azure 認知搜索中構造*的範圍搜索*外，許多 Lucene 查詢解析器語法[在 Azure 認知搜索中完整地實現](search-lucene-query-architecture.md)。 

> [!NOTE]
> 完整的 Lucene 語法用於在[搜索文檔](https://docs.microsoft.com/rest/api/searchservice/search-documents)API**的搜索**參數中傳遞的查詢運算式，不要與用於該 API [$filter](search-filters.md)參數的[OData 語法](query-odata-filter-orderby-syntax.md)混淆。 這些不同的語法具有用於構造查詢、逸出字元串等自己的規則。
...
有關其他示例，請參閱[Lucene 查詢語法示例，用於在 Azure 認知搜索 中生成查詢](search-query-lucene-examples.md)。 有關指定查詢參數的完整列隊的詳細資訊，請參閱[搜索文檔&#40;Azure 認知搜索 REST API&#41;](https://docs.microsoft.com/rest/api/searchservice/Search-Documents)。
...
### <a name="encoding-unsafe-and-reserved-characters-in-urls"></a>為 URL 中的 Unsafe 字元和保留字元編碼
 ...
 請確定所有 Unsafe 字元和保留字元在 URL 中都已編碼。 例如，'#' 是 Unsafe 字元，因為它是 URL 中的片段/錨點識別碼。 此字元在 URL 中使用時必須編碼為 `%23`。 "&"和"*"是保留字元的示例，因為它們在 Azure 認知搜索中分隔參數並指定值。 有關詳細資訊，請參閱[RFC1738：統一資源定位器 （URL）。](https://www.ietf.org/rfc/rfc1738.txt)
...
### <a name="searchmode-parameter-considerations"></a>SearchMode 參數考量  
 如 Azure `searchMode` [認知搜索中的簡單查詢語法](query-simple-syntax.md)中所述，對查詢的影響同樣適用于 Lucene 查詢語法。 也就是說，如果您不清楚您設定參數的方式會有何影響，`searchMode` 與 NOT 運算子搭配使用時可能會產生出乎您預期的查詢結果。 如果您保留預設值 `searchMode=any`，並使用 NOT 運算子，則會以 OR 動作計算作業，而使 "New York" NOT "Seattle" 傳回所有不是 Seattle 的城市。  

##  <a name="boolean-operators-and-or-not"></a><a name="bkmk_boolean"></a>布林運算子（AND、OR、不） 
...
##  <a name="query-size-limitations"></a><a name="bkmk_querysizelimits"></a>查詢大小限制  
 可以發送到 Azure 認知搜索的查詢大小有限制。 具體來說，您最多可以有 1024 個子句 (以 AND、OR 等運算子分隔的運算式)。 此外，查詢中任何個別字詞的大小也有約 32 KB 的限制。 如果您的應用程式以程式設計方式產生搜尋查詢，建議您依照此方式設計，以避免產生無大小限制的查詢。  

##  <a name="scoring-wildcard-and-regex-queries"></a><a name="bkmk_searchscoreforwildcardandregexqueries"></a>萬用字元和 regex 查詢的評分
 Azure 認知搜索使用基於頻率的評分 （[TF-IDF](https://en.wikipedia.org/wiki/Tf%E2%80%93idf)） 進行文字查詢。 不過，針對字詞範圍可能很廣泛的萬用字元和 regex 查詢，則會忽略頻率因素，以防止罕見字詞的相符項目誤獲較高的排名。 系統對於萬用字元和 regex 搜尋的所有相符項目，會同等視之。

##  <a name="fielded-search"></a><a name="bkmk_fields"></a>欄位搜索  
可以使用`fieldName:searchExpression`語法定義欄位搜索操作，其中搜索運算式可以是單個單詞或短語，也可以是括弧中的更複雜的運算式，可選使用布林運算子。 部分範例如下：  
...
> [!NOTE]
> 使用欄位搜索運算式時，不需要使用 參數，`searchFields`因為每個欄位搜索運算式都有顯式指定的欄位名稱。 但是，如果要執行查詢，`searchFields`其中某些部分限定在特定欄位，其餘部分可以應用於多個欄位，則仍可以使用 參數。 例如，查詢將僅`search=genre:jazz NOT history&searchFields=description`與`jazz``genre`欄位匹配，而該查詢將與`NOT history``description`該欄位匹配。 中`fieldName:searchExpression`提供的欄位名稱始終優先于`searchFields`參數，因此在此示例中，我們不需要在`genre``searchFields`參數中包括。
