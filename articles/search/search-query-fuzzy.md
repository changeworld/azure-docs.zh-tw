...
這是一個展開練習，會產生與具有類似組合的字詞相符的結果。 當指定模糊搜尋時，引擎會根據具[決定性的有限自動化理論](https://en.wikipedia.org/wiki/Deterministic_finite_automaton)) ，針對查詢中的所有完整詞彙，建立一個圖形 (。 例如，如果您的查詢包含三個詞彙「華盛頓大學」，則會為查詢中的每個詞彙建立圖表 `search=university~ of~ washington~` (不會在模糊搜尋中移除任何停用字，因此 "of" 會取得圖形) 。

此圖表包含每個詞彙最多50個擴充或相片順序，可在進程中同時捕捉正確和不正確的變數。 然後，引擎會傳迴響應中最重要的相關相符專案。 

對於「大學」之類的詞彙，圖形可能會有「unversty、universty、大學、universe、反向」。 結果中會包含任何符合圖形中的檔。 相較于分析文字來處理不同形式的相同單字 ( 「滑鼠」和「滑鼠」 ) 的其他查詢，模糊查詢中的比較會以臉部值來執行，而不需要任何語言分析文字。 「Universe」和「反向」（在語義上不同）會符合，因為語法差異很小。

如果不一致限制為兩個或更少的編輯，而其中的編輯是插入、刪除、取代或轉置的字元，則相符項就會成功。 指定差異的字串更正演算法是[damerau-levenshtein-Levenshtein 距離](https://en.wikipedia.org/wiki/Damerau%E2%80%93Levenshtein_distance)計量，描述為「最小數目的作業 (插入、刪除、替代或 transpositions 兩個連續的字元，) 需要將一個單字變更為另一個字組」。 

在 Azure 認知搜尋中：

+ 模糊查詢適用于整個詞彙，但您可以透過和構造支援片語。 例如，"Unviersty ~ of" Wshington ~ "會符合「華盛頓大學」的結果。

+ 編輯的預設距離為2。 的值 `~0` 表示沒有任何擴充 (只會將確切的詞彙視為符合) ，但您可以指定 `~1` 一個差異程度或一個編輯。 

+ 模糊查詢最多可以擴充到50個額外的排列。 這項限制是無法設定的，但您可以將編輯距離減少為1，藉此有效減少擴充的數目。

+ 回應是由包含最多 50) 之相關相符 (的檔所組成。

這些圖形會共同提交，以符合索引中的權杖的比對準則。 您可以想像一下，模糊搜尋原本就比其他查詢表單慢。 索引的大小和複雜度，可以判斷其效益是否足以彌補回應的延遲。

> [!NOTE]
> 因為模糊搜尋的速度很慢，所以您可以調查其他替代專案（例如 n 語法編制索引），並將其簡短字元序列的進度 (兩個字元序列和三個 bigram，並將) 。 根據您的語言和查詢介面，n 語法可能會提供更好的效能。 取捨的是，n 語法編制索引是非常密集的儲存體，而且會產生更大的索引。
>
...
一般來說，在每個欄位上指派分析器時，微調分析鏈的決策是根據主要使用案例 (篩選或全文檢索搜尋) ，而不是像模糊搜尋之類的特殊查詢表單。 基於這個理由，模糊搜尋並沒有特定的分析器建議。 
...
1. （選擇性）將要求的範圍限定于特定欄位，並使用此參數 (`searchFields=<field1,field2>`) 。 

1. `~`在整個詞彙的結尾附加波狀符號 () 運算子 (`search=<string>~`) 。

   如果您想要指定 () 的編輯距離，請包含選擇性參數，介於0和2之間的數位 (預設) `~1` 。 例如，"blue~" 或 "blue~1" 會傳回 "blue"、"blues" 和 "glue"。

在「Azure 認知搜尋」中，除了「期限」和「距離 (最大值 2) 」以外，不會在查詢上設定其他參數。
...
嘗試再試一次，從「特殊」到「比例」 ) 中取出一個最後一個字元，以進一步修改搜尋詞彙 (：
