...
> [!NOTE]  
>  在要求 URI 中傳遞敏感性資料 (例如 `api-key`) 被視為不佳的安全性做法。 基於這個理由，Azure 認知搜尋只接受查詢索引鍵作為 `api-key` 查詢字串中的，而且您應該避免這麼做，除非您的索引內容應該可公開使用。 一般規則是建議將 `api-key` 當作要求標頭來傳遞。  
...
> [!Note]
> 在[查詢 c # 中的 Azure 認知搜尋索引](search-query-dotnet.md)中可以找到顯示查詢金鑰使用方式的程式碼範例。
