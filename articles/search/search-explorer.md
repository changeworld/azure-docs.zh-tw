...
本快速入門會使用現有的索引來示範搜尋總管。 要求會使用[搜尋 REST API](/rest/api/searchservice/search-documents) 來編寫，並以詳細 JSON 文件的形式傳回回應。
...
+ 本快速入門會使用 *realestate-us-sample-index*。 使用[快速入門：建立索引](search-import-data-portal.md)來使用預設值建立索引。 Microsoft 所裝載的內建範例資料來源 (**realestate-us-sample**) 會提供資料。

## <a name="start-search-explorer"></a>開啟 [搜尋總管]

1. 在 [Azure 入口網站](https://portal.azure.com)中，從儀表板開啟搜尋概觀頁面，或是[尋找服務](https://ms.portal.azure.com/#blade/HubsExtension/BrowseResourceBlade/resourceType/Microsoft.Search%2FsearchServices)。

1. 從命令列開啟搜尋總管：

   :::image type="content" source="media/search-explorer/search-explorer-cmd2.png" alt-text="入口網站中的搜尋總管命令" border="true":::

    或者，在開啟的索引上使用內嵌的 [搜尋總管] 索引標籤：

   :::image type="content" source="media/search-explorer/search-explorer-tab.png" alt-text="搜尋總管索引標籤" border="true":::
...
空的搜尋的對等語法是 `*` 或 `search=*`。
   ...
   **結果**
   ...
   :::image type="content" source="media/search-explorer/search-explorer-example-empty.png" alt-text="不合格或空的查詢範例" border="true":::
...
請注意，當您提供搜尋準則 (例如查詢字詞或運算式) 時，搜尋排名可派上用場。 以下透過範例說明任意文字搜尋。
   ...
   **結果**
   ...
   :::image type="content" source="media/search-explorer/search-explorer-example-freetext.png" alt-text="任意文字查詢範例" border="true":::

## <a name="count-of-matching-documents"></a>相符文件的計數 

新增 **$count=true** 以取得索引中找到的相符項目數。 在空的搜尋中，計數會是索引中的文件總數。 在限定搜尋中，它是符合查詢輸入的文件數目。 先前提過，服務預設會傳回前 50 個相符項目，因此索引中的相符項目可能會比結果中包含的項目多。
   ...
   **結果**
   ...
   :::image type="content" source="media/search-explorer/search-explorer-example-count.png" alt-text="索引中相符文件的計數" border="true":::
...
新增 [ **$select**](search-query-odata-select.md) 以將結果限制為明確具名欄位，以便 **[搜尋總管]** 中的輸出更容易讀取。 若要保留搜尋字串和 **$count=true**，請在引數前加上 **&** 首碼。 
   ...
   **結果**
   ...
   :::image type="content" source="media/search-explorer/search-explorer-example-selectfield.png" alt-text="限制搜尋結果中的欄位" border="true":::

## <a name="return-next-batch-of-results"></a>傳回下一個批次的結果

Azure 認知搜尋服務會根據搜尋排名傳回前 50 個相符項目。 若要取得下一組相符文件，請附加 **$top=100,&$skip=50** 以將結果集增加到 100 份文件 (預設值為 50，最大值為 1000)，略過前 50 個文件。 您可以檢查文件索引鍵 (listingID) 來識別文件。 
...
回想一下，您必須提供搜尋準則，例如查詢字詞或運算式，以取得高排序結果。 請注意，搜尋分數會降低您在搜尋結果中的深度。
   ...
   **結果**
   ...
   :::image type="content" source="media/search-explorer/search-explorer-example-topskip.png" alt-text="傳回搜尋結果的下一個批次" border="true":::
...
如果要指定精確的準則而不是任意文字搜尋，請使用 [ **$filter**](search-query-odata-filter.md) 參數。 欄位在索引中的屬性必須為 **可篩選**。 此範例會搜尋大於 3 的臥室數目：
   ...
   **結果**
   ...
   :::image type="content" source="media/search-explorer/search-explorer-example-filter.png" alt-text="依準則篩選" border="true":::
...
新增 [ **$orderby**](search-query-odata-orderby.md) 以依照搜尋分數以外的其他欄位對結果進行排序。 欄位在索引中的屬性必須為 **可排序**。 可用來測試此輸出的範例運算式是：
   ...
   **結果**
   ...
   :::image type="content" source="media/search-explorer/search-explorer-example-ordery.png" alt-text="變更排序次序" border="true":::
...
+ 查詢和篩選條件運算式會以 Azure 認知搜尋所實作的語法表達。 預設值是[簡單語法](/rest/api/searchservice/simple-query-syntax-in-azure-search)，但您可以選擇使用[完整 Lucene 功能](/rest/api/searchservice/lucene-query-syntax-in-azure-search)進行更強大的查詢。 [篩選條件運算式](/rest/api/searchservice/odata-expression-syntax-for-azure-search)是 OData 語法。
...
若要深入了解查詢結構和語法，請使用 Postman 或對等工具來建立利用 API 更多部分的查詢運算式。 [搜尋 REST API](/rest/api/searchservice/search-documents) 特別有助於學習和探索。

> [!div class="nextstepaction"]
> [在 Postman 中建立基本查詢](search-get-started-rest.md)