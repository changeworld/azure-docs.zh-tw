...
若為不允許的比較（例如，比較類型`Edm.Int64`的欄位與`NaN`），Azure 認知搜尋 REST API 會傳回「HTTP 400：不正確的要求」錯誤。
...
如果您的索引具有類型`Edm.Double`為的欄位， `NaN`而且您將值上傳至這些欄位，則在撰寫篩選器時，您必須考慮此情況。 Azure 認知搜尋會實作為處理`NaN`值的 IEEE 754 標準，而這類值的比較會產生不明顯的結果，如下表所示。
...
您無法直接比較類型`Edm.GeographyPoint`的欄位與常數值，但您可以使用`geo.distance`函數。 此函式會傳回類型`Edm.Double`的值，因此您可以將它與數值常數做比較，以根據來自常數地理空間座標的距離進行篩選。 請參閱下列[範例](#examples)。
