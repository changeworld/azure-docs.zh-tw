...
3. 針對 [名稱]****，輸入新 CDN 端點的唯一名稱。 這個名稱會用來存取位於網域_ \<端點>_. azureedge.net 的快取資源。
...
8. 針對 [通訊協定]**** 與 [來源連接埠]****，指定通訊協定與連接埠以用來存取原始伺服器上的資源。 至少必須選取一個通訊協定 (HTTP 或 HTTPS)。 使用 CDN 提供的網域（_\<端點>_. azureedge.net）來存取 HTTPS 內容。 
