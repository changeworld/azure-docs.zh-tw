...
設置這些規則後，_&lt;終結點主機名稱&gt;_.azureedge.net/home/index.html 的請求會觸發自訂緩存規則#2，該規則設置為：**如果缺少，則設置為：如果缺少，則設置為**3 天。 因此，如果 index.html** 檔案具有 `Cache-Control` 或 `Expires` HTTP 標頭，則會優先採用它們，否則，如果未設定這些標頭，就會將檔案快取 3 天。
