...
每個以地圖為基礎的報告會將地理實體 (也就是國家/地區，並提供地圖以協助您將內容的要求位置視覺化。 作法是根據每個區域中發生的需求量，以色彩標示該區域。 淺色陰影區域表示對您的內容有較低的需求，而較暗區域則表示對您的內容有較高的需求。

地圖正下方提供每個區域的詳細流量和頻寬資訊。 這可讓您檢視每個區域的點擊總數、點擊百分比、傳輸的資料總量 (GB)，以及傳輸的資料百分比。 檢視每個這些度量的描述。 最後，當您將滑鼠停留在區域上時 (也就是 [國家/地區]、[州] 或 [省份]) ，區域中發生的名稱和點擊百分比將會顯示為工具提示。
...
| 歐洲地圖 |此報告可讓您檢視歐洲對您的 CDN 內容的需求。 每個國家/地區在此地圖上以色彩標示，以表示來自該區域的點擊百分比。 |
| 亞太地區地圖 |此報告可讓您檢視亞洲對您的 CDN 內容的需求。 每個國家/地區在此地圖上以色彩標示，以表示來自該區域的點擊百分比。 |

## <a name="geography-reports-bar-charts"></a>地理位置報告 (橫條圖)
另外有兩個報告根據地理位置提供統計資訊：「前幾名城市」和「前幾名國家/地區」。 這些報告會根據源自于這些國家/地區的點擊次數，分別為城市和國家/地區進行排名。 產生這類報告時，橫條圖會指出在特定平臺上要求內容的前10個城市或國家/地區。 這個橫條圖可讓您迅速評估對您的內容產生最多要求的區域。
...
* 「前幾名城市」報告的工具提示會依姓名、州/省和國家/地區縮寫來識別城市。
* 如果無法判斷發出要求的城市或區域 (亦即，州/省)，則會指出不明。 如果國家/地區不明，則會有兩個問號 (亦即？？將會顯示 ) 。
* 報告可能包含「歐洲」或「亞太地區」的度量。 這些項目並不是提供這些區域中所有 IP 位址的統計資訊。 相反地，它們只適用于源自歐洲或亞太地區（而不是特定城市或國家/地區）的 IP 位址所發出的要求。
