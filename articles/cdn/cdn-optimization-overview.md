...
ms.locfileid: "79252112"
---
# <a name="optimize-azure-cdn-for-the-type-of-content-delivery"></a>針對內容傳遞類型將 Azure CDN 最佳化

當您將內容傳遞給所有對象時，請務必確保最佳的傳遞內容。 [Azure 內容交付網路 （CDN）](cdn-overview.md)可以根據您擁有的內容類型優化交付體驗。 內容可以是網站、即時串流、影片或供下載的大型檔案。 當您建立 CDN 端點時，您會在 [最佳化對象]**** 選項中指定案例。 您的選擇會決定從 CDN 端點傳送的內容套用哪一個最佳化。
...
* [一般網路交付](#general-web-delivery)。 此最佳化也可用於媒體串流處理和大型檔案下載。

> [!NOTE]
> 微軟的動態網站加速是通過 Azure[前門服務](https://docs.microsoft.com/azure/frontdoor/front-door-overview)提供的。
...
2. 在"設置"下，選擇 **"優化**"。 然後，從 **"優化"下**拉清單中選擇一個類型。
...
> [!NOTE]
> 如果 CDN 端點主要是提供點播視訊內容，請使用此最佳化類型。 此優化類型與一般媒體流優化類型之間的主要區別是連接重試超時。使用即時流式處理方案的超時時間要短得多。
>
...
> [!NOTE]
> 來自 Microsoft 的動態網站加速通過[Azure 前門服務](https://docs.microsoft.com/azure/frontdoor/front-door-overview)提供，該服務是利用 Microsoft 專用全球網路提供應用工作負載的全球[任意廣播](https://en.wikipedia.org/wiki/Anycast)服務。
