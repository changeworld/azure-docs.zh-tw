...
當您將內容傳遞給所有對象時，請務必確保最佳的傳遞內容。 [Azure 內容傳遞網路 (CDN)](cdn-overview.md)可以根據您擁有的內容類型優化交付體驗。 內容可以是網站、即時串流、影片或供下載的大型檔案。 當您建立 CDN 端點時，您會在 [最佳化對象]**** 選項中指定案例。 您的選擇會決定從 CDN 端點傳送的內容套用哪一個最佳化。
...
> [!NOTE]
> 來自 Microsoft 的動態網站加速透過[Azure 前門服務](https://docs.microsoft.com/azure/frontdoor/front-door-overview)提供,該服務是利用 Microsoft 專用全球網路提供應用工作負載的全球[任意廣播](https://en.wikipedia.org/wiki/Anycast)服務。
