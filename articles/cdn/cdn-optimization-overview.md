...
description: 瞭解 Azure 內容傳遞網路如何根據內容類型來優化傳遞。 優化的最佳作法會改善效能和來源卸載。
services: cdn
documentationcenter: ''
author: asudbring
manager: danielgi
...
> [!NOTE]
> Microsoft 提供的動態網站加速是透過 [Azure Front Door 服務](https://docs.microsoft.com/azure/frontdoor/front-door-overview)提供。
...
> [!NOTE]
> 如果 CDN 端點主要是提供點播視訊內容，請使用此最佳化類型。 此優化類型和一般媒體串流優化類型之間的主要差異在於連線重試超時時間。使用即時串流案例的時間較短。
>
...
> [!NOTE]
> Microsoft 提供的動態網站加速是透過 [Azure Front Door 服務](https://docs.microsoft.com/azure/frontdoor/front-door-overview) 提供，這是利用 Microsoft 私用全球網路來提供應用程式工作負載的全球 [任意](https://en.wikipedia.org/wiki/Anycast) 傳播服務。
