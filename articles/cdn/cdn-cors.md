...
使用 Verizon Premium 規則引擎時，您必須 [建立規則](cdn-rules-engine.md) 來檢查要求的 **原始** 標頭。  如果是有效的來源，您的規則將使用要求中提供的來源設定 **Access-Control-Allow-Origin** 標頭。  如果不允許 **原始** 標頭中指定的來源，則您的規則應該省略 **存取控制-允許原始** 的標頭，這將導致瀏覽器拒絕要求。 
