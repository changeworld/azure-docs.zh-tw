...
description: 在此學習路徑中，開始瞭解反向 DNS 的運作方式，以及如何在 Azure 中使用它
services: dns
...
例如，DNS 記錄 'www.contoso.com' 是使用 DNS 'A' 記錄名稱 'www' 在區域 'contoso.com' 中實作的。  此 A 記錄會指向對應的 IP 位址，在此情況下為 64.4.6.100。  反向對應是在區域 ' 6.4.64.in-addr. arpa ' 中使用名為 ' 100 ' 的 ' PTR ' 記錄來分開實行 (請注意，在 ARPA 區域中，IP 位址會反轉 ) 。如果已正確設定，則會指向名稱 ' www.contoso.com '。
...
例如： 在建立反向區域託管 IP 前置詞為 2001:db8:1000:abdc::/64 的主機記錄時，會隔離位址的網路首碼 (2001:db8:abdc::) 來建立區域名稱。 接著展開 IPv6 網路首碼以移除[零壓縮](/previous-versions/windows/it-pro/windows-server-2003/cc781672(v=ws.10))，如果它用來縮短 IPv6 位址首碼 (2001:0db8:abdc:0000::)。 反轉順序，前置詞中每個十六進位數字之間使用句點為分隔符號，建置反轉的網路前置詞 (`0.0.0.0.c.d.b.a.8.b.d.0.1.0.0.2`)，再加上尾碼 `.ip6.arpa`。
