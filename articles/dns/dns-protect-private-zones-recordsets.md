...
* `Microsoft.Network/privateDNSZones/read` 授與讀取 DNS 私人區域的許可權，但無法修改它們，讓您查看建立 CNAME 的區域。
...
Azure Resource Manager 支援另一種類型的安全性控制，也就是鎖定資源的能力。 資源鎖定會套用至資源，並且在所有使用者和角色中都有效。 如需詳細資訊，請參閱[使用 Azure Resource Manager 來鎖定資源](../azure-resource-manager/management/lock-resources.md)。
