...
DNS 通訊協定可防止在區域頂點指派 A 或 AAAA 記錄以外的任何項目。 範例區域頂點為 contoso.com。 此限制會為在流量管理員後面有負載平衡應用程式的應用程式擁有者帶來問題。 您無法從區域頂點記錄指向流量管理員設定檔。 因此，應用程式擁有者必須使用因應措施。 位於應用程式層的重新導向，必須從區域頂點重新導向至另一個網域。 例如，從contoso.com重定向到 www\.contoso.com。 此種安排會為重新導向功能帶來單一失敗點。

透過別名記錄，此問題已不存在。 應用程式擁有者現在可以將其區域頂點記錄指向具有外部端點的流量管理員設定檔。 應用程式擁有者可以指向相同的流量管理員設定檔，以用於其 DNS 區域內的任何其他網域。

例如，contoso.com和 www\.contoso.com 可以指向相同的流量管理器設定檔。 只要流量管理員設定檔僅設定外部端點，就是這種情況。
...
現在，您需要記下 Web 應用的 IP 位址和主機名稱。

1. 打開資源組並選擇第一個 Web 應用（本示例中為**App-01）。**
2. 在左列中，選擇 **"屬性**"。
3. 記下 [URL]**** 下方的位址，並在 [輸出 IP 位址]**** 下方，記下清單中的第一個 IP 位址。 稍後在配置流量管理器終結點時，您將使用此資訊。
4. 針對 **App-02** 重複執行。
...
## <a name="add-a-txt-record-for-custom-domain-validation"></a>添加用於自訂域驗證的 TXT 記錄

當您向 Web 應用添加自訂主機名稱時，它將查找特定的 TXT 記錄來驗證您的域。

1. 打開資源組並選擇 DNS 區域。
2. 選取 [記錄集]****。
3. 使用下表添加記錄集。 對於該值，請使用以前記錄的實際 Web 應用 URL：
...
1. 打開資源組並選擇第一個 Web 應用。
2. 在左列中，選擇 **"自訂域**"。
3. 在 **"自訂域**"下，選擇 **"添加自訂域**"。
4. 在 **"自訂域"** 下，鍵入自訂功能變數名稱。 例如 contoso.com。
5. 選取 [驗證]****。

   您的域應通過驗證，並在**主機名稱可用性**和**域擁有權**旁邊顯示綠色核取記號。
5. 選取 [新增自訂網域]****。
6. 若要在 [指派至網站的主機名稱]**** 下方查看新的主機名稱，請重新整理瀏覽器。 頁面上的刷新並不總是會馬上顯示更改。
...
現在，您可以進行測試，以確保您可以訪問 Web 應用，並且它負載平衡。
