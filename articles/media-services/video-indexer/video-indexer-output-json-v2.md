---
title: 檢查 v2 API 生成的視頻索引子輸出 - Azure
titleSuffix: Azure Media Services
description: 本主題將檢查 v2 API 生成的 Azure 媒體服務視頻索引子輸出。
...
---
# <a name="examine-the-video-indexer-output-produced-by-api"></a>檢查 API 生成的視頻索引子輸出

當您呼叫**取得影片索引** API 時，若回應狀態為「確定」，您取得的回應內容會是詳細的 JSON 輸出。 JSON 內容包含指定影片深入解析的詳細資料。 見解包括：成績單、OCR、面孔、主題、塊等。每個洞察類型都包括時間範圍實例，這些時間範圍顯示見解出現在視頻中的時間範圍。 

1. 要檢索 JSON 檔，請調用[獲取視頻索引 API](https://api-portal.videoindexer.ai/docs/services/Operations/operations/Get-Video-Index?)
1. 如果您也對特定專案感興趣，請致電["獲取視頻工件下載 URL API"](https://api-portal.videoindexer.ai/docs/services/Operations/operations/Get-Video-Artifact-Download-Url?)

    在 API 呼叫中，指定請求的專案類型（OCR、面、鍵幀等）
...
|thumbnailId|影片的縮圖識別碼。 要獲取實際縮略圖，請致電[獲取縮略圖](https://api-portal.videoindexer.ai/docs/services/operations/operations/Get-Video-Thumbnail)並傳遞縮略圖視頻Id和縮略圖Id。|
...
|thumbnailId|影片的縮圖識別碼。 獲取實際縮略圖呼叫[獲取縮略圖](https://api-portal.videoindexer.ai/docs/services/operations/operations/Get-Video-Thumbnail)並傳遞視頻 ID 和縮略圖 Id。|
...
每個見解（例如，腳本行、面、品牌等）包含唯一元素的清單（例如，face1、face2、face3），每個元素都有自己的中繼資料及其實例清單（這是具有其他可選中繼資料的時間範圍）。

人臉可能具有 ID、名稱、縮略圖、其他中繼資料及其時態實例清單（例如：00：00：05 和 00：00：10、00：01：00 - 00：02：30 和 00：41：21 = 00：41：49）。每個臨時實例可以具有其他中繼資料。 例如，臉部的矩形座標 (20,230,60,60)。
...
|擷取畫面|[鏡頭](#shots)洞察。|
|brands|[品牌](#brands)洞察。|
|audioEffects|[音訊效果](#audioEffects)見解。|
|人氣|[情緒](#sentiments)洞察。|
|visualContentModeration|[可視內容適度](#visualcontentmoderation)洞察。|
|textualContentModeration|[文本內容 Moderation](#textualcontentmoderation)見解。|
|emotions| [情感](#emotions)洞察。|
|topics|[主題](#topics)見解。|
...
|---|---|
|id|擷取畫面識別碼。|
|keyFrames|鏡頭中的關鍵幀清單（每個清單都有一個 ID 和實例時間範圍清單）。 每個關鍵幀實例都有一個縮略圖 Id 欄位，該欄位包含關鍵幀的縮略圖 ID。|
|執行個體|此鏡頭的時間範圍清單（拍攝只能有 1 個實例）。|
...
|id|品牌識別碼。|
|NAME|品牌名稱。|
|referenceId | 品牌 Wikipedia URL 的尾碼。 例如，"Target_Corporation"是 的[https://en.wikipedia.org/wiki/Target_Corporation](https://en.wikipedia.org/wiki/Target_Corporation)尾碼。
...
視頻索引子根據語音和音訊提示識別情緒。被識別的情緒可以是：喜悅、悲傷、憤怒或恐懼。

|名稱|描述|
|---|---|
|id|表情識別碼。|
|type|根據言語和音訊提示確定的情感時刻。情緒可能是：快樂、悲傷、憤怒或恐懼。|
|執行個體|這一個表情出現的時間範圍清單。|
...
影片索引子會從文字記錄中推斷主要主題。 如果可能，包括二級[IPTC](https://iptc.org/standards/media-topics/)分類。 
