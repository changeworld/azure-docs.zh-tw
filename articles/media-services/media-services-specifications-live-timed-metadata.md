...
| RTMP            | [「Adobe 的 Real-Time 訊息通訊協定」，2012年12月21日](https://www.adobe.com/devnet/rtmp.html)                                                                                                            |
| [SCTE-35-2019]    | SCTE 35： 2019-適用于纜線的數位程式插入提示訊息- https://www.scte.org/SCTEDocs/Standards/ANSI_SCTE%2035%202019r1.pdf                                                                       |
| [SCTE-214-1]      | SCTE 214-1 2016 – IP-Based 纜線服務的 MPEG 虛線第1部分： MPD 限制和延伸                                                                                                                 |
| [SCTE-214-3]      | IP-Based 纜線服務的 SCTE 214-3 2015 MPEG 破折號第3部分：破折號/FF 設定檔                                                                                                                                  |
...
| 欄位名稱 | 欄位類型 | 必要？ | 說明                                                                                                                                                                                                                                                                        |
| ---------- | ---------- | --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| type       | 字串     | 必要  | 事件訊息。  應該是 "SpliceOut" 以指定簡單模式拼接。                                                                                                                                                                                                         |
...
| 欄位名稱 | 欄位類型 | 必要？ | 說明                                                                                                                                                                                                                                                                                                                                                                                                        |
| ---------- | ---------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 提示        | 字串     | 必要  | 事件訊息。  針對 [SCTE-35] 訊息，這必須是 base64 編碼的 [RFC4648] 二進位 splice_info_section ( # A1，才能將訊息傳送至 HLS、流暢和虛線的用戶端。                                                                                                                                                                                                                               |
| type       | 字串     | 必要  | 可識別訊息配置的 URN 或 URL。 針對 [SCTE-35] 訊息，這 **應該** 是 **"scte35"** ，以便將訊息傳送至 HLS、平滑和虛線用戶端，以符合 [Adobe-Primetime]。 （選擇性）您也可以使用 URN "urn： scte： scte35：2013： bin" 來發出 [SCTE-35] 訊息的信號。                                                                                                        |
...
| parentTrackName    | 字串         | 必要      | **必須** 是父代的名稱，而稀疏軌的時間碼會對齊。 父系播放軌不能是疏鬆播放軌。                                                                             |
| manifestOutput     | 布林值        | 必要      | **必須** 是 "true"，以指出將內嵌在平滑用戶端資訊清單中的稀疏播放軌。                                                                                                                        |
| Subtype            | 字串         | 必要      | **必須** 是四個字元碼「資料」。                                                                                                                                                                                  |
| 配置             | 字串         | 必要      | **必須** 是可識別訊息配置的 URN 或 URL。 針對 [SCTE-35] 訊息，這 **必須** 是 "urn： SCTE： scte35：2013： bin"，才能將訊息傳送給 HLS、流暢和虛線用戶端，以符合 [SCTE-35]。 |
| trackName          | 字串         | 必要      | **必須** 是稀疏播放軌的名稱。Trackname 前可以用來區分具有相同配置的多個事件資料流程。 每個唯一的事件資料流程都 **必須** 有唯一的追蹤名稱。                                |
| timescale          | 數字         | 選用      | **必須** 是父代播放軌的刻度。                                                                                                                                                                               |
