---
title: Azure 媒體服務-在即時串流中發出計時中繼資料的信號
description: 此規格概述擷取和串流處理至 Azure 媒體服務時，發出計時中繼資料通知的方法。 這包括 (ID3) 的一般計時中繼資料信號的支援，以及廣告插入和拼接條件信號信號的 SCTE-35 信號。
...
為了指示在用戶端播放程式上插入廣告或自訂中繼資料事件，廣播者通常會利用內嵌于影片內的計時中繼資料。 為了實現這些案例，媒體服務支援將計時中繼資料從即時串流通道的內嵌點傳輸至用戶端應用程式。
此規格概述媒體服務在即時串流信號內的計時中繼資料所支援的數種模式。

1. [SCTE-35] 符合 [SCTE-35]、[SCTE-214-1]、[SCTE-214-3] 和 [RFC8216] 所述標準的信號

2. [SCTE-35] 信號符合 RTMP 廣告信號的舊版 [Adobe Primetime] 規格。
   
3. 一般計時中繼資料信號模式，適用于 **非** [SCTE-35] 的訊息，而且可以包含 [ID3v2] 或應用程式開發人員所定義的其他自訂架構。

## <a name="11-terms-used"></a>使用的1.1 詞彙

| 詞彙                | 定義                                                                                                                                                                                                                                    |
| ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Ad Break            | 可以排程一或多個廣告進行傳遞的位置或時間點;與可用和放置機會相同。                                                                                                                     |
| Ad 決策服務 | 外部服務會決定要向使用者顯示哪些 ad () 和持續時間。 這些服務通常是由合作夥伴提供，不在本檔的討論範圍內。                                                                    |
| 提示                 | 指出即將推出之廣告中斷的時間和參數。 請注意，提示可能會指出暫止的切換至廣告中斷、暫止切換至廣告內的下一個廣告，以及擱置的參數從廣告中斷切換至主要內容。           |
| 封裝程式            | 「串流端點」 Azure 媒體服務提供虛線和 HLS 的動態封裝功能，並在媒體產業中稱為「封裝程式」。                                                                              |
...
## <a name="12-normative-references"></a>1.2 標準化參考

下列檔包含的條款，可透過此文字中的參考，構成這份檔的條款。 所有檔都受限於標準本文的修訂，並且建議讀者調查套用下列最新版本檔的可能性。 讀者也會提醒您，較新版本的參考檔可能與此版本的計時中繼資料規格不相容 Azure 媒體服務。
...
| [虛線-IF-IOP]     | 虛線產業論壇 Interop 指導方針 v 4。2 [https://dashif-documents.azurewebsites.net/DASH-IF-IOP/master/DASH-IF-IOP.html](https://dashif-documents.azurewebsites.net/DASH-IF-IOP/master/DASH-IF-IOP.html)    |
| [HLS-TMD]         | HTTP 即時串流的計時中繼資料- [https://developer.apple.com/streaming](https://developer.apple.com/streaming)                                                                                        |
| [CMAF-ID3]        | [一般媒體應用程式格式的計時中繼資料 (CMAF) ](https://github.com/AOMediaCodec/id3-emsg)                                                                                                        |
| ID3v2           | ID3 標記版本2.4。0  [http://id3.org/id3v2.4.0-structure](http://id3.org/id3v2.4.0-structure)                                                                                                                |
| [ISO-14496-12]    | ISO/IEC 14496-12： Part 12 ISO base media 檔案格式，FourthEdition 2012-07-15                                                                                                                                 |
| [MPEGDASH]        | 資訊技術--透過 HTTP 的動態彈性串流 (虛線) --第1部分：媒體呈現描述和區段格式。 5月2014。 發表。 URL： https://www.iso.org/standard/65274.html         |
| [MPEGCMAF]        | 資訊技術--多媒體應用程式格式 (MPEG-A) --第19部：通用媒體應用程式格式 (CMAF 適用于分段媒體的) 。 2018年1月。 發表。 URL： https://www.iso.org/standard/71975.html |
| [MPEGCENC]        | 資訊技術--MPEG 系統技術--第7部分： ISO base media 檔案格式檔案中的一般加密。 2016年2月。 發表。 URL： https://www.iso.org/standard/68042.html                   |
| [MS-SSTR]         | ["Microsoft Smooth Streaming Protocol"，2014 5 月15日](/openspecs/windows_protocols/ms-sstr/8383f27f-7efe-4c60-832a-387274457251)                                                     |
| [毫秒-MS-SSTR-內嵌]  | [Azure 媒體服務分散的有量的即時內嵌規格](./media-services-fmp4-live-ingest-overview.md)                                                      |
| [RFC8216]         | R. Pantos，Ed.;W：可能。 HTTP 即時串流。 2017年8月。 資訊。 [https://tools.ietf.org/html/rfc8216](https://tools.ietf.org/html/rfc8216)                                                            |
| RFC4648         | Base16、Base32 和 Base64 資料編碼- [https://tools.ietf.org/html/rfc4648](https://tools.ietf.org/html/rfc4648)                                                                                     |
| RTMP            | [2012年12月21日的「Adobe 的即時訊息通訊協定」](https://www.adobe.com/devnet/rtmp.html)                                                                                                            |
| [SCTE-35-2019]    | SCTE 35： 2019-適用于纜線的數位程式插入提示訊息- https://www.scte.org/SCTEDocs/Standards/ANSI_SCTE%2035%202019r1.pdf                                                                       |
| [SCTE-214-1]      | SCTE 214-1 2016 –以 IP 為基礎的纜線服務的 MPEG 虛線第1部分： MPD 限制和延伸                                                                                                                 |
| [SCTE-214-3]      | 以 IP 為基礎的纜線服務的 SCTE 214-3 2015 MPEG 破折號第3部分：破折號/FF 設定檔                                                                                                                                  |
...
Azure 媒體服務針對 [RTMP] 和 Smooth Streaming [MS-SSTR] 通訊協定支援即時的頻內中繼資料。 即時中繼資料可以用來定義自訂事件，其中包含您自己獨特的自訂架構 (JSON、二進位、XML) ，以及產業定義格式（例如 ID3）或 SCTE-35 （適用于廣播串流中的廣告信號）。 

本文提供詳細資料，說明如何使用 Azure 媒體服務的支援內嵌通訊協定來傳送自訂計時中繼資料信號。 本文也會說明如何使用計時中繼資料信號裝飾 HLS、破折號和 Smooth Streaming 的資訊清單，以及如何在使用 CMAF (數量的片段) 或傳輸串流 (TS) 區段進行 HLS 時，以頻外方式來傳遞內容。 

計時中繼資料的常見使用案例案例包括：

 - SCTE-35 ad 信號，可在實況活動或線性廣播中觸發廣告中斷
 - 可以在用戶端應用程式 (瀏覽器、iOS 或 Android 上觸發事件的自訂 ID3 中繼資料) 
 - 自訂的 JSON、二進位或 XML 中繼資料，可在用戶端應用程式觸發事件
 - 來自即時編碼器、IP 攝影機或無人機的遙測
 - IP 攝影機中的事件，例如移動、臉部偵測等等。
 - 來自動作攝影機、無人機或移動裝置的地理位置資訊
 - 歌詞
 - 線性即時饋送的程式界限
 - 要在即時摘要中顯示的影像或增強型中繼資料
 - 體育分數或遊戲時鐘資訊
 - 要在瀏覽器中與影片一起顯示的互動式廣告套件
 - 測驗或投票
  
Azure 媒體服務即時事件和封裝程式能夠接收這些計時中繼資料信號，並將其轉換成中繼資料串流，以使用標準通訊協定（例如 HLS 和虛線）來觸達用戶端應用程式。


## <a name="21-rtmp-timed-metadata"></a>2.1 RTMP 計時中繼資料

[RTMP] 通訊協定可讓您針對各種案例（包括自訂中繼資料和 SCTE-35 ad 信號）傳送計時中繼資料信號。 

通告 (提示訊息) 會以內嵌在 [RTMP] 串流內的 [AMF0] 提示訊息形式傳送。 提示訊息可能會在實際事件或 [SCTE35] ad 拼接信號需要發生之前傳送。 為了支援這種情況，事件的實際呈現時間戳記會在提示訊息中傳送。 如需詳細資訊，請參閱 [AMF0]。

下列 [AMF0] 命令受 Azure 媒體服務 RTMP 內嵌所支援：

- **onUserDataEvent** -用於自訂中繼資料或 [ID3v2] 計時中繼資料
- **onAdCue** -主要用於通知即時串流中的廣告放置機會。 支援兩種形式的提示：簡單模式和「SCTE-35」模式。 
- **onCuePoint** -由某些內部部署硬體編碼器（例如 Elemental 即時編碼器）支援，以發出信號 [SCTE35] 訊息。 
  

下表描述媒體服務將針對 "simple" 和 [SCTE35] 訊息模式內嵌的 AMF 訊息裝載格式。

[AMF0] 訊息的名稱可以用來區分相同類型的多個事件資料流程。  針對 [SCTE-35] 訊息和 [簡單] 模式，AMF 訊息的名稱必須是 [Adobe-Primetime] 規格中所需的 "onAdCue"。  內嵌時 Azure 媒體服務應忽略未列于下方的任何欄位。

## <a name="211-rtmp-with-custom-metadata-using-onuserdataevent"></a>2.1.1 RTMP 搭配使用 "onUserDataEvent" 的自訂中繼資料

如果您想要使用 RTMP 通訊協定，從上游編碼器、IP 攝影機、無人機或裝置提供自訂中繼資料摘要，請使用 "onUserDataEvent" [AMF0] 資料訊息命令類型。

**"OnUserDataEvent"** 資料訊息命令必須包含具有下列定義的訊息承載：媒體服務可加以捕獲並封裝為內含式檔案格式，以及 HLS、破折號和 Smooth Streaming 的資訊清單。
建議您每隔0.5 秒傳送一次計時中繼資料訊息， (500 毫秒) 或可能發生即時串流的穩定性問題。 如果您需要提供框架層級的中繼資料，每個訊息都可以從多個框架匯總中繼資料。 如果您要傳送多位元率串流，建議您只在單一位元速率上提供中繼資料，以減少頻寬並避免干擾視頻/音訊處理。 

**"OnUserDataEvent"** 的承載應該是 [MPEGDASH] EventStream XML 格式訊息。 這可讓您輕鬆地傳入自訂定義的架構，此架構可在 CMAF [MPEGCMAF] 透過 HLS 或虛線通訊協定傳遞的內容中，以 ' emsg ' 承載的形式傳遞。 每個虛線事件資料流程訊息都包含一個 schemeIdUri，可作為 URN 訊息配置識別碼，並定義訊息的承載。 某些配置（例如 " https://aomedia.org/emsg/ID3 " for [ID3v2]）或 **urn： scte： scte35：2013： bin** （適用于 [scte-35]）是由產業聯盟針對互通性所標準化。 任何應用程式提供者都可以使用其控制 (擁有的網域) 的 URL 來定義自己的自訂配置，並在選擇時提供該 URL 的規格。 如果播放程式具有已定義配置的處理常式，則這是唯一需要瞭解承載和通訊協定的元件。

[MPEG] EventStream XML 承載的架構定義為從破折號-IEC-23009-1-第3版)  (摘錄。 請注意，目前只支援每個 "EventStream" 有一個「具有類型」。 如果**EventStream**中提供了多個事件，則只會處理第一個**事件**元素。
...
### <a name="example-event-stream-with-custom-schema-id-and-base64-encoded-binary-data"></a>使用自訂架構識別碼和 base64 編碼二進位資料的事件資料流程範例  
```xml
   <?xml version="1.0" encoding="UTF-8"?>
...
### <a name="example-event-stream-with-custom-schema-id-and-custom-json"></a>使用自訂架構識別碼和自訂 JSON 的事件資料流程範例  
```xml
   <?xml version="1.0" encoding="UTF-8"?>
...
| 配置識別碼 URI                 | 說明                                                                                                                                                                                                                                          |
| ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| HTTPs： \/ /aomedia.org/emsg/ID3 | 描述如何將 [ID3v2] 中繼資料當作 CMAF 相容 [MPEGCMAF] 中的計時中繼資料來傳送。 如需詳細資訊，請參閱 [通用媒體應用程式格式中的計時中繼資料 (CMAF) ](https://github.com/AOMediaCodec/id3-emsg) |

### <a name="event-processing-and-manifest-signaling"></a>事件處理和資訊清單信號

收到有效的 **"onUserDataEvent"** 事件時，Azure 媒體服務會尋找符合 [MPEGDASH] ) 中所定義之 EventStreamType (的有效 XML 承載、剖析 XML 承載，並將其轉換為 [MPEGCMAF] 的 [] emsg ' 第1版] 方塊，以在即時封存中儲存並傳輸至媒體服務封裝程式。   封裝程式會偵測到即時資料流中的 [emsg] 方塊，以及：

-  () 「動態封裝」到 TS 區段以傳遞至 HLS 用戶端，以符合 HLS 計時中繼資料規格 [HLS-TMD]，或
-  (b) 透過 HLS 或虛線傳遞 CMAF 片段的傳遞，或 
-  (c) 將它轉換成可透過 Smooth Streaming [MS-SSTR] 傳遞的稀疏追蹤信號。

除了適用于 HLS 的頻內 ' emsg ' 格式 CMAF 或 TS PE 封包之外，虛線 (MPD) 和 Smooth Streaming 的資訊清單也會包含內含式事件資料流程的參考 (也稱為 Smooth Streaming) 中的稀疏資料流程追蹤。

個別事件或其資料承載不會直接在 HLS、虛線或平滑資訊清單中輸出。 

### <a name="additional-informational-constraints-and-defaults-for-onuserdataevent-events"></a>OnUserDataEvent 事件的其他資訊條件約束和預設值

- 如果未在 EventStream 元素中設定時間刻度，則預設會使用 RTMP 1 kHz 的刻度
- OnUserDataEvent 訊息的傳遞限制為每個500毫秒最大值一次。如果您更頻繁地傳送事件，它可能會影響即時饋送的頻寬和穩定性

## <a name="212-rtmp-ad-cue-signaling-with-onadcue"></a>2.1.2 RTMP ad 提示以 "onAdCue" 發出信號

Azure 媒體服務可以接聽和回應數個 [AMF0] 訊息類型，這些類型可用來對即時資料流中的各種即時同步中繼資料發出信號。  [Adobe Primetime] 規格會定義兩個稱為 "simple" 和 "SCTE-35" 模式的提示類型。 針對「簡單」模式，媒體服務支援單一 AMF 提示訊息（稱為 "onAdCue"），其使用的承載符合下列針對「簡單模式」信號定義的資料表。  

下一節顯示 RTMP 「簡單」模式」承載，可用來表示將傳送至用戶端資訊清單以進行 HLS、破折號和 Microsoft Smooth Streaming 的基本 "spliceOut" 廣告信號。 如果客戶沒有複雜的 SCTE 35 型廣告信號部署或插入系統，且使用基本的內部部署編碼器透過 API 傳送提示訊息，這就非常有用。 一般而言，內部部署編碼器將支援 REST 型 API 來觸發此信號，此信號也會將 IDR 的畫面格插入影片，並啟動新的 GOP，藉以「拼接條件」影片串流。

## <a name="213--rtmp-ad-cue-signaling-with-onadcue---simple-mode"></a>2.1.3 RTMP ad 提示以 "onAdCue" 進行信號-簡單模式

| 欄位名稱 | 欄位類型 | 必要？ | 說明                                                                                                                                                                                                                                                                        |
| ---------- | ---------- | --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 類型       | String     | 必要  | 事件訊息。  應該是 "SpliceOut" 以指定簡單模式拼接。                                                                                                                                                                                                         |
| id         | String     | 必要  | 描述拼接或區段的唯一識別碼。 識別訊息的此執行個體                                                                                                                                                                                       |
| duration   | 數字     | 必要  | 拼接的持續時間。 單位是小數秒。                                                                                                                                                                                                                           |
| elapsed    | 數字     | 選擇性  | 當訊號重複以支援收聽時，此欄位應該是開始拼接後經過的呈現時間量。 單位是小數秒。 當使用簡單模式時，此值不應該超過拼接的原始時間長度。 |
| time       | 數字     | 必要  | 應該是拼接的時間 (以呈現時間表示)。 單位是小數秒。                                                                                                                                                                                                |

---
 
#### <a name="example-mpeg-dash-manifest-output-when-using-adobe-rtmp-simple-mode"></a>使用 Adobe RTMP 簡單模式時的 MPEG 虛線資訊清單輸出範例

請參閱範例 [3.3.2.1 MPEG 虛線. Mpd EventStream 使用 Adobe simple mode](#3321-example-mpeg-dash-mpd-manifest-signaling-of-rtmp-streaming-using-adobe-simple-mode)

請參閱 [使用單一句點和 Adobe simple 模式的範例3.3.3.1 虛線資訊清單 ](#3331-example-mpeg-dash-manifest-mpd-with-single-period-eventstream-using-adobe-simple-mode-signals)

#### <a name="example-hls-manifest-output-when-using-adobe-rtmp-simple-mode"></a>範例 HLS 使用 Adobe RTMP simple 模式時的資訊清單輸出

請參閱[使用 Adobe simple 模式和 EXT X 提示標記的 3.2.2 HLS 資訊清單](#322-apple-hls-with-adobe-primetime-ext-x-cue)範例

## <a name="214-rtmp-ad-cue-signaling-with-onadcue---scte-35-mode"></a>2.1.4 RTMP ad 提示以 "onAdCue"-SCTE-35 模式發出信號

當您使用需要將完整 SCTE-35 承載訊息傳送到 HLS 或虛線資訊清單的更高階廣播生產工作流程時，最好使用 [Adobe-Primetime] 規格的 "SCTE-35 模式"。  此模式支援直接傳送到內部部署即時編碼器的頻外 SCTE-35 信號，然後使用 [Adobe-Primetime] 規格中指定的 "SCTE-35 Mode" 將信號編碼為 RTMP 資料流程。 

SCTE-35 訊息通常只能出現在 MPEG-2 傳輸串流中， (TS) 內部部署編碼器的輸入。 請洽詢您的編碼器製造商，以取得有關如何設定包含 SCTE-35 的傳輸串流內嵌，並讓它在 Adobe SCTE-35 模式下傳遞至 RTMP 的詳細資訊。

在此案例中，必須使用 **"onAdCue"** [AMF0] 訊息類型從內部部署編碼器傳送下列承載。

| 欄位名稱 | 欄位類型 | 必要？ | 說明                                                                                                                                                                                                                                                                                                                                                                                                        |
| ---------- | ---------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 提示        | String     | 必要  | 事件訊息。  針對 [SCTE-35] 訊息，這必須是 base64 編碼的 [RFC4648] 二進位 splice_info_section ( # A1，才能將訊息傳送至 HLS、流暢和虛線的用戶端。                                                                                                                                                                                                                               |
| 類型       | String     | 必要  | 可識別訊息配置的 URN 或 URL。 針對 [SCTE-35] 訊息，這 **應該** 是 **"scte35"** ，以便將訊息傳送至 HLS、平滑和虛線用戶端，以符合 [Adobe-Primetime]。 （選擇性）您也可以使用 URN "urn： scte： scte35：2013： bin" 來發出 [SCTE-35] 訊息的信號。                                                                                                        |
| id         | String     | 必要  | 描述拼接或區段的唯一識別碼。 識別訊息的此執行個體。  具有同等語意的訊息應該有相同的值。                                                                                                                                                                                                                                                       |
| duration   | 數字     | 必要  | 事件或廣告拼接區段的期間 (若已知)。 如果不明，此值 **應** 為0。                                                                                                                                                                                                                                                                                                                    |
| elapsed    | 數字     | 選擇性  | 當 [SCTE-35] 廣告訊號重複以收聽時，此欄位應該是開始拼接後經過的呈現時間量。 單位是小數秒。 在 [SCTE-35] 模式中，此值可能會超過原本指定的拼接或區段持續時間。                                                                                                                   |
| time       | 數字     | 必要  | 事件或拼接的呈現時間。  展示時間和持續時間 **應** 與類型1或 2 (SAP) 的串流存取點一致，如 [ISO-14496-12] 附錄 I 中所定義。針對 HLS 輸出，時間和持續時間 **應** 與區段界限對齊。 呈現時間與相同事件串流中不同事件訊息的期間不能重疊。 單位是小數秒。 |
...
#### <a name="example-hls-manifest-m3u8-with-scte-35-mode-signal"></a>範例 HLS 資訊清單. m3u8 與 SCTE-35 模式的信號
請參閱 [3.2.1.1 範例 HLS 資訊清單和 SCTE-35](#3211-example-hls-manifest-m3u8-showing-ext-x-cue-signaling-of-scte-35)



## <a name="215-rtmp-ad-signaling-with-oncuepoint-for-elemental-live"></a>2.1.5 RTMP Ad 針對 Elemental Live 以 "onCuePoint" 發出信號

Elemental Live 內部部署編碼器支援 RTMP 信號中的廣告標記。 Azure 媒體服務目前僅支援 RTMP 的 "onCuePoint" 廣告標記類型。  您可以在 Elemental 媒體即時編碼器設定或 API 的 Adobe RTMP 群組設定中啟用此功能，方法是將 "**ad_markers**" 設定為 "onCuePoint"。  如需詳細資訊，請參閱 Elemental 即時檔。 在 RTMP 群組中啟用這項功能，會將 SCTE-35 信號傳遞給 Adobe RTMP 輸出，以 Azure 媒體服務處理。

"OnCuePoint" 訊息類型是在 [Adobe-Flash AS] 中定義，而且在從 Elemental 即時 RTMP 輸出傳送時，會有下列承載結構。


| 屬性   | 描述                                                                                                                                                                                                                     |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| NAME       | Elemental Live 的名稱應該是 '**scte35**'。                                                                                                                                                                              |
| time       | 時間軸中的提示點在影片檔案中發生的時間（秒）                                                                                                                                           |
| 類型       | 提示點的類型應該設定為 "**event**"。                                                                                                                                                                             |
| 參數 | 名稱/值組字串的關聯陣列，包含 SCTE-35 訊息中的資訊，包括識別碼和持續時間。 這些值會由 Azure 媒體服務剖析，並包含在資訊清單裝飾標記中。 |


使用此 ad 標記模式時，HLS 資訊清單輸出類似于 Adobe "Simple" 模式。


#### <a name="example-mpeg-dash-mpd-single-period-adobe-simple-mode-signals"></a>範例 MPEG 破折號 MPD、單一期間、Adobe Simple mode 信號
...
#### <a name="example-hls-playlist-adobe-simple-mode-signals-using-ext-x-cue-tag-truncated--for-brevity"></a>範例 HLS 播放清單，Adobe Simple mode 使用 EXT X 提示標記 (截斷的 "..."為了簡潔起見) 

下列範例顯示使用 Adobe "simple" 模式信號和舊版 [Adobe-Primetime] EXT X 提示標籤的 RTMP 內嵌資料流程的媒體服務動態封裝輸出。  
...
如需即時資料流內嵌的需求，請參閱 [MS-MS-SSTR]。 以下各節提供有關計時呈現中繼資料內嵌的詳細資料。  計時呈現中繼資料會內嵌為「即時伺服器資訊清單」方塊中所定義的稀疏軌 (請參閱 MS-SSTR) 和 Movie Box ( ' moov ' ) 。  

每個稀疏片段都是由電影片段方塊 ( ' moof ' ) 和 Media Data Box ( ' mdat ' ) 所組成，其中 ' mdat ' 方塊是二進位訊息。

為了達到框架精確的廣告插入，編碼器必須在需要插入提示的呈現時間分割片段。  必須建立新的片段，其開頭為新建立的 IDR 框架，或串流存取點 (類型1或2的 SAP) ，如 [ISO-14496-12] 附錄 I 中所定義。
...
您 **必須** 在具有專案的即時伺服器資訊清單方塊中宣告稀疏播放軌 **\<textstream\>** ，而且 **必須** 設定下列屬性：

| **屬性名稱** | **欄位類型** | **必要？** | **說明**                                                                                                                                                                                                              |
| ------------------ | -------------- | ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| systemBitrate      | 數字         | 必要      | **必須** 是 "0"，表示具有未知、變數位元速率的追蹤。                                                                                                                                                          |
| parentTrackName    | String         | 必要      | **必須** 是父代的名稱，而稀疏軌的時間碼會對齊。 父系播放軌不能是疏鬆播放軌。                                                                             |
| manifestOutput     | Boolean        | 必要      | **必須** 是 "true"，以指出將內嵌在平滑用戶端資訊清單中的稀疏播放軌。                                                                                                                        |
| Subtype            | String         | 必要      | **必須** 是四個字元碼「資料」。                                                                                                                                                                                  |
| 配置             | String         | 必要      | **必須** 是可識別訊息配置的 URN 或 URL。 針對 [SCTE-35] 訊息，這 **必須** 是 "urn： SCTE： scte35：2013： bin"，才能將訊息傳送給 HLS、流暢和虛線用戶端，以符合 [SCTE-35]。 |
| trackName          | String         | 必要      | **必須** 是稀疏播放軌的名稱。Trackname 前可以用來區分具有相同配置的多個事件資料流程。 每個唯一的事件資料流程都 **必須** 有唯一的追蹤名稱。                                |
| timescale          | 數字         | 選擇性      | **必須** 是父代播放軌的刻度。                                                                                                                                                                               |

---

### <a name="222-movie-box"></a>2.2.2 影片方塊

[電影] 方塊 ( [moov] ) 接在即時伺服器資訊清單方塊中，作為稀疏軌的資料流程標頭的一部分。

' Moov ' 方塊 **應** 包含 **TrackHeaderBox ( ' tkhd ' ) ** BOX，如 [ISO-14496-12] 中所定義，且具有下列限制：

| **功能變數名稱** | **欄位類型**          | **必要？** | **說明**                                                                                                    |
| -------------- | ----------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------ |
| duration       | 64 位元不帶正負號的整數 | 必要      | **應該** 是0，因為追蹤方塊中有零個樣本，而 [追蹤] 方塊中樣本的總持續時間是0。 |

---

' Moov ' 方塊 **應** 包含 **HandlerBox ( ' hdlr ' ) ** 如 [ISO-14496-12] 中所定義，且具有下列限制：

| **功能變數名稱** | **欄位類型**          | **必要？** | **說明**       |
| -------------- | ----------------------- | ------------- | --------------------- |
| handler_type   | 32 位元不帶正負號的整數 | 必要      | **應** 為 ' meta '。 |

---

' Stsd ' 方塊 **應該** 包含具有編碼名稱的 MetaDataSampleEntry 方塊，如 [ISO-14496-12] 中所定義。  例如，針對 SCTE-35 訊息，編碼名稱 **應** 為 ' SCTE '。

### <a name="223-movie-fragment-box-and-media-data-box"></a>2.2.3 影片片段方塊與媒體資料方塊

稀疏軌片段包含 ( ' moof ' ) 的影片片段方塊，以及 ( ' mdat ' ) 的 Media Data Box。

> [!NOTE]
> 為了達到框架精確的廣告插入，編碼器必須在需要插入提示的呈現時間分割片段。  必須建立新的片段，其開頭為新建立的 IDR 框架，或串流存取點 (類型1或2的 SAP) ，如 [ISO-14496-12] 附錄 I 中所定義
> 

MovieFragmentBox ( ' moof ' ) 方塊 **必須** 包含 **TrackFragmentExtendedHeaderBox ( ' uuid ' ) ** 方塊，如 [MS-ms-sstr] 中所定義，且具有下欄欄位：

| **功能變數名稱**         | **欄位類型**          | **必要？** | **說明**                                                                                           |
| ---------------------- | ----------------------- | ------------- | --------------------------------------------------------------------------------------------------------- |
| fragment_absolute_time | 64 位元不帶正負號的整數 | 必要      | **必須** 是事件的抵達時間。 此值與父代播放軌的訊息一致。           |
| fragment_duration      | 64 位元不帶正負號的整數 | 必要      | **必須** 是事件的持續時間。 持續時間可以是零，指出持續時間未知。 |

---


MediaDataBox ( ' mdat ' ) box **必須** 具有下列格式：
...
| presentation_time_delta | 32 位元不帶正負號的整數 (uimsbf) | 必要      | Fragment_absolute_time 的總和（在 TrackFragmentExtendedHeaderBox 中指定）和 presentation_time_delta **必須** 是事件的呈現時間。 展示時間和持續時間 **應** 與類型1或 2 (SAP) 的串流存取點一致，如 [ISO-14496-12] 附錄 I 中所定義。針對 HLS 輸出，時間和持續時間 **應** 與區段界限對齊。 相同事件串流中不同事件訊息的呈現時間和持續時間 **不得** 重迭。 |
| 訊息                 | 位元組陣列                       | 必要      | 事件訊息。 針對 [SCTE-35] 訊息，訊息是二進位 splice_info_section ( # A1。 針對 [SCTE-35] 訊息，這 **必須** 是 ( # A1 的 splice_info_section，才能將訊息傳送給 HLS、順暢和虛線用戶端，以符合 [SCTE-35]。 對於 [SCTE-35] 訊息，二進位 splice_info_section ( # A1 是 ' mdat ' 方塊的承載，而且 **不** 是 base64 編碼。                                                                                                                     |
...
事件串流資料對「媒體服務」而言是不透明的。 「媒體服務」只會在內嵌端點與用戶端端點之間傳送三小部分的資訊。 下列屬性會傳遞至用戶端，以符合 [SCTE-35] 和/或用戶端的串流通訊協定：
...
如需有關如何格式化稀疏訊息追蹤的詳細資訊，請參閱「稀疏追蹤處理」 MS-SSTR。若為 [SCTE35] 訊息，Smooth Streaming 會將 base64 編碼的 splice_info_section ( # A1 輸出到稀疏片段。
StreamIndex **必須** 有 "DATA" 的子類型，而且 CustomAttributes **必須** 包含 Name = "Schema" 和 Value = "urn： scte： scte35：2013： bin" 的屬性。

#### <a name="smooth-client-manifest-example-showing-base64-encoded-scte35-splice_info_section"></a>顯示 base64 編碼 [SCTE35] splice_info_section ( # A1 的平滑用戶端資訊清單範例
```xml
<?xml version="1.0" encoding="utf-8"?>
...
- 在 [Adobe-Primetime] 中定義的 EXT X 提示
<!--- this mode is considered "legacy".  Customers should adopt the EXT-X-DATERANGE tag when possible. --->

每個標記的資料輸出會根據所使用的內嵌信號模式而有所不同。 例如，使用 Adobe Simple 模式的 RTMP 內嵌未包含完整的 SCTE-35 base64 編碼承載。
...
## <a name="3211-example-hls-manifest-m3u8-showing-ext-x-cue-signaling-of-scte-35"></a>3.2.1.1 範例 HLS 資訊清單. m3u8 顯示 SCTE-35 的 EXT X 提示信號

下列範例 HLS 媒體服務動態封裝程式的資訊清單輸出，在 SCTE35 模式中顯示 [Adobe Primetime] 的 EXT-X 提示標記。 
...
媒體服務 (第2版和第3版 API) 支援在 [Adobe-Primetime] "SCTE-35 模式" 中定義的 EXT X 提示標記輸出。 在此模式中，Azure 媒體服務會在 EXT X 提示標記中內嵌 base64 編碼的 [SCTE-35] splice_info_section ( # A1。  

"Legacy" EXT X 提示標記定義如下，而且也可以在 [Adobe Primetime] 規格中參考。 這應該只在需要時用於舊版 SCTE35 信號，否則建議的標記會在 [RFC8216] 中定義為 EXT-DATERANGE。 

| **屬性名稱** | **型別**                      | **必要？**                             | **說明**                                                                                                                                                                                                                                                                          |
| ------------------ | ----------------------------- | ----------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 提示                | 加上括號的字串                 | 必要                                  | 編碼為 base64 編碼字串的訊息，如 [RFC4648] 中所述。 針對 [SCTE-35] 訊息，這是 base64 編碼的 splice_info_section ( # A1。                                                                                                                                      |
| TYPE               | 加上括號的字串                 | 必要                                  | 可識別訊息配置的 URN 或 URL。 對於 [SCTE-35] 訊息，類型會採用特殊值 "scte35"。                                                                                                                                                                          |
| 識別碼                 | 加上括號的字串                 | 必要                                  | 事件的唯一識別碼。 如果內嵌訊息時未指定 ID，「Azure 媒體服務」將會產生一個唯一 ID。                                                                                                                                              |
| 持續時間           | 十進位浮點數數字 | 必要                                  | 事件的持續時間。 如果不明，此值 **應** 為0。 單位是小數秒。                                                                                                                                                                                           |
| ELAPSED            | 十進位浮點數數字 | 選擇性，但對滑動視窗而言是必要的 | 當信號重複以支援滑動呈現視窗時，此欄位 **必須** 是自活動開始以來經過的呈現時間量。 單位是小數秒。 此值可能會超過原本指定的拼接或區段持續時間。 |
...
### <a name="323-hls-m3u8-manifest-example-using-adobe-primetime-ext-x-cue"></a>3.2.3 HLS. 使用 Adobe Primetime EXT-X 提示的 m3u8 資訊清單範例

下列範例顯示使用 Adobe Primetime EXT-X 提示標記的 HLS 資訊清單裝飾。  "提示" 參數僅包含類型和持續時間屬性，表示這是使用 Adobe 「簡單」模式信號的 RTMP 來源。  
...
### <a name="324-hls-message-handling-for-adobe-primetime-ext-x-cue"></a>Adobe Primetime EXT-X 的 3.2.4 HLS 訊息處理-提示

事件會在每個影片和音訊播放軌的區段播放清單中收到信號。EXT X 提示標籤的位置 **必須** 緊接在第一個 HLS 區段之前，在)  (區段的第一個區段之前，或在最後一個 HLS 區段之後，在其時間與持續時間屬性所參考的第一個區段)  (（如 [Adobe-Primetime] 所要求）。

當滑動呈現視窗啟用時，在區段播放清單中一律完全描述**MUST**接合或區段，而且已使用的屬性**必須**用來指出拼接或區段的使用時間量（如 [Adobe-Primetime] 所要求）。」。」

當滑動呈現視窗啟用時，若它們所指的媒體時間已從滑動呈現視窗滾出，標籤會從區段播放清單移除。

## <a name="33-dash-manifest-decoration-mpd"></a>3.3 虛線資訊清單裝飾 (MPD) 

[MPEGDASH] 提供三種方式來通知事件：

1.  在 MPD EventStream 中收到的事件
2.  使用事件訊息方塊在頻外發出的事件 ( ' emsg ' ) 
3.  1 與 2 兩者的組合

MPD EventStream 中發出信號的事件適用于 VOD 串流，因為用戶端可以在下載 MPD 時立即存取所有的事件。 它也適用于 SSAI 信號，其中下游 SSAI 廠商需要從 MPD 資訊清單剖析信號，並動態插入廣告內容。  頻內 ( ' emsg ' ) 方案適用于即時串流，用戶端不需要再次下載 MPD，或在用戶端和來源之間沒有任何 SSAI 資訊清單操作發生。 

Azure 媒體服務虛線的預設行為是在 MPD EventStream 中，以及使用事件訊息方塊 ( ' emsg ' ) 的頻外來發出信號。

在 [RTMP] 或 [MS-SSTR] 上內嵌的提示訊息，會使用頻內 ' emsg ' 方塊和/或 in-MPD Eventstream 對應到虛線事件。 

頻外 SCTE-35 信號線會遵循 [SCTE-214-3] 中定義的定義和需求，而且也會在 [] 區段中 13.12.2 ( ' SCTE35 Events ' ) 。 

針對頻外 [SCTE-35] 換行，事件訊息方塊 ( ' emsg ' ) 使用 schemeId = "urn： SCTE： scte35：2013： bin"。 針對 MPD 資訊清單裝飾，EventStream schemeId 會使用 "urn： scte： scte35：2014： xml + bin"。  這種格式是事件的 XML 標記法，其中包含以內嵌方式抵達的完整 SCTE-35 訊息的二進位 base64 編碼輸出。 

您可以在 [SCTE-214-1] 秒 6.7.4 (MPD) 和 [SCTE-214-3] sec 7.3.2 (SCTE 35 提示訊息) 中，找到 [SCTE-35] 中的 [] 提示訊息的標準化參考定義。

### <a name="331-mpeg-dash-mpd-eventstream-signaling"></a>3.3.1 MPEG 破折號 (MPD) EventStream 信號

資訊清單 (MPD) 事件的裝飾會在 MPD 中使用 EventStream 元素（出現在句號元素內）來發出信號。 使用的 schemeId 是 "urn： scte： scte35：2014： xml + bin"。


> [!NOTE]
> 為了簡潔起見，[SCTE-35] 允許 (在中使用 base64 編碼的區段，而不是 SpliceInfoSection 專案，) 為完全剖析的提示訊息的換行。
> Azure 媒體服務使用這個「xml + bin」方法在 MPD 資訊清單中發出信號。
> 這也是在 [虛線------IOP] 中使用的建議方法-請參閱「如果 IOP 指導方針中的「 [Ad 插入事件資料流程](https://dashif-documents.azurewebsites.net/DASH-IF-IOP/master/DASH-IF-IOP.html#ads-insertion-event-streams)」一節。
> 
...
| **屬性名稱** | **型別**                | **必要？** | **說明**                                                                                                                                                                                                                                                                                                                                                                         |
| ------------------ | ----------------------- | ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| scheme_id_uri      | 字串                  | 必要      | 識別訊息的配置。 配置是設定為即時伺服器資訊清單方塊中 Scheme 屬性的值。 值 **應該** 是可識別訊息配置的 URN 或 URL;支援的輸出 schemeId 應該是 "urn： scte： scte35：2014： xml + bin" per [SCTE-214-1] sec 6.7.4 (MPD) ，因為服務目前僅支援 "xml + bin"，以簡化 MPD 的程式。 |
| value              | 字串                  | 選用      | 由配置擁有者用來自訂訊息語意的額外字串值。 若要區分多個具有相同配置的事件資料流程， **必須** 將值設定為事件資料流程的名稱 (trackname 前的 [ms-sstr-內嵌] 或 [RTMP] 內嵌) 的 AMF 訊息名稱。                                                                         |
| 時幅          | 32 位元不帶正負號的整數 | 必要      | 以每秒刻度為單位的時間刻度。                                                                                                                                                                                                                                                                                                                                                     |


### <a name="332-example-event-streams-for-mpeg-dash"></a>3.3.2 MPEG 虛線的範例事件串流

#### <a name="3321-example-mpeg-dash-mpd-manifest-signaling-of-rtmp-streaming-using-adobe-simple-mode"></a>3.3.2.1 範例 MPEG 虛線. mpd 使用 Adobe simple 模式 RTMP 串流的資訊清單信號

下列範例示範如何使用 Adobe 「簡單」模式信號，從媒體服務動態封裝 EventStream RTMP 串流的摘要。
...
#### <a name="3322-example-mpeg-dash-mpd-manifest-signaling-of-an-rtmp-stream-using-adobe-scte-35-mode"></a>3.3.2.2 範例 MPEG mpd 使用 Adobe SCTE-35 模式的 RTMP 資料流程的資訊清單信號

下列範例示範如何使用 Adobe SCTE-35 模式信號，從媒體服務動態封裝程式 RTMP 串流的摘要 EventStream。

使用 xml + bin 樣式信號的範例 EventStream 元素（依 [SCTE-214-1]）
...
> [!IMPORTANT]
> 請注意，presentationTime 是轉譯為相對於期間開始時間（而不是訊息抵達時間）之 [SCTE-35] 事件的呈現時間。
> [MPEGDASH] 將定義 Event@presentationTime 為 "指定事件的呈現時間（相對於期間的開始）。
> 呈現時間（以秒為單位）的值是此屬性值與屬性值的除法 EventStream@timescale 。
> 如果不存在，表示時間的值為0。


#### <a name="3331-example-mpeg-dash-manifest-mpd-with-single-period-eventstream-using-adobe-simple-mode-signals"></a>3.3.3.1 範例 MPEG 虛線資訊清單 (MPD) 搭配單句點、EventStream、使用 Adobe simple mode 信號

下列範例顯示使用 Adobe "simple" 模式 ad 信號方法之來源 RTMP 串流媒體服務動態封裝程式的輸出。 輸出為單一期間的資訊清單，顯示使用 schemeId Uri 設定為 "urn： com： adobe： DPI： simple： 2015" 且 value 屬性設為 "simplesignal" 的 EventStream。
每個簡單的信號都是在事件元素中提供 @presentationTime ， @duration 並 @id 根據傳入的簡單信號來填入、和屬性。
...
### <a name="334-mpeg-dash-in-band-event-message-box-signaling"></a>3.3.4 MPEG 虛線頻外事件訊息方塊信號

頻內事件串流要求 MPD 在「自適性集合」層必須有 InbandEventStream 元素。  此元素具有強制 schemeIdUri 屬性和選擇性的時間刻度屬性，此屬性也會出現在事件訊息方塊中 ( ' emsg ' ) 。  具有未在 MPD 中定義之配置識別碼的事件訊息方塊不 **應該** 存在。

若為內建 [SCTE-35] 的換行，信號 **必須** 使用 schemeId = "urn： SCTE： scte35：2013： bin"。
[SCTE-35] 頻外訊息的標準化定義是在 [SCTE-214-3] sec 7.3.2 () 的 SCTE 35 提示訊息中定義。

下列詳細資料會概述用戶端在符合 [SCTE-214-3] 規範的「emsg」中應預期的特定值：

| **功能變數名稱**          | **欄位類型**          | **必要？** | **說明**                                                                                                                                                                                                                                                                                        |
| ----------------------- | ----------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| scheme_id_uri           | 字串                  | 必要      | 識別訊息的配置。 配置是設定為即時伺服器資訊清單方塊中 Scheme 屬性的值。 值 **必須** 是可識別訊息配置的 URN。 若為 [SCTE-35] 訊息，則 **必須** 是 "urn： SCTE： scte35：2013： bin"，以符合 [SCTE-214-3]          |
| 值                   | 字串                  | 必要      | 由配置擁有者用來自訂訊息語意的額外字串值。 若要區分具有相同配置的多個事件串流，值必須設定為事件串流的名稱 (適用於 Smooth 內嵌的 trackName，或適用於 RTMP 內嵌的 AMF 訊息名稱)。 |
| 時幅               | 32 位元不帶正負號的整數 | 必要      | 在 [emsg] 方塊中，[時間] 和 [持續時間] 欄位的時間刻度（以秒數為單位）。                                                                                                                                                                                                            |
| presentation_time_delta | 32 位元不帶正負號的整數 | 必要      | 事件呈現時間與此區段中最早的呈現時間之間的媒體呈現時間差異。 展示時間和持續時間 **應** 與類型1或 2 (SAP) 的串流存取點一致，如 [ISO-14496-12] 附錄 I 中所定義。                                  |
...
| Message_data            | 位元組陣列              | 必要      | 事件訊息。 針對 [SCTE-35] 訊息，訊息資料是二進位 splice_info_section ( # A1 符合 [SCTE-214-3]                                                                                                                                                                        |
...
### <a name="335-dash-message-handling"></a>3.3.5 虛線訊息處理

在 [emsg] 方塊中，針對影片和音訊播放軌，事件會以頻外的方式發出信號。  針對 presentation_time_delta 為 15 秒或低於 15 秒的所有區段要求，都會發生訊號處理。 

當滑動呈現視窗啟用時，若事件訊息的時間與持續時間總和小於資訊清單中媒體資料的時間，事件訊息會從 MPD 移除。  換句話說，當其所指之媒體時間已滾出滑動呈現視窗之外，就會從資訊清單移除事件訊息。

## <a name="4-scte-35-ingest-implementation-guidance-for-encoder-vendors"></a>4. SCTE-適用于編碼器廠商的35內嵌執行指引

下列指導方針是可能會影響編碼器廠商對此規格執行的常見問題。  下列指導方針是根據真實世界的合作夥伴意見反應所收集，讓您更輕鬆地為其他人執行此規格。 

[SCTE-35] 訊息會使用 " **urn： SCTE： scte35：2013： bin" 的配置 "urn：：：2013： bin"** 來內嵌，並使用 [ms-sstr] 內嵌的 " **scte35"** 類型。 為促進 [SCTE-35] 計時的轉換 (以 MPEG-2 傳輸串流呈現時間戳記 (PTS) 為基礎)，PTS (pts_time + pts_adjustment of the splice_time()) 與媒體時間軸之間的對應是由事件呈現時間 (適用於 Smooth 內嵌的 fragment_absolute_time 欄位，或適用於 RTMP 內嵌的時間欄位) 所提供。 對應是必要的，因為 33 位元 PTS 值大約每隔 26.5 小時就會滾動一次。

Smooth Streaming 內嵌 [MS-SSTR-內嵌] 要求 ( ' mdat ' ) 的 Media Data Box **必須** 包含 [SCTE-35] 中所定義的 **Splice_info_section ( # B3 ** 。 

若為 RTMP 內嵌，AMF 訊息的 [提示] 屬性會設定為 [SCTE-35] 中所定義 ** ( # B1 ** 的 base64 編碼 splice_info_section。  


當訊息具有上述格式時，會將它們傳送至 HLS、平滑和虛線用戶端，如上面所定義。  

使用 Azure 媒體服務平臺測試您的執行時，請先使用「傳遞」 LiveEvent 先開始測試，再移至 LiveEvent 編碼的測試。
...
| 日期     | 變更                                                                                                             |
| -------- | ------------------------------------------------------------------------------------------------------------------- |
| 07/2/19  | 已修訂 RTMP 內嵌支援，新增了 Elemental Live 的 RTMP "onCuePoint"                                            |
| 08/22/19 | 已更新以將自訂中繼資料的 OnUserDataEvent 新增至 RTMP                                                          |
| 1/08/20  | 修正 RTMP Simple 和 RTMP SCTE35 模式的錯誤。 從 "onCuePoint" 變更為 "onAdCue"。 已更新簡單模式資料表。 |
| 08/4/20  | 已移除對 DATERANGE 標記的支援，使其符合生產服務中的執行。    |
