...
1. [1] 中的區段3.3.2 會定義一個稱為 **StreamManifestBox** 的選擇性方塊，以供即時內嵌之用。 由於 Azure 負載平衡器的路由邏輯，使用此方塊已遭取代。 內嵌至媒體服務時，「不應」出現方塊。 如果有此方塊，則媒體服務會以無訊息方式將其忽略。
...
1. 如 [1] 中第6節所定義的 [ **即時伺服器資訊清單** ] 方塊中，使用 *parentTrackName* 參數來指定父字幕的名稱。如需詳細資訊，請參閱 [1] 中的第4.2.1.2.1.2 節一節。
...
1. 使用個別的資料流傳送兩個最低的視訊位元速率。 這些資料流程中的每一個都應該包含每個獨特音訊播放軌的複本。例如，支援多種語言時，這些資料流程應該包含每種語言的音訊播放軌。
