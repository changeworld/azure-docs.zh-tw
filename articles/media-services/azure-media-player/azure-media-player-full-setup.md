...
使用 Azure 媒體播放機，您可以從 CDN 託管版本存取腳本。 通常建議您將 JavaScript 放在結尾主體卷 `<body>` 標之前，而不是 `<head>` ，但 Azure 媒體播放機包含 ' HTML5 Shiv '，這必須位於較舊 IE 版本的前端，才能將影片標記視為有效的元素。

> [!NOTE]
> 如果您已經使用像是[Modernizr](https://modernizr.com/)的 HTML5 shiv，您可以在任何地方包含 Azure 媒體播放機 JavaScript。 不過，請確定您的 Modernizr 版本包含適用于影片的 shiv。
...
> [!NOTE]
> 發行後 `1.2.0` ，就不再需要將位置包含在回溯 techs 中（它會自動從 azuremediaplayer.min.js 檔案的相對路徑中挑選位置）。 您可以在上述腳本後面的中新增下列腳本，以修改 fallback techs 的位置 `<head>` 。
...
1. `<data-setup>`上的屬性 `<video>` 會告知 Azure 媒體播放機在頁面就緒時自動設定影片，並從屬性讀取任何（以 JSON 格式）。
...
1. 上的*選擇性* `<data-setup>` 屬性 `<source>` 會告知 Azure 媒體播放機是否有任何來自 Azure 媒體服務的資料流程的唯一傳遞原則，包括但不限於加密類型（AES 或 PlayReady、Widevine 或 FairPlay）和權杖。
...
根據預設，大型 [播放] 按鈕位於左上角，因此不會涵蓋海報的有趣部分。 如果您想要將大型 [播放] 按鈕置中，可以將其他專案新增 `amp-big-play-centered` `class` 至您的 `<video>` 元素。
...
函式中的第一個引數 `amp` 是您影片標記的識別碼。 以您自己的取代。
