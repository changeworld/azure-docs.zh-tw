---
title: 適用于 HEVC 的 Smooth Streaming 通訊協定 (MS MS-SSTR) 修訂-Azure
...
本文針對使用 ' hev1 ' 或 ' hvc1 ' 格式追蹤在 Smooth Streaming 資訊清單中) 的 HEVC 影片編 (解碼器，提供技術實行需求，並更新標準化參考以參考目前的 MPEG 標準（包括 HEVC、一般加密 HEVC），以及 ISO Base Media 檔案格式的方塊名稱已更新為與最新規格一致。 
...
>  **撰寫時間：** 在用戶端顯示樣本的時間，如   [[ISO/IEC-iec-14496-12]](https://go.microsoft.com/fwlink/?LinkId=183695)中所定義。
> 
>   **CENC**：一般加密，如 [ISO/IEC 23001-7] 第二版所定義。
> 
>   解碼**時間：** 取樣需要在用戶端上解碼的時間，如[[ISO/IEC 14496-12:2008]](https://go.microsoft.com/fwlink/?LinkId=183695)中所定義。
...
> 
>   「不應該」、「必須」、「不得」、「不得」 **：** 這些詞彙 (在所有 caps) 中使用，如[[RFC2119]](https://go.microsoft.com/fwlink/?LinkId=90317)選用行為的所有聲明中所述，您可以使用、應該或不應該如此。
...
>  [MS-SSTR]Smooth Streaming 通訊協定 *>v20140502*[https://msdn.microsoft.com/library/ff469518.aspx](/openspecs/windows_protocols/ms-sstr/8383f27f-7efe-4c60-832a-387274457251)
> 
...
> 
>   [MPEG4-RA]進行中的註冊授權單位 "MP4REG"， [http://www.mp4ra.org](https://go.microsoft.com/fwlink/?LinkId=327787)
> 
>   RFC2119Bradner s.，S：「在 Rfc 中用來表示需求層級的主要單字」、BCP 14、RFC 2119、3月1997日   [https://www.rfc-editor.org/rfc/rfc2119.txt](https://go.microsoft.com/fwlink/?LinkId=90317)

### <a name="122-informative-references"></a>1.2.2 資訊參考 

>   [MS-GLOS] Microsoft Corporation，「*Windows 通訊協定主要詞彙*」。
> 
>   RFC3548Josefsson s.，S.，Ed.，"Base16，Base32，and Base64 Data 編碼"，RFC 3548，七月2003， [https://www.ietf.org/rfc/rfc3548.txt](https://go.microsoft.com/fwlink/?LinkId=90432)
> 
>   RFC5234Crocker、d.、Ed 和 Overell p.、P.、"增強的語法規格 BNF： ABNF"、STD 68、RFC 5234、1月2008   [https://www.rfc-editor.org/rfc/rfc5234.txt](https://go.microsoft.com/fwlink/?LinkId=123096)
...
>   實施者可以使用 MPEG4-RA 註冊擴充程式碼，以確保延伸模組不會衝突，如[[ISO/IEC-iec-14496-12]](https://go.microsoft.com/fwlink/?LinkId=183695)中所指定。
...
> 1. (無變更)
> 
>    建議的值為90000，代表影片畫面格的確切持續時間，以及包含小數幀數影片 (例如 30/1.001 Hz) 的片段。
...
>
> * "hvc1"：此播放軌的影片範例會使用 HEVC 影片，並使用 [ISO/IEC-iec-14496-15] 中指定的 ' hvc1 ' 範例描述格式。
> 
>   **CodecPrivateData (變數)：** 這項資料會指定媒體格式的特定參數和音軌中所有樣本通用的參數，以十六進位編碼位元組的字串表示。 位元組序列的格式和語意會隨著 **FourCC** 欄位的值而不同，如下所示：
> 
>   * 當 TrackElement 描述 HEVC video 時， **FourCC** 欄位應等於 **"hev1"** 或 **"hvc1"**
> 
>   **CodecPrivateData**欄位必須包含下列位元組序列的十六進位編碼字串標記法（在 ABNF [[RFC5234]](https://go.microsoft.com/fwlink/?LinkId=123096)中指定）： (沒有任何變更自 MS ms-sstr) 
> 
...
>   **注意**： **MinorVersion** 2 和 ' hev1 ' 或 ' hvc1 ' 要求的預設媒體格式是在 [iso/Iec 14496-12] Iso Base Media 檔案格式第四版和 [iso/Iec 23001-7] 一般加密第二版中指定的 ' Iso8 ' 品牌 ISO base media 檔案格式。
