...
1. 在 Visual Studio 中，建立新的 C# 主控台應用程式。 輸入**名稱**、**位置**和**解決方案名稱**，然後按一下"確定"。
2. 建置方案。
3. 使用 **NuGet** 來安裝和新增 **Azure 媒體服務 .NET SDK 延伸模組** (**windowsazure.mediaservices.extensions**)。 安裝這個封裝，也會安裝 **Media Services .NET SDK** ，並新增所有其他必要相依性。
    ...
    1. 在解決方案資源管理器中，按右鍵專案的名稱，然後選擇 **"管理 NuGet 包**"。
...
5. 加入 **System.Configuration** 組件的參考。 此程式集包含系統.配置。用於訪問設定檔（例如，App.config）的**組態管理員**類。
...
8. 在Program.cs檔開頭用以下代碼覆蓋現有**使用**語句：
