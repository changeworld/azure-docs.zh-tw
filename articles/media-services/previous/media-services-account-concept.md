...
媒體服務 v2 不支援多租使用者模型。 如果您需要將媒體服務帳戶移至新租使用者中的訂用帳戶，請在新的租使用者中建立新的 Azure Active Directory (Azure AD) 應用程式。 然後將您的帳戶移至新租使用者中的訂用帳戶。 當租使用者移動完成之後，您就可以開始使用新租使用者的 Azure AD 應用程式，以使用 v2 Api 存取媒體服務帳戶。

> [!IMPORTANT]
> 您必須重設 [Azure AD 的驗證](media-services-portal-get-started-with-aad.md) 資訊，才能存取媒體服務 v2 API。
