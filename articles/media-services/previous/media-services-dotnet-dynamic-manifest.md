...
description: 本主題說明如何建立篩選器，讓您的用戶端可以使用篩選器來串流特定的資料流區段。 媒體服務 .NET SDK 會建立動態資訊清單，以達成此選擇性資料流程。
...
* **IStreamingFilter**。  此類型是基於下列的 REST API [Filter](/rest/api/media/operations/filter)
* **IStreamingAssetFilter**。 此類型是基於下列的 REST API [AssetFilter](/rest/api/media/operations/assetfilter)
* **PresentationTimeRange**。 此類型是基於下列的 REST API [PresentationTimeRange](/rest/api/media/operations/presentationtimerange)
* **FilterTrackSelectStatement** 和 **IFilterTrackPropertyCondition**。 這些類型是基於下列的 REST API [FilterTrackSelect 和 FilterTrackPropertyCondition](/rest/api/media/operations/filtertrackselect)
