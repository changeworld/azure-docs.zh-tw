...
媒體服務支援多種方式來驗證提出金鑰要求的使用者。 內容金鑰授權原則可能有一個或多個授權限制，可能是使用 Open 或 Token 限制。 權杖限制原則必須伴隨 Security Token Service (STS) 所發出的權杖。 媒體服務支援簡單 web 權杖中的權杖 ([SWT](/previous-versions/azure/azure-services/gg185950(v=azure.100)#BKMK_2)) 和 JSON web 權杖 (JWT) 格式。
...
> 
> 當您在媒體服務中存取實體時，您必須在 HTTP 要求中設定特定的標頭欄位和值。 如需詳細資訊，請參閱 [REST API 開發媒體服務的設定](media-services-rest-how-to-use.md)。
> 
> 
> 
...
#### <a name="create-contentkeyauthorizationpolicies"></a><a id="ContentKeyAuthorizationPolicies"></a>建立 ContentKeyAuthorizationPolicies
要求：
...
#### <a name="create-contentkeyauthorizationpolicyoptions"></a><a id="ContentKeyAuthorizationPolicyOptions"></a>建立 ContentKeyAuthorizationPolicyOptions
要求：
...
#### <a name="add-an-authorization-policy-to-the-content-key"></a><a id="AddAuthorizationPolicyToKey"></a>將授權原則新增至內容金鑰
要求：
...
#### <a name="token-restriction-schema"></a><a id="schema"></a>Token 限制結構描述
```xml
...
當您使用 PlayReady 保護內容時，您需要在授權原則中指定的其中一件事，就是定義 [PlayReady 授權範本](media-services-playready-license-template-overview.md)的 XML 字串。 
...
#### <a name="create-contentkeyauthorizationpolicies"></a><a id="ContentKeyAuthorizationPolicies2"></a>建立 ContentKeyAuthorizationPolicies
要求：
