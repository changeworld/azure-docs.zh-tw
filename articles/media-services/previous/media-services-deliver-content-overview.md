...
有關詳細資訊,請參閱[動態打包](media-services-dynamic-packaging-overview.md)。
...
若要提供漸進式下載 URL 給使用者，您必須先建立 OnDemandOrigin 定位器。 建立定位器可針對包含您要串流之內容的資產，提供其基底路徑。 不過，若要能夠串流此內容，您還需要進一步修改此路徑。 若要建構串流資訊清單檔案的完整 URL，您必須串連定位器的路徑值和資訊清單 (filename.ism) 的檔案名稱。 然後將 **/清單**和適當的格式(如果需要)追加到定位器路徑。

> [!NOTE]
> 您還可以透過 TLS 連接流式傳輸內容。 若要這樣做，請確定您的串流 URL 以 HTTPS 開頭。 請注意,目前 AMS 不支援具有自定義域的 TLS。  
> 

僅當從 2014 年 9 月 10 日之後創建提供內容的流式處理終結點時,才能通過 TLS 流式傳輸。 如果您的串流 URL 是根據 2014 年 9 月 10 日之後建立的串流端點，則 URL 會包含 "streaming.mediaservices.windows.net"。 包含"origin.mediaservices.windows.net"(舊格式)的流式處理 URL 不支援 TLS。 如果您的網址 採用舊格式,並且希望能夠透過 TLS 串流式傳輸,請建立新的流式處理終結點。 使用基於新流式處理終結點的 URL 通過 TLS 流式傳輸內容。
...
http:\//testendpoint-testaccount.streaming.mediaservices.windows.net/fecebb23-46f6-490d-8b70-203e86b0df58/BigBuckBunny.ism/清單(格式=m3u8-aapl-v3,僅音訊=假)
...
### <a name="smooth-streaming-20-manifest-legacy-manifest"></a><a id="fmp4_v20"></a>Smooth Streaming 2.0 資訊清單 (舊版資訊清單)
