...
3. 在 [建立媒體服務帳戶] **** 中輸入必要的值。
   ...
   3. 在 [資源群組] **** 中，選取新的或現有資源。  資源群組是共用生命週期、權限及原則的資源集合。 [在此處](../../azure-resource-manager/management/overview.md#resource-groups)瞭解更多資訊。
   4. 在 **"位置**"中，選擇將用於存儲媒體服務帳戶的媒體和元資料記錄的地理區域。 此區域將用於處理和串流媒體。 只有可用的媒體服務區域才會出現在下拉式清單方塊中。 
