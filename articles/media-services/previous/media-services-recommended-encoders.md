...
ms.locfileid: "81535089"
---
# <a name="recommended-on-premises-encoders"></a>建議的內部部署編碼器

使用 Azure 媒體服務進行即時串流時，您可以指定想讓通道接收輸入資料流的方式。 如果選擇使用具有即時編碼通道的本地編碼器,則編碼器應將高品質的單比特率流推送為輸出。 如果選擇使用具有通道的本地編碼器,則編碼器應將多比特率流作為輸出,並具有所有所需的輸出品質。 有關詳細資訊,請參閱[使用本地端編碼器進行即時流式處理](media-services-live-streaming-with-onprem-encoders.md)。
...
6. 停止即時事件
7. 建立、啟動流式處理終結點、使用 Azure[媒體播放器等](https://aka.ms/azuremediaplayer)播放器監視存檔資產,以確保播放沒有所有品質等級的可見故障(或者,在步驟 6 之前的即時作業階段期間,透過預覽 URL 觀看和驗證)
...
6. 使用[Azure 媒體播放器](https://aka.ms/azuremediaplayer)等播放器不時觀看即時流式處理(或存檔資產),以確保播放沒有可見的故障
7. 停止即時事件
8. 記錄資產識別碼、即時封存的已發行串流 URL，以及來自您即時編碼器的所使用設定和版本
