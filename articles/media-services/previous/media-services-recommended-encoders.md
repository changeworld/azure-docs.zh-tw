...
6. 停止即時事件
7. 建立、啟動串流端點、使用播放程式（例如[Azure 媒體播放機](https://aka.ms/azuremediaplayer)）來觀看已封存的資產，以確保所有品質層級的播放沒有可見的問題（或者，在步驟6之前的即時會話期間透過預覽 URL 觀看和驗證）
...
6. 使用播放程式（例如[Azure 媒體播放機](https://aka.ms/azuremediaplayer)）即時監看即時串流（或已封存的資產），以確保播放沒有可見的問題
7. 停止即時事件
8. 記錄資產識別碼、即時封存的已發行串流 URL，以及來自您即時編碼器的所使用設定和版本
