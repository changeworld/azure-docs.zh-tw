...
>[!NOTE]
>創建 AMS 帳戶時，預設**流式**處理終結點將添加到處于 **"已停止"** 狀態的帳戶。 若要開始串流內容並利用動態封裝和動態加密功能，您想要串流內容的串流端點必須處於 [執行中]**** 狀態。 
>
...
以下**配置清除資產交付策略**方法指定不應用動態加密，並在以下任何協定中交付流：MPEG DASH、HLS 和平滑流式處理協定。 您可能想要將此原則套用到您的儲存體加密資產。
