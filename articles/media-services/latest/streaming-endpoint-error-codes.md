...
---
# <a name="streaming-endpoint-origin-errors"></a>串流式處理終結點(原點)錯誤 

本主題介紹可能從 Azure 媒體服務[流終結點服務](streaming-endpoint-concept.md)收到的錯誤。

## <a name="400-bad-request"></a>400 不正確的要求

要求包含無效資訊,並且由於以下原因之一而拒絕這些錯誤代碼:

|錯誤碼|十六進位值 |錯誤描述|
|---|---|---|
|MPE_BAD_URL_SYNTAX |0x80890201|URL 語法或格式錯誤。 範例包括無效類型、無效片段或無效軌道的請求。 |
...
|錯誤碼|十六進位值 |錯誤描述|
|---|---|---|
|MPE_STORAGE_AUTHENTICATION_FAILED |0x808900EA|存儲請求以完成請求失敗,但身份驗證失敗。 如果儲存金鑰已旋轉且服務無法同步儲存密鑰,則可能發生此情況。 <br/><br/>通過在 Azure 門戶中訪問[説明 + 支援](https://portal.azure.com/#blade/Microsoft_Azure_Support/HelpAndSupportBlade/newsupportrequest),請與 Azure 支援聯絡。|
...
|MPE_STORAGE_AUTHENTICATION_FAILURE |0x808900F3 |儲存操作錯誤,訪問失敗,由於泛型錯誤。 |
...
|MPE_EGRESS_TIMESTAMP_NOT_FOUND |0x8089020A |找不到請求的時間戳。 |
...
|MPE_MANIFEST_MEDIA_ENTRY_NOT_FOUND |0x80890256 |無法在清單中找到請求的比特率的媒體條目。 <br/><br/>可能是玩家要求某個比特率的視頻軌道不在清單中。|
|MPE_METADATA_NOT_FOUND |0 x80890257 |無法在清單中找到某些元數據,或無法從存儲中找到重資料庫。 |
|MPE_STORAGE_RESOURCE_NOT_FOUND |0x808900ED |儲存操作錯誤,找不到資源。 |
...
|錯誤碼|十六進位值 |錯誤描述|
|---|---|---|
|MPE_STORAGE_CONFLICT  |0x808900E  |儲存操作錯誤,衝突錯誤。  |

## <a name="410"></a>410

|錯誤碼|十六進位值 |錯誤描述|
|---|---|---|
|MPE_FILTER_FORCE_END_LEFT_EDGE_CROSSED_DVR_WINDOW|0x80890263|對於即時流式處理,當將強制 EndTimestamp 設置為 true 的篩選器時,開始時間戳或結束時間戳在當前 DVR 視窗之外。|
...
|MPE_STORAGE_PRECONDITION_FAILED| 0x808900EF|儲存操作錯誤,先決條件失敗。|

## <a name="415-unsupported-media-type"></a>415 不支援的媒體型態
...
|MPE_ENC_MULTIPLE_SAME_DELIVERY_TYPE|0x8088024E |原始設置可以由多種輸出格式分享。|
|MPE_FORMAT_NOT_SUPPORTED|0x80890205|不支援媒體格式或類型。 例如,媒體服務不支援超過 64 的品質級別計數。 在 FLV 視頻標籤中,媒體服務不支援具有多個 SPS 和多個 PPS 的影片幀。|
...
|MPE_INVALID_VIDEO_NAL_UNIT|0 x80890231|無效視訊 NAL 單元,例如,只有樣本中的第一個 NAL 可以是 AUD。|
|MPE_INVALID_NALU_SIZE|0 x80890260|無效的 NAL 單位大小。|
|MPE_INVALID_NALU_LENGTH_FIELD|0x80890261|無效的 NAL 單位長度值。|
|MPE_FILTER_INVALID|0x80890236|無效的動態清單篩選器。|
|MPE_FILTER_VERSION_INVALID|0 x80890237|無效或不支援的篩選器版本。|
|MPE_FILTER_TYPE_INVALID|0 x80890238|無效的篩選器類型。|
|MPE_FILTER_RANGE_ATTRIBUTE_INVALID|0 x80890239|無效範圍由篩選器指定。|
|MPE_FILTER_TRACK_ATTRIBUTE_INVALID|0x8089023A|無效軌道屬性由篩選器指定。|
|MPE_FILTER_PRESENTATION_WINDOW_INVALID|0x8089023B|無效的表示視窗長度由篩選器指定。|
|MPE_FILTER_LIVE_BACKOFF_INVALID|0x8089023C|無效的即時返回由篩選器指定。|
|MPE_FILTER_MULTIPLE_SAME_TYPE_FILTERS|0x8089023D|舊篩選器中僅支援一個 absTimeInHNS 元素。|
|MPE_FILTER_REMOVED_ALL_STREAMS|0x8089023E|應用篩選器后,根本沒有更多的流。|
...
|MPE_FILTER_COMPOSITION_MULTIPLE_FIRST_QUALITY_OPERATOR_NOT_ALLOWED|0x80890248|組合請求篩選器中不允許多個第一視頻質量運算符。|
|MPE_FILTER_FIRST_QUALITY_ATTRIBUTE_INVALID|0 x80890249|第一品質比特率屬性的數量必須為 1 (1)。|
...
|MPE_DTS_RESERVEDBOX_EXPECTED|0x80890105|特定於 DTS 的錯誤,在 DTS 框解析期間應出現在 DTSSpecficBox 中時,找不到保留框。|
...
|MPE_DTS_MULTIASSET_DTSH_MISMATCH|0x80890108|設置特定於 DTS 的錯誤,設置多資產,但 DTSH 樣本類型不匹配。|
|MPE_DTS_INVALID_CORESTREAM_SIZE|0x80890109|特定於 DTS 的錯誤,核心流大小無效。|
|MPE_DTS_INVALID_SAMPLE_RESOLUTION|0x8089010A|特定於 DTS 的錯誤,範例解析度無效。|
|MPE_DTS_INVALID_SUBSTREAM_INDEX|0x8089010B|特定於 DTS 的錯誤,子流擴展索引無效。|
|MPE_DTS_INVALID_BLOCK_NUM|0x8089010C|特定於 DTS 的錯誤,子流塊號無效。|
|MPE_DTS_INVALID_SAMPLING_FREQUENCE|0x8089010D|特定於 DTS 的錯誤,採樣頻率無效。|
|MPE_DTS_INVALID_REFCLOCKCODE|0x8089010E|特定於 DTS 的錯誤,子流擴展中的參考時鐘代碼無效。|
|MPE_DTS_INVALID_SPEAKERS_REMAP|0x8089010F|特定於 DTS 的錯誤,重新映射設置的揚聲器數量無效。|

有關加密文章和範例,請參閱:

- [概念:內容保護](content-protection-overview.md)
- [概念:內容關鍵原則](content-key-policy-concept.md)
- [概念:流式處理策略](streaming-policy-concept.md)
- [範例:使用 AES 加密進行保護](protect-with-aes128.md)
- [範例:使用 DRM 進行保護](protect-with-drm.md)

有關篩選器指南,請參閱:

- [概念:動態清單](filters-dynamic-manifest-overview.md)
- [概念:過濾器](filters-concept.md)
...
- [範例:即時流式處理教學](stream-live-tutorial-with-api.md)

## <a name="416-range-not-satisfiable"></a>416 範圍不適宜

|錯誤碼|十六進位值 |錯誤描述|
|---|---|---|
|MPE_STORAGE_INVALID_RANGE|0x808900F1|儲存操作錯誤,返回 HTTP 416 錯誤,範圍無效。|
...
|MPE_STORAGE_INTERNAL_ERROR|0x808900E6|儲存操作錯誤,一般內部錯誤之一的HTTP 500錯誤。|
|MPE_STORAGE_OPERATION_TIMED_OUT|0x808900E7|儲存操作錯誤,常規操作超時 HTTP 500 錯誤之一。|
|MPE_STORAGE_FAILURE|0x808900F2|儲存操作錯誤,其他 HTTP 500 錯誤,而不是內部錯誤或操作超時。|
...
|錯誤碼|十六進位值 |錯誤描述|
|---|---|---|
|MPE_STORAGE_SERVER_BUSY|0x808900E8|儲存操作錯誤,收到 HTTP 伺服器忙錯誤 503。|
...
[範例:使用 .NET 存取錯誤代碼和來自 ApiException 的消息](configure-connect-dotnet-howto.md#connect-to-the-net-client)
