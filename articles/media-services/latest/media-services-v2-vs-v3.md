...
* 在媒體服務 v2 中，可以指定初始化向量（IV）。 在媒體服務 v3 中，無法指定 FairPlay IV。 雖然它不會影響使用媒體服務進行封裝和授權傳遞的客戶，但使用協力廠商 DRM 系統來傳遞 FairPlay 授權（混合模式）時可能會有問題。 在此情況下，請務必知道 FairPlay IV 衍生自 cbcs 金鑰識別碼，而且可以使用下列公式來抓取：
    ...
    如需詳細資訊，請參閱[適用于即時和 VOD 作業的混合式模式中適用于媒體服務 v3 的 Azure Functions c # 程式碼](https://github.com/Azure-Samples/media-services-v3-dotnet-core-functions-integration/tree/master/LiveAndVodDRMOperationsV3)。
...
|`assetId`|`Id`-（唯一）值的`nb:cid:UUID:`開頭為前置詞。|
|`created`|`Created`|
|`description`|`Name`|
