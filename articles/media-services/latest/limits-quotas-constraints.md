...
description: 本主題介紹Microsoft Azure媒體服務v3中的配額和限制。
...
<sup>5</sup>使用自定義[流式處理策略](https://docs.microsoft.com/rest/api/media/streamingpolicies)時,應為媒體服務帳戶設計一組有限的此類策略,並在需要相同的加密選項和協定時將其重新用於流式處理器。 不建議您對每個串流定位器建立新的串流原則。

<sup>6</sup>流式處理器不是用於管理每個用戶的訪問控制。 若要給予個別使用者不同的存取權限，請使用數位版權管理 (DRM) 方案。
