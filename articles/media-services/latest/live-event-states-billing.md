...
當[即時事件](/rest/api/media/liveevents)上的**LiveEventEncodingType**設定為 [標準] 或 [Premium1080p] 時，媒體服務自動關閉在輸入摘要遺失後處於 [執行中 **] 狀態12小時的任何**即時事件，而且沒有執行中的**即時輸出**。 不過，您仍將需要支付「實況活動」處於 [正在執行]**** 狀態時間的費用。
...
|State|描述|
|---|---|
|**已經**| 這是建立後即時事件的初始狀態（除非自動啟動已設定為 true）。在此狀態下不會計費。 在此狀態下，可以更新「實況活動」屬性，但是不允許進行串流。|
