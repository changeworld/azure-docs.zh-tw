...
## <a name="map-a-custom-domain-with-only-http-enabled"></a>對應只啟用 HTTP 的自訂網域

這種方法比較簡單，但只會啟用 HTTP 存取。 如果儲存體帳戶設定為要求透過 HTTPS 進行 [安全傳輸](../common/storage-require-secure-transfer.md) ，您必須為您的自訂網域啟用 HTTPS 存取。 
...
> [!IMPORTANT]
> 當您完成設定時，使用者的自訂網域將會暫時無法使用。 如果您的網域目前支援的應用程式具有服務等級協定 (SLA) 需要零停機時間，請遵循本文中的「將 [自訂網域與零停機的自訂網域對應](#zero-down-time) 」一節中的步驟，以確保使用者可以在 DNS 對應發生時存取您的網域。

如果您要不在乎該網域在使用者短暫無法使用的情況下，請遵循下列步驟。

： heavy_check_mark：步驟1：取得儲存體端點的主機名稱。

： heavy_check_mark：步驟2：使用您的網域提供者 (CNAME) 記錄建立正式名稱。
...
#### <a name="step-2-create-a-canonical-name-cname-record-with-your-domain-provider"></a>步驟2：使用您的網域提供者 (CNAME) 記錄建立正式名稱
...
3. 建立 CNAME 記錄。 作為該記錄的一部分，請提供下列專案： 
   ...
   - 子域別名，例如 `www` 或 `photos` 。 子域為必要項，不支援根域。 
...
4. 若要註冊自訂網域，請選擇 [ **儲存** ] 按鈕。
   ...
   在 CNAME 記錄透過功能變數名稱伺服器傳播 (DNS) 之後，如果您的使用者具有適當的許可權，他們就可以使用自訂網域來查看 blob 資料。
...
### <a name="map-a-custom-domain-with-zero-downtime"></a>將自訂網域對應至零停機時間

> [!NOTE]
> 如果您要不在乎該網域在使用者短暫無法使用的情況下，請考慮依照本文的「 [對應自訂網域](#map-a-domain) 」一節中的步驟進行。 更簡單的方法是使用較少的步驟。  

如果您的網域目前支援的應用程式具有服務等級協定 (SLA) 需要零停機時間，請遵循下列步驟，以確保使用者可以在 DNS 對應發生時存取您的網域。 

： heavy_check_mark：步驟1：取得儲存體端點的主機名稱。

： heavy_check_mark：步驟2：使用您的網域提供者 (CNAME) 記錄建立中繼正式名稱。

： heavy_check_mark：步驟3：向 Azure 預先註冊自訂網域。
...
#### <a name="step-2-create-a-intermediary-canonical-name-cname-record-with-your-domain-provider"></a>步驟2：使用您的網域提供者 (CNAME) 記錄建立中繼標準名稱
...
#### <a name="step-3-pre-register-your-custom-domain-with-azure"></a>步驟3：向 Azure 預先註冊自訂網域

當您向 Azure 預先註冊自訂網域時，您會允許 Azure 辨識您的自訂網域，而不需要修改網域的 DNS 記錄。 如此一來，當您修改網域的 DNS 記錄時，它會對應至 blob 端點，而不會有停機時間。
...
這種方法需要更多步驟，但它會啟用 HTTPS 存取。 

如果您不需要使用者透過 HTTPS 存取您的 blob 或 web 內容，請參閱本文的「 [僅針對已啟用 HTTP 的自訂網域對應](#enable-http) 」一節。 

若要對應自訂網域並啟用 HTTPS 存取，請執行下列動作：

1. 在您的 blob 或 web 端點上啟用 [AZURE CDN](../../cdn/cdn-overview.md) 。 

   針對 Blob 儲存體端點，請參閱 [整合 azure 儲存體帳戶與 AZURE CDN](../../cdn/cdn-create-a-storage-account-with-cdn.md)。 

   針對靜態網站端點，請參閱 [將靜態網站與 AZURE CDN 整合](static-website-content-delivery-network.md)。
