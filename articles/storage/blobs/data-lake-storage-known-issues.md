...
## <a name="windows-azure-storage-blob-wasb-driver"></a>Windows Azure 儲存體 Blob （WASB）驅動程式

目前，與使用 WASB 驅動程式相關聯的幾個問題，以及具有階層式命名空間的帳戶。 我們建議您在工作負載中使用[Azure Blob 檔案系統（ABFS）](data-lake-storage-abfs-driver.md)驅動程式。 
