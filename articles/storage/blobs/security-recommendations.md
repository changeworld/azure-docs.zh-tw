...
ms.locfileid: "80061468"
---
# <a name="security-recommendations-for-blob-storage"></a>Blob 存儲的安全建議

本文包含 Blob 存儲的安全建議。 實施這些建議將説明您履行我們共同責任模型中所述的安全義務。 有關 Microsoft 如何履行服務提供者職責的詳細資訊，請閱讀[雲計算的共用責任](https://gallery.technet.microsoft.com/Shared-Responsibilities-81d0ff91/file/225366/1/Shared%20Responsibility%20for%20Cloud%20Computing-2019-10-25.pdf)。
...
| 打開 Blob 資料的虛刪除 | 虛刪除使您能夠在 Blob 資料被刪除後恢復資料。 有關虛刪除的詳細資訊，請參閱[Azure 存儲 blob 的虛刪除](storage-blob-soft-delete.md)。 | - |
| 將業務關鍵型資料存儲在不可變的 blob 中 | 配置法定保留和基於時間的保留原則，以將 Blob 資料存儲在 WORM（一次寫入、讀取多個）狀態。 可以不可變地存儲的 Blob，但在保留間隔期間無法修改或刪除。 有關詳細資訊，請參閱[使用不可變存儲存儲業務關鍵型 Blob 資料](storage-blob-immutable-storage.md)。 | - |
| 僅將共用訪問簽名 （SAS） 權杖限制為 HTTPS 連接 | 當用戶端使用 SAS 權杖訪問 Blob 資料時，要求 HTTPS 有助於將竊聽風險降至最低。 有關詳細資訊，請參閱[使用共用訪問簽名 （SAS） 授予對 Azure 存儲資源的有限存取權限](../common/storage-sas-overview.md)。 | - |

## <a name="identity-and-access-management"></a>身分識別和存取管理

| 建議 | 註解 | 資訊安全中心 |
|-|----|--|
| 使用 Azure 活動目錄 （Azure AD） 授權訪問 Blob 資料 | Azure AD 比共用金鑰提供卓越的安全性和易用性，用於授權對 Blob 存儲的請求。 有關詳細資訊，請參閱使用[Azure 活動目錄 授權訪問 Azure blob 和佇列](../common/storage-auth-aad.md)。 | - |
| 通過 RBAC 將許可權分配給 Azure AD 安全主體時，請記住最低特權主體 | 將角色指派給使用者、組或應用程式時，僅授予該安全主體執行其任務所需的許可權。 限制對資源的訪問有助於防止意外和惡意濫用您的資料。 | - |
| 使用使用者委派 SAS 向用戶端授予對 Blob 資料的有限存取權限 | 使用者委派 SAS 使用 Azure 活動目錄 （Azure AD） 憑據以及為 SAS 指定的許可權進行保護。 使用者委派 SAS 在範圍和功能方面類似于服務 SAS，但比服務 SAS 具有安全優勢。 有關詳細資訊，請參閱[使用共用訪問簽名 （SAS） 授予對 Azure 存儲資源的有限存取權限](../common/storage-sas-overview.md?toc=%2fazure%2fstorage%2fblobs%2ftoc.json)。 | - |
| 使用 Azure 金鑰保存庫保護帳戶訪問金鑰 | Microsoft 建議使用 Azure AD 授權對 Azure 存儲的請求。 但是，如果必須使用共用金鑰授權，請使用 Azure 金鑰保存庫保護帳戶金鑰。 您可以在運行時從金鑰保存庫檢索金鑰，而不是將金鑰與應用程式一起保存。 有關 Azure 金鑰保存庫的詳細資訊，請參閱[Azure 金鑰保存庫概述](../../key-vault/key-vault-overview.md)。 | - |
...
| 限制對容器和 blob 的匿名公共讀取存取 | 對容器及其 blob 的匿名公共讀取存取將這些資源的唯讀存取權限授予任何用戶端。 除非方案需要，否則請避免啟用公共讀取存取。 | - |
