...
Azure RBAC 會使用角色指派，有效地將許可權集套用至 *安全性主體*。 *安全性主體*是一個物件，代表要求存取 Azure 資源的 AZURE ACTIVE DIRECTORY (AD) 中所定義的使用者、群組、服務主體或受控識別。

一般而言，這些 Azure 資源會限制為最上層資源 (例如： Azure 儲存體帳戶) 。 在 Azure 儲存體的情況下，因此 Azure Data Lake Storage Gen2，此機制已擴充至容器 (檔案系統) 資源。

若要瞭解如何在您的儲存體帳戶範圍中將角色指派給安全性主體，請參閱 [使用 Azure 入口網站指派 Azure 角色以存取 blob 和佇列資料](https://docs.microsoft.com/azure/storage/common/storage-auth-aad-rbac-portal?toc=%2fazure%2fstorage%2fblobs%2ftoc.json)。
...
雖然使用 Azure 角色指派是控制存取權限的強大機制，但它是相對於 Acl 的極粗糙細微性機制。 Azure RBAC 的最小細微性是在容器層級，而這會以高於 Acl 的優先順序進行評估。 因此，如果您將角色指派給容器範圍中的安全性主體，則不論 ACL 指派為何，該安全性主體都會具有與該容器中所有目錄和檔案相關聯的授權層級。

當安全性主體透過 [內建角色](https://docs.microsoft.com/azure/storage/common/storage-auth-aad?toc=%2fazure%2fstorage%2fblobs%2ftoc.json#built-in-rbac-roles-for-blobs-and-queues)或自訂角色授與 Azure RBAC 資料許可權時，系統會先在要求授權時評估這些許可權。 如果要求的作業是由安全性主體的 Azure 角色指派所授權，則會立即解決授權，而不會執行其他 ACL 檢查。 或者，如果安全性主體沒有 Azure 角色指派，或要求的作業不符合指派的許可權，則會執行 ACL 檢查，以判斷安全性主體是否已獲授權執行要求的操作。
