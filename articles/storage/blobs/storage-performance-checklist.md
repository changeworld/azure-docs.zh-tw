---
title: Blob 存儲的性能和可擴充性檢查表 - Azure 存儲
...
Azure 儲存體具有容量、交易速率和頻寬的延展性和效能目標。 有關 Azure 存儲可伸縮性目標的詳細資訊，請參閱[標準存儲帳戶的可伸縮性和性能目標](../common/scalability-targets-standard-account.md?toc=%2fazure%2fstorage%2fblobs%2ftoc.json)以及[Blob 存儲的可伸縮性和性能目標](scalability-targets.md)。
...
| &nbsp; |Caching |[您的應用程式緩存的資料是否經常訪問且很少更改？](#reading-data) |
...
| &nbsp; |複製 Blob |[您是否使用最新版本的 AzCopy 進行批量複製操作？](#use-azcopy) |
| &nbsp; |複製 Blob |[您是否使用 Azure 資料框系列導入大量資料？](#use-azure-data-box) |
...
如果您接近特定訂閱/區域組合允許的最大存儲帳戶數，請評估您的方案並確定是否適用以下任何條件：

- 您是否使用存儲帳戶來存儲非託管磁片並將這些磁片添加到虛擬機器 （VM）？ 對於此方案，Microsoft 建議使用託管磁片。 託管磁片可自動為您擴展，無需創建和管理單個存儲帳戶。 有關詳細資訊，請參閱[Azure 託管磁片簡介](../../virtual-machines/windows/managed-disks-overview.md)
- 您是否使用每個客戶的一個存儲帳戶進行資料隔離？ 對於此方案，Microsoft 建議為每個客戶使用 Blob 容器，而不是整個存儲帳戶。 Azure 存儲現在允許您基於每個容器分配基於角色的存取控制 （RBAC） 角色。 如需詳細資訊，請參閱[在 Azure 入口網站中使用 RBAC 授與 Azure Blob 和佇列資料的存取權](../common/storage-auth-aad-rbac-portal.md) \(部分機器翻譯\)。
- 您是否使用多個存儲帳戶分片來增加每秒 （IOPS） 或容量的入口、出口、I/O 操作（ IOPS）？ 在此案例中，可能的話，我們建議您善用提高儲存體帳戶的限制，以降低您的工作負載所需的儲存體帳戶數目。 請連絡 [Azure 支援](https://azure.microsoft.com/support/options/)來要求提高儲存體帳戶的限制。 如需詳細資訊，請參閱[宣佈較大型、較大規模的儲存體帳戶](https://azure.microsoft.com/blog/announcing-larger-higher-scale-storage-accounts/)。
...
### <a name="multiple-clients-accessing-a-single-blob-concurrently"></a>多個用戶端同時訪問單個 Blob

如果同時訪問單個 Blob 的用戶端數量眾多，則需要同時考慮每個 Blob 和每個存儲帳戶的可伸縮性目標。 可以訪問單個 Blob 的確切用戶端數將因各種因素而異，例如同時請求 blob 的用戶端數、blob 的大小和網路條件。

如果 Blob 可以通過 CDN（如從網站提供的圖像或視頻）進行分發，則可以使用 CDN。 有關詳細資訊，請參閱標題為["內容分發"的部分](#content-distribution)。

在其他方案中，例如資料保密的科學類比，您有兩個選項。 第一種是錯開工作負荷的訪問，以便在一段時間內訪問 Blob，而不是同時訪問。 或者，您可以臨時將 Blob 複製到多個存儲帳戶，以增加每個 Blob 和跨存儲帳戶的總 IOPS。 結果因應用程式的行為而異，因此請確保在設計期間測試併發模式。

### <a name="bandwidth-and-operations-per-blob"></a>每個 Blob 的頻寬和操作

單個 Blob 每秒最多支援 500 個請求。 如果有多個用戶端需要讀取同一 Blob，並且可能超過此限制，請考慮使用塊 Blob 存儲帳戶。 塊 Blob 存儲帳戶提供更高的請求率，或每秒 I/O 操作 （IOPS）。

您還可以使用內容傳遞網路 （CDN）（如 Azure CDN）在 Blob 上分發操作。 有關 Azure CDN 的詳細資訊，請參閱[Azure CDN 概述](../../cdn/cdn-overview.md)。  

## <a name="partitioning"></a>資料分割

瞭解 Azure 存儲如何分區 Blob 資料對於提高性能非常有用。 Azure 存儲可以比跨多個分區的資料更快地在單個分區中提供資料。 通過適當地命名 blob，可以提高讀取請求的效率。

Blob 存儲使用基於範圍的分區方案進行縮放和負載平衡。 每個 Blob 都有一個由完整 Blob 名稱（帳戶_容器_blob）組成的分區鍵。 分區鍵用於將 Blob 資料分區到範圍中。 然後，跨 Blob 存儲對範圍進行負載平衡。

基於範圍的分區意味著使用詞法排序（例如 *，myPayroll、**我的性能*、*我的員工*等）或時間戳記 *（log20160101、log20160102、log20160102*等）的命名約定更有可能導致分區在同一分區伺服器上同時位於同一分區伺服器上。 *log20160102* *log20160102* ，直到增加負載需要它們拆分為更小的範圍。 在同一分區伺服器上共同定位 Blob 可增強性能，因此性能增強的一個重要部分涉及以最有效地組織 blob 的方式命名 blob。

例如，容器內的所有 Bob 接受單一伺服器的服務，直到這些 Blob 的負載需要進一步重新平衡分割區範圍。 同樣，一組按詞法順序排列其名稱的輕載入帳戶可能由單個伺服器提供服務，直到其中一個或所有這些帳戶上的負載要求它們跨多個分區伺服器拆分。

每個負載平衡作業都可能在作業期間影響儲存體呼叫的延遲。 服務處理分區突然突發流量的能力受到單個分區伺服器的可伸縮性的限制，直到負載平衡操作啟動並重新平衡分區金鑰範圍。
...
- 檢查用於帳戶、容器、blob、表和佇列的命名約定。 請考慮使用最適合您需求的雜湊函數，使用三位雜湊來對帳戶、容器或 blob 名稱進行首碼。
- 如果使用時間戳或數位識別碼組織資料，請確保不使用僅追加（或僅預尾）流量模式。 這些模式不適用於基於範圍的分區系統。 這些模式可能導致所有流量進入單個分區，並限制系統進行有效的負載平衡。

    例如，如果具有使用具有時間戳記（如*yyymmd）* 的 Blob 的每日操作，則該日常操作的所有流量將定向到單個 Blob，該 Blob 由單個分區伺服器提供。 考慮每個 Blob 限制和每個分區限制是否滿足您的需要，並考慮根據需要將此操作分解為多個 Blob。 同樣，如果在表中存儲時間序列資料，則所有流量可能都定向到金鑰命名空間的最後一部分。 如果使用數位 ID，則使用三位雜湊來首碼 ID。 如果使用時間戳，則用秒值（例如*syyyymmd）* 首碼時間戳記。 如果應用程式例行執行清單和查詢操作，請選擇一個雜湊函數來限制查詢數。 在某些情況下，隨機首碼可能就足夠了。
  
- 有關 Azure 存儲中使用的分區方案的詳細資訊，請參閱 Azure[存儲：具有強一致性的高度可用雲存儲服務](https://sigops.org/sosp/sosp11/current/2011-Cascais/printable/11-calder.pdf)。
...
通常，讀取資料一次比讀取資料兩次更可取。 請考慮從 Azure 存儲中檢索到 50 MiB Blob 以用作使用者內容的 Web 應用程式的示例。 理想情況下，應用程式將 Blob 本機快取到磁片，然後檢索後續使用者請求的緩存版本。
...
配置資料、查找資料以及應用程式經常使用的其他資料是緩存的良好候選資料。  

有關使用條件標頭的詳細資訊，請參閱[為 Blob 服務操作指定條件標頭](/rest/api/storageservices/specifying-conditional-headers-for-blob-service-operations)。  

### <a name="uploading-data-in-batches"></a>批量上傳資料

在某些情況下，您可以在本地聚合資料，然後定期將資料上載到批次處理中，而不是立即上載每一段資料。 例如，假設 Web 應用程式保留活動的日誌檔。 應用程式可以上載每個活動的詳細資訊，因為它發生在一個表（這需要許多存儲操作），或者它可以保存活動詳細資訊到本地日誌檔，然後定期將所有活動詳細資訊作為分隔檔上載到 Blob。 如果每個日誌條目的大小為 1 KB，則可以在單個事務中上載數千個條目。 單個事務支援上載大小高達 64 MiB 的 Blob。 應用程式開發人員必須針對用戶端設備或上載失敗的可能性進行設計。 如果需要在一段時間內下載活動資料，而不是單個活動，則建議在表存儲中使用 Blob 存儲。
...
有關詳細資訊，請參閱博客文章[Web 服務：併發連接](https://blogs.msdn.microsoft.com/darrenj/2005/03/07/web-services-concurrent-connections/)。  
...
Azure 存儲提供了許多解決方案，用於複製和移動存儲帳戶中的 Blob、存儲帳戶之間以及本地系統和雲之間。 本節從其中一些選項對性能的影響的角度介紹其中一些選項。 有關將資料有效地傳輸到 Blob 存儲或從 Blob 存儲傳輸的資料的資訊，請參閱[選擇用於資料傳輸的 Azure 解決方案](../common/storage-choose-data-transfer-solution.md?toc=%2fazure%2fstorage%2fblobs%2ftoc.json)。

### <a name="blob-copy-apis"></a>Blob 複製 API

要跨存儲帳戶複製 Blob，請使用["從 URL 放置塊](/rest/api/storageservices/put-block-from-url)"操作。 此操作將資料從任何 URL 源同步複製到塊 Blob 中。 跨存儲`Put Block from URL`帳戶遷移資料時，使用該操作可以顯著減少所需的頻寬。 由於複製操作發生在服務端，因此無需下載和重新上傳資料。

要複製同一存儲帳戶中的資料，請使用[複製 Blob](/rest/api/storageservices/Copy-Blob)操作。 複製同一存儲帳戶中的資料通常快速完成。  

### <a name="use-azcopy"></a>使用 AzCopy

AzCopy 命令列實用程式是一種簡單而高效的選項，用於批量將 blob 傳輸到存儲帳戶、從存儲帳戶中和跨存儲帳戶。 AzCopy 針對此方案進行了優化，並可實現高傳輸率。 AzCopy 版本 10`Put Block From URL`使用該操作跨存儲帳戶複製 Blob 資料。 有關詳細資訊，請參閱使用[AzCopy v10 將資料複製或移動到 Azure 存儲](/azure/storage/common/storage-use-azcopy-v10)。  

### <a name="use-azure-data-box"></a>使用 Azure 資料框

要將大量資料導入 Blob 存儲，請考慮使用 Azure 資料框系列進行離線傳輸。 當您受到時間、網路可用性或成本的限制時，Microsoft 提供的資料盒設備是將資料移動到 Azure 的一個不錯的選擇。 有關詳細資訊，請參閱 Azure[資料盒文檔](/azure/databox/)。
...
Blob 服務支援 HEAD 請求，其中可以包括 blob 屬性或中繼資料。 例如，如果應用程式需要照片中的 Exif（可 ex 可掃描圖像格式）資料，則可以檢索照片並提取照片。 為了節省頻寬並提高性能，應用程式可以在應用程式上傳照片時將 Exif 資料存儲在 Blob 的中繼資料中。 然後，您只能使用 HEAD 請求檢索中繼資料中的 Exif 資料。 僅檢索中繼資料而不是 Blob 的全部內容可節省大量頻寬，並減少提取 Exif 資料所需的處理時間。 請記住，每個 Blob 可以存儲 8 個 B 元元。  
...
要快速上載單個大型 Blob，用戶端應用程式可以並行上載其塊或頁面，同時注意單個 Blob 和整個存儲帳戶的可伸縮性目標。 Azure 存儲用戶端庫支援並行上載。 例如，可以使用以下屬性指定 .NET 或 JAVA 中允許的併發請求數。 其他受支援語言的用戶端庫提供了類似的選項。

- 對於 .NET，設置[Blob 請求選項.平行作業執行緒計數](/dotnet/api/microsoft.azure.storage.blob.blobrequestoptions.paralleloperationthreadcount)屬性。
- 對於 JAVA/Android，調用[BlobRequestOptions.set併發請求計數（最終整數併發請求計數）](/java/api/com.microsoft.azure.storage.blob._blob_request_options.setconcurrentrequestcount)方法。

### <a name="upload-many-blobs-quickly"></a>快速上傳多個 Blob

若要快速上傳多個 Blob，請平行上傳 Blob。 並行上傳比一次上傳單個 Blob 時使用並行塊上載更快，因為它將上載跨存儲服務的多個分區展開。 AzCopy 依預設會執行平行上傳，在此案例中我們建議使用此方法。 有關詳細資訊，請參閱從[AzCopy 開始](../common/storage-use-azcopy-v10.md)。  

## <a name="choose-the-correct-type-of-blob"></a>選擇正確的 blob 類型

Azure 存儲支援塊 Blob、追加 Blob 和頁面 Blob。 在指定使用的案例中，您的 Blob 類型選擇將會影響解決方案的效能和延展性。

當您要高效地上載大量資料時，阻止 Blob 是合適的。 例如，將照片或視頻上載到 Blob 存儲的用戶端應用程式將針對塊 Blob。

追加 Blob 類似于塊 blob，因為它們由塊組成。 修改追加 Blob 時，塊僅添加到 blob 的末尾。 當應用程式需要將資料添加到現有 Blob 時，附加 blob 可用於日誌記錄等方案。

如果應用程式需要對資料執行隨機寫入，則頁面 blob 是合適的。 例如，Azure 虛擬機器磁片存儲為頁面 blob。 有關詳細資訊，請參閱[瞭解塊 blob、追加 blob 和頁面 blob。](/rest/api/storageservices/understanding-block-blobs--append-blobs--and-page-blobs)  
