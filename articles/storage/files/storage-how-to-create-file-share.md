...
- **您的 Azure 檔案共用有哪些效能需求？**  
    Azure 檔案儲存體提供標準檔案共用，這些共用是裝載在以硬碟為基礎 (HDD 型) 硬體上，而高階檔案共用則是裝載在固態磁片型 (SSD 型) 硬體上。
...
Azure 檔案共用會部署至 *儲存體帳戶*，這是代表儲存體共用集區的最上層物件。 此儲存體集區可以用來部署多個檔案共用。 

Azure 支援多個儲存體帳戶類型，適用于客戶可能擁有的不同儲存體案例，但 Azure 檔案儲存體有兩種主要的儲存體帳戶類型。 您需要建立的儲存體帳戶類型，取決於您要建立標準檔案共用或 premium 檔案共用： 

- **一般用途第2版 (GPv2) 儲存體帳戶**： GPv2 儲存體帳戶可讓您將 Azure 檔案共用部署在以標準/硬碟為基礎的 (HDD 型) 硬體上。 除了儲存 Azure 檔案共用之外，GPv2 儲存體帳戶還可以儲存其他儲存體資源，例如 blob 容器、佇列或資料表。 

- **FileStorage 儲存體帳戶**： FileStorage 儲存體帳戶可讓您在高階/固態磁片型 (SSD 型) 硬體上部署 Azure 檔案共用。 FileStorage 帳戶只能用來儲存 Azure 檔案共用;沒有任何其他儲存體資源 (blob 容器、佇列、資料表等 ) 可以部署在 FileStorage 帳戶中。
...
一旦您建立儲存體帳戶之後，剩下的就是建立您的檔案共用。 無論您使用的是 premium 檔案共用或標準檔案共用，此程式的流程大多相同。 主要差異在於 **配額** 及其代表的內容。
...
# <a name="azure-cli"></a>[Azure CLI](#tab/azure-cli)
在可以使用 Azure CLI 建立 Azure 檔案共用之前，您必須先取得儲存體帳戶金鑰，以授權檔案共用的建立作業。 您可以使用下列命令來完成此動作 [`az storage account keys list`](/cli/azure/storage/account/keys) ：
...
取得儲存體帳戶金鑰後，您就可以使用命令來建立 Azure 檔案共用 [`az storage share create`](/cli/azure/storage/share) 。 
...
如果儲存體帳戶包含在虛擬網路中，且您從中叫用此命令的電腦不是虛擬網路的一部分，此命令將會失敗。 您可以使用上述的 Azure PowerShell `New-AzRmStorageShare` Cmdlet，或從屬於虛擬網路一部分的電腦執行 Azure CLI （包括透過 VPN 連線），以解決此時間點的限制。
