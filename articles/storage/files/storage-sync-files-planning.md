...
ms.locfileid: "82176085"
---
# <a name="planning-for-an-azure-file-sync-deployment"></a>規劃 Azure 檔案同步部署

:::row:::
    :::column:::
        [![Azure 檔案同步按一下即可玩的訪談和示範！](./media/storage-sync-files-planning/azure-file-sync-interview-video-snapshot.png)](https://www.youtube.com/watch?v=nfWLO7F52-s)
    :::column-end:::
    :::column:::
        Azure 檔案同步是一項服務，可讓您在內部部署 Windows Server 或雲端 VM 上快取一些 Azure 檔案共用。 
        
        本文將介紹如何 Azure 檔案同步概念和功能。 當您熟悉 Azure 檔案同步之後，請考慮遵循[Azure 檔案同步部署指南](storage-sync-files-deployment-guide.md)來試用這項服務。        
    :::column-end:::
:::row-end:::

檔案將會儲存在[Azure 檔案共用](storage-files-introduction.md)中的雲端。 Azure 檔案共用的使用方式有兩種：直接裝載這些無伺服器 Azure 檔案共用（SMB），或使用 Azure 檔案同步在內部部署快取 Azure 檔案共用。您所選擇的部署選項會變更規劃部署時所需考慮的層面。 
...
若要在 Windows Server 上啟用同步處理功能，您必須安裝 Azure 檔案同步可下載的代理程式。 Azure 檔案同步代理程式提供兩個主要元件`FileSyncSvc.exe`：，這是負責監視伺服器端點上之變更並起始同步會話的背景 Windows 服務，以及`StorageSync.sys`可啟用雲端階層處理和快速嚴重損壞修復的檔案系統篩選器。  
...
\*目前不建議同步處理超過100000000個檔案 & 目錄。 這是以我們測試的臨界值為基礎的軟限制。 如需詳細資訊，請參閱[Azure 檔案儲存體擴充性和效能目標](storage-files-scale-targets.md#azure-file-sync-scale-targets)。
...
Azure 檔案同步與您的標準 AD 型身分識別搭配使用，而不需要任何特殊設定即可設定同步。當您使用 Azure 檔案同步時，一般會預期大部分的存取都會通過 Azure 檔案同步快取伺服器，而不是透過 Azure 檔案共用。 由於伺服器端點位於 Windows Server 上，而 Windows Server 已有很長一段時間支援 AD 和 Windows 樣式的 Acl，因此除了確保向儲存體同步服務註冊的 Windows 檔案伺服器已加入網域之外，不需要任何東西。 Azure 檔案同步會將 Acl 儲存在 Azure 檔案共用中的檔案，並將其複寫至所有伺服器端點。
...
與 BitLocker 類似的協力廠商產品，在其位於 NTFS 磁片區的下方，應該也能以完全透明的方式與 Azure 檔案同步搭配使用。 
...
Azure 檔案同步無法與位於檔案系統的 NTFS 加密檔案系統（NTFS EFS）或協力廠商加密解決方案相交互操作，但位於檔案的資料流程底下。 

### <a name="encryption-in-transit"></a>傳輸中加密

> [!NOTE]
> Azure 檔案同步服務會在2020年8月移除 TLS 1.0 和1.1 的支援。 根據預設，所有支援的 Azure 檔案同步代理程式版本都已使用 TLS 1.2。 如果您的伺服器上已停用 TLS 1.2 或使用 proxy，可能會發生使用舊版的 TLS。 如果您使用 proxy，建議您檢查 proxy 設定。 5/1/2020 之後新增的 Azure 檔案同步服務區域僅支援 TLS 1.2，而 TLS 1.0 和1.1 的支援將會從8月2020的現有區域中移除。  如需詳細資訊，請參閱[疑難排解指南](storage-sync-files-troubleshoot.md#tls-12-required-for-azure-file-sync)。
...
|-------------|-------------------|--------------|-------------|
| Public | Asia | 東亞 | `eastasia` |
| Public | Asia | 東南亞 | `southeastasia` |
| Public | 澳大利亞 | 澳大利亞東部 | `australiaeast` |
| Public | 澳大利亞 | 澳大利亞東南部 | `australiasoutheast` |
| Public | 巴西 | 巴西南部 | `brazilsouth` |
| Public | Canada | 加拿大中部 | `canadacentral` |
| Public | Canada | 加拿大東部 | `canadaeast` |
| Public | 歐洲 | 北歐 | `northeurope` |
| Public | 歐洲 | 西歐 | `westeurope` |
| Public | 法國 | 法國中部 | `francecentral` |
| Public | 法國 | 法國南部 * | `francesouth` |
| Public | 印度 | 印度中部 | `centralindia` |
| Public | 印度 | 印度南部 | `southindia` |
| Public | 日本 | 日本東部 | `japaneast` |
| Public | 日本 | 日本西部 | `japanwest` |
| Public | 南韓 | 南韓中部 | `koreacentral` |
| Public | 南韓 | 南韓南部 | `koreasouth` |
| Public | 南非 | 南非北部 | `southafricanorth` |
| Public | 南非 | 南非西部 * | `southafricawest` |
| Public | 阿拉伯聯合大公國 | 阿拉伯聯合大公國中部 * | `uaecentral` |
| Public | 阿拉伯聯合大公國 | 阿拉伯聯合大公國北部 | `uaenorth` |
| Public | 英國 | 英國南部 | `uksouth` |
| Public | 英國 | 英國西部 | `ukwest` |
| Public | US | 美國中部 | `centralus` |
| Public | US | 美國東部 | `eastus` |
| Public | US | 美國東部 2 | `eastus2` |
| Public | US | 美國中北部 | `northcentralus` |
| Public | US | 美國中南部 | `southcentralus` |
| Public | US | 美國中西部 | `westcentralus` |
| Public | US | 美國西部 | `westus` |
| Public | US | 美國西部 2 | `westus2` |
...
- 只在新的檔案伺服器上建立伺服器端點，並使用`robocopy`將資料從舊的檔案共用複製到。 視新伺服器上的檔案共用拓撲而定（每個磁片區上有多少共用、每個磁片區的可用空間，等等），您可能需要暫時布建額外的存放裝置，因為從`robocopy`舊的伺服器到內部部署資料中心內的新伺服器會比將資料移至 Azure 的 Azure 檔案同步更快完成。
