...
description: 瞭解 Azure 檔案同步內部部署 proxy 和防火牆設定。 請參閱埠、網路和 Azure 的特殊連線的設定詳細資料。
author: roygara
ms.service: storage
ms.topic: how-to
ms.date: 06/24/2019
...
當伺服器向 Azure 檔案同步服務註冊之後，StorageSyncNetworkConnectivity Cmdlet 和 ServerRegistration.exe 就可以用來測試與此伺服器特定)  (Url 的所有端點的通訊。 此 Cmdlet 有助於疑難排解未完成的通訊，以防止伺服器完全使用 Azure 檔案同步，而且可以用來微調 proxy 和防火牆設定。
