---
title: 本地 NAS 遷移到 Azure 檔同步
description: 瞭解如何使用 Azure 檔同步和 Azure 檔共用將檔從本地網路附加存儲 （NAS） 位置遷移到混合雲部署。
author: fauhse
ms.service: storage
ms.topic: conceptual
ms.date: 03/19/2020
...
---
# <a name="migrate-from-network-attached-storage-nas-to-a-hybrid-cloud-deployment-with-azure-file-sync"></a>通過 Azure 檔同步從網路附加存儲 （NAS） 遷移到混合雲部署

Azure 檔同步適用于直接附加存儲 （DAS） 位置，不支援同步到網路附加存儲 （NAS） 位置。
這一事實使得您的檔遷移是必要的，本文將指導您完成此類遷移的規劃和執行。

## <a name="migration-goals"></a>移轉目標

目標是將 NAS 設備上的共用移動到 Windows 伺服器。 然後利用 Azure 檔同步進行混合雲部署。 需要以保證生產資料的完整性以及在遷移期間的可用性的方式完成此遷移。 後者要求將停機時間降至最低，以便它能夠適應或僅略高於常規維護視窗。

## <a name="migration-overview"></a>移轉概觀

如 Azure 檔[遷移概述一文中](storage-files-migration-overview.md)所述，使用正確的複製工具和方法非常重要。 您的 NAS 設備直接在本地網路上公開 SMB 共用。 RoboCopy 內置於 Windows 伺服器中，是在此遷移方案中移動檔的最佳方式。
...
* 預配或添加直接連接存儲（與不支援 NAS 的 NAS 相比的 DAS）。
    ...
    如果使用 Azure 檔同步[雲分層](storage-sync-cloud-tiering.md)功能，則預配的存儲量可能小於 NAS 設備上當前使用的存儲量。
    但是，當您在稍後階段將檔從較大的 NAS 空間複製到較小的 Windows Server 卷時，需要分批工作：

    1. 將一組適合磁片的檔移到磁片上
    2. 讓檔同步和雲分層參與
    3. 在卷上創建更多可用空間時，繼續下一批次檔。 
    
    通過在 Windows 伺服器上預配檔在 NAS 設備上佔用的等效空間，可以避免此批次處理方法。 請考慮 NAS/ Windows 上的重復資料消除。 如果不想將大量高存儲空間永久提交到 Windows 伺服器，則可以在遷移後和調整雲分層策略之前減小卷大小。 這將創建 Azure 檔共用的較小本機快取。
...
> [!WARNING]
> 如果 Windows 伺服器卷上的存儲空間比 NAS 設備上使用的資料少，則必須進行雲分層。 如果不打開雲分層，伺服器將不會釋放空間來存儲所有檔。 將分層策略（臨時用於遷移）設置為 99% 的卷可用空間。 請務必在遷移完成後返回到雲分層設置，並將其設置為更長期有用的級別。
...
基本遷移方法是從 NAS 設備到 Windows 伺服器的 RoboCopy，以及 Azure 檔同步到 Azure 檔共用。
...
以下 RoboCopy 命令會將檔從 NAS 存儲複製到 Windows Server 目的檔案夾。 Windows 伺服器將將其同步到 Azure 檔共用。 

如果在 Windows 伺服器上預配的存儲空間比檔在 NAS 設備上佔用的存儲少，則已配置雲分層。 當本地 Windows Server 卷已滿時，[雲分層](storage-sync-cloud-tiering.md)將啟動，並且已成功同步的層檔將啟動。 雲分層將生成足夠的空間來繼續從 NAS 設備複製。 雲分層每小時檢查一次，查看已同步的內容，並釋放磁碟空間以達到 99% 的卷可用空間。
...
首次運行 RoboCopy 命令時，使用者和應用程式仍在訪問 NAS 上的檔，並可能更改這些檔。 有可能，RoboCopy 已經處理了目錄，移動到下一個目錄，然後在源位置 （NAS） 上的使用者添加、更改或刪除現在在此 RoboCopy 運行中不會處理的檔。 這是預期的行為。

第一個運行是有關通過 Azure 檔同步將大部分資料移動到 Windows 伺服器並移動到雲中。此第一個副本可能需要很長時間，具體取決於：

* 您的下載頻寬
* 上傳頻寬
* 本地網路速度和 RoboCopy 執行緒數量與它匹配的最佳數量
...
當您認為停機時間可以接受，並準備使 NAS 位置離線時：為了讓使用者離線訪問，您可以選擇更改共用根上的 ACL，以便使用者無法再訪問該位置或執行任何其他適當的步驟防止內容在 NAS 上的此資料夾中更改。

運行最後一個 RoboCopy 輪。 它將接收任何可能錯過的更改。
這最後一步需要多長時間，取決於 RoboCopy 掃描的速度。 您可以通過測量上次運行所花的時間來估計時間（相當於停機時間）。

在 Windows Server 資料夾上創建共用，並可能調整 DFS-N 部署以指向該共用。 請務必設置與 NAS SMB 共用相同的共用級別許可權。 如果您有企業級域加入 NAS，則使用者 SID 將自動匹配，因為使用者存在於 Active Directory 中，RoboCopy 以全逼真度複製檔和中繼資料。 如果您在 NAS 上使用了本地使用者，則需要將這些使用者重新創建為 Windows Server 本地使用者，並將現有 SIDs RoboCopy 移動到您的 Windows 伺服器到新 Windows Server 本地使用者的 SID。
...
> [!WARNING]
> 將所有資料從 NAS 移動到 Windows 伺服器並完成遷移後：返回到 Azure 門戶***中的所有同步組***，並將雲分層卷可用空間百分比值調整為更適合緩存利用率的內容，例如 20%。 
