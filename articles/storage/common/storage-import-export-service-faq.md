...
有關資料容量跨越多個磁片導入作業時的詳細資訊和指導，請與 Microsoft 支援部門聯繫。
...
> [!NOTE]
> 印度的資料中心需要信頭（送達查蘭）上的聲明信才能返回磁碟機。 要安排所需的輸入通行證，還必須與所選運營商預訂取件，並與資料中心共用詳細資訊。
...
### <a name="are-there-any-special-requirements-for-delivering-my-disks-to-a-datacenter"></a>將磁片傳送到資料中心是否有任何特殊要求？

這些要求取決於特定的 Azure 資料中心限制。

- 出於安全原因，有幾個網站需要將 Microsoft 資料中心入站 ID 號寫入宗地。 在將磁碟機或磁片運送到資料中心之前，請與 Azure DataBox 操作部adbops@microsoft.com（ ） 聯繫以獲得此號碼。 如果沒有此編號，包將被拒絕。
- 印度的資料中心需要司機的個人資料，如政府身份證或證明號。 （例如，PAN、AADHAR、DL）、姓名、連絡人和車牌號，以獲得門進入通行證。 為避免交貨延誤，請告知您的承運商這些要求。
...
使用 Azure 門戶或[存儲資源管理器](https://docs.microsoft.com/azure/vs-azure-tools-storage-manage-with-storage-explorer)訪問 Azure 存儲帳戶下的資料。  
