...
Azure 儲存體也支援條件式標頭，例如，如 **有修改** ，則 **為，如果** 未修改，則為，如果是，則為，如果是， **則** 表示這些標頭的組合。 如需詳細資訊，請參閱[指定 Blob 服務作業的條件式標頭](/rest/api/storageservices/Specifying-Conditional-Headers-for-Blob-Service-Operations)。
...
若要鎖定 blob 以供獨佔使用，請取得其 [租用](/rest/api/storageservices/Lease-Blob) 。 當您取得租用時，您會指定租用的時間週期。 時間週期的範圍是從15到60秒或無限期，而其數量為獨佔鎖定。 更新有限的租用以進行擴充。 當您完成時，請釋放租用。 Blob 儲存體會在有限租用到期時自動加以釋放。
...
如果您直接嘗試對已租用的 Blob 執行寫入作業，而未傳送租用識別碼，要求將會失敗，並出現 412 錯誤。 如果租用在呼叫 **UploadText** 方法之前過期，但您仍然傳遞租用識別碼，要求也會失敗，並出現 **412** 錯誤。 如需管理租用到期時間和租用識別碼的詳細資訊，請參閱 [租用 Blob](/rest/api/storageservices/Lease-Blob) REST 文件。
...
* [實體上的作業](/rest/api/storageservices/Operations-on-Entities)
...
* [取得訊息](/rest/api/storageservices/Get-Messages)
...
* [管理檔案鎖定](/rest/api/storageservices/Managing-File-Locks)
...
* [Blob](../blobs/storage-quickstart-blobs-dotnet.md)、[資料表](../../cosmos-db/tutorial-develop-table-dotnet.md)、[佇列](../queues/storage-dotnet-how-to-use-queues.md)及[檔案](../files/storage-dotnet-how-to-use-files.md)的儲存體入門
