...
| 標準名稱 | 描述 |
| ------------------- | ----------------- |
| 異動 | 向儲存體服務或所指定 API 作業傳送的要求數。 此數目包括成功與失敗的要求，以及產生錯誤的要求。 <br/><br/> 單位：計數 <br/> 彙總類型：總計 <br/> 適用維度：ResponseType、GeoType、ApiName 和 Authentication ([定義](#metrics-dimensions))<br/> 值範例：1024 |
...
| 輸出 | 輸出資料量。 此數目包括從 Azure 儲存體的外部用戶端輸出，以及 Azure 內的輸出。 因此，此數目未反映可收費的輸出。 <br/><br/> 單位：位元組 <br/> 彙總類型：總計 <br/> 適用維度：GeoType、ApiName 和 Authentication ([定義](#metrics-dimensions)) <br/> 值範例：1024 |
...
| **驗證** | 交易中所使用的驗證類型。 可用的值包括： <br/> <li>**AccountKey**：交易已使用儲存體帳戶金鑰進行驗證。</li> <li>**SAS**：交易是以共用存取簽章進行驗證。</li> <li>**Oauth**：交易是以 oauth 存取權杖進行驗證。</li> <li>**Anonymous**：以匿名方式要求交易。 它不包含預檢要求。</li> <li>**AnonymousPreflight**：交易是預檢要求。</li> |
