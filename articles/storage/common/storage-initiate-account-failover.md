...
1. [上次同步時間]**** 屬性會指出次要複本與主要複本相差了多久的時間。 [上次同步時間]**** 可讓您預估在容錯移轉完成後發生資料遺失的程度。 如需有關檢查 [**上次同步時間**] 屬性的詳細資訊，請參閱[檢查儲存體帳戶的上次同步處理時間屬性](last-sync-time-get.md)。
1. 選取 [**準備容錯移轉**]。
...
若要在起始容錯移轉之前估計可能的資料遺失範圍，請檢查 [**上次同步時間**] 屬性。 如需有關檢查 [**上次同步時間**] 屬性的詳細資訊，請參閱[檢查儲存體帳戶的上次同步處理時間屬性](last-sync-time-get.md)。
