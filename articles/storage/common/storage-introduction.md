...
Azure 儲存體平臺是適用于新式資料儲存案例的 Microsoft 雲端儲存體解決方案。 核心儲存體服務為數據物件提供可大幅調整的物件存放區、適用于 Azure 虛擬機器（Vm）的磁片儲存體、適用于雲端的檔案系統服務、可靠訊息的訊息存放區，以及 NoSQL 存放區。 這些服務包括：
...
- [Azure Blob](../blobs/storage-blobs-introduction.md)：適用於文字和二進位資料且可大幅調整的物件存放區。 也包含透過 Data Lake Storage Gen2 進行海量資料分析的支援。
...
- [Azure 磁片](../../virtual-machines/windows/managed-disks-overview.md)：適用于 azure vm 的區塊層級儲存體磁片區。

每個服務都會透過儲存體帳戶存取。 若要開始使用，請參閱[建立儲存體帳戶](storage-account-create.md)。

## <a name="example-scenarios"></a>範例案例

下表會比較檔案、Blob、磁片、佇列和資料表，並顯示每個檔案的範例案例。

| 功能 | 描述 | 使用時機 |
|--------------|-------------|-------------|
| **Azure 檔案** |提供完全受控的雲端檔案共用，讓您可以從任何地方透過業界標準伺服器訊息區（SMB）通訊協定來存取。<br><br>您可以從雲端或內部部署的 Windows、Linux 和 macOS 掛接 Azure 檔案共用。 | 您想要將應用程式「隨即轉移」到雲端，其已使用原生檔案系統 Api，在其和 Azure 中執行的其他應用程式之間共用資料。<br/><br/>您想要取代或補充內部部署檔案伺服器或 NAS 裝置。<br><br> 您想要儲存需要從許多虛擬機器存取的開發和偵錯工具。 |
| **Azure Blob** | 允許在區塊 blob 中大規模地儲存及存取非結構化資料。<br/><br/>也支援將 [Azure Data Lake Storage Gen2](../blobs/data-lake-storage-introduction.md) 用於企業巨量資料分析解決方案。 | 您想要應用程式支援串流及隨機存取案例。<br/><br/>您想要能夠從任何位置存取應用程式資料。<br/><br/>您想要在 Azure 上建置企業 Data Lake，並執行巨量資料分析。 |
| **Azure 磁碟** | 允許從連接的虛擬硬碟持續儲存和存取資料。 | 您想要「隨即轉移」應用程式，使用原生檔案系統 Api 來讀取和寫入資料至持續性磁片。<br/><br/>您想要儲存不需要從連結磁碟的虛擬機器之外存取的資料。 |
| **Azure 佇列** | 允許在應用程式元件之間進行非同步訊息佇列。 | 您想要分離應用程式元件，並使用非同步訊息在兩者之間進行通訊。<br><br>如需使用佇列儲存體與服務匯流排佇列的相關指引，請參閱[儲存體佇列和服務匯流排佇列-比較和對比](https://docs.microsoft.com/azure/service-bus-messaging/service-bus-azure-and-service-bus-queues-compared-contrasted)。 |
| **Azure 資料表** | 可讓您將結構化的 NoSQL 資料儲存在雲端中，並提供具有無架構設計的索引鍵/屬性存放區。 | 您想要儲存彈性的資料集，例如 web 應用程式的使用者資料、通訊錄、裝置資訊，或服務所需的其他元資料類型。 <br/><br/>如需使用資料表儲存體與 Azure Cosmos DB 資料表 API 的相關指引，請參閱[使用 Azure Cosmos DB 資料表 API 和 Azure 資料表儲存體進行開發](../../cosmos-db/table-support.md)。 |
...
- 資源記錄、計量和損毀傾印只是三個數據範例，可寫入檔案共用並于稍後處理或分析。
...
Azure 受控磁碟是虛擬硬碟 (VHD)。 您可以將它視為內部部署伺服器中虛擬化的實體磁碟。 Azure 受控磁片會儲存為分頁 blob，這是 Azure 中的隨機 IO 儲存物件。 我們會呼叫受控磁片「受控」，因為它是對分頁 blob、blob 容器和 Azure 儲存體帳戶的抽象概念。 使用受控磁碟，您所要做的就是佈建磁碟，Azure 會負責執行剩餘的部分。
...
- **針對 Azure 檔案儲存體 Azure AD 透過 SMB 的授權。** Azure 檔案儲存體透過 Azure Active Directory Domain Services （Azure AD DS）或內部部署 Active Directory Domain Services （預覽），支援透過 SMB （伺服器訊息區）進行以身分識別為基礎的授權。 已加入網域的 Windows Vm 可以使用 Azure AD 認證來存取 Azure 檔案共用。 如需詳細資訊，請參閱[SMB 存取的 Azure 檔案儲存體身分識別型驗證支援的總覽](../files/storage-files-active-directory-overview.md)和[規劃 Azure 檔案儲存體部署](../files/storage-files-planning.md#identity)。
- **使用共用金鑰進行授權。** Azure 儲存體 Blob、檔案、佇列和表格服務都支援使用共用金鑰進行授權。 使用共用金鑰授權的用戶端會將標頭與使用儲存體帳戶存取金鑰簽署的每個要求一起傳遞。 如需詳細資訊，請參閱[使用共用金鑰進行授權](https://docs.microsoft.com/rest/api/storageservices/authorize-with-shared-key)。
...
有兩種基本的加密類型可用於核心儲存體服務。 如需安全性和加密的詳細資訊，請參閱 [Azure 儲存體安全性指南](../blobs/security-recommendations.md)。
...
為確保您的資料持久，Azure 儲存體儲存資料的多個複本。 當您設定儲存體帳戶時，您可選取備援選項。 如需詳細資訊，請參閱 [Azure 儲存體備援](/azure/storage/common/storage-redundancy?toc=/azure/storage/blobs/toc.json) \(部分機器翻譯\)。
...
您可以使用任何可提出 HTTP/HTTPS 要求的語言，來存取儲存體帳戶中的資源。 此外，核心 Azure 儲存體服務也提供數種熱門語言的程式設計程式庫。 這些程式庫可透過處理詳細資料 (例如同步和非同步叫用、進行批次作業、例外狀況管理、自動重試、運作方式等等) 來簡化使用 Azure 儲存體的許多項目。 程式庫目前適用於下列語言和平台，以及正在研發的其他語言和平台：
