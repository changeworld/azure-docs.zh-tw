...
* **URL 格式**：可利用下列 URL 格式來定址 Blob：
    ...
    在上述格式中，`storageaccount` 代表您的儲存體帳戶名稱，`container_name` 代表您的容器名稱，而 `blob_name` 代表您的 Blob 名稱。 在容器名稱中，您可以有多個路徑，並以正斜線分隔**/**。 本教學課程中的範例容器名稱為 **MyJob**，使用的共同虛擬路徑則是 **${BUILD\_ID}/${BUILD\_NUMBER}**，產生的 Blob URL 格式如下：
