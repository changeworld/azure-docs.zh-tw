...
2. 執行 OpenSSL。
    ...
    * Mac 和 Linux：從`openssl`終端機執行。
3. 執行 `s_client -showcerts -connect microsoft.com:443`。
4. 尋找自我簽署憑證。 如果您不確定哪些是自我簽署的憑證，請記下主體`("s:")`和發行者`("i:")`相同的任何位置。
5. 當您找到自我簽署的憑證時，請將其從（和包含） `-----BEGIN CERTIFICATE-----`複製並貼`-----END CERTIFICATE-----`到新的 .cer 檔案。
...
2. 尋找具有索引`<connection_type_key>/<corrupted_connection_name>`鍵的專案（例如`StorageExplorer_CustomConnections_Accounts_v1/account1`）。
3. 刪除這些專案，然後重新加入連接。
