---
title: 使用異地冗余存儲設計高可用應用程式
titleSuffix: Azure Storage
description: 瞭解如何使用讀取存取異地冗余存儲來構建足夠靈活的高可用應用程式，以處理中斷。
services: storage
author: tamram
ms.service: storage
ms.topic: conceptual
...
---
# <a name="designing-highly-available-applications-using-read-access-geo-redundant-storage"></a>使用讀取存取異地冗余存儲設計高可用應用程式

雲端式基礎結構 (例如 Azure 儲存體) 的常見功能是提供高可用性平台來裝載應用程式。 雲端式應用程式的開發人員必須仔細考慮如何運用此平台，來為使用者提供高可用性應用程式。 本文重點介紹開發人員如何使用 Azure 的異地冗余複製選項之一來確保其 Azure 存儲應用程式高度可用。

為異地冗余複製配置的存儲帳戶在主區域中同步複製，然後非同步複製到數百英里外的次要區域。 Azure 存儲提供兩種類型的異地冗余複製：

* [地理區域冗余存儲 （GZRS） （預覽）](storage-redundancy.md)為需要高可用性和最大持久性的方案提供複製。 使用區域冗余存儲 （ZRS） 在主區域的三個 Azure 可用性區域中同步複製資料，然後非同步複製到次要區域。 對於對次要區域中資料的讀取存取，啟用讀取存取地理區域冗余存儲 （RA-GZRS）。
* [異地冗余存儲 （GRS）](storage-redundancy.md)提供跨區域複製，以防止區域中斷。 使用本地冗余存儲 （LRS） 在主區域中同步複製資料三次，然後非同步複製到次要區域。 對於對次要區域中資料的讀取存取，啟用讀取存取異地冗余存儲 （RA-GRS）。

本文演示如何設計應用程式以處理主區域中的中斷。 如果主區域不可用，應用程式可以改為對次要區域執行讀取操作。 在開始之前，請確保您的存儲帳戶已配置為 RA-GRS 或 RA-GZRS。
...
## <a name="application-design-considerations-when-reading-from-the-secondary"></a>從輔助讀取時的應用程式設計注意事項
...
在為 RA-GRS 或 RA-GZRS 設計應用程式時，請記住以下要點：
...
* 您可以使用存儲用戶端庫讀取和寫入主區域或次要區域中的資料。 如果對主要區域的讀取要求逾時，您也可以將讀取要求自動重新導向到次要區域。
...
> [!NOTE]
> 客戶託管的帳戶容錯移轉（預覽）在支援 GZRS/RA-GZRS 的區域尚不可用，因此客戶當前無法使用 GZRS 和 RA-GZRS 帳戶管理帳戶容錯移轉事件。 在預覽期間，Microsoft 將管理任何影響 GZRS/RA-GZRS 帳戶的容錯移轉事件。
...
例如，假設您的客戶成功提交更新，但是在更新傳播到次要區域之前主要區域會失敗。 當客戶要求讀取資料時，他們會從次要區域接收陳舊的資料，而不是更新的資料。 設計您的應用程式時，您必須決定是否可接受此情況，如果可以，您該如何傳達給客戶。 
...
若要有效地為主區域中的中斷做好準備，您必須能夠處理失敗的讀取請求和失敗的更新要求（在這種情況下，更新意味著插入、更新和刪除）。 如果主區域失敗，讀取請求可以重定向到次要區域。 不過，因為次要資料中心是唯讀的，所以無法將更新要求重新導向到次要資料中心。 基於這個理由，您必須設計您的應用程式在唯讀模式中執行。
...
Azure 存儲用戶端庫可説明您確定哪些錯誤可以重試。 例如，不會重試 404 錯誤（未找到資源），因為重試它不太可能導致成功。 另一方面，可以重試 500 錯誤，因為它是伺服器錯誤，並且問題可能只是暫時性問題。 如需詳細資訊，請參閱 .NET 儲存體用戶端程式庫中的 [ExponentialRetry 類別的開放原始程式碼 (英文)](https://github.com/Azure/azure-storage-net/blob/87b84b3d5ee884c7adc10e494e2c7060956515d0/Lib/Common/RetryPolicies/ExponentialRetry.cs) (尋找 ShouldRetry 方法)。

### <a name="read-requests"></a>讀取要求

如果主要儲存體發生問題，就可以將讀取要求重新導向到次要儲存體。 如前面[最終使用一致的資料](#using-eventually-consistent-data)中所述，您的應用程式必須可以接受可能讀取到過時的資料。 如果使用存儲用戶端庫從次要資料庫訪問資料，則可以通過將**LocationMode**屬性的值設置為以下值來指定讀取請求的重試行為：
...
將**LocationMode**設置為 **"主集"時**，如果主終結點的初始讀取請求失敗且錯誤可以重試，則用戶端會自動對輔助終結點發出另一個讀取請求。 如果錯誤是伺服器逾時，則用戶端在接收到服務所提供的可重試錯誤之前，必須等候逾時到期。
...
地理冗余存儲的工作原理是將事務從主區域複製到次要區域。 此複寫程序可保證次要區域中的資料是*最終一致*的。 這表示，主要區域中的所有交易最終都會出現在次要區域中，但有可能會延遲出現，而且不保證交易會以原本在主要區域中套用它們的相同順序到達次要區域。 如果您的交易不按順序到達次要區域，則您可能** 要考慮讓次要區域中的資料處於不一致狀態，直到服務更新為止。

下表顯示了更新員工的詳細資訊以使其成為*管理員*角色成員時可能發生的情況的示例。 基於此範例，這會要求您更新**員工**實體，並利用系統管理員總數的計數來更新**系統管理員角色**實體。 請注意，如何在次要區域中不按順序套用更新。
...
要瞭解如何檢查上次同步時間，請參閱[檢查存儲帳戶的最後同步時間屬性](last-sync-time-get.md)。
...
* 有關如何從次要區域讀取的詳細資訊（包括如何設置上次同步時間屬性的另一個示例），請參閱[Azure 存儲冗余選項和讀取存取異地冗余存儲](https://blogs.msdn.microsoft.com/windowsazurestorage/2013/12/11/windows-azure-storage-redundancy-options-and-read-access-geo-redundant-storage/)。

* 有關演示如何在主終結點和輔助終結點之間來回切換的完整示例，請參閱 Azure 示例 —[使用具有 RA-GRS 存儲的斷路器模式](https://github.com/Azure-Samples/storage-dotnet-circuit-breaker-pattern-ha-apps-using-ra-grs)。
