...
您可以建立[預算](../../cost-management-billing/costs/tutorial-acm-create-budgets.md)來管理成本，並建立警示，以自動通知專案關係人如何耗費異常和超支風險。 警示是以支出為基礎 (相較於預算和成本閾值)。 預算和警示是針對 Azure 訂用帳戶和資源群組所建立，因此在整體成本監視策略中相當有用。 不過，它們的功能可能有限，可以管理個別的 Azure 服務成本，例如 Azure 儲存體的成本，因為它們是設計來追蹤較高層級的成本。
