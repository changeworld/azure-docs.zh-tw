...
* [地理區域-多餘儲存體 (GZRS) ](storage-redundancy.md)：資料會使用 *區域冗余儲存體 (ZRS) *，以同步方式跨主要區域中的三個 Azure 可用性區域複寫，然後以非同步方式複寫到次要區域。 如需次要區域中資料的讀取權限，請啟用讀取權限地理區域-多餘儲存體 (GZRS) 。

    Microsoft 建議在需要最大可用性和耐久性的案例中使用 GZRS/RA-GZRS。

* [異地複寫儲存體 (GRS) ](storage-redundancy.md)：資料會在主要區域中，以同步方式在主要區域中複寫三次，並使用 *本地冗余儲存體 (LRS) *，然後以非同步方式複寫到次要區域。 若要對次要區域中的資料進行讀取存取，請啟用讀取權限異地冗余儲存體 (GRS) 。
