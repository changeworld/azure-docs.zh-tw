---
title: 使用異地備援來設計高可用性的應用程式
titleSuffix: Azure Storage
description: 瞭解如何使用異地複寫儲存體來設計高可用性的應用程式，此應用程式有足夠的彈性來處理中斷。
services: storage
author: tamram
ms.service: storage
ms.topic: conceptual
...
ms.locfileid: "92480678"
---
# <a name="use-geo-redundancy-to-design-highly-available-applications"></a>使用異地備援來設計高可用性的應用程式

雲端式基礎結構（例如 Azure 儲存體）的常見功能是提供高可用性且持久的平臺來裝載資料和應用程式。 雲端式應用程式的開發人員必須仔細考慮如何利用此平臺，讓他們的使用者發揮最大的優勢。 Azure 儲存體提供異地多餘的儲存體，以確保即使在發生區域性中斷時，仍可提供高可用性。 針對地理位置冗余複寫所設定的儲存體帳戶會在主要區域中以同步方式複寫，然後以非同步方式複寫到數百英里離開的次要區域。

Azure 儲存體提供兩個選項來進行異地冗余複寫。 這兩個選項之間的唯一差異在於在主要區域中複寫資料的方式：

* [地理區域-多餘儲存體 (GZRS) ](storage-redundancy.md)：資料會使用 *區域冗余儲存體 (ZRS) *，以同步方式跨主要區域中的三個 Azure 可用性區域複寫，然後以非同步方式複寫到次要區域。 如需次要區域中資料的讀取存取權，請啟用讀取存取異地區域備援儲存體 (RA-GZRS)。

    Microsoft 建議在需要最大可用性和耐久性的案例中使用 GZRS/RA-GZRS。

* [異地複寫儲存體 (GRS) ](storage-redundancy.md)：資料會在主要區域中，以同步方式在主要區域中複寫三次，並使用 *本地冗余儲存體 (LRS) *，然後以非同步方式複寫到次要區域。 如需次要區域中資料的讀取存取權，請啟用讀取存取異地備援儲存體 (RA-GRS)。

本文說明如何設計您的應用程式來處理主要區域中斷。 如果主要區域變得無法使用，您的應用程式可以改為針對次要區域執行讀取作業。 開始之前，請確定您的儲存體帳戶已設定為使用 RA-GRS 或 GZRS。
...
* 如果主要區域無法使用，您可以進行帳戶容錯移轉。 您容錯移轉到次要區域時，指向主要區域的 DNS 項目會變更為指向次要區域。 完成容錯移轉之後，會還原 GRS 和 RA-GRS 帳戶的寫入權限。 如需詳細資訊，請參閱[災害復原與儲存體帳戶容錯移轉](storage-disaster-recovery-guidance.md)。
...
例如，假設您的客戶成功提交更新，但是在更新傳播到次要區域之前主要區域會失敗。 當客戶要求讀取資料時，會從次要區域接收過時的資料，而不是更新的資料。 設計您的應用程式時，您必須決定是否可接受此情況，如果可以，您該如何傳達給客戶。 
...
若要有效地為主要區域中的中斷做好準備，您必須能夠處理失敗的讀取要求和失敗的更新要求 (但在此案例中，這表示插入、更新和刪除) 。 如果主要區域失敗，可將讀取要求重新導向至次要區域。 不過，因為次要資料中心是唯讀的，所以無法將更新要求重新導向到次要資料中心。 基於這個理由，您必須設計您的應用程式在唯讀模式中執行。
...
Azure 儲存體用戶端程式庫可協助您判斷哪些錯誤可以重試。 例如，404錯誤 (找不到資源) 不會重試，因為重試不太可能會導致成功。 另一方面，500錯誤可能會因為伺服器錯誤而重試，而且問題可能只是暫時性的問題。 如需詳細資訊，請參閱 .NET 儲存體用戶端程式庫中的 [ExponentialRetry 類別的開放原始程式碼 (英文)](https://github.com/Azure/azure-storage-net/blob/87b84b3d5ee884c7adc10e494e2c7060956515d0/Lib/Common/RetryPolicies/ExponentialRetry.cs) (尋找 ShouldRetry 方法)。

### <a name="read-requests"></a>讀取要求

如果主要儲存體發生問題，就可以將讀取要求重新導向到次要儲存體。 如前面[最終使用一致的資料](#using-eventually-consistent-data)中所述，您的應用程式必須可以接受可能讀取到過時的資料。 如果您使用儲存體用戶端程式庫來存取來自次要資料庫的資料，您可以將 **>locationmode** 屬性的值設定為下列其中一項，以指定讀取要求的重試行為：
...
當您將 **>locationmode** 設定為 **>primarythensecondary**時，如果對主要端點的初始讀取要求失敗，並出現可重試的錯誤，則用戶端會自動對次要端點提出另一個讀取要求。 如果錯誤是伺服器逾時，則用戶端在接收到服務所提供的可重試錯誤之前，必須等候逾時到期。
...
* 針對您傳遞到儲存體要求的 [**OperationContext**](/java/api/com.microsoft.applicationinsights.extensibility.context.operationcontext) 物件上的 [**Retrying**](/dotnet/api/microsoft.azure.cosmos.table.operationcontext.retrying) 事件新增處理常式 - 這是本文所示的方法，並會在隨附的範例中使用。 每當用戶端重試要求時，這些事件即會觸發，讓您能夠追蹤用戶端在主要端點上發生可重試錯誤的頻率。
...
* 在自訂重試原則的 [**Evaluate**](/dotnet/api/microsoft.azure.cosmos.table.iextendedretrypolicy.evaluate) 方法中，您可以在每次進行重試時執行自訂程式碼。 除了記錄重試發生的時間，這也能讓您有機會修改重試行為。
...
異地冗余儲存體的運作方式是將交易從主要區域複寫到次要區域。 此複寫程式可保證次要區域中的資料最終是 *一致*的。 這表示，主要區域中的所有交易最終都會出現在次要區域中，但有可能會延遲出現，而且不保證交易會以原本在主要區域中套用它們的相同順序到達次要區域。 如果您的交易不按順序到達次要區域，則您可能** 要考慮讓次要區域中的資料處於不一致狀態，直到服務更新為止。

下表顯示當您更新員工的詳細資料，使其成為系統 *管理員* 角色的成員時，可能發生的狀況。 基於此範例，這會要求您更新**員工**實體，並利用系統管理員總數的計數來更新**系統管理員角色**實體。 請注意，如何在次要區域中不按順序套用更新。
...
若要瞭解如何檢查上次同步處理時間，請參閱 [檢查儲存體帳戶的上次同步時間屬性](last-sync-time-get.md)。
...
如需示範如何在主要與次要端點之間來回切換的完整範例，請參閱 [Azure 範例–搭配使用斷路器模式與 GRS 儲存體](https://github.com/Azure-Samples/storage-dotnet-circuit-breaker-pattern-ha-apps-using-ra-grs)。
