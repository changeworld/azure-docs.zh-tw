...
若要列出具有 Azure CLI 的可用 Azure 內建角色，請使用[az role definition list](/cli/azure/role/definition#az-role-definition-list)命令：
...
若要將 Azure 角色指派給安全性主體，請使用[az role 指派 create](/cli/azure/role/assignment#az-role-assignment-create)命令。 命令的格式可能會根據指派的範圍而有所不同。 下列範例示範如何將角色指派給不同範圍的使用者，但您可以使用相同的命令，將角色指派給任何安全性主體。
