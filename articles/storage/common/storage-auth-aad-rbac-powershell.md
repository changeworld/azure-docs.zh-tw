...
Azure Active Directory (Azure AD) 會透過 azure [RBAC () 以角色為基礎的存取控制](../../role-based-access-control/overview.md)，授權保護資源的存取權限。 Azure 儲存體定義一組 Azure 內建角色，其中包含用來存取容器或佇列的常用許可權集。
