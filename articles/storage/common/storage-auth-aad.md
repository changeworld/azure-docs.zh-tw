...
Azure 儲存體支援使用 Azure Active Directory (Azure AD) 來授權對 Blob 和佇列儲存體的要求。 使用 Azure AD 時，您可以使用 Azure 角色型存取控制 (Azure RBAC) 來授與許可權給安全性主體，該安全性主體可能是使用者、群組或應用程式服務主體。 安全性主體會由 Azure AD 進行驗證，以傳回 OAuth 2.0 權杖。 然後，權杖可以用來授權對 Blob 或佇列儲存體的要求。
