...
ms.locfileid: "79454664"
---
# <a name="configure-azure-storage-firewalls-and-virtual-networks"></a>設定 Azure 儲存體防火牆和虛擬網路

Azure 儲存體提供分層的安全性模型。 此模型使您能夠根據所使用的網路的類型和子集，保護和控制應用程式和企業環境需要的存儲帳戶的存取層級。 配置網路規則後，只有通過指定網路集請求資料的應用程式才能訪問存儲帳戶。 您可以將對存儲帳戶的訪問限制為來自指定 IP 位址、IP 範圍或 Azure 虛擬網路 （VNet） 中的子網清單的請求。

存儲帳戶具有可通過 Internet 訪問的公共終結點。 您還可以[為存儲帳戶創建專用終結點](storage-private-endpoints.md)，該終結點將私人 IP 位址從 VNet 分配給存儲帳戶，並通過專用連結保護 VNet 和存儲帳戶之間的所有流量。 Azure 存儲防火牆為存儲帳戶的公共終結點提供存取控制存取權限。 使用專用終結點時，還可以使用防火牆阻止通過公共終結點進行的所有訪問。 存儲防火牆配置還允許選擇受信任的 Azure 平臺服務來安全地訪問存儲帳戶。

在網路規則生效時訪問存儲帳戶的應用程式仍然需要對請求進行適當的授權。 用於 Blob 和佇列的 Azure 活動目錄 （Azure AD） 憑據、具有有效帳戶訪問金鑰或 SAS 權杖的 Azure 活動目錄 （Azure AD） 憑據支援授權。

> [!IMPORTANT]
> 預設情況下，打開存儲帳戶的防火牆規則會阻止傳入的資料請求，除非請求來自在 Azure 虛擬網路 （VNet） 中運行的服務或來自允許的公共 IP 位址。 封鎖的要求包括來自其他 Azure 服務、Azure 入口網站及記錄與計量服務等等的要求。
>
> 通過允許來自託管服務實例的子網的流量，可以授予對在 VNet 中運行的 Azure 服務的許可權。 您還可以通過下面描述的[例外](#exceptions)機制啟用數量有限的方案。 要通過 Azure 門戶從存儲帳戶訪問資料，您需要位於您設置的受信任邊界（IP 或 VNet）內的電腦上。

[!INCLUDE [updated-for-az](../../../includes/updated-for-az.md)]

## <a name="scenarios"></a>案例

為了保護存儲帳戶，應首先配置一條規則，以拒絕從公共終結點上的所有網路（包括 Internet 流量）訪問流量。 然後，應配置授予從特定 VNet 訪問流量的規則。 您還可以配置規則以授予從選定的公共 Internet IP 位址範圍訪問流量的許可權，從而啟用來自特定 Internet 或本地用戶端的連接。 此設定可讓您為應用程式設定安全的網路界限。

可以組合允許從特定虛擬網路和同一存儲帳戶上的公共 IP 位址範圍進行訪問的防火牆規則。 存儲防火牆規則可以應用於現有存儲帳戶，也可以在創建新存儲帳戶時應用。

存儲防火牆規則適用于存儲帳戶的公共終結點。 您不需要任何防火牆訪問規則來允許存儲帳戶的專用終結點的流量。 批准創建專用終結點的過程允許隱式訪問來自承載專用終結點的子網的流量。

所有的 Azure 儲存體網路通訊協定都會執行網路規則，包括 REST 和 SMB。 要使用 Azure 門戶、存儲資源管理器和 AZCopy 等工具訪問資料，必須配置顯式網路規則。
...
> [!WARNING]
> 變更網路規則會影響應用程式連接到 Azure 儲存體的能力。 將預設網路規則設置為**拒絕**阻止對資料的所有訪問，除非還應用了**授予**存取權限的特定網路規則。 請務必先將存取權授與任何使用網路規則的允許網路，再變更預設規則以拒絕存取。
...
1. 預設情況下，要拒絕訪問，請選擇允許從**選定網路**進行訪問。 要允許來自所有網路的流量，請選擇允許從**所有網路**進行訪問。
...
您可以將存儲帳戶配置為僅允許從特定子網進行訪問。 允許的子網可能屬於同一訂閱中的 VNet 或其他訂閱中的 VNet，包括屬於其他 Azure 活動目錄租戶的訂閱。

為 VNet 內的 Azure 儲存體啟用[服務端點](/azure/virtual-network/virtual-network-service-endpoints-overview)。 服務終結點通過 Azure 存儲服務的最佳路徑路由流量從 VNet。 子網和虛擬網路的標識也會隨每個請求一起傳輸。 然後，管理員可以為存儲帳戶配置網路規則，允許從 VNet 中的特定子網接收請求。 透過這些網路規則授與存取的用戶端，必須仍要繼續符合儲存體帳戶的授權需求，才能存取資料。
...
存儲帳戶和授予存取權限的虛擬網路可能位於不同的訂閱中，包括屬於其他 Azure AD 租戶的訂閱。
...
1. 為虛擬網路和子網路新增網路規則。
    ...
    > [!TIP]
    > 要在屬於另一個 Azure AD 租戶的 VNet 中添加子網的規則，請使用"/訂閱/\<訂閱 ID/\>資源組/\<資源組/資源組名稱\>/提供程式/Microsoft.Network/虛擬網路/vNet\<名稱\>/子網/\<子網名稱\>"中的完全限定子網 ID。
    >
...
只有**公用網際網路** IP 位址允許使用 IP 網路規則。 IP 規則中不允許保留私人網路的 IP 位址範圍 (如 [RFC 1918](https://tools.ietf.org/html/rfc1918#section-3) 中所定義)。 私人網路包括以 _10.*_、_172.16.*_ - _172.31.*_ 和 _192.168.*_ 開頭的位址。
  ...
  > [!NOTE]
  > 部署在與存儲帳戶相同的區域中的服務使用專用 Azure IP 位址進行通信。 因此，不能基於特定 Azure 服務的公共入站 IP 位址範圍限制對特定 Azure 服務的訪問。

存儲防火牆規則的配置僅支援 IPV4 位址。

每個存儲帳戶最多支援 100 個 IP 網路規則。
...
1. 要授予對互聯網 IP 範圍的存取權限，請在**防火牆** > **位址範圍**下輸入 IP 位址或位址範圍（以 CIDR 格式）。
...
網路規則有助於為應用程式和資料之間的連接創建安全的環境，適用于大多數方案。 但是，某些應用程式依賴于無法通過虛擬網路或 IP 位址規則唯一隔離的 Azure 服務。 但是，必須將此類服務授予存儲，以實現完整的應用程式功能。 在這種情況下，您可以使用 ***"允許受信任的 Microsoft 服務...***

### <a name="trusted-microsoft-services"></a>受信任的 Microsoft 服務

某些 Microsoft 服務使用網路規則中無法包括的網路進行操作。 您可以授予此類受信任 Microsoft 服務對存儲帳戶的子集，同時維護其他應用的網路規則。 然後，這些受信任的服務將使用增強式驗證安全地連接到存儲帳戶。 我們為 Microsoft 服務啟用了兩種受信任訪問模式。

- 某些服務的資源**在訂閱中註冊時**，可以訪問**同一訂閱中的**存儲帳戶，以便選擇操作，例如編寫日誌或備份。
- 某些服務的資源可以通過為其系統分配的託管標識**分配 RBAC 角色**來授予對存儲帳戶的顯式存取權限。


啟用 **"允許受信任的 Microsoft 服務..."** 設置時，在與存儲帳戶相同的訂閱中註冊的以下服務的資源將被授予對一組有限操作的存取權限，如下所述：
...
| Azure 資料箱           | Microsoft.DataBox          | 使用資料框將資料導入 Azure。 [深入了解](/azure/databox/data-box-overview)。 |
...
| Azure 檔案同步          | Microsoft.StorageSync      | 使您能夠將上置檔案伺服器轉換為 Azure 檔共用的緩存。 允許多網站同步、快速災害復原和雲端備份。 [深入了解](../files/storage-sync-files-planning.md) |
| Azure HDInsight          | Microsoft.HDInsight        | 為新的 HDInsight 群集預配預設檔案系統的初始內容。 [深入了解](/azure/hdinsight/hdinsight-hadoop-use-blob-storage)。 |
| Azure 導入匯出      | Microsoft.ImportExport     | 啟用將資料導入 Azure 並使用導入/匯出服務從 Azure 匯出資料。 [深入了解](/azure/storage/common/storage-import-export-service)。  |
| Azure 監視器            | Microsoft.Insights         | 允許將監視資料寫入安全存儲帳戶，包括資源診斷日誌、Azure 活動目錄登錄和稽核記錄以及 Microsoft Intune 日誌。 [深入了解](/azure/monitoring-and-diagnostics/monitoring-roles-permissions-security)。 |
...
| Azure Site Recovery      | Microsoft.SiteRecovery     | 使用啟用防火牆的緩存、源或目標存儲帳戶時，為 Azure IaaS 虛擬機器的災害復原啟用複製。  [深入了解](https://docs.microsoft.com/azure/site-recovery/azure-to-azure-tutorial-enable-replication)。 |

如果顯式[將 RBAC 角色指派給](storage-auth-aad.md#assign-rbac-roles-for-access-rights)[資源實例的系統分配的託管標識](../../active-directory/managed-identities-azure-resources/overview.md)，則"**允許受信任的 Microsoft 服務..."** 設置還允許以下服務的特定實例訪問存儲帳戶。 在這種情況下，實例的訪問範圍對應于分配給託管標識的 RBAC 角色。

| 服務                        | 資源提供者名稱                 | 目的            |
| :----------------------------- | :------------------------------------- | :----------------- |
| Azue 認知搜尋         | Microsoft.Search/searchServices        | 使認知搜索服務能夠訪問存儲帳戶以進行索引、處理和查詢。 |
| Azure Container Registry 工作 | Microsoft.ContainerRegistry/registries | ACR 任務可以在構建容器映射時訪問存儲帳戶。 |
| Azure Data Factory             | Microsoft.DataFactory/factories        | 允許通過 ADF 運行時訪問存儲帳戶。 |
| Azure Data Share               | 微軟.資料共用/帳戶           | 允許通過資料共用訪問存儲帳戶。 |
| Azure Logic Apps               | Microsoft.Logic/workflows              | 使邏輯應用能夠訪問存儲帳戶。 [深入了解](/azure/logic-apps/create-managed-service-identity#authenticate-access-with-managed-identity)。 |
| Azure Machine Learning 服務 | Microsoft.MachineLearningServices      | 授權的 Azure 機器學習工作區將實驗輸出、模型和日誌寫入 Blob 存儲並讀取資料。 [深入了解](/azure/machine-learning/service/how-to-enable-virtual-network#use-a-storage-account-for-your-workspace)。 | 
| Azure SQL 資料倉儲       | Microsoft.Sql                          | 允許使用 PolyBase 從特定的 SQL 資料庫實例導入和匯出資料。 [深入了解](/azure/sql-database/sql-database-vnet-service-endpoint-rule-overview)。 |
| Azure 串流分析         | Microsoft.StreamAnalytics             | 允許將流式處理作業中的資料寫入 Blob 存儲。 此功能目前為預覽狀態。 [深入了解](/azure/stream-analytics/blob-output-managed-identity)。 |
| Azure Synapse Analytics        | 微軟.Synapse/工作區          | 支援從 Synapse 分析訪問 Azure 存儲中的資料。 |


### <a name="storage-analytics-data-access"></a>儲存體分析資料存取

在某些情況下，需要來自網路界限外的存取權才能讀取診斷記錄和計量。 配置對存儲帳戶的受信任服務存取權限時，可以允許日誌檔、指標表或兩者進行讀取存取。 [深入了解儲存體分析的使用方式。](/azure/storage/storage-analytics)
...
在[Azure 存儲安全指南](../blobs/security-recommendations.md)中深入瞭解 Azure 存儲安全性。
