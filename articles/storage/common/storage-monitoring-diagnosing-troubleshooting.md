...
* [疑難排解指引]
  ...
  * [計量顯示低 PercentSuccess 或分析記錄專案具有交易狀態為 ClientOtherErrors 的作業]
  * [容量度量顯示非預期的儲存體容量使用量增加]
...
* [附錄]
  * [附錄1：使用 Fiddler 來捕捉 HTTP 和 HTTPS 流量]
  * [附錄2：使用 Wireshark 來捕獲網路流量]
  * [附錄3：使用 Microsoft Message Analyzer 來捕獲網路流量]
  * [附錄4：使用 Excel 來查看計量和記錄資料]
  * [附錄5：使用 Azure DevOps 的 Application Insights 進行監視]
...
* [表格服務錯誤碼](https://msdn.microsoft.com/library/azure/dd179438.aspx)
...
* [Fiddler](https://www.telerik.com/fiddler) 是免費的 Web 偵錯 Proxy，可讓您檢視 HTTP 與 HTTPS 要求及回應訊息的標頭與承載資料。 如需詳細資訊，請參閱[附錄1：使用 Fiddler 來捕捉 HTTP 和 HTTPS 流量](#appendix-1)。
...
* 在儲存體用戶端程式庫所建立的用戶端記錄中，用戶端要求 ID 會出現在與要求相關之每個記錄專案的 [**用戶端要求識別碼**] 欄位中。
* 在 Fiddler 所捕捉的網路追蹤中，用戶端要求識別碼會顯示在要求訊息中，做為**x-ms-用戶端要求識別碼**HTTP 標頭值。
...
* 在儲存體用戶端程式庫所建立的用戶端記錄中，伺服器要求 ID 會出現在顯示伺服器回應詳細資料之記錄專案的 [作業**文字**] 欄位中。
...
| Microsoft Azure 儲存體 |資訊 |3 |85d077ab -… |啟動對的同步要求<https://domemaildist.blob.core.windows.netazureimblobcontainer/blobCreatedViaSAS.txt?sv=2014-02-14&sr=c&si=mypolicy&sig=OFnd4Rd7z01fIvh%2BmcR6zbudIH2F5Ikm%2FyhNYZEmJNQ%3D&api-version=2014-02-14> |
...
用戶端中的例外狀況詳細資料包括要求的表格服務所指派的要求識別碼（7e84f12d ... ...）：您可以使用這項資訊，在記錄檔的 [**要求識別碼-標頭] 資料**行中搜尋，以尋找伺服器端儲存體記錄中的要求詳細資料。 您也可以使用度量，判定此類失敗情況何時發生，然後依據度量記錄此錯誤的時間搜尋記錄檔。 此記錄項目顯示刪除作業失敗，並顯示「HTTP (404) 用戶端其他錯誤」的狀態訊息。 相同的記錄專案也會在**用戶端要求識別碼**資料行（813ea74f ... ...）中包含用戶端所產生的要求識別碼。
...
[Fiddler](https://www.telerik.com/fiddler)是一個實用的工具，可用於分析用戶端應用程式與您所使用的 Azure 儲存體服務之間的 HTTP 和 HTTPS 流量。
...
[計量顯示低 PercentSuccess 或分析記錄專案具有交易狀態為 ClientOtherErrors 的作業]: #metrics-show-low-percent-success
[容量度量顯示非預期的儲存體容量使用量增加]: #capacity-metrics-show-an-unexpected-increase
[您的問題起因於使用儲存體模擬器進行開發或測試]: #your-issue-arises-from-using-the-storage-emulator
[「X」功能無法在儲存體模擬器中運作]: #feature-X-is-not-working
[使用儲存體模擬器時，發生「其中一個 HTTP 標頭的值格式不正確」錯誤]: #error-HTTP-header-not-correct-format
[執行儲存體模擬器需要系統管理員許可權]: #storage-emulator-requires-administrative-privileges
[安裝 Azure SDK for .NET 時發生問題]: #you-are-encountering-problems-installing-the-Windows-Azure-SDK
[您的儲存體服務有不同的問題]: #you-have-a-different-issue-with-a-storage-service

[附錄]: #appendices
[附錄1：使用 Fiddler 來捕捉 HTTP 和 HTTPS 流量]: #appendix-1
[附錄2：使用 Wireshark 來捕獲網路流量]: #appendix-2
[附錄3：使用 Microsoft Message Analyzer 來捕獲網路流量]: #appendix-3
[附錄4：使用 Excel 來查看計量和記錄資料]: #appendix-4
[附錄5：使用 Azure DevOps 的 Application Insights 進行監視]: #appendix-5
