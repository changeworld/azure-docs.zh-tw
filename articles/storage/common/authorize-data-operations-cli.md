...
用來讀取和寫入 blob 和佇列資料的 Azure CLI 命令包含`--auth-mode`選擇性參數。 指定此參數以指示要如何授權資料作業：
...
> [!IMPORTANT]
> 如果您省略`--auth-mode`參數，或將它設`key`為，則 Azure CLI 會嘗試使用帳戶存取金鑰進行授權。 在此情況下，Microsoft 建議您在命令或**AZURE_STORAGE_KEY**環境變數中提供存取金鑰。 如需有關環境變數的詳細資訊，請參閱標題為[授權參數設定環境變數](#set-environment-variables-for-authorization-parameters)一節。
>
...
下列範例顯示如何使用帳戶存取金鑰來建立容器。 指定帳戶金鑰，並提供具有下列`--auth-mode` `key`值的參數：
...
|    AZURE_STORAGE_AUTH_MODE            |    用來執行命令的授權模式。 允許的值`login`為（建議） `key`或。 如果您指定`login`，則 Azure CLI 會使用您的 Azure AD 認證來授權資料作業。 如果您指定舊版`key`模式，Azure CLI 會嘗試查詢帳戶存取金鑰，並使用金鑰來授權命令。    |
