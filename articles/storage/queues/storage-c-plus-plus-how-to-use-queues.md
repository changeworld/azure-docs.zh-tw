---
title: 如何使用佇列存儲 （C++） - Azure 存儲
description: 瞭解如何在 Azure 中使用佇列存儲服務。 範例是以 C++ 撰寫的。
...
* **Linux：** 按照[Azure 存儲用戶端庫中的說明操作，瞭解 C++ README：開始在 Linux](https://github.com/Azure/azure-storage-cpp#getting-started-on-linux)頁上。
* **視窗：** 在 Windows 上，使用[vcpkg](https://github.com/microsoft/vcpkg)作為依賴項管理器。 請依照[快速入門](https://github.com/microsoft/vcpkg#quick-start)來初始化 vcpkg。 然後，使用下列命令安裝二進位檔：

```powershell
.\vcpkg.exe install azure-storage-cpp
```

您可以在[README](https://github.com/Azure/azure-storage-cpp#download--install)檔中找到有關如何構建原始程式碼並匯出到 NuGet 的指南。
...
Azure 儲存體用戶端會使用儲存體連接字串來儲存存取資料管理服務時所用的端點與認證。 在用戶端應用程式中執行時，您必須以下列格式提供儲存體連接字串 (其中的 *AccountName* 和 *AccountKey* 值要使用您儲存體帳戶的名稱，以及在 [Azure 入口網站](https://portal.azure.com)中針對該儲存體帳戶而列出的儲存體存取金鑰)。 有關存儲帳戶和訪問金鑰的資訊，請參閱[有關 Azure 存儲帳戶](../common/storage-create-storage-account.md?toc=%2fazure%2fstorage%2fqueues%2ftoc.json)。 本範例將示範如何宣告靜態欄位來存放連接字串：  
...
**cloud_queue_client**物件允許您獲取佇列的引用物件。 以下代碼創建一個**cloud_queue_client**物件。
...
使用**cloud_queue_client**物件獲取對要使用的佇列的引用。 如果佇列不存在，您可以建立佇列。
...
若要將消息插入到現有佇列中，請先創建新**cloud_queue_message**。 接下來，調用**add_message**方法。 **cloud_queue_message** 便可以從字串或 **byte** 陣列建立。 以下是建立佇列 (如果佇列不存在) 並插入訊息 'Hello, World' 的程式碼：
...
您可以通過調用**peek_message**方法查看佇列前面的消息，而無需將其從佇列中刪除。
...
您的程式碼可以使用兩個步驟將訊息自佇列中清除佇列。 當您調用**get_message**時，您會收到佇列中的下一條消息。 從**get_message**返回的消息將對此佇列中讀取消息的任何其他代碼不可見。 要完成從佇列中刪除消息，還必須調用**delete_message**。 這個移除訊息的兩步驟程序可確保您的程式碼因為硬體或軟體故障而無法處理訊息時，另一個程式碼的執行個體可以取得相同訊息並再試一次。 代碼調用**delete_message**消息處理後。
...
自訂從佇列中擷取訊息的方法有兩種。 首先，您可以取得一批訊息 (最多 32 個)。 其次，您可以設定較長或較短的可見度逾時，讓您的程式碼有較長或較短的時間可以完全處理每個訊息。 以下代碼示例使用**get_messages**方法在一次調用中獲取 20 條消息。 然後，它使用**for**迴圈處理每條消息。 它也會將可見度逾時設定為每個訊息五分鐘。 請注意，系統會針對所有訊息同時開始計時 5 分鐘，所以從呼叫 **get_messages** 開始的 5 分鐘後，任何尚未刪除的訊息都會重新出現。
...
* [如何使用C++表存儲](../../cosmos-db/table-storage-how-to-use-c-plus.md)
* [在C++中列出 Azure 存儲資源](../common/storage-c-plus-plus-enumeration.md?toc=%2fazure%2fstorage%2fqueues%2ftoc.json)
* [存儲用戶端庫，用於C++參考](https://azure.github.io/azure-storage-cpp)
