...
帳戶名稱、表名稱和**分區金鑰**一起標識表服務存儲實體的存儲服務中的分區。 分割也同時是實體的定址配置的一部分，可定義交易的範圍 (請參閱下方的 [實體群組交易](#entity-group-transactions) )，並打造表格服務調整方式的基礎。 有關分區的詳細資訊，請參閱[表存儲的性能和可伸縮性檢查表](storage-performance-checklist.md)。  
...
表格儲存體比較便宜，但您在評估任何表格服務方案時，均應將容量使用量和交易數目的成本預估同時納入考量。 不過，在許多情況下，儲存反正規化或重複的資料以改善方案的效能或延展性，也是有效措施。 有關定價的詳細資訊，請參閱[Azure 存儲定價](https://azure.microsoft.com/pricing/details/storage/)。  
...
- [加密表資料](table-storage-design-encrypt-data.md)
