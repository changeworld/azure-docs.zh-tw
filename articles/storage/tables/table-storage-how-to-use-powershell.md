...
description: 瞭解如何使用 PowerShell 運行常見任務,如創建、查詢、從 Azure 表存儲帳戶中刪除數據。
author: roygara
ms.service: storage
ms.topic: article
ms.date: 04/05/2019
...
這些範例需要 Az PowerShell 模組`Az.Storage (1.1.0 or greater)`與`Az.Resources (1.2.0 or greater)`。 在 PowerShell 視窗中，執行 `Get-Module -ListAvailable Az*` 以尋找版本。 如果未顯示任何項目，或者您必須升級，請參閱[安裝 Azure PowerShell 模組](/powershell/azure/install-az-ps)。
...
安裝或更新 Azure PowerShell 後,必須安裝具有管理實體的指令的模組**AzTable**。 若要安裝此模組，請以系統管理員身分執行 PowerShell，並使用 **Install-Module** 命令。

> [!IMPORTANT]
> 出於模組名稱相容性的原因,我們仍在 PowerShell 庫中的`AzureRmStorageTables`舊名稱 下發佈此同一模組。 本文檔將僅引用新名稱。
...
要建立表,請使用新[AzStorageTable](/powershell/module/az.storage/New-AzStorageTable) cmdlet。 在此範例中，資料表名為 `pshtesttable`。
...
> [!IMPORTANT]
> 使用 AzTable PowerShell 模組時,必須使用**CloudTable。** 調用**Get-AzStorageTable**命令以獲取對此物件的引用。 如果表不存在,則此命令也會創建該表。
...
* [使用 PowerShell 中的 Azure 表 - AzureRm 儲存表/AzTable PS 模組 v2.0](https://paulomarquesc.github.io/working-with-azure-storage-tables-from-powershell)
