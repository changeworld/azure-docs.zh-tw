...
在本快速入門中，您會使用原生 Docker CLI 命令來部署 Docker 容器，並讓其應用程式可在 Azure 容器執行個體中使用。 這項功能是透過 [Docker 與 Azure 之間的整合](https://docs.docker.com/engine/context/aci-integration/) 搶鮮版 (Beta) 所啟用。 執行 `docker run` 命令的幾秒之後，您可以瀏覽至容器中執行的應用程式：

:::image type="content" source="media/container-instances-quickstart-docker-cli/view-application-running-in-an-azure-container-instance.png" alt-text="在瀏覽器中檢視使用 Azure Container Instances 所部署的應用程式":::

如果您沒有 Azure 訂用帳戶，請在開始前建立[免費帳戶][azure-account]。

針對本快速入門，您必須安裝適用於 [Windows](https://desktop.docker.com/win/edge/Docker%20Desktop%20Installer.exe) 或 [macOS](https://desktop.docker.com/mac/edge/Docker.dmg) 的 Docker Desktop Edge 2.3.2.0 版或更新版本。 或安裝[適用於 Linux 的 Docker ACI 整合 CLI](https://docs.docker.com/engine/context/aci-integration/#install-the-docker-aci-integration-cli-on-linux) 搶鮮版 (Beta)。 

> [!IMPORTANT]
> 這項功能目前為預覽狀態，需要 Docker 中的搶鮮版 (Beta) (預覽) 功能。 深入了解 [Docker Desktop 的穩定和 Edge 版本](https://docs.docker.com/desktop/#stable-and-edge-versions)。 並非所有 Azure 容器執行個體的功能都受到支援。 透過在 [aci-integration-beta](https://github.com/docker/aci-integration-beta) GitHub 存放庫中建立問題，提供有關 Docker-Azure 整合的意見反應。
...
執行 `docker context create aci` 以建立 ACI 內容。 此內容會將 Docker 與您的 Azure 訂用帳戶建立關聯，讓您可以建立容器執行個體。 例如，若要建立名為「myacicontext」的內容：
...
如果您想要建置容器映像，並從私人的 Azure 容器登錄進行部署，請繼續進行 Azure Container Instances 教學課程。
