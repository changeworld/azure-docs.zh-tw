---
title: 在容器群組中啟用管理識別
description: 瞭解如何在 Azure 容器實例中啟用託管標識,該識別可以與其他 Azure 服務進行身份驗證
ms.topic: article
ms.date: 01/29/2020
...
> * 授予對 Azure 金鑰保存庫的識別權限
> * 使用託管識別從正在執行的容器存取金鑰保管庫
...
您可以在執行中的容器使用受控身分識別，向任何[支援 Azure AD 驗證的服務](../active-directory/managed-identities-azure-resources/services-support-managed-identities.md#azure-services-that-support-azure-ad-authentication)進行驗證，而無須在您的容器程式碼中管理認證。 對於不支援 AD 身份驗證的服務,可以在 Azure 密鑰保管庫中儲存機密,並使用託管標識訪問金鑰保管庫以檢索認證。 如需有關使用受控識別的詳細資訊，請參閱[什麼是適用於 Azure 資源的受控識別？](../active-directory/managed-identities-azure-resources/overview.md)
...
### <a name="enable-a-managed-identity"></a>啟用受控識別
 ...
 在「Azure 容器執行個體」中，從 REST API 2018-10-01 版及對應的 SDK 和工具開始，便支援適用於 Azure 資源的受控識別。 當您建立容器群組時，可藉由設定 [ContainerGroupIdentity](/rest/api/container-instances/containergroups/createorupdate#containergroupidentity) 屬性來啟用一或多個受控識別。 您還可以在容器組執行後啟用或更新託管標識 - 任一操作都會導致容器組重新啟動。 若要在新的或現有的容器群組上設定身分識別，請使用 Azure CLI、Resource Manager 範本或 YAML 檔案。 
...
要使用託管標識,最初必須授予標識對訂閱中的一個或多個 Azure 服務資源(如 Web 應用、密鑰保管庫或存儲帳戶)的訪問許可權。 若要從執行中的容器存取 Azure 資源，您的程式碼必須向 Azure AD 端點取得「存取權杖」**。 接著，您的程式碼會藉由呼叫將存取權杖傳送給支援 Azure AD 驗證的服務。 
...
本文中的範例使用 Azure 容器實例中的託管標識來造訪 Azure 密鑰保管庫密鑰。 
...
使用[az 金鑰保管庫建立](/cli/azure/keyvault?view=azure-cli-latest#az-keyvault-create)命令建立金鑰保管庫。 請確保指定唯一的密鑰保管庫名稱。 
...
使用[az 金鑰函式庫金鑰集](/cli/azure/keyvault/secret?view=azure-cli-latest#az-keyvault-secret-set)命令在金鑰保管庫中儲存範例機密:
...
繼續使用以下範例,使用 Azure 容器實例中的使用者分配或系統分配的託管標識存取金鑰保管庫。

## <a name="example-1-use-a-user-assigned-identity-to-access-azure-key-vault"></a>範例 1:使用使用者配置的識別存取 Azure 金鑰保管庫

### <a name="create-an-identity"></a>建立身分識別

首先，使用 [az identity create](/cli/azure/identity?view=azure-cli-latest#az-identity-create) 命令，在您的訂用帳戶中建立身分識別。 可以使用用於創建金鑰保管庫的相同資源組,或使用其他資源組。
...
運行以下[az 容器建立](/cli/azure/container?view=azure-cli-latest#az-container-create)命令,以`azure-cli`基於 Microsoft 映像創建容器實例。 此範例提供單容器組,您可以使用該組以互動方式執行 Azure CLI 以存取其他 Azure 服務。 在本節中,僅使用基本 Ubuntu 作業系統。 

`--assign-identity` 參數會將使用者指派的受控識別傳遞給群組。 長時間執行的命令會讓容器保持執行。 此示例使用用於創建密鑰保管庫的相同資源組,但可以指定其他資源組。
...
### <a name="grant-user-assigned-identity-access-to-the-key-vault"></a>給使用者配置的配對金鑰保存的識別權限

運行以下[az 金鑰庫集策略](/cli/azure/keyvault?view=azure-cli-latest)命令,以在金鑰保管庫上設置存取策略。 以下範例允許使用者分配的識別從金鑰保管庫取得機密:
...
### <a name="use-user-assigned-identity-to-get-secret-from-key-vault"></a>使用使用者配置的身份從金鑰保管庫取得機密

現在,您可以使用正在運行的容器實例中的託管標識訪問密鑰保管庫。 首先在容器中啟動 bash 外殼:
...
在容器的 Bash 殼層中執行下列命令。 要取得存取權杖,請使用 Azure 的目錄配對金鑰保存的權限, 請執行以下指令:
...
現在使用訪問權杖對密鑰保管庫進行身份驗證並讀取機密。 請務必在網址中取代金鑰保管庫的名稱 *(Htt:\//mykeyvault.vault.azure.net/...*
...
## <a name="example-2-use-a-system-assigned-identity-to-access-azure-key-vault"></a>範例 2:使用系統分配的識別存取 Azure 金鑰保管庫

### <a name="enable-a-system-assigned-identity-on-a-container-group"></a>在容器群祖上啟用系統指派的身分識別

運行以下[az 容器建立](/cli/azure/container?view=azure-cli-latest#az-container-create)命令,以`azure-cli`基於 Microsoft 映像創建容器實例。 此範例提供單容器組,您可以使用該組以互動方式執行 Azure CLI 以存取其他 Azure 服務。 

不含任何其他值的 `--assign-identity` 參數可在群組上啟用系統指派的受控識別。 標識的範圍限定為容器組的資源組。 長時間執行的命令會讓容器保持執行。 此示例使用用於創建密鑰保管庫的相同資源組,但可以指定其他資源組。
...
### <a name="grant-container-group-access-to-the-key-vault"></a>給容器群組配對金鑰保存庫的存取

運行以下[az 金鑰庫集策略](/cli/azure/keyvault?view=azure-cli-latest)命令,以在金鑰保管庫上設置存取策略。 以下範例允許系統託管識別從金鑰保管庫取得機密:
...
### <a name="use-container-group-identity-to-get-secret-from-key-vault"></a>使用容器組識別從金鑰保存庫取得機密

現在,您可以使用託管標識訪問正在運行的容器實例中的密鑰保管庫。 首先在容器中啟動 bash 外殼:
...
在容器的 Bash 殼層中執行下列命令。 首次使用託管識別登錄到 Azure CLI:

```bash
az login --identity
```

從正在執行的容器中,從金鑰保管庫檢索機密:
...
指定的`apiVersion`最小`2018-10-01`值。
...
* 請參考使用託管識別從 Azure 容器實體存取金鑰保管庫的[Azure Go SDK 範例](https://medium.com/@samkreter/c98911206328)。
