...
了解如何在 Azure 容器執行個體中掛接 *emptyDir* 磁碟區，以在容器群組中的容器之間共用資料。 使用 *emptyDir* 磁片區做為容器化工作負載的暫時快取。

> [!NOTE]
> 目前只有 Linux 容器才能掛接 *emptyDir* 磁碟區。 當我們致力於將所有功能帶入 Windows 容器時，您可以在 [總覽](container-instances-overview.md#linux-and-windows-containers)中找到目前的平臺差異。
...
*emptyDir* 磁碟區中的資料會透過容器損毀保存。 不過，重新啟動的容器不保證會保存 *emptyDir* 磁碟區中的資料。 如果您停止容器群組，則不會保存 *emptyDir* 磁片區。
...
若要查看容器群組部署的範例，請參閱 [使用 Resource Manager 範本部署多容器群組](container-instances-multi-container-group.md) ，並 [使用 YAML 檔案部署多容器群組](container-instances-multi-container-yaml.md)。
