...
1. [ **資料收集異常** ] 索引標籤可協助您偵測資料收集程式中的異常（依資料表和資料來源）。 每個索引標籤都會顯示特定資料表的異常 ([ **一般** ] 索引標籤包含) 的資料表集合。 這些異常是使用傳回 **異常分數** 的 **Series_decompose_anomalies ( # B1** 函數來計算。 [深入了解此函式](/azure/data-explorer/kusto/query/series-decompose-anomaliesfunction?WT.mc_id=Portal-fx)。 針對要評估的函式設定下列參數：

    - **AnomaliesTimeRange**：此時間選擇器僅適用於資料收集異常檢視。
    - **SampleInterval**：在指定的時間範圍內，取樣資料的時間間隔。 異常分數只會在最後一個間隔的資料上進行計算。
    - **PositiveAlertThreshold**：這個值會定義正數異常分數閾值。 它接受十進位值。
    - **NegativeAlertThreshold**：這個值會定義負數異常分數閾值。 它接受十進位值。
