...
2. 流覽至**Azure Sentinel** > **Configuration** > 設定**分析**
...
> [!NOTE]
> Azure Sentinel 目前使用30天的歷程記錄資料來訓練機器學習系統。 當此資料通過機器學習管線時，一律會使用 Microsoft 的金鑰進行加密。 不過，如果您在 Azure Sentinel 工作區中啟用 CMK，則不會使用[客戶管理的金鑰（CMK）](customer-managed-keys.md)來加密定型資料。 若要選擇不融合，請流覽至**Azure Sentinel** \> **Configuration** \>設定分析**Status** **Disable.**  ** \>使用\>中規則] [Advanced 多階段攻擊偵測**]，然後在 [狀態] 欄中選取 [停用]。
...
在接下來的描述中，Azure Sentinel 會顯示資料中的實際值，此頁面是以括弧括住的變數。 例如，帳戶的實際顯示名稱，而不是\<*帳戶名稱*>，而不\<是>*數位*。
...
- **來自不熟悉的位置的登入事件，因而導致 Exchange Online 信箱外泄**
    ...
    此警示表示依\<*帳戶名稱*的登入事件> 從\<*位置*>、不熟悉的位置，以及在使用者的收件匣上設定可疑的收件匣轉送規則。
    
    這可能表示帳戶遭到入侵，而且信箱正用來竊取組織中的資訊。 使用者\<*帳戶名稱*> 建立或更新將所有內送電子郵件轉寄至外部地址\<*電子郵件地址*> 的收件匣轉送規則。 

- **從不熟悉的位置登入事件，因而導致可疑的雲端應用程式系統管理活動**
    
    此警示表示依\<*帳戶名稱*> 從\<*位置*> 的登入事件，這是不熟悉的位置。 
...
- **來自採用 Office 365 信箱外泄之匿名 IP 位址的登入事件**
    ...
    此警示表示依\<*帳戶名稱*的登入事件> 從匿名 proxy ip 位址\<的*ip 位址*>，隨後會在使用者的收件匣上設定可疑的收件匣轉送規則。
    
    這可能表示帳戶遭到入侵，而且信箱正用來竊取組織中的資訊。 使用者\<*帳戶名稱*> 建立或更新將所有內送電子郵件轉寄至外部地址\<*電子郵件地址*> 的收件匣轉送規則。 

- **來自匿名 IP 位址的登入事件，導致可疑的雲端應用程式系統管理活動**
    
    此警示表示依\<*帳戶名稱*> 從匿名 proxy ip 位址\< *ip 位址*> 的登入事件。 
