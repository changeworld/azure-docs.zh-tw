...
如果已設定 [Azure 資訊保護的集中式報告](/azure/information-protection/reports-aip) ，這項服務的記錄資訊會儲存在與您目前為 Azure Sentinel 所選取相同的 Log Analytics 工作區中，您可以略過此資料連線器的設定。 Azure 資訊保護的記錄資訊已可供 Azure Sentinel。
...
- Azure 資訊保護已新增至 Azure 入口網站。 如果您需要此步驟的說明，請參閱 [將 Azure 資訊保護新增至 Azure 入口網站](/azure/information-protection/quickstart-viewpolicy#add-azure-information-protection-to-the-azure-portal)。
...
6. Azure 資訊保護中的報告資料會儲存在所選工作區的 **InformationProtectionLogs_CL** 資料表中。 
    ...
    若要在此報表資料的 Azure 監視器中使用相關的架構，請搜尋 **>informationprotectionevents**。 如需有關這些事件函數的詳細資訊，請參閱 Azure 資訊保護檔中的事件函式的 [易記架構參考](/azure/information-protection/reports-aip#friendly-schema-reference-for-event-functions) 一節。
