...
| **MAC 位址** | 字串 | Colon-Hexadecimal 標記法 |
...
| **使用者** | 字串 | 可用的3個使用者欄位如下：<ul><li>[使用者名稱]</li><li>使用者 UPN</li><li>使用者網域</li></ul> |
| **使用者識別碼** | 字串 | 目前支援下列2個使用者識別碼：<ul><li>使用者 SID</li><li>Azure Active directory 識別碼</li></ul> |
| **裝置** | 字串 | 支援下列3個裝置/主機資料行：<ul><li>ID</li><li>名稱</li><li>完整網域名稱 (FQDN)</li></ul> |
| **國家/地區** | 字串 | 根據此優先順序使用 ISO 3166-1 的字串：<ul><li>Alpha-2 代碼 (也就是美國) </li><li>美國的美式程式碼 (即美國) </li><li>簡短名稱</li></ul> |
| **區域** | 字串 | 使用 ISO 3166-2 的國家/地區子部門名稱 |
| **城市** | 字串 | |
| **經度** | Double | ISO 6709 座標標記法 (帶正負號的十進位)  |
| **緯度** | Double | ISO 6709 座標標記法 (帶正負號的十進位)  |
| **雜湊演算法** | 字串 | 支援下列4個雜湊資料行：<ul><li>MD5</li><li>SHA1</li><li>SHA256</li><li>SHA512</li></ul> |
| **檔案類型** | 字串 | 檔案類型的類型：<ul><li>分機</li><li>類別</li><li>NamedType</li></ul> |
| 
...
|-|-|-|-|-|
| EventType | 字串 | 交通流量 | 要收集的事件種類 | 事件 |
| EventSubType | 字串 | 驗證 | 類型的其他描述（如果適用） | 事件 |
...
| DvcMacAddr | 字串 | 06：10：9f： eb：8f：14 | 傳送事件的來源報告裝置之網路介面的 MAC 位址。 | 裝置、<br>Mac |
| DvcHostname | 裝置名稱 (字串)  | syslogserver1.contoso.com | 產生訊息之裝置的裝置名稱。 | 裝置 |
| EventProduct | 字串 | OfficeSharepoint | 產生事件的產品。 | 事件 |
...
| EventReportUrl | 字串 | https://192.168.1.1/repoerts/ae3-56.htm | 報表裝置所建立的完整報表連結 | 事件 |
| EventVendor | 字串 | Microsoft | 產生事件的產品廠商。 | 事件 |
| EventResult | 多重值： Success、Partial、失敗、[Empty] (字串)  | 成功 | 針對活動報告的結果。 如果不適用則為空白值。 | 事件 |
| EventResultDetails | 字串 | 錯誤的密碼 | EventResult 中回報之結果的原因或詳細資料 | 事件 |
| EventSchemaVersion | Real | 0.1 | Azure Sentinel 架構版本。 目前為0.1。 | 事件 |
| EventSeverity | 字串 | 低 | 如果回報的活動有安全性影響，則表示影響的嚴重性。 | 事件 |
| EventOriginalUid | 字串 | af6ae8fe-ff43-4a4c-b537-8635976a2b51 | 來自報表裝置的記錄識別碼。 | 事件 |
...
| NetworkApplicationProtocol | 字串 | HTTPS | 連接或會話所使用的應用層通訊協定。 | 網路 |
...
| DstGeoCity | 字串 | 伯 靈 頓 | 與目的地 IP 位址相關聯的城市 | 目的地<br>地理區域 |
...
| FileHashMd5 | 字串 | 51BC68715FC7C109DCEA406B42D9D78F | 透過網路連線傳輸通訊協定的 MD5 雜湊值。 | 檔案 |
