...
> [!NOTE]
> 單一工作區內容中的安全性事件集合可以從 Azure Defender (（先前 Azure 資訊安全中心) 或 Azure Sentinel 進行設定，但不能同時設定。 如果您要在已執行 Azure Defender 的工作區上架 Azure Sentinel，且設定為收集安全性事件，您有兩個選項：
> - 讓 Azure Defender 中的安全性事件集合保持原狀。 您將能夠在 Azure Sentinel 以及 Azure Defender 中查詢及分析這些事件。 不過，您將無法在 Azure Sentinel 中監視連接器的線上狀態或變更其設定。 如果這對您很重要，請考慮第二個選項。
>
> - 停用 Azure Defender 中的[安全性事件集合](../security-center/security-center-enable-data-collection.md)，然後只在 Azure Sentinel 中新增「安全性事件」連接器。 在第一個選項中，您可以查詢和分析 Azure Sentinel 和 Azure Defender 中的事件，但現在您將能夠監視連接器的線上狀態，或只在 Azure Sentinel 中變更其設定。
