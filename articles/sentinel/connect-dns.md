...
description: 瞭解如何在 DNS 電腦上安裝代理程式，以將在 Windows 上執行的任何功能變數名稱伺服器 (DNS) 連接至 Azure Sentinel。
services: sentinel
...
您可以將在 Windows 上執行的任何功能變數名稱伺服器 (DNS) 連接到 Azure Sentinel。 這是藉由在 DNS 電腦上安裝代理程式來完成。 您可以使用 DNS 記錄檔，藉由收集、分析及關聯分析和審核記錄，以及來自 DNS 伺服器的其他相關資料，來為您組織的 DNS 基礎結構進行安全性、效能和操作相關的深入解析。
