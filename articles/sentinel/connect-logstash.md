...
| `plugin_flush_interval` | number | 選擇性欄位。 設定以定義訊息傳輸到 Log Analytics 之間的最大間隔 (以秒為單位) 。 預設值為 5。 |
    | `amount_resizing` | boolean | True 或 False。 啟用或停用自動調整機制，這會根據所接收的記錄資料量來調整訊息緩衝區大小。 |
| `max_items` | number | 選擇性欄位。 只有在 `amount_resizing` 設為 "false" 時才適用。 用來設定訊息緩衝區大小的上限， (記錄) 。 預設值為 2000。  |
