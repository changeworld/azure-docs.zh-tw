...
> [!IMPORTANT]
> 當 SFTP 連接器被棄用時，請使用[SFTP-SSH 連接器](../connectors/connectors-sftp-ssh.md)。 您無法在邏輯應用設計器中選擇 SFTP 觸發器和操作。
...
SFTP 連接器僅處理*50 MB 或更小*且不支援[消息分塊](../logic-apps/logic-apps-handle-large-messages.md)的檔。 對於較大的檔，請使用[SFTP-SSH 連接器](../connectors/connectors-sftp-ssh.md)。 有關 SFTP 連接器和 SFTP-SSH 連接器之間的差異，請查看[SFTP-SSH 與 SFTP 文章中的 SFTP-SSH 與 SFTP 的比較](../connectors/connectors-sftp-ssh.md#comparison)。
...
## <a name="how-sftp-triggers-work"></a>SFTP 觸發器的工作原理

SFTP 通過輪詢 SFTP 檔案系統並查找自上次輪詢以來更改的任何檔來觸發工作。 某些工具可讓您在檔案變更時保留時間戳記。 在這些情況下，您必須停用此功能，以便讓您的觸發程序可以運作。 以下是一些常見的設定：
