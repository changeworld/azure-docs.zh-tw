...
description: 在 Azure Logic Apps 中建立自動化工作流程，以上傳及管理 OneDrive 中的檔案
services: logic-apps
...
---
# <a name="access-and-manage-files-in-onedrive-connector-by-using-azure-logic-apps"></a>使用 Azure Logic Apps 來存取和管理 OneDrive 連接器中的檔案

藉由使用 [Azure Logic Apps](../logic-apps/logic-apps-overview.md) 和 [OneDrive 連接器](/connectors/onedriveconnector/)，您可以建立自動化的工作和工作流程來管理您的檔案，包括上傳、取得、刪除檔案等等。 使用 OneDrive，您可以執行下列工作：
...
1. 在邏輯應用程式設計工具中，輸入 `onedrive` 以取得觸發程式的清單：  
   ...
   ![標題為「顯示 Microsoft 受控 A P I」的對話方塊有一個包含 "onedrive" 的方塊。 以下是四個觸發程式的清單。 其中第一個是「OneDrive-當檔案建立時」。 選取 [OneDrive-當檔案被修改時] 的第二個。](./media/connectors-create-api-onedrive/onedrive-1.png)

2. 選取 [當檔案遭到修改時]****。 如果連線已存在，則選取 [顯示選擇器] 按鈕以選取資料夾。

   ![標題為「當檔案被修改時」的對話方塊有一個標題為「資料夾」的方塊，並有相關聯的瀏覽按鈕。](./media/connectors-create-api-onedrive/sample-folder.png)
...
3. 選取 [編輯]**** 按鈕，然後設定 [頻率]**** 和 [間隔]**** 值。 例如，如果您希望觸發程序每隔 15 分鐘輪詢一次，則將 [頻率]**** 設定為 [分鐘]**** 並將 [間隔]**** 設定為 [15]****。 
   ...
   ![標題為「修改檔案時」的對話方塊會顯示五個方塊：「資料夾」、「頻率」、「間隔」、「時區」和「開始時間」。 [頻率] 和 [時區] 欄位有下拉式清單。](./media/connectors-create-api-onedrive/trigger-properties.png)
...
1. 選取加號。 您會看到幾個選擇︰[新增動作]****、[新增條件]**** 或其中一個 [其他]**** 選項。
   ...
   ![螢幕擷取畫面顯示四個按鈕： [+ 新增步驟]、[新增動作]、[新增條件] 和 [...]其他」。](./media/connectors-create-api-onedrive/add-action.png)

2. 選擇 [新增動作] ****。

3. 在搜尋方塊中，輸入 `onedrive` 以取得所有可用動作的清單。

   ![標題為「顯示 Microsoft 受控 A P I」的對話方塊有一個包含 "onedrive" 的方塊。 以下是八個動作的清單。 第一個是「OneDrive-建立檔案」，並已選取該檔案。](./media/connectors-create-api-onedrive/onedrive-actions.png) 

4. 在我們的範例中，選擇 [OneDrive - 建立檔案]****。 如果連線已存在，則選取 [資料夾路徑]**** 以供放置檔案、輸入 [檔案名稱]****，然後選擇想要的 [檔案內容]****︰  

   ![標題為 [建立檔案] 的對話方塊會顯示三個方塊，標示為 [資料夾路徑]、[檔案名] 和 [資料夾內容]。 [資料夾路徑] 方塊旁邊有一個瀏覽目錄按鈕。](./media/connectors-create-api-onedrive/sample-action.png)
   ...
   ![標題為 "For each" 的對話方塊有一個標示為「選取先前步驟的輸出」的方塊，其中包含「附件」。 其中有一個 [建立檔案] 對話方塊，其中包含 [For each] 方塊的其餘部分，並有標示為「資料夾路徑」、「檔案名」和「檔案內容」的方塊。 ](./media/connectors-create-api-onedrive/foreach-action.png)
