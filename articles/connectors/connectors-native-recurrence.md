...
1. 設定重複的間隔和頻率。 在此範例中，將這些屬性設定為每週執行工作流程。
   ...
   | **頻率** | `frequency` | 是 | 字串 | 重複的時間單位：**秒**、**分鐘**、**小時**、**天**、**週**或**月** |
   ||||||
...
1. 若要設定 advanced 排程選項，請開啟 [**加入新的參數**] 清單。 您選取的任何選項都會出現在選取範圍後的觸發程式上。
   ...
   | **在這幾分鐘內** | `minutes` | 否 | 整數或整數陣列 | 如果您選取 [天] 或 [週]，可以選取從 0 到 59 的一或多個整數，來表示想要在小時的哪幾個分鐘執行工作流程。 <p>例如，您可以指定 "30" 作為分鐘標記，然後使用上個範例代表一天中的整點，這樣就會得出上午 10:30、下午 12:30 及下午 2:30。 <p>**注意**：有時候，觸發的執行時間戳記可能會從排定的時間內最多1分鐘。 如果您需要將時間戳記完全依照排程傳遞給後續的動作，您可以使用範本運算式來據以變更時間戳記。 如需詳細資訊，請參閱[運算式的日期和時間函數](../logic-apps/workflow-definition-language-functions-reference.md#date-time-functions)。 |
   |||||
