...
當您開始使用 Azure 服務匯流排命名空間時，您可能會想要監視您的命名空間建立、刪除或存取的方式和時間。 本文概述所有可用的操作和診斷記錄。

Azure 服務匯流排目前支援活動和作業記錄，可捕獲 Azure 服務匯流排命名空間上執行的 *管理作業* 。 具體而言，這些記錄會擷取作業類型，包括建立佇列、使用的資源，以及作業狀態。

## <a name="operational-logs-schema"></a>作業記錄結構描述

所有記錄都以 JavaScript 物件標記法 (JSON) 格式儲存在下列兩個位置：

- **AzureActivity**：顯示作業的記錄，以及針對您的命名空間在 Azure 入口網站或透過 Azure Resource Manager 範本部署進行的動作。
- **AzureDiagnostics**：使用 API 或透過語言 SDK 上的管理用戶端，顯示針對您的命名空間進行的作業和動作記錄。
...
作業記錄會捕捉在 Azure 服務匯流排命名空間上執行的所有管理作業。 因為在 Azure 服務匯流排上進行大量的資料作業，所以不會捕捉資料作業。

> [!NOTE]
> 為協助您更妥善地追蹤資料作業，我們建議使用用戶端追蹤。
...
| 佇列 | <ul> <li> 建立佇列</li> <li> 更新佇列</li> <li> 刪除佇列 </li> <li> 閒置刪除佇列 </li> </ul> | 
| 主題 | <ul> <li> 建立主題 </li> <li> 更新主題 </li> <li> 刪除主題 </li> <li> 閒置刪除主題 </li> </ul> |
| 訂用帳戶 | <ul> <li> 建立訂閱 </li> <li> 更新訂閱 </li> <li> 刪除訂閱 </li> <li> 閒置刪除訂用帳戶 </li> </ul> |
...
1. 在 [Azure 入口網站](https://portal.azure.com)中，移至您的 Azure 服務匯流排命名空間，然後在 [ **監視**] 底下選取 [  **診斷設定**]。
...
1. 執行下列動作來設定診斷設定：
   ...
   b. 針對您的診斷記錄，選取下列三個目的地的其中一個：  
   - 如果您選取 [封存 **至儲存體帳戶**]，就必須設定將儲存診斷記錄的儲存體帳戶。  
   - 如果您選取 **串流至事件中樞**，您必須設定要將診斷記錄串流至其中的事件中樞。
   - 如果您選取 [ **傳送至 Log analytics**]，則需要指定診斷將會傳送至哪一個 log analytics 實例。  
...
新的設定會在大約10分鐘內生效。 記錄檔會顯示在設定的封存目標的 [ **診斷記錄** ] 窗格中。
