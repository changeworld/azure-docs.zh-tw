...
3. 在「主要」命名空間與「次要」命名空間之間建立配對以取得***別名***。
    ...
    >[!NOTE] 
    > 如果已[將 Azure 服務匯流排標準命名空間遷移到 Azure 服務匯流排高級](service-bus-migrate-standard-premium.md)版，則必須使用預先存在的別名（即服務匯流排標準命名空間連接字串）通過**PS/CLI**或**REST API**創建災害復原配置。
    >
    >
    > 這是因為，在遷移期間，Azure 服務匯流排標準命名空間連接字串/DNS 名稱本身將成為 Azure 服務匯流排高級命名空間的別名。
    >
    > 用戶端應用程式必須使用此別名（即 Azure 服務匯流排標準命名空間連接字串）連接到已設置災害復原配對的高級命名空間。
    >
    > 如果使用門戶設置災害復原配置，則門戶將從您中抽象出此警告。
