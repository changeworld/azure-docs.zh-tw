...
ms.locfileid: "77660660"
---
# <a name="message-sessions"></a>訊息工作階段
Microsoft Azure 服務匯流排工作階段能夠聯合和依序處理未繫結的相關訊息序列。 會話可用於先出、先出 （FIFO） 和請求-回應模式。 本文演示如何在使用服務匯流排時使用會話來實現這些模式。 

## <a name="first-in-first-out-fifo-pattern"></a>先出先出 （FIFO） 模式
要在服務匯流排中實現 FIFO 保證，請使用會話。 服務匯流排並不規定消息之間的關係的性質，也沒有定義用於確定消息序列開始或結束位置的特定模型。

> [!NOTE]
> 服務匯流排的基本層不支援會話。 標準層和高階層則支援工作階段。 有關這些層之間的差異，請參閱[服務匯流排定價](https://azure.microsoft.com/pricing/details/service-bus/)。
...
不過，一般而言，應用程式會對一組相關訊息開頭與結尾位置具有清楚的概念。 服務匯流排不設置任何特定規則。
...
> [!NOTE]
> 在佇列或訂閱上啟用會話時，用戶端應用程式***無法再***發送/接收常規消息。 所有消息都必須作為會話的一部分發送（通過設置會話 ID）並通過接收會話接收。

適用於工作階段的 API 會存在於佇列和訂用帳戶用戶端上。 有一個命令模型，用於控制何時接收會話和消息，以及一個基於處理常式的模型（類似于*OnMessage）* 隱藏管理接收迴圈的複雜性。
...
當多個並行接收者從佇列提取時，就會將屬於特殊工作階段的訊息分派給目前保有該工作階段之鎖定的特定接收者。 通過此操作，一個佇列或訂閱中的交錯消息流可以乾淨地除以多工到不同的接收器，並且這些接收器也可以駐留在不同的用戶端電腦上，因為鎖管理發生在服務匯流排內的服務端。

上圖顯示三個並行工作階段接收者。 帶有 `SessionId` = 4 的工作階段沒有任何作用中的所有權用戶端，這表示沒有訊息從此特定工作階段傳遞。 工作階段的行為在許多方面類似子佇列。

工作階段接收者所保留的工作階段鎖定是對於「查看鎖定」** 安置模式所使用之訊息鎖定的保護傘。 接收方不能同時"在飛行中"有兩條消息，但必須按連續處理消息。 只有在已完成先前的訊息或使其成為失效信件時才能取得新訊息。 放棄訊息會導致下一個接收作業再次提供相同的訊息。

### <a name="message-session-state"></a>訊息工作階段狀態

在大規模高可用性雲系統中處理工作流時，與特定會話關聯的工作流處理常式必須能夠從意外故障中恢復，並可以從工作開始的地方。
...
會話狀態保持，只要它未清除（返回**null），** 即使會話中的所有消息都已使用。
...
| 會話被接受，消息完成，然後會話被顯式關閉 | 不適用（這是標準流）。 此處從會話中刪除消息） |

## <a name="request-response-pattern"></a>請求-回應模式
[請求-回復模式](https://www.enterpriseintegrationpatterns.com/patterns/messaging/RequestReply.html)是一種成熟的整合模式，它使發送方應用程式能夠發送請求，並為接收方提供正確發送回應的方法。 此模式通常需要短時間佇列或主題，應用程式才能向其發送回應。 在這種情況下，會話提供了具有類似語義的簡單替代解決方案。 

多個應用程式可以將其要求傳送到單個請求佇列，並設置特定的標頭參數以唯一標識寄件者應用程式。 接收方應用程式可以處理佇列中的請求，並在啟用會話的佇列上發送答覆，將會話 ID 設置為寄件者在請求消息上發送的唯一識別碼。 然後，發送請求的應用程式可以接收特定會話 ID 上的消息並正確處理答覆。

> [!NOTE]
> 發送初始請求的應用程式應瞭解會話 ID，並用於`SessionClient.AcceptMessageSession(SessionID)`鎖定它期望回應的會話。 最好使用將應用程式實例唯一標識為會話 ID 的 GUID。不應在會話處理常式或`AcceptMessageSession(timeout)`佇列中設置任何會話處理常式，以確保回應可供特定接收方鎖定和處理。

## <a name="next-steps"></a>後續步驟

- 有關使用 .NET 框架用戶端處理會話感知消息的示例，請參閱[Microsoft.Azure.ServiceBus 示例](https://github.com/Azure/azure-service-bus/tree/master/samples/DotNet/Microsoft.Azure.ServiceBus/Sessions)或[Microsoft.ServiceBus.消息示例](https://github.com/Azure/azure-service-bus/tree/master/samples/DotNet/Microsoft.ServiceBus.Messaging/Sessions)。 
