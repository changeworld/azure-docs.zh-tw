...
工作階段接收者所保留的工作階段鎖定是對於「查看鎖定」安置模式所使用之訊息鎖定的保護傘。 接收者不能同時「進行中」同時有兩個訊息「進行中」，但必須依序處理訊息。 只有在已完成先前的訊息或使其成為失效信件時才能取得新訊息。 放棄訊息會導致下一個接收作業再次提供相同的訊息。

### <a name="message-session-state"></a>訊息工作階段狀態

在高規模、高可用性的雲端系統中處理工作流程時，與特定會話相關聯的工作流程處理常式必須能夠從非預期的失敗中復原，而且可以在不同的進程或電腦上繼續執行部分完成的工作工作開始的位置。
...
> [!NOTE]
> 傳送初始要求的應用程式應該知道會話識別碼，並使用 `SessionClient.AcceptMessageSession(SessionID)` 來鎖定其預期回應的會話。 使用可唯一識別應用程式實例作為會話識別碼的 GUID 是個不錯的主意。佇列上不應該有會話處理常式或 `AcceptMessageSession(timeout)`，以確保回應可供特定接收者鎖定和處理。
