...
若要繼續使用服務匯流排、轉送和事件中樞，請于2021年10月31日前移至 Resource Manager。 我們鼓勵仍在使用舊 Api 的所有客戶立即切換，以利用 Resource Manager 的額外優點，其中包括資源群組、標籤、簡化的部署和管理程式，以及使用 Azure 角色型存取控制 (Azure RBAC) 的更精細存取控制。
