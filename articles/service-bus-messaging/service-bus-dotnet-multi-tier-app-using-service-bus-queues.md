...
![此圖顯示 Web 角色、服務匯流排和背景工作角色之間的通訊。][1]
...
* **負載平衡。** 當負載增加時，可以新增更多的背景工作程序來讀取佇列中的訊息。 每個訊息僅由其中一個背景工作程序處理。 此外，這個提取型負載平衡機制可讓背景工作電腦獲得最佳利用，即使背景工作電腦的處理能力有所不同也一樣，因為背景工作電腦將以自己的最大速率提取訊息。 此模式通常稱為 *競爭消費者* 模式。
  
  ![此圖顯示 Web 角色、服務匯流排和兩個背景工作角色之間的通訊。][2]
...
1. 使用系統管理員權限啟動 Visual Studio：在 **Visual Studio** 程式圖示上按一下滑鼠右鍵，然後按一下 [以系統管理員身分執行]。 本文稍後討論的 Azure 計算模擬器需要以系統管理員許可權啟動 Visual Studio。
...
5. 在 [新增 ASP.NET 專案] 對話方塊的 [選取範本] 清單中，按一下 [MVC]。
   ...
   ![Screenshotof [新增 ASP.NET 專案] 對話方塊，其中已反白顯示 MVC 並以紅色標示，並以紅色標示變更驗證選項。][12]
6. 還是在 [新增 ASP.NET 專案] 對話方塊中，按一下 [變更驗證] 按鈕。 在 [變更驗證] 對話方塊中，確定已選取 [不需要驗證]，然後按一下 [確定]。 在本教學課程中，您會部署不需要使用者登入的應用程式。
   
    ![[變更驗證] 對話方塊的螢幕擷取畫面，其中已選取 [無驗證] 選項，並以紅色外框。][16]
...
9. 按一下 [瀏覽] 索引標籤，然後搜尋 **WindowsAzure.ServiceBus**。 選取 **WindowsAzure.ServiceBus** 套件，按一下 [安裝]，然後接受使用規定。
   ...
   ![[管理 NuGet 套件] 對話方塊的螢幕擷取畫面，其中已醒目提示 [WindowsAzure]，並以紅色標示安裝選項。][13]
...
5. 現在，為先前建立的 `Submit()` 方法建立檢視。 在 `Submit()` 方法 (未採用任何參數的 `Submit()` 的多載) 內按一下滑鼠右鍵，然後選擇 [新增檢視]。
   ...
   ![將焦點放在 Submit 方法的程式碼螢幕擷取畫面，以及反白顯示 [新增視圖] 選項的下拉式清單。][14]
6. 隨即出現對話方塊，供您建立檢視。 在 [範本] 清單中，選擇 [建立]。 在 [模型類別] 清單中，選取 [OnlineOrder] 類別。
   
   ![[加入視圖] 對話方塊的螢幕擷取畫面，其中包含以紅色概述的 [範本] 和 [模型類別] 下拉式清單。][15]
7. 按一下 [新增] 。
...
10. 移除 [首頁]、[關於] 和 [連絡人] 連結。 刪除反白顯示的程式碼：
    ...
    ![醒目提示有三行 H T L L 動作連結程式代碼的程式碼螢幕擷取畫面。][28]
...
3. 按一下 [新增]，然後按一下 [新增背景工作角色專案]。 [新增新的角色專案] 對話方塊隨即出現。
   
   ![已反白顯示 [新增背景工作角色] 專案選項和 [新增] 選項的 [解決方案 Explorer] 窗格螢幕擷取畫面。][26]
4. 在 [新增新的角色專案] 對話方塊中，按一下 [具有服務匯流排佇列的背景工作角色]。
   
   ![[Ad 新增角色專案] 對話方塊的螢幕擷取畫面，其中已反白顯示 [具有服務匯流排佇列的背景工作角色] 選項，並以紅色框住。][23]
...
8. 在 [屬性] 對話方塊的 [設定] 索引標籤中，按一下 **Microsoft.ServiceBus.ConnectionString** 的 [值] 方塊內部，然後貼上您在步驟 6 中複製的端點值。
   ...
   ![[屬性] 對話方塊的螢幕擷取畫面，其中已選取 [設定] 索引標籤，並以紅色標示的 [Microsoft]。][25]
...
14. 您已完成應用程式。 您可以測試完整的應用程式，方法是以滑鼠右鍵按一下 [方案總管] 中的 MultiTierApp 專案、選取 [設定為啟始專案]，然後按下 F5。 請注意，訊息計數不會增加，因為背景工作角色會處理佇列中的項目，並將它們標示為完成。 您可以檢視 Azure 計算模擬器 UI，來查看背景工作角色的追蹤輸出。 作法為在工作列的通知區中的計算模擬器圖示上按一下滑鼠右鍵，並選取 [顯示計算模擬器 UI]。
    ...
    ![當您按一下模擬器圖示時會顯示的螢幕擷取畫面。 顯示計算模擬器 UI 位於選項清單中。][19]
    
    ![Microsoft Azure 計算模擬器 (快速) ] 對話方塊的螢幕擷取畫面。][20]
