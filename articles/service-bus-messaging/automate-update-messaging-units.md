...
1. 選取工具列上的 [ **儲存** ] 以儲存設定。 
    ...
    :::image type="content" source="./media/automate-update-messaging-units/manual-scale.png" alt-text="手動調整訊息單位":::       
...
1. 選取 [ **+ 新增規則**]。 
    ...
    :::image type="content" source="./media/automate-update-messaging-units/default-scale-metric-add-rule-link.png" alt-text="預設值-根據度量調整":::    
1. 在 [ **調整規模規則** ] 頁面上，依照下列步驟執行：
    ...
    1. 然後，選取 [**新增**]
        ...
        :::image type="content" source="./media/automate-update-messaging-units/scale-rule-cpu-75.png" alt-text="預設-如果 CPU 使用量大於75%，則相應放大":::       
...
1. 再次選取 [ **+ 新增規則** ]，然後在 [ **調整規模規則** ] 頁面上遵循下列步驟：
    ...
    1. 然後，選取 [**新增**] 
        ...
        :::image type="content" source="./media/automate-update-messaging-units/scale-rule-cpu-25.png" alt-text="預設值-如果 CPU 使用量小於25%，則相應縮小":::       

        > [!NOTE]
        > 如果在此範例中，整體 CPU 使用率低於25%，自動調整功能會減少命名空間的訊息單位。 遞減的完成從8到4、4到2，以及2到1。 
1. 設定訊息單位的 **最小值** 和 **最大** 值和 **預設** 數目。

    :::image type="content" source="./media/automate-update-messaging-units/default-scale-metric-based.png" alt-text="以度量為基礎的預設規則":::
...
1. 針對 [ **訊息單位**]，選取預設的訊息單位數目。 
    ...
    :::image type="content" source="./media/automate-update-messaging-units/default-scale-messaging-units.png" alt-text="預設值-調整為特定的訊息單位":::       
...
1. 選取 [在**預設**區塊下**新增調整規模條件**]。 
    ...
    :::image type="content" source="./media/automate-update-messaging-units/add-scale-condition-link.png" alt-text="自訂-新增調整規模條件連結":::    
1. 指定條件的 **名稱** 。 
...
6. 您也可以針對自訂條件設定 **排程** ， (但不是預設條件) 。 您可以指定條件的開始和結束日期 (或) 選取特定的天數 (星期一、星期二等等。 ) 周。 
    1. 如果您選取 [ **指定開始/結束日期**]，請選取 [ **時區**]、[ **開始日期** ] 和 [時間] 和 [ **結束日期和時間** ] (，如下圖所示) 讓條件生效。 
       ...
       :::image type="content" source="./media/automate-update-messaging-units/custom-min-max-default.png" alt-text="訊息單位數目的最小值、最大值和預設值":::
    1. 如果您選取 [ **重複特定天數**]，請選取一周中的哪幾天、[時區]、[開始時間]，以及應套用條件的結束時間。 

        :::image type="content" source="./media/automate-update-messaging-units/repeat-specific-days.png" alt-text="重複特定天數":::
...
6. 針對 [ **排程**]，指定條件的 [開始] 和 [結束日期] (或) 選取 [特定天數] ([星期一]、[星期二] 等等。 ) 周和時間。 
    ...
    :::image type="content" source="./media/automate-update-messaging-units/scale-specific-messaging-units-start-end-dates.png" alt-text="調整為特定的訊息單位-開始與結束日期":::        
    1. 如果您選取 [ **重複特定天數**]，請選取一周中的哪幾天、[時區]、[開始時間]，以及應套用條件的結束時間。
    
    :::image type="content" source="./media/automate-update-messaging-units/repeat-specific-days-2.png" alt-text="調整為特定的訊息單位-重複特定的天數":::
