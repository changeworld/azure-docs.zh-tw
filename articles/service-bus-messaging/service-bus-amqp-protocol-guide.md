...
了解 AMQP 運作方式的最可靠來源是 AMQP 1.0 規格，但此規格是為了精確引導實作而撰寫，而非用以指導通訊協定。 本節著重於盡可能介紹描述服務匯流排如何使用 AMQP 1.0 的術語。 如需 AMQP 的更完整介紹，以及 AMQP 1.0 的更廣泛討論，您可以檢閱[此影片課程][this video course]。
...
透過 TCP 使用 AMQP 連接的用戶端需要在本機防火牆中開啟埠5671和5672。 除了這些埠，如果啟用[EnableLinkRedirect](https://docs.microsoft.com/dotnet/api/microsoft.servicebus.messaging.amqp.amqptransportsettings.enablelinkredirect?view=azure-dotnet)功能，可能需要開啟其他埠。 `EnableLinkRedirect` 是新的訊息處理功能，可協助在接收訊息時略過一個躍點，因而有助於提升輸送量。 用戶端會開始透過埠範圍104XX 直接與後端服務進行通訊，如下圖所示。 

![目的地埠的清單][4]

如果防火牆封鎖了這些埠，.NET 用戶端將會失敗並出現 SocketException （「嘗試以其存取權限禁止存取通訊端」）。 您可以藉由在連接字串中設定 `EnableAmqpLinkRedirect=false` 來停用此功能，這會強制用戶端透過埠5671與遠端服務進行通訊。
...
所有交易式工作都是使用 txn 識別碼的交易傳遞狀態 `transactional-state` 來完成。在傳送訊息的情況下，交易狀態會由訊息的傳輸框架所攜帶。 
