...
> 下列計量僅適用於**進階**層。 
> 
> 針對進階層命名空間的任何中斷情況，要監視的重要計量包括：**每個命名空間的 CPU 使用量**和**每個命名空間的記憶體大小**。 使用 Azure 監視器設定這些計量的[警示](../azure-monitor/platform/alerts-metric.md)。
> 
> 另一個您可以監視的度量是：已**節流的要求**。 不過，只要命名空間維持在其記憶體、CPU 和代理連線限制內，它就不應該是問題。 如需詳細資訊，請參閱[Azure 服務匯流排 Premium 層中的節流](service-bus-throttling.md#throttling-in-azure-service-bus-premium-tier)
