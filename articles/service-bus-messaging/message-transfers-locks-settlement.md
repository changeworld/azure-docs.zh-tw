...
> [!IMPORTANT]
> 請務必注意，PeekLock 在訊息上取得的鎖定是變動的，而且可能會在下列狀況中遺失
>   * 服務更新
>   * 作業系統更新
>   * 在持有鎖定的情況下，變更實體 (佇列、主題、訂用帳戶) 的屬性。
>
> 當鎖定遺失時，Azure 服務匯流排將會產生會顯示在用戶端應用程式程式碼上的 LockLostException。 在此情況下，用戶端的預設重試邏輯應該會自動啟動，然後重試此作業。
