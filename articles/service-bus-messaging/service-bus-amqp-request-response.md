---
title: AMQP 1.0 Azure 服務匯流排中的請求/回應操作
description: 本文定義 Microsoft Azure 服務匯流排中的 AMQP 請求/基於回應的操作的清單。
...
請求郵件內文必須包含包含以下條目的**映射**的**amqp 值**部分：  
...
回應郵件內文必須包含包含以下條目的**映射**的**amqp 值**部分：  
...
回應郵件內文必須包含包含以下條目的映射的**amqp 值**部分：  
...
**規則描述**映射必須包含以下條目，其中 sql**篩選器****和關聯篩選器**是互斥的：  
...
|receiver-settle-mode|ubyte|是|在 AMQP 內核 v1.0 中指定的**接收器穩定**模式。|  
