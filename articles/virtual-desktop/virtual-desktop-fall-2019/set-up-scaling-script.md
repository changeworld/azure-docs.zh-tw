---
title: Azure 自動化 Windows 虛擬桌面 (傳統) 調整工作階段主機規模-Azure
...
ms.locfileid: "88005579"
---
# <a name="scale-windows-virtual-desktop-classic-session-hosts-using-azure-automation"></a>使用 Azure 自動化 (傳統) 工作階段主機調整 Windows 虛擬桌面
...
6. 當程式完成時，它會在指定的 Azure 自動化帳戶中建立名為**AzureRunAsConnection**的資產。 選取 [ **Azure 執行身分帳戶**]。 連線資產會保存應用程式識別碼、租用戶識別碼、訂閱識別碼，以及憑證指紋。 請記下應用程式識別碼，因為您稍後會用到它。 您也可以在 [**連接**] 頁面上找到相同的資訊。 若要移至此頁面，請在視窗左側的窗格中 **，選取 [** **共用資源**] 區段底下的 [連線]，然後按一下名為**AzureRunAsConnection**的連線資產。
...
當您找到具有您想要調整之主機集區的租使用者時，請依照[建立 Azure 自動化執行身分帳戶](#create-an-azure-automation-run-as-account)中的指示來收集**AZURERUNASCONNECTION**應用程式識別碼，並使用您從下列 Cmdlet 中的上一個 Cmdlet 取得的 Windows 虛擬桌面租使用者名稱來建立角色指派：
...
您可以藉由開啟 Azure 自動化帳戶中的 runbook 檔案，然後選取 [ **View**]，檢查您所使用的 runbook 腳本版本。 Runbook 的腳本會出現在畫面的右側。 在腳本中，您會在區段底下看到版本號碼，格式為 `v#.#.#` `SYNOPSIS` 。 您可以在[這裡](https://github.com/Azure/RDS-Templates/blob/master/wvd-templates/wvd-scaling-script/basicScale.ps1#L1)找到最新的版本號碼。 如果您在 runbook 腳本中看不到版本號碼，這表示您執行的是舊版腳本，應立即更新。 如果您需要更新 runbook 腳本，請依照[建立或更新 Azure 自動化帳戶](#create-or-update-an-azure-automation-account)中的指示進行。
