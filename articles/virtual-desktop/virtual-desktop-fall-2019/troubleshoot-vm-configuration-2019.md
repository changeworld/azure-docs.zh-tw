...
description: 如何解決在 (傳統) 工作階段主機虛擬機器設定 Windows 虛擬桌面時所發生的問題。
author: Heidilohr
...
- 手動將 Vm 新增至網域。
- 確認認證之後，重新部署範本。 請參閱 [使用 PowerShell 建立主機集](create-host-pools-powershell-2019.md)區。
...
布建 Vm 的建議方式是使用 Azure Resource Manager **建立和布建 Windows 虛擬桌面主機集** 區範本。 此範本會自動安裝 Windows 虛擬桌面代理程式和 Windows 虛擬桌面代理程式啟動載入器。
...
**修正1：** 使用 [PowerShell 建立主機集](create-host-pools-powershell-2019.md)區，手動將遺漏的元件新增至 vm。
...
- 請確定帳戶沒有 MFA。
- 確認租使用者名稱正確，且租使用者存在於 Windows 虛擬桌面。
- 確認帳戶至少具有 RDS 參與者許可權。
...
- 確認用來連線到 Windows 虛擬桌面的帳戶在租使用者上擁有建立主機集區的許可權。
- 確認帳戶沒有 MFA。
...
### <a name="error-the-status-filed-in-get-rdssessionhost-cmdlet-shows-status-as-unavailable"></a>錯誤： Get-RdsSessionHost Cmdlet 中所記載的狀態會顯示為無法使用的狀態

> [!div class="mx-imgBorder"]
> ![RdsSessionHost 指令 Cmdlet 會將狀態顯示為無法使用。](../media/23b8e5f525bb4e24494ab7f159fa6b62.png)
...
**原因：** 註冊權杖已過期或已產生，到期值為999999。

**修正：** 請遵循下列指示來修正代理程式登錄錯誤。

1. 如果已經有註冊權杖，請使用 RDSRegistrationInfo 移除該權杖。
2. 使用 Rds 產生新的權杖-NewRegistrationInfo。
3. 確認-ExpriationHours 參數設定為 72 (最大值為 99999) 。

### <a name="error-windows-virtual-desktop-agent-isnt-reporting-a-heartbeat-when-running-get-rdssessionhost"></a>錯誤：執行 Get-RdsSessionHost 時，Windows 虛擬桌面代理程式未回報心跳
...
- 使用 Azure Resource Manager **建立和布建新的 Windows 虛擬桌面主機集** 區範本
- 藉由在主要映射上包含並啟用
...
檢查下列登錄專案，並確認它們的值相符。 如果登錄機碼遺失或值不相符，請依照 [使用 PowerShell 建立主機集](create-host-pools-powershell-2019.md) 區中的指示，瞭解如何重新安裝並存堆疊。
...
2. 下載並匯入 [Windows 虛擬桌面 powershell 模組](/powershell/windows-virtual-desktop/overview/) ，以在您的 PowerShell 會話中使用（如果您還沒有這麼做），然後執行此 Cmdlet 以登入您的帳戶：

    ```powershell
    Add-RdsAccount -DeploymentUrl "https://rdbroker.wvd.microsoft.com"
    ```

3. 使用「使用 [PowerShell 建立主機集](create-host-pools-powershell-2019.md)區」來安裝並存堆疊。
...
本節中的指示可協助您卸載 Windows 虛擬桌面並存堆疊。 卸載並存堆疊後，請移至 [使用 PowerShell 建立主機集](create-host-pools-powershell-2019.md) 區中的「向 Windows 虛擬桌面主機集區註冊 VM」，以重新安裝並存堆疊。
