...
- **LoadBalancer** - 建立 Azure Load Balancer 資源、設定外部 IP 位址，並將要求的 Pod 連線至負載平衡器後端集區。 為了允許客戶的流量到達應用程式，在所需的埠上創建負載平衡規則。 
...
您可以指派內部** 和外部** 負載平衡器。 內部負載等化器僅分配私人 IP 位址，因此無法從 Internet 訪問它們。
...
節點會使用 [kubenet][kubenet] Kubernetes 外掛程式。 您可以讓 Azure 平台為您建立及設定虛擬網路，或選擇部署 AKS 叢集到現有的虛擬網路子網路。 同樣，只有節點接收可路由的 IP 位址，並且 pod 使用 NAT 與 AKS 群集外的其他資源進行通信。 這種方法可大幅減少您需要在網路空間中保留，以供 Pod 使用的 IP 位址數目。

如需詳細資訊，請參閱[為 AKS 叢集設定 kubenet 網路][aks-configure-kubenet-networking]。

### <a name="azure-cni-advanced-networking"></a>Azure CNI (進階) 網路

使用 Azure CNI，每個 Pod 都從子網路取得 IP 位址，並且可以直接存取。 這些 IP 位址在您的網路空間中必須是唯一的，且必須事先規劃。 每個節點都有一個組態參數，用於所支援的最大 Pod 數目。 然後，為該節點預先保留每個節點的相同 IP 位址數目。 此方法需要更多的規劃，否則會導致 IP 位址耗盡，或者隨著應用程式需求的增長，需要在更大的子網中重建群集。
...
kubenet 和 Azure CNI 都為您的 AKS 群集提供網路連接。 但是，每個都有優點和缺點。 在高級別上，應用以下注意事項：

* **庫貝內**
    * 節省 IP 位址空間。
    * 使用 Kubernetes 內部或外部負載等化器從群集外部到達吊艙。
    * 您必須手動管理和維護使用者定義的路由 （UDR）。
    * 每個群集最多 400 個節點。
* **Azure CNI**
    * Pods 獲得完整的虛擬網路連接，可通過其連接的網路的私人 IP 位址直接到達。
    * 需要更多的 IP 位址空間。

kubenet 和 Azure CNI 之間存在以下行為差異：

| 功能                                                                                   | 庫貝內特   | Azure CNI |
|----------------------------------------------------------------------------------------------|-----------|-----------|
| 在現有虛擬網路或新虛擬網路中部署群集                                            | 支援 - 手動應用的 UDR | 支援 |
| Pod-pod 連接                                                                         | 支援 | 支援 |
| Pod-VM 連接;同一虛擬網路中的 VM                                          | 由吊艙啟動時工作 | 雙向工作 |
| Pod-VM 連接;對等虛擬網路中的 VM                                            | 由吊艙啟動時工作 | 雙向工作 |
| 使用 VPN 或快速路由進行本地訪問                                                | 由吊艙啟動時工作 | 雙向工作 |
| 訪問受服務終結點保護的資源                                             | 支援 | 支援 |
| 使用負載等化器服務、應用閘道或入口控制器公開 Kubernetes 服務 | 支援 | 支援 |
| 預設 Azure DNS 和私人區域                                                          | 支援 | 支援 |

關於 DNS，庫貝內特和 Azure CNI 外掛程式 DNS 由在 AKS 中運行的守護進程集 CoreDNS 提供。 有關庫伯奈斯上的 CoreDNS 的詳細資訊，請參閱[自訂 DNS 服務](https://kubernetes.io/docs/tasks/administer-cluster/dns-custom-nameservers/)。 CoreDNS 按預設值配置為將未知域轉發到節點 DNS 伺服器，換句話說，轉發到部署 AKS 群集的 Azure 虛擬網路的 DNS 功能。 因此，Azure DNS 和私人區域將適用于在 AKS 中運行的窗格。

### <a name="support-scope-between-network-models"></a>網路模型之間的支援範圍

無論您使用哪種網路模型，kubenet 和 Azure CNI 都可以以以下方式之一進行部署：

* 創建 AKS 群集時，Azure 平臺可以自動創建和配置虛擬網路資源。
* 您可以手動創建和配置虛擬網路資源，並在創建 AKS 群集時附加到這些資源。

儘管 kubenet 和 Azure CNI 都支援服務終結點或 UDR 等功能，[但 AKS 的支援策略][support-policies]可以定義可以進行哪些更改。 例如：

* 如果手動為 AKS 群集創建虛擬網路資源，則在配置自己的 UDR 或服務終結點時，支援您。
* 如果 Azure 平臺自動為 AKS 群集創建虛擬網路資源，則不支援手動更改這些 AKS 管理的資源以配置自己的 UDR 或服務終結點。
...
您還可以配置入口控制器，以在 AKS 群集中向容器請求時保留用戶端源 IP。 當用戶端的請求通過入口控制器路由到 AKS 群集中的容器時，該請求的原始源 IP 將不可用到目標容器。 啟用*用戶端源 IP 保留*時，用戶端的源 IP 在*X-前轉 -For*下的請求標頭中可用。 如果您在入口控制器上使用用戶端源 IP 保留，則不能使用 SSL 傳遞。 用戶端源 IP 保留和 SSL 傳遞可與其他服務（如*負載平衡器*類型）一起使用。

## <a name="network-security-groups"></a>網路安全性群組

網路安全性群組可篩選 VM 的流量，例如 AKS 節點。 當您建立服務 (例如 LoadBalancer) 時，Azure 平台會自動設定任何所需的網路安全性群組規則。 請勿以手動方式設定對 AKS 叢集中的 Pod 進行流量篩選的網路安全性群組規則。 請將任何必要的連接埠和轉送定義為 Kubernetes 服務資訊清單的一部分，然後由 Azure 平台建立或更新適當的規則。 您還可以使用網路原則（如下一節所述）自動將流量篩選器規則應用於 pod。
...
網路原則是 AKS 中提供的 Kubernets 功能，允許您控制窗格之間的流量。 您可以根據指派的標籤、命名空間或流量連接埠等設定，選擇允許或拒絕流量。 網路安全性群組較適用於 AKS 節點而非 Pod。 使用網路原則是一種控制流量的較合適且雲端原生的方式。 由於 Pod 是在 AKS 叢集內以動態方式建立的，因此可以自動套用所需的網路原則。
...
有關相關的最佳實踐，請參閱[AKS 中網路連接和安全的最佳做法][operator-best-practices-network]。
