...
要與 Azure API 互動,AKS 叢集需要[Azure 活動目錄 (AD) 服務主體][aad-service-principal]或[託管識別](use-managed-identity.md)。 需要服務主體或託管標識來動態創建和管理其他 Azure 資源,如 Azure 負載均衡器或容器註冊表 (ACR)。
...
如果使用來自其他 Azure AD 租戶的服務主體,則部署群集時可用的許可權還有其他注意事項。 您可能沒有讀取和寫入目錄資訊的適當許可權。 有關詳細資訊,請參閱[Azure 活動目錄中的默認用戶許可權是什麼?][azure-ad-permissions]
...
> [!NOTE]
> 如果使用具有自定義機密的現有服務主體,請確保機密不超過 190 位元組。
...
AKS 叢集的服務主體可用來存取其他資源。 例如,如果要將 AKS 群集部署到現有的 Azure 虛擬網路子網或連接到 Azure 容器註冊表 (ACR),則需要將對這些資源的訪問許可權委派給服務主體。

要委派許可權,請使用[az 角色分配創建命令建立][az-role-assignment-create]角色分配。 將`appId`分配給特定作用域(如資源組或虛擬網路資源)。 接著，角色會定義資源上的服務主體可擁有哪些權限，如下列範例所示：
...
如果使用 Azure 容器註冊表 (ACR) 作為容器映像儲存,則需要向 AKS 群集的服務主體授予讀取和拉取映射的許可權。 目前,建議的配置是使用 az [aks 創建][az-aks-create]或[az aks 更新][az-aks-update]命令與註冊表集成併為服務主體分配適當的角色。 有關詳細步驟,請參閱[Azure 庫伯奈斯服務中的 Azure 容器註冊表進行身份驗證][aks-to-acr]。
...
- 默認情況下,服務主體憑據的有效期為一年。 您可以隨時[更新或輪換服務主體認證][update-credentials]。
- 每個服務主體都會與 Azure AD 應用程式相關聯。 Kubernetes 叢集服務主體可以與任何有效的 Azure AD 應用程式名稱相關聯*https://www.contoso.org/example*(例如: 。 應用程式的 URL 不一定是實際端點。
- 當您指定服務主體**用戶端識別碼**時，請使用 `appId` 的值。
- 在 Kubernetes 群集中的代理節點 VM 上,服務主體認證儲存在檔案中`/etc/kubernetes/azure.json`
- 當您使用 [az aks create][az-aks-create] 命令自動產生服務主體時，服務主體認證會寫入用來執行命令之電腦上的 `~/.azure/aksServicePrincipal.json` 檔案。
- 如果不在附加 AKS CLI 命令中專門傳遞服務主體,則`~/.azure/aksServicePrincipal.json`使用位於 的 預設服務主體。  
...
AKS 群集的服務主體憑據由 Azure CLI 緩存。 如果這些憑據已過期,則在部署 AKS 群集時會遇到錯誤。 執行[az aks 建立][az-aks-create]時發生以下錯誤訊息可能表示快取的服務主體認證有問題:
...
服務主體憑據的預設過期時間是一年。 如果您的*aksServiceThe.json*文件超過一年,請刪除該檔並嘗試再次部署 AKS 群集。

## <a name="next-steps"></a>後續步驟

有關 Azure 活動目錄服務主體的詳細資訊,請參閱[應用程式和服務主體物件][service-principal]。

有關如何更新認證資訊,請參閱[在 AKS 中更新或輪換服務主體的認證][update-credentials]。
