...
此命令也會啟用叢集[自動調整程式][cluster-autoscaler]，這是建議搭配點節點集區使用的。 根據叢集中執行的工作負載，叢集自動調整程式會相應增加並相應減少節點集區中的節點數目。 對於點節點集區，如果仍然需要其他節點，叢集自動調整程式會在收回之後相應增加節點數目。 如果您變更節點集區可以擁有的節點數目上限，您也需要調整與叢集自動調整程式相關聯的 `maxCount` 值。 如果您未使用叢集自動調整程式，在收回時，該點集區最終會降低為零，而且需要手動操作以接收任何額外的點節點。
