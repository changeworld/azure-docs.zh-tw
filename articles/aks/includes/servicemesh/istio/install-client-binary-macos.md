...
在 MacOS 上的基於 bash 的`curl`shell 中，使用 下載 Istio 版本，然後提取`tar`如下所示：
...
用戶端`istioctl`二進位檔案在用戶端電腦上運行，並允許您與 Istio 服務網格進行交互。 使用以下命令在 MacOS 上的基於`istioctl`bash 的 shell 中安裝 Istio 用戶端二進位檔案。 這些命令會將 `istioctl` 用戶端二進位檔複製到 `PATH` 中的標準使用者程式位置。
