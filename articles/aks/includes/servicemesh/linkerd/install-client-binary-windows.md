...
`linkerd`用戶端二進位檔會在您的用戶端電腦上執行，並可讓您與 Linkerd 服務網格互動。 使用下列命令， `linkerd` 在 Windows 上以 PowerShell 為基礎的 shell 中安裝 Linkerd 用戶端二進位檔。 這些命令會將 `linkerd` 用戶端二進位檔複製到 Linkerd 資料夾，然後透過您的，以立即方式（在目前的 shell 中）和永久（跨 shell 重新開機）來使用它 `PATH` 。 您不需要更高的（系統管理員）許可權來執行這些命令，也不需要重新開機 shell。
