...
本最佳實踐文章重點介紹如何在 AKS 中保護 pod。 您會了解如何：
...
Pod 的執行身分應為定義的使用者或群組，而非「根」**，您的應用程式才能正確執行。 Pod 或容器的 `securityContext` 可讓您定義如 *runAsUser* 或 *fsGroup* 的設定，取得適當的權限。 僅指派所需的使用者或群組的權限，並不使用資訊安全內容作為取得其他權限的方式。 *運行使用者*、許可權升級和其他 Linux 功能設置僅在 Linux 節點和窗格上可用。
...
**最佳做法指引** - 不在應用程式程式碼中定義認證。 使用 Azure 資源的受控身分識別，可讓 Pod 要求存取其他資源。 如 Azure Key Vault 的數位保存庫也應可用來儲存和擷取數位金鑰與認證。 Pod 託管標識僅用於 Linux pod 和容器映射。
