...
> [!NOTE]
> Azure Load Balancer 有兩種 SKU -「基本」和「標準」。 根據預設，您建立 AKS 叢集時，會使用「標準」SKU。  當您建立類型為 LoadBalancer 的服務時，您會取得與布建叢集時相同的 LB 類型。 如需詳細資訊，請參閱 [Azure 負載平衡器 SKU 比較][azure-lb-comparison]。
...
如果您使用現有的子網或資源群組，AKS 叢集服務主體需要管理網路資源的許可權。 如需詳細資訊，請參閱 [Azure Kubernetes Service (AKS 中的使用 kubenet 網路與您自己的 IP 位址範圍) ][use-kubenet] 或 [設定 Azure Kubernetes Service (AKS) 中的 Azure CNI 網路 ][advanced-networking]功能。 如果您要將負載平衡器設定為使用 [不同子網中的 IP 位址][different-subnet]，請確定 AKS 叢集服務主體也具有該子網的讀取權限。
...
Azure 負載平衡器會建立在 node 資源群組中，並聯機到與 AKS 叢集相同的虛擬網路。

當您檢視服務詳細資料時，內部負載平衡器的 IP 位址會顯示在 EXTERNAL-IP** 資料行中。 在此內容中， *external* 與負載平衡器的外部介面相關，而不是接收公用的外部 IP 位址。 IP 位址可能需要一到兩分鐘的時間才能變更 *\<pending\>* 為實際的內部 IP 位址，如下列範例所示：
...
如果您想要搭配內部負載平衡器使用特定 IP 位址，請將 loadBalancerIP** 屬性新增至負載平衡器 YAML 資訊清單中。 在此案例中，指定的 IP 位址必須位於與 AKS 叢集相同的子網中，且必須尚未指派給資源。 例如，您不應該使用指定給 Kubernetes 子網範圍內的 IP 位址。
...
當您部署並查看服務詳細資料時， *外部 ip* 資料行中的 ip 位址會反映指定的 ip 位址：
...
如需在不同的子網中設定負載平衡器的詳細資訊，請參閱 [指定不同的子網][different-subnet]
