...
description: 瞭解如何升級 Azure 庫伯奈斯服務 (AKS) 群集以獲取最新功能和安全更新。
...
對您使用多個節點池或 Windows 伺服器節點(目前在 AKS 中預覽)的 AKS 群集,請參閱[在 AKS 中升級節點池][nodepool-upgrade]。

## <a name="before-you-begin"></a>開始之前

本文要求您運行 Azure CLI 版本 2.0.65 或更高版本。 執行 `az --version` 以尋找版本。 如果您需要安裝或升級，請參閱[安裝 Azure CLI][azure-cli-install]。
...
以下範例輸出顯示叢集可以升級到版本*1.13.9*和*1.13.10*:
...
透過適用於您的 AKS 叢集的可用版本清單，使用 [az aks upgrade][az-aks-upgrade] 命令進行升級。 在升級過程中,AKS 向運行指定 Kubernetes 版本的群集添加新節點,然後仔細[設置和排空][kubernetes-drain]其中一個舊節點,以盡量減少對正在運行的應用程式的中斷。 當新節點確認為正在運行的應用程式窗格時,舊節點將被刪除。 此過程重複,直到群集中的所有節點都升級。

下面的範例將叢集升級到版本*1.13.10*:
...
> [!NOTE]
> 群集升級的總允許時間可完成。 此時間是透過取得的產品計算`10 minutes * total number of nodes in the cluster`的 。 例如,在 20 個節點群集中,升級操作必須在 200 分鐘內成功,否則 AKS 將無法執行該操作以避免無法恢復的群集狀態。 要在升級失敗時恢復,請重試超時命中后升級操作。
