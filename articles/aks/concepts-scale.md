...
若要將這些競爭事件降至最低，請設定 cooldown 或 delay 值。 這些值會定義水平 Pod 自動調整程式在調整事件之後，必須等待多久時間，才可以觸發另一個調整事件。 此行為可讓新的複本計數生效，而計量 API 會反映分散式工作負載。 根據預設，擴大事件的延遲為 3 分鐘，而縮小事件的延遲為 5 分鐘
...
### <a name="scale-down-events"></a>縮小事件
