...
3. 在包含叢集的 VNet 有自訂 DNS 設定 (4) 的情況下，除非私人 DNS 區域已連結至包含自訂 DNS 解析程式 (5) 的 VNet，否則叢集部署將會失敗。 在叢集布建期間建立私人區域，或在使用事件架構部署 (機制（例如 Azure Event Grid 和 Azure Functions) ）偵測建立區域時，可以手動建立此連結。
