...
本文還假定您具有與 AKS 群集在同一資源組中具有[DNS 區域][dns-zone]的[自訂域][custom-domain]。

本文使用 Helm 來安裝 NGINX 輸入控制器、cert-manager 及範例 Web 應用程式。 請確定您使用的是 Helm 的最新版本。 有關升級說明，請參閱[Helm 安裝文檔][helm-install]。有關配置和使用 Helm 的詳細資訊，請參閱在[Azure 庫伯奈斯服務 （AKS） 中使用 Helm 安裝應用程式][use-helm]。
...
要創建入口控制器，請使用 命令`helm`安裝*nginx 入口*。 為了新增備援，您必須使用 `--set controller.replicaCount` 參數部署兩個 NGINX 輸入控制器複本。 為充分享有執行輸入控制器複本的好處，請確定 AKS 叢集中有多個節點。
...
## <a name="add-an-a-record-to-your-dns-zone"></a>將 A 記錄添加到 DNS 區域
...
> [!NOTE]
> 或者，您可以為入口控制器 IP 位址配置 FQDN，而不是自訂域。 請注意，此示例適用于 Bash 外殼。
> 
...
在安裝示例 Helm 圖表之前，請將 Azure 示例存儲庫添加到 Helm 環境。

```console
helm repo add azure-samples https://azure-samples.github.io/helm-charts/
```

使用*azure 示例/aks-helloworld*赫爾姆圖創建名為*aks-helloworld*的演示應用程式。
...
使用以下示例`hello-world-ingress.yaml`YAML 創建名為的檔。 將 *hosts* 和 *host* 更新為您在上一個步驟中建立的 DNS 名稱。
...
## <a name="verify-a-certificate-object-has-been-created"></a>驗證已創建證書物件

接下來，必須建立憑證資源。 憑證資源會定義所需的 X.509 憑證。 有關詳細資訊，請參閱[證書管理器證書][cert-manager-certificates]。 Cert 管理器已使用入口-shim 自動為您創建一個證書物件，該物件自 v0.2.2 起自動與證書管理器一起部署。 如需詳細資訊請，請參閱 [ingress-shim 文件][ingress-shim] \(英文\)。

要驗證憑證是否成功創建，請使用`kubectl get certificate --namespace ingress-basic`命令並驗證*READY*為*True*，這可能需要幾分鐘時間。
...
打開 Web 瀏覽器以*進入 hello 世界。MY_CUSTOM_DOMAIN*庫伯內斯入口控制器。 請注意，您將重定向到使用 HTTPS，證書受信任，演示應用程式顯示在 Web 瀏覽器中。 添加 */hello-world-world-2*路徑，並注意到顯示帶有自訂標題的第二個演示應用程式。

## <a name="clean-up-resources"></a>清除資源

本文使用 Helm 來安裝輸入元件、憑證及範例應用程式。 部署 Helm 圖表時會建立一些 Kubernetes 資源。 這些資源包含 Pod、部署和服務。 要清理這些資源，可以刪除整個示例命名空間或單個資源。
...
或者，更精細的方法是刪除創建的單個資源。 首先，刪除群集頒發者資源：
