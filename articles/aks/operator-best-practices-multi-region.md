...
使用分割 TCP 型的任意傳播通訊協定， [Azure Front 開門服務](../frontdoor/front-door-overview.md)可確保您的終端使用者立即連線到最接近的前門 POP (點的目前狀態) 。 Azure 前門服務的其他功能包括 TLS 終止、自訂網域、web 應用程式防火牆、URL 重寫和會話親和性。 請檢閱應用程式流量的需求，以了解哪一種解決方案最合適。
...
容器和微服務在其中執行的進程不會保留狀態時，是最具彈性的。 因為應用程式幾乎一律包含某個狀態，所以請使用 PaaS 解決方案，例如 Azure Cosmos DB、適用於 PostgreSQL 的 Azure 資料庫、適用於 MySQL 的 Azure 資料庫或 Azure SQL Database。
