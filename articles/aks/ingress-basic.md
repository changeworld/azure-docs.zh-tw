...
或者，更細微的方法是刪除所建立的個別資源。 使用命令來列出 Helm 版本 `helm list` 。 尋找名為nginx-ingress** 和 aks-helloworld** 的圖表，如下列範例輸出所示：
...
使用命令卸載發行 `helm uninstall` 。 下列範例會卸載 NGINX 輸入部署。
