...
儲存體類別可用來定義 Azure 檔案共用的建立方式。 系統會自動在[節點資源群組][node-resource-group]中建立儲存體帳戶，以搭配儲存類別來存放 Azure 檔案共用。 為 *skuName* 選擇下列 [Azure 儲存體備援][storage-skus]：

* *Standard_LRS* - 標準本地備援儲存體 (LRS)
* *Standard_GRS* - 標準異地備援儲存體 (GRS)
* *Standard_ZRS* -標準區域冗余儲存體（ZRS）
...
永續性磁碟區宣告 (PVC) 會使用儲存體類別物件，以動態方式佈建 Azure 檔案共用。 下列 YAML 可以用來建立具有*ReadWriteMany*存取權的持續性磁片區索取*5 GB*大小。 如需有關存取模式的詳細資訊，請參閱 [Kubernetes 永續性磁碟區][access-modes]文件。
...
下列 YAML 所建立的 Pod，會使用永續性磁碟區宣告 azurefile**，將 Azure 檔案共用裝載在 /mnt/azure** 路徑。 若是 Windows Server 容器，請使用 Windows 路徑慣例來指定*mountPath* ，例如 *： '*。
