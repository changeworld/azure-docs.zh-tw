...
ms.locfileid: "80986313"
---
# <a name="manage-system-node-pools-in-azure-kubernetes-service-aks"></a>在 Azure 庫伯奈斯服務 (AKS) 中管理系統節點池

在 Azure 庫伯奈斯服務 (AKS) 中,相同配置的節點分組到*節點池*中。 節點池包含運行應用程式的基礎 VM。 系統節點池和用戶節點池是 AKS 群集的兩種不同的節點池模式。 系統節點池承載基本系統服務,如 CoreDNS。 用戶節點池是放置特定於應用程式的窗格的位置。 每個 AKS 群集必須至少包含一個至少具有一個節點的系統節點池。 如果為 AKS 群集運行單個系統節點池,我們建議您為節點池至少使用三個節點。 
