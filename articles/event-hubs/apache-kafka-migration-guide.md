...
ms.locfileid: "81677360"
---
# <a name="migrate-to-azure-event-hubs-for-apache-kafka-ecosystems"></a>遷移至 Apache Kafka 生態系統的 Azure 事件中樞
...
若要連接到已啟用 Kafka 的事件中樞，您必須更新 Kafka 用戶端設定。 如果您找不到自己的問題，請嘗試搜尋`bootstrap.servers`在應用程式中設定的位置。

在您的應用程式中有意義的地方插入下列各項。 請務必更新`bootstrap.servers`和`sasl.jaas.config`值，以使用正確的驗證將用戶端導向您的事件中樞 Kafka 端點。 
...
如果`sasl.jaas.config`在您的架構中不是支援的設定，請尋找用來設定 SASL 使用者名稱和密碼的設定，並改為使用它們。 將 [使用者名稱`$ConnectionString` ] 設為，並將密碼設定為您事件中樞連接字串。
