---
title: 將 Azure 事件中心與 Azure 專用連結服務集成
description: 瞭解如何將 Azure 事件中心與 Azure 專用連結服務集成
services: event-hubs
author: spelluru
ms.author: spelluru
ms.date: 03/12/2020
...
ms.locfileid: "79477846"
---
# <a name="integrate-azure-event-hubs-with-azure-private-link-preview"></a>將 Azure 事件中心與 Azure 專用連結（預覽）集成
Azure 專用連結服務使您能夠通過虛擬網路中的**專用終結點**訪問 Azure 服務（例如，Azure 事件中心、Azure 存儲和 Azure 宇宙資料庫）和 Azure 託管的客戶/合作夥伴服務。
...
要將事件中心命名空間與 Azure 專用連結集成，您需要以下實體或許可權：
...
### <a name="steps"></a>步驟
如果您已經有一個事件中心命名空間，可以通過以下步驟創建專用連結連接：
...
8. 在 **"資源"** 頁上，按照以下步驟操作：
    1. 對於連接方法，如果選擇 **"連接到目錄中的 Azure 資源**"，請按照以下步驟操作： 
        1. 選擇**事件中心命名空間**存在的**Azure 訂閱**。 
        2. 對於**資源類型**，選擇 **"資源類型**"的**Microsoft.eventHub/命名空間**。
    ...
    2. 如果選擇 **"按資源識別碼 或別名連接到 Azure 資源**"，請按照以下步驟操作：
        ...
        4. 然後，選擇**頁面底部的"下一步：配置>** 按鈕。
            ...
            ![創建專用終結點 - 使用資源識別碼 連接](./media/private-link-service/connect-resource-id.png)
...
12. 確認您看到創建的專用終結點連接顯示在終結點清單中。 在此示例中，私有終結點是自動批准的，因為您連接到目錄中的 Azure 資源，並且您具有足夠的許可權。 
...
下面的示例演示如何使用 Azure PowerShell 創建專用終結點連接。 它不會為您創建專用群集。 按照[本文](event-hubs-dedicated-cluster-create-portal.md)中的步驟創建專用的事件中心群集。 
...
應驗證專用終結點資源的同一子網中的資源是否通過私人 IP 位址連接到事件中心命名空間，並且它們具有正確的專用 DNS 區域集成。
...
如果運行 ns 查找命令以在公共終結點上解析事件中心命名空間的 IP 位址，您將看到如下所示的結果：
...
如果運行 ns 查找命令以解決事件中心命名空間在專用終結點上的 IP 位址，您將看到如下所示的結果：
...
## <a name="limitations-and-design-considerations"></a>限制和設計注意事項
