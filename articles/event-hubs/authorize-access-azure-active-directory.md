...
當安全性主體 (使用者或應用程式) 嘗試存取事件中樞資源時，要求必須獲得授權。 使用 Azure AD，對資源的存取是兩個步驟的程序。 
 ...
 1. 首先，會驗證安全性主體的身分識別，並傳回 OAuth 2.0 權杖。 要求權杖的資源名稱是，所有雲端/租使用者的資源名稱都 `https://eventhubs.azure.net/` 相同。 針對 Kafka 用戶端，要求權杖的資源是 `https://<namespace>.servicebus.windows.net` 。
