...
1. 建立主要命名空間。
1. 在訂用帳戶中建立次要命名空間，並在具有主要命名空間的資源群組中建立。 此為選用步驟。 您可以在下一個步驟中建立配對時建立次要命名空間。 
...
1. 在 [ **起始配對** ] 頁面上，選取現有的次要命名空間，或在訂用帳戶中建立一個，並在具有主要命名空間的資源群組中建立一個。 然後，選取 [Create] \(建立\)。 在下列範例中，已選取現有的次要命名空間。 
