...
ms.locfileid: "79281466"
---
# <a name="azure-event-hubs---geo-disaster-recovery"></a>Azure 事件中樞 - 異地災害復原 

當整個 Azure 區域或資料中心 (如果未使用[可用性區域](../availability-zones/az-overview.md)) 遇到停機時，最重要的是資料處理作業能夠繼續在不同的區域或資料中心運作。 因此，*地質災害恢復*和*異地複製*是任何企業的重要功能。 Azure 事件中樞支援命名空間層級的地理災害復原和異地複寫。 

> [!NOTE]
> 地質災害恢復功能僅適用于[標準和專用 SKU。](https://azure.microsoft.com/pricing/details/event-hubs/)  
...
地質災害恢復功能僅適用于[標準和專用 SKU。](https://azure.microsoft.com/pricing/details/event-hubs/) 您不需要進行任何連接字串變更，因為連接是透過別名建立的。
...
## <a name="supported-namespace-pairs"></a>支援的命名空間對
支援主命名空間和輔助命名空間的以下組合：  
...
> [!NOTE]
> 不能對同一專用群集中的命名空間進行配對。 可以配對位於單獨群集中的命名空間。 
...
1. 根據設計，事件中心地理災害復原不會複製資料，因此無法重用輔助事件中心上主事件中心的舊偏移值。 我們建議使用以下操作之一重新開機事件接收器：

- *事件位置.從開始（）* - 如果您希望讀取輔助事件中心的所有資料。
- *事件位置.FromEnd（）* - 如果您希望從連接到輔助事件中心時讀取所有新資料。
- *事件位置.FromEnqueuedtime（日期時間）* - 如果您希望從給定日期和時間開始讀取輔助事件中心中接收的所有資料。
