---
title: 針對連線能力問題進行疑難排解-Azure 事件中樞 |Microsoft Docs
...
檢查 [Azure 服務狀態網站](https://azure.microsoft.com/status/)上的 Azure 事件中樞服務中斷。

### <a name="verify-the-connection-string"></a>驗證連接字串 
確認您所使用的連接字串正確無誤。 請參閱 [取得連接字串](event-hubs-get-connection-string.md) ，以使用 AZURE 入口網站、CLI 或 PowerShell 來取得連接字串。 
...
確認您的應用程式正在具有命名空間存取權的虛擬網路子網中執行。 如果不是，請在具有命名空間存取權的子網中執行應用程式，或將執行應用程式之電腦的 IP 位址新增至 [ip 防火牆](event-hubs-ip-filtering.md)。 

當您建立事件中樞命名空間的虛擬網路服務端點時，命名空間只接受來自系結至服務端點之子網的流量。 此行為有例外狀況。 您可以在 IP 防火牆中新增特定 IP 位址，以啟用事件中樞公用端點的存取。 如需詳細資訊，請參閱 [網路服務端點](event-hubs-service-endpoints.md)。

### <a name="check-the-ip-firewall-settings-for-your-namespace"></a>檢查命名空間的 IP 防火牆設定
檢查 IP 防火牆未封鎖執行應用程式之電腦的公用 IP 位址。  
...
在較新版本的 SDK 中，某些暫時性連線問題可能已在您使用的版本中修正。 確定您在應用程式中使用最新版本的用戶端 Sdk。 Sdk 會透過新的/更新的功能和錯誤修正持續改善，因此請一律使用最新的套件進行測試。 請參閱版本資訊以取得已修正的問題，以及新增/更新的功能。 
...
如果應用程式程式碼使用 SDK，則重試原則已經內建且使用中。 應用程式會重新連接，而不會對應用程式/工作流程產生重大影響。 攔截這些暫時性錯誤，然後再試一次呼叫，將可確保您的程式碼能夠復原這些暫時性問題。
