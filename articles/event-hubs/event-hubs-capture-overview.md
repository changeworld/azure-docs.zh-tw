...
Azure 事件中心使您能夠在[Azure Blob 存儲](https://azure.microsoft.com/services/storage/blobs/)或 Azure[資料存儲第 1 代或](https://azure.microsoft.com/services/data-lake-store/)第 2 代帳戶中自動捕獲事件中心中的流資料，從而增加了指定時間或大小間隔的靈活性。 設定擷取的作業很快，因此執行時不需要系統管理成本，而且它可以針對事件中樞的[輸送量單位](event-hubs-scalability.md#throughput-units)自動進行調整。 事件中樞擷取是將串流資料載入至 Azure 的最簡單方式，並可讓您專注於處理資料而非擷取資料。
...
事件中心捕獲使您能夠指定自己的 Azure Blob 存儲帳戶和容器，或用於存儲捕獲資料的 Azure 資料湖存儲帳戶。 這些帳戶可以和事件中樞位於相同區域，也可以位於另一個區域，從而新增至事件中樞擷取功能的彈性。
...
如果 Azure 存儲 Blob 暫時不可用，事件中心捕獲將保留事件中心上配置的資料保留期的資料，並在存儲帳戶再次可用後返回填充資料。
...
- [使用 Azure 門戶啟用事件中心捕獲](event-hubs-capture-enable-through-portal.md)
...
[阿帕奇鑽：Azure Blob 存儲外掛程式][Apache Drill: Azure Blob Storage Plugin]
...
[Apache Spark][Apache Spark] 是進行大規模資料處理的整合分析引擎。 它支援不同的語言 (包括 SQL)，而且可以輕鬆地存取 Azure Blob 儲存體。 在 Azure 中運行 Apache Spark 有幾個選項，每個選項都提供了對 Azure Blob 存儲的輕鬆訪問：

- [HDInsight：在 Azure 存儲中的位址檔][HDInsight: Address files in Azure storage]
- [Azure 資料塊：Azure Blob 存儲][Azure Databricks: Azure Blob Storage]
...
瞭解如何使用 Azure 門戶和 Azure 資源管理器範本啟用此功能：

- [使用 Azure 入口網站啟用事件中樞擷取](event-hubs-capture-enable-through-portal.md)
- [使用 Azure 資源管理器範本啟用事件中心捕獲](event-hubs-resource-manager-namespace-event-hub-enable-capture.md)
...
[縮放時資料流：事件中心捕獲]:https://github.com/yorek/streaming-at-scale/tree/master/event-hubs-capture
