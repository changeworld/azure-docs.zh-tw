...
為了容納高載流量，IoT 中樞會在有限的時間中接受節流的要求。 這些要求的前幾個會立即處理。 不過，如果要求的數目持續違反節流，IoT 中樞會開始將要求放在佇列中，並以限制的速率處理。 這種效果稱為「*流量成形*」。 此外，此佇列的大小也會受到限制。 如果節流違規繼續，最後佇列會填滿，IoT 中樞開始拒絕使用的要求 `429 ThrottlingException` 。

例如，您可以使用模擬裝置，每秒將200的裝置到雲端訊息傳送到您的 S1 IoT 中樞（其限制為每秒 D2C 個）。 在第一分鐘或兩個中，訊息會立即處理。 不過，由於裝置會繼續傳送比節流限制更多的訊息，IoT 中樞一開始只會處理每秒100個訊息，並將其餘部分放在佇列中。 您開始注意到增加的延遲。 最後，您會開始在 `429 ThrottlingException` 佇列填滿，而[IoT 中樞的計量](iot-hub-metrics.md)中的「節流錯誤數目」開始增加。
