---
title: 使用 Azure IoT 中心監視並排除斷線連線問題
description: 瞭解如何透過 Azure IoT 中心的裝置連接監視和排除常見錯誤
author: jlian
manager: briz
ms.service: iot-hub
services: iot-hub
...
---
# <a name="monitor-diagnose-and-troubleshoot-disconnects-with-azure-iot-hub"></a>使用 Azure IoT 中心監控、診斷和排除斷線連線問題

IoT 裝置的連線問題可能因為有許多可能的失敗點而難以排解。 應用程式邏輯、物理網路、協定、硬體、IoT 中心和其他雲端服務都可能導致問題。 檢測和查明問題根源的能力至關重要。 但是,規模的IoT解決方案可以有數千台設備,因此手動檢查單個設備是不切實際的。 為了幫助大規模檢測、診斷和排除這些問題,請使用IoT中心透過Azure監視器提供的監視功能。 這些功能僅限於 IoT 中心可以觀察到的內容,因此我們還建議您遵循設備和其他 Azure 服務的監視最佳做法。
...
### <a name="set-up-alerts-for-device-disconnect-at-scale"></a>為大規模裝置斷線連接設定警示
...
5. 選擇 **「新增條件**」,然後選擇「已連接的設備(預覽)」。

6. 設置閾值並通過以下提示發出警報。

要瞭解更多資訊,請參閱[Microsoft Azure 中的警報是什麼?](../azure-monitor/platform/alerts-overview.md)

#### <a name="detecting-individual-device-disconnects"></a>偵測單裝置中斷連線

要偵測*每個裝置的*斷線連線(例如,當您需要知道工廠剛剛離線時)請使用[事件網格設定裝置斷線連接事件](iot-hub-event-grid.md)。
