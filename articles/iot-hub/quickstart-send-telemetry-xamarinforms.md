...
本文會使用預先撰寫的 Xamarin.Forms 應用程式傳送遙測，以及使用 CLI 公用程式從 IoT 中樞讀取遙測。
...
- 從 [Azure 範例](https://github.com/Azure-Samples/azure-iot-samples-xamarin/archive/master.zip)下載程式碼範例

- 最新版本的 [Visual Studio 2019](https://visualstudio.microsoft.com/)，或已安裝 Xamarin.Forms 工具的 [Visual Studio for Mac](https://visualstudio.microsoft.com/)。 本快速入門已使用 Visual Studio 16.6.0 進行測試。
...
1. 在 Azure Cloud Shell 中執行下列命令，以建立裝置身分識別。
   ...
   **myXamarinDevice**：這是您要註冊之裝置的名稱。 建議您使用 **myXamarinDevice**，如下所示。 如果您為裝置選擇不同的名稱，則也必須在本文中使用該名稱，並先在範例應用程式中更新該裝置名稱，再執行應用程式。
...
範例應用程式會在 Windows 上 (透過 UWP 應用程式)、iOS 裝置或模擬器及 Android 裝置或模擬器上執行，並連線到 IoT 中樞上的裝置特有端點，然後傳送模擬的溫度和濕度遙測。 

1. 在 Visual Studio 或 Visual Studio for Mac 中開啟範例工作區。
2. 展開 **SimulatedDevice**專案。  
3. 在 Visual Studio 中開啟 **IoTHubService.cs** 以進行編輯。 
4. 搜尋 **_iotHubConnectionString** 變數，並使用您稍早所記錄的裝置連接字串來更新此值。
5. 儲存您的變更。 
6. 使用 [建置並執行] 按鈕，或是 Windows 上的 **F5** 或 Mac 上的 **command + r** 快捷鍵，在裝置模擬器或實際裝置中執行專案。 

   ![執行專案](media/quickstart-send-telemetry-xamarinforms/run-sample.png)

7. 當模擬器開啟時，在範例應用程式中選取 [啟動]。

下列螢幕擷取畫面顯示應用程式將模擬的遙測傳送到 IoT 中樞時的一些範例輸出：![執行模擬的裝置](media/quickstart-send-telemetry-xamarinforms/view-d2c.png)
...
下列螢幕擷取畫面顯示您在本機終端機視窗中看到的遙測類型：![檢視遙測](media/quickstart-send-telemetry-ios/view-telemetry.png)
...
在本快速入門中，您已設定 IoT 中樞、註冊裝置，將模擬的遙測從 Xamarin.Forms 應用程式傳送到中樞，並從中樞讀取遙測。 
