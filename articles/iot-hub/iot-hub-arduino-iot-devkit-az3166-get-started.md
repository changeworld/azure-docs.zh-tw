...
1. 在 Azure Cloud Shell 中執行下列命令，以建立裝置身分識別。
   ...
   **MyNodeDevice**：您要註冊的裝置名稱。 使用如下所示的**MyNodeDevice** 。 如果您為裝置選擇不同的名稱，則必須在本文中使用該名稱，並先在應用程式範例中更新該裝置名稱，再執行應用程式。
   ...
   > [!NOTE]
   > 如果您在執行時發生錯誤`device-identity`，請安裝[適用于 Azure CLI 的 Azure IoT 擴充](https://github.com/Azure/azure-iot-cli-extension/blob/dev/README.md)功能。
   > 執行下列命令，將適用於 Azure CLI 的 Microsoft Azure IoT 擴充功能新增至您的 Cloud Shell 執行個體。 IoT 擴充功能會將 IoT 中樞、IoT Edge 和 IoT 裝置布建服務（DPS）特定的命令新增至 Azure CLI。
   > 
   > ```azurecli-interactive
...
1. 在 Azure Cloud Shell 中執行下列命令，以取得您剛註冊之裝置的_裝置連接字串_：
...
1. 在新開啟的專案視窗中， `F1`按一下以開啟命令選擇區，輸入並選取 [ **Azure IoT 裝置工作臺：布建 azure 服務 ...**]。遵循逐步指南，完成布建您的 Azure IoT 中樞並建立 IoT 中樞裝置。
...
> [!NOTE]
> 您可能會在測試期間發生錯誤，其中的 LED 不會閃爍，Azure 入口網站不會顯示來自裝置的傳入資料，但是 [裝置 OLED] 畫面會顯示為 [**正在**執行 ...]。若要解決此問題，請在 Azure 入口網站中，移至 IoT 中樞內的裝置，並將訊息傳送至裝置。 如果您在 VS Code 的序列監視器中看到下列回應，則可能會在路由器層級封鎖來自裝置的直接通訊。 檢查針對連線裝置所設定的防火牆和路由器規則。 此外，請確定輸出埠1833已開啟。
> 
> 錯誤： mqtt_client .c （ln 454）：錯誤：開啟端點的連接失敗  
> 資訊：  >>>線上狀態：已中斷連接  
> 錯誤： tlsio_mbedtls .c （ln 604）：基礎 IO 開啟失敗  
> 錯誤： mqtt_client .c （ln 1042）：錯誤： io_open 失敗  
> 錯誤： iothubtransport_mqtt_common .c （ln 2283）：連接到位址 atcsliothub.azure-devices.net 失敗。  
> 資訊：  >>>重新連接。  
> 資訊： IoThub 版本：1.3。6  
...
1. 在 VS Code 中， `F1`按一下，輸入並選取 [ **Azure IoT 中樞：設定 IoT 中樞連接字串**]。 將連接字串複製到其中。
...
使用`DevKitMQTTClient`的是來自 Microsoft Azure IoT sdk 的**iothub_client**包裝函式[，以及適用于 C](https://github.com/Azure/azure-iot-sdk-c/tree/master/iothub_client)的程式庫，可與 Azure IoT 中樞互動。
