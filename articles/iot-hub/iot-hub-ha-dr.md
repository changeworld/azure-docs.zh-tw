...
> [!CAUTION]
> - IoT 中樞內建事件端點的事件中樞相容名稱和端點會在容錯移轉之後變更，而已設定的取用者群組則會移除（這是將在 2020 5 月之前修正的 bug）。 使用事件中樞用戶端或事件處理器主機接收來自內建端點的遙測訊息時，您應該[使用 IoT 中樞連接字串](iot-hub-devguide-messages-read-builtin.md#read-from-the-built-in-endpoint)來建立連線。 這可以確保您的後端應用程式在容錯移轉後可繼續運作，而無需手動介入。 如果您直接在應用程式中使用事件中樞相容名稱和端點，您將需要重新設定[其使用的取用者群組，並在容錯移轉之後提取新的事件中樞相容端點](iot-hub-devguide-messages-read-builtin.md#read-from-the-built-in-endpoint)，以繼續作業。 如果您使用 Azure Functions 或 Azure 串流分析來連接內建端點，您可能需要執行**重新開機**。
>
