...
基於此目的，請在任何[支援的區域](#supported-regions)中布建新的 IoT 中樞， `minTlsVersion`並在`1.2`您 Azure Resource Manager 範本的 IoT 中樞資源規格中，將屬性設定為：
...
> [!NOTE]
> 屬性`minTlsVersion`是唯讀的，一旦建立 IoT 中樞資源之後就無法變更。 因此，您必須適當地測試並驗證您的*所有*IoT 裝置和服務都與 TLS 1.2 相容，以及預先[建議的密碼](#recommended-ciphers)。
...
> [!NOTE]
> 容錯移轉之後， `minTlsVersion`您的 IoT 中樞的屬性會在容錯移轉後的地理配對區域中維持有效。
