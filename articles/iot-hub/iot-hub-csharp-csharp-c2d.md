...
ms.locfileid: "77110208"
---
# <a name="send-messages-from-the-cloud-to-your-device-with-iot-hub-net"></a>使用 IoT 中樞將訊息從雲端傳送至裝置 (.NET)

[!INCLUDE [iot-hub-selector-c2d](../../includes/iot-hub-selector-c2d.md)]

Azure IoT 中樞是一項完全受控的服務，有助於讓數百萬個裝置和一個解決方案後端進行可靠且安全的雙向通訊。 將[遙測資料從設備發送到 IoT 中心](quickstart-send-telemetry-dotnet.md)快速入門顯示如何創建 IoT 中心、在其中預配設備標識以及編寫發送設備到雲消息的設備應用。

[!INCLUDE [iot-hub-basic](../../includes/iot-hub-basic-whole.md)]

本教程基於[將遙測資料從設備發送到 IoT 中心](quickstart-send-telemetry-dotnet.md)。 它演示如何執行以下任務：
...
在本教程結束時，您將運行兩個 .NET 主控台應用。

* **類比設備**。 此應用程式連接到 IoT 中心並接收雲到設備的消息。 此應用程式是在[將遙測資料從設備發送到 IoT 中心](quickstart-send-telemetry-dotnet.md)中創建的應用程式的修訂版本。

* **發送雲設備**。 此應用通過 IoT 中心向設備應用發送雲到設備的消息，然後接收其傳遞確認。

> [!NOTE]
> IoT 中心通過[Azure IoT 設備 SDK](iot-hub-devguide-sdks.md)對許多設備平臺和語言（包括 C、JAVA、Python 和 JAVAscript）具有 SDK 支援。 如需有關如何將您的裝置與本教學課程中的程式碼連接 (通常是連線至 Azure IoT 中樞) 的逐步指示，請參閱 [Azure IoT 中樞開發人員指南](iot-hub-devguide.md)。
>
...
在本節中，修改您在[將遙測資料從設備發送到 IoT 中心](quickstart-send-telemetry-dotnet.md)以接收來自 IoT 中心的雲到設備消息中創建的設備應用。
...
在本文中，您將創建一個後端服務，通過在[將遙測資料從設備發送到 IoT 中心](quickstart-send-telemetry-dotnet.md)中創建的 IoT 中心發送雲到設備的消息。 要發送雲到設備的消息，您的服務需要**服務連接**許可權。 預設情況下，每個 IoT 中心都使用授予此許可權的名為**服務的**共用訪問策略創建。
...
1. 將專案命名為 *SendCloudToDevice*。 在 **"解決方案**"下，選擇 **"添加到解決方案"** 並接受 .NET 框架的最新版本。 選取 [Create] \(建立\)**** 以建立專案。

   ![在視覺化工作室中配置新專案](./media/iot-hub-csharp-csharp-c2d/sendcloudtodevice-project-configure.png)

1. 在解決方案資源管理器中，按右鍵新解決方案，然後選擇 **"管理 NuGet 包**"。

1. 在 **"管理 NuGet 包"** 中，選擇 **"流覽**"，然後搜索並選擇**Microsoft.Azure.Device**。 選擇 **"安裝**"。

   此步驟下載、安裝並添加對[Azure IoT 服務 SDK NuGet 包](https://www.nuget.org/packages/Microsoft.Azure.Devices/)的引用。

1. 在**Program.cs**檔`using`的頂部添加以下語句。

   ``` csharp
   using Microsoft.Azure.Devices;
   ```

1. 將下列欄位新增到 **Program** 類別。 將預留位置值替換為以前在[獲取 IoT 中心連接字串](#get-the-iot-hub-connection-string)中複製的 IoT 中心連接字串。
...
1. 將下列方法新增至 Program**** 類別。 將設備名稱設置為在[將遙測資料從設備發送到 IoT 中心](quickstart-send-telemetry-dotnet.md)中定義設備時使用的內容。
   ...
   這個方法會將新的雲端到裝置訊息傳送給識別碼為 `myFirstDevice`的裝置。 僅當將此參數從[將遙測資料從設備發送到 IoT 中心](quickstart-send-telemetry-dotnet.md)時，才更改它。
...
1. 在 **"常見屬性** > **啟動專案**"中，選擇**多個啟動專案**，然後為 **"讀取設備到雲消息**"、**類比設備**以及**發送CloudToDevice**選擇 **"啟動**"操作。 選取 [ **確定** ] 以儲存變更。
...
可以請求從 IoT 中心對每個雲到設備消息的傳遞（或過期）確認。 這個選項可讓解決方案後端輕鬆地通知重試或補償邏輯。 如需有關雲端到裝置意見反應的詳細資訊，請參閱[使用 IoT 中樞的 D2C 和 C2D 傳訊](iot-hub-devguide-messaging.md)。
...
1. 在**Main**方法中添加以下行，在`serviceClient = ServiceClient.CreateFromConnectionString(connectionString)`之後。
