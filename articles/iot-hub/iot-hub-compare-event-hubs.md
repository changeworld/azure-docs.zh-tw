...
Azure 提供專為不同連線和通訊類型所開發的服務，協助您連接資料與雲端的強大功能。 Azure IoT 中樞與 Azure 事件中樞都是雲端服務，可以內嵌大量資料並處理或儲存那些資料以取得商業見解。 這兩個服務的相似之處在於其都支援以低延遲與高可靠性來內嵌資料，但兩者是針對不同目的而設計的。 IoT 中樞的開發目的是為了解決將 IoT 裝置連線到 Azure 雲端的獨特需求，而事件中樞則是專為大型資料串流所設計。 Microsoft 建議使用 Azure IoT 中樞將 IoT 裝置連線到 Azure
