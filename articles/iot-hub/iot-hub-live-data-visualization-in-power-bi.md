---
title: Data keyfromnode.frm Azure IoT 中樞的即時資料視覺效果– Power BI
...
您可了解如何藉由使用 Power BI 將 Azure IoT 中樞收到的即時感應器資料視覺化。 如果您想要嘗試使用 web 應用程式將 IoT 中樞的資料視覺化，請參閱 [使用 web 應用程式將 Azure IoT 中樞中的即時感應器資料視覺化](iot-hub-live-data-visualization-in-web-apps.md)。
...
3. 在 [ **輸入** ] 窗格中，選取 [ **新增資料流程輸入**]，然後從下拉式清單中選取 [ **IoT 中樞** ]。 在 [新輸入] 窗格中，輸入下列資訊：
   ...
   **共用存取原則名稱**：選取您想要串流分析作業針對您的 IoT 中樞使用的共用存取原則名稱。 在本教學課程中，您可以選取 [ *服務*]。 預設會在新的 IoT 中樞上建立 *服務* 原則，並授與 IoT 中樞所公開之雲端端端點上的傳送和接收許可權。 若要深入瞭解，請參閱 [存取控制和許可權](iot-hub-devguide-security.md#access-control-and-permissions)。
...
2. 在 [ **輸出** ] 窗格中，選取 [ **新增** ] 和 [ **Power BI**]。

3. 在 [ **Power BI-新的輸出** ] 窗格中，選取 [ **授權** ]，並遵循提示來登入您的 Power BI 帳戶。
...
在串流分析作業中，選取 **[總覽**]，然後選取 [立即**啟動**]  >  **Now**  >  ** **。 成功啟動作業後，作業狀態會從 [已停止]  變更為 [執行中]  。
...
7. 建立另一個折線圖以顯示一段時間的即時溼度。 若要這樣做，請按一下畫布的空白部分，並遵循上述相同步驟，將 **EventEnqueuedUtcTime** 放置在 X 軸上，並在 y 軸上進行 **濕度** 。

   ![將溼度的折線圖新增至 Microsoft Power BI 報告](./media/iot-hub-live-data-visualization-in-power-bi/power-bi-add-humidity.png)

8. 選取 [儲存]**** 以儲存報告。

9. 選取左窗格中的 [ **報表** ]，然後選取您剛才建立的報表。

10. 選取 **[** 檔案  >  **發佈至 web**]。

    ![選取 Microsoft Power BI 報表的 [發佈至網路]](./media/iot-hub-live-data-visualization-in-power-bi/power-bi-select-publish-to-web.png)

    > [!NOTE]
    > 如果您收到通知，告知您的系統管理員啟用內嵌程式碼建立，您可能需要聯繫它們。 您必須先啟用內嵌程式碼建立，才能完成此步驟。
    >
    > ![請洽詢您的系統管理員通知](./media/iot-hub-live-data-visualization-in-power-bi/contact-admin.png)

11. 選取 [ **建立內嵌程式碼**]，然後選取 [ **發行**]。

系統會提供您可以與任何人共用以進行報表存取的報表連結，以及可用來將報表整合到您的 blog 或網站的程式碼片段。
...
若要以另一種方式將 Azure IoT 中樞中的資料視覺化，請參閱 [使用 web 應用程式將 Azure IoT 中樞的即時感應器資料視覺化](iot-hub-live-data-visualization-in-web-apps.md)。
