...
2. 選取 [儲存體 Blob 資料參與者] ([而「不是」[參與者] 或 [儲存體帳戶參與者]](../storage/common/storage-auth-aad-rbac-portal.md#rbac-roles-for-blobs-and-queues)) 作為 [角色]、[Azure AD 使用者、群組或服務主體] 作為 [存取權指派對象為]，然後在下拉式清單中選取 IoT 中樞的資源名稱。 按一下 [儲存] 按鈕。
