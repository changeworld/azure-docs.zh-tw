...
[Azure CLI 的 IoT 擴展](https://github.com/Azure/azure-iot-cli-extension)是一個開源 IoT 擴展，它增加了[Azure CLI](https://docs.microsoft.com/cli/azure/overview?view=azure-cli-latest)的功能。 Azure CLI 包含用於與 Azure 資源管理器和管理終結點交互的命令。 例如，您可以使用 Azure CLI 來建立 Azure VM 或 IoT 中樞。 CLI 擴充功能可讓 Azure 服務強化 Azure CLI，讓您能存取其他服務專屬的功能。 IoT 擴展允許 IoT 開發人員命令列訪問所有 IoT 中心、IoT 邊緣和 IoT 中心設備佈建服務功能。
...
| 裝置對應項查詢        | 查詢所有設備孿生以檢索具有任意條件的孿生，例如標識可用的設備。 |
...
您學習將 Azure CLI 的 IoT 擴展用於開發電腦上的各種管理選項。
...
* 完成[樹莓派線上模擬器](iot-hub-raspberry-pi-web-simulator-get-started.md)教程或設備教程之一;例如，[樹莓派與node.js。](iot-hub-raspberry-pi-kit-node-get-started.md) 這些專案涵蓋以下要求：
...
* Azure CLI。 如果您需要安裝它，請參閱[安裝 Azure CLI](https://docs.microsoft.com/cli/azure/install-azure-cli?view=azure-cli-latest)。 至少，Azure CLI 版本必須為 2.0.70 或以上。 使用 `az –version` 進行驗證。

[!INCLUDE [iot-hub-cli-version-info](../../includes/iot-hub-cli-version-info.md)]

* 安裝 IoT 擴充功能。 最簡單的方式就是執行 `az extension add --name azure-iot`。 [IoT 擴充功能讀我檔案](https://github.com/Azure/azure-iot-cli-extension/blob/master/README.md)說明安裝此擴充功能的數種方式。
...
孿生報告的屬性之一為 $metadata.$lastUpdated，它顯示了設備應用上次更新其報告的屬性集的時間。
