---
title: 使用 Azure IoT 中心 （CLI） 進行大規模自動裝置管理 |微軟文檔
description: 使用 Azure IoT 中心自動設定管理多個 IoT 設備或模組
author: ChrisGMsft
manager: bruz
ms.service: iot-hub
services: iot-hub
...
ms.locfileid: "80235607"
---
# <a name="automatic-iot-device-and-module-management-using-the-azure-cli"></a>使用 Azure CLI 自動 IoT 設備和模組管理
...
* [Azure CLI 的 IoT 擴展](https://github.com/Azure/azure-cli)。
...
目標內容和指標查詢被指定為 JSON 文檔，用於描述要設置和報告要測量的屬性的設備孿生或模組孿生所需屬性。  要使用 Azure CLI 創建自動設定，請將目標內容和指標本地保存為 .txt 檔。 運行命令時，在後面部分中使用檔路徑將配置應用於設備。
...
自動模組配置的行為非常相似，但您的目標是`moduleContent`而不是`deviceContent`。
...
模組的指標查詢也類似于設備的查詢，但您可以從 中選擇`moduleId``devices.modules`。 例如： 
...
* --**目標條件**- 輸入目標條件，確定此配置將針對哪些設備或模組。對於自動設備配置，條件基於設備孿生標記或設備孿生所需屬性，並且應匹配運算式格式。例如，`tags.environment='test'` 或 `properties.desired.devicemodel='4000x'`。對於自動模組配置，條件基於模組孿生標記或模組孿生所需屬性。 例如，`from devices.modules where tags.environment='test'` 或 `from devices.modules where properties.reported.chillerProperties.model='4000x'`。
...
* **自訂指標**- 您定義的任何指標都是使用者指標。

可以使用以下命令顯示每個指標的裝置識別碼、模組 ID 或物件的清單：
...
* --**公制 ID** - 要查看裝置識別碼 或模組 ID 清單的指標的名稱，例如`appliedCount`。
