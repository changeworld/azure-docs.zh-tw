...
2. 使用文字編輯器，在工作目錄中創建名為**dmpatterns_getstarted_device.py**的檔。
...
4. 添加**CONNECTION_STRING**變數。 將`{deviceConnectionString}`預留位置值替換為設備連接字串。 之前在[IoT 中心中註冊新設備中](#register-a-new-device-in-the-iot-hub)複製了此連接字串。  
...
2. 使用文字編輯器，在工作目錄中創建名為**dmpatterns_getstarted_service.py**的檔。
...
4. 新增下列變數宣告。 將`{IoTHubConnectionString}`預留位置值替換為以前在[獲取 IoT 中心連接字串](#get-the-iot-hub-connection-string)中複製的 IoT 中心連接字串。 將`{deviceId}`預留位置值替換為您在[在 IoT 中心註冊新設備中](#register-a-new-device-in-the-iot-hub)註冊的裝置識別碼。
...
3. 您會在主控台中看到直接方法的裝置回應。
   ...
   ![類比設備應用輸出](./media/iot-hub-python-python-device-management-get-started/device.png)

   下面顯示了調用重新開機直接方法的服務，並輪詢設備孿生的狀態：

   ![觸發重新開機服務輸出](./media/iot-hub-python-python-device-management-get-started/service.png)
