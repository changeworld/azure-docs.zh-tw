...
`execute`類別中的方法會傳回 `AppMessageCallback` `IotHubMessageResult.COMPLETE` 。 這會通知 IoT 中樞訊息已成功處理，而且可以安全地從裝置佇列中移除訊息。 無論使用哪一種通訊協定，裝置的處理成功完成時，應該會傳回此值。
