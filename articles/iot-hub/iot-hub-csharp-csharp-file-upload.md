...
---
# <a name="upload-files-from-your-device-to-the-cloud-with-iot-hub-net"></a>使用 IoT 中心 (.NET) 將檔案從裝置上載到雲

[!INCLUDE [iot-hub-file-upload-language-selector](../../includes/iot-hub-file-upload-language-selector.md)]

本教程基於[IoT 中心教程中的「發送雲到設備訊息](iot-hub-csharp-csharp-c2d.md)」中的程式碼,向您展示如何使用 IoT 中心的檔上載功能。 這會說明如何：

* 安全地將 Azure Blob URI 提供給裝置，以便上傳檔案。

* 您可以使用 IoT 中樞檔案上傳通知來觸發在您的應用程式後端中處理此檔案。

通過 IoT 中心教程[將遙測數據從設備發送到 IoT 中心](quickstart-send-telemetry-dotnet.md)快速啟動和[發送雲到設備消息](iot-hub-csharp-csharp-c2d.md),顯示了 IoT 中心的基本設備到雲端和雲端到設備訊息傳遞功能。 [使用 IoT 中心配置消息路由](tutorial-routing.md)教程介紹了在 Microsoft Azure Blob 儲存中可靠地儲存設備到雲端訊息的方法。 但是,在某些情況下,您無法輕鬆地將設備發送的數據映射到 IoT 中心接受的相對較小的設備到雲端訊息中。 例如：
...
* **模擬裝置**。 此應用程式可以使用 IoT 中樞提供的 SAS URI，將檔案上傳到儲存體。 它是[使用IoT中心教程在「將雲到設備消息」](iot-hub-csharp-csharp-c2d.md)中創建的應用的修改版本。

* **閱讀檔案上傳通知**。 此應用從 IoT 中心接收檔上傳通知。

> [!NOTE]
> IoT 中心透過 Azure IoT 裝置 SDK 支援許多裝置平台和語言,包括 C、JAva、Python 和 Javascript。 如需如何將您的裝置連接到 Azure IoT 中樞的逐步指示，請參閱 [Azure IoT 開發人員中心](https://azure.microsoft.com/develop/iot)。
...
在本節中,您可以修改在[使用IoT中心發送雲到設備消息](iot-hub-csharp-csharp-c2d.md)中創建的設備應用,以便從IoT中心接收雲到設備消息。

1. 在可視化工作室解決方案資源管理器中,右鍵單擊**模擬設備**專案,然後選擇「**添加** > **現有專案**」。。 尋找影像檔並將其包含在專案中。 本教學課程假設影像名稱為 `image.jpg`。

1. 右鍵按一下圖像,然後選擇 **「屬性**」。 確定 [複製到輸出目錄]**** 是設為 [一律複製]****。
...
1. 在**Main**方法中添加以下行`Console.ReadLine()`,就在 : 之前:
...
在本文中,您將創建一個後端服務來接收從在[將遙測數據從設備發送到 IoT 中心](quickstart-send-telemetry-dotnet.md)中創建的 IoT 中心的檔上傳通知訊息。 要接收檔上傳通知消息,服務需要**服務連接**許可權。 默認情況下,每個 IoT 中心都使用授予此許可權的名為**服務的**共享存取策略創建。
...
1. 將專案命名為 *ReadFileUploadNotification*。 在**解決方案**「下,選擇 **」添加到解決方案**。 選取 [Create] \(建立\)**** 以建立專案。
    ...
    ![在視覺化工作室中設定 ReadFileUpload 通知專案](./media/iot-hub-csharp-csharp-file-upload/read-file-upload-project-configure.png)

1. 在解決方案資源管理器中,右鍵單擊**ReadFileUpload 通知**專案,然後選擇 **「管理 NuGet 包**」。

1. 在**NuGet 套件管理器中**,選擇 **「瀏覽**」。 搜尋並選擇**Microsoft.Azure.Device,** 然後選擇 **「安裝**」。

    此步驟在**ReadFileUpload 通知**專案中下載、安裝並添加對[Azure IoT 服務 SDK NuGet 包](https://www.nuget.org/packages/Microsoft.Azure.Devices/)的引用。

1. 在此項目的**Program.cs**檔案中,在檔案頂部加入以下語句:

    ```csharp
    using Microsoft.Azure.Devices;
    ```

1. 將下列欄位新增到 **Program** 類別。 將`{iot hub connection string}`佔位符值取代為以前在[取得 IoT 中心連接字串](#get-the-iot-hub-connection-string)中複製的 IoT 中心連接字串:
...
1. 在 **"常見屬性** > **啟動專案**"中,選擇**多個啟動專案**,然後選擇**ReadFileUpload 通知**和**模擬設備的****「啟動**」操作。 選取 [ **確定** ] 以儲存變更。
...
在本教學課程中，您已學到如何使用 IoT 中樞的檔案上傳功能來簡化從裝置上傳檔案。 您可以透過以下文章繼續探索 IoT 中心功能和方案:
