...
ms.locfileid: "81733140"
---
# <a name="get-started-with-device-twins-net"></a>開始使用裝置孿生 (.NET)

[!INCLUDE [iot-hub-selector-twin-get-started](../../includes/iot-hub-selector-twin-get-started.md)]

在本教學中,您將建立以下 .NET 主控台應用:

* **建立裝置識別**。 此應用創建設備標識和相關安全密鑰以連接模擬設備應用。

* **新增標籤與查詢**。 此後端應用添加標記和查詢設備孿生。

* **報告連線性**。 此設備應用類比使用較早創建的設備標識連接到IoT中心的設備,並報告其連接條件。
...
在本節中,您將使用 C# 創建 .NET 控制台應用,將位置元數據添加到與**myDeviceId**關聯的設備孿生。 接著，它會選取位於美國的裝置來查詢儲存在 IoT 中樞的裝置對應項，再查詢會報告行動電話連線的對應項。

1. 在 Visual Studio 中，選取 [建立新專案]****。 在 **"創建新專案**"中,選擇**主控台應用 (.NET 框架),** 然後選擇 **「下一步**」 。。

1. 在**設定新項目**中,命名專案**AddTag 和Query**。

    ![設定新增標籤與查詢項目](./media/iot-hub-csharp-csharp-twin-getstarted/config-addtagsandquery-app.png)

1. 在解決方案資源管理器中,右鍵單擊**AddTags 和查詢**專案,然後選擇 **「管理 NuGet 包**」。

1. 選擇 **「瀏覽**」與「搜尋」並選擇**Microsoft.Azure.Device**。 選取 [安裝]  。
...
1. 將下列欄位新增到 **Program** 類別。 取代為`{iot hub connection string}`在[取得 IoT 中心連接字串中複製的 IoT 中心連接字串](#get-the-iot-hub-connection-string)。
...
1. 將下列方法新增至 Program**** 類別：
    ...
    **RegistryManager** 類別會公開從服務來與裝置對應項進行互動時所需的所有方法。 先前的程式碼會先初始化 **registryManager** 物件，然後擷取 **myDeviceId** 的裝置對應項，最後會以所需的位置資訊來更新其標籤。
    ...
    先前的程式碼在建立**查詢**物件時，指定傳回的最大文件數。 **query** 物件包含 **HasMoreResults** 布林值屬性，可用來多次叫用 **GetNextAsTwinAsync** 方法以擷取所有結果。 名為**GetNextAsJson**的方法可用於不是設備孿生的結果,例如聚合查詢的結果。
...
1. 在 Visual Studio 中，選取 [檔案]   >  [新增]   >  [專案]  。 在**建立新項目**中,選擇**主控台應用程式 (.NET 框架),** 然後選擇 **「下一步**」 。。

1. 在 **「設定新專案」** 中,命名項目**報告連接。** 對於**解決方案**,選擇 **「添加到解決方案**」,然後選擇 **「創建**」。

1. 在解決方案資源管理器中,右鍵單擊**報表連接**專案,然後選擇 **「管理 NuGet 包**」。

1. 選擇 **「瀏覽**」和「搜尋」並選擇**Microsoft.Azure.Devices.用戶端**。 選取 [安裝]  。
...
1. 將下列欄位新增到 **Program** 類別。 替換為`{device connection string}`您在[IoT 中心中註冊新設備中注意到的設備](#register-a-new-device-in-the-iot-hub)連接字串。
...
1. 將下列方法新增至 Program**** 類別：
    ...
    **Client** 物件會公開從裝置來與裝置對應項進行互動時所需的所有方法。 上面顯示的代碼初始化**了客戶端**物件,然後檢索**myDeviceId**的設備孿生。
...
1. 在解決方案資源管理器中,右鍵單擊您的解決方案,然後選擇 **「設置啟動專案**」。。

1. 在 **"常見屬性** > **啟動項目' 中**,選擇**多個啟動項目**。 對於**報表連接,** 選擇 **「開始**作為**操作**」。 選取 [ **確定** ] 以儲存變更。  

1. 通過右鍵按一下**報表連接**專案並選擇 **「除錯**」來運行此應用程式,然後**選擇「啟動新實例**」 。 您應該看到應用程式獲取孿生資訊,然後作為***報告的屬性***發送連接。

    ![執行裝置應用程式以回報連線](./media/iot-hub-csharp-csharp-twin-getstarted/rundeviceapp.png)

   設備報告其連接資訊后,應出現在兩個查詢中。

1. 右鍵單擊**AddTags 和查詢**專案,然後選擇 **「調試** > **啟動」新實例**以再次運行查詢。 這一次,**我的DeviceId**應出現在兩個查詢結果中。
...
* 要瞭解如何從設備發送遙測數據,請參閱[將遙測數據從設備發送到 IoT 中心](quickstart-send-telemetry-dotnet.md)教程。

* 要瞭解如何使用設備孿生所需的屬性配置設備,請參閱[使用所需屬性配置設備](tutorial-device-twins.md)教程。

* 要瞭解如何以交互方式控制設備(例如從使用者控制的應用打開風扇),請參閱[使用直接方法](quickstart-control-device-dotnet.md)教程。
