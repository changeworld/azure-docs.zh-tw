...
* **裝置身分識別管理**。 每個 IoT 中樞都會公開一組 HTTPS REST 端點，用來管理裝置身分識別 (建立、擷取、更新和刪除)。 [設備標識](iot-hub-devguide-identity-registry.md)用於設備身份驗證和訪問控制。
...
* **裝置端點**。 針對身分識別登錄中的每個裝置，IoT 中樞會公開一組端點：
  ...
  * *傳送裝置到雲的訊息*。 裝置會使用這個端點來[傳送裝置到雲端的訊息](iot-hub-devguide-messages-d2c.md)。

  * 接收雲端到裝置的訊息**。 裝置使用此終結點接收目標[雲到裝置訊息](iot-hub-devguide-messages-c2d.md)。

  * *起始檔案上傳*。 裝置使用此終結點從 IoT 內端接收 Azure 儲存 SAS URI[來上傳檔案](iot-hub-devguide-file-upload.md)。
...
* **服務終結點**。 每個 IoT 中樞都會公開一組端點，供您的解決方案後端用來與您的裝置通訊。 但有一個例外狀況，這些端點只會在使用 [AMQP](https://www.amqp.org/) 通訊協定時公開。 方法引動過程端點會透過 HTTPS 通訊協定來公開。
  ...
  * *接收裝置到雲的消息*。 此端點與 [Azure 事件中樞](https://azure.microsoft.com/documentation/services/event-hubs/)相容。 後端服務可用它來讀取由您的裝置所傳送的[裝置到雲端訊息](iot-hub-devguide-messages-d2c.md)。 除了這個內建端點外，您可以在 IoT 中樞上建立自訂端點。
...
您可以使用 REST API[取得終結點執行狀況](https://docs.microsoft.com/rest/api/iothub/iothubresource/getendpointhealth#iothubresource_getendpointhealth)來取得終結點的執行狀況。 我們建議使用與路由消息延遲相關的[IoT 中心指標](iot-hub-metrics.md)來識別和調試終結點運行狀況死或不正常的錯誤,因為我們預計當終結點處於這些狀態之一時,延遲會更高。

|健康情況狀態|描述|
|---|---|
|healthy|終結點按預期接受消息。|
|不良|終結點未按預期接受消息,IoT 中心正在重試將數據發送到此終結點。 當 IoT 中心建立了最終一致的運行狀況時,不正常終結點的狀態將更新為正常。|
