...
2. 在**iot-java-twin-get 啟動**資料夾中,使用命令提示符處的以下指令建立名為**add-tag 查詢**的 Maven 專案:

    ```cmd/sh
    mvn archetype:generate -DgroupId=com.mycompany.app -DartifactId=add-tags-query -DarchetypeArtifactId=maven-archetype-quickstart -DinteractiveMode=false
    ```

3. 在命令提示符下,導航到**添加標記查詢**資料夾。

4. 使用文字編輯器,在**add-tags-查詢**資料夾中打開**pom.xml**檔,並將以下依賴項添加到**依賴項**節點。 此相依項讓您能夠在應用程式中使用**iot 服務客戶端**套件與 IoT 中心進行通訊:
...
7. 使用文字編輯器打開**添加標記查詢\src\main_java_com_mycompany_App.java**檔。
...
9. 將以下類級變數添加到**App**類。 取代為`{youriothubconnectionstring}`在[取得 IoT 中心連接字串中複製的 IoT 中心連接字串](#get-the-iot-hub-connection-string)。
...
15. 儲存並關閉**附加標記查詢\src\main_java_com_mycompany_App.java**檔案

16. 建置 **add-tags-query** 應用程式，並更正所有錯誤。 在指令提示符下,導航到**新增標籤查詢**資料夾並執行以下命令:
...
1. 在**iot-java-twin-get 啟動**資料夾中,使用命令提示符處的以下指令建立名為**模擬裝置的**Maven 專案:
...
3. 使用文字編輯器,在**模擬設備**資料夾中打開**pom.xml**檔,並將以下依賴項添加到**依賴項**節點。 此依賴項使您能夠在應用中使用**iot 設備用戶端**包與 IoT 中心進行通訊。
...
7. 使用文字編輯器打開**模擬設備\src_main_java_com_mycompany_App.java**檔。
...
15. 保存並關閉**模擬設備\src\main_java_com_mycompany_App.java**檔。

16. 建置 **simulated-device** 應用程式，並更正所有錯誤。 在指令提示符下,導航到**模擬裝置**資料夾並執行以下指令:
...
1. 在**新增標籤查詢**資料夾中的指令提示符下,執行以下指令以執行**新增標記查詢**服務應用:
...
2. 在**模擬裝置**資料夾中的指令提示符下,執行以下指令以將**連線類型**報告的屬性新增到裝置孿生:
...
3. 在**新增標籤查詢**資料夾中的指令提示符下,執行以下指令以第二次執行**新增標記查詢**服務應用:
    ...
    現在您的設備將**連接類型**屬性發送到 IoT 中心,第二個查詢將返回您的設備。
...
* 使用[IoT 中心](quickstart-send-telemetry-java.md)教程從設備發送遙測數據。

* 使用["使用直接方法'](quickstart-control-device-java.md)教程,以交互方式控制設備(例如從使用者控制的應用程式打開風扇)。
