...
[C 的 Azure IoT 裝置 SDK](iot-hub-device-sdk-c-intro.md)是本系列中介紹 C 的**Azure IoT 設備 SDK**的第一篇文章。本文解釋說,SDK 中有兩個體系結構層。 其基礎為 **IoTHubClient** 程式庫，可直接管理與 IoT 中樞之間的通訊。 還有 **序列化程式** 庫，此程式庫建置於其頂部以提供序列化服務。 在本文中,我們將提供有關**IoTHubClient**庫的其他詳細資訊。
...
這些函數都包括 API 名稱中的**LL。** 除了名稱的 **LL** 部份之外，每個函式的參數都與非 LL 的函式相同。 不過，這些函式的行為有一個重要的差異。

當您呼叫 **IoTHubClient\_CreateFromConnectionString** 時，基礎程式庫會建立在背景中執行的新執行緒。 此執行緒會將事件傳送到 IoT 中樞以及接收其訊息。 使用**LL** API 時不會建立此類線程。 背景執行緒的建立是為了方便開發人員。 您不必擔心顯式發送事件和接收來自 IoT 中心的消息 -- 它會自動在後台發生。 相反,**如果需要,LL** API 可以顯式控制與 IoT 中心的通信。
...
**LL** API 不建立後台線程。 而是必須呼叫新的 API 明確地與 IoT 中樞之間傳送和接收資料。 下列範例就將此進行示範。

SDK 中包含的**iothub\_\_\_用戶端範例 HTTP**應用程式展示較低等級的 API。 在該範例中，我們會使用類似以下的程式碼，將事件傳送給「IoT 中樞」︰
...
**IoTHubClient\_LL DoWork 經常在一個循環中調用的原因是,每次調用它時,它都會向 IoT 中心發送 一些緩衝事件,並檢索為設備排隊的下 一條消息。 \_***some**the next* 不能保證每個調用發送所有緩衝事件或檢索所有排隊的消息。 如果您想要傳送緩衝區中的所有事件，並繼續進行其他處理程序，您可以使用如下程式碼來取代此迴圈：
...
此程式碼會呼叫 **IoTHubClient\_LL\_DoWork**，直到緩衝區中的所有事件都已傳送到「IoT 中樞」為止。 請注意，這並未一併暗示已收到所有佇列的訊息。 部分原因在於,檢查"所有"消息並不是確定性操作。 如果您擷取「所有」訊息，但另一個訊息又隨即傳送至裝置，會發生什麼事？ 更好的處理方法是利用程式化的逾時。 例如,消息回調函數可以在每次調用計時器時重置計時器。 如果最後 *X* 秒都沒有收到任何訊息，您可以接著撰寫邏輯來繼續處理。

完成入侵事件和接收消息後,請確保調用相應的函數來清理資源。

```C
IoTHubClient_LL_Destroy(iotHubClientHandle);
```

基本上,只有一組 API 用於使用後台線程發送和接收數據,而另一組 API 在沒有後台線程的情況下執行相同的工作。 許多開發人員可能會偏好非 LL API，但是如果開發人員想要明確控制網路傳輸，則較低層級的 API 會相當有用。 例如，有些裝置會隨時間收集資料，並且只在指定的時間間隔輸入事件 (例如，每小時一次或一天一次)。 較低層級 API 可在您與 IoT 中樞之間傳送和接收資料時，提供您明確控制的功能。 其他人則會單純地偏好較低層級 API 所提供的簡單性。 一切動作都發生在主執行緒上，而不是有些工作會在背景中發生。
...
在前面的範例中,我們將屬性附加到發送到IoT中心的事件。 屬性也可以附加至從 IoT 中樞接收的訊息。 如果我們想要從訊息擷取屬性，可以在訊息回呼函式中使用如下的程式碼：
...
如先前所述，當訊息從 IoT 中樞送達時， **IoTHubClient** 程式庫會叫用註冊的回呼函式來回應。 此函式有一個傳回參數值得額外說明。 下面是**iothub\_\_用戶端\_範例 http**範例應用程式中回檔函數的摘錄:
...
在最後一個案例中，訊息也會傳送至 IoT 中樞，但它表示應重新傳遞訊息。 通常,如果遇到一些錯誤,但希望再次嘗試處理該消息,則會放棄郵件。 相反,當您遇到不可恢復的錯誤(或者如果您只是決定不想處理消息)時,拒絕消息是合適的。
...
這個字串中包含四項資訊：IoT 中樞名稱、IoT 中樞尾碼、裝置識別碼和共用存取金鑰。 當您在 Azure 入口網站中建立 IoT 中樞執行個體時，可以取得 IoT 中樞的完整網域名稱 (FQDN) - 這可為您提供 IoT 中樞名稱 (FQDN 的第一個部分) 和 IoT 中樞尾碼 (FQDN 的其餘部分)。 當您向 IoT 中心註冊裝置時,您將獲得設備 ID 和共享訪問密鑰(如[上一篇文章中](iot-hub-device-sdk-c-intro.md)所述)。
...
* **SetBatching** (bool) - 如果為 **true**，則傳送到「IoT 中樞」的資料會以批次傳送。 如果為 **false**，就表示訊息會個別傳送。 預設值為 **false**。 支援透過 AMQP / AMQP-WS 進行批次處理,以及在 D2C 消息上添加系統屬性。

* **Timeout** (unsigned int) - 這個值會以毫秒為單位表示。 如果傳送 HTTPS 要求或接收回應所花費的時間超過這個時間，連線就會逾時。

此批次處理選項極為重要。 根據預設，程式庫會個別輸入事件 (單一事件是您傳遞給 **IoTHubClient\_LL\_SendEventAsync** 的任何內容)。 如果批次處理選項為 **true**，程式庫會盡可能從緩衝區收集事件 (上限為 IoT 中樞將接受的最大訊息大小)。  事件批次會在單一 HTTPS 呼叫中傳送到「IoT 中樞」(個別事件會統合至 JSON 陣列中)。 啟用批次處理通常會導致性能的大幅提升,因為您減少了網路往返。 它也會大幅減少頻寬，因為您是利用事件批次傳送一組 HTTPS 標頭，而不是針對每個個別事件傳送一組標頭。 除非您有特定原因這樣做,否則通常需要啟用批處理。

## <a name="next-steps"></a>後續步驟

本文詳細介紹了**在 Azure IoT 設備 SDK 中**找到的**IoTHubClient**庫的行為。有了這些資訊,您應該能夠很好地瞭解**IoTHubClient**庫的功能。 這個系列的第二篇文章[適用於 C 的 Azure IoT 裝置 SDK - 序列化程式](iot-hub-device-sdk-c-serializer.md)提供了關於 **** 程式庫的相似詳細資料。
