...
1. 在 [方案總管]**** 中按兩下 **App.config**，以在 Visual Studio 編輯器中開啟該檔案。 在`<system.ServiceModel>`元素的底部，但仍在內`<system.ServiceModel>`，新增下列 XML 程式碼。 請務必以您`yourServiceNamespace`的命名空間名稱取代，並`yourKey`以您先前從入口網站中抓取的 SAS 金鑰取代：
