...
| 執行狀況偵測狀態 | 公用和內部負載平衡器 | 標準 Load Balancer 使用分散式健康情況探查服務，可根據您的組態設定監視應用程式端點的健康情況。 這個計量會提供負載平衡器集區中每個執行個體端點的彙總檢視，或各端點篩選過的檢視。 您可以看到 Load Balancer 藉由健康情況探查設定如何檢視應用程式的健康情況。 |  Average |
...
#### <a name="is-the-data-path-up-and-available-for-my-load-balancer-frontend"></a>數據路徑是否已打開,並且可用於負載平衡器前端?
<details><summary>展開</summary>

數據路徑可用性可用性指標描述區域內數據路徑到 VM 所在的計算主機的運行狀況。 此計量是 Azure 基礎結構的健康情況反映。 您可以使用此計量：
- 監視服務的外部可用性
- 挖掘更深入的資料，了解服務部署所在的平台是否狀況良好、您的客體 OS 或應用程式執行個體是否狀況良好。
- 找出事件是否與您的服務或基礎資料層相關。 請勿將此指標與運行狀況探測狀態("後端實例可用性")混淆。
...
#### <a name="are-the-backend-instances-for-my-load-balancer-responding-to-probes"></a>負載均衡器的後端實例響應探測器了嗎?
<details>
  <summary>展開</summary>
...
<details>
  <summary>展開</summary>
通過在單個圖表上使用數據路徑可用性和運行狀況探測狀態指標的組合,可以確定查找問題並解決問題的位置。 您可以確定 Azure 正常運作，並利用此知識確定地判斷設定或應用程式是根本原因。

您可以透過健康情況探查計量來了解 Azure 依據您所提供的設定如何看待部署的健康情況。 查看健康情況探查永遠是監視或判斷原因時最好的第一步驟。

您可以更進一步,使用資料路徑可用性指標來深入瞭解 Azure 如何查看負責特定部署的基礎數據平面的運行狀況。 當您合併這兩個計量時，就能找出錯誤可能的位置，如此圖中範例所示：
...
此圖表會顯示以下資訊：
- 託管 VM 的基礎結構不可用,圖表開頭的 0%。 稍後,基礎結構是健康的,VM是可訪問的,並且將多個 VM 放置在後端。 此資訊由數據路徑可用性的藍色跟蹤指示,該跟蹤後來為 100%。 
- 由紫色曲線指示的運行狀況探測狀態在圖表的開頭為 0%。 綠色圓圈區域突出顯示運行狀況探測狀態變得正常,此時客戶的部署能夠接受新流。
