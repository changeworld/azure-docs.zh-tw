...
| 封包計數器 |  公用和內部負載平衡器 | 標準 Load Balancer 會報告每個前端處理的封包。| Average |
  ...
  >[!NOTE]
  >當您透過 NVA 或防火牆 Syn 封包使用從內部負載平衡器散發流量時，位元組計數器和封包計數器計量無法使用，且會顯示為零。 
...
2. 設定適當的計量匯總類型。
3. 選擇性設定必要的篩選和群組。
4. （選擇性）設定時間範圍和匯總。 根據預設，會以 UTC 顯示。

  >[!NOTE] 
  >當您以每分鐘取樣一次資料的方式來解讀某些計量時，時間匯總就很重要。 如果 [時間匯總] 設定為 [5 分鐘]，而計量匯總類型 [總和] 用於 SNAT 配置之類的度量，則您的圖表會顯示總配置 SNAT 埠的五倍。 
...
Azure Standard Load Balancer 支援可輕鬆設定的多維度計量警示。 設定特定計量的自訂閾值，以觸發具有不同嚴重性層級的警示，以加強 touchless 資源監視體驗。
...
1. 建立新的警示規則
    1.  設定警示條件
    1.  選擇性新增動作群組以進行自動修復
    1.  指派可啟用直覺反應的警示嚴重性、名稱和描述

  >[!NOTE]
  >[警示條件] 設定視窗會顯示信號歷程記錄的時間序列。 有一個選項可以依據維度（例如後端 IP）篩選此時間序列。 這會篩選時間序列圖形，而**不**是警示本身。 您不能設定特定後端 IP 位址的警示。

### <a name="common-diagnostic-scenarios-and-recommended-views"></a><a name = "DiagnosticScenarios"></a>常見診斷案例與建議的檢視

#### <a name="is-the-data-path-up-and-available-for-my-load-balancer-frontend"></a>資料路徑是否已啟動，並可供我的 Load Balancer 前端使用？
<details><summary>展開</summary>

資料路徑可用性標準會描述區域內資料路徑的健全狀況，這些是 Vm 所在的計算主機。 此計量是 Azure 基礎結構的健康情況反映。 您可以使用此計量：
- 監視服務的外部可用性
- 挖掘更深入的資料，了解服務部署所在的平台是否狀況良好、您的客體 OS 或應用程式執行個體是否狀況良好。
- 找出事件是否與您的服務或基礎資料層相關。 請勿將此度量與健康情況探查狀態（「後端實例可用性」）混淆。
...
<details>
  <summary>展開</summary>
SNAT 使用計量會指出在一個負載平衡器後方的網際網路來源與後端 VM 或虛擬機器擴展集之間建立了多少個唯一的流程，而且沒有公用 IP 位址。 藉由比較此與 SNAT 配置計量，您可以判斷您的服務是否遇到 SNAT 耗盡的風險，以及產生的輸出流量失敗。 

如果您的度量指出[輸出流程](https://aka.ms/lboutbound)失敗的風險，請參考文章並採取步驟來減輕此問題，以確保服務健康狀態。

若要查看 SNAT 埠的使用方式和配置：
1. 將圖形的時間匯總設定為1分鐘，以確保顯示所需的資料。
1. 選取 [ **Snat 使用**和/或**snat 配置**] 作為 [計量類型] 和 [**平均**] 做為匯總
    * 根據預設，這是配置給每個後端 Vm 或 Vmss 所配置或使用的 SNAT 埠平均數，其對應至所有對應至 Load Balancer 的前端公用 Ip，並透過 TCP 和 UDP 進行匯總。
    * 若要查看針對負載平衡器所使用或配置的 SNAT 埠總計，請使用計量匯總**總和**
1. 篩選至特定的**通訊協定類型**、一組**後端 ip**和/或**前端 ip**。
1. 若要監視每個後端或前端實例的健全狀況，請套用分割。 
    * 請注意，分割只允許一次顯示單一度量。 
1. 例如，若要監視每部機器的 TCP 流量的 SNAT 使用量，依**平均**匯總，依**後端 ip**分割並依**通訊協定類型**篩選。 

![SNAT 配置和使用方式](./media/load-balancer-standard-diagnostics/snat-usage-and-allocation.png)

*圖：一組後端 Vm 的平均 TCP SNAT 埠配置和使用方式*

![由後端實例所使用的 SNAT](./media/load-balancer-standard-diagnostics/snat-usage-split.png)
...
<details>
  <summary>展開</summary>
藉由在單一圖表上使用資料路徑可用性和健康情況探查狀態計量的組合，您可以找出問題所在並解決問題。 您可以確定 Azure 正常運作，並利用此知識確定地判斷設定或應用程式是根本原因。

您可以透過健康情況探查計量來了解 Azure 依據您所提供的設定如何看待部署的健康情況。 查看健康情況探查永遠是監視或判斷原因時最好的第一步驟。

您可以進一步使用資料路徑可用性計量來深入瞭解 Azure 如何看待負責特定部署之基礎資料平面的健全狀況。 當您合併這兩個計量時，就能找出錯誤可能的位置，如此圖中範例所示：
...
此圖表會顯示以下資訊：
- 裝載 Vm 的基礎結構無法使用，且在圖表的開頭為0%。 之後，基礎結構狀況良好，且 Vm 可以連線，而且後端中有一個以上的 VM。 這項資訊是由資料路徑可用性的藍色追蹤所表示，而這是晚于100%。 
- 健康情況探查狀態（以紫色追蹤表示）在圖表的開頭為0%。 綠色的圓形區域會反白顯示健全狀況探查狀態變成狀況良好的位置，而在該時間點，客戶的部署能夠接受新的流程。
