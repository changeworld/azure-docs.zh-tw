...
description: 本文介紹如何將 Azure 基本內部負載平衡器升級到標準公共負載等化器
...
---
# <a name="upgrade-azure-internal-load-balancer---outbound-connection-required"></a>升級 Azure 內部負載等化器 - 需要出站連接
[Azure 標準負載等化器](load-balancer-overview.md)通過區域冗余提供了豐富的功能集和高可用性。 要瞭解有關負載等化器 SKU 的更多，請參閱[比較表](https://docs.microsoft.com/azure/load-balancer/concepts-limitations#skus)。 由於標準內部負載等化器不提供出站連接，因此我們提供了創建標準公共負載等化器的解決方案。

升級分為四個階段：

1. 將配置遷移到標準公共負載等化器
2. 將 VM 添加到標準公共負載等化器的後端池
3. 在出站連接的負載平衡器上創建出站規則
4. 為應避免從互聯網到互聯網的子網/VM 設置 NSG 規則
...
* 在指定的資源組和位置中創建標準 SKU 公共負載等化器。
* 將基本 SKU 內部負載等化器的配置無縫複製到新創建的標準公共負載等化器。

### <a name="caveatslimitations"></a>警告\限制

* 腳本支援需要出站連接的內部負載等化器升級。 如果任何 VM 不需要出站連接，請參閱[此頁面](upgrade-basicInternal-standard.md)進行最佳實踐。
* 標準負載等化器具有新的公共位址。 不可能將與現有基本內部負載平衡器關聯的 IP 位址無縫移動到標準公共負載等化器，因為它們具有不同的 SKU。
* 如果標準負載等化器是在不同區域中創建的，您將無法將舊區域中現有的 VM 與新創建的標準負載等化器相關聯。 要解決此限制，請確保在新區域中創建新的 VM。
* 如果您的負載等化器沒有任何前端 IP 配置或後端池，則運行腳本時可能會遇到錯誤。  確保它們不為空。
...
1. 檢查所需的參數：
   ...
   * **新定位： [String]： 必需**– 這是將創建標準負載等化器的位置。 我們建議將所選基本負載等化器的相同位置繼承到標準負載等化器，以便更好地與其他現有資源關聯。
...
### <a name="create-nsg-rules-for-vms-which-to-refrain-communication-from-or-to-the-internet"></a>為 VM 創建 NSG 規則，這些規則可避免與 Internet 通信
如果要避免 Internet 流量到達 VM，則可以在 VM 的網路介面上創建[NSG 規則](https://docs.microsoft.com/azure/virtual-network/manage-network-security-group)。
