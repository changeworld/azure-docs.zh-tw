...
[Azure Standard Load Balancer](load-balancer-overview.md) 透過區域冗余提供一組豐富的功能和高可用性。 若要深入瞭解 Load Balancer SKU，請參閱 [比較表](https://docs.microsoft.com/azure/load-balancer/skus#skus)。 由於 Standard Internal Load Balancer 不提供輸出連線，因此我們會提供解決方案來改為建立標準公用 Load Balancer。
