...
| [跨可用性區域的負載平衡 VM](./scripts/load-balancer-linux-cli-sample-zone-redundant-frontend.md) | 在某區域的不同可用性區域中建立三個 VM，並建立一個具有區域備援前端 IP 位址的 Standard Load Balancer。 萬一整個資料中心失敗或遺失，這個負載平衡設定有助於保護您的應用程式和資料免於受害。 |
