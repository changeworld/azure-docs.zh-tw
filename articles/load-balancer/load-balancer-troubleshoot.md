...
description: 瞭解如何使用 Azure 負載等化器解決已知問題。
...
本頁提供基本和標準常見 Azure 負載等化器問題的故障排除資訊。 如需標準 Load Balancer 的詳細資訊，請參閱[標準 Load Balancer 概觀](https://docs.microsoft.com/azure/load-balancer/load-balancer-standard-diagnostics)。
...
當後端 VM 的外部用戶端通過負載等化器時，用戶端的 IP 位址將用於通信。 確保用戶端的 IP 位址已添加到 NSG 允許清單中。 
...
* 如果這些 ping 測試都沒有收到任何回應，則
    ...
    - 分析網路擷取，並查看是否有與 ping 查詢相關的傳入和傳出封包。 
        ...
        - 如果在後端池 VM 上未觀察到傳出資料包，則需要檢查 VM 是否有任何不相關的問題（例如，應用程式阻止探測埠）。 
...
對於公共負載等化器，Internet 用戶端的 IP 位址將用於用戶端和負載等化器後端 VM 之間的通信。 確保後端 VM 的網路安全性群組中允許用戶端的 IP 位址。

1. 列出在後端 VM 上設定的網路安全性群組。 有關詳細資訊，請參閱[管理網路安全性群組](../virtual-network/manage-network-security-group.md)
1. 從網路安全性群組的清單中，檢查︰
    - 資料連接埠上的傳入或傳出流量是否有干擾。 
...
## <a name="symptom-cannot-change-backend-port-for-existing-lb-rule-of-a-load-balancer-which-has-vm-scale-set-deployed-in-the-backend-pool"></a>症狀：無法更改負載等化器的現有 LB 規則的後端埠，該規則在後端池中部署了 VM Scale 集。 
### <a name="cause--the-backend-port-cannot-be-modified-for-a-load-balancing-rule-thats-used-by-a-health-probe-for-load-balancer-referenced-by-vm-scale-set"></a>原因 ：無法為運行狀況探測用於 VM Scale Set 引用的負載等化器的負載平衡規則修改後端埠。
**解析度**為了更改埠，可以通過更新 VM 縮放集、更新埠然後再次配置運行狀況探測來刪除運行狀況探測。
