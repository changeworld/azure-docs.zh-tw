...
為所需的負載`LoadBalancerDistribution`平衡量設定元素的值。 針對兩個元組（來源 IP 和目的地 IP）負載平衡指定 sourceIP。 針對三個元組（來源 IP、目的地 IP 及通訊協定類型）負載平衡指定 sourceIPProtocol。 針對五個元組負載平衡的預設行為指定 [無]。
