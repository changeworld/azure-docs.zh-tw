...
Load Balancer 是一種穿透服務 (不會終止 TCP 連線)，且流程一律介於用戶端和虛擬機器的客體作業系統與應用程式之間。 具有所有探查關閉的集區會導致前端無法回應 (SYN) 的 TCP 連線開啟嘗試，因為沒有狀況良好的後端端點可接收流程，並以 SYN ACK 回應。
