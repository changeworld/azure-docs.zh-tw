...
>[!IMPORTANT]
>不論設定的超時閾值為何，如果伺服器傳回任何不是 HTTP 200 正常的狀態碼，或是透過 TCP 重設來終止連接，HTTP (S) Load Balancer 健全狀況探查都會自動向下探查實例。
...
1. 如果您的應用程式在第一次探查抵達之前開始產生超時探查回應，則這些事件的偵測將需要10秒的時間 (2 x 5 秒間隔) 加上應用程式的持續時間，以通知第一個探查抵達的時間。  您可以假設此偵測會稍微超過10秒。
2. 如果您的應用程式在第一次探查抵達之後才開始產生超時探查回應，這些事件的偵測將不會開始，直到下一個探查抵達 (並將) 再加上另一個10秒， (2 x 5 秒的間隔) 。  您可以假設此偵測只需要15秒的時間。

在此範例中，一旦發生偵測，平臺就會花很短的時間來回應這項變更。  這表示會根據 

1. 當應用程式開始變更狀態時，
2. 當偵測到這項變更並符合所需的準則時 (在指定的間隔內傳送的探查數目) 和
3. 當偵測已通過平臺通訊時 
...
Load Balancer 是一種穿透服務 (不會終止 TCP 連線)，且流程一律介於用戶端和虛擬機器的客體作業系統與應用程式之間。 所有探查關閉的集區會導致前端不會回應 TCP 連線的開啟嘗試 (SYN) 因為沒有狀況良好的後端端點可接收流量，並以 SYN ACK 回應。
