...
高可用性 （HA） 埠負載平衡規則是負載平衡規則的變體，在內部標準負載等化器上配置。 您可以透過提供單一規則來對抵達內部 Standard Load Balancer 所有連接埠的所有 TCP 和 UDP 流量進行負載平衡，以簡化對於負載平衡器的使用。 每次都會針對流量進行負載平衡決策。 此動作會以下列五個 Tuple 連線為基礎：來源 IP 位址、來源連接埠、目的地 IP 位址、目的地連接埠和通訊協定。

HA 埠負載平衡規則可説明您解決關鍵方案，例如虛擬網路內部網路虛擬裝置 （NVA） 的高可用性和擴展。 此功能也可以在必須對大量連接埠進行負載平衡時提供協助。 

將前端和後端埠設置為**0**以及將協定設置為 **"全部**"時配置 HA 埠負載平衡規則。 接著，內部負載平衡器資源會對所有 TCP 和 UDP 流程進行平衡，而不論連接埠號碼為何。
...
您可以為後端資源配置*一個*公共標準負載等化器資源，以及帶有 HA 埠的單個內部標準負載等化器。
...
- HA 埠負載平衡規則僅適用于內部標準負載平衡器。
- 不支援 HA 連接埠負載平衡規則與非 HA 連接埠負載平衡規則的組合。
- 現有的 IP 片段將由 HA 埠負載平衡規則轉發到第一個資料包相同的目標。  不支援將 UDP 或 TCP 資料包分割的 IP。
- 後端實例和單個 NIC（和單個 IP 配置）僅支援流對稱性（主要用於 NVA 方案），並且僅在使用上圖所示並使用 HA Ports 負載平衡規則時才支援流對稱性。 任何其他案例中均不提供此支援。 這表示兩個或多個 Load Balancer 資源及其各自規則均會進行獨立決策，而且絕對不會進行協調。 請參閱[網路虛擬設備](#nva)的描述和圖表。 當您使用多個 NIC 或將 NVA 夾在公共負載等化器和內部負載等化器之間時，流量對稱不可用。  您可以藉由對輸入流程進行來源 NAT 為應用裝置 IP，以允許回覆到達相同 NVA，來解決這個問題。  不過，我們強烈建議使用單一 NIC 和使用上述圖表所示的參考架構。


## <a name="next-steps"></a>後續步驟

- [在內部標準負載等化器上配置 HA 埠](load-balancer-configure-ha-ports.md)
- [瞭解標準負載等化器](load-balancer-standard-overview.md)
