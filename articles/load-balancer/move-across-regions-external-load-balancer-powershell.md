...
2. 取得想要移至目標區域的公用 IP 資源識別碼，並使用 [Get-AzPublicIPAddress](/powershell/module/az.network/get-azpublicipaddress?view=azps-2.6.0) 將其放在變數中：
    ...
    ```
3. 將來源公用 IP 匯出至 json 檔案，以執行命令 [匯出->new-azresourcegroup](/powershell/module/az.resources/export-azresourcegroup?view=azps-2.6.0)的目錄：
...
8. 您也可以根據需求而選擇變更範本中的其他參數：
    ...
    * **SKU** - 您可變更 **\<resource-group-name>.json** 檔案中的 **SKU** > **名稱** 屬性，以將設定中的公用 IP SKU 從標準變更為基本，或從基本變更為標準：
         ...
         如需基本和標準 SKU 公用 IP 之間差異的詳細資訊，請參閱[建立、變更或刪除公用 IP 位址](../virtual-network/virtual-network-public-ip-address.md) (機器翻譯)。
    ...
    * **公用 IP 配置方法** 和 **閒置逾時** - 您可將 **publicIPAllocationMethod** 屬性從 **動態** 變更為 **靜態**，或從 **靜態** 變更為 **動態**，以在範本中變更這兩個選項。 將 **idleTimeoutInMinutes** 屬性變更為所要的數量，即可變更閒置時間。  預設值為 **4**：
        ...
        如需配置方法和閒置逾時值的詳細資訊，請參閱[建立、變更或刪除公用 IP 位址](../virtual-network/virtual-network-public-ip-address.md) (機器翻譯)。
...
2. 取得您想要移至目的地區域的外部負載平衡器的資源識別碼，並使用 [>new-azloadbalancer](/powershell/module/az.network/get-azloadbalancer?view=azps-2.6.0)將它放置在變數中：

    ```azurepowershell-interactive
    $sourceExtLBID = (Get-AzLoadBalancer -Name <source-external-lb-name> -ResourceGroupName <source-resource-group-name>).Id

    ```
3. 將來源外部負載平衡器設定匯出至 json 檔案，以執行命令 [匯出->new-azresourcegroup](/powershell/module/az.resources/export-azresourcegroup?view=azps-2.6.0)的目錄：
...
6.  若要編輯上方移動之目標公用 IP 的值，您必須先取得資源識別碼，然後將它複製並貼到 **\<resource-group-name> json** 檔案中。  若要取得識別碼，請使用 [>get-azpublicipaddress](/powershell/module/az.network/get-azpublicipaddress?view=azps-2.6.0)：
...
12. 您也可以根據需求而選擇變更範本中的其他參數：
    ...
    * **輸出規則**-您可以藉由編輯 **\<resource-group-name> Json** 檔案中的 **outboundRules** 屬性，來新增或移除設定中的輸出規則：
        ...
        ```
         ...
         如需輸出規則的詳細資訊，請參閱 [Load Balancer 輸出規則](./load-balancer-outbound-connections.md#outboundrules)

13. 儲存 **\<resource-group-name>.json** 檔案。
    
10. 使用 [新的->new-azresourcegroup](/powershell/module/az.resources/new-azresourcegroup?view=azps-2.6.0)，在目的地區域中建立或要部署目標外部負載平衡器的資源群組。 您也可以在此程式中重複使用上述的現有資源群組：
...
部署之後，如果您想要在目標中重新開機或捨棄公用 IP 和負載平衡器，請刪除在目標中建立的資源群組，然後將刪除已移動的公用 IP 和負載平衡器。  若要移除資源群組，請使用 [Remove-AzResourceGroup](/powershell/module/az.resources/remove-azresourcegroup?view=azps-2.6.0)：
...
若要認可變更並完成 NSG 的移動，請刪除來源 NSG 或資源群組、使用 [>new-azresourcegroup](/powershell/module/az.resources/remove-azresourcegroup?view=azps-2.6.0) 或 [移除->get-azpublicipaddress](/powershell/module/az.network/remove-azpublicipaddress?view=azps-2.6.0) 和 [移除->new-azloadbalancer](/powershell/module/az.network/remove-azloadbalancer?view=azps-2.6.0)
