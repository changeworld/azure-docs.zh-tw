...
ms.locfileid: "77425896"
---
# <a name="multitenant-applications-in-azure"></a>Azure 中的多租用戶應用程式
多租戶應用程式是一種共用資源，它允許"獨立租戶中的使用者"像查看自己的應用程式一樣查看應用程式。 適合多租戶應用程式的典型方案是，來自不同租戶的應用程式的所有使用者可能希望自訂使用者體驗，但除此之外具有相同的基本業務需求。 舉例來說，Office 365、Outlook.com 和 visualstudio.com 都屬於大型多租用戶應用程式。
...
簡而言之，雖然為了提供高度可擴展的服務，必須考慮許多注意事項，但許多多租戶應用程式也有許多共同的目標和要求。 有些可能與特定案例無關，且個別目標和需求的重要性也可能隨著案例而不同。 作為多租戶應用程式的供應商，您還將有目標和要求，例如，滿足租戶的目標和要求、盈利能力、計費、多個服務等級、預配、可維護性監視和自動化。
...
資料管理（如 Azure SQL 資料庫或 Azure 存儲服務（如表服務）提供服務，用於存儲大量非結構化資料和 Blob 服務，該服務提供存儲大量非結構化文本或二進位資料，如視頻、音訊和圖像。

* 保護 SQL 資料庫中每個租戶 SQL Server 登入名中的多租戶資料。
* 通過指定容器級別訪問策略，可以使用 Azure 表用於應用程式資源，可以調整許可權，而無需為受共用訪問簽名保護的資源頒發新的 URL。
...
* Azure Active Directory (Azure AD) 是以 REST 為基礎的新式服務，可為您的雲端應用程式提供身分識別管理和存取控制功能。 使用適用于應用程式資源的 Azure AD 提供了一種簡單的方法來驗證和授權使用者訪問 Web 應用程式和服務，同時允許將身份驗證和授權功能從代碼中考慮在內。
* Azure 服務匯流排可為分散式和混合式應用程式提供安全訊息和資料流程功能 (例如 Azure 代管的應用程式與內部部署應用程式和服務之間的通訊)，而無須使用複雜的防火牆和安全性基礎結構。 使用應用程式資源的服務匯流排中繼訪問作為終結點公開的服務可能屬於租戶（例如，託管在系統外部（如本地），或者這些服務可能是專門為租戶預配的服務（因為敏感、特定于租戶的資料在它們之間傳輸）。
