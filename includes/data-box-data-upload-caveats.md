...
---
- 不要將檔直接複製到任何預先創建的共用。 您需要在共用下創建一個資料夾，然後將檔案複製到該資料夾。
...
- Azure Blob 存儲不支援目錄。 如果您在 StorageAccount_BlockBlob** 資料夾底下建立資料夾，則會以 Blob 名稱建立虛擬資料夾。 對於 Azure 檔案服務，則會維持實際的目錄結構。
- 不會上載*在StorageAccount_BlockBlob*和*StorageAccount_PageBlob*資料夾下創建的任何空目錄層次結構（沒有任何檔）。
- 如果將資料上傳至 Azure 時發生任何錯誤，則會在目標儲存體帳戶中建立錯誤記錄。 在上傳完成後，系統會提供此錯誤記錄的路徑，而您可以檢閱記錄以執行更正動作。 如果不驗證上載的資料，不要從源中刪除資料。
