...
IoT 裝置與 IoT 中樞之間的網際網路連線，是使用傳輸層安全性 (TLS) 標準進行保護。 Azure IoT 依此順序支援 [tls 1.2](https://tools.ietf.org/html/rfc5246)、tls 1.1 和 tls 1.0。 針對 TLS 1.0 的支援僅為提供回溯相容性。 檢查 [IoT 中樞的 tls 支援](../articles/iot-hub/iot-hub-tls-support.md) ，以瞭解如何將中樞設定為使用 tls 1.2，因為它提供最高的安全性。
...
* **RegistryRead**。 為身分識別登錄授與讀取權限。 如需詳細資訊，請參閱 [identity registry](../articles/iot-hub/iot-hub-devguide-identity-registry.md)。

* **RegistryReadWrite**。 為身分識別登錄授與讀取和寫入權限。 如需詳細資訊，請參閱 [identity registry](../articles/iot-hub/iot-hub-devguide-identity-registry.md)。
