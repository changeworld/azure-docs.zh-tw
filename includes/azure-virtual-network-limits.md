...
> [!NOTE]
> 我們最近已將所有預設限制提升至其最大限制。 如果沒有最大限制列,則資源沒有可調整的限制。 如果您過去因支援而增加這些限制,並且未在下表中看到更新的限制,[請免費開啟線上客戶支援請求](../articles/azure-resource-manager/templates/error-resource-quota.md)
...
| 公共 IP 前置字串 | /28 | 請連絡支援人員。 |

<sup>1</sup>公共 IP 位址的預設限制因產品/服務類別類型而異,例如免費試用、即用即付、CSP。 例如,企業協定訂閱的預設值為 1000。

#### <a name="load-balancer-limits"></a><a name="load-balancer"></a>負載均衡器限制
下列限制僅適用於透過每個訂用帳戶每一區域的 Azure Resource Manager 所管理的網路資源。 瞭解如何[根據訂閱限制目前資源使用方式](../articles/networking/check-usage-against-limits.md)。
...
| 每個 NIC 的規則(跨 NIC 上的所有 IP) | 300                          |
| 前端 IP 設定              | 200                          |
| 後端集區大小                       | 300 個 IP 設定,單一可用性集 |
| 每個負載均衡器可用性集     | 150                          |

<a name="virtual-networking-limits-classic"></a>以下限制僅適用於通過每個訂閱**的經典**部署模型管理的網路資源。 瞭解如何[根據訂閱限制目前資源使用方式](../articles/networking/check-usage-against-limits.md)。
