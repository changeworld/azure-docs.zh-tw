...
| QM SA 存留期   | 秒數（整數;**最小 300**/default 27000 秒）<br>Kb （整數;**最小 1024**/Default 102400000 kb）           |
...
不可以。 IPsec/IKE 原則只適用於透過 Azure VPN 閘道的 S2S VPN 連線和 VNet 對 VNet 連線。
...
不可以。 建立連線之後，將無法變更 IKEv1/IKEv2 通訊協定。 您必須刪除並重新建立具有所需通訊協定類型的新連線。
