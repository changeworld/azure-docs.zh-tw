...
針對生產案例，請在兩個可用性區域間部署多個資料庫的實例 (如果在 Azure) 中部署，或在 OCI) 中 (兩個可用性網域。 使用 Oracle Active Data Guard 來同步處理主要和待命資料庫。

資料庫層只會從仲介層接收要求。 如果在 OCI 中部署資料庫，建議您設定網路安全性群組 (安全性清單) 僅允許來自仲介層的埠1521上的要求，以及來自防禦伺服器的埠22，以因應系統管理的原因。

針對部署在 OCI 中的資料庫，您必須使用動態路由閘道設定不同的虛擬雲端網路， (DRG.4 連線到您的 FastConnect 電路的) 。