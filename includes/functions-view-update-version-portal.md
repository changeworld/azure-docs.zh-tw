...
1. 在 [設定] **Configuration**下，選擇 [**設定**]。 在 [**函數執行時間設定**] 索引標籤中，找出**執行階段版本**。 請注意特定的執行階段版本。 在下列範例中，版本設定為 `~3`。
...
1. 當您變更執行階段版本時，請回到 [概觀]**** 索引標籤，然後選擇 [重新啟動]**** 來重新啟動應用程式。  函式應用程式會重新開始在 1.x 版執行階段上執行，且當您建立函式時，所使用的會是 1.x 版範本。
    ...
    :::image type="content" source="./media/functions-view-update-version-portal/functions-restart-function-app.png" alt-text="重新開機您的函數應用程式。" border="true":::
