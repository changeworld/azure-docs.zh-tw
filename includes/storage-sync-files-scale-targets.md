...
| 資源 | 目標 | 固定限制 |
|----------|--------------|------------|
| 每個區域的儲存體同步服務數目 | 100儲存體同步服務 | Yes |
| 每個儲存體同步服務的同步群組 | 200同步群組 | Yes |
...
| 目錄中的檔案系統物件 (目錄和檔案) 數目上限 | 5000000物件 | Yes |
| 物件 (目錄和檔案) 安全性描述元大小上限 | 64 KiB | Yes |
| 檔案大小 | 100 GiB | No |
| 要分層之檔案的檔案大小下限 | V9：以檔案系統叢集大小（雙重檔案系統叢集大小）為基礎。 例如，如果檔案系統叢集大小為 4 kb，則檔案大小下限會是 8 kb。<br> V8 和更舊版本： 64 KiB  | Yes |
