...
除非您想要 App Service 為您執行部署自動化，否則請執行所有的組建工作 (例如、、 `npm` `bower` `gulp` 、 `composer` 和 `pip`) ，並確定您擁有執行應用程式所需的所有檔案。 如果您想要 [直接執行套件](../articles/app-service/deploy-run-package.md)，則需要此步驟。
