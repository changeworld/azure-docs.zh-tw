...
您可以在解決方案的 [ **Device Explorer** ] 頁面上，查看從您的裝置傳送的遙測資料。

1. 在 [ **Device Explorer** ] 頁面上的裝置清單中，選取您布建的裝置。 面板會顯示您裝置的相關資訊，包括裝置遙測繪圖：
...
若要在您的裝置上叫用方法，請使用遠端監視解決方案中的 [ **Device Explorer** ] 頁面。 例如，在遠端監視解決方案中， **Chiller**裝置會執行**重新開機**方法。

1. 選擇 [**裝置**] 以流覽至解決方案中的 [ **Device Explorer** ] 頁面。

1. 在 [ **Device Explorer** ] 頁面上的裝置清單中，選取您布建的裝置：

    ![選取您的實體裝置](media/iot-suite-visualize-connecting/devicesselect.png)

1. 若要顯示您可以在裝置上呼叫的方法清單，請依序選擇 [**作業**] 和 [**方法**]。 若要排程可在多個裝置上執行的作業，您可以在清單中選取多個裝置。 [作業]**** 面板會顯示所有您選取之裝置的通用方法。
...
> [!NOTE]
> 若要在解決方案中追蹤作業的狀態，請選擇 [**查看作業狀態**]。

## <a name="next-steps"></a>後續步驟

[自訂遠端監視解決方案加速器](../articles/iot-accelerators/iot-accelerators-remote-monitoring-customize.md)一文說明自訂解決方案加速器的一些方式。