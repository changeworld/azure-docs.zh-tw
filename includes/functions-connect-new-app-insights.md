...
1. 選取 [套用]  。 Application Insights 資源會建立在函式應用程式所在的資源群組和訂用帳戶。 建立資源之後，請關閉 Application Insights 視窗。

1. 回到函數應用程式，選取 [**設定**] [設定]  >  ** **，然後選取 [**應用程式設定**]。 如果您看到名為 `APPINSIGHTS_INSTRUMENTATIONKEY` 的設定，表示 Azure 中執行的函式應用程式已啟用 Application Insights 整合。
