...
因應**措施：可**將裝置重新設定為以控制系統不明的狀態運作（在已知的校正參數之外），進而提供可能被誤解的資料
...
詐騙 **/篡改/否認**性：如果未受到保護（消費者遠端控制的情況很少發生），攻擊者就可以匿名操作裝置的狀態。 遙控器就是很好的一例，既可以遙控電視機，也是很熱門的惡作劇工具。
