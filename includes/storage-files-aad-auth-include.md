...
在目錄/檔案層級，Azure 檔案儲存體支援保留、繼承和強制執行 [Windows dacl](/windows/win32/secauthz/access-control-lists) ，就像任何 Windows 檔案伺服器一樣。 您可以選擇在現有的檔案共用與 Azure 檔案共用之間，透過 SMB 複製資料時保留 Windows Dacl。 無論您是否打算強制執行授權，您都可以使用 Azure 檔案共用來備份 Acl 和您的資料。