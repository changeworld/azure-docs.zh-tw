...
Azure 檔案儲存體會在使用者存取共用和目錄/檔案層級時強制執行授權。 共用層級許可權指派可以在透過[角色型存取控制（RBAC）](../articles/role-based-access-control/overview.md)模型管理的 Azure Active Directory （Azure AD）使用者或群組上執行。 使用 RBAC 時，您用來存取檔案的認證應該可以使用或同步處理到 Azure AD。 您可以將儲存體檔案資料 SMB 共用讀取器之類的 Azure 內建角色指派給 Azure AD 中的使用者或群組，以授與 Azure 檔案共用的讀取權限。
