...
在建立時，您可能會想要將其設定為最高的可能值，也就是 32。 請記住，有多個分割區會導致事件傳送至多個分割區，而不會保留順序，除非您將傳送者設定為只傳送至 32 個分割區中的單一分割區，而留下剩餘的 31 個分割區做為備援之用。 在先前的案例中，您必須在所有32分割區上讀取事件。 在後者的情況下，除了您必須在事件處理器主機上進行的額外設定外，沒有明顯的額外成本。
