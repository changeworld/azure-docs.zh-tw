...
ms.locfileid: "97033826"
---
## <a name="prerequisites"></a>必要條件

若要完成本文章中的步驟，您需要遵循使用 **自訂應用程式** 範本建立的 Azure IoT Central 應用程式。 如需詳細資訊，請參閱[建立應用程式快速入門](../articles/iot-central/core/quick-deploy-iot-central.md)。 應用程式必須已在 2020 年 7 月 14 日或之後建立。
...
在您先前下載的 Microsoft Azure IoT SDK for C 複本中，以文字編輯器開啟 azure-iot-sdk-c/iothub_client/samples/pnp/pnp_simple_thermostat/pnp_simple_thermostat.c 檔案。

執行範例以連線到 IoT Central 時，該檔案會使用裝置佈建服務 (DPS) 來註冊裝置並產生連接字串。 此範例會從命令列環境中，取出所需的 DPS 連線資訊。

`main` 函式會先呼叫 `CreateAndConfigureDeviceClientHandleForPnP` 來執行下列動作：

* 設定 `dtmi:com:example:Thermostat;1` 模型識別碼。 IoT Central 會使用型號識別碼來識別或產生此裝置的裝置範本。 若要深入了解，請參閱[將裝置與裝置範本相關聯](../articles/iot-central/core/concepts-get-connected.md#associate-a-device-with-a-device-template)。
* 使用 DPS 來佈建和註冊裝置。
* 建立 `getMaxMinReport` 命令的命令處理常式。
* 建立可寫入 `targetTemperature` 屬性的屬性更新處理常式。
* 建立裝置用戶端控制代碼，並連線到您的 IoT Central 應用程式。

然後 `main` 函式會啟動執行緒，以定期傳送遙測資料。
...
`Thermostat_SendCurrentTemperature` 函式會顯示裝置如何將溫度遙測資料傳送至 IoT Central：
...
`SendMaxTemperatureSinceReboot` 函式會將 `maxTempSinceLastReboot` 屬性更新傳送至 IoT Central：
...
`Thermostat_DeviceTwinCallback` 函式會處理來自 IoT Central 的可寫入屬性更新：
...
`Thermostat_DeviceMethodCallback` 函式會處理從 IoT Central 呼叫的命令：
