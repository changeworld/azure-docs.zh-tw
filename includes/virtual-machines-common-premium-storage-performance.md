...
以下對託管磁片的控制平面操作可能涉及將磁片從一個存儲位置轉移到另一個存儲位置。 這是通過資料後臺副本編排的，可能需要幾個小時才能完成，通常不到 24 小時，具體取決於磁片中的資料量。 在此期間，應用程式可能會遇到比平常更高的讀取延遲，因為某些讀取可以重定向到原始位置，並且可能需要更長的時間才能完成。 在這段期間，對於寫入延遲沒有任何影響。

- 更新存儲類型。
- 將磁片從一個 VM 分離並連接到另一個 VM。
- 從 VHD 創建託管磁片。
- 從快照創建託管磁片。
- 將非託管磁片轉換為託管磁片。
...
如果要測量應用程式的效能需求，最佳方式就是使用伺服器的作業系統所提供的效能監視工具。 您可以使用 Windows 的 PerfMon 和 Linux 的 iostat。 這些工具可擷取與上一節所述的每個量值相對應的計數器。 當應用程式運行其正常、高峰和非工作時間工作負載時，必須捕獲這些計數器的值。

伺服器的處理器、記憶體及每個邏輯磁碟和實體磁碟，都有可用的 PerfMon 計數器。 當您使用進階儲存體磁碟和 VM 時，實體磁碟計數器適用於每個進階儲存體磁碟，邏輯磁碟計數器適用於進階儲存體上建立的每個磁碟區。 您必須為裝載應用程式工作負載的磁碟擷取這些值。 如果邏輯與實體磁碟之間有一對一對應，您可以參考實體磁碟計數器，否則請參考邏輯磁碟計數器。 在 Linux 上，iostat 命令會產生 CPU 和磁碟使用率報告。 磁碟使用率報告提供每個實體裝置或分割區的統計資料。 如果資料庫伺服器的資料和日誌位於單獨的磁片上，則為兩個磁片收集此資料。 下表描述了磁片、處理器和記憶體的計數器：
...
| **麥克斯。記憶** |順暢執行應用程式所需的記憶體數量 |% Committed Bytes in Use |Use vmstat |
| **麥克斯。Cpu** |順暢執行應用程式所需的 CPU 數量 |% Processor time |%util |
...
影響在高級存儲上運行的應用程式性能的主要因素是 IO 請求的性質、VM 大小、磁片大小、磁片數、磁片緩存、多執行緒和佇列深度。 您可以使用系統提供的參數來控制這些因素。 大部分應用程式可能沒有選項讓您直接改變 IO 大小和佇列深度。 例如，如果您使用 SQL Server，您無法選擇 IO 大小和佇列深度。 SQL Server 會選擇最佳 IO 大小和佇列深度值，以獲得最高效能。 務必了解這兩種因素對應用程式效能的影響，才能佈建適當的資源來滿足效能需求。

在本節中，請參閱您建立的應用程式需求檢查清單，以識別您需要將應用程式效能最佳化到何種程度。 據此，您將能夠判斷需要調整本節中的哪些因素。 若要證明每個因素對應用程式效能的影響，請在應用程式安裝上執行效能評定工具。 有關在 Windows 和 Linux VM 上運行常見基準測試控管的步驟，請參閱在末尾連結的基準測試文章。

### <a name="optimize-iops-throughput-and-latency-at-a-glance"></a>一目了然地優化 IOPS、輸送量和延遲

下表總結了性能因素以及優化 IOPS、輸送量和延遲所需的步驟。 這份摘要之後的幾節更深入說明每一個因素。
...
| **VM 和磁碟調整限制** |所選 VM 大小的 IOPS 限制應大於由附加到它的存儲磁片驅動的 IOPS 總數。 |選擇的 VM 大小的輸送量限制，應該大於它連接的進階儲存體磁碟所推動的輸送量總數。 |選擇的 VM 大小的調整限制，必須大於連接的進階儲存體磁碟的調整限制總數。 |
| **磁碟快取** |在讀取作業繁重的進階儲存體磁碟上，啟用唯讀快取可產生較高的讀取 IOPS。 | &nbsp; |在讀取作業繁重的進階儲存體磁碟上，啟用唯讀快取可產生極低的讀取延遲。 |
| **磁碟串接** |使用多個磁碟並串接在一起，可結合產生較高的 IOPS 和輸送量限制。 每個 VM 的組合限制應高於附加高級磁片的組合限制。 | &nbsp; | &nbsp; |
| **條紋大小** |在 OLTP 應用程式中，隨機小型 IO 模式使用較小的等量大小。 例如，對 SQL Server OLTP 應用程式使用 64 KB 的條帶大小。 |在資料倉儲應用程式中，循序大型 IO 模式使用較大的等量大小。 例如，對 SQL Server 資料倉儲應用程式使用 256 KB 條帶大小。 | &nbsp; |
...
IO 要求是指應用程式執行的輸入/輸出作業單位。 識別 IO 要求的本質、隨機或循序、讀取或寫入、小型或大型，可協助您判斷應用程式的效能需求。 瞭解 IO 請求的性質，在設計應用程式基礎結構時做出正確的決策非常重要。 必須均勻分佈 I，以實現最佳性能。

IO 大小是其中一個很重要的因素。 IO 大小是指應用程式所產生的輸入/輸出作業要求的大小。 IO 大小對效能有很重大的影響，特別是應用程式可達到的 IOPS 和頻寬。 以下公式顯示 IOPS、IO 大小和頻寬/輸送量之間的關係。  
...
* OLTP 應用程式會產生數百萬個小型和隨機的 IO 要求。 要處理這些類型的 IO 請求，必須設計應用程式基礎結構以獲得更高的 IOPS。  
* 資料倉儲應用程式會產生大型和循序的 IO 要求。 要處理這些類型的 IO 請求，必須設計應用程式基礎結構以獲得更高的頻寬或輸送量。
...
以下是如何計算應用程式的 IOPS 和輸送量/頻寬的範例。 假設應用程式使用 P30 磁碟。 P30 磁碟可以達到的最大 IOPS 和輸送量/頻寬，分別為 5000 IOPS 和每秒 200 MB。 現在，如果應用程式需要 P30 磁碟的最大 IOPS，但您使用較小的 IO 大小，例如 8 KB，則能夠達到的頻寬為每秒 40 MB。 不過，如果應用程式需要 P30 磁碟的最大輪送量/頻寬，而且您使用較大的 IO 大小，例如 1024 KB，則產生的 IOPS 較小，只有 200 IOPS。 因此，請將 IO 大小調整為同時滿足應用程式的 IOPS 和輸送量/頻寬需求。 下表總結了 P30 磁片的不同 IO 大小及其相應的 IOPS 和輸送量。
...
若要證明 IO 大小對應用程式效能的影響，您可以在 VM 和磁碟上執行效能評定工具。 請建立多個測試回合，而且每個回合使用不同的 IO 大小，以查看影響。 有關詳細資訊，請參閱末尾連結的基準測試文章。

## <a name="high-scale-vm-sizes"></a>高延展性 VM 大小

開始設計應用程式時，首要工作之一是選擇 VM 來裝載應用程式。 進階儲存體提供高延展性 VM 大小，可以執行需要更高計算能力和較高本機磁碟 I/O 效能的應用程式。 這些 VM 為本機磁碟提供更快的處理器、較高的記憶體與核心比率，以及固態硬碟 (SSD)。 支援高級存儲的高級 VM 的示例包括 DS 和 GS 系列 VM。

高規模 VM 提供不同大小的可用，具有不同數量的 CPU 內核、記憶體、作業系統和臨時磁片大小。 每個 VM 大小也規定您可連接至 VM 的資料磁碟數目上限。 因此，選擇的 VM 大小會影響應用程式可用的處理、記憶體和儲存體容量。 也會影響計算和儲存體成本。 例如，以下是 DS 系列和 GS 系列中最大 VM 大小的規範：
...
例如，假設應用程式需要 16,000 IOPS。 要實現此性能，您需要一個標準\_D14 Azure IaaS VM，它可以使用 32 個標準存儲 1 TB 磁片最多提供 16，000 個 IOPS。 每個 1-TB 標準存儲磁片最多可以實現 500 個 IOPS。 此 VM 的每月預估成本為 $1,570。 32 個標準儲存體磁碟的每月成本為 $1,638。 預估每月總成本為 $3,208。
...
| **每月的磁碟成本** |$1，638.40 （32 x 1-TB 磁片） |$544.34 (4 x P30 磁碟) |
...
Azure 高級存儲提供多種大小，因此您可以選擇最適合您需求的大小。 每個磁片大小對 IOPS、頻寬和存儲具有不同的擴展限制。 請根據應用程式需求和高延展性 VM 大小，選擇正確的進階儲存體磁碟大小。 下表顯示了磁片大小及其功能。 目前只有針對「受控磁碟」才支援 P4、P6、P15、P60、P70 和 P80 磁碟大小。
...
> [!WARNING]
> 磁片 4 TiB 和更大磁片不支援磁片緩存。 如果將多個磁片附加到 VM，則每個小於 4 TiB 的磁片將支援緩存。
>
...
*無*  
目前，僅資料磁片上支援**None。** 作業系統磁片不支援它。 如果在 OS 磁片上設置**None，** 它將在內部覆蓋此，並將其設置為**ReadOnly**。
...
下列 Linux 散發套件經過驗證，可使用進階 SSD。 為了借助高級 SSD 獲得更好的性能和穩定性，我們建議您將 VM 升級到這些版本或更高版本之一。 
...
| 散發 | 版本 | 支援的核心 | 詳細資料 |
| --- | --- | --- | --- |
| Ubuntu | 12.04 或更新| 3.2.0-75.110+ | &nbsp; |
| Ubuntu | 14.04 或更新| 3.13.0-44.73+  | &nbsp; |
| Debian | 7.x、8.x 或更新| 3.16.7-ckt4-1+ | &nbsp; |
| SUSE | SLES 12 或更高版本| 3.12.36-38.1+ | &nbsp; |
| SUSE | SLES 11 SP4 或更高版本| 3.0.101-0.63.1+ | &nbsp; |
| CoreOS | 584.0.0+或更新| 3.18.4+ | &nbsp; |
| CentOS | 6.5、6.6、6.7、7.0 或更新| &nbsp; | [需要 LIS4](https://www.microsoft.com/download/details.aspx?id=55106) <br> *請參閱下一節中的附註* |
| CentOS | 7.1+ 或更新| 3.10.0-229.1.2.el7+ | [建議 LIS4](https://www.microsoft.com/download/details.aspx?id=55106) <br> *請參閱下一節中的附註* |
| Red Hat Enterprise Linux (RHEL) | 6.8°、7.2°或更新 | &nbsp; | &nbsp; |
| Oracle | 6.0°、7.2°或更新 | &nbsp; | UEK4 或 RHCK |
| Oracle | 7.0-7.1 或更新 | &nbsp; | UEK4 或 RHCK 帶[LIS4](https://www.microsoft.com/download/details.aspx?id=55106) |
| Oracle | 6.4-6.7 或更新 | &nbsp; | UEK4 或 RHCK 帶[LIS4](https://www.microsoft.com/download/details.aspx?id=55106) |
...
在某些情況下，上述命令也將升級內核。 如果需要內核更新，則可能需要在重新開機後再次運行上述命令，以完整安裝 Microsoft-hyper-v 包。
...
重要事項：您可以使用伺服器管理員 UI，將一個等量磁碟區的總欄數最多設定為 8 個。 附加超過八個磁片時，請使用 PowerShell 創建卷。 您可以使用 PowerShell 將欄數設定為等於磁碟數量。 例如，如果單一等量磁碟區中有 16 個磁碟，請在 *New-VirtualDisk* PowerShell Cmdlet 的 *NumberOfColumns* 參數中指定 16 欄。
...
請根據應用程式執行的工作負載類型，選擇適當的等量大小。 對於隨機小型 IO 要求，請使用較小的等量大小。 而對於大型順序 IO 請求，請使用較大的條帶大小。 針對您要在進階儲存體上執行的應用程式，請參考等量大小建議。 對於 SQL Server，為 OLTP 工作負載配置 64 KB 的條帶大小，為資料倉儲工作負載配置 256 KB 的條帶大小。 若要深入了解，請參閱 [Azure 虛擬機器中的 SQL Server 效能最佳作法](../articles/virtual-machines/windows/sql/virtual-machines-windows-sql-performance.md#disks-guidance) 。
...
佇列深度或佇列長度或佇列大小是系統中掛起的 IO 請求數。 佇列深度的值決定了應用程式可以排隊的 IO 運算元，存儲磁片將處理這些操作。 它會影響本文中討論的所有三個應用程式性能指標，即 iOPS、輸送量和延遲。
...
通常，當每個連接的磁碟上有 8-16+ 個未完成的 IO 時，應用程式可以達到最大輸送量。 如果佇列深度為 1，則應用程式不會將足夠的 I 推送到系統中，並且它將在給定時間段內處理較少的 I。 換句話說，輸送量較少。

例如，在 SQL Server 中，將查詢的 MAXDOP 值設為 "4"，會向 SQL Server 表示它最多可以使用四個核心來執行查詢。 SQL Server 會決定查詢執行的最佳佇列深度值和核心數目。

*最佳佇列深度*  
 佇列深度值太高也有其缺點。 如果佇列深度值太高，應用程式會嘗試推動非常高的 IOPS。 除非應用程式的永續性磁碟已佈建足夠的 IOPS，否則這可能對應用程式延遲造成負面影響。 下面的公式顯示 IOPS、延遲和佇列深度之間的關係。  
...
*等量磁碟區的佇列深度*  
 對於等量磁碟區，應該維持夠高的佇列深度，讓每個磁碟有各自的尖峰佇列深度。 例如，考慮將佇列深度推 2 的應用程式，並且條帶中有四個磁片。 兩個 IO 要求會流向兩個磁碟，而剩餘的兩個磁碟會處於閒置狀態。 因此，請將佇列深度設定為讓所有磁碟都有工作執行。 下列公式顯示如何決定等量磁碟區的佇列深度。  
    ![](media/premium-storage-performance/image7.png)

## <a name="throttling"></a>節流

Azure 進階儲存體會根據您選擇的 VM 大小和磁碟大小，佈建指定數目的 IOPS 和輸送量。 每當應用程式嘗試推動的 IOPS 或輸送量超過 VM 或磁碟可處理的這些限制時，進階儲存體便會進行節流。 這會造成應用程式的效能降低。 這可能意味著更高的延遲、更低的輸送量或更低的 IOPS。 如果進階儲存體不會節流，應用程式可能會超出其資源的能力負荷而完全失敗。 因此，為了避免因為節流而造成效能問題，一定要為應用程式佈建足夠的資源。 請考量先前在 VM 大小和磁碟大小各節已討論過的因素。 為了駕馭應用程式，效能評定是找出所需資源的最佳方式。
