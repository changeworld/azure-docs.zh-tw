...
| **Latency** |完成磁碟 IO 要求的總時間。 |Average Disk sec/Read  <br>  Average disk sec/Write |await  <br>  svctm |
...
| **讀數.快閃記憶體** |順暢執行應用程式所需的記憶體數量 |% Committed Bytes in Use |Use vmstat |
| **讀數.使用率** |順暢執行應用程式所需的 CPU 數量 |% Processor time |%util |
...
採用 Azure 進階儲存體的高延展性 VM 有一項稱為 BlobCache 的多層快取技術。 BlobCache 使用主機 RAM 和本機 SSD 的組合來進行快取。 此快取適用於進階儲存體永續性磁碟和 VM 本機磁碟。 根據預設，針對作業系統磁碟，此快取設定會設為讀取/寫入，而針對裝載於進階儲存體的資料磁碟，則設為唯讀。 在進階儲存體上啟用磁碟快取時，高延展性 VM 可以達到極高的效能層級，超過基礎磁碟效能。
...
| CentOS | 6.5、6.6、6.7、7.0 或更新版本| &nbsp; | [需要 LIS4 ()](https://www.microsoft.com/download/details.aspx?id=55106) <br> *請參閱下一節中的附註* |
| CentOS | 7.1 + 或更新版本| 3.10.0-229.1.2.el7+ | [建議使用 LIS4 ()](https://www.microsoft.com/download/details.aspx?id=55106) <br> *請參閱下一節中的附註* |
