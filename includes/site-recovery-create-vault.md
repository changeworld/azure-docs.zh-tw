...
ms.locfileid: "87495962"
---
1. 登入 [Azure 入口網站](https://portal.azure.com) > [復原服務]。
2. 按一下 [建立群組] > [監視和管理] > [備份和 Site Recovery]。
3. 在 [名稱] 中，指定保存庫的易記識別名稱。 如果您有多個訂用帳戶，請選取適當的一個。
4. [建立資源群組](../articles/azure-resource-manager/templates/deploy-portal.md)，或選取現有的資源群組。 指定 Azure 區域。 
5. 若要從儀表板快速存取保存庫，請按一下 [釘選到儀表板] > [建立]。

   ![復原服務保存庫建立選項的螢幕擷取畫面。](./media/site-recovery-create-vault/new-vault-settings.png)
