...
1. 讓 Azure Active Directory 應用程式與 Azure 時間序列深入解析建立關聯。 選取 [ **API 許可權** > ] [新增我的組織所使用**的許可權** > **api**]。 

    [![將 API 與您的 Azure Active Directory 應用程式建立關聯](media/time-series-insights-aad-registration/active-directory-app-api-permission.png)](media/time-series-insights-aad-registration/active-directory-app-api-permission.png#lightbox)
...
1. 接下來，請指定應用程式所需的 API 權限類型。 根據預設，系統會醒目提示 [委派的權限]****。 選擇某個權限類型，然後選取 [新增權限]****。
    ...
    [![指定您的應用程式所需的 API 許可權類型](media/time-series-insights-aad-registration/active-directory-app-permission-grant.png)](media/time-series-insights-aad-registration/active-directory-app-permission-grant.png#lightbox)
