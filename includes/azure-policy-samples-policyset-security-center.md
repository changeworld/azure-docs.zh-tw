---
author: DCtheGeek
ms.service: azure-policy
ms.topic: include
ms.date: 04/02/2020
ms.author: dacoulte
ms.openlocfilehash: 9112c934213c3765547dfb9ede3494e580c27272
ms.sourcegitcommit: bc738d2986f9d9601921baf9dded778853489b16
ms.translationtype: HT
ms.contentlocale: zh-TW
ms.lasthandoff: 04/02/2020
ms.locfileid: "80624026"
---
|名稱 |描述 |原則 |版本 |
|---|---|---|---|
|[\[預覽\]：啟用資料保護套件](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policySetDefinitions/Security%20Center/ASC_DataProtection.json) |為 SQL 伺服器啟用資料保護。 這項動作由 Azure 資訊安全中心標準層自動指派。 |1 |1.0.0-preview |
|[啟用 Azure 資訊安全中心的監視功能](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policySetDefinitions/Security%20Center/AzureSecurityCenter.json) |監視 Azure 資訊安全中心的所有可用安全性建議。 這是 Azure 資訊安全中心的預設原則。 |96 |2.0.1 |
