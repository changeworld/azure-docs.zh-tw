...
防禦主機是選擇性的元件，您可以使用它做為存取應用程式和資料庫實例的跳躍伺服器。 防禦主機 VM 可以有指派的公用 IP 位址，雖然建議是使用內部部署網路來設定 ExpressRoute 連線或站對站 VPN，以進行安全存取。 此外，只有 SSH (埠22、Linux) 或 RDP (埠3389，應針對連入流量開啟 Windows Server) 。 如需高可用性，請在兩個可用性區域或單一可用性設定組中部署防禦主機。
