...
3. 使用下列命令並取代預留位置，以建立新的 Azure Active Directory 應用程式：
   ...
   * **{首頁 url}：** 您應用程式首頁的 url，例如 **HTTP： \/ /mysampleapp/home**。 此 URL 不需要指向實際的應用程式。
   * **{Application identifier}：** 唯一的識別碼，例如 **HTTP： \/ /mysampleapp**。 此 URL 不需要指向實際的應用程式。
