...
若要立即同步處理 Azure 檔案共用中變更的檔案，您可以使用 **Invoke-AzStorageSyncChangeDetection** PowerShell Cmdlet，手動在 Azure 檔案共用中起始變更偵測。 此 Cmdlet 是用於在 Azure 檔案共用中以自動化程序進行變更，或是由系統管理員完成變更的案例 (例如將檔案和目錄移到共用中)。 針對使用者變更，建議您在 IaaS VM 中安裝 Azure 檔案同步代理程式，並讓終端使用者透過 IaaS VM 存取檔案共用。 如此一來，所有變更都會快速同步處理至其他代理程式，而不需要使用 Invoke-AzStorageSyncChangeDetection Cmdlet。 若要深入瞭解，請參閱 [Invoke-AzStorageSyncChangeDetection](/powershell/module/az.storagesync/invoke-azstoragesyncchangedetection) 文件。
