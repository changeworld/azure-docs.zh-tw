...
ms.locfileid: "86502501"
---
Azure 共用磁片是 Azure 受控磁片的新功能，可讓您同時將受控磁片連結至多部虛擬機器（Vm）。 Attaching a managed disk to multiple VMs allows you to either deploy new or migrate existing clustered applications to Azure.

## <a name="how-it-works"></a>運作方式

叢集中的 Vm 可以根據使用[SCSI 持續保留](https://www.t10.org/members/w_spc3.htm)（scsi PR）的叢集應用程式所選擇的保留區，讀取或寫入其連接的磁片。 SCSI PR 是一種業界標準，可供在內部部署的存放區域網路 (SAN) 上執行的應用程式使用。 在受控磁碟上啟用 SCSI PR 可讓您依情況將這些應用程式移轉至 Azure。

共用的受控磁片提供可從多個 Vm 存取的共用區塊存放裝置，這些儲存體會公開為邏輯單元編號（Lun）。 然後，LUN 會從目標 (磁碟) 呈現給啟動器 (VM)。 這些 LUN 看起來像是直接連接存放裝置 (DAS) 或 VM 的本機磁碟磁碟機。

共用的受控磁碟本身並不提供完全受控的檔案系統，可以使用 SMB/NFS 來存取。 您需要使用叢集管理員（例如 Windows Server 容錯移轉叢集（WSFC）或 Pacemaker）來處理叢集節點通訊和寫入鎖定。
...
共用磁片支援數個作業系統。 請參閱[Windows](#windows)或[Linux](#linux)小節以取得支援的作業系統。
...
Windows Server 2008 和更新版本支援 Azure 共用磁片。 大部分以 Windows 為基礎的叢集都建置於 WSFC 上，其會處理叢集節點通訊的所有核心基礎結構，讓您的應用程式能夠利用平行存取模式。 WSFC 會根據您的 Windows Server 版本，啟用 CSV 和非 CSV 的選項。 如需詳細資訊，請參閱[建立容錯移轉叢集](https://docs.microsoft.com/windows-server/failover-clustering/create-failover-cluster)。

WSFC 上執行的一些熱門應用程式包括：

- [使用 Azure 共用磁片建立 FCI （在 Azure Vm 上 SQL Server）](../articles/azure-sql/virtual-machines/windows/failover-cluster-instance-azure-shared-disks-manually-configure.md)
- 向外延展檔案伺服器（SoFS） [範本] （https://aka.ms/azure-shared-disk-sofs-template)
...
支援 Azure 共用磁片：
- [SUSE SLE for SAP 和 SUSE SLE HA 15 SP1 和更新版本](https://documentation.suse.com/sle-ha/15-SP1/single-html/SLE-HA-guide/index.html)
...
- [任何 RHEL 8 版本上的 RHEL developer preview](https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/8/html/configuring_and_managing_high_availability_clusters/index)
- [Oracle Enterprise Linux](https://docs.oracle.com/en/operating-systems/oracle-linux/8/availability/hacluster-1.html)

Linux 叢集可以運用叢集管理員，例如 [Pacemaker](https://wiki.clusterlabs.org/wiki/Pacemaker)。 Pacemaker 是以 [Corosync](http://corosync.github.io/corosync/) 為基礎，可針對部署在高可用性環境中的應用程式啟用叢集通訊。 一些常見的叢集檔案系統包括 [ocfs2](https://oss.oracle.com/projects/ocfs2/) 和 [gfs2](https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/7/html/global_file_system_2/ch-overview-gfs2)。 您可以使用 SCSI 持續保留（SCSI PR）和（或）以 STONITH 封鎖裝置（SBD）為基礎的叢集模型來仲裁磁碟的存取權。 使用 SCSI PR 時，您可以使用[fence_scsi](http://manpages.ubuntu.com/manpages/eoan/man8/fence_scsi.8.html)和[sg_persist](https://linux.die.net/man/8/sg_persist)等公用程式來操作保留和註冊。
...
:::image type="content" source="media/virtual-machines-disks-shared-disks/ultra-reservation-table.png" alt-text="資料表的影像，描述保留持有者、已註冊和其他的「ReadOnly」或「讀取/寫入」存取權。":::

## <a name="performance-throttles"></a>效能節流

### <a name="premium-ssd-performance-throttles"></a>進階 SSD 效能節流

使用 premium SSD 時，磁片 IOPS 和輸送量是固定的，例如，P30 的 IOPS 為5000。 此值會維持在2部 Vm 或5個 Vm 之間是否共用磁片。 磁片限制可以從單一 VM 連線，或分散到兩個以上的 Vm。 
...
|DiskIOPSReadOnly*     |將共用磁片掛接為的所有 Vm 允許的 IOPS 總數 `ReadOnly` 。         |
|DiskMBpsReadOnly*     |所有掛接共用磁片的 Vm 所允許的總輸送量（MB/s）為 `ReadOnly` 。         |
...
以下是使用叢集共用磁碟區的 2 節點 WSFC 範例。 使用此設定時，這兩個 Vm 會同時對磁片進行寫入存取，這會導致在 `ReadWrite` 兩個 vm 之間分割節流，而 `ReadOnly` 不會使用節流。
...
以下是未使用叢集共用磁碟區的 2 節點 WSFC 範例。 使用此設定時，只有一個 VM 具有磁碟的寫入存取權。 這會導致 `ReadWrite` 僅針對主要 VM 使用節流，而 `ReadOnly` 僅供次要資料庫使用。
...
以下範例是具有單一寫入器和三個向外延展讀取器的 4 節點 Linux 叢集。 使用此設定時，只有一個 VM 具有磁碟的寫入存取權。 這會導致 `ReadWrite` 節流僅用於主要 vm，而節流是 `ReadOnly` 由次要 vm 分割。
...
Ultra 共用磁片是根據布建的容量、已布建的 IOPS （diskIOPSReadWrite + diskIOPSReadOnly）和總布建輸送量 MBps （diskMBpsReadWrite + diskMBpsReadOnly）來計價。 每個額外的 VM 掛接不會額外收費。 例如，具有下列設定的 ultra 共用磁片（diskSizeGB：1024、DiskIOPSReadWrite：10000、DiskMBpsReadWrite：600、DiskIOPSReadOnly：100、DiskMBpsReadOnly：1）會以 1024 GiB、10100 IOPS 和 601 MBps 計費，不論其是否掛接至兩部 Vm 或五部 Vm。