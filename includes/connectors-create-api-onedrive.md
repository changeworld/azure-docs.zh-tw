...
您必須先授權 Logic Apps 連接到 Azure 入口網站中的 OneDrive 帳戶，才能使用您的 OneDrive 帳戶搭配 Logic Apps。

請遵循下列步驟來授權 Logic Apps 連接到您的 OneDrive 帳戶：  
...
1. 在 Logic Apps 設計工具中，從下拉式清單中選取 [**顯示 Microsoft 受控 api** ]，然後在搜尋方塊中輸入*OneDrive* 。 選取要使用的觸發程式或動作：

   ![Logic Apps 設計工具的螢幕擷取畫面，其中顯示要新增的 OneDrive API 動作清單。](./media/connectors-create-api-onedrive/onedrive-1.png)

2. 如果您先前尚未建立任何 OneDrive 連線，請遵循提示以使用您的 OneDrive 認證登入：  
...
3. 選取 [登入]****，然後輸入您的使用者名稱和密碼。 選取 [登**入**]： 
   ...
   ![OneDrive API 授權 Microsoft 帳戶登入頁面的螢幕擷取畫面。](./media/connectors-create-api-onedrive/onedrive-3.png)   
    ...
    這些認證會用來授權邏輯應用程式存取您 OneDrive 帳戶中的資料。 
...
5. 此連接現在會列在步驟中。 選取 [儲存]，然後繼續建立邏輯應用程式。 
   ...
   ![Logic Apps 設計工具的螢幕擷取畫面，其中顯示 [動作編輯器] 與 [OneDrive API 連線]。](./media/connectors-create-api-onedrive/onedrive-5.png)
