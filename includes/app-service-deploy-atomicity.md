...
所有正式支援的部署方法都會對應用程式`/home/site/wwwroot`資料夾中的檔案進行變更。 這些檔案是用來執行您的應用程式。 因此，部署可能會因為鎖定的檔案而失敗。 應用程式在部署期間可能也會無法預期的行為，因為並非所有檔案都同時更新。 對於客戶面向的應用程式，這是不必要的。 有幾種不同的方式可以避免這些問題：
