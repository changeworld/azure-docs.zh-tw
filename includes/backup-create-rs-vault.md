...
1. 提供值之後，選取 [檢閱 + 建立]。
    ...
    ![顯示在建立「復原服務保存庫」流程中的 [審核 + 建立] 按鈕的螢幕擷取畫面。](./media/backup-create-rs-vault/review-and-create.png)
...
>[!IMPORTANT]
> 強烈建議您先檢閱 [儲存體複寫類型] 和 [安全性設定] 的預設設定，再於保存庫中設定備份。 如需詳細資訊，請參閱[設定儲存體備援](../articles/backup/backup-create-rs-vault.md#set-storage-redundancy)一節。