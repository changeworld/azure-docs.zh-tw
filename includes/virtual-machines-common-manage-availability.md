...
<!-- Link references -->
[在可用性設定組中設定多部虛擬機器以進行冗余]: #configure-multiple-virtual-machines-in-an-availability-set-for-redundancy
[將每個應用層設定為不同的可用性設定組]: #configure-each-application-tier-into-separate-availability-zones-or-availability-sets
[將 Load Balancer 與可用性設定組結合]: #combine-a-load-balancer-with-availability-zones-or-sets
