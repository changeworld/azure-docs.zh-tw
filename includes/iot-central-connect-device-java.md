...
* JAVA SE 開發套件 8 的開發電腦。 在 [Azure 和 Azure Stack 的 Java 長期支援](/java/azure/jdk/?preserve-view=true&view=azure-java-stable)中，選取 [長期支援] 下的 [Java 8]。
* [Apache Maven 3](https://maven.apache.org/download.cgi)。
* [Microsoft Azure IoT SDK for JAVA](https://github.com/Azure/azure-iot-sdk-java) GitHub 存放庫的本機複本，其中包含範例程式碼。 使用此連結來下載存放庫的複本：[下載 ZIP](https://github.com/Azure/azure-iot-sdk-java/archive/master.zip)。 接著將檔案解壓縮到本機電腦上的適當位置。

## <a name="review-the-code"></a>檢閱程式碼

在您先前下載的 Microsoft Azure IoT SDK for JAVA 複本中，以文字編輯器開啟 azure-iot-sdk-java/device/iot-device-samples/pnp-device-sample/thermostat-device-sample/src/main/java/samples/com/microsoft/azure/sdk/iot/device/Thermostat.java 檔案。
...
`sendTemperatureTelemetry` 方法會顯示裝置如何將溫度遙測資料傳送至 IoT Central：
...
`updateMaxTemperatureSinceLastReboot` 方法會將 `maxTempSinceLastReboot` 屬性更新傳送至 IoT Central：
...
`TargetTemperatureUpdateCallback` 類別包含 `TwinPropertyCallBack` 方法，可處理來自 IoT Central 的可寫入屬性更新：
...
`GetMaxMinReportMethodCallback` 類別包含 `call` 方法，可處理從 IoT Central 呼叫的命令：
...
在 Windows 上，瀏覽至所下載之 Java SDK 存放庫的根資料夾。
...
若要執行範例應用程式，請開啟命令列環境，然後瀏覽至 azure-iot-sdk-java/device/iot-device-samples/pnp-device-sample/thermostat-device-sample 資料夾，其中包含 Thermostat.java 範例檔案。
