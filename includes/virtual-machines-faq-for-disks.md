...
**如果我創建一個 128 GB 磁碟,然後將大小增加到 130 GB (GiB),是否會為下一個磁碟大小 (256 GiB)收費?**
...
第 1 代映射只能在數據磁碟上使用 GPT 分區,而不能在 OS 磁碟上使用分區。 OS 磁碟必須使用 MBR 分割區樣式。

[第 2 代映射](https://docs.microsoft.com/azure/virtual-machines/linux/generation-2)可以在作業系統磁碟和數據磁碟上使用 GPT 分區。

**哪些磁碟類型支援快照?**

高級 SSD、標準 SSD 和標準硬碟支援快照。 對於這三種磁碟類型,所有磁碟大小(包括大小高達 32 TiB 的磁碟)都支援快照。 超級磁碟不支援快照。

**什麼是 Azure 磁碟預留?**
磁碟預留是提前購買一年磁碟存儲的選項,從而降低了您的總成本。 有關 Azure 磁碟預留的詳細資訊,請參閱我們有關主題的文章:[瞭解預訂折扣如何應用於 Azure 磁碟](../articles/cost-management-billing/reservations/understand-disk-reservations.md)。

**Azure 磁碟預留提供哪些選項?**    
Azure 磁碟預留提供了在指定的 SKU 中購買進階 SSD 的選項,從 P30 (1 TiB) 到 P80 (32 TiB),期限為一年。 購買磁碟預留所需的最小磁碟數量沒有限制。 此外,您可以選擇使用單筆預付款或每月付款付款。 高級 SSD 託管磁碟沒有應用額外的事務成本。    

預訂以磁碟形式進行,而不是以容量形式進行。 換句話說,當您保留 P80 (32 TiB) 磁碟時,您會得到一個 P80 磁碟,因此無法將該特定預留劃分為兩個較小的 P70 (16 TiB) 磁碟。 當然,您可以保留您喜歡的磁碟數量或數量之少,包括兩個單獨的 P70 (16 TiB) 磁碟。

**如何應用 Azure 磁碟預留?**    
磁碟預留遵循類似於預留虛擬機器 (VM) 實體的模型。 區別在於磁碟預留不能應用於不同的 SKU,而 VM 實例可以。 有關 VM 實例的詳細資訊[,請參閱使用 Azure 預留 VM 實例節省成本](../articles/virtual-machines/linux/prepay-reserved-vm-instances.md)。     

**我能否使用通過 Azure 磁碟預留跨多個區域購買的數據存儲?**    
Azure 磁碟預留是為特定區域和 SKU(如美國東部 2 中的 P30)購買的,因此不能在這些構造之外使用。 您始終可以購買其他 Azure 磁碟預留,以滿足其他區域或 SKU 中的磁碟存儲需求。    

**Azure 磁碟保留到期時會發生什麼情況?**    
您將在到期前 30 天收到電子郵件通知,並在到期日期再次收到通知。 一旦預訂到期,部署的磁盤將繼續運行,並將用最新的即[用即付費率](https://azure.microsoft.com/pricing/details/managed-disks/)計費。

### <a name="azure-shared-disks"></a>Azure 分享磁碟

**對於非託管磁碟或頁面 Blob,是否支援共用磁碟功能?**
...
只有 P15 或更高支援的先進 SSD 支援共享磁碟。

**如果我有現有的高級 SSD,是否可以啟用共用磁碟?**

使用 API 版本 2019-07-01 或更高版本創建的所有託管磁碟都可以啟用共用磁碟。 為此,您需要從磁碟附加到的所有 VM 中卸載磁碟。 接下來,`maxShares`編輯磁碟上的屬性。

**如果我不想再在共用模式下使用磁碟,如何禁用它?**

從磁碟附加到的所有 VM 中卸載磁碟。 然後,將磁碟上的 maxShare 屬性編輯為 1。

**您可以調整共用磁碟的大小嗎?**

是。

**是否可以在同時啟用共用磁碟的磁碟上啟用寫入加速器?**

否。

**是否可以為已啟用共用磁碟的磁碟啟用主機快取?**
...
**我應該將超磁碟輸送量設置為什麼?**
如果您不確定要將磁碟輸送量設置為什麼,我們建議您首先假定 IO 大小為 16 KiB,並在監視應用程式時從該大小調整性能。 公式為:MBps 中的輸送量 = IOPS 的 + 16 / 1000。

**我把我的磁碟配置到 40000 IOPS,但我只看到 12800 IOPS,為什麼我看不到磁碟的性能?**
除了磁碟限制之外,還有一個在 VM 級別施加的 IO 限制。 請確保正在使用的 VM 大小可以支援磁碟上配置的級別。 有關 VM 施加的 IO 限制的詳細資訊,請參閱[Azure 中 Windows 虛擬機器的大小](../articles/virtual-machines/windows/sizes.md)。

**我可以使用超磁碟的緩存級別嗎?**
不,超磁碟不支援其他磁碟類型支援的不同緩存方法。 將磁碟緩存設置為"無"

**我可以將超磁碟附加到現有 VM 嗎?**
也許,您的 VM 必須位於支援 Ultra 磁碟的區域和可用性區域對中。 有關詳細資訊[,請參閱開始使用超磁碟](../articles/virtual-machines/windows/disks-enable-ultra-ssd.md)。

**我是否可以將超磁碟用作 VM 的 OS 磁碟?**
不,超級磁碟僅作為數據磁碟支援,僅作為 4K 本機磁碟受支援。

**我可以將現有磁碟轉換為超磁碟嗎?**
否,但您可以將數據從現有磁碟遷移到超磁碟。 要將現有磁碟遷移到超級磁碟,請將兩個磁碟連接到同一 VM,並將磁碟的數據從一個磁碟複製到另一個磁碟,或者利用第三方解決方案進行數據遷移。
...
**我可以將超磁碟連接到在可用性集中運行的 VM 嗎?**
否,這不受支援。

**是否可以使用超磁碟為 VM 啟用 Azure 網站恢復?**
否,超級磁碟尚不支援 Azure 網站恢復。

## <a name="uploading-to-a-managed-disk"></a>上傳到託管磁碟

**我可以將數據上載到現有託管磁碟嗎?**

否,上載只能在創建具有**ReadyToUpload**狀態的新空磁碟期間使用。

**如何上傳到託管磁碟?**

創建具有[createOption](https://docs.microsoft.com/rest/api/compute/disks/createorupdate#diskcreateoption)屬性的託管磁碟[,該](https://docs.microsoft.com/rest/api/compute/disks/createorupdate#creationdata)屬性設置為"上傳",然後您可以將資料上載到該磁碟。

**磁碟處於上載狀態時,是否可以將其連接到 VM?**

否。

**我可以拍攝處於上載狀態的已滿磁碟的快照嗎?**
...
**如何建立標準 SSD 磁碟？**
您可以使用 Azure 資源管理器樣本、SDK、PowerShell 或 CLI 創建標準 SSD 磁碟。 以下是在 Resource Manager 範本中建立標準 SSD 磁碟所需的參數：
...
與 HDD 磁碟相比,標準 SSD 磁碟可提供更好的延遲、一致性、可用性和可靠性。 因此，應用程式工作負載在「標準 SSD」上執行起來更加順暢。 請注意，針對大多數 IO 密集型生產環境工作負載，建議的解決方案是「進階 SSD」磁碟。
...
移轉涉及從一個儲存體位置將磁碟移動到另一個儲存體位置。 這是通過數據後台副本編排的,這可能需要幾個小時才能完成,通常小於 24 Hrs,具體取決於磁盤中的數據量。 在這段期間，應用程式可能出現比平常高的讀取延遲，因為某些讀取會重新導向至原始位置，因此可能需要較長的時間才能完成。 在這段期間，對於寫入延遲沒有任何影響。  
...
否。 Azure 網站恢復 Azure 到 Azure 保護的 VM 具有託管磁碟可用。
...
**哪些區域支持針對適用的優質 SSD 磁碟大小的爆破功能?**

Azure 公共雲中的所有區域當前都支援爆破功能,對主權雲的支援即將推出。 

**哪些區域支援 4/8/16 GiB 託管磁碟大小(P1/P2/P3、E1/E2/E3)?**

Azure 公共雲中的所有區域當前都支援這些新磁碟大小,並且將很快支持主權雲。 

**是否支援非託管磁碟或頁面 blob 的 P1/P2/P3 磁碟大小?**

否,它僅在高級 SSD 託管磁碟上受支援。 

**是否支援非託管磁碟或頁面 blob 的 E1/E2/E3 磁碟大小?**

否,任何大小的標準 SSD 託管磁碟不能與非託管磁碟或頁面 blob 一起使用。
...
**能否將現有託管磁碟的大小從小於 4 位元組 (TiB) 調整到新引入的磁碟大小,最多 32 TiB?**

是。

**Azure 備份和 Azure 網站恢復服務支援的最大磁碟大小是什麼?**

Azure 備份支援的最大磁碟大小是 32 TiB(加密磁碟為 4 TiB)。 Azure 網站恢復支援的最大磁碟大小為 8 TiB。 Azure 網站恢復中尚未提供對最多 32 個 TiB 的較大磁碟的支援。

**對於更大的磁碟大小(>4 TiB),標準 SSD 和標準硬碟磁碟可實現優化的磁碟 IOPS 和頻寬,建議使用哪些 VM 大小?**

為了實現標準 SSD 和標準硬碟大型磁碟(>4 TiB)超過 500 IOPS 和 60 MiB/s 的磁碟輸送量, 我們建議您從以下 VM 大小之一部署新的 VM 以優化性能:B 系列、DSv2 系列、Dsv3 系列、ESv3 系列、Fs 系列、Fsv2 系列、M 系列、GS 系列、NCv2 系列、NCv3 系列或 Ls 系列 VM。 將大型磁碟連接到不使用上述建議大小的現有 VM 或 VM 可能會降低性能。

**如何升級在較大磁碟大小預覽期間部署的磁碟(>4 TiB),以便在 GA 獲得更高的 IOPS &带宽?**

您可以停止並啟動磁碟連接到的 VM,也可以分離並重新連接磁碟。 在 GA 中,進階 SSD 和標準 SSD 的性能目標都有所增加。

**支援 8 TiB、16 TiB 和 32 TiB 的託管磁碟大小?**

在全域 Azure、Microsoft Azure 政府和 Azure 中國 21Vianet 下的所有區域都支援 8 TiB、16 TiB 和 32 TiB 磁碟 SKU。

**我們是否支援在所有磁碟大小上啟用主機快取?**

我們支援主機緩存的唯讀和讀取/寫入磁碟大小小於 4 TiB。 對於超過 4 TiB 的磁碟大小,我們不支援設置緩存選項,而不是 None。 建議您利用快取較小型磁碟大小的方式，以便您透過快取到 VM 的資料增進效能。
