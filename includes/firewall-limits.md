...
ms.locfileid: "83673521"
---
| 資源 | 限制 |
| --- | --- |
| 資料輸送量 |30 Gbps<sup>1</sup> |
|規則|10,000。 結合所有規則類型。|
|DNAT 規則數上限|298<br>如果同時針對 TCP 和 UDP 設定規則的通訊協定，則會將其算成兩個規則。|
...
|IP 群組中的 IP 位址|50 個 IP 群組或更少：每個防火牆執行個體最多 5000 個個別 IP 位址。<br>51 - 100 個 IP 群組：每個防火牆執行個體 500 個個別 IP 位址。<br><br>如需詳細資訊，請參閱 [Azure 防火牆中的 IP 群組 (預覽)](../articles/firewall/ip-groups.md#ip-address-limits)
