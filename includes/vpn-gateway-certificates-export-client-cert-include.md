...
1. 若要匯出用戶端憑證，請開啟 [管理使用者憑證]。 根據預設，您產生的用戶端憑證位於 'Certificates - Current User\Personal\Certificates'。 以滑鼠右鍵按一下您要匯出的用戶端憑證，按一下 [ **所有** 工作]，然後按一下 [ **匯出** ] 以開啟 [ **憑證匯出嚮導]** 。
   ...
   ![螢幕擷取畫面：顯示目前使用者的 [憑證] 視窗，其中選取了 [憑證] 和 [從所有工作選取匯出]。](./media/vpn-gateway-certificates-export-client-cert-include/export.png)
...
7. 按一下 [完成] 匯出憑證。
   ...
   ![螢幕擷取畫面顯示具有所輸入設定的憑證匯出 Wizard。](./media/vpn-gateway-certificates-export-client-cert-include/finish.png)