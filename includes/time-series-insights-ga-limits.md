...
Gen1 屬性限制取決於所選的 SKU 環境。 提供的事件屬性具有對應的 JSON、CSV 和圖表資料行，可在 [Azure 時間序列深入解析 Explorer](../articles/time-series-insights/time-series-quickstart.md)內查看。
...
* 瞭解如何 [新增事件中樞來源](../articles/time-series-insights/how-to-ingest-data-event-hub.md)。
* 設定 [IoT 中樞來源](../articles/time-series-insights/how-to-ingest-data-iot-hub.md)。

### <a name="api-limits"></a>API 限制

REST API Azure 時間序列深入解析 Gen1 的限制會在 [REST API 參考檔](/rest/api/time-series-insights/dataaccess(preview)/query/getavailability)中指定。