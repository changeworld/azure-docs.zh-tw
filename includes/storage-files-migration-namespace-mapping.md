...
同步處理磁片區的根目錄不一定是最佳答案。 同步多個位置有一些優點。 例如，這麼做有助於讓每個同步範圍的專案數目保持在較低的範圍。 雖然我們會測試 Azure 檔案共用，並在每個共用 () 的檔案和資料夾 100000000 Azure 檔案同步，但最佳做法是嘗試將低於20或30000000的數位保留在單一共用中。 使用較少的專案來設定 Azure 檔案同步不只對檔案同步有説明。較少的專案也會受益于下列案例：

* 在命名空間開始出現在 Azure 檔案同步啟用的伺服器上之前，雲端內容的初始掃描可能會更快完成。
* 從 Azure 檔案共用快照集進行雲端端還原的速度將會比較快。
* 內部部署伺服器的災難復原可以大幅提高速度。
* 直接在 Azure 檔案共用中進行的變更 (外部同步) 可以更快地偵測和同步。

> [!TIP]
> 如果您不確定有多少檔案和資料夾，可以查看 TreeSize 工具免于卡在軟體 GmbH。
...
最佳做法是讓每個同步處理範圍的專案數目保持低。 這是您將資料夾對應至 Azure 檔案共用時要考慮的重要因素。 雖然我們會測試 Azure 檔案共用，並在每個共用 () 的檔案和資料夾 100000000 Azure 檔案同步，但最佳做法是嘗試將低於20或30000000的數位保留在單一共用中。 如果您開始超過這些數位，請將您的命名空間分割成多個共用。 您可以繼續將多個內部內部部署的共用群組至相同的 Azure 檔案共用，只要您大致低於這些數位即可。 這可提供您成長的空間。
